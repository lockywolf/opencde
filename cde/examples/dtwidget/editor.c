...
typedef enum _LoadSaveType { LOAD_DATA, INSERT_DATA, SAVE_DATA } LoadSaveType;
...
static Widget CreateToggle(Widget, String, char, XtCallbackProc, XtPointer,
                           Boolean);
...
void main(int argc, char **argv) {
        ...
        XmMainWindowSetAreas(ad.mainWindow, ad.menuBar, NULL, NULL, NULL,
                             ad.editor);
...
static void OpenFileCb(Widget w, XtPointer cd, XtPointer cb) {
        ...
        name = XmStringUnparse(fcb->value, NULL, XmCHARSET_TEXT, XmCHARSET_TEXT,
                               NULL, 0, XmOUTPUT_ALL);
        ...
        case DtEDITOR_NULLS_REMOVED: {
                DisplayMessage(
                    "All embedded null characters removed from the file");
                break;
        }
...
static void SaveAsFileCb(Widget w, XtPointer cd, XtPointer cb) {
        ...
        DtEditorErrorCode error;
        Boolean overWrite = False, hardCarriageReturns = True,
...
static void DisplayFsbCb(Widget w, XtPointer cd, XtPointer cb) {
        ...
        if (ad.fileSelectionBox == (Widget)NULL) {
                ac = 0;
                ad.fileSelectionBox = XmCreateFileSelectionDialog(
                    ad.mainWindow, "file_sel_dlg", al, ac);
                XtAddCallback(ad.fileSelectionBox, XmNcancelCallback,
                              CancelOpenCb, (XtPointer)LoadSaveFlag);
        }
        ...
        XtVaSetValues(ad.fileSelectionBox, XmNdialogTitle, tmpStr1,
                      XmNselectionLabelString, tmpStr1, NULL);
...
static void ExitCb(Widget w, XtPointer cd, XtPointer cb) { exit(0); }
...
static Widget CreateEditMenu(Widget menuBar) {
        ...
        ad.deselectButton =
            CreateButton(editMenu, "Deselect", 'D', DeselectCb, NULL);
        XtManageChild(XmCreateSeparatorGadget(editMenu, "sep2", NULL, 0));
        CreateButton(editMenu, "Find/Change...", 'F', FindCb,
                     (XtPointer)ad.editor);
        CreateButton(editMenu, "Check Spelling...", 'p', SpellCb,
                     (XtPointer)ad.editor);
...
static Widget CreateOptionsMenu(Widget menuBar) {
        ...
        CreateToggle(optionsMenu, "Overstrike", 'O', OverstrikeCb, NULL,
                     ad.overstrikeModeOn);
        CreateToggle(optionsMenu, "Word Wrap", 'W', WordWrapCb, NULL,
                     ad.wordWrapOn);
        XtManageChild(XmCreateSeparatorGadget(optionsMenu, "sep1", NULL, 0));
        CreateToggle(optionsMenu, "Status Line", 'S', StatusLineCb, NULL,
                     ad.statusLineOn);
...
static Widget CreateEditor(Widget parent) {
        ...
        ac++;
        XtSetArg(al[ac], DtNrows, 40);
        ac++;
        XtSetArg(al[ac], DtNcolumns, 80);
        ac++;
        ...
        XtAddCallback(w, DtNtextDeselectCallback, TextUnselectedCb,
                      (XtPointer)w);
...
static Widget CreateButton(Widget parent, String label, char mnemonic,
...
static Widget CreateToggle(Widget parent, String label, char mnemonic,
                           XtCallbackProc callback, XtPointer callData,
                           Boolean value) {
        Widget button;
        XmString labelString;
...
static Widget CreateCascade(Widget parent, String label, char mnemonic,
                            Widget subMenu) {
        Widget button;
        XmString labelString;
        Arg al[10];
        ...
        XtSetArg(al[ac], XmNlabelString, labelString);
        ac++;
        XtSetArg(al[ac], XmNsubMenuId, subMenu);
        ac++;
        XtSetArg(al[ac], XmNmnemonic, mnemonic);
        ac++;
