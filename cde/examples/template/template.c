...
typedef enum { LOAD_EMPTY, LOAD_FILE, LOAD_BUFFER } LoadType;
...
static XtCallbackRec DropTransferCbList[] = {{DropTransferCb, NULL},
                                             {NULL, NULL}};
...
main(int argc, char **argv) {
        ...
        appShell =
            XtAppInitialize(&appContext, ApplicationClass, optionTable,
                            XtNumber(optionTable), &argc, argv, NULL, NULL, 0);

        msgCatalog = catopen(MessageCatalog, NL_CAT_LOCALE);

        XtGetApplicationResources(appShell, &argvals, appResources,
        ...
        WM_SAVE_YOURSELF =
            XmInternAtom(XtDisplay(appShell), "WM_SAVE_YOURSELF", False);
        ...
        if (argvals.printMode != NULL) {
                ...
                for (i = 1; i < argc; ++i) {
                        if (LoadFile(wd, argv[i]))
                                ...
                                fprintf(stderr,
                                        catgets(msgCatalog, 1, 10,
                                                "template: can't open %s\n"),
                                        argv[i]);
                }
                DestroyData(wd);
                exit(0);
        ...
        ttrc =
            ttmedia_ptype_declare(ToolTalkPType, 0, HandleTtMedia, NULL, True);
        if (tt_is_err(ttrc)) {
                errfmt = catgets(msgCatalog, 1, 8,
                                 "ttmedia_ptype_declare failed:\n%s");
        ...
        if ((ttrc = tt_ptr_error(ttpat)) != TT_OK) {
                errfmt =
                    catgets(msgCatalog, 1, 9, "ttdt_session_join failed:\n%s");
                statmsg = tt_status_message(status);
                errmsg = XtMalloc(strlen(errfmt) + strlen(statmsg) + 2);
                sprintf(errmsg, errfmt, statmsg);
        ...
        if (argvals.serverMode != NULL) {
                /*
                 * We're in server mode.  Thus do nothing until requested to do
                 * so through ToolTalk.
                 */
        } else if (argc < 2) {
         ...
         * widget, since it is the unmapped parent of all of the top-level
         * shells we pop up.
         */
 ...
 */
static Boolean NewWindow(LoadType loadtype, char *name_or_buf, int len) {
        Widget toplevel, mainWindow, menuBar, frame1, frame2, drawingArea, pd,
            cb, pb;
        char *title;
        ...
        n = 0;
        toplevel = XtCreatePopupShell("foo", topLevelShellWidgetClass, appShell,
                                      args, n);
        ...
        XtVaSetValues(toplevel, XmNiconName, appnameString, XmNiconPixmap,
                      iconPixmap, XmNiconMask, iconMask, NULL);
        ...
        n++;
        XtSetArg(args[n], XmNsubMenuId, pd);
        n++;
        ...
        labelString =
            XmStringCreateLocalized(catgets(msgCatalog, 2, 9, "New..."));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        ...
        labelString =
            XmStringCreateLocalized(catgets(msgCatalog, 2, 2, "Open..."));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        XtSetArg(args[n], XmNmnemonic, 'O');
        n++;
        ...
        labelString =
            XmStringCreateLocalized(catgets(msgCatalog, 2, 3, "Save As..."));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        ...
        labelString =
            XmStringCreateLocalized(catgets(msgCatalog, 2, 4, "Print"));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        ...
        labelString =
            XmStringCreateLocalized(catgets(msgCatalog, 2, 5, "Clear"));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        ...
        labelString =
            XmStringCreateLocalized(catgets(msgCatalog, 2, 6, "Exit"));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        XtSetArg(args[n], XmNmnemonic, 'E');
        n++;
        ...
        labelString =
            XmStringCreateLocalized(catgets(msgCatalog, 2, 8, "Overview..."));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        XtSetArg(args[n], XmNmnemonic, 'O');
        ...
        n++;
        XtSetArg(args[n], XmNmarginWidth, 20);
        n++;
        XtSetArg(args[n], XmNmarginHeight, 20);
        n++;
        ...
        n = 0;
        XtSetArg(args[n], XmNunitType, Xm100TH_MILLIMETERS);
        n++;
        XtSetArg(args[n], XmNwidth, 12000);
        n++;
        XtSetArg(args[n], XmNheight, 12000);
        n++;
        XtSetArg(args[n], XmNtranslations, drawingTranslations);
        n++;
        ...
        XmAddWMProtocolCallback(toplevel, WM_DELETE_WINDOW, ExitCb,
                                (XtPointer)wd);
...
static void HelpCb(Widget w, XtPointer cd, XtPointer cb) {
        ...
        if (helpDialog == NULL) {
                ...
                n++;
                XtSetArg(args[n], DtNlocationId, HelpTopic);
                n++;
                helpDialog =
                ...
                XtVaSetValues(helpDialog, DtNhelpVolume, HelpVolume,
                              DtNlocationId, HelpTopic, NULL);
        }
...
static void OpenCb(Widget w, XtPointer cd, XtPointer cb) {
        ...
        if (wd->openDialog == NULL) {
                ...
                dialogTitle = XmStringCreateLocalized(
                    catgets(msgCatalog, 1, 2, "Template Open"));
                ...
                n++;
                XtSetArg(args[n], XmNpattern, pattern);
                n++;
                XtSetArg(args[n], XmNdialogTitle, dialogTitle);
                n++;
                wd->openDialog = XmCreateFileSelectionDialog(
                    wd->shell, "openDialog", args, n);
...
static void OpenOkCb(Widget w, XtPointer cd, XtPointer cb) {
        ...
        XClearArea(XtDisplay(drawingArea), XtWindow(drawingArea), 0, 0, 0, 0,
                   True);
}
...
static void SaveCb(Widget w, XtPointer cd, XtPointer cb) {
        ...
        if (wd->saveDialog == NULL) {
                ...
                dialogTitle = XmStringCreateLocalized(
                    catgets(msgCatalog, 1, 3, "Template Save As"));
                pattern = XmStringCreateLocalized(SearchPattern);
                n = 0;
                XtSetArg(args[n], XmNdialogStyle,
                         XmDIALOG_PRIMARY_APPLICATION_MODAL);
                ...
                XtSetArg(args[n], XmNdialogTitle, dialogTitle);
                n++;
                wd->saveDialog = XmCreateFileSelectionDialog(
                    appShell, "saveAsDialog", args, n);
...
static void ExposeCb(Widget w, XtPointer cd, XtPointer cb) {
        ...
        if (dcb->event != NULL && dcb->event->xexpose.count > 0)
                return;
 ...
 */
static void SetTitle(Widget w, char *name) {
        char buf[1000];
        char *p;
 ...
 * realized.
 */
static void AssocData(WindowData *wd, Widget w) {
        wd->shell = w;
 ...
 */
static WindowData *FindData(Widget w) {
        WindowData *wd = NULL;
 ...
 */
static void DestroyData(WindowData *wd) {
        WindowData **p;
...
static Tt_message HandleTtMedia(Tt_message msg, void *clientdata, Tttk_op op,
                                Tt_status diagnosis, unsigned char *contents,
        ...
        if (strcmp(opstr, "Edit") == 0) {
                if (file != NULL) {
                        if (NewWindow(LOAD_FILE, file, 0))
                                ...
                                tttk_message_fail(msg, TT_ERR_OP, "open failed",
                                                  False);
                } else if (contents != NULL && len > 0) {
                        if (NewWindow(LOAD_BUFFER, (char *)contents, len))
                                tt_message_reply(msg);
                        else
                                tttk_message_fail(msg, TT_ERR_OP,
                                                  "load buffer failed", False);
                } else {
                        tttk_message_fail(msg, TT_ERR_OP, "no file or buffer",
                                          False);
                }
        } else {
...
static void AddPoint(WindowData *wd, int x, int y) {
        if (wd->npoints == wd->nalloc) {
                wd->nalloc += MallocInc;
                wd->points = (XPoint *)XtRealloc((char *)wd->points,
                                                 wd->nalloc * sizeof(XPoint));
        }
...
static void DrawPoint(Widget w, int x, int y) {
        ...
        if (initialized == False) {
                ...
                pixmap = XmGetPixmapByDepth(XtScreen(w), "template-brush.bm", 1,
                                            0, 1);
                if (pixmap == XmUNSPECIFIED_PIXMAP) {
                        XtAppWarning(
                            appContext,
                            "template:template-brush.bm not installed!");
                        /* use hardcoded fallback bitmap */
                        pixmap = XCreatePixmapFromBitmapData(
                            XtDisplay(w), XtWindow(w),
                            (char *)default_brush_bits, default_brush_width,
                            default_brush_height, 1, 0, 1);
                }
        ...
        XSetTSOrigin(XtDisplay(w), gc, x - pixmapWidth / 2,
                     y - pixmapWidth / 2);
        XFillRectangle(XtDisplay(w), XtWindow(w), gc, x - pixmapWidth / 2,
                       y - pixmapWidth / 2, pixmapWidth, pixmapHeight);
}
