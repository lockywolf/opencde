...
int usleep(usec) unsigned long usec;
{
...
int main(int argc, char *argv[]) {
        ...
        /******************************************************/
        if ((perWindow = (WinDataStruct *)malloc(
                 nWindows * sizeof(WinDataStruct))) == NULL) {
        ...
        for (i = 0; i < nWindows; i++) {
                ...
                /***********************************************/
                if (XAllocNamedColor(dpy, perWindow[i].cmap, "red", &actual,
                                     &exact)) {
                        perWindow[i].ssPixels[0] = actual.pixel;
                } else {
                        perWindow[i].ssPixels[0] =
                            BlackPixelOfScreen(attr.screen);
                }
                if (XAllocNamedColor(dpy, perWindow[i].cmap, "orange", &actual,
                                     &exact)) {
                        perWindow[i].ssPixels[1] = actual.pixel;
                } else {
                        perWindow[i].ssPixels[1] =
                            WhitePixelOfScreen(attr.screen);
                }
                if (XAllocNamedColor(dpy, perWindow[i].cmap, "yellow", &actual,
                                     &exact)) {
                        perWindow[i].ssPixels[2] = actual.pixel;
                } else {
                        perWindow[i].ssPixels[2] =
                            BlackPixelOfScreen(attr.screen);
                }
                if (XAllocNamedColor(dpy, perWindow[i].cmap, "green", &actual,
                                     &exact)) {
                        perWindow[i].ssPixels[3] = actual.pixel;
                } else {
                        perWindow[i].ssPixels[3] =
                            WhitePixelOfScreen(attr.screen);
                }
                if (XAllocNamedColor(dpy, perWindow[i].cmap, "blue", &actual,
                                     &exact)) {
                        perWindow[i].ssPixels[4] = actual.pixel;
                } else {
                        perWindow[i].ssPixels[4] =
                            BlackPixelOfScreen(attr.screen);
                        ;
                }
                if (XAllocNamedColor(dpy, perWindow[i].cmap, "purple", &actual,
                                     &exact)) {
                        perWindow[i].ssPixels[5] = actual.pixel;
                } else {
                        perWindow[i].ssPixels[5] =
                            WhitePixelOfScreen(attr.screen);
                }
        }
        ...
        while (1) {
                ...
                for (i = 0; i < nWindows; i++) {
                        ...
                        for (j = 0; j < SS_MAX_COLORS; j++) {
                                ...
                                curColor = perWindow[i].ssPixels[(int)fmod(
                                    (j + colorRotate), SS_MAX_COLORS)];
                                XSetBackground(dpy, perWindow[i].gc, curColor);
                                XSetForeground(dpy, perWindow[i].gc, curColor);
                                x = j * ((perWindow[i].width / 2) /
                                         (SS_MAX_COLORS));
                                y = j * ((perWindow[i].height / 2) /
                                         (SS_MAX_COLORS));
                                width = perWindow[i].width - (2 * x);
                                height = perWindow[i].height - (2 * y);
                                XFillRectangle(dpy, perWindow[i].id,
                                               perWindow[i].gc, x, y, width,
                                               height);
                        }
                }
                /**XFlush(dpy); **/
