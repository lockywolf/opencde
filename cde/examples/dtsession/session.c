...
static XtResource applicationResources[] = {
    {"session", "Session", XmRString, sizeof(String),
     XtOffsetOf(ApplicationArgs, session), XmRImmediate, NULL},
};
...
main(int argc, char **argv) {
        ...
        toplevel = XtAppInitialize(&appContext, "Session", commandLineOpts,
                                   XtNumber(commandLineOpts), &argc, argv, NULL,
                                   NULL, 0);
...
static void PreserveCommandLine(int argc, char **argv) {
        ...
        savedArgc = argc;
        for (i = 0; i < argc; i++)
                savedArgv[i] = XtNewString(argv[i]);
}
...
static void SetWmCommand(char *sessionId) {
        ...
        for (i = 1, j = 3; i < savedArgc; i++) {
                if (strcmp(savedArgv[i], "-session") == 0) {
                        i++;
                        continue;
                }
