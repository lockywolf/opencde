...
char **argv;
{
        ...
        n++;
        toplevel = XtAppInitialize(&app_context, "Dtwsinfo", NULL, 0, &argc,
                                   argv, NULL, args, n);
 ...
 */
static void quitCB(w, client_data,
                   call_data) Widget w; /*  widget id		*/
 ...
 */
static void updateCB(w, client_data,
                     call_data) Widget w; /*  widget id		*/
XtPointer client_data;                    /*  data from application   */
XtPointer call_data;                      /*  data from widget class  */
{ ShowCurrentWorkspaceInfo(); }
...
*/
static DtWsmWsChangeProc wschangecb(w, atom, client_data) Widget w;
Atom atom;
...
*/
static void InstallXErrorHandler(display) Display *display;
{
        char buffer[BUFSIZ];
        int i;
