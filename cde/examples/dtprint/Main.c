...
static XtResource VideoResources[] = {
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffsetOf(VideoShell, print_only),
        XmRImmediate,
        (XtPointer)False,
    },
    {
        ...
        XmRString,
        sizeof(char *),
        XtOffsetOf(VideoShell, file_name),
        XmRImmediate,
        (XtPointer)NULL,
    },
};
...
static VideoShell *VideoShell_new(Display *display) {
        ...
        me->widget =
            XtVaAppCreateShell(NULL, APP_CLASS, applicationShellWidgetClass,
                               display, XmNtitle, "DtPrint Demo", NULL);
        XtGetApplicationResources(me->widget, me, VideoResources,
                                  XtNumber(VideoResources), NULL, 0);
        return me;
}
 ...
 *     None.
 */
void CloseProgramCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (p->printed_lines) {
                /* put up a dialog saying it's waiting for the job
                   to complete */
                XtAddCallback(p->print_shell, XmNendJobCallback, CloseProgramCB,
                              p);
        } else {
                exit(0);
        }
...
static String fallbacks[] = {"Dtprint.Print*background:white",
                             "Dtprint.Print*renderTable:-dt-application-bold-r-"
                             "normal-serif-0-0-0-0-p-0-iso8859-1",
...
int main(int argc, char *argv[]) {
        ...
        video_display =
            XtOpenDisplay(app_context, NULL, NULL, APP_CLASS, XrmOptions,
                          XtNumber(XrmOptions), &argc, argv);
        ...
        if (video_shell->print_only) {
                 ...
                 */
                p->app_object =
                    AppObject_new((Widget)NULL, video_shell->file_name);
                /*
                 * create the print setup box as the child of the top level
                 * shell
                 */
                 ...
                 */
                XtAddCallback(main_window->print_menu_button,
                              XmNactivateCallback, PrintMenuCB, p);
                p->pr_button = main_window->print_menu_button;

                XtAddCallback(main_window->quick_print_button,
                              XmNactivateCallback, QuickPrintCB, p);
