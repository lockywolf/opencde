 ...
 */
void demoDragFinishCallback(Widget widget, XtPointer clientData,
                            XtPointer callData) {
        demoDoingDrag = False;
}
 ...
 * was being held down.
 */
void demoDragMotionHandler(Widget dragInitiator, XtPointer clientData,
                           XEvent *event) {
                ...
                if ((ABS(diffX) >= DRAG_THRESHOLD) ||
                    (ABS(diffY) >= DRAG_THRESHOLD)) {
                        ...
                        case DtDND_FILENAME_TRANSFER:
                                fileCheckForDrag(dragInitiator, event, initialX,
                                                 initialY);
                                break;
...
void demoProcessPress(Widget w, XEvent *event, String *parms,
                      Cardinal *nparms) {
 ...
 */
void demoDrawExposeCallback(Widget widget, XtPointer clientData,
                            XtPointer callData) {
        IconInfo *iconPtr;
 ...
 */
void demoDropSetup(Widget dropDraw) {
        ...
        DtDndVaDropRegister(
            dropDraw, DtDND_FILENAME_TRANSFER | DtDND_BUFFER_TRANSFER,
            XmDROP_COPY | XmDROP_MOVE, transferCBRec, DtNdropAnimateCallback,
            animateCBRec, DtNtextIsBuffer, True, DtNpreserveRegistration, False,
            NULL);
}
 ...
 */
void demoTransferCallback(Widget widget, XtPointer clientData,
                          XtPointer callData) {
 ...
 */
Widget demoCreateDropSite(Widget parent) {
        Widget dropFrame, dropDraw;

        dropFrame = XtVaCreateManagedWidget("dropFrame", xmFrameWidgetClass,
                                            parent, NULL);

        dropDraw = XtVaCreateManagedWidget("dropDraw", xmDrawingAreaWidgetClass,
                                           dropFrame, NULL);

        XtAddCallback(dropDraw, XmNexposeCallback, demoDrawExposeCallback,
                      NULL);
...
void main(int argc, String *argv) {
        Widget outerRowColumn, upperRowColumn, textDragSource, fileDragSource,
            apptDragSource, separator, textDropSite, drawDropSite;
        ...
        DtAppInitialize(demoAppContext, XtDisplay(demoTopLevel), demoTopLevel,
                        argv[0], "Dtdnddemo");

        outerRowColumn = XtVaCreateManagedWidget(
            "outerRowColumn", xmRowColumnWidgetClass, demoTopLevel, NULL);

        upperRowColumn = XtVaCreateManagedWidget(
            "upperRowColumn", xmRowColumnWidgetClass, outerRowColumn, NULL);
