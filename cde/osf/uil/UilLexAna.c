...
#ifndef lint
static char rcsid[] =
    "$TOG: UilLexAna.c /main/14 1997/03/12 15:10:52 dbl $"
#endif
#endif
...
static lex_buffer_type *get_lex_buffer _ARGUMENTS((lex_buffer_type *
                                                   az_current_lex_buffer));
#if debug_version
static void dump_token _ARGUMENTS((lex_buffer_type * az_current_lex_buffer,
                                   int l_lex_pos));
#endif
...
static int punc_token[tok_punc_token_num] = {
    LEFT_BRACE, RIGHT_BRACE, EQUAL_SIGN, SEMICOLON, LEFT_PAREN, RIGHT_PAREN,
    COLON,      PLUS,        MINUS,      COMMA,     AND,        NOT,
    MULTIPLY,   DIVIDE,      XOR,        OR,        POUND};

/*    Tables to correlate token numbers and ASCII value pairs for
      punctuation characters.  Used by yylex and lex_issue_error.     */

static unsigned char punc2_char[2] = {'<', '>'};
static int punc2_token[2] = {LEFT_SHIFT, RIGHT_SHIFT};
...
int yylex() {
        ...
        lex_buffer_type *az_current_lex_buffer; /* current lexical buffer */
        ...
        */
        ...
        Uil_percent_complete = CEIL(
            (int)(.5 * ((float)Uil_characters_read / (float)Uil_file_size)) *
                100,
            50);
...
continue_in_next_state:
        ...
        for (;;) {
                /* get next input char */
                /* advance source too */
                c_char = src_az_current_source_buffer
                             ->c_text[src_az_current_source_buffer
                                          ->w_current_position++];
                ...
                case move_advance:
                case advance:
                        if (l_16bit_chars_only)
                                goto found_16bit_char;
                        continue;
...
special_processing:
        ...
        case found_digit: /* next digit in a \digit...\ sequence */
                ...
                if (l_bslash_value <
                    256) /* if still in range add in next digit */
        ...
        case end_digits: /* end of \digit...\ sequence */
                ...
                if (l_bslash_value >= 256) /* issue a diagnostic */
                {
                        diag_issue_diagnostic(
                            d_out_range, src_az_current_source_record,
                            src_az_current_source_buffer->w_current_position -
                                1,
                            "\\digit...\\ sequence", "0-255");
                ...
                {
                        diag_issue_diagnostic(
                            d_null, src_az_current_source_record,
                            src_az_current_source_buffer->w_current_position -
                                1);
                }
        ...
        case insert_char: /* place special character in lex buffer */
        {
                ...
                az_current_lex_buffer->c_text[l_lex_pos++] =
                    ab_bslash_value[_index(c_char, c_bslash_char,
                                           sizeof(c_bslash_char) - 1)];
                break;
        }
        ...
        case missing_bslash: /* \digit...\ sequence not terminated */
                ...
                diag_issue_diagnostic(
                    d_unterm_seq, src_az_current_source_record,
        ...
        case string_wrap:
                ...
                if (src_get_source_line() == src_k_end_source) {
                        diag_issue_diagnostic(
                            ...
                            src_az_current_source_buffer->w_current_position -
                                1,
                            "character string", "before end of source");
        ...
        found_16bit_char : {
                ...
                */
                if (l_charset_sixteen_bit) {
                        ...
                        **	2) check that the next is not a control
                        *character *	3) place the next character in the lex
                        *buffer
                        */

                        next_char = src_az_current_source_buffer
                                        ->c_text[src_az_current_source_buffer
                                                     ->w_current_position++];
        ...
        case nocharset_gstr:
                ...
                } else if (Uil_lex_l_localized)
                        goto found_localized_string;
                else {
                ...
                sem_charset_info(l_charset, az_charset_entry,
                                 &l_write_direction, &l_parse_direction,
...
found_localized_string : {
        ...
        /* Parse the string. */
        while (TRUE) {
                mb_len = mblen((char *)&src_az_current_source_buffer
                                   ->c_text[src_az_current_source_buffer
                                                ->w_current_position],
                               MB_CUR_MAX);

                mb_byte = src_az_current_source_buffer
                              ->c_text[src_az_current_source_buffer
                                           ->w_current_position];
...
found_token:
        ...
        case token_name: {
                ...
                if (!uil_v_case_sensitive) {
                        ...
                        for (ptr = (char *)(az_current_lex_buffer->c_text);
                             (*ptr) != 0; (*ptr) = _upper(*ptr), ptr++) {
                        }
                }

                /* check if the name is a keyword */

                az_keyword = key_find_keyword(
                    l_lex_pos, (char *)az_current_lex_buffer->c_text);
                if (az_keyword != NULL) {
                        ...
                        if (l_lex_pos > key_k_keyword_max_length) {
                                ...
                                az_current_lex_buffer->c_text[l_lex_pos] = 0;
                                diag_issue_diagnostic(
                                    d_name_too_long, az_start_src_record,
                                    l_start_src_pos,
                ...
                if (l_lex_pos > lex_k_identifier_max_length) {
                        ...
                        diag_issue_diagnostic(
                            d_name_too_long, az_start_src_record,
                            l_start_src_pos, lex_k_identifier_max_length,
                            az_current_lex_buffer->c_text);
                }
        ...
        case token_punc: {
                ...
                /* found a punctuation mark - look up its token number in a
                 * table */

                l_token = punc_token[_index(az_current_lex_buffer->c_text[0],
        ...
        case token_punc2: {
                ...
                l_token = punc2_token[_index(az_current_lex_buffer->c_text[0],
        ...
        case token_integer: {
                ...
                if (errno != 0)
                        diag_issue_diagnostic(d_out_range, az_start_src_record,
                                              l_start_src_pos, "integer", " ");
        ...
        case token_ustring:
                ...
                diag_issue_diagnostic(
                    d_unterm_seq, src_az_current_source_record,
                    src_az_current_source_buffer->w_current_position - 1,
                    "character string", "before end of line");
        ...
        case token_comment: /* RAP preserve comments */
        {
                ...
                if ((size + (int)strlen(comment_text)) >= comment_size) {
                        comment_text =
                            XtRealloc(comment_text,
                                      INCR_COMMENT_SIZE + strlen(comment_text));
        ...
        found_primitive_string : {
                ...
                az_value =
                    create_str_entry(l_length, l_charset, az_charset_entry);
                ...
                for (az_lex_buffer = az_first_lex_buffer;
                     az_lex_buffer != az_current_lex_buffer;
                     az_lex_buffer = az_lex_buffer->az_next_buffer) {
                        _move(c_char, az_lex_buffer->c_text,
                              l_max_lex_buffer_pos + 1);
        ...
        case token_gstr:
                ...
                */
                if (l_parse_direction == XmSTRING_DIRECTION_R_TO_L) {
                        ...
                        /* assuming the string is confined to a single lex
                         * buffer.	    */
                        /* just flip the characters around.  16-bit characters
                         * need to  */
                        /* be done in groups of two bytes
                         */
                        if (l_charset_sixteen_bit != TRUE)
                                /*
                                **  Just reverse the bytes from the first to
                                *last
                                */
                                for (i = 0, j = l_lex_pos - 1;
                                     i < (l_lex_pos >> 1); i++, j--) {
                        ...
                        /*
                        **  Don't reverse the string if have less than 2
                        *characters (4 bytes)
                        */
                        else if ((l_lex_pos >> 1) > 1)
                                /*
                                **  This reversing doesn't work for mixed
                                *8/16-bit character
                                **  sets, but only built-in character sets allow
                                *mixing and
                                **  they are not right-to-left.  We do the same
                                *copying as in
                                **  the 8-bit case above, but we move two bytes
                                *at a time and
                                **  reverse the order as we copy so they end up
                                *correct.
                                */
                                for (i = 0, j = l_lex_pos - 1;
                                     i < (l_lex_pos >> 1); i += 2, j -= 2) {
                                        tmp1 = az_current_lex_buffer->c_text[i];
                                        tmp2 = az_current_lex_buffer
                                                   ->c_text[i + 1];
                                        az_current_lex_buffer->c_text[i] =
                                            az_current_lex_buffer
                                                ->c_text[j - 1];
                ...
                */
                if ((l_charset_sixteen_bit != TRUE) ||
                    (l_charset == lex_k_userdefined_charset))
                        ...
                        /*
                        **	if string consists solely of 8-bit ascii
                        *characters,
                        ...
                        */
                        ...
                        /*
                        **  For KANJI and HANZI treat 8-bit characters as
                        *ISO_LATIN1.
                        */
                        ...
                        */
                        ...
                        for (a_off = 0, off = 0; off < l_lex_pos;)

                        {

                                for (off = a_off; off < l_lex_pos; off++)
                                        if (az_current_lex_buffer->c_text[off] >
                                            0x97)
                                                break;
                                ...
                                if (off > a_off) {
                                        ...
                                        sem_append_str_to_cstr(cstr_entry,
                                                               str_entry, TRUE);
                                }

                                for (a_off = off; a_off < l_lex_pos; a_off += 2)
                                        if (az_current_lex_buffer
                                                ->c_text[a_off] <= 0x97)
                                                break;
        ...
        case token_lstr: {
                ...
                str_entry = create_str_entry(
                    l_length, lex_k_fontlist_default_tag, az_charset_entry);
        ...
        /* RAP we want to keep track of whether we are appending sequential
         * comments
         */
        ...
        yylval.b_source_end =
            src_az_current_source_buffer
                ->w_current_position; /*  was "l_start_src_pos + l_lex_pos;" */
...
found_error:
        ...
        case bad_prefix:
                ...
                diag_issue_diagnostic(d_unknown_seq, az_start_src_record,
                                      l_start_src_pos,
                                      az_current_lex_buffer->c_text);
...

{
         ...
         */
        az_first_lex_buffer = (lex_buffer_type *)_get_memory(
        ...
        if (language == NULL)
                ...
                Uil_lex_l_user_default_charset =
                    sem_charset_lang_name(language);
                if (Uil_lex_l_user_default_charset == sym_k_error_charset) {
                        diag_issue_diagnostic(d_bad_lang_value,
                                              diag_k_no_source,
                                              diag_k_no_column);
                        Uil_lex_l_user_default_charset = lex_k_default_charset;
                }
...

{
        ...
        diag_issue_diagnostic(d_syntax, yylval.az_source_record,
                              yylval.b_source_pos, tok_name, c_char);
}
...

{
        ...
        diag_issue_diagnostic(
            d_control_char, src_az_current_source_record,
            src_az_current_source_buffer->w_current_position - 1, c_char);
...

{
        ...
        if (az_lex_buffer == NULL) {
                /* BEGIN OSF Fix CR 4749 */
                /* The lex algorithm has the potential to write
                 * into index l_max_lex_buffer_pos + 1, so allocate
                 * l_max_lex_buffer_pos plus 2 positions in buffer.
                 */
                az_lex_buffer = (lex_buffer_type *)_get_memory(
                    l_max_lex_buffer_pos + 2 + sizeof(lex_buffer_type *));
                /* END OSF Fix CR 4749 */
...
static void dump_token(az_current_lex_buffer, l_lex_pos)
...
int l_lex_pos;
{
        ...
        _debug_output("token: %d start: %d, %d  end: %d, %d \n", yylval.b_type,
...
long cvt_ascii_to_long(c_text) unsigned char XmConst *c_text;
{
...

{
        ...
        node = (sym_value_entry_type *)sem_allocate_node(
