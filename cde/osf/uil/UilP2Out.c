...

{
        ...
        */
        ...
        urm_status = Urm__UT_SetErrorReport(URMErrOutMemory);
        ...
        */
        ...
        urm_status = UrmIdbOpenFileWrite(
            Uil_cmd_z_command.ac_resource_file, &uid_fcb.os_param,
            _host_compiler, _compiler_version, module_name, module_version,
            &out_az_idbfile_id, uid_fcb.result_file);

        if (urm_status != MrmSUCCESS) {
                diag_issue_diagnostic(d_uid_open, diag_k_no_source,
                                      diag_k_no_column, uid_fcb.result_file);

                return;
        }

        /*
        **	Set the name of the file we are accessing as the current file
        *and call *	the Status callback routine to report our progress.
        */
        ...
        */
        ...
        urm_status = UrmGetResourceContext(
            _get_memory, _free_memory, initial_context_size, &out_az_context);
        if (urm_status != MrmSUCCESS)
                issue_urm_error("allocating context");
        ...
        */
        ...
        /*
        **	Count the number of topmost widgets by scanning the external
        *list. *	A topmost widget is one that is not referenced.
        */

        topmost_widget_count = 0;

        for (ext_entry = sym_az_external_def_chain; ext_entry != NULL;
        ...
        */
        ...
        urm_status = UrmIFMInitModule(out_az_context, topmost_widget_count,
                                      URMaPublic, FALSE);
        if (urm_status != MrmSUCCESS)
        ...
        **	Values, gadgets and widgets are pushed on LIFO queue and
        *processed *	shortly. *	Each widget on the list is top most if
        *it is not referenced.  Topmost *	widgets are added to the
        *current context for the interface module.
        */
             ...
             ext_entry = ext_entry->az_next_object) {
                ...
                */
                ...
                Uil_percent_complete =
                    CEIL(80 + (.20 * ((float)topmost_index /
                                      (float)(topmost_widget_count + .5))) *
                                  100,
                         80);
                ...
                case sym_k_widget_entry: {
                        ...
                        if ((widget_entry->obj_header.az_name->b_flags &
                             sym_m_referenced) == 0) {
                                widget_entry->output_state = sym_k_queued;

                                push((sym_entry_type *)widget_entry);

                                urm_status = UrmIFMSetTopmost(
                                    out_az_context, topmost_index,
                                    widget_entry->obj_header.az_name->c_text);
                                if (urm_status != MrmSUCCESS)
                                        issue_urm_error(
                                            "adding topmost widget");
        ...
        */
        ...
        urm_status =
            UrmIFMPutModule(out_az_idbfile_id, module_name, out_az_context);
        if (urm_status != MrmSUCCESS) {
        ...
        if (Uil_cmd_z_command.v_show_machine_code) {
                save_module_machine_code(src_az_module_source_record,
                                         out_az_context);
        }
        ...
        */
        ...
        urm_status = UrmIdbCloseFile(out_az_idbfile_id, urm_status);
        ...
        */
        ...
        urm_status = UrmFreeResourceContext(out_az_context);
        if (urm_status != MrmSUCCESS)
        ...
        */
        ...
        if (out_l_next_offset > out_k_last_offset) {
                if (src_az_avail_source_buffer == NULL) {
                        src_az_avail_source_buffer =
                            (src_source_buffer_type *)_get_memory(
                                sizeof(src_source_buffer_type));
                        src_az_avail_source_buffer->az_prior_source_buffer =
                            NULL;
                }
...

{
        ...
        if (out_l_next_offset < 0) {
                ...
                avail_buffer->az_prior_source_buffer =
                    src_az_avail_source_buffer;
...

{
        ...
        urm_status =
            UrmCWRInit(out_az_context, widget_name, access_code, FALSE);
        if (urm_status != MrmSUCCESS)
        ...
        */
        if (widget_entry->header.b_type == uil_sym_user_defined_object) {
                widget_class_name = widget_entry->az_create_proc->az_proc_def
                                        ->obj_header.az_name->c_text;
        }

        /*
        ** Special processing 2. Widgets which map their (single) control to
        ** a (subtree) resource. This handles the convention (originally
        *invented
        ...
        */
        ...
        if (subtree_code != 0) {
                ...
                default:
                        ...
                        diag_issue_diagnostic(
                            d_single_control, _sar_source_pos2(subtree_control),
        ...
        urm_status = UrmCWRSetClass(out_az_context, widget_class,
                                    widget_class_name, widget_variety);
        if (urm_status != MrmSUCCESS)
        ...
        */
        ...
        if (list_entry != NULL) {
                ...
                arg_count +=
                    compute_list_size(list_entry, sym_k_callback_entry);
                callback_entry = NULL;
        ...
        */
        ...
        if (widget_entry->az_arguments != NULL)
                arg_count += compute_list_size(widget_entry->az_arguments,
                                               sym_k_argument_entry);

        if (arg_count > 0) {
                urm_status = UrmCWRInitArglist(out_az_context, arg_count);
                if (urm_status != MrmSUCCESS)
                ...
                */
                process_all_callbacks(widget_entry->az_callbacks,
                                      &arglist_index);
                process_all_arguments(widget_entry->az_arguments,
                                      &arglist_index, &related_arg_count);

                /*
                ** Process a control which is to be entered as a subtree
                *resource. Mark
                ...
                */
                if (subtree_control != NULL) {
                        ...
                        widget_form =
                            ref_control(subtree_control, &widget_access,
                                        &widget_index, &widget_id);
                        urm_status = UrmCWRSetArgResourceRef(
                            out_az_context, arglist_index, widget_access,
                            URMgWidget, RGMwrTypeSubTree, widget_form,
                            widget_index, widget_id);
                        if (urm_status != MrmSUCCESS)
        ...
        */
        ...
        if (list_entry != NULL) {
                ...
                */
                ...
                widget_index =
                    compute_list_size(list_entry, sym_k_control_entry);
                if (widget_index > 0) {
                        urm_status =
        ...
        */
        ...
        if (widget_entry->obj_header.az_name == NULL) {
                if (widget_entry->resource_id == 0) {
                        urm_status = UrmIdbGetResourceId(
                ...
                urm_status =
                    UrmPutRIDWidget(out_az_idbfile_id,
                                    widget_entry->resource_id, out_az_context);
        } else
                urm_status = UrmPutIndexedWidget(out_az_idbfile_id, widget_name,
                                                 out_az_context);
...
void extract_subtree_control(list_entry, menu_entry,
                             count) sym_list_entry_type *list_entry;
...

{
        ...
        /*
         * Process the list elements, recursing on nested lists. Ignore error
         * entries.
         */
        if (list_entry == NULL)
                return;
        for (list_member = (sym_obj_entry_type *)list_entry->obj_header.az_next;
             ...
             list_member =
                ...
                case sym_k_nested_list_entry:
                        ...
                        extract_subtree_control(nested_list_entry->az_list,
                                                menu_entry, count);
                        break;
...
void extract_create_callback(list_entry,
                             create_entry) sym_list_entry_type *list_entry;
        ...
        for (list_member = (sym_obj_entry_type *)list_entry->obj_header.az_next;
             ...
             list_member =
                ...
                case sym_k_nested_list_entry:
                        nested_list_entry =
                            (sym_nested_list_entry_type *)list_member;
                        extract_create_callback(nested_list_entry->az_list,
                                                create_entry);
                        break;
                case sym_k_callback_entry:
                        ...
                        if (value_entry->obj_header.b_flags & sym_m_builtin) {
                                key_entry = (key_keytable_entry_type *)
                                                value_entry->value.l_integer;
                                if (strcmp(uil_reason_toolkit_names
                                               [key_entry->b_subclass],
...
void process_all_callbacks(list_entry,
                           arglist_index) sym_list_entry_type *list_entry;
int *arglist_index;
        ...
        for (list_member = (sym_obj_entry_type *)list_entry->obj_header.az_next;
             ...
             list_member =
                ...
                case sym_k_nested_list_entry:
                        nested_list_entry =
                            (sym_nested_list_entry_type *)list_member;
                        process_all_callbacks(nested_list_entry->az_list,
                                              arglist_index);
                        break;
                case sym_k_callback_entry:
                        callback_entry = (sym_callback_entry_type *)list_member;
...
void process_all_arguments(list_entry, arglist_index,
                           related_count) sym_list_entry_type *list_entry;
int *arglist_index;
int *related_count;
        ...
        for (list_member = (sym_obj_entry_type *)list_entry->obj_header.az_next;
             ...
             list_member =
                ...
                case sym_k_nested_list_entry:
                        nested_list_entry =
                            (sym_nested_list_entry_type *)list_member;
                        process_all_arguments(nested_list_entry->az_list,
                                              arglist_index, related_count);
                        break;
                case sym_k_argument_entry:
                        argument_entry = (sym_argument_entry_type *)list_member;
                        emit_argument(argument_entry, *arglist_index,
                                      related_count);
                        *arglist_index -= 1;
                        break;
                case sym_k_error_entry:
...
void process_all_controls(list_entry,
                          widget_index) sym_list_entry_type *list_entry;
int *widget_index;
        ...
        for (list_member = (sym_obj_entry_type *)list_entry->obj_header.az_next;
             ...
             list_member =
                ...
                case sym_k_nested_list_entry:
                        nested_list_entry =
                            (sym_nested_list_entry_type *)list_member;
                        process_all_controls(nested_list_entry->az_list,
                                             widget_index);
                        break;
                case sym_k_control_entry:
                        control_entry = (sym_control_entry_type *)list_member;
...

{
        ...
        _assert((value_entry->obj_header.b_flags &
                 (sym_m_exported | sym_m_private)),
        ...
        */
        ...
        case sym_k_compound_string_value:
                ...
                value_size = XmCvtXmStringToByteStream(
                    tmp_str, (unsigned char **)&(value_entry->value.c_value));
                     ...
                     value_segment = value_segment->az_next_table_value) {
                        ...
                        value_size +=
                            XmCvtXmStringToByteStream(
                                value_segment->value.xms_value, NULL) +
                            sizeof(RGMTextEntry);
                }
                break;
        }
        ...
        case sym_k_font_table_value: {
                 ...
                 * We allocate one fewer FontItems than specified in the
                 ...
                 **  the lengths of component fonts.  Each component is a
                 *FontItem
                ...
                for (font_value = value_entry->az_first_table_value;
                     ...
                     font_value = font_value->az_next_table_value) {
                        /* Fix for CR 5266 Part 2a -- Pull az_charset_value off
                         * of font_value, rather than value_entry.
                         */
                        value_count += 1;
                        value_size += sizeof(RGMFontItem) +
                                      strlen(sem_charset_name(
                                          font_value->b_charset,
        ...
        */
        ...
        case sym_k_wchar_string_value: {
                ...
                _move(wcharentry->wchar_item.bytes,
                      value_entry->az_first_table_value->value.c_value,
                      value_size);
                break;
        }
                /* END OSF Fix CR 4859 */
        ...
        case sym_k_color_value: {
                ...
                case sym_k_unspecified_color:
                        color_buffer->mono_state =
                            URMColorMonochromeUnspecified;
                        break;
                ...
                _move(color_buffer->desc.name, value_entry->value.c_value,
        ...
        case sym_k_string_table_value: {
                ...
                for (value_segment = value_entry->az_first_table_value;
                     ...
                     value_segment = value_segment->az_next_table_value) {
                        ...
                        segment_size = XmCvtXmStringToByteStream(
                            tmp_str,
                        ...
                        index--;
                        string_vector->item[index].text_item.offset =
                            text_offset;
        ...
        case sym_k_font_table_value: {
                ...
                for (index = 0, font_value = value_entry->az_first_table_value;
                     font_value != NULL;
                     index++, font_value = font_value->az_next_table_value) {
                        fontitem = &fontlist->item[index];
                        /*
                         * Fix for CR 5266 Part 2b -- Pull az_charset_value off
                         * of font_value, rather than value_entry.
                         */
                        charset_name =
                        ...
                        fontitem->type =
                            Urm_code_from_uil_type(font_value->b_type);
                        fontitem->cset.cs_offs = textoffs;
                        strcpy(textptr, charset_name);
                        text_len = strlen(charset_name) + 1;
                        textoffs += text_len;
        ...
        case sym_k_trans_table_value: {
                ...
                for (value_segment = value_entry->az_first_table_value;
                     ...
                     value_segment = value_segment->az_next_table_value) {
                        ...
                        segment_size = value_segment->w_length + 1;
                        offset -= segment_size;
                        _move(&(buffer[offset]), value_segment->value.c_value,
                              segment_size - 1);
                }
        ...
        */
        if (access == URMaPublic)
                ...
                urm_status =
                    UrmPutRIDLiteral(out_az_idbfile_id,
                                     value_entry->resource_id, out_az_context);
...
void emit_callback
    ...
    (sym_callback_entry_type *callback_entry, int *arglist_index,
     boolean emit_create)

{
        ...
        if (callback_entry->az_call_proc_ref != 0)
                ...
                proc_count =
                    count_proc(callback_entry->az_call_proc_ref_list, 0);

        /*
        ** Reasons can take several forms:
        **    1) create reason: value is builtin - keyword Urm value is
        *"created" *	this need special handling
        ...
        */
        ...
        if (reason_entry->obj_header.b_flags & sym_m_builtin) {
                ...
                key_entry =
                    (key_keytable_entry_type *)reason_entry->value.l_integer;
                qcreate =
                    (strcmp(uil_reason_toolkit_names[key_entry->b_subclass],
                            MrmNcreateCallback) == 0);
                if (qcreate) {
                        /*
                         **	case 1: create reason - return if we are not to
                         *emit *	otherwise use special routine to
                         *describe
                         */
                        if (!emit_create)
                                return;
                        urm_status = UrmCWRSetCreationCallback(
                            out_az_context, proc_count, &callback_offset);
                        if (urm_status != MrmSUCCESS) {
                                if (urm_status == MrmEOF)
                                        diag_issue_diagnostic(
                        ...
                        urm_status = UrmCWRSetCompressedArgTag(
                            out_az_context, *arglist_index,
                            uil_reas_compr[key_entry->b_subclass], 0);
                        if (urm_status != MrmSUCCESS)
                                issue_urm_error("setting compressed arg");
                }
        } else {
                 ...
                 */
                if (reason_entry->obj_header.b_flags &
                    (sym_m_imported | sym_m_exported)) {
                        diag_issue_diagnostic(
                ...
                /*
                 **  case 3: private, non-builtin case - use an uncompressed
                 *argument
                 */
                urm_status =
                    UrmCWRSetUncompressedArgTag(out_az_context, *arglist_index,
        ...
        if (!qcreate) {
                urm_status =
                    UrmCWRSetArgCallback(out_az_context, *arglist_index,
                                         proc_count, &callback_offset);
                if (urm_status != MrmSUCCESS)
        ...
        emit_callback_procedures(proc_ref_entry_next, &proc_ref_index,
                                 callback_offset);
...
int count_proc(proc_list, count) sym_list_entry_type *proc_list;
int count;

{
        ...
        for (proc_list_next =
                 (sym_obj_entry_type *)proc_list->obj_header.az_next;
             proc_list_next != 0;
             proc_list_next =
                 (sym_obj_entry_type *)proc_list_next->obj_header.az_next) {
                switch (proc_list_next->header.b_tag) {
                case sym_k_nested_list_entry:
                        count = count_proc(
                            ((sym_nested_list_entry_type *)proc_list_next)
                                ->az_list,
                            count);
                        break;
...
void emit_callback_procedures
    ...
    (sym_proc_ref_entry_type *proc_ref_entry_next, int *proc_ref_index,
     MrmOffset callback_offset)

{
        ...
        for (; proc_ref_entry_next != 0;
             proc_ref_entry_next = (sym_proc_ref_entry_type *)
                                       proc_ref_entry_next->obj_header.az_next)

        {
                switch (proc_ref_entry_next->header.b_tag) {
                case sym_k_nested_list_entry:
                        nested_proc_list_entry =
                            (sym_nested_list_entry_type *)proc_ref_entry_next;
                        proc_list_entry = nested_proc_list_entry->az_list;
                        emit_callback_procedures(
                            (sym_proc_ref_entry_type *)
                                proc_list_entry->obj_header.az_next,
                            proc_ref_index, callback_offset);
                        break;
                case sym_k_proc_ref_entry:
                        ...
                        if (proc_arg_entry == NULL) {
                                ...
                                arg_form =
                                    ref_value(proc_arg_entry, &arg_type,
                                              &arg_value, &arg_access,
                                              &arg_index, &arg_id, &arg_group);
                        }

                        if (arg_form == URMrImmediate)
                                urm_status = UrmCWRSetCallbackItem(
                                    out_az_context, callback_offset,
                                    *proc_ref_index,
                                    proc_def_entry->obj_header.az_name->c_text,
                                    arg_type, arg_value);
                        else
                                urm_status = UrmCWRSetCallbackItemRes(
                                    out_az_context, callback_offset,
                                    *proc_ref_index,
                                    proc_def_entry->obj_header.az_name->c_text,
                                    arg_group, arg_access, arg_type, arg_form,
                                    arg_index, arg_id);
...

{
        ...
        if (arg_name_entry->obj_header.b_flags & sym_m_builtin) {
                ...
                key_entry =
                    (key_keytable_entry_type *)arg_name_entry->value.l_integer;

                urm_status = UrmCWRSetCompressedArgTag(
                    out_az_context, arglist_index,
                    uil_arg_compr[key_entry->b_subclass],
                    uil_arg_compr
                        [related_argument_table[key_entry->b_subclass]]);
                ...
                if (arg_name_entry->obj_header.b_flags &
                    (sym_m_imported | sym_m_exported)) {
                        diag_issue_diagnostic(
                            d_not_impl, diag_k_no_source, diag_k_no_column,
                            "EXPORTED and IMPORTED arguments and reasons");
         ...
         * Else set it up as a literal or widget reference. For literal
         * references, the expected type must be set up in order to enable
         * coercion in Mrm.
         */
        arg_value_entry = argument_entry->az_arg_value;
        arg_form = ref_value(arg_value_entry, &arg_type, &arg_value,
                             &arg_access, &arg_index, &arg_id, &arg_group);
        if (arg_form == URMrImmediate)
                urm_status = UrmCWRSetArgValue(out_az_context, arglist_index,
                                               arg_type, arg_value);
        else {
                switch (arg_group) {
                case URMgLiteral:
                        if (argument_entry->az_arg_name->obj_header.b_flags &
                            sym_m_builtin) {
                                ...
                                keytable_entry =
                                    (key_keytable_entry_type *)argument_entry
                                        ->az_arg_name->value.l_integer;
                                _assert(keytable_entry->b_class ==
                                            tkn_k_class_argument,
                                        "name is not an argument");
                                expected_type =
                                    argument_type_table[keytable_entry
                                                            ->b_subclass];
                        } else
                                expected_type =
                                    argument_entry->az_arg_name->b_arg_type;
                        urm_status = UrmCWRSetArgResourceRef(
                            out_az_context, arglist_index, arg_access,
                            arg_group, Urm_code_from_uil_type(expected_type),
                            arg_form, arg_index, arg_id);
                        break;
                case URMgWidget:
                        urm_status = UrmCWRSetArgResourceRef(
                            out_az_context, arglist_index, arg_access,
                            arg_group, RGMwrTypeReference, arg_form, arg_index,
                            arg_id);
                        break;
                }
        }
...
#define sym_k_XmTabList_object 0
        ...
        while (widget_entry->obj_header.az_reference != NULL)
                widget_entry = (sym_widget_entry_type *)
                                   widget_entry->obj_header.az_reference;

        managed =
            ((widget_entry->header.b_type != sym_k_XmRenderTable_object) &&
        ...
        */
        ...
        urm_status = UrmCWRSetChild(out_az_context, control_offset, managed,
                                    access, form, index, id);
...
MrmCode ref_value(value_entry, arg_type, arg_value, arg_access, arg_index,
                  arg_id, arg_group)
...

{
        ...
        *arg_group = URMgLiteral;
        ...
        if (value_entry->header.b_tag == sym_k_widget_entry) {
                ...
                /*  Set up a dummy control entry, and process the widget
                 * reference.  */
                ...
                *arg_type = RGMwrTypeReference;
                ...
                return ref_control(&control_entry, arg_access, arg_index,
                                   arg_id);
        }
...

{
        ...
        sprintf(buffer, "while %s encountered %s", problem,
...

{
        ...
        switch (uil_type) {
        case sym_k_integer_value:
                return MrmRtypeInteger;
        case sym_k_horizontal_integer_value:
                return MrmRtypeHorizontalInteger;
        case sym_k_vertical_integer_value:
                return MrmRtypeVerticalInteger;
        case sym_k_bool_value:
                return MrmRtypeBoolean;
        case sym_k_char_8_value:
                return MrmRtypeChar8;
        case sym_k_localized_string_value:
                return MrmRtypeChar8;
        case sym_k_argument_value:
                return MrmRtypeChar8;
        case sym_k_reason_value:
                return MrmRtypeChar8;
        case sym_k_trans_table_value:
                return MrmRtypeTransTable;
        case sym_k_asciz_table_value:
                return MrmRtypeChar8Vector;
        case sym_k_string_table_value:
                return MrmRtypeCStringVector;
        case sym_k_compound_string_value:
                return MrmRtypeCString;
        case sym_k_wchar_string_value:
                return MrmRtypeWideCharacter;
        case sym_k_integer_table_value:
                return MrmRtypeIntegerVector;
        case sym_k_color_value:
                return MrmRtypeColor;
        case sym_k_color_table_value:
                return MrmRtypeColorTable;
        case sym_k_icon_value:
                return MrmRtypeIconImage;
        case sym_k_float_value:
                return MrmRtypeFloat;
        case sym_k_horizontal_float_value:
                return MrmRtypeHorizontalFloat;
        case sym_k_vertical_float_value:
                return MrmRtypeVerticalFloat;
        case sym_k_font_value:
                return MrmRtypeFont;
        case sym_k_fontset_value:
                return MrmRtypeFontSet;
        case sym_k_font_table_value:
                return MrmRtypeFontList;
        case sym_k_identifier_value:
                return MrmRtypeAddrName;
        case sym_k_class_rec_name_value:
                return MrmRtypeClassRecName;
        case sym_k_xbitmapfile_value:
                return MrmRtypeXBitmapFile;
        case sym_k_widget_ref_value:
                return MrmRtypeAny;
        case sym_k_pixmap_value:
                return MrmRtypeIconImage;
        case sym_k_any_value:
                return MrmRtypeAny;
        case sym_k_keysym_value:
                return MrmRtypeKeysym;
        case sym_k_single_float_value:
        ...
        for (i = 0; i < (int)table_entry->b_table_count; i++) {
                ...
                */
                ...
                if ((long)color_entry > 1) {
                        /*
                        **	Call ref_value for each of the colors in the
                        *color table. *	This will cause them to be created if
                        *necessary and also *	classify the type of resource
                        *needed.
                        */

                        table_entry->value.z_color[i].w_desc_offset = size;

                        switch (ref_value(color_entry, &arg_type, &arg_value,
                                          &arg_access, &arg_index, &arg_id,
                                          &arg_group)) {
                        case URMrRID:
                        ...
                        default:
                                _assert(FALSE,
                                        "immediate color values not supported");
                        }
                }
        }
...

{
        ...
        */
        ...
        for (i = 0; i < (int)table_entry->b_table_count; i++) {
                ...
                desc =
                    (RGMResourceDesc *)(buffer + table_entry->value.z_color[i]
                                                     .w_desc_offset);
                ...
                */
                ...
                if (index > 1) {
                        ...
                        /*
                        **	Call ref_value for each of the colors in the
                        *color table. *	This provide the necessary info to fill
                        *in the resource *	descriptor
                        */

                        arg_form =
                            ref_value(table_entry->value.z_color[i].az_color,
...

{
        ...
        */
        ...
        switch (ref_value(icon_entry->value.z_icon->az_color_table, &arg_type,
                          &arg_value, &arg_access, &arg_index, &arg_id,
                          &arg_group)) {
        case URMrRID:
                size += sizeof(RGMResourceDesc);
                break;
        ...
        */
        ...
        /*
        **	Size is width * height - each row must be an even number of
        *bytes
        */

        size +=
            ((int)((icon_entry->value.z_icon->w_width << pixel_type) + 7) / 8) *
...

{
        ...
        */
        ...
        arg_form =
            ref_value(icon_entry->value.z_icon->az_color_table, &arg_type,
                      &arg_value, &arg_access, &arg_index, &arg_id, &arg_group);
        ...
        case URMrIndex:
                desc->size = strlen(arg_index) + 1;
                _move(desc->key.index, arg_index, desc->size);
                desc->size += sizeof(RGMResourceDesc) - sizeof(MrmResource_id);
                break;
        default:
        ...
        */
        ...
        for (row_entry = icon_entry->value.z_icon->az_rows,
            w_len = ((int)row_entry->w_length / (int)pixel_per_byte) *
                    pixel_per_byte,
            p_len = row_entry->w_length - w_len;
             ...
             row_entry = row_entry->az_next_table_value) {
                ...
                for (i = 0; i < w_len; tbyte++) {
                        for (*tbyte = 0, j = 0; j < 8; j += pixel_size) {
                                unsigned char t;
                ...
                if (p_len > 0) {
                        for (*tbyte = 0, j = 0; j < (p_len * pixel_size);
                             j += pixel_size) {
                                unsigned char t;
...
int compute_list_size(list_entry, type) sym_list_entry_type *list_entry;
int type;
             ...
             list_member =
                ...
                case sym_k_nested_list_entry:
                        nested_list_entry =
                            (sym_nested_list_entry_type *)list_member;
                        count +=
                            compute_list_size(nested_list_entry->az_list, type);
                        break;
                default:
...
void create_ext_compression_codes() {
        ...
        */
        ...
        /*
        ** Set up the resource context and point extern_arg_compr to the
        *resource
        ** context buffer.
        */
        ...
        for (i = 0; i <= uil_max_child; i++) {
                if (uil_child_compr[i] != 0) {
                        ...
                        if (strncmp(uil_child_names[i], AUTO_CHILD_PREFIX,
                                ...
                                name = (uil_child_names[i] +
                                        strlen(AUTO_CHILD_PREFIX));
                        else
                                name = uil_child_names[i];

                        _move(&(arg_buffer[text_offset]), name,
                              strlen(name) + 1);
                        extern_arg_compr->entry[comp_code].stoffset =
                            text_offset;
        ...
        */
        ...
        /*
        ** Set up the resource context and point extern_class_compr to the
        *resource
        ** context buffer.
        */
        ...
        */
        ...
        for (i = 0; i <= uil_max_object; i++) {
                if (uil_widget_compr[i] != 0) {
                        _move(&(class_buffer[text_offset]), uil_widget_funcs[i],
                              strlen(uil_widget_funcs[i]) + 1);
        ...
        */
        urm_status = UrmPutIndexedLiteral(
            out_az_idbfile_id, UilMrmClassTableIndex, out_az_context);
        if (urm_status != MrmSUCCESS) {
                if (urm_status == MrmEOF)
                        diag_issue_diagnostic(d_uid_write, diag_k_no_source,
                                              diag_k_no_column,
