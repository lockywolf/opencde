        ...
        (ASNHEADERLEN +                                                        \
         (((unsigned char)(a)[ASNHEADERLEN] & CSLONGBIT)                       \
              ? (((unsigned short)((a)[ASNHEADERLEN + 1]) << 8) |              \
                 (unsigned short)((a)[ASNHEADERLEN + 2])) +                    \
                    3                                                          \
...
#define _WROffset(field) (_Offset(RGMWidgetRecord, 0, field))
        ...
        src_append_machine_code(az_src_rec, _WROffset(validation),
                                sizeof(w_rec->validation),
                                (char *)&w_rec->validation, "widget record");

        sprintf(buffer, "size: %d", w_rec->size);

        src_append_machine_code(az_src_rec, _WROffset(size),
                                sizeof(w_rec->size), (char *)&w_rec->size,
                                buffer);

        w_rec_size = w_rec->size;

        sprintf(buffer, "access: %s", access_from_code(w_rec->access));

        src_append_machine_code(az_src_rec, _WROffset(access),
        ...
        src_append_machine_code(az_src_rec, _WROffset(lock),
        ...
        src_append_machine_code(az_src_rec, _WROffset(type),
        ...
        if (w_rec->name_offs == (MrmOffset)0) {
                ...
                sprintf(buffer, "name: offset %X (hex)", w_rec->name_offs);
                off_put(k_name_off, w_rec->name_offs);
        }

        src_append_machine_code(az_src_rec, _WROffset(name_offs),
                                sizeof(w_rec->name_offs),
                                (char *)&w_rec->name_offs, buffer);

        if (w_rec->class_offs == (MrmOffset)0) {
                sprintf(buffer, "class: builtin");
        } else {
                sprintf(buffer, "class: offset %X (hex)", w_rec->class_offs);
                off_put(k_class_off, w_rec->class_offs);
        }

        src_append_machine_code(az_src_rec, _WROffset(class_offs),
                                sizeof(w_rec->class_offs),
                                (char *)&w_rec->class_offs, buffer);

        if (w_rec->arglist_offs == (MrmOffset)0) {
                sprintf(buffer, "no argument list");
        } else {
                sprintf(buffer, "argument list offset: %X (hex)",
                        w_rec->arglist_offs);
                off_put(k_arglist_off, w_rec->arglist_offs);
        }

        src_append_machine_code(az_src_rec, _WROffset(arglist_offs),
                                sizeof(w_rec->arglist_offs),
                                (char *)&w_rec->arglist_offs, buffer);
        ...
        src_append_machine_code(az_src_rec, _WROffset(children_offs),
                                sizeof(w_rec->children_offs),
                                (char *)&w_rec->children_offs, buffer);
        ...
        src_append_machine_code(az_src_rec, _WROffset(comment_offs),
                                sizeof(w_rec->comment_offs),
                                (char *)&w_rec->comment_offs, buffer);
        ...
        src_append_machine_code(az_src_rec, _WROffset(creation_offs),
                                sizeof(w_rec->creation_offs),
                                (char *)&w_rec->creation_offs, buffer);
...

{
        ...
        while (!off_stack_empty) {
                ...
                case k_unknown_off: {
                        ...
                        /*	Write out everything up to the next offset or
                        ...
                        ptr = (char *)_Pointer(rec, off_offset);
                        src_append_machine_code(az_src_rec, off_offset, len,
                                                ptr, NULL);
                        break;
                }
                ...
                case k_arglist_off: {
                        ...
                        src_append_machine_code(
                            ...
                            sizeof(argdesc->count), (char *)&argdesc->count,
                            buffer);

                        sprintf(buffer, "related argument count: %d",
                                argdesc->extra);

                        src_append_machine_code(
                            az_src_rec,
                            _Offset(RGMArgListDesc, off_offset, extra),
                            sizeof(argdesc->extra), (char *)&argdesc->extra,
                            buffer);

                        off_offset =
                            _OffsetArray(RGMArgListDesc, off_offset, args);

                        for (j = 0, arg_ptr = argdesc->args; j < argdesc->count;
                             j++, arg_ptr++,
                            off_offset += sizeof(RGMArgument)) {

                                if (arg_ptr->tag_code != UilMrmUnknownCode) {
                                        sprintf(buffer, "(%d) arg type: %s", j,
                                                resource_name_from_code(
                                                    arg_ptr->tag_code));
                                } else {
                                        ptr = (char *)_Pointer(
                                            rec,
                                            arg_ptr->stg_or_relcode.tag_offs);
                                        sprintf(
                                            buffer,
                                ...
                                src_append_machine_code(
                                    ...
                                    (char *)&arg_ptr->tag_code, buffer);

                                if (arg_ptr->stg_or_relcode.tag_offs ==
                                    (MrmOffset)0) {
                                        sprintf(buffer, "(%d) no tag offset",
                                                j);
                                } else {
                                        sprintf(
                                            buffer, "(%d) tag offset: %X (hex)",
                                            j,
                                            arg_ptr->stg_or_relcode.tag_offs);
                                        off_put(
                                            k_name_off,
                                ...
                                sprintf(
                                    buffer, "(%d) type: %s", j,
                                    type_from_code(arg_ptr->arg_val.rep_type));
                                ...
                                src_append_machine_code(
                                    ...
                                    (char *)&arg_ptr->arg_val.rep_type, buffer);
                                ...
                                src_append_machine_code(
                                    ...
                                    (char *)&arg_ptr->arg_val.datum, buffer);
                        }
                ...
                case k_callback_off: {
                        ...
                        sprintf(buffer, "callback count: %d",
                                cb_desc_ptr->count);
                        ...
                        src_append_machine_code(
                            ...
                            (char *)&cb_desc_ptr->count, buffer);

                        off_offset =
                            _OffsetArray(RGMCallbackDesc, off_offset, item);

                        for (j = 0, cb_item_ptr = cb_desc_ptr->item;
                             j < cb_desc_ptr->count; j++, cb_item_ptr++,
                            off_offset += sizeof(RGMCallbackItem)) {

                                sprintf(buffer,
                                        "(%d) routine name offset: %X (hex)", j,
                                        cb_item_ptr->cb_item.routine);
                                ...
                                sprintf(buffer, "(%d) routine tag type: %s", j,
                                        type_from_code(
                ...
                case k_resource_off: {
                        ...
                        src_append_machine_code(
                            ...
                            sizeof(r_desc_ptr->size), (char *)&r_desc_ptr->size,
                            buffer);
                        ...
                        src_append_machine_code(
                            ...
                            (char *)&r_desc_ptr->access, buffer);

                        switch (r_desc_ptr->type) {
                        case URMrIndex:
                                sprintf(
                                    buffer, "index, offset: %X (hex)",
                                    _Offset(RGMResourceDesc, off_offset, key));

                                off_put(k_name_off, _Offset(RGMResourceDesc,
                                                            off_offset, key));
                                break;
                        ...
                        case URMrRID:
                                ...
                                sprintf(
                                    buffer, "resource ID, offset: %X (hex)",
                                    _Offset(RGMResourceDesc, off_offset, key));
                        ...
                        src_append_machine_code(
                            ...
                            sizeof(r_desc_ptr->type), (char *)&r_desc_ptr->type,
                            buffer);
                        ...
                        src_append_machine_code(
                            ...
                            (char *)&r_desc_ptr->res_group, buffer);

                        sprintf(
                            buffer, "resource type: %s",
                        ...
                        src_append_machine_code(
                            ...
                            (char *)&r_desc_ptr->cvt_type, buffer);
                ...
                case k_resource_id_off: {
                        ...
                        src_append_machine_code(
                            az_src_rec, off_offset, sizeof(MrmResource_id),
                            (char *)_Pointer(rec, off_offset), buffer);
                ...
                case k_children_off: {
                        ...
                        c_desc_ptr =
                            (RGMChildrenDescPtr)_Pointer(rec, off_offset);
                ...
                case k_child_off: {
                        ...
                        src_append_machine_code(
                            ...
                            sizeof(c_ptr->manage), (char *)&c_ptr->manage,
                            buffer);
                        ...
                        src_append_machine_code(
                            ...
                            sizeof(c_ptr->access), (char *)&c_ptr->access,
                            buffer);
                        ...
                        case URMrRID:
                                sprintf(buffer,
                                        "(%d) resource ID, offset: %X (hex)", j,
                                        _Offset(RGMChildDesc, off_offset, key));

                                src_append_machine_code(
                                    az_src_rec,
                                    _Offset(RGMChildDesc, off_offset, type),
                                    sizeof(c_ptr->type), (char *)&c_ptr->type,
                                    buffer);
...

{
        ...
        sprintf(buffer, "size: %d, group: %s", UrmRCSize(az_context),
        ...
        */
        ...
        case sym_k_bool_value:
                ...
                src_append_machine_code(az_src_rec, 0, rc_size, rc_buffer,
                                        buffer);
                break;
        ...
        case sym_k_vertical_integer_value:
                ...
                sprintf(buffer, "value: %d", (*(unsigned long *)rc_buffer));
                src_append_machine_code(az_src_rec, 0, rc_size, rc_buffer,
                                        buffer);
                break;
        ...
        case sym_k_rgb_value: /* rgb data type RAP */
        {
                ...
                for (offset = 0; offset < (int)rc_size;
                     offset += sizeof(int), index++) {
                        sprintf(buffer, "value[%d]: %d", index,
                                ((unsigned long *)rc_buffer)[index]);

                        src_append_machine_code(
                            az_src_rec, 0, sizeof(int),
                            (char *)&(((unsigned long *)rc_buffer)[index]),
                            buffer);
                }
                break;
        }
        ...
        case sym_k_vertical_float_value:
                sprintf(buffer, "value: %g", (*(double *)rc_buffer));
                src_append_machine_code(az_src_rec, 0, rc_size, rc_buffer,
                                        buffer);
                break;

        case sym_k_single_float_value:
                sprintf(buffer, "value: %g", (*(float *)rc_buffer));
                src_append_machine_code(az_src_rec, 0, rc_size, rc_buffer,
                                        buffer);
                break;
        ...
        case sym_k_keysym_value:
                src_append_machine_code(az_src_rec, 0, rc_size, rc_buffer, 0);
                break;
        ...
        case sym_k_string_table_value: {
                ...
                src_append_machine_code(
                    az_src_rec, _Offset(RGMTextVector, off_offset, validation),
                    sizeof(tv_ptr->validation), (char *)&tv_ptr->validation,
                    "text vector");

                sprintf(buffer, "count: %d", tv_ptr->count);
                src_append_machine_code(
                    az_src_rec, _Offset(RGMTextVector, off_offset, count),
                    sizeof(tv_ptr->count), (char *)&tv_ptr->count, buffer);
                ...
                while (!off_stack_empty) {
                        ...
                        case k_text_entry_off:
                                ...
                                te_ptr = (RGMTextEntryPtr)_Pointer(rc_buffer,
                                                                   off_offset);

                                sprintf(
                                    buffer, "(%d) type: %s, offset: %X (hex)",
                                    off_cnt,
...

{
        ...
        src_append_machine_code(az_src_rec,
                                _Offset(RGMModuleDesc, off_offset, validation),
                                sizeof(m_rec->validation),
                                (char *)&m_rec->validation, "module record");

        sprintf(buffer, "topmost count: %d", m_rec->count);

        src_append_machine_code(
            az_src_rec, _Offset(RGMModuleDesc, off_offset, count),
            sizeof(m_rec->count), (char *)&m_rec->count, buffer);

        off_offset = _OffsetArray(RGMModuleDesc, off_offset, topmost);

        for (i = 0, t_rec = m_rec->topmost; i < m_rec->count;
             i++, t_rec++, off_offset += sizeof(RGMTopmostDesc)) {

                sprintf(buffer, "(%d) %s", i, t_rec->index);

                src_append_machine_code(
                    az_src_rec, _OffsetArray(RGMTopmostDesc, off_offset, index),
                    strlen(t_rec->index), t_rec->index, buffer);
...

{
        ...
        for (i = 0, off_info_ptr = off_info_stack; i < off_info_cnt;
             i++, off_info_ptr++) {
...

{
         ...
         */
        for (sym_code = sym_k_error_object + 1; sym_code <= uil_max_object;
             sym_code++)
