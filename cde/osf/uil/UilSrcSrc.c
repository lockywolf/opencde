...
#include <sys/stat.h>
                      ...
                      */
                      ...
                      externaldef(uil_comp_glbl) src_source_buffer_type *
...
externaldef(uil_comp_glbl) src_source_record_type *src_az_current_source_record;
externaldef(uil_comp_glbl) src_source_record_type *src_az_first_source_record;

externaldef(uil_comp_glbl)
    uil_fcb_type *src_az_source_file_table[src_k_max_source_files];
externaldef(uil_comp_glbl) int src_l_last_source_file_number;
...
void src_initialize_source(void) {
        ...
        if (Uil_cmd_z_command.ac_source_file == NULL)
                diag_issue_diagnostic(d_src_open, diag_k_no_source,
                                      diag_k_no_column, "<null file name>");
...
void Uil_src_cleanup_source(void) {
        ...
        for (i = 0; i <= src_l_last_source_file_number; i++) {
                ...
                if (l_close_status == src_k_close_error) {
                        diag_issue_diagnostic(
                            d_src_close, diag_k_no_source, diag_k_no_column,
...
void src_open_file(XmConst char *c_file_name, char *full_file_name) {
        ...
        if (src_l_last_source_file_number >= src_k_max_source_files) {
                diag_issue_diagnostic(
                    d_src_limit, src_az_current_source_record,
        ...
        */
        ...
        if (stat(az_fcb->expanded_name, &stbuf) == -1) {
                diag_issue_diagnostic(d_src_open, diag_k_no_source,
                                      diag_k_no_column, az_fcb->expanded_name);
        }
        ...
        az_source_buffer->az_prior_source_buffer = src_az_current_source_buffer;
...
status src_get_source_line(void) {
        uil_fcb_type *az_fcb;
        ...
        az_fcb = src_az_source_file_table[src_az_current_source_buffer
                                              ->b_file_number];
        ...
        if ((l_read_status == src_k_read_normal) ||
            (l_read_status == src_k_read_truncated)) {
                ...
                az_source_record = (src_source_record_type *)_get_memory(
                ...
                if (l_read_status == src_k_read_truncated)
                        diag_issue_diagnostic(
                            d_src_truncate, src_az_current_source_record,
                            diag_k_no_column, src_k_max_source_line_length);
        ...
        diag_issue_diagnostic(d_src_read, src_az_current_source_record,
                              diag_k_no_column, az_fcb->expanded_name);
...
status open_source_file(XmConst char *c_file_name, uil_fcb_type *az_fcb,
        ...
        if (main_file) {
                ...
                for (len = strlen(c_file_name), ptr = &c_file_name[len - 1];
                     len > 0; len--, ptr--) {
                        if ((*ptr) == '/') {
                                break;
                        }
                ...
                /*    See if the file name has a leading slash and set the flag.
                      Look in the specified directory for the include file.  If
                   the dir is not specified (leading slash), look in the main
                   file's directory */
                ...
                if (!specific_directory) {
                        ...
                        _move(&buffer[main_dir_len], c_file_name,
                              strlen(c_file_name) + 1); /* + NULL */
                } else {
                        strcpy(buffer, c_file_name);
                }
                ...
                /*    If a specific directory was specified, or if the file was
                ...
                /*    Look in the command line specified include directories, if
                 * any.    */
                ...
                for (i = 0; i < Uil_cmd_z_command.include_dir_count; i++) {
                        ...
                        inc_dir_len =
                            strlen(Uil_cmd_z_command.ac_include_dir[i]);
                        if (inc_dir_len == 0) {
                                search_user_include = False;
                        }
                        _move(buffer, Uil_cmd_z_command.ac_include_dir[i],
                              inc_dir_len);

                        /*  Add '/' if not specified at end of directory  */

                        if (Uil_cmd_z_command
                                .ac_include_dir[i][inc_dir_len - 1] != '/') {
                                buffer[inc_dir_len] = '/';
                                inc_dir_len++;
                        };

                        _move(&buffer[inc_dir_len], c_file_name,
                              strlen(c_file_name) + 1); /* + NULL */
                ...
                if (search_user_include) {
                        _move(buffer, c_include_dir,
                              sizeof c_include_dir - 1); /* no NULL */
                        _move(&buffer[sizeof c_include_dir - 1], c_file_name,
                              strlen(c_file_name) + 1); /* + NULL */
...
status close_source_file(uil_fcb_type *az_fcb) {
        status l_close_status;
...
status get_line(uil_fcb_type *az_fcb) {
        status l_read_status;
        char *c_new_line;
        ...
        */
        ...
        if (az_fcb->v_position_before_get) {
                fseek(az_fcb->az_file_ptr, az_fcb->last_key.l_key, 0);
        ...
        if (l_read_status != 0) {
                ...
                if (c_new_line == NULL) {
                        ...
                        /* Fix for CR 3044 -- only return truncated if not at
                         * eof */
        ...
        if (feof(az_fcb->az_file_ptr)) {
                if (sym_az_current_section_entry->prev_section != NULL) {
                        ...
                        /*
                        ** This is the end of an include file.  Set the pointers
                        *so that the sections
                        ** in the include file hang off the previous list
                        *correctly.
                        */
                        include_entry = (sym_include_file_entry_type *)
                                            sym_az_current_section_entry
                                                ->prev_section->entries;
                        include_entry->sections = sym_az_current_section_entry;
                        sym_az_current_section_entry =
                            sym_az_current_section_entry->prev_section;
                }
                return src_k_end_source;
        }
...
boolean reget_line(uil_fcb_type *az_fcb, char *c_buffer,
                   XmConst z_key *z_access_key) {
        status l_read_status;
        char *c_new_line;

        fseek(az_fcb->az_file_ptr, z_access_key->l_key, 0);

        l_read_status = (status)(fgets(c_buffer, src_k_max_source_line_length,
                                       az_fcb->az_file_ptr) != NULL);
...
char *src_get_file_name(XmConst src_source_record_type *az_src_rec) {
        uil_fcb_type *fcb;
...
boolean src_retrieve_source
    ...
    (XmConst src_source_record_type *az_src_rec, char *c_buffer)
        ...
        */
        ...
        if ((az_src_rec->b_file_number ==
            ...
            (az_src_rec->w_line_number ==
             src_az_current_source_buffer->w_current_line_number)) {
                strcpy(c_buffer, src_az_current_source_buffer->c_text);
                return TRUE;
        }
...
void src_append_diag_info(XmConst src_source_record_type *az_src_rec,
                          XmConst int l_src_pos, XmConst char *c_msg_text,
        ...
        */
        ...
        az_msg_item = (src_message_item_type *)_get_memory(
            sizeof(src_message_item_type) + l_msg_length);
        ...
        */
        ...
        if (az_src_rec == diag_k_no_source)
                ...
                prior =
                    (src_message_item_type **)&(az_src_rec->az_message_list);

        current = *prior;

        for (; current != NULL;
             prior = &(current->az_next_message), current = *prior) {
...
void src_append_machine_code(src_source_record_type *az_src_rec,
                             XmConst int l_offset, XmConst int l_code_len,
                             XmConst char *c_code, XmConst char *c_text_arg) {
