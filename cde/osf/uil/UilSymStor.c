...
externaldef(uil_comp_glbl)
    sym_value_entry_type *sym_az_error_value_entry = NULL;
externaldef(uil_comp_glbl)
    sym_external_def_entry_type *sym_az_external_def_chain;
externaldef(uil_comp_glbl) sym_forward_ref_entry_type *sym_az_forward_ref_chain;
externaldef(uil_comp_glbl)
    sym_val_forward_ref_entry_type *sym_az_val_forward_ref_chain;
externaldef(uil_comp_glbl) sym_module_entry_type *sym_az_module_entry;
externaldef(uil_comp_glbl) sym_root_entry_type *sym_az_root_entry;
externaldef(uil_comp_glbl) sym_section_entry_type *sym_az_current_section_entry;
externaldef(uil_comp_glbl) sym_entry_type *sym_az_entry_list_header;
...
void sym_initialize_storage(void) {
        ...
        /*
         * Allocate a value entry for an error value and give it a name.  Giving
         * it name assures that it is not freed.  The name used is one that will
         * not conflict with a user name.
         */

        if (sym_az_error_value_entry == NULL)
                sym_az_error_value_entry =
                    (sym_value_entry_type *)sem_allocate_node(
...
void Uil_sym_cleanup_storage(boolean freealloc) {
        if (freealloc)
                ...
                else if (sym_az_freed_nodes != NULL)
...
void sym_make_external_def(XmConst sym_name_entry_type *az_name)

{
        ...
        external_entry = (sym_external_def_entry_type *)sem_allocate_node(
            sym_k_external_def_entry, sym_k_external_def_entry_size);
...
void sym_make_forward_ref(XmConst yystype *az_id_frame,
                          XmConst int l_widget_type, XmConst char *a_location) {
        ...
        _assert((az_id_frame->b_tag == sar_k_token_frame) &&
                    (az_id_frame->value.az_symbol_entry->header.b_tag ==
                     sym_k_name_entry),
                "arg1 not an id frame");

        /* allocate an external definition entry */

        fwd_ref_entry = (sym_forward_ref_entry_type *)sem_allocate_node(
            sym_k_forward_ref_entry, sym_k_forward_ref_entry_size);
...
void sym_make_value_forward_ref(XmConst yystype *az_value_frame,
...

{
        ...
        fwd_ref_entry = (sym_val_forward_ref_entry_type *)sem_allocate_node(
            sym_k_val_forward_ref_entry, sym_k_val_forward_ref_entry_size);
        ...
        case sym_k_patch_add:
                fwd_ref_entry->az_name =
                    ((sym_value_entry_type *)
                         az_value_frame->value.az_symbol_entry)
                        ->obj_header.az_name;
                break;
...
void UilDumpSymbolTable(sym_entry_type *node_entry) {
        ...
        case sym_k_value_entry:
                ...
                UilDumpSymbolTable(
                    (sym_entry_type *)val_node->az_charset_value);
                UilDumpSymbolTable(
                    (sym_entry_type *)val_node->az_first_table_value);
                UilDumpSymbolTable(
                    (sym_entry_type *)val_node->az_next_table_value);
        ...
        case sym_k_child_entry:
                ...
                UilDumpSymbolTable(
                    (sym_entry_type *)widget_node->az_create_proc);
                break;
        case sym_k_module_entry:
                ...
                UilDumpSymbolTable(
                    (sym_entry_type *)module_node->az_character_set);
                UilDumpSymbolTable(
                    (sym_entry_type *)module_node->az_case_sense);
        ...
        case sym_k_list_entry:
                 ...
                 * us can possibly be encountered only in a nested list
                 * reference.
                 */
                ...
                for (list_entry =
                     ...
                     list_entry != NULL;
                     list_entry =
                         (sym_obj_entry_type *)list_entry->obj_header.az_next)
        ...
        case sym_k_section_entry:
                ...
                default:
                        UilDumpSymbolTable(
                            (sym_entry_type *)section_node->next);
                        UilDumpSymbolTable(
                            (sym_entry_type *)section_node->entries);
                        break;
                }
                break;
        }
...
void sym_dump_symbols(void) {
        ...
        for (ndx = 0; ndx < UrmPlistNum(sym_az_allocated_nodes); ndx++) {
                cur_entry =
                    (sym_entry_type *)UrmPlistPtrN(sym_az_allocated_nodes, ndx);
...
void sym_dump_symbol(sym_entry_type *az_symbol_entry) {
        ...
        case sym_k_forward_ref_entry:
                sym_dump_forward_ref(
                    (sym_forward_ref_entry_type *)az_symbol_entry);
                break;

        case sym_k_external_def_entry:
                sym_dump_external_def(
                    (sym_external_def_entry_type *)az_symbol_entry);
                break;
        ...
        case sym_k_color_item_entry:
                sym_dump_color_item(
                    (sym_color_item_entry_type *)az_symbol_entry);
                break;

        case sym_k_parent_list_entry:
                sym_dump_parent_list_item(
                    (sym_parent_list_type *)az_symbol_entry);
                break;

        case sym_k_include_file_entry:
                sym_dump_include_file(
...
void sym_dump_widget(XmConst sym_widget_entry_type *az_widget_entry) {

        sym_dump_obj_header((sym_obj_entry_type *)az_widget_entry);

        _debug_output(
            "  %s %s  controls: %x  callbacks: %x  arguments: %x  parent_list: "
            "%x\n",
            diag_object_text(az_widget_entry->header.b_type),
            diag_tag_text(az_widget_entry->header.b_tag),
            az_widget_entry->az_controls, az_widget_entry->az_callbacks,
            az_widget_entry->az_arguments, az_widget_entry->parent_list);
...
void sym_dump_argument(XmConst sym_argument_entry_type *az_argument_entry) {
...
void sym_dump_control(XmConst sym_control_entry_type *az_control_entry) {
...
void sym_dump_callback(XmConst sym_callback_entry_type *az_callback_entry) {
...
void sym_dump_list(XmConst sym_list_entry_type *az_list_entry) {
        ...
        _debug_output("  type: %s  count: %d  gadget count: %d\n",
                      ...
                      az_list_entry->w_count, az_list_entry->w_gadget_count);
...
void sym_dump_name(XmConst sym_name_entry_type *az_name_entry) {

        _debug_output("%x name size: %d  next name: %x  object: %x",
                      az_name_entry, az_name_entry->header.w_node_size,
...
void sym_dump_module(XmConst sym_module_entry_type *az_module_entry) {

        _debug_output("%x module size: %d  name: %x  version: %x \n",
                      az_module_entry, az_module_entry->header.w_node_size,
...
void sym_dump_color_item(
    XmConst sym_color_item_entry_type *az_color_item_entry) {

        _debug_output(
            "%x color_item  size: %d  letter: %c  index: %d  color: %x  next: "
            "%x\n",
            az_color_item_entry, az_color_item_entry->header.w_node_size,
            az_color_item_entry->b_letter, az_color_item_entry->b_index,
            az_color_item_entry->az_color, az_color_item_entry->az_next);
}
...
void sym_dump_parent_list_item(
    XmConst sym_parent_list_type *az_parent_list_item) {
        _debug_output("%x parent_list  size: %d  parent: %x  next: %x \n",
                      az_parent_list_item,
                      az_parent_list_item->header.w_node_size,
                      az_parent_list_item->parent, az_parent_list_item->next);
}
...
void sym_dump_proc_def(XmConst sym_proc_def_entry_type *az_proc_def_entry) {
        ...
        _debug_output(
            ...
            az_proc_def_entry, az_proc_def_entry->header.w_node_size,
            az_proc_def_entry->obj_header.az_name, checking_flag, private_flag,
            exported_flag, imported_flag, az_proc_def_entry->b_arg_count,
...
void sym_dump_proc_ref(XmConst sym_proc_ref_entry_type *az_proc_ref_entry) {

        sym_dump_obj_header((sym_obj_entry_type *)az_proc_ref_entry);

        _debug_output("%x proc ref  size: %d  proc def: %x  value: %x\n",
                      az_proc_ref_entry, az_proc_ref_entry->header.w_node_size,
...
void sym_dump_forward_ref(
    XmConst sym_forward_ref_entry_type *az_forward_ref_entry) {

        _debug_output("%x forward ref  size: %d  next ref: %x  location: %x  "
                      "%s  parent: %x\n",
...
void sym_dump_value(XmConst sym_value_entry_type *az_value_entry) {
        ...
        _debug_output("%x value  size: %d  name: %x  %s%s%s%s", az_value_entry,
                      az_value_entry->header.w_node_size,
                      az_value_entry->obj_header.az_name, builtin_flag,
                      private_flag, exported_flag, imported_flag);
        ...
        case sym_k_fontset_value:
                if (az_value_entry->b_charset != sym_k_userdefined_charset)
                        ...
                        _debug_output(
                            "  font  charset: userdefined(%x)",
                            diag_charset_text(
                                (long)az_value_entry->az_charset_value));
        ...
        case sym_k_localized_string_value:
                if (az_value_entry->b_charset != sym_k_userdefined_charset)
                        ...
                        case XmSTRING_DIRECTION_L_TO_R:
                                _debug_output("  string length: %d\n  charset: "
                                              "userdefined(%x)  L_TO_R",
                                              az_value_entry->w_length,
                                              az_value_entry->az_charset_value);
                                break;
                        case XmSTRING_DIRECTION_R_TO_L:
                                _debug_output("  string length: %d\n  charset: "
                                              "userdefined(%x)  R_TO_L",
                                              az_value_entry->w_length,
                                              az_value_entry->az_charset_value);
                                break;
                        }
        ...
        case sym_k_icon_value:
                _debug_output(
                    "  icon  width: %d  height: %d  colors: %x  rows: %x \n",
...
void output_text(XmConst int length, XmConst char *text) {
...
void sym_dump_source_info(sym_entry_header_type *hdr) {
        ...
        if (src_rec != NULL)
                _debug_output("  Source position:	file %d, line %d, "
                              "columns %d through %d\n",
                              src_rec->b_file_number, src_rec->w_line_number,
                              hdr->b_src_pos, hdr->b_end_pos);
        else
...
void sym_dump_obj_header(XmConst sym_obj_entry_type *az_obj_entry) {

        _debug_output("%x %s  size: %d  \n", az_obj_entry,
...
void sym_dump_include_file(sym_include_file_entry_type *az_symbol_entry) {

        _debug_output("%x  include file  file name: %s  full file name: %s\n",
                      az_symbol_entry, az_symbol_entry->file_name,
                      az_symbol_entry->full_file_name);
}

void sym_dump_section(sym_section_entry_type *az_symbol_entry) {
        _debug_output("%x  %s section  prev section : %x  next section: %x  "
                      "entries: %x\n",
                      az_symbol_entry,
...
void sym_dump_object_variant(sym_def_obj_entry_type *az_symbol_entry) {
        _debug_output("%x  default obj var  next: %x  object info: %d, "
                      "variant_info: %d\n",
                      az_symbol_entry, az_symbol_entry->next,
                      az_symbol_entry->b_object_info,
                      az_symbol_entry->b_variant_info);
}

void sym_dump_root_entry(sym_root_entry_type *az_symbol_entry) {
        _debug_output(
            "%x  root  tag: %d  module: %x  sections: %x\n  module tail: ",
            az_symbol_entry, az_symbol_entry->header.b_tag,
            az_symbol_entry->module_hdr, az_symbol_entry->sections);
}

char *sym_section_text(int b_type) {
        switch (b_type) {
        case 0:
                return "";
