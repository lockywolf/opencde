...
#define _check_read(__number_returned)                                         \
        if (((__number_returned) != 1) || (feof(dbfile)) ||                    \
            (ferror(dbfile))) {                                                \
                diag_issue_diagnostic(d_bad_database, diag_k_no_source,        \
                                      diag_k_no_column);                       \
        }
 ...
 */
{
         ...
         */
        if (globals.uil_max_arg > 5000 || globals.uil_max_charset > 200 ||
            globals.charset_lang_table_max > 1000 ||
            globals.uil_max_object > 500 || globals.uil_max_reason > 1000 ||
            globals.uil_max_enumval > 3000 || globals.uil_max_enumset > 1000 ||
        ...
        if (globals.version > DB_Compiled_Version)
                diag_issue_diagnostic(d_future_version, diag_k_no_source,
                                      diag_k_no_column);

        for (;;) {
                return_num_items =
                    fread(&header, sizeof(_db_header), 1, dbfile);
                if (feof(dbfile))
                        break;
                _check_read(return_num_items);
                ...
                case Constraint_Tab:
                        ...
                        return_num_items =
                            fread(constraint_tab,
                                  sizeof(unsigned char) * header.num_items, 1,
                                  dbfile);
                        _check_read(return_num_items);
                        break;
                case Argument_Type_Table_Value:
                        /*
                         * NOTE: The first entry is not used but we copy it
                         * anyway
                         */
                        argument_type_table =
                            (unsigned char *)XtMalloc(header.table_size);
                        return_num_items =
                            fread(argument_type_table,
                                  sizeof(unsigned char) * header.num_items, 1,
                                  dbfile);
                        _check_read(return_num_items);
                        break;
                ...
                case Charset_Wrdirection_Table:
                        charset_writing_direction_table =
                            (unsigned char *)XtMalloc(header.table_size);
                        return_num_items =
                            fread(charset_writing_direction_table,
                                  sizeof(unsigned char) * header.num_items, 1,
                                  dbfile);
                        _check_read(return_num_items);
                        break;
                case Charset_Parsdirection_Table:
                        charset_parsing_direction_table =
                            (unsigned char *)XtMalloc(header.table_size);
                        return_num_items =
                            fread(charset_parsing_direction_table,
                                  sizeof(unsigned char) * header.num_items, 1,
                                  dbfile);
                        _check_read(return_num_items);
                        break;
                case Charset_Charsize_Table:
                        charset_character_size_table =
                            (unsigned char *)XtMalloc(header.table_size);
                        return_num_items =
                            fread(charset_character_size_table,
                                  sizeof(unsigned char) * header.num_items, 1,
                                  dbfile);
                        _check_read(return_num_items);
                        break;
                ...
                case Charset_Lang_Codes_Table:
                        charset_lang_codes_table =
                            (unsigned short int *)XtMalloc(header.table_size);
                        return_num_items = fread(charset_lang_codes_table,
                                                 header.table_size, 1, dbfile);
                        _check_read(return_num_items);
                        break;
                case Argument_Enum_Set_Table:
                        argument_enumset_table =
                            (unsigned short int *)XtMalloc(header.table_size);
                ...
                case Enumval_Values_Table:
                        enumval_values_table =
                            (int *)XtMalloc(header.table_size);
...
void db_read_ints_and_string(header) _db_header_ptr header;
 ...
 *  PROCEDURE DESCRIPTION:
 *
 *	This routine reads in tables of integers and one string unsigned chars
 *and places them into memory. It will Malloc new space for the table. The
 *tables supported this routine are:
 *
 *	    Key_Table:
 *	    Key_Table_Case_Ins:
 *
...

{
        ...
        case Key_Table:
                key_table =
                    (key_keytable_entry_type *)XtCalloc(1, header->table_size);
                table = key_table;
                break;
        case Key_Table_Case_Ins:
                key_table_case_ins =
                ...
                break;
        default:
                diag_issue_internal_error(
                    "Bad table_id in db_read_ints_and_string");
        }
        ...
        return_num_items =
            fread(string_table, sizeof(unsigned char) * string_size, 1, dbfile);
        _check_read(return_num_items);
...
void db_read_char_table(header) _db_header_ptr header;
...

{
         ...
         */
        table = (unsigned char *)XtMalloc(sizeof(unsigned char) *
                                          header->num_items * num_bits);
        return_num_items = fread(
            table, sizeof(char) * num_bits * header->num_items, 1, dbfile);
        _check_read(return_num_items);
...
void db_read_length_and_string(header) _db_header_ptr header;
 ...
 *  PROCEDURE DESCRIPTION:
 *
 *	This routine reads in length and strings of unsigned chars and places
 *them into memory. It will Malloc new space for the table. The tables supported
 *	this routine are:
 *
...

{
        ...
        case Charset_Xmstring_Names_Table:
                charset_xmstring_names_table =
                    (char **)XtCalloc(1, header->table_size);
        ...
        default:
                diag_issue_internal_error(
                    "Bad table_id in db_read_length_and_string");
        }
...
void db_read_int_and_shorts(header) _db_header_ptr header;
...

{
        ...
        case Enum_Set_Table:
                enum_set_table =
                    (UilEnumSetDescDef *)XtCalloc(1, header->table_size);
        ...
        /*
         * Get the entire table with one read.
         * Then loop through the table and add up the number of ints in each int
         * table. Get all the integer tables with one read. Reassign the
        ...
        int_table =
            (unsigned short int *)XtCalloc(1, sizeof(short) * int_table_size);
        return_num_items =
            fread(int_table, sizeof(short) * int_table_size, 1, dbfile);
        _check_read(return_num_items);
        for (i = 0; i <= header->num_items; i++) {
...

{
        ...
        /*
         * Use XtFindFile instead of XtResolvePathName. XtResolvePathName
         * requires a display which UIL doesn't have. At the current time there
         * is no support for $LANG in the path string. If such support was
         * deamed necessary, the %L, %l, %t, %c values would be set up as subs
         * here using globals from the fetch of LANG variable used to determine
         * the default codeset (or vice versa depending on which is called
         * first)
         *
         * If the last 4 characters of the file name are not .bdb
         * then pass in the suffix of .bdb. If a file isn't found with the
         * suffix passed in then try without the suffix.
         */

        /*
         * Make sure 'S' is the last one so we can remove the suffix for the
         * first pass.
         */
        subs[0].match = 'N';
        ...
        /*
         * Check and see if the .wmd suffix is already on the file. If not then
         * try to resolve the pathname with .wmd suffix first. If that fails or
         * the suffix is already on the file then just try to resolve the
         * pathname.
         */
        if (strcmp(&Uil_cmd_z_command
                        .ac_database[strlen(Uil_cmd_z_command.ac_database) - 4],
                   ".wmd") != 0)
                resolvedname = XtFindFile(wmdPath, subs, XtNumber(subs),
                                          (XtFilePredicate)NULL);

        /*
         * No .wmd suffix or a failure to resolve the pathname with the .wmd
         * suffix Try without the suffix.
         */
        subs[2].substitution = "";
        if (resolvedname == 0)
                resolvedname = XtFindFile(wmdPath, subs, XtNumber(subs),
        ...
        if (resolvedname == 0) {
                diag_issue_diagnostic(d_wmd_open, diag_k_no_source,
                                      diag_k_no_column,
        ...
        if (dbfile == NULL) {
                diag_issue_diagnostic(d_src_open, diag_k_no_source,
                                      diag_k_no_column, resolvedname);
        }
...
String get_root_dir_name() {
        ...
        /*
         * XAPPLRES_DEFAULT and UIDPATH_DEFAULT are intentionally split to
         * support SCCS. DO NOT reformat the lines else %-N-%-S could be
         * converted by SCCS into something totally bizarre causing
         * MrmOpenHierarchy failures.
         */
...
String init_wmd_path(filename) String filename;
{
        String path;
        String old_path;
        String homedir;
        ...
        if (filename[0] == '/') {
                ...
                if (path == NULL) {
                        ...
                        if (old_path == NULL) {
                                wmd_path = XtCalloc(
                                    1, 2 * strlen(homedir) + strlen(libdir) +
                                           strlen(incdir) +
                                           strlen(WMDPATH_DEFAULT));
                                sprintf(wmd_path, WMDPATH_DEFAULT, homedir,
                                        homedir, libdir, incdir);
                        } else {
                                wmd_path = XtCalloc(
                                    1, 1 * strlen(old_path) +
                                           2 * strlen(homedir) +
                                ...
                                sprintf(wmd_path, XAPPLRES_DEFAULT, old_path,
