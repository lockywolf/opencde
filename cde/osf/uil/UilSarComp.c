...
#ifndef lint
static char rcsid[] =
    "$XConsortium: UilSarComp.c /main/11 1995/07/14 09:36:46 drk $"
#endif
#endif
...

{
        ...
        az_cstr_entry = (sym_value_entry_type *)sem_allocate_node(
            sym_k_value_entry, sym_k_value_entry_size + sizeof(char *));
...

{
        ...
        az_wchar_str_entry = (sym_value_entry_type *)sem_allocate_node(
            sym_k_value_entry, sym_k_value_entry_size + sizeof(char *));
...

{
        ...
        **	   of it
        **	2) if the last string of the compound string has the same
        *attributes
        ...
        */
        ...
        if (last_str_entry != NULL) {
                for (; last_str_entry->az_next_table_value != NULL;
                ...
                if ((last_str_entry->b_charset == az_str_entry->b_charset) &&
                    ((last_str_entry->b_direction) ==
                     (az_str_entry->b_direction)) &&
        ...
        if (merge) {
                new_str_entry = sem_cat_str_to_str(last_str_entry, TRUE,
                ...
                */
                ...
                if (op2_temporary == FALSE) {
                        ...
                        /* must make a copy since user has access to string via
                         * name */
                        ...
                        _sym_copy_entry(new_str_entry, az_str_entry,
...

{
        ...
        /*
        **	A couple of points:
        **	1) if op2_temporary = FALSE - we must make a copy of 2nd
        *compound *	   string *	2) if the last string of 1st compound
        *string has the same attributes
        ...
        */
        ...
        for (; next_str_entry != NULL;
             next_str_entry =
                 (sym_value_entry_type *)next_str_entry->az_next_table_value) {
                sym_value_entry_type *new_str_entry;
                unsigned short old_size;

                new_str_entry = (sym_value_entry_type *)sem_allocate_node(
                    sym_k_value_entry, next_str_entry->header.w_node_size << 2);

                old_size = new_str_entry->header.w_node_size;

                _sym_copy_entry(new_str_entry, next_str_entry,
                                next_str_entry->header.w_node_size);

                new_str_entry->header.w_node_size = old_size;
                new_str_entry->obj_header.b_flags =
...
sym_value_entry_type *sem_cat_str_to_str(az_str1_entry, op1_temporary,
                                         az_str2_entry, op2_temporary)
...

{
        ...
        /*
        **	Can only append two simple strings if they have the same
        *direction and *	the first does not have the separate attribute.
        */
        _assert(
            ((az_str1_entry->b_charset == az_str2_entry->b_charset) &&
        ...
        _move(new_str_entry->value.c_value, az_str1_entry->value.c_value, l1);

        _move(&new_str_entry->value.c_value[l1], az_str2_entry->value.c_value,
              l2 + 1);
...

{
        ...
        default:
                ...
                _assert(charset_as_subclass <= uil_max_charset,
                        "unknown charset");
...

{
        ...
        /*
        **	Store the current charset so it can be used by LEX to processes
        *a *	string literal (if the CHARSET_NAME is used to prefix a string
        *literal)
        */
...
void sar_make_fallback_charset(name_frame) yystype *name_frame;
{
        ...
        if ((symbol_entry->b_flags & sym_m_charset) == 0) {
                ...
                diag_issue_diagnostic(d_default_charset,
                                      ...
                                      symbol_entry->c_text, DEFAULT_TAG);
        }

        /* Get the default charset keyword entry. */
        if (az_keyword_entry == NULL)
                az_keyword_entry =
                    key_find_keyword(strlen(DEFAULT_TAG), DEFAULT_TAG);
        ...
        /*
         **	Store the current charset so it can be used by LEX to processes
         *a *	string literal (if the NAME is used to prefix a string literal)
         */
        Uil_lex_l_literal_charset = az_keyword_entry->b_subclass;
}
        ...
        if (attr_frame->b_tag == sar_k_token_frame) {
                ...
                */
                ...
                value_entry->b_aux_flags =
                    (attr_frame->b_type & sym_m_sixteen_bit);
        }
