...
#ifndef lint
static char rcsid[] =
    "$XConsortium: UilP2Reslv.c /main/11 1995/07/14 09:36:35 drk $"
#endif
#endif
...

{
        ...
        */
        ...
        for (fwd_entry = sym_az_forward_ref_chain; fwd_entry != NULL;
             fwd_entry = next_fwd_entry) {
                ...
                /*
                **  Save the pointer to the next forward entry so we can free
                *the current
                ...
                */
                ...
                if (object_entry == NULL) {
                        diag_issue_diagnostic(
                            d_never_def, _sar_source_pos2(fwd_entry),
                            diag_object_text(object_type), name_entry->c_text);
                        continue;
                }
                ...
                */
                if ((object_entry->header.b_type != object_type) &&
                    (uil_gadget_variants[object_entry->header.b_type] !=
                     object_type) &&
                    (uil_gadget_variants[object_type] !=
                     object_entry->header.b_type)) {
                        diag_issue_diagnostic(
                            d_ctx_req, _sar_source_pos2(fwd_entry),
                ...
                *target_obj_entry = object_entry;
                ...
                /*
                ** Update objects on forward refernce chain so that their
                *parent_lists point
                ...
                */
                if (fwd_entry->parent != NULL) {
                        ...
                        if (found == FALSE) {
                                parent_node =
                                    (sym_parent_list_type *)sem_allocate_node(
                ...
                /*
                **  Free the Forward reference entry now that it is no longer
                *needed
                */
        ...
        */
        ...
        for (fwd_val_entry = sym_az_val_forward_ref_chain;
             fwd_val_entry != NULL; fwd_val_entry = next_fwd_val_entry) {
                sym_name_entry_type *name_entry;
                ...
                */
                ...
                if (value_entry == NULL) {
                        diag_issue_diagnostic(d_never_def,
                                              _sar_source_pos2(fwd_val_entry),
                                              "value", name_entry->c_text);
                        continue;
                }
                ...
                case (sym_k_patch_list_add): {
                        target_val_entry = (sym_value_entry_type **)
                                               fwd_val_entry->a_update_location;
