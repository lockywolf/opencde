...
static int (*numeric_convert_table[])() = {0,
...
static unsigned int legal_operand_type[] = {
    ...
    /* unary plus */ 1 << sym_k_integer_value |
        ...
        1 << sym_k_vertical_integer_value | 1 << sym_k_float_value |
        1 << sym_k_horizontal_float_value | 1 << sym_k_vertical_float_value |
    ...
    /* unary minus */ 1 << sym_k_integer_value |
        ...
        1 << sym_k_horizontal_float_value | 1 << sym_k_vertical_float_value |
        1 << sym_k_single_float_value,
    /* comp_str */ 1 << sym_k_char_8_value | 1 << sym_k_localized_string_value |
    ...
    /* divide */ 1 << sym_k_integer_value |
        ...
        1 << sym_k_horizontal_float_value | 1 << sym_k_vertical_float_value |
        1 << sym_k_single_float_value,
    /* add */ 1 << sym_k_integer_value | 1 << sym_k_horizontal_integer_value |
        1 << sym_k_vertical_integer_value | 1 << sym_k_float_value |
        1 << sym_k_horizontal_float_value | 1 << sym_k_vertical_float_value |
        1 << sym_k_single_float_value,
    ...
    /* and */ 1 << sym_k_bool_value | 1 << sym_k_integer_value |
        1 << sym_k_char_8_value | 1 << sym_k_localized_string_value |
        1 << sym_k_compound_string_value | 1 << sym_k_localized_string_value,
    /* xor */ 1 << sym_k_bool_value | 1 << sym_k_integer_value,
    /* or */ 1 << sym_k_bool_value | 1 << sym_k_integer_value,
    /* cat */ 1 << sym_k_char_8_value | 1 << sym_k_compound_string_value |
...
void sem_validation() {
         ...
         */
        if (arg_seen == NULL)
                arg_seen = (sym_argument_entry_type **)XtMalloc(
                    sizeof(sym_argument_entry_type *) * (uil_max_arg + 1));
        if (reason_seen == NULL)
                reason_seen = (sym_callback_entry_type **)XtMalloc(
                    sizeof(sym_argument_entry_type *) * (uil_max_reason + 1));
...
void sem_validate_node(node) sym_entry_type *node;

{
        ...
        case sym_k_child_entry:
                 ...
                 * validation will not occur since sem_validate_widget_node
                 * checks bflags for validated flag.
                 */
        ...
        case sym_k_list_entry:
                ...
                if (list_node->header.b_type != sym_k_control_list)
                        break;
                for (list_member =
                         (sym_obj_entry_type *)list_node->obj_header.az_next;
                     list_member != NULL;
                     list_member =
                         (sym_obj_entry_type *)list_member->obj_header.az_next)
                        ...
                        case sym_k_nested_list_entry:
                                ...
                                sem_validate_node(
                                    (sym_entry_type *)
                                        nested_list_entry->az_list);
                                break;
                        case sym_k_control_entry:
                                control_entry =
                                    (sym_control_entry_type *)list_member;
        ...
        case sym_k_section_entry:
                ...
                default:
                        entry_node =
                            (sym_obj_entry_type *)section_node->entries;
...
sym_value_entry_type *
    sem_validate_value_node(value_node) sym_value_entry_type *value_node;

{
        ...
        /*
         * Both evaluation and validation are done by the value evaluation
         * routine
         */
        if (value_node == NULL)
                return NULL;
...
void sem_validate_widget_node(widget_node) sym_widget_entry_type *widget_node;

{
        ...
        if (widget_node->az_controls != NULL) {
                ...
                sem_validate_control_list(widget_node, widget_type, list_entry,
                                          &gadget_count);
                list_entry->w_gadget_count = gadget_count;
                sem_validate_widget_cycle(list_entry,
                                          widget_node->obj_header.az_name);
        }
...
void sem_validate_argument_list(widget_node, widget_type, list_entry,
                                seen) sym_widget_entry_type *widget_node;
unsigned int widget_type;
             ...
             list_member =
                ...
                case sym_k_argument_entry:
                        ...
                        sem_validate_argument_entry(widget_node, widget_type,
                                                    list_entry, argument_entry,
                                                    seen);
                        break;
                default:
                        diag_issue_diagnostic(
                            d_list_item, _sar_source_pos2(list_entry),
...
void sem_validate_argument_entry(widget_node, widget_type, list_entry,
                                 argument_entry,
                                 seen) sym_widget_entry_type *widget_node;
unsigned int widget_type;
sym_list_entry_type *list_entry;
...

{
         ...
         */
        if (argument_entry->header.b_tag == sym_k_error_entry)
                return;
        ...
        /*
         * There is no need to validate the value if it is a widget since
         * widgets are validated elsewhere and you can't define widgets in an
        ...
        if ((argument_entry->az_arg_value->header.b_tag !=
             sym_k_widget_entry) &&
            (argument_entry->az_arg_value->header.b_tag != sym_k_gadget_entry))
                sem_validate_node(
                    (sym_entry_type *)argument_entry->az_arg_value);

        argname_value_entry =
            (sym_value_entry_type *)argument_entry->az_arg_name;
        if (argname_value_entry == NULL) {
                diag_issue_diagnostic(d_bad_argument,
        ...
        if (argname_value_entry->b_type != sym_k_argument_value) {
                diag_issue_diagnostic(
                    d_list_item, _sar_source_pos2(argname_value_entry),
        ...
        argvalue_value_entry =
            (sym_value_entry_type *)argument_entry->az_arg_value;
        ...
        if ((argname_value_entry->obj_header.b_flags & sym_m_builtin) &&
            ...
            (argname_value_entry->obj_header.az_name == NULL)) {
                ...
                if (_constraint_check(keytable_entry->b_subclass))
                        sem_validate_constraint_entry(
                            widget_node, argument_entry, widget_type);
                else {
                ...
                if (*seen_entry != NULL) {
                        diag_issue_diagnostic(
                            d_supersede, _sar_source_pos2(argument_entry),
                            keytable_entry->at_name,
                            diag_tag_text(argument_entry->header.b_tag),
                            diag_tag_text(list_entry->header.b_type),
                            diag_tag_text(list_entry->header.b_tag));
        ...
        */
        ...
        if ((expected_type == sym_k_compound_string_value) &&
            (actual_type == sym_k_char_8_value)) {
                valid_value = TRUE;
                if ((argvalue_value_entry->obj_header.b_flags &
                     sym_m_private) != 0) {
             ...
             (actual_type == sym_k_fontset_value))) {
                ...
                if ((argvalue_value_entry->obj_header.b_flags &
                     sym_m_private) != 0) {
                        ...
                        font_table = sem_create_value_entry(
                        ...
                        argument_entry->az_arg_value =
                            sem_evaluate_value(font_table);
                }
        }
        if ((expected_type == sym_k_keysym_value) &&
            (actual_type == sym_k_integer_value)) {
                /*
                 * If an integer is incountered when expecting a keysym then
                 * just make the integer into a keysym if the integer is
                 * private. If it isn't give an error message because Mrm won't
                 * be able to handle that problem. When allocating the space for
                 * c_value the size of the string is one since an integer can
                ...
                if ((argvalue_value_entry->obj_header.b_flags &
                     sym_m_private) != 0) {
                        ...
                        argument_entry->az_arg_value->value.c_value =
                            (char *)XtCalloc(1, 2);
                        /*
                         * This is a very strange move. While we Calloc 2 bytes
                         * we only need to move one of those bytes. We calloc 2
                         * bytes for a null termination so HP type machines will
                         * work. It looks wierd but it works.
                         */
                        _move(argument_entry->az_arg_value->value.c_value, &tmp,
                              1);
            ...
            ((expected_type == sym_k_horizontal_integer_value) &&
             (actual_type == sym_k_vertical_integer_value))) {
                /* Leave the actual type and value alone, but flag it as
                   an acceptable value. This will mean that the first
                   orientation the value is coerced to will be the orientation
                   for all occurences of the value. */
                valid_value = TRUE;
        }

        if (!valid_value)
                diag_issue_diagnostic(
                    d_obj_type, _sar_source_pos2(argname_value_entry),
                    diag_value_text(actual_type), diag_tag_text(actual_tag),
...
void sem_validate_argument_enumset(argument_entry, arg_code, arg_value_entry)
    sym_argument_entry_type *argument_entry;
int arg_code;
...

{
         ...
         */
        if (arg_value_entry == NULL)
                return;
        if ((arg_value_entry->b_type != sym_k_integer_value) &&
            (arg_value_entry->b_type != sym_k_bool_value))
                return;
        ...
        for (ndx = 0; ndx < enum_set_table[enumset_code].values_cnt; ndx++)
                if (enum_set_table[enumset_code].values[ndx] == enumval_code)
                        return;
        diag_issue_diagnostic(
            d_invalid_enumval, _sar_source_pos2(argument_entry),
            uil_argument_names[arg_code], uil_enumval_names[enumval_code]);
        return;
}
...
unsigned int widget_type;
{
         ...
         */
        for (parent_entry = widget_node->parent_list; parent_entry != NULL;
             parent_entry = parent_entry->next) {
                ...
                arg_name_entry =
                    (sym_value_entry_type *)argument_entry->az_arg_name;
                keytable_entry =
                    (key_keytable_entry_type *)arg_name_entry->value.az_data;
                supported_flag = sem_argument_allowed(
                    keytable_entry->b_subclass, parent_type);
                if (!supported_flag) {
                        /* Check for both argument and constraint, e.g.
                         * decimalPoints. */
                        supported_flag = sem_argument_allowed(
                            keytable_entry->b_subclass, widget_type);
                        if (!supported_flag)
...
void sem_validate_callback_list(widget_node, widget_type, list_entry,
                                seen) sym_widget_entry_type *widget_node;
unsigned int widget_type;
sym_list_entry_type *list_entry;
...
void sem_validate_callback_entry(widget_node, widget_type, list_entry,
                                 callback_entry,
                                 seen) sym_widget_entry_type *widget_node;
unsigned int widget_type;
sym_list_entry_type *list_entry;
...

{
         ...
         * ignore error entries, consistency check
         */
        if (callback_entry->header.b_tag == sym_k_error_entry)
                return;
        _assert(callback_entry->header.b_tag == sym_k_callback_entry,
                "unexpected non callback entry");

        reason_value_entry =
            (sym_value_entry_type *)callback_entry->az_call_reason_name;
        ...
        if (reason_value_entry->b_type != sym_k_reason_value) {
                diag_issue_diagnostic(
                    d_list_item, _sar_source_pos2(reason_value_entry),
        ...
        if ((reason_value_entry->obj_header.b_flags & sym_m_builtin) &&
            (widget_type != uil_sym_user_defined_object)) {
                ...
                supported_flag =
                    sem_reason_allowed(keytable_entry->b_subclass, widget_type);
                if (!supported_flag)
                        diag_issue_diagnostic(
                            d_unsupported, _sar_source_pos2(callback_entry),
                            keytable_entry->at_name,
                ...
                if (*seen_entry != NULL) {
                        diag_issue_diagnostic(
                            d_supersede, _sar_source_pos2(callback_entry),
                            keytable_entry->at_name,
                            diag_tag_text(callback_entry->header.b_tag),
                            diag_tag_text(list_entry->header.b_type),
                            diag_tag_text(list_entry->header.b_tag));
        ...
        if (callback_entry->az_call_proc_ref &&
            ...
            widget_az_arg_value ==
                callback_entry->az_call_proc_ref->az_arg_value) {
                diag_issue_diagnostic(d_circular_def,
                                      _sar_source_pos2(callback_entry),
                                      "callback client_data");
        } else {
                if (callback_entry->az_call_proc_ref &&
                    callback_entry->az_call_proc_ref->az_arg_value &&
                    (callback_entry->az_call_proc_ref->az_arg_value->header
                             .b_tag == sym_k_widget_entry ||
                     callback_entry->az_call_proc_ref->az_arg_value->header
                             .b_tag == sym_k_gadget_entry) &&
                    !widget_az_arg_value)
                        widget_az_arg_value =
                            callback_entry->az_call_proc_ref->az_arg_value;
                sem_validate_procref_entry(callback_entry->az_call_proc_ref);
                sem_validate_procref_list(
...
void sem_validate_control_list(widget_node, widget_type, list_entry,
                               count) sym_widget_entry_type *widget_node;
unsigned int widget_type;
sym_list_entry_type *list_entry;
int *count;
        ...
        for (list_member = (sym_obj_entry_type *)list_entry->obj_header.az_next;
             ...
             list_member =
                ...
                case sym_k_control_entry:
                        ...
                        sem_validate_control_entry(widget_node, widget_type,
                                                   list_entry, control_entry,
                                                   count);
                        break;
                }
}
...
void sem_validate_control_entry(widget_node, widget_type, list_entry,
                                control_entry, gadget_count)
    sym_widget_entry_type *widget_node;
unsigned int widget_type;
sym_list_entry_type *list_entry;
sym_control_entry_type *control_entry;
...

{
         ...
         * ignore error entries, consistency check
         */
        if (control_entry->header.b_tag == sym_k_error_entry)
                return;
        ...
        if (control_obj_entry->header.b_tag == sym_k_child_entry) {
                supported_flag = sem_child_allowed(
                    control_obj_entry->header.b_type, widget_type);
                if (!supported_flag)
                        diag_issue_diagnostic(
                            d_unsupported, _sar_source_pos2(control_entry),
                            uil_child_names[control_obj_entry->header.b_type],
                            "automatic child", diag_object_text(widget_type));
        } else {
...
void sem_validate_widget_cycle(list_entry,
                               cycle_name) sym_list_entry_type *list_entry;
...

{
         ...
         * we must step through the az_reference for named widgets. If a cycle
         * is ever detected, we exit (or we'll recurse forever). Note that an
         * error node returns TRUE, as if it had a cycle (which inhibits further
         * checking).
         *
        ...
        for (list_member = (sym_obj_entry_type *)list_entry->obj_header.az_next;
             ...
             list_member =
                ...
                case sym_k_control_entry:
                        ...
                        if (control_obj_entry->header.b_tag ==
                            sym_k_error_entry)
                                return TRUE;
                        _assert(control_obj_entry->header.b_tag ==
                                        sym_k_widget_entry ||
                                    control_obj_entry->header.b_tag ==
                                        sym_k_gadget_entry ||
                                    control_obj_entry->header.b_tag ==
                                        sym_k_child_entry,
                                "unexpected non-control object entry");
                        ...
                        if (control_obj_entry->az_controls == NULL)
                                break;
                        control_obj_name =
                            control_obj_entry->obj_header.az_name;
...
boolean sem_validate_verify_cycle(cycle_obj,
                                  list_entry) sym_widget_entry_type *cycle_obj;
sym_list_entry_type *list_entry;
...
void sem_validate_procref_list(list_entry) sym_list_entry_type *list_entry;
...

{
         ...
         * ignore error entries, consistency check
         */
        if (procref_entry == NULL)
                return;
        if (procref_entry->header.b_tag == sym_k_error_entry)
                return;
        ...
        */
        ...
        if (arg_checking) {
                ...
                if (actual_arg_count != proc_def_entry->b_arg_count) {
                        diag_issue_diagnostic(
                            d_arg_count, _sar_source_pos2(procref_entry),
                ...
                if ((expected_arg_type ==
                     sym_k_color_value) && /* RAP rgb data type */
                    ...
                    (actual_arg_type == sym_k_localized_string_value)) {
                        ...
                        if ((value_entry->obj_header.b_flags & sym_m_private) !=
                            0) {
                                ...
                                wcstr_value = (sym_value_entry_type *)
                                    sem_create_wchar_str();
                                wcstr_value->b_expr_opr = sym_k_coerce_op;
                    ...
                    (value_entry->header.b_tag == sym_k_widget_entry)) {
                        ...
                        if ((expected_arg_type > uil_max_object) ||
                            (actual_arg_type == expected_arg_type)) {
                                ...
                                diag_issue_diagnostic(
                                    d_arg_type, _sar_source_pos2(procref_entry),
...
boolean sem_argument_allowed(arg_code, class_code) unsigned int arg_code;
...
boolean sem_reason_allowed(rsn_code, class_code) unsigned int rsn_code;
unsigned int class_code;
...
boolean sem_control_allowed(ctl_code, class_code) unsigned int ctl_code;
unsigned int class_code;
...
boolean sem_child_allowed(ctl_code, class_code) unsigned int ctl_code;
unsigned int class_code;
...
sym_value_entry_type *
    sem_evaluate_value(val_entry) sym_value_entry_type *val_entry;

{
        ...
        if ((val_entry->b_aux_flags & sym_m_exp_eval) == 0)
                ...
                case sym_k_string_table_value: {
                        ...
                        for (value_segment = val_entry->az_first_table_value;
                             ...
                             value_segment =
                                ...
                                if ((value_segment->b_type ==
                                     sym_k_char_8_value) ||
                                    (value_segment->b_type ==
                                     sym_k_localized_string_value)) {
                                        ...
                                        save_next =
                                            (sym_value_entry_type
                                                 *)(value_segment
                                                        ->az_next_table_value);
                                        cstr = (sym_value_entry_type *)
                                            sem_create_cstr();
                                        sem_append_str_to_cstr(
                                            cstr, value_segment, FALSE);
                                        _sym_copy_entry(value_segment, cstr,
                ...
                case sym_k_font_table_value: {
                        ...
                        /*
                         * Fix for CR 5403 - check to make sure each item in the
                         * rgb table is an integer value.  If not, print an
                         * error message. Treat this exactly
                         * like the integer_table_value
                         * section above.
                         */
                ...
                case sym_k_color_table_value: {
                        ...
                        for (ndx = 0; ndx < (int)val_entry->b_table_count;
                             ndx++)
                                if ((int)colorVec[ndx].b_index >
                                    1) /* omit FG, BG */
                                       /*
                                        * Fix for CR 5428 - check to make sure that the
                                        * expression result is a        color value.  If not,
                                        * print diagnostics.
                                        */
                                {
                                        sem_evaluate_value_expr(
                                            colorVec[ndx].az_color);
                                        if ((colorVec[ndx].az_color->b_type !=
                                             sym_k_color_value) &&
                                            (colorVec[ndx].az_color->b_type !=
                                             sym_k_rgb_value))
                                                diag_issue_diagnostic(
                                                    d_wrong_type,
                                                    _sar_source_pos2(
                ...
                case sym_k_icon_value: {
                        ...
                        case sym_k_color_table_value: {
                                ...
                                /*
                                 * Find each icon character in the table, and
                                 * replace its character by its index.
                                 */
                                colorVec = cTable->value.z_color;
                                rowEntry = iconDesc->az_rows;
                                for (ndx = 0; ndx < (int)iconDesc->w_height;
                                     ndx++,
                                    rowEntry = rowEntry->az_next_table_value)
                                        for (row = rowEntry->value.c_value,
                                            i = 0;
                                             i < (int)rowEntry->w_length; i++) {
                                                found = FALSE;
                                                for (j = 0; j < vecLen; j++) {
                                                        if (colorVec[j]
                                                                .b_letter ==
                        ...
                        default:
                                diag_issue_diagnostic(
                                    d_wrong_type, _sar_source_pos2(cTable),
...
sym_value_entry_type *
    sem_evaluate_value_cs(csval_entry) sym_value_entry_type *csval_entry;

{
        ...
        /*
         **	If the pointer to the first segment of the compound string has
         *been
         **	cleared (az_first_table_value) and there's a pointer to the
         *evaluated *	compound string (value.xms_value), then we must have
         *already evaluated *	this compound string.  Leave!
         */
        ...
        csetptr = sem_charset_name(charset, next_segment->az_charset_value);

        if (next_segment->b_type == sym_k_localized_string_value)
                cstr_r = XmStringCreateLocalized(next_segment->value.c_value);
        else
                cstr_r = XmStringConcatAndFree(
                    XmStringDirectionCreate(next_segment->b_direction),
                    XmStringCreate(next_segment->value.c_value, csetptr));

        if (next_segment->b_aux_flags & sym_m_separate)
                cstr_r =
                    XmStringConcatAndFree(cstr_r, XmStringSeparatorCreate());
        ...
        for (next_segment = next_segment->az_next_table_value;
             ...
             next_segment = next_segment->az_next_table_value) {
                ...
                charset = sem_map_subclass_to_charset(next_segment->b_charset);
                csetptr =
                    sem_charset_name(charset, next_segment->az_charset_value);
                 ...
                 **  exists.
                 */
                cstr_1 = XmStringConcatAndFree(
                    XmStringDirectionCreate(next_segment->b_direction),
                    XmStringCreate(next_segment->value.c_value, csetptr));

                if (next_segment->b_aux_flags & sym_m_separate)
                        cstr_1 = XmStringConcatAndFree(
                            cstr_1, XmStringSeparatorCreate());
        ...
        /*
        ** Now deallocate the nodes for the compound string segments and put a
        *null
        ...
        */
        ...
        _assert(csval_entry->w_length <= MrmMaxResourceSize,
                "compound string too long");
...
sym_value_entry_type *
    sem_evaluate_value_expr(value_entry) sym_value_entry_type *value_entry;

{
        ...
        if (value_entry->l_circular_ref_chk == ref_chk_value) {
                if (value_entry->obj_header.az_name != NULL)
                        diag_issue_diagnostic(
                            d_circular_ref, _sar_source_pos2(value_entry),
                            value_entry->obj_header.az_name->c_text);
                else
                        diag_issue_diagnostic(d_circular_ref,
        ...
        in_expr = TRUE;
        op1_type =
            validate_arg(value_entry->az_exp_op1, value_entry->b_expr_opr);
        ...
        case sym_k_unary_plus_op:
                ...
                default:
                        diag_issue_diagnostic(
                            d_cannot_convert, _sar_source_pos2(value_entry),
                            diag_value_text(op1_entry->b_type),
        ...
        case sym_k_comp_str_op:
                ...
                case lstr_arg_type:
                        sem_append_str_to_cstr(value_entry,
                                               value_entry->az_exp_op1, FALSE);
                        value_entry->az_first_table_value->b_aux_flags =
                            value_entry->b_aux_flags;
                        /*
                         * Fix for CN 16149 (DTS 10023) part 2 -- If it exists,
                         * put the charset info collected by
                         * sar_chk_comp_str_attr() onto the char_8 string data
                         * structure.
                         */
                ...
                case cstr_arg_type: /*  comp_str */;
                        {
                                ...
                                /*
                                 * If we're dealing with a combination 1-byte,
                                 * 2-byte string, then we have to evaluate it
                                 * first. (if not already done)
                                 */
                                if (value_entry->az_exp_op1
                                        ->az_first_table_value != NULL) {
                                        sem_evaluate_value_cs(
                                            value_entry->az_exp_op1);
                                }
                                /*
                                 * If there is a separater invoved, makes sure
                                 * it gets concatendated onto the end of the
                                 * string.  Also free up used memory.
                                 */
                                if ((value_entry->b_aux_flags &
                                     sym_m_separate) != 0) {
                                        cstr = XmStringSeparatorCreate();
                                        value_entry->value.l_integer =
                                            (long)XmStringConcatAndFree(
                                                (XmString)
                                                    value_entry->az_exp_op1
                                                        ->value.l_integer,
                                                cstr);
        ...
        case sym_k_coerce_op:
                ...
                case sym_k_compound_string_value:
                        ...
                        case sym_k_localized_string_value:
                                sem_append_str_to_cstr(value_entry, op1_entry,
                                                       FALSE);
                                sem_evaluate_value_cs(value_entry);
                                res_type = cstr_arg_type;
                                break;
                        case sym_k_compound_string_value:
                                _sym_copy_entry(value_entry, op1_entry,
                                                sym_k_value_entry_size);
                                res_type = cstr_arg_type;
                                break;
                        default:
                ...
                case sym_k_vertical_integer_value:
                        ...
                        case sym_k_vertical_float_value:
                                res_type = sem_convert_to_integer(op1_entry,
                                                                  &res_data);
                                value_entry->value.l_integer =
                                    res_data.integer_value;
                                value_entry->b_arg_type = op1_entry->b_arg_type;
                                break;
                        case sym_k_single_float_value: /* single float data type
                                                          RAP */
                                res_type = sem_convert_to_integer(op1_entry,
                                                                  &res_data);
                                value_entry->value.l_integer =
                ...
                case sym_k_vertical_float_value:
                        ...
                        case sym_k_vertical_integer_value:
                                res_type =
                                    sem_convert_to_float(op1_entry, &res_data);
                ...
                case sym_k_single_float_value:
                        ...
                        case sym_k_horizontal_float_value:
                        case sym_k_vertical_float_value:
                                res_type = sem_convert_to_single_float(
                                    op1_entry, &res_data);
                                value_entry->value.single_float =
                                    res_data.single_float_value;
                                value_entry->b_arg_type = op1_entry->b_arg_type;
                                break;
                        case sym_k_single_float_value:
                                value_entry->value.single_float =
                ...
                default:
                        ...
                        if ((op1_entry->b_type != sym_k_char_8_value) &&
                            (op1_entry->b_type != sym_k_localized_string_value))
                                diag_issue_diagnostic(
                                    d_wrong_type, _sar_source_pos2(value_entry),
                                    diag_value_text(op1_entry->b_type),
        ...
        case sym_k_valref_op: {
                ...
                value_entry->az_first_table_value =
                    op1_entry->az_first_table_value;
                value_entry->az_charset_value = op1_entry->az_charset_value;
                /*
                 ** Because of alignment requirements, we can't just move the
                 *largest
                 ** field of the union, but actually have to move the correct
                 *value
        ...
        case sym_k_add_op:
                ...
                case single_float_arg_type:
                        value_entry->value.single_float =
                            op1_ptr->single_float_value +
                            op2_ptr->single_float_value;
                        break;
        ...
        case sym_k_and_op:
                ...
                case char_arg_type:
                case lstr_arg_type:
                        sar_cat_value_entry(&cat_str_entry, op1_entry,
                                            op2_entry);
                        value_entry->b_type = cat_str_entry->b_type;
                        if (cat_str_entry->b_type ==
                            sym_k_compound_string_value) {
                                res_type = cstr_arg_type;
                                value_entry->az_first_table_value =
                ...
                case cstr_arg_type:
                        ...
                        value_entry->b_charset = cat_str_entry->b_charset;
                        value_entry->b_direction = cat_str_entry->b_direction;
                        value_entry->value.xms_value =
                            cat_str_entry->value.xms_value;
                        value_entry->az_first_table_value =
                            cat_str_entry->az_first_table_value;
                        value_entry->w_length = cat_str_entry->w_length;
                        sem_evaluate_value_cs(value_entry);
...
continue_after_error:
        ...
        /*
         ** Set data type for expression value.  If binary operation, use
         *res_type
...
error_occurred:
        ...
        diag_issue_diagnostic(d_out_range, _sar_source_pos2(value_entry),
                              value_text[res_type], "");
        res_type = error_arg_type;
...

{
    ...
    if ((operand_entry->obj_header.b_flags & sym_m_imported) != 0) {
            ...
            diag_issue_diagnostic(d_nonpvt, _sar_source_pos2(operand_entry),
                                  value_entry->obj_header.az_name->c_text);
            return error_arg_type;
    }
...

{
        ...
        if (setjmp(uil_az_error_env_block) == 0) {
                ...
                diag_issue_diagnostic(d_out_range,
                                      _sar_source_pos2(operand_entry),
...

{
        ...
        case sym_k_vertical_integer_value:
        case sym_k_bool_value:
                data_value->single_float_value =
                    (float)operand_entry->value.l_integer;
                return single_float_arg_type;
        ...
        case sym_k_vertical_float_value:
                data_value->single_float_value =
                    (float)operand_entry->value.d_real;
                return single_float_arg_type;
...
#define k_op_state_localized 4
        ...
        **	localized strings, the result is a localized string. If not,
        *the result *	is a compound string.
        */
        ...
        case sym_k_error_value:
                ...
                break;
        default:
                diag_issue_diagnostic(d_wrong_type, _sar_source_pos2(op1_entry),
                                      diag_value_text(op1_entry->b_type),
        ...
        case sym_k_error_value:
                ...
                break;
        default:
                diag_issue_diagnostic(d_wrong_type, _sar_source_pos2(op2_entry),
        ...
        */
        ...
        if ((op1_entry->obj_header.b_flags &
             (sym_m_private | sym_m_exported)) == 0) {
                op1_state = k_op_state_error;
                diag_issue_diagnostic(d_nonpvt, _sar_source_pos2(op1_entry),
                                      value1_entry->obj_header.az_name->c_text);
        }
        if ((op2_entry->obj_header.b_flags &
             (sym_m_private | sym_m_exported)) == 0) {
                op2_state = k_op_state_error;
                diag_issue_diagnostic(d_nonpvt, _sar_source_pos2(op2_entry),
                                      value2_entry->obj_header.az_name->c_text);
        }
        /* End fixing OSF CR 5509 */
        switch (op1_state + (op2_state << 3)) {
        /*
        **	This is the case of appending two simple strings or a simple
        *string *	and a localized	string.  Just append them *	unless
        *they have different directions or the first one has the separate *
        *attribute.
        */
        ...
        case k_op_state_localized + (k_op_state_simple << 3):
                if ((value1_entry->b_charset == value2_entry->b_charset) &&
                    ((value1_entry->b_direction) ==
                     (value2_entry->b_direction)) &&
                    ((value1_entry->b_aux_flags & sym_m_separate) == 0)) {
                        *target_entry =
                            (sym_value_entry_type *)sem_cat_str_to_str(
                                value1_entry, FALSE, value2_entry, FALSE);
                        target_type = sym_k_char_8_value;
                } else {
                        *target_entry =
                            (sym_value_entry_type *)sem_create_cstr();
                        sem_append_str_to_cstr(*target_entry, value1_entry,
                                               FALSE);
                        sem_append_str_to_cstr(*target_entry, value2_entry,
                                               FALSE);
        ...
        /*
        **	This is the case of one simple/localized and one compound
        *string. *	Change the *	simple/localized to a compound and
        *append them together.  Depend on *	the append *	routine to do
        *the right thing.
        */
        ...
        case k_op_state_localized + (k_op_state_compound << 3):
                *target_entry = (sym_value_entry_type *)sem_create_cstr();
                sem_append_str_to_cstr(*target_entry, value1_entry, FALSE);
                sem_evaluate_value_cs(*target_entry);
                /*
                 * We must evaluate both  entries to the XmStringConcat routine
                 * so that it will work properly.   However this MAY be a
                 * pointer to a compound string, use that value instead or we
                 * will concat a NULL value and lose part of the string.
                 */
        ...
        */
        ...
        case k_op_state_compound + (k_op_state_localized << 3):
                ...
                *target_entry = (sym_value_entry_type *)sem_create_cstr();
                sem_append_str_to_cstr(*target_entry, value2_entry, FALSE);
                sem_evaluate_value_cs(*target_entry);
                /*
                 * We must evaluate both  entries to the XmStringConcat routine
                 * so that it will work properly.   However this MAY be a
        ...
        /*
        **  This is the case of two compound strings.  Just let the append
        *routine
        **  do the right thing.
        */
        ...
        /*
        **	This is the case of two localized strings.  Just concatenate
        *them.
        */
        case k_op_state_localized + (k_op_state_localized << 3):
                *target_entry = (sym_value_entry_type *)sem_cat_str_to_str(
                    value1_entry, FALSE, value2_entry, FALSE);
        ...
        default: /* some form of error */
                ...
                *target_entry =
                    (sym_value_entry_type *)sym_az_error_value_entry;
                break;
        }
}
