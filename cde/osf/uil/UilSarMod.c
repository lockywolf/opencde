...
externaldef(uil_comp_glbl) src_source_record_type *src_az_module_source_record;
externaldef(uil_comp_glbl) unsigned short int *uil_urm_variant = NULL;
externaldef(uil_comp_glbl) unsigned short int *uil_arg_compr = NULL;
externaldef(uil_comp_glbl) unsigned short int *uil_reas_compr = NULL;
externaldef(uil_comp_glbl) unsigned short int *uil_widget_compr = NULL;
externaldef(uil_comp_glbl) unsigned short int *uil_child_compr = NULL;
...

{
        ...
        if (strcmp(XmFALLBACK_CHARSET, "ISO8859-1") !=
            0) /* Most common case. */
                for (i = 0; i < (int)charset_lang_table_max; i++)
                        if (strcmp(XmFALLBACK_CHARSET,
                                   charset_lang_names_table[i]) == 0) {
                                uil_sym_default_charset =
        ...
        if (uil_urm_variant == NULL)
                uil_urm_variant = (unsigned short int *)XtMalloc(
                    sizeof(unsigned short int) * (uil_max_object + 1));
        if (uil_arg_compr == NULL)
                uil_arg_compr = (unsigned short int *)XtMalloc(
                    sizeof(unsigned short int) * (uil_max_arg + 1));
        if (uil_reas_compr == NULL)
                uil_reas_compr = (unsigned short int *)XtMalloc(
                    sizeof(unsigned short int) * (uil_max_reason + 1));
        if (uil_widget_compr == NULL)
                uil_widget_compr = (unsigned short int *)XtMalloc(
                    sizeof(unsigned short int) * (uil_max_object + 1));
        if (uil_child_compr == NULL)
                uil_child_compr = (unsigned short int *)XtMalloc(
                    sizeof(unsigned short int) * (uil_max_child + 1));
        /*
         **	Loop throught the array of object variants and set them all to
         *NULL.
         */
...
**  IMPLICIT OUTPUTS:
**
**	sym_az_root_entry		global pointer used thoughout the
*compilation
**	sym_az_curent_section_entry	global pointer used thoughout the
*compilation *	src_az_first_source_record	global pointer to the source
*record list
**
**  FUNCTION VALUE:
**
...

{
        ...
        */
        ...
        sym_az_root_entry = (sym_root_entry_type *)sem_allocate_node(
            sym_k_root_entry, sym_k_root_entry_size);

        /*
        ** Create a place holder entry (tail) and change the "current" section
        *list to it.
        */

        sym_az_current_section_entry =
            (sym_section_entry_type *)sem_allocate_node(
        ...
        */
        ...
        strcpy(sym_az_root_entry->full_file_name,
               src_az_source_file_table[0]->expanded_name);
...

{
        ...
        */
        ...
        sym_az_module_entry = (sym_module_entry_type *)sem_allocate_node(
            sym_k_module_entry, sym_k_module_entry_size);
        ...
        */
        ...
        if (Uil_cmd_z_command.v_listing_file)
                sprintf(Uil_lst_c_title2, "Module: %s", name_entry->c_text);
...

{
        ...
        if ((module_clauses & m_version_clause) != 0)
                diag_issue_diagnostic(d_single_occur,
                                      _sar_source_position(value_frame), "UIL",
                                      "module", "version", "clause");

        /*
        ** FORWARD REFERENCING OF VERSION NO LONGER ALLOWED
        ** If it's not a forward reference, verify its length and stick a
        *pointer
        ...
        if ((value_frame->b_flags & sym_m_forward_ref) != 0)
                ...
                if (value_entry->w_length > 31) {
                        diag_issue_diagnostic(
                            ...
                            "version string", "0..31 characters");
        ...
        */
        ...
        _sar_save_source_info(&sym_az_module_entry->az_version->header,
                              start_frame, value_frame);
        ...
        */
        ...
        if (Uil_cmd_z_command.v_listing_file)
                sprintf(Uil_lst_c_title2, "Module: %s \t Version: %s",
...

{
        ...
        if ((module_clauses & m_names_clause) != 0)
                diag_issue_diagnostic(d_single_occur,
                                      _sar_source_position(token_frame), "UIL",
                                      "module", "names", "clause");

        if ((module_clauses & m_after_names) != 0)
                diag_issue_diagnostic(d_names,
                                      _sar_source_position(token_frame));

        uil_v_case_sensitive = (token_frame->b_type == CASE_SENSITIVE);

        sym_az_module_entry->az_case_sense =
            (sym_value_entry_type *)sem_allocate_node(sym_k_value_entry,
                                                      sym_k_value_entry_size);

        sym_az_module_entry->az_case_sense->header.b_type =
            uil_v_case_sensitive;

        /*
        ** Save source info
        */

        _sar_save_source_info(&sym_az_module_entry->az_case_sense->header,
                              start_frame, token_frame);
        ...
        **	At the point that this semantic routine is called, the only
        *valid
        ...
        */
        ...
        case sar_k_token_frame: {
                ...
                value_entry = sem_create_value_entry(
                    ...
                    token_frame->value.az_keyword_entry->b_length,
                    sym_k_char_8_value);
                break;
        }
...

{
        ...
        if (uil_urm_variant[obj_type] != 0) {
                diag_issue_diagnostic(
                    d_supersede, _sar_source_position(source_frame),
                    diag_object_text(obj_type), diag_tag_text(obj_variant),
...

{
        ...
        */
        ...
        _sar_save_source_info(&section_entry->header, header_frame,
                              header_frame);
