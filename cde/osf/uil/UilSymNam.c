...
#ifndef lint
static char rcsid[] =
    "$TOG: UilSymNam.c /main/13 1997/09/08 11:12:50 cshi $"
#endif
#endif
...
#include "UilDefI.h"
    ...
    **
    **  This routine searches for a name entry of the same name as its
    *parameters.
...

{
        ...
        /*
        **  chain along hash chain looking for symbol - exit loop under 3
        *condition
...
insert_name:
        ...
        */
        ...
        az_new_name = (sym_name_entry_type *)sem_allocate_node(
            sym_k_name_entry, sym_k_name_entry_size + l_length + 1);
...
#define _shift 3
        static unsigned int XmConst mask[8] = {
            0x00000000000000FF, 0x000000000000FFFF, 0x0000000000FFFFFF,
            0x00000000FFFFFFFF, 0x00000000FFFFFFFF, 0x0000FFFFFFFFFFFF,
...
#else
#define _shift 2
        static unsigned int XmConst mask[4] = {0x000000FF, 0x0000FFFF,
...
void sym_dump_hash_table() {
        ...
        for (i = 0; i < sym_k_hash_table_limit; i++) {
                ...
                for (az_name = sym_az_hash_table[i]; az_name != NULL;
                ...
                if (bucket_count == 0)
                        empty_count++;
                max_length =
                    (max_length > bucket_count) ? max_length : bucket_count;
