...
#ifndef lint
static char rcsid[] =
    "$XConsortium: UilSarProc.c /main/12 1995/07/14 09:37:43 drk $"
#endif
#endif
...
#include "UilDefI.h"
    ...
    **/
    ...
    void
    sar_create_procedure(
        XmConst yystype * id_frame, XmConst yystype *param_frame,
        XmConst yystype *class_frame, XmConst yystype *semi_frame) {
        ...
        */
        ...
        proc_def_entry = (sym_proc_def_entry_type *)sem_allocate_node(
            sym_k_proc_def_entry, sym_k_proc_def_entry_size);
        ...
        */
        ...
        sar_assoc_comment(
            (sym_obj_entry_type *)proc_def_entry); /* preserve comments */

        /*
        ** allocate a section entry to link the proc_def entry into the
        *structure
        */
...
sym_proc_ref_entry_type *sem_reference_procedure(yystype *id_frame,
        ...
        */
        ...
        proc_def_entry = (sym_proc_def_entry_type *)sem_ref_name(
            id_frame, sym_k_proc_def_entry);
        ...
        */
        ...
        proc_ref_entry = (sym_proc_ref_entry_type *)sem_allocate_node(
            sym_k_proc_ref_entry, sym_k_proc_ref_entry_size);

        if ((id_frame->b_flags & sym_m_forward_ref) != 0)
                sym_make_value_forward_ref(
                    id_frame, (char *)&(proc_ref_entry->az_proc_def),
        ...
        if ((value_frame->b_flags & sym_m_forward_ref) != 0)
                sym_make_value_forward_ref(
                    value_frame, (char *)&(proc_ref_entry->az_arg_value),
                    sym_k_patch_add);
        else
        ...
        **
        **	At this time the compiler permits all types of values for
        *callback
...
sym_entry_type *sem_ref_name(yystype *id_frame, XmConst int tag) {
        sym_name_entry_type *name_entry;
        ...
        if (id_frame->b_type != NAME) {
                ...
                if (name_entry == NULL) {
                        diag_issue_diagnostic(
                            d_undefined, _sar_source_position(id_frame),
        ...
        */
        ...
        if (symbol_entry->header.b_tag != tag) {
                diag_issue_diagnostic(
                    d_ctx_req, _sar_source_position(id_frame),
                    diag_tag_text(tag),
