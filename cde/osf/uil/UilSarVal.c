...
#include "UilSymGen.h" /* For sym_k_[TRUE|FALSE]_enumval */
                      ...
                      */
                      ...
                      static sym_value_entry_type *
                      standard_color_table _ARGUMENTS((void));
...
yystype *keyword_frame;
{
        ...
        */
        ...
        name_entry =
            sym_insert_name(keyword_frame->value.az_keyword_entry->b_length,
...
yystype *id_frame;
{
        ...
        */
        ...
        if (id_frame->b_type != NAME) {
                name_entry =
                    sym_find_name(id_frame->value.az_keyword_entry->b_length,
                ...
                if (name_entry == NULL) {
                        ...
                        case ARGUMENT_NAME:
                                ...
                                /*
                                ** Indicate that this argument is used so we can
                                *later
                                ** generate appropriate compression code for it.
                                ** If this argument has a related argument, also
                                *mark it as
                                ** being used so we can later generate its
                                *comprssion code.
                                */

                                arg_code = id_frame->value.az_keyword_entry
                                               ->b_subclass;
                        ...
                        case REASON_NAME:
                                ...
                                uil_reas_compr[id_frame->value.az_keyword_entry
                                                   ->b_subclass] = 1;
                                break;

                        case ENUMVAL_NAME:
                                source_type = sym_k_integer_value;
                                enum_code = id_frame->value.az_keyword_entry
                                                ->b_subclass;
                                break;
                        ...
                        value_entry = sem_create_value_entry(
                            (char *)&(id_frame->value.az_keyword_entry),
                            sizeof(long), source_type);
                        if (id_frame->b_type == ENUMVAL_NAME) {
                                value_entry->b_enumeration_value_code =
                                    enum_code;
                                _assert(((enum_code > 0) &&
                                         (enum_code <= uil_max_enumval)),
        ...
        */
        ...
        if (value_entry->header.b_tag != sym_k_value_entry &&
            value_entry->header.b_tag != sym_k_widget_entry) {
                diag_issue_diagnostic(d_ctx_req, _sar_source_position(id_frame),
...

{
        ...
        case NAME:
                ...
                /*
                ** make the keyword into a name and insert it into the symbol
                *table
                */
...

{
        ...
        switch (result) {
        case XmPARSE_ERROR:
                /* I don't expect a parse error since the UIL compiler knows
                   what the valid unit strings are. */
                units_type = XmPIXELS;
                break;
...
void sar_make_private_value(value_frame, token_frame, value_type, keyword_frame,
                            arg_type)
...

{
        sym_value_entry_type *value_entry;
        /*
        **	This should be a long because the call to
        *sem_create_value_entry passes the sizeof(long).   This also *	maps on
        *top of value.l_integer which is a long and is used in
        *sar_chk_comp_str_attr.
        */
        long boolean_value;

        /*  Empty string tables and translation tables can have a null token
         * frame. */
        ...
        */
        ...
        case sym_k_localized_string_value:
                ...
                /* Save the arg_type. This value should be zero for all cases
                   above, except when the integer or float has units specified.
                   In that case, the arg_type is used to store the type of units
                   specified. */
...

        {
                ...
                for (table_entry = value_entry->az_first_table_value, count = 0;
                     table_entry != NULL;
                     table_entry = table_entry->az_next_table_value, count++) {
                }
...
void sar_make_rgb_private_value(value_frame, token_frame, value_type,
                                keyword_frame, arg_type)
...

{
        ...
        if (value_type == sym_k_error_value)
                return;
        ...
        case sym_k_icon_value:
                if ((value_frame->b_flags & sym_m_forward_ref) != 0) {
                        diag_issue_diagnostic(
                            d_undefined, _sar_source_position(value_frame),
                            "icon row",
        ...
        case sym_k_rgb_value:
                if (value_entry->obj_header.az_name != NULL) {
                        ...
                        if ((value_frame->b_flags & sym_m_forward_ref) != 0) {
                                ...
                                value_save = value_entry;
                                value_entry =
                                    sem_create_value_entry(0, 0, value_type);
                                value_entry->b_type = value_type;
                                value_entry->obj_header.b_flags = sym_m_private;
                                value_entry->b_expr_opr = sym_k_valref_op;
                                value_entry->az_exp_op1 = value_save;
...

{
        ...
        */
        ...
        diag_issue_diagnostic(d_not_impl, _sar_source_position(value_frame),
                              error_text);
}
...
#define k_op_state_localized 4
        ...
        _assert((op1_frame->b_tag == sar_k_value_frame) &&
                    (op2_frame->b_tag == sar_k_value_frame),
                "value frame missing");
        ...
        */
        ...
        case sym_k_error_value:
                ...
                break;
        default:
                diag_issue_diagnostic(d_wrong_type,
        ...
        */
        ...
        if ((op1_frame->b_flags & sym_m_private) == 0) {
                op1_state = k_op_state_error;
                diag_issue_diagnostic(d_nonpvt, _sar_source_position(op1_frame),
                                      value1_entry->obj_header.az_name->c_text);
        }
        if ((op2_frame->b_flags & sym_m_private) == 0) {
                op2_state = k_op_state_error;
                diag_issue_diagnostic(d_nonpvt, _sar_source_position(op2_frame),
                                      value2_entry->obj_header.az_name->c_text);
        }

        switch (op1_state + (op2_state << 2)) {
        /*
        **	This is the case of appending to simple strings.  Just append
        *them *	unless they have different directions or the first one has the
        *separate *	attribute.
        */
                    ...
                    ((value1_entry->b_aux_flags & sym_m_separate) == 0)) {
                        target_entry =
                            (sym_value_entry_type *)sem_cat_str_to_str(
                                value1_entry,
                                (value1_entry->obj_header.az_name == NULL),
                                value2_entry,
                                (value2_entry->obj_header.az_name == NULL));
                        target_type = sym_k_char_8_value;
                } else {
                        target_entry =
                            (sym_value_entry_type *)sem_create_cstr();
                        sem_append_str_to_cstr(
                            target_entry, value1_entry,
                            (value1_entry->obj_header.az_name == NULL));
                        sem_append_str_to_cstr(
                            target_entry, value2_entry,
                            (value2_entry->obj_header.az_name == NULL));
        ...
        /*
        **	This is the case of one simple and one compound string.  Change
        *the *	simple to a compound and append them together.  Depend on the
        *append *	routine to do the right thing.
        */
        ...
        /*
        **	This is the case of one simple and one compound string.  Append
        *the *	simple to the compound.  Depend on the append routine to do the
        *right *	thing.
        */
...
yystype *prior_value_frame;
{
        ...
        **	    b_type will hold the writing direction and separate
        *propertied *	    az_symbol_entry will hold the pointer to the
        *character set.
        */
        ...
        case sar_k_null_frame:
                ...
                */
                ...
                /* Fix for CN 16149 (DTS 10023) part 3 -- flag b_charset as
                 * non-existent */
        ...
        */
        ...
        if ((value_entry != NULL) && (value_frame->b_type != CHARACTER_SET) &&
            (value_entry->obj_header.b_flags & sym_m_forward_ref) != 0) {
                diag_issue_diagnostic(d_undefined,
        ...
        case SEPARATE: {
                /*
                **  If the value is a boolean, then just set the corresponding
                *mask
                **  accordingly.
                */
                if (value_entry->b_type == sym_k_bool_value)
        ...
        case CHARACTER_SET: {
                ...
                **  char8 string value in the symbol table that represents the
                *charset.
                */
                switch (value_frame->b_tag) {
                /*
                **  For token frames, acquire the charset from the keytable
                *entry
                **  and set frame type so sar_make_comp_str knows how to
                *interpret
                **  the frame.
                */
                case sar_k_token_frame: {
                        ...
                        target_frame->b_tag = sar_k_token_frame;
                        target_frame->b_charset = sem_map_subclass_to_charset(
                            keyword_entry->b_subclass);
                        break;
                }
                /*
...
void sar_make_comp_str(target_frame, value_frame, attr_frame, keyword_frame)
...
yystype *keyword_frame;
{
        ...
        /*
        ** Make a compound string operation.  The compound string will be
        *created
        ...
        *contains
        **  a pointer to the value entry for the userdefined charset, or a token
        *frame
        ...
        */
        ...
        case sar_k_value_frame:
                /*
                **  Set the attributes of the string, as specified by the
                *options
                **  to the COMPOUND_STRING function, without disturbing any
                **  existing bits.
                */
                cstr_entry->b_direction = attr_frame->b_direction;
                cstr_entry->b_aux_flags |=
                    (attr_frame->b_type & sym_m_separate);
                ...
                */
                if ((attr_frame->value.az_symbol_entry) != 0) {
                        ...
                        cstr_entry->az_charset_value = az_value_entry;
                }
...
void sar_make_comp_str_comp(target_frame, type_frame, value_frame,
                            keyword_frame)
...
yystype *keyword_frame;
{
        ...
        /* Evaluate type. */
        type_entry =
            (key_keytable_entry_type *)type_frame->value.az_keyword_entry;
        ...
        if (found)
                type = enumval_values_table[enumval_code];
        else {
                diag_issue_diagnostic(
                    d_arg_type, _sar_source_position(type_frame),
                    uil_enumval_names[enumval_code],
                    "compound_string_component", "XmStringComponentType");
        ...
        case XmSTRING_COMPONENT_TAB:
                ...
                if (value_frame->b_tag != sar_k_null_frame)
                        diag_issue_diagnostic(
                            d_arg_type, _sar_source_position(value_frame),
                            "non-NULL", "compound_string_component", "NULL");
        ...
        case XmSTRING_COMPONENT_LOCALE:
                if ((value_frame->b_tag != sar_k_null_frame) &&
                    (value_frame->b_type != CHARSET_NAME))
                        diag_issue_diagnostic(
                            d_arg_type, _sar_source_position(value_frame),
                            "non-NULL", "compound_string_component",
                            "NULL or _MOTIF_DEFAULT_LOCALE");
                else if (value_frame->b_type == CHARSET_NAME) {
                        cset_name = sem_charset_name(
                            (value_frame->value.az_keyword_entry)->b_subclass,
                            (sym_value_entry_type *)(value_frame->value
                                                         .az_keyword_entry));
                ...
                cstr_r = XmStringComponentCreate(
                    type, strlen(_MOTIF_DEFAULT_LOCALE), _MOTIF_DEFAULT_LOCALE);
                break;
        ...
        case XmSTRING_COMPONENT_RENDITION_END:
                ...
                if (value_frame->b_tag == sar_k_null_frame)
                        diag_issue_diagnostic(
                            d_arg_type, type_frame->az_source_record,
                            type_frame->b_source_end, "NULL",
                ...
                cstr_r =
                    XmStringComponentCreate(type, strlen(str), (XtPointer)str);
                break;
        ...
        case XmSTRING_COMPONENT_DIRECTION:
                if (value_frame->b_tag == sar_k_null_frame)
                        diag_issue_diagnostic(
                            d_arg_type, type_frame->az_source_record,
                            type_frame->b_source_end, "NULL",
                            "compound_string_component", "XmStringDirection");
                        ...
                        value_entry = (key_keytable_entry_type *)
                                          value_frame->value.az_keyword_entry;

                        enumval_code = value_entry->b_subclass;
                        enumset_code = argument_enumset_table
                            [sym_k_XmNstringDirection_arg];
                        found = FALSE;
                        for (i = 0; i < enum_set_table[enumset_code].values_cnt;
                             i++)
                                if (enum_set_table[enumset_code].values[i] ==
                                    enumval_code) {
                                        found = TRUE;
                                        break;
                                }
                        ...
                        if (found)
                                dir = enumval_values_table[enumval_code];
                        else {
                                diag_issue_diagnostic(
                                    d_arg_type,
                                    _sar_source_position(value_frame),
                ...
                cstr_r = XmStringComponentCreate(
                    type, sizeof(XmStringDirection), &dir);
                break;
        ...
        case XmSTRING_COMPONENT_LAYOUT_PUSH:
                if (value_frame->b_tag == sar_k_null_frame)
                        diag_issue_diagnostic(
                            d_arg_type, type_frame->az_source_record,
                            type_frame->b_source_end, "NULL",
                            "compound_string_component", "XmDirection");
                else if (value_frame->b_type != ENUMVAL_NAME)
                        diag_issue_diagnostic(
                            d_arg_type, _sar_source_position(value_frame),
                            diag_value_text(value_frame->b_type),
                        ...
                        if (found)
                                lay_dir = enumval_values_table[enumval_code];
                        else {
                                diag_issue_diagnostic(
                                    d_arg_type,
                                    _sar_source_position(value_frame),
                ...
                cstr_r = XmStringComponentCreate(type, sizeof(XmDirection),
                                                 &lay_dir);
                break;
        }
        ...
        _assert(cstr_entry->w_length <= MrmMaxResourceSize,
...
void sar_make_wchar_str(target_frame, value_frame, attr_frame, keyword_frame)
...
yystype *keyword_frame;
{
        ...
        /*
        ** Make a wide_character string operation.  The wide_character string
        *will
        ...
        */
        ...
        if ((value_frame->b_flags & sym_m_forward_ref) != 0)
                sym_make_value_forward_ref(
                    value_frame, (char *)&(wchar_str_entry->az_exp_op1),
                    sym_k_patch_add);
        else {
                value_entry =
                    (sym_value_entry_type *)value_frame->value.az_symbol_entry;
        ...
        */
        ...
        target_frame->b_flags = sym_m_private;
        target_frame->value.az_symbol_entry = (sym_entry_type *)wchar_str_entry;
}
...

{
        ...
        */
        ...
        if (value_frame->b_type != sym_k_error_value) {
                ...
                diag_issue_diagnostic(d_nonpvt,
                                      _sar_source_position(value_frame),
                                      value_entry->obj_header.az_name->c_text);
        }
...
yystype *token_frame;
{
        ...
        */
        ...
        value_entry = (sym_value_entry_type *)sem_allocate_node(
            sym_k_value_entry, sym_k_value_entry_size);
...
**  IMPLICIT INPUTS:
**
**	sym_az_current_section_entry	global pointer to the "current"
*section list
**
**  IMPLICIT OUTPUTS:
**
        ...
        */
        ...
        if ((flags & sym_m_imported) == 0) {
                if ((value_entry->obj_header.az_name != NULL) || error) {
                        /*
                        ** Create a new value node for the unary value reference
                        *operator.
                        ...
                        */
                        ...
                        /*
                        ** If the value is a forward reference, we'll patch in
                        *the
                        ** address of the the referenced value between passes.
                        *Otherwise,
                        ...
                        */
                        ...
                        if ((value_frame->b_flags & sym_m_forward_ref) != 0)
                                ...
                                value_entry->az_exp_op1 = saved_value_entry_ptr;
                }
...
sym_name_entry_type *sem_dcl_name(id_frame)
...

{
        ...
        ** the effect of creating a name entry if one doesn't existing or
        *finding
        ...
        */
        ...
        if (id_frame->b_type != NAME) {
                diag_issue_diagnostic(
                    d_override_builtin, _sar_source_position(id_frame),
                    id_frame->value.az_keyword_entry->at_name);

                name_entry =
                    sym_insert_name(id_frame->value.az_keyword_entry->b_length,
                                    id_frame->value.az_keyword_entry->at_name);
        } else
                name_entry =
        ...
        ** duplicate definition of the same name.  Otherwise, everything is
        *fine.
        */
        ...
        diag_issue_diagnostic(d_previous_def, _sar_source_position(id_frame),
                              name_entry->c_text, ptr);
...
sym_value_entry_type *sem_create_value_entry(value, length, value_type)
...
int value_type;
{
        ...
        */
        ...
        _sar_save_source_pos(&value_entry->header, &yylval);
        sar_assoc_comment(
            (sym_obj_entry_type *)value_entry); /* preserve comments */
...
**  IMPLICIT INPUTS:
**
**	sym_az_current_section_entry	global that points to the "current"
*section list
**
**  IMPLICIT OUTPUTS:
**
...

{
        ...
        */
        ...
        /*
        ** allocate a section entry to link the identifier entry into the
        *structure
        */
        section_entry = (sym_section_entry_type *)sem_allocate_node(
            sym_k_section_entry, sym_k_section_entry_size);
...
void sar_make_font_table(target_frame, font_frame, prior_target_frame,
                         keyword_frame)
...

{
        ...
        */
        ...
        case sar_k_null_frame:
                font_table_entry = sem_create_value_entry(
                    (char *)&font_item, sizeof(long), sym_k_font_table_value);
                ...
                if (font_item->obj_header.az_name != NULL) {
                        ...
                        font_save = font_item;
                        font_item =
                            sem_create_value_entry(0, 0, font_save->b_type);
        ...
        case sar_k_value_frame: {
                ...
                for (count = 0,
                    last_font_item = font_table_entry->az_first_table_value;
                     ...
                     last_font_item = last_font_item->az_next_table_value)
                        count++;

                if (count >= sym_k_max_list_count)
                        diag_issue_diagnostic(
                            d_too_many, _sar_source_position(font_frame),
                            diag_value_text(sym_k_font_value),
                            diag_value_text(sym_k_font_table_value),
                            sym_k_max_list_count);
                else {
                        /*
                         * If we are dealing with an indirect reference in the
                         * table, make it a valref node so we reference it
                         * correctly.
                         */
                        if (font_item->obj_header.az_name != NULL) {
                                sym_value_entry_type *font_save;
...

{
        ...
        if ((font_frame->b_flags & sym_m_forward_ref) != 0) {
                diag_issue_diagnostic(
                    d_undefined, _sar_source_position(font_frame), "font entry",
        ...
        case sym_k_fontset_value: {
                ...
                **  If the attr_frame is not null, it must be a value frame with
                *contains
                **  a pointer to the value entry for the userdefined charset, or
                *a token frame
                **  which contains the charset token subclass.
                */
...

{
        ...
        if ((value_frame->b_flags & sym_m_forward_ref) != 0)
                sym_make_value_forward_ref(
                    value_frame, (char *)&(font_value_entry->az_exp_op1),
                    sym_k_patch_add);
        else {
                value_entry =
                    (sym_value_entry_type *)value_frame->value.az_symbol_entry;
...
void sar_make_color_item(target_frame, color_frame, letter_frame)
...

{
        ...
        _assert(letter_frame->b_tag == sar_k_value_frame,
                "letter frame missing");
        ...
        */
        ...
        /*
        **	The color can be either a color value or the keyword background
        *or *	foreground.  Want to step up: *	    target_frame->b_tag:
        *with either sar_k_value_frame for no error
        ...
        */
        ...
        item_entry = (sym_color_item_entry_type *)sem_allocate_node(
            sym_k_color_item_entry, sym_k_color_item_entry_size);
        ...
        case sar_k_value_frame:
                ...
                if ((color_frame->b_flags & sym_m_forward_ref) != 0) {
                        ...
                        diag_issue_diagnostic(
                            d_undefined, _sar_source_position(color_frame),
                            "color entry",
        ...
        /*
        **	Letter frame has already been checked in the grammar to be a
        *character *	string.  Need to further check that it has a length
        *of 1.
        */
        ...
        if (letter_entry->w_length != 1) {
                diag_issue_diagnostic(d_single_letter,
        ...
        */
        ...
        case sar_k_value_frame: {
                ...
                item_entry = (sym_color_item_entry_type *)
                                 item_frame->value.az_symbol_entry;
...

{
        ...
        */
        ...
        case sar_k_value_frame: {
                ...
                for (next_item_entry = (sym_color_item_entry_type *)
                ...
                if (index >= sym_k_max_color_count) {
                        diag_issue_diagnostic(
                            d_too_many, _sar_source_position(keyword_frame),
                ...
                color_table_entry = (sym_value_entry_type *)sem_allocate_node(
                    sym_k_value_entry, sym_k_value_entry_size);
                color_table_entry->value.z_color =
                    (sym_color_element *)XtCalloc(1, sizeof(sym_color_element) *
                                                         count);
...

{
        ...
        */
        ...
        color_entry = sem_create_value_entry("", 0, sym_k_color_value);
...

{
                ...
                */
                ...
                if ((table_frame->b_flags & sym_m_forward_ref) != 0)
                        ...
                        default:
                                ...
                                /*
                                 * If the value_frame is not a
                                 * color_table_value, print a diagnostic to flag
                                 * it as an error further down the road.
                                 * Otherwise, if it is exported, will cause
                                 * problems.
                                 */
                                diag_issue_diagnostic(
                                    d_wrong_type,
        ...
        */
        ...
        case sar_k_value_frame: {
                ...
                */
                ...
                for (row_entry = (sym_value_entry_type *)
                                     list_frame->value.az_symbol_entry,
                    head_row_entry = NULL;
                    ...
                    temp_row_entry->az_next_table_value = head_row_entry,
                    head_row_entry = temp_row_entry)
                        ;

                for (row_entry = head_row_entry, width = row_entry->w_length;
                     ...
                     row_entry = row_entry->az_next_table_value) {
                        ...
                        /*
                         * Check to make sure each row_entry is a character
                         * string. If it isn't, print a diagnostic message
                         * indicating an error.
                         */
                        if ((row_entry->b_type != sym_k_char_8_value) &&
                            (row_entry->b_type !=
                             sym_k_localized_string_value)) {
                                diag_issue_diagnostic(
                                    d_wrong_type, _sar_source_pos2(row_entry),
        ...
        if (width > sym_k_max_list_count) {
                diag_issue_diagnostic(
                    d_too_many, _sar_source_position(keyword_frame), "column",
                    diag_value_text(sym_k_icon_value), sym_k_max_list_count);
        ...
        if (height > sym_k_max_list_count) {
                diag_issue_diagnostic(
                    d_too_many, _sar_source_position(keyword_frame), "row",
                    diag_value_text(sym_k_icon_value), sym_k_max_list_count);
        ...
        */
        ...
        if (target_type == sym_k_error_value)
                ...
                icon_entry = (sym_value_entry_type *)sem_allocate_node(
                    sym_k_value_entry, sym_k_value_entry_size);
                icon_entry->value.z_icon =
                    (sym_icon_element *)XtCalloc(1, sizeof(sym_icon_element));
        ...
        */
        ...
        target_frame->b_type = target_type;
        target_frame->b_flags = sym_m_private;
        target_frame->value.az_symbol_entry = (sym_entry_type *)icon_entry;
}
...
static sym_value_entry_type *standard_color_table() {
        ...
        if (color_table == NULL) {
                color_table = (sym_value_entry_type *)sem_allocate_node(
                    sym_k_value_entry, sym_k_value_entry_size);
                color_table->value.z_color = (sym_color_element *)XtCalloc(
                    1, sizeof(sym_color_element) * 2);
