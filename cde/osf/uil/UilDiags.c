...
#ifndef lint
static char rcsid[] =
    "$XConsortium: UilDiags.c /main/15 1996/10/21 11:06:46 cde-osf $"
#endif
#endif
...
void diag_store_handlers(void) {
        /*
         * Fix for CR 5534 - set the file variables for holding previous bus
         * handlers to NULL and then store the old handlers (if any) when
        ...
        **	l_start_column:	   position within the source line for
        *diagnostic. *			   A negative value indicates no
        *position. *	args:		   a variable length parameter list that
        *holds the *			   arguments to be substituted in the
        *diagnositic
        **
        **  IMPLICIT INPUTS:
        **
        **      diag_rz_msg_table: table of messages and their severities
        *defined *			   in message_table.h *
...

{
        ...
        **	the fix previous error diagnostic, if we encountered prior
        *errors;
        ...
        */
        ...
        vsprintf(msg_buffer,
                 ...
                 ap);
#else
        vsprintf(msg_buffer, diag_rz_msg_table[message_number].ac_text, ap);
#endif
        va_end(ap);
        ...
        if (az_src_rec != diag_k_no_source) {
                ...
                */
...
#ifndef NO_MESSAGE_CATALOG
                        sprintf(loc_buffer,
                                catgets(uil_catd, UIL_SET_MISC, UIL_MISC_0,
                                        "\t\t line: %d  file: %s"),
                                az_src_rec->w_line_number,
                                src_get_file_name(az_src_rec));
#else
                        sprintf(loc_buffer, "\t\t line: %d  file: %s",
                                az_src_rec->w_line_number,
                                src_get_file_name(az_src_rec));
#endif
                        ...
                        */
                        ...
                        /*
                        **	create the column pointer if a source position
                        *was given
                        */
        ...
        */
        ...
        if (Uil_cmd_z_command.v_listing_file)
                src_append_diag_info(az_src_rec, l_start_column, msg_buffer,
                                     message_number);
...

{
        ...
        diag_issue_diagnostic(d_summary, diag_k_no_source, diag_k_no_column,
...
#ifndef NO_MESSAGE_CATALOG
                return (
                    catgets(uil_catd, UIL_VALUES, UIL_VAL_1, "** unknown **"));
#else
                return "** unknown **";
#endif
        }
...
#ifndef NO_MESSAGE_CATALOG
                return (
                    catgets(uil_catd, UIL_VALUES, UIL_VAL_0, "** error **"));
#else
                return "** error **";
#endif
...

{
        ...
        */
        /*
         * Fix for CR 5534 - call diag_store_handlers to save the old signal
         * handlers.
         */
        diag_store_handlers();
        /*
...

{
        ...
        **	for the OS.  The handler checks on overflow to see if we are
        *trying *	to catch them at the moment.  Otherwise it issues an
        *internal error.
        */
...
*/
        ...
        diag_issue_diagnostic(d_bug_check, diag_k_no_source, diag_k_no_column,
                              error_text);
...
void write_msg_to_standard_error(message_number, src_buffer, ptr_buffer,
                                 msg_buffer, loc_buffer)
...

{
        ...
        /*
        **  If message callback was supplied, call it with the description of
        *the
        ...
        */
        if (Uil_cmd_z_command.message_cb != (Uil_continue_type(*)())NULL) {
                ...
                /*
                 * Fix for 5534 - restore the old signal handers before calling
                 * message_cb and restore the Uil signal handlers immediately
                 * afterwards
                 */
                diag_restore_diagnostics();
                return_status = (*Uil_cmd_z_command.message_cb)(
                    Uil_cmd_z_command.message_data, message_number,
                    diag_rz_msg_table[message_number].l_severity, msg_buffer,
                    src_buffer, ptr_buffer, loc_buffer, Uil_message_count);
                diag_store_handlers();
                /*
                 * End Fix for 5534
                 */
                ...
                /*
                **  If request is for termination, then longjmp back to main
                *routine
        ...
        fprintf(
            stderr, "%s%s\n",
            catgets(
                uil_catd, UIL_SET_MISC,
                diag_rz_msg_table[message_number].l_severity,
                severity_table[diag_rz_msg_table[message_number].l_severity]),
            msg_buffer);
#else
        fprintf(stderr, "%s%s\n",
                severity_table[diag_rz_msg_table[message_number].l_severity],
...
{ return severity_table[diag_rz_msg_table[d_message_number].l_severity]; }
...

{
        ...
        */
        if (Uil_diag_status_delay_count <= 0) {
                ...
                /*
                 * Fix for CR 5534 - restore the application signal handlers
                 * before calling status_cb and then return the Uil signal
                 * handlers immediately after.
                 */
                diag_restore_diagnostics();
                return_status = (*Uil_cmd_z_command.status_cb)(
                    Uil_cmd_z_command.status_data, Uil_percent_complete,
                    Uil_lines_processed, Uil_current_file, Uil_message_count);
                diag_store_handlers();
                /*
                 * End Fix for CR 5534
                 */
