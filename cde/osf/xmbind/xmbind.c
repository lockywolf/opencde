...
#include <Xm/Xm.h>
    ...
    extern Boolean
    _XmVirtKeysLoadFileBindings(char *fileName, String *binding);
...
char **argv;
{
        ...
        static char *atom_names[] = {"_MOTIF_DEFAULT_BINDINGS",
                                     "_MOTIF_BINDINGS"};
        ...
        display = XtOpenDisplay(app_context, NULL, argv[0], "Xmbind", NULL, 0,
                                &argc, argv);
        ...
        if (argc == 2) {
                if (_XmVirtKeysLoadFileBindings(argv[1], &bindings) == True) {
                        ...
                        XChangeProperty(display, RootWindow(display, 0),
                                        atoms[XA_MOTIF_BINDINGS], XA_STRING, 8,
                                        PropModeReplace,
