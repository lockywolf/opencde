...
#ifndef lint
static char rcsid[] =
    "$XConsortium: wmlutils.c /main/8 1995/08/29 11:11:24 drk $"
#endif
#endif
/*
...
#include <stdio.h>
    ...
    char *
    wmlAllocateString(stg) char * stg;
...
void wmlUpperCaseString(stg) char *stg;
...
void wmlInitHList(listptr, size, is_ordered) DynamicHandleListDefPtr listptr;
int size;
int is_ordered;

{
        ...
        listptr->hvec =
            (ObjectHandleDefPtr)malloc(size * sizeof(ObjectHandleDef));
...
void wmlResizeHList(listptr, new_size) DynamicHandleListDefPtr listptr;
int new_size;
...
void wmlClearHList(listptr) DynamicHandleListDefPtr listptr;
...
int wmlFindInHList(listptr, name) DynamicHandleListDefPtr listptr;
char *name;
...
void wmlInsertInHList(listptr, name, obj) DynamicHandleListDefPtr listptr;
char *name;
ObjectPtr obj;

{
        ...
        if (listptr->ordered) {
                ...
                for (londx = 0, hindx = listptr->cnt - 1; hindx >= londx;) {
                        ...
                        cmpres = strcmp(name, listptr->hvec[midndx].objname);
                        if (cmpres == 0) {
                                printf("\nwmlInsertInHList: duplicate name "
                                       "'%s'found\n",
                                       name);
                                return;
                        }
                ...
                /*
                 * The new entry will go either at midndx or after midndx. Move
                 * down the vector appropriately.
                 */
                if (cmpres < 0)
                        newndx = midndx;
                else
                ...
                for (ndx = listptr->cnt - 1; ndx >= newndx; ndx--) {
                        listptr->hvec[ndx + 1].objname =
                            listptr->hvec[ndx].objname;
                        listptr->hvec[ndx + 1].objptr =
                            listptr->hvec[ndx].objptr;
                }
 ...
 *	obj		the object to insert
 */
void wmlInsertInKeyList(listptr, name, obj) DynamicHandleListDefPtr listptr;
char *name;
ObjectPtr obj;
...
WmlClassResDefPtr wmlResolveResIsMember(resobj,
                                        resref) WmlResourceDefPtr resobj;
...
WmlClassChildDefPtr wmlResolveChildIsMember(childobj,
                                            childref) WmlChildDefPtr childobj;
