...
#ifndef lint
static char rcsid[] =
    "$XConsortium: wmlresolve.c /main/9 1995/08/29 11:11:05 drk $"
#endif
#endif
/*
...

{
        ...
        /*
         * Perform the code assignment pass. This results in assignment of
         * sym_k_... codes to all entities. Also, all objects and cross-linking
         * are validated.
         */
        wmlResolveGenerateSymK();
...

{
        ...
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                ...
                /*
                 * Create and initialize new object. Append to resolved object
                 * vector.
                 */
                ...
                wmlInsertInHList(wml_obj_datatype_ptr, newobj->tkname,
                                 (ObjectPtr)newobj);
...

{
        ...
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                ...
                cursyn->rslvdef = newobj;
                newobj->tkname = cursyn->name;
                wmlInsertInHList(wml_obj_child_ptr, newobj->tkname,
                                 (ObjectPtr)newobj);

                /* Link class to the resolved object. */
                if (cursyn->class != NULL)
                        newobj->class = (WmlClassDefPtr)wmlResolveFindObject(
                            cursyn->class, WmlClassDefValid, cursyn->name);
        }
...

{
         ...
         * syntactic object for every enumeration value named in an enumeration
         * set which has no syntactic entry. If there is an error in a name,
         * then this error won't be detected until we attempt to compile the
         * output .h files.
         */
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                ...
                if (cures->validation != WmlEnumSetDefValid)
                        continue;
                for (curesv = cures->values; curesv != NULL;
                     curesv = curesv->next)
        ...
        /*
         * Initialize the object vector. Then process the syntactic vector,
         * processing each enumeration value object encountered (the vector is
        ...
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                cursyn =
                    (WmlSynEnumValueDefPtr)wml_synobj_ptr->hvec[ndx].objptr;
                if (cursyn->validation != WmlEnumValueDefValid)
                        continue;
                ...
                newobj->syndef = cursyn;
                cursyn->rslvdef = newobj;
                wmlInsertInHList(wml_obj_enumval_ptr, cursyn->name,
                                 (ObjectPtr)newobj);
        }
        ...
        for (ndx = 0; ndx < wml_obj_enumval_ptr->cnt; ndx++) {
                newobj =
                    (WmlEnumValueDefPtr)wml_obj_enumval_ptr->hvec[ndx].objptr;
                newobj->sym_code = code;
                code += 1;
        }
}
...

{
        ...
        /*
         * Initialize the object vector. Then process the syntactic vector,
         * processing each enumeration set object encountered (the vector is
         * ordered). create and append a resolved object for each one
         * encountered. This will be ordered as well.
         */
        ...
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                ...
                cursyn->rslvdef = newobj;
                newobj->tkname = cursyn->name;
                newobj->dtype_def = (WmlDataTypeDefPtr)wmlResolveFindObject(
                    cursyn->datatype, WmlDataTypeDefValid, cursyn->name);
                wmlInsertInHList(wml_obj_enumset_ptr, newobj->tkname,
                                 (ObjectPtr)newobj);
        }
        ...
        for (ndx = 0; ndx < wml_obj_enumset_ptr->cnt; ndx++) {
                newobj =
                    (WmlEnumSetDefPtr)wml_obj_enumset_ptr->hvec[ndx].objptr;
                newobj->sym_code = code;
                code += 1;
                ...
                newobj->values_cnt = 0;
                newobj->values = NULL;
                for (esvelm = cursyn->values; esvelm != NULL;
                     esvelm = esvelm->next) {
                        evobj = (WmlEnumValueDefPtr)wmlResolveFindObject(
                            esvelm->name, WmlEnumValueDefValid, cursyn->name);
                        if (evobj == NULL)
                                continue;
                        esvobj = (WmlEnumSetValDefPtr)malloc(
                            sizeof(WmlEnumSetValDef));
                        esvobj->value = evobj;
...

{
        ...
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                ...
                if (cursyn->validation != WmlResourceDefValid)
                        continue;
                if (cursyn->type != WmlResourceTypeReason)
                        continue;
                ...
                wmlInsertInHList(wml_obj_reason_ptr, newobj->tkname,
                                 (ObjectPtr)newobj);
        ...
        for (ndx = 0; ndx < wml_obj_reason_ptr->cnt; ndx++) {
                newobj =
                    (WmlResourceDefPtr)wml_obj_reason_ptr->hvec[ndx].objptr;
                newobj->sym_code = code;
                code += 1;
        }
}
...

{
        ...
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                ...
                newobj->related_code = 0;
                newobj->enumset_def = NULL;
                wmlInsertInHList(wml_obj_arg_ptr, newobj->tkname,
                                 (ObjectPtr)newobj);
                ...
                if (cursyn->enumset != NULL)
                        newobj->enumset_def =
                            (WmlEnumSetDefPtr)wmlResolveFindObject(
                                cursyn->enumset, WmlEnumSetDefValid,
                                cursyn->name);
        }
...

{
        ...
        for (ndx = 0; ndx < wml_obj_arg_ptr->cnt; ndx++) {
                ...
                if (srcsynobj->related != NULL) {
                        dstobj = (WmlResourceDefPtr)wmlResolveFindObject(
                            srcsynobj->related, WmlResourceDefValid,
                            srcsynobj->name);
                        if (dstobj != NULL)
...

{
        ...
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                ...
                case WmlClassTypeMetaclass:
                        wmlInsertInHList(wml_obj_allclass_ptr, newobj->tkname,
                                         (ObjectPtr)newobj);
                        break;
                ...
                case WmlClassTypeWidget:
                        wmlInsertInHList(wml_obj_allclass_ptr, newobj->tkname,
                                         (ObjectPtr)newobj);
                        wmlInsertInHList(wml_obj_class_ptr, newobj->tkname,
                                         (ObjectPtr)newobj);
                        break;
                }
                ...
                if (cursyn->type != WmlClassTypeMetaclass)
                        if (cursyn->convfunc == NULL) {
                                sprintf(errmsg,
                                        "Class %s does not have a convenience "
                                        "function",
                                        cursyn->name);
                ...
                if (cursyn->ctrlmapto != NULL)
                        newobj->ctrlmapto =
                            (WmlResourceDefPtr)wmlResolveFindObject(
                                cursyn->ctrlmapto, WmlResourceDefValid,
                                cursyn->name);
        }
...

{
        ...
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                ...
                cursyn->rslvdef = newobj;
                newobj->controls = NULL;
                wmlInsertInHList(wml_obj_ctrlist_ptr, cursyn->name,
                                 (ObjectPtr)newobj);

                /*
                 * Validate and construct a resolved controls reference list.
                 */
                for (refptr = cursyn->controls; refptr != NULL;
                     refptr = refptr->next) {
                        classobj = (WmlClassDefPtr)wmlResolveFindObject(
                            refptr->name, WmlClassDefValid, cursyn->name);
                        if (classobj == NULL)
                                continue;
                        ctrlobj =
                            (WmlClassCtrlDefPtr)malloc(sizeof(WmlClassCtrlDef));
...

{
        ...
        for (ndx = 0; ndx < wml_synobj_ptr->cnt; ndx++) {
                ...
                if (cursyn->int_lit != NULL)
                        ...
                        newobj->tkname = cursyn->name;
                wmlInsertInHList(wml_obj_charset_ptr, newobj->tkname,
                                 (ObjectPtr)newobj);

                /*
                 * Parsing direction defaults to writing direction if
                 * unspecified
                 */
                ...
                if (cursyn->xms_name == NULL) {
                        sprintf(errmsg,
                                "CharacterSet %s does not have a StandardsName",
                                cursyn->name);
                        wmlIssueError(errmsg);
                }
        }
        ...
        for (ndx = 0; ndx < wml_obj_charset_ptr->cnt; ndx++) {
                newobj =
                    (WmlCharSetDefPtr)wml_obj_charset_ptr->hvec[ndx].objptr;
                newobj->sym_code = code;
                code += 1;
        }
}
...

{
        ...
        for (ndx = 0; ndx < wml_obj_allclass_ptr->cnt; ndx++) {
                ...
                if (synobj->superclass != NULL) {
                        superobj = (WmlClassDefPtr)wmlResolveFindObject(
                            synobj->superclass, WmlClassDefValid, synobj->name);
                        if (superobj == NULL)
                                continue;
        ...
        for (ndx = 0; ndx < wml_obj_allclass_ptr->cnt; ndx++) {
                ...
                if (synobj->parentclass != NULL) {
                        parentobj = (WmlClassDefPtr)wmlResolveFindObject(
                            synobj->parentclass, WmlClassDefValid,
                            synobj->name);
                        if (parentobj == NULL)
                                continue;
        ...
        for (ndx = 0; ndx < wml_obj_class_ptr->cnt; ndx++) {
                ...
                if (synobj->type == WmlClassTypeGadget) {
                        if (synobj->widgetclass == NULL) {
                                sprintf(errmsg,
                                        "Gadget class %s has no widgetclass "
                                        "reference",
                                        synobj->name);
                                wmlIssueError(errmsg);
                        } else {
                                widgobj = (WmlClassDefPtr)wmlResolveFindObject(
                                    synobj->widgetclass, WmlClassDefValid,
                                    synobj->name);
                                if (widgobj != NULL) {
                ...
                if (synobj->dialog == TRUE) {
                        ...
                        while (clsobj->nondialog->syndef->dialog == TRUE)
                                clsobj->nondialog =
                                    clsobj->nondialog->superclass;
                } else if (clsobj->superclass != NULL) {
        ...
        for (ndx = 0; ndx < wml_obj_class_ptr->cnt; ndx++) {
                clsobj = (WmlClassDefPtr)wml_obj_class_ptr->hvec[ndx].objptr;
                synobj = clsobj->syndef;
                for (refptr = synobj->controls; refptr != NULL;
                     refptr = refptr->next) {
                        refndx =
                            wmlFindInHList(wml_obj_class_ptr, refptr->name);
                        if (refndx >= 0) {
                                refcls = (WmlClassDefPtr)wml_obj_class_ptr
                                             ->hvec[refndx]
                                             .objptr;
                                ctrlobj = (WmlClassCtrlDefPtr)malloc(
                                    sizeof(WmlClassCtrlDef));
                        ...
                        refndx =
                            wmlFindInHList(wml_obj_ctrlist_ptr, refptr->name);
                        if (refndx >= 0) {
                                reflist = (WmlCtrlListDefPtr)wml_obj_ctrlist_ptr
                                              ->hvec[refndx]
                                              .objptr;
...
void wmlResolveClassInherit(clsobj) WmlClassDefPtr clsobj;

{
        ...
        /*
         * Copy the superclass resources, setting the reference pointer as we
         * go.
         */
        if (superobj != NULL) {
                for (srcref = superobj->arguments; srcref != NULL;
                     srcref = srcref->next) {
                        refobj =
                            (WmlClassResDefPtr)malloc(sizeof(WmlClassResDef));
                        refobj->next = clsobj->arguments;
                        clsobj->arguments = refobj;
                        wmlResolveInitRefObj(refobj, srcref);
                }
                for (srcref = superobj->reasons; srcref != NULL;
                     srcref = srcref->next) {
                        refobj =
                            (WmlClassResDefPtr)malloc(sizeof(WmlClassResDef));
                ...
                for (csrcref = superobj->children; csrcref != NULL;
                     csrcref = csrcref->next) {
                        crefobj = (WmlClassChildDefPtr)malloc(
                            sizeof(WmlClassChildDef));
        ...
        /*
         * Copy the parentclass resources, setting the reference pointer as we
         * go.
         */
        if (parentobj != NULL) {
                for (srcref = parentobj->arguments; srcref != NULL;
                     srcref = srcref->next) {
                        if (srcref->act_resource->ref_ptr == NULL) {
                                refobj = (WmlClassResDefPtr)malloc(
                                    sizeof(WmlClassResDef));
                                refobj->next = clsobj->arguments;
                                clsobj->arguments = refobj;
                                wmlResolveInitRefObj(refobj, srcref);
                        }
                }
                for (srcref = parentobj->reasons; srcref != NULL;
                     srcref = srcref->next) {
                        if (srcref->act_resource->ref_ptr == NULL) {
                                refobj = (WmlClassResDefPtr)malloc(
                                    sizeof(WmlClassResDef));
                ...
                for (csrcref = parentobj->children; csrcref != NULL;
                     csrcref = csrcref->next) {
         ...
         */
        for (refptr = synobj->resources; refptr != NULL;
             refptr = refptr->next) {
                resobj = (WmlResourceDefPtr)wmlResolveFindObject(
                    refptr->name, WmlResourceDefValid, synobj->name);
                if (resobj == NULL)
                        continue;

                /*
                 * Acquire the resolved resource object, and the resource
                 * reference. New references are linked in to the proper list,
                 * and have their defaults set.
                 */
                ...
                /*
                 * Override any values in the reference which are explicit in
                 * the syntactic reference.
                 */
                if (refptr->type != NULL)
                        refobj->over_dtype =
                            (WmlDataTypeDefPtr)wmlResolveFindObject(
                                refptr->type, WmlDataTypeDefValid,
                                synobj->name);
                if (refptr->dflt != NULL)
         ...
         */
        for (crefptr = synobj->children; crefptr != NULL;
             crefptr = crefptr->next) {
                childobj = (WmlChildDefPtr)wmlResolveFindObject(
                    crefptr->name, WmlChildDefValid, synobj->name);
                if (childobj == NULL)
                        continue;

                /*
                 * Acquire the resolved child object, and the child reference.
                 * New references are linked in to the proper list, and have
                 * their defaults set.
                 */
...
void wmlResolveInitRefObj(dstobj, srcobj) WmlClassResDefPtr dstobj;
...
void wmlResolveInitChildRefObj(dstobj, srcobj) WmlClassChildDefPtr dstobj;
...
void wmlResolvePrintClass(outfil, clsobj) FILE *outfil;
...

{
        ...
        case WmlClassTypeWidget:
                ...
                if (synobj->convfunc != NULL)
                        fprintf(outfil, "Convenience function: %s",
                                synobj->convfunc);
                break;
        case WmlClassTypeGadget:
                fprintf(outfil, "\n  Type: Gadget\t");
        ...
        /*
         * Print the arguments valid in the class. First the new resources for
         * the class are printed, then each ancestor's contribution is printed.
         * This is intended to match the way resources are printed in the
         * toolkit manual, so that checking is as easy as possible.
         */
        fprintf(outfil, "\n  Arguments:");
...
void wmlResolvePrintClassArgs(outfil, clsobj) FILE *outfil;
WmlClassDefPtr clsobj;

{
        ...
        for (ndx = 0; ndx < wml_obj_arg_ptr->cnt; ndx++) {
                ...
                if (resref == NULL)
                        continue;
                if (wmlResolveResIsMember(resobj, clsobj->arguments) == NULL)
                        continue;
                ...
                fprintf(outfil, "\n      %s", synres->name);
                fprintf(outfil, "\n\tType = %s",
                        resobj->dtype_def->syndef->name);
                if (strcmp(synres->name, synres->resliteral) != 0)
                        fprintf(outfil, "\tResourceLiteral = %s",
                                synres->resliteral);
        ...
        if (constr) {
                prthdr = TRUE;
                for (ndx = 0; ndx < wml_obj_arg_ptr->cnt; ndx++) {
                        resobj = (WmlResourceDefPtr)wml_obj_arg_ptr->hvec[ndx]
                                     .objptr;
                        resref = resobj->ref_ptr;
                        if (resref == NULL)
                                continue;
                        if (wmlResolveResIsMember(resobj, clsobj->arguments) ==
                            NULL)
                                continue;
                        synres = resobj->syndef;
                        switch (synres->type) {
...
void wmlResolvePrintClassReasons(outfil, clsobj) FILE *outfil;
WmlClassDefPtr clsobj;

{
        ...
        for (ndx = 0; ndx < wml_obj_reason_ptr->cnt; ndx++) {
                ...
                if (resref == NULL)
                        continue;
                if (wmlResolveResIsMember(resobj, clsobj->reasons) == NULL)
                        continue;
                synres = resobj->syndef;
                if (prthdr) {
...
void wmlMarkReferencePointers(clsobj) WmlClassDefPtr clsobj;

{
        ...
        for (resref = clsobj->reasons; resref != NULL; resref = resref->next)
                resref->act_resource->ref_ptr = resref;
        for (ctrlref = clsobj->controls; ctrlref != NULL;
             ctrlref = ctrlref->next)
...
ObjectPtr wmlResolveFindObject(name, type, requester) char *name;
int type;
char *requester;
...
void wmlIssueReferenceError(srcname, badname) char *srcname;
char *badname;
...
void wmlIssueIllegalReferenceError(srcname, badname) char *srcname;
char *badname;
 ...
 */
void wmlIssueError(errstg) char *errstg;
