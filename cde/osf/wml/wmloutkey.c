...

{
         ...
         *
         * Special handling is required for tokens whose yacc definition
         *conflicts with common literals. In these cases, the token literal does
         *not map directly to its keyword in the language, and must be mapped
         *as a special case: UILTRUE		-> true/TRUE UILFALSE	->
         *false/FALSE UILfile	 	-> file/FILE UILeof	 	->
         *eof/EOF
         */
        ...
        while (TRUE) {
                ...
                if (scanres == EOF)
                        break;
                if (scanres != 3) {
                        printf("\nBadly formatted at line %d in tokens.dat",
                               lineno);
                        continue;
                }
                lineno += 1;
                ...
                case WmlTokenClassReserved:
                        ...
                        wmlKeyWMakeTokens(sens_name, grtok->class,
                                          (ObjectPtr)grtok);
                }
        }
 ...
 */
int wmlKeyWGrammarClass(token) char *token;
...

{
        ...
        for (ndx = 0; ndx < wml_obj_class_ptr->cnt; ndx++) {
                ...
                wmlKeyWMakeTokens(synobj->name, WmlTokenClassClass,
                                  (ObjectPtr)clsobj);
        }
}
...

{
        ...
        for (ndx = 0; ndx < wml_obj_arg_ptr->cnt; ndx++) {
                ...
                wmlKeyWMakeTokens(synobj->name, WmlTokenClassArgument,
                                  (ObjectPtr)resobj);
...

{
        ...
        for (ndx = 0; ndx < wml_obj_reason_ptr->cnt; ndx++) {
                ...
                synobj = resobj->syndef;
                wmlKeyWMakeTokens(synobj->name, WmlTokenClassReason,
                                  (ObjectPtr)resobj);
                for (alias_ndx = 0; alias_ndx < synobj->alias_cnt; alias_ndx++)
                        wmlKeyWMakeTokens(synobj->alias_list[alias_ndx],
                                          WmlTokenClassReason,
...

{
        ...
        for (ndx = 0; ndx < wml_obj_child_ptr->cnt; ndx++) {
                ...
                wmlKeyWMakeTokens(synobj->name, WmlTokenClassChild,
                                  (ObjectPtr)childobj);
        }
}
...

{
        ...
        for (ndx = 0; ndx < wml_obj_charset_ptr->cnt; ndx++) {
                ...
                synobj = csobj->syndef;
                wmlKeyWMakeTokens(synobj->name, WmlTokenClassCharset,
                                  (ObjectPtr)csobj);
                for (alias_ndx = 0; alias_ndx < synobj->alias_cnt; alias_ndx++)
                        wmlKeyWMakeTokens(synobj->alias_list[alias_ndx],
...

{
        ...
        for (ndx = 0; ndx < wml_obj_enumval_ptr->cnt; ndx++) {
                ...
                synobj = esobj->syndef;
                wmlKeyWMakeTokens(synobj->name, WmlTokenClassEnumval,
                                  (ObjectPtr)esobj);
        }
}
 ...
 */
void wmlKeyWMakeTokens(sens_name, class, obj) char *sens_name;
int class;
ObjectPtr obj;
 ...
 */
void wmlOutputUilKeyTabBody(outfil, tokvec, maxlen, maxkey) FILE *outfil;
...

{
        ...
        for (ndx = 0; ndx < tokvec->cnt; ndx++) {
                ...
                fprintf(outfil, "    {%s, %s, %d, %s, \"%s\"},\n", tkclass,
                        tksym, strlen(tokstg), tktoken, tokstg);
 ...
 */
void wmlTokenClassString(dststg, tok) char *dststg;
WmlKeyWTokenPtr tok;
 ...
 */
void wmlTokenSymKString(dststg, tok) char *dststg;
WmlKeyWTokenPtr tok;
 ...
 */
void wmlTokenTokenString(dststg, tok) char *dststg;
WmlKeyWTokenPtr tok;
