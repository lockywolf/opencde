...
void emit_globals() {
        ...
        if (DEBUG)
                fprintf(afile, "%d %d %d %d %d %d %d %d %d %d ",
                        globals.version, globals.uil_max_arg,
                        globals.uil_max_charset, globals.charset_lang_table_max,
                        globals.uil_max_object, globals.uil_max_reason,
                        globals.uil_max_enumval, globals.uil_max_enumset,
                        globals.key_k_keyword_count,
                        globals.key_k_keyword_max_length);
}

void emit_header(header) _db_header_ptr header;
{

        fwrite(header, sizeof(_db_header), 1, bfile);
        if (DEBUG)
                fprintf(afile, "\n\nTableId=%d, NumEntries=%d, TableSize=%d \n",
                        header->table_id, header->num_items,
                        header->table_size);
}

void emit_chars(table_id) int table_id;
{
        _db_header header;
        unsigned char *ptr;
        int i;
...
void emit_ints_and_string(table_id) int table_id;
{
        _db_header header;
        ...
        for (i = 0; i < header.num_items; i++) {
                ...
                if (DEBUG)
                        fprintf(afile, "%d %d %d %d %s", table[i].b_class,
                                table[i].b_subclass, table[i].b_length,
                                table[i].b_token, table[i].at_name);
        }
}

void emit_char_table(table_id) int table_id;
{
        unsigned char **table;
        _db_header header;
...
void emit_length_and_string(table_id) int table_id;
{
        _db_header header;
        int *lengths;
        char *string_table;
...
void emit_shorts(table_id) int table_id;
{
        _db_header header;
...
void emit_int_and_table_shorts(table_id) int table_id;
{
        _db_header header;
        ...
        for (i = 0; i <= header.num_items; i++) /* first is not used */
        {
                if (table[i].values_cnt) {
                        fwrite(table[i].values,
                               sizeof(short) * table[i].values_cnt, 1, bfile);
                }
        }
}

void emit_ints(table_id) int table_id;
{
        _db_header header;
        int *ptr;
        int i;
