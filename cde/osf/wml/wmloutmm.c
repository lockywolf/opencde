...
#ifndef lint
static char rcsid[] =
    "$XConsortium: wmloutmm.c /main/9 1995/08/29 11:10:59 drk $"
#endif
#endif
/*
...
void wmlOutputWmlUilMmClass(outfil, clsobj, name) FILE *outfil;
...
.TH\n";
        ...
        while (ctlref != NULL) {
                wmlInsertInHList(mm_ctl_ptr, ctlref->ctrl->syndef->name,
                                 (ObjectPtr)ctlref);
                ctlref = ctlref->next;
        }
        ...
        while (rsnndx < mm_rsn_ptr->cnt || ctlndx < mm_ctl_ptr->cnt) {
                if (ctlndx < mm_ctl_ptr->cnt) {
                        ctlref =
                            (WmlClassCtrlDefPtr)mm_ctl_ptr->hvec[ctlndx].objptr;
                        fprintf(outfil, "%s@", ctlref->ctrl->syndef->name);
                        ctlndx += 1;
                } else
                        fprintf(outfil, "@");
                ...
                if (rsnndx < mm_rsn_ptr->cnt) {
                        rsnref =
                            (WmlClassResDefPtr)mm_rsn_ptr->hvec[rsnndx].objptr;
                        fprintf(outfil, "%s\n",
                                rsnref->act_resource->syndef->name);
                        rsnndx += 1;
                } else
        ...
        while (argndx < mm_arg_ptr->cnt) {
                ...
                fprintf(outfil, "%s@%s@", argref->act_resource->syndef->name,
                ...
                if (argref->dflt != NULL) {
                        ...
                        if (argref->act_resource->syndef->dflt != NULL) {
                                if (strchr(argref->act_resource->syndef->dflt,
                                           ' ') != 0)
                                        fprintf(
                                            outfil, "T{\n%s\nT}\n",
