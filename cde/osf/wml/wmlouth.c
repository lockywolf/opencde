...
#ifndef lint
static char rcsid[] =
    "$XConsortium: wmlouth.c /main/8 1995/08/29 11:10:46 drk $"
#endif
#endif
/*
...

{
        ...
        char *canned1 = "/*  Table of the class type of children\n\
...

{
        ...
        char *canned1 = "/*  Table of the types of arguments\n\
...

{
        ...
        char *canned1 = "/* Table indexed by argument indicating whether\n\
...
\t\tallowed_reason_table_vec;\n";
        ...
        /*
         * Generate the bit vectors for each class. Outer loop on the reason
         * code, inner loop on the class code.
         */
        for (resndx = 0; resndx < wml_obj_reason_ptr->cnt; resndx++) {
                resobj =
                    (WmlResourceDefPtr)wml_obj_reason_ptr->hvec[resndx].objptr;
                ...
                for (clsndx = 0; clsndx < wml_obj_class_ptr->cnt; clsndx++) {
                        clsobj = (WmlClassDefPtr)wml_obj_class_ptr->hvec[clsndx]
                                     .objptr;
...
\t\tallowed_argument_table_vec;\n";
        ...
        /*
         * Generate the bit vectors for each class. Outer loop on the argument
         * code, inner loop on the class code.
         */
        for (resndx = 0; resndx < wml_obj_arg_ptr->cnt; resndx++) {
                resobj =
                    (WmlResourceDefPtr)wml_obj_arg_ptr->hvec[resndx].objptr;
                fprintf(outfil, bvechdr, resobj->tkname, resobj->sym_code);
                ...
                for (clsndx = 0; clsndx < wml_obj_class_ptr->cnt; clsndx++) {
                        ...
                        donl = FALSE;
                        resref =
                            wmlResolveResIsMember(resobj, clsobj->arguments);
                        if (resref != NULL)
                                if (resref->exclude != WmlAttributeTrue) {
                                        sprintf(itembuf,
                                                " _BIT_MASK(sym_k_%s_object) |",
...
\t\tallowed_child_table_vec;\n";
        ...
        /*
         * Generate the bit vectors for each class. Outer loop on the child
         * code, inner loop on the class code.
         */
        for (childndx = 0; childndx < wml_obj_child_ptr->cnt; childndx++) {
                childobj =
                    (WmlChildDefPtr)wml_obj_child_ptr->hvec[childndx].objptr;
                ...
                for (clsndx = 0; clsndx < wml_obj_class_ptr->cnt; clsndx++) {
                        ...
                        donl = FALSE;
                        childref =
                            wmlResolveChildIsMember(childobj, clsobj->children);
                        if (childref != NULL) {
                                sprintf(itembuf,
                                        " _BIT_MASK(sym_k_%s_object) |",
                                        clsobj->tkname);
...
\t\tallowed_control_table_vec;\n";
        ...
        /*
         * Generate the bit vectors for each class. Outer loop on the class
         * code, inner loop on the class code.
         */
        for (ctlndx = 0; ctlndx < wml_obj_class_ptr->cnt; ctlndx++) {
                ...
                strcpy(maskbuf, " ");
                for (clsndx = 0; clsndx < wml_obj_class_ptr->cnt; clsndx++) {
                        ctlobj = (WmlClassDefPtr)wml_obj_class_ptr->hvec[clsndx]
                                     .objptr;
...
int wmlResolveCtlIsMember(ctlobj, ctlref) WmlClassDefPtr ctlobj;
...
\t\tenumval_values_table_vec;\n";
        ...
        for (ndx = 0; ndx < wml_obj_enumset_ptr->cnt; ndx++) {
                ...
                for (esvobj = enumsetobj->values; esvobj != NULL;
                     esvobj = esvobj->next) {
                        evobj = esvobj->value;
                        fprintf(outfil, "    %d,\n", evobj->sym_code);
                }
        ...
        for (ndx = 0; ndx < wml_obj_enumset_ptr->cnt; ndx++) {
                ...
                fprintf(outfil, "    {%d,enum_value_vec%d},\n",
                        enumsetobj->values_cnt, enumsetobj->sym_code);
        }

        /*
         * Create enumset table entries for arguments, similar to writing
         * sym_k...
         */
        fprintf(outfil, canned4);
        for (ndx = 0; ndx < wml_obj_arg_ptr->cnt; ndx++) {
...
externaldef(uil_sym_glbl) unsigned short int charset_lang_table_max = %d;\n";
        ...
        for (ndx = 0; ndx < wml_obj_charset_ptr->cnt; ndx++) {
                ...
                case WmlCharSetDirectionLtoR:
                        fprintf(outfil,
                                "    XmSTRING_DIRECTION_L_TO_R,\t/* %s */\n",
                                syncs->name);
                        break;
                case WmlCharSetDirectionRtoL:
                        fprintf(outfil,
                                "    XmSTRING_DIRECTION_R_TO_L,\t/* %s */\n",
                                syncs->name);
                        break;
                }
        }
        ...
        for (ndx = 0; ndx < wml_obj_charset_ptr->cnt; ndx++) {
                ...
                case WmlCharSizeOneByte:
                        fprintf(outfil,
                                "    sym_k_onebyte_charsize,\t/* %s */\n",
                                syncs->name);
                        break;
                case WmlCharSizeTwoByte:
                        fprintf(outfil,
                                "    sym_k_twobyte_charsize,\t/* %s */\n",
                                syncs->name);
                        break;
                case WmlCharSizeMixed1_2Byte:
                        fprintf(outfil,
                                "    sym_k_mixed1_2byte_charsize,\t/* %s */\n",
                                syncs->name);
                        break;
                }
        }
