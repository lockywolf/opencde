 ...
 * @OSF_COPYRIGHT@
 */
/*static char rcsid[] = "RCSfile: dspmsg.c,v Revision: 1.4  (OSF) Date: 90/10/07
 * 16:45:19 ";*/
...

{
        ...
        if (argc < 3) {
                die(catgets(catderr, MS_DSPMSG, M_DSPMSG,
                            "Usage: dtdspmsg [-s setno] <catname> <msgno> "
                            "['default' arg ... ]"));
        }
        ...
        for (p = message, idx = 0; *p; p++) {
                ...
                /* printf % style substitution */
                        ...
                        /* %n$ reorders the argument list and uses variable n
                         * next */
                        ...
                        fwrite(args.args[idx], strlen(args.args[idx]), 1,
                               stdout);
                        idx++;
                }
...

{
        ...
        if (!strcmp(*argv, "-s")) { /* check for a set number */
                if (argc < 4)       /* check for sufficient arguements */
                        die(catgets(catderr, MS_DSPMSG, M_DSPMSG,
                                    "Usage: dtdspmsg [-s setno] <catname> "
                                    "<msgno> ['default' arg ... ]"));
                argv++;
