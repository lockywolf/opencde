...
int round(float x) { return ((int)floor((double)x)); }
#endif

void worm_doit(perwindow *pwin, wormstruct *wp, int which,
               unsigned long color) {
...
void initworm(perwindow *pwin) {
        ...
        if (!init_table) {
                ...
                for (i = 0; i < SEGMENTS; i++) {
                        sintab[i] =
                            round(CIRCSIZE * sin(i * 2 * PI / SEGMENTS));
                        costab[i] =
                            round(CIRCSIZE * cos(i * 2 * PI / SEGMENTS));
                }
        }
