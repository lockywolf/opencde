...
void drawqix(perwindow *pwin) {
        ...
        XSetForeground(dsp, pwin->gc,
                       BlackPixelOfScreen(pwin->perscreen->screen));
        XDrawLine(dsp, pwin->w, pwin->gc, qp->lineq[qp->first].x,
                  qp->lineq[qp->first].y, qp->lineq[qp->first + 1].x,
                  qp->lineq[qp->first + 1].y);
