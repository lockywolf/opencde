...
void initpyro(perwindow *pwin) {
        ...
        for (rockn = 0, rp = pp->rockq; rockn < batchcount; rockn++, rp++) {
                ...
                for (starn = 0; starn < MAXSTARS; starn++) {
                        rp->Xpoints[starn].width = rp->Xpoints[starn].height =
                            rp->Xpoints2[starn].width =
                                rp->Xpoints2[starn].height = bsize;
                }
#endif
        }
        ...
        if (!mono && pwin->perscreen->npixels > 3)
                pp->rockpixel =
                    pwin->perscreen->pixels[3]; /* Just the right shade of
                                                 * orange */
        else
        ...
        if (rp->state == REDGLARE) {
                ...
                if (rp->state == BURSTINGINAIR) {
                        for (starn = 0; starn < rp->nstars; starn++) {
                                ...
                                /* This isn't accurate solid geometry, but it
                                 * looks OK. */
...
#else
        XDrawPoints(dsp, pwin->w, pp->bgGC, rp->Xpoints, nstars,
                    CoordModeOrigin);
        if (stype == DOUBLECLOUD)
                XDrawPoints(dsp, pwin->w, pp->bgGC, rp->Xpoints2, nstars,
                            CoordModeOrigin);
#endif
...
#else
        XDrawPoints(dsp, pwin->w, pwin->gc, rp->Xpoints, nstars,
                    CoordModeOrigin);
        if (stype == DOUBLECLOUD) {
                XSetForeground(dsp, pwin->gc, rp->color2);
