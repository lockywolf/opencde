...
void drawswarm(perwindow *pwin) {
        ...
        XSetForeground(dsp, pwin->gc,
                       BlackPixelOfScreen(pwin->perscreen->screen));
        XDrawLine(dsp, pwin->w, pwin->gc, sp->wx[1], sp->wy[1], sp->wx[2],
                  sp->wy[2]);
        XDrawSegments(dsp, pwin->w, pwin->gc, sp->old_segs, sp->beecount);

        XSetForeground(dsp, pwin->gc,
                       WhitePixelOfScreen(pwin->perscreen->screen));
        XDrawLine(dsp, pwin->w, pwin->gc, sp->wx[0], sp->wy[0], sp->wx[1],
                  sp->wy[1]);
