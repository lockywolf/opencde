...
static void GetResource(XrmDatabase database, char *parentname,
                ...
                if (s == (char *)NULL)
                {
                        fprintf(stderr,
                                catgets(scmc_catd, 2, 18,
                                        "%s: GetResource - couldn't allocate "
                                        "memory.\n"),
                                ProgramName);
                        exit(1);
                }
        ...
        case t_Bool:
                LowerString(buffer);
                *((int *)valuep) =
                    (!strcmp(buffer, "true") || !strcmp(buffer, "on") ||
...
void GetResources(int argc, char *argv[]) {
        ...
        if (!userpath) {
                ...
                if (env)
                        snprintf(userfile, 1024 - 1, "%s/%%N:%s/%%N", env,
                                 homeenv);
                else
        ...
        XrmParseCommand(&generalDB, genTable, genEntries, ProgramName, &argc,
                        argv);
        ...
        for (i = 0; i < NGENARGS; i++)
                GetResource(RDB, ProgramName, classname, genvars[i].name,
                            genvars[i].class, genvars[i].type, genvars[i].def,
                            genvars[i].var);

        for (i = 0; i < NMODEARGS; i++)
                GetResource(RDB, modename, modeclass, modevars[i].name,
                            modevars[i].class, modevars[i].type,
                            modevars[i].def, modevars[i].var);
