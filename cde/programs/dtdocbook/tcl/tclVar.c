...
static char *CallTraces _ANSI_ARGS_((Interp * iPtr, Var *arrayPtr, Var *varPtr,
...
static Var *LookupVar _ANSI_ARGS_((Tcl_Interp * interp, char *part1,
                                   char *part2, int flags, char *msg,
                                   int create, Var **arrayPtrPtr));
static int MakeUpvar _ANSI_ARGS_((Interp * iPtr, CallFrame *framePtr,
                                  char *otherP1, char *otherP2, char *myName,
                                  int flags));
static Var *NewVar _ANSI_ARGS_((void));
static ArraySearch *ParseSearchId _ANSI_ARGS_((Tcl_Interp * interp, Var *varPtr,
                                               char *varName, char *string));
static void VarErrMsg _ANSI_ARGS_((Tcl_Interp * interp, char *part1,
                                   char *part2, char *operation, char *reason));
...
static Var *LookupVar(interp, part1, part2, flags, msg, create, arrayPtrPtr)
                         ...
                         * this is set to NULL. */
{
        ...
        *arrayPtrPtr = varPtr;
        ...
        if (create & CRT_PART2) {
                hPtr =
                    Tcl_CreateHashEntry(varPtr->value.tablePtr, elName, &new);
                if (closeParen != NULL) {
                        *closeParen = ')';
                }
                if (new) {
...
{ return Tcl_GetVar2(interp, varName, (char *)NULL, flags | PART1_NOT_PARSED); }
...
char *Tcl_GetVar2(interp, part1, part2, flags)
                         ...
                         * bits. */
{
        ...
        if ((varPtr->tracePtr != NULL) ||
            ((arrayPtr != NULL) && (arrayPtr->tracePtr != NULL))) {
                ...
                msg =
                    CallTraces(iPtr, arrayPtr, varPtr, part1, part2,
...
char *Tcl_SetVar2(interp, part1, part2, newValue, flags)
    Tcl_Interp *interp; /* Command interpreter in which variable is
                         * to be looked up. */
                         ...
                         * PART1_NOT_PARSED. */
{
        ...
        varPtr = LookupVar(interp, part1, part2, flags, "set",
        ...
        if (flags & TCL_LIST_ELEMENT) {
                ...
                varPtr->valueLength +=
                    Tcl_ConvertElement(newValue, dst, listFlags);
        } else {
...
int Tcl_UnsetVar2(interp, part1, part2, flags)
    Tcl_Interp *interp; /* Command interpreter in which varName is
                         * to be looked up. */
                         ...
                         * or PART1_NOT_PARSED. */
{
        ...
        if ((dummyVar.tracePtr != NULL) ||
            ((arrayPtr != NULL) && (arrayPtr->tracePtr != NULL))) {
                ...
                (void)CallTraces(
                    iPtr, arrayPtr, &dummyVar, part1, part2,
                    (flags & (TCL_GLOBAL_ONLY | PART1_NOT_PARSED)) |
                        TCL_TRACE_UNSETS);
...
int Tcl_TraceVar2(interp, part1, part2, flags, proc, clientData)
    Tcl_Interp *interp; /* Interpreter in which variable is
                         * to be traced. */
...
void Tcl_UntraceVar(interp, varName, flags, proc, clientData)
...
void Tcl_UntraceVar2(interp, part1, part2, flags, proc, clientData)
    Tcl_Interp *interp; /* Interpreter containing traced variable. */
char *part1;            /* Name of variable or array. */
char *part2;            /* Name of element within array;  NULL means
                         * trace applies to scalar variable or array
...
ClientData Tcl_VarTraceInfo2(interp, part1, part2, flags, proc, prevClientData)
    Tcl_Interp *interp;    /* Interpreter containing variable. */
char *part1;               /* Name of variable or array. */
char *part2;               /* Name of element within array;  NULL means
                            * trace applies to scalar variable or array
...
char **argv;                 /* Argument strings. */
{
        ...
        if (argc < 3) {
                Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0],
                                 " option arrayName ?arg ...?\"", (char *)NULL);
                return TCL_ERROR;
        }
        ...
        if ((c == 'a') && (strncmp(argv[1], "anymore", length) == 0)) {
                ...
                if (argc != 4) {
                        Tcl_AppendResult(
                            ...
                            " anymore arrayName searchId\"", (char *)NULL);
                        return TCL_ERROR;
                }
                if (notArray) {
                        goto error;
                ...
                while (1) {
                        ...
                        searchPtr->nextEntry =
                            Tcl_NextHashEntry(&searchPtr->search);
                        if (searchPtr->nextEntry == NULL) {
                                interp->result = "0";
                                return TCL_OK;
                        }
                ...
                return TCL_OK;
                   ...
                   (strncmp(argv[1], "donesearch", length) == 0)) {
                ArraySearch *searchPtr, *prevPtr;

                if (argc != 4) {
                        Tcl_AppendResult(
                            interp, "wrong # args: should be \"", argv[0],
                            " donesearch arrayName searchId\"", (char *)NULL);
                        return TCL_ERROR;
                }
                if (notArray) {
                        goto error;
                ...
                if ((argc != 3) && (argc != 4)) {
                        Tcl_AppendResult(
                            interp, "wrong # args: should be \"", argv[0],
                            " names arrayName ?pattern?\"", (char *)NULL);
                        return TCL_ERROR;
                }
                if (notArray) {
                        return TCL_OK;
                ...
                for (hPtr = Tcl_FirstHashEntry(varPtr->value.tablePtr, &search);
                   ...
                   (strncmp(argv[1], "nextelement", length) == 0) &&
                   (length >= 2)) {
                ArraySearch *searchPtr;
                Tcl_HashEntry *hPtr;
                ...
                if (argc != 4) {
                        Tcl_AppendResult(
                            interp, "wrong # args: should be \"", argv[0],
                            " nextelement arrayName searchId\"", (char *)NULL);
                        return TCL_ERROR;
                }
                if (notArray) {
                        goto error;
                ...
                for (i = 0; i < valueArgc; i += 2) {
                        if (Tcl_SetVar2(interp, argv[2], valueArgv[i],
                                        valueArgv[i + 1],
        ...
        setDone:
                ...
                sprintf(interp->result, "%ld", (long)size);
                   ...
                   (strncmp(argv[1], "startsearch", length) == 0) &&
                   (length >= 2)) {
                ArraySearch *searchPtr;

                if (argc != 3) {
                        Tcl_AppendResult(interp, "wrong # args: should be \"",
                                         argv[0], " startsearch arrayName\"",
                                         (char *)NULL);
                        return TCL_ERROR;
                }
                if (notArray) {
                ...
                searchPtr->nextEntry = Tcl_FirstHashEntry(
                    varPtr->value.tablePtr, &searchPtr->search);
...
static int
    MakeUpvar(iPtr, framePtr, otherP1, otherP2, myName,
              flags) Interp *iPtr; /* Interpreter containing variables.  Used
                                    ...
                                    * myName. */
{
        ...
        otherPtr =
            LookupVar((Tcl_Interp *)iPtr, otherP1, otherP2, TCL_LEAVE_ERR_MSG,
                      "access", CRT_PART1 | CRT_PART2, &arrayPtr);
        ...
        if ((flags & TCL_GLOBAL_ONLY) || (iPtr->varFramePtr == NULL)) {
                ...
                hPtr = Tcl_CreateHashEntry(&iPtr->varFramePtr->varTable, myName,
                                           &new);
        }
        if (new) {
                varPtr = NewVar();
                ...
                if (varPtr->flags & VAR_UPVAR) {
                        ...
                        Tcl_AppendResult((Tcl_Interp *)iPtr, "variable \"",
                                         myName, "\" already exists",
                                         (char *)NULL);
                        return TCL_ERROR;
                } else if (varPtr->tracePtr != NULL) {
                        Tcl_AppendResult(
                            (Tcl_Interp *)iPtr, "variable \"", myName,
...
int flags;              /* TCL_GLOBAL_ONLY or 0. */
{
        ...
        return MakeUpvar((Interp *)interp, framePtr, part1, part2, localName,
                         flags);
}
...
char **argv;        /* Argument strings. */
{
        ...
        if (argc < 2) {
                Tcl_AppendResult((Tcl_Interp *)iPtr,
        ...
        for (argc--, argv++; argc > 0; argc--, argv++) {
                if (MakeUpvar(iPtr, (CallFrame *)NULL, *argv, (char *)NULL,
                              *argv, 0) != TCL_OK) {
                        return TCL_ERROR;
                }
        }
...
char **argv;        /* Argument strings. */
{
        ...
        for (; argc > 0; argc -= 2, argv += 2) {
                for (p = argv[0]; *p != 0; p++) {
                        if (*p == '(') {
                                ...
                                *openParen = '\0';
                                *p = '\0';
                                result = MakeUpvar(iPtr, framePtr, argv[0],
                                                   openParen + 1, argv[1], 0);
                                *openParen = '(';
                                *p = ')';
                                goto checkResult;
                        }
        ...
        scalar:
                result = MakeUpvar(iPtr, framePtr, argv[0], (char *)NULL,
                                   argv[1], 0);
...
static char *
    CallTraces(iPtr, arrayPtr, varPtr, part1, part2,
               flags) Interp *iPtr; /* Interpreter containing variable. */
                ...
                for (p = part1;; p++) {
                        ...
                        if (*p == '(') {
                                ...
                                if (*p == ')') {
                                        ...
                                        Tcl_DStringAppend(&nameCopy, part1,
                                                          (p - part1));
...
static void DeleteSearches(
    arrayVarPtr) register Var *arrayVarPtr; /* Variable whose searches are
                                             * to be deleted. */
{
        ArraySearch *searchPtr;
...
void TclDeleteVars(
    iPtr, tablePtr) Interp *iPtr; /* Interpreter to which variables belong. */
                                   ...
                                   * delete. */
{
        ...
        for (hPtr = Tcl_FirstHashEntry(tablePtr, &search); hPtr != NULL;
             hPtr = Tcl_NextHashEntry(&search)) {
                 ...
                 * decrement the reference count on the variable referred to,
                 * and free the referenced variable if it's no longer needed.
                 * Don't delete the hash entry for the other variable if it's in
                 * the same table as us:  this will happen automatically later
                 * on.
                 */
                ...
                if (varPtr->tracePtr != NULL) {
                        ...
                        for (activePtr = iPtr->activeTracePtr;
                             activePtr != NULL;
                             activePtr = activePtr->nextPtr) {
                                if (activePtr->varPtr == varPtr) {
                                        activePtr->nextTracePtr = NULL;
                ...
                if (varPtr->flags & VAR_ARRAY) {
                        DeleteArray(iPtr, Tcl_GetHashKey(tablePtr, hPtr),
                                    varPtr, flags);
                }
                if (varPtr->valueSpace > 0) {
                        /*
                         * SPECIAL TRICK:  it's possible that the interpreter's
                         * result currently points to this variable (for
                         * example, a "set" or "lappend" command was the last
                         * command in a procedure that's being returned from).
                         * If this is the case, then just pass ownership of the
                         * value string to the Tcl interpreter.
                         */
                ...
                /*
                 * Recycle the variable's memory space if there aren't any
                 * upvar's pointing to it.  If there are upvars, then the
...
static void DeleteArray(iPtr, arrayName, varPtr,
                        flags) Interp *iPtr; /* Interpreter containing array. */
        ...
        for (hPtr = Tcl_FirstHashEntry(varPtr->value.tablePtr, &search);
             hPtr != NULL; hPtr = Tcl_NextHashEntry(&search)) {
                ...
                if (elPtr->valueSpace != 0) {
                        /*
                         * SPECIAL TRICK:  it's possible that the interpreter's
                         * result currently points to this element (for example,
                         * a "set" or "lappend" command was the last command in
                         * a procedure that's being returned from).  If this is
                         * the case, then just pass ownership of the value
                         * string to the Tcl interpreter.
                         */
...
static void
    CleanupVar(varPtr,
               arrayPtr) Var *varPtr; /* Pointer to variable that may be a
                                       ...
                                       * element. */
{
        ...
        if (arrayPtr != NULL) {
                if ((arrayPtr->flags & VAR_UNDEFINED) &&
                    (arrayPtr->refCount == 0) && (arrayPtr->tracePtr == NULL)) {
