...
static int InterpProc _ANSI_ARGS_((ClientData clientData, Tcl_Interp *interp,
                                   int argc, char **argv));
...
char **argv;        /* Argument strings. */
{
        ...
        for (i = 0; i < argCount; i++) {
                ...
                if (fieldCount > 2) {
                        ...
                        Tcl_AppendResult(
                            interp, "too many fields in argument specifier \"",
                ...
                argPtr = (Arg *)ckalloc((unsigned)(sizeof(Arg) -
                                                   sizeof(argPtr->name) +
...
int TclGetFrame(interp, string, framePtrPtr)
...
Proc *TclFindProc(iPtr,
                  procName) Interp *iPtr; /* Interpreter in which to look. */
...
Proc *TclIsProc(cmdPtr) Command *cmdPtr; /* Command to test. */
{
        if (cmdPtr->proc == InterpProc) {
                return (Proc *)cmdPtr->clientData;
        }
...
char **argv;               /* Argument values. */
{
        ...
        for (argPtr = procPtr->argPtr, args = argv + 1, argc -= 1;
             argPtr != NULL; argPtr = argPtr->nextPtr, args++, argc--) {
...
static void
    CleanupProc(procPtr) register Proc *procPtr; /* Procedure to be deleted. */
{
        register Arg *argPtr;
...
int TclUpdateReturnInfo(iPtr) Interp *iPtr; /* Interpreter for which TCL_RETURN
                                             * exception is being processed. */
{
        ...
        if (code == TCL_ERROR) {
                Tcl_SetVar2((Tcl_Interp *)iPtr, "errorCode", (char *)NULL,
                            (iPtr->errorCode != NULL) ? iPtr->errorCode
                                                      : "NONE",
                            TCL_GLOBAL_ONLY);
                iPtr->flags |= ERROR_CODE_SET;
                if (iPtr->errorInfo != NULL) {
                        Tcl_SetVar2((Tcl_Interp *)iPtr, "errorInfo",
                                    (char *)NULL, iPtr->errorInfo,
                                    TCL_GLOBAL_ONLY);
                        iPtr->flags |= ERR_IN_PROGRESS;
                }
        }
