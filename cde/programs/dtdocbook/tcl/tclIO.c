...
typedef struct Channel {
        ...
        ChannelBuffer
            *outQueueHead; /* Points at first buffer in output queue. */
        ChannelBuffer
            *outQueueTail; /* Points at last buffer in output queue. */

        ChannelBuffer
            *saveInBufPtr;          /* Buffer saved for input queue - eliminates
        ...
        int interestMask;             /* Mask of all events this channel has
                                       * handlers for. */
        struct Channel
            *nextChanPtr; /* Next in list of channels currently open. */
...
static int ChannelHandlerEventProc _ANSI_ARGS_((Tcl_Event * evPtr, int flags));
...
static int CopyAndTranslateBuffer _ANSI_ARGS_((Channel * chanPtr, char *result,
                                               int space));
...
static void RecycleBuffer _ANSI_ARGS_((Channel * chanPtr, ChannelBuffer *bufPtr,
                                       int mustDiscard));
static void ReturnScriptRecord _ANSI_ARGS_((Tcl_Interp * interp,
                                            Channel *chanPtr, int mask));
static int ScanBufferForEOL _ANSI_ARGS_(
    (Channel * chanPtr, ChannelBuffer *bufPtr, Tcl_EolTranslation translation,
     int eofChar, int *bytesToEOLPtr, int *crSeenPtr));
                        ...
                        * to remove. */
{
        ...
        for (cbPtr = chanPtr->closeCbPtr, cbPrevPtr = (CloseCallback *)NULL;
             cbPtr != (CloseCallback *)NULL; cbPtr = cbPtr->nextPtr) {
                if ((cbPtr->proc == proc) &&
                    (cbPtr->clientData == clientData)) {
        ...
        for (chanPtr = firstChanPtr; chanPtr != (Channel *)NULL;
             chanPtr = nextChanPtr) {
                ...
                if (chanPtr->refCount <= 0) {
                        ...
                        /*
                         * Switch the channel back into synchronous mode to
                         * ensure that it gets flushed now.
                         */
...
static Tcl_HashTable *GetChannelTable(interp) Tcl_Interp *interp;
{
        ...
        if (hTblPtr == (Tcl_HashTable *)NULL) {
                ...
                (void)Tcl_SetAssocData(
                    interp, "tclIO", (Tcl_InterpDeleteProc *)DeleteChannelTable,
                    (ClientData)hTblPtr);
             ...
             hPtr = Tcl_FirstHashEntry(hTblPtr, &hSearch)) {
                ...
                for (sPtr = chanPtr->scriptRecordPtr,
                    prevPtr = (EventScriptRecord *)NULL;
                     sPtr != (EventScriptRecord *)NULL; sPtr = nextPtr) {
                        ...
                        if (sPtr->interp == interp) {
                                ...
                                Tcl_EventuallyFree((ClientData)sPtr->script,
                                                   TCL_DYNAMIC);
                                ckfree((char *)sPtr);
                        } else {
                                prevPtr = sPtr;
                ...
                /*
                 * Cannot call Tcl_UnregisterChannel because that procedure
                 * calls Tcl_GetAssocData to get the channel table, which might
                 * already be inaccessible from the interpreter structure.
                 * Instead, we emulate the behavior of Tcl_UnregisterChannel
                 * directly here.
                 */
...
Tcl_Channel Tcl_CreateChannel(typePtr, chanName, inFile, outFile, instanceData)
...
ClientData instanceData;      /* Instance specific data. */
{
        ...
        if (chanName != (char *)NULL) {
                chanPtr->channelName =
                    ckalloc((unsigned)(strlen(chanName) + 1));
...
Tcl_File Tcl_GetChannelFile(chan, direction)
...
static void RecycleBuffer(chanPtr, bufPtr, mustDiscard)
...
static void DiscardOutputQueued(
    chanPtr) Channel *chanPtr; /* The channel for which to discard output. */
{
                           ...
                           * flush callback. */
{
        ...
        while (1) {
                 ...
                 * if the current buffer is full, then move the current buffer
                 * to the queue.
                 */

                if (((chanPtr->curOutPtr != (ChannelBuffer *)NULL) &&
                     (chanPtr->curOutPtr->nextAdded ==
                      chanPtr->curOutPtr->bufSize)) ||
                ...
                written = (chanPtr->typePtr->outputProc)(
                    chanPtr->instanceData, chanPtr->outFile,
                    bufPtr->buf + bufPtr->nextRemoved, toWrite, &errorCode);

                /*
                 * If the write failed completely attempt to start the
                 * asynchronous flush mechanism and break out of this loop - do
                 * not attempt to write any more output at this time.
                 */
                ...
                if (written < 0) {
                        ...
                        /*
                         * If we would have blocked, attempt to set up an
                         * asynchronous background flushing for this channel if
                         * the channel is nonblocking, or block until more
                         * output can be written if the channel is blocking.
                         */
                        ...
                        if ((errorCode == EWOULDBLOCK) ||
                            (errorCode == EAGAIN)) {
                                if (chanPtr->flags & CHANNEL_NONBLOCKING) {
                                        ...
                                        /*
                                         * If the device driver did not emulate
                                         * blocking behavior then we must do it
                                         * it here.
                                         */

                                        TclWaitForFile(chanPtr->outFile,
                                                       TCL_WRITABLE, -1);
                                        continue;
                                }
                        }
                        ...
                        /*
                         * Decide whether to report the error upwards or defer
                         * it. If we got an error during async flush we discard
                         * all queued output.
                         */
...
int errorCode;          /* Status of operation so far. */
{
        ...
        result = (chanPtr->typePtr->closeProc)(
            chanPtr->instanceData, interp, chanPtr->inFile, chanPtr->outFile);
...
static int ChannelEventDeleteProc(
    evPtr, clientData) Tcl_Event *evPtr; /* The event to check for a match. */
...
int Tcl_Write(chan, srcPtr,
              slen) Tcl_Channel chan; /* The channel to buffer output for. */
                                       ...
                                       * and we must compute its length. */
{
        ...
        /*
         * If length passed is negative, assume that the output is null
         * terminated and compute its length.
         */
        ...
        while (slen > 0) {
                ...
                if (chanPtr->curOutPtr == (ChannelBuffer *)NULL) {
                        chanPtr->curOutPtr = (ChannelBuffer *)ckalloc(
                            (unsigned)(CHANNELBUFFER_HEADER_SIZE +
                                       chanPtr->bufSize));
                ...
                case TCL_TRANSLATE_CR:
                        ...
                        memcpy((VOID *)destPtr, (VOID *)srcPtr,
                               (size_t)destCopied);
                        for (dPtr = destPtr; dPtr < destPtr + destCopied;
                             dPtr++) {
                                if (*dPtr == '\n') {
                                        *dPtr = '\r';
                                }
                ...
                case TCL_TRANSLATE_AUTO:
                        panic("Tcl_Write: AUTO output translation mode not "
                              "supported");
                default:
                ...
                /*
                 * The current buffer is ready for output if it is full, or if
                 * it contains a newline and this channel is line-buffered, or
                 * if it contains any output and this channel is unbuffered.
                 */
...
static void DiscardInputQueued(chanPtr, discardSavedBuffers)
...
static int GetInput(chanPtr) Channel *chanPtr; /* Channel to read input from. */
{
        ...
        while (1) {
                ...
                /*
                 * If EOF is set, we should avoid calling the driver because on
                 * some platforms it is impossible to read from a device after
                 * EOF.
                 */
                ...
                nread = (chanPtr->typePtr->inputProc)(
                    chanPtr->instanceData, chanPtr->inFile,
                ...
                if (nread == 0) {
                        ...
                        if ((result == EWOULDBLOCK) || (result == EAGAIN)) {
                                ...
                                if (chanPtr->flags & CHANNEL_NONBLOCKING) {
                                        ...
                                        /*
                                         * If the device driver did not emulate
                                         * blocking behavior then we have to do
                                         * it here.
                                         */

                                        TclWaitForFile(chanPtr->inFile,
                                                       TCL_READABLE, -1);
                                }
                        } else {
                                Tcl_SetErrno(result);
...
static int CopyAndTranslateBuffer(chanPtr, result, space)
                       ...
                       * to store the copied input? */
{
         ...
         * either there is no buffer in the queue, or if the first buffer is
         * empty, it is also the last buffer (and thus there is no input in the
         * queue). Note also that if the buffer is empty, we leave it in the
         * queue.
         */
        ...
        if (chanPtr->inEofChar != 0) {
                ...
                if (i < copied) {
                        ...
                        /*
                         * Reset the start of valid data in the input buffer to
                         * the position of the eofChar, so that subsequent reads
                         * will encounter it immediately. First we set it to the
                         * position of the last byte consumed if all result
                         * bytes were the product of one input byte; since it is
                         * possible that "\r\n" contracted to "\n" in the
                         * result, we have to search back from that position
                         * until we find the eofChar, because it is possible
                         * that its actual position in the buffer is n bytes
                         * further back (n is the number of "\r\n" sequences
...
static int ScanBufferForEOL(chanPtr, bufPtr, translation, eofChar,
                            bytesToEOLPtr, crSeenPtr) Channel *chanPtr;
...
int *crSeenPtr;                 /* Has "\r" been seen? */
{
        ...
        for (EOLFound = 0, rPtr = bufPtr->buf + bufPtr->nextRemoved,
            sPtr = bufPtr->buf + bufPtr->nextAdded, bytesToEOL = *bytesToEOLPtr;
             (!EOLFound) && (rPtr < sPtr); rPtr++) {
                switch (translation) {
                case TCL_TRANSLATE_AUTO:
                        if ((*rPtr == (char)eofChar) && (eofChar != 0)) {
                                chanPtr->flags |=
                                    (CHANNEL_EOF | CHANNEL_STICKY_EOF);
                                EOLFound = 1;
                        } else if (*rPtr == '\n') {

                                /*
                                 * CopyAndTranslateBuffer wants to know the
                                 * length of the result, not the input. The
                                 * input is one larger because "\r\n" shrinks to
                                 * "\n".
                                 */
                                ...
                                if (!(*crSeenPtr)) {
                                        ...
                                        /*
                                         * This is a lf at the begining of a
                                         * buffer where the previous buffer
                                         * ended in a cr. Consume this lf
                                         * because we've already emitted the
                                         * newline for this crlf sequence. ALSO,
                                         * if bytesToEOL is 0 (which means that
                                         * we are at the first character of the
                                         * scan), unset the INPUT_SAW_CR flag in
                                         * the channel, because we already
                                         * handled it; leaving it set would
                                         * cause CopyAndTranslateBuffer to
                                         * potentially consume another lf if one
                                         * follows the current byte.
                                         */
                ...
                case TCL_TRANSLATE_CRLF:
                        if ((*rPtr == (char)eofChar) && (eofChar != 0)) {
                                 ...
                                 * CopyAndTranslateBuffer wants to know the
                                 * length of the result, not the input. The
                                 * input is one larger because crlf shrinks to
                                 * lf.
                                 */
...
static int ScanInputForEOL(chanPtr, bytesQueuedPtr)
        ...
        for (bufPtr = chanPtr->inQueueHead,
            crSeen = (chanPtr->flags & INPUT_SAW_CR) ? 1 : 0;
             ...
             bufPtr = bufPtr->nextPtr) {
                EOLFound =
                    ScanBufferForEOL(chanPtr, bufPtr, chanPtr->inputTranslation,
...
static int GetEOL(chanPtr) Channel *chanPtr; /* Channel to queue input on. */
{
        ...
        while (1) {
                ...
                if (chanPtr->flags & CHANNEL_EOF) {
                        /*
                         * Boundary case where cr was at the end of the previous
                         * buffer and this buffer just has a newline.  At EOF
                         * our caller wants to see -1 for the line length.
                         */
...
int Tcl_Read(chan, bufPtr,
             toRead) Tcl_Channel chan; /* The channel from which to read. */
...
int Tcl_Gets(chan, lineRead) Tcl_Channel chan; /* Channel from which to read. */
...
int Tcl_Seek(chan, offset,
             mode) Tcl_Channel chan; /* The channel on which to seek. */
...
int mode;                            /* Relative to which location to seek? */
{
        ...
        for (bufPtr = chanPtr->inQueueHead, inputBuffered = 0;
             bufPtr != (ChannelBuffer *)NULL; bufPtr = bufPtr->nextPtr) {
        ...
        if ((chanPtr->curOutPtr != (ChannelBuffer *)NULL) &&
            (chanPtr->curOutPtr->nextAdded > chanPtr->curOutPtr->nextRemoved)) {
                chanPtr->flags |= BUFFER_READY;
                outputBuffered += (chanPtr->curOutPtr->nextAdded -
                                   chanPtr->curOutPtr->nextRemoved);
        }
        ...
        if (chanPtr->flags & CHANNEL_NONBLOCKING) {
                ...
                if (chanPtr->typePtr->blockModeProc != NULL) {
                        result = (chanPtr->typePtr->blockModeProc)(
                            chanPtr->instanceData, chanPtr->inFile,
                            chanPtr->outFile, TCL_MODE_BLOCKING);
                }
                if (result != 0) {
                        Tcl_SetErrno(result);
                        return -1;
        ...
        if (FlushChannel(NULL, chanPtr, 0) != 0) {
                ...
                /*
                 * Now seek to the new position in the channel as requested by
                 * the caller.
                 */

                curPos = (chanPtr->typePtr->seekProc)(
                    chanPtr->instanceData, chanPtr->inFile, chanPtr->outFile,
        ...
        if (wasAsync) {
                ...
                if (chanPtr->typePtr->blockModeProc != NULL) {
                        result = (chanPtr->typePtr->blockModeProc)(
                            chanPtr->instanceData, chanPtr->inFile,
                            chanPtr->outFile, TCL_MODE_NONBLOCKING);
                }
                if (result != 0) {
                        Tcl_SetErrno(result);
                        return -1;
...
void Tcl_SetChannelBufferSize(chan, sz)
...
Tcl_DString *dsPtr;   /* Where to store value(s). */
{
        ...
        if ((len == 0) || ((len > 1) && (optionName[1] == 't') &&
                ...
                if (chanPtr->flags & TCL_READABLE) {
                        ...
                        } else if (chanPtr->inputTranslation ==
                                   TCL_TRANSLATE_CR) {
                ...
                if (chanPtr->flags & TCL_WRITABLE) {
                        if (chanPtr->outputTranslation == TCL_TRANSLATE_AUTO) {
                                Tcl_DStringAppendElement(dsPtr, "auto");
                        } else if (chanPtr->outputTranslation ==
                                   TCL_TRANSLATE_CR) {
                                Tcl_DStringAppendElement(dsPtr, "cr");
                        } else if (chanPtr->outputTranslation ==
                                   TCL_TRANSLATE_CRLF) {
...
char *newValue;         /* New value for option. */
{
        ...
        if ((len > 2) && (optionName[1] == 'b') &&
            (strncmp(optionName, "-blocking", len) == 0)) {
                ...
                if (chanPtr->typePtr->blockModeProc != NULL) {
                        result = (chanPtr->typePtr->blockModeProc)(
                            chanPtr->instanceData, chanPtr->inFile,
                            chanPtr->outFile, newMode);
                }
                if (result != 0) {
                        Tcl_SetErrno(result);
                        if (interp != (Tcl_Interp *)NULL) {
                                Tcl_AppendResult(
                                    interp, "error setting blocking mode: ",
            ...
            (strncmp(optionName, "-buffering", len) == 0)) {
                ...
                if ((newValue[0] == 'f') &&
                    (strncmp(newValue, "full", len) == 0)) {
                        ...
                        if (interp != (Tcl_Interp *)NULL) {
                                Tcl_AppendResult(
                                    interp, "bad value for -buffering: ",
        ...
        if ((len > 7) && (optionName[1] == 'b') &&
            (strncmp(optionName, "-buffersize", len) == 0)) {
                ...
                if ((chanPtr->bufSize < 10) ||
                    (chanPtr->bufSize > (1024 * 1024))) {
                        chanPtr->bufSize = CHANNELBUFFER_DEFAULT_SIZE;
                }
                return TCL_OK;
        }
        ...
        if ((len > 1) && (optionName[1] == 'e') &&
            (strncmp(optionName, "-eofchar", len) == 0)) {
                if (Tcl_SplitList(interp, newValue, &argc, &argv) ==
                    TCL_ERROR) {
                        return TCL_ERROR;
                }
                if (argc == 0) {
            ...
            (strncmp(optionName, "-translation", len) == 0)) {
                ...
                if (argc == 1) {
                        ...
                        if (chanPtr->flags & TCL_WRITABLE) {
                                if (strcmp(argv[0], "auto") == 0) {
                                        /*
                                         * This is a hack to get TCP sockets to
                                         * produce output in CRLF mode if they
                                         * are being set into AUTO mode. A
                                         * better solution for achieving this
                                         * effect will be coded later.
                                         */

                                        if (strcmp(chanPtr->typePtr->typeName,
                                                   "tcp") == 0) {
                                                chanPtr->outputTranslation =
                                                    TCL_TRANSLATE_CRLF;
                                        } else {
                                                chanPtr->outputTranslation =
                                                    TCL_PLATFORM_TRANSLATION;
        ...
        if (interp != (Tcl_Interp *)NULL) {
                Tcl_AppendResult(
                    interp, "bad option \"", optionName,
                    "\": should be -blocking, -buffering, -buffersize, ",
                    "-eofchar, -translation, ",
                    "or channel type specific option", (char *)NULL);
        }
...
/* ARGSUSED */
static void ChannelEventSourceExitProc(clientData)
...
static void ChannelHandlerSetupProc(clientData, flags)
    ClientData clientData; /* Not used. */
...
static void ChannelHandlerCheckProc(clientData, flags)
    ClientData clientData; /* Not used. */
int flags;                 /* Flags passed to Tk_DoOneEvent:
                            * if it doesn't include
                            * TCL_FILE_EVENTS then we do
        ...
        for (chanPtr = firstChanPtr; chanPtr != (Channel *)NULL;
             chanPtr = nextChanPtr) {
                ...
                if (chanPtr->interestMask & TCL_READABLE) {
                        ...
                        /*
                         * The channel is considered ready for reading if there
                         * is input buffered AND the last attempt to read from
                         * the channel did not return EWOULDBLOCK, OR if the
                         * underlying file is ready.
                         *
                         * NOTE that the input queue may contain empty buffers,
                         * hence the special check to see if the first input
                         * buffer is empty. The invariant is that if there is an
                         * empty buffer in the queue there is only one buffer in
                         * the queue, hence an empty first buffer indicates that
                         * there is no input queued.
                         */
                            ...
                            ((chanPtr->inQueueHead != (ChannelBuffer *)NULL) &&
                             (chanPtr->inQueueHead->nextRemoved <
                              chanPtr->inQueueHead->nextAdded))) {
                                ...
                                readyMask |= Tcl_FileReady(chanPtr->inFile,
                                                           TCL_READABLE);
                        }
                }
                ...
                if (chanPtr->interestMask & TCL_WRITABLE) {
                        ...
                        /*
                         * The channel is considered ready for writing if there
                         * is no output buffered waiting to be written to the
                         * device, AND the underlying file is ready.
                         */

                        if ((chanPtr->outQueueHead == (ChannelBuffer *)NULL) &&
                            (chanPtr->outFile != (Tcl_File)NULL)) {
                                readyMask |= Tcl_FileReady(chanPtr->outFile,
                                                           TCL_WRITABLE);
                        }
                }
                ...
                /*
                 * If there are any events for this channel, put a notice into
                 * the Tcl event queue.
                 */
...
static void FlushEventProc(clientData, mask)
            ...
            * handle, such as TCL_FILE_EVENTS. */
{
        ...
        for (chPtr = chanPtr->chPtr; chPtr != (ChannelHandler *)NULL;) {
                ...
                /*
                 * If this channel handler is interested in any of the events
                 * that have occurred on the channel, invoke its procedure.
                 */
...

{
        ...
        for (prevChPtr = (ChannelHandler *)NULL, chPtr = chanPtr->chPtr;
             chPtr != (ChannelHandler *)NULL; chPtr = chPtr->nextPtr) {
                if ((chPtr->chanPtr == chanPtr) &&
                    (chPtr->clientData == clientData) &&
                    (chPtr->proc == proc)) {
                        break;
                }
        ...
        /*
         * If ChannelHandlerEventProc is about to process this handler, tell it
         * to process the next one instead - we are going to delete *this* one.
         */
                         ...
                         * for which this script is stored. */
{
        ...
        for (esPtr = chanPtr->scriptRecordPtr;
             esPtr != (EventScriptRecord *)NULL; esPtr = esPtr->nextPtr) {
            ...
            prevEsPtr = (EventScriptRecord *)NULL;
             ...
             prevEsPtr = esPtr, esPtr = esPtr->nextPtr) {
                if ((esPtr->interp == interp) && (esPtr->mask == mask)) {
                        ...
                        Tcl_EventuallyFree((ClientData)esPtr->script,
                                           TCL_DYNAMIC);
                        ckfree((char *)esPtr);
...
/* ARGSUSED */
int Tcl_FileEventCmd(clientData, interp, argc,
                     argv) ClientData clientData; /* Not used. */
...
/* ARGSUSED */
int TclTestChannelCmd(clientData, interp, argc,
                      argv) ClientData clientData; /* Not used. */
...
char **argv; /* Additional arg strings. */
{
        ...
        if ((cmdName[0] == 'o') && (strncmp(cmdName, "open", len) == 0)) {
                hTblPtr =
                    (Tcl_HashTable *)Tcl_GetAssocData(interp, "tclIO", NULL);
                if (hTblPtr == (Tcl_HashTable *)NULL) {
                        return TCL_OK;
                }
                for (hPtr = Tcl_FirstHashEntry(hTblPtr, &hSearch);
        ...
        if ((cmdName[0] == 'r') && (strncmp(cmdName, "readable", len) == 0)) {
                ...
                for (hPtr = Tcl_FirstHashEntry(hTblPtr, &hSearch);
                     ...
                     hPtr = Tcl_NextHashEntry(&hSearch)) {
                        ...
                        if (chanPtr->flags & TCL_READABLE) {
                                Tcl_AppendElement(
                                    interp, Tcl_GetHashKey(hTblPtr, hPtr));
                        }
                }
                return TCL_OK;
        }
        ...
        Tcl_AppendResult(interp, "bad option \"", cmdName, "\": should be ",
                         "info, open, readable, or writable", (char *)NULL);
        return TCL_ERROR;
}
...
/* ARGSUSED */
int TclTestChannelEventCmd(dummy, interp, argc,
                           argv) ClientData dummy; /* Not used. */
...
char **argv;                                       /* Argument strings. */
{
        ...
        if ((cmd[0] == 'd') && (strncmp(cmd, "delete", (unsigned)len) == 0)) {
                if (argc != 4) {
                        Tcl_AppendResult(interp, "wrong # args: should be \"",
                                         argv[0], " channelName delete index\"",
                                         (char *)NULL);
                        return TCL_ERROR;
                }
                ...
                if (esPtr == chanPtr->scriptRecordPtr) {
                        ...
                        if (prevEsPtr == (EventScriptRecord *)NULL) {
                                panic("TclTestChannelEventCmd: damaged event "
                                      "script list");
                        }
                        prevEsPtr->nextPtr = esPtr->nextPtr;
                }
        ...
        if ((cmd[0] == 'l') && (strncmp(cmd, "list", (unsigned)len) == 0)) {
                ...
                for (esPtr = chanPtr->scriptRecordPtr;
                     esPtr != (EventScriptRecord *)NULL;
                     esPtr = esPtr->nextPtr) {
                        Tcl_AppendElement(interp, esPtr->mask == TCL_READABLE
                                                      ? "readable"
                                                      : "writable");
        ...
        if ((cmd[0] == 'r') &&
            (strncmp(cmd, "removeall", (unsigned)len) == 0)) {
                if (argc != 3) {
                        Tcl_AppendResult(interp, "wrong # args: should be \"",
                                         argv[0], " channelName removeall\"",
                                         (char *)NULL);
                        return TCL_ERROR;
                }
                for (esPtr = chanPtr->scriptRecordPtr;
                     esPtr != (EventScriptRecord *)NULL; esPtr = nextEsPtr) {
