...
#define BODY_OFFSET ((unsigned long)(&((struct mem_header *)0)->body))
...
static int MemoryCmd _ANSI_ARGS_((ClientData clientData, Tcl_Interp *interp,
                                  int argc, char **argv));
 ...
 *----------------------------------------------------------------------
 */
static void dump_memory_info(outFile) FILE *outFile;
{
 ...
 *----------------------------------------------------------------------
 */
static void ValidateMemory(memHeaderP, file, line,
                           nukeGuards) struct mem_header *memHeaderP;
char *file;
int line;
int nukeGuards;
{
        ...
        for (idx = 0; idx < LOW_GUARD_SIZE; idx++) {
                ...
                if (byte != GUARD_VALUE) {
                        ...
                        fprintf(stderr, "low guard byte %d is 0x%x  \t%c\n",
                                idx, byte, (isprint(UCHAR(byte)) ? byte : ' '));
                }
        }
        if (guard_failed) {
 ...
 *----------------------------------------------------------------------
 */
void Tcl_ValidateAllMemory(file, line) char *file;
int line;
{
 ...
 *----------------------------------------------------------------------
 */
int Tcl_DumpActiveMemory(fileName) char *fileName;
{
        FILE *fileP;
        struct mem_header *memScanP;
        char *address;
        ...
        for (memScanP = allocHead; memScanP != NULL;
             memScanP = memScanP->flink) {
                ...
                fprintf(fileP, "%8lx - %8lx  %7ld @ %s %d %s",
                        ...
                        (memScanP->tagPtr == NULL) ? ""
                                                   : memScanP->tagPtr->string);
                (void)fputc('\n', fileP);
        }
        fclose(fileP);
        return TCL_OK;
 ...
 *----------------------------------------------------------------------
 */
char *Tcl_DbCkalloc(size, file, line) unsigned int size;
char *file;
int line;
{
        ...
        result = (struct mem_header *)malloc(
            (unsigned)size + sizeof(struct mem_header) + HIGH_GUARD_SIZE);
        if (result == NULL) {
                fflush(stdout);
                dump_memory_info(stderr);
...
int Tcl_DbCkfree(ptr, file, line) char *ptr;
char *file;
int line;
{
        /*
        ...
        struct mem_header *memp =
            (struct mem_header *)(((unsigned long)ptr) - BODY_OFFSET);

        if (alloc_tracing)
                fprintf(stderr, "ckfree %lx %ld %s %d\n",
                        (long unsigned int)memp->body, memp->length, file,
                        line);
        ...
        if (memp->tagPtr != NULL) {
                ...
                if ((memp->tagPtr->refCount == 0) &&
                    (curTagPtr != memp->tagPtr)) {
 ...
 *--------------------------------------------------------------------
 */
char *Tcl_DbCkrealloc(ptr, size, file, line) char *ptr;
unsigned int size;
char *file;
int line;
{
 ...
 */
/* ARGSUSED */
static int MemoryCmd(clientData, interp, argc, argv) ClientData clientData;
Tcl_Interp *interp;
int argc;
char **argv;
{
        ...
        Tcl_AppendResult(interp, "bad option \"", argv[1],
                         ...
                         "tag, trace, trace_on_at_malloc, or validate",
                         (char *)NULL);
        return TCL_ERROR;
 ...
 *----------------------------------------------------------------------
 */
void Tcl_InitMemory(interp) Tcl_Interp *interp;
{
 ...
 *----------------------------------------------------------------------
 */
VOID *Tcl_Ckalloc(size) unsigned int size;
{
        char *result;
