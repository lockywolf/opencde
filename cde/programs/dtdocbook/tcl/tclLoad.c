...
char **argv;        /* Argument strings. */
{
        ...
        if (argc == 4) {
                ...
                if (target == NULL) {
                        Tcl_AppendResult(
                            interp, "couldn't find slave interpreter named \"",
                            argv[3], "\"", (char *)NULL);
                        return TCL_ERROR;
                }
        }
        ...
        for (pkgPtr = firstPackagePtr; pkgPtr != NULL;
             pkgPtr = pkgPtr->nextPtr) {
                ...
                if (gotPkgName) {
                        char *p1, *p2;
                        for (p1 = argv[2], p2 = pkgPtr->packageName;;
                             p1++, p2++) {
                                if ((isupper(*p1) ? tolower(*p1) : *p1) !=
                                    (isupper(*p2) ? tolower(*p2) : *p2)) {
                                        if (fullFileName[0] == 0) {
                                                /*
                                                 * We're looking for a
                                                 * statically loaded package;
                                                 * the file name is basically
                                                 * irrelevant here, so don't get
                                                 * upset that there's some other
                                                 * package with the same (empty
                                                 * string) file name.  Just skip
                                                 * this package and go on to the
                                                 * next.
                                                 */
...
gotPkg:
        ...
        if (pkgPtr != NULL) {
                for (ipPtr = ipFirstPtr; ipPtr != NULL;
                     ipPtr = ipPtr->nextPtr) {
        ...
        if (pkgPtr == NULL) {
                ...
                if (gotPkgName) {
                        ...
                        if (!TclGuessPackageName(fullFileName, &pkgName)) {
                                ...
                                /*
                                 * The platform-specific code couldn't figure
                                 * out the module name.  Make a guess by taking
                                 * the last element of the file name, stripping
                                 * off any leading "lib", and then using all of
                                 * the alphabetic characters that follow that.
                                 */
                                ...
                                Tcl_DStringAppend(&pkgName, pkgGuess,
                                                  (p - pkgGuess));
                                ckfree((char *)pargv);
                        }
                }
                ...
                Tcl_DStringAppend(&safeInitName, Tcl_DStringValue(&pkgName),
                                  -1);
                ...
                code = TclLoadFile(
                    interp, fullFileName, Tcl_DStringValue(&initName),
                ...
                pkgPtr->packageName = (char *)ckalloc(
                    (unsigned)(Tcl_DStringLength(&pkgName) + 1));
        ...
        if (Tcl_IsSafe(target)) {
                if (pkgPtr->safeInitProc != NULL) {
                        ...
                        Tcl_AppendResult(
                            ...
                            pkgPtr->packageName, "_SafeInit procedure",
                            (char *)NULL);
                        code = TCL_ERROR;
                        goto done;
                }
        ...
        if ((code == TCL_ERROR) && (target != interp)) {
                ...
                Tcl_AddErrorInfo(interp,
                                 Tcl_GetVar2(target, "errorInfo", (char *)NULL,
                                             TCL_GLOBAL_ONLY));
                                    ...
                                    * interpreters. */
{
        ...
        pkgPtr->fileName[0] = 0;
        pkgPtr->packageName = (char *)ckalloc((unsigned)(strlen(pkgName) + 1));
