...
char **argv;        /* Argument strings. */
{
        ...
        for (i = 0; i < argc; i++) {
                ...
                if (start == NULL) {
                        if (indices) {
                                result = Tcl_SetVar(interp, argPtr[i + 2],
                                                    "-1 -1", 0);
                        } else {
                                result =
                                    Tcl_SetVar(interp, argPtr[i + 2], "", 0);
                        }
                } else {
                        if (indices) {
                                sprintf(info, "%d %d", (int)(start - string),
                                        (int)(end - string - 1));
                                result =
                                    Tcl_SetVar(interp, argPtr[i + 2], info, 0);
                        } else {
                                ...
                                *last = 0;
                                result =
                                    Tcl_SetVar(interp, argPtr[i + 2], first, 0);
                                *last = savedChar;
                        }
                }
                if (result == NULL) {
...
char **argv;        /* Argument strings. */
{
        ...
        for (p = string; *p != 0;) {
                ...
                c = *src;
                *src = 0;
                newValue = Tcl_SetVar(interp, argPtr[3],
                                      argPtr[1] + (p - string), flags);
                *src = c;
                ...
                /*
                 * Append the subSpec argument to the variable, making
                 * appropriate substitutions.  This code is a bit hairy because
                 * of the backslash conventions and because the code saves up
                 * ranges of characters in subSpec to reduce the number of calls
                 * to Tcl_SetVar.
                 */

                for (src = firstChar = argPtr[2], c = *src; c != 0;
                     src++, c = *src) {
                        int index;
                        ...
                        if (firstChar != src) {
                                ...
                                *src = 0;
                                newValue =
                                    Tcl_SetVar(interp, argPtr[3], firstChar,
                                               TCL_APPEND_VALUE);
                                *src = c;
                                if (newValue == NULL) {
                                        goto cantSet;
                ...
                if (end == p) {
                        ...
                        /*
                         * Always consume at least one character of the input
                         * string in order to prevent infinite loops.
                         */
...
char **argv;        /* Argument strings. */
{
        ...
        if ((dstName[0] == 'b') && (strcmp(dstName, "bgerror") == 0)) {
                ...
                hPtr =
                    Tcl_CreateHashEntry(&iPtr->commandTable, "tkerror", &new);
                Tcl_SetHashValue(hPtr, cmdPtr);
        }
                ...
                if (strcmp(argv[0], "-code") == 0) {
                        ...
                        if ((c == 'o') && (strcmp(argv[1], "ok") == 0)) {
                                ...
                                code = TCL_RETURN;
                                   ...
                                   (strcmp(argv[1], "break") == 0)) {
                                code = TCL_BREAK;
                        } else if ((c == 'c') &&
                                   (strcmp(argv[1], "continue") == 0)) {
                                code = TCL_CONTINUE;
                        } else if (Tcl_GetInt(interp, argv[1], &code) !=
                                   TCL_OK) {
                                Tcl_ResetResult(interp);
                                Tcl_AppendResult(
                                    interp, "bad completion code \"", argv[1],
...
#define STATIC_SIZE 5
        ...
        for (fmt = argv[2]; *fmt != 0; fmt++) {
                ...
                *dst = *fmt;
                ...
                case 'c':
                        if (widthSpecified) {
                                interp->result = "field width may not be "
                                                 "specified in %c conversion";
                                code = TCL_ERROR;
                                goto done;
                        }
                        curField->fmt = 'c';
        ...
        *dst = 0;
        ...
        numScanned = sscanf(
            argv[1], fmtCopy, fields[0].location, fields[1].location,
            fields[2].location, fields[3].location, fields[4].location,
            fields[5].location, fields[6].location, fields[7].location,
            fields[8].location, fields[9].location, fields[10].location,
            fields[11].location, fields[12].location, fields[13].location,
            fields[14].location, fields[15].location, fields[16].location,
            fields[17].location, fields[18].location, fields[19].location);
        ...
        for (i = 0, curField = fields; i < numFields; i++, curField++) {
                ...
                case 'd':
                        ...
                        if (Tcl_SetVar(interp, argv[i + 3], string, 0) ==
                            NULL) {
                        storeError:
                                Tcl_AppendResult(
                                    interp, "couldn't set variable \"",
                                    argv[i + 3], "\"", (char *)NULL);
                                code = TCL_ERROR;
                                goto done;
                        }
                ...
                case 's':
                        if (Tcl_SetVar(interp, argv[i + 3], curField->location,
                                       0) == NULL) {
                                goto storeError;
                        }
                        break;
...
char **argv;        /* Argument strings. */
{
        ...
        if ((c == 'c') && (strncmp(argv[1], "compare", length) == 0)) {
                if (argc != 4) {
                        Tcl_AppendResult(interp, "wrong # args: should be \"",
                                         argv[0], " compare string1 string2\"",
                                         (char *)NULL);
                        return TCL_ERROR;
                }
        ...
        firstLast:
                ...
                if (argc != 5) {
                        Tcl_AppendResult(interp, "wrong # args: should be \"",
                                         argv[0], " range string first last\"",
                                         (char *)NULL);
                        return TCL_ERROR;
                }
        ...
        trim:
                if (argc == 4) {
                        ...
                        Tcl_AppendResult(interp, "wrong # args: should be \"",
                                         argv[0], " ", argv[1],
                                         " string ?chars?\"", (char *)NULL);
                        return TCL_ERROR;
                }
                p = argv[2];
                if (left) {
                        for (c = *p; c != 0; p++, c = *p) {
                                for (checkPtr = trimChars; *checkPtr != c;
                                     checkPtr++) {
                                        if (*checkPtr == 0) {
                                                goto doneLeft;
                                        }
        ...
        wordstartDone:
                ...
                Tcl_AppendResult(
                    ...
                    "\": should be compare, first, index, last, length, "
                    "match, ",
...
char **argv;        /* Argument strings. */
{
        ...
        for (i = 1; i < (argc - 1); i++) {
                ...
                if ((p[3] == 'b') &&
                    (strncmp(p, "-nobackslashes", length) == 0)) {
                        doBackslashes = 0;
                } else if ((p[3] == 'c') &&
...
#define REGEXP 2
        ...
        while ((switchArgc > 0) && (*switchArgv[0] == '-')) {
                if (strcmp(*switchArgv, "-exact") == 0) {
                        ...
                        Tcl_AppendResult(
                            interp, "bad option \"", switchArgv[0],
        ...
        splitArgs = 0;
        if (switchArgc == 1) {
                code = Tcl_SplitList(interp, switchArgv[0], &switchArgc,
                                     &switchArgv);
                if (code != TCL_OK) {
                        return code;
                }
        ...
        for (i = 0; i < switchArgc; i += 2) {
                ...
                if ((*switchArgv[i] == 'd') && (i == switchArgc - 2) &&
                    (strcmp(switchArgv[i], "default") == 0)) {
                        ...
                        case GLOB:
                                matched =
                                    Tcl_StringMatch(string, switchArgv[i]);
                                break;
                        case REGEXP:
                                matched = Tcl_RegExpMatch(interp, string,
                                                          switchArgv[i]);
                                if (matched < 0) {
                                        code = TCL_ERROR;
                                        goto cleanup;
                ...
                for (body = i + 1;; body += 2) {
                        if (body >= switchArgc) {
                                Tcl_AppendResult(
                                    interp, "no body specified for pattern \"",
                                    switchArgv[i], "\"", (char *)NULL);
                                code = TCL_ERROR;
                                goto cleanup;
                        }
                        if ((switchArgv[body][0] != '-') ||
                            (switchArgv[body][1] != 0)) {
                                break;
                        }
                }
...
char **argv;        /* Argument strings. */
{
        ...
        if (argc < 2) {
                Tcl_AppendResult(interp, "too few args: should be \"", argv[0],
                                 " option [arg arg ...]\"", (char *)NULL);
                return TCL_ERROR;
        }
        c = argv[1][1];
        ...
        if ((c == 'a') && (strncmp(argv[1], "variable", length) == 0) &&
            (length >= 2)) {
                ...
                if (argc != 5) {
                        Tcl_AppendResult(
                            interp, "wrong # args: should be \"", argv[0],
                            " variable name ops command\"", (char *)NULL);
                        return TCL_ERROR;
                }
                ...
                tvarPtr = (TraceVarInfo *)ckalloc(
                    (unsigned)(sizeof(TraceVarInfo) - sizeof(tvarPtr->command) +
                               length + 1));
                tvarPtr->flags = flags;
                tvarPtr->errMsg = NULL;
                ...
                if (argc != 5) {
                        Tcl_AppendResult(interp, "wrong # args: should be \"",
                                         argv[0], " vdelete name ops command\"",
                                         (char *)NULL);
                        return TCL_ERROR;
                }
                ...
                clientData = 0;
                while ((clientData =
                            Tcl_VarTraceInfo(interp, argv[2], 0, TraceVarProc,
                                             clientData)) != 0) {
                            ...
                            * information. */
{
        ...
        if ((tvarPtr->flags & flags) && !(flags & TCL_INTERP_DESTROYED)) {
                ...
                Tcl_DStringFree(&cmd);
                if (code != TCL_OK) {
                        tvarPtr->errMsg = (char *)ckalloc(
                            (unsigned)(strlen(interp->result) + 1));
