...
static char *DoTildeSubst _ANSI_ARGS_((Tcl_Interp * interp, char *user,
                                       Tcl_DString *resultPtr));
static char *ExtractWinRoot _ANSI_ARGS_((char *path, Tcl_DString *resultPtr,
                                         int offset));
static void FileNameCleanup _ANSI_ARGS_((ClientData clientData));
static int SkipToChar _ANSI_ARGS_((char **stringPtr, char *match));
static char *SplitMacPath _ANSI_ARGS_((char *path, Tcl_DString *bufPtr));
static char *SplitWinPath _ANSI_ARGS_((char *path, Tcl_DString *bufPtr));
static char *SplitUnixPath _ANSI_ARGS_((char *path, Tcl_DString *bufPtr));
...
static void FileNameCleanup(clientData) ClientData clientData; /* Not used. */
{
...
static char *ExtractWinRoot(path, resultPtr,
                            offset) char *path; /* Path to parse. */
             ...
             * stored. */
{
        ...
        if (winRootPatternPtr->startp[2] != NULL) {
                ...
                length =
                    winRootPatternPtr->endp[3] - winRootPatternPtr->startp[3];
                Tcl_DStringAppend(resultPtr, winRootPatternPtr->startp[3],
                                  length);
                Tcl_DStringAppend(resultPtr, "/", 1);
                length =
                    winRootPatternPtr->endp[4] - winRootPatternPtr->startp[4];
                Tcl_DStringAppend(resultPtr, winRootPatternPtr->startp[4],
                                  length);
        } else {
                Tcl_DStringAppend(resultPtr, "/", 1);
        }
...
Tcl_PathType Tcl_GetPathType(path) char *path;
{
        ...
        case TCL_PLATFORM_MAC:
                if (path[0] == ':') {
                        ...
                        if (!macRootPatternPtr) {
                                macRootPatternPtr =
                                    TclRegComp(MAC_ROOT_PATTERN);
                                if (!initialized) {
                                        Tcl_CreateExitHandler(FileNameCleanup,
                                                              NULL);
                                        initialized = 1;
                                }
                        }
...
void Tcl_SplitPath(
    path, argcPtr,
    argvPtr) char *path; /* Pointer to string containing a path. */
...
static char *
    SplitUnixPath(path,
                  bufPtr) char *path; /* Pointer to string containing a path. */
...
Tcl_DString *bufPtr; /* Pointer to DString to use for the result. */
{
        ...
        if (TclRegExec(macRootPatternPtr, path, path) == 1) {
                ...
                if (macRootPatternPtr->startp[2] != NULL) {
                        Tcl_DStringAppend(bufPtr, ":", 1);
                        Tcl_DStringAppend(bufPtr, path,
                                          macRootPatternPtr->endp[0] -
                ...
                length =
                    macRootPatternPtr->endp[i] - macRootPatternPtr->startp[i];

                /*
                 * Append the element and terminate it with a : and a null.
                 * Note that we are forcing the DString to contain an extra null
                 * at the end.
                 */
        ...
        if (isMac) {
                ...
                while ((p = strchr(p, ':')) != NULL) {
                        ...
                        if (length == 1) {
                                ...
                                /*
                                 * If this is a simple component, drop the
                                 * leading colon.
                                 */
                ...
                for (;;) {
                        ...
                        if (length > 0) {
                                if ((length == 1) && (elementStart[0] == '.')) {
                                        ...
                                        if (*elementStart == '~') {
                                                Tcl_DStringAppend(bufPtr, ":",
                                                                  1);
                                        }
                                        Tcl_DStringAppend(bufPtr, elementStart,
                                                          length);
                                        Tcl_DStringAppend(bufPtr, "", 1);
                                }
                        }
...
Tcl_DString *resultPtr; /* Pointer to previously initialized DString. */
{
        ...
        case TCL_PLATFORM_UNIX:
                for (i = 0; i < argc; i++) {
                        ...
                        if (*p == '/') {
                                ...
                                Tcl_DStringSetLength(resultPtr, oldLength);
                        } else if ((Tcl_DStringLength(resultPtr) !=
                                    oldLength) &&
                                   (p[0] == '.') && (p[1] == '/') &&
                                   (p[2] == '~')) {
                                p += 2;
                        ...
                        Tcl_DStringSetLength(resultPtr,
                                             (int)(length + strlen(p)));
        ...
        case TCL_PLATFORM_WINDOWS:
                ...
                for (i = 0; i < argc; i++) {
                        ...
                        /*
                         * If the pointer didn't move, then this is a relative
                         * path or a tilde prefixed path.
                         */

                        if (p == argv[i]) {
                                /*
                                 * Remove the ./ from tilde prefixed elements
                                 * unless it is the first component.
                                 */

                                if ((length != oldLength) && (p[0] == '.') &&
                                    ((p[1] == '/') || (p[1] == '\\')) &&
                                    (p[2] == '~')) {
                                        p += 2;
                                } else if (*p == '~') {
        ...
        case TCL_PLATFORM_MAC:
                ...
                for (i = 0; i < argc; i++) {
                        ...
                        for (; *p != '\0'; p += length + 1) {
                                if (p[0] == ':' && p[1] == '\0') {
                                        if (Tcl_DStringLength(resultPtr) !=
                                            oldLength) {
                                                p++;
                                        } else {
                                                needsSep = 0;
...
char *TclGetExtension(name) char *name; /* File name to parse. */
{
        char *p, *lastSep;
...
static char *DoTildeSubst(interp, user, resultPtr)
    Tcl_Interp *interp; /* Interpreter in which to store error
                         * message (if necessary). */
        ...
        if (*user == '\0') {
                ...
                if (dir == NULL) {
                        if (interp) {
                                ...
                                Tcl_AppendResult(
                                    interp, "couldn't find HOME environment ",
                ...
                if (TclGetUserHome(user, resultPtr) == NULL) {
                        if (interp) {
                                Tcl_ResetResult(interp);
                                Tcl_AppendResult(interp, "user \"", user,
                                                 "\" doesn't exist",
                                                 (char *)NULL);
                        }
                        return NULL;
...
char **argv;        /* Argument strings. */
{
        ...
        for (firstArg = 1; (firstArg < argc) && (argv[firstArg][0] == '-');
             firstArg++) {
                if (strcmp(argv[firstArg], "-nocomplain") == 0) {
                        ...
                        Tcl_AppendResult(
                            interp, "bad switch \"", argv[firstArg],
        ...
        for (i = firstArg; i < argc; i++) {
                ...
                if (argv[i][0] == '~') {
                         ...
                         * Note that we don't allow special characters in the
                         * user name.
                         */
                        ...
                        *tail = '\0';
                        ...
                        if (p == NULL) {
                                head =
                                    DoTildeSubst(interp, argv[i] + 1, &buffer);
                        } else {
                                if (!noComplain) {
                                        Tcl_ResetResult(interp);
                                        Tcl_AppendResult(
                                            interp, "globbing characters not ",
                                            "supported in user names",
                                            (char *)NULL);
                                }
                                head = NULL;
                        }
...
static int SkipToChar(stringPtr,
                      match) char **stringPtr; /* Pointer string to check. */
...
int TclDoGlob(interp, separators, headPtr, tail)
...
char *tail;             /* The unexpanded remainder of the path. */
{
        ...
        case TCL_PLATFORM_UNIX:
                ...
                if ((*tail != '\0') &&
                    (((length > 0) && (strchr(separators, lastChar) == NULL)) ||
                     ((length == 0) && (count > 0)))) {
                        Tcl_DStringAppend(headPtr, "/", 1);
                }
                break;
        ...
        *p = '\0';
        ...
        if (strpbrk(tail, "*[]?\\") != NULL) {
                *p = savedChar;
                 ...
                 * recursively call TclDoGlob.  For each file that matches, it
                 * will add the match onto the interp->result, or call TclDoGlob
