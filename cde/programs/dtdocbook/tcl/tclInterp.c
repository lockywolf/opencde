...
typedef struct {
        ...
        Tcl_HashEntry *aliasEntry; /* Entry for the alias hash table in slave.
                                    ...
                                    * alias table. */
        Tcl_HashEntry
            *targetEntry;     /* Entry for target command in master.
...
typedef struct {
        Tcl_HashTable
            slaveTable; /* Hash table for slave interpreters.
                         * Maps from command names to Slave records. */
        int isSafe;     /* Am I a "safe" interpreter? */
        Tcl_HashTable
            targetTable; /* Hash table for Target Records. Contains
...
static int AliasCmd _ANSI_ARGS_((ClientData dummy, Tcl_Interp *currentInterp,
                                 int argc, char **argv));
...
static int AliasHelper _ANSI_ARGS_((Tcl_Interp * curInterp,
                                    ...
                                    Tcl_Interp *masterInterp, Master *masterPtr,
                                    char *aliasName, char *targetName, int argc,
                                    char **argv));
static int CreateInterpObject _ANSI_ARGS_((Tcl_Interp * interp, int argc,
                                           char **argv));
...
static int DescribeAlias _ANSI_ARGS_((Tcl_Interp * interp,
                                      Tcl_Interp *slaveInterp,
                                      char *aliasName));
static int DeleteInterpObject _ANSI_ARGS_((Tcl_Interp * interp, int argc,
                                           char **argv));
static int DeleteOneInterpObject _ANSI_ARGS_((Tcl_Interp * interp, char *path));
...
static int SlaveAliasHelper _ANSI_ARGS_((Tcl_Interp * interp, int argc,
                                         char **argv));
static int SlaveObjectCmd _ANSI_ARGS_((ClientData dummy, Tcl_Interp *interp,
                                       int argc, char **argv));
...
static char *TclCommandsToKeep[] = {
    "after",   "append", "array",    "break",      "case",    "catch",
    "clock",   "close",  "concat",   "continue",   "eof",     "error",
    "eval",    "expr",   "fblocked", "fconfigure", "flush",   "for",
    "foreach", "format", "gets",     "global",     "history", "if",
    "incr",    "info",   "interp",   "join",       "lappend", "lindex",
    "linsert", "list",   "llength",  "lower",      "lrange",  "lreplace",
    "lsearch", "lsort",  "package",  "pid",        "proc",    "puts",
    "read",    "regexp", "regsub",   "rename",     "return",  "scan",
    "seek",    "set",    "split",    "string",     "switch",  "tell",
    "trace",   "unset",  "update",   "uplevel",    "upvar",   "vwait",
    "while",   NULL};
        ...
        while (1) {
                ...
                if ((strcmp(nextAliasPtr->targetName, cmdName) == 0) &&
                    (nextAliasPtr->targetInterp == cmdInterp)) {
                        Tcl_AppendResult(
                            interp, "cannot define or rename alias \"",
...
static int
    MakeSafe(interp) Tcl_Interp *interp; /* Interpreter to be made safe. */
{
...
int safe;               /* Should we make it "safe"? */
{
        ...
        if (argc < 2) {
                ...
                masterInterp =
                    GetInterp(interp, masterPtr, masterPath, &masterPtr);
                if (masterInterp == (Tcl_Interp *)NULL) {
                        Tcl_AppendResult(interp, "interpreter named \"",
                                         masterPath, "\" not found",
                                         (char *)NULL);
                        ckfree((char *)argv);
        ...
        if (slaveInterp == (Tcl_Interp *)NULL) {
                panic("CreateSlave: out of memory while creating a new "
                      "interpreter");
        }
        ...
        slavePtr->interpCmd =
            Tcl_CreateCommand(masterInterp, slavePath, SlaveObjectCmd,
                              (ClientData)slaveInterp, SlaveObjectDeleteProc);
        ...
        if (((safe) && (MakeSafe(slaveInterp) == TCL_ERROR)) ||
            ((!safe) && (Tcl_Init(slaveInterp) == TCL_ERROR))) {
                ...
                Tcl_AddErrorInfo(interp,
                                 Tcl_GetVar2(slaveInterp, "errorInfo",
                ...
                if (slaveInterp->freeProc != NULL) {
                        ...
                        Tcl_SetResult(interp, slaveInterp->result,
                                      TCL_VOLATILE);
                }
...
char **argv;            /* Argument strings. */
{
        ...
        for (i = 2; i < argc; i++) {
                ...
                if ((argv[i][0] == '-') && (moreFlags != 0)) {
                        if ((argv[i][1] == 's') &&
                            (strncmp(argv[i], "-safe", len) == 0) &&
                            (len > 1)) {
                                safe = 1;
                        } else if ((strncmp(argv[i], "--", len) == 0) &&
                                   (len > 1)) {
                                moreFlags = 0;
                        } else {
                                Tcl_AppendResult(interp, "bad option \"",
                                                 argv[i], "\": should be -safe",
                                                 (char *)NULL);
                                return TCL_ERROR;
                        }
                } else {
...
static int AliasHelper(curInterp, slaveInterp, masterInterp, masterPtr,
...
char **argv;               /* with alias. */
{
        ...
        if ((targetName == (char *)NULL) || (targetName[0] == '\0')) {
                if (argc != 0) {
                        Tcl_AppendResult(
                            curInterp, "malformed command: should be",
        ...
        aliasPtr->slaveCmd =
            Tcl_CreateCommand(slaveInterp, aliasName, AliasCmd,
        ...
        do {
                hPtr = Tcl_CreateHashEntry(&(slavePtr->aliasTable), aliasName,
                                           &new);
                if (new == 0) {
                        tmpAliasPtr = (Alias *)Tcl_GetHashValue(hPtr);
                        (void)Tcl_DeleteCommand(slaveInterp,
                                                tmpAliasPtr->aliasName);
        ...
        do {
                hPtr =
                    Tcl_CreateHashEntry(&(masterPtr->targetTable),
...
static int SlaveAliasHelper(interp, argc,
                            argv) Tcl_Interp *interp; /* Current interpreter. */
int argc;                                             /* Number of arguments. */
char **argv;                                          /* Argument strings. */
{
        ...
        if (argc < 4) {
                Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0],
                                 " alias slavePath slaveCmd masterPath "
                                 "masterCmd ?args ..?\"",
                                 (char *)NULL);
                return TCL_ERROR;
        }
...
char *aliasName;              /* The target of this allias. */
{
        ...
        if (hPtr == (Tcl_HashEntry *)NULL) {
                Tcl_AppendResult(askingInterp, "alias \"", aliasName,
                                 "\" in path \"", path, "\" not found",
                                 (char *)NULL);
                return TCL_ERROR;
        }
        aliasPtr = (Alias *)Tcl_GetHashValue(hPtr);
 ...
 */
/* ARGSUSED */
int Tcl_InterpCmd(clientData, interp, argc,
                  argv) ClientData clientData; /* Unused. */
...
char **argv;                                   /* Argument strings. */
{
        ...
        if (cmdName[0] == 'a') {
                ...
                if (strcmp(cmdName, "aliases") == 0) {
                        ...
                        if (argc == 3) {
                                slaveInterp =
                                    GetInterp(interp, masterPtr, argv[2], NULL);
                                if (slaveInterp == (Tcl_Interp *)NULL) {
                                        Tcl_AppendResult(
                        ...
                        for (hPtr = Tcl_FirstHashEntry(&(slavePtr->aliasTable),
                             ...
                             hPtr != NULL; hPtr = Tcl_NextHashEntry(&hSearch)) {
                                Tcl_AppendElement(
                                    interp, Tcl_GetHashKey(
                                                &(slavePtr->aliasTable), hPtr));
                        }
                        return TCL_OK;
                }
        }
        ...
        if (cmdName[0] == 'e') {
                if ((strncmp(cmdName, "exists", len) == 0) && (len > 1)) {
                        if (argc > 3) {
                                Tcl_AppendResult(
                                    interp, "wrong # args: should be \"",
                                    argv[0], " exists ?path?\"", (char *)NULL);
                                return TCL_ERROR;
                        }
                        if (argc == 3) {
                                if (GetInterp(interp, masterPtr, argv[2],
                                              NULL) == (Tcl_Interp *)NULL) {
                ...
                if ((strncmp(cmdName, "eval", len) == 0) && (len > 1)) {
                        ...
                        /*
                         * Now make the result and any error information
                         * accessible. We have to be careful because the slave
                         * interpreter and the current interpreter can be the
                         * same - do not destroy the result.. This can happen if
                         * an interpreter contains an alias which is directed at
                         * a target command in the same interpreter.
                         */
                        ...
                        if (interp != slaveInterp) {
                                if (result == TCL_ERROR) {
                                        ...
                                        /*
                                         * An error occurred, so transfer error
                                         * information from the target
                                         * interpreter back to our interpreter.
                                         * Must clear interp's result before
                                         * calling Tcl_AddErrorInfo, since
                                         * Tcl_AddErrorInfo will store the
                                         * interp's result in errorInfo before
                                         * appending slaveInterp's $errorInfo;
                                         * we've already got everything we need
                                         * in the slave interpreter's
                                         * $errorInfo.
                                         */
                                        ...
                                        Tcl_AddErrorInfo(
                                            interp,
                                            Tcl_GetVar2(
                                                slaveInterp, "errorInfo",
                                                (char *)NULL, TCL_GLOBAL_ONLY));
                                        Tcl_SetVar2(
                                            interp, "errorCode", (char *)NULL,
                                            Tcl_GetVar2(
                                                slaveInterp, "errorCode",
                                                (char *)NULL, TCL_GLOBAL_ONLY),
                                            TCL_GLOBAL_ONLY);
                                }
                                if (slaveInterp->freeProc != NULL) {
        ...
        if (cmdName[0] == 's') {
                if ((strncmp(cmdName, "slaves", len) == 0) && (len > 1)) {
                        ...
                        for (hPtr = Tcl_FirstHashEntry(&(masterPtr->slaveTable),
                                                       &hSearch);
                             hPtr != NULL; hPtr = Tcl_NextHashEntry(&hSearch)) {
                                Tcl_AppendElement(
                                    interp,
                                    Tcl_GetHashKey(&(masterPtr->slaveTable),
                                                   hPtr));
                        }
                        return TCL_OK;
                }
                if ((strncmp(cmdName, "share", len) == 0) && (len > 1)) {
                        ...
                        slaveInterp =
                            GetInterp(interp, masterPtr, argv[4], NULL);
                        if (slaveInterp == (Tcl_Interp *)NULL) {
                                Tcl_AppendResult(interp, "interpreter \"",
                                                 argv[4], "\" not found",
                                                 (char *)NULL);
...
static int SlaveObjectCmd(clientData, interp, argc,
                          argv) ClientData clientData; /* Slave interpreter. */
Tcl_Interp *interp; /* Current interpreter. */
int argc;           /* Number of arguments. */
char **argv;        /* Argument strings. */
{
        ...
        if (cmdName[0] == 'a') {
                if (strncmp(cmdName, "alias", len) == 0) {
                        ...
                        case 1:
                                 ...
                                 * interpreter for which the argument is an
                                 * alias in the slave interpreter, and the list
                                 * of saved arguments
                                 */
                        ...
                        default:
                                ...
                                if (masterPtr == (Master *)NULL) {
                                        panic("SlaveObjectCmd: could not find "
                                              "master record");
                                }
                                return AliasHelper(interp, slaveInterp, interp,
                                                   masterPtr, argv[2], argv[3],
                                                   argc - 4, argv + 4);
                        }
                }
        ...
        if ((cmdName[0] == 'e') && (strncmp(cmdName, "eval", len) == 0)) {
                 ...
                 * which is directed at a target command in the same
                 * interpreter.
                 */
                ...
                if (interp != slaveInterp) {
                        if (result == TCL_ERROR) {
                                ...
                                /*
                                 * An error occurred, so transfer error
                                 * information from the destination interpreter
                                 * back to our interpreter.  Must clear interp's
                                 * result before calling Tcl_AddErrorInfo, since
                                 * Tcl_AddErrorInfo will store the interp's
                                 * result in errorInfo before appending
                                 * slaveInterp's $errorInfo; we've already got
                                 * everything we need in the slave interpreter's
                                 * $errorInfo.
                                 */
    ...
    ClientData clientData; /* The SlaveRecord for the command. */
{
         ...
         * the master interpreter. This can happen if this slave is being
         * deleted because the master is being deleted and the slave deletion is
...
static int AliasCmd(clientData, interp, argc,
                    argv) ClientData clientData; /* Alias record. */
Tcl_Interp *interp;                              /* Current interpreter. */
int argc;                                        /* Number of arguments. */
char **argv;                                     /* Argument strings. */
{
        ...
        result = Tcl_GetCommandInfo(aliasPtr->targetInterp,
                                    aliasPtr->targetName, &cmdInfo);
        if (result == 0) {
                Tcl_AppendResult(interp, "aliased target \"",
                                 aliasPtr->targetName, "\" for \"", argv[0],
                                 "\" not found", (char *)NULL);
                return TCL_ERROR;
        }
        if (aliasPtr->argc <= 0) {
        ...
        if (interp != (Tcl_Interp *)iPtr) {
                if (result == TCL_ERROR) {
                         ...
                         * Some tricky points:
                         * 1. Must call Tcl_AddErrorInfo in destination
                         * interpreter to make sure that the errorInfo variable
                         * has been initialized (it's initialized lazily and
                         * might not have been initialized yet).
                         * 2. Must clear interp's result before calling
                         * Tcl_AddErrorInfo, since Tcl_AddErrorInfo will store
                         * the interp's result in errorInfo before appending
                         * aliasPtr->interp's $errorInfo; we've already got
                         * everything we need in the redirected interpreter's
                         * $errorInfo.
                         */
                        ...
                        Tcl_ResetResult(interp);
                        Tcl_AddErrorInfo(
                            interp, Tcl_GetVar2((Tcl_Interp *)iPtr, "errorInfo",
                                                (char *)NULL, TCL_GLOBAL_ONLY));
                        Tcl_SetVar2(interp, "errorCode", (char *)NULL,
...
Tcl_Interp *interp;        /* Interpreter being deleted. */
{
        ...
        if (slavePtr->interpCmd != (Tcl_Command)NULL) {
                 ...
                 * directly, rather than via "interp delete", or equivalent
                 * (deletion of the command in the master).
                 *
                 * Perform the cleanup done by SlaveObjectDeleteProc() directly,
                 * and turn off the callback now (since we are about to free
                 * slavePtr and this interpreter is going away, while the
                 * deletion of commands in the master may be deferred).
                 */
                 ...
                 * relying on the stored name; the command may have been
                 * renamed.
                 */
        ...
        for (hPtr = Tcl_FirstHashEntry(hTblPtr, &hSearch);
             ...
             hPtr = Tcl_FirstHashEntry(hTblPtr, &hSearch)) {
                 ...
                 * storage will be deleted in the command deletion callback.
                 * Hence we must use the name for the command as stored in the
                 * hash table.
                 */

                Tcl_DeleteCommand(
                    interp, Tcl_GetCommandName(interp, aliasPtr->slaveCmd));
        }
...
int TclInterpInit(interp) Tcl_Interp *interp; /* Interpreter to initialize. */
{
        ...
        (void)Tcl_SetAssocData(interp, "tclMasterRecord",
                               MasterRecordDeleteProc, (ClientData)masterPtr);
...
int Tcl_GetAlias(interp, aliasName, targetInterpPtr, targetNamePtr, argcPtr,
