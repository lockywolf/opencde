...
void Tcl_DeleteAssocData(interp, name)
    Tcl_Interp *interp; /* Interpreter to associate with. */
...
static void
    DeleteInterpProc(interp) Tcl_Interp *interp; /* Interpreter to delete. */
{
        Interp *iPtr = (Interp *)interp;
        Tcl_HashEntry *hPtr;
        ...
        if (!(iPtr->flags & DELETED)) {
                panic("DeleteInterpProc called on interpreter not marked "
                      "deleted");
        }
 ...
 * this command is deleted. */
{
        ...
        if (iPtr->flags & DELETED) {
                 ...
                 * commands;  it's not safe to muck with the interpreter
                 * anymore.
                 */
        ...
        if (!new) {
                ...
                if (!new) {
                        /*
                         * Drat.  The stupid deletion callback recreated the
                         * command. Just throw away the new command (if we try
                         * to delete it again, we could get stuck in an infinite
                         * loop).
                         */
...
char *Tcl_GetCommandName(interp, command)
...
#define NUM_ARGS 10
        ...
        while (*src != termChar) {
                ...
                while (1) {
                        ...
                        /*
                         * Note:  the "- 2" below guarantees that we won't use
                         * the last two argv slots here.  One is for a NULL
                         * pointer to mark the end of the list, and the other is
                         * to leave room for inserting the command name
                         * "unknown" as the first argument (see below).
                         */
                        ...
                        /*
                         * Careful!  Buffer space may have gotten reallocated
                         * while parsing words.  If this happened, be sure to
                         * update all of the older argv pointers to refer to the
                         * new space.
                         */
                        ...
                        /*
                         * Args didn't all fit in the current array.  Make it
                         * bigger.
                         */

                        argSize *= 2;
                        newArgv = (char **)ckalloc((unsigned)argSize *
                                                   sizeof(char *));
                        for (i = 0; i < argc; i++) {
                                newArgv[i] = argv[i];
                        }
                ...
                if (hPtr == NULL) {
                        ...
                        hPtr =
                            Tcl_FindHashEntry(&iPtr->commandTable, "unknown");
                        if (hPtr == NULL) {
                                Tcl_ResetResult(interp);
                ...
                iPtr->resultSpace[0] = 0;
                result =
                    (*cmdPtr->proc)(cmdPtr->clientData, interp, argc, argv);
                if (Tcl_AsyncReady()) {
...
void Tcl_DeleteTrace(interp, trace)
...
char *message;          /* Message to record. */
{
        ...
        if (!(iPtr->flags & ERR_IN_PROGRESS)) {
                ...
                /*
                 * If the errorCode variable wasn't set by the code that
                 * generated the error, set it to "NONE".
                 */
 ...
 */
/* VARARGS2 */ /* ARGSUSED */
int Tcl_VarEval TCL_VARARGS_DEF(Tcl_Interp *, arg1) {
        va_list argList;
        Tcl_DString buf;
        char *string;
        Tcl_Interp *interp;
...
int Tcl_GlobalEval(interp, command)
