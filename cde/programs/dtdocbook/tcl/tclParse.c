                         ...
                         * fully-substituted result of parse. */
{
        ...
        while (1) {
                if (dst == pvPtr->end) {
                        /*
                         * Target buffer space is about to run out.  Make more
                         * space.
                         */
                ...
                copy:
                        *dst = c;
                        ...
                        pvPtr->next = dst;
                        result = TclParseNestedCmd(interp, src, flags, termPtr,
                                                   pvPtr);
                        if (result != TCL_OK) {
                                return result;
                        }
        ...
        while (1) {
                ...
                *dst = c;
                ...
                if (CHAR_TYPE(c) == TCL_NORMAL) {
                        ...
                        /*
                         * Must always squish out backslash-newlines, even when
                         * in braces.  This is needed so that this sequence can
                         * appear anywhere in a command, such as the middle of
                         * an expression.
                         */
                        ...
                        Tcl_SetResult(interp, "missing close-brace",
                                      TCL_STATIC);
                        *termPtr = string - 1;
                        return TCL_ERROR;
                }
...
int TclParseWords(interp, string, flags, maxWords, termPtr, argcPtr, argv,
                  pvPtr)
    Tcl_Interp *interp;     /* Interpreter to use for nested command
                             * evaluations and error messages. */
                             ...
                             * fully-substituted word. */
{
        ...
        skipSpace:
                ...
                /*
                 * Handle the normal case (i.e. no leading double-quote or
                 * brace).
                 */
                ...
                normalArg:
                        while (1) {
                                if (dst == pvPtr->end) {
                                        /*
                                         * Target buffer space is about to run
                                         * out.  Make more space.
                                         */
                                ...
                                copy:
                                        *dst = c;
                                        ...
                                        value =
                                            Tcl_ParseVar(interp, src, termPtr);
                                        if (value == NULL) {
                                                return TCL_ERROR;
                                        }
                                        src = *termPtr;
                                        ...
                                        /*
                                         * End of command;  simulate a word-end
                                         * first, so that the end-of-command can
                                         * be processed as the first thing in a
                                         * new word.
                                         */
                                        ...
                                        *dst = Tcl_Backslash(src, &numRead);
                                        ...
                                        /*
                                         * The following special check allows a
                                         * backslash-newline to be treated as a
                                         * word-separator, as if the backslash
                                         * and newline had been collapsed before
                                         * command parsing began.
                                         */
                        ...
                        /*
                         * Now handle the special cases: open braces,
                         * double-quotes, and backslash-newline.
                         */

                        pvPtr->next = dst;
                        if (type == TCL_QUOTE) {
                                result = TclParseQuotes(interp, src + 1, '"',
                                                        flags, termPtr, pvPtr);
                        } else if (type == TCL_OPEN_BRACE) {
                                result = TclParseBraces(interp, src + 1,
                                                        termPtr, pvPtr);
                        } else if ((type == TCL_BACKSLASH) &&
                                   (src[1] == '\n')) {
                                /*
                                 * This code is needed so that a
                                 * backslash-newline at the very beginning of a
                                 * word is treated as part of the white space
                                 * between words and not as a space within the
                                 * word.
                                 */
                        ...
                        /*
                         * Back from quotes or braces;  make sure that the
                         * terminating character was the end of the word.
                         */

                        c = **termPtr;
                        if ((c == '\\') && ((*termPtr)[1] == '\n')) {
                                /*
                                 * Line is continued on next line;  the
                                 * backslash-newline sequence turns into space,
                                 * which is OK.  No need to do anything here.
                                 */
                        } else {
                                type = CHAR_TYPE(c);
                                if ((type != TCL_SPACE) &&
...
void TclExpandParseValue(
    pvPtr,
    needed) register ParseValue *pvPtr; /* Information about buffer that
...
char *
    TclWordEnd(start, nested,
               semiPtr) char *start; /* Beginning of a word of a Tcl command. */
                                      ...
                                      * If NULL then ignored. */
{
        ...
        while (1) {
                if (*p == '[') {
                        ...
                        if (p[1] == '\n') {
                                /*
                                 * Backslash-newline:  it maps to a space
                                 * character that is a word separator, so the
                                 * word ends just before the backslash.
                                 */
                        ...
                        if (nested) {
                                /*
                                 * Nested commands can't end because of the end
                                 * of the string.
                                 */
                                return p;
                        }
...
static char *
    QuoteEnd(string,
             term) char *string; /* Pointer to character just after opening
                                  * "quote". */
...
static char *
    VarNameEnd(string) char *string; /* Pointer to dollar-sign character. */
{
                ...
                if ((*p == '#') && commentOK) {
                        do {
                                if (*p == '\\') {
                                         ...
                                         * backslash-newline, then this command
                                         * isn't complete.
                                         */
...
#define NUM_CHARS 200
        ...
        if (*string == '{') {
                ...
                if (*string == '(') {
                        ...
                        /*
                         * Perform substitutions on the array element name, just
                         * as is done for quotes.
                         */
                        ...
                        if (TclParseQuotes(interp, string + 1, ')', 0, &end,
                                           &pv) != TCL_OK) {
                                char msg[200];
                                int length;
