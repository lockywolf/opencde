...
/* ARGSUSED */
int Tcl_PutsCmd(clientData, interp, argc,
...
/* ARGSUSED */
int Tcl_FlushCmd(clientData, interp, argc,
                 argv) ClientData clientData; /* Not used. */
Tcl_Interp *interp;                           /* Current interpreter. */
int argc;                                     /* Number of arguments. */
char **argv;                                  /* Argument strings. */
...
/* ARGSUSED */
int Tcl_GetsCmd(clientData, interp, argc,
                argv) ClientData clientData; /* Not used. */
Tcl_Interp *interp;                          /* Current interpreter. */
int argc;                                    /* Number of arguments. */
char **argv;                                 /* Argument strings. */
...
/* ARGSUSED */
int Tcl_ReadCmd(clientData, interp, argc,
                argv) ClientData clientData; /* Not used. */
Tcl_Interp *interp;                          /* Current interpreter. */
int argc;                                    /* Number of arguments. */
char **argv;                                 /* Argument strings. */
{
        ...
        for (charactersRead = 0; charactersRead < toRead;) {
                ...
                charactersReadNow = Tcl_Read(
                    chan, Tcl_DStringValue(&ds) + charactersRead, toReadNow);
...
int TclUnsupported0Cmd(clientData, interp, argc,
                       argv) ClientData clientData; /* Not used. */
...
/* ARGSUSED */
int Tcl_SeekCmd(clientData, interp, argc,
                argv) ClientData clientData; /* Not used. */
Tcl_Interp *interp;                          /* Current interpreter. */
int argc;                                    /* Number of arguments. */
char **argv;                                 /* Argument strings. */
        ...
        if (argc == 4) {
                ...
                if ((c == 's') && (strncmp(argv[3], "start", length) == 0)) {
                        mode = SEEK_SET;
                           ...
                           (strncmp(argv[3], "current", length) == 0)) {
                        mode = SEEK_CUR;
                } else if ((c == 'e') &&
                           (strncmp(argv[3], "end", length) == 0)) {
                        mode = SEEK_END;
                } else {
...
/* ARGSUSED */
int Tcl_TellCmd(clientData, interp, argc,
                argv) ClientData clientData; /* Not used. */
Tcl_Interp *interp;                          /* Current interpreter. */
int argc;                                    /* Number of arguments. */
char **argv;                                 /* Argument strings. */
...
/* ARGSUSED */
int Tcl_CloseCmd(clientData, interp, argc,
                 argv) ClientData clientData; /* Not used. */
Tcl_Interp *interp;                           /* Current interpreter. */
int argc;                                     /* Number of arguments. */
char **argv;                                  /* Argument strings. */
{
        ...
        if (Tcl_UnregisterChannel(interp, chan) != TCL_OK) {
                ...
                /*
                 * If there is an error message and it ends with a newline,
                 * remove the newline. This is done for command pipeline
                 * channels where the error output from the subprocesses is
...
/* ARGSUSED */
int Tcl_FconfigureCmd(clientData, interp, argc,
                      argv) ClientData clientData; /* Not used. */
Tcl_Interp *interp;                                /* Current interpreter. */
int argc;                                          /* Number of arguments. */
char **argv;                                       /* Argument strings. */
{
        ...
        for (i = 3; i < argc; i += 2) {
                result =
                    Tcl_SetChannelOption(interp, chan, argv[i - 1], argv[i]);
                if (result != TCL_OK) {
                        return result;
                }
        }
...
char **argv;        /* Argument strings. */
{
        ...
        for (firstWord = 1; (firstWord < argc) && (argv[firstWord][0] == '-');
             firstWord++) {
                if (strcmp(argv[firstWord], "-keepnewline") == 0) {
                        ...
                        Tcl_AppendResult(
                            interp, "bad switch \"", argv[firstWord],
        ...
        chan =
            Tcl_OpenCommandChannel(interp, argc - firstWord, argv + firstWord,
        ...
        if (background) {
                ...
                /*
                 * Get the list of PIDs from the pipeline into interp->result
                 * and detach the PIDs (instead of waiting for them).
                 */
...
#define EXEC_BUFFER_SIZE 4096
                ...
                while (1) {
                        ...
                        readNow =
                            Tcl_Read(chan, Tcl_DStringValue(&ds) + readSoFar,
                                     EXEC_BUFFER_SIZE);
                        if (readNow < 0) {
                                Tcl_DStringFree(&ds);
                                Tcl_AppendResult(
...
/* ARGSUSED */
int Tcl_FblockedCmd(unused, interp, argc,
                    argv) ClientData unused; /* Not used. */
Tcl_Interp *interp;                          /* Current interpreter. */
int argc;                                    /* Number of arguments. */
char **argv;                                 /* Argument strings. */
{
...
char **argv;        /* Argument strings. */
{
        ...
        if (!pipeline) {
                ...
                if (Tcl_SplitList(interp, argv[1] + 1, &cmdArgc, &cmdArgv) !=
                    TCL_OK) {
                        return TCL_ERROR;
                }
                ...
                if (mode == -1) {
                        ...
                        chan = Tcl_OpenCommandChannel(interp, cmdArgc, cmdArgv,
                                                      flags);
                }
                ckfree((char *)cmdArgv);
        }
...
/* ARGSUSED */
static void TcpAcceptCallbacksDeleteProc(clientData, interp)
...
static void RegisterTcpServerInterpCleanup(interp, acceptCallbackPtr)
 ...
 * the interpreter is deleted. */
{
        ...
        hTblPtr = (Tcl_HashTable *)Tcl_GetAssocData(
            interp, "tclTCPAcceptCallbacks", NULL);
...
static void UnregisterTcpServerInterpCleanupProc(interp, acceptCallbackPtr)
...
static void AcceptCallbackProc(callbackData, chan, address, port)
...
int port;                    /* Port of client that was accepted. */
{
        ...
        if (acceptCallbackPtr->interp != (Tcl_Interp *)NULL) {
                ...
                Tcl_RegisterChannel(interp, chan);
                result =
                    Tcl_VarEval(interp, script, " ", Tcl_GetChannelName(chan),
...
static void TcpServerCloseProc(callbackData)
...
int Tcl_SocketCmd(notUsed, interp, argc,
                  argv) ClientData notUsed; /* Not used. */
Tcl_Interp *interp;                         /* Current interpreter. */
int argc;                                   /* Number of arguments. */
char **argv;                                /* Argument strings. */
{
        ...
        for (a = 1; a < argc; a++) {
                ...
                if (arg[0] == '-') {
                        if (strcmp(arg, "-server") == 0) {
                                ...
                                if (TclSockGetPort(interp, argv[a], "tcp",
                                                   &myport) != TCL_OK) {
                                        return TCL_ERROR;
                                }
        ...
        wrongNumArgs:
                Tcl_AppendResult(
                    ...
                    argv[0], " -server command ?-myaddr addr? port",
                    (char *)NULL);
                return TCL_ERROR;
        }
        ...
        if (server) {
                acceptCallbackPtr =
                    (AcceptCallback *)ckalloc((unsigned)sizeof(AcceptCallback));
                ...
                chan = Tcl_OpenTcpClient(interp, port, host, myaddr, myport,
                                         async);
                if (chan == (Tcl_Channel)NULL) {
                        return TCL_ERROR;
                }
