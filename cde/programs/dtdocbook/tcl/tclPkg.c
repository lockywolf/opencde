...
char *version;          /* Version string for package. */
{
        ...
        Tcl_AppendResult(interp, "conflicting versions provided for package \"",
                         name, "\": ", pkgPtr->version, ", then ", version,
                         (char *)NULL);
        return TCL_ERROR;
}
                         ...
                         * use the latest compatible version. */
{
        ...
        for (pass = 1;; pass++) {
                 ...
                 * versions and invoke the script for the best available
                 * version.
                 */
                ...
                for (availPtr = pkgPtr->availPtr; availPtr != NULL;
                     availPtr = availPtr->nextPtr) {
                        if ((bestPtr != NULL) &&
                            (ComparePkgVersions(availPtr->version,
                                                bestPtr->version,
                                                (int *)NULL) <= 0)) {
                                continue;
                        }
                        if (version != NULL) {
                                result = ComparePkgVersions(
                                    availPtr->version, version, &satisfies);
                                if ((result != 0) && exact) {
                                        continue;
                                }
                                if (!satisfies) {
                ...
                if (bestPtr != NULL) {
                        /*
                         * We found an ifneeded script for the package.  Be
                         * careful while executing it:  this could cause
                         * reentrancy, so (a) protect the script itself from
                         * deletion and (b) don't assume that bestPtr will still
                         * exist when the script completes.
                         */
        ...
        if ((c == 'f') && (strncmp(argv[1], "forget", length) == 0)) {
                ...
                for (availPtr = pkgPtr->availPtr, prevPtr = NULL;
                     ...
                     prevPtr = availPtr, availPtr = availPtr->nextPtr) {
                        if (ComparePkgVersions(availPtr->version, argv[3],
                                               (int *)NULL) == 0) {
                                if (argc == 4) {
                     ...
                     hPtr = Tcl_NextHashEntry(&search)) {
                        ...
                        if ((pkgPtr->version != NULL) ||
                            (pkgPtr->availPtr != NULL)) {
                                Tcl_AppendElement(
                                    interp, Tcl_GetHashKey(tablePtr, hPtr));
                        }
                }
                ...
                version =
                    Tcl_PkgRequire(interp, argv[2 + exact], version, exact);
                if (version == NULL) {
                        return TCL_ERROR;
                }
                ...
                if (argc == 2) {
                        ...
                        if (argv[2][0] == 0) {
                                ...
                                iPtr->packageUnknown = (char *)ckalloc(
                                    (unsigned)(strlen(argv[2]) + 1));
                                strcpy(iPtr->packageUnknown, argv[2]);
                        }
                } else {
                        Tcl_AppendResult(interp, "wrong # args: should be \"",
                                         argv[0], " unknown ?command?\"",
                                         (char *)NULL);
                        return TCL_ERROR;
                }
                ...
                if (hPtr != NULL) {
                   ...
                   (strncmp(argv[1], "vsatisfies", length) == 0) &&
                   (length >= 2)) {
                if (argc != 4) {
                        Tcl_AppendResult(
                            interp, "wrong # args: should be \"", argv[0],
...
void TclFreePackageInfo(
    iPtr) Interp *iPtr; /* Interpereter that is being deleted. */
{
        Package *pkgPtr;
        Tcl_HashSearch search;
        Tcl_HashEntry *hPtr;
...
static int CheckVersion(interp, string)
