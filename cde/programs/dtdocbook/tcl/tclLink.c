                         ...
                         * OR'ed in. */
{
        ...
        code = Tcl_TraceVar(interp, varName,
...
void Tcl_UnlinkVar(interp, varName)
...
int flags;                 /* Miscellaneous additional information. */
{
        ...
        switch (linkPtr->type) {
        case TCL_LINK_INT:
                if (Tcl_GetInt(interp, value, &linkPtr->lastValue.i) !=
                    TCL_OK) {
...
static char *StringValue(
    linkPtr, buffer) Link *linkPtr; /* Structure describing linked variable. */
