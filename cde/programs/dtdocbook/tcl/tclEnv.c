...
static int FindVariable _ANSI_ARGS_((CONST char *name, int *lengthPtr));
void TclSetEnv _ANSI_ARGS_((CONST char *name, CONST char *value));
...
static int FindVariable(name, lengthPtr) CONST
...
char *TclGetEnv(name) char *name; /* Name of desired environment variable. */
{
        int i;
        size_t len;
...
void TclSetEnv(name, value) CONST
...
CONST char *value; /* New value for variable. */
{
        ...
        if (index == -1) {
                if ((length + 2) > environSize) {
                        ...
                        newEnviron = (char **)ckalloc(
                            (unsigned)((length + 5) * sizeof(char *)));
        ...
        *p = '=';
        ...
        for (eiPtr = firstInterpPtr; eiPtr != NULL; eiPtr = eiPtr->nextPtr) {
                (void)Tcl_SetVar2(eiPtr->interp, "env", (char *)name, p + 1,
                                  TCL_GLOBAL_ONLY);
        }
...
void TclUnsetEnv(name) CONST char *name; /* Name of variable to remove. */
{
        int index, dummy;
        char **envPtr;
        EnvInterp *eiPtr;
...
/* ARGSUSED */
static char *EnvTraceProc(clientData, interp, name1, name2,
...
int flags;          /* Indicates what's happening. */
{
                ...
                if (eiPtr->interp == interp) {
                        ...
                        for (prevPtr = eiPtr, eiPtr = eiPtr->nextPtr;;
                             prevPtr = eiPtr, eiPtr = eiPtr->nextPtr) {
                                if (eiPtr == NULL) {
                                        panic("EnvTraceProc couldn't find "
                                              "interpreter");
                                }
                                if (eiPtr->interp == interp) {
        ...
        if (flags & TCL_TRACE_WRITES) {
                TclSetEnv(name2,
                          Tcl_GetVar2(interp, "env", name2, TCL_GLOBAL_ONLY));
        }
...
static void EnvInit() {
        ...
        environSize = length + 5;
        newEnviron = (char **)ckalloc((unsigned)(environSize * sizeof(char *)));
        for (i = 0; i < length; i++) {
                newEnviron[i] =
                    (char *)ckalloc((unsigned)(strlen(environ[i]) + 1));
...
static void EnvExitProc(clientData) ClientData clientData; /* Not  used. */
{
        char **p;
