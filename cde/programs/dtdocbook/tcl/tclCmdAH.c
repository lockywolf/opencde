...
static int StoreStatData _ANSI_ARGS_((Tcl_Interp * interp, char *varName,
                                      struct stat *statPtr));
...
char **argv;        /* Argument strings. */
{
        ...
        if (caseArgc == 1) {
                result =
                    Tcl_SplitList(interp, caseArgv[0], &caseArgc, &caseArgv);
                if (result != TCL_OK) {
                        return result;
                }
                splitArgs = 1;
...
match:
        if (body != -1) {
                ...
                if (result == TCL_ERROR) {
                        char msg[100];
                        sprintf(msg, "\n    (\"%.50s\" arm line %d)",
                                caseArgv[body - 1], interp->errorLine);
...
char **argv;        /* Argument strings. */
{
        ...
        if (argc == 3) {
                if (Tcl_SetVar(interp, argv[2], interp->result, 0) == NULL) {
                        Tcl_SetResult(
                            interp, "couldn't save command result in variable",
                            TCL_STATIC);
                        return TCL_ERROR;
                }
        }
...
char **argv;        /* Argument strings. */
{
        ...
        if (argc == 2) {
                ...
                /*
                 * More than one argument:  concatenate them together with
                 * spaces between, then evaluate the result.
                 */
...
char **argv;        /* Argument strings. */
{
        ...
        if ((c == 'd') && (strncmp(argv[1], "dirname", length) == 0)) {
                ...
                if ((pargc == 1) && (*fileName == '~')) {
                        ckfree((char *)pargv);
                        fileName =
                            Tcl_TranslateFileName(interp, fileName, &buffer);
                        if (fileName == NULL) {
                                result = TCL_ERROR;
                                goto done;
                        }
                ...
                if (pargc > 0) {
                        if ((pargc > 1) ||
                            (Tcl_GetPathType(pargv[0]) == TCL_PATH_RELATIVE)) {
                                Tcl_SetResult(interp, pargv[pargc - 1],
                                              TCL_VOLATILE);
                        }
                }
                ckfree((char *)pargv);
        ...
        checkAccess:
                ...
                goto checkAccess;
                   ...
                   (strncmp(argv[1], "executable", length) == 0) &&
                   (length >= 3)) {
                if (argc != 3) {
                        argv[1] = "executable";
                        goto not3Args;
        ...
        if ((c == 'a') && (strncmp(argv[1], "atime", length) == 0)) {
                ...
                goto done;
                   ...
                   (strncmp(argv[1], "isdirectory", length) == 0) &&
                   (length >= 3)) {
                if (argc != 3) {
                        argv[1] = "isdirectory";
                        goto not3Args;
                ...
                if (argc != 3) {
                        ...
                        /*
                         * If S_IFLNK isn't defined it means that the machine
                         * doesn't support symbolic links, so the file can't
                         * possibly be a symbolic link.  Generate an EINVAL
                         * error, which is what happens on machines that do
                         * support symbolic links when you invoke readlink on a
                         * file that isn't a symbolic link.
                         */
...
#else
                linkLength =
                    readlink(fileName, linkValue, sizeof(linkValue) - 1);
#endif /* S_IFLNK */
                if (linkLength == -1) {
                        Tcl_AppendResult(
                            interp, "couldn't readlink \"", argv[2],
...
#define STATIC_SIZE 4
        ...
        maxj = 0;
        for (i = 0; i < numLists; i++) {
                result = Tcl_SplitList(interp, argv[1 + i * 2], &varcList[i],
                                       &varvList[i]);
                if (result != TCL_OK) {
                        goto errorReturn;
                }
                result = Tcl_SplitList(interp, argv[2 + i * 2], &argcList[i],
                                       &argvList[i]);
                if (result != TCL_OK) {
                        goto errorReturn;
                }
        ...
        for (j = 0; j < maxj; j++) {
                for (i = 0; i < numLists; i++) {
                        for (v = 0; v < varcList[i]; v++) {
                                ...
                                if (Tcl_SetVar(interp, varvList[i][v], value,
                                               0) == NULL) {
                                        Tcl_AppendResult(
                                            interp,
...
#define DOUBLE_VALUE 2
        ...
        for (format = argv[1]; *format != 0;) {
                ...
                *newPtr = '%';
                ...
                if (isdigit(UCHAR(*format))) {
                         ...
                         * there must not be a mixture of XPG3 specs and
        ...
        xpgCheckDone:
                ...
                if (isdigit(UCHAR(*format))) {
                        ...
                        if (Tcl_GetInt(interp, argv[argIndex], &width) !=
                            TCL_OK) {
                                goto fmtError;
                        }
                        argIndex++;
                ...
                if (width > 1000) {
                        /*
                         * Don't allow arbitrarily large widths:  could cause
                         * core dump when we try to allocate a zillion bytes of
                         * memory below.
                         */
                ...
                if (isdigit(UCHAR(*format))) {
                        ...
                        if (Tcl_GetInt(interp, argv[argIndex], &precision) !=
                            TCL_OK) {
                                goto fmtError;
                        }
                        argIndex++;
                ...
                *newPtr = 0;
                ...
                case 'x':
                case 'X':
                        if (Tcl_GetInt(interp, argv[argIndex],
                                       (int *)&intValue) != TCL_OK) {
                                goto fmtError;
                        }
                ...
                case 'g':
                case 'G':
                        if (Tcl_GetDouble(interp, argv[argIndex],
                                          &doubleValue) != TCL_OK) {
                                goto fmtError;
                        }
                ...
                case 0:
                        ...
                        goto fmtError;
                default:
                        sprintf(interp->result, "bad field specifier \"%c\"",
                                *format);
                        goto fmtError;
                }
                argIndex++;
        ...
        doField:
                ...
                if (noPercent) {
                        memcpy((VOID *)(dst + dstSize), (VOID *)ptrValue,
                               (size_t)size);
                        dstSize += size;
                        dst[dstSize] = 0;
                } else {
