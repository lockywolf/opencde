...
static int ExprAbsFunc _ANSI_ARGS_((ClientData clientData, Tcl_Interp *interp,
...
static int ExprDoubleFunc _ANSI_ARGS_((ClientData clientData,
                                       Tcl_Interp *interp, Tcl_Value *args,
                                       Tcl_Value *resultPtr));
static int ExprGetValue _ANSI_ARGS_((Tcl_Interp * interp, ExprInfo *infoPtr,
                                     int prec, Value *valuePtr));
static int ExprIntFunc _ANSI_ARGS_((ClientData clientData, Tcl_Interp *interp,
                                    Tcl_Value *args, Tcl_Value *resultPtr));
static int ExprLex _ANSI_ARGS_((Tcl_Interp * interp, ExprInfo *infoPtr,
                                Value *valuePtr));
static int ExprLooksLikeInt _ANSI_ARGS_((char *p));
static void ExprMakeString _ANSI_ARGS_((Tcl_Interp * interp, Value *valuePtr));
static int ExprMathFunc _ANSI_ARGS_((Tcl_Interp * interp, ExprInfo *infoPtr,
                                     Value *valuePtr));
static int ExprParseString _ANSI_ARGS_((Tcl_Interp * interp, char *string,
                                        Value *valuePtr));
static int ExprRoundFunc _ANSI_ARGS_((ClientData clientData, Tcl_Interp *interp,
                                      Tcl_Value *args, Tcl_Value *resultPtr));
static int ExprTopLevel _ANSI_ARGS_((Tcl_Interp * interp, char *string,
                                     Value *valuePtr));
static int ExprUnaryFunc _ANSI_ARGS_((ClientData clientData, Tcl_Interp *interp,
                                      Tcl_Value *args, Tcl_Value *resultPtr));
                         ...
                         * Caller must have initialized pv field. */
{
        ...
        if (*string != 0) {
                if (ExprLooksLikeInt(string)) {
                        ...
                        /*
                         * Note: use strtoul instead of strtol for integer
                         * conversions to allow full-size unsigned numbers, but
                         * don't depend on strtoul to handle sign characters;
                         * it won't in some implementations.
                         */
                        ...
                        if (*term == 0) {
                                if (errno == ERANGE) {
                                         ...
                                         * with string in interp->result, so we
                                         * have to clear the result before
                                         * logging an error message.
                                         */

                                        Tcl_ResetResult(interp);
                                        interp->result = "integer value too "
                                                         "large to represent";
                                        Tcl_SetErrorCode(
                                            interp, "ARITH", "IOVERFLOW",
...
static int ExprLex(interp, infoPtr,
                             ...
                             * initialized pv field. */
{
        ...
        if (infoPtr->token == OPEN_PAREN) {
                ...
                if (infoPtr->token != CLOSE_PAREN) {
                        Tcl_AppendResult(
                            interp, "unmatched parentheses in expression \"",
                ...
                if (infoPtr->token >= UNARY_MINUS) {
                        ...
                        result =
                            ExprGetValue(interp, infoPtr,
                                         precTable[infoPtr->token], valuePtr);
                        if (result != TCL_OK) {
                                goto done;
                        }
                        if (!iPtr->noEval) {
                                ...
                                case NOT:
                                        if (valuePtr->type == TYPE_INT) {
                                                valuePtr->intValue =
                                                   ...
                                                   TYPE_DOUBLE) {
                                                /*
                                                 * Theoretically, should be able
                                                 * to use
                                                 * "!valuePtr->intValue", but
                                                 * apparently some compilers
                                                 * can't handle it.
                                                 */
                                                if (valuePtr->doubleValue ==
                                                    0.0) {
                                                        valuePtr->intValue = 1;
                                                } else {
                                                        valuePtr->intValue = 0;
        ...
        if ((operator== AND) || (operator== OR) || (operator== QUESTY)) {
			...
			&value2);
                ...
                continue;
                         ...
                         * left.  To make this happen, use a precedence one
                         * lower than QUESTY when calling ExprGetValue
                         * recursively.
                         */
...
static void ExprMakeString(interp, valuePtr)
...
register Value *valuePtr; /* Value to be converted. */
{
        ...
        if (valuePtr->type == TYPE_INT) {
                ...
                Tcl_PrintDouble(interp, valuePtr->doubleValue,
                                valuePtr->pv.buffer);
        }
        valuePtr->type = TYPE_STRING;
}
        ...
        if (!(((Interp *)interp)->flags & EXPR_INITIALIZED)) {
                ...
                for (funcPtr = funcTable; funcPtr->name != NULL; funcPtr++) {
                        Tcl_CreateMathFunc(interp, funcPtr->name,
                                           ...
                                           funcPtr->proc, funcPtr->clientData);
                }
        }
...
int Tcl_ExprLong(interp, string,
                 ptr) Tcl_Interp *interp; /* Context in which to evaluate the
                                           * expression. */
char *string;                             /* Expression to evaluate. */
...
int Tcl_ExprDouble(interp, string,
                   ptr) Tcl_Interp *interp; /* Context in which to evaluate the
                                             * expression. */
char *string;                               /* Expression to evaluate. */
...
int Tcl_ExprBoolean(interp, string,
                    ptr) Tcl_Interp *interp; /* Context in which to evaluate the
                                              * expression. */
char *string;                                /* Expression to evaluate. */
...
char *string;           /* Expression to evaluate. */
{
        ...
        if (result == TCL_OK) {
                if (value.type == TYPE_INT) {
                        ...
                        Tcl_PrintDouble(interp, value.doubleValue,
                                        interp->result);
                } else {
                        if (value.pv.buffer != value.staticSpace) {
                                ...
                                Tcl_SetResult(interp, value.pv.buffer,
                                              TCL_VOLATILE);
                        }
                }
        }
        ...
        *p = savedChar;
        ...
        if (mathFuncPtr->numArgs == 0) {
                ...
                for (i = 0;; i++) {
                        ...
                        if (valuePtr->type == TYPE_STRING) {
                                interp->result = "argument to math function "
                                                 "didn't have numeric value";
                                return TCL_ERROR;
                        }

                        /*
                         * Copy the value to the argument record, converting it
                         * if necessary.
                         */
...
void TclExprFloatError(interp, value)
    Tcl_Interp *interp; /* Where to store error message. */
...
/* ARGSUSED */
static int ExprAbsFunc(clientData, interp, args,
                       resultPtr) ClientData clientData;
Tcl_Interp *interp;
Tcl_Value *args;
Tcl_Value *resultPtr;
{
...
/* ARGSUSED */
static int ExprDoubleFunc(clientData, interp, args,
                          resultPtr) ClientData clientData;
Tcl_Interp *interp;
Tcl_Value *args;
Tcl_Value *resultPtr;
...
/* ARGSUSED */
static int ExprIntFunc(clientData, interp, args,
                       resultPtr) ClientData clientData;
Tcl_Interp *interp;
Tcl_Value *args;
Tcl_Value *resultPtr;
...
/* ARGSUSED */
static int ExprRoundFunc(clientData, interp, args,
                         resultPtr) ClientData clientData;
Tcl_Interp *interp;
Tcl_Value *args;
Tcl_Value *resultPtr;
