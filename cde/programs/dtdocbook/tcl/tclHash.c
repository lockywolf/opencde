...
void Tcl_InitHashTable(
    tablePtr,
    keyType) register Tcl_HashTable *tablePtr; /* Pointer to table record, which
...
void Tcl_DeleteHashEntry(entryPtr) Tcl_HashEntry *entryPtr;
{
        ...
        if (*entryPtr->bucketPtr == entryPtr) {
                *entryPtr->bucketPtr = entryPtr->nextPtr;
                ...
                for (prevPtr = *entryPtr->bucketPtr;;
                     prevPtr = prevPtr->nextPtr) {
                        if (prevPtr == NULL) {
                                panic("malformed bucket chain in "
                                      "Tcl_DeleteHashEntry");
                        }
...
void Tcl_DeleteHashTable(
    tablePtr) register Tcl_HashTable *tablePtr; /* Table to delete. */
{
...
Tcl_HashEntry *Tcl_NextHashEntry(searchPtr) register Tcl_HashSearch
...
static unsigned int HashString(string) register char
    *string; /* String from which to compute hash value. */
{
...
static void RebuildTable(
    tablePtr) register Tcl_HashTable *tablePtr; /* Table to enlarge. */
{
                ...
                for (hPtr = *oldChainPtr; hPtr != NULL; hPtr = *oldChainPtr) {
                        *oldChainPtr = hPtr->nextPtr;
                        if (tablePtr->keyType == TCL_STRING_KEYS) {
                                index = HashString(hPtr->key.string) &
                                        tablePtr->mask;
                        } else if (tablePtr->keyType == TCL_ONE_WORD_KEYS) {
                                index = RANDOM_INDEX(tablePtr,
                                                     hPtr->key.oneWordValue);
                        } else {
                                register int *iPtr;
                                int count;
