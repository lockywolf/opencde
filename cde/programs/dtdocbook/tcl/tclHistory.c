...
static char *GetWords _ANSI_ARGS_((Interp * iPtr, char *command, char *words));
static void InitHistory _ANSI_ARGS_((Interp * iPtr));
static void InsertRev _ANSI_ARGS_((Interp * iPtr, HistoryRev *revPtr));
...
static int SubsAndEval _ANSI_ARGS_((Interp * iPtr, char *cmd, char *old,
                                    char *new));
...
static void
    InitHistory(iPtr) register Interp *iPtr; /* Interpreter to initialize. */
{
        int i;
        ...
        iPtr->numEvents = 20;
        iPtr->events = (HistoryEvent *)ckalloc(
                         ...
                         * instead of Tcl_Eval. */
{
        ...
        if (!(flags & TCL_NO_EVAL)) {
                ...
                iPtr->revDisables = 0;
                iPtr->evalFlags =
                    (flags & ~TCL_EVAL_GLOBAL) | TCL_RECORD_BOUNDS;
...
/* ARGSUSED */
int Tcl_HistoryCmd(dummy, interp, argc, argv) ClientData dummy; /* Not used. */
        ...
        infoCmd:
                ...
                for (i = 0, indx = iPtr->curEvent + 1 + iPtr->numEvents - count;
                     i < count; i++, indx++) {
                        ...
                        sprintf(serial, "%6d  ",
                                iPtr->curEventNum + 1 - (count - i));
                        Tcl_AppendResult(interp, newline, serial, (char *)NULL);
                        newline = "\n";

                        /*
                         * Tricky formatting here:  for multi-line commands,
                ...
                if ((count <= 0) || (count > 1000)) {
                        Tcl_AppendResult(interp, "illegal keep count \"",
                                         argv[2], "\"", (char *)NULL);
                        return TCL_ERROR;
                }
                ...
                events = (HistoryEvent *)ckalloc(
                    (unsigned)(count * sizeof(HistoryEvent)));
                if (count < iPtr->numEvents) {
                ...
                return Tcl_Eval(interp, eventPtr->command);
                   ...
                   (strncmp(argv[1], "substitute", length)) == 0) {
                if ((argc > 5) || (argc < 4)) {
                        Tcl_AppendResult(
...
static void MakeSpace(hPtr, size) HistoryEvent *hPtr;
...
static void InsertRev(iPtr, revPtr) Interp *iPtr; /* Interpreter to use. */
...
static void RevCommand(
    iPtr, string) register Interp *iPtr; /* Interpreter in which to perform the
                                          * substitution. */
...
static void
    DoRevs(iPtr) register Interp *iPtr; /* Interpreter whose history is to
                                         * be modified. */
{
        register HistoryRev *revPtr;
        register HistoryEvent *eventPtr;
        ...
        for (revPtr = iPtr->revPtr; revPtr != NULL;) {
                ...
                if (count > 0) {
                        strncpy(p, eventPtr->command + bytesSeen,
                                (size_t)count);
                        p += count;
                }
...
static HistoryEvent *
    GetEvent(iPtr,
             string) register Interp *iPtr; /* Interpreter in which to look. */
char *string;                               /* Description of event. */
{
        ...
        if (isdigit(UCHAR(*string)) || (*string == '-')) {
                if (Tcl_GetInt((Tcl_Interp *)iPtr, string, &eventNum) !=
                    TCL_OK) {
                        return NULL;
                }
                if (eventNum < 0) {
...
static int
    SubsAndEval(iPtr, cmd, old,
                new) register Interp *iPtr; /* Interpreter in which to execute
                                             * new command. */
...
static char *
    GetWords(iPtr, command,
             words) register Interp *iPtr; /* Tcl interpreter in which to place
        ...
        for (index = 0; *next != 0; index++) {
                ...
                if (*end != 0) {
                        ...
                        for (next = end; isspace(UCHAR(*next)); next++) {
                                /* Empty loop body:  just find start of next
                                 * word. */
                        }
                }
