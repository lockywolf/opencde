...
static Tcl_File FileForRedirect _ANSI_ARGS_((Tcl_Interp * interp, char *spec,
                                             int atOk, char *arg, int flags,
                                             char *nextArg, int *skipPtr,
                                             int *closePtr));
...
static Tcl_File
    FileForRedirect(interp, spec, atOk, arg, flags, nextArg, skipPtr,
                    closePtr) Tcl_Interp *interp; /* Intepreter to use for error
                                                   * reporting. */
                      ...
                      * it must be left open. */
{
        ...
        if (atOk && (*spec == '@')) {
                ...
                *closePtr = 0;
                file = Tcl_GetChannelFile(chan, writing ? TCL_WRITABLE
                                                        : TCL_READABLE);
                if (file == NULL) {
                        Tcl_AppendResult(
                            interp, "channel \"", Tcl_GetChannelName(chan),
                            "\" wasn't opened for ",
                ...
                if (writing) {
                         ...
                         * written by the child appears after stuff we've
                         * already written.
                         */
                ...
                if (file == NULL) {
                        Tcl_AppendResult(
                            interp, "couldn't ", (writing) ? "write" : "read",
...
#define RW_MODES (O_RDONLY | O_WRONLY | O_RDWR)
        ...
        *seekFlagPtr = 0;
        ...
        for (i = 0; i < modeArgc; i++) {
                ...
                if ((c == 'R') && (strcmp(flag, "RDONLY") == 0)) {
                        ...
                        *seekFlagPtr = 1;
                        ...
                        if (interp != (Tcl_Interp *)NULL) {
                                Tcl_AppendResult(
                                    interp, "invalid access mode \"", flag,
        ...
        if (!gotRW) {
                if (interp != (Tcl_Interp *)NULL) {
                        Tcl_AppendResult(
                            interp, "access mode must include either",
...
void
    Tcl_DetachPids(numPids,
                   pidPtr) int numPids; /* Number of pids to detach:  gives size
...
int TclCleanupChildren(interp, numPids, pidPtr, errorChan)
        ...
        for (i = 0; i < numPids; i++) {
                ...
                if (pid == (pid_t)-1) {
                        ...
                        if (interp != (Tcl_Interp *)NULL) {
                                ...
                                if (errno == ECHILD) {
                                        /*
                                         * This changeup in message suggested by
                                         * Mark Diekhans to remind people that
                                         * ECHILD errors can occur on some
                                         * systems if SIGCHLD isn't in its
                                         * default state.
                                         */

                                        msg = "child process lost (is SIGCHLD "
                                              "ignored or trapped?)";
                                }
                                Tcl_AppendResult(
                                    interp,
                                    "error waiting for process to exit: ", msg,
                                    (char *)NULL);
                        }
                        continue;
                }
                ...
                if (!WIFEXITED(waitStatus) || (WEXITSTATUS(waitStatus) != 0)) {
                        ...
                        if (WIFEXITED(waitStatus)) {
                                if (interp != (Tcl_Interp *)NULL) {
                                        ...
                                        Tcl_SetErrorCode(interp, "CHILDSTATUS",
                                                         msg1, msg2,
                                                         (char *)NULL);
                                }
                                abnormalExit = 1;
                                ...
                                if (interp != (Tcl_Interp *)NULL) {
                                        ...
                                        p = Tcl_SignalMsg(
                                            (int)(WTERMSIG(waitStatus)));
                                        Tcl_SetErrorCode(
                                            interp, "CHILDKILLED", msg1,
                                            Tcl_SignalId(
                                                (int)(WTERMSIG(waitStatus))),
                                            p, (char *)NULL);
                                        Tcl_AppendResult(interp,
                                ...
                                if (interp != (Tcl_Interp *)NULL) {
                                        ...
                                        p = Tcl_SignalMsg(
                                            (int)(WSTOPSIG(waitStatus)));
                                        Tcl_SetErrorCode(
                                            interp, "CHILDSUSP", msg1,
                                            Tcl_SignalId(
                                                (int)(WSTOPSIG(waitStatus))),
                                            p, (char *)NULL);
                                        Tcl_AppendResult(interp,
...
#define BUFFER_SIZE 1000
                                ...
                                count =
                                    Tcl_Read(errorChan, buffer, BUFFER_SIZE);
                                if (count == 0) {
                                        break;
                                }
                                result = TCL_ERROR;
...
int TclCreatePipeline(interp, argc, argv, pidArrayPtr, inPipePtr, outPipePtr,
                      errFilePtr)
                         ...
                         * but it will never get any data. */
{
#if defined(MAC_TCL)
        Tcl_AppendResult(
            interp, "command pipelines not supported on Macintosh OS", NULL);
        return -1;
#else  /* !MAC_TCL */
                    ...
                    (((argv[i][1] == 0)) ||
                     ((argv[i][1] == '&') && (argv[i][2] == 0)))) {
                        ...
                        if (argv[i][1] == '<') {
                                ...
                                input = 0;
                                inputFile = FileForRedirect(
                                    interp, argv[i] + 1, 1, argv[i], O_RDONLY,
                                    argv[i + 1], &skip, &closeInput);
                                if (inputFile == NULL) {
                                        goto error;
                                }

                                /* When Win32s dies out, this code can be
                                 * removed */
                                if (!hasPipes) {
                                        if (!closeInput) {
                                                Tcl_AppendResult(
                                        ...
                                        strcpy(intIn, skip == 1 ? argv[i] + 1
                                                                : argv[i + 1]);
                                }
                        }
                } else if (argv[i][0] == '>') {
                        int append, useForStdErr, useForStdOut, mustClose, atOk,
                            flags;
                        Tcl_File file;
                        ...
                        file = FileForRedirect(interp, p, atOk, argv[i], flags,
                                               argv[i + 1], &skip, &mustClose);
                        if (file == NULL) {
                                goto error;
                        }
                        ...
                        if (!hasPipes) {
                                if (!mustClose) {
                                        Tcl_AppendResult(
                                            interp, "redirection with '@'",
                                            " notation is not supported on "
                                            "this system",
                                            (char *)NULL);
                                        goto error;
                                }
                        ...
                        /*
                         * Got the file descriptor.  Now use it for standard
                         * output, standard error, or both, depending on the
                         * redirection.
                         */
                        ...
                        errorFile =
                            FileForRedirect(interp, p, atOk, argv[i], flags,
        ...
        if ((hasPipes && inputFile == NULL) ||
            (!hasPipes && intIn[0] == '\0')) {
                if (input != NULL) {
                        ...
                        if (!hasPipes ||
                            TclCreatePipe(&inPipe, &outPipe) == 0) {
                                Tcl_AppendResult(
                                    interp,
        ...
        if (errFilePtr && !errorFile) {
                *errFilePtr = TclCreateTempFile(NULL);
                if (*errFilePtr == NULL) {
                        Tcl_AppendResult(
                            interp, "couldn't create error file for command: ",
                            Tcl_PosixError(interp), (char *)NULL);
                        goto error;
                }
        ...
        if (TclSpawnPipeline(interp, pidPtr, &numPids, argc, argv, inputFile,
                             outputFile, errorFile, intIn, finalOut) == 0) {
                goto error;
        }
        *pidArrayPtr = pidPtr;
                         ...
                         * TCL_STDERR, and TCL_ENFORCE_MODE. */
{
        ...
        if (flags & TCL_ENFORCE_MODE) {
                if ((flags & TCL_STDOUT) && (outPipe == NULL)) {
                        Tcl_AppendResult(
                            interp, "can't read output from command:",
                            " standard output was redirected", (char *)NULL);
                        goto error;
                }
                if ((flags & TCL_STDIN) && (inPipe == NULL)) {
                        Tcl_AppendResult(
                            interp, "can't write input to command:",
