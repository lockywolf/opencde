...
char **argv;        /* Argument strings. */
{
        ...
        while (1) {
                 ...
                 * expression to test, either for the main expression or an
                 * expression following an "elseif".  The arguments after the
                 * expression must be "then" (optional) and a script to execute
                 * if the expression is true.
                 */

                if (i >= argc) {
                        Tcl_AppendResult(
                            interp, "wrong # args: no expression after \"",
                ...
                if (i >= argc) {
                        Tcl_AppendResult(
                            interp, "wrong # args: no script following \"",
                            argv[i - 1], "\" argument", (char *)NULL);
                        return TCL_ERROR;
                }
                if (value) {
...
/* ARGSUSED */
int Tcl_InfoCmd(dummy, interp, argc, argv) ClientData dummy; /* Not used. */
Tcl_Interp *interp; /* Current interpreter. */
int argc;           /* Number of arguments. */
char **argv;        /* Argument strings. */
{
        ...
        if ((c == 'a') && (strncmp(argv[1], "args", length)) == 0) {
                ...
                if (argc != 5) {
                        Tcl_AppendResult(
                            ...
                            " default procname arg varname\"", (char *)NULL);
                        return TCL_ERROR;
                }
                procPtr = TclFindProc(iPtr, argv[2]);
                if (procPtr == NULL) {
                ...
                for (argPtr = procPtr->argPtr;; argPtr = argPtr->nextPtr) {
                        ...
                        if (strcmp(argv[3], argPtr->name) == 0) {
                                if (argPtr->defValue != NULL) {
                                        if (Tcl_SetVar(
                                                (Tcl_Interp *)iPtr, argv[4],
                                                argPtr->defValue, 0) == NULL) {
                                        defStoreError:
                                                Tcl_AppendResult(
                                                    interp,
                                                    "couldn't store default "
                                                    "value in variable \"",
                                                    argv[4], "\"",
                                                    (char *)NULL);
                                                return TCL_ERROR;
                ...
                if (p == NULL) {
                        ...
                        if (iPtr->varFramePtr == NULL) {
                                hPtr = Tcl_FindHashEntry(&iPtr->globalTable,
                                                         argv[2]);
                        } else {
                                hPtr = Tcl_FindHashEntry(
                                    &iPtr->varFramePtr->varTable, argv[2]);
                        }
                        if (hPtr == NULL) {
                                goto noVar;
                        }
                ...
                if (argc == 2) {
                        ...
                        iPtr->result =
                            Tcl_Merge(framePtr->argc, framePtr->argv);
                ...
                for (hPtr = Tcl_FirstHashEntry(&iPtr->varFramePtr->varTable,
                                               &search);
                     hPtr != NULL; hPtr = Tcl_NextHashEntry(&search)) {
                        varPtr = (Var *)Tcl_GetHashValue(hPtr);
                        ...
                        name =
                            Tcl_GetHashKey(&iPtr->varFramePtr->varTable, hPtr);
                        if ((argc == 3) && !Tcl_StringMatch(name, argv[2])) {
                                continue;
                        }
                        Tcl_AppendElement(interp, name);
                ...
                return TCL_OK;
                   ...
                   (strncmp(argv[1], "patchlevel", length) == 0) &&
                   (length >= 2)) {
                char *value;
                ...
                if (iPtr->scriptFile != NULL) {
                         ...
                         * if this command is returned as the result of the
                         * script, then iPtr->scriptFile will go away.
                         */
                ...
                return TCL_OK;
                   ...
                   (strncmp(argv[1], "tclversion", length) == 0)) {
                char *value;
                ...
                Tcl_AppendResult(
                    ...
                    "sharedlibextension, tclversion, or vars", (char *)NULL);
                return TCL_ERROR;
        }
}
...
char **argv;        /* Argument strings. */
{
        ...
        for (i = 0; i < listArgc; i++) {
                if (i == 0) {
                        ...
                        Tcl_AppendResult(interp, joinString, listArgv[i],
                                         (char *)NULL);
                }
        }
...
/* ARGSUSED */
int Tcl_LrangeCmd(notUsed, interp, argc,
...
char **argv;                                /* Argument strings. */
{
        ...
        if ((*argv[3] == 'e') &&
            (strncmp(argv[3], "end", strlen(argv[3])) == 0)) {
                ...
                if (Tcl_GetInt(interp, argv[3], &last) != TCL_OK) {
                        ...
                        Tcl_AppendResult(
                            interp, "expected integer or \"end\" but got \"",
                            argv[3], "\"", (char *)NULL);
                        return TCL_ERROR;
                }
        }
        ...
        for (count = 0, begin = argv[1]; count < first; begin = next, count++) {
                result = TclFindElement(interp, begin, &dummy, &next,
                                        (int *)NULL, (int *)NULL);
                if (result != TCL_OK) {
                        return result;
                }
...
/* ARGSUSED */
int Tcl_LreplaceCmd(notUsed, interp, argc,
                    argv) ClientData notUsed; /* Not used. */
Tcl_Interp *interp;                           /* Current interpreter. */
int argc;                                     /* Number of arguments. */
char **argv;                                  /* Argument strings. */
...
/* ARGSUSED */
int Tcl_LsearchCmd(notUsed, interp, argc,
                   argv) ClientData notUsed; /* Not used. */
Tcl_Interp *interp;                          /* Current interpreter. */
int argc;                                    /* Number of arguments. */
char **argv;                                 /* Argument strings. */
...
#define REGEXP 2
        ...
        for (i = 0; i < listArgc; i++) {
                ...
                case GLOB:
                        ...
                        break;
                case REGEXP:
                        match = Tcl_RegExpMatch(interp, listArgv[i],
                                                argv[argc - 1]);
                        if (match < 0) {
                                ckfree((char *)listArgv);
                                return TCL_ERROR;
...
/* ARGSUSED */
int Tcl_LsortCmd(notUsed, interp, argc,
                 argv) ClientData notUsed; /* Not used. */
Tcl_Interp *interp;                        /* Current interpreter. */
int argc;                                  /* Number of arguments. */
char **argv;                               /* Argument strings. */
    ...
    *second; /* Elements to be compared. */
{
        ...
        if (sortMode == ASCII) {
                ...
                /*
                 * Generate and evaluate a command to determine which string
                 * comes first.
                 */
