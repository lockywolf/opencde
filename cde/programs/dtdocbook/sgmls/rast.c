 ...
 */
/* $XConsortium: rast.c /main/3 1996/06/19 17:16:53 drk $ */
/* rast.c
...
char **argv;
{
        ...
        if (output_file) {
                ...
                if (!outfp)
                        error("couldn't open `%s' for output: %s",
                              strerror(errno));
        } else {
                outfp = tmpfile();
                if (!outfp)
        ...
        if (argc - optind == 1) {
                if (!freopen(argv[optind], "r", stdin))
                        error("couldn't open `%s' for input: %s", argv[optind],
                              strerror(errno));
        }
        ...
        if (!do_file(stdin)) {
                ...
                if (output_file) {
                        if (!freopen(output_file, "w", stdout))
                                error("couldn't reopen `%s' for output: %s",
                                      strerror(errno));
                }
                fputs("#ERROR\n", stdout);
                exit(EXIT_FAILURE);
        ...
        if (output_file) {
                errno = 0;
                if (fclose(outfp) == EOF)
                        error("error closing `%s': %s", output_file,
                              strerror(errno));
        } else {
                errno = 0;
                if (fseek(outfp, 0L, SEEK_SET))
                        error("couldn't rewind temporary file: %s",
                              strerror(errno));
                while ((c = getc(outfp)) != EOF)
                        if (putchar(c) == EOF)
                                error("error writing standard output: %s",
                                      strerror(errno));
        }
        exit(EXIT_SUCCESS);
}
...
static int do_file(fp) FILE *fp;
{
        ...
        while (sgmls_next(sp, &e))
                ...
                case SGMLS_EVENT_START:
                        output_element_start(e.u.start.gi,
...
static void output_processing_instruction(s, len) char *s;
unsigned len;
{
        fputs("[?", outfp);
        if (len > 0) {
...
static void output_data(v, n) struct sgmls_data *v;
int n;
{
        int i;
        for (i = 0; i < n; i++) {
...
static void output_data_lines(s, n) char *s;
unsigned n;
{
        assert(n > 0);
        for (; n > 0; --n)
...
static void output_internal_sdata(s, n) char *s;
unsigned n;
{
        fputs("#SDATA-TEXT\n", outfp);
...
static void output_external_entity(e) struct sgmls_external_entity *e;
{
        fprintf(outfp, "[&%s\n", e->name);
...
static void output_element_start(gi, att) char *gi;
...
static void output_element_end(gi) char *gi;
{ fprintf(outfp, "[/%s]\n", gi); }

static void output_attribute(p) struct sgmls_attribute *p;
{
        fprintf(outfp, "%s=\n", p->name);
        switch (p->type) {
        ...
        case SGMLS_ATTR_NOTATION:
                ...
                output_external_id(p->value.notation->pubid,
                                   p->value.notation->sysid);
                break;
        }
}

static void output_tokens(v, n) char **v;
int n;
{
        int i;
        assert(n > 0);
...
static void output_markup_chars(s, n) char *s;
unsigned n;
{
        for (; n > 0; --n)
                output_char((unsigned char)*s++, '!');
...
static void output_markup_string(s) char *s;
{
        while (*s)
                output_char((unsigned char)*s++, '!');
}

static void output_char(c, delim) int c;
int delim;
{
...
static void output_flush(delim) int delim;
{
        if (char_count > 0) {
                putc(delim, outfp);
                putc('\n', outfp);
...
static void output_external_id(pubid, sysid) char *pubid;
char *sysid;
{
        if (!pubid && !sysid)
                fputs("#SYSTEM\n#NONE\n", outfp);
...
static void output_entity(e) struct sgmls_entity *e;
{
        if (e->is_internal)
...
static void output_external_entity_info(e) struct sgmls_external_entity *e;
{
        switch (e->type) {
...
static void output_internal_entity(e) struct sgmls_internal_entity *e;
{
        if (e->data.is_sdata)
...
static void input_error(num, str, lineno) int num;
char *str;
