...
nl_catd catopen(name, oflag) char *name;
int oflag;
{
        struct cat *catp;
        int i;
...
char *catgets(catd, setnum, msgnum, dflt) nl_catd catd;
int setnum, msgnum;
char *dflt;
{
        struct message *p;
...
static VOID load_catalog(catp) struct cat *catp;
{
        FILE *fp;
        char *path;
...
static FILE *find_catalog(name, pathp) char *name;
char **pathp;
{
        char *path;
...
static int parse_message(c, fp, table, setnum, quote) int c;
FILE *fp;
...
static char *parse_text(fp, quote) FILE *fp;
int quote;
{
        unsigned i = 0;
        char *p;
...
static int parse_command(fp, table, setnump, quotep) FILE *fp;
struct message **table;
unsigned *setnump;
int *quotep;
{
...
static VOID delete_set(table, setnum) struct message **table;
unsigned setnum;
{
        int i;
...
static VOID delete_message(table, setnum, msgnum) struct message **table;
...
static int read_catalog(fp, table) FILE *fp;
struct message **table;
{
        int c;
...
static int hash(setnum, msgnum) unsigned setnum, msgnum;
...
char **argv;
{
        ...
        for (i = 1; i < argc; i++) {
                ...
                if (!fp) {
                        ...
                        if (read_catalog(fp, table) < 0) {
                                ...
                                assert(cat_errno < sizeof(cat_errlist) /
                                                       sizeof(cat_errlist[0]));
        ...
        fp = fopen(argv[1], "w");
        if (!fp)
                fatal("can't open `%s' for output: %s", argv[1],
                      strerror(errno));
        nmessages = 0;
        for (i = 0; i < HASH_TAB_SIZE; i++) {
                struct message *p;
...
static int message_compare(p1, p2) UNIV p1, UNIV p2;
{
...
static VOID print_text(s, fp) char *s;
FILE *fp;
{
        for (; *s; s++) {
                if (*s == '\\')
