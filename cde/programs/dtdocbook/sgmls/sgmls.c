 ...
 */
/* $XConsortium: sgmls.c /main/3 1996/06/19 17:18:02 drk $ */
/* sgmls.c:
...
static void parse_external_entity P((char *, struct sgmls *,
                                     external_entity_s *));
...
struct sgmls *sgmls_create(fp) FILE *fp;
{
        struct sgmls *sp;
...
void sgmls_free(sp) struct sgmls *sp;
{
...
sgmls_errhandler *sgmls_set_errhandler(handler) sgmls_errhandler *handler;
{
...
int sgmls_next(sp, e) struct sgmls *sp;
event_s *e;
{
        while (read_line(sp)) {
                char *buf = sp->buf;
                ...
                case END_SUBDOC_CODE: {
                        ...
                        if (buf[0] == START_SUBDOC_CODE) {
                                ...
                                list_finish_level((struct list **)&sp->entities,
                                                  sp->subdoc_level);
                                list_finish_level(
                                    (struct list **)&sp->notations,
                                    sp->subdoc_level);
                                sp->subdoc_level--;
                                e->u.entity = lookup_external_entity(sp, name);
                        }
                ...
                case ATTRIBUTE_CODE:
                        add_attribute(&sp->attributes,
                                      parse_attribute(sp, buf + 1));
                        break;
                ...
                case PUBID_CODE:
                        ...
                        break;
                case FILE_CODE:
                        sp->files = xrealloc(sp->files,
                                             (sp->nfiles + 1) * sizeof(char *));
...
static int parse_data(p, linenop) char *p;
...
static void parse_notation(s, n) char *s;
notation_s *n;
{ n->name = strsave(scan_token(&s)); }

static void parse_internal_entity(s, e) char *s;
internal_entity_s *e;
{
        char *type;
...
static void parse_external_entity(s, sp, e) char *s;
struct sgmls *sp;
external_entity_s *e;
{
        char *type;
...
static void parse_subdoc_entity(s, e) char *s;
external_entity_s *e;
{
        e->name = strsave(scan_token(&s));
...
static attribute_s *parse_attribute(sp, s) struct sgmls *sp;
char *s;
{
        attribute_s *a;
        char *type;
        ...
        if (strcmp(type, "CDATA") == 0) {
                ...
                if (n == 0)
                        error(E_MISSING);
                a->value.entity.v =
                    (entity_s **)xmalloc(n * sizeof(entity_s *));
                a->value.entity.n = n;
                for (i = 0; i < n; i++)
                        a->value.entity.v[i] =
                            lookup_entity(sp, scan_token(&s));
...
void sgmls_free_attributes(p) attribute_s *p;
{
        while (p) {
...
static data_s *copy_data(v, n) data_s *v;
int n;
{
        if (n == 0)
                return 0;
...
static char *unescape_file(s) char *s;
{
        char *p;
        p = s = unescape(s);
...
static char *scan_token(pp) char **pp;
{
        char *start = *pp;
        while (**pp != '\0') {
                if (**pp == ' ') {
...
static int count_args(p) char *p;
{
        int n = 0;
...
static int read_line(sp) struct sgmls *sp;
{
        unsigned i = 0;
        FILE *fp = sp->fp;
        int c;
...
static notation_s *lookup_notation(sp, name) struct sgmls *sp;
char *name;
{
        struct notation_list *p = (struct notation_list *)list_find(
            (struct list *)sp->notations, name, sp->subdoc_level);
        if (!p)
                error(E_BADNOTATION);
        return &p->notation;
}

static entity_s *lookup_entity(sp, name) struct sgmls *sp;
char *name;
{
        struct entity_list *p = (struct entity_list *)list_find(
            (struct list *)sp->entities, name, sp->subdoc_level);
        if (!p)
                error(E_BADENTITY);
        return &p->entity;
}

static external_entity_s *lookup_external_entity(sp, name) struct sgmls *sp;
char *name;
{
...
static void define_external_entity(sp, e) struct sgmls *sp;
external_entity_s *e;
{
...
static void define_internal_entity(sp, e) struct sgmls *sp;
internal_entity_s *e;
{
        struct entity_list *p;
        p = (struct entity_list *)xmalloc(sizeof(struct entity_list));
...
static void define_notation(sp, np) struct sgmls *sp;
notation_s *np;
{
...
static struct list *list_find(p, name, level) struct list *p;
char *name;
int level;
{
...
static void list_finish_level(listp, level) struct list **listp;
int level;
{
...
static void add_attribute(pp, a) attribute_s **pp, *a;
{
...
static UNIV xmalloc(n) unsigned n;
{
        UNIV p = malloc(n);
        if (!p)
                error(E_NOMEM);
...
static UNIV xrealloc(p, n) UNIV p;
unsigned n;
{
...
static NO_RETURN void error(num) enum error_code num;
{
        (*errhandler)((int)num, errlist[num], input_lineno);
        abort();
}

static void default_errhandler(num, msg, lineno) int num;
char *msg;
unsigned long lineno;
{
