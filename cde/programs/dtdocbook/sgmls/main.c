...
char **argv;
{
        ...
        /* Define MAIN_HOOK in config.h if some function needs to be called
         * here. */
#ifdef MAIN_HOOK
        MAIN_HOOK(argc, argv);
#endif
        ...
        while ((opt = getopt(argc, argv, options)) != EOF) {
                ...
                case 'i': /* Define parameter entity as "INCLUDE". */
                        sw.includes =
                            (char **)xrealloc((UNIV)sw.includes,
...
static char *munge_program_name(arg, dflt) char *arg, *dflt;
{
        char *p;
...
static UNIV make_docent(argc, argv) int argc;
char **argv;
{
        UNS len = 1;
        int i;
...
static VOID usage() {
        ...
        fprintf(
            stderr,
            "Usage: %s [-deglprsuv]%s [-c file] [-i entity]%s [filename ...]\n",
            prog,
#ifdef CANT_REDIRECT_STDERR
            " [-f file]",
...
static VOID swinit(swp) struct switches *swp;
{
        swp->swenttr = 0;
        swp->sweltr = 0;
...
static VOID build_subargv(swp) struct switches *swp;
{
        if (suppsw)
                subargv[subargc++] = "-s";
        if (locsw)
...
static VOID handler(sig) int sig;
{
        signal(sig, SIG_DFL);
        cleanup();
        raise(sig);
...
static char *create_subcap_file() {
        if (subcap_file[0] == '\0') {
                FILE *fp;
                tmpnam(subcap_file);
...
char **make_argv(id) UNIV id;
{
        ...
        argv =
            (char **)xmalloc((subargc + 2 + 1 + nfiles + 1) * sizeof(char *));
...
static VOID write_caps(name, p) char *name;
struct sgmlcap *p;
{
        FILE *fp;
        int i;
        ...
        for (i = 0; i < NCAPACITY; i++)
                fprintf(fp, "%s %ld\n", p->name[i],
                        p->number[i] * p->points[i]);
        fclose(fp);
}

UNIV xmalloc(n) UNS n;
{
        UNIV p = malloc(n);
        if (!p)
                fatal(E_NOMEM);
...
static VOID do_error(errnum, ap) int errnum;
va_list ap;
{
        char *text;
