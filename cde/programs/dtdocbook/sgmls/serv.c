...
*/
PETD etddef(ename) UNCH *ename; /* Element name (GI) with length byte. */
{
        PETD p;   /* Pointer to an etd. */
        int hnum; /* Hash number for ename. */

        if ((p = (PETD)hfind((THASH)etdtab, ename,
                             hnum = hash(ename, ETDHASH))) == 0) {
...
PETD etdset(PETD p, UNCH fmin, struct thdr *cmod, PETD *mexgrp, PETD *pexgrp,
            struct entity **srm)
#else
PETD etdset(p, fmin, cmod, mexgrp, pexgrp, srm) PETD p; /* Pointer to an etd. */
 ...
 */
PETD etdref(ename) UNCH *ename; /* Element name (GI) with length byte.. */
...
*/
VOID etdcan(ename) UNCH *ename; /* GI name (with length and EOS). */
{
        PETD p;
...
*/
int hash(s, hashsize) UNCH *s; /* String to be hashed. */
 ...
 */
struct hash *hfind(htab, s, h) struct hash *htab[]; /* Hash table. */
UNCH *s;                                            /* Entity name. */
...
*/
struct hash *hin(htab, name, h, size) struct hash *htab[]; /* Hash table. */
...
UNS size; /* Size of structures pointed to by table. */
{
        ...
        if ((np = hfind(htab, name, h)) != 0)
                return np; /* Return if name found. */
...
*/
struct hash *hout(htab, s, h) struct hash *htab[]; /* Hash table. */
...
*/
UNCH *replace(p, s) UNCH *p;
UNCH *s;
{
        if (p)
                frem((UNIV)p); /* Free old storage (if any). */
        if (!s)
                return (s); /* Return NULL if new string is NULL. */
        return savestr(s);
}
...
*/
UNIV rmalloc(size) unsigned size; /* Number of bytes of initialized storage. */
{
 ...
 */
VOID frem(ptr) UNIV ptr; /* Memory area to be freed. */
{ free(ptr); }
...
*/
int mapsrch(maptab, name) struct map maptab[];
UNCH *name;
{
        int i = 0;
        ...
        do {
                UNCH *mapnm, *nm;
                for (mapnm = maptab[i].mapnm, nm = name; *nm == *mapnm;
                     mapnm++) {
                        if (!*nm++)
 ...
 */
int iddef(iname) UNCH *iname; /* ID name (with length and EOS). */
{
        PID p;
        struct fwdref *r;
...
*/
struct fwdref *idref(iname) UNCH *iname; /* ID name (with length and EOS). */
{
        PID p;
        int hnum;
        struct fwdref *rp;

        if ((p = (PID)hfind((THASH)itab, iname,
                            (hnum = hash(iname, IDHASH)))) == 0)
