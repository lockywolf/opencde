 ...
 */
/* $XConsortium: traceset.c /main/3 1996/06/19 17:19:04 drk $ */
...
VOID traceset() {
        ...
        if (trace || atrace || ctrace || dtrace || etrace || gtrace || itrace ||
            mtrace || ntrace)
                fprintf(stderr,
                        "TRACESET: "
                        "state=%d;att=%d;con=%d;dcl=%d;ent=%d;grp=%d;id=%d;ms=%"
                        "d;dcn=%d.\n",
 ...
 */
VOID traceval(pcb, atype, aval, tokencnt) struct parse *pcb;
...
{
        fprintf(stderr, "%-8s %2d-%2d-%2d-%2d at %p, atype=%02x, tokencnt=%d: ",
                pcb->pname, pcb->state, pcb->input, pcb->action, pcb->newstate,
                (UNIV)aval, atype, tokencnt);
        fprintf(stderr, "%s\n", aval);
}
/* TRACESTK: Trace entry just placed on tag stack.
 */
VOID tracestk(pts, ts2, etictr) struct tag *pts; /* Stack entry for this tag. */
int ts2;                                         /* Stack depth. */
int etictr; /* Number of "netok" tags on stack. */
{
        ...
        fprintf(stderr, " srm=%s.\n",
                pts->tsrm != SRMNULL ? (char *)(pts->tsrm[0]->ename + 1)
                                     : "#EMPTY");
}
/* TRACEDSK: Trace entry just removed from tag stack.
 */
VOID tracedsk(pts, ptso, ts3,
              etictr) struct tag *pts; /* Stack entry for new open tag. */
struct tag *ptso;                      /* Stack entry for tag just ended. */
int ts3;                               /* Stack depth. */
int etictr;                            /* Number of "netok" tags on stack. */
{
        fprintf(stderr,
                "DESTACK  %s ended; otflag=%02x; %s resumed; depth=%d; "
                "tflag=%02x; etictr=%d",
                ptso->tetd->etdgi + 1, ptso->tflags, pts->tetd->etdgi + 1, ts3,
                pts->tflags, etictr);
        fprintf(stderr, " srm=%s.\n",
                pts->tsrm != SRMNULL ? (char *)(pts->tsrm[0]->ename + 1)
                                     : "#EMPTY");
...
/* TRACECON: Trace interactions between content parse and stag/context
   ...
   */
VOID tracecon(
    etagimct, dostag, datarc, pcb, conrefsw,
    didreq) int etagimct; /* Implicitly ended elements left on stack. */
...
conrefsw=%d didreq=%d\n",
                etagimct, dostag, datarc, pcb->pname, pcb->action, conrefsw,
                didreq);
}
/* TRACESTG: Trace start-tag context validation input and results.
 */
VOID tracestg(curetd, dataret, rc, nextetd,
              mexts) struct etd *curetd; /* The etd for this tag. */
 ...
 */
VOID traceetg(pts, curetd, tsl,
              etagimct) struct tag *pts; /* Stack entry for this tag. */
...
struct entity *p;
{
        ...
        if (!p)
                return;
        fprintf(stderr, "%-8s (es=%d) type %c%c entity %s at %p containing ",
                action, es, estype1[p->estore], estype2[p->estore],
                p->ename + 1, (UNIV)p);
 ...
 */
VOID tracedcn(p) struct dcncb *p;
{
        fprintf(stderr, "DCN      dcn=%p; adl=%p; notation is %s\n", (UNIV)p,
                (UNIV)p->adl, p->ename + 1);
        if (p->adl)
                traceadl(p->adl);
}
/* TRACEESN: Print a data entity control block.
 */
VOID traceesn(p) PNE p;
{
        fprintf(stderr, "ESN      Entity name is %s; entity type is %s.\n",
                (NEENAME(p) != 0) ? ((char *)NEENAME(p)) + 1 : "[UNDEFINED]",
                /* NEXTYPE(p)); */
                (NEXTYPE(p) == 1 ? "CDATA"
                                 : (NEXTYPE(p) == 2 ? "NDATA" : "SDATA")));
 ...
 */
VOID tracesrm(action, pg, gi) char *action;
TECB pg;
UNCH *gi;
{
 ...
 */
VOID traceadl(al) struct ad al[];
{
        int i = 0;

        fprintf(stderr, "ADLIST   %p %d membe%s; %d attribut%s\n", (UNIV)al,
                ADN(al), ADN(al) == 1 ? "r" : "rs", AN(al),
                AN(al) == 1 ? "e" : "es");
        while (++i <= ADN(al)) {
                fprintf(stderr,
                        (BITOFF(ADFLAGS(al, i), AGROUP) &&
                         ADTYPE(al, i) <= ANOTEGRP)
                            ? "          %p %-8s %02x %02x %2d %2d %p %p\n"
                            : "    %p %-8s %02x %02x %2d %2d %p %p\n",
                        &al[i], ADNAME(al, i), ADFLAGS(al, i), ADTYPE(al, i),
                        ADNUM(al, i), ADLEN(al, i), ADVAL(al, i),
                        ADDATA(al, i).x);
                if (ADVAL(al, i)) {
                        ...
                        fprintf(stderr, "[%s]",
                                GET(ADFLAGS(al, i), AREQ)
                                    ? "REQUIRED"
                                    : (GET(ADFLAGS(al, i), ACURRENT) ? "CURRENT"
                                                                     : "NULL"));
        }
        fprintf(stderr, "\n");
}
 ...
 */
VOID tracemod(pg) struct thdr pg[];
{
        fprintf(stderr, "MODEL    %p %02x %d\n", (UNIV)&pg[0], pg[0].ttype,
                pg[0].tu.tnum);
        if ((pg[0].ttype & MKEYWORD) == 0) {
                ...
                for (i = 1; i < pg[0].tu.tnum + 2; i++) {
                        if (GET(pg[i].ttype, TTMASK) == TTETD)
                                fprintf(stderr,
                                        "                      %p %02x %s\n",
                                        (UNIV)&pg[i], pg[i].ttype,
                                        pg[i].tu.thetd->etdgi + 1);
 ...
 */
VOID tracegrp(pg) struct etd *pg[];
{
 ...
 */
VOID tracengr(pg) struct dcncb *pg[];
{
        int i = -1; /* Loop counter. */
 ...
 */
VOID traceetd(p) struct etd *p; /* Pointer to an etd. */
{
        fprintf(
            stderr,
            "ETD      etd=%p %s min=%02x cmod=%p ttype=%02x mex=%p, pex=%p, ",
            (UNIV)p, p->etdgi + 1, p->etdmin, (UNIV)p->etdmod, p->etdmod->ttype,
            (UNIV)p->etdmex, (UNIV)p->etdpex);
        fprintf(stderr, "adl=%p, srm=%s.\n", (UNIV)p->adl,
                (p->etdsrm == SRMNULL)
                    ? "#EMPTY"
                    : (p->etdsrm) ? (char *)(p->etdsrm[0]->ename + 1)
                                  : "#CURRENT");
}
/* TRACEID: Print an ID control block.
 */
VOID traceid(action, p) char *action;
 ...
 */
VOID tracemd(parmid) char *parmid; /* Parameter identifier. */
{
        fprintf(stderr, "MDPARM   %-8s for %-8s, token %02d, type %02u, %s.\n",
                mdname, subdcl ? (char *)subdcl : "[NONE]", parmno,
                pcbmd.action, parmid);
}
/* TRACEMS: Trace marked section activity.
 */
VOID tracems(action, code, mslevel,
             msplevel) int action; /* 1=began new level; 0=resumed previous. */
int code;
...
static VOID tracehits(h) unsigned long *h;
{
        int i;
        fprintf(stderr, " H=");
 ...
 */
VOID tracegi(stagenm, gi, mod, pos, Tstart) char *stagenm;
...
int Tstart;        /* Initial T for this group. */
{
        ...
        fprintf(stderr, " (%u) gocc=%02x gtype=%02x gnum=%d", M, GOCC, GTYPE,
                GNUM);
        tracehits(H);
        fprintf(stderr, " status=%d Tstart=%d\n", STATUX, Tstart);
        fprintf(
            stderr, "=>%-8s tocc=%02x ttype=%02x thetd=%p (%s) gietd=%p (%s)\n",
            tags[ts].tetd->etdgi + 1, TOCC, TTYPE, (UNIV)TOKEN.tu.thetd,
            (TTYPE ? (TTYPE == TTETD ? (char *)(TOKEN.tu.thetd->etdgi + 1)
                                     : "#GROUP")
                   : "#PCDATA"),
            (UNIV)gi, (gi == ETDCDATA ? "#PCDATA" : (char *)(gi->etdgi + 1)));
}
/* TRACEEND: Trace testing for end of group in CONTEXT.C processing.
 */
VOID traceend(stagenm, mod, pos, rc, opt, Tstart) char *stagenm;
struct thdr mod[]; /* Model of current open element. */
struct mpos pos[]; /* Position in open element's model. */
...
int Tstart;        /* Initial T for this group. */
{
        ...
        fprintf(stderr, " rc=%d offbitT=%d allhit=%d\n", rc,
                offbit(H, (int)T, GNUM), allhit(&GHDR, H, 0, opt));
}
