...
static int parse_spec(pp, sp) const char **pp;
struct spec *sp;
{
        char modifier = 0;
        sp->pos = NEXT;
        ...
        case 's':
                ...
                break;
        case 'p':
                /* a pointer to void has the same representation as a pointer to
                 * char */
                sp->type = PCHAR;
                break;
        case 'n':
...
static int find_arg_types(format, arg_type) const char *format;
...
static void get_arg(arg_type, app, argp) enum arg_type arg_type;
va_list *app;
union arg *argp;
{
        switch (arg_type) {
...
static int do_arg(handle, func, buf, arg_type, argp) UNIV handle;
printer func;
const char *buf;
enum arg_type arg_type;
union arg *argp;
...
static int printit(handle, func, p, ap, nargs, arg) UNIV handle;
printer func;
const char *p;
va_list ap;
int nargs;
union arg *arg;
{
        ...
        while (*p)
                if (*p == '%') {
                        ...
                        start = ++p;
                        if (!parse_spec(&p, &spec))
                                abort(); /* should have caught it in
                                            find_arg_types */
...
static int maybe_positional(format) const char *format;
{
        const char *p;
        ...
        for (;;) {
                ...
                if (!p)
                        return 0;
                if (p - format >= 2 && isdigit((unsigned char)p[-1]) &&
                    (p[-2] == '%' || p[-2] == '*'))
...
static int xdoprt(handle, func, format, ap) UNIV handle;
printer func;
const char *format;
va_list ap;
{
...
int xvfprintf(fp, format, ap) FILE *fp;
const char *format;
va_list ap;
{
        int res;
