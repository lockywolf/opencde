 ...
 */
/* $XConsortium: md1.c /main/3 1996/06/19 17:16:08 drk $ */
#include "sgmlincl.h" /* #INCLUDE statements for SGML parser. */
/* MDADL: Process ATTLIST declaration.
 */
VOID mdadl(tbuf) UNCH *tbuf; /* Work area for tokenization (tbuf). */
{
        ...
        if (AN(al) > 0) { /*  Save list only if 1 or more good atts. */
                if (reqadn)
                        SET(ADLF(al),
                            ADLREQ); /* Element must have start-tag. */
                if (noteadn)
                        SET(ADLF(al), ADLNOTE); /* Element cannot be EMPTY. */
                if (conradn)
                        SET(ADLF(al), ADLCONR); /* Element cannot be EMPTY. */
 ...
 */
VOID etdadl(p) struct etd *p; /* Pointer to element type definition. */
{
        parmno = 0;
...
*/
VOID mdnadl(tbuf) UNCH *tbuf; /* Work area for tokenization (tbuf). */
{
        int i;                 /* Loop counter; temporary variable. */
        int adlim;             /* Number of unused ad slots in al. */
        struct ad *alperm = 0; /* Attribute definition list. */
...
VOID fixdatt(p) struct dcncb *p;
{
        int i;
        for (i = 0; i < ENTHASH; i++) {
                struct entity *ep;
                for (ep = etab[i]; ep; ep = ep->enext)
                        if (ep->estore == ESN && ep->etx.n &&
                            ep->etx.n->nedcn == p) {
                                int adn;
                                initatt(p->adl);
                                /* Don't use adlval because if there were
                                   required attributes the error message
                                   wouldn't say what entity was involved. */
...
*/
int mdattdef(adlim, datt) int adlim; /* Remaining capacity of al (in tokens).*/
int datt;                            /* Non-zero if a data attribute. */
{
        ...
        ADVAL(al, attadn) = 0;
        ADDATA(al, attadn).x = 0;
        ADTYPE(al, attadn) = ANMTGRP;
        ...
        if (grppcb != NULL) {
                ...
                case GRPS: /* Name token list. */
                        ...
                        /* Call routine to parse group, create ad entries in
                         * adl. */
                        errcode =
                            anmtgrp(grppcb, al + attadn,
                        ...
                        if (errcode <= 0) {
                                if (adlim < GRPCNT)
                                        ...
                                        mderr(105, ADNAME(al, attadn),
                                              (UNCH *)0);
                                return 1;
                        }
                        ADN(al) += ADNUM(
                            al, attadn); /* Add grp size to total ad cnt.*/
                        break;
                default:
        ...
        case RNS: /* Keyword. */
                ...
                case DFIXED: /* FIXED */
                        ...
                        parsemd(lbuf, AVALCASE,
                                (ADTYPE(al, attadn) == ACHARS) ? &pcblitr
                                                               : &pcblitt,
                                LITLEN); /* Real default. */
...
parm3x:
        ...
        case LIT:  /* Literal. */
        case LITE: /* Literal. */
                /* Null string (except CDATA) is error: msg and treat as
                 * IMPLIED. */
        ...
        case NUM: /* Number or number token string. */
                  /* The name won't have a length byte because AVALCASE was
                   * specified. */
                break;
        case CDR:
        ...
        if (ADTYPE(al, attadn) == ACHARS) {
                ...
                ADVAL(al, attadn) =
                    savestr(lbuf); /* Store default; save ptr. */
                ADLEN(al, attadn) = len;
                ds.attdef += len;
                return 0;
        }
        /* Parse value and save token count (GROUP implies 1 token). */
        advalsv = (UNCH *)rmalloc(ustrlen(lbuf) +
                                  2); /* Storage for tokenized value. */
        errcode = parseval(lbuf, (UNS)ADTYPE(al, attadn), advalsv);
        if (BITOFF(ADFLAGS(al, attadn), AGROUP))
                ADNUM(al, attadn) = (UNCH)tokencnt;

        /* If value was invalid, or was a group member that was not in the
           group, issue an appropriate message and set the error switch. */
        if (errcode) {
                sgmlerr((UNS)errcode, &pcbmd, ADNAME(al, attadn), lbuf);
                errsw = 1;
        ...
        ADLEN(al, attadn) =
            vallen(ADTYPE(al, attadn), ADNUM(al, attadn), advalsv);
...
/* ANMTGRP: Parse a name or name token group, create attribute descriptors
            ...
            switch is needed.
*/
int anmtgrp(pcb, nt, grplim, adn,
            adsz) struct parse *pcb; /* PCB for name or name token grp. */
...
{
        UNCH adtype =
            (UNCH)(pcb == &pcbgrnt ? ANMTGRP : ANOTEGRP); /*Attribute type.*/
        ...
        *adn = 0; /* Group is empty to start. */
        ...
        if (*adn == grplim)
                ...
                return *adn; /* Return number of tokens. */
}
/* MDDTDS: Process start of DOCTYPE declaration (through MSO).
 */
VOID mddtds(tbuf) UNCH *tbuf; /* Work area for tokenization[LITLEN+2]. */
{
        ...
        switch (pcbmd.action) {
        case NAS:
                if (mdextid(tbuf, &fpicb, dtype + 1, &estore, (PNE)0) == 0)
                        return;
 ...
 */
VOID mddtde(tbuf) UNCH *tbuf; /* Work area for tokenization. */
{
        mdessv = es;      /* Save es for checking entity nesting. */
 ...
 */
VOID mdelem(tbuf) UNCH *tbuf; /* Work area for tokenization (tbuf). */
{
        ...
        for (i = -1; nmgrp[++i];) {
                etdgi = nmgrp[i]->etdgi;
                if (*ranksuff) {
                        if ((tbuf[0] = *etdgi + ustrlen(ranksuff)) - 2 >
                            NAMELEN) {
                ...
                if (nmgrp[i]->etdmod) {
                        mderr(56, etdgi + 1, (UNCH *)0);
                        continue;
                }
                etdset(nmgrp[i], fmin + ETDDCL, cmod, mexgrp, pexgrp,
                       nmgrp[i]->etdsrm);
                ++ds.etdcnt;
                if (nmgrp[i]->adl)
                        etdadl(nmgrp[i]); /* Check ETD conflicts. */
                TRACEETD(nmgrp[i]);
        }
}

VOID adlfree(al, aln) struct ad *al;
int aln;
{
        for (; aln <= ADN(al); aln++) {
