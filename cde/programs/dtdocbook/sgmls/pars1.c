 ...
 */
/* $XConsortium: pars1.c /main/3 1996/06/19 17:16:31 drk $ */
 ...
 */
int parsecon(tbuf, pcb) UNCH *tbuf; /* Work area for tokenization. */
struct parse *pcb;                  /* Parse control block for this parse. */
{
        ...
        if (eodsw)
                return (EOD_);
        if (didreq && (conrefsw & TAGREF)) {
                didreq = 0;
                goto conr;
        }
        if (etagimct > 0) {
                etagimsw = --etagimct ? 1 : 0;
                destack();
                return (ETG_);
        }
        if (dostag) {
        ...
        while (1) {
                parse(pcb);
                srn = (int)pcb->action -
                      SRMIN; /* Just in case it's a SHORTREF. */
                switch (pcb->action) {
                ...
                case ETG_: /* Process non-null end-tag. */
                        newetd =
                            etdref(parsenm(tbuf, NAMECASE)); /* Get the GI. */
                        parsetag(&pcbetag); /* Parse tag end. */
                        if (!newetd)        /* Error: undefined.*/
                                sgmlerr(11, &pcbetag, tbuf + 1, (UNCH *)0);
                        else if (etagetd(&pcbetag) >= 0)
                                return ETG_; /* Open element. */
                        if (pcb->action != ETC_)
                                continue;
                        /* Tag is undefined or not for an open element and we
                           are in a CDATA or RCDATA element; issue message and
                           treat as null end-tag (</>).
                        */
                        sgmlerr(57, &pcbetag, (UNCH *)0, (UNCH *)0);
                case NET_: /* Process null end-tag. */
                        if ((refrc = netetd(conpcb)) != 0)
                                return ETG_;
                        continue;
                ...
                case EOD_: /* End of primary file. */
                        if (ts < 1)
                                return (EOD_); /* Normal end: stack is empty. */
                        etagimct =
                            ts - 1; /* Treat as end-tag for top tag on stack. */
                        etagmin = MINETAG;
                        etagreal = tags[0].tetd;
                        destack();
                ...
                */
                ...
                case SR4_:        /* Process SR4 (RS, blanks). */
                        ...
                        if (*FPOS == '\r') {
                                ...
                                REPEATCC;
                        if ((refrc = shortref(srn, pcb)) == DEF_)
                                goto nrcc;
                        if (refrc > 0)
                                return refrc;
                ...
                /* Short references ending with RE:
                     ...
                     as the RE will be processed immediately as the current
                   character. For B RE and RS B RE, the input is primed with a
                   special character that will be treated as an RE that cannot
                   be a short reference.
                */
                ...
                sr6:       /* Process SR6 (RS, blanks, RE). */
                        if ((refrc = shortref(srn, pcb)) != ENTUNDEF) {
                                if (refrc == DEF_)
                                        goto nrcc; /* Defined: data entity. */
                                if (refrc > 0)
                                        return refrc; /* Defined: tag entity. */
                                continue;             /* Defined: not tag. */
                        }
                        if (pcb != &pcbconm)
                                continue; /* Not mixed; ignore chars. */
		  ...
		  See 323:3-6, 412:1-7. */
		    ...
		    TRACEGML(scbsgml, pss, conactsw, conact);
                ...
                srproc:
                        ...
                        if (refrc == ENTUNDEF) { /* Treat the SR as data. */
                                data =
                                    FPOS -
                                    (srn ==
                                     lex.s.hyp2); /* Two data chars if SR20.*/
                                if (pcb !=
                                    &pcbconm) { /* If not in mixed content: */
                                        if (srn >=
                                            lex.s.data) { /* Change PCB. */
                                                pcb = conpcb = &pcbconm;
                                                pcb->newstate = pcbcnda;
                                        }
                                } else
                                        pcb->newstate =
                                            pcbcnda; /* Now in data found state.
                                                      */
                        }
                        continue;
                ...
                case EE_: /* Entity end in RCDATA: check nesting. */
                        ...
                        /* If back at top level, re-enable the ending
                         * delimiters. */
                        if (es == rcessv) {
                                lexcon[lex.d.tago] = lex.l.tago;
                                lexcon[lex.d.net] =
                                    etictr ? lex.l.net : lex.l.nonet;
                ...
                default:
                        return (int)pcb
                            ->action; /* Default (MD_ MDC_ MSS_ MSE_ PIS_). */
                }
        }
}
/* STAGETD: Process start-tag etd.
 */
int stagetd(pcb) struct parse *pcb; /* Parse control block for this parse. */
{
        if (!newetd->etdmod) {
                sgmlerr(43, pcb, newetd->etdgi + 1, (UNCH *)0);
                ++ds.etdercnt;
                etdset(newetd, (UNCH)SMO + EMO + ETDOCC, &undechdr, (PETD *)0,
                       (PETD *)0, (PECB *)0);
                TRACEETD(newetd);
        }
        stagmin = MINNONE;
 ...
 */
int nstetd() {
        newetd = ts > 0 ? tags[ts].tetd : tags[0].tetd->etdmod[2].tu.thetd;
        stagmin = MINNULL;
        stagreal = ETDNULL;
        etisw = 0;
        return stag(0);
 ...
 */
int etagetd(pcb) struct parse *pcb; /* Parse control block for this parse. */
{
        etagmin = MINNONE;
        etagreal = newetd;
        if ((etagimct = etag()) < 0) {
 ...
 */
int netetd(pcb) struct parse *pcb; /* Parse control block for this parse. */
{
        if (ts < 1) {
...
*/
int shortref(srn, pcb) int srn; /* Short reference number. */
struct parse *pcb;              /* Parse control block for this parse. */
{
        int rc; /* Return code from entopen. */

        if (tags[ts].tsrm == SRMNULL || !tags[ts].tsrm[srn])
                return ENTUNDEF;
...
int parsepro() {
        ...
        while (1) {
                ...
                case DTD_: /* Process document type declaration. */
                        ...
                        if (!ustrcmp(tbuf + 1, sgmlkey) && !dtdsw &&
                            !sgmlsw++) {
#if 0
		    parse(&pcbmdi);
#endif
                ...
                case DTE_:        /* DOCTYPE declaration (and prolog) ended. */
                        ...
                        else if (dtdrefsw) {  /* Process referenced DTD before
                                                 real DTE. */
                                dtdrefsw = 0; /* Keep us from coming through
                                                 here again. */
                                REPEATCC;     /* Put back MSC so it follows
                                                 referenced DTD. */
                                entref(indtdent);
                        } else
                                mddtde(tbuf);
                ...
                case PIS_:        /* Return processing instruction (string). */
                        ...
                        return ((int)propcb
                                    ->action); /* Prolog will continue later. */
...
static VOID checkdtd() {
        ...
        for (np = dcntab[0]; np; np = np->enext)
                if (!np->defined)
                        sgmlerr(192, (struct parse *)0, np->ename + 1,
                                (UNCH *)0);
}
...
static VOID commbufs() {
        al = (struct ad *)rmalloc((ATTCNT + 2) * sizeof(struct ad));
        lbuf = (UNCH *)rmalloc(LITLEN + 1);
}

static struct mpos *newmpos() {
        int j;
        unsigned long *h;
        struct mpos *p =
            (struct mpos *)rmalloc((GRPLVL + 2) * sizeof(struct mpos));

        assert(grplongs > 0);
        h = (unsigned long *)rmalloc((GRPLVL + 2) * grplongs *
...
VOID endprolog() {
        ...
        scbsgml =
            (struct restate *)rmalloc((TAGLVL + 1) * sizeof(struct restate));
...
VOID setdtype() {
        ...
        if (!prcon[2].tu.thetd->etdmod) {
                ...
                ++ds.etdercnt;
                etdset(prcon[2].tu.thetd, (UNCH)SMO + EMO + ETDUSED + ETDOCC,
                       &undechdr, (PETD *)0, (PETD *)0, (PECB *)0);
        }
        TRACEETD(docetd);
        TRACEMOD(prcon);
...
*/
VOID parsetag(
    pcb) struct parse *pcb; /* Parse control block: pcbstag or pcbetag. */
{
...
/* STAG: Check whether a start-tag is valid at this point in the document
         ...
         it is never stacked.
*/
int stag(dataret) int dataret; /* Data pending: DAF_ REF_ 0=not #PCDATA. */
{
        ...
        if (dataret == 0) {
                ...
                if (GET(newetd->etdmod->ttype, MNONE))
                        conrefsw = TAGREF;
        }
        if (GET(tags[ts].tetd->etdmod->ttype, MANY))
                rc = mexts > 0 ? RCMEX : RCHIT;
        else
                rc = context(newetd, tags[ts].tetd->etdmod, tags[ts].tpos,
        ...
        case RCREQ: /* Stack compulsory GI, then retry start-tag. */
		    ...
		    sgmlerr(E_MEXERR, &pcbstag, nextetd->etdgi+1,
...
#endif
                        if (!nextetd->etdmod) {
                                sgmlerr(53, &pcbstag, nextetd->etdgi + 1,
                                        (UNCH *)0);
                                etdset(nextetd, (UNCH)SMO + EMO + ETDOCC,
                                       &undechdr, (PETD *)0, (PETD *)0,
                                       (PECB *)0);
                                ++ds.etdercnt;
                                TRACEETD(nextetd);
                        }
                ...
                if (BITOFF(nextetd->etdmin, SMO)) {
                        if (!BADPTR(stagreal))
                                sgmlerr(21, &pcbstag, nextetd->etdgi + 1,
                                        stagreal->etdgi + 1);
                        else if (stagreal == ETDCDATA)
                                sgmlerr(49, &pcbstag, nextetd->etdgi + 1,
                                        (UNCH *)0);
                        else
                                sgmlerr(50, &pcbstag, nextetd->etdgi + 1,
                                        (UNCH *)0);
                }
        ...
        case RCMISS: /* Start-tag invalid (#PCDATA or actual). */
                ...
                if (dataret) {
                        if (dataret == REF_)
                                ...
                                sgmlerr(E_CHARS, conpcb,
                                        tags[ts].tetd->etdgi + 1, (UNCH *)0);
                        return dataret;
                }
...
*/
int pexmex(curetd) struct etd *curetd; /* The etd for this GI. */
{
        int tsl;     /* Temporary stack level for looping. */
        int pex = 0; /* 1=found in plus grp; 0=not. */

        for (tsl = ts; tsl > 0; --tsl) {
                if (tags[tsl].tetd->etdmex &&
                    ingrp(tags[tsl].tetd->etdmex, curetd))
                        return (tsl);
                if (tags[tsl].tetd->etdpex &&
                    ingrp(tags[tsl].tetd->etdpex, curetd))
                        pex = -1;
        }
        return (pex);
}
...
int etag() {
        ...
        while (--tsl) {
                if (newetd != ETDNET ? newetd == tags[tsl].tetd
                                     : tags[tsl].tflags) {
...
VOID destack() {
        ...
        */
        if (!GET(tags[ts].tetd->etdmod->ttype, MKEYWORD) && !conrefsw &&
            !econtext(tags[ts].tetd->etdmod, tags[ts].tpos, &tags[ts].status)) {
                if (BADPTR(nextetd))
                        sgmlerr(54, conpcb, tags[ts].tetd->etdgi + 1,
                                (UNCH *)0);
                else
                        sgmlerr(30, conpcb, tags[ts].tetd->etdgi + 1,
                                nextetd->etdgi + 1);
        }
        ...
        */
        ...
        if (ecode)
                sgmlerr((UNS)ecode, conpcb, tags[ts + 1].tetd->etdgi + 1,
                        eparm2);
        /* TEMP: See if parser bug caused stack to go below zero. */
        else if (ts < 0) {
                sgmlerr(64, conpcb, (UNCH *)0, (UNCH *)0);
                ts = 0;
        }
