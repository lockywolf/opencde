...
VOID ioinit(swp) struct switches *swp;
{
        ismagic[EOBCHAR] = 1;
        ismagic[EOFCHAR] = 1;
        ismagic[EOS] = 1;
...
UNCH *buf;
{
        ...
        if (!f->canseek) {
                ...
                memcpy((UNIV)f->pendbuf, (UNIV)(buf + off),
                       (UNS)(s - buf - off));
                return;
        }
        f->bol = 0;
...
int ioread(p, buf, newfilep) UNIV p;
UNCH *buf;
int *newfilep;
{
        int i = 0;
...
static FILE *openfile(name, seekp) char *name;
char *seekp;
{
        FILE *fp;
...
static int closefile(fp) FILE *fp;
{
        if (fp == stdin) {
                stdinused = 0;
                clearerr(fp);
