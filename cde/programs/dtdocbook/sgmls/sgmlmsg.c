 ...
 */
/* $XConsortium: sgmlmsg.c /main/3 1996/06/19 17:17:57 drk $ */
/* sgmlmsg.c -
...
VOID msgprint(e) struct error *e;
{
        if (printit(stderr, e))
                ++cnterr;
        fflush(stderr);
...
VOID msgsfree(p) UNIV p;
{ frem(p); }

/* Return 1 if it should be counted as an error. */

static int printit(efp, e) FILE *efp;
struct error *e;
{
        int indent;
        int countit;
        ...
        if (prog) {
                fprintf(efp, "%s: ", prog);
                indent = strlen(prog) +
                         2; /* don't rely on return value of fprintf */
                ...
                do {
                        ...
                        if (loc.filesw) {
                                ...
                                xfprintf(efp, getheader(HDRLOC),
                                         ioflid(loc.fcb), loc.ename, loc.rcnt,
                                         loc.ccnt);
                        } else if (middle)
                                xfprintf(efp, getheader(HDRELOC), loc.ename,
                                         loc.rcnt + 1, loc.ccnt);
        ...
        if (filelevel >= 0) {
                ...
                xfprintf(efp, getheader(HDRLOC), ioflid(loc.fcb), loc.ename,
                         loc.rcnt, loc.ccnt);
...
static VOID elttrace(efp, indent) FILE *efp;
int indent;
{
        int i = 1;
        UNCH *gi;
...
static VOID spaces(efp, indent) FILE *efp;
int indent;
{
        while (--indent >= 0)
                putc(' ', efp);
...
VOID msginit(swp) struct switches *swp;
{
        catd = swp->catd;
        prog = swp->prog;
        sweltr = swp->sweltr;
...
int msgcnterr() { return cnterr; }

/* Transform a parameter into a form suitable for printing. */

static char *transparm(s, buf) UNCH *s;
char *buf;
{
        char *ptr;
        int cnt;
