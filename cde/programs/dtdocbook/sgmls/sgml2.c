...
#else
PECB entdef(ename, estore,
            petx) UNCH *ename; /* Entity name (with length and EOS). */
...
*/
PECB entfind(ename) UNCH *ename; /* Entity name (with length and EOS). */
{
        PECB p;
...
*/
int entref(ename) UNCH *ename; /* Entity name (with length and EOS). */
{
        PECB ecb; /* Entity control block. */

        /* Get the entity control block, if the entity has been defined. */
        if ((ecb = (PECB)hfind((THASH)etab, ename, hash(ename, ENTHASH))) ==
                0 ||
            ecb->estore == 0) {
                if (ename[1] == lex.d.pero || ecbdeflt == 0 ||
                    (ecb = usedef(ename)) == 0) {
                        sgmlerr(ename[1] == lex.d.pero || ecbdeflt == 0 ? 35
                                                                        : 150,
...
*/
int entopen(ecb) struct entity *ecb; /* Entity control block. */
{
        int i; /* Loop counter. */
        ...
        case ESN:
                if (NEXTYPE(ecb->etx.n) != ESNSUB) {
                        if (!NEDCNDEFINED(ecb->etx.n))
                                sgmlerr(78, (struct parse *)0,
                                        NEDCN(ecb->etx.n) + 1, ecb->ename + 1);
                } else {
#if 0
	       if (!NEID(ecb->etx.n)) {
		    ...
		    return ENTFILE;
                        ...
                        if (sw.nopen >= sd.subdoc)
                                sgmlerr(188, (struct parse *)0, (UNCH *)NULL,
                                        (UNCH *)NULL);
                }
                data = (UNCH *)ecb->etx.n;
                entdatsw = NDECONT;
        ...
        */
        for (i = 0; ++i <= es;)
                if (scbs[i].ecb.enext == ecb) {
                        sgmlerr(36, (struct parse *)0, ecb->ename + 1,
                                (UNCH *)0);
                        return (ENTLOOP);
                }
        ...
        */
        ...
        COPIEDSW = 0;
        memcpy((UNIV)&ECB, (UNIV)ecb,
               (UNS)ENTSZ); /* Copy the ecb into the scb. */
        ...
        */
        ...
        if (ECB.etx.x == 0) {
                ...
                switch (ecb->estore) {
                case ESF:
                        sgmlerr(149, (struct parse *)0, ecb->ename + 1,
                                (UNCH *)0);
                        break;
                case ESP:
                        sgmlerr(229, (struct parse *)0, ecb->ename + 2,
                                (UNCH *)0);
                        break;
                default:
                        abort();
...
int entget() {
        ...
        conterr:
                if (es == 0) { /* Report if it is primary file. */
                        FPOS =
                            FBUF - 1; /* Preserve CCNT for omitted end-tags. */
                        return -1;
                }
        case EOS: /* End of memory entity: pop the stack. */
                ...
                fileread(); /* Read the file. */
                if (iorc <= 0)
                        goto readerr; /* If READ not successful: */
...
*/
PECB usedef(ename) UNCH *ename; /* Entity name (with length and EOS). */
{
        ...
        if ((estore = ecbdeflt->estore) <
            ESFM) /* Default is an internal string. */
                ...
                if ((etx.x = entgen(&fpidf)) == 0)
                        return (PECB)0;
                if (estore == ESN) {
                        memcpy((UNIV)(pne = (PNE)rmalloc((UNS)NESZ)),
                               (UNIV)ecbdeflt->etx.n, (UNS)NESZ);
                        NEID(pne) = etx.x;
                        etx.n = pne;
                }
        }
        if (sw.swrefmsg)
                sgmlerr(45, (struct parse *)0, ename + 1, (UNCH *)0);
        ++ds.ecbcnt;
        ecb = entdef(ename, estore, &etx);
        ecb->dflt = 1;
        if (pne)
                NEENAME(pne) = ecb->ename;
        return (ecb);
}
...
*/
UNIV mdnmtab[] = {
    (UNIV)key[KATTLIST],  (UNIV)key[KDOCTYPE],  (UNIV)key[KELEMENT],
    (UNIV)key[KENTITY],   (UNIV)key[KLINKTYPE], (UNIV)key[KLINK],
    (UNIV)key[KNOTATION], (UNIV)sgmlkey,        (UNIV)key[KSHORTREF],
    (UNIV)key[KUSELINK],  (UNIV)key[KUSEMAP]};
UNIV pcbtab[] = {
    (UNIV)&pcbconc, (UNIV)&pcbcone, (UNIV)&pcbconm, (UNIV)&pcbconr,
    (UNIV)&pcbetag, (UNIV)&pcbgrcm, (UNIV)&pcbgrcs, (UNIV)&pcbgrnm,
    (UNIV)&pcbgrnt, (UNIV)&pcblitc, (UNIV)&pcblitp, (UNIV)&pcblitr,
    (UNIV)&pcblitt, (UNIV)&pcblitv, (UNIV)&pcbmd,   (UNIV)&pcbmdc,
    (UNIV)&pcbmdi,  (UNIV)&pcbmds,  (UNIV)&pcbmsc,  (UNIV)&pcbmsi,
    (UNIV)&pcbmsrc, (UNIV)&pcbpro,  (UNIV)&pcbref,  (UNIV)&pcbstag,
    (UNIV)&pcbval,  (UNIV)&pcbeal,  (UNIV)&pcbsd,
};
UNS ptrsrch(ptrtab, ptr) UNIV ptrtab[];
UNIV ptr;
{
        UNS i;
...
*/
VOID mderr(number, parm1, parm2) UNS number; /* Error number. */
...
UNCH *parm2; /* Additional parameters (or NULL). */
{
        ...
        err.errsp = (sizeof(pcbtab) / sizeof(pcbtab[0])) +
                    ptrsrch(mdnmtab, (UNIV)mdname);
        error(&err);
}
/* SGMLERR: Process errors for SGML parser.
 */
VOID sgmlerr(number, pcb, parm1, parm2) UNS number; /* Error number. */
 ...
 */
UNIV saverr(number, pcb, parm1, parm2) UNS number; /* Error number. */
struct parse *pcb; /* Current parse control block. */
UNCH *parm1;       /* Error message parameters. */
UNCH *parm2;       /* Error message parameters. */
{
 ...
 */
VOID svderr(p) UNIV p;
{ msgsprint(p); }
/* EXITERR: Process terminal errors for SGML parser.
 */
VOID exiterr(number, pcb) UNS number; /* Error number. */
struct parse *pcb;                    /* Current parse control block. */
{
        struct error err;
 ...
 */
VOID synerr(number, pcb) UNS number; /* Error number. */
struct parse *pcb;                   /* Current parse control block. */
{
        struct error err;
        errorinit(&err, DOCERR, number);
 ...
 */
VOID filerr(number, parm) UNS number;
UNCH *parm;
{
        struct error err;
 ...
 */
VOID errorinit(e, type, number) struct error *e;
UNS type;
UNS number;
{
        int i;
