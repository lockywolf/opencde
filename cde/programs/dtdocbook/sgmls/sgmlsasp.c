 ...
 */
/* $XConsortium: sgmlsasp.c /main/3 1996/06/19 17:18:11 drk $ */
/* sgmlsasp.c
...
static void output_replacement P((struct replacement *,
...
static void do_file(fp, tablep) FILE *fp;
struct replacement_table *tablep;
{
        struct sgmls *sp;
        ...
        while (sgmls_next(sp, &e))
                ...
                case SGMLS_EVENT_END:
                        output_replacement(
                            lookup_replacement(tablep, END_ELEMENT, e.u.end.gi),
                            0);
                        break;
...
static void output_pi(s, len) char *s;
unsigned len;
{
        for (; len > 0; len--, s++)
                output_data_char(*s);
...
static void output_replacement(repl, attributes) struct replacement *repl;
...
static void output_attribute(p) struct sgmls_attribute *p;
{
        ...
        case SGMLS_ATTR_ENTITY: {
                ...
                for (i = 0; i < n; i++) {
                        if (i > 0)
                                output_char(' ');
                        output_token(v[i]->is_internal ? v[i]->u.internal.name
                                                       : v[i]->u.external.name);
                }
        } break;
...
static void output_token(s) char *s;
{
        for (; *s; s++)
                output_char(*s);
}

static void output_data_char(c) int c;
{
        if (c != RSCHAR) {
                if (c == RECHAR)
                        c = '\n';
