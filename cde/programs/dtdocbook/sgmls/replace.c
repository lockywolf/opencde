...
static struct replacement *define_replacement P((struct replacement_table *,
                                                 enum event_type, char *));
static struct replacement_item **parse_string P((struct replacement_item **,
                                                 int));
static UNIV xmalloc P((unsigned));
static UNIV xrealloc P((UNIV, unsigned));
static struct replacement_item **
    add_replacement_data P((struct replacement_item **, char *, unsigned));
static struct replacement_item **
    add_replacement_attr P((struct replacement_item **, char *));
...
void load_replacement_file(tablep, file) struct replacement_table *tablep;
char *file;
{
        int tok;
        struct buffer name;
...
static struct replacement_item **
    parse_string(tail, recog_attr) struct replacement_item **tail;
int recog_attr;
{
        struct buffer buf;
        unsigned len;
        ...
        for (;;) {
                ...
                if (recog_attr && c == '[') {
                        if (buffer_length(&buf)) {
                                ...
                                tail = add_replacement_data(
                                    tail, buffer_extract(&buf), len);
                        }
                        scan_name(&buf, ']');
                        ...
                        if (c == '\\') {
                                ...
                                case EOF:
                                        parse_error(
                                            "unfinished string at end of file");
                                case 's':
                                        buffer_append(&buf, ' ');
                                        break;
                                case 'n':
...
static struct replacement_item **
    add_replacement_data(tail, buf, n) struct replacement_item **tail;
char *buf;
unsigned n;
{
        if (!tail)
...
static struct replacement_item **
    add_replacement_attr(tail, name) struct replacement_item **tail;
char *name;
{
        if (!tail)
                free(name);
...
static void scan_name(buf, term) struct buffer *buf;
int term;
{
        int c;
        for (;;) {
...
struct replacement *lookup_replacement(tablep, type,
                                       name) struct replacement_table *tablep;
enum event_type type;
char *name;
{
...
static struct replacement *
    define_replacement(tablep, type, name) struct replacement_table *tablep;
enum event_type type;
char *name;
{
        int h = hash(type, name);
...
static VOID buffer_init(buf) struct buffer *buf;
{
        buf->size = buf->len = 0;
        buf->s = 0;
}

static char *buffer_extract(buf) struct buffer *buf;
{
        char *s = buf->s;
        buf->s = 0;
        buf->len = 0;
...
static VOID buffer_append(buf, c) struct buffer *buf;
int c;
{
        if (buf->len >= buf->size) {
                if (!buf->size)
...
static int hash(type, s) enum event_type type;
char *s;
{
        unsigned long h = 0, g;
...
static UNIV xrealloc(p, size) UNIV p;
unsigned size;
{
