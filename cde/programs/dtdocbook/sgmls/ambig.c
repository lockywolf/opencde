...
static int listambig(list) UNCH *list;
{
        UNCH *p;
        int chars = 0;
        int rc = 0;
...
static VOID contoken(m, checkand,
                     res) int m; /* Index of content token in gbuf */
...
static VOID andgroup(m, checkand, res) int m;
int checkand;
struct contoken *res;
{
        int i, tnum;
...
static VOID seqgroup(m, checkand, res) int m;
int checkand;
struct contoken *res;
{
        int i, tnum;
...
static VOID orgroup(m, checkand, res) int m;
int checkand;
struct contoken *res;
{
        int i, tnum;
...
static VOID listmerge(p, b) UNCH *p, *b;
{
        UNCH *a = mergebuf;
...
static struct contoken *newcontoken() {
        struct contoken *p =
            (struct contoken *)rmalloc(sizeof(struct contoken) + MAXSTATES * 2);
        p->first = (UNCH *)(p + 1);
        p->last = p->first + MAXSTATES;
        return p;
}

static VOID freecontoken(p) struct contoken *p;
{ frem((UNIV)p); }
