...
#define HITSET(h, n)                                                           \
        (h[(unsigned)(n - 1) >> LONGPOW] |= (1L << ((n - 1) & (LONGBITS - 1))))
#define HITON(h, n)                                                            \
        (h[(unsigned)(n - 1) >> LONGPOW] & (1L << ((n - 1) & (LONGBITS - 1))))
...
static VOID copypos(to, from) struct mpos *to, *from;
{
        int i;
...
/* CONTEXT: Determine whether a GI is valid in the present structural context.
            ...
            TO DO: Save allowed GIs for an error message on an RCMISS.
                   ...
                   with a copy of pos.
*/
int context(gi, mod, pos, statuspt, mexts) struct etd *gi; /* ETD of new GI. */
...
int mexts;         /* >0=stack level of minus grp; -1=plus; 0=none.*/
{
        ...
        while (STATUS != RCMISS && STATUS != RCEND) {
                ...
                while (TTYPE == TTOR || TTYPE == TTSEQ || TTYPE == TTAND) {
                        ...
                        HITCLEAR(H);
                        Tstart =
                            T; /* Save starting token for AND group testing. */
                ...
                if (gi == TOKEN.tu.thetd) { /* Hit in model. */
                        ...
                        return (mexts <= 0
                                    ? RCHIT
                                    : (gtypesv == TTOR || BITON(toccsv, TOPT))
                                          ? RCMEX
                                          : RCHITMEX);
                }
                if (STATUS == RCREQ) {
...
/* ECONTEXT: Determine whether the current element can be ended, or whether
             ...
             TO DO: Support a "query" mode (what is required now?) by working
                    with a copy of pos.
*/
int econtext(mod, pos,
             statuspt) struct thdr mod[]; /* Model of current open element. */
struct mpos pos[]; /* Position in open element's model. */
UNCH *statuspt;    /* Token status: RCHIT RCMISS RCEND RCREQ RCNREQ*/
{
        ...
        if (P <= 1) {
                nextetd = 0;
                return (TOKENHIT || BITON(TOCC, TOPT));
        }
        ...
        while (STATUS != RCMISS && STATUS != RCEND) {
                ...
                STATUS = RCNREQ;
                if (GTYPE != TTAND)
                        ++T; /* T!=GNUM or group would have ended. */
                else
                        T = (UNCH)(((next = (UNS)offbit(H, (int)T, GNUM)) != 0)
                                       ? next
                                       : offbit(H, 0, GNUM));
...
UNCH *statuspt;    /* Token status: RCHIT RCMISS RCEND RCREQ RCNREQ*/
{
        ...
        */
        if ((STATUS = (UNCH)testend(mod, pos, 1, 1)) != RCNREQ)
                return;
...
retest:
        ...
        if (BITON(TOCC, TOPT) || TOKENHIT ||
            ...
            || nextand) {
                ...
                M = G + grpsz(&GHDR, (int)T - 1) + 1;
                if (GTYPE == TTAND) {
                        /* If AND group wrapped, it can end if all non-optionals
                           were hit. */
                        if (T == Tstart && !currhit) {
                                UNCH Psave = P;
                                int rc = testend(mod, pos, 0, 1);
                                if (Psave != P) {
                                        if ((STATUS = (UNCH)rc) == RCNREQ)
                                                goto retest;
                                } else
                                        STATUS = RCMISS;
                        }

                        /* We only test unhit tokens, so we must use an unhit
                           token as Tstart (which is used to detect when the AND
                           group has wrapped). */
                        else if (HITON(H, Tstart))
                                Tstart = T;
                }
        } else
                STATUS = RCMISS;
...
*/
int testend(mod, pos, andoptsw,
            newtknsw) struct thdr mod[]; /* Model of current open element. */
struct mpos pos[];                       /* Position in open element's model. */
...
int newtknsw; /* 1=new token test; 0=end element test. */
{
        ...
        while (!rc) {
                ...
                /* TESTMISS:
                   ...
                   the current token is the last unhit one in the group we can
                   test, we will end the group (it may never really have
                   started!) because we might be able to try the token that
                   follows it. In any group, a token is the last testable unhit
                   token if it is the last sequential one, as the GI was already
                   tested against the preceding unhit tokens.  In addition, in a
                   SEQ group, it is the last testable unhit token if it isn't
                   optional, because we can't skip past it to the following
                   ones. If we end the group, before popping the level, set M to
                   G, as this level`s group header will be the next level's
                   current token.
                */
                if (!ANYHIT(H) &&
                    (T == GNUM || (GTYPE == TTSEQ && BITOFF(TOCC, TOPT)))) {
                        M = G;
                        --P;
                        Tstart = T;
                        if (P <= 1) {
                ...
                /* TESTHIT:
                   ...
                   If we were called from NEWTOKEN we are trying to find the
                   token to test against the next start-tag, so if the group is
                   repeatable, process it again.  (If not, we were called from
                   ECONTEXT and are testing whether the element can be ended.)
                ...
                */
                if ((GTYPE == TTOR && TOKENHIT) ||
                    ...
                    (GTYPE == TTAND && allhit(&GHDR, H, 0, andoptsw))) {
                        ...
                        HITSET(H, T);
                        Tstart = T;
                        if (newtknsw && BITON(TOCC, TREP))
                                rc = RCHIT;
                        else if (P <= 1)
                                rc = RCEND;
                        /* If we are looking for a new token to test against the
                           next start-tag, then we need to consider optional and
                           members in this group, even if we didn't need to
                           consider them in the group that we just ended because
                           that group had wrapped. */
                        else if (newtknsw)
                                andoptsw = 1;
                        /* Else loop to test new outer group. */
                } else
                        rc = RCNREQ; /* No group ended this time, so return. */
        }
...
/* TOKENOPT: Return 1 if current token is contextually optional;
             otherwise, return 0.
*/
int tokenopt(mod, pos) struct thdr mod[]; /* Model of current open element. */
struct mpos pos[]; /* Position in open element's model. */
{
...
/* GROUPOPT: Temporarily makes the current group be the current token so that
             ...
             by TOKENOPT.
*/
int groupopt(mod, pos) struct thdr mod[]; /* Model of current open element. */
struct mpos pos[]; /* Position in open element's model. */
{
        ...
        if (P == 1)
                return (BITON(GOCC, TOPT) || TOKENHIT);
        saveM = M;
        M = G;
        --P;
        rc = tokenopt(mod, pos);
...
*/
int tokenreq(gi, mod, pos) struct etd *gi; /* ETD of new GI. */
struct thdr mod[];                         /* Model of current open element. */
struct mpos pos[]; /* Position in open element's model. */
{
        TRACEGI("TOKENREQ", gi, mod, pos, Tstart);
        return (tokenopt(mod, pos)
                    ? RCNREQ
                    : (GTYPE == TTSEQ &&
                       (ANYHIT(H) || groupreq(gi, mod, pos) == RCREQ)
#if 0
...
/* GROUPREQ: Temporarily makes the current group be the current token so that
             ...
             by TOKENREQ.
*/
int groupreq(gi, mod, pos) struct etd *gi; /* ETD of new GI. */
struct thdr mod[];                         /* Model of current open element. */
struct mpos pos[]; /* Position in open element's model. */
{
        UNCH saveM; /* Save M when testing if group is not required.*/
        ...
        if (P == 1)
                return (BITOFF(GOCC, TOPT) ? RCREQ : RCNREQ);
        saveM = M;
        M = G;
        --P;
...
*/
int grpsz(g, t) struct thdr *g; /* mod[G]: Ptr to group in the model. */
...
*/
int allhit(p, hits, but,
           opt) struct thdr *p; /* mod[G]: Ptr to group in the model. */
...
int opt;                        /* 1=optional tokens must be hit; 0=ignore. */
{
        ...
        while (++p, ++b <= e) {
                if (HITOFF(hits, b) && (opt || BITOFF(p->ttype, TOPT)) &&
                    b != but)
                        return 0;
                if ((type = GET(p->ttype, TTMASK)) == TTOR || type == TTSEQ ||
                    type == TTAND)
                        p += grpsz(p, p->tu.tnum);
        }
        return 1;
...
*/
int offbit(bits, first, last) unsigned long *bits; /* Bits to be tested. */
...
int anyhit(bits) unsigned long *bits;
{
        int i;
