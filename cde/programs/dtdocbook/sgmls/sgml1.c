 ...
 */
enum sgmlevent sgmlnext(rcbdafp, rcbtagp) struct rcbdata *rcbdafp;
struct rcbtag *rcbtagp;
{
        while (prologsw && !conactsw) {
                ...
                case APP_: /* APPINFO */
                        conactsw =
                            1; /* We can skip sgmlact() in opening state. */
                        break;
                ...
                case DCE_:
                case MSS_:
                        /* prcon[2].tu.thetd holds the etd for the document
                         * element. */
                        newetd = stagreal = prcon[2].tu.thetd;
                        stagmin = MINSTAG; /* This tag was minimized. */
                        /* It's an error if the start tag of the document
                           element is not minimizable. */
                        ...
                        if (oconact == MSS_) {
                                if (msplevel == 0)
                                        conpcb = getpcb((int)ETDCON);
                                conpcb = mdms(
                                    tbuf, conpcb); /* Parse the marked section
                                                      start. */
                        }
                        break;
        ...
        for (;;) {
                unsigned
                    swact; /* Switch action: saved conact, new, or sgmlact.*/
                ...
                case MD_: /* Process markup declaration. */
                        ...
                        if (!ustrcmp(tbuf + 1, key[KUSEMAP]))
                                mdsrmuse(tbuf);
                        else
                                sgmlerr(E_MDNAME, conpcb, tbuf + 1, (UNCH *)0);
                        continue;
                case MDC_: /* Process markup declaration comment. */
                        if (*FPOS != lex.d.mdc)
                                parsemd(tbuf, NAMECASE, (struct parse *)0,
                                        NAMELEN);
                        continue;
                ...
                case ETG_: /* Return end-tag. */
                        charmode =
                            0; /* Not in char mode unless CDATA or RCDATA.*/
                        if (msplevel == 0)
                                conpcb = getpcb((int)ETDCON);
                        ...
                        if (etagmin == MINSTAG)
                                rcbtagp->tagreal = BADPTR(stagreal)
                                                       ? stagreal
                                                       : (PETD)stagreal->etdgi;
                        else
                                rcbtagp->tagreal = BADPTR(etagreal)
                                                       ? etagreal
                                                       : (PETD)etagreal->etdgi;
                        rcbtagp->etictr = etictr;
                        rcbtagp->srmnm = tags[ts].tsrm != SRMNULL
                                             ? tags[ts].tsrm[0]->ename
                                             : 0;
                        scbset(); /* Update location in current scb. */
                        return SGMLETG;
                ...
                case STG_: /* Return start-tag. */
                        charmode =
                            0; /* Not in char mode unless CDATA or RCDATA.*/
                        if (!conrefsw && msplevel == 0)
                                conpcb = getpcb((int)ETDCON);
                        rcbtagp->contersw = tags[ts].tflags;
                        rcbtagp->tagmin = dostag ? MINSTAG : stagmin;
                        rcbtagp->curgi = tags[ts].tetd->etdgi;
                        /* Get attribute list if one was defined for this
                         * element. */
                        rcbtagp->ru.al = !tags[ts].tetd->adl
                                             ? 0
                                             : rcbtagp->tagmin == MINNONE
                                                   ? al
                                                   : tags[ts].tetd->adl;
                        rcbtagp->tagreal =
                ...
                case EOD_: /* End of source document entity. */
                        if (mslevel != 0)
                                sgmlerr(139, conpcb, (UNCH *)0, (UNCH *)0);
                        idrck();  /* Check idrefs. */
                        scbset(); /* Update location in current scb. */
                        return SGMLEOD;
...
#else
int sgmlact(
    conret) UNCH conret; /* Action returned to SGML.C by content parse. */
#endif
{
        int action;
        ...
        if (conret == STG_ && GET(tags[ts].tflags, TAGPEX)) {
                ++pss;
                scbsgml[pss].snext = ST1;
        }
        scbsgml[pss].sstate = scbsgml[pss].snext;
        scbsgml[pss].snext =
            sgmltab[scbsgml[pss].sstate][scbsgml[pss].sinput = conret - DAF_];
        scbsgml[pss].saction =
            sgmltab[scbsgml[pss].sstate + 1][scbsgml[pss].sinput];
 ...
 */
struct parse *
    getpcb(etdcon) int etdcon; /* Content type of new or resumed element. */
{
        if (BITON(etdcon, MGI)) {
        ...
        if (BITON(etdcon, MCDATA) || BITON(etdcon, MRCDATA)) {
                charmode = 1;
                return (BITON(etdcon, MCDATA) ? &pcbconc
                                              : (rcessv = es, &pcbconr));
        }
        return (&pcbconm);
}

struct markup *sgmlset(swp) struct switches *swp;
{
        /* Initialize variables based on switches structure members. */
        sw = *swp;
        rbufs =
            (UNCH *)rmalloc((UNS)3 + sw.swbufsz) + 3; /* DOS file read area. */
...
static UNCH cappoints[] = {1, 2, 1, 2, 2, 2, 2, 2, 1, 2, 2, 1, 2, 2, 2, 2, 2};

static long capnumber[NCAPACITY];
static long maxsubcap[NCAPACITY];

VOID sgmlend(p) struct sgmlcap *p;
{
        int i;
        for (; es >= 0; --es)
                if (FILESW)
        ...
        for (i = 0; i < NCAPACITY; i++) {
                ...
                if (excess > 0) {
                        ...
                        sgmlerr(162, (struct parse *)0, (UNCH *)captab[i],
                                (UNCH *)buf);
                }
        }
}

VOID sgmlsubcap(v) long *v;
{
        int i;
        for (i = 0; i < NCAPACITY; i++)
                if (v[i] > maxsubcap[i])
...
*/
int sgmlgent(iname, np, tp) UNCH *iname;
PNE *np;
UNCH **tp;
{
...
int sgmlgcnterr() { return msgcnterr(); }
...
int inprolog() { return prologsw; }

/* Used by the error handling functions to access scbs. */

int getlocation(level, locp) int level;
...
int sgmlloc(linenop, filenamep) unsigned long *linenop;
char **filenamep;
{
        int level = es;
        int es;
