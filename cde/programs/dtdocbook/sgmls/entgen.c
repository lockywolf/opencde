...
static char *classes[] = {"capacity", "charset", "notation", "syntax",
                          "document", "dtd",     "elements", "entities",
                          "lpd",      "nonsgml", "shortref", "subdoc",
                          "text"};
...
static int mindatcpy(p, q, len, fold) char *p, *q;
int len;
int fold;
{
...
static int field(f, c, buf) struct fpi *f;
int c;
char *buf;
{
        int n;
        ...
        case 'N': /* the entity, document or dcn name */
                return mindatcpy(
                    buf, (char *)f->fpinm, ustrlen(f->fpinm),
        ...
        case 'X':
                ...
                if (f->fpistore < 1 || f->fpistore > 5)
                        return -1;
                n = (f->fpipubis != 0) * (f->fpiversw > 0 ? 2 : 1) * 5 +
                    f->fpistore - 1;
                if (buf)
                        strcpy(buf, genext[n]);
        ...
        case 'P': /* public identifier */
                if (!f->fpipubis)
                        return -1;
                return mindatcpy(buf, (char *)f->fpipubis, ustrlen(f->fpipubis),
                                 0);
        ...
        case 'L': /* public text language */
                ...
                return mindatcpy(buf, (char *)f->fpipubis + f->fpil, f->fpill,
                                 1);
        case 'O': /* owner identifier */
                return mindatcpy(buf, (char *)f->fpipubis + f->fpio, f->fpiol,
                                 0);
        ...
        case 'T': /* text description */
                return mindatcpy(buf, (char *)f->fpipubis + f->fpit, f->fpitl,
                                 0);
        case 'V':
                ...
                return mindatcpy(buf, (char *)f->fpipubis + f->fpiv, f->fpivl,
                                 0);
...
static int testopen(pathname) char *pathname;
{
#ifdef HAVE_ACCESS
...
UNIV entgen(f) struct fpi *f;
{
        ...
        if (f->fpisysis &&
            (!sysidsrch || strchr((char *)f->fpisysis, SYSID_FILE_SEP) ||
