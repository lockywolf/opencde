 ...
 */
VOID mdentity(tbuf) UNCH *tbuf; /* Work area for tokenization[LITLEN+2]. */
{
        ...
        case PEN:
                ...
                if (nmbuf[1] == NAMELEN + 2) {
                        ...
                        nmbuf[0] =
                            nmbuf[1] + 1; /* Increment length for PERO. */
        ...
        case RNS: /* Reserved name started. */
                ...
                memcpy(nmbuf, indefent,
                       *indefent); /* Copy #DEFAULT to name buffer. */
        ...
        case NAS:
                if ((estore = (UNCH)mapsrch(enttab, tbuf + 1)) == 0) {
                        ...
                        if (mdextid(tbuf, fpis, nmbuf + 1 + parmsw, &estore,
                                    pne) == 0)
                                return;
                        if (defltsw)
                                etx.x = NULL;
...
*/
UNCH *sandwich(s, pref, suff) UNCH *s; /* String, with EOS. */
...
*/
struct fpi *
    mdextid(tbuf, fpis, ename, estore,
            pne) UNCH *tbuf; /* Work area for tokenization[2*(LITLEN+2)]. */
...
parm3:
        ...
        if (pcbmd.action == LIT || pcbmd.action == LITE) {
                ...
                memcpy(tbuf, sysibuf, *sysibuf);
        if (*estore != ESF || pcbmd.action != NAS)
                goto genfpi;
...
*/
int parsefpi(f) PFPI f; /* Ptr to formal public identifier structure. */
{
        ...
        if (*p == '+' || *p == '-') { /* If owner registered, unregistered. */
                ...
                if ((p += 3) >= l)
                        return 1; /* Get to owner ID field. */
        } else
                f->fpiot = '!'; /* Indicate ISO owner identifier. */
        if ((q = pubfield(p, l, '/', &len)) ==
            0) /* Find end of owner ID field. */
                return 2;
        f->fpiol = len;            /* Save owner ID length. */
        f->fpio = p - f->fpipubis; /* Save offset in pubis to owner ID. */

        if ((p = pubfield(q, l, ' ', &len)) ==
            0) /* Find end of text class field. */
                return 3;
        ...
        *p++ = ' ';                     /* Restore the SPACE delimiter. */
        if (f->fpic == 0)
                return 4; /* Error if not valid uc class name.*/

        /* The public text class in a notation identifier must be NOTATION. */
        if (f->fpistore == ESK - ESFM + 1 && f->fpic != FPINOT)
                return 10;

        if (*p == '-') {       /* If text is unavailable public text.*/
                f->fpitt = *p; /* Save text type. */
                if ((p += 3) >= l)
                        return 5; /* Get to text description field. */
        } else
                f->fpitt = '+'; /* Indicate available public text. */
        if ((q = pubfield(p, l, '/', &len)) ==
            0) /* Find end of text description. */
                return 6;
        ...
        } else if (f->fpic >= FPICMINV)
                f->fpiversw = 1; /* No version: get the best. */
        return (0);
}
...
#else
UNCH *pubfield(p, l, d,
               lenp) UNCH *p; /* Public identifier field (no length or EOS). */
UNCH *l;                      /* Pointer to EOS of public identifier. */
...
#endif
{
        ...
        while (p < l) {
                if (*p++ == d) { /* Test for delimiter character. */
                        *lenp =
                            p - psv; /* Save field length (no len or EOS). */
                        if (d == '/' &&
                            *p++ != d) /* Solidus requires a second one. */
                                continue;
...
*/
struct parse *
    mdms(tbuf, pcb) UNCH *tbuf; /* Work area for tokenization [NAMELEN+2]. */
struct parse *pcb;              /* Parse control block for this parse. */
{
        ...
        while ((ptype = parsemd(tbuf, NAMECASE, &pcblitp, NAMELEN)) == NAS) {
                ...
                if (key == MSTEMP)
                        continue; /* TEMP: for documentation. */
                msplevel = 1;     /* Special parse required. */
                if (key > pcbcode)
                        pcbcode = key; /* Update if higher priority. */
        }
        if (ptype != MDS) {
...
int mdmse() {
        ...
        if (mslevel)
                ...
                sgmlerr(26, (struct parse *)0, (UNCH *)0, (UNCH *)0);
 ...
 */
VOID mdnot(tbuf) UNCH *tbuf; /* Work area for tokenization[LITLEN+2]. */
{
        struct fpi fpicb;  /* Formal public identifier structure. */
        ...
        if (mdextid(tbuf, &fpicb, lbuf + 1, &estore, (PNE)0) == 0)
                return;
...
*/
struct dcncb *
    dcndef(nname) UNCH *nname; /* Notation name (with length and EOS). */
{
...
*/
struct dcncb *
    dcnfind(nname) UNCH *nname; /* Notation name (with length and EOS). */
{
 ...
 */
VOID mdsrmdef(tbuf) UNCH *tbuf; /* Work area for tokenization[LITLEN+2]. */
{
        ...
        if ((srhptr = srhfind(tbuf)) != 0) {
                ...
                if (SRM(0)) {
                        mderr(56, tbuf + 1, (UNCH *)0);
                        return;
                }
        } else
                srhptr = srhdef(tbuf); /* Create map with SRs mapped to NULL.*/
        ...
        while (pcbmd.newstate = 0,
               parsemd(tbuf, NAMECASE, &pcblitp, SRMAXLEN) == LIT ||
                ...
                if (SRM(srn)) {
                        mderr(56,
                              (srn < lex.s.prtmin ? (UNCH *)lex.s.pdtb[srn]
                                                  : lex.s.dtb[srn].mapnm),
                              (UNCH *)0);
                        continue;
                }
        ...
        TRACEMD(emd);
        if (parmno == 2) {
                mderr((UNS)(pcbmd.action == EMD ? 28 : 123), (UNCH *)0,
                      (UNCH *)0);
                goto cleanup;
        }
        if (pcbmd.action != EMD)
...
cleanup:
        ...
        if (mapused)
                MEMZERO((UNIV)srhptr->srhsrm,
                        sizeof(PECB) * (lex.s.dtb[0].mapdata + 1));
        else {
 ...
 */
VOID mdsrmuse(tbuf) UNCH *tbuf; /* Work area for tokenization[LITLEN+2]. */
{
        PSRH srhptr; /* Ptr to short reference map hdr (in srhtab).*/
        ...
        for (i = -1; nmgrp[++i];) {
                if (!nmgrp[i]->etdsrm)
                        nmgrp[i]->etdsrm = srmptr;
                else if (sw.swdupent)
                        mderr(68, nmgrp[i]->etdgi + 1, (UNCH *)0);
        }
realemd:
        if (es != mdessv)
                synerr(37, &pcbmd);
...
*/
PSRH srhdef(sname) UNCH *sname; /* SHORTREF map name (with length and EOS). */
{
...
/* SRHFIND: If a SHORTREF map was declared, return the ptr to its header.
            Return NULL if it is not defined.
*/
PSRH srhfind(sname) UNCH *sname; /* SHORTREF map name (with length and EOS). */
