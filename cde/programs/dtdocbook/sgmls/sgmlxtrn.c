 ...
 */
/* $XConsortium: sgmlxtrn.c /main/3 1996/06/19 17:18:20 drk $ */
...
struct map deftab[] = {/* Default value keywords. */
                       {key[KIMPLIED], DNULL}, {key[KREQUIRED], DREQ},
                       {key[KCURRENT], DCURR}, {key[KCONREF], DCONR},
                       {key[KFIXED], DFIXED},  {NULL, 0}};
struct map dvtab[] = {
    /* Declared value: keywords and type codes.*/
    /*                                TYPE      NUMBER   */
...
struct map enttab[] = {/* Entity declaration second parameter. */
                       {key[KCDATA], ESC},    {key[KSDATA], ESX},
                       {key[KMS], ESMS},      {key[KPI], ESI},
                       {key[KSTARTTAG], ESS}, {key[KENDTAG], ESE},
                       {key[KMD], ESMD},      {NULL, 0}};
...
struct map mstab[] = {/* Marked section keywords. */
                      {key[KTEMP], MSTEMP},
                      {key[KINCLUDE],
                       MSTEMP}, /* Treat INCLUDE like TEMP; both are NOPs.*/
                      {key[KRCDATA], MSRCDATA},
                      {key[KCDATA], MSCDATA},
                      {key[KIGNORE], MSIGNORE},
                      {NULL, 0}};
struct map pubcltab[] =
    {/* Names for public text class. */
     {(UNCH *)"CAPACITY", FPICAP},  {(UNCH *)"CHARSET", FPICHARS},
     {(UNCH *)"DOCUMENT", FPIDOC},  {(UNCH *)"DTD", FPIDTD},
     {(UNCH *)"ELEMENTS", FPIELEM}, {(UNCH *)"ENTITIES", FPIENT},
     {(UNCH *)"LPD", FPILPD},       {(UNCH *)"NONSGML", FPINON},
     {(UNCH *)"NOTATION", FPINOT},  {(UNCH *)"SHORTREF", FPISHORT},
     {(UNCH *)"SUBDOC", FPISUB},    {(UNCH *)"SYNTAX", FPISYN},
     {(UNCH *)"TEXT", FPITEXT},     {NULL, 0}};
