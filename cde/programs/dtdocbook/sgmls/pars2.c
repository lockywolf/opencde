 ...
 */
/* $XConsortium: pars2.c /main/3 1996/06/19 17:16:36 drk $ */
...
*/
int parse(pcb) struct parse *pcb; /* Current parse control block. */
{
        ...
        while (1) {
                ...
                case ERX_:        /* General entity reference; return. */
                        ...
                        rc = entref(entbuf);
                        if (rc == ENTDATA) {
                                /* Reference to external data/subdoc entity in
                                   replaceable character data. */
                                if (BITON(entdatsw, NDECONT)) {
                                        switch (((PNE)data)->nextype) {
                                        case ESNCDATA:
                                        case ESNSDATA:
                                                /* The standard says `non-SGML
                                                   data entity' but the
                                                   amendment should have changed
                                                   it to `external data entity'.
                                                 */
                                                synerr(145, pcb);
                                                break;
                                        case ESNNDATA:
                                ...
                                /* Reference to PI entity not allowed in
                                   replaceable character data. */
                                synerr(59, pcb);
                                entpisw = 0;
                                continue;
...
*/
VOID charrefa(r) UNCH *r; /* Undelimited char ref (with length and EOS). */
{
        UNCH thechar;
...
int charrefn(r, pcb) UNCH *r; /* Undelimited character reference. */
...
VOID initatt(adl) struct ad *adl;
{
...
*/
struct ad *parseatt(adl, pt) struct ad *adl; /* Attribute definition list. */
UNCH *pt; /* Tokenization area: tbuf[TAGLEN+ATTSPLEN]. */
{
        ...
        while (pt <= tbuflim) {
                ...
                case NVS: /* Att name or value token found. */
                        parsenm(
                            pt,
                            NAMECASE); /* Case translation wanted on name. */
                        pt +=
                            *(nm = pt); /* Save name while pointing past it. */
                        continue;
                ...
                case AVDA: /* Delimited value found (alternate delimiter). */
                        ...
                        parselit(
                            pt,
                            (adn == 0 || ADTYPE(al, adn) == ACHARS) ? &pcblitr
                                                                    : &pcblitt,
                            LITLEN,
                        ...
                        /* Tokenize and validate value; let it default if an
                         * error. */
                        /* Put value in list and bump ptr by the normalized
                           length (which is always >= the actual length). */
                        if (!attval(1, pt, adn, adl))
                                pt += ADLEN(al, adn);
                        continue;
                case AVU: /* Attribute value found: undelimited. */
                        if (!sd.shorttag)
                                sgmlerr(196, &pcbstag, (UNCH *)0, (UNCH *)0);
                ...
                case NTV:         /* Name token value found. */
                        if (!sd.shorttag)
                                sgmlerr(195, &pcbstag, (UNCH *)0, (UNCH *)0);
                        if (pcbstag.action == NTV)
                                parsenm(pt, NAMECASE);
        ...
        if (es != mdessv)
                synerr(37, &pcbstag);
        if (adn < 0)
                return ((struct ad *)0); /* List was empty. */
        TRACEADL(al);
        return al;
}
...
#define DEFLEN adl[adn].adlen /* Length of default value of current            \
                                 attribute.*/
int attval(mtvsw, adval, adn,
           adl) int mtvsw; /* Must tokenize value: 1=yes; 0=no. */
...
struct ad *adl;            /* Element's master att def list. */
{
        ...
        {
                sgmlerr(73, &pcbstag, ADNAME(al, adn), adval);
                return (1);
        }
        SET(ADFLAGS(al, adn), ASPEC); /* Indicate att was specified. */
        if (GET(ADFLAGS(al, adn),
                ACONREF))          /* If attribute is content reference: */
        ...
        if (mtvsw && ADTYPE(al, adn) != ACHARS) {
                ...
                if (((errcode = parseval(adval, ADTYPE(al, adn), lbuf)) == 0) &&
                    ...
                    !amemget(&al[adn], ADNUM(al, adn), lbuf))
                        errcode = 18;
                ...
                ustrcpy(adval, lbuf);
                if (BITOFF(ADFLAGS(al, adn), AGROUP))
                        ADNUM(al, adn) = (UNCH)tokencnt;
        }
        if (!mtvsw)
                adval--;
...
*/
VOID adlval(adsz, newetd) int adsz; /* Size of list. */
struct etd *newetd; /* Element type definition for this element. */
{
        ...
        do {
                if (ADVAL(al, adn) == NULL) { /* NULL value */
                        if (GET(ADFLAGS(al, adn),
                                AREQ + ACURRENT)) { /*Error if REQ, CURRENT*/
                                sgmlerr(19, &pcbstag, ADNAME(al, adn),
                                        (UNCH *)0);
                                SET(ADFLAGS(al, adn), AINVALID);
                        }
                } else
                        switch (ADTYPE(al, adn)) {
                        case AENTITY: /* Return data ecb pointer if valid
                                         entity. */
                                aenttst(adn, ADVAL(al, adn));
                                break;
                        case AENTITYS: /* Return data ecb pointers if valid
                                          entities. */
                                pt = ADVAL(al, adn);
                        ...
                        case AID:
                                ...
                                if (iddef(ADVAL(al, adn))) {
                                        sgmlerr(71, &pcbstag, ADNAME(al, adn),
                                                ADVAL(al, adn) + 1);
                                        SET(ADFLAGS(al, adn), AINVALID);
                                        continue;
                                }
                        ...
                        case ANOTEGRP: /* Return notation identifier. */
                                if (GET(ADFLAGS(al, adn), ASPEC))
                                        notadn = adn; /*NOTATION specified*/
                                if ((dpt = dcnfind(ADVAL(al, adn))) == 0) {
                                        sgmlerr(77, &pcbstag, ADNAME(al, adn),
                                                ADVAL(al, adn) + 1);
                                        SET(ADFLAGS(al, adn), AINVALID);
                                } else
                                        ADDATA(al, adn).x = dpt;
        ...
        } while ((adn += BITON(ADFLAGS(al, adn), AGROUP)
                             ? (int)ADNUM(al, adn) + 1
                             : 1) <= adsz);
         ...
         */
        if (notadn &&
            (conrefsw || (newetd && GET(newetd->etdmod->ttype, MNONE)))) {
                sgmlerr((UNS)(conrefsw ? 84 : 76), &pcbstag, ADNAME(al, notadn),
                        ADVAL(al, notadn) + 1);
 ...
 */
VOID aenttst(adn, pt) int adn; /* Position in list. */
        ...
        if (ept->estore == ESX || ept->estore == ESC || ept->estore == ESN) {
                ...
                if (ept->estore == ESN && NEXTYPE(ept->etx.n) != ESNSUB &&
                    !NEDCNDEFINED(ept->etx.n)) {
                        sgmlerr(78, &pcbstag, NEDCN(ept->etx.n) + 1, pt + 1);
                        SET(ADFLAGS(al, adn), AINVALID);
                }
        } else {
 ...
 */
VOID idreftst(adn, pt) int adn; /* Position in list. */
 ...
 */
int anmget(adsz, nm) int adsz; /* Size of list. */
...
*/
int antvget(adsz, nm, pp) int adsz; /* Size of list. */
UNCH *nm;                           /* Value to be found (with length byte). */
UNCH **pp;                          /* Store value here */
{
        ...
        while (++adn <= adsz) {
                ...
                if (BITON(ADFLAGS(al, adn), AGROUP)) {
                        ...
                        if ((advn = amemget(&al[adn], (int)ADNUM(al, adn),
                                            nm)) != 0) {
                                if (pp)
...
*/
int amemget(anmtgrp, adsz, nm) struct ad anmtgrp[]; /* Name token group. */
...
UNCH *nm; /* Name to be found (with length byte). */
{
        ...
        while (++adn <= adsz &&
               ustrncmp(nm + 1, anmtgrp[adn].adname + 1, (UNS)*nm - 1))
                ;
        return (adn > adsz) ? 0 : adn;
}
...
*/
UNS vallen(type, num, def) int type; /* ADTYPE(al,adn) */
...
UNS parsegrp(grp, pcb,
             tbuf) struct etd *grp[]; /* Buffer for building the group. */
struct parse *pcb;                    /* Current parse control block. */
UNCH *tbuf;
{
...
UNS parsngrp(grp, pcb,
             tbuf) struct dcncb *grp[]; /* Buffer for building the group. */
struct parse *pcb;                      /* Current parse control block. */
UNCH *tbuf;
{
 ...
 */
PETD *copygrp(pg,
              grpsz) PETD pg[]; /* Pointer to a group (array of etd ptrs). */
...
/* INGRP: Locate an etd in a name group and return its index+1 (or zero
          if not found).
*/
int ingrp(pg, ketd) PETD pg[]; /* Array of pointers to etds. */
...
#else
VOID parselit(tbuf, pcb, maxlen,
              del) UNCH *tbuf; /* Work area for tokenization (parmlen+1). */
struct parse *pcb;             /* Current parse control block. */
...
int tokdata(pt, parmlen) UNCH *pt;
int parmlen;
{
...
*/
int parsemd(pt, namecase, lpcb,
            tokenlen) UNCH *pt; /* Token buffer: >=tokenlen+2. */
...
UNS tokenlen;       /* Max length of expected token: NAMELEN LITLEN */
{
        ...
        pcb =
            (lpcb) ? &pcbmd : &pcbmdc; /* If no literal pcb, dcl is comment. */
...
doparse:
        ...
        case NUM: /* Number or number token string. */
                parsetkn(pt, (UNCH)((int)tokenlen <= NAMELEN ? NU : NMC),
                         (int)tokenlen);
                return (int)pcb->action;
        case PENR:
                REPEATCC;
...
*/
struct thdr *parsemod(dctype) int dctype; /* Content type (0=model). */
{
         ...
         */
        if (!parsegcm(&pcbgrcm, &gbuf[1], &gbuf[0]))
        ...
        if (sw.swambig)
                ambig(); /* Check content model for ambiguity. */
        return gbuf;
}
...
struct thdr *
    parsegcm(pcb, pgh,
             gbuf) struct parse *pcb; /* Current parse control block. */
...
#define MCON gbuf->ttype         /* Model type (content attributes). */
        ...
        while (gbuf->tu.tnum <= GRPGTCNT && pgh->tu.tnum <= GRPCNT &&
               parse(pcb) != GRPE)
                switch (pcb->action) {
                ...
                case RNS_: /* Reserved name started (#PCDATA). */
                        ...
                        /* If #PCDATA is the first non-group token, model is a
                         * phrase. */
                        if (!MCON)
                                SET(MCON, MPHRASE);
                case DTAG: /* Data tag template ignored; treat as #PCDATA. */
                        if (pcb->action == DTAG)
                                SET(pgh->ttype, TTSEQ); /* DTAG is SEQ grp. */
                        ++gbuf->tu.tnum;
                        ++pgh->tu.tnum;
                        (++pg)->ttype =
                            TTCHARS + TOREP; /* #PCDATA is OPT and REP. */
                ...
                case GRP_: /* Group started. */
                        ...
                        ++pgh->tu.tnum;
                        (pgsv = ++pg)->ttype =
                            0;           /* Type will be set by connector. */
                        ...
                        if (!pg)
                                return (struct thdr *)0;
                        if (GET(pgsv->ttype, TOPT))
                                ++optcnt; /* Indicate nested opt grp. */
                        --grplvl;
                        continue;
                ...
                case OPT: /* OPT occurrence indicator for current token. */
                        ...
                        if (GET(pgsv->ttype, TOPT))
                                continue; /* Exit if nested opt grp. */
                        SET(pgsv->ttype, TOPT);
...
*/
UNCH *parsenm(tbuf, nc) UNCH *tbuf; /* Buffer for name: >=NAMELEN+2. */
int nc;                             /* Namecase translation: 1=yes; 0=no. */
{
        ...
        *(tbuf + (len = 1)) = nc ? lextran[*FPOS] : *FPOS;
        while ((NEWCC, (int)lextoke[*FPOS] >= NMC) && (len < NAMELEN)) {
                ...
                if (lextoke[*(tbuf + ++len) = (nc ? lextran[*FPOS] : *FPOS)] ==
                    EOB) {
                        --len;
                        entget();
                }
...
#else
UNCH *parsetkn(tbuf, scope,
               maxlen) UNCH *tbuf; /* Buffer for token: >=maxlen+1. */
...
*/
VOID parseseq(tbuf, maxlen) UNCH *tbuf; /* Buffer for storing found sequence. */
...
#else
UNCH *s2valnm(nm, s, scope, translate) UNCH *nm; /* Name to be created. */
...
*/
int parseval(s, atype,
             tbuf) UNCH *s; /* Source string to be parsed as token list. */
...
UNCH *tbuf;                 /* Work area for tokenization. */
{
        ...
        while (1) {
                for (;;) {
                        ...
                        pcbval.newstate =
                            (*(pcbval.ptab + pcbval.state))[pcbval.input];
                        pcbval.action =
                            (*(pcbval.ptab + pcbval.state + 1))[pcbval.input];
                ...
                case INVA: /* Invalid character; terminate parse. */
                        if (*s == '\0')
                                goto alldone; /* Normal termination. */
                ...
                case ANMTOKES:
                        /* No test needed because NMTA, NUMA and NASA are all
                         * valid. */
                        s2valnm(pt, s, NMC, NAMECASE);
                        break;
