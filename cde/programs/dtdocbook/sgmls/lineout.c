...
VOID process_document(subdocsw) int subdocsw;
{
        enum sgmlevent rc;
                ...
                if (!suppsw)
                        ...
                        case SGMLDAF:
                                if (CONTERSW(rcbdaf))
                                        break;
                                if (CDESW(rcbdaf))
                                        output_cdata(CDATALEN(rcbdaf),
                                                     CDATA(rcbdaf));
                                else if (SDESW(rcbdaf))
                                        output_sdata(CDATALEN(rcbdaf),
                                                     CDATA(rcbdaf));
                                else if (NDESW(rcbdaf)) {
                                        assert(NEXTYPE(NEPTR(rcbdaf)) !=
                                               ESNSUB);
                                        if (!sgmlment(NEENAME(NEPTR(rcbdaf))))
                                                define_external_entity(
                                                    NEPTR(rcbdaf));
                                        output_entity_reference(
                                            NEENAME(NEPTR(rcbdaf)) + 1);
                                } else
                                        output_pcdata(CDATALEN(rcbdaf),
                                                      CDATA(rcbdaf));
                                break;
                        case SGMLSTG:
                                if (CONTERSW(rcbtag))
                        ...
                        case SGMLAPP:
                                if (CONTERSW(rcbdaf))
                                        break;
                                if (!subdocsw)
                                        output_appinfo(ADATALEN(rcbdaf),
                                                       ADATA(rcbdaf));
                                break;
                        default:
                                abort();
...
static VOID define_external_entity(p) PNE p;
{
        if (NEXTYPE(p) == ESNSUB)
                output_subdoc(NEENAME(p) + 1, NEID(p), NEPUBID(p), NESYSID(p));
        else {
                if (!NEDCNMARK(p))
                        output_notation(NEDCN(p) + 1, NEDCNPUBID(p),
                                        NEDCNSYSID(p));
...
static VOID define_entity(ename) UNCH *ename;
{
        int rc;
        PNE np;
        UNCH *tp;
...
static VOID handle_attributes(ent, al) UNCH *ent;
struct ad *al;
{
        int aln;
...
static VOID handle_token_list(ent, al, aln) UNCH *ent;
struct ad *al;
int aln;
{
        UNCH *ptr;
...
static VOID handle_single_token(ent, al, aln) UNCH *ent;
struct ad *al;
int aln;
{
        if (ADTYPE(al, aln) == ANOTEGRP && !DCNMARK(ADDATA(al, aln).x))
                output_notation(ADVAL(al, aln) + 1, ADDATA(al, aln).x->pubid,
...
static VOID output_notation(name, pubid, sysid) UNCH *name;
UNCH *pubid, *sysid;
{
        flush_data();
...
static VOID output_internal_entity(ename, is_sdata, text) UNCH *ename;
int is_sdata;
UNCH *text;
{
        flush_data();
...
static VOID output_subdoc(nm, id, pubid, sysid) UNCH *nm;
UNIV id;
UNCH *pubid, *sysid;
{
        flush_data();
...
static VOID process_subdoc(nm, id) UNCH *nm;
UNIV id;
{
        if (!suppsw) {
                flush_data();
...
static VOID output_external_entity(nm, xtype, id, pubid, sysid, dcn) UNCH *nm,
    *dcn;
UNIV id;
UNCH *pubid, *sysid;
int xtype;
...
static VOID output_pcdata(n, s) UNS n;
UNCH *s;
{ print_data(n, s, 0); }

static VOID output_cdata(n, s) UNS n;
UNCH *s;
{ print_data(n, s, 0); }

static VOID output_sdata(n, s) UNS n;
UNCH *s;
{ print_data(n, s, 1); }

static VOID output_entity_reference(s) UNCH *s;
{
        flush_data();
        output_location();
...
static VOID output_start_tag(s) UNCH *s;
{
        flush_data();
        output_location();
        printf("%c%s\n", START_CODE, s);
...
static VOID output_end_tag(s) UNCH *s;
{
        flush_data();
        printf("%c%s\n", END_CODE, s);
}

static VOID output_processing_instruction(n, s) UNS n;
UNCH *s;
{
        flush_data();
        output_location();
...
static VOID output_appinfo(n, s) UNS n;
UNCH *s;
{
        flush_data();
        output_location();
...
static VOID output_implied_attribute(ent, aname) UNCH *ent, *aname;
{
        flush_data();
        if (ent)
...
static char *attribute_type_string(type) int type;
{
        switch (type) {
        case ANMTGRP:
        case ANAME:
...
static VOID output_begin_attribute(ent, aname, type) UNCH *ent, *aname;
int type;
{
        flush_data();
        if (ent)
...
static VOID output_attribute_token(vallen, val) UNS vallen;
UNCH *val;
{
        putchar(' ');
...
static VOID output_end_attribute() { putchar('\n'); }

static VOID print_data(n, s, is_sdata) UNS n;
UNCH *s;
int is_sdata;
{
        if (n > 0 || is_sdata) {
...
static VOID print_string(slen, s, is_sdata) UNS slen;
UNCH *s;
int is_sdata;
{
        if (is_sdata)
...
static VOID print_id(id, pubid, sysid) UNIV id;
UNCH *pubid;
UNCH *sysid;
{
...
static VOID print_filename(s) char *s;
{
        for (; *s; s++)
                switch (*s) {
                case '\\':
