...
#ifndef lint
static char *RCSid = "$TOG: util.c /main/13 1997/10/09 16:09:50 bill $";
#endif
...
char **Split(char *s,   /* input string */
        ...
        if (flag & S_ALVEC) {
                ...
                /* if caller did not specify max tokens to find, set to more
                 * than there will possibly ever be */
                if (!ntok || !(*ntok))
                        maxnt = 10000;
        } else
                tokens = local_tokens;
        ...
        while (i < maxnt) {
                ...
                if (i >= maxnt)
                        break; /* is this the last one? */
                while (*s && !IsWhite(*s))
                        s++; /* skip past end of token */
                if (*s == EOS)
                        break; /* at end of input string? */
                if (*s)
                        *s++ = EOS; /* terminate token string */
                while (*s && IsWhite(*s))
                        s++; /* skip space - to next token */
        }
        if (ntok)
                *ntok = i; /* return number of tokens found */
...
Mapping_t *FindMapping(Map_t *M, char *name) {
        ...
        for (m = M->maps, i = 0; i < M->n_used; i++)
                if (m[i].name[0] == name[0] && !strcmp(m[i].name, name))
                        return &m[i];
        return NULL;
}
...
void SetMappingNV(Map_t *M, char *name, char *value) {
        ...
        if (!strcmp(name, "foldcase")) {
                fold_case = atoi(value);
                return;
        }
...
void SetMapping(Map_t *M, char *s) {
        ...
        strcpy(buf, s);
        name = val = buf;
        while (*val && !IsWhite(*val))
                val++; /* point past end of name */
        if (*val) {
...
void PrintLocation(Element_t *e, FILE *fp) {
        ...
        if (e->lineno) {
                if (e->infile)
                        fprintf(fp,
                                "  At or near instance file: %s, line: %d\n",
                                e->infile, e->lineno);
                else
...
void OutputString(char *s, FILE *fp, int track_pos) {
        ...
        if (!s)
                s = "^"; /* no string - go to start of line */
        ...
        for (; *s; s++) {
                if (fp && (*s == '\\')) { /* don't expand for interpreter */
                        ...
                        case '|':    /* SDATA */
                                ...
                                *cp = EOS; /* terminate sdata string */
                                ...
                                if (cp)
                                        ...
                                        /* not found - output sdata thing in
                                         * brackets */
                                        Putc('[', fp);
                                        FPuts(sdata, fp);
                                        Putc(']', fp);
                        ...
                        c = *s;
                        /* If caller wants us to track position, see if it's an
                         * anchor (ie, align at a newline). */
                        if (track_pos) {
                                if (c == ANCHOR) {
                                        /* If we're already at the start of a
                                         * line, don't do another newline. */
                                        if (*ppos != 0)
                                                c = NL;
                                        else
                                                c = 0;
 ...
 */
void ClearOutputBuffer() { outputBuffer.current = outputBuffer.base; }

/* ______________________________________________________________________ */
/* determines if there is already some text in the output buffer,
 * returns 1 if so, else 0
 */
int OutputBufferActive() { return (outputBuffer.current != outputBuffer.base); }
                ...
                *pc++ = 0;
                ...
                if (result != TCL_OK) {
                        fprintf(
                            ...
                            interpreter->result, interpreter->errorLine);
                        fprintf(stderr, "\"%s\"\n", commandBuf);
                        return EOF;
                }
                return c;
...
int FPuts(const char *s, FILE *stream) {
        ...
        if (stream) {
                if (sLength >
                    100 / 2) { /* assume that every char must be escaped */
                ...
                do {
                        switch (
                            *ps) { /* escape those things that throw off Tcl */
                        case '{':
                        case '}':
                        case '"':
                        case '\'':
...
static char *LookupSDATA(char *s) {
        char *v;
...
Element_t *QRelation(Element_t *e, char *s, Relation_t rel) {
        ...
        case REL_Parent:
                if (!e->parent || !e->parent->gi)
                        return 0;
                if (!strcmp(e->parent->gi, s))
                        return e->parent;
                break;
        case REL_Child:
                for (i = 0; i < e->necont; i++)
                        if (!strcmp(s, e->econt[i]->gi))
                                return e->econt[i];
                break;
        case REL_Ancestor:
                if (!e->parent || !e->parent->gi)
        ...
        case REL_Is1stContent:
                ...
                if (!(ep = QRelation(e, s, REL_Parent)))
                        return 0;
        ...
        case REL_Unknown:
                fprintf(stderr,
                        "You cannot query 'REL_None' or 'REL_Unknown'.\n");
                break;
        }
        return NULL;
}
...
Relation_t FindRelByName(char *relname) {
        if (!strcmp(relname, "?")) {
                ...
                return REL_Cousin;
        else if (StrEq(relname, "isfirstcon"))
                return REL_Is1stContent;
        else if (StrEq(relname, "hasonlycon"))
                return REL_HasOnlyContent;
        else
                fprintf(stderr, "Unknown relationship: %s\n", relname);
        return REL_Unknown;
}
...
void DescendTree(Element_t *e, void (*enter_f)(), void (*leave_f)(),
                 void (*data_f)(), void *dp) {
        int i;
        if (enter_f)
                (enter_f)(e, dp);
        for (i = 0; i < e->ncont; i++) {
                if (e->cont[i].type == CMD_OPEN)
                        DescendTree(e->cont[i].ch.elem, enter_f, leave_f,
                                    data_f, dp);
                else if (data_f)
                        (data_f)(&e->cont[i], dp);
        }
...
Element_t *FindElemByID(char *idval) {
        ...
        for (id = IDList; id; id = id->next)
                if (id->id[0] == idval[0] && !strcmp(id->id, idval))
                        return id->elem;
        return 0;
}
