...
static char *CopyRt = "Copyright 1993 Open Software Foundation, Inc., "
...
extern void Browse();
static int TclPrintLocation(ClientData clientData, Tcl_Interp *interp, int argc,
                            char *argv[]);
static int DefaultOutputString(ClientData clientData, Tcl_Interp *interp,
                               int argc, char *argv[]);
static int CompareI18NStrings(ClientData clientData, Tcl_Interp *interp,
                              int argc, char *argv[]);
static int TclReadLocaleStrings(ClientData clientData, Tcl_Interp *interp,
                                int argc, char *argv[]);
...
char *GetCLocale() {
        ...
        if ((_DtLcxOpenAllDbs(&myDb) != 0)) {
                fprintf(stderr, "Warning: could not open databases.\n");
                exit(1);
        }
        ...
        if (_DtLcxXlateStdToOp(myDb, myPlatform, execVer, DtLCX_OPER_SETLOCALE,
                               locale, NULL, NULL, NULL, &newLocale)) {
                fprintf(stderr, "Warning: could not translate C.ISO-8859-1 to "
                                "local locale\n");
        }
...
int main(int ac, char *av[]) {
         ...
         */
        Tcl_CreateCommand(interpreter, "OutputString", DefaultOutputString, 0,
                          0);

        /* Add a hook so the interpreter can print the location in the
         * source file for user errors */
        Tcl_CreateCommand(interpreter, "PrintLocation", TclPrintLocation, 0, 0);
        ...
        Tcl_CreateCommand(interpreter, "CompareI18NStrings", CompareI18NStrings,
                          0, 0);
         ...
         * unmunged. */
        Tcl_CreateCommand(interpreter, "ReadLocaleStrings",
                          TclReadLocaleStrings, 0, 0);
        ...
        if (interactive) {
                ...
                if (tranfile) {
                        Element_t *e;
                        /* If user wants to start at a particular ID, point to
                         * that element.  Else, point to the top of the tree. */
                        if (start_id) {
                                if (!(e = FindElemByID(start_id))) {
                                        fprintf(stderr,
                                                "Error: Can not find element "
                                                "with ID %s\n",
                                                start_id);
                                        exit(1);
                                }
 ...
 */
static char *UnEscapeI18NChars(char *source) {
        unsigned char c;
        char *buf;
...
#define LOCAL_BUFFER_LENGTH 128
        ...
        if (argc < 2) {
                Tcl_SetResult(interpreter, "Missing string to output",
                              TCL_VOLATILE);
                return TCL_ERROR;
        }
...
static int CompareI18NStrings(ClientData clientData, Tcl_Interp *interp,
        ...
        if (argc < 3) {
                Tcl_SetResult(interpreter, "Missing string(s) to compare",
                              TCL_VOLATILE);
                return TCL_ERROR;
        }
...
static void Initialize1(char *myname) {
        ...
        SetMappingNV(Variables, "user",
                     (cp = getenv("USER")) ? cp : "UnknownUser");
        time(&tnow);
...
static void CmdLineSetVariable(char *var) {
        ...
        if ((cp = strchr(buf, '='))) {
                ...
                *cp = ' ';
                ...
                if (!strcmp(tok[0], "verbose"))
                        verbose = atoi(tok[1]);
                else if (!strcmp(tok[0], "warnings"))
                        warnings = atoi(tok[1]);
                else if (!strcmp(tok[0], "foldcase"))
                        fold_case = atoi(tok[1]);
                else
                        SetMappingNV(Variables, tok[0], tok[1]);
        } else {
                fprintf(stderr,
                        "Expected an '=' in variable assignment: %s. Ignored\n",
                        var);
        }
}
...
static void HandleArgs(int ac, char *av[]) {
        ...
        while ((c = getopt(ac, av, "t:vc:s:o:huSxIl:bHVWi:D:Z")) != EOF) {
                ...
                case 's': {
                        if (thisopt = sdatafile) {
                                ...
                                while (*thisopt++)
                                        count++;
                                sdatafile = (char **)realloc(
                                    sdatafile, (count + 2) * sizeof(char *));
                                sdatafile[count + 1] = 0;
                ...
                case 'c': {
                        if (thisopt = cmapfile) {
                                ...
                                while (*thisopt++)
                                        count++;
                                cmapfile = (char **)realloc(
                                    cmapfile, (count + 2) * sizeof(char *));
                                cmapfile[count + 1] = 0;
        ...
        if (interactive && !in_file) {
                fprintf(stderr, "You must specify ESIS file on cmd line for "
                                "browser mode.\n");
                exit(1);
        }
}
 ...
 */
static char *EscapeI18NChars(char *source) {
        char *retval;
        ...
        if (MB_CUR_MAX == 1) {
                ...
                while (*from) {
                        if ((len = mblen(from, MB_CUR_MAX)) < 0) {
                                fprintf(stderr,
                                        "Bad multibyte character '%c' (0x%x) "
                                        "in source file\n",
                                        *from, *from);
                                from++;
...
static char *ReadLocaleStrings(char *file_name, int *ret_code) {
        int fd;
        char *pBuf;
        char *i18nBuf;
        off_t size;
...
static int TclReadLocaleStrings(ClientData clientData, Tcl_Interp *interp,
        ...
        if (ret_code != 0) {
                if (ret_code == 1) {
                        sprintf(errorBuf,
                                "Could not open locale strings file \"%s\" for "
                                "reading",
                                argv[1]);
                }
                if (ret_code == 2) {
...
static Element_t *AccumElemInfo(FILE *fp) {
        ...
        while (1) {
                ...
                case EOF: /* End of input */
                        fprintf(stderr,
                                "Error: Unexpectedly reached end of ESIS.\n");
                        exit(1);
                        break;
                ...
                case CMD_OPEN: /* (gi */
                        ...
                        /*  Check if this elem has a notation attr.  If yes, and
                           there is no notation specified, recall the previous
                           one. (feature of sgmls - it does not repeat notation
                           stuff if we the same is used twice in a row) */
                        if ((atval =
                                 FindAttValByName(e, "ENTITYREF")) && /* HACK */
                ...
                case CMD_ATT: /* Aname val */
                        ...
                        if (!strcmp(tok[1], "CDATA") ||
                            !strcmp(tok[1], "TOKEN") ||
                            !strcmp(tok[1], "ENTITY") ||
                            !strcmp(tok[1], "NOTATION")) {
                                ...
                                fprintf(
                                    stderr,
                                    "Error: Bad attr line (%d): A%s %s...\n",
                                    e->lineno, tok[0], tok[1]);
                        }
                        break;

                case CMD_LINE: /* Llineno */
                        /* These lines come in 2 forms: "L123" and "L123
                         * file.sgml". Filename is given only at 1st occurance.
                         * Remember it.
                         */
                ...
                case CMD_DATA: /* -data */
                        fprintf(
                            stderr,
                            "Error: Data in AccumElemInfo, line %d:\n%c%s\n",
                            e->lineno, c, i18nBuf);
                        /*return e;*/
                        exit(1);
                        break;
                ...
                case CMD_INT_ENT: /* Iename typ text */
                        fprintf(stderr,
                                "Error: Got CMD_INT_ENT in AccumElemInfo: %s\n",
                                i18nBuf);
                        break;
                ...
                default:
                        fprintf(stderr,
                                "Error: Unexpected input in AccumElemInfo, "
                                "%d:\n%c%s\n",
                                e->lineno, c, i18nBuf);
                        exit(1);
                        break;
        ...
        fprintf(stderr,
                "Error: End of AccumElemInfo - should not be here: %s\n",
                e->gi);
        /*    return e;*/
        exit(1);
}
...
static Element_t *ReadESIS(FILE *fp, int depth) {
        char *buf, *i18nBuf;
        int i, c, ncont;
        Element_t *e;
        Content_t cont[5000];
        ...
        while (1) {
                ...
                default:
                        fgets(buf, LINESIZE, fp);
                        fprintf(stderr,
                                "Error: Unexpected input at %d: '%c%s'\n",
                                e->lineno, c, buf);
                        exit(1);
                        break;
                }
        ...
        for (e = DocTree; e; e = e->next) {
                ...
                for (i = 0, n = 0; i < e->ncont; i++)
                        if (IsContElem(e, i))
                                n++;
                if (n > 0)
                        Calloc(n, e->econt, Element_t *);
                for (i = 0; i < e->ncont; i++)
                        if (IsContElem(e, i))
                                e->econt[e->necont++] = ContElem(e, i);
                ...
                for (i = 0, n = 0; i < e->ncont; i++)
                        if (IsContData(e, i))
                                n++;
                if (n > 0)
                        Calloc(n, e->dcont, char *);
                for (i = 0; i < e->ncont; i++)
                        if (IsContData(e, i))
                                e->dcont[e->ndcont++] = ContData(e, i);
