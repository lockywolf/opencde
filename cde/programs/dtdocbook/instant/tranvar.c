...
ContParse_t ExpandSpecialVar(char *name, Element_t *e, FILE *fp,
                             int track_pos) {
        FILE *infile;
        ...
        if (StrEq(tok[0], "include")) {
                ...
                if (ntok > 1) {
                        if ((infile = OpenFile(name)) == NULL) {
                                sprintf(buf, "Can not open included file '%s'",
                                        name);
                                perror(buf);
                                return CONT_CONTINUE;
                        }
                        ...
                        fprintf(stderr, "No file name specified for include\n");
                return CONT_CONTINUE;
        }
         ...
         * Format: _filename */
                ...
                if (!e->entity) {
                        fprintf(
                            stderr,
                            "Expected ext entity (internal error? bug?):\n");
        ...
        /* number of 1st child's child elements (grandchildren from first
         * child). Format: _n1gchild */
         ...
         * Format: _chasetogi gi action */
                ...
                if (ntok < 3) {
                        fprintf(stderr,
                                "Error: Not enough args for _chasetogi.\n");
                        return CONT_CONTINUE;
                }
                action = atoi(tok[2]);
                if (action)
                        ChaseIDRefs(e, tok[1], action, fp);
        }
                ...
                for (s = idrefs; *s; s++) {
                        ...
                        if ((atval = FindAttValByName(e, *s))) {
                                ...
                                if ((ep = FindElemByID(tok[0]))) {
                                        ...
                                        fprintf(stderr,
                                                "Error: Can not find elem for "
                                                "ID: %s.\n",
                                                tok[0]);
                        }
                }
                fprintf(stderr,
                        "Error: Element does not have IDREF attribute set:\n");
                PrintLocation(e, stderr);
                return CONT_CONTINUE;
        }
        ...
        /* Starting at this element's parent, decend tree (in-order), finding
         * GI. Do the action if it matches. Format: _pfind args ... */
        else if (StrEq(tok[0], "pfind")) {
                Find(e->parent ? e->parent : e, ntok, tok, fp);
        }

        /* Content is supposed to be a list of IDREFs.  Follow each, doing
         * action. If 2 actions are specified, use 1st for the 1st ID, 2nd for
         * the rest. Format: _namelist action [action2] */
         ...
         * Format: _relation relationship gi action [action] */
                ...
                if (ntok >= 4) {
                        if (!CheckRelation(e, tok[1], tok[2], tok[3], fp,
                                           RA_Current)) {
         ...
         * Format: _followrel relationship gi action */
                ...
                if (ntok >= 4)
                        (void)CheckRelation(e, tok[1], tok[2], tok[3], fp,
                                            RA_Related);
        }

        /* Find element with matching ID and do action.  If action not
         * specified, choose the right one appropriate for its context. Format:
         * _id id [action] */
         ...
         * quit. */
                ...
                if ((cp = FindMappingVal(Variables, tok[1]))) {
                        if ((ntok <= 2) || (strcmp(cp, tok[2]) == 0))
                                return CONT_BREAK;
                }
                return CONT_CONTINUE;
        }

        /* Insert a node into the tree at start/end, pointing to action to
         * perform. Format: _insertnode S|E action */
        else if (StrEq(tok[0], "insertnode")) {
                action = atoi(tok[2]);
                if (*tok[1] == 'S')
                        e->gen_trans[0] = action;
                else if (*tok[1] == 'E')
                        e->gen_trans[1] = action;
        }
 ...
 *	FILE pointer to where to write output.
 */
void ChaseIDRefs(Element_t *e, char *gi, int action, FILE *fp) {
        int ntok, i, ei;
        char **tok, **s, *atval;
                        ...
                        for (i = 0; i < ntok; i++) {
                                ...
                                if ((e = FindElemByID(tok[i]))) {
                                        ...
                                        if (StrEq(e->gi, gi)) {
                                                ...
                                                /* this elem itself did not
                                                 * match, try its children */
                                                for (ei = 0; ei < e->necont;
                                                     ei++) {
                                                        if (StrEq(e->econt[ei]
                                                                      ->gi,
                                                                  gi)) {
                                                ...
                                                /* try this elem's IDREF
                                                 * attributes */
                                                ChaseIDRefs(e, gi, action, fp);
                                                return;
                                        }
                                } else {
                                        /* should not happen, since parser
                                         * checks ID/IDREFs */
                                        fprintf(stderr,
                                                "Error: Could not find ID %s\n",
                                                atval);
                                        return;
                                }
                        }
...
static void tr_find_gipar(Element_t *e, Descent_t *ds) {
        if (StrEq(ds->gi, e->gi) && e->parent && StrEq(ds->gi2, e->parent->gi))
                if (ds->action)
                        TranByAction(e, ds->action, ds->fp);
}
