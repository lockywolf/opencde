...
void DoTranslate(Element_t *e, char *transfile, FILE *fp) {
        ...
        if (!transfile) {
                fprintf(stderr, "Translation spec file not specified. Skipping "
                                "translation.\n");
                return;
        }
        ...
        if ((t = FindTransByName("_Start"))) {
                if (t->starttext)
                        ProcesOutputSpec(t->starttext, 0, fp, 1);
                if (t->startcode)
                        CallInterpreter(t->startcode, 0);
                if (t->replace)
                        ProcesOutputSpec(t->replace, 0, fp, 1);
                if (t->message)
                        ProcesOutputSpec(t->message, 0, stderr, 0);
                if (t->endcode)
                        CallInterpreter(t->endcode, 0);
                if (t->endtext)
                        ProcesOutputSpec(t->endtext, 0, fp, 1);
        }
...
void ExpandVariables(char *in, char *out, Element_t *e) {
        ...
        while (totlen >= len && *ip) {
                ...
                if (*ip == VDELIM && *(ip + 1) == L_CURLY && *(ip + 2) != '_') {
                        ...
                        len + 2;
                        vp = vbuf;
                        /*	Look for matching (closing) curly. (watch for
                         *nesting) We store the variable content in a tmp
                         *buffer, so we don't clobber the input buffer.
                         */
                        lev = 0;
                        while (*ip) {
                                if (*ip == L_CURLY)
                        ...
                        *vp = EOS;
                        /* vbuf now contains the variable name (stuff between
                         * curlys). */
                        if (lev != 0) {
                        ...
                        /* See if this variable has a default [ format:
                         * ${varname def} ] */
                        def_val = vp;
                        while (*def_val && *def_val != ' ')
                                def_val++;
                        if (*def_val)
                                *def_val++ = EOS;
                        else
                                def_val = 0;
                        /* def_val now points to default, if it exists, null if
                         * not. */
                        ...
                        if (*modifier)
                                *modifier++ = EOS;
                                ...
                                modifier = 0;
                        /* modifier now points to modifier if it exists, null if
                         * not. */

                        s = 0;
                        /* if attribute of current elem with this name found,
                         * use value */
                        ...
                        if (!s) {
                                modifier = 0; /* assume user gave us the exact
                                                 string  */
                                s = def_val;  /* may be null if no default value
                                                 given */
                        }
 ...
 *	Pointer to element under consideration.
 */
void CallInterpreter(char *ib, Element_t *e) {
        int result;
        char line[20];
        int recursive;
...
void ProcesOutputSpec(char *ib, Element_t *e, FILE *fp, int track_pos) {
        ...
        while (*ib) {
                ...
                /* If not a $, it's a regular char.  Just copy it and go to
                 * next. */
                ...
                if (ib[1] !=
                    L_CURLY) { /* just a stray dollar sign (no variable) */
                        *dest++ = *ib++;
                        continue;
                }
                ...
                *cp = EOS; /* terminate att/var name */
                ...
                /* we now have special variable name (stuff in curly {}'s) in
                 * vname */
                /* if the special variable is _break and it's true, we're done
                 */
                if (ExpandSpecialVar(&vname[1], e, fp, track_pos) ==
                    CONT_BREAK) {
                        break;
                }
        }
...
Trans_t *FindTrans(Element_t *e) {
        ...
        for (t = TrSpecs; t; t = t->next) {
                ...
                if (t->parent)
                        if (!QRelation(e, t->parent, REL_Parent))
                                continue;

                if (t->context) { /* no context specified -> a match */
                        FindContext(e, t->depth, context);

                        /* If reg expr set, do regex compare; else just string
                         * compare. */
                        if (t->context_re) {
                                if (!tpt_regexec(t->context_re, context))
                                        continue;
                        } else {
                                ...
                                /* See if context of element matches "context"
                                 * of transpec */
                ...
                if (t->nattpairs) { /* no att specified -> a match */
                        for (match = 1, a = 0; a < t->nattpairs; a++) {
                                if (!(atval = FindAttValByName(
                                          e, t->attpair[a].name))) {
                                        match = 0;
                                        break;
                                }
                                if (!tpt_regexec(t->attpair[a].rex, atval))
                                        match = 0;
                        }
                        if (!match)
                                continue;
                ...
                if (t->relations) {
                        Mapping_t *r;
                        match = 1;
                        for (r = t->relations->maps, i = 0;
                             i < t->relations->n_used; i++) {
                                if (!CheckRelation(e, r[i].name, r[i].sval, 0,
                                                   0, RA_Current)) {
                                        match = 0;
                                        break;
                                }
                ...
                if (t->nth_child) {
                        /* First one is called "1" by the user.  Internally
                         * called "0". */
                        i = t->nth_child;
                ...
                if (t->use_id) {
                        ...
                        fprintf(stderr,
                                "Warning: transpec ID (%d) not found for %s.\n",
                                t->use_id, e->gi);
                        return &NullTrans;
                }
...
Trans_t *FindTransByName(char *s) {
        ...
        for (t = TrSpecs; t; t = t->next) {
                /* check if tag name matches (first check 1st char, for
                 * efficiency) */
                if (t->gi) {
                        if (*(t->gi) != *s)
                                continue; /* check 1st character */
                        if (!strcmp(t->gi, s))
                                return t;
                }
        }
...
void DoPI(char *pi, FILE *fp) {
        ...
        if ((t = FindTransByName(tok[0]))) {
                 ...
                 */
                if (warnings)
                        fprintf(stderr, "Warning: Unrecognized PI: [%s]\n", pi);
        }
}
...
static void set_and_increment(Trans_t *t) {
        Mapping_t *m;
        int i, inc, n;
        char *cp, buf[50];
        ...
        /* increment counters */
        if (t->incr_var) {
                for (m = t->incr_var->maps, i = 0; i < t->incr_var->n_used;
                     i++) {
                        cp = FindMappingVal(Variables, m[i].name);
                        /* if not set at all, set to 1 */
                        if (!cp)
                                SetMappingNV(Variables, m[i].name, "1");
                        else {
                                if (isdigit(*cp) ||
                                    (*cp == '-' && isdigit(cp[1]))) {
                                        n = atoi(cp);
                                        if (m[i].sval && isdigit(*m[i].sval))
                                                inc = atoi(m[i].sval);
                                        else
                                                inc = 1;
                                        sprintf(buf, "%d", (n + inc));
        ...
        if (t->ignore != IGN_ALL) {
                ...
                if (e->gen_trans[0]) {
                        ...
                        if ((tp = FindTranByID(e->gen_trans[0]))) {
                                if (tp->starttext)
                                        ProcesOutputSpec(tp->starttext, e, fp,
                                                         1);
                                if (tp->startcode)
                                        CallInterpreter(t->startcode, e);
                                if (tp->message)
                                        ProcesOutputSpec(tp->message, e, stderr,
                                                         0);
                                if (tp->endcode)
                                        CallInterpreter(t->endcode, e);
                                if (tp->endtext)
...
int CheckRelation(Element_t *e, char *relname, /* relationship name */
                  char *related,               /* related element */
                  char *actname,               /* action to take */
                  FILE *fp, RelAction_t flag) {
        Element_t *ep;
        Relation_t r;

        if ((r = FindRelByName(relname)) == REL_Unknown)
                return 0;
        if (!(ep = QRelation(e, related, r)))
                return 0;

        if (!actname)
                return 1; /* no action - return what we found */

        switch (flag) {
        case RA_Related:
                TranByAction(ep, atoi(actname), fp);
                break;
        case RA_Current:
                TranByAction(e, atoi(actname), fp);
                break;
        }
        return 1;
}
