...
void ReadTransSpec(char *transfile) {
        ...
        if ((fp = OpenFile(transfile)) == NULL) {
                fprintf(stderr,
                        "Can not open translation spec file '%s'.\n%s\n",
        ...
        while (fgets(buf, LINESIZE, fp)) /* read line from .ts file */
        {
                ...
                /* '-' indicates end of a spec.  When we hit one, remember what
                 * we've accumulated so far, and null-out the accumulating
                 * structure. */
                if (buf[0] == '-') {
                        T.lineno = lineno;
                ...
                /*  See if next line is continued from this one -- ie. it starts
                 * with whitespace.  If so, append to current line.  (This is
                 * similar to how e-mail headers work...) */
                while (1) {
                        c = getc(fp);     /* 1st char of next line */
                        if (IsWhite(c)) { /* space or tab? */
                                /* keep getting characters until it's a
                                 * non-whitespace */
                                c = getc(fp);
                                while (IsWhite(c))
                                        c = getc(fp);
                                ...
                                i = strlen(buf);
                                fn = buf +
                                     i; /* point to end of string in buffer */
                                fgets(fn, LINESIZE - i,
                                      fp); /* read and append to buf */
                                lineno++;
                                stripNL(buf);
                        } else {
                                ungetc(c, fp); /* put back non-whitespace */
                ...
                if ((cp = strchr(buf, ':'))) {
                        ...
                        fprintf(stderr,
                                "Trans spec error, missing colon (skipping "
                                "line):\n  %s\n",
                                fn);
                        continue;
                }
                ...
                if (!strncmp("GI:", fn, 3)) {
                        /* if we are folding the case of GIs, make all upper
                           (unless it's an internal pseudo-GI name, which starts
                           with '_') */
                ...
                } else if (!strncmp("StartText:", fn, 10))
                        T.starttext = strdup(cp);
                else if (!strncmp("EndText:", fn, 8))
                        T.endtext = strdup(cp);
                else if (!strncmp("Relation:", fn, 9)) {
                        if (!T.relations)
                                T.relations = NewMap(IMS_relations);
                        SetMapping(T.relations, cp);
                } else if (!strncmp("StartCode:", fn, 10))
                        T.startcode = strdup(cp);
                else if (!strncmp("EndCode:", fn, 8))
                        T.endcode = strdup(cp);
                else if (!strncmp("Replace:", fn, 8))
                        T.replace = strdup(cp);
                else if (!strncmp("AttValue:", fn, 9)) {
                        if (!T.nattpairs) {
                                Malloc(1, T.attpair, AttPair_t);
                        } else
                                Realloc((T.nattpairs + 1), T.attpair,
                                        AttPair_t);
                 ...
                 */
                else if (!strncmp("Context:", fn, 8))
                        T.context = strdup(cp);
                else if (!strncmp("Message:", fn, 8))
                        T.message = strdup(cp);
                else if (!strncmp("SpecID:", fn, 7))
                        T.my_id = atoi(cp);
                else if (!strncmp("Action:", fn, 7))
                        T.use_id = atoi(cp);
                else if (!strncmp("Content:", fn, 8))
                        T.content = strdup(cp);
                else if (!strncmp("PAttSet:", fn, 8))
                        ...
                        if (!strcmp(cp, "all"))
                                ...
                                fprintf(stderr,
                                        "Bad 'Ignore:' arg in transpec (line "
                                        "%d): %s\n",
                                        lineno, cp);
                ...
                } else if (!strncmp("NthChild:", fn, 9))
                        T.nth_child = atoi(cp);
                else if (!strncmp("Var:", fn, 4))
                        SetMapping(Variables, cp);
                else if (!strncmp("Quit:", fn, 5))
                        T.quit = strdup(cp);
                else
                        fprintf(stderr,
                                "Unknown translation spec (skipping it): %s\n",
                                fn);
        }
        fclose(fp);
}
...
void RememberTransSpec(Trans_t *t, int lineno) {
        ...
        if (t->context) {
                 ...
                 * it contains a character other than what's allowed for GIs in
                 * the OSF sgml declaration (alphas, nums, '-', and '.').
                 */
                ...
                if (do_regex) {
                        ...
                        /* If there's only one item in context, it's the parent.
                         * Treat it specially, since it's faster to just check
                         * parent gi.
                         */
                        cp = t->context;
                        if (!strchr(cp, ' ')) {
...
void ReadCharMap(char *filename) {
        ...
        if ((fp = OpenFile(filename)) == NULL) {
                fprintf(stderr,
                        "Can not open character mapping file '%s'.\n%s\n",
        ...
        while (fgets(buf, LINESIZE, fp)) {
                ...
                if (name) {
                        ...
                        if (val)
                                CharMap[nCharMap].sval = strdup(val);
                        if (CharMap[nCharMap].name[0] == '\\')
