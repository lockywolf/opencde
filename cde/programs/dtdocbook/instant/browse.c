...
#ifndef lint
static char *RCSid = "$XConsortium: browse.c /main/3 1996/06/19 17:13:03 drk $";
#endif
...
static char *br_help_msg[] = {
    ...
    "",
    "  tran file [outfile]",
    "                With trans spec in 'file' translate into 'outfile' "
    "(stdout)",
                ...
                if (!strcmp(cmd, "ls"))
                        ...
                        if (av[1]) {
                                if (ac == 3 && !strcmp(av[1], "id")) {
                                        if ((e = FindElemByID(av[2])))
                                                ce = e;
                                        else
                                                printf("Element with ID '%s' "
                                                       "not found.\n",
                                                       av[2]);
                                        continue;
                                }
...
static void do_find(Element_t *e, char **av) {
        ...
        if (!strcmp(av[0], "."))
                ...
                e = DocTree;
        if (!strcmp(av[0], "gi"))
                DescendTree(e, match_gi, 0, 0, av);
        else if (!strcmp(av[0], "attr"))
                DescendTree(e, match_attr, 0, 0, av);
        else if (!strcmp(av[0], "parent"))
                DescendTree(e, match_parent, 0, 0, av);
        else if (!strcmp(av[0], "child"))
                DescendTree(e, match_child, 0, 0, av);
        else if (!strcmp(av[0], "cont"))
                DescendTree(e, match_cont, 0, 0, av);
        else if (!strcmp(av[0], "sib"))
                DescendTree(e, match_sib, 0, 0, av);
        else if (!strcmp(av[0], "desc"))
                DescendTree(e, match_desc, 0, 0, av);
        else if (!strcmp(av[0], "anc"))
                DescendTree(e, match_anc, 0, 0, av);
        else
                fprintf(stderr, "Unknown find command: %s.\n", av[0]);
}
