...
#define WARN_MSG                                                               \
        ((char *)GETMESSAGE(18, 12,                                            \
                            "The workspace manager must be restarted in\n\
...
static char *icon[] = {"iconTL", "iconTR", "iconBL", "iconBR",
                       "iconLT", "iconLB", "iconRT", "iconRB"};

static char *placementStr[] = {"top left",     "top right",   "bottom left",
...
void popup_dtwmBB(Widget shell) {
...
static void getDtwmValues() {
        ...
        if (status = XrmGetResource(db, "dtwm.keyboardFocusPolicy",
                ...
                if (strcmp(string, "pointer") == 0) {
                        XmToggleButtonGadgetSetState(dtwm.pointerTG, True,
                                                     True);
                        dtwm.origKeyboardFocusPolicy = POINTER;
                }
                if (strcmp(string, "explicit") == 0) {
                        XmToggleButtonGadgetSetState(dtwm.explicitTG, True,
                                                     True);
        ...
        if (status =
                XrmGetResource(db, "dtwm.focusAutoRaise", "Dtwm.FocusAutoRaise",
                               &str_type_return, &value_return)) {
                /* make local copy of string */
                string = (char *)XtMalloc(value_return.size);
                strcpy(string, value_return.addr);
        ...
        if (status = XrmGetResource(db, "dtwm.secondariesOnTop",
                                    "Dtwm.secondariesOnTop", &str_type_return,
                                    &value_return)) {
                /* make local copy of string */
                string = (char *)XtMalloc(value_return.size);
                strcpy(string, value_return.addr);
                ...
                dtwm.origSecStack = (strcmp(string, "true") ? False : True);
        ...
        if (status = XrmGetResource(db, "dtwm.useIconBox", "Dtwm.UseIconBox",
                                    &str_type_return, &value_return)) {
                /* make local copy of string */
                string = (char *)XtMalloc(value_return.size);
                strcpy(string, value_return.addr);
                ...
                dtwm.origUseIconBox = (strcmp(string, "true") ? False : True);
                if (dtwm.origUseIconBox)
                        XmToggleButtonGadgetSetState(dtwm.iconBoxTG, True,
                                                     True);
                else
                        XmToggleButtonGadgetSetState(dtwm.desktopTG, True,
                                                     True);
        ...
        if (status = XrmGetResource(db, "dtwm.moveOpaque", "Dtwm.MoveOpaque",
                                    &str_type_return, &value_return)) {
                /* make local copy of string */
                string = (char *)XtMalloc(value_return.size);
                strcpy(string, value_return.addr);
                ...
                dtwm.origMoveOpaque = (strcmp(string, "true") ? False : True);
...
static Widget build_dtwmDlg(Widget shell) {
        ...
        XtSetArg(args[n], XmNtitle,
                 ((char *)GETMESSAGE(18, 1, "Style Manager - Window")));
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNimageName, DTWM_ICON);
        n++;
        XtSetArg(args[n], XmNtraversalOn, False);
        n++;
        widgetList1[count++] = dtwm.pictLabel =
            _DtCreateIcon(form, "dtwmpictLabel", args, n);
        ...
        n = 0;
        string = CMPSTR(
            (char *)GETMESSAGE(18, 13, "Point In Window To Make Active"));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        dtwm.pointerTG = XmCreateToggleButtonGadget(dtwm.focusPolicyRC,
                                                    "pointerTG", args, n);
        XmStringFree(string);

        n = 0;
        string = CMPSTR(
            (char *)GETMESSAGE(18, 14, "Click In Window To Make Active"));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        dtwm.explicitTG = XmCreateToggleButtonGadget(dtwm.focusPolicyRC,
                                                     "explicitTG", args, n);
        XmStringFree(string);

        n = 0;
        XtSetArg(args[n], XmNmargin, 0);
        n++;
        dtwm.focusSeparator =
        ...
        n = 0;
        string =
            CMPSTR((char *)GETMESSAGE(18, 15, "Raise Window When Made Active"));
        ...
        n = 0;
        string =
            CMPSTR((char *)GETMESSAGE(18, 11, "Allow Primary Windows On Top"));
        XtSetArg(args[n], XmNnavigationType, XmTAB_GROUP);
        n++;
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        ...
        n = 0;
        string =
            CMPSTR((char *)GETMESSAGE(18, 16, "Show Contents During Move"));
        XtSetArg(args[n], XmNnavigationType, XmTAB_GROUP);
        n++;
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        XtSetArg(args[n], XmNalignment, XmALIGNMENT_BEGINNING);
        n++;
        dtwm.moveOpaqueTG = XmCreateToggleButtonGadget(windowFocusForm,
                                                       "moveOpaqueTG", args, n);
        XmStringFree(string);
        ...
        XtAddCallback(style.dtwmDialog, XmNmapCallback, _DtmapCB_dtwmDlg,
                      shell);
        XtAddCallback(style.dtwmDialog, XmNcallback, ButtonCB, NULL);
        XtAddCallback(dtwm.systemDefault, XmNactivateCallback, systemDefaultCB,
                      NULL);
...
static void formLayoutCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, dtwm.pictLabel);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, dtwm.windowFocusTB);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, dtwm.focusPolicyRC);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, dtwm.focusSeparator);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, dtwm.autoRaiseTG);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing - 3);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        n++;
        XtSetArg(args[n], XmNbottomOffset, style.verticalSpacing);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, dtwm.secStackTG);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing - 3);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
...
/* _DtmapCB_dtwmDlg                        */
/*+++++++++++++++++++++++++++++++++++++++*/
static void _DtmapCB_dtwmDlg(Widget w, XtPointer client_data,
                             XtPointer call_data) {
        ...
        XtRemoveCallback(style.dtwmDialog, XmNmapCallback, _DtmapCB_dtwmDlg,
                         NULL);
}
...
static void ButtonCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        case OK_BUTTON:
                ...
                if (state != dtwm.origFocusAutoRaise || changeFlag) {
                        sprintf(dtwmRes + strlen(dtwmRes),
                                "Dtwm*focusAutoRaise: %s\n",
                ...
                if (changeFlag) {
                        if (dtwm.warnDialog == NULL) {
                                ...
                                n++;
                                XtSetArg(
                                    args[n], XmNdialogTitle,
                                    CMPSTR((char *)GETMESSAGE(2, 2, "Notice")));
                                n++;
                                XtSetArg(args[n], XmNmessageString,
                                         CMPSTR(WARN_MSG));
                                n++;
                                dtwm.warnDialog = XmCreateInformationDialog(
                                    style.shell, "warnDialog", args, n);

                                XtUnmanageChild(XmMessageBoxGetChild(
                                    dtwm.warnDialog, XmDIALOG_HELP_BUTTON));

                                XtAddCallback(dtwm.warnDialog, XmNokCallback,
                                              okWarnCB, client_data);
                                XtAddCallback(dtwm.warnDialog,
                                              XmNcancelCallback, cancelWarnCB,
                                              client_data);
                        }
        ...
        case CANCEL_BUTTON:
                ...
                XmToggleButtonGadgetSetState(
                    dtwm.autoRaiseTG, dtwm.origFocusAutoRaise ? True : False,
                    True);

                XmToggleButtonGadgetSetState(
                    dtwm.secStackTG, dtwm.origSecStack ? False : True, True);

                XmToggleButtonGadgetSetState(dtwm.moveOpaqueTG,
                                             dtwm.origMoveOpaque ? True : False,
                                             True);

                XmToggleButtonGadgetSetState(
                    dtwm.iconBoxTG, dtwm.origUseIconBox ? True : False, True);

                XmToggleButtonGadgetSetState(
                    dtwm.desktopTG, dtwm.origUseIconBox ? False : True, True);

                break;

        case HELP_BUTTON:
                XtCallCallbacks(style.dtwmDialog, XmNhelpCallback,
                                (XtPointer)NULL);
                break;
 ...
 ************************************************************************/
void restoreDtwm(Widget shell, XrmDatabase db) {
        XrmName xrm_name[5];
...
static void cancelWarnCB(Widget w, XtPointer client_data, XtPointer call_data) {
        XmToggleButtonGadgetSetState(
            dtwm.pointerTG, dtwm.origKeyboardFocusPolicy ? POINTER : EXPLICIT,
            True);

        XmToggleButtonGadgetSetState(
            dtwm.explicitTG, dtwm.origKeyboardFocusPolicy ? EXPLICIT : POINTER,
            True);

        XmToggleButtonGadgetSetState(
            dtwm.autoRaiseTG, dtwm.origFocusAutoRaise ? True : False, True);
...
static void okWarnCB(Widget w, XtPointer client_data, XtPointer call_data) {
        dtwm.origKeyboardFocusPolicy =
            XmToggleButtonGadgetGetState(dtwm.explicitTG);
        dtwm.origFocusAutoRaise =
            XmToggleButtonGadgetGetState(dtwm.autoRaiseTG);
