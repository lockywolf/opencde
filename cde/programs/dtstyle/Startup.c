...
static void MapStartup(Widget w, XtPointer client_data, XtPointer call_data);
static void LayoutCB(Widget w, XtPointer client_data, XtPointer call_data);
static void SetCurrentCB(Widget w, XtPointer client_data,
                         XmToggleButtonCallbackStruct *call_data);
static void SetHomeCB(Widget w, XtPointer client_data,
                      XmToggleButtonCallbackStruct *call_data);
static void SetConfirmCB(Widget w, XtPointer client_data,
                         XmToggleButtonCallbackStruct *call_data);
static void SetHomeSession(Widget w, XtPointer client_data,
                           XtPointer call_data);
static void SaveHomeOk(Widget w, XtPointer client_data, XtPointer call_data);
...
/* popup_startupBB                       */
/*+++++++++++++++++++++++++++++++++++++++*/
void popup_startupBB(Widget shell) {
...
since the session cannot be restored.")),
                                    style.startupDialog ? style.startupDialog
                                                        : style.shell,
                                    False);
...
static Widget BuildStartup(Widget shell) {
        ...
        int count = 0;
        int count2 = 0;
        Widget widget_list[5], widget_list2[3], modeForm, shutdownRC;
        ...
        save.poscnt++;
        style.startupDialog = __DtCreateDialogBoxDialog(
            shell, "startupDlg", save.posArgs, save.poscnt);
        ...
        XtSetArg(args[n], XmNtitle,
                 ((char *)GETMESSAGE(7, 14, "Style Manager - Startup")));
        n++;
        ...
        n++;
        startup.startupForm =
            XmCreateForm(style.startupDialog, "startupForm", args, n);
        ...
        n++;
        XtSetArg(args[n], XmNimageName, STARTUP_ICON);
        n++;
        ...
        if (style.smState.smDisplaySpecific) {
                ...
                n = 0;
                string = CMPSTR(pch2);
                XtSetArg(args[n], XmNleftWidget, widget_list[count - 1]);
                n++;
                XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
                n++;
                XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
                n++;
                XtSetArg(args[n], XmNmarginLeft, 80);
                n++;
                XtSetArg(args[n], XmNlabelString, string);
                n++;
                widget_list[count++] = XmCreateLabelGadget(
                    startup.startupForm, "sessionLabel", args, n);
                XmStringFree(string);
                XtFree(pch2);
        }
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, widget_list[count - 1]);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
        ...
        n++;
        shutdownRC =
            XmCreateRadioBox(widget_list[count - 1], "shutdownRC", args, n);
        ...
        n++;
        string =
            CMPSTR(((char *)GETMESSAGE(7, 7, "Logout Confirmation Dialog:")));
        XtSetArg(args[n], XmNtitleString, string);
        n++;
        widget_list[count++] =
            _DtCreateTitleBox(startup.startupForm, "modeBox", args, n);
        XmStringFree(string);
        ...
        n++;
        modeForm =
            XmCreateRadioBox(widget_list[count - 1], "modeForm", args, n);
        ...
        startup.modeYesTgl =
            XmCreateToggleButtonGadget(modeForm, "modeYesTgl", args, n);
        XmStringFree(string);
        ...
        startup.modeNoTgl =
            XmCreateToggleButtonGadget(modeForm, "modeNoTgl", args, n);
        XmStringFree(string);
...
static void LayoutCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (XtWidth(startup.startupForm) <
            (Dimension)(XtWidth(startup.setHome) +
...
static void SetHomeSession(Widget w, XtPointer client_data,
        ...
        if (startup.dlg == NULL) {
                string = CMPSTR(
                    ((char *)GETMESSAGE(7, 11,
                                        "This will replace your home session "
                                        "with\nthe current session.\n\n        "
                                        "           Continue?")));
                ...
                XtSetArg(args[n], XmNokLabelString, okstr);
                n++;
                XtSetArg(args[n], XmNcancelLabelString, cancelstr);
                n++;
                XtSetArg(args[n], XmNmwmFunctions, DIALOG_MWM_FUNC);
                n++;
                startup.dlg = XmCreateWarningDialog(shell, "QNotice", args, n);
                XtUnmanageChild(
                    XmMessageBoxGetChild(startup.dlg, XmDIALOG_HELP_BUTTON));
...
static void ButtonCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        case CANCEL_BUTTON:
                ...
                if (startup.origStartState == DtSM_CURRENT_STATE)
                        XmToggleButtonGadgetSetState(startup.resumeTgl, True,
                                                     True);

                else if (startup.origStartState == DtSM_HOME_STATE)
                        XmToggleButtonGadgetSetState(startup.homeTgl, True,
                                                     True);
                ...
                if (startup.origConfirmMode == DtSM_VERBOSE_MODE)
                        XmToggleButtonGadgetSetState(startup.modeYesTgl, True,
                                                     True);
                else
                        XmToggleButtonGadgetSetState(startup.modeNoTgl, True,
                                                     True);
        ...
        case HELP_BUTTON:
                XtCallCallbacks(style.startupDialog, XmNhelpCallback,
                                (XtPointer)NULL);
                break;
 ...
 ************************************************************************/
void restoreStartup(Widget shell, XrmDatabase db) {
        XrmName xrm_name[5];
