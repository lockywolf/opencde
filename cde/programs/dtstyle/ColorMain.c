...
#define STR1                                                                   \
        ((char *)GETMESSAGE(                                                   \
            ...
            "palette will overwrite the old one.\nIs this what you "           \
            "want to do?"))
...
#define CANT_DELETE                                                            \
        ((char *)GETMESSAGE(14, 18, "Can't delete the last palette.\n"))
#define COLORUSE_WHEN                                                          \
        ((char *)GETMESSAGE(14, 27,                                            \
...
static void selectPaletteCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void selectColorCB(Widget w, XtPointer client_data, XtPointer call_data);
static void timeoutCB(XtPointer client_data, XtIntervalId *id);
static void addPaletteCB(Widget w, XtPointer client_data, XtPointer call_data);
static void addCancelCB(Widget w, XtPointer client_data, XtPointer call_data);
static void addOkCB(Widget w, XtPointer client_data, XtPointer call_data);
static void setDlgOkCB(Widget w, XtPointer client_data, XtPointer call_data);
static void modifyColorCB(Widget w, XtPointer client_data, XtPointer call_data);
static void dialogBoxCB(Widget w, XtPointer client_data, XtPointer call_data);
static void AddName(palette *newPalette);
static void deletePaletteCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void deleteOkCB(Widget w, XtPointer client_data, XtPointer call_data);
static void deleteCancelCB(Widget w, XtPointer client_data,
                           XtPointer call_data);
static void resourcesCB(Widget w, XtPointer client_data, XtPointer call_data);
static void colorUseCB(Widget w, XtPointer client_data, XtPointer call_data);
static void colorUseExitCB(Widget w, XtPointer client_data,
                           XtPointer call_data);
static void activateCBexitColor(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void _DtmapCB(Widget w, XtPointer client_data, XtPointer call_data);
static void _DtmapCB_colorUse(Widget w, XtPointer client_data,
                              XtPointer call_data);
...
#define MAXLOCALE 64 /* buffer size of locale name */
        ...
        pl_desc = (char *)XtMalloc(strlen("/usr/dt/palettes/desc.") +
                                   strlen(lang) + 1);
        ...
        pl_desc = (char *)XtMalloc(strlen("/etc/dt/palettes/desc.") +
                                   strlen(lang) + 1);
        ...
        pl_desc = (char *)XtMalloc(strlen(style.home) +
                                   (strlen("/.dt/palettes/desc.palettes") + 1));
        ...
        /* load the palette description data base regardless of locale for later
         * use*/
        pl_desc = (char *)XtMalloc(strlen(style.home) +
                                   (strlen("/.dt/palettes/desc.palettes") + 1));
        strcpy(pl_desc, style.home);
        ...
        */
        ...
        if (style.colorSrv == FALSE) {
                ErrDialog(
                    ((char *)GETMESSAGE(
                        14, 25,
                        "The color portion of the Style Manager\nwill not "
                        "operate because the color server\nis not running. "
                        " Check $HOME/.dt/errorlog.")),
                    style.shell);
                return;
        }

        /* Not if useColorObj resource is False.  XmeUseColorObj will return
           false if color server is not running or if the resource UseColorObj
           is False. If the color server is not running, that case is caught
           above so if we get here, the resource UseColorObj must be False.
           This is an undocumented resource. */

        if (XmeUseColorObj() == FALSE) {
                ErrDialog(((char *)GETMESSAGE(
                              14, 26,
                              "The color portion of the Style "
                              "Manager\nwill not operate because the "
        ...
        /* also determine if system uses long filenames, used by add palette
         * only */
        CheckFileType();
...
static int CreateMainWindow(Widget parent) {
        ...
        if (NumOfPalettes == 0) {
                ...
                ErrDialog(
                    ((char *)GETMESSAGE(
                        14, 38, "The color portion of the Style Manager\n\
...
void CreatePaletteButtons(Widget parent) {
        register int i, n;
        Arg args[16];
        XmString string;
        Pixmap pixmap100;
        ...
        if (TypeOfMonitor ==
            XmCO_BLACK_WHITE) /* create pixmaps for top/bottom shadow */
        {
        ...
        pixmap100 = XmGetPixmap(
            style.screen, "background", BlackPixelOfScreen(style.screen),
        ...
        n++;
        XtSetArg(args[n], XmNrightPosition, PALETTE_RC_RIGHT_POSITION);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNmarginWidth, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNmarginHeight, style.verticalSpacing);
        n++;
        ...
        for (i = 0; i < XmCO_MAX_NUM_COLORS; i++) {
                ...
                n++;
                XtSetArg(args[n], XmNwidth, COLOR_BUTTON_WIDTH);
                n++;
                XtSetArg(args[n], XmNheight, COLOR_BUTTON_HEIGHT);
                n++;
                ...
                XtSetArg(args[n], XmNborderWidth, BORDER_WIDTH);
                n++;
                XtSetArg(args[n], XmNborderColor,
                         pCurrentPalette->color[pCurrentPalette->secondary]
                             .bg.pixel);
                n++;
                ...
                n++;
                XtSetArg(args[n], XmNmultiClick, XmMULTICLICK_KEEP);
                n++;
                if (TypeOfMonitor == XmCO_LOW_COLOR) {
                        XtSetArg(
                            args[n], XmNhighlightColor,
                            pCurrentPalette->color[pCurrentPalette->secondary]
                                .fg.pixel);
                        n++;
                } else {
...
Boolean InitializePaletteList(Widget shell, Widget list,
...
#endif
{
        ...
        while (loop_palette != NULL) {
                if (pl_DB != NULL) {
                        ...
                        class_str = (char *)XtMalloc(
                            strlen("Palettes.") + strlen(loop_palette->name) +
                            strlen(".Desc") + 1);
                        ...
                        if (XrmGetResource(pl_DB, name_str, class_str,
                                           &str_type_return, &value_return)) {
                                loop_palette->desc = (char *)XtMalloc(
                                    strlen(value_return.addr) + 1);
                                strcpy(loop_palette->desc, value_return.addr);
                        } else {
                                loop_palette->desc = (char *)XtMalloc(
                                    strlen(loop_palette->name) + 1);
                        ...
                        loop_palette->desc =
                            (char *)XtMalloc(strlen(loop_palette->name) + 1);
                        strcpy(loop_palette->desc, loop_palette->name);
                }

                XmListAddItem(list, CMPSTR(loop_palette->desc),
                              loop_palette->item_position);
        ...
        /*
         **  Make the palette named by the color Server the selected palette, if
         *the
        ...
        n = 0;
        XtSetArg(args[n], XmNselectedItemCount, 1);
        n++;
        XtSetArg(args[n], XmNselectedItems, string_table);
        n++;
...
static void allocNewColors(void) {
        ...
        for (i = 0; i < pCurrentPalette->num_of_colors; i++) {
                ...
                } else /* create pixmaps for top/bottom shadow */
                {
                        ...
                        edit.pixmap25 =
                            XmGetPixmap(style.screen, "50_foreground",
                                        pCurrentPalette->color[i].bg.pixel,
                                        WhitePixelOfScreen(style.screen));
            ...
            selected_position != cb->item_position) {
                ...
                if (tmp_palette->item_position == selected_position) {
                        ...
                        if (style.dynamicColor)
                                ...
                                if (TypeOfMonitor != XmCO_BLACK_WHITE) {
                                        /* PUT DIALOG saying can't dynamically
                                         * change */
                                        if (First) {
                                                InfoDialog(NEXT_SESSION,
                                                           style.colorDialog,
                                                           False);
                                                First = False;
                                        }
                                ...
                                {
                                        /* set color buttons for new palette -
                                         * read only cells */
                                        /* primary=color[1] secondary=color[0]
                                         */
                                        if (strcmp(pCurrentPalette->name,
                                                   WHITE_BLACK) == 0) {
                                                ...
                                                XtSetArg(args[n], XmNarmColor,
                                                         white);
                                                n++;
                                                XtSetValues(colorButton[0],
                                                            args, n);
                                                pCurrentPalette->color[0]
                                                    .fg.pixel = black;
                                                pCurrentPalette->color[0]
                                                    .bg.pixel = white;
                                                pCurrentPalette->color[0]
                                                ...
                                                XtSetArg(args[n], XmNarmColor,
                                                         black);
                                                n++;
                                                XtSetValues(colorButton[1],
                                                            args, n);
                                                pCurrentPalette->color[1]
                                                    .fg.pixel = white;
                                                pCurrentPalette->color[1]
                                                    .bg.pixel = black;
                                                pCurrentPalette->color[1]
                                                ...
                                                pCurrentPalette->color[1]
                                                    .bs.pixel = white;
                                        } else if (strcmp(pCurrentPalette->name,
                                                          BLACK_WHITE) == 0) {
...
static void selectColorCB(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        intptr_t i;
        int n;
        Arg args[4];
        ...
        if ((edit.DialogShell == NULL) || (!XtIsManaged(edit.DialogShell))) {
                ...
                n = 0;
                XtSetArg(args[n], XmNbackground, &bg_pixel);
                n++;
                ...
                color_set =
                    (ColorSet *)&pCurrentPalette->color[selected_button];
                ColorEditor(style.colorDialog, color_set);

                if (!style.dynamicColor) /* need to update pixels */
                        XtAddCallback(edit.DialogShell, XmNcallback,
                                      colorEditorCB, NULL);
        }
}
 ...
 */
static void colorEditorCB(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        static Boolean first = True;
...
static void addPaletteCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (addDialog == NULL) {
                ...
                XtSetArg(args[n], XmNokLabelString,
                         CMPSTR((String)_DtOkString));
                n++;
                XtSetArg(args[n], XmNcancelLabelString,
                         CMPSTR((String)_DtCancelString));
                n++;
                XtSetArg(args[n], XmNhelpLabelString,
                         CMPSTR((String)_DtHelpString));
                n++;
                string =
                    CMPSTR(((char *)GETMESSAGE(14, 10, "New palette name:")));
                XtSetArg(args[n], XmNselectionLabelString, string);
                n++;
                string1 = CMPSTR("");
                XtSetArg(args[n], XmNtextString, string1);
                n++;
                XtSetArg(args[n], XmNborderWidth, 3);
                n++;
                ...
                XtAddCallback(addDialog, XmNhelpCallback,
                              ...
                              (XtPointer)HELP_ADD_PALETTE_DIALOG);
                ...
                XtSetArg(args[n], XmNtitle,
                         ((char *)GETMESSAGE(14, 11, "Add Palette")));
                n++;
...
static void addOkCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        name = XmTextFieldGetString(
            XmSelectionBoxGetChild(addDialog, XmDIALOG_TEXT));
        ...
        /* make sure the length of name is ok, short file names can only be 11
         * chars
         */
        ...
        /* set the previous last palatte to this new one, it is now the last
         * one*/
        ...
        newPalette->directory =
            (char *)XtMalloc(strlen(style.home) + strlen(DT_PAL_DIR) + 1);
        ...
        for (i = 0; i < XmCO_MAX_NUM_COLORS; i++) {
                newPalette->color[i].fg.pixel =
                    pCurrentPalette->color[i].fg.pixel;
                newPalette->color[i].fg.red = pCurrentPalette->color[i].fg.red;
                newPalette->color[i].fg.green =
                    pCurrentPalette->color[i].fg.green;
                newPalette->color[i].fg.blue =
                    pCurrentPalette->color[i].fg.blue;
                newPalette->color[i].fg.flags =
                    pCurrentPalette->color[i].fg.flags;

                newPalette->color[i].bg.pixel =
                    pCurrentPalette->color[i].bg.pixel;
                newPalette->color[i].bg.red = pCurrentPalette->color[i].bg.red;
                newPalette->color[i].bg.green =
                    pCurrentPalette->color[i].bg.green;
                newPalette->color[i].bg.blue =
                    pCurrentPalette->color[i].bg.blue;
                newPalette->color[i].bg.flags =
                    pCurrentPalette->color[i].bg.flags;

                newPalette->color[i].ts.pixel =
                    pCurrentPalette->color[i].ts.pixel;
                newPalette->color[i].ts.red = pCurrentPalette->color[i].ts.red;
                newPalette->color[i].ts.green =
                    pCurrentPalette->color[i].ts.green;
                newPalette->color[i].ts.blue =
                    pCurrentPalette->color[i].ts.blue;
                newPalette->color[i].ts.flags =
                    pCurrentPalette->color[i].ts.flags;

                newPalette->color[i].bs.pixel =
                    pCurrentPalette->color[i].bs.pixel;
                newPalette->color[i].bs.red = pCurrentPalette->color[i].bs.red;
                newPalette->color[i].bs.green =
                    pCurrentPalette->color[i].bs.green;
                newPalette->color[i].bs.blue =
                    pCurrentPalette->color[i].bs.blue;
                newPalette->color[i].bs.flags =
                    pCurrentPalette->color[i].bs.flags;

                newPalette->color[i].sc.pixel =
                    pCurrentPalette->color[i].sc.pixel;
                newPalette->color[i].sc.red = pCurrentPalette->color[i].sc.red;
                newPalette->color[i].sc.green =
                    pCurrentPalette->color[i].sc.green;
                newPalette->color[i].sc.blue =
                    pCurrentPalette->color[i].sc.blue;
                newPalette->color[i].sc.flags =
                    pCurrentPalette->color[i].sc.flags;
        }
...
static void setDlgOkCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        tmpPalette->directory =
            (char *)XtMalloc(strlen(style.home) + strlen(DT_PAL_DIR) + 1);
        strcpy(tmpPalette->directory, style.home);
        strcat(tmpPalette->directory, DT_PAL_DIR);

        for (i = 0; i < XmCO_MAX_NUM_COLORS; i++) {
                tmpPalette->color[i].fg.pixel =
                    pCurrentPalette->color[i].fg.pixel;
                tmpPalette->color[i].fg.red = pCurrentPalette->color[i].fg.red;
                tmpPalette->color[i].fg.green =
                    pCurrentPalette->color[i].fg.green;
                tmpPalette->color[i].fg.blue =
                    pCurrentPalette->color[i].fg.blue;
                tmpPalette->color[i].fg.flags =
                    pCurrentPalette->color[i].fg.flags;

                tmpPalette->color[i].bg.pixel =
                    pCurrentPalette->color[i].bg.pixel;
                tmpPalette->color[i].bg.red = pCurrentPalette->color[i].bg.red;
                tmpPalette->color[i].bg.green =
                    pCurrentPalette->color[i].bg.green;
                tmpPalette->color[i].bg.blue =
                    pCurrentPalette->color[i].bg.blue;
                tmpPalette->color[i].bg.flags =
                    pCurrentPalette->color[i].bg.flags;

                tmpPalette->color[i].ts.pixel =
                    pCurrentPalette->color[i].ts.pixel;
                tmpPalette->color[i].ts.red = pCurrentPalette->color[i].ts.red;
                tmpPalette->color[i].ts.green =
                    pCurrentPalette->color[i].ts.green;
                tmpPalette->color[i].ts.blue =
                    pCurrentPalette->color[i].ts.blue;
                tmpPalette->color[i].ts.flags =
                    pCurrentPalette->color[i].ts.flags;

                tmpPalette->color[i].bs.pixel =
                    pCurrentPalette->color[i].bs.pixel;
                tmpPalette->color[i].bs.red = pCurrentPalette->color[i].bs.red;
                tmpPalette->color[i].bs.green =
                    pCurrentPalette->color[i].bs.green;
                tmpPalette->color[i].bs.blue =
                    pCurrentPalette->color[i].bs.blue;
                tmpPalette->color[i].bs.flags =
                    pCurrentPalette->color[i].bs.flags;

                tmpPalette->color[i].sc.pixel =
                    pCurrentPalette->color[i].sc.pixel;
                tmpPalette->color[i].sc.red = pCurrentPalette->color[i].sc.red;
                tmpPalette->color[i].sc.green =
                    pCurrentPalette->color[i].sc.green;
                tmpPalette->color[i].sc.blue =
                    pCurrentPalette->color[i].sc.blue;
                tmpPalette->color[i].sc.flags =
                    pCurrentPalette->color[i].sc.flags;
        }
...
static void modifyColorCB(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        ColorSet *color_set;
...
static void dialogBoxCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        case CANCEL_BUTTON:
                ...
                if ((addDialog != NULL) && (XtIsManaged(addDialog)))
                        XtCallCallbacks(addDialog, XmNcancelCallback,
                                        (XtPointer)NULL);

                if ((deleteDialog != NULL) && (XtIsManaged(deleteDialog)))
                        XtCallCallbacks(deleteDialog, XmNcancelCallback,
                                        (XtPointer)NULL);

                if ((colorUseDialog != NULL) && (XtIsManaged(colorUseDialog)))
                        XtCallCallbacks(colorUseDialog, XmNcallback,
                                        (XtPointer)NULL);

                XtUnmanageChild(style.colorDialog);

                tmp_palette = pHeadPalette;
                for (tmp_palette = pHeadPalette; tmp_palette != NULL;
                     tmp_palette = tmp_palette->next)
...
static void deletePaletteCB(Widget w, XtPointer client_data,
        ...
        if (deleteDialog == NULL) {
                ...
                n++;
                XtSetArg(args[n], XmNtitle,
                         ((char *)GETMESSAGE(14, 16, "Delete Palette")));
                n++;
                XtSetArg(args[n], XmNmwmFunctions, DIALOG_MWM_FUNC);
                n++;
        ...
        XtSetArg(args[n], XmNmessageString, CMPSTR(tmpStr));
        n++;
...
static void resourcesCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (colorUseDialog == NULL) {
                ...
                n++;
                XtSetArg(args[n], XmNbuttonCount, NUM_LABELS);
                n++;
                XtSetArg(args[n], XmNbuttonLabelStrings, button_string);
                n++;
                XtSetArg(args[n], XmNdefaultPosition, False);
                n++;
                colorUseDialog = __DtCreateDialogBoxDialog(
                    parent, "colorUseDialog", args, n);
                XtAddCallback(colorUseDialog, XmNcallback, colorUseExitCB,
                              NULL);
                XtAddCallback(colorUseDialog, XmNmapCallback, _DtmapCB_colorUse,
                              NULL);
                ...
                XtSetArg(args[n], XmNautoUnmanage, False);
                n++;
                XtSetArg(args[n], XmNcancelButton, widget_list[0]);
                n++;
                ...
                XtSetArg(
                    args[n], XmNtitle,
                    ((char *)GETMESSAGE(14, 39, "Number Of Colors To Use:")));
                n++;
                XtSetArg(args[n], XmNmwmFunctions, DIALOG_MWM_FUNC);
                n++;
                ...
                n++;
                XtSetArg(args[n], XmNverticalSpacing, style.verticalSpacing);
                n++;
                ...
                n++;
                XtSetArg(args[n], XmNpixmapForeground, style.primBSCol);
                n++;
                XtSetArg(args[n], XmNpixmapBackground, style.primTSCol);
                n++;
                XtSetArg(args[n], XmNimageName, COLOR_ICON);
                n++;
                XtSetArg(args[n], XmNtraversalOn, False);
                n++;
                ...
                n++;
                XtSetArg(args[n], XmNtopWidget, pictLabel);
                n++;
                ...
                n = 0;
                colorUseRC =
                    XmCreateRadioBox(colorUseTB, "colorUseRC", args, n);
                XtManageChild(colorUseRC);

                n = 0;
                string = CMPSTR(
                    ((char *)GETMESSAGE(14, 41, "More Colors for Desktop")));
                XtSetArg(args[n], XmNlabelString, string);
                n++;
                widget_list[count++] = colorDialog.highColorTG =
                    XmCreateToggleButtonGadget(colorUseRC, "highColorTG", args,
                                               n);
                ...
                n = 0;
                string = CMPSTR(((char *)GETMESSAGE(
                    14, 42, "More Colors for Applications")));
                XtSetArg(args[n], XmNlabelString, string);
                n++;
                widget_list[count++] = colorDialog.mediumColorTG =
                    XmCreateToggleButtonGadget(colorUseRC, "mediumColorTG",
                                               args, n);
                XmStringFree(string);
                XtAddCallback(colorDialog.mediumColorTG,
                              XmNvalueChangedCallback, colorUseCB,
                              (XtPointer)XmCO_MEDIUM_COLOR);

                n = 0;
                string = CMPSTR(((char *)GETMESSAGE(
                    14, 43, "Most Colors for Applications")));
                XtSetArg(args[n], XmNlabelString, string);
                n++;
                widget_list[count++] = colorDialog.lowColorTG =
                    XmCreateToggleButtonGadget(colorUseRC, "lowColorTG", args,
                                               n);
                XmStringFree(string);
                ...
                n = 0;
                string =
                    CMPSTR(((char *)GETMESSAGE(14, 34, "Black and White")));
                XtSetArg(args[n], XmNlabelString, string);
                n++;
                widget_list[count++] = colorDialog.blackWhiteTG =
                    XmCreateToggleButtonGadget(colorUseRC, "blackWhiteTG", args,
                                               n);
                XmStringFree(string);
                ...
                widget_list[count++] = colorDialog.defaultTG =
                    XmCreateToggleButtonGadget(colorUseRC, "defaultTG", args,
                                               n);
                XmStringFree(string);
...
static void colorUseExitCB(Widget w, XtPointer client_data,
        ...
        switch (cb->button_position) {
        case HELP_BUTTON:
                XtCallCallbacks(colorUseDialog, XmNhelpCallback,
                                (XtPointer)NULL);
                break;
        ...
        case OK_BUTTON:
                ...
                if (colorDialog.origColorUse != colorDialog.currentColorUse) {
                        ...
                        /* remove ColorUse specification from database for
                         * DEFAULT_COLOR */

                        sprintf(colorUseRes, "*%d*ColorUse: %s\n",
                                style.screenNum,
                                colorDialog.currentColorUseStr);
...
static void _DtmapCB(Widget w, XtPointer client_data,
...
static void _DtmapCB_colorUse(Widget w, XtPointer client_data,
        ...
        if (status = XrmGetResource(db, instanceString, nameString,
                                    &str_type_return, &value_return)) {
        ...
        XtRemoveCallback(colorUseDialog, XmNmapCallback, _DtmapCB_colorUse,
                         NULL);
}
 ...
 *
 ************************************************************************/
void DeletePaletteFromLinkList(Widget list) {
        register int n;
        Arg args[2];
        int i;
        XmString string;
...
void CopyPixel(ColorSet srcPixels[XmCO_MAX_NUM_COLORS],
            ...
            tmp_palette->item_position == OrgPalette.item_position) {
                ...
                for (i = 0; i < XmCO_MAX_NUM_COLORS; i++) {
                        ...
                        if (i < OrgPalette.num_of_colors &&
                            TypeOfMonitor != XmCO_BLACK_WHITE)
...
void UpdateDefaultPalette(void) {
        int i;
        ...
        for (i = 0; i < XmCO_MAX_NUM_COLORS; i++) {
                ...
                if (TypeOfMonitor == XmCO_LOW_COLOR && (i == 0 || i == 1))
                        sprintf(temp[i], "#%04x%04x%04x\n", saved_color[i].red,
                                saved_color[i].green, saved_color[i].blue);
...
void show_selection(Widget w, XtPointer client_data, Atom *selection,
        ...
        if (value != NULL) {
                if ((intptr_t)client_data == GET_TYPE_MONITOR) {
                        sscanf((char *)value, "%x_%x_%x_%x",
                               (unsigned int *)&(TypeOfMonitor),
                               (unsigned int *)&(UsePixmaps),
 ...
 ************************************************************************/
void restoreColor(Widget shell, XrmDatabase db) {
        XrmName xrm_name[5];
        XrmRepresentation rep_type;
        XrmValue value;
        palette *tmp_palette;

        /*"paletteDlg" is the resource name of the dialog shell we are saving
         * for.*/
 ...
 *
 **************************************************************************/
void SameName(Widget w, palette *tmpPalette, char *name) {
        char *tmpStr;
        int n = 0;
        Arg args[10];
        ...
        if (colorDialog.dlg == NULL) {
                ...
                XtSetArg(args[n], XmNdialogTitle,
                         CMPSTR(((char *)GETMESSAGE(14, 21, "Warning"))));
                n++;
                colorDialog.dlg = XmCreateWarningDialog(style.colorDialog,
                                                        "QNotice", args, n);
                XtAddCallback(colorDialog.dlg, XmNokCallback, setDlgOkCB,
                              (XtPointer)tmpPalette);
                XtAddCallback(colorDialog.dlg, XmNhelpCallback,
                              (XtCallbackProc)HelpRequestCB,
                              (XtPointer)HELP_ADD_PALETTE_WARNING_DIALOG);
...
void GetDefaultPal(Widget shell) {
        char *str_type_return;
        XrmValue value_return;
        XrmDatabase db;
        Boolean status;
        ...
        if (TypeOfMonitor == XmCO_BLACK_WHITE) {
                sprintf(instanceName, "dtsession.%d.monochromePalette",
                        style.screenNum);
                sprintf(instanceClass, "Dtsession.%d.MonochromePalette",
                        style.screenNum);
        } else {
                sprintf(instanceName, "dtsession.%d.colorPalette",
                        style.screenNum);
                sprintf(instanceClass, "Dtsession.%d.ColorPalette",
                        style.screenNum);
        }
...
void CreateDialogBoxD(Widget parent) {
        XmString button_string[NUM_LABELS];
        Arg args[2];
        int n;
        Widget w;
        ...
        XtSetArg(save.posArgs[save.poscnt], XmNchildType, XmWORK_AREA);
        save.poscnt++;
        ...
        save.poscnt++;
        style.colorDialog = __DtCreateDialogBoxDialog(
            parent, PALETTEDLG, save.posArgs, save.poscnt);
        ...
        XtSetArg(args[n], XmNautoUnmanage, False);
        n++;
        XtSetArg(args[n], XmNcancelButton, w);
        n++;
...
void AddToDialogBox(void) {
        ...
        n = 0;
        XtSetArg(args[n], XmNtitle,
                 ((char *)GETMESSAGE(14, 30, "Style Manager - Color")));
        n++;
        XtSetArg(args[n], XmNuseAsyncGeometry, True);
        n++;
        XtSetArg(args[n], XmNmwmFunctions, DIALOG_MWM_FUNC);
        ...
        */
        n = 0;
        XtSetArg(args[n], XmNhorizontalSpacing, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNverticalSpacing, style.verticalSpacing);
        n++;
        colorDialog.colorForm =
            XmCreateForm(style.colorDialog, "colorForm", args, n);
        XtManageChild(colorDialog.colorForm);

        style.count++;
}

void CreateTopColor1(void) {
        register int n;
        Arg args[6];
        XmString string;
        ...
        */
        ...
        XtSetArg(args[n], XmNtitleString, string);
        n++;
        colorDialog.paletteTB =
            _DtCreateTitleBox(colorDialog.colorForm, "paletteTB", args, n);
        ...
        */
        ...
        XtSetArg(args[n], XmNverticalSpacing, style.verticalSpacing);
        n++;
        colorDialog.palettesForm =
            XmCreateForm(colorDialog.paletteTB, "palettesForm", args, n);
        XtManageChild(colorDialog.palettesForm);

        style.count++;
}

void CreateTopColor2(void) {
        register int n;
        Arg args[8];
        ...
        */
        ...
        n++;
        XtSetArg(args[n], XmNvisibleItemCount, 6);
        n++;
        paletteList = XmCreateScrolledList(colorDialog.palettesForm,
                                           "paletteList", args, n);
        XtAddCallback(paletteList, XmNbrowseSelectionCallback, selectPaletteCB,
                      NULL);

        n = 0;
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
...
void CreateBottomColor(void) {
        ...
        if (TypeOfMonitor != XmCO_BLACK_WHITE) {
                ...
                n++;
                XtSetArg(args[n], XmNleftWidget, paletteList);
                n++;
                XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
                n++;
                ...
                XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
                n++;
                XtSetArg(args[n], XmNtopOffset, ADD_PALETTE_TOP_OFFSET);
                n++;
                XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
                n++;
                XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
                ...
                XtAddCallback(colorDialog.addPaletteButton, XmNactivateCallback,
                              addPaletteCB, (XtPointer)NULL);

                /* Create Delete button */
                n = 0;
                XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
                n++;
                XtSetArg(args[n], XmNtopWidget, colorDialog.addPaletteButton);
                n++;
                XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
                n++;
                XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        ...
        */
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, colorDialog.paletteTB);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNtitleString, string);
        n++;
        colorDialog.buttonsTB =
            _DtCreateTitleBox(colorDialog.colorForm, "ButtonsTB", args, n);
        ...
        n++;
        style.buttonsForm =
            XmCreateForm(colorDialog.buttonsTB, "buttonsForm", args, n);
        ...
        if (TypeOfMonitor != XmCO_BLACK_WHITE) {
                ...
                XtSetArg(args[n], XmNtopOffset,
                         style.horizontalSpacing + BORDER_WIDTH);
                n++;
                ...
                XtSetArg(args[n], XmNlabelString, string);
                n++;
                modifyColorButton = XmCreatePushButtonGadget(
                    style.buttonsForm, "modifyColorButton", args, n);
                XmStringFree(string);
                XtManageChild(modifyColorButton);
                XtAddCallback(modifyColorButton, XmNactivateCallback,
                              modifyColorCB, (XtPointer)NULL);
        }
        /*
        ...
        */
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, colorDialog.buttonsTB);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
