...
#define ERR9                                                                   \
        ((char *)GETMESSAGE(                                                   \
            16, 18,                                                            \
            "Warning, Too many directories listed in the resource "            \
            "paletteDirectories,\n Maximum number is %d."))
...
static void SetColors(int num_of_colors, palette *new_palette);
static void ReadInInfo(char *buf, int nbytes, palette *new_palette);
static void InitializeBW(unsigned long color, int num_of_colors,
                         palette *new_palette);
...
*/
void ReadPalette(char *directory, char *in_filename,
...
#endif
{
        ...
        if ((fd = open(filename, O_RDONLY)) == -1) {
                ...
                *p = '\0';
                ...
                /*
                ** Now we have a name lets check to see if it is already in the
                *linked
                ** list.  If it is and it is from the system palettes, replace
                *it.
                ** If it is already in the list but is not from the system
                *palettes,
                ** don't overwrite it - discard the one just read and go on to
                *next palette.
                */
                if (pHeadPalette != NULL) {
                        ...
                        while (tmp_palette->next != NULL) {
                                if (strcmp(tmp_palette->name,
                                           new_palette->name))
                                        tmp_palette = tmp_palette->next;
                                else {
                                        found = TRUE;
                                        if ((strcmp(tmp_palette->directory,
                                                    PALETTE_DIR) == 0) ||
                                            (strcmp(directory,
                                                    home_palette_dir) == 0)) {
                ...
                if ((modify == TRUE) || (add == TRUE)) {
                        ...
                        new_palette->directory =
                            (char *)XtMalloc(strlen(directory) + 1);
        ...
        /*
        **  Find the next available location within the linked list of palettes
        *to
        **  store the palette just created.  If it is the first one store it in
        **  pHeadPalettes, else store it in the first place a NULL is
        *encountered
        ...
        */
        if (add == TRUE) {
                ...
                if (pHeadPalette == NULL /* First entry */
                    || (pHeadPalette != NULL &&
                        strcmp(pHeadPalette->name, new_palette->name) >
                            0)) /* Earlier entry than current list head */
                {
                        new_palette->item_position = 1;
                        if (pHeadPalette) {
                                new_palette->next = pHeadPalette;
                                /* Increment position poineter of other items in
                                 * the list */
                                tmp_palette = pHeadPalette;
                        ...
                        /* Search through the linked list to find the first
                           entry with a name > new entries name, new item will
                           be inserted after it */
                        while (tmp_palette->next &&
                               strcmp(tmp_palette->next->name,
                                      new_palette->name) < 0) {
                                tmp_palette = tmp_palette->next;
                        }
                        ...
                        new_palette->item_position =
                            tmp_palette->item_position + 1;

                        /* Now continue incrementing through the list increasing
                           the position count of all items following the new
                           entry */
...
int WriteOutPalette(char *paletteName) {
        ...
        filename =
            (char *)XtMalloc(strlen(style.home) + strlen(DT_PAL_DIR) +
                             strlen(paletteName) + strlen(PALETTE_SUFFIX) + 2);
...
#endif
        {
                ...
                for (i = 0; i < XmCO_MAX_NUM_COLORS; i++) {
                        /* put the colors of the palette in the form
                         * #RRRRGGGGBBBB */
...
int WriteOutDesc(palette *tmp_palette) {
        ...
        /* write the description resource to the user's home data base and write
         * out
         */
        /* the database to the user's palatte directory */

        desc_name =
            (char *)XtMalloc(strlen("Palettes*") + strlen(tmp_palette->name) +
                             strlen(".desc") + 1);
        ...
        desc_file = (char *)XtMalloc(
            strlen(style.home) + (strlen("/.dt/palettes/desc.palettes") + 1));
...
Boolean RemovePalette(void) {
        ...
        if (strcmp(pCurrentPalette->directory, home_palette_dir) == 0) {
                /* get the $HOME environment varible and constuct the full file
                 * name */
                filename2 = (char *)XtMalloc(
                    strlen(style.home) + strlen(DT_PAL_DIR) +
                ...
                if (result != 0) {
                        ...
                        _DtSimpleErrnoError(progName, DtWarning, NULL, tmpStr,
                                            NULL);
                        XtFree(tmpStr);

                        tmpStr = (char *)XtMalloc(
                            strlen(ERR1) + strlen(pCurrentPalette->name) +
                            strlen(ERR2) + 1);
                        sprintf(tmpStr, ERR1, pCurrentPalette->name);
                        strcat(tmpStr, ERR2);
                        ErrDialog(tmpStr, style.colorDialog);
...
#endif /* __hpux */
{
        ...
        for (i = 0;
             i < strlen(file_descpt->d_name) - (strlen(PALETTE_SUFFIX) + 1);
             i++)
        ...
        if (tmp_palette->next == NULL &&
            (strcmp(tmp_palette->name, filename) == 0))
...
static void ReadInInfo(char *buf, int nbytes, palette *new_palette) {
        ...
        for (buf_count = 0; buf_count < nbytes;) {
                ...
                result = XParseColor(style.display, style.colormap, tmpbuf,
                                     &tmp_color);

                /* if the result == 0 then the parse came back bad, uses the
                   motif default background (#729FFF) */
                if (result == 0) {
                        ...
                        new_palette->color[num_of_colors].bg.red =
                            tmp_color.red;
                ...
                if (TypeOfMonitor != XmCO_BLACK_WHITE) {
                        ...
                        (*edit.calcRGB)(
                            &tmp_color, &(new_palette->color[num_of_colors].fg),
...
static void SetColors(int num_of_colors, palette *new_palette) {
        ...
        /*
        **  Set the foreground pixel to either black or white depending on
        *brightness
        */
        if (FgColor != DYNAMIC) {
                if (FgColor == BLACK) {
...
#endif
{
        ...
        while (loopcount < palette_dirs.NumOfDirectories &&
                ...
                */
                if (open_dir == False) {
                        if ((dirp = opendir(
                            ...
                            NULL) {
                                ...
                                **              long    d_fileno;     #file
                                *number of entry
                                **              short   d_reclen;     #length of
                                *this record
                                **              short   d_namlen;     #length of
                                *string in d_name
                ...
                while (file_descpt != NULL) {
                        ...
                        if (!(strncmp(PALETTE_SUFFIX,
                                      DtStrrchr(file_descpt->d_name, '.'),
                                      strlen(PALETTE_SUFFIX)))) {
                                /*
                                ** A file in the users home palette directory
                                *which begins with a ~
                                ** is a palette that the user doesn't want.
                                *Remove it from
                                ** the palette link list.
                                */
                                if ((strcmp(palette_dirs.directories[loopcount],
                                            home_palette_dir) == 0) &&
                                    (strncmp(file_descpt->d_name, "~", 1) ==
                                     0) &&
                                    pHeadPalette != NULL) {
                                        CheckForDeletedFile(file_descpt);
                                } else if (strncmp(file_descpt->d_name, "~",
                                                   1) != 0) {
                                        /* make sure if Black and white monitor
                                           the file is one that can be read for
                                           a XmCO_BLACK_WHITE monitor */
                                        if (TypeOfMonitor != XmCO_BLACK_WHITE) {
                                                if ((fnmatch(
                                                         file_descpt->d_name,
                                                         B_O_W, 0) != 0) &&
