...
static void ProcessComponentList(Widget parent, Widget mainRC);
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
void init_mainWindow(Widget shell) {
...
static void build_mainWindow(Widget shell) {
        ...
        XtSetArg(save.posArgs[save.poscnt], XmNnoResize, True);
        save.poscnt++;
        XtSetArg(save.posArgs[save.poscnt], XmNresizePolicy, XmRESIZE_NONE);
        save.poscnt++;
        style.mainWindow =
            XmCreateMainWindow(shell, "mainWindow", save.posArgs, save.poscnt);
        ...
        n = 0;
        tmpStr =
            (char *)XtMalloc(strlen(GETMESSAGE(4, 1, "Style Manager")) + 1);
        sprintf(tmpStr, "%s", GETMESSAGE(4, 1, "Style Manager"));
        XtSetArg(args[n], XmNtitle, tmpStr);
        n++;
        XtSetArg(args[n], XmNiconName, (char *)GETMESSAGE(4, 37, "Style"));
        n++;
        ...
        XtAddCallback(mainMenu, XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
        ...
        n++;
        XtSetArg(args[n], XmNsubMenuId, filePulldown);
        n++;
        ...
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        XtSetArg(args[n], XmNsubMenuId, helpPulldown);
        n++;
        ...
        n = 0;
        XtSetArg(args[n], XmNmenuHelpWidget, WidgList[count - 1]);
        n++;
        ...
        XtAddCallback(exitBtn, XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
                      (XtPointer)HELP_EXIT);
        ...
        n++;
        WidgList[count++] =
            XmCreatePushButton(helpPulldown, "stylemanager", args, n);
        XtAddCallback(WidgList[count - 1], XmNactivateCallback,
        ...
        XmGetColors(style.screen, style.colormap, bg, NULL, &style.primTSCol,
                    &style.primBSCol, NULL);

        /*  Get the secondary colors to be used later */
        n = 0;
        XtSetArg(args[n], XmNbackground, &style.secBgCol);
        n++;
        XtGetValues(exitBtn, args, n);

        XmGetColors(style.screen, style.colormap, style.secBgCol,
                    &style.tgSelectColor, &style.secTSCol, &style.secBSCol,
                    &style.secSelectColor);

        if ((style.secTSCol ==
             style.secBSCol) ||          /* B_W or shadowPixmaps==True */
            (!style.useMultiColorIcons)) /* multiColorIcons turned off */
        {
                /* force TS = White, BS = Black */
                style.secTSCol = style.primTSCol =
                    WhitePixelOfScreen(style.screen);
                style.secBSCol = style.primBSCol =
        ...
        if (colorUse == XmCO_BLACK_WHITE) {
                ...
                XtSetArg(args[n], XmNbackgroundPixmap, ditherPix);
                n++;
        }
        ...
        n++;
        XtSetArg(args[n], XmNspacing, style.horizontalSpacing);
        n++;
        mainRC = XmCreateRowColumn(frame, "mainRC", args, n);
        XtManageChild(mainRC);

        n = 0;
        XtSetArg(args[n], XmNmenuBar, mainMenu);
        n++;
        XtSetArg(args[n], XmNcommandWindow, NULL);
        n++;
        XtSetArg(args[n], XmNworkWindow, frame);
        n++;
...
static void ProcessComponentList(Widget parent, Widget mainRC) {
        ...
        if (style.xrdb.componentList == NULL)
                return;
        ...
        while (string != NULL) {
                ...
                n++;
                if (colorUse == XmCO_BLACK_WHITE) {
                        XtSetArg(args[n], XmNuseEmbossedText, False);
                        n++;
                }
                ...
                if (strcmp(string, COLOR) == 0) {
                        ...
                        dtIcon = _DtGetIconFileName(style.screen, COLOR_ICON,
                                                    NULL, NULL, 0);
                        style.workProcs = True;
                } else if (strcmp(string, FONT) == 0) {
                        cmp_string = CMPSTR(GETMESSAGE(4, 28, "Font"));
                        dtIcon = _DtGetIconFileName(style.screen, FONT_ICON,
                                                    NULL, NULL, 0);
                        ...
                        dtIcon = _DtGetIconFileName(style.screen, BACKDROP_ICON,
                                                    NULL, NULL, 0);
                        ...
                        dtIcon = _DtGetIconFileName(style.screen, KEYBOARD_ICON,
                                                    NULL, NULL, 0);
                        ...
                        dtIcon = _DtGetIconFileName(style.screen, MOUSE_ICON,
                                                    NULL, NULL, 0);
                        ...
                        dtIcon = _DtGetIconFileName(style.screen, BEEP_ICON,
                                                    NULL, NULL, 0);
                        ...
                        dtIcon = _DtGetIconFileName(style.screen, SCREEN_ICON,
                                                    NULL, NULL, 0);
                        ...
                        dtIcon = _DtGetIconFileName(style.screen, DTWM_ICON,
                                                    NULL, NULL, 0);
                        ...
                        dtIcon = _DtGetIconFileName(style.screen, STARTUP_ICON,
                                                    NULL, NULL, 0);
                        helpTag = HELP_STARTUP_BUTTON;
                } else if (strcmp(string, I18N) == 0) {
                        cmp_string = CMPSTR(GETMESSAGE(4, 34, "Intl'"));
                        dtIcon = _DtGetIconFileName(style.screen, I18N_ICON,
                                                    NULL, NULL, 0);
                ...
                XtSetArg(args[n], XmNimageName, dtIcon);
                n++;
                XtSetArg(args[n], XmNstring, cmp_string);
                n++;
...
void callCancels(void) {
        ...
        if (!calledAlready) {
                ...
                if (style.fontDialog != NULL && XtIsManaged(style.fontDialog)) {
                        CancelBut.button_position = CANCEL_BUTTON;
                        XtCallCallbacks(style.fontDialog, XmNcallback,
                                        &CancelBut);
                }

                if (style.audioDialog != NULL &&
                    XtIsManaged(style.audioDialog)) {
                        CancelBut.button_position = CANCEL_BUTTON;
                        XtCallCallbacks(style.audioDialog, XmNcallback,
                                        &CancelBut);
                }

                if (style.kbdDialog != NULL && XtIsManaged(style.kbdDialog)) {
                        CancelBut.button_position = CANCEL_BUTTON;
                        XtCallCallbacks(style.kbdDialog, XmNcallback,
                                        &CancelBut);
                }

                if (style.mouseDialog != NULL &&
                    XtIsManaged(style.mouseDialog)) {
                        CancelBut.button_position = CANCEL_BUTTON;
                        XtCallCallbacks(style.mouseDialog, XmNcallback,
                                        &CancelBut);
                }

                if (style.screenDialog != NULL &&
                    XtIsManaged(style.screenDialog)) {
                        CancelBut.button_position = CANCEL_BUTTON;
                        XtCallCallbacks(style.screenDialog, XmNcallback,
                                        &CancelBut);
                }

                if (style.startupDialog != NULL &&
                    XtIsManaged(style.startupDialog)) {
                        CancelBut.button_position = CANCEL_BUTTON;
                        XtCallCallbacks(style.startupDialog, XmNcallback,
                                        &CancelBut);
                }
...
void activateCB_exitBtn(Widget w, XtPointer client_data, XtPointer call_data) {
        /*callCancels() remembers whether its been called so it won't get
         * called*/
        /*twice on logout.*/
        callCancels();
...
static void activateCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        DtControlCallbackStruct *cbs = (DtControlCallbackStruct *)call_data;
        char *cd;
        cd = (char *)client_data;
 ...
 ************************************************************************/
void restoreMain(Widget shell, XrmDatabase db) {
        XrmName xrm_name[5];
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
                ...
                _DtSimpleError(
                    ...
                    ((char *)GETMESSAGE(4, 36,
                                        "No Dtstyle restore information "
