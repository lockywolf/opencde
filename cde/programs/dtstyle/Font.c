...
#define IMMEDIATE                                                              \
        ((char *)GETMESSAGE(                                                   \
            5, 18,                                                             \
            "The changes to fonts will show up in some\napplications the "     \
            "next time "                                                       \
            "they are started.\nOther applications, such as file manager "     \
            "and\napplication manager, will not show the font\nchanges until " \
            "you "                                                             \
            "Exit the desktop and then log\nback in."))
#define LATER                                                                  \
        ((char *)GETMESSAGE(5, 19,                                             \
                            "The selected font will be used when\n you "       \
                            "restart this session."))
#define INFO_MSG                                                               \
        ((char *)GETMESSAGE(                                                   \
            5, 24,                                                             \
            "The font that is currently used for your desktop is "             \
            "not\navailable in the Size list. If a new font is "               \
            "selected and\napplied, you will not be able to return "           \
            "to the current font\nusing the Style Manager - Font "             \
            "dialog."))
...
static void CreateFontDlg(Widget parent);
static void _DtmapCB_fontBB(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void ButtonCB(Widget w, XtPointer client_data, XtPointer call_data);
static void changeSampleFontCB(Widget w, XtPointer client_data,
                               XtPointer call_data);
...
/* popup_fontBB                          */
/*+++++++++++++++++++++++++++++++++++++++*/
void popup_fontBB(Widget shell) {
...
static void CreateFontDlg(Widget parent) {
        ...
        XtSetArg(save.posArgs[save.poscnt], XmNbuttonLabelStrings,
                 button_string);
        save.poscnt++;
        style.fontDialog = __DtCreateDialogBoxDialog(parent, "Fonts",
        ...
        n = 0;
        XtSetArg(args[n], XmNtitle, FONT_MSG);
        n++;
        ...
        n++;
        font.fontWkarea =
            XmCreateForm(style.fontDialog, "fontWorkArea", args, n);
        ...
        n++;
        XtSetArg(args[n], XmNimageName, FONT_ICON);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, font.fontpictLabel);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing + 5);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        ...
        sizeItems =
            (XmString *)XtMalloc(sizeof(XmString) * style.xrdb.numFonts);
        ...
        n++;
        XtSetArg(args[n], XmNvisibleItemCount, 7);
        n++;
        XtSetArg(args[n], XmNitemCount, style.xrdb.numFonts);
        n++;
        XtSetArg(args[n], XmNitems, sizeItems);
        n++;
        ...
        if (font.selectedFontIndex >= 0) {
                ...
                XtSetArg(args[n], XmNselectedItems, selectedSize);
                n++;
        ...
        n++;
        XtSetArg(args[n], XmNleftWidget, font.sizeTB);
        n++;
        ...
        n = 0;
        font.previewForm = XmCreateForm(font.previewTB, "previewForm", args, n);
        ...
        if (font.selectedFontIndex >= 0) {
                XtSetArg(args[n], XmNfontList,
                         style.xrdb.fontChoice[font.selectedFontIndex].sysFont);
                n++;
        ...
        if (font.selectedFontIndex >= 0) {
                XtSetArg(
                    args[n], XmNfontList,
                    style.xrdb.fontChoice[font.selectedFontIndex].userFont);
                n++;
                XtSetArg(args[n], XmNvalue, USER_MSG);
                n++;
        } else {
                XtSetArg(args[n], XmNvalue, NULL);
                n++;
        }

        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, font.systemLabel);
        n++;
        XtSetArg(args[n], XmNtopOffset, 2 * style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, font.systemLabel);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, font.systemLabel);
        n++;
        font.userText = XmCreateText(font.previewForm, "userText", args, n);
        /* Add callback to determine if user changes text in sample field */
        XtAddCallback(font.userText, XmNvalueChangedCallback, valueChangedCB,
                      NULL);
...
static void _DtmapCB_fontBB(Widget w, XtPointer client_data,
        ...
        XtRemoveCallback(style.fontDialog, XmNmapCallback, _DtmapCB_fontBB,
                         NULL);
}
...
static void ButtonCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        case OK_BUTTON:
                 ...
                 * anything.
                 */
                XtVaGetValues(font.sizeList, XmNselectedItemCount, &items,
                              NULL);

                /*   Post an info dialog explaining when the new fonts will
                 * appear */
                if ((font.selectedFontIndex != font.originalFontIndex) &&
                    (items > 0)) {
                        ...
                        /*
                           for *FontSet resource: find first font entry
                           delimited by a ":" or an "=".
                        */
                        len = strcspn(
                            style.xrdb.fontChoice[font.selectedFontIndex]
                                .userStr,
                            ":=");
                        ...
                        /*
                           Since the *Font and *FontSet resources may be used by
                           old X applications, a fontlist of multiple fonts must
                           be converted to Xt font set format (';'s converted to
                           ','s since many old X apps don't understand ';'
                           syntax.)
                        */
                        ...
                        /*
                           for *Font resource: find first font entry delimited
                           by a comma, a colon or an =
                        */
                        ...
                        /*
                          for *FontSet resource: if we got a font (instead of a
                          font set) from the first entry, then wildcard its
                          charset fields
                         */
                        len = strlen(fntsetstr);
                        if (style.xrdb.fontChoice[font.selectedFontIndex]
                                .userStr[len] != ':') {
                        ...
                        /* create the font resource specs with the selected font
                         * for xrdb */
                        sprintf(fontres,
                                "*systemFont: %s\n*userFont: %s\n*FontList: "
                                "%s\n*buttonFontList: "
        ...
        case CANCEL_BUTTON:
                ...
                if (font.originalFontIndex >= 0)
                        XmListSelectPos(font.sizeList,
                                        font.originalFontIndex + 1, True);
                else {
                        /*
                         * if no font was originally selected, need to undo any
                         * results from selections that were made by user before
                         * pressing Cancel.
                         */
                        XtVaSetValues(font.sizeList, XmNselectedItemCount, 0,
                                      NULL);
                        XtVaSetValues(font.userText, XmNvalue, BLANK_MSG,
                                      XmNfontList, style.xrdb.userFont, NULL);
                        XtVaSetValues(font.systemLabel, XmNlabelString,
                                      CMPSTR(BLANK_MSG), XmNfontList,
                                      style.xrdb.systemFont, NULL);
                        font.userTextChanged = FALSE;
                        font.selectedFontIndex = -1;
                }
                break;
        ...
        case HELP_BUTTON:
                XtCallCallbacks(style.fontDialog, XmNhelpCallback,
                                (XtPointer)NULL);
                break;
 ...
 ************************************************************************/
void restoreFonts(Widget shell, XrmDatabase db) {
        XrmName xrm_name[5];
