...
typedef struct {
        ...
        String session, backdropDir, paletteDir, timeoutScale, lockoutScale;
...
typedef struct {
        ...
        Pixel tgSelectColor, secSelectColor, /** for editable text bg **/
        ...
        Widget shell, mainWindow, colorDialog, backdropDialog, fontDialog,
            kbdDialog, mouseDialog, audioDialog, screenDialog, startupDialog,
            dtwmDialog, i18nDialog, buttonsForm;
        XmString tmpXmStr;
...
extern int local_xerror(Display *dpy,
                        XErrorEvent *rep); /* defined in hostBB.c for
                                                historical reasons*/

extern char *_DtGetMessage(char *filename, int set, int n, char *s);
extern void raiseWindow(Window dialogWin);
extern void CenterMsgCB(Widget w, XtPointer client_data, XtPointer call_data);
extern void ErrDialog(char *errString, Widget visualParent);
extern void InfoDialog(char *infoString, Widget parent, Boolean unmapParent);
extern void putDialog(Widget parent, Widget dialog);
extern void smStateCB(DtString fields[], XtPointer client_data, int num_fields);
extern void HandleStopMsg(DtString fields[], XtPointer client_data,
                          int num_words);
...
#ifdef DT_LITE
#define GETMESSAGE(set, number, string) _DtGetMessage(set, number, "")
#else /*  DT_LITE*/
