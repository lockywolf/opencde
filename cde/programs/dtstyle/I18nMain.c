...
#define MSG_IMMEDIATE                                                          \
        ((char *)GETMESSAGE(                                                   \
            19, 20,                                                            \
            "The new XmNpreeditType value will take effect\nas "               \
            "applications are restarted."))
#define MSG_LATER                                                              \
        ((char *)GETMESSAGE(                                                   \
            19, 21,                                                            \
            "Your selection will take effect\n at your next session."))
...
static char *defaultPreedit[] = {"OnTheSpot", "OverTheSpot", "OffTheSpot",
                                 "Root"};
...
void popup_i18nBB(Widget shell) {
        int ret = NoError;
...
static void SetImServerHosts(I18nEnv *env) {
        ...
        XtVaSetValues(XtNameToWidget(i18n.serverHostCB, "Text"), XmNvalue,
                      env->file_sel->hostname, NULL);

        /* update ComboBox with list of server hostnames found in
           "imServerHosts" resource */

        if (style.xrdb.imServerHosts)
                while (style.xrdb.imServerHosts[num_hosts])
                        num_hosts++;

        /* TBD: need to add file_sel->hostname if not null
           or localhost, if not already present */

        XtVaSetValues(i18n.serverHostCB, XmNitemCount, num_hosts, XmNitems,
                      style.xrdb.imServerHosts, NULL);
}
...
static void SetImsToggleList(I18nEnv *env) {
        ...
        for (i = 0; i < env->ims_sel->ims_list_size; i++) {
                string =
                    XmStringCreateLocalized(env->ims_sel->ims_list[i].im_label);
                n = 0;
                XtSetArg(args[n], XmNlabelString, string);
                n++;
                XtSetArg(args[n], XmNsensitive,
                         !(env->ims_sel->ims_list[i].inactive));
                n++;
                XtSetArg(args[n], XmNuserData,
                         env->ims_sel->ims_list[i].im_name);
                n++;
                ...
                im_tog[i] = XmCreateToggleButtonGadget(i18n.inputMethodRC, "IM",
                                                       args, n);
                XmStringFree(string);
        }
...
static void SetImsMode(I18nEnv *env) {
        ...
        if (env->file_sel->start_mode == 0)
                ...
                XmToggleButtonGadgetSetState(i18n.resumeCurrentImTG, True,
                                             True);
}
...
static void SetPreeditType(I18nEnv *env, XmStringTable preeditTable,
        ...
        XtVaSetValues(i18n.preeditTypeList, XmNvisibleItemCount,
                      preeditTableNum, XmNitemCount, preeditTableNum, XmNitems,
                      loc_preeditTable, XmNselectedItemCount, 1,
        ...
        /* If more than one item in the list, set the Button MoveDown sentivity
           to True. */
...
static Widget BuildI18nDlg(Widget shell) {
        ...
        n++;
        XtSetArg(args[n], XmNimageName, I18N_ICON);
        n++;
        ...
        n++;
        i18n.serverHostCB = (Widget)(intptr_t)XmCreateComboBox(
            inputMethodForm, "serverHostCB", args, n);
        ...
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        i18n.askAtLoginTG = XmCreateToggleButtonGadget(i18n.imStartModeRC,
                                                       "askAtLoginTG", args, n);
        XmStringFree(string);

        n = 0;
        string =
            CMPSTR((char *)GETMESSAGE(19, 9, "Resume Current Input Method"));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        i18n.resumeCurrentImTG = XmCreateToggleButtonGadget(
            i18n.imStartModeRC, "resumeCurrentImTG", args, n);
        XmStringFree(string);
        ...
        n++;
        i18n.preeditTypeList = (Widget)(intptr_t)XmCreateList(
            preeditTypeForm, "preeditTypeList", args, n);
        ...
        XtSetArg(args[n], XmNsensitive, False);
        n++;
        i18n.buttonMoveDown = XmCreatePushButtonGadget(
            preeditTypeForm, "buttonMoveDown", args, n);
        XmStringFree(string);
        ...
        XtAddCallback(i18n.systemDefault, XmNactivateCallback, SystemDefaultCB,
                      NULL);
        XtAddCallback(i18n.preeditTypeList, XmNbrowseSelectionCallback,
                      ListItemSelectCB, NULL);
        XtAddCallback(i18n.buttonMoveUp, XmNactivateCallback, MoveUpCB, NULL);
        XtAddCallback(i18n.buttonMoveDown, XmNactivateCallback, MoveDownCB,
                      NULL);
...
static void FormLayoutCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, i18n.pictLabel);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNtopOffset, 2 * style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        n++;
        XtSetArg(args[n], XmNleftWidget, i18n.serverHostLabel);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_NONE);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, i18n.serverHostCB);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, i18n.inputMethodLabel);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, i18n.inputMethodTB);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, i18n.imStartModeTB);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, i18n.preeditTypeList);
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, i18n.buttonMoveUp);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
...
static void ServerHostCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        /* Get the current hostname in the TextField of the ComboBox,
           as string */
        XtVaGetValues(XtNameToWidget(i18n.serverHostCB, "Text"), XmNvalue,
                      &hostname, NULL);
...
static void UpdateImList(I18nEnv *env, char *hostname) {
        ...
        XtVaGetValues(i18n.inputMethodRC, XmNchildren, &im_tog, XmNnumChildren,
                      &n, NULL);
        if (n) {
                XtUnmanageChildren(im_tog, n);
                for (i = 0; i < n; i++)
                        XtDestroyWidget(im_tog[i]);
                im_tog = NULL;
        }
...
static void SystemDefaultCB(Widget w, XtPointer client_data,
        ...
        if (!preeditTable) {
                ...
                preeditTable =
                    (XmStringTable)XtMalloc(NUM_PREEDIT * sizeof(XmString));
                for (i = 0; i < NUM_PREEDIT; i++)
                        preeditTable[i] =
                            XmStringCreate(defaultPreedit[i], "ISO8859-1");
        }
...
static void ButtonCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        case CANCEL_BUTTON:
                ...
                if (i18n.preeditHasChanged) {
                        ...
                        SetPreeditType(env, i18n.preeditSavedVal,
                                       i18n.preeditSavedNum);
        ...
        case HELP_BUTTON:
                XtCallCallbacks(style.i18nDialog, XmNhelpCallback,
                                (XtPointer)NULL);
                break;
...
static int SaveSelectedValues(I18nEnv *env) {
        ...
        if (i18n.preeditHasChanged) {
                ...
                XtVaGetValues(i18n.preeditTypeList, XmNitemCount, &num_preedit,
                ...
                for (i = 0; i < num_preedit; i++) {
                        ...
                        tmpStr = XmStringUnparse(list_preedit[i], NULL,
                                                 XmCHARSET_TEXT, XmCHARSET_TEXT,
                                                 NULL, 0, XmOUTPUT_ALL);
                        /* realloc mallocs the first time */
                        preeditStr =
                            XtRealloc(preeditStr,
 ...
 ************************************************************************/
void restoreI18n(Widget shell, XrmDatabase db) {
        XrmName xrm_name[5];
...
void _DtI18nSetSensitiveImTB(I18nEnv *env, Boolean sensitivity) {
...
void _DtI18nErrorDialog(int err) {
        char *err_str;
        ...
        case ErrNoHome:
                err_str = GETMESSAGE(
                    19, 30, "The environment variable HOME is not defined.");
                break;
        case ErrNoLocale:
                err_str = GETMESSAGE(
                    19, 31, "The environment variable LANG is not defined.");
                break;
        case ErrNoCDELocale:
                err_str = GETMESSAGE(
                    19, 32, "This locale is not supported by the desktop.");
                break;
        ...
        case ErrRemoteFailed:
                err_str = GETMESSAGE(
                    19, 38, "Remote execution failed on\n the selected host.");

                /* Pipe */
        case ErrNoPopen:
                err_str =
                    GETMESSAGE(19, 39, "Cannot initialize remote execution");
                break;
        case ErrTimeOut:
                err_str = GETMESSAGE(19, 40,
                                     "Getting the Input Method on the "
...
static void MoveUpCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (XmListGetSelectedPos(i18n.preeditTypeList, &position_list,
                ...
                if (position_list[0] > 1) {
                        ...
                        XtVaGetValues(i18n.preeditTypeList, XmNitems,
                                      &list_items, NULL);

                        /* makes an array of two XmStrings by reversing the
                           selected one and the item preceeding it */
                        items[0] = list_items[sel_index];
                        items[1] = list_items[sel_index - 1];

                        /* this call preserves selected state */
                        XmListReplaceItemsPos(i18n.preeditTypeList, items, 2,
                                              sel_pos - 1);

                        if (sel_pos == 2) {
                                /* it's now 1, so set the MoveUp button
                                 * insensitive */
                                XtSetSensitive(i18n.buttonMoveUp, False);
                        }

                        if (sel_pos == i18n.preeditTypeListLastPos) {
                                /* it's not anymore the last one, so set the
                                   MoveDown button sensitive */
                                XtSetSensitive(i18n.buttonMoveDown, True);
                        }
...
static void MoveDownCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (XmListGetSelectedPos(i18n.preeditTypeList, &position_list,
                ...
                /* this should always be the case as the Move Down button should
                   be insensitive when the first item is selected, but check
                   anyway */
                ...
                if (position_list[0] != i18n.preeditTypeListLastPos) {
                        ...
                        /* makes an array of two XmStrings by reversing the
                           selected one and the item succeeding it */
                        ...
                        if (sel_pos == (i18n.preeditTypeListLastPos - 1)) {
                                /* it's now the last one, so set the MoveDown
                                   button insensitive */
                                XtSetSensitive(i18n.buttonMoveDown, False);
                        }

                        if (sel_pos == 1) {
                                /* it's not anymore the first one, so set the
                                   MoveUp button sensitive */
                                XtSetSensitive(i18n.buttonMoveUp, True);
                        }
