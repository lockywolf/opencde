...
static void no_svr_ext_formLayoutCB(Widget w, XtPointer client_data,
                                    XtPointer call_data);

static void _DtmapCB_screenDialog(Widget w, XtPointer client_data,
                                  XtPointer call_data);

static void no_svr_ext_DtmapCB_screenDialog(Widget w, XtPointer client_data,
                                            XtPointer call_data);

static void timeOutvalueChangedCB(Widget w, XtPointer client_data,
                                  XtPointer call_data);

static void no_svr_ext_timeOutvalueChangedCB(Widget w, XtPointer client_data,
                                             XtPointer call_data);

static void saverToggleCB(Widget w, XtPointer client_data, XtPointer call_data);

static void no_svr_ext_saverToggleCB(Widget w, XtPointer client_data,
                                     XtPointer call_data);

static void saversToggleCB(Widget w, XtPointer client_data,
                           XtPointer call_data);

static void lockToggleCB(Widget w, XtPointer client_data, XtPointer call_data);

static void systemDefaultCB(Widget w, XtPointer client_data,
                            XtPointer call_data);

static void no_svr_ext_systemDefaultCB(Widget w, XtPointer client_data,
                                       XtPointer call_data);

static void ButtonCB(Widget w, XtPointer client_data, XtPointer call_data);

static void no_svr_ext_ButtonCB(Widget w, XtPointer client_data,
                                XtPointer call_data);

static XmString *MakeListStrings(char **list);

static void FreeListStrings(XmString *xmlist, int count);

static void ListCB(Widget w, XtPointer client_data, XtPointer call_data);

static void IconHandler(Widget shell, XtPointer client_data, XEvent *event,
                        Boolean *dispatch);
...
typedef struct {
        ...
        Widget
            useSaversOn; /* used for graying saverList and saverArea in no saver
                            extn mode */
        Widget useSaversOff; /* used for graying saverList and saverArea in no
                                saver extn mode */
        int interval;
        int blanking;
        int exposures;
        int newTimeout;  /* the min value of savertimeout and locktimeout */
        int origTimeout; /* the original value of the min of savertimeout and
                            locktimeout */
        int xTimeout; /* the value of the timeout extracted from the X server */
        Boolean
            timeMismatch; /* use to keep track of mismatch between xTimeout and
                             newTimeout */
        ...
        int saverTimeout_xrdb; /* the value of savertimeout from xrdb, in case
                                  saver is off */
        int lockTimeout_xrdb;  /* the value of locktimeout from xrdb, in case
                                  lock is  off */
        int new_blanking;
...
typedef struct {
        ...
        char *
            *selsaverActionNames; /* array of originally selected saver names */
        int *selPositions; /* array of currently selected saver positions */
        int *
            origSelPositions; /* array of originally selected saver positions */
        ...
        int orig_selsaverCount; /* number of originally selected screen savers
                                 */
        int previous_selsaverCount; /* number of previously selected screen
                                       savers */
        char *current_saver;        /* name of current screen saver actions */
        char *
            current_saverDesc; /* description of current screen saver actions */
...
char **build_saverList(char *str, int *count) {
        char tokenSep[] = " ";
        char *token;
        char **saverList = NULL;
        register int i = 0;
                ...
                *count = 0;
                while (token != NULL) {
                        if (DtActionExists(token)) {
                                saverList[*count] =
                                    (char *)XtCalloc(1, strlen(token) + 1);
...
char **build_selsaverList(char *envStr, int *count) {
        char tokenSep[] = " ";
        char *token;
        char **saverList = NULL;
        register int i = 0;
...
char **build_saverDesc(char **names, int count) {
        char **saverDesc = NULL;
        char *tmpstr;
        int i;
        ...
        for (i = 0; i < count; i++) {
                ...
                if (tmpstr == NULL) {
                        saverDesc[i] = (char *)XtMalloc(
                            strlen(savers.saverActionNames[i]) + 1);
...
char *build_selectedList(char **saverList, int count) {
...
void free_saverList(char **saverList, int count)
...
void popup_screenBB(Widget shell) {
...
static Widget build_screenDialog(Widget shell) {
        ...
        savers.black =
            BlackPixel(style.display, DefaultScreen(XtDisplay(shell)));
        ...
        save.poscnt++;
        style.screenDialog = __DtCreateDialogBoxDialog(
            shell, "ScreenDialog", save.posArgs, save.poscnt);
        ...
        n++;
        XtSetArg(args[n], XmNcancelButton, widget_list1[0]);
        n++;
        ...
        XtSetArg(args[n], XmNtitle,
                 ((char *)GETMESSAGE(12, 1, "Style Manager - Screen")));
        n++;
        XtSetArg(args[n], XmNuseAsyncGeometry, True);
        n++;
        XtSetArg(args[n], XmNmwmFunctions, DIALOG_MWM_FUNC);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNchildType, XmWORK_AREA);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNpixmapForeground, style.secBSCol);
        n++;
        XtSetArg(args[n], XmNpixmapBackground, style.secTSCol);
        n++;
        XtSetArg(args[n], XmNstring, NULL);
        n++;
        XtSetArg(args[n], XmNshadowThickness, 0);
        n++;
        XtSetArg(args[n], XmNimageName, SCREEN_ICON);
        n++;
        XtSetArg(args[n], XmNtraversalOn, False);
        n++;
        widget_list1[list1_count++] = screen.pictLabel =
            _DtCreateIcon(form, "screenLabelPixmap", args, n);

        /* Create the default button */
        n = 0;
        XtSetArg(args[n], XmNmarginHeight, LB_MARGIN_HEIGHT);
        n++;
        XtSetArg(args[n], XmNmarginWidth, LB_MARGIN_WIDTH);
        n++;
        string = CMPSTR(((char *)GETMESSAGE(12, 3, "Default")));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        XtSetArg(args[n], XmNnavigationType, XmTAB_GROUP);
        n++;
        widget_list1[list1_count++] = screen.systemDefault =
        ...
        if (style.smState.smLockOnTimeoutStatus)
                ...
                string =
                    CMPSTR(((char *)GETMESSAGE(12, 18, "Front Panel Lock")));
        XtSetArg(args[n], XmNtitleString, string);
        n++;
        widget_list1[list1_count++] = screen.lockFrame =
            _DtCreateTitleBox(form, "lockFrame", args, n);
        XmStringFree(string);
        ...
        if (style.smState.smLockOnTimeoutStatus)
                ...
                string = CMPSTR(
                    ((char *)GETMESSAGE(12, 15, "Auto Screen Blanking")));
        XtSetArg(args[n], XmNtitleString, string);
        n++;
        widget_list1[list1_count++] = screen.saverFrame =
            _DtCreateTitleBox(form, "saverFrame", args, n);
        XmStringFree(string);
        ...
        n++;
        screen.saverForm =
            XmCreateForm(screen.saverFrame, "saverForm", args, n);
        ...
        if (style.smState.smLockOnTimeoutStatus)
                ...
                string =
                    CMPSTR(((char *)GETMESSAGE(12, 20, "Time Per Background")));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        /* only if saver extensions are present */
        if (style.smState.smLockOnTimeoutStatus)
                screen.cycleTimeLabel = XmCreateLabelGadget(
                    screen.saverForm, "cycleTimeLabel", args, n);
        else
                screen.cycleTimeLabel = XmCreateLabelGadget(
                    screen.lockForm, "cycleTimeLabel", args, n);
        XmStringFree(string);

        n = 0;
        XtSetArg(args[n], XmNminimum, CYCLETIME_MIN);
        n++;
        XtSetArg(args[n], XmNmaximum, TIMEOUT_MAX);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNhighlightThickness, SCALE_HIGHLIGHT_THICKNESS);
        n++;
        if (style.smState.smLockOnTimeoutStatus)
                screen.cycleTimeScale =
                    XmCreateScale(screen.saverForm, "cycleTimeScale", args, n);
        else
                screen.cycleTimeScale =
                    XmCreateScale(screen.lockForm, "cycleTimeScale", args, n);
        ...
        n++;
        if (style.smState.smLockOnTimeoutStatus)
                screen.cycleMinutesLabel = XmCreateLabelGadget(
                    screen.saverForm, "cycleMinutes", args, n);
        else
                screen.cycleMinutesLabel = XmCreateLabelGadget(
                    screen.lockForm, "cycleMinutes", args, n);
        XmStringFree(string);
        ...
        if (style.smState.smLockOnTimeoutStatus)
                ...
                string =
                    CMPSTR(((char *)GETMESSAGE(12, 16, "Screen Blanker:")));
        XtSetArg(args[n], XmNalignment, XmALIGNMENT_BEGINNING);
        n++;
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        screen.saverLabel =
            XmCreateLabelGadget(screen.saverForm, "saverLabel", args, n);
        XmStringFree(string);

        n = 0;
        XtSetArg(args[n], XmNorientation, XmHORIZONTAL);
        n++;
        screen.saverRadioBox =
            XmCreateRadioBox(screen.saverForm, "saverRadioBox", args, n);

        n = 0;
        string = CMPSTR(((char *)GETMESSAGE(12, 10, "On")));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        screen.saverOn = XmCreateToggleButtonGadget(screen.saverRadioBox,
                                                    "saverOn", args, n);
        XmStringFree(string);
        ...
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        screen.saverOff = XmCreateToggleButtonGadget(screen.saverRadioBox,
                                                     "saverOff", args, n);
        XmStringFree(string);
        ...
        if (low_res) {
                ...
                XtSetArg(args[n], XmNheight, SAVER_HEIGHT);
                n++;
        }
        XtSetArg(args[n], XmNhighlightColor, 0);
        n++;
        /* choose the parent of saverList depending on the presence of saver
         * extensions */
        if (style.smState.smLockOnTimeoutStatus)
                screen.saverList = XmCreateScrolledList(screen.saverForm,
                                                        "saversList", args, n);
        else
                screen.saverList = XmCreateScrolledList(screen.lockForm,
                                                        "saversList", args, n);
        ...
        if (savers.saversList == NULL) {
                ...
                savers.saverActionNames =
                    build_saverList(savers.saversList, &savers.saverCount);
                savers.saverActionDesc =
                    build_saverDesc(savers.saverActionNames, savers.saverCount);
                /* convert to XmString */
                listStrings = MakeListStrings(savers.saverActionDesc);
                XmListAddItems(screen.saverList, listStrings, savers.saverCount,
                               0);
                ...
                /* check the state to see which item(s) should be selected, if
                 * none are */
                /* selected or if none in the selected list match the  available
                 * list then
                 */
                /* set the selection to the last position by default. This
                 * should be blank
                 */
                /* if the DTSCREENSAVERLIST environment variable is set to
                 * default */
                savers.selsaverActionNames = build_selsaverList(
                    style.smSaver.saverList, &savers.selsaverCount);

                /* since savers.selsaverCount may be changed below, we save */
                /* number of originally selected savers in order to accurately
                 * free the */
                ...
                if (savers.selsaverCount) {
                        savers.selPositions =
                            (int *)XtMalloc(sizeof(int) * savers.selsaverCount);
                        for (n = 0; n < savers.selsaverCount; n++) {
                                /* determine if this selected saver matches one
                                 * in valid saver list */
                                for (m = 0;
                                     (m < savers.saverCount) &&
                                     ((sel_found = strcmp(
                                           savers.selsaverActionNames[n],
                                           savers.saverActionNames[m])) != 0);
                                     m++) {
                                }
                                if (sel_found == 0) {
                                        XmListSelectPos(screen.saverList, m + 1,
                                                        False);
                                        savers.selPositions[n] = m + 1;
                                        sel_matched = True;
                                } else {
                                        /* This selected saver isn't valid, need
                                           to get rid of it. Do this by shifting
                                           each selected saver in the array back
                                           one position, i.e. n = n+1, n+1 =
                                           n+2, .... This overwrites the nth
                                           element and leaves the empty space at
                                           the end of the array.  */
                                        for (i = n;
                                             i < (savers.selsaverCount - 1);
                                             i++)
                                                savers.selsaverActionNames[i] =
                                                    savers
                                                        .selsaverActionNames[i +
                                                                             1];

                                        /* reduce # of selected savers */
                                        savers.selsaverCount--;

                                        /* reset selected saver array position
                                           counter back one position for next
                                           loop test */
                                        n--;
                                } /* if (sel_found == 0) */
                ...
                /* if there are still savers selected that match valid saver
                 * names */
                if (savers.selsaverCount) {
                        /* set current saver to first selection */
                        savers.current_saver =
                            savers.saverActionNames[savers.selPositions[0] - 1];
                        savers.current_saverDesc =
                            savers.saverActionDesc[savers.selPositions[0] - 1];
                        savers.current_position = savers.selPositions[0];
                } else {
                        /* highlight last saver */
                        XmListSelectPos(screen.saverList, savers.saverCount,
                                        False);

                        /* set current saver to last one */
                        savers.current_saver =
                            savers.saverActionNames[savers.saverCount - 1];
                        savers.current_saverDesc =
                            savers.saverActionDesc[savers.saverCount - 1];
        ...
        n++;
        XtSetArg(args[n], XmNshadowThickness, savers.shadow);
        n++;
        XtSetArg(args[n], XmNhighlightThickness, 0);
        n++;
        XtSetArg(args[n], XmNtraversalOn, False);
        n++;
        /* depending on the presence of saver extensions choose the parent of
         * saverAreaFrame */
        if (style.smState.smLockOnTimeoutStatus)
                screen.saverAreaFrame =
                    XmCreateFrame(screen.saverForm, "saverAreaFrame", args, n);
        else
                screen.saverAreaFrame =
                    XmCreateFrame(screen.lockForm, "saverAreaFrame", args, n);
        ...
        n++;
        if (low_res) {
                XtSetArg(args[n], XmNwidth, 180);
                n++;
        } else {
                XtSetArg(args[n], XmNwidth, SAVER_WIDTH);
                n++;
        }
        screen.saverArea =
            XmCreateDrawingArea(screen.saverAreaFrame, "saverArea", args, n);
        ...
        if (style.smState.smLockOnTimeoutStatus)
                screen.prevLabel =
                    XmCreateLabelGadget(screen.saverForm, "prevLabel", args, n);
        else
                screen.prevLabel =
                    XmCreateLabelGadget(screen.lockForm, "prevLabel", args, n);
        XmStringFree(string);
        ...
        n++;
        screen.timeOutLabel =
            XmCreateLabelGadget(screen.saverForm, "timeOutLabel", args, n);
        XmStringFree(string);

        n = 0;
        XtSetArg(args[n], XmNminimum, TIMEOUT_MIN);
        n++;
        XtSetArg(args[n], XmNmaximum, TIMEOUT_MAX);
        n++;
        XtSetArg(args[n], XmNorientation, XmHORIZONTAL);
        ...
        n++;
        screen.timeOutScale =
            XmCreateScale(screen.saverForm, "timeOutScale", args, n);

        n = 0;
        string = CMPSTR(((char *)GETMESSAGE(12, 7, "minutes")));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        screen.saverMinutesLabel =
            XmCreateLabelGadget(screen.saverForm, "saverMinutes", args, n);
        XmStringFree(string);
        ...
        n++;
        screen.lockLabel =
            XmCreateLabelGadget(screen.lockForm, "lockLabel", args, n);
        XmStringFree(string);
        ...
        n++;
        screen.lockRadioBox =
            XmCreateRadioBox(screen.lockForm, "lockRadioBox", args, n);

        n = 0;
        string = CMPSTR(((char *)GETMESSAGE(12, 10, "On")));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        XtSetArg(args[n], XmNset, style.smState.smCoverScreen ? True : False);
        n++;
        screen.lockOn =
            XmCreateToggleButtonGadget(screen.lockRadioBox, "lockOn", args, n);
        XmStringFree(string);
        ...
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        XtSetArg(args[n], XmNset, style.smState.smCoverScreen ? False : True);
        n++;
        screen.lockOff =
            XmCreateToggleButtonGadget(screen.lockRadioBox, "lockOff", args, n);
        XmStringFree(string);
        ...
        n++;
        screen.lockOutLabel =
            XmCreateLabelGadget(screen.lockForm, "lockOutLabel", args, n);
        XmStringFree(string);
        ...
        n++;
        screen.lockOutScale =
            XmCreateScale(screen.lockForm, "lockOutScale", args, n);

        n = 0;
        string = CMPSTR(((char *)GETMESSAGE(12, 7, "minutes")));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        screen.lockMinutesLabel =
            XmCreateLabelGadget(screen.lockForm, "lockMinutes", args, n);
        XmStringFree(string);
        ...
        XtSetArg(args[n], XmNorientation, XmHORIZONTAL);
        n++;
        XtSetArg(args[n], XmNspacing, 25);
        n++;
        screen.lockingRadioBox =
            XmCreateRadioBox(screen.lockForm, "lockingRadioBox", args, n);

        n = 0;
        string =
            CMPSTR(((char *)GETMESSAGE(12, 21, "Use Backgrounds For Lock")));
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        screen.useSaversOn = XmCreateToggleButtonGadget(screen.lockingRadioBox,
                                                        "useSaversOn", args, n);
        XmStringFree(string);
        ...
        XtSetArg(args[n], XmNlabelString, string);
        n++;
        screen.useSaversOff = XmCreateToggleButtonGadget(
            screen.lockingRadioBox, "useSaversOff", args, n);
        XmStringFree(string);
        ...
        if (!style.smState.smLockOnTimeoutStatus) {
                XtAddCallback(style.screenDialog, XmNmapCallback,
                              no_svr_ext_formLayoutCB, NULL);
                XtAddCallback(style.screenDialog, XmNmapCallback,
                              no_svr_ext_DtmapCB_screenDialog, shell);
                XtAddCallback(screen.systemDefault, XmNactivateCallback,
                              no_svr_ext_systemDefaultCB, NULL);
                ...
                XtAddCallback(style.screenDialog, XmNcallback,
                              no_svr_ext_ButtonCB, NULL);
        } else {
                XtAddCallback(style.screenDialog, XmNmapCallback, formLayoutCB,
                              NULL);
                XtAddCallback(style.screenDialog, XmNmapCallback,
                              _DtmapCB_screenDialog, shell);
                XtAddCallback(screen.systemDefault, XmNactivateCallback,
        ...
        if (style.smState.smLockOnTimeoutStatus) {
                ...
                XtVaSetValues(screen.lockOutLabel, XmNwidth, MaxLabelWidth,
                              NULL);
        }
        ...
        XtAddEventHandler(style.shell, StructureNotifyMask, False, IconHandler,
                          NULL);
...
static void formLayoutCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNrightOffset, style.horizontalSpacing);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNbottomOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, screen.lockLabel);
        n++;
        XtSetArg(args[n], XmNtopOffset, -RadioOffset);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, screen.lockLabel);
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_NONE);
        ...
        XtSetArg(args[n], XmNtopOffset,
                 style.verticalSpacing + ScaleHeight - LabelHeight);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        n++;
        XtSetArg(args[n], XmNbottomWidget, screen.lockOutLabel);
        n++;
        XtSetArg(args[n], XmNbottomOffset, ScaleOffset);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, screen.lockOutLabel);
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, screen.lockMinutesLabel);
        n++;
        XtSetArg(args[n], XmNrightOffset, style.horizontalSpacing);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, screen.pictLabel);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNbottomWidget, screen.lockFrame);
        n++;
        XtSetArg(args[n], XmNbottomOffset, 2 * style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        ...
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNbottomWidget, screen.cycleTimeLabel);
        n++;
        XtSetArg(args[n], XmNbottomOffset, ScaleOffset);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, screen.cycleTimeLabel);
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, screen.cycleMinutesLabel);
        n++;
        XtSetArg(args[n], XmNrightOffset, style.horizontalSpacing);
        n++;
        ...
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNbottomWidget, screen.timeOutLabel);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_NONE);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, screen.timeOutLabel);
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, screen.saverMinutesLabel);
        n++;
        XtSetArg(args[n], XmNrightOffset, style.horizontalSpacing);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, screen.saverLabel);
        n++;
        XtSetArg(args[n], XmNtopOffset, -RadioOffset);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        ...
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, screen.saverLabel);
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_NONE);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, screen.prevLabel);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing / 2);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_NONE);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNbottomWidget, screen.timeOutLabel);
        n++;
        XtSetArg(args[n], XmNbottomOffset, 5 * style.verticalSpacing);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, XtParent(screen.saverList));
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, XtParent(screen.saverList));
        n++;
        XtSetArg(args[n], XmNleftOffset, style.horizontalSpacing);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNbottomWidget, XtParent(screen.saverList));
        n++;
        XtSetValues(screen.saverAreaFrame, args, n);

        XtRemoveCallback(style.screenDialog, XmNmapCallback, formLayoutCB,
                         NULL);
}
...
static void no_svr_ext_formLayoutCB(Widget w, XtPointer client_data,
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, screen.saverFrame);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, screen.lockingRadioBox);
        n++;
        XtSetArg(args[n], XmNtopOffset, style.verticalSpacing);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_NONE);
        ...
        XtRemoveCallback(style.screenDialog, XmNmapCallback,
                         no_svr_ext_formLayoutCB, NULL);
}
...
static void _DtmapCB_screenDialog(Widget w, XtPointer client_data,
        ...
        if (first_time) {
                ...
                screen.saverTimeout_xrdb =
                    atoi((char *)style.xrdb.timeoutScale);
                ...
                /* initialize the value of savertimeout and locktimeout and
                 * cycle time to the */
                /* initial state or to the values retrieved from xrdb if the
                 * corresponding*/
                ...
                XtVaSetValues(screen.timeOutScale, XmNvalue,
                              screen.saverTimeout, NULL);
                XtVaSetValues(screen.lockOutScale, XmNvalue, screen.lockTimeout,
                              NULL);
                XtVaSetValues(screen.cycleTimeScale, XmNvalue,
                              screen.cycleTimeout, NULL);

                /* set the value of the saver toggles */
                XmToggleButtonGadgetSetState(
                    screen.saverOn, style.smState.smSaverTimeout ? True : False,
                    False);
                XmToggleButtonGadgetSetState(
                    screen.saverOff,
                    !style.smState.smSaverTimeout ? True : False, False);
                XmToggleButtonGadgetSetState(
                    screen.lockOn, style.smState.smLockTimeout ? True : False,
                    False);
                XmToggleButtonGadgetSetState(
                    screen.lockOff, !style.smState.smLockTimeout ? True : False,
                    False);
                ...
                if (XmToggleButtonGadgetGetState(screen.saverOn)) {
                        ...
                        XtManageChild(screen.prevLabel);
                        /* make cycletime scale sensitive if selected savers are
                         * more than 1 */
                ...
                if (!style.smState.smSaverTimeout)
                        ...
                        screen.origTimeout = screen.newTimeout =
                            style.smState.smSaverTimeout;
                else
                        screen.origTimeout = screen.newTimeout =
                            MIN(style.smState.smSaverTimeout,
                                style.smState.smLockTimeout);

                first_time = 0;
        }
        /* check the saver values from X  -  need to do this everytime dialog is
         * mapped because */
        /* the user may have changed settings thru X while style manager has
         * been running */
        /* if the value of the timeout retrieved from X is not equal to the new
         * timeout set in the */
        /* style manager interface, then set both lock time and saver time
         * scales to value retrieved from */
        ...
        if (screen.xTimeout < 0)
                screen.xTimeout = 0;
        else if ((screen.xTimeout < 60) && (screen.xTimeout > 1))
                screen.xTimeout = 60;
        else if (screen.xTimeout > TIMEOUT_MAX * 60)
        ...
        if (screen.xTimeout != screen.newTimeout) {
                ...
                XmToggleButtonGadgetSetState(
                    screen.saverOn, screen.xTimeout ? True : False, False);
                XmToggleButtonGadgetSetState(
                    screen.saverOff, !screen.xTimeout ? True : False, False);
                XmToggleButtonGadgetSetState(
                    screen.lockOn, screen.xTimeout ? True : False, False);
                XmToggleButtonGadgetSetState(
                    screen.lockOff, !screen.xTimeout ? True : False, False);

                /* set the value of the scales */
                XtVaSetValues(screen.timeOutScale, XmNvalue,
                              screen.saverTimeout, NULL);
                XtVaSetValues(screen.lockOutScale, XmNvalue,
                              screen.lockTimeout_xrdb, NULL);
        ...
        /* run the current screen saver only if screen saver is turned on and a
         * valid saver exists*/
        if (XmToggleButtonGadgetGetState(screen.saverOn) && savers.saverCount &&
            !savers.saverstate) {
                XtVaSetValues(screen.saverArea, XmNbackground, savers.black,
                              NULL);

                savers.saverstate =
                    _DtSaverStart(style.display, &screen.saverArea, 1,
...
static void no_svr_ext_DtmapCB_screenDialog(Widget w, XtPointer client_data,
        ...
        XmToggleButtonGadgetSetState(screen.saverOn,
                                     screen.newTimeout ? True : False, False);
        ...
        /* run the current screen saver only if the useSaversOn toggle is
         * enabled and valid saver exist*/
        if (XmToggleButtonGadgetGetState(screen.useSaversOn) &&
            savers.saverCount && !savers.saverstate) {
                XtVaSetValues(screen.saverArea, XmNbackground, savers.black,
                              NULL);
                savers.saverstate =
                    _DtSaverStart(style.display, &screen.saverArea, 1,
...
static void timeOutvalueChangedCB(Widget w, XtPointer client_data,
        ...
        n = 0;
        XtSetArg(args[n], XmNvalue, &screen.saverTimeout);
        n++;
        XtGetValues(screen.timeOutScale, args, n);

        n = 0;
        XtSetArg(args[n], XmNvalue, &screen.lockTimeout);
        n++;
        ...
        else if (!XmToggleButtonGadgetGetState(screen.saverOn))
                screen.newTimeout = screen.lockTimeout * 60;
        else
                screen.newTimeout =
                    MIN(screen.saverTimeout, screen.lockTimeout) * 60;

        /* register new timeout with X */
        XSetScreenSaver(style.display, screen.newTimeout, screen.interval,
                        screen.new_blanking, screen.exposures);
...
/* saverToggleCB                         */
/*+++++++++++++++++++++++++++++++++++++++*/
static void saverToggleCB(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        register int n;
        Arg args[MAX_ARGS];
        Boolean set;
        ...
        if (w == screen.saverOff && set) {
                ...
                /* screen saver is disabled, the new timeout is now the value of
                 * the */
        ...
        if (w == screen.saverOn && set) {
                ...
                if (savers.saverCount) {
                        ...
                        XtManageChild(screen.prevLabel);
                        /* Restart the screen saver that is is currently
                         * selected */
                        if (!savers.saverstate) {
                                XtVaSetValues(screen.saverArea, XmNbackground,
                                              savers.black, NULL);
                                savers.saverstate = _DtSaverStart(
                                    style.display, &screen.saverArea, 1,
                                    savers.current_saver, style.screenDialog);
                        }
                        /* set the new timeout for the X server - note that
                         * since */
                        /* screen saver is enabled, the new timeout is now the
                         * value of the */
...
static void no_svr_ext_saverToggleCB(Widget w, XtPointer client_data,
        ...
        if (w == screen.saverOn && set) {
                 ...
                 */
                /* screen saver is enabled, the new timeout is now the value of
                 * the */
                /* saver slider */
                screen.newTimeout = screen.saverTimeout * 60;
        }
...
static void lockToggleCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (w == screen.lockOff && set) {
                ...
                XSetScreenSaver(style.display, screen.newTimeout,
                                screen.interval, screen.new_blanking,
                                screen.exposures);
        }
...
static void systemDefaultCB(Widget w, XtPointer client_data,
        ...
        n = 0;
        XtSetArg(args[n], XmNvalue, screen.cycleTimeout);
        n++;
        XtSetValues(screen.cycleTimeScale, args, n);
...
static void ButtonCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        case OK_BUTTON:
                ...
                if (style.smState.smSendSettings) {
                        if (screen.systemDefaultFlag) {
                                ...
                                SmNewScreenSettings(
                                    saverset || lockset ? screen.newTimeout : 0,
                                    screen.new_blanking, screen.interval,
                                    screen.exposures);
                        }
                }

                if (savers.saverCount) {
                        savers.selsaverActionNames = (char **)XtMalloc(
                            sizeof(char *) * savers.selsaverCount);

                        for (i = 0; i < savers.selsaverCount; i++) {
                                char *tmp = savers.saverActionNames
                                                [savers.selPositions[i] - 1];
                                savers.selsaverActionNames[i] =
                                    (char *)XtMalloc(strlen(tmp) + 1);
                        ...
                        savers.selsaversList = build_selectedList(
                            savers.selsaverActionNames, savers.selsaverCount);
                }
                ...
                free_saverList(savers.selsaverActionNames,
                               savers.selsaverCount);
                savers.selsaverActionNames = NULL;
                ...
                /* send new saver time, lock time and time per saver to SM if
                 * they have changed, */
                ...
                style.smState.smSaverTimeout =
                    saverset ? screen.saverTimeout * 60 : 0;
                style.smState.smLockTimeout =
                    lockset ? screen.lockTimeout * 60 : 0;
        ...
        case CANCEL_BUTTON:
                ...
                /* initialize the value of savertimeout and locktimeout and
                 * cycle time to the */
                /* current state or to the values retrieved from xrdb if the
                 * corresponding
                 */
                /* initial state is zero */
                if (style.smState.smSaverTimeout)
                ...
                XmToggleButtonGadgetSetState(
                    screen.saverOff,
                    !style.smState.smSaverTimeout ? True : False, True);
                XmToggleButtonGadgetSetState(
                    screen.lockOn, style.smState.smLockTimeout ? True : False,
                    True);
                XmToggleButtonGadgetSetState(
                ...
                XSetScreenSaver(style.display, screen.origTimeout,
                                screen.interval, screen.blanking,
                                screen.exposures);

                /* if timeMismatch is true, set the timeout to xTimeout so that
                 * the next time */
                /* the interface is mapped the correct timeouts show up */
                if (screen.timeMismatch)
                        XSetScreenSaver(style.display, screen.xTimeout,
                                        screen.interval, screen.blanking,
                                        screen.exposures);
                ...
                if (savers.saverCount) {
                        ...
                        for (i = 0; i < savers.orig_selsaverCount; i++)
                                XmListSelectPos(screen.saverList,
                                                savers.origSelPositions[i],
                                                False);
                        /* set the current saver to be the first saver in the
                         * selected list */
                        savers.current_saver =
                            savers.saverActionNames[savers.origSelPositions[0] -
                                                    1];
                        savers.current_saverDesc =
                            savers.saverActionDesc[savers.origSelPositions[0] -
                                                   1];
                        savers.current_position = savers.origSelPositions[0];
                }

                /* Need to make sure the savers.selPositions array reflects the
                   accurate current selections.  If the user invokes the Screen
                   dialog, then cancels, then reinvokes and OKs without making
                   any changes to the selections, the savers.selPositions array
                   is assumed to contain the accurate selections. Unless we
                   reset it here to the origSelPositions, it may be inaccurate.
                */
        ...
        case HELP_BUTTON:
                XtCallCallbacks(style.screenDialog, XmNhelpCallback,
                                (XtPointer)NULL);
                break;
...
static void no_svr_ext_ButtonCB(Widget w, XtPointer client_data,
        ...
        case CANCEL_BUTTON:
                ...
                /* reset the value of the saver toggles */
                XmToggleButtonGadgetSetState(
                    screen.saverOn, screen.origTimeout ? True : False, True);
                XmToggleButtonGadgetSetState(
                    screen.saverOff, !screen.origTimeout ? True : False, True);

                /* reset the value of the locking radio box toggles */
                if (style.smState.smSaverTimeout) {
                        XmToggleButtonGadgetSetState(screen.useSaversOn, True,
                                                     False);
                        XmToggleButtonGadgetSetState(screen.useSaversOff, False,
                                                     False);
                        XtSetSensitive(screen.saverList, True);
                        XtSetSensitive(screen.saverArea, True);
                        if (savers.selsaverCount > 1) {
                        ...
                        XmToggleButtonGadgetSetState(screen.useSaversOff, True,
                                                     False);
                        XmToggleButtonGadgetSetState(screen.useSaversOn, False,
                                                     False);
                        XtSetSensitive(screen.saverList, False);
                        XtSetSensitive(screen.saverArea, False);
                        XtSetSensitive(screen.cycleTimeLabel, False);
                ...
                */
                ...
                if (savers.orig_selsaverCount > 0) {
                        savers.selPositions = (int *)XtMalloc(
                            sizeof(int) * savers.orig_selsaverCount);

                        for (i = 0; i < savers.orig_selsaverCount; i++)
                                savers.selPositions[i] =
                                    savers.origSelPositions[i];
                }
...
/************************************************************************/
void restoreScreen(Widget shell, XrmDatabase db) {
        XrmName xrm_name[5];
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
                XtSetArg(save.posArgs[save.poscnt], XmNx,
                         atoi((char *)value.addr));
                save.poscnt++;
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
                XtSetArg(save.posArgs[save.poscnt], XmNy,
                         atoi((char *)value.addr));
                save.poscnt++;
        }
...
static void ListCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        savers.selPositions =
            (int *)XtMalloc(sizeof(int) * cb->selected_item_count);
        ...
        if (savers.selsaverCount < 2) {
                ...
                XtSetSensitive(screen.cycleTimeLabel, False);
                XtSetSensitive(screen.cycleMinutesLabel, False);
        } else if (savers.previous_selsaverCount < 2) {
                XtSetSensitive(screen.cycleTimeScale, True);
                XtSetSensitive(screen.cycleTimeLabel, True);
                XtSetSensitive(screen.cycleMinutesLabel, True);
        }
         ...
         */
        /* previewed. Therfore before a screen saver is started, its position
         * must */
        /* exist in the selected_item_positions array. If not then just return
         * without
         */
        /* altering the current (previewed) saver unless the deselected saver
         * was the
         */
        /* current saver. In that case kill the current saver and preview the
         * last */
        ...
        if (!runsaver) {
                ...
                if (savers.current_position == cb->item_position) {
                        if (savers.saverstate) {
                                /* Terminate the screen saver that is currently
                                 * running */
                                _DtSaverStop(style.display, savers.saverstate);
                                XSync(style.display, False);
                                savers.saverstate = NULL;
                        ...
                        savers.saverstate = _DtSaverStart(
                            ...
                            savers.current_saver, style.screenDialog);
                        string =
                            XmStringCreateLocalized(savers.current_saverDesc);
                        XtVaSetValues(screen.prevLabel, XmNlabelString, string,
                                      NULL);
                        XmStringFree(string);
                }
                return;
        ...
        savers.current_saver =
            savers.saverActionNames[savers.current_position - 1];
        savers.current_saverDesc =
            savers.saverActionDesc[savers.current_position - 1];
        savers.saverstate =
            _DtSaverStart(style.display, &screen.saverArea, 1,
                          savers.current_saver, style.screenDialog);
...
static void IconHandler(Widget shell, XtPointer client_data, XEvent *event,
        ...
        /* run the current screen saver only if the useSaversOn/saverOn toggle
           is enabled and valid saver exist and one is not already running*/
                ...
                if (style.smState.smLockOnTimeoutStatus) {
                        if (XmToggleButtonGadgetGetState(screen.saverOn) &&
                            savers.saverCount) {
                                ...
                                string = XmStringCreateLocalized(
                                    savers.current_saverDesc);
                                XtVaSetValues(screen.prevLabel, XmNlabelString,
                                              string, NULL);
                                XmStringFree(string);
                        }
                } else {
