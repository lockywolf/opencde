...
static void DrawBitmap(Widget w, XtPointer client_data, XtPointer call_data);
static void SizeBitmap(Widget w, XtPointer client_data, XtPointer call_data);
...
char **build_dirList(char *dirStr, int *count) {
        char tokenSep[] = ":";
        char *token;
        char **dirList = NULL;
        register int i = 0;
...
void free_dirList(char **dirList, int count)
 ...
 ************************************************************************/
void BackdropDialog(Widget parent) {
        int i;
...
#define MAXLOCALE 64 /* buffer size of locale name */
        ...
        bd_desc = (char *)XtMalloc(strlen("/usr/dt/backdrops/desc.") +
                                   strlen(lang) + 1);
        ...
        bd_desc = (char *)XtMalloc(strlen("/etc/dt/backdrops/desc.") +
                                   strlen(lang) + 1);
        ...
        bd_desc = (char *)XtMalloc(strlen(style.home) +
                                   strlen("/.dt/backdrops/desc.backdrops") + 1);
        ...
        save.poscnt++;
        style.backdropDialog = __DtCreateDialogBoxDialog(
            parent, BACKDROPSDLG, save.posArgs, save.poscnt);
        ...
        XtAddCallback(style.backdropDialog, XmNhelpCallback,
                      ...
                      (XtPointer)HELP_BACKDROP_DIALOG);
        ...
        XtSetArg(args[n], XmNtitle,
                 ((char *)GETMESSAGE(11, 12, "Style Manager - Backdrop")));
        n++;
        ...
        /*  get bitmap data */
        if (!ReadBitmaps())
                return 0; /* uses style.backdropDialog */
        ...
        n++;
        XtSetArg(args[n], XmNitems, listStrings);
        n++;
        XtSetArg(args[n], XmNitemCount, backdrops.numBitmaps);
        n++;
        list = XmCreateScrolledList(mainForm, "bitmapList", args, n);
        XtAddCallback(list, XmNbrowseSelectionCallback, ListCB,
                      (XtPointer)NULL);
        ...
        n++;
        XtSetArg(args[n], XmNshadowThickness, backdrops.shadow);
        n++;
        XtSetArg(args[n], XmNhighlightThickness, 0);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNrightWidget, XtParent(list));
        n++;
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNwidth, backdrops.width + 2 * backdrops.shadow);
        n++;
        XtSetArg(args[n], XmNheight, backdrops.height + 2 * backdrops.shadow);
        n++;
        XtSetArg(args[n], XmNtraversalOn, False);
        n++;
        backdrops.drawnButton =
            XmCreateDrawnButton(mainForm, "bitmap", args, n);
        XtAddCallback(backdrops.drawnButton, XmNexposeCallback, DrawBitmap,
                      NULL);
        XtAddCallback(backdrops.drawnButton, XmNresizeCallback, SizeBitmap,
                      NULL);
...
static void MoreBitmaps(void) {
        ...
        backdrops.bitmapNames =
            (char **)XtRealloc((char *)backdrops.bitmapNames, newSize);

        /* now allocate new bitmap space */
        newSize = (backdrops.maxNumBitmaps + 100) * sizeof(Pixmap);
        backdrops.bitmaps =
            (Pixmap *)XtRealloc((char *)backdrops.bitmaps, newSize);
...
static ReadBitmaps(void) {
        ...
        /* Parse the backdropDirectories resource to get the individual
         * directories */
        if (style.xrdb.backdropDir) {
                backdrops.dirList =
                    build_dirList(style.xrdb.backdropDir, &backdrops.dirCount);
        ...
        qsort(backdrops.tmpBitmapNames, backdrops.tmpNumBitmaps, sizeof(char *),
              cmpstringp);
...

{
        ...
        for (i = 0; i < backdrops.tmpNumBitmaps; i++) {
                tmpPixmap =
                    XmGetPixmap(style.screen, backdrops.tmpBitmapNames[i],
...

{
        ...
        while (filep != NULL) {
                ...
                if (!duplicate) {
                        ...
                        if (backdrops.tmpNumBitmaps ==
                            backdrops.tmpMaxNumBitmaps) {
                                /* allocate space for more temporary bitmap info
                                 */
                                newSize = (backdrops.tmpMaxNumBitmaps + 100) *
                                          sizeof(char *);
                                backdrops.tmpBitmapNames = (char **)XtRealloc(
                                    (char *)backdrops.tmpBitmapNames, newSize);
                                backdrops.tmpMaxNumBitmaps += 100;
                        }
...
static void DrawBitmap(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (backdrops.newColors) {
                ...
                /* we could keep track of which tile pixmaps need to be updated
                   since the last workspace change, but for now simply
                   regenerate each pixmap as it is selected after a workspace
                   change has occurred */
        ...
        XFillRectangle(style.display, XtWindow(w), backdrops.gc,
                       backdrops.shadow, backdrops.shadow, backdrops.width,
                       backdrops.height);
}
...
static XmString *MakeListStrings(void) {
        ...
        backdrops.bitmapDescs =
            (char **)XtCalloc(backdrops.numBitmaps, sizeof(char *));

        for (i = 0; i < backdrops.numBitmaps; i++) {
                if (bd_DB != NULL) {
                        name_str =
                            (char *)XtMalloc(strlen("backdrops.") +
                        ...
                        if (XrmGetResource(bd_DB, name_str, class_str,
                                           &str_type_return, &value_return)) {
                                /* make copy of resource value */
                                backdrops.bitmapDescs[i] =
                                    (char *)XtMalloc(value_return.size + 1);
                                strcpy(backdrops.bitmapDescs[i],
                                       value_return.addr);
                        } else {
                                backdrops.bitmapDescs[i] = (char *)XtMalloc(
                                    strlen(backdrops.bitmapNames[i]) + 1);
                                strcpy(backdrops.bitmapDescs[i],
                                       backdrops.bitmapNames[i]);
                        }
                } else {
                        backdrops.bitmapDescs[i] = (char *)XtMalloc(
                            strlen(backdrops.bitmapNames[i]) + 1);
...
static void ButtonCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        case B_HELP_BUTTON:
                XtCallCallbacks(style.backdropDialog, XmNhelpCallback,
                                (XtPointer)NULL);
                break;
...
static void GetColors(void) {
        ...
        if (backdrops.gc) /* update the gc if there is one */
        {
                ...
                /* allocate new pixmap */
                backdrops.bitmaps[backdrops.selected] = XmGetPixmap(
                    style.screen, backdrops.bitmapNames[backdrops.selected],
                    backdrops.fg, backdrops.bg);
 ...
 ************************************************************************/
void restoreBackdrop(Widget shell, XrmDatabase db) {
        XrmName xrm_name[5];
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
                XtSetArg(save.posArgs[save.poscnt], XmNwidth,
                         atoi((char *)value.addr));
                save.poscnt++;
        }
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
                XtSetArg(save.posArgs[save.poscnt], XmNheight,
                         atoi((char *)value.addr));
                save.poscnt++;
        }
