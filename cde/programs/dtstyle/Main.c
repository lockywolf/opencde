...
static void errParentMap(Widget w, XtPointer client_data, XEvent *event);
...
/* dialog box.                               */
/*+++++++++++++++++++++++++++++++++++++++++++*/
void raiseWindow(Window dialogWin) {
        static int changeMask = CWStackMode;
        static XWindowChanges windowChanges = {0, 0, 0, 0, 0, 0, Above};

        XReconfigureWMWindow(style.display, dialogWin, style.screenNum,
                             changeMask, &windowChanges);
}
 ...
 *
 ************************************************************************/
void CenterMsgCB(Widget w, XtPointer client_data, XtPointer call_data) {
        int n;
        Position newX, newY;
        Arg args[4];
        Widget shell;
...
/* be used                                                   */
/*************************************************************/
void ErrDialog(char *errString, Widget visualParent) {
        int n;
        Arg args[10];
        XmString ok;
        ...
        if (style.errDialog == NULL) /* create it */
        {
                ...
                n++;
                XtSetArg(args[n], XmNmessageString, style.tmpXmStr);
                n++;
                ...
                n++;
                style.errDialog =
                    XmCreateErrorDialog(style.shell, "ErrorNotice", args, n);

                XtAddCallback(style.errDialog, XmNokCallback, UnmanageCB, NULL);
                XtAddCallback(style.errDialog, XmNcancelCallback, UnmanageCB,
                              NULL);
                XtAddCallback(style.errDialog, XmNmapCallback, CenterMsgCB,
                              NULL);
        ...
        if (XtIsManaged(style.errParent) || XtParent(style.errParent) == NULL) {
                ...
                XtAddEventHandler(XtParent(style.errParent),
...
static void errParentMap(Widget w, XtPointer client_data, XEvent *event) {
        if (event->type == MapNotify) {
                ...
                /* ring the bell (PM behavior) */
                XBell(style.display, 0);
                XtRemoveEventHandler(XtParent(style.errParent),
                                     StructureNotifyMask, 0,
                                     (XtEventHandler)errParentMap, NULL);
        }
}
...
/*********************************************************/
void InfoDialog(char *infoString, Widget parent, Boolean unmapParent) {
        int n;
        Arg args[10];
        ...
        XtSetArg(args[n], XmNuseAsyncGeometry, True);
        n++;
        XtSetArg(args[n], XmNtitle, ((char *)GETMESSAGE(2, 2, "Notice")));
        n++;
...
/**********************************************************/
void putDialog(Widget parent, Widget dialog) {
        int n;
...
int main(int argc, char **argv) {
        ...
        n = 0;
        XtSetArg(args[n], XmNmwmFunctions, mwmFunc);
        n++;
        XtSetArg(args[n], XmNuseAsyncGeometry, True);
        n++;
        ...
        if (_DtGetLock(style.display, STYLE_LOCK) == 0) {
                _DtSimpleError(
                    progName, DtError, NULL, "%s",
                    ((char *)GETMESSAGE(2, 5,
                                        "Style Manager is already running, "
        ...
        case LOW_RES_DISPLAY:
                style.horizontalSpacing = style.verticalSpacing = 3;
                break;

        case MED_RES_DISPLAY:
                style.horizontalSpacing = style.verticalSpacing = 5;
                break;

        case HIGH_RES_DISPLAY:
                style.horizontalSpacing = style.verticalSpacing = 8;
                break;
        }
        ...
        string = (char *)XtMalloc(strlen(style.home) +
                                  strlen("/.dt/backdrops:") + 1);
        sprintf(string, "%s/.dt/backdrops:", style.home);

        dirs =
            (char *)XtCalloc(1, strlen("/etc/dt/backdrops:/usr/dt/backdrops") +
                                    (style.xrdb.backdropDir == NULL
                                         ? 2
        ...
        _DtWsmSetBackdropSearchPath(style.screen, dirs,
                                    style.useMultiColorIcons);
 ...
 *
 ************************************************************************/
static int IOErrorHandler(display) Display *display;
...
static Boolean NewCreateD(XtPointer shell) {
        ...
        printf(
            "CreateDialogBoxD elapsed time is %ld seconds, %ld microseconds\n",
            second.tv_sec - first.tv_sec, second.tv_usec - first.tv_usec);
#endif

        XtAppAddWorkProc(XtWidgetToApplicationContext((Widget)shell), NewAddTo,
                         shell);
...
static Boolean NewAddTo(XtPointer shell) {
        ...
        XtAppAddWorkProc(XtWidgetToApplicationContext((Widget)shell),
                         NewCreateTop1, shell);
...
static Boolean NewCreateTop1(XtPointer shell) {
        ...
        printf(
            "CreateTopColor1 elapsed time is %ld seconds, %ld microseconds\n",
            second.tv_sec - first.tv_sec, second.tv_usec - first.tv_usec);
#endif
...
static Boolean NewCreateTop2(XtPointer shell) {
        ...
        printf(
            "CreateTopColor2 elapsed time is %ld seconds, %ld microseconds\n",
            second.tv_sec - first.tv_sec, second.tv_usec - first.tv_usec);
#endif
...
static Boolean NewReadPal(XtPointer shell) {
        ...
        if (ReadPaletteLoop(True)) {
                ...
                printf("ReadPaletteLoop DONE elapsed time is %ld seconds, %ld "
                       "microseconds\n",
        ...
        printf("ReadPaletteLoop NOT DONE elapsed time is %ld seconds, %ld "
               "microseconds\n",
               second.tv_sec - first.tv_sec, second.tv_usec - first.tv_usec);
#endif
...
static Boolean NewInitPal(XtPointer shell) {
        ...
        if (InitializePaletteList((Widget)shell, paletteList, True)) {
                ...
                printf("InitializePaletteList DONE elapsed time is %ld "
                       "seconds, %ld "
                       "microseconds\n",
                       second.tv_sec - first.tv_sec,
                       second.tv_usec - first.tv_usec);
        ...
        printf(
            "InitializePaletteList NOT DONE elapsed time is %ld seconds, %ld "
            "microseconds\n",
            second.tv_sec - first.tv_sec, second.tv_usec - first.tv_usec);
#endif
...
static Boolean NewAllocColor(XtPointer shell) {
        ...
        printf("AllocatePaletteCells elapsed time is %ld seconds, %ld "
               "microseconds\n",
               second.tv_sec - first.tv_sec, second.tv_usec - first.tv_usec);
#endif
...
static Boolean NewBottomColor(XtPointer shell) {
        ...
        printf(
            "CreateBottomColor elapsed time is %ld seconds, %ld microseconds\n",
            second.tv_sec - first.tv_sec, second.tv_usec - first.tv_usec);
#endif
...
static Boolean NewCreateButtons(XtPointer shell) {
        ...
        printf("CreatePaletteButtons elapsed time is %ld seconds, %ld "
               "microseconds\n",
               second.tv_sec - first.tv_sec, second.tv_usec - first.tv_usec);
#endif
