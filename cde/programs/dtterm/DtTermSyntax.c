...
static char rcs_id[] =
    "$XConsortium: DtTermSyntax.c /main/4 1996/05/16 11:22:57 ageorge $";
...
static char *options_default[] = {
    "-/+132				enable/disable 80<->132 column "
    "escape seq",
    "-/+aw				enable/disable autowrap",
    "-/+bs				turn off/on Term background is "
    "select color",
    "-display displayname		X server to contact",
    "-e command args		command to execute",
    "-fb fontset			bold text font",
    "-fn fontset			normal text font",
    "-geometry geom			size (in characters) and "
    "position of window",
    ...
    "-/+map				enable/disable map window on "
    "pty output",
    "-/+mb				enable/disable margin bell",
    "-ms color			pointer color",
    "-n string			specify icon name",
    "-name string			client instance, icon, and "
    "title strings",
    "-nb distance			specify distance for right "
    "margin bell",
    "-/+rw				enable/disable reverse wrap",
    "-/+sb				enable/disable scroll bar",
    "-/+sf				enable/disable SUN function "
    "keys",
    "-sl number[s]			number of scrolled lines "
    "[screens] to save",
    "-ti name			string used for programmatic "
    "identification",
    ...
    "-xrm resourcestring		additional resource "
    "specifications",
    "-C				console mode",
    "-Sxxd				slave mode on \"ttyxx\" file "
    "descriptor \"d\"",
    "-Sxxx.d			slave mode on \"ttyxxx\" file "
    "descriptor \"d\"",
    "End-Of-List",
};
...
static void GetUsage() {
        ...
        for (i = 1;; i++) {
                ...
                */
                if (!optHead) {
                        optHead =
                            (struct _options *)malloc(sizeof(struct _options));
                        optPtr = optHead;
                } else {
                        optPtr->next =
                            (struct _options *)malloc(sizeof(struct _options));
...
void Syntax(char *programName, char *badOption) {
        ...
        */
        ...
        for (optPtr = optHead; optPtr; optPtr = optPtr->next) {
                /*DKS*DKS*DKS*
                ** the following 3 assumes that the msg_catalog doesn't add more
                *than
...
void Help(char *programName) {
        ...
        */
        ...
        (void)fprintf(stderr, "%s",
                      (GETMESSAGE(NL_SETN_Syntax, 52, "usage:\n")));
        (void)fprintf(stderr,
                      (GETMESSAGE(NL_SETN_Syntax, 53,
                                  "\t%s [-options ...] [-e command args]\n\n")),
                      programName);
        (void)fprintf(
            stderr, "%s",
            (GETMESSAGE(NL_SETN_Syntax, 54, "where options include:\n")));
        for (optPtr = optHead; optPtr; optPtr = optPtr->next)
                if (strlen(optPtr->opt) > (size_t)width)
                        width = strlen(optPtr->opt);
