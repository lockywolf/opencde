...
static char rcs_id[] =
    "$TOG: DtTermServer.c /main/5 1998/07/23 18:09:38 mgreess $";
...
static void Initialize(Widget topLevel, int argc, char **argv, char *serverId);

static void clientMessageProc(DtSvcHandle service, Pointer clientData,
                              String *messageFields, int numFields);

static void serverRequestProc(DtSvcHandle service, DtSvcMsgContext replyContext,
                              Pointer clientData, String *messageFields,
                              int numFields);

static void serverMessageProc(DtSvcHandle service, Pointer clientData,
                              String *messageFields, int numFields);

static void Ping(XtPointer clientData, XtIntervalId *id);
...
static void Initialize(Widget topLevel, int argc, char **argv, char *serverId) {
        ...
        (void)sprintf(buffer, "%s-%s-%ld", DTTERM_SVC_CLASS, hostname,
                      (long)getuid());
        ...
        if (DT_SVC_SUCCESS ==
            _DtSvcRegister(serviceHandle, False, serverRequestProc,
                           (XtPointer)SVC_REQUEST, serverMessageProc,
                           (XtPointer)SVC_LOSE)) {
                ...
                if (!isDebugSet('n')) {
                        for (i = 0; (i < 10) && ((pid = fork()) < 0); i++) {
                                /* if we are out of process slots, then let's
                ...
                if (pid < 0) {
                        ...
                        /* parent.  Let's clean up, restart, and let the new
                         * process try again...
                         */
                 ...
                 */
                /* set the iAmTheServer flag to True.  This flag will remain
                 * True until we lose ownership of the service...
                 */
                iAmTheServer = True;
                ...
                for (i = 0;
                     i < (sizeof(trapSignalList) / sizeof(trapSignalList[0]));
                     i++) {
                        (void)sigaction(trapSignalList[i], &sa,
                                        (struct sigaction *)0);
                }
                ...
                (void)_DtSvcNotifyGroupRegister(
                    serviceHandle, serverMessageProc, (XtPointer)SVC_NOTIFY);
                ...
                /* we are not the server.  We need to register for messages.
                 * Then we are done and the service world is now for us...
                 */
                (void)_DtSvcNotifyGroupRegister(
                    serviceHandle, clientMessageProc, (XtPointer)SVC_NOTIFY);
        }

        /* install a ping timeout... */
        if (PingInterval > 0) {
                pingId = XtAppAddTimeOut(XtWidgetToApplicationContext(topLevel),
                                         1000 * 60 * PingInterval, Ping,
                                         (XtPointer)topLevel);
        }
...
static void NiceCleanUp(XtPointer clientData, XtIntervalId *id) {
        ...
        for (serviceClientInfo = serviceClientInfoHead->next; serviceClientInfo;
             serviceClientInfo = serviceClientInfo->next) {
                ...
                (void)_DtSvcNotifySend(
                    serviceHandle, DTTERM_SVC_TERMINATION_MSG, args, argcnt);
...
static void serverRequestProc(DtSvcHandle service, DtSvcMsgContext replyContext,
        ...
        case SVC_REQUEST:
                if (!strcmp(messageFields[0], DTTERM_SVC_START_MSG)) {
                        ...
                        argcnt = 0;
                        (void)XtSetArg(arglist[argcnt], XmNallowShellResize,
                                       True);
                        argcnt++;
                        shellWidget = XtAppCreateShell(
                            (char *)0, "Dtterm", applicationShellWidgetClass,
                            XtDisplay((Widget)refWidget), arglist, argcnt);

                        /* parse off messageFields and build the dttermview
                         * arglist... */
                        argcnt = 0;
                        ...
                        for (i2 = 1; i2 < numFields; i2++) {
                                if (!strcmp(messageFields[i2], "-pid")) {
                                        ...
                                        (void)i2++;
                                        if (i2 < numFields) {
                                                /* DKS: somewhere we will need
                                                 * to free this... */
                                                commandToExecute =
                                                    (char **)XtMalloc(
                                                        (numFields - i2 + 1) *
                                                        sizeof(char *));
                                                for (i1 = 0; i2 < numFields;
                                                     i1++, i2++) {
                                                        commandToExecute[i1] =
                                                            messageFields[i2];
                                                }
                        ...
                        /* create the dtterm... */
                        (void)CreateInstance(shellWidget, "Dtterm", arglist,
                                             argcnt);

                        /* create the ServiceClientInfoRec for this instance...
                         */
                        serviceClientInfo = (ServiceClientInfo)XtMalloc(
                            sizeof(ServiceClientInfoRec));
                        ...
                        if (serviceClientInfoHead->next) {
                                serviceClientInfoHead->next->prev =
                                    serviceClientInfo;
                        }
                        ...
                        reply[i2] = buffer;
                        i2++;
                        (void)_DtSvcRequestReply(serviceHandle, replyContext,
                                                 reply, i2, True);
                }
        }
}
...
static void clientMessageProc(DtSvcHandle service, Pointer clientData,
        ...
        case SVC_NOTIFY:
                ...
                if (!strcmp(messageFields[0], DTTERM_SVC_TERMINATION_MSG)) {
                        if (numFields >= 2) {
                                pid = (pid_t)strtol(messageFields[1],
                                                    (char **)0, 0);
                        } else {
                                pid = -1;
                        }
        ...
        case SVC_FAIL:
                ...
                *buffer = '\0';
                ...
                ServerFailureMessage =
                    XtRealloc(ServerFailureMessage, strlen(buffer));
...
Boolean ServerStartSession(Widget topLevel, int argc, char **argv,
        ...
        if (commandToExec && *commandToExec) {
                ...
                argcnt++;
                for (i1 = 0; commandToExec[i1]; i1++) {
                        args[argcnt] = commandToExec[i1];
                        argcnt++;
                }
        }

        if (DT_SVC_FAIL ==
            _DtSvcRequestSend(serviceHandle, DTTERM_SVC_START_MSG, args, argcnt,
                              clientMessageProc, (XtPointer)SVC_SUCCESS,
                              clientMessageProc, (XtPointer)SVC_FAIL)) {
...
void ServerInstanceTerminated(Widget w) {
        ServiceClientInfo serviceClientInfo;
        char buffer[BUFSIZ];
        String args[20];
        int argcnt;
        ...
        if ((--InstanceCount <= 0) && (!iAmTheServer)) {
            ...
            if ((InstanceCount <= 0) && !waitId) {
                 ...
                 * we exit...
                 */
                waitId = XtAppAddTimeOut(
                    XtWidgetToApplicationContext(refWidget),
...
static void Ping(XtPointer clientData, XtIntervalId *id) {
        ...
        (void)XQueryPointer(XtDisplay(w), XtWindow(w), &root, &child, &rootX,
