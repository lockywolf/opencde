...
void TestIconify(WinName, String, TestNum) char *WinName, *String;
int TestNum;
{
        ...
        Deiconify(WinName);
        if (WaitWinMap(WinName) < 0)
                LogError("Deiconify did not work");
...
void TestIconifyMap(WinName, String, TestNum) char *WinName, *String;
int TestNum;
{
        char Str[IMAGE_FILE_LEN];
        sprintf(Str, "%sriconic%d", IMAGE_DIR, TestNum);
        ...
        if (TestNum == 3) {
                if (WaitWinMap(WinName) < 0)
                        LogError("mapOnOutput: True did not work");
                PrintTermString(WinName, String);
                sprintf(Str, "%sriconic%dA", IMAGE_DIR, TestNum);
                MatchWindows(WinName, Str);
                ...
                if (WaitWinMap(WinName) == 0)
                        LogError("mapOnOutput:False did not work");
                else {
                        Deiconify(WinName);
                        if (WaitWinMap(WinName) < 0)
                                LogError("Deiconify did not work");
...
static char *LogAction[] = {
    "Testing Option  iconic True ", "Testing Option  iconic True iconName Test",
...
char *argv[];
{
        ...
        for (i = 0; i < ArrCount; i++) {
                ...
                if (i > 1)
                        strcat(Command, " -e wait5write");
                ExecCommand(Command);
                if (WaitWinMap("TermWin") < 0)
                        continue;
                AssignWinName("TermWin", TERM_EMU);
                if (i <= 1)
                        TestIconify("TermWin", OptionArr[i], i + 1);
                else
                        TestIconifyMap("TermWin", OptionArr[i], i + 1);
