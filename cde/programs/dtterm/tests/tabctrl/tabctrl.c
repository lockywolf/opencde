...
void TestTabCtrl(WinName) char *WinName;
{
        ...
        for (i = 0; i < TabCount; i++) {
                CursorForward(TabSpace);
                SetTab();
        }
        HomeUp();
        for (k = 0; k < Lines; k++) {
                for (j = 0; j < TabCount; j++) {
                        sprintf(Str, "*%c", HORI_TAB);
                        WRITETEST(Str);
#ifdef SLOW
                        sleep(1);
#endif
                ...
                if (k < (Lines - 1)) {
                        switch ((k % 3)) {
                        case 0:
                                sprintf(Str, "%c", NEWLINE);
                                WRITETEST(Str);
                                break;
                        case 1:
                                sprintf(Str, "%c%c", CARRI_RET, VERTI_TAB);
                                WRITETEST(Str);
                                break;
                        case 2:
                                sprintf(Str, "%c%c", CARRI_RET, NEWPAGE);
                                WRITETEST(Str);
                                break;
                        }
                }
