...
static char *DispAttArr[] = {"Normal", "Bold", "Underscore", "Inverse",
                             "Blink"};

static SetDispAttr(WhichAttr, NewLine, BlinkOn) int WhichAttr;
...
void TestDispAtt(WinName) char *WinName;
{
...
void TestCursor(WinName) char *WinName;
{
        char Str[IMAGE_FILE_LEN];
        SetScrollRegn(5, 15);
        SetDispBold();
        ...
        ClearScreen();
        WRITETEST("Now In Normal Char Att; Cursor Mode; in Line 1; with no "
                  "wrap and on restoring cursor");
        NextLine();
        WRITETEST(
            "You did not see full line because of no wrap;On Restoring Cursor");
        RestoreCursor();
        WRITETEST("I Should start at (46,14) with char attribute Bold Inverse "
                  "Underline, Origin mode set, scrolling region is 5,15 with "
                  "wrap mode on");
#ifdef SYNLIB
