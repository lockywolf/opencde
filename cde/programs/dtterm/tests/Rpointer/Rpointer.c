...
void TestPointer(WinName, String, TestNum, Delay) char *WinName, *String;
int TestNum, Delay;
{
...
static char *ResourceArr[] = {
    /*0*/ "pointerBlank True pointerBlankDelay 5 pointerColor red "
          "pointerColorBackground black pointerShape hand1",
    /*1*/ "pointerBlank False pointerBlankDelay 5 pointerColor blue "
          "pointerColorBackground greem pointerShape hand2"};

static char *LogAction[] = {
    "Testing Resource Option: pointerBlank True pointerBlankDelay 5 "
    "pointerColor red pointerColorBackground black pointerShape hand1",
    "Testing Resource Option: pointerBlank False pointerBlankDelay 5 "
    "pointerColor blue pointerColorBackground greem pointerShape hand2"};

#define ArrCount (int)(sizeof(ResourceArr) / sizeof(char *))

int MakeResourceFile(ResArr) char *ResArr;
{
        FILE *ResFile;
        int i, j, Len;
        ...
        if ((ResFile = fopen("res", "w")) == NULL) {
                LogError("Resource File Creation fail");
                return (-1);
        }
        for (i = 0; i < 50; i++)
                Resource[i] = BLANK;
