...
void TestScrollRegn(Lines, Cols, ScrollTop, ScrollBottom, WinName,
                    ImageFile) int Lines,
    Cols, ScrollTop, ScrollBottom;
...
void TestReverseWrap(Lines, Cols, WinName, ImageFile) int Lines, Cols;
char *WinName, *ImageFile;
{
        char Str[IMAGE_FILE_LEN];
...
void TestReverseVideo(Lines, Cols, WinName, ImageFile) int Lines, Cols;
char *WinName, *ImageFile;
{
        char Str[IMAGE_FILE_LEN];
        SetOriginMode(DECModeReset);
...
void WriteTwiceCols(Cols) int Cols;
{
        int i;
        for (i = 0; i < (2 * Cols); i++)
                if (i <= Cols)
...
void TestDECMode(WinName) char *WinName;
{
        ...
        if (GotX != Cols)
                LogError("WrapMode Off failed");
        else
                LogError("WrapMode Off Success");
        NextLine();
        WriteTwiceCols(Cols);
        GetCursorPosn(&GotX, &GotY);
        if (GotX != Cols)
