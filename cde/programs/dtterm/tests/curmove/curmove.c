...
void TestCurMove(WinName) char *WinName;
{
        ...
        while (1) {
                ...
                if ((CurrX == PrevX) && (CurrY == PrevY))
                        continue;
                if (Relative == 1)
                        RelativeGoToXY(PrevX, PrevY, CurrX, CurrY);
                else if (Relative == 2)
                        AbsoluteGoToXY(CurrX + 1, CurrY + 1);
                else
                        AbsoluteGoToXY_HVP(CurrX + 1, CurrY + 1);
                Arr[CurrX][CurrY] = 1;
                if (CurrX != (NumCols - 1)) {
                        WRITETEST("0");
                        CursorBack(1);
                ...
                if ((CurrX != (GotX - 1)) || (CurrY != (GotY - 1))) {
                        if (Relative == 1)
                                ...
                                printf("Absolute:");
                        sprintf(LogStr,
                                "Expeted X Y: %3d  %3d Got X Y: %3d  %3d \n",
...
void TestCurColLine(WinName) char *WinName;
{
        int i, Cols, Lines;
        char Str[STARLEN];
        START(1, 0, 0, 0, 0);
