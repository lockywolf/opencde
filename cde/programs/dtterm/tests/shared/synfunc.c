...
char *MemAllocate(Size) int Size;
{
        char *P;
        if (Size <= 0) {
...
void MemRelease(P) char *P;
{
        if (P == NULL) {
                LogError("Invalid memory address for release");
                return;
        }
        free(P);
}

int FileExists(FileName) char *FileName;
{
...
void InitTest(argc, argv) int argc;
char *argv[];
{
...
CloseTest(exitDialog) int exitDialog;
{
        Window Win;
        SynStatus Result;
...
void MatchWindows(WinName, ImageFile) char *WinName, *ImageFile;
{
        int Result;
        SynStatus SynResult;
       ...
       if (Result ==  ENOENT) 
        ...
        Expected =
            MemAllocate((int)(strlen(ImageFile) + strlen(EXPECTED_SUFFIX)));
        Actual = MemAllocate((int)(strlen(ImageFile) + strlen(ACTUAL_SUFFIX)));
        strcpy(Expected, ImageFile);
        strcat(Expected, EXPECTED_SUFFIX);
        strcpy(Actual, ImageFile);
        strcat(Actual, ACTUAL_SUFFIX);
        if ((SynResult = SynCompareWindowImage(test.display, WinName, ImageFile,
                                               Expected, Actual)) !=
            SYN_SUCCESS) {
                sprintf(LogStr, "ImageComparison Failed ImageFile <%s> \
          Expected <%s> Actual <%s>",
                        ImageFile, Expected, Actual);
                LogError(LogStr);
        } else {
...
int AssignWinName(WinName, TitleName) char *WinName, *TitleName;
{
        SynStatus Result;
        Window Win, *SelList;
        int SelCount;
        ...
        if (Result == SYN_SUCCESS) {
                sprintf(LogStr, "SynSucc SelCount %d ", SelCount);
                LogError(LogStr);
        } else if (Result == SYN_NAME_COLLISION) {
                sprintf(LogStr, "Window Collision ");
                LogError(LogStr);
        } else if (Result == BAD_SYN_WINDOW) {
                sprintf(LogStr, "SynFail");
                LogError(LogStr);
                return (-1);
        }
        return (0);
...
void PrintTermString(WinName, String) char *WinName, *String;
{
        int i;
...
void ExecCommand(Command) char *Command;
{
        sleep(5);
...
void ExecWinCommand(WinName, Command) char *WinName, *Command;
{
        SynMovePointer(test.display, WinName, "LocTerm1");
        SynPressAndHoldKey(test.display, "<Control_L>");
        SynPrintString(test.display, "C");
...
void PressPrevKey(WinName) char *WinName;
{
        SynClickKey(test.display, "<Prior>");
        SynFlushInputBuffer(test.display);
}

void PressCtrlGKey(WinName) char *WinName;
{
        SynPressAndHoldKey(test.display, "<Control_L>");
...
void PressKeyNtimes(WinName, Key, N) char *WinName, *Key;
int N;
{
        int i;
        for (i = 0; i < N; i++)
...
void PressBackSpace(WinName) char *WinName;
{
        SynClickKey(test.display, "<BackSpace>");
        SynFlushInputBuffer(test.display);
}

void CloseTerm(WinName) char *WinName;
{
        SynMovePointer(test.display, WinName, "LocTerm1");
        SynPressAndHoldKey(test.display, "<Control_L>");
        SynPrintString(test.display, "C");
...
void JustExit(WinName) char *WinName;
{
        SynMovePointer(test.display, WinName, "LocTerm1");
        SynPrintString(test.display, "exit");
        SynClickKey(test.display, "<Return>");
...
int WaitWinMap(WinName) char *WinName;
{
        SynStatus Result;
        Result = SynWaitWindowMap(test.display, WinName, WIN_WAIT_TIME);
        if (Result == SYN_TIME_OUT) {
                LogError("term window did not come up");
                return (-1);
        }
        return (0);
}

void WaitWinUnMap(WinName, WaitTime) char *WinName;
long WaitTime;
{
        SynStatus Result;
        Result = SynWaitWindowUnmap(test.display, WinName, WaitTime);
        if (Result == BAD_SYN_WINDOW)
                LogError("Named  window does not exist");
}
...
void CheckCapsLock() {
        ...
        unsigned int Mask;
        Window A, B;
        if (XQueryPointer(test.display, DefaultRootWindow((test.display)), &A,
                          &B, &X, &Y, &X1, &Y1, &Mask) == True) {
                if ((Mask & LockMask) == LockMask) {
                        SynClickKey(test.display, "<Caps_Lock>");
                        SynFlushInputBuffer(test.display);
                }
        } else
                LogError("QueryPointer failed ");
}

void Deiconify(WinName) char *WinName;
{
...
void TestMenu(WinName) char *WinName;
{
