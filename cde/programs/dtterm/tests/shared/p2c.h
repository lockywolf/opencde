...
extern Void TimeStamp PP((int *Day, int *Month, int *Year, int *Hour, int *Min,
                          int *Sec));
extern Void P_sun_argv PP((char *, int, int));

/* I/O error handling */
#define _CHKIO(cond, ior, val, def)                                            \
        ((cond) ? P_ioresult = 0, (val) : P_ioresult = (ior), (def))
...
/* File buffers */
#define FILEBUF(f, sc, type)                                                   \
        sc int __CAT__(f, _BFLAGS);                                            \
...
#define APUTFBUF(f, type, v)                                                   \
        (memcpy(GETFBUF(f, type), (v), sizeof(__CAT__(f, _BUFFER))))
...
#define Malloc(n)                                                              \
        ((__MallocTemp__ = malloc(n)) ? __MallocTemp__ : (Anyptr)_OutMem())
#endif
...
#define P_getbits_UB(a, i, n, L)                                               \
        ((int)((a)[(i) >> (L) - (n)] >>                                        \
                   (((~(i)) & ((1 << (L) - (n)) - 1)) << (n)) &                \
               (1 << (1 << (n))) - 1))

#define P_getbits_SB(a, i, n, L)                                               \
        ((int)((a)[(i) >> (L) - (n)]                                           \
               << (16 - ((((~(i)) & ((1 << (L) - (n)) - 1)) + 1) << (n)) >>    \
                   (16 - (1 << (n))))))

#define P_putbits_UB(a, i, x, n, L)                                            \
        ((a)[(i) >> (L) - (n)] |=                                              \
         (x) << (((~(i)) & ((1 << (L) - (n)) - 1)) << (n)))

#define P_putbits_SB(a, i, x, n, L)                                            \
        ((a)[(i) >> (L) - (n)] |=                                              \
         ((x) & (1 << (1 << (n))) - 1)                                         \
         << (((~(i)) & ((1 << (L) - (n)) - 1)) << (n)))

#define P_clrbits_B(a, i, n, L)                                                \
        ((a)[(i) >> (L) - (n)] &=                                              \
         ~(((1 << (1 << (n))) - 1)                                             \
           << (((~(i)) & ((1 << (L) - (n)) - 1)) << (n))))

/* small packed arrays */
#define P_getbits_US(v, i, n)                                                  \
        ((int)((v) >> (~(i) << (n)) & (1 << (1 << (n))) - 1))
#define P_getbits_SS(v, i, n)                                                  \
        ((int)((long)(v) << (32 - (((~(i)) + 1) << (n))) >> (32 - (1 << (n)))))
#define P_putbits_US(v, i, x, n) ((v) |= (x) << (~(i) << (n)))
#define P_putbits_SS(v, i, x, n)                                               \
        ((v) |= ((x) & (1 << (1 << (n))) - 1) << (~(i) << (n)))
#define P_clrbits_S(v, i, n)                                                   \
        ((v) &= ~(((1 << (1 << (n))) - 1) << (~(i) << (n))))
