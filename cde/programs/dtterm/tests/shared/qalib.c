...
Static void PAUSE(double Sec) { sleep((unsigned int)Sec); }

Static void SETSTRLEN(Char *S, short E) { S[(long)((int)E)] = '\0'; }

Static void STRAPPEND(Char *S1, Char *S2) {
        if ((int)strlen(S1) + (int)strlen(S2) <= 255)
                strcat(S1, S2);
}

Static void STRDELETE(Char *S, short P, short N) {
...
/* STRINSERT and strinsert both not supported yet, Do not understand
   the intent of them.
*/
Static void STRINSERT(Char *S1, Char *S2, short P) {
...
/* Starting at "start_pos" from string "input", copy the results into
   "Result".
*/
Static Char *strsub(Char *Result, Char *input, short start_pos, short len) {
        Char STR1[256];
        Char *tmp;
        int i;
...
Static void STRMOVE(short N, Char *S1_, short P1, Char *S2, short P2) {
...
Static Char *STRRPT(Char *Result, Char *S, short N) {
        Char St[256];
        short I;
...
Static short STRLEN(Char *S) { return ((short)strlen(S)); }
...
/* this function only supports returning position of one char (not
   ...
   return.                                                         */
Static short strpos2(Char *S1, Char *S2, short num) {
        Char STR1[256];
        short found = 0;
        short done = 0;
        short index = 0;
...
Static short STRPOS(Char *S1, Char *S2) { return strpos2(S2, S1, 1); }

Static Char *STRLTRIM(Char *Result, Char *S_) {
        Char S[256];
        boolean Done;
        Char *tmp;
        strcpy(S, S_);
...
Static Char *STRRTRIM(Char *Result, Char *S_) {
        Char S[256];
        boolean Done;
...
Static Char *UPCASE_STRING(Char *Result, Char *S_) {
        Char S[256];
        short I, FORLIM;
...
Static Char *TOHEXSTR(Char *Result, short Num) {
...
Static void SUBTIME(__TIME Time2, __TIME Time1, __TIME *Time_Out) {
...
Static void GET_TIME(__TIME *Cur_Time) { ; }

Static void COM_INT_ROUTINE(void) {
        uchar Com_Num, In_Char;
        boolean Use_It;
        ;
}

Static void RS232_CLEANUP(uchar Com) { ; }

/*Static */ void DONE(void) { (void)cleanup(0); }

Static void __ERROR(short Num, short Where) {
        Char STR2[256];
...
Static void SET_DATA_COM(__BAUD_TYPE Speed, uchar Parity, uchar Stop,
                         uchar Data_Bits, uchar Com) {
...
/* Only supporting test terminal (no reference terminal).  If support
   ...
   changes.                                                           */
Static void FLUSH_BUFFER_COM(short Com) {
...
Static void FLUSH_BUFFERS(void) { FLUSH_BUFFER_COM(1); }

Static short READY(uchar Com, short Time_Out) { ; }
...
Static void __WRITE_BOTH(Char Ch) {
        Char STR1[256];
...
Static void INIT_DEFAULTS(int Echo) {
        ...
        */
        ...
        if (Echo)
                ECHO_COM("ON"); /* echoing on     */
        else
                ECHO_COM("OFF"); /* echoing off     */
}

Local boolean CHECK_COM(uchar Test) { return true; }

Static boolean PORT_ASSIGNMENTS_OK(uchar Test, uchar Ref, uchar Aux_A,
                                   uchar Aux_B) {
        boolean Ok;
...
Local Char *GET_PROG_NAME(Char *Result, struct LOC_GET_PARAMS *LINK) {
        short Addres, I, J;
        Char Path[256];
...
*/
Local boolean PUT_NUM(uchar *Default, Char *Buffer,
                      struct LOC_GET_PARAMS *LINK) {
        boolean Ok;
...
*/
Local void GET_COMMAND_LINE(short *I, Char *Current_,
                            struct LOC_GET_PARAMS *LINK) {
        Char Current[256];
        Char STR1[256];
        short FORLIM;
        ...
        for (LINK->LINK->J = 2; LINK->LINK->J <= FORLIM; LINK->LINK->J++) {
                if (Current[LINK->LINK->J - 1] == ' ' &&
                    Current[LINK->LINK->J - 2] != ' ')
                        (*I)++;
        }
...
Local void GET_PARAMS(struct LOC_START *LINK) {
    ...
    sprintf(__File_Name, "%s.LOG", GET_PROG_NAME(STR1, &V));
        ...
        *__Command_Line = '\0';
        ...
        if (Bad_Params)
                MODULE_ERR(
                    "Bad parameter line.  Please see manual for information");
...
/*Static */ void START(uchar Test, uchar Ref, uchar Aux1, uchar Aux2,
                       int Echo) {
        ...
        if (!PORT_ASSIGNMENTS_OK(Test, Ref, Aux1, Aux2))
                MODULE_ERR("Invalid ports specified in START.  See manual for "
                           "details");
        __Manual = false;
        ...
        if (__Log_Msg) {
                ...
                if (__Message_File != NULL)
                        __Message_File =
                            freopen(__Message_File_NAME, "w", __Message_File);
                else
                        __Message_File = fopen(__Message_File_NAME, "w");
                _SETIO(__Message_File != NULL, FileNotFound);
                if (P_ioresult != 0)
                        MODULE_ERR(
                            "Illegal log file name specified OR not DOS 3.x");
        }
        INIT_DEFAULTS(Echo);
}
...
*/
Static void LOG_IT(char *S, short Howmany, Char Which_Way) {
        short I, Count;
        Char Who[256];
        Char S1[3];
        boolean Ok;
...
Local Char READ_CHAR(void) { ; }

Static boolean YESNOKEY(Char *Prompt_, boolean Default) { ; }

#undef display_row
#undef line_length

Static short ASCIIINT(Char *S, short *Er) {
        short I;
...
Static Char *INTASCII(Char *Result, short Num) {
        Char St[256];
...
Static char *READ_COM(int fd, char *terminator) {
        char *c;
        char STR1[BUFSIZ];
    ...
    if(!inh_hndshk && inh_DC2)
        ...
        */
        ...
        if (autolf_mode)
                STR1[(int)strlen(STR1) - 2] =
                    '\0'; /* remove "CR and new line  */
        else
                STR1[(int)strlen(STR1) - 1] =
                    '\0'; /* just remove terminating char */
        return (STR1);
}
...
/*Static */ char *READTEST(char *Terminator) {
        char *tmp;
        char STR[BUFSIZ];
...
Static short STRCOMP(Char *S1, Char *S2) {
        short I, J;
...
Static void INTERM_COM(uchar Com, Char *S) { strcpy(Input_Terminator, S); }

Static void INTRIG_COM(uchar Com, Char *S) { strcpy(Input_Trigger, S); }
...
Static void LOGMSG(Char *S) {
        if (__Log_Msg) { /*$I-*/
                ...
                if (P_ioresult != 0)
                        MODULE_ERR(
                            "Error writing to Message File. Disk may be full");
        }
        puts(S);
}

Local boolean GET_PARITY(Char *S_, uchar Com) { ; }

Static void PARITY_COM(struct termios *term, Char *Par) {
        switch (Par[0]) {
        ...
        if (tcsetattr(fd_test, TCSADRAIN, &termio_test) < 0) {
                (void)perror(
                    "tcsetattr: attempting to set parity and data bits.");
        }
}

Static void PARITYREF(Char *Par) { ; }

/* PARITYTEST used to return void, but to keep compiler happy, I
   changed to return type of int.
   Error msg was: Inconsistent type declaration: PARITYTEST        */
int PARITYTEST(Char *Par) {
        PARITY_COM(&termio_test, Par);

        return 1;
}

Static void GET_BAUD(Char *S_, uchar Com, short *Code) { ; }

Static void SPEED_COM(struct termios *term, speed_t sp) {
...
Static void SPEEDREF(speed_t S) { ; }

/* SPEEDTEST used to return void, but to keep compiler happy, I
   changed to return type of int.
   Error msg was: Inconsistent type declaration: SPEEDTEST         */
int SPEEDTEST(speed_t S) {

        SPEED_COM(&termio_test, S);
        return 1;
}

Static void TIMEOUT_COM(uchar Com, Char *S) { ; }

Static void TIMEOUT(Char *S) { ; }

Static void WRITE_COM(FILE *f, Char *S) {
        if (fputs(S, f) < 0) {
...
Static void IOCONFIG(Char *S) { ; }

Static void DATACOMM_COM(uchar Com, Char *Baud, Char *Time_Out, Char *Hand,
                         Char *Echo_Var, Char *Interm_Var, Char *Intrig_Var) {
        ;
}

Static void DATACOMM(Char *Baud, Char *Time_Out, Char *Hand, Char *Echo_Var,
                     Char *Interm_Var, Char *Intrig_Var, boolean Refpres) {
        ;
}

Static void INITINFO(Char *S) { strcpy(S, __Command_Line); }

Static void __setstrlen(Char *s, short l) { s[l] = '\0'; }
...
char *in(int fd, char *buffer, int bufsiz, char *term) {
        ...
        while (bufsiz > 0) {
                for (; i > 0; i--, c++, bufsiz--) {
                        ...
                        if (strchr(term, *c)) {
                                ...
                                /* copy the rest of the string into the hold
                                 * buffer... */
                                if (i > 0) {
