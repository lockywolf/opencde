...
int main(int argc, char **argv) {
        ...
        while (EOF != (i1 = getopt(argc, argv, "fl:"))) {
                ...
                case 'l':
                        if ((logfd = open(optarg, O_WRONLY | O_APPEND | O_CREAT,
                                          0644)) < 0) {
                                (void)perror(optarg);
                                (void)exit(1);
                        }
...
static void logStartStop(char *progName, int logfd, int start) {
        ...
        (void)snprintf(buffer, sizeof buffer, "%s: %s %s\n",
                       (savedProgName && *savedProgName) ? savedProgName
                                                         : "logger",
                       start ? "starting" : "terminating", tstring);

        if (savedLogfd >= 0)
                (void)write(savedLogfd, buffer, strlen(buffer));
}

void shutDown(int sig) { (void)logStartStop(NULL, -1, 0); }
