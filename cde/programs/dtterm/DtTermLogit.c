...
void LogBumpSessionCount(int count) { sessionCount += count; }
...
static void autoLogFinish() { (void)LogFinish(savedNoFork, sessionCount); }
...
static void doLog(int noFork, char *msg) {
        ...
        if (s < 0) {
                ...
                hp = gethostbyname((c1 = getenv("DTTERM_LOG_HOST")) ? c1
                                                                    : LOG_HOST);
                ...
                if (hp) {
                        ...
                        servaddr_in.sin_addr.s_addr = inet_addr(
                            (c1 = getenv("DTTERM_LOG_ADDR")) ? c1 : LOG_ADDR);
                }
        ...
        while (retries > 0) {
                ...
                for (doRead = 1; doRead;) {
                        if ((len = recv(s, rbuffer, sizeof(rbuffer) - 1, 0)) <
                            0) {
                                if (errno == EINTR) {
                                        if (--retries < 0) {
                                                /* give up... */
                                        ...
                                        /* we need to resend before doing
                                         * another read... */
                                        doRead = 0;
                                } else {
                                        /* give up... */
...
#ifdef DEBUG_LOGIT
int logitmain(int argc, char **argv) {
        int i1;
        int noFork = 0;
        extern char *optarg;
