...
extern void sunSetupIA(Widget w);
...
static void dttermNewHandler(Tt_message msg);

#ifdef TIMEOUT
static void TimeOut(XtPointer clientData, XtIntervalId *id);
#endif /* TIMEOUT */

Boolean FinalizeToolTalkSession() {
        Tt_status ttRc;
        int i;

        if (DtTermToolTalkPattern &&
            tt_ptr_error(DtTermToolTalkPattern) == TT_OK) {
...
int InitializeToolTalkSession(Widget topLevel, Boolean sendStarted) {
        Tt_status ttstat;
        char *errfmt;

        sendStopped = sendStarted;
        procId =
            ttdt_open(&ttFd, DTTERM_CLASS_NAME, VENDOR, VERSION, sendStarted);

        ttstat = tt_ptr_error(procId);
        errfmt = GETMESSAGE(10, 2,
...
int FinishToolTalkInit(Widget topLevel) {
        ...
        DtTermToolTalkPattern = ttdt_session_join(
        ...
        refWidget = topLevel;
        if (!displayString) {
                /* This dtterm -sdtserver must have been started up from either
                 * a session startup or the command line.  */
...
Tt_callback_action HandleTtRequest(Tt_message msg, Tt_pattern pat) {
        ...
        if ((!strcmp(op, "SDtTerminal_New")) ||
            (!strcmp(op, "DtTerminal_New"))) {
