...
static char rcs_id[] =
    "$TOG: DtTermMain.c /main/19 1999/09/17 13:26:34 mgreess $";
...
static void activateCallback(Widget w, XtPointer client_data,
                             XtPointer call_data);

static void ToggleMenuBar(Widget w, XtPointer client_data, XtPointer call_data);

static void SaveSessionCB(Widget w, caddr_t client_data, caddr_t call_data);
static Boolean RestoreSession(Widget topDog, Arg *arglist, int i);
static void SaveTerm(DtTermViewWidget dtvw, int cloneNum, int fd);
static void RestoreTerm(Widget shellWidget, DtTermViewWidget dtvw, int termNum,
                        XrmDatabase db);
static void SetWorkSpaceHints(Widget shell, char *workspaces);

static XtEventHandler TestProbeHandler(Widget w, XtPointer client_data,
                                       XEvent *event, Boolean *ctd);
static void RegisterTestProbe(Widget topLevel, DtTermViewWidget tw);
...
#ifdef TERMINAL_SERVER
static TERMATTRS attrs = {NULL,  defaultIgnoredKeysyms,
                          NULL,  False,
                          TRUE,  False,
                          False, NULL,
                          False, 5,
...
static TERMATTRS attrs = {
    NULL, defaultIgnoredKeysyms, NULL, False, False, NULL, NULL, NULL};
#else
...
static XrmOptionDescRec optionDescList[] = {
    ...
    {"-bs", "*backgroundIsSelect", XrmoptionNoArg, (XtPointer)_true},
    {"+bs", "*backgroundIsSelect", XrmoptionNoArg, (XtPointer)_false},
...
/*ARGSUSED*/
static void DestroyDtTerm(Widget w, XtPointer client_data,
                          XtPointer call_data) {
...
/*ARGSUSED*/
static void ExitTimeout(XtPointer client_data, XtIntervalId *id) {
        if (title) {
                XtFree(title);
        }
        (void)exit(0);
...
/*ARGSUSED*/
static void DecrementInstanceCountAndExit(Widget w, XtPointer client_data,
                                          XtPointer call_data) {
        ...
        /****************
         *
         * First, update the instance list cache by "deleting" the current
         *instance
         *
         ****************/
        InstanceCount--;
        if (InstanceCount > 0) {
                new_instance_list = (DtTermViewWidget *)XtMalloc(
...
#endif /* notdef */
                        if (InstanceCount <= 0) {
                                /* use a timeout so that the toolkit can finish
                                 * killing off this widget...
                                 */
                                (void)XtAppAddTimeOut(
                                    XtWidgetToApplicationContext(w), 0,
...
static void RegisterTestProbe(Widget topLevel, DtTermViewWidget tw) {
        ...
        if (xa_DTTERM_TEST_PROBE_IDS == None) {
                xa_DTTERM_TEST_PROBE_IDS = XInternAtom(
                    XtDisplay(topLevel), "_DTTERM_TEST_PROBE_IDS", False);
        }
        if (xa_DTTERM_REMOTE_PROBE_REQUEST == None) {
                xa_DTTERM_REMOTE_PROBE_REQUEST = XInternAtom(
                    XtDisplay(topLevel), "_DTTERM_REMOTE_PROBE_REQUEST", False);
        }

        XChangeProperty(XtDisplay(topLevel),
                        RootWindowOfScreen(XtScreen(topLevel)),
                        xa_DTTERM_TEST_PROBE_IDS, xa_DTTERM_TEST_PROBE_IDS, 32,
                        PropModeReplace, (unsigned char *)&win, 1);
        XtAddEventHandler(topLevel, NoEventMask, True,
                          (XtEventHandler)TestProbeHandler, (XtPointer)tw);
}
...
static XtEventHandler TestProbeHandler(Widget w, XtPointer client_data,
        ...
        *ctd = True;
        ...
        /****************
         *
         * If it's a xa_DTTERM_REMOTE_PROBE_REQUEST, then the data is three
         *32-bit quantities: 0 is the window ID, 1 is the property atom to get
         *off the window, and 2 is the number of bytes required to getvalues
         *the requested data.
         *
         ****************/
        ...
        *ctd = False;
        ...
        if (XGetWindowProperty(XtDisplay(w), win, prop_name, offset, length,
                               ...
                               &actual_format, &nitems, &bytes_left,
                               &data) == Success) {
                switch (prop_type) {
...
static void AddMenubarToggle(Widget topLevel, Widget termViewWidget) {
        Arg al[20];
        int ac;
        char *menu;
        char buffer[BUFSIZ];
        ...
        if (xa_DTTERM_TOGGLE_MENUBAR == None) {
                /* get the atom value... */
                xa_DTTERM_TOGGLE_MENUBAR = XInternAtom(
                    XtDisplay(topLevel), "_DTTERM_TOGGLE_MENUBAR", False);
        }
	...
	");
        ...
        if (xa_MOTIF_WM_MESSAGES != None) {
                ...
                (void)XmAddProtocolCallback(
                    topLevel, xa_MOTIF_WM_MESSAGES, xa_DTTERM_TOGGLE_MENUBAR,
                    ToggleMenuBar, (XtPointer)termViewWidget);
        }
}

static void CloneCallback(Widget wid, XtPointer client_data,
                          XtPointer call_data) {
        register DtTermViewWidget w = (DtTermViewWidget)wid;
        DtTermViewCallbackStruct *termCB =
            (DtTermViewCallbackStruct *)call_data;
        ...
        if (XtIsShell(parent)) {
                ...
                if (XtIsApplicationShell(parent)) {
                        (void)XtSetArg(arglist[i], XmNargc, &argc);
                        i++;
                        (void)XtSetArg(arglist[i], XmNargv, &argv);
                        i++;
                }
#ifdef COPY_WHEN_CLONING
                (void)XtSetArg(arglist[i], XmNgeometry, &geometry);
                i++;
                (void)XtSetArg(arglist[i], XmNwidth, &width);
                i++;
                (void)XtSetArg(arglist[i], XmNheight, &height);
                i++;
#endif /* COPY_WHEN_CLONING */
                (void)XtSetArg(arglist[i], XmNallowShellResize,
                               &allowShellResize);
                i++;
                ...
                (void)XtSetArg(arglist[i], XmNallowShellResize,
                               allowShellResize);
                i++;
                /* Set the title (and icon name) for the clone (new) dtterm
                   window the same as the initial title (and icon name) of the
                   top level shell dtterm window
                 */
                (void)XtSetArg(arglist[i], XmNtitle, attrs.title);
                i++;
                if (attrs.icon_name)
                        ...
                        topShellIconName = attrs.title;
                (void)XtSetArg(arglist[i], XmNiconName, topShellIconName);
                i++;

                shellWidget =
                    XtAppCreateShell((char *)0, "Dtterm", XtClass(parent),
                                     XtDisplay((Widget)w), arglist, i);
        } else {
                i = 0;
                (void)XtSetArg(arglist[i], XmNallowShellResize, True);
                i++;
                shellWidget = XtAppCreateShell(
                    (char *)0, "Dtterm", applicationShellWidgetClass,
                    XtDisplay((Widget)w), arglist, i);
        }

#ifdef COPY_WHEN_CLONING
        termWidget = DtCreateTermView(shellWidget, "dtTermView",
                                      termCB->arglist, termCB->argcount);
#else  /* COPY_WHEN_CLONING */
        termWidget =
            DtCreateTermView(shellWidget, "dtTermView", termCB->arglist, 0);
#endif /* COPY_WHEN_CLONING */
        ...
        (void)XtAddCallback(termWidget, DtNnewCallback, CloneCallback,
                            (XtPointer)0);
...
static void ToggleMenuBar(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        XmAnyCallbackStruct *cb = (XmAnyCallbackStruct *)call_data;

        if (cb->event->xclient.message_type == xa_MOTIF_WM_MESSAGES) {
                /* can't use case here, since we are not switching on a
                 * pre-defined value...
                 */
...
static void
#endif /* SUN_TERMINAL_SERVER */
CreateInstance(Widget topLevelWidget, char *name, Arg *arglist, int argcnt,
        ...
        termViewWidget =
            DtCreateTermView(topLevelWidget, name, arglist, argcnt);
...
static Boolean GetBindingsProperty(Display *display, String property,
                                   String *binding) {
        char *prop = (char *)0;
        Atom actualType;
        int actualFormat;
        ...
        (void)XGetWindowProperty(display, RootWindow(display, 0),
                                 XmInternAtom(display, property, FALSE), 0,
                                 (long)1000000, FALSE, XA_STRING, &actualType,
...
static void FixOSFBindings(Display *display) {
        Widget w;
        Arg al[10];
        int ac;
        ...
        ignoredKeysymList =
            (char **)XtMalloc((numIgnoredKeysyms + 1) * sizeof(char *));
        ...
        if (!origDisplayBindings) {
                GetBindingsProperty(display, "_MOTIF_BINDINGS",
                                    &origDisplayBindings);
        ...
        DebugF('b', 10,
               fprintf(stderr, "FixOSFBindings(): bindings: %s\n",
                       origDisplayBindings));
                        ...
                        DebugF(
                            ...
                            fprintf(stderr, "!strcmp(\"%s\", \"%s\") = %d\n",
                                    ...
                                    !strcmp(c2 - strlen(ignoredKeysymList[i1]),
                                            ignoredKeysymList[i1])));
                        if ((strlen(ignoredKeysymList[i1]) <=
                             (size_t)(c2 - cHold)) &&
                            !strcmp(c2 - strlen(ignoredKeysymList[i1]),
                                    ignoredKeysymList[i1])) {
                                /* we hit a match... */
        ...
        *c2 = '\0';
        ...
        /* changed from XtFree to free as it was allocated with malloc to avoid
         * FMM */
        if (freeOrigBindings)
...
static void removeWmCommandCallback(Widget w, XtPointer clientData,
        ...
        if (((XPropertyEvent *)event)->atom == xa_WM_STATE) {
                /* blow away the atom... */
                (void)XDeleteProperty(XtDisplay(w), XtWindow(w), XA_WM_COMMAND);

                /* we have done our job and don't need to be called again... */
                (void)XtRemoveEventHandler(w, (EventMask)XtAllEvents, True,
                                           removeWmCommandCallback, clientData);
        }
}
...
#pragma BBA_IGNORE
        ...
        topLevel = XtAppInitialize(&appContext, "Dtterm", optionDescList,
                                   XtNumber(optionDescList), &argc, argv,
                                   fallbackResources, arglist, i);

#ifdef SUN_TERMINAL_SERVER
        if (iAmTheServer) {
                (void)ServerStartSession(topLevel, savedArgc, savedArgv,
                                         attrs.loginShell, commandToExecute);
        }
#endif /* SUN_TERMINAL_SERVER */
        ...
        (void)XtGetApplicationResources(
            topLevel, (XtPointer)&attrs, applicationResources,
            XtNumber(applicationResources), arglist, i);
        ...
        i = 0;
        (void)XtSetArg(arglist[i], XmNmappedWhenManaged, False);
        i++;
        (void)XtSetArg(arglist[i], XmNallowShellResize, True);
        i++;
        topShell =
            XtAppCreateShell((char *)0, "Dtterm", applicationShellWidgetClass,
...
#pragma BBA_IGNORE
        ...
        for (i = 1; i < argc; i++) {
                ...
                case 'S':
                        ...
                        if (c2 = strchr(c, '.')) {
                                ...
                                /* skip past the first 2 characters since they
                                 * are the name of the pty...
                                 */
                                c2 = c + 2;
                        }
                        /* use scanf instead of atoi since it won't change the
                         * value of ptyMasterFd if the string does not contain a
                         * valid number...
                         */
        ...
        /* log our startup... */
                ...
                if (!RestoringTerms || !RestoreSession(topShell, arglist, i)) {
                        ...
                        if (ptyMasterFd >= 0) {
                                /* set the workspace for the initial dtterm
                                 * instance... */
                                (void)XtSetArg(arglist[i], DtNtermDevice,
                                               ptyMasterFd);
                                i++;
                                (void)XtSetArg(arglist[i],
                                               DtNtermDeviceAllocate, False);
                                i++;
                                (void)XtSetArg(arglist[i], DtNsubprocessExec,
                                               False);
                                i++;
                                (void)CreateInstance(topShell, "dtTermView",
                                                     arglist, i, False);
                        } else {
                                (void)CreateInstance(topShell, "dtTermView",
                                                     arglist, i, True);
                ...
                if ((!attrs.icon_name) && (!attrs.dtterm_name))
                        (void)XtSetArg(arglist[i], XmNiconName, attrs.title);
                i++;
                (void)XtSetArg(arglist[i], XmNmappedWhenManaged, False);
                i++;
                (void)XtSetArg(arglist[i], XmNgeometry, NULL);
                i++;
                (void)XtSetValues(topShell, arglist, i);
                (void)XtRealizeWidget(topShell);

                if (RestoringTerms != True) {
                        /* use the wm command to pass the -xrm resource to the
                         * wm... */
                        (void)XSetCommand(XtDisplay(topShell),
                                          XtWindow(topShell), savedArgv,
                                          savedArgc);
                        /* add a callback to remove it after it has been seen...
                         */
                        (void)XtAddEventHandler(
                            topShell, (EventMask)PropertyChangeMask, False,
                            removeWmCommandCallback, (XtPointer)0);
                }
                ...
                if (ptyMasterFd >= 0) {
                        (void)fprintf(
                            ...
                            GETMESSAGE(
                                NL_SETN_Main, 2,
                                "%s: the -S option cannot be used with the "
                                "-server or -serverid options\n"),
                            argv[0]);
                        (void)exit(1);
                }
                ...
                (void)ServerStartSession(topLevel, savedArgc, savedArgv,
                                         attrs.server, attrs.serverId,
                                         attrs.exitOnLastClose, attrs.block,
                                         attrs.loginShell, commandToExecute);
        }
#endif /* TERMINAL_SERVER */
...
/* ARGSUSED */
static void SaveSessionCB(Widget w,            /* widget id */
...
static void SaveTerm(DtTermViewWidget dtvw, int cloneNum, int fd) {
        ...
        if (xa_WM_STATE &&
            (Success == XGetWindowProperty(
                            XtDisplay(dtvw), XtWindow(XtParent(dtvw)),
                            xa_WM_STATE, 0L,
                            (sizeof(struct _wmStateStruct) + 3) / 4, False,
        ...
        ac = 0;
        (void)XtSetArg(al[ac], DtNmenuBar, &menuBar);
        ac++;
        (void)XtSetArg(al[ac], DtNscrollBar, &scrollBar);
        ac++;
        ...
        ac++;
        (void)XtSetArg(al[ac], DtNappCursorDefault, &cursorMode);
        ac++;
        (void)XtSetArg(al[ac], DtNappKeypadDefault, &keypadMode);
        ac++;
        (void)XtSetArg(al[ac], DtNautoWrap, &autoWrap);
        ac++;
        (void)XtSetArg(al[ac], DtNreverseWrap, &reverseWrap);
        ac++;
        (void)XtSetArg(al[ac], DtNc132, &c132);
        ac++;
...
static Boolean RestoreSession(Widget topDog, Arg *passedAl, int passedAc) {
        ...
        for (j = 0; j < numToRestore; j++) {
                ...
                ac = 0;
                (void)XtSetArg(al[ac], XmNallowShellResize, True);
                ac++;
                (void)XtSetArg(al[ac], XmNmappedWhenManaged, True);
                ac++;
...
static void RestoreTerm(Widget shellWidget, DtTermViewWidget dtvw, int termNum,
         ...
         ****************/
        ac = 0;
        (void)XtSetArg(al[ac], XmNx, x);
        ac++;
        (void)XtSetArg(al[ac], XmNy, y);
        ac++;
        (void)XtSetArg(al[ac], XmNwidth, width);
        ac++;
        (void)XtSetArg(al[ac], XmNheight, height);
        ac++;
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
                if (!strcmp((char *)value.addr, "True")) {
                        (void)XtSetArg(al[ac], XmNinitialState, IconicState);
                        ac++;
                } else {
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], XmNmenuBar, False);
                ac++;
        }
        xrm_name[1] = XrmStringToQuark("scrollBar");
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNscrollBar, False);
                ac++;
        }
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNjumpScroll, False);
                ac++;
        }
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNmarginBell, True);
                ac++;
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNvisualBell, True);
                ac++;
        }
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNreverseVideo, True);
                ac++;
        }
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNappCursorDefault, True);
                ac++;
        }
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNappKeypadDefault, True);
                ac++;
        }
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNautoWrap, False);
                ac++;
        }
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNreverseWrap, True);
                ac++;
        }
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value) ==
            True) {
                (void)XtSetArg(al[ac], DtNc132, True);
                ac++;
        }
...
static void SetWorkSpaceHints(Widget shell, char *workspaces) {
        ...
        if (workspaces) {
                do {
                         ...
                         */
                        if (thisWsAtom == None) {
                                thisWsAtom = XmInternAtom(XtDisplay(shell),
                                                          workspaces, True);
                        }
                ...
                if (num_workspaces > 0) {
                        DtWsmSetWorkspacesOccupied(
                            XtDisplay(shell), XtWindow(shell), workspace_atoms,
                            num_workspaces);
                }
