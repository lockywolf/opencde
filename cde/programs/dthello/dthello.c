...
int main(int argc, char **argv) {
        ...
        if (dpy == NULL) {
                setlocale(LC_ALL, "");
                fprintf(stderr,
                        (char *)GETMESSAGE(4, 1, "%s: can't open display\n"),
                        progName);
                exit(1);
        }
         ...
         * Fetch resources
         */
        VhGetResources(dpy, progName, DTHELLO_CLASS, restable,
                       XtNumber(restable));

        /* assign the x_offset to the value set in the resource list if given by
         * the user */
        /* RK 11.06.93 */
        if (xoffsetArg != NULL)
        ...
        for (i = 0; i < numFiles; i++) {
                ...
                if (!(fp = fopen((char *)ppchFileNames[i], "r"))) {
                        fprintf(stderr,
                                (char *)GETMESSAGE(
                                    4, 7, "%1$s: unable to open file %2$s\n"),
         ...
         */
        wmwin = XCreateSimpleWindow(dpy, DefaultRootWindow(dpy), 0, 0, 1, 1, 0,
                                    bg, bg);
        ...
        while (Done) {
                ...
                if (event.type == Expose && event.xexpose.window == welcome &&
                    event.xexpose.count == 0) {
                        ...
                        /*
                         * Create the GC for drawing the box and painting the
                         * text.
                         */
                        gcv.foreground = fg;
                        gcv.background = bg;
                        gc = XCreateGC(dpy, welcome,
...
void DrawBox(void) {
        ...
        ********************************************************/
        ...
        XSetLineAttributes(dpy, gc, box_line_width, LineSolid, CapButt,
                           JoinMiter);
