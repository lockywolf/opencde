...
static char *TOCElems[] = {"TOCPart",   "TOCChap",   "TOClevel1", "TOClevel2",
...
static t_entry langtbl[] = {{"C.ISO-8859-1", 1, "eng.sfx", "eng.stp", NULL},
...
static char *usageMsg1 =
    "USAGE:\n\
  " EXEC_NAME " -h\n\
  " EXEC_NAME " admin\n\
  " EXEC_NAME
    " build [-h] [-T <tmpdir>] [-m <catalog>] [-d <library description>]\n\
           [-n <library short name>] -l <library> <bookcase-doc>...\n\
  " EXEC_NAME
    " tocgen [-h] [-T <tmpdir>] [-m <catalog>] -f <tocfile> [-id <tocid>]\n\
            [-title <toctitle>] <document>...\n\
  " EXEC_NAME
    " update [-h] [-m <catalog>] -l <library> -b <bookcase> <stylesheet>\n\
...
static void touchFile(char *fileName) {
        ...
        fprintf(fp, "%s\n",
                "$XConsortium: dtinfogen_worker.c /main/24 1996/11/26 12:35:07 "
                "cde-hal $");
...
static Boolean checkStat(char *fileName, unsigned int flags) {
        ...
        if ((flags & FSTAT_IS_FILE) && (!S_ISREG(statBuf.st_mode))) {
                errno = ENOENT;
                return False;
        }

        if ((flags & FSTAT_IS_DIR) && (!S_ISDIR(statBuf.st_mode))) {
                errno = ENOTDIR;
                return False;
        }
...
static Boolean testExec(char *execName, Boolean tellIt) {
        char *path;
        char **pathVector;
        int i;
        ...
        for (i = 0; pathVector[i] != (char *)NULL; i++) {
                snprintf(execBuf, sizeof(execBuf), "%s/%s", pathVector[i],
                         execName);
...
static char *makeWorkDir(void) {
        ...
        for (i = 1; i != 0; i++) {
                ...
                if (!checkStat(workDir, FSTAT_EXISTS)) {
                        snprintf(cmdBuf, sizeof(cmdBuf), "mkdir -p %s",
                                 workDir);
                        runShellCmd(cmdBuf);
...
static void removeWorkDir(void) {
        ...
        if (gStruct->workDir) {
                ...
                ret = system(cmdBuf);
                if (ret != 0)
                        die(-1, "system for rm failed; exiting...\n");
...
static Boolean doBuild(int argc, char *argv[]) {
        if (strcmp(argv[1], "build") == 0) {
                ...
                fileToTouch =
                    buildPath("%s/%s.oli", gStruct->library, gStruct->libName);
...
static Boolean doUpdate(int argc, char *argv[]) {
        if (strcmp(argv[1], "update") == 0) {
                ...
                if (!checkStat(styleSheet, FSTAT_IS_READABLE))
                        die(-1, "%s: %s: %s\n", EXEC_NAME, styleSheet,
                            strerror(errno));

                cmdSrc =
                    parseDocument(False, gStruct->styleProlog, styleSheet, 0);
                cmd = buildPath("%s | StyleUpdate %s %s", cmdSrc,
                                gStruct->library, gStruct->bookCase);
                runShellCmd(cmd);
...
static char *makeAbsPathStr(char *str) {
        ...
        for (cwdVectorLen = 0; cwdVector[cwdVectorLen] != (char *)NULL;
             cwdVectorLen++)
                /* EMPTY */
                ;
        ...
        for (i = 0; pathVector[i] != (char *)NULL; i++) {
                ...
                for (j = 0; dirVector[j] != (char *)NULL &&
                            (strcmp(dirVector[j], "..") == 0);
                     j++)
                        /* EMPTY */
                        ;
...
static char *addToEnv(char *var, char *addMe, Boolean prepend) {
        ...
        if ((ptr = XtMalloc(ptrlen)) != (char *)NULL) {
                if (envStr) {
                        if (prepend)
                                snprintf(ptr, ptrlen, "%s=%s:%s", var, addMe,
                                         envStr);
                        else
                                snprintf(ptr, ptrlen, "%s=%s:%s", var, envStr,
                                         addMe);
                } else
                        snprintf(ptr, ptrlen, "%s=%s", var, addMe);
...
/* are set (may be NULL) */
static char *buildSGML(void) {
        char *sgmlPath = buildPath("%s/infolib/%s/SGML", STR(gStruct->install),
                                   STR(gStruct->clang));
...
static void defaultGlobals(void) {
        ...
        { /* resolve canonical lang using _DtLcx routines */
                ...
                if (_DtLcxOpenAllDbs(&db) == 0) {
                        if (_DtXlateGetXlateEnv(db, platform, &execver,
                                                &compver) == 0) {
                                /* _DtLcxXlateOpToStd allocates space for std
                                 * lang using strdup */
                                _DtLcxXlateOpToStd(
                                    db, platform, compver, DtLCX_OPER_SETLOCALE,
                                    gStruct->lang, &gStruct->clang, NULL, NULL,
                                    NULL);
                        }
        ...
        { /* resolve dtsearch language based on canonical lang */
                ...
                for (gStruct->dtsridx = 0, iter = langtbl; iter->name; iter++) {
                        if (strcmp(iter->name, gStruct->clang) ==
                            0) { /* found a match */
...
static void checkGlobals(void) {
        if ((!gStruct->install) || (!checkStat(gStruct->install, FSTAT_IS_DIR)))
                die(-1,
                    "%s: Cannot find DtInfo Toolkit installation directory.\n"
                    "\n"
                    "The DTINFO_HOME variable must be set to the directory "
                    "where the DtInfo\n"
                    "toolkit is installed.\n"
                    "\n"
                    "You probably invoked this script in an unsupported "
                    "manner.\n",
                    EXEC_NAME);
        ...
        if (!checkStat(gStruct->decl, FSTAT_IS_READABLE))
                die(-1, "%s: faulty installation: %s: %s\n", EXEC_NAME,
                    STR(gStruct->decl), strerror(errno));

        if (!checkStat(gStruct->spec, FSTAT_IS_READABLE))
                die(-1, "%s: faulty installation: %s: %s\n", EXEC_NAME,
                    STR(gStruct->spec), strerror(errno));

        if (!checkStat(gStruct->tmpDir, FSTAT_IS_DIR | FSTAT_IS_WRITABLE))
                die(-1, "%s: %s: %s\n", EXEC_NAME, STR(gStruct->tmpDir),
                    strerror(errno));
...
#endif
                gStruct->dbdfile =
                    buildPath("%s/infolib/etc/dtsr/%s.dbd", gStruct->install,
...
static void addCatFile(char *catalog, Bool needed) {
        ...
        if (parserIsNSGMLS) {
                ...
                if (ptr1)
                        catlen -= strlen(ptr1);
                snprintf(pathBuf, sizeof(pathBuf), "%.*s/%%P:%.*s/%%S", catlen,
                         catalog, catlen, catalog);
                ...
                if (strcmp(argv[i], "-l") == 0) {
                        ...
                        if (++i < argc) {
                                ...
                                if (checkStat(gStruct->outFile, FSTAT_EXISTS) &&
                                    !checkStat(gStruct->outFile,
                                               FSTAT_IS_WRITABLE))
                                        dieRWD(-1,
                                               "%s: %s already exists and not "
                                               "writable\n",
...
static char *parseDocument(Boolean runCmd, ...) {
        ...
        if (parserIsNSGMLS) {
                if (!gStruct->sgmlSearchPathEnv)
                        gStruct->sgmlSearchPathEnv =
                            addToEnv("SGML_SEARCH_PATH", ".", False);
        } else {
...
static void buildBookcase(char *cmdSrc, char *dirName) {
        ...
        snprintf(cmd, sizeof(cmd), "dbdrv define %s %s \"%s\"", gStruct->spec,
                 bookCaseName, bookCaseDesc);
        runShellCmd(cmd);
         ...
         */
        {
                ...
                snprintf(cmd, sizeof(cmd),
                         "cat %s | dbdrv stdin_load %s %s.stylesheet",
...

        {
                ...
                snprintf(cmd, sizeof(cmd),
                         "cat %s | dbdrv stdin_load %s %s.node", compress_file,
        ...
        if (strcmp(gStruct->searchEngine, "dtsearch") == 0) {
                ...
                snprintf(cmd, sizeof(cmd), "mkdir -p %s/%s", bookCaseDir,
                         gStruct->searchEngine);
                runShellCmd(cmd);

                snprintf(cmd, sizeof(cmd), "cp %s/%s/%s.fzk %s/%s", dataBase,
                         gStruct->searchEngine, bookCaseName, bookCaseDir,
                         gStruct->searchEngine);
                runShellCmd(cmd);

                snprintf(cmd, sizeof(cmd), "cp %s %s/%s", gStruct->dbdfile,
                ...
                if (ret2 == (char *)NULL)
                        die(-1, "getcwd failed; exiting...\n");
                snprintf(newDir, sizeof(newDir), "%s/%s", bookCaseDir,
                         gStruct->searchEngine);
                if (chdir(newDir) != 0)
        ...
        while (fgets(lineBuf, MAXPATHLEN, fp) != (char *)NULL) {
                if ((p1 = strstr(lineBuf, patt1)) != (char *)NULL) {
                        ...
                        if ((p2 = strstr(p1, patt2)) != (char *)NULL) {
                                len = p2 - p1;
                                *((char *)memcpy(bookCaseName, p1, len) + len) =
                                    '\0';
                                validateBookCaseName(bookCaseName);
                                p1 = p2 + strlen(patt2);
                                if ((p2 = strchr(p1, '\'')) != (char *)NULL) {
                                        len = p2 - p1;
                                        *((char *)memcpy(bookCaseDesc, p1,
                                                         len) +
                                          len) = '\0';
                                        fclose(fp);
...
static void validateBookCaseName(char *bookCaseName) {
        ...
        if (!isOk)
                dieRWD(-1,
                       "%s: Bookcase name `%s' is not valid;\n"
                       "only 8 alphanumeric characters are allowed\n",
                       EXEC_NAME, bookCaseName);
}

static void validateBookCase(char *mapFile, char *bookCaseName) {
        char *ret;
        FILE *fp;
        char lineBuf[MAXPATHLEN + 1];
...
static void buildTOC(int argc, char *argv[]) {
        ...
        *((char *)memcpy(titleBuf, gStruct->title, len) + len) = '\0';
        ...
        if (checkStat(gStruct->outFile, FSTAT_IS_FILE)) {
                ...
                while (fgets(lineBuf, MAXPATHLEN, fp) != (char *)NULL) {
                        if ((p1 = strstr(lineBuf, patt1start)) !=
                            (char *)NULL) {
                                p1 += strlen(patt1start);
                                if ((p2 = strstr(p1, patt1end)) !=
                                    (char *)NULL) {
                                        len = p2 - p1;
                                        *((char *)memcpy(idBuf, p1, len) +
                                          len) = '\0';
                                }
                        }
                        if ((p1 = strstr(lineBuf, patt2start)) !=
                            (char *)NULL) {
                                p1 += strlen(patt2start);
                                if ((p2 = strstr(p1, patt2end)) !=
                                    (char *)NULL) {
                                        len = p2 - p1;
                                        *((char *)memcpy(titleBuf, p1, len) +
                                          len) = '\0';
                                }
                        }
                }
...
static void makeTOC(char *id, char *title) {
        ...
        for (i = 0; i < nTOCEntries; i++) {
                 ...
                 * ## are allowed at one level. Currently , it is 99999, ie.
                 * only
...
static char *replaceData(char *inData, char *replaceMe, char *replacement) {
        ...
        while ((endP = strstr(p, replaceMe)) != (char *)NULL) {
                ...
                *((char *)memcpy(newData + slen, replacement, len) + len) =
                    '\0';
                p = endP + replaceMeLen;
        }
        slen = strlen(newData);
...
static void addTOCEntry(TOCEntry **tocEntries, int *nTOCEntries,
                        int *maxEntries, TOCEntry *newEntry) {
        if (*nTOCEntries >= *maxEntries) {
                TOCEntry *newArray;

                newArray = (TOCEntry *)XtRealloc(
                    (char *)*tocEntries, (*maxEntries + 10) * sizeof(TOCEntry));
                if (!newArray)
                        return;
...
static void freeTOCRecord(TOCRecord *tocRecord) {
...
int main(int argc, char *argv[]) {
        ...
        if (!doAdmin(argc, argv) && !doBuild(argc, argv) &&
            !doTocgen(argc, argv) && !doUpdate(argc, argv) &&
            !doValidate(argc, argv) && !doHelp(argc, argv))
