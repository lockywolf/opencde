...
class OL_Data : public BaseData {
      ...
      public:
        /*
         * istat is controlled by the invoker to determine if OL_Data should
         * really ignore the OLIAS IGNORE attribute. The reason that such action
         * is controlled by the invoker of the class is that the corresponding
         * behavior of IGNORE is controlled by the invoker, not by the OL_Data
         * class
         */

        OL_Data(const Token &t, int aType, ActionType mode = DEFAULT_ACTION);
...
inline OL_Data::OL_Data(const Token &t, ActionType mode) : BaseData(t, mode) {
