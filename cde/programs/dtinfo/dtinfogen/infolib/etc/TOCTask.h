...
class TOCTask : public ComplexTask {
         ...
         *
         * When the root TOC node completes, it uses the write_tree() and
         * write_path() methods to walk the whole tree of TOCTasks twice to
         * build the tables.
         */

      public:
        TOCTask(const Token &t, BookTask *book, TOCTask *parent = NULL,
                int sibling_index = 0);
      ...
      protected:
        ...
        TOCTask *left() {
                return f_parent && f_sibling_index > 0
                           ? (TOCTask *)f_parent->subtask(f_sibling_index - 1)
                           : NULL;
        };

        TOCTask *right() {
                return f_parent && f_sibling_index + 1 < f_parent->used
                           ? (TOCTask *)f_parent->subtask(f_sibling_index + 1)
                           : NULL;
        };
