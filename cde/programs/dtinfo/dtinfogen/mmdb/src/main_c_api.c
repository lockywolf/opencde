...
void usage(int argc, char **argv) {
        ...
        fprintf(stderr,
                "   %s 1 libpath bookcaseName locator (show bookcase)\n",
                argv[0]);
        ...
        */
        ...
        fprintf(stderr, "   %s 9 libpath bookcaseName loc (show stylesheet)\n",
                argv[0]);
        fprintf(stderr,
                "   %s 10 libpath bookcaseName mmdboid (show stylesheet)\n",
                argv[0]);
        fprintf(stderr, "   %s 11 libpath bookcaseName mmdboid (show dlp)\n",
                argv[0]);
        fprintf(
            stderr,
            "   %s 12 libpath bookcaseName TOC_section_mmdboid (show book)\n",
            argv[0]);
        fprintf(stderr,
                "   %s 13 libpath bookcaseName book_mmdboid (show book)\n",
                argv[0]);
        fprintf(stderr,
                "   %s 14 libpath bookcaseName book_seq_num (show book)\n",
                argv[0]);
        fprintf(stderr,
                "   %s 15 info_lib_path info_base_spec base_name "
                "number_of_sections max_section_size, min_section_size "
                "[true|false](delete test db)\n",
                argv[0]);
}
