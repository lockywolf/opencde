...
#include <libc.h>
#endif
#if defined(hpux) || defined(sgi) || defined(USL) ||       \
...
Boolean copy_file(const char *path, const char *filenm, const char *source_ext,
                  const char *target_ext);

Boolean exist_file(const char *name, const char *path = 0);
Boolean cat_file(const char *source1, const char *source2, const char *target);
...
void lsb_putbits(unsigned &target, unsigned position_from_lsb, unsigned bits,
                 unsigned source);
unsigned lsb_getbits(unsigned source, unsigned position_from_lsb,
                     unsigned bits);
