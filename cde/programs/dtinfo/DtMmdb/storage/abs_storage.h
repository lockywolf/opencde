...
typedef enum { mmdb_big_endian = 1, mmdb_little_endian = 2 } mmdb_byte_order_t;

class abs_storage : public root {
      public:
        int index_num;

        abs_storage(char *path, char *name, c_code_t c_id,
                    rep_policy *rep_p = 0);
        virtual ~abs_storage();
        ...
        virtual int readString(mmdb_pos_t loc, char *base, int len,
                               int str_offset = 0) = 0;
        virtual int insertString(mmdb_pos_t &loc, const char *base, int len,
                                 Boolean flush = false) = 0;
        virtual int appendString(mmdb_pos_t loc, const char *, int len,
                                 Boolean flush_opt = false) = 0;

        virtual int get_str_ptr(mmdb_pos_t loc, char *&, int &len) = 0;
        ...
        virtual int allocString(mmdb_pos_t &loc, int len, char *&,
                                int mode = 0) = 0;
        // status functions
