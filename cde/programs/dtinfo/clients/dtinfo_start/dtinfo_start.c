 ...
 */
static int InvokeAction(char *prog_name, int op_num, const char *action_name,
                        char *exec_host, char *info_lib, char *topic);

static void ActionDoneCallback(DtActionInvocationID id, XtPointer client_data,
                               DtActionArg *args, int num_args,
                               DtActionStatus status);

static void Exit(XtPointer client_data, XtIntervalId id);

static Tt_status ConnectToMessageServer(char *prog_name);

static void ReceiveMessage(XtPointer client_data, int *fd, XtInputId *id);

static void LogToolTalkError(DtMsgLogType msg_type, char *prog_name,
                             char *function_name, Tt_status error_num);
static void DieFromToolTalkError(Widget parent, char *errfmt, Tt_status status);
...
int main(int argc, char **argv) {
        ...
        if (!(display = XtOpenDisplay(app_context, NULL, NULL, name, NULL, 0,
                                      &argc, argv))) {
                DtMsgLogMessage(argv[0], DtMsgLogError,
                                (char *)GETMESSAGE(SET_NUM, 7,
                                                   "XtOpenDisplay() failed.  "
                                                   "Perhaps the DISPLAY "
                                                   "environment\nvariable is "
        ...
        i++;
        XtSetArg(args[i], XtNmappedWhenManaged, False);
        i++;
        XtSetArg(args[i], XtNheight, 1);
        i++;
        XtSetArg(args[i], XtNwidth, 1);
        i++;

        top_level = XtAppCreateShell(argv[0], name, topLevelShellWidgetClass,
                                     display, args, i);
         ...
         */
        if (DtAppInitialize(app_context, display, top_level, argv[0],
                            (char *)name) == False) {
                DtMsgLogMessage(argv[0], DtMsgLogError,
                                (char *)GETMESSAGE(SET_NUM, 8,
                                                   "DtAppInitialize() failed.  "
                                                   "Perhaps the desktop "
                                                   "environment\nis not "
                                                   "properly installed."));
                exit(1);
        ...
        if (TT_OK != status) {
                 ...
                 */
                errfmt = GETMESSAGE(
                    2, 2, "Could not connect to ToolTalk:\n%s\nExiting ...");
...
static int InvokeAction(char *prog_name, int op_num, const char *action_name,
        ...
        action_id = DtActionInvoke(
            top_level, (char *)action_name, args, num_args, NULL, exec_host,
            NULL, True, (DtActionCallbackProc)ActionDoneCallback, prog_name);
         ...
         */
        if (!action_id) {
                XtAppAddTimeOut(app_context, 10, (XtTimerCallbackProc)Exit,
                                NULL);
        }
...
static Tt_status ConnectToMessageServer(char *prog_name) {
        ...
        if (status != TT_OK) {
                LogToolTalkError(DtMsgLogError, prog_name, "tt_open", status);
                tt_release(mark);
                return (status);
        }
        ...
        if (status != TT_OK) {
                LogToolTalkError(DtMsgLogError, prog_name, "tt_ptype_declare",
                                 status);
                tt_release(mark);
                return (status);
        }
        ...
        if ((tt_int_error(ttfd)) != TT_OK) {
                LogToolTalkError(DtMsgLogError, prog_name, "tt_fd", status);
                tt_release(mark);
                return (status);
        }
        ...
        if ((tt_int_error(ttfd)) != TT_OK) {
                LogToolTalkError(DtMsgLogError, prog_name, "tt_session_join",
                                 status);
                tt_release(mark);
                return (status);
        }
...
static void ReceiveMessage(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        if (tt_pointer_error(message) != TT_OK) {
                LogToolTalkError(DtMsgLogError, prog_name, "tt_message_receive",
        ...
        default: {
                ...
                DtMsgLogMessage(
                    ...
                    (char *)GETMESSAGE(
                        SET_NUM, 11,
                        "The message operation '%d' is not supported."),
                    op_num);
        ...
        if ((op_num == LOAD_INFO_LIB && num_args != LOAD_INFO_LIB_ARGS) ||
            (op_num == SHOW_INFO_AT_LOC && num_args != SHOW_INFO_AT_LOC_ARGS)) {
                ...
                DtMsgLogMessage(
                    prog_name, DtMsgLogError,
                    (char *)GETMESSAGE(SET_NUM, 12,
                                       "Message '%s' does not have the "
                                       "required\nnumber of arguments '%d'."),
                    op_name,
                    (op_num == LOAD_INFO_LIB) ? LOAD_INFO_LIB_ARGS
                                              : SHOW_INFO_AT_LOC_ARGS);
                tt_message_reject(message);
                tt_message_destroy(message);
                exit(1);
        }
        ...
        if ((tt_ptr_error(info_lib) != TT_OK) || info_lib == NULL) {
                ...
                if ((info_lib = getenv("DTINFOLIBDEFAULT")) == NULL) {
                        DtMsgLogMessage(
                            ...
                            (char *)GETMESSAGE(
                                SET_NUM, 10,
                                "An InfoLib was not provided in the '%s' "
                                "message\nand the environment variable "
                                "DTINFOLIBDEFAULT was not defined."),
                            op_name);
                        exit(1);
                }
        }
        ...
        if ((action_name = tt_message_arg_val(message, 0)) == NULL) {
                ...
                action_name = (op_num == LOAD_INFO_LIB)
                                  ? LOAD_INFO_LIB_ACTION
                                  : SHOW_INFO_LIB_SECTION_ACTION;

                DtMsgLogMessage(prog_name, DtMsgLogWarning,
                                (char *)GETMESSAGE(SET_NUM, 13,
                                                   "Message '%s' specified a "
                                                   "NULL action name.\nAction "
                                                   "'%s' will be invoked."),
                                op_name, action_name);
        }
        ...
        if ((exec_host = tt_message_arg_val(message, 1)) == NULL) {
                ...
                DtMsgLogMessage(
                    prog_name, DtMsgLogWarning,
                    (char *)GETMESSAGE(
                        SET_NUM, 14,
                        "Message '%s' specified a NULL execution host."),
                    op_name);
        }
        ...
        if ((locale = tt_message_arg_val(message, 2)) == NULL) {
                ...
                DtMsgLogMessage(
                    prog_name, DtMsgLogWarning,
                    (char *)GETMESSAGE(SET_NUM, 15,
                                       "Message '%s' specified a NULL locale."),
                    op_name);
        } else {
                /*
        ...
        if (op_num == SHOW_INFO_AT_LOC) {
                if ((topic = tt_message_arg_val(message, 3)) == NULL) {
                        ...
                        DtMsgLogMessage(
                            ...
                            (char *)GETMESSAGE(
                                SET_NUM, 16,
                                "Message '%s' specified a NULL topic."),
                            op_name);
                        exit(1);
                }
        }
        ...
        if (op_num == last_op_num) {
                ...
                DtMsgLogMessage("dtinfo_start", DtMsgLogError,
                                ...
                                (op_num == LOAD_INFO_LIB) ? "LOAD_INFO_LIB"
                                                          : "SHOW_INFO_AT_LOC");
#endif
        } else {
#if defined(DEBUG)
...
#endif
                last_op_num = op_num;
                opstatus = InvokeAction(prog_name, op_num, action_name,
                                        exec_host, info_lib, topic);
                if (!opstatus) {
...
static void DieFromToolTalkError(Widget parent, char *errfmt,
                                 Tt_status status) {
        Arg args[10];
...
static void Exit(XtPointer client_data, XtIntervalId id) {
        ...
        XtAppAddTimeOut(app_context, time_out, (XtTimerCallbackProc)Exit, NULL);
}
