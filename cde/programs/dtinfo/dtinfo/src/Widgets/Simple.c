 ...
 */
/*
 * $Id: Simple.c,v 1.1 1992/03/20 13:52:41 jbm Exp $
 *
...
static XtResource resources[] = {
    {XtNcursor, XtCCursor, XtRCursor, sizeof(Cursor), offset(cursor),
    ...
    {XtNcursorName, XtCCursor, XtRString, sizeof(String), offset(cursor_name),
     XtRString, NULL},
#undef offset
};
...
static void ClassPartInitialize(class) WidgetClass class;
{
        register SimpleWidgetClass c = (SimpleWidgetClass) class;
        register SimpleWidgetClass super =
            (SimpleWidgetClass)c->core_class.superclass;
        ...
        if (c->simple_class.change_sensitive == NULL) {
                ...
                XtWarning(buf);
                c->simple_class.change_sensitive =
                    (Bool(*)(Widget))ChangeSensitive;
        }

        if (c->simple_class.change_sensitive == XtInheritChangeSensitive)
                c->simple_class.change_sensitive =
                    super->simple_class.change_sensitive;
}

static void Realize(w, valueMask, attributes) register Widget w;
Mask *valueMask;
                ...
                if (((SimpleWidget)w)->simple.insensitive_border == None)
                        ((SimpleWidget)w)->simple.insensitive_border =
                            XmuCreateStippledPixmap(
                                XtScreen(w), w->core.border_pixel,
...
static void ConvertCursor(w) Widget w;
{
        ...
        if (XtConvertAndStore(w, XtRString, &from, XtRColorCursor, &to)) {
                ...
                XtAppErrorMsg(XtWidgetToApplicationContext(w), "convertFailed",
                              "ConvertCursor", "XawError",
                              "Simple: ConvertCursor failed.", (String *)NULL,
                              (Cardinal *)NULL);
        }
}

/* ARGSUSED */
static Boolean SetValues(current, request, new) Widget current, request, new;
{
...
static Boolean ChangeSensitive(w) register Widget w;
{
        if (XtIsRealized(w)) {
                if (XtIsSensitive(w))
