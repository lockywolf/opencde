 ...
 */
/*
 * $Id: Panner.c,v 1.6 1992/09/26 23:15:37 brennan Exp $
 *
...
extern void _XmDrawShadow(Display *display, Drawable d, GC top_GC, GC bottom_GC,
...
#define poff(field) XtOffsetOf(PannerRec, panner.field)
    {XtNallowOff, XtCAllowOff, XtRBoolean, sizeof(Boolean), poff(allow_off),
     XtRImmediate, (XtPointer)FALSE},
    {XtNresize, XtCResize, XtRBoolean, sizeof(Boolean), poff(resize_to_pref),
    ...
    {XtNforeground, XtCForeground, XtRPixel, sizeof(Pixel), poff(foreground),
    ...
    {XtNsliderX, XtCSliderX, XtRPosition, sizeof(Position), poff(slider_x),
     XtRImmediate, (XtPointer)0},
    {XtNsliderY, XtCSliderY, XtRPosition, sizeof(Position), poff(slider_y),
     XtRImmediate, (XtPointer)0},
    ...
    PannerWidget pw;
{
        ...
        if (!pw->panner.stipple_name &&
            !XmuDistinguishablePixels(XtDisplay(pw), pw->core.colormap, pixels,
                                      3) &&
            XmuDistinguishablePixels(XtDisplay(pw), pw->core.colormap, pixels,
                                     2)) {
                valuemask = GCTile | GCFillStyle;
                values.fill_style = FillTiled;
                values.tile = XmuCreateStippledPixmap(
                    XtScreen((Widget)pw), pw->panner.foreground,
                    pw->core.background_pixel, pw->core.depth);
        } else {
...
static void check_knob(pw, knob) register PannerWidget pw;
Boolean knob;
{
        ...
        if (knob) {
                pw->panner.slider_x = (Position)(
                    ((double)pw->panner.knob_x) / pw->panner.haspect + 0.5);
                pw->panner.slider_y = (Position)(
                    ((double)pw->panner.knob_y) / pw->panner.vaspect + 0.5);
                pw->panner.last_x = pw->panner.last_y = PANNER_OUTOFRANGE;
        }
}

static void move_shadow(pw) register PannerWidget pw;
{
        if (pw->panner.shadow_thickness > 0) {
                int lw =
                    pw->panner.shadow_thickness + pw->panner.line_width * 2;
                int pad = pw->panner.internal_border;

                if ((int)pw->panner.knob_height > lw &&
                    (int)pw->panner.knob_width > lw) {
                        register XRectangle *r = pw->panner.shadow_rects;
                        r->x = (short)(pw->panner.knob_x + pad +
                                       pw->panner.knob_width);
                        r->y = (short)(pw->panner.knob_y + pad + lw);
                        r->width = pw->panner.shadow_thickness;
                        r->height =
                            (unsigned short)(pw->panner.knob_height - lw);
                        r++;
                        r->x = (short)(pw->panner.knob_x + pad + lw);
                        r->y = (short)(pw->panner.knob_y + pad +
                                       pw->panner.knob_height);
                        r->width =
                            (unsigned short)(pw->panner.knob_width - lw +
...
Boolean location, size;
{
        if (location) {
                pw->panner.knob_x =
                    (Position)PANNER_HSCALE(pw, pw->panner.slider_x);
                pw->panner.knob_y =
                    (Position)PANNER_VSCALE(pw, pw->panner.slider_y);
        }
        if (size) {
                ...
                height =
                    Min(pw->panner.slider_height, pw->panner.canvas_height);
...
static void rescale(pw) PannerWidget pw;
{
...
static void get_default_size(pw, wp, hp) PannerWidget pw;
Dimension *wp, *hp;
{
...
static Boolean get_event_xy(pw, event, x, y) PannerWidget pw;
XEvent *event;
int *x, *y;
{
        int pad = pw->panner.internal_border;
...
static int parse_page_string(s, pagesize, canvassize,
                             relative) register char *s;
...
Boolean *relative;
{
        ...
        /* skip over numbers */
        for (cp = s; isascii(*s) && (isdigit((unsigned char)*s) || *s == '.');
             s++)
                ;
        val *= atof(cp);
        ...
        *relative = rel;
        ...
        /* Only used for "rubber-banding"  DJB */

#define DRAW_TMP(pw)                                                           \
        {                                                                      \
                XDrawRectangle(                                                \
                    XtDisplay(pw), XtWindow(pw), pw->panner.xor_gc,            \
...
#define BACKGROUND_STIPPLE(pw)                                                 \
        XmuLocatePixmapFile(pw->core.screen, pw->panner.stipple_name,          \
                            pw->panner.shadow_color,                           \
                            pw->core.background_pixel, pw->core.depth, NULL,   \
...
static void Initialize(greq, gnew) Widget greq, gnew;
{
        ...
        new->panner.foreground = new->core.background_pixel;
        ...
        /* Kruft in a scrollbar-like trough color! */
        {
                Pixel trough;
                XmGetColors(new->core.screen, new->core.colormap,
                            new->core.background_pixel, NULL, NULL, NULL,
                            &trough);
                new->core.background_pixel = trough;
        }
}

static void Realize(gw, valuemaskp, attr) Widget gw;
...
static void Destroy(gw) Widget gw;
{
        PannerWidget pw = (PannerWidget)gw;
...
static void Resize(gw) Widget gw;
{ rescale((PannerWidget)gw); }

/* ARGSUSED */
static void Redisplay(gw, event, region) Widget gw;
XEvent *event;
Region region;
{
        PannerWidget pw = (PannerWidget)gw;
        ...
        static GC bottom_shadow_gc, top_shadow_gc;
        static int gc_initialized = False;
        ...
        XClearArea(XtDisplay(pw), XtWindow(pw),
                   ...
                   (unsigned int)(pw->panner.knob_height + extra), False);
        ...
        if (lw) {
                /*
                  XDrawRectangle (dpy, w, pw->panner.shadow_gc, kx, ky,
                                  ...
                                  (unsigned int) (pw->panner.knob_height - 1));
                Old Border - DJB */
                if (!gc_initialized) /* this better be in a Motif manager widget
                                      */
                {
                        /* NOTE: Create the GC based on resources in the parent.
                           This code assumes that the panner is the same color
                           as the parent, because this was quick to implement.
                           DJB */
                        XGCValues values;
                        XtGCMask valueMask;
                        Arg args[3];

                        XtSetArg(args[0], XmNbottomShadowColor,
                                 &values.foreground);
                        XtSetArg(args[1], XmNforeground, &values.background);
                        XtSetArg(args[2], XmNbottomShadowPixmap, &values.tile);
                        valueMask = GCForeground | GCBackground;
                        XtGetValues(pw->core.parent, args, 3);
                        if (values.tile != XmUNSPECIFIED_PIXMAP) {
                                valueMask |= GCFillStyle | GCTile;
                                values.fill_style = FillTiled;
                        }

                        bottom_shadow_gc =
                            XtGetGC((Widget)pw, valueMask, &values);

                        XtSetArg(args[0], XmNtopShadowColor,
                                 &values.foreground);
                        XtSetArg(args[1], XmNtopShadowPixmap, &values.tile);
                        valueMask = GCForeground | GCBackground;
                        XtGetValues(pw->core.parent, args, 2);
                        if (values.tile != XmUNSPECIFIED_PIXMAP) {
                                valueMask |= GCFillStyle | GCTile;
                                values.fill_style = FillTiled;
                        }

                        top_shadow_gc = XtGetGC((Widget)pw, valueMask, &values);

                        gc_initialized = True;
                }
                /* Now draw a Motif style shadow */
                _XmDrawShadow(dpy, w, top_shadow_gc, bottom_shadow_gc, 2, kx,
                              ky, pw->panner.knob_width,
                              pw->panner.knob_height);
        }
        /*
...
/* ARGSUSED */
static Boolean SetValues(gcur, greq, gnew) Widget gcur, greq, gnew;
{
        ...
        if ((cur->panner.stipple_name != new->panner.stipple_name ||
            ...
            XtIsRealized(gnew)) {
                ...
                if (PIXMAP_OKAY(pm)) {
                        XSetWindowBackgroundPixmap(XtDisplay(new),
                                                   XtWindow(new), pm);
        ...
        if (new->panner.resize_to_pref &&
            (cur->panner.canvas_width != new->panner.canvas_width ||
             ...
             cur->panner.resize_to_pref != new->panner.resize_to_pref)) {
                ...
                Boolean siz =
                    (cur->panner.slider_width != new->panner.slider_width ||
...
static void SetValuesAlmost(gold, gnew, req, reply) Widget gold, gnew;
...
static XtGeometryResult QueryGeometry(gw, intended, pref) Widget gw;
XtWidgetGeometry *intended, *pref;
{
        ...
        else if (pref->width == pw->core.width &&
                 pref->height == pw->core.height)
                return XtGeometryNo;
        else
...
/* ARGSUSED */
static void ActionStart(gw, event, params, num_params) Widget gw;
XEvent *event;
...
/* ARGSUSED */
static void ActionStop(gw, event, params, num_params) Widget gw;
XEvent *event;
String *params;       /* unused */
Cardinal *num_params; /* unused */
{
...
/* ARGSUSED */
static void ActionAbort(gw, event, params, num_params) Widget gw;
XEvent *event;
String *params;       /* unused */
Cardinal *num_params; /* unused */
{
...
/* ARGSUSED */
static void ActionMove(gw, event, params, num_params) Widget gw;
...
/* ARGSUSED */
static void ActionPage(gw, event, params, num_params) Widget gw;
XEvent *event; /* unused */
String *params;
Cardinal *num_params; /* unused */
{
...
/* ARGSUSED */
static void ActionNotify(gw, event, params, num_params) Widget gw;
XEvent *event;        /* unused */
String *params;       /* unused */
Cardinal *num_params; /* unused */
{
...
/* ARGSUSED */
static void ActionSet(gw, event, params, num_params) Widget gw;
XEvent *event; /* unused */
String *params;
Cardinal *num_params;
{
