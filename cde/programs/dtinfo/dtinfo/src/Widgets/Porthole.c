 ...
 */
/*
 * $Id: Porthole.c,v 1.4 1992/07/28 19:04:34 brennan Exp $
...
static XmBaseClassExtRec portholeClassExtRec = {
    NULL,                      /* Next extension       */
    NULLQUARK,                 /* record type XmQmotif */
    XmBaseClassExtVersion,     /* version              */
    sizeof(XmBaseClassExtRec), /* size                 */
    NULL,                      /* initialize prehook   */
    NULL,                      /* set_values prehook   */
...
#define PORTHOLE_EXT_REC (XtPointer) & portholeClassExtRec
#else
#define PORTHOLE_EXT_REC NULL
#define ClassInitialize NULL
#endif
...
static Widget find_child(pw) register PortholeWidget pw;
{
...
static void SendReport(pw, changed) PortholeWidget pw;
unsigned int changed;
{
        ...
        if (pw->porthole.report_callbacks && child) {
                ...
                prep.slider_y =
                    -child->core.y; /* child is outer since it is larger */
...
static void layout_child(pw, child, geomp, xp, yp, widthp,
                         heightp) PortholeWidget pw;
Widget child;
...
Dimension *widthp, *heightp;
{
        ...
        *heightp = child->core.height;
         ...
         * is no maximum size.
         */
        if (*widthp < pw->core.width)
                *widthp = pw->core.width;
        if (*heightp < pw->core.height)
                *heightp = pw->core.height;
        ...
        if (*xp < minx)
                *xp = minx; /* keep at lower right corner */
        if (*yp < miny)
                *yp = miny;

        if (*xp > 0)
                *xp = 0; /* keep at upper left corner */
        if (*yp > 0)
                *yp = 0;
}
...
static void Realize(gw, valueMask, attributes) register Widget gw;
Mask *valueMask;
...
static XtGeometryResult QueryGeometry(gw, intended, preferred) Widget gw;
...
static XtGeometryResult GeometryManager(w, req, reply) Widget w;
XtWidgetGeometry *req, *reply;
{
        ...
        *reply = *req; /* assume we'll grant everything */
        ...
        layout_child(pw, child, req, &reply->x, &reply->y, &reply->width,
                     &reply->height);
         ...
         * we will often set fields that weren't requested because we want to
