...
externaldef(restraintwidgetclass)
    WidgetClass restraintWidgetClass = (WidgetClass)&restraintClassRec;

/* **************************************************************
 * Initialize
 * ************************************************************** */

static void Initialize(RestraintWidget original, RestraintWidget w,
...
static void Realize(Widget w, XtValueMask *value_mask,
        ...
        /* Might eventually need to try set up a reasonable size before
         * realizing. */
...
static void compute_height(RestraintWidget w, Widget child) {
        ...
        if (reply.height != 0) {
                ...
                ON_DEBUG(printf("Restraint: Asking parent for height %d\n",
                                reply.height));
                result =
                    XtMakeGeometryRequest((Widget)w, &my_request, &my_reply);
                ...
                if (result == XtGeometryAlmost) {
                        ...
                        if (my_reply.height > w->core.height) {
                                ...
                                /* Parent must say Yes. */
                                XtMakeGeometryRequest((Widget)w, &my_request,
                                                      NULL);
                        }
...
static void compute_width(RestraintWidget w, Widget child) {
        XtGeometryResult result;
...
static void ChangeManaged(RestraintWidget w) {
        ...
        ON_DEBUG(
            printf("  Checking for width calc\n    child = 0x%p, first = %d\n",
                   (void *)child, w->restraint.had_child));
        if (child != NULL && w->restraint.had_child == False) {
                compute_width(w, child);
                /* Q: Should this be reset when last child is deleted, so next
                   new child causes another width computation? 14:54 02/03/93
                   DJB */
        ...
        if (child != NULL) {
                ...
                ON_DEBUG(printf(
                    "Restraint::ChangeManaged: sizing child to %d x %d\n",
                    w->core.width, w->core.height));
...
static XtGeometryResult GeometryManager(Widget w, XtWidgetGeometry *request,
        ...
        if (request->request_mode & CWWidth) {
                ...
                /* Detect conditions under which a Form bug occurs.  If the Form
                   request a width increase and a height change, this widget
                   refuses the width change, and the bogus Form bails on the
                   height change. The result is a totally messed up Form layout.
                   Therefore, the query editor must coerce the Form into never
                   requesting a width increase. */
                if (request->width > w->core.width) {
                        fprintf(stderr, "Severe warning: query term requesting "
                                        "width increase\n");
        ...
        if (yes && !almost) {
                ...
                /* Only need to check width and height below, because they are
                   the only requests that can generate a yes. */
                ...
                result =
                    XtMakeGeometryRequest(restraint, &my_request, &my_reply);
                 ...
                 */
                        ...
                        ON_DEBUG(
                            puts("Restraint: responding XtGeometryAlmost:"));
                        ...
                        if (request->request_mode & CWWidth) {
                                ...
                                /* Allowable width is always the restraint
                                 * width. */
                ...
                /* Everything's cool if it said Yes. */
                        ...
                        if (request->request_mode & CWWidth) {
                                /* Sanity Check: Yes on width is only allowed if
                                 * this is true. */
                                /* This means that width changes are essentially
                                 * not allowed. */
        ...
        if (yes || almost) {
                ...
                if (request->request_mode & CWHeight) {
                        ...
                        ON_DEBUG(printf("  Checking with parent for Almost "
                                        "height value:\n"));
                        ...
                        ON_DEBUG(printf("    Asking for height = %d\n",
                                        my_request.height));
                        result = XtMakeGeometryRequest(restraint, &my_request,
                                                       &my_reply);
                        ...
                        if (result == XtGeometryNo) /* unable to comply */
                        {
                                ...
                                reply->height = w->core.height;
                                   ...
                                   XtGeometryAlmost) /* partial compliance */
                        {
                                ON_DEBUG(printf("    Parent said Almost.\n"));
                                reply->height = my_reply.height;
                        } else if (result ==
                                   XtGeometryYes) /* total compliance */
                        {
                                ON_DEBUG(printf("    Parent said Yes.\n"));
                                reply->height = request->height;
                        }
...
static void Resize(RestraintWidget w) {
        ...
        XtResizeWidget(child, w->core.width - child->core.border_width,
