 ...
 */
/*
 * $Id: Tree.c,v 1.2 1993/05/18 16:05:53 brennan Exp $
 *
...
static XtResource resources[] = {
    /* Added border width of 0 to get rid of ugly 1 pixel line when map
       is updated or resized. 13:36 15-May-93 DJB */
    {XtNborderWidth, XtCBorderWidth, XtRDimension, sizeof(Dimension),
     XtOffsetOf(TreeRec, core.border_width), XtRImmediate, (XtPointer)0},
    ...
    {XtNforeground, XtCForeground, XtRPixel, sizeof(Pixel),
     XtOffsetOf(TreeRec, tree.foreground), XtRString, XtDefaultForeground},
    {XtNlineWidth, XtCLineWidth, XtRDimension, sizeof(Dimension),
     XtOffsetOf(TreeRec, tree.line_width), XtRImmediate, (XtPointer)0},
    {XtNgravity, XtCGravity, XtRGravity, sizeof(XtGravity),
     XtOffsetOf(TreeRec, tree.gravity), XtRImmediate, (XtPointer)WestGravity},
};
...
static void initialize_dimensions(listp, sizep, n) Dimension **listp;
int *sizep;
int n;
{
        register int i;
        ...
        if (n > *sizep) {
                *listp = (Dimension *)XtRealloc(
                    (char *)*listp, (unsigned int)(n * sizeof(Dimension)));
                if (!*listp) {
                        *sizep = 0;
                        return;
                }
...
static GC get_tree_gc(w) TreeWidget w;
{
...
static void insert_node(parent, node) Widget parent, node;
{
        TreeConstraints pc;
...
static void delete_node(parent, node) Widget parent, node;
{
        TreeConstraints pc;
        int pos, i;
...
static void check_gravity(tw, grav) TreeWidget tw;
XtGravity grav;
{
...
static void Initialize(grequest, gnew) Widget grequest, gnew;
{
        ...
        new->tree.tree_root =
            XtCreateWidget("root", widgetClass, gnew, args, TWO);
...
/* ARGSUSED */
static void ConstraintInitialize(request, new) Widget request, new;
{
...
/* ARGSUSED */
static Boolean SetValues(gcurrent, grequest, gnew) Widget gcurrent, grequest,
    gnew;
{
...
/* ARGSUSED */
static Boolean ConstraintSetValues(current, request, new, args,
                                   num_args) Widget current,
    request, new;
ArgList args;
Cardinal *num_args;
{
...
static void ConstraintDestroy(w) Widget w;
{
...
/* ARGSUSED */
static XtGeometryResult GeometryManager(w, request, reply) Widget w;
...
static void ChangeManaged(gw) Widget gw;
...
/* ARGSUSED */
static void Redisplay(tw, event, region) TreeWidget tw;
XEvent *event;
Region region;
{
        /*
...
static XtGeometryResult QueryGeometry(w, intended, preferred) Widget w;
...
static void compute_bounding_box_subtree(tree, w, depth) TreeWidget tree;
Widget w;
int depth;
{
...
static void set_positions(tw, w, level) TreeWidget tw;
Widget w;
int level;
{
        int i;
...
static void arrange_subtree(tree, w, depth, x, y) TreeWidget tree;
Widget w;
int depth;
Position x, y;
{
        ...
        for (i = 0; i < tc->tree.n_children; i++) {
                ...
                child =
                    tc->tree.children[i]; /* last value is used outside loop */
...
static void set_tree_size(tw, insetvalues, width, height) TreeWidget tw;
Boolean insetvalues;
Dimension width, height;
{
        if (insetvalues) {
                ...
                XtGeometryResult result = XtMakeResizeRequest(
                    (Widget)tw, width, height, &replyWidth, &replyHeight);
                /*
...
static void layout_tree(TreeWidget tw, Boolean insetvalues) {
        ...
        /*
         * Do a depth-first search computing the width and height of the
         * bounding box for the tree at that position (and below).  Then, walk
         * again using this information to layout the children at each level.
         */
         ...
         */
        if (XtIsRealized((Widget)tw)) {
                XClearArea(XtDisplay(tw), XtWindow((Widget)tw), 0, 0, 0, 0,
                           True);
        }
}
...
#else
    XawTreeForceLayout(tree) Widget tree;
#endif
{
