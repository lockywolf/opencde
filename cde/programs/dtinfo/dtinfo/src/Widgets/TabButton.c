...
#define TOP_GC                                                                 \
        ((tabw->tab.armed) ? tabw->primitive.bottom_shadow_GC                  \
                           : tabw->primitive.top_shadow_GC)
#define BOTTOM_GC                                                              \
        ((tabw->tab.armed) ? tabw->primitive.top_shadow_GC                     \
                           : tabw->primitive.bottom_shadow_GC)
...
#define ACTION_PROC(PROC)                                                      \
        static void PROC(Widget w, XEvent *event, String *params,              \
                         Cardinal *num_params)

ACTION_PROC(Arm);
ACTION_PROC(MultiArm);
ACTION_PROC(Activate);
ACTION_PROC(MultiActivate);
ACTION_PROC(ActivateCommon);
ACTION_PROC(ArmAndActivate);
ACTION_PROC(Disarm);
ACTION_PROC(Enter);
ACTION_PROC(Leave);
...
static void draw_rounded_shadow(Display *, Drawable, GC top, GC bottom,
                                Position x, Position y, Dimension width,
                                Dimension height, Dimension thickness,
                                Dimension radius, unsigned int type);

/* -------- Standard widget functions. -------- */

static void Initialize(Widget req, Widget new_w, ArgList, Cardinal *);
static Boolean SetValues(Widget current, Widget req, Widget new_w, ArgList,
                         Cardinal *);
...
static XtResource resources[] = {
    ...
    {XmNfillOnArm, XmCFillOnArm, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _XyzTabButtonRec, tab.fill_on_arm), XmRImmediate,
     (XtPointer)True},
    {XmNarmColor, XmCArmColor, XmRPixel, sizeof(Pixel),
     XtOffsetOf(struct _XyzTabButtonRec, tab.arm_color), XmRCallProc,
     (XtPointer)(size_t)_XmSelectColorDefault},
    {XmNarmPixmap, XmCArmPixmap, XmRPrimForegroundPixmap, sizeof(Pixmap),
     XtOffsetOf(struct _XyzTabButtonRec, tab.arm_pixmap), XmRImmediate,
     (XtPointer)XmUNSPECIFIED_PIXMAP},

    {XmNactivateCallback, XmCCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffsetOf(struct _XyzTabButtonRec, tab.activate_callback), XmRPointer,
     (XtPointer)NULL},

    {XmNarmCallback, XmCCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffsetOf(struct _XyzTabButtonRec, tab.arm_callback), XmRPointer,
     (XtPointer)NULL},

    {XmNdisarmCallback, XmCCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffsetOf(struct _XyzTabButtonRec, tab.disarm_callback), XmRPointer,
     (XtPointer)NULL},

    {XmNmultiClick, XmCMultiClick, XmRMultiClick, sizeof(unsigned char),
     XtOffsetOf(struct _XyzTabButtonRec, tab.multi_click), XmRImmediate,
     (XtPointer)XmINVALID_MULTICLICK},

    {XtNdisarmDelay, XtCDisarmDelay, XtRInt, sizeof(int),
     XtOffsetOf(struct _XyzTabButtonRec, tab.disarm_delay), XmRImmediate,
     (XtPointer)DEFAULT_DISARM_DELAY},

    {XtNradius, XtCRadius, XmRHorizontalDimension, sizeof(Dimension),
     XtOffsetOf(struct _XyzTabButtonRec, tab.radius), XmRImmediate,
     (XtPointer)12},
...
static void MultiArm(Widget w, XEvent *event, String *params,
                     Cardinal *num_params) {
...
static void Activate(Widget w, XEvent *event, String *params,
                     Cardinal *num_params) {
...
static void MultiActivate(Widget w, XEvent *event, String *params,
                          Cardinal *num_params) {
        /* NOTE: Should print warning in this case. */
...
static void ActivateCommon(Widget w, XEvent *event, String *params,
                           Cardinal *num_params) {
        ...
        if (event->type != ButtonRelease ||
            (event->xbutton.x >= (int)-bw && event->xbutton.y >= (int)-bw &&
             ...
             event->xbutton.y < (int)(tabw->core.height + bw))) {
                ...
                if (tabw->tab.activate_callback) {
                        ...
                        XtCallCallbackList(w, tabw->tab.activate_callback,
                                           &call_value);
                }
        }
}
...
static void ArmAndActivate(Widget w, XEvent *event, String *params,
                           Cardinal *num_params) {
        ...
        if (tabw->core.being_destroyed == False && tabw->tab.timer == 0)
                tabw->tab.timer = XtAppAddTimeOut(
                    XtWidgetToApplicationContext(w), tabw->tab.disarm_delay,
                    arm_timeout, (XtPointer)w);
}
...
static void arm_timeout(XtPointer data, XtIntervalId *id) {
...
static void Disarm(Widget w, XEvent *event, String *params,
                   Cardinal *num_params) {
...
static void Enter(Widget w, XEvent *event, String *params,
                  Cardinal *num_params) {
...
static void Leave(Widget w, XEvent *event, String *params,
                  Cardinal *num_params) {
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args) {
        ...
        tabw->tab.background_gc = tabw->tab.arm_gc = tabw->tab.parent_bg_gc =
            NULL;
...
static void get_arm_gc(Widget w) {
        XGCValues values;
        XtGCMask value_mask;
...
static void get_background_gc(Widget w) {
        XGCValues values;
        XtGCMask value_mask;
        short myindex;
        XFontStruct *fs;
        ...
        _XmFontListSearch(tabw->label.font, XmFONTLIST_DEFAULT_TAG, &myindex,
                          &fs);
...
static void get_parent_bg_gc(Widget w) {
        Widget parent;
        XGCValues values;
        XtGCMask value_mask;
...
static void calc_min_size(Widget w) {
        ...
        tabw->tab.min_size =
            (tabw->primitive.highlight_thickness +
             tabw->primitive.shadow_thickness + 2 /* label area */);
}
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
        ...
        if (tabw->tab.fill_on_arm != current->tab.fill_on_arm &&
            tabw->tab.armed)
                redisplay = True;
...
Widget XyzCreateTabButton(Widget parent, const char *name, ArgList arglist,
                          Cardinal argcount) {
        return (XtCreateWidget(name, xyzTabButtonWidgetClass, parent, arglist,
                               argcount));
}
...
static void draw_background(Widget w) {
        GC tmp_gc;
        static rect_t lab;
        static XRectangle rects[2];
        static XArc arcs[2];
        int radius =
            tabw->tab.current_radius - tabw->primitive.shadow_thickness;
        int r = 0;
        ...
        if (different_bg && !internal_expose) {
                ...
                rects[0].width = rects[1].width = rects[0].height =
                    rects[1].height = tabw->primitive.highlight_thickness +
                                      tabw->tab.current_radius;

                XFillRectangles(XtDisplay(w), XtWindow(w),
                                tabw->tab.parent_bg_gc, rects, 2);
                /* Take care of internal label border when bg matches shadow
                   color. The arc case is handled in the computation of
                   label_rect later. (Technically this should be in the
                   foreground color of this widget, but it's a pain to draw the
                   1 pixel-wide arc, so we use the background color of our
                   parent instead.) */
                if (tabw->tab.arm_color == tabw->primitive.top_shadow_color ||
                    tabw->tab.arm_color ==
                        tabw->primitive.bottom_shadow_color) {
                        XSegment lines[3];
                        /* side edges */
        ...
        if (radius > 0) {
                /* Filled arcs need radius + 1 because they don't overlap rect
                 * bounds */
                circle_to_arc(lab.x + radius, lab.y + radius, radius + 1, 90,
                              180, 0, &arcs[0]);
...
static void draw_label(Widget w, XEvent *event, Region region) {
        GC tmp_gc = NULL;
        ...
        if (tabw->tab.fill_on_arm && tabw->label.label_type == XmSTRING &&
            tabw->tab.armed &&
            tabw->tab.arm_color == tabw->primitive.foreground) {
...
static void draw_shadow(Widget w) {
        if (tabw->primitive.shadow_thickness > 0) {
                ...
                draw_rounded_shadow(
                    XtDisplay(w), XtWindow(w), TOP_GC, BOTTOM_GC, offset,
                    offset, XtWidth(w) - 2 * offset, XtHeight(w) - 2 * offset,
                    tabw->primitive.shadow_thickness, tabw->tab.current_radius,
                    XmSHADOW_OUT);
        }
}
...
static void get_label_rect(Widget w, rect_t *rect) {
        int dx;
        short fill = 0;
...
static void draw_arcs(Display *display, Drawable drawable, GC top_gc,
                      GC bottom_gc, Position x, Position y, Dimension width,
                      Dimension height, Dimension thickness, Dimension radius,
                      unsigned int type) {
        static XArc arcs[3];

        circle_to_arc(x + radius, y + radius, radius, 90, 180, thickness,
                      &arcs[0]);
        circle_to_arc(x + width - radius - 1, y + radius, radius, 45, 90,
                      thickness, &arcs[1]);
        circle_to_arc(x + width - radius - 1, y + radius, radius, 0, 45,
                      thickness, &arcs[2]);

        XSetLineAttributes(display, top_gc, thickness, LineSolid, CapButt,
                           JoinMiter);
        XSetLineAttributes(display, bottom_gc, thickness, LineSolid, CapButt,
                           JoinMiter);
        XDrawArcs(display, drawable, top_gc, &arcs[0], 2);
        XDrawArcs(display, drawable, bottom_gc, &arcs[2], 1);
        XSetLineAttributes(display, top_gc, 0, LineSolid, CapButt, JoinMiter);
        XSetLineAttributes(display, bottom_gc, 0, LineSolid, CapButt,
                           JoinMiter);
}
...
static void draw_rounded_shadow(Display *display, Drawable drawable, GC top_gc,
                                GC bottom_gc, Position x, Position y,
        ...
        int top = thickness * 0, left = thickness * 1, bottom = thickness * 2,
            right = thickness * 3;

        if (segs_needed > seg_count)
                segs = (XSegment *)XtRealloc((char *)segs,
                                             sizeof(XSegment) * segs_needed);
        ...
        draw_arcs(display, drawable, top_gc, bottom_gc, x, y, width, height,
                  thickness, radius, type);
}
...
static void draw_highlight(Widget w) {
        ...
        draw_rounded_shadow(
            XtDisplay(w), XtWindow(w), tabw->primitive.highlight_GC,
            tabw->primitive.highlight_GC, 0, 0, XtWidth(w), XtHeight(w),
            thickness, tabw->tab.current_radius + thickness, XmSHADOW_OUT);
}
...
static void undraw_highlight(Widget w) {
        Dimension thickness = tabw->primitive.highlight_thickness;
        ...
        draw_rounded_shadow(XtDisplay(w), XtWindow(w), tabw->tab.parent_bg_gc,
                            tabw->tab.parent_bg_gc, 0, 0, XtWidth(w),
                            XtHeight(w), thickness,
                            tabw->tab.current_radius + thickness, XmSHADOW_OUT);
}
...
static void circle_to_arc(Position center_x, Position center_y,
                          Dimension radius, int start_angle, int end_angle,
                          Dimension pen_width, XArc *arc) {
        ...
        if (arc->width > 4000 || arc->height > 4000) {
                fprintf(stderr, "Internal tab widget error.  Aborting to "
                                "prevent X crash.\n");
                fprintf(stderr,
                        "radius = %d, pen_width = %d, half_pen_width = %d\n",
