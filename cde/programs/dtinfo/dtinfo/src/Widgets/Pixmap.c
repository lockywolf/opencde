...
static void Initialize(req, new, args, nargs) PixmapWidget req, new;
ArgList args;
Cardinal *nargs;
{
        XtGCMask value_mask;
        ...
        if ((new->core.width == 0) || (new->core.height == 0)) {
                ...
                XtVaGetValues(XtParent(new), XtNwidth, &width, XtNheight,
                              &height, (char *)0);
...
static void Resize(w) PixmapWidget w;
{
        ON_DEBUG(fprintf(stderr, "PixmapWidget: RESIZE (%d, %d)\n",
                         w->core.width, w->core.height));
}

static void Destroy(w) PixmapWidget w;
{
...
static Boolean SetValues(current, request, new, args,
                         num_args) PixmapWidget current,
    request, new;
ArgList args;
Cardinal *num_args;
{
        ...
        if (current->pixmap.pixmap != new->pixmap.pixmap) {
                ...
                /* get pixmap size */
                XGetGeometry(XtDisplay(new), new->pixmap.pixmap, &root, &x, &y,
...
static XtGeometryResult QueryGeometry(w, proposed, answer) PixmapWidget w;
...
static void ReDisplay(w, event) PixmapWidget w;
XExposeEvent *event;
{
        ...
        if (w->pixmap.pixmap != 0) {
                ...
                XSetClipRectangles(display, w->pixmap.gc, /* gc */
                                   0,                     /* clip x origin  */
                                   0,                     /* clip y origin  */
                                   rect,                  /* rect[]  */
                                   1,                     /* num rects  */
...
void PixmapWidgetDimensions(Widget w, Dimension *width, Dimension *height) {
        if (IsPixmapWidget(w)) {
