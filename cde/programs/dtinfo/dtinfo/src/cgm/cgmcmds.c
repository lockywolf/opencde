...
static unsigned int b_guint(
    in_ptr,
    bits_p) unsigned char **in_ptr; /* pointer to the input data pointer */
...
static double b_fixed(dat_ptr, prec) unsigned char **dat_ptr;
rp_type *prec;
{
        double ret;
...
static double b_ieee(dat_ptr, prec) unsigned char **dat_ptr;
rp_type *prec;
{
#define TABLESIZE 128
        ...
        if ((prec->exp + prec->fract) == 32) { /* 32-bit precision */
                ...
                *dat_ptr += 4; /* skip forward */
                ...
                if (big_part == 255) {
                        if (small_part == 0) {
                                /* ret = (double) HUGE_VAL;	 biggest
                                 * possible */
                                ret = (double)1000000; /* biggest possible */
                        } else {
                                burp(stderr,
                                     "IEEE undefined real, small part = %ld, "
                                     "!= 0\n",
                                     small_part);
                                return (0.0);
                        }
...
/* the general real */
static double b_real(dat_ptr, rprec) unsigned char **dat_ptr;
rp_type *rprec;
{
...
int xl_setup(Screen *screen, Display *inDisplay, Window inWin, int inDepth,
        ...
        fprintf(
            stderr,
            "depth = %d, mask = (%ld, %ld, %ld), cmap_size = %d, bits = %d\n",
        ...
        *ret_number = 0;
        ...
        mask =
            GCLineWidth | GCLineStyle | GCFillStyle | GCCapStyle | GCJoinStyle;
...
static float getX(unsigned char **dat_ptr, cgm_s_type *cgm_s) {
        ...
        case VDC_INT:
                return xScale * (b_gsint(dat_ptr, cgm_s->vdcintprec) -
                                 cgm_s->vdcextent[0].i);
        case VDC_REAL:
                return xScale * (b_real(dat_ptr, &(cgm_s->vdcrprec)) -
                                 cgm_s->vdcextent[0].r);
        default:
                return 0; /* where did this come from ? */
        }
}
...
#define XCOLRDIF(colr1, colr2)                                                 \
        (abs(colr1.red - colr2.red) + abs(colr1.green - colr2.green) +         \
...
static Pixel getPixel(cgm_s_type *cgm_s, int iColrs[3]) {
        ...
        for (i = 0; i < 3; ++i) {
                fDiv = cgm_s->cvextent[i + 3] -
                       cgm_s->cvextent[i]; /* for safety */
        ...
        case 0:                         /* Static Grey */
                if (visualDepth == 2) { /* monochrome, a common display */
                        return ((dColrs[0] + dColrs[1] + dColrs[2]) > 0.9)
                                   ? blackPixel
                                   : whitePixel;
                }
        ...
        case 3: /* PseudoColor */
                ...
                if (ret) {
                        ...
                        if (!*ret_number)
                                return ((dColrs[0] + dColrs[1] + dColrs[2]) >
                                        0.9)
                                           ? blackPixel
                                           : whitePixel;
                        ...
                        for (i = 1; i < *ret_number;
                             ++i) { /* real ugly, but do for now */
...
static int bpicbody(unsigned char *dat_ptr, struct cmd_info_s *cmd_ptr,
        ...
        if (cgm_s->scalmode) { /* metric scaling */
                /* there are supposed to be cgm_s->scalmode_f millimetres per
                 * vdc unit */
        ...
        /* set up for clipping */
        myClipRect[0].x = myClipRect[0].y = myClipRect[3].x = myClipRect[1].y =
            0;
        ...
        /* make the Pixmap */
        *retPixmap = P =
            XCreatePixmap(display, win, *ret_width, *ret_height, depth);
...
/* Metafile Version */
static int mfversion(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
...
/* Metafile Descriptor */
static int mfdescrip(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        /* nothing for now */
...
/* VDC type */
static int vdctype(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        /* 0=>integer, 1=>real */
...
/* Integer Precision */
static int intprec(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Real Precision */
static int realprec(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Index Precision */
static int indexprec(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Colour Precision */
static int colprec(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
static int cindprec(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
static int maxcind(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Colour Value Extent */
static int cvextent(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
...
static int mfellist(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
        ...
        if (!(cgm_s->mfellist =
                  (int *)malloc(sizeof(int) * 2 * cgm_s->mfellist_s))) {
...
static int mfdefrep(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        ...
        while (dat_ptr < end_ptr) {  /* some input left */
                ...
                if (!do_b_cmd(&new_cmd, new_ptr, cgm_s)) {
                        burp(stderr,
                             "error in do_b_cmd while replacing defaults\n");
                        return (0);
                }
                ++new_cmd.sub_no;
...
static int fontlist(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
        ...
        my_ptr = dat_ptr;
        for (i = 0;
             (i < MAX_FONTS) && (my_ptr < (dat_ptr + cmd_ptr->p_len - 3));
             ++i) {
...
static int charlist(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
...
/* Character Announcer */
static int charannounce(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Picture Descriptors */
/* Scaling Mode */
static int scalmode(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
static int colselmode(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
static int lwidspecmode(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
static int marksizspecmode(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
static int edwidspecmode(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* VDC Extent */
static int vdcextent(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
...
/* Background Colour */
static int backcolr(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
...
static int vdcintprec(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* VDC Real Precision */
static int vdcrprec(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Auxiliary Colour */
static int auxcolr(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
        ...
        case I_C_M:
                cgm_s->auxcolr[3] = b_guint(&dat_ptr, cgm_s->cindprec);
                break;
        case D_C_M:
                for (i = 0; i < 3; ++i)
...
/* Transparency */
static int transp(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
unsigned int getXPoints(unsigned char *dat_ptr, struct cmd_info_s *cmd_ptr,
                        cgm_s_type *cgm_s, XPoint **inPtr) {
        int i, noPts;
        ...
        if (noPts > mySize) { /* need more points memory */
                if (myPtr)
                        free(myPtr); /* eliminate old memory */
...
unsigned int getXSegments(unsigned char *dat_ptr, struct cmd_info_s *cmd_ptr,
                          cgm_s_type *cgm_s, XSegment **inPtr) {
        int i, noSegments;
        XSegment *myPtr = NULL;
...
/* Polyline */
static int polyline(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int noPts;
...
static int dispoly(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int noSegments;
...
/* Polymarker */
static int polymarker(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        ...
        for (i = 0; i < noPts; ++i) {
                ...
                case 3: /* asterisk, use fallthru */
                        ...
                        XDrawLine(display, P, markGC, x, y - size, x, y + size);
                case 5: /* cross */
                        XDrawLine(display, P, markGC, x - asize, y - asize,
                                  x + asize, y + asize);
                        XDrawLine(display, P, markGC, x - asize, y + asize,
                                  x + asize, y - asize);
                        break;
...
void doText(cgm_s_type *cgm_s) {
        ...
        /* draw the text */
        draw_cgm_text(display, P, textGC, cgm_s, textX, textY, textW, textH,
                      textPtr);
        /* clean up memory */
...
/* Regular Text */
static int text(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int final_flag;
...
/* Restricted Text */
static int restext(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int final_flag;
...
/* Appended Text */
static int apptext(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        partialText *newPtr, *myPtr;
        if (!textPtr)
                return 0; /* can't append if there's nothing started ! */
...
/* Polygon */
static int polygon(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int noPts;
        ...
        if ((noPts < 2) || !myPtr)
                return 0;
        /* fill the polygon */
        XFillPolygon(display, P, fillGC, myPtr, noPts, Complex,
                     CoordModeOrigin);
        /* draw the edge ? */
...
/* Polyset */
static int polyset(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        ...
        switch (cgm_s->vdctype) {
        case VDC_INT:
                noPts = (8 * cmd_ptr->p_len) / (2 * cgm_s->vdcintprec + 16);
                break;
        case VDC_REAL:
                noPts =
                    (8 * cmd_ptr->p_len) /
        ...
        for (i = 0; i < noPts; ++i) {
                ...
                case 1: /* visible edge */
                        if (i < (noPts - 1))
                                XDrawLine(display, P, edgeGC, myPtr[i].x,
                                          myPtr[i].y, myPtr[i + 1].x,
                                          myPtr[i + 1].y);
                        break;
                case 2: /* close, invisible edge */
                        if (i > (lastFill + 2))
                                XFillPolygon(display, P, fillGC,
                                             myPtr + lastFill, i - lastFill + 1,
...
#define AINT(ptr, prec, out, done)                                             \
        switch (prec) {                                                        \
        case 32:                                                               \
                out = (((UINT)ptr[0]) << 24) + (((UINT)ptr[1]) << 16) +        \
                      (((UINT)ptr[2]) << 8) + ptr[3];                          \
                ptr += 4;                                                      \
                break;                                                         \
        case 24:                                                               \
                out = (((UINT)ptr[0]) << 16) + (((UINT)ptr[1]) << 8) + ptr[2]; \
                ptr += 3;                                                      \
                break;                                                         \
        case 16:                                                               \
                out = (((UINT)ptr[0]) << 8) + ptr[1];                          \
                ptr += 2;                                                      \
                break;                                                         \
        case 8:                                                                \
                out = *ptr++;                                                  \
...
/* get a packed list */
static void getListPixels(unsigned char *datPtr, cgm_s_type *cgm_s, int nx,
                          int ny, int prec, Pixel *retPtr) {
...
static void getRLPixels(unsigned char *dat_ptr, cgm_s_type *cgm_s, int nx,
                        int ny, int prec, Pixel *retPtr) {
        ...
        for (i = 0; i < ny; ++i) { /* loop through the rows */
                ...
                if (bitOffset)
                        ++dat_ptr; /* incomplete byte */
                if (((size_t)dat_ptr - (size_t)rowPtr) % 2)
                        ++dat_ptr; /* need round up */
        }                          /* end of cell array */
}

/* build a pixel array, (x, y) = (x + y * nx) storage */
static Pixel *getPixels(unsigned char *dat_ptr, cgm_s_type *cgm_s, int nx,
                        int ny, int localPrec, int repMode) {
        Pixel *retPtr;
...
static int cellarray(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        ...
        if (!(myPixels =
                  getPixels(dat_ptr, cgm_s, nx, ny, local_prec, rep_mode)))
                return 0;
        ...
        xSize = xMax - xMin;
        ySize = yMax - yMin;
        myImage = XGetImage(display, P, xMin, max_y - yMax, xSize, ySize, ~0,
                            ZPixmap);
        ...
        for (iy = 1; iy <= ySize; ++iy) {
                ...
                for (ix = 0; ix < xSize; ++ix) {
                        ...
                        if ((newX >= 0) && (newX < nx) && (newY >= 0) &&
                            (newY < ny)) {
                                XPutPixel(myImage, ix, ySize - iy,
                                          myPixels[newX + newY * nx]);
                        }
                }
        }
        /* copy the XImage back */

        XPutImage(display, P, cellGC, myImage, 0, 0, xMin, max_y - yMax, xSize,
                  ySize);
        /* return the memory */
...
/* Rectangle */
static int rectangle(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Circle */
static int circle(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int x, y, r;
...
static int getArc(float inX[3], float inY[3], int *outXC, int *outYC,
        ...
        /*
          the perpendiculars to the 2 lines thru their midpts will meet at the
          center thus we have 4 eqns for 4 unknowns: xc = x3 + a * (y1 - y0) yc
          = y3 - a * (x1 - x0) xc = x4 + b * (y2 - y1) yc = y4 - b * (x2 - x1)
...
static int circ3(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int xc, yc, r, i;
        ...
        det1 = (x[2] - x[0]) * (y[1] - y[0]) - (x[1] - x[0]) * (y[2] - y[0]);
        if (det1 == 0) { /* co-linear, just connect points */
                XDrawLine(display, P, lineGC, x[0], max_y - y[0], x[1],
                          max_y - y[1]);
                XDrawLine(display, P, lineGC, x[1], max_y - y[1], x[2],
                          max_y - y[2]);
                return 1;
        }
...
static int circ3close(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        ...
        if (close_type)
                XSetArcMode(display, fillGC, ArcChord);
        else
                XSetArcMode(display, fillGC, ArcPieSlice);
        /* now the actual fill */
...
static int circcentre(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i, xc, yc, r;
...
static int circcclose(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
*/
static int getEllipseXPoints(int x1, int y1, int x2, int y2, float dxy[4],
                             XPoint **outPtr) {
        int nSteps, i, rot;
        ...
        */
        ...
        if (rot > 0) { /* move in clockwise direction */
                delta = (theta2 > theta1) ? theta2 - theta1
                                          : theta2 + 2 * pi - theta1;
...
/* Ellipse */
static int ellipse(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Elliptical arc */
static int elarc(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
static int elarcclose(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Line Type */
static int ltype(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        /* get the line type */
...
/* Line Width */
static int lwidth(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        ...
        case ABSOLUTE:
                ...
                case VDC_INT:
                        cgm_s->lwidth.i =
                            xScale * b_gsint(&dat_ptr, cgm_s->vdcintprec);
                        break;
                case VDC_REAL:
                        cgm_s->lwidth.r =
                            xScale * b_real(&dat_ptr, &(cgm_s->vdcrprec));
                        cgm_s->lwidth.i = cgm_s->lwidth.r;
                        break;
                }
                break;
        case SCALED:
                cgm_s->lwidth.s = b_real(&dat_ptr, &(cgm_s->realprec));
...
/* Line Colour */
static int lcolr(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
        ...
        case D_C_M:
                for (i = 0; i < 3; ++i)
                        cgm_s->lcolr[i] = b_guint(&dat_ptr, cgm_s->colprec);
                setRGB(cgm_s, cgm_s->lcolr, lineGC);
                break;
        case I_C_M:
                cgm_s->lcolr[3] = b_guint(&dat_ptr, cgm_s->cindprec);
...
/* Marker Type */
static int mtype(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        /* get the line type */
...
/* Marker Size */
static int msize(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        ...
        case ABSOLUTE:
                ...
                break;
        case SCALED:
                cgm_s->msize.s = b_real(&dat_ptr, &(cgm_s->realprec));
...
/* Marker Colour */
static int mcolr(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
        ...
        case D_C_M:
                for (i = 0; i < 3; ++i)
                        cgm_s->mcolr[i] = b_guint(&dat_ptr, cgm_s->colprec);
                setRGB(cgm_s, cgm_s->mcolr, markGC);
                break;
        case I_C_M:
                cgm_s->mcolr[3] = b_guint(&dat_ptr, cgm_s->cindprec);
...
static int talign(unsigned char *dat_ptr, struct cmd_info_s *cmd_ptr,
        ...
        for (i = 0; i < 2; ++i)
                cgm_s->talign[i].i = b_gsint(&dat_ptr, 16);
        /* now the 2 real bits */
        for (i = 0; i < 2; ++i)
                cgm_s->talign[i].r = b_real(&dat_ptr, &(cgm_s->realprec));

        return 1;
}
/* Character Set Index */
static int csetindex(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
static int altcsetindex(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Interior Style */
static int intstyle(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Fill Colour */
static int fillcolr(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
        ...
        case D_C_M:
                for (i = 0; i < 3; ++i)
                        cgm_s->fillcolr[i] = b_guint(&dat_ptr, cgm_s->colprec);
                setRGB(cgm_s, cgm_s->fillcolr, fillGC);
                break;
        case I_C_M:
                cgm_s->fillcolr[3] = b_guint(&dat_ptr, cgm_s->cindprec);
...
/* Hatch Index */
static int hatchindex(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Pattern Index */
static int patindex(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Edge Type */
static int etype(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Edge Width */
static int ewidth(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        ...
        case ABSOLUTE:
                ...
                break;
        case SCALED:
                cgm_s->ewidth.s = b_real(&dat_ptr, &(cgm_s->realprec));
...
/* Edge Colour */
static int ecolr(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        int i;
        ...
        case D_C_M:
                for (i = 0; i < 3; ++i)
                        cgm_s->ecolr[i] = b_guint(&dat_ptr, cgm_s->colprec);
                setRGB(cgm_s, cgm_s->ecolr, edgeGC);
                break;
        case I_C_M:
                cgm_s->ecolr[3] = b_guint(&dat_ptr, cgm_s->cindprec);
...
/* Edge Visibility */
static int evis(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
...
/* Colour Table */
static int coltab(dat_ptr, cmd_ptr, cgm_s) unsigned char *dat_ptr;
struct cmd_info_s *cmd_ptr;
cgm_s_type *cgm_s;
{
        ...
        no_entries =
            (cmd_ptr->p_len * 8 - cgm_s->cindprec) / (3 * cgm_s->colprec);
...
#define ARG_LIST dat_ptr, cmd_ptr, cgm_s
        ...
        case B_DELIMCLASS:
                ...
                case B_EMF:
                        return (emf(ARG_LIST));
                case B_BPIC:
                        return (bpic(ARG_LIST));
                case B_BPICBODY:
                        return (bpicbody(ARG_LIST));
                case B_EPIC:
                        return (epic(ARG_LIST));
                default:
                        burp(stderr, "illegal Delimiter element %d\n",
                             cmd_ptr->element);
                        return (0);
                }
        ...
        case B_MDESCLASS:
                switch (cmd_ptr->element) {
                case B_MFVERSION:
                        return (mfversion(ARG_LIST));
                case B_MFDESCRIP:
                        return (mfdescrip(ARG_LIST));
                case B_VDCTYPE:
                        return (vdctype(ARG_LIST));
                case B_INTPREC:
                        return (intprec(ARG_LIST));
                case B_REALPREC:
                        return (realprec(ARG_LIST));
                case B_INDEXPREC:
                        return (indexprec(ARG_LIST));
                case B_COLPREC:
                        return (colprec(ARG_LIST));
                case B_CINDPREC:
                        return (cindprec(ARG_LIST));
                case B_MAXCIND:
                        return (maxcind(ARG_LIST));
                case B_CVEXTENT:
                        return (cvextent(ARG_LIST));
                case B_MFELLIST:
                        return (mfellist(ARG_LIST));
                case B_MFDEFREP:
                        return (mfdefrep(ARG_LIST)); /* special */
                case B_FONTLIST:
                        return (fontlist(ARG_LIST));
                case B_CHARLIST:
                        return (charlist(ARG_LIST));
                case B_CHARANNOUNCE:
                        return (charannounce(ARG_LIST));
                default:
                        burp(stderr, "illegal Metafile descriptor element %d\n",
                             cmd_ptr->element);
                        return (0);
                }
        ...
        case B_PDESCLASS:
                switch (cmd_ptr->element) {
                case B_SCALMODE:
                        return (scalmode(ARG_LIST));
                case B_COLSELMODE:
                        return (colselmode(ARG_LIST));
                case B_LWIDSPECMODE:
                        return (lwidspecmode(ARG_LIST));
                case B_MARKSIZSPECMODE:
                        return (marksizspecmode(ARG_LIST));
                case B_EDWIDSPECMODE:
                        return (edwidspecmode(ARG_LIST));
                case B_VDCEXTENT:
                        return (vdcextent(ARG_LIST));
                case B_BACKCOLR:
                        return (backcolr(ARG_LIST));
                default:
                        burp(stderr, "illegal Picture Descriptor element %d\n",
                             cmd_ptr->element);
                        return (0);
                }
        ...
        case B_CTRLCLASS:
                switch (cmd_ptr->element) {
                case B_VDCINTPREC:
                        return (vdcintprec(ARG_LIST));
                case B_VDCRPREC:
                        return (vdcrprec(ARG_LIST));
                case B_AUXCOLR:
                        return (auxcolr(ARG_LIST));
                case B_TRANSP:
                        return (transp(ARG_LIST));
                case B_CLIPRECT:
                        return (cliprect(ARG_LIST));
                case B_CLIPINDIC:
                        return (clipindic(ARG_LIST));
                default:
                        burp(stderr, "illegal Control element %d\n",
                             cmd_ptr->element);
                        return (0);
                }
        ...
        case B_GPRIMCLASS:
                switch (cmd_ptr->element) {
                case B_POLYLINE:
                        return (polyline(ARG_LIST));
                case B_DISPOLY:
                        return (dispoly(ARG_LIST));
                case B_POLYMARKER:
                        return (polymarker(ARG_LIST));
                case B_TEXT:
                        return (text(ARG_LIST));
                case B_RESTEXT:
                        return (restext(ARG_LIST));
                case B_APPTEXT:
                        return (apptext(ARG_LIST));
                case B_POLYGON:
                        return (polygon(ARG_LIST));
                case B_POLYSET:
                        return (polyset(ARG_LIST));
                case B_CELLARRAY:
                        return (cellarray(ARG_LIST));
                case B_GENDPRIM:
                        return 1; /* not used here */
                case B_RECTANGLE:
                        return (rectangle(ARG_LIST));
                case B_CIRCLE:
                        return (circle(ARG_LIST));
                case B_CIRC3:
                        return (circ3(ARG_LIST));
                case B_CIRC3CLOSE:
                        return (circ3close(ARG_LIST));
                case B_CIRCCENTRE:
                        return (circcentre(ARG_LIST));
                case B_CIRCCCLOSE:
                        return (circcclose(ARG_LIST));
                case B_ELLIPSE:
                        return (ellipse(ARG_LIST));
                case B_ELARC:
                        return (elarc(ARG_LIST));
                case B_ELARCCLOSE:
                        return (elarcclose(ARG_LIST));
                default:
                        burp(stderr, "illegal Graphical Primitive element %d\n",
                             cmd_ptr->element);
                        return (0);
                }
        ...
        case B_ATTRCLASS:
                ...
                case B_LBINDEX:
                        return 1;
                case B_LTYPE:
                        return (ltype(ARG_LIST));
                case B_LWIDTH:
                        return (lwidth(ARG_LIST));
                case B_LCOLR:
                        return (lcolr(ARG_LIST));
                case B_MBINDEX:
                        return 1;
                case B_MTYPE:
                        return (mtype(ARG_LIST));
                case B_MSIZE:
                        return (msize(ARG_LIST));
                case B_MCOLR:
                        return (mcolr(ARG_LIST));
                case B_TBINDEX:
                        return 1;
                case B_TFINDEX:
                        return (tfindex(ARG_LIST));
                case B_TPREC:
                        return (tprec(ARG_LIST));
                case B_CEXPFAC:
                        return (cexpfac(ARG_LIST));
                case B_CSPACE:
                        return (cspace(ARG_LIST));
                case B_TCOLR:
                        return (tcolr(ARG_LIST));
                case B_CHEIGHT:
                        return (cheight(ARG_LIST));
                case B_CORIENT:
                        return (corient(ARG_LIST));
                case B_TPATH:
                        return (tpath(ARG_LIST));
                case B_TALIGN:
                        return (talign(ARG_LIST));
                case B_CSETINDEX:
                        return (csetindex(ARG_LIST));
                case B_ALTCSETINDEX:
                        return (altcsetindex(ARG_LIST));
                case B_FILLBINDEX:
                        return 1;
                case B_INTSTYLE:
                        return (intstyle(ARG_LIST));
                case B_FILLCOLR:
                        return (fillcolr(ARG_LIST));
                case B_HATCHINDEX:
                        return (hatchindex(ARG_LIST));
                case B_PATINDEX:
                        return (patindex(ARG_LIST));
                case B_EDBINDEX:
                        return 1;
                case B_ETYPE:
                        return (etype(ARG_LIST));
                case B_EWIDTH:
                        return (ewidth(ARG_LIST));
                case B_ECOLR:
                        return (ecolr(ARG_LIST));
                case B_EVIS:
                        return (evis(ARG_LIST));
                case B_FILLREF:
                        return 1;
                case B_PATTAB:
                        return 1;
                case B_PATSIZE:
                        return 1;
                case B_COLTAB:
                        return (coltab(ARG_LIST));
                case B_ASPSFLAGS:
                        return 1;
                default:
                        burp(stderr, "illegal Attribute element %d\n",
                             cmd_ptr->element);
                        return (0);
                }
