...
static int get_cgm_rec(unsigned char *start_ptr, int buffer_size,
                       _DtGrStream *cgmptr) {
        int bytes_got;
...
int open_cgm_file(_DtGrStream *stream) {
        ...
        if (!(stream->source.file.fileptr =
                ...
                burp(stderr, "opened %s for input\n",
                     stream->source.file.filename);
...
static int get_b_bytes(unsigned char *out_ptr, int bytes_wanted,
                       _DtGrStream *stream) {
        int bytes_got, i;
        if (!begin_ptr) {
        ...
        while (bytes_wanted > 0) {
                if ((bytes_got = get_cgm_rec(begin_ptr, CGMB_SIZE, stream)) <
                    1) {
                        burp(stderr,
                             "couldn't get buffered bytes in get_b_bytes\n");
                        return (0);
                }
...
#define INIT_MEM (50 * 1024) /* initial allocation */
        ...
        if ((!mem_ptr) && (!(mem_ptr = (unsigned char *)malloc(INIT_MEM)))) {
                burp(stderr,
                     "couldn't allocate initial memory for get_b_cmd\n");
                return (NULL);
        } else
                mem_allocated = INIT_MEM;
        ...
        cmd_ptr->element =
            (int)(((mem_ptr[0] << 3) & 127) | ((mem_ptr[1] >> 5) & 7));
        cmd_ptr->p_len = (int)(mem_ptr[1] & 31);

        /* is this a short form command ? */
        if (cmd_ptr->p_len < 31) {
                if (!get_b_bytes(mem_ptr,
                                 (cmd_ptr->p_len % 2) ? cmd_ptr->p_len + 1
                                                      : cmd_ptr->p_len,
                                 stream)) {
                ...
                while (data_left) { /* some data still to get */
                        ...
                        if ((cmd_ptr->p_len + new_len) > mem_allocated) {
                                if (!(mem_ptr = (unsigned char *)realloc(
                                          mem_ptr,
                                          mem_allocated =
                                              cmd_ptr->p_len + new_len))) {
                                        burp(stderr,
                                             "couldn't get %d bytes in "
                                             "get_b_cmd\n",
                                             mem_allocated);
                                        return (NULL);
                                } else if (io_debug)
                        ...
                        if (!get_b_bytes(mem_ptr + cmd_ptr->p_len, new_len,
                                         stream)) {
                                burp(
                                    stderr,
                                    "couldn't get next %d bytes in get_b_cmd\n",
                                    new_len);
                                return (NULL);
                        }
                        cmd_ptr->p_len += new_len;
