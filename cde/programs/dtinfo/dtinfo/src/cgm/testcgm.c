...
main(int argc, char **argv) {
        ...
        win = XCreateSimpleWindow(display, RootWindow(display, scr_num), 0, 0,
                                  512, 512, 2, BlackPixel(display, scr_num),
        ...
        /* and pagenumber */
        if ((argc < 3) || !sscanf(argv[2], " %d", &pageNo))
                pageNo = 1;
        ...
        status =
            _DtGrLoad(&stream, NULL, ScreenOfDisplay(display, scr_num),
                      wa.depth, wa.colormap, wa.visual,
                      BlackPixelOfScreen(ScreenOfDisplay(display, scr_num)),
                      WhitePixelOfScreen(ScreenOfDisplay(display, scr_num)),
                      myGC, _DtGrCOLOR, True, &ret_width, &ret_height, 100,
                      &myPixmap, NULL, &ret_colors, &ret_number, &gr_context);
        ...
        scanf("%*c");
        /* display the Pixmap */
        XCopyArea(display, myPixmap, win, myGC, 0, 0, ret_width, ret_height, 0,
                  0);
        XFlush(display);
