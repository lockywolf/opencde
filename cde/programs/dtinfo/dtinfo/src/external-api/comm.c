...
static void encode_display_event(OliasDisplayEvent *event, XtPointer *value,
...
static Boolean encode_event(OliasEvent *event, XtPointer *value,
                            unsigned long *length, int *format) {
        switch (event->type) {
        case OLIAS_DISPLAY_EVENT:
                encode_display_event(&event->display_event, value, length,
                                     format);
                break;
...
Boolean _olias_convert_event(Widget w, Atom *selection, Atom *target,
...
OliasStatus olias_send_event(Widget toplevel, OliasEvent *event) {
        Window olias_server;
        ...
        if (olias_server == None) {
                /* fprintf (stderr, "OLIAS server not running or
                 * inaccessible.\n"); */
...
void _olias_handle_reply(Widget w, XtPointer client_data, XEvent *event,
                         Boolean *cont) {
...
static Boolean check_event(Display *display, XEvent *event,
...
static void wait_for_reply(Widget toplevel) {
        Display *display = XtDisplay(toplevel);
        int status;
        int width;
        fd_set readfds;
        ...
        while (reply_status == NO_REPLY) {
                ...
                if (status == -1) {
                        ...
                        if (current_time.tv_usec < start_time.tv_usec) {
                                time_spent.tv_sec =
                                    current_time.tv_sec - start_time.tv_sec - 1;
                                time_spent.tv_usec = 1000000 +
                        ...
                        if (time_spent.tv_sec > timeout.tv_sec ||
                            (time_spent.tv_sec == timeout.tv_sec &&
                             time_spent.tv_usec > timeout.tv_sec)) {
                                ON_DEBUG(
                                    puts(">> timed out waiting for reply"));
                                reply_status = OLIAS_TIMEOUT;
                                break;
                        }
                        ...
                        if (timeout.tv_usec < time_spent.tv_usec) {
                                timeout.tv_sec =
                                    timeout.tv_sec - time_spent.tv_sec - 1;
                                timeout.tv_usec = 1000000 + timeout.tv_usec -
                                                  time_spent.tv_usec;
                        } else {
                                timeout.tv_sec =
                                    timeout.tv_sec - time_spent.tv_sec;
                                timeout.tv_usec =
                                    timeout.tv_usec - time_spent.tv_usec;
                        }
                ...
                while (reply_status == NO_REPLY &&
                       XCheckIfEvent(display, &event, (Bool(*)())check_event,
                                     NULL)) {
                        /* puts ("d"); */
                        XtDispatchEvent(&event);
                        /* Stop this madness if something updated the
                         * reply_status. */
                }
                XFlush(display);
        }
