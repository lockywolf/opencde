...
class WXmPopupMenu : public WXmRowColumn {
      ...
      public:
        WXmPopupMenu() {}
        WXmPopupMenu(WComposite &father, char *name = NULL, ArgList args = NULL,
                     Cardinal card = 0);
        ...
        WXmPushButtonGadget AddPushButton(char *name, WWL *, WWL_FUN,
                                          ArgList args = NULL,
                                          Cardinal card = 0);
        ...
        inline void Popup() { Manage(); }
        inline void Popdown() { Unmanage(); }
};
...
class WXmPulldownMenu : public WXmRowColumn {
      ...
      public:
        WXmPulldownMenu(WComposite &father, char *name, ArgList args = NULL,
                        Cardinal card = 0)
            : WXmRowColumn(
                  XmCreatePulldownMenu(Widget(father), name, args, card)) {}

        WXmPulldownMenu(WComposite &father, char *name, WArgList args)
            : WXmRowColumn(XmCreatePulldownMenu(Widget(father), name,
...
class WXmOptionMenu : public WXmRowColumn {
      ...
      public:
        ...
        WXmOptionMenu(WComposite &father, char *name, ArgList args,
                      Cardinal card)
            : WXmRowColumn(
                  XmCreateOptionMenu(Widget(father), name, args, card)) {}
        ...
        WXmCascadeButtonGadget OptionButtonGadget(void);

        WXmLabelGadget OptionLabelGadget(void);
};
...
class WXmMenuBar : public WXmRowColumn {
      ...
      public:
        ...
        WXmCascadeButton AddCascadeButton(char *name, char mnemonic = 0);

        inline WXmCascadeButton AddCascadeMenu(char *name, char mnemonic,
                                               WXmPulldownMenu &menu) {
                WXmCascadeButton cb(AddCascadeButton(name, mnemonic));
                cb.SubMenuId(menu);
                return cb;
        }

        inline WXmCascadeButton AddCascadeMenu(char *name,
                                               WXmPulldownMenu &menu) {
