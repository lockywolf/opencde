...
class WXmList : public WXmPrimitive {
      public:
...
#endif
        inline void DeleteAllItems(void) const { XmListDeleteAllItems(widget); }
        ...
        inline void DeletePos(int p) const { XmListDeletePos(widget, p); }
#if XmVersion >= 1002
        inline void DeletePositions(int *p, int c) const {
                XmListDeletePositions(widget, p, c);
        }
#endif
        inline void DeselectAllItems() const { XmListDeselectAllItems(widget); }
        inline void DeselectItem(WXmString s) const {
                XmListDeselectItem(widget, s);
        }
        inline void DeselectPos(int i) const { XmListDeselectPos(widget, i); }
#if XmVersion >= 1002
        inline int GetKbdItemPos() const { return XmListGetKbdItemPos(widget); }
#endif
        ...
        inline void SetAddMode(Boolean b) const { XmListSetAddMode(widget, b); }
        inline void SetBottomItem(WXmString s) const {
                XmListSetBottomItem(widget, s);
        }
        inline void SetBottomPos(int p) const { XmListSetBottomPos(widget, p); }
        inline void SetHorizPos(int p) const { XmListSetHorizPos(widget, p); }
        inline void SetItem(WXmString s) const { XmListSetItem(widget, s); }
#if XmVersion >= 1002
        inline Boolean SetKbdItemPos(int p) const {
                return XmListSetKbdItemPos(widget, p);
        }
#endif
        inline void SetPos(int p) const { XmListSetPos(widget, p); }
#if XmVersion >= 1002
        inline void UpdateSelectedList() const {
                XmListUpdateSelectedList(widget);
        }
        inline int YToPos(Position y) const { return XmListYToPos(widget, y); }
#endif
        ...
        DEFINE_GETTER_SETTER(ScrollBarDisplayPolicy, unsigned char,
