...
class WXmText : public WXmPrimitive {
      public:
        ...
        inline void ClearSelection(Time t) const {
                XmTextClearSelection(widget, t);
        }
        inline Boolean Copy(Time copy_time) const {
                return XmTextCopy(widget, copy_time);
        }
        inline Boolean Cut(Time cut_time) const {
                return XmTextCut(widget, cut_time);
        }
        ...
        // inline	XmTextPosition	GetCursorPosition () const { return
        // XmTextGetCursorPosition (widget); }
        inline Boolean GetEditable() const { return XmTextGetEditable(widget); }
        inline XmTextPosition GetInsertionPosition() const {
                return XmTextGetInsertionPosition(widget);
        }
        inline XmTextPosition GetLastPosition() const {
                return XmTextGetLastPosition(widget);
        }
        inline XmTextPosition LastPosition() const {
                return XmTextGetLastPosition(widget);
        }
        ...
        inline Boolean GetSelectionPosition(XmTextPosition *left,
                                            XmTextPosition *right) const {
                return XmTextGetSelectionPosition(widget, left, right);
        }
        inline XmTextSource GetSource() const {
                return XmTextGetSource(widget);
        }
        inline char *GetString() const { return XmTextGetString(widget); }
        inline XmTextPosition GetTopCharacter() const {
                return XmTextGetTopCharacter(widget);
        }
        inline void Insert(XmTextPosition position, char *value) const {
                XmTextInsert(widget, position, value);
        }
        inline Boolean Paste(Widget twidget) const {
                return XmTextPaste(twidget);
        }
        inline Boolean PosToXY(XmTextPosition position, Position *x,
                               Position *y) const {
                return XmTextPosToXY(widget, position, x, y);
        }
        inline Boolean Remove() const { return XmTextRemove(widget); }
        inline void Replace(XmTextPosition from, XmTextPosition to,
                            char *s) const {
                XmTextReplace(widget, from, to, s);
        }
        inline void Scroll(int n) const { XmTextScroll(widget, n); }
        inline void SetAddMode(Boolean state) const {
                XmTextSetAddMode(widget, state);
        }
        // Non-published routine, use SetInsertionPosition - DJB
        // inline	void	SetCursorPosition (XmTextPosition position) const
        // { XmTextSetCursorPosition (widget, position); }
        inline void SetEditable(Boolean editable) const {
                XmTextSetEditable(widget, editable);
        }
        inline void SetHighlight(XmTextPosition left, XmTextPosition right,
                                 XmHighlightMode mode) const {
                XmTextSetHighlight(widget, left, right, mode);
        }
        inline void SetInsertionPosition(XmTextPosition position) const {
                XmTextSetInsertionPosition(widget, position);
        }
        inline void InsertionPosition(XmTextPosition position) const {
                XmTextSetInsertionPosition(widget, position);
        }
        inline void SetMaxLength(int max_length) const {
                XmTextSetMaxLength(widget, max_length);
        }
        inline void SetSelection(XmTextPosition first, XmTextPosition last,
                                 Time set_time) const {
                XmTextSetSelection(widget, first, last, set_time);
        }
        inline void SetSource(XmTextSource source, XmTextPosition top_character,
                              XmTextPosition cursor_position) const {
        ...
        inline void SetTopCharacter(XmTextPosition top_character) const {
                XmTextSetTopCharacter(widget, top_character);
        }
        inline void ShowPosition(XmTextPosition position) const {
                XmTextShowPosition(widget, position);
        }
        inline XmTextPosition XYToPos(Position x, Position y) const {
                return XmTextXYToPos(widget, x, y);
        }
        DEFINE_CALLBACK(ActivateCallback, "activateCallback")
        DEFINE_GETTER_SETTER(AutoShowCursorPosition, Boolean,
