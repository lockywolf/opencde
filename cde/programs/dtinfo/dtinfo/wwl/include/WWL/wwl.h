...
#define DEFINE_GETTER(rsc, typ, rnam)                                          \
        inline typ rsc() const {                                               \
                Arg a;                                                         \
                typ __value;                                                   \
                a.name = CASTRNAM rnam;                                        \
                a.value = (XtArgVal)&__value;                                  \
                _Get(a);                                                       \
                return __value;                                                \
        }
...
#define DEFINE_INIT(class_name, base_name, class)                              \
        ...
        inline class_name(const WComposite &father, WidgetClass c = class,     \
                          const char *name = _stringify(class_name),           \
                          ArgList args = NULL, Cardinal card = 0)              \
            : base_name(father, c, name, args, card) {}                        \
                                                                               \
        inline class_name(const WComposite &father, const char *name,          \
                          ArgList args = NULL, Cardinal card = 0)              \
        ...
        inline class_name(const WComposite &father, int automanage,            \
                          WidgetClass c = class,                               \
                          const char *name = _stringify(class_name),           \
                          ArgList args = NULL, Cardinal card = 0)              \
            : base_name(father, automanage, c, name, args, card) {}            \
                                                                               \
        inline class_name(const WComposite &father, const char *name,          \
                          int automanage, ArgList args = NULL,                 \
                          Cardinal card = 0)                                   \
            : base_name(father, automanage, class, name, args, card) {}        \
                                                                               \
        inline class_name(const WComposite &father, const char *name,          \
                          int automanage, WArgList args)                       \
...
#define DEFINE_POPUP_INIT(class_name, base_name, class)                        \
        inline class_name(const WComposite &father, Boolean popup,             \
                          WidgetClass c = (WidgetClass) class,                 \
                          const char *name = _stringify(class_name),           \
                          ArgList args = NULL, Cardinal card = 0)              \
            : base_name(father, popup, c, name, args, card) {}                 \
        inline class_name(const WComposite &father, Boolean popup,             \
                          const char *name, ArgList args = NULL,               \
                          Cardinal card = 0)                                   \
            : base_name(father, popup, (WidgetClass) class, name, args,        \
                        card) {}                                               \
        inline class_name(const WComposite &father, Boolean popup,             \
                          const char *name, WArgList args)                     \
            : base_name(father, popup, (WidgetClass) class, name, args.Args(), \
                        args.NumArgs()) {}                                     \
...
#define DEFINE_CALLBACK(cb_name, cb_res)                                       \
        inline void name2(Add, cb_name)(XtCallbackProc proc,                   \
                                        caddr_t closure = 0) const {           \
                XtAddCallback(widget, cb_res, proc, closure);                  \
        }                                                                      \
        inline void name2(Remove, cb_name)(XtCallbackProc proc,                \
                                           caddr_t closure = 0) const {        \
        ...
        inline WCallback *name2(Set, cb_name)(WWL * obj, WWL_FUN fun,          \
                                              void *client = 0) {              \
                return SetCallback(cb_res, obj, fun, client);                  \
        }
