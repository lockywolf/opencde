...
#define RSRC_SET(RSC, TYP, RNAM)                                               \
        inline WArgList &RSC(TYP val) {                                        \
                return Add(CASTRNAM RNAM, CASTVAL val);                        \
        }
...
class WArgList {
      ...
      public:
        ...
        RSRC_SET(ScrollBarDisplayPolicy, unsigned char,
        ...
        RSRC_SET(ScrolledWindowMarginHeight, short,
        ...
        RSRC_SET(ChildHorizontalAlignment, unsigned char,
                 "childHorizontalAlignment")
        RSRC_SET(ChildHorizontalSpacing, Dimension, "childHorizontalSpacing")
        RSRC_SET(ChildVerticalAlignment, unsigned char,
        ...
        inline WArgList &LeftAttachment(unsigned char to_what, int pos,
                                        int off) {
                LeftAttachment(to_what);
                LeftPosition(pos);
                LeftOffset(off);
        ...
        inline WArgList &RightAttachment(unsigned char to_what, int posoff) {
                RightAttachment(to_what);
                if (to_what == XmATTACH_POSITION)
                        RightPosition(posoff);
                else
                        RightOffset(posoff);
                return *this;
        }
        inline WArgList &RightAttachment(unsigned char to_what, int pos,
                                         int off) {
                RightAttachment(to_what);
                RightPosition(pos);
                RightOffset(off);
        ...
        inline WArgList &TopAttachment(unsigned char to_what, int pos,
                                       int off) {
                TopAttachment(to_what);
                TopPosition(pos);
                TopOffset(off);
        ...
        inline WArgList &BottomAttachment(unsigned char to_what, int posoff) {
                BottomAttachment(to_what);
                if (to_what == XmATTACH_POSITION)
                        BottomPosition(posoff);
                else
                        BottomOffset(posoff);
                return *this;
        }
        inline WArgList &BottomAttachment(unsigned char to_what, int pos,
                                          int off) {
                BottomAttachment(to_what);
                BottomPosition(pos);
                BottomOffset(off);
...
inline Arg &WArgList::operator[](Cardinal n) {
