...
class WXmString {
      ...
      public:
        ...
        inline WXmString(XmString s, Boolean copy = True) {
                if (copy)
        ...
        inline Boolean operator==(XmString s) const { return ByteCompare(s); }
        ...
        inline void Extent(XmFontList fl, Dimension *w, Dimension *h) const {
                XmStringExtent(fl, string, w, h);
        }
#if XmVersion >= 1002
        inline Boolean GetLtoR(char **text,
                               char *tag = XmFONTLIST_DEFAULT_TAG) const {
                return XmStringUngenerate(string, tag, text);
        }
#else
        inline Boolean
        GetLtoR(char **text,
                XmStringCharSet cs = XmSTRING_DEFAULT_CHARSET) const {
        ...
        inline int LineCount() const { return XmStringLineCount(string); }
