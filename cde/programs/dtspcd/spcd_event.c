 ...
 */
/*
 * File:         spcd_event.c $XConsortium: spcd_event.c /main/4 1996/01/15
 * 13:49:32 rswiston $ Language:     C
 *
...
struct {
        SbInputCallbackProc handler;
        void *data;
} SPCD_input_handlers[FD_SETSIZE], SPCD_except_handlers[FD_SETSIZE];
...
void SPCD_RemoveInput(SbInputId id) { FD_CLR(id, &Sb_Input_Mask); }

void SPCD_RemoveException(SbInputId id) { FD_CLR(id, &Sb_Except_Mask); }
...
void SPCD_MainLoopUntil(Boolean *flag) {
        ...
        do {
                ...
                do
                        result = select(
                            SPCD_max_fd + 1, FD_SET_CAST(&input_mask),
                            FD_SET_CAST(NULL), FD_SET_CAST(&except_mask), NULL);
                ...
                /* Modified loop to break after a single hit on the select.
                 * This */
                ...
                for (fd = 0; fd < SPCD_max_fd + 1; fd++) {
                        ...
                        if (FD_ISSET(fd, &except_mask)) {
                                (*SPCD_except_handlers[fd].handler)(
                                    SPCD_except_handlers[fd].data, &fd, &id);
                                break;
                        }
                        if (FD_ISSET(fd, &input_mask)) {
                                (*SPCD_input_handlers[fd].handler)(
                                    SPCD_input_handlers[fd].data, &fd, &id);
                                break;
                        }
                }
        } while (!*flag);
...
void SPCD_BreakMainLoop(void) { /* no need to do anything */
}
