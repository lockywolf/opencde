...
void SPCD_Handle_Application_Data(void *chn, XeString text, int size,
                                  int conn_type);
void SPCD_Termination_Handler(SPC_Channel_Ptr channel, int pid, int type,
                              int cause, void *data);
...
void SPCD_Reply(SPC_Connection_Ptr connection, protocol_request_ptr prot,
...
/*----------------------------------------------------------------------+*/
{
        ...
        for (i = 1; i < argc; i++) {
                ...
                if (!strcmp("-log", argv[i])) {
                        ...
                        /* Open an error log with whatever name the library
                         * wants to use */
                        ...
                        /*
                         * Used to override the default directory for
                         * authentication file
                         */
                        i++;
                        if (i != argc) {
                                SPC_Format_Log((XeString) "Authentication "
                                                          "directory set to "
                                                          "'%s'.",
                                               argv[i]);
                        ...
                        i++;
                        if (i != argc) {
                                (void)sprintf(tmp, "%s=%s", MOUNT_POINT,
                                              argv[i]);
                                if (putenv(tmp) == 0) {
                                        SPC_Format_Log((XeString) "Mount point "
                                                                  "set to "
                                                                  "'%s'.",
                                                       argv[i]);
                                        SPC_mount_point_env_var =
                                            (char *)malloc(strlen(tmp) + 1);
                                        (void)strcpy(SPC_mount_point_env_var,
                                                     tmp);
                                } else
                                        SPC_Format_Log(
                                            (XeString) "Failed to add the "
                                                       "mount point '%s' to "
                                                       "the "
                                                       "environment.",
                                            tmp);
                        ...
                        /*
                         * The timeout is specified, so use it instead of the
                         * default.
                         */
                        i++;
                        if (i != argc) {
                                exit_timeout = atoi(argv[i]);
                                SPC_Format_Log((XeString) "Setting the exit "
                                                          "timer to '%s' "
                                                          "minutes.",
                                               argv[i]);
                        }
                         ...
                         */
                        SPC_Format_Log(
                            (XeString) "Command line option '%s' unrecognized.",
                            argv[i]);
                }
        }
        ...
        SPC_XtAddInput(&client_channel, &client_connection->termination_id,
                       client_connection->sid, SPCD_Handle_Client_Data,
                       SPC_Client);

        if (exit_timeout != SPCD_NO_TIMER) {
                alarm_vector.sa_handler = SPCD_Alarm_Handler;
                alarm_vector.sa_flags = 0;
                (void)sigaction(SIGALRM, &alarm_vector,
        ...
        if (SPC_mount_point_env_var == NULL)
                if ((pch = getenv(MOUNT_POINT)) != NULL) {
                        SPC_mount_point_env_var = (char *)malloc(
                            strlen(pch) + strlen(MOUNT_POINT) + 2);
                        (void)sprintf(SPC_mount_point_env_var, "%s=%s",
                                      MOUNT_POINT, pch);
                }
...
/*----------------------------------------------------------------------+*/
void SPCD_Handle_Client_Data(void *channel, int *UNUSED_PARM(source),
...
/*----------------------------------------------------------------------+*/
{
        ...
        (void)sprintf(sys_env_file, "%s/%s", SPCD_ENV_INSTALL_DIRECTORY,
                      SPCD_ENV_FILE);
        default_environment =
            SPC_Add_Env_File(sys_env_file, default_environment);

        /*
         * Now add the configured environment file.
         */
        sys_env_file =
            (XeString)realloc(sys_env_file, strlen(SPCD_ENV_CONFIG_DIRECTORY) +
                                                strlen(SPCD_ENV_FILE) + 3);
        (void)sprintf(sys_env_file, "%s/%s", SPCD_ENV_CONFIG_DIRECTORY,
                      SPCD_ENV_FILE);
...
/*----------------------------------------------------------------------+*/
void SPCD_Termination_Handler(SPC_Channel_Ptr channel, int UNUSED_PARM(pid),
                              int UNUSED_PARM(type), int UNUSED_PARM(cause),
                              void *UNUSED_PARM(data))
/*----------------------------------------------------------------------+*/
{
        /* Write a termination protocol request */

        SPC_Write_Protocol_Request(client_connection, channel, APPLICATION_DIED,
                                   channel->status);
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (strcmp(username, "") != 0) {
                 ...
                 * back to the client after creating the proper path for the
                 * file.
                 */
                if (SPCD_Authentication_Dir)
                        ...
                        /*
                         * Use the $HOME directory if it can be retrieved from
                         * the password file.
                         */
                        if (!(pwent = getpwnam(username))) {
                                SPC_Format_Log(
                                    "+++> FAILURE: username '%s' is unknown.",
                                    username);
                                SPC_Write_Protocol_Request(
                                    client_connection, NULL, LOGFILE_REPLY,
                ...
                /*
                 * tempnam(3) has side effects caused by permissions of the
                 * directory given or the TMPDIR envirnoment variable.  Because
                 * of these side effects, the function may return
                 * "/tmp/.SPC_xxxxxx" and ignore tmppath.  The protocol will
                 * fail when this occurs.  The fix is to construct the tmpfile
                 * name.
                 */
                tmpnam(tmpnam_buf);
                spc_suffix =
                    basename(tmpnam_buf); /* Don't free result - not alloc'd! */
    ...
    XeFree(tmppath);
                ...
                SPC_Write_Protocol_Request(client_connection, NULL,
                                           LOGFILE_REPLY, FAILED_FILE_NAME,
                                           NULL, NULL);
                FREE_USER_PASS(username, passwd);
                SPC_Error(SPC_Bad_Username);
                return (SPC_ERROR);
        ...
        if (SPC_client_version_number >= SPC_PROTOCOL_VERSION_CDE_BASE) {
                ...
                if (tt_ptr_error(netfile) != TT_OK) {
                        SPC_Write_Protocol_Request(
                            ...
                            FAILED_FILE_NAME, NULL, NULL);
                        SPC_Format_Log(
                            "+++> FAILURE: cannot create a cannonical file "
                            "name for "
                ...
                free_netfile = 1;
                SPC_Format_Log("     Authentication 'netfile' is '%s'.",
                               netfile);
        }
        ...
        if (lstat(tmpfile, &buf) == ERROR) {
                ...
                SPC_Write_Protocol_Request(client_connection, NULL,
                                           LOGFILE_REPLY, FAILED_FILE_NAME,
                                           NULL, NULL);
                SPC_Format_Log("+++> FAILURE: lstat authentication file '%s'.",
                               tmpfile);
        ...
        if (S_ISLNK(buf.st_mode)) { /* somebody is jerkin us around */
                SPC_Write_Protocol_Request(client_connection, NULL,
                                           LOGFILE_REPLY, FAILED_FILE_NAME,
                                           NULL, NULL);
                SPC_Format_Log("+++> FAILURE: lstat authentication file '%s' "
                               "is a symlink! "
                               "Possible compromise attempt.",
                               tmpfile);
                if (free_netfile)
                        tt_free(netfile);
                XeFree(tmpfile);
        ...
        if (!(buf.st_mode & S_ISUID)) {
                SPC_Write_Protocol_Request(client_connection, NULL,
                                           ...
                                           NULL, NULL);
                SPC_Format_Log(
                    "+++> FAILURE: authentication file '%s' does not have the "
                    "setuid bit set.",
                    tmpfile);
                if (free_netfile)
                        tt_free(netfile);
        ...
        if (!(pwent = getpwuid(buf.st_uid))) {
                SPC_Write_Protocol_Request(client_connection, NULL,
                                           ...
                                           NULL, NULL);
                SPC_Format_Log(
                    "+++> FAILURE: the authentication file created by the "
                    "client has a uid '%d'\n     and this uid is not in the "
                    "password file.",
                    buf.st_uid);
                SPC_Error(SPC_Bad_Username);
                if (free_netfile)
        ...
        if (strcmp(pwent->pw_name, username) != 0) {
                SPC_Write_Protocol_Request(client_connection, NULL,
                                           ...
                                           NULL, NULL);
                SPC_Format_Log(
                    "+++> FAILURE: the request is for username '%s' with uid "
                    "'%d'\n     but this uid has name '%s' in the password "
                    "file.",
                    username, buf.st_uid, pwent->pw_name);
...
/*----------------------------------------------------------------------+*/
{
        ...
        iomode &= ~(SPCIO_WAIT | SPCIO_LINEORIENTED | SPCIO_USE_XTOOLKIT |
                    SPCIO_SYSTEM);
        ...
        if ((stdLang = SPC_Getenv((XeString) "LANG", channel->envp)) !=
            (XeString)NULL) {
                ...
                if (_DtLcxOpenAllDbs(&db) == 0) {
                        if ((_DtXlateGetXlateEnv(db, platform, &execVer,
                                                 &compVer) == 0) &&
                            (_DtLcxXlateStdToOp(
                                 db, platform, compVer, DtLCX_OPER_SETLOCALE,
                                 stdLang, NULL, NULL, NULL, &myLang) == 0)) {
                                if ((langBuf = (XeChar *)malloc(
                                         (strlen(myLang) + 6) *
                                         sizeof(XeChar))) != (XeChar *)NULL) {
                                        sprintf(langBuf, "LANG=%s", myLang);
                                        channel->envp =
                                            SPC_Putenv(langBuf, channel->envp);
                                        free(langBuf);
                                }
...
/*----------------------------------------------------------------------+*/
{
        ...
        READ_APPLICATION_SPAWN(prot->dataptr, channel->path,
                               channel->context_dir, channel->argv,
                               channel->envp);
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (client_connection->protocol_version >= 2) {
                 ...
                 */
                /* part of XPG3 so we don't honor it .. nobody ever used it
                 * anyway */

                int ok =
                    sscanf(signame, "%x", &sig); /* WRITE_INT (%x) was used */
                if ((ok == EOF) || (sig == 0)) {
...
/*----------------------------------------------------------------------+*/
{
        ...
        return (XeSPCWrite(channel, pdata->data + REQUEST_HEADER_LENGTH,
                           pdata->len));
}
...
/*----------------------------------------------------------------------+*/
{
        ...
        time(&timeval);
        fprintf(SPC_Print_Protocol, (XeString) "Begin protocol filedump: %s",
                ctime(&timeval));
        return (TRUE);
}
...
{
        if (strncmp(env_str,
                    (XeString) "HOME=", 5) && /* HOME may be different */
            strncmp(env_str,
                    (XeString) "PWD=", 4) /* PWD set by chdir, may change */
...
/*----------------------------------------------------------------------+*/
{
        ...
        ret = SPC_Get_Multi_Packet(client_connection, prot, ret, &outlen,
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (SPC_client_version_number >= SPC_PROTOCOL_VERSION_CDE_BASE &&
            IS_SPCIO_USE_LOGFILE(channel->IOMode)) {
                ...
                if (tt_ptr_error(netfile) != TT_OK) {
                        SPC_Format_Log(
                            "+++> FAILURE: cannot create a 'netfile' name for "
                            "the "
                            "logfile.\n     (%s)",
                            tt_status_message(tt_pointer_error(netfile)));
                        return (SPC_ERROR);
                ...
                SPC_Write_Protocol_Request(client_connection, channel,
                                           LOGFILE_REPLY, netfile, NULL, NULL);
                tt_free(netfile);
        } else
                SPC_Write_Protocol_Request(client_connection, channel,
                                           LOGFILE_REPLY, channel->logfile,
                                           NULL, NULL);
...
int Client_Enhanced_Spawn(protocol_request_ptr prot) {
        ...
        if (numenv == 0)
                ...
                channel->envp =
                    (char **)XeMalloc((numenv + 1) * sizeof(char *));
                channel->envp[0] = NULL;
                ...
                channel->envp =
                    SPC_Merge_Envp(channel->envp, default_environment);
        }
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (SPC_pid_list != NULL)
                for (i = 0; SPC_pid_list[i] != NULL; i++) {
                        if (SPC_pid_list[i] != SPCD_DEAD_PROCESS) {
                                /*
                                 * Have at least one sub- process running so
                                 * reset the alarm and return to select.
                                 */
                                (void)alarm(exit_timeout * 60);
                                return;
                        }
        ...
        if (request_pending != SPCD_REQUEST_PENDING) {
                SPC_Format_Log((XeString) "Exit timer expired after '%d' "
