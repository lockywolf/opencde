...
#endif /* __aix  */
{
        ...
        if (rediscoverSigCldG) {
                if (shutdownPhaseG == SDP_FINAL_LINGER) {
                        ...
                        /*
                         * Still shutting down, so flip requestTypeG so that
                         * dtexec will not linger when the shutdown process
                         * completes.
                         */
...
#endif /* __aix  */
{
        ...
        if (pid == -1) {
                ...
                /*
                 * The SIGCLD was *not* the result of being stopped, so the
                 * child has indeed terminated.
                 */
                rediscoverSigCldG = 1;
        }
}
...
static int ExecuteCommand(char **commandArray) {
        ...
        for (index1 = 0; (index1 < 10) && ((childPidG = fork()) < 0);
             index1++) {
                /* Out of resources ? */
                if (errno != EAGAIN)
                        break;
        ...
        for (argc--, argv++; argc > 0; argc--, argv++) {
                if (!strcmp(argv[0], "-open")) {
                        ...
                        if (argc == 0)
                                ...
                                /*
                                 * Pull the -ttprocid argument apart for it's 3
                                 * components. <libDtSvc's ProcId>_<Invocation
                                 * Id>_<Child Id>
                                 */
#ifdef _DTEXEC_NLS16
                        tmpi = mblen(argv[0]);
                        ...
                        if (tick1 && tick2) {
                                ...
                                *tick2 = 0;
                                ...
                                if (!(dtSvcInvIdG && dtSvcChildIdG)) {
                                        /*
                                         * Don't have two non-zero values, so we
                                         * cannot use the -ttprocid provided.
                                         */
                                        free(dtSvcProcIdG);
                        ...
                        if (argc == 0)
                                Help(argv2);
                        tmpFileCntG++;
                        tmpFilesG = (char **)realloc(
                            (char *)tmpFilesG, tmpFileCntG * sizeof(char *));
                        tmpFilesG[tmpFileCntG - 1] = argv[0];
                } else if (!strncmp(argv[0], "-h", 2)) {
                        Help(argv2);
                        /*
                         * Technically we should see if a -ttprocid was given
                         * and possibly send back a Done(Request).
                         */
                        (void)_exit(1);
                } else {
                        return (argv);
...
Tt_callback_action IdSelfToCallerReplyCB(Tt_message msg, Tt_pattern pattern) {
        ...
        if (state == TT_FAILED) {
                ...
                dtexec_tttk_message_destroy(msg);
                /*
                 * Nothing substantial to do with the request-reply in the
...
void IdSelfToCallerRequest(void) {
        ...
        msg = dtexec_tttk_message_create(
            (Tt_message)NULL, TT_REQUEST, TT_SESSION, dtSvcProcIdG,
            "_DtActDtexecID", IdSelfToCallerReplyCB);
...

{
        ...
        if (!beenhere) {
                ...
                msg = dtexec_tttk_message_create(
                    (Tt_message)NULL, TT_REQUEST, TT_SESSION, dtSvcProcIdG,
                    "_DtActDtexecDone", DoneRequestReplyCB);
...
int main(int argc, char **argv) {
        ...
        if (dtSvcProcIdG) {
                if (!InitializeTooltalk()) {
                        /*
                         * We have no hope of talking to our caller via
                         * Tooltalk.
                         */
                        dtSvcProcIdG = (char *)NULL;
                }
         ...
         */
        if (errorpipeG[0] != -1)
                BITSET(allactivefdsG,
                       errorpipeG[0]); /* add read side of error pipe */
         ...
         */
        shutdownPhaseG =
            SDP_DONE_STARTING;   /* triggered with rediscoverSigCldG */
        ...
        for (;;) {
...
#if defined(linux)
                /* JET 9/1/98 - linux select will actually modify the timeout
                 * struct - if a select exits early then the timeout struct will
                 * contain the amount remaining.  When this gets to 0,0, an
                 * infinite loop will occur.  So... setup the timeouts each
                 * iteration.
                 */
                ...
                if (rediscoverSigCldG || rediscoverUrgentSigG) {
                        nfound = select(MAXSOCKS, FD_SET_CAST(&readfds),
                                        FD_SET_CAST(NULL),
                ...
                if (nfound == -1) {
                        ...
                        if (errno == EINTR) {
                                /*
                                 * A signal happened - let rediscover flags
                                 * redirect flow via short select timeouts.
                                 */
                                ...
                                if (ttfdG != -1) {
                                        if (GETBIT(exceptfds, ttfdG)) {
                                                /*
                                                 * Tooltalk connection has gone
                                                 * bad.
                                                 *
                                                 * Judgement call - when the
                                                 * Tooltalk connection goes bad,
                                                 * let dtexec continue rather
                                                 * than doing an exit.
                                                 */
                                                DetachFromTooltalk(NULL);
                                        }
                        ...
                        /*
                         * So that select() errors cannot dominate, now behave
                         * as though only a timeout had occured.
                         */
                        nfound = 0;
                }
                ...
                if (nfound > 0) {
                        ...
                        if (ttfdG != -1) {
                                if (GETBIT(readfds, ttfdG)) {
                                        /* Clear bit first, since calling
                                         * input_handler() could */
                                        /* have the side-effect of setting ttfdG
                                         * to -1! */
                                         ...
                                         * Tooltalk activity.
                                         *
                                         * Note that the input_handler
                                         * parameters match an XtInputHandler()
                                         * style callback in case Xt is ever
                                         * used.
                                         */
                                        input_handler((char *)NULL,
                                                      (int *)&junki,
                                                      (unsigned long *)&junki);
                                }
                        }
                        ...
                        if (errorpipeG[0] != -1) {
                                if (GETBIT(readfds, errorpipeG[0])) {
                                         ...
                                         * Stderr activity.
                                         *
                                         * Read the errorpipe until no more
                                         * seems available. Call that good
                                         * enough and write a time-stamped block
                                         * to the errorLog file.
                                         */
                                        errorBytes =
                                            0; /* what we have so far */
                                        tmpBuffer = NULL;
                                        firstPass = 1;
                ...
                if (nfound == 0) {
                        /*
                         * Timeout.  We are probably rediscovering and have
                         * entered a shutdown phase.   The following rediscover
                         * handlers are in priority order.
                         *
                        ...
                        if (rediscoverUrgentSigG) {
                                 ...
                                 *
                                 * Tact: wait awhile and see if a SIGCLD will
                                 * happen. If it does, then a normal shutdown
                                 * will suffice. If a SIGCLD does not happen,
                                 * then do a raw exit(0). Exit is required for
                                 * BBA anyway.
                                 */

                                if (rediscoverSigCldG)
                                        /*
                                         * Rather than act on the Urgent Signal,
                                         * defer to the SIGCLD Signal shutdown
                                         * process.
                                         */
                                        rediscoverUrgentSigG = 0;
                                else
                                        /*
                                         * Still in a mode where we have an
                                         * outstanding Urgent Signal but no
                                         * SIGCLD.  Bump a counter which moves
                                         * us closer to doing an exit().
                                         */
                                        rediscoverUrgentSigG++;

                                /*
                                 * After 5 seconds (add select timeout too)
                                 * waiting for a SIGCLD, give up and exit.
                                 */
                                if (rediscoverUrgentSigG >
                        ...
                        if (rediscoverSigCldG) {
                                 ...
                                 *
                                 * Under SIGCLD, we will make multiple passes
                                 * through the following, implementing a phased
                                 * shutdown.
                                 */
                                if (shutdownPhaseG == SDP_DONE_STARTING) {
                                        ...
                                        if (dtSvcProcIdG) {
                                                ...
                                                /*
                                                 * Unable to send Done Reply.
                                                 * Assume we're on our own from
                                                 * now on.
                                                 */
                                                shutdownPhaseG =
                                ...
                                if (shutdownPhaseG == SDP_DONE_REPLY_WAIT) {
                                        /*
                                         * After 5 minutes of passing through
                                         * REPLY_WAIT, assume the Done(Reply)
                                         * will never come in and move on.
                                         */
                                        rediscoverSigCldG++;
                                        if (rediscoverSigCldG >
                                            ((1000 / SHORT_SELECT_TIMEOUT) *
                                             300)) {
                                        ...
                                        /*
                                         * See if the Tooltalk connection is
                                         * still alive.   If not, then no reason
                                         * to wait around.
                                         */
                                ...
                                if (shutdownPhaseG == SDP_DONE_PANIC_CLEANUP) {
                                        /*
                                         * We cannot talk with caller, so do
                                ...
                                if (shutdownPhaseG == SDP_FINAL_LINGER) {
                                        ...
                                        if (skipFirst) {
                                                /*
                                                 * Rather than a quick departure
                                                 * from the select() loop, make
                                                 * one more pass.  If the child
                                                 * has gone down quickly, the
                                                 * SIGCLD may have caused us to
                                                 * get here before any
                                                 * errorPipeG information has
