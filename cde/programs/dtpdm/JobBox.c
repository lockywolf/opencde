...
static String PdmJobBoxFallbackResources[] = {
    "*JobSetup.locationId: JobSetup", "*JobSetup.marginHeight: 5",
    "*JobSetup.marginWidth: 5",       "*JobSetup.verticalSpacing: 5",
    "*JobTab.compoundString: Job",    "*JobTab.labelString: Job"};
...
static void PdmJobBoxCreate(PdmSetupBox *me, Widget parent) {
        ...
        if (data->job_name_supported) {
                XtVaSetValues(data->job_name_text, XmNvalue, data->job_name,
                              NULL);
        } else {
                XtSetSensitive(data->job_name_text, False);
                XtSetSensitive(XtNameToWidget(me->widget, "*BannerLabel"),
                               False);
        }
        /*
        ...
        if (data->job_options_supported) {
                XtVaSetValues(data->job_options_text, XmNvalue,
                              data->job_options, NULL);
        } else {
                XtSetSensitive(data->job_options_text, False);
                XtSetSensitive(XtNameToWidget(me->widget, "*OptionsLabel"),
                               False);
        }
        /*
        ...
        if (data->notification_profile_supported) {
                Boolean set;
                set = PDMOID_NOTIFY_EMAIL == data->notification_profile ? True
                                                                        : False;
