...
static XtResource PdmResources[] = {
    {
        ...
        XmRString,
        sizeof(String),
        XtOffsetOf(PdmShell, print_display_spec),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRString,
        sizeof(String),
        XtOffsetOf(PdmShell, print_context_str),
        XmRImmediate,
        (XtPointer)NULL,
    }};
 ...
 * fallback resources
 */
static String PdmShellFallbackResources[] = {"Dtpdm.title: Print Setup"};
 ...
 *
 */
static int PdmShellCreate(PdmShell *me, const String *fallback_resources,
        ...
        XtAppSetFallbackResources(me->app_context,
                                  (String *)fallback_resources);

        me->display = XtOpenDisplay(me->app_context, (String)NULL, (String)NULL,
                                    "Dtpdm", PdmOptions, XtNumber(PdmOptions),
        ...
        me->parent_window =
            (Window)strtoul((char *)value.addr, (char **)NULL, 0);
        /*
         * obtain the parent video window's attributes
         */
        if (!XGetWindowAttributes(me->display, me->parent_window,
                                  &me->parent_attr))
                return PDM_EXIT_ERROR;
        /*
        ...
        funcs = MWM_FUNC_ALL | MWM_FUNC_RESIZE | MWM_FUNC_MINIMIZE |
                MWM_FUNC_MAXIMIZE;
        me->widget = XtVaAppCreateShell(
            app_name, app_class, applicationShellWidgetClass, me->display,
            XmNdeleteResponse, XmDESTROY, XmNmappedWhenManaged, False,
            XmNmwmDecorations, decor, XmNmwmFunctions, funcs, XmNmwmInputMode,
            MWM_INPUT_PRIMARY_APPLICATION_MODAL, XmNscreen,
            me->parent_attr.screen, XmNtransient, True, NULL);
        /*
         ...
         * created)
         */
        if (XmeGetColorObjData(XtScreen(me->widget), (int *)NULL, pixelSets,
                               XmCO_NUM_COLORS, (short *)NULL, (short *)NULL,
                               (short *)NULL, &secondary_id, (short *)NULL)) {
                /*
                 * In the Xrm database, reassign primary color resource values
                 * with the corresponding secondary color values. This will
                 * cause subsequently created widgets that utilize colors from
                 * the primary set to actually present secondary colors.
                 */
         ...
         */
        XtGetApplicationResources(me->widget, me, PdmResources,
                                  XtNumber(PdmResources), NULL, 0);
        /*
         * return
         */
        return 0;
...
static void PdmShellPresent(PdmShell *me) {
        ...
        XTranslateCoordinates(me->display, me->parent_window,
                              me->parent_attr.root, 0, 0, &parent_abs_x,
                              &parent_abs_y, &dummy_child);
        /*
...
int main(int argc, char *argv[]) {
         ...
         */
        fallback_resources =
            PdmMainWinMergeFallbacks(pdm_main_window, PdmShellFallbackResources,
         ...
         */
        PdmMainWinCreate(pdm_main_window, pdm_shell->widget,
         ...
         */
        XtAppAddWorkProc(pdm_shell->app_context, PdmStartOkWorkProc,
                         (XtPointer)NULL);
        /*
         * main loop
         */
