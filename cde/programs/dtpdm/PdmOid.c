 ...
 */
static PdmOid PdmOidParse(const char *value_string, const char **ptr_return);
/*
...
MediumDiscreteSizeListParse(const char *, const char **, int i);
static void MediumDiscreteSizeListDelete(PdmOidMediumDiscreteSizeList *list);

static Boolean ParseArea(const char *value_string, const char **ptr_return,
...
TrayMediumListParse(const char *value_string, const char **ptr_return, int i);
...
static Boolean ParseRealValue(const char *value_string, const char **ptr_return,
                              float *real_return);
static Boolean ParseSeqEnd(const char *value_string, const char **ptr_return);
static Boolean ParseSeqStart(const char *value_string, const char **ptr_return);
static Boolean ParseUnspecifiedValue(const char *value_string,
                                     const char **ptr_return);
static int SpanToken(const char *string);
 ...
 *
 */
const char *PdmOidString(PdmOid pdm_oid) {
        /*
 ...
 *
 */
int PdmOidMsgSet(PdmOid pdm_oid) {
        /*
         * PdmOid enum values are index values into the string map
         */
 ...
 *
 */
int PdmOidMsgNum(PdmOid pdm_oid) {
        /*
         * PdmOid enum values are index values into the string map
         */
 ...
 *
 */
const char *PdmOidDefaultMsg(PdmOid pdm_oid) {
 ...
 *
 */
int PdmOidStringLength(PdmOid pdm_oid) {
        /*
         * PdmOid enum values are index values into the string map
         */
 ...
 *
 */
PdmOid PdmOidFromString(const char *value) {
...
static PdmOid PdmOidParse(const char *value_string, const char **ptr_return) {
        ...
        for (i = 0; i < PdmOidStringMapCount; i++)
                if (length == PdmOidStringMap[i].length)
                        if (strncmp(ptr, PdmOidStringMap[i].string, length) ==
                            0)
                                break;
 ...
 *
 */
PdmOidList *PdmOidListNew(const char *value_string) {
 ...
 *
 */
Boolean PdmOidListHasOid(const PdmOidList *list, PdmOid oid) {
        int i;
 ...
 *
 */
int PdmOidListGetIndex(const PdmOidList *list, PdmOid oid) {
        int i;
        if (list != (PdmOidList *)NULL)
                for (i = 0; i < list->count; i++)
                        if (list->list[i] == oid)
 ...
 *
 */
void PdmOidListDelete(PdmOidList *list) {
        if (list != (PdmOidList *)NULL) {
 ...
 *
 */
PdmOidLinkedList *PdmOidLinkedListNew() {
 ...
 *
 */
void PdmOidLinkedListDelete(PdmOidLinkedList *me) {
 ...
 *
 */
PdmOid PdmOidLinkedListGetOid(PdmOidLinkedList *me, int i) {
        if (i < 0 || i >= me->count)
                return pdmoid_none;
        me->current = me->head;
        while (i--)
 ...
 *
 */
void PdmOidLinkedListAddOid(PdmOidLinkedList *me, PdmOid oid) {
 ...
 *
 */
int PdmOidLinkedListGetIndex(PdmOidLinkedList *me, PdmOid oid) {
        int i = 0;
        me->current = me->head;
        while (me->current)
 ...
 *
 */
Boolean PdmOidLinkedListHasOid(PdmOidLinkedList *me, PdmOid oid) {
        me->current = me->head;
        while (me->current)
                if (me->current->oid == oid)
                        return True;
 ...
 *
 */
PdmOid PdmOidLinkedListFirstOid(PdmOidLinkedList *me) {
 ...
 *
 */
PdmOid PdmOidLinkedListNextOid(PdmOidLinkedList *me) {
 ...
 *
 */
const char *PdmOidLocalString(PdmOid pdm_oid) {
        /*
         * PdmOid enum values are index values into the string map
         */
 ...
 *
 */
PdmOidMediumSS *PdmOidMediumSSNew(const char *value_string) {
        if (value_string == (const char *)NULL)
...
static PdmOidMediumSS *MediumSSParse(const char *value_string,
        ...
        if (ParseSeqStart(value_string, ptr_return)) {
                ...
                if (mss.ms.continuous_size !=
                    (PdmOidMediumContinuousSize *)NULL) {
                        mss.mstag = PdmOidMediumSS_CONTINUOUS;
                } else {
                        /*
                         * not continuous, try Discrete MediumSize
                         */
                        mss.ms.discrete = MediumDiscreteSizeListParse(
                            *ptr_return, ptr_return, 0);
                        if (mss.ms.discrete ==
                            (PdmOidMediumDiscreteSizeList *)NULL) {
                                /*
                                 * syntax error (MediumDiscreteSizeListParse
                                 * reports error)
                                 */
                                return NULL;
                        }
                 ...
                 */
                memmove((medium_ss->mss) + i, &mss,
                        sizeof(PdmOidMediumSourceSize));
        } else {
                /*
                ...
                if (**ptr_return == '\0') {
                         ...
                         */
                        medium_ss = (PdmOidMediumSS *)XtCalloc(
                            1, sizeof(PdmOidMediumSS));
                        medium_ss->count = i;
                        medium_ss->mss = (PdmOidMediumSourceSize *)XtCalloc(
                            i, sizeof(PdmOidMediumSourceSize));
                } else {
                        /*
                         * syntax error
                         */
 ...
 *
 */
void PdmOidMediumSSDelete(PdmOidMediumSS *me) {
        if (me != (PdmOidMediumSS *)NULL) {
                ...
                for (i = 0; i < me->count; i++) {
                        ...
                        case PdmOidMediumSS_CONTINUOUS:
                                MediumContinuousSizeDelete(
                                    (me->mss)[i].ms.continuous_size);
                                break;
                        case PdmOidMediumSS_DISCRETE:
                                MediumDiscreteSizeListDelete(
                                    (me->mss)[i].ms.discrete);
                                break;
                        }
                }
                XtFree((char *)me);
 ...
 *
 */
PdmOidLinkedList *PdmOidMediumSSGetAllSizes(PdmOidMediumSS *me) {
                ...
                case PdmOidMediumSS_DISCRETE:
                        ...
                        for (i_ds = 0; i_ds < ds_list->count; i_ds++) {
                                ...
                                if (page_size == pdmoid_none)
                                        continue;
                                if (!PdmOidLinkedListHasOid(all_sizes,
                                                            page_size)) {
                                        PdmOidLinkedListAddOid(all_sizes,
                                                               page_size);
                                }
                        }
                        break;
 ...
 *
 */
void PdmOidMediumSSGetTraysSizes(PdmOidMediumSS *me,
                                 PdmOidTrayMediumList *trays_medium,
                                 PdmOidList **trays, PdmOidList **sizes) {
        ...
        if (me == (PdmOidMediumSS *)NULL ||
            trays_medium == (PdmOidTrayMediumList *)NULL) {
                if (trays != (PdmOidList **)NULL)
                        *trays = (PdmOidList *)NULL;
                if (sizes != (PdmOidList **)NULL)
                        *sizes = (PdmOidList *)NULL;
                return;
        }
        /*
        ...
        for (i_itm = 0; i_itm < tray_count; i_itm++) {
                ...
                if (current_tray == pdmoid_none)
                        continue;
                current_medium =
                    PdmOidTrayMediumListMedium(trays_medium, i_itm);
                if (current_medium == pdmoid_none)
                        continue;
                /*
                 * loop through the MediumSourceSizes, looking for an
                 * appropriate discrete sizes spec for the current tray
                 */
                unspecified_tray_ds = (PdmOidMediumDiscreteSizeList *)NULL;
                tray_ds = (PdmOidMediumDiscreteSizeList *)NULL;
                for (i_mss = 0; i_mss < me->count &&
                                tray_ds == (PdmOidMediumDiscreteSizeList *)NULL;
                     i_mss++) {
                        switch ((me->mss)[i_mss].mstag) {
                        case PdmOidMediumSS_DISCRETE:
                                ...
                                else if ((me->mss)[i_mss].input_tray ==
                                         pdmoid_unspecified)
                                        unspecified_tray_ds =
                                            (me->mss)[i_mss].ms.discrete;
                                break;
                ...
                /*
                 * if the tray was not matched, use the unspecifed tray size
                 * list
                 */
                if (tray_ds == (PdmOidMediumDiscreteSizeList *)NULL)
                        if (unspecified_tray_ds ==
                            (PdmOidMediumDiscreteSizeList *)NULL) {
                                /*
                ...
                for (i_ds = 0; i_ds < tray_ds->count; i_ds++) {
                         ...
                         *
                         * Note: in the CDEnext SI, medium identifiers coincide
                         * with medium-size identifiers. If the DP-Medium object
                         * is ever implemented, this check would need to
                         * be changed so that the input tray's medium size
                         * is obtained from the indicated Medium object,
                         * and not inferred from the medium identifier
                         * itself.
                         */
                        if ((tray_ds->list)[i_ds].page_size == current_medium) {
                                /*
                                 * the current input tray's medium size matches
                                 * the current discrete size; add the tray and
                                 * medium size Oids to the return lists.
                                 */
                                if (trays != (PdmOidList **)NULL)
                                        (*trays)->list[i_its] = current_tray;
                                if (sizes != (PdmOidList **)NULL)
                                        (*sizes)->list[i_its] =
                                            (tray_ds->list)[i_ds].page_size;
                                ++i_its;
                                break;
                        }
                }
...
MediumContinuousSizeParse(const char *value_string, const char **ptr_return) {
         ...
         * Ok, let's go for it
         */
        mcs = (PdmOidMediumContinuousSize *)XtCalloc(
            1, sizeof(PdmOidMediumContinuousSize));
        /*
         * "range across the feed direction"
         */
        if (!ParseUnsignedRange(*ptr_return, ptr_return,
                                &mcs->range_across_feed))
                goto MediumContinuousSizeParse_error;
        /*
        ...
        else if (!ParseBooleanValue(*ptr_return, ptr_return,
                                    &mcs->long_edge_feeds))
                goto MediumContinuousSizeParse_error;
        /*
 ...
 */
static PdmOidMediumDiscreteSizeList *
MediumDiscreteSizeListParse(const char *value_string, const char **ptr_return,
                            int i) {
        ...
        if (ParseSeqStart(value_string, ptr_return)) {
                 ...
                 */
                list =
                    MediumDiscreteSizeListParse(*ptr_return, ptr_return, i + 1);
                if (list != (PdmOidMediumDiscreteSizeList *)NULL) {
                        /*
                         * copy the current discrete MediumSize into the list
                         */
                        memmove((list->list) + i, &mds,
                                sizeof(PdmOidMediumDiscreteSize));
                }
        } else {
                /*
                 * MediumSize sequence start not found; end of the discrete
                 * sizes list
                 */
                list = (PdmOidMediumDiscreteSizeList *)XtCalloc(
                    1, sizeof(PdmOidMediumDiscreteSizeList));
                list->count = i;
                list->list = (PdmOidMediumDiscreteSize *)XtCalloc(
                    i, sizeof(PdmOidMediumDiscreteSize));
        }
        return list;
}
 ...
 *
 */
PdmOidTrayMediumList *PdmOidTrayMediumListNew(const char *value_string) {
        if (value_string == (const char *)NULL)
...
TrayMediumListParse(const char *value_string, const char **ptr_return, int i) {
        ...
        if (ParseSeqStart(value_string, ptr_return)) {
                ...
                if (list != (PdmOidTrayMediumList *)NULL) {
                         ...
                         */
                        memmove((list->list) + i, &tm,
                                sizeof(PdmOidTrayMedium));
                }
        } else {
                /*
                ...
                if (**ptr_return == '\0') {
                         ...
                         * end of the list
                         */
                        list = (PdmOidTrayMediumList *)XtCalloc(
                            1, sizeof(PdmOidTrayMediumList));
                        list->count = i;
                        if (i == 0)
                                list->list = NULL;
                        else
                                list->list = (PdmOidTrayMedium *)XtCalloc(
                                    i, sizeof(PdmOidTrayMedium));
                } else {
                        /*
                         * syntax error
                         */
 ...
 *
 */
void PdmOidTrayMediumListDelete(PdmOidTrayMediumList *list) {
        if (list != (PdmOidTrayMediumList *)NULL) {
                XtFree((char *)list->list);
                XtFree((char *)list);
        }
...
static Boolean ParseUnsignedRange(const char *value_string,
        ...
        if (!ParseUnsignedValue(
                ptr, &ptr, range_return ? &range_return->lower_bound : NULL))
                goto ParseUnsignedRange_error;
        /*
         * parse the upper bound
         */
        if (!ParseUnsignedValue(
                ptr, &ptr, range_return ? &range_return->upper_bound : NULL))
                goto ParseUnsignedRange_error;
        /*
...
PdmOidNotify PdmOidNotifyParse(const char *value_string) {
         ...
         */
        if (pdmoid_val_delivery_method_electronic_mail !=
            PdmOidParse(ptr, &ptr))
                return PDMOID_NOTIFY_UNSUPPORTED;
        /*
 ...
 *
 */
char *PdmOidDocumentFormatParse(const char *value_string) {
         ...
         */
        document_format = PdmOidDocumentFormatNext(value_string, &ptr);
 ...
 *
 */
char *PdmOidDocumentFormatDefault(const char *value_string) {
        /*
 ...
 *
 */
static Boolean ParseSeqStart(const char *value_string,
                             const char **ptr_return) {
        const char *ptr;
        Boolean status;
        /*
