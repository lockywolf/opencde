...
static String PdmPrinterBoxFallbackResources[] = {
    ...
    "*PrinterSetup*OrientCtlTop.PortraitIconFileName: " ICON_INSTALL_DIR
    "portrait.pm",
    "*PrinterSetup*OrientCtlTop.LandscapeIconFileName: " ICON_INSTALL_DIR
    "landscap.pm",
    "*PrinterSetup*OrientCtlTop.ReversePortraitIconFileName: " ICON_INSTALL_DIR
    "rportrai.pm",
    "*PrinterSetup*OrientCtlTop.ReverseLandscapeIconFileName: " ICON_INSTALL_DIR
    "rlandsca.pm",
    "*PrinterSetup*PlexCtlTop.SimplexPortraitIconFileName: " ICON_INSTALL_DIR
    "psimplex.pm",
    "*PrinterSetup*PlexCtlTop.SimplexLandscapeIconFileName: " ICON_INSTALL_DIR
    "lsimplex.pm",
    "*PrinterSetup*PlexCtlTop.DuplexPortraitIconFileName: " ICON_INSTALL_DIR
    "pduplex.pm",
    "*PrinterSetup*PlexCtlTop.DuplexLandscapeIconFileName: " ICON_INSTALL_DIR
    "lduplex.pm",
    "*PrinterSetup*PlexCtlTop.TumblePortraitIconFileName: " ICON_INSTALL_DIR
    "ptumble.pm",
    "*PrinterSetup*PlexCtlTop.TumbleLandscapeIconFileName: " ICON_INSTALL_DIR
    "ltumble.pm",
...
static void PdmPrinterBoxResizeCB(Widget w, XtPointer client_data,
                                  XtPointer call_data);
/*
...
static void OrientCtlCreate(OrientCtl *me, Widget parent, PdmOid orientation,
...
static void OrientCtlResizeCB(Widget w, XtPointer client_data,
                              XtPointer call_data);
...
static void PlexCtlCreate(PlexCtl *me, Widget parent, PdmOid plex,
                          const PdmOidList *document_attrs_supported,
                          const PdmOidList *plexes_supported);
...
static void PlexCtlResizeCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
...
static void TrayCtlCreate(TrayCtl *me, Widget parent, PdmOid default_input_tray,
                          PdmOid default_medium,
                          const PdmOidList *document_attrs_supported,
                          const PdmOidList *input_trays,
                          const PdmOidList *tray_sizes);
...
static void TrayCtlResizeCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
...
static void SizeCtlSetSizeLists(PdmOidList **oids, XmStringTable *xmstrs,
                                PdmOidLinkedList *oids_ll);
static void SizeCtlCreate(SizeCtl *me, Widget parent, PdmOid default_medium,
                          const PdmOidList *document_attrs_supported,
                          const PdmOidList *tray_sizes,
                          PdmOidLinkedList *sizes_supported);
...
static void SizeCtlResizeCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void SizeCtlSetSingleSizeMode(SizeCtl *me, PdmOid size);
static void SizeCtlSetMultiSizeMode(SizeCtl *me);
static void SizeCtlReadyChangedCB(Widget w, XtPointer client_data,
                                  XtPointer call_data);
static void SizeCtlSupportedChangedCB(Widget w, XtPointer client_data,
                                      XtPointer call_data);
static void SizeCtlSelectCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
/*
...
static void PdmPixmapListSetImageName(PdmPixmapList *me, const char *name,
                                      int i);

/*
 * option menu list creation function
 */
static Widget CreateOptionMenuPulldown(String pulldown_name, Widget option_menu,
                                       XtCallbackProc activate_proc,
                                       XtPointer user_data, XmString *item_list,
                                       int item_count, int initial_item);
/*
 ...
 */
static void IconMaxDimension(Widget icon_label, PdmPixmapList *pixmap_list,
                             int i, Dimension *max_width,
...
static void PdmPrinterBoxDelete(PdmSetupBox *me) {
        if (me != (PdmSetupBox *)NULL) {
                PdmPrinterBoxData *data =
                    (PdmPrinterBoxData *)me->subclass_data;
...
static Widget PdmPrinterBoxCreateWindow(PdmSetupBox *me, Widget parent) {
         ...
         */
        manager = XtVaCreateManagedWidget("PrinterSetup",
                                          xmRowColumnWidgetClass, parent, NULL);
        /*
        ...
        if (data->printer_model != (const char *)NULL) {
                ...
                XtFree(desc);
                w = XtVaCreateManagedWidget("PrinterModel", xmLabelGadgetClass,
        ...
        if ((char *)NULL != data->document_format) {
                 ...
                 */
                row = XtVaCreateManagedWidget(
                    "DocumentFormatRow", xmRowColumnWidgetClass, manager,
                    XmNorientation, XmHORIZONTAL, NULL);
                /*
        ...
        draw_area = XtVaCreateWidget("PrinterSetupControls",
                                     xmDrawingAreaWidgetClass, manager, NULL);
        XtAddCallback(draw_area, XmNresizeCallback, PdmPrinterBoxResizeCB,
                      (XtPointer)me);
        /*
         * orientation control
         */
        OrientCtlCreate(data->orient_ctl, draw_area, data->orientation,
                        data->document_attrs_supported,
                        data->orientations_supported);
        /*
         * page size control
         */
        SizeCtlCreate(data->size_ctl, draw_area, data->default_medium,
                      data->document_attrs_supported, data->tray_sizes,
                      data->sizes_supported);
        /*
         * plex control
         */
        PlexCtlCreate(data->plex_ctl, draw_area, data->plex,
                      data->document_attrs_supported, data->plexes_supported);
        /*
         * input tray control
         */
        TrayCtlCreate(data->tray_ctl, draw_area, data->default_input_tray,
                      data->default_medium, data->document_attrs_supported,
                      data->input_trays, data->tray_sizes);
...
static void PdmPrinterBoxResizeCB(Widget w, XtPointer client_data,
         ...
         */
        XtVaGetValues(TrayCtlGetWidget(data->tray_ctl), XmNx, &rt.x, XmNy,
                      &rt.y, XmNwidth, &rt.width, XmNheight, &rt.height, NULL);
        XtVaGetValues(SizeCtlGetWidget(data->size_ctl), XmNx, &rs.x, XmNy,
                      &rs.y, XmNwidth, &rs.width, XmNheight, &rs.height, NULL);
        XtVaGetValues(OrientCtlGetWidget(data->orient_ctl), XmNx, &ro.x, XmNy,
                      &ro.y, XmNwidth, &ro.width, XmNheight, &ro.height, NULL);
        XtVaGetValues(PlexCtlGetWidget(data->plex_ctl), XmNx, &rp.x, XmNy,
                      &rp.y, XmNwidth, &rp.width, XmNheight, &rp.height, NULL);
        /*
         ...
         */
        XtVaSetValues(OrientCtlGetWidget(data->orient_ctl), XmNx, c1.x, XmNy,
                      r1.y, XmNwidth, c1.width, XmNheight, r1.height, NULL);
        XtVaSetValues(PlexCtlGetWidget(data->plex_ctl), XmNx, c2.x, XmNy, r1.y,
                      XmNwidth, c2.width, XmNheight, r1.height, NULL);
        XtVaSetValues(SizeCtlGetWidget(data->size_ctl), XmNx, c1.x, XmNy, r2.y,
                      XmNwidth, c1.width, XmNheight, r2.height, NULL);
        XtVaSetValues(TrayCtlGetWidget(data->tray_ctl), XmNx, c2.x, XmNy, r2.y,
                      XmNwidth, c2.width, XmNheight, r2.height, NULL);
        /*
         * resize the drawing area to accomodate the new layout
         */
        XtVaSetValues(w, XmNwidth, r1.width, XmNheight, c1.height, NULL);
}
...
static void OrientCtlCreate(OrientCtl *me, Widget parent, PdmOid orientation,
        ...
        if (PdmOidListHasOid(document_attrs_supported,
            ...
            PdmOidListCount(orientations_supported) > 0) {
                 ...
                 */
                me->dummy_orientations_supported = PdmOidListNew(
                    PdmOidString(pdmoid_val_content_orientation_portrait));
         ...
         */
        OrientCtlSetSelectedItem(
            me, PdmOidListGetIndex(me->orientations_supported, orientation));
        /*
...
static void OrientCtlCreateWindow(OrientCtl *me, Widget parent) {
         ...
         */
        me->widget =
            XtVaCreateManagedWidget("OrientCtlTop", xmFrameWidgetClass, parent,
                                    XmNshadowType, XmSHADOW_ETCHED_IN, NULL);
        /*
         * orientation label
         */
        me->frame_label = XtVaCreateManagedWidget(
            "OrientationLabel", xmLabelWidgetClass, me->widget,
            XmNframeChildType, XmFRAME_TITLE_CHILD, NULL);
        /*
         * orientation controls manager
         */
        control_manager = XtVaCreateManagedWidget(
            "OrientationControls", xmDrawingAreaWidgetClass, me->widget,
            XmNframeChildType, XmFRAME_WORKAREA_CHILD, NULL);
        XtAddCallback(control_manager, XmNresizeCallback, OrientCtlResizeCB,
                      me);
        /*
         * orientations option menu
         */
        me->option_menu = XtVaCreateManagedWidget(
            "OrientationOptionMenu", xmRowColumnWidgetClass, control_manager,
            XmNmarginHeight, 0, XmNrowColumnType, XmMENU_OPTION, NULL);
        /*
         * orientation icon
         */
        me->icon_label = XtVaCreateManagedWidget(
            "OrientationIcon", xmLabelWidgetClass, control_manager,
            XmNlabelType, XmPIXMAP, XmNalignment, XmALIGNMENT_CENTER, NULL);
}
...
static void OrientCtlResizeCB(Widget w, XtPointer client_data,
         ...
         */
        XtVaGetValues(w, XmNheight, &height, XmNwidth, &width, XmNmarginWidth,
                      &margin_width, NULL);
...
static void OrientCtlCreatePulldown(OrientCtl *me) {
        ...
        CreateOptionMenuPulldown("OrientationOptions", me->option_menu,
                                 OrientCtlSelectCB, (XtPointer)me, list, count,
                                 me->selected_item);
        /*
         * clean up
         */
 ...
 *
 */
static void OrientCtlSelectCB(Widget w, XtPointer client_data,
                              XtPointer call_data) {
        OrientCtl *me;
...
static void OrientCtlInitPixmapList(OrientCtl *me) {
        ...
        for (i = 0; i < count; i++) {
                ...
                case pdmoid_val_content_orientation_portrait:
                        image_name =
                            GetIconFileName(me->widget, "PortraitIconFileName");
                        break;
                case pdmoid_val_content_orientation_landscape:
                        image_name = GetIconFileName(me->widget,
                                                     "LandscapeIconFileName");
                        break;
                case pdmoid_val_content_orientation_reverse_portrait:
                        image_name = GetIconFileName(
                            me->widget, "ReversePortraitIconFileName");
                        break;
                case pdmoid_val_content_orientation_reverse_landscape:
                        image_name = GetIconFileName(
                            me->widget, "ReverseLandscapeIconFileName");
                        break;
                default:
                        continue;
                }
                ...
                IconMaxDimension(me->icon_label, me->pixmap_list, i, &max_width,
                                 &max_height);
        }
        XtVaSetValues(me->icon_label, XmNwidth, max_width, XmNheight,
                      max_height, XmNrecomputeSize, False, NULL);
}
...
static void OrientCtlSetSelectedItem(OrientCtl *me, int selected_item) {
        ...
        if (me->plex_ctl != (PlexCtl *)NULL)
                PlexCtlSetOrientation(me->plex_ctl,
                                      OrientCtlGetOrientation(me));
}
...
static void OrientCtlUpdateIcon(OrientCtl *me) {
        if (me->icon_label != (Widget)NULL)
                XtVaSetValues(
                    me->icon_label, XmNlabelPixmap,
                    PdmPixmapListGetPixmap(me->pixmap_list, me->selected_item),
...
static PdmOid OrientCtlGetOrientation(OrientCtl *me) {
        if (me->orientations_supported == me->dummy_orientations_supported)
                ...
                return PdmOidListGetOid(me->orientations_supported,
                                        me->selected_item);
}
...
static void PlexCtlCreate(PlexCtl *me, Widget parent, PdmOid plex,
         ...
         * set the initially selected list item
         */
        PlexCtlSetSelectedItem(me,
                               PdmOidListGetIndex(me->plexes_supported, plex));
        /*
...
static void PlexCtlCreateWindow(PlexCtl *me, Widget parent) {
         ...
         */
        me->widget =
            XtVaCreateManagedWidget("PlexCtlTop", xmFrameWidgetClass, parent,
                                    XmNshadowType, XmSHADOW_ETCHED_IN, NULL);
        /*
         * plex label
         */
        me->frame_label = XtVaCreateManagedWidget(
            "PlexLabel", xmLabelGadgetClass, me->widget, XmNframeChildType,
            XmFRAME_TITLE_CHILD, NULL);
        /*
         * plex controls manager
         */
        control_manager = XtVaCreateManagedWidget(
            "PlexControls", xmDrawingAreaWidgetClass, me->widget,
            XmNframeChildType, XmFRAME_WORKAREA_CHILD, NULL);
         ...
         */
        me->option_menu = XtVaCreateManagedWidget(
            "PlexOptionMenu", xmRowColumnWidgetClass, control_manager,
            XmNrowColumnType, XmMENU_OPTION, XmNmarginHeight, 0, NULL);
        /*
         * plex icon
         */
        me->icon_label = XtVaCreateManagedWidget(
            "PlexIcon", xmLabelWidgetClass, control_manager, XmNlabelType,
            XmPIXMAP, XmNalignment, XmALIGNMENT_CENTER, NULL);
}
...
static void PlexCtlCreatePulldown(PlexCtl *me) {
        ...
        CreateOptionMenuPulldown("PlexOptions", me->option_menu,
                                 PlexCtlSelectCB, (XtPointer)me, list, count,
                                 me->selected_item);
        /*
         * clean up
         */
 ...
 *
 */
static void PlexCtlSelectCB(Widget w, XtPointer client_data,
                            XtPointer call_data) {
        PlexCtl *me;
        XtArgVal /* int */ selected_item = (XtArgVal)client_data;
...
static void PlexCtlInitPixmapLists(PlexCtl *me) {
         ...
         */
        count = PdmOidListCount(me->plexes_supported);
        me->portrait_pixmap_list =
            PdmPixmapListNew(count, me->icon_label, False);
        me->i_portrait_pixmap_list =
            PdmPixmapListNew(count, me->icon_label, True);
        me->landscape_pixmap_list =
            PdmPixmapListNew(count, me->icon_label, False);
        me->i_landscape_pixmap_list =
            PdmPixmapListNew(count, me->icon_label, True);
        for (i = 0; i < count; i++) {
                switch (PdmOidListGetOid(me->plexes_supported, i)) {
                case pdmoid_val_plex_simplex:
                        portrait_image_name = GetIconFileName(
                            me->widget, "SimplexPortraitIconFileName");
                        landscape_image_name = GetIconFileName(
                            me->widget, "SimplexLandscapeIconFileName");
                        break;
                case pdmoid_val_plex_duplex:
                        portrait_image_name = GetIconFileName(
                            me->widget, "DuplexPortraitIconFileName");
                        landscape_image_name = GetIconFileName(
                            me->widget, "DuplexLandscapeIconFileName");
                        break;
                case pdmoid_val_plex_tumble:
                        portrait_image_name = GetIconFileName(
                            me->widget, "TumblePortraitIconFileName");
                        landscape_image_name = GetIconFileName(
                            me->widget, "TumbleLandscapeIconFileName");
                        break;
                default:
                        continue;
                }
                ...
                PdmPixmapListSetImageName(me->i_landscape_pixmap_list,
                                          landscape_image_name, i);
                IconMaxDimension(me->icon_label, me->portrait_pixmap_list, i,
                                 &max_width, &max_height);
                IconMaxDimension(me->icon_label, me->landscape_pixmap_list, i,
                                 &max_width, &max_height);
        }
        XtVaSetValues(me->icon_label, XmNwidth, max_width, XmNheight,
                      max_height, XmNrecomputeSize, False, NULL);
...
static void PlexCtlUpdateIcon(PlexCtl *me) {
        if (me->icon_label != (Widget)NULL)
                XtVaSetValues(me->icon_label, XmNlabelPixmap,
...
static PdmOid PlexCtlGetPlex(PlexCtl *me) {
        if (me->plexes_supported == me->dummy_plexes_supported)
                ...
                return PdmOidListGetOid(me->plexes_supported,
                                        me->selected_item);
}
 ...
 *
 */
static void TrayCtlDelete(TrayCtl *me) { XtFree((char *)me); }
...
static void TrayCtlCreate(TrayCtl *me, Widget parent, PdmOid default_input_tray,
        ...
        if (default_medium != pdmoid_none) {
                ...
                if (!supported || i == -1)
                        /*
                         * default-input-tray unsupported or not found in list;
                         * set "Auto-select" (index 0)
                         */
                        TrayCtlSetSelectedItem(me, 0);
                else
                        /*
                         * Select the list item corresponding to
                         * default-input-tray - ie. add 1 to the input trays
                         * list index since the option menu is always built with
                         * "Auto-select" as the first entry, followed by the
                         * input trays list entries.
                         */
...
static void TrayCtlCreateWindow(TrayCtl *me, Widget parent) {
         ...
         */
        me->widget =
            XtVaCreateManagedWidget("TrayCtlTop", xmFrameWidgetClass, parent,
                                    XmNshadowType, XmSHADOW_ETCHED_IN, NULL);
        /*
         * tray label
         */
        me->frame_label = XtVaCreateManagedWidget(
            "TrayLabel", xmLabelGadgetClass, me->widget, XmNframeChildType,
            XmFRAME_TITLE_CHILD, NULL);
        /*
         * tray controls manager
         */
        control_manager = XtVaCreateManagedWidget(
            "TrayControls", xmDrawingAreaWidgetClass, me->widget,
            XmNframeChildType, XmFRAME_WORKAREA_CHILD, NULL);
         ...
         */
        me->option_menu = XtVaCreateManagedWidget(
            "TrayOptionMenu", xmRowColumnWidgetClass, control_manager,
            XmNmarginHeight, 0, XmNrowColumnType, XmMENU_OPTION, NULL);
}
...
static void TrayCtlCreatePulldown(TrayCtl *me) {
        ...
        i = 0;
        xmstr_list[i++] =
            XmStringCreateLocalized((char *)PDM_MSG_TRAY_AUTO_SELECT);
        /*
         * add the input trays
         */
        for (; i < count; i++) {
                oid = PdmOidListGetOid(me->input_trays, i - 1);
                str = DTPDM_GETMESSAGE(PdmOidMsgSet(oid), PdmOidMsgNum(oid),
         ...
         * create the option menu pulldown
         */
        CreateOptionMenuPulldown("TrayOptions", me->option_menu,
                                 TrayCtlSelectCB, (XtPointer)me, xmstr_list,
                                 count, me->selected_item);
        /*
         * clean up
         */
...
static void TrayCtlSetSelectedItem(TrayCtl *me, int selected_item) {
        ...
        if (me->selected_item == 0)
                ...
                SizeCtlSetSingleSizeMode(
                    me->size_ctl,
                    PdmOidListGetOid(me->tray_sizes, me->selected_item - 1));
}
 ...
 *
 */
static void TrayCtlSelectCB(Widget w, XtPointer client_data,
                            XtPointer call_data) {
        TrayCtl *me;
        XtArgVal /* int */ selected_item = (XtArgVal)client_data;
 ...
 *
 */
static void SizeCtlSetSizeLists(PdmOidList **oid_list, XmStringTable *xmstrs,
        ...
        if (count == 0) {
                ...
                *xmstrs = (XmStringTable)NULL;
                 ...
                 */
                sort_list =
                    (SizeCtlSortList)XtCalloc(count, sizeof(SizeCtlSortRec));
                for (i = 0, oid = PdmOidLinkedListFirstOid(oid_ll); i < count;
                     i++, oid = PdmOidLinkedListNextOid(oid_ll)) {
                        sort_list[i].oid = oid;
                        sort_list[i].str = DTPDM_GETMESSAGE(
                            PdmOidMsgSet(oid), PdmOidMsgNum(oid),
                            PdmOidDefaultMsg(oid));
                }
                ...
                *xmstrs = (XmStringTable)XtCalloc(count, sizeof(XmString));
                for (i = 0; i < count; i++) {
                        ...
                        (*xmstrs)[i] =
                            XmStringCreateLocalized((char *)sort_list[i].str);
                }
...
static void SizeCtlCreateWindow(SizeCtl *me, Widget parent) {
        ...
        me->widget = XtVaCreateManagedWidget(
            "PageSizeCtlTop", xmFrameWidgetClass, parent, XmNshadowType,
            XmSHADOW_ETCHED_IN, NULL);
        /*
         * page size label
         */
        me->frame_label = XtVaCreateManagedWidget(
            "PageSizeLabel", xmLabelGadgetClass, me->widget, XmNframeChildType,
            XmFRAME_TITLE_CHILD, NULL);
         ...
         */
        control_manager = XtVaCreateManagedWidget(
            "PageSizeControls", xmDrawingAreaWidgetClass, me->widget,
            XmNframeChildType, XmFRAME_WORKAREA_CHILD, NULL);
         ...
         */
        n = 0;
        XtSetArg(args[n], XmNvisibleItemCount, 3);
        n++;
        XtSetArg(args[n], XmNscrollBarDisplayPolicy, XmSTATIC);
        n++;
        XtSetArg(args[n], XmNhorizontalScrollBar, NULL);
        n++;
        /*
        ...
        if (me->selected_list == SIZECTL_UNSUPPORTED) {
                ...
                XtSetArg(args[n], XmNitemCount, 1);
                n++;
                XtSetArg(args[n], XmNitems, &default_xmstr);
                n++;
        } else {
                XtSetArg(args[n], XmNitems, me->sizes_supported_xmstr);
                n++;
        ...
        me->list_box =
            XmCreateScrolledList(control_manager, "PageSizes", args, n);
        if ((XmString)NULL != default_xmstr)
                XmStringFree(default_xmstr);
        XtAddCallback(me->list_box, XmNbrowseSelectionCallback, SizeCtlSelectCB,
                      (XtPointer)me);
        /*
        ...
        XtVaSetValues(hscroll, XmNmappedWhenManaged, False, XmNx, 0, XmNy, 0,
         ...
         */
        me->radio_box =
            XtVaCreateWidget("LoadedAllowed", xmRowColumnWidgetClass,
                             control_manager, XmNradioBehavior, True, NULL);
        me->ready_radio = XtVaCreateManagedWidget(
            "button_0", xmToggleButtonWidgetClass, me->radio_box, NULL);
        XtAddCallback(me->ready_radio, XmNvalueChangedCallback,
                      SizeCtlReadyChangedCB, (XtPointer)me);
        me->supported_radio = XtVaCreateManagedWidget(
            "button_1", xmToggleButtonWidgetClass, me->radio_box, NULL);
...
static void SizeCtlUpdate(SizeCtl *me) {
        ...
        if (me->single_size_mode) {
                ...
                item = XmStringCreateLocalized((char *)DTPDM_GETMESSAGE(
                    PdmOidMsgSet(oid), PdmOidMsgNum(oid),
                    PdmOidDefaultMsg(oid)));
                ...
                if (me->selected_list != SIZECTL_UNSUPPORTED) {
                        ...
                        XmToggleButtonSetState(me->supported_radio, False,
                                               False);
                }
        } else {
                if (me->selected_list == SIZECTL_UNSUPPORTED) {
                        ...
                        default_item = XmStringCreateLocalized(
                            (char *)PDM_MSG_SIZE_DEFAULT);
                         ...
                         */
                        if (!PdmOidListHasOid(me->sizes_supported,
                                              me->selected_size)) {
                                /*
                                ...
                                if (PdmOidListCount(me->sizes_ready) > 0) {
                                         ...
                                         */
                                        me->selected_size = PdmOidListGetOid(
                                            me->sizes_ready, 0);
                                        me->selected_list = SIZECTL_SIZES_READY;
                                } else {
                                        /*
                                         * the ready list is empty, use the 1st
                                         * entry in the supported list
                                         */
                                        me->selected_size = PdmOidListGetOid(
                                            me->sizes_supported, 0);
                                        me->selected_list =
                                            SIZECTL_SIZES_SUPPORTED;
                                }
                        } else if (me->selected_list == SIZECTL_SIZES_READY &&
                                   !PdmOidListHasOid(me->sizes_ready,
                                                     me->selected_size)) {
                                /*
                                 * the selected size is not in the ready list;
                                 * use the supported list instead
                                 */
                        ...
                        if (me->selected_list == SIZECTL_SIZES_READY) {
                                XmToggleButtonSetState(me->ready_radio, True,
                                                       False);
                                XmToggleButtonSetState(me->supported_radio,
                                                       False, False);
                                ...
                                XmToggleButtonSetState(me->supported_radio,
                                                       True, False);
                                XmToggleButtonSetState(me->ready_radio, False,
                                                       False);

                                list = me->sizes_supported_xmstr;
                                count = PdmOidListCount(me->sizes_supported);
                                pos =
                                    1 + PdmOidListGetIndex(me->sizes_supported,
                                                           me->selected_size);
                        }
...
static void SizeCtlResizeCB(Widget w, XtPointer client_data,
        ...
        XtVaGetValues(w, XmNheight, &height, XmNwidth, &width, XmNmarginWidth,
                      &margin_width, NULL);
        XtVaGetValues(scrolled_list, XmNheight, &list_h, XmNwidth, &list_w,
                      NULL);
...
static void SizeCtlSelectCB(Widget w, XtPointer client_data,
        ...
        if (!me->single_size_mode) {
                ...
                case SIZECTL_SIZES_READY:
                        me->selected_size = PdmOidListGetOid(
                            me->sizes_ready, cbs->item_position - 1);
                        break;
                case SIZECTL_SIZES_SUPPORTED:
                        me->selected_size = PdmOidListGetOid(
                            me->sizes_supported, cbs->item_position - 1);
                        break;
                }
        }
}
...
static Widget CreateOptionMenuPulldown(String pulldown_name, Widget option_menu,
        ...
        for (i = 0; i < item_count; i++) {
                ...
                button = XtVaCreateManagedWidget(
                    button_name, xmPushButtonGadgetClass, pulldown, XmNuserData,
                    user_data, XmNlabelString, item_list[i], NULL);
        ...
        XtVaSetValues(option_menu, XmNsubMenuId, pulldown, XmNmenuHistory,
...
static void IconMaxDimension(Widget icon_label, PdmPixmapList *pixmap_list,
         ...
         */
        XtVaSetValues(icon_label, XmNlabelPixmap,
                      PdmPixmapListGetPixmap(pixmap_list, i), NULL);
        /*
         * get the label dimensions
         */
        XtVaGetValues(icon_label, XmNwidth, &width0, XmNheight, &height0, NULL);
...
static PdmPixmapList *PdmPixmapListNew(int count, Widget w, Boolean stipple) {
        ...
        if (stipple) {
                ...
                XtVaGetValues(xmscr, XmNinsensitiveStippleBitmap, &me->stipple,
                              NULL);
        } else {
                me->stipple = None;
        }
        ...
        if (me != (PdmPixmapList *)NULL) {
                ...
                for (i = 0; i < PdmPixmapListCount(me); i++) {
                        if (me->pixmap[i] != (Pixmap)NULL) {
                                if (me->stipple == None) {
                                        if (me->pixmap[i] != None)
                                                XmDestroyPixmap(me->screen,
                                                                me->pixmap[i]);
                                } else {
                                        if (me->pixmap[i] != None)
                                                XFreePixmap(
                                                    DisplayOfScreen(me->screen),
                                                    me->pixmap[i]);
                                }
 ...
 *
 */
static void PdmPixmapListSetImageName(PdmPixmapList *me, const char *image_name,
                                      int i) {
...
static Pixmap PdmPixmapListGetPixmap(PdmPixmapList *me, int i) {
        if (me->pixmap[i] == None) {
                ...
                if (me->pixmap[i] == XmUNSPECIFIED_PIXMAP) {
                         ...
                         */
                        XGetGeometry(display, xmpixmap, &root, &x, &y, &width,
                                     &height, &border_width, &depth);
                        me->pixmap[i] = XCreatePixmap(display, xmpixmap, width,
                                                      height, depth);
                        gc = XCreateGC(display, me->pixmap[i], (unsigned long)0,
                                       &values);
                        XCopyArea(display, xmpixmap, me->pixmap[i], gc, 0, 0,
                                  width, height, 0, 0);
                        /*
                         * stipple it
                         */
                        ...
                        XFillRectangle(display, me->pixmap[i], gc, 0, 0, width,
                                       height);
                        /*
                         * clean up
                         */
...
static void SetListBoxSelection(Widget list_box, int position) {
         ...
         */
        XtVaGetValues(list_box, XmNitemCount, &item_count0, XmNvisibleItemCount,
                      &visible_item_count0, NULL);
...
static String GetIconFileName(Widget w, String resource_name) {
        ...
        XtGetApplicationResources(w, (XtPointer)&icon_file_name, &res_struct, 1,
                                  (ArgList)NULL, 0);
