 ...
 *
 */
String *PdmMainWinMergeFallbacks(PdmMainWin *me, const String *app_fallbacks,
                                 int count) {
        ...
        for (node = me->box_list_head; node != (PdmBoxNode *)NULL;
             node = node->next)
...
static void PdmMainWinCreateSetupBoxes(PdmMainWin *me) {
        ...
        for (node = me->box_list_head; node != (PdmBoxNode *)NULL;
             node = node->next) {
                 ...
                 */
#ifdef USE_EXM_TABS
                node->tab = XtVaCreateManagedWidget(
                    box->tab_name, exmTabButtonWidgetClass, me->notebook, NULL);
#else
                node->tab = XtVaCreateManagedWidget(
                    box->tab_name, xmPushButtonWidgetClass, me->notebook, NULL);
#endif /* USE_EXM_TABS */
        }
}
 ...
 *
 */
static void PdmMainWinOkCB(Widget w, XtPointer client_data,
                           XtPointer call_data) {
         ...
         */
        for (node = me->box_list_head, page = 1; node != (PdmBoxNode *)NULL;
             node = node->next, page++) {
                PdmSetupBox *box = node->box;

                if ((*box->verify_attr_proc)(box, me->pdm_xp) != PDM_SUCCESS) {
                        /*
                         * Ensure this setup box is the top notebook page; the
                         * setup box is responsible for providing error messages
                         * to the user.
                         *
                         * developer hint: do not use the setup box as the
                         * parent of a message box; if the setup box is not the
                         *                 current notebook page, the message
                         * box will not be positioned properly; use the first
                         * shell ancestor (or even the parent)
                         *                 of the setup box instead
                         */
                        XtVaSetValues(me->notebook, XmNcurrentPageNumber, page,
                                      NULL);
                        /*
 ...
 *
 */
static void PdmMainWinCancelCB(Widget w, XtPointer client_data,
                               XtPointer call_data) {
        PdmMainWin *me = (PdmMainWin *)client_data;
        PdmMainWinDelete(me);
 ...
 *
 */
static void PdmMainWinHelpCB(Widget w, XtPointer client_data,
                             XtPointer call_data) {
        PdmMainWin *me = (PdmMainWin *)client_data;
        PdmSetupBox *box;
         ...
         */
        if ((Widget)NULL == me->help_dialog) {
                me->help_dialog = DtCreateHelpDialog(XtParent(me->widget),
                                                     "HelpDialog", NULL, 0);
         ...
         */
        {
                ...
                XtVaGetValues(me->notebook, XmNcurrentPageNumber, &current_page,
                              NULL);
         ...
         */
        {
                ...
                XtGetApplicationResources(
                    box->widget, (XtPointer)&box_resources, resources,
                    XtNumber(resources), (ArgList)NULL, 0);
        }
        /*
         * set the help volume and location
         */
        XtVaSetValues(me->help_dialog, DtNhelpVolume, box_resources.help_volume,
                      DtNlocationId, box_resources.location_id, DtNhelpType,
 ...
 *     None.
 */
static void PdmMainWinHelpDestroyCB(Widget w, XtPointer client_data,
                                    XtPointer call_data) {
        PdmMainWin *me = (PdmMainWin *)client_data;
 ...
 *
 */
PdmMainWin *PdmMainWinCreate(PdmMainWin *me, Widget parent,
...
static Widget PdmMainWinCreateWindow(PdmMainWin *me, Widget parent) {
         ...
         */
        me->widget =
            XtVaCreateManagedWidget("Main", xmMessageBoxWidgetClass, parent,
                                    XmNdialogType, XmDIALOG_MESSAGE, NULL);
         ...
         */
        XtAddCallback(me->widget, XmNokCallback, PdmMainWinOkCB, (XtPointer)me);
        XtAddCallback(me->widget, XmNcancelCallback, PdmMainWinCancelCB,
                      (XtPointer)me);
        XtAddCallback(me->widget, XmNhelpCallback, PdmMainWinHelpCB,
                      (XtPointer)me);
        /*
         * create the main manager widget
         */
        manager = XtVaCreateManagedWidget("Manager", xmRowColumnWidgetClass,
                                          me->widget, NULL);
        /*
        ...
        if (me->printer_descriptor != (const char *)NULL) {
                ...
                row = XtVaCreateManagedWidget(
                    "PrinterDescriptionRow", xmRowColumnWidgetClass, manager,
        ...
        if (me->printer_name != (const char *)NULL) {
                ...
                row = XtVaCreateManagedWidget(
                    "PrinterNameRow", xmRowColumnWidgetClass, manager,
                    XmNorientation, XmHORIZONTAL, NULL);
                /*
                ...
                XtFree(printer_spec);
                w = XtVaCreateManagedWidget("PrinterName", xmLabelGadgetClass,
         ...
         * top separator
         */
        w = XtVaCreateManagedWidget("TopSeparator", xmSeparatorGadgetClass,
                                    manager, NULL);
        /*
         * notebook label
         */
        row = XtVaCreateManagedWidget("PrinterNameRow", xmRowColumnWidgetClass,
                                      manager, XmNorientation, XmHORIZONTAL,
                                      NULL);
        w = XtVaCreateManagedWidget("NotebookLabel", xmLabelGadgetClass, row,
                                    NULL);
        /*
         * create the notebook
         */
        row = XtVaCreateManagedWidget("NotebookRow", xmRowColumnWidgetClass,
                                      manager, XmNorientation, XmHORIZONTAL,
                                      NULL);
        me->notebook = XtVaCreateManagedWidget(
            "Notebook", xmNotebookWidgetClass, row, NULL);
        /*
         ...
         */
        XtVaCreateWidget("DummyPageScroller", xmScrollBarWidgetClass,
                         me->notebook, NULL);
        /*
         * return
         */
