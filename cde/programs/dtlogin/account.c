...
extern void GetLoginInfo(struct display *d, int *loginType, char *ttyName,
                         char **hostname);
#endif
 ...
 ***************************************************************************/
void Account(struct display *d, char *user, char *line, pid_t pid,
        ...
        Debug("Account: id=%s, user=%s, line=%s, pid=%d, type=%s\n", d->utmpId,
              user_str, line_str, pid, buf);

#ifdef PAM
        PamAccounting("dtlogin", d->name, d->utmpId, user, line, pid, type,
                      exitcode);
#else
#ifdef SUNAUTH
        solaris_accounting("dtlogin", d->name, d->utmpId, user, line, pid, type,
                           exitcode);
#endif
#endif
         ...
         *	      Currently, an unlimited number of foreign displays can log
         *in.
         */
...
#ifdef _AIX
                /*
                      For AIX the Init process writes the exact mapped device
                   name for console to the utmp file (like hft/0), if a getty on
                   /dev/console record exists in the Inittab file.Hitherto, we
                   need to have a similar logic to make sure for having the
                   correct entry in the utmp file in order for the correct
                   operation of the GettyRunning function. It should be noted
                   that by having the correct value in the d->gettyLine field,
                   the utmp file eventuallly updated by the Account function in
                   dm.c will have the right value. And thus the GettyRunning
                   function returns the appropriate value. So, it is important
                   that the following logic be included here for AIX platform
                   only. Raghu Krovvidi         07.06.93
                 */
        ...
        if (type) {
                ...
                if (type == USER_PROCESS)
                        u->ut_exit.e_exit =
                            (d->displayType.location == Local ? 1 : 0);
        }
        ...
        if (line)
                strncpy(rtmp.rt_line, u->ut_line, sizeof(u->ut_line));
        ...
        if (tty_slot > 0 &&
            (rtmp_fd = open("/etc/rtmp", O_WRONLY | O_CREAT, 0644)) >= 0) {
        ...
        if (type == USER_PROCESS) {
                ...
                if (line) {
                        ...
                        last_login.stty =
                            (char *)malloc(strlen(tempTtyName) + 1);
...
#if defined(AIXV3) && !defined(_POWER)
void dt_lastlogin(user, llogin) char *user;
struct lastlogin *llogin;
{
        ...
        if (llogin->ftime) {
                ...
                llogin->fcount++;
                putuserattr(user, S_ULOGCNT, (void *)llogin->fcount, SEC_INT);
                putuserattr(user, S_ULASTTIME, (void *)llogin->ftime, SEC_INT);
         ...
         */
        if (llogin->stime) {
                putuserattr(user, S_LASTTIME, (void *)llogin->stime, SEC_INT);
