...
static void DebugFontPath(char *note, char **path, int nelems) {
        int i;
        Debug("  %s: %d elements\n", note, nelems);
        for (i = 0; i < nelems; i++)
                Debug("    %s\n", path[i]);
        ...
        */
        ...
        if (numHeads || numTails) {
                ...
                if (newList) {
                        ...
                        if (debugLevel > 0)
                                DebugFontPath("Request (XSetFontPath)", newList,
                                              i);

                        /*
                        **  Tell X server to set new font path now.  Log
                        *failure,
                        ...
                        */
                        ...
                        if (debugLevel > 0) {
                                ...
                                DebugFontPath("Confirm (XGetFontPath)", newList,
                                              i);
...
void GetSysParms(char **tzpp, char **fhpp, char **ftpp) {
        ...
        while (fgets(buf, 255, fin)) {
                ...
                while (*t && *t != '\n')
                        t++; /* t -> EOS or newline  */
                if (*t)
                        *t = '\0'; /* discard newline char */
                if (tzpp && !*tzpp)
                        if ((s = strstr(buf, "TZ=")) &&
                            (t = strtok(s + 3, "; \t")))
                                *tzpp = (char *)strdup(t);
                if (fhpp && !*fhpp)
                        if ((s = strstr(buf, "FONT_PATH_HEAD=")) &&
                            (t = strtok(s + 15, "; \t")))
                                *fhpp = (char *)strdup(t);
                if (ftpp && !*ftpp)
                        if ((s = strstr(buf, "FONT_PATH_TAIL=")) &&
                            (t = strtok(s + 15, "; \t")))
                                *ftpp = (char *)strdup(t);
        }
        fclose(fin);
