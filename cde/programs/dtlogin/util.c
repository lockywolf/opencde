...
unsigned char *ReadCatalog(int set_num, int msg_num, char *def_str) {
        ...
        if (alreadyopen == False) {
                ...
                /*
                 * Desktop message catalogs are in DT directory, so append
                 * desktop search paths to current NLSPATH.
                 */
#define NLS_PATH_STRING                                                        \
        CDE_INSTALLATION_TOP                                                   \
        "/nls/msg/%L/%N.cat:" CDE_INSTALLATION_TOP                             \
        ...
        "/lib/nls/msg/%l/%c/%N.cat"
                ...
                newNlsPath =
                    malloc(newNlsPathLen); /* placed in environ, do not free */

                if (curNlsPath != NULL) {
                        sprintf(newNlsPath, "NLSPATH=%s:%s", curNlsPath,
                                NLS_PATH_STRING);
                } else {
                ...
                /*
                 * Store new NLSPATH in environment. Note this memory cannot be
                 * freed
                 */
                putenv(newNlsPath);

                /*
                 * Open message catalog. Note, if invalid descriptor returned
                 * (ie msg catalog could not be opened), subsequent call to
                 * catgets() using that descriptor will return 'def_str'.
                 */
...
static char *makeEnv(char *name, char *value) {
        ...
        if (!result) {
                LogOutOfMem(ReadCatalog(MC_LOG_SET, MC_LOG_MAKEENV,
                                        MC_DEF_LOG_MAKEENV));
                return 0;
        }
...
char **setEnv(char **e, char *name, char *value) {
        ...
        if (e) {
                ...
                envsize = old - e;
                new = (char **)realloc(
                    (char *)e, (unsigned)((envsize + 2) * sizeof(char *)));
        } else {
                envsize = 0;
...
char **parseArgs(char **argv, char *string) {
        ...
        if (!argv) {
                ...
                if (!argv) {
                        LogOutOfMem(ReadCatalog(MC_LOG_SET, MC_LOG_PARSEARGS,
...
static unsigned char time16_bits[] = {
    0x00, 0x00, 0xfe, 0x7f, 0x14, 0x28, 0x14, 0x28, 0x14, 0x28, 0x24,
    0x24, 0x44, 0x22, 0x84, 0x21, 0x84, 0x21, 0x44, 0x22, 0x24, 0x24,
    0x14, 0x28, 0x94, 0x29, 0xd4, 0x2b, 0xfe, 0x7f, 0x00, 0x00};

#define time16m_width 16
#define time16m_height 16
static unsigned char time16m_bits[] = {
    0xff, 0xff, 0xff, 0xff, 0xfe, 0x7f, 0xfe, 0x7f, 0xfe, 0x7f, 0xfe,
...
Cursor GetHourGlass(Display *dpy) {
        ...
        if (XQueryBestCursor(dpy, DefaultRootWindow(dpy), 32, 32, &cWidth,
                             &cHeight)) {
        ...
        pixmap = XCreateBitmapFromData(dpy, DefaultRootWindow(dpy), bits, width,
                                       height);
        ...
        waitCursor = XCreatePixmapCursor(dpy, pixmap, maskPixmap, &(xcolors[0]),
...
void MakeLangList(void) {
        ...
        if (!setjmp(langJump)) {
                ...
                LogError(
                    ReadCatalog(MC_LOG_SET, MC_LOG_NO_SCAN, MC_DEF_LOG_NO_SCAN),
...
static int MatchesFileSuffix(const char *filename, const char *suffix) {
    ...
    defined(CSRG_BASED)
        ...
        if (strlen(filename) >= strlen(suffix)) {
                different = strcmp(
                    filename + (strlen(filename) - strlen(suffix)), suffix);
        }
                ...
                while ((dp = readdir(dirp)) != NULL) {
                        ...
                        if ((strcmp(dp->d_name, LOCALEOLD) == 0) ||
                            (strcmp(dp->d_name, LOCALE) == 0)) {
                                 ...
                                 */
                                if ((p = strstr(dirname, DEF_NLS_DIR)) !=
                                    NULL) {
                                        ...
                                        /*
                                         *  append to global list of
                                         * languages...
                                         */
                                        if ((int)(strlen(languageList) +
                                                  strlen(s) + 2) <
                                            LANGLISTSIZE) {
                                                strcat(languageList, " ");
 ...
 */
{
        ...
        while (NULL != fgets(locale, sizeof(locale), f)) {
                if (locale[0] != '.' &&
                    LANGLISTSIZE >
                        (int)(strlen(languageList) + strlen(locale) + 2)) {
...
#include <fnmatch.h>
        ...
        for (p = strtok(str, delim); p; p = strtok(NULL, delim)) {
                if (num_ignore >= max_ignore) {
                        max_ignore += 16;
                        if (!(ignore = realloc(ignore,
                                               max_ignore * sizeof(char *)))) {
                                free(str);
                                return;
                        }
                }
                ...
                while (entp = readdir(dirp)) {
                        ...
                        if (namelen >= 5 && entp->d_name[2] == '_') {
                                ...
                                for (i = 0; i < num_ignore; i++) {
                                        if (!fnmatch(ignore[i], entp->d_name,
                                                     0)) {
                                                match = 1;
                                                break;
                                        }
 ...
 */
{
        ...
        if ((nls_dirp = opendir(dirname)) != NULL) {
                while ((dp = readdir(nls_dirp)) != NULL) {
                        ...
                        /*
                         * A locale is indicated by a directory which has one or
                         *more of the following subdirectories: LC_COLLATE
                         *		LC_CTYPE
                         *		LC_MESSAGES
                         *		LC_MONETARY
                         *		LC_NUMERIC
                        ...
                        if (is_locale &&
                            LANGLISTSIZE > (int)(strlen(languageList) +
                                                 strlen(locale) + 2)) {
                                strcat(languageList, " ");
                                strcat(languageList, locale);
                        }
                ...
                while ((dp = readdir(dirp)) != NULL) {
                        ...
                        if (locale[0] != '.' &&
                            LANGLISTSIZE > (int)(strlen(languageList) +
                                ...
                                ;
                        {
                                (void)sprintf(locale_path, "%s/%s", dirname,
                                              locale);
                                retval = stat(locale_path, &locale_stat);

                                if (0 == retval &&
                                    S_ISDIR(locale_stat.st_mode)) {
                                        strcat(languageList, " ");
                                        strcat(languageList, locale);
                                }
                        }
