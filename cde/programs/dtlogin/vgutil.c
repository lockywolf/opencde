...
int InitArg(WidgetType wtype) {
        ...
        if (wtype == DrawingA || wtype == Frame || wtype == Form ||
            wtype == Label || wtype == MessageBox || wtype == PushB ||
            wtype == Text) {
        ...
        if (wtype == Form || wtype == MessageBox) {
                ...
                i++;
                /*XtSetArg(argt[i], XmNlabelFontList,	appInfo.textFont
                 * ); i++;
                 */
                ...
                i++;
                XtSetArg(argt[i], XmNstringDirection,
        ...
        if (wtype == CascadeBG || wtype == Label || wtype == LabelG ||
            wtype == PushB || wtype == PushBG || wtype == ToggleBG) {
                ...
                i++;
                /*	XtSetArg(argt[i], XmNalignment,
                 * XmALIGNMENT_CENTER	); i++;*/
...
int FromPixel(Widget w, int orientation, int pixel) {
        ...
        if (unit_type == XmPIXELS)
                ...
                return (XmConvertUnits(w, orientation, XmPIXELS, pixel,
                                       (int)unit_type));
}
...
void CloseCatalog() { catclose(nl_fd); }
...
void OpenCatalog() {
        ...
        if (NULL == nlspath || 0 == strlen(nlspath)) {
                ...
                newnlspath =
                    malloc(8 + strlen(nlspath) + 1 + strlen(NLSPATH_ENV) + 1);
        ...
        if (NULL != langenv) {
                ...
                if (0 > (long)nl_fd)
                        LogError((unsigned char *)MC_DEF_LOG_NO_MSGCAT,
                                 langenv);
        }
}
...
XmString ReadCatalogXms(int setn, int msgn, char *dflt) {
        ...
        if (0 > (long)nl_fd)
                ...
                return XmStringCreateLocalized(
                    (char *)ReadCatalog(setn, msgn, dflt));
}
...
static SIGVAL syncTimeout(int arg) { longjmp(syncJump, 1); }
...
int SecureDisplay(void) {
        ...
        if (setjmp(syncJump)) {
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_SECDPY,
                                     MC_DEF_LOG_NO_SECDPY),
                         dpyinfo.name);
                return (1);
        }
        ...
        if (XGrabKeyboard(dpyinfo.dpy, DefaultRootWindow(dpyinfo.dpy), True,
                ...
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_SECKEY,
                                     MC_DEF_LOG_NO_SECKEY),
                         dpyinfo.name);
                return (1);
        }
...
#define NUM_CO_RESOURCES (sizeof defResourceCO / sizeof defResourceCO[0])
      ...
      XrmPutStringResource(&defDb, VnNmatteHeight, defaultHeight);
        */
        ...
        if (!XrmGetResource(defDb, "*labelFont", "*LabelFont", &rmtype,
                            &rmvalue))
        ...
        for (i = 0; i < NUM_RESOURCES; i++)
                XrmPutStringResource(&defDb, defResource[i].name,
                                     defResource[i].value);
        ...
        if (XrmGetResource(userDb, VnNmatteWidth, VnCMatteWidth, &rmtype,
                           &rmvalue)) {

                sscanf(rmvalue.addr, "%d", &i);
                if (appInfo.unitType == XmPIXELS && i >= dpyinfo.width) {
                        XrmPutStringResource(&userDb, VnNmatteWidth,
                                             defaultWidth);
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_BAD_MATTE,
                                             MC_DEF_LOG_BAD_MATTE),
                                 dpyinfo.name);
                }
        }
        ...
        if (XrmGetResource(userDb, VnNmatteHeight, VnCMatteHeight, &rmtype,
                ...
                if (appInfo.unitType == XmPIXELS && i >= dpyinfo.height) {
                        XrmPutStringResource(&userDb, VnNmatteHeight,
                                             defaultHeight);
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_BAD_HMATTE,
                                             MC_DEF_LOG_BAD_HMATTE),
                                 dpyinfo.name);
                }
        }
}
...
int ToPixel(Widget w, int orientation, int pixel) {
        ...
        if (unit_type == XmPIXELS)
                ...
                return (XmConvertUnits(w, orientation, (int)unit_type, pixel,
                                       XmPIXELS));
        }
}
