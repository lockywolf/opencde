...
#ifdef sun
#define DEF_USER_PATH                                                          \
        "/usr/openwin/bin:/bin:/usr/bin:/usr/contrib/bin:/usr/local/bin:."
#else
#define DEF_USER_PATH                                                          \
        "/usr/bin/X11:/bin:/usr/bin:/usr/contrib/bin:/usr/local/bin"
#endif /* sun */
#endif
...
struct displayResources {
    ...
    "StartAttempts",
    DM_INT,
    boffset(startAttempts),
    "4",
    "pingInterval",
    "PingInterval",
    DM_INT,
    boffset(pingInterval),
    "5",
    "pingTimeout",
    "PingTimeout",
    DM_INT,
...
"gettySpeed",	"GettySpeed",	DM_STRING,	boffset(gettySpeed),
    ...
    "PmSearchPath",
    DM_STRING,
    boffset(pmSearchPath),
    DEF_PM_SEARCH_PATH,
    "bmSearchPath",
    "bmSearchPath",
    DM_STRING,
    boffset(bmSearchPath),
    DEF_BM_SEARCH_PATH,
};

#define NUM_DISPLAY_RESOURCES                                                  \
        (sizeof DisplayResources / sizeof DisplayResources[0])

XrmDatabase DmResourceDB;

int GetResource(char *name, char *class, int valueType, char **valuep,
                char *default_value) {
        char *type;
        XrmValue value;
        ...
        if (DmResourceDB &&
            XrmGetResource(DmResourceDB, name, class, &type, &value)) {
                string = value.addr;
                len = value.size;
        } else {
        ...
        if (valueType == DM_STRING && *valuep) {
                if (string != NULL && strlen(*valuep) == len &&
                    !strncmp(*valuep, string, len))
                        return 0;
                else {
                        free(*valuep);
                        *(valuep) = NULL;
        ...
        case DM_BOOL:
                ...
                if (!strcmp(str_buf, "true") || !strcmp(str_buf, "on") ||
                    !strcmp(str_buf, "yes"))
                        *((int *)valuep) = 1;
                else if (!strcmp(str_buf, "false") || !strcmp(str_buf, "off") ||
                         !strcmp(str_buf, "no"))
                        *((int *)valuep) = 0;
                break;
        case DM_ARGV:
...
void ReinitResources(void) {
        ...
        if (!argv)
                LogOutOfMem(ReadCatalog(MC_LOG_SET, MC_LOG_NO_SPACE,
                                        MC_DEF_LOG_NO_SPACE));
        ...
        XrmParseCommand(&DmResourceDB, configTable,
                        sizeof(configTable) / sizeof(configTable[0]), "Dtlogin",
                        &argc, argv);

        configFileName =
            qualifyWithFirst(DEF_XDM_CONFIG, CDE_CONFIGURATION_TOP
                             "/config:" CDE_INSTALLATION_TOP "/config");

        GetResource("Dtlogin.configFile", "Dtlogin.ConfigFile", DM_STRING,
        ...
        if (newDB) {
                ...
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_OPENCFG,
                                     MC_DEF_LOG_NO_OPENCFG),
                         config);
        ...
        XrmParseCommand(&DmResourceDB, optionTable,
                        sizeof(optionTable) / sizeof(optionTable[0]), AppName,
                        &argc, argv);
         ...
         */
        if ((XrmGetResource(DmResourceDB, "Dtlogin*session", "Dtlogin*Session",
                            &type, &value)) == True)
                session_set = True;

        if (argc > 1) {
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_EXTRA_ARG,
                                     MC_DEF_LOG_EXTRA_ARG));
                for (a = argv + 1; *a; a++)
...
void LoadDisplayResources(struct display *d) {
        int i;
        char name[1024], class[1024];
        ...
        for (i = 0; i < NUM_DISPLAY_RESOURCES; i++) {
                sprintf(name, "%s.%s.%s", AppName, dpyName,
                        DisplayResources[i].name);
                sprintf(class, "%s.%s.%s", AppName, dpyClass,
                        DisplayResources[i].class);
...
static char *getFullFileName(char *name, int special, char *lang) {
        ...
        case 0: /** no special case **/
                /*********************/
                newname =
                    qualifyWithFirst(name, CDE_CONFIGURATION_TOP
                                     "/config:" CDE_INSTALLATION_TOP "/config");
                return (newname);
        ...
        case 1: /** special handling for servers file **/
                ...
                if (strchr(name, ':') != NULL) {
                        ...
                        newname = qualifyWithFirst(
                            name, CDE_CONFIGURATION_TOP
                            "/config:" CDE_INSTALLATION_TOP "/config");
                        return (newname);
                }
        ...
        case 2: /** special handling for d->resources file **/
                ...
                if (strchr(name, '%') == NULL) {
                        ...
                        if ((tempName = qualifyWithFirst(
                                 langString, CDE_CONFIGURATION_TOP
                                 "/config:" CDE_INSTALLATION_TOP "/config")) ==
                            NULL) {
                                free(langString);
                        ...
                        tempLen = strlen(tempName) - strlen(langString) +
                                  strlen(name) + 1;
                        if ((newname = (char *)malloc(tempLen *
                                                      sizeof(char))) == NULL) {
                                free(langString);
                                return (tempName);
                        } else {
