...
static char *GetLangName(char *label) {
        /*
         * Default implementation is to use langName resource. Language names
         * can be set in Dtlogin file as follows. (In this case, en_US is a
         * locale name which can be set to LANG environment variable.
         *
...
static void _enumLangEnd(void *state) {
         ...
         *
         *    A sysadmin can set the Dtlogin.languageList resource to set the
         *list of languages. The dtlogin process provides this list to dtgreet
         *    in the LANGLIST environment variable. This has priority.
         *
         *  * Login manager determined language list
         *
         *    If the sysadm does not set Dtlogin.languageList, ie. LANGLIST
         *unset, the login manager will generate the list. There are two methods
         *for doing this, one of which is selected at compile time with the
         ...
         *   provide a descriptive name for a particular language. If
         *languageName unset, the value used depends on ENABLE_DYNAMIC_LANGLIST.
         *If ENABLE_DYNAMIC_LANGLIST set, the value used is the descriptive
         *text provided by languageListCmd. If ENABLE_DYNAMIC_LANGLIST unset,
         *the value used is simply the locale name.
         *
         * * Default language
         *
...
void MakeLangMenu(void) {
        ...
        if (state) {
                while (_enumLangNext(state, &list[nlang].lang,
                                     &list[nlang].desc)) {
                        nlang++;
                }
                _enumLangEnd(state);
        ...
        if (nlang > 0) {
                ...
                i = 0;
                lang_menu =
                    XmCreatePulldownMenu(options_menu, "lang_menu", argt, i);
                item_menu = lang_menu;
                ...
                for (k = 0; k < nlang; k++) {
                        ...
                        if (nlangMenus > 1 && k % maxitems == 0) {
                                i = 0;
                                item_menu = XmCreatePulldownMenu(
                                    lang_menu, "item_menu", argt, i);
                        }
                        ...
                        tostr = (char *)ReadCatalog(MC_LABEL_SET, MC_TO_LABEL,
                                                    MC_DEF_TO_LABEL);

                        if (nlangMenus > 1 && k % maxitems == 0) {
                                int first = k;
                                int last = k + maxitems >= nlang
                                               ? nlang - 1
                                               : k + maxitems - 1;

                                i = InitArg(CascadeBG);

                                sprintf(cblab, "%s %s %s", list[first].desc,
                                        tostr, list[last].desc);
                                ...
                                XtSetArg(argt[i], XmNrecomputeSize, True);
                                i++;
                                button = XmCreateCascadeButtonGadget(
                                    lang_menu, cblab, argt, i);
                                XtManageChild(button);
                                XmStringFree(xmstr);
                        }
                }
                 ...
                 */
                for (k = 0; k < nlang; k++) {
                        /* free(list[k].lang); -- used as callback data, don't
                         * free */
                        free(list[k].desc);
                }
        }
