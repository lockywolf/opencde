...
int main(int argc, char **argv) {
        ...
        if (debugLevel == 0 && getuid() != 0) {
                fprintf(stderr,
                        (char *)ReadCatalog(MC_ERROR_SET, MC_BAD_ROOT,
                                            MC_DEF_BAD_ROOT),
                        argv[0]);
                exit(1);
        }
        ...
        if ((oldpid = StorePid()) != 0) {
                if (oldpid == (long)-1)
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_CREATE,
                                             MC_DEF_LOG_NO_CREATE),
                                 pidFile);
                else
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_LOCK,
                                             MC_DEF_LOG_NO_LOCK),
                                 pidFile, oldpid);
                exit(1);
        }
        ...
        /*
         * Initialize error file, open XDMCP socket, and set up interrupt
         * handlers.
         */
        ...
        if ((p = getpwnam("nobody")) != NULL) {
                ...
                /*
                 * This should not happen, the "nobody" user should always be
                 * present. If it does, fall back to traditional values of the
                 * "root" user
                 */
                puser.pw_uid = 0;
                puser.pw_gid = 1;
...
#define NumTypes (sizeof(acceptableTypes) / sizeof(acceptableTypes[0]))
        ...
        if (servers[0] == '/') {
                ...
                if (serversFile == NULL) {
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_SRVACC,
                                             MC_DEF_LOG_NO_SRVACC),
                                 servers);
                        return 0;
                }
                if (ServersModTime == 0) {
        ...
        if (stat(config, &statb) != -1) {
                if (statb.st_mtime != ConfigModTime) {
                        Debug("Config file %s has changed, rereading\n",
                              config);
                        LogInfo(ReadCatalog(MC_ERROR_SET, MC_LOG_REREADCFG,
                                            MC_DEF_LOG_REREADCFG),
                                config);
                        ConfigModTime = statb.st_mtime;
                        ReinitResources();
                        LoadDMResources();
        ...
        if (servers[0] == '/' && stat(servers, &statb) != -1) {
                if (statb.st_mtime != ServersModTime) {
                        Debug("Servers file %s has changed, rescanning\n",
                              servers);
                        LogInfo(ReadCatalog(MC_ERROR_SET, MC_LOG_REREADSRV,
                                            MC_DEF_LOG_REREADSRV),
                                servers);
                        ServersModTime = statb.st_mtime;
                        ForEachDisplay(MarkDisplay);
                        ScanServers();
        ...
        if (accessFile && accessFile[0] && stat(accessFile, &statb) != -1) {
                if (statb.st_mtime != AccessFileModTime) {
                        Debug("Access file %s has changed, rereading\n",
                              accessFile);
                        LogInfo(ReadCatalog(MC_ERROR_SET, MC_LOG_REREADACC,
                                            MC_DEF_LOG_REREADACC),
                                accessFile);
                        AccessFileModTime = statb.st_mtime;
                        ScanAccessDatabase();
                }
...
static SIGVAL StopAll(int arg) {
        if (parent_pid != getpid()) {
                 ...
                 */
                Debug("Child dtlogin caught signal %d before it could become a "
                      "real child\n",
                      arg);
...
static void ProcessChildDeath(int pid, waitType status) {
        ...
        if ((d = FindDisplayByPid(pid)) != 0) {
                ...
                */
                 ...
                 *
                 *  Note: if startTries expires and a "getty" is run on the
                 *local display, startTries will be reset to zero before
                ...
                default:
                        Debug("Display exited with unknown status %d\n",
                              waitVal(status));
                        LogError((unsigned char *)"Unknown session exit code "
                                                  "%d from process %d\n",
                                 waitVal(status), pid);
                        StopDisplay(d);
                        break;
                ...
                case running:
                        ...
                        LogError(
                            (unsigned char *)"Server for display %s terminated "
                                             "unexpectedly %d\n",
                            d->name, waitVal(status));
                        if (d->pid != -1) {
                        ...
                        break;
                case notRunning:
                        Debug("Server exited for notRunning session on display "
                              "%s\n",
                              d->name);
                        break;
                case suspended:
                                ...
                                if (GettyRunning(d) ||
                                    (d->gettyLine &&
                                     (strcmp(d->gettyLine, "??") == 0)))
                                        ...
                                        Debug("Polling of suspended server %s "
                                              "continued.\n",
                                              d->name);
                                        wakeupTime =
                                            wakeupInterval; /* continue
                                                               polling*/
                                }
...
int StartDisplay(struct display *d) {
        ...
        /*
         * The following call to RemoveDisplay is to catch race conditions
         * during shutdown. There is no point in starting a display if Dtlogin
...

        {
                ...
                /* copy it back to authFile field od display structure for X
                 * server */
                ...
                Debug(
                    "(Mid StartDisplay) d->authfile %s; authDir %s; bkup %s\n",
        ...
        if (d->displayType.location == Local) {
                ...
                if (d->authorize) {
                        ...
                        /*
                         * reset the server after writing the authorization
                         * information to make it read the file (for
                         * compatibility with old servers which read auth file
                         * only on reset instead of at first connection)
                         */
                ...
                if (d->utmpId == NULL) {
                        ...
                        do {
                                if (firsttime || UtmpIdOpen(d->utmpId)) {
                                        ...
                                        strncpy(&(d->utmpId[strlen(d->utmpId)]),
                                                t++, 1);
                                }
                        } while (*t != '\0');

                        if (*t == '\0') {
                                Debug("All DT utmp IDs already in use. "
                                      "Removing display %s\n",
                                      d->name);
                                LogError(
                                    (unsigned char *)"All DT utmp IDs already "
                                                     "in use. Removing display "
                                                     "%s\n",
                                    d->name);
                                RemoveDisplay(d);
                                return 0;
                ...
                /*
                 *  if gettyLine is set to "console", set gettySpeed to
                 * "console" also
                 */
                ...
                /*
                 *  start server. If it cannot be started and this is the
                 * console, run a getty...
                 */
                ...
                if (d->serverPid == -1 &&
                    (d->startTries++ >= d->startAttempts || !StartServer(d))) {
                        LogError(
                            (unsigned char
                                 *)"Server for display %s can't be started.\n",
                            d->name);
                ...
                if (d->authorizations)
                        SaveServerAuthorizations(d, d->authorizations,
                                                 d->authNum);

                        /*
                         *  Generate a utmp ID address for a foreign display.
                         * Use the last four characters of the DISPLAY name,
                         * shifting left if they are already in use...
                         */
                ...
                if (d->utmpId == NULL) {
                        ...
                        /*
                         * If "networkDev" does not start with "/dev/" then
                         * foreign accounting is turned off. Return utmpId to
                         * NULL.
                         */
                        if (networkDev &&
                            strncmp(networkDev, "/dev/", 5) != 0) {
                                free(d->utmpId);
                                d->utmpId = NULL;
                        }
#endif
        ...
        case 0:
...
#endif
#if !defined(CSRG_BASED)
                        Account(d, "LOGIN", line, getpid(), LOGIN_PROCESS,
                                status);
#endif
                }
                ...
                if (!WaitForServer(d))
                         ...
                         *
                         * if the display is remote, fbconsole should be started
                         * on remote host.
                         */
#ifdef sun
                if (d->displayType.location == Local) {
                        ...
                        sprintf(start_fbconsole, "%s -d %s &", FBCONSOLE,
                                d->name);

                        if (system(start_fbconsole) == -1)
                                Debug("Failed to start the fallback console - "
                                      "%s\n",
                                      FBCONSOLE);
                }
#endif
...
void StopDisplay(struct display *d) {
        ...
        bzero(&status, sizeof(waitType));
        Debug("StopDisplay(): %s, server pid = %d, manager pid = %d, "
              "dt_shutdown = "
              "%d\n",
        ...
        if (d->serverPid != -1)
                ...
                if ((d->displayType.location == Local) && !dt_shutdown)
                        ...
                        d->status =
                            zombie; /* be careful about race conditions */
        ...
        if (d->serverPid != -1) {
...
#ifdef sun
                {
                        ...
                        } else if (ioctl(kbd_fd, KIOCTRANS,
                                         (caddr_t)&translate)) {
                                Debug("Could not set /dev/kbd back to ASCII "
                                      "mode\n");
                        }
                }
#endif
...
static long StorePid(void) {
        ...
        if (pidFile && pidFile[0] != '\0') {
                ...
                if (pidFd == -1 && errno == ENOENT) {
                         ...
                         * open it Read/Write.  This will be transparent to
                         * HP-UX.
                         */
...
static int StartGetty(struct display *d) {
        ...
        case 0:
...
#ifdef _AIX
                /* The tty argument for getty on AIX must be of the form
                   "/dev/any tty" and so the following logic Raghu krovvidi
                   07.07.93
                 */
                strcpy(tynm, "/dev/");
                ...
                LogError(
                    (unsigned char *)"Can not execute %s for %s: errno = %d\n",
        ...
        case -1:
                ...
                LogError(
                    (unsigned char *)"Can not fork to execute /etc/getty %s\n",
                    d->name);
                return FALSE;
...
void GettyMessage(struct display *d, int msgnum) {
        ...
        if ((tf = fopen(buf, "a")) != NULL) {
                ...
                case 1:
                        ...
                        fprintf(tf, "%s",
                                (char *)ReadCatalog(MC_LABEL_SET, MC_SUS1_LABEL,
                                                    MC_DEF_SUS1_LABEL));

                        fprintf(tf, "%s",
                                (char *)ReadCatalog(MC_LABEL_SET, MC_SUS2_LABEL,
                                                    MC_DEF_SUS2_LABEL));

                        fprintf(tf, "%s",
                                (char *)ReadCatalog(MC_LABEL_SET, MC_SUS3_LABEL,
                                                    MC_DEF_SUS3_LABEL));
                ...
                case 2:
                        ...
                        fprintf(tf, "%s",
                                (char *)ReadCatalog(MC_LABEL_SET, MC_RES_LABEL,
                                                    MC_DEF_RES_LABEL));
                ...
                case 3:
                        ...
                        fprintf(tf,
                                (char *)ReadCatalog(MC_LABEL_SET,
                                                    MC_START_LBLDPY,
                                                    MC_DEF_START_LBLDPY),
                                d->name);

                        fprintf(tf, "%s",
                                (char *)ReadCatalog(MC_LABEL_SET, MC_WAIT_LABEL,
                                                    MC_DEF_WAIT_LABEL));
                ...
                case 4:
                        ...
                        fprintf(tf,
                                (char *)ReadCatalog(MC_LABEL_SET, MC_X_LABEL,
                                                    MC_DEF_X_LABEL),
                                d->name);
...
int GettyRunning(struct display *d) {
#if defined(__FreeBSD__) && OSMAJORVERSION > 8
        ...
        while ((u = getutent()) != NULL) {
                ...
                if ((strncmp(u->ut_line, utmp.ut_line, sizeof(u->ut_line)) !=
                     0) ||
                ...
                case INIT_PROCESS:
                        strcpy(buf, "INIT_PROCESS");
                        break;
                case LOGIN_PROCESS:
                        strcpy(buf, "LOGIN_PROCESS");
                        break;
                case USER_PROCESS:
                        strcpy(buf, "USER_PROCESS");
                        break;
                case DEAD_PROCESS:
                        strcpy(buf, "DEAD_PROCESS");
                        break;
                default:
                        strcpy(buf, "UNKNOWN");
                        break;
...
static void CheckRestartTime(void) {
        ...
        if (servers[0] == '/' && stat(servers, &statb) != -1) {
                ...
                if (sleeptime > 0) {
                        Debug("Restarting too soon after shutdown. Sleeping %d "
                              "seconds.\n",
                              sleeptime);
                        sleep(sleeptime);
                }
