...
void InitErrorLog(void) {
        ...
        if (errorLogFile && errorLogFile[0]) {
                ...
                if (i != -1) {
                        ...
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_ERRLOG,
...
void TrimErrorFile(void) {
        ...
        if (errorLogSize > (200 * 1024)) {
                ...
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_MAX_LOGFILE,
                                     MC_DEF_LOG_MAX_LOGFILE));
        }
        ...
        if (errorLogFile && errorLogFile[0] &&
            ...
            (statb.st_size > errorLogSize)) {
                ...
                deleteBytes =
                    (statb.st_size - errorLogSize) + (errorLogSize / 4);

                Debug("TrimErrorLog(): discarding oldest %d bytes from logfile "
                      "%s\n",
                ...
                if ((f1 = open(errorLogFile, O_RDWR)) < 0 ||
                    (f2 = open(errorLogFile, O_RDWR)) < 0) {
                        Debug("TrimErrorLog(): Cannot open file %s, error "
                              "number = %d\n",
                              errorLogFile, errno);
                        if (f1 >= 0) {
                                close(f1);
                ...
                if ((status = lseek(f2, deleteBytes, SEEK_SET)) < 0) {
                        Debug("TrimErrorLog(): Cannot lseek() in file %s, "
                              "error number = %d\n",
                              errorLogFile, errno);
                        close(f1);
                        close(f2);
...
void LogOutOfMem(unsigned char *fmt, ...) {
        ...
        if (SyncErrorFile(1)) {
                fprintf(stderr, "%s",
                        (char *)ReadCatalog(MC_ERROR_SET, MC_NO_MEMORY,
                                            MC_DEF_NO_MEMORY));
                vfprintf(stderr, (char *)fmt, args);
                fflush(stderr);
        }
