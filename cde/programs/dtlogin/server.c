...
int StartServerOnce(struct display *d) {
        ...
        case 0:
                ...
                if (!argv) {
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_ARGS,
                                             MC_DEF_LOG_NO_ARGS));
                        exit(1);
                }
                 ...
                 */
#ifdef sun
                if (solaris_setdevperm(d->gettyLine, puser.pw_uid,
                                       puser.pw_gid) == 0)
                ...
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_EXESRV,
                                     MC_DEF_LOG_NO_EXESRV),
                         argv[0]);
                exit(1);
        case -1:
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_FAIL_FORK,
                                     MC_DEF_LOG_FAIL_FORK));
                return 0;
        default:
                break;
        }
...
static int serverPause(unsigned t, int serverPid) {
        ...
        if (serverPauseRet) {
                Debug("Server died\n");
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_SRV_DIED,
                                     MC_DEF_LOG_SRV_DIED));
        }
...
static SIGVAL abortOpen(int arg) { longjmp(openAbort, 1); }
...
int WaitForServer(struct display *d) {
        ...
        if (LogOpenError(d->startTries))
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_FAIL_SRVOPEN,
                                     MC_DEF_LOG_FAIL_SRVOPEN),
                         d->startTries, d->name);
        return 0;
}
