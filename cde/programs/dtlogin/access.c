...
static int scanHostlist(HostEntry *h, ARRAY8Ptr clientAddress,
                        ...
                        int (*function)(), char *closure, int depth,
                        int broadcast);
...
tryagain:
        ...
        if (*hostOrAlias == ALIAS_CHARACTER) {
                ...
                if (!hostOrAlias) {
                        ...
                        LogError(
                            (unsigned char *)"Access file \"%s\", No username "
                ...
                if (!strcmp(hostOrAlias, "root")) {
                        LogError((unsigned char *)"Access file \"%s\", root "
                                                  "bypass disallowed\n",
                                 accessFile);
                        free((char *)h);
                        return NULL;
                ...
                if (!hostent) {
                        ...
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_ACC_FILE,
                                             MC_DEF_LOG_ACC_FILE),
                                 accessFile, hostOrAlias);
                        free((char *)h);
                        goto tryagain;
                }
                if (!XdmcpAllocARRAY8(&h->entry.hostAddress,
                                      hostent->h_length)) {
                        LogOutOfMem(ReadCatalog(MC_LOG_SET, MC_LOG_HOST_ENT,
                                                MC_DEF_LOG_HOST_ENT));
                        free((char *)h);
                        return NULL;
                }
...
static DisplayEntry *ReadDisplayEntry(FILE *file) {
        ...
        if (*displayOrAlias == ALIAS_CHARACTER) {
                ...
                if (HasGlobCharacters(displayOrAlias)) {
                        ...
                        d->entry.displayPattern =
                            malloc(strlen(displayOrAlias) + 1);
                        ...
                        if ((hostent = gethostbyname(displayOrAlias)) == NULL) {
                                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_ACC_DPY,
                                                     MC_DEF_LOG_ACC_DPY),
                        ...
                        if (!XdmcpAllocARRAY8(&display->clientAddress,
                                              hostent->h_length)) {
                                free((char *)d);
                                return NULL;
                        }
                        bcopy(hostent->h_addr,
                              (char *)display->clientAddress.data,
                              hostent->h_length);
...
int ScanAccessDatabase(void) {
        ...
        if (accessFile && strlen(accessFile) > 0) {
                ...
                if (!datafile) {
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_ACC_CTL,
                                             MC_DEF_LOG_ACC_CTL),
                                 accessFile);
                        return 0;
                }
...
ARRAY8Ptr getLocalAddress(void) {
        ...
        if (!haveLocalAddress) {
                ...
                bcopy(hostent->h_addr, (char *)localAddress.data,
                      hostent->h_length);
        }
        return &localAddress;
}
        ...
        /*
         * calls the given function for each valid indirect entry.  Returns TRUE
         * if the local host exists on any of the lists, else FALSE
         */
        ...
        for (; h; h = h->next) {
                ...
                case HOST_ALIAS:
                        if (indirectAlias(h->entry.aliasName, clientAddress,
                                          connectionType, function, closure,
                                          depth, broadcast))
                                haveLocalhost = 1;
                        break;
                case HOST_ADDRESS:
                        if (XdmcpARRAY8Equal(getLocalAddress(),
                                             &h->entry.hostAddress))
                                haveLocalhost = 1;
                        else if (function)
                                (*function)(connectionType,
                                            &h->entry.hostAddress, closure);
                        break;
                case HOST_BROADCAST:
                        if (broadcast) {
                                ARRAY8 temp;
...
static int indirectAlias(char *alias, ARRAY8Ptr clientAddress,
        ...
        for (d = database; d; d = d->next) {
                if (d->type != DISPLAY_ALIAS ||
                    !patternMatch(alias, d->entry.aliasName))
                        continue;
...
int ForEachMatchingIndirectHost(ARRAY8Ptr clientAddress,
        ...
        for (d = database; d; d = d->next) {
                ...
                case DISPLAY_PATTERN:
                        if (!clientName)
                                clientName = NetworkAddressToHostname(
                                    connectionType, clientAddress);
                        if (!patternMatch(clientName, d->entry.displayPattern))
                                continue;
                        break;
                case DISPLAY_ADDRESS:
                        if (d->entry.displayAddress.connectionType !=
                                connectionType ||
                            !XdmcpARRAY8Equal(
                                &d->entry.displayAddress.clientAddress,
                                clientAddress)) {
                                continue;
                        }
                        break;
...
char *BypassLogin(char *displayName) {
        ...
        if (!NameToNetworkAddress(displayName, &connectionType,
