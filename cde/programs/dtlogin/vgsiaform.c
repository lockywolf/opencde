...
void SiaForm(SiaFormInfo *siaFormInfo) {
        ...
        XmString xmstr;
        XtTranslations noecho_trans, text_trans, activate_trans,
            activatePBtrans;
         ...
         * XmCreatePushButton
         *		PB callback will collect answers, send them down the
         *pipe and destroy the sia_form.
         */
        ...
        prompts_row_col =
            XmCreateRowColumn(sia_form, "prompts_row_col", argt, i);
        XtManageChild(prompts_row_col);

        prompt_labels =
            (Widget *)alloca(sizeof(Widget *) * siaFormInfo->num_prompts);
        ...
        for (j = 0; j < siaFormInfo->num_prompts; j++) {
                ...
                case SIAMENUONE: {
                        ...
                        prompt_answers[j] = XmCreateToggleButtonGadget(
                            prompts_row_col, answer_name, argt, i);
                } break;
                ...
                case SIAFORM: {
                        ...
                        prompt_labels[j] =
                            XmCreateLabel(prompts_row_col, label_name, argt, i);
                        ...
                        prompt_answers[j] = XmCreateTextField(
                            prompts_row_col, answer_name, argt, i);
                        ...
                        if (!siaFormInfo->visible[j]) {
                                 ...
                                 */
                                siaFormInfo->answers[j] =
                                    XtMalloc(SIAMXPASSWORD + 1);
                                (siaFormInfo->answers[j])[0] = '\0';

                                XtAddCallback(prompt_answers[j],
                                              XmNmodifyVerifyCallback, NoEchoCB,
                                              siaFormInfo);
                                XtAddCallback(prompt_answers[j],
                                              XmNmotionVerifyCallback, NoEchoCB,
                                              siaFormInfo);

                                /* handle Delete/Backspace in the no-echo case
                                 */
                                XtOverrideTranslations(prompt_answers[j],
                                                       noecho_trans);
                        }
        ...
        if (siaFormInfo->rendition == SIAONELINER ||
            siaFormInfo->rendition == SIAFORM) {
                ...
                XtOverrideTranslations(
                    prompt_answers[siaFormInfo->num_prompts - 1],
                    activate_trans);
                XtAddCallback(prompt_answers[siaFormInfo->num_prompts - 1],
                              XmNactivateCallback, RespondSiaFormCB,
                              siaFormInfo);
                XSetInputFocus(XtDisplay(prompt_answers[0]),
                               XtWindow(prompt_answers[0]), RevertToPointerRoot,
                               CurrentTime);
                ...
                if (siaFormInfo->rendition == SIAMENUONE ||
                    siaFormInfo->rendition == SIAMENUANY) {
                        if (XmToggleButtonGadgetGetState(
                                siaFormInfo->answer_widgets[i]))
                                siaFormInfo->answers[i] = "X";
                        else
                                siaFormInfo->answers[i] = NULL;
                } else if (siaFormInfo->visible[i])
                        siaFormInfo->answers[i] = XmTextFieldGetString(
                            siaFormInfo->answer_widgets[i]);
                if (!siaFormInfo->answers[i] ||
                    (siaFormInfo->answers[i])[0] == '\0') {
                        *answer_ptr = '\0';
                        *answer_ptr++;
                        r->hdr.length++;
                } else {
