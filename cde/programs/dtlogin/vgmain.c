...
int main(int argc, char **argv) {
        ...
        if (dpyinfo.DtXineramaInfo == NULL) { /* No xinerama, no joy. */
                ...
                fprintf(stderr,
                        "### JET VGMAIN: Xinerama available, scrns = %d\n",
...
static Widget InitToolKit(int argc, char **argv) {
        ...
        if (setjmp(syncJump)) {
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_DPYINIT,
                                     MC_DEF_LOG_NO_DPYINIT),
                         dpyinfo.name);
                exit(NOTIFY_RESTART);
        }
...
static void MakeBackground(void) {
        ...
        login_shell = XtCreatePopupShell(
            "login_shell", transientShellWidgetClass, toplevel, argt, i);
...
static void MakeAltDtButtons(void) {
        ...
        if (appInfo.altDts > 0) {
                if ((alt_dts = (Widget *)calloc(appInfo.altDts,
                                                sizeof(Widget))) == NULL)
                        LogError(ReadCatalog(MC_ERROR_SET, MC_NO_MEMORY,
                                             MC_DEF_NO_MEMORY),
                                 dpyinfo.name);
                ...
                for (i = 0; i < appInfo.altDts; ++i) {
                        ...
                        sprintf(altdtkeyclass, "%s%d", "Dtlogin*AltDtKey",
                                i + 1);

                        sprintf(altdtstart, "%s%d", "Dtlogin*altDtStart",
                                i + 1);
                        sprintf(altdtstartclass, "%s%d", "Dtlogin*AltDtStart",
                                i + 1);

                        if (XrmGetResource(XtDatabase(dpyinfo.dpy), altdtkey,
                                           altdtkeyclass, &keyrmtype,
                                           &keyrmvalue) == True) {
                                /*
                                ...
                                if (stat(temp_p, &statb) == 0) {
                                        ...
                                        sprintf(altdtlogo, "%s%d",
                                                "Dtlogin*altDtLogo", i + 1);
                                        sprintf(altlogoclass, "%s%d",
                                                "Dtlogin*AltDtLogo", i + 1);
                                        if (XrmGetResource(
                                                XtDatabase(dpyinfo.dpy),
                                        ...
                                        alt_dts[i] = XmCreateToggleButtonGadget(
                                            session_menu, rmvalue.addr, argt,
                                            j);
                                        XmStringFree(xmstr);
                                        XtAddCallback(
                                            alt_dts[i], XmNvalueChangedCallback,
                                        ...
                                        LogError(
                                            (unsigned char *)"Couldn't find "
                                                             "the keyfile \n");
                        } else
                                LogError((unsigned char *)"Couldn't find the "
                                                          "altkeyfile resource "
                                                          "in the database\n");
                }
        }
        ...
        i++;
        options_failsafe = XmCreateToggleButtonGadget(
            session_menu, "options_failsafe", argt, i);
        XmStringFree(xmstr);
...
static void MakeButtons(void) {
        ...
        i -= 1;
        xmstr =
            ReadCatalogXms(MC_LABEL_SET, MC_CLEAR_LABEL, MC_DEF_CLEAR_LABEL);
        XtSetArg(argt[i], XmNlabelString, xmstr);
        i++;

        clear_button =
            XmCreatePushButtonGadget(matte1, "clear_button", argt, i);
        ...
        XtSetArg(argt[i], XmNbottomPosition, 95);
        i++;
        xmstr = ReadCatalogXms(MC_LABEL_SET, MC_OPTIONS_LABEL,
        ...
        XtOverrideTranslations(
            options_button, XtParseTranslationTable(" <Key> Return: Select()"));
...
void MakeDialog(DialogType dtype) {
        ...
        if (!_DtXineramaGetScreen(
                dpyinfo.DtXineramaInfo, appInfo.xineramaPreferredScreen,
                &dpwidth, &dpheight, &xorg,
        ...
        cancel =
            ReadCatalogXms(MC_LABEL_SET, MC_CANCEL_LABEL, MC_DEF_CANCEL_LABEL);
        ...
        case help:
                ...
                xmstr = ReadCatalogXms(MC_LABEL_SET, MC_HELP_LABEL,
                                       MC_DEF_HELP_LABEL);
                XtSetArg(argt[i], XmNmessageString, xmstr);
                i++;
                ...
                XtSetArg(argt[i], XmNvalue,
                         ReadCatalog(MC_HELP_SET, MC_HELP, MC_DEF_HELP));
                i++;
        ...
        case copyright:
                if ((fp = fopen(COPYRIGHT, "r")) == NULL)
                        ...
                        xmstr = XmStringCreate(
                            "Cannot open copyright file '/usr/X11/copyright'.",
                            XmFONTLIST_DEFAULT_TAG);
#else
                        xmstr = XmStringCreate(
                            "Cannot open copyright file '/etc/copyright'.",
                            XmFONTLIST_DEFAULT_TAG);
#endif
                else {
                        xmstr = (XmString)NULL;
                        ...
                        while (fgets(buffer, 128, fp) != NULL) {
                                ...
                                if (xmstr != NULL)
                                        xmstr = XmStringConcat(
                                            xmstr, XmStringSeparatorCreate());
        ...
        case hostname:
                ...
                sv = ReadCatalogXms(MC_LABEL_SET, MC_START_LABEL,
                                    MC_DEF_START_LABEL);
...
static void MakeGreeting(void) {
        ...
        */
        ...
        strncpy(disp,
                dpyinfo.name ? dpyinfo.name : (DisplayString(dpyinfo.dpy)),
                127);
        disp[127] = '\0';
        p = strchr(disp, '.');
        t = strchr(disp, ':');
        ...
        if (greetInfo.persLabelString &&
            strcmp(greetInfo.persLabelString, "default") == 0) {
                const char *msg;
                msg = (const char *)ReadCatalog(
                    MC_LABEL_SET, MC_PERS_GREET_LABEL, MC_DEF_PERS_GREET_LABEL);
        ...
        if (greetInfo.labelString &&
            strcmp(greetInfo.labelString, "default") == 0) {
                ...
                xmstr = ReadCatalogXms(MC_LABEL_SET, MC_GREET_LABEL,
                                       MC_DEF_GREET_LABEL);
                xmstr = XmStringConcat(
                    xmstr, XmStringCreate(" ", XmFONTLIST_DEFAULT_TAG));
                xmstr = XmStringConcat(
                    xmstr, XmStringCreate(host, XmFONTLIST_DEFAULT_TAG));
        } else {
                /*
                 *  scan user's message for %LocalHost% token. Replace with
                 * hostname if found...
                 */
                ...
                do {
                        ...
                        /*
                         *  scan for a new line character in remaining label
                         * string. If found, work with that substring first...
                         */
                        ...
                        /*
                         *  replace all occurrances of %LocalHost% and
                         * %DisplayName% in the current substring...
                         */
                        ...
                        while (1) {
                                ...
                                if (p && t) { /* both present? do whichever
                                                 comes first */
                                        if (p > t)
                                                p = NULL;
                                        else
                                ...
                                } else if (t) { /* replace a %DisplayName%
                                                   string */
                                        p = t;
                                        t = disp;
                                ...
                                *p = '\0';
                                xmstr = XmStringConcat(
                                    xmstr,
                                    XmStringCreate(q, XmFONTLIST_DEFAULT_TAG));
                                xmstr = XmStringConcat(
                                    xmstr,
                                    XmStringCreate(t, XmFONTLIST_DEFAULT_TAG));
                                q = p + skip - 1;
                        }
                        ...
                        /*
                         *  add a line seperator if this is a multi-line
                         * greeting...
                         */
...
static void MakeLogin(void) {
        ...
        */
        ...
        XtOverrideTranslations(
            login_text,
            XtParseTranslationTable(" Shift <Key>Tab: prev-tab-group() \n\
				   Ctrl <Key>Tab: next-tab-group() "));
    ...
    XtSetArg(argt[i], XmNtranslations,          textTable               ); i++;
        ...
        */
        ...
        xmstr =
            ReadCatalogXms(MC_LABEL_SET, MC_LOGIN_LABEL, MC_DEF_LOGIN_LABEL);
        XtSetArg(argt[i], XmNlabelString, xmstr);
        i++;
...
void MakeOptionsMenu(void) {
        ...
        /*
         *  build [ Language ] menu pane if there are languages to choose
         * from...
         */
        if (lang_menu != NULL) {
                /*
                 ...
                 */
                i = InitArg(CascadeBG);
                xmstr = ReadCatalogXms(MC_LABEL_SET, MC_LANG_LABEL,
                                       MC_DEF_LANG_LABEL);
                XtSetArg(argt[i], XmNlabelString, xmstr);
                i++;
                ...
                XtSetArg(argt[i], XmNrecomputeSize, True);
                i++;
                options_item[j] = XmCreateCascadeButtonGadget(
                    options_menu, "options_languages", argt, i);
                XmStringFree(xmstr);
                j++;
                ...
                options_item[j] = XmCreateSeparatorGadget(
                    options_menu, "options_sep2", argt, i);
                j++;
        }

        if (session_menu == NULL) {
                session_menu =
                    XmCreatePulldownMenu(options_menu, "session_menu", NULL, 0);
                ...
                xmstr =
                    ReadCatalogXms(MC_LABEL_SET, MC_DT_LABEL, MC_DEF_DT_LABEL);
                XtSetArg(argt[i], XmNlabelString, xmstr);
                i++;
                XtSetArg(argt[i], XmNrecomputeSize, True);
                i++;
                 ...
                 */
                i = k;
                xmstr = ReadCatalogXms(MC_LABEL_SET, MC_DTLITE_LABEL,
                                       MC_DEF_DTLITE_LABEL);
                XtSetArg(argt[i], XmNlabelString, xmstr);
                i++;
                XtSetArg(argt[i], XmNrecomputeSize, True);
                ...
                options_dtlite = XmCreateToggleButtonGadget(
                    session_menu, "options_dtlite", argt, i);
                XmStringFree(xmstr);
                XtAddCallback(options_dtlite, XmNvalueChangedCallback,
                              MenuItemCB, (XtPointer)OB_DTLITE);
        }
        ...
        if (session_menu != NULL) {
                 ...
                 */
                i = InitArg(CascadeBG);
                xmstr = ReadCatalogXms(MC_LABEL_SET, MC_SES_LABEL,
                                       MC_DEF_SES_LABEL);
                XtSetArg(argt[i], XmNlabelString, xmstr);
                i++;
                ...
                i++;
                options_item[j] = XmCreateCascadeButtonGadget(
                    options_menu, "session_menus", argt, i);
                XmStringFree(xmstr);
                j++;
                ...
                i = InitArg(SeparatorG);
                options_item[j] = XmCreateSeparatorGadget(
                    options_menu, "options_sep1", argt, i);
                j++;
        }
        ...
        XtSetArg(argt[i], XmNlabelString, xmstr);
        i++;
        options_item[j] = options_nowindows = XmCreatePushButtonGadget(
            options_menu, "options_noWindows", argt, i);
        XmStringFree(xmstr);
        XtAddCallback(options_item[j], XmNactivateCallback, MenuItemCB,
                      (XtPointer)OB_NO_WINDOWS);
        ...
        XtSetArg(argt[i], XmNlabelString, xmstr);
        i++;
        options_item[j] = XmCreatePushButtonGadget(
            options_menu, "options_restartServer", argt, i);
        XmStringFree(xmstr);
        XtAddCallback(options_item[j], XmNactivateCallback, MenuItemCB,
        ...
        i++;
        options_item[j] = XmCreatePushButtonGadget(
            options_menu, "options_copyright", argt, i);
        XmStringFree(xmstr);
        XtAddCallback(options_item[j], XmNactivateCallback, MenuItemCB,
         ...
         *  other code (ex. MenuItemCB()) tries to obtain some of their
         * resources.
         */
...
static void MyBackspace(Widget w, XEvent *event, char **params,
        ...
        if (textdata && !textdata->bEcho &&
            (int)strlen(textdata->noechobuf) > 0) {
