...
#endif /* NeedWidePrototypes */
{
        ...
        for (d = displays; d; d = d->next)
                if (d->displayType.origin == FromXDMCP &&
                    ...
                    addressEqual((char *)d->from, d->fromlen, (char *)addr,
                                 addrlen)) {
                        return d;
                }
        return 0;
...
void RemoveDisplay(struct display *old) {
        ...
        for (d = displays; d; d = d->next) {
                if (d == old) {
                        ...
                        /*
                         *  turn off polling if we are removing a suspended
                         * display...
                         */
...
struct display *NewDisplay(char *name, char *class) {
        ...
        if (!d) {
                LogOutOfMem(ReadCatalog(MC_LOG_SET, MC_LOG_NEW_DPY,
                                        MC_DEF_LOG_NEW_DPY));
                return 0;
        }
        d->next = displays;
