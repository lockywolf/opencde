...
int Verify(struct display *d, struct greet_info *greet,
        ...
        if (d->bypassLogin) {
                ...
                if (!strcmp(p->pw_name, "Nobody"))
                        ...
                        s = "Greetless login failed; bad user home dir "
                            "access\n";
                else if (p->pw_uid == 0)
        ...
        case NOTIFY_LAST_DT:
                ...
                if ((lastsession = fopen(last_sess, "r")) == NULL) {
                        ...
                        if ((argv = setDt(d, argv, notify_dt)) == NULL)
                                if (d->session) {
                                        argv = parseArgs(argv, d->session);
                                        /* No altdts, set the Default Regular
                                         * session */
                                        Debug("No AltDestops in the resources "
                                              "file \n");
                                }
                } else {
                        strcpy(last_sess, "\0");
                        ...
                        if (access(last_sess, X_OK) != 0) {
                                if ((argv = setDt(d, argv, notify_dt)) == NULL)
                                        if (d->session) {
                                                argv =
                                                    parseArgs(argv, d->session);
                                                /* No altdts, set the Default
                                                 * Regular session */
                                                Debug("No AltDestops in the "
                                                      "resources file setting "
                                                      "to %s  \n",
                                                      argv[0]);
                                        }
                        } else {
...
char **setDt(struct display *d, char **argv, int dt_type) {
        int i;
        char *resources = NULL;
        ...
        if (dt_type == NOTIFY_LAST_DT) {
                ...
                if (XrmGetResource(XresourceDB, altdts, altdtsclass, &rmtype,
                                   &value_dts) == True) {
                        adts = atoi(value_dts.addr);
                        for (i = 0; i < adts; i++) {
                                sprintf(altdtstart, "%s%d",
                                        "Dtlogin*altDtStart", i + 1);
                                sprintf(altdtstartclass, "%s%d",
                                        "Dtlogin*AltDtStart", i + 1);
                                if (XrmGetResource(XresourceDB, altdtstart,
                                                   altdtstartclass, &rmtype,
                                                   &rmvalue) == True) {
...
char **userEnv(struct display *d, char *user, char *home, char *shell,
               struct passwd *p) {
        char **env;
        char *value;
        char **exp;
 ...
 */
#ifdef sun
char *login_defaults(struct passwd *p, struct display *d) {
        extern int defcntl();
        register int flags;
        register char *ptr;
        char *Def_path;
...
int groupMember(char *name, char **members) {
        while (*members) {
...
void getGroups(char *name, struct verify_info *verify, int gid) {
        int ngroups;
        struct group *g;
        int i;
