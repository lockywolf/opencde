...
static SIGVAL catchTerm(int arg) { longjmp(abortSession, 1); }

static jmp_buf pingTime;

static SIGVAL catchAlrm(int arg) { longjmp(pingTime, 1); }

static int FileNameCompare(a, b)
#if defined(__STDC__)
    const void *a,
    *b;
#else
...
static void SessionPingFailed(struct display *d) {
        if (clientPid > 1) {
...
#if defined(PAM) || defined(SUNAUTH)
                {
                         ...
                         * remote accouting is enabled (networkDev start with
                         * /dev/...) Set tty line name to match network device
                         * for accouting. Unless the resource was specifically
                         * set, default is value of DEF_NETWORK_DEV define
                         * (/dev/dtremote)
                         */
...
#else
                        solaris_accounting(verify.argv[0], d->name, d->utmpId,
                                           user, ttyLine, clientPid, ACCOUNTING,
                                           NULL);
#endif
...
static int IOErrorHandler(Display *dpy) {
        ...
        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_FATAL_IO, MC_DEF_LOG_FATAL_IO),
                 errno, s);
...
void ManageSession(struct display *d) {
        ...
        if (IsVerifyName(VN_KRB)) {
                ...
                verify.systemEnviron = setEnv(verify.systemEnviron, "KRBTKFILE",
                                              krb_ticket_string);
        ...
        if (d->displayType.location == Local) {
                verify.systemEnviron =
                    setEnv(verify.systemEnviron, LOCATION, "local");
                /* ITE is needed only for Local displays */
                /* set ITE env var */
                if (d->gettyLine)
                        verify.systemEnviron =
                            setEnv(verify.systemEnviron, "ITE", d->gettyLine);
        } else
                verify.systemEnviron =
                    setEnv(verify.systemEnviron, LOCATION, "remote");

        {
                ...
                verify.systemEnviron =
                    setEnv(verify.systemEnviron, "USER_GID", gid);
                ...
                if (pwd) {
                        ...
                        verify.systemEnviron =
                            setEnv(verify.systemEnviron, "ROOT_GID", gid);
                }
        }
        ...
        if (!setjmp(abortSession)) {
                ...
                if (StartClient(&verify, d, &clientPid)) {
                        ...
                        /*
                         * We've changed dtlogin to pass HUP's down to the
                         * children so ignore any HUP's once the client has
                         * started.
                         */
                        ...
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_FAIL_START,
                                             MC_DEF_LOG_FAIL_START));
                }
        } else {
                /*
...
int LoadXloginResources(struct display *d) {
        ...
        if (d->resources && d->resources[0]) {
                ...
                if ((XresourceDB = XrmGetFileDatabase(resources)) == NULL)
                        Debug("LoadXloginResources - Loading resource db from "
                              "%s failed\n",
                              resources);
                ...
                sprintf(cmd, "%s%s %s -display %s -load %s", auth_key,
                        authority, d->xrdb, d->name, tmpname);
...
static void LoadAltDtsResources(struct display *d) {
        ...
        if (XrmGetResource(XresourceDB, "Dtlogin*altDts", "Dtlogin*AltDts",
                           &rmtype, &rmvalue)) {
                        ...
                        while ((dp = readdir(dirp)) != NULL) {
                                ...
                                if ((strcmp(dp->d_name, DOT) != 0) &&
                                    (strcmp(dp->d_name, DOTDOT) != 0)) {
                                        ...
                                        sprintf(res_file, "%s%s", dirname[j],
                                                dp->d_name);
                                        if ((access(res_file, R_OK)) != 0) {
                                                Debug("LoadAltDtsResources- "
                                                      "cant access %s.\n",
                                                      resources);
                                                Debug("\t %s.\n",
                                                      strerror(errno));
        ...
        for (j = 0; j < file_count; j++) {
                ...
                if (XrmGetResource(XresourceDB, "Dtlogin*altDtsIncrement",
                                   "Dtlogin*AltDtsIncrement", &rmtype,
                                   &rmvalue)) {
                        ...
                        if ((strcmp(tempbuf, "True") == 0) ||
                            (strcmp(tempbuf, "TRUE") == 0)) {
                                ...
                                if (XrmGetResource(XresourceDB,
                                                   "Dtlogin*altDtKey",
                                                   "Dtlogin*AltDtKey", &rmtype,
                                                   &rmvalue)) {
                                        ++i;
                                        sprintf(altdtres, "Dtlogin*altDtKey%d",
                                                i);
                                        XrmPutStringResource(&XresourceDB,
                                                             altdtres,
                                                             rmvalue.addr);
                                } else
                                        continue;
                                ...
                                if (XrmGetResource(XresourceDB,
                                                   "Dtlogin*altDtName",
                                                   "Dtlogin*AltDtName", &rmtype,
                                                   &rmvalue)) {
                                        sprintf(altdtres, "Dtlogin*altDtName%d",
                                                i);
                                        XrmPutStringResource(&XresourceDB,
                                                             altdtres,
                                                             rmvalue.addr);
                                ...
                                if (XrmGetResource(XresourceDB,
                                                   "Dtlogin*altDtLogo",
                                                   "Dtlogin*AltDtLogo", &rmtype,
                                                   &rmvalue)) {
                                        sprintf(altdtres, "Dtlogin*altDtLogo%d",
                                                i);
                                        XrmPutStringResource(&XresourceDB,
                                                             altdtres,
                                                             rmvalue.addr);
                ...
                if ((pch = strchr(pch, '%')) != NULL) {
                        ...
                        if (pch == NULL) {
                                ...
                                if (lang_len == 0) {
                                        ...
                                        /*
                                         * Remove the "%L" and then append the
                                         * LANG.
                                         */
                                        (void)strncat(tmp, trail,
                                                      (pch - trail) - 1);
...
#define NOPAG 0xffffffff
        ...
        case 0:
                 ...
                 * SIA has already attempted to chdir to HOME, and the current
                 * dir will be set to / if it failed. We just check to see if
                 * the HOME path is our current directory or not.
                 */
	 ...
	 */
        if (strcmp(home, currentdir)) {
            ...
            verify->userEnviron = setEnv(verify->userEnviron, "HOME", "/");
                 ...
                 * without execing. This allows us to set up the process
                 * environment and proceed to the execute() call as do the other
                 * platforms.
                 *
                 ...
                 * must be reflected here by adjusting SYS_ENV_TAG or
                 * USR_ENV_TAG.
                 */
...
#define USR_ENV_TAG 3
...
#ifdef _POWER
                {
                        ...
                        /*
                         * Save pointers to tags. The setpenv() function clears
                         * the pointers to the tags in userEnviron as a
                         * side-effect.
                         */
                        ...
                        /*
                         * Set the users process environment. Store protected
                         * variables and obtain updated user environment list.
                         * This call will initialize global 'newenv'.
                         */
#define SESSION_PENV (PENV_INIT | PENV_ARGV | PENV_NOEXEC)
                        if (setpenv(user, SESSION_PENV, verify->userEnviron,
                                    NULL) != 0) {
                                Debug(
                                    "Can't set process environment (user=%s)\n",
                                    user);
                                return (0);
                        }
                        ...
                        /*
                         * Free old userEnviron and replace with newenv from
                         * setpenv().
                         */
...
#if defined(PAM)
                if (PamSetCred(verify->argv[0], user, verify->uid,
                               verify->gid) > 0) {
                        Debug("Can't set User's Credentials (user=%s)\n", user);
                        return (0);
                }
...
#ifdef SUNAUTH
                if (solaris_setcred(verify->argv[0], user, verify->uid,
                                    verify->gid) > 0) {
                        Debug("Can't set User's Credentials (user=%s)\n", user);
                        return (0);
                }
                ...
                if (ISSECURE) {
                        Debug("BLS - Setting user's clearance, security level "
                              "and luid.\n");
                        ...
                        if (verify_user_seclevel(verify, sensitivityLevel) !=
                            1) {
                                Debug("BLS - Could not verify sensitivity "
                                      "level.\n");
                                LogError(ReadCatalog(MC_LOG_SET,
                                                     MC_LOG_NO_VFYLVL,
                                                     MC_DEF_LOG_NO_VFYLVL));
                                exit(1);
                        }
                        ...
                        /*
                         * This should never fail since everything has been
                         * verified already. If it does it must mean registry
                         * strangeness, so exit, and try again...
                         */

                        if (!DoLogin(user, greet.password, d->name))
                                exit(1);

                        /*
                         * extract the SYSTYPE and ISP environment values and
                         * set into user's environment. This is necessary since
                         * we do an execve below...
                         */

                        verify->userEnviron = setEnv(
                            verify->userEnviron, "SYSTYPE", getenv("SYSTYPE"));

                        verify->userEnviron =
                            setEnv(verify->userEnviron, "ISP", getenv("ISP"));

#else /* ! __apollo */

#ifdef __AFS
                if (IsVerifyName(VN_AFS)) {
                        pagval = get_pag_from_groups(verify->groups[0],
                                                     verify->groups[1]);
                        Debug("AFS - get_pag_from_groups() returned pagval = "
                              "%d\n",
                              pagval);
                        ...
                        if ((pagval != NOPAG) &&
                            (get_pag_from_groups(groups[0], groups[1])) ==
                                NOPAG) {
                                /* we will have to shift grouplist to make room
                                 * for pag */
                                ...
                                ngroups += 2;
                                get_groups_from_pag(pagval, &groups[0],
                                                    &groups[1]);
                                if (setgroups(ngroups, groups) == -1) {
                                        LogError(ReadCatalog(
                                            MC_LOG_SET, MC_LOG_AFS_FAIL,
                                            MC_DEF_LOG_AFS_FAIL));
                                        exit(1);
                                }
                        }
...
/* setpenv() will set uid for AIX */
                ...
                if (setuid(verify->uid) != 0) {
                        Debug("Setuid failed for user %s, errno = %d\n", user,
                              errno);
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_FAIL_SETUID,
                                             MC_DEF_LOG_FAIL_SETUID),
                                 user, errno);
                        exit(1);
                }
#endif
                ...
                if (home) {
                        if (chdir(home) == -1) {
                                LogError(
                                    ReadCatalog(MC_LOG_SET, MC_LOG_NO_HMDIR,
                                                MC_DEF_LOG_NO_HMDIR),
                                    home, getEnv(verify->userEnviron, "USER"));
                                if (-1 == chdir("/")) {
                                        perror(strerror(errno));
                                ...
                                verify->userEnviron =
                                    setEnv(verify->userEnviron, "HOME", "/");
                        } else if (!failsafe) {
                                strcpy(lastsessfile,
                                       home); /* save user's last session */
                                strcat(lastsessfile, LAST_SESSION_FILE);
                                if ((lastsession = fopen(lastsessfile, "w")) ==
                                    NULL)
                                        Debug("Unable to open file for "
                                              "writing: %s\n",
                                              lastsessfile);
                                else {
                 ...
                 * The file name should really be settable by some kind of
                 * resource but time is short so we hard-wire it to
                 * ".dtlogininfo".
                 */
                ...
                if (verify->argv) {
                        if (!failsafe) {
                                ...
                                LogError(ReadCatalog(MC_LOG_SET,
                                                     MC_LOG_SES_EXEFAIL,
                                                     MC_DEF_LOG_SES_EXEFAIL),
                                         verify->argv[0]);
                        }
                } else {
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_CMDARG,
                                             MC_DEF_LOG_NO_CMDARG));
                }
        ...
        case -1:
                ...
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_SESFORK,
                                     MC_DEF_LOG_NO_SESFORK),
                         d->name);
                return 0;
        default:
...
static SIGVAL waitAbort(int arg) { longjmp(tenaciousClient, 1); }
...
int source(struct verify_info *verify, char *file) {
        ...
        if (file && file[0]) {
                ...
                case 0:
                        ...
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_EXE,
                                             MC_DEF_LOG_NO_EXE),
                                 args[0]);
                        exit(1);
                case -1:
                        Debug("Source(): fork failed\n");
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_FORK,
                                             MC_DEF_LOG_NO_FORK),
                                 file);
                        return 1;
                        break;
...
int execute(char **argv, char **environ) {
        ...
        if (errno == ENOEXEC) {
                ...
                for (av = argv, argc = 0; *av; av++, argc++)
                        ;
                newargv =
                    (char **)malloc((argc + (optarg ? 3 : 2)) * sizeof(char *));
                if (!newargv)
                        return -1;
                av = newargv;
                *av++ = p;
         ...
         *    RunGreeter
         *
         *    Invoke the Greeter process and wait for completion. If the user
         *was successfully verified, return to the calling process. If the user
         *    selected a restart or abort option, or there was an error invoking
         *the Greeter, exit this entire process with appropriate status.
         *
         *****************************************************************************/
...
#define U_NAMELEN sizeof(rgy_$name_t)
        ...
        if (!setjmp(abortSession)) {
                ...
                case 0:
                        ...
                        if (d->pmSearchPath)
                                env = setEnv(env, "XMICONSEARCHPATH",
                                             d->pmSearchPath);

                        if (d->bmSearchPath)
                                env = setEnv(env, "XMICONBMSEARCHPATH",
                                             d->bmSearchPath);
                        ...
                        if (d->verifyName) {
                                if ((strcmp(d->verifyName, VN_AFS) == 0) ||
                                    (strcmp(d->verifyName, VN_KRB) == 0)) {
                                        ...
                                        env = setEnv(env, VERIFYNAME,
                                                     d->verifyName);
                                } else {
                                        LogError(
                                            ReadCatalog(MC_LOG_SET,
                                                        MC_LOG_IMPROP_AUTH,
                        ...
                        /*
                         * Writing to file descriptor 1 goes to response pipe
                         * instead.
                         */
                        close(1);
                        ...
                        /*
                         * Reading from file descriptor 0 reads from request
                         * pipe instead.
                         */
                        close(0);
                        ...
                        execle(msg, "dtgreet", "-display", d->name, (char *)0,
                               env);
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_DTGREET,
                                             MC_DEF_LOG_NO_DTGREET),
                                 msg, d->name);
                        exit(NOTIFY_ABORT_DISPLAY);

                case -1:
                        Debug("Fork of Greeter failed.\n");
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_FORKCG,
                                             MC_DEF_LOG_NO_FORKCG),
                                 d->name);
                        close(request[0]);
                        close(request[1]);
                        close(response[0]);
                ...
                default:
                        ...
                        close(
                            response[1]); /* Close write end of response pipe */
                        ...
                        /*
                         * atexit() registers this function to be called if
                         * exit() is called.  This is needed because in enhanced
                         * security mode, SIA may call exit() whn the user fails
                        ...
                        while (siaStatus != SIASUCCESS) {
                                while (siaStatus != SIASUCCESS &&
                                       siaGreeterInfo.status) {
                                        Debug("RunGreeter: before "
                                              "sia_ses_authent\n");
                                        dt_in_sia_ses_authent = True;
                                        siaStatus = sia_ses_authent(
                                            SiaManageGreeter, NULL, siaHandle);
                                        dt_in_sia_ses_authent = False;
                                        Debug("RunGreeter: after "
                                              "sia_ses_authent status = %d\n",
                                              siaStatus);
                                        if (siaStatus == SIAFAIL &&
                                            siaGreeterInfo.status) {
                                                state.id =
                                                    GREET_STATE_ERRORMESSAGE;
                                                state.vf = VF_INVALID;
                                                ManageGreeter(d, greet, verify,
                                                              &state);
                                        }
                                ...
                                siaStatus =
                                    sia_ses_estab(SiaManageGreeter, siaHandle);
                                Debug("RunGreeter: after sia_ses_estab status "
                                      "= %d\n",
                                      siaStatus);
                                ...
                                if (siaStatus == SIASUCCESS) {
                                        Debug("RunGreeter: before "
                                              "sia_ses_launch\n");
                                        siaStatus = sia_ses_launch(
                                            SiaManageGreeter, siaHandle);
                                        Debug("RunGreeter: after "
                                              "sia_ses_launch status = %d\n",
                                              siaStatus);
                                }
                                if (!siaGreeterInfo.status)
                                        break;
                                ...
                                if (siaStatus != SIASUCCESS) {
                                        Debug("RunGreeter: sia_ses_launch "
                                              "failure\n");
                                        /* establish & launch failures do a
                                         * release */
                                        siaHandle = NULL;
                                        siaStatus = sia_ses_init(
                                            &siaHandle, argc, argv, hostName,
                                            loginName, d->name, 1, NULL);
                                        if (siaStatus != SIASUCCESS) {
                                                Debug("sia_ses_init failure "
                                                      "status %d\n",
                                                      siaStatus);
                                                exit(RESERVER_DISPLAY);
                                        }
                        ...
                        if (waitVal(status) > NOTIFY_ALT_DTS)
                                notify_dt =
                                    NOTIFY_ALT_DTS; /* It is alt desktops */
                        else
                        ...
                        case NOTIFY_ALT_DTS:
                                ...
                                /*
                                 *  greet->name, greet->password set in
                                 * ManageGreeter().
                                 */
                                ...
                                if (IsVerifyName(VN_AFS)) {
                                        ...
                                        Debug(
                                            "Greeter returned groups[0] '%d'\n",
                                            verify->groups[0]);
                                        Debug(
                                            "Greeter returned groups[1] '%d'\n",
                                            verify->groups[1]);
                                }
#endif
                        ...
                        case NOTIFY_NO_WINDOWS:
                                ...
                                if (d->serverPid >= 2)
                                        /*
                                         *  Don't do a SessionExit() here since
                                         * that causes the X-server to be reset.
                                         * We know we are going to terminate it
                                         * anyway, so just go do that...
                                         */
                        ...
                        case NOTIFY_LANG_CHANGE:
                                ...
                                /*
                                 * copy requested language into display struct
                                 * "d". Note, this only happens in this child's
                                 * copy of "d", not in the master struct. When
                                 * the user logs out, the resource-specified
                                 * language (if any) will reactivate.
                                 */
                                if (d->language)
                                        Debug(
                                            "Greeter returned language '%s'\n",
                                            d->language);
                                else
                                        Debug("Greeter returned language "
                                              "(NULL)\n");
        ...
        * ManageGreeter
          ...
          enter -> ENTER:
            This is the entry into greeter state processing. Allocate and
        initialize state structure.

          ENTER -> LOGIN:
            Display the login screen. Upon display, the login screen can be
        'reset'. If reset is true, the username and password fields are cleared
        and the focus is set to the username field. If reset is false, the
        username and password field is untouched and the focus is set to the
        password field.
...
static int ManageGreeter(struct display *d, struct greet_info *greet,
        ...
        if (state->waitForResponse) {
                if (!AskGreeter(NULL, (char *)state->response,
                                REQUEST_LIM_MAXLEN)) {
                        /*
                ...
                if (state->request->opcode != state->response->opcode) {
                        ...
                        case REQUEST_OP_LANG: {
                                /*
                                 * User has changed language. Recursively handle
                                 * this state and return to current state.
                                 */
        ...
        case GREET_STATE_AUTHENTICATE: {
                 ...
                 */
                state->vf =
                    Authenticate(d, greet->name, greet->password, &state->msg);
        ...
        case GREET_STATE_ERRORMESSAGE: {
                ...
                case VF_NO_LOGIN:
                        ...
                        break;
#ifdef BLS
                case VF_BAD_SEN_LEVEL:
                        SETMC(msg, BAD_SEN_LEVEL);
                        break;
#endif
                case VF_MESSAGE:
                        msg.id = 0;
        ...
        case GREET_STATE_LANG: {
                ...
                d->language = (d->language == NULL ? malloc(len)
                                                   : realloc(d->language, len));
        ...
        case GREET_STATE_FORM: {
                ...
                AskGreeter(state->request, (char *)state->response,
                           REQUEST_LIM_MAXLEN);
...
static int AskGreeter(RequestHeader *preqhdr, char *buf, int blen) {
        ...
        if (count == sizeof(*phdr)) {
                ...
                if (remainder > 0) {
                         ...
                         */
                        count +=
                            read(response[0], buf + sizeof(*phdr), remainder);
                }
        }
...
static void SetTicketFileName(uid_t uid) {
        ...
        if (env = (char *)getenv("KRBTKFILE")) {
                ...
                if (gethostname(lhost, sizeof(lhost)) != -1) {
                        if (p = index(lhost, '.'))
                                *p = '\0';
                        (void)sprintf(krb_ticket_string, "%s%ld.%s", TKT_ROOT,
                                      (long)uid, lhost);
                } else {
                        /* 32 bits of signed integer will always fit in 11
                           characters (including the sign), so no need to worry
                           about overflow */
                        (void)sprintf(krb_ticket_string, "%s%ld", TKT_ROOT,
                                      (long)uid);
                }
        }
}
...
static int session_execve(char *path, char *argv[], char *envp[]) {
        ...
        if (user == NULL) {
                ...
                /*
                 * Save pointers to tags. The setpenv() function clears the
                 * pointers to the tags in userEnviron as a side-effect.
                 */
                 ...
                 * should not return. It it should return, restore the envp
                 * tags.
                 */
        ...
        case SIAFORM:
                if (rendition == SIAFORM && dt_in_sia_ses_authent &&
                    (num_prompts == 2)) {
                        ...
                        while (
                            siaGreeterInfo.state->id !=
                                GREET_STATE_TERMINATEGREET &&
                        ...
                        strncpy((char *)prompt[0].result,
                                siaGreeterInfo.greet->name,
                                prompt[0].max_result_length);
                        strncpy((char *)prompt[1].result,
                                siaGreeterInfo.greet->password,
                        ...
                        request_form->offPrompts = sizeof(RequestForm) +
                                                   strlen((const char *)title) +
                                                   1;
                        strcpy((char *)request_form + request_form->offTitle,
                               (const char *)title);

                        pmpt_ptr =
                            (char *)request_form + request_form->offPrompts;
                        for (i = 0; i < num_prompts; i++) {
                                if (!prompt[i].prompt ||
                                    prompt[i].prompt[0] == '\0')
                                        *pmpt_ptr++ = '\0';
                                        ...
                                        Debug("  prompt[%d]: %s\n", i,
                                              prompt[i].prompt);
                                        strcpy(pmpt_ptr,
                                               (const char *)prompt[i].prompt);
                                        pmpt_ptr += strlen(
                                            (const char *)prompt[i].prompt);
                                }
                                request_form->visible[i] =
                                    (prompt[i].control_flags & SIARESINVIS)
                                        ? False
                                        : True;
                        }
                        ...
                        response_form =
                            (ResponseForm *)siaGreeterInfo.state->response;
                        res_ptr =
                            (char *)response_form + response_form->offAnswers;
                        for (i = 0; i < response_form->num_answers; i++) {
                                if (rendition == SIAMENUONE ||
                                    rendition == SIAMENUANY) {
                                        if (res_ptr[0])
                                                prompt[i].result =
                                                    (unsigned char *)1;
                                        else
