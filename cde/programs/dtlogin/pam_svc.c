...
int PamAccounting(char *prog_name, char *display_name, char *entry_id,
        ...
        if (status == PAM_SUCCESS)
                ...
                case USER_PROCESS:
                        ...
                        if (status != PAM_SUCCESS) {
                                Debug("PamAccounting: USER_PROCESS "
                                      "open_session error=%d\n",
                                      status);
                        }
#ifdef legacysun
                        if (status == PAM_SUCCESS)
                        ...
                        status = solaris_setutmp_mgmt(
                            user, tty_line, display_name, session_type,
                            entry_type, entry_id);
                        if (status != SOLARIS_SUCCESS) {
                                Debug("PamAccounting: USER_PRCESS set_utmp "
                                      "error=%d\n",
                                      status);
                        }
                        break;
                ...
                case ACCOUNTING:
                        ...
                        status = pam_open_session(pamh, 0);
                        if (status != PAM_SUCCESS) {
                                Debug("PamAccounting: ACCOUNTING open_session "
                                      "error=%d\n",
                                      status);
                        }
                        ...
                        if (status != SOLARIS_SUCCESS) {
                                Debug("PamAccounting: ACCOUNTING set_utmp "
                                      "error=%d\n",
                                      status);
                        }
                ...
                case DEAD_PROCESS:
                        ...
                        if (status != PAM_SUCCESS) {
                                Debug("PamAccounting: DEAD_PROCES "
                                      "close_session error=%d\n",
                                      status);
                        }
                        ...
                        if (status != SOLARIS_SUCCESS) {
                                Debug("PamAccounting: DEAD_PROCESS reset_utmp "
                                      "error=%d\n",
                                      status);
                        }
                        break;
                ...
                default:
                        status = pam_open_session(pamh, 0);
                        if (status != PAM_SUCCESS) {
                                Debug("PamAccounting:LOGIN_PROCESS "
                                      "open_session error=%d\n",
                                      status);
                        }
                        session_type = 0;
                        status = solaris_setutmp_mgmt(
                            ...
                            entry_type, entry_id);
                        if (status != SOLARIS_SUCCESS) {
                                Debug("PamAccounting: LOGIN_PROCESS set_utmp "
                                      "error=%d\n",
                                      status);
                        }
                        break;
