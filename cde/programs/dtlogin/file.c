...
static DisplayType parseDisplayType(char *string, int *usedDefaultType,
                                    int *parse_uid);
...
static int DisplayTypeMatch(DisplayType d1, DisplayType d2) {
        return d1.location == d2.location && d1.lifetime == d2.lifetime &&
               d1.origin == d2.origin;
}
...
static char **splitIntoWords(char *s) {
        ...
        while (*s) {
                ...
                if (!args) {
                        ...
                        newargs = (char **)realloc(
                            (char *)args, (nargs + 2) * sizeof(char *));
                        if (!newargs) {
                                freeArgs(args);
                                return NULL;
                        }
...
int ParseDisplay(char *source, DisplayType *acceptableTypes, int numAcceptable,
                 struct passwd *puser) {
        char **args, **argv, **a;
        ...
        if (!args)
                return 0;
        if (!args[0]) {
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_MISS_NAME,
                                     MC_DEF_LOG_MISS_NAME));
                freeArgs(args);
                return 0;
        }
        name = strdup(args[0]);
        if (!args[1]) {
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_MISS_TYPE,
                                     MC_DEF_LOG_MISS_TYPE),
                         args[0]);
                freeArgs(args);
                free(name);
                return 0;
        ...
        if (strcmp(name, "*") == 0) {
                ...
                if ((hostent = gethostbyname(tname)) == NULL) {
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_INV_HOSTNM,
                                             MC_DEF_LOG_INV_HOSTNM),
                                 tname);
                        tname[0] = 0;
                }
        ...
        if (usedDefaultType && args[2]) {
                ...
                displayType =
                    parseDisplayType(args[2], &usedDefaultType, &parse_uid);
                if (!usedDefaultType) {
                        class = args[1];
                        type = args[2];
                        argv = args + 3;
        ...
        if (parse_uid) {
                ...
                if ((p = getpwnam(*argv)) != NULL) {
                        *puser = *p;
                        ...
                        Debug(
                            "Could not get password entry for user name '%s'\n",
                            *argv);
        ...
        if (!numAcceptable) {
                LogError(ReadCatalog(MC_LOG_SET, MC_LOG_BAD_DPYTYPE,
                                     MC_DEF_LOG_BAD_DPYTYPE),
                         type, name);
        }
        ...
        if (d) {
                ...
                Debug("Found existing display:  %s %s %s", d->name,
                      d->class ? d->class : "", type);
                freeArgs(d->argv);
        } else {
        ...
        if (d->gettyLine && (strcmp(d->gettyLine, "None") == 0 ||
