...
int DoLogin(char *user, char *passwd, char *host) {
        ios_$id_t logid;
        ...
        opts = login_$no_prompt_name_sm | login_$no_prompt_pass_sm |
               login_$addut_sm | login_$chown_tty_sm;
        ...
        if ((!login_$chk_login(opts, STRING(user), STRING(passwd),
            ...
            (status.all != status_$ok)) {
                ...
                sprintf(
                    buf,
                    ReadCatalog(MC_LOG_SET, MC_LOG_DOMAIN, MC_DEF_LOG_DOMAIN),
