 ...
 */
int verify_user_seclevel(struct verify_info verify, char *desired_label) {
        int uid;
 ...
 */
int password_exists(struct verify_info *verify) {
        struct pr_passwd *prpwd;
        BOOL nocheck;
 ...
 * /dev/tty* terminals.
 */
int verify_sec_xterm(struct verify_info *verify, char *desired_label) {
        return 1;
}
 ...
 */
int set_sec_label(struct verify_info *verify) {
        struct pr_passwd *prpwd;
        struct passwd *pwd;
        /* set clearance */
        prpwd = verify->prpwd;
        ...
        if (setclrnce(verify->sec_label_ir) == -1) {
                ...
                case EPERM:
                        audit_login(prpwd, pwd, verify->terminal,
                                    "Insufficient privs to set clearance",
                                    ES_LOGIN_FAILED);
        ...
        if (setslabel(verify->sec_label_ir) == -1) {
                ...
                case EPERM:
                        audit_login(prpwd, pwd, verify->terminal,
                                    "Insufficient privs to set sec label",
                                    ES_LOGIN_FAILED);
                        Debug("login failed: insufficient priv. to "
                              "setslabel()\n");
                        break;
                case EFAULT:
                        /* audit:login failed: xdm memory fault */
 ...
 */
int set_sec_privs(struct verify_info *verify) {
        ...
        if (setpriv(SEC_EFFECTIVE_PRIV, bprivs) == -1) {
                ...
                case EFAULT:
                        ...
                        break;
                default:
                        Debug("setpriv (effective) failed for unknown error: "
                              "%d\n",
                              errno);
                        break;
                }
 ...
 */
int change_to_user(struct verify_info *verify) {
        struct pr_passwd *prpwd;
        struct passwd *pwd;
        int new_nice;
        ...
        if (setluid(verify->uid) == -1) {
                ...
                case EINVAL:
                        ...
                        audit_login(prpwd, pwd, verify->terminal,
                                    "Unable to set luid - out of range",
                                    ES_LOGIN_FAILED);
                        break;
                default:
                        Debug("Can't set luid-Unknown error %d\n", errno);
                        audit_login(prpwd, pwd, verify->terminal,
                                    "Unable to set luid-unknown error",
                                    ES_LOGIN_FAILED);
                        break;
                }
                return 0;
 ...
 */
int dump_sec_debug_info(struct verify_info *verify) {
        mand_ir_t *level_ir;
        priv_t privs[SEC_SPRIVVEC_SIZE];
        ...
        if (getclrnce(level_ir) == -1) {
                switch (errno) {
                case EFAULT:
                        Debug("getclrnce EFAULT\n");
                        break;
                case EINVAL:
                        Debug("getclrnce EINVAL\n");
                        break;
                default:
                        Debug("getclrnce unknown error:%d\n", errno);
                        break;
                }
                return 0;
        } else
        ...
        if (getslabel(level_ir) == -1) {
                switch (errno) {
                case EFAULT:
                        Debug("getslabel EFAULT\n");
                        break;
                case EINVAL:
                        Debug("getslabel EINVAL\n");
                        break;
                default:
                        Debug("getslabel unknown error:%d\n", errno);
                        break;
                }
                return 0;
        } else
        ...
        if (getpriv(SEC_MAXIMUM_PRIV, privs) == -1) {
                switch (errno) {
                case EFAULT:
                        Debug("getpriv max EFAULT\n");
                        break;
                case EINVAL:
                        Debug("getpriv max EINVAL\n");
                        break;
                default:
                        Debug("getpriv max unknown error:%d\n", errno);
                        break;
                }
                return 0;
        } else
        ...
        if (getpriv(SEC_EFFECTIVE_PRIV, privs) == -1) {
                switch (errno) {
                case EFAULT:
                        Debug("getpriv eff EFAULT\n");
                        break;
                case EINVAL:
                        Debug("getpriv eff EINVAL\n");
                        break;
                default:
                        Debug("getpriv eff unknown error:%d\n", errno);
                        break;
                }
                return 0;
        } else
        ...
        if (getpriv(SEC_BASE_PRIV, privs) == -1) {
                switch (errno) {
                case EFAULT:
                        Debug("getpriv base EFAULT\n");
                        break;
                case EINVAL:
                        Debug("getpriv base EINVAL\n");
                        break;
                default:
                        Debug("getpriv base unknown error:%d\n", errno);
                        break;
                }
                return 0;
        } else
...
int writeLoginInfo(char *filename, struct verify_info *verify) {
