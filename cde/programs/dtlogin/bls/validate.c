        ...
        if (ISSECURE) {
                ...
                if (b1_pwd == NULL || strlen(name) == 0) {
                        ...
                        audit_login((struct pr_passwd *)0, (struct passwd *)0,
                                    dpyinfo.name,
                                    "No entry in protected password db",
                                    ES_LOGIN_FAILED);
                        return (FALSE);
                }
        ...
        if ((UserHasPassword = password_exists(verify)) != 0)
                if (strcmp(bigcrypt(passwd, b1_pwd->ufld.fd_encrypt),
                        ...
                        audit_login(b1_pwd, p, dpyinfo.name,
                                    "Password incorrect", ES_LOGIN_FAILED);
                        return (FALSE);
                } else {
...
static struct passwd nouser = {"",
                               "nope"}; /* invalid user password struct	   */

int BLS_Verify(char *name, char *passwd) {
        ...
        if (CheckPassword(name, passwd) == FALSE) {
                ...
                if (focusWidget == passwd_text) {
                        ...
                        /*
                         * The user has not password -- this must be the initial
                         * login for this user.  Treat it like an expired
                         * password.  This should invoke the password program on
                         * behalf of the user.
                         */
                        UserHasPassword = 1;
        ...
        if (locked_out(prpwd)) {
                ...
                audit_login(prpwd, p, verify->terminal, "Account locked",
                            ES_LOGIN_FAILED);
                return VF_INVALID;
        }
        ...
        if (time_lock(prpwd)) {
                ...
                audit_login(prpwd, p, verify->terminal, "Account time-locked",
                            ES_LOGIN_FAILED);
                return VF_INVALID;
        }
...
/*	    strict_count = 1;*/
        ...
        if (passwordExpired(verify)) {
                audit_login(b1_pwd, p, dpyinfo.name, "Password expired",
                            ES_LOGIN_FAILED);
                return (VF_PASSWD_AGED);
        }
        ...
        audit_login(b1_pwd, p, dpyinfo.name, "Successful login",
                    ES_LOGIN_REMOTE);
        return (VF_OK);
}
...
int VerifySensitivityLevel(void) {
...
getGroups(char *name, struct verify_info *verify, int gid) {
        ...
        while (g = getgrent()) {
                ...
                if (groupMember(name, g->gr_mem)) {
                        if (ngroups >= NGROUPS)
                                LogError("%s belongs to more than %d groups, "
                                         "%s ignored\n",
 ...
 */
int passwordExpired(struct verify_info *verify) {
