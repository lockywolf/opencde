...
#define IsVerifyName(n)                                                        \
        ((d->verifyName && (strcmp(d->verifyName, (n)) == 0)) ? TRUE : FALSE)
...
#define FD_SET(f, fdp)                                                         \
        ((fdp)->fds_bits[(f) / (sizeof(int) * 8)] |=                           \
         (1 << ((f) % (sizeof(int) * 8))))
#define FD_CLR(f, fdp)                                                         \
        ((fdp)->fds_bits[(f) / (sizeof(int) * 8)] &=                           \
         ~(1 << ((f) % (sizeof(int) * 8))))
#define FD_ISSET(f, fdp)                                                       \
        ((fdp)->fds_bits[(f) / (sizeof(int) * 8)] &                            \
         (1 << ((f) % (sizeof(int) * 8))))
...
extern int ForEachMatchingIndirectHost(ARRAY8Ptr clientAddress,
                                       ...
                                       CARD16 connectionType,
#endif /* NeedWidePrototypes */
                                       int (*function)(CARD16, struct _ARRAY8 *,
                                                       char *),
                                       char *closure);
extern int ScanAccessDatabase(void);
extern int UseChooser(ARRAY8Ptr clientAddress,
#if NeedWidePrototypes
                      int connectionType);
#else
                      CARD16 connectionType);
...
extern void Account(struct display *d, char *user, char *line, pid_t pid,
#if NeedWidePrototypes
                    int type,
#else
                    short type,
...
extern int UtmpIdOpen(char *utmpId);

/*******************************     apollo.c     **************************/

extern int DoLogin(char *user, char *passwd, char *host);

/*******************************     auth.c       **************************/

extern int ConvertAddr(XdmcpNetaddr saddr, int *len, /* return */
                       char **addr);                 /* return */

extern int ConvertFamily(int sockfamily);

extern int SaveServerAuthorizations(struct display *d, Xauth **auth, int count);
...
extern void GenerateCryptoKey(char *auth, int len);
...
extern int SetTitle(char *name, char *ptr);
...
extern struct display *FindDisplayByServerPid(int serverPid);
extern struct display *FindDisplayBySessionID(CARD32 sessionID);

extern void ForEachDisplay(void (*f)());
...
extern int Panic(char *mesg);
extern int SyncErrorFile(int stamp);
extern void TrimErrorFile(void);

/*******************************     file.c	  **************************/

extern int ParseDisplay(char *source, DisplayType *acceptableTypes,
                        int numAcceptable, struct passwd *puser);
...
extern int Willing(ARRAY8Ptr addr,
#if NeedWidePrototypes
                   int connectionType,
#else
                   CARD16 connectionType,
...
extern void TimeoutProtoDisplays(long now);
...
extern struct protoDisplay *
NewProtoDisplay(struct sockaddr *address, int addrlen,
#if NeedWidePrototypes
                int displayNumber, int connectionType,
#else
                CARD16 displayNumber, CARD16 connectionType,
#endif /* NeedWidePrototypes */
...
extern int GetResource(char *name, char *class, int valueType, char **valuep,
                       char *default_value);
...
extern int LogOpenError(int count);
...
extern int StartServerOnce(struct display *d);
extern int WaitForServer(struct display *d);

/*******************************     session.c	  **************************/

extern void ManageSession(struct display *d);
extern int execute(char **argv, char **environ);
...
extern Cursor GetHourGlass(Display *dpy);
#if !defined(ENABLE_DYNAMIC_LANGLIST)
extern void MakeLangList(void);
#endif
extern void SetHourGlassCursor(Display *dpy, Window w);
...
extern char **setLang(struct display *d, char **env, char *langptr);

/*******************************     verify.c	  **************************/

extern void getGroups(char *name, struct verify_info *verify, int gid);
extern int groupMember(char *name, char **members);
extern char **setDt(struct display *d, char **argv, int dt_type);
...
extern char **userEnv(struct display *d, char *user, char *home, char *shell,
                      struct passwd *p);
extern int Verify(struct display *d, struct greet_info *greet,
                  struct verify_info *verify);

extern char *login_defaults(struct passwd *p, struct display *d);
...
extern void XdmGetXdmcpAuth(struct protoDisplay *pdpy,
#if NeedWidePrototypes
                            unsigned int authorizationNameLen,
#else
                            unsigned short authorizationNameLen,
...
extern int XdmGetKey(struct protoDisplay *pdpy, ARRAY8Ptr displayID);
