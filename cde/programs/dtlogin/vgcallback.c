...
XmString multiline_xmstring(char *text) {
        ...
        {             /* looking for \n */
                ...
                if ((start != end) || (start == text)) {
                        ...
                        buffer[end - start] = '\0';
                        if (!string)
                                string = XmStringCreate(buffer,
                                                        XmFONTLIST_DEFAULT_TAG);
                        else {
                                tmp_string = XmStringCreate(
...
void FakeFocusIn(Widget focus_widget, XtPointer client_data, XEvent *eventprm,
        ...
        XSetInputFocus(XtDisplay(focus_widget), XtWindow(focus_widget),
        ...
        XtRemoveEventHandler(focus_widget, ExposureMask, FALSE, FakeFocusIn,
                             NULL);
}
...
void MenuItemCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        case OB_LAST_DT:
                ...
                logoName = _DtGetIconFileName(
                    DefaultScreenOfDisplay(dpyinfo.dpy), temp_p, NULL, NULL, 0);

                if (logoName == NULL) {
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_NO_LOGOBIT,
                                             MC_DEF_LOG_NO_LOGOBIT),
                                 logoFile);
                        logoFile = NULL;
                }
                i = 0;
...
int PingServer(void) {
        ...
        if (!setjmp(pingTime)) {
                ...
                if (serverDead) {
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_DEADSRV,
                                             MC_DEF_LOG_DEADSRV),
                                 dpyinfo.name);
                        alarm(0);
...
void PingServerCB(XtPointer call_data, XtIntervalId *id) {
        ...
        if (pingInterval == 0) {
                pingInterval =
                    ((t = (char *)getenv(PINGINTERVAL)) == NULL ? 0 : atoi(t));
                pingTimeout =
                    ((t = (char *)getenv(PINGTIMEOUT)) == NULL ? 0 : atoi(t));
        }
...
void RequestCB(XtPointer client_data, int *source, XtInputId *id) {
                ...
                if (r->idMC) {
                        ...
                        if (r->idMC == MC_NO_LOGIN &&
                            (fp = fopen(NO_LOGIN_FILE, "r")) != NULL) {
                                 ...
                                 */
                                string = ReadCatalogXms(MC_ERROR_SET, r->idMC,
                                                        buf + r->offMessage);
                        }
                } else {
                /*
...
#else
                        string = XmStringCreate(buf + r->offMessage,
                                                XmFONTLIST_DEFAULT_TAG);
#endif
                }
        ...
        case REQUEST_OP_CHALLENGE: {
                ...
                XtAddEventHandler(textdata->text[textdata->bEcho], ExposureMask,
                                  False, FakeFocusIn, NULL);

                XtMapWidget(textdata->text[textdata->bEcho]);

                XtPopup(login_shell, XtGrabNone);

                XGrabKeyboard(dpyinfo.dpy,
                              XtWindow(textdata->text[textdata->bEcho]), False,
                ...
                XmTextFieldSetSelection(
                    textdata->text[1], 0,
                    XmTextFieldGetLastPosition(textdata->text[1]), CurrentTime);

                if (r->idMC) {
                        /*
                         * Read message from message catalog.
                         */
                        string = ReadCatalogXms(MC_LABEL_SET, r->idMC,
                                                buf + r->offChallenge);
                } else {
                        /*
                         * Generate message from provided string.
                         */
                        string = XmStringCreate(buf + r->offChallenge,
                                                XmFONTLIST_DEFAULT_TAG);
                }
                ...
                if (change) {
                        ...
                        i = 0;
                        if (textdata->bEcho) {
                                XtSetArg(argt[i], XmNlabelString,
                                         textdata->onGreeting);
                                i++;
                        } else {
                                ...
                                string =
                                    XmStringCreate(buf, XmFONTLIST_DEFAULT_TAG);
                                XtSetArg(argt[i], XmNlabelString, string);
                                i++;
...
static void PromptSensitivityLevel(void) {
