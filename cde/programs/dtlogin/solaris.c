...
int solaris_authenticate(char *prog_name, char *display_name, char *user_passwd,
        ...
        status =
            ia_start(prog_name, user, line_dev, display_name, &ia_conv, &iah);
...
int solaris_accounting(char *prog_name, char *display_name, char *entry_id,
        ...
        if (entry_type == ACCOUNTING) {
                status = ia_start(prog_name, user, line, display_name, &ia_conv,
                                  &iah);
        } else {
                status = ia_start(prog_name, user, line_dev, display_name,
                                  &ia_conv, &iah);
        }
        ...
        if (status != IA_SUCCESS) {
                ...
                case USER_PROCESS:
                        ...
                        status = ia_open_session(iah, session_type, entry_type,
                                                 entry_id, &out);
                        if (status == IA_SUCCESS)
                                audit_login_success();
                        break;
                ...
                case ACCOUNTING:
                        ...
                        status = ia_open_session(iah, session_type, entry_type,
                                                 entry_id, &out);
                        if (status != IA_SUCCESS) {
                                Debug("solaris_accounting: PAM check seuser "
                                      "error=%d\n",
                                      status);
                        }
                ...
                case DEAD_PROCESS:
                        ...
                        status = ia_close_session(iah, 0, pid, exitcode,
                                                  entry_id, &out);
                        break;
...
static char *create_devname(char *short_devname) {
         ...
         *
         * This code is directly from Sun Solaris 5.3 login.  Eventually this
         *will be a fuction in a Solaris library.  Till then, this is duplicate
         *copy.
         ***************************************************************************/
...
static int login_conv(int conv_id, int num_msg, struct ia_message **msg,
        ...
        *response =
            (struct ia_response *)calloc(num_msg, sizeof(struct ia_response));
        if (*response == NULL)
                return (IA_CONV_FAILURE);
...
static void end_conv() {}
         ...
         * private libauth open and close session code.  There are no equivalent
         *calls in libpam that replaces it, so the code is copied to here.
         *Eventually this code may become functions in either PAM or other
