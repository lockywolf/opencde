                ...
                if (status != 0) {
                        ...
                        case EPERM:
                                LogError(ReadCatalog(MC_LOG_SET,
                                                     MC_LOG_NOT_SUSER,
                                                     MC_DEF_LOG_NOT_SUSER));
                                break;

                        case EINVAL:
                                LogError(ReadCatalog(MC_LOG_SET,
                                                     MC_LOG_INV_EVENT,
                                                     MC_DEF_LOG_INV_EVENT));
                                break;

                        default:
                                LogError(ReadCatalog(MC_LOG_SET,
                                                     MC_LOG_ERR_ERRNO,
                                                     MC_DEF_LOG_ERR_ERRNO),
                                         errno);
                                break;
                        }
                }
...
static int CountUsers(int added_users) {
         ...
         *		collectively count as 1 user IF count[3] is 0,
         *otherwise, they are not counted. Starting with HP-UX 8.0 they are no
         *longer counted at all. [3]     logins via a pty which have been
         *logged through login (i.e. rlogin and telnet).  these count as 1
         *"real" user per unique user name. [4-15]  may be used for groups of
         *users which collectively count as 1
         */
        nusers = count[1];
...
static int CountUsersStrict(char *new_user) {
        ...
        while ((entry = getutent()) != NULL) {
                if (entry->ut_type == USER_PROCESS) {
                        ...
                        /* if out of range, then count as ordinary user logged
                           in via a tty */
                        if (i == 1 || (i < 0 || i >= NCOUNT))
                                count[1]++;
                        /* See if it is a pty login granted by login program */
                                ...
                                count[3]++;
                                /* See if user is already logged in via login
                                 * pty */
                                uname_off = -1;
                                for (cnt = 0; cnt <= pty_off; cnt++)
                                        if (strncmp(pty_users[cnt],
                                                    entry->ut_user, 8) == 0)
                                                uname_off = cnt;

                                if (uname_off ==
                                    -1) { /* user is not logged in via pty yet
                                           */

                                        if (pty_off >=
                                            MAX_STRICT_USERS) /* cannot add any
                                                                 more users */
                                                return (MAX_STRICT_USERS + 1);
                                        /* add the user name to the array of pty
                                         * users */
                                        else
                                                strncpy(pty_users[++pty_off],
                                                        entry->ut_user, 8);
                                }
                        } /* end if (i == 3) */
                        else
...
static int PasswordAged(register struct passwd *pw) {
        ...
        **	*   if the last week is less than the first week (e.g., the
        *aging
        **	    field looks like "./"), only the superuser can change the
        **	    password.  We don't request a new password.
        **	*   if the week the password was last changed is after this
        *week, *	    we have a problem, and request a new password. *
        ...
        *ppwd = p;
        ...
        if (vg_IsVerifyName(VN_AFS)) {
                ...
                if (ka_UserAuthenticateGeneral(
                        ...
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_AFS_FAILATH,
                                             MC_DEF_LOG_AFS_FAILATH),
                                 reason);
                }
        }
#endif /* __AFS */
        ...
        if (vg_IsVerifyName(VN_KRB)) {
                ...
                kerno = krb_get_pw_in_tkt(p->pw_name, "", realm, "krbtgt",
                                          realm, KRBLIFE, passwd);
...
int Authenticate(struct display *d, char *name, char *passwd, char **msg) {
        ...
        if ((!strncmp(utsnam.machine, "9000/834", UTSLEN)) ||
            ...
            (!strncmp(utsnam.machine, "9000/853", UTSLEN))) {
                ...
                if (CountUsersStrict(name) > MAX_STRICT_USERS) {
                        Audit(p,
                              " attempted to login - too many users on the "
                              "system",
                              20);
                ...
                if (utsnam.version[0] != UNLIMITED) {
                        ...
                        if (CountUsers(1) > num_users[n]) {
                                Audit(p,
                                      " attempted to login - too many users on "
                                      "the system",
                                      20);
        ...
        if (!rgy_$using_local_registry()) {
                if (rgy_$is_acct_expired(&admin_part) != 0) {
                        LogError(ReadCatalog(MC_LOG_SET, MC_LOG_ACC_EXP,
                                             MC_DEF_LOG_ACC_EXP),
                                 name);
                        return (VF_INVALID);
                }
...
#else
        status =
            solaris_authenticate("dtlogin", d->name, passwd, name, ttyLine);
...
static void Audit(struct passwd *p, char *msg, int errnum) {
        ...
        seteuid(0);
        if ((auditwrite("USER_Login", AUDIT_OK, p->pw_name,
                        strlen(p->pw_name) + 1, msg, strlen(msg) + 1, NULL)) ==
            -1)
...
static void WriteBtmp(char *name) {
        ...
        if ((fd = open("/etc/security/failedlogin", O_CREAT | O_RDWR, 0644)) !=
            -1) {
...
static int PasswordAged(char *name, register struct passwd *pw) {
        ...
        if (!err && maxage) {
                ...
                /*
                 * Could not retrieve system password aging info or maxage set
                 * to zero. In either case, consider password has having not
                 * expired.
                 */
                return (FALSE);
        }
...
int tsm_check_login(uid_t uid) {
        ...
        if ((key = ftok(CDE_INSTALLATION_TOP "/bin/dtlogin", 1)) != (key_t)-1) {
                Debug("key created\n");
                if ((semid = semget(key, 1, IPC_CREAT | IPC_EXCL | 0600)) !=
                    -1) {
                        int i;
...
static int PasswordAged(register struct passwd *pw) { return (FALSE); }
