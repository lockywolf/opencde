...
typedef enum { PULLDOWN_MENU, OPTION_MENU, POPUP_MENU } MenuType;

typedef enum { LEFT_JUSTIFIED, CENTERED, RIGHT_JUSTIFIED } LabelType;

typedef enum { VERTICAL_SCALE, HORIZONTAL_SCALE } ScaleType;

typedef enum { FIXED_LIST, SCROLLED_LIST } ListType;
...
typedef enum { AS_PLACED, GRID, BROWSER, TREE, PROPERTIES } ViewStyle;
...
#define TraceFunctionCalls(flag)
      ...
      protected:
        ...
        BaseUI(BaseUI *parent, const char *name, const char *category = NULL);
        ...
        void _Find(void *, int, int *, BaseUI ***, SelectProc, boolean, boolean,
                   boolean, int);
      ...
      public:
        ...
        virtual PointerCursor PointerShape() {
                return LEFT_SLANTED_ARROW_CURSOR;
        }
        ...
        virtual void AttachTop(BaseUI *, int /*offset*/ = 0,
                               boolean /*opposite*/ = false) {}
        virtual void AttachBottom(BaseUI *, int /*offset*/ = 0,
                                  boolean /*opposite*/ = false) {}
        virtual void AttachLeft(BaseUI *, int /*offset*/ = 0,
                                boolean /*opposite*/ = false) {}
        virtual void AttachRight(BaseUI *, int /*offset*/ = 0,
                                 boolean /*opposite*/ = false) {}
        ...
        BaseUI *FindByName(char *pattern, int depth = 1, int *n_mactches = NULL,
                           BaseUI ***matches = NULL,
                           SelectProc select_proc = NULL,
                           boolean regular_expression = false,
                           boolean case_sensitive = true);
        BaseUI *FindByCategory(char *pattern, int depth = 1,
                               int *n_mactches = NULL, BaseUI ***matches = NULL,
                               SelectProc select_proc = NULL,
                               boolean regular_expression = false,
                               boolean case_sensitive = true);
        ...
        virtual void Dump(boolean verbose = false, int level = 0);
        // Dumps object heirarchy to stdout
        void DumpHierarchy(boolean verbose = false, int level = 0);
        ...
        // Get selected items
        virtual void Selection(int *n_items, BaseUI ***items = NULL);
