...
#define INT_MESSAGE4                                                           \
        ...
        "remaining.\r\n"
#define INT_MESSAGE5                                                           \
        "    The connection was probably broken by a server shudown or "       \
        "KillClient.\r\n"
...
void Debug(XtAppContext app_context) {
 ...
 */
static int XIOError(Display *dpy) {
        fprintf(stderr, INT_MESSAGE3, errno, SysErrorMsg(errno),
                DisplayString(dpy));
        fprintf(stderr, INT_MESSAGE4, NextRequest(dpy) - 1,
        ...
        if (event->error_code >= 128) {
                ...
                for (ext = dpy->ext_procs; ext; ext = ext->next) {
                        ...
                        if (ext->codes.first_error &&
                            ...
                            (!bext ||
                             ext->codes.first_error > bext->codes.first_error))
                                bext = ext;
                }
                if (bext)
                ...
                if (event->error_code == BadValue)
                        XGetErrorDatabaseText(dpy, mtype, "Value",
                                              "Value 0x%lx", mesg, BUFSIZ);
                else if (event->error_code == BadAtom)
                        XGetErrorDatabaseText(dpy, mtype, "AtomID",
                                              "AtomID 0x%lx", mesg, BUFSIZ);
                else
                        XGetErrorDatabaseText(dpy, mtype, "ResourceID",
                                              "ResourceID 0x%lx", mesg, BUFSIZ);
                fputs("  ", fp);
                fprintf(fp, mesg, event->resourceid);
                fputs("\n", fp);
        }
 ...
 *   RETURNS:
 */
static void _XtError(String string) {
