...
*/
static void rfp_toggle_for_grey(RFP *rfp, Boolean status) {
        Widget list, text;
...
static void rfp_toggle_repeat_grey(RFP *rfp, Boolean status) {
        Widget list;
...
extern void rfp_set_repeat_values(RFP *rfp)

{
        ...
        if (limit == 11) {
                ...
                xmstr = XmStringCreateLocalized(
                    catgets(calendar->DT_catd, 1, 901, "Repeat Every ..."));
                XmComboBoxAddItem(rfp->repeat_menu, xmstr, 0, False);
                XmStringFree(xmstr);

        } else if (limit == 7) {
                xmstr = XmStringCreateLocalized(
                    catgets(calendar->DT_catd, 1, 902, "Monday Thru Friday"));
                XmComboBoxAddItem(rfp->repeat_menu, xmstr, 0, False);
                XmStringFree(xmstr);

                xmstr = XmStringCreateLocalized(
                    catgets(calendar->DT_catd, 1, 903, "Mon, Wed, Fri"));
                XmComboBoxAddItem(rfp->repeat_menu, xmstr, 0, False);
                XmStringFree(xmstr);
                xmstr = XmStringCreateLocalized(
                    catgets(calendar->DT_catd, 1, 861, "Tuesday, Thursday"));
                XmComboBoxAddItem(rfp->repeat_menu, xmstr, 0, False);
                XmStringFree(xmstr);

                if (rfp->cal->general->version == DATAVER3) {
                        xmstr = XmStringCreateLocalized(catgets(
                            calendar->DT_catd, 1, 862, "Repeat Every..."));
                        XmComboBoxAddItem(rfp->repeat_menu, xmstr, 0, False);
                        XmStringFree(xmstr);
                }
        }
...
*/
static void rfp_set_default_for(RFP *rfp, Repeat_menu_op op, Boolean status) {
        Widget text;
        ...
        new_scope_str =
            XmStringCreateLocalized(default_repeat_scope_str(c->DT_catd, op));
        XtVaSetValues(rfp->for_scope, XmNlabelString, new_scope_str, NULL);
...
*/
static void rfp_repeat_scope_proc(Widget w, XtPointer client_data,
                                  XtPointer cbs) {
...
static void rfp_apply_proc(Widget w, XtPointer client_data, XtPointer cbs) {
        RFP *rfp = (RFP *)client_data;
        char *str, buf[128];
        int buf_size = 128;
        XmString xmstr;
        ...
        if (!string_is_number(str)) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 969, "Calendar : Error - Repeat Every"));
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 970,
                    "The Repeat Every value must be an integer with no sign."));
                char *ident1 =
        ...
        case REPEAT_DAYS:
                repeat_str = catgets(c->DT_catd, 1, 825, "Days");
                break;

        case REPEAT_WEEKS:
                repeat_str = catgets(c->DT_catd, 1, 826, "Weeks");
                break;

        case REPEAT_MONTHS:
                repeat_str = catgets(c->DT_catd, 1, 827, "Months");
                break;
        }
         ...
         */
        {
                ...
                snprintf(buf, buf_size,
                         catgets(c->DT_catd, 1, 531, "Every %d %s"),
...
static void rfp_cancel_proc(Widget w, XtPointer client_data, XtPointer cbs) {
        RFP *rfp = (RFP *)client_data;
         ...
         * down the popup.
         */
        XtVaSetValues(rfp->repeat_menu, XmNselectedPosition, ONE_TIME + 1,
                      NULL);
}
...
*/
static void rfp_repeat_every_popup(RFP *rfp) {
        ...
        title =
            XtNewString(catgets(c->DT_catd, 1, 532, "Calendar : Repeat Every"));
        rfp->repeat_popup_frame = XtVaCreatePopupShell(
            "repeat_popup_frame", xmDialogShellWidgetClass, rfp->parent,
            XmNtitle, title, XmNallowShellResize, True, XmNmwmFunctions,
            MWM_FUNC_MOVE | MWM_FUNC_CLOSE, XmNdeleteResponse, XmDO_NOTHING,
            NULL);
        XtFree(title);
        ...
        rfp->repeat_form_mgr = XtVaCreateWidget(
            "form_mgr", xmFormWidgetClass, rfp->repeat_popup_frame,
            XmNautoUnmanage, False, XmNhorizontalSpacing, 0, XmNverticalSpacing,
            5, XmNfractionBase, 2, NULL);

        label_str =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 460, "Apply"));
        rfp->repeat_apply_button = XtVaCreateWidget(
            "repeat_apply_button", xmPushButtonWidgetClass,
            rfp->repeat_form_mgr, XmNlabelString, label_str, XmNleftOffset, 10,
            XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 0,
            XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, 1,
            XmNbottomAttachment, XmATTACH_FORM, NULL);
        XmStringFree(label_str);
        XtAddCallback(rfp->repeat_apply_button, XmNactivateCallback,
                      rfp_apply_proc, rfp);

        label_str =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 680, "Close"));
        rfp->repeat_cancel_button = XtVaCreateWidget(
            "repeat_cancel_button", xmPushButtonWidgetClass,
            rfp->repeat_form_mgr, XmNlabelString, label_str, XmNleftAttachment,
            XmATTACH_POSITION, XmNleftPosition, 1, XmNrightAttachment,
            XmATTACH_POSITION, XmNrightPosition, 2, XmNrightOffset, 10,
            XmNbottomAttachment, XmATTACH_FORM, NULL);
        XmStringFree(label_str);
        XtAddCallback(rfp->repeat_cancel_button, XmNactivateCallback,
                      rfp_cancel_proc, rfp);

        separator = XtVaCreateWidget(
            "separator", xmSeparatorGadgetClass, rfp->repeat_form_mgr,
            XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            rfp->repeat_apply_button, XmNbottomOffset, 5, NULL);

        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 533, "Repeat Every:"));
        label = XtVaCreateWidget(
            "every", xmLabelGadgetClass, rfp->repeat_form_mgr, XmNlabelString,
            xmstr, XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 10,
            XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 5,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget, separator,
            NULL);
        XmStringFree(xmstr);

        rfp->repeat_popup_text = XtVaCreateWidget(
            "text_field", xmTextWidgetClass, rfp->repeat_form_mgr, XmNeditMode,
            XmSINGLE_LINE_EDIT, XmNcolumns, FOR_LEN, XmNmaxLength, FOR_LEN - 1,
            XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 10,
            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, label,
            XmNleftOffset, 5, XmNbottomAttachment, XmATTACH_WIDGET,
            XmNbottomWidget, separator, NULL);

        rfp->repeat_popup_menu = create_repeat_scope_menu(
            rfp->repeat_form_mgr, NULL, rfp_repeat_scope_proc, (XtPointer)rfp);

        XtVaSetValues(rfp->repeat_popup_menu, XmNuserData, REPEAT_DAYS,
                      XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 6,
                      XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
                      rfp->repeat_popup_text, XmNleftOffset, 5,
                      XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
                      separator, NULL);
...
static void show_repeat_every(XtPointer uData, XtIntervalId *id) {
        RFP *rfp = (RFP *)uData;
...
static void hide_repeat_every(XtPointer uData, XtIntervalId *id) {
        RFP *rfp = (RFP *)uData;
...
*/
static void rfp_repeat_menu_proc(Widget w, XtPointer data, XtPointer cbs) {
        RFP *rfp = (RFP *)data;
        ...
        XtVaGetValues(rfp->repeat_menu, XmNselectedPosition, &item_no, NULL);
        --item_no;
        ...
        if (item_no == REPEAT_EVERY) {
                XtAppAddTimeOut(XtWidgetToApplicationContext(w), 0,
                                show_repeat_every, (XtPointer)rfp);
        } else {
                XtAppAddTimeOut(XtWidgetToApplicationContext(w), 0,
                                hide_repeat_every, (XtPointer)rfp);
...
**
*******************************************************************************/
extern void build_rfp(RFP *rfp, Calendar *c, Widget parent) {
        XmString tmp;
        ...
        rfp->rfp_form_mgr =
            XtVaCreateWidget("rfp_form_mgr", xmFormWidgetClass, rfp->parent,
                             XmNautoUnmanage, False, XmNuserData, rfp, NULL);

        label_str =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 836, "Frequency"));
        rfp->frequency_label = XtVaCreateWidget(
            "frequency_label", xmLabelGadgetClass, rfp->rfp_form_mgr,
            XmNlabelString, label_str, XmNtopAttachment, XmATTACH_FORM,
            XmNtopOffset, GAP, NULL);
        XmStringFree(label_str);

        tmp = XmStringCreateLocalized(catgets(c->DT_catd, 1, 534, "Occurs:"));
        rfp->repeat_label = XtVaCreateWidget(
            "repeat", xmLabelGadgetClass, rfp->rfp_form_mgr, XmNlabelString,
            label_str, XmNlabelString, tmp, NULL);
        XmStringFree(tmp);

        tmp = XmStringCreateLocalized(catgets(c->DT_catd, 1, 535, "For:"));
        rfp->for_label =
            XtVaCreateWidget("for", xmLabelGadgetClass, rfp->rfp_form_mgr,
                             XmNlabelString, tmp, XmNsensitive, False, NULL);
        XmStringFree(tmp);

        tmp = XmStringCreateLocalized(catgets(c->DT_catd, 1, 536, "Privacy:"));
        rfp->privacy_label =
            XtVaCreateWidget("privacy", xmLabelGadgetClass, rfp->rfp_form_mgr,
                             XmNlabelString, tmp, NULL);
        XmStringFree(tmp);

        max_left_label_width =
            ComputeMaxWidth(rfp->frequency_label, rfp->repeat_label,
                            rfp->for_label, rfp->privacy_label) +
            GAP;

        XtVaGetValues(rfp->frequency_label, XmNheight, &freq_label_height,
                      NULL);

        rfp->repeat_menu = create_repeat_menu(
            rfp->rfp_form_mgr, rfp_repeat_menu_proc, (XtPointer)rfp);

        rfp_set_repeat_values(rfp);
        XtVaSetValues(rfp->repeat_menu, XmNuserData, ONE_TIME,
                      XmNleftAttachment, XmATTACH_FORM, XmNleftOffset,
                      max_left_label_width, XmNtopAttachment, XmATTACH_FORM,
                      XmNtopOffset, freq_label_height + 3 * GAP, NULL);
        tmp = XmStringCreateLocalized("\0");

        rfp->for_menu = create_for_menu(rfp->rfp_form_mgr);
        XtVaSetValues(rfp->for_menu, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, max_left_label_width, XmNtopAttachment,
                      XmATTACH_WIDGET, XmNtopWidget, rfp->repeat_menu,
                      XmNtopOffset, 2 * GAP, NULL);

        rfp->for_scope = XtVaCreateWidget(
            "for_scope", xmLabelGadgetClass, rfp->rfp_form_mgr, XmNlabelString,
            tmp, XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET, XmNbottomWidget,
            rfp->for_label, XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
            rfp->for_menu, XmNleftOffset, GAP, NULL);

        XmStringFree(tmp);

        rfp->privacy_menu = create_privacy_menu(rfp->rfp_form_mgr);
        XtVaSetValues(rfp->privacy_menu, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, max_left_label_width, XmNtopAttachment,
                      XmATTACH_WIDGET, XmNtopWidget, rfp->for_menu,
                      XmNtopOffset, 2 * GAP, NULL);

        XtVaSetValues(rfp->frequency_label, XmNrightAttachment, XmATTACH_WIDGET,
                      XmNrightWidget, rfp->repeat_menu, XmNrightOffset, GAP,
                      NULL);

        XtVaSetValues(rfp->repeat_label, XmNbottomAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNbottomWidget,
                      rfp->repeat_menu, XmNbottomOffset, GAP,
                      XmNrightAttachment, XmATTACH_WIDGET, XmNrightWidget,
                      rfp->repeat_menu, XmNrightOffset, GAP, NULL);

        XtVaSetValues(rfp->for_label, XmNbottomAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNbottomWidget, rfp->for_menu,
                      XmNbottomOffset, GAP, XmNrightAttachment, XmATTACH_WIDGET,
                      XmNrightWidget, rfp->for_menu, XmNrightOffset, GAP, NULL);

        XtVaSetValues(rfp->privacy_label, XmNbottomAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNbottomWidget,
                      rfp->privacy_menu, XmNbottomOffset, GAP,
                      XmNrightAttachment, XmATTACH_WIDGET, XmNrightWidget,
                      rfp->privacy_menu, XmNrightOffset, GAP, NULL);
}

/*
**  Get and set values on the screen
*/
extern void get_rfp_repeat_val(RFP *rfp, time_t tick) {
        ...
        */
        ...
        case MONTHLY_BY_WEEKDAY: {
                ...
                if (weekofmonth(tick, &wk) && wk == 5)
                        snprintf(rule_buf1, rule_buf_size, "MP1 1- %s ",
                                 dow_str(tick));
                else
        ...
        case REPEAT_EVERY:
                ...
                case CSA_X_DT_REPEAT_EVERY_NDAY:
                        snprintf(rule_buf1, rule_buf_size, "D%d ",
                                 rfp->repeat_nth);
                        break;
                case CSA_X_DT_REPEAT_EVERY_NWEEK:
                        snprintf(rule_buf1, rule_buf_size, "W%d ",
                                 rfp->repeat_nth);
                        break;
                case CSA_X_DT_REPEAT_EVERY_NMONTH:
                default:
                        snprintf(rule_buf1, rule_buf_size, "MD%d ",
                                 rfp->repeat_nth);
                        break;
                }
                break;
        ...
        */
        XtVaGetValues(rfp->for_menu, XmNtextField, &tf, NULL);
        XtVaGetValues(tf, XmNsensitive, &sensitive, NULL);

        if (sensitive) {
                char *forever_str =
                    catgets(calendar->DT_catd, 1, 876, "forever");
                ...
                if (strcmp(str, forever_str) == 0) {
                        ...
                        if (rfp->for_val == 0)
                                ...
                                if (op != REPEAT_EVERY) {
                                        snprintf(rule_buf2, rule_buf_size,
                                                 "#%d", rfp->for_val);
                                } else {
                                        int duration;
...
extern void get_rfp_privacy_val(RFP *rfp) {
        int i;
        ...
        case 0:
                rfp->privacy_val = CSA_CLASS_PUBLIC;
                break;

        case 1:
                rfp->privacy_val = CSA_CLASS_CONFIDENTIAL;
                break;

        case 2:
                rfp->privacy_val = CSA_CLASS_PRIVATE;
                break;
        }
}

extern void get_rfp_vals(RFP *rfp, time_t tick) {
...
*/
extern Boolean rfp_form_to_appt(RFP *rfp, Dtcm_appointment *a, char *name) {
        return rfp_form_flags_to_appt(rfp, a, name, (int *)NULL);
}

extern Boolean rfp_form_flags_to_appt(RFP *rfp, Dtcm_appointment *a, char *name,
                                      int *flagsP) {
        int wk;
        char buf[MAXNAMELEN];
        time_t tick;
        ...
        if (a->version < DATAVER4) {
                ...
                a->repeat_times->value->item.uint32_value =
                    rfp->for_val == PRIVATE_FOREVER ? CSA_X_DT_DT_REPEAT_FOREVER
                                                    : rfp->for_val;

                if ((a->version == DATAVER3) ||
                    (a->version == DATAVER_ARCHIVE)) {
        ...
        if (rfp->repeat_type != CSA_X_DT_REPEAT_ONETIME) {
                ...
                if (rfp->repeat_type == CSA_X_DT_REPEAT_MONTHLY_BY_WEEKDAY) {
                        if (weekofmonth(tick, &wk) && wk == 4) {
                                if (*flagsP == 0) {
                                        char *title = XtNewString(
                                            catgets(c->DT_catd, 1, 537,
                                                    "Calendar : Editor - "
                                                    "Schedule Appointment"));
                                        char *text = XtNewString(catgets(
                                            c->DT_catd, 1, 538,
                                            "Would you like to schedule this "
                                            "appointment as the last\nweek of "
                                            "the month or the 4th week of the "
                                            "month?"));
                                        char *ident1 = XtNewString(catgets(
                                            c->DT_catd, 1, 923, "Cancel"));
                                        char *ident2 = XtNewString(catgets(
                                            c->DT_catd, 1, 540, "Last Week"));
                                        char *ident3 = XtNewString(catgets(
                                            c->DT_catd, 1, 541, "4th Week"));
                                        *flagsP = dialog_popup(
                                            rfp->parent, DIALOG_TITLE, title,
                                            DIALOG_TEXT, text, BUTTON_IDENT, -1,
                                            ident1, BUTTON_IDENT, RFP_MBW_LAST,
                                            ident2, BUTTON_IDENT, RFP_MBW_4TH,
                                            ident3, DIALOG_IMAGE,
                                            p->xm_question_pixmap, NULL);
                                        XtFree(ident3);
                                        XtFree(ident2);
                                        XtFree(ident1);
                                        XtFree(text);
                        ...
                        if (a->repeat_type == NULL) {
                                if ((a->repeat_type = (CSA_attribute *)ckalloc(
                                         sizeof(CSA_attribute))) == NULL)
                                        /* return (CSA_E_INSUFFICIENT_MEMORY);
                                         */
                                        return False;

                                memset(a->repeat_type, 0,
                                       sizeof(CSA_attribute));

                                _DtCm_set_sint32_attrval(
                                    rfp->repeat_type, &a->repeat_type->value);
                        } else
                                a->repeat_type->value->item.sint32_value =
                                    rfp->repeat_type;
                }
        } else {
                if (a->repeat_type == NULL) {
 ...
 */
static void change_to_last_week(Dtcm_appointment *a, int wk, time_t tick,
                                RFP *rfp) {
        if (a->version < DATAVER4)
                return;
        else {
                char *weekday = dow_str(tick), *new_rule, duration_buf[10];
...
*/
extern Boolean rfp_appt_to_form(RFP *rfp, CSA_entry_handle entry) {
        Boolean ret_val;
        CSA_return_code stat;
        Dtcm_appointment *appt;

        appt = allocate_appt_struct(appt_read, rfp->cal->general->version,
        ...
        backend_err_msg(rfp->cal->frame, rfp->cal->view->current_calendar, stat,
                        ((Props_pu *)rfp->cal->properties_pu)->xm_error_pixmap);
        if (stat != CSA_SUCCESS) {
...
extern Boolean rfp_attrs_to_form(RFP *rfp, Dtcm_appointment *appt) {
...
extern void set_rfp_repeat_val(RFP *rfp) {
        char buf[128];
        int buf_size = 128;
        Boolean status = True;
        XmString xmstr;
        ...
        */
        ...
        if (rfp->repeat_type == CSA_X_DT_REPEAT_ONETIME) {
                ...
                snprintf(buf, buf_size, "%s",
                         repeat_str(c->DT_catd, EVERY_TWO_WEEKS));
        } else if (rfp->repeat_type == CSA_X_DT_REPEAT_MONTHLY_BY_DATE) {
                op = MONTHLY_BY_DATE;
                repeat_scope =
                    default_repeat_scope_str(c->DT_catd, MONTHLY_BY_DATE);
                snprintf(buf, buf_size, "%s",
                         repeat_str(c->DT_catd, MONTHLY_BY_DATE));
        } else if (rfp->repeat_type == CSA_X_DT_REPEAT_MONTHLY_BY_WEEKDAY) {
                op = MONTHLY_BY_WEEKDAY;
                repeat_scope =
                    default_repeat_scope_str(c->DT_catd, MONTHLY_BY_WEEKDAY);
                snprintf(buf, buf_size, "%s",
                         repeat_str(c->DT_catd, MONTHLY_BY_WEEKDAY));
        ...
        */
        if (rfp->for_val == CSA_X_DT_DT_REPEAT_FOREVER) {
                if (rfp->repeat_type != CSA_X_DT_REPEAT_ONETIME) {
                        snprintf(buf, buf_size, "%s",
                                 catgets(c->DT_catd, 1, 876, "forever"));
                        xmstr = XmStringCreateLocalized(buf);
        ...
        XtVaSetValues(rfp->for_scope, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        rfp_toggle_for_grey(rfp, status);
}

extern void set_rfp_privacy_val(RFP *rfp) {
        int i;
...
extern void set_rfp_vals(RFP *rfp) {
        set_rfp_repeat_val(rfp);
        set_rfp_privacy_val(rfp);
}

extern void set_rfp_defaults(RFP *rfp) {
...
extern void rfp_init(RFP *rfp, Calendar *c, Widget parent) {
