...
#ifndef lint
static char sccsid[] =
    "@(#)cm_insert.c 1.31 95/05/19 Copyr 1993 Sun Microsystems, Inc.";
#endif
...
static char **grab(char **argv, /* command line arguments */
                   char *buf,   /* buffer for keyed data */
                   int buf_size, char stop_key) {
        if (!argv || !*argv)
                return (argv);
...
static void cm_args(int argc, char **argv) {
        ...
        while (++argv && *argv) {
                ...
                default:
                        fprintf(stderr, "%s",
                                catgets(DT_catd, 1, 191,
                                        "Usage:\n\tdtcm_insert [ -c calendar ] "
                                        "[-d <mm/dd/yy>] [ -v view ]\n"));
                        fprintf(stderr, "%s",
                                catgets(DT_catd, 1, 192,
                                        "                   [-w what string] "
                                        "[-s <HH:MMam/pm>] [-e "
                                        "<HH:MMam/pm>]\n"));
                        exit(1);
...
static void prompt_for_line(char *prompt, char *defval, char *buffer,
                            int buffer_size) {
        char input_buf[1024];
...
static void prompt_for_insert(Props *p) {
        ...
        printf("%s", catgets(DT_catd, 1, 193,
                             "Please enter the information for the appointment "
                             "you wish to add.\nDefaults will be shown in "
                             "parentheses.\n"));
        prompt_for_line(catgets(DT_catd, 1, 194, "Calendar (%s): "),
                        cm_get_credentials(), cm_target, 256);
        prompt_for_line(catgets(DT_catd, 1, 195, "Date (%s): "), date_str,
                        cm_date, 256);
        while (valid != TRUE) {
                format_time(now(), dt, cm_start, 16);
                prompt_for_line(catgets(DT_catd, 1, 196, "Start (%s): "),
                                cm_start, cm_start, 16);
                if (cm_start[0]) {
        ...
        if (cm_start[0])
                prompt_for_line(catgets(DT_catd, 1, 198, "End (%s): "), cm_end,
                                cm_end, 16);
        else
                prompt_for_line(catgets(DT_catd, 1, 199, "End (%s): "), "None",
                                cm_end, 16);

        strlcpy(cm_repeatstr, catgets(DT_catd, 1, 200, "One Time"), 256);

        prompt_for_line(catgets(DT_catd, 1, 201, "Repeat (%s): "), cm_repeatstr,
                        cm_repeatstr, 256);

        if (strcmp(cm_repeatstr, catgets(DT_catd, 1, 200, "One Time"))) {
                sprintf(buf, "%s", catgets(DT_catd, 1, 203, "no default"));
                prompt_for_line(catgets(DT_catd, 1, 204, "For (%s): "), buf,
                                cm_for, 256);
        }

        printf(
            "%s",
            catgets(DT_catd, 1, 205,
...
int main(int argc, char **argv) {
        int cnt, status = 0;
        char *date = NULL, *view = NULL, *target = NULL, *start = NULL,
             *end = NULL, *repeat = NULL, *numrepeat = NULL, *what = NULL,
             *uname, *loc;
        Props *p = NULL;
        ...
        if (argc > 1) {
                ...
                if (!cm_date[0])
                        format_tick(now(), get_int_prop(p, CP_DATEORDERING),
                                    ...
                                    256);
                if (cm_strlen(cm_date))
                        date = cm_date;
                if (cm_strlen(cm_view))
                        view = cm_view;
                if (cm_strlen(cm_start))
                        start = cm_start;

                if (!cm_end[0] && cm_start[0]) {
                        format_time((int)cm_getdate(cm_start, NULL) + hrsec, dt,
                                    cm_end, 16);
                }

                if (cm_strlen(cm_end))
                        end = cm_end;
                if (cm_strlen(cm_repeatstr))
                        repeat = cm_repeatstr;
                if (cm_strlen(cm_for))
                        numrepeat = cm_for;
                if (cm_strlen(cm_what))
                        what = cm_what;
                if (!cm_appt_file[0])
                ...
                if (cm_strlen(cm_what))
                        what = cm_what;
                status = cm_tty_insert(DT_catd, c_handle, version, date, start,
                                       end, repeat, numrepeat, what, NULL, p);
        }
        if ((cnt = cm_tty_lookup(DT_catd, c_handle, version, date, view, &list,
                                 p)) > 0)
                csa_free(list);
