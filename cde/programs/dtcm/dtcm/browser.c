...
static void mb_init_array(Browser *b, int begin, int end) {
        b->segs_in_array = BOX_SEG * (end - begin) * 7;
        b->multi_array = (char *)ckalloc(b->segs_in_array);
}

static void reset_ticks(Calendar *c, Boolean use_sel_idx) {
        int beg, end;
        ...
        if (use_sel_idx) {
                b->begin_day_tick = next_ndays(b->begin_week_tick, b->col_sel);
                b->begin_hr_tick =
...
extern void br_display(Calendar *c) {
        ...
        for (i = 0; i < pos_cnt; i++) {
                bd =
                    (BlistData *)CmDataListGetData(bl->blist_data, pos_list[i]);
...
static void invalid_date_msg(Calendar *c, Widget widget) {
        Browser *b = (Browser *)c->browser;
        char *title = XtNewString(catgets(
            c->DT_catd, 1, 1070, "Calendar : Error - Compare Calendars"));
        char *text = XtNewString(
            catgets(c->DT_catd, 1, 20, "Invalid Date In Go To Field."));
        char *ident = XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));

        dialog_popup(b->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                     BUTTON_IDENT, 1, ident, DIALOG_IMAGE,
                     ((Props_pu *)c->properties_pu)->xm_error_pixmap, NULL);
...
void set_entry_date(Calendar *c) {
        char *date = NULL;
        Browser *b;
        Props *p;
        OrderingType ot;
...
extern void make_browser(Calendar *c) {
        ...
        XmString xmstr;
        XmString goto_label, prev_week, this_week, next_week, prev_month,
            next_month;
        int num_children;
        Widget *children;
        ...
        mb_init_array(b, get_int_prop(p, CP_DAYBEGIN),
                      get_int_prop(p, CP_DAYEND));
        ...
        title = XtNewString(
            catgets(c->DT_catd, 1, 1010, "Calendar : Compare Calendars"));
        b->frame = XtVaCreatePopupShell(
            "frame", xmDialogShellWidgetClass, c->frame, XmNtitle, title,
            XmNallowShellResize, True, XmNmappedWhenManaged, False,
         ...
         * and message area
         */
        b->outer_pane = XtVaCreateManagedWidget(
            "outerPane", xmPanedWindowWidgetClass, b->frame, XmNsashHeight, 1,
            XmNsashWidth, 1, XmNwidth, outpane_width, XmNheight, outpane_height,
            NULL);

        b->inner_pane =
            XtVaCreateManagedWidget("innerPane", xmPanedWindowWidgetClass,
                                    b->outer_pane, XmNallowResize, True, NULL);

        b->upper_form = XtVaCreateManagedWidget(
            "upperForm", xmFormWidgetClass, b->inner_pane, XmNallowResize, True,
            XmNpaneMinimum, upform_min, NULL);

        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 22, "Browse Menu Items"));
        b->list_label = XtVaCreateWidget(
            "browseMenuLabel", xmLabelGadgetClass, b->upper_form,
            XmNlabelString, xmstr, XmNleftAttachment, XmATTACH_FORM,
            XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 10, NULL);
        XmStringFree(xmstr);

        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 21, "Edit List..."));
        b->edit_list = XtVaCreateWidget(
            "editList", xmPushButtonGadgetClass, b->upper_form, XmNlabelString,
            xmstr, XmNtopAttachment, XmATTACH_FORM, XmNrightAttachment,
            XmATTACH_FORM, XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
            b->list_label, XmNleftOffset, 80, NULL);
        XmStringFree(xmstr);
        ...
        b->browse_list =
            (Widget)XmCreateScrolledList(b->upper_form, "browseList", NULL, 0);
        XtAddCallback(b->browse_list, XmNmultipleSelectionCallback,
                      mb_box_notify, (XtPointer)c);
        XtAddCallback(b->browse_list, XmNdefaultActionCallback, mb_box_notify,
                      (XtPointer)c);
        XtVaSetValues(b->browse_list, XmNselectionPolicy, XmMULTIPLE_SELECT,
                      XmNvisibleItemCount, item_count, NULL);

        b->browse_list_sw = XtParent(b->browse_list);
        XtVaSetValues(b->browse_list_sw, XmNtopAttachment, XmATTACH_WIDGET,
                      XmNtopWidget, b->edit_list, XmNleftAttachment,
                      XmATTACH_FORM, XmNleftOffset, 20, XmNrightAttachment,
                      XmATTACH_FORM, XmNrightOffset, 20, XmNbottomAttachment,
                      XmATTACH_FORM, XmNvisualPolicy, XmVARIABLE, NULL);
         ...
         */
        b->gotomenu = XmVaCreateSimpleOptionMenu(
            b->upper_form, "goToOptionMenu", goto_label, 0, 0, gotomenu_cb,
            XmVaPUSHBUTTON, prev_week, NULL, NULL, NULL, XmVaPUSHBUTTON,
            this_week, NULL, NULL, NULL, XmVaPUSHBUTTON, next_week, NULL, NULL,
            NULL, XmVaPUSHBUTTON, prev_month, NULL, NULL, NULL, XmVaPUSHBUTTON,
            next_month, NULL, NULL, NULL, XmNbottomAttachment, XmATTACH_FORM,
            XmNbottomOffset, 5, XmNleftAttachment, XmATTACH_FORM, XmNleftOffset,
            20, XmNuserData, c, XmNnavigationType, XmTAB_GROUP, NULL);
        ...
        text_field_form = XtVaCreateManagedWidget(
            "text_field_form", xmFormWidgetClass, b->upper_form,
            XmNbottomAttachment, XmATTACH_FORM, XmNbottomOffset, 5,
            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, b->gotomenu,
            NULL);

        b->datetext = XtVaCreateWidget("dateText", xmTextWidgetClass,
                                       text_field_form, NULL);
        ...
        b->lower_form = XtVaCreateManagedWidget(
            "lowerForm", xmFormWidgetClass, b->inner_pane, XmNallowResize, True,
            XmNpaneMinimum, 200, XmNtraversalOn, False, NULL);

        /*
         * create drawing area for chart
         */
        b->canvas = XtVaCreateManagedWidget(
            "canvas", xmDrawingAreaWidgetClass, b->lower_form, XmNtopAttachment,
            XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
            XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, XmNtraversalOn,
            False, NULL);
        b->xcontext =
            gr_create_xcontext(c, b->canvas, gr_color, c->xcontext->app);

        XtVaSetValues(b->canvas, XmNheight, 300, NULL);

        XtAddCallback(b->canvas, XmNresizeCallback, mb_resize_proc,
                      (XtPointer)c);
        XtAddCallback(b->canvas, XmNinputCallback, bcanvas_event, (XtPointer)c);
        XtAddCallback(b->canvas, XmNexposeCallback, bcanvas_repaint,
                      (XtPointer)c);
         ...
         */
        b->action =
            XtVaCreateWidget("action", xmFormWidgetClass, b->outer_pane, NULL);

        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 29, "Schedule..."));
        b->schedule = XtVaCreateWidget("schedule", xmPushButtonGadgetClass,
                                       b->action, XmNlabelString, xmstr,
                                       XmNtopAttachment, XmATTACH_FORM,
                                       XmNleftAttachment, XmATTACH_FORM, NULL);
        XmStringFree(xmstr);
        XtAddCallback(b->schedule, XmNactivateCallback, schedule_cb,
                      (XtPointer)c);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 30, "Mail..."));
        b->mail = XtVaCreateWidget("mail", xmPushButtonGadgetClass, b->action,
                                   XmNlabelString, xmstr, XmNleftAttachment,
                                   XmATTACH_WIDGET, XmNleftWidget, b->schedule,
        ...
        b->cancel = XtVaCreateWidget(
            "cancel", xmPushButtonGadgetClass, b->action, XmNlabelString, xmstr,
            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, b->mail,
            XmNtopAttachment, XmATTACH_FORM, NULL);
        XmStringFree(xmstr);
        ...
        b->helpbutton = XtVaCreateWidget(
            "help", xmPushButtonGadgetClass, b->action, XmNlabelString, xmstr,
            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, b->cancel,
            XmNtopAttachment, XmATTACH_FORM, NULL);
        XmStringFree(xmstr);
        XtAddCallback(b->helpbutton, XmNactivateCallback,
                      (XtCallbackProc)help_cb, COMPARE_CALS_HELP_BUTTON);
        XtAddCallback(b->action, XmNhelpCallback, (XtCallbackProc)help_cb,
                      (XtPointer)COMPARE_CALS_HELP_BUTTON);

        b->message_text = XtVaCreateWidget(
            "message", xmLabelGadgetClass, b->action, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, b->schedule, XmNleftAttachment,
            XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
            XmNbottomAttachment, XmATTACH_FORM, XmNalignment,
        ...
        XtVaSetValues(b->action, XmNpaneMaximum, (2 * height), XmNpaneMinimum,
                      (2 * height), NULL);

        XtVaGetValues(b->outer_pane, XmNchildren, &children, XmNnumChildren,
                      &num_children, NULL);

        while (num_children-- > 0) {
                if (XmIsSash(children[num_children])) {
                        XtVaSetValues(children[num_children], XmNtraversalOn,
                                      False, NULL);
                }
        }
...
static void browselist_from_browser(Widget w, XtPointer client_data,
                                    XtPointer call_data) {
...
static void goto_date_cb(Widget w, XtPointer client_data, XtPointer call_data) {
        Calendar *c = (Calendar *)client_data;
        Browser *b;
...
static void goto_unit(Calendar *c, int item_no) {
        char buf[DATESIZ];
        Browser *b;
        Props *p;
        OrderingType ord_t;
...
static void bcanvas_repaint(Widget w, XtPointer client_data,
                            XtPointer call_data) {
        Calendar *c = (Calendar *)client_data;
        Browser *b;
        XRectangle clip;
...
static void mb_display_footermess(Browser *b, Calendar *c) {
        int num_cals;
        char buf[BUFSIZ];
...
static void browser_to_gaccess_list(Calendar *c) {
...
void mb_update_busystatus(Browser *b, Calendar *c) {
        ...
        while ((bd = (BlistData *)CmDataListGetData(bl->blist_data, j))) {
                ...
                if (bd->cache == NULL) {
                        ...
                        stat =
                            csa_list_entries(bd->cal_handle, r_cnt, range_attrs,
                                             ops, &num_entries, &entries, NULL);
                        free_range(&range_attrs, &ops, r_cnt);
                        backend_err_msg(
                            b->frame, bd->name, stat,
...
static void bcanvas_event(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        Calendar *c = (Calendar *)client_data;
        Browser *b;
        XmDrawingAreaCallbackStruct *cbs =
...
static void schedule_cb(Widget w, XtPointer client_data, XtPointer call_data) {
        Calendar *c = (Calendar *)client_data;
        Browser *b;
        Props *p;
        OrderingType ord_t;
...
static void mail_cb(Widget w, XtPointer client_data, XtPointer call_data) {
        Calendar *c = (Calendar *)client_data;
        Props *p = (Props *)c->properties;
        Browser *b = (Browser *)c->browser;
        Tt_message msg;
...
nt */
        ...
        appointment_buf = parse_attrs_to_string(
            appt, p, attrs_to_string(appt->attrs, appt->count));
...
static void gotomenu_cb(Widget w, XtPointer data, XtPointer cbs) {
...
extern void mb_update_segs(Browser *b, Tick tick, Tick dur, int *start_index,
                           int *end_index) {
        ...
        if (start_hour < get_int_prop(p, CP_DAYBEGIN)) {
                start = 0;
                duration =
                    dur - ((double)(get_int_prop(p, CP_DAYBEGIN) -
                                    ((double)start_hour +
                                     (double)minute(tick) / (double)60)) *
                           (double)hrsec);
        } else {
        ...
        *start_index = (double)start / (double)MINS_IN_SEG +
                       (nday * (b->segs_in_array / 7));
...
void mb_update_array(char *entry_text, Calendar *c) {
        ...
        if (bd->cache == NULL) {
                setup_range(&range_attrs, &ops, &r_cnt, start, stop,
                            CSA_TYPE_EVENT, 0, B_FALSE, bd->version);
                stat = csa_list_entries(bd->cal_handle, r_cnt, range_attrs, ops,
        ...
        for (i = 0; i < bd->cache_size; i++) {
                ...
                mb_update_segs(b, start_tick, end_tick - start_tick, &start_ind,
                               &end_ind);
        }
}

static Boolean register_names(char *name, Calendar *c) {
        int i;
        char *user, *location;
        ...
        if (!bd) {
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 1070,
                            "Calendar : Error - Compare Calendars"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 607,
        ...
        flags =
            CSA_CB_ENTRY_ADDED | CSA_CB_ENTRY_UPDATED | CSA_CB_ENTRY_DELETED;
        ...
        stat =
            csa_register_callback(cal, flags, mb_update_handler, NULL, &cb_ext);
        if (stat != CSA_SUCCESS) {
                backend_err_msg(
                    b->frame, name, stat,
                    ((Props_pu *)c->properties_pu)->xm_error_pixmap);
...
extern void mb_deregister_names(char *name, Calendar *c) {
        int i;
        Browser *b = (Browser *)c->browser;
        Props_pu *p = (Props_pu *)c->properties_pu;
        BlistData *bd;
...
static void mb_box_notify(Widget widget, XtPointer client_data,
                          XtPointer call_data) {
...
extern void mb_clear_selected_calendar(char *name, Calendar *c) {
...
extern void mb_init_datefield(Browser *b, Calendar *c) {
        char *date;
        Props *p = (Props *)c->properties;
...
static void mb_init_browchart(Browser *b, Calendar *c) {
        ...
        XtVaGetValues(b->canvas, XmNwidth, &canvas_width, XmNheight,
                      &canvas_height, NULL);
...
extern void mb_draw_chartgrid(Browser *b, Calendar *c) {
        ...
        /* draw hour labels */
        for (n = get_int_prop(p, CP_DAYBEGIN); n <= get_int_prop(p, CP_DAYEND);
             n++) {
                ...
                gr_draw_line(xc, x, y, x + b->chart_width, y, gr_solid, NULL);
                y += b->boxh;
        }
        ...
        /* draw month */
        format_date(b->begin_week_tick + 1, get_int_prop(p, CP_DATEORDERING),
                    buf, 160, 0, 0, 0);
        gr_text(xc, c->view->outside_margin + 4, dayy - char_height - 4,
                c->fonts->labelfont, buf, NULL);

        for (n = 0; n < 7; n++) {
                if (daytick >= get_bot() && daytick < get_eot()) {

                        CalTextExtents(c->fonts->viewfont, days3[n + 1],
                                       strlen(days3[n + 1]), &nop, &nop,
                                       &s_width, &nop);
                        ...
                        CalTextExtents(c->fonts->viewfont,
                                       numbers[dom(daytick)],
                                       strlen(numbers[dom(daytick)]), &nop,
                                       &nop, &s_width, &nop);
                ...
                daytick += daysec;
                gr_draw_line(xc, b->chart_x + (b->boxw * n), y,
                             b->chart_x + (b->boxw * n), y + b->chart_height,
                             gr_solid, NULL);
        }
         ...
         */
        gr_draw_box(xc, b->chart_x, b->chart_y, b->chart_width, b->chart_height,
                    NULL);
        gr_draw_box(xc, b->chart_x - 1, b->chart_y - 1, b->chart_width + 2,
                    b->chart_height + 2, NULL);
}
...
void mb_resize_proc(Widget w, XtPointer client_data, XtPointer call) {
...
void mb_refigure_chart(Calendar *c) {
        mb_resize_proc(((Browser *)c->browser)->canvas, (XtPointer)c, NULL);
}

extern void browser_reset_list(Calendar *c) {
        int i;
        Browser *b = (Browser *)c->browser;
        BlistData *bd;
        Browselist *bl = (Browselist *)c->browselist;
        ...
        XtVaGetValues(b->browse_list, XmNselectedItemCount,
                      &selected_items_count, XmNselectedItems,
                      &list_selected_items, NULL);

        selected_items =
            (XmStringTable)calloc(selected_items_count, sizeof(XmStringTable));
...
extern void init_browser(Calendar *c) {
        pr_pos xy;
        Browser *b = (Browser *)c->browser;
...
static void popup_cb(Widget w, XtPointer client, XtPointer call) {
        Calendar *c = (Calendar *)client;
        Browser *b = (Browser *)c->browser;
        Position x, y;
 ...
 */
static void mb_update_handler(CSA_session_handle cal, CSA_flags reason,
