...
static Boolean load_icon(Calendar *c, IconInfo *icon, char *icon_name, int size,
                         Pixel, Pixel);

static int wm_icon_preference(Display *dpy, Screen *screen, int size) {
        int icon_size_count, cal_icon_size, i;
        XIconSize *icon_sizes;

        if (XGetIconSizes(dpy, XRootWindowOfScreen(screen), &icon_sizes,
                          &icon_size_count) == 0)
                return (size);
...
static Boolean load_icons(Calendar *c) {
        Pixel fg, bg;
        int resolution, size;
        ...
        XtVaGetValues(c->canvas, XmNforeground, &fg, XmNbackground, &bg, NULL);
        ...
        if (c->icon->icon_depth >= 8) {
                if (!load_icon(c, c->icon_inverted, DTCM_INV_ICON, size, fg,
                               bg))
                        return (FALSE);
        } else {
                /* Reverse fg/bg on STD_ICON to get inverted icon. */
                /* XXX: This probably won't work on 1 < depth < 8 visuals */
                if (!load_icon(c, c->icon_inverted, DTCM_STD_ICON, size, bg,
                               fg))
                        return (FALSE);
        }
        return (TRUE);
...
static Boolean load_icon(Calendar *c, IconInfo *icon, char *icon_name, int size,
        ...
        if (!(icon_filename =
                  XmGetIconFileName(screen, NULL, icon_name, NULL, size))) {
                icon->icon = 0;
                return (FALSE);
        }
        ...
        XGetGeometry(XtDisplay(c->frame), icon->icon, &root, &x, &y,
                     &icon->icon_width, &icon->icon_height, &border_width,
                     &icon->icon_depth);
...
static void free_icon(Calendar *c, IconInfo *icon) {
        if (!icon)
                return;
...
static void paint_icon_date(Calendar *c) {
        Tick current_time;
        char buf[25];
        Cal_Font *font = c->fonts->iconfont;
        new_XContext *xc;
        int len, x, y, width, height, nop;
        ...
        if (c->icon->icon_depth == 1) {
                XtVaGetValues(c->canvas, XmNforeground, &fg, XmNbackground, &bg,
                              NULL);
        } else {
        ...
        gc = XCreateGC(xc->display, c->icon->icon,
                       GCForeground | GCFunction | GCFillStyle, &gc_vals);
        ...
        if (c->icon_inverted && c->icon_inverted->icon &&
            c->icon_inverted->icon_mask)
                CalDrawString(dpy, c->icon_inverted->icon, font, gc, x, y, buf,
                              len);
...
void paint_icon(Calendar *c) {
        ...
        if (c->icon && c->icon->icon && c->icon->icon_mask)
                XtVaSetValues(c->frame, XmNiconPixmap, c->icon->icon,
                              XmNiconMask, c->icon->icon_mask, NULL);
}
