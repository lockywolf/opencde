...
#ifdef GR_DEBUG
Boolean inDebugMode(Calendar *c) {
...
static Boolean pd_set_start_date(Calendar *c, Tick tick) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
        ...
        if (XtIsManaged(pd->from_day)) {
                nargs = 0;
                XtSetArg(args[nargs], XmNposition, dom(tick));
                nargs++;
                XtSetArg(args[nargs], XmNmaximumValue, monthlength(tick));
                nargs++;
...
static Boolean pd_set_end_date(Calendar *c, Tick tick) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
        Props *p = (Props *)c->properties;
        OrderingType ot = get_int_prop(p, CP_DATEORDERING);
        SeparatorType st = get_int_prop(p, CP_DATESEPARATOR);
...
static Boolean pd_set_max_start_date(Calendar *c, Tick tick) {
        Tick curTick = pd_get_start_date(c);

        return (tick < curTick) ? pd_set_start_date(c, tick) : True;
}

static Boolean pd_set_min_end_date(Calendar *c, Tick tick) {
        Tick curTick = pd_get_end_date(c);

        return (tick > curTick) ? pd_set_end_date(c, tick) : True;
}

static Boolean pd_set_report_type(Calendar *c, int reportType) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
        ...
        nargs = 0;
        XtSetArg(args[nargs], XmNsubMenuId, &optionMenu);
        nargs++;
        ...
        if (optionMenu != (Widget)NULL) {
                ...
                nargs = 0;
                XtSetArg(args[nargs], XmNchildren, &optionChildren);
                nargs++;
...
static Boolean pd_set_report_managed(Calendar *c, int reportType) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
        Arg args[10];
        int nargs;
...
static Boolean pd_get_start_positions(Calendar *c, int *m, int *d, int *y) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
...
static Boolean pd_get_end_positions(Calendar *c, int *m, int *d, int *y) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
        int dayPos = 1, monthPos = 0, yearPos;
...
static void from_modify_verify_cb(Widget w, XtPointer uDataP,
                                  XtPointer cbDataP) {
        ...
        if (pd_set_start_date(c, newTick) && pd_set_min_end_date(c, newTick)) {
                ...
                if (cbStruct->widget == pd->from_day)
                        ...
                        cbStruct->position = yearPos;
                cbStruct->doit = True;
        }
}

static void to_modify_verify_cb(Widget w, XtPointer uDataP, XtPointer cbDataP) {
        Calendar *c = (Calendar *)uDataP;
        XmSpinBoxCallbackStruct *cbStruct = (XmSpinBoxCallbackStruct *)cbDataP;
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
        Tick newTick;
...
static void spin_field_changed_cb(Widget field, XtPointer uData,
                                  XtPointer cbData) {
        char *newStr;
        int curPos;
        int newPos;
        ...
        nargs = 0;
        XtSetArg(args[nargs], XmNspinBoxChildType, &sbcType);
        nargs++;
        XtSetArg(args[nargs], XmNmaximumValue, &maxValue);
        nargs++;
        XtSetArg(args[nargs], XmNminimumValue, &minValue);
        nargs++;
        XtSetArg(args[nargs], XmNnumValues, &nValues);
        nargs++;
        XtSetArg(args[nargs], XmNposition, &curPos);
        nargs++;
        XtSetArg(args[nargs], XmNvalues, &strings);
        nargs++;
        XtSetArg(args[nargs], XmNvalue, &newStr);
        nargs++;
        XtSetArg(args[nargs], XmNcursorPosition, &cursorPosition);
        nargs++;
        ...
        if (newPos != curPos) {
                nargs = 0;
                XtSetArg(args[nargs], XmNposition, newPos);
                nargs++;
                XtSetValues(field, args, nargs);

                /* After the above does its thing, restore original cursor
                 * position. */
                nargs = 0;
                XtSetArg(args[nargs], XmNcursorPosition, cursorPosition);
                nargs++;
...
static void more_opts_cb(Widget w, XtPointer uData, XtPointer cbData) {
...
static Tick pd_modify_day(Calendar *c, int reason, Boolean isTo) {
        int monthPos, dayPos, yearPos;
        int maxDay;
        Tick tmpTick;
        Tick curTick;
        ...
        case XmCR_SPIN_NEXT:
                ...
                newTick = (reportType == PR_WEEK_VIEW) ? nextweek(curTick)
                                                       : nextday(curTick);
                break;
        ...
        case XmCR_SPIN_PRIOR:
                ...
                if (dayPos >= maxDay)
                        ...
                        dayPos++;
                curTick = monthdayyear(monthPos + 1, dayPos, yearPos);
                newTick = (reportType == PR_WEEK_VIEW) ? prevweek(curTick)
                                                       : prevday(curTick);
                break;
        ...
        case XmCR_SPIN_FIRST:
                ...
                if (reportType == PR_WEEK_VIEW) {
                        if (isTo) {
                                ...
                                if (dayOffset > 0)
                                        newTick =
                                            next_ndays(newTick, 7 - dayOffset);
                        }
                }
                break;
...
static Tick pd_modify_month(Calendar *c, int reason, Boolean isTo) {
        int monthPos, dayPos, yearPos;
        int maxDay;
        Tick tmpTick;
        Tick curTick;
        ...
        case XmCR_SPIN_NEXT:
                ...
                if (reportType == PR_WEEK_VIEW)
                        ...
                        if (dayPos > maxDay)
                                dayPos = maxDay;
                        newTick =
                            monthdayyear(month(tmpTick), dayPos, year(tmpTick));
                }
                break;
...
static Tick pd_modify_year(Calendar *c, int reason, Boolean isTo) {
        int monthPos, dayPos, yearPos;
        int maxDay;
        Tick tmpTick;
        Tick curTick;
        ...
        case XmCR_SPIN_NEXT:
                ...
                if (reportType == PR_WEEK_VIEW)
                        ...
                        if (dayPos > maxDay)
                                dayPos = maxDay;
                        newTick =
                            monthdayyear(monthPos + 1, dayPos, yearPos + 1);
                }
                break;
 ...
 */
void post_print_dialog(Calendar *c) {
        _DtCmPrintData *pd;
        ...
        if ((pd = (_DtCmPrintData *)c->print_data) != (_DtCmPrintData *)NULL) {
                if (!XtIsManaged(pd->pdb))
                        ...
                        XRaiseWindow(XtDisplay(pd->pdb),
                                     XtWindow(XtParent(pd->pdb)));
        }
}

static void clearSetupData(_DtCmPrintData *pd) {
...
static void createPrintShell(Calendar *c) {
#if 0 && defined(PRINTING_SUPPORTED)
  _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
...
void create_print_dialog(Calendar *c) {
        XmString xmstr, day_view, week_view, month_view, year_view, appt_list,
            todo_list, view;
        _DtCmPrintData *pd;
        ...
        if ((pd = (_DtCmPrintData *)c->print_data) == NULL) {
                ...
                print_file =
                    get_char_prop((Props *)c->properties, CP_PRINTFILENAME);
                if (!print_file)
                        print_file = "";
                print_dir =
                    get_char_prop((Props *)c->properties, CP_PRINTDIRNAME);
                if (!print_dir)
                        print_dir = "";
                snprintf(fnamebuf, BUFSIZ, "%s/%s", print_dir, print_file);

                title = XtNewString(
                    catgets(c->DT_catd, 1, 728, "Calendar : Print"));
                nargs = 0;
                XtSetArg(args[nargs], XmNtitle, title);
                nargs++;
                XtSetArg(args[nargs], XmNdeleteResponse, XmUNMAP);
                nargs++;
                XtSetArg(args[nargs], DtNfileName, fnamebuf);
                nargs++;
                XtSetArg(args[nargs], XmNautoUnmanage, False);
                nargs++;
                XtSetArg(args[nargs], XmNdeleteResponse, XmDO_NOTHING);
                nargs++;
                ...
                XtAddCallback(pd->pdb, DtNcancelCallback, cancel_cb,
                              (XtPointer)c);
                XtAddCallback(pd->pdb, DtNprintCallback, print_cb,
                              (XtPointer)c);
                XtAddCallback(pd->pdb, DtNclosePrintDisplayCallback,
                              close_print_display_cb, (XtPointer)c);
                XtAddCallback(pd->pdb, DtNsetupCallback, print_setup_cb,
                              (XtPointer)c);

                helpButton = XtNameToWidget(pd->pdb, "Help");
                if (helpButton != (Widget)NULL)
                        XtAddCallback(helpButton, XmNactivateCallback, help_cb,
                                      (XtPointer)PRINT_HELP_BUTTON);
                XtAddCallback(pd->pdb, XmNhelpCallback, help_cb,
                              (XtPointer)PRINT_HELP_BUTTON);

                nargs = 0;
                XtSetArg(args[nargs], XmNfractionBase, 3);
                nargs++;
                ...
                pd->report_type_option = XmVaCreateSimpleOptionMenu(
                    pd->form, "TypeOptionMenu", view, 0, pd->report_type,
                    report_option_cb, XmVaPUSHBUTTON, day_view, NULL, NULL,
                    NULL, XmVaPUSHBUTTON, week_view, NULL, NULL, NULL,
                    XmVaPUSHBUTTON, month_view, NULL, NULL, NULL,
                    XmVaPUSHBUTTON, year_view, NULL, NULL, NULL, XmVaPUSHBUTTON,
                    appt_list, NULL, NULL, NULL, XmVaPUSHBUTTON, todo_list,
                    NULL, NULL, NULL, XmNorientation, XmVERTICAL,
                    XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 10,
                    XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 1,
                    XmNleftOffset, 5, XmNmarginWidth, 0, XmNnavigationType,
                    XmTAB_GROUP, NULL);
                ...
                xmstr = XmStringCreateLocalized(
                    catgets(c->DT_catd, 1, 731, "From:"));
                pd->from_label = XtVaCreateWidget(
                    "FromLabel", xmLabelGadgetClass, pd->form, XmNlabelString,
                    xmstr, XmNleftAttachment, XmATTACH_POSITION,
                    XmNleftPosition, 0, XmNleftOffset, 10, XmNtopAttachment,
                    XmATTACH_OPPOSITE_WIDGET, XmNtopWidget,
                    pd->report_type_option, XmNtopOffset, 0, NULL);
                XmStringFree(xmstr);
                ...
                nargs++;
                XtSetArg(args[nargs], XmNleftWidget, pd->from_label);
                nargs++;
                XtSetArg(args[nargs], XmNleftOffset, 0);
                nargs++;
                XtSetArg(args[nargs], XmNtopAttachment, XmATTACH_WIDGET);
                nargs++;
                XtSetArg(args[nargs], XmNtopWidget, pd->from_label);
                nargs++;
                XtSetArg(args[nargs], XmNtopOffset, 4);
                nargs++;
                XtSetArg(args[nargs], XmNshadowThickness, 1);
                nargs++;
                pd->from_spin =
                    XmCreateSpinBox(pd->form, "FromSpinBox", args, nargs);
                XtAddCallback(pd->from_spin, XmNmodifyVerifyCallback,
                              from_modify_verify_cb, (XtPointer)c);

                nargs = 0;
                XtSetArg(args[nargs], XmNspinBoxChildType, XmNUMERIC);
                nargs++;
                XtSetArg(args[nargs], XmNminimumValue, 1);
                nargs++;
                XtSetArg(args[nargs], XmNmaximumValue, 31);
                nargs++;
                XtSetArg(args[nargs], XmNcolumns, 2);
                nargs++;
                XtSetArg(args[nargs], XmNmaxLength, 2);
                nargs++;
                XtSetArg(args[nargs], XmNposition, 1);
                nargs++;
                pd->from_day =
                    XmCreateTextField(pd->from_spin, "FromDay", args, nargs);
                ...
                for (i = 0; i < 12; i++) {
                        abbrevMonths[i] =
                            XmStringCreateLocalized(months2[i + 1]);
                        if (cm_strlen(months2[i + 1]) > maxMonthLen)
                                maxMonthLen = cm_strlen(months2[i + 1]);
                }
                nargs = 0;
                XtSetArg(args[nargs], XmNspinBoxChildType, XmSTRING);
                nargs++;
                XtSetArg(args[nargs], XmNnumValues, 12);
                nargs++;
                XtSetArg(args[nargs], XmNvalues, abbrevMonths);
                nargs++;
                XtSetArg(args[nargs], XmNcolumns, maxMonthLen);
                nargs++;
                XtSetArg(args[nargs], XmNmaxLength, maxMonthLen);
                nargs++;
                XtSetArg(args[nargs], XmNposition, 0);
                nargs++;
                pd->from_month =
                    XmCreateTextField(pd->from_spin, "FromMonth", args, nargs);
                ...
                XtSetArg(args[nargs], XmNspinBoxChildType, XmNUMERIC);
                nargs++;
                XtSetArg(args[nargs], XmNminimumValue, year(get_bot()));
                nargs++;
                XtSetArg(args[nargs], XmNmaximumValue, year(get_eot()));
                nargs++;
                XtSetArg(args[nargs], XmNcolumns, 4);
                nargs++;
                XtSetArg(args[nargs], XmNmaxLength, 4);
                nargs++;
                XtSetArg(args[nargs], XmNposition, year(get_bot()));
                nargs++;
                XtSetArg(args[nargs], XmNwrap, False);
                nargs++;
                pd->from_year =
                    XmCreateTextField(pd->from_spin, "FromYear", args, nargs);
                ...
                xmstr =
                    XmStringCreateLocalized(catgets(c->DT_catd, 1, 732, "To:"));
                pd->to_label = XtVaCreateWidget(
                    "ToLabel", xmLabelGadgetClass, pd->form, XmNlabelString,
                    xmstr, XmNleftAttachment, XmATTACH_POSITION,
                    XmNleftPosition, 2, XmNleftOffset, 10, XmNtopAttachment,
                    XmATTACH_OPPOSITE_WIDGET, XmNtopWidget,
                    pd->report_type_option, XmNtopOffset, 0, NULL);
                XmStringFree(xmstr);
                ...
                XtSetArg(args[nargs], XmNleftAttachment,
                         XmATTACH_OPPOSITE_WIDGET);
                nargs++;
                XtSetArg(args[nargs], XmNleftWidget, pd->to_label);
                nargs++;
                XtSetArg(args[nargs], XmNleftOffset, 0);
                nargs++;
                XtSetArg(args[nargs], XmNtopAttachment, XmATTACH_WIDGET);
                nargs++;
                XtSetArg(args[nargs], XmNtopWidget, pd->to_label);
                nargs++;
                XtSetArg(args[nargs], XmNtopOffset, 4);
                nargs++;
                XtSetArg(args[nargs], XmNshadowThickness, 1);
                nargs++;
                pd->to_spin =
                    XmCreateSpinBox(pd->form, "ToSpinBox", args, nargs);
                ...
                nargs++;
                pd->to_day =
                    XmCreateTextField(pd->to_spin, "ToDay", args, nargs);
                ...
                nargs++;
                pd->to_month =
                    XmCreateTextField(pd->to_spin, "ToMonth", args, nargs);
                ...
                nargs++;
                pd->to_year =
                    XmCreateTextField(pd->to_spin, "ToYear", args, nargs);
                ...
                xmstr = XmStringCreateLocalized(
                    catgets(c->DT_catd, 1, 679, "More..."));
                nargs = 0;
                XtSetArg(args[nargs], XmNlabelString, xmstr);
                nargs++;
                XtSetArg(args[nargs], XmNrightAttachment, XmATTACH_FORM);
                nargs++;
                XtSetArg(args[nargs], XmNrightOffset, 10);
                nargs++;
                XtSetArg(args[nargs], XmNtopAttachment, XmATTACH_WIDGET);
                nargs++;
                XtSetArg(args[nargs], XmNtopWidget, pd->to_spin);
                nargs++;
                XtSetArg(args[nargs], XmNtopOffset, 10);
                nargs++;
                pd->more_opts =
                    XmCreatePushButtonGadget(pd->form, "MoreOpts", args, nargs);
                XtAddCallback(pd->more_opts, XmNactivateCallback, more_opts_cb,
                              (XtPointer)c);
                ...
                nargs++;
                XtSetArg(args[nargs], XmNleftOffset, 10);
                nargs++;
                XtSetArg(args[nargs], XmNtopAttachment,
                         XmATTACH_OPPOSITE_WIDGET);
                nargs++;
                XtSetArg(args[nargs], XmNtopWidget, pd->more_opts);
                nargs++;
                XtSetArg(args[nargs], XmNtopOffset, 10);
                nargs++;
                pd->debugToggle = XmCreateToggleButtonGadget(
                    pd->form, "Debug Mode", args, nargs);
        ...
        case dayGlance:
                reportType = PR_DAY_VIEW;
                break;
        case weekGlance:
                reportType = PR_WEEK_VIEW;
                break;
        case yearGlance:
                reportType = PR_YEAR_VIEW;
                break;
        default:
        case monthGlance:
                reportType = PR_MONTH_VIEW;
                break;
        }
 ...
 * Close dialog
 */
static void cancel_cb(Widget w, XtPointer data, XtPointer ignore) {
...
static void report_error(Calendar *c, char *title, char *errText) {
        ...
        dialog_popup(c->frame, DIALOG_TITLE, title, DIALOG_TEXT, errText,
                     BUTTON_IDENT, 1, label, DIALOG_IMAGE, pu->xm_error_pixmap,
                     NULL);

        XtFree(label);
}

static void pdm_notify_cb(Widget w, XtPointer uData, XtPointer cbData) {
#if 0 && defined(PRINTING_SUPPORTED)
 ...
 */
static void close_print_display_cb(Widget w, XtPointer uData,
                                   XtPointer cbData) {
        Calendar *c = (Calendar *)uData;
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
 ...
 */
static void print_cb(Widget w, XtPointer data, XtPointer cbDataP) {
        Calendar *c = (Calendar *)data;
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
...
static void print_setup_cb(Widget w, XtPointer uData, XtPointer cbData) {
#if 0 && defined(PRINTING_SUPPORTED)
  Calendar *c = (Calendar *)uData;
  _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
  DtPrintSetupCallbackStruct *cbStruct =
...
void print_report(Calendar *c) {
        ...
        if ((start_date == DATE_BBOT) || (end_date == DATE_BBOT) ||
            (start_date == DATE_AEOT) || (end_date == DATE_BBOT)) {
                ...
                text = XtNewString(
                    catgets(c->DT_catd, 1, 894, "Malformed \"From\" date"));
        } else if (end_date <= 0) {
                text = XtNewString(
                    catgets(c->DT_catd, 1, 896, "Malformed \"To\" date"));
        } else if (start_date > end_date) {
                text = XtNewString(catgets(c->DT_catd, 1, 898,
                                           "The \"To\" date for printing must "
                                           "be after the \"From\" date for "
                                           "printing"));
        }

        if (text) {
                char *title =
                    XtNewString(catgets(c->DT_catd, 1, 736, printErrorTitle));
        ...
        if (!pd->setupDataValid) {
                if (DtPrintFillSetupData(pd->pdb, &pd->setupData) !=
                    DtPRINT_SUCCESS) {
                        /*
                         * NOTE: DtPrintFillSetupData() displays an error dialog
                         * if it is unsuccessful; no need to display our own.
                         */
                        return;
                }
        ...
        case PR_APPT_LIST:
                ...
                break;
        case PR_TODO_LIST:
                x_print_list_range(c, CSA_TYPE_TODO, (int)VIEW_ALL, start_date,
                                   end_date);
                break;
        }
 ...
 */
static void report_option_cb(Widget w, XtPointer client_data,
                             XtPointer call_data) {
...
char *pd_get_printer_name(Calendar *c) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;

        if ((pd == (_DtCmPrintData *)NULL) || (!pd->setupDataValid) ||
            pd_print_to_file(c))
                return (char *)NULL;

        return pd->setupData.printer_name
                   ? XtNewString(pd->setupData.printer_name)
                   : (char *)NULL;
}

int pd_get_copies(Calendar *c) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
        int nCopies = 0;
...
Tick pd_get_start_date(Calendar *c) {
...
Tick pd_get_end_date(Calendar *c) {
        int dayPos, monthPos, yearPos;
...
char *pd_get_print_options(Calendar *c) { return XtNewString(""); }

Boolean pd_print_to_file(Calendar *c) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
...
char *pd_get_file_name(Calendar *c) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
        ...
        return pd->setupData.dest_info ? XtNewString(pd->setupData.dest_info)
                                       : (char *)NULL;
}

int pd_get_report_type(Calendar *c) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
...
Widget pd_get_print_shell(Calendar *c) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
...
Display *pd_get_print_display(Calendar *c) {
...
void pd_set_bad_alloc_error(Calendar *c, Boolean errorOn) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
...
Boolean pd_get_bad_alloc_error(Calendar *c) {
        _DtCmPrintData *pd = (_DtCmPrintData *)c->print_data;
