...
static void ge_mail_proc(Widget w, XtPointer client_data, XtPointer data);
static Dtcm_appointment *form_to_appt(GEditor *ge, Boolean no_reminders,
                                      int version);
...
**
*******************************************************************************/
static void ge_add_to_gappt_list(Access_data *ad, int idx, GEditor *ge,
                                 Boolean reset) {
        int cnt = 1;
        ...
        */
        appt = allocate_appt_struct(
            appt_read, ad->version, CSA_ENTRY_ATTR_START_DATE_I,
        ...
        */
        step = (cnt <= 1)
                   ? NULL
                   : (List_data *)CmDataListGetData(ge->list_data, cnt - 1);
        name1 = cm_target2name(ad->name);
        if (step && step->ad)
                name2 = cm_target2name(step->ad->name);
        if (!step || start_tick != step->tick || reset)
                format_gappt(appt, ad->name, buf, DEFAULT_GAPPT_LEN, dt,
                             DEFAULT_GAPPT_LEN);
        else {
                ...
                _csa_tick_to_iso8601(0,
                                     appt->time->value->item.date_time_value);
                if (!step || strcmp(ad->name, name2) != 0)
                        format_gappt(appt, ad->name, buf, DEFAULT_GAPPT_LEN, dt,
                                     DEFAULT_GAPPT_LEN);
                else
                        format_gappt(appt, NULL, buf, DEFAULT_GAPPT_LEN, dt,
                                     DEFAULT_GAPPT_LEN);
        }
...
*/
static void appt_to_form(GEditor *ge, CSA_entry_handle a, char *name,
                         int version) {
        char buf[MAXNAMELEN];
...
*/
static void ge_list_select_proc(Widget w, XtPointer client_data,
                                XtPointer data) {
        int cnt;
        ...
        if ((a = geditor_nth_appt(ge, cbs->item_position, &ad)))
                appt_to_form(ge, a, (ad && ad->name) ? ad->name : "\0",
                             ad->version);
        ...
        while (step_ad && strcmp(step_ad->name, ad->name) != 0)
                step_ad =
                    (Access_data *)CmDataListGetData(ge->access_data, ++cnt);
        if (step_ad)
...
static void ge_set_modify_buttons(GEditor *ge, int cnt) {
        char buf[MAXNAMELEN];
        Boolean val;

        if (cnt == 1)
                snprintf(
                    buf, MAXNAMELEN, "%d %s.", cnt,
...
*/
static void ge_access_select_proc(Widget w, XtPointer client_data,
                                  XtPointer data) {
        GEditor *ge = (GEditor *)client_data;
        XmListCallbackStruct *cbs = (XmListCallbackStruct *)data;
...
static Tt_message reply_cb(Tt_message m, void *c_data, Tttk_op op,
...
static char *get_mail_address_list(Calendar *c) {
...
nt */
        ...
        appointment_buf =
            parse_attrs_to_string(appt, (Props *)c->properties,
        ...
        msg = ttmedia_load(0, (Ttmedia_load_msg_cb)reply_cb, NULL,
                           TTME_MAIL_EDIT, "RFC_822_MESSAGE",
                           (unsigned char *)mime_buf, strlen(mime_buf), NULL,
                           "dtcm_appointment_attachment", 0);
...
*/
static void ge_build_expand(GEditor *ge) {
        Props *p = (Props *)ge->cal->properties;
        XmString xmstr;
        Calendar *c = ge->cal;
        Widget widgets[20];
        ...
        */
        ...
        XtVaSetValues(ge->rfp.rfp_form_mgr, XmNbottomAttachment,
                      XmATTACH_WIDGET, XmNbottomWidget, ge->separator2,
                      XmNbottomOffset, 5, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 5, NULL);
        XtVaGetValues(ge->rfp.rfp_form_mgr, XmNchildren, &children,
        ...
        ge->mail_button = XtVaCreateWidget(
            "mail", xmPushButtonWidgetClass, ge->base_form_mgr, XmNlabelString,
            xmstr, XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET,
            XmNbottomWidget, ge->rfp.rfp_form_mgr, XmNrightAttachment,
...
*/
static void ge_expand_ui_proc(Widget w, XtPointer client_data, XtPointer data) {
        GEditor *ge = (GEditor *)client_data;
        XmString xmstr;
        ...
        if (expand_state_closed) {
                ...
                XtSetSensitive(ge->rfp.rfp_form_mgr, False);
                xmstr = XmStringCreateLocalized(
                    catgets(ge->cal->DT_catd, 1, 626, "More"));
                XtVaSetValues(ge->expand_ui_button, XmNlabelString, xmstr,
                              NULL);
                XmStringFree(xmstr);
...
static Tick ge_reminder_val(Props *p, Props_op scope, Props_op unit) {
...
static Dtcm_appointment *form_to_appt(GEditor *ge, Boolean no_reminders,
        ...
        if (no_reminders) {
                ...
                if (version <= DATAVER2)
                        a = allocate_appt_struct(
                            appt_write, version, CSA_ENTRY_ATTR_START_DATE_I,
                            CSA_ENTRY_ATTR_TYPE_I, CSA_ENTRY_ATTR_SUBTYPE_I,
                            CSA_ENTRY_ATTR_CLASSIFICATION_I,
                            CSA_ENTRY_ATTR_END_DATE_I,
                            CSA_X_DT_ENTRY_ATTR_SHOWTIME_I,
                            CSA_ENTRY_ATTR_SUMMARY_I, CSA_ENTRY_ATTR_STATUS_I,
                            CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I,
                            CSA_X_DT_ENTRY_ATTR_REPEAT_TIMES_I, NULL);
                        ...
                        a = allocate_appt_struct(
                            ...
                            CSA_X_DT_ENTRY_ATTR_SEQUENCE_END_DATE_I, NULL);
                ...
                _csa_duration_to_iso8601(
                    ge_reminder_val(p, CP_BEEPUNIT, CP_BEEPADV),
                ...
                a->flash->value->item.reminder_value->lead_time =
                    malloc(BUFSIZ);
                _csa_duration_to_iso8601(
                    ge_reminder_val(p, CP_FLASHUNIT, CP_BEEPADV),
                    a->flash->value->item.reminder_value->lead_time);
                ...
                a->popup->value->item.reminder_value->lead_time =
                    malloc(BUFSIZ);
                _csa_duration_to_iso8601(
                    ge_reminder_val(p, CP_OPENUNIT, CP_BEEPADV),
                    a->popup->value->item.reminder_value->lead_time);
                ...
                _csa_duration_to_iso8601(
                    ge_reminder_val(p, CP_MAILUNIT, CP_BEEPADV),
        ...
        all_ok = dssw_form_to_appt(
            &ge->dssw, a, ge->cal->view->current_calendar, ge->cal->view->date);
        if (all_ok)
...
**  Action procedures
*/
static void ge_insert_proc(Widget w, XtPointer client_data, XtPointer data) {
        int i, c_cnt, *c_list;
        GEditor *ge = (GEditor *)client_data;
        Props_pu *p = (Props_pu *)ge->cal->properties_pu;
        Calendar *c = ge->cal;
        ...
        if (c_cnt <= 0) {
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 305,
                            "Calendar : Error - Group Appointment Editor"));
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 306, "Select a calendar and INSERT again."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                dialog_popup(ge->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
        ...
        for (i = 0; i < c_cnt; i++) {
                ...
                if (!ad)
                        ...
                        stat = csa_add_entry(ad->cal_handle, appt->count,
                                             appt->attrs, &new_a, NULL);
                }
...
static void ge_delete_proc(Widget w, XtPointer client_data, XtPointer data) {
        ...
        if (!XmListGetSelectedPos(ge->appt_list, &item_list, &item_cnt)) {
                ...
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                answer =
                    dialog_popup(ge->frame, DIALOG_TITLE, title, DIALOG_TEXT,
        ...
        if (!(entry = geditor_nth_appt(ge, item_list[0], &ad))) {
                ...
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                answer = dialog_popup(ge->frame, DIALOG_TITLE, title,
                                      DIALOG_TEXT, text, BUTTON_IDENT, 1,
                                      ident1, BUTTON_HELP, RESELECT_ERROR_HELP,
        ...
        XtVaGetValues(ge->access_list, XmNselectedItemCount, &c_cnt, NULL);
        if (c_cnt <= 0) {
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 305,
                            "Calendar : Error - Group Appointment Editor"));
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 315, "Select a calendar and DELETE again."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                answer =
                    dialog_popup(ge->frame, DIALOG_TITLE, title, DIALOG_TEXT,
        ...
        answer = 0;
        appt = allocate_appt_struct(appt_read, ad->version,
        ...
        if (appt->repeat_type->value) {
                if (appt->repeat_type->value->item.sint32_value !=
                    CSA_X_DT_REPEAT_ONETIME) {
                        ...
                        answer = dialog_popup(
                            ge->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
...
static void ge_change_proc(Widget w, XtPointer client_data, XtPointer data) {
        int *item_list = NULL, item_cnt = 0, c_cnt, i;
        GEditor *ge = (GEditor *)client_data;
        Calendar *c = ge->cal;
        ...
        if (c_cnt <= 0) {
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 329,
                            "Calendar : Group Appointment Editor - Change"));
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 330, "Select a calendar and CHANGE again."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                answer =
                    dialog_popup(ge->frame, DIALOG_TITLE, title, DIALOG_TEXT,
        ...
        answer = 0;
        appt = allocate_appt_struct(
            appt_read, ad->version, CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I,
            CSA_X_DT_ENTRY_ATTR_REPEAT_TIMES_I,
            CSA_X_DT_ENTRY_ATTR_REPEAT_INTERVAL_I,
            CSA_X_DT_ENTRY_ATTR_REPEAT_OCCURRENCE_NUM_I,
        ...
        if ((new_a = form_to_appt(
                 ge, (same_user(ad->name, c->calname)) ? False : True,
                 ad->version)) == NULL) {
                free_appt_struct(&appt);
                _DtTurnOffHourGlass(ge->frame);
                return;
            ...
            (appt->repeat_type->value->item.sint32_value ==
             CSA_X_DT_REPEAT_OTHER_YEARLY)) {
                ...
                answer = dialog_popup(
                    ...
                    (compare_repeat_info(appt, &(ge->rfp), ad->cal_handle,
                                         ad->version)
                         ? BUTTON_IDENT
                         : BUTTON_INSENSITIVE),
        ...
        stat = csa_update_entry_attributes(ad->cal_handle, old_a, scope,
                                           CSA_FALSE, new_a->count,
                                           new_a->attrs, &new_entry, NULL);
        backend_err_msg(ge->frame, ad->name, stat, p->xm_error_pixmap);
...
static void ge_clear_proc(Widget w, XtPointer client_data, XtPointer data) {
        GEditor *ge = (GEditor *)client_data;
...
static void ge_close_proc(Widget w, XtPointer client_data, XtPointer data) {
        GEditor *ge = (GEditor *)client_data;
...
static void ge_quit_handler(Widget w, XtPointer cdata, XtPointer data) {
        ge_close_proc(w, cdata, data);
}

static void FormGroupApptDragMotionHandler(Widget dragInitiator,
...
static void ge_make_editor(Calendar *c) {
        int cnt;
        Arg args[15];
        char *buf;
        GEditor *ge = (GEditor *)c->geditor;
        ...
        */
        cnt = 0;
        XtSetArg(args[cnt], XmNdeleteResponse, XmDO_NOTHING);
        ++cnt;
        XtSetArg(args[cnt], XmNallowShellResize, True);
        ++cnt;
        XtSetArg(args[cnt], XmNmappedWhenManaged, False);
        ++cnt;
        ...
        */
        cnt = 0;
        XtSetArg(args[cnt], XmNautoUnmanage, False);
        ++cnt;
        XtSetArg(args[cnt], XmNfractionBase, 100);
        ++cnt;
        ...
        */
        cnt = 0;
        XtSetArg(args[cnt], XmNalignment, XmALIGNMENT_BEGINNING);
        ++cnt;
        XtSetArg(args[cnt], XmNbottomAttachment, XmATTACH_FORM);
        ++cnt;
        XtSetArg(args[cnt], XmNbottomOffset, 2);
        ++cnt;
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_FORM);
        ++cnt;
        XtSetArg(args[cnt], XmNleftOffset, 2);
        ++cnt;
        XtSetArg(args[cnt], XmNrightAttachment, XmATTACH_FORM);
        ++cnt;
        XtSetArg(args[cnt], XmNrightOffset, 2);
        ++cnt;
        ge->message_text =
            XmCreateLabelGadget(ge->base_form_mgr, "message", args, cnt);
        /*
        **  Create insert, delete, change, and clear buttons in rc manager
        */

        ge->button_rc_mgr = XtVaCreateWidget(
            "button_rc_mgr", xmFormWidgetClass, ge->base_form_mgr,
            XmNautoUnmanage, False, XmNleftAttachment, XmATTACH_FORM,
            XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment,
            XmATTACH_WIDGET, XmNbottomWidget, ge->message_text, XmNtopOffset,
            10, XmNbottomOffset, 5, XmNhorizontalSpacing, 10, XmNfractionBase,
            8, NULL);

        cnt = 0;
        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 342, "Insert"));
        XtSetArg(args[cnt], XmNlabelString, xmstr);
        ++cnt;
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNleftPosition, 1);
        ++cnt;
        XtSetArg(args[cnt], XmNrightAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNrightPosition, 2);
        ++cnt;
        ge->insert_button =
            XmCreatePushButton(ge->button_rc_mgr, "insert", args, cnt);
        XmStringFree(xmstr);
        ...
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNleftPosition, 2);
        ++cnt;
        XtSetArg(args[cnt], XmNrightAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNrightPosition, 3);
        ++cnt;
        ge->change_button =
            XmCreatePushButton(ge->button_rc_mgr, "change", args, cnt);
        XmStringFree(xmstr);
        ...
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNleftPosition, 3);
        ++cnt;
        XtSetArg(args[cnt], XmNrightAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNrightPosition, 4);
        ++cnt;
        ge->delete_button =
            XmCreatePushButton(ge->button_rc_mgr, "delete", args, cnt);
        XmStringFree(xmstr);
        ...
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNleftPosition, 4);
        ++cnt;
        XtSetArg(args[cnt], XmNrightAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNrightPosition, 5);
        ++cnt;
        ge->clear_button =
            XmCreatePushButton(ge->button_rc_mgr, "clear", args, cnt);
        XmStringFree(xmstr);
        XtAddCallback(ge->clear_button, XmNactivateCallback, ge_clear_proc, ge);
        ...
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNleftPosition, 5);
        ++cnt;
        XtSetArg(args[cnt], XmNrightAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNrightPosition, 6);
        ++cnt;
        ge->close_button =
            XmCreatePushButton(ge->button_rc_mgr, "close", args, cnt);
        XmStringFree(xmstr);
        XtAddCallback(ge->close_button, XmNactivateCallback, ge_close_proc, ge);
        ...
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNleftPosition, 6);
        ++cnt;
        XtSetArg(args[cnt], XmNrightAttachment, XmATTACH_POSITION);
        ++cnt;
        XtSetArg(args[cnt], XmNrightPosition, 7);
        ++cnt;
        ge->help_button =
            XmCreatePushButton(ge->button_rc_mgr, "help", args, cnt);
        XmStringFree(xmstr);
        ...
        */
        cnt = 0;
        XtSetArg(args[cnt], XmNbottomAttachment, XmATTACH_WIDGET);
        ++cnt;
        XtSetArg(args[cnt], XmNbottomWidget, ge->button_rc_mgr);
        ++cnt;
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_FORM);
        ++cnt;
        XtSetArg(args[cnt], XmNleftOffset, 5);
        ++cnt;
        XtSetArg(args[cnt], XmNrightAttachment, XmATTACH_FORM);
        ++cnt;
        XtSetArg(args[cnt], XmNrightOffset, 5);
        ++cnt;
        ge->separator2 =
            XmCreateSeparatorGadget(ge->base_form_mgr, "separator2", args, cnt);

        cnt = 0;
        XtSetArg(args[cnt], XmNbottomAttachment, XmATTACH_WIDGET);
        ++cnt;
        XtSetArg(args[cnt], XmNbottomWidget, ge->separator2);
        ++cnt;
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_FORM);
        ++cnt;
        XtSetArg(args[cnt], XmNleftOffset, 5);
        ...
        ++cnt;
        ge->separator1 =
            XmCreateSeparatorGadget(ge->base_form_mgr, "separator1", args, cnt);
        ...
        */
        ...
        XtSetArg(args[cnt], XmNbottomAttachment, XmATTACH_WIDGET);
        ++cnt;
        XtSetArg(args[cnt], XmNbottomWidget, ge->separator1);
        ++cnt;
        XtSetArg(args[cnt], XmNbottomOffset, 3);
        ++cnt;
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_FORM);
        ++cnt;
        XtSetArg(args[cnt], XmNleftOffset, 5);
        ++cnt;
        ge->expand_ui_button = XmCreatePushButton(
            ge->base_form_mgr, "expand_ui_button", args, cnt);
        XmStringFree(xmstr);
        ...
        */
        cnt = 0;
        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 971, "Time Calendar"));
        XtSetArg(args[cnt], XmNlabelString, xmstr);
        ++cnt;
        XtSetArg(args[cnt], XmNtopAttachment, XmATTACH_FORM);
        ++cnt;
        XtSetArg(args[cnt], XmNtopOffset, 5);
        ++cnt;
        ...
        */
        ...
        XtVaSetValues(ge->dssw.dssw_form_mgr, XmNtopAttachment, XmATTACH_FORM,
                      XmNtopOffset, label_height + 5, XmNbottomAttachment,
                      XmATTACH_WIDGET, XmNbottomWidget, ge->expand_ui_button,
        ...
        ge->drag_source = XtVaCreateWidget(
            "drag_source", dtIconGadgetClass, ge->dssw.dssw_form_mgr,
        ...
        if ((Boolean)btn1_transfer != True)
                XtAddEventHandler(
                    XtParent(ge->drag_source), Button2MotionMask, False,
                    (XtEventHandler)FormGroupApptDragMotionHandler,
                    (XtPointer)c);

        if (p->drag_icon_xbm)
                XtVaSetValues(ge->drag_source, XmNpixmap, p->drag_icon_xbm,
                              NULL);

        XtVaSetValues(ge->dssw.what_scrollwindow, XmNrightAttachment,
                      XmATTACH_WIDGET, XmNrightWidget, ge->drag_source, NULL);
        ...
        */
        ...
        cnt = 0;
        XtSetArg(args[cnt], XmNheight,
                 4 * listfontextents.max_logical_extent.height);
        ++cnt;
        XtSetArg(args[cnt], XmNlistSizePolicy, XmCONSTANT);
        ++cnt;
        XtSetArg(args[cnt], XmNselectionPolicy, XmMULTIPLE_SELECT);
        ++cnt;
        XtSetArg(args[cnt], XmNwidth,
                 23 * listfontextents.max_logical_extent.width);
        ++cnt;
        XtSetArg(args[cnt], XmNdoubleClickInterval, 5);
        ++cnt;
        ge->access_list =
            XmCreateScrolledList(ge->base_form_mgr, "access_list", args, cnt);
        ...
        XtVaSetValues(ge->access_list_sw, XmNleftAttachment, XmATTACH_WIDGET,
                      XmNleftWidget, ge->dssw.dssw_form_mgr, XmNleftOffset, 5,
                      XmNrightAttachment, XmATTACH_FORM, XmNrightOffset, 5,
                      XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET,
                      XmNbottomWidget, ge->dssw.dssw_form_mgr, NULL);
        ...
        */
        cnt = 0;
        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 340, "Calendar"));
        XtSetArg(args[cnt], XmNlabelString, xmstr);
        ++cnt;
        XtSetArg(args[cnt], XmNbottomAttachment, XmATTACH_WIDGET);
        ++cnt;
        XtSetArg(args[cnt], XmNbottomWidget, ge->access_list_sw);
        ++cnt;
        XtSetArg(args[cnt], XmNbottomOffset, 5);
        ++cnt;
        XtSetArg(args[cnt], XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET);
        ++cnt;
        XtSetArg(args[cnt], XmNleftWidget, ge->access_list_sw);
        ++cnt;
        ge->access_list_label = XmCreateLabelGadget(
            ge->base_form_mgr, "access_list_label", args, cnt);
        XmStringFree(xmstr);
        ...
        XtSetArg(args[cnt], XmNleftWidget, ge->access_list_sw);
        ++cnt;
        XtSetArg(args[cnt], XmNleftOffset,
                 23 * listfontextents.max_logical_extent.width);
        ++cnt;
        second_access_label = XmCreateLabelGadget(
            ge->base_form_mgr, "second_access_label", args, cnt);
        XmStringFree(xmstr);
        ...
        */
        ...
        ++cnt;
        XtSetArg(args[cnt], XmNwidth, 200);
        ++cnt;
        XtSetArg(args[cnt], XmNdoubleClickInterval, 5);
        ++cnt;
        ge->appt_list =
            XmCreateScrolledList(ge->base_form_mgr, "ge_appt_list", args, cnt);
        ...
        XtVaSetValues(
            ge->appt_list_sw, XmNtopAttachment, XmATTACH_FORM, XmNtopOffset,
            label_height + 11, XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET,
            XmNleftWidget, ge->access_list_sw, XmNrightAttachment,
            XmATTACH_OPPOSITE_WIDGET, XmNrightWidget, ge->access_list_sw,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            ge->access_list_label, XmNbottomOffset, 10, NULL);
        XtManageChild(ge->appt_list);

        /*
        **  Move the label over to the top of the access list scrolling window
        */
        XtVaSetValues(ge->appt_list_label, XmNleftAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNleftWidget, ge->appt_list_sw,
                      NULL);

        XtVaSetValues(second_list_label, XmNleftAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNleftWidget, ge->appt_list_sw,
                      XmNleftOffset,
                      17 * listfontextents.max_logical_extent.width, NULL);

        XtVaSetValues(ge->button_rc_mgr, XmNdefaultButton, ge->insert_button,
                      NULL);
        XtVaSetValues(ge->button_rc_mgr, XmNcancelButton, ge->close_button,
                      NULL);
        XmProcessTraversal(ge->dssw.what_text, XmTRAVERSE_CURRENT);
        XtVaSetValues(ge->base_form_mgr, XmNinitialFocus, ge->dssw.what_text,
                      NULL);
...
**
*******************************************************************************/
extern void add_to_gaccess_list(char *name, CSA_session_handle cal_handle,
                                unsigned int user_access, int version,
                                GEditor *ge, Boolean select_val) {
        ...
        if (same_user(name, c->calname) || c->my_cal_handle == cal_handle) {
                ...
                if (version < DATAVER4) {
                        access = (user_access &
                                  (CSA_X_DT_INSERT_ACCESS | CSA_OWNER_RIGHTS))
                                     ? 'Y'
                                     : 'N';
                } else {
                        access =
        ...
        int buf_size =
            ACCESS_NAME_LEN +
            cm_strlen(catgets(c->DT_catd, 1, 348, "Insert Permission")) + 5;
...
extern void add_all_gappt(GEditor *ge) {
        ...
        date =
            get_date_from_widget(ge->cal->view->date, ge->dssw.date_text, o, s);
        if (!date)
                return;
        ...
        for (i = 1; i <= ge->access_data->count && stat == CSA_SUCCESS; i++) {
                ...
                if (!step || !step->cal_handle ||
                    !((step->version < DATAVER4 &&
                       step->entry_access &
                           (CSA_OWNER_RIGHTS | CSA_X_DT_BROWSE_ACCESS)) ||
                      (step->version >= DATAVER4 &&
                       step->entry_access &
                           (CSA_OWNER_RIGHTS | CSA_VIEW_PUBLIC_ENTRIES |
                ...
                csa_list_entries(step->cal_handle, range_count, range_attr, ops,
                                 &entry_count, &entry_list, NULL);
...
extern void geditor_clean_up(GEditor *ge) { remove_all_gaccess(ge); }

/*
**  Return the nth appointment from the array
*/
extern CSA_entry_handle geditor_nth_appt(GEditor *ge, int idx,
                                         Access_data **ad) {
        List_data *ld;
...
extern Boolean geditor_showing(GEditor *ge) {
        if (ge)
...
extern void remove_all_gaccess_data(GEditor *ge) {
        int i;
        Access_data *step;
...
extern void remove_all_gaccess(GEditor *ge) {
        remove_all_gaccess_data(ge);
        XmListDeleteAllItems(ge->access_list);
}

extern void remove_from_gaccess_list(char *name, GEditor *ge) {
        int cnt;
        List_data *ld;
        Access_data *ad;
...
extern void set_geditor_defaults(GEditor *ge, Tick start, Tick stop) {
        int *list = NULL, cnt;
...
extern void set_geditor_title(GEditor *ge, char *name) {
        char buf[MAXNAMELEN];
        Calendar *c = ge->cal;

        if (ge->frame) {
                snprintf(buf, MAXNAMELEN, "%s - %s",
                         catgets(c->DT_catd, 1, 349,
                                 "Calendar : Group Appointment Editor"),
                         name);
                XtVaSetValues(ge->frame, XmNtitle, buf, NULL);
        }
}

extern void set_geditor_vals(GEditor *ge, Tick start, Tick stop) {
        if (start > 0 || stop > 0) {
                load_dssw_times(&ge->dssw, start, stop, False);
...
extern void show_geditor(Calendar *c, Tick start, Tick stop) {
        GEditor *ge = (GEditor *)c->geditor;
        ...
        if (!geditor_showing(ge)) {
                ...
                XmProcessTraversal(ge->dssw.what_text, XmTRAVERSE_CURRENT);
                XtVaSetValues(ge->base_form_mgr, XmNinitialFocus,
                              ge->dssw.what_text, NULL);
        }
