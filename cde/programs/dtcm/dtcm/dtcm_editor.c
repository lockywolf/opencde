...
static void de_mark_change(Widget w, XtPointer data, XtPointer cbs) {

        DTCM_editor *de = (DTCM_editor *)data;

        de->modified = True;

        if ((w == de->dssw->start_text) || (w == de->dssw->start_am) ||
            (w == de->dssw->start_pm) || (w == de->dssw->stop_text) ||
            (w == de->dssw->stop_am) || (w == de->dssw->stop_pm))
                de->dsswFlags = 0;
...
static void merge_old_values(Dtcm_appointment *original,
        ...
        for (source_count = 0, new_attrs = 0; source_count < original->count;
             source_count++) {
                found = False;
                for (dest_count = 0; dest_count < dest_attr_num; dest_count++) {
                        if (original->attrs[source_count].name &&
                            new->attrs[dest_count].name) {
                                if (strcmp(original->attrs[source_count].name,
                                           new->attrs[dest_count].name) == 0) {
                                        found = True;
                                        break;
                                }
                        }
        ...
        new->attrs = (CSA_attribute *)realloc(
            new->attrs, (dest_attr_num + new_attrs) * sizeof(CSA_attribute));
...
Widget CreateDragSourceIcon(Widget widget, Pixmap pixmap, Pixmap mask) {
        Widget dragIcon;
        Window rootWindow;
        int pixmapX, pixmapY;
        ...
        XGetGeometry(XtDisplayOfObject(widget), pixmap, &rootWindow, &pixmapX,
                     &pixmapY, &pixmapWidth, &pixmapHeight, &pixmapBorder,
                     &pixmapDepth);

        XtSetArg(args[nn], XmNwidth, pixmapWidth);
        nn++;
        XtSetArg(args[nn], XmNheight, pixmapHeight);
        nn++;
        XtSetArg(args[nn], XmNmaxWidth, pixmapWidth);
        nn++;
        XtSetArg(args[nn], XmNmaxHeight, pixmapHeight);
        nn++;
        XtSetArg(args[nn], XmNpixmap, pixmap);
        nn++;
        XtSetArg(args[nn], XmNmask, mask);
        nn++;
        XtSetArg(args[nn], XmNdepth, pixmapDepth);
        nn++;
 ...
 * icon type and name.
 */
static void GetIcon(DTCM_editor *de) {
        ...
        if (de->drag_bitmap == 0) {
                de->drag_bitmap = XCreateBitmapFromData(
                    display, window, (char *)drag_xbm_bits, drag_xbm_width,
                    drag_xbm_height);
                if (de->drag_bitmap == 0) {

                        printf("%s", catgets(calendar->DT_catd, 1, 237,
        ...
        if (de->drag_mask == 0) {
                de->drag_mask = XCreateBitmapFromData(
                    display, window, (char *)drag_mask_xbm_bits,
                    drag_mask_xbm_width, drag_mask_xbm_height);
                if (de->drag_mask == 0) {
                        printf(
                            "%s",
                            catgets(
                                calendar->DT_catd, 1, 238,
                                "XCreateBitmapFromData() failed for mask.\n"));
                        return;
                }
        }
}
 ...
 * with the drag.
 */
static void DragFinishCB(Widget widget, XtPointer clientData,
                         XtPointer callData) {
 ...
 */
static void ApptConvertCB(Widget dragContext, XtPointer clientData,
                          XtPointer callData) {
        ...
        data->name =
            XtNewString(catgets(c->DT_catd, 1, 236, "CalendarAppointment"));
}

void StandaloneApptDragStart(Widget widget, XEvent *event, DTCM_editor *de,
        ...
        /* Convert appointment into string.  If not successful, don't start
         * drag. */
        ...
        if (!dssw_form_flags_to_appt(de->dssw, appt, de->c->calname, now(),
                                     &de->dsswFlags) ||
            !rfp_form_flags_to_appt(de->rfp, appt, de->c->calname,
                                    &de->rfpFlags) ||
            (preDsswFlags != de->dsswFlags) || (preRfpFlags != de->rfpFlags)) {
                de->doing_drag = False;
        ...
        apptstr = parse_attrs_to_string(
            appt, de->p, attrs_to_string(appt->attrs, appt->count));
        ...
        if (de->drag_icon == NULL) {
                de->drag_icon = CreateDragSourceIcon(widget, de->drag_bitmap,
                                                     de->drag_mask);
        }

        if (DtDndVaDragStart(widget, event, DtDND_BUFFER_TRANSFER, 1,
                             XmDROP_COPY, convertCBRec, dragFinishCBRec,
                             DtNsourceIcon, de->drag_icon, NULL) == NULL) {

                printf("%s", catgets(c->DT_catd, 1, 239,
 ...
 * was being held down.
 */
static void EditApptDragMotionHandler(Widget dragInitiator,
        ...
        if (!de->doing_drag) {
                ...
                XtVaGetValues(de->drag_source, XmNx, &source_x, XmNy, &source_y,
                ...
                if ((ABS(diffX) >= DRAG_THRESHOLD) ||
                    (ABS(diffY) >= DRAG_THRESHOLD)) {
                        de->doing_drag = True;
                        StandaloneApptDragStart(dragInitiator, event, de,
                                                StandAloneEditor);
                        de->initialX = -1;
                        de->initialY = -1;
                }
...
**  Static callbacks
*/
static void de_quit_handler(Widget w, XtPointer cdata, XtPointer data) {
        DTCM_editor *de = (DTCM_editor *)cdata;
        int answer;

        if (de->modified == True) {
                char *title = XtNewString(catgets(
                    de->c->DT_catd, 1, 1008, "Calendar Appointment : Help"));
                char *text = XtNewString(
                    catgets(de->c->DT_catd, 1, 451,
                            "You have made unsaved changes.\nYou may save your "
                            "changes, discard your changes, \nor return to "
                            "your previous place in the dialog."));
                char *ident1 =
                    XtNewString(catgets(de->c->DT_catd, 1, 452, "Save"));
                char *ident2 =
                    XtNewString(catgets(de->c->DT_catd, 1, 700, "Discard"));
                char *ident3 =
                    XtNewString(catgets(de->c->DT_catd, 1, 923, "Cancel"));
                answer = dialog_popup(de->top_level, DIALOG_TITLE, title,
                                      DIALOG_TEXT, text, BUTTON_IDENT, 1,
                                      ident1, BUTTON_IDENT, 2, ident2,
                                      BUTTON_IDENT, 3, ident3, DIALOG_IMAGE,
                                      de->pu->xm_warning_pixmap, NULL);
                XtFree(ident3);
                XtFree(ident2);
                XtFree(ident1);
                XtFree(text);
                ...
                if (answer == 1) {
                        ...
                        if (de->contract)
                                tttk_message_fail(de->contract,
                                                  TT_DESKTOP_ECANCELED, NULL,
                                                  True);
                        XtPopdown(de->top_level);
                } else if (answer == 3) {
                        return;
                ...
                if (de->contract)
                        tttk_message_fail(de->contract, TT_DESKTOP_ECANCELED,
                                          NULL, True);
                XtPopdown(de->top_level);
        }
...
static void load_from_file(DTCM_editor *de) {
        int i;
        CmDataList *list = NULL;
        Dtcm_appointment *appt;

        list = CmDataListCreate();
        parse_appt_from_file(de->c->DT_catd, de->file, list, de->p, query_user,
        ...
        for (i = 1; i <= list->count; i++)
                if ((appt = (Dtcm_appointment *)CmDataListGetData(list, i))) {
                        if (de->orig_appt)
...
static void de_set_defaults(DTCM_editor *de) {
...
static void de_apply_proc(Widget w, XtPointer client_data, XtPointer data) {
        char *str;
        FILE *fp;
        ...
        old_attr_count = appt->count;
        merge_old_values(de->orig_appt, appt);
        str = parse_attrs_to_string(appt, de->p,
        ...
        if (w == NULL) {
                ...
                if (de->read_only) {
                        ...
                        if (de->init == file_tt) {
                                ...
                                ttmedia_load_reply(de->contract,
                                                   (unsigned char *)str,
                                                   strlen(str), True);
                        }
                }
                ...
                if ((de->init == no_tt) || (de->init == file_tt)) {
                        ...
                        if (de->init == file_tt)
                                ttmedia_Deposit(de->contract, NULL, de->vtype,
                                                NULL, 0, de->file, app, 30000);
                } else {
                        ttmedia_Deposit(de->contract, NULL, de->vtype,
...
static void de_reset_proc(Widget w, XtPointer client_data, XtPointer data) {
        DTCM_editor *de = (DTCM_editor *)client_data;
...
static void display_command_usage() {
        fprintf(stderr, "\ndtcm_editor Usage:  dtcm [filename]\n\n");
        exit(0);
}

static void OKCB(Widget dialog, XtPointer client_data, XtPointer call_data) {
        XtUnmanageChild((Widget)client_data);
}

void DieFromToolTalkError(DTCM_editor *de, char *errfmt, Tt_status status) {
        Arg args[10];
        ...
        xms_title = XmStringCreateLocalized(catgets(
            de->c->DT_catd, 2, 4, "Calendar : Appointment Editor - Warning"));
        ...
        dialog =
            XmCreateMessageDialog(de->top_level, "IconEditorError", args, n);
...
Tt_status cmtt_init(char *toolname, DTCM_editor *de, XtAppContext context,
                    Widget shell)
...
Tt_message reply_cb(Tt_message m, void *c_data, Tttk_op op,
...
static Tt_message contract_cb(Tt_message msg, void *clientdata,
                              Tt_message contract) {
 ...
 */
static Tt_message load_cb(Tt_message msg, void *clientdata, Tttk_op op,
                          Tt_status diagnosis, unsigned char *contents, int len,
...

{
        ...
        case TTME_EDIT:
                ...
                if (file == NULL) {
                         ...
                         * parse_appt_from_file
                         */

                        strlcpy(filename, "/tmp/cmXXXXXX", filename_size);
                        mkstemp(filename);
                        if ((fp = fopen(filename, "w")) == 0) {
                                tttk_message_fail(msg, TT_DESKTOP_ENODATA, 0,
                                                  1);
                                return 0;
                        }
                        ...
                        fclose(fp);
                        parse_appt_from_file(de->c->DT_catd, filename, list,
                                             de->p, query_user, de->c,
                                             DATAVER_ARCHIVE);
                        unlink(filename);
                        de->init = buffer_tt;
                } else {
                        de->file = strdup(file);
                        parse_appt_from_file(de->c->DT_catd, de->file, list,
                                             de->p, query_user, de->c,
                                             DATAVER_ARCHIVE);
                        de->init = file_tt;
                }
                ...
                for (i = 1; i <= list->count; i++)
                        if ((appt = (Dtcm_appointment *)CmDataListGetData(list,
                                                                          i))) {
                                if (de->orig_appt)
                                        free_appt_struct(&de->orig_appt);
...
static void handle_drop_cb(Widget w, XtPointer client_data,
                           XtPointer call_data) {
        Display *display = XtDisplay(w);
        DtDndDropCallbackStruct *transfer_info =
...
void de_register_drop_site(DTCM_editor *de, Widget w, Boolean registerchildren)

{
        ...
        DtDndVaDropRegister(w, DtDND_FILENAME_TRANSFER | DtDND_BUFFER_TRANSFER,
                            XmDROP_COPY | XmDROP_MOVE, transfer_cb_rec,
                            DtNregisterChildren, registerchildren, NULL);
        return;
}
...
int main(int argc, char **argv) {
        ...
        de->top_level = XtVaAppInitialize(
            &app, "Dtcm", NULL, 0, &argc, argv, NULL, XmNallowShellResize, True,
        ...
        */
        ...
        title = XtNewString(
            catgets(calendar->DT_catd, 1, 1074, "Calendar Appointment"));
        XtVaSetValues(de->top_level, XmNtitle, title, NULL);
        XtFree(title);
        ...
        */
        de->form =
            XtVaCreateWidget("form", xmFormWidgetClass, de->top_level,
                             XmNautoUnmanage, True, XmNfractionBase, 5, NULL);

        label_str =
            XmStringCreateLocalized(catgets(de->c->DT_catd, 1, 846, "Save"));
        de->attach_button = XtVaCreateWidget(
            "attach_button", xmPushButtonGadgetClass, de->form, XmNlabelString,
            label_str, XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 1,
            XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, 2,
            XmNbottomAttachment, XmATTACH_FORM, XmNbottomOffset, 10, NULL);
        XmStringFree(label_str);
        XtAddCallback(de->attach_button, XmNactivateCallback, de_apply_proc,
                      (XtPointer)de);
        if (!de->file)
                XtSetSensitive(de->attach_button, False);

        label_str =
            XmStringCreateLocalized(catgets(de->c->DT_catd, 1, 691, "Reset"));
        de->reset_button = XtVaCreateWidget(
            "reset_button", xmPushButtonGadgetClass, de->form, XmNlabelString,
            label_str, XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 3,
        ...
        XtVaSetValues(de->rfp->rfp_form_mgr, XmNbottomAttachment,
                      XmATTACH_WIDGET, XmNbottomWidget, de->attach_button,
                      XmNbottomOffset, 25, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 5, NULL);
        XtVaGetValues(de->rfp->rfp_form_mgr, XmNchildren, &children,
        ...
        de->drag_source = XtVaCreateWidget(
            "drag_source", dtIconGadgetClass, de->form, XmNpixmapPosition,
            XmPIXMAP_TOP, XmNstringPosition, XmSTRING_BOTTOM, XmNalignment,
            XmALIGNMENT_CENTER, XmNstring, xmstr, XmNbottomAttachment,
            XmATTACH_WIDGET, XmNbottomWidget, de->attach_button,
            XmNbottomOffset, 25, XmNleftAttachment, XmATTACH_WIDGET,
            XmNleftWidget, de->rfp->rfp_form_mgr, XmNrightAttachment,
        ...
        if ((Boolean)btn1_transfer != True)
                XtAddEventHandler(
                    XtParent(de->drag_source), Button2MotionMask, False,
                    (XtEventHandler)EditApptDragMotionHandler, (XtPointer)de);

        if (de->pu->drag_icon_xbm)
                XtVaSetValues(de->drag_source, XmNpixmap, de->pu->drag_icon_xbm,
                              NULL);

        de->dsswFlags = 0;
        de->dssw = (DSSW *)ckalloc(sizeof(DSSW));
        build_dssw(de->dssw, de->c, de->form, True, True);
        XtVaSetValues(de->dssw->dssw_form_mgr, XmNtopAttachment, XmATTACH_FORM,
                      XmNtopOffset, 10, XmNbottomAttachment, XmATTACH_WIDGET,
                      XmNbottomWidget, de->rfp->rfp_form_mgr, XmNbottomOffset,
                      15, XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 10,
                      NULL);
        ...
        XtAddCallback(de->dssw->start_text, XmNvalueChangedCallback,
                      de_mark_change, de);
        XtAddCallback(de->dssw->start_am, XmNvalueChangedCallback,
                      de_mark_change, de);
        XtAddCallback(de->dssw->start_pm, XmNvalueChangedCallback,
                      de_mark_change, de);
        XtAddCallback(de->dssw->stop_text, XmNvalueChangedCallback,
                      de_mark_change, de);
        XtAddCallback(de->dssw->stop_am, XmNvalueChangedCallback,
                      de_mark_change, de);
        XtAddCallback(de->dssw->stop_pm, XmNvalueChangedCallback,
                      de_mark_change, de);
        XtAddCallback(de->dssw->what_text, XmNvalueChangedCallback,
                      de_mark_change, de);
        XtAddCallback(de->rfp->repeat_menu, XmNselectionCallback,
                      de_mark_change, de);
        XtAddCallback(de->rfp->for_menu, XmNselectionCallback, de_mark_change,
                      de);
        ...
        */
        ...
        /* 	Don't need these any more.
                XtVaGetValues(de->dssw->date_label, XmNwidth,
           &longest_dssw_label, NULL);
                ...
                XtVaGetValues(de->rfp->frequency_label, XmNwidth,
           &longest_rfp_label, NULL); XtVaGetValues(de->rfp->for_label,
           XmNwidth, &width, NULL); if (width > longest_rfp_label)
                        longest_rfp_label = width;

                XtVaSetValues(de->dssw->dssw_form_mgr,
                                XmNleftOffset, longest_rfp_label -
           longest_dssw_label, NULL);
        */
