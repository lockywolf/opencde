...
static CSA_return_code t_execute_change(ToDo *, CSA_entry_handle,
                                        Dtcm_appointment *, Widget);
extern boolean_t compare_repeat_info(Dtcm_appointment *, RFP *,
                                     CSA_session_handle, int);
extern void change_rule_for_this_one_only(Calendar *, Dtcm_appointment *,
                                          Dtcm_appointment *);
...
*/
extern CSA_entry_handle t_nth_appt(ToDo *t, int idx) {
        if (idx >= 0 && idx < t->todo_count)
                return t->todo_head[idx];
        return 0;
}

static void clear_flag_on_modify(Widget w, XtPointer uData, XtPointer cbData) {
        int *flagP = (int *)uData;
...
*/
static void appt_to_form(ToDo *t, CSA_entry_handle a) {
        char buf[MAXNAMELEN];
        ...
        appt = allocate_appt_struct(appt_read, t->cal->general->version,
                                    CSA_ENTRY_ATTR_STATUS_I,
                                    CSA_ENTRY_ATTR_ORGANIZER_I, NULL);
        ...
        XmToggleButtonGadgetSetState(t->completed_toggle, t->completed_val,
                                     True);
        snprintf(buf, MAXNAMELEN, "%s:  %s",
                 catgets(t->cal->DT_catd, 1, 565, "Author"),
...
*/
static void t_list_select_proc(Widget w, XtPointer client_data,
                               XtPointer data) {
        CSA_entry_handle a;
...
*/
static void t_build_expand(ToDo *t) {
        ...
        */
        ...
        XtVaSetValues(t->rfp.rfp_form_mgr, XmNtopAttachment, XmATTACH_WIDGET,
                      XmNtopWidget, t->separator1, XmNleftAttachment,
                      XmATTACH_FORM, XmNleftOffset, 5, NULL);
        XtVaGetValues(t->rfp.rfp_form_mgr, XmNchildren, &children,
        ...
        */
        ...
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                      t->reminders.alarm_label, NULL);
        XtVaSetValues(t->reminders.bfpm_form_mgr, XmNtopAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNtopWidget,
                      t->rfp.rfp_form_mgr, XmNrightAttachment, XmATTACH_FORM,
                      XmNrightOffset, 5, XmNleftAttachment, XmATTACH_WIDGET,
                      XmNleftWidget, t->rfp.rfp_form_mgr, NULL);

        XtManageChildren(widgets, 6);
}

static void clear_view_changes(ToDo *t) {
...
static void write_view_changes(ToDo *t) {
        ...
        if (t->cal->general->version <= DATAVER3) {
                appt = allocate_appt_struct(
                    appt_write, t->cal->general->version,
                    CSA_ENTRY_ATTR_STATUS_I, CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I,
                    NULL);
                ...
                appt = allocate_appt_struct(
                    appt_write, t->cal->general->version,
                    CSA_ENTRY_ATTR_STATUS_I, CSA_ENTRY_ATTR_RECURRENCE_RULE_I,
                    NULL);
        ...
        while (step) {
                if (step->modified) {
                        ...
                        appt->state->value->item.sint32_value =
                            (toggle_state) ? CSA_STATUS_COMPLETED
                                           : CSA_X_DT_STATUS_ACTIVE;

                        stat = csa_update_entry_attributes(
                            c->cal_handle, step->appt, scope, CSA_FALSE,
                            appt->count, appt->attrs, &updated_a, NULL);
                        ...
                        if (stat != CSA_SUCCESS)
                                XtVaSetValues(step->view_item_toggle, XmNset,
                                              !toggle_state, NULL);
                        step->modified = False;
                }
                step = step->next;
...
static Boolean flush_view_changes(ToDo *t) {
        Calendar *c = t->cal;
        ...
        if (t->view_list_modified) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 778, "Calendar : To Do List - Help"));
                ...
                answer = dialog_popup(t->view_frame, DIALOG_TITLE, title,
                                      DIALOG_TEXT, text, BUTTON_IDENT, 1,
                                      ident1, BUTTON_IDENT, 2, ident2,
                                      BUTTON_IDENT, 3, ident3, DIALOG_IMAGE,
...
*/
static void t_view_quit_handler(Widget w, XtPointer cdata, XtPointer data) {
        ToDo *t = (ToDo *)cdata;
...
static void t_view_cancel_cb(Widget widget, XtPointer client_data,
...
static void t_view_apply_cb(Widget widget, XtPointer client_data,
                            XmPushButtonCallbackStruct *cbs) {
        ToDo *t = (ToDo *)client_data;
...
static void t_view_ok_cb(Widget widget, XtPointer client_data,
                         XmPushButtonCallbackStruct *cbs) {
        ToDo *t = (ToDo *)client_data;
...
*/
static void t_view_filter_proc(Widget w, XtPointer client_data, XtPointer cbs) {
        ToDo *t = (ToDo *)client_data;
        int op;
...
extern Widget create_filter_menu(Widget parent, XtCallbackProc cb_func,
                                 XtPointer data) {
        int i;
        Widget menuitem;
        Widget menu;
        ...
        label =
            XmStringCreateLocalized(catgets(calendar->DT_catd, 1, 40, "View"));
        option1 =
            XmStringCreateLocalized(catgets(calendar->DT_catd, 1, 272, "All"));
        option2 = XmStringCreateLocalized(
            catgets(calendar->DT_catd, 1, 783, "Pending"));
        option3 = XmStringCreateLocalized(
            catgets(calendar->DT_catd, 1, 784, "Completed"));

        menu = XmCreatePulldownMenu(parent, "_pulldown", NULL, 0);

        XtSetArg(args[ac], XmNsubMenuId, menu);
        ac++;

        cascade = XmCreateOptionMenu(parent, "option_m", args, ac);

        menuitem = XtVaCreateWidget("all", xmPushButtonGadgetClass, menu,
                                    XmNlabelString, option1, XmNuserData,
                                    VIEW_ALL, NULL);

        XtAddCallback(menuitem, XmNactivateCallback, cb_func, data);

        menuitem = XtVaCreateWidget("pending", xmPushButtonGadgetClass, menu,
                                    XmNlabelString, option2, XmNuserData,
                                    VIEW_PENDING, NULL);

        XtAddCallback(menuitem, XmNactivateCallback, cb_func, data);

        menuitem = XtVaCreateWidget("completed", xmPushButtonGadgetClass, menu,
                                    XmNlabelString, option3, XmNuserData,
                                    VIEW_COMPLETED, NULL);
...
*/
static void t_build_view_popup(ToDo *t) {
        ...
        */
        title =
            XtNewString(catgets(c->DT_catd, 1, 1012, "Calendar : To Do List"));
        t->view_frame = XtVaCreatePopupShell(
            "todo_list", xmDialogShellWidgetClass, c->frame, XmNtitle, title,
            XmNdeleteResponse, XmDO_NOTHING, XmNallowShellResize, True,
            XmNmappedWhenManaged, False, XmNautoUnmanage, False, NULL);
        XtFree(title);
        setup_quit_handler(t->view_frame, t_view_quit_handler, (caddr_t)t);

        t->view_form =
            XtVaCreateWidget("apptform", xmFormWidgetClass, t->view_frame,
                             XmNfractionBase, 4, XmNautoUnmanage, False, NULL);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 655, "OK"));

        t->view_ok_button = XtVaCreateWidget(
            "ok", xmPushButtonWidgetClass, t->view_form, XmNlabelString, xmstr,
            XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 0,
            XmNleftOffset, 10, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 1, XmNbottomAttachment, XmATTACH_FORM,
            XmNbottomOffset, 5, NULL);

        XmStringFree(xmstr);

        XtAddCallback(t->view_ok_button, XmNactivateCallback,
                      (XtCallbackProc)t_view_ok_cb, t);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 460, "Apply"));

        t->view_apply_button = XtVaCreateWidget(
            "apply", xmPushButtonWidgetClass, t->view_form, XmNlabelString,
            xmstr, XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 1,
            XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, 2,
            XmNbottomAttachment, XmATTACH_FORM, XmNbottomOffset, 5, NULL);
        XmStringFree(xmstr);

        XtAddCallback(t->view_apply_button, XmNactivateCallback,
                      (XtCallbackProc)t_view_apply_cb, t);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 923, "Cancel"));

        t->view_cancel_button = XtVaCreateWidget(
            "cancel", xmPushButtonWidgetClass, t->view_form, XmNlabelString,
            xmstr, XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 2,
            XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, 3,
            XmNbottomAttachment, XmATTACH_FORM, XmNbottomOffset, 5, NULL);

        XmStringFree(xmstr);

        XtAddCallback(t->view_cancel_button, XmNactivateCallback,
                      (XtCallbackProc)t_view_cancel_cb, t);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 77, "Help"));

        t->view_help_button = XtVaCreateWidget(
            "help", xmPushButtonWidgetClass, t->view_form, XmNlabelString,
            xmstr, XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 3,
            XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, 4,
            XmNrightOffset, 10, XmNbottomAttachment, XmATTACH_FORM,
            XmNbottomOffset, 5, NULL);
        XmStringFree(xmstr);

        separator = XtVaCreateWidget(
            "separator", xmSeparatorGadgetClass, t->view_form,
            XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            t->view_ok_button, XmNbottomOffset, 5, NULL);

        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 789, "To Do Type:"));
        label = XtVaCreateWidget(
            "type_label", xmLabelGadgetClass, t->view_form, XmNtopAttachment,
            XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 5,
            XmNtopOffset, 8, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        t->view_filter_menu =
            create_filter_menu(t->view_form, t_view_filter_proc, (XtPointer)t);

        XtVaSetValues(t->view_filter_menu, XmNuserData, REPEAT_DAYS,
                      XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment,
                      XmATTACH_WIDGET, XmNleftWidget, label, XmNtopOffset, 3,
                      XmNleftOffset, 5, NULL);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 790, "context"));
        t->view_list_label = XtVaCreateWidget(
            "type_label", xmLabelGadgetClass, t->view_form, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, t->view_filter_menu,
            XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 5, XmNtopOffset, 5,
            XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        t->view_sw_mgr = XtVaCreateWidget(
            "view_sw_mgr", xmScrolledWindowWidgetClass, t->view_form,
            XmNscrollingPolicy, XmAUTOMATIC, XmNlistSizePolicy, XmCONSTANT,
            XmNscrolledWindowMarginHeight, 5, XmNscrolledWindowMarginWidth, 5,
            XmNwidth, 400, XmNheight, 200, XmNleftAttachment, XmATTACH_FORM,
            XmNleftOffset, 5, XmNrightAttachment, XmATTACH_FORM, XmNrightOffset,
            5, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
            t->view_list_label, XmNbottomAttachment, XmATTACH_WIDGET,
            XmNbottomWidget, separator, XmNbottomOffset, 5, NULL);
        t->view_form_mgr = XtVaCreateWidget("view_form_mgr", xmFormWidgetClass,
                                            t->view_sw_mgr, NULL);

        XtAddCallback(t->view_help_button, XmNactivateCallback,
                      (XtCallbackProc)help_cb, TODO_LIST_HELP_BUTTON);
        XtAddCallback(t->view_form, XmNhelpCallback, (XtCallbackProc)help_cb,
                      (XtPointer)TODO_LIST_HELP_BUTTON);
...
*/
static void t_expand_ui_proc(Widget w, XtPointer client_data, XtPointer data) {
        ToDo *t = (ToDo *)client_data;
        Props_pu *p = (Props_pu *)t->cal->properties_pu;
        XmString xmstr;
        Dimension h1, h2;
        ...
        if (expand_state_closed) {
                ...
                xmstr = XmStringCreateLocalized(
                    catgets(t->cal->DT_catd, 1, 625, "Less"));
                XtVaSetValues(t->expand_ui_button, XmNlabelString, xmstr, NULL);
                XmStringFree(xmstr);
                ...
                XtSetSensitive(t->rfp.rfp_form_mgr, False);
                xmstr = XmStringCreateLocalized(
                    catgets(t->cal->DT_catd, 1, 626, "More"));
                XtVaSetValues(t->expand_ui_button, XmNlabelString, xmstr, NULL);
                XmStringFree(xmstr);
...
static Boolean first_line_contains_text(char *str) {
...
*/
static Dtcm_appointment *t_form_to_appt(ToDo *t) {
        boolean_t all_ok;
        Dtcm_appointment *appt;
        char *str;
        Calendar *c = t->cal;
        ...
        if (t->cal->general->version < DATAVER4)
                appt = allocate_appt_struct(
                    ...
                    CSA_ENTRY_ATTR_LAST_UPDATE_I, CSA_ENTRY_ATTR_ORGANIZER_I,
                    CSA_ENTRY_ATTR_START_DATE_I, CSA_ENTRY_ATTR_TYPE_I,
                    CSA_ENTRY_ATTR_CLASSIFICATION_I, CSA_ENTRY_ATTR_END_DATE_I,
                    CSA_X_DT_ENTRY_ATTR_SHOWTIME_I, CSA_ENTRY_ATTR_SUMMARY_I,
                    CSA_ENTRY_ATTR_STATUS_I, CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I,
                    ...
                    CSA_ENTRY_ATTR_POPUP_REMINDER_I, NULL);
        else
                appt = allocate_appt_struct(
                    appt_write, t->cal->general->version,
                    CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I,
                    ...
                    CSA_ENTRY_ATTR_RECURRENCE_RULE_I, NULL);
        ...
        XtFree(str);
        XmToggleButtonSetState(t->dssw.stop_am,
                               XmToggleButtonGetState(t->dssw.start_am), True);
        XmToggleButtonSetState(t->dssw.stop_pm,
                               XmToggleButtonGetState(t->dssw.start_pm), True);
        ...
        if (!first_line_contains_text(str)) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 837, "Calendar : Error - To Do Editor"));
                char *text =
                    XtNewString(catgets(c->DT_catd, 1, 913,
                                        "A To Do item must have text in the "
                                        "first line of the What item."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                dialog_popup(t->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
        ...
        all_ok = dssw_form_to_appt(
            &t->dssw, appt, t->cal->view->current_calendar, t->cal->view->date);
        if (all_ok)
        ...
        appt->state->value->item.sint32_value =
            (t->completed_val) ? CSA_STATUS_COMPLETED : CSA_X_DT_STATUS_ACTIVE;
...
**  Action procedures
*/
static void t_insert_proc(Widget w, XtPointer client_data, XtPointer data) {
        int wk;
        ToDo *t = (ToDo *)client_data;
        Props_pu *p = (Props_pu *)t->cal->properties_pu;
        ...
        */
        ...
        if (op != ONE_TIME) {
                char *title =
                    XtNewString(catgets(c->DT_catd, 1, 1102, "Insert To Do"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 1103,
                ...
                answer = dialog_popup(
                    c->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                    BUTTON_IDENT, 1, ident1, BUTTON_IDENT, 4, ident4,
                    DIALOG_IMAGE, p->xm_question_pixmap, NULL);
                XtFree(ident4);
                XtFree(ident1);
                XtFree(text);
                XtFree(title);
        ...
        stat = csa_add_entry(t->cal->cal_handle, appt->count, appt->attrs,
                             &new_a, NULL);
        backend_err_msg(t->frame, t->cal->view->current_calendar, stat,
                        p->xm_error_pixmap);
        free_appt_struct(&appt);
...
static void t_delete_proc(Widget w, XtPointer client_data, XtPointer data) {
        ...
        if (!XmListGetSelectedPos(t->todo_list, &item_list, &item_cnt)) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 566, "Calendar : Error - To Do Editor"));
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 567, "Select a To Do and DELETE again."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                answer =
                    dialog_popup(t->frame, DIALOG_TITLE, title, DIALOG_TEXT,
                                 text, BUTTON_IDENT, 1, ident1, DIALOG_IMAGE,
                                 p->xm_error_pixmap, NULL);
                XtFree(ident1);
                XtFree(text);
                XtFree(title);
        ...
        answer = 0;
        appt = allocate_appt_struct(appt_read, c->general->version,
        ...
        if (appt->repeat_type->value) {
                if (appt->repeat_type->value->item.sint32_value !=
                    CSA_X_DT_REPEAT_ONETIME) {
                        char *title = XtNewString(catgets(
                            c->DT_catd, 1, 591, "Calendar : To Do Editor"));
                        char *text = XtNewString(
                            catgets(c->DT_catd, 1, 573,
                                    "This To Do is part of a repeating "
                                    "series.\nDo you want to delete ...?"));
                        char *ident1 =
                            XtNewString(catgets(c->DT_catd, 1, 923, "Cancel"));
                        char *ident2 = XtNewString(
                            catgets(c->DT_catd, 1, 270, "This One Only"));
                        char *ident3 =
                            XtNewString(catgets(c->DT_catd, 1, 271, "Forward"));
                        char *ident4 =
                            XtNewString(catgets(c->DT_catd, 1, 272, "All"));
                        answer = dialog_popup(
                            t->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                            BUTTON_IDENT, 1, ident1, BUTTON_IDENT, 2, ident2,
                            BUTTON_IDENT, 3, ident3, BUTTON_IDENT, 4, ident4,
                            DIALOG_IMAGE, p->xm_question_pixmap, NULL);
                        XtFree(ident4);
                        XtFree(ident3);
                        XtFree(ident2);
                ...
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 591, "Calendar : To Do Editor"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 791,
                            "This To Do repeats in an unknown fashion.  All "
...
**  list.
*/
static CSA_return_code t_execute_change(ToDo *t, CSA_entry_handle old_a,
                                        Dtcm_appointment *new_a, Widget frame) {
        Calendar *c = t->cal;
        Props_pu *p = (Props_pu *)t->cal->properties_pu;
        CSA_entry_handle updated_a;
        CSA_return_code stat;
        ...
        answer = 0;
        appt = allocate_appt_struct(
            appt_read, c->general->version, CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I,
            CSA_X_DT_ENTRY_ATTR_REPEAT_TIMES_I,
            CSA_X_DT_ENTRY_ATTR_REPEAT_INTERVAL_I,
            CSA_X_DT_ENTRY_ATTR_REPEAT_OCCURRENCE_NUM_I,
            CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I, CSA_ENTRY_ATTR_START_DATE_I,
            CSA_ENTRY_ATTR_RECURRENCE_RULE_I, NULL);
            ...
            (appt->repeat_type->value->item.sint32_value ==
             CSA_X_DT_REPEAT_OTHER_YEARLY)) {
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 591, "Calendar : To Do Editor"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 708,
                            "This appointment repeats in an unknown fashion.  "
                ...
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 591, "Calendar : To Do Editor"));
                char *text =
                    XtNewString(catgets(c->DT_catd, 1, 579,
                                        "This To Do is part of a repeating "
                                        "series.\nDo you want to change ...?"));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 923, "Cancel"));
                char *ident2 =
                    XtNewString(catgets(c->DT_catd, 1, 270, "This One Only"));
                char *ident3 =
                    XtNewString(catgets(c->DT_catd, 1, 271, "Forward"));
                char *ident4 = XtNewString(catgets(c->DT_catd, 1, 272, "All"));
                answer = dialog_popup(
                    frame, DIALOG_TITLE, title, DIALOG_TEXT, text, BUTTON_IDENT,
                    1, ident1, BUTTON_IDENT, 2, ident2, BUTTON_IDENT, 3, ident3,
                    (compare_repeat_info(appt, &(t->rfp), c->cal_handle,
                                         c->general->version)
                         ? BUTTON_IDENT
                         : BUTTON_INSENSITIVE),
                    4, ident4, DIALOG_IMAGE, p->xm_question_pixmap, NULL);
                XtFree(ident4);
                XtFree(ident3);
                XtFree(ident2);
                XtFree(ident1);
        ...
        if (c->general->version == DATAVER3 &&
            ...
            (appt->repeat_type->value->item.sint32_value !=
             ...
             appt->repeat_times->value->item.uint32_value !=
                ...
                if (new_a->sequence_end_date &&
                    new_a->sequence_end_date->value) {
                        if (new_a->sequence_end_date->value->item
                                .date_time_value)
                                free(new_a->sequence_end_date->value->item
                                         .date_time_value);
        ...
        stat = csa_update_entry_attributes(c->cal_handle, old_a, scope,
                                           CSA_FALSE, new_a->count,
                                           new_a->attrs, &updated_a, NULL);
        backend_err_msg(t->frame, c->view->current_calendar, stat,
                        p->xm_error_pixmap);
        if (stat != CSA_SUCCESS)
                return stat;
...
static void t_view_change_proc(Widget w, XtPointer cdata, XtPointer cbs) {
        ToDo *t = (ToDo *)cdata;
        TodoView *step = t->view_list;
...
static void t_change_proc(Widget w, XtPointer client_data, XtPointer data) {
        int *item_list = NULL, item_cnt = 0;
        ToDo *t = (ToDo *)client_data;
        Calendar *c = t->cal;
        Props_pu *p = (Props_pu *)t->cal->properties_pu;
        ...
        if (!XmListGetSelectedPos(t->todo_list, &item_list, &item_cnt)) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 566, "Calendar : Error - To Do Editor"));
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 585, "Select a To Do and CHANGE again."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                answer =
                    dialog_popup(t->frame, DIALOG_TITLE, title, DIALOG_TEXT,
...
static void t_clear_proc(Widget w, XtPointer client_data, XtPointer data) {
        ToDo *t = (ToDo *)client_data;
...
static void t_close_proc(Widget w, XtPointer client_data, XtPointer data) {
        ToDo *t = (ToDo *)client_data;
...
static void t_quit_handler(Widget w, XtPointer cdata, XtPointer data) {
        ToDo *t = (ToDo *)cdata;
 ...
 * was being held down.
 */
static void FormApptDragMotionHandler(Widget dragInitiator,
        ...
        if (!t->doing_drag) {
                ...
                XtVaGetValues(t->drag_source, XmNx, &source_x, XmNy, &source_y,
                              XmNheight, &source_height, XmNwidth,
                              &source_width, NULL);
...
extern void t_make_todo(Calendar *c) {
        int cnt = 0;
        Arg args[15];
        ToDo *t = (ToDo *)c->todo;
        ...
        Dimension label_height, max_left_label_width;
        ...
        */
        t->frame = XtVaCreatePopupShell("frame", xmDialogShellWidgetClass,
                                        t->cal->frame, XmNdeleteResponse,
                                        XmDO_NOTHING, XmNallowShellResize, True,
        ...
        t->base_form_mgr =
            XtVaCreateWidget("base_form_mgr", xmFormWidgetClass, t->frame,
                             XmNautoUnmanage, False, XmNfractionBase, 15, NULL);
        ...
        */
        ...
        t->list_label =
            XtVaCreateWidget("todo_label", xmLabelGadgetClass, t->base_form_mgr,
        ...
        t->message_text = XtVaCreateWidget(
            "message", xmLabelGadgetClass, t->base_form_mgr, XmNshadowThickness,
            0, XmNcursorPositionVisible, False, XmNresizeWidth, True,
            XmNalignment, XmALIGNMENT_BEGINNING, XmNbottomAttachment,
            XmATTACH_FORM, XmNbottomOffset, 2, XmNleftAttachment, XmATTACH_FORM,
            XmNleftOffset, 2, XmNrightAttachment, XmATTACH_FORM, XmNrightOffset,
            2, NULL);
        ...
        */
        ...
        t->insert_button = XtVaCreateWidget(
            "insert", xmPushButtonWidgetClass, t->base_form_mgr,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            t->message_text, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 1, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 3, XmNlabelString, xmstr, NULL);
        ...
        t->change_button = XtVaCreateWidget(
            "change", xmPushButtonWidgetClass, t->base_form_mgr,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            t->message_text, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 3, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 5, XmNlabelString, xmstr, XmNsensitive, False,
            NULL);
        ...
        t->delete_button = XtVaCreateWidget(
            "delete", xmPushButtonWidgetClass, t->base_form_mgr,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            t->message_text, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 5, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 7, XmNlabelString, xmstr, XmNsensitive, False,
            NULL);
        ...
        t->clear_button = XtVaCreateWidget(
            "clear", xmPushButtonWidgetClass, t->base_form_mgr,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            t->message_text, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 7, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 9, XmNlabelString, xmstr, NULL);
        XtAddCallback(t->clear_button, XmNactivateCallback, t_clear_proc, t);
        XmStringFree(xmstr);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 923, "Cancel"));
        t->close_button = XtVaCreateWidget(
            "close", xmPushButtonWidgetClass, t->base_form_mgr,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            t->message_text, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 9, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 11, XmNlabelString, xmstr, NULL);
        XtAddCallback(t->close_button, XmNactivateCallback, t_close_proc, t);
        XmStringFree(xmstr);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 77, "Help"));
        t->help_button = XtVaCreateWidget(
            "help", xmPushButtonWidgetClass, t->base_form_mgr,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            t->message_text, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 11, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 13, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);
        ...
        XtAddCallback(t->base_form_mgr, XmNhelpCallback,
                      ...
                      (XtPointer)TODO_EDITOR_HELP_BUTTON);

        t->separator2 = XtVaCreateWidget(
            "separator2", xmSeparatorGadgetClass, t->base_form_mgr,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget,
            t->insert_button, XmNbottomOffset, 5, XmNleftAttachment,
            XmATTACH_FORM, XmNleftOffset, 5, XmNrightAttachment, XmATTACH_FORM,
            XmNrightOffset, 5, NULL);

        t->separator1 = XtVaCreateWidget(
            "separator1", xmSeparatorGadgetClass, t->base_form_mgr,
        ...
        t->expand_ui_button = XtVaCreateWidget(
            "expando", xmPushButtonWidgetClass, t->base_form_mgr,
            XmNlabelString, xmstr, XmNleftAttachment, XmATTACH_FORM,
            XmNleftOffset, 5, XmNbottomAttachment, XmATTACH_WIDGET,
            XmNbottomWidget, t->separator1, XmNbottomOffset, 3,
            XmNnavigationType, XmTAB_GROUP, NULL);
        XmStringFree(xmstr);
        XtAddCallback(t->expand_ui_button, XmNactivateCallback,
                      t_expand_ui_proc, t);
        ...
        */
        ...
        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 590, "Due Date:"));
        XtVaSetValues(t->dssw.date_label, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 798, "Time Due:"));
        XtVaSetValues(t->dssw.start_label, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);
        ...
        XtVaSetValues(t->dssw.date_label, XmNleftOffset, _toLabel, NULL);
        XtVaSetValues(t->dssw.date_text, XmNleftOffset, _toText, NULL);
        /*
        ...
        XtVaSetValues(t->dssw.start_label, XmNleftOffset, _toLabel, NULL);
        XtVaSetValues(t->dssw.start_text, XmNleftOffset, _toText, NULL);
        XtVaSetValues(t->dssw.start_menu, XmNleftOffset, _toMenu, NULL);
        XtVaSetValues(t->dssw.start_ampm_rc_mgr, XmNleftOffset, _toRC, NULL);

        _i18n_HighestWidget(4, &prev, &highest, t->dssw.start_label,
                            t->dssw.start_text, t->dssw.start_menu,
                            t->dssw.start_ampm_rc_mgr);
        _i18n_WidestWidget(1, &curr, &widest, t->dssw.what_label);
        _toLabel = max_left_label_width + 2 * 5 - 5 - widest;
        _toText = max_left_label_width + 2 * 5;
        XtVaSetValues(t->dssw.what_label, XmNtopAttachment, XmATTACH_WIDGET,
                      XmNtopWidget, prev, XmNleftOffset, _toLabel, NULL);

        /*
         * Add a drag source icon inside the dssw, lower right
         */
        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 906, "Drag To Do"));
        t->drag_source = XtVaCreateWidget(
            "drag_source", dtIconGadgetClass, t->dssw.dssw_form_mgr,
            XmNpixmapPosition, XmPIXMAP_TOP, XmNstringPosition, XmSTRING_BOTTOM,
            XmNalignment, XmALIGNMENT_CENTER, XmNstring, xmstr,
            XmNbottomAttachment, XmATTACH_FORM, XmNrightAttachment,
            XmATTACH_FORM, XmNtraversalOn, False, NULL);
        XmStringFree(xmstr);

        XtVaSetValues(t->dssw.what_scrollwindow, XmNtopAttachment,
                      XmATTACH_WIDGET, XmNtopWidget, prev, XmNleftOffset,
                      _toText, XmNrightAttachment, XmATTACH_WIDGET,
                      XmNrightWidget, t->drag_source, NULL);
        ...
        if ((Boolean)btn1_transfer != True)
                XtAddEventHandler(
                    XtParent(t->drag_source), Button2MotionMask, False,
                    (XtEventHandler)FormApptDragMotionHandler, (XtPointer)c);

        XtVaSetValues(t->drag_source, XmNpixmap, p->drag_icon_xbm, NULL);

        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 784, "Completed"));

        t->completed_toggle = XtVaCreateManagedWidget(
            "completed", xmToggleButtonGadgetClass, t->dssw.dssw_form_mgr,
            XmNlabelString, xmstr, XmNbottomAttachment, XmATTACH_FORM,
            XmNnavigationType, XmTAB_GROUP, NULL);
        XmStringFree(xmstr);

        _toText = max_left_label_width + 2 * 5 - 4;
        XtVaSetValues(t->completed_toggle, XmNleftAttachment, XmATTACH_FORM,
        ...
        XtVaSetValues(prev, XmNbottomAttachment, XmATTACH_WIDGET,
                      XmNbottomWidget, t->completed_toggle, XmNbottomOffset, 10,
                      NULL);

        XtVaSetValues(t->dssw.dssw_form_mgr, XmNtopAttachment, XmATTACH_FORM,
                      XmNtopOffset, label_height + 5, XmNleftAttachment,
                      XmATTACH_FORM, XmNleftOffset, 5, XmNbottomAttachment,
                      XmATTACH_WIDGET, XmNbottomWidget, t->expand_ui_button,
                      XmNbottomOffset, 3, NULL);

        XtVaGetValues(t->dssw.dssw_form_mgr, XmNchildren, &children,
                      XmNnumChildren, &n, NULL);
        ...
        */
        ...
        cnt = 0;
        XtSetArg(args[cnt], XmNlistSizePolicy, XmCONSTANT);
        ++cnt;
        XtSetArg(args[cnt], XmNwidth,
                 15 * listfontextents.max_logical_extent.width);
        ++cnt;
        XtSetArg(args[cnt], XmNscrollBarDisplayPolicy, XmSTATIC);
        ++cnt;
        XtSetArg(args[cnt], XmNdoubleClickInterval, 5);
        ++cnt;
        ...
        XtVaSetValues(t->todo_list_sw, XmNtopAttachment, XmATTACH_FORM,
                      XmNtopOffset, label_height + 11, XmNrightAttachment,
                      XmATTACH_FORM, XmNrightOffset, 5, XmNleftAttachment,
                      XmATTACH_WIDGET, XmNleftWidget, t->dssw.dssw_form_mgr,
                      XmNleftOffset, 10, XmNbottomAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNbottomWidget,
                      t->dssw.dssw_form_mgr, NULL);

        XtManageChild(t->todo_list);
        XtAddCallback(t->todo_list, XmNbrowseSelectionCallback,
                      t_list_select_proc, t);

        XtVaSetValues(t->list_label, XmNleftAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNleftWidget, t->todo_list_sw,
                      NULL);

        /* set default button */
        XtVaSetValues(t->base_form_mgr, XmNdefaultButton, t->insert_button,
                      NULL);

        XtVaSetValues(t->base_form_mgr, XmNcancelButton, t->close_button, NULL);

        XmProcessTraversal(t->dssw.what_text, XmTRAVERSE_CURRENT);
        XtVaSetValues(t->base_form_mgr, XmNinitialFocus, t->dssw.what_text,
                      NULL);
...
**
*******************************************************************************/
extern void add_to_todo_list(CSA_entry_handle entry, ToDo *t) {
        char *buf;
        Lines *lines;
        XmString str;
        CSA_return_code stat;
...
extern void add_all_todo(ToDo *t) {
        int i;
        CSA_uint32 count;
        ...
        date =
            get_date_from_widget(t->cal->view->date, t->dssw.date_text, o, s);
        if (!date)
                return;
...
static void set_list_title(ToDo *t) {
        Calendar *c = t->cal;
        ...
        case yearGlance:
                ...
                break;
        case monthGlance:
                format_date(t->view_list_date + 1,
                            get_int_prop(p, CP_DATEORDERING), buffer2, BUFSIZ,
                            0, 0, 0);
        ...
        if (t->view_frame) {
                ...
                XtVaSetValues(t->view_list_label, XmNlabelString, xmstr, NULL);
                XmStringFree(xmstr);
        }
}

extern void build_todo_list(ToDo *t, Tick date, Glance glance,
                            CSA_entry_handle **a, CSA_uint32 *count,
                            todo_view_op vf) {
        int range_count;
        CSA_sint32 state;
        time_t start, stop;
        ...
        stat = csa_list_entries(t->cal->cal_handle, range_count, range_attrs,
                                ops, count, a, NULL);
...
extern int build_todo_view(ToDo *t, Glance glance, Boolean redisplay) {
        int cnt;
        ...
        */
        ...
        if (redisplay == True) {
                ...
                build_todo_list(t, t->view_list_date, t->view_list_glance,
                                &entry_list, &entry_count, t->view_filter);
        } else {
        ...
        step = t->view_list;
        appt = allocate_appt_struct(
            appt_read, t->cal->general->version, CSA_ENTRY_ATTR_START_DATE_I,
            CSA_ENTRY_ATTR_SUMMARY_I, CSA_ENTRY_ATTR_STATUS_I, NULL);
        ...
        for (cnt = 1; cnt <= entry_count; cnt++) {
                ...
                */
                if (!step) {
                        ...
                        step->view_item_number = XtVaCreateManagedWidget(
                            "cnt", xmLabelGadgetClass, t->view_form_mgr,
                            XmNlabelString, str, NULL);
                        XmStringFree(str);

                        step->view_item_toggle = XtVaCreateManagedWidget(
                            str1, xmToggleButtonGadgetClass, t->view_form_mgr,
                            XmNalignment, XmALIGNMENT_BEGINNING,
                            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
                            step->view_item_number, XmNleftOffset, 5,
                            XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET,
                            XmNtopWidget, step->view_item_number, XmNtopOffset,
                            -3, NULL);
                        XtAddCallback(step->view_item_toggle,
                                      XmNvalueChangedCallback,
                                      t_view_change_proc, t);

                        step->next = NULL;
                        step->appt = 0;
                        if (last) {
                                XtVaSetValues(
                                    step->view_item_number, XmNtopAttachment,
                                    XmATTACH_WIDGET, XmNtopWidget,
                                    last->view_item_number, XmNtopOffset, 10,
                ...
                */
                stat = query_appt_struct(t->cal->cal_handle,
                                         entry_list[cnt - 1], appt);
                backend_err_msg(
                    t->frame, t->cal->view->current_calendar, stat,
                    ((Props_pu *)t->cal->properties_pu)->xm_error_pixmap);
                if (stat == CSA_SUCCESS) {
                        ...
                        if (lines && lines->s) {
                                buf_size =
                                    cm_strlen(str1) + cm_strlen(lines->s) + 3;
                        ...
                        str = XmStringCreateLocalized(buf);
                        XtVaSetValues(step->view_item_toggle, XmNlabelString,
                                      str, XmNset,
                                      (appt->state->value->item.sint32_value ==
...
extern void todo_clean_up(ToDo *t) {
        if (t->todo_head && t->todo_count > 0)
                csa_free((CSA_buffer)t->todo_head);
        t->todo_head = NULL;
        t->todo_count = 0;
...
extern Boolean todo_showing(ToDo *t) {
        if (t)
                return t->todo_is_up;
        return False;
}

extern Boolean todo_view_showing(ToDo *t) {
        if (t)
...
*/
extern void set_todo_defaults(ToDo *t) {
...
extern void set_todo_title(ToDo *t, char *name) {
        Calendar *c = t->cal;
        char buf[MAXNAMELEN];
        ...
        if (t->frame) {
                snprintf(buf, MAXNAMELEN, "%s - %s",
                         catgets(c->DT_catd, 1, 591, "Calendar : To Do Editor"),
                         name);
                XtVaSetValues(t->frame, XmNtitle, buf, NULL);
        }
}

extern void show_todo(Calendar *c) {
        ...
        if (!todo_showing(t)) {
                ...
                /* set default button */
                XmProcessTraversal(t->dssw.what_text, XmTRAVERSE_CURRENT);
                XtVaSetValues(t->base_form_mgr, XmNinitialFocus,
                              t->dssw.what_text, NULL);
        }
...
extern void show_todo_view(Calendar *c, todo_view_op view_filter) {
        int dv = get_data_version(c->cal_handle);
        ToDo *t = (ToDo *)c->todo;
        Props_pu *p = (Props_pu *)c->properties_pu;
        ...
        XtVaSetValues(t->view_form, XmNdefaultButton, t->view_apply_button,
                      NULL);
        XtVaSetValues(t->view_form, XmNcancelButton, t->view_cancel_button,
                      NULL);

        build_todo_view(t, c->view->glance, False);
        if (!todo_view_showing(t))
                ds_position_popup(c->frame, t->view_frame, DS_POPUP_LOR);
        XtVaSetValues(t->view_frame, XmNmappedWhenManaged, True, NULL);
 ...
 */
extern Boolean todo_insert(Dtcm_appointment *appt, CSA_entry_handle *new_a,
                           Calendar *c) {
        CSA_return_code stat;
        ToDo *t = (ToDo *)c->todo;
        Props_pu *p = (Props_pu *)c->properties_pu;
        ...
        stat =
            csa_add_entry(c->cal_handle, appt->count, appt->attrs, new_a, NULL);
