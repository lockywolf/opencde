...
#ifndef lint
static char sccsid[] =
    "@(#)yearglance.c 1.37 95/07/27 Copyr 1991 Sun Microsystems, Inc.";
#endif
 ...
 */
static Boolean allocated(Calendar *c) { return (c->view->year_info != NULL); }
...
static void allocator(Calendar *c) {
        ...
        y->month_panels = (Widget *)ckalloc(12 * sizeof(Widget));
...
extern CSA_return_code paint_year(Calendar *c) {
 ...
 */
void year_button(Widget widget, XtPointer data, XtPointer cbs) {
        Calendar *c = calendar;
...
extern void print_std_year_range(int start_year, int end_year) {
        void *xp;
        int n, year;
        Calendar *c = calendar;
 ...
 */
static void update_year(Calendar *c) {
        XmString str;
        char buf[BUFSIZ];
        Year *y = (Year *)c->view->year_info;
        ...
        */
        ...
        /* update each panel */
        for (i = 0; i < 12; i++) {
                XtVaSetValues(y->month_panels[i], XmNyear, year_num, XmNmonth,
                              i + 1, NULL);
        }
...
static void create_month_panels(Calendar *c) {
         ...
         */
        static int anchors[] = {
            0,  33, 24, 1,  0,  66, 24,  34, 0,  99, 24,  67, 25, 33, 49,  1,
            25, 66, 49, 34, 25, 99, 49,  67, 50, 33, 74,  1,  50, 66, 74,  34,
            50, 99, 74, 67, 75, 33, 100, 1,  75, 66, 100, 34, 75, 99, 100, 67};
        /*
       static int anchors[] = {
               0,	24,	33,	1,
               0,	49,	33,	26,
        ...
        ac++;
        XtSetArg(al[ac], XmNtopWidget, c->previous);
        ac++;
        ...
        XtSetArg(al[ac], XmNtopAttachment, XmATTACH_WIDGET);
        ac++;
        XtSetArg(al[ac], XmNtopWidget, y->label_form);
        ac++;
        XtSetArg(al[ac], XmNrightAttachment, XmATTACH_FORM);
        ac++;
        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_FORM);
        ...
        for (i = 0; i < 12; i++) {
                ...
                XtSetArg(al[ac], XmNtopPosition, anchors[4 * i]);
                ac++;
                XtSetArg(al[ac], XmNrightPosition, anchors[4 * i + 1]);
                ac++;
                XtSetArg(al[ac], XmNbottomPosition, anchors[4 * i + 2]);
                ac++;
                XtSetArg(al[ac], XmNleftPosition, anchors[4 * i + 3]);
                ac++;
 ...
 */
static void year_btn_cb(Widget w, XtPointer client, XtPointer call) {
...
extern void cleanup_after_yearview(Calendar *c) {
        Year *y = (Year *)c->view->year_info;
