...
static void v4_gap_pending_change(Widget w, XtPointer data, XtPointer cbs) {
...
static void v4_gap_clear_pending_change(Props_pu *p) {
...
extern void p_set_change(Props_pu *p) {
        ...
        if (p->base_form_mgr)
                XtVaSetValues(p->base_form_mgr, XmNdefaultButton,
                              p->apply_button, NULL);
}

static void p_clear_change(Props_pu *p) {
        ...
        if (p->cal->my_cal_version >= DATAVER4) {
                char *str;
                if (*(str = (char *)XmTextGetString(p->gap_user_text)) ==
                    '\0') {
        ...
        XtVaSetValues(p->base_form_mgr, XmNdefaultButton, p->close_button,
                      NULL);
}
static void p_mark_change(Widget w, XtPointer data, XtPointer cbs) {
...
static void set_menu_marker(Widget menu, Props_pu *p) {
        ...
        XtVaGetValues(submenu, XmNchildren, &children, XmNnumChildren, &num_c,
                      NULL);

        while (num_c > 0)
                XtAddCallback(children[--num_c], XmNactivateCallback,
                              p_mark_change, p);
}

#define GAP 5

static void p_create_editor_pane(Calendar *c) {
        int ac;
        Arg args[10];
        char buf[MAXNAMELEN];
        ...
        **  Base form
        */
        ep_form_mgr = XtVaCreateWidget(
            "ep_form_mgr", xmFormWidgetClass, pu->base_form_mgr,
            XmNautoUnmanage, False, XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, pu->separator1, XmNleftAttachment, XmATTACH_FORM,
            XmNrightAttachment, XmATTACH_FORM, XmNbottomOffset, 10, NULL);
        ...
        */
        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 644, "Appointment Times:"));
        pu->ep_appt_label = XtVaCreateWidget(
            "apptTimes", xmLabelGadgetClass, ep_form_mgr, XmNlabelString, xmstr,
            XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, GAP,
            XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, GAP, NULL);
        XmStringFree(xmstr);
        ...
        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 645, "Duration:"));
        pu->ep_dur_label = widgets[4] = XtVaCreateWidget(
            "duration", xmLabelGadgetClass, pu->ep_dssw.dssw_form_mgr,
            XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        ac = 0;
        XtSetArg(args[ac], XmNcomboBoxType, XmDROP_DOWN_COMBO_BOX);
        ++ac;
        XtSetArg(args[ac], XmNvisibleItemCount, 5);
        ++ac;
        XtSetArg(args[ac], XmNpositionMode, XmONE_BASED);
        ++ac;
        pu->ep_dur_cb = widgets[5] =
            XmCreateComboBox(pu->ep_dssw.dssw_form_mgr, "ep_dur_cb", args, ac);
        ...
        for (ac = 15; ac <= 90; ac += 15) {
                if (ac != 75) {
                        ...
                        XmComboBoxAddItem((Widget)pu->ep_dur_cb, xmstr, 0,
                                          False);
                        XmStringFree(xmstr);
                }
        }
        ...
        pu->ep_dur_message = widgets[6] = XtVaCreateWidget(
            "message", xmLabelGadgetClass, pu->ep_dssw.dssw_form_mgr,
            XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);
        ...
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, GAP,
                      NULL);
        XtVaSetValues(pu->ep_dur_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _toLabel, NULL);
        XtVaSetValues(pu->ep_dur_cb, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _toText, NULL);
        XtVaSetValues(pu->ep_dur_message, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _toMenu, NULL);
        prev = curr;
        ...
        _i18n_HighestWidgetAdjust(
            4, &curr, &highest, pu->ep_dssw.start_label, pu->ep_dssw.start_text,
            pu->ep_dssw.start_menu, pu->ep_dssw.start_ampm_rc_mgr);
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                      prev, XmNtopOffset, GAP, NULL);
        XtVaSetValues(pu->ep_dssw.start_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _toLabel, NULL);
        XtVaSetValues(pu->ep_dssw.start_text, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _toText, NULL);
        XtVaSetValues(pu->ep_dssw.start_menu, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _toMenu, NULL);
        XtVaSetValues(pu->ep_dssw.start_ampm_rc_mgr, XmNleftAttachment,
                      XmATTACH_FORM, XmNleftOffset, _toRC, NULL);

        XtVaSetValues(pu->ep_dssw.dssw_form_mgr, XmNtopAttachment,
                      XmATTACH_WIDGET, XmNtopWidget, pu->ep_appt_label,
                      XmNtopOffset, 10, XmNrightAttachment, XmATTACH_FORM,
                      XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET,
                      XmNleftWidget, pu->ep_appt_label, XmNleftOffset, 0, NULL);
        ...
        XtVaSetValues(pu->ep_reminders.bfpm_form_mgr, XmNtopAttachment,
                      XmATTACH_WIDGET, XmNtopWidget, pu->ep_dssw.dssw_form_mgr,
                      XmNtopOffset, 5, XmNleftAttachment, XmATTACH_FORM, NULL);

        /*
        **  Build the stuff for privacy ...
        */
        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 390, "Privacy:"));
        pu->privacy_label = XtVaCreateWidget("privacy", xmLabelGadgetClass,
                                             pu->ep_reminders.bfpm_form_mgr,
                                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        pu->privacy_menu = create_privacy_menu(pu->ep_reminders.bfpm_form_mgr);
        XtVaSetValues(pu->privacy_menu, XmNuserData, 0, NULL);
        ...
        _i18n_HighestWidgetAdjust(2, &curr, &highest, pu->privacy_label,
                                  pu->privacy_menu);
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                      prev, XmNtopOffset, 30, XmNbottomAttachment,
                      XmATTACH_FORM, XmNbottomOffset, 10, NULL);
        XtVaSetValues(pu->privacy_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 15, /* See reminders.c l. 783 */
                      NULL);
        XtVaSetValues(pu->privacy_menu, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 15 + widest + 5, NULL);

        XtAddCallback(pu->privacy_menu, XmNselectionCallback, p_mark_change,
                      pu);

        XtAddCallback(pu->ep_dssw.start_text, XmNvalueChangedCallback,
                      p_mark_change, pu);
        XtAddCallback(pu->ep_dssw.start_am, XmNvalueChangedCallback,
                      p_mark_change, pu);
        XtAddCallback(pu->ep_dssw.start_pm, XmNvalueChangedCallback,
                      p_mark_change, pu);
        XtAddCallback(pu->ep_reminders.beep_text, XmNvalueChangedCallback,
                      p_mark_change, pu);
        XtAddCallback(pu->ep_reminders.beep_toggle, XmNvalueChangedCallback,
                      p_mark_change, pu);

        set_menu_marker(pu->ep_reminders.beep_menu, pu);
        XtAddCallback(pu->ep_reminders.flash_text, XmNvalueChangedCallback,
                      p_mark_change, pu);
        XtAddCallback(pu->ep_reminders.flash_toggle, XmNvalueChangedCallback,
                      p_mark_change, pu);
        set_menu_marker(pu->ep_reminders.flash_menu, pu);
        XtAddCallback(pu->ep_reminders.mail_text, XmNvalueChangedCallback,
                      p_mark_change, pu);
        XtAddCallback(pu->ep_reminders.mail_toggle, XmNvalueChangedCallback,
                      p_mark_change, pu);
        set_menu_marker(pu->ep_reminders.mail_menu, pu);
        XtAddCallback(pu->ep_reminders.mailto_text, XmNvalueChangedCallback,
                      p_mark_change, pu);
        XtAddCallback(pu->ep_reminders.popup_text, XmNvalueChangedCallback,
                      p_mark_change, pu);
        XtAddCallback(pu->ep_reminders.popup_toggle, XmNvalueChangedCallback,
                      p_mark_change, pu);
...
static void format_hour(char buffer[], int buffer_size, int hour,
                        DisplayType dt, Calendar *c)

{
        if (dt == HOUR12) {
                if ((hour == 24) || (hour == 0))
                        cm_strlcpy(buffer,
                                   catgets(c->DT_catd, 1, 758, "midnight"),
                                   buffer_size);
                else if (hour > 12)
...
static void set_scale_value(Widget w, int val, Props_pu *p)

{
        ...
        format_hour(label_buffer, 50, val,
                    (DisplayType)get_int_prop(props, CP_DEFAULTDISP), c);
...
static void print_it(Widget w, XtPointer data, XtPointer cbs) {
        int val;
        Props_pu *p = (Props_pu *)data;
...
*/
static void p_dp_scale_mod_proc(Widget w, XtPointer data, XtPointer cbs) {
        int val;
        Props_pu *p = (Props_pu *)data;
        XmScaleCallbackStruct *scbs = (XmScaleCallbackStruct *)cbs;
...
static void p_create_display_pane(Props_pu *p) {
        Calendar *c = calendar;
        Widget dp_form_mgr;
        XmString xmstr;
        Widget widgets[2];
        ...
        **  Base form
        */
        dp_form_mgr = XtVaCreateWidget(
            "dp_form_mgr", xmFormWidgetClass, p->base_form_mgr, XmNautoUnmanage,
            False, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
            p->separator1, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
            XmATTACH_FORM, XmNbottomAttachment, XmATTACH_WIDGET,
            XmNbottomOffset, 10, XmNbottomWidget, p->separator2, NULL);
        ...
        */
        ...
        p->dp_loc_label =
            XtVaCreateWidget("location", xmLabelGadgetClass, dp_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        p->dp_loc_text = XtVaCreateWidget("dp_loc_text", xmTextWidgetClass,
                                          dp_form_mgr, NULL);

        XtAddCallback(p->dp_loc_text, XmNvalueChangedCallback, p_mark_change,
                      p);
        ...
        */
        ...
        p->dp_day_label =
            XtVaCreateWidget("dayBoundaries", xmLabelGadgetClass, dp_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 393, "Begin:"));
        p->dp_beg_label =
            XtVaCreateWidget("begin", xmLabelGadgetClass, dp_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        p->dp_beg_scale = XtVaCreateWidget(
            "dp_beg_scale", xmScaleWidgetClass, dp_form_mgr, XmNorientation,
            XmHORIZONTAL, XmNmaximum, 23, XmNminimum, 0, XmNvalue, 0,
            XmNscaleMultiple, 1, NULL);
        XtAddCallback(p->dp_beg_scale, XmNvalueChangedCallback,
                      p_dp_scale_mod_proc, p);
        XtAddCallback(p->dp_beg_scale, XmNdragCallback, p_dp_scale_mod_proc, p);
        ...
        p->dp_end_label =
            XtVaCreateWidget("end", xmLabelGadgetClass, dp_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        p->dp_end_scale = XtVaCreateWidget(
            "dp_end_scale", xmScaleWidgetClass, dp_form_mgr, XmNorientation,
            XmHORIZONTAL, XmNmaximum, 24, XmNminimum, 1, XmNvalue, 1,
            XmNscaleMultiple, 1, NULL);
        XtAddCallback(p->dp_end_scale, XmNvalueChangedCallback,
                      p_dp_scale_mod_proc, p);
        XtAddCallback(p->dp_end_scale, XmNdragCallback, p_dp_scale_mod_proc, p);
        ...
        */
        ...
        p->dp_hour_label =
            XtVaCreateWidget("hourDisplay", xmLabelGadgetClass, dp_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        p->dp_hour_rc_mgr = XtVaCreateWidget(
            "dp_hour_rc_mgr", xmRowColumnWidgetClass, dp_form_mgr, XmNpacking,
            XmPACK_COLUMN, XmNorientation, XmHORIZONTAL, XmNradioBehavior, True,
            XmNisHomogeneous, True, XmNentryClass, xmToggleButtonGadgetClass,
            NULL);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 396, "12 Hour"));
        p->dp_hour12_toggle = widgets[0] =
            XtVaCreateWidget("twelveHour", xmToggleButtonGadgetClass,
                             p->dp_hour_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dp_hour12_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 397, "24 Hour"));
        p->dp_hour24_toggle = widgets[1] =
            XtVaCreateWidget("twentyFourHour", xmToggleButtonGadgetClass,
                             p->dp_hour_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dp_hour24_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);
        ...
        */
        ...
        p->dp_view_label =
            XtVaCreateWidget("defaultView:", xmLabelGadgetClass, dp_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        p->dp_view_rc_mgr = XtVaCreateWidget(
            "dp_view_rc_mgr", xmRowColumnWidgetClass, dp_form_mgr, XmNpacking,
            XmPACK_COLUMN, XmNorientation, XmHORIZONTAL, XmNradioBehavior, True,
            XmNisHomogeneous, True, XmNentryClass, xmToggleButtonGadgetClass,
            NULL);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 402, "Day"));
        p->dp_view_day_toggle =
            XtVaCreateWidget("day", xmToggleButtonGadgetClass,
                             p->dp_view_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dp_view_day_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 401, "Week"));
        p->dp_view_week_toggle =
            XtVaCreateWidget("week", xmToggleButtonGadgetClass,
                             p->dp_view_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dp_view_week_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 400, "Month"));
        p->dp_view_month_toggle =
            XtVaCreateWidget("month", xmToggleButtonGadgetClass,
                             p->dp_view_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dp_view_month_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 399, "Year"));
        p->dp_view_year_toggle =
            XtVaCreateWidget("year", xmToggleButtonGadgetClass,
                             p->dp_view_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dp_view_year_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);
        ...
        */
        ...
        p->dp_init_view_label =
            XtVaCreateWidget("initialView", xmLabelGadgetClass, dp_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        p->dp_init_view_text = XtVaCreateWidget(
            "dp_init_view_text", xmTextWidgetClass, dp_form_mgr, NULL);

        XtAddCallback(p->dp_init_view_text, XmNvalueChangedCallback,
                      p_mark_change, p);

        /*
         * Here, do a layout
         */
        _i18n_WidestWidget(7, &curr, &widest, p->dp_loc_label, p->dp_day_label,
                           p->dp_beg_label, p->dp_end_label, p->dp_hour_label,
        ...
        _i18n_HighestWidgetAdjust(2, &curr, &highest, p->dp_loc_label,
                                  p->dp_loc_text);
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 12,
                      NULL);
        XtVaSetValues(p->dp_loc_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to1st, NULL);
        XtVaSetValues(p->dp_loc_text, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to2nd, NULL);
        prev = curr;
        ...
        _to1st = _to2nd - 10 - widest;
        XtVaSetValues(p->dp_day_label, XmNtopAttachment, XmATTACH_WIDGET,
                      XmNtopWidget, prev, XmNtopOffset, 10, NULL);
        XtVaSetValues(p->dp_day_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to1st, NULL);
        prev = p->dp_day_label;
        ...
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                      prev, XmNtopOffset, 12, NULL);
        XtVaSetValues(p->dp_beg_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to1st, NULL);
        XtVaSetValues(p->dp_beg_scale, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to2nd, NULL);
        XtVaSetValues(p->dp_beg_value, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to3rd, NULL);
        prev = curr;
        ...
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                      prev, XmNtopOffset, 12, NULL);
        XtVaSetValues(p->dp_end_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to1st, NULL);
        XtVaSetValues(p->dp_end_scale, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to2nd, NULL);
        XtVaSetValues(p->dp_end_value, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to3rd, NULL);
        prev = curr;
        ...
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                      prev, XmNtopOffset, 12, NULL);
        XtVaSetValues(p->dp_hour_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to1st, NULL);
        XtVaSetValues(p->dp_hour_rc_mgr, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to2nd, NULL);
        prev = curr;
        ...
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                      prev, XmNtopOffset, 12, NULL);
        XtVaSetValues(p->dp_view_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to1st, NULL);
        XtVaSetValues(p->dp_view_rc_mgr, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to2nd, NULL);
        prev = curr;
        ...
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                      prev, XmNtopOffset, 12, NULL);
        XtVaSetValues(p->dp_init_view_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to1st, NULL);
        XtVaSetValues(p->dp_init_view_text, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _to2nd, NULL);
        prev = curr;
...
extern unsigned int p_gap_convert_permissions(Props_pu *p) {
        Calendar *c = calendar;
        ...
        if (p->cal->my_cal_version >= DATAVER4) {
                ...
                if (XmToggleButtonGadgetGetState(p->gap2_semi_tog[GAP_VIEW]))
                        perm_val |= CSA_VIEW_CONFIDENTIAL_ENTRIES;
                if (XmToggleButtonGadgetGetState(p->gap2_semi_tog[GAP_INSERT]))
                        perm_val |= CSA_INSERT_CONFIDENTIAL_ENTRIES;
                if (XmToggleButtonGadgetGetState(p->gap2_semi_tog[GAP_CHANGE]))
                        perm_val |= CSA_CHANGE_CONFIDENTIAL_ENTRIES;
                if (XmToggleButtonGadgetGetState(p->gap2_private_tog[GAP_VIEW]))
        ...
        if (perm_val == 0) {
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 750,
                            "Calendar : Error - Access List and Permissions"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 405,
                            "Please select at least one permission.        "));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                dialog_popup(p->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                             BUTTON_IDENT, 1, ident1, BUTTON_HELP,
                             ACCESS_RIGHTS_ERROR_HELP, DIALOG_IMAGE,
                             p->xm_error_pixmap, NULL);
                XtFree(ident1);
                XtFree(text);
                XtFree(title);
        }
...
static char *p_gap_create_perm_str(CSA_flags perm_val) {
        int i = 0;
...
static void p_gap_create_entry_string(Props_pu *p, char *who, char *p_str,
                                      XmString *val) {
        char *buf;
...
static void p_gap_add_proc(Widget w, XtPointer client_data,
        ...
        */
        ...
        if (XmListGetSelectedPos(p->gap_access_list, &item_list, &item_cnt)) {
                ...
                */
                ...
                XmListReplaceItemsPos(p->gap_access_list, &xmstr, 1,
                                      *item_list);
...
extern void p_gap_remove_proc(Widget w, XtPointer client_data,
                              XtPointer callback_struct) {
        Calendar *c = calendar;
        int *item_list = NULL, item_cnt = 0;
        Props_pu *p = (Props_pu *)client_data;
                ...
                */
                ...
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 750,
                            "Calendar : Error - Access List and Permissions"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 408,
                            "Please select a name first.                   "));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                dialog_popup(p->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                             BUTTON_IDENT, 1, ident1, DIALOG_IMAGE,
                             p->xm_error_pixmap, NULL);
                XtFree(ident1);
                XtFree(text);
                XtFree(title);
...
extern void gap_list_select_proc(Widget w, XtPointer client_data,
                                 XtPointer data) {
        int *item_list = NULL, item_cnt = 0;
        Props_pu *p = (Props_pu *)client_data;
        ...
        if (p->cal->my_cal_version >= DATAVER4) {
                ...
                XmToggleButtonGadgetSetState(
                    p->gap2_public_tog[GAP_INSERT],
                    (step->rights & CSA_INSERT_PUBLIC_ENTRIES) ? True : False,
                    True);
                XmToggleButtonGadgetSetState(
                    p->gap2_public_tog[GAP_CHANGE],
                    (step->rights & CSA_CHANGE_PUBLIC_ENTRIES) ? True : False,
                    True);
                XmToggleButtonGadgetSetState(
                    p->gap2_semi_tog[GAP_VIEW],
                    (step->rights & CSA_VIEW_CONFIDENTIAL_ENTRIES) ? True
                                                                   : False,
                    True);
                XmToggleButtonGadgetSetState(
                    p->gap2_semi_tog[GAP_INSERT],
                    (step->rights & CSA_INSERT_CONFIDENTIAL_ENTRIES) ? True
                                                                     : False,
                    True);
                XmToggleButtonGadgetSetState(
                    p->gap2_semi_tog[GAP_CHANGE],
                    (step->rights & CSA_CHANGE_CONFIDENTIAL_ENTRIES) ? True
                                                                     : False,
                    True);
                XmToggleButtonGadgetSetState(
                    p->gap2_private_tog[GAP_VIEW],
                    (step->rights & CSA_VIEW_PRIVATE_ENTRIES) ? True : False,
                    True);
                XmToggleButtonGadgetSetState(
                    p->gap2_private_tog[GAP_INSERT],
                    (step->rights & CSA_INSERT_PRIVATE_ENTRIES) ? True : False,
                    True);
                XmToggleButtonGadgetSetState(
                    p->gap2_private_tog[GAP_CHANGE],
                    (step->rights & CSA_CHANGE_PRIVATE_ENTRIES) ? True : False,
                    True);
        } else {
                XmToggleButtonGadgetSetState(
                    p->gap_browse_toggle,
...
static void p_create_v4_group_access_pane(Props_pu *p) {
        Calendar *c = calendar;
        char *buf;
        Widget gap_form_mgr;
        XmString xmstr;
        ...
        **  Base form
        */
        gap_form_mgr = XtVaCreateWidget(
            "gap_form_mgr", xmFormWidgetClass, p->base_form_mgr,
            XmNautoUnmanage, False, XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, p->separator1, XmNleftAttachment, XmATTACH_FORM,
            XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment,
            XmATTACH_WIDGET, XmNbottomOffset, 10, XmNbottomWidget,
            p->separator2, NULL);
        ...
        */
        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 410, "User Name:"));
        p->gap_user_label =
            XtVaCreateWidget("userName", xmLabelGadgetClass, gap_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        p->gap_user_text = XtVaCreateWidget("gap_user_text", xmTextWidgetClass,
                                            gap_form_mgr, NULL);

        XtAddCallback(p->gap_user_text, XmNvalueChangedCallback,
                      v4_gap_pending_change, (XtPointer)p);

        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 411, "Add Name"));
        p->gap_add_button = XtVaCreateWidget(
            "addName", xmPushButtonGadgetClass, gap_form_mgr, XmNlabelString,
            xmstr, XmNnavigationType, XmTAB_GROUP, XmNsensitive, False, NULL);
        XmStringFree(xmstr);
        XtAddCallback(p->gap_add_button, XmNactivateCallback, p_gap_add_proc,
                      p);
        ...
        */
        {
                char *user =
                    XtNewString(catgets(c->DT_catd, 1, 983, "User Name"));
                char *perm =
                    XtNewString(catgets(c->DT_catd, 1, 413, "Permissions"));
        ...
        p->gap_access_header =
            XtVaCreateManagedWidget("gap_access_header", xmLabelGadgetClass,
                                    gap_form_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);
        free(buf);

        p->gap_list = NULL;
        p->gap_access_list = (Widget)XmCreateScrolledList(
            gap_form_mgr, "gap_access_list", NULL, 0);
        p->gap_access_list_sw = XtParent(p->gap_access_list);
        XtVaSetValues(p->gap_access_list_sw, NULL);
        XtVaSetValues(p->gap_access_list, XmNvisibleItemCount, 10,
                      XmNdoubleClickInterval, 5, NULL);
        XtAddCallback(p->gap_access_list, XmNbrowseSelectionCallback,
                      gap_list_select_proc, p);
        XtManageChild(p->gap_access_list);

        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 414, "Remove Name"));
        p->gap_remove_button = XtVaCreateWidget(
            "removeName", xmPushButtonGadgetClass, gap_form_mgr, XmNlabelString,
            xmstr, XmNnavigationType, XmTAB_GROUP, XmNsensitive, False, NULL);
        XmStringFree(xmstr);
        XtAddCallback(p->gap_remove_button, XmNactivateCallback,
                      p_gap_remove_proc, p);

        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 415, "B = Browse I = Insert D = Delete"));
        p->gap_access_msg =
            XtVaCreateWidget("browseInsertDelete", xmLabelGadgetClass,
                             gap_form_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);
        ...
        */
        ...
        p->gap_perm_label =
            XtVaCreateWidget("permissions", xmLabelGadgetClass, gap_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        p->gap_perm_rc_mgr = XtVaCreateWidget(
            "gap_perm_rc_mgr", xmRowColumnWidgetClass, gap_form_mgr, XmNpacking,
            XmPACK_COLUMN, XmNorientation, XmHORIZONTAL, XmNradioBehavior,
            False, XmNisHomogeneous, True, XmNindicatorType, XmN_OF_MANY,
            XmNentryClass, xmToggleButtonGadgetClass, NULL);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 417, "Browse"));
        p->gap_browse_toggle =
            XtVaCreateWidget("browse", xmToggleButtonGadgetClass,
                             p->gap_perm_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 342, "Insert"));
        p->gap_insert_toggle =
            XtVaCreateWidget("insert", xmToggleButtonGadgetClass,
                             p->gap_perm_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 419, "Delete"));
        p->gap_delete_toggle =
            XtVaCreateWidget("delete", xmToggleButtonGadgetClass,
                             p->gap_perm_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);
        ...
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 12,
                      NULL);
        prev = curr;
        XtVaSetValues(p->gap_user_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 5, NULL);
        _i18n_WidestWidget(1, &curr, &widest, p->gap_user_label);
        XtVaSetValues(p->gap_user_text, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 5 + widest + 10, NULL);
        _i18n_WidestWidget(1, &curr, &dmy, p->gap_user_text);
        _toEdge = 5 + widest + 10 + dmy;
        XtVaSetValues(p->gap_add_button, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, _toEdge + 10, NULL);

        /*
         * p->gap_access_header
         */
        XtVaSetValues(p->gap_access_header, XmNleftAttachment, XmATTACH_FORM,
        ...
        _i18n_HighestWidget(2, &curr, &highest, p->gap_access_list_sw,
                            p->gap_remove_button);
        XtVaSetValues(p->gap_access_list_sw, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 5, XmNrightAttachment,
                      XmATTACH_OPPOSITE_FORM, XmNrightOffset, -1 * _toEdge,
                      XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, prev,
                      XmNtopOffset, 5, NULL),
            XtVaSetValues(p->gap_remove_button, XmNleftAttachment,
                          XmATTACH_FORM, XmNleftOffset, _toEdge + 10,
                          XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, prev,
                          XmNtopOffset, 5, NULL),
            prev = curr;
         ...
         * p->gap_access_msg
         */
        XtVaSetValues(p->gap_access_msg, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 5, XmNtopAttachment, XmATTACH_WIDGET,
                      XmNtopWidget, prev, XmNtopOffset, 5, NULL);
        ...
        XtVaSetValues(curr, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                      prev, XmNtopOffset, 12, NULL);
        prev = curr;
        XtVaSetValues(p->gap_perm_label, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 5, NULL);
        _i18n_WidestWidget(1, &curr, &widest, p->gap_perm_label);
        XtVaSetValues(p->gap_perm_rc_mgr, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 5 + widest + 5, NULL);
...
static char *hdrDefaults[] = {"Date", "User Id", "Page Number", "Report Type",
                              "None"};
static char *hdrStrings[POP_HDR_NUM_CHOICES];

static void p_create_printer_ops_pane(Props_pu *p) {
        Calendar *c = calendar;
        Widget pop_form_mgr;
        XmString xmstr;
        int i;
        ...
        **  Base form
        */
        pop_form_mgr = XtVaCreateWidget(
            "pop_form_mgr", xmFormWidgetClass, p->base_form_mgr,
            XmNautoUnmanage, False, XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, p->separator1, XmNtopOffset, GAP, XmNleftAttachment,
            XmATTACH_FORM, XmNleftOffset, GAP, XmNrightAttachment,
            XmATTACH_FORM, XmNrightOffset, GAP, XmNbottomAttachment,
            XmATTACH_WIDGET, XmNbottomWidget, p->separator2, XmNbottomOffset,
            GAP, NULL);
        p->pane_list[PRINTER_OPS_PANE] = pop_form_mgr;

        p->pop_margin_frame = _DtPrintCreateMarginFrame(pop_form_mgr);
        XtVaSetValues(p->pop_margin_frame, XmNtopAttachment, XmATTACH_FORM,
                      XmNtopOffset, 0, XmNleftAttachment, XmATTACH_FORM,
                      XmNleftOffset, 0, XmNrightAttachment, XmATTACH_FORM,
                      XmNrightOffset, 0, NULL);
        p->pop_left_text = _DtPrintMarginFrameEnumToWidget(
            p->pop_margin_frame, DTPRINT_OPTION_MARGIN_LEFT);
        XtAddCallback(p->pop_left_text, XmNvalueChangedCallback, p_mark_change,
                      (XtPointer)p);
        p->pop_top_text = _DtPrintMarginFrameEnumToWidget(
            p->pop_margin_frame, DTPRINT_OPTION_MARGIN_TOP);
        XtAddCallback(p->pop_top_text, XmNvalueChangedCallback, p_mark_change,
                      (XtPointer)p);
        p->pop_right_text = _DtPrintMarginFrameEnumToWidget(
            p->pop_margin_frame, DTPRINT_OPTION_MARGIN_RIGHT);
        XtAddCallback(p->pop_right_text, XmNvalueChangedCallback, p_mark_change,
                      (XtPointer)p);
        p->pop_bottom_text = _DtPrintMarginFrameEnumToWidget(
            p->pop_margin_frame, DTPRINT_OPTION_MARGIN_BOTTOM);
        XtAddCallback(p->pop_bottom_text, XmNvalueChangedCallback,
                      p_mark_change, (XtPointer)p);

        for (i = 0; i < POP_HDR_NUM_CHOICES; i++)
                hdrStrings[i] = XtNewString(
                    catgets(c->DT_catd, 1, 1106 + i, hdrDefaults[i]));

        p->pop_header_frame = _DtPrintCreateHdrFtrFrame(
            pop_form_mgr, POP_HDR_NUM_CHOICES, hdrStrings, (void **)NULL);
        XtVaSetValues(p->pop_header_frame, XmNtopAttachment, XmATTACH_WIDGET,
                      XmNtopWidget, p->pop_margin_frame, XmNtopOffset, GAP,
                      XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 0,
                      XmNrightAttachment, XmATTACH_FORM, XmNrightOffset, 0,
                      NULL);
        _DtPrintHdrFtrFrameMenuWidgets(p->pop_header_frame, &optionPane,
                                       (int *)NULL, (Widget **)NULL);
        XtAddCallback(optionPane, XmNentryCallback, p_mark_change,
                      (XtPointer)p);

        p->pop_tl_option = _DtPrintHdrFtrFrameEnumToWidget(
            p->pop_header_frame, DTPRINT_OPTION_HEADER_LEFT);

        p->pop_tr_option = _DtPrintHdrFtrFrameEnumToWidget(
            p->pop_header_frame, DTPRINT_OPTION_HEADER_RIGHT);

        p->pop_bl_option = _DtPrintHdrFtrFrameEnumToWidget(
            p->pop_header_frame, DTPRINT_OPTION_FOOTER_LEFT);

        p->pop_br_option = _DtPrintHdrFtrFrameEnumToWidget(
            p->pop_header_frame, DTPRINT_OPTION_FOOTER_RIGHT);

        xmstr =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 434, "Include:"));
        p->pop_privacy_label = XtVaCreateWidget(
            "include", xmLabelGadgetClass, pop_form_mgr, XmNlabelString, xmstr,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopOffset, GAP, XmNtopWidget,
            p->pop_header_frame, XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET,
            XmNleftOffset, 0, XmNleftWidget, p->pop_margin_frame, NULL);
        XmStringFree(xmstr);

        /*
        **  Include label and toggle choice
        */
        p->pop_privacy_rc_mgr = XtVaCreateWidget(
            "pop_privacy_rc_mgr", xmRowColumnWidgetClass, pop_form_mgr,
            XmNpacking, XmPACK_COLUMN, XmNorientation, XmVERTICAL,
            XmNradioBehavior, False, XmNisHomogeneous, True, XmNindicatorType,
            XmN_OF_MANY, XmNentryClass, xmToggleButtonGadgetClass,
            XmNmarginHeight, 0, XmNmarginWidth, 0, XmNtopAttachment,
            XmATTACH_OPPOSITE_WIDGET, XmNtopOffset, 0, XmNtopWidget,
            p->pop_privacy_label, XmNleftAttachment, XmATTACH_WIDGET,
            XmNleftOffset, GAP, XmNleftWidget, p->pop_privacy_label, NULL);

        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 435, "Show Time and Text Appts"));
        p->pop_privacy_public_toggle = XtVaCreateWidget(
            "showTimeText", xmToggleButtonGadgetClass, p->pop_privacy_rc_mgr,
            XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->pop_privacy_public_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 436, "Show Time Only Appts"));
        p->pop_privacy_semi_toggle = XtVaCreateWidget(
            "showTimeOnly", xmToggleButtonGadgetClass, p->pop_privacy_rc_mgr,
            XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->pop_privacy_semi_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 437, "Show Nothing Appts"));
        p->pop_privacy_private_toggle = XtVaCreateWidget(
            "showNothing", xmToggleButtonGadgetClass, p->pop_privacy_rc_mgr,
            XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->pop_privacy_private_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);
...
*/
static void p_create_date_format_pane(Props_pu *p) {
        Calendar *c = calendar;
        Widget dfp_form_mgr;
        XmString xmstr;
        Dimension max_left_label_width;
        ...
        **  Base form
        */
        dfp_form_mgr = XtVaCreateWidget(
            "dfp_form_mgr", xmFormWidgetClass, p->base_form_mgr,
            XmNautoUnmanage, False, XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, p->separator1, XmNleftAttachment, XmATTACH_FORM,
            XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment,
            XmATTACH_WIDGET, XmNbottomOffset, 10, XmNbottomWidget,
        ...
        */
        ...
        p->dfp_order_label =
            XtVaCreateWidget("dateOrdering", xmLabelGadgetClass, dfp_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);
        ...
        p->dfp_sep_label =
            XtVaCreateWidget("dateSeparator", xmLabelGadgetClass, dfp_form_mgr,
                             XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        max_left_label_width =
            ComputeMaxWidth(p->dfp_order_label, p->dfp_sep_label, NULL, NULL) +
            2 * GAP;

        p->dfp_order_rc_mgr = XtVaCreateWidget(
            "dfp_order_rc_mgr", xmRowColumnWidgetClass, dfp_form_mgr,
            XmNpacking, XmPACK_COLUMN, XmNorientation, XmVERTICAL,
            XmNradioBehavior, True, XmNisHomogeneous, True, XmNentryClass,
            xmToggleButtonGadgetClass, XmNleftAttachment, XmATTACH_FORM,
            XmNleftOffset, max_left_label_width, XmNtopAttachment,
            XmATTACH_FORM, XmNtopOffset, GAP, NULL);

        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 439, "MM | DD | YY"));
        p->dfp_order_mmddyy_toggle =
            XtVaCreateWidget("mmddyy", xmToggleButtonGadgetClass,
                             p->dfp_order_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dfp_order_mmddyy_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 440, "DD | MM | YY"));
        p->dfp_order_ddmmyy_toggle =
            XtVaCreateWidget("ddmmyy", xmToggleButtonGadgetClass,
                             p->dfp_order_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dfp_order_ddmmyy_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 441, "YY | MM | DD"));
        p->dfp_order_yymmdd_toggle =
            XtVaCreateWidget("yymmdd", xmToggleButtonGadgetClass,
                             p->dfp_order_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dfp_order_yymmdd_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);
        ...
        */
        p->dfp_sep_rc_mgr = XtVaCreateWidget(
            "dfp_sep_rc_mgr", xmRowColumnWidgetClass, dfp_form_mgr, XmNpacking,
            XmPACK_COLUMN, XmNorientation, XmVERTICAL, XmNradioBehavior, True,
            XmNisHomogeneous, True, XmNentryClass, xmToggleButtonGadgetClass,
            XmNleftAttachment, XmATTACH_FORM, XmNleftOffset,
            max_left_label_width, XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, p->dfp_order_rc_mgr, XmNtopOffset, GAP, NULL);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 443, "Blank"));
        p->dfp_sep_blank_toggle =
            XtVaCreateWidget("blank", xmToggleButtonGadgetClass,
                             p->dfp_sep_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dfp_sep_blank_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized("/");
        p->dfp_sep_slash_toggle =
            XtVaCreateWidget("oblique", xmToggleButtonGadgetClass,
                             p->dfp_sep_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dfp_sep_slash_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized(".");
        p->dfp_sep_period_toggle =
            XtVaCreateWidget("period", xmToggleButtonGadgetClass,
                             p->dfp_sep_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dfp_sep_period_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        xmstr = XmStringCreateLocalized("-");
        p->dfp_sep_dash_toggle =
            XtVaCreateWidget("hyphen", xmToggleButtonGadgetClass,
                             p->dfp_sep_rc_mgr, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        XtAddCallback(p->dfp_sep_dash_toggle, XmNvalueChangedCallback,
                      p_mark_change, p);

        XtVaSetValues(p->dfp_order_label, XmNtopAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNtopWidget,
                      p->dfp_order_rc_mgr, XmNtopOffset, GAP,
                      XmNrightAttachment, XmATTACH_WIDGET, XmNrightWidget,
                      p->dfp_order_rc_mgr, XmNrightOffset, GAP, NULL);

        XtVaSetValues(p->dfp_sep_label, XmNtopAttachment,
                      XmATTACH_OPPOSITE_WIDGET, XmNtopWidget, p->dfp_sep_rc_mgr,
                      XmNtopOffset, GAP, XmNrightAttachment, XmATTACH_WIDGET,
                      XmNrightWidget, p->dfp_sep_rc_mgr, XmNrightOffset, GAP,
                      NULL);
...
*/
static void p_category_notify(Widget w, XtPointer client_data,
                              XtPointer call_data) {
         ...
         */
        XtVaGetValues(base_form_mgr, XmNuserData, &p, NULL);
        if (p)
                show_ui(p, item_no);
}
...
static void activate_ui(Props_pu *p, PaneType item_no) {
        ...
        nargs = 0;
        XtSetArg(args[nargs], XmNsubMenuId, &optMenu);
        nargs++;
        XtGetValues(p->category_menu, args, nargs);

        if (optMenu != (Widget)NULL) {
                nargs = 0;
                XtSetArg(args[nargs], XmNchildren, &children);
                nargs++;
                XtSetArg(args[nargs], XmNnumChildren, &nChildren);
                nargs++;
...
static void show_ui(Props_pu *p, PaneType item_no) {
        ...
        if (p_flush_changes(c) == False) {
                ...
                XtVaGetValues(optMenu, XmNchildren, &children, XmNnumChildren,
                              &nChildren, NULL);
        ...
        */
        ...
        XtVaSetValues(p->separator2, XmNtopWidget, p->pane_list[item_no], NULL);
        ...
        XtVaSetValues(p->dp_beg_scale, XmNbottomOffset,
                      ((int)label_height - (int)slider_height) / 2, NULL);
        XtVaSetValues(p->dp_end_scale, XmNbottomOffset,
                      ((int)label_height - (int)slider_height) / 2, NULL);
...
extern void set_editor_defaults_later(XtPointer client_data,
...
static Boolean p_save_changes(Calendar *c) {
        ...
        if (!save_props(p)) {
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 454, "Calendar : Error - Options"));
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 445,
                    "An error occurred writing to ~/.desksetdefaults.\nMake "
                    "sure you have permission to write to this file."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                dialog_popup(pu->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                             BUTTON_IDENT, 1, ident1, BUTTON_HELP,
                             DESKSET_DEFAULTS_ERROR_HELP, DIALOG_IMAGE,
                             pu->xm_error_pixmap, NULL);
                XtFree(ident1);
                XtFree(text);
                XtFree(title);
                return (False);
        ...
        if (redisplay_mask & PROPS_REDO_DISPLAY) {
                ...
                XtVaGetValues(c->canvas, XmNwidth, &width, XmNheight, &height,
                              NULL);
                ...
                if (c->editor && ((Editor *)c->editor)->view_list)
                        build_editor_view(
                            (Editor *)c->editor,
                            ((Editor *)(c->editor))->view_list_glance, True);

                if (c->todo && ((ToDo *)c->todo)->view_frame)
                        build_todo_view((ToDo *)c->todo,
                                        ((ToDo *)(c->todo))->view_list_glance,
                                        True);
        ...
        if (redisplay_mask & PROPS_REDO_EDITORS) {
                ...
                XtVaSetValues(c->browse_button, XmNsubMenuId, c->browse_menu,
                              NULL);
                if (c->browser)
                ...
                XtAppAddTimeOut(c->xcontext->app, 50, set_editor_defaults_later,
                                c);
...
*/
static void p_apply_proc(Widget w, XtPointer client_data, XtPointer data) {
        Calendar *c = (Calendar *)client_data;

        p_save_changes(c);
}

static void p_ok_proc(Widget w, XtPointer client_data, XtPointer data) {
        Calendar *c = (Calendar *)client_data;
        Props_pu *pu = (Props_pu *)c->properties_pu;
...
static void p_defaults_proc(Widget w, XtPointer client_data, XtPointer data) {
...
extern void p_defaults_proc_helper(XtPointer client_data) {
        ...
        if (!read_props(p)) {
                ...
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 448,
                    "An error occurred reading from ~/.desksetdefaults.\nMake "
                    "sure you have permission to read this file."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                dialog_popup(pu->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                             BUTTON_IDENT, 1, ident1, BUTTON_HELP,
...
static void p_close_proc(Widget w, XtPointer client_data, XtPointer data) {
        Props_pu *p = (Props_pu *)client_data;

        XtUnmanageChild(p->base_form_mgr);
}

extern Boolean p_flush_changes(Calendar *c) {
        Props *p = (Props *)c->properties;
        Props_pu *pu = (Props_pu *)c->properties_pu;
        static int answer;
        int redisplay_mask;
        ...
        if (pu->changed_flag == True) {
                char *title = XtNewString(
                    catgets(c->DT_catd, 1, 450, "Calendar : Options - Help"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 451,
                            "You have made unsaved changes.\nYou may save your "
                            "changes, discard your changes, \nor return to "
                            "your previous place in the dialog."));
                char *ident1 = XtNewString(catgets(c->DT_catd, 1, 452, "Save"));
                char *ident2 =
                    XtNewString(catgets(c->DT_catd, 1, 700, "Discard"));
                char *ident3 =
                    XtNewString(catgets(c->DT_catd, 1, 923, "Cancel"));
                answer =
                    dialog_popup(pu->frame, DIALOG_TITLE, title, DIALOG_TEXT,
                                 text, BUTTON_IDENT, 1, ident1, BUTTON_IDENT, 2,
                                 ident2, BUTTON_IDENT, 3, ident3, DIALOG_IMAGE,
                                 pu->xm_warning_pixmap, NULL);
                XtFree(ident3);
                XtFree(ident2);
                XtFree(ident1);
                XtFree(text);
...
static void p_quit_handler(Widget w, XtPointer cdata, XtPointer data) {
...
static void p_make_props_pu(Calendar *c) {
        char *buf;
        char *popuplabel = catgets(c->DT_catd, 1, 458, "Calendar : Options");
        PaneType pt;
        ...
        */
        p->cal = c;
        p->pane_list =
            (Widget *)ckalloc(sizeof(Widget) * (DATE_FORMAT_PANE + 1));
        ...
        */
        ...
        p->frame = XtVaCreatePopupShell(
            "frame", xmDialogShellWidgetClass, c->frame, XmNdeleteResponse,
            XmDO_NOTHING, XmNtitle, buf, XmNallowShellResize, True, NULL);
        free(buf);
        setup_quit_handler(p->frame, p_quit_handler, (caddr_t)c);

        p->base_form_mgr = XtVaCreateWidget(
            "base_form_mgr", xmFormWidgetClass, p->frame, XmNautoUnmanage,
            False, XmNhorizontalSpacing, 1, XmNverticalSpacing, 1, XmNuserData,
            p, XmNresizable, False, XmNfractionBase, 29, XmNtopAttachment,
            XmATTACH_FORM, XmNtopOffset, 0, XmNbottomAttachment, XmATTACH_FORM,
            XmNbottomOffset, 0, XmNrightAttachment, XmATTACH_FORM,
            XmNrightOffset, 0, XmNleftAttachment, XmATTACH_FORM, XmNleftOffset,
            0, NULL);
        ...
        */
        ...
        Pane_XmStrs[EDITOR_PANE] = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 762, "Editor Defaults"));
        Pane_XmStrs[DISPLAY_PANE] = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 763, "Display Settings"));
        Pane_XmStrs[GROUP_ACCESS_PANE] = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 764, "Access List and Permissions"));
        Pane_XmStrs[PRINTER_OPS_PANE] = XmStringCreateLocalized(
            catgets(c->DT_catd, 1, 765, "Printer Settings"));
        Pane_XmStrs[DATE_FORMAT_PANE] =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 766, "Date Format"));

        label =
            XmStringCreateLocalized(catgets(c->DT_catd, 1, 459, "Category:"));

        p->category_menu = widgets[0] = XmVaCreateSimpleOptionMenu(
            p->base_form_mgr, "category_menu", label, 0, 0, p_category_notify,
            ...
            XmNrightAttachment, XmATTACH_NONE, XmNnavigationType, XmTAB_GROUP,
            NULL);
        XmStringFree(label);

        p->separator1 = widgets[1] = XtVaCreateWidget(
            "separator1", xmSeparatorGadgetClass, p->base_form_mgr,
            XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, p->category_menu,
            NULL);
        ...
        */
        ...
        p->separator2 = widgets[2] =
            XtVaCreateWidget("separator2", xmSeparatorGadgetClass,
                             p->base_form_mgr, XmNleftAttachment, XmATTACH_FORM,
         ...
         */
        XtVaSetValues(p->pane_list[EDITOR_PANE], XmNbottomAttachment,
                      XmATTACH_WIDGET, XmNbottomOffset, 10, XmNbottomWidget,
                      p->separator2, NULL);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 655, "OK"));
        p->ok_button = widgets[3] = XtVaCreateWidget(
            "ok", xmPushButtonGadgetClass, p->base_form_mgr, XmNlabelString,
            xmstr, XmNrecomputeSize, False, XmNbottomAttachment, XmATTACH_FORM,
            XmNbottomOffset, 7, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 2, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 7, XmNrightOffset, 5, NULL);
        XmStringFree(xmstr);
        XtAddCallback(p->ok_button, XmNactivateCallback, p_ok_proc, c);

        XtVaSetValues(p->separator2, XmNbottomAttachment, XmATTACH_WIDGET,
                      XmNbottomOffset, 10, XmNbottomWidget, p->ok_button, NULL);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 460, "Apply"));
        p->apply_button = widgets[4] = XtVaCreateWidget(
            "apply", xmPushButtonGadgetClass, p->base_form_mgr, XmNlabelString,
            xmstr, XmNrecomputeSize, False, XmNbottomAttachment, XmATTACH_FORM,
            XmNbottomOffset, 7, XmNtopWidget, p->separator2, XmNleftAttachment,
            XmATTACH_POSITION, XmNleftPosition, 7, XmNrightAttachment,
            XmATTACH_POSITION, XmNrightPosition, 12, XmNrightOffset, 5, NULL);
        XmStringFree(xmstr);
        ...
        p->defaults_button = widgets[5] = XtVaCreateWidget(
            "reset", xmPushButtonGadgetClass, p->base_form_mgr, XmNlabelString,
            xmstr, XmNrecomputeSize, False, XmNbottomAttachment, XmATTACH_FORM,
            XmNbottomOffset, 7, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 12, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 17, XmNrightOffset, 5, NULL);
        XmStringFree(xmstr);
        XtAddCallback(p->defaults_button, XmNactivateCallback, p_defaults_proc,
                      c);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 923, "Cancel"));
        p->close_button = widgets[6] = XtVaCreateWidget(
            "close", xmPushButtonGadgetClass, p->base_form_mgr, XmNlabelString,
            xmstr, XmNbottomAttachment, XmATTACH_FORM, XmNbottomOffset, 7,
            XmNtopWidget, p->separator2, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 17, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 22, XmNrightOffset, 5, NULL);
        XmStringFree(xmstr);
        XtAddCallback(p->close_button, XmNactivateCallback, p_close_proc, p);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 77, "Help"));
        p->help_button = widgets[7] = XtVaCreateWidget(
            "help", xmPushButtonGadgetClass, p->base_form_mgr, XmNlabelString,
            xmstr, XmNbottomAttachment, XmATTACH_FORM, XmNbottomOffset, 7,
            XmNtopWidget, p->separator2, XmNleftAttachment, XmATTACH_POSITION,
            XmNleftPosition, 22, XmNrightAttachment, XmATTACH_POSITION,
            XmNrightPosition, 27, NULL);
        XmStringFree(xmstr);
...
static Boolean string_is_number(char *str) {
...
static Boolean string_is_time(char *str) {
        ...
        if ((found_colon == True) &&
            ((((found_first_number > 2) || (found_first_number < 1)) ||
              (found_second_number != 2))))
                return (False);
        else if ((found_colon == False) &&
                 ((found_first_number > 4) || found_first_number < 3))
                return (False);
...
**
*******************************************************************************/
extern Boolean get_editor_vals_from_ui(Props_pu *pu, Props *p) {
        int i, hr, min;
        char buf[5];
        Widget text;
        ...
        if (string_is_number(dur_txt) == False) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 1079, "Calendar : Error - Editor Settings"));
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 819,
                    "The Duration value must be an integer with no sign.\nYour "
                    "properties settings have not been saved."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                ...
                dialog_popup(pu->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                             BUTTON_IDENT, 1, ident1, BUTTON_HELP,
                             DURATION_ERROR_HELP, DIALOG_IMAGE,
                             pu->xm_error_pixmap, NULL);
                XtFree(ident1);
                XtFree(text);
                XtFree(title);
        ...
        if (atoi(dur_txt) >= (60 * 24)) {
                ...
                char *text = XtNewString(catgets(
                    c->DT_catd, 1, 916,
                    "The Duration value must be less than 24 hours.\nYour "
                    "properties settings have not been saved."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
        ...
        if (string_is_time(pu->ep_dssw.start_val.val) == False) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 1079, "Calendar : Error - Editor Settings"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 820,
                            "The start value must be a valid time.\nYour "
                            "properties settings have not been saved."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                dialog_popup(pu->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                             BUTTON_IDENT, 1, ident1, BUTTON_HELP,
                             START_ERROR_HELP, DIALOG_IMAGE,
                             pu->xm_error_pixmap, NULL);
                XtFree(ident1);
                XtFree(text);
                XtFree(title);
        ...
        if ((status = get_reminders_vals(&pu->ep_reminders, False)) !=
            REMINDERS_OK) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 1079, "Calendar : Error - Editor Settings"));
                char *text;
                ...
                case ADVANCE_BLANK:
                        text = XtNewString(catgets(
                            c->DT_catd, 1, 908,
                            "You may not set a blank reminder value.\nYour "
                            "properties settings have not been saved."));
                        break;

                case ADVANCE_CONTAINS_BLANK:
                        text = XtNewString(
                            catgets(c->DT_catd, 1, 909,
                                    "You may not set a reminder value with an "
                ...
                case ADVANCE_NONNUMERIC:
                        text = XtNewString(
                            catgets(c->DT_catd, 1, 910,
                                    "Reminder values must be a number with an "
                                    "optional sign.\nYour properties settings "
                                    "have not been saved."));
                        break;
                }
                dialog_popup(pu->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                             BUTTON_IDENT, 1, ident1, DIALOG_IMAGE,
                             pu->xm_error_pixmap, BUTTON_HELP,
                             ADVANCE_ERROR_HELP, NULL);
                XtFree(ident1);
                XtFree(text);
                XtFree(title);
                return (True);
...
extern Boolean get_display_vals_from_ui(Props_pu *pu, Props *p) {
        int val;
        char *str;
        Calendar *c = pu->cal;
        ...
        if (blank_buf(str) || embedded_blank(str)) {
                char *title =
                    XtNewString(catgets(c->DT_catd, 1, 1083,
                                        "Calendar : Error - Display Settings"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 772,
                            "You may not save a blank User Calendar "
                ...
                dialog_popup(pu->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                             BUTTON_IDENT, 1, ident1, BUTTON_HELP,
                             CAL_LOCATION_ERROR_HELP, DIALOG_IMAGE,
                             pu->xm_error_pixmap, NULL);
                XtFree(ident1);
                XtFree(text);
                XtFree(title);
        ...
        XtVaGetValues(pu->dp_beg_scale, XmNvalue, &val, NULL);
        set_int_prop(p, CP_DAYBEGIN, val);
        XtVaGetValues(pu->dp_end_scale, XmNvalue, &val, NULL);
        set_int_prop(p, CP_DAYEND, val);

        set_int_prop(p, CP_DEFAULTDISP,
                     XmToggleButtonGadgetGetState(pu->dp_hour24_toggle)
                         ? HOUR24
                         : HOUR12);
        ...
        if (blank_buf(str) || embedded_blank(str)) {
                char *title =
                    XtNewString(catgets(c->DT_catd, 1, 1083,
                                        "Calendar : Error - Display Settings"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 773,
                            "You may not save a blank Initial Calendar "
                            "View\nor one that contains blanks.\nYour "
                            "properties settings have not been saved."));
                char *ident1 =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
...
extern Boolean get_gap_vals_from_ui(Props_pu *pu, Props *p) {
        CSA_return_code stat;
        Dtcm_calendar *cal;

        cal = allocate_cal_struct(appt_write, pu->cal->my_cal_version,
                                  CSA_CAL_ATTR_ACCESS_LIST_I, NULL);
        cal->access_list->value->item.access_list_value = pu->gap_list;
        pu->gap_list = NULL;

        stat = csa_update_calendar_attributes(pu->cal->my_cal_handle,
                                              cal->count, cal->attrs, NULL);
...
static void print_option_error(Props_pu *pu, int setNum, int msgNum,
                               char *str) {
        Calendar *c = pu->cal;
        ...
        if (title == (char *)NULL)
                title =
                    XtNewString(catgets(c->DT_catd, 1, 1085,
                                        "Calendar : Error - Printer Settings"));
        if (ident1 == (char *)NULL)
                ident1 = XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));

        dialog_popup(pu->frame, DIALOG_TITLE, title, DIALOG_TEXT, text,
                     BUTTON_IDENT, 1, ident1, BUTTON_HELP, PRINT_HELP_BUTTON,
                     DIALOG_IMAGE, pu->xm_error_pixmap, NULL);
        XtFree(text);
}

extern Boolean get_printer_vals_from_ui(Props_pu *pu, Props *p) {
        char *str;
        int plevel = 0;
        ...
        if ((str = _DtPrintGetMarginSpec(pu->pop_right_text)) != (char *)NULL) {
                if (valid_margin_str(pu, str)) {
                        ...
                        print_option_error(pu, 1, 821,
                                           ...
                                           "right margin value.\nYour "
                                           "properties settings have not been "
                                           "saved.");
                        XtFree(str);
                        return True;
                }
        ...
        if ((str = _DtPrintGetMarginSpec(pu->pop_bottom_text)) !=
            (char *)NULL) {
                if (valid_margin_str(pu, str)) {
        ...
        if ((str = _DtPrintGetMarginSpec(pu->pop_top_text)) != (char *)NULL) {
                if (valid_margin_str(pu, str)) {
                        ...
                        print_option_error(pu, 1, 912,
                                           "You have specified an illegal top "
                                           "margin value.\nYour properties "
                                           "settings have not been saved.");
                        XtFree(str);
                        return True;
...
extern Boolean get_date_vals_from_ui(Props_pu *pu, Props *p) {
...
extern Boolean get_props_from_ui(Props_pu *pu, Props *p, int *redisplay_mask) {
        ...
        *redisplay_mask = 0;
        ...
        case DISPLAY_PANE:
                if (get_display_vals_from_ui(pu, p) == True)
                        ...
                        *redisplay_mask |=
                            (PROPS_REDO_EDITORS | PROPS_REDO_DISPLAY);
                break;
        case GROUP_ACCESS_PANE:
...
extern void set_editor_vals_on_ui(Props_pu *pu, Props *p) {
        int i;
        char buf[MAXNAMELEN];
        Widget text;
        Reminders_val *val;
        ...
        XtVaGetValues(pu->ep_dur_cb, XmNtextField, &text, NULL);
        if (text) {
                snprintf(buf, MAXNAMELEN, "%d",
                         get_int_prop(p, CP_APPTDURATION));
        ...
        strlcpy(pu->ep_reminders.mailto_val, get_char_prop(p, CP_MAILTO),
                MAILTO_LEN);
...
extern void set_display_vals_on_ui(Props_pu *pu, Props *p) {
        ...
        case DAY_VIEW:
                XmToggleButtonGadgetSetState(pu->dp_view_day_toggle, True,
                                             True);
                break;
        case WEEK_VIEW:
                XmToggleButtonGadgetSetState(pu->dp_view_week_toggle, True,
                                             True);
                break;
        case YEAR_VIEW:
                XmToggleButtonGadgetSetState(pu->dp_view_year_toggle, True,
                                             True);
                break;
        case MONTH_VIEW:
        default:
                XmToggleButtonGadgetSetState(pu->dp_view_month_toggle, True,
                                             True);
                break;
        }
...
extern void set_gap_vals_on_ui(Props_pu *pu, Props *p) {
        char *p_str;
        XmString xmstr;
        CSA_return_code stat;
        Dtcm_calendar *cal;
        ...
        cal = allocate_cal_struct(appt_read, pu->cal->my_cal_version,
                                  CSA_CAL_ATTR_ACCESS_LIST_I, NULL);
...
static Boolean valid_margin_str(Props_pu *pu, char *str) {
        XtEnum badStr;

        XmConvertStringToUnits(XtScreen(pu->pop_margin_frame), str,
                               XmHORIZONTAL, XmINCHES, &badStr);

        return (Boolean)!badStr;
}

extern void set_printer_vals_on_ui(Props_pu *pu, Props *p) {
        int plevel;
        ...
        */
        ...
        if (plevel & PRINT_SEMIPRIVATE)
                XmToggleButtonGadgetSetState(pu->pop_privacy_semi_toggle, True,
                                             True);
...
extern void set_date_vals_on_ui(Props_pu *pu, Props *p) {
        SeparatorType st = get_int_prop(p, CP_DATESEPARATOR);
        OrderingType ot = get_int_prop(p, CP_DATEORDERING);

        switch (ot) {
        case ORDER_MDY:
                XmToggleButtonGadgetSetState(pu->dfp_order_mmddyy_toggle, True,
                                             True);
                break;
        case ORDER_DMY:
                XmToggleButtonGadgetSetState(pu->dfp_order_ddmmyy_toggle, True,
                                             True);
                break;
        case ORDER_YMD:
        default:
                XmToggleButtonGadgetSetState(pu->dfp_order_yymmdd_toggle, True,
                                             True);
                break;
        }
        ...
        case SEPARATOR_BLANK:
                XmToggleButtonGadgetSetState(pu->dfp_sep_blank_toggle, True,
                                             True);
                break;
        case SEPARATOR_SLASH:
                XmToggleButtonGadgetSetState(pu->dfp_sep_slash_toggle, True,
                                             True);
                break;
        case SEPARATOR_DOT:
                XmToggleButtonGadgetSetState(pu->dfp_sep_period_toggle, True,
                                             True);
                break;
        case SEPARATOR_DASH:
                XmToggleButtonGadgetSetState(pu->dfp_sep_dash_toggle, True,
                                             True);
                break;
        }
}

extern void set_props_on_ui(Props_pu *pu, Props *p) {
...
extern void show_props_pu(Calendar *c) {
        Props *p = (Props *)c->properties;
        Props_pu *pu = (Props_pu *)c->properties_pu;
...
void show_print_props_pu(Calendar *c) {
        Props *p = (Props *)c->properties;
        Props_pu *pu = (Props_pu *)c->properties_pu;
...
extern void props_pu_clean_up(Props_pu *p) {
        if (p->pane_list)
                free(p->pane_list);
        p->pane_list = NULL;
}

extern Boolean props_pu_showing(Props_pu *p) {
...
static void p_gap_clear_toggles(Props_pu *p) {
        ...
        if (p->cal->my_cal_version >= DATAVER4) {
                ...
                XmToggleButtonGadgetSetState(p->gap2_public_tog[GAP_CHANGE],
                                             False, True);
                XmToggleButtonGadgetSetState(p->gap2_semi_tog[GAP_VIEW], False,
                                             True);
                ...
                XmToggleButtonGadgetSetState(p->gap_browse_toggle, False, True);
                XmToggleButtonGadgetSetState(p->gap_insert_toggle, False, True);
                XmToggleButtonGadgetSetState(p->gap_delete_toggle, False, True);
        }
}

static void p_gap_center_cat_menu(Widget frame, Widget menu) {
