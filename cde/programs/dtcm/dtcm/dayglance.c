...
#ifndef lint
static char sccsid[] =
    "@(#)dayglance.c 1.76 95/04/24 Copyr 1991 Sun Microsystems, Inc.";
#endif
...
static Boolean _print_day(Calendar *, int, void *, Tick, Props *, Boolean);
...
extern void paint_day_header(Calendar *c, Tick date, void *rect) {
        ...
        */
        ...
        if (c->xcontext->screen_depth >= 8 && inrange)
                gr_text_rgb(c->xcontext, x, c->view->topoffset - (pfy / 2),
                            c->fonts->labelfont, buf, foreground_pixel, cmap,
                            rect);
        else
...
extern int morning(int hr) { return (hr < 12); }

static void paint_dayview_appts(Calendar *c, Paint_cache *cache, int a_total,
                                void *rect) {
        int w = c->view->boxw;
        int h = c->view->boxh;
        ...
        for (i = begin_time - 1; i < end_time; i++) {
                ...
                /* REVISIT: unclear why we're still distinguishing between
                   gr_text[_rgb] if (c->xcontext->screen_depth >= 8)
        ...
        for (i = 0; i < a_total; i++) {
                ...
                if (cache[i].show_time == 0) {
                        ...
                        if (curr_line < maxlines) {
                                ...
                                headlines = lines =
                                    text_to_lines(cache[i].summary, 4);
                                ...
                                /* REVISIT: unclear why we're still
                                   distinguishing between gr_text[_rgb] if
                                   (c->xcontext->screen_depth >= 8)
                                                                        gr_text_rgb(c->xcontext,
                                   x, y, pf2, appt_str, fg, cmap, rect); else
                                */
                                ...
                                if (curr_line < maxlines && lines != NULL) {
                                        ...
                                        /* REVISIT: unclear why we're still
                                           distinguishing between gr_text[_rgb]
        ...
        for (i = 0; i < a_total; i++) {
                ...
                if (hr >= begin_time && hr < end_time &&
                    (cache[i].show_time && !magic_time(start_tick))) {
                        if (last_hr != hr)
                                curr_line = 0;
                        y = c->view->topoffset + 2 + pfy;
                        if (curr_line < maxlines) {
...
extern void init_mo(Calendar *c) {
...
extern void init_dayview(Calendar *c) {
        ...
        day_info->mobox_width =
            (int)MOBOX_AREA_WIDTH - 2 * c->view->outside_margin;
        /* col width of day number in month boxes */
        day_info->col_w = (day_info->mobox_width - INSIDE_MARGIN * 2) / 7;
        /* width of all of the month boxes */
        day_info->mobox_width = 7 * day_info->col_w + 2 * INSIDE_MARGIN;
        ...
        day_info->row_h =
            (c->view->winh - 3 * c->view->topoffset - c->view->outside_margin) /
            tot_rows;

        /* height of 1st month */
        day_info->mobox_height1 = day_info->row_h * (wks_1 + 1) + 1;
        /* height of 2nd month */
        day_info->mobox_height2 = day_info->row_h * (wks_2 + 1) + 1;
        /* height of 2rd month */
        day_info->mobox_height3 = day_info->row_h * (wks_3 + 1) + 1;

        day_info->month1_y = c->view->topoffset;
        day_info->month2_y = 2 * c->view->topoffset + day_info->mobox_height1;
        day_info->month3_y = 3 * c->view->topoffset + day_info->mobox_height1 +
                             day_info->mobox_height2;
        ((Selection *)(c->view->current_selection))->row = 0;
}

extern void monthbox_xytodate(Calendar *c, int x, int y) {
        char str[5];
        Day *day_info = (Day *)c->view->day_info;
        XFontSetExtents fontextents;
        int pfy;
        ...
        if (y < (day_info->month1_y + day_info->mobox_height1)) {
                row = (y - day_info->month1_y - row_h) / row_h;
                day_selected =
                    (7 * (row + 1)) - fdom(day_info->month1) - (6 - col);
                if (day_selected <= 0 ||
                    day_selected > monthlength(day_info->month1))
                        return;
        ...
        } else if (y < (day_info->month2_y + day_info->mobox_height2)) {
                row = (y - day_info->month2_y - row_h) / row_h;
                day_selected =
                    (7 * (row + 1)) - fdom(day_info->month2) - (6 - col);
                if (day_selected <= 0 ||
                    day_selected > monthlength(day_info->month2))
                        return;
                day_info->day_selected = day_selected;
                day_info->day_selected_y =
                ...
                c->view->date = next_ndays(first_dom(day_info->month2),
                                           day_info->day_selected);
        } else if (y < (day_info->month3_y + day_info->mobox_height3)) {
                row = (y - day_info->month3_y - row_h) / row_h;
                day_selected =
                    (7 * (row + 1)) - fdom(day_info->month3) - (6 - col);
                if (day_selected <= 0 ||
                    day_selected > monthlength(day_info->month3))
                        return;
                day_info->day_selected = day_selected;
                day_info->day_selected_y =
...
extern void monthbox_datetoxy(Calendar *c) {
        char str[5];
        ...
        day_info->day_selected_x =
            c->view->outside_margin + INSIDE_MARGIN + col_w * dayw;

        if (mo == month(day_info->month1))
                day_info->day_selected_y = day_info->month1_y + row_h * week;
        else if (mo == month(day_info->month2))
                day_info->day_selected_y = day_info->month2_y + row_h * week;
        else if (mo == month(day_info->month3))
                day_info->day_selected_y = day_info->month3_y + row_h * week;
...
static Boolean in_moboxes(Calendar *c, int x, int y) {
        ...
        if (x < (MOBOX_AREA_WIDTH - margin - 2 * INSIDE_MARGIN) && x > margin &&
            y > topoff &&
            ((y < (day_info->month3_y + day_info->mobox_height3) &&
              y > (day_info->month3_y + row_h)) ||
             (y < (day_info->month2_y + day_info->mobox_height2) &&
              y > (day_info->month2_y + row_h)) ||
             (y < (day_info->month1_y + day_info->mobox_height1) &&
...
extern void paint_dayview(Calendar *c, Boolean repaint, XRectangle *rect,
                          Boolean update_months) {
        Props *p = (Props *)c->properties;
        int num_hrs;
        ...
        if (c->paint_cache == NULL) {
                ...
                csa_list_entries(c->cal_handle, j, range_attrs, ops, &a_total,
                                 &list, NULL);
        ...
        XtVaGetValues(d->month_panels[1], XmNyear, &panel1_year, XmNmonth,
                      &panel1_month, NULL);
        ...
        XtVaGetValues(d->month_panels[top_panel], XmNyear, &panel0_year,
                      XmNmonth, &panel0_month, NULL);

        if ((year_num < panel0_year) ||
            ((year_num == panel0_year) && (month_num < panel0_month)))
                day_not_on_panel = True;

        XtVaGetValues(d->month_panels[bottom_panel], XmNyear, &panel2_year,
                      XmNmonth, &panel2_month, NULL);
        ...
        if (repaint) {
                ...
                line_length =
                    c->view->topoffset + ((end - beg + 1) * c->view->boxh);
                /* draw line separating mo. boxes and appts. */
                gr_draw_line(c->xcontext, (int)MOBOX_AREA_WIDTH + 1, 0,
                             (int)MOBOX_AREA_WIDTH + 1, line_length, gr_solid,
                             rect);
                gr_draw_line(c->xcontext, (int)MOBOX_AREA_WIDTH + 2, 0,
                             (int)MOBOX_AREA_WIDTH + 2, line_length, gr_solid,
                             rect);
                ...
                paint_dayview_appts(c, c->paint_cache, c->paint_cache_size,
                                    rect);
        }
...
extern void paint_day(Calendar *c) {
        Day *day_info = (Day *)c->view->day_info;
 ...
 */
extern void day_button(Widget widget, XtPointer data, XtPointer cbs) {
        Calendar *c = calendar;
...
extern void print_day_range(Calendar *c, Tick start_tick, Tick end_tick) {
        int n;
        ...
        for (; n > 0; n--) {
                while (!done) {
                            ...
                            _print_day(c, num_page, xp, first_date, pr, first);
                        num_page++;
                        first = False;
                }
                done = False;
...
static int count_day_pages(Calendar *c, int lines_per_page, Tick tick) {
        ...
        for (i = daybegin; i < dayend; i++) {
                ...
                setup_range(&range_attrs, &ops, &j, start, end, CSA_TYPE_EVENT,
                            0, B_FALSE, c->general->version);
                csa_list_entries(c->cal_handle, j, range_attrs, ops, &a_total,
                                 &list, NULL);
                free_range(&range_attrs, &ops, j);
...
static Boolean _print_day(Calendar *c, int num_page, void *xp, Tick first_date,
                          Props *p, Boolean first) {
        char buf[100];
        ...
        if ((!morning(daybegin)) || dayend <= 12)
                ...
                timeslots = ((12 - daybegin) > (dayend - 12)) ? (12 - daybegin)
                                                              : (dayend - 12);
        ...
        for (i = daybegin; i < dayend; i++) {
                ...
                if (lines_per_page > 0)
                        done = x_print_multi_appts(xp, list, a_total, num_page,
                                                   dayGlance);
                else
                        done = True;
...
static int day_xytoclock(Calendar *c, int x, int y, Tick t) {
        int daybegin, hr, val;
        char buf[10];
        struct tm tm;
        Props *p;
        ...
        (void)snprintf(buf, 10, "%d/%d/%d", tm.tm_mon + 1, tm.tm_mday,
                       tm.tm_year + 1900);
...
extern void day_event(XEvent *event) {
        ...
        case ButtonPress:
                if (ds_is_double_click(&lastevent, event)) {
                        ...
                        if (lastcol == xy.x && lastrow == xy.y)
                                show_editor(c, j, next_nhours(j, 1),
                                            xy.y == 0 ? True : False);
                        if (in_mbox) {
                                monthbox_deselect(c);
                        ...
                        if (in_mbox) {
                                ...
                                if (j >= 0) {
                                        ...
                                        if (geditor_showing(ge)) {
                                                set_geditor_defaults(
                                                    ge, j, next_nhours(j, 1));
                                                add_all_gappt(ge);
                                        }
                                }
                        }
...
static void display_monthpanels(Calendar *c) {
        Day *d = (Day *)c->view->day_info;
        char buf[BUFSIZ];
        ...
        XtVaGetValues(d->month_panels[1], XmNyear, &panel_year, XmNmonth,
                      &panel_month, NULL);
        ...
        if ((panel_year == firstyear) && (panel_month == 1)) {
                ...
                snprintf(buf, BUFSIZ, "%s",
                         catgets(c->DT_catd, 1, 623,
                                 "Calendar does not display dates prior to "
                                 "January 1, 1970"));
                ...
                snprintf(buf, BUFSIZ, "%s",
                         catgets(c->DT_catd, 1, 624,
                                 "Calendar does not display dates after "
                                 "December 31, 2037"));
                set_message(c->message_text, buf);
        } else {
                XtMapWidget(d->month_panels[0]);
...
extern void cleanup_after_dayview(Calendar *c) {
        Day *d = (Day *)c->view->day_info;
        ...
        /* DITTO ABOVE: THIS ISN'T NEEDED TILL WE ACTUALLY DO RESIZE THE
           CANVAS TO ADJOIN THE MONTHPANELS WHILE DAY VIEW IS UP
                XtVaSetValues(c->canvas, XmNleftAttachment, XmATTACH_FORM,
           NULL);
        */
...
static void create_month_panels(Calendar *c) {
        Day *d = (Day *)c->view->day_info;
        Arg al[20];
        int ac;
        ...
        XtSetArg(al[ac], XmNbottomAttachment, XmATTACH_POSITION);
        ac++;
        XtSetArg(al[ac], XmNbottomPosition, 33);
        ac++;
        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_FORM);
        ac++;
        XtSetArg(al[ac], XmNmonth, month(previousmonth(c->view->date)));
        ac++;
        XtSetArg(al[ac], XmNyear, year(previousmonth(c->view->date)));
        ac++;
        d->month_panels[0] =
            XmCreateMonthPanel(d->panel_form, "lastMonth", al, ac);
        XtAddCallback(d->month_panels[0], XmNactivateCallback, day_btn_cb,
                      (XtPointer)c);
        XtAddCallback(d->month_panels[0], XmNhelpCallback,
                      (XtCallbackProc)help_view_cb, NULL);

        ac = 0;
        XtSetArg(al[ac], XmNtopAttachment, XmATTACH_POSITION);
        ac++;
        XtSetArg(al[ac], XmNtopPosition, 33);
        ac++;
        XtSetArg(al[ac], XmNrightAttachment, XmATTACH_FORM);
        ac++;
        XtSetArg(al[ac], XmNbottomAttachment, XmATTACH_POSITION);
        ac++;
        XtSetArg(al[ac], XmNbottomPosition, 66);
        ac++;
        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_FORM);
        ac++;
        XtSetArg(al[ac], XmNmonth, month(c->view->date));
        ac++;
        XtSetArg(al[ac], XmNyear, year(c->view->date));
        ac++;
        d->month_panels[1] =
            XmCreateMonthPanel(d->panel_form, "thisMonth", al, ac);
        XtAddCallback(d->month_panels[1], XmNactivateCallback, day_btn_cb,
                      (XtPointer)c);
        ...
        XtSetArg(al[ac], XmNtopAttachment, XmATTACH_POSITION);
        ac++;
        XtSetArg(al[ac], XmNtopPosition, 66);
        ac++;
        XtSetArg(al[ac], XmNrightAttachment, XmATTACH_FORM);
        ac++;
        ...
        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_FORM);
        ac++;
        XtSetArg(al[ac], XmNmonth, month(nextmonth(c->view->date)));
        ac++;
        XtSetArg(al[ac], XmNyear, year(nextmonth(c->view->date)));
        ac++;
        d->month_panels[2] =
            XmCreateMonthPanel(d->panel_form, "nextMonth", al, ac);
        XtAddCallback(d->month_panels[2], XmNactivateCallback, day_btn_cb,
                      (XtPointer)c);
 ...
 */
static void update_quarter(Calendar *c) {
        Day *d = (Day *)c->view->day_info;
        int year_num, month_num;
        ...
        XtVaSetValues(d->month_panels[0], XmNyear, year_num, XmNmonth,
                      month_num, NULL);

        /* current month */
        year_num = year(c->view->date);
        month_num = month(c->view->date);
        XtVaSetValues(d->month_panels[1], XmNyear, year_num, XmNmonth,
                      month_num, NULL);

        /* next month */
        year_num = year(nextmonth(c->view->date));
        month_num = month(nextmonth(c->view->date));
        XtVaSetValues(d->month_panels[2], XmNyear, year_num, XmNmonth,
                      month_num, NULL);
}

static void day_btn_cb(Widget w, XtPointer client, XtPointer call) {
