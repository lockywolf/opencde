...
static Boolean print_week(Calendar *c, int num_page, void *xp, Tick first_date,
...
#define inweek(w, x, y)                                                        \
        ((x >= w->x && x <= w->x + w->width && y >= w->y &&                    \
          y <= w->y + w->day_height) ||                                        \
         (x >= w->x + 3 * w->day_width && x <= w->x + w->width &&              \
          y >= w->y + w->day_height && y <= w->y + w->height))

extern void format_week_header(Tick date, OrderingType order, char *buf) {
        Calendar *c = calendar;
        struct tm *tm;
        ...
        */
        strftime(buf, 80,
                 catgets(c->DT_catd, 1, 993, "Week Starting %A, %B %e, %Y"),
                 tm);
}

static int count_week_pages(Calendar *c, int lines_per_page, Tick start_date) {
        time_t start, stop;
...
static Boolean print_week(Calendar *c, int num_page, void *xp, Tick first_date,
        ...
        static char *days[] = {(char *)NULL, (char *)NULL, (char *)NULL,
        ...
        if (days[0] == (char *)NULL) {
                ...
                days[1] =
                    XtNewString(catgets(c->DT_catd, 1, 597, "Tuesday %d"));
                days[2] =
                    XtNewString(catgets(c->DT_catd, 1, 598, "Wednesday %d"));
                days[3] =
                    XtNewString(catgets(c->DT_catd, 1, 599, "Thursday %d"));
                days[4] = XtNewString(catgets(c->DT_catd, 1, 600, "Friday %d"));
                days[5] =
                    XtNewString(catgets(c->DT_catd, 1, 601, "Saturday %d"));
        ...
        if (num_page > 1) {
                ...
                total_pages =
                    (lines_per_page > 0)
                        ? count_week_pages(c, lines_per_page, start_date)
                        : 1;
...
extern void print_week_range(Calendar *c, Tick start_tick, Tick end_tick) {
        ...
        for (; num_weeks > 0; num_weeks--) {
                while (!done) {
                            ...
                            print_week(c, num_page, xp, first_date, p, first);
                        num_page++;
                        first = False;
                }
                done = False;
...
extern int count_multi_appts(CSA_entry_handle *list, int num_entries,
                             Calendar *c) {
        CSA_return_code stat;
        ...
        appt = allocate_appt_struct(
            ...
            CSA_X_DT_ENTRY_ATTR_SHOWTIME_I, CSA_ENTRY_ATTR_SUMMARY_I, NULL);
        for (i = 0; i < num_entries; i++) {
                ...
                l_ptr = lines =
                    text_to_lines(appt->what->value->item.string_value, 10);
                while (lines != NULL) {
                        count++;
                        lines = lines->next;
                }
 ...
 */
void week_button(Widget widget, XtPointer data, XtPointer cbs) {
        Calendar *c = calendar;
 ...
 */
extern void cleanup_after_weekview(Calendar *c) {
        int n;
 ...
 */
static void allocator(Calendar *c) {
        int n;
        Week *w;
        Arg args[1];
 ...
 * allocate storage & subwidgets used by week view
 */
static void deallocator(Calendar *c) {
        Week *w = (Week *)c->view->week_info;
        int n;
 ...
 */
static void init_week(Calendar *c, Boundary *boundary) {
        Week *w = (Week *)c->view->week_info;
        ...
        *boundary = okay;
         ...
         */
        XtVaGetValues(c->canvas, XmNwidth, &w->canvas_w, XmNheight,
                      &w->canvas_h, NULL);
...
extern void paint_weekview(Calendar *c, XRectangle *rect) {
        Boundary boundary;
...
static void cm_update_segs(Week *w, Tick tick, Tick dur, int *start_index,
                           int *end_index, Boolean addto) {
...
static void add_extra_pixels(int i, int num_pixels, int *h) {
        if (((i + 1) % BOX_SEG) == 0)
                *h += num_pixels;
}

static void chart_draw_appts(Week *w, int start, int end) {
        ...
        x = w->chart_x +
            ((double)start / (double)segs_in_col * (double)w->chart_day_width);
...
extern void resize_weekview(Calendar *c, Boundary *boundary) {
        register int x, y;
        register int n;
        Week *w = (Week *)c->view->week_info;
        ...
        for (n = 0; n < 7; n++) {
                ...
                XtVaSetValues(w->hot_button[n], XmNx, x + 2, XmNy, y + 1,
                              XmNwidth, w->day_width - 3, XmNheight,
                              w->label_height - 2, NULL);
                x += w->day_width;
        }
}
...
static void draw_week(Calendar *c, XRectangle *rect, Boundary boundary) {
        ...
        if (c->paint_cache == NULL) {
                setup_range(&range_attrs, &ops, &i, start, stop, CSA_TYPE_EVENT,
                            0, B_FALSE, c->general->version);
                csa_list_entries(c->cal_handle, i, range_attrs, ops, &a_total,
                                 &list, NULL);
        ...
        CalTextExtents(w->font, days2[3], cm_strlen(days2[3]), &notused,
                       &notused, &width1, &notused);
        CalTextExtents(w->font, "  00", cm_strlen("  00"), &notused, &notused,
                       &width2, &notused);
        CalTextExtents(w->font, "Wed 00", cm_strlen("Wed 00"), &notused,
                       &notused, &width3, &notused);
        ...
        gr_draw_box(xc, x - 1, y, 2 * w->day_width + 2, w->day_height + 1,
                    rect);
        ...
        for (n = 0; n < 4; n++) {
                if (n < 3) {
                        gr_draw_line(xc, x, y, x, y + w->day_height, gr_solid,
                                     rect);
                } else {
        ...
        for (n = 0; n < 7; n++) {
                ...
                if (boundary == okay) {
                        ...
                        fill_day(c, w, x, y, current_day, c->paint_cache,
                                 c->paint_cache_size, rect);
                        current_day += daysec;
                        if (lower_bound > 0) {
                                snprintf(buf, 128, "%s",
                                         catgets(c->DT_catd, 1, 623,
                                                 "Calendar does not display "
                                                 "dates prior to January 1, "
                                                 "1970"));
                                footer_message = buf;
                        } else
                                footer_message = NULL;
                        ...
                        snprintf(buf, 128, "%s",
                                 catgets(c->DT_catd, 1, 624,
                                         "Calendar does not display dates "
                                         "after December 31, 2037"));
                        footer_message = buf;
                        if (upper_bound > 4)
                                clear_hot_btn(c, n);
                }
 ...
 */
static void format_entry(Paint_cache *cache_entry, char *buf1, int buf1_size,
         ...
         */
        *buf1 = *buf2 = '\0';
        if (cache_entry == NULL || cache_entry->summary == NULL)
                return;
...
static int paint_entry(Calendar *c, int x, int y, int maxchars,
                       Paint_cache *cache_entry, XRectangle *rect) {
        ...
        if (cache_entry->show_time && !magic_time(tick) && (buf1[0] != '\0')) {
                maxchars =
                    gr_nchars(w->day_width - 5, buf1, c->fonts->boldfont);
        ...
        if (buf2[0] != '\0') {
                maxchars =
                    gr_nchars(w->day_width - 5, buf2, c->fonts->viewfont);
...
static void fill_day(Calendar *c, Week *w, int x, int y, int day,
        ...
        for (loop = 0; loop < 2; loop++) {
                for (i = 0; i < a_total; i++) {
                        ...
                        if ((cache[i].start_time < lower) ||
                            (cache[i].start_time >= upper))
                                continue;
                        ...
                        if (nlines < maxlines) {
                                n = paint_entry(c, x, y, maxchars, &cache[i],
                                                rect);
                                y += n * char_height;
                                nlines += n;
                        }
...
static void draw_chart(Calendar *c, register Week *w, XRectangle *rect) {
        register int x, y;
        int n;
        XFontSetExtents fontextents;
        int char_height;
        ...
        y = w->chart_y;
        for (n = 0; n < 7; n++) {
                gr_draw_line(xc, w->chart_x + (w->chart_day_width * n), y,
                             w->chart_x + (w->chart_day_width * n),
         ...
         */
        gr_draw_box(xc, w->chart_x, w->chart_y, w->chart_width, w->chart_height,
                    rect);
        gr_draw_box(xc, w->chart_x - 1, w->chart_y - 1, w->chart_width + 2,
                    w->chart_height + 2, rect);
        ...
        for (n = w->begin_hour; n <= w->end_hour; n++) {
                ...
                gr_text(xc, w->x - 8, y + 3, w->small_bold_font, label, rect);
                y += w->chart_hour_height + w->add_pixels;
        }
...
extern void week_event(XEvent *event) {
        Calendar *c = calendar;
        Props *p = (Props *)c->properties;
        static int lastdate;
        ...
        case ButtonPress:
                ...
                if (j == -1)
                        return;
                hr = (inchart(w, x, y)) ? week_xytohour(w, x, y)
                                        : (wsel->row + w->begin_hour);

                if (ds_is_double_click(&lastevent, event)) {
                        _DtTurnOnHourGlass(c->frame);
                        if (j == lastdate) {
                                show_editor(c, next_nhours(j, hr),
                                            next_nhours(j, hr + 1), False);
                        } else if (editor_showing(e)) {
                                set_editor_defaults(e, next_nhours(j, hr),
                                                    next_nhours(j, hr + 1),
                                                    False);
                                add_all_appt(e);
                        }
                        ...
                        calendar_deselect(c);
                        (wsel->col = dow(j)) == 0 ? wsel->col = 6 : wsel->col--;
                        if (inchart(w, x, y))
                                wsel->row = (double)(y - w->chart_y) /
                                            (double)(w->chart_hour_height +
                                                     w->add_pixels);
                        if (j > 0) {
                                c->view->olddate = c->view->date;
                                c->view->date = j;
...
static int week_xytohour(Week *w, int x, int y) {
        ...
        y -= w->chart_y;
        return (w->begin_hour +
                ((double)y / (double)(w->chart_hour_height + w->add_pixels)));
}
 ...
 */
static void quick_button_cb(Widget widget, XtPointer client, XtPointer call) {
        Calendar *c = calendar;
        Week *w = (Week *)c->view->week_info;
 ...
 */
static void display_hot_btn(Calendar *c, int n, /* which button (0..6) */
 ...
 *  prior to Jan 1, 1970
 */
static void clear_hot_btn(Calendar *c, int n) /* which button (0..6) */
{
        Week *w = (Week *)c->view->week_info;
        Widget btn = w->hot_button[n];
