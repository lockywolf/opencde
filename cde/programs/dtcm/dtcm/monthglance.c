...
#ifndef lint
static char sccsid[] =
    "@(#)monthglance.c 1.82 95/07/27 Copyr 1994 Sun Microsystems, Inc.";
#endif
...
static void paint_day_entries(Tick, int, int, int, int, Paint_cache *,
                              CSA_uint32, XRectangle *);
...
static Boolean print_month(Calendar *, int, void *, Tick, Props *, Boolean);
...
static int compute_pad_width(Cal_Font *pf) {
        int nop, width;
...
static void paint_day_entries(Tick day, int x, int y, int w, int h,
                              Paint_cache *cache, CSA_uint32 appt_total,
                              XRectangle *rect) {
        char *buf;
        ...
        for (loop = 0; loop < 2; loop++) {
                for (i = 0; i < appt_total; i++) {
                        ...
                        if (start_tick >= lower && start_tick < upper &&
                            (loop == cache[i].show_time)) {
                                if (nlines < maxlines) {
                                        ...
                                        if (lines != NULL) {
                                                ...
                                                pad = format_line(
                                                    start_tick, lines->s, buf,
                                                    buf_size, start_tick,
                                                    cache[i].show_time, dt);
                                        } else {
                                                buf = ckalloc(15);
                                                pad = format_line(
                                                    start_tick, (char *)NULL,
                                                    buf, 15, start_tick,
                                                    cache[i].show_time, dt);
                                        }
                                        destroy_lines(lines);
...
extern void month_button(Widget widget, XtPointer data, XtPointer cbs) {
        Calendar *c = calendar;
 ...
 */
static void paint_month(Calendar *c, Tick key, XRectangle *rect) {
        ...
        day = timelocal(&tm);
#endif /* SVR4 */
        m->ndays = ((tm.tm_mon == 1) && leapyr(tm.tm_year + 1900))
                       ? 29
                       : monthdays[tm.tm_mon];
        ...
        for (i = 1; i <= m->ndays; i++) {
                 ...
                 */
                paint_day_entries(day, box_origin_x + 2,
                                  box_origin_y + default_height + btn_ht,
                                  boxw - 4, boxh - btn_ht - 4, c->paint_cache,
 ...
 */
static void display_header(Calendar *c) {
        Month *m = (Month *)c->view->month_info;
        char buf[BUFSIZ];
        XmString str;
        Position x, y;
...
static void paint_daynames(Calendar *c, XRectangle *rect) {
        int i, middle;
        ...
        for (i = 0; i < 7; i++) {
                gr_draw_box(xc, (boxw * i) + margin, c->view->topoffset, boxw,
                            dayname_height, rect);
                middle = gr_center(boxw, days[i], pf);
                gr_text(xc, (boxw * i) + middle + margin,
                        c->view->topoffset + dayname_height - 3, pf, days[i],
                        rect);
        }
}

extern void paint_grid(Calendar *c, XRectangle *rect) {
        int i;
        ...
        gr_draw_box(xc, margin - 1, c->view->topoffset - 1, 7 * boxw + 2,
                    nrows * boxh + 2 + dayname_height, rect);
}

extern void layout_month(Calendar *c, Tick date) {
        Month *m = (Month *)c->view->month_info;
        int i, x, y, firstdom, boxw = calendar->view->boxw,
                               boxh = calendar->view->boxh,
                               margin = calendar->view->outside_margin,
        ...
        for (i = 1; i <= m->ndays; i++) {
                ...
                XtMoveWidget(m->hot_button[i - 1], m->button_loc[i - 1].x,
                             m->button_loc[i - 1].y);
        }
...
extern void prepare_to_paint_monthview(Calendar *c, XRectangle *rect) {
        Dimension w, h;
        c = calendar;
...
/* ADDED FOR PRINTING */
extern void get_time_str(Dtcm_appointment *appt, char *buf, int buf_size) {
        Calendar *c = calendar;
        Props *p = (Props *)c->properties;
        DisplayType dt = get_int_prop(p, CP_DEFAULTDISP);
        struct tm *tm;
...
extern void month_event(XEvent *event) {
        ...
        /* boundary conditions */
        if (x < margin || col > 6 || y < (dayname_height + toffset) ||
            row > c->view->nwks - 1) {
                calendar_deselect(c);
                lastcol = 0;
                lastrow = 0;
...
static int count_month_pages(Calendar *c, Tick start_date, int lines_per_box) {
        int i, j;
        int rows, pages;
        struct tm tm;
...
#endif /* SVR4 */
        ndays = ((tm.tm_mon == 1) && leapyr(tm.tm_year + 1900))
                    ? 29
                    : monthdays[tm.tm_mon];
...
static Boolean print_month(Calendar *c, int num_page, void *xp, Tick first_date,
        ...
        if (num_page == 1)
                total_pages = (lines_per_box > 0)
                                  ? count_month_pages(c, tick, lines_per_box)
                                  : 1;
...
extern void print_month_range(Calendar *c, Tick start_tick, Tick end_tick) {
        Props *p = (Props *)c->properties;
        ...
        for (; n > 0; n--) {
                while (!done) {
                            ...
                            print_month(c, num_page, xp, first_date, p, first);
                        num_page++;
                        first = False;
                }
...
static int count_month_appts(CSA_entry_handle *list, int a_total, Calendar *c) {
        ...
        appt = allocate_appt_struct(appt_read, c->general->version,
                                    CSA_ENTRY_ATTR_CLASSIFICATION_I, NULL);
        for (i = 0; i < a_total; i++) {
 ...
 */
extern void layout_children(Calendar *c) {
        Month *m = (Month *)c->view->month_info;
...
static void manage_children(Calendar *c) {
        int i;
        Month *m = (Month *)c->view->month_info;
...
static void unmanage_children(Calendar *c) {
        int i;
        Month *m = (Month *)c->view->month_info;
 ...
 */
extern void cleanup_after_monthview(Calendar *c) {
 ...
 */
static Boolean allocated(Calendar *c) { return (c->view->month_info != NULL); }
...
allocator(Calendar *c) {
        ...
        for (n = 0; n < 31; n++) {
                ...
                m->hot_button[n] = XmCreatePushButton(c->canvas, buf, NULL, 0);
