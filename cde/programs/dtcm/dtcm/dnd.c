...
static Boolean validate_dropped_appt(char *filename, Calendar *c) {
...
static void handle_animate_cb(Widget dragContext, XtPointer client_data,
                              XtPointer call_data) {
                ...
                case DtDND_FILENAME_TRANSFER:
                        ...
                        if (cmfns_use_fns(c->properties) &&
                            cmfns_name_from_file(data, buf, sizeof(buf)) == 1) {
                                /*
                                 * Looks like an HFS file has been dropped on
                                 * us. Get the calendar service associated with
                                 * the FNS name and browse it
                                 */
...
void cm_register_drop_site(Calendar *c, Widget w)

{
        ...
        /*
         * The above string "CalendarAppointment" is hard coded to match the
         * type used in dtdnddemo.  In the future we need to use the true type
         * from the data typing database
         */
        ...
        DtDndVaDropRegister(w, DtDND_FILENAME_TRANSFER | DtDND_BUFFER_TRANSFER,
                            XmDROP_COPY, transfer_cb_rec,
                            DtNdropAnimateCallback, animateCBRec,
                            DtNtextIsBuffer, True, NULL);
        return;
}
 ...
 *		-1	Failure
 */
static int schedule_appt(Calendar *c, Dtcm_appointment *a) {
        ...
        if (strcmp(c->calname, c->view->current_calendar) != 0) {
                ...
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 212, "Calendar : Schedule Appointment"));
                ...
                answer = dialog_popup(c->frame, DIALOG_TITLE, title,
        ...
        if (a->end_time) {
                ...
                if ((rc = editor_insert(a, &entry, c)) == True) {
                        ...
                        _csa_iso8601_to_tick(a->time->value->item.string_value,
                                             &tick);
                        format_tick(tick, ot, st, date_buf, MAXNAMELEN);
                        snprintf(buf, BUFSIZ,
                                 catgets(c->DT_catd, 1, 214,
 ...
 */
extern int drag_load_proc(char *filename, Calendar *c) {
        int ret_val, i = 1;
        ...
        case INVALID_START:
                snprintf(buf, buf_size, "%s",
                         catgets(c->DT_catd, 1, 219,
                                 "Invalid START time specified"));
                break;
        case INVALID_STOP:
                snprintf(
                    buf, buf_size, "%s",
                    catgets(c->DT_catd, 1, 220, "Invalid END time specified"));
                break;
        case MISSING_DATE:
                snprintf(
                    buf, buf_size, "%s",
                    catgets(c->DT_catd, 1, 221, "Empty or missing DATE field"));
                break;
        case MISSING_START:
                snprintf(buf, buf_size, "%s",
                         catgets(c->DT_catd, 1, 222,
                                 "Empty or missing START field"));
                break;
        case MISSING_WHAT:
                snprintf(
                    buf, buf_size, "%s",
                    catgets(c->DT_catd, 1, 223, "Empty or missing WHAT field"));
                break;
        case REPEAT_FOR_MISMATCH:
                snprintf(buf, buf_size, "%s",
                         catgets(c->DT_catd, 1, 224,
        ...
        if (op != VALID_APPT) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 1073, "Calendar : Error - Drag and Drop"));
                char *ident =
                ...
                dialog_popup(c->frame, DIALOG_TITLE, title, DIALOG_TEXT, buf,
                             ...
                             DND_ERROR_HELP, DIALOG_IMAGE, pu->xm_error_pixmap,
                             NULL);
                XtFree(ident);
                XtFree(title);
                return -1;
...
CSA_entry_handle get_appt_struct(DragContext *context) {
        ...
        if (!XmListGetSelectedPos(list, &item_list, &item_cnt)) {
                char *title = XtNewString(catgets(
                    c->DT_catd, 1, 230, "Calendar : Error - Drag Appointment"));
                char *text = XtNewString(
                    catgets(c->DT_catd, 1, 231,
                            "Select an appointment and DRAG again."));
                char *ident =
                    XtNewString(catgets(c->DT_catd, 1, 95, "Continue"));
                answer = dialog_popup(c->frame, DIALOG_TITLE, title,
                                      DIALOG_TEXT, text, BUTTON_IDENT, 1, ident,
                                      BUTTON_HELP, RESELECT_ERROR_HELP,
                                      DIALOG_IMAGE, pr->xm_error_pixmap, NULL);
                XtFree(ident);
                XtFree(text);
                XtFree(title);
                return (0);
        ...
        if (context->editor_type == SingleEditorList) {
                ...
                entry = t_nth_appt((ToDo *)context->editor, item_list[0] - 1);
                entry_set = !!entry;
        } else
                entry_set = 0;
 ...
 * icon type and name.
 */
static void GetIcon(Calendar *calendar) {
        ...
        if (e->drag_bitmap == 0) {
                e->drag_bitmap = XCreateBitmapFromData(
        ...
        if (e->drag_mask == 0) {
                e->drag_mask = XCreateBitmapFromData(
...
void TranslationDragStart(Widget widget, XEvent *event, String *parms,
                          Cardinal *num_params) {
        ...
        if (((entry = get_appt_struct(context)) == (CSA_entry_handle)NULL) ||
            ((apptstr = parse_appt_to_string(calendar->cal_handle, entry, p,
...
void ApptDragStart(Widget widget, XEvent *event, Calendar *calendar,
        ...
        if (editor_type == SingleEditorIcon) {
                ...
                if (!dssw_form_flags_to_appt(&e->dssw, appt, calendar->calname,
                                             now(), &e->dsswFlags) ||
                    !rfp_form_flags_to_appt(&e->rfp, appt, calendar->calname,
                                            &e->rfpFlags) ||
                    (preDsswFlags != e->dsswFlags) ||
                    (preRfpFlags != e->rfpFlags)) {
                        e->doing_drag = False;
                ...
                if (!dssw_form_flags_to_appt(&ge->dssw, appt, calendar->calname,
                                             now(), &ge->dsswFlags) ||
                    !rfp_form_flags_to_appt(&ge->rfp, appt, calendar->calname,
                                            &ge->rfpFlags) ||
                    (preDsswFlags != ge->dsswFlags) ||
                    (preRfpFlags != ge->rfpFlags)) {
                        ge->doing_drag = False;
                ...
                if (!rfp_form_flags_to_appt(&t->rfp, appt, calendar->calname,
                                            &t->rfpFlags) ||
                    (preRfpFlags != t->rfpFlags)) {
                        t->doing_drag = False;
                        free_appt_struct(&appt);
                ...
                appt->state->value->item.sint32_value =
                    (t->completed_val) ? CSA_STATUS_COMPLETED
        ...
        apptstr =
            parse_attrs_to_string(appt, (Props *)calendar->properties,
...
void DtcmProcessPress(Widget w, XEvent *event, String *params,
                      Cardinal *num_params) {
        ...
        if (cur_item > 0) {
                XtVaGetValues(w, XmNselectedPositions, &selected_positions,
                ...
                for (i = 0; i < nselected_positions; i++) {
                        if (cur_item == selected_positions[i]) {
                                /*
                                 * The determination of whether this is a
                                 * transfer drag cannot be made until a Motion
                                 * event comes in.  It is not a drag as soon as
                                 * a ButtonUp event happens.
                                 */
                                XEvent new_event;

                                XPeekIfEvent(XtDisplay(w), &new_event,
                                             lookForButton, (XPointer)event);
