...
#ifndef lint
static char sccsid[] =
    "@(#)cmtt.c 1.13 95/05/01 Copyr 1993 Sun Microsystems, Inc.";
#endif
#include <EUSCompat.h>
#include <sys/stat.h>
#include <unistd.h>
...
static void cm_iconic(Widget win, int iconic)

{
        if (iconic) {
                XIconifyWindow(XtDisplay(win), XtWindow(win),
...
void DieFromToolTalkError(Calendar *c, char *errfmt, Tt_status status) {
        ...
        xms_title = XmStringCreateLocalized(
            catgets(c->DT_catd, 2, 1, "Calendar - Warning"));
 ...
 */
Tt_status cmtt_init(char *toolname, Calendar *c, XtAppContext context,
                    Widget shell)
...
#ifdef FNS_DEMO
static Tt_message contract_cb(Tt_message msg, Tttk_op op, Widget shell,
                              void *clientdata, Tt_message contract) {

        DP(("contract_cb(msg=%d, op=%d, shell=%d, data=%d, cntrct=%d)\n", msg,
            op, shell, clientdata, contract));
...
docname=%s, data=%d)\n",
        ...
        contract_pats =
            ttdt_message_accept(msg, contract_cb, clientdata, c->frame, 1, 1);
