...
static XtGeometryResult QueryProc(Widget, XtWidgetGeometry *,
                                  XtWidgetGeometry *);
...
static XtResource resources[] = {
    {XmNyear, XmCYear, XmRInt, sizeof(int),
     XtOffsetOf(struct _XmMonthPanelRec, month_panel.year), XmRImmediate,
     (XtPointer)NULL},
    {XmNmonth, XmCMonth, XmRInt, sizeof(int),
     XtOffsetOf(struct _XmMonthPanelRec, month_panel.month), XmRImmediate,
     (XtPointer)NULL},
    {XmNactivateCallback, XmCCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffsetOf(struct _XmMonthPanelRec, month_panel.callback), XmRImmediate,
     (XtPointer)NULL},
    {XmNtitleFormat, XmCTitleFormat, XmRString, sizeof(char *),
     XtOffsetOf(struct _XmMonthPanelRec, month_panel.title_format), XtRString,
     (XtPointer) "%B %Y"},
    ...
    {XmNactiveDays, XmCActiveDays, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _XmMonthPanelRec, month_panel.active_days), XtRImmediate,
     (XtPointer)False},
...
externaldef(xmmonthpanelclassrec) XmMonthPanelClassRec xmMonthPanelClassRec = {
    {
        /* core_class fields      */
    ...
    },
    {
        ...
        /* delete_child	  */ XtInheritDeleteChild, /* Inherit from
                                                          superclass */
        /* Extension          */ NULL,
    },
...
externaldef(xmmonthpanelwidgetclass)
    WidgetClass xmMonthPanelWidgetClass = (WidgetClass)&xmMonthPanelClassRec;
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args) {
        ...
        /*
         * create and cache panel header ( eg. "January") (depends on format
         * resource)
         *
         * Whether XmLabels or XmPushButton gadgets are used is selectable via
         * the boolean "activeDays" and "activeHeader" resource. True gets you
         * buttons.
         */
        ...
        if (new_w->month_panel.active_header) {
                ...
                new_w->month_panel.separator = XmCreateSeparatorGadget(
                    (Widget)new_w, "separator", NULL, 0);
        ...
        */
         ...
         */
        new_w->month_panel.day_labels = (Widget *)XtMalloc(7 * sizeof(Widget));
        for (i = 0; i < 7; i++) {
                XmString str;
                Widget label;
                ...
                label =
                    XmCreateLabelGadget((Widget)new_w, col_hdr[i], wargs, 1);
                /* Managing of column headers is switchable */
                if (new_w->month_panel.show_column_headers)
                        XtManageChild(label);
                new_w->month_panel.day_labels[i] =
                    label; /* cache in panel inst. */
                XmStringFree(str);
        }
        ...
        for (i = 0; i < 31; i++) {
                ...
                if (new_w->month_panel.active_days) {
                        btn = XmCreatePushButtonGadget((Widget)new_w, buf,
                                                       wargs, 1);
                ...
                new_w->month_panel.days[i] =
                    btn; /* cache id in panel instance */
                XmStringFree(str);
        }
 ...
 *                                                                      *
 ************************************************************************/
static void DoLayout(XmMonthPanelWidget mw) {
        ...
        if (mpp->separator != NULL) {
                if (tall_header)
                        XtMoveWidget(mpp->separator, (Position)w / 8,
                                     (Position)2 * row_h);
                else
                        XtMoveWidget(mpp->separator, (Position)w / 8,
                                     (Position)row_h);
         ...
         */
        x = (Position)0;
        y = (Position)(mw->month_panel.display_rows - (tall_header ? 6 : 7)) *
            row_h;
        ...
        y = (Position)(mw->month_panel.display_rows - (tall_header ? 5 : 6)) *
            row_h;
...
static void scale_components(XmMonthPanelWidget mw) {
         ...
         ******************************************************************/
        XtVaGetValues(XtParent(mw), XmNforeground, &fg, XmNbackground, &bg,
                      NULL);
        ...
        if (tall_header)
                XtResizeWidget(mpp->header, w, 2 * row_h,
                               mpp->header->core.border_width);
        else
                XtResizeWidget(mpp->header, w, row_h,
                               mpp->header->core.border_width);

        if (clipped(mpp->header)) {
                XtVaSetValues(mpp->header, XmNlabelType, XmPIXMAP,
                              XmNlabelPixmap, head_pm, NULL);
        } else
                XtVaSetValues(mpp->header, XmNlabelType, XmSTRING, NULL);
        ...
        if (clipped(this_widget)) {
                if (ltype == XmSTRING) {
                        for (i = 0; i < 31; i++) {
                                XtVaSetValues((mpp->days)[i], XmNlabelType,
                                              XmPIXMAP, XmNlabelPixmap, btn_pm,
                                              NULL);
                        }
                }
        } else {
 ...
 */
static Boolean clipped(Widget w) {
        XmString str;
        XmFontList fl;
        ...
        XtVaGetValues(w, XmNlabelString, &str, XmNfontList, &fl,
                      XmNhighlightThickness, &hlt_w, XmNshadowThickness, &shd_w,
                      XmNwidth, &act_w, XmNheight, &act_h, NULL);
 ...
 *									   *
 ***************************************************************************/
static XtGeometryResult QueryProc(Widget w, XtWidgetGeometry *request,
                                  XtWidgetGeometry *reply) {
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
        ...
        if (request->month_panel.show_panel_header !=
            current->month_panel.show_panel_header) {
                if (request->month_panel.show_panel_header) {
        ...
        if (request->month_panel.show_column_headers !=
            current->month_panel.show_column_headers) {
                if (request->month_panel.show_column_headers) {
                        int col = 0;
                        while (col < 7) {
                                XtManageChild(
                                    request->month_panel.day_labels[col]);
                                col++;
                        }
                        (request->month_panel.display_rows)++;
                } else {
        ...
        if (request->month_panel.show_separator !=
            current->month_panel.show_separator) {
                if (request->month_panel.show_separator) {
 ...
 */
static void set_header_string(XmMonthPanelWidget mw) {
        XmString str;
        struct tm *tm_ret;
        Tick tmptick;
        Arg wargs[3];
        ...
        tmptick = monthdayyear(mw->month_panel.month, 1, mw->month_panel.year);
 ...
 *									*
 ************************************************************************/
Widget XmCreateMonthPanel(Widget parent, char *name, ArgList args,
                          Cardinal argCount) {

        return (XtCreateWidget(name, xmMonthPanelWidgetClass, parent, args,
                               argCount));
}
