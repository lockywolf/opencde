...
#ifndef lint
static char sccsid[] =
    "@(#)graphics.c 1.23 94/11/29 Copyr 1994 Sun Microsystems, Inc.";
#endif
...
static unsigned char black_data[] = {0xFF, 0xFF, 0xFF, 0xFF,
                                     0xFF, 0xFF, 0xFF, 0xFF};
static unsigned char gray_data_75[] = {0xDD, 0xBB, 0xEE, 0xF7,
                                       0xDD, 0xBB, 0xEE, 0xF7};

static unsigned char gray_data_50[] = {0xAA, 0x55, 0xAA, 0x55,
                                       0xAA, 0x55, 0xAA, 0x55};

static unsigned char gray_data_25[] = {0x88, 0x22, 0x44, 0x11,
                                       0x88, 0x22, 0x44, 0x11};
...
static unsigned char *dash_list[] = {
    solid,        short_dotted, dotted,     dot_dashed,
    short_dashed, long_dashed,  odd_dashed,
};

extern void gr_clear_area(new_XContext *xc, int x, int y, Dimension w,
                          Dimension h) {
...
extern void gr_clear_box(new_XContext *xc, int x, int y, int w, int h) {
...
extern void gr_make_grayshade(new_XContext *xc, int x, int y, int w, int h,
                              int shade) {

        XColor colorcell_del, rgb_db_ref;

        XSetForeground((Display *)xc->display, (GC)xc->gc,
                       (unsigned long)xc->colorcell_del[shade].pixel);

        XSetFillStyle(xc->display, xc->gc, FillSolid);
        XFillRectangle(xc->display, xc->xid, xc->gc, x, y, w, h);
}

extern void gr_make_gray(new_XContext *xc, int x, int y, int w, int h,
                         int percent) {
...
extern void gr_make_rgbcolor(new_XContext *xc, Colormap cms, int x, int y,
...
extern int gr_nchars(int area, char *str, Cal_Font *font) {
...
void gr_draw_rgb_box(new_XContext *xc, int x, int y, int w, int h,
                     int thickness, int shade, Colormap cms)
...
void gr_dissolve_box(new_XContext *xc, int x, int y, int w, int h,
                     int thickness) {
        for (; thickness > 0; thickness--) {
...
extern int gr_center(int area, char *str, Cal_Font *font) {
...
extern void gr_text(new_XContext *xc, int x, int y, Cal_Font *font, char *str,
                    XRectangle *rect) {
        ...
        CalDrawString(xc->display, xc->xid, font, xc->draw_gc, x, y, str,
                      cm_strlen(str));
}

extern void gr_text_rgb(new_XContext *xc, int x, int y, Cal_Font *font,
                        char *str, Pixel shade, Colormap cms,
                        XRectangle *rect) {
        ...
        CalDrawString(xc->display, xc->xid, font, xc->gc, x, y, str,
                      cm_strlen(str));
}

extern void gr_draw_line(new_XContext *xc, int x1, int y1, int x2, int y2,
        ...
        if (style == gr_solid) {
                ...
                XSetDashes(xc->display, xc->gc, dash_offset,
                           (char *)dash_list[style], short_dotted_list_length);
                XSetLineAttributes(xc->display, xc->gc, 0, LineOnOffDash,
                                   CapNotLast, JoinMiter);
                XDrawLine(xc->display, xc->xid, xc->gc, x1, y1, x2, y2);
        }
}

extern void gr_draw_box(new_XContext *xc, int x, int y, int w, int h,
                        XRectangle *rect) {
...
extern void gr_draw_glyph(new_XContext *src_xc, new_XContext *dst_xc,
                          Pixmap pixmap, int x, int y, int w, int h) {
...
static void saturate(XColor *xclr)
 ...
 */
new_XContext *gr_create_xcontext(Calendar *c, Widget widget, GR_depth depth,
                                 XtAppContext app) {
        ...
        if (depth == gr_mono) {
                ...
                XtVaGetValues(widget, XmNbackground, &xc->background,
                              XmNforeground, &xc->foreground, NULL);
                /*
        ...
        xc->gc =
            XCreateGC(xc->display, xc->xid,
                      GCForeground | GCBackground | GCFillStyle, xc->gcvals);
        ...
        xc->draw_gc = XCreateGC(xc->display, xc->xid, GCForeground | GCFunction,
                                &gc_vals);
        XSetLineAttributes(xc->display, xc->draw_gc, 0, LineSolid, CapNotLast,
                           JoinMiter);
...
extern void gr_set_clip_rectangles(new_XContext *xc, int x, int y,
                                   XRectangle *rectangles, int n,
                                   int ordering) {

        XSetClipRectangles(xc->display, xc->gc, x, y, rectangles, n, ordering);
        XSetClipRectangles(xc->display, xc->draw_gc, x, y, rectangles, n,
                           ordering);
        XSetClipRectangles(xc->display, xc->clear_gc, x, y, rectangles, n,
                           ordering);
        XSetClipRectangles(xc->display, xc->invert_gc, x, y, rectangles, n,
                           ordering);
        return;
}

extern void gr_clear_clip_rectangles(new_XContext *xc) {
...
extern Boolean gr_create_stipple(new_XContext *xc, unsigned char *data,
                                 int datawidth, int dataheight, Pixmap *stipple,
                                 unsigned int *width, unsigned int *height) {

        Boolean ok = True;

        if ((*stipple = XCreateBitmapFromData(
                 xc->display, xc->xid,
...
extern Boolean gr_init(new_XContext *xc, Widget canvas) {
        ...
        ok = gr_create_stipple(xc, black_data, black_data_width,
                               black_data_height, &black_data_pixmap, &width,
                               &height);
        ok = gr_create_stipple(xc, gray_data_75, gray_data_75_width,
                               gray_data_75_height, &gray_data_75_pixmap,
                               &width, &height);
        ok = gr_create_stipple(xc, gray_data_50, gray_data_50_width,
                               gray_data_50_height, &gray_data_50_pixmap,
                               &width, &height);
        ok = gr_create_stipple(xc, gray_data_25, gray_data_25_width,
                               gray_data_25_height, &gray_data_25_pixmap,
                               &width, &height);

        ok = XAllocNamedColor(dpy, cmap, "dark slate grey",
                              &xc->colorcell_del[DARKGREY], &rgb_db_ref);
        ok = XAllocNamedColor(dpy, cmap, "dim grey",
                              &xc->colorcell_del[DIMGREY], &rgb_db_ref);
        ok = XAllocNamedColor(dpy, cmap, "grey", &xc->colorcell_del[GREY],
                              &rgb_db_ref);
        ok = XAllocNamedColor(dpy, cmap, "light grey",
                              &xc->colorcell_del[LIGHTGREY], &rgb_db_ref);
        ok = XAllocNamedColor(dpy, cmap, "red", &xc->colorcell_del[RED],
                              &rgb_db_ref);
