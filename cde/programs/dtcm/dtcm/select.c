...
#ifndef lint
static char sccsid[] =
    "@(#)select.c 1.24 95/02/21 Copyr 1994 Sun Microsystems, Inc.";
#endif
...
extern int selection_active(Selection *s) { return (s->active); }

extern void deactivate_selection(Selection *s) { s->active = 0; }

extern void activate_selection(Selection *s) { s->active = 1; }

extern void weekchart_deselect(Calendar *c) {
        ...
        gr_dissolve_box(c->xcontext, chart_x, chart_y - 6, w->chart_day_width,
                        char_height + 6, 2);
        ...
        for (i = index; i < (index + BOX_SEG); i++) {
                ...
                if (w->time_array[i] == 1) {
                        ...
                        if ((c->xcontext->screen_depth < 8) || FAKE_MONOCHROME)
                                gr_make_gray(c->xcontext, x, y, wi + 1, h, 50);
                        else
                                gr_make_rgbcolor(c->xcontext, cms, x, y, wi + 1,
                                                 h, MIDGREY, MIDGREY, MIDGREY);
...
extern void weekchart_select(Calendar *c) {
        ...
        if (j >= 0) {
                ...
                if (c->xcontext->screen_depth < 8) {
                        gr_draw_box(c->xcontext,
                                    (i * w->chart_day_width) + w->chart_x + 1,
                                    (j * w->chart_hour_height) +
                                        (j * w->add_pixels) + w->chart_y + 1,
                                    w->chart_day_width - 2,
                                    w->chart_hour_height - 2 + w->add_pixels,
                                    NULL);
                        gr_draw_box(c->xcontext, chart_x, chart_y,
                                    w->chart_day_width, char_height + 1, NULL);
                } else {
                        /* Select time slot */
                        gr_draw_rgb_box(c->xcontext,
                                        (i * w->chart_day_width) + w->chart_x,
                                        (j * w->chart_hour_height) +
                                            (j * w->add_pixels) + w->chart_y,
                                        w->chart_day_width,
                                        w->chart_hour_height + w->add_pixels, 1,
...
static void select_weekday(Calendar *c, Boolean select) {
        int n, x, y;
...
extern void calendar_select(Calendar *c, Selection_unit unit, caddr_t ref) {
        int i, j;
        ...
        */
        ...
        case daySelect:
                ...
                if (j >= 0) {
                        ...
                        if (c->xcontext->screen_depth < 8)
                                ...
                                gr_draw_rgb_box(
                                    xc, (i * boxw) + margin,
                                    (sel->row * boxh) + topoff + dayname_height,
                                    boxw, boxh, 1, foreground_pixel, cms);
                        }
                        activate_selection(sel);
                }
                break;
        ...
        case monthSelect:
                ...
                if (c->xcontext->screen_depth < 8)
                        ...
                        /*
                                                        gr_draw_rgb_box(xc,
                           xpos, j*boxh+topoff+10, boxw-2, boxh-2, 1,
                                                                foreground_pixel,
                           cms);
                        */
                        gr_draw_rgb_box(xc, xpos, j * boxh + topoff + 10,
                                        boxw - 56, boxh + 2, 1,
                                        foreground_pixel, cms);
                }
                sel->row = j;
        ...
        case hourSelect:
                ...
                if (c->xcontext->screen_depth < 8)
                        ...
                        gr_draw_rgb_box(xc, MOBOX_AREA_WIDTH + 3,
                                        j * boxh + topoff, c->view->boxw - 2,
                                        c->view->boxh, 1, foreground_pixel,
                                        cms);
                }
                sel->row = j;
        ...
        if (selection_active(s)) {
                ...
                case dayGlance: {
                        ...
                        for (i = 0; i < nunits; i++) {
                                gr_dissolve_box(xc, MOBOX_AREA_WIDTH + 3,
                                                k * boxh + topoff + 1, boxw - 3,
                                                boxh - 2, 3);
                                x = MOBOX_AREA_WIDTH + 2;
                                y = k * boxh + topoff;

                                gr_draw_line(xc, x, y, x + boxw, y, gr_solid,
                                             NULL);
                                y += boxh;
                                gr_draw_line(xc, x, y, x + boxw, y, gr_solid,
                                             NULL);
                        ...
                        gr_draw_line(xc, MOBOX_AREA_WIDTH + 2 + hrbox_margin,
                                     c->view->topoffset,
                                     MOBOX_AREA_WIDTH + 2 + hrbox_margin,
                                     c->view->topoffset +
                                         num_hrs * c->view->boxh,
                                     gr_solid, NULL);
                        break;
                ...
                case yearGlance:
                        for (i = 0; i < nunits; i++) {
                                ...
                                /*
                                                                gr_dissolve_box(xc,
                                   (j*boxw)+margin+10, k*boxh+topoff-5, boxw-2,
                                   boxh-2, 1);
                                */
                                ...
                                /*
                                                                gr_dissolve_box(xc,
                                   (j*boxw)+margin+11, k*boxh+topoff-4, boxw-2,
                                   boxh-2, 1);
                                */
                                j++;
                        }
...
extern void monthbox_deselect(Calendar *c) {
        char buf[3];
        ...
        if (c->xcontext->screen_depth < 8)
                gr_make_gray(c->xcontext, x, y, day_info->col_w + 1,
                             day_info->row_h - 1, 25);
        else
                gr_make_grayshade(c->xcontext, x, y, day_info->col_w + 1,
                                  day_info->row_h - 1, LIGHTGREY);
        buf[0] = '\0';
        snprintf(buf, 3, "%d", day_info->day_selected);
        gr_text(c->xcontext, day_info->day_selected_x2 + 2,
                day_info->day_selected_y2, c->fonts->viewfont, buf, NULL);
}
extern void monthbox_select(Calendar *c) {
        char buf[3];
        Day *day_info = (Day *)c->view->day_info;
        int x, y;
        ...
        gr_clear_box(c->xcontext, x, y, day_info->col_w, day_info->row_h - 3);
        gr_draw_box(c->xcontext, x, y, day_info->col_w, day_info->row_h - 3,
                    NULL);
        buf[0] = '\0';
        snprintf(buf, 3, "%d", day_info->day_selected);
        gr_text(c->xcontext, day_info->day_selected_x2 + 2,
...
extern void browser_deselect(Calendar *c, Browser *b) {
        ...
        for (i = index; i < (index + BOX_SEG); i++) {
                if (b->multi_array[i] == 1) {
                        if ((c->xcontext->screen_depth < 8) || FAKE_MONOCHROME)
                                gr_make_gray(b->xcontext, x, y, w + 1, h, 25);
                        else
                                gr_make_grayshade(b->xcontext, x, y, w + 1, h,
                                                  LIGHTGREY);
                } else if (b->multi_array[i] == 2) {
                        if ((c->xcontext->screen_depth < 8) || FAKE_MONOCHROME)
                                gr_make_gray(b->xcontext, x, y, w + 1, h, 50);
                        else
                                gr_make_rgbcolor(b->xcontext, cms, x, y, w + 1,
                                                 h, MIDGREY, MIDGREY, MIDGREY);
                } else if (b->multi_array[i] >= 3) {
                        if ((c->xcontext->screen_depth < 8) || FAKE_MONOCHROME)
                                gr_make_gray(b->xcontext, x, y, w + 1, h, 75);
                        else
                                gr_make_grayshade(b->xcontext, x, y, w + 1, h,
                                                  DIMGREY);
                }
...
extern void browser_select(Calendar *c, Browser *b, pr_pos *xy) {
        int i, j;
        Selection *sel = (Selection *)b->current_selection;
        Colormap cms;
        Pixel pixel;
        ...
        if (j >= 0) {
                if (c->xcontext->screen_depth < 8) {
                        gr_draw_box(b->xcontext, (i * b->boxw) + b->chart_x + 2,
                                    (j * b->boxh) + b->chart_y + 2, b->boxw - 4,
                                    b->boxh - 4, NULL);
                } else {
                        pixel = c->xcontext->hilight_color;
                        XtVaGetValues(b->canvas, XmNcolormap, &cms, NULL);
                        gr_draw_rgb_box(
                            b->xcontext, (i * b->boxw) + b->chart_x + 2,
                            (j * b->boxh) + b->chart_y + 2, b->boxw - 4,
                            b->boxh - 4, 1, pixel, cms);
                        gr_draw_rgb_box(
                            b->xcontext, (i * b->boxw) + b->chart_x + 1,
                            (j * b->boxh) + b->chart_y + 1, b->boxw - 2,
                            b->boxh - 2, 1, pixel, cms);
                }
                sel->row = j;
                sel->col = i;
                sel->nunits = 1;
...
static void select_weekhotbox(Calendar *c) {
        ...
        if (c->xcontext->screen_depth < 8)
                gr_draw_box(xc, x, y, w->day_width - 4, w->label_height - 4,
                            NULL);
        else {
                gr_draw_rgb_box(xc, x, y, w->day_width - 4, w->label_height - 4,
                                1, background_pixel, cms);
                gr_draw_rgb_box(xc, x - 1, y - 1, w->day_width - 2,
                                w->label_height - 2, 1, background_pixel, cms);
        }
}

extern void paint_selection(Calendar *c) {
        Props *p = (Props *)c->properties;
        Selection *sel;
        ...
        case weekGlance:
                ...
                sel->col = (d = dow(c->view->date)) == 0 ? 6 : --d;
