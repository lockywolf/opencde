...
#ifndef lint
static char sccsid[] =
    "@(#)cm_delete.c 1.29 95/03/17 Copyr 1993 Sun Microsystems, Inc.";
#endif
...
static void cm_args(int argc, char **argv) {
        ...
        while (++argv && *argv) {
                ...
                default:
                        fprintf(stderr, "%s",
                                catgets(DT_catd, 1, 187,
                                        "Usage:\n\tdtcm_delete [-c calendar] "
                                        "[-d <mm/dd/yy>] [-v view]\n"));
                        exit(1);
                }
        }
...
int main(int argc, char **argv) {
        int cnt;
        char index[10], *target = NULL, *date = NULL, *view = NULL, *uname,
                        *loc;
        Props *p = NULL;
        ...
        while (!cm_index) {
                ...
                fprintf(stdout, "%s",
                        catgets(DT_catd, 1, 190, "Item to delete (number)? "));
                fgets(index, 9, stdin);
                fprintf(stdout, "\n\n");
                if (*index < '0' || *index > '9') {
                        csa_free(list);
                ...
                cm_index = atoi(index);
                if (cnt >= cm_index)
                        cm_tty_delete(DT_catd, c_handle, version, cm_index - 1,
                                      list);
                memset(index, '\0', 10);
                csa_free(list);
                cm_index = 0;
