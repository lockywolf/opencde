...
static char **arpaPhrase(const char *name) {
        register char c;
...
static char *formatMessage(char **addrs, const char *subject,
                           const char *body) {
        char *msg;
        int size = 0;
        int line;
        ...
        if ((_DtLcxOpenAllDbs(&db) == 0) &&
            (_DtXlateGetXlateEnv(db, plat, &execver, &compver) != 0)) {
                ...
                isAllASCII =
                    CvtStr((char *)NULL, (void *)body, (unsigned long)body_len,
                           (void **)&NewBuf, &_len, CURRENT_TO_INTERNET);

                enc = getEncodingType(body, body_len, FALSE);
                /*
                 * Here is an ugly adjustment again. If mime_type is text/plain
                 * and if ret_codeset is ISO-2022-JP/KR/TW/CN, we have to always
                 * use enc = MIME_7BIT This means if the user inputs UDC/VDC
                 * into the e-mail body, fold7 may convert it to the string with
                 * MSB-on character and dtmail passes it to sendmail as if I had
                 * all 7bit chars.
                 */
                ...
                writeContentHeaders(hdr_buf, mime_type, enc, (char *)digest,
                                    isAllASCII);
                strlcat(hdr_buf, "\n", 1024);
...
static int deliver(char **addrs, char *msg) {
        char **argv;
        int fd[2];
        int c_pid;
        int status;
...
int submit_mail(const char *to, const char *subject, const char *body) {
        char **addrs;
        char **ad;
        char *msg;
        int status;
