 ...
 **************************************************************************/
void CMSaveSessionCB(Widget w, XtPointer clientData, XtPointer callbackArg) {
        char *path, *command, *name;
        FILE *fp;
        int n, actualFormat, command_len, save_session = True;
        Position x, y;
        ...
        if (save_session) {
                ...
                wmStateAtom =
                    XmInternAtom(XtDisplay(calendar->frame), "WM_STATE", False);

                XGetWindowProperty(XtDisplay(calendar->frame),
                                   XtWindow(calendar->frame), wmStateAtom, 0L,
                                   (long)BUFSIZ, False, wmStateAtom,
...
void GetSessionInfo(Calendar *c) {
        XrmDatabase db;
        char *path;
        XrmName xrm_name[5];
        ...
        if (DtSessionRestorePath(c->frame, &path, c->app_data->session_file) ==
            False) {
                /* XXX: Should generate error about not being able to restore
                 * session.*/
                return;
        }
        ...
        if (XrmQGetResource(c->view->sessiondb, xrm_name, xrm_name, &rep_type,
                            &value)) {
                if (value.addr)
