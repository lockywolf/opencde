 ...
 * not be freed.
 */
static XtPointer get_font(XmFontList font_list, XmFontType *type_return) {
        XmFontContext fl_context;
        XmFontListEntry fl_entry;
        XtPointer fl_entry_font, font_to_use;
        char *fl_entry_font_tag;
        Boolean found_font_set = False, found_font_struct = False;
...
extern Boolean fontlist_to_font(XmFontList font_list, Cal_Font *cal_font) {
 ...
 */
static Boolean ProbeForFont(Calendar *cal, Font_Weight weight,
                            unsigned long target_pixel_size, int with_foundry,
                            char *font_name, int font_name_size) {
        char **font_names, notused[48];
        int i, nnames, size = 0, closest_size_diff = 100000, closest_size,
                       closest_index = -1;

        if (!strcmp(cal->app_data->app_font_family, "application")) {
                snprintf(font_name, font_name_size,
                         "-%s-%s-%s-r-normal-sans-*-*-*-*-p-*",
                         (with_foundry) ? "dt" : "*",
        ...
        for (i = 0; i < nnames; i++) {
                sscanf(font_names[i],
                       "-%[^-]-%[^-]-%[^-]-%[^-]-%[^-]-%[^-]-%d-%s", notused,
                       notused, notused, notused, notused, notused, &size,
                       notused);

                if (!size)
                        sscanf(font_names[i],
                               "-%[^-]-%[^-]-%[^-]-%[^-]-%[^-]--%d-%s", notused,
                               notused, notused, notused, notused, &size,
                               notused);
        ...
        snprintf(
            font_name, font_name_size, "-%s-%s-%s-r-normal-%s-%d-*-*-*-*-*",
            (with_foundry) ? "dt" : "*", cal->app_data->app_font_family,
            (weight == BOLD) ? "bold" : "medium",
            (!strcmp(cal->app_data->app_font_family, "application")) ? "sans"
                                                                     : "",
            closest_size);
...
extern void load_app_font(Calendar *cal, Font_Weight weight, Cal_Font *userfont,
                          Cal_Font *return_font) {
        unsigned long pixel_size;
        Display *dpy = XtDisplay(cal->frame);
        char font_name[128], *font_name_ptr = font_name, **font_names;
        int nnames;
        XrmValue in_font;
        ...
        if (userfont->cf_type == XmFONT_IS_FONT) {
                ...
                if (!(list_size = XFontsOfFontSet(userfont->f.cf_fontset,
                        ...
                        int i;
                        if (!XGetFontProperty(font_struct_list[0], pixel_atom,
                                              &pixel_size))
                                pixel_size = 12;
                }
        }
         ...
         */
        if (!strcmp(cal->app_data->app_font_family, "application")) {
                snprintf(font_name, 128,
                         "-dt-%s-%s-r-normal-sans-%lu-*-*-*-p-*",
                         cal->app_data->app_font_family,
                         (weight == BOLD) ? "bold" : "medium", pixel_size);
        } else {
        ...
        if (!nnames) {
                if (!ProbeForFont(cal, weight, pixel_size, TRUE, font_name_ptr,
                                  128)) {
 ...
 */
void CalTextExtents(Cal_Font *font, char *string, int nbytes, int *x_return,
        ...
        if (font->cf_type == XmFONT_IS_FONT) {
                ...
                XTextExtents(font->f.cf_font, string, nbytes, &direction,
                ...
                *height_return = overall.ascent + overall.descent;
                ...
                XmbTextExtents(font->f.cf_fontset, string, nbytes, &ink,
                               &logical);
 ...
 */
void CalDrawString(Display *dpy, Drawable draw, Cal_Font *font, GC gc, int x,
        ...
        if (font->cf_type == XmFONT_IS_FONT) {
                ...
                XmbDrawString(dpy, draw, font->f.cf_fontset, gc, x, y, string,
                              length);
        }
}

void CalFontExtents(Cal_Font *font, XFontSetExtents *fse) {
        if (font->cf_type == XmFONT_IS_FONT) {
                /*
                ...
                fse->max_logical_extent.height =
                    font->f.cf_font->ascent + font->f.cf_font->descent;
        } else {
