...
#ifndef lint
static char sccsid[] =
    "@(#)alarm.c 1.50 95/07/27 Copyr 1991 Sun Microsystems, Inc.";
#endif
...
extern int debug;
extern int submit_mail(const char *to, const char *subject, const char *body);
...
extern void view_flasher(XtPointer client_data, XtIntervalId *interval_id) {
        ...
        XGetWindowProperty(XtDisplay(c->frame), XtWindow(c->frame), wmStateAtom,
                           0L, (long)BUFSIZ, False, wmStateAtom, &actualType,
        ...
        if (wmState->state == IconicState) {
                ...
                if (c->view->flashes % 2) {
                        if (c->icon && c->icon->icon && c->icon->icon_mask)
                                XtVaSetValues(c->frame, XmNiconPixmap,
                                              c->icon->icon, NULL);
                } else {
                ...
                XFillRectangle(c->xcontext->display, XtWindowOfObject(c->form),
                               c->xcontext->invert_gc, 0, 0,
...
extern void flash_it(XtPointer client_data, XtIntervalId *interval_id) {
        int i, j;
        Calendar *c = (Calendar *)client_data;
...
extern void mail_it(XtPointer client_data, XtIntervalId *interval_id,
                    CSA_reminder_reference *r) {
        Calendar *c = (Calendar *)client_data;
        int hr;
        ...
        appt = allocate_appt_struct(
            ...
            CSA_ENTRY_ATTR_SUMMARY_I, CSA_ENTRY_ATTR_ORGANIZER_I,
            CSA_X_DT_ENTRY_ATTR_SHOWTIME_I, CSA_ENTRY_ATTR_END_DATE_I,
        ...
        /* compose to field */
        if (appt->mail->value->item.reminder_value->reminder_data.data ==
                NULL ||
        ...
        if (showtime_set(appt) && !magic_time(start)) {
                ...
                if (dt == HOUR12) {
                        snprintf(startbuf, 100, "%2d:%02d %s", hr,
                                 minute(start), pm ? "pm" : "am");
                } else {
        ...
        if (showtime_set(appt) && !magic_time(start) && appt->end_time) {
                _csa_iso8601_to_tick(
                    appt->end_time->value->item.date_time_value, &stop);
                hr = hour(stop);
                pm = (dt == HOUR12 && !adjust_hour(&hr)) ? True : False;
                if (dt == HOUR12) {
        ...
        if (stopbuf[0] != '\0') {
                snprintf(bodybuf, BUFSIZ,
                         catgets(c->DT_catd, 1, 7,
...
extern void open_it(XtPointer client_data, XtIntervalId *interval_id,
                    CSA_reminder_reference *r) {
        Calendar *c = (Calendar *)client_data;
...
extern void reminder_driver(XtPointer client_data, XtIntervalId *interval_id) {
        Calendar *c = (Calendar *)client_data;
        Tick run_tick;
        int i;
        ...
        if (now() >= run_tick) {
                ...
                for (i = 0; i < c->view->next_alarm_count; i++) {
                        if (strcmp(c->view->next_alarm[i].attribute_name,
                                   CSA_ENTRY_ATTR_AUDIO_REMINDER) == 0)
                                ring_it(client_data, interval_id);
                        else if (strcmp(c->view->next_alarm[i].attribute_name,
                                        CSA_ENTRY_ATTR_FLASHING_REMINDER) == 0)
                                flash_it(client_data, interval_id);
                        else if (strcmp(c->view->next_alarm[i].attribute_name,
                                        CSA_ENTRY_ATTR_POPUP_REMINDER) == 0)
                                open_it(client_data, interval_id,
                                        &c->view->next_alarm[i]);
                        else if (strcmp(c->view->next_alarm[i].attribute_name,
                                        CSA_ENTRY_ATTR_MAIL_REMINDER) == 0)
                                mail_it(client_data, interval_id,
                                        &c->view->next_alarm[i]);
                }
        }
...
extern void bell_ringer(XtPointer client_data, XtIntervalId *interval_id) {
        Calendar *c = (Calendar *)client_data;
...
extern void ring_it(XtPointer client_data, XtIntervalId *interval_id) {
        Calendar *c = (Calendar *)client_data;
 ...
 * associated memory.
 */
static void close_cb(Widget w, XtPointer data, XtPointer ignore) {
...
*/
extern void postup_show_proc(Calendar *c, CSA_reminder_reference *r) {
        int start_hr, stop_hr;
        char text[BUFSIZ];
        time_t st, sp = 0;
        Lines *lines, *l = NULL;
        Props *p = (Props *)c->properties;
        Widget pu_frame, pu_base_form, pu_text_form, separator, pu_form,
            button_form, pu_date, pu_range, pu_image, pu_close, line, last_line;
        ...
        if (!c->postup)
                c->postup = XtAppCreateShell("calendar_postup", "Dtcm",
                                             ...
                                             c->xcontext->display, NULL, 0);

        /*
        **  Create the Motif objects
        */
        title = XtNewString(catgets(c->DT_catd, 1, 839, "Calendar : Reminder"));
        pu_frame = XtVaCreatePopupShell(
            "pu_frame", topLevelShellWidgetClass, c->postup, XmNtitle, title,
            XmNmwmFunctions, MWM_FUNC_MOVE | MWM_FUNC_CLOSE, XmNdeleteResponse,
            XmDESTROY, NULL);
        XtFree(title);

        pu_base_form =
            XtVaCreateWidget("pu_base_form", xmFormWidgetClass, pu_frame, NULL);

        pu_text_form = XtVaCreateWidget(
            "pu_test_form", xmFormWidgetClass, pu_base_form, XmNtopAttachment,
            XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, XmNtopAttachment,
            XmATTACH_FORM, XmNleftOffset, 1, XmNrightOffset, 1, XmNtopOffset, 1,
            NULL);

        pu_image = XtVaCreateWidget(
            "pu_image", xmLabelGadgetClass, pu_text_form, XmNleftAttachment,
            XmATTACH_FORM, XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 5,
            XmNlabelType, XmPIXMAP, XmNlabelPixmap,
            ((Props_pu *)c->properties_pu)->postup_pixmap, NULL);

        button_form = XtVaCreateWidget(
            "pu_form", xmFormWidgetClass, pu_base_form, XmNbottomAttachment,
            XmATTACH_FORM, XmNbottomOffset, 2, XmNleftOffset, 1, XmNrightOffset,
            1, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
            XmATTACH_FORM, XmNfractionBase, 3, NULL);

        xmstr = XmStringCreateLocalized(catgets(c->DT_catd, 1, 680, "Close"));
        pu_close = XtVaCreateManagedWidget(
            "close", xmPushButtonWidgetClass, button_form, XmNtopAttachment,
            XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM,
            XmNleftAttachment, XmATTACH_POSITION, XmNrightAttachment,
            XmATTACH_POSITION, XmNleftPosition, 1, XmNrightPosition, 2,
            XmNbottomOffset, 1, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);

        separator = XtVaCreateWidget(
            "separator", xmSeparatorGadgetClass, pu_base_form,
            XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
            XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget, button_form,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, pu_text_form,
            XmNtopOffset, 1, NULL);

        pu_form = XtVaCreateWidget(
            "pu_form", xmFormWidgetClass, pu_text_form, XmNtopAttachment,
            XmATTACH_FORM, XmNtopOffset, 1, XmNleftAttachment, XmATTACH_WIDGET,
            XmNleftWidget, pu_image, XmNverticalSpacing, 10, NULL);

        pu_date = XtVaCreateWidget("pu_date", xmLabelGadgetClass, pu_form,
                                   XmNleftAttachment, XmATTACH_FORM,
                                   XmNtopAttachment, XmATTACH_FORM, NULL);

        pu_range = XtVaCreateWidget(
            "pu_range", xmLabelGadgetClass, pu_form, XmNleftAttachment,
            XmATTACH_FORM, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
            pu_date, XmNtopOffset, 8, NULL);
        ...
        if (appt->end_time)
                _csa_iso8601_to_tick(
                    appt->end_time->value->item.date_time_value, &sp);
        l = lines = text_to_lines(appt->what->value->item.string_value, 5);
        last_line = 0;
        while (l) {
                xmstr = XmStringCreateLocalized(l->s);
                line = XtVaCreateWidget(
                    "text_line", xmLabelGadgetClass, pu_text_form,
                    XmNlabelString, xmstr, XmNleftAttachment, XmATTACH_WIDGET,
                    XmNleftWidget, pu_form, XmNleftOffset, 15, NULL);
                XmStringFree(xmstr);

                if (last_line)
                        XtVaSetValues(line, XmNtopAttachment, XmATTACH_WIDGET,
                                      XmNtopWidget, last_line, XmNtopOffset, 8,
                                      NULL);
                else
        ...
        */
                ...
                if (dis_t == HOUR12) {
                        ...
                        if (sp) {
                                ...
                                */
                                ...
                                snprintf(text, BUFSIZ,
                                         catgets(c->DT_catd, 1, 1087,
                                                 "From %1$2d:%2$02d%3$s to "
                                                 "%4$2d:%5$02d%6$s"),
                                         start_hr, minute(st),
                                         (start_am) ? am : pm, stop_hr,
                                         minute(sp), (stop_am) ? am : pm);
                                XtFree(am);
                                XtFree(pm);
                        } else {
                                /* NL_COMMENT
                                ...
                                */
                                char *meridian = XtNewString(
                                    (start_am)
                                        ? catgets(c->DT_catd, 1, 4, "am")
                                        : catgets(c->DT_catd, 1, 3, "pm"));

                                snprintf(text, BUFSIZ,
                                         catgets(c->DT_catd, 1, 1088,
                                                 "%1$2d:%2$02d%3$s"),
                        ...
                        if (sp) {
                                ...
                                */
                                snprintf(
                                    ...
                                    start_hr, minute(st), stop_hr, minute(sp));
                        } else {
                                /* NL_COMMENT
        ...
        XtVaSetValues(pu_range, XmNlabelString, xmstr, NULL);
        XmStringFree(xmstr);
