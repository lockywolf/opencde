...
#ifndef lint
static char sccsid[] =
    "@(#)cm_lookup.c 1.21 94/08/05 Copyr 1993 Sun Microsystems, Inc.";
#endif
...
static void usage_msg() {
        fprintf(stderr, catgets(DT_catd, 1, 696,
                                "Usage:\n\tdtcm_admin [ -d ] [ -a action ] [ "
                                "-c calendar ][ -s <mm/dd/yy> ] [ -e "
                                "<mm/dd/yy> ] [-f filename]\n"));
}
...
int main(int argc, char **argv) {
        ...
        if (stat != CSA_SUCCESS) {
                char *format = XtNewString(catgets(
