...
#ifndef lint
static char sccsid[] =
    "@(#)dtfns.c 1.3 94/11/07 Copyr 1993 Sun Microsystems, Inc.";
#endif
...
static char *initial_context_names[] = {
    "user:", "thisuser", "org:", "site:", "myorg", "hostorg", "host:", NULL,
};
...
static int (*NS_Reference_prepend_addr_p)(NS_Reference_t *,
                                          const NS_ReferenceAddress_t *);
static int (*NS_Reference_insert_addr_p)(NS_Reference_t *, void **,
                                         const NS_ReferenceAddress_t *);
static int (*NS_Reference_delete_addr_p)(NS_Reference_t *, void **);
static void (*NS_ReferenceAddress_delete_p)(NS_ReferenceAddress_t *);
...
static NS_String_t *(*NS_ReferenceAddress_type_p)(
    const NS_ReferenceAddress_t *);
...

{
        ...
        NS_Context_bind_p =
            (int (*)(const NS_Context_t *, const NS_CompositeName_t *,
                     const NS_Reference_t *, unsigned,
                     NS_ContextStatus_t *))dlsym(libfns_handle,
                                                 "NS_Context_bind");

        NS_Reference_new_p = (NS_Reference_t * (*)(const NS_String_t *))
            dlsym(libfns_handle, "NS_Reference_new");
        NS_Reference_delete_p = (void (*)(const NS_Reference_t *))dlsym(
            libfns_handle, "NS_Reference_delete");
        ;

        NS_Reference_type_p = (NS_String_t * (*)(const NS_Reference_t *))
            dlsym(libfns_handle, "NS_Reference_type");

        NS_Reference_count_p = (int (*)(const NS_Reference_t *))dlsym(
            libfns_handle, "NS_Reference_count");

        NS_Reference_first_p =
            (NS_ReferenceAddress_t * (*)(const NS_Reference_t *, void **))
        ...
        NS_Reference_append_addr_p =
            (int (*)(NS_Reference_t *, const NS_ReferenceAddress_t *))dlsym(
                libfns_handle, "NS_Reference_append_addr");

        NS_Reference_prepend_addr_p =
            (int (*)(NS_Reference_t *, const NS_ReferenceAddress_t *))dlsym(
                libfns_handle, "NS_Reference_prepend_addr");

        NS_Reference_insert_addr_p =
            (int (*)(NS_Reference_t *, void **, const NS_ReferenceAddress_t *))
                dlsym(libfns_handle, "NS_Reference_insert_addr");

        NS_Reference_delete_addr_p = (int (*)(NS_Reference_t *, void **))dlsym(
            libfns_handle, "NS_Reference_delete_addr");

        NS_ReferenceAddress_delete_p = (void (*)(NS_ReferenceAddress_t *))dlsym(
            libfns_handle, "NS_ReferenceAddress_delete");

        NS_ReferenceAddress_new_p =
            (NS_ReferenceAddress_t *
             (*)(const NS_String_t *, unsigned, const void *))
                dlsym(libfns_handle, "NS_ReferenceAddress_new");

        NS_ReferenceAddress_data_p =
            (void *(*)(const NS_ReferenceAddress_t *))dlsym(
                libfns_handle, "NS_ReferenceAddress_data");

        NS_ReferenceAddress_length_p =
            (unsigned (*)(const NS_ReferenceAddress_t *))dlsym(
                libfns_handle, "NS_ReferenceAddress_length");

        NS_ReferenceAddress_type_p =
            (NS_String_t * (*)(const NS_ReferenceAddress_t *))
                dlsym(libfns_handle, "NS_ReferenceAddress_type");

        NS_String_cstring_p = (const char *(*)(const NS_String_t *))dlsym(
            libfns_handle, "NS_String_cstring");

        NS_String_from_cstring_p = (NS_String_t * (*)(const char *))
            dlsym(libfns_handle, "NS_String_from_cstring");

        NS_CompositeName_from_cstring_p =
            (NS_CompositeName_t * (*)(const char *))
 ...
 * are very cheap.
 */
NS_Context_t *dtfns_get_initial_ctx(void)
...

{
        ...
        if (org != NULL && *org != '\0') {
                ...
                snprintf(buf, size, "%s:%s:%s:%s:%s:%s", DTFNS_ORG_NAME, org,
                         type, name, DTFNS_SERVICE_NAME, service);
                ...
                snprintf(buf, size, "%s:%s:%s:%s", type, name,
 ...
 */
int dtfns_isa_helix_name(const char *name)
 ...
 */
void dtfns_myorg_name(char *buf, int len)
 ...
 *		1	Success
 */
int dtfns_lookup_str(const char *name, /* Absolute Helix name */
 ...
 *		1	Success
 */
int dtfns_bind_str(const char *name,     /* Absolute Helix name */
...
EXIT:
        /* Clean up and return */
        if (new_addr != NULL)
                (*NS_ReferenceAddress_delete_p)(new_addr);
        if (ref != NULL)
                (*NS_Reference_delete_p)(ref);
        if (status != NULL)
                (*NS_ContextStatus_delete_p)(status);
 ...
 */
NS_Reference_t *
dtfns_lookup_ref(const char *name) /* Absolute Helix name to lookup */
            ...
            (strcmp(type, DTFNS_NULL_CONTEXT_TYPE) == 0 && p != NULL &&
             strcmp(p, DTFNS_SERVICE_NAME) == 0)) {
                ...
                *buf = (char *)malloc(buf_size);
                ...
                int buf_size =
                    n + strlen(DTFNS_SERVICE_NAME) + strlen(service) + 3;
                *buf = (char *)malloc(buf_size);
 ...
 */
const NS_ReferenceAddress_t *
dtfns_addr_from_ref(const NS_Reference_t *ref, /* Reference to get addr from */
...

{
        ...
        if (dbuf != NULL) {
                ...
                xdrmem_create(
                    &xdr, (caddr_t)(*NS_ReferenceAddress_data_p)(addr),
 ...
 *		NULL on an error
 */
NS_ReferenceAddress_t *dtfns_create_str_addr(const char *data, const char *type)
 ...
 *		1	Success
 */
int dtfns_insert_addr(NS_Reference_t *ref, void **iter_pos,
                      const NS_ReferenceAddress_t *addr)
 ...
 *		1	Success
 */
int dtfns_delete_addr(NS_Reference_t *ref, void **iter_pos)
