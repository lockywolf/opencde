...
**
*******************************************************************************/
static void copy_and_pad_newlines(register char *dest, register char *source) {
        while (*source)
                if ((*dest++ = *source++) == '\n')
                        *dest++ = '\t';
}

static int count_newlines(register char *string) {
        int count = 0;
...
static boolean_t query_user(void *client_data) {
        ...
        */
        ...
        if (what_str && what_str[0] != '\0')
                fprintf(stdout,
                        catgets(catd_global, 1, 1047,
                                "This appointment: '%s' has an end\n"),
                        what_str);
        else
                fprintf(stdout, "%s",
                        catgets(catd_global, 1, 1048,
                                "This appointment has an end\n"));
        fprintf(stdout, "%s",
...
extern int cm_tty_delete(nl_catd catd, CSA_session_handle session, int version,
        ...
        appt =
            allocate_appt_struct(appt_read, version, CSA_ENTRY_ATTR_SUMMARY_I,
            ...
            CSA_X_DT_REPEAT_ONETIME)
                *ans = '1';
                ...
                /* NL_COMMENT
                   ...
                   "The appointment '<appt text>' is part of a repeating series.
                   Do you want to:
                ...
                */
                fprintf(stdout,
                        catgets(catd, 1, 1051,
                                "The appointment '%s' is part of a repeating "
                                "series.  "),
                        appt->what->value->item.string_value);
                fprintf(stdout, "%s",
                        catgets(catd, 1, 1052, "Do you want to:"));
                fprintf(stdout, "%s",
                        catgets(catd, 1, 1053, "\n\t1.  Delete all of them"));
                fprintf(stdout, "%s",
        ...
        switch (*ans) {
        case '1':
                if (csa_delete_entry(session, list[index], CSA_SCOPE_ALL,
                                     NULL) != CSA_SUCCESS)
                        mini_err_msg(catd, appt->what->value->item.string_value,
                                     TTY_Delete);
                break;
        case '2':
                if (csa_delete_entry(session, list[index], CSA_SCOPE_ONE,
                                     NULL) != CSA_SUCCESS)
                        mini_err_msg(catd, appt->what->value->item.string_value,
                                     TTY_Delete);
                break;
...
extern void cm_tty_format_header(Props *p, Tick tick, char *buf) {
        ...
        case ORDER_MDY:
                snprintf(buf, sizeof(buf), "%s %s %d, %d", day_str(d_op),
                         month_str(m_op), dom(tick), year(tick));
                break;
        case ORDER_DMY:
                snprintf(buf, sizeof(buf), "%s %d %s, %d", day_str(d_op),
                         dom(tick), month_str(m_op), year(tick));
                break;
        case ORDER_YMD:
                snprintf(buf, sizeof(buf), "%s, %d %s %d", day_str(d_op),
                         year(tick), month_str(m_op), dom(tick));
                break;
        default:
                buf[0] = '\0';
                break;
...
extern void scrub_attr_list(Dtcm_appointment *appt) {
        ...
        for (i = 0; i < appt->count; i++) {
                if (appt->attrs[i].value->type == CSA_VALUE_REMINDER) {
                ...
                } else if ((appt->attrs[i].value->type ==
                            CSA_VALUE_ACCESS_LIST) &&
                           (appt->attrs[i].value->item.access_list_value ==
                            NULL)) {
                        free(appt->attrs[i].name);
                        appt->attrs[i].name = NULL;
                        ...
                        appt->attrs[i].name = NULL;
                            ...
                            CSA_VALUE_DATE_TIME) &&
                           (appt->attrs[i].value->item.date_time_value ==
                            NULL)) {
                        free(appt->attrs[i].name);
                        appt->attrs[i].name = NULL;
                }
...
extern int cm_tty_insert(nl_catd catd, CSA_session_handle target, int version,
        ...
        if (filename) {
                ...
                op =
                    validate_appt(catd, appt, start, end, date, 0, what, repeat,
                                  repeatfor, query_user, what, version);
        ...
        for (cnt = 1; cnt <= list->count; cnt++) {
                ...
                case INVALID_DATE:
                        t1 =
                            catgets(catd, 1, 1058, "Invalid Date specified.\n");
                        break;
                case INVALID_START:
                ...
                if (op == VALID_APPT) {
                        ...
                        if ((status = csa_add_entry(target, appt->count,
                                                    appt->attrs, &new_entry,
                                                    NULL)) != CSA_SUCCESS) {
                                mini_err_msg(
                                    catd, appt->what->value->item.string_value,
                                    TTY_Insert);
                                ret_stat = -1;
                        } else
...
extern int cm_tty_lookup(nl_catd catd, CSA_session_handle target, int version,
        ...
        setup_range(&range_attrs, &ops, &i, start, stop, CSA_TYPE_EVENT, 0,
                    B_FALSE, version);
        status =
        ...
        appt = allocate_appt_struct(
            appt_read, version, CSA_ENTRY_ATTR_START_DATE_I,
            CSA_ENTRY_ATTR_SUMMARY_I, CSA_X_DT_ENTRY_ATTR_SHOWTIME_I,
            CSA_ENTRY_ATTR_END_DATE_I, NULL);
        for (i = 0; i < a_total; i++) {

                if (query_appt_struct(target, (*list)[i], appt) !=
                    CSA_SUCCESS) {
                        mini_err_msg(catd, appt->what->value->item.string_value,
                                     TTY_Lookup);
                        continue;
                ...
                if ((lines = text_to_lines(appt->what->value->item.string_value,
                                           5)) &&
                    lines->s) {
...
extern char *get_datemsg(OrderingType order, SeparatorType sep) {
        ...
        switch (order) {
        case ORDER_DMY:
                snprintf(buf, 20, "%s %s %s %s %s", "Day", str, "Month", str,
                         "Year");
                break;
        case ORDER_YMD:
                snprintf(buf, 20, "%s %s %s %s %s", "Year", str, "Month", str,
                         "Day");
                break;
        case ORDER_MDY:
        default:
                snprintf(buf, 20, "%s %s %s %s %s", "Month", str, "Day", str,
                         "Year");
                break;
        }
...

{
        ...
        if (!strcmp(tag, "caluser")) {
                ...
                s_ptr = strchr(value, ':');
                *s_ptr = '\0';
                vptr->item.calendar_user_value =
                    calloc(sizeof(CSA_calendar_user), 1);
                vptr->item.calendar_user_value->user_name =
                    cm_strdup(s_ptr + 1);
        ...
        if (!strcmp(tag, "reminder")) {
                ...
                *s_ptr = '\0';
                ...
                _csa_duration_to_iso8601(atoi(value),
                                         vptr->item.reminder_value->lead_time);
                vptr->item.reminder_value->reminder_data.data =
                    (CSA_uint8 *)strdup(s_ptr + 1);
                vptr->item.reminder_value->reminder_data.size =
                    strlen(s_ptr + 1) + 1;
        }
...

{
        ...
        if (!new_appt_end_delimiter) {
                ...
                snprintf(new_appt_end_delimiter, size, "%s%s",
                         CSA_X_DT_ENTRY_ATTR_ENTRY_DELIMITER, ":string:end");
        }
        ...
        while (fgets(line, MAXNAMELEN - 1, fp)) {
                ...
                if (strncmp(line, new_appt_end_delimiter,
                            strlen(new_appt_end_delimiter)) == 0)
                        break;
                ...
                if (line[0] == '\t') {
                        ...
                        a_value = realloc(a_value,
                                          strlen(a_value) + strlen(b_ptr) + 2);
                        strlcat(a_value, "\n", sizeof(a_value));
        ...
        if (a_name && a_tag && a_value) {
                ...
                if (ident_name_ro(a_name, DATAVER_ARCHIVE) == B_FALSE) {
                        ...
                        build_new_attrval(&avlist->attrs[attrs_written], a_name,
                                          a_tag, a_value);
                        ++attrs_written;
                }
...
static void generate_recurrence_rule(Dtcm_appointment *appt, int version) {
        ...
        if (!appt->recurrence_rule || !appt->recurrence_rule->value) {
                ...
                case CSA_X_DT_REPEAT_EVERY_NMONTH:
                        snprintf(rule_buf1, rule_buf_size, "MD%ld ",
                                 repeat_nth);
                        break;
                }
                ...
                appt->attrs = realloc(appt->attrs, sizeof(CSA_attribute) *
                                                       (appt->count + 1));

                build_new_attrval(&appt->attrs[appt->count],
                                  CSA_ENTRY_ATTR_RECURRENCE_RULE, "string",
                                  rule_buf1);
...
extern Validate_op parse_appt_from_file(nl_catd catd, char *file,
        ...
        while (valid_op == VALID_APPT && fgets(line, MAXNAMELEN - 1, fp)) {
                ...
                case APPOINTMENT_START:
                        if (processing_appt) {
                                ...
                                valid_op = validate_appt(
                                    catd, appt, s_buf, e_buf, d_buf, dur, w_buf,
                                    r_buf, f_buf, query, key_data, version);
                                if (valid_op == VALID_APPT) {
                                        appt = allocate_appt_struct(
                                            appt_write, version, NULL);
                                        load_appt_defaults(appt, p);
                                } else
                ...
                case WHAT_KEY:
                        ...
                        if (!processing_what) {
                                ...
                                tmp = w_buf;
                                int size =
                                    cm_strlen(tmp) + cm_strlen(key_str) + 2;
        ...
        if (processing_appt) {
                CmDataListAdd(list, (void *)appt, 0);
                valid_op =
                    validate_appt(catd, appt, s_buf, e_buf, d_buf, dur, w_buf,
                                  r_buf, f_buf, query, key_data, version);
                if (w_buf)
                        free(w_buf);
        }
        ...
        for (i = 0; i < num_attrs; i++) {
                ...
                case CSA_VALUE_DATE_TIME:
                        ...
                        if (attrs[i].value->item.date_time_value)
                                cat_indented_string(
                                    &buffer, attrs[i].value->item.string_value);
                        growcat(&buffer, "\n");
                        break;
                ...
                case CSA_VALUE_CALENDAR_USER:
                        ...
                        growcat(&buffer, tmp_buf);
                        snprintf(
                            tmp_buf, MAXNAMELEN, "caluser:%ld:",
                            attrs[i]
                                .value->item.calendar_user_value->user_type);
                        growcat(&buffer, tmp_buf);
                ...
                case CSA_VALUE_REMINDER:
                        if (attrs[i].value->item.reminder_value->lead_time ==
                            NULL)
                                continue;
                        ...
                        _csa_iso8601_to_duration(
                            ...
                            &advance_time);
                        snprintf(tmp_buf, MAXNAMELEN,
                                 "reminder:%d:", advance_time);
                        growcat(&buffer, tmp_buf);
                        if (attrs[i]
                                .value->item.reminder_value->reminder_data.data)
                                cat_indented_string(
                                    &buffer, (char *)attrs[i]
...
char *entry_to_attrval_string(CSA_session_handle target,
        ...
        csa_read_entry_attributes(target, entry, num_attrs, names,
                                  &num_attrs_ret, &attrs_ret, NULL);
...
extern char *parse_attrs_to_string(Dtcm_appointment *appt, Props *p,
         ...
         */
        cm_strlcpy(r_buf, periodstr_from_period(repeat_type, repeat_nth),
                   MAXNAMELEN);
        ...
        int b_ptr_length =
            ...
            cm_strlen(f_buf) + cm_strlen(w_buf) + cm_strlen(format_string) +
            cm_strlen(attr_string) + 1;

        b_ptr = malloc(b_ptr_length);

        snprintf(b_ptr, b_ptr_length, format_string,
                 CSA_X_DT_ENTRY_ATTR_ENTRY_DELIMITER, attr_string,
                 CSA_X_DT_ENTRY_ATTR_ENTRY_DELIMITER, d_buf, s_buf, e_buf,
                 r_buf, f_buf, w_buf);
...
char *create_rfc_message(char *address_list, char *subject,
        ...
        while (done != B_TRUE) {
                snprintf(divider_string, sizeof(divider_string),
                         "%x_%x-%x_%x-%x_%x", arc4random(), arc4random(),
                ...
                for (i = 0; i < num_objects; i++) {
                        if (strstr(appointment_objects[i], divider_string) !=
                            NULL)
                                done = B_FALSE;
                }
        }
        ...
        snprintf(return_buffer, buffer_size + 1, "%s%s\n%s%s\n%s%s\n\n",
                 address_header, address_list, subject_header, subject,
                 content_label, divider_string);
...
boolean_t appointments_to_file(CSA_session_handle target,
        ...
        for (i = 0; i < num_appts; i++) {
                entry_string =
                    entry_to_attrval_string(target, appointment_list[i]);
...
extern void str_to_period(char *ps, CSA_sint32 *repeat_type, int *repeat_nth) {
        ...
        *repeat_nth = 0;
        ...
        else if (strncasecmp(ps, periodstrings[6], strlen(periodstrings[6])) ==
                 0)
        ...
        else if (strncasecmp(ps, periodstrings[15],
                             strlen(periodstrings[15])) == 0) {
...
extern char *periodstr_from_period(CSA_sint32 repeat_type, int repeat_nth) {
        ...
        case CSA_X_DT_REPEAT_MONTHLY_BY_WEEKDAY:
                ...
                break;
        case CSA_X_DT_REPEAT_EVERY_NDAY:
                snprintf(pstr, length, "Every %d %s", repeat_nth,
                         periodstrings[7]);
                break;
        case CSA_X_DT_REPEAT_EVERY_NWEEK:
                snprintf(pstr, length, "Every %d %s", repeat_nth,
...
extern Validate_op validate_dssw(Dtcm_appointment *a, char *s_buf, char *e_buf,
        ...
        if (!blank_buf(s_buf)) {
                ...
                _csa_tick_to_iso8601(appt_time,
                ...
                if (dur > 0) {
                        ...
                        _csa_tick_to_iso8601(
                            end_tick, a->end_time->value->item.date_time_value);
...
extern Validate_op validate_rfp(nl_catd catd, Dtcm_appointment *a, char *r_buf,
        ...
        int repeat_nth, repeat_wk = -1, repeat_every = 0,
        ...
        case CSA_X_DT_REPEAT_MONTHLY_BY_WEEKDAY: {
                ...
                if (a && a->time && a->time->value) {
                        _csa_iso8601_to_tick(
                            a->time->value->item.date_time_value, &tick);
                }
                ...
                if (tick && weekofmonth(tick, &wk) && wk == 5)
                        snprintf(rule_buf, rule_buf_size, "MP1 1- %s ",
                                 dow_str(tick));
                else
