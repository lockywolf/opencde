...
#ifndef lint
static char sccsid[] =
    "@(#)props.c 1.13 94/11/07 Copyr 1991 Sun Microsystems, Inc.";
#endif
...
static const char *property_names[] = {
    "BegOp",        "BeepOn",         "BeepAdvance",  "BeepUnit",
    "FlashOn",      "FlashAdvance",   "FlashUnit",    "OpenOn",
    "OpenAdvance",  "OpenUnit",       "MailOn",       "MailAdvance",
    "MailUnit",     "MailTo",         "UnixOn",       "UnixAdvance",
    "UnixCommand",  "DayBegin",       "DayEnd",       "CalendarList",
    "DefaultView",  "DefaultDisplay", "PrintDest",    "PrintPrivacy",
    "PrinterName",  "PrintOptions",   "PrintDirName", "PrintFileName",
    "PrintRMargin", "PrintBMargin",   "PrintLMargin", "PrintTMargin",
    "PrintLHeader", "PrintRHeader",   "PrintLFooter", "PrintRFooter",
    "PrintUnit",    "PrintCopies",    "DefaultCal",   "Location",
    "DateOrdering", "DateSeparator",  "Privacy",      "UseFNS",
    "ApptBegin",    "ApptDuration",   "EndOp"};
...
**
*******************************************************************************/
static void p_free_props_entries(Props_entry *e) {
        if (!e)
                return;
        if (e->next)
...
static Props_entry *p_get_props_entry(Props *p, Props_op op) {
...
*/
extern boolean_t cal_convert_cmrc(Props *p) {
...
extern char *get_char_prop(Props *p, Props_op op) {
...
extern char *get_char_prop_default(Props_op op) {
...
extern int get_int_prop(Props *p, Props_op op) {
        Props_entry *step = p_get_props_entry(p, op);
...
extern void props_clean_up(Props *p) {
...
extern boolean_t read_props(Props *p) {
        ...
        for (op = CP_BEGOP + 1; op < CP_ENDOP; op++) {
                ...
                entry_ptr->property_name =
                    (char *)cm_strdup((char *)property_names[op]);
                ...
                if (!resource || *resource == '\0')
                        resource = (char *)get_resource(
                            other_rdb, (char *)CLASS_NAME, (char *)APP_NAME,
                            entry_ptr->property_name, NULL);
                if (!resource || *resource == '\0')
...
extern boolean_t save_props(Props *p) {
...
extern boolean_t set_char_prop(Props *p, Props_op op, char *value) {
        Props_entry *step = p_get_props_entry(p, op);
...
extern boolean_t set_int_prop(Props *p, Props_op op, int value) {
        char buf[MAXNAMELEN];
