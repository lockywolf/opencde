...
typedef enum { SameInterval = 0, NextInterval } MoveIndicator;
...
Tick NextTick(const Tick cur_time, const Tick start_time, RepeatEvent *re,
...
static Tick NextDayTick(const Tick cur_time, const Tick start_time,
                        const RepeatEvent *re, RepeatEventState *res) {
        struct tm *tm;
        struct tm start_tm;
        _Xltimeparams localtime_buf;
...
static Tick NextWeekTick(const struct tm *current_tm, const struct tm *start_tm,
        ...
        cur_tm->tm_mday +=
            GetWDayDiff(cur_tm->tm_wday, wd_daytime[res_daytime].dt_day);
...
static Tick NextMonthTick(const struct tm *current_time, const RepeatEvent *re,
