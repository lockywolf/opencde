...
static int monthsecs[12] = {31 * daysec, 28 * daysec, 31 * daysec, 30 * daysec,
...
extern int _DtCms_get_ninstance_v4(Appt_4 *appt) {
        ...
        case everyNthMonth_4:
                ninstance =
                    (appt->ntimes + (appt->period.nth - 1)) / appt->period.nth;
                break;
        ...
        case daysOfWeek_4:
                ...
                ndelta =
                    get_ndelta(appt->appt_id.tick, appt->period, appt->ntimes);
                dninstance =
                    (double)(appt->ntimes - 1) * timesperweek + pdelta - ndelta;
                ninstance = (dninstance > _DtCM_OLD_REPEAT_FOREVER)
                                ? _DtCM_OLD_REPEAT_FOREVER
                                : (int)dninstance;
 ...
 */
extern int _DtCms_get_new_ntimes_v4(Period_4 period, time_t tick,
                                    int ninstance) {
        struct tm *tm;
        int ntimes;
        ...
        case monWedFri_4:
                ...
                if (ninstance % 3)
                        delta =
                            ((ninstance % 3) > ((7 - tm->tm_wday) / 2)) ? 2 : 1;
                else if (tm->tm_wday != 1)
                        delta = 1;
        ...
        case daysOfWeek_4:
                ...
                if (ninstance % timesperweek)
                        delta =
                            ((ninstance % timesperweek) > firstweek) ? 2 : 1;
 ...
 */
extern time_t _DtCms_closest_tick_v4(time_t target, time_t ftick,
        ...
        case everyNthDay_4:
                ...
                if (tm1.tm_isdst == 1 && tm1.tm_isdst != tm2.tm_isdst)
                        remainder -= hrsec;
                *ordinal = delta + (remainder > 0 ? 1 : 0) + 1;
                ctick = nextnday_exacttime(ftick, period.nth * (*ordinal - 1));
                break;
        case everyNthWeek_4:
                ...
                if (tm1.tm_isdst == 1 && tm1.tm_isdst != tm2.tm_isdst)
                        remainder -= hrsec;
                *ordinal = delta + (remainder > 0 ? 1 : 0) + 1;
                ctick = nextnwk_exacttime(ftick, period.nth * (*ordinal - 1));
                break;
        case everyNthMonth_4:
                tm = _XLocaltime(&ftick, localtime_buf);
                if (tm->tm_mday < 29) {
                        ...
                        remainder =
                            target - ftick - (daysec * 32 * period.nth) * delta;
                        *ordinal = delta + (remainder > 0 ? 1 : 0) + 1;
        ...
        case daysOfWeek_4:
                ...
                case daysOfWeek_4:
                        *ordinal =
                            delta * ntimes_this_week((u_int)period.nth, 0) +
                            ntimes_this_week((u_int)period.nth, tm->tm_wday);
                }

                /* delta*daysperweek+(lastapptofweek-firstday in first week) */
                if (period.period == daysOfWeek_4) {
                        ndays = delta * 7 + lastapptofweek((u_int)period.nth) -
                                tm->tm_wday;
 ...
 */
extern time_t _DtCms_last_tick_v4(time_t ftick, Period_4 period, int ntimes) {
        struct tm *tm;
        double dltick;
        time_t ltick;
        int i;
        ...
        case nthWeekday_4:
                ...
                break;
        case everyNthDay_4:
                ltick = nextnday_exacttime(
                    ftick,
                    period.nth *
                        (((ntimes + (period.nth - 1)) / period.nth) - 1));
                break;
        case everyNthWeek_4:
                ltick = nextnwk_exacttime(
                    ftick,
                    period.nth *
                        (((ntimes + (period.nth - 1)) / period.nth) - 1));
                break;
        case everyNthMonth_4:
                tm = _XLocaltime(&ftick, localtime_buf);
                if (tm->tm_mday < 29)
                        ltick = nextnmth_exactday(
                            ftick, period.nth * (((ntimes + (period.nth - 1)) /
                                                  period.nth) -
                                                 1));
                else
                        ltick = EOT;
        ...
        case daysOfWeek_4:
                ...
                if (period.period == daysOfWeek_4)
                        dltick = ftick + ((double)(ntimes - 1) * 7 +
                                          lastapptofweek((u_int)period.nth) -
                                          tm->tm_wday) *
                                             daysec;
                else if (period.period == tueThur_4)
                        dltick = ftick + ((double)(ntimes - 1) * 7 +
 ...
 */
extern int _DtCms_in_repeater(Id_4 *key, Appt_4 *p_appt,
        ...
        tick = _DtCms_closest_tick_v4(key->tick, p_appt->appt_id.tick, period,
                                      &ordinal);
        ordinal--;
...
extern boolean_t _DtCmsInExceptionList(cms_entry *eptr, time_t tick) {
        ...
        if (eptr->attrs[CSA_ENTRY_ATTR_EXCEPTION_DATES_I].value)
                dt = eptr->attrs[CSA_ENTRY_ATTR_EXCEPTION_DATES_I]
...
static int get_ndelta(time_t startdate, Period_4 period, int ntimes) {
        ...
        if (period.period == monThruFri_4 || period.period == monWedFri_4)
                ...
                lastdate =
                    lastdate +
                    daysec * (lastapptofweek((u_int)period.nth) - tm->tm_wday);
