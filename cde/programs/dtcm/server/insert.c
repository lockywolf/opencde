...
extern CSA_return_code _DtCmsInsertEntry(_DtCmsCalendar *cal,
                                         cms_entry *entry) {
        ...
        if (key == 0) {
                /* set start date */
                date = entry->attrs[CSA_ENTRY_ATTR_START_DATE_I]
                ...
                if ((stat = _DtCm_set_opaque_attrval(
                         &opq,
        ...
        if (_IsOnetimeEntry(entry) == B_FALSE) {
                ...
                if (count == 1) {
                        ...
                        /* adjust start date */
                        tick = ClosestTick(entry->key.time, entry->key.time, re,
                                           &res);
...
extern CSA_return_code _DtCmsInsertEntryAndLog(_DtCmsCalendar *cal,
                                               cms_entry *entry) {
        CSA_return_code stat;
...
static CSA_return_code _RuleToRepeatInfo(cms_entry *entry, RepeatEvent *re) {
        ...
        if (re == NULL)
                return (_DtCm_set_sint32_attrval(
                    CSA_X_DT_REPEAT_ONETIME,
                    &entry->attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I].value));

        if ((stat = _DtCm_set_sint32_attrval(
                 _RuleToRepeatType(re),
                 &entry->attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I].value)) !=
            CSA_SUCCESS)
                return (stat);

        type = entry->attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I]
        ...
        if (re->re_duration != RE_NOTSET) {
                ...
                if ((stat = _DtCm_set_uint32_attrval(
                         duration,
                         &entry->attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_TIMES_I]
                              .value)) != CSA_SUCCESS)
                        return (stat);
        }
