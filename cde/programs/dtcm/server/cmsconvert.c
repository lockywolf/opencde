...
extern CSA_return_code _DtCmsCmsentriesToAppt4ForClient(cms_entry *entries,
                                                        Appt_4 **appt) {
...
extern CSA_return_code _DtCmsCmsentriesToAbbrAppt4ForClient(cms_entry *entries,
                                                            Abb_Appt_4 **abbr) {
        CSA_return_code stat;
...
extern Access_Entry_4 *_DtCmsConvertV5AccessList(cms_access_entry *cmslist,
...
extern cms_access_entry *_DtCmsConvertV4AccessList(Access_Entry_4 *alist) {
        cms_access_entry *l, *head, *prev;
        boolean_t cleanup = B_FALSE;

        prev = head = NULL;
        while (alist != NULL) {
                if ((l = (cms_access_entry *)calloc(
                         1, sizeof(cms_access_entry))) == NULL) {
                        cleanup = B_TRUE;
                        break;
                }
...
extern CSA_return_code _DtCmsReminderRefToReminder(cms_reminder_ref *rems,
                                                   Reminder_4 **r4) {
        ...
        while (rems != NULL) {
                ...
                if ((rptr->attr.attr =
                         _ConvertReminderName(rems->reminder_name)) == NULL) {
                        free(rptr);
                        stat = CSA_E_INSUFFICIENT_MEMORY;
                        break;
                }
...
extern CSA_return_code _DtCmsV4ReminderToReminderRef(char *calname,
                                                     Reminder_4 *r4,
                                                     _DtCmsEntryId *ids,
        ...
        while (r4 != NULL) {
                ...
                if ((rptr = (cms_reminder_ref *)calloc(
                         1, sizeof(cms_reminder_ref))) == NULL) {
                        stat = CSA_E_INSUFFICIENT_MEMORY;
                        break;
                }
...
extern CSA_return_code _DtCmsCmsAccessToV4Access(cms_access_entry *alist,
                                                 Access_Entry_4 **a4) {
        CSA_return_code stat = CSA_SUCCESS;
        ...
        head = prev = NULL;
        while (alist != NULL) {
                if ((to = (Access_Entry_4 *)calloc(
                         1, sizeof(Access_Entry_4))) == NULL) {
                        stat = CSA_E_INSUFFICIENT_MEMORY;
                        break;
                }

                if ((to->access_type =
                         _DtCmAccessRightToV4AccessType(alist->rights)) < 0) {
...
_DtCmsAppt4ToCmsentriesForClient(char *calname, Appt_4 *appt, cms_entry **e_r) {
        CSA_return_code stat = CSA_SUCCESS;
        ...
        while (appt != NULL) {
                ...
                if ((stat =
                         _Appt4ToCmsentryAttrs(calname, appt, &entry->num_attrs,
                                               &entry->attrs)) == CSA_SUCCESS) {
...
extern CSA_return_code _DtCmsAppt4ToCmsentry(char *calname, Appt_4 *a4,
                                             cms_entry **entry_r,
                                             boolean_t rerule) {
        CSA_return_code stat;
        cms_entry *entry;
 ...
 */
extern CSA_return_code _DtCmsAttrsToAppt4(uint size, cms_attribute *attrs,
                                          Appt_4 *appt, boolean_t check) {
        CSA_return_code stat = CSA_SUCCESS;
        char *string;
        time_t endtick;
        ...
        for (i = 0; i < size && stat == CSA_SUCCESS; i++) {
                ...
                case CSA_ENTRY_ATTR_ORGANIZER_I:
                        free(appt->author);
                        if ((appt->author = strdup(
                                 attrs[i].value->item.calendar_user_value)) ==
                            NULL)
                                stat = CSA_E_INSUFFICIENT_MEMORY;
                        break;
                ...
                case CSA_ENTRY_ATTR_AUDIO_REMINDER_I:
                        ...
                        if (attrs[i].value == NULL)
                                ...
                                stat = _DtCm_add_reminder(
                                    ...
                                    attrs[i].value->item.reminder_value, appt);
                        break;
                ...
                if (attrs[enddateindex].value == NULL ||
                    attrs[enddateindex].value->item.date_time_value == NULL) {
                        appt->duration = 0;
                               ...
                               attrs[enddateindex].value->item.date_time_value,
                               &endtick)) {
                        stat = CSA_E_INVALID_DATE_TIME;
                } else {
        ...
        if (stat == CSA_SUCCESS && (typeindex >= 0 || typeindex >= 0)) {
                if (typeindex >= 0) {
                        if ((stat = _DtCm_attrs_to_eventtype4(
                                 ...
                                 (stypeindex >= 0 ? attrs[stypeindex].value
                                                  : NULL),
                                 appt->tag))) {
                                if (check == B_FALSE) {
...
static CSA_return_code _AttrsToAbbrAppt(uint size, cms_attribute *attrs,
        ...
        if (enddateindex >= 0) {
                _csa_iso8601_to_tick(
                    attrs[enddateindex].value->item.date_time_value, &endtick);
                appt->duration = endtick - appt->appt_id.tick;
        } else
                appt->duration = 0;

        if (_DtCm_attrs_to_eventtype4(
                attrs[typeindex].value,
...
static uint _ConvertV5Access(uint v5rights, boolean_t strictmode) {
        ...
        if ((strictmode == B_TRUE && (v5rights & CSA_INSERT_PUBLIC_ENTRIES) &&
        ...
        if ((strictmode == B_TRUE && (v5rights & CSA_CHANGE_PUBLIC_ENTRIES) &&
...
static CSA_return_code _Appt4ToCmsentryAttrs(char *calname, Appt_4 *a4,
        ...
        if ((attrs = (cms_attribute *)calloc(
                 1, sizeof(cms_attribute) * (_DtCM_OLD_ENTRY_ATTR_SIZE + 1))) ==
            NULL)
                return (CSA_E_INSUFFICIENT_MEMORY);
        ...
        tmpval.type = CSA_VALUE_DATE_TIME;
        tmpval.item.date_time_value = buf;
        if ((stat = _csa_tick_to_iso8601(a4->appt_id.tick + a4->duration, buf)
                        ? CSA_E_INVALID_DATE_TIME
                        : CSA_SUCCESS) != CSA_SUCCESS ||
            (stat = _DtCm_copy_cms_attribute(&attrs[count++], &tmpattr,
                                             B_TRUE)) != CSA_SUCCESS) {
                goto error;
        }
        ...
        while (reminders != NULL) {
                if ((attrs[count].name.num = _DtCm_old_reminder_name_to_index(
                        ...
                        if ((stat = _DtCmAttr4ToReminderValue(
                                 reminders, &attrs[count++].value)) !=
                            CSA_SUCCESS) {
                                count++;
                                goto error;
                        }
