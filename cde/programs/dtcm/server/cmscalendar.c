...
static _DtCmGetAttrFunc _GetAttrFuncPtrs[] = {NULL,
                                              _GetAccessList,
                                              _GetCalendarName,
                                              _GetCalendarOwner,
                                              _GetCalendarSize,
...
extern _DtCmsCalendar *_DtCmsMakeCalendar(char *owner, char *name) {
        _DtCmsCalendar *cal;
        ...
        if (_DtCm_set_string_attrval(
                name, &cal->attrs[CSA_CAL_ATTR_CALENDAR_NAME_I].value,
                CSA_VALUE_STRING)) {
                _DtCmsFreeCalendar(cal);
                return (NULL);
        }
        ...
        if ((cal->types = (int *)calloc(
                 1, sizeof(int) * (_DtCm_entry_name_tbl->size + 1))) == NULL) {
                _DtCmsFreeCalendar(cal);
                return (NULL);
        } else
        ...
        if (!(cal->list =
                  hc_create(_DtCmsGetEntryKey, _DtCmsCompareRptEntry))) {
                _DtCmsFreeCalendar(cal);
                return (NULL);
        }
...
extern CSA_return_code _DtCmsSetFileVersion(_DtCmsCalendar *cal, int version) {
        ...
        if (version == 1) {
                if (!(cal->tree =
                          rb_create(_DtCmsGetApptKey, _DtCmsCompareAppt))) {
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }

                if (!(cal->list =
                          hc_create(_DtCmsGetApptKey, _DtCmsCompareRptAppt))) {
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
        } else {
                if (!(cal->tree =
                          rb_create(_DtCmsGetEntryKey, _DtCmsCompareEntry))) {
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
                ...
                if ((cal->types = (int *)calloc(
                         1, sizeof(int) * (_DtCm_entry_name_tbl->size + 1))) ==
                    NULL) {
                        _DtCmsFreeCalendar(cal);
                        return (0);
                } else
 ...
 */
extern CSA_return_code start_log(_DtCmsCalendar *cal, char *filename) {
        int fd;
        FILE *f;
 ...
 */
extern CSA_return_code _DtCmsLoadCalendar(char *target,
                                          _DtCmsCalendar **infoptr) {
        ...
        *infoptr = NULL;
        ...
        if (cal->fversion == _DtCMS_VERSION1) {
                ...
                if (cal->attrs[CSA_CAL_ATTR_CALENDAR_OWNER_I].value)
                        cal->owner =
                            strdup(cal->attrs[CSA_CAL_ATTR_CALENDAR_OWNER_I]
                                       .value->item.calendar_user_value);
                else {
                        stat = CSA_X_DT_E_BACKING_STORE_PROBLEM;
                        fprintf(stderr, "%s: %s\n", pgname,
                                "calendar owner attribute is missing from "
                                "callog file");
                        goto ERROR;
                }

                if (cal->attrs[CSA_CAL_ATTR_PRODUCT_IDENTIFIER_I].value ==
                    NULL) {
                        if ((stat = _DtCm_set_string_attrval(
                                 _DtCM_PRODUCT_IDENTIFIER,
                                 &cal->attrs[CSA_CAL_ATTR_PRODUCT_IDENTIFIER_I]
                                      .value,
...
extern CSA_return_code _DtCmsGetCalendarByName(char *target, boolean_t load,
                                               _DtCmsCalendar **cal) {
        char *name;
        _DtCmsCalendar *clist = calendar_list;
 ...
 * For parser only.
 */
extern CSA_return_code _DtCmsInsertAppt_for_parser(_DtCmsCalendar *cal,
                                                   Appt_4 *appt) {
 ...
 * For parser only.
 */
extern CSA_return_code _DtCmsInsertEntry4Parser(_DtCmsCalendar *cal,
                                                cms_entry *entry) {
        cms_entry *eptr;
        CSA_return_code stat;

        if ((stat = _DtCmsMakeHashedEntry(cal, entry->num_attrs, entry->attrs,
                                          &eptr)) != CSA_SUCCESS)
                return (stat);
...
extern CSA_return_code _DtCmsEnumerateUp(_DtCmsCalendar *cal,
 ...
 * For parser only.
 */
extern void _DtCmsSetAccess4Parser(_DtCmsCalendar *cal, Access_Entry_4 *list,
                                   int type) {
...
extern void _DtCmsSetCalendarAttrs4Parser(_DtCmsCalendar *cal, int len,
                                          cms_attribute *attrs) {
...
extern CSA_return_code _DtCmsGetCalAttrsByName(_DtCmsCalendar *cal,
                                               uint num_names,
                                               cms_attr_name *names,
                                               uint *num_attrs_r,
...
extern CSA_return_code _DtCmsGetAllCalAttrs(_DtCmsCalendar *cal,
                                            uint *num_attrs_r,
                                            cms_attribute **attrs_r,
                                            boolean_t returnall) {
        CSA_return_code stat = CSA_SUCCESS;
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                ...
                for (i = 1, j = 0; i <= num_cal_attrs; i++) {
                        if (i <= _DtCM_DEFINED_CAL_ATTR_SIZE) {
                                if ((stat = cal->getattrfuncs[i](
                                         cal, i, &attrs[j])) != CSA_SUCCESS) {
                                        _DtCm_free_cms_attributes(j, attrs);
                                        free(attrs);
                                        return (stat);
                                } else if (attrs[j].name.name)
                                ...
                                if (returnall) {
                                        stat = _DtCm_copy_cms_attribute(
                                            &attrs[j], &cal->attrs[i], B_TRUE);
                                } else {
                                        /* just return the attribute name */
                                        if ((attrs[j].name.name =
                                                 strdup(cattrs[i].name.name))) {
                                                attrs[j].name.num =
                                                    cattrs[i].name.num;
                                        } else
                                                stat =
...
extern CSA_return_code _DtCmsGetCalAttrNames(_DtCmsCalendar *cal,
                                             uint *num_names_r,
...
extern CSA_return_code _DtCmsUpdateCalAttributesAndLog(_DtCmsCalendar *cal,
                                                       uint numsrc,
        ...
        if (cal->attrs && (stat = _DtCm_copy_cms_attributes(
                               cal->num_attrs, cal->attrs, &oldnum,
                               &oldattrs)) != CSA_SUCCESS)
                return (stat);
        ...
        for (i = 0; i < numsrc && stat == CSA_SUCCESS; i++) {
                ...
                if (srcattrs[i].name.num <= 0)
                        srcattrs[i].name.num = _DtCm_get_index_from_table(
                            cal->cal_tbl, srcattrs[i].name.name);
                ...
                if (srcattrs[i].name.num < 0) {
                        ...
                        stat = _DtCmUpdateAttribute(
                            &srcattrs[i], &cal->attrs[srcattrs[i].name.num]);
                }
        }
        ...
        if (stat == CSA_SUCCESS) {
                ...
                /* dump file */
                stat =
                    _DtCmsAppendCalAttrsByFN(log, cal->num_attrs, cal->attrs);
                free(log);
        }
...
extern CSA_return_code _DtCmsV5TransactLog(_DtCmsCalendar *cal, cms_entry *e,
                                           _DtCmsLogOps op) {
        CSA_return_code stat;
        char *log;
        ...
        if (cal->hashed == B_FALSE ||
            cal->num_entry_attrs < cal->entry_tbl->size) {
                if ((stat = _DtCmsAppendHTableByFN(
                         log, cal->entry_tbl->size, cal->entry_tbl->names,
 ...
 */
static CSA_return_code get_file_owner(char *calname, char **owner) {
        struct stat info;
        struct passwd *pw;
        char buf[BUFSIZ];
        char *log;
...
static CSA_return_code _CopyCalendarAttr(_DtCmsCalendar *cal, int index,
        ...
        if (cal->attrs && cal->attrs[index].value)
                return (
                    _DtCm_copy_cms_attribute(attr, &cal->attrs[index], B_TRUE));
        else
                return (CSA_SUCCESS);
}
...
static CSA_return_code _GetCalendarSize(_DtCmsCalendar *cal, int index,
        ...
        if ((stat = _DtCmsGetFileSize(cal->calendar, &size)) == CSA_SUCCESS) {
                ...
                if ((attr->name.name = strdup(CSA_CAL_ATTR_CALENDAR_SIZE)))
                        return (
                            _DtCm_set_uint32_attrval((uint)size, &attr->value));
                else
                        return (CSA_E_INSUFFICIENT_MEMORY);
        }
        return (stat);
...
static CSA_return_code _GetAccessList(_DtCmsCalendar *cal, int index,
                                      cms_attribute *attr) {
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                return (_DtCm_copy_cms_attribute(
                    attr, &cal->attrs[CSA_CAL_ATTR_ACCESS_LIST_I], B_TRUE));
        } else {
                ...
                if ((stat = _DtCm_accessentry4_to_cmsaccesslist(
                         alist, &tmpval.item.access_list_value)) ==
                    CSA_SUCCESS) {
                        ...
                        return (
                            _DtCm_copy_cms_attribute(attr, &tmpattr, B_TRUE));
                } else
                        return (stat);
        }
}
...
static CSA_return_code _GetCalendarName(_DtCmsCalendar *cal, int index,
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                return (_DtCm_copy_cms_attribute(
                    attr, &cal->attrs[CSA_CAL_ATTR_CALENDAR_NAME_I], B_TRUE));
        } else {
                attr->name.num = CSA_CAL_ATTR_CALENDAR_NAME_I;
                if ((attr->name.name = strdup(CSA_CAL_ATTR_CALENDAR_NAME))) {
                        return (_DtCm_set_string_attrval(
                            cal->calendar, &attr->value, CSA_VALUE_STRING));
                } else {
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
        }
...
static CSA_return_code _GetServerVersion(_DtCmsCalendar *cal, int index,
        ...
        if ((attr->name.name = strdup(CSA_X_DT_CAL_ATTR_SERVER_VERSION)))
                return (
                    _DtCm_set_uint32_attrval((uint)TABLEVERS, &attr->value));
        else
                return (CSA_E_INSUFFICIENT_MEMORY);
}
...
static CSA_return_code _GetProductId(_DtCmsCalendar *cal, int index,
        ...
        if ((attr->name.name = strdup(CSA_CAL_ATTR_PRODUCT_IDENTIFIER))) {
                return (_DtCm_set_string_attrval(
                    _DtCM_PRODUCT_IDENTIFIER, &attr->value, CSA_VALUE_STRING));
        } else {
                return (CSA_E_INSUFFICIENT_MEMORY);
        }
}
