...
_EnumerateSequence(char *sender, uint access, List_node *lnode, time_t start1,
                   time_t start2, boolean_t no_end_time_range, time_t end1,
                   time_t end2, CSA_uint32 num_attrs, cms_attribute *attrs,
                   CSA_enum *ops, cms_entry **head, cms_entry **tail);
...
extern CSA_return_code _DtCmsLookupEntriesById(
    _DtCmsCalendar *cal, char *sender, uint access,
    boolean_t no_start_time_range, boolean_t no_end_time_range, time_t start1,
    time_t start2, time_t end1, time_t end2, long id, CSA_uint32 num_attrs,
    cms_attribute *attrs, CSA_enum *ops, cms_entry **entries) {
        ...
        stat = _DtCmsEnumerateSequenceById(
            cal, sender, access, no_start_time_range, no_end_time_range, start1,
            start2, end1, end2, id, num_attrs, attrs, ops, &head);
        ...
        while ((eptr = (cms_entry *)rb_lookup_next_larger(cal->tree,
               ...
               eptr->key.time < start2) {
                if (eptr->key.id != id) {
                        ...
                        if (_DtCmsMatchAttributes(eptr, num_attrs, attrs,
                                                  ops) &&
                            (((stat2 = _DtCmsCheckViewAccess(
                                   sender, access, eptr)) == CSA_SUCCESS) ||
                             (stat2 == CSA_E_TIME_ONLY))) {
...
_DtCmsLookupEntries(_DtCmsCalendar *cal, char *sender, uint access,
                    time_t start1, time_t start2, boolean_t no_end_time_range,
                    time_t end1, time_t end2, CSA_uint32 num_attrs,
                    cms_attribute *attrs, CSA_enum *ops, cms_entry **entries) {
        CSA_return_code stat = CSA_SUCCESS;
        CSA_return_code stat2 = CSA_SUCCESS;
        cms_entry *eptr, *head = NULL, *tail = NULL;
...
extern CSA_return_code _DtCmsEnumerateSequenceById(
    ...
    cms_attribute *attrs, CSA_enum *ops, cms_entry **entries) {
        ...
        if (((stat = _DtCmsCheckViewAccess(
                  sender, access, (cms_entry *)lnode->data)) != CSA_SUCCESS) &&
            (stat != CSA_E_TIME_ONLY))
                return (stat);
...
extern CSA_return_code _DtCmsLookupEntriesByKey(
    _DtCmsCalendar *cal, char *user, uint access, uint num_keys, cms_key *keys,
    uint num_names, cms_attr_name *names, cms_get_entry_attr_res_item **res) {
        CSA_return_code stat = CSA_SUCCESS;
        ...
        for (i = 0, head = NULL; i < num_keys; i++) {
                if ((stat = _DtCmsGetEntryAttrByKey(cal, user, access, keys[i],
                                                    num_names, names, NULL,
                                                    &eptr)) == CSA_SUCCESS) {
                        eptr->next = head;
                        head = eptr;
                } else {
                        if (head)
                                _DtCmsFreeEntryAttrResItem(head);
                        return (stat);
                }
        }
 ...
 */
extern CSA_return_code
_DtCmsGetEntryAttrByKey(_DtCmsCalendar *cal, char *user, uint access,
                        cms_key key, uint num_names, cms_attr_name *names,
                        cms_entry **entry_r,
        ...
        if (res_r && (res = (cms_get_entry_attr_res_item *)calloc(
                          1, sizeof(cms_get_entry_attr_res_item))) == NULL)
                return (CSA_E_INSUFFICIENT_MEMORY);

        /* do lookup on one-time entries first */
        if ((entry = (cms_entry *)rb_lookup(cal->tree, (caddr_t)&key)) ==
            NULL) {
                if ((lnode = (List_node *)hc_lookup_node(
                         cal->list, (caddr_t)&key)) != NULL) {
        ...
        if ((entry == NULL) || (((stat2 = _DtCmsCheckViewAccess(
                                      user, access, entry)) != CSA_SUCCESS) &&
                                (stat2 != CSA_E_TIME_ONLY))) {
                if (entry == NULL)
        ...
        if (entry->key.time != key.time) {
                ...
                if (entry->attrs[CSA_ENTRY_ATTR_END_DATE_I].value) {
                        ...
                        entry->attrs[CSA_ENTRY_ATTR_END_DATE_I]
                            .value->item.date_time_value = ebuf;
                }
        }
        ...
        if (entry_r)
                ...
                if (num_names == 0)
                        ...
                        stat =
                            _GetEntryAttrsByName(cal, entry, num_names, names,
...
static CSA_return_code _AddToLinkedEntries(cms_entry *eptr, cms_entry **head,
        ...
        if ((stat = _DtCmsGetCmsEntryForClient(eptr, &newptr, time_only)) !=
            CSA_SUCCESS)
                return (stat);
...
_EnumerateSequence(char *sender, uint access, List_node *lnode, time_t start1,
        ...
        if (lnode->lasttick <= start1 || fsttick >= start2 ||
            ...
            (!no_end_time_range &&
             (lnode->lasttick + lnode->duration) <= end1) ||
        ...
        for (tick = ClosestTick(start1, fsttick, lnode->re, &restate);
             ...
             tick = NextTick(tick, fsttick, lnode->re, restate)) {
                ...
                stime = eptr->attrs[CSA_ENTRY_ATTR_START_DATE_I]
                            .value->item.date_time_value;
                _csa_tick_to_iso8601(tick, sbuf);
                eptr->attrs[CSA_ENTRY_ATTR_START_DATE_I]
                    .value->item.date_time_value = sbuf;
                ...
                if (eptr->attrs[CSA_ENTRY_ATTR_END_DATE_I].value) {
                        ...
                        eptr->attrs[CSA_ENTRY_ATTR_END_DATE_I]
                            .value->item.date_time_value = ebuf;
                }
        ...
        for (i = 0; i < num_names && stat == CSA_SUCCESS; i++) {
                ...
                if (names[i].num > 0) {
                        stat = _DtCm_copy_cms_attribute(
                            &attrs[i], &entry->attrs[names[i].num], B_TRUE);
                } else {
                        attrs[i].name.num = -1;
                        if ((attrs[i].name.name = strdup(names[i].name)) ==
                            NULL)
...
static CSA_return_code _GetAllEntryAttrs(cms_entry *entry, uint *num_attrs_r,
        ...
        if ((attrs = calloc(1, sizeof(cms_attribute) * entry->num_attrs)) ==
            NULL)
                return (CSA_E_INSUFFICIENT_MEMORY);
        ...
        for (i = 1, j = 0; i <= entry->num_attrs && stat == CSA_SUCCESS; i++) {
                if (entry->attrs[i].value) {
                        if ((stat = _DtCm_copy_cms_attribute(
                                 &attrs[j], &entry->attrs[i], B_TRUE)) ==
                            CSA_SUCCESS) {
                                j++;
                        }
                }
