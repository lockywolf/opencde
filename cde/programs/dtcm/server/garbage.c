...
extern void _DtCmsCollectOne(_DtCmsCalendar *cal) {
        ...
        if (status != rb_ok || clog == NULL || temp == NULL || bak == NULL) {
                fprintf(
                    stderr,
                    "%s: cannot acquire files to execute garbage collection.\n",
                    pgname);
                fprintf(stderr,
                        "possible causes: cannot find home directory.\n");
        ...
        if (stat != CSA_SUCCESS) {
                if (stat == (CSA_X_DT_E_BACKING_STORE_PROBLEM))
                        print_file_error(
                            temp, "file error during garbage collection");
                else {
                        fprintf(
                            stderr,
                            "%s: file error on %s during garbage collection\n",
                            pgname, temp);
                        fprintf(stderr, "Reason: getpwnam() failed. %s%s\n",
                                "No passwd entry for owner of ", cal->calendar);
        ...
        if (stat != CSA_SUCCESS) {
                print_file_error(temp,
                                 (stat == (CSA_X_DT_E_BACKING_STORE_PROBLEM)
                                      ...
                                      : "can't dump data structure to file "
...
extern CSA_return_code _DtCmsDumpDataV2(char *file, _DtCmsCalendar *cal) {
        ...
        if (cal->num_attrs > 0) {
                if ((stat = _DtCmsAppendCalAttrsByFD(
                         fd, cal->num_attrs, cal->attrs)) != CSA_SUCCESS)
                        return (stat);
        }
...
static boolean_t visit1(caddr_t node, caddr_t d) {
        ...
        if ((dump_error = _DtCmsAppendAppt4ByFD(fd, (Appt_4 *)d,
