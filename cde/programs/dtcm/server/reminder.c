...
static _DtCmsRemInfo *_BuildReminder4Entry(cms_entry *entry, int aindex,
                                           List_node *lnode, time_t cutoff,
                                           _DtCmsRemInfo **active);

static void _InsertReminder(_DtCmsRemInfo **head, _DtCmsRemInfo *rem);

static CSA_return_code _GetNextReminders(_DtCmsRemQueue *remq, time_t tick,
                                         cms_reminder_ref **rems);

static CSA_return_code _GetNextRemindersFromQ(_DtCmsRemQueue *remq, int qindex,
                                              time_t tick,
                                              cms_reminder_ref **rems);
...
static void _RemoveReminderFromQ(_DtCmsRemQueue *remq, int qindex,
                                 cms_entry *entry, List_node *lnode,
                                 time_t starttime, boolean_t delfwd);

static _DtCmsRemInfo *_RemoveReminderFromList(_DtCmsRemInfo **remq,
                                              cms_entry *entry,
                                              time_t starttime,
                                              boolean_t delfwd);

static long _GetNextActiveTick(cms_entry *entry, time_t target, time_t lasttick,
                               RepeatEvent *re);

static void _UpdateReminderQ(_DtCmsRemQueue *remq, int qindex);

static CSA_return_code _GetNextRemindersFromList(_DtCmsRemInfo *rlist,
                                                 time_t giventime,
                                                 cms_reminder_ref **rf_r);

static void _DtCmsAddReminder4EntryToQ(_DtCmsRemQueue *remq, cms_entry *entry,
                                       int aindex, List_node *lnode);
...
extern Rm_que *_DtCmsRemoveReminderV4(Rm_que **qhead, Rm_que *p_prev,
                                      Rm_que *p_curr) {
        if (p_prev == NULL)
                *qhead = p_curr->next;
        else
...
extern Rm_que *build_reminder(time_t current_time, Appt_4 *p_appt,
                              Attr_4 p_attr, time_t start_tick,
                              u_int start_ord) {
        int ntimes;
        Period_4 period;
        ...
        if (is_appointment(p_appt)) {
                ...
                if (start_tick >= current_time) {
                        if ((p_reminder =
                                 (Rm_que *)calloc(1, sizeof(Rm_que))) == NULL)
                                return (NULL);
                ...
                while (start_ord <= ntimes) {
                        ...
                        if (start_tick >= current_time) {
                                ...
                                if (!_DtCms_marked_4_cancellation(p_appt,
                                                                  start_ord)) {
                                        if ((p_reminder = (Rm_que *)calloc(
                                                 1, sizeof(Rm_que))) == NULL)
                                                return (NULL);
...
extern Reminder_4 *_DtCmsGetReminderInfoV4(Rm_que *original) {
        ...
        if ((copy = calloc(1, sizeof(Reminder_4))) != NULL) {
                ...
                if ((copy->attr.value = strdup(original->attr->value)) ==
                    NULL) {
                        _DtCm_free_reminder4(copy);
                        return (NULL);
                }

                if ((copy->attr.clientdata =
                         strdup(original->attr->clientdata)) == NULL) {
                        _DtCm_free_reminder4(copy);
                        return (NULL);
                }
 ...
 * reminder queue.
 */
extern void _DtCmsObsoleteReminderV4(Rm_que **qhead, Appt_4 *p_appt, int ord,
        ...
        while (p_hdr != NULL) {
                ...
                p_next = p_hdr->next;
                p_node = build_reminder(p_hdr->remind_at + 1, p_hdr->appt,
                                        p_hdr->attr, p_hdr->remind_at, ord);
...
extern void _DtCmsAddReminders4Entry(_DtCmsRemQueue **remq, cms_entry *entry,
                                     List_node *lnode) {
        _DtCmsRemQueue *queue;
        int i;

        if (*remq == NULL) {
                if ((queue = (_DtCmsRemQueue *)calloc(
                         1, sizeof(_DtCmsRemQueue))) == NULL)
                        return;

                /* initialize queues for the cde defined reminders */
                if ((queue->aindex = (int *)malloc(
                         sizeof(int) * _DtCms_NUM_REMINDERS)) == NULL) {
                        free(queue);
                        return;
                }

                if ((queue->names = (char **)malloc(
                         sizeof(char *) * _DtCms_NUM_REMINDERS)) == NULL) {
                        free(queue->aindex);
                        free(queue);
                        return;
                }

                if ((queue->active = (_DtCmsRemInfo **)calloc(
                         1, sizeof(_DtCmsRemInfo) * _DtCms_NUM_REMINDERS)) ==
                    NULL) {
                        free(queue->names);
                        free(queue->aindex);
                        free(queue);
                ...
                if ((queue->oldhead = (_DtCmsRemInfo **)calloc(
                         1, sizeof(_DtCmsRemInfo) * _DtCms_NUM_REMINDERS)) ==
                    NULL) {
                        free(queue->active);
                        free(queue->names);
...
extern void _DtCmsObsoleteReminder4Entry(_DtCmsRemQueue *remq, cms_entry *entry,
                                         List_node *lnode, time_t starttime,
                                         boolean_t delfwd) {
        int i;

        for (i = 0; i < remq->num_queues; i++) {
                if (entry->attrs[remq->aindex[i]].value)
                        _RemoveReminderFromQ(remq, i, entry, lnode, starttime,
                                             delfwd);
        }
}

extern CSA_return_code _DtCmsLookupReminder(_DtCmsRemQueue *remq, time_t tick,
                                            uint num_names,
                                            cms_attr_name *names,
                                            cms_reminder_ref **rems) {
        int i, j;
        ...
        for (i = 0; i < num_names; i++) {
                for (j = 0; j < remq->num_queues; j++) {
                        if (strcmp(names[i].name, remq->names[j]) == 0) {
                                ...
                                if ((stat = _GetNextRemindersFromQ(
                                         remq, i, tick, rems)) != CSA_SUCCESS) {
                                        if (*rems)
...
static _DtCmsRemInfo *_BuildReminder4Entry(cms_entry *entry, int aindex,
        ...
        _csa_iso8601_to_duration(
            entry->attrs[aindex].value->item.reminder_value->lead_time, &lead);
        ...
        if (lnode == NULL) {
                ...
                if (active && rptr->runtime < cutoff &&
                    ...
                    (tick = _GetNextActiveTick(entry, cutoff + lead,
                                               lnode->lasttick, lnode->re)) >
                        0) {
                        if ((rptr2 = (_DtCmsRemInfo *)calloc(
                                 1, sizeof(_DtCmsRemInfo))) == NULL) {
                                free(rptr);
                                return (NULL);
                        }
...
static CSA_return_code _GetNextRemindersFromQ(_DtCmsRemQueue *remq, int qindex,
        ...
        if (tick >= remq->cutoff)
                ...
                if ((stat = _GetNextRemindersFromList(
                         remq->oldhead[qindex], tick, rems)) == CSA_SUCCESS &&
                    rem == *rems)
...
_GetReminderRefFromInfo(_DtCmsRemInfo *rem, time_t starttime, time_t runtime) {
        ...
        if ((rptr->reminder_name =
                 strdup(entry->attrs[rem->rem.i].name.name)) == NULL) {
                free(rptr);
                return (NULL);
        }

        size = entry->attrs[CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I]
                   .value->item.opaque_data_value->size;
        ...
        if ((rptr->entryid = malloc(size)) == NULL) {
                ...
                strncpy(
                    rptr->entryid,
                    (char *)entry->attrs[CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I]
                        .value->item.opaque_data_value->data,
                    size);
...
static void
_RemoveReminderFromQ(_DtCmsRemQueue *remq, int qindex, cms_entry *entry,
        ...
        _csa_iso8601_to_duration(entry->attrs[remq->aindex[qindex]]
        ...
        if (lnode == 0) {
                ...
                if ((do_old = (tick - lead < remq->cutoff)))
                        rptr1 = _RemoveReminderFromList(
                            &remq->oldhead[qindex], entry, starttime, delfwd);

                if ((do_new = (lnode->lasttick - lead >= remq->cutoff)))
                        rptr2 = _RemoveReminderFromList(
                            &remq->active[qindex], entry, starttime, delfwd);
                        ...
                        if (do_old && do_new) {
                                ...
                                if (rptr1 == NULL)
                                        _RemoveReminderFromList(
                                            &remq->oldhead[qindex], entry, 0,
                                            B_FALSE);
                                else
                                        _RemoveReminderFromList(
                                            &remq->active[qindex], entry, 0,
                                            B_FALSE);
                        }

                        _DtCmsAddReminder4EntryToQ(
                            remq, entry, remq->aindex[qindex], rptr->lnode);
...
static _DtCmsRemInfo *_RemoveReminderFromList(_DtCmsRemInfo **qhead,
                                              cms_entry *entry,
                                              time_t starttime,
                                              boolean_t delfwd) {
        _DtCmsRemInfo *rptr, *prev;
               ...
               rptr->runtime < remq->cutoff;) {
                ...
                if (rptr->lnode == NULL)
                        ...
                        if (tick == rptr->starttime) {
                                ...
                                /* make copy of rptr */
                                nptr = (_DtCmsRemInfo *)calloc(
                                    1, sizeof(_DtCmsRemInfo));
                                *nptr = *rptr;
                                rptr = nptr;
                        }

                        if ((rptr->lasttick - lead < remq->cutoff) ||
                            (tick = _GetNextActiveTick(
                                 entry, remq->cutoff + lead, rptr->lasttick,
                                 rptr->lnode->re)) <= 0) {
                                free(rptr);
                        } else {
                                rptr->starttime = tick;
...
static CSA_return_code _GetNextRemindersFromList(_DtCmsRemInfo *rlist,
        ...
        for (; rlist != NULL; rlist = rlist->next) {
                ...
                if (giventime < rlist->runtime)
                        ...
                        tick = _GetNextActiveTick(
                            rlist->data.e, giventime + lead + 1,
                            rlist->lasttick, rlist->lnode->re);

                        if (tick > 0 &&
                            (head == NULL || (tick - lead <= head->runtime))) {
                                if ((rptr = _GetReminderRefFromInfo(
                                         rlist, tick, tick - lead)) == NULL) {
                                        if (head)
                                                _DtCmsFreeReminderRef(head);
