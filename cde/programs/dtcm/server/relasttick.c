...
Tick LastTick(const Tick start_time, RepeatEvent *re) {
        Tick last_time;
...
static Tick DoMinute(const Tick start_time, const RepeatEvent *re) {
        return (Tick)NULL;
}

static Tick DoDay(const Tick start_time, const RepeatEvent *re) {
        Tick last_time1 = EOT, last_time2 = EOT;
        ...
        if (re->re_duration != RE_NOTSET) {
                ...
                start_tm =
                    _XLocaltime((const time_t *)&start_time, localtime_buf);
                ...
                if (RE_DAILY(re)->dd_ntime) {
                        start_tm->tm_hour =
                            RE_DAILY(re)->dd_time[RE_DAILY(re)->dd_ntime - 1] /
                            100;
                        start_tm->tm_min =
                            RE_DAILY(re)->dd_time[RE_DAILY(re)->dd_ntime - 1] %
                            100;
...
static Tick DoWeek(const Tick start_time, RepeatEvent *re) {
        struct tm *start_tm;
        unsigned int wd_ndaytime = RE_WEEKLY(re)->wd_ndaytime, dt_ntime,
                     start_hour, start_min;
        Tick _start_time = start_time, last_time1 = EOT, last_time2 = EOT;
        ...
        if (wd_ndaytime) {
                ...
                start_tm =
                    _XLocaltime((const time_t *)&_start_time, localtime_buf);
...
static Tick DoMonthDay(const Tick start_time, const RepeatEvent *re) {
        struct tm start_tm, *cur_tm;
        unsigned int md_nitems = RE_MONTHLY(re)->md_nitems,
                     *md_days = RE_MONTHLY(re)->md_days, interval = 1;
        Tick cur_time, last_time1 = EOT, last_time2 = EOT;
        _Xltimeparams localtime_buf;
        ...
        if (md_days[0] > 28) {
                ...
                do {
                        ...
                        if (DayExists(DayOfMonth(md_days[0], cur_tm->tm_mon,
                                                 cur_tm->tm_year),
                ...
                start_tm =
                    *_XLocaltime((const time_t *)&cur_time, localtime_buf);
                /* Get the right day (LASTDAY converted to a real day) */
                start_tm.tm_isdst = -1;
                start_tm.tm_mday = DayOfMonth(
                    md_days[md_nitems - 1], start_tm.tm_mon, start_tm.tm_year);
        } else
                start_tm.tm_mon += (re->re_duration - 1) * re->re_interval;
...
static Tick DoMonthPos(const Tick _start_time, RepeatEvent *re) {
        struct tm start_tm, cur_tm;
        Tick last_time1 = EOT, last_time2 = EOT, start_time = _start_time;
        unsigned int nwdt_list = RE_MONTHLY(re)->md_nitems, num_intervals = 1,
                     i, j, brute_force = TRUE;
...
static Tick DoYearByMonth(const Tick start_time, RepeatEvent *re) {
        struct tm *start_tm;
        int start_day;
        Tick _start_time, last_time1 = EOT, last_time2 = EOT;
        RepeatEventState *res;
        _Xltimeparams localtime_buf;
...
static Tick DoYearByDay(const Tick start_time, RepeatEvent *re) {
        struct tm *start_tm;
        int start_day;
        Tick _start_time, last_time1 = EOT, last_time2 = EOT;
        RepeatEventState *res;
 ...
 */
static int LastDayExists(const struct tm *cur_tm, const unsigned int md_nitems,
 ...
 * time in the month.
 */
static Tick LastOccurence(const Tick cur_time, const WeekDayTime *wdt_list,
                          const unsigned int nwdt_list) {
        int i, j, k;
        Tick oldest_time = 0, current_time;

        for (i = 0; i < nwdt_list; i++) {
                for (j = 0; j < wdt_list[i].wdt_nweek; j++) {
                        for (k = 0; k < wdt_list[i].wdt_nday; k++) {
                                if ((current_time = WeekNumberToDay(
                                         cur_time, wdt_list[i].wdt_week[j],
 ...
 */
static Tick LastTickFromEndDate(const Tick cur_time, const RepeatEvent *re) {
        RepeatEventState *res;
        RepeatEvent *_re = (RepeatEvent *)re;
        Tick end_date = re->re_end_date, last_time;
