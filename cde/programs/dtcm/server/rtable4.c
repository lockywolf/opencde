...
static Table_Res_4 *table_lookup_next(Table_Args_4 *args, struct svc_req *svcrq,
                                      caddr_t (*rb_func)(),
                                      Appt_4 *(*rp_func)());
 ...
 */
extern Table_Res_4 *_DtCm_rtable_lookup_4_svc(Table_Args_4 *args,
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                while (p_keys != NULL) {
                        ...
                        if ((stat = _DtCmsGetEntryAttrByKey(
                                 cal, user, access, cmskey, 0, NULL, &entries,
                                 NULL)) == CSA_SUCCESS) {
        ...
        while (p_keys != NULL) {
                ...
                if (p_appt == NULL) {
                        if ((p_appt = (Appt_4 *)hc_lookup(
                                 REPT_LIST(cal), (caddr_t)key)) != NULL) {
 ...
 */
extern Table_Res_4 *
_DtCm_rtable_lookup_next_larger_4_svc(Table_Args_4 *args,
                                      struct svc_req *svcrq) {
        Table_Res_4 *res;

        if (debug)
                fprintf(stderr,
                        "_DtCm_rtable_lookup_next_larger_4_svc called\n");
 ...
 */
extern Table_Res_4 *
_DtCm_rtable_lookup_next_smaller_4_svc(Table_Args_4 *args,
                                       struct svc_req *svcrq) {
        Table_Res_4 *res;

        if (debug)
                fprintf(stderr,
                        "_DtCm_rtable_lookup_next_smaller_4_svc called\n");
 ...
 * supports both data format
 */
extern Table_Res_4 *_DtCm_rtable_lookup_range_4_svc(Table_Args_4 *args,
                                                    struct svc_req *svcrq) {
        static Table_Res_4 res;
        CSA_return_code stat;
        _DtCmsCalendar *cal;
 ...
 */
extern Table_Res_4 *
_DtCm_rtable_abbreviated_lookup_range_4_svc(Table_Args_4 *args,
                                            struct svc_req *svcrq) {
        static Table_Res_4 res;
        CSA_return_code stat;
        _DtCmsCalendar *cal;
...
extern Table_Res_4 *_DtCm_rtable_insert_4_svc(Table_Args_4 *args,
                                              struct svc_req *svcrq) {
        static Table_Res_4 res;
        _DtCmsCalendar *cal;
        CSA_return_code stat;
        ...
        while (appt != NULL) {
                ...
                if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                        ...
                        _DtCm_free_cms_attribute_value(
                         ...
                         */
                        if ((stat = _DtCmsInsertApptAndLog(cal, a)) !=
                            CSA_SUCCESS) {
                                res.status = csastat2accessstat(stat);
                                goto insert_error;
                        }
        ...
        for (appt = ap; appt != NULL; appt = appt->next) {
                cal->rlist =
                    _DtCmsDoInsertEntryCallback(cal->rlist, cal->calendar, user,
...
insert_error:
        if (prev != NULL) {
                ...
                /* do callback */
                cal->rlist =
                    _DtCmsDoV1Callback(cal->rlist, user, args->pid, ap);
                for (appt = ap; appt != NULL; appt = appt->next) {
                        cal->rlist = _DtCmsDoInsertEntryCallback(
                            cal->rlist, cal->calendar, user, appt->appt_id.key,
                            args->pid);
                }
...
extern Table_Res_4 *_DtCm_rtable_delete_4_svc(Table_Args_4 *args,
                                              struct svc_req *svcrq) {
        static Table_Res_4 res;
        _DtCmsCalendar *cal;
        CSA_return_code stat;
        ...
        while (p_keys != NULL) {
                ...
                if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                        ...
                        if (p_keys->option == do_all_4)
                                stat = _DtCmsDeleteEntryAndLog(
                                    cal, user, access, &key, &entry);
                        else
                                stat = _DtCmsDeleteInstancesAndLog(
                                    cal, user, access, &key,
                        ...
                        if (p_keys->option == do_all_4)
                                stat = _DtCmsDeleteApptAndLog(
                                    cal, user, access, &p_keys->appt_id, &a);
                        else {
                                stat = _DtCmsDeleteApptInstancesAndLog(
                                    cal, user, access, &p_keys->appt_id,
        ...
        if (h != NULL) {
                ...
                for (a = h, p_keys = args->args.Args_4_u.uidopt; a != NULL;
                     a = a->next) {
                        while (a->appt_id.key != p_keys->appt_id.key)
                                p_keys = p_keys->next;

                        cal->rlist = _DtCmsDoDeleteEntryCallback(
                            cal->rlist, cal->calendar, user, a->appt_id.key,
                            p_keys->option,
                            (p_keys->option == do_all_4 ? a->appt_id.tick
                                                        : p_keys->appt_id.tick),
                            args->pid);
                }
        }
...
extern Table_Res_4 *_DtCm_rtable_delete_instance_4_svc(Table_Args_4 *args,
                                                       struct svc_req *svcrq) {
        static Table_Res_4 res;
...
extern Table_Res_4 *_DtCm_rtable_change_4_svc(Table_Args_4 *args,
                                              struct svc_req *svcrq) {
        static Table_Res_4 res;
        _DtCmsCalendar *cal;
        CSA_return_code stat;
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                ...
                if ((stat = _DtCmsAppt4ToCmsentry(args->target, newa, &entry,
                        ...
                        _DtCm_free_cms_attributes(
                            1, &entry->attrs[CSA_ENTRY_ATTR_ORGANIZER_I]);
                        _DtCm_free_cms_attributes(
                            1, &entry->attrs
                                    [CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I]);
                        _DtCm_free_cms_attributes(
                            1, &entry->attrs[CSA_ENTRY_ATTR_TYPE_I]);

                        /* update entry */
                        if (option == do_all_4)
                                stat = _DtCmsUpdateEntry(
                                    cal, user, access, &key, entry->num_attrs,
                                    &entry->attrs[1], &oldentry, NULL);
                        else
                                stat = _DtCmsUpdateInstances(
                                    cal, user, access, &key,
                                    (option == do_one_4 ? CSA_SCOPE_ONE
                                                        : CSA_SCOPE_FORWARD),
        ...
        if (stat == CSA_SUCCESS) {
                ...
                cal->rlist = _DtCmsDoUpdateEntryCallback(
                    ...
                    (newa->appt_id.key == olda->appt_id.key
                         ? 0
                         : newa->appt_id.key),
                    olda->appt_id.key, option,
                    (option == do_all_4 ? olda->appt_id.tick : p_key->tick),
                    args->pid);
...
extern Table_Res_4 *_DtCm_rtable_change_instance_4_svc(Table_Args_4 *args,
                                                       struct svc_req *svcrq) {
        static Table_Res_4 res;
...
extern Table_Res_4 *
_DtCm_rtable_lookup_next_reminder_4_svc(Table_Args_4 *args,
                                        struct svc_req *svcrq) {
        static Table_Res_4 res;
        CSA_return_code stat;
        char *user;
        ...
        if (debug)
                fprintf(stderr,
                        "_DtCm_rtable_lookup_next_reminder_4_svc called\n");
        ...
        if (cal->fversion > 1) {
                if ((stat = _DtCmsLookupReminder(cal->remq, tick, 0, NULL,
                                                 &rems)) == CSA_SUCCESS) {
                        stat = _DtCmsReminderRefToReminder(
                            rems, &res.res.Table_Res_List_4_u.r);
                        _DtCmsFreeReminderRef(rems);
                }
        } else {

                stat = _DtCmsGetV4Reminders(
                    cal, tick, &res.res.Table_Res_List_4_u.r, NULL);
        }
...
extern Table_Status_4 *_DtCm_rtable_check_4_svc(Table_Args_4 *args,
                                                struct svc_req *svcrq) {
...
extern Table_Status_4 *_DtCm_rtable_flush_table_4_svc(Table_Args_4 *args,
                                                      struct svc_req *svcrq) {
        static Table_Status_4 s;
        Id_4 key;
        CSA_return_code stat;
        ...
        while ((p_appt = (Appt_4 *)rb_lookup_next_larger(APPT_TREE(cal),
                                                         (caddr_t)&key))) {
                p_node =
                    rb_delete(APPT_TREE(cal), (caddr_t) & (p_appt->appt_id));
                if (p_node != NULL) {
                        n++;
...
extern int *_DtCm_rtable_size_4_svc(Table_Args_4 *args, struct svc_req *svcrq) {
        /* must be static! */
        static int size;
        CSA_return_code stat;
        _DtCmsCalendar *cal;
...
_DtCm_register_callback_4_svc(Registration_4 *r, struct svc_req *svcrq) {
        ...
        if (_DtCmsGetRegistration(&(cal->rlist), source, r->prognum, r->versnum,
                ...
                if (debug) {
                        fprintf(stderr,
                                "%s requested registration on %s. registered "
                                "pid= %d\n",
                                source, r->target, r->pid);
                        _DtCmsListRegistration(cal->rlist, cal->calendar);
                }
...
_DtCm_deregister_callback_4_svc(Registration_4 *r, struct svc_req *svcrq) {
        ...
        while (p != NULL) {
                ...
                if ((strcmp(p->client, source) == 0) &&
                    (p->prognum == r->prognum) && (p->versnum == r->versnum) &&
                    (p->procnum == r->procnum) &&
                    (p->pid == r->pid)) { /* a match */
                        if (debug) {
                                fprintf(stderr,
                                        "%s requested deregistration on %s. "
                                        "registered pid= %d\n",
                                        source, r->target, r->pid);
                        }
                        if (p == q)
...
extern Access_Status_4 *_DtCm_rtable_set_access_4_svc(Access_Args_4 *args,
                                                      struct svc_req *svcrq) {
        /* must be static! */
        ...
        if ((stat = _DtCmsV4LoadAndCheckAccess(svcrq, args->target, &user,
                ...
                if ((cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION &&
                     !(access &
                       (CSA_OWNER_RIGHTS | CSA_CHANGE_CALENDAR_ATTRIBUTES))) ||
                    (cal->fversion < _DtCM_FIRST_EXTENSIBLE_DATA_VERSION &&
        ...
        if ((s = csastat2accessstat(stat)) == access_ok_4) {
                ...
                attr.name.name = CSA_CAL_ATTR_ACCESS_LIST;
                cal->rlist = _DtCmsDoUpdateCalAttrsCallback(
                    cal->rlist, cal->calendar, user, 1, &attr, -1);
        }

        return (&s);
}

extern Access_Args_4 *_DtCm_rtable_get_access_4_svc(Access_Args_4 *args,
                                                    struct svc_req *svcrq) {
        ...
        if (cal->fversion > 1) {
                if (useronly) {
                        ...
                        aentry.next = NULL;
                        res.access_list =
                            _DtCmsConvertV5AccessList(&aentry, B_TRUE);
                } else {
...
extern Table_Res_4 *
_DtCm_rtable_abbreviated_lookup_key_range_4_svc(Table_Args_4 *args,
                                                struct svc_req *svcrq) {
        static Table_Res_4 res;
        CSA_return_code stat;
        _DtCmsCalendar *cal;
        ...
        while (p_range != NULL) {
                ...
                if ((stat = _DtCmsLookupKeyrangeV4(
                         cal, user, access, B_FALSE, B_TRUE, p_range->tick1,
                         p_range->tick2, 0, 0, p_range->key, NULL, 0, NULL,
                         NULL, NULL, &abbr_r)) != CSA_SUCCESS) {
                        break;
                }
...
extern long *_DtCm_rtable_gmtoff_4_svc(void *args, struct svc_req *svcrq) {
        static long gmtoff;
#if !defined(CSRG_BASED)
...
extern Table_Status_4 *_DtCm_rtable_create_4_svc(Table_Op_Args_4 *args,
                                                 struct svc_req *svcrq) {
        ...
        if ((ptr = strchr(source, '.'))) {
                if (debug)
                        fprintf(
                            stderr, "rpc.cmsd: %s %s(target) and %s(sender)\n",
                            "check domains, comparing", args->target, source);
...
extern Table_Status_4 *_DtCm_rtable_remove_4_svc(Table_Op_Args_4 *args,
                                                 struct svc_req *svcrq) {
        static Table_Status_4 res;
...
extern Table_Status_4 *_DtCm_rtable_rename_4_svc(Table_Op_Args_4 *args,
                                                 struct svc_req *svcrq) {
        static Table_Status_4 res;
...
extern void *_DtCm_rtable_ping_4_svc(void *args, struct svc_req *svcrq) {
        if (debug)
...
static Appt_4 *rtable_lookup_internal(_DtCmsCalendar *cal, char **p_src,
        ...
        if (p_appt != NULL) {
                if ((p_level = _DtCmCheckPrivacyLevel(p_src, p_appt)) !=
                    private_4) {
                        if (_DtCms_in_repeater(key, p_appt, B_FALSE)) {
                                if (p_level == public_4)
                                        p_appt = _DtCm_copy_one_appt4(p_appt);
                                else
                                        p_appt = _DtCm_copy_semiprivate_appt4(
                                            p_appt);
        ...
        while (p_lnode != NULL) {
                ...
                if (p_lnode->lasttick == 0)
                        p_lnode->lasttick = _DtCms_last_tick_v4(
                            APPT_TICK(p_appt), p_appt->period, p_appt->ntimes);
                ...
                while (++ord <= ntimes) {
                        if ((id.tick < key->tick) ||
                            (id.tick == key->tick && id.key <= key->key))
        ...
        while (p_lnode != NULL) {
                ...
                while ((++ord <= ntimes) && (id.tick <= key->tick)) {
                        if (id.tick == key->tick && id.key >= key->key)
                                ...
                                if (!_DtCms_marked_4_cancellation(p_appt,
                                                                  ord)) {
