...
extern CSA_return_code _DtCmsDeleteEntry(_DtCmsCalendar *cal, char *sender,
        ...
        if (lnode == NULL) {
                if ((tnode = rb_delete(cal->tree, (caddr_t)key)) != NULL) {
                        _DtCmsObsoleteReminder4Entry(cal->remq, entry, NULL, 0,
                                                     B_FALSE);
                        free(tnode);
                } else
                        return (CSA_X_DT_E_ENTRY_NOT_FOUND);
        } else {
                _DtCmsObsoleteReminder4Entry(cal->remq, entry, lnode, 0,
                                             B_FALSE);
...
extern CSA_return_code _DtCmsDeleteEntryAndLog(_DtCmsCalendar *cal,
...
extern CSA_return_code _DtCmsDeleteInstancesAndLog(_DtCmsCalendar *cal,
                                                   char *sender, uint access,
                                                   cms_key *key, int scope,
                                                   cms_entry **newe,
                                                   cms_entry **olde) {
        CSA_return_code stat;
        ...
        if (scope == CSA_SCOPE_ONE)
                ...
                if (key->time == nentry->key.time)
                        ...
                        stat = _DtCmsAddEndDateToRule(
                            &nentry->attrs[CSA_ENTRY_ATTR_RECURRENCE_RULE_I],
                            lnode->re, key->time - 1);
        ...
        if (!delentry) {
                ...
                count = CountEvents(
                    nentry->key.time, lnode->re,
                    (aptr->value ? aptr->value->item.date_time_list_value
                                 : NULL));
        }
        ...
        if (count == 0 || delentry) {
                ...
                if (count == 1) {
                        ...
                        stat = _DtCmsRbToCsaStat(
                            rb_insert(cal->tree, (caddr_t)nentry,
                                      (caddr_t) & (nentry->key)));
                ...
                if (stat || (stat = _DtCmsV5TransactLog(
                                 cal, nentry, _DtCmsLogAdd)) != CSA_SUCCESS) {
                        ...
                        if (count == 1) {
                                ...
                                hc_delete_node(cal->list, lnode);
                                free(lnode);
                                _DtCmsAddReminders4Entry(&cal->remq, nentry,
                                                         NULL);
                        } else {
                        ...
                        if (olde)
                                *olde = (count == 1) ? entry : nentry;
                                ...
                                _DtCm_free_cms_entry((count == 1) ? entry
                                                                  : nentry);
                }
        }
...
static CSA_return_code _AddException(cms_attribute *attr, time_t time) {
        ...
        if ((dt = (CSA_date_time_entry *)calloc(
        ...
        if (attr->value->item.date_time_list_value == NULL) {
                ...
                for (dlist = attr->value->item.date_time_list_value,
                    prev = NULL;
                     dlist != NULL; prev = dlist, dlist = dlist->next) {
...
static void _TruncateExceptionDates(cms_entry *newe, time_t ltick) {
        ...
        head = newe->attrs[CSA_ENTRY_ATTR_EXCEPTION_DATES_I]
