...
typedef void (*Action_proc)(/* Private *private; Tree_node *y, *z, *root */);

static Tree_node *balance(Tree_node *gg, Tree_node *g, Tree_node *f,
                          Tree_node *x) {
        Tree_node *t;
        Color tc;
        if (gg == NULL || g == NULL)
...
static void doit(Rb_tree *tree, Action_proc proc) {
        Private *private;
        Tree_node *root;
...
extern Rb_tree *rb_create(_DtCmsGetKeyProc get, _DtCmsCompareProc compare) {
        Private *p;
        Tree_node *root, *y, *z;
        Rb_tree *tree;
...
extern void rb_destroy(Rb_tree *tree, _DtCmsEnumerateProc destroy) {
        Private *p = NULL;
        caddr_t data = NULL;
        Tree_node *node = NULL;
        caddr_t key;
...
/* ARGSUSED */
static void size_callback(Private *private, Tree_node *y, Tree_node *z,
                          Tree_node *root) {
        /* dummy proc for monitor */
}

extern int rb_size(Rb_tree *tree) {
        Private *p;
        if (tree == NULL)
                return (0);
        p = (Private *)tree->private;
...
/* ARGSUSED */
static void insert_callback(Private *private, Tree_node *y, Tree_node *z,
                            Tree_node *root) {
...
extern Rb_Status rb_insert_node(Rb_tree *tree, Tree_node *node, caddr_t key) {
        Private *private;
...
extern Rb_Status rb_insert(Rb_tree *tree, caddr_t data, caddr_t key) {
        Tree_node *node;
...
static void delete_callback(Private *private, Tree_node *y, Tree_node *z,
                            Tree_node *root) {
...
extern Tree_node *rb_delete(Rb_tree *tree, caddr_t key) {
        Private *p;
        if (tree == NULL)
...
/* ARGSUSED */
static void lookup_callback(Private *private, Tree_node *y, Tree_node *z,
                            Tree_node *root) {
        _DtCmsComparisonResult c;
...
extern caddr_t rb_lookup(Rb_tree *tree, caddr_t key) {
        Private *private;
        if (tree == NULL)
                return ((caddr_t)NULL);
      private
      ...
      private
        ->key = key;
      private
        ->data = NULL; /* might have been previously used */
        doit(tree, lookup_callback);
        return (private->data);
}

/* ARGSUSED */
static void lookup_smallest_callback(Private *private, Tree_node *y,
                                     Tree_node *z, Tree_node *root) {
...
extern caddr_t rb_lookup_smallest(Rb_tree *tree) {
        Private *private;
        if (tree == NULL)
                return ((caddr_t)NULL);
      private
...
/* ARGSUSED */
static void next_larger_callback(Private *private, Tree_node *y, Tree_node *z,
                                 Tree_node *root) {
...
extern caddr_t rb_lookup_next_larger(Rb_tree *tree, caddr_t key) {
        Private *private;
        if (tree == NULL)
                return ((caddr_t)NULL);
      private
...
/* ARGSUSED */
static void lookup_largest_callback(Private *private, Tree_node *y,
                                    Tree_node *z, Tree_node *root) {
...
extern caddr_t rb_lookup_largest(Rb_tree *tree) {
        Private *private;
...
/* ARGSUSED */
static void next_smaller_callback(Private *private, Tree_node *y, Tree_node *z,
                                  Tree_node *root) {

        Tree_node *smaller = NULL;
        Tree_node *x = root->rlink;
        while (x != z) {
                if (private->compare(private->key, x->data) ==
                    _DtCmsIsGreater) {
                        smaller = x;
                        x = x->rlink;
                } else
...
extern caddr_t rb_lookup_next_smaller(Rb_tree *tree, caddr_t key) {
        Private *private;
        if (tree == NULL)
                return ((caddr_t)NULL);
      private
...
static boolean_t visit_subtree(Tree_node *node, Private *p, Tree_node *z,
                               Direction dir) {
        Tree_node *link;
        link = (dir == up) ? node->llink : node->rlink;
        if (link != z && visit_subtree(link, p, z, dir))
                return (B_TRUE);
        if (p->enumerate((caddr_t)node, node->data))
                return (B_TRUE);
        link = (dir == up) ? node->rlink : node->llink;
        if (link != z)
                return (visit_subtree(link, p, z, dir));
        else
                return (B_FALSE);
}

/* ARGSUSED */
static boolean_t enumerate_up_callback(Private *private, Tree_node *y,
                                       Tree_node *z, Tree_node *root) {
        if (root == NULL || root->rlink == z)
                return (B_FALSE);
        return (visit_subtree(root->rlink, private, z, up));
}

extern Rb_Status rb_enumerate_up(Rb_tree *tree, _DtCmsEnumerateProc proc) {
        Private *private;
        if (tree == NULL)
                return (rb_badtable);
      private
...
/* ARGSUSED */
static void enumerate_down_callback(Private *private, Tree_node *y,
                                    Tree_node *z, Tree_node *root) {
        if (root == NULL || root->rlink == z)
                return;
...
extern void rb_enumerate_down(Rb_tree *tree, _DtCmsEnumerateProc proc) {
        Private *private;
        if (tree == NULL)
                return;
      private
...
static int assert(int p) { return (p); }
...
static void check1(Tree_node *x, caddr_t max, Tree_node *z, Rec *rec,
                   Private *private) {
        int dl, dr;
        boolean_t redchild;
        Rec localrec;
...
static void check_tree_callback(Private *private, Tree_node *y, Tree_node *z,
                                Tree_node *root) {
        if (!assert(z->llink == y)) {
              private
                ->status = rb_badtable;
        ...
        if (root->rlink != z) {
                ...
                check1(root->rlink, private->get(smallest->data), z, localp,
                       private);
                if (private->status == rb_badtable)
                        return;
        }
...
extern Rb_Status rb_check_tree(Rb_tree *tree) {
        Private *p;
        if (tree == NULL)
                return (rb_notable);
        p = (Private *)tree->private;
