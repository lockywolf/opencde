...
static int MonthDayNumIntervals(struct tm *, struct tm *, const RepeatEvent *,
                                const unsigned int *, struct tm *);
static int MonthPosNumIntervals(struct tm *, struct tm *, const RepeatEvent *,
                                const WeekDayTime *, const unsigned int,
                                struct tm *);
 ...
 * recurrence rule.
 */
Tick ClosestTick(const Tick _target_time, const Tick start_time,
                 RepeatEvent *re, RepeatEventState **res) {
        Tick closest_tick, real_start_time, target_time = _target_time;
        ...
        case RT_MONTHLY_POSITION:
                ...
                if (target_time == real_start_time) {
                        ...
                        closest_tick =
                            DoMonthPos(target_time, real_start_time, re, *res);
                break;
        case RT_MONTHLY_DAY:
 ...
 * Example M60 #4
 */
static Tick DoMinute(const Tick target_time, const Tick start_time,
                     const RepeatEvent *re, RepeatEventState *res) {
        int delta_seconds;
        int num_intervals;
        Tick closest_tick;
 ...
 * Example: D2 #4
 */
static Tick DoDay(const Tick target_time, const Tick start_time,
                  const RepeatEvent *re, RepeatEventState *res) {
        int delta_seconds, num_intervals, dst_adj, daysec = 60 * 60 * 24, i;
        struct tm target_tm, start_tm, base_tm;
        unsigned int ntime = RE_DAILY(re)->dd_ntime;
        Tick base_time, next_time = 0;
        ...
        for (i = 0; i < ntime; i++) {
                ...
                if (next_time >= target_time) {
                        res->res_duration =
                            re->re_duration - (num_intervals + 1);
 ...
 */
static Tick DoWeek(const Tick _target_time, const Tick _start_time,
                   const RepeatEvent *re, RepeatEventState *res) {
        int delta_seconds, num_intervals, dst_adj, appt_time,
            daysec = 60 * 60 * 24, wksec = daysec * 7;
        unsigned int ntime = RE_WEEKLY(re)->wd_ndaytime;
        struct tm target_tm, start_tm, base_tm;
        Tick target_time = _target_time, start_time = _start_time, base_time,
             begin_time, adj_start_time, next_time = 0;
 ...
 */
static Tick DoMonthDay(const Tick target_time, const Tick start_time,
                       const RepeatEvent *re, RepeatEventState *res) {
        int num_intervals, event_day, nmonths;
        ...
        num_intervals =
            MonthDayNumIntervals(&start_tm, &target_tm, re, day_list, &base_tm);
        ...
        base_tm.tm_mday =
            DayOfMonth(event_day, base_tm.tm_mon, base_tm.tm_year);
        base_tm.tm_isdst = -1;
        next_time = mktime(&base_tm);
 ...
 */
static Tick DoMonthPos(const Tick target_time, const Tick start_time,
                       const RepeatEvent *re, RepeatEventState *res) {
        int num_intervals, nmonths;
        unsigned int ndays = RE_MONTHLY(re)->md_nitems;
        struct tm target_tm, start_tm, base_tm;
        Tick base_time = 0;
        ...
        do {
                ...
                if (SAME_MONTH(&target_tm, &base_tm)) {
                        base_time = mktime(&base_tm);
                        base_tm = *_XLocaltime((const time_t *)&base_time,
                                               localtime_buf);
                        base_time =
                            WeekNumberToDay(base_time, wdt_list[0].wdt_week[0],
                ...
                base_time = mktime(&base_tm);
                base_tm =
                    *_XLocaltime((const time_t *)&base_time, localtime_buf);
                base_time = WeekNumberToDay(base_time, wdt_list[0].wdt_week[0],
                                            wdt_list[0].wdt_day[0]);
        } while (!base_time);
 ...
 * Example: YM1 2 5 9 #4
 */
static Tick DoYearByMonth(const Tick _target_time, const Tick _start_time,
                          const RepeatEvent *re, RepeatEventState *res) {
        int num_intervals, nyears;
        unsigned int nitems = RE_YEARLY(re)->yd_nitems;
        struct tm target_tm, start_tm, base_tm;
        Tick base_time = 0, start_time = _start_time,
        ...
        if (base_tm.tm_year == target_tm.tm_year) {
                ...
                for (i = 0; i < nitems; i++) {
                        ...
                        if (month_list[i] - 1 == target_tm.tm_mon) {
                                ...
                                base_time = mktime(&base_tm);
                                base_tm =
                                    *_XLocaltime(&base_time, localtime_buf);
 ...
 */
static Tick DoYearByDay(const Tick _target_time, const Tick _start_time,
                        const RepeatEvent *re, RepeatEventState *res) {
        int num_intervals, nyears;
        unsigned int nitems = RE_YEARLY(re)->yd_nitems;
        struct tm target_tm, start_tm, base_tm;
        Tick base_time = 0, target_time = _target_time,
...
static int GetMonthDiff(const struct tm *start_tm, const struct tm *end_tm) {
        return ((end_tm->tm_year - start_tm->tm_year + 1) * 12 -
                (start_tm->tm_mon + 1) - (12 - (end_tm->tm_mon + 1)));
}

static Tick DSTAdjustment(const struct tm *tm1, const struct tm *tm2) {
...
static void DoDSTAdjustment(const Tick begin_time,
                            struct tm *end_time) /* Return */
{
        struct tm next_day;
        Tick dst_adj, _begin_time = begin_time;
        _Xltimeparams localtime_buf;
 ...
 */
static RepeatEventState *InitRepeatEventState(const RepeatEvent *re) {
 ...
 * target_tm.
 */
static int MonthDayNumIntervals(struct tm *start_tm, struct tm *target_tm,
                                ...
                                struct tm *base_tm) /* Return */
{
        int num_intervals = 0;
        struct tm cur_tm;
        Tick cur_time, base_time, last_time, target_time;
        _Xltimeparams localtime_buf;
        ...
        if (md_days[0] > 28) {
                *base_tm = *start_tm;
                ...
                while (cur_time < target_time) {
                        ...
                        cur_tm.tm_isdst = -1;
                        cur_time = mktime(&cur_tm);
                        cur_tm = *_XLocaltime((const time_t *)&cur_time,
                                              localtime_buf);
                        ...
                        if (SAME_MONTH(target_tm, &cur_tm))
                                break;
                }
        } else {
                num_intervals =
                    GetMonthDiff(start_tm, target_tm) / re->re_interval;
                *base_tm = *start_tm;
                base_tm->tm_isdst = -1;
 ...
 */
static int MonthPosNumIntervals(struct tm *start_tm, struct tm *target_tm,
                                const RepeatEvent *re,
                                const WeekDayTime *wdt_list,
                                const unsigned int nwdt_list,
                                struct tm *base_tm) /* Return */
{
        int num_intervals = 0, brute_force = TRUE, i, j;
        struct tm cur_tm;
        Tick cur_time, base_time, target_time;
        _Xltimeparams localtime_buf;
...
void FillInRepeatEvent(const Tick start_time, RepeatEvent *re) {
        struct tm *start_tm;
        int i;
        _Xltimeparams localtime_buf;
        ...
        case RT_DAILY:
                if (!RE_DAILY(re)->dd_ntime) {
                        ...
                        RE_DAILY(re)->dd_time[0] =
                            start_tm->tm_hour * 100 + start_tm->tm_min;
