 ...
 */
static _DtCmsRegistrationInfo *_DtCmsDoCallback(_DtCmsRegistrationInfo *rlist,
                                                uint type, char *source,
                                                int pid, int version,
                                                void *args);
...
_DtCmsMakeRegistrationInfo(char *client, int types, u_long prognum,
                           u_long versnum, u_long procnum, int pid) {
        _DtCmsRegistrationInfo *rinfo;

        if ((rinfo = (_DtCmsRegistrationInfo *)calloc(
                 1, sizeof(_DtCmsRegistrationInfo))) == NULL)
                return (NULL);
...
extern void _DtCmsFreeRegistrationInfo(_DtCmsRegistrationInfo *rinfo) {
        if (rinfo == NULL)
                return;
 ...
 */
extern _DtCmsRegistrationInfo *
_DtCmsGetRegistration(_DtCmsRegistrationInfo **_rlist, char *client,
                      u_long prognum, u_long versnum, u_long procnum, int pid) {
        _DtCmsRegistrationInfo *rlist = *_rlist, *prev, *ptr, *tmp_ptr;

        for (prev = ptr = rlist; ptr != NULL;) {
                if ((strcmp(ptr->client, client) == 0) &&
                    (ptr->prognum == prognum) && (ptr->versnum == versnum) &&
                    (ptr->procnum == procnum)) {
                        if (ptr->pid == pid) {
                                *_rlist = rlist;
                                return (ptr);
...
_DtCmsCheckRegistrationList(_DtCmsRegistrationInfo *rlist) {
        ...
        while (rlist != NULL) {
                ...
                if (debug) {
                        fprintf(stderr,
                                "%s: pinging %s on prog: %ld, vers: %ld, proc: "
                                "%ld\n",
                                pgname, rlist->client, rlist->prognum,
                                rlist->versnum, rlist->procnum);
                }
                ...
                if (cl == NULL ||
                    clnt_call(cl, 0, (xdrproc_t)xdr_void, (char *)NULL,
...
extern _DtCmsRegistrationInfo *
_DtCmsDoV1CbForV4Data(_DtCmsRegistrationInfo *rlist, char *source, int pid,
                      cms_key *key1, cms_key *key2) {
        Appt_4 appt1, appt2;
 ...
 * protocol.
 */
extern _DtCmsRegistrationInfo *_DtCmsDoV1Callback(_DtCmsRegistrationInfo *rlist,
                                                  char *source, int pid,
                                                  Appt_4 *a) {
        Uid_4 *k, *ids = NULL;
        Table_Res_4 res;
        ...
        rlist =
            _DtCmsDoCallback(rlist, 0, source, pid, AGENTVERS, (void *)&res);
...
extern void _DtCmsListRegistration(_DtCmsRegistrationInfo *rlist, char *cal) {
        int n = 0;
...
extern _DtCmsRegistrationInfo *
_DtCmsRemoveRegistration(_DtCmsRegistrationInfo *rlist,
                         _DtCmsRegistrationInfo *rinfo) {
...
extern _DtCmsRegistrationInfo *
_DtCmsDoOpenCalCallback(_DtCmsRegistrationInfo *rlist, char *cal, char *user,
                        int pid) {
...
extern _DtCmsRegistrationInfo *
_DtCmsDoRemoveCalCallback(_DtCmsRegistrationInfo *rlist, char *cal, char *user,
                          int pid) {
        cmcb_update_callback_args args;
        char calendar[BUFSIZ];
...
extern _DtCmsRegistrationInfo *
_DtCmsDoUpdateCalAttrsCallback(_DtCmsRegistrationInfo *rlist, char *cal,
                               char *user, uint num_attrs, cms_attribute *attrs,
                               int pid) {
        cmcb_update_callback_args args;
...
extern _DtCmsRegistrationInfo *
_DtCmsDoInsertEntryCallback(_DtCmsRegistrationInfo *rlist, char *cal,
                            char *source, long id, int pid) {
        cmcb_update_callback_args args;
...
extern _DtCmsRegistrationInfo *
_DtCmsDoDeleteEntryCallback(_DtCmsRegistrationInfo *rlist, char *cal,
                            char *source, long id, int scope, time_t time,
                            int pid) {
        cmcb_update_callback_args args;
...
extern _DtCmsRegistrationInfo *
_DtCmsDoUpdateEntryCallback(_DtCmsRegistrationInfo *rlist, char *cal,
                            char *source, long newid, long oldid, int scope,
                            long time, int pid) {
        cmcb_update_callback_args args;
...
static _DtCmsRegistrationInfo *_DtCmsDoCallback(_DtCmsRegistrationInfo *rlist,
        ...
        for (ptr = prev = rlist; ptr != NULL;) {
                ...
                if (version != ptr->versnum || (type && !(type & ptr->types)) ||
                ...
                if (debug) {
                        fprintf(stderr,
                                "%s: calling back %s on prog: %ld, vers: %ld, "
                                "proc: %ld\n",
