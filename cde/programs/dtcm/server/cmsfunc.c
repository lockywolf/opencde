...
extern void *cms_ping_5_svc(void *args, struct svc_req *svcrq) {
        if (debug)
...
extern cms_list_calendars_res *cms_list_calendars_5_svc(void *dummy,
...
extern cms_open_res *cms_open_calendar_5_svc(cms_open_args *args,
                                             struct svc_req *svcrq) {
        ...
        if ((res.stat = _DtCmsV5LoadAndCheckAccess(svcrq, args->cal, &user,
            ...
            CSA_SUCCESS) {
                ...
                res.stat = _DtCmsGetAllCalAttrs(cal, &res.num_attrs, &res.attrs,
                                                B_FALSE);
        }

        if (res.stat == CSA_SUCCESS) {
                /* do callbacks */
                cal->rlist = _DtCmsDoOpenCalCallback(cal->rlist, cal->calendar,
                                                     user, args->pid);
        }
...
extern CSA_return_code *cms_create_calendar_5_svc(cms_create_args *args,
                                                  struct svc_req *svcrq) {
        ...
        if ((res = _DtCm_check_cal_cms_attributes(
                 _DtCMS_VERSION4, args->num_attrs, args->attrs, user, args->cal,
...
extern CSA_return_code *cms_remove_calendar_5_svc(cms_remove_args *args,
                                                  struct svc_req *svcrq) {
        static CSA_return_code res;
        _DtCmsCalendar *cal;
        char *user;
        ...
        if ((res = _DtCmsV5LoadAndCheckAccess(svcrq, args->cal, &user, &access,
                                              &cal)) != CSA_SUCCESS)
                return (&res);
        ...
        if (!(access & CSA_OWNER_RIGHTS)) {
                ...
                /* do callbacks */
                cal->rlist = _DtCmsDoRemoveCalCallback(
                    cal->rlist, cal->calendar, user, args->pid);
...
extern CSA_return_code *cms_register_5_svc(cms_register_args *args,
                                           struct svc_req *svcrq) {
        static CSA_return_code res;
        char *user;
        _DtCmsCalendar *cal;
        ...
        if ((rinfo = _DtCmsGetRegistration(&(cal->rlist), user, args->prognum,
                                           args->versnum, args->procnum,
                                           args->pid)) == NULL) {
                /* this client has not registered */

                if ((rinfo = _DtCmsMakeRegistrationInfo(
                         user, args->update_type, args->prognum, args->versnum,
...
extern CSA_return_code *cms_unregister_5_svc(cms_register_args *args,
                                             struct svc_req *svcrq) {
        static CSA_return_code res;
        char *user;
        _DtCmsCalendar *cal;
        ...
        if (cal == NULL ||
            (rinfo = _DtCmsGetRegistration(&(cal->rlist), user, args->prognum,
                                           args->versnum, args->procnum,
                                           args->pid)) == NULL) {
        ...
        if (debug) {
                fprintf(stderr,
                        "%s unregistered types %d on %s, new types = %d\n",
...
cms_enumerate_calendar_attr_5_svc(buffer *args, struct svc_req *svcrq) {
        ...
        if ((res.stat = _DtCmsV5LoadAndCheckAccess(svcrq, *args, &user, &access,
                                                   &cal)) != CSA_SUCCESS)
                return (&res);
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                ...
                res.stat =
                    _DtCmsGetCalAttrNames(cal, &res.num_names, &res.names);
        } else {
                res.stat =
                    _DtCmsGetOldCalAttrNames(cal, &res.num_names, &res.names);
        }
...
cms_get_calendar_attr_5_svc(cms_get_cal_attr_args *args,
                            struct svc_req *svcrq) {
        ...
        if ((res.stat = _DtCmsV5LoadAndCheckAccess(
                 svcrq, args->cal, &user, &access, &cal)) != CSA_SUCCESS)
                return (&res);
        ...
        if (args->num_names > 0)
                res.stat =
                    _DtCmsGetCalAttrsByName(cal, args->num_names, args->names,
...
extern CSA_return_code *cms_set_calendar_attr_5_svc(cms_set_cal_attr_args *args,
                                                    struct svc_req *svcrq) {
        static CSA_return_code res;
        _DtCmsCalendar *cal;
        char *user;
        ...
        if ((res = _DtCm_check_cal_cms_attributes(
                 (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION
                      ? cal->fversion
                      : _DtCM_FIRST_EXTENSIBLE_DATA_VERSION - 1),
                 args->num_attrs, args->attrs, NULL, NULL, B_TRUE, B_FALSE,
                 B_TRUE)) != CSA_SUCCESS)
                return (&res);

        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                if ((res = _DtCmsUpdateCalAttributesAndLog(
                         cal, args->num_attrs, args->attrs, access)) !=
                    CSA_SUCCESS) {
                        return (&res);
                }
        } else {
...
extern cms_archive_res *cms_archive_5_svc(cms_archive_args *args,
                                          struct svc_req *svcrq) {
...
extern CSA_return_code *cms_restore_5_svc(cms_restore_args *args,
                                          struct svc_req *svcrq) {
        static CSA_return_code res;
...
extern cms_reminder_res *cms_lookup_reminder_5_svc(cms_reminder_args *args,
                                                   struct svc_req *svcrq) {
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                res.stat =
                    _DtCmsLookupReminder(cal->remq, args->tick, args->num_names,
                                         args->names, &res.rems);
        } else {
                Reminder_4 *v4rem;
                _DtCmsEntryId *ids;
                ...
                if ((res.stat = _DtCmsGetV4Reminders(cal, time(0), &v4rem,
                                                     &ids)) == CSA_SUCCESS) {
                        res.stat = _DtCmsV4ReminderToReminderRef(
                            cal->calendar, v4rem, ids, &res.rems);
...
extern cms_entries_res *cms_lookup_entries_5_svc(cms_lookup_entries_args *args,
                                                 struct svc_req *svcrq) {
        ...
        /* check operator */
        if ((res.stat = _DtCm_check_operator(args->num_attrs, NULL, args->attrs,
                                             args->ops)) != CSA_SUCCESS)
                return (&res);
        ...
        if ((res.stat = _DtCmHashCriteria(
                 cal->fversion < _DtCM_FIRST_EXTENSIBLE_DATA_VERSION
                     ...
                     : cal->entry_tbl,
                 args->num_attrs, NULL, args->attrs, args->ops, &no_match,
                 &no_start_time_range, &no_end_time_range, &start1, &start2,
                 &end1, &end2, &id, &hnum, &hattrs, &hops)) ==
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                if (id > 0)
                        res.stat = _DtCmsLookupEntriesById(
                            cal, user, access, no_start_time_range,
                            no_end_time_range, start1, start2, end1, end2, id,
                            hnum, hattrs, hops, &res.entries);
                else
                        res.stat = _DtCmsLookupEntries(
                            cal, user, access, start1, start2,
                            no_end_time_range, end1, end2, hnum, hattrs, hops,
                            &res.entries);
        } else {
                Range_4 prange;
                Appt_4 *appt = NULL;
                ...
                if (id > 0) {
                        res.stat = _DtCmsLookupKeyrangeV4(
                            cal, user, access, no_start_time_range,
                            no_end_time_range, start1, start2, end1, end2, id,
                            _DtCm_match_one_appt, hnum, hattrs, hops, &appt,
                            NULL);
                } else {
                        prange.key1 = start1;
                        prange.key2 = start2;
                        prange.next = NULL;
                        res.stat = _DtCmsLookupRangeV4(
                            cal, user, access, &prange, no_end_time_range, end1,
                            end2, _DtCm_match_one_appt, hnum, hattrs, hops,
                            &appt, NULL);
                }

                if (res.stat == CSA_SUCCESS && appt) {
                        res.stat = _DtCmsAppt4ToCmsentriesForClient(
                            args->cal, appt, &res.entries);
...
extern cms_entries_res *cms_enumerate_sequence_5_svc(cms_enumerate_args *args,
                                                     struct svc_req *svcrq) {
        static cms_entries_res res;
        _DtCmsCalendar *cal;
        char *user;
        ...
        /* do lookup */
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                res.stat = _DtCmsEnumerateSequenceById(
                    cal, user, access, B_FALSE, B_TRUE, args->start, args->end,
                ...
                res.stat = _DtCmsLookupKeyrangeV4(
                    cal, user, access, B_FALSE, B_TRUE, args->start, args->end,
                    0, 0, args->id, NULL, 0, NULL, NULL, &appt, NULL);
...
cms_get_entry_attr_5_svc(cms_get_entry_attr_args *args, struct svc_req *svcrq) {
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                res.stat = _DtCmsLookupEntriesByKey(
                    cal, user, access, args->num_keys, args->keys,
                    args->num_names, args->names, &res.entries);
        } else {
                res.stat = CSA_E_NOT_SUPPORTED;
        }
...
extern cms_entry_res *cms_insert_entry_5_svc(cms_insert_args *args,
                                             struct svc_req *svcrq) {
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                ...
                /* set organizer */
                if ((res.stat = _DtCm_set_string_attrval(
                         user, &entry->attrs[CSA_ENTRY_ATTR_ORGANIZER_I].value,
                ...
                if ((res.stat = _DtCmsAttrsToAppt4(args->num_attrs, args->attrs,
                                                   appt, B_TRUE)) !=
                    CSA_SUCCESS) {
                        _DtCm_free_appt4(appt);
                        return (&res);
        ...
        /* do callback */
        cal->rlist =
            _DtCmsDoV1CbForV4Data(cal->rlist, user, args->pid, &key, NULL);
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION ||
            (cal->fversion < _DtCM_FIRST_EXTENSIBLE_DATA_VERSION &&
             (res.stat = _DtCmsAppt4ToCmsentriesForClient(
                  cal->calendar, appt, &entry)) == CSA_SUCCESS)) {

                res.stat =
                    _DtCmsGetCmsEntryForClient(entry, &res.entry, B_FALSE);
...
extern cms_entry_res *cms_update_entry_5_svc(cms_update_args *args,
                                             struct svc_req *svcrq) {
        static cms_entry_res res;
        _DtCmsCalendar *cal;
        cms_entry *newentry;
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                if (event != NULL || args->scope == CSA_SCOPE_ALL) {
                        res.stat = _DtCmsUpdateEntry(
                            cal, user, access, &args->entry, args->num_attrs,
                            args->attrs, NULL, &newentry);
                } else {
                        res.stat = _DtCmsUpdateInstances(
                            cal, user, access, &args->entry, args->scope,
                            args->num_attrs, args->attrs, NULL, &newentry);
                }
        } else {
                Options_4 opt;
                ...
                if (event != NULL || args->scope == CSA_SCOPE_ALL)
                        ...
                        opt = (args->scope == CSA_SCOPE_ONE) ? do_one_4
                                                             : do_forward_4;

                if (event == NULL &&
                    (event = hc_lookup(cal->list, (caddr_t)&args->entry)) ==
                        NULL) {
                        ...
                        if ((res.stat = _DtCmsAttrsToAppt4(
                                 args->num_attrs, args->attrs, appt, B_TRUE)) ==
                            CSA_SUCCESS) {

                                if (opt == do_all_4)
                                        res.stat = _DtCmsChangeAll(
                                            cal, user, access,
                                            (Id_4 *)&args->entry, appt, NULL);
                                else
                                        res.stat = _DtCmsChangeSome(
                                            cal, user, access,
                                            (Id_4 *)&args->entry, appt, opt,
                                            NULL);
                        }
                }
        }
        ...
        /* do callback */
        cal->rlist = _DtCmsDoV1CbForV4Data(
            cal->rlist, user, args->pid, &args->entry,
            (appt ? (cms_key *)&appt->appt_id : &newentry->key));

        cal->rlist = _DtCmsDoUpdateEntryCallback(
            cal->rlist, cal->calendar, user,
            (appt ? appt->appt_id.key : newentry->key.id), args->entry.id,
            args->scope, args->entry.time, args->pid);

        /* reply */
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION ||
            (cal->fversion < _DtCM_FIRST_EXTENSIBLE_DATA_VERSION &&
             (res.stat = _DtCmsAppt4ToCmsentriesForClient(
                  cal->calendar, appt, &newentry)) == CSA_SUCCESS)) {

                res.stat =
                    _DtCmsGetCmsEntryForClient(newentry, &res.entry, B_FALSE);
...
extern CSA_return_code *cms_delete_entry_5_svc(cms_delete_args *args,
                                               struct svc_req *svcrq) {
        static CSA_return_code res;
        _DtCmsCalendar *cal;
        caddr_t event;
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                if (event != NULL || args->scope == CSA_SCOPE_ALL) {
                        ...
                        res = _DtCmsDeleteInstancesAndLog(
                            cal, user, access, &args->entry, args->scope, NULL,
                            NULL);
                }
        } else {
                if (event != NULL || args->scope == CSA_SCOPE_ALL) {
                        res = _DtCmsDeleteApptAndLog(
                            cal, user, access, (Id_4 *)&args->entry, NULL);
                } else {
                        res = _DtCmsDeleteApptInstancesAndLog(
                            cal, user, access, (Id_4 *)&args->entry,
        ...
        cal->rlist = _DtCmsDoDeleteEntryCallback(
            cal->rlist, cal->calendar, user, args->entry.id, args->scope,
...
static CSA_return_code _DtCmsCreateCallog(char *user, cms_create_args *args,
        ...
        if ((stat = _DtCm_set_string_attrval(
                 datestr, &cal->attrs[CSA_CAL_ATTR_DATE_CREATED_I].value,
        ...
        if ((stat = _DtCmUpdateAccessListAttrVal(
        ...
        if ((stat = _DtCmUpdateAttributes(
                 args->num_attrs, args->attrs, &cal->num_attrs, &cal->attrs,
                 &cal->cal_tbl, B_TRUE, NULL, B_FALSE)) != CSA_SUCCESS) {
                _DtCmsFreeCalendar(cal);
                free(log);
                return (stat);
        }
        ...
        if (nidx)
                args->attrs[nidx].name.name = name;
        if (oidx)
                args->attrs[oidx].name.name = owner;
        ...
        /* dump file */
        if ((stat = _DtCmsAppendCalAttrsByFN(log, cal->num_attrs,
...
static CSA_return_code _DtCmsGetOldCalAttrNames(_DtCmsCalendar *cal,
        ...
        if ((names = (cms_attr_name *)calloc(
                 1, sizeof(cms_attr_name) * _DtCM_OLD_CAL_ATTR_SIZE)) == NULL)
                return (CSA_E_INSUFFICIENT_MEMORY);
        ...
        for (i = 1, j = 0; i <= _DtCM_DEFINED_CAL_ATTR_SIZE; i++) {
                if ((_CSA_cal_attr_info[i].fst_vers > 0 &&
                    ...
                    i == CSA_CAL_ATTR_CALENDAR_SIZE_I) {
                        if ((names[j].name = strdup(
                                 _CSA_calendar_attribute_names[i])) == NULL) {
