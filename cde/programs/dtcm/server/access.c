...
static Access_Entry_4 *combine_access_list(Access_Entry_4 *p_list,
                                           Access_Entry_4 *p_head, int type,
                                           int *p_world);
 ...
 */
extern CSA_return_code _DtCmsV4LoadAndCheckAccess(struct svc_req *svcrq,
                                                  char *target, char **sender,
                                                  uint *access,
                                                  _DtCmsCalendar **cal) {
 ...
 */
extern CSA_return_code _DtCmsV5LoadAndCheckAccess(struct svc_req *svcrq,
                                                  char *target, char **sender,
                                                  uint *access,
                                                  _DtCmsCalendar **cal) {
        CSA_return_code stat;
...
extern CSA_return_code _DtCmsGetClientInfo(struct svc_req *svcrq,
                                           char **source) {
        char *name;
        char *uname;
        ...
        case AUTH_UNIX:
                ...
                if ((uname = malloc(uname_size)) == NULL)
                        ...
                        snprintf(uname, uname_size, "%s@%s", name,
...
extern boolean_t _DtCmsIsFileOwner(char *owner, char *user, char *target) {
        ...
        if (debug) {
                fprintf(stderr, "rpc.cmsd: %s, %s = %s, %s = %s, %s = %s\n",
                        "check file owner", "owner",
                        ((owner == NULL) ? "NULL" : owner), "user",
                        ((user == NULL) ? "NULL" : user), "target",
 ...
 */
extern CSA_return_code _DtCmsCheckViewAccess(char *user, uint access,
                                             cms_entry *eptr) {
        uint need;
...
extern CSA_return_code _DtCmsCheckChangeAccess(char *user, uint access,
                                               cms_entry *eptr) {
        uint need;
        cms_attribute_value *oval, *sval;
...
static CSA_return_code _GetV5AccessRights(_DtCmsCalendar *cal, char *target,
        ...
        isowner =
            _DtCmsIsFileOwner(owner->item.calendar_user_value, sender, target);
        ...
        alist = cal->attrs[CSA_CAL_ATTR_ACCESS_LIST_I]
