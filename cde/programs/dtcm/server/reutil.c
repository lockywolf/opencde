 ...
 * duration.
 */
int InTimeRange(const unsigned int ninterval, const Duration duration) {
        if (duration == RE_INFINITY || duration == RE_NOTSET)
                return TRUE;
 ...
 */
int GetWDayDiff(const int start, const int end) {
        if (start <= end)
                return end - start;
 ...
 */
int DayExists(const int day, const int month, const int year) {
 ...
 */
int OccurenceExists(const WeekDayTime *wdt_list, const unsigned int nwdt_list,
                    const Tick date) {
        int i, j, k;
...
WeekNumber GetWeekNumber(const Tick date) {
 ...
 */
WeekNumber DayToWeekNumber(const Tick date) {
 ...
 */
Tick WeekNumberToDay(const Tick date, const WeekNumber week,
                     const WeekDay weekday) {
        _Xltimeparams localtime_buf;
        struct tm *date_tm = _XLocaltime(&date, localtime_buf);
        int first_weekday, last_weekday, day_of_month,
...
unsigned int DayOfMonth(const int day, const int month, const int year) {
...
int same_week(struct tm *tm1, struct tm *tm2) {
        struct tm tm11 = *tm1;
        struct tm tm22 = *tm2;
        Tick time1, time2;
        _Xltimeparams localtime_buf;
...
Tick DeriveNewStartTime(const Tick start_time, RepeatEvent *old_re,
                        const Tick current_time, const Tick target_time,
                        RepeatEvent *new_re) {
        Tick end_date;
        Tick an_event;
        int num_events;
 ...
 */
boolean_t RulesMatch(char *rule1, char *rule2) {
        Duration old_duration;
        time_t old_end_date;
        char *new_rule1, *new_rule2;
        RepeatEvent *re1, *re2;
