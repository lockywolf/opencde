...
char *ReToString(RepeatEvent *re) {
        char *cmd_buf = NULL;
 ...
 */
static void NumsToBuf(unsigned int *array, unsigned int array_size,
                      NumType type, char *buffer, unsigned int buf_size) {
        int i, size = 0;
        char tmp_buf[32], tmp_buf2[32];
        int tmp_buf_size = 32;
...
static void WeekDayToString(WeekDay day, char *buffer, int buffer_size) {
...
static void WeekNumberToString(WeekNumber week, char *buffer, int buffer_size) {
...
static void ConvertDaily(RepeatEvent *re, char *subcommand,
                         unsigned int buf_size) {
        char tmp_buf[32];
        unsigned int size = 0, num_time, i;
...
static void ConvertWeekly(RepeatEvent *re, char *subcommand,
                          unsigned int subcommand_size) {
        int tmp_buf_size = 32;
        char tmp_buf[32];
        int size = 0, num_items, i;
...
static void ConvertMonthly(RepeatEvent *re, char *subcommand,
                           unsigned int subcommand_size) {
        char tmp_buf[32];
        int size = 0, num_items, i;
        ...
        if (re->re_type == RT_MONTHLY_POSITION) {
                ...
                for (i = 0; i < num_items; i++) {
                        ...
                        /* The week: 1+ 3- etc. */
                        NumsToBuf((unsigned int *)RE_MONTHLY(re)
                                      ->md_weektime[i]
                                      .wdt_week,
                                  RE_MONTHLY(re)->md_weektime[i].wdt_nweek,
                                  NUM_WEEK, subcommand, subcommand_size - size);
                        ...
                        if (size > subcommand_size) {
                                printf(
                                    "Error: Internal buffer size exceeded\n");
                                return;
                        }
                        ...
                        NumsToBuf((unsigned int *)RE_MONTHLY(re)
                                      ->md_weektime[i]
                                      .wdt_day,
                                  RE_MONTHLY(re)->md_weektime[i].wdt_nday,
                                  NUM_DAY, subcommand, subcommand_size - size);
                        ...
                        NumsToBuf((unsigned int *)RE_MONTHLY(re)
                                      ->md_weektime[i]
                                      .wdt_time,
                                  RE_MONTHLY(re)->md_weektime[i].wdt_ntime,
                                  NUM_NUM, subcommand, subcommand_size - size);

                        size = strlen(subcommand);
                }
        } else { /* RT_MONTHLY_DAY */
                 /* The days: 1 15 31 etc. */
                NumsToBuf((unsigned int *)RE_MONTHLY(re)->md_days, num_items,
                          NUM_NUM, subcommand, subcommand_size - size);
        }
        /* Tack on the duration information */
        snprintf(tmp_buf, 32, " #%d", re->re_duration);
...
static void ConvertYearly(RepeatEvent *re, char *subcommand,
                          unsigned int subcommand_size) {
        char tmp_buf[32];
        int size = 0, num_items, i;
        ...
        NumsToBuf(RE_YEARLY(re)->yd_items, RE_YEARLY(re)->yd_nitems, NUM_NUM,
                  subcommand, subcommand_size - size);
