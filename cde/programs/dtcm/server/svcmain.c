 ...
 */
static int _GetGtime(char *timestr) {
...
static void init_dir() {
        ...
        if (create_dir || info.st_mode != (mode | S_IFDIR)) {
                ...
                if (chmod(dir, mode)) {
                        snprintf(msgbuf, BUFSIZ,
                                 ...
                                 pgname,
                                 ": This might happen if you are mounting the "
                                 "directory.",
                                 pgname, ": System error");
                        perror(msgbuf);
                        if (create_dir)
        ...
        if (create_dir || info.st_uid != daemon_uid ||
            info.st_gid != daemon_gid) {
        ...
        if ((fp = popen(buf, "r")) == NULL) {
                ...
                while (fgets(buf, sizeof(buf), fp) != NULL) {
                        if ((pid = atol(buf)) != mypid) {
                                if (kill(pid, SIGHUP))
                                        perror(
                                            "rpc.cmsd: failed to send SIGHUP");
                                if (debug)
...
static int lock_it() {
        ...
        if (fd < 0) {
                ...
                /*
                 * Note, we have to use flock() instead of lockf() because cms
                 * process is run in each host.
                 */
#ifdef SVR4
...
static void program(struct svc_req *rqstp, register SVCXPRT *transp) {
        char *result;
        char *argument = NULL;
 ...
 */
static int _GetFirstGarbageCollectionTime() {
...
int main(int argc, char **argv) {
        ...
        for (version = 0; version < ph->nvers; version++) {
                ...
                if (udp_transp == (SVCXPRT *)-1) {
                        udp_transp = svc_tli_create(
                            standalone ? RPC_ANYFD : 0, nconf_udp,
                ...
                if (tcp_transp == (SVCXPRT *)-1) {
                        tcp_transp = svc_tli_create(
                            RPC_ANYFD, nconf_tcp, (struct t_bind *)NULL, 0, 0);
...
#endif
                        if (udp_transp == NULL) {
                                (void)fprintf(stderr, "rtable_main.c: cannot "
                                                      "create udp service.\n");
                                exit(1);
                        }
                }
...
#endif
                        (void)fprintf(stderr,
                                      "rtable_main.c: unable to register");
                        exit(1);
                }
