...
typedef enum { SameInterval, NextInterval } MoveIndicator;
...
static int DurationCheck(const Duration res_dur, const Duration re_dur) {
        if (re_dur == RE_INFINITY || re_dur == RE_NOTSET)
                return TRUE;
...
Tick PrevTick(const Tick cur_time, const Tick start_time, RepeatEvent *re,
        ...
        if (!DurationCheck(res->res_duration, re->re_duration))
                return (Tick)0;
 ...
 */
static Tick DoMinute(const Tick cur_time, const Tick start_time,
                     const RepeatEvent *re, RepeatEventState *res) {
        return (Tick)NULL;
}

static Tick DoDay(const Tick cur_time, const Tick start_time,
                  const RepeatEvent *re, RepeatEventState *res) {
        Tick prev_time;
        ...
        if (dd_ntime) {
                ...
                if (RES_DSTATE(res).res_time == 0) {
                        prev_time = PrevDayTick(
                            cur_time, RE_DAILY(re)->dd_time[dd_ntime - 1] / 100,
                            RE_DAILY(re)->dd_time[dd_ntime - 1] % 100, re, res);
...
static Tick DoWeek(const Tick cur_time, const Tick start_time,
                   const RepeatEvent *re, RepeatEventState *res) {
        unsigned int res_daytime = RES_WSTATE(res).res_daytime,
                     res_time = RES_WSTATE(res).res_time, re_ntime,
...
static Tick DoMonthDay(const Tick cur_time, const Tick start_time,
                       const RepeatEvent *re, RepeatEventState *res) {
        ...
        do {
                ...
                cur_tm =
                    *_XLocaltime((const time_t *)&_cur_time, localtime_buf);
...
static Tick DoMonthPos(const Tick cur_time, const Tick start_time,
                       const RepeatEvent *re, RepeatEventState *res) {
...
static Tick DoYearByMonth(const Tick cur_time, const Tick start_time,
                          const RepeatEvent *re, RepeatEventState *res) {
...
static Tick DoYearByDay(const Tick cur_time, const Tick start_time,
                        const RepeatEvent *re, RepeatEventState *res) {
        struct tm start_tm, cur_tm;
        Duration res_duration = res->res_duration;
        Tick _cur_time;
...
static Tick PrevDayTick(const Tick tick, const int hour, const int min,
                        const RepeatEvent *re, RepeatEventState *res) {
        struct tm *tm;
        _Xltimeparams localtime_buf;
...
static Tick PrevWeekTick(const struct tm *current_tm, const struct tm *start_tm,
                         const RepeatEvent *re, const DayTime *wd_daytime,
                         const MoveIndicator move, RepeatEventState *res) {
        ...
        cur_tm->tm_mday -= GetWDayDiff(
            RE_WEEKLY(re)->wd_daytime[res_daytime].dt_day, cur_tm->tm_wday);
...
static Tick PrevMonthTick(const struct tm *current_tm, const RepeatEvent *re,
                          const WeekDayTime *wdt_list, const MoveIndicator move,
                          RepeatEventState *res) {
        struct tm *cur_tm = (struct tm *)current_tm;
        ...
        if (move == (const MoveIndicator)NextInterval) {
                ...
                } while (!(next_time = WeekNumberToDay(
                               mktime(cur_tm),
