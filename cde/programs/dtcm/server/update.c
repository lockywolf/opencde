 ...
 ******************************************************************************/
static boolean_t _SameRecurrenceRule(cms_attribute_value *newval,
                                     cms_attribute_value *oldval);

static CSA_return_code _SetNewStartDate(cms_entry *olde, RepeatEvent *oldre,
                                        cms_entry *newe, RepeatEvent *newre,
                                        cms_key *key);

static CSA_return_code
_AdjustStartEndTimeForUpdateInst(cms_entry *newe, cms_entry *olde, cms_key *key,
                                 uint num_attrs, cms_attribute *attrs);

static CSA_return_code _AdjustStartEndTimeForUpdateEntry(List_node *lnode,
                                                         cms_entry *newe,
                                                         cms_key *key,
                                                         uint num_attrs,
                                                         cms_attribute *attrs);

static void _GetStartEndIndex(uint num_attrs, cms_attribute *attrs, int *starti,
                              int *endi);
...
extern CSA_return_code _DtCmsUpdateEntry(_DtCmsCalendar *cal, char *sender,
        ...
        if ((stat = _DtCmUpdateAttributes(num_attrs, attrs, &newe->num_attrs,
                                          &newe->attrs, &cal->entry_tbl,
                                          B_FALSE, &cal->types, B_FALSE)) !=
            CSA_SUCCESS) {
        ...
        if (lnode != NULL &&
            (stat = _AdjustStartEndTimeForUpdateEntry(
...
extern CSA_return_code _DtCmsUpdateInstances(_DtCmsCalendar *cal, char *sender,
                                             uint access, cms_key *key,
                                             int scope, uint num_attrs,
                                             cms_attribute *attrs,
                                             cms_entry **oldentry,
                                             cms_entry **newentry) {
        CSA_return_code stat;
        ...
        if (scope == CSA_SCOPE_ONE) {
                if (rulei == -1) {
                        _DtCmUpdateSint32AttrVal(
                            NULL,
                            &newe->attrs[CSA_ENTRY_ATTR_NUMBER_RECURRENCES_I]
                                 .value);
                        _DtCmUpdateStringAttrVal(
                            NULL, &newe->attrs[CSA_ENTRY_ATTR_RECURRENCE_RULE_I]
                                       .value);
                        _DtCmUpdateSint32AttrVal(
                            NULL,
                            &newe->attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I]
                                 .value);
                        _DtCmUpdateSint32AttrVal(
                            NULL,
                            &newe->attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_TIMES_I]
                                 .value);
                        _DtCmUpdateSint32AttrVal(
                            NULL,
                            &newe->attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_INTERVAL_I]
                                 .value);
                        _DtCmUpdateSint32AttrVal(
                            NULL,
                            &newe->attrs
                ...
                if (rulei == -1 ||
                    _SameRecurrenceRule(
                        attrs[rulei].value,
                        olde->attrs[CSA_ENTRY_ATTR_RECURRENCE_RULE_I].value)) {
                        ...
                        if (newe->attrs[CSA_ENTRY_ATTR_EXCEPTION_DATES_I]
                                .value) {
                                _DtCmsCleanupExceptionDates(newe, key->time);
                        }

                        oaptr = olde->attrs[CSA_ENTRY_ATTR_NUMBER_RECURRENCES_I]
                                    .value;
        ...
        if ((stat = _AdjustStartEndTimeForUpdateInst(newe, olde, key, num_attrs,
                                                     attrs)) != CSA_SUCCESS)
                goto _cleanup;
...
_AdjustStartEndTimeForUpdateInst(cms_entry *newe, cms_entry *olde, cms_key *key,
        ...
        if (starti >= 0 && endi == -1 &&
            newe->attrs[CSA_ENTRY_ATTR_END_DATE_I].value) {
                ...
                _csa_tick_to_iso8601(endtime,
                                     newe->attrs[CSA_ENTRY_ATTR_END_DATE_I]
                                         .value->item.date_time_value);
                ...
                if (newe->attrs[CSA_ENTRY_ATTR_END_DATE_I].value) {
                        ...
                        _csa_tick_to_iso8601(
                            endtime, newe->attrs[CSA_ENTRY_ATTR_END_DATE_I]
                                         .value->item.date_time_value);
                }
        } else {
                _csa_iso8601_to_tick(newe->attrs[CSA_ENTRY_ATTR_START_DATE_I]
...
static CSA_return_code _SetNewStartDate(cms_entry *olde, RepeatEvent *oldre,
                                        cms_entry *newe, RepeatEvent *newre,
                                        cms_key *key) {
        if ((newe->key.time = DeriveNewStartTime(
                 olde->key.time, oldre, key->time, newe->key.time, newre)) == 0)
                return (CSA_E_FAILURE);

        if (_csa_tick_to_iso8601(newe->key.time,
                                 newe->attrs[CSA_ENTRY_ATTR_START_DATE_I]
                                     .value->item.date_time_value))
                return (CSA_E_FAILURE);
...
static CSA_return_code _AdjustStartEndTimeForUpdateEntry(List_node *lnode,
        ...
        if (key->time != olde->key.time &&
            (newaptr == NULL || newaptr->item.string_value == NULL ||
             RulesMatch(oldaptr->item.string_value,
                        newaptr->item.string_value) == B_FALSE)) {
        ...
        if (starti >= 0 && key->time != olde->key.time) {
                ...
                if (newbod == instbod) {
                        ...
                        /* get new start date */
                        if ((stat = _SetNewStartDate(olde, lnode->re, newe,
                                                     newre, key)) !=
                            CSA_SUCCESS) {
                                _DtCm_free_re(newre);
                                return (stat);
