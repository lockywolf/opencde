 ...
 */
int CountEvents(Tick start_time, RepeatEvent *re, CSA_date_time_entry *dte) {
        time_t exclude_time, close_time;
        int excluded_days = 0;
        unsigned int nevents1 = (unsigned int)-1, nevents2 = (unsigned int)-1;
        ...
        for (; dte; dte = dte->next) {
                ...
                if (!(close_time =
                          ClosestTick(exclude_time, start_time, re, &res))) {
                        time_t last_time;
        ...
        case RT_WEEKLY:
                if (!RE_WEEKLY(re)->wd_ndaytime)
                        ...
                        nevents2 = re->re_duration * RE_WEEKLY(re)->wd_ndaytime;
        ...
        case RT_YEARLY_MONTH:
                if (!RE_YEARLY(re)->yd_nitems)
                        ...
                        nevents2 = re->re_duration * RE_YEARLY(re)->yd_nitems;
                nevents2 -= InitialEventsToExclude(start_time, re);
                break;
        case RT_YEARLY_DAY:
                if (!RE_YEARLY(re)->yd_nitems)
 ...
 */
static int InitialEventsToExclude(time_t start_time, RepeatEvent *re) {
        struct tm *start_tm;
        _Xltimeparams localtime_buf;
        ...
        if (re->re_type == RT_WEEKLY) {
                ...
                int nevent_days = RE_WEEKLY(re)->wd_ndaytime, i;
 ...
 */
static int EventsPerMonth(RepeatEvent *re) {
        int nevents = 1, i;
 ...
 */
static int DoBruteForce(const time_t start_time, RepeatEvent *re) {
        RepeatEventState *res;
        time_t cur_time;
        int nevents = 0;
