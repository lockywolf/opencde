...
char *tag_string[] = {
    "0:boolean",         "1:enum",          "2:flags",        "3:sint32",
    "4:uint32",          "5:string",        "6:user",         "7:date_time",
    "8:date_time_range", "9:time_duration", "10:access_list", "11:attendees",
    "12:date_time_list", "13:reminder",     "14:opaque_data", NULL};

/*
 * forward declaration of functions used within this file
 */
static char *cr_to_str(char *s, int size);
static void periodtostr(Interval_4 i, char *q, int buf_size);
static void privacytostr(Privacy_Level_4 p, char *q, int buf_size);
static void apptstatustostr(Appt_Status_4 p, char *q, int buf_size);
static void tagstostr(Event_Type_4 p, char *q, int buf_size);
...
extern char *_DtCmsGetLogFN(char *who) {
        return (get_fname(spool_dir, _DtCMS_DEFAULT_LOG, who));
}

extern char *_DtCmsGetBakFN(char *who) {
        return (get_fname(spool_dir, _DtCMS_DEFAULT_BAK, who));
}

extern char *_DtCmsGetTmpFN(char *who) {
        return (get_fname(spool_dir, _DtCMS_DEFAULT_TMP, who));
}

extern char *_DtCmsGetDelFN(char *who) {
        return (get_fname(spool_dir, _DtCMS_DEFAULT_DEL, who));
}

extern CSA_return_code _DtCmsCreateLogV1(char *owner, char *file) {
        return (create_log(owner, file, _DtCMS_VERSION1));
}

extern CSA_return_code _DtCmsAppendHTableByFN(char *file, uint size,
                                              char **names, int *types) {
        int f;
 ...
 */
extern CSA_return_code _DtCmsAppendHTableByFD(int fd, uint size, char **names,
                                              int *types) {
        CSA_return_code stat;
        ...
        tcount = BUFSIZ;
        strlcpy(buf, "(entrytable ", tcount);

        for (i = 1; i <= size; i++) {
                snprintf(tmpbuf, BUFSIZ / 4, "(%d \"%s\" \"%s\")\n", i,
                         names[i], tag_string[types[i]]);

                count += strlen(tmpbuf);
                if (tcount < count) {
                        if ((tptr = grow_n_concat(buf, tmpbuf,
                                                  tcount + BUFSIZ)) == NULL) {
                                free(buf);
                                return (CSA_E_INSUFFICIENT_MEMORY);
                        }
...
extern CSA_return_code _DtCmsAppendEntryByFN(char *file, cms_entry *entry,
                                             _DtCmsLogOps op) {
        int f;
        CSA_return_code stat;
...
extern CSA_return_code _DtCmsAppendEntryByFD(int f, cms_entry *entry,
                                             _DtCmsLogOps op) {
        CSA_return_code stat;
        int count;
        char *cptr, *tptr, *buf;
        ...
        snprintf(buf, buf_size, "(%s \"%s\" key: %ld%s",
                 (op == _DtCmsLogAdd ? "add" : "remove"), isotime,
                 entry->key.id,
                 (op == _DtCmsLogAdd ? "\nhashedattributes: (" : ""));
        count = strlen(buf) + 4;

        if (op == _DtCmsLogAdd && entry->num_attrs > 0) {
                if ((cptr = attrs_to_attrliststr(entry->num_attrs, entry->attrs,
                                                 B_TRUE, B_TRUE)) == NULL) {
                        free(buf);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
...
extern CSA_return_code _DtCmsAppendCalAttrsByFN(char *file, int size,
                                                cms_attribute *attrs) {
        int f;
        CSA_return_code stat;
...
extern CSA_return_code _DtCmsAppendCalAttrsByFD(int f, int size,
                                                cms_attribute *attrs) {
        CSA_return_code stat;
        char *buf, *attrstr;
        ...
        snprintf(buf, count, "%s%s%s", prefix, attrstr, subfix);
...
extern CSA_return_code _DtCmsAppendAppt4ByFN(char *file, Appt_4 *appt,
                                             _DtCmsLogOps op) {
        int f;
        CSA_return_code stat;
...
extern CSA_return_code _DtCmsAppendAppt4ByFD(int f, Appt_4 *appt,
                                             _DtCmsLogOps op) {
        char *cptr;
        char tmpbuf[BUFSIZ];
        int buf_size = BUFSIZ * 3, buf2_size = BUFSIZ * 2;
        ...
        switch (op) {
        case _DtCmsLogAdd:
                snprintf(buf, buf_size, "(add \"%s\" key: %ld ", cptr,
                         appt->appt_id.key);
                if (appt->what) {
                        cptr = cr_to_str(appt->what, strlen(appt->what));
                        if (cptr != NULL) {
                                snprintf(buf2, buf2_size, "what: \"%s\" ",
                                         cptr);
                                strlcat(buf, buf2, buf_size);
                                free(cptr);
                        } else {
                                return (CSA_E_INSUFFICIENT_MEMORY);
                        }
                ...
                if (appt->client_data) {
                        snprintf(buf2, buf2_size, "details: \"%s\" ",
                                 appt->client_data);
                        strlcat(buf, buf2, buf_size);
                }
                if (appt->duration) {
                        snprintf(buf2, buf2_size, "duration: %d ",
                                 appt->duration);
                        strlcat(buf, buf2, buf_size);
                }

                periodtostr(appt->period.period, tmpbuf, BUFSIZ);
                snprintf(buf2, buf2_size, "period: %s ", tmpbuf);
                strlcat(buf, buf2, buf_size);

                if (appt->period.nth != 0) {
                        snprintf(buf2, buf2_size, "nth: %d ", appt->period.nth);
                        strlcat(buf, buf2, buf_size);
                }
                if (appt->period.enddate != 0) {
                        cptr = ctime(&(appt->period.enddate));
                        cptr[24] = '\0'; /* strip off CR */
                        snprintf(buf2, buf2_size, "enddate: \"%s\" ", cptr);
                        strlcat(buf, buf2, buf_size);
                }

                snprintf(buf2, buf2_size, "ntimes: %d ", appt->ntimes);
                strlcat(buf, buf2, buf_size);

                if (appt->exception != NULL) {
                        struct Except_4 *e = appt->exception;
                        strlcat(buf, "exceptions: (", buf_size);
                        while (e != NULL) {
                                snprintf(buf2, buf2_size, "%d ", e->ordinal);
                                strlcat(buf, buf2, buf_size);
                                e = e->next;
                        }
                ...
                if (appt->author != NULL) {
                        snprintf(buf2, buf2_size, "author: \"%s\" ",
                                 appt->author);
                        strlcat(buf, buf2, buf_size);
                }
                if (appt->attr != NULL) {
                        struct Attribute_4 *item = appt->attr;
                        strlcat(buf, "attributes: (", buf_size);
                        while (item != NULL) {
                                snprintf(buf2, buf2_size,
                                         "(\"%s\",\"%s\",\"%s\")", item->attr,
                                         item->value, item->clientdata);
                                strlcat(buf, buf2, buf_size);
                                item = item->next;
                        }
                        strlcat(buf, ") ", buf_size);
                ...
                if (appt->tag != NULL) {
                        ...
                        strlcat(buf, "tags: (", buf_size);
                        while (item != NULL) {
                                tagstostr(item->tag, tmpbuf, BUFSIZ);
                                snprintf(buf2, buf2_size, "(%s , %d)", tmpbuf,
                                         item->showtime);
                                strlcat(buf, buf2, buf_size);
                                item = item->next;
                        }
                ...
                apptstatustostr(appt->appt_status, tmpbuf, BUFSIZ);
                snprintf(buf2, buf2_size, "apptstat: %s ", tmpbuf);
                strlcat(buf, buf2, buf_size);

                privacytostr(appt->privacy, tmpbuf, BUFSIZ);
                snprintf(buf2, buf2_size, "privacy: %s )\n", tmpbuf);
                strlcat(buf, buf2, buf_size);

                break;

        case _DtCmsLogRemove:
                snprintf(buf, buf_size, "(remove \"%s\" key: %ld)\n", cptr,
                         appt->appt_id.key);
                break;
        }
...
extern CSA_return_code _DtCmsAppendAccessByFN(char *file, int type,
                                              Access_Entry_4 *p) {
        int f;
        CSA_return_code stat;
...
extern CSA_return_code _DtCmsAppendAccessByFD(int f, int type,
                                              Access_Entry_4 *p) {
        CSA_return_code stat;
        int count = 12; /* (access  )\n */
        char *p_type;
        ...
        snprintf(buf, count, "(access %s ", p_type);
...
extern boolean_t _DtCmsPrintAppt4(caddr_t data) {
        ...
        buf[0] = '\0';
        periodtostr(appt->period.period, buf, BUFSIZ);
        ...
        buf[0] = '\0';
        apptstatustostr(appt->appt_status, buf, BUFSIZ);
        if (fprintf(stderr, "apptstat: %s ", buf) == EOF)
                return (B_TRUE);

        buf[0] = '\0';
        privacytostr(appt->privacy, buf, BUFSIZ);
 ...
 */
extern int _DtCmsSetFileMode(char *file, uid_t uid, gid_t gid, mode_t mode,
                             boolean_t changeeuid, boolean_t printerr) {
        int error;
        char buff[BUFSIZ];
        ...
        if (chown(file, uid, gid) < 0) {
                if (printerr == B_TRUE) {
                        ...
                        snprintf(
                            buff, BUFSIZ,
                            "%s: chown %s to (uid=%ld,gid=%ld) failed.\n%s%s",
                            pgname, file, (long)uid, (long)gid, pgname,
                            ": System error");
                        errno = error;
                        perror(buff);
                }
...
#endif
        {
                if (printerr == B_TRUE) {
                        ...
                        snprintf(buff, BUFSIZ,
                                 "%s: Can't switch process uid back to "
                                 "daemon.\n%s%s",
...
static void periodtostr(Interval_4 i, char *q, int buf_size) {
        ...
        switch (i) {
        case single_4:
                strlcpy(q, "single", buf_size);
                break;
        case daily_4:
                strlcpy(q, "daily", buf_size);
                break;
        case weekly_4:
                strlcpy(q, "weekly", buf_size);
                break;
        case biweekly_4:
                strlcpy(q, "biweekly", buf_size);
                break;
        case monthly_4:
                strlcpy(q, "monthly", buf_size);
                break;
        case yearly_4:
                strlcpy(q, "yearly", buf_size);
                break;
        case nthWeekday_4:
                strlcpy(q, "nthWeekday", buf_size);
                break;
        case everyNthDay_4:
                strlcpy(q, "everyNthDay", buf_size);
                break;
        case everyNthWeek_4:
                strlcpy(q, "everyNthWeek", buf_size);
                break;
        case everyNthMonth_4:
                strlcpy(q, "everyNthMonth", buf_size);
                break;
        case monThruFri_4:
                strlcpy(q, "monThruFri", buf_size);
                break;
        case monWedFri_4:
                strlcpy(q, "monWedFri", buf_size);
                break;
        case tueThur_4:
                strlcpy(q, "tueThur", buf_size);
                break;
        case daysOfWeek_4:
                strlcpy(q, "daysOfWeek", buf_size);
                break;
        default:
                strlcpy(q, "single", buf_size);
                break;
...
static void privacytostr(Privacy_Level_4 p, char *q, int buf_size) {
        ...
        switch (p) {
        case public_4:
                strlcpy(q, "public", buf_size);
                break;
        case private_4:
                strlcpy(q, "private", buf_size);
                break;
        case semiprivate_4:
                strlcpy(q, "semiprivate", buf_size);
                break;
        default:
                strlcpy(q, "public", buf_size);
                break;
...
static void apptstatustostr(Appt_Status_4 p, char *q, int buf_size) {
        ...
        switch (p) {
        case active_4:
                strlcpy(q, "active", buf_size);
                break;
        case pendingAdd_4:
                strlcpy(q, "pendingAdd", buf_size);
                break;
        case pendingDelete_4:
                strlcpy(q, "pendingDelete", buf_size);
                break;
        case committed_4:
                strlcpy(q, "committed", buf_size);
                break;
        case cancelled_4:
                strlcpy(q, "cancelled", buf_size);
                break;
        case completed_4:
                strlcpy(q, "completed", buf_size);
                break;
        default:
                strlcpy(q, "active", buf_size);
                break;
...
static void tagstostr(Event_Type_4 p, char *q, int buf_size) {
        ...
        switch (p) {
        case appointment_4:
                strlcpy(q, "appointment", buf_size);
                break;
        case reminder_4:
                strlcpy(q, "reminder", buf_size);
                break;
        case otherTag_4:
                strlcpy(q, "otherTag", buf_size);
                break;
        case holiday_4:
                strlcpy(q, "holiday", buf_size);
                break;
        case toDo_4:
                strlcpy(q, "toDo", buf_size);
                break;
        default:
                strlcpy(q, "appointment", buf_size);
                break;
...
static char *get_fname(char *dir, char *fname, char *who) {
        char *buf;
        int buf_length = strlen(dir) + strlen(fname) + strlen(who) + 3;

        buf = (char *)malloc(buf_length);
        if (buf != NULL)
                snprintf(buf, buf_length, "%s/%s.%s", dir, fname, who);
        return (buf);
}
...
static char *grow_n_concat(char *base, char *newstr, int newcount) {
        ...
        if (base)
                strlcat(ptr, newstr, newcount);
        else
                strlcpy(ptr, newstr, newcount);
 ...
 * 				(hash_number
 * "string:string:number:number:string") CSA_VALUE_OPAQUE_DATA:
 * (hash_number "number:string")
 *
 * Note: element 0 of the array is not used
 */
static char *attrs_to_attrliststr(int num_attr, cms_attribute *attrs,
                                  boolean_t write_hash, boolean_t entryattrs) {
        int i, count, tcount;
        char *ptr, *buf = NULL;
        char tmpbuf[BUFSIZ / 2], tmpbuf2[BUFSIZ / 2], *body;
        int tmpbuf_size = BUFSIZ / 2;
        CSA_opaque_data *opq;
        ...
        for (i = 1, count = 0, *buf = '\0'; i <= num_attr; i++) {
                ...
                if (write_hash)
                        ...
                        snprintf(tmpbuf, tmpbuf_size, "(%d \"",
                                 attrs[i].name.num);
                else
                        /* open bracket name, type, open quote for value */
                        snprintf(tmpbuf, tmpbuf_size, "(\"%s\",\"%s\",\"",
                                 attrs[i].name.name,
                ...
                *tmpbuf2 = '\0';
                ...
                case CSA_VALUE_SINT32:
                        snprintf(tmpbuf2, tmpbuf_size, "%ld",
                ...
                case CSA_VALUE_UINT32:
                        snprintf(tmpbuf2, tmpbuf_size, "%lu",
                ...
                case CSA_VALUE_REMINDER:
                        opq =
                            &attrs[i].value->item.reminder_value->reminder_data;
                        if (attrs[i].value->item.reminder_value->repeat_count >
                            1) {
                                snprintf(
                                    tmpbuf2, tmpbuf_size, "%s:%s:%lu:%lu:",
                                    attrs[i]
                ...
                case CSA_VALUE_OPAQUE_DATA:
                        ...
                        snprintf(tmpbuf2, tmpbuf_size, "%lu:", opq->size);
                ...
                strlcat(buf, tmpbuf, tcount);
                if (*tmpbuf2 != '\0')
                        strlcat(buf, tmpbuf2, tcount);
                if (body)
                        strlcat(buf, body, tcount);
                strlcat(buf, "\")\n", tcount);
                if (body)
                        free(body);
        }
...
static char *get_access_list_string(cms_access_entry *list) {
        ...
        /* do the first one */
        if ((ptr = malloc(BUFSIZ + 1)) == NULL)
                return (NULL);
        tcount = BUFSIZ;
        snprintf(ptr, BUFSIZ + 1, "%s:%u", list->user, list->rights);
        count = strlen(ptr);

        while (list->next) {
                list = list->next;
                snprintf(buf, BUFSIZ / 4, " %s:%u", list->user, list->rights);
...
static char *get_date_time_list_string(CSA_date_time_list list) {
        ...
        /* do the first one */
        snprintf(buf, 80, "%s", list->date_time);
        ...
        if ((ptr = malloc(BUFSIZ + 1)) == NULL)
                return (NULL);
        tcount = BUFSIZ;
        strlcpy(ptr, list->date_time, BUFSIZ + 1);
        count = strlen(ptr);
