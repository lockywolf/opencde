...
#define unput_char()                                                           \
        (((current_ptr > start_of_mmapped_area) ? (current_ptr--) : 0))
...
#define PVAL(a, b)                                                             \
        { #a, -1, -1, &externPeriod.period, b, PERIODVAL }
#define AVAL(a, b)                                                             \
        { #a, -1, -1, &externApptStatus, b, APPTSTATUSVAL }
#define TVAL(a, b)                                                             \
        { #a, -1, -1, &externTag.tag, b, TAGSVAL }
#define SVAL(a, b)                                                             \
        { #a, -1, -1, &externPrivacy, b, PRIVACYVAL }

#define EMP(a, b)                                                              \
        { #a, -1, -1, 0, 0, b }

token_data_t tokens[] = {PVAL(single, single_4),
                         PVAL(daily, daily_4),
        ...
        while (1) {
                ...
                case DASH:
                        ...
                        if (initial_mask[*(++ptr) = input_char()] != DIGIT) {
                                fprintf(stderr,
                                        "%s: Unsupported char %c (ascii %d) "
                                        "found in callog file\n",
                                        pgname, *ptr, *ptr);
                                return (0);
                        }
                        /* fall through */
                ...
                case ALPHA: {
                        ...
                        *ptr = '\0';
                        ...
                        if ((bucket = tokens[hash_string((char *)parse_buffer)]
                                          .first_token) == -1) {
                                fprintf(stderr,
                                        "%s: cannot lex %s in callog file\n",
                                        pgname, parse_buffer);
                                return (0);
                        }

                        while (strcmp(tokens[bucket].name,
                                      (char *)parse_buffer) != 0)
                                if ((bucket = tokens[bucket].next_token) ==
                                    -1) /* end of chain */ {
                                        fprintf(stderr,
                                                "%s: cannot lex %s in callog "
                                                "file\n",
                                                pgname, parse_buffer);
                                        return (0);
                                }
                ...
                case QUOTE: /* note that code removes leading and trailing
                             * quotes
                             */
                        while (1) {
                                ...
                                case '"':
                                        *ptr = 0;
                                        strescapes(
                                            (char *)
                                                parse_buffer); /* process any
                                                                  escape
                                                                  sequences */
                                        externQuotedString = (char *)strdup(
                                            (char *)parse_buffer);
                                        return (QUOTEDSTRING);
                                case EOFILE:
                                case '\n':
                ...
                case COMMENT: {
                        int i;
                        for (i = 0; i < 3;
                             i++) /* look for 4 **** as comment lead-in */
                                if (initial_mask[*(++ptr) = input_char()] !=
                                    COMMENT) {
                                        *(++ptr) = 0;
                                        fprintf(stderr,
                                                "%s: cannot lex %s in callog "
                ...
                default:
                        fprintf(stderr,
                                "%s: Unsupported char %c (ascii %d) found in "
                                "callog file\n",
                                pgname, *ptr, *ptr);
                        return (0);
                }
...
extern int yyywrap(FILE *f) {
        fclose(f);
        munmap(start_of_mmapped_area, len);
        return (1);
}

extern int setinput(FILE *f) {
        struct stat buff;
        ...
        if ((start_of_mmapped_area = mmap(NULL, len = buff.st_size, PROT_READ,
                                          MAP_PRIVATE, fileno(f), 0)) ==
            (caddr_t)-1) { /* mmap failed??? */
                perror("mmap");
                return (-1);
        }
