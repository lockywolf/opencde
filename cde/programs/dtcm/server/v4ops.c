...
extern CSA_return_code _DtCmsInsertAppt(_DtCmsCalendar *cal, Appt_4 *appt4) {
        ...
        if (rb_stat == rb_ok) {
                ...
                while (p_attr != NULL) {
                        ...
                        p_reminder = build_reminder(current_time, appt4, p_attr,
                                                    tick, 1);
                        if (p_reminder != NULL)
 ...
 */
extern CSA_return_code _DtCmsDeleteAppt(_DtCmsCalendar *cal, char *user,
                                        uint access, Id_4 *p_key,
                                        Appt_4 **appt_r) {
        Appt_4 *p_appt;
        List_node *p_lnode;
        Tree_node *p_node;
         ...
         */
        if ((user != NULL) && (p_appt = (Appt_4 *)rb_lookup(
                                   APPT_TREE(cal), (caddr_t)p_key)) != NULL) {
        ...
        p_lnode = (List_node *)hc_lookup_node(REPT_LIST(cal), (caddr_t)p_key);
...
extern CSA_return_code _DtCmsDeleteApptAndLog(_DtCmsCalendar *cal, char *user,
                                              uint access, Id_4 *key,
                                              Appt_4 **oldappt) {
        CSA_return_code stat;
        Appt_4 *appt;
...
_DtCmsDeleteApptInstancesAndLog(_DtCmsCalendar *cal, char *source, uint access,
                                Id_4 *key, Options_4 option, int *remain,
                                Appt_4 **oldappt) {
        List_node *p_lnode;
        ...
        if (option == do_one_4) {
                ...
                if (ninstance == 1) {
                        ...
                        stat = _DtCmsRbToCsaStat(
                            rb_insert(cal->tree, (caddr_t)p_appt,
                                      (caddr_t) & (p_appt->appt_id)));
                } else {
                        p_appt->ntimes = _DtCms_get_new_ntimes_v4(
                            p_appt->period, p_appt->appt_id.tick, ninstance);
        ...
        if (ordinal == 0) {
                ...
                if (stat ||
                    (stat = v4_transact_log(cal->calendar, p_appt,
                ...
                if (ninstance == 1) {
                        ...
                        while (p_attr != NULL) {
                                ...
                                tick =
                                    p_appt->appt_id.tick - atol(p_attr->value);
                                p_reminder = build_reminder(
                                    current_time, p_appt, p_attr, tick, 1);
                                if (p_reminder != NULL)
...
extern CSA_return_code _DtCmsChangeAll(_DtCmsCalendar *cal, char *source,
                                       uint access, Id_4 *p_key, Appt_4 *newa,
                                       Appt_4 **oldappt) {
        CSA_return_code stat = CSA_SUCCESS;
         ...
         * and the callog file
         */
        if ((stat = _DtCmsDeleteApptAndLog(cal, source, access, p_key,
                                           &olda)) != CSA_SUCCESS)
                return (stat);
...
extern CSA_return_code _DtCmsChangeSome(_DtCmsCalendar *cal, char *source,
                                        uint access, Id_4 *p_key, Appt_4 *newa,
                                        Options_4 option, Appt_4 **oldappt) {
        CSA_return_code stat;
        Appt_4 *olda, *newcopy = NULL;
        List_node *lnode;
         ...
         * and the callog file
         */
        if ((stat = _DtCmsDeleteApptInstancesAndLog(cal, source, access, p_key,
                                                    option, &remain, &olda)) !=
            CSA_SUCCESS)
                return (stat);
                ...
                if (olda->period.period == newa->period.period &&
                    ...
                    olda->ntimes == newa->ntimes) {
                        ...
                        if (olda->exception != NULL) {
                                ...
                                if ((stat = _DtCmsTruncateElist(
                                         olda, remain, &newcopy->exception)) !=
                                    CSA_SUCCESS) {
 ...
 */
extern CSA_return_code _DtCmsInsertApptAndLog(_DtCmsCalendar *cal,
                                              Appt_4 *appt) {
        CSA_return_code stat;
...
extern CSA_return_code v4_transact_log(char *calendar, Appt_4 *a,
                                       _DtCmsLogOps op) {
        CSA_return_code stat;
        char *log = NULL;
...
extern CSA_return_code
_DtCmsLookupRangeV4(_DtCmsCalendar *cal, char *user, uint access,
                    Range_4 *p_range, boolean_t no_end_time_range, long end1,
                    long end2, boolean_t (*match_func)(), uint num_attrs,
                    cms_attribute *attrs, CSA_enum *ops, Appt_4 **appt_r,
                    Abb_Appt_4 **abbr_r) {
        CSA_return_code stat = CSA_SUCCESS;
        Period_4 period;
        caddr_t ilp = NULL;
        ...
        if (cal->fversion > 1) {
                if ((stat = _DtCmsLookupEntries(
                         cal, user, access, p_range->key1, p_range->key2,
                ...
                if (appt_r)
                        stat =
                            _DtCmsCmsentriesToAppt4ForClient(entries, appt_r);
                else
        ...
        while (p_range != NULL) {
                ...
                while ((p_appt = (Appt_4 *)rb_lookup_next_larger(
                       ...
                       (APPT_TICK(p_appt) < hi_tick)) {
                        ...
                        if ((stat = (*add_list_func)(p_appt, user, access,
                                                     &ilp)) != CSA_SUCCESS) {
                                if (ilp != NULL) {
                                        if (appt_r)
                ...
                while (p_lnode != NULL) {
                        ...
                        if (p_lnode->lasttick == 0)
                                p_lnode->lasttick = _DtCms_last_tick_v4(
                                    APPT_TICK(p_appt), p_appt->period,
                                    p_appt->ntimes);
                        ...
                        if (p_lnode->lasttick <= lo_tick ||
                            ...
                            (!no_end_time_range &&
                             (p_appt->appt_id.tick + p_appt->duration) >=
                                 end2) ||
                            (match_func &&
                             !match_func(p_appt, num_attrs, attrs, ops)) ||
                            _GetAccessLevel(user, access, p_appt) ==
                                private_4) {
                        ...
                        for (tick = _DtCms_closest_tick_v4(
                                 lo_tick, APPT_TICK(p_appt), period, &ordinal),
                            ordinal--;
                             tick < hi_tick;
...
extern CSA_return_code _DtCmsLookupKeyrangeV4(
    _DtCmsCalendar *cal, char *user, uint access, boolean_t no_start_time_range,
    boolean_t no_end_time_range, time_t start1, time_t start2, time_t end1,
    time_t end2, long id, boolean_t (*match_func)(), uint num_attrs,
    cms_attribute *attrs, CSA_enum *ops, Appt_4 **appt_r, Abb_Appt_4 **abbr_r) {
        CSA_return_code stat = CSA_SUCCESS;
        Period_4 period;
        ...
        if (cal->fversion >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                if ((stat = _DtCmsLookupEntriesById(
                ...
                if (appt_r) {
                        stat =
                            _DtCmsCmsentriesToAppt4ForClient(entries, &tappt);
        ...
        if (p_lnode != NULL) {
                ...
                for (tick = _DtCms_closest_tick_v4(start1, APPT_TICK(p_appt),
                                                   period, &ordinal),
                    ordinal--;
                       ...
                       (APPT_TICK(p_appt) < start2)) {
                        if (p_appt->appt_id.key != lo_key.key) {
                                ...
                                if ((!no_end_time_range &&
                                    ...
                                    (match_func &&
                                     !match_func(p_appt, num_attrs, attrs,
                                                 ops))) {
...
extern CSA_return_code _AddToLinkedAppts(Appt_4 *p_appt, char *user,
                                         uint access, caddr_t *ilp) {
        Appt_4 *p_prev;
        Appt_4 *copy;
...
extern CSA_return_code _AddToLinkedAbbrAppts(Appt_4 *p_appt, char *user,
                                             uint access, caddr_t *ilp) {
        Abb_Appt_4 *copy;
...
extern Privacy_Level_4 _GetAccessLevel(char *user, uint access,
                                       Appt_4 *p_appt) {
...
extern CSA_return_code _DtCmsSetV4AccessListAndLog(_DtCmsCalendar *cal,
...
extern CSA_return_code _DtCmsGetV4Reminders(_DtCmsCalendar *cal, long tick,
                                            Reminder_4 **rem_r,
                                            _DtCmsEntryId **ids_r) {
        ...
        if (p_node != NULL) {
                ...
                do {
                        ...
                        v4rem = rptr;
                        if (ids_r) {
                                idptr = (_DtCmsEntryId *)calloc(
                                    1, sizeof(_DtCmsEntryId));
...
extern CSA_return_code _DtCmsTruncateElist(Appt_4 *parent_p, int remain,
                                           Except_4 **excpt) {
...
extern CSA_return_code _DtCmsSetV4AccessListInCal(_DtCmsCalendar *cal,
                                                  Access_Entry_4 *e) {
        Access_Entry_4 *q;
