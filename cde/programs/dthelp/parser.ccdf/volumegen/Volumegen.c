...
static void GenTopicList(FILE *infile, FILE *outfile);

static void GenTopicHeir(FILE *infile, FILE *outfile);

static void GenTopicLoc(FILE *infile, FILE *outfile);

static void GenHomeLocation(FILE *outfile);

static void GenKeyWordList(FILE *infile, FILE *outfile);
...
void main(int argc, char **argv) {
        ...
        if ((metaFile = fopen(fName, "r")) == NULL) {
                fprintf(stderr, "%s: error opening meta file %s for reading\n",
                        *argv, fName);
                perror(NULL);
                exit(1);
        }
        ...
        if ((volFile = fopen(fName, "w")) == NULL) {
                fprintf(stderr,
                        "%s: error opening Volume file %s for writing\n", *argv,
                        fName);
                perror(NULL);
                exit(1);
        }
        ...
        if ((keyWordFile = fopen(fName, "w")) == NULL) {
                fprintf(stderr,
                        "%s: error opening Keyword file %s for writing\n",
                        *argv, fName);
                perror(NULL);
                exit(1);
        }
...
static void GenTopicLoc(FILE *infile, FILE *outfile) {
        ...
        fprintf(outfile, "# Topic Locations (file names, file positions, and "
                         "non-topic IDs)\n");
        ...
        while (1) {
                ...
                /* If the topic ID ends with ":", then we've found a primary
                 * entry. */
                 ...
                 */
                        ...
                        if ((locationIdNew = GetNextToken(infile, TRUE)) ==
                            NULL)
                                break;

                        /* If the ID list hasn't been started for this topic,
                         * start it. */
                        if (idList == (char *)NULL) {
                        ...
                        /* Else, add the ID we've just found to the list. */
                                 ...
                                 */
                                idList = realloc(idList,
                                                 strlen(idList) +
                                                     strlen(locationIdNew) + 2);

                                strcat(idList, " ");
                                strcat(idList, locationIdNew);
                        }
                } else {
                        /* If the token does not end with ":" or ">", we have a
                         * problem. */
...

{
        ...
        /* idx files (e.g. keyword) have a strict syntax where each line
         * contains three tokens (the sort key, the keyword, and the locationID
         * of the topic in which the keyword resides). Therefore it is an error
         * if the number of tokens in the file is not an integral multiple of 3.
         */
        ...
        while (1) {
                ...
                if (keyWordLast && (strcmp(keyWordNew, keyWordLast) == 0)) {
                        ...
                        /* We have a duplicate so add it to our previous keyword
                         * list */
                        idList = realloc(idList, strlen(idList) +
                                                     strlen(locationIdNew) + 2);
...
static char *GetAChar(FILE *ifile) {
        ...
        while (1) {
                ...
                if (length == MB_CUR_MAX) { /* reached max without a hit */
                        printf("An invalid multi-byte character was found in "
                               "the input");
                        mbyte[0] = ' ';
                        mbyte[1] = '\0';
                        return mbyte;
                ...
                if ((c = getc(ifile)) == EOF) { /* huh? */
                        printf(
                            "End-of-file found within a multi-byte character");
                        mbyte[0] = '\0';
                        return mbyte;
                }
        }
...
static void MergeMetaInfo(FILE *infile, FILE *outfile) {
        ...
        while ((*(nextC = GetAChar(infile)) != '\0') &&
               (mblen(nextC, MB_CUR_MAX) == 1) && isspace(*nextC))
                ;
...
static char *GetNextToken(FILE *file, Boolean delimIsSpace) {
        ...
        while ((*(nextC = GetAChar(file)) != '\0') &&
               ((length = mblen(nextC, MB_CUR_MAX)) == 1) && isspace(*nextC))
                ;
