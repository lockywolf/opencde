...
#else
void options(filelenonly) LOGICAL filelenonly;
#endif
{
        int i;
        FILE *optfile;
        ...
        nameofoptfile =
            (char *)m_malloc(strlen(install) + strlen(TAGDOTOPT) + 1,
    ...
    defined(__uxp__)
        ...
        if (indir) {
                nameofoptfile = (char *)m_malloc(
                    strlen(indir) + strlen(TAGDOTOPT) + 1, "input helptag.opt");
                strcpy(nameofoptfile, indir);
                strcat(nameofoptfile, TAGDOTOPT);
        } else
                nameofoptfile = TAGDOTOPT;
        ...
        if (indir)
                m_free(nameofoptfile, "input helptag.opt");

        /* Check basename.opt in input directory */
        nameofoptfile =
            (char *)m_malloc((indir ? strlen(indir) : 0) + strlen(nodirbase) +
                                 strlen(DOTOPT) + 1,
                             "basename.opt");
        *nameofoptfile = M_EOS;
        ...
        for (i = 3; i < m_argc; i++)
                if (*m_argv[i] == '@') {
                        if (optfile = fopen(m_argv[i] + 1, "r")) {
                                ...
                                m_mberr1("Unable to open option file %s",
                                         (m_argv[i] + 1));
                        }
                } else
                        setopt(m_argv[i], filelenonly);

        if (filelenonly)
                return;

        if (optval) {
                m_mberr1("%s: Expecting value for option on command line or in "
                         "option file",
                         optkey[optval - 1]);
        }
        optval = M_NULLVAL;
...
#else
void setopt(string, filelenonly) char *string;
LOGICAL filelenonly;
#endif
{
        char *p;
        ...
        if (p = strchr(string, '=')) {
                *p = M_EOS;
                ...
                ok = (LOGICAL)((thisopt < NUMOPTIONS) &&
                               setvalopt(thisopt + 1, p + 1, filelenonly));
                *p = '=';
                if (!ok && !filelenonly)
                        m_mberr1("%s: Unrecognized option on command line or "
                                 "in option file",
                                 string);
                return;
        } /* End strchr(string, '=') */
...
#else
LOGICAL setvalopt(thisopt, string, filelenonly) int thisopt;
char *string;
LOGICAL filelenonly;
#endif
{
        ...
        /* Check for empty string (occurs if "option= val" is entered in .opt
         * file) */
        if (!*string)
                switch (thisopt) {
                case SEARCHKEY:
        ...
        switch (thisopt) {
        case SEARCHKEY:
                if ((parentsrch == TRUE) && (*string != M_EOS) &&
                    (*string != dirsep)
#if defined(MSDOS)
                    && (*(string + 1) != ':')
#endif
                ) {
                        /* prepend "..", for index processing in temporary
                         * sub-directory */
                        tempstr = (char *)m_malloc(strlen(string) + 4,
                                                   "tempstr search");
                        strcpy(tempstr, "..");
                        strcat(tempstr, SSEP);
                        strcat(tempstr, string);
                } else {
                        tempstr = (char *)m_malloc(strlen(string) + 1,
                                                   "tempstr search");
                ...
                *endpath = (SEARCH *)m_malloc(sizeof(SEARCH), "search path");
                ...
                if (p != tempstr && *(p - 1) != dirsep
                    ...
                    && *(p - 1) != ':'
                        ...
                        i = strlen(tempstr);
                        (*endpath)->directory =
                            (char *)m_malloc(i + 2, "search directory");
                        strcpy((*endpath)->directory, tempstr);
                        (*endpath)->directory[i] = dirsep;
                        (*endpath)->directory[i + 1] = M_EOS;
                } else {
                        (*endpath)->directory = (char *)m_malloc(
                            strlen(tempstr) + 1, "search directory");
                        strcpy((*endpath)->directory, tempstr);
                }
                (*endpath)->next = NULL;
        ...
        case ONERROR:
                if (!m_mbmbupstrcmp(string, "GO"))
                        ...
                        m_mberr1("Invalid ONERROR option: %s", string);
                return (TRUE);
        default:
                return (FALSE);
        }
