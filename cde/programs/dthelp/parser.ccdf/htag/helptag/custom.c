...
void m_dumpline(file, line) M_WCHAR *file;
int line;
{
        char buffer[10];
        char *mbyte;
        ...
        m_errline(buffer);
        if (!file) {             /* no entity file */
                if (inputname) { /* use main input, instead.  Only if set
                                    though. */
...
void m_exit(status) int status;
{
        if (filefound) {
                if (m_outfile && (m_outfile != stdout)) {
                        if (have_index) { /* sph: is this really necessary? */
...
int m_getc(m_ptr) void *m_ptr;
{
        ...
        do {
                ...
                while (1) {
                        mbyte[length++] = c;
                        mbyte[length] = 0;
                        if (mblen(mbyte, length) != -1)
                                break; /* hurray! */
                        if (length ==
                            MB_CUR_MAX) { /* reached max without a hit */
                                m_error("An invalid multi-byte character was "
                                        "found in the input");
                                c = ' ';
                                length = 1;
                                break;
                        }
                        if ((c = getc((FILE *)m_ptr)) == EOF) { /* huh? */
                                m_error("End-of-file found in within a "
...
void *m_openent(entcontent) M_WCHAR *entcontent;
{
        ...
        for (searchp = path; searchp; searchp = searchp->next) {
                filename = (char *)m_malloc(strlen(searchp->directory) +
...
void *m_openfirst(M_NOPAR) {
        ...
        if (defaultext) {
                input = (char *)m_malloc(strlen(m_argv[1]) + strlen(".htg") + 1,
                                         "input file name");
                ...
                length = strlen(input);
                inputname =
                    (M_WCHAR *)m_malloc(length + 1, "saved input file name");
...
void m_setoptions() {
        ...
        if (m_argc > 2) {
                ...
                if (strchr(m_argv[2], 'o'))
                        tracetostd = TRUE;
                if (strchr(m_argv[2], 'O'))
                        tracetostd = TRUE;
                if (strchr(m_argv[2], 'x'))
                        showmem = TRUE;
                if (strchr(m_argv[2], 'X'))
                        showmem = TRUE;
                /* Option "p" tells the search path to look at paths one level
                   ...
                   The following two lines of code should be here, but they are
                moved to global START-CODE of tex.if.  For some reason, global
                START-CODE is executed before this procedure is called
                (parser.c), but we need to have the order reversed.  Moving this
                procedure to a spot earlier is risky, since we don't know full
                reasons of why things are in a particular order.  This way, we
                minimize the damage:
...
void m_startdoc() {
        ...
        if (!filelist) {
                texinit();
        } else { /* list files that make up document */
        ...
        if (showmem)
                showmemavail(); /* markup's X option, (causes an exit) */
}

/* Write debugging trace information */
void m_trace(p) char *p;
{
        if (tracetostd)
                fputs(p, stdout);
        else
