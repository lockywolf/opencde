...
#else
void outcall(textchar, outfile) M_WCHAR textchar;
FILE *outfile;
#endif
{
        if (textchar != M_RE)
        ...
        if (!abbrev || fabbrev) {
                ...
                if (savid)
                        shchar(textchar, &xstrlen,
                               (sizeof(xrefstring) / sizeof(M_WCHAR)),
                               xrefstring, idstring,
                               "Too many characters in corresponding "
                               "cross-reference",
                               &iderr);
...
M_WCHAR *m_entname;
{
        ...
        if (m_enttype == M_SDATA) {
                /* check for output of \needbegin, SDATA only */
                mbtowc(&wc, " ", 1);
                if (!m_wcmbupstrcmp(m_entname, "SIGSPACE"))
                        termchar(wc);
                else {
                        for (pc = m_ero; *pc; pc++) {
                                mbtowc(&wc, pc, 1);
