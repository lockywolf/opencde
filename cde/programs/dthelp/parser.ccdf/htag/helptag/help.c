...
/* Start an appendix */
void appstart(id, letter) M_WCHAR *id, *letter;
{
        M_WCHAR *p, *q, wnull;
        char *pc;
        int length;
        ...
        /* User-specified letter */
                ...
                if ((long)(int)xchapter != xchapter) {
                        m_err1("%s: Too many characters in appendix letter",
                               letter);
                        chapter++;
                } else
                        chapter = (int)xchapter;
                mbtowc(&wnull, "", 1);
                for (pc = chapstring, q = letter; *q != wnull;
                     pc += length, q++) {
                        char mbyte[32]; /* larger than the largest possible
                                           multibyte char */
                        length = wctomb(mbyte, *q);
                        if ((pc + length) >=
                            (&chapstring[sizeof(chapstring) - 1])) {
                                m_err1("%s: Too many characters in appendix "
                                       "letter",
                                       letter);
                                break;
                        }
...
void assert_hometopic_exists(M_NOPAR) {
        char *string = GetDefaultHeaderString(
            "UntitledElementDefaultHeadingString", M_SDATA, "Untitled");
        if (nohometopic) {
                char *pc;
                ...
                fprintf(outfile, "%s%s%s",
                        ...
                        pc ? pc : string, "</SIZE></WEIGHT></TYPE></TITLE>\n");
                if (pc)
...
#define CONTRIB "/usr/hphelp/bin/"
        ...
        /* fills ``install'' with path to ourself */
        {
                ...
                if (*(m_argv[0]) == '/') {
                        ...
                        if (access(m_argv[0], 1) == 0) {
                                ...
                                *p = M_EOS;
                                install =
                                    (char *)m_malloc(strlen(m_argv[0]) + 1,
                        ...
                        /* not fully specified, check each component of path for
                         * ourself */
                        ...
                        while (1) {
                                ...
                                if (access(buf, 1) == 0) {
                                        install = (char *)m_malloc(
                                            strlen(path) + 1,
                                            "installation directory");
                                        strcpy(install, path);
                                        none = 0;
                                }
                                /* else, not an error if we can't find a
                                 * particular one. */

                                path = ++cp;
                                if (quit || !none) {
                                        break; /* either out of paths, or we
                                                  found it. */
                                }
                        } /* end while */
                }
        ...
        if (p > m_argv[1]) {
                save = *p;
                *p = M_EOS;
                work = (char *)m_malloc(strlen(m_argv[1]) + 1,
                                        "working directory");
                strcpy(work, m_argv[1]);
                indir =
                    (char *)m_malloc(strlen(m_argv[1]) + 1, "input directory");
        ...
        if (!defaultext)
                *q = M_EOS;
        nodirbase =
            (char *)m_malloc(strlen(p) + 1, "basename without directory");
        strcpy(nodirbase, p);
        ...
        if (usingshortnames) {
                ...
                helpbase = (char *)m_malloc(strlen(work) + BASENAME_LIMIT +
                                                PRE_EXTENSION_LIMIT +
                                                strlen(".ext") + 1,
                                            "help basename");
                ...
                /* set up global helpbase and helpext */
                helpbase = (char *)m_malloc(strlen(work) + strlen(nodirbase) +
                                                PRE_EXTENSION_LIMIT +
                                                strlen(".ext") + 1,
                                            "help basename");
                strcpy(helpbase, work);
...
void chapstart(id, number, type) M_WCHAR *id, *number;
char type[]; /* values: locase "chapter" or "part" ONLY  */
{
        ...
        char *chapterPrefixString = GetDefaultHeaderString(
            "ChapterElementDefaultHeadingString", M_SDATA, "Chapter");
        char *chapterSuffixString = GetDefaultHeaderString(
            "ChapterSuffixElementDefaultHeadingString", M_SDATA, "");
        char *partPrefixString = GetDefaultHeaderString(
            "PartElementDefaultHeadingString", M_SDATA, "Part");
        char *partSuffixString = GetDefaultHeaderString(
            "PartSuffixElementDefaultHeadingString", M_SDATA, "");
        ...
        if (!number) {
                ...
                if (strlen(mbyte) + 1 > sizeof(chapstring)) {
                        ...
                        m_err2("%s: Too many characters in %s number", number,
                               wc);
...
M_WCHAR *checkdimen(val, paramname, elt) M_WCHAR *val;
M_WCHAR *paramname;
M_WCHAR *elt;
{
        ...
        if (valid == NULL) {
                ...
                m_err5("[%s] Illegal value of %s for %s%s%s", val, paramname,
                       wc_stago, elt, wc_tagc);
...
M_WCHAR *mb_checkdimen(val, paramname, elt) M_WCHAR *val;
char *paramname;
char *elt;
{
...
M_WCHAR *checkent(entcontent) M_WCHAR *entcontent;
{
        M_WCHAR *filename;
        SEARCH *searchp;
        ...
        if (access(mb_entc, 04) == 0) { /* entcontent is readable */
        ...
        for (searchp = path; searchp; searchp = searchp->next) {
                if (((dir_leng = strlen(searchp->directory)) + econt_leng) >=
                    2048) {
                        M_WCHAR *wc;

                        wc = MakeWideCharString(searchp->directory);
                        m_err2("%s and %s overflow the file name space", wc,
                               entcontent);
                        m_free(wc, "wide character string");
                        continue;
                }
...
/* End Error _DtMessage macro \starterrmsg call, check to see if Error
   _DtMessage head is user specified or default */
void checkmsghead(M_NOPAR) {
        char *string = GetDefaultHeaderString(
            "MessagesElementDefaultHeadingString", M_SDATA, "Messages");
        if (emsghead == DEFHEAD) { /* head not output yet */
                fprintf(outfile, "%s%s%s",
                        "<TITLE><TYPE serif><WEIGHT bold><SIZE 14>", string,
                        "</TYPE></WEIGHT></SIZE></TITLE>\n");
                emsghead = FALSE;
        } else if (emsghead == USERHEAD) { /* user specified head */
                emsghead = FALSE;
        }
        m_free(string, "GetDefaultHeaderString return");
} /* end checkmsghead() */
...
#if defined(M_PROTO)
M_WCHAR *checkreal(M_WCHAR *val, M_WCHAR *paramname, LOGICAL msgout,
                   M_WCHAR *elt)
#else
M_WCHAR *checkreal(val, paramname, msgout, elt) M_WCHAR *val, *paramname;
LOGICAL msgout;
M_WCHAR *elt;
#endif
{
        ...
        if (valid == NULL) {
                ...
                if (msgout) {
                        wc_stago = MakeWideCharString(m_stago);
                        wc_tagc = MakeWideCharString(m_tagc);
                        m_err5("[%s] Illegal value of %s for %s%s%s", val,
                               paramname, wc_stago, elt, wc_tagc);
                        m_free(wc_stago, "wide character string");
                        m_free(wc_tagc, "wide character string");
                }
        }
...
void exvexend(textsize) M_WCHAR *textsize;
{
        int tsize;
        ...
        } else { /* textsize is either SMALLER or SMALLEST */
...
#else
void exvexstart(type, textsize, listinfo) M_WCHAR *type, *textsize;
char *listinfo;
#endif
{
        int tsize;
        ...
        if (tsize == NORMAL) {
                fprintf(outfile,
                        "<PARAGRAPH%s firstindent %d leftindent %d "
                        "nowrap><TYPE serif><SPACING m>",
                        listinfo, LEFTINDENT, LEFTINDENT);
        } else { /* textsize is either SMALLER or SMALLEST */
                fprintf(outfile,
                        "<PARAGRAPH%s firstindent %d leftindent %d "
                        "nowrap><TYPE serif><SPACING m>",
...
int mb_getqualified(qualname, unqualname) char *qualname;
char *unqualname;
{
#if defined(hpux) || defined(_AIX) || defined(sun) || defined(USL) ||          \
    defined(__uxp__) || defined(linux)
        ...
        if (strlen(unqualname) < FNAMELEN)
                ...
                m_mberr1(
                    "Internal Error. File name too long for qualifying: %s",
                    unqualname);
                return (-1);
        }
    ...
    defined(__uxp__) || defined(linux)
        ...
        if (*fnp == CSEP) {
                ...
                if (strlen(gp) >= roomleft) {
                        m_mberr1("Internal error: possible stray pointer in "
                                 "getqualified(): %s",
                                 gp);
                        return (-1);
                }
        ...
        if (*fnp == CSEP) {
                ...
                *gp = CSEP;
                ...
                if (*curdir != *qualname) {
                        m_err1("Relative directory %s for non-current drive, "
                               "can't qualify",
                               unqualname);
                        return (-1);
                }
                if (strlen(curdir) > 3) {
                        if ((strlen(curdir + 3) + 1) <
                            roomleft) { /* "1" for SSEP */
                                strcpy(gp + 1, curdir + 3);
                                strcat(qualname, SSEP);
                                roomleft = roomleft - strlen(curdir + 3) -
                                           1; /* "1" for SSEP */
                        } else {
                                m_err1("Internal error. File name too long for "
                                       "qualifying: %s",
                                       unqualname);
                                return (-1);
                        }
                ...
                if (*p == '.') /* alias */
                {
                        if (*(p + 1) == '.') /* parent */
                        {
                                *strrchr(qualname, CSEP) = '\0';
                                ...
                                if (pp == NULL) /* FAIL */
                                {
                                        m_mberr1("Internal error. Failed in "
                                                 "qualifying %s",
                                                 unqualname);
                                        return (-1);
                                } else {
                        ...
                        if ((strlen(p) + 1) < roomleft) { /* "1" for SSEP */
                                strcat(qualname, p);
                                strcat(qualname, SSEP);
                                roomleft = roomleft - strlen(p) - 1;
                        } else {
                                m_mberr1("Internal error. File name too long "
                                         "for qualifying: %s",
                                         unqualname);
                                return (-1);
                        }
...
int getqualified(qualname, unqualname) M_WCHAR *qualname;
M_WCHAR *unqualname;
{
        int retval;
        char mb_qualname[FNAMELEN], mb_unqualname[FNAMELEN];
...
void handle_link_and_graphic(parent, gentity, gposition, ghyperlink, glinktype,
                             gdescription) M_WCHAR *parent,
    *gentity, *gposition, *ghyperlink, *glinktype, *gdescription;
{
        ...
        */
        ...
        if (gentity) {
                ...
                if (etype != M_SYSTEM) {
                        ...
                        wc_stago = MakeWideCharString(m_stago);
                        wc_tagc = MakeWideCharString(m_tagc);
                        m_err6("%s not a %s %s, as required for the ENTITY "
                               "parameter of %s%s%s",
                               gentity, wc_entsystem, wc_entkw, wc_stago,
                               m_parent(0), wc_tagc);
                        ...
                        if (f_content) {
                                ...
                                m_err2("Can't find file %s (declared in entity "
                                       "%s)",
                                       f_file, gentity);
                        }
                }
        ...
        if (gposition) {
                ...
                if (!m_wcupstrcmp(gposition, wc_left)) {
                        ...
                        if (!m_wcupstrcmp(gposition, wc_right)) {
                                ...
                                m_err1("Invalid value for gposition: `%s'",
                                       gposition);
                        }
        ...
        if (!glinktype) { /* default to type jump */
                          /* do nothing */
                ...
                if (!m_wcmbupstrcmp(glinktype, QJUMP)) {
                        ...
                        wc_stago = MakeWideCharString(m_stago);
                        wc_tagc = MakeWideCharString(m_tagc);
                        m_err3("%sLINK%s (glinktype=``%s'') did not match an "
                               "allowed value",
                               wc_stago, wc_tagc, glinktype);
                        m_free(wc_stago, "wide character string");
                        m_free(wc_tagc, "wide character string");
                }
        }
        ...
        if (ghyperlink) {
                ...
                if (glinktype || gdescription) {
                        ...
                        fprintf(m_errfile,
                                ...
                                m_stago, m_tagc);
                        fprintf(stderr,
                                "Beware of using a reserved value such "
                                "as:\n%s, %s, %s, %s, %s, or %s.\n",
                                QJUMP, QJUMPNEWVIEW, QDEFINITION, QEXECUTE,
                                QAPPDEFINED, QMAN);
                        fprintf(m_errfile,
                                "Beware of using a reserved value such "
                                "as:\n%s, %s, %s, %s, %s, or %s.\n",
                                QJUMP, QJUMPNEWVIEW, QDEFINITION, QEXECUTE,
                                QAPPDEFINED, QMAN);
                        m_errline("Use ``ghyperlink='' if the value contains "
                                  "non-alphabetics");
                        m_esuffix();
...
void item(id) M_WCHAR *id;
{
        char orderString[32];
        static char *ROMAN0[] = {"",  "I",  "II",  "III",  "IV",
                                 "V", "VI", "VII", "VIII", "IX"};
        static char *ROMAN10[] = {"",  "X",  "XX",  "XXX",  "XL",
                                  "L", "LX", "LXX", "LXXX", "XC"};
        static char *ROMAN100[] = {"",  "C",  "CC",  "CCC",  "CD",
                                   "D", "DC", "DCC", "DCCC", "CM"};
        ...
        if (lastlist->lastlist->type == ORDER)
                fprintf(outfile, "<PARAGRAPH leftindent %d",
                        ORDEREDLISTITEMINDENT);
        else
        ...
        case UALPHA:
                ...
                count %= 26;
                while (--metaCount >= 0)
                        strncat(orderString, &ALPHABET[count], 1);
                break;
        case ARABIC:
        ...
        if (lastlist->lastlist->type == ORDER) {
                fprintf(/* keep the '('s balanced */
                        outfile, "><LABEL><WEIGHT bold>%s%2s%c</WEIGHT></LABEL",
                        (strlen(orderString) > 1) ? "" : "\\", orderString,
                ...
                if (id) {
                        ...
                        setid(id, TRUE, FALSE, inchapter, chapstring, xrffile,
                              xrfline, TRUE);
        ...
        } else { /* Bullet, Check or Plain list */
                if (id)
                        m_error(
                            "Cross-reference ID in non-ORDER list not allowed");
                if (lastlist->lastlist->type == BULLET) {
                        fprintf(outfile,
                                "><LABEL><CHARACTERSET symbol>%c</></LABEL",
                                183 /* BULLET CHAR */);
                } else if (lastlist->lastlist->type == CHECK) {
                        /* no hollow square in the symbol charset, use hollow
                         * diamond */
                        fprintf(outfile,
                                "><LABEL><CHARACTERSET symbol>%c</></LABEL",
                                224 /* CHECK CHAR */);
                } else /* if ( lastlist->lastlist->type == PLAIN) */
                { /* above commented out to make this the default for MILSPEC
                     lists */
                        /* emit a hard space label to make things look right
                         * when TIGHT */
...
#else
void itoletter(n, start, dest, length) int n;
char start;
char *dest;
int length;
#endif
{
        ...
        if (n <= 0) {
                m_error(
                    "Internal error: converting non-positive number to letter");
                dest[0] = start;
                dest[1] = M_EOS;
                return;
        }
        for (p = invert; n; p++, n = (n - 1) / 26) {
                *p = start + (n - 1) % 26;
                if (p == &invert[length] || p == &invert[sizeof(invert)]) {
                        m_error("Internal error: out of space converting "
                                "number to letter");
                        break;
                }
        }
...
void manustart(language, idxvol, status) M_WCHAR *language, *idxvol, *status;
{
        if (idxvol)
                w_strcpy(volume, idxvol);
...
void notimp(eltname, option) char *eltname;
char *option;
{
        char *p;
        ...
        m_err4("%s%s%s not suppported in %s", wc_stago, wc_eltname, wc_tagc,
               wc_option);
...
void open_new_helpfile() {
        ...
        if ((j = strlen(htfilenostring)) > PRE_EXTENSION_LIMIT) {
                ...
                m_err2("Error:  Tail of new output help file, `%s', is longer "
                       "than %s characters",
                       wc_1, wc_2);
        ...
        switch (j) {
        case 0:
                m_error("Fatal Error: Null tail on new help file.");
                m_exit(m_errexit);
                break;
        case 1:
                *helpext = '0';
...
M_WCHAR *okdimen(val) M_WCHAR *val;
{
        /* returns val if a valid TeX dimension, else NULL */
...
/* Start a rsect */
void rsectstart(id, pagebreak) M_WCHAR *id, *pagebreak;
{
        savid = checkid(id);
        iderr = FALSE;
        if (rsectseq) {
                ...
                if (pagebreak && (rsectsame != (LOGICAL)(!m_wcupstrcmp(
                                                   pagebreak, wc_samepage)))) {
                        m_err1("%s specification ignored within a sequence of "
                               "reference sections",
                               pagebreak);
                }
        ...
        if (rsectsame && m_mblevel("S3"))
                st4 = TRUE;
        didabbrev = FALSE;
        if (!rsectsame) { /* Finish off NEED BEGIN if necessary */
                if (m_mblevel("S3")) {
...
M_WCHAR *searchforfile(file) M_WCHAR *file;
{
        M_WCHAR *filename;
        SEARCH *searchp;
        ...
        if (!access(mb_file, READABLE)) {
                filename = (M_WCHAR *)m_malloc(w_strlen(file) + 1,
                                               "figure/icon filename");
        ...
        for (searchp = path; searchp; searchp = searchp->next) {
                if (((dir_leng = strlen(searchp->directory)) + file_leng) >=
                    2048) {
                        M_WCHAR *wc;
...
#else
void starthelpnode(id, suppress_topic_map) M_WCHAR *id;
...
#endif
{
        ...
        if (!suppress_topic_map) {
                ...
                if (thisnodelevel ==
                    lastnodelevel) { /* this node at same level */
                        if (!isfirsttopicmap) {
                                fputs("\n", topicmapfp);
                        } else {
                        ...
                        /* indent routine >here< */
                        for (i = thisnodelevel; --i >= 0;) {
                                fputs("  ", topicmapfp);
                        }
                        fprintf(topicmapfp, "%s", mb_nodeid);
                }
                /* elseif */
                if (thisnodelevel > lastnodelevel) { /* this node is deeper */
                ...
                /* else */
                if (thisnodelevel < lastnodelevel) { /* this node is higher */
                        fputs("\n", topicmapfp);
                        while (thisnodelevel != lastnodelevel--) {
                                /* indent routine >here< */
                                for (i = lastnodelevel; --i >= 0;) {
                                        fputs("  ", topicmapfp);
                                }
                                /* keep the '{' balanced */
                                fputs("}\n", topicmapfp);
...
#else
void mb_starthelpnode(id, suppress_topic_map) char *id;
LOGICAL suppress_topic_map; /* should we suppress .tpc entry */
#endif
{
        M_WCHAR *wc;
...
void startlablist(longlabel, width, spacing) M_WCHAR *longlabel, *width,
    *spacing;
{
        STARTSTUFF;
        rshnewclear();
...
/* Start a list */
void startlist(type, order, spacing, cont) M_WCHAR *type, *order, *spacing,
    *cont;
{
        LIST *nextlist;
        CONTCHAIN *chain;
        ...
        if (type && !m_wcmbupstrcmp(type, QORDER))
                fprintf(outfile, "<PARAGRAPH firstindent %d leftindent %d>\n",
                        ORDEREDLISTFIRSTINDENT, ORDEREDLISTFIRSTINDENT);
        else
                fprintf(outfile, "<PARAGRAPH firstindent %d leftindent %d>\n",
                        LISTFIRSTINDENT, LISTFIRSTINDENT);
        ...
        if (cont) {
                if (order && (vordertype(order) != lastlist->order)) {
                        m_error("Unable to continue a list and change the "
                                "numbering scheme");
                        cont = NULL;
                }
                ...
                if (type && (vtype(type) != lastlist->type)) {
                        m_error(
                            "Unable to continue a list and change its type");
                        cont = NULL;
                }
        }

        if (!cont) { /* clear old list? */
        ...
        if (spacing && !m_wcmbupstrcmp(spacing, QTIGHT))
                lastlist->space = TIGHT;

        if (type && order && m_wcmbupstrcmp(type, QORDER))
                m_err2("Incompatible specification for list: %s and %s", type,
                       order);
...
void testindexfile(first) FILE *first;
{
        char *p;
        int c;
...
void texinit(M_NOPAR) {
        ...
        if (!xrf)
                ...
                for (p = m_signon; *p; p++) {
                        ic = getc(xrf);
                        if (ic != (int)*p) {
                                m_errline("Output files from different version "
                                          "of Tag, regenerating. . .\n");
                                fclose(xrf);
                                xrf = NULL;
                                rebuild = TRUE;
        ...
        savehelpfilename =
            (M_WCHAR *)m_malloc(strlen(helpbase) + 1, "help file name");
        ...
        while (name = m_cyclent(init, &type, &content, &wheredef)) {
                ...
                if (type == M_SYSTEM) {
                        ...
                        if (w_strchr(content, wbs) || w_strchr(content, wsl) ||
                            w_strchr(content, wcl)) {
                                m_err2("Avoid directory names in FILE entity "
                                       "%s: %s",
                                       name, content);
...
/* Issue error message and text in document when starting an unimplemented
   element */
void mb_unimp(eltname) char *eltname;
{
...
fprintf(outfile, " not implemented for HelpTag. </WEIGHT></PARAGRAPH>\n\n");
        ...
        m_err3("%s%s%s is not implemented for HelpTag", wc_stago, wc_eltname,
               wc_tagc);
...
void unimp(eltname) M_WCHAR *eltname;
{
        char *mb_eltname;
...
long usertosp(s) char *s;
{
        ...
        temp[0] = M_EOS;
        if (sscanf(s, "%lf%s%lf", &num, temp, &num2) ==
            3) { /* get numeric portion */
                M_WCHAR *wc_s, *wc_temp;

                wc_s = MakeWideCharString(s);
                wc_temp = MakeWideCharString(temp);
                m_err2("%s: Ignoring number after %s in Tag dimension", wc_s,
                       wc_temp);
...
char *
#if defined(M_PROTO)
GetDefaultHeaderString(char *elementName, unsigned char desiredType,
                       char *defaultString)
#else
    GetDefaultHeaderString(elementName, desiredType,
                           defaultString) char *elementName;
        ...
        if (m_lookent(wc_elementName, &type, &content, &wheredef)) {
                if (type == desiredType) {
                        ...
                        if (*content) {
                                ...
                                if (!path) {
                                        m_err2("Can't find file %s (declared "
                                               "in entity %s)",
                                               content, wc_elementName);
                                        m_free(wc_elementName,
...
void SetDefaultLocale() {
        ...
        if (m_lookent(elementName, &type, &content, &wheredef)) {
                if (type == M_SDATA) {
                        ...
                        helpcharset =
                            m_malloc(w_strlen(content) + 1, "help charset");
...
#if defined(M_PROTO)
void paragraph(M_WCHAR *indent, M_WCHAR *id, M_WCHAR *gentity,
               M_WCHAR *gposition, M_WCHAR *ghyperlink, M_WCHAR *glinktype,
               M_WCHAR *gdescription, char *listinfo)
#else
void paragraph(indent, id, gentity, gposition, ghyperlink, glinktype,
               gdescription, listinfo) M_WCHAR *indent;
M_WCHAR *id;
M_WCHAR *gentity;
M_WCHAR *gposition;
M_WCHAR *ghyperlink;
...
#endif
{
        if (id) {
                ...
                setid(id, TRUE, FALSE, inchapter, chapstring, xrffile, xrfline,
                      FALSE);
        ...
        if (indent) {
                fprintf(outfile, " firstindent %d leftindent %d", LEFTINDENT,
                        LEFTINDENT);
        }

        handle_link_and_graphic(m_parent(0), gentity, gposition, ghyperlink,
                                glinktype, gdescription);
...
#if defined(M_PROTO)
void figure(M_WCHAR *number, M_WCHAR *tonumber, M_WCHAR *id, M_WCHAR *file,
            M_WCHAR *figpos, M_WCHAR *cappos, M_WCHAR *oldtype, M_WCHAR *xwidth,
            M_WCHAR *xdepth, M_WCHAR *xhadjust, M_WCHAR *xvadjust,
            M_WCHAR *border, M_WCHAR *type, M_WCHAR *xmagnify, M_WCHAR *video,
            M_WCHAR *strip, M_WCHAR *mirror, M_WCHAR *margin, M_WCHAR *clip,
            M_WCHAR *penwidth, M_WCHAR *snap, M_WCHAR *autoscale,
            M_WCHAR *plottype, M_WCHAR *callout, M_WCHAR *textsize,
            M_WCHAR *ghyperlink, M_WCHAR *glinktype, M_WCHAR *gdescription,
            char *listinfo)
#else
void figure(number, tonumber, id, file, figpos, cappos, oldtype, xwidth, xdepth,
            xhadjust, xvadjust, border, type, xmagnify, video, strip, mirror,
            margin, clip, penwidth, snap, autoscale, plottype, callout,
            textsize, ghyperlink, glinktype, gdescription,
            listinfo) M_WCHAR *number;
M_WCHAR *tonumber;
M_WCHAR *id;
M_WCHAR *file;
M_WCHAR *figpos;
...
#endif
{
        unsigned char etype, wheredef;
        char *string = GetDefaultHeaderString(
            "FigureElementDefaultHeadingString", M_SDATA, "Figure");
        ...
        if (number) {
                ...
                if (!ftonumber)
                        m_err1("Explicit figure number %s inconsistent with "
                               "NONUMBER option",
                               number);
        }
        svheadlen = 0;
        ...
        */
        ...
        if (file) {
                ...
                if (etype != M_SYSTEM) {
                        ...
                        m_err6("%s not a %s %s, as required for the ENTITY "
                               "parameter of %s%s%s",
                               file, wc_entsystem, wc_entkw, wc_stago,
                               m_parent(0), wc_tagc);
                        m_free(wc_stago, "wide character string");
                        m_free(wc_tagc, "wide character string");
                        m_free(wc_entsystem, "wide character string");
        ...
        /* if caption position is not specified, have it track the figure
         * position */
        if (!cappos)
                cappos = figpos;
...
#else
void endterm(base, gloss, linktype) M_WCHAR *base;
M_WCHAR *gloss;
int linktype;
#endif
{
...
#else
M_WCHAR wc_toupper(wc) M_WCHAR wc;
#endif
{
        if ((wc >= 0) && (wc <= 255)) {
                return _toupper(wc);
