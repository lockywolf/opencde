 ...
 */
/* $XConsortium: scan.c /main/3 1995/11/08 11:30:56 rswiston $ */
/*
...
void m_entexpand(openent) M_ENTITY *openent;
{
        M_WCHAR *p;
        M_HOLDTYPE dchar;
        char buffer[10];
        ...
        if (!openent->wheredef) {
                m_eopencnt--;
                m_err1(
                    "%s: System error -- no definition for predeclared entity",
                    openent->name);
                m_eopencnt++;
                return;
        }
        if (m_curcon == ENTINLIT)
                if (openent->type != M_GENERAL) {
                        m_eopencnt--;
                        m_err1(
                            "%s: Typed entity not allowed in parameter value",
                            openent->name);
                        m_eopencnt++;
                        return;
                }
        ...
        for (i = 0; i < m_eopencnt - 1; i++)
                if (m_opene[i] == openent) {
                        m_eopencnt--;
                        m_err1("Recursive call to entity %s ignored",
                               openent->name);
                        m_eopencnt++;
                        return;
                }
        ...
        if (m_curcon == PREAMBLE &&
            (openent->type == M_SDATA || openent->type == M_CODESDATA ||
             openent->type == M_PI || openent->type == M_CODEPI)) {
                m_startdoc();
                m_curcon = START;
                m_adjuststate();
        }
                ...
                if (!m_stacktop->intext) {
                        if (!m_strtproc(M_NULLVAL)) {
                                if (m_stacktop->oldtop)
                                        m_err1(
                                            "SDATA entity not allowed at this "
                                            "point in %s",
                                            m_nameofelt(m_stacktop->element));
                                else if (!m_start)
                                        m_error("Document may not start with "
                                                "SDATA entity");
                        }
                        m_start = TRUE;
...
LOGICAL m_gendelim(srlen, context) int srlen;
int context;
{
        int ghold[MAXD + 1];
        int ucase;
        ...
        linestart = TRUE;
        for (i = 0; i <= srlen; i++)
                if (m_srefchartype[i] != M_RSCHAR &&
                    m_srefchartype[i] != M_WSCHAR) {
                        linestart = FALSE;
                        break;
                }
        if (linestart)
        ...
        while (TRUE) {
                ...
                if (oldchars <= srlen)
                        ...
                        if (dhold[n] != M_NORMAL && dhold[n] != M_ENTNORMAL)
                                break;
                }
                for (i = current;
        ...
        while (n >= newcharstart - 1) {
                ...
                if (delim[n]) {
                        /* Found a delimiter. If it ends with a letter, verify
                           that the following character is not a letter, in
                           order to issue error messages in cases such as
                           <!ENTITYrunon ... */
                        if (m_cttype(ghold[n]) != M_NMSTART)
                                found = TRUE;
                        else {
...
/* Reads next input character from the current source file or from an
   entity expansion */
int m_getachar(dchar) M_HOLDTYPE *dchar;
{
        int c;
        int i;
        char buffer[10];
        ...
        if (m_toundo && m_sysecnt <= m_sourcefile[m_toundo - 1]) {
                ...
                *dchar = M_NORMAL;
                if (m_whitespace((M_WCHAR)c) && c != wc_re) {
                        ...
                        for (m_wscount = 0; m_wscount < M_WSPACELEN;
                             m_wscount++) {
                                m_wspace[m_wscount] = m_actgetc();
                                if (!m_whitespace(
                                        (M_WCHAR)m_wspace[m_wscount]) ||
                        ...
                        if (m_wscount > m_maxws)
                                m_maxws = m_wscount;
                        if (m_wspace[m_wscount] == wc_re)
                                c = wc_re;
                        else {
                                for (i = 0; i <= m_wscount; i++)
                                        m_ungetachar(m_wspace[m_wscount - i],
                                                     M_NORMAL, FALSE);
                        }
...
#else
void m_getname(first) M_WCHAR first;
#endif
{
        M_WCHAR *p;
        M_HOLDTYPE dchar;
...
int m_gettoken(c, dchar, context) int *c;
M_HOLDTYPE *dchar;
int context;
{
        ...
        while (n >= 0) {
                ...
                if (delim[n]) {
                        /* Found a delimiter. If it ends with a letter, verify
                           ...
                           order to issue error messages in cases such as
                           <!ENTITYrunon ... */
                        if (m_cttype(hold[n]) != M_NMSTART)
                                found = TRUE;
                        else {
                                *c = m_getachar(dchar);
                                m_ungetachar(*c, *dchar, TRUE);
                                if (*c == EOF || m_cttype(*c) != M_NMSTART)
                                        found = TRUE;
                        }
                }
                if (found) {
                                            ...
                                            m_gettoken(c, dchar,
                                                       (m_curcon == RCNEWENT ||
                                                        m_curcon == ENTINLIT)
                                                           ? m_curcon
                                                           : context));
                        /* Can be an M_ERO or M_CRO here only if not in context
                           and hence should not be treated as a delimiter */
                        if (delim[n] != M_STAGO && delim[n] != M_ETAGO &&
                            delim[n] != M_ERO && delim[n] != M_CRO)
                                return (delim[n]);
                        /* M_STAGO and M_ETAGO recognized only if immediately
                           followed by a M_NMSTART character or by an
                           appropriate closing delimiter (latter is a short tag)
                         */
                        if (delim[n] == M_STAGO || delim[n] == M_ETAGO) {
                                next = m_getachar(dchar);
                                m_ungetachar(next, *dchar, TRUE);
                                if (m_cttype(next) == M_NMSTART &&
                                     ...
                                     *dchar == M_ENTNORMAL))
                                        return (delim[n]);
                                nexttoken = m_gettoken(
                                    &next, dchar,
                                    delim[n] == M_STAGO ? SELEMENT : EELEMENT);
                                if (nexttoken) {
                                        m_undodelim(m_dlmptr[nexttoken - 1],
                                                    TRUE);
                                        return (delim[n]);
                                } else
                                        m_ungetachar(next, *dchar, TRUE);
                ...
                if (n)
                        m_ungetachar(hold[n], dhold[n], TRUE);
                n--;
        }
...
/* Reads a literal */
void m_litproc(delim) int delim;
{
        int n, i;
        M_HOLDTYPE dchar;
...
#else
void m_missingtagc(c, dchar, start) int c;
M_HOLDTYPE dchar;
LOGICAL start;
#endif
{
...
#else
void m_nextdelimchar(n, i, linestart, newlinestart, skipblank, type) int *n;
int i;
LOGICAL *linestart;
...
int m_scan(M_NOPAR) {
        ...
        while (TRUE) {
                ...
                if (dchar == wc_ee) {
                        m_eopencnt--;
                        if (m_element[m_stacktop->element - 1].content ==
                            M_RCDATA) {
                        ...
                        if (m_newcon(m_curcon - 1, M_ENTITYEND - 1))
                                return (M_ENTITYEND);
                        continue;
                }
                ...
                if (m_whitespace((M_WCHAR)c)) {
                        if (!m_newcon(m_curcon - 1, M_TEXT - 1))
                                continue;
                        if (c != wc_re || m_curcon == PROCINT ||
                            m_curcon == LITCON || m_curcon == LITENT ||
                            m_curcon == LITAENT) {
                                m_scanval = c;
                                return (M_TEXT);
                        }
                ...
                if (((m_curcon == SELEMENT || m_curcon == EELEMENT ||
                      m_curcon == ENTNAME || m_curcon == MAPNAME ||
                      m_curcon == AMAPNAME) &&
...
#else
void m_setmap(map, useoradd) int map;
LOGICAL useoradd;
#endif
{
        int i;
        ...
        if (map == 1) {
                if (m_stacktop->map &&
                    m_stacktop->oldtop->map != m_stacktop->map)
                ...
                if (m_element[m_stacktop->element - 1].srefptr)
                        m_setmap(m_element[m_stacktop->element - 1].srefptr,
                                 (LOGICAL)m_element[m_stacktop->element - 1]
                                     .useoradd);
                return;
        }
        ...
        if (!m_stacktop->map || m_stacktop->map == m_stacktop->oldtop->map) {
                m_stacktop->map = (int *)m_malloc(sizeof(int) * M_SREFCNT,
                                                  "short reference map");
        ...
        for (sref = m_map[map - 2]; sref; sref = m_sref[sref - 1].next)
                m_stacktop->map[m_sref[sref - 1].sref - 1] =
                    m_sref[sref - 1].entity;
}

/* Check for short reference delimiters */
void m_shortref(context) int context;
{
        int n = 0;
        int i;
        int c;
        ...
        if (sizeof(m_sreftree) / sizeof(M_PTRIE) == 1)
                return;

        /* Can return if using MARKUP extensions and no map is active */
        if (!m_conform && !m_stacktop->map)
                return;
        ...
        while (TRUE) {
                ...
                while (TRUE) {
                        ...
                        if (linestart && m_srefchartype[n] >= M_RSCHAR) {
                                for (i = m_current[n];
                                     m_sreftree[i].more &&
                                     m_sreftree[i].symbol < RS;
                                     i++)
                                        ;
                                if (m_sreftree[i].symbol == RS) {
                                        m_nextdelimchar(&n, i, &linestart,
                                                        FALSE, FALSE, M_RSCHAR);
                                        continue;
                                }
                        }
                ...
                while (TRUE) {
                        if (m_delim[n]) {
                                /* Found a delimiter. If letters were allowed in
                                short references would check here for runon
                                situations such as <!ENTITYrunon ... */
                                if (m_gendelim(n, context)) {
                                        for (i = n; i >= 0; i--)
                                                if (m_srefchartype[i] <
                                                    M_WSCHAR)
                                                        m_ungetachar(m_hold[i],
                                                                     m_dhold[i],
                                                                     TRUE);
                                        return;
                                }
                                ...
                                if (linestart)
                                        m_atrs = FALSE;
                                if (m_stacktop->map &&
                                    m_stacktop->map[m_delim[n] - 1]) {
                                        m_entexpand(
                                            &m_entities[m_stacktop->map
                                                            [m_delim[n] - 1] -
                                                        1]);
                                        return;
                                }
...
void m_sigre(M_NOPAR) {
        ...
        if (m_start && (!m_stacktop->firstre &&
                        m_oldlinestat[m_oldlsindex] != M_DCORCET)) {
                m_stacktop->firstre = TRUE;
                return;
        }
        /* Check for line containing other than data characters or contextual
           subelements */
        if (m_start && m_oldlinestat[m_oldlsindex] == M_SOMETHING)
                return;
...
#else
void m_undodelim(delim, flag) M_WCHAR *delim;
LOGICAL flag;
#endif
{
        M_WCHAR *p;
...
#else
void m_ungetachar(c, dchar, preread) int c;
M_HOLDTYPE dchar;
LOGICAL preread;
#endif
{
...
LOGICAL m_vldentref(M_NOPAR) {
        ...
        if (next != EOF && m_cttype(next) == M_NMSTART && dchar != wc_ee) {
                ...
                if (openent = (M_ENTITY *)m_lookfortrie(m_name, m_enttrie))
                        ...
                        m_err1("Reference to undefined entity '%s'", m_name);
                return (TRUE);
        }
        m_ungetachar(next, dchar, TRUE);
        return (FALSE);
