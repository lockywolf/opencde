...
void actptrout(array, name) ACTION **array;
char *name;
{
        int i;
...
#else
void closeiffile(flag, file, count, table, proto) LOGICAL flag;
FILE *file;
int count;
char *table;
char *proto;
#endif
{
        ...
        if (count) {
                fprintf(file,
                        ...
                        table, proto);
                fprintf(file,
                        "  m_%s1, /* Place holder for 1-based indexing */\n",
                        table);
                for (i = 1; i <= count; i++) {
...
#else
void endcode(flag, file) LOGICAL flag;
FILE *file;
#endif
{
        if (flag)
...
void endsignon(M_NOPAR) {
        ...
        len = sochar + strlen(signon1) + strlen(signon2) + strlen(iffile) +
              strlen(timeofday)
...
void endstring(M_NOPAR) {
        ...
        /* If called after ENDFILE, ensure at least one character in output
           file*/
        else if (stringstart)
                fprintf(string, "  0");
}
...
/* Returns pointer to data field in action node for current chain of
   elements */
int *getaction(array) ACTION **array;
{
        ACTION *start, *node;
        CHAIN *chainp;
        ...
        if (start->data)
                m_error("Duplicate specification");
        freechain();
        return (&start->data);
}
...
void initialize(M_NOPAR) {
        ...
        if (debug) {
                tfile = sfile = efile = stfile = entfile = pfile;
        } else {
...
void startcode(caseno, flag, file, prefix, proto, formal, formtype) int caseno;
LOGICAL *flag;
FILE *file;
char *prefix;
char *proto;
...
void storecvar(M_NOPAR) {
        ...
        new->cvarptr =
            (M_WCHAR *)m_malloc(w_strlen(name) + 1, "C variable name");
...
void storepname(M_NOPAR) {
        ...
        for (i = 0, par = m_element[openelt - 1].parptr;
             i < m_element[openelt - 1].parcount; i++, par++)
        ...
        if (m_partype(cvarlist->param + m_element[openelt - 1].parptr, p)) {
                if (pval->line) {
                        if (w_strcmp(pval->value, p)) {
                                ...
                                m_err5("Can't #define %s to %s. %s #define'd "
                                       "to %s on line %s",
                                       pval->cname, p, pval->cname, pval->value,
                                       w_buffer);
                                m_free(w_buffer, "wide character string");
                        }
                } else {
                        pval->value =
                            (M_WCHAR *)m_malloc(w_strlen(p) + 1, "pval value");
                        w_strcpy(pval->value, p);
                        pval->line = m_line;
                }
        } else
                m_err3("\"%s\" illegal value for parameter %s of %s", p,
                       &m_pname[m_parameter[cvarlist->param +
                                            m_element[openelt - 1].parptr - 1]
                                    .paramname],
                       &m_ename[m_element[openelt - 1].enptr]);
}
