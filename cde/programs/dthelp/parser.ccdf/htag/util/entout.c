 ...
 */
/* $XConsortium: entout.c /main/3 1995/11/08 11:38:59 rswiston $ */
...
void entout(fname) char *fname;
{
        ...
        if (m_entcnt) {
                for (ent = firstent, conindex = 0, nameindex = 0; ent;
                     ent = ent->next) {
                        if (ent->content)
        ...
        if (m_entcnt) {
                ...
                for (ent = firstent, conindex = 0, nameindex = 0; ent;
                     ent = ent->next) {
                        fprintf(entfile, "  %s, %s, ", typetype(ent->type),
                        ...
                        if (ent != lastent)
                                fprintf(entfile, ", &m_entities[%d], 0",
                                        ent->index);
                        else
                                fputs(", NULL, 0", entfile);
#else
        ...
        if (m_enttrie->data)
                countdown(m_enttrie, &count);
        fprintf(entfile, "M_ENTEXTERN M_TRIE m_enttrie[%d]\n", count);
        if (m_enttrie->data) {
                count = 0;
                fputs("#if defined(M_ENTDEF)\n  = {\n  0, NULL, &m_enttrie[1]",
                      entfile);
                m_dumptrie(entfile, m_enttrie->data, "m_enttrie", &count,
                           entptr);
                fprintf(entfile, "}\n#endif\n");
        }
        fprintf(entfile, "  ;\n\n");
...
#if defined(BUILDEXTERN)
        if (m_entcnt) {
                fputs("M_ENTEXTERN M_ENTITY *firstent "
                      "M_ENTINIT(&m_entities[0]) ;\n",
                      entfile);
                fprintf(entfile,
                        "M_ENTEXTERN M_ENTITY *lastent "
                        "M_ENTINIT(&m_entities[%d]) ;\n",
                        m_entcnt - 1);
        } else {
...
#else
char *typewhere(n) unsigned char n;
#endif
{
        switch (n) {
        case M_DBUILD:
