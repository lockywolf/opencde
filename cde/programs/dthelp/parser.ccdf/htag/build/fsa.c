...
#if defined(M_PROTO)
int addarc(STATE *from, STATE *to, ELTSTRUCT *label, ANDGROUP *and,
           LOGICAL optional, int id, LOGICAL minim, ELTSTRUCT **errelt)
#else
int addarc(from, to, label, and, optional, id, minim, errelt) STATE *from, *to;
ELTSTRUCT *label;
ANDGROUP *and;
LOGICAL optional;
int id;
...
void checkand(andstart, andptr, start, root, errelt) ANDGROUP *andstart,
    *andptr;
STATE *start;
TREE *root;
ELTSTRUCT **errelt;
        ...
        for (parc = start->first; parc; parc = parc->next) {
                if (parc->group) {
                        if (parc->group != andstart)
                                for (pand = parc->group; pand;
                                     pand = pand->next)
                                        checkand(andstart, andptr, pand->start,
                                                 root, errelt);
                } else if (c = checkdfsa(andptr->start, parc->label,
                                         parc->group, parc->id, errelt))
...
int checkdfsa(from, label, and, id, errelt) STATE *from;
ELTSTRUCT *label;
ANDGROUP *and;
int id;
ELTSTRUCT **errelt;
        ...
        for (parc = from->first; parc; parc = parc->next) {
                if (parc->group) {
                        ...
                        for (group = parc->group; group; group = group->next)
                                if (c = checkdfsa(group->start, label, and, id,
                                                  errelt))
                                        return (c);
...
int checkrepeat(from, and, errelt) STATE *from;
ANDGROUP *and;
ELTSTRUCT **errelt;
{
        ARC *parc;
        ...
        for (; and; and = and->next)
                for (parc = and->start->first; parc; parc = parc->next) {
                        if (parc->group)
                                if (c = checkrepeat(from, parc->group, errelt))
                                        return (c);
                                else
                                        ;
                        else if (c = checkdfsa(from, parc->label, M_NULLVAL,
                                               parc->id, errelt))
                                return (c);
                        else
                                ;
...
void copyintolist(from, to) STATELIST *from, **to;
{
        STATELIST **new, *old;
        ...
        for (; from; from = from->next) {
                if (notinlist(from, old)) {
                        *new = (STATELIST *)m_malloc(sizeof(STATELIST),
                                                     "state list");
...
void dellist(list) STATELIST **list;
{
        STATELIST *p, *q;
...
void makeand(canbenull, root, optional) LOGICAL *canbenull;
TREE *root;
int optional;
{
        TREE *child;
        ...
        for (child = root->first; child; child = child->right) {
                ...
                if (andptr->start->datacontent)
                        for (index = top->oldtop->starts; index;
                             index = index->next)
                                index->value->datacontent = TRUE;
                if (!groupbenull)
                        *canbenull = FALSE;
                /* Check for ambiguity between start state of branch just
                   completed and start state of the and-group (i.e., parent of
                   branch just completed) */
                for (start = top->oldtop->starts; start; start = start->next)
                        checkand(saveand, andptr, start->value, root, &errelt);
                /* Check for ambiguity between start state of branch just
                   completed and final states of previous branches */
                ...
                /* Check for ambiguity between start states of branch just
                   completed and start states of previous branches */
                for (otherand = saveand; otherand != andptr;
                     otherand = otherand->next)
                        checkand(saveand, andptr, otherand->start, root,
                                 &errelt);
        }
        pop();
        if (*canbenull)
...
/* Makefsa builds the portion of an FSA that corresponds to an entire
   ...
   */
LOGICAL makefsa(root, optional) TREE *root;
int optional;
{
        LOGICAL canbenull;
...
void makeseq(canbenull, root, optional) LOGICAL *canbenull;
TREE *root;
int optional;
{
        LOGICAL branchnull;
...
/* Nondeterm issues a diagnostic when a nondeterministic model is
   encountered */
void nondeterm(root, c, eltp) TREE *root;
int c;
ELTSTRUCT *eltp;
{
        M_WCHAR *wtemp;
...
LOGICAL notinlist(item, list) STATELIST *item, *list;
{
...
LOGICAL permitspcd(a) ARC *a;
{
        ...
        if (a->group) {
                for (pand = a->group; pand; pand = pand->next)
                        for (b = pand->start->first; b; b = b->next)
                                if (permitspcd(b))
                                        return (TRUE);
                return (FALSE);
        }
...
LOGICAL regenerate(start, stop) TREE *start, *stop;
{
        TREE *child;
        ...
        if (start->terminal) {
                ...
                for (child = start->first; child; child = child->right) {
                        if (regenerate(child, stop))
                                return (TRUE);
                        if (child->right) {
                                if (start->connector == SEQ)
                                        msg1line("%s", seq);
                                if (start->connector == OR)
                                        msg1line("%s", or);
                                if (start->connector == AND)
                                        msg1line("%s", and);
                        }
                }
                msgline(")");
        ...
        if (start->occurrence == OPT)
                msg1line("%s", opt);
        if (start->occurrence == PLUS)
                msg1line("%s", plus);
        if (start->occurrence == REP)
                msg1line("%s", rep);
...
void repeat(root) TREE *root;
{
        ...
        for (a = top->starta; a; a = a->next) {
                for (final = top->allfinal; final; final = final->next) {
                        if (a->group)
                                if (c = checkrepeat(final->value, a->group,
                                                    &errelt)) {
                                        wtemp = MakeWideCharString(
                                            root->occurrence == PLUS ? plus
                                                                     : rep);
                                        warning1("Conflict in use of %s",
                                                 wtemp);
                                        m_free(wtemp, "wide character string");
                                        nondeterm(root, c, errelt);
                                } else
                                        ;
                        else if (c = checkdfsa(final->value, a->label, a->group,
                                               a->id, &errelt))
                                nondeterm(root, c, errelt);
                        else
                                ;
                ...
                for (final = top->finals; final; final = final->next) {
                        if (samelabelarc(a, final->value))
                                continue;
                        if (a->group)
                                if (c = checkrepeat(final->value, a->group,
                                                    &errelt))
                                        nondeterm(root, c, errelt);
                        if (a->label || a->group || !final->value->frompcdata) {
                                if (c = addarc(final->value, a->to, a->label,
                                               a->group, TRUE, a->id, a->minim,
                                               &errelt))
                                        nondeterm(root, c, errelt);
                                if (permitspcd(a))
                                        final->value->datacontent = TRUE;
                        }
                }
        }
}
...
LOGICAL samelabelarc(a, s) ARC *a;
STATE *s;
{
        ARC *b;
...
#else
void savelhs(param) LOGICAL param;
#endif
{
        STATE *end;
        ELTSTRUCT *errelt;
        ...
        *nextlhs = (LHS *)m_malloc(sizeof(LHS), "lhs");
        ...
        if (!startstate) {
                ...
                addarc(startstate, end, thiselt, M_NULLVAL, FALSE, 1, FALSE,
                       &errelt);
                end->final = TRUE;
        }
...
void simplebranch(root, value, group, optional) TREE *root;
ELTSTRUCT *value;
ANDGROUP *group;
int optional;
{
        ...
        for (index = top->allfinal; index; index = index->next)
                if (c = checkdfsa(index->value, value, group, root->eltid,
                                  &errelt))
                        nondeterm(root, c, errelt);
        for (index = top->starts; index; index = index->next) {
                ...
                c = addarc(index->value, new, value, group,
                           (LOGICAL)(optional > index->level), root->eltid,
                           root->minim, &errelt);
                if (c)
                        nondeterm(root, c, errelt);
                if (!group && !value)
        ...
        if (new) {
                top->finals =
                    (STATELIST *)m_malloc(sizeof(STATELIST), "state list");
...
STATE *startfsa(root, canbenull) TREE *root;
LOGICAL *canbenull;
{
        STATELIST *item;
        STATE *first;
