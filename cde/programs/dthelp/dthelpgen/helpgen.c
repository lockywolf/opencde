...
static const char *defaultTitle12 = "<TYPE serif><WEIGHT bold><SIZE 12><ANGLE "
                                    "italic>\n%s\n</ANGLE></SIZE></WEIGHT></"
                                    "TYPE>\n";
static const char *defaultTitle14 = "<TITLE><TYPE serif><WEIGHT bold><SIZE "
                                    "14>\n%s\n</SIZE></WEIGHT></TYPE></"
                                    "TITLE>\n";
...
int CreateVolumeLink(CanvasHandle canvas, FILE *outTopic, char *volume_name) {
        ...
        if (pathName != NULL &&
            _DtHelpCeOpenVolume(canvas, pathName, &volume) == 0) {
                if (_DtHelpCeGetVolumeTitle(canvas, volume, &title) == 0)
                        result = 0;
                else if (_DtHelpCeGetTopicTitle(
                             canvas, volume, (char *)TopLocId, &title) == True)
                        result = 0;

                if (result == 0) {
                        if (_DtHelpCeGetAsciiVolumeAbstract(canvas, volume,
                                                            &abstract) == -1)
                                abstract = NULL;
        ...
        if (result == 0) {
                ...
                fprintf(outTopic, "<LINK 0 \"%s %s\">\n", volume_name,
                        (char *)TopLocId);
                fprintf(outTopic, (GetMessage(3, 5, (char *)defaultTitle12)),
                        title);
...
int CreateFamily(CanvasHandle canvas, char *family_name, FILE *out_volume,
        ...
        if (db) {
                ...
                if (XrmGetResource(db, "Family.Title", "family.title", &resType,
                         ...
                         * get the abstract
                         */
                        if (XrmGetResource(db, "Family.Abstract",
                                           "family.abstract", &resType,
                                           &resValue)) {
                                 ...
                                 */
                                if (XrmGetResource(db, "Family.Volumes",
                                                   "family.volumes", &resType,
                                                   &resValue)) {
                                         ...
                                         */
                                        if (XrmGetResource(db, "Family.CharSet",
                                                           "family.charSet",
                                                           &resType,
                                                           &resValue)) {
                ...
                if (title && abstract && list && charSet) {
                         ...
                         */
                        fprintf(out_topic,
                                (GetMessage(3, 1, (char *)defaultTopic)),
                                charSet, title);
                         ...
                         */
                        fprintf(out_topic,
                                (GetMessage(3, 2, (char *)defaultTitle14)),
                                title);
                        fprintf(out_topic,
                                (GetMessage(3, 3,
                                            "<P before 1 first 1 left 1>\n")));
                        fprintf(out_topic, "%s\n", abstract);
                        fprintf(out_topic, "</P>\n");
                        ...
                        do {
                                ...
                                if (token && *token != '\0' && *token != '\n' &&
                                    CreateVolumeLink(canvas, out_topic,
                                                     token) == 0)
                                        count++;
                        ...
                        if (count) {
                                ...
                                fprintf(
                                    out_header,
                                    (GetMessage(3, 5, (char *)defaultTitle12)),
                                    title);
                                ...
                                TopicList = (char **)_DtHelpCeAddPtrToArray(
                                    (void **)TopicList, strdup(familyName));
                        } else {
                                /*
...
void ScanDirectory(char *directory, long *ret_time) {
        ...
        *ret_time = buf.st_mtime;
        ...
        while (pDirent) {
                ...
                } else if (strcmp(ext, Ext_Hv) == 0 ||
                           strcmp(ext, Ext_Sdl) == 0) {
                        strcpy(ptr, pDirent->d_name);
                        VolumeList = (char **)_DtHelpCeAddPtrToArray(
                            (void **)VolumeList, strdup(pDirent->d_name));
                        FullVolName = (char **)_DtHelpCeAddPtrToArray(
                            (void **)FullVolName, strdup(fullName));
                }
...
int CheckTimeStamps(XrmDatabase db, char **dir_list) {
        ...
        while (*dir_list != NULL) {
                ...
                value = _DtHelpCeGetResourceString(db, *dir_list, "TimeStamp",
                                                   "timeStamp");
                timeVal = atol(value);
...
int CheckInfo(char *file) {
        ...
        if (db != NULL) {
                volDirList = _DtHelpCeGetResourceStringArray(
                    db, NULL, "DirList", "dirList");
                if (volDirList != NULL) {
                        result = 0;
                        list1 = volDirList;
                        list2 = FUserList;
                        while (result == 0 && *list1 != NULL && list2 != NULL &&
                               *list2 != NULL) {
...
int main(int argc, char *argv[]) {
        ...
        for (i = 1; i < argc; i++) {
                if (argv[i][0] == '-') {
                        if (argv[i][1] == 'd' && i + 1 < argc)
                                ...
                                fprintf(stderr,
                                        (GetMessage(1, 1, ((char *)UsageStr))),
                                        myName);
                                exit(1);
                        }
                } else {
                        fprintf(stderr, (GetMessage(1, 1, ((char *)UsageStr))),
                                myName);
                        exit(1);
                }
        }
        ...
        if (App_args.dir[0] != '/') {
                if (getcwd(baseName, MAXPATHLEN) == NULL) {
                        fprintf(stderr,
                                (GetMessage(1, 18,
                                            "%s: Unable to access current "
                                            "working directory - error status "
                                            "number %d\n")),
                                myName, errno);
                        exit(1);
                }
        ...
        while (endDir && *endDir != '\0') {
                ...
                *endDir = '\0';
                ...
                /*
                 * append the rest of the directory spec that hasn't been
                 * checked.
                 */
        ...
        if (result == 0) {
                if (access(tmpVolume, W_OK) == -1) {
                        if (errno == EROFS)
                                ptr = GetMessage(1, 7,
                                                 "%s: File system containing "
                                                 "%s is read only\n");
                        else if (errno == EACCES)
                                ptr = GetMessage(1, 8,
                                                 "%s: Requires root permission "
                                                 "to write to %s\n");
                        else
        ...
        if (canvasHandle == NULL) {
                fprintf(stderr,
                        GetMessage(1, 19, "%s: Unable to allocate memory\n"),
                        myName);
                MyExit(1, -1);
        }
                ...
                if (!ret) {
                        ...
                        if (!ret) {
                                ret = _DtLcxXlateOpToStd(
                                    db, plat, execver, DtLCX_OPER_SETLOCALE,
        ...
        fprintf(outHeader, (GetMessage(3, 2, (char *)defaultTitle14)),
                topicTitle);
        free(topicTitle);
        ...
        for (next = FullFamilyName; next != NULL && *next != NULL; next++) {
                result = CreateFamily(canvasHandle, *next, outVolume, outHeader,
                                      outTopic);
                if (result == 0) {
                        FamilyNum++;
                        foundVolumes = 1;
