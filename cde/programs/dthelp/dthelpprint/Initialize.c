...
static XtResource s_OptionResources[] = {
    {RN_display, RC_display, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, display), XtRImmediate,
     (XtPointer)DFLT_DISPLAY},

    {RN_printer, RC_printer, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, printer), XtRImmediate,
     (XtPointer)DFLT_PRINTER},

    {RN_copies, RC_copies, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, copies), XtRImmediate,
     (XtPointer)DFLT_COPY_CNT},

    {RN_outputFile, RC_outputFile, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, outputFile), XtRImmediate,
    ...
    {RN_printer "." RN_paperSize, /* see CalculatePageSize() to grok this */
     ...
     XtRString, sizeof(String), XtOffset(_DtHPrOptionsPtr, paperSize),
     XtRImmediate, (XtPointer)DFLT_PAPER_SIZE},

    {RN_rsrcname, RC_rsrcname, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, rsrcname), XtRImmediate,
     (XtPointer)DFLT_RSRCNAME},

    {RN_rsrcclass, RC_rsrcclass, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, rsrcclass), XtRImmediate,
     (XtPointer)DFLT_RSRCCLASS},

    {RN_helpType, RC_helpType, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, helpType), XtRImmediate,
     (XtPointer)DFLT_HELPTYPE},

    {RN_helpVolume, RC_helpVolume, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, helpVolume), XtRImmediate, (XtPointer)0},

    {RN_locationId, RC_locationId, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, locationId), XtRImmediate,
     (XtPointer)DFLT_LOCATIONID},

    {RN_allTopics, RC_allTopics, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, allTopics), XtRImmediate,
     (XtPointer)DFLT_ALLTOPICS},

    {RN_subTopics, RC_subTopics, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, subTopics), XtRImmediate,
    ...
    {RN_recurse, RC_recurse, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, subTopics), XtRImmediate,
     (XtPointer)DFLT_SUBTOPICS},

    {RN_oneTopic, RC_oneTopic, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, oneTopic), XtRImmediate,
     (XtPointer)DFLT_ONETOPIC},

    {RN_toc, RC_toc, XtRString, sizeof(String), XtOffset(_DtHPrOptionsPtr, toc),
     XtRImmediate, (XtPointer)DFLT_TOC},

    {RN_index, RC_index, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, index), XtRImmediate, (XtPointer)DFLT_INDEX},

    {RN_frontMatter, RC_frontMatter, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, frontMatter), XtRImmediate,
     (XtPointer)DFLT_FRONTMATTER},

    {RN_evenTocFooter, RC_evenTocFooter, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, tocHF.evenFooter), XtRImmediate,
     (XtPointer)DFLT_TOC_FOOTER},

    {RN_oddTocFooter, RC_oddTocFooter, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, tocHF.oddFooter), XtRImmediate,
     (XtPointer)DFLT_TOC_FOOTER},

    {RN_evenTocHeader, RC_evenTocHeader, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, tocHF.evenHeader), XtRImmediate,
     (XtPointer)DFLT_TOC_HEADER},

    {RN_oddTocHeader, RC_oddTocHeader, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, tocHF.oddHeader), XtRImmediate,
     (XtPointer)DFLT_TOC_HEADER},

    {RN_evenBodyFooter, RC_evenBodyFooter, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, bodyHF.evenFooter), XtRImmediate,
     (XtPointer)DFLT_BODY_FOOTER},

    {RN_oddBodyFooter, RC_oddBodyFooter, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, bodyHF.oddFooter), XtRImmediate,
     (XtPointer)DFLT_BODY_FOOTER},

    {RN_evenBodyHeader, RC_evenBodyHeader, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, bodyHF.evenHeader), XtRImmediate,
     (XtPointer)DFLT_BODY_HEADER},

    {RN_oddBodyHeader, RC_oddBodyHeader, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, bodyHF.oddHeader), XtRImmediate,
     (XtPointer)DFLT_BODY_HEADER},

    {RN_evenIndexFooter, RC_evenIndexFooter, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, indexHF.evenFooter), XtRImmediate,
     (XtPointer)DFLT_INDEX_FOOTER},

    {RN_oddIndexFooter, RC_oddIndexFooter, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, indexHF.oddFooter), XtRImmediate,
     (XtPointer)DFLT_INDEX_FOOTER},

    {RN_evenIndexHeader, RC_evenIndexHeader, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, indexHF.evenHeader), XtRImmediate,
     (XtPointer)DFLT_INDEX_HEADER},

    {RN_oddIndexHeader, RC_oddIndexHeader, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, indexHF.oddHeader), XtRImmediate,
     (XtPointer)DFLT_INDEX_HEADER},

    {RN_manPage, RC_manPage, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, manPage), XtRImmediate, (XtPointer)0},

    {RN_stringData, RC_stringData, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, stringData), XtRImmediate, (XtPointer)0},

    {RN_helpFile, RC_helpFile, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, helpFile), XtRImmediate, (XtPointer)0},

    {RN_topicTitle, RC_topicTitle, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, topicTitle), XtRImmediate,
     (XtPointer)DFLT_TOPICTITLE},

    {RN_echoCommand, RC_echoCommand, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, echoCommand), XtRImmediate,
     (XtPointer)DFLT_ECHO_CMD},

    {RN_echoArgs, RC_echoArgs, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, echoArgs), XtRImmediate,
     (XtPointer)DFLT_ECHO_ARGS},

    {RN_foldCommand, RC_foldCommand, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, foldCommand), XtRImmediate,
     (XtPointer)DFLT_FOLD_CMD},

    {RN_foldArgs, RC_foldArgs, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, foldArgs), XtRImmediate,
     (XtPointer)DFLT_FOLD_ARGS},

    {RN_prCommand, RC_prCommand, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, prCommand), XtRImmediate,
     (XtPointer)DFLT_PR_CMD},

    {RN_prArgs, RC_prArgs, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, prArgs), XtRImmediate, (XtPointer)DFLT_PR_ARGS},

    {RN_prOffsetArg, RC_prOffsetArg, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, prOffsetArg), XtRImmediate,
     (XtPointer)DFLT_PR_OFFSET_ARG},

    {RN_manCommand, RC_manCommand, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, manCommand), XtRImmediate,
     (XtPointer)DFLT_MAN_CMD},

    {RN_manArgs, RC_manArgs, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, manArgs), XtRImmediate,
     (XtPointer)DFLT_MAN_ARGS},

    {RN_redirectCmdAndArgs, RC_redirectCmdAndArgs, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, redirectCmdAndArgs), XtRImmediate,
     (XtPointer)DFLT_REDIR_CMD_AND_ARGS},

    {RN_lpCommand, RC_lpCommand, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, lpCommand), XtRImmediate,
     (XtPointer)DFLT_LP_CMD},

    {RN_shCommand, RC_shCommand, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, shCommand), XtRImmediate,
     (XtPointer)DFLT_SH_CMD},

    {RN_iconvCmdAndArgs, RC_iconvCmdAndArgs, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, iconvCmdAndArgs), XtRImmediate,
     (XtPointer)DFLT_ICONV_CMD_AND_ARGS},

    {RN_debugHelpPrint, RC_debugHelpPrint, XtRString, sizeof(String),
     XtOffset(_DtHPrOptionsPtr, debugHelpPrint), XtRImmediate,
...
== == == == == == == == == == == == == == == == == == == == == == == == == == ==
    == == == == == ==
    = $PFUNBEG$ : Usage() $1LINER$ : Prints usage message and returns $DESCRIPT$
    : This routine prints a generic help message $RETURNS$ : $ARGS$
    : == ==
      == == == == == == == == == == == == == == == == == == == == == == == == ==
      == $SKIP$ == ==
    = * /
      ...
      static void Usage(void) { /*$CODE$*/
        static char *usage[] = {
            ...
            "\t" AN_display
            " displayname  display from which to get resources\n",
            "\t" AN_rsrcname
            " name            program name used when getting resources\n",
            "\t" AN_rsrcclass
            " name           class name used when getting resources\n",
            "\t" AN_xrm " resourcestring   additional resources\n",
            "Options controlling what to print:\n",
            "\t" AN_helpType " type        type of Help data\n",
            "\t\ttype = 0 (help volume), 1 (string), 2 (man page), 3 (help "
            "file)\n",
            "\t" AN_helpVolume " volume    full path of help volume file\n",
            "\t" AN_locationId " location  name of Help topic in the volume\n",
            "\t" AN_allTopics
            "            print all topics, toc, & index in the help volume\n",
            "\t" AN_subTopics
...
== == == == == == == == == == == == == == == == == == == == == == == == == == ==
    == == == == == ==
    = $PFUNBEG$ : CalculatePageSize() $1LINER$
    : calculates the page size based on resource values $DESCRIPT$
    : Reads the resources from the database and calculates the page
          dimensions and stores them in the resources structure.$RETURNS$
    : $ARGS$ : == ==
               == == == == == == == == == == == == == == == == == == == == == ==
               == == == == $SKIP$ == ==
    = * /
      ...
      static void CalculatePageSize(XrmDatabase appDB, _DtHPrOptions *options,
                                    char *appname, char *appclass) { /*$CODE$*/
...
static float dphm = 300.*2540.;
        ...
        if (XrmGetResource(appDB, resource_name, resource_class, str_type,
                           &value) == True)
                name = value.addr;
        else
                name = EMPTY_STR;
        if (name[0] != EOS) {
                ...
                strcat(class_prefix,
                       name); /* e.g. Dthelpprint.Printer.<name> */
        }
        ...
        if (XrmGetResource(appDB, resource_name, resource_class, str_type,
                           &value) == True) {
                if ((*stricmp)(value.addr, RV_letter) == 0)
                        papersize = LETTER;
                else if ((*stricmp)(value.addr, RV_b5) == 0)
                        papersize = B5;
                else if ((*stricmp)(value.addr, RV_a4) == 0)
                        papersize = A4;
                else if ((*stricmp)(value.addr, RV_legal) == 0)
                        papersize = LEGAL;
                else if ((*stricmp)(value.addr, RV_executive) == 0)
                        papersize = EXECUTIVE;
                else {
                        fprintf(stderr,
                                _DTGETMESSAGE(INSET, 40,
                                              "%s Warning: Illegal paper size "
                                              "'%s'.  " RV_letter " used.\n"),
        ...
        if (papersize < 0 && (width == 0 || height == 0)) {
                fprintf(stderr,
                        _DTGETMESSAGE(INSET, 41,
                                      "%s Warning: Missing paper size, height, "
                                      "or width value.  " RV_letter " used.\n"),
                        options->programName);
                papersize = LETTER;
        }

        /* if an invaild papersize, dflt to letter */
        if (papersize < 0 || papersize > MAXVALIDSIZE)
                papersize = LETTER;
        ...
        if (options->debugHelpPrint) {
                ...
                printf("options.%s: %d\n", "colsLeftMargin",
                       options->colsLeftMargin);
                printf("options.%s: %d\n", "colsRightMargin",
                       options->colsRightMargin);
                printf("options.%s: %d\n", "rowsTopMargin",
                       options->rowsTopMargin);
                printf("options.%s: %d\n", "rowsBottomMargin",
                       options->rowsBottomMargin);
                printf("options.%s: %d\n", "rowsTextHeight",
                       options->rowsTextHeight);
                printf("options.%s: %d\n", "colsTextWidth",
                       options->colsTextWidth);
                printf("options.%s: %d\n", "colsAdjLeftMargin",
                       options->colsAdjLeftMargin);
                printf("options.%s: %d\n", "colsAdjRightMargin",
                       options->colsAdjRightMargin);
                printf("options.%s: %d\n", "rowsAdjTopMargin",
                       options->rowsAdjTopMargin);
                printf("options.%s: %d\n", "rowsAdjBottomMargin",
                       options->rowsAdjBottomMargin);
        }
} /*$END$*/
...
{       /*$CODE$*/
        static XrmOptionDescRec optionsTable[] = {
            ...
            {AN_paperSize, RN_printer "." RN_paperSize, XrmoptionSepArg,
             (caddr_t)NULL},
            ...
            {AN_evenTocFooter, RN_evenTocFooter, XrmoptionSepArg,
             (caddr_t)NULL},
            {AN_oddTocFooter, RN_oddTocFooter, XrmoptionSepArg, (caddr_t)NULL},
            {AN_evenTocHeader, RN_evenTocHeader, XrmoptionSepArg,
             (caddr_t)NULL},
            {AN_oddTocHeader, RN_oddTocHeader, XrmoptionSepArg, (caddr_t)NULL},
            {AN_evenBodyFooter, RN_evenBodyFooter, XrmoptionSepArg,
             (caddr_t)NULL},
            {AN_oddBodyFooter, RN_oddBodyFooter, XrmoptionSepArg,
             (caddr_t)NULL},
            {AN_evenBodyHeader, RN_evenBodyHeader, XrmoptionSepArg,
             (caddr_t)NULL},
            {AN_oddBodyHeader, RN_oddBodyHeader, XrmoptionSepArg,
             (caddr_t)NULL},
            {AN_evenIndexFooter, RN_evenIndexFooter, XrmoptionSepArg,
             (caddr_t)NULL},
            {AN_oddIndexFooter, RN_oddIndexFooter, XrmoptionSepArg,
             (caddr_t)NULL},
            {AN_evenIndexHeader, RN_evenIndexHeader, XrmoptionSepArg,
             (caddr_t)NULL},
            {AN_oddIndexHeader, RN_oddIndexHeader, XrmoptionSepArg,
             (caddr_t)NULL},
            /* what to print */
        ...
        static int numOptions =
            XtNumber(optionsTable); /*number entries in table*/
        ...
        if (invocationName == NULL) {
                ...
                if (_DtHelpCeStrrchr(argv[0], DIR_SLASH_STR, MB_CUR_MAX,
                                     &commandLineName) == 0)
        ...
        XrmParseCommand(&commandLineDB, optionsTable, numOptions,
                        invocationName, argc, argv);
        ...
        strcpy(class_prefix,
               HELPPRINT_APPLICATION_CLASS); /* e.g. Dthelpprint */
        ...
        if ((tmp_string = getenv("DISPLAY")) != NULL) {
                i = strlen(tmp_string);
                value.addr =
                    malloc(i + 1); /* FIX: use tmp_string here directly? */
                value.size = i + 1;
                strcpy(value.addr, tmp_string);
                strcpy(resource_name, name_prefix);
                strcat(resource_name,
                       RN_display); /* e.g. dthelpprint.display */
                strcpy(resource_class, class_prefix);
                strcat(resource_class,
                       RC_display); /* e.g. Dthelpprint.Display */
        ...
        if ((tmp_string = getenv("PRINTER")) != NULL) {
                ...
                strcpy(resource_name, name_prefix);
                strcat(resource_name,
                       RN_printer); /* e.g. dthelpprint.printer */
                strcpy(resource_class, class_prefix);
                strcat(resource_class,
                       RC_printer); /* e.g. Dthelpprint.Printer */
                XrmPutResource(appDB, resource_name, NULL, &value);
                XrmPutResource(appDB, resource_class, NULL, &value);
        }
        ...
        */
        ...
        strcat(filename,
               class_prefix); /*eg. /usr/dt/app-defaults/C/Dthelpprint*/
        ...
        strcat(filename,
               class_prefix); /*eg. /usr/lib/X11/app-defaults/Dthelpprint*/
        appClassDB = XrmGetFileDatabase(filename);
        if (appClassDB != NULL)
                (void)XrmMergeDatabases(appClassDB, appDB);
        ...
        if (XrmGetResource((XrmDatabase)commandLineDB, resource_name,
                           resource_class, str_type, &value) == True ||
            XrmGetResource((XrmDatabase)*appDB, resource_name, resource_class,
                           str_type, &value) == True)
                display = value.addr;
        else
                display = NULL;
        ...
        *pDpy = XOpenDisplay(display); /* FIX: should this be .printer ?? */
        if (*pDpy == NULL) {           /* Bad display */
                /* FIX: chg msg to match above semantics */
                fprintf(
                    stderr,
                    _DTGETMESSAGE(INSET, 42,
                                  "%s Warning: Unable to open display %s\n"),
                    argv[0], (NULL == display ? "" : display));
        }      /* if bad display */
        else { /* Good display */
                /* try to overlay the display's resources (i.e. the xrdb-managed
                 * ones) */
                dispDBStr = XResourceManagerString(*pDpy);
                if (dispDBStr)
                        dispDB = XrmGetStringDatabase(dispDBStr);
                      ...
                      */
                {
                        ...
                        (void)strcat(
                            filename,
                            XDEFAULTS_FILE); /* e.g. $HOME/.Xdefaults */
                ...
                scrnDBStr =
                    XScreenResourceString(DefaultScreenOfDisplay(*pDpy));
                if (scrnDBStr)
                        scrnDB = XrmGetStringDatabase(scrnDBStr);
                if (scrnDB != NULL)
        ...
        if (*appDB == NULL)
                *appDB = commandLineDB;
                ...
                XrmMergeDatabases(commandLineDB, appDB);
...
{       /*$CODE$*/
        ...
        /* Get name and class */
        if (XrmGetResource(db, RN_appname, RC_appname, str_type, &value) ==
            True)
                name_prefix = value.addr;
        else
                name_prefix = HELPPRINT_APPLICATION_NAME; /*e.g. dthelpprint*/

        if (XrmGetResource(db, RN_appclass, RC_appclass, str_type, &value) ==
            True)
                class_prefix = value.addr;
        else
                class_prefix = HELPPRINT_APPLICATION_CLASS; /*e.g. Dthelpprint*/
        ...
        for (rsrc = s_OptionResources, cnt = XtNumber(s_OptionResources);
             cnt > 0; cnt--, rsrc++) {
                ...
                if (XrmGetResource(db, resource_name, resource_class, str_type,
                                   &value) == True)
    ...
    == == == == == == == == == == == == == == == == == == == == == == == == ==
    == == == == == == == ==
    = $FUNBEG$ : _DtHPrCreateTmpFile() $1LINER$ : Creates a tmp file in $HOME /
                                                      .dt /
                                                  tmp $DESCRIPT$
    : The file is put in $HOME /.dt /
      tmp so that the file can be referenced from other
          systems.This assumes that the user home directory is mounted on those
              systems.$RETURNS$ : $ARGS$
    : ==
      == == == == == == == == == == == == == == == == == == == == == == == == ==
      == == $SKIP$ == ==
    = * /
      ...
      char * _DtHPrCreateTmpFile(char *prefix, char *suffix) { /*$CODE$*/
        ...
        for (tmppath = strchr(tmppath, DIR_SLASH); tmppath != NULL;
...
#define FILENAMELEN 25
        ...
        if (NULL == newtmpfile) {
                fprintf(stderr, _DTGETMESSAGE(INSET, 45,
                                              ...
                                              "memory for temporary file\n"));
        } else {
                sprintf(newtmpfile,
                        _DTGETMESSAGE(INSET, 50, "%1$s/%2$s%3$d_%4$d%5$s"),
