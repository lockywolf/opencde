...
M_WCHAR *checkid(id) M_WCHAR *id;
{
        ...
        if (!xref)
                return (id); /* not in trie, so not previously defined */
        ...
        if (xref->file) {
                ...
                buffer = (char *)m_malloc(
                    1 - 2 + strlen(offile) + strlen(mb_file), "error buffer");
...
static void defxref(xfile, id, xref) FILE *xfile;
M_WCHAR *id;
struct xref *xref;
{
        char *csname;
        ...
        if (xref->defined) {
                ...
                fprintf(xfile, "\\definexref %s\\\\endxref\001%s\002\001%s",
                        csname, mb_text, xref->chapstring);
                fprintf(xfile, "\002\001%c\002\001%s\002\001%c\002%%\n",
                        xref->csensitive ? 'Y' : 'N',
...
void dumpxref(M_NOPAR) {
        ...
        fputs(
            "% Generated Cross-Reference Macros (for a particular document)\n",
            tex);
        if (!xtree.data)
                return;
...
void idstring(string) M_WCHAR *string;
{
        int length;
        ...
        if (xstrlen + length + 1 > (sizeof(xrefstring) / sizeof(M_WCHAR))) {
                if (!iderr) {
                        m_error("Internal error. Too many characters in "
...
void loadxref(M_NOPAR) {
        ...
        while (TRUE) {
                ...
                for (n = 0; n <= M_NAMELEN; n++) {
                        nextc = mb_getwc(
                            xrf); /* use mb_getwc to read multi-byte chars */
                        if (nextc == weof) {
                ...
                for (xstrlen = 0, braces = 1;
                     ...
                     xstrlen++) {
                        ...
                        if (xrefstring[xstrlen] == wca)
                                braces++;
                        else if (xrefstring[xstrlen] == wcb)
                                if (!--braces)
                                        break;
                }
                ...
                if ((fscanf(xrf,
                            "\001%14[^\002]\002\001%c\002\001%8[^\002]\002\001%"
                            "c\002%%\n",
                            chapnum, &c, string, &d) != 4) ||
                    (c != 'Y' && c != 'N') || (d != 'Y' && d != 'N') ||
                    (strcmp(string, "\\LOCapp") &&
                     strcmp(string, "\\LOCchap"))) {
                        m_error("Error in cross-reference file");
                        fclose(xrf);
                        return;
                }
                ...
                setid(id, FALSE, csensitive, inchapter, chapnum, NULL, 0,
                      xrefable);
        }
...
#else
void setid(id, where, csensitive, inchapter, chapter, xrffile, xrfline,
           xrefable) M_WCHAR *id;
LOGICAL where;
LOGICAL csensitive;
LOGICAL inchapter;
...
#endif
{
        ...
        if (old = (struct xref *)m_ntrtrie(id, &xtree, (M_TRIE *)xref)) {
                ...
                if (xref->wheredef) {
                        ...
                        mb_id = MakeMByteString(id);
                        m_err1("Redefinition of ID '%s'", id);
                        buffer = (char *)m_malloc(1 + strlen(firstused) +
                                                      w_strlen(id) + 6,
                                                  "error buffer");
                        sprintf(buffer, firstused, mb_id, xref->line);
                        m_errline(buffer);
                        ...
                        if (xref->file) {
                                ...
                                mb_file = MakeMByteString(xref->file);
                                buffer = (char *)m_malloc(
                                    1 - 2 + strlen(offile) + strlen(mb_file),
                                    "error message");
                                sprintf(buffer, offile, mb_file);
                                m_errline(buffer);
                                m_free(buffer, "error buffer");
                ...
                if (xref->defined) {
                        if (w_strcmp(xrefstring, xref->textptr) !=
                            0) /* ref has changed */
                                have_forward_xrefs =
                                    TRUE; /* so force a second pass */
...
#define format "\\<xref %s>"
        ...
        if (old = (struct xref *)m_ntrtrie(id, &xtree, (M_TRIE *)xref)) {
                ...
                if (!xref->xrefable) {
                        m_error(
                            "You can't cross reference to that type of tag");
                ...
                if (xref->textptr) {
                        ...
                        fprintf(outfile, "<LINK RID=\"%s\">%s</LINK>", csname,
                                mb_textptr);
                ...
                if (rebuild) {
                        m_errline("Unresolved xrefs. Running HelpTag a second "
                                  "time may help.\n");
