 ...
 */
/* $XConsortium: out.c /main/4 1995/12/04 11:42:39 rswiston $ */
...
#ifdef _AIX
        /* This is a bug fix for AIX. Using putc() for EUC Gaiji characters on
         * aixterm has a problem.
         */
...
void endhead(M_NOPAR) {
        if (savid) {
                ...
                mb_shchar(
                    M_EOS, &xstrlen, (sizeof(xrefstring) / sizeof(M_WCHAR)),
                    xrefstring, idstring,
                    "Too many characters in corresponding cross-reference",
                    &iderr);
                setid(savid, TRUE, TRUE, inchapter, chapstring, xrffile,
                      xrfline, TRUE);
        }
...
#else
void esoutchar(textchar) M_WCHAR textchar;
#endif
{
...
#else
void vexoutchar(textchar) M_WCHAR textchar;
#endif
{
...
#else
void exoutchar(textchar) M_WCHAR textchar;
#endif
{
...
#else
void imoutchar(textchar) M_WCHAR textchar;
#endif
{
...
#else
void indexchar(textchar) M_WCHAR textchar;
#endif
{
...
#else
void outchar(textchar, outfile) M_WCHAR textchar;
FILE *outfile;
#endif
{
        M_WCHAR buffer[2];
...
void outpi(enttype, pi, entname) int enttype;
M_WCHAR *pi;
M_WCHAR *entname;
{
        strcode(pi, outfile);
...
#else
void realoutchar(textchar, outfile) M_WCHAR textchar;
FILE *outfile;
#endif
{
        char mb_textchar[32]; /* arbitrarily large */
        ...
        if (savid)
                shchar(textchar, &xstrlen,
                       (sizeof(xrefstring) / sizeof(M_WCHAR)), xrefstring,
                       idstring,
                       "Too many characters in corresponding cross-reference",
                       &iderr);

        if (savhd)
                shchar(textchar, &svheadlen,
                       (sizeof(savehead) / sizeof(M_WCHAR)), savehead,
                       svhdstring, "Too many characters in head or caption",
                       &hderr);

        if (savtc)
                shchar(textchar, &svtclen,
                       (sizeof(savetabcap) / sizeof(M_WCHAR)), savetabcap,
                       svtcstring, "Too many characters in table caption",
                       &hderr);
}
...
#if defined(M_PROTO)
void shchar(M_WCHAR textchar, int *len, int max, M_WCHAR *string,
            void (*proc)(M_WCHAR *string), char *msg, LOGICAL *errflg)
#else
void shchar(textchar, len, max, string, proc, msg, errflg) M_WCHAR textchar;
int *len;
int max;
M_WCHAR *string;
void (*proc)(
...
#if defined(M_PROTO)
void mb_shchar(char textchar, int *len, int max, M_WCHAR *string,
               void (*proc)(M_WCHAR *string), char *msg, LOGICAL *errflg)
#else
void mb_shchar(textchar, len, max, string, proc, msg, errflg) char textchar;
int *len;
int max;
M_WCHAR *string;
void (*proc)(
...
/* Save a string in the array used to store table of contents entries
   when processing a head */
void shstring(addstring, len, max, storestring, msg, errflg) M_WCHAR *addstring;
int *len;
int max;
M_WCHAR *storestring;
char *msg;
...
void mb_shstring(addstring, len, max, storestring, msg, errflg) char *addstring;
int *len;
int max;
M_WCHAR *storestring;
char *msg;
...
void strcode(string, outfile) M_WCHAR *string;
FILE *outfile;
{
        char exbuff[32]; /* arbitrarily large */
        int bufflen;
        ...
        if (savid)
                shstring(string, &xstrlen,
                         (sizeof(xrefstring) / sizeof(M_WCHAR)), xrefstring,
                         "Too many characters in corresponding cross-reference",
                         &iderr);

        if (savhd)
                shstring(string, &svheadlen,
                         (sizeof(savehead) / sizeof(M_WCHAR)), savehead,
                         "Too many characters in head or caption", &hderr);

        if (savtc)
                shstring(string, &svtclen,
                         (sizeof(savetabcap) / sizeof(M_WCHAR)), savetabcap,
                         "Too many characters in table caption", &hderr);
...
void mb_strcode(string, outfile) char *string;
FILE *outfile;
{
        M_WCHAR *wc;
...
void svhdstring(string) M_WCHAR *string;
{
        int length;
...
void svtcstring(string) M_WCHAR *string;
{
        int length;
...
#else
void termchar(textchar) M_WCHAR textchar;
#endif
{
        int length;
        char mb_textchar[32]; /* arbitrarily large */
        ...
        if ((length == 1) && (pc = special[(unsigned char)*mb_textchar])) {
                ...
                if (termp - term + length > MAXTERM) {
                        if (termp - term <= MAXTERM) {
                                ...
                                m_err4("Too many characters in %sTERM%s or "
                                       "%sDTERM%s",
                                       wc_stago, wc_tagc, wc_stago, wc_tagc);
...
void termpi(m_enttype, m_pi, m_entname) int m_enttype;
M_WCHAR *m_pi;
M_WCHAR *m_entname;
{
        int length;
