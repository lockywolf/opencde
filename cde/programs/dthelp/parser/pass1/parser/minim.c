        ...
        for (stackptr = m_stacktop; stackptr; stackptr = stackptr->oldtop) {
                ...
                if (!stackptr->oldtop ||
                    m_element[stackptr->element - 1].content == M_REGEXP) {
                        /* Note the following statement, which checks the type
                           of the element at the top of the stack, is not a
                           repeat of the previous one, which checks the type of
                           an element embedded in the stack. The second
                           comparison prevents traversing paths from a parent of
                           an RCDATA or CDATA element and still allows
                           displaying the end-tag of the parent */
                        if (!stackptr->oldtop ||
                            m_element[m_stacktop->element - 1].content ==
                                ...
                                for (fsastack = stackptr->fsastack; fsastack;
                                     fsastack = fsastack->oldtop) {
                                        ...
                                        if (required)
                                                return;
                                        m_expexpand(&expstart,
                                                    fsastack->current,
                                                    &required, &data);
                                        if (!m_state[fsastack->current - 1]
                                                 .final)
                                                return;
                                }
                } else if (m_element[stackptr->element - 1].content ==
                               M_CDATA ||
                ...
                if (!m_element[stackptr->element - 1].emin)
                        return;
        }
}
...
void m_expexpand(expstart, node, required, data) LOGICAL *expstart;
M_STATE node;
LOGICAL *required;
LOGICAL *data;
{
        ...
        for (parc = m_state[node - 1].first; parc;
...
void m_expline(expstart, data, label) LOGICAL *expstart;
LOGICAL *data;
M_ELEMENT label;
{
        ...
        if (m_explimit && m_expcount == M_EXPLIMIT) {
                ...
                mb_enptr =
                    MakeMByteString(&m_ename[m_element[label - 1].enptr]);
...
void m_exptend(expstart, stackptr) LOGICAL *expstart;
M_PARSE *stackptr;
{
        char buffer[M_NAMELEN + 2 * MAXD + 1];
        ...
        if (m_explimit && m_expcount == M_EXPLIMIT) {
                ...
                mb_enptr = MakeMByteString(
                    &m_ename[m_element[stackptr->element - 1].enptr]);
...
/* M_findunique is used to test for start tag minimization.  If the current
   ...
   no unique element.
*/
M_ELEMENT m_findunique(from, newleft) M_STATE from;
int *newleft;
{
        M_ARC parc;
        ...
        for (parc = m_state[from - 1].first; parc;
             parc = m_arc[parc - 1].next) {
                if (m_arc[parc - 1].group) {
                        if (!m_conform)
                                for (pand = m_arc[parc - 1].group; pand;
                                     pand = m_andgroup[pand - 1].next) {
                                        testminim = m_findunique(
                                            m_andgroup[pand - 1].start,
                                            &testleft);
                        ...
                        if (!m_conform) {
                                if (m_arc[parc - 1].minim &&
                                    ...
                                    !m_excluded(m_arc[parc - 1].label)) {
                                        /* Save the explicitly minimizable
                                           element plus its position in the
                                           content model */
                                        ...
                                        minim = m_arc[parc - 1].label + 1;
                                         ...
                                         !m_arc[parc - 1].next &&
                                         m_element[m_arc[parc - 1].label - 1]
                                             .smin &&
                                         !m_excluded(m_arc[parc - 1].label))
...
void m_nullendtag(M_NOPAR) {
        ...
        while (m_stacktop->oldtop) {
                ...
                if (!m_ckend(m_stacktop->element, foundnet)) {
                        ...
                        wc_found =
                            MakeWideCharString(foundnet ? "Null" : "Implied");
                        m_err2("%s end tag for %s unexpected", wc_found,
                               m_nameofelt(m_stacktop->element));
...
LOGICAL m_omitend(M_NOPAR) {
        ...
        for (fsastack = m_stacktop->fsastack; fsastack;
             fsastack = fsastack->oldtop) {
                for (pand = fsastack->andgroup; pand;
                     pand = m_andgroup[pand - 1].next) {
                        /* Doesn't matter if optional submodel of and-group has
                         * occurred */
...
LOGICAL m_omitstart() {
                     ...
                     pand = m_andgroup[pand - 1].next) {
                        ...
                        if (!usedand) {
                                if (!m_state[m_andgroup[pand - 1].start - 1]
                                         .final)
                                        required = TRUE;
                                newc = m_findunique(m_andgroup[pand - 1].start,
                                                    &newleft);
                                if (newleft < leftmost) {
                                        leftmost = newleft;
                                        c = newc;
        ...
        c--;
        if (m_element[c - 1].content == M_NONE)
                return (FALSE);
        if (m_element[c - 1].content == M_CDATA)
                return (FALSE);
        if (m_element[c - 1].content == M_RCDATA)
                return (FALSE);
