...
#else
void dumptree(sparse) LOGICAL sparse;
#endif
{
        int i, count = 0;
        ...
        for (i = 0; i < ccount; i++) {
                if (contree[i]) {
                        ...
                        dumpnode(&first, delim, contree[i], &count,
                                 (void *)printval);
                } else
                        fprintf(context, "0");
                if (i < ccount - 1)
...
void enterdelim(n) int n;
{
        if (!contree[n]) {
                contree[n] = m_gettrienode();
        }

        if (m_ntrtrie(dstruct->string, contree[n],
                ...
                dcount--;
                warning2("Duplicate assignment to delimiter \"%s\" in context "
                         "\"%s\"",
                         mb_dstring, mb_contexts);
...
void getcode(n) int n;
{
        ...
        if (!wlb) {
                int ret = mbtowc(&wlb, "{", 1); /* keep the "}" balanced */
                ret = mbtowc(&wcm, ",", 1);
                ret = mbtowc(&wnl, "\n", 1);
                ret = mbtowc(&wsl, "/", 1);
                ret = mbtowc(&wst, "*", 1);
        ...
        while (TRUE) {
                ...
                if (i >= CNAMELEN) {
                        while (!m_whitespace((M_WCHAR)(c = readchar(TRUE))) &&
                               c != wcl && c != wcm && c != EOF && c != wsm)
                                ;
                        unread(c);
                        break;
                }
                c = readchar(TRUE);
                if (m_whitespace((M_WCHAR)c) || c == wsm || c == wcl ||
                    c == wcm || c == EOF) {
                        unread(c);
                        break;
                }
...
LOGICAL getdname(M_NOPAR) {
        ...
        for (p = dname; !m_whitespace((M_WCHAR)c) && c != EOF;
             c = readchar(TRUE)) {
                if (p - dname >= DNAMELEN) {
                        while (!m_whitespace((M_WCHAR)c) && c != EOF)
                                c = readchar(TRUE);
                        break;
                }
                *p++ = (M_WCHAR)c;
...
void inccon(M_NOPAR) {
        ...
        if (!newtrie || !newtransit || !newcontexts) {
                trysize = NUMCON + 1;
                if (newtrie)
                        free((M_POINTER)newtrie);
                if (newtransit)
                        free((M_POINTER)newtransit);
                if (newcontexts)
                        free((M_POINTER)newcontexts);
                newtrie = (M_TRIE **)calloc(trysize, sizeof(M_TRIE *));
                newtransit = (int *)calloc(trysize * NUMDELIM, sizeof(int));
                newcontexts = (M_WCHAR **)calloc(trysize, sizeof(M_WCHAR *));
        }
...
void incdelim(M_NOPAR) {
        ...
        free((M_POINTER)xtransit);
        if (loading) {
                memcpy((M_POINTER)newdlm, (M_POINTER)dlmptr,
                       dcount * sizeof(M_TRIE **));
...
void loaddelim(M_NOPAR) {
        ...
        while ((c = getc(ddat)) != EOF) {
                ...
                *p = M_EOS;
                if (w_strlen(dstring) > maxd)
                        maxd = w_strlen(dstring);
                /* Write definitions to output file.  Save data for later. */
                incdelim();
                dstruct = (struct dstruct *)m_malloc(sizeof(struct dstruct),
                                                     "delimiter structure");
                dstruct->string =
                    (M_WCHAR *)m_malloc(w_strlen(dstring) + 1, "delimiter");
                ...
                mb_dstring = MakeMByteString(dstring);
                fprintf(delim, "M_DELIMEXTERN char %s[%d] M_DELIMINIT(\"",
                        mb_dname, (int)strlen(mb_dstring) + 1);
...
#else
void nextcon(sparse) LOGICAL sparse;
#endif
{
        int i, j;
        int firstinrow;
...
/* If sparse matrix output option, generate a single element of transit
   array */
void prtctxt(column, value) int column, value;
{
