 ...
 */
/* $XConsortium: scan.c /main/3 1995/11/08 10:03:16 rswiston $ */
/*
...
/* Reads a name */
LOGICAL getname(first) int first;
{
        M_WCHAR *p;
        int c;

        *(p = name) = first;
        if ((curcon != DEFAULT && curcon != DEFVAL && curcon != DEFAULT2 &&
             curcon != DEF2VAL) ||
            newpar->type != CDATA)
                *p = m_ctupper(*p);
        while (TRUE) {
...
int scan(M_NOPAR) {
        ...
        while (TRUE) {
                ...
                if (n) {
                        ...
                        /* Litproc returns FALSE in case of error, when returned
                           have scanned to the end of erroneous rule */
                        else
                                continue;
                }
                ...
                if (m_newcon(curcon - 1, NAME - 1))
                        if (m_cttype(c) == M_NMSTART ||
                            (m_cttype(c) != M_NONNAME &&
                             (curcon == DEFAULT || curcon == DEFVAL ||
                              curcon == NTNAME || curcon == DEFAULT2 ||
                              curcon == DEF2VAL || curcon == NT2NAME))) {
                                if (getname(c)) {
                                        if (scantrace) {
                                                char *mb_name;
