 ...
 */
/* $XConsortium: param.c /main/3 1995/11/08 10:02:38 rswiston $ */
/*
...
void addkeyword(M_NOPAR) {
        ...
        for (kw = newpar->ptypep; kw; kw = kw->nextptr)
                if (w_strcmp(kw->keyword, name) == 0) {
                        warning3("Warning: Repeated keyword %s in parameter %s "
                                 "of %s",
                                 name, newpar->paramname, thisrule);
                        return;
                }
        ...
        for (par = plist; par != newpar; par = par->next)
                for (kw = par->ptypep; kw; kw = kw->nextptr) {
                        if (w_strcmp(kw->keyword, name) == 0)
                                warning4("Keyword %s used in parameters %s and "
                                         "%s of %s",
                                         name, par->paramname,
                                         newpar->paramname, thisrule);
                        if (!kw->next)
                                break;
                }
...
void addpar(M_NOPAR) {
        ...
        for (paramp = plist; paramp; paramp = paramp->next) {
                if (w_strcmp(name, paramp->paramname) == 0)
                        warning2("Multiple definition of parameter %s for "
                                 "element %s",
                                 name, thisrule);
                last = paramp;
        }
...
#else
LOGICAL checkdefault(string) M_WCHAR *string;
#endif /* M_PROTO */
{
        const M_WCHAR *p;
        int len;
        ...
        if (newpar->type == CDATA)
                return (TRUE);
        if (m_allwhite(string))
                return (FALSE);
        ...
        case NUTOKEN:
                ...
                for (p = string; *p; p++) {
                        if (m_cttype(*p) == M_NONNAME)
                                ...
                                case NAMETYPE:
                                case ENTATT:
                                        if (m_cttype(*p) != M_NMSTART)
                                                return (FALSE);
                                        break;
                                case NUMBER:
                                case NUTOKEN:
                                        if (m_cttype(*p) != M_DIGIT)
                                                return (FALSE);
                                        break;
                                }
                        first = FALSE;
...
#else
void normdefault(string) M_WCHAR *string;
#endif /* M_PROTO */
{
        M_WCHAR *p, *q;
        int i, ret;
        ...
        default:
                for (; *string; string++) {
                        char mbyte[32]; /* bigger than the biggest multibyte
                                           char */
                        int length;
...
void prulend(M_NOPAR) {
        ...
        for (lhsp = lhs; lhsp;) {
                if (lhsp->elt->parptr)
                        m_err1("Program error: prulend for %s, element with "
...
#else
void setdefault() M_WCHAR *string;
#endif /* M_PROTO */
{
        PTYPE *kw;
        int keycount;
        ...
        if (newpar->type != GRPO) {
                ...
                if (!checkdefault(string)) {
                        m_err2("\"%s\": Not a valid default value for "
                               "parameter %s",
                               string, newpar->paramname);
                        return;
                }
                ...
                /* Value is valid.  Change tabs and RE's to spaces, if value is
                   not CDATA, put it in all caps, remove extra white space. */
                ...
                m_err2("Invalid default: %s not a valid value for %s", string,
                       newpar->paramname);
        }
}
