 ...
 */
/* $XConsortium: buildutl.c /main/3 1995/11/08 10:00:51 rswiston $ */
/*
...
/* Verify that <PARAM>, <MIN> or <USEMAP> has not previously occurred in
   this rule */
void found(flag, delim) LOGICAL *flag;
char *delim;
{
        M_WCHAR wcbuff[129];
        ssize_t length;
...
void rulend(M_NOPAR) {
        ...
        if (contype == GRPO)
                fsa = startfsa(ruletree, &canbenull);
        if (pcount > maxpar)
                maxpar = pcount;
        inptr = lhs->elt->inptr;
        exptr = lhs->elt->exptr;
        for (lhsp = lhs; lhsp;) {
                ...
                if (plist) {
                        if (lhsp->elt->parptr)
                                warning1(
                                    "Redefining parameter list for element %s",
                                    lhsp->elt->enptr);
...
void skiptoend(M_NOPAR) {
        ...
        if (!errlev++) {
                curcon = ERROR;
                for (i = scan();; i = scan())
                        if ((i == REND && !entrule) || (i == TAGC && entrule) ||
                            i == ENDFILE)
                                break;
                curcon = RULE;
                entrule = FALSE;
