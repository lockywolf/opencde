 ...
 */
/* $XConsortium: sref.c /main/3 1995/11/08 10:03:32 rswiston $ */
...
/* Add an entity with the default name constructed by adding a suffix
   ...
   a prefix m- */
void adddefent(mapname) M_WCHAR *mapname;
{
        M_WCHAR *p;
        int n;
        char c;
...
LOGICAL addent(name) M_WCHAR *name;
{
        M_ENTITY *new;
...
#else
LOGICAL addmapname(p, define) M_WCHAR *p;
LOGICAL define;
#endif
{
        MAP *old;
...
void addsref(p) M_WCHAR *p;
{
        ...
        delim = (SREFSTRUCT *)m_malloc(sizeof(SREFSTRUCT),
                ...
                for (q = p; *q; q++) {
                        ...
                        if (m_cttype(c) == M_NMSTART) {
                                if (c != BLANKSEQ && c != WSSEQ && c != RS) {
                                        m_free(delim,
                                               "short reference delimiter");
                                        m_err1("Letters not permitted in short "
                                               "reference delimiters: %s",
                                               p);
                                        return;
                                }
                                if (c != RS)
        ...
        /* Add the delimiter to this map and prepare for the associated
           entity */
        thissref =
            (SREFDATA *)m_malloc(sizeof(SREFDATA), "short reference delimiter");
        thissref->map = curmap;
