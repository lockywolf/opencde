 ...
 */
/* $XConsortium: upcmp8.c /main/3 1995/11/08 09:56:49 rswiston $ */
/*
...
#else
int m_wcupstrcmp(p, q) M_WCHAR *p, *q;
#endif /* M_PROTO */
{
        do {
                if (m_ctupper(*p) < m_ctupper(*q))
                        return (-1);
                if (m_ctupper(*p) > m_ctupper(*q))
                        return (1);
                q++;
...
#else
int m_wcmbupstrcmp(p, q) M_WCHAR *p;
char *q;
#endif
{
        M_WCHAR *wc;
