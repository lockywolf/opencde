 ...
 */
/* $XConsortium: malloc.c /main/3 1995/11/08 09:52:58 rswiston $ */
...
void m_free(block, msg) void *block;
char *msg;
{
        char buffer[32];
...
void *m_malloc(size, msg) int size;
char *msg;
{
        char buffer[32];
        void *p;
    ...
    defined(__uxp__)
                snprintf(buffer, 32, "%5x:%5x",
                         (unsigned int)((unsigned long)p >> 16),
                         (unsigned int)p);
#else
...
void *m_realloc(ptr, size, msg) void *ptr;
int size;
char *msg;
{
        char buffer[32];
