 ...
 */
/* $XConsortium: struct.c /main/3 1995/11/08 09:44:24 rswiston $ */
/*
...
/* M_checkstart tests to see if the element (or #PCDATA) indicated by VAL
   ...
   inclusion exception. */
int m_checkstart(val) M_ELEMENT val;
{
        M_PARSE *stackptr;
        int except;
        ...
        if (m_stacktop->oldtop) {
                if (m_element[m_stacktop->element - 1].content == M_ANY)
                        return (TRUE);
        ...
        /* Check content model */
        if (m_transition(val, TRUE))
                return (TRUE);

        /* Check for inclusions */
        for (stackptr = m_stacktop; stackptr->oldtop;
             stackptr = stackptr->oldtop)
                for (except = m_element[stackptr->element - 1].inptr; except;
                     except = m_exception[except - 1].next)
                        if (m_exception[except - 1].element == val)
                                return (M_NONCONTEXTUAL);
...
#else
LOGICAL m_ckend(val, neednet) M_ELEMENT val;
LOGICAL neednet;
#endif
{
        M_PARSE *stackptr;
        ...
        for (stackptr = m_stacktop; stackptr; stackptr = stackptr->oldtop) {
                /* If the element at stackptr has a content model, make sure
                   each open fsa is in a final state and that all required
                   submodels of open and-groups have occurred */
                for (fsastack = stackptr->fsastack; fsastack;
                     fsastack = fsastack->oldtop) {
                        if (!m_state[fsastack->current - 1].final) {
                                m_freemin(m_minend, "end-tag minimization");
                                return (FALSE);
                        }
                        for (pand = fsastack->andgroup; pand;
                             pand = m_andgroup[pand - 1].next) {
                                /* Don't bother checking if optional submodel of
                                   an and-group has occurred */
                                if (m_state[m_andgroup[pand - 1].start - 1]
                                        .final)
                                        continue;
                                for (usedand = fsastack->usedand; usedand;
                                     usedand = usedand->next)
                                        if (usedand->group == pand)
                                                break;
                                if (!usedand) {
                ...
                /* Have confirmed that the element indicated by stackptr can end
                 * now */
                if (stackptr->element == val)
                        break;
                *m_nextme =
                    (M_MIN *)m_malloc(sizeof(M_MIN), "end-tag minimization");
                (*m_nextme)->next = NULL;
        ...
        for (minend = m_minend; minend;) {
                ...
                if (m_stacktop->neednet && !neednet) {
                        ...
                        m_err2("Expecting %s to end %s", wc_net,
        ...
        if (m_stacktop->neednet != neednet) {
                ...
                wc_net = MakeWideCharString(m_net);
                if (neednet)
                        m_err4("Expecting %s%s%s instead of %s", wc_etago,
                               m_nameofelt(m_stacktop->element), wc_tagc,
                               wc_net);
                else
                        m_err4("Expecting %s to end %s%s%s", wc_net, wc_stago,
                               m_nameofelt(m_stacktop->element), wc_tagc);
                m_showcurelt();
...
M_PARSE *m_copystackelt(M_NOPAR) {
        ...
        copy->param = NULL;
        for (oldfsa = m_stacktop->fsastack, newfsa = &copy->fsastack; oldfsa;
             oldfsa = oldfsa->oldtop, newfsa = &(*newfsa)->oldtop) {
                *newfsa = (M_OPENFSA *)m_malloc(sizeof(M_OPENFSA), "FSA");
                ...
                for (oldand = oldfsa->usedand, newand = &(*newfsa)->usedand;
                     oldand; oldand = oldand->next, newand = &(*newand)->next) {
                        *newand = (M_ANDLIST *)m_malloc(sizeof(M_ANDLIST),
                                                        "and list");
...
void m_done(M_NOPAR) {
        ...
        while (m_stacktop->oldtop) {
                ...
                if (!m_ckend(m_stacktop->element, FALSE)) {
                        m_err1("More content expected in element %s at end of "
                               "document",
                               m_nameofelt(m_stacktop->element));
                        m_expecting();
                        m_showcurelt();
...
LOGICAL m_excluded(elt) M_ELEMENT elt;
{
        M_PARSE *stackptr;
        int except;
        ...
        for (stackptr = m_stacktop; stackptr->oldtop;
             stackptr = stackptr->oldtop)
                for (except = m_element[stackptr->element - 1].exptr; except;
                     except = m_exception[except - 1].next)
                        if (m_exception[except - 1].element == elt)
                                return (TRUE);
        return (FALSE);
}
...
/* Free the OPEN FSA substructures associated with an element on
   the parse stack */
void m_freeFSA(stackelt) M_PARSE *stackelt;
{
        M_OPENFSA *fsastack;
        M_ANDLIST *usedand;
...
void m_freemin(min, msg) M_MIN *min;
char *msg;
{
        M_MIN *discard;
...
LOGICAL m_nextand(thisfsa, label) M_OPENFSA *thisfsa;
M_ELEMENT label;
{
        M_ANDLIST *newgroup;
        M_ANDGROUP pand;
        ...
        /* Verify currently within an and-group and in final state of this
           branch */
        if (!m_state[thisfsa->current - 1].final)
                return (FALSE);
        if (!thisfsa->oldtop)
                return (FALSE);
        ...
        m_stacktop->fsastack = (M_OPENFSA *)m_malloc(sizeof(M_OPENFSA), "FSA");
        ...
        for (pand = thisfsa->oldtop->andgroup; pand;
             pand = m_andgroup[pand - 1].next) {
        ...
        if (m_nextand(thisfsa->oldtop, label))
                return (TRUE);
        return (FALSE);
}
...
#else
void m_push(elt, current, need) M_ELEMENT elt;
M_STATE current;
LOGICAL need;
#endif
{
...
void m_strtcdata(scanval) int scanval;
{
        if (!m_strtproc(M_NULLVAL))
                if (m_whitespace((M_WCHAR)scanval)) {
                        ...
                        if (m_stacktop->oldtop) {
                                m_err1("Data characters not allowed at this "
                                       "point in %s",
                                       m_nameofelt(m_stacktop->element));
                                m_expecting();
                                m_showcurelt();
                        } else if (!m_start) {
                                m_error("Document may not start with data "
                                        "characters");
                                m_expecting();
                        }
                }
...
*/
LOGICAL m_strtproc(scanval) M_ELEMENT scanval;
{
        int check;
        M_PARSE *original;
        int savecontext;
        ...
        if (check = m_checkstart(scanval)) {
                ...
                if (check == M_NONCONTEXTUAL)
                        m_stacktop->contextual = FALSE;
                else if (scanval)
                        m_stacktop->oldtop->intext = FALSE;
                return (TRUE);
        }
        ...
        for (min = m_minstart; min;) {
                ...
                if (!m_element[min->val - 1].smin)
                        m_err1("Missing start tag for %s",
                               m_nameofelt(min->val));
                m_stkdefaultparams();
                discard = min;
                min = min->next;
...
#else
void m_strttag(val, net) M_ELEMENT val;
LOGICAL net;
#endif
{
...
LOGICAL m_textpermitted(M_NOPAR) {
        ...
        /* If current state emits an arc labelled #PCDATA, text can be
           entered */
        for (fsastack = m_stacktop->fsastack; fsastack;
             fsastack = fsastack->oldtop) {
                for (pand = fsastack->andgroup; pand;
                     pand = m_andgroup[pand - 1].next) {
                        for (usedand = fsastack->usedand; usedand;
                ...
                if (morebranches)
                        return (FALSE);
                if (m_state[fsastack->current - 1].datacontent)
                        return (TRUE);
                if (!m_state[fsastack->current - 1].final)
                        return (FALSE);
        }
...
#else
LOGICAL m_transition(label, recur) M_ELEMENT label;
LOGICAL recur;
#endif
{
        M_ARC parc;
        ...
        if (m_stacktop->oldtop &&
            m_element[m_stacktop->element - 1].content != M_REGEXP)
                return (FALSE);
        for (parc = m_state[m_stacktop->fsastack->current - 1].first; parc;
        ...
        if (recur && m_nextand(m_stacktop->fsastack, label))
                return (TRUE);
        return (FALSE);
} /* End transition */
