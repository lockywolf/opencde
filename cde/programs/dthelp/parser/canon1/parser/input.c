...
void m_etcomplete(M_NOPAR) {
        if (!m_scanel) {
                ...
                m_err3("%s%s%s ignored: element undefined", wc_etago, m_name,
                       wc_tagc);
                m_free(wc_etago, "multi-byte string");
                m_free(wc_tagc, "multi-byte string");
        } else if (!m_ckend(m_scanel, FALSE)) {
...
void m_frcend(val) M_ELEMENT val;
{
        M_PARSE *stackptr;
        M_ELEMENT poppedval;
...
void m_holdproc(M_NOPAR) {
        if (m_stacktop->holdre) {
                ...
                if (m_stacktop->intext)
                        ...
                        if (m_curcon == NETDATACON)
                                m_curcon = NETCDATA;
                        else if (m_curcon == DATACON)
                                m_curcon = POUNDCDATA;
                }
        }
}
...
void m_stcomplete(M_NOPAR) {
        ...
        if (m_strtproc(m_scanel))
                ...
                if (m_stacktop->holdre)
                        m_holdproc();
                /* M_strtproc should not return TRUE as long as #PCDATA is
                   considered optional, but this code should work if the
                   standard is changed */
                if (m_strtproc(m_scanel))
                        m_stkparams();
                else {
                        if (m_stacktop->oldtop) {
                                ...
                                wc_tagc = MakeWideCharString(m_tagc);
                                m_err6("Expecting %s%s%s instead of %s%s%s to "
                                       "start document",
                                       wc_stago, m_nameofelt(1), wc_tagc,
                                       wc_stago, m_nameofelt(m_scanel),
                                       wc_tagc);
        ...
        if (m_element[m_scanel - 1].content == M_NONE)
                m_endtag(m_scanel);
}
