...
/* Process the value for the parameter whose index number was previously
   saved in m_ppsave */
void m_attval(string) M_WCHAR *string;
{
        const M_WCHAR *p;
        ...
        if (p = m_partype(m_ppsave, string)) {
                if (m_parameter[m_ppsave - 1].type == M_KEYWORD)
                        ...
                        m_poccur[m_psave] = (M_WCHAR *)m_malloc(
                            w_strlen(p) + 1, "parameter value");
                        w_strcpy(m_poccur[m_psave], p);
                        m_parupper(m_ppsave, m_poccur[m_psave]);
                }
                m_updatedefault(m_ppsave, p);
        } else
                m_err2("\"%s\" -- error in value for parameter %s", string,
...
LOGICAL m_attvonly(string) M_WCHAR *string;
{
        const M_WCHAR *p;
        int par, i;
        ...
        for (par = m_element[m_scanel - 1].parptr, i = 0;
             i < m_element[m_scanel - 1].parcount; par++, i++)
                if (m_parameter[par - 1].type == M_KEYWORD)
                        if (p = m_partype(par, string)) {
                                if (m_poccur[i])
                                        m_err2("Redefinition of parameter %s. "
                                               "Discarding old value '%s'.",
                                               &m_pname[m_parameter[par - 1]
                                                            .paramname],
                                               m_poccur[i]);
             ...
             i < m_element[m_scanel - 1].parcount; par++, i++)
                if (!m_poccur[i]) {
                        ...
                        if (m_poccur[i]) {
                                ...
                                m_poccur[i] = (M_WCHAR *)m_malloc(
                                    w_strlen(string) + 1, "parameter value");
                                w_strcpy(m_poccur[i], string);
                                m_updatedefault(par, m_poccur[i]);
                                return (TRUE);
                        }
                }
        m_err2("%s: impossible value for any parameters of %s", string,
...
void m_findatt(M_NOPAR) {
        ...
        if (!m_wholetag) {
                ...
                m_err3("No %s parameter for %s (possibly missing %s)",
                       m_saveatt, m_nameofelt(m_scanel), wc_tagc);
...
void m_freeparam(stackelt) M_PARSE *stackelt;
{
        int i;
        int par;
                     ...
                     i < m_element[stackelt->element - 1].parcount; i++, par++)
                        if (m_parameter[par].type != M_KEYWORD)
                                if (m_parameter[par].deftype != M_NAMEDEF ||
                                    stackelt->param[i] !=
                                        m_parameter[par].defval)
                                        if (stackelt->param[i])
                                                m_free(stackelt->param[i],
                                                       "parameter value");
...
void m_parupper(par, string) int par;
M_WCHAR *string;
{
        M_WCHAR *p;
        M_WCHAR *q;
        ...
        case M_NUMS:
                ...
                if (i && m_whitespace(string[i - 1]))
                        string[i - 1] = M_EOS;
                return;
        default:
                for (; *string; string++)
                        if (*string == M_RE || *string == M_TAB)
                                *string = M_SPACE;
                return;
        }
}
...
int i;
{
        ...
        if (m_parameter[par - 1].type == M_ENTATT)
                if (!(M_ENTITY *)m_lookfortrie(m_parameter[par - 1].defval,
                                               m_enttrie)) {
                        m_err1("Interface error: Default entity %s undefined",
                               poccur[i]);
                        poccur[i] = NULL;
                        return;
                }
        ...
        if (m_parameter[par - 1].deftype != M_CURRENT &&
            m_parameter[par - 1].deftype != M_CHANGEDCUR)
                return;
        poccur[i] =
            (M_WCHAR *)m_malloc(w_strlen(poccur[i]) + 1, "parameter value");
...
void m_stkparams(M_NOPAR) {
        ...
        for (i = 0, par = m_element[m_scanel - 1].parptr;
             i < m_element[m_scanel - 1].parcount; i++, par++) {
                if (!m_poccur[i])
                        m_stkonedef(par, m_scanel, m_stacktop->param, i);
                else
                        m_stacktop->param[i] = m_poccur[i];
        }
        m_strtaction(m_stacktop->element);
}
...
#else
void m_updatedefault(par, string) int par;
M_WCHAR *string;
#endif /* M_PROTO */
{
        if (m_parameter[par - 1].deftype != M_CURRENT &&
        ...
        /* Save value in allocated storage */
                ...
                if (m_parameter[par - 1].deftype == M_CHANGEDCUR)
                        m_free(m_parameter[par - 1].defval,
                               "updateable default");
                m_parameter[par - 1].defval = (M_WCHAR *)m_malloc(
                    w_strlen(string) + 1, "updateable default");
...
LOGICAL m_validinpar(string) M_WCHAR *string;
{
        int par;
        M_WCHAR *p;
        int i;
        ...
        for (par = m_element[m_scanel - 1].parptr, i = 0;
             i < m_element[m_scanel - 1].parcount; par++, i++) {
                ...
                if (!m_wcupstrcmp(&m_pname[m_parameter[par - 1].paramname],
                                  string))
                        return (TRUE);
                /* Check if possible value of a keyword parameter or of a
                   non-keyword parameter that has not yet occurred*/
        ...
        wc_tagc = MakeWideCharString(m_tagc);
        if (!m_wholetag)
                m_err1("Invalid parameter or missing %s", wc_tagc);
        ...
        for (p = string; *p; p++)
                ;
        for (p--; p >= string; p--)
                m_ungetachar((int)*p, M_NORMAL, TRUE);
        return (FALSE);
}
