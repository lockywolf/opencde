...
#else
void m_ckmap(name, useoradd) M_WCHAR *name;
LOGICAL useoradd;
#endif
{
        int mapid;
        ...
        if (mapid = m_packedlook(m_maptree, name))
                ...
                m_err1("Undefined short reference map %s", name);
}
...
void m_ntrent(p) M_WCHAR *p;
{
        ...
        if (m_entity = (M_ENTITY *)m_ntrtrie(p, m_enttrie, (M_TRIE *)new)) {
                ...
                if (m_entity->wheredef == M_DPARSER) {
                        ...
                        if (m_entity->content) {
                                m_err1("Warning: Redefinition of predefined "
                                       "entity %s",
                                       p);
