...
void chapstart(id) M_WCHAR *id;
{
        ...
        tableno = 0;
        footnoteno = 1;
        fprintf(stderr, "\n%s %s%s. ", chapterPrefixString, chapstring,
                chapterSuffixString);
...
#else
static int mb_getqualified(qualname, unqualname) char *qualname;
char *unqualname;
#endif
{
    ...
    *gentity, *gposition, *ghyperlink, *glinktype, *gdescription;
{
        ...
        */
        ...
        if (gentity) {
                ...
                fprintf(outfile, "<HEAD SSI=\"%s-GRAPHIC-%s\">", ssi,
                        leftright);
                if (ghyperlink)
        ...
        if (!ghyperlink && (glinktype || gdescription)) {
                m_eprefix();
                fprintf(stderr, "Error: %sP%s ghyperlink was undefined.\n",
                        m_stago, m_tagc);
                fprintf(m_errfile, "Error: %sP%s ghyperlink was undefined.\n",
                        m_stago, m_tagc);
                fprintf(stderr,
                        "Beware of using a reserved value such as:\n%s, %s, "
                        "%s, %s, %s, or %s.\n",
                        QJUMP, QJUMPNEWVIEW, QDEFINITION, QEXECUTE, QAPPDEFINED,
                        QMAN);
                fprintf(m_errfile,
                        "Beware of using a reserved value such as:\n%s, %s, "
                        "%s, %s, %s, or %s.\n",
...
#endif
{
        ...
        if ((listtype != PLAIN) &&
            (listtype != MILSPEC)) { /* don't emit label for type==PLAIN and
                                        type==MILSPEC */
                if (isBullet) {
                        if (isLoose) {
                                if (isFirst)
                                        ...
                                        pThisBulletId =
                                            &looseBulletId[looseBulletIdIndex];
                                }
                        } else {
                                if (isFirst)
                                        ...
                                        pThisBulletId =
                                            &bulletId[bulletIdIndex];
                                }
                        }
                        if (*pThisBulletId) { /* use an existing block
                                                 containing a bullet */
                                strcpy(label_id, sdlReservedName);
                                m_itoa(*pThisBulletId,
                                       label_id + SDLNAMESIZ - 1);
                        } else {
...
#if defined(M_PROTO)
void starthelpnode(M_WCHAR *ssi, M_WCHAR *id, int level)
#else
void starthelpnode(ssi, id, level) M_WCHAR *ssi;
M_WCHAR *id;
int level;
#endif
{
        ...
        fprintf(outfile,
                "<VIRPAGE ID=\"%s\" LEVEL=\"%d\" LANGUAGE=\"%s\" "
                "CHARSET=\"%s\" DOC-ID=\"%s\" SSI=\"%s\">\n",
                mb_nodeid, level, GetLanguage(), GetCharset(), docId, mb_ssi);
        m_free(mb_ssi, "multi-byte string");
        snbstart = ftell(outfile);
} /* End starthelpnode(id) */

#if defined(M_PROTO)
void mb_starthelpnode(char *ssi, char *id, int level)
#else
void mb_starthelpnode(ssi, id, level) char *ssi;
char *id;
int level;
#endif
{
...
void StartLabList(spacing, longlabel) M_WCHAR *spacing;
M_WCHAR *longlabel;
{
        char *mb_spacing;
...
void EndList() {
        ...
        for (chain = lastlist->chain; chain; chain = chain->next) {
                if (lastlist->count > 999) {
                        m_error("Unable to support more than 999 items in an "
                                "ordered list");
                        lastlist->count = 999;
                }
        }
        if (lastlist->type == BULLET)
                bulcount--;
}
...
#else
    SetStdLocale(pLang, pCharset) char *pLang;
char *pCharset;
#endif
{
        ...
        } else { /* already local - set locale and get standard versions */
                if (_DtLcxXlateOpToStd(myDb, myPlatform, compVer,
                                       DtLCX_OPER_SETLOCALE, myLocale, NULL,
                                       &lang, &charset, NULL)) {
                        fprintf(stderr, "Warning: could not translate local "
                                        "locale to CDE\n");
                        if (m_errfile != stderr)
                                fprintf(m_errfile, "Warning: could not "
                                                   "translate local locale to "
                                                   "CDE\n");
                        strcpy(pLang, cString);
                        strcpy(pCharset, isoString);
                        _DtLcxCloseDb(&myDb);
...
#endif
{
        ...
        if (_DtLcxXlateOpToStd(myDb, myPlatform, compVer, DtLCX_OPER_SETLOCALE,
                               setlocale(LC_ALL, NULL), &stdLocale, NULL, NULL,
                               NULL)) {
                fprintf(stderr,
                        "Warning: could not translate local locale to CDE\n");
                if (m_errfile != stderr)
                        fprintf(m_errfile, "Warning: could not translate local "
                                           "locale to CDE\n");
                _DtLcxCloseDb(&myDb);
                return cString;
        }

        if (_DtLcxXlateStdToOp(myDb, myPlatform, compVer, DtLCX_OPER_SETLOCALE,
                               stdLocale, NULL, NULL, NULL, &opLocale)) {
                fprintf(stderr,
                        "Warning: could not translate CDE locale to local\n");
                if (m_errfile != stderr)
                        fprintf(m_errfile, "Warning: could not translate CDE "
                                           "locale to local\n");
                mb_free(&stdLocale);
                _DtLcxCloseDb(&myDb);
                return cString;
...
void SetDefaultLocale() {
        ...
        if (helpcharset) {
                ...
                elementName =
                    MakeWideCharString("LanguageElementDefaultCharset");
                if (m_lookent(elementName, &type, &content, &wheredef)) {
                        if (type == M_SDATA) {
...
void figure(M_WCHAR *number, M_WCHAR *tonumber, M_WCHAR *id, M_WCHAR *file,
            M_WCHAR *figpos, M_WCHAR *cappos, M_WCHAR *ghyperlink,
            M_WCHAR *glinktype, M_WCHAR *gdescription)
#else
void figure(number, tonumber, id, file, figpos, cappos, ghyperlink, glinktype,
            gdescription) M_WCHAR *number;
M_WCHAR *tonumber;
M_WCHAR *id;
M_WCHAR *file;
M_WCHAR *figpos;
...
#endif
{
        ...
        */
        ...
        fprintf(outfile, ">\n<REFITEM RID=\"%s\" CLASS=\"FIGURE\" SSI=\"FIG",
                snb_id);
        ...
        if (ftonumber) {
                fprintf(outfile,
                        "<HEAD TYPE=\"LINED\" CLASS=\"CAPTION\" "
                        "SSI=\"NUMBER%s\">%s %d.",
                        capposition, string, figno);
        }
...
#endif
{
        ...
        if (!(m_mblevel("HEAD") || m_mblevel("EXAMPLESEG") ||
              m_mblevel("ANNOTATION"))) {
                /* Don't re-enable output yet if in a <HEAD>, <EX> or
                 * <ANNOTATION> */
                tooutput = TRUE;
        }
        ...
        *termp = M_EOS;
                        ...
                        for (p = base; *p; p++) {
                                if ((p - base) >= MAXTERM) {
                                        if ((p - base) == MAXTERM) {
                                                ...
                                                wc_stago =
                                                    MakeWideCharString(m_stago);
                                                wc_tagc =
                                                    MakeWideCharString(m_tagc);
                                                m_err2("Too many characters in "
                                                       "BASEFORM for %sTERM%s",
                                                       wc_stago, wc_tagc);
                                                m_free(wc_stago,
                                                       "wide character string");
                                                m_free(wc_tagc,
        ...
        /* Handle the output */
        if (vgloss(gloss) == GLOSS) {
                sprintf(buffer, "<LINK WINDOW=\"%s\" RID=\"%s%d\">", linktype,
                        sdlReservedName, idn);
...
#else
M_WCHAR *wc_stringtoupper(wcp) M_WCHAR *wcp;
#endif
{
...
#else
void HandleLink(hyperlink, type, description) M_WCHAR *hyperlink;
M_WCHAR *type;
M_WCHAR *description;
#endif
{
        ...
        if (type) {
                ...
                if (m_wcmbupstrcmp(type, QJUMP)) { /* not type jump */
                        if (!m_wcmbupstrcmp(type, QJUMPNEWVIEW)) {
                                mb_strcode("WINDOW=\"NEW\" ", outfile);
                                global_linktype = 1;
                        } else if (!m_wcmbupstrcmp(type, QDEFINITION)) {
                                mb_strcode("WINDOW=\"POPUP\" ", outfile);
                                global_linktype = 2;
        ...
        if (hyperlink) {
                ...
                m_eprefix();
                fprintf(stderr, "Error: %sLINK%s hyperlink was undefined.\n",
                        m_stago, m_tagc);
                fprintf(m_errfile, "Error: %sLINK%s hyperlink was undefined.\n",
                        m_stago, m_tagc);
                fprintf(stderr,
                        "Beware of using a reserved value such as:\n%s, %s, "
                        "%s, %s, %s, or %s.\n",
                ...
                fprintf(m_errfile,
                        ...
                        QMAN);
                m_errline(
                    "Use ``hyperlink='' if the value contains non-alphabetics");
                m_esuffix();
                mb_hyperlink = mb_undefined;
        }

        if ((global_linktype <= 2) && hyperlink &&
            (!w_strchr(hyperlink, wsp))) {
                if (mb_hyperlink)
                ...
                case 1: /* jump new */
                case 2: /* definition */
                        sprintf(tmpsnb, "<CROSSDOC ID=\"%s%d\" XID",
                                sdlReservedName, nextId);
                        break;
                case 3: /* execute */
                        sprintf(tmpsnb, "<SYS-CMD ID=\"%s%d\" COMMAND",
                                sdlReservedName, nextId);
                        break;
                case 4: /* man */
                        sprintf(tmpsnb, "<MAN-PAGE ID=\"%s%d\" XID",
                                sdlReservedName, nextId);
                        break;
                case 5: /* app defined */
                        sprintf(tmpsnb, "<CALLBACK ID=\"%s%d\" DATA",
                                sdlReservedName, nextId);
                        break;
                }
                if (savesnb) {
                        ...
                        savesnb =
                            mb_realloc(savesnb, snblen + strlen(tmpsnb) + 1);
...
#else
static void AddToRowVec(length, rowvec, id) int *length;
char **rowvec;
char *id;
#endif
{
...
#else
void Add2ToRowVec(length, rowvec, id1, id2) int *length;
char **rowvec;
char *id1, *id2;
#endif
{
...
#else
void StartEx(*notes, *lines, *textsize) M_WCHAR *notes;
M_WCHAR *lines;
M_WCHAR *textsize;
#endif
{
...
#endif
{
        ...
        if (saveexseg) {
                if (exTextSize == SMALLEST)
                        ...
                        ssi = "NML";
                length =
                    sprintf(buffer, "<P TYPE=\"LITERAL\" SSI=\"EX-%s\">", ssi);
                ...
                saveex = mb_realloc(
                    saveex, svexlen + length + (svexseglen - 1) + annotLen + 5);
...
#else
void StartNCW(which) char *which;
#endif
{
        notehead = FALSE;
...
#else
void StartBlock(pclass, ssi, id) char *pclass, *ssi, *id;
#endif
{
        char localId[32];
        char *realId;
        ...
        if (formStackBase &&
            (formStackTop >=
             formStackBase)) { /* there is a <form> in progress */
                if (!id) {
                        sprintf(localId, "%s%d", sdlReservedName, NextId());
                        realId = localId;
                }
                AddToRowVec(&(formStackTop->vecLen), &(formStackTop->rowVec),
                            realId);
        }
...
#else
void StartNCWtext(which, iconFile, headingString) char *which;
char *iconFile;
char *headingString;
#endif
{
...
#endif
{
        ...
        if (tossfile >= 0) {
                fputs("<TOSS>\n", outfile);
                while ((bytesread = read(tossfile, filebuf, sizeof(filebuf))) >
                       0)
                        if (fwrite(filebuf, sizeof(*filebuf), bytesread,
                                   outfile) != bytesread)
                                break;
                if (bytesread != 0) {
                        ...
                        m_err2("error copying %s to output (%s) as the <toss> "
                               "element",
                               wc_try, wc_outputname);
...
/* Cyclent.c contains procedure m_cyclent(), callable by interface
   ...
   about them */
#if defined(M_PROTO)
M_WCHAR *CycleEnt(LOGICAL init, unsigned char *type, M_WCHAR ***content,
                  unsigned char *wheredef)
#else
M_WCHAR *m_cyclent(init, type, content, wheredef) LOGICAL init;
unsigned char *type;
M_WCHAR ***content;
unsigned char *wheredef;
#endif
...
#else
void PushForm(class, ssi, id) char *class;
char *ssi;
char *id;
#endif
{
        ...
        if (formStackTop == formStackMax) {
                if (!formStackBase) {
                        formStackBase =
                            (FORMINFO *)malloc(10 * sizeof(FORMINFO));
                        formStackTop = formStackBase;
                        formStackMax = formStackBase + 9;
                } else {
                        stackSize = formStackMax - formStackBase + 1;
                        formStackBase = realloc(
                            formStackBase, (stackSize + 10) * sizeof(FORMINFO));
...
#else
void PushForm2(class, ssi, id1, id2) char *class;
char *ssi;
char *id1;
char *id2;
#endif
{
        ...
        if (formStackBase &&
            (formStackTop >=
             formStackBase)) { /* there is a <form> in progress */
                Add2ToRowVec(&(formStackTop->vecLen), &(formStackTop->rowVec),
                             id1, id2);
        }
...
void EmitSavedAnchors() {
        ...
        if (parTextId) {
                sprintf(buffer, "<ANCHOR ID=\"%s%d\">", sdlReservedName,
                        parTextId);
                mb_strcode(buffer, outfile);
                parTextId = 0;
        }
