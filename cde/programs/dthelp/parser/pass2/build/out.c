...
void dumpentnode(file, value) FILE *file;
M_TRIE *value;
...
void eltreeout(M_NOPAR) {
        ...
        for (eltp = firstelt; eltp; eltp = eltp->next) {
                if (first)
                        ...
                        fputs(",\n", dtd);
                if (!eltp->content)
                        warning1("No content model for element %s",
                                 eltp->enptr);
                fprintf(dtd, "  {%d, %d, %s, ", enameindex,
                        eltp->model ? eltp->model->count : 0,
                        typecon(eltp->content));
                fprintf(dtd, "%d, %d, ", eltp->inptr, eltp->exptr);
                fprintf(dtd, "%d, %d, %d", eltp->parindex, eltp->paramcount,
                        eltp->srefptr);
                enameindex += w_strlen(eltp->enptr) + 1;
                fprintf(dtd, ", %s, %s, %s}", boolean(eltp->stmin),
                        boolean(eltp->etmin), boolean(eltp->useoradd));
        }
        fputs(ndif, dtd);
}

/* Enttype returns a string indicating the type of an entity */
char *enttype(n) int n;
{
        switch (n) {
        case M_GENERAL:
                return (mgeneral);
...
void fsa(M_NOPAR) {
        ...
        for (pstate = firststate; pstate; pstate = pstate->next) {
                if (first)
                        ...
                        fputs(",\n", dtd);
                fprintf(dtd, "  {%s, %s, %d}", boolean(pstate->final),
                        boolean(pstate->datacontent),
                ...
                for (parc = pstate->first; parc; parc = parc->next) {
                        ...
                        fprintf(farc, "  %d, %s, %d, %d, %d, %d",
                                ...
                                parc->to->count, parc->next ? ++arcount : 0);
                }
        }
        fputs(ndif, dtd);
        fputs(ndif, farc);
        ...
        if (andused) {
                ...
                for (pand = firstand; pand; pand = pand->nextptr) {
                        if (first)
                                ...
                                fputs(",\n", dtd);
                        fprintf(dtd, "  {%d, %d}", pand->start->count,
        ...
        if (parcount) {
                ...
                for (paramp = firstpar; paramp; paramp = paramp->nextptr) {
                        if (first)
                                ...
                                fputs(",\n", dtd);
                        fprintf(dtd, "  {%d, %s, %d, %s, ", pnameindex,
                                partype(paramp->type), paramp->kwlist,
                                deftype(paramp->deftype));
                        pnameindex += w_strlen(paramp->paramname) + 1;
                        if (paramp->defval)
                                fprintf(dtd, "&m_keyword[%d]}",
                                        paramp->defval - 1);
...
void srefout(M_NOPAR) {
        ...
        if (mapcnt) {
                ...
                if (!mapbysref) {
                        m_error("Unable to allocate workspace to process short "
                                "references");
                        exit(TRUE);
                }
        ...
        fputs("M_DTDEXTERN M_SREF m_sref", dtd);
        if (count) {
                fprintf(dtd, "[%d]\n#if defined(M_DTDDEF)\n  = {\n", count);
                first = TRUE;
                count = 0;
...
void tempelt(eltp, tempfile) ELTSTRUCT *eltp;
FILE *tempfile;
{
        PARAMETER *paramp;
        PTYPE *ptypep;
        ...
        if (eltp->parptr) {
                ...
                for (paramp = eltp->parptr; paramp; paramp = paramp->next) {
                        ...
                        for (p = paramp->paramname; *p; p++, indent += 2) {
                                int length;
                                char mbyte[32]; /* larger than any multibyte
                                                   character */
                                char *pc;
                                unsigned char c;
                                ...
                                if (length < 0)
                                        m_error("Invalid multibyte character "
                                                "found in an element");
                                else {
                                        pc = mbyte;
                                        if (length == 1) {
                                                c = *pc;
                                                putc(isupper(c) ? tolower(c)
                                                                : c,
                                                     tempfile);
                                        } else
                                                while (--length >= 0)
                        ...
                        if (paramp->type == GRPO) {
                                ...
                                for (ptypep = paramp->ptypep; ptypep;) {
                                        ...
                                        mb_keyword =
                                            MakeMByteString(ptypep->keyword);
                                        fprintf(tempfile, "%s = %s", mb_keyword,
                                                mb_keyword);
                                        m_free(mb_keyword, "multi-byte string");
                                        ptypep = ptypep->next ? ptypep->nextptr
                                                              : (PTYPE *)NULL;
                                        if (ptypep) {
                                                fputs(",\n", tempfile);
                                                for (i = 0; i < indent; i++)
