...
#else
void PutString(string) char *string;
#endif
{
...
#else
void PutWString(string) M_WCHAR *string;
#endif
{
        char *mb_string;
...
#else
void PutWChar(wchar) M_WCHAR wchar;
#endif
{
        M_WCHAR wc_string[2];
        char *mb_string;
...
#else
void SaveWChar(wchar) M_WCHAR wchar;
#endif
{
        CDATA_el *pCdata;
        ...
        if (pCdata->buffPos == pCdata->buffSize) {
                pCdata->buffSize += 256;
                pCdata->buffer = m_realloc(pCdata->buffer, pCdata->buffSize,
                                           "u_cdata.buffer");
        }
