...
#else
char *mb_realloc(ptr, size) char *ptr;
long size;
#endif
{
        void *vptr;
...
#else
void OpenTag(hasAttributes, newlineOK) LOGICAL hasAttributes;
LOGICAL newlineOK;
#endif
{
        PutString("<");
...
#else
static void OpenNamedTag(name, hasAttributes, newlineOK) LOGICAL hasAttributes;
LOGICAL newlineOK;
char *name;
#endif
{
...
#else
void CloseTag(newlineOK) LOGICAL newlineOK;
#endif
{
        char *etagc;
...
#else
static void CloseNamedTag(name, newlineOK) LOGICAL newlineOK;
char *name;
#endif
{
        char *etagc;
...
#else
void ImpliedAttribute(name, value) char *name;
M_WCHAR *value;
#endif
{
        if (value) {
...
#else
void RequiredAttribute(name, value) char *name;
M_WCHAR *value;
#endif
{
        M_WCHAR null;
...
#else
M_WCHAR *CopyWString(string) M_WCHAR *string;
#endif
{
        M_WCHAR *retVal;
...
#else
void MakeNewElement(type) ElementTypes type;
#endif
{
        ...
        if (type == e_cdata) {
                pCurrentElement->u.u_cdata.buffer =
                    m_malloc(256, "u_cdata.buffer");
...
#else
void DeleteElement(pElement) ElementPtr pElement;
#endif
{
        switch (pElement->type) {
        case e_sdldoc:
                if (pElement->u.u_sdldoc.pub_id)
                        m_free(pElement->u.u_sdldoc.pub_id,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.doc_id)
                        m_free(pElement->u.u_sdldoc.doc_id,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.first_page)
                        m_free(pElement->u.u_sdldoc.first_page,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.timestmp)
                        m_free(pElement->u.u_sdldoc.timestmp,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.product)
                        m_free(pElement->u.u_sdldoc.product,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.prodpn)
                        m_free(pElement->u.u_sdldoc.prodpn,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.prodver)
                        m_free(pElement->u.u_sdldoc.prodver,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.license)
                        m_free(pElement->u.u_sdldoc.license,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.language)
                        m_free(pElement->u.u_sdldoc.language,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.charset)
                        m_free(pElement->u.u_sdldoc.charset,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.author)
                        m_free(pElement->u.u_sdldoc.author,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.version)
                        m_free(pElement->u.u_sdldoc.version,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.sdldtd)
                        m_free(pElement->u.u_sdldoc.sdldtd,
                               wideCharacterString);
                if (pElement->u.u_sdldoc.srcdtd)
                        m_free(pElement->u.u_sdldoc.srcdtd,
                               wideCharacterString);
                break;
        case e_virpage:
                ...
                if (pElement->u.u_virpage.level)
                        m_free(pElement->u.u_virpage.level,
                               wideCharacterString);
                if (pElement->u.u_virpage.version)
                        m_free(pElement->u.u_virpage.version,
                               wideCharacterString);
                if (pElement->u.u_virpage.language)
                        m_free(pElement->u.u_virpage.language,
                               wideCharacterString);
                if (pElement->u.u_virpage.charset)
                        m_free(pElement->u.u_virpage.charset,
                               wideCharacterString);
                if (pElement->u.u_virpage.doc_id)
                        m_free(pElement->u.u_virpage.doc_id,
                               wideCharacterString);
        ...
        case e_form:
                ...
                if (pElement->u.u_form.language)
                        m_free(pElement->u.u_form.language,
                               wideCharacterString);
        ...
        case e_frowvec:
                ...
                if (pElement->u.u_frowvec.cells)
                        m_free(pElement->u.u_frowvec.cells,
                               wideCharacterString);
                break;
        case e_fdata:
                break;
        ...
        case e_head:
                ...
                if (pElement->u.u_head.language)
                        m_free(pElement->u.u_head.language,
                               wideCharacterString);
        ...
        case e_subhead:
                ...
                if (pElement->u.u_subhead.version)
                        m_free(pElement->u.u_subhead.version,
                               wideCharacterString);
                if (pElement->u.u_subhead.class)
                        m_free(pElement->u.u_subhead.class,
                               wideCharacterString);
                if (pElement->u.u_subhead.language)
                        m_free(pElement->u.u_subhead.language,
                               wideCharacterString);
                if (pElement->u.u_subhead.charset)
                        m_free(pElement->u.u_subhead.charset,
                               wideCharacterString);
                ...
                if (pElement->u.u_subhead.abbrev)
                        m_free(pElement->u.u_subhead.abbrev,
                               wideCharacterString);
                break;
        case e_anchor:
        ...
        case e_refitem:
                ...
                if (pElement->u.u_refitem.class)
                        m_free(pElement->u.u_refitem.class,
                               wideCharacterString);
                if (pElement->u.u_refitem.button)
                        m_free(pElement->u.u_refitem.button,
                               wideCharacterString);
        ...
        case e_sphrase:
                if (pElement->u.u_sphrase.class)
                        m_free(pElement->u.u_sphrase.class,
                               wideCharacterString);
        ...
        case e_graphic:
                ...
                if (pElement->u.u_graphic.format)
                        m_free(pElement->u.u_graphic.format,
                               wideCharacterString);
                if (pElement->u.u_graphic.method)
                        m_free(pElement->u.u_graphic.method,
                               wideCharacterString);
        ...
        case e_text:
                ...
                if (pElement->u.u_text.language)
                        m_free(pElement->u.u_text.language,
                               wideCharacterString);
        ...
        case e_animate:
                ...
                if (pElement->u.u_animate.format)
                        m_free(pElement->u.u_animate.format,
                               wideCharacterString);
                if (pElement->u.u_animate.method)
                        m_free(pElement->u.u_animate.method,
                               wideCharacterString);
        ...
        case e_script:
                ...
                if (pElement->u.u_script.interp)
                        m_free(pElement->u.u_script.interp,
                               wideCharacterString);
                break;
        case e_crossdoc:
        ...
        case e_textfile:
                ...
                if (pElement->u.u_textfile.offset)
                        m_free(pElement->u.u_textfile.offset,
                               wideCharacterString);
                if (pElement->u.u_textfile.format)
                        m_free(pElement->u.u_textfile.format,
                               wideCharacterString);
                break;
        case e_sys_cmd:
                ...
                if (pElement->u.u_sys_cmd.command)
                        m_free(pElement->u.u_sys_cmd.command,
                               wideCharacterString);
                break;
        case e_callback:
                ...
                if (pElement->u.u_callback.data)
                        m_free(pElement->u.u_callback.data,
                               wideCharacterString);
                break;
        case e_switch:
                ...
                if (pElement->u.u_switch.interp)
                        m_free(pElement->u.u_switch.interp,
                               wideCharacterString);
                if (pElement->u.u_switch.branches)
                        m_free(pElement->u.u_switch.branches,
                               wideCharacterString);
                break;
        case e_cdata:
        ...
        case e_keystyle:
                if (pElement->u.u_keystyle.class)
                        m_free(pElement->u.u_keystyle.class,
                               wideCharacterString);
                if (pElement->u.u_keystyle.ssi)
                        m_free(pElement->u.u_keystyle.ssi, wideCharacterString);
                if (pElement->u.u_keystyle.rlevel)
                        m_free(pElement->u.u_keystyle.rlevel,
                               wideCharacterString);
                if (pElement->u.u_keystyle.phrase)
                        m_free(pElement->u.u_keystyle.phrase,
                               wideCharacterString);
                if (pElement->u.u_keystyle.srch_wt)
                        m_free(pElement->u.u_keystyle.srch_wt,
                               wideCharacterString);
                if (pElement->u.u_keystyle.enter)
                        m_free(pElement->u.u_keystyle.enter,
                               wideCharacterString);
                if (pElement->u.u_keystyle.exit)
                        m_free(pElement->u.u_keystyle.exit,
                               wideCharacterString);
                if (pElement->u.u_keystyle.pointsz)
                        m_free(pElement->u.u_keystyle.pointsz,
                               wideCharacterString);
                if (pElement->u.u_keystyle.set_width)
                        m_free(pElement->u.u_keystyle.set_width,
                               wideCharacterString);
                if (pElement->u.u_keystyle.color)
                        m_free(pElement->u.u_keystyle.color,
                               wideCharacterString);
                if (pElement->u.u_keystyle.xlfd)
                        m_free(pElement->u.u_keystyle.xlfd,
                               wideCharacterString);
                if (pElement->u.u_keystyle.xlfdi)
                        m_free(pElement->u.u_keystyle.xlfdi,
                               wideCharacterString);
                if (pElement->u.u_keystyle.xlfdb)
                        m_free(pElement->u.u_keystyle.xlfdb,
                               wideCharacterString);
                if (pElement->u.u_keystyle.xlfdib)
                        m_free(pElement->u.u_keystyle.xlfdib,
                               wideCharacterString);
                if (pElement->u.u_keystyle.typenam)
                        m_free(pElement->u.u_keystyle.typenam,
                               wideCharacterString);
                if (pElement->u.u_keystyle.typenami)
                        m_free(pElement->u.u_keystyle.typenami,
                               wideCharacterString);
                if (pElement->u.u_keystyle.typenamb)
                        m_free(pElement->u.u_keystyle.typenamb,
                               wideCharacterString);
                if (pElement->u.u_keystyle.typenamib)
                        m_free(pElement->u.u_keystyle.typenamib,
                               wideCharacterString);
                if (pElement->u.u_keystyle.style)
                        m_free(pElement->u.u_keystyle.style,
                               wideCharacterString);
                if (pElement->u.u_keystyle.spacing)
                        m_free(pElement->u.u_keystyle.spacing,
                               wideCharacterString);
                if (pElement->u.u_keystyle.weight)
                        m_free(pElement->u.u_keystyle.weight,
                               wideCharacterString);
                if (pElement->u.u_keystyle.slant)
                        m_free(pElement->u.u_keystyle.slant,
                               wideCharacterString);
                if (pElement->u.u_keystyle.special)
                        m_free(pElement->u.u_keystyle.special,
                               wideCharacterString);
                break;
        case e_headstyle:
                if (pElement->u.u_headstyle.class)
                        m_free(pElement->u.u_headstyle.class,
                               wideCharacterString);
                if (pElement->u.u_headstyle.ssi)
                        m_free(pElement->u.u_headstyle.ssi,
                               wideCharacterString);
                if (pElement->u.u_headstyle.rlevel)
                        m_free(pElement->u.u_headstyle.rlevel,
                               wideCharacterString);
                if (pElement->u.u_headstyle.phrase)
                        m_free(pElement->u.u_headstyle.phrase,
                               wideCharacterString);
                if (pElement->u.u_headstyle.srch_wt)
                        m_free(pElement->u.u_headstyle.srch_wt,
                               wideCharacterString);
                if (pElement->u.u_headstyle.orient)
                        m_free(pElement->u.u_headstyle.orient,
                               wideCharacterString);
                if (pElement->u.u_headstyle.vorient)
                        m_free(pElement->u.u_headstyle.vorient,
                               wideCharacterString);
                if (pElement->u.u_headstyle.placement)
                        m_free(pElement->u.u_headstyle.placement,
                               wideCharacterString);
                if (pElement->u.u_headstyle.headw)
                        m_free(pElement->u.u_headstyle.headw,
                               wideCharacterString);
                if (pElement->u.u_headstyle.stack)
                        m_free(pElement->u.u_headstyle.stack,
                               wideCharacterString);
                if (pElement->u.u_headstyle.flow)
                        m_free(pElement->u.u_headstyle.flow,
                               wideCharacterString);
                if (pElement->u.u_headstyle.pointsz)
                        m_free(pElement->u.u_headstyle.pointsz,
                               wideCharacterString);
                if (pElement->u.u_headstyle.set_width)
                        m_free(pElement->u.u_headstyle.set_width,
                               wideCharacterString);
                if (pElement->u.u_headstyle.color)
                        m_free(pElement->u.u_headstyle.color,
                               wideCharacterString);
                if (pElement->u.u_headstyle.xlfd)
                        m_free(pElement->u.u_headstyle.xlfd,
                               wideCharacterString);
                if (pElement->u.u_headstyle.xlfdi)
                        m_free(pElement->u.u_headstyle.xlfdi,
                               wideCharacterString);
                if (pElement->u.u_headstyle.xlfdb)
                        m_free(pElement->u.u_headstyle.xlfdb,
                               wideCharacterString);
                if (pElement->u.u_headstyle.xlfdib)
                        m_free(pElement->u.u_headstyle.xlfdib,
                               wideCharacterString);
                if (pElement->u.u_headstyle.typenam)
                        m_free(pElement->u.u_headstyle.typenam,
                               wideCharacterString);
                if (pElement->u.u_headstyle.typenami)
                        m_free(pElement->u.u_headstyle.typenami,
                               wideCharacterString);
                if (pElement->u.u_headstyle.typenamb)
                        m_free(pElement->u.u_headstyle.typenamb,
                               wideCharacterString);
                if (pElement->u.u_headstyle.typenamib)
                        m_free(pElement->u.u_headstyle.typenamib,
                               wideCharacterString);
                if (pElement->u.u_headstyle.style)
                        m_free(pElement->u.u_headstyle.style,
                               wideCharacterString);
                if (pElement->u.u_headstyle.spacing)
                        m_free(pElement->u.u_headstyle.spacing,
                               wideCharacterString);
                if (pElement->u.u_headstyle.weight)
                        m_free(pElement->u.u_headstyle.weight,
                               wideCharacterString);
                if (pElement->u.u_headstyle.slant)
                        m_free(pElement->u.u_headstyle.slant,
                               wideCharacterString);
                if (pElement->u.u_headstyle.special)
                        m_free(pElement->u.u_headstyle.special,
                               wideCharacterString);
                if (pElement->u.u_headstyle.l_margin)
                        m_free(pElement->u.u_headstyle.l_margin,
                               wideCharacterString);
                if (pElement->u.u_headstyle.r_margin)
                        m_free(pElement->u.u_headstyle.r_margin,
                               wideCharacterString);
                if (pElement->u.u_headstyle.t_margin)
                        m_free(pElement->u.u_headstyle.t_margin,
                               wideCharacterString);
                if (pElement->u.u_headstyle.b_margin)
                        m_free(pElement->u.u_headstyle.b_margin,
                               wideCharacterString);
                if (pElement->u.u_headstyle.border)
                        m_free(pElement->u.u_headstyle.border,
                               wideCharacterString);
                if (pElement->u.u_headstyle.vjust)
                        m_free(pElement->u.u_headstyle.vjust,
                               wideCharacterString);
                if (pElement->u.u_headstyle.justify)
                        m_free(pElement->u.u_headstyle.justify,
                               wideCharacterString);
                break;
        case e_formstyle:
                if (pElement->u.u_formstyle.class)
                        m_free(pElement->u.u_formstyle.class,
                               wideCharacterString);
                if (pElement->u.u_formstyle.ssi)
                        m_free(pElement->u.u_formstyle.ssi,
                               wideCharacterString);
                if (pElement->u.u_formstyle.rlevel)
                        m_free(pElement->u.u_formstyle.rlevel,
                               wideCharacterString);
                if (pElement->u.u_formstyle.phrase)
                        m_free(pElement->u.u_formstyle.phrase,
                               wideCharacterString);
                if (pElement->u.u_formstyle.srch_wt)
                        m_free(pElement->u.u_formstyle.srch_wt,
                               wideCharacterString);
                if (pElement->u.u_formstyle.colw)
                        m_free(pElement->u.u_formstyle.colw,
                               wideCharacterString);
                if (pElement->u.u_formstyle.colj)
                        m_free(pElement->u.u_formstyle.colj,
                               wideCharacterString);
                if (pElement->u.u_formstyle.pointsz)
                        m_free(pElement->u.u_formstyle.pointsz,
                               wideCharacterString);
                if (pElement->u.u_formstyle.set_width)
                        m_free(pElement->u.u_formstyle.set_width,
                               wideCharacterString);
                if (pElement->u.u_formstyle.color)
                        m_free(pElement->u.u_formstyle.color,
                               wideCharacterString);
                if (pElement->u.u_formstyle.xlfd)
                        m_free(pElement->u.u_formstyle.xlfd,
                               wideCharacterString);
                if (pElement->u.u_formstyle.xlfdi)
                        m_free(pElement->u.u_formstyle.xlfdi,
                               wideCharacterString);
                if (pElement->u.u_formstyle.xlfdb)
                        m_free(pElement->u.u_formstyle.xlfdb,
                               wideCharacterString);
                if (pElement->u.u_formstyle.xlfdib)
                        m_free(pElement->u.u_formstyle.xlfdib,
                               wideCharacterString);
                if (pElement->u.u_formstyle.typenam)
                        m_free(pElement->u.u_formstyle.typenam,
                               wideCharacterString);
                if (pElement->u.u_formstyle.typenami)
                        m_free(pElement->u.u_formstyle.typenami,
                               wideCharacterString);
                if (pElement->u.u_formstyle.typenamb)
                        m_free(pElement->u.u_formstyle.typenamb,
                               wideCharacterString);
                if (pElement->u.u_formstyle.typenamib)
                        m_free(pElement->u.u_formstyle.typenamib,
                               wideCharacterString);
                if (pElement->u.u_formstyle.style)
                        m_free(pElement->u.u_formstyle.style,
                               wideCharacterString);
                if (pElement->u.u_formstyle.spacing)
                        m_free(pElement->u.u_formstyle.spacing,
                               wideCharacterString);
                if (pElement->u.u_formstyle.weight)
                        m_free(pElement->u.u_formstyle.weight,
                               wideCharacterString);
                if (pElement->u.u_formstyle.slant)
                        m_free(pElement->u.u_formstyle.slant,
                               wideCharacterString);
                if (pElement->u.u_formstyle.special)
                        m_free(pElement->u.u_formstyle.special,
                               wideCharacterString);
                if (pElement->u.u_formstyle.l_margin)
                        m_free(pElement->u.u_formstyle.l_margin,
                               wideCharacterString);
                if (pElement->u.u_formstyle.r_margin)
                        m_free(pElement->u.u_formstyle.r_margin,
                               wideCharacterString);
                if (pElement->u.u_formstyle.t_margin)
                        m_free(pElement->u.u_formstyle.t_margin,
                               wideCharacterString);
                if (pElement->u.u_formstyle.b_margin)
                        m_free(pElement->u.u_formstyle.b_margin,
                               wideCharacterString);
                if (pElement->u.u_formstyle.border)
                        m_free(pElement->u.u_formstyle.border,
                               wideCharacterString);
                if (pElement->u.u_formstyle.vjust)
                        m_free(pElement->u.u_formstyle.vjust,
                               wideCharacterString);
                break;
        case e_frmtstyle:
                if (pElement->u.u_frmtstyle.class)
                        m_free(pElement->u.u_frmtstyle.class,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.ssi)
                        m_free(pElement->u.u_frmtstyle.ssi,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.rlevel)
                        m_free(pElement->u.u_frmtstyle.rlevel,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.phrase)
                        m_free(pElement->u.u_frmtstyle.phrase,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.f_margin)
                        m_free(pElement->u.u_frmtstyle.f_margin,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.srch_wt)
                        m_free(pElement->u.u_frmtstyle.srch_wt,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.pointsz)
                        m_free(pElement->u.u_frmtstyle.pointsz,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.set_width)
                        m_free(pElement->u.u_frmtstyle.set_width,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.color)
                        m_free(pElement->u.u_frmtstyle.color,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.xlfd)
                        m_free(pElement->u.u_frmtstyle.xlfd,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.xlfdi)
                        m_free(pElement->u.u_frmtstyle.xlfdi,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.xlfdb)
                        m_free(pElement->u.u_frmtstyle.xlfdb,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.xlfdib)
                        m_free(pElement->u.u_frmtstyle.xlfdib,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.typenam)
                        m_free(pElement->u.u_frmtstyle.typenam,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.typenami)
                        m_free(pElement->u.u_frmtstyle.typenami,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.typenamb)
                        m_free(pElement->u.u_frmtstyle.typenamb,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.typenamib)
                        m_free(pElement->u.u_frmtstyle.typenamib,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.style)
                        m_free(pElement->u.u_frmtstyle.style,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.spacing)
                        m_free(pElement->u.u_frmtstyle.spacing,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.weight)
                        m_free(pElement->u.u_frmtstyle.weight,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.slant)
                        m_free(pElement->u.u_frmtstyle.slant,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.special)
                        m_free(pElement->u.u_frmtstyle.special,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.l_margin)
                        m_free(pElement->u.u_frmtstyle.l_margin,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.r_margin)
                        m_free(pElement->u.u_frmtstyle.r_margin,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.t_margin)
                        m_free(pElement->u.u_frmtstyle.t_margin,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.b_margin)
                        m_free(pElement->u.u_frmtstyle.b_margin,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.border)
                        m_free(pElement->u.u_frmtstyle.border,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.vjust)
                        m_free(pElement->u.u_frmtstyle.vjust,
                               wideCharacterString);
                if (pElement->u.u_frmtstyle.justify)
                        m_free(pElement->u.u_frmtstyle.justify,
                               wideCharacterString);
                break;
        case e_grphstyle:
                if (pElement->u.u_grphstyle.class)
                        m_free(pElement->u.u_grphstyle.class,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.ssi)
                        m_free(pElement->u.u_grphstyle.ssi,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.rlevel)
                        m_free(pElement->u.u_grphstyle.rlevel,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.phrase)
                        m_free(pElement->u.u_grphstyle.phrase,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.l_margin)
                        m_free(pElement->u.u_grphstyle.l_margin,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.r_margin)
                        m_free(pElement->u.u_grphstyle.r_margin,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.t_margin)
                        m_free(pElement->u.u_grphstyle.t_margin,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.b_margin)
                        m_free(pElement->u.u_grphstyle.b_margin,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.border)
                        m_free(pElement->u.u_grphstyle.border,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.vjust)
                        m_free(pElement->u.u_grphstyle.vjust,
                               wideCharacterString);
                if (pElement->u.u_grphstyle.justify)
                        m_free(pElement->u.u_grphstyle.justify,
                               wideCharacterString);
                break;
        default:
                fprintf(stderr, "internal error - unrecognized element type in "
                                "DeleteElement");
                if (m_errfile && (m_errfile != stderr))
                        fprintf(m_errfile, "internal error - unrecognized "
                                           "element type in DeleteElement");
                exit(xxx);
        }
...
void PushCurrentElement() {
        ...
        if (currentElementStackSize == 0) {
                ...
                currentElementStackSize += 100;
                currentElementStack = (ElementPtr *)mb_realloc(
                    (char *)currentElementStack,
...
#else
static void PushThisElement(pThis) ElementPtr pThis;
#endif
{
        thisElementStackTop++;
        if (thisElementStackSize == 0) {
                ...
                thisElementStackSize += 100;
                thisElementStack = (ElementPtr *)mb_realloc(
                    (char *)thisElementStack,
...
void PopCurrentElement() {
        ...
        if (currentElementStackTop < 0) {
                ...
                if (m_errfile && (m_errfile != stderr))
                        fprintf(m_errfile, "internal error - stack underflow "
                                           "in PopCurrentElement");
                exit(xxx);
        } else
                --currentElementStackTop;
}
...
#else
static void RemoveSuperfluousBlocks(ppBlockOrForm) ElementPtr *ppBlockOrForm;
#endif
{
...
#else
static void ReplaceFormWithBlock(ppBlockOrForm) ElementPtr ppBlockOrForm;
#endif
{
        ...
        while (pElement) {
                ...
                if (!(pFdata =
                          pElement->u.u_form.pFdata)) { /* illegal <form> */
                        ppBack = &(pElement->pNext);
                        pElement = pElement->pNext;
                        continue;
                }
                ...
                if ((pFstyle = pElement->u.u_form.pFstyle) &&
                    ...
                    !pFrowvec->pNext &&
                    (!pFstyle->u.u_fstyle.ncols ||
                     !w_strcmp(
                         pFstyle->u.u_fstyle.ncols,
                         wc_quote1))) { /* found a <form> containing only one
                                           element */
                        if (((pSubElement = pFdata->u.u_fdata.pBlockOrForm)
                                 ->type == e_form)) {
                                if (!pSubElement->u.u_form.level &&
                                    ...
                                    !pSubElement->u.u_form.ssi) {
                                        ...
                                        *ppBack = pSubElement;
                                        ...
                                        pSubElement->u.u_form.id =
                                            pElement->u.u_form.id;
                                        pSubElement->u.u_form.level =
                                            pElement->u.u_form.level;
                                        pSubElement->u.u_form.version =
                                            pElement->u.u_form.version;
                                        pSubElement->u.u_form.class =
                                            pElement->u.u_form.class;
                                        pSubElement->u.u_form.language =
                                            pElement->u.u_form.language;
                                        pSubElement->u.u_form.charset =
                                            pElement->u.u_form.charset;
                                        pSubElement->u.u_form.length =
                                            pElement->u.u_form.length;
                                        pSubElement->u.u_form.app =
                                            pElement->u.u_form.app;
                                        pSubElement->u.u_form.ssi =
                                            pElement->u.u_form.ssi;
                                ...
                                if (!pSubElement->u.u_block.level &&
                                    ...
                                    !pSubElement->u.u_block.ssi) {
                                        ...
                                        *ppBack = pSubElement;
                                        pSubElement->pNext = pElement->pNext;
                                        pSubElement->u.u_block.id =
                                            pElement->u.u_form.id;
                                        pSubElement->u.u_block.level =
                                            pElement->u.u_form.level;
                                        pSubElement->u.u_block.version =
                                            pElement->u.u_form.version;
                                        pSubElement->u.u_block.class =
...
#else
static void OptimizeSDL(ppBlockOrForm) ElementPtr ppBlockOrForm;
#endif
{
 ...
 */
#if defined(M_PROTO)
static void AddToLOIDS(char *pElementName, M_WCHAR *pwcId, M_WCHAR *pwcSSI,
                       M_WCHAR *pwcLevel, long int virpageOffset)
#else
static void AddToLOIDS(pElementName, pwcId, pwcSSI, pwcLevel,
                       virpageOffset) char *pElementName;
M_WCHAR *pwcId;
M_WCHAR *pwcSSI;
M_WCHAR *pwcLevel;
...
#else
int Compress(fileName, zFileName, doCompress) char *fileName;
char *zFileName;
LOGICAL doCompress;
#endif
{
...
static void DecompressSDL() {
        ...
        if (length < 4) {
                fprintf(stderr, "%s: premature end of %s\n", progName,
                        inFileName);
                if (m_errfile && (m_errfile != stderr)) {
                        fprintf(m_errfile, "%s: premature end of %s\n",
                                progName, inFileName);
                }
                exit(xxx);
        }
        ...
        {
                length += FreadFile(inFile, inFileName, (char *)(buffer + 4),
                                    BUFSIZ - 4, xxx);
                do {
                        WriteFile(outFd, tempFileName, (char *)buffer, length,
                                  xxx);
                        length = FreadFile(inFile, inFileName, (char *)buffer,
                                           BUFSIZ, xxx);
                } while (length > 0);
        ...
        do {
                ...
                while (length = FreadFile(inFile, inFileName, (char *)buffer,
                                          MIN(delta, BUFSIZ), xxx)) {
                        WriteFile(zFd, compZFileName, (char *)buffer, length,
                                  xxx);
                        delta -= length;
                }
                if (delta != 0) {
                ...
                while (length = ReadFile(zFd, compFileName, (char *)buffer,
                                         MIN(delta, BUFSIZ), xxx)) {
                        WriteFile(outFd, tempFileName, (char *)buffer, length,
                                  xxx);
                        delta -= length;
                ...
                while ((delta = FreadFile(inFile, inFileName, (char *)buffer, 1,
                                          xxx)) &&
                       (*buffer == 0xff))
                        ;
                if (delta) {
                        if (*buffer ==
                            '<') /* we're at the end, "</SDLDOC>\n" */
                        {
                                delta +=
                                    FreadFile(inFile, inFileName,
                                              (char *)(buffer + 1), 9, xxx);
                                if (delta != 10)
                                        delta = 1; /* force a premature end of
                                                      document */
                                else {
                                        WriteFile(outFd, tempFileName,
                                                  (char *)buffer, delta, xxx);
                                        delta = 0;
                                }
                        } else
                                delta +=
...
#else
static void MarkUsedStyle(level, class, ssi) M_WCHAR *level;
M_WCHAR *class;
M_WCHAR *ssi;
#endif
{
...
#else
static ElementPtr LookupInSnb(pSnb, rid) ElementPtr pSnb;
M_WCHAR *rid;
#endif
{
        ElementPtr pThis;
...
#else
static void ClearTree(pRoot) ElementPtr pRoot;
#endif
{
        ElementPtr pThis;
        static char errMess[] =
...
#else
static void MarkUsedTOSS(pRoot) ElementPtr pRoot;
#endif
{
        ...
        while (pThis) {
                ...
                case e_virpage:
                        if (!pThis->tossChecked) {
                                pThis->inheritedLevel =
                                    pThis->u.u_virpage.level;
                                pThis->inheritedClass =
                                    MakeWideCharString("TEXT");
                ...
                case e_snb:
                        ...
                        if (pThis->u.u_snb
                                .pSystemNotations) { /* don't do anything now;
                                                        wait for <snref> */
                        }
                        if (pThis->pNext) {
                                pThis = pThis->pNext;
                ...
                case e_block:
                        if (!pThis->tossChecked) {
                                if (!(pThis->inheritedLevel =
                                          pThis->u.u_block.level))
                                        pThis->inheritedLevel =
                                            pParent->inheritedLevel;
                                if (!(pThis->inheritedClass =
                                          pThis->u.u_block.class))
                                        pThis->inheritedClass =
                                            pParent->inheritedClass;
                ...
                case e_head:
                        if (!pThis->tossChecked) {
                                pThis->inheritedLevel = pParent->inheritedLevel;
                                if (!(pThis->inheritedClass =
                                          pThis->u.u_head.class))
                                        pThis->inheritedClass =
                                            MakeWideCharString("HEAD");
                                MarkUsedStyle(pThis->inheritedLevel,
                                              pThis->inheritedClass,
                                              pThis->u.u_head.ssi);
                                pThis->tossChecked = TRUE;
                ...
                case e_subhead:
                        if (!pThis->tossChecked) {
                                pThis->inheritedLevel = pParent->inheritedLevel;
                                if (!(pThis->inheritedClass =
                                          pThis->u.u_subhead.class))
                                        pThis->inheritedClass =
                                            pParent->inheritedClass;
                                if (pThis->u.u_subhead.class ||
                ...
                case e_refitem:
                        if (!pThis->tossChecked) {
                                pThis->inheritedLevel = pParent->inheritedLevel;
                                pThis->inheritedClass =
                                    pThis->u.u_refitem.class;
                                MarkUsedStyle(pThis->inheritedLevel,
                                              pThis->inheritedClass,
                                              pThis->u.u_refitem.ssi);
                                if ((pItem = LookupInSnb(
                                         pSnb, pThis->u.u_refitem.rid)) &&
                                    (pItem->type == e_text)) {
                                        pThis->u.u_refitem.pReferencedItem =
                                            pItem;
                                        ClearTree(pItem);
                                }
                                pThis->tossChecked = TRUE;
...
#else
void EmitSDL(pRoot) ElementPtr pRoot;
#endif
{
        ...
        if (pRoot) {
                ...
                if (pRoot->type == e_virpage) {
                        ...
                        if (compressFlag) {
                                ...
                                saveOutFile = outFile;
                                outFile =
                                    FopenFile(compFileName, O_WRONLY, xxx);
                        }
                }
                pThis = pRoot;
        } else {
        ...
        while (pThis) {
                ...
                case e_virpage:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("VIRPAGE", pThis->u.u_virpage.id,
                                           pThis->u.u_virpage.ssi,
                                           pThis->u.u_virpage.level,
                                           virpageOffset);
                                RequiredAttribute("LEVEL",
                                                  pThis->u.u_virpage.level);
                                ImpliedAttribute("VERSION",
                                                 pThis->u.u_virpage.version);
                                ImpliedAttribute("LANGUAGE",
                                                 pThis->u.u_virpage.language);
                                ImpliedAttribute("CHARSET",
                                                 pThis->u.u_virpage.charset);
                                RequiredAttribute("DOC-ID",
                                                  pThis->u.u_virpage.doc_id);
                ...
                case e_block:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("BLOCK", pThis->u.u_block.id,
                                           pThis->u.u_block.ssi,
                                           pThis->u.u_block.level,
                                           virpageOffset);
                                ImpliedAttribute("LEVEL",
                                                 pThis->u.u_block.level);
                                ImpliedAttribute("VERSION",
                                                 pThis->u.u_block.version);
                                ImpliedAttribute("CLASS",
                                                 pThis->u.u_block.class);
                                ImpliedAttribute("LANGUAGE",
                                                 pThis->u.u_block.language);
                                ImpliedAttribute("CHARSET",
                                                 pThis->u.u_block.charset);
                                ImpliedAttribute("LENGTH",
                                                 pThis->u.u_block.length);
                                ImpliedAttribute("APP", pThis->u.u_block.app);
                                ImpliedAttribute("TIMING",
                                                 pThis->u.u_block.timing);
                ...
                case e_form:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("FORM", pThis->u.u_form.id,
                                           pThis->u.u_form.ssi,
                                           pThis->u.u_form.level,
                                           virpageOffset);
                                ImpliedAttribute("LEVEL",
                                                 pThis->u.u_form.level);
                                ImpliedAttribute("VERSION",
                                                 pThis->u.u_form.version);
                                ImpliedAttribute("CLASS",
                                                 pThis->u.u_form.class);
                                ImpliedAttribute("LANGUAGE",
                                                 pThis->u.u_form.language);
                                ImpliedAttribute("CHARSET",
                                                 pThis->u.u_form.charset);
                                ImpliedAttribute("LENGTH",
                                                 pThis->u.u_form.length);
                ...
                case e_fstyle:
                        if (!pThis->beenEmitted) {
                                ...
                                ImpliedAttribute("NCOLS",
                                                 pThis->u.u_fstyle.ncols);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->u.u_fstyle.pFrowvec) {
                ...
                case e_frowvec:
                        if (!pThis->beenEmitted) {
                                ...
                                RequiredAttribute("CELLS",
                                                  pThis->u.u_frowvec.cells);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->pNext) {
                ...
                case e_p:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("P", pThis->u.u_p.id,
                                           pThis->u.u_p.ssi, NULL,
                                           virpageOffset);
                                ImpliedAttribute("VERSION",
                                                 pThis->u.u_p.version);
                ...
                case e_cp:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("CP", pThis->u.u_cp.id,
                                           pThis->u.u_cp.ssi, NULL,
                                           virpageOffset);
                                ImpliedAttribute("VERSION",
                                                 pThis->u.u_cp.version);
                ...
                case e_head:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("HEAD", pThis->u.u_head.id,
                                           pThis->u.u_head.ssi, NULL,
                                           virpageOffset);
                                ImpliedAttribute("VERSION",
                                                 pThis->u.u_head.version);
                                ImpliedAttribute("CLASS",
                                                 pThis->u.u_head.class);
                                ImpliedAttribute("LANGUAGE",
                                                 pThis->u.u_head.language);
                                ImpliedAttribute("CHARSET",
                                                 pThis->u.u_head.charset);
                                ImpliedAttribute("TYPE", pThis->u.u_head.type);
                                ImpliedAttribute("SSI", pThis->u.u_head.ssi);
                                ImpliedAttribute("ABBREV",
                                                 pThis->u.u_head.abbrev);
                                PutString(">");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->u.u_head.pAtomicOrPCDATA) {
                ...
                case e_subhead:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("SUBHEAD", pThis->u.u_subhead.id,
                                           pThis->u.u_subhead.ssi, NULL,
                                           virpageOffset);
                                ImpliedAttribute("VERSION",
                                                 pThis->u.u_subhead.version);
                                ImpliedAttribute("CLASS",
                                                 pThis->u.u_subhead.class);
                                ImpliedAttribute("LANGUAGE",
                                                 pThis->u.u_subhead.language);
                                ImpliedAttribute("CHARSET",
                                                 pThis->u.u_subhead.charset);
                                ImpliedAttribute("TYPE",
                                                 pThis->u.u_subhead.type);
                                ImpliedAttribute("SSI", pThis->u.u_subhead.ssi);
                                ImpliedAttribute("ABBREV",
                                                 pThis->u.u_subhead.abbrev);
                                PutString(">");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->u.u_subhead.pAtomicOrPCDATA) {
                ...
                case e_anchor:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("ANCHOR", pThis->u.u_anchor.id, NULL,
                                           NULL, virpageOffset);
                                PutString(">");
                                pThis->beenEmitted = TRUE;
                        }
                ...
                case e_link:
                        if (!pThis->beenEmitted) {
                                ...
                                ImpliedAttribute("BUTTON",
                                                 pThis->u.u_link.button);
                                ImpliedAttribute("LINKINFO",
                                                 pThis->u.u_link.linkinfo);
                                ImpliedAttribute("DESCRIPT",
                                                 pThis->u.u_link.descript);
                                ImpliedAttribute("WINDOW",
                                                 pThis->u.u_link.window);
                                ImpliedAttribute("TRAVERSAL",
                                                 pThis->u.u_link.traversal);
                                PutString(">");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->u.u_link.pAtomicOrPCDATA) {
                ...
                case e_snref:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("SNREF", pThis->u.u_snref.id, NULL,
                                           NULL, virpageOffset);
                                PutString(">");
                                pThis->beenEmitted = TRUE;
                        }
                ...
                case e_refitem:
                        if (!pThis->beenEmitted) {
                                ...
                                RequiredAttribute("RID",
                                                  pThis->u.u_refitem.rid);
                                RequiredAttribute("CLASS",
                                                  pThis->u.u_refitem.class);
                                ImpliedAttribute("BUTTON",
                                                 pThis->u.u_refitem.button);
                ...
                case e_cond:
                        if (!pThis->beenEmitted) {
                                ...
                                ImpliedAttribute("INTERP",
                                                 pThis->u.u_cond.interp);
                                PutString(">");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->u.u_cond.pCDATA) {
                ...
                case e_graphic:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("GRAPHIC", pThis->u.u_graphic.id,
                                           NULL, NULL, virpageOffset);
                                ImpliedAttribute("FORMAT",
                                                 pThis->u.u_graphic.format);
                                ImpliedAttribute("METHOD",
                                                 pThis->u.u_graphic.method);
                                RequiredAttribute("XID",
                                                  pThis->u.u_graphic.xid);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->pNext) {
                ...
                case e_text:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("TEXT", pThis->u.u_text.id, NULL,
                                           NULL, virpageOffset);
                                ImpliedAttribute("LANGUAGE",
                                                 pThis->u.u_text.language);
                                ImpliedAttribute("CHARSET",
                                                 pThis->u.u_text.charset);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->u.u_text.pCpOrP) {
                ...
                case e_audio:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("AUDIO", pThis->u.u_audio.id, NULL,
                                           NULL, virpageOffset);
                                ImpliedAttribute("FORMAT",
                                                 pThis->u.u_audio.format);
                                ImpliedAttribute("METHOD",
                                                 pThis->u.u_audio.method);
                ...
                case e_video:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("VIDEO", pThis->u.u_video.id, NULL,
                                           NULL, virpageOffset);
                                ImpliedAttribute("FORMAT",
                                                 pThis->u.u_video.format);
                                ImpliedAttribute("METHOD",
                                                 pThis->u.u_video.method);
                ...
                case e_animate:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("ANIMATE", pThis->u.u_animate.id,
                                           NULL, NULL, virpageOffset);
                                ImpliedAttribute("FORMAT",
                                                 pThis->u.u_animate.format);
                                ImpliedAttribute("METHOD",
                                                 pThis->u.u_animate.method);
                                RequiredAttribute("XID",
                                                  pThis->u.u_animate.xid);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->pNext) {
                ...
                case e_script:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("SCRIPT", pThis->u.u_script.id, NULL,
                                           NULL, virpageOffset);
                                ImpliedAttribute("INTERP",
                                                 pThis->u.u_script.interp);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->u.u_script.pCDATA) {
                ...
                case e_crossdoc:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("CROSSDOC", pThis->u.u_crossdoc.id,
                                           NULL, NULL, virpageOffset);
                                RequiredAttribute("XID",
                                                  pThis->u.u_crossdoc.xid);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->pNext) {
                ...
                case e_man_page:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("MAN-PAGE", pThis->u.u_man_page.id,
                                           NULL, NULL, virpageOffset);
                                RequiredAttribute("XID",
                                                  pThis->u.u_man_page.xid);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->pNext) {
                ...
                case e_textfile:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("TEXTFILE", pThis->u.u_textfile.id,
                                           NULL, NULL, virpageOffset);
                                RequiredAttribute("XID",
                                                  pThis->u.u_textfile.xid);
                                ImpliedAttribute("OFFSET",
                                                 pThis->u.u_textfile.offset);
                                ImpliedAttribute("FORMAT",
                                                 pThis->u.u_textfile.format);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->pNext) {
                ...
                case e_sys_cmd:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("SYS-CMD", pThis->u.u_sys_cmd.id,
                                           NULL, NULL, virpageOffset);
                                RequiredAttribute("COMMAND",
                                                  pThis->u.u_sys_cmd.command);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->pNext) {
                ...
                case e_callback:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("CALLBACK", pThis->u.u_callback.id,
                                           NULL, NULL, virpageOffset);
                                ImpliedAttribute("DATA",
                                                 pThis->u.u_callback.data);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->pNext) {
                ...
                case e_switch:
                        if (!pThis->beenEmitted) {
                                ...
                                AddToLOIDS("SWITCH", pThis->u.u_switch.id, NULL,
                                           NULL, virpageOffset);
                                ImpliedAttribute("INTERP",
                                                 pThis->u.u_switch.interp);
                                RequiredAttribute("BRANCHES",
                                                  pThis->u.u_switch.branches);
                                PutString(">\n");
                                pThis->beenEmitted = TRUE;
                        }
                        if (pThis->u.u_switch.pCDATA) {
                ...
                case e_keystyle:
                        ...
                        ImpliedAttribute("SRCH-WT",
                                         pThis->u.u_keystyle.srch_wt);
                        ImpliedAttribute("ENTER", pThis->u.u_keystyle.enter);
                        ImpliedAttribute("EXIT", pThis->u.u_keystyle.exit);
                        ImpliedAttribute("POINTSZ",
                                         pThis->u.u_keystyle.pointsz);
                        ImpliedAttribute("SET-WIDTH",
                                         pThis->u.u_keystyle.set_width);
                        ...
                        ImpliedAttribute("TYPENAM",
                                         pThis->u.u_keystyle.typenam);
                        ImpliedAttribute("TYPENAMI",
                                         pThis->u.u_keystyle.typenami);
                        ImpliedAttribute("TYPENAMB",
                                         pThis->u.u_keystyle.typenamb);
                        ImpliedAttribute("TYPENAMIB",
                                         pThis->u.u_keystyle.typenamib);
                        ImpliedAttribute("STYLE", pThis->u.u_keystyle.style);
                        ImpliedAttribute("SPACING",
                                         pThis->u.u_keystyle.spacing);
                        ImpliedAttribute("WEIGHT", pThis->u.u_keystyle.weight);
                        ImpliedAttribute("SLANT", pThis->u.u_keystyle.slant);
                        ImpliedAttribute("SPECIAL",
                                         pThis->u.u_keystyle.special);
                        PutString(">\n");
                        pThat = pThis;
                        pThis = pThat->pNext;
                        pThat->pNext = NULL;
                ...
                case e_headstyle:
                        ...
                        ImpliedAttribute("SRCH-WT",
                                         pThis->u.u_headstyle.srch_wt);
                        ImpliedAttribute("ORIENT", pThis->u.u_headstyle.orient);
                        ImpliedAttribute("VORIENT",
                                         pThis->u.u_headstyle.vorient);
                        ImpliedAttribute("PLACEMENT",
                                         pThis->u.u_headstyle.placement);
                        ...
                        ImpliedAttribute("POINTSZ",
                                         pThis->u.u_headstyle.pointsz);
                        ImpliedAttribute("SET-WIDTH",
                                         pThis->u.u_headstyle.set_width);
                        ...
                        ImpliedAttribute("TYPENAM",
                                         pThis->u.u_headstyle.typenam);
                        ImpliedAttribute("TYPENAMI",
                                         pThis->u.u_headstyle.typenami);
                        ImpliedAttribute("TYPENAMB",
                                         pThis->u.u_headstyle.typenamb);
                        ImpliedAttribute("TYPENAMIB",
                                         pThis->u.u_headstyle.typenamib);
                        ImpliedAttribute("STYLE", pThis->u.u_headstyle.style);
                        ImpliedAttribute("SPACING",
                                         pThis->u.u_headstyle.spacing);
                        ImpliedAttribute("WEIGHT", pThis->u.u_headstyle.weight);
                        ImpliedAttribute("SLANT", pThis->u.u_headstyle.slant);
                        ImpliedAttribute("SPECIAL",
                                         pThis->u.u_headstyle.special);
                        ImpliedAttribute("L-MARGIN",
                                         pThis->u.u_headstyle.l_margin);
                        ImpliedAttribute("R-MARGIN",
                                         pThis->u.u_headstyle.r_margin);
                        ImpliedAttribute("T-MARGIN",
                                         pThis->u.u_headstyle.t_margin);
                        ImpliedAttribute("B-MARGIN",
                                         pThis->u.u_headstyle.b_margin);
                        ImpliedAttribute("BORDER", pThis->u.u_headstyle.border);
                        ImpliedAttribute("VJUST", pThis->u.u_headstyle.vjust);
                        ImpliedAttribute("JUSTIFY",
                                         pThis->u.u_headstyle.justify);
                        PutString(">\n");
                        pThat = pThis;
                        pThis = pThat->pNext;
                        pThat->pNext = NULL;
                ...
                case e_formstyle:
                        ...
                        ImpliedAttribute("TYPENAM",
                                         pThis->u.u_formstyle.typenam);
                        ImpliedAttribute("TYPENAMI",
                                         pThis->u.u_formstyle.typenami);
                        ImpliedAttribute("TYPENAMB",
                                         pThis->u.u_formstyle.typenamb);
                        ImpliedAttribute("TYPENAMIB",
                                         pThis->u.u_formstyle.typenamib);
                        ImpliedAttribute("STYLE", pThis->u.u_formstyle.style);
                        ImpliedAttribute("SPACING",
                                         pThis->u.u_formstyle.spacing);
                        ImpliedAttribute("WEIGHT", pThis->u.u_formstyle.weight);
                        ImpliedAttribute("SLANT", pThis->u.u_formstyle.slant);
                        ImpliedAttribute("SPECIAL",
                                         pThis->u.u_formstyle.special);
                        ImpliedAttribute("L-MARGIN",
                                         pThis->u.u_formstyle.l_margin);
                        ImpliedAttribute("R-MARGIN",
                                         pThis->u.u_formstyle.r_margin);
                        ImpliedAttribute("T-MARGIN",
                                         pThis->u.u_formstyle.t_margin);
                        ImpliedAttribute("B-MARGIN",
                                         pThis->u.u_formstyle.b_margin);
                ...
                case e_frmtstyle:
                        ...
                        ImpliedAttribute("F-MARGIN",
                                         pThis->u.u_frmtstyle.f_margin);
                        ImpliedAttribute("SRCH-WT",
                                         pThis->u.u_frmtstyle.srch_wt);
                        ImpliedAttribute("POINTSZ",
                                         pThis->u.u_frmtstyle.pointsz);
                        ImpliedAttribute("SET-WIDTH",
                                         pThis->u.u_frmtstyle.set_width);
                        ...
                        ImpliedAttribute("TYPENAM",
                                         pThis->u.u_frmtstyle.typenam);
                        ImpliedAttribute("TYPENAMI",
                                         pThis->u.u_frmtstyle.typenami);
                        ImpliedAttribute("TYPENAMB",
                                         pThis->u.u_frmtstyle.typenamb);
                        ImpliedAttribute("TYPENAMIB",
                                         pThis->u.u_frmtstyle.typenamib);
                        ImpliedAttribute("STYLE", pThis->u.u_frmtstyle.style);
                        ImpliedAttribute("SPACING",
                                         pThis->u.u_frmtstyle.spacing);
                        ImpliedAttribute("WEIGHT", pThis->u.u_frmtstyle.weight);
                        ImpliedAttribute("SLANT", pThis->u.u_frmtstyle.slant);
                        ImpliedAttribute("SPECIAL",
                                         pThis->u.u_frmtstyle.special);
                        ImpliedAttribute("L-MARGIN",
                                         pThis->u.u_frmtstyle.l_margin);
                        ImpliedAttribute("R-MARGIN",
                                         pThis->u.u_frmtstyle.r_margin);
                        ImpliedAttribute("T-MARGIN",
                                         pThis->u.u_frmtstyle.t_margin);
                        ImpliedAttribute("B-MARGIN",
                                         pThis->u.u_frmtstyle.b_margin);
                        ImpliedAttribute("BORDER", pThis->u.u_frmtstyle.border);
                        ImpliedAttribute("VJUST", pThis->u.u_frmtstyle.vjust);
                        ImpliedAttribute("JUSTIFY",
                                         pThis->u.u_frmtstyle.justify);
                        PutString(">\n");
                        pThat = pThis;
                        pThis = pThat->pNext;
                        pThat->pNext = NULL;
                ...
                case e_grphstyle:
                        ...
                        ImpliedAttribute("L-MARGIN",
                                         pThis->u.u_grphstyle.l_margin);
                        ImpliedAttribute("R-MARGIN",
                                         pThis->u.u_grphstyle.r_margin);
                        ImpliedAttribute("T-MARGIN",
                                         pThis->u.u_grphstyle.t_margin);
                        ImpliedAttribute("B-MARGIN",
                                         pThis->u.u_grphstyle.b_margin);
                        ImpliedAttribute("BORDER", pThis->u.u_grphstyle.border);
                        ImpliedAttribute("VJUST", pThis->u.u_grphstyle.vjust);
                        ImpliedAttribute("JUSTIFY",
                                         pThis->u.u_grphstyle.justify);
                        PutString(">\n");
                        pThat = pThis;
                        pThis = pThat->pNext;
                        pThat->pNext = NULL;
        ...
        if (doCompression) {
                ...
                while (length =
                           ReadFile(zFd, compZFileName, buffer, BUFSIZ, xxx))
...
#else
void OptimizeAndEmit(pVirpage) ElementPtr pVirpage;
#endif
{
        if (optimizeFlag)
...
static void Help() {
        printf(
            "The available options to %s are 'c', 'd', 'h', 'o' and 't'.\n\n",
            progName);
        printf("Their meanings are:\n");
        printf(
            "    c: compress an SDL document (on a per virtual page basis)\n");
        printf("    d: decompress an SDL document\n");
        printf("    h: print this message and exit\n");
        printf("    o: perform peephole optimizations on this SDL document\n");
        printf("    t: eliminate any TOSS elements not used in this SDL "
               "document\n\n");
        printf("You may specify either one filename or two.  If one file is\n");
        printf(
            "specified, it will be used as both the input and output file.\n");
        printf("If the input file has no .sdl extension, one will be added.  "
               "If\n");
        printf("an explicit output filename is specified, it will be used "
               "without\n");
        printf("modification.\n\n");
        exit(0);
}
...
static void Usage() {
        fprintf(stderr, "Usage: %s [-cdhot] <inFileName> [<outFileName>]\n",
                progName);
        if (m_errfile && (m_errfile != stderr)) {
                fprintf(m_errfile,
...
#else
int OpenFile(name, type, code) char *name;
int type;
int code;
#endif
{
        ...
        if ((fd = open(name, oflag, mode)) == -1) {
                fprintf(stderr, "%s: error opening \"%s\" for %s\n", progName,
                        name, (type == O_RDONLY) ? "reading" : "writing");
                perror(name);
                if (m_errfile && (m_errfile != stderr)) {
                        fprintf(m_errfile, "%s: error opening \"%s\" for %s\n",
                                progName, name,
...
#else
int ReadFile(fd, name, buffer, amount, code) int fd;
char *name;
char *buffer;
int amount;
int code;
...
#else
int WriteFile(fd, name, buffer, amount, code) int fd;
char *name;
char *buffer;
int amount;
int code;
...
#else
void AssertFileIsReadable(name, code) char *name;
int code;
#endif
{
...
#else
int FileSize(name, code) char *name;
int code;
#endif
{
        struct stat buf;
        ...
        if (stat(name, &buf) == -1) {
                ...
                perror(name);
                if (m_errfile && (m_errfile != stderr)) {
                        fprintf(m_errfile, "%s: error getting size of \"%s\"\n",
                                progName, name);
                        fprintf(m_errfile, "%s: %s", name, strerror(errno));
                }
                exit(code);
...
#else
void CloseFile(fd, name, code) int fd;
char *name;
int code;
#endif
{
...
#else
FILE *FopenFile(name, type, code) char *name;
int type;
int code;
#endif
{
        ...
        if (type == O_RDONLY)
                ...
                fprintf(stderr, "%s: bad type (%d) in opening \"%s\"", progName,
                        type, name);
                if (m_errfile && (m_errfile != stderr)) {
                        fprintf(m_errfile,
                                "%s: bad type (%d) in opening \"%s\"", progName,
                                type, name);
                }
                exit(code);
        }
        ...
        if ((file = fopen(name, mode)) == NULL) {
                fprintf(
                    stderr, "%s: error opening \"%s\" for %s\n", progName, name,
                    (*mode == 'r') ? "reading"
...
#else
int GetALine(file, name, line, max, code) FILE *file;
char *name;
char *line;
int max;
int code;
...
#else
int FreadFile(file, name, buffer, amount, code) FILE *file;
char *name;
char *buffer;
int amount;
int code;
...
#else
void FwriteFile(file, name, buffer, amount, code) FILE *file;
char *name;
char *buffer;
size_t amount;
int code;
...
#else
void FcloseFile(file, name, code) FILE *file;
char *name;
int code;
#endif
{
...
#else
int StringToUpper(string) char *string;
#endif
{
        char *pc, c;
...
void BuildIndex() {
        ...
        sprintf(buffer + strlen(buffer), "sort -f %s > %s", idxFileName,
                sortedIdxFileName);
        if (status = system(buffer)) {
                if (status == -1) {
                        fprintf(stderr, "%s: error forking to execute \"%s\"\n",
                                progName, buffer);
                        perror(buffer);
                        if (m_errfile && (m_errfile != stderr)) {
                                fprintf(m_errfile,
        ...
        remnant = 0;
        pFrom = buffer;
        while (size || ((size = ReadFile(sortedIdxFd, sortedIdxFileName,
                                         buffer + remnant, BUFSIZ - remnant,
                                         xxx)) > 0)) {
                /* add in remnant in case we just got a new buffer load */
                size += remnant;
                remnant = 0;

                /* First split out the "sort", "text" and "rid" fields.  They
                   are seperated by an ASCII Record Seperator (036) character */
                pTo = sort;
                pRestart = pFrom;
                found = 0;
                while (--size >= 0) {
                ...
                *(pTo - 1) = '\0';
                ...
                /* if buffer is consumed, reset ourselves for the next buffer
                 * load */
                if (size <= 0) {
                        size = 0;
                        remnant = 0;
                        ...
                        if (strlen(locs) != 0) {
                                if (strcmp(lastsort, lasttext) != 0) {
                                        length =
                                            fprintf(sdlIdxFile,
                                                    "%s%s\" SORT=\"%s\">%s%s",
                                                    openEntry, locs, lastsort,
                                                    lasttext, closeEntry);
                                } else {
                                        length =
                                            fprintf(sdlIdxFile, "%s%s\">%s%s",
                                                    openEntry, locs, lasttext,
                                                    closeEntry);
                                }
                                if (length < 0) {
                                        fprintf(stderr,
                                                "%s: error writing to \"%s\"\n",
                                                progName, sortedIdxFileName);
        ...
        if (strlen(locs) != 0) {
                if (strcmp(lastsort, lasttext) != 0) {
                        length = fprintf(sdlIdxFile, "%s%s\" SORT=\"%s\">%s%s",
                                         openEntry, locs, lastsort, lasttext,
                                         closeEntry);
                } else {
                        length = fprintf(sdlIdxFile, "%s%s\">%s%s", openEntry,
                                         locs, lasttext, closeEntry);
                }
                if (length < 0) {
                        fprintf(stderr, "%s: error writing to \"%s\"\n",
                                progName, sortedIdxFileName);
                        ...
                        if (m_errfile && (m_errfile != stderr)) {
                                fprintf(m_errfile,
                                        "%s: error writing to \"%s\"\n",
                                        progName, sortedIdxFileName);
                                fprintf(m_errfile, "%s: %s", sortedIdxFileName,
                                        strerror(errno));
                        }
                        exit(1);
                }
        ...
        CloseFile(sortedIdxFd, sortedIdxFileName, xxx);
        FwriteFile(sdlIdxFile, sortedIdxFileName, closeIndex,
                   sizeof(closeIndex) - 1, xxx);
...
static void HandleSNB() {
        ...
        if (FileExists(snbFileName)) {
                ...
                while ((length = GetALine(snbFile, snbFileName, line, LINEMAX,
                                          xxx)) > 0) {
                        if (isdigit(*line)) {
                                if (newOffset != 0)
                                        WriteFile(outFd, tempFileName, closeSNB,
                                                  sizeof(closeSNB) - 1, xxx);
                                newOffset = atoi(line);
                                delta = newOffset - oldOffset;
                                while (delta) {
                                        length = ReadFile(
                                            inFd, realInFileName, buffer,
                                            MIN(BUFSIZ, delta), xxx);
                                        WriteFile(outFd, tempFileName, buffer,
                                                  length, xxx);
                                        delta -= length;
                                }
                                WriteFile(outFd, tempFileName, openSNB,
                                          sizeof(openSNB) - 1, xxx);
                                oldOffset = newOffset;
                        } else
                                WriteFile(outFd, tempFileName, line, length,
                                          xxx);
                }
                if (newOffset != 0)
                        WriteFile(outFd, tempFileName, closeSNB,
        ...
        while (pThis) {
                ...
                case e_keystyle:
                        ...
                        if (pThis->u.u_keystyle.class)
                                size +=
                                    5 + 4 + w_strlen(pThis->u.u_keystyle.class);
                        if (pThis->u.u_keystyle.ssi)
                                size +=
                                    3 + 4 + w_strlen(pThis->u.u_keystyle.ssi);
                        if (pThis->u.u_keystyle.rlevel)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_keystyle.rlevel);
                        if (pThis->u.u_keystyle.phrase)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_keystyle.phrase);
                        if (pThis->u.u_keystyle.srch_wt)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_keystyle.srch_wt);
                        if (pThis->u.u_keystyle.enter)
                                size +=
                                    5 + 4 + w_strlen(pThis->u.u_keystyle.enter);
                        if (pThis->u.u_keystyle.exit)
                                size +=
                                    4 + 4 + w_strlen(pThis->u.u_keystyle.exit);
                        if (pThis->u.u_keystyle.pointsz)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_keystyle.pointsz);
                        if (pThis->u.u_keystyle.set_width)
                                size += 9 + 4 +
                                        w_strlen(pThis->u.u_keystyle.set_width);
                        if (pThis->u.u_keystyle.color)
                                size +=
                                    5 + 4 + w_strlen(pThis->u.u_keystyle.color);
                        if (pThis->u.u_keystyle.xlfd)
                                size +=
                                    4 + 4 + w_strlen(pThis->u.u_keystyle.xlfd);
                        if (pThis->u.u_keystyle.xlfdi)
                                size +=
                                    5 + 4 + w_strlen(pThis->u.u_keystyle.xlfdi);
                        if (pThis->u.u_keystyle.xlfdb)
                                size +=
                                    5 + 4 + w_strlen(pThis->u.u_keystyle.xlfdb);
                        if (pThis->u.u_keystyle.xlfdib)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_keystyle.xlfdib);
                        if (pThis->u.u_keystyle.typenam)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_keystyle.typenam);
                        if (pThis->u.u_keystyle.typenami)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_keystyle.typenami);
                        if (pThis->u.u_keystyle.typenamb)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_keystyle.typenamb);
                        if (pThis->u.u_keystyle.typenamib)
                                size += 9 + 4 +
                                        w_strlen(pThis->u.u_keystyle.typenamib);
                        if (pThis->u.u_keystyle.style)
                                size +=
                                    5 + 4 + w_strlen(pThis->u.u_keystyle.style);
                        if (pThis->u.u_keystyle.spacing)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_keystyle.spacing);
                        if (pThis->u.u_keystyle.weight)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_keystyle.weight);
                        if (pThis->u.u_keystyle.slant)
                                size +=
                                    5 + 4 + w_strlen(pThis->u.u_keystyle.slant);
                        if (pThis->u.u_keystyle.special)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_keystyle.special);
                        size += 2 /* strlen(">\n") */;
                        break;
                case e_headstyle:
                        ...
                        if (pThis->u.u_headstyle.class)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_headstyle.class);
                        if (pThis->u.u_headstyle.ssi)
                                size +=
                                    3 + 4 + w_strlen(pThis->u.u_headstyle.ssi);
                        if (pThis->u.u_headstyle.rlevel)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_headstyle.rlevel);
                        if (pThis->u.u_headstyle.phrase)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_headstyle.phrase);
                        if (pThis->u.u_headstyle.srch_wt)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_headstyle.srch_wt);
                        if (pThis->u.u_headstyle.orient)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_headstyle.orient);
                        if (pThis->u.u_headstyle.vorient)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_headstyle.vorient);
                        if (pThis->u.u_headstyle.placement)
                                size +=
                                    9 + 4 +
                                    w_strlen(pThis->u.u_headstyle.placement);
                        if (pThis->u.u_headstyle.headw)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_headstyle.headw);
                        if (pThis->u.u_headstyle.stack)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_headstyle.stack);
                        if (pThis->u.u_headstyle.flow)
                                size +=
                                    4 + 4 + w_strlen(pThis->u.u_headstyle.flow);
                        if (pThis->u.u_headstyle.pointsz)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_headstyle.pointsz);
                        if (pThis->u.u_headstyle.set_width)
                                size +=
                                    9 + 4 +
                                    w_strlen(pThis->u.u_headstyle.set_width);
                        if (pThis->u.u_headstyle.color)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_headstyle.color);
                        if (pThis->u.u_headstyle.xlfd)
                                size +=
                                    4 + 4 + w_strlen(pThis->u.u_headstyle.xlfd);
                        if (pThis->u.u_headstyle.xlfdi)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_headstyle.xlfdi);
                        if (pThis->u.u_headstyle.xlfdb)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_headstyle.xlfdb);
                        if (pThis->u.u_headstyle.xlfdib)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_headstyle.xlfdib);
                        if (pThis->u.u_headstyle.typenam)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_headstyle.typenam);
                        if (pThis->u.u_headstyle.typenami)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_headstyle.typenami);
                        if (pThis->u.u_headstyle.typenamb)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_headstyle.typenamb);
                        if (pThis->u.u_headstyle.typenamib)
                                size +=
                                    9 + 4 +
                                    w_strlen(pThis->u.u_headstyle.typenamib);
                        if (pThis->u.u_headstyle.style)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_headstyle.style);
                        if (pThis->u.u_headstyle.spacing)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_headstyle.spacing);
                        if (pThis->u.u_headstyle.weight)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_headstyle.weight);
                        if (pThis->u.u_headstyle.slant)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_headstyle.slant);
                        if (pThis->u.u_headstyle.special)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_headstyle.special);
                        if (pThis->u.u_headstyle.l_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_headstyle.l_margin);
                        if (pThis->u.u_headstyle.r_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_headstyle.r_margin);
                        if (pThis->u.u_headstyle.t_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_headstyle.t_margin);
                        if (pThis->u.u_headstyle.b_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_headstyle.b_margin);
                        if (pThis->u.u_headstyle.border)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_headstyle.border);
                        if (pThis->u.u_headstyle.vjust)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_headstyle.vjust);
                        if (pThis->u.u_headstyle.justify)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_headstyle.justify);
                        size += 2 /* strlen(">\n") */;
                        break;
                case e_formstyle:
                        ...
                        if (pThis->u.u_formstyle.class)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_formstyle.class);
                        if (pThis->u.u_formstyle.ssi)
                                size +=
                                    3 + 4 + w_strlen(pThis->u.u_formstyle.ssi);
                        if (pThis->u.u_formstyle.rlevel)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_formstyle.rlevel);
                        if (pThis->u.u_formstyle.phrase)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_formstyle.phrase);
                        if (pThis->u.u_formstyle.srch_wt)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_formstyle.srch_wt);
                        if (pThis->u.u_formstyle.colw)
                                size +=
                                    4 + 4 + w_strlen(pThis->u.u_formstyle.colw);
                        if (pThis->u.u_formstyle.colj)
                                size +=
                                    4 + 4 + w_strlen(pThis->u.u_formstyle.colj);
                        if (pThis->u.u_formstyle.pointsz)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_formstyle.pointsz);
                        if (pThis->u.u_formstyle.set_width)
                                size +=
                                    9 + 4 +
                                    w_strlen(pThis->u.u_formstyle.set_width);
                        if (pThis->u.u_formstyle.color)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_formstyle.color);
                        if (pThis->u.u_formstyle.xlfd)
                                size +=
                                    4 + 4 + w_strlen(pThis->u.u_formstyle.xlfd);
                        if (pThis->u.u_formstyle.xlfdi)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_formstyle.xlfdi);
                        if (pThis->u.u_formstyle.xlfdb)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_formstyle.xlfdb);
                        if (pThis->u.u_formstyle.xlfdib)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_formstyle.xlfdib);
                        if (pThis->u.u_formstyle.typenam)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_formstyle.typenam);
                        if (pThis->u.u_formstyle.typenami)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_formstyle.typenami);
                        if (pThis->u.u_formstyle.typenamb)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_formstyle.typenamb);
                        if (pThis->u.u_formstyle.typenamib)
                                size +=
                                    9 + 4 +
                                    w_strlen(pThis->u.u_formstyle.typenamib);
                        if (pThis->u.u_formstyle.style)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_formstyle.style);
                        if (pThis->u.u_formstyle.spacing)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_formstyle.spacing);
                        if (pThis->u.u_formstyle.weight)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_formstyle.weight);
                        if (pThis->u.u_formstyle.slant)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_formstyle.slant);
                        if (pThis->u.u_formstyle.special)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_formstyle.special);
                        if (pThis->u.u_formstyle.l_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_formstyle.l_margin);
                        if (pThis->u.u_formstyle.r_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_formstyle.r_margin);
                        if (pThis->u.u_formstyle.t_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_formstyle.t_margin);
                        if (pThis->u.u_formstyle.b_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_formstyle.b_margin);
                        if (pThis->u.u_formstyle.border)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_formstyle.border);
                        if (pThis->u.u_formstyle.vjust)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_formstyle.vjust);
                        size += 2 /* strlen(">\n") */;
                        break;
                case e_frmtstyle:
                        ...
                        if (pThis->u.u_frmtstyle.class)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.class);
                        if (pThis->u.u_frmtstyle.ssi)
                                size +=
                                    3 + 4 + w_strlen(pThis->u.u_frmtstyle.ssi);
                        if (pThis->u.u_frmtstyle.rlevel)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.rlevel);
                        if (pThis->u.u_frmtstyle.phrase)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.phrase);
                        if (pThis->u.u_frmtstyle.f_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.f_margin);
                        if (pThis->u.u_frmtstyle.srch_wt)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.srch_wt);
                        if (pThis->u.u_frmtstyle.pointsz)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.pointsz);
                        if (pThis->u.u_frmtstyle.set_width)
                                size +=
                                    9 + 4 +
                                    w_strlen(pThis->u.u_frmtstyle.set_width);
                        if (pThis->u.u_frmtstyle.color)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.color);
                        if (pThis->u.u_frmtstyle.xlfd)
                                size +=
                                    4 + 4 + w_strlen(pThis->u.u_frmtstyle.xlfd);
                        if (pThis->u.u_frmtstyle.xlfdi)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.xlfdi);
                        if (pThis->u.u_frmtstyle.xlfdb)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.xlfdb);
                        if (pThis->u.u_frmtstyle.xlfdib)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.xlfdib);
                        if (pThis->u.u_frmtstyle.typenam)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.typenam);
                        if (pThis->u.u_frmtstyle.typenami)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.typenami);
                        if (pThis->u.u_frmtstyle.typenamb)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.typenamb);
                        if (pThis->u.u_frmtstyle.typenamib)
                                size +=
                                    9 + 4 +
                                    w_strlen(pThis->u.u_frmtstyle.typenamib);
                        if (pThis->u.u_frmtstyle.style)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.style);
                        if (pThis->u.u_frmtstyle.spacing)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.spacing);
                        if (pThis->u.u_frmtstyle.weight)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.weight);
                        if (pThis->u.u_frmtstyle.slant)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.slant);
                        if (pThis->u.u_frmtstyle.special)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.special);
                        if (pThis->u.u_frmtstyle.l_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.l_margin);
                        if (pThis->u.u_frmtstyle.r_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.r_margin);
                        if (pThis->u.u_frmtstyle.t_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.t_margin);
                        if (pThis->u.u_frmtstyle.b_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.b_margin);
                        if (pThis->u.u_frmtstyle.border)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.border);
                        if (pThis->u.u_frmtstyle.vjust)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.vjust);
                        if (pThis->u.u_frmtstyle.justify)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_frmtstyle.justify);
                        size += 2 /* strlen(">\n") */;
                        break;
                case e_grphstyle:
                        ...
                        if (pThis->u.u_grphstyle.class)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.class);
                        if (pThis->u.u_grphstyle.ssi)
                                size +=
                                    3 + 4 + w_strlen(pThis->u.u_grphstyle.ssi);
                        if (pThis->u.u_grphstyle.rlevel)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.rlevel);
                        if (pThis->u.u_grphstyle.phrase)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.phrase);
                        if (pThis->u.u_grphstyle.l_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.l_margin);
                        if (pThis->u.u_grphstyle.r_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.r_margin);
                        if (pThis->u.u_grphstyle.t_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.t_margin);
                        if (pThis->u.u_grphstyle.b_margin)
                                size += 8 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.b_margin);
                        if (pThis->u.u_grphstyle.border)
                                size += 6 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.border);
                        if (pThis->u.u_grphstyle.vjust)
                                size += 5 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.vjust);
                        if (pThis->u.u_grphstyle.justify)
                                size += 7 + 4 +
                                        w_strlen(pThis->u.u_grphstyle.justify);
                        size += 2 /* strlen(">\n") */;
                        break;
                }
                pThis = pThis->pNext;
...
void IterateLoids() {
        ...
        while (pLoids) {
                loidsSize += sprintf(buffer, "<ID TYPE=\"%s\" RID=\"%s\"",
                                     pLoids->type, pLoids->rid);
                if (pLoids->rssi)
                        loidsSize +=
                            sprintf(buffer, " RSSI=\"%s\"", pLoids->rssi);
                if (pLoids->rlevel)
                        loidsSize +=
                            sprintf(buffer, " RLEVEL=\"%s\"", pLoids->rlevel);
                loidsSize +=
        ...
        while (incr) {
                loidsSize = sprintf(buffer, openLoids, nIds);
                pLoids = loids.next; /* first one is a dummy (simplified some
                                        logic) */
                while (pLoids) {
                        pLoids->offset += incr;
                        loidsSize +=
                            sprintf(buffer, "<ID TYPE=\"%s\" RID=\"%s\" ",
                                    pLoids->type, pLoids->rid);
                        if (pLoids->rssi)
                                loidsSize += sprintf(buffer, "RSSI=\"%s\" ",
                                                     pLoids->rssi);
                        if (pLoids->rlevel)
                                loidsSize += sprintf(buffer, "RLEVEL=\"%s\" ",
                                                     pLoids->rlevel);
...
#define COMPRESS_PAD_LENGTH 10
        ...
        pad = 0;
        change +=
            COMPRESS_PAD_LENGTH; /* compensate for adding pad on first round */
        while (1) {
                oldZsize = newZsize;
                ...
                while (pLoids) {
                        pLoids->offset =
                            pLoids->offset - change + COMPRESS_PAD_LENGTH;
                        pLoids = pLoids->next;
                }
                ...
                delta = startOfLOIDS;
                while (length = ReadFile(inFd, compFileName, buffer,
                                         MIN(BUFSIZ, delta), xxx)) {
                ...
                pLoids = loids.next; /* first one is a dummy (simplified some
                                        logic) */
                while (pLoids) {
                        fprintf(outFile, "<ID TYPE=\"%s\" RID=\"%s\" ",
                                pLoids->type, pLoids->rid);
                        if (pLoids->rssi)
                                fprintf(outFile, "RSSI=\"%s\" ", pLoids->rssi);
                        if (pLoids->rlevel)
                                fprintf(outFile, "RLEVEL=\"%s\" ",
                                        pLoids->rlevel);
                ...
                FwriteFile(outFile, compFileName, closeLoids,
                           sizeof(closeLoids) - 1, xxx);
                 ...
                 */
                while (length =
                           ReadFile(inFd, compFileName, buffer, BUFSIZ, xxx))
                        FwriteFile(outFile, compFileName, buffer, length, xxx);
        ...
        while (pad >= 0) {
                FwriteFile(outFile, outFileName, padString,
                           MIN(pad, COMPRESS_PAD_LENGTH), xxx);
                pad -= COMPRESS_PAD_LENGTH;
        }
}
...
static void IncorporateVstructElements() {
        ...
        /* copy inFd to outFile up to start of <loids> */
        delta = startOfLOIDS;
        while (length = ReadFile(inFd, inFileName, buffer, MIN(BUFSIZ, delta),
                                 xxx)) {
        ...
        while (pLoids) {
                fprintf(outFile, "<ID TYPE=\"%s\" RID=\"%s\" ", pLoids->type,
                        pLoids->rid);
                if (pLoids->rssi)
                        fprintf(outFile, "RSSI=\"%s\" ", pLoids->rssi);
                if (pLoids->rlevel)
        ...
        /* followed by the end-tag for <loids> */
        FwriteFile(outFile, realOutFileName, closeLoids, sizeof(closeLoids) - 1,
                   xxx);
        ...
        {
                if (delta) /* an old <toss> is in the document, copy it to
                              outFile */
                {
                        delta -= endOfLOIDS;
                        while (length = ReadFile(inFd, inFileName, buffer,
                                                 MIN(BUFSIZ, delta), xxx)) {
                                FwriteFile(outFile, realOutFileName, buffer,
                                           length, xxx);
                                delta -= length;
                        }
                }
        ...
        if (endOfLOPhrases) {
                ...
                while (length = ReadFile(inFd, inFileName, buffer,
                                         MIN(BUFSIZ, delta), xxx)) {
                        FwriteFile(outFile, realOutFileName, buffer, length,
                                   xxx);
                        delta -= length;
                }
        }
        ...
        if (haveIndex) {
                ...
                while (length = ReadFile(idxFd, sortedIdxFileName, buffer,
                                         BUFSIZ, xxx))
                        FwriteFile(outFile, realOutFileName, buffer, length,
                                   xxx);
                unlink(sortedIdxFileName);
...
/* Cyclent.c contains procedure m_cyclent(), callable by interface
   ...
   about them */
#if defined(M_PROTO)
static M_WCHAR *CycleEnt(LOGICAL init, unsigned char *type, M_WCHAR ***content,
                         unsigned char *wheredef)
#else
static M_WCHAR *m_CycleEnt(init, type, content, wheredef) LOGICAL init;
unsigned char *type;
M_WCHAR ***content;
unsigned char *wheredef;
#endif
...
void UpdateTossFileName() {
        ...
        do {
                if ((type == M_SYSTEM) && (wheredef == M_DELTDEF) && name &&
                    *content) {
...
#else
    SetLocale(pLang, pCharset) M_WCHAR *pLang;
M_WCHAR *pCharset;
#endif
{
        ...
        if ((_DtLcxOpenAllDbs(&myDb) != 0) ||
            (_DtXlateGetXlateEnv(myDb, myPlatform, &execVer, &compVer) != 0)) {
                ...
                if (m_errfile != stderr)
                        fprintf(m_errfile, "Warning: could not open locale "
        ...
        isStd = !_DtLcxXlateOpToStd(myDb, "CDE", 0, DtLCX_OPER_STD, myLocale,
                                    NULL, NULL, NULL, NULL);

        if (isStd) { /* already standard - get local versions and set locale */
                if (_DtLcxXlateStdToOp(myDb, myPlatform, compVer,
                                       DtLCX_OPER_SETLOCALE, myLocale, NULL,
                                       NULL, NULL, &locale)) {
                        fprintf(stderr, "Warning: could not translate CDE "
                                        "locale to local\n");
                        if (m_errfile != stderr)
                                fprintf(m_errfile, "Warning: could not "
                                                   "translate CDE locale to "
                                                   "local\n");
                        _DtLcxCloseDb(&myDb);
                        if (mb_lang != cString)
                                mb_free(&mb_lang);
        ...
        } else { /* already local - just set locale */
