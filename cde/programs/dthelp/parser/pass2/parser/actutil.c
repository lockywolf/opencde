...
void m_endaction(m_elt) M_ELEMENT m_elt;
{
        M_ELEMENT m_action;
        ...
        if (m_tagtrace) {
                if (m_element[m_elt - 1].content != M_NONE) {
                        ...
                        sprintf(buffer, "%s%s%s", m_etago, m_nameofelt(m_elt),
                                m_tagc);
                        m_trace(buffer);
                }
                m_toptstat = M_OTHER;
...
int m_findact(elt, array) M_ELEMENT elt;
int *array;
{
        int chainlen = 0;
        int index;
...
#if defined(M_PROTO)
void m_findchain(M_PARSE *stackptr, int start, int chainin, int *chainout,
                 int *index, LOGICAL wild)
#else
void m_findchain(stackptr, start, chainin, chainout, index,
                 wild) M_PARSE *stackptr;
int start;
int chainin;
int *chainout;
int *index;
        ...
        for (node = m_action[start - 1].son; node;
             node = m_action[node - 1].next) {
                if (m_action[node - 1].element == 1)
                        m_findchain(stackptr, node, chainin, chainout, index,
                                    TRUE);
                else
                        for (stackp = stackptr; stackp->oldtop;
                             stackp = stackp->oldtop) {
                                if (stackp->element ==
                                    m_action[node - 1].element - 1) {
                                        ...
                                        m_findchain(stackp->oldtop, node,
                                                    chainin + 1, chainout,
                                                    index, FALSE);
                                }
                                if (!wild)
...
void m_globes(M_NOPAR) {
        ...
        if (m_ges)
                m_stcaction(wc_string, FALSE, TRUE);
...
void m_globss(M_NOPAR) {
        ...
        if (m_gss)
                m_stcaction(wc_string, TRUE, FALSE);
        m_free(wc_string, "wide character string");
}

/* When an explicit or implied start-tag occurs */
void m_strtaction(m_elt) M_ELEMENT m_elt;
{
        int m_par, m_i;
        M_WCHAR *m_p;
        M_ELEMENT m_action;
        ...
        if (m_tagtrace) {
                ...
                for (m_i = 0, m_par = m_element[m_elt - 1].parptr;
                     m_i < m_element[m_elt - 1].parcount; m_i++, m_par++)
                        if (m_stacktop->param[m_i]) {
                                sprintf(
                                    buffer, "%s%s%s", newpar,
                                    &m_pname[m_parameter[m_par - 1].paramname],
                                    quote);
                                m_trace(buffer);
                                buffer[1] = M_EOS;
...
#else
void m_textout(format, start, end) char *format;
LOGICAL start;
LOGICAL end;
#endif
{
        ...
        for (p = string; *format;) {
                if (*format == M_ESCAPECHAR) {
                        ...
                        if (!i) {
                                ...
                                mbtowc(p, &mb, 1);
                                p++;
                                /* Double escape character used to insert a
                                   single escape character in the output string
                                 */
                                if (*format == M_ESCAPECHAR)
                                        format++;
                                continue;
                        ...
                        for (found = FALSE, i = 0,
                            par = m_element[m_stacktop->element - 1].parptr;
                             ...
                             i++, par++)
                                if (!m_wcupstrcmp(&m_pname[m_parameter[par - 1]
                                                               .paramname],
                                                  name)) {
                                        ...
                                        changelen = w_strlen(q) -
                                                    w_strlen(name) - 1 - unused;
                                        if (changelen > 0) {
                                                new = (M_WCHAR *)m_malloc(
                                                    stringlen + changelen,
                                                    "string space");
                                                for (r = string, s = new;
                                                     r < p;)
