...
char **argv;
{
        ...
        static char sopt[] = "prevcon=%d,token=%d,curcon=%d,scanval='%c'(%d),"
                             "line=%d,netlevel=%d\n";
        ...
        while (TRUE) {
                ...
                if (!m_curcon) {
                        if (m_token != M_ENDFILE) {
                                ...
                                sprintf(buffer, parserr, m_token, m_prevcon,
                                        m_scanval);
                                m_errline(buffer);
                        } else {
                ...
                if (m_scantrace) {
                        sprintf(buffer, sopt, m_prevcon, m_token, m_curcon,
                                m_scanval ? m_scanval : ' ', m_scanval,
                                m_line[m_sysecnt], m_netlevel);
                        m_trace(buffer);
                        if (m_token == M_NAME) {
