...
static void CloseQuickHelpCB(Widget w, XtPointer clientData,
                             XtPointer callData);
 ...
 *
 ***************************************************************************/
void CloseAndExitCB(Widget w, XtPointer clientData, XtPointer callData) {
 ...
 *
 ***************************************************************************/
void CloseHelpCB(Widget w, XtPointer clientData, XtPointer callData) {
...

{
        ...
        case DtHELP_LINK_MAN_PAGE:
                ...
                DisplayMan(topLevel, hyperData->specification,
                           NO_EXIT_ON_CLOSE);
 ...
 *
 ****************************************************************************/
void DisplayTopic(Widget parent, char *helpVolume, char *locationId, int argc,
                  char **argv)

{
        ...
        if (cachedNode) {
                ...
                n = 0;
                XtSetArg(args[n], XmNtitle, titleString);
                n++;
                if (helpVolume != NULL) {
                ...
                XtSetArg(args[n], XmNtitle, titleString);
                n++;
                XtSetArg(args[n], DtNshowTopLevelButton, TRUE);
                n++;
                XtSetArg(args[n], DtNshowNewWindowButton, TRUE);
                n++;
                if (helpVolume != NULL) {
                        XtSetArg(args[n], DtNhelpVolume, helpVolume);
                        n++;
                ...
                if (argc != 0)
                        XSetCommand(
                            appDisplay,
                            XtWindow(XtParent(pCurrentNode->helpDialog)), argv,
                            argc);
        }
 ...
 *
 ****************************************************************************/
void DisplayFile(Widget parent, char *file)

{
        ...
        printString =
            XmStringCreateLocalized((char *)_DTGETMESSAGE(7, 19, "Print ..."));
        exitString =
        ...
        n++;
        XtSetArg(args[n], DtNprintLabelString, printString);
        n++;
        XtSetArg(args[n], XmNokLabelString, exitString);
        n++;
        XtSetArg(args[n], DtNhelpType, DtHELP_TYPE_FILE);
        n++;
        XtSetArg(args[n], DtNhelpFile, file);
        n++;
        XtSetArg(args[n], XmNscrollBarPlacement, DtHELP_AS_NEEDED_SCROLLBARS);
        n++;
        ...
        XtAddCallback(viewWidget, DtNcloseCallback, CloseAndExitCB,
                      (XtPointer)NULL);

        /* We do not want a help button for now so we unmap it */
        helpWidget =
            DtHelpQuickDialogGetChild(viewWidget, DtHELP_QUICK_HELP_BUTTON);
        XtUnmanageChild(helpWidget);

        /* We do not want a backtrack button for now so we unmap it */
        backWidget =
            DtHelpQuickDialogGetChild(viewWidget, DtHELP_QUICK_BACK_BUTTON);
 ...
 *
 ****************************************************************************/
void DisplayMan(Widget parent, char *man, int closeOption)

{
        ...
        if (viewWidget == NULL) {
                ...
                printString = XmStringCreateLocalized(
                    (char *)_DTGETMESSAGE(7, 19, "Print ..."));

                backString = XmStringCreateLocalized(
                    (char *)_DTGETMESSAGE(7, 18, "Backtrack"));

                if (closeOption == EXIT_ON_CLOSE) {
                        exitString = XmStringCreateLocalized(
                            (char *)_DTGETMESSAGE(7, 16, "Exit"));
                } else {
                        exitString = XmStringCreateLocalized(
                            (char *)_DTGETMESSAGE(7, 17, "Close"));
                }
                ...
                XtSetArg(args[n], XmNtitle, title);
                n++;
                XtSetArg(args[n], DtNbackLabelString, backString);
                n++;
                XtSetArg(args[n], DtNprintLabelString, printString);
                n++;
                XtSetArg(args[n], XmNokLabelString, exitString);
                ...
                helpWidget = DtHelpQuickDialogGetChild(
                    viewWidget, DtHELP_QUICK_HELP_BUTTON);
                XtUnmanageChild(helpWidget);
