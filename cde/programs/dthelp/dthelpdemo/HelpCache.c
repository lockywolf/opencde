...
static Boolean GetFromCache(Widget parent, CacheListStruct **pCurrentNode);
 ...
 *
 *****************************************************************************/
XtCallbackProc HelpMapCB(Widget w, XtPointer clientData, XtPointer callData)
...
static void CloseHelpCB(Widget w, XtPointer clientData, XtPointer callData) {
        ...
        if (pTemp == NULL)
                /* ERROR */
                printf("We did not find our help dialog widget in the cache "
                       "list??? /n");
 ...
 *
 ***************************************************************************/
static void CloseMainCB(Widget w, XtPointer clientData, XtPointer callData) {
 ...
 *
 ****************************************************************************/
void ProcessLinkCB(Widget w, XtPointer clientData, XtPointer callData)
        ...
        case DtHELP_LINK_APP_DEFINE:
                ...
                if (appLinkNum == 100) /* Move the window */
                {
                        ...
                        for (count = 1; count < 500; count = count + 5) {
                                n = 0;
                                XtSetArg(args[n], XmNx, count);
                                ++n;
                                XtSetArg(args[n], XmNy, count);
                                ++n;
                                XtSetValues(topLevel, args, n);
                ...
                if (appLinkNum == 101) /* Resize the window */
                {
                        ...
                        if (goBigger) {
                                n = 0;
                                XtSetArg(args[n], XmNheight, height + 100);
                                n++;
                                XtSetArg(args[n], XmNwidth, width + 50);
                                n++;
                                XtSetValues(topLevel, args, n);
                                goBigger = FALSE;
                        } else {
 ...
 *
 ****************************************************************************/
void DisplayMan(Widget parent, char *man)

{
        ...
        if (manWidget == NULL) {
                ...
                n++;
                XtSetArg(args[n], DtNhelpType, DtHELP_TYPE_MAN_PAGE);
                n++;
                XtSetArg(args[n], DtNmanPage, man);
                n++;
                manWidget =
                    DtCreateHelpQuickDialog(topLevel, "manBox", args, n);
                XtFree((char *)title);

                XtAddCallback(manWidget, DtNcloseCallback, CloseMainCB,
                              (XtPointer)manWidget);

                /* Add the popup position callback to our man dialog */
                XtAddCallback(XtParent(manWidget), XmNpopupCallback,
                              (XtCallbackProc)HelpMapCB, (XtPointer)topLevel);

                /* We do not want a help button for now so we unmap it */
                helpWidget = DtHelpQuickDialogGetChild(
                    manWidget, DtHELP_QUICK_HELP_BUTTON);
 ...
 *
 ****************************************************************************/
void DisplayTopic(Widget parent, char *helpVolume, char *locationId)

{
        ...
        if (cachedNode) {
                n = 0;
                XtSetArg(args[n], XmNtitle, "HelpDemo Help");
                n++;
                if (helpVolume != NULL) {
 ...
 *
 ****************************************************************************/
void DisplayMain(Widget parent, char *helpVolume, char *locationId)

{
        ...
        if (helpMain != NULL) {
                n = 0;
                XtSetArg(args[n], XmNtitle, "hemodemo Help");
                n++;
                if (helpVolume != NULL) {
                        XtSetArg(args[n], DtNhelpVolume, helpVolume);
                        n++;
                ...
                XtAddCallback(helpMain, DtNhyperLinkCallback, ProcessLinkCB,
                              NULL);

                XtAddCallback(helpMain, DtNcloseCallback, CloseMainCB,
                              (XtPointer)helpMain);

                /* Add the popup position callback to our main help dialog */
                XtAddCallback(XtParent(helpMain), XmNpopupCallback,
                              (XtCallbackProc)HelpMapCB, (XtPointer)parent);
 ...
 *
 ****************************************************************************/
void DisplayVersion(Widget parent, char *helpVolume, char *locationId)

{
        ...
        if (versionMain != NULL) {
                n = 0;
                XtSetArg(args[n], XmNtitle, "Helpdemo Version Dialog");
                n++;
                if (helpVolume != NULL) {
                        XtSetArg(args[n], DtNhelpVolume, helpVolume);
                        n++;
                ...
                n++;
                versionMain =
                    DtCreateHelpQuickDialog(parent, "versionWidget", args, n);

                XtAddCallback(versionMain, DtNcloseCallback, CloseMainCB,
                              (XtPointer)versionMain);

                /* We do not want a print button for now so we unmap it */
                printWidget = DtHelpQuickDialogGetChild(
                    versionMain, DtHELP_QUICK_PRINT_BUTTON);
                XtUnmanageChild(printWidget);

                /* We do not want a help button for now so we unmap it */
                helpWidget = DtHelpQuickDialogGetChild(
                    versionMain, DtHELP_QUICK_HELP_BUTTON);
                XtUnmanageChild(helpWidget);

                backWidget = DtHelpQuickDialogGetChild(
                    versionMain, DtHELP_QUICK_BACK_BUTTON);
...

{
        ...
        if (pCacheListHead == NULL) {
                ...
                *pCurrentNode = pCacheListHead;
                ...
                /* If we did not find a free nod then we must add a new one to
