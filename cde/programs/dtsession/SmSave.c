...
int CompleteSave() {
         ...
         */
        if (saveToHome == True)
                SaveDefaultPalette(smGD.display, smGD.savePath,
                                   DtSM_HOME_STATE);
        else
...
static void PruneSessionDirectory() {
        ...
        if (numDirs > smRes.numSessionsBackedup) {
                ...
                if ((tfd = mkstemp(tmpName)) == -1) {
                        ...
                        sprintf(saveDir, "%s/%s", smGD.savePath,
                                smGD.restoreSession);
...
static Boolean WriteClientDatabase() {
        ...
        if ((outputDB = OpenOutputClientDB(db, smXSMP.dbVersion,
                ...
                str = strdup(
                    (char *)GETMESSAGE(40, 18,
                                       "The following database file cannot be "
                                       "opened for writing:\n\n   %s\n\nThe "
...
static void OutputProxyClients(ClientDB outputDB) {
        ...
        for (i = 0; i < numProxyClients; i++)
                PrintStartupInfo(outputDB, proxyList[i].screen,
                                 proxyList[i].window);
}
...
static void OutputXSMPClients(ClientDB outputDB) {
        ...
        for (pClientRec = connectedList; pClientRec != NULL;
             pClientRec = pClientRec->next) {
                ...
                dbRec.cwd =
                    GetArrayPropertyValue(pClientRec, SmCurrentDirectory);
                ...
                dbRec.environment =
                    GetListOfArrayPropertyValue(pClientRec, SmEnvironment);
                ...
                if (!PutXSMPClientDBRec(outputDB, &dbRec)) {
                        ...
                        str = strdup((char *)GETMESSAGE(
                            40, 19, "Application '%s' cannot be saved."));
...
static Boolean CheckRequiredProperties(ClientRecPtr pClientRec) {
        ...
        if ((program = GetArrayPropertyValue(pClientRec, SmProgram)) == NULL) {
                ...
                str = strdup((char *)GETMESSAGE(
                    40, 23,
                    "The following application did not set the "
                    "required\nproperty '%s':\n\n   %s\n\nThis "
        ...
        if ((pPropRec = GetPropertyRec(pClientRec, SmRestartCommand)) == NULL) {
                ...
                str = strdup(
                    (char *)GETMESSAGE(40, 24,
                                       "24 Application '%s' will not be saved "
                                       "because it did not set\nthe required "
                                       "property '%s'."));

                if (str) {
                        DtMsgLogMessage(smGD.programName, DtMsgLogError, str,
                                        program, SmRestartCommand);
                        free(str);
                }
                return (False);
        }
...
void NotifyProxyClients() {
        ...
        for (j = 0; j < smGD.numSavedScreens; j++) {
                 ...
                 */
                if (GetTopLevelWindows(j, &topList, &topListLength,
                                       &containedListLength))
                        return;

                /*
                 * Figure out how to restart the client associate with each
                 * window.
                 */
                for (i = 0; i < (topListLength + containedListLength); ++i) {
                        ...
                        if (XGetWindowProperty(
                                smGD.display, topLevelWindowInfo.wid,
                                XA_WM_CLASS, 0L, (long)BUFSIZ, False, XA_STRING,
                        ...
                        /*
                         * If the client participates in the WM_SAVE_YOURSELF
                         * protocol, send the appropriate messages and wait for
                         * the proper responses.
                         */
                        (void)SaveYourself(topLevelWindowInfo);

                        CacheProxyClient((numClients == 0), j,
                                         topLevelWindowInfo.wid);
...
static int QueryServerSettings(void) {
         ...
         */
        numButton =
            XGetPointerMapping(smGD.display, (unsigned char *)buttonRet, 128);
        if (numButton > 128) {
                buttonRet =
                    (char *)SM_REALLOC(buttonRet, numButton * sizeof(char));
                if (buttonRet == NULL) {
                        if (fontPath)
                                XFreeFontPath(fontPath);
                        PrintErrnoError(DtError, smNLS.cantMallocErrorString);
                ...
                numButton = XGetPointerMapping(
                    smGD.display, (unsigned char *)buttonRet, numButton);
        }
        ...
        smSettings.numKeyCode =
            smGD.display->max_keycode - smGD.display->min_keycode;
        tmpSyms = XGetKeyboardMapping(
        ...
        for (i = 1; i < numLoops; i++) {
                if ((strlen(resSpec) + 5) >= resSize) {
                        resSize += 5000;
                        resSpec =
                            (char *)SM_REALLOC(resSpec, resSize * sizeof(char));
                        if (resSpec == NULL) {
        ...
        for (i = 0; i <= numLoops; i++) {
                ...
                tmpMod =
                    XKeycodeToKeysym(smGD.display, modMap->modifiermap[i], 0);
...
static int SaveCustomizeSettings(void) {
        ...
        if (smCust.pointerChange) {
                ...
                if (newStat != Success) {
                        ...
                        sscanf(ptrRet, "%d %s %d %d %s", &smToSet.numButton,
                               &tmpMap, &smSettings.accelNum,
                               &smSettings.threshold, &smToSet.dClickBuf);
        ...
        *resSpec = 0;
        ...
        if (smCust.fontChange == True) {
                ...
                if (newStat == Success) {
                        ...
                        /*
                         * Now add this to the resource manager property to be
                         * saved
                         */
...
static void PrintScreenSaver(XrmDatabase *smBase) {
        ...
        if (smCust.screenSavChange == True) {
                newScreenStat =
                    _DtGetSmScreen(smGD.display, smGD.topLevelWindow,
...
static int PrintKeyboardControl(XrmDatabase *smBase) {
        ...
        if (smSettings.kbdState.global_auto_repeat != AutoRepeatModeOn) {
                for (i = 0; i < 32; i++) {
                        ...
                        for (j = 0; j < 8; j++) {
                                if ((bitTest &
                                     smSettings.kbdState.auto_repeats[i]) !=
                                    0) {
                                        if (firstRepeat == True) {
                                                firstRepeat = False;
                                        } else {
