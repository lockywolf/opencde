...
#define MSG2                                                                   \
        ((char *)GETMESSAGE(                                                   \
            28, 3, "Color Server Warning, the size of file is invalid: "))
#define MSG3                                                                   \
        ((char *)GETMESSAGE(28, 4,                                             \
                            "Color Server Warning, invalid information in '"))
#define MSG3a                                                                  \
        ((char *)GETMESSAGE(28, 5, "' removing file and starting again.\n"))

/* Static Function Declarations */

static Boolean FindPalette(char *palette, char *directory);
static struct _palette *ReadPaletteFile(Display *dpy, int screen_number,
                                        char *palettePath, char *palette);
static int ParsePaletteInfo(Display *dpy, int numDisplay, char *buf, int nbytes,
...
**
************************************************************************/
struct _palette *GetPaletteDefinition(Display *dpy, int screen_number,
                                      char *palette)

{
        ...
        if (FindPalette(palette, SYSTEM_PALETTE_DIR)) {
                 ...
                 */
                palettePath =
                    (char *)SRV_MALLOC(strlen(SYSTEM_PALETTE_DIR) + 1);
        ...
        if (XrmGetResource(db, "dtstyle.paletteDirectories",
                           "Dtstyle.PaletteDirectories", &str_type_return,
                           &value_return)) {
        ...
        if (FindPalette(palette, path)) {
                palettePath =
                    (char *)SRV_REALLOC(palettePath, strlen(path) + 1);
        ...
        if (match) {
                ...
                paletteDef = (struct _palette *)ReadPaletteFile(
                    dpy, screen_number, palettePath, palette);
        } else /* default to system Default */
        {
                palettePath = (char *)SRV_REALLOC(
                    palettePath, strlen(SYSTEM_PALETTE_DIR) + 1);
                strcpy(palettePath, SYSTEM_PALETTE_DIR);
                paletteDef = (struct _palette *)ReadPaletteFile(
                    dpy, screen_number, palettePath, DEFAULT_PALETTE);
        }
...
static struct _palette *ReadPaletteFile(Display *dpy, int screen_number,
         ...
         */
        fullPath =
            (char *)SRV_MALLOC(strlen(palettePath) + strlen(palette) + 2);
        ...
        */
        ...
        {                                     /* read failed */
                 ...
                 */
                tmpStr =
                    (char *)SRV_MALLOC(strlen(MSG2) + strlen(fullPath) + 3);
                sprintf(tmpStr, "%s%s.\n", MSG2, palettePath);
                _DtSimpleError(XmSCOLOR_SRV_NAME, DtWarning, NULL, tmpStr,
                               NULL);
                SRV_FREE(tmpStr);
                SRV_FREE(fullPath);
                close(fd);
                ...
                new_palette =
                    (palettes *)SRV_MALLOC(sizeof(struct _palette) + 1);
                ...
                if (ParsePaletteInfo(dpy, screen_number, buf, nbytes,
                                     new_palette) == -1) {
                        /* palette file is bad */
                        tmpStr =
                            (char *)SRV_MALLOC(strlen(MSG3) + strlen(MSG3a) +
                                               strlen(palettePath) + 1);
 ...
 *
 ***********************************************************************/
static int ParsePaletteInfo(Display *dpy, int screen_num, char *buf, int nbytes,
                            struct _palette *new_palette) {
        char tmpbuf[BUFSIZE];
        int count;
        int num_of_colors;
        ...
        while (buf_count < nbytes && buf[buf_count] != '!') {
                ...
                /*
                ** Now lets generate all the colors which go along with this bg
                *i.e. ts,
                ** fg, bs, and sc.
                */
                if (calcRGB == NULL)
                        calcRGB = XmGetColorCalculation();
                (*calcRGB)(&tmp_color, &(new_palette->color[num_of_colors].fg),
                ...
                else /* Not Black and White */
                {
                        if (colorSrv.UsePixmaps[screen_num] == TRUE) {
                                /* the values generated by XmCalculateColorRBG
                                 * are invalid */
                                new_palette->color[num_of_colors].ts.red =
                                    65535;
                                new_palette->color[num_of_colors].ts.blue =
                                    65535;
                                new_palette->color[num_of_colors].ts.green =
                                    65535;
                        ...
                        if (colorSrv.FgColor[screen_num] != DYNAMIC) {
                                if (colorSrv.FgColor[screen_num] == BLACK) {
                                        new_palette->color[num_of_colors]
                                            .fg.red = 0;
                                        new_palette->color[num_of_colors]
                                            .fg.blue = 0;
                                        new_palette->color[num_of_colors]
...
void SwitchAItoPS(struct _palette *new_palette) {
 ...
 *
 ***********************************************************************/
void SaveDefaultPalette(Display *dpy, char *dtPath, int mode) {}
