        ...
        if (localWmLaunched && wmTimeout) {
                ...
                sprintf(localWmErrorString,
                        GETMESSAGE(
                            ...
                            "This will only work with X terminals that support "
                            "this "
                            "protocol.\n" CDE_INSTALLATION_TOP
     ...
     */
     if ((pgrm = getenv("DTLOADRESOURCES")) == NULL)
     {
       pgrm = CDE_INSTALLATION_TOP "/bin/dtsession_res";
         ...
         */
        argv[0] =
            (p = strrchr(pgrm, '/')) != NULL && *(p + 1) != '\0' ? p + 1 : pgrm;
...
int RestorePreferences(char *filename) {
        ...
        if (filename) {
                ...
                if (size == statinfo.st_size) {
                        /*
                         * Merge .Xdefaults string into RESOURCE_MANAGER
                         * database, and also convert to Xrm database form for
                         * later subtraction.
                         */
                        data[size] = '\0';
...
static int RestoreSettings(void) {
        ...
        XtGetApplicationResources(smGD.topLevelWid, (XtPointer)&smSettings,
                                  settingsResources,
                                  XtNumber(settingsResources), NULL, 0);
        ...
        if ((smSettings.accelNum > -1) || (smSettings.threshold > -1)) {
                XChangePointerControl(
                    smGD.display, True, True, smSettings.accelNum,
        ...
        if ((smSettings.timeout > -1) || (smSettings.interval > -1) ||
            ...
            (smSaverRes.cycleTimeout > -1)) {
                ...
                /*
                 * For the screen saver time, use the minimum of the values set
                 * for saverTimeout or lockTimeout if the user has set one of
                 * these values.
                 *
                 ...
                 * saverTimeout or lockTimeout.  Set timeout to the Timeout
                    ...
                    (smSaverRes.lockTimeout > -1)) {
                        if (smSaverRes.saverTimeout > -1) {
                                if ((smSaverRes.lockTimeout > -1) &&
                                    (smSaverRes.lockTimeout <
                                     smSaverRes.saverTimeout)) {
                                        screenSaverVals.smTimeout =
                                            smSaverRes.lockTimeout;
                                } else {
                                        screenSaverVals.smTimeout =
                                            smSaverRes.saverTimeout;
        ...
        *tmpRestore = strtok(smGD.fontDirs, ",");
        ...
        while (*tmpRestore != NULL) {
                ...
                *tmpRestore = strtok(NULL, ",");
                if ((numArgs >= ptrSize) && (*tmpRestore != NULL)) {
                        ptrSize += 50;
                        restorePtrArray = (char **)SM_REALLOC(
                            (char *)restorePtrArray, ptrSize * sizeof(char *));
                        if (restorePtrArray == NULL) {
        ...
        if ((smSettings.kbdState.key_click_percent > -1) ||
            ...
            (smSettings.kbdState.bell_duration > -1)) {
                ...
                if ((smSettings.kbdState.bell_percent > -1) ||
                    ...
                    (smSettings.kbdState.bell_duration > -1)) {
                        kbdControl.bell_percent =
                            smSettings.kbdState.bell_percent;
                        kbdControl.bell_pitch = smSettings.kbdState.bell_pitch;
                        kbdControl.bell_duration =
                            smSettings.kbdState.bell_duration;
                        kbdControlMask |=
                            (KBBellPercent | KBBellPitch | KBBellDuration);
                        smToSet.audioChange = True;
                        audioVals.smBellPercent =
                            smSettings.kbdState.bell_percent;
                        audioVals.smBellPitch = smSettings.kbdState.bell_pitch;
                        audioVals.smBellDuration =
                            smSettings.kbdState.bell_duration;
                } else {
                ...
                if ((smSettings.kbdState.key_click_percent > -1) ||
                    (smSettings.kbdState.global_auto_repeat !=
                     AutoRepeatModeOn)) {
                        ...
                        keyboardVals.smKeyClickPercent =
                            kbdControl.key_click_percent;
                } else {
                ...
                /*
                 * NOTICE THAT THE LED'S DON'T GET RESET.  THIS IS BECAUSE LED
                 * STUFF IS MACHINE DEPENDENT.
                 */
                /*
                    ...
                    ((smSettings.kbdState.global_auto_repeat ==
                     ...
                     (smSettings.kbdState.global_auto_repeat ==
                      AutoRepeatModeOn))) {
                        ...
                        XChangeKeyboardControl(smGD.display, kbdControlMask,
                                               &kbdControl);
                        ...
                        if (tmpKey != NULL) {
                                ...
                                XChangeKeyboardControl(
                                    smGD.display, kbdControlMask, &kbdControl);
                                kbdControl.auto_repeat_mode = AutoRepeatModeOn;
                                kbdControlMask = KBAutoRepeatMode | KBKey;
                                /*
                                 * This is only involked when there is auto
                                 * repeats set for specific keys only.  It is
                                 * VERY SLOW code so unless you have to save off
                                 * auto repeats for single keys - DONT
                                 */
                                while (tmpKey != NULL) {
        ...
        while (tmpKey != NULL) {
                ...
                if ((numArgs >= charSize) && (tmpKey != NULL)) {
                        charSize += 500;
                        restoreCharArray = (char *)SM_REALLOC(
                            restoreCharArray, charSize * sizeof(char));
                        if (restoreCharArray == NULL) {
                                PrintErrnoError(DtError,
                                                smNLS.cantMallocErrorString);
                                SM_FREE((char *)restorePtrArray);
        ...
        if (numArgs > 0) {
                XSetPointerMapping(smGD.display,
                                   (unsigned char *)restoreCharArray, numArgs);
        ...
        if (smSettings.numKeyCode > 0) {
                ...
                for (i = 0; tmpKey != NULL; i++) {
                        ...
                        if ((numArgs >= symSize) && (tmpKey != NULL)) {
                                charSize += 1000;
                                symSize =
                                    (charSize * sizeof(char)) / sizeof(KeySym);
                                restoreCharArray = (char *)SM_REALLOC(
                                    restoreCharArray,
                ...
                XChangeKeyboardMapping(
                    smGD.display, (KeyCode)smGD.display->min_keycode,
        ...
        if (tmpKey != NULL) {
                for (i = 0; i < (8 * smSettings.maxKeyPerMod); i++) {
                        if (tmpKey != NULL) {
                                ...
                                if (codeSym != 0) {
                                        tmpCode[i] = XKeysymToKeycode(
                                            smGD.display, codeSym);
                                } else {
                                        tmpCode[i] = (KeyCode)0;
                                }
                        } else {
...
void RestoreIndependentResources(void) {
        ...
        if ((strcmp(currentLangPtr, smGD.sessionLang)) ||
            (intYRes[0] != intYRes[1])) {
                ...
                snprintf(fontPath, MAXPATHLEN, "%s/%s/%s/%s.%s", smGD.savePath,
                         sessionType, currentLangPtr, SM_FONT_FILE, sessionRes);
                        ...
                        if (status == -1) {
                                if ((currentLangPtr != NULL) &&
                                    ...
                                    (strcmp(currentLangPtr, "C"))) {
                                        memset(fontPath, 0, MAXPATHLEN + 1);
                                        strcpy(fontPath, "/C/");
                                        strncat(fontPath, SM_SYSTEM_FONT_FILE,
                                                MAXPATHLEN);
                ...
                if (resIndep == True) {
                         ...
                         */
                        RestoreResources(True, "-merge", "-file", smGD.fontPath,
                                         NULL);
                        ...
                        if (XGetWindowProperty(
                                smGD.display, RootWindow(smGD.display, 0),
                        ...
                        if (smBase == NULL) {
                                 ...
                                 * the auxillary resources into the display
                                 * struct
                                 */
                                resRet = XrmGetResource(smBase, "*fontList",
                                                        "*FontList", &resValRet,
                                                        &fontResourceReturn);
                                ...
                                /*
                                 * Overwrite the old font preferences,
                                 * otherwise, they'll come back to haunt us if
                                 * the user logs in again under the current
                                 * language. We need to get the font resources
                                 * which have just been loaded, then overlay
                                 * them onto the _DT_SM_PREFERENCES root
                                 * property to make sure we get them back when
                                 * we login again.
                                 */
                                for (i = 0; i < XtNumber(fonttype); i++) {
                                        resRet = XrmGetResource(
                                            smBase, fonttype[i], fontclass[i],
                                            &resValRet, &fontResourceReturn);
                                        if (resRet) {
                                                resdata = (char *)SM_MALLOC(
                                                    fontResourceReturn.size +
                                                    30);
                                                bzero(resdata,
                                                      fontResourceReturn.size +
                                                          30);
                                                ...
                                                strcat(resdata, ": ");
                                                strcat(resdata,
                                                       fontResourceReturn.addr);
                                                strcat(resdata, "\n");
                                                _DtAddResString(smGD.display,
                                                                resdata,
                                                                _DT_ATR_PREFS);
                                                SM_FREE(resdata);
                                        }
...
static int RestoreClients(void) {
        ...
        if (dispEnv != NULL) {
                dispSav = (char *)SM_MALLOC(
                    ((strlen(dispEnv) + strlen(DISPLAY_NAME_EQUAL)) *
                         sizeof(char) +
                     1));
                ...
                if (string != NULL && !strcmp((char *)string, "dtsmcmd")) {
                        while ((string = GetSmartString(&lineP)) != NULL) {
                                if (!strcmp((char *)string, "-host")) {
                                        /*
                                         * Extract a host pointer - host
                                         * pointers only exist on remote
                                         * executions
                                         */
                                        string = GetSmartString(&lineP);
                                        hostPtr = string;
                        ...
                        /*
                         * Now put our information in buffers and reinitialize
                         * the pointers
                         */
                        if (hintPtr != NULL) {
                                if (FillHintBuf(hintPtr, hintsBuf, maxHintsSize,
                                                actHintsSize, screenNum,
                                                numHints) != 0) {
                                        /*
                                         * Free all malloc'd buffers and exit
                                         */
                                        for (i = 0; i < smGD.numSavedScreens;
                                             i++) {
                                                if (actHintsSize[i] > 0) {
                                                        SM_FREE((
                                                            char *)hintsBuf[i]);
                        ...
                        if ((cmdPtr != NULL) && (hostPtr == NULL)) {
                                ...
                                if ((cmdPtr != NULL) && (hostPtr != NULL)) {
                                        if (FillRemoteBuf(
                                                cmdPtr, hostPtr, remoteDisplay,
                                                remoteBuf, maxRemoteSize,
                                                actRemoteSize,
                                                screenNum) != 0) {
                                                /*
                                                 * Free all malloc'd buffers and
                                                 * exit
                                                 */
                                                for (i = 0;
                                                     i < smGD.numSavedScreens;
                                                     i++) {
                                                        if (actHintsSize[i] >
                                                            0) {
                                                                SM_FREE(
        ...
        for (i = 0; i < smGD.numSavedScreens; i++) {
                ...
                if (actHintsSize[i] > 0) {
                        ...
                        strncpy((char *)hintsBuf[i], (char *)tmpChar,
                                strlen((char *)tmpChar));
                        XChangeProperty(
                            smGD.display, RootWindow(smGD.display, i),
        ...
        if (smRes.contManagement & SM_CM_SYSTEM) {
                fprintf(stderr,
                        "SM_CM_SYSTEM flag set in smRes.contManagement\n");
        }
        if (smRes.contManagement & SM_CM_HANDSHAKE) {
                fprintf(stderr,
                        "SM_CM_HANDSHAKE flag set in smRes.contManagement\n");
        }
#endif /* DEBUG_CONT */
        numClientsExec = 0;
        if ((smRes.contManagement & SM_CM_SYSTEM) &&
             ...
             (clientsDone == False))) {
                while ((GetMemoryUtilization() == MEM_NOT_FULL) &&
                       (clientsDone == False)) {
                        ...
                        if (smExecArray[0] != NULL) {
                                (void)StartClient(smExecArray[0],
                                                  (char **)smExecArray, NULL,
                                                  NULL, NULL, False, False, -1);
                        }
                        numClientsExec++;
                        ...
                        if (numClientsExec >= numCmd[i]) {
                                ResetScreenInfo(cmdBuf, &i, actCmdSize,
                                                &clientsDone, envVal,
                                                displayName);
                                numClientsExec = 0;
                        }
        ...
        if (clientsDone == False) {
                if ((smGD.dtwmRunning) &&
                    (smRes.contManagement & SM_CM_HANDSHAKE)) {
                        /*
                         * Get the window id of the workspace manager and tell
                         * it to start messaging
                         */
                        _DtGetMwmWindow(smGD.display,
                                        RootWindow(smGD.display, 0), &dtwmWin);
                        ...
                        if (XSendEvent(smGD.display, dtwmWin, False,
                                       NoEventMask,
        ...
        for (i = 0; i < smGD.numSavedScreens; i++) {
                ...
                if (actRemoteSize[i] > 0) {
                         ...
                         */
                        sprintf(envVal, "%s%s%d", DISPLAY_NAME_EQUAL,
                                displayName, i);
                        putenv(envVal);
                        ...
                        while (numRemoteExecs > 0 && GetNextLine() != NULL) {
                                /*
                                 * Get the host and the command and send them
                                 * off On failure of the command invoker do a
                                 * remsh
                                 */
                                lineP = line;
                                string = GetSmartString(&lineP);
                                ...
                                _DtCommandInvokerExecute(
                                    DtSTART_SESSION, NULL, NULL, NULL, "-",
                                ...
                                if (numRemoteDone == MAX_REMOTE_CLIENTS) {
                                        PrintError(
                                            ...
                                            GETMESSAGE(16, 5,
                                                       ...
                                                       "of remote clients.  No "
                                                       "further remote clients "
                                                       "will be restored."));
                                        break;
                                }
                        }
...
void StartEtc(Boolean exiting) {
        ...
        if ((status = stat(execArray[0], &buf)) != -1) {
                StartClient(execArray[0], execArray, NULL, NULL, NULL, False,
                            False, -1);
        }
}
        ...
        /* Quoted string */
        {
                 ...
                 */
                {
                        ...
                        if ((chlen == 1) && (*endP == '\\') &&
                            ((chlen = mblen((char *)curP, MB_CUR_MAX)) > 0)) {
                                /*
                                 * Check to see if this is a quoted quote - if
                                 * it is strip off a level - if not - it's
                                 * sacred leave it alone
                                 */
                                checkLev =
                                    PeekAhead((curP - 1), quoteLevel[level]);
                                if (checkLev > 0) {
...
static int FillHintBuf(unsigned char *newHint, unsigned char **hintBuf,
         ...
         * malloc it.  If it has - check to make sure it's big enough to hold
         * the new information.
         */
        if (maxSize[screen] == 0) {
                hintBuf[screen] =
                    (unsigned char *)SM_MALLOC(hintBufSize * sizeof(char));
                ...
                if ((actSize[screen] + strlen((char *)newHint) + 2) >=
                    maxSize[screen]) {
...
static int FillCmdBuf(unsigned char *newCmd, unsigned char **cmdBuf,
        ...
        if (maxSize[screen] == 0) {
                cmdBuf[screen] =
                    (unsigned char *)SM_MALLOC(cmdBufSize * sizeof(char));
...
static int FillRemoteBuf(unsigned char *newCmd, unsigned char *hostName,
        ...
        if (maxSize[screen] == 0) {
                ...
                if ((actSize[screen] + strlen((char *)newCmd) +
                     strlen((char *)hostName) + strlen((char *)displayName) +
...
#define ARG_AMT 100
        ...
        while (string != NULL) {
                ...
                if (argc >= iSizeArgv) {
                        iSizeArgv += ARG_AMT;
                        smExecArray = (char **)XtRealloc(
                            (char *)smExecArray, (iSizeArgv * sizeof(char *)));
                }
                string = (char *)GetSmartString((unsigned char **)&execString);
        }
...
static void SetTemporaryDisplay(int screenNum) {
        ...
        if (!savedDisplay) {
                ...
                if ((dispEnv = getenv(DISPLAY_NAME)) == NULL)
                        return;
                savedDisplay =
                    XtMalloc(strlen(DISPLAY_NAME_EQUAL) + strlen(dispEnv) + 2);
                if (!savedDisplay)
                        return;
...
Boolean StartClient(char *program, char *execArray[], char *hostName, char *cwd,
        ...
        if (!ignoreEnvPtr && useIgnoreEnvironmentResource) {
                if (smRes.ignoreEnvironment)
                        ignoreEnvPtr =
                            _DtVectorizeInPlace(smRes.ignoreEnvironment, ',');
        }
        ...
        if (!hostName || (_DtIsSameHost(localHost, hostName)))
                startStatus =
                    StartLocalClient(program, execArray, cwd, envp, checkCwd,
                                     useIgnoreEnvironmentResource);
        else
                startStatus =
                    StartRemoteClient(program, execArray, hostName, cwd, envp,
                                      useIgnoreEnvironmentResource);
...
static Boolean StartLocalClient(char *program, char *execArray[], char *cwd,
        ...
        if (clientFork == 0) {
                ...
                if (execStatus != 0) {
                        sprintf(
                            clientMessage,
                            ((char *)GETMESSAGE(16, 3, "Unable to exec %s.")),
                            execArray[0]);
                        PrintErrnoError(DtError, clientMessage);
                        SM_EXIT(-1);
                }
...
static Boolean StartRemoteClient(char *program, char *execArray[],
        ...
        if (channel == SPC_ERROR) {
                message = strdup((char *)GETMESSAGE(
                    40, 13,
                    "The following application cannot be started on host "
                    "'%s'\nbecause "
        ...
        if (tt_pointer_error(netfile) != TT_OK) {
                ...
                message = strdup(
                    (char *)GETMESSAGE(40, 14,
                                       "An attempt to start application "
                                       "'%s'\non host '%s' failed because the "
                                       "following directory\ncould not be "
                                       "translated into cannonical "
                                       "form:\n\n   %s\n\n[%s]"));

                if (message) {
                        DtMsgLogMessage(
                            smGD.programName, DtMsgLogError, message, program,
                            hostName, cwd,
        ...
        if ((_DtSPCSpawn(program, netfile, execArray, tmpEnv, channel, hostName,
                         NULL, cwd, errorMessage)) == SPC_ERROR) {
        ...
        if (gethostname(hostName, (sizeof(hostName) - 1)) == 0) {
                ...
                if (dpy && homeDir) {
                        ...
                        if (access(tmpExecWmFile, X_OK) != 0) {
                                strcpy(tmpExecWmFile,
                                       CDE_INSTALLATION_TOP "/bin/dtwm");
                        } else {
...
static void WaitForWM(void) {
         ...
         */
        wmTimeout = False;
        wmTimerId = XtAppAddTimeOut(smGD.appCon, smRes.waitWmTimeout,
                                    WaitWMTimeout, NULL);
...
static void FixEnvironmentData(void) {
        ...
        for (i = 0, ppchar = environ; *ppchar; ppchar++, i++) {
                if ((strncmp(*ppchar, DISPLAY_NAME_EQUAL,
                             strlen(DISPLAY_NAME_EQUAL))) == 0) {
                        /*
...
static void ResetScreenInfo(unsigned char **cmdBuf, int *screen,
        ...
        if (*screen >= smGD.numSavedScreens) {
                *done = True;
                ...
                sprintf((char *)env, "%s%s%d", DISPLAY_NAME_EQUAL, displayName,
                        *screen);
                putenv(env);
...
static void RemoteRequestFailed(char *message, void *client_data) {
        ...
        if (tmpSize >= cmdBufSize) {
                ...
                if (tmpCmdBuf != NULL) {
                        tmpCmdBuf =
                            SM_REALLOC(tmpCmdBuf, (tmpSize) * sizeof(char));
                        if (tmpCmdBuf == NULL) {
                                PrintErrnoError(DtError,
                                                smNLS.cantMallocErrorString);
                        }
        ...
        if (smExecArray[0] != NULL) {
                (void)StartClient(smExecArray[0], smExecArray, NULL, NULL, NULL,
                                  False, False, -1);
                ...
                if (tmpCmdBuf != NULL) {
                        tmpString = GETMESSAGE(
                            ...
                            "(using the DT remote execution process)\non host "
                            "\"%s\" failed:\n\n      %s\n\nThe following "
                            "execution string will be tried:\n\n   %s\n\n");
...
Boolean StartXSMPClient(XSMPClientDBRecPtr pDbRec, char *databaseName) {
        return (StartClient(pDbRec->restartCommand[0], pDbRec->restartCommand,
                            pDbRec->clientHost, pDbRec->cwd,
                            pDbRec->environment, True, True,
                            pDbRec->screenNum));
}

Boolean StartProxyClient(ProxyClientDBRecPtr pDbRec) {
        return (StartClient(pDbRec->command[0], pDbRec->command,
                            pDbRec->clientHost, NULL, NULL, False, False,
                            pDbRec->screenNum));
}
...
Boolean ExecuteCommandProperty(char *commandName, ClientRecPtr pClientRec) {
        ...
        argv =
            (char **)XtMalloc((pPropRec->prop.num_vals + 1) * sizeof(char *));
        if (!argv)
                return (False);
        ...
        retValue = StartClient(argv[0], argv, pClientRec->clientHost, cwd, envp,
                               True, True, pClientRec->screenNum);
...
void ExecuteDiscardCommands(char *db) {
        ...
        if ((inputDB = OpenInputClientDB(db, &version, &id)) == NULL) {
                str = strdup((char *)GETMESSAGE(
                    40, 26,
                    "The following client database cannot be "
                    "opened:\n\n   %s\n\nThe 'DiscardCommand' "
                    "properties cannot be run."));
                if (!str)
                        return;
        ...
        for (;;) {
                ...
                if (!StartClient(pXSMPRec->discardCommand[0],
                                 pXSMPRec->discardCommand, pXSMPRec->clientHost,
                                 pXSMPRec->cwd, pXSMPRec->environment, True,
                                 True, pXSMPRec->screenNum)) {
                        str = strdup((char *)GETMESSAGE(
                            40, 27,
                            "An attempt to execute the "
                            "'DiscardCommand' property "
...
static Boolean CheckRequiredProperties(XSMPClientDBRecPtr pDbRec,
        ...
        if (!pDbRec->program) {
                LogMissingPropertyMessage(pDbRec, SmProgram, databaseName,
                                          DtMsgLogError);
                propsOK = False;
        }
        ...
        case 2:
                ...
                message = strdup(
                    (char *)GETMESSAGE(40, msgNum,
                                       "The following session database does "
...
static void LogCWDMessage(char *badDir, char *appName, char *goodDir) {
        ...
        DtMsgLogMessage(smGD.programName, DtMsgLogWarning, message, badDir,
                        appName, goodDir);
