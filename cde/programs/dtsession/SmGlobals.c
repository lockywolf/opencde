...
static void _SmWaitClientTimeoutDefault(Widget widget, int offset,
                                        XrmValue *value);

static void _SmWaitWmTimeoutDefault(Widget widget, int offset, XrmValue *value);
...
static Boolean InitializeSpecificSession(char *session_name, Display *disp,
                                         unsigned int argc, char **argv);

static void InitializeGenericSession(Display *disp);

static void InitializePaths(char *session_option, Display *disp);

static Boolean SetAlternateSession(char *session_dir, char *alt_dir,
                                   Boolean make_dir);
...
static XtResource sessionResources[] = {
    {SmNwmStartup, SmCwmStartup, XtRString, sizeof(String),
     XtOffset(SessionResourcesPtr, wmStartup), XtRImmediate, (XtPointer)NULL},
    {SmNquerySettings, SmCquerySettings, XtRBoolean, sizeof(Boolean),
     XtOffset(SessionResourcesPtr, querySettings), XtRImmediate,
     (XtPointer)False},
    ...
    {SmNalarmTime, SmCalarmTime, XtRInt, sizeof(int),
     XtOffset(SessionResourcesPtr, alarmTime), XtRImmediate, (XtPointer)10},
    {SmNmemThreshold, SmCmemThreshold, XtRInt, sizeof(int),
     XtOffset(SessionResourcesPtr, memThreshold), XtRImmediate, (XtPointer)100},
    {SmNsessionVersion, SmCsessionVersion, XtRString, sizeof(String),
     XtOffset(SessionResourcesPtr, sessionVersion), XtRImmediate,
     (XtPointer)NULL},
    {SmNdisplayResolution, SmCdisplayResolution, XtRInt, sizeof(int),
     XtOffset(SessionResourcesPtr, displayResolution), XtRImmediate,
     (XtPointer)0},
    {SmNsessionLang, SmCsessionLang, XtRString, sizeof(String),
     XtOffset(SessionResourcesPtr, sessionLang), XtRImmediate, (XtPointer) ""},
    {SmNcontManagement, SmCContManagement, SmRContManagement, sizeof(long),
     XtOffset(SessionResourcesPtr, contManagement), XtRImmediate,
     (XtPointer)(SM_CM_DEFAULT)},
    {SmNwaitClientTimeout, SmCWaitClientTimeout, XtRInt, sizeof(int),
     XtOffset(SessionResourcesPtr, waitClientTimeout), XtRCallProc,
     (XtPointer)_SmWaitClientTimeoutDefault},
    {SmNwaitWmTimeout, SmCWaitWmTimeout, XtRInt, sizeof(int),
     XtOffset(SessionResourcesPtr, waitWmTimeout), XtRCallProc,
     (XtPointer)_SmWaitWmTimeoutDefault},
    {SmNuseMessaging, SmCUseMessaging, XtRBoolean, sizeof(Boolean),
     XtOffset(SessionResourcesPtr, useBMS), XtRImmediate, (XtPointer)True},
    {SmNsaveFontPath, SmCsaveFontPath, XtRBoolean, sizeof(Boolean),
     XtOffset(SessionResourcesPtr, saveFontPath), XtRImmediate,
     (XtPointer)False},
    {SmNsaveYourselfTimeout, SmCsaveYourselfTimeout, XtRInt, sizeof(int),
     XtOffset(SessionResourcesPtr, saveYourselfTimeout), XtRImmediate,
     (XtPointer)5},
    {SmNnumSessionsBackedup, SmCnumSessionsBackedup, XtRInt, sizeof(int),
     XtOffset(SessionResourcesPtr, numSessionsBackedup), XtRImmediate,
     (XtPointer)DEFAULT_NUM_SESSIONS_BACKED_UP},
    {SmNignoreEnvironment, SmCignoreEnvironment, XtRString, sizeof(String),
     XtOffset(SessionResourcesPtr, ignoreEnvironment), XtRImmediate,
     (XtPointer)NULL},
#if defined(USE_XINERAMA) /* JET - Xinerama */
    {SmNxineramaPreferredScreen, SmCxineramaPreferredScreen, XtRInt,
     sizeof(int), XtOffset(SessionResourcesPtr, xineramaPreferredScreen),
...
static XtResource saverResources[] = {
    {SmNcycleTimeout, SmCcycleTimeout, XtRInt, sizeof(int),
     XtOffset(SaverResourcesPtr, cycleTimeout), XtRImmediate, (XtPointer)-1},
    {SmNlockTimeout, SmClockTimeout, XtRInt, sizeof(int),
     XtOffset(SaverResourcesPtr, lockTimeout), XtRImmediate, (XtPointer)-1},
    {SmNsaverTimeout, SmCsaverTimeout, XtRInt, sizeof(int),
     XtOffset(SaverResourcesPtr, saverTimeout), XtRImmediate, (XtPointer)-1},
    {SmNrandom, SmCrandom, XtRBoolean, sizeof(Boolean),
     XtOffset(SaverResourcesPtr, random), XtRImmediate, (XtPointer)False},
    {SmNsaverList, SmCsaverList, XtRString, sizeof(String),
     XtOffset(SaverResourcesPtr, saverList), XtRImmediate, (XtPointer) ""},
 ...
 *
 *************************************<->***********************************/
static void _SmWaitWmTimeoutDefault(Widget widget, int offset,
                                    XrmValue *value) {
        int i;

        smGD.userSetWaitWmTimeout =
            False; /* if we are here, it is not user set */
...
void InitSMGlobals(void) {
        ...
        XtGetApplicationResources(smGD.topLevelWid, (XtPointer)&smRes,
                                  sessionResources, XtNumber(sessionResources),
                                  NULL, 0);
        ...
        XtGetSubresources(smGD.topLevelWid, (XtPointer)&smSaverRes,
                          smGD.SmNextension, smGD.SmCextension, saverResources,
...
#define SMBOUND(A) (A < 0 ? 0 : A)
        ...
        XChangeProperty(smGD.display, RootWindow(smGD.display, 0), XaVsmInfo,
                        XaVsmInfo, 32, PropModeReplace,
 ...
 *
 *************************************<->***********************************/
int SetRestorePath(unsigned int argc, char *argv[]) {
        Display *tmpDisplay;
        int i;
        ...
        for (i = 0; i < argc; i++) {
                if (!strcmp(argv[i], "-display")) {
                         ...
                         */
                        if (getenv("DISPLAY") == 0) {
                                snprintf(tmpDisplayName, MAXPATHLEN,
                ...
                if (!strcmp(argv[i], "-session")) {
                        ...
                        if (i >= argc) {
                                ...
                                pch = strdup((char *)GETMESSAGE(
                                    40, 15,
                                    " No session name was provided for the "
        ...
        if (tmpDisplay == NULL) {
                PrintError(DtError,
                           GETMESSAGE(4, 1, "Invalid display name - exiting."));
                SM_EXIT(-1);
        }

        if (session_option) {
                if (!InitializeSpecificSession(session_option, tmpDisplay, argc,
                                               argv))
...
void FixPath(char *the1stPath) {
        char *tempPath;
        char *pathList =
            (char *)XtMalloc(strlen(SM_SYSTEM_PATH) +
 ...
 *
 *************************************<->***********************************/
int SetSavePath(Boolean saveToHome, int mode) {
        struct stat buf;
        int status;
        ...
        if ((mode == DtSM_HOME_STATE) && (saveToHome == False) &&
            (smSettings.startState == DtSM_HOME_STATE)) {
                ...
                if (status != -1) {
                        /*
                         * The "current" directory exists and must be moved to
                         * its
                         * ".old" version.  But first, if the ".old" version
                         * exists, it must be moved to a temporary directory.
                         * This temporary directory will eventually be pruned -
                         * when a user next runs a Current session and saves the
                         * session.
                         */
                        strcpy(savedDir, smGD.clientPath);
                        sprintf(smGD.etcPath, "%s.%s", smGD.clientPath,
                                SM_OLD_EXTENSION);
                        ...
                        if (status == 0) {
                                ...
                                if ((tfd = mkstemp(tmpName)) == -1) {
                                        ...
                                        MoveDirectory(smGD.etcPath, tmpName,
                                                      False);
                ...
                if (status == -1) {
                         ...
                         * be moved to a temporary directory that will be
                         * removed when the session directories are pruned -
 ...
 *
 *************************************<->***********************************/
int SetFontSavePath(char *langPtr) {
        struct stat buf;
        int status;
        char *sessionSaved;
        ...
        snprintf(smGD.fontPath, MAXPATHLEN, "%s/%s", smGD.savePath,
                 sessionSaved);
        ...
        if (clientFork == 0) {
                ...
                if (execStatus != 0) {
                        tmpString = ((char *)GETMESSAGE(
                            4, 4,
                            "Unable to remove session directory.  Make sure "
                            "write permissions exist on $HOME/.dt directory. "
                            " Invalid session files will not be removed."));
 ...
 *
 *************************************<->***********************************/
void MoveDirectory(char *pathFrom, char *pathTo, Boolean removeDestDir) {
        struct stat buf;
        pid_t clientFork;
...
void InitNlsStrings(void) {
         ...
         */
        smNLS.cantMallocErrorString = strdup(((char *)GETMESSAGE(
            4, 5, "Unable to malloc memory for operation.")));

        /*
         * Error message when dtsession cant lock the display
         */
        smNLS.cantLockErrorString = strdup(
            ((char *)GETMESSAGE(4, 6,
                                "Unable to lock display.  Another application "
                                "may have the pointer or keyboard grabbed.")));

        /*
         * Error message when dtsession cant open files for reading or writing
         */
        smNLS.cantOpenFileString = strdup(((char *)GETMESSAGE(
            4, 7,
            "Unable to open session file.  No clients will be restarted.")));

        /*
         * Error message when dtsession cant fork client process
         */
        smNLS.cantForkClientString = strdup(
            ((char *)GETMESSAGE(4, 8, "Unable to fork client process.")));

        /*
         * Error message when dtsession cant create dt directories
         */
        smNLS.cantCreateDirsString = strdup(
            ((char *)GETMESSAGE(4, 9,
                                "Unable to create DT directories.  Check "
                                "permissions on home directory.")));

        /*
         * Error message when trying to lock display on trusted system
         */
        smNLS.trustedSystemErrorString = strdup(((char *)GETMESSAGE(
            4, 10, "Unable to lock display due to security restrictions")));
}
...
static void TrimErrorlog(void) {
        ...
        if (len > MAXPATHLEN)
                savePath = SM_REALLOC(savePath, len);
        ...
        if (len + strlen(DtOLDER_ERRORLOG_FILE) > MAXPATHLEN)
                checkPath1 =
                    SM_REALLOC(savePath, len + strlen(DtOLDER_ERRORLOG_FILE));
        sprintf(checkPath1, "%s/%s", savePath, DtOLDER_ERRORLOG_FILE);

        if (len + strlen(DtOLD_ERRORLOG_FILE) > MAXPATHLEN)
                checkPath2 =
                    SM_REALLOC(savePath, len + strlen(DtOLD_ERRORLOG_FILE));
        ...
        if (len + strlen(DtERRORLOG_FILE) > MAXPATHLEN)
                checkPath1 =
                    SM_REALLOC(savePath, len + strlen(DtERRORLOG_FILE));
...
#define CM_NONE_STR (unsigned char *)"none"
        ...
        if (*pch && (_DtNextToken(pch, &len, &pchNext)) && (*pch == '-')) {
                cval = SM_CM_ALL;
                fHit = True;
        } else {
                cval = SM_CM_NONE;
...
Boolean _DtWmStringsAreEqual(unsigned char *pch1, unsigned char *pch2,
        ...
        while (len && ((chlen1 = mbtowc(&wch1, (char *)pch1, 2)) > 0) &&
 ...
 *
 *************************************<->***********************************/
static void InitializeGenericSession(Display *tmpDisplay) {
        struct stat buf;
        ...
        if (session_needed) {
                ...
                if (status == 0) {
                        /*
                         * sessions/current/dt.settings exist, restore to
                         * 'current' session
                         */
                        strcpy(smGD.resourcePath, smGD.savePath);
                        strcat(smGD.resourcePath, "/");
        ...
        if (session_needed) {
                ...
                if (status == 0) {
                        /*
                         * sessions/home/dt.settings exist, restore to 'home'
                         * session
                         */
                        strcpy(smGD.resourcePath, smGD.savePath);
                        strcat(smGD.resourcePath, "/");
 ...
 *
 *************************************<->***********************************/
static Boolean InternSessionDir(char *session_option, Display *disp) {
        char *pch;
        ...
        XChangeProperty(disp, RootWindow(disp, 0), XaSmRestoreDir, XA_STRING, 8,
                        PropModeReplace, (unsigned char *)pch, strlen(pch));
 ...
 *
 *************************************<->***********************************/
static Boolean InitializeSpecificSession(char *session_option, Display *disp,
                                         unsigned int argc, char **argv) {
        ...
        if (!strcmp(session_option, SM_HOME_DIRECTORY)) {
                ...
                if (smGD.clientPath[0] == '\0') {
                         ...
                         */
                        (void)sprintf(session_dir, "%s/%s", smGD.savePath,
                                      SM_CURRENT_DIRECTORY);
                        (void)sprintf(alt_dir, "%s/%s", smGD.savePath,
                                      SM_HOME_DIRECTORY);
                ...
                if (smGD.clientPath[0] == '\0') {
                        ...
                        (void)sprintf(alt_dir, "%s/%s/%s/%s", home,
                ...
                if ((stat(session_dir, &buf)) != 0) {
                         ...
                         * change to exit.
                         */
                        if (!ConfirmSessionCreation(CURRENT_SESSION, argc,
                                                    argv))
                                SM_EXIT(0);
                }
                ...
                if (smGD.clientPath[0] == '\0') {
                        ...
                        if (!SetAlternateSession(session_dir, alt_dir, False)) {
                                ...
                                snprintf(alt_dir, len, "%s/%s/%s/%s", home,
                                         DtPERSONAL_CONFIG_DIRECTORY,
                                         DtSM_SESSION_DIRECTORY,
                                         SM_HOME_DIRECTORY);

                                if (!SetAlternateSession(session_dir, alt_dir,
                                                         True))
                                        SetSysDefaults();
                        }
                }
 ...
 *
 *************************************<->***********************************/
static void InitializePaths(char *session_name, Display *disp) {
        ...
        (void)sprintf(smGD.settingPath, "%s/%s/%s", smGD.savePath, session_name,
                      SM_SETTING_FILE);

        (void)sprintf(smGD.resourcePath, "%s/%s/%s", smGD.savePath,
                      session_name, SM_RESOURCE_FILE);

        smGD.clientPath[0] = '\0';

        (void)sprintf(db_file, "%s/%s/%s", smGD.savePath, session_name,
                      SM_CLIENT_FILE2);
...
static Boolean SetAlternateSession(char *session_dir, char *alt_dir,
        ...
        if ((stat(session_dir, &buf)) != 0) {
                ...
                (void)sprintf(db_file1, "%s/%s", alt_dir, SM_CLIENT_FILE2);
                (void)sprintf(db_file2, "%s/%s", alt_dir, SM_CLIENT_FILE);
                ...
                if (tmp_dir) {
                        ...
                        tmp = XtMalloc(strlen(alt_dir) + strlen(session_dir) +
                                       10);
                        if (!tmp)
                                return (False);

                        (void)sprintf(tmp, "cp -r %s %s", alt_dir, session_dir);
                         ...
                         */
                        if (tmp_dir == db_file1)
                                sprintf(smGD.clientPath, "%s/%s", session_dir,
                                        SM_CLIENT_FILE2);
                        else
                                sprintf(smGD.clientPath, "%s/%s", session_dir,
                                        SM_CLIENT_FILE);
