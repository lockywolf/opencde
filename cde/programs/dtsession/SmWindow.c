...
static WindowInfo GetTopLevelWindowInfo(Window window);
static void WaitForCommand(Window window);
...
static WindowInfo GetTopLevelWindowInfo(Window window) {
        ...
        /*
         * If WM_STATE could not be interned at the beginning - the window
         * manager may have been slow in coming up.  Try it again now.
         */
        if (XaWmState == None) {
                XaWmState = XInternAtom(smGD.display, _XA_WM_STATE, True);
        }

        XGetWindowProperty(smGD.display, window, XaWmState, 0L, (long)BUFSIZ,
                           False, XaWmState, &actualType, &actualFormat,
        ...
        if (actualType == XaWmState) {
                ...
                if (XQueryTree(smGD.display, window, &root, &parent, &children,
                               &nchildren) != 0) {
                        if (nchildren > 0) {
                                i = 0;
                                while (nchildren--) {
                                        retInfo = GetTopLevelWindowInfo(
                                            children[i++]);
                                        if (retInfo.wid != 0) {
                ...
                if ((isThere = XtAppPending(smGD.appCon)) != 0) {
                        if (isThere & XtIMXEvent) {
                                ...
                                if (event.type == PropertyNotify &&
                                    pEvent->window == window &&
                                    pEvent->atom == XA_WM_COMMAND) {
                                        commandUpdated = True;
                                }
                        }
                        if (commandTimeout == False) {
                                XtAppProcessEvent(smGD.appCon,
 ...
 *
 *************************************<->***********************************/
int SaveYourself(WindowInfo windowInfo) {
         ...
         */
        if (XGetWMProtocols(smGD.display, windowInfo.wid, &protoRet, &nitems) ==
            0) {
                /*
 ...
 *
 *************************************<->***********************************/
int GetTopLevelWindows(int screen, WindowInfo **topList,
         ...
         */
        if (XQueryTree(smGD.display, RootWindow(smGD.display, screen),
                       &rootWindow, &parentWindow, &childList,
                       topListLength) == 0) {
                PrintError(DtError, GETMESSAGE(20, 2,
                                               "Invalid root window.  Can not "
         ...
         */
        if (_DtGetEmbeddedClients(smGD.display,
                                  RootWindow(smGD.display, screen),
                                  &embeddedList, &numEmbedded) != Success) {
                numEmbedded = 0;
        }

        if (*topListLength)
                *topList = (WindowInfo *)SM_MALLOC(
                    sizeof(WindowInfo) * (*topListLength + numEmbedded));
        ...
        for (i = 0; i < *topListLength; ++i, tmpChild++) {
                ...
                if (!topLevelWindowInfo.wid) {
                        ...
                        /*
                         * Assume if you can't find a state that it is "don't
                         * care" this could be a faulty assumption CHECK IT OUT
                         */
 ...
 *
 *************************************<->***********************************/
static void WaitTimeout(XtPointer client_data, XtIntervalId *id) {
        ...
        if (success && winName) {
                tmpString = GETMESSAGE(
                    20, 4,
                    "Session restoration information not updated for "
                    "client %s.  Invalid information may be saved.");
