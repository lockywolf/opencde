...
static XtResource resources[] = {
    ...
    {"colorUse", DtRColorUse, DtRColorUse, sizeof(int),
     XtOffset(AppdataPtr, ColorUse), XmRString, "DEFAULT"},

    {"shadowPixmaps", DtRShadowPixmaps, DtRShadowPixmaps, sizeof(int),
     XtOffset(AppdataPtr, ShadowPixmaps), XmRString, "DEFAULT"},

    {"foregroundColor", DtRForegroundColor, DtRForegroundColor, sizeof(int),
     XtOffset(AppdataPtr, ForegroundColor), XmRString, "DYNAMIC"},

    {"dynamicColor", "DynamicColor", XmRBoolean, sizeof(Boolean),
     XtOffset(AppdataPtr, DynamicColor), XmRImmediate, (XtPointer)True},

    {"writeXrdbColors", "WriteXrdbColors", XmRBoolean, sizeof(Boolean),
     XtOffset(AppdataPtr, WriteXrdbColors), XmRImmediate, (XtPointer)True},

    {"colorPalette", "ColorPalette", XmRString, sizeof(char *),
     XtOffset(AppdataPtr, ColorPalette), XmRImmediate, (XtPointer) "DEFAULT"},

    {"monochromePalette", "MonochromePalette", XmRString, sizeof(char *),
     XtOffset(AppdataPtr, MonochromePalette), XmRImmediate,
     (XtPointer) "Black.dp"},
};
...
static Boolean AllocateColors(Display *dpy);
static char *convert_pixel_set(int typeOfMonitor, ColorSet *color);
...
static int FindMaximumDefault(Display *dpy, int screen_number);
static int FindNumOfPixels(Display *dpy, int screen_number);
static int GetNumOfPixels(int screen_number);
static void MatchAndStore(Display *dpy, int screen_number,
                          unsigned long *pixels);
static Boolean AllocReadWrite(Display *dpy, int screen_number, int numOfPixels);
static void AllocReadOnly(Display *dpy, int screen_number);
static void CvtStringToColorUse(XrmValue *args, Cardinal *num_args,
                                XrmValue *from_val, XrmValue *to_val);
static void CvtStringToForegroundColor(XrmValue *args, Cardinal *num_args,
                                       XrmValue *from_val, XrmValue *to_val);
static void CvtStringToShadowPixmaps(XrmValue *args, Cardinal *num_args,
                                     XrmValue *from_val, XrmValue *to_val);
static Boolean _DtWmStringsAreEqual(register char *in_str,
                                    register char *test_str);
static void SetDefaults(Display *dpy, int screen_number);

static void _DtCacheProperties(Display *dpy, Window Win);
/********    End Static Function Declarations    ********/

#define MSG1                                                                   \
        ((char *)GETMESSAGE(26, 1,                                             \
                            "Color Server Warning: couldn't get ownership of " \
                            "the selection: "))
#define MSG2                                                                   \
        ((char *)GETMESSAGE(                                                   \
            26, 2,                                                             \
...
#define MSG3                                                                   \
        ((char *)GETMESSAGE(26, 4,                                             \
                            "Warning, found more pixels then are available."))
 ...
 *
 **********************************************************************/
int InitializeDtcolor(Display *dpy, short sessionType) {
        ...
        if (status == 0) {
                ...
                for (screen_number = 0; screen_number != colorSrv.NumOfScreens;
                     screen_number++) {
        ...
        if (colorSrv.TypeOfMonitor[0] != XmCO_BLACK_WHITE) {
                int chipnum =
                    colorSrv.TypeOfMonitor[0] == XmCO_HIGH_COLOR ? 4 : 1;
                sprintf(
                    xrdb_string,
        ...
        else /* XmCO_BLACK_WHITE */
        {
                if (strcmp(colorSrv.pCurrentPalette[0]->name, W_O_B) == 0 ||
                    strcmp(colorSrv.pCurrentPalette[0]->name, B_ONLY) == 0) {
                        sprintf(xrdb_string, "*background: "
                                             "#000000000000\n*foreground: "
                                             "#FFFFFFFFFFFF\n");
                } else {
                        sprintf(xrdb_string, "*background: "
...
static void _DtCacheProperties(Display *dpy, Window win) {
        ...
        if (palette->converted == NULL) {
                palette->converted =
                    convert_pixel_set(typeOfMonitor, palette->color);
                palette->converted_len = strlen(palette->converted);
        }

        *(palette->converted + palette->converted_len) =
            XmPIXEL_SET_PROP_VERSION;
             ...
             screen_number++) {
                ...
                if (numOfPixels != 0) /* Not XmCO_BLACK_WHITE */
                {
                        if (colorSrv.DynamicColor[screen_number] == True) {
                                /* go allocate Read/Write cells for the color
                                 * server */
                                if (!AllocReadWrite(dpy, screen_number,
                                                    numOfPixels))
                                        return (False);
                        } else
                                /* go allocate Read Only cells for the color
                                 * server */
...
#define WhiteColorSet(dpy, scr, xcolor)                                        \
        ...
        (xcolor).red = (xcolor).green = (xcolor).blue = 65535;
                        ...
                        /* note: color[0] = secondary, color[1] = primary (as of
                         * 8/8/90) */
                        if (!(strcmp(
                                colorSrv.pCurrentPalette[screen_number]->name,
                                W_O_B))) {
                                WhiteColorSet(dpy, screen_number,
                                              palette->color[0].bg);
                                BlackColorSet(dpy, screen_number,
                                              palette->color[0].fg);
                                BlackColorSet(dpy, screen_number,
                                              palette->color[0].ts);
                                BlackColorSet(dpy, screen_number,
...
int CheckMonitor(Display *dpy) {
        ...
        for (screen_number = 0; screen_number != colorSrv.NumOfScreens;
             screen_number++) {
                ...
                n++;
                shell[screen_number] =
                    XtAppCreateShell(screenStr, XmSCOLOR_SRV_NAME,
                                     applicationShellWidgetClass, dpy, args, n);
                ...
                result = XtOwnSelection(
                    shell[screen_number], colorSrv.XA_CUSTOMIZE[screen_number],
                    CurrentTime, convert_selection, lose_selection, NULL);
                ...
                if (result == False) {
                         ...
                         */
                        tmpStr =
                            (char *)SRV_MALLOC(strlen(MSG1) + 25 + 5 + 1 + 1);
                        sprintf(tmpStr, "%s colorSrv.XA_CUSTOMIZE[%d].\n", MSG1,
                                screen_number);
                        _DtSimpleError(XmSCOLOR_SRV_NAME, DtWarning, NULL,
                                       tmpStr, NULL);
                        SRV_FREE(tmpStr);
                        return (-1);
                }
                ...
                XtGetSubresources(mainShell, &pColorSrvRsrc, screenStr,
                                  screenStr, resources, XtNumber(resources),
                                  NULL, 0);
                ...
                if (colorSrv.TypeOfMonitor[screen_number] != XmCO_BLACK_WHITE) {
                        colorSrv.pCurrentPalette[screen_number] =
                            (struct _palette *)GetPaletteDefinition(
                                dpy, screen_number, pColorSrvRsrc.ColorPalette);
                } else {
                        /* Allocate space for new palette. */
                        colorSrv.pCurrentPalette[screen_number] =
                            (struct _palette *)SRV_MALLOC(
                                sizeof(struct _palette) + 1);
                        ...
                        strcpy(colorSrv.pCurrentPalette[screen_number]->name,
                               (char *)tmpPalette);
                        colorSrv.pCurrentPalette[screen_number]->converted =
                            NULL;
                }

                if (colorSrv.pCurrentPalette[screen_number] ==
                    (struct _palette *)NULL) {
                        return (-1);
                }

                /* write out the color or monochrome palette resource for the
                 * screen */
...
static void lose_selection(Widget w, Atom *selection) {
        ...
        sprintf(tmpStr, "%s%s%s", tmpStr2,
                (tmpStr3 == NULL) ? "(null)" : tmpStr3, MSG2a);
...
static int FindMaximumDefault(Display *dpy, int screen_number) {
        ...
        if ((colorSrv.TypeOfMonitor[screen_number] == XmCO_HIGH_COLOR &&
             numOfPixelsLeft >= 40) ||
            ...
            (colorSrv.TypeOfMonitor[screen_number] == XmCO_LOW_COLOR &&
             numOfPixelsLeft >= 10)) {
                          ...
                          next highest XmCO_MEDIUM_COLOR */
                {
                        colorSrv.TypeOfMonitor[screen_number] =
                            XmCO_MEDIUM_COLOR;
                        colorSrv.pCurrentPalette[screen_number]->num_of_colors =
                            4;
                }
        }

        /* need to do an if instead of an else because TypeOfMonitor can be
           reset in the else if above */
...
static int FindNumOfPixels(Display *dpy, int screen_number) {
        ...
        /* get the number of iterations to be used .. the number of plane in
           this screen */
        iterations = XDisplayPlanes(dpy, screen_number);

        /* Allocate enough space to store the pixels.  */
        pixels =
            (unsigned long *)SRV_MALLOC(num_of_pixels * sizeof(unsigned long));
        ...
        for (i = 0; i < iterations; i++) {
                status = XAllocColorCells(dpy, colormap, (Boolean)0,
                                          &plane_mask, 0, pixels, count);

                countdown = countdown / 2;
                if (status == False) {
                        count = base + countdown;
                } else {
                        XFreeColors(dpy, colormap, pixels, count,
                                    (unsigned long)0);
        ...
        status = XAllocColorCells(dpy, colormap, (Boolean)0, &plane_mask, 0,
                                  pixels, count);
        if (status == False)
                count--;
        else
        ...
        } else /* non Black and White monitor */
        {
                if (colorSrv.UsePixmaps[screen_number] == FALSE) {
                        if (colorSrv.FgColor[screen_number] == DYNAMIC) {
                                return (colorSrv.pCurrentPalette[screen_number]
                                            ->num_of_colors *
                                        5);
                        } else /*FgColor == BLACK or WHITE ... bg, ts, bs, & sc
                                  used */
                        {
                                return (colorSrv.pCurrentPalette[screen_number]
                                            ->num_of_colors *
                ...
                {
                        if (colorSrv.FgColor[screen_number] ==
                            DYNAMIC) /* fg, bg, & sc used */
                        {
                                return (colorSrv.pCurrentPalette[screen_number]
                                            ->num_of_colors *
                                        3);
...
static void MatchAndStore(Display *dpy, int screen_number,
        ...
        if (count > 0) {
                XStoreColors(dpy, DefaultColormap(dpy, screen_number), xcolor,
                             count);
        }
...
**
************************************************************************/
static Boolean AllocReadWrite(Display *dpy, int screen_number,
                              int numOfPixels) {
        char *tmpStr;
        unsigned long *pixels;
        unsigned long plane_mask;
        ...
        /* Allocate enough space to store the pixels. */
        pixels =
            (unsigned long *)SRV_MALLOC(numOfPixels * sizeof(unsigned long));

        /* Now actually allocate R/W pixels */
        status =
            XAllocColorCells(dpy, DefaultColormap(dpy, screen_number),
        ...
        if (status == False) {
                ...
                if (numOfPixels == 0) {
                        ...
                        if (colorSrv.TypeOfMonitor[screen_number] !=
                            XmCO_BLACK_WHITE) {
                                /* Allocate enough space to store the pixels. */
                                pixels = (unsigned long *)SRV_MALLOC(
                                    numOfPixels * sizeof(unsigned long));

                                /* Now actually allocate R/W pixels */
                                status = XAllocColorCells(
                                    dpy, DefaultColormap(dpy, screen_number),
                                    (Boolean)0, &plane_mask, 0, pixels,
                                    numOfPixels);

                                if (status == False) {
                                        SRV_FREE((char *)pixels);
                                        _DtSimpleError(
                                            XmSCOLOR_SRV_NAME, DtWarning, NULL,
                                            ((char *)GETMESSAGE(
                                                26, 5,
                                                "Warning, can't allocate "
                                                "enough pixels.\n")),
                                            NULL);
                                        return (False);
                                }
...
static void AllocReadOnly(Display *dpy, int screen_number) {
        ...
        for (i = 0; i < colorSrv.pCurrentPalette[screen_number]->num_of_colors;
             i++) {
                XAllocColor(
                    dpy, DefaultColormap(dpy, screen_number),
                ...
                } else /* colorSrv.UsePixmaps[screen_number] == True */
                {
                        colorSrv.pCurrentPalette[screen_number]
                            ->color[i]
                            .ts.pixel = WhitePixel(dpy, screen_number);
                        colorSrv.pCurrentPalette[screen_number]
                            ->color[i]
                            .bs.pixel = BlackPixel(dpy, screen_number);
                }
                ...
                if (colorSrv.FgColor[screen_number] == DYNAMIC)
                        ...
                        colorSrv.pCurrentPalette[screen_number]
                            ->color[i]
                            .fg.pixel = BlackPixel(dpy, screen_number);

                else
                        colorSrv.pCurrentPalette[screen_number]
                            ->color[i]
                            .fg.pixel = WhitePixel(dpy, screen_number);
        }
}
...
static void SetDefaults(Display *dpy, int screen_number) {
        ...
        /* Initialize colorSrv data for this screen with specified resource
         * values */
        ...
        if (strcmp(pColorSrvRsrc.ColorPalette, "DEFAULT") == 0) {
                if ((visual->class == GrayScale) ||
                    (visual->class == StaticGray)) {
                        ...
                        strcpy(pColorSrvRsrc.ColorPalette,
        ...
        } else if (numPlanes == 4) /* 4 planes */
        {
                switch (pColorSrvRsrc.ColorUse) {
                case XmCO_MEDIUM_COLOR:
                        /* for 4 planes ColorUse = Med_color shadowPixmaps have
                         * to be True */
        ...
        } else if (numPlanes == 5) /* 5 planes */
        {
                switch (pColorSrvRsrc.ColorUse) {
                case XmCO_HIGH_COLOR:
                        /* for 5 planes ColorUse = hi_color shadowPixmaps have
                         * to be True */
                        pColorSrvRsrc.ShadowPixmaps = -1;
