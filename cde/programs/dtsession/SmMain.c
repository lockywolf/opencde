...
#include <sys/proc.h>
        ...
        if (smGD.appCon == 0) {
                ...
                smGD.display =
                    XtOpenDisplay(smGD.appCon, NULL, argv[0], SM_RESOURCE_CLASS,
                                  NULL, 0, &argc, argv);
        }

        srvDisplay = XtOpenDisplay(smGD.appCon, NULL, argv[0],
        ...
        if (!_DtGetLock(smGD.display, SM_RUNNING_LOCK)) {
                PrintError(
                    ...
                    GETMESSAGE(
                        2, 2,
                        "Another dtsession is currently running - exiting."));
                SM_EXIT(-1);
        }
        ...
        if (smGD.DtXineramaInfo == NULL) { /* No xinerama, how... sad. */
                ...
                fprintf(stderr,
                        "### JET SmMain: Xinerama available, scrns = %d\n",
         ...
         */
        XtAddEventHandler(smGD.topLevelWid, PropertyChangeMask, True,
                          ProcessEvent, NULL);
        ...
        while (1) {
                ...
                if (next.type != 0) {
                        ...
                        if (next.type == smGD.ssEventType) {
                                /*
                                 * We should simply be calling XtDispatchEvent()
                                 * but the toolkit doesn't seem to know how to
                                 * dispatch a run-time generated event type.
                                 */
                                ProcessEvent(smGD.topLevelWid, NULL, &next,
                                             NULL);
                        } else {
...
static void StopAll(int i) { ImmediateExit(-1, 0, True); }
         ...
         *  Description:
         *  -----------
         *  Register with X11 screen saver server extension for screen saver
         *events.
         *
         *  Inputs:
         *  ------
...
static int RegisterX11ScreenSaver(Display *display, int *pssEventType) {
        ...
        if (XScreenSaverQueryExtension(display, pssEventType, &ssErrorBase) &&
            ...
            majorVersion == 1) {
                /*
                 * Server supports requested version of X11 screen saver
                 * extension.
                 */
                ...
                if (!XScreenSaverGetRegistered(display, screen, &xid, &type)) {
                        /*
                         * No other clients registered with this server so
                         * register this one.
                         */
                        XScreenSaverRegister(display, screen,
                                             XtWindow(smGD.topLevelWid),
                                             XA_WINDOW);
                        result = 0;
                }
                ...
                if (result == 0) {
                        ...
                        /* Even though OverrideRedirect is the default attribute
                         * in this check, lets make sure it's set, and make sure
                         * only the CWOverrideRedirect attr is being looked at.
                         * The rest is just random garbage anyway, since it's
                         * allocated from the stack.
                         */
                        attr.override_redirect = True;
                        XScreenSaverSetAttributes(
                            display, root, 0, 0, 1, 1, 0, CopyFromParent,
