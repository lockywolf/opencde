...
Boolean InitXSMP() {
        ...
        networkIds =
            IceComposeNetworkIdList(smXSMP.numTransports, smXSMP.listenObjs);
        if (!networkIds)
                PostXSMPFailureDialog(XSMP_FAILURE_ICE_COMPOSE_IDS, False);

        env = (char *)XtMalloc(strlen(SM_SESSION_MANAGER) + strlen(networkIds) +
                               2);
        if (!env) {
                free(networkIds);
...
Status NewClientProc(SmsConn smsConn, SmPointer managerData,
        ...
        if (!newClient) {
                ...
                if (str) {
                        if ((*failureReasonRet =
                                 (char *)XtMalloc(strlen(str) + 1)) != NULL)
...
void NewConnectionXtProc(XtPointer client_data, int *source, XtInputId *id) {
        ...
        if (!ice_conn) {
                ...
                message = strdup(
                    (char *)GETMESSAGE(40, 20, "IceAcceptConnection failed."));
...
Status RegisterClientProc(SmsConn smsConn, SmPointer managerData,
        ...
        if (!previousId) {
                ...
                for (pDbRec = smXSMP.xsmpDbList;
                     pDbRec != NULL && found == False; pDbRec = pDbRec->next) {
...
void InteractRequestProc(SmsConn smsConn, SmPointer managerData,
        ...
        if (dialogType == SmDialogError)
                ...
                (void)printf(
                    "\tSMlib Error: should have checked for bad value\n");
#endif /* DEBUG */
        ...
        if (smXSMP.saveState.interactCount == 0) {
                ...
                if (!SendGetWsmClientsMessage()) {
                        ...
                        pch = strdup((char *)GETMESSAGE(
                            40, 17,
                            "An attempt to get a client list from the 'Window "
                            "Manager' failed."));
                        if (pch) {
        ...
        if (cancelShutdown && smXSMP.saveState.shutdown == True &&
            (smXSMP.saveState.interactStyle == SmInteractStyleErrors ||
             (smXSMP.saveState.interactStyle == SmInteractStyleAny))) {
                ...
                pch = strdup((char *)GETMESSAGE(40, 22,
                                                "A session shutdown was "
                                                "cancelled by the application "
                                                "'%s'."));
                if (pch) {
                        DtMsgLogMessage(
                            smGD.programName, DtMsgLogInformation, pch,
...
void SaveYourselfReqProc(SmsConn smsConn, SmPointer managerData, int saveType,
        ...
        for (pClientRec = connectedList; pClientRec != NULL;
             pClientRec = pClientRec->next) {
                ...
                if (pClientRec->active) {
                        SmsSaveYourself(pClientRec->smConn, saveType, shutdown,
                                        interactStyle, fast);
#ifdef DEBUG
                        (void)printf("\tSent saveyourself to %d\n",
                                     pClientRec->smConn);
...
void CloseConnectionProc(SmsConn smsConn, SmPointer managerData, int count,
        ...
        if (count > 0) {
                PostReasonsDialog(GetArrayPropertyValue(tmp, SmProgram), count,
                                  reasonMsgs, True);
        ...
        if (!smXSMP.saveState.shutdown) {
                ...
                for (pClientRec = connectedList; pClientRec != NULL;
                     pClientRec = pClientRec->next) {
                        ...
                        if (!GetCardPropertyValue(
                                pClientRec, SmRestartStyleHint, &restartHint))
