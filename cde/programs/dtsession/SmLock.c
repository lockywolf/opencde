...
static Boolean CanReAuthenticate(char *name, uid_t uid, char *passwd,
        ...
        *pwent = (name == NULL) ? getpwuid(uid) : getpwnam(name);
        ...
        fprintf(stderr, "CanReAuthenticate(): %s %s %s\n",
                (*pwent) ? "PWENT" : "NULL", (*spent) ? "SPENT" : "NULL",
                (name) ? name : "NULL");
#endif
...
#ifdef JET_AUTHDEBUG
                fprintf(stderr, "CanReAuthenticate(): (*pwent)->pw_passwd == "
                                "NULL - FALSE\n");
#endif
...
void LockDisplay(Boolean lockNow) {
        ...
        timerId = lockTimeId = lockDelayId = cycleId = flash_id =
            (XtIntervalId)0;
            ...
            ((smDD.lockDialog == NULL) && (smGD.coverScreen == False))) {
                ...
                if (smGD.coverScreen == True) {
                        for (i = (smGD.numSavedScreens - 1); i >= 0; i--) {
                                if (i == screenNum) {
                                        smDD.coverDialog[i] =
                                            CreateCoverDialog(i, True);
                                        parent = smDD.coverDrawing[i];
                                } else {
                                        smDD.coverDialog[i] =
                                            CreateCoverDialog(i, False);
                                }
                        }
                        smDD.lockCoverDialog =
                ...
                if ((smDD.lockDialog == NULL) && (smGD.coverScreen == False)) {
                        ...
                        i++;
                        XtSetArg(uiArgs[i], XmNbottomShadowColor,
                                 &(xcolors[1]));
                        i++;
         ...
         * is lost. Ah X.
         */
        XtAddEventHandler(grabWidget, VisibilityChangeMask, False,
                          FinishLocking, NULL);

        XtManageChild(lockDlg);

        if (smGD.coverScreen == True) {
                flash_id = XtAppAddTimeOut(smGD.appCon, 1000, BlinkCaret,
                                           smDD.indLabel[1]);
        } else {
                flash_id = XtAppAddTimeOut(smGD.appCon, 1000, BlinkCaret,
                                           smDD.indLabel[0]);
        ...
        if (lockDelay > 0) {
                 ...
                 */
                lockDelayId = XtAppAddTimeOut(smGD.appCon, lockDelay * 1000,
        ...
        if (smGD.coverScreen == True && smSaverRes.cycleTimeout > 0) {
                 ...
                 */
                cycleId =
                    XtAppAddTimeOut(smGD.appCon, smSaverRes.cycleTimeout * 1000,
                                    CycleSaver, NULL);
        }
        ...
        lockTimeId =
            XtAppAddTimeOut(smGD.appCon, 90000, LockAttemptFailed, lockDlg);
        return;
}
...
static void FinishLocking(Widget wid, XtPointer ptr, XEvent *ev, Boolean *bl) {
        ...
        XtRemoveEventHandler(wid, VisibilityChangeMask, False, FinishLocking,
                             NULL);
...

        {
                pointerGrabbed =
                    (XtGrabPointer(
                         grabWidget, False, ButtonPressMask | PointerMotionMask,
        ...
        if ((pointerGrabbed != True) || (kbdGrabbed != True)) {
                 ...
                 */
                hostList = XListHosts(smGD.display, &hostListLen,
                                      (Bool *)&hostListActive);
...
void CreateLockCursor(void) {
        ...
        if (InitCursorInfo() == False) {
                ...
                mask = XCreateBitmapFromData(
                    smGD.display, XRootWindow(smGD.display, smGD.screen),
                    (char *)lock_s_m_bm_bits, lock_s_m_bm_width,
                    lock_s_m_bm_height);
                ...
                smGD.padlockCursor = XCreatePixmapCursor(
                    smGD.display, source, mask, &(xcolors[0]), &(xcolors[1]),
                    lock_s_hot_x, lock_s_hot_y);
                ...
                mask = XCreateBitmapFromData(
                    smGD.display, XRootWindow(smGD.display, smGD.screen),
                    (char *)lock_m_m_bm_bits, lock_m_m_bm_width,
                    lock_m_m_bm_height);
        ...
        smGD.blankCursor = XCreatePixmapCursor(
            smGD.display, source, source, &(xcolors[0]), &(xcolors[0]), 0, 0);
        XFreePixmap(smGD.display, source);
}
...
static void EventDetected(Widget w, XtPointer client_data, XEvent *ev,
        ...
        if (smGD.lockedState == LOCKED) {
                if (lockDlgVisible == False) {
                        /*
                         * Anytime input is received - show the passwd dialog
                         * and discard event. This is so a keypress event that
                         * causes the passwd dialog to appear will not be used
                         * in the password. CMVC 612.
                         */
        ...
        if (event->type != KeyPress)
                ...
                /*
                 * If XLookupString() returns anything (which it won't in the
                 * case of, say, pressing the shift key or an arrow key),
                 * process it.
                 */
#ifdef USE_HP_SPECIFIC_XLIB
...
#define BUF_ALLOC_LEN 64
        ...
        /*
         * password rules:
         *	- If pw_length >  MAX_PASSWORD_LENGTH, we've gone over the limit
         *and won't accept any more.
        ...
        /* extend buffer by BUF_ALLOC_LEN bytes if needed*/
                ...
                if (pw_length == pw_buf_length) {
                        tmpptr =
                            SM_REALLOC(passwd, pw_buf_length + BUF_ALLOC_LEN);
                        if (!tmpptr) {
...
static void UnlockDisplay(Boolean pointerGrabbed, Boolean kbdGrabbed) {
        ...
        XtRemoveEventHandler(
            grabWidget, (KeyPressMask | ButtonPressMask | PointerMotionMask),
            False, EventDetected, NULL);
...
static void LockAttemptFailed(XtPointer ptr, XtIntervalId *invId) {
        ...
        PrintError(DtError, smNLS.cantLockErrorString);
        smGD.lockedState = UNLOCKED;
        XtRemoveEventHandler(lockWid, VisibilityChangeMask, False,
                             FinishLocking, NULL);
...

{
        ...
        if (done == False) {
                if (passwd == NULL) {
                        /*
                         * Caller just checking if it is possible to access
                         * password file (ie is dtsession suid bit set
                         * properly).
                         */
                        rc = True;
                        done = True;
        ...
        if (done == False) {
                ...
                if (sp != NULL &&
                    !strcmp(sp->sp_pwdp,
                            crypt(passwd, sp->sp_pwdp))) { /* a shadow match */
                        rc = True;
                        done = True;
                } else if (pwent != NULL &&
                           !strcmp(
...
#endif
#else
                if (strcmp(pwent->pw_passwd, crypt(passwd, pwent->pw_passwd)) !=
                    0)
#endif
                {
                        /*
