 ...
 *
 *************************************<->***********************************/
void TopicHelpRequested(Widget w, XtPointer client_data, XtPointer call_data) {
        int i;
        ...
        if ((smDD.smHelpDialog != NULL) && (XtIsManaged(smDD.smHelpDialog))) {
                return;
        }
        ...
        if (smDD.smHelpDialog != NULL) {
                i = 0;
                XtSetArg(uiArgs[i], DtNlocationId, locationId);
                i++;
                XtSetValues(smDD.smHelpDialog, uiArgs, i);

                i = 0;
                XtSetArg(uiArgs[i], XmNtransientFor, transientShell);
                i++;
                ...
                i = 0;
                XtSetArg(uiArgs[i], XmNtitle,
                         GETMESSAGE(29, 1, "Session Manager Help"));
                i++;
                XtSetArg(uiArgs[i], DtNhelpVolume, SM_HELP_VOLUME);
                i++;
                XtSetArg(uiArgs[i], DtNhelpType, DtHELP_TYPE_TOPIC);
                i++;
                XtSetArg(uiArgs[i], DtNlocationId, locationId);
                i++;
                smDD.smHelpDialog = DtCreateHelpQuickDialog(
                    smGD.topLevelWid, "helpWidget", uiArgs, i);
                ...
                n2 = 0;
                XtSetArg(setArgs2[n2], XmNlabelString, smDD.okString);
                n2++;
                XtSetValues(okButton, setArgs2, n2);

                printButton = DtHelpQuickDialogGetChild(
                    smDD.smHelpDialog, DtHELP_QUICK_PRINT_BUTTON);
                helpButton = DtHelpQuickDialogGetChild(
                    smDD.smHelpDialog, DtHELP_QUICK_HELP_BUTTON);
                backButton = DtHelpQuickDialogGetChild(
                    smDD.smHelpDialog, DtHELP_QUICK_BACK_BUTTON);
 ...
 *
 *************************************<->***********************************/
static void CloseHelpDialog(Widget w, XtPointer clientData,
                            XtPointer callData) {
