...
static void ProcessSaveSessionMessage(Tt_message saveMsg) {
        int i;
        ...
        for (i = 0; i < num_args; i++) {
                ...
                if (!strcmp(type, SAVE_TYPE)) {
                        ...
                        pch1 = strdup((char *)GETMESSAGE(
                            40, 16,
                            "The Save_Session message contains the "
                            "unrecognized argument '%s'."));
                        if (pch1) {
                                pch2 =
                                    XtMalloc(strlen(pch1) + strlen(type) + 3);
                                if (pch2) {
        ...
        SaveState((smGD.sessionType == HOME_SESSION), smSettings.startState,
                  saveType, shutdown, interactStyle, fast, global);
 ...
 *
 *************************************<->***********************************/
Tt_callback_action handleSessionMgrRequest(Tt_message msg, Tt_pattern pat) {
        char *op;
        Tt_status status;
...
void StartMsgServer(void) {
        ...
        smDD.okString =
            XmStringCreateLocalized(((char *)GETMESSAGE(18, 39, "OK")));
        smDD.okLogoutString =
            XmStringCreateLocalized(((char *)GETMESSAGE(18, 39, "OK")));
        ...
        if (smGD.bmsDead) {
                ...
                errfmt = SmNewString(
                    (char *)GETMESSAGE(6, 1,
                                       "Could not connect to ToolTalk message "
                                       "server:\n%sExiting ...\n"));
 ...
 *
 *************************************<->***********************************/
void RestoreDefaults(Atom toRestore) {
 ...
 *
 *************************************<->***********************************/
void ProcessPropertyNotify(XEvent *next) {
 ...
 *
 *************************************<->***********************************/
void ProcessClientMessage(XEvent *next) {
        ...
        if (cEvent->message_type == XaDtSmStmProtocol) {
                if (cEvent->data.l[0] == XaDtSmSaveToHome) {
                        ...
                        if (smGD.smState == READY) {
                                ...
                                SaveState(True, smSettings.startState,
                                          SmSaveLocal, DEFAULT_SHUTDOWN,
                                          SmInteractStyleNone, DEFAULT_FAST,
                                          DEFAULT_GLOBAL);
                                smGD.homeSave = False;
                 ...
                 */
                if (_DtGetSmState(smGD.display, smGD.topLevelWindow, &state) ==
                    Success) {
                        ...
                        if (flags & SM_STATE_CYCLETIMEOUT) {
                                ...
                                sprintf(newRes + strlen(newRes),
                                        "dtsession*%scycleTimeout: %d\n",
                                        smGD.extensionSpec,
                 ...
                 */
                if (_DtGetSmSaver(smGD.display, smGD.topLevelWindow, &saver) ==
                    Success) {
                        char *pRes;
                        ...
                        if (pRes) {
                                sprintf(pRes, "dtsession*%ssaverList: %s\n",
                                        smGD.extensionSpec, smGD.saverList);
                ...
                if (cEvent->data.l[0] == SS_ON) {
                        if (smSaverRes.saverTimeout + smSaverRes.lockTimeout >
                                0 &&
                            smGD.smState == READY) {
                                smGD.smState = IN_PROCESS;
                                LockDisplay(False);
...
void ProcessScreenSaverMessage(XEvent *next) {
        ...
        if (ssEvent->state == ScreenSaverOn) {
                ...
                if (smSaverRes.saverTimeout + smSaverRes.lockTimeout > 0 &&
                    smGD.smState == READY) {
                        /*
                         * Resource says to lock the display and SM is ready, so
                         * lock it.
                         */
                        smGD.smState = IN_PROCESS;
                        LockDisplay(False);
...
void ProcessReloadActionsDatabase(void) {
        static int needtoregister = 1;

        if (needtoregister) {
                /*
                 * Have not yet registered with the actions database to call
                 * this callback when the database changes. Do so.
                 */
                DtDbReloadNotify(
                    (DtDbReloadCallbackProc)ProcessReloadActionsDatabase,
                    (XtPointer)NULL);
                needtoregister = 0;
        }
...
void ProcessEvent(w, client_data, event, continue_to_dispatch) Widget w;
