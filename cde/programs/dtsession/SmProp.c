 ...
 */
static SmPropValue *CopyPropValues(int numPropValues,
                                   SmPropValue *thePropValues);

void SetPropertiesProc(SmsConn smsConn, SmPointer managerData, int numProps,
                       SmProp **theProps) {
        ...
        for (i = 0; i < numProps; i++) {
                ...
                for (pProp = pClient->props; pProp != NULL;
                     pProp = pProp->next) {
                ...
                if (!found) {
                        ...
                        pProp->prop.vals = CopyPropValues(theProps[i]->num_vals,
                                                          theProps[i]->vals);
                }
...
#ifdef DEBUG
        for (i = 0, pProp = pClient->props; pProp != NULL;
             i++, pProp = pProp->next) {
...
void DeletePropertiesProc(SmsConn smsConn, SmPointer managerData, int numProps,
                          char **propNames) {
        ClientRecPtr pClient = (ClientRecPtr)managerData;
        int i;
        ...
        for (i = 0; i < numProps; i++) {
                ...
                for (pProp = pClient->props, trail = pClient->props;
                     pProp != NULL; trail = pProp, pProp = pProp->next) {
...
void GetPropertiesProc(SmsConn smsConn, SmPointer managerData) {
        ...
        for (pProp = pClient->props, numProps = 0; pProp != NULL;
             ...
             i++, pProp = pProp->next) {
                ...
                for (j = 0; j < pProp->prop.num_vals; j++) {
                        ...
                        pPropsRet[i]->vals[j].value =
                            (SmPointer)XtMalloc(pProp->prop.vals[j].length);
...
#ifdef DEBUG
        for (i = 0; i < numProps; i++) {
                (void)printf("\t[%2d] name = %s\n", i + 1, pPropsRet[i]->name);
                (void)printf("\t[%2d] type = %s\n", i + 1, pPropsRet[i]->type);
                (void)printf("\t[%2d] num props = %d\n", i + 1,
...
static SmPropValue *CopyPropValues(int numPropValues,
        ...
        pPropValueRet =
            (SmPropValue *)XtMalloc(numPropValues * sizeof(SmPropValue));
        ...
        for (i = 0; i < numPropValues; i++) {
                ...
                pPropValueRet[i].value =
                    (SmPointer)XtMalloc(thePropValues[i].length + 1);

                if (!pPropValueRet[i].value)
                        return (NULL);

                memcpy(pPropValueRet[i].value, thePropValues[i].value,
                       thePropValues[i].length);

                ((char *)pPropValueRet[i].value)[pPropValueRet[i].length] =
                    '\0';
        }
 ...
 * (char *).
 */
char *GetArrayPropertyValue(ClientRecPtr pClientRec, char *propName) {
        PropertyRecPtr pPropRec;
 ...
 *
 *************************************<->***********************************/
char **GetListOfArrayPropertyValue(ClientRecPtr pClientRec, char *propName) {
        PropertyRecPtr pPropRec;
        char **ppchar;
        int i;
 ...
 *
 *************************************<->***********************************/
Boolean GetCardPropertyValue(ClientRecPtr pClientRec, char *propName,
