...
static Bool _fillStringArrayProc(XrmDatabase *, XrmBindingList, XrmQuarkList,
                                 XrmRepresentation *, XrmValue *, XPointer);
static Boolean _getStringResource(XrmDatabase, char *, char *, char **, char *);
static Boolean _getIntResource(XrmDatabase, char *, char *, int *, int);
static Boolean _getStringArrayResource(XrmDatabase, char *, char *, char ***,
                                       char **);
...
static void _initXrm(void) {
        if (!xrmInitialized) {
                ...
                XSMPName[1] = XSMPClass[1] = proxyName[1] = proxyClass[1] =
                    NULLQUARK;
                xrmInitialized = True;
        }
}

static Bool _countProc(XrmDatabase *clientDB, XrmBindingList bindingList,
...
static Bool _fillClientIDProc(XrmDatabase *clientDB, XrmBindingList bindingList,
                              XrmQuarkList quarkList, XrmRepresentation *reps,
                              XrmValue *value, XPointer uData) {
...
static Bool _fillStringArrayProc(XrmDatabase *clientDB,
                                 XrmBindingList bindingList,
                                 XrmQuarkList quarkList,
                                 XrmRepresentation *reps, XrmValue *value,
                                 XPointer uData) {
...
static Boolean _getStringResource(XrmDatabase xrmDB, char *fmtStr,
        ...
        if (NULL == resourceBuf)
                resourceBuf = XtMalloc(RESOURCE_BUF_SZ);
        sprintf(resourceBuf, fmtStr, clientID);
        if (XrmGetResource(xrmDB, resourceBuf, resourceBuf, &resourceType,
                           &resourceValue)) {
                if ((*resourcePtr = XtNewString(resourceValue.addr)) ==
                    (char *)NULL)
                        return False;
...
static Boolean _getIntResource(XrmDatabase xrmDB, char *fmtStr, char *clientID,
                               int *resourcePtr, int defaultVal) {
        char *resourceType;
        XrmValue resourceValue;
        ...
        *resourcePtr = (XrmGetResource(xrmDB, resourceBuf, resourceBuf,
                           ...
                           ? atoi(resourceValue.addr)
                           : defaultVal;

        return True;
}

static Boolean _getStringArrayResource(XrmDatabase xrmDB, char *fmtStr,
                                       char *clientID, char ***resourcePtr,
                                       char **defaultVal) {
        ...
        XrmEnumerateDatabase(xrmDB, resourceName, resourceClass,
                             XrmEnumOneLevel, _countProc, (XPointer)&nStrings);

        if (nStrings > 0) {
                char **stringsPtr;
                int i;

                if ((stringsPtr = (char **)XtMalloc(
                         (nStrings + 1) * sizeof(char *))) == (char **)NULL)
                        return False;
                ...
                if (XrmEnumerateDatabase(xrmDB, resourceName, resourceClass,
...
static void _freeStringArray(char **stringsPtr) {
        if (stringsPtr) {
                int i;
...
static Boolean _putStringResource(ClientDBRec *clientDB, char *fmtStr,
                                  char *clientID, char *resourceVal) {
        if (resourceVal != (char *)NULL) {
                if (NULL == clientIDBuf)
                        clientIDBuf = XtMalloc(RESOURCE_BUF_SZ);
...
static Boolean _putIntResource(ClientDBRec *clientDB, char *fmtStr,
                               char *clientID, int resourceVal) {
        if (NULL == clientIDBuf)
                clientIDBuf = XtMalloc(RESOURCE_BUF_SZ);
        sprintf(clientIDBuf, fmtStr, clientID);
...
static Boolean _putStringArrayResource(ClientDBRec *clientDB, char *fmtStr,
                                       char *clientID, char **resourceVal) {
        int i;
...
static char *_tmpFileName(char *fileName) {
        int fileNameLen = strlen(fileName);
        char *newFileName = (char *)XtMalloc((fileNameLen + 20) * sizeof(char));
        ...
        if (newFileName != (char *)NULL) {
                ...
                /* Hm.  I suppose if a couple billion versions of this file
                 * exist */
...
ClientDB OpenInputClientDB(char *fileName, char **version, char **dtsessionID) {
        ClientDBRec *inputDB;
        char **tmpPtr;
        char *resourceType;
        XrmValue resourceValue;
        ...
        *dtsessionID = (char *)NULL;
        ...
        if (inputDB->nXSMPClients > 0) {
                if ((inputDB->XSMPClients = (char **)XtMalloc(
                         inputDB->nXSMPClients * sizeof(char *))) ==
                    (char **)NULL) {
        ...
        if (inputDB->nProxyClients > 0) {
                if ((inputDB->proxyClients = (char **)XtMalloc(
                         inputDB->nProxyClients * sizeof(char *))) ==
                    (char **)NULL) {
                        XrmDestroyDatabase(inputDB->xrmDB);
        ...
        if ((!XrmGetResource(inputDB->xrmDB, versionStr, versionStr,
            ...
            (!XrmGetResource(inputDB->xrmDB, dtsessionIDStr, dtsessionIDStr,
                             &resourceType, &resourceValue)) ||
            ((*dtsessionID = XtNewString(resourceValue.addr)) ==
             (char *)NULL)) {
                if (*version) {
                        XtFree(*version);
                        *version = (char *)NULL;
...
ClientDB OpenOutputClientDB(char *fileName, char *version, char *dtsessionID) {
        ClientDBRec *outputDB;

        _initXrm();

        if ((fileName == (char *)NULL) || (version == (char *)NULL) ||
            (dtsessionID == (char *)NULL) ||
        ...
        if (((outputDB->xrmDBFileName = XtNewString(fileName)) ==
             (char *)NULL) ||
            ((outputDB->tmpDBFileName = _tmpFileName(fileName)) ==
             (char *)NULL)) {
...
XSMPClientDBRecPtr GetXSMPClientDBRec(ClientDB inputDBPtr) {
        ClientDBRec *inputDB = (ClientDBRec *)inputDBPtr;
        XSMPClientDBRecPtr clientPtr;
        char *clientID;

        if ((inputDB == (ClientDBRec *)NULL) || (!inputDB->openForInput) ||
            (inputDB->nextXSMPClientIndex >= inputDB->nXSMPClients) ||
            ((clientPtr = (XSMPClientDBRecPtr)XtMalloc(
                  sizeof(XSMPClientDBRec))) == (XSMPClientDBRecPtr)NULL)) {
        ...
        clientPtr->clientHost = clientPtr->program = clientPtr->cwd =
            (char *)NULL;
...
ProxyClientDBRecPtr GetProxyClientDBRec(ClientDB inputDBPtr) {
        ClientDBRec *inputDB = (ClientDBRec *)inputDBPtr;
        ...
        if ((inputDB == (ClientDBRec *)NULL) || (!inputDB->openForInput) ||
            ...
            ((clientPtr = (ProxyClientDBRecPtr)XtMalloc(
                  sizeof(ProxyClientDBRec))) == (ProxyClientDBRecPtr)NULL)) {
...
Boolean PutXSMPClientDBRec(ClientDB outputDBPtr, XSMPClientDBRecPtr clientPtr) {
        ClientDBRec *outputDB = (ClientDBRec *)outputDBPtr;
        char clientID[CLIENT_ID_SZ];

        if ((outputDB == (ClientDBRec *)NULL) || (outputDB->openForInput) ||
        ...
        if (!_putStringResource(outputDB, clientStr, clientID, clientID) ||
            ...
            !_putStringResource(outputDB, programStr, clientID,
                                clientPtr->program) ||
            !_putStringResource(outputDB, cwdStr, clientID, clientPtr->cwd) ||
...
Boolean PutProxyClientDBRec(ClientDB outputDBPtr,
                            ProxyClientDBRecPtr clientPtr) {
        ClientDBRec *outputDB = (ClientDBRec *)outputDBPtr;
        char clientID[CLIENT_ID_SZ];
...
Boolean CloseClientDB(ClientDB clientDBPtr, Boolean writeDB) {
...
void FreeXSMPClientDBRec(XSMPClientDBRecPtr clientPtr) {
...
void FreeProxyClientDBRec(ProxyClientDBRecPtr clientPtr) {
