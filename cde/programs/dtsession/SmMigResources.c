...
MigrateResources(char *inputfile, char *outputfile) {
        ...
        if ((out_fh != NULL) && (in_fh != NULL)) {
                ...
                unmodified_buffer =
                    malloc((size_of_mbuffer + 1) * sizeof(char *));
                        ...
                        do {
                                ...
                                *unmodified_buffer = '\0';
...

                                {
                                        ...
                                        rc = RC_SUCCESS;
                                        string = fgets((char *)inputbuffer1,
                                                       size_of_buffer, in_fh);

                                        /* if read was successful, then ...*/
                                        if (string != NULL) {
                                                /* determine if a larger
                                                   unmodified buffer needs to be
                                                   allocated
                                                */
                                                new_size =
                                                    strlen(unmodified_buffer) +
                                                ...
                                                strcat(unmodified_buffer,
                                                       inputbuffer1);

                                                /* set indicator if entire line
                                                 * was not read */
                                                DtLastChar(inputbuffer1,
                                                           &realend, &charlen);
                                                if ((charlen == 1) &&
                                                    (*realend != '\n')) {
                                                        rc = RC_PARTIAL_LINE;
                                                ...
                                                /* concatenate modified buffer
                                                   to previously read buffer
                                                */
                                                strcat(inputbuffer,
                                                       inputbuffer1);
                                        } else {
                                ...
                                } while ((rc == RC_LINE_CONTINUED) ||
                                         (rc == RC_PARTIAL_LINE));

                                /* if read was successful, then determine if the
                                   resources read are part of the set that need
                                   to be retained.
                                */
                                type_found = FALSE;
                                number_fields = 0;
                                ...
                                if (rc == RC_SUCCESS) {
                                        ...
                                        if (strncmp(inputbuffer, "!", 1) != 0) {
                                                /* determine the non-value
                                                   portion of the resource
                                                   specification (i.e. the part
                                                   to the left of the ":" and to
                                                   the left of the first space).
                                                */
                                                resource_end =
                                                    DtStrchr(inputbuffer, ':');
                                                first_space =
                                                    DtStrchr(inputbuffer, ' ');
