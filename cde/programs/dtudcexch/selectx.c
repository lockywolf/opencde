...
static char *spc(str, ch, count) char *str;
char ch;
int count;
{
        char *p;
...
static void OpenWindowCB() /* dtex change */
{
        ...
        msg2 =
            GETMESSAGE(4, 4,
                       "Failed to open the selected font. You have no right to "
                       "access for the font file, or the format of the file is "
                       "not consistent.");
        ...
        str = (char *)_XmStringUngenerate((XmString)st[0], NULL,
...
/*ARGSUSED*/
static void OpenCancelCB(widget, clientData, callData) /* dtex change */
    Widget widget;
caddr_t clientData;
caddr_t callData;
{
...
void data_sort(data, count) int *data;
int count;
{
        int *p1, *p2, tmp, i;

        for (; count; count--) {
                for (i = 1, p1 = data, p2 = data + 1; i < count;
                     i++, p1++, p2++) {
                        if (*p1 > *p2) {
                                tmp = *p2;
                                *p2 = *p1;
...
static Widget CreateSelectXLFD(top) /* dtex change */
    Widget top;
{
