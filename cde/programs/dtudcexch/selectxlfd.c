...
Widget xlfdDialog, xlfdWform, slctBLabel[BUTTONITEMS], slctBText[BUTTONITEMS],
    slctButton[BUTTONITEMS], listPop[BUTTONITEMS], listW[BUTTONITEMS],
...
void xlfdPopdownDialog(w) Widget w;
{
        if (XtIsManaged(w)) {
                XtUnmanageChild(w);
        }
 ...
 * get pixel width
 */
Dimension GetPixelWidth(w, columns, width) Widget w;
int columns;
Dimension *width;
{
        Arg args[1];
...
void quit(w, client_data, call_data) Widget w;
 ...
 * Unmanage widgets
 */
int ClearText(num, slctBText) int num;
Widget *slctBText;
{
        int i;
        /* toggle button unset */
...
int ClearButtons(num, slctButton) int num;
Widget *slctButton;
{
        int i, cnt;
        Boolean isSet;
        ...
        for (i = 0; i < BUTTONITEMS; i++) {
                if (i != num) {
                        cnt = 0;
                        XtSetArg(args[cnt], XmNset, &isSet);
                        cnt++;
                        XtGetValues(slctButton[i], args, cnt);
                        if (isSet == True) {
                                cnt = 0;
                                XtSetArg(args[cnt], XmNset, False);
                                cnt++;
 ...
 */
int GetPositionOfLists(num, xlfdDialog, listPop, slctBText, x, y) int num;
Widget xlfdDialog;
Widget *listPop;
Widget *slctBText;
Position *x;
Position *y;
{
        ...
        i = 0;
        XtSetArg(args[i], XmNx, &tbx);
        i++;
        XtSetArg(args[i], XmNy, &tby);
        i++;
        XtGetValues(xlfdDialog, args, i);
        i = 0;
        XtSetArg(args[i], XmNx, &tx);
        i++;
        XtSetArg(args[i], XmNy, &ty);
        i++;
        XtSetArg(args[i], XmNheight, &theight);
        i++;
        if (num == BUTTONITEMS - 1)
                XtGetValues(slctBText[num], args, i);
        else
                XtGetValues(XtParent(slctBText[num]), args,
                            i); /* ScrolledText */
        ...
        *x = tx + tbx + CX;
        ...
        for (i = 0; i < BUTTONITEMS; i++) {
                if (XtIsManaged(listPop[i]))
                        XtUnmanageChild(listPop[i]);
        }
        if (!XtIsManaged(listPop[num]))
                XtManageChild(listPop[num]);

        return;
}

void listEH(w, num, event) Widget w;
int num;
XButtonEvent *event;
{
        int i, j, ITEM_COUNT;
        ...
        if (num == 0 && ITEM_COUNT == 1) {
                ...
                str = (char *)_XmStringUngenerate(
                    (XmString)xms[0], NULL, XmMULTIBYTE_TEXT, XmMULTIBYTE_TEXT);
                if (str)
         ...
         */
        i = 0;
        XtSetArg(args[i], XmNitems, xms);
        i++;
        XtSetArg(args[i], XmNitemCount, ITEM_COUNT);
        i++;
...
void listPrintCB(w, num, call_data) Widget w;
int num;
XmListCallbackStruct *call_data;
{
	...
	if( SetKeyBuff( num, str ) ){
        ...
        if (XtIsManaged(listPop[num]))
                XtUnmanageChild(listPop[num]);
}

void OpenWindow(w, client_data, call_data) Widget w;
caddr_t client_data, *call_data;
{
        int r;
        int i;
        ...
        if (ed->function == EXPORT) {
                ...
                if (r == -1) { /* file open error */
                        ...
                        return;
                           ...
                           1) { /* file is being editted by other UDC client */
...
#define SetLabel(cnt)                                                          \
        {                                                                      \
                switch (cnt) {                                                 \
                case 0:                                                        \
                        buf = resource.xlfd_label;                             \
                        break;                                                 \
                case 1:                                                        \
                        buf = resource.code_label;                             \
                        break;                                                 \
                case 2:                                                        \
                        buf = resource.style_label;                            \
                        break;                                                 \
                case 3:                                                        \
                        buf = resource.size_label;                             \
                        break;                                                 \
                }                                                              \
                xms = XmStringCreateLocalized(buf);                            \
        }

int CreateXLFDLabelAndText(owner, slctBLabel, slctBText,
                           slctButton) Widget owner;
Widget *slctBLabel;
Widget *slctBText;
Widget *slctButton;
{
         ...
         */
        i = 0;
        XtSetArg(args[i], XmNx, 20);
        i++;
        XtSetArg(args[i], XmNheight, 20);
        i++;
        XtSetArg(args[i], XmNtopAttachment, XmATTACH_FORM);
        i++;
        XtSetArg(args[i], XmNtopOffset, 20);
        i++;
        /*
        ...
        */
        ...
        i++;
        XtSetArg(args[i], XmNtopWidget, _slctBLabel);
        i++;
        XtSetArg(args[i], XmNcolumns, XLFD_COLUMNS);
        i++;
        XtSetArg(args[i], XmNspacing, 0);
        i++;
        XtSetArg(args[i], XmNeditMode, XmSINGLE_LINE_EDIT);
        i++;
        XtSetArg(args[i], XmNscrollBarDisplayPolicy, XmAS_NEEDED);
        i++;
        XtSetArg(args[i], XmNscrollingPolicy, XmAPPLICATION_DEFINED);
        i++;
        /*
        ...
        */
         ...
         */
        i = 0;
        XtSetArg(args[i], XmNindicatorType, XmONE_OF_MANY);
        i++;
        XtSetArg(args[i], XmNtopWidget, _slctBText);
        i++;
        XtSetArg(args[i], XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET);
        i++;
        XtSetArg(args[i], XmNtopOffset, 15);
        i++;
        XtSetArg(args[i], XmNleftAttachment, XmATTACH_WIDGET);
        i++;
        XtSetArg(args[i], XmNleftWidget, _slctBText);
        i++;
        ...
        i++;
        /*
                _slctButton = XmCreateToggleButtonGadget( owner , "xlfdbtn",
           args, i);
        */
...
int CreateOtherLabelAndText(num, owner, baseForm, slctBLabel, topW, slctBText,
                            slctButton) int num;
Widget owner;
Widget baseForm;
Widget *slctBLabel;
...
Widget *slctButton;
{
         ...
         */
        i = 0;
        XtSetArg(args[i], XmNtopWidget, (num == 1) ? baseForm : topW);
        i++;
        XtSetArg(args[i], XmNx, 20);
        i++;
        XtSetArg(args[i], XmNheight, 20);
        ...
        */
        XtSetArg(args[i], XmNtopOffset, 36);
        i++;
        SetLabel(num);
        XtSetArg(args[i], XmNlabelString, xms);
        i++;
        ...
        */
        ...
        XtSetArg(args[i], XmNtopWidget, _slctBLabel);
        i++;
        XtSetArg(args[i], XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET);
        i++;
        XtSetArg(args[i], XmNrightOffset, 0);
        i++;
        XtSetArg(args[i], XmNrightWidget, baseForm);
        i++;
        XtSetArg(args[i], XmNcolumns, KEY_COLUMNS);
        i++;
        XtSetArg(args[i], XmNspacing, 0);
        i++;
        XtSetArg(args[i], XmNeditMode, XmSINGLE_LINE_EDIT);
        ...
        */
        ...
        i++;
        /*
                _slctButton = XmCreateToggleButtonGadget( owner , "button",
           args, i);
        */
...
Widget xlfdCreateScrolledList(owner, name, args, n) Widget owner;
char *name;
Arg *args;
int n;
{
        ...
        n++;
        XtSetArg(args[n], XmNlistSpacing, 4);
        n++;
 ...
 */
void PopupSelectXLFD(top) Widget top;
{
        ...
        if (xlfdDialog == NULL) {
                ...
                XtSetArg(args[n], XmNhelpLabelString, xms);
                n++;
                Dprintf(stdout,
                        "(PopupSelectXLFD)XmCreateTemplateDialog: start\n");
                xlfdDialog =
                    XmCreateTemplateDialog(top, "SelectDialog", args, n);
                Dprintf(stdout,
                        "(PopupSelectXLFD)XmCreateTemplateDialog: end\n");
                 ...
                 */
                Dprintf(stdout,
                        "(PopupSelectXLFD)CreateXLFDLabelAndText : start\n");
                CreateXLFDLabelAndText(xlfdWform, &slctBLabel[0], &slctBText[0],
                                       &slctButton[0]);
                n = 0;
                SetLabel(0);
                ...
                Dprintf(stdout,
                        "(PopupSelectXLFD)CreateXLFDLabelAndText : end\n");

                /*
                 *  create other key columns
                 */
                Dprintf(stdout,
                        "(PopupSelectXLFD)CreateOtherLabelAndText :start\n");
                for (j = 1; j < BUTTONITEMS; j++) {
                        CreateOtherLabelAndText(
                            j, xlfdWform, slctBText[0], &slctBLabel[j],
                            slctBLabel[j - 1], &slctBText[j], &slctButton[j]);
                }
                Dprintf(stdout,
                        "(PopupSelectXLFD)CreateOtherLabelAndText : end\n");

                /* set bottom offsets */
                n = 0;
                XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
                n++;
                XtSetArg(args[n], XmNbottomOffset, 36);
                n++;
                ...
                for (j = 0; j < BUTTONITEMS; j++) {
                        n = 0;
                        listPop[j] = XtCreatePopupShell(
                            shell_n[j], overrideShellWidgetClass, xlfdWform,
                            args, n);
                }
                ...
                for (j = 0; j < BUTTONITEMS; j++) {
                        ...
                        n++;
                        if (j == 0) {
                                XtSetArg(args[n], XmNwidth, xwidth);
                                n++;
                        } else {
                                XtSetArg(args[n], XmNwidth, kwidth);
                                n++;
                                XtSetArg(args[n], XmNlistMarginWidth, 0);
                                n++;
                        }
                        listW[j] = xlfdCreateScrolledList(listPop[j], list_n[j],
                                                          args, n);
                }
	...
	n = 0 ;
        ...
        XtUnmanageChild(msgBox);
                 ...
                 */
                for (i = 0; i < BUTTONITEMS; i++)
                        XtAddEventHandler(slctButton[i], ButtonReleaseMask,
                                          False, (XtEventHandler)listEH,
                                          (XtPointer)i);

                for (i = 0; i < BUTTONITEMS; i++)
                        XtAddCallback(listW[i], XmNsingleSelectionCallback,
                                      (XtCallbackProc)listPrintCB,
                                      (XtPointer)i);

                XtAddCallback(xlfdDialog, XmNokCallback,
                              (XtCallbackProc)OpenWindow, NULL);
                XtAddCallback(xlfdDialog, XmNhelpCallback, (XtCallbackProc)quit,
                              NULL);
