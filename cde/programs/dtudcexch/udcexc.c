...
Widget excCreateRadioButton(Widget parent, String name, String label,
        ...
        widget =
            XtVaCreateManagedWidget(name, xmToggleButtonWidgetClass, parent,
                                    XmNindicatorType, XmONE_OF_MANY, NULL);
        XtAddCallback(widget, XmNvalueChangedCallback,
                      (XtCallbackProc)callback_func, (XtPointer)client_data);
...
int main(int argc, char *argv[]) {
        ...
        toplevel = XtAppInitialize(&app_context, "UDCexchange",
                                   (XrmOptionDescList)NULL, 0, (int *)&argc,
                                   argv, (String *)NULL, args, n);
        ...
        errmsg_org[5] =
            GETMESSAGE(8, 14, "The specified font file does not exist.");
        errmsg_org[6] =
            GETMESSAGE(8, 16, "This font is not a pcf or snf font.");
        ...
        errmsg_org[19] = GETMESSAGE(
            8, 42, "Cannot get lock information from the font file.");
        errmsg_org[20] =
            GETMESSAGE(8, 44, "Cannot find the specified font file.");
        ...
        errmsg_org[24] =
            GETMESSAGE(8, 52, "The specified font has not been opened.");
        errmsg_org[25] = GETMESSAGE(8, 54, "Fatal error occurred.");

        resource.xlfd_label = GETMESSAGE(8, 56, "XLFD name          :"),
        resource.copy_xlfd_label =
            GETMESSAGE(8, 58, "Original font (XLFD name)  :");
        ...
        row = XtVaCreateManagedWidget("row", xmRowColumnWidgetClass, mainw,
                                      XmNradioBehavior, True, XmNradioAlwaysOne,
                                      True, NULL);
        expfunc = excCreateRadioButton(
            row, "expfunc", explabel, (XtCallbackProc)callexpCB, (XtPointer)ed);

        impfunc = excCreateRadioButton(
            row, "impfunc", implabel, (XtCallbackProc)callimpCB, (XtPointer)ed);
