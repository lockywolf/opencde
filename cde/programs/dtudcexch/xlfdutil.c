...
extern Widget xlfdWform, cpyWform, slctBLabel[BUTTONITEMS],
    slctBText[BUTTONITEMS], slctButton[BUTTONITEMS], listPop[BUTTONITEMS],
    listW[BUTTONITEMS], errorMD, msgBox;

/****************************************************************
 * functions for get font infomation
 ****************************************************************/

int GetXLFDInfomations() {
        ...
        if (FalxGetFontList(&font_L, XtDisplayOfObject(toplevel))) {
                Dprintf(stderr,
                        "GetXLFDInfomations() : FalxGetFontList() error.\n");
                return (-1);
        }
...
int GetItemsToDisplay(num, itemcnt, xms_list) int num;
int *itemcnt;
XmString **xms_list;
{
        int i, j, cnt;
        ...
        case 1: /* code area	*/
                ...
                for (i = 0; i < tmp_cnt; i++) {
                        sp = string;
                        rlen = sprintf(sp, "CS:%d ",
                                       (CodeList->dlist[i]->fontset));
                        sp += rlen;
                        for (j = 0; j < CodeList->dlist[i]->code_area_num;
                             j++) {
                                rlen = sprintf(
                                    sp, "%x - %x ",
        ...
        case 2: /* style	*/
                ...
                tmp_dlist = (char **)malloc(sizeof(char *) * tmp_cnt);
                for (i = 0; i < tmp_cnt; i++) {
                        if ((tmp_dlist[i] = strdup(StyleList->list[i])) ==
                            NULL) {
                                return (-1);
                        }
                }
...
static char *skipSpace(str, skipcnt) char *str;
int skipcnt;
{
        int i, spacecnt;
        char *sp;
...
int SetKeyBuff(num, str) int num;
char *str;
{
        int i, j;
        int cdset, start, end;
        ...
        case 1: /* code area	*/
                ...
                while (sscanf(sp, "%x %c %x ", &start, &hyp, &end) == 3) {
                        ...
                        if (tmp_list == NULL) {
                                tmp_list =
                                    (FalxUdcArea *)malloc(sizeof(FalxUdcArea));
                        } else {
                                tmp_list = (FalxUdcArea *)realloc(
                                    KeyBuff.FileData.list,
