...
void createbdf(Exc_data *ed) {
        ...
        i = ExpGpftoBDF(ed->fontfile, ed->bdffile, ed->code_num,
                        ed->gpf_code_list, comment_num, comment_list, 0);
        if (i != 0) {
...
XmString *setxmslist(ListData *ld) {
        ...
        if ((xmslist = (XmString *)calloc(ld->existcode_num,
                                          sizeof(XmString *))) == NULL) {
...
void selcharcd(Exc_data *ed) {
        ...
        cancel =
            excCreatePushButton(mainw, "cancel", cancellabel,
