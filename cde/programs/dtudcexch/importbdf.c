...
int ImpBDFCodeList(bdf_name, code_num, code_list) char *bdf_name;
int *code_num;
int **code_list;
{
        int *glyph_list;
        ...
        if ((glyph_list =
                 (int *)calloc(*code_num, (unsigned int)sizeof(int))) == NULL) {
                return (-1);
        }
...
int ImpBDFCodeListFree(code_list) int **code_list;
{
        free(*code_list);
        return (0);
}

int ImpBDFCheck(bdf_name, gpf_name) char *bdf_name; /* BDF file name	*/
...
int ImpBDFtoGpf(bdf_name, gpf_name, bdf_codenum, bdf_codelist) char *bdf_name;
char *gpf_name;
int bdf_codenum;
int *bdf_codelist;
{
         ...
         */
        if ((rtn = impMergePtn(&ReadUdc, &ReadGpf, buf, bdf_codenum,
                               bdf_codelist))) {
                if (WriteGpf.out_file) {
                        UNLINK_TMPFILE(WriteGpf.out_file);
                }
        ...
        return (Make_NewFefFile(WriteGpf.snf_file, WriteGpf.out_file,
                                ...
                                (gid_t)statbuf.st_gid, "ImpBDFtoGpf()"));
}
...
static impFileConvInit(r_udc, r_gpf, w_gpf) struct btophead *r_udc;
...
struct ptobhead *w_gpf;
{
        ...
        switch (fork()) {
        case 0:
                if ((ofd = open(w_gpf->out_file, O_WRONLY | O_CREAT, 0664)) <
                    0) {
                        return BDF_OPEN_OUT;
                }
                close(0);
...
static impGetGpfInf(r_gpf, w_gpf, buf, bdf_codenum) struct btophead *r_gpf;
struct ptobhead *w_gpf;
char *buf;
int bdf_codenum;
{
...
static impReadBdfHeaderAndPut(r_gpf, w_gpf, buf) struct btophead *r_gpf;
struct ptobhead *w_gpf;
char *buf;
{
        char *p;
        ...
        while (1) {
                ...
                if (!strncmp(p, SIZE, (unsigned int)strlen(SIZE))) {
                        if ((sscanf(p, "SIZE %f%d", &(r_gpf->bdf_point),
                                    &(r_gpf->bdf_xdpi))) != 2) {
                                return BDF_INVAL;
                        }
...
static impMergePtn(r_udc, r_gpf, buf, bdf_codenum,
                   bdf_codelist) struct btophead *r_udc;
struct btophead *r_gpf;
char *buf;
int bdf_codenum;
int *bdf_codelist;
        ...
        for (i = 0; i < r_udc->num_chars; i++) {
                ...
                for (j = 0; j < r_gpf->num_chars; j++) {
                        if (r_gpf->code[j] == code) {
                                if ((rtn =
                                         impModifyPtn(r_udc, r_gpf, buf, j))) {
                                        return rtn;
                                }
                                break;
                        } else if (r_gpf->code[j] > code) {
                                if ((rtn = impInsertPtn(r_udc, r_gpf, buf, code,
                                                        j))) {
                                        return rtn;
                                }
                                break;
...
static impModifyPtn(r_udc, r_gpf, buf, ix) struct btophead *r_udc;
struct btophead *r_gpf;
char *buf;
int ix;
{
        ...
        if (r_udc->zoomf) {
                if ((rtn =
                         GetBdfPtn(r_udc, buf, r_udc->ptn[0], mwidth, msize))) {
                        return rtn;
                }
                ...
                if ((rtn = GetBdfPtn(r_udc, buf, r_gpf->ptn[ix], mwidth,
                                     msize))) {
                        return rtn;
                }
        }
...
static impInsertPtn(r_udc, r_gpf, buf, code, ix) struct btophead *r_udc;
struct btophead *r_gpf;
char *buf;
int code;
int ix;
...
static impWriteSnf(r_gpf, w_gpf) struct btophead *r_gpf;
struct ptobhead *w_gpf;
{
        w_gpf->zoomf = 0;
