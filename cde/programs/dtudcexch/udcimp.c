                ...
                if ((code >= code1) && (code <= code2)) { /*in udc area */
                        for (j = 0; j < ld->existcode_num; j++) {
                                if (code == ld->existcode[j])
                                        return (1); /* the font has same glyph
                                                       index */
                        }
                        return (0);
                }
...
void getcharcd(Exc_data *ed) {
        ...
        msg = GETMESSAGE(
            12, 2, "Glyph images in this BDF file can't be added to the font.");
        msg2 = GETMESSAGE(
            12, 4,
            "Failed to open the selected BDF font. You have no right "
            "to access for the font file, or the format of the file is "
            "not consistent.");
        msg3 = GETMESSAGE(
            12, 6,
...
void addcharptn(Exc_data *ed) {
        ...
        msg = GETMESSAGE(
            ...
            "access for the font file, or the format of the file is not "
            "consistent.");
        msg2 = GETMESSAGE(
            12, 10,
            "Glyph images in this BDF file can't be added to the font.");
        msg3 =
            GETMESSAGE(12, 12, "Failed in the registration of the font file.");
        ...
        i = 0;
        i = ImpBDFtoGpf(ed->bdffile, ed->fontfile, ed->code_num,
                        ed->bdf_code_list);
        FalCloseFont(fontid);
        if (i != 0) {
