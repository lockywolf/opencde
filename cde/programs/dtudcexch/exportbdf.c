...
int expCheckCode(code, code_num, code_list) unsigned int code;
int code_num;
int *code_list;
{
        int i;
...
int ExpGpftoBDF(
    gpf_name, bdf_name, code_num, code_list, comment_num, comment_list,
    make_all) char *gpf_name; /* pointer to GPF file name area 	*/
...
int make_all;                 /* convert whole GPF fomat file to BDF  */
{
        ...
        if ((rtn =
                 readBdfToMemory(&r_gpf, buf, code_num, code_list, make_all))) {
...
static int writeBdfHeader(head, comment_num,
                          comment_list) struct ptobhead *head;
int comment_num;     /* number comments 			*/
char **comment_list; /* pointer to the list of comments 	*/
{
        FILE *fp;
        ...
        while (1) {
                 ...
                 * write user comments
                 */
                if (!strncmp(p, "FONT", strlen("FONT")) && comment_list &&
                    !comflg) {
                        int i;
                ...
                if (!strncmp(p, FONTBOUNDINGBOX, FONTBOUNDINGBOXsz)) {
                        if ((cnt =
                                 sscanf(p, "FONTBOUNDINGBOX %d%d%d%d",
...
static int readBdfToMemory(head, buf, code_num, code_list,
                           make_all) struct btophead *head;
char *buf;
int code_num;   /* number of GPF code 			*/
int *code_list; /* pointer to GPF code lists 		*/
int make_all;   /* convert whole GPF fomat file to BDF  */
