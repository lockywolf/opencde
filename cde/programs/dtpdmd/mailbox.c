...
void mbox_build(XpPdmServiceRec *rec) {
        ...
        rec->mbox_window = XCreateSimpleWindow(
            rec->selection_display, DefaultRootWindow(rec->selection_display),
            0, 0, 1, 1, 1, BlackPixel(rec->selection_display, tscreen),
...
void mbox_reply(XpPdmServiceRec *rec) {
        ...
        XChangeProperty(rec->selection_display, rec->requestor, rec->prop_atom,
                        ...
                        (unsigned char *)&(rec->mbox_window), 1);
...
void mbox_receive(XpPdmServiceRec *rec, XEvent *report) {
        ...
        if (cme->format == 16) {
                ...
                if (rec->cookie_state == 0) {
                        ...
                        XDestroyWindow(rec->selection_display,
                                       rec->mbox_window);
                ...
                rec->cookies[rec->cookie_cnt - 1] =
                    (Xauth *)Xmalloc(sizeof(Xauth));
                ...
                memcpy((char *)&(rec->in_buf[rec->in_sofar]), cme->data.b,
                       maxgrab);
