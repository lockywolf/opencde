...
int xio_quickie_handler(Display *dpy) { longjmp(xio_quickie_jmp_buf, 1); }
...
void xp_add_argv(char ***argv, char *str) {
        ...
        if (*argv) {
                for (i = 0; (*argv)[i]; i++)
                        ;
                *argv =
                    (char **)Xrealloc((char *)*argv, sizeof(char *) * (i + 2));
        } else {
                i = 0;
