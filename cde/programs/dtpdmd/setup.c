...
Bool _PdmMgrSetup(Display *sel_display, Window sel_window, Bool security_flag) {
        ...
        static char *atom_names[] = {
            "PDM_START",        "PDM_REPLY",        "TARGETS",
            "TIMESTAMP",        "MULTIPLE",         "PDM_START_OK",
            "PDM_START_VXAUTH", "PDM_START_PXAUTH", "PDM_START_ERROR",
            "PDM_EXIT_OK",      "PDM_EXIT_CANCEL",  "PDM_EXIT_VXAUTH",
            "PDM_EXIT_PXAUTH",  "PDM_EXIT_ERROR",   "PDM_MBOX",
            "PDM_MAIL"};
         ...
         */
        XInternAtoms(sel_display, atom_names, XtNumber(atom_names), False,
                     atoms);
        ...
        if (XGetSelectionOwner(sel_display, g.pdm_selection) != None) {
                 ...
                 */
                i = 0;
                while ((XGetSelectionOwner(sel_display, g.pdm_selection) !=
                        sel_window)) {
                        if (i++ > 100)
                                return (/* cannot become owner */ False);

                        XSetSelectionOwner(sel_display, g.pdm_selection,
                                           sel_window, CurrentTime);
                }
        }
