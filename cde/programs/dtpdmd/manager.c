                ...
                if (g.serviceRecs[i]->message_pipe[0] == *source) {
                        ...
                        if (statBuffer.st_size > 0) {
                                if (g.serviceRecs[i]->message_string) {
                                        ...
                                        g.serviceRecs[i]->message_string =
                                            Xrealloc((char *)g.serviceRecs[i]
                                                         ->message_string,
                                                     statBuffer.st_size + inc +
                                                         1);
                                } else {
                                        inc = 0;

                                        g.serviceRecs[i]->message_string =
                                            Xmalloc(statBuffer.st_size + 1);
                                }
                                ...
                                while (keepon) {
                                        ...
                                        if (n == statBuffer.st_size) {
                                                /* read all there is (per the
                                                 * previous stat) */
                                                keepon = False;
                                        } else if ((n == -1) &&
                                                   (errno == EINTR)) {
                                                /* an interrupt came in before
                                                 * the read could start */
                                                keepon = True;
                                        } else if ((n == -1) || (n == 0)) {
                                                /* problems - bail out */
                                                g.serviceRecs[i]
                                ...
                                /*
                                 * NULL terminate what we have so far to make it
                                 * look like a string.
                                 */
                                g.serviceRecs[i]
                                    ->message_string[statBuffer.st_size + inc] =
                                    '\0';
                        } else {
                                /*
                                 * No more to read - this really means the pipe
                                 * is going down.
                                 */
...
void mgr_initialize(XEvent *report, XpPdmServiceRec *rec) {
        ...
        if (XGetWindowProperty(
                selection_display, requestor, prop_atom, 0, 100000, True,
                AnyPropertyType, &text_prop.encoding, &text_prop.format,
                &text_prop.nitems, &tafter, &text_prop.value) != Success) {
                /*
                 * Error
                 */
...
void mgr_launch_pdm(XpPdmServiceRec *rec) {
        ...
        rec->message_xtid = XtAppAddInput(
            g.context, rec->message_pipe[0], (XtPointer)XtInputReadMask,
        ...
        if (rec->cookie_cnt) {
                ...
                if (rec->auth_file) {
                        ...
                        if (existing_name) {
                                if (access(existing_name, R_OK) ==
                                    0) { /* checks REAL id */
                                        existing_file =
                                            fopen(existing_name, "r");
                                        if (existing_file) {
                                                for (;;) {
                                                        entry = XauReadAuth(
                                                            existing_file);
        ...
        if (rec->pid < 0) {
                 ...
                 */
                if ((rec->cookie_cnt) && (rec->auth_filename) &&
                    (rec->auth_file)) {
                ...
                (void)fprintf(stderr, PDMD_MSG_10, g.prog_name,
        ...
        while (1) {
                if (firstTime) {
                        ...
                        if (!tptr3) {
                                 ...
                                 */
                                xp_add_argv(&(rec->pdm_exec_argvs),
                                            xpstrdup(g.default_pdm));
                                break;
                        }
                } else {
        ...
        if ((rec->pdm_exec_errormessage) && (g.log_file)) {
                if ((errlog = fopen(g.log_file, "a+")) != NULL) {
                        ...
                        fprintf(errlog, PDMD_MSG_11, g.prog_name, ctime(&now),
                                rec->pdm_exec_errormessage, eec,
                                rec->pdm_exec_argvs[0], rec->print_display_str,
...
void mgr_shutdown_reply(XpPdmServiceRec *rec) {
        ...
        if (setjmp(xio_quickie_jmp_buf) == 0) {
                ...
                if (rec->seldpy_as_printdpy) {
                         ...
                         * ahead and use it.
                         */
                        XSendEvent(rec->selection_display, rec->print_window,
                                   False, 0L, &cme);
                } else {
                        ...
                        if (pdpy) {
                                ...
                                XSendEvent(pdpy, rec->print_window, False, 0L,
                                           &cme);
                                XCloseDisplay(pdpy);
                        }
                }
...
Bool has_exec_token(XpPdmServiceRec *rec) {
        ...
        if (s1) {
                ...
                if (s2) {
                        ...
                        if (strlen(s1) == 0) {
                                /*
                                 * The token was it - free the buffer now so
                                 * that it appears no stderr was ever generated.
                                 */
        ...
        for (i = 0; i < g.serviceRecNum; i++) {
                ...
                if ((g.serviceRecs[i]->do_launch_reply) &&
                    (has_exec_token(g.serviceRecs[i]))) {
                         ...
                         * a pdm_exit_* code later.  If we send
                         * pdm_start_vxauth, pdm_start_pxauth, or
                         * pdm_start_error, it indicates to the client a fatal
                         * condition, and they will NOT receive any more
                         * messages, including any pdm_exit_* codes.  The dtpdmd
                         * will hang around however, if for no other reason than
                         * to capture stderr and log it when the child finally
                         * goes down.
                         */
                        mgr_launch_reply(g.serviceRecs[i]);
                        g.serviceRecs[i]->do_launch_reply =
                            False; /* it has been done */
                }
                ...
                if ((g.serviceRecs[i]->exit_received) &&
                    (g.serviceRecs[i]->message_pipe[0] == -1)) {
                        ...
                        if (((g.serviceRecs[i]->message_string) ||
                            ...
                            (g.log_file)) {
                                if ((errlog = fopen(g.log_file, "a+")) !=
                                    NULL) {
                                        now = time((time_t)0);

                                        fprintf(
                                            errlog, PDMD_MSG_14, g.prog_name,
                                            ctime(&now),
                                        ...
                                        fprintf(stderr, PDMD_MSG_16,
                                                g.prog_name, g.log_file);
                                        fflush(stderr);
