...
static void pusage(char *prog_name) {
        fprintf(stderr, "\n");
        fprintf(stderr, PDMD_MSG_1, prog_name, "PDM_MANAGER",
                DEFAULT_PDM_EXECUTABLE);
        fprintf(stderr, "\n");
        fprintf(stderr, "\n");
}
...
static int generic_error_handler(edpy, eevent) Display *edpy;
XErrorEvent *eevent;
{
...
#endif /* __aix */
{
        ...
        if (pid == -1) {
                 ...
                 * child has indeed terminated.  Look for its tracking record
                 * and mark accordingly for later shutdown.
                 */
                for (i = 0; i < g.serviceRecNum; i++) {
                        if (g.serviceRecs[i]->pid == pid) {
                                ...
                                /*
                                 * Figure out who will deliver the "tickle" to
                                 * XtAppNextEvent that will allow this child's
                                 * status to be rediscovered so that a
                                 * subsequent shutdown can be done.
                                 *
                                 *    - if the child's message_pipe is still up,
                                 *      let its upcoming disconnection be the
                                 * tickle.
                                 *
                                 *    - if the child's message pipe is aleady
                                 * down, then this is our last event related to
                                 * the child, so tickle the "Xt Signal
                                 * Pipe" we installed.
                                 *
                                 *    - if the Xt Signal Pipe is down, then
                                 * we'll have to rely upon a 3rd party to
                                 * deliver an event that will reactivate
                                 * the Xt Main Loop.
                                 */
...
char **argv;
{
        ...
        if ((sel_display = XtOpenDisplay(g.context, display_str, "dtpdmd",
                                         "Dtpdmd", 0, 0, &argc, argv)) ==
            NULL) {
                fprintf(stderr, "\n");
        ...
        sel_window = XCreateSimpleWindow(
            sel_display, DefaultRootWindow(sel_display), 0, 0, 1, 1, 1,
            BlackPixel(sel_display, tscreen), WhitePixel(sel_display, tscreen));
        ...
        xtid = XtAppAddInput(g.context, xtkick_pipeG[0],
                             (XtPointer)XtInputReadMask, xtkick_proc,
                             (XtPointer)NULL);
        ...
        while (1) {
                 ...
                 *    - Alt input occurs because a pdm message pipe was
                 * disconnected
                ...
                case SelectionRequest:
                        if (report.xselectionrequest.selection ==
                            g.pdm_selection) {
                                if (report.xselectionrequest.target ==
                                    g.pdm_start) {
                                        dispatch_mgr(&report);
                                } else if (report.xselectionrequest.target ==
                                           g.pdm_targets) {
