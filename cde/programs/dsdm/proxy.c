...
int Swap2Bytes(char byte_order, CARD16 data) {
...
int Swap4Bytes(char byte_order, CARD32 data) {
...
static drop_info_t *NewDropInfo(void) {
        ...
        for (i = 1; i < DROP_TABLE_MAX; i++)
                if (drop_table[i].time_stamp <
                    drop_table[oldest_index].time_stamp)
                        oldest_index = i;
...
static drop_info_t *GetDropInfo(Window proxy_sel_req_win) {
        int i;
...
static drop_info_t *GetDropInfoUsingProxyHandle(Atom proxy_handle) {
        int i;
...
static void ClearDropInfo(Atom proxy_handle) {
        int i;
        for (i = 0; i < DROP_TABLE_MAX; i++)
                if (drop_table[i].proxy_handle == proxy_handle) {
...
Window GetAtomWindow(Display *dpy, Window win, Atom atom) {
        Window root, parent;
        Window *children;
        ...
        XGetWindowProperty(dpy, win, atom, 0, 0, False, AnyPropertyType, &type,
...
Window GetPropertyWindow(Display *dpy, Window in_win, Atom atom) {
        Atom type;
        int format;
        ...
        if ((XGetWindowProperty(dpy, in_win, atom, 0L, 1, False,
                                AnyPropertyType, &type, &format, &lengthRtn,
                                &bytesafter,
...
void ProxyInit(Display *dpy, Window dsdm_win) {
        enum { XA_MOTIF_DRAG_WINDOW,
               XA_MOTIF_DRAG_PROXY_WINDOW,
               XA_MOTIF_DRAG_AND_DROP_MESSAGE,
               XA_MOTIF_DRAG_INITIATOR_INFO,
               XA_MOTIF_DRAG_RECEIVER_INFO,
               XAXmTRANSFER_SUCCESS,
               XAXmTRANSFER_FAILURE,
               XA_MOTIF_DRAG_TARGETS,
               XA_SUN_DRAGDROP_TRIGGER,
               XA_SUN_DRAGDROP_DONE,
               XA_SUN_SELECTION_END,
               XA_SUN_SELECTION_ERROR,
               XA_SUN_DRAGDROP_ACK,
               XATARGETS,
               XAMULTIPLE,
               NUM_ATOMS };
        static char *atom_names[] = {"_MOTIF_DRAG_WINDOW",
                                     "_MOTIF_DRAG_PROXY_WINDOW",
        ...
        for (i = 0; i < DROP_TABLE_MAX; i++) {
                ...
                drop_table[i].proxy_sel_req_win =
                    XCreateWindow(dpy, DefaultRootWindow(dpy), 0, 0, 1, 1, 0, 0,
                                  InputOnly, CopyFromParent, 0, &attr);
        }
        motif_drag_win =
            GetPropertyWindow(dpy, DefaultRootWindow(dpy), ATOM_MOTIF_DRAG_WIN);

        /* if motif_drag_win doesn't exist then define and declare it */
        if (motif_drag_win == None) {
                motif_drag_win = dsdm_win;
                XChangeProperty(
                    dpy, DefaultRootWindow(dpy), ATOM_MOTIF_DRAG_WIN, XA_WINDOW,
                    32, PropModeReplace, (unsigned char *)&motif_drag_win, 1);
        }
        ...
        for (i = 0; i < DROP_TABLE_MAX; i++)
                XSetSelectionOwner(dpy, drop_table[i].proxy_handle, proxy_win,
                                   CurrentTime);
}

/* Place the motif receiver property on the Openlook receiver's top
   level window */
int AdvertiseMotifDropSite(Display *dpy, Window win) {
...
#define MOTIF_RECEIVER_FLAG 0x80000000
        ...
        XChangeProperty(dpy, win, ATOM_MOTIF_RECEIVER_INFO, /* property */
...
static char InRect(int drop_x, int drop_y, int x, int y, unsigned int width,
                   unsigned int height) {
        if ((drop_x >= x) && (drop_x <= (x + width)) && (drop_y >= y) &&
            (drop_y <= (y + height)))
                return True;
        else
                return False;
...
static void GetOlitDropSite(Display *dpy, Window top_level_win, INT16 drop_x,
                            INT16 drop_y, unsigned long *ret_site_id,
        ...
        *ret_event_win = *ret_site_id = 0;
        data =
            (unsigned long *)GetInterestProperty(dpy, top_level_win, &nitems);
        if (data == NULL)
                return;

        /* get root window */
        if (XGetGeometry(dpy, top_level_win, &root_win, &junk, &junk, &ujunk,
                         &ujunk, &ujunk, &ujunk) == 0)
                return;

        NEXTWORD(version);
        if (version != DRAGDROP_VERSION)
                return;
        NEXTWORD(nsites);
        for (i = 0; i < nsites && !done; ++i) {
                ...
                case INTEREST_RECT:
                        ...
                        for (j = 0; j < nrects && !done; ++j) {
                                ...
                                NEXTWORD(width);
                                NEXTWORD(height);
                                if (XTranslateCoordinates(dpy, top_level_win,
                                                          root_win, x, y, &x,
                                                          &y, &wjunk))
                                        done = InRect((int)drop_x, (int)drop_y,
                                                      x, y, width, height);
                        }
                        break;
                case INTEREST_WINDOW:
                        ...
                        for (j = 0; j < nrects && !done; ++j) {
                                NEXTWORD(areawin);
                                if (0 == XGetGeometry(dpy, areawin, &wjunk,
                                                      &junk, &junk, &width,
                                                      &height, &border,
                                                      &ujunk)) {
                                        fprintf(stderr,
                                ...
                                if (XTranslateCoordinates(dpy, areawin,
                                                          root_win, 0, 0, &x,
                                                          &y, &wjunk)) {
                                        x = x - border;
                                        y = y - border;
...
static CARD16 ConvertOlitAction(CARD32 olit_action) {
        CARD16 motif_action;
        ...
        if (olit_action & OLIT_DROP_MOVE_ACTION)
                motif_action |=
                    MOTIF_DROP_MOVE_ACTION | (MOTIF_DROP_MOVE_ACTION << 8);
        else if (olit_action & OLIT_DROP_LINK_ACTION)
                motif_action |=
                    MOTIF_DROP_LINK_ACTION | (MOTIF_DROP_LINK_ACTION << 8);

        return motif_action;
}

/* translate a Motif operation to an OLIT operation */
static CARD32 ConvertMotifAction(CARD16 motif_action) {
        CARD32 olit_action;
...
void HandleMotifMessage(Display *dpy, XClientMessageEvent *event) {
        ...
        if (info.message_type & MOTIF_RECEIVER_BIT) {
                ...
                if (event->window != proxy_win)
                        XSendEvent(dpy, event->window, False, NoEventMask,
                                   (XEvent *)event);
        } else {
                /* initiator message */
                if ((info.message_type & MOTIF_MESSAGE_TYPE_MASK) ==
                    MOTIF_DROP_START) {
                        unsigned long site_id;
                        Window event_win;
                        ...
                        info.flags =
                            Swap2Bytes(info.byte_order, event->data.s[1]);

                        /* since dropSiteStatus can't be determined in a short
                           amount of time, always return a valid drop site */
                        info.flags &= MOTIF_DROP_SITE_STATUS_MASK;
                        info.flags |= MOTIF_DROP_SITE_STATUS_VALID;

                        info.time =
                            Swap4Bytes(info.byte_order, event->data.l[1]);
                        info.x = Swap2Bytes(info.byte_order, event->data.s[4]);
                        info.y = Swap2Bytes(info.byte_order, event->data.s[5]);
                        info.icc_handle =
                            Swap4Bytes(info.byte_order, event->data.l[3]);
                        info.src_window =
                            Swap4Bytes(info.byte_order, event->data.l[4]);
                        ...
                        info.message_type = event->data.b[0] |=
                            MOTIF_RECEIVER_BIT;
                        event->data.s[1] =
                            Swap2Bytes(info.byte_order, info.flags);
                        event->window = info.src_window;
                        XSendEvent(dpy, event->window, False, NoEventMask,
                                   (XEvent *)event);

                        /* get olit drop site */
                        GetOlitDropSite(dpy, dest_win, info.x, info.y, &site_id,
                                        &event_win);
                        ...
                        if (event_win && olit_action) {
                                ...
                                out_event.data.l[OLIT_DROP_SELECTION] =
...
static void HandleOlitTrigger(Display *dpy, XClientMessageEvent *event) {
        drop_site_t *site;
        char success = False;
        ...
        for (site = MasterSiteList; site != NULL; site = site->next) {
                if (site->site_id == event->data.l[OLIT_DROP_SITE_ID]) {
                        ...
                        drop_info->motif_action = ConvertOlitAction(
                            event->data.l[OLIT_DROP_OPERATION]);
                        drop_info->receiver_win = site->window_id;
                        drop_info->source_handle = source_handle;
                        drop_info->trigger_x =
                            (event->data.l[OLIT_DROP_COORDINATE]) >> 16;
                        drop_info->trigger_y =
                        ...
                        XConvertSelection(
                            dpy, drop_info->source_handle, /* selection */
                            ATOM_TARGETS,
        ...
        if (!success)
                ...
                XConvertSelection(dpy, source_handle, ATOM_SUN_DND_DONE,
                                  ATOM_SUN_DND_DONE, /* property */
                                  proxy_win, time_stamp);
}

/* Place the Motif initiator property on the initiator's top level window */
static void UpdateInitiatorAtom(Display *dpy, Window win, Atom channel,
        ...
        XChangeProperty(dpy, win, channel,                /* property */
...
void SendStartDrop(Display *dpy, Window src_win, drop_info_t *drop_info) {
        motif_drop_start_t info;
...
static int AtomCompare(const void *a1, const void *a2) {
        const Atom *atom1 = a1;
        const Atom *atom2 = a2;
...
static void GetTargetsList(Display *dpy, motif_targets_t **targets_list_p) {
        motif_targets_t *targets_list;
        Atom type;
        int format, status;
        unsigned long length, bytes_after;

        status = XGetWindowProperty(dpy, motif_drag_win, ATOM_MOTIF_TARGETS, 0L,
                                    GET_PROPERTY_MAX, False, ATOM_MOTIF_TARGETS,
                                    &type, &format, &length, &bytes_after,
                                    (unsigned char **)&targets_list);
        if ((status != Success) || (type != ATOM_MOTIF_TARGETS) ||
            (length == 0)) {
                /* allocate targets_list of zero targets */
                targets_list =
                    (motif_targets_t *)malloc(sizeof(motif_targets_t));
...
static void MatchTargets(motif_targets_t *targets_list, Atom *atoms,
                         unsigned long atom_cnt, CARD16 *targets_index_p) {
        int target_index;
        CARD32 *data;
        ...
        num_target_lists =
            Swap2Bytes(byte_order, targets_list->num_target_lists);
        data = (CARD32 *)(targets_list + 1);
        for (target_index = 0; target_index < num_target_lists;
             target_index++) {
                ...
                if (num_sub_list_items == atom_cnt) {
                        for (sub_list_index = 0;
                             sub_list_index < num_sub_list_items;
                             sub_list_index++) {
                                /* address isn't long word aligned so copy one
                                 * byte at a time */
...
static void GetTargetsIndex(Display *dpy, unsigned char *in_data,
                            unsigned long atom_cnt, CARD16 *targets_index_p) {
        ...
        /* search existing targets list for a match, otherwise add our
           targets to the list */
        qsort((void *)atoms, (size_t)atom_cnt, (size_t)sizeof(Atom),
              AtomCompare);
        ...
        new_list = (motif_targets_t *)realloc(targets_list, new_size);
        ...
        *target_cnt = Swap2Bytes(byte_order, atom_cnt); /* target_cnt */
        ...
        XChangeProperty(dpy, motif_drag_win, ATOM_MOTIF_TARGETS,
                        ATOM_MOTIF_TARGETS, 8, PropModeReplace,
 ...
 */
static void ContinueHandleOlitTrigger(Display *dpy, unsigned char *data,
...
void ProcessOlitInitiatorConversion(XSelectionEvent *event,
                                    drop_info_t *drop_info) {
        Atom type;
        int format;
        unsigned long length, bytes_after;
        ...
        if (event->target == ATOM_SUN_DND_ACK)
                XChangeProperty(event->display, event->requestor,
                                event->property, event->target, 32,
                                PropModeReplace, NULL, 0);

        /* get OLIT targets
           sometimes type is TARGETS and sometimes type is ATOM so don't bother
           checking type */
        if ((event->target == ATOM_TARGETS) && event->property &&
            (XGetWindowProperty(
                 event->display, event->requestor, event->property, 0L,
                 GET_PROPERTY_MAX, False, AnyPropertyType, &type, &format,
                 &length, &bytes_after, (unsigned char **)&data) == Success) &&
            length) {

                ContinueHandleOlitTrigger(event->display, data, length,
                                          drop_info);
                XFree(data);
        }
}
...
static int CopyProperty(Display *dpy, Atom property, Window old_win,
                        Window new_win) {
        Atom type;
        int format;
        unsigned long length, bytes_after;
        ...
        if (property &&
            (XGetWindowProperty(dpy, old_win, property, 0L, GET_PROPERTY_MAX,
                                False, AnyPropertyType, &type, &format, &length,
                                &bytes_after,
                                (unsigned char **)&data) == Success) &&
            format) {
                XChangeProperty(dpy, new_win, property, type, format,
...
static int CopyTargets(Display *dpy, Atom prop, Window old_win,
                       Window new_win) {
        Atom type;
        int format;
        unsigned long length, bytes_after;
        ...
        if ((XGetWindowProperty(dpy, old_win, prop, 0L, GET_PROPERTY_MAX, False,
            ...
            format) {
                ...
                for (i = 0; i < (length / 2); i++, target_info++) {
                        DPRINTF(("CopyTargets: targets[%d]=%s\n", i,
                                 a2s(target_info->target)));
                        CopyProperty(dpy, target_info->property, old_win,
                                     new_win);
...
*/
void ForwardMultpleSelectionRequest(XSelectionRequestEvent *event,
                                    drop_info_t *drop_info) {
        int format, status;
        unsigned long length, bytes_after;
        unsigned char *data;
        ...
        if (CopyProperty(event->display, event->property, event->requestor,
                ...
                XConvertSelection(event->display, event->selection,
                                  event->target, event->property,
                                  event->requestor, event->time);
        }
}

/* tell the initiator to clean up and do internal cleanup */
void DndDone(Display *dpy, Atom status, drop_info_t *drop_info) {
        XConvertSelection(dpy, drop_info->source_handle, status,
                          drop_info->proxy_handle, drop_info->proxy_sel_req_win,
                          drop_info->time_stamp);

        /* XView needs this */
        if (status == ATOM_SUN_DND_DONE)
                XConvertSelection(
                    dpy, drop_info->source_handle, ATOM_SUN_SELECTION_END,
                    drop_info->proxy_handle, drop_info->proxy_sel_req_win,
                    drop_info->time_stamp);
}
...
void ProxyMain(Display *dpy, XEvent *event) {
        ...
        case MapNotify: {
                ...
                if (client_win) {
                        /* if client has olit dnd interest then attach motif dnd
                           interest to client */
                        if (GetAtomWindow(dpy, client_win,
                                          ATOM_DRAGDROP_INTEREST)) {
                                AdvertiseMotifDropSite(dpy, client_win);
                        }
                        /* look for PropertyChange because olit receivers
                           don't always declare ATOM_DRAGDROP_INTEREST at
                           MapNotify time
                           */
        ...
        case PropertyNotify:
                ...
                if (event->xproperty.atom == ATOM_DRAGDROP_INTEREST) {
                        ...
                        if ((motif_drag_win == event->xproperty.window) &&
                            (proxy_win !=
                             GetPropertyWindow(dpy, motif_drag_win,
        ...
        case SelectionRequest: {
                drop_info_t *drop_info;
#ifdef DEBUG
                printf("SelectionRequest, selection=%s, target=%s, prop=%s, "
                       "win=0x%lx, "
                       "time=%ld\n",
                ...
                if (event->xselectionrequest.target == ATOM_SUN_DND_DONE ||
                    (event->xselectionrequest.target ==
                     ATOM_SUN_SELECTION_END)) {
                         ...
                         * we know what to forward when receiving
                         * SelectionNotify.
                         *
                         * Note that olit protocol doesn't have a
                         * XmTRANSFER_FAILURE equivalence, the closest is
                         * _SUN_DRAGDROP_ERROR. We may want to consider it
                         * otherwise olit app will think the drop was
                         * successful...
                         */
                        drop_info->status = event->xselectionrequest.target;
                        DndDone(dpy, ATOM_SUN_DND_DONE, drop_info);
                } else if (event->xselectionrequest.target ==
                           ATOM_SUN_SELECTION_ERROR) {
...
#ifdef DEBUG
                printf("SelectionNotify, sel=%s, target=%s, prop=%s, w=0x%lx, "
                       "time=%ld\n",
                       a2s(event->xselection.selection),
                       a2s(event->xselection.target),
                       a2s(event->xselection.property),
                       event->xselection.requestor, event->xselection.time);
#endif
                {
                        drop_info_t *drop_info =
                            GetDropInfo(event->xselection.requestor);

                        if (drop_info == NULL ||
                            (event->xselection.time < drop_info->time_stamp))
                                /* got SelectionNotify of unknown selection
                                 * (source) */
                                break;

                        /* Trap these two targets, so that the destination side
                         * will not see selection-time-out */
                        if (event->xselection.target ==
                                ATOM_SUN_DND_DONE || /* olit->motif */
                            event->xselection.target ==
                                ATOM_MOTIF_SUCCESS) { /* motif->olit */
                        ...
                        /* if event->property is proxy_handle then the convert
                           request was from the proxy agent else the convert
                           request was from the receiver */
                        if (event->xselection.property ==
                                drop_info->proxy_handle ||
                            (event->xselection.property == ATOM_SUN_DND_ACK)) {
                                ProcessOlitInitiatorConversion(
                                    &event->xselection, drop_info);
                        } else {
                                if (ForwardConversion(&event->xselection,
                                                      drop_info)) {

                                        /* We are here ONLY if the target is
                                         * ATOM_MULTIPLE and the target list
                                         * contains either _SUN_DRAGDROP_DONE or
                                         * _SUN_SELECTION_END. Probably for
                                         * XView support?! */
                                        DndDone(dpy, ATOM_MOTIF_SUCCESS,
                                                drop_info);
                                }
                        }
                        break;
