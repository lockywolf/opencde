 ...
 */
void FlashRegion(Display *dpy, int s, GC gc) {
        XEvent e;
        Bool done = False;
        Window root = RootWindow(dpy, s);

        if (GrabSuccess != XGrabPointer(dpy, root, False, ButtonPressMask,
        ...
        do {
                XFillRectangle(dpy, root, gc, 0, 0, DisplayWidth(dpy, s),
                               DisplayHeight(dpy, s));
                XFlush(dpy);
...
void FlashDropSites(Display *dpy) {
                                   ...
                                   unsigned long *nitems)
{
        ...
        s = XGetWindowProperty(dpy, win, ATOM_DRAGDROP_INTEREST, 0L,
                               ...
                               &acttype, &actfmt, nitems, &remain, &data);
 ...
 */
Bool FindRecursively(Display *dpy, Window root, Window win, Window *pwin,
                     void **psite, unsigned long *plen, int *px, int *py) {
        ...
        if (data != NULL) {
                if (!XTranslateCoordinates(dpy, win, root, 0, 0, px, py,
                                           &junk)) {
                        fprintf(stderr,
        ...
        s = XGetWindowProperty(dpy, win, ATOM_WM_STATE, 0L, 1, False,
                               ATOM_WM_STATE, &acttype, &actfmt, &nitems,
                               &remain, &data);
...
#define MOTIF_RECEIVER_FLAG 0x80000000
                ...
                if (GetAtomWindow(dpy, win, ATOM_MOTIF_RECEIVER_INFO)) {
                        ...
                        CARD32 *data =
                            (CARD32 *)malloc(sizeof(CARD32) * DATA_LEN);

                        data[0] = DRAGDROP_VERSION;
                        data[1] = 1; /* site count */
                        data[2] =
                            proxy_win; /* event_window, xview needs this */
                        data[3] = win; /* site_id */
                        data[4] = MOTIF_RECEIVER_FLAG; /*flag to indicate motif
                                                          receiver */
                        ...
                        *psite = (void *)data;
                        *plen = DATA_LEN;
                        XTranslateCoordinates(dpy, win, root, 0, 0, px, py,
                                              &junk);
 ...
 */
Region MakeRegionFromRect(int x, int y, unsigned int w, unsigned int h) {
        XRectangle r;
        Region reg;
 ...
 * corner is at (0,0).
 */
Region GetWindowRegion(Display *dpy, Window win, Bool offset) {
        Window wjunk;
        int x, y;
 ...
 */
void SubtractWindowFromVisibleRegion(Display *dpy, Window win, Region visrgn) {
...
int xoff, yoff;
{
        ...
        if (array[cur] != DRAGDROP_VERSION) {
                fprintf(stderr,
                        "%s: unknown drop interest property version (%ld) on "
                        "0x%lx\n",
...
void FindDropSites(Display *dpy) {
        ...
        for (s = 0; s < ScreenCount(dpy); ++s) {
                ...
                if (XQueryTree(dpy, root, &junk, &junk, &children,
                               &nchildren) == 0) {
                        fprintf(stderr,
                                "%s: XQueryTree failed on root window 0x%lx\n",
                                ProgramName, root);
                        continue;
                }

                /*
                 * For each mapped, InputOutput, child-of-root window, look for
                 * a drop interest property.  This will be a forwarding interest
                 * property placed by the window manager.  Then, find the
                 * top-level window underneath this window and process its
                 * interest property.
                 */
                ...
                for (i = nchildren - 1; i >= 0; --i) {
                        if (XGetWindowAttributes(dpy, children[i], &attr) ==
                            0) {
                                fprintf(stderr,
                                        "%s: XGetWindowAttributes failed for "
                                        "window 0x%lx\n",
                        ...
                        if (foundtoplevel && sitedata != NULL) {
                                ...
                                if (fwdsitedata != NULL) {
                                        framergn = MakeRegionFromRect(
                                            ...
                                            attr.height);
                                        XIntersectRegion(framergn, visrgn,
                                                         framergn);
                                        toprgn =
                                            GetWindowRegion(dpy, topwin, False);
                                        XOffsetRegion(toprgn, xoff, yoff);
                                        XSubtractRegion(framergn, toprgn,
                                                        framergn);
                        ...
                        SubtractWindowFromVisibleRegion(dpy, children[i],
                                                        visrgn);
                }
 ...
 *	8k+7	flags
 */
void WriteSiteRectList(Display *dpy, Window win, Atom prop) {
        unsigned long *cur;
        unsigned long *array;
        drop_site_t *site;
        int numrects = 0;
...
int main(int argc, char **argv) {
        enum { XA_SUN_DRAGDROP_DSDM,
               XA_SUN_DRAGDROP_INTEREST,
               XA_SUN_DRAGDROP_SITE_RECTS,
               XAWM_STATE,
               NUM_ATOMS };
        static char *atom_names[] = {"_SUN_DRAGDROP_DSDM",
        ...
        if (win != None) {
                fprintf(stderr, "%s: another DSDM is already running\n",
                        ProgramName);
                exit(1);
        }
         ...
         * DO NOT start the window manager or the file manager in the
         * background.  This way, the initialization code for the two
         * applications is run sequentially.
         */
        if (argc == 1)
                if (fork() != 0)
        ...
        for (;;) {
                ...
                case SelectionRequest:
                        ...
                        if (e.xselectionrequest.owner != selwin) {
                                fprintf(stderr,
                                        "%s: got SelectionRequest on wrong "
                                        "window?\n",
                                        ProgramName);
                                break;
                        }
                        ...
                        if (e.xselectionrequest.target != ATOM_SITE_RECTS) {
                                fprintf(stderr,
                                        "%s: got SelectionRequest for unknown "
                                        "target\n",
                                        ProgramName);
                        ...
                        (void)XSendEvent(dpy, reply.xselection.requestor, False,
                                         0, &reply);
