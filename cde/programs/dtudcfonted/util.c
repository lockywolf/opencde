...
/*ARGSUSED*/
Widget CreateCaptionFrame(owner, name, labelstr, type, thickness) Widget owner;
String name;
String labelstr;
int type;
int thickness;
{
        ...
        n++;
        XtSetArg(args[n], XmNresizable, resource.capt_resize);
        n++;
        XtSetArg(args[n], XmNshadowType, type);
        n++;
        XtSetArg(args[n], XmNshadowThickness, thickness);
        n++;
        XtSetArg(args[n], XmNleftOffset, resource.capt_lftoff);
        n++;
        XtSetArg(args[n], XmNtopOffset, resource.capt_topoff);
        n++;
        XtSetArg(args[n], XmNrightOffset, resource.capt_rghoff);
        n++;
...
Widget CreatePixButton(owner, name, data) Widget owner;
String name;
RadioButt *data;
{
        Arg args[20];
        ...
        n = 0;
        XtSetArg(args[n], XmNborderWidth, 1);
        n++;
        ...
        for (i = 0; i < NUMPIX; i++) {
                ...
                n++;
                XtSetArg(args[n], XmNlabelPixmap, pix[i]);
                n++;
                ...
                n++;
                XtSetArg(args[n], XmNset, data->items[i].set);
                n++;
                XtSetArg(args[n], XmNwidth, 36);
                n++;
                XtSetArg(args[n], XmNheight, 36);
                n++;
                data->items[i].w =
                    XmCreateToggleButton(top, data->items[i].name, args, n);
...
XtPointer
#endif
    GetTextFieldValue(textf) TextField *textf;
{
        char *s1, *s2, *s3;
        if (textf->w2 == NULL) {
...
/*ARGSUSED*/
static void arrow_change(w, data) TextField *data;
{
...
/*ARGSUSED*/
void CreateTextField(owner, name, labelstr, data, maxlength) Widget owner;
String name;
String labelstr;
TextField *data;
int maxlength;
{
        ...
        n = 0;
        XtSetArg(args[n], XmNorientation, (XtArgVal)XmHORIZONTAL);
        n++;
        ...
        n = 0;
        XtSetArg(args[n], XmNcolumns, maxlength);
        n++;
        XtSetArg(args[n], XmNmaxLength, maxlength);
        n++;
        ...
        n = 0;
        XtSetArg(args[n], XmNlabelPixmap, arrow_pix);
        n++;
        XtSetArg(args[n], XmNlabelType, XmPIXMAP);
        n++;
        XtSetArg(args[n], XmNindicatorOn, False);
        ...
        XtAddCallback(code, XmNactivateCallback, (XtCallbackProc)code_input,
                      (XtPointer)textfield);
}
...
#else
    CreateMenuButtons(owner, buttons, buttons_cnt) Widget owner;
Button *buttons;
int buttons_cnt;
#endif
{
                ...
                for (i = 0; i < buttons_cnt; i++) {
                        if (LABEL(i) && *((char *)LABEL(i))) {
                                ...
                                if (strchr(LABEL(i), NMNIC(i))) {
                                        ...
                                        sprintf(buf, "%s(%c)", LABEL(i),
                                                NMNIC(i));
                                }
                                xms = XmStringCreateLocalized(buf);
                                XtSetArg(args[n], XmNlabelString, xms);
                                n++;
                                XtSetArg(args[n], XmNmnemonic, NMNIC(i));
                                n++;
                                XtSetArg(args[n], XmNsensitive, SENS(i));
                                n++;
                                buttons->w[i] = XmCreatePushButtonGadget(
                                    owner, "button", args, n);
                                if (buttons->items[i].cb) {
                                        XtAddCallback(
                                            buttons->w[i], XmNactivateCallback,
                                            buttons->items[i].cb,
                                            buttons->items[i].clientdata);
                                }
                                XmStringFree(xms);
                        } else {
                                n = 0;
                                XtSetArg(args[n], XmNseparatorType,
                                         XmSHADOW_ETCHED_IN);
                                n++;
                                XtSetArg(args[n], XmNmargin,
                                         resource.menu_margin);
                                n++;
                                buttons->w[i] = XmCreateSeparatorGadget(
                                    owner, "separater", args, n);
                        }
...
Widget
#if __STDC__
CreateDialogAndButtons(Widget owner, String name, void (*delcb)(), Button *btns,
                       int btns_cnt, Widget *pop)
#else
    CreateDialogAndButtons(owner, name, delcb, btns, btns_cnt,
                           pop) Widget owner;
String name;
void (*delcb)();
Button *btns;
...
#endif
{
        ...
        n = 0;
        XtSetArg(args[n], XmNautoUnmanage, resource.dia_tm_automng);
        n++;
        XtSetArg(args[n], XmNmarginWidth, resource.dia_tm_width);
        n++;
        XtSetArg(args[n], XmNmarginHeight, resource.dia_tm_height);
        n++;
        if (btns->itemcnt > 0) {
                cs1 = XmStringCreateLocalized(btns->items[0].label);
                XtSetArg(args[n], XmNokLabelString, cs1);
                n++;
        }
        if (btns->itemcnt > 1) {
                cs2 = XmStringCreateLocalized(btns->items[1].label);
                XtSetArg(args[n], XmNcancelLabelString, cs2);
                n++;
        }
        if (btns->itemcnt > 2) {
                cs3 = XmStringCreateLocalized(btns->items[2].label);
                XtSetArg(args[n], XmNhelpLabelString, cs3);
                n++;
        }
        XtSetArg(args[n], XmNnoResize, resource.dia_tm_resize);
        n++;
        XtSetArg(args[n], XmNminimizeButtons, resource.dia_tm_minimize);
        n++;
        XtSetArg(args[n], XmNtitle, name);
        n++;
        *pop = XmCreateTemplateDialog(toplevel, "dialog", args, n);
        if (delcb)
                XmAddWMProtocolCallback(XtParent(*pop), DeleteWindowAtom(),
                                        delcb, 0);
        n = 0;
        XtSetArg(arg[n], XmNmarginWidth, resource.dia_rw_width);
        n++;
        XtSetArg(arg[n], XmNmarginHeight, resource.dia_rw_height);
        n++;
        XtSetArg(arg[n], XmNspacing, resource.dia_rw_spacing);
        n++;
        ...
        if (btns->itemcnt > 0) {
                ...
                XtAddCallback(*pop, XmNokCallback, btns->items[0].cb,
                              (btns->items[0].clientdata != NULL)
                                  ? btns->items[0].clientdata
                                  : 0);
        }
        if (btns->itemcnt > 1) {
                if (!btns->items[1].cb)
                ...
                XtAddCallback(*pop, XmNcancelCallback, btns->items[1].cb,
                              (btns->items[1].clientdata != NULL)
                                  ? btns->items[1].clientdata
                                  : 0);
        }
        if (btns->itemcnt > 2) {
                if (!btns->items[2].cb)
                ...
                XtAddCallback(*pop, XmNhelpCallback, btns->items[2].cb,
                              (btns->items[2].clientdata != NULL)
                                  ? btns->items[2].clientdata
                                  : 0);
        }
        n = 0;
...
Widget GuiInitialize(app, class_name, ac, av) XtAppContext *app;
String class_name;
int *ac;
String av[];
{
...
Widget CreateDrawingArea(owner, name, width, height, proc, val) Widget owner;
String name;
int width;
int height;
void (*proc)();
int val;
{
        ...
        n = 0;
        XtSetArg(arg[n], XmNwidth, width);
        n++;
        XtSetArg(arg[n], XmNheight, height);
        n++;
        XtSetArg(arg[n], XmNresizePolicy, XmRESIZE_NONE);
        n++;
        XtSetArg(arg[n], XmNborderWidth, resource.draw_border);
        n++;
        XtSetArg(arg[n], XmNbackground, resource.pane_background);
        n++;
        XtSetArg(arg[n], XmNtopAttachment, (XtArgVal)XmATTACH_FORM);
        n++;
        XtSetArg(arg[n], XmNleftAttachment, (XtArgVal)XmATTACH_FORM);
        n++;
        XtSetArg(arg[n], XmNtopOffset, (XtArgVal)resource.draw_topoff);
        n++;
        XtSetArg(arg[n], XmNleftOffset, (XtArgVal)resource.draw_lftoff);
        n++;
...
void AddLeftAttachWidget(w, ref, offset) Widget w;
Widget ref;
int offset;
{
        XtVaSetValues(w, XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, ref,
                      XmNleftOffset, offset, NULL);
}

void AddLeftAttachForm(w, offset) Widget w;
int offset;
{
...
void AddTopAttachWidget(w, ref, offset) Widget w;
Widget ref;
int offset;
{
...
void AddTopAttachForm(w, offset) Widget w;
int offset;
{
        XtVaSetValues(w, XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, offset,
                      NULL);
}

void AddRightAttachWidget(w, ref, offset) Widget w;
Widget ref;
int offset;
{
        XtVaSetValues(w, XmNrightAttachment, XmATTACH_WIDGET, XmNrightWidget,
                      ref, XmNrightOffset, offset, NULL);
}

void AddRightAttachForm(w, offset) Widget w;
int offset;
{
...
void AddBottomAttachForm(w, offset) Widget w;
int offset;
{
...
void ForcePopdownDialog(w) Widget w;
{
        if (XtIsManaged(w)) {
                XtUnmanageChild(w);
        }
...
void SetLabelString(w, str) Widget w;
String str;
{
        XmString cs;
...
void SetFooterString(w, str) Widget w;
String str;
{
        XmString cs;
        cs = XmStringCreateLocalized(str);
...
static void format_str(st, charcnt, str) Widget st;
int charcnt;
char *str;
{
        int i;
...
static void _destroy(w) Widget w;
{
        if (w) {
                XtDestroyWidget(w);
        }
...
/*ARGSUSED*/
void PopupNotice(owner, message, type, button, do_format, title) Widget owner;
char *message;
unsigned char type;
NButton *button;
Boolean do_format;
String title;
{
        ...
        n++;
        XtSetArg(args[n], XmNnoResize, resource.pop_resize);
        n++;
        XtSetArg(args[n], XmNminimizeButtons, resource.pop_minimize);
        n++;
        ...
        if (button->itemcnt > 0) {
                ...
                if (button->items[0].cb)
                        XtAddCallback(notice, XmNokCallback,
                                      button->items[0].cb, NULL);
        ...
        if (button->itemcnt > 1) {
                ...
                if (button->items[1].cb)
                        XtAddCallback(notice, XmNcancelCallback,
                                      button->items[1].cb, NULL);
        }
        if (button->itemcnt > 2) {
                ...
                if (button->items[2].cb)
                        XtAddCallback(notice, XmNhelpCallback,
                                      button->items[2].cb, NULL);
                XtAddCallback(help, XmNactivateCallback, (XtCallbackProc)_unmap,
                              NULL);
        }
        ...
        if (do_format)
                ...
                XtVaSetValues(notice, XmNmessageString, (XtArgVal)cs1,
                              (String)0);
                XtVaSetValues(notice, XmNmessageAlignment, XmALIGNMENT_CENTER,
                              (String)0);
                XmStringFree(cs1);
        }
...
/*ARGSUSED*/
static void _layout_centerEH(w, clientdata) Widget w;
XtPointer clientdata;
{
        ...
        XtVaGetValues(XtParent(w), XmNwidth, &bbw, XmNmarginWidth, &mw,
                      XmNshadowThickness, &sw, NULL);
        XtVaGetValues(child[0], XmNwidth, &cw, NULL);
        XtVaSetValues(w, XmNwidth, ((int)bbw - 2 * ((int)mw + (int)sw)), NULL);
        XtVaSetValues(child[0], XmNx,
                      ((((int)bbw - 2 * ((int)mw + (int)sw)) - (int)cw) / 2),
                      NULL);
}

/*ARGSUSED*/
Widget CreateTemplateDialog(w, message, type, button, title, pop) Widget w;
char *message;
unsigned char type;
NButton *button;
String title;
Widget *pop;
{
        ...
        XtSetArg(args[n], XmNnoResize, resource.temp_resize);
        n++;
        XtSetArg(args[n], XmNminimizeButtons, resource.temp_minimize);
        n++;
        XtSetArg(args[n], XmNdialogStyle, XmDIALOG_FULL_APPLICATION_MODAL);
        n++;
        XtSetArg(args[n], XmNmessageString, cs);
        n++;
        ...
        *pop = XmCreateTemplateDialog(w, "TemplateDialog", args, n);
        ...
        if (button->itemcnt > 1 && button->items[1].cb)
                XtAddCallback(*pop, XmNcancelCallback, button->items[1].cb,
                              NULL);
        ...
        n = 0;
        XtSetArg(args[n], XmNwidth, resource.temp_width);
        n++;
        XtSetArg(args[n], XmNheight, resource.temp_height);
        n++;
        brtnb = XmCreateBulletinBoard(*pop, "BulletinBo", args, n);
        XtManageChild(brtnb);
        XtAddEventHandler(brtnb, StructureNotifyMask, True,
                          (XtEventHandler)_layout_centerEH, NULL);
        return (brtnb);
}

void AddDeleteProc(w, delcb) Widget w;
void (*delcb)();
{
...
void AddPopupProc(w, popupcb) Widget w;
void (*popupcb)();
{ XtAddCallback(XtParent(w), XmNpopupCallback, popupcb, 0); }

void AddDestroyProc(w, destroycb) Widget w;
void (*destroycb)();
{ XtAddCallback(XtParent(w), XmNdestroyCallback, destroycb, 0); }

Widget CreateMenuBarAndFooterMessageForm(owner, name, buttons, bcnt, pop,
                                         footer) Widget owner;
String name;
MButton *buttons;
int bcnt;
...
Widget *footer;
{
        ...
        n = 0;
        XtSetArg(arg[n], XmNiconName, name);
        n++;
        XtSetArg(arg[n], XmNdeleteResponse, XmUNMAP);
        n++;
        XtSetArg(arg[n], XmNmwmFunctions,
                 (MWM_FUNC_MOVE | MWM_FUNC_MINIMIZE | MWM_FUNC_CLOSE));
        n++;
        *pop = XtCreatePopupShell(name, applicationShellWidgetClass, owner, arg,
                                  n);

        n = 0;
        XtSetArg(arg[n], XmNshowSeparator, True);
        n++;
        ...
        for (i = 0; i < bcnt; i++) {
                n = 0;
                XtSetArg(arg[n], XmNtearOffModel, XmTEAR_OFF_ENABLED);
                n++;
                buttons->items[i].menu =
                    XmCreatePulldownMenu(menuBar, "menu", arg, n);
                n = 0;
                if (strchr(buttons->items[i].label,
                           buttons->items[i].mnemonic)) {
                ...
                cs = XmStringCreateLocalized(buf);
                XtSetArg(arg[n], XmNmnemonic, buttons->items[i].mnemonic);
                n++;
                XtSetArg(arg[n], XmNlabelString, cs);
                n++;
                XtSetArg(arg[n], XmNsubMenuId, buttons->items[i].menu);
                n++;
        ...
        n = 0;
        XtSetArg(arg[n], XmNshadowType, XmSHADOW_IN);
        n++;
        ...
        n = 0;
        XtSetArg(arg[n], XmNtopAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(arg[n], XmNbottomAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(arg[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(arg[n], XmNrightAttachment, XmATTACH_FORM);
        n++;
        ...
        XtSetArg(arg[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(arg[n], XmNmarginTop, resource.ed_msg_margintop);
        n++;
        XtSetArg(arg[n], XmNmarginBottom, resource.ed_msg_marginbtm);
        n++;
        ...
        XtSetArg(arg[n], XmNrightAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(arg[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(arg[n], XmNleftWidget, footerLabel);
        n++;
        XtSetArg(arg[n], XmNalignment, XmALIGNMENT_BEGINNING);
        n++;
        XtSetArg(arg[n], XmNmarginTop, resource.ed_mesg_margintop);
        n++;
        XtSetArg(arg[n], XmNmarginBottom, resource.ed_mesg_marginbtm);
        n++;
        XtSetArg(arg[n], XmNlabelString, cs);
        n++;
...
Widget GetMenuWidget(buttons, buttons_num) MButton *buttons;
int buttons_num;
{ return (buttons->items[buttons_num].menu); }

Widget CreateForm(owner, name) Widget owner;
String name;
{
        Widget form;
        int n;
...
/*ARGSUSED*/
Widget CreateLabel(owner, name, str) Widget owner;
String name;
String str;
{
        Widget label;
...
/*ARGSUSED*/
Widget CreateFrame(owner, name, type, thickness) Widget owner;
String name;
XtPointer type;
XtPointer thickness;
{
        ...
        n = 0;
        XtSetArg(args[n], XmNresizable, resource.frame_resize);
        n++;
        XtSetArg(args[n], XmNshadowType, type);
        n++;
        XtSetArg(args[n], XmNshadowThickness, thickness);
...
/*ARGSUSED*/
Widget CreateRowColumn(owner, name, layout, space, marginw,
                       marginh) Widget owner;
String name;
int layout;
int space;
int marginw;
int marginh;
{
        ...
        n = 0;
        XtSetArg(args[n], XmNorientation, layout);
        n++;
        XtSetArg(args[n], XmNspacing, space);
        n++;
        XtSetArg(args[n], XmNmarginWidth, marginw);
        n++;
        XtSetArg(args[n], XmNmarginHeight, marginh);
        n++;
...
/*ARGSUSED*/
Widget CreateScrollBar(owner, name, height, val, min, max, proc) Widget owner;
String name;
int height;
int val;
int min;
...
void (*proc)();
{
        ...
        n = 0;
        XtSetArg(arg[n], XmNsliderSize, (XtArgVal)val);
        n++;
        XtSetArg(arg[n], XmNpageIncrement, (XtArgVal)val);
        n++;
        XtSetArg(arg[n], XmNmaximum, (XtArgVal)max);
        n++;
...
/*ARGSUSED*/
static void _scbase_cb(w, proc, calldata) Widget w;
void (*proc)();
XmScrollBarCallbackStruct *calldata;
{ (*proc)(calldata->value); }

Widget CreateScrollBase(owner, name, min, max, val, vcnt, sbproc) Widget owner;
String name;
int min;
int max;
int val;
...
void (*sbproc)();
{
        ...
        n = 0;
        XtSetArg(arg[n], XmNwidth, resource.scll_fr_width);
        n++;
        ...
        XtSetArg(arg[n], XmNshadowType, XmSHADOW_IN);
        n++;
        XtSetArg(arg[n], XmNshadowThickness, resource.scll_fr_thick);
        n++;
        XtSetArg(arg[n], XmNbottomAttachment, XmATTACH_FORM);
        n++;
        ...
        if (vcnt < (max - min)) {
                n = 0;
                XtSetArg(arg[n], XmNsliderSize, vcnt);
                n++;
                XtSetArg(arg[n], XmNminimum, min);
                n++;
                XtSetArg(arg[n], XmNmaximum, max);
                n++;
                XtSetArg(arg[n], XmNvalue, val);
                n++;
                XtSetArg(arg[n], XmNleftAttachment, XmATTACH_WIDGET);
                n++;
                XtSetArg(arg[n], XmNleftWidget, rwclm);
                n++;
                XtSetArg(arg[n], XmNleftOffset, resource.scll_bar_lftoff);
                n++;
                XtSetArg(arg[n], XmNtopAttachment, XmATTACH_FORM);
                n++;
                XtSetArg(arg[n], XmNbottomAttachment, XmATTACH_FORM);
