...
char *argv[];
{
        ...
        if (getarg(argc, argv, fname, &bitorder, &byteorder, &scanunit,
                   &glyphPad, &outLevel, &fix) != 0) {
                exit(-1);
        }
        ...
        if (!isPcf) {
                ...
                glyphPad = (((fip->maxbounds.metrics.rightSideBearing -
                              fip->maxbounds.metrics.leftSideBearing) +
                             31) /
                            32) *
                           4;
                ...
                fpropvnPtr =
                    ((char *)fpropPtr) + (fip->nProps) * sizeof(FontPropRec);
        }
        ...
        if (isPcf) {
                ...
                fontname = (char *)0;
                Anafprop(fip, fpropPtr, fpropvnPtr, &fontname, &psprop,
                         &resprop, &fplistPtr, &fplistNum);
        }
        ...
        if (isPcf) {
                ...
                *bufp = '\0';
                ...
                fprintf(stdout, "FONTBOUNDINGBOX %d %d %d %d\n",
                        ...
                        cip->metrics.leftSideBearing, -(cip->metrics.descent));
                pPROPS(fip, fplistPtr, fplistNum);
                if (outLevel == FALSE) {
                        if (fix == TRUE) {
                                pfixCHARS(fip, cip, glyphPtr, charNum,
                                          glyphPad);
                        } else {
 ...
 *
 */
static char *readfontfile(fname) char *fname;
{
        int fd;
        char *fp;
        struct stat stat;
        ...
        if (fstat(fd, &stat) == 0) {
                fp =
                    (char *)mmap(0, stat.st_size, PROT_READ, MAP_SHARED, fd, 0);
                if (fp == (char *)-1) {
...
static CARD32 getLSB32(p) unsigned char *p;
{
        CARD32 c;
...
static int getINT32(p, format) unsigned char *p;
CARD32 format;
{
        CARD32 c;
...
static int getINT16(p, format) unsigned char *p;
CARD32 format;
{
        CARD32 c;
...
static Bool seekToType(tables, ntables, type, formatp, sizep,
                       offsetp) PCFTablePtr tables;
int ntables;
CARD32 type;
CARD32 *formatp;
CARD32 *sizep;
...
static void getMetric(buf, format, metric) caddr_t buf;
CARD32 format;
xCharInfo *metric;
{
...
static Bool getAccel(pFontInfo, maxink, buf_top, tables, ntables,
                     type) FontInfoPtr pFontInfo;
xCharInfo *maxink;
caddr_t buf_top;
PCFTablePtr tables;
int ntables;
...
static Bool getProperties(pcf, buf_top, tables, ntables) PcfTmp *pcf;
caddr_t buf_top;
PCFTablePtr tables;
int ntables;
{
        ...
        if (!seekToType(tables, ntables, (CARD32)PCF_PROPERTIES, &format, &size,
                        &offset)) {
                return FALSE;
        }
        ...
        if (!(pcf->props =
                  (FontPropPtr)malloc(pcf->nprops * sizeof(FontPropRec)))) {
                return FALSE;
        }
...
static PcfTmp *openPcfFont(fontname) char *fontname;
{
        PcfTmp *pcf_tmp;
        ...
        if ((format = getLSB32((unsigned char *)pcf_tmp->pcf_buffer)) !=
            PCF_FILE_VERSION) {
        ...
        if (!getAccel(&pcf_tmp->info, &maxink, pcf_tmp->pcf_buffer,
        ...
        if (!getProperties(pcf_tmp, pcf_tmp->pcf_buffer, pcf_tmp->tables,
                           pcf_tmp->ntables)) {
                USAGE("dtgpftobdf : getProperties error.\n");
                goto Bail;
        }
        if (!seekToType(pcf_tmp->tables, pcf_tmp->ntables, (CARD32)PCF_BITMAPS,
                        &format, &size, &offset)) {
        ...
        pcf_tmp->sizebitmaps = getINT32(
            (unsigned char *)&bitmapSizes[PCF_GLYPH_PAD_INDEX(format)], format);
        pcf_tmp->bmp_fmt = format;
        buffp += sizeof(*bitmapSizes) * GLYPHPADOPTIONS;
        pcf_tmp->bitmaps = buffp;
        buffp += pcf_tmp->sizebitmaps;

        if (!seekToType(pcf_tmp->tables, pcf_tmp->ntables,
        ...
        if (!PCF_FORMAT_MATCH(format, PCF_DEFAULT_FORMAT)) {
                USAGE("dtgpftobdf : "
                      "error.!PCF_FORMAT_MATCH(PCF_BDF_ENCODINGS)\n");
                goto Bail;
        }
         ...
         */
        if (!seekToType(pcf_tmp->tables, pcf_tmp->ntables, (CARD32)PCF_SWIDTHS,
                        &format, &size, &offset)) {
                goto Bail;
        }
         ...
         * get metrics
         */
        if (!seekToType(pcf_tmp->tables, pcf_tmp->ntables, (CARD32)PCF_METRICS,
                        &format, &size, &offset)) {
                goto Bail;
        }
        ...
        if (PCF_FORMAT_MATCH(format, PCF_DEFAULT_FORMAT)) {
                ...
                pcf_tmp->compress = 1;
                pcf_tmp->mtr_num =
                    (int)getINT16((unsigned char *)buffp, format);
                buffp += 2;
        } else {
                goto Bail;
        }
...
static void Anafprop(fip, fpropPtr, fpropvnPtr, fontname, ps, res, fplistPtr,
                     fplistNum) FontInfoRec *fip;
...
static void setProp(pcf, proptmp) PcfTmp *pcf;
PropTmp *proptmp;
{
        int i, fpnL;
        char *fpnp;
 ...
 *
 */
static void pSIZE(ps, res) FontPropRec *ps, *res;
{
        int k;
        float f;
...
static void pSIZE_pcf(proptmp) PropTmp *proptmp;
{
        int k;
        float f;
        char buf1[16], buf2[16];
...
static void pPROPS(fip, fplistPtr, fplistNum) FontInfoRec *fip;
FontPropRec *fplistPtr;
unsigned int fplistNum;
{
        FontPropRec *wkp;
...
static void pPROPS_pcf(pcf) PcfTmp *pcf;
{
        FontPropPtr wkp;
        int i, k;
 ...
 *
 */
static void pCHARS(fip, cip, glyphPtr, charNum, glyphPad) FontInfoRec *fip;
CharInfoRec *cip;
char *glyphPtr;
 ...
 *
 */
static void pfixCHARS(fip, cip, glyphPtr, charNum, glyphPad) FontInfoRec *fip;
CharInfoRec *cip;
char *glyphPtr;
unsigned int charNum;
unsigned int glyphPad;
...
static void putPtn(
    bits, width,
    height) unsigned char *bits; /* 1 byte boundary , no byte swap data */
int width, height;
{
        int bytewidth;
        int i, j;
...
static void ByteSwap(p, scan) char *p;
int scan;
{
        char w;
...
static void BitOrderInvert(buf, nbytes) register unsigned char *buf;
register int nbytes;
{
...
static void invertBits(src, format, width, height, dest) char *src;
CARD32 format;
int width, height;
char *dest;
{
        ...
        for (i = 0; i < height;
             i++, src += src_bytewidth, dest += dest_bytewidth) {
...
static void pCHARS_pcf(pcf, fix) PcfTmp *pcf;
int fix;
{
        char *bmp_ptn;
        char *bitmap;
        ...
        if (fix == TRUE) {
                ...
                if ((adj_hi = pcf->info.fontAscent -
                              pcf->info.maxbounds.metrics.ascent) > 0) {
                        width_bytes = 8 * PCF_GLYPH_PAD(pcf->bmp_fmt);
                        width_bytes =
                            ((width_bytes + bmp_width - 1) / width_bytes) *
                            PCF_GLYPH_PAD(pcf->bmp_fmt);
        ...
        for (encoding = 0; encoding < nencoding; encoding++) {
                ...
                encodingOffset =
                    getINT16((unsigned char *)(pcf->encodingOffsets + encoding),
                             pcf->enc_fmt);
                ...
                fprintf(
                    ...
                    pcf->glyph_name +
                        getINT32(
                            (unsigned char *)&pcf->glyph_offset[encodingOffset],
                            pcf->glyph_fmt));
                fprintf(stdout, "ENCODING %d\n", (row << 8) + col);

                fprintf(
                    stdout, "SWIDTH %d 0\n",
                    getINT32((unsigned char *)(pcf->swidth + encodingOffset),
                             pcf->swd_fmt));
                ...
                if (pcf->compress) {
                        ...
                        bl = getINT16(
                            (unsigned char *)&pcf->metrics[encodingOffset]
                                .leftSideBearing,
                            pcf->mtr_fmt);
                        br = getINT16(
                            (unsigned char *)&pcf->metrics[encodingOffset]
                ...
                if (fix == FALSE) {
                        bmp_width =
                            ...
                            pcf->info.maxbounds.metrics.rightSideBearing;
                        bmp_height =
                            pcf->info.fontAscent + pcf->info.fontDescent;
                        ptn_width = bbw;
                        ptn_height = bbh;
                        ...
                        if ((adj_hi = pcf->info.fontAscent -
                                      pcf->info.maxbounds.metrics.ascent) > 0) {
                                width_bytes = 8 * PCF_GLYPH_PAD(pcf->bmp_fmt);
                                width_bytes = ((width_bytes + bmp_width - 1) /
                                               width_bytes) *
                                              PCF_GLYPH_PAD(pcf->bmp_fmt);
                                bmp_adj = width_bytes * adj_hi;
                                ptn_adj = 0;
                                if ((cpy_height = bmp_height - adj_hi) >
                                    ptn_height)
                                        cpy_height = ptn_height;
                        } else if (adj_hi < 0) {
                                adj_hi *= -1;
                                ...
                                bmp_adj = 0;
                                ptn_adj = width_bytes * adj_hi;
                                if ((cpy_height = ptn_height - adj_hi) >
                                    bmp_height)
                                        cpy_height = bmp_height;
                        } else {
                                bmp_adj = 0;
                ...
                bitmap =
                    pcf->bitmaps +
                    getINT32((unsigned char *)(pcf->offsets + encodingOffset),
                             pcf->bmp_fmt);
                invertBits(bitmap + bmp_adj, pcf->bmp_fmt, ptn_width,
                           cpy_height, bmp_ptn + ptn_adj);
...
static char *make_toptn(width, height) int width, height;
{
        int byte_width;
...
static int getarg(argc, argv, fname, bitorder, byteorder, scanunit, glyphPad,
                  outLevel, fix) int argc;
char *argv[];
char *fname;
int *bitorder;
int *byteorder;
        ...
        for (i = 1; i < argc; i++) {
                ...
                if (*arg == '-') {
                        *arg++;
                        ...
                        case 'p':
                                *glyphPad = atoi(arg + 1);
                                if (*glyphPad != 1 && *glyphPad != 2 &&
                                    *glyphPad != 4 && *glyphPad != 8) {
                                        USAGE("dtgpftobdf : illegal padding "
                                              "number (1/2/4/8)\n");
                                        return -1;
                                }
                                break;
