...
static int GetSize(size_str, body,
                   letter) char *size_str; /* a character of a size */
...
#else
    falcom_atoi(str, ed_code, val) char *str; /* numerical character */
char ed_code; /* an end code in numerical character */
int *val;     /* set numarical data */
#endif
{
...
static int falcom_cr_to_null(buf) char *buf;
{
        buf = strchr(buf, '\n');
        if (buf != NULL) {
                *buf = '\0';
...
#else
    GetDefaultFile(size, style, fname) int size; /* character size */
...
#endif
{
        ...
        GETLISTFILEPATH(locale, fal_search_path, uflist_d, uflist_c, uflist_l,
                        UDFLIST_DEF);
...
#endif
{
        ...
        while (fgets(buf, BUFSIZE, fp) != (char *)NULL) {
                ...
                if (style) {
                        if ((size_tmp == size) &&
                            !strcmp(elm[FALCOM_ELM_STYLE], style)) {
...
static int falcom_split_data(
    buf, entry_num,
    elm) char *buf; /* read a font information character array from a file */
int entry_num;
char *elm[];
{
        int cnt;        /* counter */
...
#else
    falcom_get_base_name(
        str) char *str; /* a full path character of a file name */
#endif
{
        char *str_slash;
...
static int
    is_letter_size_not_only_one(lst) FalFontDataList *lst; /* a font list */
{
        int i, letter_cmp;
...
GetFileName(
    ...
    GetFileName(com, size_str, style, codeset, ofile) char
        *com;   /* a character of a command name(case error) */
...
#endif
{
        ...
        if ((size_sw = GetSize(size_str, &body, &letter)) == -1) {
                USAGE2("%s : The specification of the character size is "
                       "improper.\"%s\"\n",
                       com, size_str);
                return -1;
        }
        ...
        if (lst->num == 0) {
                USAGE3("%s : The user defined character of the specified size "
                       "and style does not exist.\"%s , %s\"\n",
        ...
        case BODY_ONLY:
                ...
                if (is_letter_size_not_only_one(lst)) {
                        ...
                        if (ret < 0) {
                                COMM_GET_DEFAULTFILE_ERROR(
                                    buf, com, ret, key.size.h, style, uflist_d,
                                    uflist_c, uflist_l, statbuf);
                        }
                        for (i = 0; i < lst->num; i++) {
                                ret =
                                    search_Font_File_Name(lst->list[i], fname);
                                if (ret == FAL_ERROR) {
                                        FalFreeFontList(lst);
                                        return -1;
                                }
        ...
        case BODY_AND_LETTER:
                ...
                default:
                        ...
                        for (i = 0; i < lst->num; i++) {
                                ret =
                                    search_Font_File_Name(lst->list[0], fname);
                                if (ret == FAL_ERROR) {
                                        FalFreeFontList(lst);
                                        return -1;
                                }
                        ...
                        USAGE1("%s : Two or more corresponding user defined "
                               "characters exist.\n",
                               com);
                        for (i = 0; i < lst->num; i++) {
                                ret =
...
char *fal_errmsg_org[0xff] = {
    /* 0x00 */
    " ", "Cannot open the font file.", "Cannot read the font file.",
    "There is no more memory .", "Fatal error occurred.",
    "The specified font file does not exist.",
    "This font is not a pcf or snf format.", "Cannot open fonts.list file.",
    "The format of fonts.list file is illegal.",
    "The descriptions of the fonts.list file are incorrect.",
    "The format of fonts.list file is illegal.", "Cannot open fonts.dir file.",
    "Cannot read fonts.dir file.", "Cannot read font properties.",
    "Cannot get \"FONT\" property.", "Cannot get \"FAMILY_NAME\" property.",
    /* 0x10 */
    "This font file is already opened by other application.",
    "Cannot lock font file.", "Cannot unlock font file.",
    "Cannot get lock information from the font file.",
    "Cannot find the specified font file.", "Cannot read NLS database.",
    "Cannot get charset names from NLS database.",
    "Charset name not defined in NLS database.",
    "The specified font has not been opened.", "Fatal error occurred.", " ",
    " ", " ", " ", " ", " ",
    /* 0x20 */
    " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ",
    " ",
...
#else
    ErrMsgTable_FalGetFontList(com, utyerr,
                               utyderr) char *com; /* command name */
...
#endif
{
        ...
        case 1:
                if (fal_utyerrno) {
                        USAGE2("%s : %s\n", com,
                               fal_errmsg_org[fal_utyerrno & 0xff]);
                        return;
                }
                USAGE4("%s : The font directory information file(%s/%s/%s) "
                       "cannot be opened.\n",
                       com, fontsdir, locale, FONTS_LIST_FILE);
                break;
        case 2:
                USAGE4("%s : Information cannot be read from the font "
                       "directory information file(%s/%s/%s).\n",
                       com, fontsdir, locale, FONTS_LIST_FILE);
                break;
        case 3:
                ...
                break;
        case 7:
                USAGE4("%s :  Abnormality is found in the content of the font "
                       "directory information file((%s/%s/%s).\n",
                       com, fontsdir, locale, FONTS_LIST_FILE);
                break;
        case 8:
...
static void ErrMsgTable_GetDefaultFile(com, ret, size, style,
                                       fname) char *com; /* a command name */
int ret;                                                 /* return code */
...
char *fname; /* user defined character information file name */
{
        switch (ret) {
        case -1:
                USAGE2("%s : The user defined character information file "
                       "cannot be opened. \"%s \"\n",
                       com, fname);
                break;
        case -2:
                USAGE4("%s : The representative user defined character of the "
                       "character size (%d) and style (%s) is not "
                       "defined.\"%s\"\n",
                       com, size, (style) ? style : " ", fname);
                break;
        case -3:
                USAGE2("%s : Information cannot be read from the user defined "
                       "character information file.\"%s\"\n",
                       com, fname);
                break;
        case -4:
                USAGE2("%s : The mistake is found in the description of the "
                       "user defined character information file. \"%s\"\n",
                       com, fname);
                break;
        }
}
...
#else
    fal_cut_tailslash(name) char *name;
#endif
{
        char *p;
...
#else
    GetRealFileName(name) char *name;
#endif
{
...
#else
    IsInRegion(code, num_gr, gr) int code;
int num_gr;
FalGlyphRegion *gr;
#endif
{
        ...
        if (code < MIN_CODE || code > MAX_CODE)
                return -1;
        for (i = 0; i < num_gr; i++) {
                if (gr[i].start < MIN_CODE || gr[i].end > MAX_CODE)
                        continue;
...
static int search_Font_File_Name(data, fname) FalFontData data;
char *fname;
{
        FalFontID fid;
        ...
        if (fid == 0) {
                ...
                if (flist) {
                        ...
                        case _FAL_MALOC_ER:
                                ...
                                return (-2);
                        default:
                                USAGE3("There is no font file correspond to "
                                       "specified font.\n(%s)\n fal_utyerror = "
                                       "%d fal_utyderror = %d\n",
                                       ((data.xlfdname) ? data.xlfdname
                                                        : "null font name"),
                                       fal_utyerror, fal_utyderror);
                                return (-2);
                        }
        ...
        if (FalFontIDToFileName(fid, &tmp_fname) == FAL_ERROR) {
                USAGE3("There is no font file correspond to specified "
                       "font.\n(%s)\n fal_utyerror = %d fal_utyderror = %d\n",
                       ((data.xlfdname) ? data.xlfdname : "null font name"),
                       fal_utyerror, fal_utyderror);
                FalCloseFont(fid);
                return (-1);
        }
...
#else
    GetUdcFileName(com, code_no, xlfdname, fname) char *com;
int code_no;
char *xlfdname;
char *fname;
#endif
...
static void dsp_font_list(flist) FalFontDataList *flist;
{
        int i;
        ...
        for (i = 0; i < flist->num; i++) {
                USAGE4("\t%d\t%d\t%s\t%s\n",
                       ...
                       flist->list[i].size.h, flist->list[i].style.name,
...
#else
    GetUdcRegion(com, codeset, gpf_file, num_gr, gr) char *com;
int codeset;
char *gpf_file;
int *num_gr;
FalGlyphRegion **gr;
#endif
{
        ...
        if (FalGetUDCGIArea(locale, FALGETFALCODESET(codeset), char_set, gr,
                            num_gr) == FAL_ERROR) {
                USAGE3("%s : Failed to get UDC code region. ( code set %d "
                       "charset %s )\n",
...
#endif
{
        ...
        for (cd_set = 0; cd_set < MAX_CODESET; cd_set++) {
                ...
                if (FalGetUDCCPArea(locale, fls->list[0].cd_set, char_set, &cr,
                                    &num_cr) == FAL_ERROR) {
                        return -1;
                }
                if (num_cr == 0) {
...
#else
    GetUdcFontName(gpf_file, bdf_file, fontname) char *gpf_file;
char *bdf_file;
char **fontname;
#endif
{
