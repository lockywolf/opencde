...
#else
    ReadBdfHeader(head, buf) struct btophead *head;
char *buf;
#endif
{
        char *p;
        ...
        while (1) {
                ...
                if (!strncmp(p, SIZE, SIZEsz)) {
                        if ((sscanf(p, "SIZE %f%d", &(head->bdf_point),
                                    &(head->bdf_xdpi))) != 2) {
                                return (BDF_INVAL);
                        }
                        getstat |= 0x01;
...
#else
    ReadGpfHeader(head, buf) struct ptobhead *head;
char *buf;
#endif
{
        char *p;
...
#else
    WriteBdfHeader(head) struct ptobhead *head;
#endif
{
        FILE *fp;
        int fd[2];
...
#else
    WriteGpfHeader(head) struct btophead *head;
#endif
{
...
#else
    WritePtnToBdf(head) struct ptobhead *head;
#endif
{
        ...
        for (i = 0; i < head->num_chars; i++) {
                ...
                if (head->zoomf) {
                        ...
                        if ((rtn = PutBdfPtn(head->ptn[i], head->bdf_width,
                                             head->bdf_height, head->output))) {
                                return (rtn);
                        }
                }
...
#else
    putDefaultChars(head) struct ptobhead *head;
#endif
{
...
#else
    WritePtnToGpf(head) struct btophead *head;
#endif
{
        int msize, rtn, i;
        char *zoomptn;
        ...
        for (i = 0; i < head->num_chars; i++) {
                ...
                if (head->zoomf) {
                        if ((rtn = PtnZoom(zoomptn, head->ptn[i],
                                           head->bdf_width, head->bdf_height,
                                           head->p_width, head->p_height))) {
                                return (rtn);
                        }
                        ...
                        if ((rtn = PutGpfPtn(head->ptn[i], head->bdf_width,
                                             head->bdf_height, head->output))) {
                                return (rtn);
                        }
                }
...
#else
    ReadBdfToMemory(head, buf) struct btophead *head;
char *buf;
#endif
{
...
#else
    ReadBdfToMemory_with_init(head, init_start, init_end,
                              buf) struct btophead *head;
int init_start;
int init_end;
char *buf;
#endif
...
#else
    GetBdfCode(head, buf, code) struct btophead *head;
char *buf;
int *code;
#endif
{
...
#else
    GetBdfPtn(head, buf, ptn, mwidth, bsize) struct btophead *head;
char *buf;
char *ptn;
int mwidth;
int bsize;
...
#else
    ReadGpfToMemory(head, buf) struct ptobhead *head;
char *buf;
#endif
{
        int code, mwidth, num_char, bsize, rtn;
...
#else
    GetGpfCode(head, buf, code) struct ptobhead *head;
char *buf;
int *code;
#endif
{
...
#else
    GetGpfPtn(head, buf, ptn, mwidth, bsize) struct ptobhead *head;
char *buf;
char *ptn;
int mwidth;
int bsize;
...
#else
    GetBdfLinePtn(mem, buf, width) char *mem;
char *buf;
int width;
#endif
{
...
#else
    GetGpfLinePtn(mem, buf, width) char *mem;
char *buf;
int width;
#endif
{
...
#else
    PutBdfPtn(ptn, width, height, fp) unsigned char *ptn;
int width;
int height;
FILE *fp;
#endif
...
#else
    PutGpfPtn(ptn, width, height, fp) char *ptn;
int width;
int height;
FILE *fp;
#endif
...
#else
    PtnZoom(dmem, smem, sw, sh, dw, dh) char *dmem;
char *smem;
int sw;
int sh;
int dw;
...
static void exline(sp, dbuf, sw, dw) char *sp;
char *dbuf;
int sw;
int dw;
{
...
#else
    Link_NewFile(rd_file, wr_file) char *rd_file;
char *wr_file;
#endif
{
        FILE *rfp, *wfp;
...
#if defined(SVR4)
        if ((buf = (char *)mmap(0, statbuf.st_size, PROT_READ, MAP_SHARED, rfd,
                                0)) == (char *)-1) {
#endif /* SVR4 */
...
#endif
{
        ...
        case BDF_OPEN_IN:
                USAGE2(
                    "%s : The input font file cannot be opened.\"%s\"\A1\A5\n",
        ...
        case BDF_OPEN_OUT:
                USAGE2(
                    "%s : The output font file cannot be opened.\"%s\"\A1\A5\n",
        ...
        case BDF_WRITE:
                USAGE2("%s : It is not possible to write to the font "
                       "file.\"%s\"\A1\A5\n",
                       prog_name, (snf_out != NULL) ? snf_out : "\0");
                rtn = DEVICE_FAIL;
                break;
        ...
        case BDF_INVAL:
                USAGE2("%s : The format of the font file is "
                       "illegal.\"%s\"\A1\A5 \n",
                       prog_name, (snf_in != NULL) ? snf_in : "\0");
                rtn = 1;
                break;
        ...
        case BDF_READ_HEAD:
                USAGE2("%s : Information of the BDF file cannot be "
                       "extracted.\"%s\"\A1\A5 \n",
                       prog_name, (bdf_in != NULL) ? bdf_in : "\0");
                rtn = READ_FAIL;
                break;
        ...
        case GPF_OPEN_IN:
                USAGE2("%s : The input character pattern file cannot be "
                       "opened.\"%s\"\A1\A5\n",
        ...
        case GPF_READ:
                USAGE2("%s : The character pattern file cannot be "
                       "read.\"%s\"\A1\A5\n",
                       prog_name, (gpf_in != NULL) ? gpf_in : "\0");
                rtn = READ_FAIL;
                break;
        ...
        case GPF_WRITE:
                USAGE2("%s : It is not possible to write to the character "
                       "pattern file.\"%s\"\A1\A5\n",
                       prog_name, (gpf_out != NULL) ? gpf_out : "\0");
                rtn = DEVICE_FAIL;
                break;
...
#else
    fatal_err_msg(er_no, prog_name) int er_no;
char *prog_name;
#endif
{
...
#else
    Put_File_Create_Err_Msg(msg_level, org_name, tmp_name, save_name,
                            com) int msg_level;
char *org_name;
char *tmp_name;
char *save_name;
...
#endif
{
        ...
        case GPF_BUCK_UP_FAIL:
                USAGE1("%s : Failed to write temporary file. Terminates "
                       "abnormally.\n",
                       com);
        ...
        case GPF_MODIFY_FAIL:
                USAGE1("%s : Failed in the renewal of the font file. "
        ...
        case RESTORE_FAIL:
                USAGE3("%s : The file before updating cannot be changed to "
                       "former file name.\tPlease execute the following "
                       "command after the inquiry the system manager.\n\t mv "
                       "%s %s\n",
...
#else
    Make_NewFefFile(org_name, tmp_name, mode, owner, group, com) char *org_name;
char *tmp_name;
mode_t mode;
uid_t owner;
gid_t group;
...
#else
    Unlink_Tmpfile(file, com) char *file;
char *com;
#endif
{
        errno = 0;
        if (unlink(file) != 0) {
                USAGE2("%s : The work file cannot be deleted. Please unlink "
                       "the following files\tafter the inquiry the system "
                       "manager.\"%s\" \n",
                       com, file);
                return (1);
        }
...
#else
    Chmod_File(fname, mode, com) char *fname;
mode_t mode;
char *com;
#endif
{
        ...
        if (chmod(fname, mode) != 0) {
                USAGE3("%s : Failed in the mode change of the following files. "
                       "Please change the mode to %o\tafter the inquiry the "
                       "system manager.\"%s\" \n",
                       com, (int)mode, fname);
                return (1);
        }
...
#else
    Chown_File(fname, owner, group, com) char *fname;
uid_t owner;
gid_t group;
char *com;
#endif
{
        ...
        if (chown(fname, owner, group) != 0) {
                USAGE2("%s : Failed in setteing of the owner and the group of "
                       "the following files. \tPlease change to the file of "
                       "the owner and the group of following ID \tafter the "
                       "inquiry the system manager.\"%s\"\n",
                       com, fname);
...
#else
    ChkPcfFontFile(filename) char *filename;
#endif
{
        char *suffix;
...
#else
    ChkSnfFontFile(filename) char *filename;
#endif
{
        char *suffix;
...
#else
    get_cmd_path(path, cmd) char *path;
char *cmd;
#endif
{
        char *cmd_path;
                ...
                *end = chr_sv;
                ...
                if (stat(cmd_path, &st) == 0) {
                        if (st.st_mode & S_IFREG) {
                                cmd_path =
                                    realloc(cmd_path, strlen(cmd_path) + 1);
                                return cmd_path;
                        } else
                                continue;
                }
...
#endif
{
        ...
        if (stat(dflt_path, &statbuf)) {
                ...
                if (!(statbuf.st_mode & S_IXUSR)) {
                        USAGE2("%s: \"%s\" command don't have permission to "
                               "execute.\n",
                               com, cmd);
                        return STAT_ERROR;
                }
