...
char **argv;
{
        ...
        *create_font = '\0';
        ...
        for (i = 1; i < argc; i++) {
                if (*argv[i] != '-') {
                        ...
                        if ((i < argc - 1) && (*argv[i + 1] != '-')) {
                                ...
                                USAGE1("%s : The character width is not "
                                       "specified.\n",
                                       com);
                                exit(1);
                        }
                        ...
                        if ((i < argc - 1) && (*argv[i + 1] != '-')) {
                                OutputGpf.p_height =
                                    check_height(argv[0], argv[++i]);
                                if (OutputGpf.p_height == 0) {
                                        exit(1);
                                }
                        } else {
                                USAGE1("%s :  The character height is not "
                                       "specified.\n",
                                       com);
                                exit(1);
                        }
                        ...
                        if ((i < argc - 1) && (*argv[i + 1] != '-')) {
                                ...
                                USAGE1("%s : The extract start code is not "
                                       "specified.\n",
                                       com);
                                exit(1);
                        }
                        ...
                        if ((i < argc - 1) && (*argv[i + 1] != '-')) {
                                ...
                                USAGE1("%s : The extract end code is not "
                                       "specified.\n",
                                       com);
                                exit(1);
                        }
        ...
        if (no_infile) {
                if (no_infile == 1)
                        USAGE1("%s : The font file name for input cannot be "
                               "omitted.\n",
                               com);
                else
                        USAGE1("%s : The font file name for output cannot be "
                               "omitted.\n",
                               com);
                exit(1);
        }
         ...
         *****************************************************************/
        /* copy origin */
        if (getGpfFileName(com, &InputGpf, code_area, code_no, ixlfdname,
                           isnf_file, istyle)) {
                exit(1);
        }

        /* copy target */
        if (getGpfFileName(com, &OutputGpf, code_area, code_no, oxlfdname,
                           osnf_file, ostyle)) {
                exit(1);
        }
        ...
        if ((!stat(OutputGpf.in_file, &statbuf))) {
                if (GetUdcFontName(OutputGpf.in_file, NULL, &xlfdname)) {
                        USAGE2("%s : This font cannot get XLFD(%s). Terminates "
                               "abnormally.\n",
                ...
                if (!strcmp("p", spacing) || !strcmp("P", spacing)) {
                        USAGE2("%s : cannot edit proportional fonts.(SPACING "
                               "\"%s\")\n",
                               com, spacing);
                        exit(1);
                }
         ...
         *****************************************************************/
        if (!(code_area & SYSTM)) {
                if (GetUdcRegion(com, code_no, InputGpf.in_file, &num_gr,
                                 &gr)) {
                        USAGE1("%s : This font cannot get UDC code region. "
                ...
                num_gr = 1;
                if ((gr = (FalGlyphRegion *)malloc(sizeof(FalGlyphRegion) *
                                                   num_gr)) == NULL) {
        ...
        if (start_code && end_code) {
                if ((startcode = check_code(com, start_code, START_CODE,
                                            char_set, num_gr, gr)) < 0) {
                        exit(1);
                }
                if ((endcode = check_code(com, end_code, END_CODE, char_set,
                                          num_gr, gr)) < 0) {
                        exit(1);
                }

                if (startcode > endcode) {
                        USAGE1("%s : The specification of extract start/end "
                               "code is improper.\n",
                               com);
                        exit(1);
                }
                ...
                if (is_in_udcregion(startcode, endcode, num_gr, gr)) {
                        USAGE3("%s : The specified code \"%s-%s\" is outside "
                               "of the UDC code region.\n",
                               com, start_code, end_code);
                        USAGE("The UDC code region of specified font is showed "
                              "below.\n");
                        for (i = 0; i < num_gr; i++)
                                USAGE2("\t0x%x-0x%x\n", gr[i].start, gr[i].end);
                        exit(1);
        ...
        if ((targ_file = GetRealFileName(OutputGpf.in_file)) == NULL) {
                if (errno == ENOENT && (code_area & SYSTM)) {
                        /* create new file */
                        if ((chk_fd = open(OutputGpf.in_file, O_RDWR | O_CREAT,
                                           0644)) < 0) {
                                USAGE2("%s : The font file of substance \"%s\" "
                                       "cannot be create.\n",
                                       com, OutputGpf.in_file);
                                exit(1);
                        }
                        ...
                        USAGE2("%s : It is not possible to refer to the "
                               "substance of the font file. \"%s\"\n",
                               com, OutputGpf.in_file);
                        exit(1);
                }
        }
        ...
        if ((chk_fd = open(targ_file, O_RDWR)) < 0) {
                USAGE2("%s : The font file of substance \"%s\" cannot be "
                       "opened.\n",
                       com, targ_file);
                exit(1);
        }
        ...
        COMM_SBFNT_SETUDCAREA(argv[0], OutputGpf, char_set, code_area, num_gr,
                              gr);
        ...
        /* read copy origin */
        if ((rtn =
                 readGpf(&InputGpf, readbuf, startcode, endcode, num_gr, gr))) {
                if (WriteGpf.out_file) {
                        Unlink_Tmpfile(WriteGpf.out_file, com);
                }
                put_error_and_exit(&InputGpf, &WriteGpf, rtn, com);
        ...
        exit_code = Make_NewFefFile(WriteGpf.snf_file, WriteGpf.out_file,
                                    FONT_FILE_PARM, (uid_t)statbuf.st_uid,
                                    (gid_t)statbuf.st_gid, com);
...
static int getGpfFileName(com, gpfinf, code_area, code_no, xlfdname, snf_file,
                          style) char *com;
...
char *style;
{
        ...
        if (gpfinf->in_file == '\0' && xlfdname == '\0') {
                if (code_area & SYSTM) {
                        USAGE1("%s : The GPF output file name cannot be "
                               "omitted.\n",
                               com);
                } else { /* string of charcter size */
                        USAGE1("%s : The character size specification cannot "
                               "be omitted.\n",
                               com);
                }
                return -1;
        ...
        if (!(code_area & SYSTM)) {
                if (xlfdname) {
                        if (GetUdcFileName(com, code_no, xlfdname, snf_file)) {
                                USAGE2("%s : Failed to get font file.( XLFD: "
                        ...
                        switch (GetFileName(com, gpfinf->in_file, style,
                                            code_no, snf_file)) {
                        case 0:
                                gpfinf->in_file = snf_file;
                                break;
                        case -1:
                                return -1;
                        default:
                                USAGE1("%s : The font file name cannot be "
                                       "obtained. Terminates abnormally.\n",
                                       com);
                                return -1;
                        }
                }
...
static CpyGpfInit(i_gpf, o_gpf, w_gpf, type) struct btophead *i_gpf;
...
int type;
{
        ...
        if (!new_target) {
                if (ChkPcfFontFile(w_gpf->snf_file)) {
                        ...
                        if ((snf_fd = open(w_gpf->snf_file, O_RDONLY)) >= 0) {
                                COMM_SNF_FILEVERSION(snf_fd, finf, buf,
                                                     permission);
                                if (permission < 0) {
                                        return BDF_INVAL;
                                }
        ...
        if (ChkPcfFontFile(w_gpf->snf_file) == 0) {
                ...
                COMM_SNF_POPEN(permission, w_gpf->snf_file, w_gpf->out_file, ep,
                               buf, command);
        }
        ...
        switch (fork()) {
        case 0:
                if ((out_fd = open(w_gpf->out_file, O_WRONLY | O_CREAT, 0664)) <
                    0) {
                        return BDF_OPEN_OUT;
                }
                close(0);
...
static int readGpf(i_gpf, buf, startcode, endcode, num_gr,
                   gr) struct btophead *i_gpf;
char *buf;
int startcode;
int endcode;
FalGlyphRegion *gr;
...
static int readBdfHeaderAndPut(o_gpf, w_gpf, buf) struct btophead *o_gpf;
struct ptobhead *w_gpf;
char *buf;
{
        FILE *fp;
        ...
        while (1) {
                ...
                if (!strncmp(p, SIZE, strlen(SIZE))) {
                        if ((sscanf(p, "SIZE %f%d", &(o_gpf->bdf_point),
                                    &(o_gpf->bdf_xdpi))) != 2) {
                                fclose(fp);
                                return BDF_INVAL;
                        }
                        ...
                        if (zoom) {
                                /* -width / -height */
                                if (o_gpf->p_width)
                                        o_gpf->bdf_width = o_gpf->p_width;
                                if (o_gpf->p_height)
                                        o_gpf->bdf_height = o_gpf->p_height;
                ...
                } else if (!strncmp(p, "ENDPROPERTIES",
                                    strlen("ENDPROPERTIES"))) {
                        fprintf(fp, "%s", buf);
                        fclose(fp);
                        getstat |= 0x04;
                        break;
                        ...
                        if (zoom) {
                                if (!strncmp(p, "STARTPROPERTIES",
                                             strlen("STARTPROPERTIES"))) {
                                        property = 1;
                                        fprintf(fp, "%s", buf);
                                } else if (!strncmp(p, "FONT ",
                                                    strlen("FONT "))) {
                                        char font[BUFSIZE];
                                        if (property) {
                                                if ((sscanf(buf, "FONT \"%s\"",
                                                            font)) != 1) {
                                                        fclose(fp);
                                                        return BDF_INVAL;
                                                }
                                                MkFontProp(o_gpf->bdf_height,
                                                           font, create_font);
                                                fprintf(fp, "FONT \"%s\"\n",
                                                        create_font);
                                        } else {
                                                ...
                                                MkFontProp(o_gpf->p_height,
                                                           font, create_font);
                                                fprintf(fp, "FONT %s\n",
                                                        create_font);
                                        }
                                } else if (!strncmp(p, "PIXEL_SIZE ",
                                                    strlen("PIXEL_SIZE "))) {
                                        fprintf(fp, "PIXEL_SIZE %d\n",
                                                o_gpf->bdf_height);
                                } else if (!strncmp(p, "AVERAGE_WIDTH ",
                                                    strlen("AVERAGE_WIDTH "))) {
                                        fprintf(fp, "AVERAGE_WIDTH %d\n",
                                                o_gpf->bdf_height * 10);
                                } else if (!strncmp(p, "POINT_SIZE ",
                                                    strlen("POINT_SIZE "))) {
                                        fprintf(fp, "POINT_SIZE %d\n",
                                                o_gpf->bdf_height * 10);
                                } else if (!strncmp(p, "RESOLUTION ",
                                                    strlen("RESOLUTION "))) {
                                        int resol;
                                        ...
                                        fprintf(fp, "RESOLUTION %d\n",
                                                (int)((resol < 0)
                                                          ? DEF_RESOLUTION
                                                          : resol));
                                } else if (!strncmp(p, "X_HEIGHT ",
                                                    strlen("X_HEIGHT "))) {
                                        fprintf(fp, "X_HEIGHT %d\n",
                                                o_gpf->bdf_height);
                                } else if (!strncmp(p, "QUAD_WIDTH ",
                                                    strlen("QUAD_WIDTH "))) {
                                        fprintf(fp, "QUAD_WIDTH %d\n",
                                                o_gpf->bdf_width);
                                } else if (!strncmp(p, "FONT_ASCENT ",
                                                    strlen("FONT_ASCENT "))) {
                                        fprintf(
                                            fp, "FONT_ASCENT %d\n",
                                            (o_gpf->bdf_height + o_gpf->bdf_y));
                                } else if (!strncmp(p, "FONT_DESCENT ",
                                                    strlen("FONT_DESCENT "))) {
                                        fprintf(fp, "FONT_DESCENT %d\n",
                                                -(o_gpf->bdf_y));
                                } else if (!strncmp(p, "DEFAULT_CHAR ",
                                                    strlen("DEFAULT_CHAR "))) {
...
static writeGpf(i_gpf, o_gpf, w_gpf) struct btophead *i_gpf;
struct btophead *o_gpf;
struct ptobhead *w_gpf;
{
        FILE *fp;
...
static int check_width(prog_name, len_str) char *prog_name;
char *len_str;
{ return (check_width_height(prog_name, len_str, 0)); }

static int check_height(prog_name, len_str) char *prog_name;
char *len_str;
{ return (check_width_height(prog_name, len_str, 1)); }

static int check_width_height(prog_name, len_str, mode) char *prog_name;
char *len_str;
int mode; /* width ... 0 , height ... 1 */
{
        ...
        if ((tmp == len_str) || (ret < MIN_FONT_WIDTH) ||
            (ret > MAX_FONT_WIDTH)) {
                switch (mode) {
                case 0:
                        USAGE2("%s : The specification of the character width "
                               "is improper.\"%s\"．\n",
                               prog_name, len_str);
                        break;
                case 1:
                        USAGE2("%s : The specification of the character height "
                               "is improper.\"%s\"．\n",
                               prog_name, len_str);
                        break;
                default:
...
static int check_code(prog_name, code_str, mode, charset, num_gr,
                      gr) char *prog_name;
char *code_str;
...
FalGlyphRegion *gr;
{
        ...
        if (COMM_SBFNT_ISSBFNT(charset)) {
                ...
                if (fal_code_to_glyph(locale, code, &gi, &num_gi)) {
                        USAGE2("%s : Failed to convert code point into glyph "
                               "index.\"%s\" \n",
                               prog_name, code_str);
                        err_flg = 1;
                }
        ...
        if (err_flg) {
                switch (mode) {
                case 0:
                        USAGE2("%s : The specification of the extract start "
                               "code is improper.\"%s\"．\n",
                               prog_name, code_str);
                        break;
                case 1:
                        USAGE2("%s : The specification of the extract end code "
                               "is improper.\"%s\"．\n",
                               prog_name, code_str);
                        break;
                default:
                        USAGE2("%s : The specification of the extract code is "
                               "improper.\"%s\"．\n",
                               prog_name, code_str);
                }
                return (-1);
...
static int readBdfToMemory(head, buf, startcode, endcode, num_gr,
                           gr) struct btophead *head;
char *buf;
int startcode;
int endcode;
int num_gr;
FalGlyphRegion *gr;
{
        ...
        while (1) {
                ...
                if ((ptn = head->ptn[num_char++] = (char *)malloc(bsize)) ==
                    NULL) {
                        return (MALLOC_ERROR);
                }
...
static void getDefaultCh(code) unsigned int code;
{
        unsigned int cbuf;
        cbuf = code;
        if ((cbuf & 0xff) < firstCol)
                firstCol = cbuf & 0xff;
        if ((cbuf >> 8) < firstRow)
                firstRow = cbuf >> 8;
        if ((cbuf & 0xff) > lastCol)
                lastCol = cbuf & 0xff;
        if ((cbuf >> 8) > lastRow)
                lastRow = cbuf >> 8;
}

static int cpg_atoi(numstr) char *numstr;
{
        int val;
        char *ep, *cbuf;
...
static int is_in_udcregion(start, end, num_gr, gr) int start, end, num_gr;
FalGlyphRegion *gr;
{
        int i;
...
static void MkFontProp(font_height, font, output) int font_height;
char *font;
char *output;
{
        ...
        *av_width = '\0';
        ...
        sprintf(output, "%s-%d-%s-%d%s", fbuf, font_height, /* pixel size */
                rsl_x, font_height * 10,                    /* average width */
                char_set);
}

static void put_error_and_exit(ptob_in, ptob_out, er_no,
                               prog_name) struct btophead *ptob_in;
struct ptobhead *ptob_out;
int er_no;
char *prog_name;
{
        ErrMsgTable_AndExit(er_no, ptob_in->in_file, ptob_out->out_file, NULL,
                            NULL, NULL, prog_name);
        return;
}

static void put_help(prog_name) char *prog_name;
{
        ...
        USAGE("----------------------------------------\n");
        DispUdcCpArea();
        USAGE("If the -start and -end option is omitted, the start/end code of "
              "each extractive area is specified.\n");
        USAGE("The xlfd name and character size may be obtained using dtlsgpf "
              "command.\n");
        return;
}
