...
static char *default_string(lcd) XLCd lcd;
...
static Bool load_public(lcd) XLCd lcd;
{
...
static Bool initialize_core(lcd) XLCd lcd;
{
...
static void destroy_core(lcd) XLCd lcd;
{
        if (lcd->core) {
                if (lcd->core->name)
                        Xfree(lcd->core->name);
...
static char *get_values(lcd, args, num_args) register XLCd lcd;
...
register int num_args;
{
        ...
        return _fallcGetValues((XPointer)pub, resources, XlcNumber(resources),
