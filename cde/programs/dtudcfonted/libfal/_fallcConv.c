...
static XlcConv get_converter(from_lcd, from_type, to_lcd,
        ...
        for (list = conv_list; list; list = list->next) {
                if (list->from_lcd == from_lcd && list->to_lcd == to_lcd &&
                    list->from_type == from_type && list->to_type == to_type) {
                        ...
                        return (*list->converter)(from_lcd, list->from, to_lcd,
                                                  list->to);
                }
...
Bool _fallcSetConverter(from_lcd, from, to_lcd, to, converter) XLCd from_lcd;
char *from;
XLCd to_lcd;
char *to;
...
static int indirect_convert(lc_conv, from, from_left, to, to_left, args,
                            num_args) XlcConv lc_conv;
XPointer *from;
int *from_left;
XPointer *to;
int *to_left;
...
int num_args;
{
        ...
        while (*from_left > 0) {
                ...
                ret = (*from_conv->methods->convert)(
                    from_conv, from, from_left, &cs, &cs_left, tmp_args, 1);
                if (ret < 0)
                        break;
                ...
                ret = (*to_conv->methods->convert)(to_conv, &cs, &cs_left, to,
                                                   to_left, tmp_args, 1);
                if (ret < 0) {
...
static void close_indirect_converter(lc_conv) XlcConv lc_conv;
{
        Conv conv = (Conv)lc_conv->state;
...
static void reset_indirect_converter(lc_conv) XlcConv lc_conv;
{
        Conv conv = (Conv)lc_conv->state;
...
static XlcConv open_indirect_converter(from_lcd, from, to_lcd,
                                       to) XLCd from_lcd;
char *from;
XLCd to_lcd;
char *to;
{
        ...
        if (from_type == QCharSet || from_type == QChar ||
            to_type == QCharSet || to_type == QChar)
                return (XlcConv)NULL;
        ...
        if (from_conv == NULL)
                from_conv =
                    get_converter(from_lcd, from_type, from_lcd, QCharSet);
        if (from_conv == NULL)
                from_conv =
                    get_converter((XLCd)NULL, from_type, (XLCd)NULL, QCharSet);
        if (from_conv == NULL)
        ...
        if (to_conv == NULL)
                to_conv =
                    get_converter((XLCd)NULL, QCharSet, (XLCd)NULL, to_type);
        if (to_conv == NULL)
                goto err;
...
XlcConv _fallcOpenConverter(from_lcd, from, to_lcd, to) XLCd from_lcd;
char *from;
XLCd to_lcd;
char *to;
{
...
void _fallcCloseConverter(conv) XlcConv conv;
{ close_converter(conv); }

int _fallcConvert(conv, from, from_left, to, to_left, args,
...
int num_args;
{
        return (*conv->methods->convert)(conv, from, from_left, to, to_left,
                                         args, num_args);
}

void _fallcResetConverter(conv) XlcConv conv;
{
