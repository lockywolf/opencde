...
CARD32 *offsetp;
{
        ...
        for (i = 0; i < ntables; i++) {
                if (getLSB32((unsigned char *)&tables[i].type) == type) {
                        if (formatp)
                                *formatp = getLSB32(
                                    (unsigned char *)&tables[i].format);
                        if (sizep)
                                *sizep =
                                    getLSB32((unsigned char *)&tables[i].size);
                        if (offsetp)
                                *offsetp = getLSB32(
                                    (unsigned char *)&tables[i].offset);
                        return (TRUE);
                }
        }
        return (FALSE);
...
xCharInfo *metric;
{
        metric->leftSideBearing =
            getINT16((unsigned char *)buf, (CARD32)format);
        buf += 2;
...
CARD32 type;
{
        ...
        pFontInfo->fontAscent =
            getINT32((unsigned char *)buffer, (CARD32)format);
        buffer += 4;
        pFontInfo->fontDescent =
            getINT32((unsigned char *)buffer, (CARD32)format);
        buffer += 4;

        /*  pFontInfo->maxOverlap = getINT32( (unsigned char *)buffer, (CARD32)format); */
        buffer += 4;
...
int falInitReadPcf(pcfinf, buftop) struct pcf_inf *pcfinf;
caddr_t buftop;
{
        CARD32 format;
        CARD32 offset;
        ...
        if (!seekToType(pcfinf->tables, pcfinf->ntables,
                        (CARD32)PCF_BDF_ENCODINGS, &format, (CARD32 *)NULL,
                        &offset))
                goto Bail;
        ...
        pcfinf->info.firstCol =
            getINT16((unsigned char *)buffp, (CARD32)format);
        buffp += 2;
        pcfinf->info.lastCol = getINT16((unsigned char *)buffp, (CARD32)format);
        buffp += 2;
        pcfinf->info.firstRow =
            getINT16((unsigned char *)buffp, (CARD32)format);
        buffp += 2;
...
static void repadBits(src, format, width, height, dest) char *src;
CARD32 format;
int width, height;
char *dest;
{
...
int falPcfGlyph(glyph, finf, code) char *glyph;
Oak_FontInf *finf;
int code;
{
        int encode;
        ...
        repadBits(bitmap + bmp_adj, finf->pcfinf.bmp_fmt, finf->width,
                  cpy_height, glyph + ptn_adj);
        return (0);
}

void falGetPcfGSize(pcfinf, widthp, heightp) struct pcf_inf *pcfinf;
...
int falReadFontProp(file, protect_key_data, databuff,
                    islock) char *file; /* name of font file */
...
int islock;
{
         ...
         *	open GPF font file
         */
        rtn = falReadGpfProp(FAL_UPDATE_FONTINFO, &finf, protect_key_data,
                             databuff);
#if defined(SVR4)
        if (finf.ismmap == TRUE) {
 ...
 */
int falReadGpfProp(updflg, finf, protect_key_data, databuff) int updflg;
Oak_FontInf *finf;
int protect_key_data;
FalFontData *databuff;
{
        ...
        if (updflg == FAL_UPDATE_FONTINFO) {
                ...
                {
                        FAL_READ_SNF_HEADER(finf, protect_key_data,
        ...
        if (finf->isFef) {
                ...
                if (rtn = falInitReadSnfProp(finf, (caddr_t)finf->pFinf,
                                             databuff)) {
 ...
 */
int falInitReadPcfProp(updflg, finf, databuff) int updflg;
Oak_FontInf *finf;
FalFontData *databuff;
{
        ...
        if (updflg == FAL_UPDATE_FONTINFO) {
                ...
                if (!getAccel(&pcfinf->info, &maxink, buftop, pcfinf->tables,
                              pcfinf->ntables, (CARD32)PCF_BDF_ACCELERATORS)) {
                        if (!getAccel(&pcfinf->info, &maxink, buftop,
                                      pcfinf->tables, pcfinf->ntables,
        ...
        if (buffp = getPcfFontProp(buftop, pcfinf->tables, pcfinf->ntables,
                                   "FONT")) {
                if ((databuff->xlfdname = (char *)strdup(buffp)) ==
                    (char *)NULL) {
                        fal_utyerrno = FAL_ERR_MALLOC;
                        goto Bail;
                }
...
static char *getPcfFontProp(buftop, tables, ntables, propname) caddr_t buftop;
PCFTablePtr tables;
int ntables;
char *propname;
{
        ...
        for (i = 0; i < nprops; i++) {
                ...
                if (strcmp(propstr + name_ofs, propname) == 0) {
                        if (*buffer) { /* isStringProp */
                                return (propstr +
                                        getINT32((unsigned char *)(buffer + 1),
                                                 (CARD32)format));
                        } else {
 ...
 */
int falInitReadSnfProp(finf, buftop, databuff)
...
static char *getSnfFontProp(buftop, nprops, propname) caddr_t buftop;
int nprops;
char *propname;
{
        caddr_t buffer;
