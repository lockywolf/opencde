 ...
 */
static unsigned char getbyte(code, i) unsigned long code;
int i;
{
        unsigned long byteL;
        unsigned char byte;
...
fal_get_codeset(locale, charset_str, codeset, num_codeset) char *locale;
char *charset_str;
int **codeset;
int *num_codeset;
{
        ...
        for (num = 0;; num++) {
                ...
                _fallcGetLocaleDataBase(lcd, "XLC_FONTSET", buf, &value,
                                        &count);
                if (count < 1) {
                        break;
                }
...
static Bool gi_to_vgi(gi, vgi, scope) unsigned long gi, *vgi;
FontScope scope;
{
...
static void shift_area(udc, scope) FalGlyphRegion *udc;
FontScope scope;
{
        if (scope->shift_direction == (unsigned long)'+') {
...
/*
    ...
    charset_str     :  JISX0208.1983-0
*/
FalGetUDCGIArea(locale, codeset, charset_str, gr, num_gr) char *locale;
int codeset;
char *charset_str;
FalGlyphRegion **gr;
int *num_gr;
{
        ...
        *num_gr = count;
        ...
        if (count > 0) {
                ...
                for (i = 0; i < count; i++) {
                        if (!_fallcCompareISOLatin1(font_data[i].name,
                                                    charset_str)) {
                                for (j = 0; j < (*num_gr); j++) {
                                        for (k = 0; k < font_data[i].scopes_num;
                                             k++) {
                                                if (udc[j].start ==
                                                        font_data[i]
                                                            .scopes[k]
...
int fal_gi_to_vgi(lcd, locale, charset_str, codeset, gi, vgi,
                  charsetname) XLCd lcd;
char *locale;
char *charset_str;
int codeset;
unsigned long gi;
...
Bool falnon_standard(lcd, charset) XLCd lcd;
XlcCharSet charset;
{
        char buf[256];
        int count, i;
        ...
        for (i = 0;; i++) {
                ...
                _fallcGetLocaleDataBase(lcd, "XLC_CHARSET_DEFINE", buf, &value,
                                        &count);
                if (count > 0) {
                        if (!_fallcNCompareISOLatin1(value[0], charset->name,
                                                     strlen(value[0]))) {
                                return (True);
                        }
                } else {
...
void falmake_none_standard(from, charset, src) char *from;
XlcCharSet charset;
char *src;
{
...
int fal_glyph_to_code(locale, charset_str, codeset, glyph_index,
                      codepoint) char *locale;
char *charset_str;
int codeset;
...
unsigned long *codepoint;
{
        ...
        fal_gi_to_vgi(lcd, locale, charset_str, codeset, glyph_index,
                      &glyph_index, charsetname);
...
int FalGetUDCCPArea(locale, codeset, charset_str, cr, num_cr) char *locale;
int codeset;
char *charset_str;
FalCodeRegion **cr;
int *num_cr;
 ...
 *
 */
static Bool vgi_to_gi(gi, vgi, scope) unsigned long *gi, vgi;
FontScope scope;
{
 ...
 *
 */
static Bool fal_vgi_to_gi(lcd, locale, vglyph, glyph, charset,
                          charsetname) XLCd lcd;
char *locale;
...
int fal_code_to_glyph(locale, codepoint, gi, num_gi) char *locale;
unsigned long codepoint;
FalGIInf **gi;
int *num_gi;
{
