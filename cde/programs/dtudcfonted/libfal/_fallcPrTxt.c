...
static XPointer *alloc_list(is_wide_char, count, nitems) Bool is_wide_char;
int count;
int nitems;
{
        if (is_wide_char) {
...
static void copy_list(is_wide_char, text, list, count) Bool is_wide_char;
XPointer text;
XPointer *list;
int count;
{
...
static int _XTextPropertyToTextList(lcd, dpy, text_prop, to_type, list_ret,
                                    count_ret) XLCd lcd;
Display *dpy;
...
int *count_ret;
{
        ...
        if (encoding == XA_STRING)
                ...
                from_type = XlcNCompoundText;
                 ...
                 falInternAtom(dpy, XLC_PUBLIC(lcd, encoding_name), False))
        ...
        while (1) {
                if (nitems == 0 || *str_ptr == 0) {
                        ...
                        ret = _fallcConvert(conv, &from, &from_left, &to,
                                            &to_left, NULL, 0);
...
int _falmbTextPropertyToTextList(lcd, dpy, text_prop, list_ret,
                                 count_ret) XLCd lcd;
Display *dpy;
XTextProperty *text_prop;
char ***list_ret;
...
int _falwcTextPropertyToTextList(lcd, dpy, text_prop, list_ret,
                                 count_ret) XLCd lcd;
Display *dpy;
XTextProperty *text_prop;
wchar_t ***list_ret;
...
void _falwcFreeStringList(lcd, list) XLCd lcd;
wchar_t **list;
{
        if (list) {
                if (*list)
