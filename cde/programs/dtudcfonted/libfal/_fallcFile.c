...
static int parse_line(line, argv, argsize) char *line;
char **argv;
int argsize;
{
        int argc = 0;
...
static void xlocaledir(path) char *path;
{
        char *dir, *p = path;
        int len;
...
static int parse_path(path, argv, argsize) char *path;
char **argv;
int argsize;
{
        char *p = path;
...
static char *_XlcResolveName(lc_name, file_name, direction) char *lc_name;
char *file_name;
...
*/
static char *lowercase(dst, src) char *dst;
char *src;
{
        char *s, *t;
...
/************************************************************************/
char *_fallcFileName(lcd, category) XLCd lcd;
char *category;
{
        ...
        if (!_fallcResolveLocaleName(XLC_PUBLIC(lcd, siname), lc_name, NULL,
                                     NULL, NULL)) {
                return NULL;
        }
...
int _fallcResolveLocaleName(lc_name, full_name, language, territory,
                            codeset) char *lc_name;
char *full_name;
char *language;
char *territory;
char *codeset;
...
int _fallcResolveDBName(lc_name, file_name) char *lc_name;
char *file_name;
{
        char dir[BUFSIZE], buf[BUFSIZE], *name = NULL;
        int i, n;
...
/************************************************************************/
int _fallcResolveI18NPath(path_name) char *path_name;
{
        if (path_name != NULL) {
