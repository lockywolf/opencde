...
static int compare(src, encoding, length) register char *src;
...
static unsigned long conv_to_dest(conv, code) Conversion conv;
unsigned long code;
{
        int i;
...
static unsigned long conv_to_source(conv, code) Conversion conv;
unsigned long code;
{
        int i;
        int conv_num;
...
static unsigned long mb_to_gi(mb, codeset) unsigned long mb;
CodeSet codeset;
{
        int i;
...
static unsigned long gi_to_mb(glyph_index, codeset) unsigned long glyph_index;
CodeSet codeset;
{
        int i;
...
static Bool gi_to_wc(lcd, glyph_index, codeset, wc) XLCd lcd;
...
static Bool wc_to_gi(lcd, wc, glyph_index, codeset) XLCd lcd;
wchar_t wc;
unsigned long *glyph_index;
CodeSet *codeset;
{
...
static CodeSet byteM_parse_codeset(lcd, inbufptr) XLCd lcd;
XPointer inbufptr;
{
        unsigned char ch;
        CodeSet codeset;
        ...
        for (i = 0; i < codeset_num; i++) {
                ...
                for (j = 0; j < codeset->length; j++) {
                        ...
                        for (hit = False, k = 0; k < byteM_rec.byteinfo_num;
                             k++) {
                                byteinfo_rec = byteinfo[k];
                                if (byteinfo_rec.start <= ch &&
                                    ch <= byteinfo_rec.end) {
                                        hit = True;
                                        break;
                                }
...
static CodeSet GLGR_parse_codeset(lcd, ch) XLCd lcd;
unsigned char ch;
{
        int i;
...
static XlcCharSet gi_parse_charset(glyph_index,
                                   codeset) unsigned long glyph_index;
CodeSet codeset;
{
        int i;
        size_t table_size =
            sizeof(default_ct_data) / sizeof(default_ct_data[0]);
        ...
        for (i = 0; i < table_size; i++)
                if (compare(charset->ct_sequence, default_ct_data[i].encoding,
                            strlen(charset->ct_sequence)))
...
check_extended_seg:
        ...
        for (i = 0; i < area_num; i++) {
                ...
                if (area[i].start <= glyph_index &&
                    glyph_index <= area[i].end) {
...
static Bool ct_parse_csi(inbufptr, ctr_seq_len) XPointer inbufptr;
int *ctr_seq_len;
{
        int i;
...
static int cmp_esc_sequence(inbufptr, ct_sequence,
                            encoding_name) XPointer inbufptr;
char *ct_sequence;
char *encoding_name;
{
        size_t table_size =
        ...
        /* Standard Character Set Encoding ? */
        for (i = 0; i < table_size; i++) {
                if (compare(ct_sequence, default_ct_data[i].encoding,
                            strlen(ct_sequence)))
                        return (seq_len);
        }
...
static Bool ct_parse_charset(lcd, inbufptr, charset, ctr_seq_len) XLCd lcd;
XPointer inbufptr;
XlcCharSet *charset;
int *ctr_seq_len;
{
        ...
        for (i = 0; i < codeset_num; i++) {
                ...
                for (j = 0; j < num_charsets; j++) {
                        *charset = charset_list[j];
                        if (*ctr_seq_len = cmp_esc_sequence(
                                inbufptr, (*charset)->ct_sequence,
        ...
        for (i = 0; i < segment_conv_num; i++) {
                *charset = segment_conv[i].source;
                if (*ctr_seq_len =
                        cmp_esc_sequence(inbufptr, (*charset)->ct_sequence,
                                         (*charset)->encoding_name))
                        return (True);
...
static Bool segment_conversion(lcd, charset, glyph_index) XLCd lcd;
XlcCharSet *charset;
unsigned long *glyph_index;
{
        int i;
...
CodeSet _fallcGetCodeSetFromName(lcd, name) XLCd lcd;
char *name;
{
        int i, j;
        XlcCharSet charset;
...
static Bool _XlcGetCodeSetFromCharSet(lcd, charset, codeset,
                                      glyph_index) XLCd lcd;
XlcCharSet charset;
CodeSet *codeset;
unsigned long *glyph_index;
{
...
static Bool check_string_encoding(codeset) CodeSet codeset;
{
        int i;
        XlcCharSet charset;
        XlcCharSet *charset_list = codeset->charset_list;
...
static void init_state(conv) XlcConv conv;
{
...
static int mbstowcs_org(conv, from, from_left, to, to_left, args,
...
static int wcstombs_org(conv, from, from_left, to, to_left, args,
                        num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
        ...
        while (*from_left && *to_left) {
                ...
                if (!wc_to_gi(lcd, wc, &glyph_index, &codeset)) {
                        ...
                        if (*to_left < defstr_len)
                                break;
                        if (outbufptr) {
                                strncpy((char *)outbufptr, default_string,
                                        defstr_len);
                        }
                        ...
                        if (codeset->parse_info) {
                                ...
                                if (*to_left < length)
                                        break;
                                if (outbufptr) {
                                        strncpy((char *)outbufptr, encoding,
                                                length);
                                }
...
static int wcstocts(conv, from, from_left, to, to_left, args,
                    num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
int num_args;
{
        ...
        while (*from_left && *to_left) {
                 ...
                 * encoding name     | STX |
                 * +-----+-----+-----+-----+-----+-----+-----+----
                 * ----+-----+-----+ 4bytes         1byte 1byte     variable
                 * length     1byte |
                 * |
                 * 	                   +-----------------------------------------+
                 * 	                     name length  = ((M - 128) * 128) +
                 * (L - 128)
                 */
                ...
                if ((charset != old_charset) &&
                    !(first_flag &&
                      (strcmp(charset->encoding_name, "ISO8859-1") == 0))) {
                        ...
                        if (outbufptr) {
                                ...
                                if (!standard_flag) {
                                        ...
                                        *outbufptr++ = name_len % 128 + 128;
                                        strcpy((char *)outbufptr,
                                               charset->encoding_name);
                                        outbufptr =
                                            outbufptr + name_len - 2 - 1;
                                        *outbufptr++ = STX;
                                }
                        }
                ...
                if (codeset->ctconv)
                        glyph_index =
                            conv_to_dest(codeset->ctconv, glyph_index);
                if (*to_left < charset->char_size) {
                        unconv_num++;
                        break;
                }

                p = ((XPointer)&glyph_index) + sizeof(glyph_index) -
                    charset->char_size;
...
static int ctstowcs(conv, from, from_left, to, to_left, args,
                    num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
int num_args;
{
        ...
        while (*from_left && *to_left) {
                ...
                /* escape sequence ? */
                if (ch == ESC) {
                        if (!ct_parse_charset(lcd, inbufptr - 1,
                                              &state->charset, &ctr_seq_len))
                                goto skip_the_seg;
        ...
        output_one_wc:
                ...
                if (!gi_len_left) {
                        ...
                        if (!segment_conversion(lcd, &charset_tmp,
                                                &glyph_index)) {
                                unconv_num += gi_len;
                                continue;
                        }
                        ...
                        /* get codeset */
                        if (!_XlcGetCodeSetFromCharSet(
                                lcd, charset_tmp, &codeset, &glyph_index)) {
                                unconv_num += gi_len;
                                continue;
                        }
...
static int mbstocts(conv, from, from_left, to, to_left, args,
                    num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
int num_args;
{
        ...
        unconv_num = mbstowcs_org(conv, from, from_left, &buf_ptr1, &buf_left1,
                                  args, num_args);
        if (unconv_num < 0)
                goto ret;
...
static int mbstostr(conv, from, from_left, to, to_left, args,
                    num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
static int mbtocs(conv, from, from_left, to, to_left, args,
                  num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
static int mbstocs(conv, from, from_left, to, to_left, args,
                   num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
static int wctocs(conv, from, from_left, to, to_left, args,
                  num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
static int wcstocs(conv, from, from_left, to, to_left, args,
                   num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
static int ctstombs(conv, from, from_left, to, to_left, args,
                    num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
int num_args;
{
        ...
        unconv_num = ctstowcs(conv, from, from_left, &buf_ptr1, &buf_left1,
                              args, num_args);
        if (unconv_num < 0)
                goto ret;
        ...
        unconv_num += wcstombs_org(conv, &buf_ptr2, &buf_left2, to, to_left,
                                   args, num_args);
        if (unconv_num < 0)
                goto ret;
...
static int strtombs(conv, from, from_left, to, to_left, args,
                    num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
static XlcConv create_conv(lcd, methods) XLCd lcd;
...
static XlcConvMethodsRec mbstowcs_methods = {close_converter, mbstowcs_org,
                                             NULL};

static XlcConv open_mbstowcs(from_lcd, from_type, to_lcd,
...
static XlcConv open_mbstocts(from_lcd, from_type, to_lcd,
                             to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
static XlcConv open_mbstostr(from_lcd, from_type, to_lcd,
                             to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
static XlcConv open_mbstocs(from_lcd, from_type, to_lcd, to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
static XlcConv open_mbtocs(from_lcd, from_type, to_lcd, to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
{ return create_conv(from_lcd, &mbtocs_methods); }

static XlcConvMethodsRec wcstombs_methods = {close_converter, wcstombs_org,
                                             NULL};

static XlcConv open_wcstombs(from_lcd, from_type, to_lcd,
                             to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
static XlcConv open_wcstocts(from_lcd, from_type, to_lcd,
                             to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
static XlcConv open_wcstocs(from_lcd, from_type, to_lcd, to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
static XlcConv open_wctocs(from_lcd, from_type, to_lcd, to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
static XlcConv open_ctstombs(from_lcd, from_type, to_lcd,
                             to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
static XlcConv open_ctstowcs(from_lcd, from_type, to_lcd,
                             to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
static XlcConv open_strtombs(from_lcd, from_type, to_lcd,
                             to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
...
XLCd _fallcGenericLoader(name) char *name;
{
        XLCd lcd;
        ...
        _fallcSetConverter(lcd, XlcNCompoundText, lcd, XlcNMultiByte,
                           open_ctstombs);
        _fallcSetConverter(lcd, XlcNCompoundText, lcd, XlcNWideChar,
                           open_ctstowcs);
