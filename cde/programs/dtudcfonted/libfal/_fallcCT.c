...
static CTInfo _XlcGetCTInfoFromEncoding(encoding,
                                        length) register char *encoding;
register int length;
{
...
static unsigned int _XlcParseCT(parse, text, length) register CTParse parse;
char **text;
int *length;
{
        unsigned int ret = 0;
                ...
                if (*str == XctOtherCoding && *(str + 1) == XctNonStandard &&
                    *(str + 2) >= 0x30 && *(str + 2) <= 0x3f && *length >= 6) {
                        ...
                        if (*str <= 0x34) {
                                ...
                                if (parse->char_size == 0)
                                        parse->char_size = 1;
                                ret = XctExtSeg;
                                parse->ct_info =
                                    _XlcGetCTInfoFromEncoding(*text, *length);
                        } else
                                ret = XctOtherSeg;
                        str++;
                        parse->ext_seg_length =
                            (*str - 128) * 128 + *(str + 1) - 128;
                        str += 2;
...
XlcCharSet _fallcAddCT(name, encoding) char *name;
char *encoding;
{
        CTInfo ct_info;
        XlcCharSet charset;
...
static CTInfo _XlcGetCTInfoFromCharSet(charset) register XlcCharSet charset;
{
        register CTInfo ct_info;
...
Bool _fallcParseCharSet(charset) XlcCharSet charset;
{
        CTParseRec parse;
        char *ptr, buf[BUFSIZ];
        unsigned int type;
        ...
        if (charset->name) {
                ...
                charset->encoding_name =
                    falrmQuarkToString(charset->xrm_encoding_name);
        } else {
                charset->xrm_name = 0;
...
static int _XlcCheckCTSequence(state, ctext, ctext_len) State state;
char **ctext;
int *ctext_len;
{
        XlcCharSet charset;
...
static int cttocs(conv, from, from_left, to, to_left, args,
        ...
        while (ctext_len > 0 && buf_len > 0) {
                ...
                if (ch == 0x1b || ch == 0x9b) {
                        ...
                        if (charset) {
                                ...
                                charset = ch & 0x80 ? state->GR_charset
                                                    : state->GL_charset;
...
static int cstoct(conv, from, from_left, to, to_left, args,
                  num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
int num_args;
{
        ...
        if (ct_info->ext_segment) {
                if (charset != state->ext_seg_charset &&
                    state->ext_seg_left < 1) {
                        length = ct_info->encoding_len;
                        if (ct_len < length)
                                return -1;
                        strcpy(ctptr, ct_info->encoding);
                        ctptr[4] =
                            ((ct_info->ext_segment_len + csstr_len) / 128) |
                            0x80;
                        ctptr[5] =
                            ((ct_info->ext_segment_len + csstr_len) % 128) |
                            0x80;
                ...
                state->ext_seg_charset =
                    (state->ext_seg_left > 0) ? charset : NULL;
        } else {
...
static int strtocs(conv, from, from_left, to, to_left, args,
                   num_args) XlcConv conv;
XPointer *from;
int *from_left;
XPointer *to;
...
static XlcConvMethodsRec cttocs_methods = {close_converter, cttocs, init_state};

static XlcConv open_cttocs(from_lcd, from_type, to_lcd, to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
{ return create_conv(&cttocs_methods); }

static XlcConvMethodsRec cstoct_methods = {close_converter, cstoct, init_state};

static XlcConv open_cstoct(from_lcd, from_type, to_lcd, to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
{ return create_conv(&cstoct_methods); }

static XlcConvMethodsRec strtocs_methods = {close_converter, strtocs,
                                            init_state};

static XlcConv open_strtocs(from_lcd, from_type, to_lcd, to_type) XLCd from_lcd;
char *from_type;
XLCd to_lcd;
char *to_type;
{ return create_conv(&strtocs_methods); }

static XlcConvMethodsRec cstostr_methods = {close_converter, cstostr,
                                            init_state};
...
static void init_converter() {
        _fallcSetConverter((XLCd)NULL, XlcNCompoundText, (XLCd)NULL,
                           XlcNCharSet, open_cttocs);
        _fallcSetConverter((XLCd)NULL, XlcNString, (XLCd)NULL, XlcNCharSet,
                           open_strtocs);

        _fallcSetConverter((XLCd)NULL, XlcNCharSet, (XLCd)NULL,
                           XlcNCompoundText, open_cstoct);
