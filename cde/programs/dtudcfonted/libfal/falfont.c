...
void set_errfile_str(obuf, ibuf) char *obuf;
char *ibuf;
{
        char *sp;
 ...
 */
FalFontID FalOpenFont(file, protect_key_data, codeset) char *file;
 ...
 */
FalFontID __FalOpenFont(file, protect_key_data, codeset, lockflag) char *file;
int protect_key_data;
int codeset;
int lockflag;
{
        ...
        if (!(openfontfile = (char *)malloc(strlen(file) + 1))) {
                fal_seterrcode(_FAL_MALOC_ER, 0, FAL_ERR_MALLOC,
                               FAL_FUNCNUM_OPNFNT);
        ...
        if (strcmp(FILE_SUFFIX(openfontfile), PCFSUFFIX) == 0) {
                ...
                FAL_GET_SNF_HEADER(buf, protect_key_data, fal_utyerror,
                                   fal_utyderror, fd, finf, openfontfile);
        }
        finf->buf = buf;

        if (finf->isFef) {
                finf->pCinf =
                    (CharInfoPtr)((char *)finf->pFinf + sizeof(FontInfoRec));
                finf->pGlyphs = (unsigned char *)((char *)finf->pCinf +
                                                  (sizeof(CharInfoRec) *
                                                   GETNUMCHARS(finf->pFinf)));
                finf->width = finf->pFinf->maxbounds.metrics.rightSideBearing -
                              finf->pFinf->maxbounds.metrics.leftSideBearing;
                finf->height = finf->pFinf->maxbounds.metrics.ascent +
...
#else
        FalCloseFont(fid) FalFontID fid;
#endif
{
        int __FalCloseFont();
 ...
 * 	0: OFF --  for "FalOpenFont()"
 */
__FalCloseFont(fid, lockflag) FalFontID fid;
int lockflag;
{
        Oak_FontInf *finf;
...
#else
        FalQueryFont(fid, fontinfo) FalFontID fid;
...
#else
        char *FalReadFont(fid, code, width, height) FalFontID fid;
        int code;
        int width;
        int height;
#endif
        ...
        if (zoom_on == TRUE) {
                ...
                if (ptnBsize > finf->dptnBufL) {
                        if (finf->dptnBuf == NULL) {
                                ...
                                toglyph =
                                    (char *)realloc(finf->dptnBuf, ptnBsize);
                        }
                        if (toglyph == NULL) {
                                fal_utyerror = _FAL_MALOC_ER;
                                fal_utyderror = 0;
                ...
                falZoom(finf->dptnBuf, finf->sptnBuf, finf->width, finf->height,
                        width, height, finf->dbuf);
                return (finf->dptnBuf);
        } else {
                return finf->sptnBuf;
        }
...
static falGetGlyph(glyph, finf, code) char *glyph;
Oak_FontInf *finf;
int code;
{
...
static falGetCharIndex(finf, code) Oak_FontInf *finf;
...
static falZoom(dmem, smem, sw, sh, dw, dh, dbuf) char *dmem;
char *smem;
int sw;
int sh;
int dw;
...
static exline(sp, dbuf, sw, dw) char *sp;
char *dbuf;
int sw;
int dw;
{
...
static char falGetMask(width) int width;
{
        int ix = width % 8;
...
#if NeedFunctionPrototypes
int FalGetFontList(FalFontData *key_data, int mask, FalFontDataList **list_ret)
#else
        int FalGetFontList(key_data, mask, list_ret)
...
#endif
{
        ...
        FalFontDataList
            *fls; /* a pointer of a structure of "FalFontDataList()" */
        ...
        if ((fls = (FalFontDataList *)malloc(sizeof(FalFontDataList))) ==
            NULL) {
                fal_utyerror = _FAL_MALOC_ER;
                fal_utyerrno = FAL_ERR_MALLOC;
                fal_utyerrno |= (FAL_FUNCNUM_GFLST << 8);
...
static int chk_key_str(key, mask)
...
static int fal_make_fontlist(fls, key, mask) FalFontDataList
    *fls;         /* a pointer of a structure of a font information list */
FalFontData *key; /* a structure of searching information */
int mask;         /* a mask for a saerch */
{
        return falReadFontInfoLists(MKFONTLIST, key, mask, fls, NULL, NULL,
                                    NULL, NULL);
}
...
#else
        int FalFreeFontList(list) FalFontDataList *list;
#endif
{
        int i;
...
static int fal_split_data(buf, elm_num, elm) char *buf;
int elm_num;
char *elm[];
{
        int cnt;
...
static int fal_clear_data(tmp) FalFontData *tmp;
{
        /* search a character */
...
static int fal_check_already_exist(data, lst) FalFontData *data;
FalFontDataList *lst;
{
        int i;
        int mask;
...
static int fal_sort_fontlist(lst) FalFontDataList *lst;
{
        int i, j;
        FalFontDataList srt;
        FalFontData target;
...
static int fal_atoi(str, val) char *str;
int *val;
{
        char *ptr;
        char *str_end;
...
static int fal_set_cs(str, cs) char *str;
int *cs;
{
        if (!strcmp(str, "CS0")) {
                *cs = FAL_FONT_CS0;
...
static int fal_set_prm(str, prm) char *str;
int *prm;
{
        int tmp = 0;
        for (; *str != '\0'; str++) {
...
static int fal_read_db(str, db) char *str;
FalFontDB *db;
{
        int i;
...
int FalGetFontPath(dlist_ret) FalFontPath **dlist_ret;
{
...
static int set_default_path() { return set_font_pathlist(&orgn, 0); }

static int cpy_default_path(p) FalFontPath *p;
{
        int i;
        FalFontPath tmp;
...
static int comp_default_path(p) FalFontPath *p;
{
        int i;
...
static int fal_clear_font_path(ls) FalFontPath *ls;
{
        int i;
...
int set_struct(tmp_data, elm) FalFontData *tmp_data;
char *elm[];
{
        char *dup_p;
        ...
        if (fal_atoi(elm[FAL_FONT_ELM_SIZE_W], &(tmp_data->size.w)) ==
            FAL_ERROR) {
                goto FalError02;
        }
        /* a height of a character size */
        if (fal_atoi(elm[FAL_FONT_ELM_SIZE_H], &(tmp_data->size.h)) ==
            FAL_ERROR) {
                goto FalError02;
        }
...
int searchFontFileName(data, fullPathName) FalFontData data;
char *fullPathName;
{
        int rtn;
        rtn = falReadFontInfoLists(SRCHFNAME, NULL, NULL, NULL, data,
                                   fullPathName, NULL, NULL);
        if (fal_utyerrno != 0) {
...
int fal_eq_data(data, tmp_data) FalFontData data;
FalFontData tmp_data;
{
        ...
        if ((data.style.name != NULL) && (tmp_data.style.name != NULL)) {
                if ((flg == 0) &&
                    (strcmp(data.style.name, tmp_data.style.name) != 0)) {
                        flg++;
                }
        }
        ...
        if ((data.shape.name != NULL) && (tmp_data.shape.name != NULL)) {
                if ((flg == 0) &&
                    (strcmp(data.shape.name, tmp_data.shape.name) != 0)) {
                        flg++;
                }
        }
...
static int CR_to_NULL(buf) char *buf;
{
        for (; *buf != '\0'; buf++) {
                if (*buf == '\n') {
                        *buf = '\0';
...
char *fal_get_base_name(str) char *str;
{
        char *str_slash;
...
static int fal_cmp_data(op1, key, mask) FalFontData *op1;
FalFontData *key;
int mask;
{
        if (mask == 0) {
        ...
        if ((mask & FAL_FONT_MASK_XLFDNAME) &&
        ...
        if ((mask & FAL_FONT_MASK_LETTER_W) &&
            (op1->letter.w != key->letter.w)) {
                return _FAL_TRY_NEXT;
        }
        if ((mask & FAL_FONT_MASK_LETTER_H) &&
            (op1->letter.h != key->letter.h)) {
                return _FAL_TRY_NEXT;
        }
        if ((mask & FAL_FONT_MASK_LETTER_X) &&
            (op1->letter.x != key->letter.x)) {
                return _FAL_TRY_NEXT;
        }
        if ((mask & FAL_FONT_MASK_LETTER_Y) &&
            (op1->letter.y != key->letter.y)) {
                return _FAL_TRY_NEXT;
        }
        if ((mask & FAL_FONT_MASK_STYLE_DEF) &&
        ...
        if ((mask & FAL_FONT_MASK_SHAPE_DEF) &&
...
static int new_target(target, choose) FalFontData *target;
FalFontData *choose;
{
        FalFontData diff;
        DEF_STR_CHK;
...
#else
        int FalFontOfFontID(fid, fontdata) FalFontID fid;
        FalFontData *fontdata;
#endif
{
        int rtn;
        rtn = falReadFontInfoLists(FONTOFID, NULL, NULL, NULL, NULL, NULL, fid,
                                   fontdata);
        if (fal_utyerrno != 0) {
...
#else
        FalFontID FalOpenSysFont(open_font_data, font_data_mask,
                                 missing_font_list_return)
...
#endif
{
        ...
        retFL = FalGetFontList(open_font_data, font_data_mask,
                               missing_font_list_return);
        ...
        if ((*missing_font_list_return)->num > 1) {
                ...
                if (searchFontFileName((*missing_font_list_return)->list[0],
                                       fname) == FAL_ERROR) {
                ...
                if (retOF == (FalFontID)FAL_ERROR) {
                        ...
                        *missing_font_list_return = NULL;
                        FalFreeFontList(*missing_font_list_return);
                        if (fal_add_fidinf(retOF, dspcode, slct_cdset,
                                           islock) == FAL_ERROR) {
                                fal_utyerrno |= (FAL_FUNCNUM_OPNFNT << 8);
                                return 0;
                        }
...
static int set_font_pathlist(pathlist, nodef) FalFontPath *pathlist;
int nodef;
{
        FILE *fp;
                ...
                if (buf[0] == '#') { /* comments */
                        ...
                        if (font_path) {
                                ...
                                if (pathlist->path == NULL) {
                                        tmp_list = (char **)malloc(
                                            sizeof(char *) * (num + 1));
                                } else {
                                        tmp_list = (char **)realloc(
                                            pathlist->path,
                                            sizeof(char *) * (num + 1));
                                }
                                if (tmp_list == NULL) {
        ...
        if (font_path) {
                ...
                if (pathlist->path == NULL) {
                        ...
                        tmp_list = (char **)realloc(pathlist->path,
                                                    sizeof(char *) * (num + 1));
                }
                if (tmp_list == NULL) {
                        fclose(fp);
...
static int make_default_path(pathlist) FalFontPath *pathlist;
{
        struct stat statbuf;
        ...
        while ((ep = strchr(sp, ':')) != (char *)NULL) {
                ...
                /* get path */
                *ep = '\0';
                if ((path = (char *)malloc(sizeof(char) * (strlen(sp) + 2))) ==
                    NULL) {
                        fal_clear_font_path(pathlist);
                        fal_utyerror = _FAL_MALOC_ER;
                        fal_utyerrno = FAL_ERR_MALLOC;
                ...
                if (pathlist->path == NULL) {
                        ...
                        plist = (char **)realloc(pathlist->path,
                                                 sizeof(char *) * (num + 1));
                }
                if (plist == NULL) {
                        fal_clear_font_path(pathlist);
...
static int falReadFontInfoLists(func, lstkey, mask, fls, fnkey, fullpath, fid,
                                fdata) int func;
...
FalFontData *fdata;
{
        ...
        if (execDefined == TRUE && IsDefaultPath == FALSE) {
                ...
                while (fgets(buf, FAL_LINE_MAX, fp) != (char *)NULL) {
                         ...
                         */
                        switch (fal_get_def_fontdata(func, pname, buf, elm,
                                                     &tmp_data, keyfname)) {
                        case _FAL_TRY_NEXT:
                                if (buf[0] == '/') {
                                        buf[strlen(buf) - 1] = '/';
                        ...
                        case MKFONTLIST: /* FalGetFontList 	*/
                                switch (falgetfontlist(&tmp_data, lstkey, mask,
                                                       fls, 1)) {
                                case _FAL_TRY_NEXT:
                                        continue;
                                case 0:
                        ...
                        case SRCHFNAME: /* searchFontFileName 	*/
                                ...
                                switch (eq) {
                                case 0:
                                        sprintf(fullpath, "%s%s", pname,
                                                elm[FAL_FONT_ELM_FILENAME]);
        ...
        if (execUndefined == TRUE) {
                ...
                for (i = 0; i < wpath.num; i++) {
                        ...
                        fonts_list_file = (char *)malloc(
                            sizeof(char) *
                            (strlen(wpath.path[i]) + strlen(FONTSDIR) + 1));
                        if (fonts_list_file == (char *)NULL) {
                                fal_utyerror = _FAL_MALOC_ER;
                                fal_utyerrno = FAL_ERR_MALLOC;
                                return FAL_ERROR;
                        }
                        sprintf(fonts_list_file, "%s%s", wpath.path[i],
                                FONTSDIR);
                        ...
                        if (func & MKFONTLIST) {
                                if (fls->list == NULL) {
                                        p_bak = (FalFontData *)malloc(
                                            sizeof(FalFontData) *
                                            (fls->num + fontnum));
                                } else {
                                        p_bak = (FalFontData *)realloc(
                                            fls->list,
                                            sizeof(FalFontData) *
                                                (fls->num + fontnum));
                                }
                        ...
                        while (fgets(buf, FAL_LINE_MAX, fp) != (char *)NULL) {
                                ...
                                if ((strcmp(FILE_SUFFIX(buf), PCFSUFFIX) !=
                                     0) &&
                                    (strcmp(FILE_SUFFIX(buf), SNFSUFFIX) !=
                                     0) &&
                                    (ISOPTFONT(buf) != 0)) {
                                        continue;
                                }
                                ...
                                if ((func & SRCHFNAME) || (func & FONTOFID)) {
                                        ...
                                        sprintf(fontFileName, "%s%s",
                                                wpath.path[i], buf);
                                }

                                /* read font file and get properties */
                                xlfd = ++ep;
                                switch (fal_get_undef_fontdata(
                                    func, fontFileName, xlfd, &tmp_data,
                                ...
                                case SRCHFNAME: /* searchFontFileName 	*/
                                        ...
                                        for (j = 0; j < codeset_num; j++) {
                                                ...
                                                if (func &
                                                    MKFONTLIST) { /* FalGetFontList
                                                                   */
                                                        /* Ignore fonts already
                                                         * exists. */
                                                        if (fal_check_already_exist(
                                                                &tmp_data,
                                                                fls) >= 0) {
                                                                continue;
                                                        }

                                                        switch (falgetfontlist(
                                                            &tmp_data, lstkey,
                                                            mask, fls, 1)) {
                                                        case _FAL_TRY_NEXT:
                                                                continue;
                                        ...
                                        if (func &
                                            SRCHFNAME) { /* searchFontFileName
                                                ...
                                                fal_clear_data(&tmp_data);
                                                FreeString(
                                                    fontFileName); /* _FAL_TRY_NEXT
                                                                    */
                                        }
                                        break;

                                case FONTOFID: /* FalFontOfFontID 	*/
                                        memmove(fdata, &tmp_data,
                                                sizeof(FalFontData));
                                        if (fal_read_fidinf(fid, &fontid_inf) !=
                                            FAL_ERROR)
                                                fdata->cd_set =
                                                    fontid_inf.cd_set;
                                        else {
...
static int fal_get_def_fontdata(func, pname, buf, elm, tmp_data,
                                key_fname) int func;
...
static int falgetfontlist(tmp_data, key, mask, fls,
                          fontnum) FalFontData *tmp_data;
FalFontData *key;
int mask;
FalFontDataList *fls;
int fontnum;
{
        ...
        if (fls->list == NULL) {
                ...
                p_bak = realloc(fls->list,
                                sizeof(FalFontData) * (fls->num + fontnum));
        }
        if (p_bak == NULL) {
                fal_utyerror = _FAL_MALOC_ER;
                fal_utyerrno = FAL_ERR_MALLOC;
...
static int fal_get_undef_fontdata(func, full_path, xlfd, tmp_data, key_fname,
                                  codeset_num, codeset_list) int func;
char *full_path;
char *xlfd;
FalFontData *tmp_data;
char *key_fname;
        ...
        if ((func & MKFONTLIST) || (func & SRCHFNAME)) {
                ...
                if (charset_str_buf && !strcmp(charset_str_buf, char_set)) {
                        ...
                        *codeset_list = codeset_list_sav;
                        ...
                        if ((rtn =
                                 fal_get_codeset(fal_locale, char_set,
                                                 codeset_list, codeset_num))) {
                                return rtn;
                        }
                        /* save code set infomations */
                        if ((charset_str_buf = (char *)strdup(char_set)) ==
                            NULL) {
                                fal_utyerror = _FAL_MALOC_ER;
                                fal_utyerrno = FAL_ERR_MALLOC;
                                return FAL_ERROR;
...
#else
        int FalFontIDToFileName(fid, file_name) FalFontID fid;
        char **file_name;
#endif
{
        char *fname;
...
int FalFreeGI(ginf, num) FalGIInf *ginf;
int num;
{
        int i;
        if (ginf == NULL) {
...
static int fal_add_fidinf(fid, dspcode, cd_set, islock) FalFontID fid;
int dspcode;
int cd_set;
int islock;
{
        ...
        if (!exist) {
                if (mngfid.finf == NULL) {
                        cnt = 0;
                        if ((flist = (FontIDInfo **)malloc(
                                 sizeof(FontIDInfo *))) == NULL) {
                                fal_utyerror = _FAL_MALOC_ER;
                                fal_utyerrno = FAL_ERR_MALLOC;
                                return FAL_ERROR;
                        }
                        ...
                        if ((flist = (FontIDInfo **)realloc(
                                 mngfid.finf,
                                 sizeof(FontIDInfo *) * (cnt + 1))) == NULL) {
                                fal_utyerror = _FAL_MALOC_ER;
                                fal_utyerrno = FAL_ERR_MALLOC;
                                return FAL_ERROR;
                        }
                }
                if ((flist[cnt] = (FontIDInfo *)malloc(sizeof(FontIDInfo))) ==
                    NULL) {
                        fal_utyerror = _FAL_MALOC_ER;
                        fal_utyerrno = FAL_ERR_MALLOC;
                        return FAL_ERROR;
...
static int fal_read_fidinf(fid, fontid_inf) FalFontID fid;
FontIDInfo *fontid_inf;
{
        int i;
        if (fid == NULL) {
...
static int fal_del_fidinf(fid) FalFontID fid;
{
        int i, cnt, target;
        FontIDInfo **flist;
...
static int fal_conv_code_to_glyph(fid, code, glidx) FalFontID fid;
unsigned int code;
unsigned int *glidx;
{
        Oak_FontInf *finf;
        ...
        if (fontid_inf.dspcode != FAL_FONT_GLYPH_INDEX) {
                ...
                if (cmpfid != fid) {
                        if (falReadGpfProp(FAL_READ_FONTINFO, finf,
                                           FAL_FONT_PRM, &tmp_data)) {
                                fal_clear_data(&tmp_data);
                                return FAL_ERROR;
                        }
                        GETCHARSETSTR(char_set, tmp_data.xlfdname);
...
static int fal_conv_glyph_to_code(finf, dspcode, cd_set, glidx,
                                  code) Oak_FontInf *finf;
int dspcode;
int cd_set;
unsigned int glidx;
unsigned int *code;
{
        ...
        if (dspcode != FAL_FONT_GLYPH_INDEX) {
                 ...
                 */
                memset(&tmp_data, 0, sizeof(tmp_data));
                if (falReadGpfProp(FAL_READ_FONTINFO, finf, FAL_FONT_PRM,
                                   &tmp_data)) {
                        fal_clear_data(&tmp_data);
                        return FAL_ERROR;
                }
                ...
                if (COMM_SBFNT_ISSBFNT(char_set)) {
                        ...
                        if (fal_glyph_to_code(fal_locale, char_set, cd_set,
                                              glidx, &inner_code)) {
                                fal_clear_data(&tmp_data);
                                return FAL_ERROR;
                        }
...
static int file_lock(fd) int fd; /* a file descripter */
{
        struct flock flpar;
...
static int file_unlock(fd) int fd; /* a file descripter */
{
        struct flock flpar;
...
static int is_lock(fd) int fd; /* file descripter */
{
        struct flock flpar;
