...
static Bool realloc_parse_info(len) int len;
{
        char *p;

        parse_info.bufMaxSize =
            BUFSIZE * ((parse_info.bufsize + len) / BUFSIZE + 1);
...
static void free_line(line) Line *line;
{
        if (line->str != NULL) {
                Xfree(line->str);
        }
...
static int realloc_line(line, size) Line *line;
int size;
{
        char *str = line->str;
...
static void zap_comment(str, quoted) char *str;
int *quoted;
{
        char *p = str;
#ifdef never
        *quoted = 0;
        ...
        while (*p) {
                ...
                if (*p == SYM_COMMENT && !*quoted) {
                        ...
                        if (pos == 0 ||
                            iswhite(p[-1]) &&
                                (pos == 1 || p[-2] != SYM_BACKSLASH)) {
                                int len = strlen(p);
                                if (len > 0 && p[len - 1] == SYM_NEWLINE) {
                                        /* newline is the identifier for finding
                                           end of value. therefore, it should
                                           not be removed. */
                                        *p++ = SYM_NEWLINE;
                                }
                                *p = '\0';
...
static int read_line(fd, line) FILE *fd;
Line *line;
{
        char buf[BUFSIZE], *p;
        int len;
                ...
                if (!quoted) {
                        if (cur > 1 && str[cur - 2] == SYM_BACKSLASH &&
                            str[cur - 1] == SYM_NEWLINE) {
                                /* the line is ended backslash followed by
                                   newline. need to concatinate the next line.
                                 */
                                cur -= 2;
                                str[cur] = '\0';
                        } else {
...
static Token get_token(str) char *str;
{
        switch (*str) {
        case SYM_NEWLINE:
                return T_NEWLINE;
...
static int get_word(str, word) char *str;
char *word;
{
        char *p = str, *w = word;
        Token token;
...
static int get_quoted_word(str, word) char *str;
char *word;
{
        char *p = str, *w = word;
        Token token;
...
static int append_value_list() {
        ...
        if (value_list == (char **)NULL) {
                ...
                *value_list = NULL;
                ...
                value_list = (char **)Xrealloc(value_list, sizeof(char *) *
                                                               (value_num + 2));
        }
        if (value_list == (char **)NULL) {
                goto err;
...
static int construct_name(name) char *name;
{
...
static int store_to_database(db) Database *db;
{
...
static int check_category_end(str) char *str;
{
        char *p;
        int len;
...
static int f_newline(str, token, db) char *str;
Token token;
Database *db;
{
...
static int f_comment(str, token, db) char *str;
Token token;
Database *db;
{
...
static int f_white(str, token, db) char *str;
Token token;
Database *db;
{
        char *p = str;
...
static int f_semicolon(str, token, db) char *str;
Token token;
Database *db;
{
        switch (parse_info.pre_state) {
...
static int f_left_brace(str, token, db) char *str;
Token token;
Database *db;
{
        switch (parse_info.pre_state) {
...
static int f_right_brace(str, token, db) char *str;
Token token;
Database *db;
{
...
static int f_double_quote(str, token, db) char *str;
Token token;
Database *db;
{
        char word[BUFSIZE];
...
static int f_backslash(str, token, db) char *str;
Token token;
Database *db;
{ return f_default(str, token, db); }

static int f_numeric(str, token, db) char *str;
Token token;
Database *db;
{
        char word[BUFSIZE], *p;
        ...
        case S_VALUE:
                ...
                if ((parse_info.bufsize + token_len + (int)strlen(word) + 1) >=
                    parse_info.bufMaxSize) {
                        if (realloc_parse_info(token_len + strlen(word) + 1) ==
                            False) {
                                goto err;
                        }
                }
...
static int f_default(str, token, db) char *str;
Token token;
Database *db;
{
        char word[BUFSIZE], *p;
...
#ifdef DEBUG
static void PrintDatabase(db) Database db;
{
        Database p = db;
        int i = 0, j;
...
static void DestroyDatabase(db) Database db;
{
        Database p = db;
...
static int CountDatabase(db) Database db;
{
        Database p = db;
        int cnt = 0;
...
static Database CreateDatabase(dbfile) char *dbfile;
{
...
/************************************************************************/
void _fallcGetResource(lcd, category, class, value, count) XLCd lcd;
char *category;
char *class;
char ***value;
int *count;
...
/*	This function retrieves XLocale database information.		*/
/************************************************************************/
void _fallcGetLocaleDataBase(lcd, category, name, value, count) XLCd lcd;
char *category;
char *name;
char ***value;
int *count;
{
        ...
        for (; lc_db->db; ++lc_db) {
                if (category_q == lc_db->category_q &&
                    name_q == lc_db->name_q) {
...
/************************************************************************/
void _fallcDestroyLocaleDataBase(lcd) XLCd lcd;
{
        XlcDatabase lc_db = (XlcDatabase)XLC_PUBLIC(lcd, xlocale_db);
...
/*	the specified XLCd.						*/
/************************************************************************/
XPointer _fallcCreateLocaleDataBase(lcd) XLCd lcd;
{
        ...
        for (list = _db_list; list; list = list->next) {
                if (name_q == list->name_q) {
                        list->ref_count++;
                        return XLC_PUBLIC(lcd, xlocale_db) =
                                   (XPointer)list->lc_db;
                }
        }
