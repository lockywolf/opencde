...
static XLCd create(name, methods) char *name;
XLCdMethods methods;
{
        XLCd lcd;
...
static Bool string_to_encoding(str, encoding) char *str;
char *encoding;
{
        char *next;
        long value;
...
static Bool string_to_ulong(str, value) char *str;
unsigned long *value;
{
        char *tmp1 = str;
        int base;
...
static Bool add_charset(codeset, charset) CodeSet codeset;
XlcCharSet charset;
{
        XlcCharSet *new_list;
        int num;
        ...
        if (num = codeset->num_charsets)
                new_list = (XlcCharSet *)Xrealloc(
                    codeset->charset_list, (num + 1) * sizeof(XlcCharSet));
        else
...
static CodeSet add_codeset(gen) XLCdGenericPart *gen;
{
...
static Bool add_parse_list(gen, type, encoding, codeset) XLCdGenericPart *gen;
EncodingType type;
char *encoding;
CodeSet codeset;
{
...
static void free_charset(lcd) XLCd lcd;
{
...
static char *getscope(str, scp) char *str;
FontScope scp;
{
        char buff[256], *next;
...
static int count_scopemap(str) char *str;
{
        char *ptr;
        int num = 0;
        for (ptr = str; *ptr; ptr++) {
...
FontScope falparse_scopemaps(str, size) char *str;
int *size;
{
        int num = 0, i;
...
void dbg_printValue(str, value, num) char *str;
char **value;
int num;
{
        /*
...
void dmpscope(name, sc, num) FontScope sc;
int num;
{
        /*
            int i;
...
static XlcCharSet srch_charset_define(name, new) char *name;
int *new;
{
...
static int read_charset_define(lcd, gen) XLCd lcd;
XLCdGenericPart *gen;
{
        int i = 0;
        ...
        for (i = 0;; i++) { /* loop start */
                ...
                _fallcGetResource(lcd, "XLC_CHARSET_DEFINE", name, &value,
                                  &num);
                dbg_printValue(name, value, num);
                if (num > 0) {
                        strcpy(cset_name, value[0]);
                        sprintf(name, "%s.%s", csd, "side");
                        _fallcGetResource(lcd, "XLC_CHARSET_DEFINE", name,
                                          &value, &num);
                        if (num > 0) {
                                dbg_printValue(name, value, num);
                                if (!_fallcNCompareISOLatin1(value[0], "none",
                                                             4)) {
                                        side = XlcNONE;
                                        strcat(cset_name, ":none");
                                } else if (!_fallcNCompareISOLatin1(value[0],
                                                                    "GL", 2)) {
                                        side = XlcGL;
                                        strcat(cset_name, ":GL");
                                } else {
                ...
                if (num > 0) {
                        ...
                        */
                        ...
                        charsetd->xrm_encoding_name = falrmStringToQuark(tmp);
                }
        }
        return 1;
}

SegConv faladd_conversion(gen) XLCdGenericPart *gen;
{
        SegConv new_list;
        int num;
...
static int read_segmentconversion(lcd, gen) XLCd lcd;
XLCdGenericPart *gen;
{
        int i = 0;
        char conv[16];
        ...
        for (i = 0;; i++) { /* loop start */
                ...
                _fallcGetResource(lcd, "XLC_SEGMENTCONVERSION", name, &value,
                                  &num);
                if (num > 0) {
                        char *tmp;
                        if (conversion == NULL &&
                ...
                if (num > 0) {
                        ...
                        if (new) {
                                tmp = (char *)Xmalloc(
                                    strlen(conversion->source_encoding) + 1);
                                if (tmp == NULL) {
                                        return 0;
                                }
                ...
                if (num > 0) {
                        ...
                        if (new) {
                                tmp = (char *)Xmalloc(
                                    strlen(conversion->destination_encoding) +
                                    1);
                                if (tmp == NULL) {
                                        return 0;
                                }
...
static ExtdSegment create_ctextseg(value, num) char **value;
int num;
{
        ExtdSegment ret;
        ...
        if (strchr(value[0], ':')) {
                ...
                *ptr = 0;
                ...
                } else if (!_fallcNCompareISOLatin1(ptr, "GL", 2)) {
                        ret->side = XlcGL;
        ...
        scope = ret->area;
        for (i = 1; i < num; i++) {
                sscanf(value[i], "\\x%lx,\\x%lx", &scope[i - 1].start,
                       &scope[i - 1].end);
        }
...
static Bool load_generic(lcd) XLCd lcd;
{
        XLCdGenericPart *gen = XLC_GENERIC_PART(lcd);
        char **value;
        int num;
        ...
        /***** force_convert_to_mb *****/
        _fallcGetResource(lcd, "XLC_XLOCALE", "force_convert_to_mb", &value,
                          &num);
        if (num > 0 && !_fallcCompareISOLatin1(value[0], "True"))
        ...
        for (i = 0;; i++) {
                ...
                if (num > 0) {
                        ...
                        if (codeset == NULL &&
                            (codeset = add_codeset(gen)) == NULL)
                                goto err;

                        /* 3.4.1 side */
                        if (!_fallcNCompareISOLatin1(value[0], "none", 4)) {
                                codeset->side = XlcNONE;
                        } else if (!_fallcNCompareISOLatin1(value[0], "GL",
                                                            2)) {
                                codeset->side = XlcGL;
                        } else {
                                codeset->side = XlcGR;
                        ...
                        tmp = strrchr(value[0], ':');
                        if (tmp != NULL &&
                            !_fallcCompareISOLatin1(tmp + 1, "Default")) {
                ...
                if (num > 0) {
                        ...
                        for (; num-- > 0; value++) {
                                ...
                                if (charset) {
                                        if (add_charset(codeset, charset) ==
                                            False)
                                                goto err;
                                }
                        }
                ...
                for (M = 1; M - 1 < codeset->length; M++) {
                        ...
                        _fallcGetResource(lcd, "XLC_XLOCALE", name, &value,
                                          &num);
                        ...
                        if (M == 1) {
                                ...
                                codeset->byteM = (ByteInfoListRec *)Xmalloc(
                                    (codeset->length) *
                        ...
                        if (num > 0) {
                                ...
                                (codeset->byteM)[M - 1].byteinfo =
                                    (ByteInfo)Xmalloc(num *
                                                      sizeof(ByteInfoRec));
                                for (ii = 0; ii < num; ii++) {
                                        char tmp[128];
                                        tmpb = (codeset->byteM)[M - 1].byteinfo;
                                        /* default 0x00 - 0xff */
                                        sscanf(value[ii], "\\x%lx,\\x%lx",
                                               (long unsigned *)&start,
                                               (long unsigned *)&end);
                ...
                if (num > 0) {
                        ...
                        codeset->mbconv->convlist = falparse_scopemaps(
                            value[0], &(codeset->mbconv->conv_num));
                ...
                if (num > 0) {
                        ...
                        codeset->ctconv->convlist = falparse_scopemaps(
                            value[0], &(codeset->ctconv->conv_num));
...
static Bool initialize(lcd) XLCd lcd;
{
...
static void freeByteM(codeset) CodeSet codeset;
{
        int i;
        ByteInfoList blst;
        if (codeset->byteM == NULL) {
...
static void freeConversion(codeset) CodeSet codeset;
{
        int i;
...
static void freeExtdSegment(codeset) CodeSet codeset;
{
        int i;
...
static void freeParseInfo(codeset) CodeSet codeset;
{
        int i;
        ParseInfo parse_info;
        if (codeset->parse_info == NULL) {
...
static void destroy_CodeSetList(gen) XLCdGenericPart *gen;
{
...
/*  */
static void destroy_SegConv(gen) XLCdGenericPart *gen;
{
...
static void destroy_gen(lcd) XLCd lcd;
{
        XLCdGenericPart *gen = XLC_GENERIC_PART(lcd);
        destroy_SegConv(gen);
...
/* VW/UDC end 95.01.08 */
static void destroy(lcd) XLCd lcd;
{
