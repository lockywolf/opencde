...
static void our_wctomb(
    ...
    unsigned short r, char **bufptr, int *buf_len
#endif
);
static int our_mbtowc(
#if NeedFunctionPrototypes
    unsigned long *p, char *s, size_t n
#endif
);
...
static void set_latin_nop(table, default_val) long *table;
long default_val;
{
        register int i;
        for (i = 0; i < 0x1fff; i++)
...
static void set_latin_tab(fptr, table, fb_default) FILE *fptr;
long *table;
long fb_default;
{
        int i = 0;
...
static void init_latin1tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp = NULL;
...
static void init_latin2tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_latin3tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_latin4tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_latin5tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_latin6tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_latin7tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_latin8tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_latin9tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_jis0201tab(tbl, fb_default) long *tbl;
long fb_default;
{
        int i;
        for (i = 0; i < NRUNE; i++)
...
static void set_cjk_nop(to_tbl, to_max, default_val) long **to_tbl;
long default_val;
int to_max;
{
        register int i;
...
static void set_table(fptr, to_tbl, from_tbl, to_max, fb_default) FILE *fptr;
...
static void init_jis0208tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_ksc5601tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static void init_gb2312tab(tbl, fb_default) long *tbl;
long fb_default;
{
        FILE *fp;
        char dirname[BUFSIZE];
...
static int once = 0;
static int _XlcInitUTFInfo(lcd) XLCd lcd;
{
        if (!once) {
                int i;
                ...
                for (i = 0; i < codeset_num; i++) {
                        ...
                        if (pdata->next) {
                                ...
                                for (j = 0; j < MAX_UTF_CHARSET; j++) {
                                        if (_fallcCompareISOLatin1(
                                                charset->encoding_name,
                                                default_utf_data[j].name) ||
                                            charset->side !=
                                                default_utf_data[j].side) {
                                                continue;
                                        } else {
                                                pdata->initialize =
                                                    default_utf_data[j]
...
static int utftocs(conv, from, from_left, to, to_left, args,
...
static int utf1tocs(conv, from, from_left, to, to_left, args,
                    num_args) XlcConv conv;
char **from;
int *from_left;
char **to;
...
int num_args;
{
        ...
        if (from == NULL || *from == NULL)
                return utftocs(conv, from, from_left, to, to_left, args,
                               num_args);
...
static int ucstocs(conv, from, from_left, to, to_left, args,
                   num_args) XlcConv conv;
XPointer *from;
int *from_left;
char **to;
...
#else
    getutfrune(read_from, from_len) char **read_from;
int *from_len;
#endif
{
        int c, i;
...
static cstoutf(conv, from, from_left, to, to_left, args, num_args) XlcConv conv;
char **from;
int *from_left;
char **to;
int *to_left;
...
int num_args;
{
        ...
        while (pdata->next) {
                if (!_fallcNCompareISOLatin1(charset->name,
                                             pdata->charset->name, cmp_len)) {
...
static cstoucs(conv, from, from_left, to, to_left, args, num_args) XlcConv conv;
char **from;
int *from_left;
XPointer *to;
int *to_left;
...
#else
    our_wctomb(r, utfptr, utf_len) Rune r;
char **utfptr;
int *utf_len;
#endif
{
...
#else
    latin2rune(c, r) unsigned char c;
Rune *r;
#endif
{
        *r = (Rune)c;
...
#else
    jis02012rune(c, r) unsigned char c;
Rune *r;
#endif
{
        /* To Be Implemented */
...
#else
    jis02082rune(c, r) unsigned char c;
Rune *r;
#endif
{
        static enum { state0, state1 } state = state0;
...
#else
    our_mbtowc(p, s, n) unsigned long *p;
char *s;
size_t n;
#endif
{
...
static XlcConv open_mbtocs(from_lcd, from, to_lcd, to) XLCd from_lcd;
char *from;
XLCd to_lcd;
char *to;
...
static XlcConv open_mbstocs(from_lcd, from, to_lcd, to) XLCd from_lcd;
char *from;
XLCd to_lcd;
char *to;
...
static XlcConv open_wcstocs(from_lcd, from, to_lcd, to) XLCd from_lcd;
char *from;
XLCd to_lcd;
char *to;
...
static XlcConv open_cstombs(from_lcd, from, to_lcd, to) XLCd from_lcd;
char *from;
XLCd to_lcd;
char *to;
...
static XlcConv open_cstowcs(from_lcd, from, to_lcd, to) XLCd from_lcd;
char *from;
XLCd to_lcd;
char *to;
{ return create_conv(from_lcd, &cstowcs_methods); }

XLCd _fallcUtfLoader(name) char *name;
{
        XLCd lcd;
