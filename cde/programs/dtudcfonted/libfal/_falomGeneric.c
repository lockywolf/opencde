...
extern Status _falmbDefaultTextPerCharExtents(),
...
extern Status _falmbGenericTextPerCharExtents(),
...
static FontData init_fontdata(font_data, font_data_count) FontData font_data;
int font_data_count;
{
        FontData fd;
        int i;
...
static VRotate init_vrotate(font_data, font_data_count, type, code_range,
                            code_range_num) FontData font_data;
int font_data_count;
int type;
...
static Bool init_fontset(oc) XOC oc;
{
        XOCGenericPart *gen;
        FontSet font_set;
        OMData data;
        ...
        for (; count-- > 0; data++, font_set++) {
                ...
                if ((font_set->font_data = init_fontdata(
                         data->font_data, data->font_data_count)) == NULL)
                        goto err;
                font_set->font_data_count = data->font_data_count;
                if ((font_set->substitute = init_fontdata(
                         data->substitute, data->substitute_num)) == NULL)
                        goto err;
                font_set->substitute_num = data->substitute_num;
                if ((font_set->vmap =
                         init_fontdata(data->vmap, data->vmap_num)) == NULL)
                        goto err;
                ...
                if (data->vrotate_type != VROTATE_NONE) {
                        ...
                        if ((font_set->vrotate = init_vrotate(
                                 data->font_data, data->font_data_count,
                                 data->vrotate_type, data->vrotate,
...
static char *get_prop_name(dpy, fs) Display *dpy;
XFontStruct *fs;
{
        unsigned long fp;
...
static FontData check_charset(font_set, font_name) FontSet font_set;
char *font_name;
{
        FontData font_data;
        char *last;
...
static int check_fontname(oc, name, found_num) XOC oc;
char *name;
int found_num;
{
        ...
        for (i = 0; i < list_num; i++) {
                ...
                for (; font_set_num-- > 0; font_set++) {
                        ...
                        if ((data = check_charset(font_set, fname)) == NULL) {
                                if ((fn2_list = falListFontsWithInfo(
                                         dpy, name, MAXFONTS, &list2_num,
                                         &fs_list)) &&
                                    (prop_fname =
                                         get_prop_name(dpy, fs_list)) &&
                                    (data =
                                         check_charset(font_set, prop_fname)))
                                        fname = prop_fname;
                        }
                        if (data) {
...
static Bool load_fontdata(oc, font_data, font_data_num) XOC oc;
FontData font_data;
int font_data_num;
{
        Display *dpy = oc->core.om->core.display;
...
static Bool load_font(oc) XOC oc;
{
        int i;
        XOCGenericPart *gen = XOC_GENERIC(oc);
...
static Bool load_font_info(oc) XOC oc;
{
        Display *dpy = oc->core.om->core.display;
        XOCGenericPart *gen = XOC_GENERIC(oc);
        FontSet font_set = gen->font_set;
        ...
        for (; num-- > 0; font_set++) {
                ...
                if (font_set->info == NULL) {
                        fn_list =
                            falListFontsWithInfo(dpy, font_set->font_name, 1,
                                                 &fn_num, &font_set->info);
                        if (font_set->info == NULL)
                                return False;
...
static void check_fontset_extents(overall, logical_ascent, logical_descent,
                                  font) XCharStruct *overall;
...
static void set_fontset_extents(oc) XOC oc;
{
...
static Bool init_core_part(oc) XOC oc;
{
        XOCGenericPart *gen = XOC_GENERIC(oc);
        FontSet font_set;
        int font_set_num;
        ...
        font_struct_list =
            (XFontStruct **)Xmalloc(sizeof(XFontStruct *) * count);
...
static char *get_font_name(oc, pattern) XOC oc;
char *pattern;
{
        char **list, *name;
        int count = 0;
...
static char *get_rotate_fontname(font_name) char *font_name;
{
        ...
        rotate_font[0] = '\0';
        for (field_num = 0;
             field_num < CHARSET_ENCODING_FIELD && fields[field_num];
             field_num++) {
...
static Bool is_match_charset(font_data, font_name) FontData font_data;
char *font_name;
{
        char *last;
        int length, name_len;
...
static int parse_all_name(oc, font_data, pattern) XOC oc;
FontData font_data;
char *pattern;
{
...
static int parse_omit_name(oc, font_data, pattern) XOC oc;
FontData font_data;
char *pattern;
{
        ...
        if (is_match_charset(font_data, pattern) == True) {
                ...
                if (font_name = get_font_name(oc, buf)) {
                        font_data->xlfd_name =
                            (char *)Xmalloc(strlen(font_name) + 1);
                        if (font_data->xlfd_name == NULL) {
                                Xfree(font_name);
                                return (-1);
                        }
...
static int parse_fontdata(oc, font_data, font_data_count, name_list,
                          name_list_count, class) XOC oc;
FontData font_data;
int font_data_count;
char **name_list;
int name_list_count;
        ...
        for (; font_data_count-- > 0; font_data++) {
                ...
                case C_VROTATE:
                        if (is_found == True) {
                                ...
                                if ((rotate_name = get_rotate_fontname(
...
static int parse_vw(oc, font_set, name_list, count) XOC oc;
FontSet font_set;
char **name_list;
int count;
{
        ...
        if (vmap_num > 0) {
                if (parse_fontdata(oc, vmap, vmap_num, name_list, count,
                                   C_VMAP) == -1)
                        return (-1);
        }
        ...
        if (vrotate_num > 0) {
                ...
                if (ret == -1) {
                        ...
                        if (sub_num > 0) {
                                ...
                                for (i = 0; i < sub_num; i++) {
                                        vrotate[i].charset_name =
                                            font_set->substitute[i].name;
                                        vrotate[i].side =
                                            font_set->substitute[i].side;
                                        vrotate[i].code_range = code_range;
...
static int parse_fontname(oc) XOC oc;
{
        XOCGenericPart *gen = XOC_GENERIC(oc);
        FontSet font_set;
                ...
                if (font_set->font_data_count > 0) {
                        ...
                        if (ret == -1) {
                                ...
                                if (font_set->font_name == (char *)NULL)
                                        goto err;
                                strcpy(font_set->font_name,
                                       font_set->font_data->xlfd_name);
                                font_set->side = font_set->font_data->side;

                                if (parse_vw(oc, font_set, name_list, count) ==
                                    -1)
                                        goto err;
                                found_num++;

                                /* The substitute font is serched, when the
                                 * primary fonts */
                                /* is not found. */
                        } else {
                                /* The primary information is free from FontSet
                                 * structure */
                                ...
                                /* A vrotate member is replaced to substitute
                                 * information */
                                ...
                                if (ret == -1) {
                                        ...
                                        for (i = 0;
                                             i < font_set->substitute_num;
                                             i++) {
                                                if (font_set->substitute[i]
                                                        .xlfd_name != NULL) {
                                                        break;
                                                }
                                        }
                                        font_set->font_name = (char *)Xmalloc(
                                            strlen(font_set->substitute[i]
                                                       .xlfd_name) +
                                            1);
                                        if (font_set->font_name == (char *)NULL)
                                                goto err;
                                        strcpy(
                                            font_set->font_name,
                                            font_set->substitute[i].xlfd_name);
                                        font_set->side =
                                            font_set->substitute[i].side;
                                        if (parse_vw(oc, font_set, name_list,
                                                     count) == -1)
                                                goto err;
                                        found_num++;
                                }
                        ...
                        if (ret == -1) {
                                ...
                                for (i = 0; i < font_set->substitute_num; i++) {
                                        if (font_set->substitute[i].xlfd_name !=
                                            NULL) {
                                                break;
                                        }
                                }
                                font_set->font_name = (char *)Xmalloc(
                                    strlen(font_set->substitute[i].xlfd_name) +
                                    1);
                                if (font_set->font_name == (char *)NULL)
                                        goto err;
                                strcpy(font_set->font_name,
...
static Bool set_missing_list(oc) XOC oc;
{
        XOCGenericPart *gen = XOC_GENERIC(oc);
        FontSet font_set;
                        ...
                        if (font_set->substitute_num <= 0 ||
                            font_set->substitute == (FontData)NULL)
                                if (font_set->charset_list != NULL) {
                                        length +=
                                            strlen(font_set->charset_list[0]
                                                       ->encoding_name) +
                                            1;
                                } else {
                                        length += 1;
                                ...
                                length +=
                                    strlen(font_set->substitute->name) + 1;
                } else {
...
static Bool create_fontset(oc) XOC oc;
{
...
/* For VW/UDC start */
static void free_fontdataOC(dpy, font_data, font_data_count) Display *dpy;
FontData font_data;
int font_data_count;
{
        for (; font_data_count--; font_data++) {
                ...
                if (font_data->font) {            /* ADD 1996.01.7 */
                        if (font_data->font->fid) /* Add 1996.01.23 */
                                falFreeFont(
                                    dpy, font_data->font); /* ADD 1996.01.7 */
                        else                               /* Add 1996.01.23 */
                                falFreeFontInfo(NULL, font_data->font,
                                                1); /* Add 1996.01.23 */
                        font_data->font = NULL;
                }
...
static void destroy_fontdata(gen, dpy) XOCGenericPart *gen;
Display *dpy;
{
        ...
        if (gen->font_set) {
                ...
                for (; font_set_num--; font_set++) {
                        if (font_set->font_data) {
                                free_fontdataOC(dpy, font_set->font_data,
                                                font_set->font_data_count);
                                Xfree(font_set->font_data);
                                font_set->font_data = NULL;
                        }
                        if (font_set->substitute) {
                                free_fontdataOC(dpy, font_set->substitute,
                                                font_set->substitute_num);
                                Xfree(font_set->substitute);
                                font_set->substitute = NULL;
                        }
                        if (font_set->vmap) {
                                free_fontdataOC(dpy, font_set->vmap,
                                                font_set->vmap_num);
                                Xfree(font_set->vmap);
                                font_set->vmap = NULL;
                        }
...
static void destroy_oc(oc) XOC oc;
{
        Display *dpy = oc->core.om->core.display;
        XOCGenericPart *gen = XOC_GENERIC(oc);
        XFontStruct **font_list, *font;
...
static char *set_oc_values(oc, args, num_args) XOC oc;
XlcArgList args;
int num_args;
{
        ...
        return _fallcSetValues((XPointer)oc, oc->core.resources,
                               oc->core.num_resources, args, num_args,
                               XlcSetMask);
}

static char *get_oc_values(oc, args, num_args) XOC oc;
XlcArgList args;
int num_args;
{
        if (oc->core.resources == NULL)
...
static XOC create_oc(om, args, num_args) XOM om;
XlcArgList args;
int num_args;
{
        XOC oc;
        ...
        if (oc_resources[0].xrm_name == NULLQUARK)
                _fallcCompileResourceList(oc_resources,
        ...
        if (gen->object_name) {
                ...
                for (; count-- > 0; methods_list++) {
                        if (!_fallcCompareISOLatin1(gen->object_name,
                                                    methods_list->name)) {
...
static void free_fontdataOM(font_data, font_data_count) FontData font_data;
int font_data_count;
{
        for (; font_data_count--; font_data++) {
                if (font_data->name) {
...
static Status close_om(om) XOM om;
{
        XOMGenericPart *gen = XOM_GENERIC(om);
        OMData data;
        int count;
        ...
        if (data = gen->data) {
                for (count = gen->data_num; count-- > 0; data++) {
                        ...
                        if (data->font_data) {
                                free_fontdataOM(data->font_data,
...
static char *set_om_values(om, args, num_args) XOM om;
XlcArgList args;
int num_args;
{
        ...
        return _fallcSetValues((XPointer)om, om->core.resources,
                               om->core.num_resources, args, num_args,
                               XlcSetMask);
}

static char *get_om_values(om, args, num_args) XOM om;
XlcArgList args;
int num_args;
{
        if (om->core.resources == NULL)
...
static XOM create_om(lcd, dpy, rdb, res_name, res_class) XLCd lcd;
Display *dpy;
XrmDatabase rdb;
char *res_name;
char *res_class;
{
        ...
        if (om_resources[0].xrm_name == NULLQUARK)
                _fallcCompileResourceList(om_resources,
                                          XlcNumber(om_resources));
...
static OMData add_data(om) XOM om;
{
        XOMGenericPart *gen = XOM_GENERIC(om);
        OMData new;
        int num;
        ...
        if (num = gen->data_num)
                new =
                    (OMData)Xrealloc(gen->data, (num + 1) * sizeof(OMDataRec));
        else
...
/* For VW/UDC */
FontData falread_EncodingInfo(count, value) int count;
char **value;
{
        ...
        for (; count-- > 0; font_data++) {
                ...
                */
                ...
                if (bufptr && (scp = strchr(bufptr, '['))) {
                        font_data->scopes =
                            falparse_scopemaps(scp, &(font_data->scopes_num));
                }
        }
        return (ret);
}

static CodeRange read_vrotate(count, value, type, vrotate_num) int count;
char **value;
int *type;
int *vrotate_num;
{
...
static void read_vw(lcd, font_set, num) XLCd lcd;
OMData font_set;
int num;
{
        ...
        if (count > 0) {
                dbg_printValue(buf, value, count);
                font_set->vrotate =
                    read_vrotate(count, value, &(font_set->vrotate_type),
                                 &(font_set->vrotate_num));
        }
}
/* VW/UDC end */
static Bool init_om(om) XOM om;
{
        ...
        _fallcGetResource(lcd, "XLC_FONTSET", "on_demand_loading", &value,
                          &count);
        ...
        for (num = 0;; num++) {
                ...
                if (count < 1) {
                        ...
                        _fallcGetResource(lcd, "XLC_FONTSET", buf, &value,
                                          &count);
                        if (count < 1)
                                break;
                }
                ...
                charset_list =
                    (XlcCharSet *)Xmalloc(sizeof(XlcCharSet) * count);
                if (charset_list == NULL)
                        return False;
...
#else
    _falomGenericOpenOM(lcd, dpy, rdb, res_name, res_class) XLCd lcd;
Display *dpy;
XrmDatabase rdb;
char *res_name;
char *res_class;
