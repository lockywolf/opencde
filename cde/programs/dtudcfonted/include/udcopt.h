...
#define COMM_SNF_FILEVERSION(snf_fd, finf, buf, permission)                    \
        {                                                                      \
                ...
                if (((finf)->version1 != FONT_FILE_VERSION) ||                 \
...
#define FAL_GET_SNF_HEADER(buf, protect_key_data, fal_utyerror, fal_utyderror, \
        ...
        {                                                                      \
                ...
                if (((finf)->pFinf->version1 != (finf)->pFinf->version2) ||    \
...
#define GETLISTFILEPATH(locale, srch_path, list_d, list_c, list_l, list_file)  \
        {                                                                      \
                ...
                if (((srch_path) = getenv(DTUDCFONTS)) == NULL) {              \
                        sprintf((list_d), "%s/%s", DTUDCFONTSLISTDIR,          \
                                (list_file));                                  \
                        sprintf((list_c), "%s/C/%s", DTUDCFONTSLISTDIR,        \
                                (list_file));                                  \
                        sprintf((list_l), "%s/%s/%s", DTUDCFONTSLISTDIR,       \
                                (locale), (list_file));                        \
                } else {                                                       \
                        sprintf((list_d), "%s/%s", (srch_path), (list_file));  \
                        sprintf((list_c), "%s/C/%s", (srch_path),              \
                                (list_file));                                  \
                        sprintf((list_l), "%s/%s/%s", (srch_path), (locale),   \
                                (list_file));                                  \
                }                                                              \
        }
