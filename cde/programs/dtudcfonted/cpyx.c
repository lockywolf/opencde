...
/*ARGSUSED*/
static void CpyXLFDCB(w, client_data, call_data) Widget w;
caddr_t client_data, *call_data;
{
        ...
        str = _XmStringUngenerate(st[0], NULL, XmMULTIBYTE_TEXT,
                                  XmMULTIBYTE_TEXT);
...
void CpyPtnXLFD() {
        if (cpyDialog == NULL) {
                 ...
                 * add callbacks
                 */
                XtAddCallback(cpyDialog, XmNokCallback, (XtCallbackProc)_unmap,
                              NULL);
                XtAddCallback(cpyDialog, XmNokCallback,
                              (XtCallbackProc)CpyXLFDCB, NULL);
                XtAddCallback(cpyDialog, XmNokCallback, (XtCallbackProc)CpyCB,
                              NULL);
...
static void udc_call(w) Widget w;
{
        XmString label;
        char *moji;
        XtVaGetValues(w, XmNlabelString, &label, NULL);
        moji = _XmStringUngenerate(label, NULL, XmMULTIBYTE_TEXT,
                                   XmMULTIBYTE_TEXT);
        if (strncmp(moji, "*", 1) == 0) {
                udc_flag = False;
...
static void sty_call(w) Widget w;
{
        XmString label;
        char *moji;
        if (sty_val) {
...
static void wls_call(w) Widget w;
{
        XmString label;
        char *moji;
        XtVaGetValues(w, XmNlabelString, &label, NULL);
...
static void hls_call(w) Widget w;
{
        XmString label;
        char *moji;
        XtVaGetValues(w, XmNlabelString, &label, NULL);
...
static void button_set1() {
        ...
        for (j = 0; j < udc_count; j++) {
                for (i = 0, f = fontlist->list, found = False;
                     i < fontlist->num; i++, f++) {
                        if (udc[j] == f->cd_set) {
                                found = True;
                                break;
...
static void font_init() {
        ...
        for (i = 0, f = fontlist->list; i < fontlist->num; i++, f++) {
                ...
                if (found == False) {
                        sty[sty_count] = XtMalloc(sizeof(char) *
                                                  (strlen(f->style.name) + 1));
        ...
        XtAddCallback(button, XmNactivateCallback, (XtCallbackProc)udc_call,
                      NULL);
        ...
        XtManageChild(button);
        XtAddCallback(button, XmNactivateCallback, (XtCallbackProc)sty_call,
                      NULL);
        ...
        XtManageChild(button);
        XtAddCallback(button, XmNactivateCallback, (XtCallbackProc)wls_call,
                      NULL);
        ...
        XtManageChild(button);
        XtAddCallback(button, XmNactivateCallback, (XtCallbackProc)hls_call,
                      NULL);
...
/*ARGSUSED*/
static void arrow_change(w, wid) Widget w;
Widget wid;
{
...
static void code_input2() {
        extern void CodeWindow();
        CodeWindow(codeText[2], fullFontData.xlfdname, False);
}

static Widget CreateCopyXLFD(top) Widget top;
{
        ...
        XtSetArg(args[n], XmNcancelLabelString, xms1);
        n++;
        xms2 = XmStringCreateLocalized(resource.quit_label);
        XtSetArg(args[n], XmNhelpLabelString, xms2);
        n++;
        XtSetArg(args[n], XmNnoResize, True);
        n++;
        XtSetArg(args[n], XmNminimizeButtons, True);
        n++;
        XtSetArg(args[n], XmNtitle, resource.l_copy_title);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNmaxLength, 4);
        n++;
        XtSetArg(args[n], XmNeditable, True);
        n++;
        XtSetArg(args[n], XmNcursorPositionVisible, True);
        n++;
        focus_widget = codeText[0] = XmCreateText(row4, "copytext", args, n);
        XtManageChild(codeText[0]);

        XtAddCallback(codeText[0], XmNfocusCallback, (XtCallbackProc)focus,
                      NULL);
        ...
        XtAddCallback(codeText[1], XmNfocusCallback, (XtCallbackProc)focus,
                      NULL);
        ...
        XtAddCallback(code, XmNactivateCallback, (XtCallbackProc)code_input2,
                      (XtPointer)NULL);
