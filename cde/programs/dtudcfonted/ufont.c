...
static char *GetMessage(set, number, string) int set, number;
char *string;
{
        char *tmp, *ret;
...
int efctPtnNum() {
        ...
        for (cnt = 0, sq = edlist.sqstart; sq < (edlist.sqstart + edlist.nptn);
             sq++) {
                no = ptnSqToNo(sq);
...
void Error_message(widget, message) Widget widget;
char *message;
{
        ...
        PopupNotice((widget), message, D_ERROR, &LockBTN, True,
                    resource.l_question_title);
}

void Error_message2(widget, message) Widget widget;
char *message;
{
        static NoticeButton is_lock[] = {
            NBTNARGS(CancelCB, NULL, 'C', True, True)};
 ...
 */
/*ARGSUSED*/
static void CBmOblB_edit(widget, clientData, callData) Widget widget;
caddr_t clientData;
caddr_t callData;
{
        int ptn_n;
        ...
        if (efctPtnNum() > 0) {
                for (i = edlist.sqstart; i < (edlist.sqstart + edlist.nptn);
                     i++) {
...
static void ContReadCB(w) Widget w;
{
...
/*ARGSUSED*/
static int QuitEditPtn(widget, clientData, callData) Widget widget;
caddr_t clientData;
caddr_t callData;
{
...
void OpenCB(w, client_data, call_data) Widget w;
...
XtPointer call_data;
{
        ...
        if (QuitEditPtn((Widget)NULL, (caddr_t)NULL, (caddr_t)NULL)) {
                ...
                if (!fullFontData.prm) {
                        ...
                        SetItemLabel(&SaveReadBTN1, 1,
                                     resource.l_dont_save_exit);
                        SetItemLabel(&SaveReadBTN1, 2, resource.l_cancel);
                        PopupNotice((w), resource.mn_saved_open, D_QUESTION,
                                    &SaveReadBTN1, True,
                                    resource.l_question_title);
                } else {
                        SetItemLabel(&SaveReadBTN2, 0,
                                     resource.l_dont_save_exit);
...
void ReadCB(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
{
...
static Boolean WPwriteSNF(restart) int restart;
{
        int rc, err;
        char str[MG_MAXSIZE];
        ...
        case -1:
                ...
                switch (err) {
                case 001:
                        sprintf(str, "%s(permission denied)",
                                resource.me_write_snf);
                        break;
                case 002:
        ...
        default:
                if (rc == 1101)
                        ...
                        sprintf(str, "%s(%3d%%)", resource.mg_write_snf,
                                rc - 1000);
...
/*ARGSUSED*/
void CBeOblB_aEnd(widget, clientData, callData) Widget widget;
caddr_t clientData;
caddr_t callData;
{
...
/*ARGSUSED*/
void CBeOblB_rCmd(widget, proc, callData) Widget widget;
int proc;
caddr_t callData;
{
...
/*ARGSUSED*/
void CBeOblB_rCmdp(widget, proc, callData) Widget widget;
int proc;
caddr_t callData;
{
        ...
        case PROC_CPY:
                if (!select_x && !select_y && !select_w && !select_h)
                        return;
                rubBand(r1_x, r1_y, r2_x, r2_y);
                bitDrawCpy(edg.ptn, select_x, select_y, select_w, select_h,
                           False);
                cut_w = select_w;
                cut_h = select_h;
                select_x = select_y = select_w = select_h = 0;
        ...
        case PROC_PASTE:
                XtAddEventHandler(wgeBulB_edit,
                                  ButtonReleaseMask | PointerMotionMask, False,
                                  musPasteProc, NULL);
        }
}
...
/*ARGSUSED*/
void CBeOblB_rCan(widget, clientData, callData) Widget widget;
caddr_t clientData;
caddr_t callData;
{
 ...
 */
int RelToAbsSq(from, cnt) int from;
int cnt;
{
        int i;
        int no;
 ...
 */
int AbsSqToRel(from, to) int from;
int to;
{
        int sq;
        int cnt;
...
/*ARGSUSED*/
void EHeStaT_list(widget, select, e) Widget widget;
int select;
XEvent *e; /*    X Event	*/
{
        int sq, no;
...
/*ARGSUSED*/
void CBeScro(widget, clientData, callData) Widget widget;
caddr_t clientData;
caddr_t callData;
{
        int newl;
        ...
        n = 0;
        XtSetArg(arg[n], XmNvalue, (XtArgVal)&newl);
        n++;
        ...
        /*
         */
        new_slct = edlist.slctloc - AbsSqToRel(edlist.sqstart + edlist.statloc,
...
/*ARGSUSED*/
void EHeBulB_eMEv(widget, clientData, e) Widget widget;
caddr_t clientData;
XEvent *e; /*    X Event			*/
{
        int px, py;
...
/*ARGSUSED*/
void EHeBulB_eExp(widget, clientData, e) Widget widget;
caddr_t clientData;
XEvent *e; /*    X Event	*/
{
        int x1, y1;
...
/*ARGSUSED*/
void CBeRecB_obj(widget, obj, call) Widget widget;
int obj;
...
/*ARGSUSED*/
void EHeBulB_dExp(widget, clientData) Widget widget;
caddr_t clientData;
{
        if (xl.display == NULL) {
                return;
...
static int codeAreaSet(s_code, e_code) int *s_code;
int *e_code;
{
        char *str;
        char delm;
        ...
        *e_code = 0;
        ...
        if (codeCheck(*s_code) || codeCheck(*e_code) || (*s_code > *e_code)) {
                return (-1);
        }
        return (0);
}
...
Boolean BeforeMngCheck(s_code, e_code) int *s_code;
int *e_code;
{
...
void DoAddProc(s_code, e_code) int s_code;
int e_code;
{
        int code;
...
void DoDelProc(s_code, e_code) int s_code;
int e_code;
{
        resetEditMode(RES_MSG | RES_PROC | RES_SLCT | RES_RSV);
        ...
        if (setRealDelArea(&dn.s_ncode, &dn.e_ncode, &dn.sq_start,
                           &dn.sq_end) == -1) {
...
static int setRealDelArea(s_ncode, e_ncode, sq_start, sq_end) int *s_ncode,
    *e_ncode;
int *sq_start, *sq_end;
{
        int ncode;
...
Boolean BeforeCpyCheck(proc) int proc;
{
        int s_ncode, e_ncode;
        int r1_code, r2_code;
        int no;
        ...
        if (r1_code != 0) {
                if (r1_code == r2_code) {
                        ...
                        sprintf(rstr, "%s %x - %x ", resource.l_code_range,
                                r1_code, r2_code);
                }
                PopupCpyNotice(rstr);
...
void DoCpyProc() {
        ...
        copyPatterns(cpm.fdata, cpm.s1_code, cpm.s2_code, cpm.d1_code,
                     cpm.proc);
}
...
/*ARGSUSED*/
static void xerror(d, ev) Display *d;
...
char *argv[];
{
        ...
        XtGetApplicationResources(toplevel, &resource, app_resources,
        ...
        resource.mg_write_snf =
            GETMESSAGE(2, 2, "Registration processing inside.");
        resource.me_write_snf =
            GETMESSAGE(2, 4, "Failed in the registration of the character.");
        resource.me_illegal_code = GETMESSAGE(2, 6, "Illegal code.");
        resource.me_non_memory =
            GETMESSAGE(2, 8,
                       "Because the memory allocation cannot be "
                       "done, it is not possible to add.");
        resource.mg_non_code =
            GETMESSAGE(2, 10, "There is no character of the specified code.");
        resource.me_illegal_srccode = GETMESSAGE(
            2, 12, "The mistake is found in the specified copy origin code.");
        resource.me_illegal_destcode = GETMESSAGE(
            2, 14, "The mistake is found in the specified copy target code.");
        resource.me_nodelete_code =
            GETMESSAGE(2, 16, "There is no character in the specified area.");
        resource.me_wait = GETMESSAGE(2, 18, "Please wait for a while.");
        resource.me_non_srcfile =
            GETMESSAGE(2, 20, "No copy origin code file is specified.");
        resource.mn_no_font =
            GETMESSAGE(2, 22,
                       "Informations of user defined character cannot "
                       "collect.\nThere is no font specified.");
        resource.mn_plural_font =
            GETMESSAGE(2, 24,
                       "Informations of user defined character cannot "
                       "collect.\nThere are plural fonts specified.");
        resource.mn_not_udc =
            GETMESSAGE(2, 26, "The specified font do not have any UDCs areas.");
        resource.mn_no_mem =
            GETMESSAGE(2, 28, "There are no memories any more.");
        resource.mn_illegal_area =
            GETMESSAGE(2, 30, "The information of UDCs area is illegal.");
        resource.mn_no_read = GETMESSAGE(
            2, 32, "The font file cannot read.\nPattern area was broken.");
        resource.mn_prop_font = GETMESSAGE(
            2, 34, "Dtudcfonted cannot edit proportional character font.");
        resource.mn_saved_open =
            GETMESSAGE(2, 36,
                       "There are unsaved edits.\nDo you want to save them "
                       "before opening?");
        resource.mn_saved_exit =
            GETMESSAGE(2, 38,
                       "There are unsaved edits.\nDo you want to save them "
                       "before exiting?");
        resource.mn_cpy_lost = GETMESSAGE(
            2, 40,
            "When the copy processing is done, the glyph\nimage within the "
            "following "
            "ranges is changed.\nDo you want to do the copy processing?\n\n");
        resource.mn_no_perm = GETMESSAGE(
            2, 42,
            "No write permission for this font. You should change the "
            "file access mode before saving.\n");

        resource.l_selectitem =
            GETMESSAGE(2, 100, "Select Items"); /* font open */
        ...
        resource.l_manage_w =
            GETMESSAGE(2, 146, "Add/Delete..."); /* character menu */
        resource.l_copy_w = GETMESSAGE(2, 148, "Copy...");

        resource.l_xlfd =
            GETMESSAGE(2, 150, "XLFD name..."); /* information menu */
        ...
        resource.l_manage_code =
            GETMESSAGE(2, 160, "Code(hex):"); /* copy dialog */
        resource.l_copy_src_code =
            GETMESSAGE(2, 162, " Copy origin code(hex):");
        resource.l_copy_dest_code =
            GETMESSAGE(2, 164, "Copy target code(hex):");
        ...
        resource.mg_start_del_s = GETMESSAGE(
            2, 180, "The following character will be deleted."); /* delete
                                                                    notice
                                                                    dialog */
        resource.mg_start_del_cs = GETMESSAGE(2, 182, "character");
        resource.mg_start_del_m =
            GETMESSAGE(2, 184, "The following characters will be deleted.");
        ...
        resource.l_question_title =
            GETMESSAGE(2, 204, "dtudcfonted - Question");
        ...
        resource.l_codearea_title =
            GETMESSAGE(2, 218, "dtudcfonted - CodeArea");
        ...
        resource.falerrmsg[1] =
            GETMESSAGE(2, 301, "Cannot open the font file.");
        resource.falerrmsg[2] =
            GETMESSAGE(2, 302, "Cannot read the font file.");
        resource.falerrmsg[3] = GETMESSAGE(2, 303, "There is no more memory.");
        resource.falerrmsg[4] = GETMESSAGE(2, 304, "Fatal error occurred.");
        resource.falerrmsg[5] =
            GETMESSAGE(2, 305, "The specified font file does not exist.");
        resource.falerrmsg[6] =
            GETMESSAGE(2, 306, "This font is not a pcf or snf font.");
        resource.falerrmsg[7] =
            GETMESSAGE(2, 307, "Cannot open fonts.list file.");
        resource.falerrmsg[8] =
            GETMESSAGE(2, 308, "The format of fonts.list file is illegal.");
        resource.falerrmsg[9] = GETMESSAGE(
            2, 309, "The descriptions of the fonts.list file are incorrect.");
        resource.falerrmsg[10] =
            GETMESSAGE(2, 310, "The format of fonts.list file is illegal.");
        resource.falerrmsg[11] =
            GETMESSAGE(2, 311, "Cannot open fonts.dir file.");
        resource.falerrmsg[12] =
            GETMESSAGE(2, 312, "Cannot read fonts.dir file.");
        resource.falerrmsg[13] =
            GETMESSAGE(2, 313, "Cannot read font properties.");
        resource.falerrmsg[14] =
            GETMESSAGE(2, 314, "Cannot get FONT property.");
        resource.falerrmsg[15] =
            GETMESSAGE(2, 315, "Cannot get FAMILY_NAME property.");
        resource.falerrmsg[16] = GETMESSAGE(
            2, 316, "This font file is already opened by other application.");
        ...
        resource.falerrmsg[19] = GETMESSAGE(
            2, 319, "Cannot get lock information from the font file.");
        resource.falerrmsg[20] =
            GETMESSAGE(2, 320, "Cannot find the specified font file.");
        resource.falerrmsg[21] =
            GETMESSAGE(2, 321, "Cannot read NLS database.");
        resource.falerrmsg[22] =
            GETMESSAGE(2, 322, "Cannot get charset names from NLS database.");
        resource.falerrmsg[23] =
            GETMESSAGE(2, 323, "Charset name not defined in NLS database.");
        resource.falerrmsg[24] =
            GETMESSAGE(2, 324, "The specified font has not been opened.");
...
void chgEdCode(code, mode) int code;
char mode;
{
        int ncode;
        int esq;
        ...
        if (esq < 0) {
                ...
                while (RelToAbsSq(edlist.sqstart + statloc, edlist.nlist - 1) <
                       0)
                        statloc = RelToAbsSq(edlist.sqstart + statloc, -1) -
                                  edlist.sqstart;
                slctloc =
                    AbsSqToRel(edlist.sqstart + statloc, edlist.sqstart + esq);
        }

        n = 0;
        if ((slimax = efctPtnNum()) < edlist.nlist)
                slimax = edlist.nlist;
        XtSetArg(arg[n], XmNmaximum, (XtArgVal)slimax);
        n++;
        XtSetArg(
            arg[n], XmNvalue,
            (XtArgVal)AbsSqToRel(edlist.sqstart, edlist.sqstart + statloc));
        n++;
...
void chgEdList(statloc, slctloc, mode) int statloc;
int slctloc;
char mode;
{
        int sq;
        ...
        if ((mode == ON) || (edlist.statloc != statloc)) {
                for (i = 0, sq = edlist.sqstart + statloc; i < edlist.nlist;
                     sq++) {
                        no = ptnSqToNo(sq);
                        sq_disp[i] = sq;
...
static void chgEdPtn(code) int code;
{
...
static void DrawRectEdPn(x1, y1, x2, y2) int x1;
int y1;
int x2;
int y2;
{
        ...
        if (nron)
                XFillRectangles(xl.display, xl.edit_winID, xl.borderGC, recOn,
                                nron);
        if (nroff)
                XFillRectangles(xl.display, xl.edit_winID, xl.backGC, recOff,
                                nroff);
}
...
static void DrawBorderEdPn(x1, y1, x2, y2) int x1;
int y1;
int x2;
int y2;
{
        ...
        if (ndseg) {
                XDrawSegments(xl.display, xl.edit_winID, xl.dashGC, dseg,
                              ndseg);
        }
}
...
static void DrawPointEdPn(x, y, mode) int x;
int y;
int mode;
{
        int x1, y1;
        ...
        if (mode != 0) {
                XFillRectangle(xl.display, xl.edit_winID, xl.borderGC, x1, y1,
                               x2 - x1, y2 - y1);
        } else {
                XFillRectangle(xl.display, xl.edit_winID, xl.backGC, x1, y1,
                               x2 - x1, y2 - y1);
        }
}
...
static void DrawDpPn() {
        ...
        XPutImage(xl.display, xl.disp_winID, xl.borderGC, xl.dispImage, 0, 0, 0,
                  0, edg.width, edg.height);
        if (xl.list_winID) {
                XPutImage(xl.display, xl.list_winID, xl.borderGC, xl.dispImage,
...
static void DrawPointDpPn(x, y, mode) int x;
int y;
int mode;
{
        ...
        if (mode != 0) {
                ...
                if (xl.list_winID)
                        XDrawPoint(xl.display, xl.list_winID, xl.borderGC, x,
                                   y);
        } else {
...
static void musPoint(evtype, px, py) int evtype;
int px;
int py;
{
        switch (evtype) {
...
static void musLine(evtype, px, py) int evtype;
int px;
int py;
{
        int r1_x, r1_y;
        ...
        default:
                ...
                bitDrawLine(edg.ptn, em.src1_px, em.src1_py, em.src2_px,
                            em.src2_py, edpane.color);
                edg.flag = ON;
...
static void musCircle(evtype, px, py) int evtype;
int px;
int py;
{
        int r1_x, r1_y;
        ...
        default:
                ...
                if ((px < 0) || (py < 0) || (edg.width <= px) ||
                    (edg.height <= py)) {
                        return;
                }
                bitDrawCircle(edg.ptn, em.src1_px, em.src1_py, px, py,
                              edpane.color);
...
static void musRect(proc, evtype, px, py) int proc;
int evtype;
int px;
int py;
{
...
static void musRegionProc(proc, evtype, px, py) int proc;
int evtype;
int px;
int py;
{
...
/*ARGSUSED*/
static void musPasteProc(w, client_data, event) Widget w;
XtPointer client_data;
XEvent *event;
{
        ...
        switch (event->type) {
        case MotionNotify:
                tx = (event->xmotion.x / edpane.pix_w) * edpane.pix_w +
                     edpane.pix_w / 2;
                ty = (event->xmotion.y / edpane.pix_h) * edpane.pix_h +
                     edpane.pix_h / 2;
                if (tx == ox && ty == oy)
                        return;
                if (ox) {
                        rubBand(ox, oy, ox + cut_w * edpane.pix_w,
                                oy + cut_h * edpane.pix_h);
                }
                ox = tx;
                oy = ty;
                rubBand(ox, oy, ox + cut_w * edpane.pix_w,
...
static void rubLine(x1, y1, x2, y2) int x1;
int y1;
int x2;
int y2;
{
...
static void rubBand(x1, y1, x2, y2) int x1;
int y1;
int x2;
int y2;
{
...
static void rubCircle(ox, oy, rx, ry) int ox;
int oy;
int rx;
int ry;
{
        ...
        x = ox - (int)r;
        y = oy - (int)r;
        XDrawArc(xl.display, xl.edit_winID, xl.rubGC, x, y, 2 * r - 1,
                 2 * r - 1, 0, 360 * 64);
}

static void resetEditMode(flag) UINT flag;
{
        int r1_x, r1_y;
        int r2_x, r2_y;
...
/*ARGSUSED*/
static void copyPatterns(fdata, s1_code, s2_code, d1_code,
                         proc) FalFontData *fdata;
int s1_code;
int s2_code;
int d1_code;
int proc;
