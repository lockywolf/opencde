...
static void OpenWindowCB() {
        ...
        str = (char *)_XmStringUngenerate(st[0], NULL, XmMULTIBYTE_TEXT,
                                          XmMULTIBYTE_TEXT);
...
/*ARGSUSED*/
static void OpenCancelCB(widget, clientData, callData) Widget widget;
caddr_t clientData;
caddr_t callData;
{
...
static void udc_call(w) Widget w;
{
        ...
        moji = (char *)_XmStringUngenerate(label, NULL, XmMULTIBYTE_TEXT,
                                           XmMULTIBYTE_TEXT);
        if (strncmp(moji, "*", 1) == 0) {
                udc_flag = False;
...
static Widget CreateSelectXLFD(top) Widget top;
{
        ...
        n++;
        XtSetArg(args[n], XmNtitle, resource.l_open_title);
        n++;
        ...
        XtAddCallback(editW, XmNhelpCallback, (XtCallbackProc)OpenCancelCB,
                      NULL);
