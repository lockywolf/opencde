...
char *argv[];
{
        ...
        if (dsp_opt & C_OPT) {
                ...
                if (DispUdcCpArea(stdout) == -1) {
                        ErrMsgTable_FalGetFontList(argv[0], fal_utyerror,
                                                   fal_utyderror);
                        exit(1);
                }
                exit(0);
...
static void dsp_info_list(com, dspopt, lst, unknown_file_lst, errflg) char *com;
int dspopt;
FalFontDataList *lst;
...
int *errflg;
{
        ...
        for (i = 0, srt_cnt = 0; i < lst->num; i++) {
                ...
                if (entry == entry_end) {
                        if (dspopt & L_OPT || dspopt & X_OPT)
                                dspinfo_1line(lst->list[entry], stdout,
                                              unknown_file_lst, entry, errflg,
                                              dspopt);
                        else
                                memmove(&(srt_lst.list[srt_cnt++]),
                                        &(lst->list[entry]),
                                        sizeof(FalFontData));
                        continue;
                }
                ...
                if (j > entry_end) {
                        ret =
                            GetDefaultFile(lst->list[entry].size.h,
                        ...
                        for (j = entry; j <= entry_end; j++) {
                                ret = search_fontfile_name(
                                    lst->list[j], tmpfname, unknown_file_lst, j,
                                    errflg);
                                if (ret == -1)
                                        continue;
                                else if (ret == -2)
                ...
                if (ret) {
                        ...
                        case -1:
                                ...
                                break;
                        case -2:
                                unknown_file_lst->entry[unknown_file_lst->num]
                                    .start = entry;
                                unknown_file_lst->entry[unknown_file_lst->num]
                                    .end = entry_end;
                                ...
                                *errflg |= REDEFINED_FONT;
                                break;
                        case -3:
                                USAGE4("%s : The user defined character "
                                       "information file cannot be "
                                       "read.\"%s/%s/%s \"\n\n",
                                       com, fontdir, utillocale, UDFLIST_DEF);
                                exit(1);
                        case -4:
                                USAGE4("%s : The mistake is found in the "
                                       "description of the user defined "
                                       "character information file.\"%s/%s/%s "
                                       "\"\n\n",
                ...
                for (j = entry; j <= entry_end; j++) {
                        ret = search_fontfile_name(lst->list[j], tmpfname,
                                                   unknown_file_lst, j, errflg);
                        if (ret == -1)
                                continue;
                        else if (ret == -2)
                ...
                if (j > entry_end) {
                        unknown_file_lst->entry[unknown_file_lst->num].start =
                            entry;
                        unknown_file_lst->entry[unknown_file_lst->num].end =
                            entry_end;
                        unknown_file_lst->num++;
                        *errflg |= REDEFINED_FONT;
                }
        ...
        if (!(dspopt & L_OPT)) { /* display character size and style */
                ...
                if (dspopt & A_OPT)
                        mask = REQ_STYLE | REQ_CHARSIZE | REQ_LETTER |
                               REQ_DELEQUAL;
                else
                ...
                for (i = 0; i < cnt; i++)
                        dspinfo_1line(srt_lst.list[sort_list[i]], stdout,
                                      unknown_file_lst, sort_list[i], errflg,
                                      dspopt);
                free(sort_list);
                free(srt_lst.list);
        }
...
static void dsp_error_title(com, errflg, dspopt) char *com;
int *errflg;
int dspopt;
{
        USAGE("\n\n");
        if (dspopt & A_OPT) {
                if (*errflg & SAME_FONT)
                        USAGE1("%s :  There are fonts that are same character "
                               "size and have same style, \ncode set and XLFD. "
                               "It is not possible to select user defined "
                               "character fonts uniquely.\n\n",
                               com);
                USAGE("Please inquire of the system manager.\n\n");
        } else {
                if (*errflg & NO_FONT)
                        USAGE1("\n%s : Mistake is found in the name of user "
                               "defined character fonts \nfollowing character "
                               "size and style.\n\n",
                               com);
                if (*errflg & REDEFINED_FONT)
                        USAGE3(" Please describe the following file name in "
...
static int dsp_error_files(lst, unknown) FalFontDataList *lst;
MISS_ENTRY *unknown;
{
        int i, j;
        ...
        for (i = 0; i < unknown->num; i++) {
                ...
                for (j = entry_start; j <= entry_end; j++) {
                        ret = search_fontfile_name(lst->list[j], tmp_fname,
                                                   NULL, 0, 0);
                        if (ret == -1) {
                                USAGE("\t\tThere is no font file that is "
                                      "required.");
                                USAGE1("\t%s\n", lst->list[j].xlfdname);
                                continue;
                        } else if (ret == -2) {
...
static void sort_offset_list(flist, mask, sort_list,
                             data_cnt) FalFontDataList *flist;
int mask;
int **sort_list;
int *data_cnt;
{
        ...
        if ((size_offset_list = (int *)malloc(flist->num * sizeof(int))) ==
            NULL) {
                USAGE("dtlsgpf: malloc error.\n");
                exit(1);
        }
        for (i = 0; i < flist->num; i++)
                size_offset_list[i] = -1;

        for (i = 0, cnt = 0; i < flist->num; i++) {
                if (mask & REQ_DELEQUAL) {
                        exist = REQ_DELEQUAL;
                        for (j = 0; j < cnt; j++) {
                                if (size_offset_list[j] == -1)
                                        break;
                                ...
                                if ((mask & REQ_LETTER) &&
                                    (flist->list[size_offset_list[j]]
                                         .letter.h ==
                                     flist->list[i].letter.h)) {
                                        exist |= REQ_LETTER;
                                }
                                if ((mask & REQ_STYLE) &&
                                    !strcmp(flist->list[size_offset_list[j]]
                                                .style.name,
        ...
        for (i = 0; i < cnt - 1; i++) {
                for (j = i + 1; j < cnt; j++) {
                        ...
                        if (mask & REQ_STYLE) {
                                if (strcoll(flist->list[size_offset_list[i]]
                                                .style.name,
                                            flist->list[size_offset_list[j]]
                                                .style.name) > 0) {
                                        SORTOFFSETNUM;
...
static void dsp_title(fp, dspopt) FILE *fp;
int dspopt;
{
        if (dspopt & L_OPT || dspopt & X_OPT) {
                fprintf(fp, "\n");
...
static void dspinfo_1line(data, fp, unknown_file_lst, offset, errflg,
                          dspopt) FalFontData data;
FILE *fp;
MISS_ENTRY *unknown_file_lst;
int offset;
int *errflg;
int dspopt;
{
        ...
        ret = search_fontfile_name(data, tmp_fname, unknown_file_lst, offset,
                                   errflg);
        if (ret == -1)
                return;
        else if (ret == -2)
        ...
        if (dspopt & X_OPT)
                fprintf(fp, "%d \t", FALCODESETTONUM(data.cd_set));
...
static void disp_no_fontslist(dspopt) int dspopt;
{
        fprintf(
            stdout,
...
static void put_help(prog_name) char *prog_name;
{
        ...
        USAGE(
            "\t-help    :  display of of the command parameter informations\n");

        USAGE("        (If the option is not specified, only the character "
              "size are displayed.)\n");
}

static int search_fontfile_name(data, fname, unknown_file_lst, offset,
                                errflg) FalFontData data;
char *fname;
MISS_ENTRY *unknown_file_lst;
int offset;
int *errflg;
{
        ...
        mask = FAL_FONT_MASK_UNDEFINED | FAL_FONT_MASK_DEFINED |
               FAL_FONT_MASK_XLFDNAME | FAL_FONT_MASK_SIZE_H |
                ...
                if (flist) {
                        ...
                        case _FAL_STAT_ER:
                                ...
                                case ENOENT:
                                        ...
                                        return (-1);
                                default:
                                        USAGE2("System call error occured. "
                                               "fal_utyerror = %d "
                                               "fal_utyderror = %d\n",
                        ...
                        default:
                                USAGE1(
                                    "%s : cannot get font information list.\n",
                                    command_name);
                                return (-2);
                        }
                }
...
static int sort_fontlist(lst) FalFontDataList *lst;
{
        int i, j;
        FalFontDataList srt;
        FalFontData target;
        ...
        if ((srt.list = (FalFontData *)malloc(sizeof(FalFontData) * srt.num)) ==
            NULL) {
                return FAL_ERROR;
        }
...
static int get_new_target(target, choose) FalFontData *target;
FalFontData *choose;
{
        FalFontData diff;
        DEF_STR_CHK;
