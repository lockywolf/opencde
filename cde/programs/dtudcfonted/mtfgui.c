...
Widget editPopW, wgeScro, wgeBulB_edit;
static Widget wgeStaT_form[EDLIST_MAX], wgeStaT_disp[EDLIST_MAX],
    wgeStaT_list[EDLIST_MAX], wgeBulB_disp;
...
static void XlfdCB() {
        ...
        XmStringFree(cs2);
        n = 0;
        XtSetArg(args[n], XmNtitle, resource.l_xlfd_title);
        n++;
...
static void CodeAreaCB() {
        ...
        cs = XmStringCreateLocalized(mess);
        n = 0;
        XtSetArg(args[n], XmNtitle, resource.l_codearea_title);
        n++;
        XtSetArg(args[n], XmNmessageString, cs);
        n++;
        XtSetArg(args[n], XmNdialogStyle, XmDIALOG_MODELESS);
...
void ListSetLabelStr(i, str) int i;
String str;
{ SetLabelString(wgeStaT_list[i], str); }

void ListSelectItem(i) int i;
{
        XtVaSetValues(wgeStaT_list[i], XmNbackground,
                      (XtArgVal)resource.foreground, XmNforeground,
                      (XtArgVal)resource.background, NULL);
        xl.list_winID = XtWindow(wgeStaT_disp[i]);
}

void ListUnselectItem(i) int i;
{
        XtVaSetValues(wgeStaT_list[i], XmNbackground,
                      (XtArgVal)resource.background, XmNforeground,
                      (XtArgVal)resource.foreground, NULL);
        xl.list_winID = 0;
}

void ListSetGlyphImage(i) int i;
{
        int code;
        ...
        if (code == edg.code && xl.dispImage != NULL) {
                ...
                if (ptnGet(code, dl_glyph[i].ptn))
                        return;
                dl_glyph[i].dispImage = XCreateImage(
                    xl.display,
                ...
                XPutImage(xl.display, dl_glyph[i].disp_winID, xl.borderGC,
                          dl_glyph[i].dispImage, 0, 0, 0, 0, edg.width,
                          edg.height);
        }
}

/*ARGSUSED*/
static void EHStaT_disp(widget, i) Widget widget;
...
void CopySet() { XtSetSensitive(EditBTN.w[5], True); }

void UndoSet() { XtSetSensitive(EditBTN.w[9], True); }

void UndoUnset() { XtSetSensitive(EditBTN.w[9], False); }
...
void PopupEditPtn(owner) Widget owner;
{
        if (!editPtnW) {
...
static Widget CreateEditPtn(owner) Widget owner;
{
        int slimax;
        int i;
        ...
        baseForm = (Widget)CreateMenuBarAndFooterMessageForm(
            owner, "dtudcfonted", &MenuBTN, XtNumber(menu_btn), &pop,
            &FooterMessage);
        ...
        listFrame = CreateFrame(baseForm, "listFrame", XmSHADOW_IN, 2);
        ...
        listBase = CreateRowColumn(listFrame, "listFrame", L_VERTICAL, 4, 2, 2);

        for (i = 0; i < edlist.nlist; i++) {
                wgeStaT_form[i] = CreateForm(listBase, "listform");
                wgeStaT_list[i] = CreateLabel(wgeStaT_form[i], "label", "0000");
                wgeStaT_disp[i] =
                    CreateDrawingArea(wgeStaT_form[i], "image", edg.width,
                                      edg.height, EHStaT_disp, i);
                ...
                XtAddEventHandler(wgeStaT_list[i],
                                  ...
                                  EHeStaT_list, (XtPointer)i);
                XtAddEventHandler(wgeStaT_disp[i],
                                  ButtonPressMask | ButtonReleaseMask, False,
                                  EHeStaT_list, (XtPointer)i);

                if (i == 0) {
                        XtVaGetValues(wgeStaT_disp[i], XmNheight,
                                      &(edlist.elem_h), XmNwidth,
                                      &(edlist.elem_w), NULL);
                        ...
                        edlist.nlist =
                            (edpane.height + 2) / (int)(edlist.elem_h + 8);
                        edlist.nlist = (edlist.nlist < EDLIST_MAX)
                                           ? edlist.nlist
                                           : EDLIST_MAX;
                }
        }
        ...
        editFrame = CreateFrame(baseForm, "editFrame", XmSHADOW_IN, 2);
        ...
        figure_w = CreatePixButton(rc, "Draw", &DrawEX);
        AddTopAttachForm(figure_w, 0);
        AddLeftAttachForm(figure_w, 0);
        AddRightAttachForm(figure_w, 0);

        imageFrame = CreateCaptionFrame(rc, "Frame", "  ", XmSHADOW_IN, 1);
        ...
        imageForm = CreateForm(imageFrame, "separator");

        /* create the Editing pane */
        wgeBulB_edit = editPane =
            CreateDrawingArea(editFrame, "editPane", edpane.width,
                              edpane.height, EHeBulB_eExp, 0);

        XtAddEventHandler(
            editPane, ButtonPressMask | ButtonMotionMask | ButtonReleaseMask,
...
void SetCodeString(code) int code;
{
        char str[8];
...
static void _create_editptn_after() {
        ...
        XSetLineAttributes(xl.display, xl.dashGC, 0, LineDoubleDash, CapButt,
                           JoinMiter);
         ...
         */
        xl.dispImage = XCreateImage(
            xl.display, DefaultVisual(xl.display, DefaultScreen(xl.display)), 1,
            XYBitmap, 0, edg.ptn, edg.width, edg.height, 8, 0);
...
void UpdateMessage(str) String str;
{
