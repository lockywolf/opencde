...
char *argv[];
{
        ...
        if (Head.snf_file == NULL && xlfdname == '\0') {
                if (code_area & SYSTM) {
                        USAGE1("%s : The SNF output file name cannot be "
                               "omitted.\n",
                               argv[0]);
                } else {
        ...
        if (!(code_area & SYSTM)) {
                ...
                if (xlfdname) {
                        ...
                        switch (GetFileName(argv[0], Head.snf_file, style,
                                            code_no, snf_file)) {
                        case 0:
                                Head.snf_file = snf_file;
                                break;
        ...
        sleep(1);
        if ((stat(Head.out_file, &statbuf)) || (statbuf.st_size == 0)) {
                Unlink_Tmpfile(Head.out_file, argv[0]);
        ...
        exit(Make_NewFefFile(targ_file, Head.out_file, FONT_FILE_PARM,
                             (uid_t)statbuf.st_uid, (gid_t)statbuf.st_gid,
                             argv[0]));
}

static CnvGPFtoBDF(head) struct ptobhead *head;
{
        int rtn;
...
static void put_error_and_exit(ptob, er_no, prog_name) struct ptobhead *ptob;
int er_no;
char *prog_name;
{
        ErrMsgTable_AndExit(er_no, ptob->snf_file, ptob->out_file,
                            ptob->in_file, NULL, ptob->bdf_file, prog_name);
        return;
}
...
static void put_help(prog_name) char *prog_name;
{
        ...
        COMM_HELP_MSG;
        USAGE(
            "The character size may be obtained using the dtlsgpf command.\n");
        USAGE("\n");
}
