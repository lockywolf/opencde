...
char *argv[];
{
        ...
        if (Head.bdf_file == NULL) {
                if (Head.snf_file == NULL) {
                        USAGE1("%s: If the output is stdout, the user defined "
                               "font file name cannot be omitted.\n",
                               argv[0]);
                        exit(PARAM_ERROR * (-1));
                }
                Head.output = stdout;
                ...
                if ((Head.output = fopen(Head.out_file, "w")) == NULL) {
                        USAGE2("%s: The work file for the output cannot be "
                               "opened. \"%s\"\n",
...
static void put_error(head, er_no, prog_name) struct ptobhead *head;
int er_no;
char *prog_name;
{
        switch (er_no) {
        ...
        case BDF_WRITE:
                USAGE2(
                    "%s: It is not possible to write to the BDF file. \"%s\"\n",
                    prog_name, head->out_file);
                break;

        case BDF_INVAL:
                if (head->snf_file) {
                        USAGE2("%s:  The information from the font file cannot "
                               "be obtained.\"%s\"\n",
                               prog_name, head->snf_file);
                } else {
                        USAGE2("%s: The format of the BDF file is illegal. "
                               "\"%s\"\n",
        ...
        case GPF_OPEN:
                USAGE2("%s:  The character pattern file cannot be opened. "
                       "\"%s\"\n",
        ...
        case GPF_READ:
                USAGE2(
                    "%s:  The character pattern file cannot be read. \"%s\"\n",
                    prog_name, head->in_file);
                break;
...
{
        USAGE1(
            "Usage: %s [-bdf BDF_file_name] [-p chracter_pattern_file_name]\n",
            prog_name);
