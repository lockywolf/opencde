...
char *argv[];
{
        ...
        if (no_infile) {
                USAGE1("%s : The input file name following -p option cannot be "
                       "omitted.\n",
                       argv[0]);
                exit(1);
        }
        ...
        if (!(code_area & SYSTM)) {
                if (xlfdname) {
                        if (GetUdcFileName(argv[0], code_no, xlfdname,
                                           snf_file)) {
                                USAGE1("%s : Failed to get font file. "
                        ...
                        switch (GetFileName(argv[0], ReadSnf.in_file, style,
                                            code_no, snf_file)) {
                        case 0:
                                ReadSnf.in_file = snf_file;
                                break;
                ...
                if (GetUdcRegion(argv[0], code_no, ReadSnf.in_file, &num_gr,
                                 &gr)) {
        ...
        if (GetUdcFontName(ReadSnf.in_file, NULL, &xlfdname)) {
                USAGE1(
                    "%s : This font cannot get XLFD. Terminates abnormally.\n",
                    argv[0]);
                exit(1);
        }
        ...
        if ((targ_file = GetRealFileName(ReadSnf.in_file)) == NULL) {
                USAGE2("%s : It was not possible to refer to the substance of "
                       "the font file. \"%s\"\n",
        ...
        case SYSTM | CDSET1:
                ...
                break;
        default:
                COMM_SBFNT_SETUDCAREA(argv[0], ReadGpf, char_set, code_area,
                                      num_gr, gr);
        }
        ...
        if (init_all) {
                ...
                rtn = readSnf_with_init(&ReadGpf, &ReadSnf, &WriteSnf, init,
                                        buf, num_gr, gr);
        } else {
                /* We read whole characters from gpf file. */
        ...
        if ((rtn = mergePtn(argv[0], &ReadGpf, &ReadSnf, buf, code_area, modify,
                            argv[0], num_gr, gr, code_no))) {
                if (WriteSnf.out_file) {
                        Unlink_Tmpfile(WriteSnf.out_file, argv[0]);
                }
                put_error_and_exit(&ReadGpf, &ReadSnf, &WriteSnf, rtn, argv[0]);
        ...
        if (pclose(WriteSnf.output)) {
                Unlink_Tmpfile(WriteSnf.out_file, argv[0]);
                put_error_and_exit(&ReadGpf, &ReadSnf, &WriteSnf, PCLOSE_ERROR,
                                   argv[0]);
        }
#else
    ...
    !defined(__FreeBSD__)
        ...
        if (!(WIFEXITED(chld_stat) && !WEXITSTATUS(chld_stat))) {
                ...
                Unlink_Tmpfile(WriteSnf.out_file, argv[0]);
                put_error_and_exit(&ReadGpf, &ReadSnf, &WriteSnf, FATAL_ERROR,
                                   argv[0]);
        }
#endif
        ...
        if ((stat(WriteSnf.out_file, &statbuf)) || (statbuf.st_size == 0)) {
                Unlink_Tmpfile(WriteSnf.out_file, argv[0]);
                put_error_and_exit(&ReadGpf, &ReadSnf, &WriteSnf, BDF_WRITE,
                                   argv[0]);
        }
        ...
        exit(Make_NewFefFile(WriteSnf.snf_file, WriteSnf.out_file,
                             FONT_FILE_PARM, (uid_t)statbuf.st_uid,
                             (gid_t)statbuf.st_gid, argv[0]));
}

static rw_init(r_gpf, r_snf, w_snf, init_all) struct ptobhead *r_gpf;
...
int init_all;
{
        ...
        if (ChkPcfFontFile(w_snf->snf_file) == 0) {
                ...
                COMM_SNF_POPEN(permission, w_snf->snf_file, w_snf->out_file, ep,
                               buf, command);
        }
        ...
        case 0:
                if ((ofd = open(w_snf->out_file, O_WRONLY | O_CREAT, 0664)) <
                    0) {
                        return BDF_OPEN_OUT;
                }
                close(0);
...
static readSnf(r_gpf, r_snf, w_snf, buf) struct ptobhead *r_gpf;
struct btophead *r_snf;
struct ptobhead *w_snf;
char *buf;
{
...
static readSnf_with_init(r_gpf, r_snf, w_snf, init, buf, num_gr,
                         gr) struct ptobhead *r_gpf;
struct btophead *r_snf;
struct ptobhead *w_snf;
int init;
char *buf;
...
static readSnfHeader(r_gpf, r_snf, w_snf, buf) struct ptobhead *r_gpf;
struct btophead *r_snf;
struct ptobhead *w_snf;
char *buf;
{
        ...
        if ((r_snf->code = (int *)malloc(sizeof(int) * r_gpf->num_chars)) ==
            NULL) {
                return MALLOC_ERROR;
        }
        if ((r_snf->ptn = (char **)malloc(sizeof(char *) * r_gpf->num_chars)) ==
            NULL) {
                return MALLOC_ERROR;
        }
        return 0;
...
static readBdfHeaderAndPut(r_snf, w_snf, buf) struct btophead *r_snf;
struct ptobhead *w_snf;
char *buf;
{
        char *p;
        ...
        while (1) {
                ...
                if (!strncmp(p, SIZE, strlen(SIZE))) {
                        if ((sscanf(p, "SIZE %f%d", &(r_snf->bdf_point),
                                    &(r_snf->bdf_xdpi))) != 2) {
                                return BDF_INVAL;
                        }
...
#define DispCodePoint(command, char_set, gidx, dspcode, code_no, locale)       \
        {                                                                      \
                if (COMM_SBFNT_ISSBFNT((char_set))) {                          \
                        ...
                        if (fal_glyph_to_code((locale), (char_set),            \
                                              FALGETFALCODESET((code_no)),     \
                                              (gidx), &(dspcode))) {           \
                                USAGE3("%s : Failed to convert glyph index "   \
                                       "into code point.(0x%x charset: %s)\n", \
...
static mergePtn(com, r_gpf, r_snf, buf, code_area, modify, prog_name, num_gr,
                gr, code_no) char *com;
...
int code_no;
{
        ...
        for (i = 0; i < r_gpf->num_chars; i++) {
                ...
                if (COMM_SBFNT_ISSBFNT(char_set))
                        CONVGLYPHINDEX(code);
                if ((code < r_gpf->start_code) || (code > r_gpf->end_code) ||
                    (!IN_CODE(r_gpf->code_category, SHIFT_ON(code))) ||
                    ((r_gpf->code_category == ALL_CODE) &&
                     !(code_area & SYSTM) && IsInRegion(code, num_gr, gr))) {
                        DispCodePoint(com, char_set, code, dspcode, code_no,
                                      util_locale);
                        USAGE2("%s : The font of a specified code cannot be "
                               "added/changed \"0x%x\".\n",
                               prog_name, dspcode);
                ...
                for (j = 0; j < r_snf->num_chars; j++) {
                        if (r_snf->code[j] == code) {
                                if (!modify) {
                                        DispCodePoint(com, char_set, code,
                                                      dspcode, code_no,
                                                      util_locale);
                                        USAGE2("%s : The font has already been "
                                               "registered in a specified "
                                               "code. \"0x%x\"\n",
                                               prog_name, dspcode);
                                        fgets(buf, BUFSIZE, r_gpf->input);
                                        break;
                                ...
                                if ((rtn = InsertPtn(r_gpf, r_snf, buf, code,
                                                     j))) {
                                        return rtn;
                                }
                                break;
...
static ModifyPtn(r_gpf, r_snf, buf, ix) struct ptobhead *r_gpf;
struct btophead *r_snf;
char *buf;
int ix;
{
        ...
        if (r_gpf->zoomf) {
                if ((rtn =
                         GetGpfPtn(r_gpf, buf, r_gpf->ptn[0], mwidth, msize))) {
                        return rtn;
                }
                ...
                if ((rtn = GetGpfPtn(r_gpf, buf, r_snf->ptn[ix], mwidth,
                                     msize))) {
                        return rtn;
                }
        }
...
static InsertPtn(r_gpf, r_snf, buf, code, ix) struct ptobhead *r_gpf;
struct btophead *r_snf;
char *buf;
int code;
int ix;
...
static writeSnf(r_snf, w_snf) struct btophead *r_snf;
struct ptobhead *w_snf;
{
        w_snf->zoomf = 0;
...
static void put_error_and_exit(ptob_in, btop, ptob_out, er_no,
                               prog_name) struct ptobhead *ptob_in;
...
char *prog_name;
{
        ErrMsgTable_AndExit(er_no, ptob_in->in_file, ptob_out->out_file,
                            btop->in_file, NULL, ptob_in->bdf_file, prog_name);
        return;
}
...
static void put_help(prog_name) char *prog_name;
{
        ...
        USAGE("\t\t[-style style]");
        USAGE("\t\t[-init]\t(clear whole glyphs of gpf file that used for user "
              "defined characters)\n");
...
static int readBdfToMemory_with_init(head, init, buf, num_gr,
...
FalGlyphRegion *gr;
{
        ...
        while (1) {
                ...
                if ((code < head->start_code) || (code > head->end_code) ||
