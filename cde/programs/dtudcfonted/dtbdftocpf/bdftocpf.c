...
char *argv[];
{
                ...
                if (!strcmp(argv[i], "-p")) {
                        ...
                        if ((Head.p_width > MAX_FONT_WIDTH) ||
                            (Head.p_width < MIN_FONT_WIDTH)) {
                                USAGE2("%s: The specification of the character "
                                       "width is improper.\"%s\"\n",
                                       argv[0], argv[i + 1]);
                                exit(PARAM_ERROR * (-1));
                        }
        ...
        if (Head.in_file == NULL) {
                ...
                if (GetUdcFontName(NULL, Head.in_file, &xlfdname)) {
                        USAGE1("%s : This font cannot get XLFD. Terminates "
                               "abnormally.\n",
                               argv[0]);
                        exit(1);
                }
        ...
        if (Head.out_file == NULL) {
                ...
                if (stat(Head.out_file, &st) != 0) {
                        if ((Head.output = fopen(Head.out_file, "w")) == NULL) {
                                USAGE2("%s: The character pattern file cannot "
                                       "be opened. \"%s\"\n",
...
static CnvBDFtoGPF(head) struct btophead *head;
{
        char bdfbuf[BUFSIZE];
        int rtn;
...
static void put_error(head, er_no, prog_name) struct btophead *head;
int er_no;
char *prog_name;
{
        switch (er_no) {
        ...
        case GPF_OPEN:
                USAGE2(
                    "%s: The character pattern file cannot be opened. \"%s\"\n",
...
{
        USAGE1(
            "Usage: %s [-p character_pattern_file_name] [-bdf BDF_file_name]\n",
            prog_name);
