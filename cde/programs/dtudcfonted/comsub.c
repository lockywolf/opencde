...
#define FAL_ERROR_STR                                                          \
        resource                                                               \
            .falerrmsg[((fal_utyerrno & 0xff) > 25) ? 0                        \
                                                    : (fal_utyerrno & 0xff)]
...
int ptnSqToNo(num) int num; /* sequential number 	*/
{
...
int ptnNoToSq(ncode) int ncode; /* relative code 	*/
{
        int sq;
...
int codeToNo(code) int code; /* character code */
...
int noToCode(sno) int sno; /* relative code */
...
static int ptnOpen(n, maxc, width,
                   height) int n; /* initial number of charcters	*/
int maxc;
int width;  /* pattern width 		*/
int height; /* pattern height 		*/
{
...
int ptnAdd(code, ptn) int code; /* code to be add 	*/
...
int ptnGetInfo(n, width,
               height) int *n; /* the number of characters in editting */
int *width;                    /* pattern width 	*/
int *height;                   /* pattern height 	*/
{
        if (pm.flag == 0)
...
static void bitSetInfo(width, height) int width; /* pattern width 	*/
int height;                                      /* pattern height 	*/
{
        bitInfo.width = width;
...
void bitSet(ptn, cx, cy) char *ptn;
int cx;
int cy;
{
        if ((cx < 0) || (bitInfo.width <= cx) || (cy < 0) ||
            (bitInfo.height <= cy))
                return;
...
void bitReset(ptn, cx, cy) char *ptn;
int cx;
int cy;
{
        if ((cx < 0) || (bitInfo.width <= cx) || (cy < 0) ||
...
int bitRead(ptn, cx, cy) char *ptn;
int cx;
int cy;
{
        if ((cx < 0) || (bitInfo.width <= cx) || (cy < 0) ||
            (bitInfo.height <= cy))
                return (0);
        return (ptn[bitInfo.xbytesize * cy + cx / 8] & (char)(0x80 >> (cx % 8))
                    ? 1
                    : 0);
}
...
void bitPtnCopy(d_ptn,
                s_ptn) char *d_ptn; /* pointer of the destination file 	*/
...
int bitDrawLine(ptn, x1, y1, x2, y2,
                mode) char *ptn; /* pointer of the bit map file */
int x1;
int y1;
int x2;
int y2;
int mode; /* 0: erase 1: draw	*/
{
        ...
        for (i = 0; i <= (int)delta; i++) {
                mode == 0 ? bitReset(ptn, (int)x, (int)y)
                          : bitSet(ptn, (int)x, (int)y);
                x += dx;
                y += dy;
        }
        return (0);
...
int bitDrawCircle(ptn, x1, y1, x2, y2, mode) char *ptn;
int x1;
int y1;
int x2;
int y2;
...
int bitDrawRect(ptn, x, y, width, height, mode) char *ptn;
int x;
int y;
int width;
int height;
...
int bitDrawCls(ptn, x, y, width, height) char *ptn;
int x;
int y;
int width;
int height;
...
int bitDrawSet(ptn, x, y, width, height) char *ptn;
int x;
int y;
int width;
int height;
...
int bitDrawRev(ptn, x, y, width, height) char *ptn;
int x;
int y;
int width;
int height;
...
int bitDrawCpy(ptn, sx, sy, width, height, cut_flag) char *ptn;
int sx;
int sy;
int width;
int height;
...
int bitDrawPaste(ptn, dx, dy) char *ptn;
int dx;
int dy;
{
        int i, j;
        ...
        if ((dx < 0) || (dy < 0) || (cut_buffer == 0) || (cut_buffer_w == 0) ||
            (cut_buffer_h == 0))
                return (-1);

        width = (cut_buffer_w <= bitInfo.width - dx) ? cut_buffer_w
                                                     : bitInfo.width - dx;
        height = (cut_buffer_h <= bitInfo.height - dy) ? cut_buffer_h
                                                       : bitInfo.height - dy;
...
int bitDrawRoll(ptn, x, y, width, height) char *ptn;
int x;
int y;
int width;
int height;
{
        ...
        for (count = 0, yy = 0; yy < height; yy++) {
                for (xx = 0; xx < width; xx++) {
                        point[count++] == 0
                            ? bitReset(ptn, x + height - yy - 1, y + xx)
...
int bitDrawSymV(ptn, x, y, width, height) char *ptn;
int x;
int y;
int width;
int height;
...
int bitDrawSymH(ptn, x, y, width, height) char *ptn;
int x;
int y;
int width;
int height;
...
static char *char_set(char *str) {
        int i, count;
        char *p;
        for (i = strlen(str), p = &(str[i]), count = 0; i && count < 2;
             i--, p--) {
                if (*p == '-')
                        count++;
        }
 ...
 *		-2 : file is locked
 */
int readSNF(fdata, width, height, err) FalFontData **fdata;
int *width;
int *height;
char *err;
{
        ...
        *fdata = &(fulllist->list[0]);
        ...
        if (font_id == 0 && (fal_utyerrno & 0xff) == FAL_ERR_FNT_OPN) {
                mes = (char *)malloc(strlen(resource.mn_no_perm) +
                                     strlen(fal_err_file) + 10);
                if (mes == NULL) {
        ...
        for (i = 0, gr_p = gr; i < num_gr; i++, gr_p++) {
                ...
                if (!CodePoint) {
                        for (code = gr_p->start; code <= gr_p->end; code++) {
                                ...
                                ptn = (char *)FalReadFont(
                                    font_id, code, finfo.width, finfo.height);
...
static void bicopy(s1, s2, size) char *s1, *s2;
int size;
{
        register int i;
...
int copySNF(start, end, ptn, num, err) int start;
int end;
char ***ptn;
int *num;
char *err;
{
        ...
        if (!CodePoint) {
                for (i = start, count = 0, p = _ptn; i <= end; i++) {
                        ...
                        point =
                            (char *)FalReadFont(fid, i, pm.width, pm.height);
...
void freeSNF(addr, count) char **addr;
int count;
{
        char **p;
...
int writeSNF(restart, err) int restart; /* ON:continue OFF:initial	*/
int *err;                               /* errors : 			*/
{
        static int pfstdi[2];
        static FILE *fstdi;
        ...
        /* get command path */
        if (!(command =
                  (char *)get_cmd_path(getenv("PATH"), resource.l_ptog_cmd))) {
...
static void writePtn(ptn, width, height, fp) char *ptn;
int width;
int height;
FILE *fp;
{
