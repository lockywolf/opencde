...
void InitCpyPtn() { /* initialize font infomation */
}
...
void PopdownCpyPtn() { ForcePopdownDialog(cpyDialog); }
...
String CpySrcCodeTfValue() { return (GetTextFieldValue(&SrcTf)); }
...
String CpyDestCodeTfValue() { return (GetTextFieldValue(&DestTf)); }
...
void PopupCpyNotice(message) String message;
{
        char bmsg[512] = "";
        ...
        PopupNotice(cpyPtnW, bmsg, D_WARNING, &BTN, False,
                    resource.l_warning_title);
}
...
static void PopdownMngPtn() { PopdownDialog(mngPtnW); }
...
static ButtonItems add_btn[] = {
    BTNARGS((void (*)(struct _WidgetRec *, void *, void *))AddCB, NULL, NULL,
            True, True),
    BTNARGS((void (*)(struct _WidgetRec *, void *, void *))DelCB, NULL, NULL,
            True, False),
    BTNARGS((void (*)(struct _WidgetRec *, void *, void *))PopdownMngPtn, NULL,
            NULL, True, False),
};
...
void MngPtnCB() {
        if (!mngPtnW) {
                ...
                rowcol = CreateDialogAndButtons(toplevel, resource.l_add_title,

                                                NULL, &AddBTN,
                                                XtNumber(add_btn), &mngPtnW);

                CreateTextField(rowcol, "wgkTxtF", resource.l_manage_code,
                                &MngCodeTf, 4);
        }
        PopupDialog(mngPtnW);
}
...
String MngCodeTfValue() { return (GetTextFieldValue(&MngCodeTf)); }
...
void PopupDelNotice(owner) Widget owner;
{
        int i;
...
static void dstrypaneDelNotice() {
        int i;
...
static void delScProc(value) int value;
{
        int i;
...
static Widget CreateDelNotice(owner) Widget owner;
{
        ...
        static NoticeButton buttons[] = {
            NBTNARGS((void (*)(struct _WidgetRec *, void *, void *))DelOkCB,
        ...
        if (c == 1) {
                sprintf(buf, "%s(%d %s)", resource.mg_start_del_s, c,
                        resource.mg_start_del_cs);
        } else {
                sprintf(buf, "%s(%d %s)", resource.mg_start_del_m, c,
                        resource.mg_start_del_cm);
        }
        SetItemLabel(&BTN, 0, resource.l_ok);
        SetItemLabel(&BTN, 1, resource.l_cancel);
        ...
        if (dn.disp_num < (AbsSqToRel(dn.sq_start, dn.sq_end) + 1)) {
                ...
                listBase = CreateScrollBase(form, "listBase", min, max, val,
                                            dn.disp_num, delScProc);
        } else {
                listBase =
        ...
        for (i = 0; i < dn.disp_num; i++) {
                ...
                /* create widgets for the image of the codes to be deleted in
                 * the list */
                dnBulB_disp[i] =
                    CreateDrawingArea(itemform[i], "image", edg.width,
                                      edg.height, EHdnBulB_disp, i);
                AddLeftAttachWidget(dnBulB_disp[i], dnStaT_code[i], 0);

                /* */
                dl[i].dispImage = XCreateImage(
                    xl.display,
                    DefaultVisual(xl.display, DefaultScreen(xl.display)), 1,
                    XYBitmap, 0, dl[i].ptn, dn.ptn_w, dn.ptn_h, 8, 0);
...
static void CBdnOblB_del() {
        ...
        if ((sq_now != -1) && ((ncode = ptnSqToNo(sq_now)) != -1)) {
                 ...
                 * to input.
                 * It can realize to put 0 to the first parameter of
                 * chgEdCode().
                 */
                code = 0;
        }
...
static void drawDelPtn(i) int i; /* window number */
{
...
static void drawDelCode(i) int i; /* window number */
{
        char str[20];

        if (RelToAbsSq(dn.sq_top, i) <= dn.sq_end) {
                sprintf(str, "%4x ",
                        noToCode(ptnSqToNo(RelToAbsSq(dn.sq_top, i))));
...
/*ARGSUSED*/
static void EHdnBulB_disp(widget, i) Widget widget;
...
static void DispErrorMessage(owner, title, msg) Widget owner;
String title;
String msg;
{
        static NoticeButton buttons[] = {
...
void DispMngErrorMessage(msg) String msg;
...
void DispCpyErrorMessage(msg) String msg;
