...
char *argv[];
{
        ...
        for (i = 1; i < argc; i++) {
                if (!strcmp(argv[i], "-g")) {
                        ...
                        if ((i < argc - 1) && (*argv[i + 1] != '-')) {
                                ...
                                USAGE1("%s : The output file name following -p "
        ...
        if (Head.in_file == NULL && xlfdname == '\0') {
                if (code_area & SYSTM) {
                        ...
                        USAGE1("%s : The specification of the character size "
                               "cannot be omitted.\n",
                               argv[0]);
                }
                exit(1);
        ...
        if (!(code_area & SYSTM)) {
                ...
                if (xlfdname) {
                        ...
                        switch (GetFileName(argv[0], Head.in_file, style,
                                            code_no, snf_file)) {
                        case 0:
                                Head.in_file = snf_file;
                                break;
                ...
                if (GetUdcRegion(argv[0], code_no, Head.in_file, &num_gr,
                                 &gr)) {
                ...
                if ((gr = (FalGlyphRegion *)malloc(sizeof(FalGlyphRegion) *
                                                   num_gr)) == NULL) {
                        USAGE1("%s : malloc error occured. Terminates "
                               "abnormally.\n",
                               argv[0]);
                        exit(1);
                }
        ...
        case SYSTM | CDSET1:
                ...
                break;
        default:
                COMM_SBFNT_SETUDCAREA(argv[0], Head, char_set, code_area,
                                      num_gr, gr);
                break;
        }

        if (start_code && end_code) {
                Head.start_code = check_code(argv[0], start_code, START_CODE,
                                             char_set, num_gr, gr);
                if (Head.start_code < 0) {
                        exit(1);
                }
                Head.end_code = check_code(argv[0], end_code, END_CODE,
                                           char_set, num_gr, gr);
                if (Head.end_code < 0) {
                        exit(1);
                }
        ...
        if (Head.start_code > Head.end_code) {
                USAGE1("%s : The specification of extract start/end code is "
                       "improper.\n",
                       argv[0]);
                exit(1);
        }
        ...
        if (Head.out_file == NULL) {
                ...
                if (stat(Head.out_file, &st) != 0) {
                        if ((Head.output = fopen(Head.out_file, "w")) == NULL) {
                                put_error_and_exit(&Head, GPF_OPEN_OUT,
                                                   argv[0]);
                        }
                } else {
                        Head.text_file = Head.out_file;
                        if ((Head.out_file = GetTmpPath(Head.out_file)) ==
                            NULL) {
...
static int CnvBDFtoGPF(head, num_gr, gr, code_area) struct btophead *head;
int num_gr;
FalGlyphRegion *gr;
int code_area;
{
        ...
        if ((head->code = (int *)malloc(sizeof(int) * head->num_chars)) ==
            NULL) {
                return (MALLOC_ERROR);
        }

        if ((head->ptn = (char **)malloc(sizeof(char *) * head->num_chars)) ==
            NULL) {
                return (MALLOC_ERROR);
        }
...
static void put_error_and_exit(btop, er_no, prog_name) struct btophead *btop;
int er_no;
char *prog_name;
{
        ErrMsgTable_AndExit(er_no, btop->in_file, NULL, NULL, btop->out_file,
                            NULL, prog_name);
        return;
}
...
static void put_help(prog_name) char *prog_name;
{
        ...
        COMM_HELP_MSG;
        USAGE1("%s can extract glyphs in cpf file format in the following code "
               "area.\n",
               prog_name);
...
static void Usage(prog_name) char *prog_name;
{
        put_help(prog_name);
        exit(1);
}
...
static int readBdfToMemory(head, buf, num_gr, gr,
                           code_area) struct btophead *head;
char *buf;
int num_gr;
FalGlyphRegion *gr;
int code_area;
