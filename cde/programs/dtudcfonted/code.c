...
static void DrawCode(w, low_code) Widget w;
int low_code;
{
        int hi_code;
        char *f;
        ...
        if (open_font) {
                f = (char *)FalReadFont(fid, hi_code | low_code, font_w,
                                        font_h);
                if (f == (char *)FAL_ERROR)
                        return;
                if (EXISTS_FLAG == 0) {
                ...
                XPutImage(display, XtWindow(toggle[low_code]), gc, &image, 0, 0,
                          6, 6, font_w, font_h);
        } else {
...
static void SetPixmap(start_code) {
        ...
        if (open_font) {
                for (low_code = 0; low_code < NUM; low_code++) {
                        ...
                        XPutImage(display, XtWindow(toggle[low_code]), gc,
                                  &image, 0, 0, 6, 6, font_w, font_h);
                }
        } else {
                for (low_code = 0; low_code < NUM; low_code++) {
                        if (ptnGet(hi_code | low_code, ptn) == 0) {
...
static void Code1Call(w, start_code, data) Widget w;
int start_code;
...
static void Code2Call(w, code2, data) Widget w;
int code2;
...
static void PrevPage(w, scroll) Widget w, scroll;
{
        int *list;
        int num;
...
static void NextPage(w, scroll) Widget w, scroll;
{
        int *list;
        int num, item;
...
static void Cancel() { XtUnmapWidget(XtParent(code_w)); }
...
static void CreateItem(item, item_count, start_code) XmString *item;
int *item_count;
int *start_code;
{
        int count;
...
static Widget CreateCodeWindow(w) Widget w;
{
        ...
        XtSetArg(args[n], XmNmwmFunctions,
                 MWM_FUNC_ALL | MWM_FUNC_CLOSE | MWM_FUNC_RESIZE);
        n++;
        ...
        frame =
            XtVaCreateManagedWidget("frame", xmFrameWidgetClass, base1, NULL);
        ...
        n++;
        XtSetArg(args[n], XmNitemCount, item_count);
        n++;
        ...
        base3 = XtVaCreateManagedWidget(
            "base3", xmRowColumnWidgetClass, base2, XmNorientation,
            XmHORIZONTAL, XmNpacking, XmPACK_COLUMN, XmNradioAlwaysOne, True,
            XmNradioBehavior, True, XmNmarginHeight, 0, XmNmarginWidth, 0,
            XmNspacing, 0, XmNnumColumns, 9, NULL);
        ...
        for (i = 0; i < NUM; i++) {
                if ((i % 16) == 0) {
                        ...
                        XtVaCreateManagedWidget(add, xmLabelWidgetClass, base3,
                                                NULL);
                }
                toggle[i] = XtVaCreateWidget("toggle", xmPushButtonWidgetClass,
                                             base3, XmNwidth, font_w, XmNheight,
                                             font_h, XmNlabelType, XmPIXMAP,
        ...
        base4 =
            XtVaCreateManagedWidget("base4", xmFormWidgetClass, base1, NULL);

        xcs = XmStringCreateLocalized(resource.l_code);
        label = XtVaCreateManagedWidget(
            "code", xmLabelWidgetClass, base4, XmNlabelString, xcs,
            XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 30, NULL);

        text = XtVaCreateManagedWidget(
            "text", xmTextWidgetClass, base4, XmNeditable, False,
            XmNcursorPositionVisible, False, XmNcolumns, 4, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, label, NULL);
        ...
        push1 = XtVaCreateManagedWidget(
            "PreviousPage", xmPushButtonWidgetClass, form, XmNleftAttachment,
            XmATTACH_POSITION, XmNlabelString, xcs, XmNleftPosition, 10, NULL);
        XtAddCallback(push1, XmNactivateCallback, (XtCallbackProc)PrevPage,
                      (XtPointer)scroll);
        XmStringFree(xcs);

        xcs = XmStringCreateLocalized(resource.next);
        push2 = XtVaCreateManagedWidget(
            "NextPage", xmPushButtonWidgetClass, form, XmNleftAttachment,
            XmATTACH_POSITION, XmNlabelString, xcs, XmNleftPosition, 35, NULL);
        XtAddCallback(push2, XmNactivateCallback, (XtCallbackProc)NextPage,
                      (XtPointer)scroll);
        XmStringFree(xcs);

        xcs = XmStringCreateLocalized(resource.apply);
        push3 = XtVaCreateManagedWidget(
            "Apply", xmPushButtonWidgetClass, form, XmNleftAttachment,
            XmATTACH_POSITION, XmNlabelString, xcs, XmNleftPosition, 60, NULL);
        XtAddCallback(push3, XmNactivateCallback, (XtCallbackProc)Apply, NULL);
        XmStringFree(xcs);

        xcs = XmStringCreateLocalized(resource.l_cancel);
        push4 = XtVaCreateManagedWidget(
            "Cancel", xmPushButtonWidgetClass, form, XmNleftAttachment,
            XmATTACH_POSITION, XmNlabelString, xcs, XmNleftPosition, 80, NULL);
...
void CodeWindow(widget, font_name, load_font) Widget widget;
char *font_name;
Boolean load_font;
{
