...
/*
cc -g -o Main Main.c -I/usr/dt/include -I/usr/openwin/include
-R/export2/build/SunOS/lib1/DtSvc:/usr/dt/lib:/usr/openwin/lib
-L/export2/build/SunOS/lib1/DtSvc -L/usr/dt/lib -L/usr/openwin/lib -lDtSvc -lXt
 */
#include <locale.h>
#include <Tt/tt_c.h>
...
int main(int argc, char **argv) {
        ...
        while (!init_flag && more) {
                ...
                if (msg != 0) {
                        ...
                        if (strcmp(op, RebuildMsg) != 0) {
                                ...
                                /*
                                                                m =
                                   tt_message_create(); tt_message_scope_set(m,
                                   TT_SESSION); tt_message_class_set(m,
                                   TT_NOTICE); tt_message_op_set(m,
                                   "DtTypes_Reloaded");
        ...
        if (init_flag) {
                if (!_DtDtsMMInit(1)) {
                         ...
                         */
                        _DtSimpleError(
                            ApplicationClass, DtFatalError, NULL,
...
static void DbCacheCreateErrorCallback(Widget w, XtPointer userData,
                                       XtPointer callData) {
        XtUnmanageChild(w);
        XFlush(XtDisplay(w));
        XmUpdateDisplay(w);
...
static void DbCacheCreateError(Widget toplevel) {
        ...
        messagein =
            "Couldn't recreate the Desktop Actions/DataTypes Database.\n\
Check disk space and/or permissions.";
        ...
        XtSetArg(args[0], XmNx, (WidthOfScreen(XtScreen(err)) - dialogWd) / 2);
        XtSetArg(args[1], XmNy, (HeightOfScreen(XtScreen(err)) - dialogHt) / 2);
...
static void DieFromToolTalkError(char *errfmt, Tt_status status) {
        char *statmsg;
