...
#include "WmIPC.h" /* must be after DtP.h */
    ...
    **/
    ...
    static Pixmap
    WmXmGetPixmap2(Screen * screen, char *pchName, Pixel fg, Pixel bg);
...
void ProcessBackdropResources(WmWorkspaceData *pWS, unsigned long callFlags) {
        ...
        if (!no_backdrop_string &&
            (no_backdrop_string = XtNewString(DTWM_REQP_BACKDROP_NONE))) {
                ...
                xa_NO_BACKDROP =
                    XmInternAtom(DISPLAY, no_backdrop_string, False);
        ...
        if (!no_backdrop_string) {
                Warning(((char *)GETMESSAGE(
                    6, 4, "Insufficient memory for backdrop window.")));
                return;
        }
        ...
        if (pWS->backdrop.image) {
                ...
                if (chlen == 1 && *pch++ == '@') {
                        ...
                        if (chlen >= 1) {
                                ...
                                unsigned char *pchD =
                                    (unsigned char *)pWS->backdrop.image;
                ...
                if ((pLine = pchImageName =
                         (unsigned char *)strdup(pWS->backdrop.image)) &&
                    (pch = GetString(&pLine))) {
                        ...
                        if (pch && !bNone) {
                                ...
                                if ((callFlags & CHANGE_BACKDROP)) {
                                         ...
                                         * imagePixmap actually contains a depth
                                         * 1 pixmap. Convert it into a pixmap of
                                         * the proper depth.
                                         */
                                    ...
                                    (pWS->backdrop.window)) {
                                        ...
                                        if ((wmGD.keyboardFocusPolicy ==
                                            ...
                                            (wmGD.colormapFocusPolicy ==
                                             CMAP_FOCUS_POINTER)) {
                                                /*
                                                 * Listen for enter/levae events
                                                 * if we have a pointer tracking
                                                 * focus policy
                                                 */
                                        ...
                                        pWS->backdrop.window = XCreateWindow(
                                            DISPLAY, pWS->pSD->rootWindow, 0, 0,
                                ...
                                if (pch &&
                                    ...
                                    (pWS->backdrop.window)) {
                                        /*
                                         * Succeeded in setting up a bitmap
                                         * backdrop.
                                         */
                                        pWS->backdrop.flags |= BACKDROP_BITMAP;

                                        pWS->backdrop.nameAtom = XmInternAtom(
                                            DISPLAY, pWS->backdrop.image,
                                            False);
                                } else {
 ...
 *
 *************************************<->***********************************/
static Pixmap WmXmGetPixmap2(Screen *screen, char *pchName, Pixel fg,
                             Pixel bg) {
        Pixmap pixReturn;
        char *pch;
