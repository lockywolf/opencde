...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmCDecor.c /main/7 1996/06/20 09:38:16 rswiston $"
#endif
#endif
/*
...
void FrameExposureProc(ClientData *pcd) {
        ...
        if (DECOUPLE_TITLE_APPEARANCE(pcd) && (pcd->decor & MWM_DECOR_TITLE)) {
                if (pcd == wmGD.keyboardFocus) {
                        ...
                        botGC =
                            CLIENT_TITLE_APPEARANCE(pcd).activeBottomShadowGC;
                } else {
                        topGC =
                            CLIENT_TITLE_APPEARANCE(pcd).inactiveTopShadowGC;
                        botGC =
                            CLIENT_TITLE_APPEARANCE(pcd).inactiveBottomShadowGC;
                }

                if (pcd->pclientTitleTopShadows) {
                        XFillRectangles(DISPLAY, pcd->clientTitleWin, topGC,
                                        pcd->pclientTitleTopShadows->prect,
                                        pcd->pclientTitleTopShadows->used);
                }

                if (pcd->pclientTitleBottomShadows) {
                        XFillRectangles(DISPLAY, pcd->clientTitleWin, botGC,
...
void BaseWinExposureProc(ClientData *pcd) {
        ...
        if (pcd->matteWidth > 0) {
                ...
                if (pcd->pclientMatteTopShadows) {
                        XFillRectangles(DISPLAY, pcd->clientBaseWin,
...
Boolean ConstructFrame(ClientData *pcd) {
        ...
        window_attribs.event_mask =
            (ButtonPressMask | ButtonReleaseMask | SELECT_BUTTON_MOTION_MASK |
             DMANIP_BUTTON_MOTION_MASK | ExposureMask);
        ...
        pcd->clientFrameWin = XCreateWindow(
            DISPLAY, RootWindow(DISPLAY, SCREEN_FOR_CLIENT(pcd)), frmX, frmY,
            pcd->frameInfo.width, pcd->frameInfo.height, 0, CopyFromParent,
            InputOutput, CopyFromParent, attr_mask, &window_attribs);
        ...
        if (decoration & MWM_DECOR_TITLE) {
                ...
                if (DECOUPLE_TITLE_APPEARANCE(pcd)) {
                        /* title bar has a different appearance than rest of
                         * frame */
                        wclass = InputOutput;
                        ...
                        /*
                         * Use background pixmap if one is specified, otherwise
                ...
                pcd->clientTitleWin = XCreateWindow(
                    DISPLAY, pcd->clientFrameWin,
                    (int)pcd->frameInfo.upperBorderWidth,
                    (int)pcd->frameInfo.upperBorderWidth,
                    pcd->frameInfo.width - 2 * pcd->frameInfo.upperBorderWidth,
                    pcd->frameInfo.titleBarHeight, 0, CopyFromParent, wclass,
                    CopyFromParent, attr_mask, &window_attribs);
        }
        ...
        pcd->clientBaseWin = XCreateWindow(
            DISPLAY, pcd->clientFrameWin, BaseWindowX(pcd), BaseWindowY(pcd),
            BaseWindowWidth(pcd), BaseWindowHeight(pcd), 0, CopyFromParent,
            InputOutput, CopyFromParent, attr_mask, &window_attribs);
...
void GenerateFrameDisplayLists(ClientData *pcd) {
        ...
        if (decoration & MWM_DECOR_RESIZEH) {
                ...
                StretcherCorner(
                    pcd->pclientTopShadows, /* NW */
                    pcd->pclientBottomShadows, x, y, STRETCH_NORTH_WEST,
                ...
                if ((int)width > 0)
                        BevelRectangle(pcd->pclientTopShadows, /* N */
                                       pcd->pclientBottomShadows, x, y, width,
                                       height - inset, 2, 1,
                ...
                StretcherCorner(
                    pcd->pclientTopShadows, pcd->pclientBottomShadows, x, y,
                    STRETCH_NORTH_EAST, pcd->frameInfo.upperBorderWidth - inset,
                    width, height);

                GetFramePartInfo(pcd, FRAME_RESIZE_E, &x, &y, &width, &height);
                if ((int)height > 0)
                        BevelRectangle(pcd->pclientTopShadows, /* E */
                                       pcd->pclientBottomShadows, x + diffBevel,
                                       y, width - diffBevel, height, 1, 2, 1,
                                       ((wmGD.frameStyle == WmSLAB) ? 0 : 1));

                GetFramePartInfo(pcd, FRAME_RESIZE_SE, &x, &y, &width, &height);
                StretcherCorner(
                    pcd->pclientTopShadows, /* SE */
                    pcd->pclientBottomShadows, x, y, STRETCH_SOUTH_EAST,
                    pcd->frameInfo.upperBorderWidth - inset, width, height);

                GetFramePartInfo(pcd, FRAME_RESIZE_S, &x, &y, &width, &height);
                if ((int)width > 0)
                        BevelRectangle(pcd->pclientTopShadows, /* S */
                                       pcd->pclientBottomShadows, x,
                                       y + diffBevel, width, height - diffBevel,
                                       ((wmGD.frameStyle == WmSLAB) ? 0 : 1), 1,
                                       2, 1);

                GetFramePartInfo(pcd, FRAME_RESIZE_SW, &x, &y, &width, &height);
                StretcherCorner(
                    pcd->pclientTopShadows, /* SW */
                    pcd->pclientBottomShadows, x, y, STRETCH_SOUTH_WEST,
                    pcd->frameInfo.upperBorderWidth - inset, width, height);

                GetFramePartInfo(pcd, FRAME_RESIZE_W, &x, &y, &width, &height);
                if ((int)height > 0)
                        BevelRectangle(pcd->pclientTopShadows, /* W */
                                       pcd->pclientBottomShadows, x, y,
                                       width - diffBevel, height, 1,
                                       ((wmGD.frameStyle == WmSLAB) ? 0 : 1), 1,
                                       2);

                if (diffBevel) {
                        /*
                         * Draw second inside bevel level. This goes just around
                         * the client area under the title bar.
                         */
                        BevelRectangle(
                            pcd->pclientBottomShadows, /* inside */
...
#ifdef WSM
                BevelRectangle(
                    pcd->pclientTopShadows, /* outside */
                    pcd->pclientBottomShadows, 0, 0, pcd->frameInfo.width,
                    pcd->frameInfo.height, FRAME_EXTERNAL_SHADOW_WIDTH,
                    FRAME_EXTERNAL_SHADOW_WIDTH, FRAME_EXTERNAL_SHADOW_WIDTH,
                    FRAME_EXTERNAL_SHADOW_WIDTH);
#else  /* WSM */
                BevelRectangle(pcd->pclientTopShadows, /* outside */
                               pcd->pclientBottomShadows, 0, 0,
                               pcd->frameInfo.width, pcd->frameInfo.height, 2,
                               2, 2, 2);
#endif /* WSM */

                if ((pcd->internalBevel > 1) && !matte_width &&
                    (decoration & MWM_DECOR_TITLE)) {
                        /*
                         * Need to do special beveling around the inside of the
                         * client area separately from around title area.
                         */
                        GetFramePartInfo(pcd, FRAME_TITLE, &x, &y, &width,
                                         &height);
                        inset = 1 + (pcd->frameInfo.lowerBorderWidth -
                                     pcd->frameInfo.upperBorderWidth);
                        BevelRectangle(
                            pcd->pclientBottomShadows, pcd->pclientTopShadows,
                        ...
                        BevelRectangle(pcd->pclientBottomShadows, /* inside */
                                       pcd->pclientTopShadows,
                                       (int)(pcd->frameInfo.lowerBorderWidth -
                                             insideBevel),
                                       (int)(pcd->frameInfo.lowerBorderWidth -
                                             insideBevel),
                                       pcd->frameInfo.width -
        ...
        if (decoration & (MWM_DECOR_RESIZEH | MWM_DECOR_BORDER)) {
                ...
                if (wmGD.frameStyle == WmSLAB) {
                        ...
                        sTitleBevel = (matte_width > 0) ? insideBevel
                                                        : EXTERNAL_BEVEL(pcd);
                }
                eTitleBevel =
                    (decoration & (MWM_DECOR_MINIMIZE | MWM_DECOR_MAXIMIZE))
                        ? JOIN_BEVEL(pcd)
                        : EXTERNAL_BEVEL(pcd);
        ...
        if (decoration & MWM_DECOR_TITLE) {
                ...
                if (pcd->decorFlags & TITLE_DEPRESSED) {
                        ...
                        GetDepressInfo(pcd, FRAME_TITLE, &jX, &jY, &jW, &jH,
                                       &inWidth);
                        BevelDepressedRectangle(
                            prlTop, prlBot, x - xAdj, y - yAdj, width, height,
                            nTitleBevel, eTitleBevel, sTitleBevel, wTitleBevel,
                            inWidth);
                } else {
                        /* show normal title gadget */
                        BevelRectangle(prlTop, prlBot, x - xAdj, y - yAdj,
                                       width, height, nTitleBevel, eTitleBevel,
        ...
        if (decoration & MWM_DECOR_MENU) {
                ...
                if (pcd->decorFlags & SYSTEM_DEPRESSED) {
                        ...
                        BevelDepressedRectangle(
                            prlTop, prlBot, x - xAdj, y - yAdj, width, height,
                            nTitleBevel, wTitleBevel, sTitleBevel, nTitleBevel,
                            inWidth);
                } else {
                        /* show normal system gadget */
                        BevelRectangle(prlTop, prlBot, x - xAdj, y - yAdj,
                                       width, height, nTitleBevel, wTitleBevel,
                                       sTitleBevel, nTitleBevel);
                }

                /* system icon */
                BevelSystemButton(prlTop, prlBot, x - xAdj, y - yAdj, width,
                                  height);
        }
        ...
        if (decoration & MWM_DECOR_MINIMIZE) {
                ...
                if (pcd->decorFlags & MINIMIZE_DEPRESSED) {
                        ...
                        BevelDepressedRectangle(
                            prlTop, prlBot, x - xAdj, y - yAdj, width, height,
                            nTitleBevel, meTitleBevel, sTitleBevel, eTitleBevel,
                            inWidth);
                } else {
                        /* show normal minimize gadget */
                        BevelRectangle(prlTop, prlBot, x - xAdj, y - yAdj,
                                       width, height, nTitleBevel, meTitleBevel,
                                       sTitleBevel, eTitleBevel);
                }

                BevelMinimizeButton(prlTop, /* minimize icon */
                                    prlBot, x - xAdj, y - yAdj, height);
        }
        ...
        if (decoration & MWM_DECOR_MAXIMIZE) {
                ...
                if (pcd->decorFlags & MAXIMIZE_DEPRESSED) {
                        ...
                        BevelDepressedRectangle(
                            prlTop, prlBot, x - xAdj, y - yAdj, width, height,
                            nTitleBevel, nTitleBevel, sTitleBevel, eTitleBevel,
                            inWidth);
                } else {
                        /* show normal maximize gadget */
                        BevelRectangle(prlTop, prlBot, x - xAdj, y - yAdj,
                                       width, height, nTitleBevel, nTitleBevel,
                                       sTitleBevel, eTitleBevel);
                }

                /* maximize icon - in or out depending on client state */
                if (pcd->maxConfig) {
                        BevelMaximizeButton(prlBot, prlTop, x - xAdj, y - yAdj,
                                            height);
                } else {
                        BevelMaximizeButton(prlTop, prlBot, x - xAdj, y - yAdj,
                                            height);
                }
        }
        ...
        if (matte_width > 0) {
                ...
                BevelRectangle(pcd->pclientMatteTopShadows,
                               pcd->pclientMatteBottomShadows, 0, 0, mWidth,
                               mHeight, tMatteBevel, exMatteBevel, exMatteBevel,
                               exMatteBevel);

                /* reversed beveling on inside rectange ! */

                BevelRectangle(
                    pcd->pclientMatteBottomShadows, pcd->pclientMatteTopShadows,
...
void AdoptClient(ClientData *pcd) {
        ...
        if (wmGD.hasShape) {
                ...
                XShapeQueryExtents(DISPLAY, pcd->client, &boundingShaped, &xws,
                                   &yws, &wws, &hws, &clipShaped, &xbs, &ybs,
                                   &wbs, &hbs);
...
void DrawWindowTitle(ClientData *pcd, Boolean eraseFirst) {
        ...
        if (eraseFirst) {
                XClearArea(DISPLAY, win, textBox.x, textBox.y,
                           ...
                           (unsigned int)textBox.height, FALSE);
        }
...
void CreateStretcherWindows(ClientData *pcd) {
        ...
        for (iWin = 0; iWin < STRETCH_COUNT; iWin++) {
                ...
                case STRETCH_NORTH_WEST:
                        GetFramePartInfo(pcd, FRAME_RESIZE_NW, &x, &y, &width,
                                         &height);
                        break;

                case STRETCH_NORTH:
                        GetFramePartInfo(pcd, FRAME_RESIZE_N, &x, &y, &width,
                                         &height);
                        break;

                case STRETCH_NORTH_EAST:
                        GetFramePartInfo(pcd, FRAME_RESIZE_NE, &x, &y, &width,
                                         &height);
                        break;

                case STRETCH_EAST:
                        GetFramePartInfo(pcd, FRAME_RESIZE_E, &x, &y, &width,
                                         &height);
                        break;

                case STRETCH_SOUTH_EAST:
                        GetFramePartInfo(pcd, FRAME_RESIZE_SE, &x, &y, &width,
                                         &height);
                        break;

                case STRETCH_SOUTH:
                        GetFramePartInfo(pcd, FRAME_RESIZE_S, &x, &y, &width,
                                         &height);
                        break;

                case STRETCH_SOUTH_WEST:
                        GetFramePartInfo(pcd, FRAME_RESIZE_SW, &x, &y, &width,
                                         &height);
                        break;

                case STRETCH_WEST:
                        GetFramePartInfo(pcd, FRAME_RESIZE_W, &x, &y, &width,
                                         &height);
                        break;
                }
                ...
                pcd->clientStretchWin[iWin] =
                    XCreateWindow(DISPLAY, pcd->clientFrameWin, x, y, width,
                                  height, 0, CopyFromParent, InputOnly,
                                  CopyFromParent, attr_mask, &win_attribs);
        }
...
void CountFrameRectangles(WmScreenData *pSD) {
        ...
        for (i = FRAME_SYSTEM; i <= FRAME_MAXIMIZE; i++) {
                pSD->Num_Title_Ts_Elements +=
                    ((Bevels[i].top.external * pSD->externalBevel) +
                     (Bevels[i].top.internal * MAX_INTERNAL_BEVEL) +
                     (Bevels[i].top.join * pSD->joinBevel));

                pSD->Num_Title_Bs_Elements +=
                    ((Bevels[i].bottom.external * pSD->externalBevel) +
        ...
        for (i = FRAME_RESIZE_NW; i <= FRAME_RESIZE_W; i++) {
                pSD->Num_Resize_Ts_Elements +=
                    ((Bevels[i].top.external * pSD->externalBevel) +
                     (Bevels[i].top.internal * MAX_INTERNAL_BEVEL) +
                     (Bevels[i].top.join * pSD->joinBevel));

                pSD->Num_Resize_Bs_Elements +=
                    ((Bevels[i].bottom.external * pSD->externalBevel) +
                     (Bevels[i].bottom.internal * MAX_INTERNAL_BEVEL) +
                     (Bevels[i].bottom.join * pSD->joinBevel));
        }
...
Boolean AllocateFrameDisplayLists(ClientData *pcd) {
         ...
         */
        if (DECOUPLE_TITLE_APPEARANCE(pcd) && (pcd->decor & MWM_DECOR_TITLE)) {
                if (((pcd->pclientTitleTopShadows = AllocateRList(
                          (unsigned)NUM_TITLE_TS_ELEMENTS(pcd))) == NULL) ||
                    ((pcd->pclientTitleBottomShadows = AllocateRList(
                          (unsigned)NUM_TITLE_BS_ELEMENTS(pcd))) == NULL)) {
                        /* out of memory! */
                ...
                frame_top_count =
                    NUM_RESIZE_TS_ELEMENTS(pcd) + NUM_TITLE_TS_ELEMENTS(pcd);
                frame_bottom_count =
                    NUM_RESIZE_BS_ELEMENTS(pcd) + NUM_RESIZE_BS_ELEMENTS(pcd);
        }
        ...
        if ((pcd->pclientTopShadows == NULL) &&
            ((pcd->pclientTopShadows =
                ...
                /* out of memory! */
                Warning(((char *)GETMESSAGE(
                    8, 2, "Insufficient memory for client window framing")));
                return (FALSE);
        }
        ...
        if ((pcd->pclientBottomShadows == NULL) &&
            ((pcd->pclientBottomShadows =
                ...
                /* out of memory! */
                Warning(((char *)GETMESSAGE(
                    8, 3, "Insufficient memory for client window framing")));
                return (FALSE);
        }
         ...
         */
        if ((pcd->matteWidth) && (pcd->pclientMatteTopShadows == NULL) &&
            ((pcd->pclientMatteTopShadows =
                  AllocateRList((unsigned)NUM_MATTE_TS_RECTS)) == NULL)) {
                /* out of memory! */
                Warning(((char *)GETMESSAGE(
                    8, 4, "Insufficient memory for client window framing")));
                return (FALSE);
        }

        if ((pcd->matteWidth) && (pcd->pclientMatteBottomShadows == NULL) &&
            ((pcd->pclientMatteBottomShadows =
                  AllocateRList((unsigned)NUM_MATTE_BS_RECTS)) == NULL)) {
                /* out of memory! */
                Warning(((char *)GETMESSAGE(
                    8, 5, "Insufficient memory for client window framing")));
                return (FALSE);
        }
...
Boolean AllocateGadgetRectangles(ClientData *pcd) {
        ...
        if (decor & MWM_DECOR_TITLE) {
                ...
                if (pcd->pTitleGadgets == NULL) {
                        ...
                        pgr = (GadgetRectangle *)XtMalloc(
                            num_rects * sizeof(GadgetRectangle));
                        if (pgr == NULL) {
                                /* out of memory! */
                                Warning(((char *)GETMESSAGE(8, 6,
                                                            "Insufficient "
                                                            "memory for client "
                                                            "window framing")));
                                return (FALSE);
        ...
        if (decor & MWM_DECOR_RESIZEH) {
                ...
                if (pcd->pResizeGadgets == NULL) {
                        /* allocate memory for these guys */
                        pgr = (GadgetRectangle *)XtMalloc(
                            STRETCH_COUNT * sizeof(GadgetRectangle));
                        if (pgr == NULL) {
                                /* out of memory! */
                                Warning(((char *)GETMESSAGE(8, 7,
                                                            "Insufficient "
                                                            "memory for client "
                                                            "window framing")));
                                return (FALSE);
...
void ComputeGadgetRectangles(ClientData *pcd) {
        ...
        if (decor & MWM_DECOR_TITLE) {
                ...
                /*
                 * Fixed bug where last button in title bar did not activate
                 * when the client's X border was showing.
                 */
                pcd->titleRectangle.width =
                    clientWidth + (XBorderIsShowing(pcd) ? 2 * pcd->xBorderWidth
                                                         : 2 * pcd->matteWidth);
                ...
                GetFramePartInfo(pcd, FRAME_TITLE, &fpX, &fpY, &fpWidth,
                                 &fpHeight);
                ...
                if (decor & MWM_DECOR_MENU) {
                        ...
                        GetFramePartInfo(pcd, FRAME_SYSTEM, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                ...
                if (decor & MWM_DECOR_MINIMIZE) {
                        ...
                        GetFramePartInfo(pcd, FRAME_MINIMIZE, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        /* copy in and convert to shorts */
                        pgr[igr].rect.x = fpX;
                        pgr[igr].rect.y = fpY;
                ...
                if (decor & MWM_DECOR_MAXIMIZE) {
                        ...
                        GetFramePartInfo(pcd, FRAME_MAXIMIZE, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        /* copy in and convert to shorts */
                        pgr[igr].rect.x = fpX;
                        pgr[igr].rect.y = fpY;
        ...
        if (decor & (MWM_DECOR_RESIZEH | MWM_DECOR_BORDER)) {
                ...
                pcd->matteRectangle.height =
                    pcd->frameInfo.height - pcd->frameInfo.titleBarHeight;
        }
        ...
        if (decor & MWM_DECOR_RESIZEH) {
                ...
                if (decor & MWM_DECOR_RESIZEH) {
                        ...
                        GetFramePartInfo(pcd, FRAME_RESIZE_NW, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        /* copy in and convert to shorts */
                        pgr[igr].rect.x = fpX;
                        pgr[igr].rect.y = fpY;
                        ...
                        GetFramePartInfo(pcd, FRAME_RESIZE_N, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        if ((int)fpWidth > 0) {
                                /* copy in and convert to shorts */
                                pgr[igr].rect.x = fpX;
                        ...
                        GetFramePartInfo(pcd, FRAME_RESIZE_NE, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        /* copy in and convert to shorts */
                        pgr[igr].rect.x = fpX;
                        pgr[igr].rect.y = fpY;
                        ...
                        GetFramePartInfo(pcd, FRAME_RESIZE_W, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        if ((int)fpHeight > 0) {
                                /* copy in and convert to shorts */
                                pgr[igr].rect.x = fpX;
                        ...
                        GetFramePartInfo(pcd, FRAME_RESIZE_E, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        if ((int)fpHeight > 0) {
                                /* copy in and convert to shorts */
                                pgr[igr].rect.x = fpX;
                        ...
                        GetFramePartInfo(pcd, FRAME_RESIZE_SW, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        /* copy in and convert to shorts */
                        pgr[igr].rect.x = fpX;
                        pgr[igr].rect.y = fpY;
                        ...
                        GetFramePartInfo(pcd, FRAME_RESIZE_S, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        if ((int)fpWidth > 0) {
                                /* copy in and convert to shorts */
                                pgr[igr].rect.x = fpX;
                        ...
                        GetFramePartInfo(pcd, FRAME_RESIZE_SE, &fpX, &fpY,
                                         &fpWidth, &fpHeight);
                        /* copy in and convert to shorts */
                        pgr[igr].rect.x = fpX;
                        pgr[igr].rect.y = fpY;
        ...
        if ((pcd->clientState == MINIMIZED_STATE) ||
            ((pcd->clientState != MINIMIZED_STATE) &&
             (context == F_SUBCONTEXT_IB_WICON))) {
                 ...
                 * If it would hit the top of the screen then try to put it
                 * below the icon and label. If it would then hit the bottom of
                 * the screen turn of the hotspot processing.
                 */
                 ...
                 *   titlebar/border.
                 * If it would hit the bottom of the screen then try to put it
                 * directly above the NW corner of the titlebar/border. If it
                 * would then hit the top of the screen turn of the hotspot
                 *   processing.
                 */
                ...
                if ((pcd->decor & MWM_DECOR_TITLE) &&
                    !(pcd->decor & (MWM_DECOR_RESIZEH | MWM_DECOR_BORDER))) {
                        ...
                        *px =
                            pcd->frameInfo.x + pcd->frameInfo.lowerBorderWidth;
                        *py = pcd->frameInfo.y +
                              pcd->frameInfo.upperBorderWidth +
                              pcd->frameInfo.titleBarHeight;
                }
                        ...
                        if ((pcd->decor & MWM_DECOR_TITLE) &&
                            !(pcd->decor &
                              (MWM_DECOR_RESIZEH | MWM_DECOR_BORDER))) {
                                ...
                                *py = pcd->frameInfo.y +
                                      pcd->frameInfo.upperBorderWidth - height;
                        }
                        if (*py < 0) {
                                wmGD.checkHotspot = FALSE;
                        }
                ...
                wmGD.hotspotRectangle.y =
                    pcd->frameInfo.y + pcd->frameInfo.upperBorderWidth;
...
void ShowActiveClientFrame(ClientData *pcd) {
...
void ShowInactiveClientFrame(ClientData *pcd) {
        unsigned long attr_mask = 0;
        XSetWindowAttributes window_attribs;
...
void RegenerateClientFrame(ClientData *pcd) {
        ...
        XMoveResizeWindow(DISPLAY, pcd->clientFrameWin, pcd->frameInfo.x,
                          pcd->frameInfo.y, pcd->frameInfo.width,
                          pcd->frameInfo.height);
        ...
        if (SHOW_RESIZE_CURSORS(pcd) && (decor & MWM_DECOR_RESIZEH)) {
                XMoveResizeWindow(
                    DISPLAY, pcd->clientStretchWin[STRETCH_NORTH_WEST], 0, 0,
                    pcd->frameInfo.cornerWidth, pcd->frameInfo.cornerHeight);

                XMoveResizeWindow(DISPLAY, pcd->clientStretchWin[STRETCH_NORTH],
                ...
                XMoveResizeWindow(
                    DISPLAY, pcd->clientStretchWin[STRETCH_NORTH_EAST],
                    (int)(pcd->frameInfo.width - pcd->frameInfo.cornerWidth), 0,
                    pcd->frameInfo.cornerWidth, pcd->frameInfo.cornerHeight);

                XMoveResizeWindow(DISPLAY, pcd->clientStretchWin[STRETCH_EAST],
                                  (int)(pcd->frameInfo.width -
                                        pcd->frameInfo.lowerBorderWidth),
                ...
                XMoveResizeWindow(
                    DISPLAY, pcd->clientStretchWin[STRETCH_SOUTH_EAST],
                    (int)(pcd->frameInfo.width - pcd->frameInfo.cornerWidth),
                    (int)(pcd->frameInfo.height - pcd->frameInfo.cornerHeight),
                    pcd->frameInfo.cornerWidth, pcd->frameInfo.cornerHeight);

                XMoveResizeWindow(DISPLAY, pcd->clientStretchWin[STRETCH_SOUTH],
                                  (int)pcd->frameInfo.cornerWidth,
                                  (int)(pcd->frameInfo.height -
                                        pcd->frameInfo.lowerBorderWidth),
                                  pcd->frameInfo.width -
                                      2 * pcd->frameInfo.cornerWidth,
                                  pcd->frameInfo.lowerBorderWidth);
                ...
                XMoveResizeWindow(
                    DISPLAY, pcd->clientStretchWin[STRETCH_SOUTH_WEST], 0,
                    (int)(pcd->frameInfo.height - pcd->frameInfo.cornerHeight),
                    pcd->frameInfo.cornerWidth, pcd->frameInfo.cornerHeight);

                XMoveResizeWindow(DISPLAY, pcd->clientStretchWin[STRETCH_WEST],
...
void BevelSystemButton(RList *prTop, RList *prBot, int x, int y,
        ...
        if (height >= 5) {
                ...
                BevelRectangle(prTop, prBot, /* system icon */
                               (x + offset1), (y + offset2), dim1, dim2, 1, 1,
                               1, 1);
        }
...
void BevelMinimizeButton(RList *prTop, RList *prBot, int x, int y,
        ...
        if (height >= 7) {
                /* minimize icon */
                BevelRectangle(prTop, prBot, (x + offset1), (y + offset2), dim1,
                               dim2, 1, 1, 1, 1);
        }
...
void BevelMaximizeButton(RList *prTop, RList *prBot, int x, int y,
        ...
        /* maximize icon */
        BevelRectangle(prTop, prBot, (x + offset1), (y + offset2), dim1, dim2,
                       1, 1, 1, 1);
...
Boolean DepressGadget(ClientData *pcd, int gadget, Boolean depressed) {
        ...
        case FRAME_MINIMIZE:
        case FRAME_MAXIMIZE:
                if (!GetDepressInfo(pcd, gadget, &x, &y, &width, &height,
                                    &invertWidth))
                        return (FALSE);
        ...
        if ((pTopRect && pBotRect) ||
            ((pTopRect = AllocateRList(2)) && (pBotRect = AllocateRList(2)))) {
                pTopRect->used = 0;
                pBotRect->used = 0;
                BevelRectangle(pTopRect, pBotRect, x, y, width, height,
                               invertWidth, invertWidth, invertWidth,
                               invertWidth);
        }
        ...
        if (depressed) {
                XFillRectangles(DISPLAY, win, botGC, pTopRect->prect,
                                pTopRect->used);
                XFillRectangles(DISPLAY, win, topGC, pBotRect->prect,
                                pBotRect->used);
        } else {
                XFillRectangles(DISPLAY, win, topGC, pTopRect->prect,
                                pTopRect->used);
                XFillRectangles(DISPLAY, win, botGC, pBotRect->prect,
                                pBotRect->used);
        }
        return (TRUE);
...
void SetFrameShape(ClientData *pcd) {
        ...
        if (pcd->wShaped) {
                ...
                XShapeCombineShape(DISPLAY, pcd->clientBaseWin, ShapeBounding,
                                   xOffset, yOffset, pcd->client, ShapeBounding,
                                   ShapeSet);

                XShapeCombineShape(DISPLAY, pcd->clientFrameWin, ShapeBounding,
                                   BaseWindowX(pcd), BaseWindowY(pcd),
                                   pcd->clientBaseWin, ShapeBounding, ShapeSet);

                if (pcd->decor & MWM_DECOR_TITLE) {
                        XShapeCombineShape(
                            DISPLAY, pcd->clientFrameWin, ShapeBounding,
                            pcd->frameInfo.upperBorderWidth,
                            pcd->frameInfo.upperBorderWidth,
                            pcd->clientTitleWin, ShapeBounding, ShapeUnion);
                }
        } else {
                (void)XShapeCombineMask(DISPLAY, pcd->clientFrameWin,
                                        ShapeBounding, 0, 0, None, ShapeSet);
                (void)XShapeCombineMask(DISPLAY, pcd->clientFrameWin, ShapeClip,
                                        0, 0, None, ShapeSet);
                (void)XShapeCombineMask(DISPLAY, pcd->clientBaseWin,
                                        ShapeBounding, 0, 0, None, ShapeSet);
                (void)XShapeCombineMask(DISPLAY, pcd->clientBaseWin, ShapeClip,
                                        0, 0, None, ShapeSet);
        }
