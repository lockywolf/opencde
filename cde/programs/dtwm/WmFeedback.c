...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmFeedback.c /main/6 1996/10/23 17:20:55 rswiston $"
#endif
#endif
/*
...
static char *confirm_mesg[4] = {"Switch to Default Behavior?",
                                "Switch to Custom Behavior?", "Restart Mwm?",
                                "QUIT Mwm?"};
...
#else  /* MINIMAL_DT */
                tmpString =
                    ((char *)GETMESSAGE(22, 11, "QUIT Workspace Manager?"));
#endif /* MINIMAL_DT */
        }
...
static char *confirm_mesg[4] = {"Toggle to Default Behavior?",
                                "Toggle to Custom Behavior?", "Restart Mwm?",
                                "QUIT Mwm?"};

#endif
static char *confirm_widget[4] = {"confirmDefaultBehavior",
                                  "confirmCustomBehavior", "confirmRestart",
                                  "confirmQuit"};

typedef void (*ConfirmFunc)(Boolean);
static ConfirmFunc confirm_func[4] = {Do_Set_Behavior, Do_Set_Behavior,
                                      Do_Restart, Do_Quit_Mwm};
...
void ShowFeedbackWindow(WmScreenData *pSD, int x, int y, unsigned int width,
        ...
        if (mask & (XValue | YValue)) {
                winX = (mask & XNegative) ? DisplayWidth(DISPLAY, pSD->screen) +
                                                tmpX - pSD->fbWinWidth
                                          : tmpX;
                winY = (mask & YNegative)
                           ? DisplayHeight(DISPLAY, pSD->screen) + tmpY -
                                 pSD->fbWinHeight
                           : tmpY;
        } else {
                winX =
                    (DisplayWidth(DISPLAY, pSD->screen) - pSD->fbWinWidth) / 2;
                winY =
                    (DisplayHeight(DISPLAY, pSD->screen) - pSD->fbWinHeight) /
                    2;
        }
        ...
        if ((pSD->fbTop && pSD->fbBottom) ||
            ((pSD->fbTop = AllocateRList((unsigned)2 * FEEDBACK_BEVEL)) &&
             (pSD->fbBottom = AllocateRList((unsigned)2 * FEEDBACK_BEVEL)))) {
                pSD->fbTop->used = 0;
                pSD->fbBottom->used = 0;
                BevelRectangle(pSD->fbTop, pSD->fbBottom, 0, 0, pSD->fbWinWidth,
                               pSD->fbWinHeight, FEEDBACK_BEVEL, FEEDBACK_BEVEL,
        ...
        if (!pSD->feedbackWin) {
                ...
                pSD->feedbackWin = XCreateWindow(
                    DISPLAY, pSD->rootWindow, winX, winY, pSD->fbWinWidth,
                    pSD->fbWinHeight, 0, CopyFromParent, InputOutput,
        ...
        if (pSD->feedbackWin) {
                ...
                if (pSD->fbBottom->used > 0) {
                        XFillRectangles(
                            ...
                            pSD->fbBottom->prect, pSD->fbBottom->used);
                }

                /*
                 * clear old text
                 */
                XClearArea(DISPLAY, pSD->feedbackWin, FEEDBACK_BEVEL,
                           FEEDBACK_BEVEL, pSD->fbWinWidth - 2 * FEEDBACK_BEVEL,
                           pSD->fbWinHeight - 2 * FEEDBACK_BEVEL, FALSE);
                ...
                if (pSD->fbStyle & FB_POSITION) {
                        WmDrawString(DISPLAY, pSD->feedbackWin,
                                     ...
                                     pSD->fbLocX, pSD->fbLocY, pSD->fbLocation,
                                     strlen(pSD->fbLocation));
                }
                if (pSD->fbStyle & FB_SIZE) {
                        WmDrawString(DISPLAY, pSD->feedbackWin,
                                     pSD->feedbackAppearance.inactiveGC,
                                     pSD->fbSizeX, pSD->fbSizeY, pSD->fbSize,
                                     strlen(pSD->fbSize));
                }
        }
}
 ...
 *
 *************************************<->***********************************/
void UpdateFeedbackInfo(WmScreenData *pSD, int x, int y, unsigned int width,
                        unsigned int height) {
/*
 ...
 *
 *************************************<->***********************************/
void UpdateFeedbackText(WmScreenData *pSD, int x, int y, unsigned int width,
                        unsigned int height) {
        int direction, ascent, descent;
        XCharStruct xcs;
...
void ConfirmAction(WmScreenData *pSD, int nbr) {
        ...
        /* First time for this one */
        {
                ...
                XtSetArg(args[n], XmNx,
                         (XtArgVal)(DisplayWidth(DISPLAY, pSD->screen) / 2));
                n++;
                XtSetArg(args[n], XmNy,
                         (XtArgVal)(DisplayHeight(DISPLAY, pSD->screen) / 2));
                n++;
                XtSetArg(args[n], XtNallowShellResize, (XtArgVal)TRUE);
                n++;
                XtSetArg(args[n], XtNkeyboardFocusPolicy, (XtArgVal)XmEXPLICIT);
                n++;
                XtSetArg(args[n], XtNdepth,
                         (XtArgVal)DefaultDepth(DISPLAY, pSD->screen));
                n++;
                XtSetArg(args[n], XtNscreen,
                         (XtArgVal)ScreenOfDisplay(DISPLAY, pSD->screen));
                n++;

                dialogShellW = XtCreatePopupShell(
                    (String)WmNfeedback, transientShellWidgetClass,
                    pSD->screenTopLevelW, args, n);

                /*
                 * Create a QuestionBox as a child of the popup shell.
                 * Set traversalOn and add callbacks for the OK and CANCEL
                 * buttons. Unmanage the HELP button.
                 */

                n = 0;
                XtSetArg(args[n], XmNdialogType, (XtArgVal)XmDIALOG_QUESTION);
                n++;
                XtSetArg(args[n], XmNmessageAlignment,
                         (XtArgVal)XmALIGNMENT_CENTER);
                n++;
                XtSetArg(args[n], XmNtraversalOn, (XtArgVal)TRUE);
                n++;

                /*
                 * In 1.2 confirmbox's widget name changed from the generic
                 * WmNconfirmbox (ie. 'confirmbox') to a more descriptive name
                 * so that each confirm dialog can be customized separately
                 * (e.g. "Mwm*confirmRestart*messageString: restart it?").
                 */

                pSD->confirmboxW[nbr] = XtCreateManagedWidget(
                    confirm_widget[nbr], xmMessageBoxWidgetClass, dialogShellW,
                    args, n);

                n = 0;
                XtSetArg(args[n], XmNmessageString, &messageString);
                n++;
                ...
                if (XmStringCompare(messageString, defaultMessageString)) {
                        messageString =
                            XmStringCreateLocalized(confirm_mesg[nbr]);
                        XtSetArg(args[n], XmNmessageString,
                                 (XtArgVal)messageString);
                        n++;
                ...
                XtSetArg(args[n], XmNhighlightThickness,
                         (XtArgVal)CB_HIGHLIGHT_THICKNESS);
                n++;
#ifndef NO_MESSAGE_CATALOG
                XtSetArg(args[n], XmNlabelString, wmGD.okLabel);
                n++;
#endif
...
#ifndef NO_MESSAGE_CATALOG
                n--;
                XtSetArg(args[n], XmNlabelString, wmGD.cancelLabel);
                n++;
#endif
                XtSetValues(XmMessageBoxGetChild(pSD->confirmboxW[nbr],
                ...
                XtUnmanageChild(XmMessageBoxGetChild(pSD->confirmboxW[nbr],
                ...
                n = 0;
                XtSetArg(args[n], XmNx, (XtArgVal)x);
                n++;
                XtSetArg(args[n], XmNy, (XtArgVal)y);
                n++;
...
void ShowWaitState(Boolean flag) {
        ...
        if (flag) {
                XGrabPointer(DISPLAY, DefaultRootWindow(DISPLAY), FALSE, 0,
                             GrabModeAsync, GrabModeAsync, None, waitCursor,
                             CurrentTime);
...
void InitCursorInfo(void) {
        ...
        if (XQueryBestCursor(DISPLAY, DefaultRootWindow(DISPLAY), 32, 32,
                             &cWidth, &cHeight)) {
