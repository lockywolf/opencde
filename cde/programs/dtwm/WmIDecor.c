...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmIDecor.c /main/6 1996/06/20 09:38:43 rswiston $"
#endif
#endif
/*
...
Boolean MakeIcon(WmWorkspaceData *pWS, ClientData *pcd) {
        ...
        {
#ifdef WSM
                pWsc->iconFrameWin = XCreateWindow(
                    DISPLAY, ROOT_FOR_CLIENT(pcd), /* parent */
                    pWsc->iconX, pWsc->iconY,
#else  /* WSM */
                pcd->iconFrameWin = XCreateWindow(
                    DISPLAY, ROOT_FOR_CLIENT(pcd), /* parent */
                    pcd->iconX, pcd->iconY,
#endif /* WSM */
                    (unsigned int)ICON_WIDTH(pcd),
                    (unsigned int)ICON_HEIGHT(pcd), 0, /* border width */
                 ...
                 * Don't make icon in the box for any icon box (or any WM
                 * window) OR any client that doesn't have the MWM_FUNC_MINIMIZE
                 * bit set in pcd->clientFunctions
                 */
        ...
        if ((pcd->piconTopShadows =
                ...
                /* Out of memory! */
                Warning(((char *)GETMESSAGE(
                    30, 2, "Insufficient memory for icon creation")));
                return (FALSE);
        }
        ...
        if ((pcd->piconBottomShadows =
                ...
                /* Out of memory! */
                Warning(((char *)GETMESSAGE(
                    30, 3, "Insufficient memory for icon creation")));
                return (FALSE);
        }
...
void MakeIconShadows(ClientData *pcd, int xOffset, int yOffset) {
        ...
        case ICON_LABEL_PART:
                BevelRectangle(
                    pcd->piconTopShadows, /* label */
                    pcd->piconBottomShadows, 0 + xOffset,
                    (int)ICON_IMAGE_HEIGHT(pcd) + yOffset,
                    (unsigned int)ICON_WIDTH(pcd),
                    (unsigned int)ICON_LABEL_HEIGHT(pcd),
                    ICON_EXTERNAL_SHADOW_WIDTH, ICON_EXTERNAL_SHADOW_WIDTH,
                    ICON_EXTERNAL_SHADOW_WIDTH, ICON_EXTERNAL_SHADOW_WIDTH);
                break;

        case ICON_IMAGE_PART:
                BevelRectangle(
                    pcd->piconTopShadows, /* image outside */
                    pcd->piconBottomShadows, 0 + xOffset, 0 + yOffset,
                    (unsigned int)ICON_WIDTH(pcd),
                ...
                if (wmGD.frameStyle == WmRECESSED)
                        BevelRectangle(
                            pcd->piconBottomShadows, /* image inside */
                            pcd->piconTopShadows, ICON_INNER_X_OFFSET + xOffset,
        ...
        if (ACTIVE_PSD->useIconBox && P_ICON_BOX(pcd)) {
                ...
                if (pcd->clientState == MINIMIZED_STATE) {
                        ...
                        if (pcd->piconTopShadows) {
                                ...
                                XFillRectangles(DISPLAY, ICON_FRAME_WIN(pcd),
                                                topGC,
                        ...
                        XFillRectangles(DISPLAY, ICON_FRAME_WIN(pcd),
                                        SHRINK_WRAP_GC(pcd), &shrinkRects[0],
                                        4);
                }
        ...
        */
        ...
        if (expose &&
            ((!pcd->iconWindow && (ICON_DECORATION(pcd) & ICON_IMAGE_PART)) ||
             (ACTIVE_PSD->useIconBox && P_ICON_BOX(pcd) && pcd->iconWindow &&
              pcd->clientState != MINIMIZED_STATE &&
                ...
                if (image) {
                        ...
                        if ((ACTIVE_PSD->useIconBox) && (P_ICON_BOX(pcd))) {
                                ...
                                if (pcd->clientState != MINIMIZED_STATE) {
                                        ...
                                        XCopyArea(
                                            DISPLAY, image, ICON_FRAME_WIN(pcd),
                                            iconGC, ICON_INTERNAL_SHADOW_WIDTH,
                                            ICON_INTERNAL_SHADOW_WIDTH, width,
                                        ...
                                        if (FADE_NORMAL_ICON(pcd)) {
                                                ...
                                                XFillRectangle(
                                                    DISPLAY,
                                                    ICON_FRAME_WIN(pcd), iconGC,
                                                    dest_x, dest_y, width,
                                                    height);
                                        }

                                } else {
                                        dest_x = ICON_IMAGE_X_OFFSET + xOffset;

                                        dest_y = ICON_IMAGE_Y_OFFSET + yOffset;
                                        if (wmGD.frameStyle == WmSLAB) {
                                                /* less beveling in this style
                                                 */
                                                dest_x -=
                                        ...
                                        width =
                                            ICON_IMAGE_MAXIMUM(pcd).width +
                                            (2 * ICON_INTERNAL_SHADOW_WIDTH);
                                        height =
                                            ICON_IMAGE_MAXIMUM(pcd).height +
                                            (2 * ICON_INTERNAL_SHADOW_WIDTH);
...

                        {
                                ...
                                XCopyArea(DISPLAY, image, ICON_FRAME_WIN(pcd),
                                          iconGC, 0, 0, width, height, dest_x,
                                          dest_y);
                        }
                }
        }
...
void GetIconTitleBox(ClientData *pcd, XRectangle *pBox) {
        ...
        if ((P_ICON_BOX(pcd)) && (pcd->clientState != MINIMIZED_STATE)) {
                ...
                pBox->x = ICON_EXTERNAL_SHADOW_WIDTH + ICON_IMAGE_LEFT_PAD +
                          (2 * ICON_INTERNAL_SHADOW_WIDTH) +
                          ICON_IMAGE_LEFT_PAD + xOffset;

                pBox->y =
                    ICON_IMAGE_HEIGHT(pcd) + yOffset +
                    ((ICON_IMAGE_HEIGHT(pcd) > 0)
                         ? -ICON_IMAGE_BOTTOM_PAD
                         : ICON_EXTERNAL_SHADOW_WIDTH) +
                    ((ICON_IMAGE_HEIGHT(pcd) > 0) ? 0 : WM_TOP_TITLE_PADDING);
                ...
                pBox->y = ICON_IMAGE_HEIGHT(pcd) + yOffset +
                          ((ICON_IMAGE_HEIGHT(pcd) > 0)
                               ? ICON_INTERNAL_SHADOW_WIDTH
                               : ICON_EXTERNAL_SHADOW_WIDTH) +
                ...
                pBox->x =
                    ICON_EXTERNAL_SHADOW_WIDTH + WM_TOP_TITLE_PADDING + xOffset;
                ...
                pBox->y =
                    ICON_IMAGE_HEIGHT(pcd) + WM_TOP_TITLE_PADDING + yOffset +
                    ((ICON_IMAGE_HEIGHT(pcd) > 0) ? ICON_INTERNAL_SHADOW_WIDTH
                                                  : ICON_EXTERNAL_SHADOW_WIDTH);
...
void DrawIconTitle(ClientData *pcd) {
        ...
        WmDrawXmString(DISPLAY, ICON_FRAME_WIN(pcd),
                       ICON_APPEARANCE(pcd).fontList, pcd->iconTitle, iconGC,
...
void RedisplayIconTitle(ClientData *pcd) {
        ...
        if (ICON_DECORATION(pcd) & ICON_LABEL_PART && ICON_FRAME_WIN(pcd)) {
                ...
                /* out with the old */
                XClearArea(DISPLAY, ICON_FRAME_WIN(pcd), textBox.x, textBox.y,
...
#ifdef WSM
                WmDrawXmString(DISPLAY, ICON_FRAME_WIN(pcd),
                               ICON_APPEARANCE(pcd).fontList, pcd->iconTitle,
                               iconGC, textBox.x, textBox.y, textBox.width,
                               &textBox, True);
#else  /* WSM */
                WmDrawXmString(DISPLAY, ICON_FRAME_WIN(pcd),
                               ICON_APPEARANCE(pcd).fontList, pcd->iconTitle,
...
void GetIconDimensions(WmScreenData *pSD, unsigned int *pWidth,
        ...
        *pWidth = pSD->iconImageMaximum.width +
                  ((wmGD.frameStyle == WmSLAB) ? 2 : 0) + ICON_IMAGE_LEFT_PAD +
                  ICON_IMAGE_RIGHT_PAD + 2 * ICON_EXTERNAL_SHADOW_WIDTH +
                  4 * ICON_INTERNAL_SHADOW_WIDTH;
        if (wmGD.frameStyle == WmSLAB) {
                /* less beveling in this style */
        ...
        case ICON_LABEL_PART:
                ...
                *pLabelHeight =
                    ICON_EXTERNAL_SHADOW_WIDTH + WM_TOP_TITLE_PADDING +
                    TEXT_HEIGHT(pSD->iconAppearance.font) +
                    WM_BOTTOM_TITLE_PADDING + ICON_EXTERNAL_SHADOW_WIDTH;
                break;

        case ICON_IMAGE_PART:
                *pImageHeight =
                    ICON_EXTERNAL_SHADOW_WIDTH + ICON_IMAGE_TOP_PAD +
                    ICON_INTERNAL_SHADOW_WIDTH + ICON_INTERNAL_SHADOW_WIDTH +
                    pSD->iconImageMaximum.height +
                    ((wmGD.frameStyle == WmSLAB) ? 2 : 0) +
                    ICON_INTERNAL_SHADOW_WIDTH + ICON_INTERNAL_SHADOW_WIDTH +
                    ICON_IMAGE_BOTTOM_PAD + ICON_EXTERNAL_SHADOW_WIDTH;
                if (wmGD.frameStyle == WmSLAB) {
                        /* less beveling in this style */
        ...
        case (ICON_IMAGE_PART | ICON_LABEL_PART):
                *pImageHeight =
                    ...
                    ((wmGD.frameStyle == WmSLAB) ? 2 : 0) +
                    ICON_INTERNAL_SHADOW_WIDTH + ICON_INTERNAL_SHADOW_WIDTH +
                    ICON_IMAGE_BOTTOM_PAD + ICON_INTERNAL_SHADOW_WIDTH;

                *pLabelHeight =
                    ICON_INTERNAL_SHADOW_WIDTH + WM_TOP_TITLE_PADDING +
                    TEXT_HEIGHT(pSD->iconAppearance.font) +
                    WM_BOTTOM_TITLE_PADDING + ICON_EXTERNAL_SHADOW_WIDTH;
                if (wmGD.frameStyle == WmSLAB) {
                        /*
...
void InitIconSize(WmScreenData *pSD) {
        ...
        GetIconDimensions(pSD, (unsigned int *)&(pSD->iconWidth), &label,
                          &image);
        ...
        iconShrinkX = IB_MARGIN_WIDTH + ICON_EXTERNAL_SHADOW_WIDTH +
                      ICON_IMAGE_LEFT_PAD + 2 * ICON_INTERNAL_SHADOW_WIDTH;

        iconShrinkY =
            IB_MARGIN_HEIGHT + ICON_EXTERNAL_SHADOW_WIDTH +
            ((pSD->iconDecoration & ICON_IMAGE_PART)
                 ? (ICON_IMAGE_TOP_PAD + (2 * ICON_INTERNAL_SHADOW_WIDTH))
                 : (WM_TOP_TITLE_PADDING));
        if (wmGD.frameStyle == WmSLAB) {
                /* less beveling in this style */
        ...
        case (ICON_IMAGE_PART | ICON_LABEL_PART):
                iconShrinkHeight =
                    pSD->iconHeight - ICON_EXTERNAL_SHADOW_WIDTH -
                    ICON_IMAGE_TOP_PAD - ICON_INTERNAL_SHADOW_WIDTH -
                    ICON_INTERNAL_SHADOW_WIDTH - ICON_IMAGE_BOTTOM_PAD -
                    WM_BOTTOM_TITLE_PADDING - ICON_EXTERNAL_SHADOW_WIDTH;
                if (wmGD.frameStyle == WmSLAB) {
...
void ShowActiveIcon(ClientData *pcd) {
        ...
        if (ICON_FRAME_WIN(pcd)) {
                ...
                if ((!ACTIVE_PSD->useIconBox) || (P_ICON_BOX(pcd) == NULL)) {
                        ...
                        if (ICON_DECORATION(pcd) & ICON_LABEL_PART) {
                                XClearArea(DISPLAY, ICON_FRAME_WIN(pcd), 0,
                                           ICON_IMAGE_HEIGHT(pcd),
                                           (unsigned int)ICON_WIDTH(pcd),
        ...
        if (ICON_FRAME_WIN(pcd)) {
                ...
                if (refresh) {
                        ...
                        if ((!ACTIVE_PSD->useIconBox) ||
                            (P_ICON_BOX(pcd) == NULL)) {
#ifndef MOTIF_ONE_DOT_ONE
                                XmeClearBorder(DISPLAY, ICON_FRAME_WIN(pcd), 0,
                                               0, ICON_WIDTH(pcd),
                                               ICON_IMAGE_HEIGHT(pcd), 4);
                                ...
                                /*
                                 * clear only area of real frame, not highlight
                                 * area
                                 */
...
void ReparentIconWindow(ClientData *pcd, int xOffset, int yOffset) {
        ...
        if (!pcd->pECD) {
                 ...
                 */
                XGetGeometry(DISPLAY, pcd->iconWindow, &root, &x, &y, &width,
                             &height, &bw, &depth);
                ...
                if (width > ((ICON_IMAGE_MAXIMUM(pcd).width) +
                             ((wmGD.frameStyle == WmSLAB) ? 2 : 0))) {
                        width = windowChanges.width =
                            ICON_IMAGE_MAXIMUM(pcd).width +
                            ((wmGD.frameStyle == WmSLAB) ? 2 : 0);
                        mask |= CWWidth;
                } else if (width < ICON_IMAGE_MINIMUM(pcd).width) {
                        width = windowChanges.width =
                            ICON_IMAGE_MINIMUM(pcd).width;
                        mask |= CWWidth;
                }

                if (height > ((ICON_IMAGE_MAXIMUM(pcd).height) +
                              ((wmGD.frameStyle == WmSLAB) ? 2 : 0))) {
                        height = windowChanges.height =
                            ICON_IMAGE_MAXIMUM(pcd).height +
                            ((wmGD.frameStyle == WmSLAB) ? 2 : 0);
                        mask |= CWHeight;
                } else if (height < ICON_IMAGE_MINIMUM(pcd).height) {
                        height = windowChanges.height =
                            ICON_IMAGE_MINIMUM(pcd).height;
                        mask |= CWHeight;
                }

                if (mask)
                        XConfigureWindow(DISPLAY, pcd->iconWindow, mask,
                                         &windowChanges);

                /*
                 * Reparent the icon window to the center of the icon image
                 * frame
                 */
                ...
                XReparentWindow(DISPLAY, pcd->iconWindow, ICON_FRAME_WIN(pcd),
                                rpX, rpY);
 ...
 *
 *************************************<->***********************************/
void PutBoxInIconBox(ClientData *pCD, int *px, int *py, unsigned int *width,
                     unsigned int *height) {
        ...
        i = 0;
        XtSetArg(getArgs[i], XmNwidth, (XtArgVal)&bBoardWidth);
        i++;
        XtSetArg(getArgs[i], XmNheight, (XtArgVal)&bBoardHeight);
        i++;
...
void CreateActiveIconTextWindow(WmScreenData *pSD) {
        ...
        pSD->activeIconTextWin =
            XCreateWindow(DISPLAY, pSD->rootWindow, /* parent */
                          0, 0,                     /* x, y */
                          1, 1,                     /* width, height */
                          0,                        /* border width */
                          CopyFromParent,           /* depth */
...
void PaintActiveIconText(ClientData *pcd, Boolean erase) {
        ...
        if (pActiveIconTopRects) {
                XFillRectangles(DISPLAY, pcd->pSD->activeIconTextWin, topGC,
                                pActiveIconTopRects->prect,
                                pActiveIconTopRects->used);
        }

        if (pActiveIconBotRects) {
                XFillRectangles(DISPLAY, pcd->pSD->activeIconTextWin, botGC,
        ...
        if (erase) {
                XClearArea(DISPLAY, pcd->pSD->activeIconTextWin, textBox.x,
                           textBox.y, (unsigned int)textBox.width,
                           (unsigned int)textBox.height, FALSE);
        }
...
void ShowActiveIconText(ClientData *pcd) {
        ...
        if (pcd->pSD->activeIconTextWin) {
                ...
                XGetWindowAttributes(DISPLAY, ICON_FRAME_WIN(pcd),
                                     &iconFrameAttribs);
                ...
                XGetGeometry(DISPLAY, (Drawable)ICON_FRAME_WIN(pcd), &root, &x,
                             &y, &junk, &junk, &junk, &junk);
                ...
                if (!(P_ICON_BOX(pcd))) {
                        ...
                        PutBoxInIconBox(pcd, &x, &y, &activeIconTextWidth,
                                        &activeIconTextHeight);
                        if (ACTIVE_LABEL_PARENT(pcd) != pcd->client) {
                                XReparentWindow(
                                    DISPLAY, pcd->pSD->activeIconTextWin,
                                    XtWindow(P_ICON_BOX(pcd)->bBoardWidget), x,
                                    y);
                ...
                if ((pActiveIconTopRects && pActiveIconBotRects) ||
                    ((pActiveIconTopRects = AllocateRList(
                          (unsigned)4 * ICON_EXTERNAL_SHADOW_WIDTH)) &&
                     (pActiveIconBotRects = AllocateRList(
                          (unsigned)4 * ICON_EXTERNAL_SHADOW_WIDTH)))) {
                        pActiveIconTopRects->used = 0;
                        pActiveIconBotRects->used = 0;
                        BevelRectangle(pActiveIconTopRects, pActiveIconBotRects,
                                       0, 0, activeIconTextWidth,
                                       activeIconTextHeight,
                                       ICON_EXTERNAL_SHADOW_WIDTH,
                                       ICON_EXTERNAL_SHADOW_WIDTH,
                                       ICON_EXTERNAL_SHADOW_WIDTH,
...
void HideActiveIconText(WmScreenData *pSD) {
        ...
        if ((pSD && pSD->activeIconTextWin) || ACTIVE_ICON_TEXT_WIN) {
                ...
                XDeleteContext(DISPLAY,
                               pSD ? pSD->activeIconTextWin
                                   : ACTIVE_PSD->activeIconTextWin,
                               wmGD.windowContextType);

                /* hide the big label */
                XUnmapWindow(DISPLAY, pSD ? pSD->activeIconTextWin
                                          : ACTIVE_PSD->activeIconTextWin);
