...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmWrkspace.c /main/7 1996/10/23 17:26:33 rswiston $"
#endif
#endif
...

{
        ...
        if (pSD->presence.shellW && pSD->presence.onScreen &&
...

{
        ...
        if (!pPresence) {
                ...
                if (!(pPresence = (Atom *)XtMalloc(pCD->pSD->numWorkspaces *
                                                   sizeof(Atom)))) {
                        Warning(((char *)GETMESSAGE(76, 1,
        ...
        if (cPresence < pCD->numInhabited) {
                ...
                if (!(pPresence = (Atom *)XtRealloc((char *)pPresence,
                                                    pCD->numInhabited *
                                                        sizeof(Atom)))) {
...

{
        ...
        while (1) {
                 ...
                 */
                if ((pCLE->type == NORMAL_STATE) && (pCLE->pCD->putInAll)) {
...
WmWorkspaceData *CreateWorkspace(WmScreenData *pSD, unsigned char *pchTitle)

{
        ...
        if (pSD->numWsDataAllocated <= pSD->numWorkspaces) {
                iActiveWS =
                    (pSD->pActiveWS - pSD->pWS) / sizeof(WmWorkspaceData);
                pSD->numWsDataAllocated += WS_ALLOC_AMOUNT;
                pSD->pWS = (WmWorkspaceData *)XtRealloc(
                    (char *)pSD->pWS,
...

{
        ...
        if (pSD->numWorkspaces > 1) {
                ...
                if (pWS == ACTIVE_WS) {
                        ...
                        /*
                         * Use the "current" workspace as the default
                         * destination
                         */
                        pWSdest = ACTIVE_WS;
                }
                ...
                SaveWorkspaceResources(
                    pWSdest, (WM_RES_WORKSPACE_LIST | WM_RES_WORKSPACE_COUNT));

                dtSendWorkspaceModifyNotification(pSD, aOldId,
                                                  DtWSM_REASON_DELETE);
            ...
            && (_DtWsmGetDtWmHints(DISPLAY, pCD->client, &pHints) == Success)) {
                ...
                if (pHints->flags & DtWM_HINTS_BEHAVIORS) {
                        ...
                        if (pCD->dtwmBehaviors & DtWM_BEHAVIOR_SUB_RESTORED) {
                                ...
                                _DtWsmSetDtWmHints(DISPLAY, pCD->client,
                                                   pHints);
                        }
                }
...

{
        ...
        if ((pCD->pWsList = (WsClientData *)XtMalloc(
                ...
                Warning(((char *)GETMESSAGE(
                    76, 4, "Insufficient memory for client data")));
                return (False);
        }
        pCD->currentWsc = 0;
...
Boolean WorkspaceIsInCommand(Display *dpy, ClientData *pCD, WorkspaceID **ppIDs,
        ...
        if (FindClientDBMatch(pCD, (char **)&pch)) {
                ...
                if (pCD->pSD->remainingSessionItems) {
                        if (!(XGetWMClientMachine(dpy, pCD->client,
                                                  &clientMachineProp))) {
                                clientMachineProp.value = NULL;
                        }
                        if (FindDtSessionMatch(
                                wmcArgc, wmcArgv, pCD, pCD->pSD, (char **)&pch,
                                (char *)clientMachineProp.value)) {
                                /*
                                 * If we found a match to a client description
                                 * in the DtSessionHints, use the information
                                 * from the Hints instead of the command line
                                 */
                                if (pch) {
                                        if (ConvertNamesToIDs(pCD->pSD, pch,
                                                              ppIDs, pNumIDs)) {
                                                rval = True;
                                        }
                                        /*
                                         * free malloced memory containing
                                         * workspace list
                                         */
                                        XtFree((char *)pch);
                                }
...

{
        ...
        if ((pLocalIDs = (WorkspaceID *)XtMalloc(
                 WS_ALLOC_AMOUNT * sizeof(WorkspaceID))) == NULL) {
                Warning(((char *)GETMESSAGE(
                    76, 5, "Insufficient Memory (ConvertNamesToIDs)")));
            ...
            (pchLocal = (unsigned char *)XtMalloc(1 + strlen((char *)pchIn)))) {
                ...
                while ((pchName = GetSmartString(&pch))) {
                        ...
                        if (iwsx < pSD->numWorkspaces) {
                                 ...
                                 */
                                pLocalIDs[num] = (WorkspaceID)XInternAtom(
                                    DISPLAY, (char *)pchName, False);
                                num++;
                        }
                        ...
                        if (num >= numLocalIDs) {
                                /* list too small */
                                numLocalIDs += WS_ALLOC_AMOUNT;
                                if ((pLocalIDs = (WorkspaceID *)XtRealloc(
                                         (char *)pLocalIDs,
                                         numLocalIDs * sizeof(WorkspaceID))) ==
                                    NULL) {
                                        Warning(((char *)GETMESSAGE(
                                            76, 6,
                                            "Insufficient Memory "
...
void CheckForPutInAllRequest(ClientData *pCD, Atom *pIDs, unsigned int numIDs)
...
Boolean FindWsNameInCommand(int argc, char *argv[], unsigned char **ppch)
                ...
                while (--argc && !rval) {
                        if (!strcmp(argv[i], XRM_OPT) && (argc > 1)) {
                                ...
                                if ((*pchRes) && (*pch)) {
                                        ...
                                        pch = (unsigned char *)strrchr(
                                            (char *)pchRes, ':');
                                        if (pch) {
                                                *pch = '\0';
                                        }

                                        /* find beginning of last component of
                                         * resource spec
                                         */
                                        pch = (unsigned char *)strrchr(
                                            (char *)pchRes, '*');
                                        pchTmp = (unsigned char *)strrchr(
                                            (char *)pchRes, '.');
                                        if (pchTmp > pch) {
                                                pch = pchTmp;
                                        }
                                        ...
                                        if ((!strcmp((char *)pch,
                                            ...
                                            (!strcmp((char *)pch,
                                                     WmCWorkspaceList))) {
                                                /* match, compute return
                                                   position in passed in string
                                                 */
...

{
        ...
        if (pCD->sizeWsList < pCD->pSD->numWorkspaces) {
                ...
                pCD->sizeWsList = pCD->pSD->numWorkspaces;
                pCD->pWsList = (WsClientData *)XtRealloc(
                    (char *)pCD->pWsList,
...
void TakeClientOutOfWorkspace(WmWorkspaceData *pWS, ClientData *pCD)

{
        ...
        if (pWS && pCD && ClientInWorkspace(pWS, pCD)) {
                        ...
                        if ((pCD->pSD->useIconBox) && (pWsc->pIconBox) &&
                            (pCD->clientFunctions & MWM_FUNC_MINIMIZE)) {
                                ...
                                if ((pWsc->iconPlace != NO_ICON_PLACE) &&
                                    (pWS->IPData.placeList[pWsc->iconPlace]
                                         .pCD == pCD)) {
                                        pWS->IPData.placeList[pWsc->iconPlace]
                                            .pCD = NULL;
                ...
                for (ixA = 0; ixA < pCD->numInhabited; ixA++) {
                        if (Copying) {
                                memcpy(&pCD->pWsList[ixA - 1],
                                       &pCD->pWsList[ixA],
                                       sizeof(WsClientData));
                        } else if (pCD->pWsList[ixA].wsID == pWS->id) {
                                /*
                                 *  This is the one we're removing, start
                                 * copying here.
                                 */
                                Copying = True;
                        }
...
WmWorkspaceData *GetWorkspaceData(WmScreenData *pSD, WorkspaceID wsID)
...
Boolean InWindowList(Window w, Window wl[], int num)
...
Boolean ClientInWorkspace(WmWorkspaceData *pWS, ClientData *pCD)
...
WsClientData *GetWsClientData(WmWorkspaceData *pWS, ClientData *pCD)
...
void SetClientWsIndex(ClientData *pCD)
...
Boolean ProcessWorkspaceHints(ClientData *pCD)

{
        ...
        if ((pCD->client) &&
            (GetWorkspaceHints(DISPLAY, pCD->client, &pIDs, &numIDs, &bAll) ==
             Success) &&
            (bAll ||
             (numIDs && (pDiff = (WorkspaceID *)XtMalloc(
                             sizeof(WorkspaceID) * MAX(numIDs, numOld)))))) {
                /*
                ...
                if (pCD->putInAll) {
                        for (i = 0; i < pCD->pSD->numWorkspaces; i++) {
                                if (!ClientInWorkspace(&pCD->pSD->pWS[i],
                                                       pCD)) {
                                        AddClientToWorkspaces(
                                            pCD, &pCD->pSD->pWS[i].id, 1);
                                }
                        }
                } else {
                        /*
...
void ProcessWorkspaceHintList(ClientData *pCD, WorkspaceID *pIDs,
                              unsigned int numIDs)
...
void RemoveSingleClientFromWorkspaces(ClientData *pCD, WorkspaceID *pIDs,
                                      unsigned int numIDs)
                    ...
                    (ClientInWorkspace(pWS, pCD))) {
                        ...
                        if ((pWS == pCD->pSD->pActiveWS) &&
                            ...
                            !(pCD->clientState & UNSEEN_STATE)) {
                                SetClientState(
                                    pCD, (pCD->clientState | UNSEEN_STATE),
                                    CurrentTime);
                        }
                        TakeClientOutOfWorkspace(pWS, pCD);
...
void RemoveSubtreeFromWorkspaces(ClientData *pCD, WorkspaceID *pIDs,
                                 unsigned int numIDs)
 ...
 *
 ******************************<->***********************************/
WorkspaceID *GetListOfOccupiedWorkspaces(ClientData *pCD, int *numIDs) {
        int i;
        ...
        *numIDs = 0;
        ...
        if ((pLocalIDs = (WorkspaceID *)XtMalloc(
                 pCD->numInhabited * sizeof(WorkspaceID))) == NULL) {
...
void HonorAbsentMapBehavior(ClientData *pCD) {
        int inWorkspace = 0;
        ...
        if (pCD->absentMapBehavior == AMAP_BEHAVIOR_MOVE) {
                 ...
                 */
                for (wsCnt = 0; wsCnt < pCD->numInhabited;
                     wsCnt = inWorkspace) {
                        if (pCD->pWsList[wsCnt].wsID !=
                            pCD->pSD->pActiveWS->id) {
                                RemoveClientFromWorkspaces(
                                    pCD, &pCD->pWsList[wsCnt].wsID, 1);
                        } else
                                inWorkspace++;
                }
        }
...
void RemoveClientFromWorkspaces(ClientData *pCD, WorkspaceID *pIDs,
                                unsigned int numIDs)
...
void AddSingleClientToWorkspaces(ClientData *pCD, WorkspaceID *pIDs,
                                 unsigned int numIDs)
        ...
        for (i = 0; i < numIDs; i++) {
                ...
                if ((pWS = GetWorkspaceData(pCD->pSD, pIDs[i])) &&
                    (!ClientInWorkspace(pWS, pCD))) {
                        ...
                        /*
                         * Update the presence property (only on transient
                         * leader)
                         */
                        UpdateWorkspacePresenceProperty(pCD);
                }
...
void AddSubtreeToWorkspaces(ClientData *pCD, WorkspaceID *pIDs,
                            unsigned int numIDs)
...
void AddClientToWorkspaces(ClientData *pCD, WorkspaceID *pIDs,
                           unsigned int numIDs)
...
void AddClientToWsList(WmWorkspaceData *pWS, ClientData *pCD)

{
        if (pWS->numClients >= pWS->sizeClientList) {
                if (pWS->sizeClientList == 0) {
                        pWS->ppClients = (ClientData **)XtMalloc(
                            WINDOW_ALLOC_AMOUNT * sizeof(ClientData *));
                } else {
                        pWS->ppClients = (ClientData **)XtRealloc(
                            (char *)pWS->ppClients,
                ...
                if (!pWS->ppClients) {
                        Warning(((char *)GETMESSAGE(
                            76, 9,
                            "Insufficient memory to add window to workspace")));
                        ExitWM(WM_ERROR_EXIT_VALUE);
                }
...
void RemoveClientFromWsList(WmWorkspaceData *pWS, ClientData *pCD)
...
Boolean F_CreateWorkspace(String args, ClientData *pCD, XEvent *event)
...
Boolean F_DeleteWorkspace(String args, ClientData *pCD, XEvent *event)
...
Boolean F_GotoWorkspace(String args, ClientData *pCD, XEvent *event)
...
Boolean F_AddToAllWorkspaces(String args, ClientData *pCD, XEvent *event)
...
Boolean F_Remove(String args, ClientData *pCD, XEvent *event)
 ...
 *  --------
 *************************************<->***********************************/
int GetCurrentWorkspaceIndex(WmScreenData *pSD) {
...
void InsureIconForWorkspace(WmWorkspaceData *pWS, ClientData *pCD)

{
        ...
        if (pCD->clientFunctions & MWM_FUNC_MINIMIZE) {
                ...
                if ((pCD->pSD->useIconBox) &&
                    (!(pCD->clientFlags &
                       (CLIENT_WM_CLIENTS | FRONT_PANEL_BOX)))) {
                        /*
                        ...
                        if ((pCD->clientState & ~UNSEEN_STATE) !=
                            MINIMIZED_STATE) {
                                ...
                                PlaceIconOnScreen(pCD, &pWsc->iconX,
                                                  &pWsc->iconY);
                        } else /* icon auto placement */
                        {
                                pWsc->iconPlace = CvtIconPositionToPlace(
                                    &pWS->IPData, pWsc->iconX, pWsc->iconY);
                                if (pWS->IPData.placeList[pWsc->iconPlace]
                                        .pCD) {
                                        /* The spot is already occupied!  Find a
                                           spot nearby. */
                                        pWsc->iconPlace = FindIconPlace(
                                            pCD, &pWS->IPData, pWsc->iconX,
                                            pWsc->iconY);

                                        if (pWsc->iconPlace == NO_ICON_PLACE) {
                                                /* Can't find a spot close by.
                                ...
                                CvtIconPlaceToPosition(
                                    &pWS->IPData, pWsc->iconPlace, &pWsc->iconX,
                                    &pWsc->iconY);
...
Boolean GetLeaderPresence(ClientData *pCD, WorkspaceID **ppIDs,
                          unsigned int *pnumIDs)

{
        ...
        if ((pLocalIDs = (WorkspaceID *)XtMalloc(
                 pCD->pSD->numWorkspaces * sizeof(WorkspaceID))) == NULL) {
                Warning(((char *)GETMESSAGE(
                    76, 10, "Insufficient Memory (GetLeaderPresence)")));
                ExitWM(WM_ERROR_EXIT_VALUE);
        }
...
Boolean GetMyOwnPresence(ClientData *pCD, WorkspaceID **ppIDs,
                         unsigned int *pnumIDs)
...
void ReserveIdListSpace(int numIDs)

{
        if (numResIDs == 0) {
                pResIDs = (WorkspaceID *)XtMalloc(numIDs * sizeof(WorkspaceID));
                if (pResIDs) {
                        numResIDs = numIDs;
                }
        } else if (numResIDs < numIDs) {
                pResIDs = (WorkspaceID *)XtRealloc(
                    (char *)pResIDs, numIDs * sizeof(WorkspaceID));
 ...
 *
 *************************************<->***********************************/
void SaveResources(WmScreenData *pSD) {
        int wsCnt;
#ifdef PANELIST
 ...
 *
 *************************************<->***********************************/
void SaveWorkspaceResources(WmWorkspaceData *pWS, unsigned long flags) {
        char *buffer = NULL;
        int bufferLength = 0;
        char *res_class;
        char *data;
        ...
        *buffer = '\0';
        ...
        if ((flags & WM_RES_INITIAL_WORKSPACE) && (!wmGD.useStandardBehavior)) {
                iLen = strlen(res_class) + strlen(screenName) +
        ...
        if ((flags & WM_RES_WORKSPACE_LIST) && (!wmGD.useStandardBehavior)) {
...
#ifdef PANELIST
        if ((flags & WM_RES_FP_POSITION) && (O_Shell(pPanelist)) &&
            (!wmGD.useStandardBehavior) &&
            (!XFindContext(DISPLAY, XtWindow(O_Shell(pPanelist)),
                           wmGD.windowContextType, (XtPointer)&pCD_Panel))) {
                ...
                if (midX <= (Position)screenWidth / 2) {
                        if (wmGD.positionIsFrame) {
                                clientX -=
                                    pCD_Panel->frameInfo.upperBorderWidth;
                        }
                        ...
                        clientX =
                            screenWidth - clientX - pCD_Panel->clientWidth;
                        if (wmGD.positionIsFrame) {
                                clientX -=
                                    pCD_Panel->frameInfo.lowerBorderWidth;
                        }
        ...
        if ((flags & WM_RES_ICONBOX_GEOMETRY) && (!wmGD.useStandardBehavior)) {
                ...
                if (wmGD.positionIsFrame) {
                        CalculateGravityOffset(pWS->pIconBox->pCD_iconBox,
                                               &xoff, &yoff);
 ...
 *
 *************************************<->***********************************/
Boolean DuplicateWorkspaceName(WmScreenData *pSD, unsigned char *name,
                               int num) {
        int i;
...
#ifdef DEBUG
int PrintWorkspaceList(pSD) WmScreenData *pSD;
{
        int i, j, k;
        WmWorkspaceData *pWS;
        ClientData *pCD;
