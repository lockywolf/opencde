...
Boolean ManageEmbeddedClient(ClientData *pCD, WmFpEmbeddedClientData *pECD,
                             long manageFlags);
Boolean IsPushRecallClient(ClientData *pCD, WmFpPushRecallClientData **ppPRCD);
static void HandleSubstructEvents(Widget w, caddr_t ptr, XEvent *event);
Boolean UpdateEmbeddedClientsProperty(WmScreenData *pSD);
...
void AdoptInitialClients(WmScreenData *pSD) {
        ...
        if (!pAncillaryWindows) {
                Warning(((char *)GETMESSAGE(
                    46, 1, "Insufficient memory to adopt initial clients")));
        ...
        *pWin1 = pSD->activeIconTextWin;
        ...
        if (XQueryTree(DISPLAY, pSD->rootWindow, &root, &parent, &clients,
                 ...
                 * window.  Icon windows will be process in
                 * SetupClientIconWindow().
                 */
                XWMHints *tmphint;

                for (i = 0; i < nclients; i++) {
                        if (clients[i]) {
                                if ((tmphint = XGetWMHints(
                                         DISPLAY, clients[i])) != NULL) {
                ...
                /* determine if the client window should be managed by wm */
#ifdef WSM
                        if (InWindowList(clients[i], pAncillaryWindows,
                                         nAncillaries)) {
                        ...
                        if ((clients[i] == XtWindow(pSD->screenTopLevelW)) ||
                            (clients[i] ==
                             XtWindow(pSD->pActiveWS->workspaceTopLevelW)) ||
...
#endif /* WSM */
                        if (!XFindContext(DISPLAY, clients[i],
                                          wmGD.windowContextType,
                                          (caddr_t *)&pcd)) {
                                /* don't manage a window we've already
                                   established a context for (e.g. icon windows)
                                 */
                                continue;
                        }
                        ...
                        /*
                         * Get the window WM_STATE property value to determine
                         * the initial window state if the wm is being
                         * restarted.
                         */
                                ...
                                if ((wmStateProp = GetWMState(clients[i])) !=
                                    NULL) {
                                        if (wmStateProp->state == IconicState) {
                                                manageFlags |=
                                                    MANAGEW_WM_RESTART_ICON;
                                        } else if (wmStateProp->state !=
                                                   NormalState) {
                        ...
                        /*
                         * Don't manage any override_redirect windows (mapped or
                         * not). Manage an unmapped window if it has a WM_STATE
                         * property and it is not Withdrawn. Manage any window
                         * that is mapped.
                         */
...
void ManageWindow(WmScreenData *pSD, Window clientWindow, long manageFlags) {
        ...
        if (wmGD.dtSD && (wmGD.dtSD == pCD->pSD)) {
                if (pCD->transientLeader && pCD->transientLeader->pECD) {
                        ...
                        (void)XFindContext(
                            DISPLAY, XtWindow(O_Shell(pPanelist)),
    ...
    if (pCD->transientLeader && pCD->transientLeader->pAccessPanel)
    {
        ...
        if ((pCD->protocolFlags & PROTOCOL_MWM_OFFSET) &&
            ...
            ((pCD->clientOffset.x != 0) || (pCD->clientOffset.y != 0))) {
                ...
                /*
                 * Make an icon for the client window if it is not a valid
                 * transient window.
                 */
        ...
        if ((pCD->clientFunctions & MWM_FUNC_MINIMIZE) &&
            (pCD->transientLeader == NULL)) {
                ...
                for (i = 0; i < pCD->numInhabited; i++) {
                        if ((pwsi = GetWorkspaceData(pCD->pSD,
                                                     pCD->pWsList[i].wsID))) {
                                ...
                                if ((pCD->pSD->useIconBox &&
                                    ...
                                    (i == 0)) {
                                        /*
                                         *   Make icon inside an icon box for
                                         * non-root case
                                         */
                                        if (!MakeIcon(pwsi, pCD)) {
                                                /*
                                                 * Error in making an icon for
                                                 * the client window; clean up
                                                 * the wm resources; do not
                                                 * manage the client window.
                                                 */
        ...
        if ((pCD->clientFunctions & MWM_FUNC_MINIMIZE) &&
            ...
            !MakeIcon(pCD->pSD->pActiveWS, pCD)) {
                /*
                 * Error in making an icon for the client window; clean up the
                 * wm resources; do not manage the client window.
                 */
...
#ifndef IBM_169380
                                AddColormapWindowReference(pCD,
                                                           pCD->cmapWindows[i]);
#else
        ...
        if (pCD->iconWindow && pCD->iconFrameWin) {
                XGrabButton(
                    DISPLAY, AnyButton, AnyModifier, pCD->iconFrameWin, True,
        ...
        if (pCD->systemMenuSpec && (pCD->systemMenuSpec->accelKeySpecs)) {
                SetupKeyBindings(pCD->systemMenuSpec->accelKeySpecs,
                                 pCD->clientFrameWin, GrabModeSync,
                                 F_CONTEXT_ALL);
#ifdef WSM
                for (i = 0; i < pCD->numInhabited; i++) {
                        if (!pCD->pWsList[i].pIconBox &&
...
#ifdef WSM
                                iconKeySpec = SetupKeyBindings(
                                    KEY_SPECS(pCD),
                                    pCD->pWsList[i].iconFrameWin, GrabModeSync,
                                    F_CONTEXT_ICON);
#else  /* WSM */
                        iconKeySpec =
                            SetupKeyBindings(KEY_SPECS(pCD), pCD->iconFrameWin,
                                             GrabModeSync, F_CONTEXT_ICON);
#endif /* WSM */
                        }

                        if ((iconAccelSpec != 0) &&
                            ACCELERATOR_MENU_COUNT(pCD)) {
                                int n;
         ...
         * the current one.
         */
        AddClientToList(GetWorkspaceData(pSD, pCD->pWsList[0].wsID), pCD,
                        True /*on top*/);
#else  /* WSM */
        ...
        /*
         * Set the keyboard input focus to the newly managed window if
         * appropriate:
            ...
            ((pCD->inputMode == MWM_INPUT_SYSTEM_MODAL) ||
             ((!wmGD.systemModalActive ||
              ...
              (wmGD.startupKeyFocus ||
               (wmGD.keyboardFocus &&
                (IS_APP_MODALIZED(wmGD.keyboardFocus)))) &&
              !(manageFlags & (MANAGEW_WM_STARTUP | MANAGEW_WM_RESTART |
                               MANAGEW_WM_CLIENTS)) &&
              (pCD->clientState != MINIMIZED_STATE) &&
#ifdef WSM
        ...
        if (smAckState == SM_START_ACK) {
                SendClientMsg(wmGD.dtSmWindow, (long)wmGD.xa_DT_SM_WM_PROTOCOL,
                              (long)wmGD.xa_DT_WM_WINDOW_ACK, CurrentTime, NULL,
                              0);
        }
...
void UnManageWindow(ClientData *pCD) {
        ...
        if (pCD->pECD) {
                ...
                XtRemoveEventHandler(
                    XtWindowToWidget(DISPLAY1, pECD->winParent),
                    (SubstructureRedirectMask | SubstructureNotifyMask), False,
                    (XtEventHandler)HandleSubstructEvents, (XtPointer)(pCD));
        ...
        if (pCD->pPRCD) {
                ...
                for (j = 0; j < pCD->pSD->numPushRecallClients; j++, pPRCD++) {
                        /*
...
void WithdrawWindow(ClientData *pCD) {
        ...
        if (!(pCD->clientFlags & CLIENT_DESTROYED)) {
                ...
                if (XCheckTypedWindowEvent(DISPLAY, pCD->clientBaseWin,
                                           DestroyNotify, &eventReturn)) {
            ...
            !(pCD->clientFlags & CLIENT_DESTROYED)) {
                ...
                if (pCD->iconWindow && (pCD->clientFlags & ICON_REPARENTED)) {
                        ...
                        XReparentWindow(DISPLAY, pCD->iconWindow,
                                        ROOT_FOR_CLIENT(pCD), ICON_X(pCD),
                                        ICON_Y(pCD));
#endif /* WSM */
                }
        }
        ...
        for (i = pCD->numInhabited - 1; i >= 0; i--) {
                TakeClientOutOfWorkspace(
                    GetWorkspaceData(pCD->pSD, pCD->pWsList[i].wsID), pCD);
        }
#endif /* WSM */
...
void DeleteClientContext(ClientData *pCD) {
        ...
        if (pCD->clientFlags & CLIENT_CONTEXT_SAVED) {
                XDeleteContext(DISPLAY, pCD->clientFrameWin,
                               wmGD.windowContextType);
                XDeleteContext(DISPLAY, pCD->clientBaseWin,
                               wmGD.windowContextType);
        ...
        /* END fix for CDExc21090 */
        {
                if (wmGD.autoKeyFocus &&
                    (wmGD.keyboardFocusPolicy == KEYBOARD_FOCUS_EXPLICIT)) {
                        ...
                        ** focus window, be sure to use wmGD.nextKeyboardFocus;
                        *otherwise
                        ** AutoResetKeyFocus chooses an essentially arbitrary
                        *window to
                        ** set focus to.
                        */
                        if (wmGD.nextKeyboardFocus == pCD)
                                ...
                                Do_Focus_Key(
                                    (ClientData *)wmGD.nextKeyboardFocus,
                                    GetTimestamp(), ALWAYS_SET_FOCUS);
                } else {
                        /*
                         * Set the focus to the default state if the focus is
                         * not in the process of being set (i.e. a FocusIn event
                         * will be comming along shortly.
                         */
...
void FreeIcon(ClientData *pCD) {
        ...
        if ((pCD->pSD->useIconBox) && pCD->pWsList[0].pIconBox) {
                 ...
                 */
                for (i = 0; i < pCD->numInhabited; i++) {
                        if ((pWsTmp = GetWorkspaceData(pCD->pSD,
                                                       pCD->pWsList[i].wsID))) {
...
void ReManageDialog(WmScreenData *pSD, Widget dialogboxW) {
        ...
        if (pSD->clientList) {
                StackWindow(pSD->pActiveWS, &pCD->clientEntry, TRUE,
...
void RegisterEmbeddedClients(Widget wPanelist, WmFpEmbeddedClientList pECD,
                             int count) {
        WmScreenData *pSD;
        int i;
...
static void ListTransientSubtree(ClientData *pCD, Window **ppWins, int *pSize,
                                 int *pCount) {
        /*
         * Check size
         */
        if (*pCount == *pSize) {
                *pSize += LTT_INCREMENT;
                *ppWins = (Window *)XtRealloc((char *)*ppWins,
                                              (*pSize * sizeof(Window)));
        }
        /*
        ...
        *pCount += 1;
        ...
        if (pCD->transientSiblings)
                ListTransientSubtree(pCD->transientSiblings, ppWins, pSize,
                                     pCount);
        ...
        if (pCD->transientChildren)
                ListTransientSubtree(pCD->transientChildren, ppWins, pSize,
                                     pCount);
...
static Window *ListTransientTree(ClientData *pCD) {
        Window *pWins;
        int count;
        int iSize;
        ...
        if (count == iSize) {
                iSize += LTT_INCREMENT;
                pWins = (Window *)XtRealloc((char *)pWins,
                                            (iSize * sizeof(Window)));
        }
        pWins[count++] = None;
...
void ScanForEmbeddedClients(WmScreenData *pSD) {
        ClientData *pCD;
...
ManageEmbeddedClient(ClientData *pCD, WmFpEmbeddedClientData *pECD,
                     long manageFlags)

{
        ...
        XReparentWindow(DISPLAY1, pCD->client, pECD->winParent, pECD->x,
                        pECD->y);
...
ReparentEmbeddedClient(WmFpEmbeddedClientData *pECD, Widget newControl,
                       Window newWin, int x, int y, unsigned int width,
                       unsigned int height)

{
        ...
        if (newWin != pECD->winParent) {
                ...
                XtAddEventHandler(
                    XtWindowToWidget(DISPLAY1, newWin),
                    (SubstructureRedirectMask | SubstructureNotifyMask), False,
                    (XtEventHandler)HandleSubstructEvents, (XtPointer)(pCD));
        }
...
static void ForceSubpanelWMState(Window win) {
        ...
        while (!bDone) {
                if (!XQueryTree(DISPLAY, win, &root, &parent, &children,
                                &numChildren)) {
                        break;
                }
                ...
                if (!XFindContext(DISPLAY, win, wmGD.windowContextType,
                                  (caddr_t *)&pCD)) {
                        /*
                         * Only continue if we're not already managing this
                         * subpanel.
                         */
                        bDone = True;
                } else if (parent == root) {
                        if ((wmStateProp = GetWMState(win))) {
                                ...
                                /*
                                 * Add a dummy WM_STATE to foil the session
                                 * manager search.
                                 */
...
void RegisterPushRecallClients(Widget wPanelist, WmFpPushRecallClientList pPRCD,
                               int count) {
        WmScreenData *pSD;
        int i;
...

{
        ...
        for (i = 0; i < pSD->numPushRecallClients && !bFoundMatch;
             i++, pPRCD++) {
                /*
...
void ScanForPushRecallClients(WmScreenData *pSD) {
        ClientData *pCD;
        ClientListEntry *pCLE;
        WmFpPushRecallClientData *pPRCD;
...
Boolean UpdateEmbeddedClientsProperty(WmScreenData *pSD) {
        ...
        SetEmbeddedClientsProperty(pSD->rootWindow, pClients, numClients);
 ...
 *
 ******************************<->***********************************/
void UnParentControls(WmScreenData *pSD, Boolean unmap) {
        int i;
        ClientData *pCD;
        WmFpEmbeddedClientData *pECD;
                                ...
                                if ((pCD->clientFlags & CLIENT_IN_SAVE_SET) &&
                                    !(pCD->clientFlags & CLIENT_DESTROYED)) {
                                        XRemoveFromSaveSet(DISPLAY,
                                                           pCD->client);
                                        XRemoveFromSaveSet(DISPLAY1,
                                                           pCD->client);
                                }

                                XReparentWindow(DISPLAY, pCD->client,
                                                pSD->rootWindow, pCD->clientX,
                                                pCD->clientY);
                                if (unmap) {
                                        XUnmapWindow(DISPLAY, pCD->client);
                                        if (pCD->iconWindow) {
