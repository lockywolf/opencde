...

{
        ...
        if (control_type == CONTROL_FILE && !control_data->is_action) {
                ...
                DtActionInvoke(control_data->icon,
                               control_data->actions[indx]->action_name, aap, 1,
                ...
                DtActionInvoke(control_data->icon,
                               ...
                               control_data->actions[indx]->count, NULL, NULL,
                               NULL, 1, NULL, NULL);
        }
}
...
void AddWorkspaceCB(Widget w, XtPointer client_data, XtPointer call_data)

{
        ...
        while (1) {
                ...
                for (i = 0; i < switch_data->switch_count; i++) {
                        if (strcmp(switch_name, switch_data->switch_names[i]) ==
                            0) {
                                good_name = False;
                                snprintf(switch_name, slen - 1, "%s_%d",
                                         temp_name, append);
                                continue;
                        }
                }
...
static void DeleteControlOkCB(Widget w, XtPointer client_data,
                              XtPointer call_data)
...
static void DeleteSubpanelOkCB(Widget w, XtPointer client_data,
                               XtPointer call_data) {
        ControlData *control_data;
        Arg args[1];
...
void DeleteControlCB(Widget w, XtPointer client_data, XtPointer call_data)

{
        ...
        if (control_data->element_values[CONTROL_LOCKED].parsed_value) {
                title = FPGETMESSAGE(
                    82, 1, "Workspace Manager - Delete Locked Control Error");
                title = XtNewString(title);

                del_msg = FPGETMESSAGE(
                    82, 5,
                ...
                title =
                    FPGETMESSAGE(82, 2, "Workspace Manager - Delete Control");
                title = XtNewString(title);
                ...
                if (SessionFileNameLookup(
                        ...
                        del_msg =
                            FPGETMESSAGE(82, 32,
                                         "If you delete this control, you "
                                         "cannot restore it using\nthe "
                                         "\"Install Icon\" drop zone.\n\nYou "
                                         "can use \"RestorePanel\" in the "
                                         "Application Manager\nto put back "
                                         "this control at a later "
                ...
                if (control_data->parent_type == SUBPANEL)
                        subpanel_data =
                            (SubpanelData *)control_data->parent_data;
                else
                        subpanel_data =
                            (SubpanelData *)control_data->subpanel_data;

                dialog = _DtMessageDialog(subpanel_data->shell, title, message,
                                          NULL, TRUE,
#else
                dialog =
                    _DtMessageDialog(panel.shell, title, message, NULL, TRUE,
#endif
                                          DeleteCancelCB, DeleteControlOkCB,
                                          NULL, NULL, False, QUESTION_DIALOG);
...
void ToggleControlCB(Widget w, XtPointer client_data, XtPointer call_data)

{
        ...
        ToggleDefaultControl(subpanel_data->parent_control_data, subpanel_data,
                             control_data);
}
...
void AddSubpanelCB(Widget w, XtPointer client_data, XtPointer call_data)
...
void DeleteSubpanelCB(Widget w, XtPointer client_data, XtPointer call_data)

{
        ...
        spanel_name =
            (char *)subpanel_data->element_values[SUBPANEL_NAME].parsed_value;

        if (subpanel_data->element_values[SUBPANEL_LOCKED].parsed_value) {
                title = FPGETMESSAGE(
                    82, 3, "Workspace Manager - Delete Locked Subpanel Error");
                title = XtNewString(title);
                ...
                title =
                    FPGETMESSAGE(82, 4, "Workspace Manager - Delete Subpanel");
                title = XtNewString(title);

                if (subpanel_data->control_data_count > 0) {
                        if (SessionFileNameLookup(
                                spanel_name, SUBPANEL,
                                (char *)subpanel_data
                                ...
                                del_msg = FPGETMESSAGE(
                                    82, 34,
                                    "If you delete this subpanel, you cannot "
                                    "restore all\nof the "
                                    "controls contained in the subpanel using "
                                    "the\n\"Install Icon\" "
                                    "drop zone.\n\nYou can use "
                                    "\"RestorePanel\" in the Application "
                                    "Manager\nto put back this subpanel and "
                                    "its controls at a later "
                                    "time.\n\nAre you sure you want to delete "
                                    "this subpanel?");
                        else
                                del_msg =
                                    FPGETMESSAGE(82, 8,
                                                 "This subpanel contains more "
                                                 "than one control.\n\nAre "
                ...
                dialog =
                    _DtMessageDialog(panel.shell, title, message, NULL, TRUE,
#endif
                                          DeleteCancelCB, DeleteSubpanelOkCB,
                                          NULL, NULL, False, QUESTION_DIALOG);
...
void PostWorkspacePopupMenu(Widget w, XtPointer client_data, XEvent *event)

{
        ...
        if (b_event->button != wmGD.bMenuButton)
                return;
...
void PostPopupMenu(Widget w, XtPointer client_data, XEvent *event)
...
static void GetBoxDataFromForm(Widget form, BoxData **box_data_rtn)
...
static void GetSubpanelDataFromForm(Widget form,
        ...
        for (i = 0; i < panel.box_data_count; i++) {
                ...
                for (j = 0; j < box_data->control_data_count; j++) {
                        ...
                        if (control_data->subpanel_data != NULL) {
                                ...
                                if (subpanel_data == *subpanel_data_rtn)
                                        continue;
...
static void KeyMenuPosition(Widget popup, Widget focus_widget)
...
static ControlData *GetMainControlData(BoxData *box_data, Widget focus_widget)
...
static ControlData *GetSubpanelControlData(SubpanelData *subpanel_data,
                                           Widget focus_widget)
...
static void SetupActionMenuItems(Widget w, ControlData *control_data,
                                 PopupData *popup_data, Widget *action_item,
                                 long *action_count)

{
        ...
        if (num_actions > *action_count) {
                ...
                for (i = 0; i < num_actions - *action_count; i++) {
                        action_item[action_index] = XmCreatePushButtonGadget(
                            w, "action_button", args, 0);

                        (*action_count)++;

                        XtManageChild(action_item[action_index]);
                        XtAddCallback(action_item[action_index],
                                      XmNactivateCallback, ActionCB,
                                      (XtPointer)action_index);
                        action_index++;
                }
        ...
        for (i = 0; i < num_actions; i++) {
                if (control_data->actions[i]->action_label != NULL)
                        ...
                        label_string = XmStringCreateLocalized(
                            control_data->actions[i]->action_name);
...
static void UpdateMainControlMenu(ControlData *control_data)

{
        ...
        if (control_data->subpanel_data != NULL &&
            control_data != control_data->subpanel_data->default_control) {
                ...
                control_data->element_values =
                    control_data->subpanel_data->default_control
                        ->element_values;
        }
        ...
        if (control_label == NULL)
                control_label =
                    (char *)control_data->element_values[CONTROL_NAME]
                        .parsed_value;
        ...
        if (control_type != CONTROL_BLANK) {
                ...
                if (control_data->subpanel_data == NULL) {
                        if (panel.popup_data->subpanel_add_state != True) {
                                ...
                                XtRemoveCallback(
                                    panel.popup_data->modify_subpanel_item,
                                    XmNactivateCallback, DeleteSubpanelCB,
                                    NULL);
                                XtAddCallback(
                                    panel.popup_data->modify_subpanel_item,
                        ...
                        if (panel.popup_data->subpanel_add_state == True) {
                                label_string = XmStringCreateLocalized(
                                    (FPGETMESSAGE(82, 12, "Delete Subpanel")));
                                XtSetArg(args[0], XmNlabelString, label_string);
...
static void UpdateSubpanelControlMenu(SubpanelData *subpanel_data,
                                      ControlData *control_data)

{
        ...
        if (control_data == NULL) {
                ...
                XtRemoveAllCallbacks(panel.popup_data->help_item,
                                     XmNactivateCallback);
                ...
                XtAddCallback(panel.popup_data->deinstall_item,
                              XmNactivateCallback, DeleteControlCB,
                              (XtPointer)NULL);
                ...
                XtAddCallback(panel.popup_data->toggle_item,
                              XmNactivateCallback, ToggleControlCB,
                              (XtPointer)subpanel_data);
        ...
        if (!(subpanel_data->parent_control_data->element_values[CONTROL_LOCKED]
                  .parsed_value))
...

{
        ...
        if (event->type == ButtonPress) {
                ...
                XmMenuPosition(panel.popup_data->popup,
...

{
        ...
        popup_data = panel.popup_data =
            (PopupData *)XtMalloc(sizeof(PopupData));

        i = 0;
        XtSetArg(args[i], XmNwhichButton, wmGD.bMenuButton);
        i++;
        ...
        children[child_num++] =
            XmCreateSeparatorGadget(popup_data->popup, "sep1", NULL, 0);
        children[child_num++] =
            XmCreateSeparatorGadget(popup_data->popup, "sep2", NULL, 0);
        ...
        for (i = 0; i < 5; i++) {
                children[child_num] = popup_data->action_item[i] =
                    XmCreatePushButtonGadget(popup_data->popup, "action_button",
                                             args, 0);
        ...
        label_string = XmStringCreateLocalized(
            (FPGETMESSAGE(82, 14, "Copy To Main Panel")));
        XtSetArg(args[0], XmNlabelString, label_string);
        mnemonic = ((char *)FPGETMESSAGE(82, 15, "C"));
        XtSetArg(args[1], XmNmnemonic, mnemonic[0]);
        popup_data->toggle_item = children[child_num] =
            XmCreatePushButtonGadget(popup_data->popup, "Toggle_Control", args,
                                     2);
        XmStringFree(label_string);

        label_string =
            XmStringCreateLocalized((FPGETMESSAGE(82, 16, "Delete")));
        XtSetArg(args[0], XmNlabelString, label_string);
        mnemonic = ((char *)FPGETMESSAGE(82, 17, "D"));
        XtSetArg(args[1], XmNmnemonic, mnemonic[0]);
        popup_data->deinstall_item = children[child_num] =
            XmCreatePushButtonGadget(popup_data->popup, "Delete_Control", args,
                                     2);
        XmStringFree(label_string);
        ...
        popup_data->modify_subpanel_item = children[child_num] =
            XmCreatePushButtonGadget(popup_data->popup, "Add_Subpanel", args,
                                     2);
        XmStringFree(label_string);
...
static void CreateActionButtons(Widget w, SwitchData *switch_data,
                                ControlData *control_data)
...
static void ShowWorkspaceItems(SwitchData *switch_data, Widget focus_widget)

{
        ...
        for (i = 0;
             i < switch_data->switch_count && switch_data->buttons[i] != NULL;
             i++) {
                if (switch_data->buttons[i] == focus_widget)
                        break;
        }
...
static void ShowPartialWorkspaceItems(SwitchData *switch_data)

{
        ...
        label_string =
            XmStringCreateLocalized((FPGETMESSAGE(82, 22, "Switch Area")));
        XtSetArg(args[0], XmNlabelString, label_string);
        XtSetValues(popup_data->popup_title, args, 1);
        XmStringFree(label_string);
...
static void GetSwitchDataFromForm(Widget form, SwitchData **switch_data_rtn)
...
static ControlData *GetSwitchControlData(SwitchData *switch_data,
                                         Widget focus_widget)
...
static void WSPopupMenu(Widget w, XtPointer client_data, XtPointer call_data)

{
        ...
        if (event->type == ButtonPress) {
                ...
                if (focus_widget == NULL) {
                        ...
                        focus_widget = (Widget)XmObjectAtPoint(
                            switch_data->rc, event->xbutton.x - x,
                            event->xbutton.y - y);
                        is_button = True;
                } else
                        control_data =
        ...
        if (focus_widget != NULL) {
                if (is_button)
                        ...
                        if (control_data != NULL) {
                                ...
                                if (control_type != CONTROL_BLANK)
                                        /* The mouse is over a control in the
                                         * switch area */
                                        CreateActionButtons(w, switch_data,
                                                            control_data);
                                else
                                        /* The mouse is over a blank control in
                                         * the switch area */
                                        ShowPartialWorkspaceItems(switch_data);
                        } else {
                                /*
                                 * The mouse is over area other than the buttons
                                 * and control in the switch area
                                 */
                                ShowPartialWorkspaceItems(switch_data);
                        }
                }
...
void CreateWorkspacePopupMenu(Widget w, SwitchData *switch_data)

{
        ...
        popup_data->add_ws_item = children[child_num] =
            XmCreatePushButtonGadget(popup_data->popup, "Add_Workspace", args,
                                     2);
        XmStringFree(label_string);

        XtAddCallback(children[child_num++], XmNactivateCallback,
                      AddWorkspaceCB, (XtPointer)NULL);

        label_string =
            XmStringCreateLocalized((FPGETMESSAGE(82, 25, "Delete")));
        XtSetArg(args[0], XmNlabelString, label_string);
        mnemonic = ((char *)FPGETMESSAGE(82, 26, "D"));
        XtSetArg(args[1], XmNmnemonic, mnemonic[0]);
        popup_data->delete_ws_item = children[child_num] =
            XmCreatePushButtonGadget(popup_data->popup, "Delete_Workspace",
                                     args, 2);
        XmStringFree(label_string);
        ...
        label_string =
            XmStringCreateLocalized((FPGETMESSAGE(82, 27, "Rename")));
        XtSetArg(args[0], XmNlabelString, label_string);
        mnemonic = ((char *)FPGETMESSAGE(82, 28, "R"));
        XtSetArg(args[1], XmNmnemonic, mnemonic[0]);
        popup_data->rename_ws_item = children[child_num] =
            XmCreatePushButtonGadget(popup_data->popup, "Rename_Workspace",
                                     args, 2);
        XmStringFree(label_string);
