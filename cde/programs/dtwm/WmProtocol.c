...
#ifndef NO_WMQUERY
    static Boolean
    wmq_convert(Widget w, Atom *pSelection, Atom *pTarget, Atom *pType_return,
                XtPointer *pValue_return, unsigned long *pLength_return,
                int *pFormat_return);
static Boolean wmq_convert_all_clients(Widget w, int screen, Atom *pType_return,
...
static Boolean WMiConvert(Widget, Atom, Atom, XtPointer, unsigned long, int,
...
void SetupWmICCC(void) {
        enum {
                ...
                XA_MOTIF_WM_CLIENT_WINDOW,
                XA_MOTIF_WM_POINTER_WINDOW,
                XA_MOTIF_WM_ALL_CLIENTS,
#endif
#if ((!defined(WSM)) || defined(MWM_QATS_PROTOCOL))
                XA_MOTIF_WM_DEFINE_COMMAND,
                XA_MOTIF_WM_INCLUDE_COMMAND,
                XA_MOTIF_WM_REMOVE_COMMAND,
                XA_MOTIF_WM_ENABLE_COMMAND,
                XA_MOTIF_WM_DISABLE_COMMAND,
                XA_MOTIF_WM_RENAME_COMMAND,
                XA_MOTIF_WM_INVOKE_COMMAND,
                XA_MOTIF_WM_REQUEST_COMMAND,
        ...
        static char *atom_names[] = {
                ...
                _XA_MOTIF_WM_CLIENT_WINDOW,
                _XA_MOTIF_WM_POINTER_WINDOW,
                _XA_MOTIF_WM_ALL_CLIENTS,
#else
                "_MOTIF_WM_CLIENT_WINDOW",
                "_MOTIF_WM_POINTER_WINDOW",
                "_MOTIF_WM_ALL_CLIENTS"
#endif
#endif
#if ((!defined(WSM)) || defined(MWM_QATS_PROTOCOL))
                _XA_MOTIF_WM_DEFINE_COMMAND,
                _XA_MOTIF_WM_INCLUDE_COMMAND,
                _XA_MOTIF_WM_REMOVE_COMMAND,
                _XA_MOTIF_WM_ENABLE_COMMAND,
                _XA_MOTIF_WM_DISABLE_COMMAND,
                _XA_MOTIF_WM_RENAME_COMMAND,
                _XA_MOTIF_WM_INVOKE_COMMAND,
                _XA_MOTIF_WM_REQUEST_COMMAND,
...
#ifndef NO_WMQUERY
        if (!(xa_WM_QUERY =
                  (Atom *)XtMalloc(wmGD.numScreens * (sizeof(Atom))))) {
        ...
        for (scr = 0; scr < wmGD.numScreens; scr++) {
                if (wmGD.Screens[scr].managed) {
                        sizeList.min_width =
                            wmGD.Screens[scr].iconImageMinimum.width;
                        sizeList.min_height =
                            wmGD.Screens[scr].iconImageMinimum.height;
                        sizeList.max_width =
                            wmGD.Screens[scr].iconImageMaximum.width;
                        sizeList.max_height =
                            wmGD.Screens[scr].iconImageMaximum.height;
        ...
        for (scr = 0; scr < wmGD.numScreens; scr++) {
                if (wmGD.Screens[scr].managed) {
                        if (!XtOwnSelection(wmGD.topLevelW, xa_WM_QUERY[scr],
                                            GetTimestamp(), wmq_convert,
                                            wmq_lose, wmq_done)) {
                                Warning(((char *)GETMESSAGE(56, 3,
                                                            "Failed to own "
                                                            "_MOTIF_WM_QUERY_"
                                                            "nn selection")));
                        }
...
void SendConfigureNotify(ClientData *pCD) {
        ...
        if (pCD->pECD) {
                ...
                WmFpEmbeddedClientData *pECD =
                    (WmFpEmbeddedClientData *)pCD->pECD;
                ...
                XTranslateCoordinates(DISPLAY, pECD->winParent,
                                      ROOT_FOR_CLIENT(pCD), pCD->clientX,
                                      pCD->clientY, &rootX, &rootY, &wChild);
...
void SendClientOffsetMessage(ClientData *pCD) {
        ...
        case EastGravity: {
                clientMsgEvent.data.l[1] = -(offsetX - (2 * borderWidth));
                clientMsgEvent.data.l[2] =
                    borderWidth + (offsetY - offsetX) / 2;
                break;
        }
...
void SendClientMsg(Window window, long type, long data0, Time time, long *pData,
                   int dataLen) {
        XClientMessageEvent clientMsgEvent;
        int i;
...
Boolean AddWmTimer(unsigned int timerType, unsigned long timerInterval,
                   ClientData *pCD) {
        WmTimer *pWmTimer;

        if (!(pWmTimer = (WmTimer *)XtMalloc(sizeof(WmTimer)))) {
                Warning(((char *)GETMESSAGE(
                    56, 1, "Insufficient memory for window manager data")));
                return (False);
        }

        /* !!! handle for XtAppAddTimeOut error !!! */
        pWmTimer->timerId =
            XtAppAddTimeOut(wmGD.mwmAppContext, timerInterval,
                            (XtTimerCallbackProc)TimeoutProc, (caddr_t)pCD);
                ...
                case TIMER_RAISE: {
                        ...
                        if ((wmGD.keyboardFocus == pWmTimer->timerCD) &&
                            (pWmTimer->timerCD->focusPriority ==
                             (PSD_FOR_CLIENT(pWmTimer->timerCD))
                                 ->focusPriority) &&
                            (wmGD.keyboardFocusPolicy ==
                             KEYBOARD_FOCUS_POINTER) &&
                            (pWmTimer->timerCD ==
                             GetClientUnderPointer(&sameScreen))) {
                                Do_Raise(pWmTimer->timerCD,
                                         (ClientListEntry *)NULL, STACK_NORMAL);
                        }
                        break;
                }
                }
...
static Boolean wmq_convert(Widget w, Atom *pSelection, Atom *pTarget,
                           Atom *pType_return, XtPointer *pValue_return,
                           unsigned long *pLength_return, int *pFormat_return) {
        ...
        if (wm_query_found) {
                if (*pTarget == xa_WM_POINTER_WINDOW) {
                        ...
                        return (wmq_convert_all_clients(
                            w, scr, pType_return, pValue_return, pLength_return,
                            pFormat_return));
                }
        }
...
static void wmq_lose(Widget w, Atom *pSelection) {
        Warning(
            ((char *)GETMESSAGE(56, 4, "Lost _MOTIF_WM_QUERY_nn selection")));
...
static void wmq_bump_xids(void) {
        ...
        if (pXids) {
                if (!(px = (XID *)XtRealloc((char *)pXids,
                                            (numXids + 32) * (sizeof(XID))))) {
                        Warning(((char *)GETMESSAGE(
                            56, 5,
                            "Insufficient memory to convert _MOTIF_WM_QUERY_nn "
                            "selection")));
                } else {
                        pXids = px;
                        numXids += 32;
                ...
                if (!(pXids = (XID *)XtMalloc(32 * (sizeof(XID))))) {
                        Warning(((char *)GETMESSAGE(
                            56, 5,
                            "Insufficient memory to convert _MOTIF_WM_QUERY_nn "
                            "selection")));
...
#ifdef MWM_WSM
                        /*
                         * This registers the callback to be invoked when a
                         * request is made against a WSM Protocol target.  The
                         * request callback is stored by the WSM Protocol code
                         * and is invoked in the convert routine (WMiConvert)
                         * below. See WSMProcessProtoTargets().
                         */

                        WSMRegisterRequestCallback(
                            DISPLAY, scr, HandleWsmConvertRequest, NULL);
#endif

                        /*
                         * Own the selection through UTM.  This sets-up a
                         * convert function that is invoked when a convert
                         * request is made on this selection. The convert
                         * function is specified in the drawing area's
                         * XmNconvertCallback resource.
                         */

                        XtAddCallback(wmGD.Screens[scr].utmShell,
                                      XmNconvertCallback, WMiConvertCB, NULL);

                        if (!XmeNamedSource(wmGD.Screens[scr].utmShell,
                                            wmGD.xa_WM[scr], timestamp)) {
                                Warning(((char *)GETMESSAGE(
                                    56, 6, "Failed to own WM_nn selection")));
                        } else {
...
/*ARGSUSED*/
static Boolean WMiConvert(Widget w, Atom selection, Atom target,
                          XtPointer input, unsigned long inputLen, int inputFmt,
                          Atom *outputType, XtPointer *output,
                          unsigned long *outputLen, int *outputFmt) {
        int scr;
        Boolean found = False;
        ...
        *outputFmt = 8;
        ...
        if (!wmGD.Screens[scr].managed) {
                Warning(((char *)GETMESSAGE(
                    56, 7, "Got convert request from unmanaged screen")));
                found = False;
        }
                ...
                if (target == wmGD.xa_TARGETS) {
                        Atom *targs =
                            (Atom *)XtMalloc((unsigned)(28 * sizeof(Atom)));
                        int targetCount = 0;

                        *output = (XtPointer)targs;

                        /* required targets */
                        *targs++ = wmGD.xa_TARGETS;
                        targetCount++;
                        *targs++ = wmGD.xa_MULTIPLE;
                        targetCount++;
                        *targs++ = wmGD.xa_TIMESTAMP;
                        targetCount++;

#ifdef MWM_WSM
                        /* other targets */
                        *targs++ = _WSMReqTypeToTarget(DISPLAY, WSM_CONNECT);
                        targetCount++;
                        *targs++ = _WSMReqTypeToTarget(DISPLAY, WSM_EXTENSIONS);
                        targetCount++;
                        *targs++ = _WSMReqTypeToTarget(DISPLAY, WSM_CONFIG_FMT);
                        targetCount++;
                        *targs++ = _WSMReqTypeToTarget(DISPLAY, WSM_GET_STATE);
                        targetCount++;
                        *targs++ = _WSMReqTypeToTarget(DISPLAY, WSM_SET_STATE);
                        targetCount++;
                        *targs++ = _WSMReqTypeToTarget(DISPLAY, WSM_REG_WINDOW);
                        targetCount++;
#endif

                        /* menu command interface support */
                        *targs++ = wmGD._MOTIF_WM_DEFINE_COMMAND;
                        targetCount++;
                        *targs++ = wmGD._MOTIF_WM_INCLUDE_COMMAND;
                        targetCount++;
                        *targs++ = wmGD._MOTIF_WM_REMOVE_COMMAND;
                        targetCount++;
                        *targs++ = wmGD._MOTIF_WM_ENABLE_COMMAND;
                        targetCount++;
                        *targs++ = wmGD._MOTIF_WM_DISABLE_COMMAND;
                        targetCount++;
                        *targs++ = wmGD._MOTIF_WM_RENAME_COMMAND;
                        targetCount++;
                        *targs++ = wmGD._MOTIF_WM_INVOKE_COMMAND;
                        targetCount++;
                        *targs++ = wmGD._MOTIF_WM_REQUEST_COMMAND;
                        targetCount++;
                        *targs++ = wmGD._MOTIF_WM_WINDOW_FLAGS;
                        targetCount++;

#ifdef MWM_WSM
                        /* virtual screen support */
                        *targs++ = wmGD._MOTIF_WM_PAN;
                        targetCount++;
                        *targs++ = wmGD._MOTIF_WM_GOTO;
                        targetCount++;
#endif

                        /* automation support */

                        *targs++ = wmGD._MOTIF_WM_AUTOMATION;
                        targetCount++;
                ...
                /* virtual screen support */
                        ...
                        PanRoot(x - ACTIVE_PSD->panDx, y - ACTIVE_PSD->panDy,
                                1);
                 ...
                 */
                        ...
                        found = WSMProcessProtoTarget(
                            w, target, input, inputLen, inputFmt, outputType,
                            output, outputLen, outputFmt);
                }
#endif /* MWM_WSM */
                 ...
                 */
                        ...
                        PRINT("Convert request made for "
                              "_MOTIF_WM_DEFINE_COMMAND.\n");
                        DefineCommand(w, target, (MessageData)input, inputLen,
                                      inputFmt);
                        ...
                        IncludeCommand(w, target, (MessageData)input, inputLen,
                                       inputFmt);
                        ...
                        RemoveCommand(w, target, (MessageData)input, inputLen,
                                      inputFmt);
                        ...
                        EnableCommand(w, target, (MessageData)input, inputLen,
                                      inputFmt);
                        ...
                        DisableCommand(w, target, (MessageData)input, inputLen,
                                       inputFmt);
                        ...
                        /*
                         * This function needs to pack the necessary info into
                         * the output data variable to send back to the
                         * requesting
                         */

                        GetAutomationData(input, outputType, output, outputLen,
                                          outputFmt);
                        found = True;
                }
                        ...
                        Warning(
                            ((char *)GETMESSAGE(56, 8,
                                                "Conversion request made for "
                                                "unknown target type")));
        }
...
/*ARGSUSED*/
static void WMiConvertCB(Widget w, XtPointer clientData, XtPointer callData) {
         ...
         */
                ...
                WMiConvert(w, cnv->selection, cnv->target, cnv->parm,
                           cnv->parm_length, cnv->parm_format, &(cnv->type),
                           &(cnv->value), &(cnv->length), &(cnv->format));
        }
}
#endif /* !defined(WSM) || defined(MWM_QATS_PROTOCOL) */
