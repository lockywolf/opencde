...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmImage.c /main/7 1996/11/14 13:50:30 rswiston $"
#endif
#endif
/*
...
Pixmap MakeClientIconPixmap(ClientData *pCD, Pixmap iconBitmap,
        ...
        if (!XGetGeometry(DISPLAY, iconBitmap, &root, &x, &y, &bitmapWidth,
                          &bitmapHeight, &border, &depth)) {
         ...
         */
                ...
                Warning(((char *)GETMESSAGE(
                    38, 9, "Icon bitmap cannot be used on this screen")));
#else  /* WSM */
        ...
        if (depth != 1) {
                Warning(((char *)GETMESSAGE(
                    38, 10, "Color icon pixmap not supported")));
                return ((Pixmap)NULL);
        }
#endif
        ...
        return (MakeIconPixmap(pCD, iconBitmap, (Pixmap)iconMask, bitmapWidth,
                               bitmapHeight, depth));
...
#ifdef WSM
        if ((bitmapIndex =
                 GetBitmapIndex(PSD_FOR_CLIENT(pCD), iconName, False)) < 0)
#else  /* WSM */
        if ((bitmapIndex = GetBitmapIndex(PSD_FOR_CLIENT(pCD), iconName)) < 0)
#endif /* WSM */
        {
#ifdef WSM
                if ((PSD_FOR_CLIENT(pCD)->displayResolutionType ==
                     LOW_RES_DISPLAY) ||
                    (PSD_FOR_CLIENT(pCD)->displayResolutionType ==
                     VGA_RES_DISPLAY)) {
                ...
                sIconFileName = XmGetIconFileName(
                    XtScreen(PSD_FOR_CLIENT(pCD)->screenTopLevelW1), NULL,
                    iconName, NULL, iconSizeDesired);

                if (sIconFileName != NULL) {
                        pixmap = XmGetPixmap(
                            XtScreen(PSD_FOR_CLIENT(pCD)->screenTopLevelW1),
                            sIconFileName, pCD->iconImageForeground,
                ...
                if (pixmap == XmUNSPECIFIED_PIXMAP) {
                        pixmap = XmGetPixmap(
                            XtScreen(PSD_FOR_CLIENT(pCD)->screenTopLevelW1),
                            iconName, pCD->iconImageForeground,
                            pCD->iconImageBackground);
                }

                if (pixmap == XmUNSPECIFIED_PIXMAP) {
                        MWarning(((char *)GETMESSAGE(
                                     38, 7, "Unable to read bitmap file %s\n")),
                                 iconName);
                } else {
                        mask = XmeGetMask(
                            XtScreen(PSD_FOR_CLIENT(pCD)->screenTopLevelW1),
                        ...
                        /*
                         * We need to synchronize DISPLAY1 here because the
                         * pixmap and mask were created on that display
                         * connection, and we must ensure that the server has
                         * processed those requests before we attempt to use the
                         * pixmap and mask on DISPLAY.  We thus assume that the
                         * subsequent XGetGeometry() call is successful.
                         */
                        XSync(DISPLAY1, False);

                        (void)XGetGeometry(DISPLAY, pixmap, &root, &x, &y,
                                           &width, &height, &border_width,
                                           &depth);

                        pixmap_r = MakeIconPixmap(pCD, pixmap, mask, width,
                                                  height, depth);
                        ...
                        if (mask)
                                XmDestroyPixmap(
                                    XtScreen(
                                        PSD_FOR_CLIENT(pCD)->screenTopLevelW1),
                                    mask);
...
Pixmap MakeCachedIconPixmap(ClientData *pCD, int bitmapIndex, Pixmap mask) {
        ...
        /*
         * If a matching pixmap was not found in the pixmap cache for this
         * bitmap then create the icon pixmap with the appropriate colors. If
         * have sufficient memory, save the pixmap info in the pixmapCache.
         */

        if (!pixmap &&
            (pixmap = MakeIconPixmap(pCD, bitmapc->bitmap, mask, bitmapc->width,
                                     bitmapc->height, 1)) &&
...
Pixmap MakeIconPixmap(ClientData *pCD, Pixmap bitmap, Pixmap mask,
        ...
        if ((top_rects == NULL) &&
            (top_rects = AllocateRList((unsigned)2 *
                                       ICON_INTERNAL_SHADOW_WIDTH)) == NULL) {
                /* Out of memory! */
                Warning(((char *)GETMESSAGE(
                    38, 3, "Insufficient memory to bevel icon image")));
                return ((Pixmap)NULL);
        }

        if ((bot_rects == NULL) &&
            (bot_rects = AllocateRList((unsigned)2 *
                                       ICON_INTERNAL_SHADOW_WIDTH)) == NULL) {
                /* Out of memory! */
                Warning(((char *)GETMESSAGE(
                    38, 4, "Insufficient memory to bevel icon image")));
                return ((Pixmap)NULL);
        }
        ...
        iconPixmap =
            XCreatePixmap(DISPLAY, pSD->rootWindow, imageWidth, imageHeight,
        ...
        imageGC =
            XCreateGC(DISPLAY, iconPixmap, (GCForeground | GCBackground), &gcv);
#endif /* WSM */
        ...
        XFillRectangle(DISPLAY, iconPixmap, imageGC, 0, 0, imageWidth,
                       imageHeight);
        ...
        if ((depth > 1) && (depth == DefaultDepth(DISPLAY, pSD->screen))) {
#ifndef NO_CLIP_CENTER
                XCopyArea(DISPLAY, bitmap, iconPixmap, imageGC, src_x, src_y,
                          width, height, dest_x, dest_y);
#else  /* NO_CLIP_CENTER */
                XCopyArea(DISPLAY, bitmap, iconPixmap, imageGC, 0, 0, width,
                          height, dest_x, dest_y);
#endif /* NO_CLIP_CENTER */
        } else
#endif /* DISALLOW_DEEP_ICONS */
#ifndef NO_CLIP_CENTER
                XCopyPlane(DISPLAY, bitmap, iconPixmap, imageGC, src_x, src_y,
         ...
         */
                ...
                if (mask && (pPS != NULL)) {
                        ...
                        botGC =
                            GetHighlightGC(pSD, pPS->bs, pPS->bg,
                                           pCD->iconImageBottomShadowPixmap);
                } else {
#endif /* WSM */
                        topGC =
                            GetHighlightGC(pSD, pCD->iconImageTopShadowColor,
                                           pCD->iconImageBackground,
                                           pCD->iconImageTopShadowPixmap);

                        botGC =
                            GetHighlightGC(pSD, pCD->iconImageBottomShadowColor,
                                           pCD->iconImageBackground,
                                           pCD->iconImageBottomShadowPixmap);
#ifdef WSM
                }
                ...
                BevelRectangle(
                    top_rects, bot_rects, 0, 0, imageWidth, imageHeight,
...
Pixmap MakeCachedLabelPixmap(WmScreenData *pSD, Widget menuW, int bitmapIndex) {
        ...
        i++;
        XtSetArg(args[i], XtNbackground, &bg);
        i++;
         ...
         */
        {
                 ...
                 * Only BadAlloc error possible; BadDrawable and BadValue are
                 * avoided.
                 */

                pixmap = XCreatePixmap(DISPLAY, pSD->rootWindow, bitmapc->width,
                                       bitmapc->height,
                                       DefaultDepth(DISPLAY, pSD->screen));

                /*
                 * Create a GC and copy the bitmap to the pixmap.
                 * Only BadAlloc and BadDrawable errors are possible; others are
                 * avoided
                 */
                ...
                /*
                 * Fill in the background, set the foreground, copy the bitmap
                 * to the pixmap, and free the gc.
                 */

                XFillRectangle(DISPLAY, pixmap, gc, 0, 0, bitmapc->width,
                               bitmapc->height);
                ...
                /*
                 * If have sufficient memory, save the pixmap info in the
                 * pixmapCache.
                 */
...
#endif /* WSM */
{
        ...
        for (n = 0, bitmapc = pSD->bitmapCache; n < pSD->bitmapCacheCount;
             n++, bitmapc++) {
                if ((!path && !bitmapc->path) ||
                    (path && bitmapc->path && !strcmp(path, bitmapc->path))) {
                        return (n);
                }
        }
        ...
        /* create */
        {
                ...
                pSD->bitmapCache = (BitmapCache *)XtMalloc(BITMAP_CACHE_INC *
                                                           sizeof(BitmapCache));
        } else if (pSD->bitmapCacheCount == pSD->bitmapCacheSize)
        /* enlarge */
        {
                pSD->bitmapCacheSize += BITMAP_CACHE_INC;
                pSD->bitmapCache = (BitmapCache *)XtRealloc(
                    (char *)pSD->bitmapCache,
                    pSD->bitmapCacheSize * sizeof(BitmapCache));
        }

        if (pSD->bitmapCache == NULL) {
                MWarning(((char *)GETMESSAGE(
                             38, 5, "Insufficient memory for bitmap %s\n")),
                         name);
...
char *BitmapPathName(string) char *string;

{
         ...
         * If the bitmapDirectory resource is nonNULL, interpret path as
         * relative to it. Else, or if bitmapDirectory has no such file, use a
         * XBMLANGPATH lookup.
         */
