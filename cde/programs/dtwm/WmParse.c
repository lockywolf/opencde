...
static char rcsid[] =
    "$XConsortium: WmParse.c /main/5 1996/10/30 11:36:06 drk $";
...
static DtWmpParseBuf *_DtWmpIncBuf(DtWmpParseBuf *pWmPB);
...
unsigned char *_DtWmParseNextToken(DtWmpParseBuf *pWmPB) {
...
unsigned char *_DtWmParseNextTokenExpand(DtWmpParseBuf *pWmPB) {
        unsigned char *pch;
...
unsigned char *_DtWmParseBackUp(DtWmpParseBuf *pWmPB, unsigned char *pchTok) {
            ...
            (pchTok < (pWmPB->pchLine + pWmPB->cLineSize))) {
                ...
                while ((pch < pchTok)) {
                        ...
                        if (*pch == '\0') {
                                ...
                                if (pch == pchTok) {
                                        /*
                                         * Found the NULL preceding the string
                                         * passed in! Replace it with a blank
                                         * and return the previous token
                                         * (pointed to by pchLast).
                                         */
...
unsigned char *_DtWmParseNextTokenC(unsigned char **linePP,
                                    Boolean SmBehavior) {
        /***********************************************************************
         *
...
DtWmpParseBuf *_DtWmParseNewBuf(void)
...

{
        if (pWmPB) {
                ...
                pWmPB->pchLine = (unsigned char *)XtRealloc(
                    (char *)pWmPB->pchLine, (pWmPB->cLineSize + MAXLINE));
                if (pWmPB->pchLine) {
...
void _DtWmParseDestroyBuf(DtWmpParseBuf *pWmPB)
...
void _DtWmParseSetLine(DtWmpParseBuf *pWmPB, unsigned char *pch)
...
void _DtWmParseSetFile(DtWmpParseBuf *pWmPB, FILE *pFile)
...
unsigned char *_DtWmParseNextLine(DtWmpParseBuf *pWmPB)
        ...
        /* read fopened file */
        {
                if ((string = (unsigned char *)fgets((char *)line, MAXLINE,
                        ...
                        if (strlen((char *)string) > (size_t)pWmPB->cLineSize) {
                                ...
                                pWmPB->pchLine = (unsigned char *)XtRealloc(
                                    (char *)pWmPB->pchLine, (pWmPB->cLineSize));
                        }
#endif /* PARSE_LIB */
#ifndef NO_MULTIBYTE
...
unsigned char *_DtWmParseCurrentChar(DtWmpParseBuf *pWmPB)
...
unsigned char *_DtWmParseNextChar(DtWmpParseBuf *pWmPB)
...
int _DtWmParseLineNumber(DtWmpParseBuf *pWmPB)
 ...
 *
 *************************************<->***********************************/
unsigned char *_DtWmParseFilenameExpand(unsigned char *pchFilename)

{
        ...
        int len, n, nx, ix;
        unsigned char pchBrk[] = {DTWM_CHAR_ENVIRONMENT, DTWM_CHAR_DIRECTORY,
                                  '\0'};
        ...
        while (pchNew && pchO && *pchO) {
                ...
                pchEnv0 = (unsigned char *)strchr((char *)pchO,
                                                  (int)DTWM_CHAR_ENVIRONMENT);
                if (pchEnv0) {
                         ...
                         *   end of string
                         */
                        pchEnv1 = (unsigned char *)strpbrk((char *)pchEnv0,
                                                           (char *)pchBrk);
                        ...
                        if (pchEnv1) {
                                ...
                                *pchEnv1 = '\0';
                                ...
                                /* This environment variable is the last thing
                                 * on the line. Signal all done.
                                 */
                                n = strlen((char *)pchO);
                                pchO += n;
                        }
                        ...
                        if (pchEnv) {
                                ...
                                if (nx > n) {
                                        len += nx - n;
                                        pchNew = (unsigned char *)XtRealloc(
                                            (char *)pchNew, 1 + len);
                                }
                                if (pchNew) {
 ...
 *
 *************************************<->***********************************/
unsigned char *_DtWmParseExpandEnvironmentVariables(unsigned char *pch,
...

{
        ...
        int lenReturn;
        int lenSave;
        static unsigned char pchDefaultBrk[] = {DTWM_CHAR_ENVIRONMENT,
                                                DTWM_CHAR_SPACE,
                                                DTWM_CHAR_TAB,
                                                DTWM_CHAR_NEW_LINE,
                                                DTWM_CHAR_BACKSLASH,
                                                DTWM_CHAR_DIRECTORY,
                                                '\0'};
        unsigned char pchParenBrk[] = {DTWM_CHAR_R_PAREN, '\0'};
        unsigned char pchBraceBrk[] = {DTWM_CHAR_R_BRACE, '\0'};
                ...
                if (chlen == 1) {
                        ...
                        case DTWM_CHAR_BACKSLASH:
                                ...
                                if (!pchReturn) {
                                        ...
                                        pchReturn = (unsigned char *)XtMalloc(
                                ...
                                *pch = chSave;
                                ...
                                /*
                                 * The next character is "escaped", skip over
                                 * it.
                                 */
                                pchStart = pch += chlen;
#ifdef MULTIBYTE
                        ...
                        case DTWM_CHAR_ENVIRONMENT:
                                ...
                                memcpy(&pchReturn[lenSave], pchStart,
                                       lenNonEnv);
                                 ...
                                 */
                                pchNext = (unsigned char *)strpbrk(
                                    (char *)pch, (char *)pchBreak);
                                ...
                                if (!pchNext) {
                                        ...
                                        /* temporarily put a string terminator
                                         * here */
                                        chSave = *pchNext;
                                        *pchNext = '\0';
                                }
                                ...
                                pchEnvValue =
                                    (unsigned char *)getenv((char *)pch);

                                if (pchEnvValue) {
                                        /*
                                         * Insure there's enough room in the
                                         * return string
                                         */
                                        lenEnvValue =
                                            strlen((char *)pchEnvValue);
                                        if (!pchReturn) {
                                                lenReturn = lenOriginal + 1 -
                                                            lenEnvVar +
                                                            lenEnvValue;
                                                pchReturn =
                                                    (unsigned char *)XtMalloc(
                                                        lenReturn *
                                                        sizeof(unsigned char));
                                                pchReturn[0] = '\0';
                                        } else {
                                                lenReturn = lenReturn + 1 -
                                                            lenEnvVar +
                                                            lenEnvValue;
                                                pchReturn =
                                ...
                                if (chSave) {
                                        ...
                                        *pchNext = chSave;
                                        ...
                                        /*
                                         *  If this was a closing paren, then
                                         * skip it
                                         */
                                        if (bEatBreak) {
#ifdef MULTIBYTE
                                                chlen = mblen((char *)pchNext,
                                                              MB_CUR_MAX);
#endif /* MULTIBYTE */
                                                pchNext += chlen;
                                        }
...
unsigned char *_DtWmParseMakeQuotedString(unsigned char *pchLine) {
