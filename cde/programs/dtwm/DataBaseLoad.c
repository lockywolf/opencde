...
static char *box_keywords[] = {
    "BOX",    "CONTAINER_NAME", "POSITION_HINTS", "LOCKED",
    "DELETE", "HELP_STRING",    "HELP_VOLUME",    "HELP_TOPIC",
};
...
static char *subpanel_keywords[] = {
    "SUBPANEL", "CONTAINER_NAME", "CONTROL_INSTALL", "TITLE",      "LOCKED",
    "DELETE",   "HELP_STRING",    "HELP_VOLUME",     "HELP_TOPIC",
};
...
static char *switch_keywords[] = {
    "SWITCH", "CONTAINER_NAME", "POSITION_HINTS", "NUMBER_OF_ROWS", "LOCKED",
    "DELETE", "HELP_STRING",    "HELP_VOLUME",    "HELP_TOPIC",
};
...
static char *control_keywords[] = {
    "CONTROL",        "TYPE",           "CONTAINER_TYPE", "CONTAINER_NAME",
    "POSITION_HINTS", "ICON",           "ALTERNATE_ICON", "LABEL",
    "PUSH_ACTION",    "PUSH_ANIMATION", "DROP_ACTION",    "DROP_ANIMATION",
    "PUSH_RECALL",    "MONITOR_TYPE",   "CLIENT_NAME",    "CLIENT_GEOMETRY",
    "FILE_NAME",      "DATE_FORMAT",    "LOCKED",         "DELETE",
    "HELP_STRING",    "HELP_VOLUME",    "HELP_TOPIC",
};
...
char *entry_types[] = {"PANEL", "BOX", "SUBPANEL", "SWITCH", "CONTROL"};

char *control_types[] = {
    "blank", "busy", "icon", "client", "clock", "date", "file",
};

char *resolution_types[] = {"high", "medium", "low", "match_display"};
...
static RecordKeywords record_keywords[] = {
    {"PANEL", PANEL_KEYWORD_COUNT, panel_keywords, panel_parse_functions},

    {"BOX", BOX_KEYWORD_COUNT, box_keywords, box_parse_functions},

    {"SUBPANEL", SUBPANEL_KEYWORD_COUNT, subpanel_keywords,
     subpanel_parse_functions},

    {"SWITCH", SWITCH_KEYWORD_COUNT, switch_keywords, switch_parse_functions},

    {"CONTROL", CONTROL_KEYWORD_COUNT, control_keywords,
     control_parse_functions},
};
...
static Boolean ControlSingleParseCB(DtDtsDbField *, DtDbPathId, char *,
                                    Boolean);
...
static void ProcessControl(XtPointer, char, ControlData ***, int *,
                           ElementValue *);
...
static DtDbConverter panel_converter[] = {(DtDbConverter)PanelParseCB, NULL};
static DtDbConverter box_converter[] = {(DtDbConverter)BoxParseCB, NULL};
static DtDbConverter subpanel_converter[] = {(DtDbConverter)SubpanelParseCB,
                                             NULL};
static DtDbConverter switch_converter[] = {(DtDbConverter)SwitchParseCB, NULL};
...
Boolean FrontPanelReadDatabases(void)

{
        ...
        for (i = panel_count - 1; i > 0; i--) {
                if ((intptr_t)(panel_data[i]
                                   .element_values[PANEL_LOCKED]
                                   .parsed_value))
                        break;
        }
        ...
        InitializeField(panel.element_values, PANEL_GEOMETRY,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_DISPLAY_HANDLES,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_DISPLAY_MENU,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_DISPLAY_MINIMIZE,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_RESOLUTION,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_CONTROL_BEHAVIOR,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_DISPLAY_CONTROL_LABELS,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_SUBPANEL_UNPOST,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_HELP_STRING,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_HELP_TOPIC,
                        panel_parse_functions);
        InitializeField(panel.element_values, PANEL_HELP_VOLUME,
                        panel_parse_functions);
        ...
        /* it is a locked file.					      */
        EliminateDeleted(box_data, &box_count, BOX_CONTAINER_NAME, BOX_NAME,
                         BOX, BOX_DELETE, BOX_LOCKED);
        EliminateDeleted(switch_data, &switch_count, SWITCH_CONTAINER_NAME,
                         SWITCH_NAME, SWITCH, SWITCH_DELETE, SWITCH_LOCKED);
        EliminateDeleted(subpanel_data, &subpanel_count,
                         SUBPANEL_CONTAINER_NAME, SUBPANEL_NAME, SUBPANEL,
                         SUBPANEL_DELETE, SUBPANEL_LOCKED);
        EliminateDeleted(control_data, &control_count, CONTROL_CONTAINER_NAME,
                         CONTROL_NAME, CONTROL, CONTROL_DELETE, CONTROL_LOCKED);

        name_list =
            GetNameList(box_data, box_count, BOX_NAME, ANY_CONTAINER_TYPE, 0);

        EliminateUnused(switch_data, &switch_count, name_list,
                        SWITCH_CONTAINER_NAME, ANY_CONTAINER_TYPE, BOX, SWITCH);

        EliminateUnused(control_data, &control_count, name_list,
                        CONTROL_CONTAINER_NAME, CONTROL_CONTAINER_TYPE, BOX,
                        CONTROL);
        ...
        if (switch_count != 0) {
                for (i = switch_count - 1; i > 0; i--) {
                        if ((intptr_t)(switch_data[i]
                                           .element_values[SWITCH_LOCKED]
                                           .parsed_value))
                                break;
                }
                ...
                InitializeField(switch1->element_values, SWITCH_POSITION_HINTS,
                                switch_parse_functions);
                InitializeField(switch1->element_values, SWITCH_NUMBER_OF_ROWS,
                                switch_parse_functions);
                InitializeField(switch1->element_values, SWITCH_HELP_STRING,
                                switch_parse_functions);
                InitializeField(switch1->element_values, SWITCH_HELP_TOPIC,
                                switch_parse_functions);
                InitializeField(switch1->element_values, SWITCH_HELP_STRING,
                                switch_parse_functions);
        }
        ...
        if (switch_count != 0)
                name_list[0] =
                    switch1->element_values[SWITCH_NAME].parsed_value;
        else
                name_list[0] = NULL;
        name_list[1] = NULL;
        EliminateUnused(control_data, &control_count, name_list,
                        CONTROL_CONTAINER_NAME, CONTROL_CONTAINER_TYPE, SWITCH,
                        CONTROL);
        XtFree((char *)name_list);

        name_list = GetNameList(control_data, control_count, CONTROL_NAME,
                                CONTROL_CONTAINER_TYPE, BOX);

        EliminateUnused(subpanel_data, &subpanel_count, name_list,
                        SUBPANEL_CONTAINER_NAME, ANY_CONTAINER_TYPE, CONTROL,
                        SUBPANEL);
        XtFree((char *)name_list);

        name_list = GetNameList(subpanel_data, subpanel_count, SUBPANEL_NAME,
                                ANY_CONTAINER_TYPE, SUBPANEL);
        ...
        ReorderByContainerType(control_data, control_count,
                               CONTROL_CONTAINER_TYPE);

        ReorderByContainerName(control_data, control_count,
                               CONTROL_CONTAINER_NAME, CONTROL_CONTAINER_TYPE);
        ...
        ResolveDuplicates(box_data, &box_count, BOX_NAME, ANY_CONTAINER_TYPE,
                          BOX_CONTAINER_NAME, BOX_LOCKED, BOX);
        ...
        for (i = 0; i < box_count; i++) {
                InitializeField(box_data[i].element_values, BOX_POSITION_HINTS,
                                box_parse_functions);
                InitializeField(box_data[i].element_values, BOX_HELP_STRING,
                                box_parse_functions);
                InitializeField(box_data[i].element_values, BOX_HELP_TOPIC,
                                box_parse_functions);
                InitializeField(box_data[i].element_values, BOX_HELP_VOLUME,
                                box_parse_functions);
        }

        for (i = 0; i < subpanel_count; i++) {
                InitializeField(subpanel_data[i].element_values,
                                SUBPANEL_CONTROL_INSTALL,
                                subpanel_parse_functions);
                InitializeField(subpanel_data[i].element_values, SUBPANEL_TITLE,
                                subpanel_parse_functions);
                InitializeField(subpanel_data[i].element_values,
        ...
        for (i = 0; i < control_count; i++) {
                InitializeSecondaryControlFields(
                    control_data[i].element_values);
                InitializeFileControlFields(control_data[i].element_values,
                                            NULL);
        }
        ...
        for (i = 0; i < box_count; i++) {
                printf("box %d = %s\n",
                       (int)box_data[i]
                           .element_values[BOX_POSITION_HINTS]
                           .parsed_value,
                       box_data[i].element_values[BOX_NAME].parsed_value);
        }
        ...
        for (i = 0; i < box_count; i++) {
                ...
                if (switch_count != 0 &&
                    strcmp(
                        (char *)box->element_values[BOX_NAME].parsed_value,
                        (char *)switch1->element_values[SWITCH_CONTAINER_NAME]
                            .parsed_value) == 0) {
...
static Boolean PanelParseCB(DtDtsDbField *fields, DtDbPathId path_id,
                            char *host_prefix, Boolean rejection)

{
        ...
        ProcessRecord(
            fields, (ElementValue *)panel_data[panel_count - 1].element_values);

        InitializeField(panel_data[panel_count - 1].element_values, PANEL_NAME,
                        panel_parse_functions);
...
static Boolean BoxParseCB(DtDtsDbField *fields, DtDbPathId path_id,
                          char *host_prefix, Boolean rejection)

{
        AllocateRecordData(&box_data, sizeof(ElementValue) * BOX_KEYWORD_COUNT,
                           &box_data_count, &box_count);

        ProcessRecord(fields, box_data[box_count - 1].element_values);

        InitializeField(box_data[box_count - 1].element_values, BOX_NAME,
                        box_parse_functions);
        InitializeField(box_data[box_count - 1].element_values,
                        BOX_CONTAINER_NAME, box_parse_functions);
        InitializeField(box_data[box_count - 1].element_values, BOX_LOCKED,
                        box_parse_functions);
        InitializeField(box_data[box_count - 1].element_values, BOX_DELETE,
                        box_parse_functions);
...
static Boolean SubpanelParseCB(DtDtsDbField *fields, DtDbPathId path_id,
                               char *host_prefix, Boolean rejection)
...
static Boolean SwitchParseCB(DtDtsDbField *fields, DtDbPathId path_id,
                             char *host_prefix, Boolean rejection)
...
static Boolean ControlParseCB(DtDtsDbField *fields, DtDbPathId path_id,
                              char *host_prefix, Boolean rejection)
...
static Boolean AnimationParseCB(DtDtsDbField *fields, DtDbPathId path_id,
                                char *host_prefix, Boolean rejection)

{
        ...
        panel.animation_data = (AnimationData *)XtRealloc(
            (char *)panel.animation_data, sizeof(AnimationData) * (count + 1));
        ...
        panel.animation_data[count].items =
            (AnimationItem *)XtMalloc(sizeof(AnimationItem) * field_count);

        for (i = 0; i < field_count; i++) {
                if (fields[i + 1].fieldName == animation_quark) {
                        head_ptr = source = (unsigned char *)strdup(
                            (char *)fields[i + 1].fieldValue);

                        if ((string = _DtWmParseNextTokenC(&source, False)) !=
                            NULL) {
                                panel.animation_data[count]
                                    .items[i]
                                    .image_name = strdup((char *)string);
                                if ((string = _DtWmParseNextTokenC(
                                         &source, False)) != NULL) {
                                        StringToInt((char *)string, &val_rtn);
                                        panel.animation_data[count]
                                            .items[i]
                                            .delay = (int)(intptr_t)val_rtn;
                                } else {
                                        panel.animation_data[count]
                                            .items[i]
                                            .delay = 0;
...
static Boolean ControlSingleParseCB(DtDtsDbField *fields, DtDbPathId path_id,
                                    char *host_prefix, Boolean rejection)

{
        ...
        control_element_values = (ElementValue *)XtMalloc(
            sizeof(ElementValue) * CONTROL_KEYWORD_COUNT);
...
static void ProcessRecord(DtDtsDbField *fields, ElementValue *element_values)

{
        ...
        for (entry = 0; entry < TOTAL_ENTRY_COUNT; entry++) {
                if (fields[0].fieldName ==
                    XrmStringToQuark(record_keywords[entry].record_keyword)) {
...
static void AllocateRecordData(RecordData **record_data, int record_size,
                               int *record_data_count, int *record_count)

{
        if (*record_count >= *record_data_count) {
                *record_data_count += 10;
                *record_data = (RecordData *)XtRealloc(
                    (char *)*record_data,
...
static void OrderRecord(RecordData *record_data, int record_count,
                        int value_define)

{
        ...
        for (new_loc = record_count - 2; new_loc >= 0; new_loc--) {
                compare_value = record_data[new_loc]
                                    .element_values[value_define]
                                    .parsed_value;
...
static void SwapEntries(RecordData *rec_data, int i, int j)

{
         ...
         *	performs a quick sort.  Positions that are equal will retain
         *current positioning.
         *
         ************************************************************************/

#ifdef NOT_DEF
static void QuickSort(RecordData *rec_data, int left, int right, int elem_index,
                      int container) {
        int i, last;
        ...
        if (container == BOX && elem_index == BOX_POSITION_HINTS ||
            (container == CONTROL && elem_index == CONTROL_POSITION_HINTS)) {
                for (i = left + 1; i <= right; i++)
                        if (rec_data[i]
                                .element_values[elem_index]
                                .parsed_value < rec_data[left]
                                                    .element_values[elem_index]
                                                    .parsed_value)
                                SwapEntries(rec_data, ++last, i);
        } else {
                for (i = left + 1; i <= right; i++)
                        if (strcmp(rec_data[i]
                                       .element_values[elem_index]
                                       .parsed_value,
                                   rec_data[left]
                                       .element_values[elem_index]
...
static void BubbleSort(RecordData *rec_data, int start, int rec_count,
                       int elem_index)

{
        ...
        if (elem_index == CONTROL_CONTAINER_NAME || elem_index == CONTROL_NAME)
                is_string = True;
        ...
        while (bound != 0) {
                ...
                for (j = start; j < bound; j++) {
                        if (is_string) {
                                str1 = (char *)rec_data[j]
                                           .element_values[elem_index]
                                           .parsed_value;
                                str2 = (char *)rec_data[j + 1]
                                           .element_values[elem_index]
                                           .parsed_value;
                                ...
                                if ((intptr_t)rec_data[j]
                                        .element_values[elem_index]
                                        .parsed_value >
                                    (intptr_t)rec_data[j + 1]
                                        .element_values[elem_index]
                                        .parsed_value) {
                                        SwapEntries(rec_data, j, j + 1);
                                        t = j;
...
static void ReversePositions(RecordData *rec_data, int start, int end)
...
static void ReorderByContainerName(RecordData *rec_data, int rec_count,
                                   int name, int elem_type)

{
        ...
        while (i < rec_count) {
                ...
                while (
                    (intptr_t)(
                        rec_data[i].element_values[elem_type].parsed_value) ==
                    cont_type) {
                        i++;
                        if (rec_count == i)
                                break;
...
static void ReorderByName(RecordData *rec_data, int rec_count, int name,
                          int elem_type)

{
        ...
        while (i < rec_count) {
                cont_name = (char *)(rec_data[start]
                                         .element_values[elem_type]
                                         .parsed_value);
                while (strcmp((char *)rec_data[i]
                                  .element_values[elem_type]
                                  .parsed_value,
                              cont_name) == 0) {
                        i++;
...
static void ReorderByPosition(RecordData *rec_data, int rec_count,
                              int pos_hints, int cont_type, int cont_name)

{
        ...
        while (i < rec_count) {
                container_name = (char *)(rec_data[start]
                                              .element_values[cont_name]
                                              .parsed_value);
                ...
                if (cont_type == ANY_CONTAINER_TYPE) {
                        ...
                        container_type =
                            (int)(intptr_t)(rec_data[start]
                                                .element_values[cont_type]
                                                .parsed_value);
                        while (((intptr_t)rec_data[i]
                                    .element_values[cont_type]
                                    .parsed_value == container_type) &&
                               (strcmp((char *)rec_data[i]
                                           .element_values[cont_name]
                                           .parsed_value,
                                       container_name) == 0)) {
                                i++;
...
static void ReorderByContainerType(RecordData *rec_data, int rec_count,
                                   int container_type)
...
static void EliminateEntries(RecordData *record_data, int *record_count,
                             int start, int end, int *i, int *count,
                             int record_type)

{
        ...
        if (end + 1 < *record_count) {
                memmove((void *)&record_data[start],
                        (void *)&record_data[end + 1],
...
static void ResolveDuplicates(RecordData *record_data, int *record_count,
                              int name_type, int container_type,
                              int container_name, int lock_type,
                              int record_type)

{
        ...
        while (i >= 0) {
                ...
                if (ANY_CONTAINER_TYPE != container_type) {
                        cont_type = (int)(intptr_t)(
                            element_values[container_type].parsed_value);
                        cont_name = (char *)(element_values[container_name]
                                                 .parsed_value);
                }
                record_name = (char *)(element_values[name_type].parsed_value);

                locked =
                    (Boolean)(intptr_t)(element_values[lock_type].parsed_value);
                if (locked) {
                        i--;
                        if (i < 0)
                                break;
                        ...
                        if (ANY_CONTAINER_TYPE == container_type) {
                                while (
                                    (strcmp((char *)(element_values[name_type]
                                                         .parsed_value),
                                            cont_name) == 0)) {
                                        last_index = i;
                                        i--;
                                        if (i < 0)
                                                break;
                                        element_values =
                                            record_data[i].element_values;
                                }
                        } else {
                                while (
                                    ((intptr_t)(element_values[container_type]
                                                    .parsed_value) ==
                                     cont_type) &&
                                    (strcmp(
                                         (char *)(element_values[container_name]
                                                      .parsed_value),
                                         cont_name) == 0) &&
                                    (strcmp((char *)(element_values[name_type]
                                                         .parsed_value),
                        ...
                        if (start_index != last_index)
                                EliminateEntries(record_data, record_count,
                        ...
                        if (ANY_CONTAINER_TYPE == container_type) {
                                while (
                                    (strcmp((char *)(element_values[name_type]
                                                         .parsed_value),
                                            record_name) == 0)) {
                                        locked = (Boolean)(intptr_t)(
                                            element_values[lock_type]
                                                .parsed_value);
                        ...
                        if (start_index != last_index) {
                                if (start_index == lock_index)
                                        EliminateEntries(
                                            ...
                                            last_index + 1, start_index, &i,
                                            &count, record_type);
                                else {
                                        int diff = start_index - lock_index;
                                        EliminateEntries(
                                            record_data, record_count,
                                            lock_index + 1, start_index, &i,
                                            &count, record_type);
                                        lock_index += diff;
                                        last_index += diff;
...
static char **GetNameList(RecordData *record_data, int record_count,
                          int name_type, int container_type, int container)

{
        ...
        for (i = 0; i < record_count; i++) {
                if (ANY_CONTAINER_TYPE == container_type ||
                    (intptr_t)(record_data[i]
                                   .element_values[container_type]
                                   .parsed_value) == container) {
                        if (found_count >= name_list_count) {
                                name_list_count += 10;
                                name_list = (char **)XtRealloc(
                                    (char *)name_list,
                                    sizeof(char **) * (name_list_count + 1));
                        }
                        name_list[found_count] = record_data[i]
                                                     .element_values[name_type]
                                                     .parsed_value;
                        found_count++;
                }
        }
...
static void EliminateUnused(RecordData *record_data, int *record_count,
                            char **cont_name_list, int cont_name, int cont_type,
                            int container, int record_type)

{
        ...
        while (i < count) {
                element_values = record_data[i].element_values;
                if (cont_type == ANY_CONTAINER_TYPE ||
                    (intptr_t)(element_values[cont_type].parsed_value) ==
                        container) {
                        ...
                        if (!name_found) {
                                ...
                                printf("Entry Eliminated - %s\n",
                                       (char *)element_values[cont_name]
                                           .parsed_value);
#endif
                                EliminateEntries(record_data, record_count, i,
                                                 i, &dummy, &count,
                                                 record_type);
                        } else {
                                name_found = False;
...
static void EliminateDeleted(RecordData *record_data, int *record_count,
                             int cont_name, int record_name, int record_type,
                             int delete_type, int lock_type)
        ...
        while (i < count) {
                ...
                if ((intptr_t)element_values[delete_type].parsed_value) {
                        ...
                        if (record_type == CONTROL) {
                                rec_type =
                                    (intptr_t)element_values[CONTROL_TYPE]
                                        .parsed_value;
                                container_type =
                                    (intptr_t)
                                        element_values[CONTROL_CONTAINER_TYPE]
                        ...
                        container_name = XtNewString(
                            (char *)element_values[cont_name].parsed_value);
                        rec_name = XtNewString(
                            (char *)element_values[record_name].parsed_value);

                        for (j = count - 1; j >= 0; j--) {
                                other_element_values =
                                    record_data[j].element_values;
                                if ((record_type != CONTROL ||
                                     ((intptr_t)other_element_values
                                              [CONTROL_CONTAINER_TYPE]
                                                  .parsed_value ==
                                          container_type &&
                                      (intptr_t)
                                              other_element_values[CONTROL_TYPE]
                                                  .parsed_value == rec_type)) &&
                                    strcmp(
                                        (char *)other_element_values[cont_name]
                                            .parsed_value,
                                        container_name) == 0 &&
                                    strcmp((char *)
                                               other_element_values[record_name]
                                                   .parsed_value,
                                           rec_name) == 0) {
                                        if ((intptr_t)
                                                other_element_values[lock_type]
                                                    .parsed_value) {
                                                if ((intptr_t)
                                                        other_element_values
                                                            [delete_type]
...
static void InitializeField(ElementValue *element_values, int keyword,
                            ParseFunction *parse_functions)
...
static int CountElements(RecordData *record_data, int record_count,
                         char *container_name, int name_type,
                         int container_type, int container)
...
void DeleteControlActionList(ControlData *control_data)
...
void AddControlActionList(ControlData *control_data)

{
        ...
        if (data_type != NULL) {
                ...
                if (attr_list != NULL) {
                        for (i = 0; attr_list[i] != NULL; i++) {
                                for (j = 0; j < 4; j++)
                                        if (strcmp(attr_list[i]->name,
                                                   attr[j]) == 0)
                                                break;

                                /*  If we have found a match, find the
                                 * appropriate attr  */
        ...
        if (action_list == NULL) {
                if (control_data->element_values[CONTROL_PUSH_ACTION]
                        .string_value != NULL) {
                        action_list = (char **)XtMalloc(sizeof(char *) * 2);

                        action_list[0] =
                            control_data->element_values[CONTROL_PUSH_ACTION]
                                .string_value;
                        action_list[1] = NULL;
                        num_actions = 1;
                } else
        ...
        control_data->actions = (PanelActionData **)XtMalloc(
            sizeof(PanelActionData *) * (num_actions + 1));
        ...
        for (i = 0, j = 0; j < num_actions; i++, j++) {
                ...
                control_data->actions[i] =
                    (PanelActionData *)XtMalloc(sizeof(PanelActionData));
                ...
                if (label != NULL)
                        ...
                        control_data->actions[i]->action_label =
                            strdup(action_list[j]);
        }
...
static void ProcessBox(BoxData *box_data)

{
        ...
        for (i = 0; i < control_count; i++) {
                ...
                if ((intptr_t)element_values[CONTROL_CONTAINER_TYPE]
                            .parsed_value == BOX &&
                    strcmp(
                        (char *)box_data->element_values[BOX_NAME].parsed_value,
                        (char *)element_values[CONTROL_CONTAINER_NAME]
                            .parsed_value) == 0) {
                        ProcessControl((XtPointer)box_data, BOX,
                                       &(box_data->control_data),
                                       &box_control_count, element_values);

                        box_data->subpanel_count += ProcessBoxControl(
                            box_data->control_data[box_control_count - 1]);
                }
        }
...
static void ProcessControl(XtPointer parent, char parent_type,
                           ControlData ***control_data_ptr, int *control_count,
                           ElementValue *element_values)

{
        ControlData *control;

        *control_data_ptr = (ControlData **)XtRealloc((char *)*control_data_ptr,
                                                      sizeof(ControlData *) *
                                                          (*control_count + 1));
...
static int ProcessBoxControl(ControlData *control_data)

{
        ...
        for (i = 0; i < subpanel_count; i++) {
                ...
                if (strcmp((char *)control_data->element_values[CONTROL_NAME]
                               .parsed_value,
                           (char *)element_values[SUBPANEL_CONTAINER_NAME]
                               .parsed_value) == 0) {
...
static void ProcessSubpanel(SubpanelData *subpanel_data)

{
        ...
        for (i = 0; i < control_count; i++) {
                ...
                if ((intptr_t)element_values[CONTROL_CONTAINER_TYPE]
                    ...
                    strcmp((char *)subpanel_data->element_values[SUBPANEL_NAME]
                               .parsed_value,
                           (char *)element_values[CONTROL_CONTAINER_NAME]
                               .parsed_value) == 0) {
...
static void ProcessSwitch(SwitchData *switch_data)

{
        ...
        for (i = 0; i < control_count; i++) {
                ...
                if ((intptr_t)element_values[CONTROL_CONTAINER_TYPE]
                            .parsed_value == SWITCH &&
                    strcmp((char *)switch_data->element_values[SWITCH_NAME]
                               .parsed_value,
                           (char *)element_values[CONTROL_CONTAINER_NAME]
                               .parsed_value) == 0) {
...
static char *CreateComponentFileName(RecordData *record_data)
...
static void WriteComponentToFile(RecordData *record_data, int record_type,
                                 char **keywords, char *container_name,
                                 int container_type, Boolean delete)
        ...
        if ((fd = fopen(file_name, "w")) != NULL) {
                ...
                if (delete) {
                        if (record_type == CONTROL) {
                                fprintf(
                                    fd, "   TYPE      %s\n",
                                    record_data->element_values[CONTROL_TYPE]
                                        .string_value);
                                fprintf(
                                    fd, "   CONTAINER_TYPE      %s\n",
                                    record_data
                        ...
                        for (k = 1;
                             k < record_descriptor[record_type].maxFields;
                             k++) {
                                if (record_data->element_values[k]
                                            .string_value != NULL &&
                                    record_data->element_values[k]
                                            .use_default == False) {
                                        if (record_type == CONTROL &&
                                            (intptr_t)record_data
                                                    ->element_values
                                                        [CONTROL_TYPE]
                                                    .parsed_value ==
                                                CONTROL_FILE &&
                ...
                if (record_type == CONTROL)
                        SessionAddFileData(
                            file_name,
                            record_data->element_values[CONTROL_NAME]
                                .string_value,
                            (int)CONTROL, container_name, container_type,
                            delete);
                else
                        SessionAddFileData(
                            file_name,
...
static void RemoveComponentFile(RecordData *record_data, int record_type,
                                char **keywords, char *container_name,
                                int container_type)
...
void WriteControlComponentFile(ControlData *control_data)

{
        ...
        WriteComponentToFile(
            (RecordData *)control_data, CONTROL, control_keywords,
...
void WriteSubpanelComponentFile(SubpanelData *subpanel_data)

{
        ElementValue *element_values = subpanel_data->element_values;

        WriteComponentToFile(
            (RecordData *)subpanel_data, SUBPANEL, subpanel_keywords,
...
void RemoveControlComponentFile(ControlData *control_data)
...
void RemoveSubpanelComponentFile(SubpanelData *subpanel_data)
...
void _WriteControlElementValues(ElementValue *element_values)

{
        int k;

        printf("%s	%s\n{\n", control_keywords[0],
               element_values[0].string_value);

        for (k = 1; k < record_descriptor[CONTROL].maxFields; k++) {
                if (element_values[k].string_value != NULL)
                        printf("   %s	%s\n", control_keywords[k],
                               element_values[k].string_value);
        }
...
void InitParse(char *file_name, ElementValue **elem_vals)
...
static void FreeFileControlField(ElementValue *element_values, int indx)
...
static void InitializePrimaryControlFields(ElementValue *element_values)

{
        InitializeField(element_values, CONTROL_NAME, control_parse_functions);
        InitializeField(element_values, CONTROL_CONTAINER_NAME,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_CONTAINER_TYPE,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_LOCKED,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_DELETE,
                        control_parse_functions);
}
...
static void InitializeSecondaryControlFields(ElementValue *element_values)

{
        InitializeField(element_values, CONTROL_TYPE, control_parse_functions);
        InitializeField(element_values, CONTROL_POSITION_HINTS,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_ALTERNATE_ICON,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_PUSH_ANIMATION,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_DROP_ANIMATION,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_PUSH_RECALL,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_MONITOR_TYPE,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_CLIENT_NAME,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_CLIENT_GEOMETRY,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_FILE_NAME,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_DATE_FORMAT,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_HELP_STRING,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_HELP_VOLUME,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_HELP_TOPIC,
                        control_parse_functions);
}
...
static void InitializeFileControlFields(ElementValue *element_values,
                                        char *data_type)

{
        if ((intptr_t)element_values[CONTROL_TYPE].parsed_value ==
            CONTROL_FILE) {
                ...
                file_name =
                    (char *)element_values[CONTROL_FILE_NAME].parsed_value;
                ...
                if (data_type != NULL) {
                        if (valid_file) {
                                icon_name = DtDtsDataTypeToAttributeValue(
                                    data_type, DtDTS_DA_ICON, NULL);

                                if (element_values[CONTROL_NORMAL_ICON]
                                        .string_value != NULL) {
                                        XtFree(
                                            element_values[CONTROL_NORMAL_ICON]
                                                .string_value);
                                        element_values[CONTROL_NORMAL_ICON]
                                            .string_value = NULL;
                                }
                                ...
                                element_values[CONTROL_NORMAL_ICON]
                                    .string_value = XtNewString(icon_name);

                                DtDtsFreeAttributeValue(icon_name);
                        }

                        description = DtDtsDataTypeToAttributeValue(
                            data_type, DtDTS_DA_DESCRIPTION, NULL);

                        if (element_values[CONTROL_HELP_STRING].string_value !=
                            NULL) {
                                XtFree(element_values[CONTROL_HELP_STRING]
                                           .string_value);
                                element_values[CONTROL_HELP_STRING]
                                    .string_value = NULL;
                        }

                        element_values[CONTROL_HELP_STRING].string_value =
                            XtNewString(description);
                        DtDtsFreeAttributeValue(description);

                        act_list = DtDtsDataTypeToAttributeValue(
                            data_type, DtDTS_DA_ACTION_LIST, NULL);
                        if (act_list)
                                action_list =
                                    _DtVectorizeInPlace(act_list, ',');

                        if (action_list && action_list[0] != NULL) {
                                element_values[CONTROL_PUSH_ACTION]
                                    .string_value = strdup(action_list[0]);
                                element_values[CONTROL_DROP_ACTION]
                                    .string_value = strdup(action_list[0]);
                        ...
                        if (label == NULL) {
                                label = DtDtsDataTypeToAttributeValue(
                                    data_type, DtDTS_DA_LABEL, NULL);
                        ...
                        if (label == NULL && file_name != NULL) {
                                if ((file_str = (char *)strrchr(file_name,
                                                                '/')) == NULL)
                                        file_str = file_name;
                                else
                                        file_str++;
        ...
        InitializeField(element_values, CONTROL_NORMAL_ICON,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_HELP_STRING,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_LABEL, control_parse_functions);
        InitializeField(element_values, CONTROL_PUSH_ACTION,
                        control_parse_functions);
        InitializeField(element_values, CONTROL_DROP_ACTION,
                        control_parse_functions);
}
...
void InitializeControlFields(ElementValue *element_values, char *data_type)
...
static void UpdateFileType(ControlData *control_data)

{
        ...
        if ((intptr_t)control_data->element_values[CONTROL_CONTAINER_TYPE]
                .parsed_value == SUBPANEL) {
...
void UpdateFileTypeControlFields(void)
...
void InitializeSubpanelFields(ElementValue *element_values)

{
        InitializeField(element_values, SUBPANEL_NAME,
                        subpanel_parse_functions);
        InitializeField(element_values, SUBPANEL_CONTAINER_NAME,
                        subpanel_parse_functions);
        InitializeField(element_values, SUBPANEL_LOCKED,
                        subpanel_parse_functions);
        InitializeField(element_values, SUBPANEL_CONTROL_INSTALL,
                        subpanel_parse_functions);
        InitializeField(element_values, SUBPANEL_TITLE,
                        subpanel_parse_functions);
        InitializeField(element_values, SUBPANEL_DELETE,
                        subpanel_parse_functions);
        InitializeField(element_values, SUBPANEL_HELP_STRING,
                        subpanel_parse_functions);
        InitializeField(element_values, SUBPANEL_HELP_TOPIC,
                        subpanel_parse_functions);
        InitializeField(element_values, SUBPANEL_HELP_VOLUME,
                        subpanel_parse_functions);
}
