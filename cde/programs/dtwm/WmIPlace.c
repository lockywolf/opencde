...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmIPlace.c /main/4 1995/11/01 11:41:20 rswiston $"
#endif
#endif
/*
...
void InitIconPlacement(WmWorkspaceData *pWS) {
        ...
        if (pWS->IPData.iconPlacement & ICON_PLACE_TIGHT) {
                ...
                do {
                        if (useMargin) {
                                ...
                                iSpaceX = (sW - (pWS->IPData.placementCols *
                                                 pWS->pSD->iconWidth)) /
                                          pWS->IPData.placementCols;
                        }
                ...
                do {
                        if (useMargin) {
                                ...
                                iSpaceY = (sH - (pWS->IPData.placementRows *
                                                 pWS->pSD->iconHeight)) /
                                          pWS->IPData.placementRows;
                        }
        ...
        if (pWS->IPData.iconPlacement & ICON_PLACE_TIGHT) {
                ...
                if (useMargin) {
                        ...
                        extraXSpace = sW - (2 * pWS->pSD->iconPlacementMargin) -
                                      (placementW - iSpaceX);
                        extraPX =
                            (pWS->IPData.iconPlacement &
                             ICON_PLACE_RIGHT_PRIMARY)
                                ? 1
                                : (pWS->IPData.placementCols - extraXSpace);
                        ...
                        extraPY =
                            (pWS->IPData.iconPlacement &
                             ICON_PLACE_BOTTOM_PRIMARY)
                                ? 1
                                : (pWS->IPData.placementRows - extraYSpace);
                } else {
                        xMargin =
                            (sW - placementW +
                             ((pWS->IPData.iPlaceW - pWS->pSD->iconWidth) &
                              1)) /
                            2;
                        extraXSpace = 0;
                        yMargin =
                            (sH - placementH +
                             ((pWS->IPData.iPlaceH - pWS->pSD->iconHeight) &
                              1)) /
                            2;
                        extraYSpace = 0;

                        if (pWS->IPData.iconPlacement &
                            ICON_PLACE_RIGHT_PRIMARY) {
                                xMargin = sW - placementW - xMargin;
        ...
        if ((pWS->IPData.placementRowY = (int *)XtMalloc(
                 (pWS->IPData.placementRows + 2) * sizeof(int))) == NULL) {
                Warning(((char *)GETMESSAGE(
                    34, 1, "Insufficient memory for icon placement")));
                wmGD.iconAutoPlace = False;
                return;
        } else if ((pWS->IPData.placementColX = (int *)XtMalloc(
                        (pWS->IPData.placementCols + 2) * sizeof(int))) ==
                   NULL) {
                XtFree((char *)pWS->IPData.placementRowY);
                Warning(((char *)GETMESSAGE(
                    34, 2, "Insufficient memory for icon placement")));
                wmGD.iconAutoPlace = False;
                return;
        }
        ...
        for (i = 1; i <= pWS->IPData.placementRows; i++) {
                pWS->IPData.placementRowY[i] =
                    pWS->IPData.placementRowY[i - 1] + pWS->IPData.iPlaceH;
        ...
        for (i = 1; i <= pWS->IPData.placementCols; i++) {
                pWS->IPData.placementColX[i] =
                    pWS->IPData.placementColX[i - 1] + pWS->IPData.iPlaceW;
        ...
        if ((pWS->IPData.placeList = (IconInfo *)XtMalloc(
                 pWS->IPData.totalPlaces * sizeof(IconInfo))) == NULL) {
                Warning(((char *)GETMESSAGE(
                    34, 3, "Insufficient memory for icon placement")));
                XtFree((char *)pWS->IPData.placementRowY);
...
void CvtIconPlaceToPosition(IconPlacementData *pIPD, int place, int *pX,
                            int *pY) {
        int row;
        int col;
...
int FindIconPlace(ClientData *pCD, IconPlacementData *pIPD, int x, int y) {
        ...
        for (i = 0; i < 2; i++) {
                ...
                for (j = 0; j < 4; j++) {
                        ...
                        case 0: {
                                if (pIPD->iconPlacement &
                                    ICON_PLACE_LEFT_PRIMARY) {
                                        altX = x - diffX;
                                        altY = y;
                                } else if (pIPD->iconPlacement &
                                           ICON_PLACE_RIGHT_PRIMARY) {
                                        altX = x + diffX;
                                        altY = y;
                                } else if (pIPD->iconPlacement &
                                           ICON_PLACE_TOP_PRIMARY) {
                                        altX = x;
                                        altY = y - diffY;
                                } else {
                                        altX = x;
                        ...
                        case 2: {
                                if (pIPD->iconPlacement &
                                    ICON_PLACE_LEFT_SECONDARY) {
                                        altX = x - diffX;
                                        altY = y;
                                } else if (pIPD->iconPlacement &
                                           ICON_PLACE_RIGHT_SECONDARY) {
                                        altX = x + diffX;
                                        altY = y;
                                } else if (pIPD->iconPlacement &
                                           ICON_PLACE_TOP_SECONDARY) {
                                        altX = x;
                                        altY = y + diffY;
                                } else {
                                        altX = x;
                        ...
                        if (P_ICON_BOX(pCD)) {
                                ...
                                if (((int)altX) > ((int)clipX + (int)clipWidth -
                                                   ((int)ICON_WIDTH(pCD)))) {
                                        return (NO_ICON_PLACE);
                                }
                                if (((int)altY) >
                                    ((int)clipY + (int)clipHeight -
                                     ((int)ICON_HEIGHT(pCD)))) {
                                        return (NO_ICON_PLACE);
                                }
                        }
        ...
        if (wmGD.iconAutoPlace) {
                for (iOld = iNew = 0; iOld < ACTIVE_WS->IPData.totalPlaces;
                     iOld++, iNew++) {
                        ...
                        if (iOld < ACTIVE_WS->IPData.totalPlaces &&
                            iOld != iNew) {
                                /* move the icon from its old place to the new
                                 * place */
                                ...
                                CvtIconPlaceToPosition(
                                    &ACTIVE_WS->IPData, pWsc->iconPlace,
                                    &pWsc->iconX, &pWsc->iconY);
#else  /* WSM */
                                pCD->iconPlace = iNew;
                                CvtIconPlaceToPosition(
                                    &ACTIVE_WS->IPData, pCD->iconPlace,
                                    &pCD->iconX, &pCD->iconY);
#endif /* WSM */
                                ...
                                if (hasActiveText && (pCD == pCD_active)) {
                                        ...
                                        HideActiveIconText(
                                            (WmScreenData *)NULL);
#ifdef WSM
                                        XMoveWindow(DISPLAY, pWsc->iconFrameWin,
                                                    pWsc->iconX, pWsc->iconY);
#else  /* WSM */
                                        XMoveWindow(DISPLAY,
                                                    ICON_FRAME_WIN(pCD),
