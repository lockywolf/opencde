...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmEvent.c /main/7 1996/11/20 15:27:47 rswiston $"
#endif
#endif
/*
        ...
        for (scr = 0; scr < wmGD.numScreens; scr++) {
                ...
                if (pSD->managed) {
                        ...
                        if (pSD->buttonBindings) {
                                /*
                                 * The desktop menu and button bindings for
                                 * window manager functions use button press and
                        ...
                        /*
                         * Setup event handling for "accelerated" window
                         * management functions done with key bindings.
                         */
                        ...
                        if (pSD->acceleratorMenuCount) {
                                for (n = 0; n < pSD->acceleratorMenuCount;
                                     n++) {
                                        SetupKeyBindings(
                                            pSD->acceleratorMenuSpecs[n]
                                                ->accelKeySpecs,
                                            pSD->rootWindow, GrabModeSync,
                                            F_CONTEXT_ALL);
                                }
                        }
                } /* end if (managed) */
...
static unsigned int *_WmGrabMasks(unsigned int modifiers, int *pnum_masks) {
        ...
        if ((pRetMasks == NULL) || (len_ret_masks < num_masks + 2)) {
                ...
                pRetMasks = (unsigned int *)XtCalloc(len_ret_masks,
                                                     sizeof(unsigned int));
        }
...
void WmGrabButton(Display *display, unsigned int button, unsigned int modifiers,
        ...
        for (i = 0; i < num_masks; i++, pGrabMasks++) {
                XGrabButton(display, button, *pGrabMasks, grab_window,
                            event_mask, owner_events, pointer_mode,
        ...
        while (keySpec) {
                ...
                if (((keySpec->context == F_CONTEXT_ICON) && iconContext) ||
                    ((keySpec->context != F_CONTEXT_ICON) && !iconContext))
                        ...
                        if (((F_CONTEXT_ICON ==
                              (keySpec->context ^
                               (F_CONTEXT_ICONBOX | F_SUBCONTEXT_IB_IICON |
                            ...
                            ((F_CONTEXT_ICON !=
                             ...
                             !iconContext)) {
                                ...
                                WmGrabKey(DISPLAY, keySpec->keycode,
                                          keySpec->state, grabWindow, False,
...
Boolean WmDispatchMenuEvent(XButtonEvent *event) {
        ...
        if (event->type == KeyPress) {
                if (wmGD.menuActive->accelKeySpecs) {
                         ...
                         * (don't require context match for system menu
                         * accelerators). If so, the active menu will be
                         * unposted and the KeyPress event will not be sent on
                         * to the toolkit.
                         */

                        checkContext =
                            (!pCD || (pCD->systemMenuSpec != wmGD.menuActive));
                        if (checkContext) {
                                if (pCD) {
                                        if (pCD->clientState ==
                                            MINIMIZED_STATE) {
                                                context = F_CONTEXT_ICON;
                                        } else if (pCD->clientState ==
                                                   NORMAL_STATE) {
                        ...
                        while (keySpecs) {
                                if ((keyEvent->keycode == keySpecs->keycode) &&
                                    ((keyEvent->state == keySpecs->state) ||
                                     (NOLOCKMOD(keyEvent->state) ==
                                      keySpecs->state)) &&
                                    ((!checkContext) ||
                                     (context & keySpecs->context))) {
                                        doXtDispatchEvent = XtIsSensitive(
                                            menuBtnPtr->buttonWidget);
                                        break;
                                }
                ...
                if (wmGD.menuActive && wmGD.menuUnpostKeySpec) {
                        if ((wmGD.menuUnpostKeySpec->keycode ==
                             event->button) &&
                            ((wmGD.menuUnpostKeySpec->state == event->state) ||
                             (wmGD.menuUnpostKeySpec->state ==
                              NOLOCKMOD(event->state)))) {
                                /*
                                 * This is an alternate key for unposting a menu
                                 * from the keyboard (in addition to [ESC]).
                                 */
                ...
                if (wmGD.menuActive && wmGD.F_NextKeySpec) {
                        if (((wmGD.F_NextKeySpec->state == event->state) ||
                             (wmGD.F_NextKeySpec->state ==
                              NOLOCKMOD(event->state))) &&
                            (wmGD.F_NextKeySpec->keycode == event->button)) {
                                /*
                                 * This is a key spec to traverse to the next
                ...
                if (wmGD.menuActive && wmGD.F_PrevKeySpec) {
                        if (((wmGD.F_PrevKeySpec->state == event->state) ||
                             (wmGD.F_PrevKeySpec->state ==
                              NOLOCKMOD(event->state))) &&
                            (wmGD.F_PrevKeySpec->keycode == event->button)) {
                                /*
                                 * This is a key spec to traverse to the
                                 * previous window via the keyboard.
                                 */
...
#endif /* WSM */
        {
                ...
                /*
                 * The event triggers hotspot processing for the system menu
                 * button or an icon.
                 */
                ...
                if (event->type == ButtonRelease) {
...
#endif /* WSM */
                                /*
                                 * The system menu is posted from a system menu
                                 * button or an icon.  By doing a button release
                                 * over the system menu button or icon the
                                 * system menu that is posted is put into
                                 * keyboard traversal mode.
                                 */

                                ProcessClickBRelease(event, pCD,
                                                     wmGD.clickData.context,
                                                     wmGD.clickData.subContext);

                                if (wmGD.clickData.context ==
...
#else
                        _XmGetMenuState(
                            XtParent(pCD->systemMenuSpec->menuWidget))
                        ...
                        doXtDispatchEvent = True;
                                   ...
                                   ((((XButtonEvent *)event)->state ==
                                    ...
                                    (NOLOCKMOD(
                                     ...
                                     wmGD.clickData.releaseState))) {
                                /*
                                 * This is a button release over the root. Check
                                 * for root menu click and keep the menu up in a
                                 * sticky fashion.
                                 */
                                Time timeDiff;
                        ...
                        /*
                         * A button press over a system menu button or an icon
                         * when the system menu is posted indicates that a
                         * double-click action is to be done if appropriate and
                         * the menu is to be taken out of traversal mode (done
                         * by the menu widget).
                         */
                        ...
                        if (wmGD.clickData.subContext ==
                            F_SUBCONTEXT_W_SYSTEM) {
                                PushGadgetIn(pCD, FRAME_SYSTEM);
                        }

                        if (wmGD.clickData.doubleClickContext ==
                            F_SUBCONTEXT_W_SYSTEM) {
                                if (wmGD.systemButtonClick2 &&
                                    (pCD->clientFunctions & MWM_FUNC_CLOSE)) {
                                        /*
                                         * Close the client window.  Cancel
                                         * other system menu button actions.
                                         */
                        ...
                        } else if (wmGD.clickData.doubleClickContext ==
                                   F_SUBCONTEXT_I_ALL) {
                                /*
                                 * Normalize the icon.
                                 */
                                ...
                                UnpostMenu(pCD->systemMenuSpec);
                                F_Restore_And_Raise((String)NULL, pCD,
                                                    (XEvent *)NULL);
...
*/ doXtDispatchEvent = False;
                        ...
                        /*
                         * Else no special button press processing; have the
                         * toolkit dispatch the event to the menu widgets.
                         */
                }
        }
...
Boolean WmDispatchWsEvent(XEvent *event) {
        ...
        case ButtonPress: {
                ...
                if (wmGD.menuActive) {
                        dispatchEvent =
                            True; /* have the toolkit dispatch the event */
                } else {
        ...
        case UnmapNotify: {
                ...
                {
                        /*
                         * This is a synthetic UnmapNotity used to withdraw a
                         * client window form window manager control.
                         */
        ...
        case MapRequest: {
                ...
                if ((XFindContext(DISPLAY, event->xmaprequest.window,
                    ...
                    (pSD = GetScreenForWindow(event->xmaprequest.window))) {
                         ...
                         * screen/root window that we manage. Start to manage
                         * the new window.  Management details are dependent on
                         * the type of the window.  For a typical top-level
                         * application window reparent the window to a window
                         * frame, add it to the wm saveset, ...
                         */

                        ManageWindow(pSD, event->xmaprequest.window,
                                     MANAGEW_NORMAL);
                }
                /* else ...
...
Boolean HandleWsKeyPress(XKeyEvent *keyEvent) {
        ...
        if (wmGD.menuActive) {
                /*
                 *  The active menu accelerators have been checked and keyEvent
                 * was not one of them.  We will check for pass keys mode and
                 * then have the toolkit dispatch the event, without searching
                 * any other key or accelerator specification list.
                 */
        ...
        if (checkKeyEvent) {
                if (wmGD.keyboardFocus) {
                        if (wmGD.keyboardFocus->clientState ==
                            MINIMIZED_STATE) {
                                context = F_CONTEXT_ICON;
                        } else if (wmGD.keyboardFocus->clientState ==
                                   NORMAL_STATE) {
                                context = F_CONTEXT_NORMAL;
                        } else {
                                context = F_CONTEXT_MAXIMIZE;
                ...
                if (HandleKeyPress(keyEvent, ACTIVE_PSD->keySpecs, TRUE,
                                   context, FALSE, (ClientData *)NULL) &&
                    ACTIVE_PSD->acceleratorMenuCount) {
                        for (n = 0; ((keyEvent->keycode != 0) &&
                                     (n < ACTIVE_PSD->acceleratorMenuCount));
                             n++) {
                                if (!HandleKeyPress(
                                        keyEvent,
                                        ACTIVE_PSD->acceleratorMenuSpecs[n]
                                            ->accelKeySpecs,
                                        TRUE, context, TRUE,
                                        (ClientData *)NULL)) {
                                        break;
                ...
                /*
                 * Fix for CR 3117 - Do the XAllowEvents after calling
                 * HandleKeyPress so that keys meant for an application can be
                 * sent to it.
                 */
                XAllowEvents(DISPLAY, AsyncKeyboard, CurrentTime);
                /*
                ...
                if (((keyEvent->state == keySpecs->state) ||
                    ...
                    (keyEvent->keycode == keySpecs->keycode)) {
                        if ((!checkContext) || (context & keySpecs->context)) {
                                 ...
                                 * Determine the client to which the key binding
                                 * function is to apply. Unpost any active menu
                                 * and specify that no futher key binding
                                 *   processing should be done.
                                 * Do the function associated with the matching
                                 * key binding. Stop if onlyFirst == TRUE
                                 */
                                ...
                                if (wmGD.menuActive) {
                                        functionClient =
                                            wmGD.menuClient; /* might not have
                                                                focus! */
                                        UnpostMenu(wmGD.menuActive);
                                        processKey = False;
                                } else if (onlyFirst) {
                                ...
                                if ((keySpecs->wmFunction == F_Menu) ||
                                    ...
                                    (keySpecs->wmFunction == F_Post_SMenu)) {
                                        wmGD.menuUnpostKeySpec =
                                            keySpecs; /* menu unpost key spec */
                                ...
                                if (!(keySpecs->wmFunction(keySpecs->wmFuncArgs,
                                                           ...
                                                           keyEvent))) {
                                        /*
                                         * The window manager function return
                                         * indicates that further key binding
                                ...
                                /*
                                 * Note that for key bindings, frame, title,
                                 * border, and app contexts are equivalent to
                                 * the window context. This is NOT the same as
                                 * for button bindings.
                                 */
                                if ((context & (F_CONTEXT_WINDOW)))
                                        haveWindowBinding = True;
                        }
                        /* Fix for 3117 -- If the keypress looks as if it had
                         * been intended for the application, send it back.
                         */
...
void HandleWsButtonPress(XButtonEvent *buttonEvent) {
        ...
        if ((buttonEvent->subwindow == None) ||
            (XFindContext(DISPLAY, buttonEvent->subwindow,
                          wmGD.windowContextType, (caddr_t *)&pCD))) {
        ...
        if (CheckForButtonAction(buttonEvent, context, subContext, pCD) &&
            pCD) {
                 ...
                 * that are associated with the built-in semantics of the window
                 * frame decorations.
                 */
...
void HandleWsButtonRelease(XButtonEvent *buttonEvent) {
        ...
        if (CheckForButtonAction(buttonEvent, context, subContext, pCD) &&
            pCD) {
                ...
                CheckButtonReleaseBuiltin(buttonEvent, context, subContext,
                                          pCD);
        }
        /*
         * Else skip built-in processing due to execution of a function that
                     ...
                     (NOLOCKMOD(buttonEvent->state) == buttonSpec->state))) {
                        ...
                        if ((buttonEvent->type == buttonSpec->eventType) &&
                            ...
                            (subContext & buttonSpec->subContext)) {
                                ...
                                /*
                                 * For click type bindings check for a match
                                 * between the event context and the click /
                                 * double-click context.
                                 */

                                if (buttonEvent->type == ButtonRelease) {
                                        /*
                                         * Clicks occur on button releases.  A
                                         * button release binding is always
                                         * treated as a click binding.
                                         */

                                        if ((buttonSpec->subContext |
                                             wmGD.clickData.clickContext) !=
                                            buttonSpec->subContext) {
                                                /* click binding and event
                                                 * contexts do not match */
                                                buttonSpec =
                                                    buttonSpec->nextButtonSpec;
                                                continue;
                                        }
                                        /* else there is a click match */
                                } else if (buttonSpec->click &&
                                           (buttonEvent->type == ButtonPress)) {
                                        /*
                                         * Double-clicks occur on button
                                         * presses.
                                         */
                                        ...
                                        if ((buttonSpec->subContext |
                                            ...
                                            buttonSpec->subContext) {
                                                ...
                                                /*
                                                 * The is a double-click match.
                                                 * Don't do any click or
                                                 * double-click matches for the
                                                 * following button press and
                                                 * release.
                                                 */
                                ...
                                if (!(buttonSpec->wmFunction(
                                        buttonSpec->wmFuncArgs, pCD,
                                        buttonEvent))) {
                                        /*
                                         * The window manager function return
                                         * indicates that further button binding
                                         * processing should not be done.
                                         */
        ...
        if (pCD) {
                ...
                if (actionWindow == pCD->clientFrameWin) {
                        *pContext = F_CONTEXT_WINDOW;
                        ...
                        if (eventOnRoot) {
                                clientX = event->x -
                                          (pCD->maxConfig ? pCD->maxX
                                                          : pCD->clientX) +
                                          pCD->clientOffset.x;
                                clientY = event->y -
                                          (pCD->maxConfig ? pCD->maxY
                                                          : pCD->clientY) +
                                          pCD->clientOffset.y;
                        } else {
                                clientX = event->x;
...
void ProcessClickBPress(XButtonEvent *buttonEvent, ClientData *pCD,
        ...
        if (!(pCD && (buttonEvent->window == pCD->clientBaseWin) &&
              passButton)) {
                wmGD.clickData.button = buttonEvent->button;
                wmGD.clickData.state = buttonEvent->state;
                /* add in event button mask (this will show up in the button
                 * release */
                wmGD.clickData.releaseState =
                    buttonEvent->state |
                ...
                if (clientEvent->data.l[0] == wmGD.xa_WM_SAVE_YOURSELF) {
                        for (scr = 0; scr < wmGD.numScreens; scr++) {
                                if (wmGD.Screens[scr].managed) {
                                        /*
                                         * Write out current workspace,
                                         * frontpanel position and iconbox
                                         * position and size.
                                         */
...
void HandleDtWmRequest(WmScreenData *pSD, XEvent *pev) {
        ...
        while (more) {
                ...
                if (pchReq) {
                        ...
                        _DtWmParseSetLine(wmGD.pWmPB,
                                          (unsigned char *)sRequest);
                        lineP = wmGD.pWmPB->pchLine;
                        iFuncIndex = ParseWmFunction(
                            &lineP, CRS_BUTTON | CRS_KEY, &wmFunction);

                        if (iFuncIndex != F_NOP_INDEX) {
                                if (functionTable[iFuncIndex].greyedContext &
                                    ctxDisallowed) {
                                        /*
                                         * Sorry, we have to disallow this
                                         * function request based on context
                                         * problems.
                                         */
                                ...
                                if ((*(functionTable[iFuncIndex].parseProc))(
                                         ...
                                         */
                                        if (wmFuncArgs) {
                                                if ((functionTable[iFuncIndex]
                                                         .parseProc ==
                                                     ParseWmFuncStrArg) ||
                                                    (functionTable[iFuncIndex]
                                                         .parseProc ==
                                ...
                                if (pch) {
                                        ...
                                        Warning(((char *)GETMESSAGE(
                                            32, 3,
                                            "Missing path name for backdrop "
                                            "change request.")));
                                }
                                if (pchFile) {
                                        ...
                                        if (pch) {
                                                ...
                                                SetNewBackdrop(ACTIVE_WS,
                                                               pixmap,
                                                               (String)pchFile);
                                        } else {
                                                Warning(((char *)GETMESSAGE(
                                                    32, 4,
                                                    "Missing pixmap id for "
                                                    "backdrop change "
                                                    "request.")));
                                        }
                                        XtFree(pchFile);
        ...
        if (!wmGD.menuActive &&
            ((leaveEvent->detail == NotifyNonlinear) ||
             (leaveEvent->detail == NotifyNonlinearVirtual))) {
                    ...
                    (wmGD.colormapFocusPolicy == CMAP_FOCUS_POINTER)) {
                        if (wmGD.keyboardFocusPolicy ==
                            KEYBOARD_FOCUS_POINTER) {
                                Do_Focus_Key(
                                    (ClientData *)NULL, leaveEvent->time,
                                    (SCREEN_SWITCH_FOCUS | ALWAYS_SET_FOCUS));
                        }
                        if (wmGD.colormapFocusPolicy == CMAP_FOCUS_POINTER) {
                                SetColormapFocus(ACTIVE_PSD,
                                                 (ClientData *)NULL);
                        }
                }
                ...
                if (!XFindContext(DISPLAY, leaveEvent->root,
...
void HandleWsConfigureRequest(XConfigureRequestEvent *configureEvent) {
        ...
        if (XFindContext(DISPLAY, configureEvent->window,
                         wmGD.windowContextType, (caddr_t *)&pCD)) {
                /*
                ...
                if (WmGetWindowAttributes(configureEvent->window)) {
                        ...
                        if (configChanged) {
                                ...
                                XConfigureWindow(
                                    DISPLAY, configureEvent->window,
                                    (unsigned int)(configureEvent->value_mask),
                                    &values);
                        }

                        /*
                         * Some clients expect a ConfigureNotify event even if
                         * the XConfigureWindow call has NO effect.  Send a
                         * synthetic ConfigureNotify event just to be sure.
                         */
                        ...
                        if (!configChanged) {
                                ...
                                notifyEvent.border_width =
                                    configureEvent->border_width;
                                notifyEvent.above = None;
                                notifyEvent.override_redirect = False;

                                XSendEvent(DISPLAY, configureEvent->window,
...
void HandleWsFocusIn(XFocusInEvent *focusEvent) {
         ...
         * control the focus). Reset the focus to a client window if
         * appropriate.
         */
        ...
        if (((focusEvent->mode == NotifyNormal) ||
            ...
            ((focusEvent->detail == NotifyPointerRoot) ||
             ...
             (focusEvent->detail == NotifyInferior))) {
                /*
                 * Fix the keyboard focus if it should be set to a particular
                 * client.
                 */
                ...
                if (wmGD.keyboardFocus &&
                    (focusEvent->detail != NotifyInferior)) {
                        if (sameScreen) {
                                /*
                                 * Assume that the focus still belongs to the
                                 * screen controlled by mwm.  Repair the focus
                                 * if the client is still active.
                                 */

                                if (wmGD.keyboardFocusPolicy ==
                                    KEYBOARD_FOCUS_EXPLICIT) {
                                        Do_Focus_Key(wmGD.keyboardFocus,
                                                     GetTimestamp(),
                                                     ALWAYS_SET_FOCUS);
                                } else {
                                        if (pCD || (focusEvent->detail ==
                                                    NotifyDetailNone)) {
                                                /* !!! check for redundant focus
                                                 * setting !!! */
                                                Do_Focus_Key(pCD,
                                                             GetTimestamp(),
                                                             ALWAYS_SET_FOCUS);
                                ...
                                SetKeyboardFocus((ClientData *)NULL,
                                                 REFRESH_LAST_FOCUS);
                        } else {
                                /*
                        ...
                        /*
                         * No client window currently has the focus.  If the
                         * pointer is on the mwm-controlled screen set the focus
                         * to the window management window if the focus is
                         * explicit.
                         */
                                        ...
                                        if (((focusEvent->detail ==
                                            ...
                                            (wmGD.keyboardFocus !=
                                             wmGD.nextKeyboardFocus)) {
                                                 ...
                                                 * went away.  Try to reset the
                                                 * window to the next keyboard
                                                 * focus client window if there
                                                 * is one.
                                                 */
                                                 ...
                                                 * pass NULL widget to this  */
                                                /* this routine.  This doesn't
                                                 * seem to make sense. NULL */
                                                /* has been replaced by pCD
                                                 * which seems to fix the icon
                                                 */
                                                /* focus problem. */
                                                /* Another related patch is made
                                                 * in WmCEvent.c.         */
                                                Do_Focus_Key((ClientData *)pCD,
                                                             GetTimestamp(),
                                                             ALWAYS_SET_FOCUS);
                                        }
...
void PullExposureEvents(void) {
        ...
        while (XCheckMaskEvent(DISPLAY, ExposureMask | VisibilityChangeMask,
                               &event) ||
               XCheckMaskEvent(DISPLAY1, ExposureMask | VisibilityChangeMask,
                               &event))
#else  /* WSM */
...
Boolean ReplayedButtonEvent(XButtonEvent *pevB1, XButtonEvent *pevB2) {
        ...
        if ((pevB1->type == pevB2->type) &&
            ...
            (pevB1->window == pevB2->window) && (pevB1->root == pevB2->root) &&
