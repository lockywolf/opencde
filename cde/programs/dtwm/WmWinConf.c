...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmWinConf.c /main/8 1996/10/30 11:15:17 drk $"
#endif
#endif
/*
...
void GetClipDimensions(ClientData *pCD, Boolean fromRoot) {
        ...
        if (fromRoot) {
                XtTranslateCoords(P_ICON_BOX(pCD)->scrolledWidget, tmpX, tmpY,
                                  &clipX, &clipY);
        } else {
                clipX = tmpX;
                clipY = tmpY;
        ...
        while (!moveDone) {
                ...
                if (firstTime) {
                        ...
                        GetConfigEvent(DISPLAY, grab_win, CONFIG_MASK,
                                       ...
                                       moveX, moveY, moveWidth, moveHeight,
                                       &event);
                }
                ...
                if (pev->type == KeyPress) {
                        ...
                        while (keyMultiplier <= big_inc &&
                               XCheckIfEvent(DISPLAY, &KeyEvent,
                                             IsRepeatedKeyEvent, (char *)pev)) {
                                keyMultiplier++;
                        }
                        ...
                        case XK_Right:
                                tmpX =
                                    keyMultiplier * ((control) ? big_inc : 1);
                                break;
                        ...
                        if (tmpX || tmpY) {
                                ...
                                ForceOnScreen(SCREEN_FOR_CLIENT(pcd), &warpX,
                                              &warpY);

                                if ((warpX != moveLastPointerX) ||
                                    (warpY != moveLastPointerY)) {
                                        SetPointerPosition(warpX, warpY, &newX,
                                                           &newY);
                                        ...
                                        /*
                                         * make like motion event and move
                                         * frame.
                                         */
                                        moveX += tmpX;
                                        moveY += tmpY;
                ...
                if (tmpX || tmpY) {
                        FixFrameValues(pcd, &moveX, &moveY, &moveWidth,
                                       &moveHeight, FALSE /* no size checks */);
                        if (pcd->pSD->moveOpaque) {
                                MoveOpaque(pcd, moveX, moveY, moveWidth,
                                           moveHeight);
                        } else {
                                MoveOutline(moveX, moveY, moveWidth,
                                            moveHeight);
                        }

                        if (!wmGD.movingIcon &&
                            (wmGD.showFeedback & WM_SHOW_FB_MOVE)) {
                                DoFeedback(pcd, moveX, moveY, moveWidth,
                                           moveHeight, (unsigned long)0,
...
void HandleClientFrameResize(ClientData *pcd, XEvent *pev) {
        ...
        while (!resizeDone) {
                if (!pev) /* first time through will already have event */
                {
                        ...
                        GetConfigEvent(DISPLAY, grab_win, CONFIG_MASK, pointerX,
                                       pointerY, resizeX, resizeY, resizeWidth,
                                       resizeHeight, &event);
                }
                ...
                if (pev->type == MotionNotify) {
                        ...
                        /*
                         * Really start resizing once the pointer hits a resize
                         * area (This only applies to accelerator and keyboard
                         * resizing!)
                         */
                        if (!wmGD.configSet &&
...
Boolean HandleResizeKeyPress(ClientData *pcd, XEvent *pev) {
        ...
        case XK_Left:
                ...
                case FRAME_NONE:
                        ...
                        warpX = resizeX + ((control) ? (-resizeBigWidthInc)
                                                     : (-pcd->widthInc));
                        break;
        ...
        case XK_Up:
                ...
                case FRAME_NONE:
                        ...
                        warpY = resizeY + ((control) ? (-resizeBigHeightInc)
                                                     : (-pcd->heightInc));
                        ReGrabPointer(pcd->clientFrameWin, pev->xkey.time);
                        break;
                ...
                default:
                        warpX = pointerX;
                        warpY = pointerY +
                                ((control) ? (-resizeBigHeightInc * keyMult)
...
void DoFeedback(ClientData *pcd, int x, int y, unsigned int width,
        ...
        /*
         * Put size in client specific units.  Do not include base into
         * calculations when increment is not specified (i.e. = 1).
         */
...
Boolean CheckVisualPlace(ClientData *pCD, int tmpX, int tmpY) {
        ...
        XTranslateCoordinates(DISPLAY, XtWindow(P_ICON_BOX(pCD)->bBoardWidget),
                              ROOT_FOR_CLIENT(pCD), tmpX, tmpY, &newX, &newY,
                              &child);
        ...
        if (((int)newX) >
            ((int)clipX + (int)clipWidth - ((int)ICON_WIDTH(pCD)))) {
                return (False);
        }
        if (((int)newY) >
            ((int)clipY + (int)clipHeight - ((int)ICON_HEIGHT(pCD)))) {
                return (False);
        }
...
void CompleteFrameConfig(ClientData *pcd, XEvent *pev) {
        ...
        if (wmGD.configAction == RESIZE_CLIENT) {
...
#else
                FrameToClient(pcd, &resizeX, &resizeY, &resizeWidth,
                              &resizeHeight);
#endif
                ...
                ProcessNewConfiguration(pcd, resizeX, resizeY, resizeWidth,
                                        resizeHeight, FALSE);
                ...
                if (wmGD.movingIcon) {
                        ...
                        if (wmGD.iconAutoPlace || inIconBox) {
                                ...
                                if (place != ICON_PLACE(pcd)) {
                                        ...
                                        /*
                                         * Replace icon into same place - as if
                                         * it didn't move.
                                         */
                                        XMoveWindow(DISPLAY,
                                                    ICON_FRAME_WIN(pcd),
                                ...
                                XMoveWindow(DISPLAY, ICON_FRAME_WIN(pcd), moveX,
                                            moveY);
                                ICON_X(pcd) = moveX;
                                ICON_Y(pcd) = moveY;
                        }
                        ...
                        ProcessNewConfiguration(
                            ...
                            moveX, moveY,
#else
                            moveX + offsetX, moveY + offsetY,
#endif
                            (unsigned int)(moveWidth - 2 * offsetX),
                            (unsigned int)(moveHeight - offsetX - offsetY),
                            FALSE);
                }
        }
#ifdef WSM
...
static void CreateOutlineWindows(WmScreenData *pSD) {
        ...
        xswamask =
            (CWOverrideRedirect | CWBackingStore | CWBackPixmap | CWSaveUnder);

        pSD->woN =
            XCreateWindow(DISPLAY, pSD->rootWindow, x, y, width, height, 0,
...
void DrawOutline(int x, int y, unsigned int width, unsigned int height) {
        ...
        if (lastOutlineWidth || lastOutlineHeight) {
                SetOutline(outline, lastOutlineX, lastOutlineY,
                           lastOutlineWidth, lastOutlineHeight, OUTLINE_WIDTH);
...
void ProcessNewConfiguration(ClientData *pCD, int x, int y, unsigned int width,
        ...
        if (pCD->maxConfig) {
                ...
                if (!toNewMax && (changedValues & CWWidth)) {
                        ...
                        if (changedValues & CWHeight) {
                                /*
                                 * Hacked to update maxHeight for 'horizontal'
                                 * max client
                                 */
                        ...
                        /*
                         * Hacked to update maxHeight for 'horizontal' max
                         * client
                         */
                        if (IS_MAXIMIZE_HORIZONTAL(pCD)) {
                                pCD->maxHeight = height;
        ...
        if (changedValues & (CWWidth | CWHeight)) {
                if (pCD->maxConfig) {
                        if (!toNewMax) {
                                /* maximized window resized, return to normal
                                 * state */
                                pCD->maxConfig = FALSE;
        ...
        if (changedValues & (CWX | CWY)) {
                if (pCD->maxConfig) {
                        /*
                         * Fix for 5217 - If the request is from the client, use
                         * the clients offsets instead of the static offsets
                         */
                        if (clientRequest) {
...
Boolean StartResizeConfig(ClientData *pcd, XEvent *pev) {
        ...
        if (!configGrab) {
                ...
                if (pev) {
                        grabbed = DoGrabs(
                            grab_win, ConfigCursor((int)wmGD.configPart),
                            PGRAB_MASK, pev->xbutton.time, pcd, True);
                } else {
                        grabbed = DoGrabs(grab_win,
...
void StartClientResize(ClientData *pcd, XEvent *pev) {
        ...
        if (!pev || pev->type == KeyPress) {
                ...
                if (wmGD.enableWarp) {
                        XWarpPointer(DISPLAY, None, ROOT_FOR_CLIENT(pcd), 0, 0,
                                     0, 0, pointerX, pointerY);
                }
        }
...
Boolean StartClientMove(ClientData *pcd, XEvent *pev) {
        ...
        if (!configGrab) {
                ...
                if (pev) {
                        grabbed =
                            DoGrabs(grab_win, wmGD.configCursor, PGRAB_MASK,
        ...
        if (pev &&
            ((pev->type == ButtonPress) || (pev->type == ButtonRelease))) {
                ...
                if (!XQueryPointer(DISPLAY, ROOT_FOR_CLIENT(pcd), &junk_win,
                                   &junk_win, &(wmGD.preMoveX),
                                   &(wmGD.preMoveY), &junk, &junk,
                                   (unsigned int *)&junk)) {
                        CancelFrameConfig(pcd);
                        return (False);
                }
        ...
        /*
         *  Warp pointer to middle of window if started from the menu (no
         * event).
         */
        if (!pev || pev->type == KeyPress) {
                ...
                ForceOnScreen(SCREEN_FOR_CLIENT(pcd), &moveLastPointerX,
                              &moveLastPointerY);
                if (wmGD.enableWarp) {
                        XWarpPointer(DISPLAY, None, ROOT_FOR_CLIENT(pcd), 0, 0,
                                     0, 0, moveLastPointerX, moveLastPointerY);
                }
        }
 ...
 *
 *************************************<->***********************************/
void SetGrabServer(void) { grabServer = FALSE; }
#endif /* !defined(WSM) || defined(MWM_QATS_PROTOCOL) */
...
Boolean DoGrabs(Window grab_win, Cursor cursor, unsigned int pmask,
        ...
        if (!wmGD.menuActive) {
                ...
                /*
                 * If running automation version of mwm, do not grab the server,
                 * since this will confuse the automation input synthesis code.
                 */
#if ((!defined(WSM)) || defined(MWM_QATS_PROTOCOL))
        if (grabServer == TRUE)
#endif /* !defined(WSM) || defined(MWM_QATS_PROTOCOL) */
...
void CancelFrameConfig(ClientData *pcd) {
        ...
        if (pcd) {
...

                {
                        ...
                        } else if (!wmGD.movingIcon) /* we are not moving in the
                                                        iconbox */
                        {
                                XMoveWindow(DISPLAY, pcd->clientFrameWin,
        ...
        if (wmGD.configAction == MARQUEE_SELECT) {
                dtSendMarqueeSelectionNotification(ACTIVE_PSD,
                                                   DT_MARQUEE_SELECT_CANCEL,
        ...
        if (pcd)
                ...
                if (!anyMotion && wmGD.enableWarp) {
                        XWarpPointer(DISPLAY, None, ROOT_FOR_CLIENT(pcd), 0, 0,
                                     0, 0, wmGD.preMoveX, wmGD.preMoveY);
                }
        anyMotion = FALSE;
...
void CheckEatButtonRelease(ClientData *pcd, XEvent *pev) {
        ...
        if ((pev->type == KeyPress || pev->type == KeyRelease) &&
            (pev->xbutton.state & ButtonMask)) {
                 ...
                 */
                if (XGrabPointer(
                        DISPLAY, grab_win, False,         /* owner_events */
...
void EatButtonRelease(unsigned int releaseButtons) {
        ...
        while (releaseButtons) {
                ...
                if (event.type == ButtonRelease) {
                        ...
                        new_state = event.xbutton.state &
                                    ~ButtonStateBit(event.xbutton.button);
...
#define MAX_BUTTON 5
        ...
        static ButtonAssoc bmap[MAX_BUTTON] = {
            {Button1, Button1Mask}, {Button2, Button2Mask},
            {Button3, Button3Mask}, {Button4, Button4Mask},
            {Button5, Button5Mask},
        };
...
int ResizeType(ClientData *pcd, XEvent *pev) {
        ...
        if (x <= resizeX) {
                ...
                else if (y >= resizeY + resizeHeight -
...
void FixFrameValues(ClientData *pcd, int *pfX, int *pfY, unsigned int *pfWidth,
        ...
        if (wmGD.movingIcon && P_ICON_BOX(pcd)) {
                ...
                if (((int)*pfX) >
                    ((int)clipX + (int)clipWidth - ((int)*pfWidth))) {
                        *pfX = clipX + clipWidth - *pfWidth;
                }

                /* bottom edge of outline */
                if (((int)*pfY) >
                    ((int)clipY + (int)clipHeight - ((int)*pfHeight))) {
...
void SetPointerPosition(int newX, int newY, int *actualX, int *actualY) {
         ...
         */
        if (wmGD.enableWarp) {
                XWarpPointer(DISPLAY, None, ACTIVE_ROOT, 0, 0, 0, 0, newX,
                             newY);
        }
        ...
        {
                /* failed to get pointer position or not warping, return
                 * something */
                *actualX = newX;
                *actualY = newY;
        }
        ...
        while (!eventToReturn) {
                ...
                if (!gotEvent && (polling || !wmGD.freezeOnConfig)) {
                        ...
                        while (True) {
                                ...
                                if (!wmGD.freezeOnConfig &&
                                    !wmGD.pActiveSD->moveOpaque) {
                                        /* flash the outline if server is not
                                         * grabbed */
                                        MoveOutline(oX, oY, oWidth, oHeight);
                                }

                                if (!XQueryPointer(display, window, &root_ret,
                                                   &child_ret, &root_x, &root_y,
                                ...
                                if ((root_x != curX) || (root_y != curY)) {
                                        ...
                                        if (!(--pollCount)) {
                                                /*
                                                 * No pointer motion in some
                                                 * time. Stop polling and wait
                                                 * for next event.
                                                 */
                                                polling = False;
                                                break; /* from while loop */
                            ...
                            pev->xmotion.is_hint == NotifyHint) {
                                ...
                                if ((XQueryPointer(display, window, &root_ret,
                                    ...
                                    ((root_x != curX) || (root_y != curY))) {
                                        /*
                                         * The query pointer values say that the
                                         * pointer moved to a new location.
                                         */
                                        pev->xmotion.window = root_ret;
                                        pev->xmotion.subwindow = child_ret;
                                        pev->xmotion.x = root_x;
                                        ...
                                        /*
                                         * Query failed. Change curX to force
                                         * position to be returned on first
                                         * sucessful query.
                                         */
                                        eventToReturn = False;
                                        curX++;
...
void HandleMarqueeSelect(WmScreenData *pSD, XEvent *pev) {
        ...
        while (!bDone && (wmGD.configAction == MARQUEE_SELECT)) {
                ...
                if (!bDone) {
                        MoveOutline(marqueeX, marqueeY, marqueeWidth,
                                    marqueeHeight);
                }
...
void StartMarqueeSelect(WmScreenData *pSD, XEvent *pev) {
        ...
        if ((pev->type == ButtonPress) || (pev->type == ButtonRelease)) {
                pointerX = pev->xbutton.x_root;
                pointerY = pev->xbutton.y_root;
        } else if (!XQueryPointer(DISPLAY, pSD->rootWindow, &junk_win,
                                  &junk_win, &pointerX, &pointerY, &junk, &junk,
                                  (unsigned int *)&junk)) {
...
void UpdateMarqueeSelectData(WmScreenData *pSD) {
        ...
        if ((wmGD.marqueeSelectGranularity > 0) &&
            ((ABS(marqueeWidth - marqueeWidth0) >
              wmGD.marqueeSelectGranularity) ||
             (ABS(marqueeHeight - marqueeHeight0) >
              wmGD.marqueeSelectGranularity))) {
                dtSendMarqueeSelectionNotification(
                    pSD, DT_MARQUEE_SELECT_CONTINUE, marqueeX, marqueeY,
                    marqueeWidth, marqueeHeight);
