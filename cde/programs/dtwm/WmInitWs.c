...
static void InitMouseBinding(void) {
        wmGD.numMouseButtons =
            XGetPointerMapping(DISPLAY, (unsigned char *)0, 0);
...
static void SetupLockingModifierMask(void) {
        ...
        for (i = start_index; i < modifier_map->max_keypermod * 8; i++) {
                int this_mod =
                    ((i - start_index) / modifier_map->max_keypermod) + 1;
...
void InitWmGlobal(int argc, char *argv[], char *environ[]) {
        ...
        wmGD.display =
            XtOpenDisplay(wmGD.mwmAppContext, NULL, wmGD.mwmName, res_class,
                          NULL, 0, &argc, /* R5 changed from Cardinal to int*/
                          argv);
...
#if defined(sun) && defined(ALLPLANES)
        {
                ...
                wmGD.allplanes =
                    XAllPlanesQueryExtension(wmGD.display, &dummy, &dummy);
        }
        ...
        argnum++;
#else
        XtSetArg(args[argnum], XtNrestartStyle, SmRestartIfRunning);
        argnum++;
#endif
         ...
         */
        {
                ...
                XmeGetIconControlInfo(ScreenOfDisplay(DISPLAY, sNum),
                                      &useMaskRtn, &useMultiColorIcons,
                                      &useIconFileCacheRtn);
        ...
        if (!(wmGD.Screens = (WmScreenData *)XtCalloc(wmGD.numScreens,
                                                      sizeof(WmScreenData)))) {
                ShowWaitState(FALSE);
                Warning(((char *)GETMESSAGE(
                    40, 2, "Insufficient memory for Screen data")));
                ExitWM(WM_ERROR_EXIT_VALUE);
        } else {
                ...
                for (scr = 0; scr < wmGD.numScreens; scr++) {
                        ...
                        sNum = (wmGD.numScreens == 1) ? DefaultScreen(DISPLAY)
                                                      : scr;
                        wmGD.errorFlag = False;

                        XChangeWindowAttributes(DISPLAY,
                                                RootWindow(DISPLAY, sNum),
                             ...
                             X_ChangeWindowAttributes)) {
                                ...
                                if (!processedGlobalResources) {
                                        ...
                                        static char *atom_names[] = {
                                            _XA_DT_SESSION_HINTS,
                                            _XA_DT_SM_WM_PROTOCOL,
                                            _XA_DT_SM_START_ACK_WINDOWS,
                                            _XA_DT_SM_STOP_ACK_WINDOWS,
                                            _XA_DT_WM_WINDOW_ACK,
                                            _XA_DT_WM_EXIT_SESSION,
                                            _XA_DT_WM_LOCK_DISPLAY,
                                            _XA_DT_WM_READY};

                                        Atom atoms[XtNumber(atom_names)];
                                        XInternAtoms(DISPLAY, atom_names,
                                                     XtNumber(atom_names),
                                                     False, atoms);

                                        wmGD.xa_DT_SESSION_HINTS =
                                            atoms[XA_DT_SESSION_HINTS];
                                        wmGD.xa_DT_SM_WM_PROTOCOL =
                                            atoms[XA_DT_SM_WM_PROTOCOL];
                                        ...
                                        wmGD.xa_DT_WM_WINDOW_ACK =
                                            atoms[XA_DT_WM_WINDOW_ACK];
                                        wmGD.xa_DT_WM_EXIT_SESSION =
                                            atoms[XA_DT_WM_EXIT_SESSION];
                                        wmGD.xa_DT_WM_LOCK_DISPLAY =
                                            atoms[XA_DT_WM_LOCK_DISPLAY];
                                        wmGD.xa_DT_WM_READY =
                                            atoms[XA_DT_WM_READY];
#endif /* WSM */
#ifndef NO_OL_COMPAT
                                        InitOLCompat();
#endif /* NO_OL_COMPAT */
#ifndef NO_SHAPE
                                        wmGD.hasShape = XShapeQueryExtension(
                                            DISPLAY, &wmGD.shapeEventBase,
                                            &wmGD.shapeErrorBase);
#endif /*  NO_SHAPE  */
                                        ...
                                        wmGD.clickData.doubleClickPending =
                                            False;
                                         ...
                                         *  internal WM windows.
                                         */
                                        wmGD.display1 = XtOpenDisplay(
                                            wmGD.mwmAppContext, NULL,
                                            wmGD.mwmName, res_class, NULL, 0,
                                            &dpy2Argc, dpy2Argv);
                                        if (!wmGD.display1) {
                                                ShowWaitState(FALSE);
                                                Warning(((char *)GETMESSAGE(
                                                    40, 4,
                                                    "Could not open second "
                                                    "display connection.")));
                                                ExitWM(WM_ERROR_EXIT_VALUE);
                                        }
...
#ifdef PANACOMM
                                        /*
                                         * If this is the first screen we've
                                         * managed, tell the session manager
                                         * we're ready
                                         */
                                        if (!processedGlobalResources) {
                                                SendClientMsg(
                                        ...
                                        /* create topmost shell (application
                                         * shell) */
                                        argnum = 0;
                                        XtSetArg(args[argnum], XtNgeometry,
                                                 NULL);
                                        argnum++;
                                        XtSetArg(args[argnum], XtNx, 10000);
                                        argnum++;
                                        ...
                                        wmGD.topLevelW1 = XtAppCreateShell(
                                            NULL, res_class,
                                        ...
                                        /*
                                         * if this is the first screen we can
                                         * manage, process global.
                                         */

                                        processedGlobalResources = True;

                                        /*
                                         * Get the _MOTIF_WM_INFO property and
                                         * determine the startup / restart
                                         * state.
                                         */
...

        {
                ...
                Atom atoms[XtNumber(atom_names)];
                XInternAtoms(DISPLAY, atom_names, XtNumber(atom_names), False,
                             atoms);
                ...
                wmGD.xa_DT_EMBEDDED_CLIENTS =
                    atoms[XA_DT_WORKSPACE_EMBEDDED_CLIENTS];
        ...
        for (scr = 0; scr < wmGD.numScreens; scr++) {
                ...
                if (pSD->managed) {
                        ...
                        if (XDefaultScreen(wmGD.display) == pSD->screen) {
                                wmGD.commandWindow =
                                    wmGD.Screens[scr].wmWorkspaceWin;
                        }
                        ...
                        /*
                         * Process the window manager resource description file
                         * (.mwmrc):
                         */
                        ...
                        /*
                         * Setup default resources for the system menu and key
                         * bindings:
                         */
                        ...
                        /*
                         * Make menus and other resources that are used by
                         * window manager functions that are activated by menus,
                         * buttons and keys.
                         */
         ...
         */
        {
                for (scr = 0; scr < wmGD.numScreens; scr++) {
                        ...
                        if (pSD->managed) {
                                ...
                                /*
                                 * Adopt client windows that exist before wm
                                 * startup:
                                 */
                                ...
                                /*
                                 * Setup initial keyboard focus and colormap
                                 * focus:
                                 */
                ...
                for (scr = 0; scr < wmGD.numScreens; scr++) {
                        ...
                        if (pSD->managed) {
                                ...
                                if (wmGD.useFrontPanel && (pSD == wmGD.dtSD)) {
                                        ...
                                        pPanelist =
                                            (WmPanelistObject)pSD->wPanelist;

                                        if (pPanelist != NULL &&
                                            O_Shell(pPanelist)) {
                                                /*
                                                 * Make a default front panel
                                                 * icon image.
                                                 */
                                                iconBitmap =
                                ...
                                if (wmGD.useFrontPanel && pSD->wPanelist &&
                                    (pSD == wmGD.dtSD)) {
                                        ...
                                        /*
                                         * Find special clients associated with
                                         * the front panel. This needs to be
                                         * done after WmPanelistShow where the
                                         * data is set up.
                                         */
...
#define LENCBUFFER 256
        ...
        if (!(strcmp((char *)wmGD.screenNames[sNum],
                ...
                if ((wmGD.screenNames[sNum] = (unsigned char *)XtRealloc(
                        ...
                        Warning(
                            ((char *)GETMESSAGE(40, 7,
                                                "Cannot create enough memory "
        ...
        pSD->screenTopLevelW = XtCreatePopupShell(
            (String)wmGD.screenNames[sNum], vendorShellWidgetClass,
         ...
         */
        pSD->utmShell =
            XmCreateDrawingArea(pSD->screenTopLevelW, "UTM_Shell", NULL, 0);
        ...
        pSD->screenTopLevelW1 = XtCreatePopupShell(
            (String)wmGD.screenNames[sNum], vendorShellWidgetClass,
        ...
        if (pSD->initialWorkspace) {
                ...
                for (iwsx = 0; iwsx < pSD->numWorkspaces; iwsx++) {
                        if (!strcmp(pSD->pWS[iwsx].name,
                                    pSD->initialWorkspace)) {
                                break;
                        }
                }
                /* check bounds */
                if (iwsx >= pSD->numWorkspaces) {
                        /* make first workspace in list the active one to start
                         * with */
        ...
        if (!(pSD->pWS =
                  (WmWorkspaceData *)XtMalloc(sizeof(WmWorkspaceData)))) {
                ShowWaitState(FALSE);
                Warning(((char *)GETMESSAGE(
                    40, 8, "Insufficient memory for Workspace data")));
                ExitWM(WM_ERROR_EXIT_VALUE);
        }
...
#define DEFAULT_WS_NAME "workspace"
        ...
        XtSetArg(args[argnum], XtNcolormap,
                 DefaultColormap(DISPLAY, pSD->screen));
        argnum++;
        ...
        pWS->workspaceTopLevelW =
            XtCreateWidget(pWS->name, xmPrimitiveWidgetClass,
...
static void InsureDefaultBackdropDir(char **ppchBackdropDirs) {
        ...
        while (!bFound && (pch != NULL) && (*pch != 0)) {
                if (strncmp(pch, pchD, len) == 0) {
                        ...
                        pch2 = pch + len;
                        if ((pch2 <= pchEnd) &&
                            ((*pch2 == 0) || (((mblen(pch2, MB_CUR_MAX) == 1) &&
                                               (*pch2 == ':'))))) {
                                bFound = True;
                        }
                } else {
        ...
        if ((pMwmInfo = (MotifWmInfo *)GetMwmInfo(rootWindowOfScreen)) !=
            NULL) {
                wmGD.useStandardBehavior =
                    (pMwmInfo->flags & MWM_INFO_STARTUP_STANDARD) ? True
                                                                  : False;
...
void SetupWmWorkspaceWindows(void) {
        ...
        for (scr = 0; scr < wmGD.numScreens; scr++) {
                ...
                if (pSD->managed) {
                        sAttributes.override_redirect = True;
                        sAttributes.event_mask =
                            FocusChangeMask | PropertyChangeMask;
                        pSD->wmWorkspaceWin = XCreateWindow(
                            DISPLAY, pSD->rootWindow, -100, -100, 10, 10, 0, 0,
                            InputOnly, CopyFromParent,
                            (CWOverrideRedirect | CWEventMask), &sAttributes);
...
void MakeWmFunctionResources(WmScreenData *pSD) {
        ...
        while (buttonSpec) {
                if (buttonSpec->wmFunction == F_Menu) {
                        ...
                        menuSpec = MAKE_MENU(pSD, NULL, buttonSpec->wmFuncArgs,
                                             menuContext, buttonSpec->context,
                                             (MenuItem *)NULL, FALSE);
        ...
        while (keySpec) {
                if (keySpec->wmFunction == F_Menu) {
                        ...
                        menuSpec = MAKE_MENU(pSD, NULL, keySpec->wmFuncArgs,
                                             menuContext, keySpec->context,
                                             (MenuItem *)NULL, FALSE);
...
void CopyArgv(int argc, char *argv[]) {
        ...
        if ((wmGD.argv = (char **)XtMalloc((argc + 1) * sizeof(char *))) ==
            NULL) {
                Warning(((char *)GETMESSAGE(
                ...
                wmGD.argv[i] = NULL;
#ifdef WSM
                if ((dpy2Argv = (char **)XtMalloc((argc + 1) *
                                                  sizeof(char *))) == NULL) {
                        Warning(((char *)GETMESSAGE(
                            40, 11,
...
void InitScreenNames(void) {
        ...
        if (!(wmGD.screenNames =
                ...
                ShowWaitState(FALSE);
                Warning(((char *)GETMESSAGE(
                    40, 12, "Insufficient memory for screen names")));
                ExitWM(WM_ERROR_EXIT_VALUE);
        }
        ...
        for (num = 0; num < numScreens; num++) {
                if (!(wmGD.screenNames[num] =
                        ...
                        ShowWaitState(FALSE);
                        Warning(((char *)GETMESSAGE(
                            40, 13, "Insufficient memory for screen names")));
                        ExitWM(WM_ERROR_EXIT_VALUE);
                }
...
static void SetBindingsProperty(Display *dsp, Atom property, String binding) {
        XChangeProperty(dsp, RootWindow(dsp, 0), property, XA_STRING, 8,
                        PropModeReplace, (unsigned char *)binding,
                        strlen(binding));
}
...
static String FixupBindingsString(String bindingsString, str_xref repl_xref) {
        ...
        while (repl_xref->default_name != NULL) {
                ...
                if (ks_ptr == NULL) {
                        /* Only increment to next replacement when no other
                         * instances are found in fixed_str.
                         */
                        ++repl_xref;
                        ptr_next = fixed_str;
                        continue;
                ...
                if ((strpbrk((ks_ptr - 1), " \t>") == (ks_ptr - 1)) &&
                    (strpbrk(ks_ptr, " \t\n") == (ks_ptr + orig_len))) {
                        ...
                        if (new_len > orig_len) {
                                unsigned new_ttl_len =
                                    strlen(fixed_str) + new_len - orig_len;
                                unsigned prefix_len;

                                *ks_ptr = '\0';
                                prefix_len = strlen(fixed_str);
                                fixed_str =
                                    XtRealloc(fixed_str, (new_ttl_len + 1));
                                ks_ptr = fixed_str + prefix_len;
                        }
                        memmove((ks_ptr + new_len), (ks_ptr + orig_len),
                                (suffix_len + 1));
...
Boolean VirtKeys4DIN(Display *dsp) {
        ...
        while (i < num_props) {
                if (GetBindingsProperty(dsp, prop_atoms[i], &bindingsString)) {
                        String new_bstring =
                            FixupBindingsString(bindingsString, vkeysym_xref);
                        prop_existed = TRUE;
                        XFree(bindingsString);

                        if (new_bstring != NULL) {
                                SetBindingsProperty(dsp, prop_atoms[i],
                                                    new_bstring);
                                XtFree(new_bstring);
                        }
                }
        ...
        if (!prop_existed) {
                ...
                i = 0;
                while (i < num_props) {
                        if (GetBindingsProperty(dsp, prop_atoms[i],
                                                &bindingsString)) {
                                String new_bstring = FixupBindingsString(
                                    bindingsString, vkeysym_xref);
                                XtFree(bindingsString);
                                if (new_bstring != NULL) {
