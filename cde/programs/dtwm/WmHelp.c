...
static int LocateTheControl(Display *dpy, int screen, Widget *pWidget);
static WmScreenData *WmScreenDataFromWidget(Widget theWidget);

static int WmDtReturnSelectedWidget(Widget theWidget, Cursor cursor,
                                    Widget *pWidget);
static void WmDtDisplayTopic(Widget parent, char *accessPath, char *idString,
                             int helpType, Widget wmClient, Boolean wmHelp,
                             ArgList argList, int argCount, ClientData *pCData,
                             Boolean hyperLink,
                             DisplayTopicInfo *displayTopicInfo);
static void WmDtCloseHelpCB(Widget w, XtPointer clientData, XtPointer callData);
static void WmDtWmCloseHelpCB(Widget w, XtPointer clientData,
                              XtPointer callData);
static void WmDtVersionCloseCB(Widget w, XtPointer clientData,
                               XtPointer callData);
static void WmDtVersionPopupCB(Widget w, XtPointer clientData,
                               XtPointer callData);

static void WmDtProcessLinkCB(Widget w, XtPointer clientData,
                              XtPointer callData);
static Boolean WmDtGetFromCache(CacheListStruct **pCurrentNode,
                                WmScreenData *pSD);
static void ShowHelpDialog(ClientData *pClient);

static void HideHelpDialog(WmScreenData *pSD, Boolean userDismissed);
static void wmDtHelpSetPosition(PtrWsDtHelpData pHelp, WmScreenData *pSD,
                                Boolean restorePos);
static void wmDtHelpMapEventHandler(Widget, XtPointer, XEvent *, Boolean *);
static void wmDtErrorDialog(WmScreenData *pSD, String sMessage, String sTitle);
static void wmDtErrorDialogOkCB(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void wmDtErrorDialogPopupCB(Widget w, XtPointer client_data,
                                   XtPointer call_data);
static void wmDtErrorDialogHide(WmScreenData *pSD);
...
static int LocateTheControl(Display *dpy, int screen, Widget *pWidget) {
        ...
        status =
            XGrabPointer(wmGD.display, RootWindow(wmGD.display, screen), False,
                         ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
                         GrabModeAsync, None, cursor, CurrentTime);
        if (status != GrabSuccess) {
                Warning(
                    ((char *)GETMESSAGE(29, 1,
                                        "Help on Item failed. Another program "
                                        "may have grabbed the mouse.")));
                return (DtHELP_SELECT_ERROR);
        }

        /* Grab the Keyboard so we can catch the ESC button press */
        status =
            XGrabKeyboard(wmGD.display, RootWindow(wmGD.display, screen), False,
                          GrabModeAsync, GrabModeAsync, CurrentTime);
        if (status != GrabSuccess) {
                XUngrabPointer(wmGD.display, CurrentTime);
                Warning(
                    ((char *)GETMESSAGE(29, 2,
                                        "Help on Item failed. Another program "
                                        "may have grabbed the mouse.")));
                return (DtHELP_SELECT_ERROR);
        }
...
void WmDtStringHelpCB(Widget theWidget, XtPointer client_data,
                      XtPointer call_data) {
        WmScreenData *pSD;
        ...
        WmDtDisplayTopic(pSD->screenTopLevelW1, NULL, theHelpString,
                         DtHELP_TYPE_DYNAMIC_STRING, (Widget)NULL, True, NULL,
                         0, NULL, False, NULL);
...
void WmDtWmStringHelpCB(Widget theWidget, XtPointer client_data,
                        XtPointer call_data) {
        WmScreenData *pSD;
        char *theHelpString = (char *)client_data;
        pSD = WmScreenDataFromWidget(theWidget);
        ...
        if (XtDisplay(theWidget) == DISPLAY) {
                WmDtDisplayTopic(pSD->screenTopLevelW1, NULL, theHelpString,
                                 DtHELP_TYPE_STRING, theWidget, True, NULL, 0,
                                 NULL, False, NULL);
        } else {
...
#endif /* OLD */
        WmDtDisplayTopic(pSD->screenTopLevelW1, NULL, theHelpString,
                         DtHELP_TYPE_DYNAMIC_STRING, theWidget, True, NULL, 0,
                         NULL, False, NULL);
...
void WmDtDisplayTopicHelp(Widget widget, char *helpVolume, char *locationID) {
        WmScreenData *pSD = WmScreenDataFromWidget(widget);

        WmDtDisplayTopic(pSD->screenTopLevelW1, helpVolume, locationID,
                         DtHELP_TYPE_TOPIC, (Widget)NULL, True, NULL, 0, NULL,
                         False, NULL);
}
...
void WmDtDisplayStringHelp(Widget widget, char *helpString) {
        WmScreenData *pSD = WmScreenDataFromWidget(widget);

        WmDtDisplayTopic(pSD->screenTopLevelW1, NULL, helpString,
                         DtHELP_TYPE_DYNAMIC_STRING, widget, True, NULL, 0,
                         NULL, False, NULL);
}
...
void WmDtWmTopicHelpCB(Widget theWidget, XtPointer client_data,
                       XtPointer call_data) {
        Widget shellWidget;
        WmScreenData *pSD;
        ...
        WmDtDisplayTopic(pSD->screenTopLevelW1, helpVolume, locationID,
                         DtHELP_TYPE_TOPIC, shellWidget, True, NULL, 0, NULL,
                         False, NULL);
 ...
 *
 ***************************************************************************/
static void WmDtCloseHelpCB(Widget w, XtPointer clientData,
                            XtPointer callData) {
        WmScreenData *pSD = NULL;
        int scr;
        ...
        if (pTemp == NULL) {
                /* ERROR */
                /* printf("Didn't find our help dialog widget in the cache
                 * list??? /n");*/
        }
 ...
 *
 ***************************************************************************/
static void WmDtWmPopupHelpCB(Widget w, XtPointer clientData,
                              XtPointer callData) {
...
static void WmDtVersionPopupCB(Widget w, XtPointer clientData,
        ...
        if (w) {
#ifdef OLD
                n = 0;
                XtSetArg(setArgs[n], XmNx, 0);
                n++;
                XtSetArg(setArgs[n], XmNy, 0);
                n++;
#endif /* OLD */
                ...
                n = 0;
                XtSetArg(getArgs[n], XmNheight, &height);
                n++;
                XtSetArg(getArgs[n], XmNwidth, &width);
                n++;
                XtGetValues(w, (ArgList)getArgs, n);

                x = (DisplayWidth(DISPLAY, XScreenNumberOfScreen(XtScreen(w))) -
                     ((int)width)) /
                    2;
                y = (DisplayHeight(DISPLAY,
                                   XScreenNumberOfScreen(XtScreen(w))) -
                     ((int)height)) /
                    2;
                n = 0;
                XtSetArg(setArgs[n], XmNx, (XtArgVal)x);
                n++;
                XtSetArg(setArgs[n], XmNy, (XtArgVal)y);
                n++;
 ...
 *
 ***************************************************************************/
static void WmDtPopupHelpCB(Widget helpShellW, XtPointer clientData,
                            XtPointer callData) {
        char *ptr;
        ...
        n++;
        XtSetArg(args[n], XmNwaitForWm, False);
        n++;
        ...
        if (pTemp) {
                if (pTemp->workspaces) {
                        ...
                        do {
                                ...
                                atom_names = (char **)XtRealloc(
                                    (char *)atom_names,
                                    sizeof(char *) * (num_workspaces + 1));
                                atom_names[num_workspaces] =
                                    XtNewString(pTemp->workspaces);
                                num_workspaces++;
                ...
                XtSetArg(args[n], XmNx, pTemp->xPos);
                n++;
                XtSetArg(args[n], XmNy, pTemp->yPos);
                n++;
                pTemp->restored = False;
                XtRemoveCallback(helpShellW, XmNpopupCallback, WmDtPopupHelpCB,
                                 clientData);
                XtAddCallback(helpShellW, XmNpopupCallback, WmDtPopupHelpCB,
                              NULL);
        } else {
...

{
        ...
        case DtHELP_LINK_JUMP_NEW:
                WmDtDisplayTopic(
                    XtParent(w), hyperData->helpVolume, hyperData->locationId,
                    /* DtHELP_LINK_JUMP_NEW */ DtHELP_TYPE_TOPIC, (Widget)NULL,
                    False, NULL, 0, NULL, True, NULL);
                break;

        default: /* This catches the application definded link types */
                if (hyperData->hyperType >= 100) {
                        /* The application would put any code here to handle
                         * their Applicaiton-defined-links.
                         */
                } else {
                        /* ERROR */
                        /* printf("We some how got a bogus hyptertext link
                         * type/n");*/
                }
 ...
 *
 ****************************************************************************/
void WmDtHelpOnVersion(Widget parent)

{
        ...
        if ((versionWidget != NULL) && (XtIsManaged(versionWidget))) {
                return;
        }
        ...
        if (!(versionWidget)) {
                ...
                XtSetArg(setArgs[n], XmNtitle, wmNLS.defaultVersionTitle);
                n++;
#else
                XtSetArg(setArgs[n], XmNtitle, "About Workspace Manager");
                n++;
#endif

                XtSetArg(setArgs[n], DtNhelpType, DtHELP_TYPE_TOPIC);
                n++;
                XtSetArg(setArgs[n], DtNlocationId, "_COPYRIGHT");
                n++;
                XtSetArg(setArgs[n], DtNhelpVolume, WM_DT_HELP_VOLUME);
                n++;
                ...
                XtAddCallback(versionWidget, DtNcloseCallback,
                              WmDtVersionCloseCB, (XtPointer)versionWidget);

                XtAddCallback(XtParent(versionWidget), XmNpopupCallback,
                              WmDtVersionPopupCB, NULL);

                closeButton = DtHelpQuickDialogGetChild(
                    versionWidget, DtHELP_QUICK_CLOSE_BUTTON);
                n2 = 0;

#ifndef NO_MESSAGE_CATALOG
                XtSetArg(setArgs2[n2], DtNcloseLabelString, wmGD.okLabel);
                n2++;
#else
                XtSetArg(setArgs2[n2], DtNcloseLabelString,
                         label = XmStringCreateLocalized("OK"));
                n2++;
#endif
                ...
                printButton = DtHelpQuickDialogGetChild(
                    versionWidget, DtHELP_QUICK_PRINT_BUTTON);
                helpButton = DtHelpQuickDialogGetChild(
                    versionWidget, DtHELP_QUICK_HELP_BUTTON);
                backButton = DtHelpQuickDialogGetChild(
                    versionWidget, DtHELP_QUICK_BACK_BUTTON);
                ...
                if (args) {
                        ...
                        if (argCount == 1) {
                                WmDtDisplayTopic(
                                    ...
                                    (char *)topic, DtHELP_TYPE_TOPIC, theWidget,
                                    True, NULL, 0, NULL, False, NULL);
                        } else {
                                WmDtDisplayTopic(
                                    pSD->screenTopLevelW1, (char *)volume,
                                    (char *)topic, DtHELP_TYPE_TOPIC, theWidget,
                                    False, NULL, 0, NULL, False, NULL);
                        }

                } else {
                        WmDtDisplayTopic(pSD->screenTopLevelW1, theHelpVolume,
                                         theHelpTopic, DtHELP_TYPE_TOPIC,
                                         theWidget, True, NULL, 0, NULL, False,
                                         NULL);
                }
                return (True);
        } else {
...
Boolean WmDtHelpMode(void) {
        ...
        iStatus =
            WmDtReturnSelectedWidget(wmGD.topLevelW, None, &selectedWidget);
...
#ifndef NO_MESSAGE_CATALOG
                sTmp = (char *)GETMESSAGE(
                    29, 3, "You must select a Workspace Manager\n\
dialog, an item in the Front Panel,\n\
or an item in a subpanel.");
                sMessage = XtNewString(sTmp);
                sTmp = (char *)GETMESSAGE(
                    29, 4, "Workspace Manager - Help On Item Error");
 ...
 *
 ****************************************************************************/
static void WmDtDisplayTopic(Widget parent, char *helpVolume, char *locationId,
                             int helpType, Widget wmClient, Boolean wmHelp,
                             ArgList argList, int argCount, ClientData *pCData,
                             Boolean hyperLink,
                             DisplayTopicInfo *displayTopicInfo)

{
        ...
        case DtHELP_TYPE_TOPIC:
                if (helpVolume != NULL) {
                        XtSetArg(setArgs[n], DtNhelpVolume, helpVolume);
                        n++;
                }
                XtSetArg(setArgs[n], DtNlocationId, locationId);
                n++;
                helpTitle = locationId;
                break;

        case DtHELP_TYPE_DYNAMIC_STRING:
                XtSetArg(setArgs[n], DtNstringData, locationId);
                n++;
                /*	helpTitle=locationId;*/
        ...
        if (n) {
                XtSetArg(setArgs[n], DtNhelpType, helpType);
                n++;
                ...
                if (wmHelp) {
                        if (pSD->dtHelp.shell) {
                                ...
                                pSD->dtHelp.dialog = DtCreateHelpDialog(
                                    shellWidget, "helpWidget", setArgs, n);
                                helpWidget = pSD->dtHelp.dialog;
                                pSD->dtHelp.shell = XtParent(helpWidget);
                                pSD->dtHelp.bMapped = False;

                                n = 0;
                                XtSetArg(setArgs[n], XmNname, "wmHelp");
                                n++;
                                if (argCount) {
                                        XtSetValues(pSD->dtHelp.shell, argList,
                                                    argCount);
                                }

                                XtAddCallback(helpWidget, DtNhyperLinkCallback,
                                ...
                                XtAddCallback(pSD->dtHelp.shell,
                                              XmNpopupCallback,
                        ...
                        if (pCData) {
                                ...
                                if (wmClient) {
                                        XFindContext(
                                            DISPLAY,
                                            XtWindow(XtParent(wmClient)),
                                            wmGD.windowContextType,
                                            (caddr_t *)&pCD);
                                } else {
                                        XFindContext(
                                            DISPLAY,
                                            XtWindow(O_Shell(pPanelist)),
                                            wmGD.windowContextType,
                                            (caddr_t *)&pCD);
                                }
                                ShowHelpDialog(pCD);
                        ...
                        if (cachedNode) {
                                XtSetValues(pCurrentNode->helpDialog, setArgs,
                                            n);
                        } else {
#ifndef NO_MESSAGE_CATALOG
                                XtSetArg(setArgs[n], XmNtitle,
                                         wmNLS.defaultHelpTitle);
                                n++;
#else
                                XtSetArg(setArgs[n], XmNtitle,
                                         "Workspace Manager - Help");
                                ...
                                XtSetArg(setArgs[n], XmNgeometry, NULL);
                                n++;
                                XtSetArg(setArgs[n], XmNx, 0);
                                n++;
                                XtSetArg(setArgs[n], XmNy, 0);
                                ...
                                pCurrentNode->helpDialog = DtCreateHelpDialog(
                                    shellWidget, "helpWidget", setArgs, n);
                                n = 0;
                                XtSetArg(setArgs[n], XmNname, "cachedHelp");
                                n++;
                                if (argCount) {
                                        XtSetValues(shellWidget, argList,
                                                    argCount);
                                }
                                ...
                                XtAddCallback(
                                    pCurrentNode->helpDialog, DtNcloseCallback,
                                    WmDtCloseHelpCB,
                                        ...
                                        if (displayTopicInfo) {
                                                pCurrentNode->workspaces =
                                                    displayTopicInfo
                                                        ->workspaces;
                                                pCurrentNode->xPos =
                                                    displayTopicInfo->xPos;
                                                pCurrentNode->yPos =
                                                    displayTopicInfo->yPos;
                                                pCurrentNode->restored = True;
                                                XtAddCallback(
                                                    XtParent(pCurrentNode
                                                                 ->helpDialog),
                                                    XmNpopupCallback,
                                                    WmDtPopupHelpCB,
...
static Boolean WmDtGetFromCache(CacheListStruct **pCurrentNode,
        ...
        if (pSD->cachedHelp == NULL) {
                ...
                *pCurrentNode = pSD->cachedHelp;
                ...
                /*
                 * If we did not find a free node then we must add a new one to
                 * the top of the list, and return it.
                 */
                ...
                /* Re-Assign our head pointer to point to the new head of the
                 * list */
        ...
        if (pSD->helpResources) {
                ...
                if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type,
                        ...
                        if (strcmp(value.addr, "True") == 0) {
                                ...
                                if (XrmQGetResource(db, xrm_name, xrm_name,
                                                    &rep_type, &value)) {
                                        XtSetArg(
                                            setArgs[ac], XmNcolumns,
                                            (short)atoi((char *)value.addr));
                                        ac++;
                                        columns =
                                            (short)atoi((char *)value.addr);
                                }
                                ...
                                if (XrmQGetResource(db, xrm_name, xrm_name,
                                                    &rep_type, &value)) {
                                        tTitle =
                                            XtNewString((char *)value.addr);
                                        XtSetArg(setArgs[ac], DtNtopicTitle,
                                                 tTitle);
                                        ac++;
                                }
                                ...
                                case DtHELP_TYPE_TOPIC:
                                        /* get helpVolume */
                                        xrm_name[1] =
                                            XrmStringToQuark("helpVolume");
                                        if (XrmQGetResource(db, xrm_name,
                                                            xrm_name, &rep_type,
                                                            &value)) {
                                                helpVolume = XtNewString(
                                                    (char *)value.addr);
                                                /*			XtSetArg
                                                   (setArgs[ac], DtNhelpVolume,
                                                                                  helpVolume); ac++;*/
                                        }
                                        /* get locationId */
                                        xrm_name[1] =
                                            XrmStringToQuark("locationId");
                                        if (XrmQGetResource(db, xrm_name,
                                                            xrm_name, &rep_type,
                                                            &value)) {
                                                locationId = XtNewString(
                                                    (char *)value.addr);
                                                /*			XtSetArg
                                                   (setArgs[ac], DtNlocationId,
                                                                                  locationId); ac++;*/
                                        }
                                ...
                                case DtHELP_TYPE_DYNAMIC_STRING:
                                        ...
                                        /* get stringData */
                                        xrm_name[1] =
                                            XrmStringToQuark("stringData");
                                        if (XrmQGetResource(db, xrm_name,
                                                            xrm_name, &rep_type,
                                                            &value)) {
                                                stringData = XtNewString(
                                                    (char *)value.addr);
                                                /*			XtSetArg
                                                   (setArgs[ac], DtNstringData,
                                                                                  stringData); ac++;*/
                                ...
                                /* get name of wm client for which wmHelp is
                                 * transient */
                                ...
                                if (XrmQGetResource(db, xrm_name, xrm_name,
                                        ...
                                        if ((!strcmp(wmClientName,
                                                     pSD->iconBoxName)) &&
                                            (pSD->useIconBox)) {
                                                xrm_name[1] =
                                                    XrmStringToQuark("wsName");
                                                if (XrmQGetResource(
                                                        db, xrm_name, xrm_name,
                                                        &rep_type, &value)) {
                                                        wsName =
                                                            (char *)value.addr;
                                                        for (wsCnt = 0;
                ...
                if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type,
                        ...
                        for (cCount = 1; cCount < (cachedCount + 1); cCount++) {
                                ...
                                WmDtDisplayTopic(
                                    pSD->screenTopLevelW1, helpVolume,
                                    locationId, helpType, NULL, False, setArgs,
                                    ac, NULL, False, &displayTopicInfo);
...
void SaveHelpResources(WmScreenData *pSD) {
#ifndef NO_DT
        PtrWsDtHelpData pHelp;
        CacheListStruct *pTemp;
        Arg getArgs[20];
        ...
        if ((data = (char *)XtMalloc(MAXWMPATH + 1)) == NULL) {
                Warning(((char *)GETMESSAGE(
                    76, 12, "Insufficient memory to save resources")));
                Do_Quit_Mwm(False);
        }
        cum_len = 1;
        *data = '\0';
        ...
        if (pHelp->onScreen) {
                ...
                ac = 0;
                XtSetArg(getArgs[ac], XmNcolumns, &columns);
                ac++;
                XtSetArg(getArgs[ac], XmNrows, &rows);
                ac++;
                XtSetArg(getArgs[ac], DtNhelpType, &helpType);
                ac++;
                XtSetArg(getArgs[ac], DtNvisiblePathCount, &vPCount);
                ac++;
                XtSetArg(getArgs[ac], DtNtopicTitle, &topicTitle);
                ac++;
                ...
                switch (helpType) {
                case DtHELP_TYPE_TOPIC:
                        XtSetArg(getArgs[ac], DtNhelpVolume, &helpVolume);
                        ac++;
                ...
                ac = 0;
                XtSetArg(getArgs[ac], XmNwindowGroup, &wmGroupWindow);
                ac++;
                XtGetValues(pSD->dtHelp.shell, getArgs, ac);
        ...
        if (pSD->cachedHelp != NULL) {
                ...
                while (pTemp != NULL) {
                        if (pTemp->inUseFlag == FALSE) {
                                ...
                                XtSetArg(getArgs[ac], XmNy, &yLoc);
                                ac++;
                                XtGetValues(XtParent(pTemp->helpDialog),
                                            getArgs, ac);
                                if (!XFindContext(
                                        DISPLAY,
                                        ...
                                        if (wmGD.positionIsFrame) {
                                                xLoc =
                                                    xLoc -
                                                    pCDforHelp->clientOffset.x;
                                                yLoc =
                                                    yLoc -
                                                    pCDforHelp->clientOffset.y;
                                        }
                                } else {
                                        pCDforHelp = NULL;
                                }
                                ...
                                XtSetArg(getArgs[ac], DtNhelpType, &helpType);
                                ac++;
                                XtSetArg(getArgs[ac], DtNvisiblePathCount,
                                         &vPCount);
                                ac++;
                                XtSetArg(getArgs[ac], DtNtopicTitle,
                                         &topicTitle);
                                ac++;
                                ...
                                switch (helpType) {
                                case DtHELP_TYPE_TOPIC:
                                        XtSetArg(getArgs[ac], DtNhelpVolume,
                                                 &helpVolume);
                                        ac++;
                                        XtSetArg(getArgs[ac], DtNlocationId,
                                                 &locationId);
                                        ac++;
                                ...
                                case DtHELP_TYPE_DYNAMIC_STRING:
                                        XtSetArg(getArgs[ac], DtNstringData,
                                                 &stringData);
                                        ac++;
                                        XtGetValues(pTemp->helpDialog, getArgs,
                                                    ac);
...
static void ShowHelpDialog(ClientData *pClient) {
        ...
        if (pSD->dtHelp.onScreen) {
                ...
                if ((!pSD->dtHelp.bMapped) ||
                    (pCD && (!ClientInWorkspace(ACTIVE_WS, pCD) ||
...
static void wmDtHelpSetPosition(PtrWsDtHelpData pHelp, WmScreenData *pSD,
        ...
        if (!restorePos) {
                ...
                if (wmGD.positionIsFrame) {
                        ...
                        height +=
                            TEXT_HEIGHT(font) + (2 * pSD->resizeBorderWidth);
                }
                ...
                if (pHelp->pCDforClient) {
                        GetSystemMenuPosition(pHelp->pCDforClient, &x, &y,
                                              height, (Context)0);
#ifdef NO
                        if (y < pHelp->pCDforClient->frameInfo.y) {
                                if (!XFindContext(DISPLAY,
                                                  XtWindow(pHelp->shell),
                                                  wmGD.windowContextType,
                                                  (caddr_t *)&pCDforHelp)) {
                                        if (wmGD.positionIsFrame) {
                                                /*			x = x -
        ...
        n++;
        XtSetArg(args[n], XmNtransientFor, NULL);
        n++;
        ...
        if (pHelp->pCDforClient) {
                ...
                ac = 0;
                XtSetArg(al[ac], XmNwindowGroup, &wGroup);
                ac++;
                ...
                if (pHelp->contextForClient != F_CONTEXT_ICON) {
                        ...
                        XtSetArg(args[n], XmNwindowGroup,
                                 P_ICON_BOX(pHelp->pCDforClient)
                                     ->pCD_iconBox->client);
                        n++;
                } else {
                        XtSetArg(args[n], XmNwindowGroup, 0);
                        n++;
                }
#else  /* FUTURE */
                ...
                XtSetArg(args[n], XmNwindowGroup, pHelp->pCDforClient->client);
                n++;
#endif /* FUTURE */
        } else {
                if (wGroup != None) {
 ...
 *
 ******************************<->***********************************/
static void wmDtHelpMapEventHandler(Widget wHelp, XtPointer ptr, XEvent *pEvent,
...
static void wmDtErrorDialog(WmScreenData *pSD, String sMessage, String sTitle) {
        ...
        xmsMessage = XmStringCreateLocalized(sMessage);
        xmsTitle = XmStringCreateLocalized(sTitle);

        if (pSD->dtHelp.errorDialog == NULL) {
                XmString tmpXmString = (XmString)NULL;

                ac = 0;
                XtSetArg(al[ac], XmNmessageString, xmsMessage);
                ac++;
                XtSetArg(al[ac], XmNdialogTitle, xmsTitle);
                ac++;
#ifndef NO_MESSAGE_CATALOG
                XtSetArg(al[ac], XmNokLabelString, wmGD.okLabel);
                ac++;
#else
                tmpXmString = XmStringCreateLocalized("OK");
                XtSetArg(al[ac], XmNokLabelString, tmpXmString);
                ac++;
#endif
	...
	else
	{
	    wParent = pSD->screenTopLevelW1;
                ...
                ac = 0;
                XtSetArg(al[ac], XmNmwmFunctions,
...
static void wmDtErrorDialogPopupCB(Widget w, XtPointer client_data,
        ...
        XQueryPointer(DISPLAY, XtWindow(pSD->screenTopLevelW1), &wRoot, &wChild,
                      &rootX, &rootY, &winX, &winY, &mask);

        ac = 0;
        XtSetArg(al[ac], XmNwidth, &dWidth);
        ac++;
        XtSetArg(al[ac], XmNheight, &dHeight);
        ac++;
        XtGetValues(w, al, ac);
        ...
        ac++;
        XtSetArg(al[ac], XmNwindowGroup, XtUnspecifiedWindowGroup);
        ac++;
