...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmPresence.c /main/7 1996/10/23 17:24:56 rswiston $"
#endif
#endif
...
#include "WmPresence.h"
    ...
    static void
    wspSetWindowName(PtrWsPresenceData pPres);
...
static Widget wspCreateLabel(Widget mgrW, unsigned char *pchName,
                             unsigned char *pchString);
static Widget wspCreateSeparator(Widget mgrW);
static Widget wspCreateWorkspaceList(Widget mgrW, PtrWsPresenceData pPres,
                                     WmScreenData *pSD);
...
static void wspHelpCB(Widget buttonW, XtPointer client_data,
                      XtPointer call_data);
static void wspAllWsCB(Widget buttonW, WmScreenData *pSD,
                       XmToggleButtonCallbackStruct *xmTbcs);
static void wspCancelCB(Widget buttonW, XtPointer client_data,
                        XtPointer call_data);
static void wspExtendedSelectCB(Widget w, XtPointer client_data,
...

{
        ...
        if (wspCreateWidgets(pSD)) {
...
#ifdef PANELIST
                DtWsmRemoveWorkspaceFunctions(DISPLAY1,
                                              XtWindow(pPres->shellW));
#endif /* PANELIST */
                ...
                Warning(((char *)GETMESSAGE(
                    52, 1, "Unable to create Occupy Workspace dialog.")));
                rval = False;
        }
...

{
        ...
        if (pPres->contextForClient == F_CONTEXT_ICON) {
                ...
                nameN++;
                XtSetArg(labelArgs[labelN], XmNlabelString, iconLabelString);
                labelN++;
        } else {
                XtSetArg(nameArgs[nameN], XmNlabelString,
                         pPres->pCDforClient->clientTitle);
                nameN++;
                XtSetArg(labelArgs[labelN], XmNlabelString, windowLabelString);
                labelN++;
        }
...
static Boolean wspCreateWidgets(WmScreenData *pSD) {
        ...
        if (rval) {
                rval =
                    ((pPres->formW = wspCreateManager(pPres->shellW)) != NULL);
        }

        if (rval) {
                rval = ((pPres->windowLabelW = wspCreateLabel(
                             pPres->formW, (unsigned char *)"window", NULL)) !=
                        NULL);
        }

        if (rval) {
                rval = ((pPres->windowNameW = wspCreateLabel(
                             pPres->formW, (unsigned char *)"windowName",
                             (unsigned char *)" ")) != NULL);
        }

        if (rval) {
                rval = ((pPres->workspaceLabelW = wspCreateLabel(
                             pPres->formW, (unsigned char *)"workspaces",
                             ((unsigned char *)GETMESSAGE(
                                 52, 3, "Workspaces: ")))) != NULL);
        }

        if (rval) {
                rval = ((pPres->workspaceListW = wspCreateWorkspaceList(
                             pPres->formW, pPres, pSD)) != NULL);
                pPres->workspaceScrolledListW = XtParent(pPres->workspaceListW);
        }

        if (rval) {
                rval = ((pPres->allWsW = wspCreateToggleButton(
                             pPres->formW, ((unsigned char *)GETMESSAGE(
                                               52, 4, "All Workspaces")))) !=
                        NULL);
        ...
        if (rval) {
                rval =
                    ((pPres->sepW = wspCreateSeparator(pPres->formW)) != NULL);
        }

        if (rval) {
#ifndef NO_MESSAGE_CATALOG
                rval = ((pPres->OkW = wspCreatePushButton(
                             pPres->formW, "ok", wmGD.okLabel)) != NULL);
#else
                tmpXmString = XmStringCreateLocalized("OK");
                rval = ((pPres->OkW = wspCreatePushButton(
                             pPres->formW, "ok", tmpXmString)) != NULL);
                XmStringFree(tmpXmString);
#endif

                /* set the default action */
                n = 0;
                XtSetArg(args[n], XmNdefaultButton, pPres->OkW);
                n++;
        ...
        if (rval) {
#ifndef NO_MESSAGE_CATALOG
                rval =
                    ((pPres->CancelW = wspCreatePushButton(
                          pPres->formW, "cancel", wmGD.cancelLabel)) != NULL);
#else
                tmpXmString = XmStringCreateLocalized("Cancel");
                rval = ((pPres->CancelW = wspCreatePushButton(
                             pPres->formW, "cancel", tmpXmString)) != NULL);
                XmStringFree(tmpXmString);
#endif
                ...
                n = 0;
                XtSetArg(args[n], XmNcancelButton, pPres->CancelW);
                n++;
                XtSetValues(pPres->formW, args, n);
        }
        ...
        if (rval) {
#ifndef NO_MESSAGE_CATALOG
                rval = ((pPres->HelpW = wspCreatePushButton(
                             pPres->formW, "help", wmGD.helpLabel)) != NULL);
#else
                tmpXmString = XmStringCreateLocalized("Help");
                rval = ((pPres->HelpW = wspCreatePushButton(
                             pPres->formW, "help", tmpXmString)) != NULL);
                XmStringFree(tmpXmString);
#endif
...
static Widget wspCreateShell(WmScreenData *pSD) {
        ...
        XtSetArg(setArgs[i], XmNmwmFunctions, PRESENCE_BOX_FUNCTIONS);
        i++;
        ...
        XtSetArg(setArgs[i], XmNdepth,
                 (XtArgVal)DefaultDepth(DISPLAY1, pSD->screen));
        i++;

        XtSetArg(setArgs[i], XmNscreen,
                 (XtArgVal)ScreenOfDisplay(DISPLAY1, pSD->screen));
        i++;

        XtSetArg(setArgs[i], XtNcolormap,
                 (XtArgVal)DefaultColormap(DISPLAY1, pSD->screen));
        i++;

        shellW = (Widget)XtCreatePopupShell(
            WmNfeedback, transientShellWidgetClass, pSD->screenTopLevelW1,
            (ArgList)setArgs, i);
...
static Widget wspCreateManager(Widget shellW) {
        ...
        XtAddCallback(formW, XmNhelpCallback, WmDtWmTopicHelpCB,
...
static Widget wspCreateLabel(Widget mgrW, unsigned char *pchName,
        ...
        if (!strcmp((char *)pchName, "window")) {
                ...
                windowLabelString = XmStringCreateLocalized(
                    (char *)GETMESSAGE(52, 2, "Window: "));
                /*
                 * If we do this, USE the message catalog for iconLabelString
                 * just as we do for windowLabelString !!!
                 */
                iconLabelString = XmStringCreateLocalized(
                    (char *)GETMESSAGE(52, 6, "Icon: "));

                XtSetArg(setArgs[i], XmNlabelString, windowLabelString);
                i++;
        } else {
                tmpXmString = XmStringCreateLocalized((char *)pchString);
                XtSetArg(setArgs[i], XmNlabelString, tmpXmString);
                i++;
        }
...
#define MIN_VISIBLE 6
        ...
        if (((pPres->ItemStrings = (XmStringTable)XtMalloc(
            ...
            ((pPres->ItemSelected = (Boolean *)XtMalloc(
                ...
                Warning(((char *)GETMESSAGE(
                    52, 5,
                    "Insufficient memory to create Occupy Workspace dialog.")));
                return (NULL);
        }
        ...
        XtSetArg(setArgs[i], XmNitemCount, pSD->numWorkspaces);
        i++;

        XtSetArg(setArgs[i], XmNitems, pPres->ItemStrings);
        i++;

        XtSetArg(setArgs[i], XmNselectionPolicy, XmEXTENDED_SELECT);
        i++;

        XtSetArg(setArgs[i], XmNlistSizePolicy, XmRESIZE_IF_POSSIBLE);
        i++;
         ...
         */
        i = 0;
        XtSetArg(setArgs[i], XmNvisibleItemCount, &numVisible);
        i++;
        XtGetValues(listW, setArgs, i);

        if (numVisible < MIN_VISIBLE) {
                i = 0;
                XtSetArg(setArgs[i], XmNvisibleItemCount, MIN_VISIBLE);
                i++;
        ...
        XtAddCallback(listW, XmNdefaultActionCallback, (XtCallbackProc)wspOkCB,
                      (XtPointer)pSD);
...
static void wspUpdateWorkspaceList(PtrWsPresenceData pPres) {
        ...
        pxmsSelected = (XmString *)XtMalloc(
            pPres->pCDforClient->pSD->numWorkspaces * sizeof(XmString));
        numSelected = 0;
        for (wsnum = 0; wsnum < pPres->pCDforClient->pSD->numWorkspaces;
             wsnum++) {
        ...
        n = 0;
        XtSetArg(args[n], XmNselectedItems, pxmsSelected);
        n++;
        XtSetArg(args[n], XmNselectedItemCount, numSelected);
        n++;
        XtSetValues(pPres->workspaceListW, args, n);

        /* set state of all workspaces button */
        n = 0;
        XtSetArg(args[n], XmNset, pPres->pCDforClient->putInAll);
        n++;
...
static Widget wspCreateToggleButton(Widget mgrW, unsigned char *pch) {
        ...
        XtSetArg(setArgs[i], XmNlabelString, labelString);
        i++;
...

{
        ...
        XtSetArg(setArgs[i], XmNlabelString, label);
        i++;
...
static void wspSetPosition(PtrWsPresenceData pPres) {
         ...
         */
        GetSystemMenuPosition(pPres->pCDforClient, &x, &y, height,
        ...
        n++;
        if (pPres->contextForClient != F_CONTEXT_ICON) {
                XtSetArg(args[n], XmNwindowGroup, pPres->pCDforClient->client);
                n++;
        } else if (pSD->useIconBox && P_ICON_BOX(pPres->pCDforClient)) {
                XtSetArg(args[n], XmNwindowGroup,
                         P_ICON_BOX(pPres->pCDforClient)->pCD_iconBox->client);
                n++;
        } else {
...
static Dimension wspCharWidth(XmFontList xmfl) {
        ...
        while (entry) {
                ...
                case XmFONT_IS_FONT:
                        dTmpWidth =
                            ((XFontStruct *)pFont)->max_bounds.rbearing -
...
#define IW_OFFSET_0 4
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, pPres->windowLabelW);
        n++;
        XtSetArg(args[n], XmNtopOffset, IW_OFFSET_0);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNleftOffset, 5);
        ...
        n++;
        XtSetArg(args[n], XmNleftWidget, pPres->workspaceLabelW);
        n++;
        XtSetArg(args[n], XmNleftOffset, 5);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, pPres->windowNameW);
        n++;
        XtSetArg(args[n], XmNtopOffset, IW_OFFSET_0);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, pPres->workspaceScrolledListW);
        n++;
        XtSetArg(args[n], XmNtopOffset, IW_OFFSET_1);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, pPres->workspaceLabelW);
        ...
        XtSetArg(args[n], XmNalignment, XmALIGNMENT_BEGINNING);
        n++;
        XtSetArg(args[n], XmNmarginRight, 5);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_NONE);
        n++;
        XtSetArg(args[n], XmNbottomOffset, 0);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, pPres->allWsW);
        n++;
        XtSetArg(args[n], XmNtopOffset, SEP_OFFSET);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, pPres->sepW);
        n++;
        XtSetArg(args[n], XmNtopOffset, SEP_OFFSET);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNrightPosition, 30);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_POSITION);
        n++;
        XtSetArg(args[n], XmNbottomPosition, 95);
        n++;
        ...
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
        n++;
        XtSetArg(args[n], XmNleftPosition, 36);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
        n++;
        XtSetArg(args[n], XmNrightPosition, 66);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_POSITION);
        n++;
        XtSetArg(args[n], XmNbottomPosition, 95);
        ...
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
        n++;
        XtSetArg(args[n], XmNleftPosition, 71);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
        n++;
...
static void wspOkCB(Widget buttonW, WmScreenData *pSD, XtPointer call_data) {
        ...
        n = 0;
        XtSetArg(args[n], XmNselectedItemCount, &selectedItemCount);
        n++;
        XtSetArg(args[n], XmNselectedItems, &selectedItems);
        n++;
        XtGetValues(pPres->workspaceListW, args, n);

        /* find the state of all workspaces button */
        n = 0;
        XtSetArg(args[n], XmNset, &bAllSelected);
        n++;
...
static void wspAllWsCB(Widget buttonW, WmScreenData *pSD,
        ...
        if (xmTbcs->reason == XmCR_VALUE_CHANGED) {
                if (xmTbcs->set) {
                        pxmsSelected = (XmString *)XtMalloc(pSD->numWorkspaces *
                                                            sizeof(XmString));
                        ...
                        XmListDeselectAllItems(pPres->workspaceListW);
                        XmListSelectPos(pPres->workspaceListW,
                                        pPres->currentWsItem, TRUE);
                }
        }
} /* END OF FUNCTION   */
        ...
        if (pPres->shellW) {
                if (pPres->numWorkspaces < pSD->numWorkspaces) {
                        if (((pPres->ItemStrings = (XmStringTable)XtRealloc(
                                  ...
                                  (pSD->numWorkspaces * sizeof(XmString *)))) ==
                             NULL) ||
                            ((pPres->ItemSelected = (Boolean *)XtRealloc(
                                  (char *)pPres->ItemSelected,
                                  (pSD->numWorkspaces * sizeof(Boolean)))) ==
                             NULL)) {
                                Warning(((char *)GETMESSAGE(
                                    52, 5,
                                    "Insufficient memory to create Occupy "
                                    "Workspace dialog.")));
                                pPres->shellW = NULL;
                                return;
                        }
                ...
                for (wsnum = 0; wsnum < pPres->numWorkspaces; wsnum++) {
                        if (wsnum < pSD->numWorkspaces) {
                                ...
                                if (!XmStringCompare(
                                        xmsTmp, pPres->ItemStrings[wsnum])) {
                                        /*
                                         * Replace the string in our local list
                                         */
                                        XmStringFree(pPres->ItemStrings[wsnum]);
                                        ...
                                        /*
                                         * Replace the item in the scrolled
                                         * list.
                                         */
                                        XmListDeletePos(pPres->workspaceListW,
                                                        1 + wsnum);
                                        XmListAddItem(pPres->workspaceListW,
                                                      xmsTmp, 1 + wsnum);
                                } else {
                                        XmStringFree(xmsTmp);
                                }
                ...
                for (; wsnum < pSD->numWorkspaces; wsnum++) {
                        ...
                        if (pPres->pCDforClient &&
                            (ClientInWorkspace(
                                &pPres->pCDforClient->pSD->pWS[wsnum],
                                pPres->pCDforClient))) {
                                XmListSelectPos(pPres->workspaceListW,
                                                1 + wsnum, TRUE);
                                pPres->ItemSelected[wsnum] = True;
