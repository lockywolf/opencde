...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmCPlace.c /main/5 1996/08/09 15:18:04 rswiston $"
#endif
#endif
/*
...
void SetupPlacement(ClientData *pcd) {
        ...
        XQueryPointer(DISPLAY, ACTIVE_ROOT, &junk_win, &junk_win, &cX, &cY,
...
void StartInteractiveSizing(ClientData *pcd, Time time) {
        ...
        XChangeActivePointerGrab(DISPLAY, gmask, wmGD.sizePlacementCursor,
                                 time);
...
void HandlePlacementKeyEvent(ClientData *pcd, XKeyEvent *pev) {
        ...
        if (valid) {
                ...
                if (placeResize) {
                        ...
                        FixFrameValues(pcd, &keyPlaceX, &keyPlaceY,
                                       &keyPlaceWidth, &keyPlaceHeight,
                                       placeResize);
        ...
        if (wmGD.showFeedback & WM_SHOW_FB_PLACEMENT) {
                DoFeedback(pcd, placeX, placeY, placeWidth, placeHeight, 0,
                           placeResize);
        }
...
void DoPlacement(ClientData *pcd) {
        ...
        while (!placementDone) {
                GetConfigEvent(DISPLAY, ACTIVE_ROOT, GRAB_MASK, placePointerX,
                               placePointerY, placeX, placeY, placeWidth,
                               placeHeight, &event);
...
void PlaceWindowInteractively(ClientData *pcd) {
         ...
         */
        if (!DoGrabs(ACTIVE_ROOT, wmGD.movePlacementCursor, gmask, CurrentTime,
                     pcd, True)) {
                return;
        }
