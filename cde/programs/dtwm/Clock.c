...
static void ClockTick(XtPointer client_data, XtIntervalId *id) {
        ...
        if (id || !w->clock.interval_id)
                w->clock.interval_id = XtAppAddTimeOut(
        ...
        if (!tm || tm->tm_min != w->clock.otm.tm_min ||
            tm->tm_hour != w->clock.otm.tm_hour) {
                XDrawLines(dpy, win, gc, w->clock.segbuff, VERTICES_IN_HANDS,
                           CoordModeOrigin);
                XDrawLines(dpy, win, gc, w->clock.hour, VERTICES_IN_HANDS,
                           CoordModeOrigin);
                XFillPolygon(dpy, win, gc, w->clock.segbuff, VERTICES_IN_HANDS,
                             Convex, CoordModeOrigin);
                XFillPolygon(dpy, win, gc, w->clock.hour, VERTICES_IN_HANDS,
                             Convex, CoordModeOrigin);
        }
}
...
static void DrawClockFace(DtClockGadget g) {
        ...
        register int i;
        register int delta = (int)(g->clock.radius - g->clock.tick_spacing) / 3;
        ...
        for (i = 0; i < 60; i++) {
                if (draw_minute_ticks) {
                        ...
                        DrawLine(g, g->clock.radius - 1, g->clock.radius,
                                 ((double)i) / 60.);
        }
...
static void Initialize(Widget request_w, Widget new_w) {
        ...
        Dimension w, h, h_t = G_HighlightThickness(new),
...
static void Resize(Widget widget) {
        ...
        if (G_ClockWidth(g) < (Dimension)(SIZE_DEFAULT / 2)) {
                ...
                g->clock.minute_hand_length =
                    ((Dimension)(SMALL_MINUTE_HAND_FRACT * g->clock.radius) /
                     100);
                g->clock.hour_hand_length =
                    ((Dimension)(SMALL_HOUR_HAND_FRACT * g->clock.radius) /
                     100);
                g->clock.hand_width =
                    ((Dimension)(SMALL_HAND_WIDTH_FRACT * g->clock.radius) /
                     100);
        }
...
Widget DtCreateClock(Widget parent, String name, ArgList arglist,
                     Cardinal argcount) {
        return (XtCreateWidget(name, dtClockGadgetClass, parent, arglist,
                               argcount));
}
