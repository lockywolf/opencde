...
#define LOCALE_MSG                                                             \
        GETMESSAGE(70, 7,                                                      \
                   ...
                   "supported. (Check $LANG).")
#else
#define LOCALE_MSG                                                             \
        "[XmbTextPropertyToTextList]:\n     Locale (%.100s) not supported. "   \
        "(Check "                                                              \
        "$LANG)."
#endif
...

{
        ...
        if (!(pCD = (ClientData *)XtMalloc(sizeof(ClientData)))) {
                ...
                Warning(((char *)GETMESSAGE(
                    70, 1, "Insufficient memory for client data")));
                return (NULL);
        }
        ...
        XSaveContext(DISPLAY, clientWindow, wmGD.windowContextType,
                     (caddr_t)pCD);
...

{
        ...
        if (manageFlags & MANAGEW_ICON_BOX) {
                ...
                pCD->internalBevel = (wmGD.frameStyle == WmSLAB)
                                         ? 0
        ...
        if (ICON_DECORATION(pCD) & ICON_IMAGE_PART) {
                ...
                if (!pCD->useClientIcon && pCD->iconImage) {
                         ...
                         * Do not use the default icon image if iconImage is not
                         * found.
                         */
                ...
                if (!pCD->iconPixmap) {
                        /*
                         * Use a built-in icon image for the window manager
                         * client. The image may differ from the default icon
                         * image, depending on the particular client (eg the
                         * iconbox).
                         */
                        ...
                        if (manageFlags & MANAGEW_ICON_BOX) {
                                ...
                                iconBitmap = XCreateBitmapFromData(
                                    DISPLAY, ROOT_FOR_CLIENT(pCD),
                                    (char *)iconBox_bits, iconBox_width,
                                    iconBox_height);

                                pCD->iconPixmap = MakeIconPixmap(
                                    pCD, iconBitmap, (Pixmap)0L, iconBox_width,
                                    iconBox_height, 1);
                        }
                }
        }
         ...
         * fill with NULL IDs
         */
        if ((pCD->pWsList = (WsClientData *)XtMalloc(
                 pCD->pSD->numWorkspaces * sizeof(WsClientData))) == NULL) {
                Warning(((char *)GETMESSAGE(
                    70, 2, "Insufficient memory for client data")));
                return (NULL);
        }
        ...
        if (manageFlags & MANAGEW_ICON_BOX) {
                ...
                XGetGeometry(DISPLAY, pCD->client, &root, &(pCD->clientX),
                             &(pCD->clientY), &cbWidth, &cbHeight,
                             (unsigned int *)&(pCD->xBorderWidth), &depth);

                pCD->clientWidth = cbWidth;
                pCD->clientHeight = cbHeight;
                pCD->minWidth = pCD->baseWidth = pCD->maxWidth =
                    pCD->clientWidth;
                pCD->minHeight = pCD->baseHeight = pCD->maxHeight =
                    pCD->clientHeight;
        ...
        /*
         * Set offset from frame of client window (need client size
         * information).
         */
        ...
        if (manageFlags & MANAGEW_ICON_BOX) {
                ...
                pCD->systemMenuSpec = MAKE_MENU(
                    PSD_FOR_CLIENT(pCD), pCD, pCD->systemMenu, F_CONTEXT_WINDOW,
                    F_CONTEXT_WINDOW | F_CONTEXT_ICON, NULL, TRUE);
                if (pCD->systemMenuSpec != (MenuSpec *)NULL) {
                        pCD->systemMenuSpec =
                            DuplicateMenuSpec(pCD->systemMenuSpec);
                        ...
                        /* Now recreate the menu widgets since we've appended
                           the icon box menu items */
                        DestroyMenuSpecWidgets(pCD->systemMenuSpec);
                        pCD->systemMenuSpec->menuWidget = CreateMenuWidget(
                            PSD_FOR_CLIENT(pCD), pCD, "IconBoxMenu",
                ...
                pCD->systemMenuSpec = MAKE_MENU(
                    ...
                    GetIconBoxMenuItems(PSD_FOR_CLIENT(pCD)), TRUE);
...
void ProcessSmClientID(ClientData *pCD) {
        ...
        if ((XGetWindowProperty(DISPLAY, pCD->client, wmGD.xa_SM_CLIENT_ID, 0L,
                                (long)1000000, False, AnyPropertyType,
                                &actualType, &actualFormat, &nitems, &leftover,
                                (unsigned char **)&clientID) == Success) &&
...
void ProcessWmSaveHint(ClientData *pCD) {
        ...
        if ((XGetWindowProperty(DISPLAY, pCD->client, wmGD.xa_WMSAVE_HINT, 0L,
                                (long)1000000, False, AnyPropertyType,
                                &actualType, &actualFormat, &nitems, &leftover,
                                (unsigned char **)&saveHintFlags) == Success) &&
...
void ProcessWmHints(ClientData *pCD, Boolean firstTime) {
        ...
        if (firstTime) {
                ...
                if (flags & WindowGroupHint) {
                        ...
                        if ((pCD->bPseudoTransient) &&
                            (pCD->transientFor == (Window)0L)) {
                                ...
                                /*
                                 * Treat this like a transient window. This is
                                 * transient for the window group window.
                                 */
                                ...
                                if ((pCD->client != pCD->windowGroup) &&
                                    !XFindContext(DISPLAY, pCD->windowGroup,
                                        ...
                                        /*
                                         * Insure it is in the same set of
                                         * workspaces as the leader.
                                         */
                                        if (pCD->transientLeader &&
                                            GetLeaderPresence(pCD, &pIDs,
                                                              &numIDs)) {
...
#endif /* NO_OL_COMPAT */
                /*
                 * The window manger does not do anything with the input hint.
                 * Input always goes to the selected window.
                 */
                 ...
                 *  (e.g., ZoomState from the R2 ICCC) indicate to the window
                 * manager that the NORMAL_STATE is to be used.
                 */
                ...
                } else if ((flags & StateHint) &&
                           (pXWMHints->initial_state == IconicState) &&
                           (pCD->clientFunctions & MWM_FUNC_MINIMIZE)) {
                 ...
                 * pixmap or a window (a client icon window takes precedence
                 * over a pixmap).
                 */
                ...
                if ((pCD->clientFunctions & MWM_FUNC_MINIMIZE) &&
                    (pCD->transientLeader == NULL)) {
                            ...
                            (pCD->useClientIcon || !pCD->iconImage)) {
                                if ((flags & IconWindowHint) &&
                                    (pXWMHints->icon_window != pCD->client)) {
                                        /*
                                         * An icon window has been supplied that
                                         * is different from the client window.
                                         * Check out the window and get it ready
                                         * to be reparented to the window
                                         * manager supplied icon frame.
                                         */

                                        if (!SetupClientIconWindow(
                                                pCD, pXWMHints->icon_window)) {
                                                /*
                                                 * Cannot use the client
                                                 * supplied icon window.  Use an
                                                 * icon image if specified or a
                                                 * default image.
                                                 */
                                        }
                                }
                                if (!pCD->iconWindow &&
                                    (flags & IconPixmapHint)) {
                                         ...
                                         * available so use the client supplied
                                         * icon image.
                                         */

                                        if ((pCD->iconPixmap =
                                                 MakeClientIconPixmap(
                                                     pCD,
                                                     pXWMHints->icon_pixmap,
                                                     iconMask)) != None) {
                                                /*
                                                 * Indicate that a client
                                                 * supplied icon image is being
                                                 * used.
                                                 */
                                                ...
                                                /*
                                                 * Cannot make a client supplied
                                                 * image.  Use a user specified
                                                 * icon image if it is available
                                                 * or a default icon image.
                                                 */
                                        }
                                }
                        }
                        ...
                        if ((ICON_DECORATION(pCD) & ICON_IMAGE_PART) &&
                            !pCD->iconPixmap) {
                                /*
                                 * Use a user supplied icon image if it is
                                 * available or a default icon image.
                                 */

                                if (pCD->iconImage) {
                                        /*
                                         * Try to make a user specified icon
                                         * image.
                                         */

                                        pCD->iconPixmap = MakeNamedIconPixmap(
                                            pCD, pCD->iconImage);
                                }

                                if (!pCD->iconPixmap) {
                                        /*
                                         * The icon image was not provided or
                                         * not available. Use the default icon
                                         * image.
                                         */

                                        pCD->iconPixmap =
                                            MakeNamedIconPixmap(pCD, NULL);
                                }
                        }
                        ...
                        if ((flags & IconPositionHint) ||
                            (pCD->clientFlags & (SM_ICON_X | SM_ICON_Y))) {
                                ...
                                {
                                        /*
                                         * Initialize icon placement data in all
                                         * inhabited workspaces
                                         */
                                        for (iws = 0; iws < pCD->numInhabited;
                                             iws++) {
                                                pWsc = &(pCD->pWsList[iws]);
                                                if ((pWsTmp = GetWorkspaceData(
                                                         pCD->pSD,
                                                         pWsc->wsID))) {
                                                        tmpIconX =
                                                            (pCD->clientFlags &
...
#else  /* WSM */
                                {
                                        ...
                                        tmpIconY =
                                            (pCD->clientFlags & SM_ICON_Y)
                                                ? pCD->iconY
                                                : pXWMHints->icon_y;
                                        pCD->iconPlace = FindIconPlace(
                                            pCD, &(pCD->pSD->pActiveWS->IPData),
                                            tmpIconX, tmpIconY);
                                        if (pCD->iconPlace != NO_ICON_PLACE) {
                                                CvtIconPlaceToPosition(
                                                    &(pCD->pSD->pActiveWS
                                                          ->IPData),
                                                    pCD->iconPlace, &pCD->iconX,
                                                    &pCD->iconY);
                                        }
                                } else {
        ...
        } else /* not the first time the hints are processed */
        {
                if (flags & IconPixmapHint) {
                        /*
                         * Process an icon image change if the icon image was
                         * initially set up with a client supplied icon image
                         * OR, if the client now wants to supply an image.
                         */
                        iconMask = (flags & IconMaskHint) ? pXWMHints->icon_mask
                                                          : (Pixmap)NULL;

                        if ((iconPixmap = MakeClientIconPixmap(
                                 pCD, pXWMHints->icon_pixmap, iconMask)) !=
                            None) {
                                /*
                                 * Made new icon image; free up the old image
                                 * and display the new image.
                                 */
                                if (pCD->iconFlags & ICON_HINTS_PIXMAP) {
                                        /*
                                         * ICON_HINTS_PIXMAP was set either
                                         * initally or below because a new
                                         * pixmap was made for the client. It is
                                         * now safe to free the previous pixmap
                                         * since it is not the shared default
                                         * iconPixmap
                                         */
                                        if (pCD->iconPixmap) {
                                                XFreePixmap(DISPLAY,
                                ...
                                /*
                                 * Display new icon image if the icon is
                                 * showing:
                                 */
...
void ProcessWmNormalHints(ClientData *pCD, Boolean firstTime,
        ...
        if (firstTime) {
                ...
                pCD->sizeFlags =
                    flags & (US_POSITION | US_SIZE | P_POSITION | P_SIZE);

                /*
                 * The R2 conventions and Xlib manual indicate that the window
                 * size and position should be taken out of the WM_NORMAL_HINTS
                 * property if they are specified there.  The current
                 * conventions indicate that the size and position information
                 * should be gotten from the window configuration. Mwm 1.1
                 * always uses the current conventions.
                 */
                ...
                if (!(pCD->clientFlags & SM_Y))
                        ...
                        /*
                         * Use current conventions for initial window
                         * dimensions.
                         */
        ...
        if (flags & P_MIN_SIZE) {
                ...
                pCD->minHeight = (pNormalHints->min_height < 0)
                                     ? 0
                                     : pNormalHints->min_height;
        ...
        if (flags & P_RESIZE_INC) {
                ...
                pCD->heightInc = (pNormalHints->height_inc < 1)
                                     ? 1
                                     : pNormalHints->height_inc;
        } else if (firstTime) {
                pCD->widthInc = 1;
                pCD->heightInc = 1;
        }
        ...
        if (flags & P_BASE_SIZE) {
                pCD->baseWidth = (pNormalHints->base_width < 0)
                                     ? 0
                                     : pNormalHints->base_width;
                pCD->baseHeight = (pNormalHints->base_height < 0)
                                      ? 0
                                      : pNormalHints->base_height;
        } else if ((pNormalHints->icccVersion == ICCC_R2) &&
                   ((firstTime) || (!firstTime && (flags & P_MIN_SIZE)))) {
                /*
        ...
        if (firstTime) {
                ...
                if (pCD->clientFlags & SM_HEIGHT) {
                        pCD->clientHeight =
                            ((pCD->clientHeight * pCD->heightInc) +
                             pCD->baseHeight);
                }
        }
        ...
        if (pCD->maximumClientSize.width) {
                ...
                if (IS_MAXIMIZE_HORIZONTAL(pCD)) {
                        ...
                        /*
                         * Hack to set max client to the current client height,
                         * maxHeight will be kept up to date whenever the window
                         * is reconfigured
                         */
                        ...
                        pCD->maxWidth =
                            (pCD->maximumClientSize.width * pCD->widthInc) +
                            pCD->baseWidth;
                }
        } else {
                if (flags & P_MAX_SIZE) {
                        if (pNormalHints->max_width < 0) {
                                /* go to min (full screen width, max maximum
                                 * width) */
                                pCD->maxWidth =
                ...
                /* Don't reset maxWidth if it has been set earlier */
                         ...
                         */
                        if (!firstTime && ((oldBaseWidth != pCD->baseWidth) ||
                                           (oldWidthInc != pCD->widthInc))) {
                                incWidth = (pCD->maxWidth - oldBaseWidth) /
                                           oldWidthInc;
                                pCD->maxWidth =
        ...
        if (pCD->maximumClientSize.height) {
                ...
                if (IS_MAXIMIZE_VERTICAL(pCD)) {
                        ...
                        /*
                         * Hack to set max client to the current client width,
                         * maxWidth will be kept up to date whenever the window
                         * is reconfigured
                         */
                        ...
                        pCD->maxHeight =
                            (pCD->maximumClientSize.height * pCD->heightInc) +
                            pCD->baseHeight;
                }
        } else {
                if (flags & P_MAX_SIZE) {
                        if (pNormalHints->max_height < 0) {
                                /* go to min (full screen height, max maximum
                                 * height) */
                                pCD->maxHeight =
                ...
                /* Don't reset maxHeight if it has been set above */
                         ...
                         * size (if not specified in the hints).
                         */
                        if (!firstTime && ((oldBaseHeight != pCD->baseHeight) ||
                                           (oldHeightInc != pCD->heightInc))) {
                                incHeight = (pCD->maxHeight - oldBaseHeight) /
                                            oldHeightInc;
                                pCD->maxHeight = (incHeight * pCD->heightInc) +
                                                 pCD->baseHeight;
                        } else {
                                /* reset the maxHeight before further processing
                                 */
        ...
        if (flags & P_ASPECT) {
                ...
                if (pCD->minAspect.y > 0 && pCD->maxAspect.y > 0 &&
                    pCD->minAspect.x > 0 && pCD->maxAspect.x > 0 &&
        ...
        } else if ((diff = ((pCD->minHeight - pCD->baseHeight) %
                            pCD->heightInc)) != 0) {
        ...
        /*
         * Hack to use maxHeightLimit as the real maxHeight when
         * maximumClientSize set to 'horizontal'.
         */
        if (IS_MAXIMIZE_HORIZONTAL(pCD)) {
                /* go to min (full screen height, max maximum height) */
                pCD->maxHeightLimit =
        ...
        if (!firstTime && pCD->maxConfig) {
                ...
                if (!firstTime && ((oldBaseWidth != pCD->baseWidth) ||
                                   ...
                                   (oldHeightInc != pCD->heightInc))) {
                        incWidth =
                            (pCD->clientWidth - oldBaseWidth) / oldWidthInc;
                        incHeight =
                            (pCD->clientHeight - oldBaseHeight) / oldHeightInc;
                        pCD->clientWidth =
                            (incWidth * pCD->widthInc) + pCD->baseWidth;
                        pCD->clientHeight =
        ...
        if (firstTime) {
                if (!pCD->maximumClientSize.width) {
                        if (pCD->clientWidth >
                            pCD->pSD->maximumMaximumSize.width) {
                                pCD->clientWidth =
                                    pCD->pSD->maximumMaximumSize.width;
                        }
                }
                ...
                if (!pCD->maximumClientSize.height) {
                        if (pCD->clientHeight >
                            pCD->pSD->maximumMaximumSize.height) {
                                pCD->clientHeight =
                                    pCD->pSD->maximumMaximumSize.height;
                        }
                }
...
static XmString WmICCCMToXmString(XTextProperty *wmNameProp) {
        int status;
        XmString xms_return;
        ...
        if (((status = XmCvtTextPropertyToXmStringTable(
                  DISPLAY, wmNameProp, &xmsTable, &nStrings)) != Success) ||
            (nStrings <= 0)) {
                switch (status) {
...
void ProcessWmWindowTitle(ClientData *pCD, Boolean firstTime) {
        ...
        if (title_xms) {
                if (!firstTime && (pCD->iconTitle == pCD->clientTitle)) {
                        /*
                         * The client window title is being used for the icon
                         * title so change the icon title with the window title.
                         */
        ...
        /*
         * The client frame does not have a place to put the title or the
         * WM_NAME property does not exist or there was some error in getting
         * the property information, so use a default value.
         */
        else if (firstTime) {
                if (pCD->clientName) {
                        pCD->clientTitle =
                            XmStringCreateLocalized(pCD->clientName);
                } else {
                        pCD->clientTitle = wmGD.clientDefaultTitle;
                }
        }
                ...
                /*
                 * If this is a tear-off menu, then make sure title text is not
                 * clipped
                 */
...
#endif /* PANELIST */
        {
                 ...
                 */
                minWidth =
                    XmStringWidth(fontList, pCD->clientTitle) +
#ifdef PANELIST
...
static void FixSubpanelEmbeddedClientGeometry(ClientData *pCD) {
        ...
        if (pSD->wPanelist && wSubpanel) {
                ...
                if (pCD->clientWidth < pCD->minWidth) {
                        ac = 0;
                        XtSetArg(al[ac], XmNwidth, pCD->minWidth);
                        ac++;
                        ...
                        if (pECD->pCD) {
                                ...
                                if ((pCD2->clientWidth != pECD->width) ||
                                    ...
                                    (pCD2->clientY != pECD->y)) {
                                        ...
                                        XMoveResizeWindow(
                                            DISPLAY1, pCD2->client, pECD->x,
                                            pECD->y, pECD->width, pECD->height);
                                }
                        }
                }
        }
...
void ProcessWmTransientFor(ClientData *pCD) {
...
#endif /* WSM */
        {
                 ...
                 * client window and it is already managed by the window
                 * manager.
                 */
...
void MakeSystemMenu(ClientData *pCD) {
        ...
        if (pCD->systemMenuSpec == NULL) {
                ...
                pCD->systemMenuSpec = MAKE_MENU(
                    PSD_FOR_CLIENT(pCD), pCD, builtinSystemMenuName,
                    F_CONTEXT_WINDOW, F_CONTEXT_WINDOW | F_CONTEXT_ICON,
                    pCD->mwmMenuItems, TRUE);
        }
#endif

#if defined(MWM_QATS_PROTOCOL)
                /* Added to fix CDExc23338
                 * Not sure what the MWM_QATS_PROTOCOL is trying to accomplish
                 * here, but this code is causing the system menu to loose it's
                 * default actions whenever client defined actions are added.  I
                 * thought it prudent to minimize the changes.  It could be that
                 * the #if ((!defined(WSM)) || defined(MWM_QATS_PROTOCOL))
                 * should be
                 * #if ((!defined(WSM)) && defined(MWM_QATS_PROTOCOL))
...
Boolean InitClientPlacement(ClientData *pCD, long manageFlags) {
        ...
        if ((wmGD.positionIsFrame) && !interactivelyPlaced && !autoPlaced) {
                CalculateGravityOffset(pCD, &xoff, &yoff);
                if (!(pCD->clientFlags & SM_X))
                        pCD->clientX += xoff;
                if (!(pCD->clientFlags & SM_Y))
        ...
        if (!wmGD.iconAutoPlace) {
                ...
                if (!(pCD->iconFlags & ICON_HINTS_POSITION)) {
                        for (iwsc = 0; iwsc < pCD->numInhabited; iwsc++) {
                                ...
                                PlaceIconOnScreen(pCD,
                                                  &pCD->pWsList[iwsc].iconX,
                ...
                if (pCD->transientLeader == pCD_FP) {
                        ...
                        if ((pCD->clientY + pCD->clientHeight +
                            ...
                            XDisplayHeight(DISPLAY, pCD->pSD->screen)) {
                                /*
                                 * If the bottom of the slide-up is off the
                                 * bottom of the screen, then don't slide, just
                                 * pop it up.
                                 */
...
void PlaceFrameOnScreen(ClientData *pCD, int *pX, int *pY, int w, int h) {
        int clientOffsetX;
        int clientOffsetY;
        int frameX;
        int frameY;
...
void FixWindowSize(ClientData *pCD, unsigned int *pWidth, unsigned int *pHeight,
                   unsigned int widthInc, unsigned int heightInc) {
        register int deltaW;
        register int deltaH;
        ...
        if ((int)*pWidth < pCD->minWidth) {
                *pWidth = pCD->minWidth;
        } else if (*pWidth > pCD->maxWidthLimit && pSD->limitResize &&
        ...
        if ((int)*pHeight < pCD->minHeight) {
                *pHeight = pCD->minHeight;
        } else if (*pHeight > pCD->maxHeightLimit && pSD->limitResize &&
                   !(pCD->clientFlags & CLIENT_WM_CLIENTS)) {
         ...
         */
        {
                ...
                /*
                 * Candidate height >= client height:
                 *   Try to increase the client's height without violating
                 * bounds. If this fails, use maximum height and try to decrease
                 * its width.
                 */
                {
                        ...
                        if (*pHeight + deltaH <= pCD->maxHeightLimit ||
                            ...
                            pCD->clientFlags & CLIENT_WM_CLIENTS) {
                                ...
                                *pHeight = pCD->maxHeightLimit;
                                deltaW = makemult(
                                    *pWidth - *pHeight * pCD->maxAspect.x /
                                                  pCD->maxAspect.y,
                                    widthInc);
                ...
                /*
                 * Candidate height < client height and candidate width <=
                 * client width. Try to decrease the client's width without
                 * violating bounds. If this fails, use minimum width and try
                 * to increase its height.
                 */
                {
                        deltaW =
                            makemult(*pWidth - *pHeight * pCD->maxAspect.x /
                                                   pCD->maxAspect.y,
                                     widthInc);
         ...
         */
        {
                ...
                /*
                 * Candidate width >= client width:
                 *   Try to increase the client's width without violating
                 * bounds. If this fails, use maximum width and try to decrease
                 * its height.
                 */
                {
                        ...
                        if (*pWidth + deltaW <= pCD->maxWidthLimit ||
                            ...
                            pCD->clientFlags & CLIENT_WM_CLIENTS) {
                                ...
                                *pWidth = pCD->maxWidthLimit;
                                deltaH = makemult(
                                    *pHeight - *pWidth * pCD->minAspect.y /
                                                   pCD->minAspect.x,
                                    heightInc);
                ...
                /*
                 * Candidate width < client width and Candidate height <= client
                 * height: Try to decrease the client's height without violating
                 * bounds. If this fails, use minimum height and try to
                 * increase its width.
                 */
                {
                        deltaH =
                            makemult(*pHeight - *pWidth * pCD->minAspect.y /
                                                    pCD->minAspect.x,
                                     heightInc);
                        if (*pHeight - deltaH >= pCD->minHeight) {
                                *pHeight -= deltaH;
...
void FindClientPlacement(ClientData *pCD) {
        ...
        if (!clientPlacementInitialized) {
                if (pCD->clientDecoration & WM_DECOR_RESIZEH) {
                        borderWidth = ((RESIZE_BORDER_WIDTH(pCD) >
                                        FRAME_BORDER_WIDTH(pCD))
                                           ? RESIZE_BORDER_WIDTH(pCD)
                                           : FRAME_BORDER_WIDTH(pCD));
                } else {
        ...
        while (!placed) {
                ...
                if (offScreenX || offScreenY) {
                        if (clientPlacementX == clientPlacementOrigin) {
                                /*
                                 * Placement location is already as far to the
                                 * NW as it is going to go.
                                 */

                                placed = True;
                        } else if (clientPlacementY == clientPlacementOrigin) {
                                /*
                                 * Placement location is as far to the N as it
                                 * is going to go. Use the current placement if
                                 * the window is not off the screen in the x
                                 * coordinate otherwise reset the placement back
                                 * to the NW origin.
                                 */

                                if (offScreenX) {
                                        clientPlacementXOrigin =
                                            clientPlacementOrigin;
                                        clientPlacementX =
                                            clientPlacementXOrigin;
                                }
                                placed = True;
                                ...
                                /*
                                 * If window is off the right edge of screen,
                                 * just move window in the X direction onto
                                 * screen.  Process similarly for windows that
                                ...
                                if (offScreenX && !offScreenY) {
                                        ...
                                        /*
                                         * Reset the placement location back to
                                         * the NW of the current location.  Go
                                         * as far N as possible and step the x
                                         * coordinate to the E.
                                         */
         ...
         * placement that is offset from the previous column.  Previously, the
         * new column was place right over the old column, obscuring it. NOTE:
         * column == diagonal
         */
...
void ProcessMwmHints(ClientData *pCD) {
        PropMwmHints *pHints;
		...
		pCD->dtwmFunctions &= ~DtWM_FUNCTION_OCCUPY_WS;
                ...
                if (pHints->flags & MWM_HINTS_INPUT_MODE) {
                        ...
                        /*
                         * Don't allow a system modal window to be a secondary
                         * window (except with respect to applicable functions
                         * and frame decorations). Also, don't allow system
                         * modal window to be minimized.
                         */
        ...
        if (!((FindTransientTreeLeader(pCD))->clientFunctions &
              MWM_FUNC_MINIMIZE)) {
                pCD->clientFunctions &= ~MWM_FUNC_MINIMIZE;
        }
