...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmCEvent.c /main/10 1996/08/09 15:05:39 rswiston $"
#endif
#endif
/*
                ...
                if ((buttonSpec->context & F_CONTEXT_WINDOW) &&
                    (buttonSpec->subContext & F_SUBCONTEXT_W_CLIENT)) {
                        ...
                        if (buttonSpec->eventType == ButtonRelease) {
                                /*
                                 * Don't include the button down in the grab
                                 * state.
                                 */
...
Boolean WmDispatchClientEvent(XEvent *event) {
        ...
        if ((XFindContext(DISPLAY, event->xany.window, wmGD.windowContextType,
                          (caddr_t *)&pCD)) &&
            (XFindContext(DISPLAY, event->xany.window,
        ...
        case Expose: {
                ...
                if (event->xexpose.count == 0) {
                        if (event->xexpose.window == ICON_FRAME_WIN(pCD)) {
                                 ...
                                 *  front panel and we want the toolkit to
                                 * repaint it.
                                 *
                                 */
                                dispatchEvent = True;
        ...
        case FocusOut: {
                dispatchEvent =
                    HandleCFocusOut(pCD, (XFocusChangeEvent *)event);
                break;
        }
...
Boolean HandleEventsOnSpecialWindows(XEvent *pEvent) {
        ...
        if (pEvent->xany.window == ACTIVE_ROOT) {
                ...
                dispatchEvent =
                    False; /* don't have the toolkit dispatch the event */
                ...
                case ConfigureRequest: {
                        if (GetParentWindow(pEvent->xconfigurerequest.window) ==
                            ACTIVE_ROOT) {
                                /*
                                 * This is an event for a client base window
                                 * that no longer exists.  Handle the event as
                                 * if it is a root window event.
                                 */
        ...
        if (wmGD.menuActive) {
                ...
                if (!((buttonEvent->window == pCD->clientFrameWin) &&
                      ...
                      (buttonEvent->time == baseWinTime))) {
                        ...
                        /*
                         * Motif 1.2, ignore replayed events UNPOST_AND_REPLAY
                         * events generated from the menu system (time stamps
                         * are exactly the same for the replayed event)
                         */

                        if (wmGD.clickData.time == buttonEvent->time) {
                                dispatchEvent = False;
                        } else {
                                ProcessClickBPress(buttonEvent, pCD, context,
                                                   subContext);
                        }
#else
                        ProcessClickBPress(buttonEvent, pCD, context,
                        ...
                        if (CheckForButtonAction(buttonEvent, context,
                                                 subContext, pCD) &&
                            pCD) {
                                /*
                                 * Button bindings have been processed, now
                                 * check for bindings that associated with the
                                 * built-in semantics of the window frame
                                 * decorations.
                                 */

                                CheckButtonPressBuiltin(buttonEvent, context,
                                                        subContext, partContext,
                                                        pCD);

                                /*
                                 * For case where button action causes lower,
                                 * but builtin causes focus - disable auto raise
                                 * until we receive focusIn or focusOut.
                                 */
                                pCD->focusAutoRaiseDisablePending = False;
                        } else {
                                /*
                                 * Else skip built-in processing due to
                                 * execution of a function that does on-going
                                 * event processing or that has changed the
...
void ProcessButtonGrabOnClient(ClientData *pCD, XButtonEvent *buttonEvent,
                               Boolean replayEvent) {
        ButtonSpec *buttonSpec;
        Boolean passButton;
        ...
        if (IS_APP_MODALIZED(pCD) || !passButton) {
                ...
                /*
                 * Replay the event as long as there is not another button
                 * binding for the button release.
                 */
        ...
        if (replayEvent && wmGD.passButtonsCheck) {
                ...
                if (IS_APP_MODALIZED(pCD)) {
                        /*
                         * The grab is done on a window that has an application
                         * modal secondary window.  Beep to indicate no client
...
void CheckButtonPressBuiltin(XButtonEvent *buttonEvent, Context context,
                             Context subContext, int partContext,
                             ClientData *pCD) {
        /*
        ...
        if (context & F_CONTEXT_ICON) {
                ...
                /*
                 * If the keyboard focus policy is explicit then all window
                 * frame components set the keyboard input focus when selected.
                 */
                ...
                if (wmGD.keyboardFocusPolicy == KEYBOARD_FOCUS_EXPLICIT) {
                        ...
                        Do_Focus_Key(pCD, buttonEvent->time,
                                     (long)((partContext == FRAME_CLIENT)
                                                ? CLIENT_AREA_FOCUS
                                                : 0));
                }

                /*
                 * Process the builtin button bindings based on the client
                 * window frame component that was selected.
                 */
                    ...
                    (subContext == F_SUBCONTEXT_W_SYSTEM)) {
                        ...
                        if ((wmGD.clickData.doubleClickContext ==
                            ...
                            (pCD->clientFunctions & MWM_FUNC_CLOSE)) {
                                /*
                                 * Close the client window.  Don't do any of the
                        ...
                        /*
                         * Fix for 5075 - Check to make sure that MWM_FUNC_MOVE
                         * is set in the clientFunctions.  This is necessary
                         * because the title bar is added based
                         * on a number of decorations even if the
                         * resources or the user has specifically
                         * requested that "move" not be one of them.
                         */
...
void HandleIconButtonPress(ClientData *pCD, XButtonEvent *buttonEvent) {
        ...
        if (wmGD.clickData.doubleClickContext == F_SUBCONTEXT_I_ALL) {
                 ...
                 * double-click).  Set the focus and top the icon if
                 * appropriate.
                 */
...
void HandleIconBoxButtonPress(ClientData *pCD, XButtonEvent *buttonEvent,
                              Context subContext) {
        ...
        if ((P_ICON_BOX(pCD)->pCD_iconBox == wmGD.keyboardFocus) ||
            (P_ICON_BOX(pCD)->pCD_iconBox == wmGD.nextKeyboardFocus)) {
                XmProcessTraversal(
                    XtWindowToWidget(DISPLAY, ICON_FRAME_WIN(pCD)),
                    XmTRAVERSE_CURRENT);
        }
...
Boolean HandleCKeyPress(ClientData *pCD, XKeyEvent *keyEvent) {
        ...
        if (wmGD.menuActive) {
                 ...
                 * was not one of them.  We will check for an iconbox icon
                 * widget key and for pass keys mode and then have the toolkit
                 * dispatch the event, without rechecking the client accelerator
                 * list.
                 */
        ...
        if (((keyEvent->window == ICON_FRAME_WIN(pCD)) ||
            ...
            P_ICON_BOX(pCD)) {
                /*
                 * This is a non-grabbed key that is intended for the icon
                 * widget in the iconbox.
                 */
        ...
        if (checkKeyEvent && (keyEvent->window == ICON_FRAME_WIN(pCD))) {
                if ((checkKeyEvent = HandleKeyPress(
                         keyEvent, ACTIVE_PSD->keySpecs, True, F_CONTEXT_ICON,
...
void CheckButtonReleaseBuiltin(XButtonEvent *buttonEvent, Context context,
            ...
            (context & F_CONTEXT_ICON)) {
                ...
                if (wmGD.iconClick &&
                    (wmGD.clickData.clickContext == F_SUBCONTEXT_I_ALL)) {
                        ...
                        /*
                         * Post the system menu with traversal on (Button 1
                         * should be used to manipulate the menu).
                         */
                        pCD->grabContext = F_CONTEXT_ICON;
                        PostMenu(pCD->systemMenuSpec, pCD, 0, 0, NoButton,
                                 F_CONTEXT_ICON, POST_STICKY,
                                 (XEvent *)buttonEvent);
                }
        }
                ...
                if ((wmGD.iconClick) &&
                    (((pCD->clientState == MINIMIZED_STATE) &&
                     ...
                     (wmGD.clickData.clickContext == F_SUBCONTEXT_IB_WICON))) {
                         ...
                         * should be used to manipulate the menu.
                         */
                        if ((wmGD.clickData.clickContext ==
                             F_SUBCONTEXT_IB_IICON) &&
                            (pCD->clientState == MINIMIZED_STATE)) {
                                pCD->grabContext = F_SUBCONTEXT_IB_IICON;
                                PostMenu(pCD->systemMenuSpec, pCD, 0, 0,
                                         NoButton, F_SUBCONTEXT_IB_IICON,
                                         POST_STICKY, (XEvent *)buttonEvent);
                        } else {
                                pCD->grabContext = F_SUBCONTEXT_IB_WICON;
                                PostMenu(pCD->systemMenuSpec, pCD, 0, 0,
                                         NoButton, F_SUBCONTEXT_IB_WICON,
                                         POST_STICKY, (XEvent *)buttonEvent);
                        }
                }
        }
...
void HandleCMotionNotify(ClientData *pCD, XMotionEvent *motionEvent) {
        ...
        /*
         * Do pre-move processing (to support the move threshold) if
         * appropriate:
         */
        ...
        if (wmGD.preMove) {
                ...
                if ((diffX >= wmGD.moveThreshold) ||
                    (diffY >= wmGD.moveThreshold)) {
                        /*
                         * The move threshold has been exceded; start the config
                         * action.
                         */
                        ...
                        if (wmGD.configAction == MOVE_CLIENT) {
                                HandleClientFrameMove(pCD,
                                                      (XEvent *)motionEvent);
                        } else if (wmGD.configAction == RESIZE_CLIENT) {
                                HandleClientFrameResize(pCD,
                                                        (XEvent *)motionEvent);
                        }
                }
        }
...
void HandleCEnterNotify(ClientData *pCD, XEnterWindowEvent *enterEvent) {
        ...
        MatchFound =
            XCheckTypedWindowEvent(DISPLAY, enterWindow, LeaveNotify, &report);
        ...
        if ((enterEvent->detail != NotifyInferior) &&
            ...
            ((wmGD.keyboardFocusPolicy == KEYBOARD_FOCUS_POINTER) ||
             (wmGD.colormapFocusPolicy == CMAP_FOCUS_POINTER))) {
                /*
                 * Make sure that EnterNotify is applicable; don't do anything
                 * if the window is minimized (not currently visible) or the
                 * event is associated with an icon in the icon box.
                 */
...

                {
                        if (wmGD.keyboardFocusPolicy ==
                            KEYBOARD_FOCUS_POINTER) {
                                /*
                                 * Set the focus only if the window does not
                                 * currently have or if another window is in the
                                 * process of getting the focus (this check
                                 * avoids redundant focus setting).
                                 */

                                if ((pCD != wmGD.keyboardFocus) ||
                                    (pCD != wmGD.nextKeyboardFocus)) {

                                        Do_Focus_Key(pCD, enterEvent->time,
                                                     ALWAYS_SET_FOCUS);

                                        /* Does the event need to be replayed
                                         * for modalized windows ? */
                                        if (wmGD.replayEnterEvent)
                                                /* Yes, save the event. */
                                                memcpy(
                                                    &wmGD.savedEnterEvent,
                                                    enterEvent,
...
void HandleCLeaveNotify(ClientData *pCD, XLeaveWindowEvent *leaveEvent) {
        ...
        if (P_ICON_BOX(pCD) &&
            ...
            (P_ICON_BOX(pCD)->pCD_iconBox != wmGD.nextKeyboardFocus)) {
                XCheckTypedWindowEvent(DISPLAY, leaveWindow, EnterNotify,
                                       &report);
        }
        ...
        if ((focusChangeEvent->window == ICON_FRAME_WIN(pCD)) &&
            P_ICON_BOX(pCD)) {
                ...
                if (wmGD.keyboardFocus != pCD) {
                        ...
                        /* Re: CR 4896 */
                        /* this part added to try and fix the %#$!@!!&* focus
                           bug.      */
                        /* this seems to solve most of the problem.  This still
                           leaves  */
                        /* times when clicking on an icon toggles the focus back
                           to the */
                        /* the previous focus window. */
                        /* Another patch was added to WmEvent.c to fix that
                           problem.    */
                        else {
                                SetKeyboardFocus(pCD, REFRESH_LAST_FOCUS);
                                if (setupNextFocus)
                        ...
                        /*
                         * The client window was withdrawn (unmapped or
                         * destroyed). Reset the focus.
                         * !!! pointer focus !!!
                         */
                        ...
                        if (wmGD.autoKeyFocus) {
                                 ...
                                 * !!! */
                                AutoResetKeyFocus(wmGD.keyboardFocus,
                                                  GetTimestamp());
                        } else {
...
void HandleCConfigureRequest(ClientData *pCD,
            ...
            (mask & (CWX | CWY | CWWidth | CWHeight | CWBorderWidth))) {
                if (pCD->maxConfig) {
                        ProcessNewConfiguration(
                            pCD, (mask & CWX) ? configureRequest->x : pCD->maxX,
                        ...
                        ProcessNewConfiguration(
                            pCD,
                            (mask & CWX) ? configureRequest->x
                                         : pCD->clientX - xOff,
                            (mask & CWY) ? configureRequest->y
                                         : pCD->clientY - yOff,
                            (unsigned int)((mask & CWWidth)
                                               ? configureRequest->width
                                               : pCD->clientWidth),
        ...
        if (mask & CWStackMode) {
                ...
                if (changeMask & CWSibling) {
                        if (XFindContext(DISPLAY, configureRequest->above,
                                ...
                                /*
                                 * For client requests only primary windows can
                                 * be restacked relative to one another.
                                 */

                                pcdLeader = FindTransientTreeLeader(pCD);
                                pcdSibling =
                                    FindTransientTreeLeader(pcdSibling);
                                if (pcdLeader == pcdSibling) {
                                        ...
                                        if ((stackMode == Above) ||
                                            (stackMode == TopIf)) {
                                                /* lower the window to just
                                                 * above the sibling */
                                                Do_Lower(pcdLeader, pStackEntry,
                                                         STACK_NORMAL);
                                        } else if ((stackMode == Below) ||
                                                   (stackMode == BottomIf)) {
                                                /* raise the window to just
                                                 * below the sibling */
...
void HandleCColormapNotify(ClientData *pCD, XColormapEvent *colorEvent) {
        ...
        if (colorEvent->new) {
                ...
                if (WmGetWindowAttributes(colorEvent->window)) {
                        ...
                        /*
                         * Identify the colormap that the window manager has
                         * associated with the window.
                         */

#ifndef IBM_169380
                if ((pCD->clientCmapCount == 0) &&
                    (colorEvent->window == pCD->client))
#endif
                        if (pCD->clientCmapCount == 0) {
                        /* no subwindow colormaps; change top-level window
                         * colormap */
#ifdef IBM_169380
                ...
                if (!XFindContext(DISPLAY, colorEvent->window,
                                  ...
                                  (caddr_t *)&cmap_window_data)) {
                        /*
                         * The WM_COLORMAP_WINDOWS property of a toplevel window
                         * may specify colorEvent->window.  If so, we must
                         * update the colormap information it holds in
                         * clientCmapList.
                         */
                        ClientData *any_pCD;
                        int j;
                        ...
                        for (j = 0; cmap_window_data[j] != NULL; j++) {
                                ...
                                for (i = 0; i < any_pCD->clientCmapCount; i++) {
                                        if (any_pCD->cmapWindows[i] ==
                                            colorEvent->window) {
                                                if (colorEvent->colormap ==
                                                    None) {
                                                        /* use the workspace
                                                         * colormap */
                                                        any_pCD->clientCmapList
                                                            [i] =
                                                            ACTIVE_PSD
                                                                ->workspaceColormap;
                                                } else {
...
void HandleClientMessage(ClientData *pCD, XClientMessageEvent *clientEvent) {
        unsigned int newState = NORMAL_STATE;
 ...
 *
 *************************************<->***********************************/
void HandleCShapeNotify(ClientData *pCD, XShapeEvent *shapeEvent) {
        if (pCD) {
...
WmScreenData *GetScreenForWindow(win) Window win;
