...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmWinState.c /main/6 1996/06/20 09:39:39 rswiston $"
#endif
#endif
/*
...
void SetClientStateWithEventMask(ClientData *pCD, int newState, Time setTime,
                                 unsigned int event_mask) {
        ...
        case MAXIMIZED_STATE: {
                SetupWindowStateWithEventMask(pCD, newState, setTime,
                                              event_mask);
#ifdef WSM
        ...
        case MINIMIZED_STATE: {
                ...
                /*
                 * Transient windows are minimized with the rest of the
                    ...
                    (pCD->clientState == MAXIMIZED_STATE)) {
                        ...
                        if (clientHasFocus ||
                            ((wmGD.nextKeyboardFocus == pCD) ||
                             (pCD->transientChildren && wmGD.keyboardFocus &&
                              (pCD == FindTransientTreeLeader(
                                          wmGD.nextKeyboardFocus))))) {
                                /*
                                 * Give up the keyboard focus when minimized
                                 * (including the case in which an associated
                                 * transient window has the focus).  Immediately
                                 * remove the focus indication from the window
                                 * being minimized.
                                 */
                ...
                if ((pSD->useIconBox) && P_ICON_BOX(pCD)) {
                        if ((pCD->clientFlags & ICON_BOX) &&
                            ACTIVE_ICON_TEXT_WIN) {
                                /*
                                 * Hide active icon text window and reparent it
                                 * to root
                                 */
...
static void SetupWindowStateWithEventMask(ClientData *pCD, int newState,
        ...
        /*
         * A transient window is not restored or maximized if the transient
         * leader is minimized.
         */
        ...
        if (newState == NORMAL_STATE) {
                if (pCD->maxConfig == True) {
                        /*
                         * The configuration function uses maxConfig to
                         * determine what the current configuration is (and then
                         * resets maxConfig) and uses the state paramenter to
                         * determine what the new configuration is.
                         */
                        ...
                        if (pSD->useIconBox && P_ICON_BOX(pCD) &&
                            !(pCD->clientFlags & ICON_BOX)) {
                                ...
                                if (wmGD.iconAutoPlace) {
                                        for (wsI = 0; wsI < pCD->numInhabited;
                                             wsI++) {
                                                iplace =
                                                    pCD->pWsList[wsI].iconPlace;
...
#else  /* WSM */
                                if ((wmGD.iconAutoPlace) &&
                                    (ICON_PLACE(pCD) != NO_ICON_PLACE)) {
                                        pWS->IPData.placeList[ICON_PLACE(pCD)]
                                            .pCD = NULL;
                                }
#endif /* WSM */
                        }
...
void ConfigureNewState(ClientData *pcd) {
        ...
        if (pcd->maxConfig) {
                ...
                XResizeWindow(DISPLAY, pcd->client, (unsigned int)pcd->maxWidth,
        ...
        if (event_mask)
                doGrab = (Success ==
                          XGrabPointer(DISPLAY, DefaultRootWindow(DISPLAY),
                                       False, event_mask, GrabModeAsync,
 ...
 *  --------
 ******************************<->***********************************/
void SlideOutTimerProc(XtPointer client_data, XtIntervalId *id) {
        ...
        if (pSOR) {
                ...
                if (pSOR->currHeight > 0) {
                        XMoveResizeWindow(DISPLAY, pSOR->coverWin, pSOR->currX,
                                          pSOR->currY, pSOR->currWidth,
                                          pSOR->currHeight);
...
static void SlideWindowOut(ClientData *pCD) {
        ...
        if (pSOR) {
                 ...
                 */
                pSOR->incHeight =
                    (Dimension)(DisplayHeight(DISPLAY, SCREEN_FOR_CLIENT(pCD)) /
                                SLIDE_UP_DIVISOR);
        }
        ...
        if ((pCD->slideDirection != SLIDE_NOT) && pSOR &&
            (pSOR->incHeight < pCD->frameInfo.height)) {
                ...
                pSOR->coverWin = XCreateWindow(
                    DISPLAY, RootWindow(DISPLAY, SCREEN_FOR_CLIENT(pCD)),
                    pSOR->currX, pSOR->currY, pSOR->currWidth, pSOR->currHeight,
                    0, CopyFromParent, InputOnly, CopyFromParent, mask,
                    &window_attribs);
                ...
                XConfigureWindow(DISPLAY, pSOR->coverWin, mask,
                                 &window_changes);
                ...
                XMoveResizeWindow(DISPLAY, pSOR->coverWin, pSOR->currX,
                                  pSOR->currY, pSOR->currWidth,
                                  pSOR->currHeight);
                XMoveResizeWindow(DISPLAY, pCD->clientFrameWin, pSOR->currX,
                                  pSOR->currY, pSOR->currWidth,
                                  pSOR->currHeight);
...
void SlideSubpanelBackIn(ClientData *pCD, Widget wSubpanel) {
        SlideOutRec *pSOR;
                ...
                if ((ICON_PLACE(pCD) == NO_ICON_PLACE) ||
                    ((pWS->IPData.placeList[ICON_PLACE(pCD)].pCD) &&
                     (pWS->IPData.placeList[ICON_PLACE(pCD)].pCD != pCD))) {
                        ...
                        if ((ICON_PLACE(pCD) = GetNextIconPlace(
                                 &pWS->IPData)) == NO_ICON_PLACE) {
                                ICON_PLACE(pCD) = CvtIconPositionToPlace(
                                    &pWS->IPData, pCD->clientX, pCD->clientY);
                        }
                        ...
                        if ((&pCD->iconEntry != pSD->lastClient) &&
                            (pSD->lastClient)) {
                                if (pSD->lastClient->type == MINIMIZED_STATE) {
                                        windowChanges.sibling = ICON_FRAME_WIN(
                                            pSD->lastClient->pCD);
                                } else {
