...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmOL.c /main/4 1995/11/01 11:45:08 rswiston $"
#endif
#endif
...
#define ValidPropertyList(pcd) ((pcd)->paInitialProperties != NULL)
    ...
    void
    InitOLCompat(void) {
        wmGD.xa_OL_WIN_ATTR = XmInternAtom(DISPLAY, OL_WIN_ATTR, False);
        wmGD.xa_OL_DECOR_RESIZE = XmInternAtom(DISPLAY, OL_DECOR_RESIZE, False);
        wmGD.xa_OL_DECOR_HEADER = XmInternAtom(DISPLAY, OL_DECOR_HEADER, False);
        wmGD.xa_OL_DECOR_CLOSE = XmInternAtom(DISPLAY, OL_DECOR_CLOSE, False);
        wmGD.xa_OL_DECOR_PIN = XmInternAtom(DISPLAY, OL_DECOR_PIN, False);
        wmGD.xa_OL_DECOR_ADD = XmInternAtom(DISPLAY, OL_DECOR_ADD, False);
        wmGD.xa_OL_DECOR_DEL = XmInternAtom(DISPLAY, OL_DECOR_DEL, False);
        wmGD.xa_OL_WT_BASE = XmInternAtom(DISPLAY, OL_WT_BASE, False);
        wmGD.xa_OL_WT_COMMAND = XmInternAtom(DISPLAY, OL_WT_CMD, False);
        wmGD.xa_OL_WT_HELP = XmInternAtom(DISPLAY, OL_WT_HELP, False);
        wmGD.xa_OL_WT_NOTICE = XmInternAtom(DISPLAY, OL_WT_NOTICE, False);
        wmGD.xa_OL_WT_OTHER = XmInternAtom(DISPLAY, OL_WT_OTHER, False);
        wmGD.xa_OL_PIN_IN = XmInternAtom(DISPLAY, OL_PIN_IN, False);
        wmGD.xa_OL_PIN_OUT = XmInternAtom(DISPLAY, OL_PIN_OUT, False);
        wmGD.xa_OL_MENU_LIMITED = XmInternAtom(DISPLAY, OL_MENU_LIMITED, False);
        wmGD.xa_OL_MENU_FULL = XmInternAtom(DISPLAY, OL_MENU_FULL, False);
...
Boolean HasOpenLookHints(ClientData *pCD) {
        ...
        if (ValidPropertyList(pCD) && HasProperty(pCD, wmGD.xa_OL_WIN_ATTR)) {
                rval = True;
...
OLWinAttr *GetOLWinAttr(ClientData *pCD) {
        ...
        ret_val = XGetWindowProperty(
            DISPLAY, pCD->client, wmGD.xa_OL_WIN_ATTR, 0L, ENTIRE_CONTENTS,
            False, wmGD.xa_OL_WIN_ATTR, &actual_type, &actual_format, &nitems,
 ...
 *
 *************************************<->***********************************/
Boolean GetOLDecorFlags(ClientData *pCD, Atom property, unsigned long *pDecor) {
        int status, i;
        Boolean rval;
        Atom actual_type;
        int actual_format;
        ...
        status = XGetWindowProperty(DISPLAY, pCD->client, property, 0L,
                                    ENTIRE_CONTENTS, False, XA_ATOM,
                                    &actual_type, &actual_format, &nitems,
                                    &leftover, (unsigned char **)&pAtoms);

        if ((status != Success) || !pAtoms || (nitems == 0) ||
 ...
 *
 *************************************<->***********************************/
void ProcessOLDecoration(ClientData *pCD) {
        ...
        if (HasOpenLookHints(pCD) &&
            ((pOLWinAttr = GetOLWinAttr(pCD)) != NULL)) {
                ...
                if ((pOLWinAttr->flags & WA_WINTYPE) == 0) {
                         ...
                         */
                        decorMask =
                            WM_DECOR_TITLE | WM_DECOR_SYSTEM | WM_DECOR_RESIZEH;
                         ...
                         */
                        decorMask = (WM_DECOR_TITLE | WM_DECOR_SYSTEM |
                                     WM_DECOR_BORDER);
