 ...
 */
void SetupWorkspaceButtons(form, top) Widget form;
Widget top;
{
        Window root;
        ...
        if (DtWsmGetWorkspaceList(XtDisplay(form), root, &paWs,
                                  (int *)&numWorkspaces) == Success) {
                ...
                for (i = 0; i < numWorkspaces; i++, pWid++) {
                        ...
                        if (left == None) {
                                ...
                                XtSetArg(args[n], XmNleftAttachment,
                                         XmATTACH_WIDGET);
                                n++;
                                XtSetArg(args[n], XmNleftWidget, left);
                                n++;
                        }
                        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
                        n++;
                         ...
                         */
                        *pWid = XtCreateWidget(
                            "button", xmToggleButtonWidgetClass, form, args, n);
                        /*
                         * add callback
                         */
                ...
                fprintf(stderr,
                        "Failed to get workspace info -- dtwm not running!\n");
                exit(1);
        }
}
...
void ShowWorkspaceOccupancy() {
        ...
        rval = DtWsmGetWorkspacesOccupied(
            XtDisplay(toplevel), XtWindow(toplevel), &paWsIn, &numWsIn);
 ...
 */
void showCB(w, client_data, call_data) Widget w; /*  widget id		*/
caddr_t client_data;                             /*  data from application   */
caddr_t call_data;                               /*  data from widget class  */
{ ShowWorkspaceOccupancy(); }
 ...
 */
void setCB(w, client_data, call_data) Widget w; /*  widget id		*/
caddr_t client_data;                            /*  data from application   */
caddr_t call_data;                              /*  data from widget class  */
{
        int numSet;
        ...
        DtWsmSetWorkspacesOccupied(XtDisplay(toplevel), XtWindow(toplevel),
                                   paWsSet, numSet);
}
 ...
 */
void wsCB(w, client_data, call_data) Widget w; /*  widget id		*/
caddr_t client_data;                           /*  data from application   */
caddr_t call_data;                             /*  data from widget class  */
{
 ...
 */
void allWsCB(w, client_data, call_data) Widget w; /*  widget id		*/
caddr_t client_data;                              /*  data from application   */
caddr_t call_data;                                /*  data from widget class  */
