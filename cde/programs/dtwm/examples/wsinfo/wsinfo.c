...
char **argv;
{
        ...
        n++;
        toplevel = XtAppInitialize(&app_context, "XMDemos", NULL, 0, &argc,
                                   argv, NULL, args, n);
         ...
         */
        top = QUIT_BUTTON =
            XtCreateWidget("button", xmPushButtonWidgetClass, form, args, n);
        XmStringFree(xms);
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, top);
        n++;
        /*
         * create button
         */
         ...
         */
        top = SEPARATOR =
            (Widget)XmCreateSeparatorGadget(form, "separator", args, n);
         ...
         * create label
         */
        WS_LABEL = XtCreateWidget("label", xmLabelWidgetClass, form, args, n);
        XmStringFree(xms);
        ...
        XtSetArg(args[n], XmNlabelType, XmSTRING);
        n++;
        XtSetArg(args[n], XmNlabelString, xmsBlank);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, WS_LABEL);
        n++;
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, top);
         ...
         */
        top = WS_VALUE =
            XtCreateWidget("label", xmLabelWidgetClass, form, args, n);
        ...
        n++;
        XtSetArg(args[n], XmNrightWidget, WS_LABEL);
        n++;
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, top);
        ...
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, TITLE_LABEL);
        n++;
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, top);
        ...
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, BACKDROP_LABEL);
        n++;
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, top);
        ...
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, COLORSET_LABEL);
        n++;
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, top);
         ...
         */
        (void)DtWsmAddCurrentWorkspaceCallback(
            toplevel, (DtWsmWsChangeProc)wschangecb, NULL);
        ...
        if (rval == Success) {
                ...
                if (rval == Success) {
                        ...
                        if (pWsInfo->backdropName != None)
                                pchName =
                                    XmGetAtomName(dpy, pWsInfo->backdropName);
                        else
 ...
 */
void quitCB(w, client_data, call_data) Widget w; /*  widget id		*/
 ...
 */
void updateCB(w, client_data, call_data) Widget w; /*  widget id
                                                    */
caddr_t client_data; /*  data from application   */
caddr_t call_data;   /*  data from widget class  */
{ ShowCurrentWorkspaceInfo(); }
...
*/
DtWsmWsChangeProc wschangecb(w, atom, client_data) Widget w;
Atom atom;
...
*/
static int _HandleXErrors(display, errorEvent) Display *display;
...
*/
void InstallXErrorHandler(display) Display *display;
{
        char buffer[BUFSIZ];
        int i;
        ...
        for (i = 0; i < NUM_E_STRINGS; i++) {
                XGetErrorDatabaseText(display, "XlibMessage",
                                      pchErrorFormatNames[i], " %d ", buffer,
                                      BUFSIZ);
