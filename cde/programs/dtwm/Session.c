...
static char *SessionFileGetName()
...

{
        ...
        if ((fd = fopen(session_file, "r")) != NULL) {
                ...
                for (i = 0; i < subpanel_posted_count; i++) {
                        fscanf(fd, "%s%d%d%d", name_buffer1, &subpanel_x,
                               &subpanel_y, &subpanel_torn);
                        ...
                        for (j = 0; j < subpanel_count; j++) {
                                if (strcmp(name_buffer1,
                                           subpanel_data[j]
                                               ...
                                               .parsed_value) == 0) {
                                        /*  If the subpanel is torn, modify the
                                         * window manager    */
                                        /*  behavior hints so that it can be
                                         * positioned properly  */
                                        ...
                                        if (subpanel_torn != 0) {
                                                ...
                                                vHints.flags =
                                                    DtWM_HINTS_BEHAVIORS |
                                                    DtWM_HINTS_ATTACH_WINDOW;
                                                vHints.behaviors =
                                                    DtWM_BEHAVIOR_PANEL |
                                                    DtWM_BEHAVIOR_SUBPANEL |
                                                    DtWM_BEHAVIOR_SUB_RESTORED;
                                                vHints.attachWindow =
                                                    XtWindow(panel.shell);

                                                _DtWsmSetDtWmHints(
                                                    XtDisplay(panel.shell),
                                                    XtWindow(subpanel_data[j]
                                                                 ->shell),
                                                    &vHints);
                                        }

                                        /*  Manage and position the form and
                                         * shell of the subpanel  */
                                        /*  Note: this is a close copy of the
                                         * ArrowCB posting       */
                                        /*  function logic. */

                                        XtSetMappedWhenManaged(
                                            subpanel_data[j]->shell, False);
                                        XtManageChild(subpanel_data[j]->form);

                                        XtSetArg(al[0], XmNx, subpanel_x);
                                        XtSetArg(al[1], XmNy, subpanel_y);
                                        XtSetValues(subpanel_data[j]->form, al,
                                                    2);

                                        XtManageChild(subpanel_data[j]->shell);
                                        XtSetMappedWhenManaged(
                                            subpanel_data[j]->shell, True);
                ...
                for (i = 0; i < panel.dynamic_data_count; i++) {
                        fscanf(fd, "%s%s%d%s%d%d", name_buffer1, name_buffer2,
                               &component_type, name_buffer3,
                               &parent_component_type, &delete_value);

                        /*  Stat the file name to see if it still exists.  If
                         * not,    */
                        /*  continue at the next iteration of the loop.  This
                         * has     */
                        /*  the effect of not putting the data into the dynamic
                         * list  */
                        ...
                        /*  Loop through the subpanels and verify that each of
                         * the    */
                        /*  records matches component within the hierarchy.  If
                         * not,  */
                        ...
                        for (j = 0; j < subpanel_count; j++) {
                                ...
                                /*  If the data is a reference to a deleted
                                 * component, put   */
                                /*  it in the list.  If it is a subpanel, verify
                                 * that there  */
                                /*  is a subpanel of that name and a main
                                 * control parent of  */
                                /*  the correct name.  If it is a control,
                                 * reloop through    */
                                /*  the subpanel set a look for a control which
                                 * matches the  */
                                /*  name and a subpanel which matches the parent
                                 * name.       */
                                ...
                                if (delete)
                                        have_it = True;
                                         ...
                                         strcmp(
                                             subpanel_data[j]
                                                 ->element_values
                                                     [SUBPANEL_CONTAINER_NAME]
                                                 .parsed_value,
                                             name_buffer3) == 0) {
                                        have_it = True;
                                } else {
                                        if (parent_component_type == SUBPANEL) {
                                                for (k = 0; k < subpanel_count;
                                                     k++) {
                                                        if (strcmp(
                                                                subpanel_data[k]
                                                                    ->element_values
                                        ...
                                        } else if (parent_component_type ==
                                                   BOX) {
                                                for (k = 0;
                                                     k < panel.box_data_count;
                                                     k++) {
                                                        if (strcmp(
                                                                panel
                                                                    .box_data[k]
                                ...
                                if (have_it) {
                                        panel.dynamic_data_list
                                            [dynamic_data_count] =
                                            (DynamicComponent *)XtMalloc(
                                                sizeof(DynamicComponent));
                ...
                if (fscanf(fd, "%d", &mapped) == 1) {
                        if (mapped == 0)
                                XIconifyWindow(XtDisplay(panel.shell),
                                               XtWindow(panel.shell),
                                               XScreenNumberOfScreen(
                                                   XtScreen(panel.shell)));
                }
...

{
        ...
        if ((fd = fopen(session_file, "w")) != NULL) {
                ...
                for (i = 0; i < panel.box_data_count; i++) {
                        ...
                        for (j = 0; j < box_data->control_data_count; j++) {
                                ...
                                if (control_data->subpanel_data != NULL) {
                                        ...
                                        if (subpanel_data[subpanel_count]
                                                ->default_control !=
                                            subpanel_data[subpanel_count]
                                                ->parent_control_data)
                ...
                XGetWindowAttributes(XtDisplay(panel.shell),
                                     XtWindow(panel.shell), &window_attributes);
...

{
        ...
        panel.dynamic_data_list[count]->component_name =
            XtNewString(component_name);
...

{
        ...
        for (i = 0; i < panel.dynamic_data_count; i++) {
                if (strcmp(file_name, panel.dynamic_data_list[i]->file_name) ==
                    0) {
                        XtFree((char *)panel.dynamic_data_list[i]->file_name);
                        XtFree(
                            (char *)panel.dynamic_data_list[i]->component_name);
                        ...
                        for (j = i; j < panel.dynamic_data_count - 1; j++)
                                panel.dynamic_data_list[j] =
                                    panel.dynamic_data_list[j + 1];
