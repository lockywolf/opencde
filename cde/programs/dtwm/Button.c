...
static void UpdateGCs(Widget w);
...
static void ArmTimeout(Widget w, XtIntervalId *id);
static void Enter(Widget wid, XEvent *event, String *params,
                  Cardinal *num_params);
static void Leave(Widget wid, XEvent *event, String *params,
                  Cardinal *num_params);
static void CallCallback(DtButtonGadget g, XtCallbackList cb, int reason,
                         XEvent *event);
static void DefaultSelectColor(Widget widget, int offset, XrmValue *value);

/*  Resource list for button  */

#define R_Offset(field) XtOffset(DtButtonGadget, button.field)
static XtResource resources[] = {
    {XmNcursorFont, XmCCursorFont, XmRInt, sizeof(int), R_Offset(cursor_font),
     XmRImmediate, (XtPointer)1},
    {XmNarmColor, XmCArmColor, XmRPixel, sizeof(Pixel), R_Offset(arm_color),
    ...
    {XmNimageName, XmCString, XmRString, sizeof(String), R_Offset(image_name),
     XmRImmediate, (XtPointer) ""},

    {XmNcallback, XmCCallback, XmRCallback, sizeof(XtCallbackList),
     R_Offset(callback), XmRImmediate, (caddr_t)NULL},

    {XmNmultiClick, XmCMultiClick, XmRMultiClick, sizeof(unsigned char),
     R_Offset(multiClick), XmRImmediate, (XtPointer)XmMULTICLICK_KEEP}};
...
externaldef(dtbuttonwidgetclass)
    WidgetClass dtButtonGadgetClass = (WidgetClass)&dtButtonGadgetClassRec;
        ...
        if (B_ImageName(new_g) != NULL) {
                ...
                B_Pixmap(new_g) =
                    XmGetPixmap(XtScreen(new_g), B_ImageName(new_g),
                                M_BottomShadowColor(mw), M_TopShadowColor(mw));
                B_Mask(new_g) = _DtGetMask(XtScreen(new_g), B_ImageName(new_g));
                ...
                if (B_Pixmap(new_g) != XmUNSPECIFIED_PIXMAP) {
                        ...
                        XGetGeometry(XtDisplay(new_g), B_Pixmap(new_g), &root,
...
static void Redisplay(Widget w, XEvent *event, Region region) {
        ...
        XFillRectangle(XtDisplay(bg), XtWindow(bg), gc, G_X(bg) + s_t + h_t,
                       G_Y(bg) + s_t + h_t, G_Width(bg) - 2 * (s_t + h_t),
        ...
        if (B_Mask(bg) != XmUNSPECIFIED_PIXMAP) {
                ...
                XSetClipOrigin(
                    XtDisplay(bg), gc, x,
                    G_Y(bg) +
                        (Position)(G_Height(bg) - B_PixmapHeight(bg)) / 2);
        } else
                gc = bg->button.gc_normal;

        pix = B_Pixmap(bg);
        if ((gc != NULL) && (pix != XmUNSPECIFIED_PIXMAP)) {
                XCopyArea(XtDisplay((Widget)bg), pix, XtWindow((Widget)bg), gc,
                          0, 0, B_PixmapWidth(bg), B_PixmapHeight(bg), x,
                          G_Y(bg) +
                              (Position)(G_Height(bg) - B_PixmapHeight(bg)) /
                                  2);
        }
...
static void ArmAndActivate(Widget w, XEvent *event, String *params,
        ...
        if (bg->object.being_destroyed == False) {
                bg->button.timer = XtAppAddTimeOut(
                    ...
                    (XtTimerCallbackProc)ArmTimeout, (caddr_t)bg);
        }
}
...
static void InputDispatch(Widget w, XButtonEvent *event, Mask event_mask) {
        ...
        if (event_mask & XmARM_EVENT || event_mask & XmMULTI_ARM_EVENT) {
                if (event->button == Button1)
                        Arm(w, (XEvent *)event, (String *)NULL,
                            (Cardinal *)NULL);
                ...
                if (event->button == Button1) {
                        if (event->x >= G_X(bg) &&
                            ...
                            event->y <= (Position)(G_Y(bg) + G_Height(bg))) {
                                Activate(w, (XEvent *)event, (String *)NULL,
                                         (Cardinal *)NULL);
                                Disarm(w, (XEvent *)event, (String *)NULL,
                                       (Cardinal *)NULL);
                        } else
                                Disarm(w, (XEvent *)event, (String *)NULL,
                                       (Cardinal *)NULL);
                ...
                _XmSocorro(w, (XEvent *)event, (String *)NULL,
                           (Cardinal *)NULL);
                ...
                _XmFocusInGadget(w, (XEvent *)event, (String *)NULL,
                                 (Cardinal *)NULL);
        else if (event_mask & XmFOCUS_OUT_EVENT)
                _XmFocusOutGadget(w, (XEvent *)event, (String *)NULL,
                                  (Cardinal *)NULL);
}
...
static Boolean VisualChange(Widget w, Widget current_w, Widget new_w) {
        ...
        if (update) {
                if (B_ImageName(bg) != NULL) {
                        ...
                        B_Pixmap(bg) =
                            XmGetPixmap(XtScreen(bg), B_ImageName(bg),
                                        M_TopShadowColor(new_m),
                                        M_BottomShadowColor(new_m));

                        if (B_Pixmap(bg) != XmUNSPECIFIED_PIXMAP)
                                B_Mask(bg) = (Pixmap)_DtGetMask(
                                    XtScreen(bg), B_ImageName(bg));
...
*/
static void CallCallback(DtButtonGadget w, XtCallbackList cb, int reason,
                         XEvent *event) {
...
Widget DtCreateButtonGadget(Widget parent, char *name, ArgList arglist,
                            Cardinal argcount) {
        return (XtCreateWidget(name, dtButtonGadgetClass, parent, arglist,
                               argcount));
}
