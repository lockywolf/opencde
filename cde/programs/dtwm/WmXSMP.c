...
static void getClientGeometry(ClientData *, int *, int *, unsigned int *,
                              unsigned int *);
static Boolean getProxyClientInfo(ClientData *, ProxyClientInfo *);
static Bool cmpProxyClientProc(XrmDatabase *, XrmBindingList, XrmQuarkList,
...
static void smSaveYourselfCallback(Widget w, XtPointer clientData,
                                   XtPointer callData) {
        ...
        if (cpToken->shutdown &&
            (cpToken->cancel_shutdown || cpToken->request_cancel ||
         ...
         */
        if ((newClientDB = SaveClientResourceDB()) != (XrmDatabase)NULL) {
                ...
                if (wmGD.dbFileName != (char *)NULL) {
                        ...
                        nargs = 0;
                        XtSetArg(args[nargs], XtNrestartCommand, newRestartCmd);
                        nargs++;
                        XtSetArg(args[nargs], XtNdiscardCommand, newDiscardCmd);
                        nargs++;
...
static void smDieCallback(Widget w, XtPointer clientData, XtPointer callData) {
        /* We assume we've saved our state by the time this is called. */
        ExitWM(0);
}

static void buildDBFileName(char fileNameBuf[MAXPATHLEN], Boolean doingSave) {
#ifdef WSM
...
static char **getNewRestartCmd(void) {
        ...
        nargs = 0;
        XtSetArg(args[nargs], XtNrestartCommand, &restartCmd);
        nargs++;
        ...
        for (argc = 0, argP = restartCmd; *argP != (char *)NULL;
             argP++, argc++) {
                if (strcmp(*argP, dbFileArgStr) == 0) {
                        if (*(++argP) == (char *)NULL)
                                break;
                        ...
                        fileArgIndex =
                            argc++; /* Point at dbFileArgStr, not filename */
                }
        }
...
static Boolean addClientToList(ClientData ***cdList, int *nClients,
                               ClientData *pCD) {
        ClientData **newPtr = (ClientData **)XtRealloc(
            (char *)*cdList, (*nClients + 1) * sizeof(ClientData *));
...
static Boolean findXSMPClientDBMatch(ClientData *pCD, char **workSpaceNamesP) {
        if (pCD->smClientID != (String)NULL) {
                if (wmGD.clientResourceDB != (XrmDatabase)NULL) {
                        ...
                        if ((resourcePtr = getXSMPResource(pCD, WMSAVE_X,
                                                           xPositionStr)) !=
                            (char *)NULL) {
                        ...
                        if ((resourcePtr = getXSMPResource(pCD, WMSAVE_Y,
                                                           yPositionStr)) !=
                            (char *)NULL) {
...
#ifndef WSM
                        if ((resourcePtr = getXSMPResource(pCD, WMSAVE_ICON_X,
                                                           iconXPosStr)) !=
                            (char *)NULL) {
                        ...
                        if ((resourcePtr = getXSMPResource(pCD, WMSAVE_ICON_Y,
                                                           iconYPosStr)) !=
                            (char *)NULL) {
                        ...
                        if ((workSpaceNamesP != (char **)NULL) &&
                            ((resourcePtr = getXSMPResource(
                                  pCD, WMSAVE_WORKSPACES, workspacesStr)) !=
                             (char *)NULL)) {
...
static Boolean getProxyClientInfo(ClientData *pCD,
        ...
        if ((textProperty.encoding != XA_STRING) ||
            (textProperty.format != 8) || (textProperty.value[0] == '\0')) {
        ...
        if (XGetWMClientMachine(wmGD.display, pCD->client, &textProperty))
                ...
                proxyClientInfo->wmClientMachine = (char *)NULL;
...
static Bool cmpProxyClientProc(XrmDatabase *clientDB,
        ...
        if (((wmCommand = getClientResource(clientID, wmCommandStr)) ==
             (char *)NULL) ||
            (strcmp(wmCommand, proxyClientInfo->wmCommand) != 0) ||
            ((clientScreen = getClientResource(clientID, screenStr)) ==
             (char *)NULL) ||
        ...
        if (!proxyClientInfo->wmClientMachine ||
            ((wmClientMachine = getClientResource(
                  clientID, wmClientMachineStr)) == (char *)NULL) ||
        ...
        if ((pCD->smClientID == (String)NULL) &&
            ...
            (!smClientDBCheckpointed)) {
                ...
                if ((proxyClientID = findProxyClientID(pCD)) != (char *)NULL) {
                        ...
                        if ((resourcePtr = getClientResource(proxyClientID,
                                                             xPositionStr)) !=
                            (char *)NULL) {
                                pCD->clientX = atoi(resourcePtr);
                                pCD->clientFlags |= SM_X;
                        ...
                        if ((resourcePtr = getClientResource(
                                 proxyClientID, iconXPosStr)) != (char *)NULL) {
                                ICON_X(pCD) = atoi(resourcePtr);
                                pCD->clientFlags |= SM_ICON_X;
                        }

                        if ((resourcePtr = getClientResource(
                                 proxyClientID, iconYPosStr)) != (char *)NULL) {
                                ICON_Y(pCD) = atoi(resourcePtr);
                                pCD->clientFlags |= SM_ICON_Y;
                        }
#endif
...
#ifdef WSM
                        if ((workSpaceNamesP != (char **)NULL) &&
                            ((resourcePtr = getClientResource(proxyClientID,
                                                              workspacesStr)) !=
                             (char *)NULL)) {
                                *workSpaceNamesP = XtNewString(resourcePtr);
                        }
...
#ifndef WSM
                        /* This is done in LoadClientIconPositions() if WSM
                         * defined. */
 ...
 */
static Boolean saveProxyClient(FILE *fp, ClientData *pCD, int clientIDNum) {
        char clientID[50];
        int clientState;
        ProxyClientInfo proxyClientInfo;
        int clientX, clientY;
...
static void dbRemoveProxyClientEntry(char *proxyClientID) {
        char resourceBuf[MAX_RESOURCE_LEN];
...
void AddSMCallbacks(void) {
        XtAddCallback(wmGD.topLevelW, XtNsaveCallback, smSaveYourselfCallback,
                      (XtPointer)NULL);
        XtAddCallback(wmGD.topLevelW, XtNdieCallback, smDieCallback,
                      (XtPointer)NULL);
}
...
void ResignFromSM(void) {
        if (wmGD.topLevelW) {
                XtVaSetValues(wmGD.topLevelW, XtNjoinSession, False, NULL);
        }
}
...
XrmDatabase SaveClientResourceDB(void) {
        ...
        XtVaGetValues(wmGD.topLevelW, XtNsessionID, &mySessionID, NULL);
        ...
        for (scr = 0; scr < wmGD.numScreens; scr++) {
                ...
                for (pCL = pSD->clientList; pCL != (ClientListEntry *)NULL;
                     pCL = pCL->nextSibling) {
...
void SortClientListByWorkspace(ClientData **clients, int nClients) {
        if (nClients > 0) {
                qsort((void *)clients, nClients, sizeof(ClientData *),
                      clientWorkspaceCompare);
        }
}
