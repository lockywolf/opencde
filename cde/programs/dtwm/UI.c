...
void SubpanelControlCreate(SubpanelData *, ControlData *, ControlData *, Widget,
                           Boolean, Boolean);
...
static XtCallbackRec customizeDropCB[] = {{CustomizeDropCB, NULL},
                                          {NULL, NULL}};
static XtCallbackRec customizeTransferDropCB[] = {
    {CustomizeTransferDropCB, NULL}, {NULL, NULL}};
...
void FrontPanelCreate(Widget toplevel)

{
        DtPanelShellWidget panel_shell;
        char *panel_name =
            (char *)panel.element_values[PANEL_NAME].parsed_value;
        ...
        ac++;
        XtSetArg(al[ac], XmNiconY, display_height);
        ac++;
        XtSetArg(al[ac], XmNmwmDecorations, MWM_DECOR_BORDER);
        ac++;
        ...
        ac = 0;
        XtSetArg(al[ac], XmNforeground, panel.inactive_pixel_set->fg);
        ac++;
        XtSetArg(al[ac], XmNbackground, panel.inactive_pixel_set->bg);
        ac++;
        XtSetArg(al[ac], XmNtopShadowColor, panel.inactive_pixel_set->ts);
        ac++;
        XtSetArg(al[ac], XmNbottomShadowColor, panel.inactive_pixel_set->bs);
        ac++;
        XtSetArg(al[ac], XmNselectColor, panel.inactive_pixel_set->sc);
        ac++;
        ...
        if ((intptr_t)panel.element_values[PANEL_DISPLAY_HANDLES]
                .parsed_value) {
                XtTranslations handle_translations = HandleInputTranslations();

                panel.left_handle =
                    PanelControlCreate(panel.form, "handle", handle_image);
                panel.right_handle =
                    PanelControlCreate(panel.form, "handle", handle_image);
                ...
                if ((intptr_t)panel.element_values[PANEL_DISPLAY_MENU]
                        .parsed_value) {
                        panel.menu = PanelControlCreate(panel.form, "menu",
                                                        menu_normal_image);
                        ...
                        ac++;
                        XtSetArg(al[ac], XmNtopOffset, 1);
                        ac++;
                        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_FORM);
                        ac++;
                        XtSetArg(al[ac], XmNleftOffset, 1);
                        ac++;
                        if (panel.resolution == HIGH ||
                            panel.resolution == MEDIUM) {
                                XtSetArg(al[ac], XmNwidth, 21);
                                ac++;
                                XtSetArg(al[ac], XmNheight, 15);
                                ac++;
                        } else {
                                XtSetArg(al[ac], XmNwidth, 17);
                                ac++;
                                XtSetArg(al[ac], XmNheight, 13);
                                ac++;
                        }
                        ...
                        ac++;
                        XtSetArg(al[ac], XmNtopWidget, panel.menu);
                        ac++;
                        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_FORM);
                        ac++;
                        XtSetArg(al[ac], XmNleftOffset, 1);
                        ...
                        ac++;
                        XtSetArg(al[ac], XmNbottomOffset, 1);
                        ac++;
                        if (panel.resolution == HIGH ||
                            panel.resolution == MEDIUM) {
                                XtSetArg(al[ac], XmNwidth, 20);
                                ac++;
                        } else {
                                XtSetArg(al[ac], XmNwidth, 16);
                                ac++;
                        }
                ...
                if ((intptr_t)panel.element_values[PANEL_DISPLAY_MINIMIZE]
                        .parsed_value) {
                        panel.iconify = PanelControlCreate(
                            panel.form, "minimize", minimize_normal_image);
                        ...
                        ac++;
                        XtSetArg(al[ac], XmNrightOffset, 1);
                        ac++;
                        if (panel.resolution == HIGH ||
                            panel.resolution == MEDIUM) {
                                XtSetArg(al[ac], XmNwidth, 21);
                        ...
                        XtSetArg(al[ac], XmNtopAttachment, XmATTACH_WIDGET);
                        ac++;
                        XtSetArg(al[ac], XmNtopWidget, panel.iconify);
                        ac++;
                        XtSetArg(al[ac], XmNrightAttachment, XmATTACH_FORM);
                        ac++;
                        XtSetArg(al[ac], XmNrightOffset, 1);
...
static void Initialize(DtPanelShellWidget panel_shell)

{
        ...
        busy_pixmap = XCreateBitmapFromData(display, RootWindowOfScreen(screen),
                                            (char *)time16_bits, time16_width,
                                            time16_height);
        busy_pixmap_mask = XCreateBitmapFromData(
            display, RootWindowOfScreen(screen), (char *)time16m_bits,
            time16_width, time16_height);

        panel.busy_cursor = XCreatePixmapCursor(
            display, busy_pixmap, busy_pixmap_mask, &(xcolors[0]),
            &(xcolors[1]), time16_x_hot, time16_y_hot);

        XFreePixmap(display, busy_pixmap);
        XFreePixmap(display, busy_pixmap_mask);

        panel.resolution =
            (int)(intptr_t)panel.element_values[PANEL_RESOLUTION].parsed_value;
        ...
        if (panel.resolution == MATCH_DISPLAY ||
            (panel.resolution != HIGH && panel.resolution != MEDIUM &&
             panel.resolution != LOW)) {
                resolution = _DtGetDisplayResolution(
        ...
        XmeGetIconControlInfo(screen, &use_mask_return, &panel.use_color_icons,
                              &use_icon_file_cache_return);

        panel.pixel_set =
            (XmPixelSet *)XtMalloc(sizeof(XmPixelSet) * XmCO_NUM_COLORS);

        if (XmeGetPixelData(XScreenNumberOfScreen(screen), &color_use,
                            panel.pixel_set, &active, &inactive, &primary,
                            &secondary)) {
                ...
                if (panel.active_pixel_set->bg == black_pixel) {
                        ...
                        if (panel.active_pixel_set->bg == white_pixel) {
                                ...
                                /*  Get the Motif defaults and assign into a
                                 * single allocated  */
                                /*  pixel set which is then referenced throught
                                 * the other 3    */
                                /*  pixel set datas. */
        ...
        post_monitor_arrow_image =
            GetIconName(UP_MONITOR_ARROW_IMAGE_NAME, size);
        unpost_monitor_arrow_image =
            GetIconName(DOWN_MONITOR_ARROW_IMAGE_NAME, size);
        ...
        minimize_selected_image =
            GetIconName(MINIMIZE_SELECTED_IMAGE_NAME, size);
...
static Widget PanelControlCreate(Widget parent, char *control_name,
                                 String image_name)

{
        ...
        if (strcmp(control_name, "minimize") == 0) {
                ...
                minimize_selected_pixmap = XmGetPixmap(
                    XtScreen(parent), minimize_selected_image, fg, bg);
        ...
        ac++;
        XtSetArg(al[ac], XmNwidth, 11);
        ac++;
        XtSetArg(al[ac], XmNshadowThickness, 0);
        ac++;
        XtSetArg(al[ac], XmNborderWidth, 0);
        ac++;
        XtSetArg(al[ac], XmNbackgroundPixmap, pixmap);
        ac++;
...

{
        ...
        for (i = 0; i < panel.box_data_count; i++) {
                ...
                /*  Create the outer box form which will contain the inner box
                 * set  */
                ...
                if (panel.left_handle != NULL) {
                        ...
                        ac++;
                        XtSetArg(al[ac], XmNrightWidget, panel.right_handle);
                        ac++;
                        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_WIDGET);
                        ac++;
                        XtSetArg(al[ac], XmNleftWidget, panel.left_handle);
                        ac++;
                } else {
                        XtSetArg(al[ac], XmNrightAttachment, XmATTACH_FORM);
                        ac++;
                ...
                if (prev_form != NULL) {
                        XtSetArg(al[ac], XmNtopAttachment, XmATTACH_WIDGET);
                        ac++;
                        XtSetArg(al[ac], XmNtopWidget, prev_form);
                        ac++;
                } else {
                        XtSetArg(al[ac], XmNtopAttachment, XmATTACH_FORM);
                        ac++;
                ...
                if (panel.color_use == XmCO_BLACK_WHITE) {
                        XtSetArg(al[ac], XmNbottomShadowColor,
                                 BlackPixelOfScreen(XtScreen(panel.shell)));
                        ac++;
                }
                ...
                XtSetArg(al[ac], XmNbackground, panel.primary_pixel_set->bg);
                ac++;
                XtSetArg(al[ac], XmNforeground, panel.primary_pixel_set->fg);
                ac++;

                /*  Create the outer form widget that will contain the entire
                 * panel  */

                box_data->form = XmCreateForm(
                    panel.form,
                    (char *)box_data->element_values[BOX_NAME].parsed_value, al,
                    ac);
        ...
        for (i = 0; i < panel.box_data_count; i++) {
                ...
                if (box_data->switch_data != NULL) {
                        ...
                        switch_position =
                            (intptr_t)box_data->switch_data
                                ->element_values[SWITCH_POSITION_HINTS]
                                .parsed_value;

                        /*  Determine if the defined switch position is actually
                         * the  */
                        /*  first or last position. */

                        if (switch_position != POSITION_FIRST ||
                            switch_position != POSITION_LAST) {
                                for (j = 0; j < box_data->control_data_count;
                                     j++) {
                                        if (switch_position <
                                            (intptr_t)(
                                                (box_data->control_data[j])
                                                    ->element_values
                                                        [CONTROL_POSITION_HINTS]
                                                    .parsed_value))
                                                break;
                                }
                        ...
                        XtSetArg(al[ac], XmNuserData, SWITCH);
                        ac++;
                        ...
                        if (panel.color_use == XmCO_BLACK_WHITE) {
                                XtSetArg(
                                    al[ac], XmNbottomShadowColor,
                                    BlackPixelOfScreen(XtScreen(panel.form)));
                                ac++;

                                pixmap = XmGetPixmap(
                                    XtScreen(panel.form), "50_foreground",
                        ...
                        XtSetArg(al[ac], XmNbackground,
                                 panel.primary_pixel_set->bg);
                        ac++;
                        XtSetArg(al[ac], XmNforeground,
                                 panel.primary_pixel_set->fg);
                        ac++;
                ...
                if (box_data->switch_form != NULL) {
                        if (switch_position == POSITION_FIRST) {
                                ...
                                XtSetArg(al[ac], XmNleftWidget,
                                         box_data->switch_form);
                                ac++;
                                XtSetArg(al[ac], XmNleftOffset, -4);
                                ac++;
                                ...
                                XtSetArg(al[ac], XmNtopShadowColor,
                                         panel.inactive_pixel_set->ts);
                                ac++;
                                XtSetArg(al[ac], XmNselectColor,
                                         panel.inactive_pixel_set->sc);
                                ac++;
                                ...
                                if (panel.color_use == XmCO_BLACK_WHITE) {
                                        ...
                                        XtSetArg(al[ac], XmNbackgroundPixmap,
                                                 pixmap);
                                        ac++;
                                } else {
                                        XtSetArg(al[ac], XmNbottomShadowColor,
                                                 panel.inactive_pixel_set->bs);
                                        ac++;
                                }
                                ...
                                box_data->right_arrow_form = XmCreateForm(
                                    box_data->form, "right_arrow_form", al, ac);
                                ...
                                XtAddCallback(
                                    box_data->right_arrow_form, XmNhelpCallback,
                                    (XtCallbackProc)GeneralTopicHelpCB,
                                ...
                                XtSetArg(al[ac], XmNrightAttachment,
                                         XmATTACH_FORM);
                                ac++;
                                XtSetArg(al[ac], XmNrightOffset, -2);
                                ac++;
                        }
                } else {
                        ac = 0;
                ...
                if (panel.color_use == XmCO_BLACK_WHITE) {
                        ...
                        pixmap =
                            XmGetPixmap(XtScreen(panel.form), "50_foreground",
                                        panel.inactive_pixel_set->fg,
                                        panel.inactive_pixel_set->bg);
                ...
                if (box_data->switch_form != NULL) {
                        if (switch_position == POSITION_FIRST ||
                            switch_position == POSITION_LAST) {
                                ...
                                XtSetArg(al[ac], XmNuserData, CONTROL);
                                ac++;
                                ...
                                box_data->left_control_form =
                                    XmCreateForm(box_data->form,
                                                 "left_control_form", al, ac);
                        ...
                        box_data->left_control_form = XmCreateForm(
                            box_data->form, "left_control_form", al, ac);
                        XtManageChild(box_data->left_control_form);
                }

                if (box_data->left_control_form != NULL) {
                        if (panel.popup_data == NULL ||
                            panel.popup_data->popup == NULL)
                                CreatePopupMenu(box_data->left_control_form);
                        else
                                XmAddToPostFromList(
                                    panel.popup_data->popup,
                                    box_data->left_control_form);

                        /* Event handler for posting popup menu */

                        XtAddEventHandler(
                            box_data->left_control_form, ButtonPressMask, False,
                ...
                if (box_data->right_control_form != NULL) {
                        ...
                        XtAddEventHandler(box_data->right_control_form,
                ...
                if (box_data->switch_data != NULL) {
                        if (box_data->switch_form != NULL) {
                                if (box_data->switch_data->popup_data == NULL ||
                                    box_data->switch_data->popup_data->popup ==
                                        NULL)
                                        ...
                                        XmAddToPostFromList(
                                            box_data->switch_data->popup_data
                                                ->popup,
                                            box_data->switch_form);
                                ...
                                XtAddEventHandler(
                                    box_data->switch_form, ButtonPressMask,
                                    False,
...
static void MainControlCreate(int switch_position)
                ...
                if (switch_position == POSITION_FIRST ||
                    ...
                    box_data->switch_form == NULL) {
                        ControlCreate(box_data->left_control_form,
                                      box_data->control_data,
                                      box_data->control_data_count);

                        ArrowCreate(
                            box_data->left_arrow_form, box_data->control_data,
                            box_data->control_data_count, False, first_box);
                } else {
                        int j;

                        /*  Find the position within the control list that is
                         * less than  */
                        /*  the switch position. */

                        for (j = 0; j < box_data->control_data_count; j++) {
                                if (switch_position <
                                    (intptr_t)((box_data->control_data[j])
                                                   ->element_values
                                                       [CONTROL_POSITION_HINTS]
                                                   .parsed_value)) {
                                        ControlCreate(
...
static void ControlSetVisualData(ControlData *control_data, ArgList al, int *ac)

{
        ...
        if (panel.color_use == XmCO_BLACK_WHITE) {
                XtSetArg(al[*ac], XmNuseEmbossedText, False);
                (*ac)++;
                XtSetArg(al[*ac], XmNarmColor, panel.primary_pixel_set->bg);
                (*ac)++;
        } else {
                XtSetArg(al[*ac], XmNarmColor, panel.primary_pixel_set->sc);
                (*ac)++;
        }
        ...
        if (panel.use_color_icons) {
                ...
                XtSetArg(al[*ac], XmNpixmapForeground,
                         panel.primary_pixel_set->ts);
                (*ac)++;
        } else {
                if (control_data != NULL &&
                    (intptr_t)control_data->element_values[CONTROL_TYPE]
                            .parsed_value == CONTROL_CLOCK) {
                        if (panel.color_use != XmCO_BLACK_WHITE) {
                                XtSetArg(al[*ac], XmNpixmapForeground,
                                         panel.primary_pixel_set->fg);
                                (*ac)++;
                                ...
                                XtSetArg(
                                    al[*ac], XmNpixmapForeground,
                                    BlackPixelOfScreen(XtScreen(panel.form)));
                                (*ac)++;
                                XtSetArg(
                                    al[*ac], XmNpixmapBackground,
                                    WhitePixelOfScreen(XtScreen(panel.form)));
                                (*ac)++;
                        }
                } else {
                        XtSetArg(al[*ac], XmNpixmapBackground,
        ...
        XtSetArg(al[*ac], XmNtopShadowColor, panel.primary_pixel_set->ts);
        (*ac)++;
        XtSetArg(al[*ac], XmNbottomShadowColor, panel.primary_pixel_set->bs);
        (*ac)++;
        XtSetArg(al[*ac], XmNselectColor, panel.primary_pixel_set->sc);
        (*ac)++;

        if ((intptr_t)control_data->element_values[CONTROL_TYPE].parsed_value ==
            CONTROL_DATE) {
                /*
                      XtSetArg (al[*ac], XmNfontList, panel.date_font_list);
                   (*ac)++; XtSetArg (al[*ac], XmNuseEmbossedText, False);
                   (*ac)++; XtSetArg (al[*ac], XmNforeground, BlackPixelOfScreen
                   (XtScreen (panel.form)));
                   (*ac)++;
                */
                if (panel.color_use == XmCO_BLACK_WHITE) {
                ...
                XtSetArg(al[*ac], XmNfontList, panel.font_list);
                (*ac)++;
        }

        XtSetArg(al[*ac], XmNbehavior, XmICON_DRAG);
        (*ac)++;
}
...
static void ControlSetIconData(Widget parent, ControlData *control_data,
                               XmString *icon_label, int container_type,
                               ArgList al, int *ac, char **icon_name,
...

{
        ...
        *icon_name = (char *)control_data->element_values[CONTROL_NORMAL_ICON]
                         .parsed_value;

        if (*icon_name != NULL) {
                *icon_name = GetIconName(*icon_name, icon_size);
                XtSetArg(al[*ac], XmNimageName, *icon_name);
                (*ac)++;
        }

        *alternate_icon_name =
            (char *)control_data->element_values[CONTROL_ALTERNATE_ICON]
                .parsed_value;

        if (*alternate_icon_name != NULL) {
                *alternate_icon_name =
                    GetIconName(*alternate_icon_name, icon_size);
                XtSetArg(al[*ac], XmNalternateImage, *alternate_icon_name);
                (*ac)++;
        }
        ...
        if (container_type != SUBPANEL &&
            (intptr_t)panel.element_values[PANEL_DISPLAY_CONTROL_LABELS]
                    .parsed_value == False) {
                XtSetArg(al[*ac], XmNstring, NULL);
                (*ac)++;
        } else {
                control_label =
                ...
                if ((intptr_t)control_data->element_values[CONTROL_TYPE]
                        .parsed_value != CONTROL_BLANK) {
                        if (control_label != NULL) {
                                *icon_label =
                                    XmStringCreateLocalized(control_label);
                                XtSetArg(al[*ac], XmNstring, *icon_label);
                                (*ac)++;
...
static void ControlSetBehavior(ControlData *control_data, ArgList al, int *ac,
                               Boolean in_subpanel, String *expanded_file_name)

{
        ...
        if (control_data->element_values[CONTROL_PUSH_ACTION].parsed_value !=
            NULL) {
                sensitive = True;
        ...
        if (panel.resolution == HIGH &&
            (intptr_t)control_data->element_values[CONTROL_CONTAINER_TYPE]
                    .parsed_value == BOX &&
            in_subpanel == False)
                margin_width = 5;
        else
        ...
        XtSetArg(al[*ac], XmNuserData, control_data);
        (*ac)++;
        ...
        case CONTROL_BLANK: {
                ...
                (*ac)++;
                XtSetArg(al[*ac], XmNcontrolType, XmCONTROL_BLANK);
                (*ac)++;
                XtSetArg(al[*ac], XmNbehavior, XmICON_LABEL);
                (*ac)++;
                XtSetArg(al[*ac], XmNsensitive, False);
                (*ac)++;
        } break;
        ...
        case CONTROL_BUSY: {
                XtSetArg(al[*ac], XmNshadowThickness, 0);
                (*ac)++;
                XtSetArg(al[*ac], XmNcontrolType, XmCONTROL_BUSY);
                (*ac)++;
                XtSetArg(al[*ac], XmNhighlightThickness, 0);
                (*ac)++;
                XtSetArg(al[*ac], XmNsensitive, False);
        ...
        case CONTROL_FILE: {
                ...
                (*ac)++;
                XtSetArg(al[*ac], XmNsensitive, sensitive);
                (*ac)++;
                ...
                if ((intptr_t)control_data->element_values[CONTROL_MONITOR_TYPE]
                        .parsed_value == MONITOR_NONE) {
                        XtSetArg(al[*ac], XmNcontrolType, XmCONTROL_BUTTON);
                        (*ac)++;
                } else if ((intptr_t)control_data
                               ->element_values[CONTROL_MONITOR_TYPE]
                               .parsed_value == MONITOR_FILE) {
                        String file_name;

                        file_name = (String)control_data
                                        ->element_values[CONTROL_FILE_NAME]
                                        .parsed_value;
                        *expanded_file_name = (String)_DtWmParseFilenameExpand(
                            (unsigned char *)file_name);

                        XtSetArg(al[*ac], XmNfileName, *expanded_file_name);
                        (*ac)++;
                        XtSetArg(al[*ac], XmNcontrolType, XmCONTROL_MONITOR);
                        (*ac)++;
                } else if ((intptr_t)control_data
                               ->element_values[CONTROL_MONITOR_TYPE]
                               .parsed_value == MONITOR_MAIL) {
                        String file_name;
                        ...
                        if ((file_name != (String)NULL) &&
                            (*file_name != '\0')) {
                                *expanded_file_name =
                                    (String)_DtWmParseFilenameExpand(
                                        (unsigned char *)file_name);
                                XtSetArg(al[*ac], XmNfileName,
                                         *expanded_file_name);
                                (*ac)++;
                        }

                        XtSetArg(al[*ac], XmNcontrolType, XmCONTROL_MAIL);
                        (*ac)++;
                }
        } break;
        ...
        case CONTROL_CLIENT: {
                ...
                XtSetArg(al[*ac], XmNcontrolType, XmCONTROL_CLIENT);
                (*ac)++;
                XtSetArg(al[*ac], XmNshadowThickness, shadow_thickness);
                (*ac)++;
                XtSetArg(al[*ac], XmNsensitive, sensitive);
                ...
                geometry_data = (GeometryData *)control_data
                                    ->element_values[CONTROL_CLIENT_GEOMETRY]
                                    .parsed_value;

                if (geometry_data) {
                        if (geometry_data->flags & WidthValue &&
                            geometry_data->width > 0) {
                                XtSetArg(al[*ac], XmNwidth,
                                         geometry_data->width + 6);
                                (*ac)++;
                        }

                        if (geometry_data->flags & HeightValue &&
                            geometry_data->height > 0) {
                                XtSetArg(al[*ac], XmNheight,
                                         geometry_data->height + 6);
                                (*ac)++;
                        }
                }
        } break;
        ...
        case CONTROL_CLOCK: {
                ...
                XtSetArg(al[*ac], XmNleftInset, inset);
                (*ac)++;
                XtSetArg(al[*ac], XmNrightInset, inset);
                (*ac)++;
                XtSetArg(al[*ac], XmNtopInset, inset);
                (*ac)++;
                XtSetArg(al[*ac], XmNbottomInset, inset);
                (*ac)++;
        } break;
        ...
        case CONTROL_DATE: {
                ...
                XtSetArg(al[*ac], XmNcontrolType, XmCONTROL_DATE);
                (*ac)++;
                XtSetArg(al[*ac], XmNshadowThickness, shadow_thickness);
                (*ac)++;
                XtSetArg(al[*ac], XmNsensitive, sensitive);
                ...
                XtSetArg(al[*ac], XmNmarginWidth, margin_width);
                (*ac)++;
                XtSetArg(al[*ac], XmNuseLabelAdjustment, False);
                (*ac)++;

                if (!control_data->element_values[CONTROL_DATE_FORMAT]
                         .use_default) {
                        XtSetArg(
                            al[*ac], XmNformat,
                            control_data->element_values[CONTROL_DATE_FORMAT]
                                .parsed_value);
                        (*ac)++;
                }

                if (!in_subpanel) {
                        XtSetArg(al[*ac], XmNpixmapPosition, XmPIXMAP_MIDDLE);
                        (*ac)++;
                        XtSetArg(al[*ac], XmNfontList, panel.date_font_list);
                        (*ac)++;
                        XtSetArg(al[*ac], XmNuseEmbossedText, False);
                        ...
                        XtSetArg(al[*ac], XmNuseEmbossedText, True);
                        (*ac)++;
                }
        } break;
        }
...
static Widget ControlCreateAndRegister(Widget parent, ControlData *control_data,
                                       Boolean main_copy, ArgList al, int ac)

{
        ...
        XtAddCallback(icon, XmNhelpCallback, (XtCallbackProc)ControlTopicHelpCB,
                      control_data);
         ...
         */
        /*  a file type the has either move, copy or link actions associated
         * with */
        ...
        if (control_data->element_values[CONTROL_DROP_ACTION].parsed_value !=
            NULL) {
                ...
                if (operations != 0) {
                        ...
                        XtSetArg(al2[2], XmNanimationStyle,
                                 XmDRAG_UNDER_SHADOW_IN);
                        XtSetArg(al2[3], DtNpreserveRegistration, True);

                        /* Temporarily hard-coded to reject buffer drops on the
                           trash until the desktop clients have trash actions
                           which can take appropriate action on calendar
                           appointments, mail attachments, selected text, etc.
                           File Manager is also hard-coded to reject buffer
                           drops on the trash window. Any changes here should be
                           coordinated with File Manager.
                         */
                        if (strcmp(control_data->element_values[CONTROL_NAME]
                                       .parsed_value,
                                   "Trash") == 0)
                                DtDndDropRegister(icon, DtDND_FILENAME_TRANSFER,
                                                  operations, transferDropCB,
                                                  al2, 4);
                        else
        ...
        if (!main_copy) {
                if ((intptr_t)control_data->element_values[CONTROL_PUSH_RECALL]
                        .parsed_value)
                        PushRecallRegister(control_data, True);

                if ((intptr_t)control_data->element_values[CONTROL_TYPE]
                        .parsed_value == CONTROL_CLIENT)
...
static void SwitchCreate(BoxData *box_data)

{
        ...
        panel.switch_row_count = (int)(intptr_t)(
        ...
        XtSetArg(al[ac], XmNnumColumns, panel.switch_row_count);
        ac++;
        ...
        for (i = 0; i < switch_data->control_data_count; i++) {
                ...
                XtSetArg(al[ac], XmNpixmapPosition, XmPIXMAP_TOP);
                ac++;
                ...
                if (i % 2) {
                        if (prev_right == NULL) {
                                ...
                                XtSetArg(al[ac], XmNtopAttachment,
                                         XmATTACH_WIDGET);
                                ac++;
                                XtSetArg(al[ac], XmNtopWidget, prev_right);
                                ac++;
                        }

                        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_WIDGET);
                        ac++;
                        XtSetArg(al[ac], XmNleftWidget, switch_data->rc);
                        ac++;
                        XtSetArg(al[ac], XmNrightAttachment, XmATTACH_FORM);
                        ac++;
                        XtSetArg(al[ac], XmNrightOffset, 1);
                        ...
                        if (prev_left == NULL) {
                                ...
                                XtSetArg(al[ac], XmNtopAttachment,
                                         XmATTACH_WIDGET);
                                ac++;
                                XtSetArg(al[ac], XmNtopWidget, prev_left);
                                ac++;
                        }
                ...
                /*  Call the function used to set the control's icon and label
                 * data  */
                ...
                /*  Call the function used to set the control's behavioral
                 * aspects  */

                ControlSetBehavior(control_data, al, &ac, False,
                                   &exp_file_name);

                /*  Call the function used to create and register the control */

                control_data->icon = ControlCreateAndRegister(
                    box_data->switch_form, control_data, False, al, ac);
...
void SwitchButtonCreate(SwitchData *switch_data, Boolean one)

{
        ...
        if (!switch_height) {
                ...
                label_string =
                    XmStringCreateLocalized(switch_data->switch_names[0]);
                XmStringExtent(panel.font_list, label_string, &ignore_width,
                               &switch_height);
        ...
        XtSetArg(al[ac], XmNfillOnArm, False);
        ac++;

        if (panel.resolution == HIGH || panel.resolution == MEDIUM) {
                XtSetArg(al[ac], XmNwidth, SWITCH_HIGH_BUTTON_WIDTH);
                ac++;
                ...
                XtSetArg(al[ac], XmNwidth, SWITCH_LOW_BUTTON_WIDTH);
                ac++;
                XtSetArg(al[ac], XmNshadowThickness, 2);
                ac++;
                XtSetArg(al[ac], XmNmarginWidth, 2);
                ac++;
        }

        XtSetArg(al[ac], XmNheight, switch_height);
        ac++;
        XtSetArg(al[ac], XmNbehavior, XmICON_TOGGLE);
        ac++;
        XtSetArg(al[ac], XmNfillMode, XmFILL_SELF);
        ac++;
        XtSetArg(al[ac], XmNalignment, XmALIGNMENT_BEGINNING);
        ac++;
        XtSetArg(al[ac], XmNpixmapPosition, XmPIXMAP_BOTTOM);
        ac++;
        ...
        ac++;
        XtSetArg(al[ac], XmNcontrolType, XmCONTROL_SWITCH);
        ac++;
        XtSetArg(al[ac], XmNfontList, panel.font_list);
        ac++;
        XtSetArg(al[ac], XmNrecomputeSize, False);
        ac++;
        ...
        for (i = 0; i < switch_count; i++) {
                ...
                XtSetArg(al[ac], XmNforeground, pixel_set->fg);
                ac++;
                XtSetArg(al[ac], XmNbackground, pixel_set->bg);
                ac++;
                XtSetArg(al[ac], XmNarmColor, pixel_set->bg);
                ac++;
                XtSetArg(al[ac], XmNfillOnArm, False);
                ac++;
                ...
                if (panel.color_use == XmCO_BLACK_WHITE) {
                        ...
                        XtSetArg(al[ac], XmNpixmapBackground, pixel_set->ts);
                        ac++;
                        XtSetArg(al[ac], XmNpixmapForeground, pixel_set->bs);
                        ac++;
                }
                ...
                label_string =
                    XmStringCreateLocalized(switch_data->switch_names[i]);
                XtSetArg(al[ac], XmNstring, label_string);
                ac++;
...
static void ControlCreate(Widget parent, ControlData **control_data,
                          int control_count)

{
        ...
        for (i = 0; i < control_count; i++) {
                ...
                if (prev_icon == NULL) {
                        ...
                        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_WIDGET);
                        ac++;
                        XtSetArg(al[ac], XmNleftWidget, prev_icon);
                        ac++;
                }
                ...
                ControlSetIconData(parent, control_data[i], &icon_label, BOX,
                                   al, &ac, &icon_name, &alt_icon_name);

                /*  Call the function used to set the control's behavioral
                 * aspects  */

                ControlSetBehavior(control_data[i], al, &ac, False,
                                   &exp_file_name);

                /*  Call the function used to create and register the control */

                control_data[i]->icon = ControlCreateAndRegister(
                    parent, control_data[i], False, al, ac);
                ...
                if (control_data[i]->subpanel_data != NULL)
                        SubpanelCreate(control_data[i],
                                       control_data[i]->subpanel_data);
        }
}
...
static void ArrowCreate(Widget parent, ControlData **control_data,
                        int arrow_count, Boolean right_side, Boolean first_box)

{
        ...
        ac2 = 0;
        XtSetArg(al2[ac2], XmNtopAttachment, XmATTACH_FORM);
        ac2++;
        XtSetArg(al2[ac2], XmNtopOffset, 0);
        ac2++;
        XtSetArg(al2[ac2], XmNbottomAttachment, XmATTACH_FORM);
        ac2++;
        XtSetArg(al2[ac2], XmNbottomOffset, 0);
        ac2++;
        XtSetArg(al2[ac2], XmNbackground, panel.inactive_pixel_set->bg);
        ac2++;
        XtSetArg(al2[ac2], XmNforeground, panel.inactive_pixel_set->fg);
        ac2++;
        XtSetArg(al2[ac2], XmNtopShadowColor, panel.inactive_pixel_set->ts);
        ac2++;
        XtSetArg(al2[ac2], XmNbottomShadowColor, panel.inactive_pixel_set->bs);
        ac2++;
        XtSetArg(al2[ac2], XmNshadowThickness, 2);
        ac2++;
        XtSetArg(al2[ac2], XmNorientation, XmVERTICAL);
        ac2++;
        ac2_save = ac2;
        ...
        for (i = 0; i < arrow_count; i++) {
                ...
                if (prev_separator == NULL) {
                        ...
                        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_WIDGET);
                        ac++;
                        XtSetArg(al[ac], XmNleftWidget, prev_separator);
                        ac++;
                }
                ...
                XtSetArg(al[ac], XmNwidth, control_width);
                ac++;

                if (control_data[i]->subpanel_data == NULL) {
                        XtSetArg(al[ac], XmNimageName, blank_arrow_image);
                        ac++;
                } else {
                        if ((intptr_t)(
                                control_data[i]
                                    ->element_values[CONTROL_MONITOR_TYPE]
                                    .parsed_value) != MONITOR_NONE) {
                                if (CheckOtherMonitorsOn(
                                        control_data[i]->subpanel_data)) {
                                        XtSetArg(al[ac], XmNimageName,
                                                 post_monitor_arrow_image);
                                        ac++;
                                } else {
                                        XtSetArg(al[ac], XmNimageName,
                ...
                XtSetArg(al[ac], XmNmultiClick, XmMULTICLICK_DISCARD);
                ac++;
                ...
                XtAddCallback(control_data[i]->arrow, XmNcallback, ArrowCB,
                              (XtPointer)control_data[i]);

                if (i < arrow_count - 1) {
                        XtSetArg(al2[ac2], XmNleftAttachment, XmATTACH_WIDGET);
                        ac2++;
                        XtSetArg(al2[ac2], XmNleftWidget,
                                 control_data[i]->arrow);
                        ac2++;

                        if (i == 0 && right_side == False) {
                                XtSetArg(al2[ac2], XmNleftOffset, 1);
                                ac2++;
                        } else if (i == 0 && right_side == True) {
                                XtSetArg(al2[ac2], XmNleftOffset, 3);
                                ac2++;
                        } else {
                                XtSetArg(al2[ac2], XmNleftOffset, 0);
                                ac2++;
                        }

                        control_data[i]->arrow_separator =
                            XmCreateSeparatorGadget(parent, "arrow_separator",
                                                    al2, ac2);
...
static void SubpanelCreate(ControlData *main_control_data,
                           SubpanelData *subpanel_data)

{
        DtPanelShellWidget subpanel_shell;
        char *subpanel_name =
            (char *)subpanel_data->element_values[SUBPANEL_TITLE].parsed_value;
        XmString icon_label;
        ...
        XtSetArg(al[ac], XmNallowShellResize, True);
        ac++;
        XtSetArg(al[ac], XmNmwmDecorations, MWM_DECOR_MENU | MWM_DECOR_TITLE);
        ac++;
        XtSetArg(al[ac], XmNtitle, subpanel_name);
        ac++;

        subpanel_data->shell = XtCreatePopupShell(
            subpanel_name, xmDialogShellWidgetClass, panel.shell, al, ac);
        ...
        XtSetArg(al[ac], XmNuserData, SUBPANEL);
        ac++;
        ...
        control_install = (Boolean)(intptr_t)subpanel_data
                              ->element_values[SUBPANEL_CONTROL_INSTALL]
                              .parsed_value;

        if (control_install) {
                icon_label =
                    XmStringCreateLocalized(GETMESSAGE(84, 1, "Install Icon"));
                ...
                XtSetArg(al[ac], XmNstring, icon_label);
                ac++;
                XtSetArg(al[ac], XmNpixmapPosition, XmPIXMAP_LEFT);
                ac++;
                XtSetArg(al[ac], XmNbehavior, XmICON_LABEL);
                ac++;
                XtSetArg(al[ac], XmNimageName, dropzone_image);
                ac++;
                XtSetArg(al[ac], XmNuserData, subpanel_data);
                ac++;
                XtSetArg(al[ac], XmNfontList, panel.font_list);
                ac++;
                XtSetArg(al[ac], XmNtraversalOn, False);
                ...
                DtDndDropRegister(subpanel_data->dropzone,
                                  DtDND_FILENAME_TRANSFER, XmDROP_COPY,
                                  customizeTransferDropCB, al2, 3);

                /*  Create the separator that goes between the dropzone and
                 * controls  */

                ac = ac_save;
                XtSetArg(al[ac], XmNtopAttachment, XmATTACH_WIDGET);
                ac++;
                XtSetArg(al[ac], XmNtopWidget, subpanel_data->dropzone);
                ac++;
                XtSetArg(al[ac], XmNtopOffset, 5);
                ac++;
                XtSetArg(al[ac], XmNleftAttachment, XmATTACH_FORM);
                ...
                subpanel_data->separator = XmCreateSeparatorGadget(
                    subpanel_data->form, "subpanel_separator", al, ac);
        ...
        for (i = -1; i < subpanel_data->control_data_count; i++) {
                ...
                SubpanelControlCreate(subpanel_data, main_control_data,
                                      control_data, attach_widget, main,
                                      monitor);
        ...
        if (subpanel_data->control_data)
                XtSetArg(al[0], XmNcancelButton,
                         subpanel_data->control_data[0]->icon);
        else
                XtSetArg(al[0], XmNcancelButton,
...
void SubpanelControlCreate(SubpanelData *subpanel_data,
                           ControlData *main_control_data,
                           ControlData *control_data, Widget attach_widget,
                           Boolean main, Boolean monitor)

{
        ...
        if (monitor) {
                XtSetArg(al[ac], XmNleftOffset, 20);
                ac++;
        } else {
                XtSetArg(al[ac], XmNleftOffset, 5);
                ac++;
        ...
        XtSetArg(al[ac], XmNtopAttachment, XmATTACH_WIDGET);
        ac++;
        XtSetArg(al[ac], XmNtopWidget, attach_widget);
        ac++;
        XtSetArg(al[ac], XmNtopOffset, 5);
        ac++;
        XtSetArg(al[ac], XmNrightAttachment, XmATTACH_FORM);
        ...
        ControlSetIconData(subpanel_data->form, control_data, &icon_label,
                           SUBPANEL, al, &ac, &icon_name, &alt_icon_name);
        ...
        control_icon = ControlCreateAndRegister(subpanel_data->form,
                                                control_data, main, al, ac);
        ...
        if ((intptr_t)(control_data->element_values[CONTROL_MONITOR_TYPE]
                ...
                ac++;
                XtSetArg(al[ac], XmNtopWidget, control_icon);
                ac++;
                XtSetArg(al[ac], XmNhighlightThickness, 0);
                ac++;
                XtSetArg(al[ac], XmNmarginWidth, 1);
                ...
                if (m_state == DtMONITOR_ON) {
                        XtSetArg(al[ac], XmNimageName, indicator_on_image);
                        ac++;
                } else /* DtMONITOR_OFF */
                {
                        XtSetArg(al[ac], XmNimageName, indicator_off_image);
                        ac++;
                }
...
void UpdateSwitchGeometry(BoxData *box_data)
...
void AddSubpanel(ControlData *control_data)
...
void DeleteControl(ControlData *control_data)

{
        ...
        if (control_data->icon) {
                if (control_data->element_values[CONTROL_DROP_ACTION]
                        .parsed_value != NULL)
...
void DeleteSubpanel(ControlData *control_data)

{
        ...
        if (subpanel_data->parent_control_data != control_data) {
                ...
                DeleteSubpanelControl(subpanel_data,
                                      subpanel_data->parent_control_data);
        ...
        if (subpanel_data->element_values[SUBPANEL_CONTROL_INSTALL]
                .parsed_value) {
...
static void SwapElementValue(ElementValue *to_value, ElementValue *from_value)
...
void DeleteSubpanelControl(SubpanelData *subpanel_data,
                           ControlData *control_data)

{
        ...
        if (main_control_data == control_data) {
                ...
                if (SessionFileNameLookup(
                        (char *)main_control_data->element_values[CONTROL_NAME]
                            .parsed_value,
                        (int)CONTROL,
                        (char *)main_control_data
                            ->element_values[CONTROL_CONTAINER_NAME]
                            .parsed_value,
                        (int)BOX) != NULL)
                ...
                for (subpanel_control_position = 0;
                     ...
                     subpanel_control_position++) {
                        if (subpanel_data
                                ->control_data[subpanel_control_position] ==
                            subpanel_control_data)
                                break;
                }
                ...
                subpanel_data->main_panel_icon_copy =
                    subpanel_control_data->icon;
                subpanel_data->default_control = main_control_data;

                /*  Move the necessary data from the MP control to the SP
                 * control  */
                /*  and write out the new control file. */

                SwapElementValue(
                    &subpanel_control_data->element_values[CONTROL_NAME],
                ...
                SwapElementValue(
                    &subpanel_control_data
                         ->element_values[CONTROL_CONTAINER_TYPE],
                ...
                main_control_data->element_values =
                    subpanel_control_data->element_values;
                ...
                XtRemoveCallback(main_control_data->icon, XmNhelpCallback,
                                 (XtCallbackProc)ControlTopicHelpCB,
                                 subpanel_control_data);
                ...
                XtRemoveCallback(subpanel_data->main_panel_icon_copy,
                                 XmNcallback, (XtCallbackProc)PushCB,
                                 subpanel_control_data);

                XtRemoveCallback(
                    subpanel_data->main_panel_icon_copy, XmNhelpCallback,
                 ...
                 */
                /*  subpanel's list to denote the removal of the subpanel
                 * control  */
                /*  and free up the remaining subpanel data. */

                for (i = subpanel_control_position;
                     i < subpanel_data->control_data_count - 1; i++)
                        subpanel_data->control_data[i] =
                ...
                for (i = 0; i < subpanel_data->control_data_count; i++) {
                        if (control_data == subpanel_data->control_data[i])
                                break;
                }
                        ...
                        if (i < subpanel_data->control_data_count - 1) {
                                ...
                                if (i > 0)
                                        ToggleDefaultControl(
                                            main_control_data, subpanel_data,
                                            subpanel_data->control_data[i - 1]);
                                else if (subpanel_data->main_panel_icon_copy !=
                                         NULL)
                                        ToggleDefaultControl(main_control_data,
                                                             subpanel_data,
                                                             main_control_data);
                        }
                }
                ...
                for (j = i + 1; j < subpanel_data->control_data_count; j++) {
                        subpanel_data->control_data[j - 1] =
                            subpanel_data->control_data[j];
                }
        ...
        if (last) {
                ...
                XtSetArg(al[0], XmNheight,
                         XtHeight(subpanel_data->form) -
        ...
        if (control_monitor == False) {
                ...
                for (j = 0; j < subpanel_data->control_data_count; j++)
                        XtSetValues(subpanel_data->control_data[j]->icon, al,
                                    1);
...

{
        ...
        if (main_control_data == subpanel_data->default_control) {
                ...
                if ((intptr_t)main_control_data
                        ->element_values[CONTROL_PUSH_RECALL]
                        .parsed_value)
                        PushRecallRegister(main_control_data, True);

                if ((intptr_t)main_control_data->element_values[CONTROL_TYPE]
                        .parsed_value == CONTROL_CLIENT)
                ...
                if ((intptr_t)main_control_data->element_values[CONTROL_TYPE]
                        ...
                        EmbeddedClientReparent(
                            (char *)main_control_data
                                ->element_values[CONTROL_CLIENT_NAME]
                                .parsed_value,
                            subpanel_data->main_panel_icon_copy);
                }
        } else {
                if ((intptr_t)subpanel_data->default_control
                        ->element_values[CONTROL_PUSH_RECALL]
                        .parsed_value)
                        PushRecallRegister(subpanel_data->default_control,
                                           True);

                if ((intptr_t)subpanel_data->default_control
                        ->element_values[CONTROL_TYPE]
                        .parsed_value == CONTROL_CLIENT)
                        EmbeddedClientRegister(subpanel_data->default_control,
                                               True);
                ...
                if ((intptr_t)subpanel_data->default_control
                        ...
                        .parsed_value == CONTROL_CLIENT) {
                        EmbeddedClientReparent(
                            (char *)subpanel_data->default_control
                                ->element_values[CONTROL_CLIENT_NAME]
                                .parsed_value,
        ...
        if (next_icon != NULL) {
                ...
                XtSetArg(al[ac], XmNleftAttachment, XmATTACH_WIDGET);
                ac++;
                XtSetArg(al[ac], XmNleftWidget, main_control_data->icon);
                ac++;
        ...
        if (new_width < width) {
                ...
                if (new_width < normal_size) {
                        ...
                        XtSetArg(al[1], XmNmarginWidth,
                                 (Dimension)(normal_size - new_width) / 2);
...
static void SetupPushAnimation(ControlData *control_data)

{
        char *animation_name =
            control_data->element_values[CONTROL_PUSH_ANIMATION].parsed_value;
        int count;
        int delay;
        String image_name;
        ...
        if (i < count) {
                for (j = 0; j < panel.animation_data[i].item_count; j++) {
                        image_name = GetIconName(
                            panel.animation_data[i].items[j].image_name,
                            panel.main_icon_size);

                        delay = panel.animation_data[i].items[j].delay;

                        _DtControlAddPushAnimationImage(control_data->icon,
                                                        image_name, delay);
                        XtFree(image_name);
                }
        }
...
static void SetupDropAnimation(ControlData *control_data)

{
        char *animation_name =
            control_data->element_values[CONTROL_DROP_ANIMATION].parsed_value;
        int count;
        int delay;
        String image_name;
        ...
        if (i < count) {
                for (j = 0; j < panel.animation_data[i].item_count; j++) {
                        ...
                        _DtControlAddDropAnimationImage(control_data->icon,
                                                        image_name, delay);
                        XtFree(image_name);
                }
        }
...

{
        ...
        if (return_name == NULL)
                return_name = _DtGetIconFileName(screen, DEFAULT_IMAGE_NAME,
                                                 NULL, NULL, icon_size);
