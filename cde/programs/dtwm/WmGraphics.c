...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmGraphics.c /main/4 1995/11/01 11:38:53 rswiston $"
#endif
#endif
/*
...
#define GE1(x) ((x) > 0 ? 1 : 0)
     ...
     *                      width, height, top_wid, right_wid, bot_wid,
     *left_wid)
     *
     *  Description:
     *  -----------
    ...
    void
    BevelRectangle(RList * prTop, RList *prBot, int x, int y,
        ...
        if (((prTop->used + (top_wid + left_wid)) > prTop->allocated) &&
            (!ExtendRList(prTop,
                          MAX(top_wid + left_wid, RLIST_EXTENSION_SIZE)))) {
        ...
        if (((prBot->used + (bot_wid + right_wid)) > prBot->allocated) &&
            (!ExtendRList(prBot,
                          MAX(bot_wid + right_wid, RLIST_EXTENSION_SIZE)))) {
                return;
        }
...
void BevelDepressedRectangle(RList *prTop, RList *prBot, int x, int y,
                             unsigned int width, unsigned int height,
                             unsigned int top_wid, unsigned int right_wid,
                             unsigned int bot_wid, unsigned int left_wid,
                             unsigned int in_wid) {
        ...
        y1 = y;
        len = width;
        for (count = top_wid - in_wid; count > 0;
             count--, prect++, (*piTop)++) {
                prect->x = x1;
                prect->y = y1;
                prect->width = len;
        ...
        y1 = y + GE1(join1);
        len = height - GE1(join1);
        for (count = left_wid - in_wid; count > 0;
             count--, prect++, (*piTop)++) {
                prect->x = x1;
                prect->y = y1;
                prect->width = 1;
        ...
        for (count = bot_wid - in_wid; count > 0;
             count--, prect++, (*piBot)++) {
                prect->x = x1;
                prect->y = y1;
                prect->width = len;
        ...
        for (count = right_wid - in_wid; count > 0;
             count--, prect++, (*piBot)++) {
                prect->x = x1;
                prect->y = y1;
                prect->width = 1;
...
void StretcherCorner(RList *prTop, RList *prBot, int x, int y, int cnum,
                     unsigned int swidth, unsigned int cwidth,
        ...
        case STRETCH_NORTH_WEST:
                ...
                prect->width = 1;
                prect->height =
                    swidth - 1 - ((wmGD.frameStyle == WmSLAB) ? 0 : 1);
                (*piBot)++;
        ...
        case STRETCH_NORTH_EAST:
                ...
                if (wmGD.frameStyle == WmRECESSED) {
                        prect->x =
                            x + cwidth - swidth; /* left inside (col 1) */
                        prect->y = y + swidth;
                        prect->width = 1;
                        prect->height = cheight - swidth;
                        (*piTop)++;
                ...
                /* bottom end */
                prect->x =
                    x + cwidth - swidth + ((wmGD.frameStyle == WmSLAB) ? 0 : 1);
                prect->y = y + cheight - 1;
        ...
        case STRETCH_SOUTH_EAST:
                ...
                prect->y = y;
                prect->width =
                    swidth - 2 + ((wmGD.frameStyle == WmSLAB) ? 1 : 0);
                ;
                prect->height = 1;
                prect++;
                (*piTop)++;
 ...
 *
 *************************************<->***********************************/
void DrawStringInBox(Display *dpy, Window win, GC gc, XFontStruct *pfs,
                     XRectangle *pbox, String str) {
        XGCValues gcv;
        int textWidth;
        int xCenter;
        XRectangle clipBox;
        ...
        if (textWidth < (int)pbox->width) { /* center text if there's room */
                ...
                gcv.clip_y_origin = 0;
                gcv.clip_mask = None;
                XChangeGC(dpy, gc, GCClipXOrigin | GCClipYOrigin | GCClipMask,
                          &gcv);
        }
        ...
        if ((pNewRect = (XRectangle *)XtMalloc(total * sizeof(XRectangle))) ==
            NULL) {
                Warning(((char *)GETMESSAGE(
                    28, 1, "Insufficient memory for graphics data")));
                rval = False;
        } else {
                prl->allocated = total;
                rval = True;
                ...
                if (prl->used != 0) { /* copy from old structure */
                        ...
                        (void)memcpy((void *)pNewRect, (void *)prl->prect,
                                     count);
                        if (prl->prect != NULL)
    ...
    unsigned int amt;
{
        ...
        if ((prl = (RList *)XtMalloc(sizeof(RList))) != NULL) {
                if ((prl->prect = (XRectangle *)XtMalloc(
                         amt * sizeof(XRectangle))) == NULL) {
                        XtFree((char *)prl);
                        prl = NULL;
                } else {
                        prl->allocated = amt;
 ...
 *
 *************************************<->***********************************/
void WmDrawString(Display *dpy, Drawable d, GC gc, int x, int y, char *string,
                  unsigned int length) {
 ...
 *
 *************************************<->***********************************/
void WmInstallBitmapIntoXmCache(unsigned char *pchName, Pixmap bitmap,
                                unsigned int width, unsigned int height) {
        XImage *pImage;
 ...
 *
 *************************************<->***********************************/
void WmInstallBitmapDataIntoXmCache(WmScreenData *pSD, unsigned char *pchName,
                                    char *pData, unsigned int width,
                                    unsigned int height) {
        XImage *pImage;
