 ...
 *
 ******************************<->***********************************/
void dtInitialize(char *program_name, XtAppContext appContext) {
...
void dtInitializeMessaging(Widget toplevel) {
        int i;
        WmScreenData *pSD;
        String sName;
        ...
        Tt_callback_action NoticeMsgCB(Tt_message m, Tt_pattern p);
        Tt_callback_action RequestMsgCB(Tt_message m, Tt_pattern p);
        ...
        if (status == TT_OK) {
                XtAppAddInput(XtWidgetToApplicationContext(wmGD.topLevelW), fd,
                              (XtPointer)XtInputReadMask, tttk_Xt_input_handler,
                              procId);
        } else {
        ...
        if (status != TT_OK) {
                errfmt = GETMESSAGE(
                    2, 4, "Could not get default ToolTalk session:\n%s\n");
                ToolTalkError(toplevel, errfmt, status);
                return;
        }
 ...
 *
 ******************************<->***********************************/
void dtCloseIPC(void) {} /* END OF FUNCTION dtCloseIPC */
 ...
 *
 ******************************<->***********************************/
void dtReadyNotification(void) {
        SendClientMsg(wmGD.dtSmWindow, (long)wmGD.xa_DT_SM_WM_PROTOCOL,
                      (long)wmGD.xa_DT_WM_READY, CurrentTime, NULL, 0);
 ...
 *				TITLE
 ******************************<->***********************************/
void dtSendWorkspaceModifyNotification(WmScreenData *pSD, Atom aWs, int iType) {
        char sNum[40];
        char pch[40];
        char pchType[40];
        Tt_message msg;
 ...
 *				CANCEL
 ******************************<->***********************************/
void dtSendMarqueeSelectionNotification(WmScreenData *pSD, int type, Position x,
                                        Position y, Dimension width,
                                        Dimension height) {
        Tt_message msg;
        Tt_status status;
        char sNum[40];
                ...
                if (GetSmClientIdClientList(&clients, &nClients)) {
                        ...
                        if (nClients > 0) {
                                ...
                                for (i = 0; i < nClients; i++)
                                        clientIdLen +=
                                            strlen(clients[i]->smClientID) + 1;

                                if (((clientIds = (char *)XtMalloc(
                                          clientIdLen * sizeof(char))) ==
                                     (char *)NULL) ||
                                    ((clientWorkspaces = (int *)XtMalloc(
                                          nClients * sizeof(int))) ==
                                     (int *)NULL)) {
                        ...
                        tt_message_arg_bval_set(m, 1,
                                                (unsigned char *)clientIds,
                                                clientIdLen * sizeof(char));
                        tt_message_arg_bval_set(
                            m, 2, (unsigned char *)clientWorkspaces,
                            nClients * sizeof(int));
...
static void ToolTalkError(Widget parent, char *errfmt, Tt_status status) {
        Arg args[10];
