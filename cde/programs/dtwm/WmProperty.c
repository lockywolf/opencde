...

{
        ...
        if ((!HasProperty(pCD, XA_WM_NORMAL_HINTS)) ||
            ((Success !=
              XGetWindowProperty(DISPLAY, pCD->client, XA_WM_NORMAL_HINTS, 0L,
                                 (long)PROP_SIZE_HINTS_ELEMENTS, False,
                                 XA_WM_SIZE_HINTS, &actualType, &actualFormat,
                                 &nitems, &leftover,
                                 (unsigned char **)&property)) ||
             (actualType != XA_WM_SIZE_HINTS) ||
             (nitems < (PROP_SIZE_HINTS_ELEMENTS - 3)) || (actualFormat != 32)))
#else  /* WSM */
        if ((Success != XGetWindowProperty(
                            DISPLAY, pCD->client, XA_WM_NORMAL_HINTS, 0L,
                            (long)PROP_SIZE_HINTS_ELEMENTS, False,
                            XA_WM_SIZE_HINTS, &actualType, &actualFormat,
...
void ProcessWmProtocols(ClientData *pCD) {
...
#endif /* WSM */
                rValue = XGetWindowProperty(
                    DISPLAY, pCD->client, wmGD.xa_WM_PROTOCOLS, 0L,
...
#endif /* ICCC_COMPLIANT */
        {
                ...
                if (!(pCD->clientProtocols =
                          (Atom *)XtMalloc(nitems * sizeof(Atom)))) {
                        ...
                        /*
                         * Save the protocols in the client data and look for
                        ...
                        for (i = 0; i < nitems; i++) {
                                ...
                                if (property[i] == wmGD.xa_WM_SAVE_YOURSELF) {
                                        pCD->protocolFlags |=
                                            PROTOCOL_WM_SAVE_YOURSELF;
                                } else if (property[i] ==
                                           wmGD.xa_WM_TAKE_FOCUS) {
                                        pCD->protocolFlags |=
                                            PROTOCOL_WM_TAKE_FOCUS;
                                } else if (property[i] ==
                                           wmGD.xa_WM_DELETE_WINDOW) {
                                        pCD->protocolFlags |=
                                            PROTOCOL_WM_DELETE_WINDOW;
                                } else if (property[i] ==
                                           wmGD.xa_MWM_MESSAGES) {
                                        pCD->protocolFlags |=
                                            PROTOCOL_MWM_MESSAGES;
                                }
                        }
                }
        }
...
void ProcessMwmMessages(ClientData *pCD) {
...
#endif /* WSM */
                rValue = XGetWindowProperty(
                    DISPLAY, pCD->client, wmGD.xa_MWM_MESSAGES, 0L,
        ...
        if ((rValue != Success) || (actualType == None) ||
            (actualFormat != 32) || (nitems == 0)) {
                ...
                if (!(pCD->mwmMessages =
                          (long *)XtMalloc(nitems * sizeof(long)))) {
                        /* unable to allocate space */
                        Warning(((char *)GETMESSAGE(
                            54, 2,
                            "Insufficient memory for window management data")));
                        ...
                        for (i = 0; i < nitems; i++) {
                                if ((pCD->mwmMessages[i] = property[i]) ==
                                    wmGD.xa_MWM_OFFSET) {
                                        pCD->protocolFlags |=
                                            PROTOCOL_MWM_OFFSET;
                                }
...
void SetMwmSaveSessionInfo(Window wmWindow) {
        ...
        XChangeProperty(DISPLAY, wmWindow, wmGD.xa_WM_PROTOCOLS, XA_ATOM, 32,
                        PropModeReplace, (unsigned char *)&property, 1);
...
PropWMState *GetWMState(Window window) {
        ...
        ret_val = XGetWindowProperty(
            DISPLAY, window, wmGD.xa_WM_STATE, 0L, PROP_WM_STATE_ELEMENTS,
            False, wmGD.xa_WM_STATE, &actual_type, &actual_format, &nitems,
            &leftover, (unsigned char **)&property);

        if (!((ret_val == Success) && (actual_type == wmGD.xa_WM_STATE) &&
              (nitems == PROP_WM_STATE_ELEMENTS))) {
                /*
                 * The property could not be retrieved or is not correctly set
                 * up.
                 */
...
void SetWMState(Window window, int state, Window icon) {
        ...
        XChangeProperty(DISPLAY, window, wmGD.xa_WM_STATE, wmGD.xa_WM_STATE, 32,
                        PropModeReplace, (unsigned char *)&property,
                        PROP_WM_STATE_ELEMENTS);
...

{
...
#endif /* WSM */
                ret_val = XGetWindowProperty(
                    DISPLAY, pCD->client, wmGD.xa_MWM_HINTS, 0L,
                    PROP_MWM_HINTS_ELEMENTS, False, wmGD.xa_MWM_HINTS,
         ...
         *         since...
         *
         * If running Motif 1.1.n client with Mwm 1.2, then ignore extra
         * elements since property.flags won't have extra elements set.
         *
         * If running Motif 1.2 client with Mwm 1.1.n, then ignore extra
         * elements since Mwm 1.1.n won't try to access the extra elements.
         */
...
PropMwmInfo *GetMwmInfo(Window rootWindowOfScreen) {
        ...
        ret_val = XGetWindowProperty(
            DISPLAY, rootWindowOfScreen, wmGD.xa_MWM_INFO, 0L,
            PROP_MWM_INFO_ELEMENTS, False, wmGD.xa_MWM_INFO, &actual_type,
...
void ProcessWmColormapWindows(ClientData *pCD) {
        ...
        rValue = XGetWindowProperty(
            DISPLAY, pCD->client, wmGD.xa_WM_COLORMAP_WINDOWS, 0L,
        ...
        if ((rValue == Success) && (actualType != None) &&
            (actualFormat == 32) && (nitems > 0)) {
                ...
                if (!(pWindows =
                          (Window *)XtMalloc((nitems * sizeof(Window)) + 1)) ||
                    !(pColormaps = (Colormap *)XtMalloc(
                          (nitems * sizeof(Colormap)) + 1))) {
                        /* unable to allocate space */
                        Warning(((char *)GETMESSAGE(
                            54, 3,
                            "Insufficient memory for window management data")));
                ...
                /* Is the above OSF code a bug -- allocates one extra byte,
                   rather */
                /* than one extra element, for the top window if needed? */
                else if (!(pCmapFlags =
                               (int *)XtCalloc(nitems + 1, sizeof(int)))) {
                        /* unable to allocate space */
                        Warning(((char *)GETMESSAGE(
                            54, 4,
                        ...
                        /*
                         * Check to see if the top-level client window is in the
                         * list. If it is not then add it to the head of the
                         * list.
                         */
                        ...
                        colormapCount = 0;
                        if (i == nitems) {
                                /* add the client window to the colormap window
                                 * list */
                        ...
                        for (i = 0; i < nitems; i++) {
                                if ((pColormaps[colormapCount] = FindColormap(
                                         pCD, property[i])) != None) {
                                        ...
                                        colormapCount++;
                                                        ...
                                                        (caddr_t *)&pcd)) {
                                        /*
                                         * The window is not a top level window
                                         * or a window that is already being
                                         * tracked for colormap changes. Track
                                         * colormap attribute changes.
                                         */

                                        XChangeWindowAttributes(
                                            DISPLAY, property[i], CWEventMask,
                                            &sAttributes);

                                        if (XGetWindowAttributes(
                                                DISPLAY, property[i],
                                                &wAttributes)) {
                                                pWindows[colormapCount] =
                                                    property[i];
                                                pColormaps[colormapCount] =
                                                    wAttributes.colormap;
                                                colormapCount++;
                                        }
                                }
                        }
                        ...
                        /*
                         * Free up the old colormap window data if it has been
                         * set.  Set new window contexts.
                         */
                        ...
                        if (colormapCount > 1) {
                                /*
                                 * The top level window and at least one other
                                 * window is in the colormap windows list.
                                 */
                                ...
                                /*
                                 * Only the top level window is being tracked
                                 * for colormap data.
                                 */
...
MenuItem *GetMwmMenuItems(ClientData *pCD) {
        ...
        /* parse the property string */
        {
                ...
                if (XmbTextPropertyToTextList(DISPLAY, &textProperty, &textList,
                                              &nItems) != Success) {
                        menuItems = NULL;
                } else {
                        menuItems = ParseMwmMenuStr(
                            PSD_FOR_CLIENT(pCD), (unsigned char *)textList[0]);
...
Status GetWorkspaceHints(Display *display, Window window, Atom **ppWsAtoms,
                         unsigned int *pCount, Boolean *pbAll) {
        int rcode;
        ...
        if (rcode == Success) {
                if (pWsHints->flags & DT_WORKSPACE_HINTS_WORKSPACES) {
                        paWs = (Atom *)XtMalloc(pWsHints->numWorkspaces *
                                                sizeof(Atom));
                        memcpy(paWs, pWsHints->pWorkspaces,
...
void SetEmbeddedClientsProperty(Window propWindow, Window *pEmbeddedClients,
                                unsigned long cEmbeddedClients) {
        XChangeProperty(DISPLAY, propWindow, wmGD.xa_DT_EMBEDDED_CLIENTS,
                        wmGD.xa_DT_EMBEDDED_CLIENTS, 32, PropModeReplace,
                        (unsigned char *)pEmbeddedClients, cEmbeddedClients);
...
void SetWorkspaceInfo(Window propWindow, WorkspaceInfo *pWsInfo,
                      unsigned long cInfo) {
        XChangeProperty(DISPLAY, propWindow, wmGD.xa_DT_WORKSPACE_INFO,
                        wmGD.xa_DT_WORKSPACE_INFO, 32, PropModeReplace,
                        (unsigned char *)pWsInfo,
...
void SetWorkspaceListProperty(WmScreenData *pSD) {
        ...
        pWsList = (Atom *)XtMalloc(pSD->numWorkspaces * sizeof(Atom));
        ...
        XChangeProperty(DISPLAY, pSD->wmWorkspaceWin, wmGD.xa_DT_WORKSPACE_LIST,
                        XA_ATOM, 32, PropModeReplace, (unsigned char *)pWsList,
...
void SetCurrentWorkspaceProperty(WmScreenData *pSD) {
        ...
        XChangeProperty(DISPLAY, pSD->wmWorkspaceWin,
                        wmGD.xa_DT_WORKSPACE_CURRENT, XA_ATOM, 32,
                        PropModeReplace, (unsigned char *)&aCurrent,
...
#define WIP_NUMBER_SIZE 16
        ...
        if ((status == Success) || (status > 0)) {
                 ...
                 */
                XSetTextProperty(DISPLAY, pWS->pSD->wmWorkspaceWin, &tp,
                                 aProperty);
                XFree(tp.value);
        }
...
void SetWorkspacePresence(Window propWindow, Atom *pWsPresence,
...

{
        ...
        rValue = XGetWindowProperty(
            DISPLAY, pSD->rootWindow, wmGD.xa_DT_SESSION_HINTS, 0L,
            (long)1000000, False, AnyPropertyType, &actualType, &actualFormat,
            &nitems, &leftover, (unsigned char **)&property);
...
void GetDtWmRequest(WmScreenData *pSD, char **pszReq, Boolean *pmore)

{
        ...
        if (property == NULL) {
                 ...
                 */
                rValue = XGetWindowProperty(
                    DISPLAY, pSD->wmWorkspaceWin, wmGD.xa_DT_WM_REQUEST, 0L,
                    (long)1000000, True, AnyPropertyType, &actualType,
...
Boolean HasProperty(ClientData *pCD, Atom aProperty) {
