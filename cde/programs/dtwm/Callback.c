        ...
        if (event->type == ButtonPress) {
                if (buttonEvent->button == wmGD.bMenuButton) {
                        ...
                        if (buttonEvent->button == Button1) {
                                CallWmFunction(F_Raise, NULL, panel.shell,
                                               event);
                                XSync(DISPLAY, FALSE);
...

{
        ...
        if (event->type == ButtonPress) {
                if (bevent->button == Button1) {
                        XtSetArg(al[0], XmNbackgroundPixmap,
...

{
        ...
        if (new_width != width ||
            x + (Position)new_width > (Position)screen_width) {
        ...
        if (new_height != height ||
            (Dimension)(new_height + y - 1) > screen_height) {
...

{
        ...
        case DtWSM_REASON_ADD: {
                ...
                if (XtIsManaged(box_data->switch_edit))
                        SwitchRenameCancel(box_data->switch_edit, NULL, NULL,
                                           NULL);

                DtWsmGetWorkspaceList(
                    XtDisplay(switch_data->rc),
                    RootWindowOfScreen(XtScreen(switch_data->rc)), &atom_names,
                    &switch_count);
                ...
                switch_data->switch_names = (char **)XtRealloc(
                    (char *)switch_data->switch_names,
                ...
                switch_data->buttons = (Widget *)XtRealloc(
                    (char *)switch_data->buttons,
                ...
                if (switch_data->switch_count <=
                    (intptr_t)(
                        switch_data->element_values[SWITCH_NUMBER_OF_ROWS]
                            .parsed_value)) {
                        Arg al[1];
        ...
        case DtWSM_REASON_DELETE: {
                ...
                for (i = 0; i < switch_data->switch_count; i++) {
                        if (atom_name == switch_data->atom_names[i]) {
                                ...
                                for (j = i; j < switch_data->switch_count;
                                     j++) {
                                        switch_data->switch_names[j] =
                                            switch_data->switch_names[j + 1];
                                        switch_data->buttons[j] =
                                            switch_data->buttons[j + 1];
                                }
                ...
                if (panel.switch_row_count >
                    (intptr_t)(
                        ...
                        if ((int)(screen_width - (width + button_width +
                                                  button_spacing)) > 0) {
                                panel.switch_row_count--;
        ...
        case DtWSM_REASON_TITLE: {
                ...
                for (i = 0; i < switch_data->switch_count; i++) {
                        if (atom_name == switch_data->atom_names[i]) {
                                ...
                                toggle_string = XmStringCreateLocalized(
                                    workspace_info->pchTitle);
        ...
        case DtWSM_REASON_CURRENT: {
                ...
                for (i = 0; i < switch_data->switch_count; i++) {
                        if (switch_data->atom_names[i] == atom_name) {
                                ...
                                _DtIconSetState(
                                    switch_data
                                        ->buttons[switch_data->active_switch],
                                    False, False);
...
{
        DtControlCallbackStruct *callback =
...

{
        ...
        if (strlen(edit_string) == 0)
                ...
                for (i = 0; i < switch_data->switch_count; i++) {
                        if (strcmp(switch_data->switch_names[i], edit_string) ==
                            0) {
                                if (i == current_switch) {
                                        XtUnmanageChild(w);
                                        XmProcessTraversal(switch_button,
                                                           XmTRAVERSE_CURRENT);
                                        XtFree(edit_string);
                                        return;
                                } else {
...

{
        ...
        if (first) {
                ...
                XtAppAddActions(XtWidgetToApplicationContext(panel.shell),
                                action_table, 1);
        ...
        if ((edit_string = (char *)_XmStringUngenerate(
                 toggle_string, NULL, XmCHARSET_TEXT, XmCHARSET_TEXT)) !=
            (char *)NULL) {
...
Boolean CheckOtherMonitorsOn(SubpanelData *subpanel_data) {
        ...
        for (i = -1; i < subpanel_data->control_data_count; i++) {
                if (i == -1) {
                        m_state = _DtControlGetMonitorState(
...

{
        ...
        control_behavior = (intptr_t)(
            panel.element_values[PANEL_CONTROL_BEHAVIOR].parsed_value);
        ...
        for (i = 0; i < panel.box_data_count; i++) {
                if (panel.box_data[i]->switch_edit != NULL) {
                        if (XtIsManaged(panel.box_data[i]->switch_edit)) {
                                ...
                                XtSetArg(al[0], XmNuserData, &switch_button);
                                XtGetValues(panel.box_data[i]->switch_edit, al,
                                            1);
        ...
        if (control_data->parent_type == SUBPANEL) {
                ...
                for (i = 0; i < panel.box_data_count; i++) {
                        for (j = 0; j < panel.box_data[i]->control_data_count;
                             j++) {
                                ControlData *box_control;

                                box_control =
                                    panel.box_data[i]->control_data[j];

                                if (box_control->subpanel_data != NULL &&
                                    box_control->subpanel_data
                                            ->main_panel_icon_copy == w) {
                                        subpanel_data =
                                            box_control->subpanel_data;
                                        main_control_data =
        ...
        if (callback->reason == XmCR_BUSY_START) {
                ...
                for (i = 0, pWid = M_PopupList(panel.shell);
                     i < M_NumPopups(panel.shell); i++)
                        XUndefineCursor(XtDisplay(panel.shell),
                                        XtWindow(pWid[i]));
                ...
                if (subpanel_data != NULL) {
                        ...
                        if (XtIsManaged(subpanel_data->shell)) {
                                if (m_state == DtMONITOR_ON) {
                                        ...
                                        XtSetValues(main_control_data->arrow,
                                                    al, 1);
                                } else {
                        ...
                        if (CheckControlTypeFile(control_data) &&
                            push_action != NULL) {
                                ...
                                if (push_recall == False) {
                                        /*  If the control is of type file,
                                         * build and object  */
                                        /*  list that contains the host and file
                                         * name to      */
                                        ...
                                        if (control_type == CONTROL_FILE &&
                                            !control_data->is_action) {
                                                ...
                                                aap = (DtActionArg *)XtCalloc(
                                                    1, sizeof(DtActionArg));
                                                ...
                                                DtActionInvoke(
                                                    ...
                                                    aap, 1, NULL, NULL, NULL, 1,
                                                    NULL, NULL);

                                                XtFree((char *)aap);
                                        } else {
                                                if (strcmp(push_action
                                                               ->action_name,
                                                           "FPOnItemHelp") ==
                                                    0) {
                                                        WmDtHelpMode();
                                        ...
                                        WmGlobalData *wm_global_data =
                                            (WmGlobalData *)panel.global_data;
                                        ClientData *wm_client_data = NULL;
                                        WmPushRecallArg push_argument;

                                        /*  Parse out the function and arguments
                                         * used by the  */
                                        ...
                                        function_index = ParseWmFunction(
                                            (unsigned char **)&s1, CRS_ANY,
                                            &wm_func);
                                        XtFree(s1save);
                                        ...
                                        ParseWmFunctionArg(
                                            (unsigned char **)&s1,
                                            function_index, wm_func,
                                            (void **)&func_arg, client_name,
                                            client_name);

                                        /*  Set the push argument to the parsed
                                         * data  */
                                        ...
                                        /*  Get the window manager client data
                                         * for the panel and  */
                                        /*  call the push recall function to get
                                         * the client run.  */

                                        if (subpanel_data != NULL)
                                                XFindContext(
                                                    wm_global_data->display,
                                                    XtWindow(
                                                        subpanel_data->shell),
                                                    wm_global_data
                                                        ->windowContextType,
                ...
                /*  Call the unposting function and reset the arrow behavior if
                 * the  */
                /*  action occured out of a subpanel. */

                if (subpanel_data != NULL && unpost_subpanel &&
                    (Boolean)(intptr_t)panel
                        .element_values[PANEL_SUBPANEL_UNPOST]
                        .parsed_value &&
        ...
        if (CheckControlTypeFile(control_data) &&
            (animate_data->dropData->protocol == DtDND_FILENAME_TRANSFER ||
             animate_data->dropData->protocol == DtDND_BUFFER_TRANSFER)) {
                ...
                drop_action = (PanelActionData *)control_data
                ...
                } else if (animate_data->dropData->protocol ==
                           DtDND_BUFFER_TRANSFER) {
                        DtDndBuffer *buf_list =
                            animate_data->dropData->data.buffers;
                        int m = 0;
                ...
                DtActionInvoke(w, drop_action->action_name, aap, numItems, NULL,
...

{
        ...
        help_volume =
            (char *)subpanel_data->element_values[SUBPANEL_HELP_VOLUME]
                .parsed_value;
        help_topic = (char *)subpanel_data->element_values[SUBPANEL_HELP_TOPIC]
                         .parsed_value;
        help_string =
            (char *)subpanel_data->element_values[SUBPANEL_HELP_STRING]
                .parsed_value;
...

{
        ...
        help_volume = (char *)switch_data->element_values[SWITCH_HELP_VOLUME]
                          .parsed_value;
        help_topic =
            (char *)switch_data->element_values[SWITCH_HELP_TOPIC].parsed_value;
        help_string = (char *)switch_data->element_values[SWITCH_HELP_STRING]
                          .parsed_value;
...

{
        ...
        help_volume = (char *)control_data->element_values[CONTROL_HELP_VOLUME]
                          .parsed_value;
        help_topic = (char *)control_data->element_values[CONTROL_HELP_TOPIC]
                         .parsed_value;
        help_string = (char *)control_data->element_values[CONTROL_HELP_STRING]
                          .parsed_value;
...

{
        ...
        *element_values = (ElementValue *)XtMalloc(sizeof(ElementValue) *
        ...
        (*element_values)[CONTROL_FILE_NAME].string_value =
...

{
        ...
        for (i = 0; i < file_count; i++) {
                ...
                if (!strcmp(data_type, "FP")) {
                        ...
                        /*  Reset the values of the container name and type to
                         * this one  */

                        if (element_values[CONTROL_CONTAINER_NAME]
                                .use_default == False) {
                                if (element_values[CONTROL_CONTAINER_NAME]
                                        .parsed_value != NULL)
                                        XtFree(element_values
                        ...
                        if (element_values[CONTROL_CONTAINER_TYPE]
                                .use_default == False) {
                                if (element_values[CONTROL_CONTAINER_TYPE]
                                        .string_value != NULL)
                                        XtFree(element_values
                        ...
                        if (element_values[CONTROL_POSITION_HINTS]
                                .use_default == False) {
                                if (element_values[CONTROL_POSITION_HINTS]
                                        .string_value != NULL)
                                        XtFree(element_values
                        ...
                        element_values[CONTROL_CONTAINER_NAME].use_default =
                            False;
                        element_values[CONTROL_CONTAINER_NAME].string_value =
                            XtNewString(
                                subpanel_data->element_values[SUBPANEL_NAME]
                                    .parsed_value);
                        ...
                        element_values[CONTROL_CONTAINER_TYPE].use_default =
                            False;
                        element_values[CONTROL_CONTAINER_TYPE].string_value =
                            XtNewString("SUBPANEL");
                        ...
                        element_values[CONTROL_POSITION_HINTS].use_default =
                            False;
                        element_values[CONTROL_POSITION_HINTS].string_value =
                            XtNewString("last");
                ...
                new_control_name =
                    (char *)element_values[CONTROL_NAME].parsed_value;
                bad_control = False;
                ...
                for (j = 0; j < subpanel_data->control_data_count; j++) {
                        ...
                        if (strcmp(control_name, new_control_name) == 0) {
                                ControlData control_data;
                                String title, del_ctrl, ctrl_name, del_msg,
                                    message;

                                /*  Display an error dialog for the unusable
                                 * drop data  */

                                title = FPGETMESSAGE(
                                    86, 3,
                                    "Workspace Manager - Install Icon Error");
                                title = XtNewString(title);
                                ...
                                /*  Set up a temporary control data structure so
                                 * that the   */
                                /*  appropriate function can be used to free the
                                 * allocated  */
                                /*  element values. */

                                control_data.element_values = element_values;
                                RemoveEntry((RecordData *)&control_data,
                                            CONTROL);
                ...
                if ((intptr_t)element_values[CONTROL_MONITOR_TYPE]
                        .parsed_value != MONITOR_NONE)
                ...
                position_hints =
                    (intptr_t)element_values[CONTROL_POSITION_HINTS]
                        .parsed_value;
                ...
                if (control_monitor) {
                        ...
                        for (j = 0; j < subpanel_data->control_data_count; j++)
                                XtSetValues(
                                    subpanel_data->control_data[j]->icon, al,
                                    1);

                        if (subpanel_data->main_panel_icon_copy != NULL)
                                XtSetValues(subpanel_data->main_panel_icon_copy,
                                            al, 1);
                }
                ...
                subpanel_data->control_data = (ControlData **)XtRealloc(
                    (char *)subpanel_data->control_data,
                    sizeof(ControlData *) * count);

                for (j = count - 2; j >= 0; j--) {
                        if (position_hints >=
                            (intptr_t)subpanel_data->control_data[j]
                ...
                for (k = count - 1; k > j; k--)
                        subpanel_data->control_data[k] =
                            subpanel_data->control_data[k - 1];
                ...
                if (j == 0 && subpanel_data->main_panel_icon_copy != NULL)
                        ...
                        attach_widget =
                            subpanel_data->control_data[j - 1]->icon;
