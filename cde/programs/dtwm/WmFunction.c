...
#ifndef lint
static char rcsid[] =
    "$TOG: WmFunction.c /main/19 1998/04/20 13:00:48 mgreess $"
#endif
#endif
/*
...
Boolean F_Action(String actionName, ClientData *pCD, XEvent *event) {
        ...
        DtActionInvoke(wmGD.pActiveSD->screenTopLevelW1, pAP->actionName,
                       pAP->aap, pAP->numArgs, pAP->szExecParms, NULL, NULL, 1,
                       NULL, NULL);
 ...
 */
ClientData *FindSecondToTopTransient(pcd) ClientData *pcd;
{
        ClientData *pcdNext;
        ...
        } else /* NORMAL_STATE, MAXIMIZED_STATE, adoption */
        {
                ...
                if ((pcdLeader->transientChildren) &&
                    ...
                    (!wmGD.bSuspendSecondaryRestack)) {
                        if (pCD == pcdLeader) {
                                /*
                                 * Lower requested on the leader itself, insure
                                 * it's at the bottom.
                                 */
                                bLeaderRestacked =
                                ...
                                /*
                                 * Lower requested on the leader of a subtree.
                                 * Insure that this subtree leader is at the
                                ...
                                /*
                                 * Lower requested on a transient. Insure all
                                 * the subtree leaders up to the top are at the
                                 * bottom of their respective transient
                                 * subtrees.
                                 */
                                for (pcdLdr = pCD->transientLeader; pcdLdr;
                                     pcdLdr = pcdLdr->transientLeader) {
                                        bLeaderRestacked |=
                                            BumpPrimaryToBottom(pcdLdr);
                                }
                        }
                }
#endif /* WSM */
                ...
                if (pCD->transientLeader) {
                        ...
                        /*
                         * If freeFamily stacking, then put dirty transient
                         * window (if any) back in place before force lowering
                         * current window to the bottom of the global window
                         * stack.  Then return.
                         */

                        if (flags & STACK_FREE_FAMILY) {
                                /* Restore dirty transient if not current
                                 * window. */
                                if ((dirtyStackEntry) &&
                                    (dirtyStackEntry != pCD)) {
                                        RestackTransients(dirtyStackEntry);
                        ...
                        /*
                         * Reach here only if NOT doing a f.lower freeFamily
                         * (see return; statement above).  Put current transient
                         * below its sibling transient windows.
                         */
                ...
                if (pStackEntry) {
                        if ((pStackEntry->prevSibling !=
                             &pcdLeader->clientEntry) &&
                    ...
                    ((pNextEntry->type == MINIMIZED_STATE) &&
                     (pcdNext->clientState == MINIMIZED_STATE))) {
                        if (pcdNext->clientState == MINIMIZED_STATE) {
                                ...
                                if (pcdNext->transientLeader ||
                                    pcdNext->transientChildren) {
                        ...
                        if (types & windowType) {
                                ...
                                /*
                                 * This window (or window tree) is obscuring
                                 * another window on the screen.  Lower the
                                 * window.
                                 */
                    ...
                    ((pNextEntry->type == MINIMIZED_STATE) &&
                     (pcdNext->clientState == MINIMIZED_STATE))) {
                        ...
                        if (types & windowType) {
                                ...
                                /*
                                 * This window (or window tree) is obscured by
                                 * another window on the screen.  Raise the
                                 * window.
                                 */
...
Boolean F_Focus_Color(String args, ClientData *pCD, XEvent *event) {
        ...
        if (wmGD.colormapFocusPolicy == CMAP_FOCUS_EXPLICIT) {
                if (pCD) {
                        /*
                         * The window selected for the colormap focus is a
                         * top-level client window.  If there are subwindow
                         * colormaps then determine if the selection was in one
                         * of the subwindows.
                         */
...
Boolean F_Exec(String args, ClientData *pCD, XEvent *event) {
...
#endif
        {
...
#ifndef NO_SETPGRP
#if defined(SVR4) || defined(__OSF1__) || defined(linux)
                setsid();
#else
#ifdef SYSV
                setpgrp();
...

                {
                        ...
                        if (shellname == NULL) {
                                /*
                                If the shell pathname obtained from SHELL or
                                MWMSHELL does not have a "/" in the path and if
                                the user expects this shell to be obtained using
                                the PATH variable rather than the current
                                directory, then we must call execlp and not
                                execl
                                */
                                shellname = shell;
                        ...
                        /*
                         * There is no SHELL environment variable or the first
                         * execl failed. Try /bin/sh .
                         */
#ifdef SVR4
                ...
                if (!(reMapClient)) {
                        if (pCD->maxConfig) {
                                ...
                                if (wmGD.positionIsFrame) {
                                        CalculateGravityOffset(pCD, &xoff,
                                                               &yoff);
                                        x = pCD->clientX - xoff;
                                        y = pCD->clientY - yoff;
                                } else {
...
void Do_Focus_Key(ClientData *pCD, Time focusTime, long flags) {
...
#endif /* WSM */
        {
                if (pCD->clientState == MINIMIZED_STATE) {
                        focusWindow = ICON_FRAME_WIN(pCD);
                } else if (wmGD.keyboardFocusPolicy ==
                           KEYBOARD_FOCUS_EXPLICIT) {
                        /*
                         * Set the keyboard focus to the indicated client
                         * window. If the window has an application modal
                         * subordinate then set the input focus to that window
                         * if the focus isn't already owned by a subordinate.
                         */
                        ...
                        if (IS_APP_MODALIZED(pCD)) {
                                ...
                                /*
                                 * Handle case where a modal window exists when
                                 * Mwm starts up. wmGD.keyboardFocus is NULL,
                                ...
                                while (pcdFocusLeader->transientLeader &&
                                       (pcdFocusLeader != currFocusLeader)) {
                                        pcdFocusLeader =
                                            pcdFocusLeader->transientLeader;
                                }
                                ...
                                if (pcdFocusLeader == currFocusLeader) {
                                        ...
                                        pcdFocus =
                                            FindTransientFocus(pcdFocusLeader);
                                }
                        }
                         ...
                         * it had modal transients when in fact all its
                         *transients had been withdrawn (fixed in
                         *WithdrawTransientChildren()). As a result,
                         *FindTransientFocus() returns wmGD.keyboardFocus; if
                         *nobody has the focus, FindTransientFocus() returns
                         *NULL.
                         */
                        if (pcdFocus) {
                                focusWindow = pcdFocus->client;
                                ...
                                focusWindow = (wmGD.keyboardFocus)
                                                  ? wmGD.keyboardFocus->client
                                                  : ACTIVE_PSD->wmWorkspaceWin;
                        }
                } else {
                        /*
                         * If the focus policy is "pointer" don't set the focus
                         * to a window if it has an application modal
                         * subordinate.
                         */
                        ...
                        if (IS_APP_MODALIZED(pCD)) {
                                ...
                                /* Replay this later when the modal window is
                                 * removed. */
                ...
                /*
                 * Set up the default (non client specific) keyboard input
                 * focus.
                 */
                ...
                if (wmGD.keyboardFocusPolicy == KEYBOARD_FOCUS_POINTER) {
                        ...
                        if (WORKSPACE_IF_NULL & flags) {
                                ...
                                /* find some reasonable client so that focus is
                                 * not lost */
        ...
        if ((pcdFocus != wmGD.keyboardFocus) || (flags & ALWAYS_SET_FOCUS)) {
                if (pcdFocus) {
                        /*
                         * Set the focus and/or send a take focus client
                         * message.  This is not done if a client area button
                         * press was done to set set the focus and the window is
                         * a globally active input style window (See ICCCM).
                         */

                        if ((flags & CLIENT_AREA_FOCUS) &&
                            (pcdFocus->protocolFlags &
                             PROTOCOL_WM_TAKE_FOCUS) &&
                            ...
                            !(pcdFocus->clientState == MINIMIZED_STATE)) {
                                ...
                                /* this window has WM_TAKE_FOCUS set and
                                 * InputField false. */
                                /* just send a message. */
                                ...
                                if ((pcdFocus->protocolFlags &
                                     PROTOCOL_WM_TAKE_FOCUS) &&
                                    !(pcdFocus->clientState ==
                                      MINIMIZED_STATE)) {
                                        /*
                                         * Locally Active Input Model - Send a
                                         * take focus message to the client.
                                         */
                                ...
                                /*
                                 * Don't set the input focus if the window has
                                 * input_field set to False or has expressed an
                                 * interest in WM_TAKE_FOCUS (ie. 'No Input',
                                 * 'Globally Active', or 'Locally Active'), and
                                 * the user click in the client area.  If the
                                 * user clicks on the titlebar or traverses to
                                 * this window via f.next_key, set the focus so
                                 * that the user can access the window menu and
                                 * accelerators.
                                 */

                                if (wmGD.enforceKeyFocus || /* res - default ==
                                                               True. */
                                    (flags & ALWAYS_SET_FOCUS) ||
                                    !(flags &
                                      CLIENT_AREA_FOCUS) ||      /* clicked on
                                                                    frame? */
                                    pcdFocus->inputFocusModel || /* Pass.|Glob.
                                                                    Active */
                                    (pcdFocus->clientState ==
                                     MINIMIZED_STATE)) {
                                        if (!(flags & CLIENT_AREA_FOCUS) &&
                                            ...
                                            !(pcdFocus->clientState ==
                                              MINIMIZED_STATE)) {
                                                /* the window doesn't want the
                                                 * focus - set it to the frame
                                                 */
                                                /* user clicked on the frame but
                                                 * we don't want the focus */
                                                /* set it to the client's frame
                                                 */
                                                XSetInputFocus(
                                                    DISPLAY,
                                                    pcdFocus->clientBaseWin,
                                                    RevertToPointerRoot,
                                                    focusTime);
                                        } else if (!(flags &
                                                     CLIENT_AREA_FOCUS) &&
                                        ...
                                        /*
                                         * We've decided that the window
                                         * shouldn't get the focus, so don't
                                         * change the focus.
                                         */
...
#ifdef ROOT_ICON_MENU
                if (focused && wmGD.iconClick && event &&
                    event->type == KeyPress && wmGD.nextKeyboardFocus &&
        ...
        if (!fpMenuSpec) {
                ...
                for (oldSpec = pSD->menuSpecs; oldSpec != (MenuSpec *)NULL;
                     oldSpec = oldSpec->nextMenuSpec) {
                        if (oldSpec->name &&
                            (strcmp(oldSpec->name, newMenuName) == 0))
                                break;
                }
                if (!oldSpec)
                        return False;
                ...
                (void)MAKE_MENU(pSD, pCD, newMenuName, F_CONTEXT_NORMAL,
                                F_CONTEXT_NORMAL, (MenuItem *)NULL, FALSE);
...
Boolean F_Push_Recall(String args, ClientData *pCD, XEvent *event) {
        ...
        if (pPRP->ixReg < pSD->numPushRecallClients) {
                ...
                /*
                 * If the client is already running, then top it in this
                 * workspace, else invoke the function to start it.
                 */
                if (pPRCD->pCD) {
                        ...
                        if (!(ClientInWorkspace(pSD->pActiveWS, pPRCD->pCD))) {
                                 ...
                                 */
                                wsRemoveList = GetListOfOccupiedWorkspaces(
                                    pPRCD->pCD, &sizeRemoveList);
                                RemoveClientFromWorkspaces(
                                    pPRCD->pCD, wsRemoveList, sizeRemoveList);
                                XtFree((char *)wsRemoveList);
                                AddClientToWorkspaces(pPRCD->pCD,
                                                      &(pSD->pActiveWS->id), 1);
                                SetClientWsIndex(pPRCD->pCD);
                                SetClientState(pPRCD->pCD,
                                               pPRCD->pCD->clientState &
                                                   ~UNSEEN_STATE,
                                               CurrentTime);
                        }
                        ...
                        if (!bWaiting) {
                                ...
                                if (pPanelist && panel.busy_light_data) {
                                        ...
                                        XtSetArg(al[ac],
                                                 ...
                                                 &clientTimeout);
                                        ac++;
                                        XtGetValues(panel.busy_light_data->icon,
                                                    (ArgList)al, ac);
                                }

                                /*
                                 * ClientTimeout is in milliseconds, timeval
                                 * values are in seconds and microseconds.
                                 */
...
Boolean F_Pass_Key(args, pCD, event) String args;
ClientData *pCD;
XEvent *event;
...
Boolean F_Menu(String args, ClientData *pCD, XEvent *event) {
            ...
            ((event->type == ButtonPress) || (event->type == ButtonRelease))) {
                 ...
                 */
                        ...
                        if (wmGD.bReplayedButton) {
                                /* This button was replayed, it most likely
                                   dismissed a previous sticky menu, don't post
                                   a menu here */
                                return (False);
                        }
        ...
        if ((menuSpec = MAKE_MENU(pSD, pCD, args, menuContext, menuContext,
                ...
                PostMenu(menuSpec, pCD, x, y, button, menuContext, flags,
                         event);
        }
...
Boolean F_Minimize(String args, ClientData *pCD, XEvent *event) {
        ...
        if (pCD) {
                /*
                 * If the window is a transient then minimize the entire
                 * transient tree including the transient leader.
                 */

                pcdLeader =
                    (pCD->transientLeader) ? FindTransientTreeLeader(pCD) : pCD;
...
Boolean F_Normalize_And_Raise(String args, ClientData *pCD, XEvent *event) {
        ...
        if (args) {
                ...
                if (pSD->useIconBox && wmGD.useFrontPanel &&
                    pSD->iconBoxControl && (!strcmp(args, WmNiconBox))) {
                        /*
        ...
        if (pCD) {
                if (pCD->clientState == MINIMIZED_STATE) {
                        ...
                        SetClientStateWithEventMask(
                            pCD, NORMAL_STATE,
                            (Time)(event ? GetFunctionTimestamp(
                                               (XButtonEvent *)event)
                                         : GetTimestamp()),
                            GetEventInverseMask(event));
                } else {
                        ...
                        /* Raise the window and set the keyboard focus to the
                         * window */
#ifdef WSM
                        wmGD.bSuspendSecondaryRestack = True;
#endif /* WSM */
...
Boolean F_Restore(String args, ClientData *pCD, XEvent *event) {
        ...
        if (pCD) {
                /*
                 * If current state is MAXIMIZED state then just go to NORMAL
                 * state, otherwise (you are in MINIMIZED state) return to
                 * previous state.
                 */
...
Boolean F_Post_RMenu(String args, ClientData *pCD, XEvent *event) {
        ...
        if ((event->type == KeyPress) || (event->type == KeyRelease)) {
                ...
                if (rootMenu == (MenuSpec *)NULL)
                        ...
                        XQueryPointer(DISPLAY, ACTIVE_ROOT, &rwin, &cwin, &x,
                                      &y, &winx, &winy, &mask);
...
Boolean F_Post_SMenu(String args, ClientData *pCD, XEvent *event) {
        ...
        if (event && pCD && pCD->systemMenuSpec) {
                ...
                if ((event->type == KeyPress) || (event->type == KeyRelease)) {
                        ...
                        } else if (wmGD.systemButtonClick &&
                                   (pCD->decor & MWM_DECOR_MENU)) {
                                wmGD.checkHotspot = True;
                        }
                        ...
                        if (wmGD.rootButtonClick && (!wmGD.checkHotspot)) {
                                ...
                                wmGD.hotspotRectangle.x =
                                    ...
                                    wmGD.moveThreshold / 2;
                                wmGD.hotspotRectangle.y =
                                    event->xbutton.y_root -
                                    wmGD.moveThreshold / 2;
                                wmGD.hotspotRectangle.width =
                                    wmGD.moveThreshold;
                                wmGD.hotspotRectangle.height =
                                    wmGD.moveThreshold;
                        }
#endif /* WSM */
                        PostMenu(pCD->systemMenuSpec, pCD,
                                 event->xbutton.x_root, event->xbutton.y_root,
                                 event->xbutton.button, menuContext, POST_AT_XY,
                                 event);
...
Boolean F_Kill(String args, ClientData *pCD, XEvent *event) {
        if (pCD && (pCD->clientFunctions & MWM_FUNC_CLOSE)) {
                ...
                if (pCD->dtwmBehaviors & DtWM_BEHAVIOR_SUBPANEL) {
                         ...
                         */
                        wPanel =
                            WmPanelistWindowToSubpanel(DISPLAY1, pCD->client);
                        if (wPanel) {
                ...
                if (pCD->clientFlags & ICON_BOX) {
                    ...
                    if (!do_delete_window && !do_save_yourself) {
                        ...
                        if (do_delete_window) {
                                ...
                                SendClientMsg(pCD->client,
                                              (long)wmGD.xa_WM_PROTOCOLS,
                                              (long)wmGD.xa_WM_DELETE_WINDOW,
                                              CurrentTime, NULL, 0);
                        }
#ifdef WSM
                        /*
                        ...
                        {
                                /*
                                 * Send a WM_SAVE_YOURSELF message and wait for
                                 * a change to the WM_COMMAND property.
                                 * !!! button and key input should be kept from
                                 * the window !!!
                                 */

                                if (AddWmTimer(TIMER_QUIT,
                                               (unsigned long)wmGD.quitTimeout,
                                               pCD)) {
                                        SendClientMsg(
                                            pCD->client,
                                            (long)wmGD.xa_WM_PROTOCOLS,
                                            (long)wmGD.xa_WM_SAVE_YOURSELF,
                                            CurrentTime, NULL, 0);
...
static void RefreshByClearing(Window win) {
        ...
        status =
            XQueryTree(DISPLAY, win, &root, &parent, &winChildren, &nchildren);
        if (status != 0) {
...
Boolean F_Refresh(String args, ClientData *pCD, XEvent *event) {
        ...
        if (wmGD.refreshByClearing) {
                ...
                win = XCreateWindow(
                    DISPLAY, ACTIVE_ROOT, 0, 0,
                    (unsigned int)DisplayWidth(DISPLAY, ACTIVE_SCREEN),
                    (unsigned int)DisplayHeight(DISPLAY, ACTIVE_SCREEN), 0, 0,
...
void RestartWm(long startupFlags) {
        ...
        for (scr = 0; scr < wmGD.numScreens; scr++) {
                if (wmGD.Screens[scr].managed) {
                        ...
                        SetMwmInfo(wmGD.Screens[scr].rootWindow, startupFlags,
                                   0);
#ifdef WSM
                        SaveResources(&wmGD.Screens[scr]);
#endif /* WSM */
                        /*
                         * Unmap client windows and reparent them to the root
                         * window.
                         */
...
void DeFrameClient(ClientData *pCD) {
        ...
        while (pCD) {
                ...
                XReparentWindow(DISPLAY, pCD->client, ROOT_FOR_CLIENT(pCD), x,
                                y);
...
Boolean F_Send_Msg(String args, ClientData *pCD, XEvent *event) {
        ...
        if (pCD && pCD->mwmMessagesCount) {
                 ...
                 * _MWM_MESSAGES property before the associated message can be
                 * sent.
                 */
...
void Do_Raise(ClientData *pCD, ClientListEntry *pStackEntry, int flags) {
        ...
        } else /* NORMAL_STATE, MAXIMIZED_STATE, adoption */
        {
                ...
                if ((pcdLeader->transientChildren) &&
                    ...
                    (!IS_APP_MODALIZED(pCD))) {
                        if (pCD != pcdLeader) {
                                ...
                                if (pCD->transientChildren) {
                                        /*
                                         * This isn't the overall leader of the
                                         * transient tree, but it does have
                                         * transient's of its own. Move it to
                                         * the top of its own transient
                                         * sub-tree.
                                         */
                                        bLeaderRestacked |=
                                            BumpPrimaryToTop(pCD);
                                }
                        } else {
                                /*
                ...
                /*
                 * Fix for 5325 - The following code has been reorganized to
                 * cause the action of F_Raise to match the current
                 * documentation. The new algorithm is as
                 * follows:
                 *
                 *                if (dirtyStackEntry)
                 *                  restore dirty tree
                 *                if (not withinFamily)
                 *                  bring window group to the top of global
                 * stack if (freeFamily) raise the requested window to top of
                 * family else raise requested window to top of
                 * siblings if (need to restack windows)
                 *                  restack windows
                 *                return
                 */
                /*
                ...
                if (dirtyStackEntry) {
                        /*
                         * Check to make sure that the dirty pCD has either
                         * transient children or a transient leader.  If not, do
                         * not restore the transients.
                         */
                        if (dirtyStackEntry->transientChildren ||
                            dirtyStackEntry->transientLeader)
                                RestackTransients(dirtyStackEntry);
                ...
                /*
                 * If the flags do not indicate "within", raise the window
                 * family to the top of the window stack. If the window is the
                 * primary, raise it to the top regardless of the flags.
                 */
                if (!pCD->transientLeader || !(flags & STACK_WITHIN_FAMILY)) {
                        if (pStackEntry) {
                                if (pStackEntry->nextSibling !=
                                    &pcdLeader->clientEntry) {
                                        StackWindow(
                                            pWS, &pcdLeader->clientEntry,
                                            False /*below*/, pStackEntry);
                                        MoveEntryInList(
                                            pWS, &pcdLeader->clientEntry,
                                            False /*below*/, pStackEntry);
                 ...
                 * labelled as dirty (what's to dirty it up?).  If it has
                 * either, raise the window to the top of the family stack.
                 */
                if ((flags & STACK_FREE_FAMILY) &&
                ...
                /*
                 * If withinFamily stacking is requested, put the current
                 * transient on top of its sibling transient windows.
                 */
                else {
                        restackTransients = PutTransientOnTop(pCD);
                }

                        /* At this point, if doing a regular f.raise the window
                         * family has already been brought to the top of the
                         * stack, so nothing further needs to be done for it.
                         */
...
Boolean F_Raise_Lower(String args, ClientData *pCD, XEvent *event) {
        ...
        if (pCD) {
                ...
                /*
                 * Treat a raise/lower on a window in a transient tree as if it
                 * is a raise/lower for the whole tree.
                 */
...
Boolean F_Refresh_Win(String args, ClientData *pCD, XEvent *event) {
        ...
        if (pCD && ((pCD->clientState == NORMAL_STATE) ||
                ...
                if (wmGD.refreshByClearing) {
                        ...
                        /* default background_pixmap is None */
                        win = XCreateWindow(
                            DISPLAY, pCD->clientBaseWin, pCD->matteWidth,
...
Boolean F_Screen(String args, ClientData *pCD, XEvent *event) {
        ...
        while (!newscr) {
                ...
                if (!wmGD.Screens[scr]
                         .managed) { /* make sure screen is managed */
