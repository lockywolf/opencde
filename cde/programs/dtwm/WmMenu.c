...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmMenu.c /main/15 1996/11/20 15:20:17 rswiston $"
#endif
#endif
/*
...
#include "WmWrkspace.h"
    ...
    static void
    UnmapCallback(Widget w, XtPointer client_data, XtPointer call_data);
...
static Boolean FindClientCommandMatch(MenuSpec *menuSpec, String clientCommand,
                                      MenuItem **menuItem);
static void InsertTreeOnClient(WmScreenData *pSD, ClientData *pCD,
                               CmdTree *tree, MatchList **client_match_list,
                               MatchList **global_match_list,
                               MenuSpec *menuSpec, MenuItem *template,
                               String command_so_far, Boolean duplicate_globals,
                               Atom selection, Context greyed_context,
                               Boolean inLine);
...
MenuSpec *MakeMenu(WmScreenData *pSD,
        ...
        /* the menuSpecs list is exhausted */
        {
                MWarning(((char *)GETMESSAGE(
                             48, 1, "Menu specification %s not found\n")),
                         menuName);
                return (NULL);
        }
        ...
        /*
         * If making a custom menu, create a custom copy of the specification
         * with which to build the custom menu. Otherwise, if the menu widget
         * exists, possibly modify the accelerator contexts and return the
         * specification.
         */

        if (moreMenuItems != NULL) {
#if ((!defined(WSM)) || defined(MWM_QATS_PROTOCOL))
                if ((newMenuSpec = DuplicateMenuSpec(menuSpec)) ==
                    (MenuSpec *)NULL)
                        return NULL;
#else
                if ((newMenuSpec = (MenuSpec *)XtMalloc(sizeof(MenuSpec))) ==
                    NULL)
        ...
        if (n) {
                if ((menuSpec->menuButtons = (MenuButton *)XtMalloc(
                         n * sizeof(MenuButton))) == NULL)
                /* insufficent memory */
                {
                        MWarning(
                            ((char *)GETMESSAGE(
                                48, 3, "Insufficient memory for menu %s\n")),
                            menuName);
                        return (NULL);
                }
        ...
        menuSpec->menuWidget =
            CREATE_MENU_WIDGET(pSD, pCD, menuName, pSD->screenTopLevelW, TRUE,
                               menuSpec, moreMenuItems);
...
/*************************************<->***********************************/
void CheckTerminalSeparator(menuSpec, buttonWidget, manage) MenuSpec *menuSpec;
Widget buttonWidget;
Boolean manage;
{
        CompositeWidget cw;
        ...
        for (curMenuItem = menuItems; curMenuItem != (MenuItem *)NULL;
             curMenuItem = curMenuItem->nextMenuItem) {
                ...
                if (curMenuItem->accelText != (String)NULL)
                        newMenuItem->accelText =
                            XtNewString(curMenuItem->accelText);
                else
                ...
                if ((curMenuItem->wmFunction == F_Send_Msg)
                    ...
                    || (curMenuItem->wmFunction == F_Action)
                        ...
                        newMenuItem->wmFuncArgs =
                            XtNewString(curMenuItem->wmFuncArgs);
                else
...
MenuSpec *DuplicateMenuSpec(MenuSpec *menuSpec) {
        ...
        /* Handle insufficent memory */
        {
                Warning((char *)GETMESSAGE(
                    48, 9, "Insufficient memory for menu specification\n"));
                return (NULL);
        }
 ...
 *
 *************************************<->***********************************/
static MenuItem *MakeMenuItem(String label, WmFunction wmFunction,
                              String funcArgs, KeySym mnemonic,
                              unsigned int accelState, KeyCode accelKeyCode,
                              String accelText) {
        MenuItem *menuItem;
        /*
        ...
        */
        ...
        {
                MWarning(((char *)GETMESSAGE(
                             48, 10, "Insufficient memory for menu item %s\n")),
                         label);
                return (NULL);
        }
...
static MenuItem *MakeClientCommandMenuItem(String label, String funcArgs) {
        return (MakeMenuItem(label, F_InvokeCommand, funcArgs, (KeySym)NULL,
                             (unsigned int)0, (KeyCode)NULL, (String)NULL));
}
...
static MenuItem *PerformClientCommandMatch(String clientCommand,
        ...
        for (segments = 0; *menuItemCommand != '\0' && *clientCommand != '\0';
             ++segments) {
                /* Skip past the '<' at the beginning of the next segment and
                   any whitespace. */
                ++menuItemCommand;
                ++clientCommand;
                while (isspace(*menuItemCommand))
                        ++menuItemCommand;
                while (isspace(*clientCommand))
                        ++clientCommand;
                ...
                if (*menuItemCommand == '*') {
                        /* Since the menuItemCommand segment is a wildcard, skip
                           it and the current segment of the client command
                           since the wildcard has to match at least one segment
                        ...
                        /* Calculate how long the current segment of the
                           menuItemCommand is */
                        for (seglength = 0; menuItemCommand[seglength] != '>' &&
                         ...
                         */
                        if (menuItemCommand[seglength] == '\0')
                                return (bestMatchSoFar);
                        ...
                        /* Now string compare this segment with the
                           clientCommand segment, up to the number of characters
                           in the menu item segment. */
                        if (strncmp(menuItemCommand, clientCommand,
                                    seglength) == 0) {
                                /* So far so good. Just make sure clientCommand
                                   doesn't have anything but whitespace after
                                   its seglength character. */
                                ...
                                /* We have a match. Clear the foundWildcard
                                   since we have sync'ed up and keep trying to
                                   match. */
                                foundWildcard = FALSE;
                                ...
                                /* We didn't match and there wasn't wildcard to
                                   swallow the discrepancy. Therefore, this is
                                   not a match. */
                                return (bestMatchSoFar);
                        }
                }
        ...
        /* So the menuItemCommand must have matched. If the current best
           match is NULL, then just return the menuItem. Otherwise calculate
           some matching quality metrics for the bestMatchSoFar and compare them
        ...
        for (bestSegments = 0; *bestMatchStr != '\0'; ++bestSegments) {
                /* Skip past the '<' at the beginning of the next segment and
                   any whitespace. */
                ++bestMatchStr;
                while (isspace(*bestMatchStr))
                        ++bestMatchStr;
        ...
        if (bestWildcards != wildcards) {
                ...
                for (i = 0; i < wildcards; ++i)
                        if (wildcardPositions[i] != bestWildcardPositions[i]) {
                                return (bestWildcardPositions[i] <
                                                wildcardPositions[i]
                                            ? bestMatchSoFar
                                            : menuItem);
                        }
...
static Boolean ExcludeClientCommand(MenuSpec *menuSpec, String clientCommand) {
        ...
        for (curExclusion = menuSpec->exclusions;
             ...
             curExclusion = curExclusion->nextExclusion) {
                ...
                if (PerformClientCommandMatch(clientCommand, &placeholder,
                                              NULL) != (MenuItem *)NULL) {
                        return (TRUE);
                }
        }
...
static Boolean ForceInLineToCascade(MenuSpec *menuSpec, String clientCommand,
                                    MenuItem **bestMatch) {
...
static Boolean FindClientCommandMatch(MenuSpec *menuSpec, String clientCommand,
        ...
        for (curMenuItem = menuSpec->menuItems; curMenuItem != (MenuItem *)NULL;
             curMenuItem = curMenuItem->nextMenuItem) {
                bestMatch = PerformClientCommandMatch(clientCommand,
                                                      curMenuItem, bestMatch);
        }
        ...
        for (curExclusion = menuSpec->exclusions;
             ...
             curExclusion = curExclusion->nextExclusion) {
                ...
                bestExclusionItem = PerformClientCommandMatch(
                    clientCommand, &placeholder, bestExclusionItem);
...
static void PerformInsertionsOnMatchList(MatchList **matchlist) {
        ...
        for (curmatch = *matchlist; curmatch != (MatchList *)NULL;
             curmatch = curmatch->next) {
                if (curmatch->menuitem != (MenuItem *)NULL) {
                        ...
                        newMenuItem = MakeMenuItemFromTemplate(
                            curmatch->menuitem, curmatch->treenode->defaultName,
                            curmatch->funcargs);
                        ...
                        newMenuItem->clientCommandID =
                            curmatch->treenode->commandID;
                        ...
                        if (curmatch->menuspec != NULL) {
                                ...
                                for (last = curmatch->menuspec->menuItems;
                                     ...
                                     last = last->nextMenuItem) {
                                        /* If the next item is f.quit and it is
                                           the last item, then stop searching
                                           now. We don't want to insert after a
                                           trailing f.kill (i.e. Close). */
                                        if ((last->nextMenuItem->wmFunction ==
                                             F_Kill) &&
                                            (last->nextMenuItem->nextMenuItem ==
                                             (MenuItem *)NULL))
                                                break;
                                }
                        }
                        ...
                        newMenuItem = MakeClientCommandMenuItem(
...
void DestroyMenuSpec(WmScreenData *pSD, CARD32 commandID) {
        ...
        for (curClient = pSD->clientList; curClient != (ClientListEntry *)NULL;
...
static MenuSpec *FindLastMenuSpecToModify(MenuSpec *menuSpec,
        ...
        for (curMenuSpec = menuSpec; curMenuSpec != (MenuSpec *)NULL;
             curMenuSpec = curMenuSpec->nextMenuSpec) {
...
static void RecreateMenuWidgets(WmScreenData *pSD, ClientData *pCD,
        ...
        for (current = *matchlist; current != (MatchList *)NULL;
        ...
        for (current = *matchlist; current != (MatchList *)NULL;
             current = current->next) {
                for (i = 0; i < count; ++i) {
                        if (to_change[i] == current->menuspec)
                                break;
        ...
        for (i = 0; i < count && to_change[i] != (MenuSpec *)NULL; ++i) {
                to_change[i]->menuWidget = CreateMenuWidget(
                    pSD, pCD, to_change[i]->name, pSD->screenTopLevelW, TRUE,
                    to_change[i], NULL);
        }
...
static void SearchForOtherMatches(WmScreenData *pSD, ClientData *pCD,
        ...
        for (current = (pCD == NULL ? NULL : pCD->systemMenuSpec);
             current != (MenuSpec *)NULL; current = current->nextMenuSpec) {
                             ...
                             strncmp(match->label, "=>", 2) == 0)) {
                                ...
                                for (tree = treenode->subTrees;
                                     ...
                                     tree = tree->next) {
                                        ...
                                        StoreMatchedCommand(
                                            client_match_list, current, match,
                                            new_command_str, tree,
                                            inLine_function, new_funcargs,
                                            greyed_context);
                                }
                        } else {
                                StoreMatchedCommand(client_match_list, current,
        ...
        for (current = pSD->menuSpecs; current != (MenuSpec *)NULL;
             current = current->nextMenuSpec) {
                FindClientCommandMatch(current, command_string, &match);
                if (match != (MenuItem *)NULL) {
                        if (duplicate_globals == TRUE) {
                                /* Create a duplicate of the current menuspec
                                   and store that away in the client instead of
                                   the current menuspec. */
                                newMenuSpec =
                                    ReplaceMenuSpecForClient(current, pCD);

                                /* Now store enough information so that we can
                                   actually create the insertion later. */
                                StoreMatchedCommand(
                                    client_match_list, newMenuSpec, NULL,
                                    command_string, treenode, function,
                                    funcargs, greyed_context);
                        } else /* Change global menu */
                        {
                                StoreMatchedCommand(global_match_list, current,
                                                    match, command_string,
                                                    treenode, function,
                                                    funcargs, greyed_context);
                        }
        ...
        while (tree != (CmdTree *)NULL) {
                    ...
                    ForceInLineToCascade(menuSpec, new_command_str,
                        ...
                        for (subtree = tree->subTrees;
                             ...
                             subtree = subtree->next) {
                                /* Pass along the bestMatch. If it is a valid
                                   menu item then we want the insertion to occur
                                   at that menuitem instead of at the end of the
                                   menu. */
                                InsertTreeOnClient(
                                    pSD, pCD, subtree, client_match_list,
                        ...
                        /* We don't want to search for other matches because we
                           want the items to be inserted inLine. Any other
                           matches will be found in the recursive calls. (??? or
                           am I wrong?) */
                }
                /* If tree is a command set then we need to create a new
                   menuSpec. */
                        ...
                        /* Store the cascade button information so it can be
                           created later. */
                        StoreMatchedCommand(
                            (menuSpec->clientLocal ? client_match_list
                                                   : global_match_list),
                            menuSpec, template, new_command_str, tree, F_Menu,
                            funcarg_buf, greyed_context);

                        /* We need to create a menu spec for the menu that this
                           cascade button will cascade to. Try to find one in
                           the clients menu spec list, stopping the first time
                           we hit a global menu. If we can't find one there and
                           if we are *not* supposed to duplicate globals, then
                           try to find it in the global list. In all other
                           cases, create a new one using the funcarg_buf that we
                           created above as the name of the menuspec. */
                        for (newMenuSpec =
                                 (pCD == NULL ? NULL : pCD->systemMenuSpec);
                             newMenuSpec != (MenuSpec *)NULL;
                        ...
                        /* If we didn't find it in the client list, maybe we
                           should look in the global list. */
                        if (newMenuSpec == (MenuSpec *)NULL &&
                            duplicate_globals == FALSE) {
                                for (newMenuSpec = pSD->menuSpecs;
                                     newMenuSpec != (MenuSpec *)NULL;
                                     newMenuSpec = newMenuSpec->nextMenuSpec) {
                                        if (strcmp(newMenuSpec->name,
                                                   funcarg_buf) == 0)
                                                break;
                                }
                        }

                        /* If we still don't have a menu spec, then create a new
                         * one. */
                        if (newMenuSpec == (MenuSpec *)NULL) {
                                newMenuSpec = MakeMenuSpec(
                                    funcarg_buf, tree == NULL
                                                     ? (CARD32)NULL
                                                     : tree->commandID);
                                if (duplicate_globals)
                                        newMenuSpec->clientLocal = TRUE;
                                else
                                        newMenuSpec->clientLocal = FALSE;

                                /* If we are duplicating globals, then add the
                                   new menu spec to the client list. Otherwise
                        ...
                        /* Search for any other matches in the existing menu
                           specs for this command, excluding newMenuSpec. */
                        SearchForOtherMatches(
                            pSD, pCD, tree, client_match_list,
                            global_match_list, newMenuSpec, new_command_str,
                            F_Menu, funcarg_buf, duplicate_globals, selection,
                ...
                } else /* the tree is a simple command */
                {
                        ...
                        /* If the menuSpec is global and we are supposed to be
                           duplicating global menu specs, then create a
                           duplicate and replace the menuspec with the duplicate
                           for this client. */
                        if (duplicate_globals)
                                dupMenuSpec =
                                    ReplaceMenuSpecForClient(menuSpec, pCD);
                        ...
                        /* Now store the match away in the appropriate match
                         * list */
                        StoreMatchedCommand(
                            (dupMenuSpec->clientLocal ? client_match_list
                                                      : global_match_list),
                        ...
                        SearchForOtherMatches(
                            pSD, pCD, tree, client_match_list,
                            global_match_list, dupMenuSpec, new_command_str,
...
void InsertTreeOnAllClients(WmScreenData *pSD, CmdTree *tree, Atom selection,
        ...
        for (current = pSD->clientList; current != (ClientListEntry *)NULL;
             current = current->nextSibling) {
                ...
                InsertTreeOnClient(pSD, current->pCD, tree, &client_matchlist,
                                   &global_matchlist,
                                   current->pCD->systemMenuSpec, NULL, NULL,
                                   FALSE, selection, greyed_context, inLine);
 ...
 *
 *************************************<->***********************************/
void InsertTreeOnSingleClient(WmScreenData *pSD, ClientData *pCD, CmdTree *tree,
        ...
        InsertTreeOnClient(pSD, pCD, tree, &client_matchlist, &global_matchlist,
                           pCD->systemMenuSpec, NULL, NULL, TRUE, selection,
                           greyed_context, inLine);
        PerformInsertionsOnMatchList(&client_matchlist);
...
void InsertTreeOnRootMenu(WmScreenData *pSD, CmdTree *tree, Atom selection,
        ...
        for (rootMenu = pSD->menuSpecs; rootMenu != (MenuSpec *)NULL;
        ...
        InsertTreeOnClient(pSD, NULL, tree, &client_matchlist,
                           &global_matchlist, rootMenu, NULL, NULL, FALSE,
                           selection, greyed_context, inLine);
        PerformInsertionsOnMatchList(&client_matchlist);
 ...
 *
 *************************************<->***********************************/
static Boolean RemoveClientCommandFromMenuSpec(MenuSpec *menuSpec, CARD32 id) {
 ...
 *
 *************************************<->***********************************/
static Boolean ModifyClientCommandForMenuSpec(MenuSpec *menuSpec, CARD32 id,
                                              CmdModifier modifier,
                                              Context context, String newname) {
                ...
                if ((curMenuItem->clientCommandID == id) &&
                    (curMenuItem->wmFunction == F_InvokeCommand)) {
                        ...
                        case ENABLE:
                                ...
                                /* Adjust the pushbutton to the state it would
                                   have had given the last posting context. */
                                if (menuSpec->currentContext &
                                    curMenuItem->greyedContext)
                                        XtSetSensitive(menuSpec->menuButtons[i]
                                                           .buttonWidget,
                                                       FALSE);
                                else
                                        XtSetSensitive(menuSpec->menuButtons[i]
                        ...
                        case RENAME:
                                if (newname != NULL && *newname != '\0') {
                                        /* When renaming a command, we shouldn't
                                         * cause the entire menu to be
                                         * recreated.  Recreating the menu will
                                         * cause problems with tearoffs since
                                         * the menu will disappear when it is
                                         * destroyed. CR 9719
                                         */
                                        ...
                                        curMenuItem->label =
                                            XtNewString(newname);
                                        was_changed =
                                            False; /*  all taken care of here.
                                                    */

                                        /* This is needed when changing the
                                         * label since mwm will wait for a
                                         * geometry reply from itself which it
                                         * can never service. CR 9719
                                         */
                                        XtVaSetValues(
                                            XtParent(XtParent(
                                                menuSpec->menuButtons[i]
                                                    .buttonWidget)),
                                            XmNuseAsyncGeometry, TRUE, NULL);

                                        labelString = XmStringGenerate(
                                            curMenuItem->label,
                                            XmFONTLIST_DEFAULT_TAG,
                                            XmCHARSET_TEXT, NULL);
                                        XtVaSetValues(menuSpec->menuButtons[i]
                                                          .buttonWidget,
                                                      XmNlabelString,
                                                      labelString, NULL);
 ...
 *
 *************************************<->***********************************/
static void ModifyClientCommandID(WmScreenData *pSD, ClientData *pCD,
                                  OpRange range, CARD32 id,
                ...
                for (curClient = pSD->clientList;
                     ...
                     curClient = curClient->nextSibling) {
                        for (curMenuSpec = curClient->pCD->systemMenuSpec;
                             ...
                             curMenuSpec = curMenuSpec->nextMenuSpec) {
                                /* If the menu spec is global then stop
                                   searching for this client. */
                                if (curMenuSpec->clientLocal == FALSE)
                                        break;
                                if (ModifyClientCommandForMenuSpec(
                                        curMenuSpec, id, modifier, context,
                                        newname) == TRUE) {
                ...
                for (curMenuSpec = pSD->menuSpecs;
                     ...
                     curMenuSpec = curMenuSpec->nextMenuSpec) {
                        if (ModifyClientCommandForMenuSpec(curMenuSpec, id,
                                                           ...
                                                           newname) == TRUE) {
                                DestroyMenuSpecWidgets(curMenuSpec);
                                curMenuSpec->menuWidget = CreateMenuWidget(
                                    pSD, NULL, curMenuSpec->name,
                                    pSD->screenTopLevelW, TRUE, curMenuSpec,
                                    NULL);
                        }
                }
                break;
        case ROOT: {
                /*
                 * This section was changed to search the entire global menu
                 * list. This was done to allow access to menu entries included
                 * using the cci/.mwmrc interface.  Before, only the actual root
                 * menu could be modified; however, the user could still include
                 * commands in other menus specified in the .mwmrc file using
                 * the f.cci command.
                 */
                ...
                for (curMenuSpec = pCD->systemMenuSpec;
                     ...
                     curMenuSpec = curMenuSpec->nextMenuSpec) {
                        if (curMenuSpec->clientLocal == FALSE) {
                                ...
                                /* Find the last global menuspec in the clients
                                   list that needs to be changed and return it.
                                   Replace all menuspecs between the current one
                                   and the "last" one that needs changing. All
                                   the replaced menuspecs will be marked as
                                   local, so that next time clientLocal is FALSE
                                   in the enclosing for loop above, there will
                                   be no global menu specs needing changes. In
                                   other words, all the required menu spec
                                   replacing will occur the first time we find a
                                   global menu spec. */
                                last =
                                    FindLastMenuSpecToModify(curMenuSpec, id);
                                if (last != (MenuSpec *)NULL) {
                                        MenuSpec *newMenuSpec =
                                            (MenuSpec *)NULL;
                                        MenuSpec *firstMenuSpec =
                                            (MenuSpec *)NULL;
                                        MenuSpec *lastMenuSpec =
                                            (MenuSpec *)NULL;

                                        /* Replace all the global menu specs
                                           with local ones. */
                                        for (cur = curMenuSpec;
                                             cur != (MenuSpec *)NULL &&
                                             cur != last->nextMenuSpec;
                                             cur = cur->nextMenuSpec) {
                                                newMenuSpec =
                                                    ReplaceMenuSpecForClient(
                                                        cur, pCD);
                                                if (cur == curMenuSpec)
                                                        curMenuSpec =
                                                            firstMenuSpec =
                                        ...
                                        /* Now that we have replaced all the
                                           menu specs, recreate all the widgets
                                           for the new menu specs. */
                                        for (cur = firstMenuSpec;
                                             cur != (MenuSpec *)NULL &&
                                             cur != lastMenuSpec->nextMenuSpec;
                                             cur = cur->nextMenuSpec) {
                                                DestroyMenuSpecWidgets(
                                                    newMenuSpec);
 ...
 *
 *************************************<->***********************************/
void ModifyClientCommandTree(WmScreenData *pSD, ClientData *pCD, OpRange range,
                             CmdTree *tree, CmdModifier modifier,
                             Context context, String newname) {
        CmdTree *curTree;
        CARD32 cmdID;

        /* Run through the top level of the tree. */
        for (curTree = tree; curTree != (CmdTree *)NULL;
             curTree = curTree->next) {
                cmdID = curTree->commandID;
                ModifyClientCommandID(pSD, pCD, range, cmdID, modifier, context,
                                      newname);
                if (curTree->subTrees != (CmdTree *)NULL)
                        ModifyClientCommandTree(pSD, pCD, range,
                                                curTree->subTrees, modifier,
                                                context, newname);
        }
}
#endif /* !defined(WSM) || defined(MWM_QATS_PROTOCOL) */
...
static Boolean AdjustPBs(MenuSpec *menuSpec, ClientData *pCD,
        ...
        for (n = 0, menuButton = menuSpec->menuButtons;
             n < menuSpec->menuButtonCount; n++, menuButton++) {
                ...
                /* f.send_msg button:  set according to context and message. */
                {
                        ...
                        /*
                         * Have a context sensitive f.send_msg item and a client
                         * with a nonempty message list.  Set sensitive only if
                         * the message is supported by this client.  Otherwise
                         * set insensitive.
                         */
                        {
                                ...
                                XtSetSensitive(menuButton->buttonWidget,
                                               fSupported);
                        }
                } else
                /*
                 ...
                 */
                {
                        ...
                        if (menuItem->wmFunction == F_Remove) {
                                /*
                                 * Only allow remove from workspace if the
                                 * client is in more than one workspace
                                 */
                        ...
                        /* PushButton might not apply */
                        {
                                ...
                                if ((pCD->clientFunctions & menuItem->mgtMask &
                                     MWM_MGT_MASK) ||
                                    (pCD->dtwmFunctions & menuItem->mgtMask &
                                     DTWM_MGT_MASK))
#else  /* WSM */
                                if (pCD->clientFunctions & menuItem->mgtMask)
#endif /* WSM */
...
static Boolean SavePBInfo(MenuSpec *topMenuSpec, MenuItem *menuItem,
        ...
        /* need to create array */
        {
                ...
                topMenuSpec->menuButtons = (MenuButton *)XtMalloc(
                    MENU_BUTTON_INC * sizeof(MenuButton));
 ...
 *
 *************************************<->***********************************/
static Boolean AdjustTearOffControl(Widget cascade, XtPointer closure,
                                    XtPointer cbackdata) {
        ...
        argn = 0;
        XtSetArg(args[argn], XmNtearOffModel, &tearoff_model);
        ++argn;
        ...
        if ((tearoff_model == XmTEAR_OFF_ENABLED) && !isMwmMenu) {
                ...
                argn = 0;
                XtSetArg(args[argn], XmNtearOffModel, XmTEAR_OFF_DISABLED);
                ++argn;
        ...
        if (cbackdata != (XtPointer)NULL &&
            wmGD.menuActive == (MenuSpec *)NULL) {
                ...
                if (wmGD.menuActive == (MenuSpec *)NULL) {
                        ...
                        /* Add a callback that will clear menuActive when this
                           cascade is unmapped. */
#if 0
...
static Boolean CreateClientCommandSeparator(MenuItem *menuItem,
        ...
        if (menuItem->labelType == TOP_SEPARATOR) {
                /* If we find a real menu item (not just a template) before we
                   find a bottom separator, then create the separator. */
                for (curMenuItem = menuItem; curMenuItem != (MenuItem *)NULL;
                     curMenuItem = curMenuItem->nextMenuItem) {
                        /* If we found the closing separator, then return FALSE
                           and our new menu item position. */
                ...
                /* If by some bizarre chance we get to the end of the list
                   without finding either, then return FALSE. Something is
                   wrong. */
                if (curMenuItem == (MenuItem *)NULL)
                        return (FALSE);
        }
...
Widget CreateMenuWidget(WmScreenData *pSD,
                        ...
                        ClientData *pCD,
#endif /* !defined(WSM) || defined(MWM_QATS_PROTOCOL) */
                        String menuName, Widget parent, Boolean fTopLevelPane,
                        MenuSpec *topMenuSpec, MenuItem *moreMenuItems) {
        int i, n;
        Arg sepArgs[1];
        Arg args[10];
         ...
         *  be custom).  A submenu specification must be found and might not
         * exist. Return NULL if a submenu specification is not found.
         */
        if (fTopLevelPane) {
        ...
        {
                MWarning(((char *)GETMESSAGE(
                             48, 4, "Menu specification %s not found\n")),
                         menuName);
                return (NULL);
        }
        ...
        {
                MWarning(((char *)GETMESSAGE(
                             48, 5, "Menu recursion detected for %s\n")),
                         menuName);
                return (NULL);
        }
        ...
        if (fTopLevelPane) {
                ...
                i++;
                menuW = XtCreateWidget(menuName, xmRowColumnWidgetClass,
                                       menuShellW, (ArgList)args, i);
        ...
        while (menuItem) {
                ...
                {
#if ((!defined(WSM)) || defined(MWM_QATS_PROTOCOL))
                        /* If it is a client command, then we only want to
                         * create the separator under particular circumstances.
                         * Specifically, we want to make sure that:
                         *   1. a separator doesn't directly precede this one
                         *   2. a separator doesn't directly follow this one
                         *   3. this separator won't be the first or last item
                         * in the menu
                         */
                        if (labelIsClientCommand) {
                                if (CreateClientCommandSeparator(
                                        menuItem, n,
                                        (n > 0 ? children[n - 1]
                                               : (Widget)NULL),
                                        &menuItem)) {
                                        /* Increment the counter here because we
                                           only increment at the end of the loop
                                           if the item is not a client command
                                           item (i.e. labelIsClientCommand ==
                                           FALSE) */
                                        children[n++] = XmCreateSeparatorGadget(
                                            menuW, SEPARATOR_NAME,
                                            (ArgList)NULL, 0);
                                        fUseTitleSep = FALSE;
                                }
                        } else
                        ...
                        {
                                children[n] = XmCreateSeparatorGadget(
                                    menuW, SEPARATOR_NAME, (ArgList)NULL, 0);
                                fUseTitleSep = FALSE;
                        }
                } /* F_Separator */
                 ...
                 */
                {
                        ...
                        if ((menuItem->labelType == XmPIXMAP) &&
                            (labelPixmap = MakeCachedLabelPixmap(
                                 pSD, menuW, menuItem->labelBitmapIndex))) {
                                XtSetArg(args[i], XmNlabelType,
                                         (XtArgVal)XmPIXMAP);
                                i++;
                                XtSetArg(args[i], XmNlabelPixmap,
                                         (XtArgVal)labelPixmap);
                                i++;
                         ...
                         * Don't include the top one if the title is the first
                         * pane item or immediately follows a user-supplied
                         * separator.
                         */
                        {
                                if (fUseTitleSep) {
                                        children[n] = XmCreateSeparatorGadget(
                                            menuW, SEPARATOR_NAME, sepArgs, 1);
                                        n++;
                                }

                                XtSetArg(args[i], XmNalignment,
                                         XmALIGNMENT_CENTER);
                                i++;
                                children[n] = XmCreateLabelGadget(
                                    menuW, TITLE_NAME, (ArgList)args, i);
                                n++;
                                children[n] = XmCreateSeparatorGadget(
                                    menuW, SEPARATOR_NAME, sepArgs, 1);
                         ...
                         *   PushButton
                         *
                         * Both support mnemonics; only PushButtons support
                         * accelerators.
                         */
                        {
                                /*
                                ...
                                if (menuItem->mnemonic) {
                                        XtSetArg(args[i], XmNmnemonic,
                                                 (XtArgVal)menuItem->mnemonic);
                                        i++;
                                }

                                if (menuItem->wmFunction == F_Menu)
                                /*
                                 * Create a PopupShell and PulldownMenu for a
                                 * submenu (the menushells are linked together).
                                 * Create a CascadeButton Widget
                                 * The submenu widget is attached to the
                                 * CascadeButton gadget using the subMenuId
                                 * resource. Make the CascadeButton
                                 * insensitive if the submenu cannot be created.
                                 */
                                {
                                        subMenuW = CREATE_MENU_WIDGET(
                                            pSD, pCD, menuItem->wmFuncArgs,
                                            menuW, FALSE, topMenuSpec,
                                            (MenuItem *)NULL);
                                        if (subMenuW)
                                        /*
                                         ...
                                         */
                                        {
                                                XtSetArg(args[i], XmNsubMenuId,
                                                         (XtArgVal)subMenuW);
                                                i++;
                                                children[n] =
                                                    XmCreateCascadeButtonGadget(
                                                        menuW, CASCADE_BTN_NAME,
                                                        (ArgList)args, i);
#if ((!defined(WSM)) || defined(MWM_QATS_PROTOCOL))
                                                XtAddCallback(
                                                    children[n],
                                                    ...
                                                    (XtCallbackProc)
                                                        AdjustTearOffControl,
                                                    (XtPointer)subMenuW);
#endif /* !defined(WSM) || defined(MWM_QATS_PROTOCOL) */
                                        } else
                                        /*
                                         * Unable to create submenupane: make
                                         * the entry insensitive.
                                         */
                                        {
                                                children[n] =
                                                    XmCreateCascadeButtonGadget(
                                                        menuW, CASCADE_BTN_NAME,
                                                        (ArgList)args, i);
                                                XtSetSensitive(children[n],
                                                               FALSE);
                                        }

                                        /*
                                         * A following title will have both
                                         * separators.
                                         */
                                ...
                                {
                                        /*
                                         * If an accelerator is specified, set
                                         * acceleratorText, then create an
                                         * accelerator KeySpec and insert it at
                                         * the head of the toplevel MenuSpec's
                                         * list.
                                         */
                                        if (menuItem->accelText) {
                                                XtSetArg(
                                                    args[i], XmNacceleratorText,
                                                    (XtArgVal)(
                                                        tmpstr =
                                                ...
                                                accelKeySpec->state =
                                                    menuItem->accelState;
                                                accelKeySpec->keycode =
                                                    menuItem->accelKeyCode;
                                                accelKeySpec->context =
                                                    topMenuSpec->accelContext;
                                                accelKeySpec->subContext = 0;
                                                accelKeySpec->wmFunction =
                                                    menuItem->wmFunction;
                                                accelKeySpec->wmFuncArgs =
                                                    menuItem->wmFuncArgs;
                                                accelKeySpec->nextKeySpec =
                                                    topMenuSpec->accelKeySpecs;
                                                topMenuSpec->accelKeySpecs =
                                                    accelKeySpec;
                                        }

                                        children[n] = XmCreatePushButtonGadget(
                                            menuW, PUSH_BTN_NAME, (ArgList)args,
                                            i);

                                        /*
                                         * Set sensitivity.  Initially we only
                                         * consider the context of the top level
                                         * menupane.
                                         */

                                        if (menuItem->greyedContext &
                                            topMenuSpec->currentContext)
                                        ...
                                        /*
                                         * If necessary, fill a menuButtons
                                         * element for this PushButton.  Malloc
                                         * or Realloc the array if necessary.
                                         */
                                        if ((menuItem->greyedContext) ||
                                            (menuItem->mgtMask)) {
                                                if (!SavePBInfo(topMenuSpec,
                                                                menuItem,
                                                                children[n])) {
                                                        MWarning(
                                                            ((char *)GETMESSAGE(
                                        ...
                                        XtAddCallback(
                                            children[n], XmNactivateCallback,
                ...
                if (n >=
                    CHILDREN_CACHE - 2) /* leave room for title separators */
                {
...
void PostMenu(MenuSpec *menuSpec, ClientData *pCD, int x, int y,
        ...
        {
                GetSystemMenuPosition(pCD, &x, &y, menuSpec->height,
                                      newContext);
        }
...
void FreeCustomMenuSpec(MenuSpec *menuSpec) {
        ...
        /*
         * Fix for CR 5450 - If the custom menu is the same as wmGD.menuActive,
         * call the UnmapCallback directly to clean things up.  Since the menu
         * is going to be destroyed, this callback will not
         * get called, leaving MWM in a failure state.
         */
