...
#ifndef lint
static char rcsid[] =
    "$XConsortium: WmResCvt.c /main/4 1995/11/01 11:49:54 rswiston $"
#endif
#endif
/*
...
#define AMAP_BEHAVIOR_IGNORE_STR (unsigned char *)"ignore"
        ...
        if (*pch && NextToken(pch, &len, &pchNext)) {
                if ((*pch == 'A') || (*pch == 'a')) {
                        ...
                        if (StringsAreEqual(pch, AMAP_BEHAVIOR_IGNORE_STR,
                                            len)) {
...
#define CMAP_FOCUS_POINTER_STR (unsigned char *)"pointer"
        ...
        if (*pch && NextToken(pch, &len, &pchNext)) {
                if ((*pch == 'E') || (*pch == 'e')) {
                        if (StringsAreEqual(pch, CMAP_FOCUS_EXPLICIT_STR,
                                            len)) {
                        ...
                        if (StringsAreEqual(pch, CMAP_FOCUS_KEYBOARD_STR,
                                            len)) {
        ...
        while (*pch && NextToken(pch, &len, &pchNext)) {
                ...
                if (*pch == '+') {
                        ...
                        if (StringsAreEqual(pch, WM_DECOR_TITLE_STR, len)) {
                                ...
                                fHit = True;
                        } else if (StringsAreEqual(pch, WM_DECOR_TITLEBAR_STR,
                                                   len)) {
                        ...
                        if (StringsAreEqual(pch, WM_DECOR_MINIMIZE_STR, len)) {
                                ...
                                fHit = True;
                        } else if (StringsAreEqual(pch, WM_DECOR_MAXIMIZE_STR,
                                                   len)) {
                                cval = fAddNext ? (cval | WM_DECOR_MAXIMIZE)
                                                : (cval & ~MWM_DECOR_MAXIMIZE);
                                fHit = True;
                        } else if (StringsAreEqual(pch, WM_DECOR_MENU_STR,
                                                   len)) {
        ...
        while (*pch && NextToken(pch, &len, &pchNext)) {
                ...
                if (*pch == '+') {
                        ...
                        if (StringsAreEqual(pch, WM_FUNC_MINIMIZE_STR, len)) {
                                ...
                                fHit = True;
                        } else if (StringsAreEqual(pch, WM_FUNC_MAXIMIZE_STR,
                                                   len)) {
                                cval = fAddNext ? (cval | MWM_FUNC_MAXIMIZE)
                                                : (cval & ~MWM_FUNC_MAXIMIZE);
                                fHit = True;
                        } else if (StringsAreEqual(pch, WM_FUNC_MOVE_STR,
                                                   len)) {
...
#define FRAME_STYLE_SLAB_STR (unsigned char *)"slab"
        ...
        if (*pch && NextToken(pch, &len, &pchNext)) {
                if ((*pch == 'R') || (*pch == 'r')) {
                        if (StringsAreEqual(pch, FRAME_STYLE_RECESSED_STR,
                                            len)) {
...
#define ICON_DECOR_ACTIVE_LABEL_STR (unsigned char *)"activelabel"
        ...
        while (*pch && NextToken(pch, &len, &pchNext)) {
                if ((*pch == 'A') || (*pch == 'a')) {
                        if (StringsAreEqual(pch, ICON_DECOR_ACTIVE_LABEL_STR,
                                            len)) {
...
#define ICON_PLACE_TIGHT_STR (unsigned char *)"tight"
        ...
        while (*pch && NextToken(pch, &len, &pchNext)) {
                if ((*pch == 'B') || (*pch == 'b')) {
                        ...
                        else if (StringsAreEqual(pch, ICON_PLACE_TIGHT_STR,
                                                 len)) {
...
#define KEYBOARD_FOCUS_POINTER_STR (unsigned char *)"pointer"
        ...
        if (*pch && NextToken(pch, &len, &pchNext)) {
                if ((*pch == 'E') || (*pch == 'e')) {
                        if (StringsAreEqual(pch, KEYBOARD_FOCUS_EXPLICIT_STR,
                                            len)) {
                        ...
                        if (StringsAreEqual(pch, KEYBOARD_FOCUS_POINTER_STR,
                                            len)) {
...
#define HORIZONTAL_STR (unsigned char *)"horizontal"
        ...
        if (*pch) {
                ...
                if (!((cval.width == 0) && (pchNext == pch))) {
                        ...
                        if (*pch && NextToken(pch, &len, &pchNext)) {
                                if ((*pch == 'V') || (*pch == 'v')) {
                                        if (StringsAreEqual(pch, VERTICAL_STR,
                                                            len)) {
                                                cval.height = BIGSIZE;
                                        }
                                } else if ((*pch == 'H') || (*pch == 'h')) {
                                        if (StringsAreEqual(pch, HORIZONTAL_STR,
                                                            len)) {
                                                cval.width = BIGSIZE;
                                        }
                                }
        ...
        while (*pch && NextToken(pch, &len, &pchNext)) {
                ...
                if ((*pch == 'A') || (*pch == 'a')) {
                        ...
                        if (StringsAreEqual(pch, SHOW_FB_RESIZE_STR, len)) {
                                ...
                                fHit = True;
                        } else if (StringsAreEqual(pch, SHOW_FB_RESTART_STR,
                                                   len)) {
...
#define USE_PPOSITION_OFF_STR (unsigned char *)"off"
        ...
        if (*pch && NextToken(pch, &len, &pchNext)) {
                if ((*pch == 'N') || (*pch == 'n')) {
                        if (StringsAreEqual(pch, USE_PPOSITION_NONZERO_STR,
                                            len)) {
...
unsigned char *NextToken(unsigned char *pchIn, int *pLen,
        ...
        for (i = 0; ((chlen = mblen((char *)pchIn, MB_CUR_MAX)) > 0) &&
                    (pchIn[0] != '\0');
             i++)
