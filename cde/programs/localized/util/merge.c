...
char *argv[];
{
        ...
        if (pfile == NULL)
                fatal("Usage: merge [-lang language][-dfile Default.tmsg] "
                      "Primary.tmsg < File.nls > File \n",
                      0, 9);

        if (lang != NULL) {
                setlocale(LC_ALL, lang);
                /* LC_CTYPE need to be set to make "gencat" command work
                 * correctly */
        ...
        while (c != EOF) {
                while (c == '%') {
                        ...
                        /* if the string matchs "%|nls-", go to
                         * process_message(). */
                        if (*s == 0) {
                                process_message();
                                c = get_char();
        ...
        while (c = get_char(), c != '-')
                ...
                case '\n':
                        fatal("Unterminated NLS sequence.\n", crt_line[2] - 1,
                              2);
                default:
                        fatal("Bad character in NLS sequence.\n", crt_line[2],
                              2);
                case '0':
                        m = m * 10 + 0;
                        break;
 ...
 */
int find_message(msg) int msg; /* message number to be searched */
{
        int ret = 0;
 ...
 */
int find_msg_in_file(msg, file) int msg; /* message number to be searched */
...
/*                                9: N/A                  */
{
        ...
        case 0:
                fprintf(stderr,
                        "           [Line %d in Primary message file]\n", line);
                break;
        case 1:
                fprintf(stderr,
                        "           [Line %d in Default message file]\n", line);
                break;
        case 2:
 ...
 */
void get_option(argc, argv) int *argc;
char *argv[];
{
        int i;
