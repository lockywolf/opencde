...
int main(int argc, char *argv[]) {
        ...
        if (argc < 3) {
                fprintf(stderr,
                        "mkcatdefs: Usage: %s header_file msg_file "
                        "[msg_file...] [-h]\n",
                        argv[0]);
                exit(0);
        }
        ...
        if (inclfile) {
                ...
                if ((int)strlen((t = strrchr(mname, '/')) ? t + 1 : mname) >
                    MDIRSIZ) {
                        fprintf(stderr,
                                "mkcatdefs: header file name too long\n");
                        exit(1);
                }
                ...
                if ((outfp = fopen(outname, "w")) == NULL) {
                        ...
                        for (cp = mname; *cp; cp += i) {
                                ...
                                if (i < 0) {
                                        fprintf(stderr, "mkcatdefs: filename "
                                                        "contains invalid "
                                                        "character\n");
                                        exit(1);
                                }
        ...
        for (i = 2; i < count; i++) {
                ...
                if (strcmp(inname, "-") == 0) {
                        strcpy(inname, "stdin");
                        descfile =
                            stdin; /* input from stdin if no source files */
                        mkcatdefs(inname);
                } else {
        ...
        if (errflg) {
                ...
                if (inclfile) {
                        if (symbflg)
                                ...
                                fprintf(stderr,
                                        "mkcatdefs: No symbolic identifiers; "
                                        "no %s created\n",
                                        outname);
                                unlink(outname);
                        }
 ...
 * RETURNS: None
 */
static void mkcatdefs(char *fname)
 ...
 *          1 - continuation line.
 */
static int chkcontin(char *line) {
        ...
        for (wc = 0; *line; line += len) {
                ...
                if (len < 0) {
                        fprintf(stderr,
                                "mkcatdefs: sourcefile contains invalid "
                                "character:\n\t%s",
                                line);
                        errflg = 1;
                        return (0);
...
static int insert(char *tname, int seqno)
/*
...
static int hash(char *name) /* pointer to symbol */
{
