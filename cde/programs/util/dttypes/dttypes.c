...
static enum { r_info, r_name, f_name, f_value } list_shift;
...
static int init(int *argc, char **argv) {
        ...
        toplevel = XtInitialize(argv[0], "Dttype", NULL, 0, (int *)argc, argv);
        ...
        if (DtInitialize(XtDisplay(toplevel), toplevel, argv[0], "Dttype") ==
            False) {
...
void add_rec(int rec, List *l) {
        int i;

        if (l->rec_total == 0 || l->rec_count >= l->rec_total - 2) {
                l->rec_total += 10;
                l->rec_list =
                    (int *)realloc(l->rec_list, (l->rec_total) * sizeof(int));
        }
...
void rec_list(List *l) {
...
#else
        if ((compile(l->rec_name ? l->rec_name : "^.*", expbuf_rn,
                     &expbuf_rn[sizeof(expbuf_rn)], 0)) == (char *)0)
#endif
        {
                /* error */
                fprintf(
                    stderr,
                    catgets(dtcatd, 1, 36, "error in regular expression %s\n"),
...
#else
        if ((compile(l->fld_name ? l->fld_name : "^.*", expbuf_fn,
                     &expbuf_fn[sizeof(expbuf_fn)], 0)) == (char *)0)
#endif
        {
                /* error */
                fprintf(
...
#else
        if ((compile(l->fld_value ? l->fld_value : "^.*", expbuf_fv,
                     &expbuf_fv[sizeof(expbuf_fv)], 0)) == (char *)0)
#endif
        {
                /* error */
                fprintf(
...
#else
        if ((compile(l->display_fld ? l->display_fld : "^.*", expbuf_df,
                     &expbuf_df[sizeof(expbuf_df)], 0)) == (char *)0)
#endif
        {
                /* error */
                fprintf(
        ...
        for (rec = 0; rec < db_ptr->recordCount; rec++) {
                ...
                if (regexec(&regex_rn,
                            ...
                            0, NULL, 0) == 0)
#else
                if (advance((char *)_DtDtsMMBosonToString(rec_ptr->recordName),
                            expbuf_rn) != 0)
#endif
                {
                        for (fld = 0; fld < rec_ptr->fieldCount; fld++) {
                                ...
                                fn = _DtDtsMMExpandValue(
                                    _DtDtsMMBosonToString(fld_ptr->fieldName));
                                fv = _DtDtsMMExpandValue(
        ...
        for (i = 0; l->rec_list && l->rec_list[i] != -1; i++) {
                ...
                if (l->display_list & (1 << r_info)) {
                        char *path =
                            (char *)_DtDtsMMBosonToString(rec_ptr->pathId);
...
int parse_args(List *l, int argc, char **argv) {
        int *rl;
        int df = 0;
        int i;
        int error = 0;
        enum st { none, where, list };
        enum st state = none;
        ...
        for (i = 1; i < argc; i++) {
                if (strcmp(argv[i], catgets(dtcatd, 1, 16, "-db")) == 0) {
                        ...
                        state = list;
                                  ...
                                  catgets(dtcatd, 1, 19, "rec_info")) == 0) {
                        if (state == where) {
                                ...
                                error |= 1 << NO_STATE;
                                  ...
                                  catgets(dtcatd, 1, 20, "rec_name")) == 0) {
                        if (state == where) {
                                if (!argv[i + 1]) {
                                        error |= 1 << NO_ARGUMENT;
                                } else {
                                ...
                                error |= 1 << NO_STATE;
                                  ...
                                  catgets(dtcatd, 1, 21, "fld_name")) == 0) {
                        if (state == where) {
                                if (!argv[i + 1]) {
                                        error |= 1 << NO_ARGUMENT;
                                } else {
                                ...
                                error |= 1 << NO_STATE;
                                  ...
                                  catgets(dtcatd, 1, 22, "fld_value")) == 0) {
                        if (state == where) {
                                if (!argv[i + 1]) {
                                        error |= 1 << NO_ARGUMENT;
                                } else {
                ...
                } else if (strcmp(argv[i], catgets(dtcatd, 1, 23, "all")) ==
                           0) {
                        if (state == where) {
                                if (!argv[i + 1]) {
                                        error |= 1 << NO_ARGUMENT;
                ...
                } else if (strcmp(argv[i], catgets(dtcatd, 1, 26, "-type")) ==
                           0) {
                        char *type, *new;
                        ...
                        i++;
                } else if (strcmp(argv[i], catgets(dtcatd, 1, 35, "-help")) ==
                           0) {
                        usage();
                } else {
                        error |= 1 << INVALID_ARG;
...
usage() {
        fprintf(stderr, "%s",
                catgets(dtcatd, 1, 25,
                        "usage:  dttypes [-help]\n\tdttypes [-type "
                        "filename]\n\tdttypes [-db database] [-w [rec_name "
                        "regexp] [fld_name regexp]\n\t\t\t[fld_value "
                        "regexp]]\n\t\t[-l [rec_name] [rec_info] [fld_name "
                        "regexp] [fld_value]]\n"));
        exit(1);
}
