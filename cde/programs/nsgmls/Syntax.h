...
class SP_API Syntax : public Resource, public EntityCatalog::Syntax {
      public:
        ...
        enum StandardFunction { fRE, fRS, fSPACE };
        enum FunctionClass { cFUNCHAR, cSEPCHAR, cMSOCHAR, cMSICHAR, cMSSCHAR };
        enum Set {
                nameStart,
                digit,
      ...
      private:
        ...
        void checkUnivControlChar(
            UnivChar univChar, const CharsetInfo &docCharset,
            const /* ::SP_NAMESPACE_SCOPE */ Syntax *otherSyntax,
            ISet<WideChar> &invalid) const;
...
inline Number Syntax::quantity(Quantity q) const { return quantity_[q]; }

inline void Syntax::setQuantity(int i, Number n) { quantity_[i] = n; }
...
inline Boolean Syntax::hasShortrefs() const {
        return delimShortrefComplex_.size() > 0 ||
               !delimShortrefSimple_.isEmpty();
}
...
inline const ISet<Char> *Syntax::charSet(int i) const { return &set_[i]; }
...
inline size_t Syntax::attcnt() const { return quantity(qATTCNT); }

inline size_t Syntax::attsplen() const { return quantity(qATTSPLEN); }

inline size_t Syntax::namelen() const { return quantity(qNAMELEN); }

inline size_t Syntax::penamelen() const {
        return quantity(qNAMELEN) - delimGeneral(dPERO).size();
}

inline size_t Syntax::litlen() const { return quantity(qLITLEN); }

inline size_t Syntax::normsep() const { return quantity(qNORMSEP); }

inline size_t Syntax::dtemplen() const { return quantity(qDTEMPLEN); }

inline size_t Syntax::grpcnt() const { return quantity(qGRPCNT); }

inline size_t Syntax::grpgtcnt() const { return quantity(qGRPGTCNT); }

inline size_t Syntax::grplvl() const { return quantity(qGRPLVL); }

inline size_t Syntax::taglvl() const { return quantity(qTAGLVL); }

inline size_t Syntax::taglen() const { return quantity(qTAGLEN); }

inline size_t Syntax::entlvl() const { return quantity(qENTLVL); }

inline size_t Syntax::pilen() const { return quantity(qPILEN); }

inline Char Syntax::space() const { return standardFunction(fSPACE); }

inline void Syntax::setSgmlChar(const ISet<Char> &set) { set_[sgmlChar] = set; }

inline int Syntax::referenceQuantity(Quantity i) {
        return referenceQuantity_[i];
}

inline void Syntax::setShuncharControls() { shuncharControls_ = 1; }

inline const XcharMap<unsigned char> &Syntax::markupScanTable() const {
        return markupScanTable_;
}

inline Boolean Syntax::multicode() const { return multicode_; }

inline Boolean Syntax::namecaseGeneral() const { return namecaseGeneral_; }

inline Boolean Syntax::namecaseEntity() const { return namecaseEntity_; }
