...
struct SP_API StorageObjectSpec {
        ...
        enum Records { find, cr, lf, crlf, asis };
        Records records;
...
struct SP_API ParsedSystemIdMap {
        enum Type { catalogDocument, catalogPublic };
        Type type;
        StringC publicId;
};
      ...
      public:
        class SP_API CatalogManager {
              public:
                ...
                makeCatalog(StringC &systemId, const CharsetInfo &charset,
                            ExtendEntityManager *, Messenger &) const = 0;
        ...
        virtual void registerCodingSystem(const char *,
                                          const InputCodingSystem *) = 0;
        virtual void setCatalogManager(CatalogManager *) = 0;
        virtual InputSource *openIfExists(const StringC &sysid,
                                          const CharsetInfo &,
                                          InputSourceOrigin *,
                                          Boolean mayRewind, Messenger &) = 0;
        virtual Boolean expandSystemId(const StringC &, const Location &,
                                       Boolean isNdata, const CharsetInfo &,
                                       const StringC *mapCatalogPublic,
                                       Messenger &, StringC &) = 0;
        virtual Boolean mergeSystemIds(const Vector<StringC> &sysids,
                                       Boolean mapCatalogDocument,
                                       const CharsetInfo &, Messenger &mgr,
                                       StringC &) const = 0;
        virtual Boolean
        parseSystemId(const StringC &str, const CharsetInfo &idCharset,
                      Boolean isNdata, const StorageObjectSpec *defSpec,
                      Messenger &mgr, ParsedSystemId &parsedSysid) const = 0;
        static Boolean externalize(const ExternalInfo *, Offset,
