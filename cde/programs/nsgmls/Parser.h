...
class Parser : private ParserState {
      ...
      private:
        ...
        Boolean shortrefCanPreemptDelim(const StringC &sr, const StringC &d,
                                        Boolean dIsSr, const Syntax &);
        ...
        Boolean parseExternalId(const AllowedParams &, const AllowedParams &,
        ...
        Boolean parseLiteral(Mode litMode, Mode liteMode, size_t maxLength,
                             const MessageType1 &tooLongMessage, unsigned flags,
                             Text &text);

        Boolean parseGroupToken(const AllowedGroupTokens &allow,
                                unsigned nestingLevel, unsigned declInputLevel,
                                unsigned groupInputLevel, GroupToken &gt);
        ...
        Boolean parseGroup(const AllowedGroupTokens &allowToken,
                           unsigned declInputLevel, Param &parm);
        ...
        Boolean parseDataTagGroup(unsigned nestingLevel,
        ...
        Boolean parseIndicatedReservedName(const AllowedParams &allow,
                                           Param &parm);
        ...
        Boolean parseAttributeSpec(Boolean inDeclaration, AttributeList &,
                                   Boolean &netEnabling);
        Boolean handleAttributeNameToken(Text &text, AttributeList &,
                                         unsigned &specLength);
        struct AttributeParameter {
                enum Type { end, name, nameToken, vi, recoverUnquoted };
        };

        Boolean parseAttributeParameter(Boolean inDecl, Boolean allowVi,
                                        AttributeParameter::Type &result,
                                        Boolean &netEnabling);
        void extendUnquotedAttributeValue();

        Boolean parseAttributeValueSpec(Boolean inDecl, const StringC &name,
                                        AttributeList &atts,
                                        unsigned &specLength);
        ...
        Boolean parseExternalEntity(StringC &name, Entity::DeclType declType,
                                    unsigned declInputLevel, Param &parm);
        ...
        Boolean sdParseCharset(SdBuilder &sdBuilder, SdParam &parm,
                               Boolean isDocument, CharsetDecl &,
                               UnivCharsetDesc &);
        ...
        Boolean translateSyntax(SdBuilder &sdBuilder, WideChar syntaxChar,
                                Char &docChar);
        ...
        Boolean translateSyntaxNoSwitch(SdBuilder &sdBuilder,
                                        WideChar syntaxChar, Char &docChar,
                                        Number &count);
        Boolean translateName(SdBuilder &sdBuilder, const StringC &name,
                              StringC &str);
        void translateRange(SdBuilder &sdBuilder, SyntaxChar start,
                            SyntaxChar end, ISet<Char> &chars);
        UnivChar translateUniv(UnivChar univChar, CharSwitcher &switcher,
                               const CharsetInfo &syntaxCharset);
        ...
        void addLinkRule(LinkSet *linkSet, const ElementType *sourceElement,
                         const ConstPtr<SourceLinkRuleResource> &linkRule);
        Boolean parseResultElementSpec(unsigned declInputLevel, Param &parm,
                                       Boolean idlink, Boolean &implied,
        ...
        void findMissingTag(const ElementType *e,
