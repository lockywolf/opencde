...
class SP_API StorageManager {
      public:
        StorageManager();
        virtual StorageObject *
        makeStorageObject(const StringC &specId, const StringC &baseId,
                          Boolean search, Boolean mayRewind, Messenger &mgr,
                          StringC &actualId) = 0;
        virtual const char *type() const = 0;
        virtual Boolean inheritable() const;
        virtual Boolean transformNeutral(StringC &, Boolean fold,
                                         Messenger &) const;
        // Resolve a possibly relative ID by examining the base and specified
        // IDs. Put the resolved ID in specID. Return 0 if it cannot be resolved
        // yet becase the specified ID is relative and physical searching is
        // required to resolve it and search is true; in this case the base will
        // be passed to makeStorageObject. Otherwise return 1; in this case the
        // base will be discarded, and the resolved ID will be passed to
        // makeStorageObject.
        virtual Boolean resolveRelative(const StringC &base, StringC &specId,
