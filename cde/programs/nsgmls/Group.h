...
struct GroupConnector {
        enum Type { andGC, orGC, seqGC, grpcGC, dtgcGC };
        Type type;
};
...
inline Boolean AllowedGroupTokens::groupToken(GroupToken::Type i) const {
        return ((1 << i) & flags_) != 0;
}

inline GroupToken::Type AllowedGroupTokens::group() const {
...
inline GroupToken::Type AllowedGroupTokens::nameStart() const {
...
inline void AllowedGroupTokens::allow(GroupToken::Type t) {
        flags_ |= (1 << t);
}

inline Boolean
AllowedGroupConnectors::groupConnector(GroupConnector::Type c) const {
        return (flags_ & (1 << c)) != 0;
}

inline void AllowedGroupConnectors::allow(GroupConnector::Type c) {
        flags_ |= (1 << c);
}
