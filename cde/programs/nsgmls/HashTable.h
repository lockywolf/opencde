...
template <class K, class V> class HashTableItem : public HashTableItemBase<K> {
      public:
...
template <class K, class V> class HashTable {
      public:
        ...
        const V *lookup(const K &key) const {
                HashTableItem<K, V> *tem =
                    (HashTableItem<K, V> *)table_.lookup(key);
                return tem ? &tem->value : 0;
        }
        size_t count() const { return table_.count(); }

      private:
        CopyOwnerTable<HashTableItemBase<K>, K, Hash, HashTableKeyFunction<K> >
            table_;
...
template <class K, class V> class HashTableIter {
      ...
      private:
        OwnerTableIter<HashTableItemBase<K>, K, Hash, HashTableKeyFunction<K> >
            iter_;
};
