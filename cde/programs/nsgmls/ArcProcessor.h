...
class ArcProcessor : private ContentState, private AttributeContext {
      public:
        struct MetaMap {
                ...
                // list of indexes into element's attlist of architectural
                // attributes
        ...
        void init(const EndPrologEvent &, const ConstPtr<Sd> &,
                  const ConstPtr<Syntax> &, const SgmlParser *parser,
                  Messenger *, const Vector<StringC> &superName,
      ...
      private:
        ...
        const MetaMap &buildMetaMap(const ElementType *, const Notation *,
                                    const AttributeList &,
                                    const AttributeList *linkAtts,
                                    unsigned suppressFlags);
        void considerSupr(const AttributeList &atts,
                          ...
                          unsigned &newSuppressFlags, Boolean &inhibitCache,
        ...
        void considerIgnD(const AttributeList &atts,
                          ...
                          unsigned &arcSuprIndex);
        const Attributed *
        considerForm(const AttributeList &atts, const AttributeList *linkAtts,
                     const StringC &name, Boolean isNotation,
                     unsigned thisSuppressFlags, unsigned &newSuppressFlags,
                     Boolean &inhibitCache, unsigned &arcFormIndex);
        const Attributed *autoForm(const AttributeList &atts,
                                   const StringC &name, Boolean isNotation,
                                   unsigned thisSuppressFlags,
                                   unsigned &newSuppressFlags,
                                   Boolean &inhibitCache, unsigned &idIndex);
        const Text *considerNamer(const AttributeList &atts,
                                  Boolean &inhibitCache,
                                  unsigned &arcNamerIndex);
        void buildAttributeMapRename(MetaMap &map, const Text &rename,
                                     const AttributeList &atts,
                                     const AttributeList *linkAtts,
                                     Vector<PackedBoolean> &attRenamed);
        void buildAttributeMapRest(MetaMap &map, const AttributeList &atts,
                                   const AttributeList *linkAtts,
                                   const Vector<PackedBoolean> &attRenamed);
        Boolean matchName(const StringC &name, const char *key);
        void split(const Text &text, Char space, Vector<StringC> &tokens,
                   Vector<size_t> &tokenPos);
        Boolean mapAttributes(const AttributeList &from,
                              const AttributeList *fromLink,
                              const Text *content, AttributeList &to,
        ...
        void emitArcContent(const Text &text, EventHandler &handler,
                            Allocator &allocator);
