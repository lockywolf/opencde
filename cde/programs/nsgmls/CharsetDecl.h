...
class SP_API CharsetDeclRange {
      public:
        enum Type { number, string, unused };
        CharsetDeclRange();
        ...
        Boolean getCharInfo(WideChar fromChar, CharsetDeclRange::Type &type,
                            Number &n, StringC &str, Number &count) const;
...
class SP_API CharsetDeclSection {
      public:
        ...
        void rangeDeclared(WideChar min, Number count,
                           ISet<WideChar> &declared) const;
        void usedSet(ISet<Char> &) const;
        Boolean getCharInfo(WideChar fromChar, const PublicId *&id,
                            CharsetDeclRange::Type &type, Number &n,
                            StringC &str, Number &cout) const;
        void stringToChar(const StringC &str, ISet<WideChar> &to) const;
        void numberToChar(const PublicId *id, Number n, ISet<WideChar> &to,
                          Number &count) const;
...
class SP_API CharsetDecl {
      public:
        ...
        Boolean getCharInfo(WideChar fromChar, const PublicId *&id,
                            CharsetDeclRange::Type &type, Number &n,
                            StringC &str, Number &count) const;
        void stringToChar(const StringC &str, ISet<WideChar> &to) const;
        void numberToChar(const PublicId *id, Number n, ISet<WideChar> &to,
                          Number &count) const;
...
inline Boolean CharsetDecl::getCharInfo(WideChar fromChar, const PublicId *&id,
                                        CharsetDeclRange::Type &type, Number &n,
                                        StringC &str) const {
        Number tem;
...
inline void CharsetDecl::numberToChar(const PublicId *id, Number n,
                                      ISet<WideChar> &to) const {
        Number tem;
        numberToChar(id, n, to, tem);
}

inline void CharsetDecl::declaredSet(ISet<WideChar> &set) const {
        set = declaredSet_;
}

inline Boolean CharsetDecl::charDeclared(WideChar c) const {
