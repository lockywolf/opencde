...
struct NamedResourceKeyFunction {
        static inline const StringC &key(const NamedResource &p) {
                return p.name();
        }
};
...
template <class T> class NamedResourceTable {
#ifdef __lucid
        struct X {
...
template <class T> class NamedResourceTableIter {
      public:
        NamedResourceTableIter(const NamedResourceTable<T> &table)
            : iter_(table.table_) {}
        Ptr<T> next() { return (T *)iter_.next().pointer(); }
...
template <class T> class ConstNamedResourceTableIter {
      public:
        ConstNamedResourceTableIter(const NamedResourceTable<T> &table)
            : iter_(table.table_) {}
        ConstPtr<T> next() { return (T *)iter_.next().pointer(); }
        const T *nextTemp() { return (const T *)iter_.next().pointer(); }
