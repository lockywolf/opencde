...
class OutputState {
      public:
        OutputState();
        void init();
        void handleRe(EventHandler &, Allocator &, const EventsWanted &, Char,
                      const Location &);
        ...
        void noteStartElement(Boolean included, EventHandler &, Allocator &,
                              const EventsWanted &);
        void noteEndElement(Boolean included, EventHandler &, Allocator &,
                            const EventsWanted &);
...
inline Boolean OutputStateLevel::hasPendingRe() const {
        return int(state) >= int(OutputState::pendingAfterRsOrRe);
}

inline OutputStateLevel &OutputState::top() { return *stack_.head(); }
