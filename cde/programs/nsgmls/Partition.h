...
class Partition {
      public:
        Partition(const ISet<Char> &chars, const ISet<Char> **sets, int nSets,
...
inline EquivCode Partition::maxCode() const { return maxCode_; }

inline EquivCode Partition::charCode(Char c) const { return map_[c]; }

inline EquivCode Partition::eECode() const { return 0; }

inline const String<EquivCode> &Partition::setCodes(int i) const {
        return setCodes_[i];
}

inline const XcharMap<EquivCode> &Partition::map() const { return map_; }
