...
class Entity : public EntityDecl {
      ...
      private:
        virtual void normalReference(ParserState &, const Ptr<EntityOrigin> &,
...
class PiEntity : public InternalEntity {
      public:
        ...
        void litReference(Text &, ParserState &, const Ptr<EntityOrigin> &,
                          Boolean) const;
        void normalReference(ParserState &, const Ptr<EntityOrigin> &,
                             Boolean) const;
        void declReference(ParserState &, const Ptr<EntityOrigin> &) const;
        void rcdataReference(ParserState &, const Ptr<EntityOrigin> &) const;
        Entity *copy() const;
};
...
class InternalTextEntity : public InternalEntity {
      public:
        enum Bracketed { none, starttag, endtag, ms, md };
...
class ExternalNonTextEntity : public ExternalEntity {
      public:
        ExternalNonTextEntity(const StringC &, DataType, const Location &,
                              const ExternalId &);
        Boolean isDataOrSubdoc() const;
        void litReference(Text &, ParserState &, const Ptr<EntityOrigin> &,
                          Boolean) const;
...
class ExternalDataEntity : public ExternalNonTextEntity {
      public:
        ...
        Entity *copy() const;
        void contentReference(ParserState &, const Ptr<EntityOrigin> &) const;
...
class SP_API EntityOrigin : public InputSourceOrigin {
      public:
        ...
        EntityOrigin(const ConstPtr<Entity> &, const Location &refLocation);
        EntityOrigin(const ConstPtr<Entity> &, const Location &refLocation,
                     Index refLength, Owner<Markup> &markup);
        ~EntityOrigin();
...
inline Boolean Entity::used() const { return used_; }

inline void Entity::setUsed() { used_ = 1; }

inline Boolean Entity::defaulted() const { return defaulted_; }

inline void Entity::setDefaulted() { defaulted_ = 1; }

inline const StringC &InternalEntity::string() const { return text_.string(); }

inline const Text &InternalEntity::text() const { return text_; }

inline const ExternalId &ExternalEntity::externalId() const {
        return externalId_;
}

inline const AttributeList &ExternalDataEntity::attributes() const {
        return attributes_;
}

inline const Notation *ExternalDataEntity::notation() const {
        return notation_.pointer();
}

inline const Markup *EntityOrigin::markup() const { return markup_.pointer(); }
