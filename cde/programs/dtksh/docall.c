...
static int allprint(unsigned long *pargs, memtbl_t *tbls);
static pp_usage(void);
static int call_postprompt(char *argv0, unsigned long *pargs, memtbl_t *tbls,
                           int *freeit);
static long get_prdebug(void);
static long set_prdebug(long n);
static int myprompt(char *prompt);
...
int do_field_get(int argc, char **argv) {
        ...
        if (!type || !ptr || (parse_decl(argv[0], tbl, type, 1) == FAIL)) {
                if (!type || !ptr) {
                        ...
                        errmsg = strdup(GETMESSAGE(
                            4, 1,
                            "Cannot parse the structure named '%s'; it "
        ...
        if ((always_ptr || !IS_SIMPLE(tbl)) && !tbl->ptr &&
            !(tbl->flags & F_TYPE_IS_PTR))
                tbl->ptr = 1;
        else
                while (tbl->ptr > 1) {
                        ptr = *((void **)ptr);
        ...
        while ((i < argc) && (fld = argv[i++])) {
                ...
                if (!tbl2) {
                        ...
                        printerrf(argv[0], errmsg, fld, type, NULL, NULL, NULL,
                                  NULL, NULL, NULL);
                        free(errmsg);
                        fail = 1;
                        break;
...
static pp_usage(void) {
        ...
        errmsg = strdup(GETMESSAGE(
            4, 4, "Please enter p(rint), s(end), q(uit) or field=value\n"));
        printf(errmsg);
        free(errmsg);
}
        ...
        for (;;) {
                ...
                *buf = '\0';
                ...
                if (xk_Strncmp(buf, quitStr, 2) == 0) {
                        errmsg = strdup(GETMESSAGE(
                            4, 9, "Warning: command was not executed\n"));
                        printf(errmsg);
                        free(errmsg);
                        returnVal = 0;
                        break;
                        ...
                        break;
                           ...
                           (asl_set(argv0, tbls, buf,
                                    (unsigned char **)pargs) == SH_FAIL)) {
                        pp_usage();
                }
        }
...
*/
#define CALL_RETURN(RET)                                                       \
        return (SET_RET(RET),                                                  \
                ((ret_type == ZERORET)                                         \
                     ? (RET)                                                   \
                     : ((ret_type == NONZERO) ? !(RET) : ((RET) < 0))))
#define EARLY_RETURN(RET) return (SET_RET(RET))
#define SET_RET(RET)                                                           \
        (((int)sprintf(xk_ret_buffer, use, (RET))),                            \
         (int)(xk_ret_buf = xk_ret_buffer), RET)
...
int do_call(int argc, char **argv) {
        ...
        for (j = 1; (j < argc) && argv[j][0] == '-'; j++) {
                for (i = 1; argv[j][i]; i++) {
                        ...
                        default:
                                errmsg =
                                    strdup(GetSharedMsg(DT_UNKNOWN_OPTION));
                                printerrf(argv[0], errmsg, argv[j], NULL, NULL,
        ...
        if (!func &&
            ((argv[j][0] != '0') || (UPP(argv[j][1]) != 'X') ||
             !(func = (unsigned long (*)())strtoul(argv[j], &p, 16)) || *p)) {
                errmsg = strdup(
                    GETMESSAGE(4, 10, "Unable to locate the function '%s'"));
                printerrf(argv[0], errmsg, argv[j], NULL, NULL, NULL, NULL,
                          NULL, NULL, NULL);
                free(errmsg);
                CALL_RETURN(1);
        }
        ...
        for (i = 0; (i < MAX_CALL_ARGS) && (j < argc) && argv[j]; j++, i++) {
                ...
                if (argv[j][0] == '@') {
                        ...
                        type[val - argv[j] - 1] = '\0';
                        val++;
                        if (parse_decl(argv[0], tblarray + i, type, 1) ==
                            FAIL) {
                                dorun = 0;
                                ret = -1;
                                break;
                                ...
                                if (!strparse(tblarray + i,
                                        ...
                                        errmsg = strdup(
                                            GETMESSAGE(4, 11,
                                                       ...
                                                       "the definition for "
                                                       "structure '%s'"));
                                        printerrf(argv[0], errmsg, val, type,
                                                  NULL, NULL, NULL, NULL, NULL,
                                                  NULL);
                                        free(errmsg);
                                        dorun = 0;
...
static long get_prdebug(void) { return (_Prdebug); }

static long set_prdebug(long n) { _Prdebug = n; }

int asl_set(char *argv0, memtbl_t *tblarray, char *desc,
            unsigned char **pargs) {
        char *ptr;
        char *val;
        memtbl_t *tbl;
        ...
        if (!tbl || (i > top)) {
                errmsg =
                    strdup(GETMESSAGE(4, 12, "Cannot locate the field '%s'"));
                printerrf(argv0, errmsg, fldp, NULL, NULL, NULL, NULL, NULL,
                          NULL, NULL);
                free(errmsg);
                return (SH_FAIL);
        }
        if (!op || !(tbl->flags & F_SIMPLE)) {
                if (XK_PARSE(tbl, &val, ptr, 0, 0, NULL, all_tbl_find) < 0) {
                        errmsg =
                            strdup(GETMESSAGE(4, 13,
                                              "Cannot set the following value "
                                              "for the field '%s': %s"));
                        printerrf(argv0, errmsg, val, NULL, NULL, NULL, NULL,
                                  NULL, NULL, NULL);
                        free(errmsg);
                }
        } else {
...
int do_field_comp(int argc, char **argv) {
        char *val, *type;
        ...
        for (; (i < argc) && argv[i]; i++) {
                ...
                if (val = strchr(argv[i], '=')) {
                        *val++ = '\0';
                        ...
                        if (!tbl2) {
                                errmsg =
                                    strdup(GetSharedMsg(DT_BAD_FIELD_NAME));
                                printerrf(argv[0], errmsg, argv[i], type, NULL,
                                          NULL, NULL, NULL, NULL, NULL);
                                free(errmsg);
                                return (SH_FAIL);
                        }
                ...
                if (strcmp(pr1, pr2)) {
                        if (env_get((char *)"PRCOMPARE")) {
                                errmsg = strdup(GETMESSAGE(
                                    4, 16,
                                    "The following comparision failed: "
                                    "'%s'\n\tActual:  %s\n\tCompare: %s"));
                                printerrf(argv[0], errmsg, argv[i], pr1, pr2,
                                          NULL, NULL, NULL, NULL, NULL);
                                free(errmsg);
                        }
                        return (SH_FAIL);
...
static int myprompt(char *prompt) { fprintf(stderr, prompt); }
