...
static int freemem(struct memtbl *mem);
static growmem(void);
static char *endtok(char *start);
static int chg_structlist(struct memtbl **memptr, int id);
static struct_init(void);

memtbl_t *ffind(memtbl_t *tbl, char *fld, char **pptr) {
...
static growmem(void) {
        if (!(Dynmem = (struct memtbl **)realloc(
                  Dynmem, (Sdynmem + 20) * sizeof(memtbl_t *))))
                return (SH_FAIL);
        chg_structlist(Dynmem, DYNMEM_ID);
        memset(((char *)Dynmem) + Sdynmem * sizeof(memtbl_t *), '\0',
               20 * sizeof(memtbl_t *));
        Sdynmem += 20;
}

int do_struct(int argc, char **argv) {
        ...
        name = argv[argstart++];
        for (i = 0; i < Ndynmem; i++)
                if (!(Dynmem[i]->flags & F_FIELD) &&
                    (strcmp(name, Dynmem[i]->name) == 0))
                        break;
        if ((i < Ndynmem) && !redo) {
                if (!redo)
                        return (SH_SUCC);
        ...
        */
        if (!(mem = (struct memtbl *)malloc(2 * sizeof(struct memtbl))))
                return (SH_FAIL);
        if (!(fmem =
                  (struct memtbl *)malloc((argc - 1) * sizeof(struct memtbl))))
                return (SH_FAIL);
        ...
        for (j = argstart; (j < argc) && argv[j]; j++) {
                ...
                mem->size += (fmem[j - argstart].ptr) ? sizeof(void *)
                                                      : fmem[j - argstart].size;
        }
        return (SH_SUCC);
}

int do_typedef(int argc, char **argv) {
        struct memtbl *mem;
        int i, redo;
        char *name, *decl;
...
int parse_decl(char *argv0, struct memtbl *mem, char *decl, int tst) {
        struct memtbl *tbl;
        char *p, *end;
        char hold;
        int flag = 0, done;
        ...
        if (!p[0]) {
                ...
                printerrf(argv0, errmsg, decl, NULL, NULL, NULL, NULL, NULL,
                          NULL, NULL);
                free(errmsg);
...
int do_structlist(int argc, char **argv) {
        int i, j, id = 0;
        char *prefix = NULL;
        ...
        for (i = 1; (i < argc) && argv[i]; i++) {
                if (argv[i][0] == '-') {
                        for (j = 1; argv[i][j]; j++) {
                                ...
                                default:
                                        errmsg = strdup(
                                            GetSharedMsg(DT_UNKNOWN_OPTION));
...
int add_structlist(struct memtbl **memptr, char *prefix, int id) {
        int i;

        if (!Structlist)
                Structlist = (struct structlist *)malloc(
                    (Nstructlist + 1) * sizeof(struct structlist));
        else
                Structlist = (struct structlist *)realloc(
                    Structlist, (Nstructlist + 1) * sizeof(struct structlist));
        if (!Structlist)
                return (SH_FAIL);
        ...
        if (memptr[0] && memptr[0][0].name) {
                for (i = 1;
                     memptr[i] && memptr[i][0].name && memptr[i][0].name[0];
                     i++)
                        if (strcmp(memptr[i][0].name, memptr[i - 1][0].name) <
                            0)
                                break;
...
int strparse(memtbl_t *tbl, char **pbuf, char *val) {
        char *p, *phold;
        int ret;
...
int strfree(char *buf, char *type) {
        memtbl_t tbl;
...
int do_sizeof(int argc, char **argv) {
        memtbl_t *tbl;
        char *errmsg;
        ...
        if ((tbl = all_tbl_search(argv[1], 0)) == NULL) {
                errmsg = strdup(GETMESSAGE(12, 3,
                                           "The following is not a valid data "
                                           "type or structure name: %s"));
        ...
        if (argc >= 3) {
                ...
                sprintf(buf, use2, argv[2],
                        tbl->ptr ? sizeof(void *) : tbl->size);
                env_set(buf);
        } else {
                sprintf(xk_ret_buffer, use,
                        tbl->ptr ? sizeof(void *) : tbl->size);
...
memtbl_t *all_tbl_find(char *name, int tbl, long id) {
        int i;
...
memtbl_t *all_tbl_search(char *name, int flag) {
        register int i;
        void *found;
        ...
        if (found = (void *)xkhash_find(Hashnams, name))
                ...
                for (i = 0; i < Nstructlist; i++) {
                        if (subtbl = Structlist[i].mem)
                                for (j = 0; subtbl[j]; j++)
                                        if (!(subtbl[j]->flags & F_FIELD) &&
                                            (strcmp(name, subtbl[j]->name) ==
                                             0) &&
                                            ((subtbl[j]->kind != K_TYPEDEF) ||
                                             (subtbl[j]->tbl != -1))) {
                                                if (!(flag & NOHASH))
                                                        xkhash_add(
                                                            Hashnams, name,
...
memtbl_t *asl_find(memtbl_t *ptbl, memtbl_t *tbls, char *fld, char **pptr) {
        int i;
        memtbl_t *tbl;
        ...
        for (i = 0; tbls[i].name; i++) {
                if ((xk_Strncmp(tbls[i].name, fld, strlen(fld)) == 0) &&
                    (strlen(fld) == strlen(tbls[i].name))) {
                        if (pptr && ptbl &&
                            ((ptbl->kind == K_STRUCT) || (ptbl->kind == K_ANY)))
                                *pptr += tbls[i].offset;
                        return (tbls + i);
                }
        }
        for (i = 0; tbls[i].name; i++) {
                if ((tbls[i].kind == K_TYPEDEF) || (tbls[i].kind == K_STRUCT) ||
                    (tbls[i].kind == K_UNION) || (tbls[i].kind == K_ANY)) {
                        char *hold;

                        if (!pptr) {
                                if ((tbl = asl_find(tbls + i,
                                                    all_tbl_find(tbls[i].tname,
                                                                 tbls[i].tbl,
                                                                 tbls[i].id),
                                                    fld, pptr)) != NULL)
                                        return (tbl);
...
static struct_init(void) {
        ...
        if (!(Dynmem =
                  (struct memtbl **)malloc(20 * sizeof(struct memtbl *)))) {
