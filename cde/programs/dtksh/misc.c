...
int symcomp(void *sym1, void *sym2)

{
        return (strcmp(((struct symarray *)sym1)->str,
                       ((struct symarray *)sym2)->str));
}
...
int do_deref(int argc, char **argv) {
        unsigned char *ptr;
        long i, len = 0;
        short longwise = -1;
        char printit = 0;
        ...
        if (ptr) {
                if (((i < argc) && argv[i]) || printit) {
                        ...
                        for (i = 0; i < len; i += incr, p += 2 * incr)
                                sprintf(p, "%*.*x", incr * 2, incr * 2,
                                        longwise ? *((unsigned long *)(ptr + i))
                                                 : (unsigned long)(ptr[i]));
                        if (printit)
                                ALTPUTS(dbuf);
                        else
                        ...
                        if (len > sizeof(unsigned long)) {
                                ...
                                errmsg =
                                    strdup(GETMESSAGE(10, 2,
                                                      "To set RET, the length "
                                                      "must be less than: %s"));
                                printerrf(argv[0], errmsg, tmpBuf, NULL, NULL,
                                          NULL, NULL, NULL, NULL, NULL);
                                free(errmsg);
                                return (SH_FAIL);
                        }
        ...
        printerrf(argv[0], errmsg, argv[--i], NULL, NULL, NULL, NULL, NULL,
                  NULL, NULL);
        free(errmsg);
        return (SH_FAIL);
}

void *nop(void *var) { return (var); }

void *save_alloc(void *var) { return (var); }
