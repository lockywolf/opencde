...
static void fixupresources(char *name, Hash_table_t *res, resfixup_t *fixups);
static int rescompare(XtResource **r1, XtResource **r2);
static void _pr_class(classtab_t *c);
static void _pr_resource_list(XtResource *res);
static void sort_and_print_res(void);
static void gather_resource_list(char *name, char *r, void *notUsed);
static void _pr_resource(classtab_t *c, wtab_t *w);
static void pr_resource(char *name, char *r, void *notUsed);
static void pr_class(char *name, char *c, void *notUsed);
static char *getname(wtab_t *w, char *buf, int max);
static void pr_widinfo(wtab_t *w);
...
void init_widgets(void) {
        ...
        for (i = 0; C[i].cname != NULL; i++) {
                if ((nam = hashput(Wclasses, C[i].cname, (char *)(&C[i]))) ==
                    NULL) {
                        errhdr = strdup(GetSharedMsg(DT_ERROR));
                        errmsg = strdup(GETMESSAGE(
                            14, 1,
                            "Internal hash table failure during widget "
                            "class initialization; exiting"));
...
classtab_t *str_to_class(char *arg0, char *s) {
        register int i, j, n;
        Widget w;
        char *nam;
        classtab_t *ret;
        ...
        if ((nam = hashget(Wclasses, s)) != NULL) {
                ...
                if (ret->res == NULL) {
                        ...
                        XtGetResourceList(ret->class, &resources,
                                          &numresources);
                        for (i = 0; i < numresources; i++) {
                                if ((nam = (char *)hashput(
                                         (Hash_table_t *)ret->res,
                                         resources[i].resource_name,
                                         (char *)&resources[i])) == NULL) {
                                        errhdr = strdup(GetSharedMsg(DT_ERROR));
                                        errmsg = strdup(
                                            GetSharedMsg(DT_HASHING_FAILURE));
                                        printerrf(errhdr, errmsg,
                                                  resources[i].resource_name,
                                                  ret->cname, NULL, NULL, NULL,
                                                  NULL, NULL, NULL);
                                        free(errhdr);
                                        free(errmsg);
                                }
                        ...
                        /*
                         * The following is a special hack to work around a
                         * Motif bug (which has been reported).  DialogShell is
                         * a grandchild class of VendorShell, which defines
                         * secondary resources.  DialogShell is also a child
                         * class of TransientShell.  Secondary resource
                         * information is contained within extension records,
                         * which the TransientShell class knows nothing about,
                         * thus preventing XmGetSecondaryResourceData from
                         * getting any secondary resources further up the
                         * inheritance chain.
                         */
                        if (ret->class == xmDialogShellWidgetClass) {
                                numBlocks = XmGetSecondaryResourceData(
                                    vendorShellWidgetClass, &blockArray);
                        } else
                                numBlocks = XmGetSecondaryResourceData(
                                    ret->class, &blockArray);

                        if (numBlocks > 0) {
                                for (i = 0; i < numBlocks; i++) {
                                        for (j = 0;
                                             j < blockArray[i]->num_resources;
                                             j++) {
                                                if ((nam = (char *)hashput(
                                                         (Hash_table_t *)
                                                             ret->res,
                                        ...
                                        /*
                                         * We can't free up the actual array of
                                         * resources, since we hashed a
                                         * reference to the entries into our
                                         * resource hash table: XtFree((char
                                         * *)blockArray[i]->resources);
                                         */
                         ...
                         */
                        XtGetConstraintResourceList(ret->class, &resources,
                                                    &numresources);
                        if (resources != NULL) {
                                ret->con = (char *)hashalloc(NULL, 0);
                                for (i = 0; i < numresources; i++) {
                                        if ((nam = (char *)hashput(
                                                 (Hash_table_t *)ret->con,
                                                 resources[i].resource_name,
                                                 &resources[i])) == NULL) {
                                                errhdr = strdup(
                                                    GetSharedMsg(DT_ERROR));
                                                errmsg = strdup(GetSharedMsg(
                                                    DT_HASHING_FAILURE));
                                                printerrf(
                                                    errhdr, errmsg,
        ...
        errmsg = strdup(
            GETMESSAGE(14, 2, "Could not find a widget class named '%s'"));
        printerrf(arg0, errmsg, s, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
        free(errmsg);
        return (NULL);
}

Widget DtkshNameToWidget(String s) {
        Widget w;
        char *p;
        int len;
        char *errhdr;
...
wtab_t *str_to_wtab(char *arg0, char *v) {
        int index, len;
        char *errmsg;
            ...
            strspn(&v[1], str_0123456789) != len - 1) {
                ...
                if ((wid = DtkshNameToWidget(v)) == NULL) {
                        if (arg0) {
                                errmsg = strdup(
                                    GETMESSAGE(14, 3,
                                               "The identifier '%s' is not a "
                                               "valid widget handle"));
                                printerrf(arg0, errmsg, v, NULL, NULL, NULL,
                                          NULL, NULL, NULL, NULL);
                                free(errmsg);
                        }
                        return (NULL);
        ...
        if (index < 0 || index >= NumW) {
                if (arg0) {
                        ...
                        printerrf(arg0, errmsg, v, NULL, NULL, NULL, NULL, NULL,
                                  NULL, NULL);
                        free(errmsg);
                }
                return (NULL);
...
wtab_t *widget_to_wtab(Widget w) {
        register int i;
        char *errmsg;
         ...
         */
        {
                ...
                if (class == NULL) {
                        errmsg = strdup(GETMESSAGE(
                            14, 4, "Unable to find the widget class"));
                        printerr("widget_to_wtab", errmsg, NULL);
                        free(errmsg);
                        return (NULL);
                }
...
void get_new_wtab(wtab_t **w, char *name)

{
        ...
        if (i == NumW) {
                if (NumW < MaxW) {
                        ...
                        MaxW += WIDGETALLOC;
                        W = (wtab_t **)XtRealloc((char *)W,
                                                 sizeof(wtab_t *) * MaxW);
                        wentries =
                            (wtab_t *)XtMalloc(sizeof(wtab_t) * WIDGETALLOC);
...
int do_DtLoadWidget(int argc, char *argv[]) {
        classtab_t *classtab;
        void *address;
        char *nam;
        char *errhdr;
        ...
        if (argc != 3) {
                errmsg =
                    strdup(GETMESSAGE(14, 5,
                                      "Usage: DtLoadWidget widgetClassName "
                                      "widgetClassRecordName"));
        ...
        if ((address = (void *)fsym(argv[2], -1)) == NULL) {
                ...
                if ((nam = hashput(Wclasses, classtab->cname,
                                   (char *)classtab)) == NULL) {
                        errhdr = strdup(GetSharedMsg(DT_ERROR));
                        errmsg = strdup(
                            GETMESSAGE(14, 7,
                                       "Internal hash table failure during "
                                       "initialization of widget class '%s'"));
                        printerrf(errhdr, errmsg, classtab->cname, NULL, NULL,
...
static void _pr_class(classtab_t *c) { printf("%s\n", c->cname); }

static void _pr_resource_list(XtResource *res) {
        printf("\t%-24.24s %-24.24s %s\n", res->resource_name,
               res->resource_class, res->resource_type);
}
...
static void _pr_resource(classtab_t *c, wtab_t *w) {
        ...
        errmsg2 = strdup(Show_constraint ? GETMESSAGE(14, 9, "CONSTRAINT ")
                                         : str_nill);
        printf(errmsg, errmsg2, c->cname, w ? " " : str_nill,
               w ? w->widid : str_nill);
        free(errmsg);
        free(errmsg2);
        ...
        hashwalk((Hash_table_t *)(Show_constraint ? c->con : c->res), 0,
                 (int (*)())gather_resource_list, NULL);
        if (!Show_constraint && w && w->parent != NULL &&
            XtIsConstraint(w->parent->w)) {
                hashwalk((Hash_table_t *)w->parent->wclass->con, 0,
                         (int (*)())gather_resource_list, NULL);
        }
...
static void pr_widinfo(wtab_t *w) {
        ...
        if (errmsg == NULL) {
                errmsg = strdup(
                    GETMESSAGE(14, 13, "%-15s %-6s %-6s %-18s %-6s %s\n"));
        ...
        printf(errmsg, w->envar, w->widid,
               w->parent ? w->parent->widid : "none", w->wclass->cname, statbuf,
               name);
}
...
int do_DtWidgetInfo(int argc, char *argv[]) {
        register int i, j;
        char buf[1024];
        wtab_t *w;
        classtab_t *c;
...
void alt_env_set_var(char *variable, char *value) {
