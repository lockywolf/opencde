...
static int xk_uppercase(char *s);
static int getsize(int fd, int *psize);
...
int xk_skipwhite(char **buf) {
        while (isspace(**buf))
                (*buf)++;
        return (SUCCESS);
}

int xk_backskip(char **buf, int *n) {
        *n = 0;
        while (isspace(**buf)) {
                (*buf)--;
                *n++;
...
int xk_parexpect(char **buf, char *str) {
...
int xk_parpeek(char **buf, char *str) {
        RIF(xk_skipwhite(buf));
        if (strncmp(*buf, str, strlen(str)) == 0)
                return (TRUE);
        else
...
int xk_prin_int(memtbl_t *tbl, char **buf, unsigned long *old_v)

{
        ...
        **buf = '\0';
        if ((Pr_format & PRSYMBOLIC) &&
            ((Val_list.syms != NULL) || ((sym = fsymbolic(tbl)) != NULL))) {
...
int xk_par_int(char **buf, long *v, struct envsymbols *env) {
...
int xk_prin_nts(char **buf, char *str) {
        return (
            xk_prin_charstr(buf, (unsigned char *)str, str ? strlen(str) : 0));
}

int xk_prin_charstr(char **buf, unsigned char *str, int len) {
        register int i;
...
int xk_prin_hexstr(char **buf, char *str, int len) {
        register int i;
        unsigned char tempc;
...
int xk_par_chararr(char **buf, char *str, int *len) {
...
int xk_par_nts(char **buf, char **str) {
        int temp = 0;
...
int xk_par_charstr(char **buf, char **str, int *len) {
        register int i;
        char delim;
                ...
                *p++ = '\0';
                if ((fp = fopen(filename, "r")) == NULL) {
                        errmsg = strdup(GETMESSAGE(
                            7, 1,
                            "Unable to open the file '%s'; verify that it "
        ...
        i = 0;
        while ((*buf)[0] != '\0' &&
               ((getmode == CHAR_QUOTED && (*buf)[0] != delim) ||
...
int xk_Strncmp(char *s1, char *s2, int len) {
        int diff, i;
