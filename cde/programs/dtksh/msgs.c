...
char *GetSharedMsg(int msgId) {
        ...
        case DT_BAD_FIELD_NAME:
                msg = GETMESSAGE(
                    11, 1,
                    "Cannot find a field named '%s' in the structure '%s'");
                break;

        case DT_CONV_BUF_OVFL:
                msg = GETMESSAGE(11, 2,
                                 "DtkshCvtCallbackToString: An internal "
                                 "conversion buffer overflowed");
                break;

        case DT_HASHING_FAILURE:
                msg = GETMESSAGE(
                    11, 3,
                    "Hashing failure for resource '%s' in widget class '%s'");
                break;
        ...
        case DT_TK_NOT_INIT:
                msg = GETMESSAGE(11, 5,
                                 "The command cannot be executed; the toolkit "
        ...
        case DT_BAD_ATOM:
                msg = GETMESSAGE(11, 8,
                                 "The following property atom is invalid: %s");
                break;

        case DT_UNDEF_RESOURCE:
                msg = GETMESSAGE(11, 9,
                                 "The following resource is not defined for "
                                 "widget '%s': %s");
                break;

        case DT_BAD_POSITION:
                msg =
                    GETMESSAGE(11, 10, "The position specified is invalid: %s");
                break;
        ...
        case DT_BAD_WIDGET_HANDLE:
                msg = GETMESSAGE(11, 12,
                                 "The widget handle '%s' does not refer to an "
                                 "existing widget");
                break;

        case DT_CMD_WIDGET:
                msg =
                    GETMESSAGE(11, 13, "The widget must be a 'command' widget");
                break;

        case DT_MAIN_WIN_WIDGET:
                msg = GETMESSAGE(11, 14,
                                 "The widget must be a 'mainWindow' widget");
                break;
        ...
        case DT_SCROLLBAR_WIDGET:
                msg = GETMESSAGE(11, 16,
                                 "The widget must be a 'scrollBar' widget");
                break;

        case DT_TOGGLE_WIDGET:
                msg = GETMESSAGE(
                    11, 17,
                    "The widget must be a 'toggleButton' widget or gadget");
                break;
        ...
        case DT_ALLOC_FAILURE:
                msg = GETMESSAGE(11, 19,
                                 "Unable to allocate required memory; exiting");
                break;
        ...
        case DT_BAD_DECL:
                msg = GETMESSAGE(11, 23,
                                 "Unable to parse the declaration '%s'; using "
                                 "'unsigned long'");
                break;
        ...
        case DT_USAGE_DISPLAY_ROOT_VAR:
                msg =
                    GETMESSAGE(11, 33, "Usage: %s display rootWindow variable");
                break;

        case DT_BAD_TIMEOUT:
                msg =
                    GETMESSAGE(11, 34, "The timeout parameter is invalid: %s");
                break;
        ...
        case DT_BAD_PATTERN:
                msg =
                    GETMESSAGE(11, 36, "The patterns parameter is invalid: %s");
                break;
        }
        return (msg);
}
