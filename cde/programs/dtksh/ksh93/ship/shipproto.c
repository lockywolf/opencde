...
int replace __PARAM__((const char *newfile, const char *oldfile, int preserve),
    ...
    __OTORP__(const char *newfile; const char *oldfile; int preserve;) {
        ...
        if (preserve && stat(oldfile, &st) || remove(oldfile) ||
...
extern __MANGLE__ void ppmacref __PROTO__((struct ppsymbol *, char *, int,
                                           int));
extern __MANGLE__ void ppop __PROTO__((int, ...));
extern __MANGLE__ void pppragma __PROTO__((char *, char *, char *, char *,
                                           int));
...
static struct fsminit fsminit[] = {
...

    {
        (0 + 24),
        {6},
        (((((0401 + 1)) - 0401) << (7 + 1)) | (((0 + 26) + 14))),
    },
    {
        (0 + 24),
        {'.'},
        (((((0401 + 29)) - 0401) << (7 + 1)) | (((0 + 26) + 13))),
    },
...
void ppfsm __PARAM__((int op, register char *s), (op, s))
    __OTORP__(int op; register char *s;) {
        register int c;
        register int n;
        register int i;
        register short *rp;
        ...
        case 4:
                for (fp = fsminit;; fp++) {
                        ...
                        for (i = 0; i < sizeof(fp->ch) && (c = fp->ch[i]);
                             i++) {
                                switch (c) {
                                case 6:
                                        for (c = 0; c <= 255; c++)
...
static int sstrlen __PARAM__((register char *s), (s))
...
static int sstrncmp
__PARAM__((register char *s, register char *t, register int n), (s, t, n))
    __OTORP__(register char *s; register char *t; register int n;) {
        register char *e = s + n;
...
static char *memcopy __PARAM__((register char *s, register char *t, int n),
                               (s, t, n))
    __OTORP__(register char *s; register char *t; int n;) {
        register char *e = t + n;
...
static char *strcopy __PARAM__((register char *s, register char *t), (s, t))
    __OTORP__(register char *s; register char *t;) {
        while (*s++ = *t++)
                ;
        return (s - 1);
}

static char *comment __PARAM__((register char *p, register char *s,
                                register int n, int u),
                               (p, s, n, u))
    __OTORP__(register char *p; register char *s; register int n; int u;) {
        register char *e;
        char *m;
...
static char *copyright __PARAM__((register char *p, register char *s), (p, s))
    __OTORP__(register char *p; register char *s;) {
        register char *v;
        register char *x;
        register int c;
        int i;
        ...
        s = buf;
        if (notice.type == 2) {
                if ((x = notice.item[i = 3].data) ||
                    (x = notice.item[i = 0].data)) {
                ...
                if (notice.item[4].data) {
                        ...
                        s = strcopy(
                            buf, "This is unpublished proprietary source code");
                        if (x)
                                s = strcopy(s, " of");
                        p = comment(p, buf, s - buf, 1);
                        s = buf;
                        ...
                        if (s > buf)
                                p = comment(p, buf, s - buf, 1);
                        p = comment(
                            p, "and is not to be disclosed or used except in",
                            sizeof("and is not to be disclosed or used except "
                                   "in") -
                                1,
                            1);
                        p = comment(
                            p, "accordance with applicable agreements",
                            sizeof("accordance with applicable agreements") - 1,
                            1);
                }
                p = comment(p, ((char *)0), 0, 0);
        } else if (notice.type == 1) {
                p = comment(p, "For nonexclusive individual use",
                            sizeof("For nonexclusive individual use") - 1, 1);
                p = comment(p, ((char *)0), 0, 0);
        }
        ...
        if (notice.type == 2) {
                if (!notice.item[4].data)
                        p = comment(
                            p, "Unpublished & Not for Publication",
                            sizeof("Unpublished & Not for Publication") - 1, 0);
                p = comment(p, "All Rights Reserved",
                            sizeof("All Rights Reserved") - 1, 0);
        }
        p = comment(p, ((char *)0), 0, 0);
        if (notice.item[4].data) {
                ...
                p = comment(p, buf, s - buf, 0);
                p = comment(
                    p, "under the terms and conditions of the license in",
                ...
                p = comment(
                    p, "The copyright notice above does not evidence any",
                    sizeof("The copyright notice above does not evidence any") -
                        1,
                    0);
                p = comment(
                    p, "actual or intended publication of such source code",
                    sizeof(
        ...
        if (x = notice.item[1].data) {
                p = comment(p, "This software was created by the",
                            sizeof("This software was created by the") - 1, 0);
        ...
        if (x = notice.item[2].data) {
                p = comment(p, "For further information contact",
                            sizeof("For further information contact") - 1, 0);
...
static char *number __PARAM__((register char *p, register long n), (p, n))
    __OTORP__(register char *p; register long n;) {
        register long d;
    ...
    __OTORP__(char *iob; int level; char *msg; char *arg;) {
        ...
        if (iob) {
                register struct proto *proto =
                    (struct proto *)(iob - sizeof(struct proto));
...
static char *linesync __PARAM__((register struct proto * proto,
                                 register char *p, register long n),
                                (proto, p, n))
    __OTORP__(register struct proto *proto; register char *p;
              register long n;) {
...
static char *init __PARAM__((char *op, int flags), (op, flags))
    __OTORP__(char *op; int flags;) {
        if (flags & (1L << 9))
                op = strcopy(op, "\
\n\
#if !defined(__PROTO__)\n\
...
static int lex __PARAM__((register struct proto * proto, register long flags),
                         (proto, flags))
    __OTORP__(register struct proto *proto; register long flags;) {
        register char *ip;
        register char *op;
        register int c;
        register int state;
...
fsm_terminal:
        ...
        case ((0 + 26) + 1):
                ...
                default:
                        ...
                        if ((flags & ((1L << 5) | (1L << 14))) == (1L << 5))
                                ...
                                (*op++ = (c));
                        rp = _pp_fsmtab[((rp) >= _pp_fsmtab[(0 + 14)] &&
                                         (rp) <= _pp_fsmtab[(0 + 16)])
                                            ? (0 + 14)
                                            : (0 + 12)];
                        break;
        ...
        case ((0 + 26) + 2):
                if (c) {
                        if (state = _pp_fsmtab[(0 + 26)][(((rp)-_pp_fsmtab[0]) /
                                                          (255 + 1)) +
                                                         1])
                                goto fsm_terminal;
                        do {
...
");
                                                                                ...
                                                                                proto_error(
                                                                                    (char
                                                                                        ...
                                                                                        sizeof(
                                                                                            ...
                                                                                            proto),
                                                                                    1,
                                                                                    "function pointer argument prototype omitted",
                                                                                    ((char
                                                                                          *)0));
                                                                        (*op++ =
                                                                             (*ie++));
        ...
        do {
                ...
                (proto->op = op);
                proto->flags &= ~((1L << 8) | (1L << 27) | (1L << 28));
                proto->flags |= flags & ((1L << 8) | (1L << 27) | (1L << 28));
        } while (0);
        return (c);
}
...
char *pppopen __PARAM__((char *file, int fd, char *notice, int flags),
                        (file, fd, notice, flags))
    __OTORP__(char *file; int fd; char *notice; int flags;) {
        register struct proto *proto;
        register char *iob;
        register long n;
        register char *s;
...

        {
                ...
                n = (4 * 1024);
                if (!(proto =
                          ((0) ? (struct proto *)realloc(
                                     (char *)(0),
                                     sizeof(struct proto) * (1) + (5 * n + 2))
                               : (struct proto *)calloc(
                                     1, sizeof(struct proto) * (1) +
                                            (5 * n + 2)))))
                        return (0);
                proto->iz = n;
                proto->oz = 3 * n;
        ...
        *(proto->ip + n) = 0;
        ...
        while (m-- > 0 && *s) {
                ...
                if (*s == '#') {
                        ...
                        while (*s == ' ' || *s == '\t')
                                s++;
                        if (!sstrncmp(s, "pragma", sizeof("pragma") - 1) &&
                            (*(s += sizeof("pragma") - 1) == ' ' ||
                             *s == '\t')) {
                                while (*s == ' ' || *s == '\t')
                                        s++;
                                ...
                                if (!sstrncmp(s, "prototyped",
                                              sizeof("prototyped") - 1) &&
                                    (*(s += sizeof("prototyped") - 1) == ' ' ||
                                     *s == '\t' || *s == '\n' || *s == '\r')) {
                                        while (*s)
                                                if (*s++ == '\n') {
                                                        pragma += 2;
                ...
                if (!(flags & (1 << 0))) {
                        if (proto->flags & (1L << 29)) {
                                ...
                                proto->op = strcopy(
                                    ...
                                    "/* : : generated by proto : : */\n");
                                if (n)
                                        proto->op = linesync(proto, proto->op,
                                                             proto->line);
                                else if (proto->flags &
                                         ((1L << 9) | (1L << 10)))
                                        proto->op =
                                            init(proto->op, proto->flags);
                        }
                }
        }
...
int pppread __PARAM__((char *iob), (iob)) __OTORP__(char *iob;) {
        ...
        if (proto->flags & (1L << 17)) {
                if (proto->iz) {
                        n = proto->iz;
                        proto->iz = 0;
                } else if (!(proto->flags & (1L << 15)))
                        n = 0;
                else if ((n = read(proto->fd, proto->ob, proto->oz)) <= 0 ||
                         (proto->flags & (1L << 21)) && n < proto->oz) {
                        proto->flags &= ~(1L << 15);
                        close(proto->fd);
                }
        } else {
                if (proto->op == proto->ob) {
                        ...
                        if (proto->flags & (1L << 29)) {
                                ...
                                proto->op = memcopy(proto->ob, proto->ip,
                                                    ip - proto->ip);
                                proto->ip = ip;
                        } else

                                lex(proto, proto->flags);
                        if ((proto->flags & ((1L << 4) | (1L << 15))) ==
                            (1L << 4))
                                proto->op = strcopy(
                                    proto->op, "/* NOTE: some constructs may "
...
static int proto __PARAM__((char *file, char *notice, int flags),
                           (file, notice, flags))
    __OTORP__(char *file; char *notice; int flags;) {
        char *b;
        char *e;
        int n;
        int x;
        ...
        if (file && access(file, 4))
                ...
                if (file) {
                        ...
                        x = *e;
                        *e = '_';
                        if ((fd = (flags & ((1 << 13) << 1)) ? creat(file, 0666)
                                                             : 1) < 0) {
                                proto_error(b, 2,
        ...
        while ((file = *++argv) && *file == '-' && *(file + 1)) {
                for (;;) {
                        ...
                        case '+':
                                ...
                                continue;
                        default:
                                proto_error(((char *)0), 2, file,
                                            "unknown option");

                        case '?':
                                b = "Usage: proto [-dfhinprst+] [-c "
                                    "\"name='value' ...\"] [-l list] file "
                                    "...\n";
                                write(2, b, sstrlen(b));
                                exit(2);
                        }
