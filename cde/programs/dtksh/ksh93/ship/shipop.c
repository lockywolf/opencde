...
static void error __PARAM__((int level, const char *file, const char *msg),
                            (level, file, msg))
    __OTORP__(int level; const char *file; const char *msg;) {
...
static int shipnewer __PARAM__((register char **argv), (argv))
    __OTORP__(register char **argv;) {
        unsigned long ref;
        struct stat st;

        if (!*argv || stat(*argv, &st))
                return (1);
        ref = (unsigned long)st.st_mtime;
        while (*++argv)
                if (stat(*argv, &st) || ref < (unsigned long)st.st_mtime)
                        return (1);
        return (0);
}
...
static unsigned long sealfile __PARAM__((char *file, int fd, unsigned long s),
                                        (file, fd, s))
    __OTORP__(char *file; int fd; unsigned long s;) {
...
static int shipseal __PARAM__((register char **argv), (argv))
    __OTORP__(register char **argv;) {
        register char *f;
        register int fd;
        unsigned long s;
        ...
        s = 0;
        if (!*argv)
                s = sealfile("/dev/stdin", 0, s);
        else
                while (f = *argv++) {
                        if (*f == '-' && !*(f + 1))
                                s = sealfile("/dev/stdin", 0, s);
                        else if ((fd = open(f, 0)) < 0)
                                error(2, f, "cannot read");
                        else {
                                s = sealfile(f, fd, s);
                                close(fd);
                        }
...
static int shipstate __PARAM__((register char **argv), (argv))
    __OTORP__(register char **argv;) {
        register char *s;
        register int c;
        long ref;
        ...
        if (!(s = *argv++) || stat(s, &st))
                error(3, (char *)0, "reference file omitted");
...
static int shiptime __PARAM__((register char **argv), (argv))
    __OTORP__(register char **argv;) {
        struct stat st;
        time_t date;
...
static int shipxap __PARAM__((char **argv), (argv)) __OTORP__(char **argv;) {
...
static struct optab ops[] = {
    "newer",   shipnewer, "seal",   shipseal, "state",
    shipstate, "time",    shiptime, "xap",    shipxap,
};
