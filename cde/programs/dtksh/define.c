...
static growdef(void);
static int add_deflist(struct symarray *defptr, char *prefix);
...
static growdef(void) {
        ...
        if (!(Dyndef = (struct symarray *)realloc(
                  Dyndef, (Sdyndef + 20) * sizeof(struct symarray)))) {
                return (SH_FAIL);
        }
        Deflist->defs = Dyndef;
...
int do_define(int argc, char **argv) {
        int i, argstart, redo;
        char *name;
...
int fdef(char *str, unsigned long *val) {
        struct symarray *found, dummy;
        int i;
        ...
        for (i = 0; i < Ndeflist; i++) {
                if (Deflist[i].defs) {
                        if (Deflist[i].size < 0) {
                                found = (struct symarray *)lfind(
                                    (char *)&dummy, Deflist[i].defs,
                                    (unsigned int *)&Deflist[i].size,
                                    sizeof(struct symarray), symcomp);
                        } else {
                                found = (struct symarray *)bsearch(
                                    (char *)&dummy, Deflist[i].defs,
                                    Deflist[i].size, sizeof(struct symarray),
                                    symcomp);
                        }
...
int do_deflist(int argc, char **argv) {
        int i, j;
        char *prefix = NULL;
        ...
        for (i = 1; (i < argc) && argv[i]; i++) {
                if (argv[i][0] == '-') {
                        ...
                        if ((defptr = (struct symarray *)getaddr(argv[i])) ==
                            NULL) {
                                errmsg =
                                    strdup(GETMESSAGE(3, 1,
...
static int add_deflist(struct symarray *defptr, char *prefix) {
        ...
        if (!Deflist) {
                ...
                Deflist = (struct deflist *)realloc(
                    Deflist, (Ndeflist + 1) * sizeof(struct deflist));
        }
...
int do_finddef(int argc, char **argv) {
        unsigned long found;
        ...
        printerrf(argv[0], errmsg, argv[1], NULL, NULL, NULL, NULL, NULL, NULL,
                  NULL);
        free(errmsg);
        return (SH_FAIL);
}
...
static def_init(void) {
        ...
        defInited = 1;
        if (!(Dyndef =
                  (struct symarray *)malloc(20 * sizeof(struct symarray)))) {
