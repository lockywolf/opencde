...
#define MALMEMBERS                                                             \
        (4) /* initial number of members of malloc'ed array to malloc */
...
int (*find_special(int type, char *name))() {
        int i;
...
int set_special(char *name, int (*free)(), int (*parse)(), int (*print)()) {
        int i;
        ...
        if (i == Nspecs) {
                if (!Special) {
                        Special =
                            (struct special *)malloc(sizeof(struct special));
                        Special[0].name = strdup(name);
                        Nspecs = 1;
                } else {
                        Special = (struct special *)realloc(
                            Special, (Nspecs + 1) * sizeof(struct special));
...
int xk_parse(memtbl_t *tbl, char **buf, char *p, int nptr, int sub, void *pass,
             memtbl_t *(*tbl_find)()) {
        memtbl_t *ntbl;
        register int i = 0;
            ...
            (((UPPER((*buf)[0]) == 'P') && isdigit((*buf)[1])) ||
             ((*buf)[0] == '&'))) {
                ...
                if (Ndont == Sdont) {
                        if (Dont)
                                Dont = (char **)realloc(
                                    Dont, (Sdont + 20) * sizeof(char *));
                        else
                                Dont = (char **)malloc((Sdont + 20) *
                                                       sizeof(char *));
                        if (!Dont) {
        ...
        if (sub > 0 && tbl->subscr > 0) {
                if (_Prdebug) {
                        errmsg = strdup(GETMESSAGE(8, 2,
                                                   "xk_parse: Multiple array "
                                                   "subscripts are not handled "
                                                   "in '%s'\n"));
         ...
         * recursively.
         */
        if ((nptr > 1 && tbl->delim != 0) ||
            (nptr == 1 && tbl->delim == 0 && tbl->subscr == 0)) {
                if (PARPEEK(buf, ",") || PARPEEK(buf, Str_close_curly)) {
                        ((char **)p)[0] = NULL;
                        if (_Prdebug) {
                                errmsg =
                                    strdup(GetSharedMsg(DT_XK_PARSE_SET_NULL));
                                fprintf(stderr, errmsg, tbl->name);
                                free(errmsg);
                        }
                        return (SUCCESS);
                ...
                if (_Prdebug) {
                        errmsg = strdup(GETMESSAGE(8, 3,
                                                   "xk_parse: Setting '%s' to "
                                                   "malloc'ed address 0x%x, of "
                                                   "size %d\n"));
                        fprintf(stderr, errmsg, tbl->name, ((char **)p)[0],
                                tbl->size);
                        free(errmsg);
                }
                return (xk_parse(tbl, buf, ((char **)p)[0], nptr - 1 - tbl->ptr,
                                 sub, pass, tbl_find));
        }
        /*
        ...
        if (tbl->delim != 0 &&
            ((nptr == 1 && tbl->subscr == 0) ||
             (nptr == 0 && tbl->subscr != 0 && tbl->kind != K_STRING))) {
                ...
                if (tbl->subscr == 0) {
                        if (PARPEEK(buf, ",") ||
                            PARPEEK(buf, Str_close_curly)) {
                                ((char **)p)[0] = NULL;
                                if (_Prdebug) {
                                        errmsg = strdup(GETMESSAGE(
                                            8, 4,
                                            "xk_parse: Setting malloc'ed array "
                                            "'%s' to NULL\n"));
                                        fprintf(stderr, errmsg, tbl->name);
                                        free(errmsg);
                                }
                        ...
                        if (xk_parpeek(buf, "NULL")) {
                                ...
                                if (_Prdebug) {
                                        errmsg = strdup(
                                            GetSharedMsg(DT_XK_PARSE_SET_NULL));
                                        fprintf(stderr, errmsg, tbl->name);
                                        free(errmsg);
                                }
                                return (SUCCESS);
                        ...
                        if (_Prdebug) {
                                errmsg = strdup(
                                    GETMESSAGE(8, 5,
                                               "xk_parse: Setting member '%s' "
                                               "to malloc'ed "
                                               "pointer 0x%x, of size %d\n"));
                ...
                *buf += 1;
                ...
                while (PARPEEK(buf, Str_close_curly) == FALSE) {
                        if (tbl->subscr == 0 && i >= nmal) {
                                nmal += MALMEMBERS;
                                if ((np = realloc(np, nmal * tbl->size)) ==
                                    NULL) {
                                        return (FAIL);
                                }
                                ((char **)p)[0] = np;
                                if (_Prdebug) {
                                        ...
                                        fprintf(stderr, errmsg, tbl->name, nmal,
                                                nmal * tbl->size);
                                        free(errmsg);
                                }
                        ...
                        if (_Prdebug) {
                                errmsg = strdup(GETMESSAGE(
                                    8, 7,
                                    "xk_parse: Parsing array element [%d] of "
                                    "'%s' into address 0x%x\n"));
                        ...
                        if (i) {
                                ...
                                if (PARPEEK(buf, ",") == FALSE) {
                                        if (PARPEEK(buf, Str_close_curly) ==
                                            FALSE) {
                                                RIF(PAREXPECT(buf, ","));
                                                *buf += 1;
                                        }
                        ...
                        RIF(xk_parse(tbl, buf, &np[i * tbl->size],
                                     nptr ? -1 : 0, 0, (void *)-1, tbl_find));
                        i++;
                        struct_size = i;
                        xk_skipwhite(buf);
                }
        ...
        case K_STRUCT:
                ...
                *buf += 1;
                ...
                for (i = 0; ntbl[i].name != NULL; i++) {
                        ...
                        if (xk_parse(&ntbl[i], buf, p + ntbl[i].offset, nptr,
                                     sub, pass, tbl_find) == FAIL) {
                                if (_Prdebug) {
                                        errmsg = strdup(
                                            GetSharedMsg(DT_XK_PARSE_ERROR));
                                        fprintf(stderr, errmsg, ntbl[i].name);
                                        free(errmsg);
                                }
                                return (FAIL);
                ...
                for (i = 0; ntbl[i].name != NULL; i++) {
                        if (xk_parpeek(buf, ntbl[i].name) == TRUE) {
                                ...
                                if (_Prdebug) {
                                        errmsg = strdup(GETMESSAGE(
                                            8, 11,
                                            "xk_parse: Parsing union member "
                                            "'%s' into location 0x%x\n"));
                                        fprintf(stderr, errmsg, ntbl[i].name,
                                                p + ntbl[i].offset);
                                        free(errmsg);
                                }
                                if (xk_parse(&ntbl[i], buf, p, nptr, sub, pass,
                                             tbl_find) == FAIL) {
                                        if (_Prdebug) {
                                                errmsg = strdup(GetSharedMsg(
                                                    DT_XK_PARSE_ERROR));
...
int xk_get_delim(memtbl_t *tbl, char *p) {
        ...
        if (_Prdebug) {
                errmsg = strdup(GETMESSAGE(8, 12,
                                           "xk_get_delim: The delimiter for "
                                           "field '%s' is field '%s'\n"));
        ...
        default:
                if (_Prdebug) {
                        errmsg =
                            strdup(GETMESSAGE(8, 13,
                                              "xk_get_delim: Cannot find a "
                                              "delimiter value in '%s'\n"));
                        fprintf(stderr, errmsg, tbl->name);
                        free(errmsg);
                }
                return (0);
...
int xk_get_pardelim(memtbl_t *tbl, char *p) {
        memtbl_t *dtbl = &tbl[tbl->delim];
        char *errmsg;
        ...
        if (_Prdebug) {
                errmsg = strdup(GETMESSAGE(8, 14,
                                           "xk_get_pardelim: The delimiter for "
                                           "field '%s' is field '%s'\n"));
                fprintf(stderr, errmsg, tbl->name, dtbl->name);
                free(errmsg);
        }
        ...
        default:
                if (_Prdebug) {
                        errmsg =
                            strdup(GETMESSAGE(8, 15,
                                              "xk_get_pardelim: Cannot find a "
                                              "delimiter value in '%s'\n"));
                        fprintf(stderr, errmsg, tbl->name);
                        free(errmsg);
                }
...
int xk_print(memtbl_t *tbl, char **buf, char *p, int nptr, int sub, void *pass,
             memtbl_t *(*tbl_find)()) {
        memtbl_t *ntbl;
        register int i;
        ...
        if ((nptr > 1 && tbl->delim != 0) ||
            (nptr == 1 && tbl->delim == 0 && tbl->subscr == 0)) {
                ...
                return (xk_print(tbl, buf, ((char **)p)[0], nptr - 1 - tbl->ptr,
                                 sub, pass, tbl_find));
        }
        /*
         * If there is exactly one pointer level, or one subscripting level,
        ...
        if (tbl->delim != 0 &&
            ((nptr == 1 && tbl->subscr == 0) ||
             nptr == 0 && tbl->subscr != 0 && tbl->kind != K_STRING)) {
                ...
                if (_Prdebug) {
                        errmsg = strdup(GETMESSAGE(
                            8, 19, "xk_print: The delimiter for '%s' is %d\n"));
                ...
                *buf += lsprintf(*buf, Str_open_curly);
                for (i = 0; i < delim; i++) {
                        if (_Prdebug) {
                                errmsg = strdup(GETMESSAGE(
                                    8, 21,
                                    "xk_print: Printing array level %d of "
                                    "member '%s' at location 0x%x\n"));
                                fprintf(stderr, errmsg, i, tbl->name,
                                        &np[i * tbl->size]);
                                free(errmsg);
                        }
                        if (i)
                                *buf += lsprintf(*buf, ", ");
                        RIF(xk_print(tbl, buf, &np[i * tbl->size],
                                     nptr ? -1 : 0, 0, (void *)-1, tbl_find));
                }
        ...
        case K_STRING:
                if (delim > 0) {
                        if (tbl->subscr) {
                                ...
                                RIF(xk_prin_hexstr(buf, ((char **)p)[0],
                                                   delim));
                        }
                } else {
                        if (tbl->subscr) {
        ...
        case K_STRUCT:
                *buf += lsprintf(*buf, Str_open_curly);
                ...
                for (i = 0; ntbl[i].name != NULL; i++) {
                        ...
                        RIF(xk_print(&ntbl[i], buf, p + ntbl[i].offset, nptr,
                                     sub, pass, tbl_find));
                        ...
                        if (ntbl[i].choice == val) {
                                *buf += lsprintf(*buf, "%s ", ntbl[i].name);
                                if (_Prdebug) {
                                        errmsg = strdup(GETMESSAGE(
                                            8, 25,
                                            "xk_print: Printing union member "
                                            "'%s' into location 0x%x\n"));
                                        fprintf(stderr, errmsg, ntbl[i].name,
                                                p + ntbl[i].offset);
                                        free(errmsg);
                                }
                                RIF(xk_print(&ntbl[i], buf, p, nptr, sub, pass,
                                             tbl_find));
                                break;
                        }
                }
...
int xk_free(memtbl_t *tbl, char *p, int nptr, int sub,
            memtbl_t *(*tbl_find)()) {
        memtbl_t *ntbl;
        register int i;
        long val; /* used for choice selection */
        ...
        if ((nptr > 1 && tbl->delim != 0) ||
            (nptr == 1 && tbl->delim == 0 && tbl->subscr == 0)) {
                if (((char **)p)[0] != NULL) {
                        RIF(xk_free(tbl, ((char **)p)[0], nptr - 1 - tbl->ptr,
                                    sub, tbl_find));
                        free(((char **)p)[0]);
                        if (_Prdebug) {
                                errmsg = strdup(GETMESSAGE(
                                    8, 28,
                                    "xk_free: The member '%s' at location 0x%x "
                                    "was freed and set to NULL\n"));
                                fprintf(stderr, errmsg, tbl->name,
                                        ((char **)p)[0]);
                                free(errmsg);
                        }
                        ...
                        if (_Prdebug) {
                                errmsg = strdup(
                                    GETMESSAGE(8, 29,
                                               "xk_free: The member '%s' is "
                                               "NULL; no free occurred\n"));
                                fprintf(stderr, errmsg, tbl->name);
                                free(errmsg);
                        }
                }
        ...
        if (tbl->delim != 0 &&
            ((nptr == 1 && tbl->subscr == 0) ||
             nptr == 0 && tbl->subscr != 0 && tbl->kind != K_STRING)) {
                if (_Prdebug) {
                        errmsg = strdup(GETMESSAGE(
                            8, 30, "xk_free: The delimiter for '%s' is %d\n"));
                        fprintf(stderr, errmsg, tbl->name, delim);
                        free(errmsg);
                }
                if (tbl->subscr == 0)
                ...
                for (i = 0; i < delim; i++) {
                        if (_Prdebug) {
                                errmsg = strdup(GETMESSAGE(
                                    8, 31,
                                    "xk_free: Freeing array element [%d] of "
                                    "'%s' at address 0x%x\n"));
                                fprintf(stderr, errmsg, i, tbl->name,
                                        &np[i * tbl->size]);
                                free(errmsg);
                        }
                        RIF(xk_free(tbl, &np[i * tbl->size], nptr ? -1 : 0, 0,
                                    tbl_find));
                }
                if (tbl->subscr == 0) {
                        if (np != NULL) {
                                if (_Prdebug) {
                                        errmsg = strdup(GETMESSAGE(
                                            8, 32,
                                            "xk_free: Freeing pointer to array "
                                            "of '%s' at "
                                            "location 0x%x and setting to "
                                            "NULL\n"));
                                        fprintf(stderr, errmsg, tbl->name, np);
                                        free(errmsg);
                                }
                                ...
                                errmsg = strdup(GETMESSAGE(
                                    8, 33,
                                    "xk_free: The pointer to array of '%s'is "
                                    "NULL; no free occurred\n"));
                                fprintf(stderr, errmsg, tbl->name);
                                free(errmsg);
                        }
                ...
                if (!tbl->subscr) {
                        if (((char **)p)[0] != NULL) {
                                if (_Prdebug) {
                                        errmsg = strdup(GETMESSAGE(
                                            8, 34,
                                            "xk_free: Freeing string '%s' at "
                                            "location "
                                            "0x%x, and setting to NULL\n"));
        ...
        case K_STRUCT:
                ntbl = tbl_find(tbl->tname, tbl->tbl, tbl->id);
                for (i = 0; ntbl[i].name != NULL; i++) {
                        if ((ntbl[i].flags & F_SIMPLE) &&
                            nptr + ntbl[i].ptr == 0) {
                                if (_Prdebug) {
                                        errmsg = strdup(
                                            GetSharedMsg(DT_XK_FREE_NO_MEMBER));
                                        fprintf(stderr, errmsg, ntbl[i].name,
                                                p + ntbl[i].offset);
                                        free(errmsg);
                                }
                        ...
                        if (xk_free(&ntbl[i], p + ntbl[i].offset, nptr, sub,
                                    tbl_find) == FAIL) {
                                if (_Prdebug) {
                                        errmsg = strdup(GETMESSAGE(
                                            8, 37,
                                            "xk_free: A failure occurred while "
                                            "freeing the '%s' member\n"));
                                        fprintf(stderr, errmsg, ntbl[i].name);
                                        free(errmsg);
                                }
                                return (FAIL);
        ...
        case K_UNION:
                ...
                if (ntbl[i].name == NULL && _Prdebug)
                        if (_Prdebug) {
                                errmsg =
                                    strdup(GETMESSAGE(8, 40,
                                                      "xk_free: There is no "
                                                      "legal union choice for "
                                                      "'%s' (value is 0x%x); "
                                                      "no free occurred\n"));
