...
static void CvtStringToMWMValue(MWMTable *table, Display *dpy, XrmValuePtr args,
                                Cardinal *nargs, XrmValuePtr fval,
                                XrmValuePtr toval, XtPointer data);
static void CvtMWMValueToString(MWMTable *table, Display *dpy, XrmValuePtr args,
                                Cardinal *nargs, XrmValuePtr fval,
                                XrmValuePtr toval, XtPointer data);
...
void DtkshCvtWindowToString(XrmValuePtr args, Cardinal *nargs, XrmValuePtr fval,
                            XrmValuePtr toval) {
        ...
        if (fval->size != sizeof(Window)) {
                errmsg = strdup(GETMESSAGE(6, 1,
                                           "DtkshCvtWindowToString: The 'from' "
                                           "value is an invalid size"));
                XtWarning(errmsg);
                free(errmsg);
                toval->addr = NULL;
                toval->size = 0;
...
void DtkshCvtScreenToString(XrmValuePtr args, Cardinal *nargs, XrmValuePtr fval,
                            XrmValuePtr toval) {
        static char result[16];
        Screen *screen;
        char *errmsg;
        ...
        if (fval->size != sizeof(Screen *)) {
                errmsg = strdup(GETMESSAGE(6, 14,
                                           "DtkshCvtScreenToString: The 'from' "
                                           "value is an invalid size"));
                XtWarning(errmsg);
                free(errmsg);
                toval->addr = NULL;
...
void DtkshCvtStringToScreen(XrmValuePtr args, Cardinal *nargs, XrmValuePtr fval,
                            XrmValuePtr toval) {
...
void DtkshCvtStringToTopItemPosition(XrmValuePtr args, Cardinal *nargs,
...
void DtkshCvtHexIntToString(XrmValuePtr args, Cardinal *nargs, XrmValuePtr fval,
                            XrmValuePtr toval) {
        static char result[16];
        char *errmsg;
        ...
        if (fval->size != sizeof(long) && fval->size != sizeof(int) &&
            fval->size != sizeof(short) && fval->size != sizeof(char)) {
                errmsg = strdup(GETMESSAGE(6, 2,
                                           "DtkshCvtHexIntToString: The 'from' "
                                           "value is an invalid size"));
                XtWarning(errmsg);
                free(errmsg);
                toval->addr = NULL;
...
void DtkshCvtIntToString(XrmValuePtr args, Cardinal *nargs, XrmValuePtr fval,
                         XrmValuePtr toval) {
        static char result[16];
        char *errmsg;
...
void DtkshCvtBooleanToString(XrmValuePtr args, Cardinal *nargs,
                             XrmValuePtr fval, XrmValuePtr toval) {
        char *errmsg;
        ...
        if (fval->size != sizeof(int) && fval->size != sizeof(short) &&
            fval->size != sizeof(char)) {
                errmsg = strdup(GETMESSAGE(6, 4,
                                           "DtkshCvtBooleanToString: The "
                                           "'from' value is an invalid size"));
                XtWarning(errmsg);
                free(errmsg);
                toval->addr = NULL;
                toval->size = 0;
...
void DtkshCvtStringToPointer(XrmValuePtr args, Cardinal *nargs,
                             XrmValuePtr fval, XrmValuePtr toval) {
...
void DtkshCvtStringToWidget(Display *dpy, XrmValuePtr args, Cardinal *nargs,
                            XrmValuePtr fval, XrmValuePtr toval,
                            XtPointer data) {
        char *wname;
        static Widget wid;
        ...
        if (fval->size <= 0) {
                errmsg = strdup(GETMESSAGE(6, 5,
                                           "DtkshCvtStringToWidget: The 'from' "
                                           "value is an invalid size"));
                XtWarning(errmsg);
                free(errmsg);
                toval->addr = NULL;
         ...
         */
        {
                ...
                errmsg = strdup(GETMESSAGE(6, 6,
                                           "DtkshCvtStringToWidget: Unable to "
                                           "find a widget named '%s'"));
...
void DtkshCvtStringToCallback(Display *dpy, XrmValuePtr args, Cardinal *nargs,
                              XrmValuePtr fval, XrmValuePtr toval,
                              XtPointer data) {
        ...
        cdata =
            GetNewCBData((String)fval->addr, w, DTKSHConversionResource, None);
...
void DtkshCvtCallbackToString(Display *display, XrmValuePtr args,
                              Cardinal *nargs, XrmValuePtr fval,
                              XrmValuePtr toval, XtPointer converterData) {
        XtCallbackList cb;
        char buf[2048];
        register char *p;
        char *errmsg;
        ...
        p = &buf[0];
        *p = '\0';
        for (cb = ((XtCallbackList *)(fval->addr))[0]; cb->callback != NULL;
             cb++) {
                if (cb->callback == (XtCallbackProc)stdCB) {
                        dtksh_client_data_t *cdata =
                            (dtksh_client_data_t *)cb->closure;

                        if (p + strlen((String)cdata->ksh_cmd) + 1 - buf >
                            sizeof(buf)) {
...
void DtkshCvtWidgetToString(Display *dpy, XrmValuePtr args, Cardinal *nargs,
                            XrmValuePtr fval, XrmValuePtr toval,
                            XtPointer data) {
        char *wname;
        Widget widget;
        ...
        if (fval->size != sizeof(Widget) || fval->addr == NULL) {
                errmsg = strdup(GETMESSAGE(6, 9,
                                           "DtkshCvtWidgetToString: The 'from' "
                                           "value is an invalid size"));
                XtWarning(errmsg);
                free(errmsg);
                toval->addr = NULL;
        ...
        if ((w = widget_to_wtab(widget)) == NULL) {
                errmsg = strdup(GETMESSAGE(6, 10,
                                           "DtkshCvtWidgetToString: Unable to "
                                           "find a name for the widget"));
                XtWarning(errmsg);
                free(errmsg);
                toval->addr = NULL;
                toval->size = 0;
...
void DtkshCvtStringToEventMask(Display *dpy, XrmValuePtr args, Cardinal *nargs,
                               XrmValuePtr fval, XrmValuePtr toval,
                               XtPointer data) {
...
void DtkshCvtStringToListItems(Display *dpy, XrmValuePtr args, Cardinal *nargs,
                               XrmValuePtr fval, XrmValuePtr toval,
                               XtPointer data) {
        XtConvert(Toplevel, XtRString, fval, XmRXmStringTable, toval);
}

void DtkshCvtWidgetClassToString(Display *dpy, XrmValuePtr args,
                                 Cardinal *nargs, XrmValuePtr fval,
                                 XrmValuePtr toval, XtPointer data) {
        static char result[16];
        char *errmsg;
        ...
        if (fval->size != sizeof(WidgetClass)) {
                errmsg = strdup(GETMESSAGE(6, 11,
                                           "DtkshCvtWidgetClassToString: The "
                                           "'from' value is an invalid size"));
                XtWarning(errmsg);
                free(errmsg);
                toval->addr = NULL;
        ...
        errmsg = strdup(GETMESSAGE(
            6, 12, "DtkshCvtWidgetClassToString: Unknown widget class"));
        XtWarning(errmsg);
        free(errmsg);
        toval->addr = NULL;
        toval->size = 0;
...
void DtkshCvtStringToWidgetClass(Display *dpy, XrmValuePtr args,
                                 Cardinal *nargs, XrmValuePtr fval,
                                 XrmValuePtr toval, XtPointer data) {
        static char result[16];
        char *errmsg;
        ...
        /* No match found */
        errmsg = strdup(GETMESSAGE(
            6, 13, "DtkshCvtStringToWidgetClass: Unknown widget class name"));
        XtWarning(errmsg);
        free(errmsg);
        toval->addr = NULL;
        toval->size = 0;
...
void DtkshCvtStringToMWMDecoration(Display *dpy, XrmValuePtr args,
                                   Cardinal *nargs, XrmValuePtr fval,
                                   XrmValuePtr toval, XtPointer data) {
        CvtStringToMWMValue(mwmDecorations, dpy, args, nargs, fval, toval,
                            data);
}

void DtkshCvtMWMDecorationToString(Display *dpy, XrmValuePtr args,
                                   Cardinal *nargs, XrmValuePtr fval,
                                   XrmValuePtr toval, XtPointer data) {
        CvtMWMValueToString(mwmDecorations, dpy, args, nargs, fval, toval,
                            data);
}

void DtkshCvtStringToMWMFunctions(Display *dpy, XrmValuePtr args,
                                  Cardinal *nargs, XrmValuePtr fval,
                                  XrmValuePtr toval, XtPointer data) {
        CvtStringToMWMValue(mwmFunctions, dpy, args, nargs, fval, toval, data);
}

void DtkshCvtMWMFunctionsToString(Display *dpy, XrmValuePtr args,
                                  Cardinal *nargs, XrmValuePtr fval,
                                  XrmValuePtr toval, XtPointer data) {
        CvtMWMValueToString(mwmFunctions, dpy, args, nargs, fval, toval, data);
}

void DtkshCvtStringToPanedWinPosIndex(Display *dpy, XrmValuePtr args,
                                      Cardinal *nargs, XrmValuePtr fval,
                                      XrmValuePtr toval, XtPointer data) {
        static short value;
        char *pos;
...
void DtkshCvtPanedWinPosIndexToString(Display *dpy, XrmValuePtr args,
                                      Cardinal *nargs, XrmValuePtr fval,
                                      XrmValuePtr toval, XtPointer data) {
...
void DtkshCvtStringToPrintSetupProc(Display *dpy, XrmValuePtr args,
                                    Cardinal *nargs, XrmValuePtr fval,
                                    XrmValuePtr toval, XtPointer data) {
        static void (*proc)();
        ...
        if (w != NULL) {
                ...
                if (strcmp(DTKSHConversionResource,
                           CONSTCHAR "printerInfoProc") == 0) {
                        if (pinfo->printerInfoProcCommand != NULL)
                                XtFree(pinfo->printerInfoProcCommand);
                        pinfo->printerInfoProcCommand =
                            strdup((String)fval->addr);
                        proc = stdPrinterInfoProc;
                } else if (strcmp(DTKSHConversionResource,
                                  CONSTCHAR "selectFileProc") == 0) {
                        if (pinfo->selectFileProcCommand != NULL)
                                XtFree(pinfo->selectFileProcCommand);
                        pinfo->selectFileProcCommand =
                            strdup((String)fval->addr);
                        proc = stdSelectFileProc;
                } else if (strcmp(DTKSHConversionResource,
                                  CONSTCHAR "selectPrinterProc") == 0) {
                        if (pinfo->selectPrinterProcCommand != NULL)
                                XtFree(pinfo->selectPrinterProcCommand);
                        pinfo->selectPrinterProcCommand =
                            strdup((String)fval->addr);
                        proc = stdSelectPrinterProc;
                } else if (strcmp(DTKSHConversionResource,
                                  CONSTCHAR "setupProc") == 0) {
                        if (pinfo->setupProcCommand != NULL)
                                XtFree(pinfo->setupProcCommand);
                        pinfo->setupProcCommand = strdup((String)fval->addr);
                        proc = stdSetupProc;
                } else if (strcmp(DTKSHConversionResource,
                                  CONSTCHAR "verifyPrinterProc") == 0) {
                        if (pinfo->verifyPrinterProcCommand != NULL)
                                XtFree(pinfo->verifyPrinterProcCommand);
                        pinfo->verifyPrinterProcCommand =
                            strdup((String)fval->addr);
                        proc = stdVerifyPrinterProc;
                } else {
                        XtWarningMsg(CONSTCHAR "CvtStringToPrintSetupProc",
                                     CONSTCHAR
                                     "unsupported DtPrintSetupBox resource.",
                                     CONSTCHAR "XtToolkitError",
                                     "This resource is not currently supported "
                                     "by dtksh.",
                                     NULL, 0);
                }
        } else {
                XtWarningMsg(CONSTCHAR "CvtStringToPrintSetupProc",
                             CONSTCHAR "widget must exist.",
                             CONSTCHAR "XtToolkitError",
                             "This resource cannot be set at creation time by "
                             "dtksh, use "
                             "XtSetValues after creation instead.",
                             NULL, 0);
                toval->size = 0;
                toval->addr = NULL;
                return;
