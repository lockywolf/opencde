...
static int
    mk_remote_conf(/* list, locale, ims_name, status, confbuf, conflen */);
...
int put_remote_conf(locale, ims_name) char *locale, *ims_name;
{
        int ret;
        ...
        ret = mk_remote_conf(list, locale, ims_name, msg_status, confbuf,
                             &conflen);
        if (list) {
                clear_ImsList(list);
                FREE(list);
...
int get_remote_conf(listp, hostname, locale, ims_name) ImsList **listp;
char *hostname, *locale, *ims_name;
{
        ...
        if (!locale)
                locale =
                    userEnv.real_locale ? userEnv.real_locale : userEnv.locale;

        CDE_locale = userEnv.CDE_locale;

        if (CDE_locale)
                DPR2(("get_remote_conf(%s, %s, %s)\n", hostname, CDE_locale,
                      ims_name ? ims_name : "<all>"));
        else
                DPR2(("get_remote_conf(%s, %s, %s)\n", hostname, locale,
                      ims_name ? ims_name : "<all>"));
...
#define PUT_DATA(nm, val)                                                      \
        *bp++ = ' ', bp = strcpyx(bp, (nm)), *bp++ = '=',                      \
        bp = strcpyx(bp, (val)), *bp++ = '\n'

static int mk_remote_conf(list, locale, ims_name, status, confbuf,
                          conflen) ImsList *list;
int status;
...
int *conflen;
{
        ...
        bp = strcpyx(bp, "ImsList: ");
        bp = strcpyx(bp, locale);
        *bp++ = '\n';
...
static char *mk_ims_ent(bp, idx, ent) ImsEnt *ent;
int idx;
register char *bp;
{
        ImsConf *ims = ent->ims;
        ...
        *bp++ = '\n';
        ...
        if (ims->servername) {
                PUT_DATA("sn", ims->servername);
        }
        if (ims->servername2) {
                PUT_DATA("sN", ims->servername2);
        }
        if (ims->classname) {
                PUT_DATA("cn", ims->classname);
        }
        if (ims->property) {
                PUT_DATA("pp", ims->property);
        }
        if (ims->cmd_path) {
                PUT_DATA("cp", ims->cmd_path);
        }
        if (ims->cmd_param) {
                PUT_DATA("cr", ims->cmd_param);
        }
        if (ims->env_set) {
                PUT_DATA("es", ims->env_set);
        }
        if (ims->env_unset) {
                PUT_DATA("eu", ims->env_unset);
        }
        if (ims->env_pass) {
                PUT_DATA("ep", ims->env_pass);
        }
...
static int parse_ims_list(ptr, list) char *ptr;
ImsList *list;
{
        ...
        while (np = strchr(bp, '\n')) {
                ...
                *np = 0;
                if (strncmp(bp, "Ent-", 4) == 0) {
                        ...
                        vp = bp + 3;
                        /*list */
                        if (strncmp(bp, "ST", 2) == 0)
                                list->status = atoi(vp);
                        else if (strncmp(bp, "ne", 2) == 0)
                                list->num_ent = atoi(vp);
                        else if (strncmp(bp, "df", 2) == 0)
                                def_name = vp;
                        else if (strncmp(bp, "sm", 2) == 0)
                                list->def_selmode = atoi(vp);
#ifdef DEBUG
                        else if (!ent) {
                                DPR(("parse_ims_list(): ImsEnt: "
                                     "list->elist[%d]=%%x\n",
...
#endif
                        /* ent */
                        else if (strncmp(bp, "nm", 2) == 0) {
                                RENEWSTR(ent->name, vp);
                        } else if (strncmp(bp, "lb", 2) == 0) {
                                RENEWSTR(ent->label, vp);
                        } else if (strncmp(bp, "st", 2) == 0) {
                                ent->status = atoi(vp);
                        } else if (strncmp(bp, "fg", 2) == 0) {
                                ims->flags = atoi(vp);
                        } else if (strncmp(bp, "pr", 2) == 0) {
                                ims->protocols = atoi(vp);
                        } else if (strncmp(bp, "to", 2) == 0) {
                                ims->timeout = atoi(vp);
                        } else if (strncmp(bp, "it", 2) == 0) {
                                ims->interval = atoi(vp);
                        } else if (strncmp(bp, "sn", 2) == 0) {
                                RENEWSTR(ims->servername, vp);
                        } else if (strncmp(bp, "sN", 2) == 0) {
                                RENEWSTR(ims->servername2, vp);
                        } else if (strncmp(bp, "cn", 2) == 0) {
                                RENEWSTR(ims->classname, vp);
                        } else if (strncmp(bp, "pp", 2) == 0) {
                                RENEWSTR(ims->property, vp);
                        } else if (strncmp(bp, "cp", 2) == 0) {
                                RENEWSTR(ims->cmd_path, vp);
                        } else if (strncmp(bp, "cr", 2) == 0) {
                                RENEWSTR(ims->cmd_param, vp);
                        } else if (strncmp(bp, "es", 2) == 0) {
                                RENEWSTR(ims->env_set, vp);
                        } else if (strncmp(bp, "eu", 2) == 0) {
                                RENEWSTR(ims->env_unset, vp);
                        } else if (strncmp(bp, "ep", 2) == 0) {
                                RENEWSTR(ims->env_pass, vp);
                        } else {
        ...
        if (num_ent > 0) {
                for (i = 0; i < num_ent; i++) { /* check indispensable entry */
                        ...
                        if (ent->status == NoError)
                                ent->status =
                                    check_ims_conf(ent->ims, ent->name);
                }

                if (def_name) { /* set default_idx */
                        for (i = 0; i < num_ent; i++)
                                if (strcmp(list->elist[i]->name, def_name) ==
                                    0) {
                                        list->default_idx = i;
                                        break;
                                }
...
static int parse_remote_conf(listp, locale, confbuf, conflen) ImsList **listp;
char *locale, *confbuf;
int conflen;
{
        int ret = NoError;
...
int exec_remote_ims(sel) UserSelection *sel;
{
        int ret = NoError;
        int n, num_opts, binc;
        char *bp, *np;
    ...
    opts[n++] = bp; bp += binc = 1;
        ...
        if (ret == NoError) {
                put_xims_log("'%s' started for %s on %s.", sel->name,
                             userEnv.displayname, sel->hostname);
        }
...
int check_hostname(hostname) char *hostname;
{
                ...
                if (!local_addr) {
                        if ((hp = gethostbyname(local)) &&
                            hp->h_addrtype == AF_INET) {
                                ...
                                DPR(("check_hostname(%s)\tgethostbyname() "
                                     "failed\n",
                                     local));
...
int set_remote_confdata(confbuf, conflen) char *confbuf;
int conflen;
{
        char *av[2];
...
int read_remote_confdata(confbuf, conflen) char **confbuf;
int *conflen;
{
        char **av = NULL;
        int ac = 0;
...
static int prepare_action(act_typ, av, ac) int act_typ;
char **av;
int ac;
{
        int ret;
...
int change_window_status(status) int status;
{
        if (winEnv.atom_status == None || winEnv.atom_owner == None)
                return ErrInternal;
        ...
        (void)XChangeProperty(winEnv.Dpy, winEnv.atom_owner, winEnv.atom_status,
...
int set_window_data(ac, av) int ac;
char **av;
{
        register int i;
        register int nbytes;
        ...
        if (bp = buf = XtMalloc(nbytes)) { /* copy args into single buffer */
                ...
                (void)XChangeProperty(
                    winEnv.Dpy, winEnv.atom_owner, winEnv.atom_data, XA_STRING,
                    8, PropModeReplace, (unsigned char *)buf, nbytes);
                XSync(winEnv.Dpy, False);
                XtFree(buf);
        }
...
int get_window_data(acp, avp) int *acp;
char ***avp;
{
        int ac;
        char *data;
        ...
        if (read_property(winEnv.atom_data, XA_STRING, 8, True, &data, &len) !=
            True) {
                *acp = 0;
                *avp = NULL;
...
static int read_property(prop, type, format, del_flag, datapp, lenp) Atom prop;
Atom type;
int format;
int del_flag;
...
unsigned long *lenp;
{
        ...
        *datapp = NULL;
        ...
        (void)XGetWindowProperty(winEnv.Dpy, winEnv.atom_owner, prop, 0L,
                                 1000000L, del_flag, type, &realtype,
                                 &realformat, lenp, &bytesafter, datapp);
