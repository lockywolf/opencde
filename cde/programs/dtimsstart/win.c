...
static void
    add_cmd_btn(/* parent_rc, cb_ok, cb_clear, cb_cancel, cb_host, cb_help */);
/* host window */
...
int window_env_ok() { return winEnv.status != WIN_ST_NONE ? True : False; }

int init_window_env() {
        enum { XA_DTIMS_ATOM_MAIN,
               XA_DTIMS_ATOM_STATUS,
               XA_DTIMS_ATOM_DATA,
               NUM_ATOMS };
        static char *atom_names[] = {DTIMS_ATOM_MAIN, DTIMS_ATOM_STATUS,
                                     DTIMS_ATOM_DATA};
        ...
        if (winEnv.status != WIN_ST_NONE)
                return NoError;

        /* disable XIM on my own XmText* */
        putenv(
            "XMODIFIERS=@im=_XIMP_None"); /* putenv("XMODIFIERS=@im=none"); */
        ...
        topW = XtAppInitialize(&app, DTIMS_CLASS, options, XtNumber(options),
                               &Wargc, Wargv, fallbacks, (ArgList)NULL,
                               (Cardinal)0);
        ...
        XtGetApplicationResources(topW, &appres, app_resources,
...
static int ignoreBadWindow(dpy, error) Display *dpy;
XErrorEvent *error;
{
...
#ifdef unused
static void finish_window(w, end_window) Widget *w;
int end_window;
{
        if (*w) {
                XtUnmapWidget(*w);
    ...
    Window win;
{
        ...
        if (XGetSelectionOwner(winEnv.Dpy, winEnv.atom_main) ==
            winEnv.atom_owner)
                XSetSelectionOwner(winEnv.Dpy, winEnv.atom_main, None,
                                   CurrentTime);
        winEnv.atom_owner = XGetSelectionOwner(winEnv.Dpy, winEnv.atom_main);
...
int merge_RM(res1, res2) char *res1, *res2;
{
        ...
        if (isDT()) {
                ...
                if (is_executable(dtres_cmd)) {
                        sprintf(cmdbuf, "%s -merge -system -xdefaults ",
                                dtres_cmd);
                        if (res1) {
                                strcat(cmdbuf, " -file ");
                                strcat(cmdbuf, res1);
        ...
        if (!cmdbuf[0]) {
                sprintf(cmdbuf,
                        "%s %s %s 2>/dev/null | %s -merge -quiet - 2>/dev/null",
...
int restore_RM() {
	...
	{   char	buf[BUFSIZ];
        ...
        if (len > max) {
                XGrabServer(Dpy);
                do {
                        XChangeProperty(Dpy, root, XA_RESOURCE_MANAGER,
                                        XA_STRING, 8, mode, bp, len);
                        bp += max;
                        len -= max;
                        mode = PropModeAppend;
                } while (len > max);
        }
        XChangeProperty(Dpy, root, XA_RESOURCE_MANAGER, XA_STRING, 8, mode, bp,
                        len);
...
static void free_ims_list(list) ImsList *list;
{
...
static void finish_selection_window(end_window) int end_window;
{
...
static void done_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
        ...
        if (!canceled) { /* update userSel */
                ...
                if ((CurHost != sel->hostname) ||
                    (CurHost && sel->hostname &&
                     strcmp(sel->hostname, CurHost)) ||
                    sel->list != curList || sel->ims_idx != idx)
                        selection_changed = True;

                if (selection_changed == True) {
                        update_user_selection(sel, curList, idx, CurHost,
                                              CurHostType);
                        curList = (ImsList *)0;
                }
        }
...
static void select_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
...
static void host_btn_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{ start_host_window(CurHost); }

static void help_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
...
static int create_selection_window() {
        ...
        i = 0;
        XtSetArg(args[i], XmNmwmFunctions, WIN_FUNC);
        i++;
        XtSetArg(args[i], XmNmwmDecorations, WIN_DECR);
        i++;
        ...
        i++;
        SelW = form = XmCreateFormDialog(TopW, "Selection", args, i);
        ...
        if (RemoteOn()) {
                XtSetArg(args[i], XmNtopWidget, w_tbl[n]);
                j = i + 1;
                XtSetArg(args[j], XmNorientation, XmHORIZONTAL);
                j++;
                XtSetArg(args[j], XmNpacking, XmPACK_TIGHT);
                j++;
                XtSetArg(args[j], XmNisAligned, TRUE);
                j++;
                XtSetArg(args[j], XmNentryAlignment, XmALIGNMENT_BEGINNING);
                j++;
                XtSetArg(args[j], XmNentryBorder, 0);
                j++;
                XtSetArg(args[j], XmNmarginHeight, 1);
                j++;
                XtSetArg(args[j], XmNmarginWidth, 20);
                j++;
                HostRC = w_tbl[++n] = host_rc =
                    XmCreateRowColumn(form, "host_rc", args, j);
        }
        ...
        XtSetArg(args[j], XmNorientation, XmHORIZONTAL);
        j++;
        XtSetArg(args[j], XmNpacking, XmPACK_COLUMN);
        j++;
        XtSetArg(args[j], XmNnumColumns, curList->num_ent);
        j++;
        XtSetArg(args[j], XmNisAligned, TRUE);
        j++;
        XtSetArg(args[j], XmNentryAlignment, XmALIGNMENT_BEGINNING);
        ...
        XtSetArg(args[j], XmNentryBorder, 0);
        j++;
        XtSetArg(args[j], XmNmarginHeight, 10);
        j++;
        /* XtSetArg(args[j], XmNmarginWidth, 40); j++; */
        SelRC = w_tbl[++n] = sel_rc =
            XmCreateRadioBox(form, "select_rc", args, j);
        ...
        add_cmd_btn(cmd_rc, done_cb, 0, done_cb, RemoteOn() ? host_btn_cb : 0,
                    help_cb);
...
static int change_host(new_host, host_type) char *new_host;
int host_type;
{
        int ret = NoError;
...
static void add_btn_trans(btn) Widget btn;
{
...
static void change_ims_list(last_ims_name, init_idx) char *last_ims_name;
int init_idx;
{
        int idx;
        int i, j;
...
#define IMS_OK(i) (curList->elist[i]->status == NoError)
        ...
        if (idx < 0) {
                if (init_idx >= 0 && init_idx < curList->num_ent &&
                    IMS_OK(init_idx)) {
                        idx = init_idx;
                } else {
                        idx = curList->default_idx;
                        if (!(idx >= 0 && idx < curList->num_ent &&
                              IMS_OK(idx))) {
        ...
        for (j = 0; j < curList->num_ent; j++) {
                ...
                i = 0;
                XtSetArg(args[i], XmNset, j == idx ? True : False);
                i++;
                XtSetArg(args[i], XmNsensitive, IMS_OK(j));
                i++;
                XtSetArg(args[i], XmNlabelString, str);
                i++;
                XtSetArg(args[i], XmNspacing, 10);
                i++;
                if (j < num_chld) {
                        XtSetValues(tb[j], args, i);
                } else {
                        tb[j] = createTB(SelRC, "ims", args, i);
                        XtAddCallback(tb[j], XmNvalueChangedCallback, select_cb,
                                      (XtPointer)j);
                        add_btn_trans(tb[j]);
                }
                XmStringFree(str);
...
static void set_host_label(host_type, hostname) int host_type;
char *hostname;
{
        ...
        if (host_type == HOST_REMOTE) { /* change HostLabel & manage HostRC */
                ...
                if (!lb_host) {
                        ...
                        XtSetArg(args[n], XmNmarginLeft, 100);
                        n++;
                        lb = XmCreateLabelGadget(HostRC, "host_label", args, n);
                        lb_host =
                            XmCreateLabelGadget(HostRC, "hostname", NULL, 0);
                        XtManageChild(lb);
...
static void add_cmd_btn(parent_rc, cb_ok, cb_clear, cb_cancel, cb_host,
                        cb_help) Widget parent_rc;
void (*cb_ok)(), (*cb_cancel)(), (*cb_clear)(), (*cb_host)(), (*cb_help)();
{
        Widget pb[4];
        Arg args[12];
        int n, nbtn;
        ...
        if (cb_ok) {
                ...
                XtAddCallback(pb[nbtn], XmNactivateCallback, cb_ok,
                              (XtPointer)OK_BTN);
                FocusW = pb[nbtn];
        ...
        if (cb_clear) {
                ...
                XtAddCallback(pb[nbtn], XmNactivateCallback, cb_clear,
                              (XtPointer)CLEAR_BTN);
                nbtn++;
        }
        if (cb_cancel) {
                pb[nbtn] = createPB(parent_rc, "Cancel", args, n);
                XtAddCallback(pb[nbtn], XmNactivateCallback, cb_cancel,
                              (XtPointer)CANCEL_BTN);
                nbtn++;
        }
        if (cb_host) {
                pb[nbtn] = createPB(parent_rc, "ChangeHost", args, n);
                XtAddCallback(pb[nbtn], XmNactivateCallback, cb_host,
                              (XtPointer)HOST_BTN);
                nbtn++;
        }
        if (cb_help) {
                pb[nbtn] = createPB(parent_rc, "Help", args, n);
                XtAddCallback(pb[nbtn], XmNactivateCallback, cb_help,
                              (XtPointer)HELP_BTN);
                nbtn++;
        }
#ifdef SelectByReturn
...
static void start_host_window(cur_host) char *cur_host;
{
        int ret;
...
static void host_done_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
...
static void host_clear_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
        /* if (!HostText)	return; */
...
static void host_done_action(w, ev, args, num_args) Widget w;
XEvent *ev;
String *args;
Cardinal *num_args;
{ host_done_cb(w, (XtPointer)OK_BTN, 0); }

static void create_host_window(cur_host) char *cur_host;
{
        ...
        i++;
        HostW = form = XmCreateFormDialog(TopW, "Host", args, i);
        ...
        XtSetArg(args[i], XmNtopWidget, w_tbl[n]);
        j = i + 1;
        XtSetArg(args[j], XmNmarginHeight, 8);
        j++;
        XtSetArg(args[j], XmNmarginWidth, 8);
        j++;
        w_tbl[++n] = host_desc =
            XmCreateLabelGadget(form, "host_desc", args, j);
        ...
        nhw = 0;
        j = 0;
        XtSetArg(hargs[j], XmNalignment, XmALIGNMENT_BEGINNING);
        j++;
        hw[nhw++] = host_lb =
            XmCreateLabelGadget(host_rc, "host_label", hargs, j);
        j = 0;
        /* XtSetArg(hargs[j], XmNcolumns, TEXT_COLUMNS); j++; */
        /* XtSetArg(hargs[j], XmNmaxLength, MAX_HOSTNAME_LEN); j++; */
        XtSetArg(hargs[j], XmNeditable, True);
        j++;
        XtSetArg(hargs[j], XmNvalue, cur_host);
        j++;
        XtSetArg(hargs[j], XmNmarginHeight, 1);
        j++;
        XtSetArg(hargs[j], XmNmarginWidth, 1);
        j++;
        HostText = hw[nhw++] = host_txt =
            XmCreateTextField(host_rc, "host_text", hargs, j);
...
int start_mode_window(cur_mode) int cur_mode;
{
        int ret;
        ...
        /* handle system Menu's "close" */
        XmAddWMProtocolCallback(
            XtParent(ModeW), XmInternAtom(Dpy, "WM_DELETE_WINDOW", True),
...
static void finish_mode_window(end_window) int end_window;
{
        if (ModeW) {
...
static void mode_done_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
        int ret = NoError;
        int canceled = (int)client_data == CANCEL_BTN;
...
static void mode_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
...
static void mode_help_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{ ximsHelp(HELP_MODE); }

static int create_mode_window(cur_mode) int cur_mode;
{
        ...
        XtSetArg(args[i], XmNmappedWhenManaged, False);
        i++;
        ModeW = form = XmCreateFormDialog(TopW, "Mode", args, i);
        ...
        XtSetArg(args[j], XmNmarginHeight, 10);
        j++;
        XtSetArg(args[j], XmNmarginWidth, 30);
        j++;
        ...
        XtSetArg(args[j], XmNpacking, XmPACK_COLUMN);
        j++;
        XtSetArg(args[j], XmNnumColumns, NUM_SEL_MODE);
        j++;
        w_tbl[++n] = mode_rc = XmCreateRadioBox(form, "mode_rc", args, j);
        {
                ...
                k = 0;
                XtSetArg(bargs[k], XmNspacing, 10);
                k++;
                tb[0] = createTB(mode_rc, "button_0", bargs, k);
                tb[1] = createTB(mode_rc, "button_1", bargs, k);
                for (k = 0; k < NUM_SEL_MODE; k++) {
                        XtVaSetValues(tb[k], XmNset,
                                      k == set_idx ? True : False, NULL);
                        XtAddCallback(tb[k], XmNvalueChangedCallback, mode_cb,
                                      (XtPointer)k);
                        add_btn_trans(tb[k]);
                }
...
#else
        /* use TBGadget */
        XtSetArg(args[j], XmNbuttonCount, NUM_SEL_MODE);
        j++;
        XtSetArg(args[j], XmNbuttonSet, cur_mode == SEL_MODE_AUTO ? 1 : 0);
        j++;
        XtSetArg(args[j], XmNsimpleCallback, mode_cb);
        j++;
...
static void help_ok(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{ stop_help(); }
...
void ximsHelp(help_type) int help_type;
{
        char *msg = NULL;
        XmString str;
        ...
        if (help_type != last_help_type) {
                ...
                switch (help_type) {
                case HELP_MODE:
                        msg = appres.modeHelpMsg;
                        break;
                default:
                case HELP_SELECTION:
                        msg = appres.selectionHelpMsg;
                        break;
                }
...
#ifdef DEBUG
                        if (DebugLvl > 0)
                                msg = "help message not defined in resource "
                                      "file\n";
                        else
#endif
                                return;
...
static void dialog_resp_cb(w, client_data, call_data) Widget w;
XtPointer client_data, call_data;
{
...
#ifdef unused
static int wait_confirmation(w) Widget w;
{
...
int put_msg_win(type, msg) int type;
char *msg;
{
        int reply;
        Widget btn;
        ...
        if (!MsgW) { /* create message dialog */
                ...
                XtUnmanageChild(
                    XmMessageBoxGetChild(MsgW, XmDIALOG_HELP_BUTTON));
                XtAddCallback(MsgW, XmNokCallback, dialog_resp_cb,
                              (XtPointer)MsgW);
                XtAddCallback(MsgW, XmNcancelCallback, dialog_resp_cb,
                              (XtPointer)MsgW);
        ...
        str = XmStringCreateLocalized(msg);
        XtVaSetValues(
            MsgW, XmNdialogType, type, XmNmessageString, str, XmNdialogStyle,
        ...
        if (wait_resp) {
#ifdef unused
                DPR3(
                    ("wait_dialog_resp(START): WaitingDialogReply ==> True\n"));
                ...
                WaitingDialogReply = False;
                DPR3(
                    ("wait_dialog_resp(DONE): WaitingDialogReply ==> False\n"));
#endif /* unused */
        } else {
...
static int window_location(loc_str) char *loc_str;
{
        ...
        if (p = lower_str) {
                to_lower_str(p);
                if (strstr(lower_str, "center"))
                        locate_type |= LOC_CENTER;
                if (strstr(lower_str, "top"))
                        locate_type |= LOC_TOP;
                if (strstr(lower_str, "bottom"))
                        locate_type |= LOC_BOTTOM;
                if (strstr(lower_str, "left"))
                        locate_type |= LOC_LEFT;
                if (strstr(lower_str, "right"))
                        locate_type |= LOC_RIGHT;
                FREE(lower_str);
        }
        return locate_type;
}

static void locate_window(w) Widget w;
{
        int scr;
        int x, y;
        int dpy_w, dpy_h;
        ...
        x = y = 0;
        if (locate_type) {
                if (locate_type & LOC_TOP)
                        y = LOC_MARGIN;
                if (locate_type & LOC_BOTTOM)
                        y = dpy_h - height - LOC_MARGIN * 2;
                if (locate_type & LOC_LEFT)
                        x = LOC_MARGIN;
                if (locate_type & LOC_RIGHT)
                        x = dpy_w - width - LOC_MARGIN * 2;
        }
        if (!x)
                x = (int)(dpy_w - width - LOC_MARGIN * 2) / 2;
        if (!y)
                y = (int)(dpy_h - height - LOC_MARGIN * 2) / 2;
...
static void set_cursor(w, is_wait) Widget w;
int is_wait;
{
        ...
        if (cursors[idx] == None) {
                cursors[idx] =
                    XCreateFontCursor(Dpy, idx ? XC_watch : XC_left_ptr);
        }
...
static void xt_timer_cb(client_data, timer_id) XtPointer client_data;
