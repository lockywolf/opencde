...
int find_CDE_locale_name() {
        ...
        if (first) {
                ...
                if (ret == NoError)
                        ret = _DtLcxXlateOpToStd(
                            db, plat, compver, DtLCX_OPER_SETLOCALE,
                            uenv->locale, &uenv->CDE_locale, NULL, NULL, NULL);
                if (ret == NoError)
...
char *find_system_locale_name(CDE_locale) char *CDE_locale;
{
        _DtXlateDb db = NULL;
        int ret = NoError;
        char plat[_DtPLATFORM_MAX_LEN];
        ...
        if (ret == NoError)
                ret =
                    _DtLcxXlateStdToOp(db, plat, compver, DtLCX_OPER_SETLOCALE,
                                       CDE_locale, NULL, NULL, NULL, &locale);
        if (ret == NoError)
                ret = _DtLcxCloseDb(&db);
...
char *strcpyx(dest, src) register char *dest, *src;
{
        while (*dest++ = *src++)
                ;
        return (--dest);
...
#ifdef unused
char *strcpy2(dest, src1, src2) register char *dest, *src1, *src2;
{
        while (*dest++ = *src1++)
                ;
        --dest;
...
char *newstradded(src1, src2, src3) register char *src1, *src2, *src3;
{
        register char *p;
        char *dest;
...
int str_to_int(ptr, val) char *ptr;
int *val;
{
        int base;
        char *pp;
...
bool str_to_bool(ptr, def_val) char *ptr;
bool def_val;
{
        if (!ptr || !*ptr)
                return def_val;
...
char **parse_strlist(ptr, sep_ch) register char *ptr;
char sep_ch;
{
...
#ifdef unused
int pack_strlist(ptr, listp, sep_ch) char *ptr, **listp;
char sep_ch;
{
...
bool parse_dpy_str(display_str, host, dnum, snum, dnet) char *display_str,
    **host;
int *dnum, *snum, *dnet;
{
...
char *std_dpy_str(display_str, snum) char *display_str;
int *snum;
{
...
int make_user_dir(path) char *path;
{
...
int is_directory(path, must_writable) char *path;
int must_writable;
{
        struct stat stb;
        int mode = R_OK | X_OK;
...
#ifdef unused
int is_regularfile(path) char *path;
{
        struct stat stb;
        return stat(path, &stb) == 0 && S_ISREG(stb.st_mode);
}

int is_emptyfile(path) char *path;
{
        struct stat stb;
        return stat(path, &stb) == 0 && S_ISREG(stb.st_mode) &&
               stb.st_size == 0;
...
int is_executable(path) char *path;
{
        struct stat stb;
        return stat(path, &stb) == 0 && S_ISREG(stb.st_mode) &&
...
#ifdef unused
int is_writable(path) char *path;
{
...
int is_readable(path, allow_empty) char *path;
int allow_empty;
{
        struct stat stb;
        return stat(path, &stb) == 0 && S_ISREG(stb.st_mode) &&
...
int start_tag_line(fname) char *fname;
{
        if (fname) {
                if (!tag_linebuf)
...
int read_tag_line(fp, tagp, valp) FILE *fp;
char **tagp, **valp;
{
        char *lp, *lp2;
...
void put_xims_msg(msg_type, err_num, arg1, arg2, arg3) int msg_type, err_num;
void *arg1, *arg2, *arg3;
{
        int ret = -2;
        char *msg;
...
void put_xims_errmsg(err_num, arg1, arg2, arg3) int err_num;
void *arg1, *arg2, *arg3;
{
        if (Verbose > 0)
...
void put_xims_warnmsg(err_num, arg1, arg2, arg3) int err_num;
void *arg1, *arg2, *arg3;
{
        if (Verbose > 0)
...
#ifdef DEBUG
/* for DPR*() macro */
void print_msg(fmt, arg1, arg2, arg3, arg4, arg5) char *fmt;
void *arg1, *arg2, *arg3, *arg4, *arg5;
{ fprintf(LogFp, fmt, arg1, arg2, arg3, arg4, arg5); }
#endif

int put_xims_log(fmt, arg1, arg2, arg3) char *fmt;
void *arg1, *arg2, *arg3;
{
        FILE *fp;
        ...
        if (fp = fopen(logpath, "a")) {
                ...
                if (strftime(time_str, 80, /* "%x %X" */ "%D %T",
                             localtime(&tm)) == 0)
                        time_str[0] = 0;
...
void clear_ImsConf(ims) ImsConf *ims;
{
        FREE(ims->servername);
...
void clear_ImsEnt(ent) ImsEnt *ent;
{
        FREE(ent->name);
        FREE(ent->fname);
        FREE(ent->label);
...
void clear_ImsList(list) ImsList *list;
{
        int i;
...
void clear_FileSel(fsel) FileSel *fsel;
{
        FREE(fsel->name);
        FREE(fsel->hostname);
...
void clear_UserSelection(sel) UserSelection *sel;
{
        FREE(sel->name);
        FREE(sel->hostname);
        if (sel->fsel) {
...
void clear_RunEnv(renv) RunEnv *renv;
{
        FREE(renv->im_mod);
        FREE(renv->atom_name);
        FREE(renv->cmdbuf);
...
void clear_OutEnv(oenv) OutEnv *oenv;
{
        EnvEnt *ep;
...
void clear_WinEnv() { CLR(&winEnv, WinEnv); }

void clear_CmdOpt() { CLR(&Opt, CmdOpt); }
...
void pr_FileSel(fsel) FileSel *fsel;
{
        fprintf(LogFp,
                "FileSel:\tdpy_specific=%d  real_fname=%s  select_mode=%d\n",
                fsel->dpy_specific, fsel->real_fname, fsel->select_mode);
        fprintf(LogFp,
                "\tname=%s  host=%s  iconic=%d  com_opt='%s'  opts=%#x\n",
                fsel->name, fsel->hostname, fsel->iconic, fsel->com_opt,
                fsel->opts);
        ...
        if (fsel->opts) {
                ...
                for (op = fsel->opts; *op; op++)
                        fprintf(LogFp, "\t  opts[%d] <%s>\t'%s'\n",
                                op - fsel->opts, (*op)->ims_name,
                                (*op)->opt_str);
        }
}

void pr_UserSelection(sel) UserSelection *sel;
{
...
void pr_ImsConf(conf, ims_name) ImsConf *conf;
char *ims_name;
{
        char **p;
        ...
        fprintf(LogFp, "\tproperty=%s  servername=%s (%s) class=%s\n",
                conf->property, conf->servername, conf->servername2,
                conf->classname);
...
void pr_ImsEnt(ent, idx) ImsEnt *ent;
int idx;
{
...
void pr_ImsList(list) ImsList *list;
{
        int i;
        ImsEnt *ent;

        fprintf(
            LogFp, "ImsList:\tstatus=%d  num_ent=%d  def_idx=%d  def_sel=%d\n",
            list->status, list->num_ent, list->default_idx, list->def_selmode);
        for (i = 0; i < list->num_ent; i++) {
                ent = list->elist[i];
                pr_ImsEnt(ent, i);
                if (ent->ims)
                        pr_ImsConf(ent->ims, ent->name);
        }
}

void pr_WinEnv(wenv) WinEnv *wenv;
{
        ...
        fprintf(LogFp, "\tatom_owner=%#x  atom: main=%d status=%d data=%d\n",
                wenv->atom_owner, wenv->atom_main, wenv->atom_status,
                wenv->atom_data);
}

void pr_RunEnv(renv) RunEnv *renv;
{
        fprintf(LogFp, "RunEnv:\tstatus=%s  is_remote=%d  proto=%d\n",
                error_name(renv->status), renv->is_remote,
                proto_name(renv->proto));
...
void pr_OutEnv(oenv) OutEnv *oenv;
{
        EnvEnt *ep;
...
void pr_CmdConf() {
        ...
        fprintf(LogFp, "\tImXmod:\tXIM=%s  Ximp=%s  Xsi=%s  Xhp=%s  None=%s\n",
                ...
                conf->xmod[Proto_Xsi], conf->xmod[Proto_Xhp],
                conf->atom[Proto_None]);
        fprintf(LogFp, "\tImAtom:\tXIM=%s  Ximp=%s  Xsi=%s  Xhp=%s  None=%s\n",
                conf->atom[Proto_XIM], conf->atom[Proto_Ximp],
                conf->atom[Proto_Xsi], conf->atom[Proto_Xhp],
                conf->atom[Proto_None]);
#else
...
void pr_OpModeFlag() {
        ...
        if (OpFlag) {
                if (OpFlag & FLAG_ENV)
                        fputs("Env, ", LogFp);
                if (OpFlag & FLAG_NOSAVE)
                        fputs("NoSave, ", LogFp);
                if (OpFlag & FLAG_NOSTART)
                        fputs("NoStart, ", LogFp);
                if (OpFlag & FLAG_NOWAIT)
                        fputs("NoWait, ", LogFp);
                if (OpFlag & FLAG_NOTIMEOUT)
                        fputs("NoTimeout, ", LogFp);
                if (OpFlag & FLAG_NORESOURCE)
                        fputs("NoResource, ", LogFp);
                if (OpFlag & FLAG_NOREMOTE)
                        fputs("NoRemote, ", LogFp);
                if (OpFlag & FLAG_NOTIFY)
                        fputs("Notify, ", LogFp);
                if (OpFlag & FLAG_CONNECT)
                        fputs("Connect, ", LogFp);
                if (OpFlag & FLAG_REMOTERUN)
                        fputs("RemoteRun, ", LogFp);
                if (OpFlag & FLAG_WINDOW)
                        fputs("Window, ", LogFp);
                if (OpFlag & FLAG_DT)
                        fputs("Dt, ", LogFp);
#ifdef old_hpux
...
char *StateName() {
        return (OpState >= 0 && OpState < NumStates) ? StateNameTbl[OpState]
                                                     : "<unknown>";
}

/* ********  error name  ******** */

char *error_name(error) ximsError error;
{
        char *name = "<unknown>";
...
char *proto_name(proto_idx) int proto_idx;
{
...
char *sig_name(sig) int sig;
{ return (sig > 0 && sig <= MaxSigNum) ? SigNames[sig - 1] : "<unknown>"; }

void pr_brk(msg) char *msg;
{
