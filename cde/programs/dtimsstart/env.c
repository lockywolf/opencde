...
int expand_string(in_str, out_str, out_len, ims) char *in_str, *out_str;
int out_len;
ImsConf *ims;
{
        ...
        while (*p && out_len > 0) {
                ...
                if (*p == '%') {
                        ...
                        case 'c':
                                ...
                                break;
                        case 's':
                                sprintf(str, "%ld", (long)uenv->screen_num);
                                if (str[0] != '\0')
                                        ep = str;
                                break;
                        case 'r':
                        ...
                        case '%':
                                p++;
                        default:
                                DPR2(("expand_string: '%%%c' unknown\n"));
                                ep = 0;
                                break;
                        }
                        if (ep) {
                                if ((out_len -= (int)strlen(ep)) <= 0)
                                        break;
                                q = strcpyx(q, ep);
                                p += 2;
                                continue;
...
static int shell_type(shell) char *shell;
{
        char *p;
        int len;
...
int make_new_environ(oenv, sel) OutEnv *oenv;
UserSelection *sel;
{
        ImsConf *ims;
        EnvEnt *ep, *ep2;
        ...
        setp = unsetp = 0;
        if (ims->env_set)
                setp = parse_strlist(ims->env_set, ' ');
        if (ims->env_unset)
                unsetp = parse_strlist(ims->env_unset, ' ');

        /* set: ims->env_set, XMODIFIERS & X?INPUT */
        num = 0;
        if (setp)
                for (pp = setp; *pp; pp++, num++)
                        ;
        ...
        if (!xinput_done && (proto == Proto_Xhp)) {
                ...
                if (!ims->servername) {
                        DPR((
                            "make_new_environ(): '%s' servername not defined\n",
                            sel->name));
                }
#endif
...
int set_remote_env(ptr, env_pass) char *ptr, *env_pass;
{
        char *bp = ptr, *ep;
        char **ls, **ls2, **pp, **pp2;
        bool dup_ent;
...
#ifdef old_hpux
char *xhp_xinput_name(locale) char *locale;
{
        ...
        if (!xhp)
                return NULL;
        if (!locale)
                locale = userEnv.locale;

        for (i = 0; !typ && i < XHP_LANG_NUM; i++) {
                if (!(ls = parse_strlist(xhp[i].locales, ' ')))
                        continue;
                for (pp = ls; *pp; pp++) {
                        if (locale[0] != (*pp)[0])
                                continue;
...
char *get_real_locale(locale, aliases) char *locale, ***aliases;
{
        int i;
        int match_idx = -1;
        ...
        DPR(("get_real_locale(%s): real_locale=%s  aliases=%s\n", locale,
             real_locale, aliases ? *aliases : NULL));
