...
int init_log_file(org_path, check_size) char *org_path;
int check_size;
{
        char path[MAXPATHLEN];
        ...
        if (log_exists) {
                ...
                if (check_size && stb.st_size > MAX_LOGSIZE) {
                        ...
                        if (rename(path, save_path) == 0) {
                                ...
                                DPR((
                                    "init_log_file(): rename(to: %s) failed.\n",
                                    save_path));
                        }
                }
        }
...
int set_errorlog(path) char *path;
{
        int fd;
...
_default:
        ...
        if (!conf->imsConfDir)
                conf->imsConfDir = NEWSTR(conf_dir);
        if (!conf->imsAppDir)
                conf->imsAppDir = NEWSTR(DTIMS_APPDIR);
        if (!conf->imsLogDir)
                conf->imsLogDir = NEWSTR(DTIMS_LOGDIR);
        if (!conf->imsLogFile)
                conf->imsLogFile = NEWSTR(DTIMS_LOGFILE);
        if (!conf->imsDir)
                conf->imsDir = NEWSTR(DTIMS_IMSDIR);
        if (!conf->userImsDir)
                conf->userImsDir = NEWSTR(DTIMS_USRIMSDIR);
        if (!conf->userTmpDir)
                conf->userTmpDir = NEWSTR(DTIMS_USRTMPDIR);
        if (!conf->userAltDir)
                conf->userAltDir = NEWSTR(DTIMS_USRALTDIR);
        if (!conf->dt)
                conf->dt = ALLOC(1, DtEnv);
        if (!conf->dt->confDir)
                conf->dt->confDir = NEWSTR(DT_CONFDIR);
        if (!conf->dt->userDir)
                conf->dt->userDir = NEWSTR(DT_USERDIR);
#ifdef old_hpux
        if (!conf->vue)
                conf->vue = ALLOC(1, VueEnv);
        if (!conf->vue->confDir)
                conf->vue->confDir = NEWSTR(VUE_CONFDIR);
        if (!conf->vue->userDir)
                conf->vue->userDir = NEWSTR(VUE_USERDIR);
#endif /* old_hpux */
        if (!conf->xmod[Proto_XIM])
                conf->xmod[Proto_XIM] = NEWSTR(IM_XMOD_XIM);
        if (!conf->xmod[Proto_Ximp])
                conf->xmod[Proto_Ximp] = NEWSTR(IM_XMOD_XIMP);
        if (!conf->xmod[Proto_Xsi])
                conf->xmod[Proto_Xsi] = NEWSTR(IM_XMOD_XSI);
        if (!conf->atom[Proto_XIM])
                conf->atom[Proto_XIM] = NEWSTR(IM_ATOM_XIM);
        if (!conf->atom[Proto_Ximp])
                conf->atom[Proto_Ximp] = NEWSTR(IM_ATOM_XIMP);
        if (!conf->atom[Proto_Xsi])
                conf->atom[Proto_Xsi] = NEWSTR(IM_ATOM_XSI);
...
int read_imsconf(conf, ims_name, ims_fname) ImsConf *conf;
char *ims_name, *ims_fname;
{
        ...
        while ((line_num = read_tag_line(fp, &lp, &valp)) > 0) {
                ...
                if (strcmp(lp, "name") == 0)
                        ...
                        if (p = strchr(valp,
                                       ',')) { /* contain secondary names */
                                conf->servername2 =
                                    NEWSTR(valp); /* save full name */
                                *p = 0;
                        }
                        conf->servername =
                            NEWSTR(valp); /* save primary name only */
                ...
                /* TYP_NUMERIC */
                        ...
                        /* TYP_BOOL */
#define SET_FLAG(f)                                                            \
        (str_to_bool(valp, False) ? (conf->flags |= (f))                       \
                                  : (conf->flags &= ~(f)))
...
int check_ims_conf(ims, ims_name) ImsConf *ims;
char *ims_name;
{
        int ret = NoError;
        char *missing_entry = NULL;

        if (strcmp(ims_name, NAME_NONE) == 0)
                return NoError;

        if ((ims->cmd_path) && strcmp(ims->cmd_path, NAME_BUILTIN) == 0) {
                ims->flags |=
                    (F_BUILTIN | F_NO_SERVER | F_NO_REMOTE | F_NO_OPTION);
...
int read_localeconf(list, locale_name) ImsList *list;
char *locale_name;
{
        char path[MAXPATHLEN];
        char *lp, *valp, *pp;
        ...
        if (!locale_name || !*locale_name) {
                locale_name =
                    uenv->real_locale ? uenv->real_locale : uenv->locale;
        }
        ...
        if (num_ent) {
                ...
                memcpy((void *)list->elist, (void *)etmp,
                       num_ent * sizeof(ImsEnt *));
                ...
                if (def_name) {
                        ...
                        for (i = 0; i < list->num_ent; i++)
                                if (strcmp(def_name, list->elist[i]->name) ==
                                    0) {
                                        list->default_idx = i;
                                        break;
                                }
...
int read_user_selection(fselp, locale_name) FileSel **fselp;
char *locale_name;
{
        char path[MAXPATHLEN];
        int ret;
...
int read_selection_file(fsel, fp) FileSel *fsel;
FILE *fp;
{
        char *lp, *valp, *vp, *p;
        ...
        while ((line_num = read_tag_line(fp, &lp, &valp)) > 0) {
                ...
                if (strncmp(lp + 1, STR_SELECTMODE, 3) == 0) {
                        ...
                        if (p = strchr(lp + 1,
                                       STR_PREFIX_CHAR)) { /* indiv. opt */
                                if (nopts >= MAXIMSENT) {
                                        DPR(("\t[line=%d] too many options - "
                                             "'%s' ignored\n",
                                             line_num, lp));
                                        continue;
                                }
                                if (!*(++p)) {
                                        DPR(("\t[line=%d] no ims name - '%s' "
                                             "ignored\n",
                                             line_num, lp));
                                        continue;
                                }
...
int save_user_selection(sel, locale_name) UserSelection *sel;
char *locale_name;
{
        char path[MAXPATHLEN];
        int dpy_specific;
...
static void put_selection_entry(fp, ent_type, is_valid, val, val2) FILE *fp;
int ent_type;
bool is_valid;
char *val, *val2;
{
        ...
        if (ent_type == _IMSOPT2)
                ...
                fprintf(fp, "%c%s%c\t%s\n", STR_PREFIX_CHAR, name, TAG_END_CHAR,
                        val);
}

static void put_ims_conf(fp, sel) FILE *fp;
UserSelection *sel;
{
        char *valp, val[20];
	...
	opt_valid = !sel->fsel->name || strcmp(sel->name, sel->fsel->name) == 0;
        ...
        if (sel->fsel && sel->fsel->opts) {
                ...
                for (op = sel->fsel->opts; *op; op++)
                        put_selection_entry(fp, _IMSOPT2, True, (*op)->opt_str,
                                            (*op)->ims_name);
        }
}

static void put_select_mode(fp, select_mode) FILE *fp;
int select_mode;
{
        char val[20];
...
static void put_selection_header(fp) FILE *fp;
{
...
static void put_selection_sep(fp) FILE *fp;
{ fprintf(fp, "%s\n", COMMENT_CHARS); }
...
int set_select_mode(cur_mode, new_mode) int cur_mode, new_mode;
{
        char path[MAXPATHLEN];
        int dpy_specific;
        FILE *fp;
...
static int user_selection_fname(buf, buf_len, dpy_specific) char *buf;
int buf_len, dpy_specific;
{
        ...
        if (Opt.UserPath) {
                ...
                len = expand_string(dpy_specific ? "%S/%d/" : "%S/", buf,
                                    buf_len, 0);
                ...
                strcpy(buf + len,
                       real_done ? uenv->real_locale : uenv->CDE_locale);
                buf[buf_len - 1] = 0;
        }
...
int parse_protolist(valp) char *valp;
{
        int proto_bits = 0;

        if (strstr(valp, "XIM"))
                proto_bits |= ProtoBit(Proto_XIM);
        if (strstr(valp, "Ximp"))
                proto_bits |= ProtoBit(Proto_Ximp);
        if (strstr(valp, "Xsi"))
                proto_bits |= ProtoBit(Proto_Xsi);
#ifdef old_hpux
        if (strstr(valp, "Xhp"))
                proto_bits |= ProtoBit(Proto_Xhp);
#endif /* old_hpux */
        if (strstr(valp, "None"))
                proto_bits |= ProtoBit(Proto_None);
        return proto_bits;
}

int default_protocol(conf) ImsConf *conf;
{
        if (conf->protocols & ProtoBit(Proto_XIM))
                return Proto_XIM;
        else if (conf->protocols & ProtoBit(Proto_Ximp))
                return Proto_Ximp;
        else if (conf->protocols & ProtoBit(Proto_Xsi))
                return Proto_Xsi;
#ifdef old_hpux
        else if (conf->protocols & ProtoBit(Proto_Xhp))
                return Proto_Xhp;
#endif /* old_hpux */
        else
                return Proto_None;
