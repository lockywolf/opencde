...
static void action_cb(DtActionInvocationID id, XtPointer cd, DtActionArg *args,
                      int argcnt, int status);
...
static void DbReloadProc(cd) XtPointer cd;
{
...
static bool init_action_env() {
        ...
        if (actionEnabled != True) {
                ...
                if (DtAppInitialize(appC, Dpy, TopW, Wargv[0], DTIMS_CLASS) ==
                    False) {
                        /* DtInitialize() has already logged an appropriate
                         * error msg */
                        return False;
                }
...
static void action_cb(id, cd, args, argcnt, status) DtActionInvocationID id;
XtPointer cd; /* unused */
DtActionArg *args;
int argcnt;
int status;
{
        ...
        if (actionId == 0) { /* action done */
                send_dtims_msg(WIN_ST_ACTION_DONE,
                               actionDone ? NoError : ErrRemoteAction);
        }
...
int invoke_action(action, host) char *action;
char *host;
{
        if (init_action_env() != True)
                return ErrInternal;
        ...
        actionId = DtActionInvoke(TopW, action, NULL, 0, NULL, host, NULL,
                                  False, action_cb, NULL);
...
void send_dtims_msg(msg, errcode) int msg, errcode;
{
        XEvent ev;
...
void dtims_msg_proc(w, cd, event, continue_dispatch) Widget w;
XtPointer cd;
XEvent *event;
Boolean *continue_dispatch; /* never changed */
{
        ...
        if (ev->message_type != winEnv.atom_status && ev->format != 32) {
                DPR(("\tinvalid type([%d]%s) or format(%d) -- ignored\n",
                     ev->message_type, XGetAtomName(Dpy, ev->message_type),
                     ev->format));
                return;
        }
        msg = (int)ev->data.l[0];
