...
#ifndef lint
static char *RCS_ID =
    "@(#)dtimsstart $Revision: /main/10 $ $Date: 1996/10/28 12:41:03 $";
#endif
...
void Exit(err_code) int err_code;
{
        int exit_code = IsNoError(err_code) ? 0 : 1;

        DPR(("Exit(%s[%d]): exit(%d)\n", error_name(err_code), err_code,
             exit_code));
...
int NotifyErrCode(err_code) int err_code;
{
        ...
        *bp = 0;
        ...
        if (OpMode == MODE_REMCONF) {
                ...
                fflush(stdout);
                fflush(stderr);
                (void)fwrite((void *)buf, (size_t)(bp - buf), (size_t)1,
                             stdout);
                fflush(stdout);
        }
...
static int IsNoError(error) ximsError error;
{
        switch (error) {
        case NoError:
        case ErrIsNone:
...
static int EnvNeeded(error) ximsError error;
{
        switch (error) {
        case NoError:
        case ErrIsNone:
...
static int ErrMsgDisabled(error) ximsError error;
{
        switch (error) {
        case NoError:
        case ErrIsNone:
...
int ErrFilePathNeeded(error) ximsError error;
{
        switch (error) {
        case ErrFileOpen:
        case ErrFileCreate:
...
void ximsMain() {
        ...
        case State_Select_Canceled:
                ximsStart();

        case State_Start_Done:
                ximsWait();
        ...
        case State_Finish_Done:
                break;
        case State_Finish_Err:
                break;

        case State_Mode_Done:
        case State_Mode_Canceled:
                break;

        default:
                OpErrCode = ErrInvState;
                break;
        }
...
void ximsFinish() {
        ...
        if (oldOpState != State_Finish_Defered) {
                ...
                if (WaitingDialogReply) {
                        DPR2((
                            "ximsFinish(): OpState => State_Finish_Defered\n"));
                        ;
        ...
        if (OpMode == MODE_START && (OpFlag & FLAG_ENV)) {
                if (userSel.name && EnvNeeded(OpErrCode)) {
                        ...
                        DPR(("ximsFinish(): Not put env (pri->status=%s[%d])\n",
                             error_name(userSel.renv->status),
                             userSel.renv->status));
                }
        }
...
static int ximsShowImsList() {
        ...
        if (ret != NoError) {
                ...
                for (i = 0; i < list->num_ent; i++) {
                        /* If invoked from dtstyle with -listname put the "#" if
                        ...
                        if (Verbose > 1) {
                                ...
                                case NoError:
                                        ...
                                        break;
                                case ErrNoExecutable:
                                        msg = "no executable file";
                                        break;
                                case ErrNoImsConf:
                                        msg = "no configuration file";
                                        break;
                                case ErrMissEntry:
                                        msg = "invalid configuration file";
                                        break;
                                }
                                len = strlen(list->elist[i]->name) +
                                      (err ? 2 : 0);
                                for (; len < 16; len++)
                                        putchar(' ');
                                putchar(' ');
...
static int prepare_remote(win_st) int win_st;
{
        int ret;
        char **av = NULL;
        int ac = 0;
...
static int ximsSetMode() {
        ...
        case SEL_MODE_NONE:
        case SEL_MODE_QUERY:
                show_select_mode(cur_mode);
                break;
        case SEL_MODE_WIN:
                start_mode_window(cur_mode);
                /* never returned */
                break;
        ...
        case SEL_MODE_NOAUTO:
                ret = set_select_mode(cur_mode, Opt.SelectMode);
                break;
        }
...
static void show_select_mode(mode) int mode;
{
        char *valp;
        ...
        case SEL_MODE_AUTO:
                valp =
                    (catgets(catd, NL_SETN, 22, "resume_current_input_method"));
                break;
#ifdef SelectMode_ONCE
        case SEL_MODE_ONCE:
...
static int set_locale_env(locale) char *locale;
{
...
static void init_command(progname) char *progname;
{
        /* globals */
...
#ifdef unused
        if (strstr(ProgramName, "mode"))
                ProgramType = MODE_MODE;
        else if (strstr(ProgramName, "list"))
                ProgramType = MODE_LIST;
        else if (strstr(ProgramName, "current"))
                ProgramType = MODE_CURRENT;
        else if (strstr(ProgramName, "conf"))
                ProgramType = MODE_REMCONF;
        else
#endif
...
static bool optname_match(name, str, minlen) char *name, *str;
int minlen;
{
        int nlen, slen;
        ...
        nlen = strlen(name);
        slen = strlen(str);
        if (slen >= nlen || slen < minlen)
                return False;
        if (strncmp(name, str, slen) == 0)
                return True;

        return False;
}

static int parse_options(argc, argv) int argc;
char **argv;
{
        char *opt;
        int i, n;
...
#define CHK_FLAG(f) (OpFlag & (f))
	...
	case MODE_STYLE:
        ...
        if (CHK_FLAG(USE_WINDOW_MASK)) {
                ...
                Wargv =
                    ALLOC(wac + 1 + 4, char *); /* spare for '-xnlLanguage' */
...
static void usage(force) int force;
{
        char *fmt = "\t%-20s%s\n";
        ...
        if (OpMode != MODE_MODE) {
                fprintf(LogFp, fmt, "-env",
                        (catgets(catd, NL_SETN, 2,
                                 "print modified environment variables")));
                fprintf(LogFp, fmt, "-shell <name>",
                        (catgets(catd, NL_SETN, 3, "override $SHELL")));
                fprintf(
                    LogFp, fmt, "-ims <name>",
                    (catgets(catd, NL_SETN, 4, "specifies input method name")));
                fprintf(
                    LogFp, fmt, "-imsopt <string>",
                    (catgets(
                        catd, NL_SETN, 5,
                        "specifies command options for input method server")));
                fprintf(LogFp, fmt, "-host <name>",
                        (catgets(catd, NL_SETN, 6,
                                 "specifies host where input method server "
                                 "should be run")));
                fprintf(LogFp, fmt, "-mode",
                        (catgets(catd, NL_SETN, 7,
                                 "change input method selection mode")));
                fprintf(LogFp, fmt, "-list",
                        (catgets(catd, NL_SETN, 8,
                                 "print registered input method")));
                fprintf(LogFp, fmt, "-help",
                        (catgets(catd, NL_SETN, 9, "show this message")));
        } else {
                fprintf(LogFp, fmt, "-auto",
                        (catgets(catd, NL_SETN, 23,
                                 "force 'resume_current_input_method' mode")));
                fprintf(
                    LogFp, fmt, "-noauto",
                    (catgets(catd, NL_SETN, 24, "force 'ask_at_login' mode")));
        }
...
char *xims_errmsg(err_num, arg1, arg2, arg3) int err_num;
void *arg1, *arg2, *arg3;
{
        char *fmt = NULL, *bp;
        int len;
        ...
        case ErrFileCreate: /* arg1: file name */
                fmt =
                    (catgets(catd, NL_SETN, 37, "cannot create file\n  [%s]"));
                setErrArg1(errFilePath);
                break;
        case ErrDirCreate: /* arg1: dir name */
                fmt = (catgets(catd, NL_SETN, 38,
        ...
        case ErrMissEntry: /* arg1: entry name, arg2: file name */
                fmt = (catgets(
                    catd, NL_SETN, 39,
                    "missing '%s' entry in configuration file\n  [%s]"));
                setErrArg2(errFilePath);
                break;

        case ErrAnotherProg:
                fmt = (catgets(catd, NL_SETN, 40,
                               "another '%s' is already running"));
                setErrArg1(ProgramName);
                break;
        case ErrNoSelectionFile: /* arg1: locale name */
                fmt =
                    (catgets(catd, NL_SETN, 41, "no selection file for '%s'"));
                setErrArg1(userEnv.locale);
                break;
        case ErrSaveSelection: /* arg1: file name */
                fmt = (catgets(catd, NL_SETN, 42,
                               "cannot create selection file\n  [%s]"));
                setErrArg1(errFilePath);
                break;
        ...
        case ErrNoLocaleConf: /* arg1: file name */
                fmt = (catgets(catd, NL_SETN, 44,
                               "no locale configuration file for '%s'"));
                setErrArg1(userEnv.locale);
                break;
        case ErrNoImsEntry: /* arg1: locale name */
                fmt =
                    (catgets(catd, NL_SETN, 45, "no ims configured for '%s'"));
                setErrArg1(userEnv.locale);
                break;
        case ErrNoImsConf: /* arg1: ims name */
                fmt = (catgets(catd, NL_SETN, 46,
                               "no ims configuration file for '%s'"));
        ...
        case ErrNoExecutable: /* arg1: ims name, arg2: file name */
                fmt = (catgets(catd, NL_SETN, 48,
                               "no executable file for '%s'\n  [%s]"));
                setErrArg1(userSel.name);
        ...
        case ErrImsRunning: /* arg1: ims name */
                fmt =
                    (catgets(catd, NL_SETN, 49, "ims '%s' is already running"));
                setErrArg1(userSel.name);
                break;
        ...
        case ErrImsAborted: /* arg1: ims name, arg2: log file */
                fmt = (catgets(catd, NL_SETN, 51,
                               "ims '%s' aborted.  See log file.\n  [%s]"));
                setErrArg1(userSel.name);
        ...
        case ErrImsTimeout: /* arg1: file name */
                fmt = (catgets(catd, NL_SETN, 52,
                               "ims '%s' is not available yet"));
                setErrArg1(userSel.name);
                break;
        ...
        case ErrRemoteData: /* arg1: host name */
                fmt = (catgets(catd, NL_SETN, 55,
                               "remote execution failed on '%s'"));
                setErrArg1(userSel.hostname);
                break;
        case ErrNoImsstart: /* arg1: host name */
                fmt =
                    (catgets(catd, NL_SETN, 56,
                             "remote functionality is not available on '%s'"));
                setErrArg1(userSel.hostname);
                break;
        ...
        case ErrRemoteMissIms: /* arg1: ims name, arg2: host name */
                fmt = (catgets(catd, NL_SETN, 58,
                               "ims '%1$s' not registered on '%2$s'"));
                setErrArg1(userSel.name);
