...
void ximsSelect() {
        ...
        if (Opt.SelectMode == SEL_MODE_GIVEN) {
                ...
                if ((OpFlag & FLAG_NOSAVE) ||
                    Opt.SelectMode == SEL_MODE_NOAUTO) {
                        ...
                        DPR(("ximsSelect(): def_selmode==AUTO in "
                             "locale_conf\n"));
                        use_win = False;
                }
        }
        ...
        if (sel->iconic == -1) {
                sel->iconic = fsel ? fsel->iconic : 0;
                   ...
                   fsel->iconic) { /* this should never happen ! */
...
static int fill_ims_ent(list, top, last) ImsList *list;
int top, last;
{
        int ret;
        int i;
...
static int setup_local_selection(sel, list, conf_all) UserSelection *sel;
ImsList *list;
int conf_all;
{
        int ret;
...
static int setup_remote_selection(sel) UserSelection *sel;
{
        int ret;
        int idx;
        ImsList *new_list;
...
static int setup_user_selection(sel, list, idx) UserSelection *sel;
ImsList *list;
int idx;
{
        if (sel->name) {
...
int update_user_selection(sel, list, idx, host, host_type) UserSelection *sel;
ImsList *list;
int idx;
char *host;
int host_type;
...
int get_ims_idx(list, name) ImsList *list;
char *name;
{
        int idx;
...
int set_ims_status(ent) ImsEnt *ent;
{
        int ret = NoError;
        ImsConf *ims = ent->ims;
...
int get_ims_list(listp, locale, fill_ent) ImsList **listp;
char *locale;
int fill_ent;
{
        int ret = NoError;
