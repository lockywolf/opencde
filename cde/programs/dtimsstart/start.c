...
void ximsWait() {
        ...
        if (im_mod_available(sel->renv) != 1) {
                ...
                while (is_waiting()) {
                        ...
                        if (im_mod_available(sel->renv) != 0 ||
                            lapse >= Opt.Timeout) {
                                DPR((
                                    "ximsWait(tmout=%d): wait done (%d sec.)\n",
                                    Opt.Timeout, lapse));
                                break;
                        }
                }
...
void set_sig_chld(enable) int enable;
{
        DPR(("set_sig_chld(%s)\n", enable ? "Enabled" : "Disabled"));
        signal(SIGCHLD, enable ? on_sig_chld : SIG_IGN);
}

int im_mod_available(renv) RunEnv *renv;
{
        Window owner;
        ...
        if (owner != None) {
...
#ifdef ReconfirmProperty
                {
                        ...
                        DPR(("im_mod_available(): [RECONFIRM] sleep(%d)\n",
                             wait_period));
                        sleep(wait_period);
                        if (property_owner(&renv->prop_atom, renv->atom_name) ==
                            None) {
...
int mk_ims_option(ptr, sel) char *ptr;
UserSelection *sel;
{
        char *bp = ptr;
                        ...
                        for (op = fsel->opts; *op; op++) {
                                if (strcmp((*op)->ims_name, sel->name) == 0 &&
                                    check_ims_opt((*op)->opt_str)) {
                                        ...
                                        bp += expand_string((*op)->opt_str, bp,
                                                            BUFSIZ, ims);
                                        *bp++ = ' ';
                                }
                        }
...
static int check_ims_opt(ptr) char *ptr;
{
...
int load_resources() {
        ...
        if (!empty) { /* load if RESOURCE_MANAGER is empty */
                DPR2(("load_resources: RESOURCE_MANGER is not empty -- not "
                      "loaded\n"));
                return False;
        }
...
static char *find_session_resfile(res_type) int res_type;
{
        ...
        if (res_type == RES_TYPE_DT) { /* DT */
                ...
                res = vue->resPath;
                if (vue->uselite) {
                        expand_string(vue->uselite, path, MAXPATHLEN,
                                      (ImsConf *)0);
...
static int check_selection(sel) UserSelection *sel;
{
        int ret = NoError;
...
static int build_run_env(sel) UserSelection *sel;
{
        char *p;
        int proto;
        int len;
        ...
        *bp = 0;
        ...
        len = strlen(envbuf) + strlen(ims->cmd_path) + strlen(cmd_param) +
              strlen(optbuf) + strlen(log_path) + 40;

        /* for local execution */
        renv->cmdbuf = ALLOC(len, char);
        sprintf(renv->cmdbuf,
                " %s; export XMODIFIERS; exec  %s %s %s >> %s 2>&1 ", envbuf,
                ims->cmd_path, cmd_param, optbuf, log_path);

        /* timeout & interval */
        if (Opt.Timeout > 0)
                tmout = Opt.Timeout;
        else if (ims->timeout > 0)
                tmout = ims->timeout;
        else
                tmout = DEFAULT_TIMEOUT;
        if (Opt.Interval > 0)
                intv = Opt.Interval;
        else if (ims->interval > 0)
                intv = ims->interval;
        else
...
static int run_ims(sel) UserSelection *sel;
{
        int ret = NoError;
...
static int invoke_ims(sel) UserSelection *sel;
{
        ...
        if (pid == (pid_t)-1) {
                put_xims_log("fork failed [%s]",
                             (errno <= sys_nerr) ? sys_errlist[errno] : NULL, 0,
                             0);
#ifdef DEBUG
                perror("fork");
#endif
...
static void on_sig_chld(sig) int sig;
{
        int wait_status = 0;
        pid_t pid;
        int cause;
        ...
        if (renv->pid == pid) {
                if (renv->status == ErrImsWaiting ||
                    renv->status == ErrImsConnecting) {
                        ...
                        DPR((
                            "\tsig_chld: renv->state=%s is not ErrImsWaiting\n",
...
static bool is_ims_running(renv, ims) RunEnv *renv;
ImsConf *ims;
{
        char *prop_str;
        Window owner;
...
static int settle_ims(sel) UserSelection *sel;
{
...
static Window property_owner(prop_atom, prop_str) Atom *prop_atom;
char *prop_str;
{
...
static void catch_alarm(sig) int sig;
{
...
static int try_connection(sel) UserSelection *sel;
{
        RunEnv *renv = sel->renv;
        ImsConf *ims = sel->ent->ims;
        ...
        if (setjmp(jmp_env) == 0) {
                ...
                for (retry_cnt = 0; !ic_ok && retry_cnt <= MAX_RETRY;
                     retry_cnt++) {
                        if (retry_cnt)
                                sleep(retry_cnt * retry_cnt);

#ifdef DEBUG
                        last_time = time((time_t)0);
#endif
                        xim = XOpenIM(Dpy, (XrmDatabase)0, ims->servername,
                                      ims->classname);
                        if (xim) {
                                DPR(("try_connection(%d): XOpenIM() OK [%d "
                                     "sec.]",
...
static int create_dummy_ic(xim) XIM xim;
{
        int scr;
        XFontSet fset;
        XIMStyles *im_styles;
        ...
        dmy_win = XCreateSimpleWindow(Dpy, RootWindow(Dpy, scr), 0, 0, 1, 1, 0,
                                      bg, fg);
        ...
        if (XGetIMValues(xim, XNQueryInputStyle, &im_styles, NULL)) {
                DPR(("create_dummy_ic(): XGetIMValues(XNQueryInutStyle) "
                     "failed.\n"));
                goto _err;
        }
        ...
        if ((int)im_styles->count_styles > 0) {
                ...
                for (i = 0, alt = -1; i < (int)im_styles->count_styles; i++)
                        if (im_styles->supported_styles[i] &
                            XIMPreeditNothing) {
                                if (im_styles->supported_styles[i] &
                                    XIMStatusNothing) {
                                        style = im_styles->supported_styles[i];
                                        break;
                                } else if (im_styles->supported_styles[i] &
                                           XIMStatusArea) {
                                        alt = i;
                                }
                        }
                if (!style && alt >= 0)
                        style = im_styles->supported_styles[alt];
                XFree(im_styles);
        }
        if (!style) {
                DPR(("create_dummy_ic(): 'PreeditNothing' styles not "
                     "supported.\n"));
                goto _err;
        ...
        if (style & XIMStatusArea) {
                ...
                status_attr = XVaCreateNestedList(
                    NULL, XNArea, &area, XNForeground, fg, XNBackground, bg,
                    XNFontSet, fset, NULL);
        }

        ic = XCreateIC(xim, XNInputStyle, style, XNClientWindow, dmy_win,
                       XNStatusAttributes, status_attr, XNFocusWindow, dmy_win,
                       NULL);
