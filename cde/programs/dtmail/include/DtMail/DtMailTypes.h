 ...
 * specific implementations. These should begin after the DTMC_RESERVED
 * constant.
 */
typedef enum {
        DTMC_NOOP = 0,
...
typedef DtMailBoolean (*DtMailCallback)(DtMailCallbackOp op, const char *path,
...
typedef void (*DtMailStatusCallback)(DtMailOperationId, DtMailEnv &error,
                                     void *client_data);
...
typedef enum { DTM_TARGET_MAILBOX, DTM_TARGET_TRANSPORT } DtMailCallbackTarget;
...
typedef enum { DtMailLockRead = 1, DtMailLockWrite = 2 } DtMailLock;
