...
class SendMsgDialog : public MenuWindow, public AbstractEditorParent {
      ...
      private:
        ...
        Boolean _first_time;
        Boolean
            _already_sending; // Flag to make sure we don't send more than once.
        ...
        // As user adds/deletes attachment, this data structure is update
        // immediately. This data structure is also updated at periodic
        // checkpoints to store the latest Compose window data. Before sending,
        // this is also updated.
        DtMail::Message *_msgHandle;

        // Pointer to the message that is being included as an attachment.
        // When updateMsgHndAtt() is called, data in this handle is copied to
        // _msgHandle.
        DtMail::Message *_inclMsgHandle;

        // Pointer to the last Attachment BodyPart so that newBodyPart(error,
        // _lastAttBP) returns a message body part after the last body part.
        // This is so that attachments will be ordered properly.
        ...
        // indicates that the first body part of _msgHandle has already been
        // used (probably for text).  Otherwise, the first body part can be used
        // for attachment.
        ...
        static void aliasMenuButtonHandler(Widget, XtPointer client_data,
                                           XEvent *event, Boolean *);
        void createAliasList(DtVirtArray<PropStringPair *> *aliases);
        void destroyAliasList(DtVirtArray<PropStringPair *> *aliases);
        Widget createAliasPopupMenu(Widget parent, MenuBar **menubar,
                                    CmdList **cmdlist,
                                    DtVirtArray<PropStringPair *> *aliases);
        void destroyAliasPopupMenu(Widget parent, MenuBar *menubar,
                                   CmdList *cmdlist, Widget menu);
      ...
      public:
        ...
        void activate_edit_paste_indented() {
                _edit_paste_special[0]->activate();
        }
        void activate_edit_paste_bracketed() {
                _edit_paste_special[1]->activate();
        }
