...
void Do_FileIO(Widget wid, XtPointer client_unused,
        ...
        /* get file name */
        if (SaveMeNot) {
                tmpSave = (char *)_XmStringUngenerate(callback_data->value,
                                                      NULL, XmMULTIBYTE_TEXT,
                                                      XmMULTIBYTE_TEXT);
        }
        ...
        if (fileIOMode == FILE_READ) {
                if (!Read_File(tmpSave)) {
                        ...
                        Dirty = False;
                        SavedOnce = True; /* Implicitly saved, since we loaded
                                             the file */
                        /*
                           Turn off the HotSpot thing in the File menu.
                        */
                        if (X_Hot == -1)
                                XtSetSensitive((Widget)editMenu_deleteHS_pb,
                                               False);
...
Boolean Read_File(char *fnameIn) {
        ...
        if (first == FORMAT_XPM) {
                status =
                    _DtXpmReadFileToPixmap(dpy, tablet_win, fname, &pix_ret,
                                           &shape_ret, &xpm_ReadAttribs);
                ...
                if (debug) {
                        ...
                        debug_status = _DtXpmReadFileToImage(
                            dpy, fname, &debug_image, &debug_shape,
                            &xpm_ReadAttribs);
                }
#endif
...
#ifdef DEBUG
                                if (debug)
                                        stat_out("Leaving Read_File - XPM/XBM "
                                                 "read attempt failed.\n");
#endif
                                return (False);
                        } else
                ...
                status = XReadBitmapFile(dpy, tablet_win, fname, &width_ret,
                                         &height_ret, &pix_ret, &x_hot, &y_hot);
                if (status != BitmapSuccess) {
                        status = _DtXpmReadFileToPixmap(dpy, tablet_win, fname,
                                                        &pix_ret, &shape_ret,
                                                        &xpm_ReadAttribs);
                        if (status != XpmSuccess) {
#ifdef DEBUG
                                if (debug)
                                        stat_out("Leaving Read_File - XBM/XPM "
                                                 "read attempt failed.\n");
#endif
                                return (False);
                        } else
        ...
        if (successFormat == FORMAT_XBM) {
                ...
                status = XReadBitmapFile(dpy, tablet_win, dummy,
                                         &mask_width_ret, &mask_height_ret,
                                         &mask_ret, &mask_x_hot, &mask_y_hot);
                if (status == BitmapSuccess) {
                        if ((width_ret != mask_width_ret) ||
                            (height_ret != mask_height_ret)) {
...
Boolean Write_File(char *fnameIn) {
        ...
        if (fileFormat == FORMAT_XPM) {
                ...
                /*******
                    status = _DtXpmWriteFileFromPixmap(dpy, fname, color_icon,
                NULL, NULL);
                ********/
                if (status != XpmSuccess) {
#ifdef DEBUG
                        if (debug) {
                                stat_out(
                                    "Leaving Write_File - XPM write failed. ");
                                switch (status) {
                                case XpmOpenFailed:
                                        stat_out("(XpmOpenFailed)\n");
                                        break;
                                case XpmNoMemory:
                                        stat_out("(XpmNoMemory)\n");
                                        break;
                                default:
                                        stat_out("(UNKNOWN cause)\n");
                                        break;
                                }
                        }
#endif
                 ...
                 * ***/
                mask_needed = False;
                scratch_shape = XGetImage(dpy, mono_icon, 0, 0, icon_width,
                                          icon_height, AllPlanes, format);
                scratch_mask = XGetImage(dpy, mono_icon, 0, 0, icon_width,
                                         icon_height, AllPlanes, format);
                ...
                if (strncmp(vend, "Sun", 3) == 0) {
                        ...
                        for (i = 0; i < icon_width; i++)
                                for (j = 0; j < icon_height; j++)
                                        if (XGetPixel(scratch_shape, i, j) ==
                                            Transparent) {
                                                XPutPixel(scratch_shape, i, j,
                                                          white_pixel);
                                                mask_needed = True;
                                        }
                        for (i = 0; i < icon_width; i++)
                                for (j = 0; j < icon_height; j++)
                                        if (XGetPixel(scratch_mask, i, j) !=
                                            Transparent)
                                                XPutPixel(scratch_mask, i, j,
                                                          black_pixel);
                                        else
                                                XPutPixel(scratch_mask, i, j,
                                                          white_pixel);
                ...
                status = XWriteBitmapFile(dpy, fname, scratch_pix, icon_width,
                ...
                if (mask_needed) {
                        XPutImage(dpy, scratch_pix, Mono_gc, scratch_mask, 0, 0,
                                  0, 0, icon_width, icon_height);
                        status = XWriteBitmapFile(dpy, dummy, scratch_pix,
                                                  icon_width, icon_height,
                                                  X_Hot, Y_Hot);
                        if (status != BitmapSuccess) {
                                XDestroyImage(scratch_shape);
...
#endif
        if ((fileFormat != FORMAT_XPM) && (!mask_needed) &&
            (stat(tmpSave, &statbuf) == 0))
                unlink(dummy);
#ifdef __TOOLTALK
        if (tt_tmpfile_fd != -1)
                send_tt_saved();
...
void Display_XPMFile(int width, int height) {
        ...
        scratch_img =
            XGetImage(dpy, pix_ret, 0, 0, width, height, AllPlanes, format);
        mono_img =
            XGetImage(dpy, pix_ret, 0, 0, width, height, AllPlanes, format);
        mask_img = NULL;
        if (shape_ret)
        ...
        XPutImage(dpy, color_icon, Color_gc, scratch_img, 0, 0, 0, 0, width,
                  height);
        XPutImage(dpy, mono_icon, Mono_gc, mono_img, 0, 0, 0, 0, width, height);

        /*** this following stuff all gets done regardless ***/

        XCopyArea(dpy, color_icon, XtWindow(iconImage), Color_gc, 0, 0, width,
                  height, 0, 0);
        XCopyArea(dpy, mono_icon, XtWindow(monoImage), Mono_gc, 0, 0, width,
                  height, 0, 0);
        icon_width = width;
        icon_height = height;
        fileFormat = FORMAT_XPM;
        XmToggleButtonGadgetSetState(formatMenu_xpm_tb, True, True);
        /* This line is not realy needed since an Exposed event will be
         * generated */
...
void Display_XBMFile(int width, int height) {
        ...
        XCopyPlane(dpy, pix_ret, scratch_pix, scratch_gc, 0, 0, width, height,
                   0, 0, 1);
        if (mask_ret) {
                ...
                for (i = 0; i < width; i++)
                        for (j = 0; j < height; j++)
                                if (!XGetPixel(test_img, i, j))
                                        XPutPixel(scratch_img, i, j,
                                                  Transparent);
                XPutImage(dpy, scratch_pix, scratch_gc, scratch_img, 0, 0, 0, 0,
                          width, height);
        } /* if(mask_ret) */
        XCopyArea(dpy, scratch_pix, color_icon, Color_gc, 0, 0, width, height,
                  0, 0);
        XCopyArea(dpy, scratch_pix, mono_icon, Mono_gc, 0, 0, width, height, 0,
                  0);
        XCopyArea(dpy, color_icon, XtWindow(iconImage), Color_gc, 0, 0, width,
                  height, 0, 0);
        XCopyArea(dpy, mono_icon, XtWindow(monoImage), Mono_gc, 0, 0, width,
...
void SetFileIODialogInfo(void) {
        ...
        if (DialogFlag == OPEN) /* Dialog is for File-Open menu item */
        {
                ...
                n = 0;
                XtSetArg(args[n], XmNdialogTitle, openTitle);
                n++;
                ...
                n = 0;
                XtSetArg(args[n], XmNselectionLabelString, openLabel);
                n++;
                OpenOKLabel = GETXMSTR(2, 22, "Open");
                XtSetArg(args[n], XmNokLabelString, OpenOKLabel);
                n++;
                XtSetValues(fileIODialog, args, n);
        } else /* Dialog is for File-Save or File-Save_As menu item */
        {
                ...
                n = 0;
                XtSetArg(args[n], XmNdialogTitle, saveTitle);
                n++;
                XtSetValues(fileIODialog, args, n);
                ...
                n = 0;
                XtSetArg(args[n], XmNselectionLabelString, saveLabel);
                n++;
                SaveOKLabel = GETXMSTR(2, 24, "Save");
                XtSetArg(args[n], XmNokLabelString, SaveOKLabel);
                n++;
                XtSetValues(fileIODialog, args, n);
        }
        /*                                                  */
        ...
        if (!textWidget)
                textWidget =
                    XmFileSelectionBoxGetChild(fileIODialog, XmDIALOG_TEXT);
        ...
        if (tst == 0) {     /* untitled */
                ...
                n = 0;
                XtSetArg(args[n], XmNdirectory, tmpXmStr);
                n++;
                XtSetValues(fileIODialog, args, n);
        ...
        XmTextFieldSetSelection(textWidget, startSelect, endSelect,
                                CurrentTime);
 ...
 *                                                                         *
 ***************************************************************************/
int Dump_AttribStruct(XpmAttributes *xpma) {
        int i, j;
        ...
        if (xpma->numsymbols) {
                for (i = 0; i < xpma->numsymbols; i++)
                        stat_out("    %18s | %18s | %d\n",
                                 ...
                                 xpma->colorsymbols[i].value,
                                 xpma->colorsymbols[i].pixel);
        }
