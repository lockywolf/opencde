...
static void cancelCB_fileIODialog(Widget w, XtPointer clientData,
...
Widget create_fileIODialog(void) {
        ...
        fileIODialog_shell = XtVaCreatePopupShell(
            "fileIODialog_shell", xmDialogShellWidgetClass, mainWindow,
            XmNtitle, GETSTR(2, 2, "Icon Editor - File I/O"), NULL);
        ...
        fileIODialog = XtVaCreateWidget(
            "fileIODialog", xmFileSelectionBoxWidgetClass, fileIODialog_shell,
            XmNdialogTitle, tmpXmStr, XmNfilterLabelString, tmpXmStr1,
            XmNdirListLabelString, tmpXmStr2, XmNfileListLabelString, tmpXmStr3,
            XmNselectionLabelString, tmpXmStr4, XmNapplyLabelString, tmpXmStr1,
            XmNcancelLabelString, tmpXmStr6, XmNhelpLabelString, tmpXmStr7,
            NULL);
        ...
        if (NULL == userIconPath) {
                ...
                if (0 == stat(buf, &sbuf) && S_IFDIR & sbuf.st_mode &&
        ...
        XtAddCallback(fileIODialog, XmNcancelCallback, cancelCB_fileIODialog,
                      NULL);
        ...
        if (!xrdb.useFileFilter) {
                ...
                wid = XmFileSelectionBoxGetChild(fileIODialog,
                ...
                if (!xrdb.useFileLists) {
                        XtUnmanageChild(XmFileSelectionBoxGetChild(
                            fileIODialog, XmDIALOG_APPLY_BUTTON));
                } else {
                        n = 0;
                        tmpXmStr = GETXMSTR(2, 18, "Update");
                        XtSetArg(args[n], XmNapplyLabelString, tmpXmStr);
                        n++;
        ...
        if (!xrdb.useFileLists) {
                ...
                XtUnmanageChild(wid);
                wid =
                    XmFileSelectionBoxGetChild(fileIODialog, XmDIALOG_DIR_LIST);
