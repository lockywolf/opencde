...
int main(int argc, char *argv[]) {
        ...
        TopLevel =
            XtAppInitialize(&AppContext, CLASS_NAME, option_list,
        ...
        */
        ...
        tmpPtr =
            GETSTR(8, 8, "Could not connect to ToolTalk:\n%s\nExiting ...");
        ...
        case TTME_EDIT:
                ...
                if (tt_message_status(msg) != TT_WRN_START_MESSAGE) {
                        ...
                        if (file) {
                                ...
                                if (tt_message_arg_bval(msg, 0, &buf, &blen) !=
                                    TT_OK) {
                                        tt_message_fail(msg);
...
void send_tt_saved() {
        ...
        if ((fileFormat != FORMAT_XPM) && (stat(dummy, &statbuf) == 0)) {
                ...
                if ((rlen = read(fdm, Mbuffer, Mlen)) != Mlen) {
                        /* didn't read whole file! */
                        printf("dtcreate: Only read %d of %d bytes of icon "
                               "file!\n",
                               rlen, Mlen);
                }
                (void)close(fdm);
