...
void Flicker_Arc(Window win, int x1, int y1, int x2, int y2) {
        ...
        if ((width > 0) && (height > 0))
                XDrawArc(dpy, win, Flicker_gc, x, y, width, height, 0,
                         360 * 64);
}
...
void Circle_Box(Window win, int x1, int y1, int x2, int y2, XRectangle *box) {
         ...
         *            calculate the area to enclose. * not TRUE.
         **
         *                                                                         *
         *X11***********************************************************************/
...
void Start_HotBox(int flag) {
        ...
        selectTimerID = XtAppAddTimeOut(AppContext, FLASH_INTERVAL,
                                        (XtTimerCallbackProc)Do_HotBox, NULL);
...
static void Do_HotBox(XtPointer *client_data, XtIntervalId *local_id) {
        ...
        if (*local_id == selectTimerID) {
                if (Selected) {
                        ...
                        XSetLineAttributes(dpy, scratch_gc, 1, LineSolid,
                                           CapButt, JoinMiter);
                        XDrawRectangle(dpy, tablet_win, scratch_gc, flash_x,
                                       flash_y, flash_width, flash_height);
                        selectTimerID = XtAppAddTimeOut(
                            AppContext, FLASH_INTERVAL,
                            (XtTimerCallbackProc)Do_HotBox, NULL);
                } else
                        Stop_HotBox();
        }
...
void Stop_HotBox(void) {
        ...
        if (GridEnabled) {
                XDrawLine(dpy, tablet_win, Grid_gc, flash_x, flash_y,
                          (flash_x + flash_width), flash_y);
                XDrawLine(dpy, tablet_win, Grid_gc, flash_x,
                          (flash_y + flash_height), (flash_x + flash_width),
                          (flash_y + flash_height));
                XDrawLine(dpy, tablet_win, Grid_gc, flash_x, flash_y, flash_x,
                          (flash_y + flash_height));
                XDrawLine(dpy, tablet_win, Grid_gc, (flash_x + flash_width),
                          flash_y, (flash_x + flash_width),
                          (flash_y + flash_height));
        } else {
                /* since Rotate left and right moves ix and iy revert to tmp_ix,
                 * tmp_iy */
                if (Rotate_Move) {
                        Rotate_Move = False;
                        ix = tmp_ix;
