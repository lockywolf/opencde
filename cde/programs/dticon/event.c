...
static void Clear_ClickCount(void) {
#ifdef DEBUG
        if (debug)
...
void ProcessTabletEvent(Widget w, XEvent *xptr, String *params,
                        Cardinal num_params) {
#ifdef DEBUG
        Arg args[10];
        ...
        case MotionNotify:
                ...
                */
                if (xGrid != Prev_x || yGrid != Prev_y) {
                        ...
                        XtVaSetValues(coordinateText, XmNlabelString, str,
                                      NULL);
                        XmStringFree(str);
                }
#ifdef DEBUG
                ...
                if (xptr->xexpose.window == tablet_win) {
                        ...
                        if (debug)
                                printf("Exposure at [%d,%d] of %dx%d\n", x, y,
                                       width, height);
#endif
                        Repaint_Tablet(xptr->xexpose.window, x, y, width,
                                       height);
                } else if (xptr->xexpose.window == XtWindow(iconImage)) {
...
#endif
                } else if (xptr->xexpose.window == XtWindow(monoImage)) {
                        XCopyArea(dpy, mono_icon, XtWindow(monoImage), Color_gc,
        ...
        case LeaveNotify:
                ...
                if (debug) {
                        ...
                        i = 0;
                        XtSetArg(args[i], XmNwidth, &db_width);
                        i++;
                        XtSetArg(args[i], XmNheight, &db_height);
                        i++;
        ...
        case ButtonPress:
                ...
                case S_SCALE_2:
                        ...
                        if (GraphicsOp == S_PASTE)
                                XDrawRectangle(
                                    dpy, tablet_win, Flicker_gc, last_tx,
                                    last_ty,
                ...
                case POINT:
                        ...
                        if (GraphicsOp == ERASER) {
                                for (i = ix - 1; i <= ix + 1; i++)
                                        for (j = iy - 1; j <= iy + 1; j++) {
                                                XDrawPoint(dpy, color_icon,
                                                           Erase_gc, i, j);
                                                XDrawPoint(dpy,
                                                           XtWindow(iconImage),
                                                           Erase_gc, i, j);
                                                XDrawPoint(dpy, mono_icon,
                                                           Erase_gc, i, j);
                                                XDrawPoint(dpy,
                                                           XtWindow(monoImage),
                                                           Erase_gc, i, j);
                                ...
                                XDrawPoint(dpy, XtWindow(iconImage), Color_gc,
                                           ix, iy);
                                XDrawPoint(dpy, mono_icon, Mono_gc, ix, iy);
                                XDrawPoint(dpy, XtWindow(monoImage), Mono_gc,
                                           ix, iy);
                ...
                case POLYGON:
                        ...
                        if (!Anchored) {
                                ...
                                Icon_Coords(last_tx, last_ty, &last_ix,
                                            &last_iy);
                        ...
                        last_ty = ty;
                        ClickCount++;
                        polyTimerID = XtAppAddTimeOut(
                            AppContext, multiClickTime,
                            (XtTimerCallbackProc)Clear_ClickCount, NULL);
                        break;
                } /* switch(GraphicsOp) */
                break;
        case ButtonRelease:
                ...
                case POINT:
                        if (Pressed) {
                                ...
                                if (GraphicsOp == ERASER) {
                                        ...
                                        XDrawPoint(dpy, color_icon, Color_gc,
                                                   ix, iy);
                                        XDrawPoint(dpy, XtWindow(iconImage),
                                                   Color_gc, ix, iy);
                                        XDrawPoint(dpy, mono_icon, Mono_gc, ix,
                                                   iy);
                ...
                case S_GRAB:
                        if (Pressed) {
                                XDrawRectangle(
                                    dpy, lwin, Flicker_gc, min(tx, last_tx),
                                    min(ty, last_ty), abs(tx - last_tx),
                                    abs(ty - last_ty));
                ...
                case CIRCLE:
                        if (Pressed) {
                                Quantize(&x, &y, True);
                                Circle_Box(tablet_win, last_tx, last_ty, tx, ty,
                                           &box);
                                tx = x;
                                ty = y;
                                Pressed = False;
        ...
        case MotionNotify:
                        ...
                        if (!FirstRigid) {
                                if (GraphicsOp == S_PASTE)
                                        XDrawRectangle(
                                            dpy, tablet_win, Flicker_gc,
                                            last_tx, last_ty,
                                            CutCopy->width * MagFactor -
                                                MagFactor,
                                            CutCopy->height * MagFactor -
                                                MagFactor);
                                else if (GraphicsOp == S_ROTATE)
                                        XDrawRectangle(
                                            dpy, tablet_win, Flicker_gc,
                                            last_tx, last_ty,
                                            Rotate->width * MagFactor -
                                                MagFactor,
                                            Rotate->height * MagFactor -
                                                MagFactor);
                                else
                                        XDrawRectangle(
                                            dpy, tablet_win, Flicker_gc,
                                            last_tx, last_ty,
                                            Scale->width * MagFactor -
                                                MagFactor,
                                            Scale->height * MagFactor -
                                                MagFactor);
                        }
                        if (GraphicsOp == S_PASTE)
                                XDrawRectangle(
                                    dpy, tablet_win, Flicker_gc, tx, ty,
                                    CutCopy->width * MagFactor - MagFactor,
                                    CutCopy->height * MagFactor - MagFactor);
                        else if (GraphicsOp == S_ROTATE)
                                XDrawRectangle(
                ...
                case POLYGON:
                        if (Anchored) {
                                Quantize(&x, &y, True);
                                if (!First)
                                        XDrawLine(dpy, tablet_win, Flicker_gc,
                                                  tx, ty, last_tx, last_ty);
                                tx = x;
                                ty = y;
                                XDrawLine(dpy, tablet_win, Flicker_gc, tx, ty,
                                          last_tx, last_ty);
                ...
                case S_GRAB:
                        if (Pressed) {
                                ...
                                sprintf(
                                    ...
                                    abs(tx - last_tx), abs(ty - last_ty));
                                str = XmStringCreateLocalized(tmpstr);
                                XtVaSetValues(coordinateText, XmNlabelString,
                                              str, NULL);
                                XmStringFree(str);
                        }
                        break;
                case CIRCLE:
                        if (Pressed) {
                                Quantize(&x, &y, True);
                                if (!First)
                                        Circle_Box(tablet_win, last_tx, last_ty,
                                                   tx, ty, &box);
                                tx = x;
                                ty = y;
                                Circle_Box(tablet_win, last_tx, last_ty, tx, ty,
...
void iRectangle(int x, int y, int width, int height, Boolean backupFlag) {
        if (backupFlag)
                Backup_Icons();
        if (FillSolids) {
                XFillRectangle(dpy, color_icon, Color_gc, x, y, width + 1,
                               height + 1);
                XFillRectangle(dpy, mono_icon, Mono_gc, x, y, width + 1,
                               height + 1);
                XFillRectangle(dpy, XtWindow(iconImage), Color_gc, x, y,
                               width + 1, height + 1);
                XFillRectangle(dpy, XtWindow(monoImage), Mono_gc, x, y,
                               width + 1, height + 1);
        } else {
                XDrawRectangle(dpy, color_icon, Color_gc, x, y, width, height);
                XDrawRectangle(dpy, mono_icon, Mono_gc, x, y, width, height);
                XDrawRectangle(dpy, XtWindow(iconImage), Color_gc, x, y, width,
                               height);
                XDrawRectangle(dpy, XtWindow(monoImage), Mono_gc, x, y, width,
                               height);
        }
...
void iArc(int x, int y, int width, int height, Boolean backupFlag) {
        if (backupFlag)
                Backup_Icons();
        if (FillSolids) {
                XFillArc(dpy, color_icon, Color_gc, x, y, width, height, 0,
                         360 * 64);
                XFillArc(dpy, mono_icon, Mono_gc, x, y, width, height, 0,
                         360 * 64);
                XFillArc(dpy, XtWindow(iconImage), Color_gc, x, y, width,
                         height, 0, 360 * 64);
                XFillArc(dpy, XtWindow(monoImage), Mono_gc, x, y, width, height,
                         0, 360 * 64);
        } else {
                XDrawArc(dpy, color_icon, Color_gc, x, y, width, height, 0,
                         360 * 64);
                XDrawArc(dpy, mono_icon, Mono_gc, x, y, width, height, 0,
                         360 * 64);
                XDrawArc(dpy, XtWindow(iconImage), Color_gc, x, y, width,
                         height, 0, 360 * 64);
                XDrawArc(dpy, XtWindow(monoImage), Mono_gc, x, y, width, height,
                         0, 360 * 64);
        }
        Transfer_Back_Image(x, y, x + width, y + height, FILL);
}
...
void iPolygon(void) {
        ...
        if (FillSolids) {
                ...
                XFillPolygon(dpy, mono_icon, Mono_gc, pointList, pointCount,
                             Complex, CoordModeOrigin);
                XFillPolygon(dpy, XtWindow(iconImage), Color_gc, pointList,
                             pointCount, Complex, CoordModeOrigin);
                XFillPolygon(dpy, XtWindow(monoImage), Mono_gc, pointList,
                             pointCount, Complex, CoordModeOrigin);
        } else {
                if (pointCount < MAX_PTS) {
                ...
                XDrawLines(dpy, color_icon, Color_gc, pointList, pointCount,
                           Complex);
                XDrawLines(dpy, mono_icon, Mono_gc, pointList, pointCount,
                           Complex);
