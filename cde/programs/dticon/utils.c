...
extern Widget color_pb1, color_pb2, color_pb3, color_pb4, color_pb5, color_pb6,
    color_pb7, color_pb8;
extern Widget grey_pb1, grey_pb2, grey_pb3, grey_pb4, grey_pb5, grey_pb6,
    grey_pb7, grey_pb8;
extern Widget fgColorToggle, bgColorToggle, tsColorToggle, bsColorToggle,
    selectColorToggle, transparentColorToggle;
extern Widget pointButton, floodButton, lineButton, polylineButton,
    rectangleButton, polygonButton, circleButton, ellipseButton, eraseButton,
    selectButton;
...
extern void *Process_DropCheckOp(Widget, XtPointer, XtPointer);
extern void *Process_DropOp(Widget, XtPointer, XtPointer);
...
void Create_Gfx_Labels(unsigned long fg, unsigned long bg) {
        ...
        rectangle_height, fg, bg, depth); polygonPix =
        XCreatePixmapFromBitmapData(dpy, root, polygon_bits, polygon_width,
        polygon_height, fg, bg, depth); circlePix =
        XCreatePixmapFromBitmapData(dpy, root, circle_bits, circle_width,
        circle_height, fg, bg, depth); ellipsePix =
        XCreatePixmapFromBitmapData(dpy, root, ellipse_bits, ellipse_width,
        ellipse_height, fg, bg, depth); eraserPix =
        XCreatePixmapFromBitmapData(dpy, root, eraser_bits, eraser_width,
        eraser_height, fg, bg, depth); selectPix =
        XCreatePixmapFromBitmapData(dpy, root, select_bits, select_width,
        select_height, fg, bg, depth);
        **********/
        ...
        status = _DtXpmCreatePixmapFromData(dpy, root, Circle, &circlePix,
                                            &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(
                    GETSTR(10, 2, "Cannot initialize button icon for circle"));
        status = _DtXpmCreatePixmapFromData(
            dpy, root, CircleSolid, &circleSolidPix, &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(GETSTR(10, 4,
                             "Cannot initialize button icon for solid circle"));
        status = _DtXpmCreatePixmapFromData(dpy, root, Connected, &polylinePix,
                                            &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(GETSTR(10, 6,
                             "Cannot initialize button icon for polyline"));
        status = _DtXpmCreatePixmapFromData(
            dpy, root, ConnectedClosed, &polygonPix, &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(
                    GETSTR(10, 8, "Cannot initialize button icon for polygon"));
        status = _DtXpmCreatePixmapFromData(dpy, root, ConnecClosedSolid,
                                            &polygonSolidPix, &mask,
                                            &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(GETSTR(
                    10, 10, "Cannot initialize button icon for solid polygon"));
        status = _DtXpmCreatePixmapFromData(dpy, root, Ellipse, &ellipsePix,
                                            &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(GETSTR(10, 12,
                             "Cannot initialize button icon for ellipse"));
        status = _DtXpmCreatePixmapFromData(
            dpy, root, EllipseSolid, &ellipseSolidPix, &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(GETSTR(
                    10, 14, "Cannot initialize button icon for solid ellipse"));
        status = _DtXpmCreatePixmapFromData(dpy, root, Eraser, &eraserPix,
                                            &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(
                    GETSTR(10, 16, "Cannot initialize button icon for eraser"));
        status = _DtXpmCreatePixmapFromData(dpy, root, Line, &linePix, &mask,
                                            &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(GETSTR(10, 18, "Cannot initialize button icon for line"));
        status = _DtXpmCreatePixmapFromData(dpy, root, PaintBucket, &floodPix,
                                            &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(
                    GETSTR(10, 20, "Cannot initialize button icon for flood"));
        status = _DtXpmCreatePixmapFromData(dpy, root, Pencil, &pointPix, &mask,
                                            &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(
                    GETSTR(10, 22, "Cannot initialize button icon for point"));
        status = _DtXpmCreatePixmapFromData(dpy, root, Rectangle, &rectPix,
                                            &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(GETSTR(10, 24,
                             "Cannot initialize button icon for rectangle"));
        status = _DtXpmCreatePixmapFromData(
            dpy, root, RectangleSolid, &rectSolidPix, &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(GETSTR(
                    10, 26,
                    "Cannot initialize button icon for solid rectangle"));
        status = _DtXpmCreatePixmapFromData(dpy, root, SelectArea, &selectPix,
                                            &mask, &xpm_ReadAttribs);
        if (status != XpmSuccess)
                Abort(
                    GETSTR(10, 28, "Cannot initialize button icon for select"));

        q_markPix = XCreatePixmapFromBitmapData(dpy, root, (char *)q_mark_bits,
                                                q_mark_width, q_mark_height, fg,
                                                bg, depth);

        i = 0;
        XtSetArg(args[i], XmNlabelPixmap, pointPix);
        i++;
        XtSetValues(pointButton, args, i);
        i = 0;
        XtSetArg(args[i], XmNlabelPixmap, linePix);
        i++;
        XtSetValues(lineButton, args, i);
        i = 0;
        XtSetArg(args[i], XmNlabelPixmap, eraserPix);
        i++;
        XtSetValues(eraseButton, args, i);
        i = 0;
        XtSetArg(args[i], XmNlabelPixmap, floodPix);
        i++;
        XtSetValues(floodButton, args, i);
        i = 0;
        XtSetArg(args[i], XmNlabelPixmap, polylinePix);
        i++;
        XtSetValues(polylineButton, args, i);
        i = 0;
        XtSetArg(args[i], XmNlabelPixmap, selectPix);
        i++;
        XtSetValues(selectButton, args, i);
        i = 0;
        XtSetArg(args[i], XmNlabelPixmap, q_markPix);
        i++;
...
void Init_Editor(Widget wid) {
        ...
        i = 0;
        XtSetArg(args[i], XmNbackground, Transparent);
        i++;
        ...
        i = 0;
        XtSetArg(args[i], XmNset, FillSolids);
        i++;
        XtSetValues(fillToggle, args, i);
        i = 0;
        XtSetArg(args[i], XmNclipWindow, &clip_wid);
        i++;
...
void GetMarginData(void) {
        ...
        i = 0;
        XtSetArg(args[i], XmNx, &lx);
        i++;
        XtSetArg(args[i], XmNy, &ly);
        i++;
...
void New_MagFactor(int new_value) {
        ...
        MagFactor = new_value;
        i = 0;
        XtSetArg(args[i], XmNwidth, (icon_width * MagFactor));
        i++;
        XtSetArg(args[i], XmNheight, (icon_height * MagFactor));
        i++;
        XtSetValues(tablet_wid, args, i);
        if (Selected)
                Set_HotBox_Coords();
...
void Repaint_Exposed_Tablet(void) {
        ...
        i = 0;
        XtSetArg(args[i], XmNwidth, &c_width);
        i++;
        XtSetArg(args[i], XmNheight, &c_height);
        i++;
        XtGetValues(clip_wid, args, i);
        i = 0;
        XtSetArg(args[i], XmNx, &t_x);
        i++;
        XtSetArg(args[i], XmNy, &t_y);
        i++;
        XtSetArg(args[i], XmNwidth, &t_width);
        i++;
        XtSetArg(args[i], XmNheight, &t_height);
        i++;
...
void Repaint_Tablet(Window win, int x, int y, int width, int height) {
        ...
        for (i = x; i < x + width + MagFactor; i += MagFactor)
                for (j = y; j < y + height + MagFactor; j += MagFactor) {
                        ...
                        if ((lx >= 0) && (lx < icon_width) && (ly >= 0) &&
                            (ly < icon_height)) {
                                XSetForeground(dpy, scratch_gc,
                                               XGetPixel(scratch_img, lx, ly));
                                XFillRectangle(dpy, tablet_win, scratch_gc,
                                               lx * MagFactor, ly * MagFactor,
                                               MagFactor, MagFactor);
                                if (hotSpot)
                                        if ((lx == X_Hot) && (ly == Y_Hot)) {
                                                XDrawLine(
                                                    dpy, tablet_win, Grid_gc,
                                                    lx * MagFactor,
                                                    ly * MagFactor + MagFactor,
                                                    lx * MagFactor + MagFactor,
                                                    ly * MagFactor);
                                                XDrawLine(
                                                    dpy, tablet_win, Grid_gc,
                                                    lx * MagFactor,
...
void Paint_Tile(int x, int y, GC gc) {
        XFillRectangle(dpy, tablet_win, gc, x * MagFactor, y * MagFactor,
                       MagFactor, MagFactor);

        if (GridEnabled)
                XDrawRectangle(dpy, tablet_win, Grid_gc, x * MagFactor,
                               y * MagFactor, MagFactor, MagFactor);

        if (hotSpot)
                if ((x == X_Hot) && (y == Y_Hot)) {
                        XDrawLine(dpy, tablet_win, Grid_gc, x * MagFactor,
                                  y * MagFactor + MagFactor,
                                  x * MagFactor + MagFactor, y * MagFactor);
                        XDrawLine(dpy, tablet_win, Grid_gc, x * MagFactor,
                                  y * MagFactor, x * MagFactor + MagFactor,
                                  y * MagFactor + MagFactor);
                }
}
...
void Transfer_Back_Image(int x1, int y1, int x2, int y2, Boolean tflag) {
        ...
        if (debug) {
                ...
                stat_out("Values are x1:%d, y1:%d, x2:%d, y2:%d, flag = ", x1,
                         y1, x2, y2);
        ...
        if (tflag == FILL) {
                ...
                for (i = min_x; i <= max_x; i++) {
                        XSetForeground(dpy, scratch_gc,
                                       XGetPixel(scratch_img, i, min_y));
                        Paint_Tile(i, min_y, scratch_gc);
                }
                for (i = min_x; i <= max_x; i++) {
                        XSetForeground(dpy, scratch_gc,
                                       XGetPixel(scratch_img, i, max_y));
                        Paint_Tile(i, max_y, scratch_gc);
                }
                for (i = min_y; i <= max_y; i++) {
                        XSetForeground(dpy, scratch_gc,
                                       XGetPixel(scratch_img, min_x, i));
                        Paint_Tile(min_x, i, scratch_gc);
                }
                for (i = min_y; i <= max_y; i++) {
                        XSetForeground(dpy, scratch_gc,
                                       XGetPixel(scratch_img, max_x, i));
...
void Init_Pen_Colors(Widget wid) {
        ...
        i = 0;
        XtSetArg(arg[i], XmNbackground, &Background);
        i++;
        ...
        i = 0;
        XtSetArg(arg[i], XmNbackground, Foreground);
        i++;
        pixelTableIndex = PixelTableLookup(Foreground, False);
        if (PIXEL_TABLE_MONO(pixelTableIndex) == white_pixel) {
                XtSetArg(arg[i], XmNforeground, black_pixel);
                i++;
        } else {
                XtSetArg(arg[i], XmNforeground, white_pixel);
                i++;
        }
        ...
        XtSetArg(arg[i], XmNbackground, Background);
        i++;
        XtSetArg(arg[i], XmNforeground, Foreground);
        i++;
        XtSetValues(bgColorToggle, arg, i);

        /*** Set TOP_SHADOW Button fg/bg colors ***/
        i = 0;
        XtSetArg(arg[i], XmNbackground, TopShadow);
        i++;
        if (TopShadow == Foreground) /* B & W */
        {
                XtSetArg(arg[i], XmNforeground, Background);
                i++;
        } else {
                XtSetArg(arg[i], XmNforeground, Foreground);
                i++;
        ...
        i = 0;
        XtSetArg(arg[i], XmNbackground, BottomShadow);
        i++;
        ...
        i = 0;
        XtSetArg(arg[i], XmNbackground, Select);
        i++;
        XtSetArg(arg[i], XmNforeground, Foreground);
        i++;
        ...
        i = 0;
        XtSetArg(arg[i], XmNbackground, &Transparent);
        i++;
        XtSetArg(arg[i], XmNforeground, &transFg);
        i++;
        ...
        for (i = 0; i < NUM_STATICS; i++) {
                ...
                if (!status) {
                        sprintf(
                            err_str, "%s %d",
                            GETSTR(10, 30, "Unable to parse static color no."),
                            i + 1);
                        Abort(err_str);
                }
                ...
                if (!status) {
                        sprintf(err_str, "%s %d",
                                GETSTR(10, 32,
                                       "Unable to allocate static color no."),
                                i + 1);
                        Abort(err_str);
                }
                StaticPen[i] = exact_def.pixel;
                j = 0;
                XtSetArg(arg[j], XmNbackground, StaticPen[i]);
                j++;
...
void Init_Color_Table(void) {
        ...
        ***/
        ...
        colorSymbols =
            (XpmColorSymbol *)XtMalloc(NUM_PENS * sizeof(XpmColorSymbol));
        if (!colorSymbols)
                Abort(GETSTR(10, 38, "No memory(0)"));
        for (i = 0; i < NUM_STATICS; i++) {
                colorSymbols[i].name =
                    (char *)XtMalloc(strlen(color_table[i][1]) + 1);
        ...
        if (debug) {
                ...
                for (i = 0; i < NUM_PENS; i++) {
                        stat_out("    %d - %s | %s | %d\n", (i + 1),
                                 colorSymbols[i].name, colorSymbols[i].value,
                                 colorSymbols[i].pixel);
                }
        }
#endif
        ...
        for (i = 0; i < NUM_PENS; i++) {
                ...
                for (j = 0; j < 6; j++) {
                        if (color_table[i][j] &&
                            strlen(color_table[i][j]) > 0) {
                                colorTable[i][j] = (char *)XtMalloc(
                                    strlen(color_table[i][j]) + 1);
                                if (!colorTable[i][j])
...
void Size_IconForm(Dimension width, Dimension height) {
         ...
         *********************************************************************/
        i = 0;
        XtSetArg(args[i], XmNfontList, &label_fontlist);
        i++;
        XtSetArg(args[i], XmNlabelString, &label1_str);
        i++;
        XtGetValues(iconSize, args, i);
        i = 0;
        XtSetArg(args[i], XmNlabelString, &label2_str);
        i++;
        ...
        i = 0;
        XtSetArg(args[i], XmNheight, &label1_h);
        i++;
        XtGetValues(iconSize, args, i);
        i = 0;
        XtSetArg(args[i], XmNheight, &label2_h);
        i++;
        XtGetValues(monoLabel, args, i);
        ...
        if ((label1_w > width) || (label2_w > width))
                diff =
                    (Position)(((label1_w > label2_w) ? label1_w : label2_w) -
                               width) /
                    2;
        else
                diff = 0;
        ...
        i = 0;
        XtSetArg(args[i], XmNleftOffset, diff);
        i++;
         ...
         *********************************************************************/
        i = 0;
        XtSetArg(args[i], XmNtopOffset, &top_offset);
        i++;
...
#endif
        i = 0;
        XtSetArg(args[i], XmNwidth, form_width);
        i++;
        XtSetArg(args[i], XmNheight, form_height);
        i++;
...
void Init_Icons(Dimension width, Dimension height, Boolean saveFlag) {
        ...
        color_icon =
            XCreatePixmap(dpy, root, width, height, DefaultDepth(dpy, screen));
        mono_icon =
            XCreatePixmap(dpy, root, width, height, DefaultDepth(dpy, screen));

        if ((color_icon == 0) || (mono_icon == 0))
                Abort(GETSTR(10, 50,
                             "Cannot initialize application icon storage"));
        ...
        XtSetArg(args[i], XmNlabelString, local_str);
        i++;
        ...
        if (saveFlag) {
                ...
                XCopyArea(dpy, tmp_mono, mono_icon, Mono_gc, 0, 0, icon_width,
                          icon_height, x_offset, y_offset);
                if (tmp_color)
        ...
        if (XtWindow(iconImage))
                XCopyArea(dpy, color_icon, XtWindow(iconImage), Color_gc, 0, 0,
                          width, height, 0, 0);
        if (XtWindow(monoImage))
                XCopyArea(dpy, mono_icon, XtWindow(monoImage), Mono_gc, 0, 0,
                          width, height, 0, 0);
        i = 0;
        XtSetArg(args[i], XmNwidth, ((width * MagFactor) + 1));
        i++;
        XtSetArg(args[i], XmNheight, ((height * MagFactor) + 1));
        i++;
        XtSetValues(tablet_wid, args, i);
...
void RegisterDropSites(void) {
         ...
         */
        n = 0;
        XtSetArg(args[n], XmNanimationStyle, XmDRAG_UNDER_NONE);
        n++; /* first */
        XtSetArg(args[n], DtNdropAnimateCallback, animateCB);
        n++;
        XtSetArg(args[n], DtNregisterChildren, True);
        n++; /* last */
...
void stat_out(char *msg, char *arg0, char *arg1, char *arg2, char *arg3,
        ...
        /*        sprintf(str, msg, arg0, arg1, arg2, arg3, arg4, arg5, arg6);
                  _DtSimpleError ("dticon-stat_out", DtError, NULL, str,
           NULL);*/
}
...
int PixelTableLookup(Pixel pixelIn, Boolean allocNew) {
        ...
        if (pixelTable.lastFound < pixelTable.numItems)
                if (pixelTable.item[pixelTable.lastFound].xcolor.pixel ==
                    pixelIn)
        ...
        if (pixelTable.numItems == pixelTable.pixelTableSize) {
                ...
                pixelTable.item = (PixelTableItem *)XtRealloc(
                    (char *)pixelTable.item,
        ...
        grayValue = ((int)((pixelTable.item[i].xcolor.red * 299) +
                           ...
                           (pixelTable.item[i].xcolor.blue * 114)) /
                     1000) >>
                    8;
        ...
        if (allocNew) {
                ...
                if (!XAllocColor(dpy, Cmap, &tmpXColor)) {
                         ...
                         */
                        pixelTable.item[i].newCell =
                            pixelTable.item[i].xcolor.pixel;
                        DoErrorDialog(GETSTR(
                            10, 62, "Operation failed.\nColormap is full"));
                } else
...
int Select_New_Pen(int n) {
        ...
        if ((new_block == ColorBlock) && (new_pen == CurrentColor)) {
                if (new_block == STATIC_COLOR)
                        ...
                        XmToggleButtonSetState(DynamicWid[new_pen], True,
                                               False);
                return 0;
        }
        ...
        XSetForeground(dpy, Mono_gc,
                       (ColorBlock ? DynamicMono[CurrentColor]
                                   : StaticMono[CurrentColor]));
}
...
void Backup_Icons(void) {
        ...
        if ((icon_width != backup_width) || (icon_height != backup_height) ||
            (!prev_color_icon) || (!prev_mono_icon)) {
                if (prev_color_icon)
                        XFreePixmap(dpy, prev_color_icon);
                if (prev_mono_icon)
                        XFreePixmap(dpy, prev_mono_icon);
                prev_color_icon =
                    XCreatePixmap(dpy, root, icon_width, icon_height,
        ...
        XCopyArea(dpy, color_icon, prev_color_icon, Color_gc, 0, 0, icon_width,
                  icon_height, 0, 0);
        XCopyArea(dpy, mono_icon, prev_mono_icon, Mono_gc, 0, 0, icon_width,
                  icon_height, 0, 0);
        UndoFlag = True;
...
int LoadGrabbedImage(int x, int y, int width, int height) {
        ...
        if (width > 0 && height > 0)
                img = XGetImage(dpy, root, x, y, width, height, AllPlanes,
                                format);
        ...
        for (i = 0; i < img->width; i++) {
                for (j = 0; j < img->height; j++) {
                        pixelTableIndex =
                            PixelTableLookup(XGetPixel(img, i, j), True);
                        XPutPixel(img, i, j,
                                  PIXEL_TABLE_NEW_CELL(pixelTableIndex));
                        XPutPixel(mono_img, i, j,
                                  PIXEL_TABLE_MONO(pixelTableIndex));
                }
        }

        Backup_Icons();
        Init_Icons(img->width, img->height, DO_NOT_SAVE);
        XPutImage(dpy, color_icon, Color_gc, img, 0, 0, 0, 0, img->width,
                  img->height);
        XPutImage(dpy, mono_icon, Mono_gc, mono_img, 0, 0, 0, 0, img->width,
                  img->height);
        ...
        if (xrdb.session != NULL)
                ...
                for (i = 1; i < num; i++) {
                            ...
                            '-') /* process command line switches */
                        {
                                ...
                                case 'f':
                                        if (++i < num) {
                                                if (strncpy(start_file, cmd[i],
                                                            255)) {
                                                        param_flag[AUTO_FILE] =
                                                            True;
                                                        argsNeedProcessed =
                                                            True;
                                                } else
                                ...
                                case 'x':
                                        if (++i < num) {
                                                if (strcpy(start_size,
                                                           cmd[i])) {
                                                        param_flag[AUTO_SIZE] =
                                                            True;
                                                        argsNeedProcessed =
                                                            True;
                                                } else
...
void ProcessAppArgs(void) {
        ...
        if (param_flag[AUTO_FILE] ||
            (session.useSession && start_file[0] != '\0')) {
                ...
                if (!Read_File(start_file))
                        ...
                        if (successFormat == FORMAT_XPM) {
                                ...
                                Display_XPMFile(xpm_ReadAttribs.width,
                                                xpm_ReadAttribs.height);
                ...
                result = XParseGeometry(start_size, &x_ret, &y_ret, &width_ret,
                                        &height_ret);
                if ((!MASK) & result)
...
void Set_Gfx_Labels(Boolean flag) {
        ...
        if (flag == HOLLOW) {
                i = 0;
                XtSetArg(args[i], XmNlabelPixmap, rectPix);
                i++;
                XtSetValues(rectangleButton, args, i);
                i = 0;
                XtSetArg(args[i], XmNlabelPixmap, circlePix);
                i++;
                XtSetValues(circleButton, args, i);
                i = 0;
                XtSetArg(args[i], XmNlabelPixmap, polygonPix);
                i++;
                XtSetValues(polygonButton, args, i);
                i = 0;
                XtSetArg(args[i], XmNlabelPixmap, ellipsePix);
                i++;
                XtSetValues(ellipseButton, args, i);
        } else {
                i = 0;
                XtSetArg(args[i], XmNlabelPixmap, rectSolidPix);
                i++;
                XtSetValues(rectangleButton, args, i);
                i = 0;
                XtSetArg(args[i], XmNlabelPixmap, circleSolidPix);
                i++;
                XtSetValues(circleButton, args, i);
                i = 0;
                XtSetArg(args[i], XmNlabelPixmap, polygonSolidPix);
                i++;
                XtSetValues(polygonButton, args, i);
                i = 0;
                XtSetArg(args[i], XmNlabelPixmap, ellipseSolidPix);
                i++;
                XtSetValues(ellipseButton, args, i);
        }
}
...
static int jskXerrorDebug(disp, error_event) Display *disp;
 ...
 **************************************************************************/
static int jskXerrorIODebug(disp) Display *disp;
{
        exit(-1);
        return (TRUE);
}
...
void SaveSession(void) {
        ...
        n = 0;
        xa_CommandStr[n] = execName;
        n++;
        xa_CommandStr[n] = "-session";
        n++;
...
void GetSessionInfo(void) {
        ...
        if (session.width < DEFAULT_WIDTH)
                session.width = DEFAULT_WIDTH;
        ...
        if (session.height < DEFAULT_HEIGHT)
                session.height = DEFAULT_HEIGHT;
