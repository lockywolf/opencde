...
static int build_tree(void) {
        ...
        for (i = 0; i < 257; i++) {
                ...
                if (hctree1[i].sort > MAX_BITLEN) {
                        fprintf(stderr, "%s",
                                catgets(dtsearch_catd, MS_huff, 30,
                                        "\n183 Bit strings have grown too "
                                        "large.  You probably "
                                        "have literals\n  turned off with "
                                        "grossly unbalanced "
                                        "character counts.\n\7"));
                ...
                if (hctree1[curr].count >= total_count) {
                        fprintf(
                            stderr, "%s",
                            catgets(
                                dtsearch_catd, MS_huff, 31,
                                "\n191 Programming Error:  Still trying to "
                                "build\n"
                ...
                if (low0 == -1 || hctree1[curr].count < hctree1[low0].count ||
                    (hctree1[curr].count == hctree1[low0].count &&
                     hctree1[i].sort < len0)) {
                        low1 = low0;
                        len1 = len0;
                        low0 = curr;
                ...
                if (low1 == -1 || hctree1[curr].count < hctree1[low1].count ||
                    (hctree1[curr].count == hctree1[low1].count &&
                     hctree1[i].sort < len1)) {
                        low1 = curr;
                        len1 = hctree1[i].sort;
                        continue;
...
static void init_treebase(void) {
         ...
         * at output time.  FILE FORMAT IS NOT CHECKED--IT HAD BETTER BE
         * CORRECT!
         */
        else {
                ...
                while (fgets(filebuf, sizeof(filebuf) - 1, instream_huf) !=
                       NULL) {
                        ...
                        strtok(NULL, DELIMITERS); /* skip over current huff
                                                   * code */
                        hctree1[i].count =
                            (i == 256) ? 0L : atol(strtok(NULL, DELIMITERS));
                        hctree1[i].bit = LAST_BIT;
                        hctree1[i].father = -1;
                        hctree1[i].son0 = -1;
                        hctree1[i].son1 = -1;
...
static void huffman_code(time_t idstamp) {
        ...
        if ((outstream_huc = fopen(filename_huc, "w")) == NULL) {
                fprintf(stderr,
                        catgets(dtsearch_catd, MS_huff, 34,
                                "424 File '%s' failed to open for write.  Is "
                                "it read-only?\n"),
                        filename_huc);
                exit(2);
        }
        if ((outstream_huf = fopen(filename_huf, "w")) == NULL) {
                fprintf(stderr,
                        catgets(dtsearch_catd, MS_huff, 34,
                                "439 File '%s' failed to open for write.  Is "
                                "it read-only?\n"),
                        filename_huf);
                exit(2);
        }
...
static void print_usage(void) {
        fprintf(
            stderr,
            catgets(
                dtsearch_catd, MS_huff, 35,
                "USAGE: huffcode [-lN | -l-] [-o] <huffname> [<infile>]\n"
                "  -l<N> specifies the 'literal' threshold count.  Any "
                "character "
                "occurring\n"
                "       <= <N> times will be coded with the Huffman literal.  "
                "Default is -l0,\n"
                "       literal coding only for bytes with counts of zero.\n"
                "  -l-  turns off literal coding.  Turning off literal coding "
                "in "
                "unbalanced\n"
                "       trees leads to EXTREMELY LONG bit string codes--don't "
                "do it "
                "unless\n"
                "       the input is known to be a well balanced binary file.\n"
                "  -o   preauthorizes overwriting any currently existing "
                "decode "
                "file.\n"
                "  <huffname> is the filename prefix for the Huffman Code "
                "files.\n"
                "       If the encode file (%s) already exists, byte counts "
                "from "
                "infile will\n"
                "       be added to it, otherwise it will be newly created.\n"
                "       The decode file (%s) is always newly created each "
                "run.\n"
                "  <infile>   is an input file containing bytes to be "
                "counted.\n"
                "       It may be omitted if the encode file already "
                "exists.\n"),
...
static void user_args_processor(int argc, char **argv) {
        ...
        while (--argc > 0 && (*++argv)[0] == '-') {
                ...
                default:
                        fprintf(stderr,
                                catgets(dtsearch_catd, MS_huff, 36,
        ...
        if (argc <= 0) {
                fprintf(
                    stderr, "%s",
                    catgets(dtsearch_catd, MS_huff, 37,
...
int main(int argc, char *argv[]) {
        ...
        printf(catgets(dtsearch_catd, MS_huff, 40, "HUFFCODE Version %s\n"),
               AUSAPI_VERSION);
        ...
        if (!input_file_specified && no_huffcode_file) {
                fprintf(
                    stderr,
                    catgets(dtsearch_catd, MS_huff, 43,
        ...
        if (input_file_specified) {
                ...
                BAD_INPUT_FILE:
                        fprintf(stderr,
                                catgets(dtsearch_catd, MS_huff, 44,
                                        "Could not open input file '%s' or "
                                        "access status: %s\n"),
                ...
                while ((mychar = getc(instream)) != EOF) {
                        ...
                        if (!(++bytes_in % 10000L))
                                printf(
                                    catgets(dtsearch_catd, MS_huff, 46,
                                            "\r%ld%% done. %2ld Kbytes read.  "
                                            "Estimate %3ld seconds to "
                                            "completion.   "),
