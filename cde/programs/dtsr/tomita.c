...
static void print_exit_code(int exit_code) {
        printf(catgets(dtsearch_catd, MS_tomita, 3, "%s: Exit Code = %d.\n"),
               aa_argv0, exit_code);
        return;
} /* print_exit_code() */
...
static void kill_delete(int sig) {
        shutdown_now = TRUE;
        printf(
            catgets(dtsearch_catd, MS_tomita, 1,
...
static void retncode_abort(int location) {
        ...
        fprintf(aa_stderr,
                PROGNAME
                "%d Program abort.  usrblk.retncode = %d.  Exit code = 3.\n",
...
static int change_database(char *newname) {
        ...
        retncode = 1;
        fprintf(aa_stderr,
                catgets(dtsearch_catd, MS_tomita, 4,
        ...
        fprintf(aa_stderr, "%s",
                catgets(dtsearch_catd, MS_tomita, 5, "Available choices are:"));
        for (db = usrblk.dblist; db != NULL; db = db->link)
...
static int parse_infbuf(char *infbuf) {
        ...
        if ((ptr = strtok(mybuf, " \t")) == NULL) {
                ...
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_tomita, 8,
...
static int browser(void) {
        ...
        while (!done) {
                ...
                printf(
                    catgets(
                        dtsearch_catd, MS_tomita, 10,
                        "\n---------- SHOW NEXT RECORD ----------- Database = "
                        "'%s'\n"
                        "q      QUIT.                        Current Record "
                        "Count = %ld\n"
                        ...
                        "> "),
                    usrblk.dblk->name, usrblk.dblk->dbrec.or_reccount,
                ...
                *userbuf = '\0';
                ...
                case '-':
                        usrblk.request =
                            (*userbuf == '+') ? OE_NEXT_DBA : OE_PREV_DBA;
                        Opera_Engine();
                        break;
                ...
                case 'x':
                        ...
                        fprintf(outf, DISCARD_FORMAT, usrblk.dblk->name,
                                usrblk.objrec.or_objkey, usrblk.userid,
                                datestr);
                        printf(catgets(dtsearch_catd, MS_tomita, 15,
                                       "%s '%s' appended to file of confirmed "
                                       "deletions.\n"),
                               PROGNAME "317", usrblk.objrec.or_objkey);
                        continue;

                default:
                        printf("%s", catgets(dtsearch_catd, MS_tomita, 16,
                                             "...what?\n"));
                        continue;
                } /* end switch */
                ...
                if (usrblk.retncode == OE_WRAPPED)
                        fprintf(
                            aa_stderr,
                            catgets(dtsearch_catd, MS_tomita, 17,
        ...
        DISPLAY_RECORD:
                ...
                for (ptr = usrblk.cleartext; *ptr != 0; ptr++) {
                        ...
                        if (pausing && *ptr == '\n') {
                                if (++pause_counter >= PAUSE_ROWS) {
                                        ...
                                        printf("%s",
                                               catgets(dtsearch_catd, MS_tomita,
                                                       21,
                                                       "\n...push ENTER to "
                                                       "continue... "));

                                        *userbuf = '\0';
                                        if (NULL == fgets(userbuf,
                                                          sizeof(userbuf),
                                                          stdin)) {
                ...
                if (usrblk.notes != NULL) {
                        printf(
                            catgets(dtsearch_catd, MS_tomita, 20,
                                    "--------------------------------------\n"
                            ...
                            usrblk.objrec.or_objkey);
                        pause_counter += 5;
                        for (llptr = usrblk.notes; llptr != NULL;
                             llptr = llptr->link) {
...
KEEP_READING:
        ...
        while (usrblk.dbacount < max_dbacount) {
                ...
                if (usrblk.retncode == OE_WRAPPED) {
                        ...
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_tomita, 24,
...
static void deleter(char *infname) {
        ...
        if (!yesarg) {
                printf(
                    catgets(
                        dtsearch_catd, MS_tomita, 25,
                        "\nDO NOT CONTINUE under any of the following "
                        "circumstances:\n"
                        "-> If the input file which lists record ids to be "
                        "deleted is not\n"
                        "      named '%s'.\n"
                        "-> If any users are still accessing the affected "
                        "database(s).\n"
                        "-> If any database files have not been backed up.\n\n"
                        "If you are sure you are ready to start deleting, "
                        "enter 'y' "
                        "now... "),
                    infname, OE_prodname);
        ...
        usrblk.dbatab = austext_malloc(sizeof(DB_ADDR) * (max_dbacount + 2),
                                       PROGNAME "531", NULL);
        ...
        while (load_dbatab()) {
                ...
                printf(
                    catgets(
                        dtsearch_catd, MS_tomita, 26,
                        "%s %ld read, %ld deleted, %ldh %2ldm %2lds elapsed.\n"
                        "  Database '%s': Current record count = %ld, Batch "
                        "size = %d.\n"),
                    aa_argv0, records_read, records_deleted, hours, minutes,
                    seconds, usrblk.dblk->name, usrblk.dblk->dbrec.or_reccount,
                    usrblk.dbacount);
                /*****fflush (stdout);*****/
        ...
        printf(
            catgets(
                dtsearch_catd, MS_tomita, 27,
                "%s %ld records read from input file.  %ld were deleted and\n"
                "  %ld were not found in %ld hours, %ld minutes, %ld "
                "seconds,\n"),
            aa_argv0, records_read, records_deleted,
            records_read - records_deleted, hours, minutes, seconds);
        ...
        printf(catgets(dtsearch_catd, MS_tomita, 28,
                       "  or an average of %ld minutes, %ld seconds per record "
                       "deleted.\n"),
               minutes, seconds);
        return;
} /* deleter() */
...
int main(int argc, char *argv[]) {
        ...
        strftime(timebuf, sizeof(timebuf),
                 ...
                 localtime(&mytime));
        printf(catgets(dtsearch_catd, MS_tomita, 29, "%s.  Run %s.\n"),
               aa_argv0, timebuf);
        ...
        BAD_ARGS:
                fprintf(
                    aa_stderr,
                    catgets(
                        dtsearch_catd, MS_tomita, 30,
                        "\nUSAGE: %s [options]\n"
                        "  -i    Input file name.  If not specified, defaults "
                        "to %s.\n"
                        "  -d[v] Print debug statements.\n"
                        "        -dv turns on verbose (record-by-record) "
                        "debugging.\n"
                        "  -t<N> Max desired number of seconds of run time.\n"
                        "        Ctrl-C/Break will also stop deletion at next "
                        "record.\n"
                        "  -n<N> Change number of records in a batch from %d "
                        "to <N>.\n"
                        "  -y    Automatically answers 'yes' to Delete mode "
                        "confirm "
                        "prompt.\n"
                        "  -d    trace deletion operations.\n"),
                    aa_argv0, FNAME_DISCARD_DATA, FNAME_CONFIRM_LIST,
                    FNAME_CONFIRM_LIST, DBACOUNT);
                DtSearchExit(2);
        }
        ...
        for (argc -= 2, argv += 2; argc > 0; argc--, argv++) {
                ...
                default:
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_tomita, 31,
        ...
        if ((inf = fopen(infname, "r")) == NULL) {
                if (prog == 'D') {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_tomita, 32,
        ...
        if (prog == 'B') {
                ...
                /* the blank in "a " works around old aix bug */
                {
                        fprintf(
                            aa_stderr,
                            catgets(dtsearch_catd, MS_tomita, 33,
