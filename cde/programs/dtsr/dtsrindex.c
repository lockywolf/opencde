...
static unsigned long default_hashsize;
char dicname[10];
...
struct or_hwordrec got_word;
static FILE *instream;
char *inbuf;
int inbuf_overflowed = FALSE;
size_t inbufsz = INBUFSZ;
int is_pmr;
static DtSrINT32 or_maxdba = 0;
static char msg_374[] = "\n%s Out of Memory!\n"
...
static unsigned long seconds_left;
extern int shutdown_now;
static DtSrINT32 or_recslots;
...
void write_to_file(TREENODE *output_node) {
        ...
        if (debugging & (DEBUG_T | DEBUG_t)) { /* Print out tree node */
                printf(" node '%s' %c%c%c", displayable(output_node->word),
                       (output_node->llink) ? 'L' : '.',
                       (output_node->rlink) ? 'R' : '.',
                       (debugging & DEBUG_t) ? '\n' : ' ');
        }
        ...
        while (print_dba != NULL) {
                ...
                if (debugging & DEBUG_t)
                        printf("    dba #%ld: node adr=%ld cnt=%ld",
                               (long)num_addrs_for_word, (long)mydba,
                               (long)print_dba->w_c);
                ...
                record_addr_word[num_addrs_for_word] +=
                    (log((double)(print_dba->w_c) + 0.5) /
                     log((double)(dbas_word_count[mydba] + 1))) *
                    256;
                ...
                if (num_addrs_for_word >= batch_size) {
                        printf(catgets(dtsearch_catd, MS_cborodin, 280,
                                       "\n%s num_addrs_for_word (%ld) >= "
                                       "batchsz (%ld).\n"),
                               PROGNAME "280", (long)num_addrs_for_word,
                               (long)batch_size);
                        DtSearchExit(91);
                }
        }
...
static void print_usage_msg(void) {
        printf(
            catgets(
                dtsearch_catd, MS_cborodin, 17,
                "\n"
                "USAGE: %s -d<dbname> [options] <infile>\n"
                "       Listed default file name extensions can be "
                "overridden.\n"
                "  -d<dbname>  1 - 8 character database name, include optional "
                "path "
                "prefix.\n"
                "  -t<etxstr>  End of text document delimiter string.  Default "
                "'\\f\\n'.\n"
                "  -r<N>       Change Pass 1 records-per-dot from %d to <N>.\n"
                "  -b<N>       Change max batch size from %ld to <N>.\n"
                "  -c<N>       Change database paging cache from %ld 1K pages "
                "to <N> "
                "1K pages.\n"
                "              <N> >= 16 by powers of 2.  Initially try only "
                "small "
                "changes.\n"
                "  -i<N>       Change (i)nput buffer size from default %d to "
                "<N>.\n"
                "  -h<N>       Change duplicate record id hash table size from "
                "%ld "
                "to <N>.\n"
                "              -h0 means there are no duplicates, do not check "
                "for "
                "them.\n"
                "  <infile>    Input [path]file name.  Default extension "
                "%s.\n"),
...
void user_args_processor(int argc, char **argv) {
        ...
        while (--argc > 0 && (*++argv)[0] == '-') {
                ...
                case 'c':
                        ...
                        CACHE_ADJUSTED:
                                printf(
                                    catgets(dtsearch_catd, MS_cborodin, 600,
                ...
                default:
                        printf(
                            catgets(dtsearch_catd, MS_cborodin, 567,
        ...
        if (dicname[0] == 0) {
                printf(
                    catgets(dtsearch_catd, MS_cborodin, 589,
 ...
 */
static void put_addrs_2_dtbs_addr_file(DB_ADDR *addrs_array, DtSrINT32 nitems) {
        ...
        if (nitems >= batch_size) {
                printf(catgets(dtsearch_catd, MS_cborodin, 6,
                               "put_addrs_2_dtbs_addr_file() nitems=%d, "
                               "batchsz=%ld\n"),
        ...
        got_word.or_hwaddrs +=
            nitems; /** somehow, this can exceed total
        ...
        if (nitems > got_word.or_hwfree) {
                ...
                if (free_slot == NULL) {
                        ...
                        got_word.or_hwfree =
                            free_slot->hole_size - got_word.or_hwaddrs;
                }
                ...
                if (num_writes != num_addrs) {
                        printf(
                            catgets(dtsearch_catd, MS_cborodin, 776, msg_776),
...
void write_2_dtbs_addr_file(void) {
        ...
        if (debugging & DEBUG_O)
                printf("  old vis '%s' ofs=%ld adr=%ld fre=%ld\n",
                       ...
                       (long)got_word.or_hwoffset, (long)got_word.or_hwaddrs,
        ...
        if (num_addrs_ii > or_reccount) {
                printf(
                    catgets(
                        dtsearch_catd, MS_cborodin, 713,
                        "\n%s Word '%s' occurs in %ld records,\n"
                        "  but there are only %ld records in database!\n"
                        "  (This may be a good candidate for the stoplist).\n"),
                    PROGNAME "713", (long)got_word.or_hwordkey,
                    (long)num_addrs_ii, (long)or_reccount);
                DtSearchExit(68);
        }

        if (fseek(dtbs_addr_fp, (long)got_word.or_hwoffset, SEEK_SET) != 0) {
                printf(
                    catgets(dtsearch_catd, MS_cborodin, 875,
                            "\n%s Could not fseek d99 file to offset %ld.\n"),
                    PROGNAME "875", got_word.or_hwoffset);
                DtSearchExit(98);
        }
        num_reads = fread(word_addrs_ii, sizeof(DB_ADDR), (size_t)num_addrs_ii,
                          dtbs_addr_fp);
        if (num_reads != num_addrs_ii) {
                printf(catgets(dtsearch_catd, MS_cborodin, 848,
                               "\n%s Could not fread %ld bytes (%ld dba's) of "
                               "d99 file\n"
                               "  at offset %ld.  Number of dba's read (return "
                               "code) = %ld.\n"),
                       PROGNAME "848", sizeof(DB_ADDR) * num_addrs_ii,
                       (long)num_addrs_ii, (long)got_word.or_hwoffset,
                       (long)num_reads);
                DtSearchExit(98);
        }
#ifdef BYTE_SWAP
        ...
        if (check_existing_addrs) {
                ...
                for (i = 0; i < num_addrs_ii; i++) {
                        if (debugging & DEBUG_o)
                                printf("  old d99 %ld: x%lx(%ld:%ld)", (long)i,
                                       (long)word_addrs_ii[i],
                                       (long)word_addrs_ii[i] >> 8,
                                       (long)word_addrs_ii[i] & 0xffL);

                        /* Get 'record number' by shifting hi 3 bytes 1 byte (8
                         * bits) to right over stat wt byte.  D99 rec#'s start
                         * at 1, so subtract 1 to start at 0 for bit vector.
                         */
                        temp1 = (*(word_addrs_ii + i) >> 8) -
                                1; /* = rec#, base 0 */
                        cur_byte =
                            temp1 >> 3; /* get matching byte# in bit vector */
                        if (cur_byte >= bit_vector_size) {
                                printf(catgets(dtsearch_catd, MS_cborodin, 9,
                                               "\n%s Corrupted d99 file for "
                                               "word '%s',\n"
                                               " database address %ld @ file "
                                               "position %ld => bitvector[%ld],"
                                               " but max bitvector allocation "
                                               "= %ld.\n"),
                                       PROGNAME "727",
                                       displayable(got_word.or_hwordkey),
                                       (long)temp1, (long)i, (long)cur_byte,
                         ...
                         * addrs list, is also a doc in the fzk file
                         * (dbas_bits_batch), delete it from the d99 list by
                         * writing subsequent dba's over it.  Boy this recursive
                         * nested loop has gotta be slow. Faster algorithm?  Add
                         * 'good' addrs to the end of record_addr_word[].  No
                         * nested overlay loop, only one write!
                         */
                        if (bit_addrs & (*(dbas_bits_batch + cur_byte))) {
                                ...
                                if (i_start == 0) {
                                        ...
                                        if (i_start < i) {
                                                /* compress: move good addrs
                                                 * over space of deleted ones */
                                                for (k = i_start; k < i; k++) {
                                                        word_addrs_ii[cur_ind] =
                                                            word_addrs_ii[k];
                                                        cur_ind++;
                                                }
                                        }
...
void write_new_word_2_dtbs(void) {
        ...
        if (debugging & (DEBUG_n | DEBUG_N))
                printf("  new word '%s', adrs=%ld,", got_word.or_hwordkey,
                       (long)num_addrs_for_word);
        ...
        if (free_slot == NULL) {
                ...
                ret_fseek =
                    fseek(dtbs_addr_fp, (long)free_slot->offset, SEEK_SET);
                got_word.or_hwoffset = free_slot->offset;
                got_word.or_hwfree = free_slot->hole_size - num_addrs_for_word;
                if (debugging & (DEBUG_n | DEBUG_N))
                        printf(" REUSE slot ofs=%ld, fre=%ld\n",
                               (long int)got_word.or_hwoffset,
                               (long int)got_word.or_hwfree);
        }
...
void fill_data1(char *node_word) {
        ...
        if (!(count_word_ii % words_per_dot)) {
                ...
                if (dotcount >= 50) {
                        ...
                        printf(
                            catgets(dtsearch_catd, MS_cborodin, 849,
                                    "\n%s: Word #%ld, %.0f%% done.  Est %lum "
                                    "%02lus "
                                    "to completion.\n"),
        ...
        if (db_status == S_NOTFOUND) { /* this is a NEW word */
                ...
                fillnew_wordrec(&got_word,
                                0); /* write (empty) vista word rec */
                if (db_status != S_OKAY)
 ...
 */
static void load_into_bintree(char *parser_token, int token_is_stem,
                              DB_ADDR dba) {
        ...
        for (this_link = &root_node; *this_link != NULL;) {
                ...
                if (i == 0) {
                         ...
                         */
                                ...
                                if ((newdba = malloc(sizeof(DBALIST))) ==
                                    NULL) {
                                        printf(catgets(dtsearch_catd,
                                                       MS_cborodin, 374,
                                                       msg_374),
                                               PROGNAME "1150");
                                        DtSearchExit(26);
...
main(int argc, char **argv) {
        ...
        if (!load_language(&dblk, NULL)) {
                ...
                printf(catgets(dtsearch_catd, MS_cborodin, 1097,
                               "%s Aborting due to errors in loading language "
                               "files.\n"),
                       PROGNAME "1097");
                DtSearchExit(3);
        }
        ...
        bit_vector_size =
            ((or_maxdba / or_recslots + 1) >> 3) + 1;  /* DtSrINT32 */
        ...
        if (debugging)
                printf(PROGNAME "1286 "
                       ...
                       (long)or_reccount, (long)or_recslots,
                       (long)bit_vector_size, (int)dba_offset, (long)or_maxdba);
        ...
        word_addrs_ii = (DB_ADDR *)austext_malloc(
            sizeof(DB_ADDR) * (or_reccount + 1) + 48, PROGNAME "1152", NULL);
        mallocsz = sizeof(DtSrINT32) * (or_maxdba + 1) + 48;
        dbas_word_count =
            (DtSrINT32 *)austext_malloc(mallocsz, PROGNAME "1154", NULL);
        ...
        if ((dtbs_addr_fp = fopen(dtbs_addr_file, "r+b")) == NULL) {
                ...
                if (dtbs_addr_fp == NULL) {
                        ...
                        printf(catgets(dtsearch_catd, MS_cborodin, 1068,
                                       "%s Can't open new inverted index file "
                                       "'%s': %s\n"),
                               PROGNAME "1068", dtbs_addr_file,
                               strerror(errno));
                        DtSearchExit(13);
                }
                ...
                if (!fread_d99_header(&fl_hdr, dtbs_addr_fp)) {
                        /* msg 1068 used multiple places */
                        printf(
                            catgets(dtsearch_catd, MS_cborodin, 1068,
        ...
        if (!src)
                src = getenv("PWD");
        printf(
            catgets(dtsearch_catd, MS_misc, 24,
        ...
        while (!feof(instream)) {
                 ...
                 * 5. Let the parser read and parse rest of record, ie doc
                 * text...
                 */
                ...
                if (shutdown_now) {
                        printf(catgets(dtsearch_catd, MS_cborodin, 164,
                                       "\n%s: %s Abort due to signal %d.  "
                                       "Database %s\n"
                                       "  possibly corrupted.  Restore backup "
                                       "database.\n"),
                               aa_argv0, PROGNAME "1299", shutdown_now,
                               dicname);
                        DtSearchExit(11);
                }
                ...
                if (i == 2) { /* out of memory */
                        printf(
                            catgets(dtsearch_catd, MS_cborodin, 374, msg_374),
                            PROGNAME "1317");
                        DtSearchExit(57);
                        ...
                        if (dotcount > 0)
                                putchar('\n');
                        printf(
                            catgets(dtsearch_catd, MS_cborodin, 1402,
                ...
                CRGET(PROGNAME "224", &temp_dba,
                      0);               /* vista already byte swapped */
                temp_dba &= 0x00FFFFFF; /* = slot# */
                dba =
                    (temp_dba + dba_offset) / or_recslots; /* = rec#, base 1 */
                /*
                ...
                if (dba < 1 || dba > or_maxdba) {
                        printf(
                            catgets(dtsearch_catd, MS_cborodin, 21,
                                    "\n%s '%s' record overflows word counter "
                                    "array.\n"
                                    "Record number %ld > maxdba %ld, dba=%ld, "
                                    "recslots=%ld, offs=%d.\n"),
                            PROGNAME "1526", displayable(db_key), (long)dba,
                            (long)or_maxdba, (long)temp_dba, (long)or_recslots,
                            (int)dba_offset);
                        DtSearchExit(68);
                }
                ...
                if (cur_byte >= bit_vector_size) {
                        printf(
                            catgets(dtsearch_catd, MS_cborodin, 22,
                                    "\n%s '%s' record in database (dba=%ld)\n"
                                    "  overflows bitvector allocation (%ld >= "
                                    "%ld).\n"),
                ...
                if (!(record_count % recs_per_dot)) {
                        ...
                        if (dotcount >= 50) {
                                ...
                                seconds_left =
                                    (unsigned long)(((float)fstat_input
                                                             .st_size /
                                                         (float)bytes_in -
                                                     1.) *
                                                    (float)(time(NULL) -
                                                            timestart));
                                printf(
                                    catgets(dtsearch_catd, MS_cborodin, 1190,
                                            "\n%s: Rec #%lu, %.0f%% done.  "
                                            "Est %lum %02lus to end Pass 1.\n"),
                                    aa_argv0, record_count,
                                    (float)bytes_in /
                                        (float)fstat_input.st_size * 100.0,
                                    seconds_left / 60UL, seconds_left % 60UL);
                        }
                        fflush(stdout);
                }
                ...
                if (debugging & DEBUG_P)
                        printf("\nRecord #%lu '%s'\n"
                               "Offset Word----               Stem----\n",
                               record_count, db_key);
                for (cptr = dblk.parser(&parg); cptr;
        ...
        if (duplicate_recids > 0L) {
                normal_retncode = 1; /* 'warning' */
                sprintf(buf,
                        catgets(dtsearch_catd, MS_cborodin, 40,
        ...
        printf(catgets(dtsearch_catd, MS_cborodin, 1225,
                       ...
                       "  %s, parsed %lu words.\n"),
               aa_argv0, elapsed / 60L, elapsed % 60L, record_count, buf,
               num_of_diff_words);
        if (record_count > batch_size) {
                printf(
                    catgets(
                        dtsearch_catd, MS_cborodin, 33,
                        "\n%s Number of incoming records exceeded %d.\n"
                        "  This will usually result in 'Out of Paging Space' "
                        "error in Pass 2\n"
                        "  and corruption of database.  Either split the "
                        "incoming file to\n"
                        "  reduce record count or use the -b option, and "
                        "rerun.\n"),
        ...
        printf(catgets(dtsearch_catd, MS_cborodin, 1233,
                       "%s: Beginning Pass 2: batch index traversal and "
                       "database update.\n"
                       "  Each dot = %d words.\n"),
        ...
        elapsed = time(NULL) - timestart;
        printf(
            catgets(dtsearch_catd, MS_cborodin, 1246,
