        ...
        if (!outmode_specified)
                if ((temp = fopen(outfile, "r")) != NULL) {
                        ...
                        printf(catgets(dtsearch_catd, MS_chandel, 3,
                               ...
                               outfile);
                        printf("%s",
                               catgets(dtsearch_catd, MS_chandel, 4,
...
char *my_strtok(char *s1, const char *s2) {
        ...
        sbegin =
            (s1) ? s1 : ssave; /* start of string or where we last left off */
...
void process_profile() {
        ...
        do {
                ...
                case LINE:
                        ...
                        for (;;) {
                                ...
                                line_current->comp->next =
                                    (struct line_comp *)malloc(
                                        sizeof(struct line_comp));
                                ...
                                if (!strcmp(tok, "*"))
                                        ...
                                        line_current->comp->column_number =
                                            atoi(tok);
                                if (line_current->comp->column_number == 0) {
                                        printf(
                                            catgets(dtsearch_catd, MS_chandel,
                                                    16,
                                                    "Error line %d - zero or "
                                                    "bad value for "
                                                    "identifier '%s'\n  "
                                                    "offensive token: %s.\n"),
                                            line_num, line_current->name, tok);
                                ...
                                tok = my_strtok('\0', "\"");
                                if (!tok) {
                                        if (line_current->comp->column_number ==
                                            ANY)
                                                printf(
                ...
                case FIELD:
                        if (field_table == NULL) {
                                field_table = (struct field_id *)malloc(
                        ...
                        if (field_current->constant) {
                                ...
                                if (!tok) {
                                        printf(
                                            catgets(
                                                dtsearch_catd, MS_chandel, 93,
                                                "Error line %d - '%s' string "
                                                "not "
                                ...
                                field_current->text_length =
                                    strlen(field_current->text);
                ...
                case TEXTEXCLUDE:
                        ...
                        if (validate_id(tok)) {
                                ...
                                /* if valid */
                                        ...
                                        include_current->next =
                                            (struct include *)malloc(
                                                sizeof(struct include));
                                ...
                                if (tok == NULL)
                                        ...
                                        if (validate_id(tok)) {
                                                ...
                                                include_current =
                                                    include_current->next;
                                                include_current->next = NULL;
                                                strcpy(include_current->line_id,
                                                       tok);
                ...
                case IMAGE:
                        ...
                        for (i = 0; i < strlen(tok); i++)
                                tok[i] = tolower(tok[i]);
                        if (strcmp(tok, catgets(dtsearch_catd, MS_chandel, 34,
                                                "all")) == 0)
                                imagemode = INCLUDE;
                        else if (strcmp(tok, catgets(dtsearch_catd, MS_chandel,
                                                     35, "none")) == 0)
                                imagemode = EXCLUDE;
                        else {
                        BAD_IMAGE:
                                printf(
                                    catgets(dtsearch_catd, MS_chandel, 36,
                                            "Error line %d: image mode must be "
                                            "'all' or "
                ...
                case TEXT:
                        ...
                        BAD_TEXT:
                                printf(
                                    catgets(dtsearch_catd, MS_chandel, 37,
                                            "Error line %d: text mode must be "
                                            "'all' or "
                                            "'none' - '%s' not recognized.\n"),
                                    NULLORSTR(tok));
                                bad_profile = TRUE;
                ...
                case DATEFLD:
                        if (date_pos_defined) {
                                printf(catgets(dtsearch_catd, MS_chandel, 110,
                                               "Warning line %d - date field "
                                               "redefined.\n"),
                                       line_num);
                                null_date_specified = FALSE;
                        }
                        date_table =
                            (struct date_id *)malloc(sizeof(struct date_id));
                ...
                case KEY:
                        ...
                        if (warnings && key_pos_defined)
                                printf(catgets(dtsearch_catd, MS_chandel, 40,
                                               "Warning line %d - key field "
                                               "redefined.\n"),
                                       line_num);
                        key_table =
                            (struct key_id *)malloc(sizeof(struct key_id));
                ...
                case DELBLANKLINES:
                        ...
                        BAD_DELBLANKLINES:
                                printf(
                                    catgets(dtsearch_catd, MS_chandel, 46,
                ...
                default:
                        printf(catgets(dtsearch_catd, MS_chandel, 48,
                                       "Error line %d -unknown identifier type "
                                       "'%s'.\n"),
                               line_num, NULLORSTR(tok));
                        bad_profile = TRUE;
                        break;
        ...
        if (!date_pos_defined)
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_chandel, 115,
        ...
        if (!key_defined) {
                bad_profile = TRUE;
                printf("%s",
                       catgets(dtsearch_catd, MS_chandel, 50,
        ...
        if (top_rec_name[0] != 0 && top_rec == NULL) {
                ...
                printf(catgets(dtsearch_catd, MS_chandel, 53,
                               "Error - delimiter defined as '%s' references a "
                               "physical "
                               "line in the record.\n   Since the delimiter "
                               "defines the "
                               "physical lines\n  it cannot be referenced as a "
                               "physical line.\n"),
                       top_rec_name);
                bad_profile = TRUE;
        }
        ...
        while (field_current != NULL) {
                ...
                line_current = line_table;
                while (line_current != NULL) {
                        if (!strcmp(field_current->line_id,
                                    line_current->name)) {
                                found = TRUE;
        ...
        while (finclude_current != NULL) {
                ...
                found = FALSE;
                while (field_current != NULL) {
                        if (!strcmp(field_current->name,
                                    finclude_current->field_id)) {
                                field_current->text_action =
                                    finclude_current->value;
                                found = TRUE;
                        }
                        field_current = field_current->next;
                }
        ...
        while (include_current != NULL) {
                ...
                found = FALSE;
                while (line_current != NULL) {
                        if (!strcmp(line_current->name,
                                    include_current->line_id)) {
                                line_current->text_action =
                                    include_current->value;
                                found = TRUE;
                        }
                        line_current = line_current->next;
             ...
             date_current = date_current->next) {
                ...
                if (!found) {
                        printf(catgets(dtsearch_catd, MS_chandel, 116,
                                       "Error - date references undefined "
                                       "field '%s'.\n"),
        ...
        while (abstract_current != NULL) {
                ...
                if (!found) {
                        printf(catgets(dtsearch_catd, MS_chandel, 59,
                                       "Error - abstract definition references "
                                       "field '%s'\n"
                                       "  which was never defined.\n"),
                               abstract_current->field_id);
                        bad_profile = TRUE;
...
void write_record() {
        ...
        if (fprintf(outstream, "%c\n", CNTRL_L) <= 0) {
                printf(
                    catgets(dtsearch_catd, MS_chandel, 124,
...
static void mmm_to_digits(struct field_id *fld) {
        ...
        if (months == NULL)
                months =
                    strdup(catgets(dtsearch_catd, MS_chandel, 125,
...
void process_record(void) {
                ...
                while (field_current != NULL) {
                        if (field_current->line == record->line) {
                                ...
                                if (field_current->text[0] == 0) {
                                        if (field_current->defined_length ==
                                            EOLN)
                                                strncpy(
                                                    value,
                                                    record->text +
                                                        (field_current->offset -
                                                         1),
                                                    sizeof(value) - 1);
                                        else if (field_current
                                                     ->defined_length == EOW)
                                ...
                                /* ...else if profile pattern str was not empty
                                   ("xxx") */
                                else {
                                        pos = strstr(record->text,
                                                     field_current->text);
                                        if (pos !=
                                            NULL) { /* pattern found... */
                                                if (field_current
                                ...
                                if (field_current->constant == FALSE) {
                                        ...
                                        field_current->value
                                            [sizeof(field_current->value) - 1] =
                                            0;
                                        field_current->length =
                                            strlen(field_current->value);
        ...
        BAD_DATE_VALUE:
                ...
                printf(catgets(dtsearch_catd, MS_chandel, 133,
                               ...
                               "  Using '%s' date for record number %ld that "
                               "began: %.30s\n"),
        ...
        if (dummy && warnings) {
                ...
                printf(catgets(dtsearch_catd, MS_chandel, 69,
                               "Warning - record #ld deemed meaningless, "
                               "discarding...\n"
                               "  record began: %.60s\n"),
                       rec_count, record_head->text);
        } else {
...
void process_infile() {
        ...
        while (fgets(buffer, sizeof(buffer) - 1, instream) != NULL) {
                ...
                if (new_rec(buffer)) {
                        if (bot_defined) {
                                if (record_head == NULL) {
                                        record_head = (struct rec *)malloc(
                                            sizeof(struct rec));
                                        record = record_head;
                                } else {
                                        record->next = (struct rec *)malloc(
                                            sizeof(struct rec));
                                        record = record->next;
                                }
                                record->next = NULL;
                                ...
                                record->line = NULL;
                                /*-- search list of line_id's for a possible
                                 * match --*/
                                line_current = line_table;
                                while (line_current != NULL) {
                                        cant_be = FALSE;
                                        ...
                                        while (line_current->comp != NULL) {
                                                if (strcmp(line_current->comp
                                                               ->text,
                                                           line_mode) != 0) {
                                                        if (line_current->comp
                                                                ->column_number ==
                                                            ANY) {
                                                                if (strstr(
                                                                        buffer,
                ...
                while (line_current != NULL) {
                        ...
                        line_current->comp = line_current->head;
                        while (line_current->comp != NULL) {
                                if (strcmp(line_current->comp->text,
                                           line_mode) != 0) {
                                        if (line_current->comp->column_number ==
                                            ANY) {
                                                if (strstr(buffer,
...
static void usage_msg(void) {
        ...
        if (!warnings)
                return;
        printf(catgets(dtsearch_catd, MS_chandel, 71, default_text), aa_argv0,
               EXT_FZKEY);
        return;
} /* usage_msg() */
...
static void user_arg_processor(int argc, char **argv) {
        ...
        while (--argc > 0 && (*++argv)[0] == '-') {
                ...
                case 'o':
                        ...
                        if (argptr[2] == 'o')
                                ...
                                printf(
                                    catgets(dtsearch_catd, MS_chandel, 75,
        ...
        if (argc-- <= 0) {
                ...
                *****************/
                if (strcmp(infile, "-") == 0) {
                        printf("%s", catgets(dtsearch_catd, MS_chandel, 79,
                                             "Error - using "
                                             "stdin as input, output filename "
                                             "is required!\n"));
                        exit(FILE_ERROR);
                } else {
...
****************************************************************************/
static void flag_shutdown(int sig) { shutdown_now = TRUE; }
...
int main(int argc, char **argv) {
        ...
        printf(
            catgets(dtsearch_catd, MS_chandel, 88, "%s. %s %s Text Filter.\n"),
