...
static struct or_swordrec d21new, d21old;
static struct or_lwordrec d22new, d22old;
static struct or_hwordrec d23new, d23old;
...
static DtSrINT32 reccount = 0;
static DtSrINT32 recslots; /* dbrec.or_recslots promoted to INT32 */
static DtSrINT32 dba_offset;
static DtSrINT32 recs_per_dot = RECS_PER_DOT;
...
static DtSrINT32 total_num_addrs = 0;
...
static void print_usage(void) {
        fprintf(
            aa_stderr,
            catgets(
                dtsearch_catd, MS_dtsrclean, 1,
                "\nUSAGE: %s [options] <dbname> <newpath>\n"
                "       Compresses unused d99 space and validates d00-d99 "
                "links.\n"
                "  -p<N>     Progress dots printed every <N> records (default "
                "%lu).\n"
                "            Complete progress message printed every %d dots.\n"
                "  -oy       Authorizes overlaying preexisting d99/d2<N> files "
                "in "
                "newpath.\n"
                "  -on       Forces exit if preexisting d99/d2<N> files in "
                "newpath.\n"
                "  -v        Validates d99 and d00 links, uncorrupts d99 file, "
                "and "
                "ensures\n"
                "            accurate record count.  Also use -c0 to uncorrupt "
                "entire database.\n"
                "  -v<fname> Same as -v but also writes all d00 recs "
                "unreferenced by "
                "d99\n"
                "            to <fname> in format suitable to extract into "
                ".fzk file "
                "format.\n"
                "  -c<N>     Exits if more than <N> corrupted/incomplete links "
                "(default %d).\n"
                "            Corruption limit turned off by -c0.\n"
                "  <dbname>  1 - 8 char database name = the old d99/d2<N> "
                "files to "
                "be updated.\n"
                "            Files found in local directory or DBFPATH "
                "environment "
                "variable.\n"
                "  <newpath> Specifies where the new d99/d2<N> files will be "
                "placed.\n"
                "            If first char is not slash, path is relative to "
                "local "
                "directory.\n"
                "EXIT CODES:\n"
                "  0: Complete success.  1: Warning.  2: Job never started.\n"
                "  3-49: Job ended prematurely, old files ok, new files "
                "unusable.\n"
                "  50-99: Fatal Error, even old database may be corrupted.\n"
                "  100+: Ctrl-C, kill, and all other signal interrupts cause "
                "premature\n"
                "     end, new files may be unusable.  Signal = exit code - "
                "100.\n"),
...
static void print_progress(char *label) {
        ...
        if ((float)bytes_in / (float)size_d99_old >= 99.5)
                ...
                compression =
                    (int)(100. * (float)bytes_in / (float)size_d99_old);
        ...
        TERMINATE_LINE();
        fprintf(aa_stderr,
                catgets(dtsearch_catd, MS_dtsrclean, 2,
                        "%s: %s Compression %d%% (about %lu KB) in %ld:%02ld "
                        "min:sec.\n"),
                aa_argv0, label, compression, bytes_in / 1000L, seconds / 60UL,
                seconds % 60UL);
        if (*label == 'F')
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_dtsrclean, 3,
...
static void end_of_job(int exitcode, int show_flags) {
        ...
        if (corruption_count > 0L) {
                if (max_corruption > 0L && corruption_count >= max_corruption)
                        fprintf(
                            aa_stderr,
                            catgets(dtsearch_catd, MS_dtsrclean, 193,
                                    "%s Aborting at %ld corrupted links.\n"),
                            PROGNAME "193", corruption_count);
                else
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_dtsrclean, 194,
                                        "%s Detected%s %ld "
                                        "corrupted/incomplete link(s).\n"),
                                PROGNAME "194",
        ...
        if (show_flags & SHOW_EXITCODE)
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_dtsrclean, 5,
                                "%s: Exit code = %d.\n"),
                        aa_argv0, exitcode);
...
static void user_args_processor(int argc, char **argv) {
        ...
        while (--argc > 0) {
                ...
                case 'm':
                        fprintf(
                            aa_stderr,
                            catgets(
                                dtsearch_catd, MS_dtsrclean, 301,
                ...
                case 'o':
                        ...
                        INVALID_ARG:
                                fprintf(aa_stderr,
                                        catgets(dtsearch_catd, MS_dtsrclean,
                                                177,
                ...
                case 'v':
                        ...
                        if (argptr[2] != '\0') {
                                if ((frecids = fopen(argptr + 2, "w")) ==
                                    NULL) {
                                        fprintf(aa_stderr,
                                                catgets(dtsearch_catd,
                                                        MS_dtsrclean, 802,
                                                        "%s Unable to open "
                                                        "'%s' to output"
                                                        " unreferenced d00 "
                ...
                default:
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_dtsrclean, 159,
        ...
        if (argc != 2) {
                if (argc <= 0)
                        fprintf(
                            aa_stderr,
                            catgets(dtsearch_catd, MS_dtsrclean, 210,
                                    "%s Missing required dbname argument.\n"),
                            PROGNAME "210");
                if (argc <= 1)
                        fprintf(
                            aa_stderr,
                            catgets(dtsearch_catd, MS_dtsrclean, 211,
                                    "%s Missing required newpath argument.\n"),
                            PROGNAME "211");
                if (argc > 2)
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_dtsrclean, 212,
        ...
        if (strlen(arg_dbname) > 8) {
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_dtsrclean, 229,
...
static void validation_error(DB_ADDR dbaorig) {
        ...
        if (dbaorig != -1)
                slot = ((slot + 1) * recslots - dba_offset) | (OR_D00 << 24);

        fprintf(
            aa_stderr,
            catgets(dtsearch_catd, MS_dtsrclean, 6,
            ...
            d23old.or_hwordkey + 1, d23old.or_hwoffset, d23old.or_hwaddrs,
            d23old.or_hwfree);
...
/*						*/
/************************************************/
static void open_all_files(FILE **fp, char *fname, char *mode, size_t *size,
                           int *oops) {
        struct stat fstatbuf;

        if ((*fp = fopen(fname, mode)) == NULL) {
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_dtsrclean, 439,
                                "%s Can't open %s: %s\n"),
        ...
        if (fstat(fileno(*fp), &fstatbuf) == -1) {
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_dtsrclean, 440,
        ...
        if (size)
                if ((*size = fstatbuf.st_size) <= 0L) {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_dtsrclean, 499,
                                        "%s %s is empty.\n"),
                                PROGNAME "499", fname);
                        *oops = TRUE;
                }
...
/*						*/
/************************************************/
static void copy_old_d2x_to_new(char *fname_old, char *fname_new, FILE *fp_old,
                                FILE *fp_new) {
        char readbuf[1024 + 32];
        int i, j;

        fprintf(aa_stderr,
                catgets(dtsearch_catd, MS_dtsrclean, 7,
        ...
        for (;;) { /* loop ends when eof set on input stream */
                ...
                if (errno) {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_dtsrclean, 517,
                                        "%s Read error on %s: %s.\n"),
                ...
                if (i != j) {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_dtsrclean, 489,
...
static void copy_new_d99(long keyfield) {
        ...
        while (db_status == S_OKAY) {
                ...
                while (!done) { /* loop on each block in this word */
                        ...
                        errno = 0;
                        ret = fread(word_addrs, sizeof(DB_ADDR),
                                    (size_t)num_reads, fp_d99_old);
                        if (errno || -1 == ret) {
                                TERMINATE_LINE();
                                fprintf(aa_stderr,
                                        catgets(dtsearch_catd, MS_dtsrclean,
                        ...
                        if (validation_mode) {
                                 ...
                                 * the loop is skipped if the entire block is
                                 * free slots.
                                 */
                                good_addrs_this_block = 0;
                                for (a = 0; a < x;
                                     a++) { /* a = index to curr dba */
                                        /*
                                         * Get rec#.  Save original rec# for err
                                         * msgs, then shift slot number to lower
                                         * 3 bytes, discarding weight.
                                         */
                                        dbaorig = word_addrs
                                            [a]; /* rec#,rec#,rec#:wt */
                                        dba =
                                            dbaorig >> 8; /* 0,rec#,rec#,rec# */
                                        is_valid_dba = TRUE; /* default */

                                        /*
                                         * If original rec# == -1 we've overrun
                                         * the good rec#'s into the expansion
                                         * area, which is filled with -1's.
                                         * This is real bad news because if the
                                         * counts in d02 are bad, the online
                                         * programs will quickly crash, and we
                                         * can't continue this program. Advance
                                         * to next rec# because we can't mark
                                         * the bit vector.
                                         */
                                        if (dbaorig == -1L) {
                                                TERMINATE_LINE();
                                        ...
                                        /*
                                         * If slot number > max totrecs, we have
                                         * a corrupted d99-d00 link because
                                         * we've already validated the d00 file
                                         * and we know that it has no slots >
                                         * max.  Also we have to advance to next
                                         * slot because we can't mark the bit
                                         * vector.
                                         */
                                        /******if (dba >= max_totrecs)*******/
                                        ...
                                        /*
                                         * Verify that dba exists in d00 file
                                         * (test bit #1). If not, mark bit #3
                                         * (3rd lowest) in nibble and print
                                         * error msg unless bit #3 previously
                                         * marked.
                                         */
                                        ...
                                        /*
                                         * Mark bit #2 in bit vector indicating
                                         * a d99 reference.
                                         */
                                        *bvptr |= (is_odd_nibble)
                                                      ? 0x02
                                        ...
                                        /*
                                         * move good dba to curr output block,
                                         * incr counter
                                         */
                                        if (is_valid_dba)
                                                word_addrs_out
                                ...
                                /*
                                 * Write out only validated addrs in current
                                 * block. If this was the last block, fill out
                                 * all the free slots, if any, with -1 values,
                                 * and exit the dba loop for this word.
                                 */
                                if (good_addrs_this_block > 0) {
#ifdef BYTE_SWAP
                                        for (swapx = 0;
                                             swapx < good_addrs_this_block;
                                             swapx++)
                                                NTOHL(word_addrs_out[swapx]);
#endif
                                        num_writes = fwrite(
                                            word_addrs_out, sizeof(DB_ADDR),
                                            (size_t)good_addrs_this_block,
                                            fp_d99_new);
                                ...
                                if (good_addrs_left <= 0) {
                                        /*
                                         * Write blocks of -1s until new d2x
                                         * free slot count is exhausted.  The
                                         * last block may be < MAX_REC_READ.
                                         */
                                        slots_left = d23new.or_hwfree;
                                        while (slots_left > 0) {
                                                /*
                                                 * set x to number of -1's to
                                                ...
                                                for (a = 0; a < x; a++)
                                                        word_addrs_out[a] =
                                                            (DtSrINT32)-1;
                                                /* BYTE_SWAP not required for
                                                 * foxes */
                                                num_writes = fwrite(
                                                    word_addrs_out,
                                                    sizeof(DB_ADDR), (size_t)x,
                                                    fp_d99_new);
                                                if (num_writes != x)
                                                        goto WRITE_ERROR;
                                        } /* end while loop to write out all
                                             -1's */
                                        done = TRUE;
                                }
                         ...
                         */
                                ...
                                num_writes =
                                    fwrite(word_addrs, sizeof(DB_ADDR),
                                ...
                                WRITE_ERROR:
                                        fprintf(
                                            aa_stderr,
                                            catgets(
                                                dtsearch_catd, MS_dtsrclean,
                                                665,
                                                "%s Write error on %s: %s.\n"),
                                            PROGNAME "665", fname_d99_new,
                                            strerror(errno));
                                        end_of_job(4, SHOW_PROGRESS +
                                                          SHOW_EXITCODE);
                                }
...
int main(int argc, char *argv[]) {
                ...
                if (*ptr == 0)
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_dtsrclean, 12,
                                        "%s: Ignoring empty DBFPATH "
                                        "environment variable.\n"),
                                aa_argv0);
                else {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_dtsrclean, 13,
                                        "%s: Using DBFPATH = '%s'.\n"),
                                aa_argv0, ptr);
        ...
        if (strcmp(full_dbname_old, full_dbname_new) == 0) {
                fprintf(
                    aa_stderr,
                    catgets(
                        dtsearch_catd, MS_dtsrclean, 393,
        ...
        fprintf(aa_stderr,
                catgets(dtsearch_catd, MS_dtsrclean, 14,
                        "%s: Old files: '%s.d2x, .d99'.\n"),
                aa_argv0, full_dbname_old);
        fprintf(aa_stderr,
                catgets(dtsearch_catd, MS_dtsrclean, 15,
         ...
         */
        copy_old_d2x_to_new(fname_d21_old, fname_d21_new, fp_d21_old,
                            fp_d21_new);
        copy_old_d2x_to_new(fname_d22_old, fname_d22_new, fp_d22_old,
                            fp_d22_new);
        copy_old_d2x_to_new(fname_d23_old, fname_d23_new, fp_d23_old,
                            fp_d23_new);
        ...
        if (!austext_dopen(arg_dbname, (dbfpath[0] == 0) ? NULL : dbfpath, NULL,
                           0, &dbrec)) {
         ...
         */
        total_num_addrs =
            (dbrec.or_maxdba - (dba & 0xffffff) + 1) / recslots + 1;
        fprintf(
            aa_stderr,
            catgets(
                dtsearch_catd, MS_dtsrclean, 25,
        ...
        if (validation_mode) {
                ...
                if ((bit_vector = malloc((size_t)max_bitvec + 64)) == NULL) {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_dtsrclean, 465,
                                        "%s WARNING: Can't allocate memory for "
                                        "bit vector.\n"
                ...
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_dtsrclean, 26,
                                "%s: Reading d00 file.  Each dot appx %ld "
                                "database documents...\n"),
                        aa_argv0, (long)x);
                reccount = 0;
                dot_count = 0L;
                ...
                while (db_status == S_OKAY) {
                        ...
                        dba &= 0x00ffffff; /* mask out file number in high order
                                              byte */
                        dba1 = (dba + dba_offset) /
                               recslots; /* ="rec number", base 1 */
                        if (dba1 >= total_num_addrs) {
                                TERMINATE_LINE();
                                fprintf(aa_stderr,
                                        catgets(dtsearch_catd, MS_dtsrclean,
                                                561,
                                                "%s DBA '%d:%ld' (rec #%ld) in "
                                                "d00 exceeds "
                                                "total num addrs %ld;\n"
                                                "  Bit vector overflow because "
                                                "maxdba %ld"
                                                " in dbrec is incorrect.\n"),
                                        PROGNAME "561", OR_D00, (long)dba,
                                        (long)dba1, (long)total_num_addrs,
        ...
        fprintf(
            aa_stderr,
            catgets(dtsearch_catd, MS_dtsrclean, 29,
                ...
                for (x = 0, bvptr = bit_vector; x < max_bitvec; x++, bvptr++) {
                        for (j = 0; j < 8; j += 4) { /* j = 0 or 4, amount of
                                ...
                                if (a & 0x10 && !(a & 0x20)) {
                                        ...
                                        /* now efim retranslates back to real
                                         * dba */
                                        dba = ((dbaorig + 1) * recslots -
                                               dba_offset) |
                                              (OR_D00 << 24);

                                        /* ...print out err msg */
                                        CRSET(PROGNAME "734", &dba, 0);
                                        CRREAD(PROGNAME "735", OR_OBJKEY,
                                               readbuf, 0);
                                        fprintf(aa_stderr,
                                                catgets(dtsearch_catd,
                                                        MS_dtsrclean, 444,
                                        ...
                                        /*...if albeniz compatible output
                                         * requested, do it */
                                        if (frecids) {
                                                fprintf(frecids, DISCARD_FORMAT,
                                                        arg_dbname, readbuf,
                                                        "MrClean", datestr);
                                        }
         ...
         */
        /***by the way, corruption_count can be > 0 only if in
         * validation_mode.**/
        if (corruption_count > 0L) {
                if (validation_mode)
                        normal_exitcode = 1;
