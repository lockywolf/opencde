...
static char *language_name(int language_number) {
        ...
        case DtSrLaDEU:
                return "GERMAN";
        case DtSrLaJPN:
                return "JAPANESE (AUTO COMPOUNDS)";
        case DtSrLaJPN2:
                return "JAPANESE (COMPOUNDS FROM LIST)";
        default:
                return "USER DEFINED LANGUAGE";
        }
}
...
void print_dbrec(char *dbname, struct or_dbrec *dbrec) {
        ...
        printf(
            catgets(dtsearch_catd, MS_dbrec, 1,
        ...
        printf(
            catgets(dtsearch_catd, MS_dbrec, 3,
        ...
        printf(
            catgets(dtsearch_catd, MS_dbrec, 12,
        ...
        if (dbrec->or_abstrsz == 0)
                ...
                if (dbrec->or_hufid != 0L)
                        printf(catgets(dtsearch_catd, MS_dbrec, 20,
                               ...
                               (ORC_COMPABSTR & dbrec->or_compflags) ? ""
                                                                     : "not ");
        }
        ...
        case ORA_VARIES:
                puts(catgets(dtsearch_catd, MS_dbrec, 50,
                             "Engine accessibility to data may vary from "
                             "object to object."));
                blobs_are_possible = TRUE;
                break;
        case ORA_NOTAVAIL:
                puts(catgets(dtsearch_catd, MS_dbrec, 54,
                             "Data objects are not directly accessible from "
                             "the engine."));
                break;
        case ORA_BLOB:
        ...
        case ORA_REFBLOB:
                puts(catgets(dtsearch_catd, MS_dbrec, 60,
                             "Only server file references to objects are "
                             "stored in the blobs."));
                break;
        case ORA_CREFBLOB:
                puts(catgets(dtsearch_catd, MS_dbrec, 64,
                             "Only client file references to objects are "
                             "stored in the blobs."));
                break;
        case ORA_REFKEY:
                puts(catgets(
                    dtsearch_catd, MS_dbrec, 68,
                    "Object keys are server file references to the objects."));
                break;
        case ORA_CREFKEY:
                puts(catgets(
                    dtsearch_catd, MS_dbrec, 72,
        ...
        case ORA_REFABSTR:
                puts(catgets(dtsearch_catd, MS_dbrec, 80,
                             "Server file references to objects are stored in "
                             "the abstracts."));
                break;
        case ORA_CREFABSTR:
                puts(catgets(dtsearch_catd, MS_dbrec, 86,
                             "Client file references to objects are stored in "
                             "the abstracts."));
                break;
        default:
                printf(catgets(dtsearch_catd, MS_dbrec, 90,
                               "Error: meaning of or_dbaccess value (%hd) is "
                               "unknown.\n"),
                       dbrec->or_dbaccess);
                blobs_are_possible = TRUE;
                break;
        ...
        if (blobs_are_possible) {
                ...
                if (dbrec->or_hufid != 0L)
                        printf(catgets(dtsearch_catd, MS_dbrec, 100,
                               ...
                               (ORC_COMPBLOB & dbrec->or_compflags) ? ""
                                                                    : "not ");
        } else
                puts(
                    catgets(dtsearch_catd, MS_dbrec, 110,
        ...
        printf(
            catgets(dtsearch_catd, MS_dbrec, 200,
...
int main(int argc, char *argv[]) {
        ...
        printf(
            ...
            aa_argv0, DtSrVERSION, renamebuf);
        ...
        if (db_status != S_OKAY) {
                printf(
                    catgets(dtsearch_catd, MS_dbrec, 350,
