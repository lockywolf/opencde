...
/*--------------- EXTERNS ------------------*/
extern volatile int shutdown_now;
...
#define UPDATE_MAXDBA(dba)                                                     \
        {                                                                      \
                if ((dba & 0xffffff) > maxdba)                                 \
                        maxdba = dba & 0xffffff;                               \
        }
...
static void user_args_processor(int argc, char **argv) {
        ...
        PRINT_USAGE:
                printf(catgets(dtsearch_catd, MS_cravel, 1,
                               ...
                               "       Listed default file name extensions can "
                               "be overridden.\n"
                               "  -d<dbname>  1 - 8 char database name, incl "
                               "optional path "
                               "prefix.\n"
                               "              File name extensions "
                               "automatically appended.\n"
                               "  -t<etxstr>  End of text doc delimiter "
                               "string.  Default "
                               "'\\f\\n'.\n"
                               "  -c          Initialize database record count "
                               "by counting "
                               "records.\n"
                               "  -p<N>       Print a progress dot every <N> "
                               "records (default "
                               "%d).\n"
                               "  -h<N>       Change duplicate rec id hash "
                               "table size from %ld "
                               "to <N>.\n"
                               "              -h0 means there are no "
                               "duplicates, don't check "
                               "for them.\n"
                               "  -e<path>    Path-filename of huffman encode "
                               "table (default "
                               "%s).\n"
                               "  <infile>    Input [path]file name.  Default "
                               "extension %s.\n"),
                       aa_argv0, RECS_PER_DOT, default_hashsize,
        ...
        for (argc--, argv++;
             ...
             argc--, argv++) {
                ...
                case 'e':
                        append_ext(fname_huffcode_tab,
                                   sizeof(fname_huffcode_tab), argptr + 2,
                                   EXT_HUFFCODE);
                        break;
        ...
        if (argc <= 0) {
                ...
                append_ext(fname_input, sizeof(fname_input), argv[0],
                           EXT_FZKEY);
...
static void read_dbrec(void) {
        ...
        if (debug_mode) {
                printf(PROGNAME
                       ...
                       "  hufid=%ld flags=x%x compflags=x%x uflags=x%lx "
                       "sec=x%lx\n",
                       (long)dbrec.or_reccount, (long)dbrec.or_maxdba,
                       dbrec.or_version, (int)dbrec.or_dbaccess,
                       (int)dbrec.or_fzkeysz, (int)dbrec.or_abstrsz,
                       (int)dbrec.or_maxwordsz, (int)dbrec.or_dbotype,
                       (int)dbrec.or_language, (long)dbrec.or_hufid,
                       (int)dbrec.or_dbflags, (int)dbrec.or_compflags,
                       (long)dbrec.or_dbuflags, (long)dbrec.or_dbsecmask);
        }
        ...
        if (!is_compatible_version(dbrec.or_version, SCHEMA_VERSION)) {
                printf(
                    catgets(dtsearch_catd, MS_misc, 14,
        ...
        case ORA_REFBLOB: /* refs to objects stored in blobs */
                ...
                if (!(dbrec.or_compflags & ORC_COMPBLOB)) {
                        ...
                        printf(catgets(dtsearch_catd, MS_cravel, 717,
                                       "%s Aborting: Uncompressed blobs not "
                                       "yet supported.\n"),
                               PROGNAME "717");
                        DtSearchExit(5);
                }
        ...
        printf(catgets(dtsearch_catd, MS_cravel, 18,
                       "%s: '%s' schema ver = %s, rec count = %ld, last slot = "
                       "%ld.\n"),
               aa_argv0, dicname, dbrec.or_version, (long)system_reccount,
               (long)maxdba);
        return;
} /* read_dbrec() */
...
static void print_progress(void) {
        ...
        TERMINATE_LINE
        printf(
            catgets(dtsearch_catd, MS_cravel, 20,
                    "%s: %ld input records processed in %ldm %lds, (%ld%%).\n"
                    "  %ld duplicates, %ld new, %ld updates.\n"),
            aa_argv0, input_reccount, seconds / 60L, seconds % 60L,
            (bytes_in * 100L) / fstat_input.st_size, duplicate_recids,
            created_reccount, updated_reccount);
...
static void print_exit_code(int exit_code) {
        ...
        printf(catgets(dtsearch_catd, MS_cravel, 2, "%s: Exit code = %d\n"),
               aa_argv0, exit_code);
        fflush(aa_stderr);
        fflush(stdout);
        return;
...
static int load_next_miscrec(int first_call) {
        ...
        if (first_call) {
                ...
                if (dbrec.or_fzkeysz > 0) {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_cravel, 522,
                                        "%s This version of %s does not "
                                        "support semantic databases.\n"),
        ...
        for (i = 0; i < sizeof(miscrec.or_misc); i++, totbytes--) {
                ...
                default:
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_misc, 25,
...
static void create_object(char *key) {
        ...
        if (debug_mode)
                printf("---> new rec: inrecno %6ld, slot %6ld, key '%s'\n",
                       (long int)input_reccount, (long int)objdba & 0xffffff,
                       objrec.or_objkey);
        ...
        if (hc_encode(&blobrec, ucharbuf, buflen, FALSE)) {
                if (debug_encode) {
                        ...
                        printf("---> WRITE sumlines = %d, bloblen = %d, "
                               ...
                               sumlines, (int)blobrec.or_bloblen, (int)sumblobs,
                               (long)objsize);
                        sumlines = 0;
                }
...
int main(int argc, char *argv[]) {
        ...
        strftime(linebuf, sizeof(linebuf),
                 ...
                 tmptr);
        printf(
            catgets(dtsearch_catd, MS_misc, 23, "%s: Version %s.  Run %s.\n"),
            aa_argv0, DtSrVERSION, linebuf);
        ...
        if ((infile = fopen(fname_input, "r")) == NULL) {
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_misc, 12,
                                "%sUnable to open %s:\n  %s\n"),
        ...
        /* Read in starting database record count and other db config/status
         * data */
        read_dbrec();
        ...
        if (dbrec.or_fzkeysz > 0) {
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_cravel, 522,
                                "%s This version of %s does not support "
                                "semantic databases.\n"),
        ...
        if (dbrec.or_abstrsz > 0)
                abstrbuf =
                    austext_malloc(dbrec.or_abstrsz + 16, PROGNAME "744", NULL);

        /* Get input file size for progress msgs */
        if (fstat(fileno(infile), &fstat_input) == -1) {
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_cravel, 29,
        ...
        if (fstat_input.st_size <= 0L) {
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_cravel, 30,
                                "%s File %s is empty.\n"),
        ...
        while (!feof(infile)) {
                ...
                INVALID_FORMAT:
                        ...
                        printf(catgets(dtsearch_catd, MS_cravel, 579,
                                       "%s Discarded rec #%ld: Invalid .fzk "
                                       "file format.\n"),
                               cptr, input_reccount);
                        if (strcmp(linebuf, parg.etxdelim) != 0)
                                discard_to_ETX(&parg);
                ...
                if (i == 2) {
                        ...
                        TERMINATE_LINE
                        printf(
                            catgets(dtsearch_catd, MS_cravel, 35,
                ...
                if (!is_objdatestr(linebuf, &objdate)) {
                        ...
                        TERMINATE_LINE
                        printf(
                            catgets(dtsearch_catd, MS_cravel, 1086,
                ...
                if (hufftab_never_loaded) {
                        ...
                        if (dbrec_hufid != gen_vec_hufid &&
                            dbrec_hufid != -1L) {
                                TERMINATE_LINE
                                printf(
                                    catgets(
                                        dtsearch_catd, MS_cravel, 1153,
                ...
                if ((linebuf[0] = readchar_ftext(&parg)) == 0) {
                        ...
                        TERMINATE_LINE
                        printf(
                            catgets(dtsearch_catd, MS_cravel, 1215,
        ...
        if (created_reccount <= 0L && updated_reccount <= 0L) {
                ...
                fprintf(stderr,
                        catgets(dtsearch_catd, MS_cravel, 1048,
