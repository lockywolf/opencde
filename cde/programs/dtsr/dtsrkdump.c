                ...
                if (do_verbose) {
                        ...
                        if (addrs >= min_threshold) {
                                ...
                                printf(catgets(dtsearch_catd, MS_dtsrkdump, 2,
                                       ...
                                       dba >> 24, dba & 0xffffff, offset, addrs,
                                       free);
                        }
                } /* end verbose */
...
int main(int argc, char *argv[]) {
        ...
        strftime(buf, sizeof(buf), "%m/%d/%Y, %I:%M %p", localtime(&now));
        printf(
            catgets(dtsearch_catd, MS_dtsrkdump, 3, "%s %s, engine %s.  %s.\n"),
        ...
        PRINT_USAGE:
                printf(
                    catgets(
                        dtsearch_catd, MS_dtsrkdump, 4,
                        "\nUSAGE: %s -o|w|ow [-v] [-t<N> | -p<N>] dbname\n"
                        "       Reads DtSearch key files and prints summary "
                        "report.\n"
                        "  -o        Keys examined are OBJECT record keys.\n"
                        "  -w        Keys examined are inverted index WORDS.\n"
                        "  -v        VERBOSE mode, lists every key.\n"
                        "  -t<N>     Threshold.  Sets w and v options, and "
                        "lists only "
                        "words\n"
                        "            with >= <N> addresses.  All words will be "
                        "listed if "
                        "<N> = 1.\n"
                        "  -p<N>     Another threshold.  Same as -t except <N> "
                        "is percent\n"
                        "            of the entire database (<N> may include a "
                        "decimal "
                        "point).\n"
                        "            For example -p99.9 prints out every word "
                        "that occurs\n"
                        "            in 99.9%% or more of the records--an "
                        "excellent way to "
                        "find\n"
                        "            candidates for the stop list.\n"
                        "            If w and v are set without threshold, "
                        "default is "
                        "-t%d.\n"
                        "  <dbname>  1 - 8 character database name with "
                        "optional path "
                        "prefix.\n"),
        ...
        if (i < 1 || i > 8) {
                fprintf(stderr,
                        catgets(dtsearch_catd, MS_dtsrkdump, 58,
        ...
        *ptr = 0; /* truncate dbname off of full path/dbname */
        ...
        printf(catgets(dtsearch_catd, MS_dtsrkdump, 60,
                       "%s: '%s' reccount=%ld maxdba=%ld recslots=%hd minw=%hd "
                       "maxw=%hd\n"),
               aa_argv0, dbname, dbrec.or_reccount, dbrec.or_maxdba,
               dbrec.or_recslots, dbrec.or_minwordsz, dbrec.or_maxwordsz);

        /* Adjust threshold if necessary */
        if (percent > 0.0)
                min_threshold =
                    (long)((float)percent * (float)dbrec.or_reccount / 100.0);
        ...
        if (do_wordkeys && do_verbose) {
                ...
                if (min_threshold > 1 && min_threshold < dbrec.or_reccount) {
                        printf(catgets(dtsearch_catd, MS_dtsrkdump, 70,
                               ...
                               aa_argv0, min_threshold);
                        listing_most_words =
                            (float)min_threshold / (float)dbrec.or_reccount >
                            .90;
                } else {
        ...
        if (do_objkeys) {
                 ...
                 */
                counters =
                    austext_malloc(258 * sizeof(long), PROGNAME "113", NULL);
                ...
                while (db_status == S_OKAY) {
                        ...
                        if (do_verbose) {
                                /* Mark control and nonascii chars with a
                                 * period.  */
                                i = 0;
                                putchar('\"');
                ...
                if (dbpath[0] == 0)
                        ...
                        sprintf(
                            buf,
                            catgets(dtsearch_catd, MS_dtsrkdump, 110, " in %s"),
                            dbpath);
        ...
        if (do_wordkeys) {
                if (listing_most_words)
                        printf(catgets(dtsearch_catd, MS_dtsrkdump, 180,
                                       "%s: * Words marked with asterisk occur "
                                       "in every record.\n"),
                               aa_argv0);
                 ...
                 */
                counters =
                    austext_malloc(8 * sizeof(long), PROGNAME "113", NULL);
