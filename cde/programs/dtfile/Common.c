...
void CvtStringToStringList(String string, String **listPtr, int *countPtr) {
        int num_items = 0;
        String *items = NULL;
        ...
        while ((ptr != NULL) && (*ptr != '\0')) {
                if (num_items >= size_item_array) {
                        ...
                        items = (String *)XtRealloc(
                            (char *)items, sizeof(String) * size_item_array);
                }
...
String CvtStringListToString(String *list, int count) {
        int i, offset;
        int stringSize = 0;
        String string = NULL;
 ...
 *
 ***********************************************************************/
void VFTextChangeSpace(Widget text, XEvent *event, XtPointer params,
        ...
        if (val == 1 && strcmp(*temp, test) != 0) {
                ...
                if (currentDirectoryText == NULL)
                        ...
                        XmTextFieldSetString(
                            text, *temp + strlen(currentDirectoryText));
                XmTextFieldSetInsertionPosition(
                    text, XmTextFieldGetLastPosition(text));
        } else if (val > 1) {
                ...
                /* find the most characters which will match in all patterns
                 * found. i.e. if I have /use, /user, /users, and /used, the
                 * most chars which match are /use
                 */
                match = True;
 ...
 *
 ***********************************************************************/
void CancelOut(Widget widget, XEvent *event, XtPointer params,
               XtPointer num_params) {
...
void AddString(String *buf, int *size, String string, String header) {
        int delta;
        int buflen = 0;
