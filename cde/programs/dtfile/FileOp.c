...
static void SendModifyMsg(int pipe_fd, int mode, Boolean first, char *to,
        ...
        if (mode == MOVE_FILE && !updates[j].time_sent) {
                ...
                dir_path =
                    ResolveLocalPathName(updates[i].host, updates[i].directory,
...
static int FileMoveCopyProcess(int pipe_s2m, int pipe_m2s, char *to_file,
        ...
        for (i = 0; i < file_count; i++) {
                ...
                if (mode != MAKE_BUFFER) {
                        from = ResolveLocalPathName(
                            updates[i].host, updates[i].directory,
                        ...
                        if (from == NULL) {
                                ...
                                sprintf(
                                    msg1,
                                    GetSharedMessage(CANNOT_CONNECT_ERROR_1),
                                    updates[i].directory, updates[i].host,
                                    updates[i].host);
#else
                                sprintf(
                                    msg1,
                                    GetSharedMessage(CANNOT_CONNECT_ERROR_2),
                                    updates[i].directory, updates[i].host,
                                    updates[i].host);
#endif
                                PipeWriteErrmsg(pipe_s2m, -1, msg1, NULL);
                        ...
                        /* do some extra error checking if a target filename is
                         * specified */
                        /*   this is meant for the case of a menu-initiated
                         * operation     */
                        if (to_file != NULL) {
                                /* Verify that the target directory already
                                 * exists */
                                if ((stat(target_dir, &stat_buf) != 0) ||
                                    (!S_ISDIR(stat_buf.st_mode))) {
                                        ...
                                        msg = XtNewString(GETMESSAGE(
                                            11, 41,
                                            "The folder\n%s\ndoes not exist."));
                                        PipeWriteErrmsg(pipe_s2m, -1, msg,
                                                        target_dir);
                                        errorCount++;
                                        XtFree(msg);
                                        XtFree(from);
                        ...
                        if (updates[i].app_man_dir) {
                                ...
                                temp_dir = _DtResolveAppManPath(
                                    temp_dir, updates[i].app_man_dir);
                                if (mode == MOVE_FILE &&
                                    strcmp(temp_dir, from) == 0) {
                                        char *msg;

                                        msg = XtNewString(GETMESSAGE(
                                            11, 16,
                                            "A folder cannot be moved into "
                                            "itself.\n%s"));
                                        PipeWriteErrmsg(pipe_s2m, -1, msg,
                                                        temp_dir);
                                        errorCount++;
                                ...
                                XtFree(temp_dir);
                                   ...
                                   strcmp(target_dir, from) == 0) {
                                char *msg;

                                msg = XtNewString(GETMESSAGE(11, 16,
                                                             "A folder cannot "
                                                             "be moved into "
                                                             "itself.\n%s"));
                                ...
                                msg = XtNewString(
                                    GETMESSAGE(11, 30,
                                               "Cannot move or rename the "
                                               "folder %s.\nAll File Manager "
                                               "views displayed for a folder "
                                               "or its sub-folders\nmust "
                                               "be closed before a folder can "
                                               "be moved or renamed."));
                ...
                /* for copy operations, check for read permission on the source
                 * file */
                ...
                if (mode == COPY_FILE)
                        if (CheckAccess(from, R_OK) == -1) {
                                ...
                                /* 'errno' is not giving correct results like
                                   'ENOENT' in order to use it here, may be
                                   because of the other system calls in
                                    CheckAccess() */
                                ...
                                if (stat(from, &sbuf) < 0)
                                        ...
                                        tmpStr =
                                            GetSharedMessage(CANT_READ_ERROR);

                                permissionErrors = appendErrorMessage(
                                    permissionErrors, tmpStr, from);
                                errorCount++;
                                continue;
                        }
                ...
                if (to_file == NULL) {
                        if (strcmp(target_dir, "/") == 0) {
                                target_file = (char *)XtMalloc(
                                    strlen(target_dir) +
                                    strlen(updates[i].file) + 1);
                                sprintf(target_file, "%s%s", target_dir,
                                        updates[i].file);
                        } else {
                                target_file = (char *)XtMalloc(
                                    strlen(target_dir) +
                        ...
                        if (strcmp(target_dir, "/") == 0) {
                                target_file = (char *)XtMalloc(
                                    strlen(target_dir) + strlen(to_file) + 1);
                                sprintf(target_file, "%s%s", target_dir,
                                        to_file);
                        } else {
                                target_file = (char *)XtMalloc(
                                    strlen(target_dir) + strlen(to_file) + 2);
                                sprintf(target_file, "%s/%s", target_dir,
                                        to_file);
                        }
                }
                if (updates[i].app_man_dir)
                        target_file = _DtResolveAppManPath(
                            target_file, updates[i].app_man_dir);
                /* check if source and target files are the same.
                 * if they are the same, set a flag noting that they are.  If
                 * the the operation is a move, it doesn't make any sense to
                 * move it, so send back a MOVE_TO_SAME_DIR msg.
                 */

                if ((mode != MAKE_BUFFER) && (strcmp(target_file, from) == 0)) {
                        if (mode == MOVE_FILE &&
                            stat(target_file, &stat_buf) == 0) {
                                if (S_ISDIR(stat_buf.st_mode)) {
                                        pipe_msg = PIPEMSG_MOVE_TO_SAME_DIR;
                                        write(pipe_s2m, &pipe_msg,
                                              sizeof(short));
                 ...
                 */
                /*    directory */
                /* for move operations, check for write permission on the source
                 * and */
                /*    target directories */
                /* the error message is appended to a list and dealt with later
                 */
                ...
                if (mode == COPY_FILE) {
                        ...
                        Boolean error = FALSE;
                        if (targetChecked ||
                            CheckAccess(target_dir, W_OK) == -1) {
                                MoveError1 = TRUE;
                                if (MoveString1 == NULL)
                                        MoveString1 = XtNewString(from);
                                else {
                                        MoveString1 = (char *)XtRealloc(
                                            MoveString1, strlen(MoveString1) +
                                                             strlen(from) + 2);
                                        strcat(MoveString1, "\n");
                        ...
                        if (CheckAccess(_DtPName(from), W_OK) == -1) {
                                ...
                                if (MoveString2 == NULL)
                                        ...
                                        MoveString2 = (char *)XtRealloc(
                                            MoveString2, strlen(MoveString2) +
                                                             strlen(from) + 2);
                                        strcat(MoveString2, "\n");
                ...
                if (stat(target_file, &stat_buf) == 0) {
                        /* target file already exists: remember and deal with it
                         * later */
                        ...
                        if (sameIndex == NULL) {
                                sameIndex =
                                    (int *)XtMalloc(file_count * sizeof(int));
                                sameConfirmType =
                                    (int *)XtMalloc(file_count * sizeof(int));
                        }
                        ...
                        if (mode == MAKE_BUFFER) {
                                sameConfirmType[sameCount] =
                                    PIPEMSG_REPLACE_RENAME;
                        } else {
                                stat(from, &from_stat_buf);
                                if (S_ISDIR(from_stat_buf.st_mode) &&
                                    S_ISDIR(stat_buf.st_mode) &&
                                    mode == COPY_FILE) {
                                        /* if its a directory and there already
                                         * is a directory of the same name the
                                         * user may want to merge the
                                         * directories into one directory.  But
                                         * if the directory to be copied is
                                         * being copied into the same directory
                                         * it came from, it doesn't make sense
                                         * to merge.  Set up message to
                                         * REPLACE_RENAME_SAME indicating it is
                                         * being copied from and to the same
                                         * directory
                                         */
                                        if (Same)
                                                sameConfirmType[sameCount] =
                                                    PIPEMSG_REPLACE_RENAME_SAME;
                                        else
                                                sameConfirmType[sameCount] =
                                                    PIPEMSG_REPLACE_MERGE;
                                } else {
                                        /* If the copy/move/link is to the same
                                         * directory, set up a different case
                                         * then when the op is happening from
                ...
                if (mode == MAKE_BUFFER) {
                        ...
                        return_val =
                            CreateFileFromBuffer(pipe_s2m, to, target_file,
                                                 updates[i].bufferInfo.buf_ptr,
                                                 updates[i].bufferInfo.size);
                } else {
                        if (strncmp(directory, desktop_dir,
                                    strlen(desktop_dir)) == 0)
                                return_val = FileManip(
                                    (Widget)(intptr_t)pipe_s2m, mode, from, to,
                                    isContainer, FileOpError, True, DESKTOP);
                        else
                                return_val =
                                    FileManip((Widget)(intptr_t)pipe_s2m, mode,
                                              from, to, isContainer,
        ...
        if (sameCount != 0) {
                ...
                /*
                 * @@@ Note: The code below for sending a target-time pipe
                 * message at this point shouldn't really be necessary. The
                 * problem is that the target directory time stamp MUST be
                 * obtained BEFORE any changes are made to the directory;
                 * otherwise, the window will be updated and redrawn twice
                 * instead of just once.  Unfortunately, currently, if the user
                 * chooses to replace or rename the existing file, the rename or
                 * delete operation is done in the main process (inside the
                 * replace_rename_ok_callback in Overwrite.c), instead of here
                 * in the child process where it belongs (the main process
                 * shouldn't do any file system operations because they could
                 * block for a long time on a slow server).  If and when
                 * overwrite dialog code is fixed, the code below can be
                 * deleted; the target-time message will then be sent by the
                 * call to SendModifyMsg further below after the call to
                 * PipeRead.
                 */
                if (first_op) {
                        if (stat(target_dir, &stat_buf) == 0) {
                 ...
                 * remove/rename files) */
                if (file_count == 1) {
                        if (to_file == NULL) /* note target_file here is only
                                                file name, above it is full path
                                              */
                        ...
                        DPRINTF(("FileMoveCopyProcess: sending msg %d\n",
                                 pipe_msg));
                        write(pipe_s2m, &pipe_msg, sizeof(short));
                        DPRINTF(("FileMoveCopyProcess: sending: mode %d "
                                 "directory \"%s\" file "
                                 "\"%s\" \n",
                        ...
                        /* If the copy/move/link is to the same directory, set
                         * up a different case then when the op is happening
                        ...
                        PipeWriteString(pipe_s2m, directory);
                        DPRINTF(("FileMoveCopyProcess: sending %d filename "
                                 "strings\n",
                                 sameCount));
                        for (i = 0; i < sameCount; i++)
                                PipeWriteString(pipe_s2m,
                                                updates[sameIndex[i]].file);
                }
                ...
                if (rc != PIPEMSG_CANCEL) {
                        ...
                        for (i = 0; i < sameCount; i++) {
                                ...
                                if (rc != PIPEMSG_RENAME_BUFFER) {
                                        SendModifyMsg(pipe_s2m, mode, first_op,
                                                      target_dir, updates, j,
                                                      updates[j].file);
                                        first_op = FALSE;
                                }
                                if (rc == PIPEMSG_MULTI_PROCEED) {
                                        int opvalue;
                                        PipeRead(pipe_m2s, &opvalue,
                                                 sizeof(int));
                                        if (opvalue != 0)
                                                continue;
                                }
                                ...
                                if (mode != MAKE_BUFFER) {
                                        ...
                                        if (TT_OK != tt_status) {
                                                ...
                                                sprintf(
                                                    msg1,
                                                    GetSharedMessage(
                                                        CANNOT_CONNECT_ERROR_2),
                                                    updates[j].directory,
                                                    updates[j].host,
                                                    updates[j].host);
#endif
                                                PipeWriteErrmsg(pipe_s2m, -1,
                                                                msg1, NULL);
                                                continue;
                                        }
                                } /* endif */
                                ...
                                case PIPEMSG_MERGE:
                                        ...
                                        if (strncmp(directory, desktop_dir,
                                                    strlen(desktop_dir)) == 0)
                                                return_val = FileManip(
                                                    (Widget)(intptr_t)pipe_s2m,
                                                    MERGE_DIR, from, to,
                                                    isContainer, FileOpError,
                                                    True, DESKTOP);
                                        else
                                                return_val = FileManip(
                                                    (Widget)(intptr_t)pipe_s2m,
                                ...
                                case PIPEMSG_REPLACE_BUFFER:
                                        target_file = (char *)XtMalloc(
                                            strlen(to) +
                                            strlen(updates[j].file) + 2);
                                        sprintf(target_file, "%s/%s", to,
                                                updates[j].file);
                                        DPRINTF(
                                            ("file is %s", updates[j].file));
                                        return_val = CreateFileFromBuffer(
                                            pipe_s2m, to, target_file,
                                ...
                                default:
                                        if (strncmp(directory, desktop_dir,
                                                ...
                                                return_val = FileManip(
                                                    (Widget)(intptr_t)pipe_s2m,
                                                    mode, from, to, isContainer,
                                                    FileOpError, True, DESKTOP);
                                        else {
                                                /* if the operation
                                                 * (move/copy/link) is happening
                                                 * from and to the same folder,
                                                 * we want to create a new name
                                                 * for the object for which the
                                                 * operation is happening on.
                                                 */
                                                if (Same) {
                                                        char path[MAX_PATH],
                                                            newDir[MAX_PATH],
                                                        ...
                                                        strcpy(path, from);
                                                        generate_NewPath(path,
                                                                         path);
                                                        split_path(path, newDir,
                                                                   newFile);
                                                        toFile =
                                                            (char *)XtMalloc(
                                                                strlen(newDir) +
...
static int FileMoveCopyProcessDesktop(int pipe_s2m, int pipe_m2s,
        ...
        /*  Get the fully qualified destination path.  */
        to = ResolveLocalPathName(host, directory, NULL, home_host_name,
                                  &tt_status);
        if (TT_OK != tt_status) {
                char msg1[2 * MAX_PATH];
#ifdef sun
        ...
        for (i = 0; i < file_count; i++) {
                ...
                SendModifyMsg(pipe_s2m, mode, first_op, to, updates, i,
                              updates[i].file);
                ...
                if (mode == MAKE_BUFFER) {
                        ...
                        if (strcmp(to, "/") == 0) {
                                target_file = (char *)XtMalloc(
                                    strlen(to) + strlen(updates[i].file) + 1);
                                sprintf(target_file, "%s%s", to,
                                        updates[i].file);
                        } else {
                                target_file = (char *)XtMalloc(
                                    strlen(to) + strlen(updates[i].file) + 2);
                                sprintf(target_file, "%s/%s", to,
                                        updates[i].file);
                        }
                        ...
                        return_val =
                            FileManip((Widget)(intptr_t)pipe_s2m, mode, from,
                                      to, TRUE, FileOpError, True, DESKTOP);
                        XtFree(from);
                        from = NULL;
                }
        }
...
static void RemoveIconFromWorkspace(char *iconName, char *targetDir) {
        ...
        for (i = 0; i < desktop_data->numIconsUsed; i++) {
                ...
                if (strcmp(fileName, iconName) == 0) {
                        ...
                        currentScreen = XScreenOfDisplay(
                            XtDisplay(desktopWin->shell), screen);
                        rootWindow = RootWindowOfScreen(currentScreen);

                        if (DtWsmGetCurrentWorkspace(
                                XtDisplay(desktopWin->shell), rootWindow,
                                &pCurrent) == Success)
                                workspace_name = XGetAtomName(
                                    XtDisplay(desktopWin->shell), pCurrent);
                        else
                                workspace_name = XtNewString("One");

                        if (strcmp(workspace_name,
                                   desktopWin->workspace_name) == 0) {
                                RemoveDT(desktopWin->shell,
                                         (XtPointer)desktopWin,
                                         (XtPointer)NULL);
                        } else {
                                XtFree(desktopWin->dir_linked_to);
                                desktopWin->dir_linked_to =
                                    XtNewString(targetDir);
                        }
...
static void ChangeWorkspaceIconLink(FileMgrData *fmd, char *iconName,
        ...
        for (i = 0; i < desktop_data->numIconsUsed; i++) {
                ...
                if (strcmp(fileName, iconName) == 0)
                        ...
                        dirp = XtCalloc(1, strlen(targetDir) +
                                               strlen(&tptr[index]) + 1);
                        ...
                        if (fmd) {
                                if (fmd->restricted_directory == NULL)
                                        ...
                                        desktopWin->restricted_directory =
                                            XtNewString(
                                                fmd->restricted_directory);

                                if (fmd->helpVol == NULL)
                                        desktopWin->helpVol = NULL;
                                else
                                        desktopWin->helpVol =
                                            XtNewString(fmd->helpVol);
                                ...
                                desktopWin->positionEnabled =
                                    fmd->positionEnabled;
                                desktopWin->toolbox = fmd->toolbox;
                        }

                        full_dir_name =
                            ResolveLocalPathName(desktopWin->host, dirp, NULL,
                                                 home_host_name, &tt_status);
                        if (TT_OK == tt_status) {
                                ...
                                if (strcmp(desktopWin->file_name, ".") == 0)
                                        ReadFileData2(&file_data2,
                                                      full_dir_name, NULL,
                                                      IsToolBox);
                                else
                                        ReadFileData2(
                                            &file_data2, full_dir_name,
                                            desktopWin->file_name, IsToolBox);

                                file_view_data->file_data =
                                    FileData2toFileData(&file_data2, &n);
...
static void FileOpPipeCB(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        case PIPEMSG_FILEOP_ERROR:
                ...
                if (cb_data->desktop)
                        ...
                        /* routine can be called with a NULL file_mgr_rec, use
                         * the top level widget if this is the case.
                         */
                        if (cb_data->file_mgr_rec)
                                FileOperationError(
                                    cb_data->file_mgr_rec->file_window, err_msg,
                                    err_arg);
                        else
                ...
                if (cb_data->callback_data) {
                        RenameDoneData *rdd =
                            (RenameDoneData *)cb_data->callback_data;
                        ResetFlag(rdd->call_struct->dialog_widget, rdd->w);
                        ResetFlag(rdd->call_struct->dialog_widget,
                                  rdd->call_struct->Cancel);
                }
        ...
        case PIPEMSG_CONFIRM:
                err_msg = PipeReadString(*fd);
                title = XtNewString(
                    GETMESSAGE(9, 11, "File Manager - Move/Copy/Link Warning"));
        ...
        case PIPEMSG_REPLACE_RENAME_SAME:
                 ...
                 * to indicate the to the ok dialog and the building of the
                 * replace name dialog.
                 */
                replace_rename_actionItems[0].data = (XtPointer)True;
                if (cb_data->file_mgr_rec)
                        create_replace_rename_dialog(
                            cb_data->file_mgr_rec->shell, mode, directory, file,
                            cb_data->pipe_m2s, replace_renameActions, True);
                else
                        create_replace_rename_dialog(
                            toplevel, mode, directory, file, cb_data->pipe_m2s,
        ...
        case PIPEMSG_REPLACE_RENAME:
                ...
                if (cb_data->file_mgr_rec)
                        create_replace_rename_dialog(
                            cb_data->file_mgr_rec->shell, mode, directory, file,
                            cb_data->pipe_m2s, replace_renameActions, False);
                else
                        create_replace_rename_dialog(
                            toplevel, mode, directory, file, cb_data->pipe_m2s,
        ...
        case PIPEMSG_REPLACE_MERGE:
                ...
                if (cb_data->file_mgr_rec)
                        ...
                        create_replace_merge_dialog(toplevel, mode, directory,
        ...
        case PIPEMSG_MULTICOLLIDE_SAME:
                ...
                fileList = (String *)XtMalloc(
                    nCollisions * sizeof(String)); /* de- allocated in
                                                      dialog's callback
                                                      functions */
                ...
                if (cb_data->file_mgr_rec)
                        create_multicollide_dialog(
                            cb_data->file_mgr_rec->shell, mode, nSelected,
                            nCollisions, directory, fileList, cb_data->pipe_m2s,
                            multicollideActions, True);
                else
                        create_multicollide_dialog(toplevel, mode, nSelected,
                                                   nCollisions, directory,
                                                   fileList, cb_data->pipe_m2s,
                                                   multicollideActions, True);
                XtFree(directory);
                break;
        ...
        case PIPEMSG_MULTICOLLIDE:
                ...
                file = XtMalloc(MAX_PATH);
                fileList = (String *)XtMalloc(
                    nCollisions * sizeof(String)); /* de-allocated in dialog's
                                                      callback functions */
                for (i = 0; i < nCollisions; i++) {
                        fileList[i] = PipeReadString(*fd);
                }
        ...
        case PIPEMSG_TARGET_TIME:
                ...
                DirectoryModifyTime(cb_data->host, cb_data->directory,
                                    modify_time);
                break;
        ...
        default:
                fprintf(stderr,
                        "Internal error in FileOpPipeCB: bad pipe_msg %d\n",
                        pipe_msg);
                rc = -1;
                done = True;
        }
        ...
        if (done) {
                ...
                if (!cb_data->finish_callback && cb_data->file_mgr_data) {
                        char **file_set;
                        int actual_count = 0;
                        /* Do this only if it is the current directory and
                           Random placement is ON */
                        if (cb_data->file_mgr_data->positionEnabled !=
                                RANDOM_OFF &&
                            strcmp(cb_data->directory,
                                   cb_data->file_mgr_data->current_directory) ==
                                0) {
                                file_set = (char **)XtCalloc(
                                    1, cb_data->file_count * sizeof(char *));
                                for (i = 0; i < cb_data->file_count; i++) {
                                        if (cb_data->updates[i]
                                                .operationStatus == True)
                                                file_set[actual_count++] =
                                                    cb_data->updates[i].file;
                                }
                                RepositionIcons(cb_data->file_mgr_data,
                                                file_set, actual_count, G_dropx,
                                                G_dropy, True);
                ...
                for (i = 0; i < cb_data->file_count; i++) {
                        ...
                        /* Scroll the window to show the created object, since
                           we cannot keep scrolling for each object we just do
                           it for one object and we do it in case of Drag/Drop,
                           but not for Select.MoveTo/CopyTo ... */
                        ...
                        if (!i && cb_data->callback_data == NULL) {
                                ...
                                if (cb_data && cb_data->file_mgr_data) {
                                        ...
                                        fmd->scrollToThisDirectory =
                                            XtNewString(cb_data->directory);
                                        fmd->scrollToThisFile = XtNewString(
                                            cb_data->updates[i].file);
                                }
                        }
                        ...
                        if (cb_data->mode == MOVE_FILE) {
                                sprintf(fileName, "%s/%s", tmpDir,
                                        cb_data->updates[i].file);
                                if (cb_data->updates[i].operationStatus ==
                                    True) {
                                        if (status ==
                                            PIPEMSG_MOVE_TO_SAME_DIR) {
                                                RemoveIconFromWorkspace(
                                                    fileName,
                                                    cb_data->directory);
                                        } else if (status !=
                                                   PIPEMSG_FILEOP_ERROR) {
                                                ChangeWorkspaceIconLink(
                                                    cb_data->file_mgr_data,
                                                    fileName,
                                                    cb_data->directory, tmpDir);

                                                /* If it is workspace drag and
                                                   drop and the move operation
                                                   is not because of
                                                   Select.MoveTo menu option  */
...
static Boolean _FileMoveCopy(XtPointer data, char *to_file, char *directory,
                ...
                for (i = 0; i < file_count; i++) {
                        ...
                        for (j = 0; j < i; j++)
                                if (strcmp(updates[j].host, host_set[i]) == 0 &&
                                    strcmp(updates[j].directory, source_dir) ==
                                        0)
                                        break;
                        ...
                        if (j < i) { /* already in the list */
                                ...
                                /* mark the directory as being modified in the
                                 * directory cache */
                                if (mode == MOVE_FILE)
        ...
        if (pid == -1) {
                ...
                for (i = 0; i < file_count; i++)
                        if (mode == MOVE_FILE && updates[i].first_index == i)
                                DirectoryAbortModify(updates[i].host,
                                                     updates[i].directory);
        ...
        if (pid == 0) {
                DBGFORK(("%s:  child forked, m2s %d s2m %d\n", pname,
                         pipe_m2s[0], pipe_s2m[1]));

                close(pipe_m2s[1]); /* won't write to pipe_m2s */
                close(pipe_s2m[0]); /* won't read from pipe_s2m */

                if (desktopWindow != NULL)
                        rc = FileMoveCopyProcessDesktop(
                            pipe_s2m[1], pipe_m2s[0], directory, host, updates,
                            file_count, mode, desktopWindow);
                else
                        rc = FileMoveCopyProcess(
                            pipe_s2m[1], pipe_m2s[0], to_file, directory, host,
                            updates, file_count, mode, NULL);
        ...
        /*
         * We wait a certain amount of time for the background process to
         * finish. If it doesn't finish within that time, we do the rest
         * asynchronously.
         */
        ...
        for (;;) {
                ...
                if ((int)select_timeout.tv_sec < 0) {
                        ...
                        DPRINTF(("FileMoveCopy: timed out; adding input "
                                 "callback\n"));
                ...
                rc = select(fd + 1, (int *)&select_fds, NULL, NULL,
                            &select_timeout);
#else
                ...
                if (rc < 0 && errno != EINTR) {
                        ...
                        /* call FileOpPipeCB to read & process the data from the
                         * pipe */
                        FileOpPipeCB((XtPointer)cb_data, &fd, NULL);
                        DPRINTF(
                            ("FileMoveCopy: FileOpPipeCB -> fd = %d\n", fd));

                        /*
                         * If the background process is done, FileOpPipeCB sets
                         * fd to zero (in case of success) or -1 (in case of
                         * failure).
                         */
                        if (fd <= 0) {
                                operation_done = (fd == 0);
...
Boolean FileMoveCopy(FileMgrData *file_mgr_data, char *to_file, char *directory,
                     ...
                     void (*finish_callback)(), XtPointer callback_data) {
        return _FileMoveCopy((XtPointer)file_mgr_data, to_file, directory, host,
                             host_set, file_set, NULL, file_count, modifiers,
                             NULL, finish_callback, callback_data);
}
...
Boolean FileMoveCopyDesktop(FileViewData *file_view_data, char *directory,
        ...
        return _FileMoveCopy((XtPointer)file_view_data, NULL, directory,
                             home_host_name, host_set, file_set, NULL,
                             file_count, modifiers, desktopWindow,
...
static int ChangeIconNameProcess(int pipe_fd, char *host_name,
        ...
        /* rename the file */
        old_full_name = ResolveLocalPathName(
            host_name, directory_name, old_name, home_host_name, &tt_status);
        if (TT_OK != tt_status) {
                DPRINTF(("ChangeIconNameProcess: sending exist error\n"));
                pipe_msg = PIPEMSG_FILEOP_ERROR;
                write(pipe_fd, &pipe_msg, sizeof(short));
...
static void ChangeIconPipeCB(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        if (pipe_msg == PIPEMSG_FILE_MODIFIED) {
                 ...
                 */
                if (modify_time != 0)
                        DirectoryModifyTime(cb_data->host_name,
                                            cb_data->directory_name,
                                            modify_time);
                DirectoryFileModified(cb_data->host_name,
                                      cb_data->directory_name,
                                      cb_data->old_name);
                DirectoryFileModified(cb_data->host_name,
                                      cb_data->directory_name,
        ...
        if (pipe_msg == PIPEMSG_EXIST_ERROR) {
                ...
                if (cb_data->desktop) {
                        ...
                        err_msg = XtNewString(GETMESSAGE(
                            9, 9,
                ...
                if (rc == 0) {
                        ...
                        if (cb_data->desktop) {
                                XmProcessTraversal(desktopWindow->iconGadget,
                                                   XmTRAVERSE_CURRENT);
                                desktopWindow->text = NULL;
                                /* we'll catch this icon label in the loop after
                                 * the else */
                        } else {
                                file_mgr_data->renaming = NULL;

                                label =
                                    XmStringCreateLocalized(cb_data->new_name);
                                XtSetArg(args[0], XmNstring, label);
                                XtSetValues(file_view_data->widget, args, 1);
                                XmStringFree(label);
                                XmProcessTraversal(file_view_data->widget,
                                                   XmTRAVERSE_CURRENT);
                                XmUpdateDisplay(file_mgr_rec->file_window);

                                /*
                                 * To prevent the positional data from becoming
                                 * disassociated with this file, we need to
                                 * change the name in the positional data
                                 * structure also.
                                 */
                        ...
                        /*
                         * Check all desktop windows to see if they were linked
                         * to the file we just renamed.  If so, we need to
                         * change the link in .dt/Desktop that points to it.
                         *
                         * Note: We could do this in a background process, but
                         * we assume that .dt/Desktop is local or, if it's
                         * remote and the remote server is down, the user is
                         * screwed anyway.  So we assume it's ok to do blocking
                         * operations on .dt/Desktop in the main process. Hence
                         * we go for the simpler solution here.
                         */
                        desktop_changed = False;
                        for (i = 0; i < desktop_data->numIconsUsed; i++) {
                                desktopWindow = desktop_data->desktopWindows[i];

                                if (strcmp(cb_data->host_name,
                                           desktopWindow->host) == 0 &&
                                    strcmp(cb_data->directory_name,
                                           desktopWindow->dir_linked_to) == 0 &&
                                    strcmp(cb_data->old_name,
                                           desktopWindow->file_name) == 0) {
                                        XmString label;

                                        /* Force the icon label to be updated
                                         * immediately */
                                        label = XmStringCreateLocalized(
                                            cb_data->new_name);
                                        XtSetArg(args[0], XmNstring, label);
                                        XtSetValues(desktopWindow->iconGadget,
                                                    args, 1);
                                        XmStringFree(label);
                                        ...
                                        XtFree(desktopWindow->file_view_data
                                                   ->file_data->file_name);
                                        desktopWindow->file_view_data->file_data
                                            ->file_name =
                                            XtNewString(cb_data->new_name);
                                        if (strcmp(desktopWindow->title,
                                                   cb_data->old_name) == 0) {
                ...
                fprintf(stderr,
                        "Internal error in ChangeIconPipeCB: bad pipe_msg %d\n",
                        pipe_msg);
...
static void _ChangeIconName(Widget w, Boolean desktop,
        ...
        /* Ensure the new name has length less than or equal to the maximum
           ...
           If maxFileNameLength == -1 the file system is not supporting POSIX,
           use MAXNAMLEN
        */
        ...
        if (desktopWindow == NULL) {
                for (i = 0; i < desktop_data->numIconsUsed; i++) {
                        if (strcmp(host_name,
                                   desktop_data->desktopWindows[i]->host) ==
                                0 &&
        ...
        if (desktopWindow != NULL) {
                ...
                for (j = 0; j < desktop_data->numIconsUsed; j++) {
                        if (strcmp(new_name, desktop_data->desktopWindows[j]
                                                 ...
                                                 ->file_name) == 0) {
                                title = XtNewString(
                                    GetSharedMessage(FILE_RENAME_ERROR_TITLE));
                                if (desktop)
                                        tmpStr = GETMESSAGE(
                                            28, 9,
                                            "An object with this name already "
                                            "exists on the "
                                            "Workspace.\nPlease choose a "
                                            "different name.");
                                else
                                        tmpStr = GETMESSAGE(
                                            ...
                                            "Workspace back drop.\nAnother "
                                            "object on the "
                                            "back drop already has the name "
                                            "you\nare trying "
                                            "to enter.\nPlease choose a "
                                            "different name.");
                                msg = XtNewString(tmpStr);
                                _DtMessage(XtParent(w), title, msg, NULL,
                                           HelpRequestCB);
                                XtFree(title);
                                XtFree(msg);
                                XtFree(new_name);
        ...
        if (pid == 0) {
                ...
                rc = ChangeIconNameProcess(pipe_fd[1], host_name,
                                           directory_name, old_name, new_name);
                close(pipe_fd[1]);
...
void ChangeIconName(Widget w, XtPointer client_data,
        ...
        if (call_data->reason == XmCR_MODIFYING_TEXT_VALUE ||
            call_data->reason == XmCR_MOVING_INSERT_CURSOR) {
                int SWwidth;
                /* x1     - x value of the text widget with respect to scroll
                   window x2,y2  - x,y values of the cursor position (new) w.r.t
                   text widget x3,y3  - x,y values of the cursor position
                   (previous) w.r.t textW
                */
                ...
                if ((Dimension)(call_data->newInsert) < stringWidth) {
                        if (XtIsManaged(file_mgr_rec->horizontal_scroll_bar)) {
                                XmScrollBarGetValues(
                                    file_mgr_rec->horizontal_scroll_bar, &value,
                                    &size, &increment, &page);
                                /*
                                  printf("\n value = %d",value);
                                */
                                ...
                                /* case where cursor is moved in reverse
                                   direction */
                                else if ((x2 - x3) < 0) {
                                        int min = 0;
...
static int MakeFileProcess(int pipe_fd, char *to_host, char *to_dir,
        ...
        /* now create the file/dir */
                ...
                /* send a modified message back through the pipe */
                modify_time = 0;
                dir_path = ResolveLocalPathName(to_host, to_dir, NULL,
                                                home_host_name, &tt_status);
                if (stat(dir_path, &stat_buf) == 0)
                        modify_time = stat_buf.st_mtime;
                XtFree(dir_path);
...
static void MakeFilePipeCB(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        if (pipe_msg == PIPEMSG_FILE_MODIFIED) {
                ...
                if (modify_time != 0)
                        DirectoryModifyTime(cb_data->to_host, cb_data->to_dir,
                                            modify_time);
        ...
        if (pipe_msg == PIPEMSG_DONE) {
                ...
                fprintf(stderr,
                        "Internal error in MakeFilePipeCB: bad pipe_msg %d\n",
                        pipe_msg);
                rc = -1;
                full_name = NULL;
        }
        ...
        DPRINTF(
            ("MakeFilePipeCB: n %d, pipe_msg %d, rc %d\n", n, pipe_msg, rc));
        ...
        */
        {
                MakeFileDoneData *data =
                    (MakeFileDoneData *)cb_data->callback_data;
        ...
        /* call the callback routine */
        if (cb_data->finish_callback)
                (*cb_data->finish_callback)(cb_data->callback_data, full_name,
                                            rc);
...
void MakeFile(Widget w, char *host_name, char *directory_name, char *new_name,
              unsigned char type, void (*finish_callback)(),
              XtPointer callback_data) {
        ...
        GetTarget(host_name, directory_name, new_name, to_host, to_dir,
                  to_file);
        ...
        if (pid == 0) {
                ...
                rc =
                    MakeFileProcess(pipe_fd[1], to_host, to_dir, to_file, type);
                close(pipe_fd[1]);
        ...
        XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_fd[0],
                      (XtPointer)XtInputReadMask, MakeFilePipeCB,
                      (XtPointer)cb_data);
}
...
Boolean MakeFilesFromBuffers(FileMgrData *file_mgr_data, char *directory,
        ...
        return _FileMoveCopy((XtPointer)file_mgr_data, NULL, directory, host,
                             host_set, file_set, buffer_set, num_of_buffers, 0,
                             NULL, finish_callback, callback_data);
}
...
static Boolean CreateFileFromBuffer(int pipe_s2m, char *directory,
        ...
        if ((fnew = open(fully_qualified_name, O_CREAT | O_WRONLY, mode)) < 0) {
                ...
                if ((rc = write(fnew, buffer, size)) < 0) {
                        DPRINTF(("CreateBufferFromFile: Could not write buffer "
                                 "to %s\n",
                                 fully_qualified_name));
                        savedError = errno;
                } else {
                        close(fnew);
                        DPRINTF(
                            ("CreateBuffeFromFile: Target file %s created\n",
                             fully_qualified_name));
                }
        }
...

{
        ...
        if (cb_data->mode == MOVE_FILE) {
                if (cb_data->callback_data == NULL)
                        if (initiating_view == NULL)
                                ...
                                title = XtCalloc(
                                    1, strlen(GETMESSAGE(9, 73, "Move")) +
                                           strlen(GETMESSAGE(9, 94, "Error")) +
                                           5);
                                tchar = XtNewString(GETMESSAGE(9, 73, "Move"));
                                sprintf(title, "%s %s", tchar,
                                        GETMESSAGE(9, 94, "Error"));
                                XtFree(tchar);
                                err_msg = GETMESSAGE(
                                    11, 135,
                ...
                XtFree(tchar);
                if (cb_data->callback_data == NULL) {
                        if (cb_data->file_mgr_data &&
                            cb_data->file_mgr_data->toolbox) {
                                XtFree(title);
                                return;
                        } else
                                err_msg = GETMESSAGE(
                                    11, 136,
                                    "Cannot copy object %s onto itself");
                } else {
                        err_msg = GETMESSAGE(
                            11, 45,
                ...
                if (cb_data->callback_data == NULL) {
                        err_msg = GETMESSAGE(
                            11, 137, "Cannot link object %s onto itself");
                } else {
                        err_msg = GETMESSAGE(
                            11, 46,
        ...
        msgptr = XtCalloc(1, strlen(err_msg) +
                                 strlen(cb_data->updates[index].file) + 10);
        ...
        if (message == NULL) {
                ...
                *message = '\0';
                ...
                message = XtRealloc(message,
                                    (strlen(message) + strlen(newMessage) + 2));
