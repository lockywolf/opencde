        ...
        if (da_cbs->reason == XmCR_EXPOSE &&
            file_mgr_data->show_type == MULTIPLE_DIRECTORY) {
                ...
                if (event && event->type == Expose)
                        RedrawTreeLines(w, event->x, event->y, event->width,
                                        event->height, event->count,
                                        file_mgr_rec, file_mgr_data);
                else
                        RedrawTreeLines(w, 0, 0, 999999, 999999, 0,
                                        file_mgr_rec, file_mgr_data);
        }
}
...
void FileWindowResizeCallback(Widget w, XtPointer client_data, XEvent *event) {
        ...
        if (event->type == ConfigureNotify) {
                ...
                /*
                 * If the incoming size is different than the current size of
                 * the scrolled window, then ignore this event, since it means
                 * that our size has changed since this configure notify event
                 * was generated; we'll wait for the next one to arrive, and try
                 * again.
                 */
                if ((file_mgr_rec->scroll_window->core.height !=
                     cevent->height) ||
                ...
                if (dialog_data != NULL) {
                        ...
                        LayoutFileIcons(file_mgr_rec, file_mgr_data, False,
                                        False);
                }
        }
}
...
void FileWindowInputCallback(Widget w, XtPointer client_data,
        ...
        if (new_event->type == ButtonPress) {
                ...
                if (first) {
                        ...
                        NumSelectKeys = XmeVirtualToActualKeysyms(
                            XtDisplay(w), osfXK_Select, &SelectBindings);
                ...
                */
                XmTranslateKey(kevent->display, kevent->keycode,
                               offset ? ShiftMask : 0, &modif, &keysym);
        }
        ...
        if (!desktopRec && new_event->type == KeyPress &&
            (keysym == PlusKeysym || keysym == MinusKeySym)) {
                ...
                if (keybdFocusPolicy == XmEXPLICIT)
                        ...
                        eventIcon =
                            (Widget)InputForGadget(w, kevent->x, kevent->y);
                if (eventIcon == NULL)
                        return;

                /* find the widget in the file_list */
                found = False;
                for (i = -1; i < file_mgr_data->directory_count && !found;
                     i++) {
                        for (j = 0;
                             j < file_mgr_data->directory_set[i]->file_count;
                             j++) {
                                fileViewData = file_mgr_data->directory_set[i]
                                                   ->file_view_data[j];
                ...
                if (found && fileViewData->file_data->is_subdir) {
                        ...
                        DirTreeExpand(file_mgr_data, fileViewData,
                                      (keysym == PlusKeysym));
                        ...
                        RedrawTreeLines(w, -w->core.x, -w->core.y,
                                        XtParent(w)->core.width,
                                        XtParent(w)->core.height, 0,
                                        file_mgr_rec, file_mgr_data);
                }
        }
                ...
                if (new_event->type == KeyPress) {
                        /* We have a KeyPress: need to check to see if the press
                           is "return", "space", "select". If it is, need to
                           select the Icon the focus is in. */
                        ...
                        if (desktopRec)
                                ...
                                eventIcon = (Widget)XmGetFocusWidget(
                                    file_mgr_rec->file_window);
                        event = (XButtonPressedEvent *)new_event;
                } else {
                        event = (XButtonPressedEvent *)new_event;
                        if (event->button != Button1) {
                                if (event->button == bMenuButton &&
                                    desktopRec == NULL)
                                        FmPopup(NULL, NULL, (XEvent *)event,
                                                file_mgr_data);
                                return;
                        }
                        eventIcon =
                            (Widget)InputForGadget(w, event->x, event->y);
                }

                /*
                 * If the event occurred in an icon, then we are either doing
                 * a selection or a drag.  If it started in the file window,
                 * then we can only be doing a region selection.
                 */
                if (eventIcon != NULL) {
                        /*
                         * If this icon is currently selected, then we can't yet
                         * tell whether the user is starting a drag operation,
                         * or simply redoing the icon selection; we must wait
                         * for either the drag threshold to be surpassed, or the
                         * button up event.
                         */
                        ...
                        if (((file_mgr_rec &&
                            ...
                            event->type == ButtonPress) {
                                ...
                                if ((file_mgr_rec &&
                                     FileIsSelected(file_mgr_data,
                                                    fileViewData)) &&
                                    file_mgr_data->selected_file_count > 1) {
                                        if (PositioningEnabledInView(
                                                file_mgr_data)) {
                                                ObjectPtr top;
                                                ...
                                                /* now make it the top of the
                                                 * selection list */
                                                /* Force selection list order to
                                                 * match stacking order */
                                                selection_list =
                                                    (FileViewData **)XtMalloc(
                                                        sizeof(FileViewData *) *
                                                        (file_mgr_data
                                                             ->selected_file_count +
                                                         1));
                                ...
                                /*
                                 * The icon is not currently selected, so we
                                 * will select it. It is still possible that the
                                 * user may initiate a drag.
                                 */
                                ProcessBtnUp = False;
                                ...
                                case osfXK_Delete: {
                                        ...
                                        _DtFreeActionArgs(action_args,
                                                          arg_count);
                                } break;
                                case osfXK_Menu:
                                        if (file_mgr_data) /* if not, then it is
                                                              a desktop object
                                                            */
                                                FmPopup(NULL,
                ...
                /*
                 * When a button 1 up event is received, we will first see if
                 * the select occurred in the icon's title; if so, then we will
                 * post the rename text field.  If the icon in which the button
                 * down occurred was not already selected, then all of the
                 * selection work was done at button 1 down time, so there is
                 * nothing left for us to do but return.  However, if the icon
                 * was already selected, then we could not do any select
                 * processing at button down time, because the user may have
                 * been trying to initate a drag request; this can't be
                 * determined until either the drag threshold is surpassed, or
                 * the button up occurs.
                 */
                if (((initialDragX >= 0) && (initialDragY >= 0)) &&
                    (eventIcon = (Widget)InputForGadget(w, initialDragX,
                                                        initialDragY))) {
                        /*
                         * If processing was postponed until the button up, then
                         * do it now.
                         */
                        B1DragPossible = False;
                        if (ProcessBtnUp) {
                        ...
                        /*
                         * If the select occurred in the title portion of the
                         * icon, then put up a text edit field, which can be
                         * used to change the name of the icon.  The last field
                         * '0' means its not on the Desktop.
                         */
                        if (_DtIconSelectInTitle(eventIcon,
                                                 (Position)initialDragX,
                            ...
                            ProcessBtnUpCD) {
                                ...
                                if (desktopRec) {
                                        CreateNameChangeDialog(
                                            eventIcon, fileViewData,
                                            (XtPointer)desktopRec, DESKTOP);
                                } else if (file_mgr_data->view != BY_NAME) {
                                        /* We don't allow users to rename when
                                           they are viewing a directory in 'By
                                           Name Only' mode. Because clicking on
                                           the label is the only way for them to
                                           select a file. Popup a text widget to
                                           allow rename would be very confusing
                                        */
                                        if (file_mgr_data->view !=
                                                BY_ATTRIBUTES ||
                                            (file_mgr_data->view ==
                                                 BY_ATTRIBUTES &&
                                             TurnOnTextName))
...
static void GetSelectionRectangle(Display *display, Window window,
        ...
        {
                XQueryPointer(display, window, &root, &sub_win, &win_x, &win_y,
                              &x2, &y2, &mask);
                ...
                if ((mask & Button1Mask) == 0) {
                        ...
                        /*
                         * The real bound of the rectangle is contained in the
                         * button up event; you can't use the pointer position,
                         * because if the system is slow, then by the time we
                         * query the mouse, the button may have been long ago
                         * release, and the pointer subsequently moved.
                         */
                        if (XPeekIfEvent(display, &btnUpEvent,
                                         (Bool(*)())CheckForBtnUp, NULL)) {
                ...
                if ((x2 > old_x && !hsbVisible &&
                     (Dimension)old_x < file_window_width) ||
                    (x2 > old_x && hsbVisible &&
                    ...
                    (y2 > old_y && !vsbVisible &&
                     (Dimension)old_y < file_window_height) ||
                    (y2 > old_y && vsbVisible &&
                    ...
                    (y2 < old_y && vsbVisible && vb_val_ret >= 0)) {
                        DrawRectangle(display, window, draw_gc, x1, y1, old_x,
                                      old_y);
                        if ((Dimension)x2 >= max_width) {
                                if (hsbVisible) {
                                        hb_val_ret += (x2 - max_width);
                                        if ((Dimension)(hb_val_ret +
                                                        hb_slide_size) <=
                                            (Dimension)(file_window_width +
                                                        1)) {
                                                XmScrollBarSetValues(
                                                    hb, hb_val_ret,
                                                    hb_slide_size, hb_inc_ret,
                                                    hb_pg_inc_ret, True);
                                                min_width = (x2 - max_width) +
                                                            min_width;
                                                max_width = x2;
                                                XmUpdateDisplay(
                                                    (Widget)file_window);
                                        } else if ((Dimension)(hb_val_ret +
                                                               hb_slide_size) >
                                                       (Dimension)(
                                                           file_window_width +
                                                           1) &&
                                                ...
                                                XmScrollBarSetValues(
                                                    ...
                                                    hb_slide_size, hb_inc_ret,
                                                    hb_pg_inc_ret, True);
                                                temp = (file_window_width -
                                                        hb_slide_size + 1) -
                                                       hb_val_ret;
                                                hb_val_ret = file_window_width -
                                                             hb_slide_size + 1;
                        ...
                        if ((Dimension)y2 > max_height) {
                                if (vsbVisible) {
                                        ...
                                        if ((Dimension)(vb_val_ret +
                                                        vb_slide_size) <=
                                            (Dimension)(file_window_height +
                                                        1)) {
                                                XmScrollBarSetValues(
                                                    vb, vb_val_ret,
                                                    vb_slide_size, vb_inc_ret,
                                                    vb_pg_inc_ret, True);
                                                min_height = (y2 - max_height) +
                                                             min_height;
                                                max_height = y2;
                                                XmUpdateDisplay(
                                                    (Widget)file_window);
                                        } else if ((Dimension)(vb_val_ret +
                                                               vb_slide_size) >
                                                       (Dimension)(
                                                           file_window_height +
                                                           1) &&
                                                ...
                                                XmScrollBarSetValues(
                                                    vb,
                                                    file_window_height + 1 -
                                                        vb_slide_size,
                                                    vb_slide_size, vb_inc_ret,
                                                    vb_pg_inc_ret, True);
                                                temp = (file_window_height + 1 -
                                                        vb_slide_size) -
                                                       vb_val_ret;
                                                vb_val_ret =
                                                    file_window_height + 1 -
                        ...
                        /*  Run through the gadget set to cause each icon within
                         * the  */
                        ...
                        for (i = 0; i < directory_count; i++) {
                                for (j = 0; j < directory_set[i]->file_count;
                                     j++) {
                                        DtIconGadget g;

                                        file_view_data =
                                            directory_set[i]->order_list[j];
                                        ...
                                        marginHeight =
                                            g->icon.cache->margin_height;
                                        pixmapPosition =
                                            g->icon.cache->pixmap_position;
                                        ...
                                        /*
                                                       pixmap_x1 = pixmap_x +
                                           pixmapWidth; pixmap_y1 = pixmap_y +
                                           pixmapHeight;
                                        */

                                        if (XPointInRegion(region, pixmap_x,
                                                           pixmap_y)
                                            /*
                                                               &&
                                               XPointInRegion(region, pixmap_x1,
                                               pixmap_y1)
                                            */
                                        ) {
...
static void DrawRectangle(Display *display, Window window, GC gc, int x1,
        ...
        if (width > 2 && height > 2)
                XDrawRectangle(display, window, gc, x + 1, y + 1, width - 2,
                               height - 2);
}
...
static void ProcessSelection(FileMgrData *file_mgr_data, Widget icon,
        ...
        if (icon == NULL && event->type == ButtonPress) {
                ...
                GetSelectionRectangle(
                    XtDisplay(file_mgr_rec->file_window),
                ...
                for (; i < directory_count; i++) {
                        if (PositioningEnabledInView(file_mgr_data)) {
                                ...
                                /*
                                 * Stacking order requires us to handle this
                                 * specially.  Objects must be searched in top
                                 * to bottom order when determining if they are
                                 * in the selection rectangle, but they must be
                                 * drawn bottom to top, to guarantee that the
                                 * visuals are correct.
                                 */
                                ...
                                while (top) {
                                        ...
                                        WidgetRectToRegion(
                                            ...
                                            widget_region);
                                        XIntersectRegion(region, widget_region,
                                                         intersect_region);
                                        ...
                                        if (!XEmptyRegion(intersect_region)) {
                                                ...
                                                /* Higher item 'cover' lower
                                                 * ones */
                                                XSubtractRegion(region,
                                                                widget_region,
                                                                region);
                                        }
                                ...
                                if (!XEmptyRegion(redraw_region))
                                        RepaintDesktop(file_mgr_data, NULL,
                                                       redraw_region);

                                RegisterDesktopHotspots(file_mgr_data,
                                                        file_mgr_rec);
                                                    ...
                                                    FileIsSelected(
                                                        ...
                                                        if (file_mgr_data
                                                            ...
                                                            BY_ATTRIBUTES) {
                                                                ...
                                                                textExtent
                                                                    .height =
                                                                    (unsigned short)file_view_data
                                                                        ->widget
                                                                        ->core
                                                                        .height;

                                                                textExtent.x =
                                                                    (short)file_view_data
                                                                        ->widget
                                                                        ->core
                                                                        .x +
                                                                    g->icon
...
void DrawHighlight(Widget w, FileViewData *file_view_data,
        ...
        if (type == NOT_DESKTOP && file_view_data != NULL) {
                if (PositioningEnabledInView(file_mgr_data)) {
                        top = GetTopOfStack(file_mgr_data);
                        if (top != NULL &&
                            top->file_view_data != file_view_data) {
                ...
                if (alignment == XmALIGNMENT_BEGINNING) {
                        ...
                        if (stringWidth > pixmapWidth) {
                                ...
                                rect[num_rectangles].x = x + stringWidth +
                                                         (2 * adj_x) -
                                                         highlightThickness;
                                rect[num_rectangles].y = y + pixmapHeight +
                                                         (2 * adj_y) -
                                                         highlightThickness;
                        ...
                        rect[num_rectangles].x = x;
                        rect[num_rectangles].y =
                            y + pixmapHeight + stringHeight + spacing +
                        ...
                        if (stringWidth > pixmapWidth) {
                                ...
                                rect[num_rectangles].x =
                                    x + (Dimension)(stringWidth - pixmapWidth) /
                                            (Dimension)2;
                                rect[num_rectangles].y = y;
                                ...
                                rect[num_rectangles].x =
                                    x +
                                    (Dimension)(stringWidth - pixmapWidth) /
                                        (Dimension)2 +
                                    pixmapWidth + (2 * adj_x) -
                                    highlightThickness;
                                rect[num_rectangles].y = y;
                                ...
                                rect[num_rectangles].x =
                                    x + pixmapWidth + adj_x -
                                    (Dimension)(pixmapWidth - stringWidth) /
                                        (Dimension)2 +
                                    highlightThickness;
                                rect[num_rectangles].y = y + pixmapHeight +
                                                         spacing +
                                ...
                                /* 5 */
                                rect[num_rectangles].x =
                                    x + (Dimension)(pixmapWidth - stringWidth) /
                                            (Dimension)2;
                                rect[num_rectangles].y =
                                    y + pixmapHeight + stringHeight + spacing +
                                    (2 * adj_y) - highlightThickness;
        ...
        case XmPIXMAP_LEFT:
                if ((Dimension)(stringHeight + 4) >= pixmapHeight)
                        ...
                        rect[num_rectangles].width = highlightThickness;
                        rect[num_rectangles++].height =
                            (Dimension)(pixmapHeight - stringHeight) /
                            (Dimension)2;
                        ...
                        rect[num_rectangles].x =
                            ...
                            (Dimension)(spacing + 1) / (Dimension)2;
                        rect[num_rectangles].y =
                            y + (Dimension)(pixmapHeight - stringHeight) /
                                    (Dimension)2;
                        rect[num_rectangles].width =
                        ...
                        /* 4 */
                        rect[num_rectangles].x = x + adj_x + pixmapWidth +
                                                 spacing + stringWidth + adj_x -
                                                 highlightThickness;
                        rect[num_rectangles].y =
                            y + (Dimension)(pixmapHeight - stringHeight) /
                                    (Dimension)2;
        ...
        XFillRectangles(XtDisplay(w), XtWindow(w), gc, &rect[0],
                        num_rectangles);
}
...
void DrawUnhighlight(Widget w, int type) {
                ...
                if (alignment == XmALIGNMENT_BEGINNING) {
                        ...
                        if (stringWidth > pixmapWidth) {
                                ...
                                XClearArea(XtDisplay(w), XtWindow(w),
                                           x + pixmapWidth + (2 * adj_x) -
                                               highlightThickness,
                                           y, highlightThickness,
                                           pixmapHeight + spacing, False);
                                ...
                                XClearArea(XtDisplay(w), XtWindow(w),
                                           x + pixmapWidth + (2 * adj_x) -
                                               highlightThickness,
                                           y + pixmapHeight + spacing,
                                           stringWidth - pixmapWidth +
                                               highlightThickness,
                                           highlightThickness, False);

                                /* 4 */
                                XClearArea(XtDisplay(w), XtWindow(w),
                                           x + stringWidth + (2 * adj_x) -
                                               highlightThickness,
                                           y + pixmapHeight + spacing,
                                           highlightThickness,
                        ...
                        /* 6 */
                        XClearArea(
                            XtDisplay(w), XtWindow(w), x, y, highlightThickness,
                            pixmapHeight + stringHeight + spacing + (2 * adj_y),
                            False);
        ...
        case XmPIXMAP_LEFT:
                if ((Dimension)(stringHeight + 4) >= pixmapHeight)
                        ...
                        XClearArea(
                            XtDisplay(w), XtWindow(w),
                            x + adj_x + pixmapWidth + spacing + stringWidth +
                                adj_x - highlightThickness,
                            y + (Dimension)(pixmapHeight - stringHeight) /
                                    (Dimension)2,
                            highlightThickness, stringHeight + 2 * adj_y,
                            False);
                        ...
                        /* 8 */
                        XClearArea(XtDisplay(w), XtWindow(w), x, y,
                                   highlightThickness, pixmapHeight + 2 * adj_y,
                                   False);
                }
                break;
        case XmPIXMAP_BOTTOM:
...
void DrawShadowTh(Widget w, GC gc, int type) {
         ...
         */
        s_out =
            (shadowType == XmSHADOW_OUT || shadowType == XmSHADOW_ETCHED_OUT)
                ? True
                : False;
        ...
        if (gc == NULL) {
                if (shadowType == XmSHADOW_ETCHED_OUT) {
                        top_gc = (s_out) ? mgr->manager.top_shadow_GC
                                         : mgr->manager.bottom_shadow_GC;
                        bottom_gc = (s_out) ? mgr->manager.bottom_shadow_GC
                                            : mgr->manager.top_shadow_GC;
                } else {
                        bottom_gc = (s_out) ? mgr->manager.top_shadow_GC
                                            : mgr->manager.bottom_shadow_GC;
                        top_gc = (s_out) ? mgr->manager.bottom_shadow_GC
                                         : mgr->manager.top_shadow_GC;
                }
        } else
                top_gc = bottom_gc = gc;
        ...
        if (!s_etched) {
                ...
                shadowThickness = (shadowThickness % 2) ? (shadowThickness - 1)
                                                        : (shadowThickness);
                        ...
                        if (alignment == XmALIGNMENT_BEGINNING) {
                                if (type == NOT_DESKTOP) {
                                        ...
                                        rect[num_rects].width =
                                            half_shadowThickness;
                                        rect[num_rects++].height =
                                            stringHeight + highlightThickness +
                                            (2 * marginHeight) +
                                            2 * shadowThickness;
                                ...
                                if (stringWidth > pixmapWidth) {
                                        ...
                                        /* 3 */
                                        rect[num_rects].x = x + stringWidth +
                                                            (2 * marginWidth) +
                                                            shadowThickness +
                                                            highlightThickness;
                                        rect[num_rects].y =
                        ...
                        if (type == NOT_DESKTOP) {
                                if ((Dimension)(stringHeight + 5) >=
                                    (Dimension)pixmapHeight) {
                                        ...
                                        rect[num_rects].width = stringWidth +
                                                                marginWidth +
                                                                shadowThickness;
                                        rect[num_rects++].height =
                                            half_shadowThickness;
                                        ...
                                        rect[num_rects].x =
                                            x + highlightThickness +
                                            pixmapWidth + 3 * marginWidth +
                                            2 * shadowThickness + stringWidth;
                                        rect[num_rects].y =
                                            y + highlightThickness +
                                            ((Dimension)(pixmapHeight -
                                                         stringHeight) /
                ...
                case XmPIXMAP_BOTTOM:
                case XmPIXMAP_RIGHT:
                        XmeDrawShadows(XtDisplay(w), XtWindow(w), top_gc,
                                       bottom_gc, x + highlightThickness,
                                       y + highlightThickness,
                                       width - 2 * highlightThickness,
                                       height - 2 * highlightThickness,
                                       shadowThickness, XmSHADOW_IN);
