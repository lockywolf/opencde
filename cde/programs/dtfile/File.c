...
static int FileNameAscending(FileViewData **t1, FileViewData **t2);
static int FileNameDescending(FileViewData **t1, FileViewData **t2);
static int FileTypeAscending(FileViewData **t1, FileViewData **t2);
static int FileTypeDescending(FileViewData **t1, FileViewData **t2);
static int FileSizeAscending(FileViewData **t1, FileViewData **t2);
static int FileSizeDescending(FileViewData **t1, FileViewData **t2);
static int FileDateAscending(FileViewData **t1, FileViewData **t2);
static int FileDateDescending(FileViewData **t1, FileViewData **t2);
...
static void RedisplayUsingStackingOrder(FileMgrData *file_mgr_data, Widget w,
                                        register XEvent *event, Region region);
...
static XmGadget InputInGadget(Widget w, register int x, register int y);
...
static int StrCaseCmp(const char *s1, const char *s2) {
        ...
        */
        ...
        for (; tolower(*s1) == tolower(*s2); ++s1, ++s2) {
                if (*s1 == '\0') {
                        /* File Manager depends heavily on this routine to
                           position the icons in its window. We want case
                           INSENSITIVE comparision, however when we have 2 same
                           size and case equivalent strings i.e.   "A" and "a",
                           we need this routine to behave like a case SENSITIVE
                           comparision, so the position of the icon is constant
                           so "A" is always before "a".
                        */
                        if (*s2 == 0x0) {
                                for (; *s1a == *s2a; ++s1a, ++s2a) {
                                        if (*s1 == '\0')
...
static Boolean SpecialCases(FileViewData **t1, FileViewData **t2, int *rc) {
        /* Tree mode */
        if (((FileMgrData *)((DirectorySet *)((*t1)->directory_set))
                 ->file_mgr_data)
                ->show_type == MULTIPLE_DIRECTORY) {
                if (!(*t1)->file_data->is_subdir && (*t2)->file_data->is_subdir)
                        *rc = -1;
                else if ((*t1)->file_data->is_subdir &&
                         !(*t2)->file_data->is_subdir)
                        *rc = 1;
                else
                        return False;
                return True;
...
static int FileTypeAscending(FileViewData **t1, FileViewData **t2) {
        ...
        rc = strcoll((*t1)->file_data->logical_type,
                     (*t2)->file_data->logical_type);
        if (rc == 0)
...
static int FileSizeAscending(FileViewData **t1, FileViewData **t2) {
        ...
        else if ((*t1)->file_data->stat.st_size >
                 (*t2)->file_data->stat.st_size)
                return 1;
...
void FilterFiles(FileMgrData *file_mgr_data, DirectorySet *directory_set) {
        ...
        for (i = 0; i < directory_set->file_count; i++) {
                ...
                /* if we want to show the hidden files no more should be
                 * filtered out */
                if (show_hidden)
                        continue;
                ...
                /* Check for a match against the filter expression string,
                 * except for files in the trash directory and sub directories
                 * in tree mode. */
                        ...
                        if (!strcmp(file_view_data[i]->file_data->file_name,
                            ...
                            !strcmp(file_view_data[i]->file_data->file_name,
                                ...
                                if (fnmatch((const char *)filter,
                                            (const char *)file_view_data[i]
                                                ->file_data->action_name,
                                            0) == 0) {
                                        if (mustMatch) {
                                                filterCount++;
                                                file_view_data[i]->filtered =
                                                    True;
                                                continue;
                                        } else
                                                matches = True;
                ...
                /* now lets check through the filter filetypes and if the file
                 * is  */
                /* filtered, filter it out */
                if (file_mgr_data != trashFileMgrData) {
                        /* This is for the case of files likes 'action' files
                           which do not have a logical_type */

                        if (strcmp(
                                file_view_data[i]->file_data->file_name,
                                file_view_data[i]->file_data->logical_type) ==
                            0) {
                                ...
                                for (j = 0; j < filter_data->count; j++) {
                                        if (strcmp(filter_data->user_data[j]
                                                       ->filetype,
                                                   file_view_data[i]
                                                       ->file_data
                                                       ->logical_type) == 0) {
                                                if ((filter_data->user_data[j]
                                                             ->selected ==
                                                         True &&
                                                     filter_data->match_flag) ||
                                                    (filter_data->user_data[j]
                                                             ->selected ==
                                                         False &&
                                                     !filter_data
                                                          ->match_flag)) {
 ...
 */
{
        ...
        if (ip->desc) {
                ...
                for (i = 0; i < directory_set->file_count; i++)
                        CopyOrderedEntries(directory_set->order_list[i],
                                           fvd_array, index);
        }
}
...
void IconCallback(Widget w, XtPointer clientData, XtPointer callData) {
        ...
        if ((callback->reason == XmCR_ARM) ||
            (callback->reason == XmCR_SELECT)) {
                /*
                 * Both ARM and SELECT are generated using Button1. We pass on
                 * these requests to the code responsible for processing button
                 * 1 selects and drags.
                 */
                ...
                if (event->type == KeyPress) {
                        ...
                        if (callback->reason == XmCR_ARM)
                                ...
                                keysym =
                                    XLookupKeysym((XKeyEvent *)kevent, offset);
                ...
                if (desktopRec) {
                        ...
                        XtCallCallbacks(desktopRec->drawA, XmNinputCallback,
                                        callData);
                } else {
                        B1DragPossible = False;
                        B2DragPossible = False;
                        ...
                        XtCallCallbacks(fileMgrRec->file_window,
                                        XmNinputCallback, callData);
                }
                 ...
                 * If we are in the middle of a button 2 drag, then we'll ignore
                 * this; otherwise, we now know a drag will not start, so clear
                 * all state flags.
                 */
                if (B2DragPossible)
                        return;
                 ...
                 * If we are in the middle of a button 1 drag, then we'll ignore
                 * this; otherwise, we now know a drag will not start, so clear
                 * all state flags.
                 */
                if (B1DragPossible)
        ...
        run_default_action:
                ...
                if (event->type == ButtonPress) {
                        if (loc_cursor1 == None)
                                loc_cursor1 =
                                    XCreateFontCursor(XtDisplay(w), 40);
                        if (loc_cursor2 == None)
                                loc_cursor2 =
                                    XCreateFontCursor(XtDisplay(w), 34);

                        root = RootWindowOfScreen(XtScreen(w));
                        XGrabPointer(XtDisplay(w), root, False,
                                     ButtonReleaseMask | PointerMotionMask,
                                     GrabModeAsync, GrabModeAsync, None,
                                     loc_cursor1, CurrentTime);
                        do {
                                XIfEvent(XtDisplay(w), &bevent,
                                         IsButtonOrMotion, NULL);
                                if (!dragged && bevent.type == MotionNotify) {
                                        dx = event->x_root -
                                             bevent.xmotion.x_root;
                                        if (dx < 0)
                                                dx = -dx;
                                        dy = event->y_root -
                                             bevent.xmotion.y_root;
                                        if (dy < 0)
                                                dy = -dy;
                                        if (dx > DBLCLICK_DRAG_THRESHOLD ||
                                            dy > DBLCLICK_DRAG_THRESHOLD) {
                                                DPRINTF(("dragged!\n"));
                                                XGrabPointer(
                                                    XtDisplay(w), root, False,
                                                    ButtonReleaseMask |
                                                        PointerMotionMask,
                                                    GrabModeAsync,
                                                    GrabModeAsync, None,
                                                    loc_cursor2, CurrentTime);
                                                dragged = True;
                                        }
                                }
                        ...
                        if ((openDirType == NEW &&
                            ...
                            (openDirType != NEW &&
                             strcmp(command, openNewView) == 0)) {
                                ...
                                RunCommand(
                                    openNewView, fileMgrData, fileViewData,
                                    dragged ? &position : NULL, NULL, NULL);
                                if ((modifiers != 0) &&
                                    ((modifiers & ControlMask) != 0)) {
                                        DialogData *dialog_data;

                                        dialog_data = _DtGetInstanceData(
                                            fileMgrData->file_mgr_rec);
                                        CloseView(dialog_data);
                                }
                        } else {
                                if (command) {
                                        if ((fileMgrData->show_type ==
                                                 MULTIPLE_DIRECTORY ||
                                             dragged) &&
                                            strcmp(command, openInPlace) == 0) {
                                                RunCommand(
                                                    openNewView, fileMgrData,
                                                    fileViewData,
                                                    dragged ? &position : NULL,
                                                    NULL, NULL);
                                        } else
                                                RunCommand(command, fileMgrData,
                                                           fileViewData, NULL,
                                                           NULL, NULL);
                                } else
                                        Error = True;
                ...
                if (Error) {
                        ...
                        (void)sprintf(
                            ...
                            logicalType);
                        if (desktopRec)
                                _DtMessage(desktopRec->shell, title, msg, NULL,
                                           HelpRequestCB);
                        else
                                _DtMessage(fileViewData->widget, title, msg,
                                           NULL, HelpRequestCB);
                        XtFree(title);
                }
                ...
                if (desktopRec) {
                        ...
                        if (fileMgrData &&
                            !FileIsSelected(fileMgrData, fileViewData) &&
                            adjustState) {
                ...
                if (dragActive) {
                        if ((g->icon.border_type == DtRECTANGLE) ||
                            (!g->icon.pixmap))
                ...
                if ((highlightType != INIT_VALUE) && (highlightWidget)) {
                        if (desktopRec) {
                                if (highlightType == NOT_DESKTOP)
                                        DrawUnhighlight(highlightWidget,
                                                        NOT_DESKTOP);
                ...
                if (desktopRec) {
                        ...
                        DrawHighlight(w, fileViewData, fileMgrData,
                                      NOT_DESKTOP);
        ...
        if ((desktopRec == NULL) && FileIsSelected(fileMgrData, fileViewData))
                ...
                if (DTFileIsSelected(desktopRec, fileViewData)) {
                        if (DtWsmGetCurrentWorkspace(
                                ...
                                &pCurrent) == Success) {
                                wsName = XGetAtomName(
                                    XtDisplay(desktopRec->shell), pCurrent);
                                CleanUpWSName(wsName);
                        } else
                                wsName = XtNewString("One");

                        for (*dt = 0; *dt < desktop_data->numWorkspaces;
                             (*dt)++) {
                                if (strcmp(wsName,
                                           desktop_data->workspaceData[*dt]
                                               ->name) == 0) {
                                        selectedCount =
                                            desktop_data->workspaceData[*dt]
                                                ->files_selected;
        ...
        if (initiating_view &&
            ((FileMgrData *)initiating_view)->view == BY_ATTRIBUTES) {
                ...
                wid = XmStringWidth(fontList, fileNameString) +
                      g->icon.pixmap_width + g->icon.cache->margin_width +
                      g->icon.cache->spacing + G_ShadowThickness(g) +
...
static void NewConvertDelete(Widget w, FileViewData *fileViewData,
        ...
        if (child_pid == -1) {
                ...
                tmpStr = GETMESSAGE(11, 59,
                                    "Cannot create a child process to delete "
                                    "the dropped files.");
                msg = XtNewString(tmpStr);
                title =
                    XtNewString((GETMESSAGE(11, 58, "Process Create Error")));
        ...
        if (selectedCount > 1) {
                for (i = (selectedCount - 1); i >= 0; i--) {
                        ...
                        if ((!desktopRec) && (fileMgrData->toolbox))
                                path = _DtResolveAppManPath(
                                    path, fileMgrData->restricted_directory);
                ...
                *numFiles = selectedCount;
                ...
                if (desktopRec) {
                        ...
                        directoryName =
                            ((DirectorySet *)fileViewData->directory_set)->name;
                        fileName = fileViewData->file_data->file_name;
                } else {
...

{
        ...
        dragIconPixmapOffsetY =
            G_ShadowThickness(iconG) + G_MarginHeight(iconG);
                ...
                if (XmIsGadget(widget) && XtIsManaged(widget)) {
                        if (simpleCheck) {
                                if (x >= widget->core.x &&
                                    y >= widget->core.y &&
                                    (Position)x <
                                        (Position)(widget->core.x +
                                                   widget->core.width) &&
                                    (Position)y <
                                        (Position)(widget->core.y +
                                                   widget->core.height)) {
                                ...
                                _DtIconGetIconRects((DtIconGadget)widget,
                                                    &flags, &pRect, &lRect);
        ...
        if ((B1DragPossible && (event->xmotion.state & Button1Mask)) ||
            (B2DragPossible && (event->xmotion.state & Button2Mask))) {
                ...
                if ((ABS(diffX) >= dragThreshold) ||
                    (ABS(diffY) >= dragThreshold)) {
                        /* Map the original (x,y) into a gadget Id */
                        if (dragIcon = (Widget)InputForGadget(w, initialDragX,
                                                              initialDragY)) {
                                /* Map the icon into its fileViewData structure
                                 */
                                /* Check for desktop icon first */
                                for (i = 0, fileViewData = NULL;
                                     i < desktop_data->numIconsUsed; i++) {
                                        if (desktop_data->desktopWindows[i]
                                                ->iconGadget == dragIcon) {
                                                fileViewData =
                                                    desktop_data
                                                        ->desktopWindows[i]
                                ...
                                if (fileViewData == NULL) {
                                        ...
                                        XtSetArg(args[0], XmNuserData,
                                                 (XtPointer)&directoryData);
                                        XtGetValues(dragIcon, args, 1);
                                        for (i = 0;
                                             i < directoryData->file_count;
                                             i++) {
                                                if (directoryData
                                                            ->file_view_data[i]
                                                            ->widget ==
                                ...
                                XtTranslateCoords(w, (Position)initialDragX,
                                ...
                                if (directoryData) {
                                        ...
                                        fmd = (FileMgrData
                                                   *)(directoryData
                                                          ->file_mgr_data);
                                        if (fmd)
                                                fmd->drag_file_view_data =
                                                    fileViewData;
                                }
                                InitiateIconDrag(fileViewData, (int)rootX,
                                                 (int)rootY, event);
                        } else {
                                /*
                                 * The file manager view must have changed
                                 * between the time the user did the button
                                 * down, and the time they moved enough to pass
                                 * the drag threshold, because there is now no
                                 * icon located where the drag initiated.
                                 * Therefore, we'll just clean up, because there
...

{
         ...
         * such that the pixmap within the icon gadget will end up at the
         * desired position, we need to know the offset of the gadget's icon
         * pixmap relative to the desktop icon shell. Since a desktop icon
         * consists a frame widget, a drawing area and an icon gadget (all
         * within a popup shell), this offset is computed by adding the frame
         * width and and drawing area margins to the icon gadget's highlight
         * thickness. The frame width is 3 and and and the drawing area margin
         * width is 1 (both hardcoded in Desktop.c).  We determine the
         * remaining components by looking at one of the existing desktop icon
         * gadgets. */
        ...
        if (!file_view_data->need_update) {
                if (file_view_data->file_data->link == NULL)
                        SetToSelectColors(
                            ...
                            (Widget)((FileMgrRec *)file_mgr_data->file_mgr_rec)
                                ->file_window,
                            0);
                else
                        SetToSelectColors(
...
void SelectFile(FileMgrData *file_mgr_data, FileViewData *file_view_data) {
        ...
        for (i = file_mgr_data->selected_file_count; i > 0; i--)
                file_mgr_data->selection_list[i] =
                    file_mgr_data->selection_list[i - 1];
...
void DeselectFile(FileMgrData *file_mgr_data, FileViewData *file_view_data,
        ...
        for (j = i; j < selection_count - 1; j++)
                file_mgr_data->selection_list[j] =
                    file_mgr_data->selection_list[j + 1];
...
void DeselectAllFiles(FileMgrData *file_mgr_data) {
        ...
        if (!PositioningEnabledInView(file_mgr_data)) {
                ...
                /*
                 * We can't simply redraw the selected files; we must also
                 * redraw any unselected files which are higher on the stacking
                 * order.
                 */
                ...
                for (i = 0; i < file_mgr_data->selected_file_count; i++) {
                        file_view_data = file_mgr_data->selection_list[i];
                        for (j = 0; j < i; j++) {
                                if (file_view_data->position_info
                                        ->stacking_order <
                                    repaint_list[j]
                                        ->position_info->stacking_order) {
                                        /* Insert here, pushing down all lower
                                         * entries */
                                        for (k = file_mgr_data
                                                     ->selected_file_count -
                                                 1;
                ...
                while (bottom) {
                        if ((i >= 0) &&
                            (bottom == repaint_list[i]->position_info)) {
                                ...
                                SetFileUnselected(file_mgr_data,
                                                  bottom->file_view_data);
                                i--;
                        }
                        if (bottom->file_view_data != NULL &&
                            !bottom->file_view_data->need_update) {
                                RedrawOneGadget(bottom->file_view_data->widget,
                                                NULL, NULL);
                        }
                        bottom = bottom->prev;
                }
...
void SelectAllFiles(FileMgrData *file_mgr_data) {
        ...
        if (PositioningEnabledInView(file_mgr_data)) {
                ...
                selection_list = (FileViewData **)XtMalloc(
                    sizeof(FileViewData *) * (file_mgr_data->num_objects));
                        ...
                        for (j = 0; j < directory_data->file_count; j++) {
                                ...
                                */
                                if (order_list[j]->filtered == True ||
                                    strcmp(order_list[j]->file_data->file_name,
                                           "..") == 0)
                                        continue;
                                ...
                                file_mgr_data->selection_list =
                                    (FileViewData **)XtRealloc(
                                        (char *)file_mgr_data->selection_list,
                                        sizeof(FileViewData *) *
                                            (selection_count + 1));

                                file_mgr_data->selection_list[selection_count] =
                                    NULL;
                                file_mgr_data
                                    ->selection_list[selection_count - 1] =
                                    order_list[j];
        ...
        if (file_mgr_data != trashFileMgrData) {
                if (selection_count == 0)
                        ...
                        ActivateSingleSelect(
                            (FileMgrRec *)file_mgr_data->file_mgr_rec,
                            file_mgr_data->selection_list[0]
...
int
#ifdef _NO_PROTO
    GetInsertPosition(x1, x2, fontList, name) int x1;
int x2;
XmFontList fontList;
char *name;
#else
...
#else  /* MULTIBYTE */
        for (tmp = name + 1, i = 0; *tmp != 0x0; ++tmp, ++i)
#endif /* MULTIBYTE */
        {
                XmString string;
...
void CreateNameChangeDialog(Widget w, FileViewData *file_view_data,
        ...
        */
        ...
        XtAddCallback(text, XmNdestroyCallback, DestroyIconName,
                      (XtPointer)NULL);
        ...
        if (type == DESKTOP && sHeight > fHeight) {
                ...
                x = textExtent.x;
                y = textExtent.y -
                    (Dimension)(text->core.height - textExtent.height);
        }

        {
                ...
                XQueryPointer(XtDisplay(parent),
                              XtWindow(file_view_data->widget), &rootWindow,
                              &childWindow, &rootX, &rootY, &pX, &pY, &mask);
                if (pX == 0)
                        insertPosition = strlen(name);
                else {
                        if (type != DESKTOP) {
                                if (pX > (int)x &&
                                    pX < (int)x + (int)stringWidth)
        ...
        if (shapeExtension)
                if (type == DESKTOP) {
                        ...
                        _DtIconGetIconRects(
                            (DtIconGadget)desktopWindow->iconGadget, &flags,
                            &rect[0], &rect[1]);
                        /* 1 */
                        rect[0].x += 1;
                        rect[0].y += 1;
                        ...
                        /* 2 */
                        if (type == DESKTOP && sHeight > fHeight) {
                                /* status area has been attached in multi-byte
                                 * case */
                                rect[1].x = x + 1;
                                rect[1].y = y + 1;
        ...
        if (file_mgr_data->positionEnabled == RANDOM_ON &&
            ...
            file_mgr_data->host != NULL) {
                ...
                fprintf(fd_stream, "*%s.%s.%s.%s:   %d\n#\n", name_list[0],
                        name_list[1], name, "num_positions",
                ...
                for (i = 0; i < file_mgr_data->num_objects; i++) {
                        ...
                        fprintf(fd_stream, "*%s.%s.%s.%s%d:   %s %d %d %d\n",
                                ...
                                ptr->name, ptr->x, ptr->y, ptr->stacking_order);
                }
        } else {
                /* Number of object positions */
                fprintf(fd_stream, "*%s.%s.%s.%s:   %d\n#\n", name_list[0],
                        name_list[1], name, "num_positions", 0);
        }
...
void RestorePositionalData(XrmDatabase db, char **name_list,
        ...
        for (j = 0; j < num_objects; j++) {
                ...
                sscanf((char *)value.addr, "%s %d %d %d", obj_name, &x, &y,
                       &s_order);
                ptr->name = obj_name;
                ptr->x = x;
                ptr->y = y;
...

{
        if (child->core.widget_class->core_class.expose)
                (*(child->core.widget_class->core_class.expose))(child, event,
                                                                 region);
}
                ...
                if (!bottom->late_bind) {
                        if (bottom->file_view_data != NULL &&
                            !bottom->file_view_data->need_update) {
                                ...
                                if (XRectInRegion(region, child->core.x,
                                                  child->core.y,
                                                  child->core.width,
                                                  child->core.height)) {
...

{
        ...
        /*
         * Capture and encompass any other exposure events which are destined
         * for us, but are waiting in the wings.
         */
                        ...
                        if (file_view_data != NULL) {
                                ...
                                if (child && XmIsGadget(child) &&
                                    XtIsManaged(child)) {
                                        ...
                                        WidgetRectToRegion(file_mgr_data, child,
                                                           widget_region);
                                        XIntersectRegion(redrawRegion,
                                                         widget_region,
                                                         widget_region);
                                        if (!XEmptyRegion(widget_region)) {
                                                XSubtractRegion(redrawRegion,
                                                                widget_region,
                                                                redrawRegion);
                                                children = (Widget *)XtRealloc(
                                                    (char *)children,
                                                    (numChildren + 1) *
                                                        sizeof(Widget));
                        ...
                        if (file_view_data != NULL) {
                                ...
                                if (child && XmIsGadget(child) &&
                                    XtIsManaged(child)) {
                                        ...
                                        if ((numChildren >= 0) &&
                                            (children[numChildren] == child)) {
                                                XUnionRegion(redrawRegion,
                                                             widget_region,
                                                             redrawRegion);
                                                RedrawOneGadget(child, event,
                                                                redrawRegion);
                                                numChildren--;
                                        } else {
                                                /* Do we overlap something which
                                                 * was previously redrawn? */
...

{
        ...
        if (file_mgr_data && PositioningEnabledInView(file_mgr_data))
                RedisplayUsingStackingOrder(file_mgr_data, (Widget)da, event,
                                            region);
        else
...

{
        ...
        for (i = 0; (i < file_window->composite.num_children) &&
                    (file_window->composite.children[i] !=
                     savePtr->file_view_data->widget);
             i++)
                ;

        /* Push intervening entries down */
        for (j = i; j >= newPos; j--)
                file_window->composite.children[j] =
                    file_window->composite.children[j - 1];

        /* Insert into new position in children list */
        file_window->composite.children[newPos - 1] =
            savePtr->file_view_data->widget;
}
...

{
        ...
        while (top) {
                if (top->file_view_data != NULL &&
                    top->file_view_data->widget != NULL)
...

{
        ...
        if (file_view_data->displayed) {
                ...
                if (!file_view_data->registered) {
                        /* register drop site for MCL but make drop site
                         * inactive */
                ...
                if (operations) {
                         ...
                         *    The icon gadget leaves space of width
                         * shadowThickness around the pixmap and label; use this
                         * space to make the drop zone larger.
                         *
                         *  - Compensate for bug in drop zone
                         * registration/drawing: For some reason the drop zone
                         * displayed on the screen is actually smaller than
                         * the rectangle we register: The drag&drop library
                         * seems to reduce the size of the drop area by the
                         * value of highlightThickness at the top and left
                         *    border and by highlightThickness-1 at the bottom
                         * and right. Is this a Motif drag&drop bug? We
                         * compensate by registering a larger rectangle.
                         */
                        ...
                        if (numRects) {
                                ...
                                n++;
                                XtSetArg(args[n], XmNnumDropRectangles,
                                         numRects);
                                n++;
                        }
                        ...
                        /* add client data */
                        XtRemoveAllCallbacks(file_view_data->widget,
                                             XmNdropCallback);
...

{
        ...
        if (action = DtDtsDataTypeToAttributeValue(
                file_type, DtDTS_DA_MOVE_TO_ACTION, NULL)) {
                operations = operations | XmDROP_MOVE;
                DtDtsFreeAttributeValue(action);
        }
        if (action = DtDtsDataTypeToAttributeValue(
                file_type, DtDTS_DA_COPY_TO_ACTION, NULL)) {
                operations = operations | XmDROP_COPY;
                DtDtsFreeAttributeValue(action);
        }
        if (action = DtDtsDataTypeToAttributeValue(
                file_type, DtDTS_DA_LINK_TO_ACTION, NULL)) {
...
char *TypeToAction(unsigned char dropOperation, char *logical_type) {
        ...
        case XmDROP_MOVE:
                action = DtDtsDataTypeToAttributeValue(
                    logical_type, DtDTS_DA_MOVE_TO_ACTION, NULL);
                break;
        case XmDROP_COPY:
                action = DtDtsDataTypeToAttributeValue(
                    logical_type, DtDTS_DA_COPY_TO_ACTION, NULL);
                break;
        case XmDROP_LINK:
                action = DtDtsDataTypeToAttributeValue(
                    logical_type, DtDTS_DA_LINK_TO_ACTION, NULL);
                break;
        default:
                action = NULL;
                break;
...
static Boolean IsDesktopPtr(FileViewData *fileViewData,
        ...
        *fileMgrData = NULL;
        ...
        for (i = 0; i < desktop_data->numIconsUsed; i++) {
                if (desktop_data->desktopWindows[i]->file_view_data ==
                    fileViewData) {
        ...
        *fileMgrData =
            (FileMgrData *)((DirectorySet *)fileViewData->directory_set)
                ->file_mgr_data;
        return (False);
}
...
void PositionFileView(FileViewData *file_view_data,
        ...
        p = XtVaCreateManagedWidget(
            ...
            XmNmappedWhenManaged, False, XmNx, file_view_data->x, XmNy, y,
            XmNwidth, file_mgr_data->grid_width, XmNheight,
            file_mgr_data->grid_height, NULL);
...
static Boolean InMultipleObjectRegion(FileMgrData *file_mgr_data,
        ...
        for (i = 0; i < file_mgr_data->selected_file_count; ++i)
                if (strcmp(
                        file_mgr_data->selection_list[i]->file_data->file_name,
...
void FmPopup(Widget w, XtPointer client_data, XEvent *event,
        ...
        if ((w == NULL) && (client_data == NULL)
            ...
            */
                ...
                for (i = 0; i < directory_set->file_count; i++) {
                        if (strcmp(directory_set->order_list[i]
                                       ->file_data->file_name,
                                   ".") == 0) {
                ...
                /* manage the white space buttons and unmanage the object
                 * buttons */
                ...
                if (file_mgr_data == trashFileMgrData) {
                        ...
                        if (PositioningEnabledInView(file_mgr_data)) {
                                ...
                                XtSetValues(fileMgrPopup.wsPopup[BTN_CLEANUP],
                                            args, 1);
                        } else {
                                file_mgr_rec->menuStates &= ~(CLEAN_UP);
                                XtUnmanageChild(
                                    fileMgrPopup.wsPopup[BTN_CLEANUP]);
                        }
                        XtSetValues(fileMgrPopup.wsPopup[BTN_SELECTALL], args,
                                    1);
                        ...
                        for (i = num_of_children;
                             i < action_pane->composite.num_children; i++)
                                XtUnmanageChild(
                                    action_pane->composite.children[i]);
                } else {
                        Dimension margin;
                        ...
                        if (file_mgr_data->toolbox)
                                XtUnmanageChild(
                                    fileMgrPopup.wsPopup[BTN_PROPERTIES]);
                        else
                                XtManageChild(
                                    fileMgrPopup.wsPopup[BTN_PROPERTIES]);
                        ...
                        XtGetValues(fileMgrPopup.wsPopup[BTN_SHOWHIDDEN], args,
                                    1);
                        XtSetArg(args[0], XmNmarginLeft, margin);
                        ...
                        /* ensure that there was fileViewData for the current
                         * directory */
                        if (fileViewData != NULL) {
                                /* attach the current fileViewData to the
                                 * 'Properties' button */
                                if (!file_mgr_data->toolbox) {
                                        XtRemoveAllCallbacks(
                                            fileMgrPopup
                                                .wsPopup[BTN_PROPERTIES],
                                            XmNactivateCallback);
                                        XtAddCallback(
                                            fileMgrPopup
                                                .wsPopup[BTN_PROPERTIES],
                                ...
                                UpdateActionMenuPane(
                                    (XtPointer)fileViewData, file_mgr_rec,
                                    fileViewData->file_data->logical_type,
        ...
        /* we are dealing with an object popup */
                ...
                if (file_mgr_data == trashFileMgrData) {
                        ...
                        if (file_mgr_data->selected_file_count > 1 &&
                            InMultipleObjectRegion(file_mgr_data,
                                                   fileViewData)) {

                                label_string = XmStringCreateLocalized(
                                    (GETMESSAGE(33, 1, "Multiple Objects")));
                        } else {
                                char *tmp_label;

                                if (fileViewData->file_data->action_name)
                                        tmp_label = fileViewData->file_data
                                                        ->action_name;
                                else
                                        tmp_label =
                        ...
                        /* trash popup--unmanage the non-trash buttons, manage
                         * the trash */
                        XtUnmanageChildren(fileMgrPopup.objPopup, obj_btns);
                        XtManageChildren(fileMgrPopup.trash_objPopup,
                                         TRASH_OBJ_BTNS);

                        /* adjust callbacks */
                        XtRemoveAllCallbacks(
                            fileMgrPopup.trash_objPopup[BTN_RESTORE],
                            XmNactivateCallback);
                        XtAddCallback(fileMgrPopup.trash_objPopup[BTN_RESTORE],
                                      XmNactivateCallback, Restore,
                                      (XtPointer)fileViewData);

                        XtRemoveAllCallbacks(
                            fileMgrPopup.trash_objPopup[BTN_REMOVE],
                            XmNactivateCallback);
                        ...
                        /* non-trash popup--manage the non-trash buttons,
                         * unmanage the trash */
                        XtManageChildren(fileMgrPopup.objPopup, obj_btns);
                        XtUnmanageChildren(fileMgrPopup.trash_objPopup,
                                           TRASH_OBJ_BTNS);
                        if (file_mgr_data->toolbox)
                                XtUnmanageChild(
                                    fileMgrPopup.objPopup[BTN_PROPERTIES]);

                        if (file_mgr_data->selected_file_count > 1 &&
                            InMultipleObjectRegion(file_mgr_data,
                                                   fileViewData)) {
                                /* we have many files selected; remove callbacks
                                 * from the  */
                                /* 'Properties', 'Put on Desktop', and 'Review'
                                 * buttons    */
                                /* and attach the selection list to the
                                 * remaining buttons; */
                                /* desensitize the 3 buttons listed above;
                                 * unmanage the    */
                                ...
                                if (!file_mgr_data->toolbox)
                                        XtRemoveAllCallbacks(
                                            fileMgrPopup
                                                .objPopup[BTN_PROPERTIES],
                                            XmNactivateCallback);

                                XtRemoveAllCallbacks(
                                    fileMgrPopup.objPopup[BTN_PUTON],
                                    XmNactivateCallback);
                                XtAddCallback(fileMgrPopup.objPopup[BTN_PUTON],
                                              XmNactivateCallback, PutOnDTCB,
                                              (XtPointer)fileViewData);

                                XtRemoveAllCallbacks(
                                    fileMgrPopup.objPopup[BTN_TRASH],
                                    XmNactivateCallback);
                                XtAddCallback(fileMgrPopup.objPopup[BTN_TRASH],
                                              XmNactivateCallback, TrashFiles,
                                              (XtPointer)fileViewData);

                                if (file_mgr_data->toolbox &&
                                    geteuid() != root_user &&
                                    access(file_mgr_data->current_directory,
                                           W_OK | X_OK) != 0)
                                        XtSetSensitive(
                                            fileMgrPopup.objPopup[BTN_TRASH],
                                            False);
                                ...
                                XtSetSensitive(fileMgrPopup.objPopup[BTN_HELP],
                                               False);
                                ...
                                XtUnmanageChild(fileMgrPopup.action_separator);
                                action_pane =
                                    (XmManagerWidget)fileMgrPopup.menu;
                                for (i = num_of_children;
                                     i < action_pane->composite.num_children;
                                     i++)
                                        XtUnmanageChild(
                                ...
                                /* we have one file selected; attach
                                 * fileViewData to buttons; */
                                /* sensitize any desensitized buttons; update
                                 * the actions     */
                                ...
                                XtRemoveAllCallbacks(
                                    fileMgrPopup.objPopup[BTN_HELP],
                                    XmNactivateCallback);
                                ...
                                XtSetSensitive(fileMgrPopup.objPopup[BTN_PUTON],
                                               True);
                                XtSetSensitive(fileMgrPopup.objPopup[BTN_HELP],
                                               True);
        ...
        if (event == NULL) {
                ...
                displayWidth =
                    DisplayWidth(XtDisplay(w), DefaultScreen(XtDisplay(w)));
                displayHeight =
                    DisplayHeight(XtDisplay(w), DefaultScreen(XtDisplay(w)));

                x = file_mgr_rec->shell->core.x +
                    file_mgr_rec->scroll_window->core.x + w->core.x +
                    gWidth / 2;
                ;
                ...
                y = file_mgr_rec->shell->core.y +
                    ...
                    file_mgr_rec->scroll_window->core.y + w->core.y +
                    gHeight / 2;
                ;
                ...
                if ((event->type == ButtonPress ||
                    ...
                    ((event->type == KeyPress || event->type == KeyRelease) &&
                     _XmIsEventUnique(event))) {
                        XmMenuPosition(fileMgrPopup.menu,
        ...
        if (event->type == KeyPress || event->type == KeyRelease) {
                ...
                /* This function is being called twice.
                   Record this event so the above check _XmIsEventUnique will
                   work
                */
...
static void DropOnRootCB(Widget w, XtPointer client_data, XtPointer call_data) {
        DtDndDropCallbackStruct *fileList =
        ...
        if (fileList->reason != DtCR_DND_DROP_ANIMATE) {
                /* set the complete move flag to False since it is not required
                 * to be called
                 */
                ...
                if (initiating_view != NULL) {
                        file_mgr_rec =
                            (FileMgrRec *)(((FileMgrData *)initiating_view)
                                               ->file_mgr_rec);
                ...
                if (numFiles > 1) {
                        /*
                         * We will place the icons by marching down a diagonal
                         * starting at the drop point.  But we don't want to
                         * march off-screen if there are too many icons.  So ...
                         *  - We choose a step width of 20, 10, 5, or 2
                         * depending on the number of icons dropped.
                         *  - When we run into the edge of the screen, we start
                         * a new diagonal shifted one step to the right.
                         *  - If all icons still won't fit, we move the starting
                         * point closer to the top left corner of the screen.
                         */

                        /* compute effective screen size (largest x,y where an
                         * icon can be placed without going off-screen) */
                        effScreenWidth =
                            WidthOfScreen(XtScreen(w)) - (4 + 4 + 48 + 4 + 5);
                        effScreenHeight = HeightOfScreen(XtScreen(w)) -
                                          (4 + 4 + 48 + 3 + 15 + 4 + 5);
                        ...
                        DPRINTF(("DropOnRootCB: numFiles %d, root_x %d, root_y "
                                 "%d, step %d\n",
                                 numFiles, root_x, root_y, step));

                        /* check if all icons will fit */
                        for (;;) {
                                /* cacluclate how many will fit (add up icons on
                                 * all diagonals) */
                                int n = 0, nd = 0;
                                for (root_x = start_x; root_x <= effScreenWidth;
                                     root_x += dstep) {
                                        int nx =
                                            (effScreenWidth - root_x) / step +
                                            1;
                                        int ny =
                                            (effScreenHeight - start_y) / step +
                                            1;
                                        n += (nx <= ny) ? nx : ny;
                                        nd++;
                                }
                                ...
                                /* move the start point closer to the top left
                                 * corner */
                                if (effScreenWidth - start_x <
                                        effScreenHeight - start_y &&
                                    start_x >= step) {
                                        /* move left so more icons will fit */
                                        start_x -= step;
                                } else if (effScreenWidth - start_x >
                                               effScreenHeight - start_y &&
                                           start_y >= step) {
                                        /* move up so one more icon will fit in
                                         * each diagonal */
                                        start_y -= step;
                ...
                if (file_mgr_data == NULL) {
                        ...
                        /* loop through the existing desktop icons to determine
                           which ones are being dragged, then change their
                           location */
                        for (i = 0; i < desktop_data->numIconsUsed; i++) {
                                if ((Widget)desktop_data->desktopWindows[i]
                                        ->iconGadget ==
                        ...
                        /* if the file manager data is the trash, we want to
                           tell the user that they can't drop trash on the
                           desktop
                        */
                        char *msg;
                        char *tmpStr;

                        DPRINTF(("DropOnRoot: Attempting to Drag Trash Object "
                                 "to Desktop\n"));
                        file_mgr_rec =
                            (FileMgrRec *)trashFileMgrData->file_mgr_rec;
                        tmpStr = (GETMESSAGE(
                            11, 37,
                            ...
                            "menu.\nYou can then drag the object out of File "
                            "Manager and drop it on\nthe Workspace backdrop."));
                        msg = XtNewString(tmpStr);
                        ...
                        */
                {
                        ...
                        for (i = 0; i < numFiles; i++) {
                                ...
                                if (strcmp(file_mgr_data->current_directory,
                                           file_set[i]) == 0) {
                                        SetupDesktopWindow(
                                            XtDisplay(file_mgr_rec->shell),
                                            file_mgr_data, file_mgr_rec, ".",
                                            host_set[i], file_set[i], root_x,
                                        ...
                                        *ptr = '\0';
                                        if (*(file_set[i]) == 0) {
                                                ...
                                                SetupDesktopWindow(
                                                    ...
                                                    file_mgr_data, file_mgr_rec,
                                                    ptr + 1, host_set[i],
                                                    file_set[i], root_x, root_y,
                                                    NULL, EndIndex);
                                        }
                                }
                                root_x += step;
                                root_y += step;
                                if (root_x > effScreenWidth ||
                                    root_y > effScreenHeight) {
                                        start_x += dstep;
...
void UnmanageFileIcons(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
        ...
        if (XtIsManaged(file_mgr_rec->file_window)) {
                ...
                if (child != NULL &&
                    XtParent(child) == file_mgr_rec->file_window)
...
static int WidgetCmp(Widget *w1, Widget *w2) { return *w1 - *w2; }
...
static void MakeReuseList(Widget *children, int num_children,
        ...
        *reuse_btns = (Widget *)XtMalloc((num_children + 1) * sizeof(Widget));
        ...
        if (num_children > 0) {
                ...
                qsort(sorted_chilren, num_children, sizeof(Widget),
                      (int (*)())WidgetCmp);

                /* create reuse flags; initially assume all children can be
                 * reused */
                ...
                for (i = 0; i < order_count; i++) {
                        if (order_list[i]->filtered &&
                            strcmp(order_list[i]->file_data->file_name, ".") !=
                                0) {
                        /* don't reuse this widget later */
                                ...
                                if (order_list[i]->treebtn)
                                        del_btn++;
#endif
                                order_list[i]->widget = order_list[i]->treebtn =
                                    NULL;
                        } else {
                                if (order_list[i]->widget) {
                                        p = bsearch(&order_list[i]->widget,
                                                    ...
                                                    (int (*)())WidgetCmp);
                                        if (p) {
                                                /* don't reuse this widget for
                                                 * any other file */
                                                reuse[p - sorted_chilren] =
                                                    False;
#ifdef DEBUG
                                                n_old++;
#endif
        ...
        DPRINTF(("MakeReuseList: count %d (%d new, %d old, %d filtered)\n",
                 order_count, order_count - n_filtered - n_old, n_old,
                 n_filtered));
        DPRINTF(("               reuse %d + %d, del %d + %d\n", icon_count,
                 btn_count, del_icon, del_btn));
}
...
static void UpdateOneIconLabel(FileMgrData *file_mgr_data,
        ...
        if (file_mgr_data->view != BY_ATTRIBUTES) {
                if (strcmp(file_view_data->file_data->file_name, "..") == 0) {
                        ...
                        /* label = action name */
                        label =
                            XtNewString(file_view_data->file_data->action_name);
                else
                        /* label = file name */
                        label =
                            XtNewString(file_view_data->file_data->file_name);
        } else /* file_mgr_data->view == BY_ATTRIBUTES */
        {
                ...
                if (strcmp(file_view_data->file_data->file_name, "..") == 0) {
                        s = GetSharedMessage(UP_ONE_LEVEL_LABEL);
                        label = (char *)XtRealloc(label, strlen(label) +
                                                             strlen(s) + 1);
                        strcat(label, s);
                }
        }
...
static void UpdateOneFileIcon(FileMgrRec *file_mgr_rec,
        ...
        XmManagerWidget file_window =
            (XmManagerWidget)file_mgr_rec->file_window;
        DirectorySet *directory_set =
            (DirectorySet *)file_view_data->directory_set;
        IconLayoutData *layout_data =
            (IconLayoutData *)file_mgr_data->layout_data;
        ...
        if (file_mgr_data->view == BY_NAME)
                ...
                pixmapData = CheckForOpenDirectory(
                    file_view_data, directory_set, file_mgr_data, logical_type);
        } else {
        ...
        if (layout_data->background == white_pixel) {
                ...
                if (file_view_data->selected) {
                        ...
                        XtSetArg(args[n], XmNforeground,
                                 layout_data->foreground);
                        n++;
                        XtSetArg(args[n], XmNpixmapBackground,
                                 layout_data->pixmap_back);
                        n++;
                        XtSetArg(args[n], XmNpixmapForeground,
                                 layout_data->pixmap_fore);
                        n++;
                }
        }
        ...
        if (icon_widget) {
                ...
                if (icon_widget != file_view_data->widget ||
                 ...
                 * the correct position once it is determined in
                 * LayoutFileIcons.
                 */
                ...
                if (layout_data->dup_icon_widget == NULL) {
                        ...
                        icon_widget = _DtDuplicateIcon(
                            (Widget)file_window, layout_data->dup_icon_widget,
                            icon_label,
                        ...
                        /*
                         * Move the gadget off the visible area; this avoids
                         * unnecessary redraw events at the old position when
                         * the gadget is moved to the correct position once it
                         * is determined in LayoutFileIcons.
                         */
                        icon_widget->core.x = -999;
                        icon_widget->core.y = -999;

                        /* make sure colors, drop operations, and clipping are
                         * right */
                        XtSetArg(args[i], XmNdropSiteOperations, XmDROP_NOOP);
                        i++;
                        XtSetArg(args[i], XmNmaxPixmapWidth,
                                 layout_data->pixmap_width);
                        i++;
                        XtSetArg(args[i], XmNmaxPixmapHeight,
        ...
        if (file_mgr_data->view != BY_NAME_AND_ICON ||
            file_mgr_data->show_type == MULTIPLE_DIRECTORY) {
                Dimension pixmap_width =
                    ((DtIconGadget)icon_widget)->icon.pixmap_width;

                if (pixmap_width < layout_data->pixmap_width) {
                        XtSetArg(args[0], XmNspacing,
                                 layout_data->spacing +
                                     layout_data->pixmap_width - pixmap_width);
                        XtSetValues(icon_widget, args, 1);
                }
        }
        ...
        if (file_mgr_data->show_type != MULTIPLE_DIRECTORY ||
            !file_view_data->file_data->is_subdir) {
                ...
                if (btn_widget) {
                        ...
                        btn_widget = XmCreatePushButtonGadget(
                            (Widget)file_window, "tree_button", args, n);
                }
...
static void _UpdateFileIcons(FileMgrRec *file_mgr_rec,
        ...
        if (XtIsManaged(file_mgr_rec->file_window)) {
                ...
                DPRINTF(("  focus_widget = %p (%s)\n",
                         file_mgr_rec->focus_widget,
                         file_mgr_rec->focus_widget
                             ? XtName(file_mgr_rec->focus_widget)
                             : "nil"));
        ...
        MakeReuseList(file_window->composite.children,
                      file_window->composite.num_children, order_list,
                      order_count, &layout_data->reuse_icons,
        ...
        layout_data->manage =
            (Widget *)XtMalloc(2 * order_count * sizeof(Widget));
...
static void CreateTreeIcons(Widget w) {
        ...
        for (i = 0; i < 3; i++) {
                ...
                for (j = 0; j < tpxN; j++) {
                        ...
                        TreePxTab[j].px[i] =
                            _DtGetPixmap(XtScreen(w), pxname, foreground_color,
                                         background_color);
                        width = height = 0;
                        XGetGeometry(
                            XtDisplay(w), TreePxTab[j].px[i],
...
Pixmap GetTreebtnPixmap(FileMgrData *file_mgr_data,
        ...
        if (file_view_data->ts == tsNotRead)
                ...
                pxid = (file_view_data->nfile == 0 ||
                        file_mgr_data->tree_files == TREE_FILES_NEVER)
                           ? tpxLess
                           : (file_view_data->ndir == 0) ? tpxMore : tpxBoth;
...
static void GetIconLayoutParms(FileMgrRec *file_mgr_rec,
        ...
        if (g) {
                ...
                if (g->icon.pixmap_width < maxWidth)
                        ld->spacing =
                            ld->spacing - (maxWidth - g->icon.pixmap_width);

                ld->width = ((Widget)g)->core.width + ld->pixmap_width -
                            g->icon.pixmap_width;
                ld->height = ((Widget)g)->core.height + ld->pixmap_height -
                             g->icon.pixmap_height;

                ld->char_width =
                    (g->icon.string_width) / strlen(file_list[i]->label);
...
static void EstimateIconSize(FileMgrRec *file_mgr_rec,
...
#ifdef MULTIBYTE
                label_len =
                    DtCharCount(file_view_data->label == NULL
        ...
        if (layout_data->pixmap_position == XmPIXMAP_TOP) {
                label_width = 2 * layout_data->margin +
                              label_len * layout_data->char_width;
                ...
                *width =
                    layout_data->width + (label_len)*layout_data->char_width;
...
void EraseTreeLines(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
        ...
        XClearArea(XtDisplay(file_window), XtWindow(file_window), x, y,
                   fw_width - x, fw_height - y, False);
...
void RedrawTreeLines(Widget w, int ex, int ey, int ewidth, int eheight,
        ...
        for (k = 0; k < order_count; k++) {
                ...
                extra_height =
                    GetExtraHeight(file_mgr_data, file_view_data, sz);
                GetLevel(file_view_data, &level);

                /* check if we need to go to the next column */
                tmp = (int)y + grid_height + extra_height +
                      YSPACING(file_mgr_data);
                if (tmp + MARGIN > MAXWINSIZE) {
                        /* window would exceed height limit; go to the next
                         * column */
                ...
                if (x <= ex + ewidth && x + TreeWd(level, sz) > ex &&
                    y0 <= ey + eheight && y1 > ey) {
                        GetAncestorInfo(file_mgr_data, file_view_data, NULL,
                                        NULL, more);
                        ...
                        if (level > 0) {
                                XDrawLine(
                                    XtDisplay(w), XtWindow(w), solid_gc, xl, y0,
                                    xl, more[level] ? y1 : y + grid_height / 2);

                                if (file_view_data->file_data->is_subdir ||
                                    !more[level]) {
                        ...
                        if (extra_height) {
                                ...
                                e_height =
                                    extra_height - YSPACING(file_mgr_data);

                                XDrawLine(XtDisplay(w), XtWindow(w), solid_gc,
                                          xl, y0, xl, y1 + e_height / 2);
                                ...
                                XCopyArea(XtDisplay(w),
                                          TreePxTab[tpxNil].px[sz], XtWindow(w),
                                          solid_gc, 0, 0, TreeNilWd[sz],
                                          TreeNilHt[sz], xl, y1);
                                /*
                                            if (empty_msg == NULL)
                                               empty_msg =
                                   XtNewString("(empty)");
                                            ...
                                            if (file_mgr_data->cd_fonttype ==
                                   XmFONT_IS_FONTSET)
                                            {
                                                extents =
                                   XExtentsOfFontSet(file_mgr_data->cd_fontset);
                                                font_yoffset =
                                   extents->max_logical_extent.y; font_height =
                                   extents->max_logical_extent.height;
                                            }
                                            else
                                            {
                                                font_yoffset =
                                   file_mgr_data->cd_font->ascent; font_height =
                                   file_mgr_data->cd_font->ascent +
                                                                file_mgr_data->cd_font->descent;
                                            }

                                            XDrawImageString(XtDisplay(w),
                                   XtWindow(w), solid_gc, xl + TreeNilWd[sz] +
                                   2, y1 + (TreeNilHt[sz] - font_height)/2 +
...
static int DisplaySomeIcons(FileMgrRec *file_mgr_rec,
        ...
        change =
            (FileViewData **)XtMalloc(order_count * sizeof(FileViewData *));
        ...
        for (; k < order_count && workCount1 < workLimit; k++) {
                /*
                 * Process focus widget before everything else so that we can
                 * restore the focus when we manage the file_window again.  If a
                 * file was being renamed, we do that one first.
                 */
                if (!layout_data->focus_done) {
                        /* search for focus widget in order_list and see if
                         * still displayed */
                        file_view_data = NULL;
                        ...
                        if (file_view_data) {
                                k--;
                                /* decrement loop index, so that the entry that
                                   was supposed to be processed here will be
                                   looked at again in the next iteration
                                */
                                goto do_this_entry;
                        }
                }
                ...
                /*
                 * If the file is not currenly displayed (collapsed tree branch)
                 * or is currenly scrolled out of view, we don't need to do
                 * anything at this time.  Except ...
                 */
                if (!file_view_data->displayed ||
                    ...
                    ((int)file_view_data->y - (int)grid_height) >= (ey + eht)) {
                        /*
                         * ... if this file still has an old icon widget, AND if
                         * the old icon position is visible, AND if the icon was
                         * registered as a drop site, we will have a "ghost drop
                         * zone" showing up where the icon used to be.  The old
                         * icon gadget is unmanaged and will remain unmanaged
                         * until later when we update it and move it to the
                         * correct positon (happens only when the user scrolls
                         * to the new position).  In the meantime, even though
                         * the gadget is unmanged, its old position still
                         * remains registered as a drop site, so we have to
                         * either unregister it or move the gadget to its new
                         * position now.
                         */
                        child = file_view_data->widget;
                        if (child != NULL &&
                            (Position)(child->core.x + child->core.width) >=
                                (Position)ex &&
                            child->core.x < ex + ewd &&
                            (Position)(child->core.y + child->core.height) >=
                                (Position)ey &&
                            child->core.y < ey + eht) {
                                if (!file_view_data->displayed) {
                                        XtGetValues(child, args_dso_get, 1);
                                        if (operations != XmDROP_NOOP) {
                                                XtSetValues(child, args_dso_set,
                                                            1);
                                                workCount1++;
                                                file_view_data->registered =
                                                    False;
                                        }
                                        continue;
                                }
        ...
        do_this_entry:
                 ...
                 */
                if (file_view_data->need_update) {
                        UpdateOneFileIcon(file_mgr_rec, file_mgr_data,
                                          file_view_data);
                        child = file_view_data->widget;

                        /*
                         * We may need to adjust the icon position based on the
                         * difference between estimated size and actual size.
                         */
                        if (layout_data->alignment == XmALIGNMENT_CENTER &&
                            file_mgr_data->view == BY_NAME_AND_ICON) {
                                EstimateIconSize(file_mgr_rec, file_mgr_data,
                                                 layout_data, file_view_data,
                        ...
                        if (PositioningEnabledInView(file_mgr_data)) {
                                ...
                                if (position_data->late_bind) {
                                        /*
                                         * When new objects are dropped on the
                                         * random placement window, they do not
                                         * yet have a widget, so the drop_y
                                         * could not be adjusted to take into
                                         * account the height of the icon; this
                                         * must be done now, after the object
                                         * has an icon.
                                         */
                                        /*
                                                       position_data->y -=
                                        ...
                                        */
                                        ...
                                        file_view_data->y = position_data->y +
                                                            child->core.height;
                                } else if (child->core.height != grid_height) {
                                        /* @@@ ??? @@@
                                         * Not quite right: have to distinguish
                                         * two cases: (1) position_data->y read
                                         * from .!xxx file file_view_data->y
                                         * computed by adding grid_height
                                         *  (2) file_view_data->y from layout
                                         *      position_data->y computed by
                                         * subtracting grid_height In case (1)
                                         * have to recompute file_view_data->y;
                                         * in case (2) have to recompute
                                         * position_data->y.  Code below only
                                         * works for case (2).
                                         */
                                        /*
                                                       position_data->y =
                ...
                /* if focus is supposed to be in the file window, focus on this
                 * widget */
                ...
                } else /* file_mgr_data->show_type == MULTIPLE_DIRECTORY */
                {
                        GetLevel(file_view_data, &level);
                        extra_height =
                            GetExtraHeight(file_mgr_data, file_view_data,
                        ...
                        if (file_view_data->treebtn) {
                                ...
                                x = file_view_data->x +
                                    TreeLX(level, layout_data->treebtn_size) +
                                    TreeOffset;
                        ...
                        if (child->core.height < grid_height) {
                                x = file_view_data->x +
                                    TreeWd(level, layout_data->treebtn_size);
                                y = file_view_data->y - grid_height +
                                    (Dimension)(grid_height -
                                                child->core.height) /
                                        (Dimension)2;
                ...
                /* make sure the icon gadget and tree button, if any, are
                 * managed */
                ...
                if (changed) {
                        ...
                        /*
                         * If the icon we just changed was being renamed, make
                         * sure the corresponding text widget is positioned
                         * correctly.
                         */
                                ...
                                for (i = 0;
                                     ...
                                     i++) {
                                        ...
                                        if (XmIsTextField(child)) {
                                                /* Check if the text field is
                                                 * still needed */
                                                if (!child->core
                                                         .being_destroyed) {
                                                        /* move to the correct
                                                         * position */
                                                        _DtIconGetTextExtent_r(
                                                            file_view_data
                                                                ->widget,
                                                            &textExtent);
                                                        x = textExtent.x;
        ...
        for (wp = layout_data->next_icon_to_use; (child = *wp) != NULL; wp++) {
                if ((Position)(child->core.x + child->core.width) >=
                        (Position)ex &&
                    child->core.x < ex + ewd &&
                    (Position)(child->core.y + child->core.height) >=
                        (Position)ey &&
...
static void CommitWorkProcUpdates(FileMgrRec *file_mgr_rec,
        ...
        DPRINTF(("CommitWorkProcUpdates: %d+%d updates (m %d, n %d, i %d:%d, "
                 "reset %d)\n",
                 g_workCount1, g_workCount2, layout_data->manage_count,
                 g_callCount, layout_data->i_do_next_vis,
                 layout_data->i_do_next_all, reset));
        ...
        if (layout_data->manage_count > 0) {
                ...
                XtManageChildren(layout_data->manage,
        ...
        if (!XtIsManaged((Widget)file_window)) {
                ...
                XtVaGetValues(file_mgr_rec->vertical_scroll_bar, XmNuserData,
                              &incr, NULL);
        ...
        if (file_mgr_rec->focus_widget) {
                /* see if the widget that previously had the focus was just
                 * managed */
                w = NULL;
                for (i = 0; i < layout_data->manage_count; i++) {
                        if (layout_data->manage[i] ==
                            file_mgr_rec->focus_widget) {
...
static Boolean DisplayWorkProc(XtPointer client_data) {
        ...
        if (layout_data->visible_done)
                ...
                n = DisplaySomeIcons(file_mgr_rec, file_mgr_data, ex, ey, ewd,
                                     eht, n1, False);
                if (n == 0) {
        ...
        if (layout_data->visible_done && n < n2) {
                n = DisplaySomeIcons(file_mgr_rec, file_mgr_data, 0, 0, 32767,
                                     32767, n2 - n, True);
...
void LayoutFileIcons(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
        ...
        unsigned char operations = 0L;
        static XtCallbackRec dropCB[] = {{DropOnFileWindow, NULL},
                                         {NULL, NULL}};
        ...
        if (!file_mgr_data->dropSite) {
                ...
                DtDndVaDropRegister(
                    (Widget)file_mgr_rec->file_window,
                    DtDND_FILENAME_TRANSFER | DtDND_BUFFER_TRANSFER, operations,
                    dropCB, DtNdropAnimateCallback, dropCB, DtNregisterChildren,
                    True, DtNtextIsBuffer, True, XmNanimationStyle,
                    XmDRAG_UNDER_SHADOW_IN, NULL);
                file_mgr_data->dropSite = True;

                if (!operations) {
                        XtSetArg(args[0], XmNdropSiteOperations, XmDROP_NOOP);
                        XmDropSiteUpdate((Widget)file_mgr_rec->file_window,
                                         args, 1);
                }
        } else {
                if (operations)
         ...
         * This gadget may not actually be displayed (depends of filter
         * settings; by default it is filtered out), but we need at least one
         * gadget created here so we can figure out fonts and margins used by
         * icon gadgets.  This is necessary to compute the grid spacing.
         */
        for (i = 0; i < order_count; i++)
        ...
        EstimateIconSize(file_mgr_rec, file_mgr_data, layout_data,
                         file_view_data, &grid_width, &grid_height);
        ...
        horiz_scrollbar_height =
            file_mgr_rec->horizontal_scroll_bar->core.height;
        ...
        if (file_mgr_data->show_type == MULTIPLE_DIRECTORY) {
                ...
                for (k = 0; k < order_count; k++) {
                        ...
                        extra_height =
                            GetExtraHeight(file_mgr_data, order_list[k],
                                           layout_data->treebtn_size);
                        tmp = (int)y + grid_height + extra_height +
                              YSPACING(file_mgr_data);
                        if (tmp + MARGIN > MAXWINSIZE) {
                                /* window would exceed height limit; start a new
                                 * column */
                                x += grid_width + XSPACING;
                                y = MARGIN;
        ...
        } else /* show_type == SINGLE_DIRECTORY, view != BY_ATTRIBUTES */
        {
                ...
                column_count =
                    ((int)scrolled_window_width - 4 - 2 * MARGIN + XSPACING) /
                 ...
                 */
                row_count =
                    (total_icon_count + column_count - 1) / column_count;
                ...
                if (tmp >= (int)scrolled_window_height - 2 * MARGIN) {
                        /* need to recompute everything because of space for
                         * vert scrollbar */
                        column_count =
                            ((int)scrolled_window_width - 4 - 2 * MARGIN +
                             XSPACING - (int)vert_scrollbar_width) /
                            ((int)grid_width + XSPACING);
                        if (column_count == 0)
                                column_count = 1;
                        row_count = (total_icon_count + column_count - 1) /
                                    column_count;
                        tmp = 2 * MARGIN - YSPACING(file_mgr_data) +
                              row_count *
                                  ((int)grid_height + YSPACING(file_mgr_data));
                }
                ...
                if (tmp <= MAXWINSIZE)
                        ...
                        overflow = True;
                        row_count =
                            (int)(MAXWINSIZE - 2 * MARGIN +
                                  YSPACING(file_mgr_data)) /
                            (int)((int)grid_height + YSPACING(file_mgr_data));
                        column_count =
                            (total_icon_count + row_count - 1) / row_count;
                        row_count = (total_icon_count + column_count - 1) /
                                    column_count;
                        file_window_height =
                            (Dimension)(2 * MARGIN - YSPACING(file_mgr_data) +
                                        row_count * ((int)grid_height +
                                                     YSPACING(file_mgr_data)));
                }

                DPRINTF(("  %d columns, %d rows (scroll_window width %d)\n",
                         column_count, row_count, scrolled_window_width));
                ...
                for (i = 0; i < row_count; i++) {
                        ...
                        for (j = 0; j < column_count && k < order_count;) {
                                ...
                                while (k < order_count &&
                                       !order_list[k]->displayed)
                                        k++;
                                if (k == order_count)
                                        break;

                                if (layout_data->alignment ==
                                        XmALIGNMENT_CENTER &&
                                    file_mgr_data->view == BY_NAME_AND_ICON) {
                                        EstimateIconSize(
                                            file_mgr_rec, file_mgr_data,
                                            layout_data, order_list[k],
                                            &icon_width, &icon_height);
                                        order_list[k]->x =
                                            x + (Dimension)(grid_width -
                                                            icon_width) /
                                                    (Dimension)2;
                                } else
                                        order_list[k]->x = x;
        ...
        if (VerticalScrollbarIsVisible(file_mgr_rec->vertical_scroll_bar,
                ...
                if (file_window_width <=
                    (Dimension)(scrolled_window_width -
                                (vert_scrollbar_width + 4)))

                        file_window_width =
                            scrolled_window_width - (vert_scrollbar_width + 4);
        } else {
                if (file_window_width < (Dimension)(scrolled_window_width - 4))
        ...
        if (HorizontalScrollbarIsVisible(file_mgr_rec->horizontal_scroll_bar,
                ...
                XtSetArg(args[0], XmNhighlightThickness,
                         &sb_highlight_thickness);
                XtGetValues((Widget)file_mgr_rec->horizontal_scroll_bar, args,
                            1);
                ...
                pad =
                    (int)(((sb_highlight_thickness + sw_shadow_thickness) * 2) +
                          space);

                if (file_window_height <
                    (Dimension)(scrolled_window_height -
                                (horiz_scrollbar_height + pad)))
                        file_window_height = scrolled_window_height -
                                             (horiz_scrollbar_height + pad);
        } else {
                if (file_window_height <
                    (Dimension)(scrolled_window_height - 4))
        ...
        if (file_mgr_data->scrollToThisFile == NULL) {
                ...
                XtSetArg(args[0], XmNincrement,
                         grid_height + YSPACING(file_mgr_data));
                XtSetArg(args[1], XmNuserData,
                         grid_height + YSPACING(file_mgr_data));
        ...
        /*
         * Don't keep up the hourglass; this helps the user to get the
         * impression that all of the work is done.
         */
...
layout_done:
        ...
        if (layout_data->work_id == 0) {
                ...
                layout_data->work_id = XtAppAddWorkProc(
                    app_context, DisplayWorkProc, file_mgr_rec);
        }
        ...
        if (file_mgr_data->scrollToThisFile) {
                ...
                for (i = 0; i < file_mgr_data->directory_count; ++i) {
                        if (strcmp(((DirectorySet *)
                                        file_mgr_data->directory_set[i])
                                       ->name,
                ...
                if (directory_set) {
                        for (i = 0; i < directory_set->file_count; ++i) {
                                if (strcmp(directory_set->order_list[i]
                                               ->file_data->file_name,
                                           file_mgr_data->scrollToThisFile) ==
                                    0) {
                                        file_view_data =
...

{
        ...
        gc = XCreateGC(XtDisplay(file_window), XtWindow(file_window), mask,
                       &values);
...

{
        ...
        for (i = 0; i < order_count; i++) {
                object = order_list[i];
                if (object->displayed) {
                        if (position_data = GetPositionalData(
                                file_mgr_data, object, 0, False)) {
                                /* Save; used later during redraw */
                                position_data->file_view_data = object;
                                object->position_info =
                                    (ObjectPtr)position_data;
                        }
                } else if (position_data = GetPositionalData(
                               file_mgr_data, object, 0, False)) {
                        /*
                         * If an object has position information, but is
                         * currently filtered, don't discard its position info;
                         * mark it as in-use.
                         */
                        position_data->file_view_data = object;
        ...
        for (i = 0; i < file_mgr_data->num_objects;) {
                if (!file_mgr_data->object_positions[i]->in_use) {
                        ...
                        for (k = 0; k < file_window->composite.num_children;
                             k++) {
                                if (XmIsTextField(
                                        file_window->composite.children[k]) &&
                                    !file_window->composite.children[k]
        ...
        for (i = 0; i < order_count; i++) {
                object = order_list[i];
                if (object->displayed) {
                        position_data = GetPositionalData(
                            file_mgr_data, object, file_window_width, True);

                        /* Save; used later during redraw */
                        position_data->file_view_data = object;
                        object->position_info = (ObjectPtr)position_data;

                        /* record position of bottom left corner for
                         * DisplaySomeIcons */
                        object->x = position_data->x;
                        object->y =
                            position_data->y +
                            (object->need_update ? grid_height
                                                 : object->widget->core.height);
                }
        }
        ...
        while (bottom) {
                if (bottom->file_view_data != NULL &&
                    bottom->file_view_data->displayed) {
                        if (bottom->file_view_data->position_info->x >
                            largest_x)
                                largest_x =
                                    bottom->file_view_data->position_info->x;
                        if (bottom->file_view_data->position_info->y >
                            largest_y)
                                largest_y =
                                    bottom->file_view_data->position_info->y;
                }
                bottom = (ObjectPosition *)bottom->prev;
        }
        ...
        if ((Dimension)(current_wd) <
            (Dimension)(file_mgr_rec->scroll_window->core.width - 4)) {
                if (VerticalScrollbarIsVisible(
                        ...
                        current_wd =
                            file_mgr_rec->scroll_window->core.width - 4;
                set_size = True;
        }
...
static ObjectPosition *GetPositionalData(FileMgrData *file_mgr_data,
        ...
        if (file_mgr_data->num_objects == 1) {
                ...
                for (i = 0; i < file_mgr_data->num_objects; i++) {
                        ...
                        if (file_mgr_data->object_positions[i]
                                ->stacking_order == 1) {
                                top = file_mgr_data->object_positions[i];
                                continue;
                        } else if (file_mgr_data->object_positions[i]
                                       ->stacking_order == 2) {
                                if (top == NULL)
                                        top = GetTopOfStack(file_mgr_data);
                                top->next = (ObjectPtr)entry;
                                entry->next =
                                    (ObjectPtr)
                                        file_mgr_data->object_positions[i];
                                entry->prev = (ObjectPtr)top;
        ...
        do {
                ...
                for (i = 0; i < file_mgr_data->num_objects; i++) {
                        ...
                        if (oP->file_view_data->widget &&
                            !oP->file_view_data->need_update) {
                                oWidth = oP->file_view_data->widget->core.width;
                                oHeight =
                                    oP->file_view_data->widget->core.height;
                        } else {
                                oWidth = grid_width;
                                oHeight = grid_height;
                        }
                        if (oP->file_view_data->displayed &&
                            IntersectRects(x, y, objWidth, objHeight, oP->x,
                                           oP->y, oWidth, oHeight)) {
                                /* Try next grid spot */
                                x += grid_width + XSPACING;
...
static void BuildObjectPositions(FileMgrData *file_mgr_data) {
        ...
        for (i = 0, j = 0; i < file_mgr_data->directory_set[0]->file_count;
             i++) {
                ...
                for (k = 0; k < j; k++) {
                        if ((ptr->y < file_mgr_data->object_positions[k]->y) ||
                            ((ptr->y ==
                              file_mgr_data->object_positions[k]->y) &&
                             (ptr->x <
                              file_mgr_data->object_positions[k]->x))) {
                                /* Shift others down, to open up a spot */
                                for (l = j; l > k; l--) {
                                        file_mgr_data->object_positions[l] =
                                            file_mgr_data
                                                ->object_positions[l - 1];
                                }
                                break;
                        }
                }
...

{
         ...
         * margins and shadow area, but not the highlight area of the icon
         * gadget that was dragged.  Coordinates drop_x, drop_y are the upper
         * left corner of the drag icon when it was dropped.  To convert to icon
         * gadget coordinates we need to subtract the highlight thickness.
         */
        ...
        for (i = file_count - 1; i >= 0; i--) {
                ...
                for (j = 0; j < file_mgr_data->num_objects; j++) {
                        if (strcmp(name,
                                   file_mgr_data->object_positions[j]->name) ==
                            0) {
                                /* Find the associated widget */
                                for (k = 0; strcmp(
                                         name, file_mgr_data->directory_set[0]
                                                   ->file_view_data[k]
                                                   ->file_data->file_name);
                                     k++)
                                        ;
                                ...
                                /* If this object has a text field, then move it
                                 * also */
                                for (l = 0;
                                     l < file_window->composite.num_children;
                                     l++) {
                                        if (XmIsTextField(file_window->composite
                                                              .children[l]) &&
                                            !file_window->composite.children[l]
                                                 ->core.being_destroyed) {
                                                XtSetArg(args[0], XmNuserData,
                                                         &edit_name);
                                                XtGetValues(
                ...
                if (newEntry) {
                        ...
                        /* Create positional data at the top of the ordered
                         * array */
                        newEntryCount++;
                        j = 0;
                        data =
                        ...
                        for (i2 = 0; i2 < file_mgr_data->num_objects; i2++) {
                                /* Find the previous top of the stack */
                                if (file_mgr_data->object_positions[i2]
                                        ->stacking_order == 1) {
                                        data->next = (ObjectPtr)file_mgr_data
                                                         ->object_positions[i2];
                                        file_mgr_data->object_positions[i2]
                                            ->prev = (ObjectPtr)data;
                                }
                                file_mgr_data->object_positions[i2]
                                    ->stacking_order++;
                        ...
                        RepositionUpInStack(
                            file_mgr_data,
                            file_mgr_data->object_positions[j]->stacking_order,
                            1);
                }
                ...
                if (!newEntry) {
                        ...
                        XtSetArg(args[0], XmNx, x);
                        XtSetArg(args[1], XmNy, y);
                        XtSetValues(file_mgr_data->directory_set[0]
                                        ->file_view_data[k]
                                        ->widget,
                                    args, 2);
                 ...
                 * Bubble down toward the end of the list if the object has
                 * moved farther from the origin; bubble up if it has moved
                ...
                if ((y > orig_y) || ((y == orig_y) && (x > orig_x))) {
                        ...
                        /* See if it goes at the end */
                        if (k < 0)
                                file_mgr_data->object_positions[0] =
                                    save_object;
                }
         ...
         * objects were specified, then we need to register hotspots now,
         * because the layout function will never be called because the
         * directory never really changed.  This situation can occur when an
         * icon is dropped on the desktop from a regular dtfile view, but that
         * icon is already on the desktop (it thus is just a reposition).
         */
        ...
        while (top) {
                ...
                if (file_view_data != NULL && file_view_data->displayed &&
                    !file_view_data->need_update) {
                        ...
                        if ((Position)(icon->core.x + icon->core.width) >=
                                (Position)ex &&
                            icon->core.x < ex + ewd &&
                            (Position)(icon->core.y + icon->core.height) >=
                                (Position)ey &&
                            icon->core.y < ey + eht) {
