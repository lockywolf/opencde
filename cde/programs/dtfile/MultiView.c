...
Widget _DtCreateMenuSystem(Widget parent, char *menuBarName,
                           ...
                           int numMenuComponents, void (*getMenuDataProc)(),
        ...
        for (i = 0; i < numMenuComponents; i++) {
                ...
                case MENU_PANE: {
                        if (createSharedComponents ||
                            (menuDesc[i].type == MENU_PANE)) {
                                child = XmCreatePulldownMenu(
                                    menubar, menuDesc[i].name, NULL, 0);

                                if (firstMenuPane) {
                                        XtAddCallback(
                                            XtParent(child), XmNpopupCallback,
                                            setMenuSensitivity,
                                            (XtPointer)getMenuDataProc);
                                        XtAddCallback(
                                            XtParent(child), XmNpopdownCallback,
                ...
                case MENU_SEPARATOR: {
                        /* Need to always make components for non-shared
                         * menupanes */
                        if ((createSharedComponents) ||
                            (lastMenuPaneType == MENU_PANE)) {
                                child = XmCreateSeparatorGadget(
                                    lastMenuPane, menuDesc[i].name, NULL, 0);
                                XtManageChild(child);
                                menuDesc[i].widget = child;
                        }
                        break;
                        ...
                        if ((menuDesc[i].maskBit) &&
                            ((*globalMenuStates & menuDesc[i].maskBit) !=
                             (*viewMenuStates & menuDesc[i].maskBit))) {
                                _DtGlobalUpdateMenuItemState(
                                    menuDesc[i].widget, menuDesc[i].maskBit,
                                    *viewMenuStates & menuDesc[i].maskBit,
                                    globalMenuStates);
                        }
                        break;
        ...
        if (on != 0) {
                ...
                *globalMenuStates |= mask;
                ...
                if (btn != *findBtn && btn != *change_directoryBtn &&
                    btn != *preferencesBtn && btn != *defaultEnvBtn &&
                    btn != *filterBtn) {
...
static Widget CreateMenuBtn(Widget parent, Boolean isPushButton,
        ...
        if (isPushButton)
                child =
                    XmCreatePushButtonGadget(parent, menuDesc->name, args, n);
        else {
                XtSetArg(args[n], XmNvisibleWhenOff, TRUE);
                n++;
                child =
                    XmCreateToggleButtonGadget(parent, menuDesc->name, args, n);
        }
