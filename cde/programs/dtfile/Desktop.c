...
void InitializeDesktopWindows(int number_cache, Display *display) {
        ...
        if (desktop_data == NULL) {
                ...
                desktop_data->desktopWindows = (DesktopRec **)XtMalloc(
                    sizeof(DesktopRec *) * number_cache);

                desktop_data->popupMenu =
                    (PopupRec *)XtMalloc(sizeof(PopupRec));
                ...
                /*
                 *  Build desktop directories and get the number of workspaces
                 * for this screen.
                 */
                ...
                desktop_data->desktopWindows = (DesktopRec **)XtRealloc(
                    (char *)desktop_data->desktopWindows,
        ...
        for (i = desktop_data->numCachedIcons + desktop_data->numIconsUsed;
             i < new_size; i++) {
                ...
                /* We need to save the background color for the popup shell.
                 * When we activate a text edit widget for the label of a
                 * workspace object, the shell's background_pixel color will
                 * change.  If we don't save the old background color and reuse
                 * the icon gadget, the background will be different
                 */
                ...
                n++;
                desktop->frame = frame =
                    XmCreateFrame(pu_shell, "frame", args, n);
                XtManageChild(frame);
                ...
                n++;
                desktop->drawA = drawA =
                    XmCreateDrawingArea(frame, "drawA", args, n);
                XtManageChild(drawA);
                ...
                if (desktop_data->popupMenu->popup == NULL) {
                        ...
                        XmAddToPostFromList(desktop_data->popupMenu->popup,
                                            drawA);
                ...
                n++;
                desktop->iconGadget =
                    _DtCreateIcon(drawA, "desktop_icon", args, n);
...
static FileViewData *BuildNewOrderlist(FileViewData *order_list, Widget icon,
        ...
        ((DirectorySet *)file_view_data->directory_set)->filtered_file_count =
            0;
        ((DirectorySet *)file_view_data->directory_set)->invisible_file_count =
            0;
        ((DirectorySet *)file_view_data->directory_set)->file_mgr_data = NULL;

        file_view_data->parent = file_view_data->next = file_view_data->desc =
            NULL;
        ...
        if (order_list != NULL) {
                ...
                file_data->file_name =
                    XtNewString(order_list->file_data->file_name);
                file_data->action_name =
                    XtNewString(order_list->file_data->action_name);
                ...
                ((DirectorySet *)file_view_data->directory_set)->name =
                    XtNewString(
                        ((DirectorySet *)order_list->directory_set)->name);
        } else {
                Tt_status tt_status;

                full_dir_name = ResolveLocalPathName(
                    hostName, dirName, NULL, home_host_name, &tt_status);
                if (TT_OK == tt_status) {
                        ...
                        if (strcmp(file, ".") == 0) {
                                ReadFileData2(&file_data2, full_dir_name, NULL,
                                              IsToolBox);
                        } else {
                                ReadFileData2(&file_data2, full_dir_name, file,
                                              IsToolBox);
                        }
        ...
        for (j = 0; j < desktop_data->numWorkspaces; ++j) {
                ...
                for (k = 0; k < workSpace->files_selected; ++k) {
                        if (workSpace->selectedDTWindows[k] == desktopWindow) {
                                for (l = k; l < workSpace->files_selected;
                                     ++l) {
...
void DesktopObjectChanged(DesktopRec *desktopWindow) {
        ...
        pixmapData =
            _DtRetrievePixmapData(file_data->logical_type, file_data->file_name,
                                  desktopWindow->dir_linked_to,
                                  desktopWindow->shell, desktopIconType);
...
static void MakeDesktopWindow(DesktopRec *desktopWindow, int rX, int rY) {
        ...
        pixmapData = _DtRetrievePixmapData(
            ...
            desktopWindow->dir_linked_to, desktopWindow->shell,
            desktopIconType);
        if (pixmapData)
        ...
        width =
            gadgetWidth + (Dimension)(2 * gadgetX) + (Dimension)(2 * drawAX);
        height =
            gadgetHeight + (Dimension)(2 * gadgetY) + (Dimension)(2 * drawAY);

        if (rX == -1) {
                CalculateRootCoordinates(desktopWindow->workspace_num, width,
                                         height, &rX, &rY);
        }
...
void SaveDesktopInfo(int session) {
        ...
        case NORMAL_RESTORE:
                ...
                fileName = ResolveLocalPathName(home_host_name, desktop_dir,
        ...
        for (i = 0; i < desktop_data->numIconsUsed; i++) {
                ...
                fprintf(fptr, "%s\n%s\n%s\n%s\n%s\n%s\n%d %d %d %d %d %d %d\n",
                        ...
                        desktopWindow->toolbox, desktopWindow->view,
                        desktopWindow->order, desktopWindow->direction,
                        desktopWindow->positionEnabled, desktopWindow->root_x,
                        desktopWindow->root_y);

                /* if not normal restore (ie save current or home) remove the
                 * entry */
                if (session == CURRENT_DIR_RESTORE) {
                        ...
                        XtRemoveAllCallbacks(
                            desktop_data->desktopWindows[i]->iconGadget,
                            XmNcallback);
                        ...
                        XtFree(desktop_data->desktopWindows[i]
                                   ->restricted_directory);
                        desktop_data->desktopWindows[i]->restricted_directory =
                            NULL;
                        desktop_data->desktopWindows[i]->restricted_directory =
                            NULL;
                        ...
                        desktop_data->desktopWindows[i]->direction =
                            UNSET_VALUE;
                        desktop_data->desktopWindows[i]->positionEnabled =
                            UNSET_VALUE;
...
void LoadDesktopInfo(char *session) {
        ...
        if (numWindows >= desktop_data->numCachedIcons)
                InitializeDesktopWindows(
                    numWindows + 5 - desktop_data->numCachedIcons, NULL);
        ...
        for (i = count; i < numWindows + count; i++) {
                ...
                sscanf(message, "%d %d %d %d %d %d %d\n", &toolbox, &view,
                       &order, &direction, &positionEnabled, &rX, &rY);
                ...
                for (j = 0; j < desktop_data->numWorkspaces; j++) {
                        if (strcmp(workSpace,
                                   desktop_data->workspaceData[j]->name) == 0) {
                                haveOne = True;
                ...
                if (!haveOne) {
                        ...
                        desktopWindow->workspace_name = (char *)XtMalloc(
                            strlen(desktop_data->workspaceData[0]->name) + 1);
                ...
                desktopWindow->registered =
                    desktopWindow->file_view_data->registered;
...
static void FreeDesktopWindow(DesktopRec *desktopWindow) {
        ...
        RegisterInGrid((int)width, (int)height, desktopWindow->root_x,
                       desktopWindow->root_y, desktopWindow->workspace_num,
                       False);
        ...
        if (desktop_data->numCachedIcons > MAX_CACHED_ICONS)
                FreeCachedIcons(desktop_data->numCachedIcons -
                                MAX_CACHED_ICONS);
...
static void DrawAInput(Widget w, XtPointer client_data, XEvent *event,
        ...
        for (i = 0; i < desktop_data->numWorkspaces; i++) {
                if (desktop_data->workspaceData[i]->number ==
                    desktopWindow->workspace_num)
                        break;
        }
        ...
        if (num_of_files > 1) {
                ...
                XtSetArg(args[0], XmNuserData,
                         (XtPointer)desktop_data->workspaceData[i]
                             ->selectedDTWindows);
                XtSetValues(desktop_data->popupMenu->popup, args, 1);

                XtRemoveAllCallbacks(desktop_data->popupMenu->removeDT,
                                     XmNactivateCallback);
                XtAddCallback(desktop_data->popupMenu->removeDT,
                              XmNactivateCallback, RemoveDT,
                              (XtPointer)desktop_data->workspaceData[i]
                                  ->selectedDTWindows[0]);
                XtRemoveAllCallbacks(desktop_data->popupMenu->openFolder,
                                     XmNactivateCallback);
                XtAddCallback(desktop_data->popupMenu->openFolder,
                              XmNactivateCallback, OpenFolder, (XtPointer)NULL);

                XtRemoveAllCallbacks(desktop_data->popupMenu->rename,
                                     XmNactivateCallback);
                XtAddCallback(desktop_data->popupMenu->rename,
                              XmNactivateCallback, renameDT, (XtPointer)NULL);
                ...
                if (!desktop_data->popupMenu || !desktopWindow->file_view_data)
                        return;
                /* we have zero or one files selected, so just put up the popup
                   for the one */
                ...
                XtRemoveAllCallbacks(desktop_data->popupMenu->openFolder,
                                     XmNactivateCallback);
                XtAddCallback(desktop_data->popupMenu->openFolder,
                              XmNactivateCallback, OpenFolder,
                              (XtPointer)desktopWindow);
                ...
                if (desktopWindow->toolbox == False) { /* Sensitize rename
                        ...
                        XtAddCallback(desktop_data->popupMenu->rename,
                                      XmNactivateCallback, renameDT,
                                      (XtPointer)desktopWindow);
                } else { /* Don't allow user to rename the objects in the tool
                            box. These things are action labels.
                         */
                        XtSetSensitive(desktop_data->popupMenu->rename, False);
                }
                    ...
                    desktopWindow->file_name) {
                        ...
                        if (ptr != NULL) {
                                ...
                                if (strcmp(root_title, ptr) ==
                                    0) { /* The desktop object is a ROOT
                                            directory */
                                        XtSetSensitive(
                                            desktop_data->popupMenu->openFolder,
                                            False);
                                        XtSetSensitive(
                                            desktop_data->popupMenu->rename,
                                            False);
                                        XtRemoveAllCallbacks(
                                            desktop_data->popupMenu->rename,
                                            XmNactivateCallback);
        ...
        if (event == NULL) {
                ...
                if (beventx == -1)
                        beventx =
                            desktopWindow->root_x + g->icon.pixmap_width / 2;
...
void RemoveDT(w, client_data, call_data) Widget w;
...
XtPointer call_data;
{
        ...
        for (j = 0; j < num_of_files; j++) {
                if (selected)
                        desktopWindow = desktop_data->workspaceData[i]
                                            ->selectedDTWindows[j];

                XtPopdown(desktopWindow->shell);
                XWithdrawWindow(
                    XtDisplay(desktopWindow->shell),
                    XtWindow(desktopWindow->shell),
                    XDefaultScreen(XtDisplay(desktopWindow->shell)));
...
#ifdef SUN_PERF
void RemoveMovedObjectFromDT(w, client_data, file_cnt, file_list) Widget w;
XtPointer client_data;
int file_cnt;
char **file_list;
{
...
static void OpenFolder(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (desktopWindow->toolbox) {
                ...
                if (action_args) {
                        ...
                        action_args[0].u.file.name =
                            desktopWindow->dir_linked_to;
                }

                pwd_dir = XtNewString(desktopWindow->dir_linked_to);
                DtActionInvoke(desktopWindow->shell, "OpenParentAppGroup",
                               action_args, 1, NULL, NULL, pwd_dir, True, NULL,
                               NULL);
                XtFree(pwd_dir);
                pwd_dir = NULL;
        } else {
                ...
                if (desktopWindow->restricted_directory != NULL) {
                        ...
                        special_restricted =
                            XtNewString(desktopWindow->restricted_directory);
                        if (desktopWindow->title == NULL)
                                special_title = NULL;
                        else
                                special_title =
                                    XtNewString(desktopWindow->title);
                        special_helpVol = XtNewString(desktopWindow->helpVol);
                        if (desktopWindow->toolbox)
                                dialog_data = GetNewView(
                                    desktopWindow->host,
                                    desktopWindow->dir_linked_to,
                                    desktopWindow->restricted_directory, NULL,
                                    0);
                ...
                if (!directory_data->file_count)
                        file_mgr_data->desktop_file =
                            XtNewString(desktopWindow->file_name);
                else
                ...
                if (strcmp(workspace_name,
                        ...
                        desktop_data->workspaceData[i]->selectedDTWindows =
                            (DesktopRec **)XtRealloc(
                                (char *)desktop_data->workspaceData[i]
                                    ->selectedDTWindows,
                                sizeof(DesktopRec *) *
                                    (desktop_data->workspaceData[i]
                                         ->files_selected +
                        ...
                        for (j = desktop_data->workspaceData[i]->files_selected;
                             j > 0; j--) {
                                desktop_data->workspaceData[i]
                                    ->selectedDTWindows[j] =
                                    desktop_data->workspaceData[i]
                                        ->selectedDTWindows[j - 1];
                        }
                        desktop_data->workspaceData[i]->selectedDTWindows[0] =
                            desktopWindow;
                        break;
                }
        }
...
void DeselectAllDTFiles(WorkspaceRec *workspaceData) {
        ...
        for (i = 0; i < selection_count; i++) {
                SetToNormalColors(
                    workspaceData->selectedDTWindows[i]->iconGadget,
...

{
        if ((strcmp(openNewView, command) == 0) ||
            (strcmp(openInPlace, command) == 0)) {
                ...
                /* this statement applies to the case where a user traverses
                 * down the * part of the directory tree containing the
                 * application manager      * directories or the trash directory
                 */
                if ((strcmp(ltype, LT_AGROUP) == 0) ||
                    (strstr(ltype, LT_AGROUP_SUBDIR)) ||
                    (strcmp(ltype, LT_TRASH) == 0)) {
                        ProcessAction(command, desktopWindow->file_view_data,
                                      drop_parameters, desktopWindow->host,
                                      desktopWindow->dir_linked_to,
                                      desktopWindow->restricted_directory,
                                      desktopWindow->shell);
                } else {
                ...
                if (drop_parameters->dropData->protocol ==
                    DtDND_FILENAME_TRANSFER)
                        ProcessMoveCopyLinkDT(command, desktopWindow,
                                              drop_parameters);
                else
                        ProcessBufferDropOnFolderDT(command, desktopWindow,
                                                    drop_parameters);
        }

        else {
                ProcessAction(
                    command, desktopWindow->file_view_data, drop_parameters,
...
static void ProcessNewViewDT(DesktopRec *desktopWindow) {
        ...
        if (desktopWindow->restricted_directory == NULL) {
                ...
                if (desktopWindow->toolbox)
                        ...
                        GetNewView(desktopWindow->host, directory_name, NULL,
                                   NULL, 0);
        }
}
                ...
                for (i = 0; i < numFiles; i++) {
                        if (strcmp(dir_name, file_set[i]) == 0 &&
                            strcmp(desktopWindow->host, host_set[i]) == 0) {
                                ...
                                FileOperationError(file_view_data->widget, msg,
                                                   dir_name);
                                XtFree(msg);
                        }
                }
...
ProcessBufferDropOnFolderDT(char *command, DesktopRec *desktopWindow,
        ...
        buffer_set =
            (BufferInfo *)XtMalloc(sizeof(BufferInfo) * num_of_buffers);

        _DtSetDroppedBufferInfo(file_set, buffer_set, host_set,
                                drop_parameters);
        ...
        MakeFilesFromBuffersDT(file_view_data, directory, file_set, host_set,
                               buffer_set, num_of_buffers, desktopWindow, NULL,
                               NULL);

        /******************************/
        /* free file_set + buffer_set */
        /******************************/
        _DtFreeDroppedBufferInfo(file_set, buffer_set, host_set,
                                 num_of_buffers);
}
...
char *IsAFileOnDesktop2(char **file_set, int file_count, int *number,
        ...
        *number = 0;
        for (i = 0; i < file_count; i++) {
                /* Let's check to see if file coming in is a link, if it is
                 * let's test it instead of where it is linked to.
                 */
                ...
                for (j = 0; j < desktop_data->numIconsUsed; j++) {
                        ...
                        */
                        ...
                        if (Link)
                                ...
                                tt_status = tt_message_file_set(dummy_msg,
                                                                tmp_filename);
                                filename2 = tt_message_file(dummy_msg);
                        }
                        filename2 = (char *)XtRealloc(
                            filename2, strlen(filename2) +
                                           strlen(desktopWindow->file_view_data
                                                      ->file_data->file_name) +
                                           2);
                        strcat(filename2, "/");
                        strcat(filename2, desktopWindow->file_view_data
                                              ->file_data->file_name);
...
void ProcessDTSelection(DesktopRec *desktopRec, XButtonEvent *event) {
        ...
        if (DtWsmGetCurrentWorkspace(XtDisplay(desktopRec->shell), rootWindow,
                                     &pCurrent) == Success) {
                workspace_name =
                    XGetAtomName(XtDisplay(desktopRec->shell), pCurrent);
                CleanUpWSName(workspace_name);
        } else
                workspace_name =
                    XtNewString(desktop_data->workspaceData[0]->name);
        ...
        for (i = 0; i < desktop_data->numWorkspaces; i++) {
                if (strcmp(workspace_name,
                        ...
                        if (toggle_select == False)
                                ...
                                for (j = 0; j < desktop_data->workspaceData[i]
                                                    ->files_selected;
                                     j++) {
                                        if (desktop_data->workspaceData[i]
                                                ->selectedDTWindows[j] ==
                                            desktopRec) {
                                                DeselectDTFile(
                                                    desktop_data
...
Boolean DTFileIsSelected(DesktopRec *desktopRec, FileViewData *fileViewData) {
        ...
        for (i = 0; i < desktop_data->numWorkspaces; i++) {
                if (strcmp(workspace_name,
                           desktop_data->workspaceData[i]->name) == 0) {
                        for (j = 0;
                             j < desktop_data->workspaceData[i]->files_selected;
                             j++) {
                                if (desktop_data->workspaceData[i]
                                        ->selectedDTWindows[j] == desktopRec) {
                                        XtFree(workspace_name);
                                        return (True);
                                }
                        }
...
#define EXPECTED_PANEL_HEIGHT 83
        RegisterInGrid(
            EXPECTED_PANEL_WIDTH, EXPECTED_PANEL_HEIGHT,
        ...
        for (i = (column > 0) ? column : 0; i <= columnWidth; i++) {
                for (j = (row > 0) ? row : 0; j <= rowHeight; j++) {
                        if (type) {
                                ...
                                if (desktop_grid[desktop_grid_index +
                                                 (i * numRows) + j] > 0) {
                                        desktop_grid[desktop_grid_index +
                                                     (i * numRows) + j]--;
                                }
...
void PutOnDTCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (client_data == NULL)
                ...
                file_mgr_data->popup_menu_icon =
                    NULL; /* Reset it, we don't need it */

                /* No need to see the selection_list if file_view_data is NULL
                 */

                if (file_view_data) {
                        for (i = 0; i < file_mgr_data->selected_file_count;
                             i++) {
                                if (file_mgr_data->selection_list[i] ==
                                    file_view_data) {
                                        file_view_data = NULL;
                                        break;
                                }
        ...
        if (file_view_data == NULL) {
                for (i = 0; i < file_mgr_data->selected_file_count; i++) {
                        if (file_mgr_data->toolbox) {
                                ...
                                path = _DtGetSelectedFilePath(
                                    file_mgr_data->selection_list[i]);
                                path = _DtResolveAppManPath(
                                    path, file_mgr_data->restricted_directory);
                                ptr = strrchr(path, '/');
                                *ptr = '\0';
                                directory = path;
                                file_name = ptr + 1;
                                ...
                                SetupDesktopWindow(
                                    XtDisplay(w), file_mgr_data, file_mgr_rec,
                                    file_name, file_mgr_data->host, directory,
                                    -1, -1, NULL, EndIndex);
                                ...
                                *ptr = '/';
                                ...
                                directory =
                                    ((DirectorySet *)(file_mgr_data
                                                          ->selection_list[i]
                                                          ->directory_set))
                                        ->name;
                                file_name = file_mgr_data->selection_list[i]
                                                ->file_data->file_name;
                ...
                if (file_mgr_data->toolbox) {
                        ...
                        *ptr = '\0';
                        ...
                        SetupDesktopWindow(XtDisplay(w), file_mgr_data,
                                           file_mgr_rec, file_name,
                                           file_mgr_data->host, directory, -1,
                                           -1, NULL, EndIndex);
                        ...
                        *ptr = '/';
                        ...
                        SetupDesktopWindow(XtDisplay(w), file_mgr_data,
                                           file_mgr_rec, file_name,
                                           file_mgr_data->host,
                                           file_mgr_data->current_directory, -1,
                                           -1, NULL, EndIndex);
                }
        }
}
        ...
        while (1) {
                if (desktop_grid[ws_num + (column * numRows) + row] == 0) {
                        if (numGridsR == 1 && numGridsC == 1) {
                                if (desktopIconType == LARGE) {
                                        *root_x =
                                            column * PIXELS_PER_COLUMN_LARGE;
                                        *root_y = row * PIXELS_PER_ROW_LARGE;
                                } else {
                                        *root_x =
                                            column * PIXELS_PER_COLUMN_SMALL;
...
void SetupDesktopWindow(Display *display, FileMgrData *file_mgr_data,
        ...
        for (i = 0; i < EndIndex; i++) {
                if (strcmp(desktop_data->desktopWindows[i]->title, title) ==
                        0 &&
                    strcmp(desktop_data->desktopWindows[i]->workspace_name,
                        ...
                        tmpStr = (GETMESSAGE(28, 11,
                                             "An object named:\n\n   "
                                             "%s\n\nalready exists on the "
                                             "backdrop in this Workspace.\nYou "
                                             "cannot put another "
                                             "object with the same name on the "
                                             "Workspace.\nTo have "
                                             "both objects on the Workspace, "
                                             "rename one of them."));
                        msg = XtNewString(tmpStr);
        ...
        desktopWindow->file_view_data = BuildNewOrderlist(
            ...
            desktopWindow->iconGadget, home_host_name,
            desktopWindow->dir_linked_to, file_name, desktopWindow->registered,
        ...
        for (i = desktop_data->numCachedIcons + desktop_data->numIconsUsed;
             i > new_size; i--) {
                ...
                if (desktop == sacredDesktop) {
                        desktop =
                            desktop_data
                                ->desktopWindows[desktop_data->numIconsUsed];
                        desktop_data
                            ->desktopWindows[desktop_data->numIconsUsed] =
                            sacredDesktop;
                }
...
static void CreatePopupMenu(Widget drawA) {
        ...
        popupBtns[0] = (Widget)XmCreateLabelGadget(
            desktop_data->popupMenu->popup, "title", args, 1);
        ...
        desktop_data->popupMenu->openFolder = popupBtns[4] =
            XmCreatePushButtonGadget(desktop_data->popupMenu->popup, "info",
                                     args, 2);
        XmStringFree(label_string);
        ...
        desktop_data->popupMenu->rename = popupBtns[5] =
            XmCreatePushButtonGadget(desktop_data->popupMenu->popup, "rename",
                                     args, 2);
        XmStringFree(label_string);
...
void GenerateShape(DesktopRec *desktopWindow) {
        ...
        XShapeCombineRectangles(XtDisplay(desktopWindow->shell),
                                XtWindow(desktopWindow->shell), ShapeBounding,
                                0, 0, &rect[0], 2, ShapeSet, Unsorted);
}
#endif
...
static void BuildDesktopLinks(Display *display) {
        ...
        while (retry > 0) {
                result =
                    DtWsmGetWorkspaceList(display, rootWindow, &paWS, &numInfo);
                if (result == Success) {
...
static int InitializeNewWorkspaces(Display *display, char *workspaceName) {
        ...
        if (DtWsmGetWorkspaceList(display, rootWindow, &paWS, &numInfo) ==
            Success) {
                if (numInfo <= desktop_data->numWorkspaces) {
                        /* should never get here, just put the new object in ws
                         * 0 */
                        return (0);
                }
                ...
                for (i = 0; i < numInfo - desktop_data->numWorkspaces; i++) {
                        ...
                        workspaceData =
                            (WorkspaceRec *)XtMalloc(sizeof(WorkspaceRec));

                        for (j = 0; j < numInfo; j++) {
                                workspaceData->name =
                                    XGetAtomName(display, paWS[j]);
                                CleanUpWSName(workspaceData->name);
                                haveIt = False;
                                for (k = 0; k < desktop_data->numWorkspaces;
                                     k++) {
                                        if (strcmp(
                                                workspaceData->name,
                                                desktop_data->workspaceData[k]
                                                    ->name) == 0) {
                                                haveIt = True;
                                                break;
                                        }
                        ...
                        workspaceData->number =
                            desktop_data->numWorkspaces + i + 1;
                        workspaceData->files_selected = 0;
                        workspaceData->selectedDTWindows =
                            (DesktopRec **)XtMalloc(sizeof(DesktopRec *));
                        workspaceData->selectedDTWindows[0] = NULL;
                        ...
                        desktop_data
                            ->workspaceData[desktop_data->numWorkspaces + i] =
                            workspaceData;
                }
                XFree((char *)paWS);
                paWS = NULL;
...
void CheckDesktopMarquee(int x, int y, int width, int height) {
        ...
        for (i = 0; i < desktop_data->numIconsUsed; i++) {
                ...
                _DtIconGetIconRects((DtIconGadget)g, &flags, &pixmapRect,
                                    &labelRect);
                ...
                case XmPIXMAP_LEFT: /* 2 */
                        if (flags & XmPIXMAP_RECT) {
                                ...
                                /* now lets check to see if the upper left and
                                 * lower right corners of the pixmap falls
                                 * within the passed in region
                                 */
                                pointInRegion =
                                    (XPointInRegion(region, pixmapRect.x,
                                                    pixmapRect.y) &&
                                     XPointInRegion(
                                         region,
                                         pixmapRect.x + pixmapRect.width,
                                         pixmapRect.y + pixmapRect.height));
                        } else if (flags &
                                   XmLABEL_RECT) { /* For the ones that don't
                                                      have pixmap */
                                labelRect.x =
                                    labelRect.x +
                                    desktop_data->desktopWindows[i]->root_x + 1;
                                 ...
                                 */
                                pointInRegion =
                                    (XPointInRegion(region, labelRect.x,
                                                    labelRect.y) &&
                                     XPointInRegion(
                                         region, labelRect.x + labelRect.width,
                                         labelRect.y + labelRect.height));
                        } else
                                pointInRegion = False;

                        if (pointInRegion) {
                                /* it falls within the region, lets see if its
                                 * already selected.
                                 */
                                match = False;
                                for (j = 0; j < workspaceData->files_selected;
                                     j++) {
                                        if (workspaceData
                                                ->selectedDTWindows[j] ==
                                            desktop_data->desktopWindows[i]) {
                                ...
                                /* Add to the front of the list */
                                for (j = workspaceData->files_selected; j > 0;
                                     j--) {
                                        workspaceData->selectedDTWindows[j] =
                                            workspaceData
                                                ->selectedDTWindows[j - 1];
                                ...
                                /* its pixmap is not it the region so lets see
                                 * if it is selected.  If it is lets deslect it.
                                 */
                                for (j = 0; j < workspaceData->files_selected;
                                     j++) {
                                        if (workspaceData
...
static void DropOnDesktopObject(Widget w, XtPointer client_data,
        ...
        case DtDND_BUFFER_TRANSFER:
                ProcessDropOnDesktopObject(w, dropInfo,
                                           (DesktopRec *)client_data);
                break;
        default:
                dropInfo->status = DtDND_FAILURE;
        } /* endswitch */
...
void WorkSpaceRemoved(Widget w, Atom deleted_ws_atom, int type,
        ...
        if (type == DtWSM_MODIFY_WORKSPACE_TYPE_DELETE) {
                ...
                while (retry > 0) {
                        result = DtWsmGetWorkspaceList(display, rootWindow,
                                                       &paWS, &numInfo);
                        if (result == Success) {
                                desktop_data->numWorkspaces = (int)numInfo;
                                break;
                ...
                old_ws_num = GetWorkspaceNum(old_workspace_name,
                                             oldNumWorkspaces, &old_index);
                ws_num = GetWorkspaceNum(workspace_name, oldNumWorkspaces,
                                         &new_index);

                /* Free up or reassign any workspace data in the deleted
                 * workspace */
                FreeUpOldWorkspace(oldNumWorkspaces, old_ws_num, ws_num,
                                   old_index, new_index);

                /* Look up for the duplication of workspace objects in the new
                 * workspace */
                for (i = 0; i < desktop_data->numIconsUsed;
                     i++) { /* Those objects whose workspace number matches the
                               deleted workspace number */
                        if (desktop_data->desktopWindows[i]->workspace_num ==
                            old_ws_num) { /* Look up in all of the workspace
                                             objects */
                                for (j = 0; j < desktop_data->numIconsUsed;
                                     j++) { /* Those objects in the current
                                               workspace */
                                        if (j != i &&
                                            desktop_data->desktopWindows[j]
                                                    ->workspace_num == ws_num) {
                                                /* If there is an object
                                                 * matching the old workspace
                                                 * object delete the old */
                                                if (strcmp(
                                                        desktop_data
                                                            ->desktopWindows[j]
                                                            ->file_name,
...
static void FreeUpOldWorkspace(int oldNumWorkspaces, int old_ws_num, int ws_num,
        ...
        if (numoldfiles) {
                desktop_data->workspaceData[new_index]->selectedDTWindows =
                    (DesktopRec **)XtRealloc(
                        (XtPointer)desktop_data->workspaceData[new_index]
                            ->selectedDTWindows,
                ...
                XtFree((XtPointer)desktop_data->workspaceData[old_index]
                           ->selectedDTWindows);
                desktop_data->workspaceData[old_index]->selectedDTWindows =
                    NULL;
...
void PutOnWorkspaceHandler(Tt_message ttMsg) {
        ...
        if (NULL == fullName || (stat(fullName, &fileInfo) != 0 &&
                ...
                dialogTitle =
                    XtNewString(GETMESSAGE(20, 84, "Put in Workspace"));
         ...
         */
        for (i = 0; i < desktop_data->numWorkspaces; ++i) {
                if (strcmp(workspace, desktop_data->workspaceData[i]->name) ==
                    0) {
                ...
                if (strcmp(desktop_data->desktopWindows[i]->title, title) ==
                    ...
                    strcmp(desktop_data->desktopWindows[i]->workspace_name,
                        ...
                        if (invalidWorkspace)
                                template = XtNewString(GETMESSAGE(
                                    28, 12,
                                    "An invalid workspace was passed. The "
                                    "default workspace will "
                                    "be\nworkspace one and an object "
                                    "named:\n\n   %s\n\nalready exists "
                                    "on the backdrop in this Workspace.\nYou "
                                    "cannot put another object "
                                    "with the same name on the Workspace.\nTo "
                                    "have both objects on the "
        ...
        desktopWindow->file_view_data = BuildNewOrderlist(
            NULL /* @@@ use cached info if possible! */,
            desktopWindow->iconGadget, home_host_name,
            desktopWindow->dir_linked_to, fileName, desktopWindow->registered,
            desktopWindow->toolbox);
