...
static void radio_callback(Widget w, XtPointer client_data,
                           XtPointer call_data) {
...
static void ok_callback(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        case G_RENAME_EXISTING:
                /* get object (i.e. file) name from text field and build the
                 * complete path */
                /* this procedure restricts renaming to be within a folder
                 * (directory) */
                ...
                if ((lstat(renameNewName, &buf)) == 0) {
                        ...
                        sprintf(msg,
                                ...
                                renameNewName);
                        _DtMessageDialog(G_over_dialog, title, msg, 0, FALSE,
                ...
                /* attempt the rename ... since rename function is being used,
                 * this */
                ...
                if (rename(G_rename_oldname, renameNewName) != 0) {
                        ...
                        sprintf(msg,
                                ...
                                G_rename_oldname, renameNewName,
                                strerror(saveError));
                        _DtMessageDialog(G_over_dialog, title, msg, 0, FALSE,
                                         NULL, renameError_ok_callback, NULL,
                                         NULL, False, ERROR_DIALOG);
                ...
                /* the rename went ok ... create an empty file so SyncDirectory
                   doesn't */
...
static void confirmtoggle_callback(Widget w, XtPointer client_data,
                                   XtPointer call_data) {
...
void create_overwrite_dialog(Widget parent_widget, char *sname, char *tname,
        ...
        char gname[MAX_PATH], gfolder[MAX_PATH],
            gobject[MAX_PATH]; /* g for generated */
        ...
        static ActionAreaItem action_items[] = {
            ...
            {"Cancel Copy", 6, 7, overwrite_cancel_callback,
             NULL}, /* possibly changed below to "Cancel Move" */
            {"Help", 6, 4, help_callback, (XtPointer)HELP_OVERWRITE},
        };

        ActionAreaDefn overwriteActions = {XtNumber(action_items),
        ...
        if (G_move) {
                XtSetArg(args[n], XmNtitle,
                         GETMESSAGE(5, 2, "Folder Move - File Overwrite"));
                n++;
        } else {
                XtSetArg(args[n], XmNtitle,
                         GETMESSAGE(5, 9, "Folder Copy - File Overwrite"));
                n++;
        }
        G_over_dialog =
            XmCreateDialogShell(parent_widget, "dialog_shell", args, n);

        /* create a frame around the dialog for better separation from window
         * border */
        outerFrame = XtVaCreateWidget("outerFrame", xmFrameWidgetClass,
                                      G_over_dialog, XmNshadowThickness, 3,
                                      XmNshadowType, XmSHADOW_ETCHED_IN, NULL);

        /* Create the Manager Widget, form, for the copy dialog */
        form = XtVaCreateWidget("dialog_form", xmFormWidgetClass, outerFrame,
                                XmNverticalSpacing, VERTICAL_SPACING,
                                XmNhorizontalSpacing, HORIZONTAL_SPACING, NULL);
        ...
        msg_label = XtVaCreateManagedWidget(
            "msg_header", xmLabelWidgetClass, form, XmNlabelString, msg,
        ...
        action_label = XtVaCreateManagedWidget(
            "action-question", xmLabelWidgetClass, form, XmNlabelString, msg,
            XmNalignment, XmALIGNMENT_BEGINNING, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, msg_label, XmNtopOffset,
            2 * VERTICAL_SPACING, XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET,
            XmNleftWidget, msg_label, NULL);
        XmStringFree(msg);
        ...
        n++;
        G_overwrite_radio =
            (Widget)XmCreateRadioBox(form, "radio_box", args, n);

        /* create the radio buttons */
        xm_string[G_OVERWRITE_EXISTING] =
            XmStringCreateLocalized(GETMESSAGE(5, 3, "Replace existing file"));
        xm_string[G_RENAME_EXISTING] = XmStringCreateLocalized(
            GETMESSAGE(5, 4, "Rename existing file to:"));
        xm_string[G_SKIP] =
            XmStringCreateLocalized(GETMESSAGE(5, 6, "Skip this file"));

        for (i = 0; i < N_OVERWRITE_RADIO_BUTTONS; i++) {

                radiobtn[i] = XtVaCreateManagedWidget(
                    "radio_toggle", xmToggleButtonWidgetClass,
                    G_overwrite_radio, XmNalignment, XmALIGNMENT_BEGINNING,
                    XmNlabelString, xm_string[i], NULL);
                XtAddCallback(radiobtn[i], XmNvalueChangedCallback,
                              radio_callback, (XtPointer)(XtArgVal)i);
        ...
        G_rename_text = XtVaCreateManagedWidget(
            "rename_text", xmTextFieldWidgetClass, form, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, G_overwrite_radio, XmNleftOffset,
            HORIZONTAL_SPACING, XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET,
            XmNbottomWidget, G_overwrite_radio, XmNrightAttachment,
            XmATTACH_FORM, XmNrightOffset, 5, XmNvalue, gobject, XmNsensitive,
            FALSE, XmNcursorPosition, strlen(gobject), NULL);

        /* create the toggle button */
        msg = XmStringCreateLocalized(
            GETMESSAGE(5, 7, "Apply this action to subsequent name conflicts"));
        G_over_confirm = XtVaCreateManagedWidget(
            "over_confirm", xmToggleButtonWidgetClass, form, XmNlabelString,
            msg, XmNset, !G_overwrite_confirm, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, G_overwrite_radio, XmNleftAttachment,
            XmATTACH_OPPOSITE_WIDGET, XmNleftWidget, action_label,
            XmNleftOffset, 0, XmNrightAttachment, XmATTACH_FORM, XmNalignment,
            XmALIGNMENT_BEGINNING, NULL);
        XtAddCallback(G_over_confirm, XmNvalueChangedCallback,
                      confirmtoggle_callback, NULL);
        XmStringFree(msg);
        ...
        separator = XtVaCreateManagedWidget(
            ...
            XmATTACH_WIDGET, XmNtopWidget, G_over_confirm, XmNbottomAttachment,
