...
static void error_cancel_callback(Widget w, XtPointer client_data,
                                  XtPointer call_data) {
...
void toggle_callback(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (w == G_toggle_main) {
                if (G_error_dialog != NULL)
                        XtVaSetValues(G_toggle_error, XmNset, !G_ignore_errors,
                                      NULL);
        } else {
                XtVaSetValues(G_toggle_main, XmNset, !G_ignore_errors, NULL);
        }
        return;
}
...
void create_error_dialog(Widget parent_widget, char *op, char *filename,
                         int errnum) {
        char buf[1024];
        char *msg;
        ...
        ActionAreaDefn copyActions = {XtNumber(copy_action_items),
        ...
        ActionAreaDefn moveActions = {XtNumber(move_action_items),
                                      0, /* Ok is default action */
                                      move_action_items};
        ...
        if (G_move) {
                XtSetArg(args[n], XmNtitle,
                         GETMESSAGE(4, 4, "Folder Move - Error"));
                n++;
        } else {
                XtSetArg(args[n], XmNtitle,
                         GETMESSAGE(4, 3, "Folder Copy - Error"));
                n++;
        }

        G_error_dialog =
            XmCreateDialogShell(G_copy_dialog, "dialog_shell", args, n);

        /* create a frame around the dialog for better separation from window
         * border */
        outerFrame = XtVaCreateManagedWidget(
            "outerFrame", xmFrameWidgetClass, G_error_dialog,
            XmNshadowThickness, 3, XmNshadowType, XmSHADOW_ETCHED_IN, NULL);

        /* Create the Manager Widget, form, for the copy error dialog */
        form = XtVaCreateManagedWidget(
            "dialog_form", xmFormWidgetClass, outerFrame, XmNverticalSpacing,
            VERTICAL_SPACING, XmNhorizontalSpacing, HORIZONTAL_SPACING, NULL);
        ...
        separator = XtVaCreateManagedWidget(
            "seperator", xmSeparatorWidgetClass, form, XmNbottomAttachment,
        ...
        px = XmGetPixmapByDepth(XtScreen(form), "xm_error", foreground,
        ...
        icon = XtVaCreateManagedWidget(
            "error_icon", xmLabelWidgetClass, form, XmNlabelType, XmPIXMAP,
        ...
        error_header_label = XtVaCreateManagedWidget(
            "msg_header", xmLabelWidgetClass, form, XmNlabelString, xm_string,
            XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_WIDGET,
            XmNleftWidget, icon, XmNrightAttachment, XmATTACH_FORM,
        ...
        error_text_label = XtVaCreateManagedWidget(
            "msg_text", xmLabelWidgetClass, form, XmNlabelString, xm_string,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, error_header_label,
            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, icon,
            XmNrightAttachment, XmATTACH_FORM, XmNalignment,
        ...
        G_toggle_error = XtVaCreateManagedWidget(
            "toggle", xmToggleButtonWidgetClass, form, XmNlabelString,
            xm_string, XmNset, !G_ignore_errors, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, error_text_label, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, icon, XmNbottomAttachment,
            XmATTACH_WIDGET, XmNbottomWidget, separator, XmNalignment,
            XmALIGNMENT_BEGINNING, NULL);
        XtAddCallback(G_toggle_error, XmNvalueChangedCallback, toggle_callback,
                      NULL);
        XmStringFree(xm_string);
