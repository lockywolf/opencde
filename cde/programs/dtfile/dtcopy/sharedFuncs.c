...
static unsigned char questionBits[] = {
    0xf0, 0x3f, 0x00, 0x58, 0x55, 0x00, 0xac, 0xaa, 0x00, 0xd6, 0x5f,
    0x01, 0xea, 0xbf, 0x02, 0xf6, 0x7f, 0x01, 0xea, 0xba, 0x02, 0xf6,
    0x7d, 0x05, 0xea, 0xba, 0x0a, 0x56, 0x7d, 0x15, 0xaa, 0xbe, 0x1e,
    0x56, 0x5f, 0x01, 0xac, 0xaf, 0x02, 0x58, 0x57, 0x01, 0xb0, 0xaf,
    0x00, 0x60, 0x55, 0x01, 0xa0, 0xaa, 0x00, 0x60, 0x17, 0x00, 0xa0,
    0x2f, 0x00, 0x60, 0x17, 0x00, 0xb0, 0x2a, 0x00, 0x50, 0x55, 0x00};

static unsigned char warningBits[] = {
    0x00, 0x00, 0x18, 0x00, 0x2c, 0x00, 0x56, 0x00, 0x2a, 0x00, 0x56,
    0x00, 0x2a, 0x00, 0x56, 0x00, 0x2c, 0x00, 0x14, 0x00, 0x2c, 0x00,
    0x14, 0x00, 0x2c, 0x00, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3c,
    0x00, 0x14, 0x00, 0x2c, 0x00, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00};
...

{
        ...
        if (lastSlash == NULL)
                lastSlash =
                    oldPath - 1; /* allows for no path and first char is dot */
        len = lastDot - oldPath;
        if (lastDot != NULL && /* no dot */
            lastDot >
                lastSlash + 1 && /* dot is in filename, not directory name */
            len !=
                strlen(oldPath) - 1) /* dot is not last character of filename */
        {
...
XImage *CreateDefaultImage(Display *display, char *bits, unsigned int width,
        ...
        image = XCreateImage(
            display, DefaultVisual(display, DefaultScreen(display)), 1,
...
#if !defined(CSRG_BASED)
static int CopyFileSysType(int dev) {
        struct ustat u1;
        if (ustat(dev, &u1) < 0)
                return -2;
        return u1.f_tinode;
...
void CheckDeleteAccess(XtAppContext app_context, int delay, Boolean checkPerms,
        ...
        if (checkPerms && move) {
                ...
                if (!tmpptr) /* Error */
                        ...
                        perm_status =
                            CopyCheckDeletePermission(tmpptr, source_name);
                        free(tmpstring);
                }
                ...
                tmpmsg = GETMESSAGE(
                    ...
                    "choice from the object's\npopup menu or from the Selected "
                    "menu to turn on your\nRead permission on the "
                    "object.\n\nNote: If this object is a folder, you must "
                    "also have\nRead permission for each of the objects inside "
...
char *
#ifdef _NO_PROTO
    _DtCopyPathFromInput(input_string, current_dir) char *input_string;
char *current_dir;
#else
...
char *
#ifdef _NO_PROTO
    _DtCopyChangeTildeToHome(input_string) char *input_string;
#else
...
#endif
{
        ...
        if ((input_string[1] != '/')) {
                ...
                full_path = (char *)XtMalloc(strlen(homedir) +
...
int
#ifdef _NO_PROTO
    Check(spth, dpth, mode) register char *spth;
register char *dpth;
int mode;
#else
    Check(
        ...
        while (dbuf.st_ino != ROOTINO) {
                ...
                if (dbuf.st_ino == sbuf.st_ino) {
                        if (mode)
                                tmpStr = GETMESSAGE(
                                    3, 19,
                                    "Cannot move folder into itself. %s");
                        else
                                tmpStr = GETMESSAGE(
                                    3, 20,
                                    "Cannot copy folder into itself. %s");
