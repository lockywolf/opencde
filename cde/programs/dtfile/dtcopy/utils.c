...
void Display_Help(char *helpVolume, char *locationId);
...
void help_close_callback(Widget widget, XtPointer client_data,
                         XtPointer call_data) {
        XtUnmapWidget(XtParent(widget));
}

void help_hyperyperlink_callback(Widget widget, XtPointer client_data,
                                 XtPointer call_data) {
...
void _DtChildPosition(Widget w, Widget parent, Position *newX, Position *newY) {
...

        {
                ...
                XGetWindowAttributes(XtDisplay(parent), XtWindow(parent),
                                     &attr);
        }
...
void Help(char *helpVolume, char *locationId) {
        ...
        if (G_help_dialog == NULL) {
                ...
                XtSetArg(args[n], XmNtitle,
                         GETMESSAGE(6, 5, "Directory Copy Help"));
                n++;

                G_help_dialog =
                    DtCreateHelpDialog(G_toplevel, "helpDlg", args, n);

                XtAddCallback(G_help_dialog, DtNhyperLinkCallback,
                              (XtCallbackProc)help_hyperyperlink_callback,
                              NULL);
...
Widget Create_Action_Area(Widget parent_widget, ActionAreaDefn actions,
        ...
        action_area = XtVaCreateManagedWidget(
