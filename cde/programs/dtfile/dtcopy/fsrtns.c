...
static int CopyFile(char *sourceP, char *targetP, int repl, struct stat *statP)
/* copy a file; if repl non-zero, overwrite any existing file */
{
        ...
        if (tgt < 0 && errno == EEXIST && repl) {
                ...
                tgt = open(targetP, O_CREAT | O_EXCL | O_WRONLY,
                           statP->st_mode & 0777);
        }
        if (tgt < 0) {
                rc = errno;
...
static int CopyLink(char *sourceP, char *targetP, int repl, struct stat *statP)
...
static int CopyDir(char *sourceP, char *targetP, int repl, int link,
                   struct stat *statP)
/* copy a directory and recursively all its subdirectories */
{
        ...
        /* if we have root privileges, make sure directory ownership is
         * preserved */
        if (geteuid() == 0) {
                if (statP->st_uid != 0 || statP->st_gid != getegid()) {
...
/* copy a directory, file, or symbolic link */
{
        ...
        copy_switch:
                ...
                case S_IFLNK:
                        if (link)
                                rc =
                                    CopyLink(sourceP, targetP, repl, &src_stat);
...
int EmptyDir(char *sourceP, int rm, int force) {
        DIR *dirP;             /* open directory */
        struct dirent *entryP; /* directory entry */
        char srcname[1024];
        int srclen;
...
void fsRename(char *sourceP, char *targetP, int replace, int *rcP) {
        int rc;
        struct stat buf;
        ...
        *rcP = rename(sourceP, targetP);
        ...
        if (replace && *rcP == ENOTDIR || *rcP == EISDIR) {
                /* error reason: tried to replace file by directory or vice
                 * versa */
...
void fsMove(char *sourceP, char *targetP, int replace, int *rcP) {
        /* try to rename */
        ...
        {
                /* first check if we have write permission in the source
                 * directory */
                char dir[1024], *p;
...
void fsCopy(char *sourceP, char *targetP, int replace, int *rcP) {
        *rcP = CopyObject(sourceP, targetP, replace, 0);
}

void fsCopyLink(char *sourceP, char *targetP, int replace, int *rcP) {
        *rcP = CopyObject(sourceP, targetP, replace, 1);
}

void fsErase(char *nameP, int *rcP, int force) {
        *rcP = EraseObject(nameP, force);
}

void fsEmpty(char *nameP, int *rcP) { *rcP = EmptyDir(nameP, 0, 0); }
