        ...
        */
        ...
        if ((strncmp(G_source_dir, msg2, lenSourceDir) == 0) &&
            (lenMsg2 > lenSourceDir)) {
                msg = XtMalloc(strlen(msg1) + lenEllipsis +
                               (lenMsg2 - lenSourceDir) + 2);
...
static void UpdateErrorCount(void) {
        ...
        if (nerr == 1) {
                XtVaSetValues(G_status_text, XmNbottomAttachment, XmATTACH_NONE,
                              NULL);
                /*
                    XtVaSetValues(G_status_text2,
...
/* process X events until there are no more pending events queued */
{
        ...
        /* if the user clicked on the Cancel button and Warning 'yes' btn, abort
         * the copy */
        if (G_do_copy)
                return 0;
        else {
...
static int ErrorHandler(FileOp op, char *fname, int errnum) {
        ...
        if (!G_ignore_errors) {
                create_error_dialog(G_toplevel, LongFileOpNames[op], fname,
                                    errnum);
...
static int ConfirmHandler(FileOp op, char *sname, int stype, char *tname,
        ...
        if (is_overwrite) {
                ...
                /* set overwrite_dialog so we know if the user was prompted for
                 * this file */
                /*   Otherwise, G_overwrite_selection is either the default or
                 * was set   */
                /*   by the user in a prior invocation of the overwrite dialog
                 */
                if (G_overwrite_confirm) {
                        overwrite_dialog = TRUE;
                        create_overwrite_dialog(G_toplevel, sname, tname,
                                                ttype & ft_isdir);
...
#define USAGE "Usage: %s [options ...] source target\n"
        ...
        if (i < argc || app_args.source_name == NULL ||
...
int main(int argc, char *argv[]) {
        ...
        /* XtAddEventHandler(G_toplevel, (EventMask) 0, True,
        ...
        if (copy_status == False) {
                ...
                XtAppAddTimeOut(app_context, app_args.delay, TimeoutHandler,
                                NULL);
        ...
        nfiles = 0;
        ndirs = -1; /* -1 so the source folder does not get counted, only
                       subfolders */
        UpdateSummary();
        nerr = 0;
        UpdateErrorCount();
         ...
         * Reason:  The routine for selecting a window position for popup
         * windows (error and overwrite warning dialogs) won't work until the
         * main window is mapped.  Therefore, we don't want to start the
         * actual copy operation (which might generate error or warning
         * messages) before the main window is mapped.
         */
        XtAddEventHandler(G_toplevel, StructureNotifyMask, False, CheckForMap,
                          NULL);
        while (!is_mapped)
                EventCheck();
        ...
        /* if move was requested and the copy was successful, delete the source
         * now */
        if (G_move) {
                if (nerr > 0) {
                        sprintf(msg,
                                GETMESSAGE(2, 13,
                                           "%s not removed because of errors."),
                                app_args.sync.source);
                        AppendErrorMsg(msg);
                } else {
                        /* arrange for callback's during recursive directory
                         * delete */
        ...
        XtVaSetValues(G_error_msgs, XmNcursorPosition, 0, XmNtopCharacter, 0,
                      NULL);
