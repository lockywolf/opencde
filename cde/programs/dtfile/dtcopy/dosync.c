...
char *FileOpNames[] = {"synchronize", "opendir",   "lstat",   "stat",
                       "readlink",    "delete",    "copy",    "mkdir",
                       "copy link",   "make link", "working", "is a copy",
                       "is a link"};

int (*syncConfirmCallback)(FileOp op, char *sname, int stype, char *tname,
                           int ttype, char *link) = NULL;
...
#define CONFIRM(op, sn, st, tn, tt, l, rc)                                     \
        if (periodicCallback && (*periodicCallback)() != 0)                    \
                return -1;                                                     \
                 ...
                 (rc = syncConfirmCallback(op, sn, st, tn, tt, l)) != 0)       \
                return rc;                                                     \
        else

#define ERROR_CHECK(op, f, rc)                                                 \
        if (rc < 0 ||                                                          \
            rc && syncErrorCallback && syncErrorCallback(op, f, rc) < 0)       \
                return -1;                                                     \
        else
 ...
 */
static void DumpDir(DirEntry *dP) {
        DirEntry *eP;
        ...
        for (eP = dP; eP; eP = eP->next)
                printf(" %-15s %2d  %c %c %c %c  %c  %s\n", eP->name, eP->rc,
                       (eP->ftype & ft_isdir) ? 'D' : ' ',
                       (eP->ftype & ft_islnk) ? 'L' : ' ',
                       eP->exclude ? 'X' : ' ', eP->skip ? 'S' : ' ',
                       eP->teP ? 'F' : ' ', eP->link ? eP->link : "");
}
 ...
 */
static DirEntry *SortDir(DirEntry *dP, int (*cmp)(const void *, const void *)) {
        int i, n;
        DirEntry *eP, **linkPP;
 ...
 */
static void FreeDir(DirEntry *deP) {
        DirEntry *nextP;
 ...
 */
static DirEntry *GetDirEntry(char *fname, FileOp *op, int *rc) {
        DirEntry *deP;
        char *p;
        char link[1024];
        int l;
 ...
 */
static int GetDir(char *dirname, PatternList *xl, PatternList *sl,
                  DirEntry **listPP) {
        ...
        *fnP++ = '/';
        ...
        while ((entryP = readdir(dirP)) != NULL) {
                ...
                *linkPP = deP;
                ...
                if (rc != 0 && syncErrorCallback &&
                    syncErrorCallback(op, fname, rc) < 0) {
                        FreeDir(firstP);
                        return -1;
                }
 ...
 */
static int dirEraseCallback(char *fname) {
        erase_rc = syncConfirmCallback(op_delete, "", ft_noent, fname, 0, "");
        return erase_rc;
}

static int dirErrorCallback(char *fname, int errnum) {
 ...
 */
static int doUnlink(char *fname, DirEntry *fP, int confirm) {
        int rc;
        ...
        if (SP->dontdoit)
                ...
                /* arrange for syncConfirmCallback's during recursive directory
                 * deletes */
                erase_rc = 0;
                if (syncErrorCallback)
 ...
 */
static int doCopy(char *sname, DirEntry *sP, char *tname, DirEntry *delP) {
        int rc;
 ...
 */
static int doMkdir(char *sname, DirEntry *sP, char *tname, DirEntry *delP) {
        int rc;
        ...
        rc = SP->dontdoit
                 ? 0
                 : (mkdir(tname, sP->stat.st_mode & 07777) < 0) ? errno : 0;
 ...
 */
static void targetLink(char *source, char *target, char **tlPP, char **msgPP)
/*
 *
 */
{
 ...
 */
static int doCopyLink(char *sname, DirEntry *sP, char *tname, DirEntry *delP) {
        char *tl, *msg;
        int rc;
 ...
 */
static int doSymlink(char *sname, DirEntry *sP, char *tname, DirEntry *delP) {
        char *tl, *msg;
        int rc;
 ...
 */
static int cmpSymlink(char *sname, DirEntry *seP, char *tname, DirEntry *teP) {
        char *tl, *msg;
 ...
 */
static int SyncItem(char *sname, DirEntry *sP, char *tname, DirEntry *tP) {
        int rc;
        ...
        rpl_target:
                ...
                if (sP->ftype & ft_isdir) {
                        /* source is a dir: create target directory and do
                         * recursive sync */
                ...
                if ((tP->ftype & ft_isdir) == (sP->ftype & ft_isdir)) {
                        /*
                         * target points to the right type of object (dir vs
                         * regular file): check if target points to the right
                         * place
                         */
                        ...
                        if (SP->listlinks && rc == 1)
                                CONFIRM(op_info_link, sname, sP->ftype, tname,
                                        tP->ftype, "", rc);
                ...
                if (sP->ftype & ft_isdir)
                        ...
                        if (sP->ftype & ft_islnk) {
                                ...
                                if (!SP->keepcopies && !SP->copyfiles)
                                        ...
                                        CONFIRM(op_info_copy, sname, sP->ftype,
                                                tname, tP->ftype, "", rc);
                        }
                        ...
                        if (!SP->forcecopies &&
                            ...
                            tP->stat.st_size == sP->stat.st_size) {
                                /* we assume the file hasn't changed: don't do
                                 * anything */
                                return 0;
                        }
                        ...
                        if (SP->keepnew &&
                            tP->stat.st_mtime > sP->stat.st_mtime)
                                /* we should keep the target because it is newer
                                 * than the source */
                                return 0;
                ...
                if (sP->ftype & ft_isdir) {
                        ...
                        /* source is dir: recursively sync source and target
                         * dirs */
                        return SyncDirRecur(sname, tname);
 ...
 */
static int CmpPred(DirEntry *dP) {
        ...
        else if (!dP->teP || !(dP->teP->ftype & ft_isdir) ||
 ...
 */
static int DirCmp2(const void *p1, const void *p2) {
        int d = CmpPred(*(DirEntry **)p1) - CmpPred(*(DirEntry **)p2);
        if (d)
                return d;
        else
                return strcmp((*(DirEntry **)p1)->name,
                              (*(DirEntry **)p2)->name);
}
...
static int SyncDirRecur(char *sdir, char *tdir) {
        ...
        *ftP++ = '/';
        ...
        /* re-sort source dir so that regular files are processed before sub
         * dirs */
...
void SyncDirectory(SyncParams *p) {
        ...
        if (rc) {
                if (op == op_lstat && rc == ENOENT) {
                        ...
                        /* we'll try to procede anyway ... unless the error
                         * callback says no */
                        if (syncErrorCallback &&
                            syncErrorCallback(op, tbuf, rc) < 0) {
                                FreeDir(sP);
                                FreeDir(tP);
                                return;
                        }
