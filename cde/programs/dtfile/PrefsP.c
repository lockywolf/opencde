...
static void PreferencesChange(XtPointer client_data,
                              DialogData *old_dialog_data,
                              DialogData *new_dialog_data, XtPointer call_data);
static void PreferencesClose(XtPointer client_data, DialogData *old_dialog_data,
...
void ShowPreferencesDialog(Widget w, XtPointer client_data,
                           XtPointer callback) {
        ...
        /* set the title */
        preferences_rec =
            (PreferencesRec *)_DtGetDialogInstance(file_mgr_data->preferences);
        ...
        if (file_mgr_data->title != NULL &&
            (strcmp(file_mgr_data->helpVol, DTFILE_HELP_NAME) != 0 ||
             strcmp(file_mgr_data->title, "Trash Can") == 0)) {
                ...
                tmpStr =
                    (GETMESSAGE(23, 22, "File Manager - Set View Options"));
...
static void PreferencesChange(XtPointer client_data,
        ...
        if (file_mgr_data->show_type != preferences_data->show_type ||
            ...
            file_mgr_data->positionEnabled !=
                preferences_data->positionEnabled ||
            ...
            file_mgr_data->show_status_line !=
                ...
                /*
                 * Depending upon which of the preferences values changed,
                 * positioning may need to be disabled, and the preferences
                 * dialog updated.
                 */
                if (((file_mgr_data->order != preferences_data->order) ||
                     (file_mgr_data->direction !=
                      preferences_data->direction)) &&
                ...
                /*
                 * In tree mode, if we switch from showing files "always" to
                 * "choose", change all branches from tsAll back to tsDirs.
                 */
                if (file_mgr_data->tree_files == TREE_FILES_ALWAYS &&
                    preferences_data->tree_files == TREE_FILES_CHOOSE) {
                        for (i = 0; i < file_mgr_data->directory_count; i++) {
                                branch =
                                    file_mgr_data->directory_set[i]->sub_root;
                                if (branch->ts == tsAll) {
                                        branch->ts = tsDirs;
                                        UpdateBranchList(
                                            file_mgr_data,
                ...
                file_mgr_data->show_status_line =
                    preferences_data->show_status_line;
                if (file_mgr_data->positionEnabled ==
                    preferences_data->positionEnabled)
