...
void ShowCopyFileDialog(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        DialogData *dialog_data;
        FileMgrData *file_mgr_data;
        static FileViewData *file_view_data; /* static because it is passed to
                                                RenameOk callback */
        ...
        XtAddCallback(current_name, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_COPY_DIALOG_STR);
        ...
        XtAddCallback(file_type_box, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_COPY_DIALOG_STR);
        ...
        label_string = XmStringCreateLocalized(
            ((char *)GETMESSAGE(10, 38, "Name for copy : ")));
        ...
        XtAddCallback(file_label, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_COPY_DIALOG_STR);
        ...
        call_struct->original_logical_type =
        ...
        XtAddCallback(ok, XmNactivateCallback, RenameOk,
        ...
        delete_window_atom =
            XmInternAtom(XtDisplay(shell), "WM_DELETE_WINDOW", True);
...
void ShowMoveFileDialog(Widget w, XtPointer client_data, XtPointer call_data) {
        FileMgrRec *file_mgr_rec;
        DialogData *dialog_data;
        static FileMgrData *file_mgr_data; /* static because it is passed to
                                              RenameOk callback */
        FileViewData *file_view_data;
        ...
        XtManageChild(current_name);
        XtAddCallback(current_name, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_MOVE_DIALOG_STR);
...
void ShowLinkFileDialog(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        XtManageChild(current_name);
        XtAddCallback(current_name, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_LINK_DIALOG_STR);
...
static void RenameOk(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (strlen(new_dir) == 0)
                msg = XtNewString(GETMESSAGE(
                    10, 11, "A file or folder cannot have a blank name."));
#ifdef _CHECK_FOR_SPACES
        ...
        file_set[0] =
            build_path(call_struct->directory_name, call_struct->file_name);
        ...
        FileMoveCopy(call_struct->file_mgr_data, to_file, to_dir, to_host,
                     host_set, file_set, file_count, modifiers, RenameDone,
                     (XtPointer)data);
...
static Boolean GetToPath(char *to, int to_size, char *new_name,
        ...
        if (strncmp(new_name, "/", 1) == 0) {
                if ((path = (char *)ResolveLocalPathName(host_in, new_name,
                                                         NULL, home_host_name,
                                                         &tt_status)) == NULL)
                        return (False);
        } else {
...
static void FileTypePreview(Widget w, String new_name, XtPointer client_data,
        ...
        orig_name = ResolveLocalPathName(
            call_struct->host_name, call_struct->directory_name,
            ...
            strcmp(call_struct->original_logical_type, LT_BROKEN_LINK) == 0)
                ...
                if (strcmp(orig_name, whole_name) == 0)
                        file_type = (char *)DtDtsDataToDataType(
                            orig_name, NULL, 0, NULL, NULL, NULL, NULL);
                else {
                        if (call_struct->menu_widget == *renameBtn) {
                                ...
                                if ((link_path = _DtFollowLink(orig_name)) &&
                                    (strcmp(link_path, orig_name) == 0)) {
                                        file_type = (char *)DtDtsDataToDataType(
                                            new_link_path, NULL, 0, NULL, NULL,
                                            NULL, NULL);
                                } else {
                                        /* Links are typed according to what
                                         * they are linked to */
                                        file_type =
                                            call_struct->original_logical_type;
                                }
                                ...
                                /* We want to find out the possible filetype of
                                   a file that has such name (stored in
                                   link_path). However, the file does not even
                                   existed yet. In order for the file typing
                                   code to work correctly, we have to pass in a
                                   dummy stat structure indicates that if the
                                   file is created, this is its mode.
                                */
                                struct stat fileStat;
                                unsigned int creationMask =
                                    umask(DtFILE_DATA_CREATION_MASK);

                                memset(&fileStat, 0, sizeof(struct stat));
                                fileStat.st_mode = creationMask | S_IFREG;

                                file_type = (char *)DtDtsDataToDataType(
                                    link_path, NULL, 0, &fileStat, NULL, NULL,
                                    NULL);
                                umask(creationMask);
                        }
                }
        ...
        if (file_type) {
                if (call_struct->previous_file_type &&
                    strcmp(call_struct->previous_file_type, file_type) == 0) {
                        ...
                        if (Flag == False) {
                                ...
                                tmpStr =
                                    GETMESSAGE(10, 41,
                                               "The correct icon for this "
                                               "datatype is already shown.");
                ...
                if (DtActionExists(file_type)) {
                        ...
                        char *str = (char *)DtDtsDataTypeToAttributeValue(
                            file_type, FM_TYPE_LABEL, NULL);
                        if (str) {
                                string = XmStringCreateLocalized(str);
                ...
                pixmapData = _DtRetrievePixmapData(
                    file_type, call_struct->file_name,
                ...
                _DtCheckAndFreePixmapData(
                    file_type, toplevel,
                    (DtIconGadget)(call_struct->file_type_widget), pixmapData);
        } else {
                XtSetArg(args[n], XmNimageName, NULL);
                n++;
                XtSetValues(call_struct->file_type_widget, args, n);
...
void ShowMakeFileDialog(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (w == *create_directoryBtn) {
                ...
                XtSetArg(args[n], XmNtitle, tempStr);
                n++;
                shell = XmCreateDialogShell(file_mgr_rec->shell, "new_folder",
                                            args, n);
                label_string = XmStringCreateLocalized(
                ...
                if (file_mgr_data->title != NULL &&
                    strcmp(file_mgr_data->helpVol, DTFILE_HELP_NAME) != 0) {
                        ...
                        tmpStr =
                            (GETMESSAGE(10, 29, "File Manager - New File"));
                        tempStr = XtNewString(tmpStr);
                }
                XtSetArg(args[n], XmNtitle, tempStr);
                n++;
                shell = XmCreateDialogShell(file_mgr_rec->shell,
                                            "new_data_file", args, n);
                label_string = XmStringCreateLocalized(
                    ((char *)GETMESSAGE(10, 7, "New File Name: ")));
        }
        XtFree(tempStr);
        ...
        */
        ...
        XtAddCallback(ok, XmNactivateCallback, MakeFileOk,
                      (XtPointer)call_struct);
        ...
        if (w == *create_directoryBtn) {
                ...
                XtAddCallback(new_name, XmNhelpCallback,
                              (XtCallbackProc)HelpRequestCB,
                              HELP_CREATE_DIR_DIALOG_STR);
                XtAddCallback(new_text, XmNhelpCallback,
                              (XtCallbackProc)HelpRequestCB,
                              HELP_CREATE_DIR_DIALOG_STR);
                ...
                XtAddCallback(file_type_box, XmNhelpCallback,
                              (XtCallbackProc)HelpRequestCB,
                              HELP_CREATE_DIR_DIALOG_STR);
                XtAddCallback(file_type, XmNhelpCallback,
                              (XtCallbackProc)HelpRequestCB,
                              HELP_CREATE_DIR_DIALOG_STR);
                XtAddCallback(ok, XmNhelpCallback,
                              (XtCallbackProc)HelpRequestCB,
                              HELP_CREATE_DIR_DIALOG_STR);
                XtAddCallback(apply, XmNhelpCallback,
                              (XtCallbackProc)HelpRequestCB,
                              HELP_CREATE_DIR_DIALOG_STR);
                XtAddCallback(cancel, XmNhelpCallback,
                              (XtCallbackProc)HelpRequestCB,
                              HELP_CREATE_DIR_DIALOG_STR);
                XtAddCallback(preview, XmNhelpCallback,
                              (XtCallbackProc)HelpRequestCB,
                              HELP_CREATE_DIR_DIALOG_STR);
...
static void MakeFileOk(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        int maxFileNameLength =
            pathconf(call_struct->directory_name, _PC_NAME_MAX);
        unsigned int btnMask;
        ...
        */
                ...
                if (type == DtDIRECTORY) {
                        title = XtNewString(
                            GETMESSAGE(10, 20, "Create Folder Error"));
                        msg = XtNewString(
                            GETMESSAGE(10, 23, "Invalid folder name."));
                } else {
                        title = XtNewString(
                            GETMESSAGE(10, 21, "Create File Error"));
                        msg = XtNewString(
                            GETMESSAGE(10, 24, "Invalid file name."));
                }
        }
#ifdef _CHECK_FOR_SPACES
        else if (DtStrchr(new_name, ' ') != NULL ||
...
static void MakeFileDone(XtPointer client_data, char *to, int rc) {
        ...
        if (rc == EINVAL) {
                ...
                if (rc == EEXIST)
                        msg =
                            XtNewString(GetSharedMessage(CANT_OVERWRITE_ERROR));
                else
        ...
        if (rc == 0) {
                 ...
                 * won't work anymore, because at this point, we won't yet be
                 * able to find the new file in the cached directory.
                 *
                 * Possible solutions:
                ...
                for (i = 0; i < file_mgr_data->directory_count; i++) {
                        directory_set =
                            (DirectorySet *)file_mgr_data->directory_set[i];
                        if (strcmp(directory_set->name,
                                   call_struct->directory_name) == 0)
                                break;
                }
