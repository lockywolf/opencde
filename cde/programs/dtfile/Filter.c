...
static DialogResource resources[] = {
    {"matchFilter", XmRBoolean, sizeof(Boolean),
     XtOffset(FilterDataPtr, match_flag), (XtPointer)True, _DtBooleanToString},
...
static void filterTypeChanged(Widget w, XtPointer client_data,
                              XtPointer call_data);
static int FileTypeAscending(FTData **t1, FTData **t2);
...
static void Create(Display *display, Widget parent, Widget *return_widget,
        ...
        resolvedFilterTranslationString = ResolveTranslationString(
            FilterTranslationString, (char *)filter_rec);
        ...
        label_string = XmStringCreateLocalized(
            (GETMESSAGE(13, 31, "Select Datatypes to be:")));
        n = 0;
        ...
        n++;
        fileTypeScrollWin =
            XmCreateScrolledWindow(form, "filetypeSWin", args, n);
        ...
        n++;
        file_window =
            XmCreateDrawingArea(fileTypeScrollWin, "file_window", args, n);
        ...
        label_string =
            XmStringCreateLocalized((GETMESSAGE(13, 21, "Select All")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, label_string);
        n++;
        ...
        label_string =
            XmStringCreateLocalized((GETMESSAGE(13, 22, "Deselect All")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, label_string);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
        ...
        n++;
        unselectAll =
            XmCreatePushButtonGadget(sel_form, "deselectAll", args, n);
...
static void InstallClose(FilterRec *filter_rec, XtCallbackProc callback,
        ...
        XtAddCallback(filter_rec->close, XmNactivateCallback, callback,
                      client_data);

        delete_window_atom = XmInternAtom(XtDisplay(filter_rec->shell),
...
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list) {
        ...
        _DtDialogGetResources(data_base, name_list, FILE_FILTER,
                              (char *)filter_data, resources,
...
static FilterData *CopyFilterData(FilterData *filter_data) {
        ...
        for (i = 0; i < filter_data->count; ++i) {
                CopiedFilterData->user_data[i] =
                    (FTData *)XtMalloc(sizeof(FTData));
...
static void SetValues(FilterRec *filter_rec, FilterData *filter_data) {
        ...
        for (i = 0, x = 5, count = 0; i < filter_data->count; i++) {
                ...
                if ((ptr = (char *)DtDtsDataTypeToAttributeValue(
                        ...
                        icon_label =
                            XmStringCreateLocalized(user_data->filetype);
                }

                pixmapData =
                    _DtRetrievePixmapData(user_data->filetype, NULL, NULL,
                ...
                if (i - count < num_children) {
                        ...
                        if (mgr->composite.num_children == 0) {
                                ...
                                n++;
                                icon_widget =
                                    _DtCreateIcon((Widget)mgr, "icon", args, n);
                        } else {
                                icon_widget = _DtDuplicateIcon(
                                    (Widget)mgr, mgr->composite.children[0],
                                    (XmString)args[0].value,
                                    (String)args[1].value, (XtPointer)user_data,
                                    (Boolean)args[2].value);
                                n = 0;
                                XtSetArg(args[n], XmNx, x);
                                n++;
                                XtSetArg(args[n], XmNy, 5);
                ...
                _DtCheckAndFreePixmapData(
                    user_data->filetype, filter_rec->file_window,
                ...
                if (XtIsManaged(mgr->composite.children[i])) {
                        if (mgr->composite.children[i]->core.height != max) {
                                ...
                                XtSetArg(args[n], XmNy,
                                         5 + (max - mgr->composite.children[i]
                                                        ->core.height));
                                n++;
                                XtSetValues(mgr->composite.children[i], args,
                                            n);
                        }
                }
        }
...
static void OkCallback(Widget w, XtPointer client_data, XtPointer call_data) {
        FilterRec *filter_rec =
            (FilterRec *)((FilterApply *)client_data)->filter_rec;
        DialogData *dialog_data;
...
static void ApplyCallback(Widget w, XtPointer client_data,
        ...
        for (i = 0, count = 0;
             i < num_children && i + count < new_filter_data->count; i++) {
                XtSetArg(args[0], XmNuserData,
                         new_filter_data->user_data[i + count]);
...
static Boolean GetFilterValues(FilterRec *filter_rec, FilterData *filter_data) {
        ...
        if (dialog_data != NULL) {
                ...
                for (i = 0; i < filter_data->count; i++) {
                        ...
                        filter_data->user_data[i] =
                            (FTData *)XtMalloc(sizeof(FTData));
                        filter_data->user_data[i]->filetype = XtNewString(
                            old_filter_data->user_data[i]->filetype);
                        filter_data->user_data[i]->index =
...

{
        ...
        filter_rec =
            (FilterRec *)_DtGetDialogInstance(file_mgr_data->filter_edit);
...
int ReadInFiletypes(FilterData *filter_data) {
        ...
        qsort(filter_data->user_data, count, sizeof(FTData *),
              (int (*)())FileTypeAscending);
        ...
        for (i = 0; i < filter_data->filetypesFilteredCount; i++) {
                ...
                for (j = 0; j < count; j++) {
                        if (strcmp(file_type,
                                   filter_data->user_data[j]->filetype) == 0) {
...

{
        ...
        if (filter_rec && result) {
                ...
                num_children = mgr->composite.num_children;
                for (i = 0, count = 0;
                     i < num_children && i + count < filter_data->count; i++) {
                        XtSetArg(args[0], XmNuserData,
                                 new_filter_data->user_data[i]);
                        XtSetValues(mgr->composite.children[i], args, 1);
                }
                /* Display the new type  */
                SetValues(filter_rec, filter_data);
