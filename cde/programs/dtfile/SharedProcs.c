...
char *_DtPName(register char *name)
...
Boolean _DtStringsAreEquivalent(register char *in_str,
        ...
        if (is_multibyte) {
                for (;;) {
                        ...
                        /* No match, if the two characters have different byte
                         * counts */
                        if (c1size != c2size)
                                return (False);
...

{
        ...
        */
        if (path[0] != 0x0) {
                pixmapData->instanceIconName = DtDtsDataTypeToAttributeValue(
                    dataType, DtDTS_DA_INSTANCE_ICON, path);
        } else
                pixmapData->instanceIconName = NULL;

        if (pixmapData->instanceIconName == NULL) {
                pixmapData->iconName = DtDtsDataTypeToAttributeValue(
                    dataType, DtDTS_DA_ICON, NULL);
        ...
        if (pixmapData->size == LARGE)
                pixmapData->iconFileName = _DtGetIconFileName(
                    XtScreen(shell), pixmapData->instanceIconName,
                    pixmapData->iconName, pixmapData->hostPrefix, DtMEDIUM);
        else
                pixmapData->iconFileName = _DtGetIconFileName(
                    XtScreen(shell), pixmapData->instanceIconName,
                    pixmapData->iconName, pixmapData->hostPrefix, DtTINY);
        ...
        */
        if (pixmapData->instanceIconName) {
                ...
                if (iconGadget->icon.pixmap == 0 ||
                    ...
                    (Dimension)iconGadget->icon.pixmap_width >
                        (Dimension)instanceWidth ||
                    (Dimension)iconGadget->icon.pixmap_height >
                        (Dimension)instanceHeight) {
        ...
        *action_args =
        ...
        if (*action_args) {
                ...
                for (i = 0; i < selected_count; i++) {
                        ...
                        ((*action_args)[(*arg_count)++]).u.file.name =
                            _DtGetSelectedFilePath(
                                selection_list[i]->file_view_data);
                }
        }
}
        ...
        *arg_count = 0;
        ...
        if (dropped_on_obj)
                *action_args = (DtActionArg *)XtCalloc(
                    1, sizeof(DtActionArg) *
                           (drop_parameters->dropData->numItems + 1));
        else
                *action_args = (DtActionArg *)XtCalloc(
                    1,
                    sizeof(DtActionArg) * drop_parameters->dropData->numItems);
        ...
        if (*action_args) {
                ...
                for (i = 0; i < drop_parameters->dropData->numItems; i++) {
                        ...
                        ((*action_args)[(*arg_count)++]).u.file.name =
                            XtNewString(
                                drop_parameters->dropData->data.files[i]);
                }
        }
}
...

{
        *arg_count = 0;
        ...
        if (*action_args) {
                ...
                for (i = 0; i < drop_parameters->dropData->numItems; i++) {
                        buffer_arg.bp =
                            drop_parameters->dropData->data.buffers[i].bp;
                        buffer_arg.size =
                            drop_parameters->dropData->data.buffers[i].size;
                        buffer_arg.name =
                            drop_parameters->dropData->data.buffers[i].name;

                        ((*action_args)[(*arg_count)]).argClass =
                            DtACTION_BUFFER;
...

{
        ...
        /* Initialize file_set and ensure of unique file names for unamed
         * buffers*/
        ...
        for (i = 0; i < num_of_buffers; i++) {
              ...
              buffer_set[%d].buf_ptr=%p, buffer_set[%d].size=%d\n",
                         i, host_set[i], i, buffer_set[i].buf_ptr, i,
                         buffer_set[i].size));
        }
}
...
static void SetBufferFileNames(char **file_set, DtDndBuffer *buffers,
        ...
        for (i = 0; i < num_of_buffers; i++) {
                if (buffers[i].name == NULL) {
                        ...
                        file_set[i] =
                            BuildBufferFileName(DEFAULT_BUFFER_FILENAME, -1,
...

{
        ...
        exe_attribute = DtDtsBufferToAttributeValue(buffer, buffer_size,
                                                    "IS_EXECUTABLE", NULL);
        if (exe_attribute) {
...

{
        ...
        *host = XtNewString(home_host_name);
        ...
        /* Resolve, if there're any, environment variables */
        {
                ...
                if ((pfp = popen(command, "r")) != NULL) {
                        ...
                        if (NULL == (fgets(command, MAXPATHLEN, pfp))) {
                                /*
                                 * Try a few more reads and if the read still
                                 * fails, just use the path as is.
                                 */
                                int i;
                                for (i = 0; i < 5; i++) {
                                        sleep(1);
        ...
        if (path) {
                ...
                true_path = ResolveLocalPathName(*host, path, NULL,
                ...
                if (path) {
                        ...
                        *path = '/';
                        ...
                        if (strcmp(pwInfo->pw_dir, "/") == 0) {
                                /* We don't want to end up with double '/' in
                                 * the path */
