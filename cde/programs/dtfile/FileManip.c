 ...
 *
 ************************************************************************/
static int Check(Widget w, register char *spth, register ino_t dinode, int mode,
                 void (*errorHandler)()) {
        struct stat sbuf;
...
static Boolean MoveDir(Widget w, register char *source, register char *target,
                       struct stat *sourceStatInfo, void (*errorHandler)(),
        ...
        *targetRtn = NULL;
        ...
        if (val < 0 || val1 < 0) {
                if (errorHandler) {
                        tmpStr = GETMESSAGE(
                            11, 14, "Cannot find the folders location.");
                        msg = XtNewString(tmpStr);
                        (*errorHandler)(w, msg, NULL);
                        XtFree(msg);
                }
        ...
        if (s1.st_dev != s2.st_dev) {
                ...
                if (child_pid == -1) {
                        if (errorHandler) {
                                tmpStr = GETMESSAGE(
                                    ...
                                    "number of "
                                    "processes for this system has been "
                                    "reached.\nStop some "
                                    "of the processes or programs that are "
                                    "currently\nrunning and then retry this "
                                    "function.");
                                msg = XtNewString(tmpStr);
                                (*errorHandler)(w, msg, NULL);
                                XtFree(msg);
                ...
                if (child_pid == 0) {
                        ...
                        /* pass in geometry, and other command lines params when
                         * available */
                        if (type == TRASH_DIRECTORY)
                                rc = execlp(DTCOPY, "dtfile_copy", "-move",
                                            "-confirmReplace", "-confirmErrors",
                                            "-popDown", "-checkPerms", source,
                                            target, NULL);
                        else
                                rc = execlp(DTCOPY, "dtfile_copy", "-move",
                                            "-confirmReplace", "-confirmErrors",
                                            "-popDown", source, target, NULL);
 ...
 *
 ************************************************************************/
static Boolean CopyDir(Widget w, int mode, register char *from,
                       register char *to, Boolean isContainer, struct stat *s1,
        ...
        if (target_rc >= 0) {
                ...
                if (rc < 0 || (s2.st_mode & S_IFMT) != S_IFDIR) {
                        if (errorHandler) {
                                tmpStr = GETMESSAGE(
                                    11, 14,
                                    "Cannot find the folders location.");
                                msg = XtNewString(tmpStr);
                                (*errorHandler)(w, msg, NULL);
                                XtFree(msg);
                        }
        ...
        if (s1->st_dev == s2.st_dev) {
                if (target_rc >= 0) {
                        if (Check(w, to, s1->st_ino, COPY_FILE, errorHandler))
                                return False;
                } else /* destination dir does not exist, look at its proposed
                          parent */
                {
                        if (Check(w, target_dir, s1->st_ino, COPY_FILE,
                                  errorHandler))
                                return False;
                }
        }
        ...
        if (child_pid == -1) {
                if (errorHandler) {
                        tmpStr = GETMESSAGE(
                            ...
                            "some of the processes or programs that are "
        ...
        if (child_pid == 0) {
                ...
                if (mode == MERGE_DIR)
                        ...
                        rc = execlp(DTCOPY, "dtfile_copy", "-dontDelete",
                                    "-forceCopies", "-copyTop",
                                    "-confirmReplace", "-confirmErrors",
                                    "-popDown", from, target, 0);
                else
                        /* replace target dir */
                        rc = execlp(DTCOPY, "dtfile_copy", "-forceCopies",
                                    "-copyTop", "-confirmErrors", "-popDown",
                                    from, target, 0);
 ...
 *
 ************************************************************************/
Boolean FileManip(Widget w, int mode, register char *from, register char *to,
                  Boolean isContainer, /* described in function CopyDir */
                  void (*errorHandler)(), Boolean checkForBusyDir, int type) {
        register int fold;
        register int fnew;
        register int n;
        ...
        if (stat(from, &s1) < 0) {
                if (lstat(from, &s1) < 0) {
                        if (errorHandler) {
                                tmpStr =
                                    (GETMESSAGE(11, 28, "%s cannot be found."));
                                msg = XtNewString(tmpStr);
                                (*errorHandler)(w, msg, from);
                                XtFree(msg);
                        }
        ...
        if ((s1.st_mode & S_IFMT) == S_IFDIR) /* from is a directory */
        {
                if (mode == COPY_FILE || mode == MERGE_DIR) {
                        ...
                        copy_dir_return =
                            CopyDir(w, mode, from, to, isContainer, &s1,
                ...
                if (mode == MOVE_FILE) {
                        if (checkForBusyDir && DirectoryBusy(from)) {
                                if (errorHandler) {
                                        ...
                                        tmpStr = (GETMESSAGE(
                                            11, 30,
                                            "Cannot move or rename the folder "
                                            "%s.\nAll File "
                        ...
                        move_dir_return = MoveDir(
                            w, from, to, &s1, errorHandler, &realTarget, type);
                ...
                if (mode == LINK_FILE) {
                        ...
                        if (type == DESKTOP) {
                                ...
                                *ptr = '\0';
                                ...
                                /* if we don't get "/Desktop" then there is
                                   another filename attached to the end of the
                                   to name passed in */
                        ...
                        if ((!restricted && type != TRASH_DIRECTORY) &&
                            isContainer) {
                                DtLastChar(to, &cptr, &len);
                                if ((len != 1) || (*cptr != '/'))
                                        (void)strcat(filename, "/");
                        ...
                        } else if ((!restricted && type != TRASH_DIRECTORY) &&
                                   isContainer)
                                (void)strcat(filename, DName(from));

                        to = filename;

                        if (((link_result = symlink(from, to)) != 0) &&
                            errorHandler) {
                                if (type == NOT_DESKTOP || errno != EEXIST) {
                                        char *tmpStr;

                                        tmpStr =
                                            GetSharedMessage(CANT_CREATE_ERROR);
                                        msg = XtNewString(tmpStr);
                                        (*errorHandler)(w, msg, to);
                                        XtFree(msg);
                                }
        ...
        if (CheckAccess(from, R_OK) == -1) {
                /*
                 * A move operation does not require read permission, but a copy
                 * does.
                 */
                if (mode == COPY_FILE) {
                        if (errorHandler) {
    ...
    defined(CSRG_BASED)
        ...
        if (mode != COPY_FILE) {
                /* Try to maintain symbolic links, except when we're doing a
                 * link! */
                if (((s1.st_mode & S_IFMT) == S_IFLNK) && (mode != LINK_FILE)) {
                        char link_path[MAX_PATH + 1];
                        int link_len;

                        if ((link_len = readlink(from, link_path, MAX_PATH)) >
                            0) {
                                ...
                                if (mode == MOVE_FILE)
                                        if (RunFileCommand(MOVE_CMD, from, to,
                                                           NULL) == 0)
                                                return (True);
                                        else if (RunFileCommand(LINK_CMD, "-s",
                                                                from, to) == 0)
                                                return (True);
                ...
                if ((mode == MOVE_FILE) && (link_result >= 0)) {
                        if (unlink(from) < 0) {
                                if (errorHandler) {
                                        ...
                                        tmpStr =
                                            GetSharedMessage(CANT_DELETE_ERROR);
                                        msg = XtNewString(tmpStr);
                                        (*errorHandler)(w, msg, from);
                                        XtFree(msg);
                                }
        ...
        while (n = read(fold, buf, BLOCK_SIZE)) {
                ...
                if (result != n) {
                        ...
                        if (errno) {
                                ...
                                /* If catmsg is NULL then one of the
                                 * miscellanous error's occured. Set up a
                                 * generic error message which will output the
                                ...
                                /* Build a concatination of the possible message
                                 * parts */
                ...
                if (unlink(from) < 0) {
                        if (errorHandler) {
                                ...
                                tmpStr = (GETMESSAGE(
                                    ...
                                    "and\n  - drag the object with your "
                                    "mouse.\nOr\n  - use 'Copy To' "
                                    "in the 'Selected' menu popup of the menu "
                                    "bar."));
                                msg = XtNewString(tmpStr);
