...
static void ErrorHandler(Display *disp, XErrorEvent *event);
...
static void ExitApp(XtPointer clientData, String *words, int num_fields);
...
int main(int argc, char **argv) {
        ...
        {
                int numMouseButtons =
                    XGetPointerMapping(display, (unsigned char *)NULL, 0);
        ...
        XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                          "OpenFolder", (XtConverter)DtfileCvtStringToOpenDir,
                          NULL, 0);
        XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                          "DesktopIcon", (XtConverter)DtfileCvtStringToDTIcon,
                          NULL, 0);
        XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                          "Tree", (XtConverter)DtfileCvtStringToTree, NULL, 0);
        XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                          "TreeFiles", (XtConverter)DtfileCvtStringToTreeFiles,
                          NULL, 0);
        XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                          "View", (XtConverter)DtfileCvtStringToView, NULL, 0);
        XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                          "Order", (XtConverter)DtfileCvtStringToOrder, NULL,
                          0);
        XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                          "Direction", (XtConverter)DtfileCvtStringToDirection,
                          NULL, 0);
        XtAppAddConverter(XtWidgetToApplicationContext(toplevel), XtRString,
                          "Grid", (XtConverter)DtfileCvtStringToGrid, NULL, 0);
        ...
        printf("    done Setting up Converters and got resources, time: "
               "%ld.%ld\n\n",
               update_time_f.tv_sec - update_time_s.tv_sec,
               update_time_f.tv_usec - update_time_s.tv_usec);
                ...
                if (application_args.session != NULL) {
                        ...
                        if (current_directory[0] != '\0') {
                                msg = tttk_message_create(
                                    0, TT_REQUEST, TT_SESSION, 0,
                                    "DtFolder_Show",
                                    (Tt_message_callback)ExitApp);
                                tt_message_file_set(msg, current_directory);
                        } else {
                                msg = tttk_message_create(
                                    0, TT_REQUEST, TT_SESSION, 0, "DtHome_Show",
                                    (Tt_message_callback)ExitApp);
                        }
                        SetupSendRequestArgs(application_args, msg);
                        tt_message_send(msg);
        ...
        if (TT_OK != status) {
                ...
                title = GETMESSAGE(21, 38, "File Manager Error");
                errfmt = GETMESSAGE(
                    18, 40, "Could not connect to ToolTalk:\n%s\nExiting ...");
                statmsg = tt_status_message(status);
        ...
        if (application_args.directories != NULL ||
            ...
            restrictMode) {
                special_view = True;
                if (strcmp(application_args.restricted, RESTRICTED_HEADER) ==
                    0) {
                        if (application_args.directories != NULL)
                                special_restricted =
                                    XtNewString(application_args.directories);
                        else {
                                /* Get users pwd so we can set the restricted
                                 * dir to it */
                                GetPWD(current_directory);

                                if (current_directory[0] != '\0')
                                        special_restricted =
                                            XtNewString(current_directory);
                                else
                ...
                if (application_args.help_volume == NULL) {
                        ...
                        special_helpVol =
                            XtNewString(application_args.help_volume);
                        fileMgrHelpVol =
                            XtNewString(application_args.help_volume);
                }
        ...
        if (restrictMode) {
                if (application_args.directories != NULL &&
                    strncmp(application_args.directories, "~", 1) != 0) {
                        if (strncmp(application_args.directories,
                                    users_home_dir,
        ...
        if (dt_path == NULL) {
                ...
                tmpStr = GETMESSAGE(
                    18, 2, "Could not create the ~/.dt folder or sub folders.");
                ...
                while (TRUE) {
                        XtAppProcessEvent(XtWidgetToApplicationContext(dialog),
                                          XtIMAll);
                }
        }
        ...
        if (special_view == True && special_restricted != NULL) {
                if (strncmp(special_restricted, "~", 1) == 0) {
                        ...
                        special_restricted =
                            _DtChangeTildeToHome(special_restricted);
        ...
        */
        ...
        if (strcmp(application_args.no_view, "-noview") != 0) {
                if (application_args.session != NULL)
                        session_flag = RestoreSession(application_args.session,
                                                      NORMAL_RESTORE, NULL);
                else if (application_args.directories != NULL) {
        ...
        if ((strcmp(application_args.no_view, "-noview") != 0) &&
            ((application_args.session == NULL) || (session_flag != 0))) {
                if (view_count == 0) {
                        /* Get users pwd so we can create a fileviewer window of
                         * it */
                        GetPWD(current_directory);

                        if (current_directory[0] != '\0') {
                                if (!GetNewView(home_host_name,
...
static void Usage(char **argv) {
        ...
        char *message_string1 =
            ...
            "Dtfile uses the help volume specified in the HelpVolume "
            "parameter.\n\n"
            "   -restricted\n\n\t"
            "Dtfile will not display folders above the restricted folder.\n\t"
            "If the -dir option is used, the folder specified in that "
            "option\n\t"
        ...
        char *message_string2 =
            ...
            "never  = Tree mode has two states: partially expanded or "
            "collapsed.\n\t"
            "always = Tree mode has two states: fully expanded or "
            "collapsed.\n\t"
            "choose = Tree mode has three states: partially expanded, fully\n\t"
            "         expanded, or collapsed.\n\n"
            "   -order alphabetical/file_type/date/size\n\n\t"
            "Files are displayed in the specified order: alphabetical, by "
            "file\n\t"
            "type, by date, or by size.\n\n"
            "   -view no_icon/large_icon/small_icon/attributes\n\n\t"
            "Files are displayed in the specified format: text only, text "
            "and\n\t"
            ...
            "        The display area size for the icon images in File "
            "Manager\n"
            "        Icon images larger than this size will be clipped to this "
            "size\n"
            "        The default display area size for large is 38 and small "
            "is "
            "24\n\n"
            "\n";
...
static void RestoreSettingsFile(void) {
        ...
        XGetWindowProperty(display, RootWindow(display, 0), save_mode, 0L,
                           ...
                           &actualFormat, &nitems, &leftover, &data);
        ...
        toolSavePath =
            (char *)XtMalloc(strlen(dtPath) + strlen(TOOL_SETTINGS_FILENAME) +
                             strlen(dirName) + 4);

        homeSavePath =
            (char *)XtMalloc(strlen(dtPath) + strlen(HOME_SETTINGS_FILENAME) +
                             strlen(dirName) + 4);

        sprintf(homeSavePath, "%s/%s/%s", dtPath, dirName,
                HOME_SETTINGS_FILENAME);
        sprintf(toolSavePath, "%s/%s/%s", dtPath, dirName,
        ...
        if ((fd = open(homeSavePath, O_RDONLY)) != -1) {
                ...
                homeHomePath = (char *)XtMalloc(
                    strlen(dtPath) + strlen(HOME_SETTINGS_FILENAME) + 2);
                ...
                /* copy the saved one into $HOME/.dt/$DISPLAY ... this is the
                 * one that will be used by the dtfile
                 */
                ...
                if (status != 0) {
                        ...
                        msg = XtNewString(tmpStr);
                        _DtSimpleErrnoError(application_name, DtWarning, NULL,
                                            msg, NULL);
                        XtFree(msg);
                }
        }
        ...
        if ((fd = open(toolSavePath, O_RDONLY)) != -1) {
                close(fd);
                /* create where the saved file is going to go */
                toolHomePath = (char *)XtMalloc(
                    strlen(dtPath) + strlen(TOOL_SETTINGS_FILENAME) + 2);
...
void SaveSettingsCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (file_mgr_data->restoreKind == HOME_RESTORE) {
                tmpStr = (GETMESSAGE(
                    18, 29,
                    "Save the current File Manager View window "
                    "size,\npreferences, and filter settings?\n\nThey "
                    "will be used for any new File Manager View "
                    "windows\nstarted from the Front Panel."));
        } else {
                tmpStr = (GETMESSAGE(
                    18, 35,
                    "Save the current Application Manager\nView window "
                    "size, preferences, and filter settings?\n\nThey will "
                    "be used for any new Application Manager\nwindows "
                    "started from the Front Panel."));
        }
        ...
        if (file_mgr_data->title != NULL &&
            strcmp(file_mgr_data->helpVol, DTFILE_HELP_NAME) != 0) {
                ...
                tmpStr = GETMESSAGE(18, 32,
                                    "File Manager - Save As Default Options");
                title = XtNewString(tmpStr);
        }
        dlog = (Widget)_DtMessageDialog(
            mbar, title, message, NULL, TRUE, SaveDefaultCancelCB,
            SaveDefaultOkCB, NULL, HelpRequestCB, False, QUESTION_DIALOG);
...
static void SaveDefaultOkCB(Widget w, XtPointer client_data,
        ...
        if ((fd = creat(full_path, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP)) ==
            -1) {
                char *title;
                tmpStr = GETMESSAGE(
                    18, 17,
                    "Unable to create a file to store the default options.");
                title =
                    XtNewString((GETMESSAGE(21, 38, "Object Create Error")));
                msg = XtNewString(tmpStr);
...
static void SaveSession(char *path) {
        static char *name_list[] = {
            DTFILE_CLASS_NAME, NULL, NULL, NULL, NULL, NULL};
        char view_number[5];
        char number[5];
        ...
        for (i = -1; i < view_count; i++) {
                ...
                /* This is a bug fix ... We don't want to save the title if the
                   View is the Trash Can of if it is an Application Manager.
                   This is because if the user saves a session in one Locale,
                   then logs in in another, the Title will be in the locale that
                   the session was saved in rather then the new local.  So let's
                   save the Title, Null it out, Save the session info, and
                   finally restore the Title.
                */
                ...
                if (DtWsmGetWorkspacesOccupied(
                        display, XtWindow(file_mgr_rec->shell), &ws_presence,
                        ...
                        for (j = 0; j < num_workspaces; j++) {
                                if (j != 0)
                                        (void)write(fd, "*", strlen("*"));
                                workspace_name =
                                    XGetAtomName(display, ws_presence[j]);
                                (void)write(fd, workspace_name,
                                            strlen(workspace_name));
                ...
                if (i == -1)
                        ...
                        _DtWriteDialogData(
                            (DialogData *)view_set[i]->dialog_data, fd,
                            name_list);
                (void)write(fd, "#\n", strlen("#\n"));
        ...
        for (i = 0; i < desktop_data->numWorkspaces; i++) {
                ...
                sprintf(view_number, "%d",
                        workspaceInfo->secondaryHelpDialogCount);
                write(fd, view_number, strlen(view_number));
                write(fd, "\n#\n", strlen("\n#\n"));
                ...
                for (j = 0; j < workspaceInfo->secondaryHelpDialogCount; j++) {
                        ...
                        _DtWriteDialogData(
                            workspaceInfo->secondaryHelpDialogList[j], fd,
                            name_list);
                }
                ...
                if (workspaceInfo->primaryHelpDialog) {
                        sprintf(number, "%d", 0);
                        _DtWriteDialogData(workspaceInfo->primaryHelpDialog, fd,
                                           name_list);
                }
                write(fd, "#\n", strlen("#\n"));
        }
...
void ForceMyIconOpen(char *host_name, char *directory_name) {
        ...
        for (i = 0; i < view_count; i++) {
                ...
                if (strcmp(host_name, view_set[i]->host_name) == 0) {
                        for (j = 0; j < fmd->directory_count; j++) {
                                if (strcmp(parent,
                                           fmd->directory_set[j]->name) == 0) {
                                        for (k = 0;
                                             k <
                                             fmd->directory_set[j]->file_count;
                                             k++) {
                                                file_view_data =
                                                    fmd->directory_set[j]
                                                        ->file_view_data[k];
                                                if (strcmp(file_view_data
                                                               ->file_data
                                                               ->file_name,
                                                           fname) == 0)
                                                        break;
                ...
                if (strcmp(buf, directory_name) == 0 &&
                    strcmp(desktopWindow->host, host_name) == 0) {
                        file_type_name =
                            file_view_data->file_data->logical_type;
                        if (desktopIconType == LARGE)
                                BuildAndShowIconName(file_type_name,
                                                     BY_NAME_AND_ICON,
                                                     SINGLE_DIRECTORY,
                                                     desktopWindow->iconGadget);
                        else /* SMALL */
                                BuildAndShowIconName(file_type_name,
                                                     BY_NAME_AND_SMALL_ICON,
                                                     SINGLE_DIRECTORY,
                                                     desktopWindow->iconGadget);
                }
        }
...
static int RestoreSession(char *path, int type_of_restore, char *directory) {
        ...
        if (type_of_restore == NORMAL_RESTORE) {
                ...
                if (stat(full_path, &stat_buf) != 0) {
                        ...
                        tmpStr = GETMESSAGE(18, 18,
                                            "Could not open the session file.");
                        msg = XtNewString(tmpStr);
                        title = XtNewString(
                            (GETMESSAGE(21, 39, "File Open Error")));
                        _DtMessage(toplevel, title, msg, NULL, HelpRequestCB);
                        XtFree(msg);
                        XtFree(title);
                        XtFree(full_path);
        ...
        if (type_of_restore == NORMAL_RESTORE) {
                ...
                xrm_name[2] = 0;
                if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type,
                                    &value)) {
        ...
        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type, &value)) {
                ...
                if (num_views > 0)
                        LoadViews(num_views, db, NULL, directory,
                                  special_restricted, 0);
        }
        ...
        if (type_of_restore == NORMAL_RESTORE) {
                for (i = 0; i < desktop_data->numWorkspaces; i++) {
                        ...
                        xrm_name[3] =
                            XrmStringToQuark(SEC_LOAD_HELP_RES_HEADER);
                        xrm_name[4] = 0;
                        ...
                        if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type,
                                ...
                                if (num_sec_help_dialogs > 0) {
                                        ...
                                        for (j = 0; j < num_sec_help_dialogs;
                                             j++) {
                                                name_list[0] =
                                                    DTFILE_CLASS_NAME;
                                                name_list[1] =
                        ...
                        dialogData = _DtGetResourceDialogData(help_dialog, db,
                                                              name_list);
                        ...
                        desktop_data->workspaceData[i]->primaryHelpDialog =
                            dialogData;
                        if (dialogData) {
                                ShowDTHelpDialog(
                                    NULL, i, MAIN_HELP_DIALOG, dialogData, NULL,
...

{
        ...
        for (i = 0; i < num_views; i++) {
                ...
                dialog_data =
                    _DtGetResourceDialogData(file_mgr_dialog, db, name_list);
                file_mgr_data = (FileMgrData *)dialog_data->data;
                ...
                if (trashFileMgrData && (file_mgr_data->IsTrashCan == True) &&
                    strcmp(file_mgr_data->current_directory, trash_dir) == 0) {
                        ...
                        trashFileMgrData->positionEnabled =
                            file_mgr_data->positionEnabled;
                        trashFileMgrData->preferences =
                ...
                /* Increment the view list size if necessary and add directory
                 * to list */

                if (view_count == view_set_size) {
                        view_set_size += 10;
                        view_set = (View **)XtRealloc(
                            (char *)view_set, sizeof(View **) * view_set_size);
                }
                ...
                if (directory_name == NULL) {
                        view_set[view_count]->host_name =
                            XtNewString(file_mgr_data->host);
                ...
                if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type,
                                    &value)) {
                        /*  Make sure we have some valid workspaces names to
                         * work with */
                ...
                if (XrmQGetResource(db, xrm_name, xrm_name, &rep_type,
                                    &value)) {
                        /*  If there is an iconify resource and its value is
                         * True,  */
                ...
                _DtShowDialog(NULL, NULL, NULL, dialog_data, NULL, NULL,
                              RemoveTextFields, NULL, workspaces,
                              iconify_window, special_view, title, &classHints);
...
static void GetPWD(char current_directory[]) {
        ...
        if (pwd_file != NULL) {
                while (1) {
                        errno = 0;
                        if (fread(&current_directory[i], sizeof(char), 1,
                                  pwd_file) != 1) {
                                if (errno == EINTR)
                                        continue;
                                else
...
static void OpenDirectories(char *directory_set, char *type) {
        ...
        while (1) {
                ...
                if (path) {
                        ...
                        tmpStr = GETMESSAGE(
                            32, 2, "File Manager Open Directory Error");
                        errTitle = XtNewString(tmpStr);
                        tmpStr = GETMESSAGE(18, 38,
                                            "Invalid folder specification, %s");
                        errMsg = XtNewString(tmpStr);
                        dmsg = XtMalloc(strlen(errMsg) + strlen(directory_set) +
                                        1);
                        sprintf(dmsg, errMsg, directory_set);
                        _DtMessage(toplevel, errTitle, dmsg, NULL,
                                   HelpRequestCB);
...
DialogData *GetNewView(char *host_name, char *directory_name, char *type,
        ...
        if (stat(directory_name, &stat_buf) == 0) {
                ...
                tmpStr = GETMESSAGE(
                    18, 20, "The folder specification,\n%s\ndoes not exist.");
                errMsg = XtNewString(tmpStr);
        }
        ...
        if (openDirType == NEW ||
            (strcmp(real_directory_name, desktop_dir) == 0)) {
                for (i = 0; i < view_count; i++) {
                        if ((strcmp(real_directory_name,
                                    view_set[i]->directory_name) == 0 &&
                             (strcmp(host_name, view_set[i]->host_name) ==
                              0))) {
                                Window rootWindow;
                                Atom pCurrent;
                                ...
                                dialog_data =
                                    (DialogData *)view_set[i]->dialog_data;
                                file_mgr_data =
                                    (FileMgrData *)dialog_data->data;
                                file_mgr_rec =
                                    (FileMgrRec *)file_mgr_data->file_mgr_rec;

                                screen = XDefaultScreen(display);
                                currentScreen =
                                    XScreenOfDisplay(display, screen);
                                rootWindow = RootWindowOfScreen(currentScreen);

                                /* Get the current Workspace */
                                if (DtWsmGetCurrentWorkspace(
                                        display, rootWindow, &pCurrent) ==
                                    Success) {
                                        Atom *ws_presence = NULL;
                                        unsigned long num_workspaces = 0;
                                        int k;
                                ...
                                /* must map the window to catch iconified
                                 * windows */
                                ...
                                {
                                        Tt_message msg;
                                        msg = tt_pnotice_create(
                                            TT_SESSION, "DtActivity_Began");
                                        tt_message_send(msg);
        ...
        if (view_count == view_set_size) {
                view_set_size += 10;
                view_set = (View **)XtRealloc((char *)view_set,
        ...
        if (initiating_view != NULL)
                ...
                tmp_path = _DtCreateDtDirs(display);
                if (type != NULL)
                        sprintf(full_path, "%s/%s", tmp_path,
                                TOOL_SETTINGS_FILENAME);
                else
                        sprintf(full_path, "%s/%s", tmp_path,
                                HOME_SETTINGS_FILENAME);
                ...
                if (db != NULL) {
                        if (type != NULL) {
                                ...
                                LoadViews(1, db, NULL, directory_name, type,
                                          msg);
                        } else {
                                restoreType = HOME_RESTORE;
                                LoadViews(1, db, host_name, directory_name,
                                          NULL, msg);
                        }
                        ...
                        XrmDestroyDatabase(db);
                        dialog_data =
                            (DialogData *)view_set[view_count - 1]->dialog_data;
                        file_mgr_data = (FileMgrData *)dialog_data->data;
                        if (type != NULL) {
                                application_args.tool_width =
                                    file_mgr_data->width;
                                application_args.tool_height =
                                    file_mgr_data->height;
                        } else {
                                application_args.dir_width =
                                    file_mgr_data->width;
                                application_args.dir_height =
                                    file_mgr_data->height;
                        }
                                    ...
                                    NULL) {
                                        if (restrictMode)
                                                find_data->directories =
                                                    XtNewString(users_home_dir);
                                        else
                                                find_data
                                                    ->directories = XtNewString(
                                                    file_mgr_data
                                                        ->current_directory);
                                } else {
                                        if (strcmp(
                        ...
                        /* If in novice mode, force the icon for all the dir's
                           to the 'Open' state */
                        if (openDirType == NEW)
                                ForceMyIconOpen(host_name, NULL);

                        return ((DialogData *)view_set[view_count - 1]
                                    ->dialog_data);
                } else
                        dialog_data = _DtGetDefaultDialogData(file_mgr_dialog);
        }
        ...
        if (initiating_view != NULL) {
                FileMgrPropagateSettings((FileMgrData *)initiating_view,
                                         file_mgr_data);
                /* force new window to come up in flat mode */
                file_mgr_data->show_type =
                    ((PreferencesData *)file_mgr_data->preferences->data)
                        ->show_type = SINGLE_DIRECTORY;
        ...
        if (type != NULL) {
                ...
                /* we want to default for the toolboxes (i.e. Application
                 * Manager) to look different from a normal File Manager view.
                 * Let's turn off the iconic_path, current_directory, and
                 * status_line.
                 */
                ...
                */
                ...
                preferences_data =
                    (PreferencesData *)file_mgr_data->preferences->data;
                preferences_data->show_iconic_path =
                    file_mgr_data->show_iconic_path;
                preferences_data->show_current_dir =
                    file_mgr_data->show_current_dir;
                preferences_data->show_status_line =
        ...
        _DtShowDialog(NULL, NULL, NULL, dialog_data, NULL, NULL,
                      RemoveTextFields, NULL, NULL, False, special_view, title,
                      &classHints);
        ...
        /* If in novice mode, force the icon for all the dir's to the 'Open'
         * state */
        if (openDirType == NEW)
                ForceMyIconOpen(host_name, NULL);
...
void CloseView(DialogData *dialog_data) {
        ...
        for (i = 0; i < view_count; i++) {
                if (dialog_data == (DialogData *)(view_set[i]->dialog_data)) {
                        ...
                        directory_name = (char *)XtMalloc(
                            strlen(view_set[i]->directory_name) + 1);
                        strcpy(directory_name, view_set[i]->directory_name);
                        host_name = (char *)XtMalloc(
                            strlen(view_set[i]->host_name) + 1);
                        ...
                        /* If it is an Application Manager view, then no point
                         in caching this dialog, so free it */
...
void DirectoryChanged(XtPointer file_mgr_data, char *old_host_name,
        ...
        for (i = 0; i < view_count; i++) {
                if (((DialogData *)(view_set[i]->dialog_data))->data ==
                    file_mgr_data) {
                        if (strcmp(old_host_name, view_set[i]->host_name) ==
                                0 &&
                            strcmp(old_directory_name,
                                   view_set[i]->directory_name) == 0) {
                                XtFree((char *)view_set[i]->host_name);
                                view_set[i]->host_name =
                                    XtNewString(new_host_name);

                                XtFree((char *)view_set[i]->directory_name);
                                view_set[i]->directory_name =
                                    XtNewString(new_directory_name);
                        }
                }
        }
...
static void ViewSessionHandler(Tt_message msg) {
        ...
        if (!tt_is_err(tt_ptr_error(file))) {
                if (view_count == 0 && desktop_data->numIconsUsed == 0) {
                        ...
                        session_flag =
                            RestoreSession(session_name, NORMAL_RESTORE, NULL);
                        ...
                        tmpStr = GETMESSAGE(18, 39,
                                            "Cannot start a second File "
                                            "Manager session because a "
                                            "File Manager\nsession is already "
                                            "open on the desktop.  "
                                            "If you want to start a new "
                                            "File\nManager session, you "
                                            "must first close all current File "
                                            "Manager windows\nand "
                                            "remove all File Manager objects "
                                            "from the workspaces.");
                        msg = XtNewString(tmpStr);
                        title =
                            XtNewString((GETMESSAGE(18, 40, "Session Error")));
                        _DtMessage(toplevel, title, msg, NULL, HelpRequestCB);
                        XtFree(msg);
                        XtFree(title);
                }
...
static void ViewDirectoryHandler(Tt_message msg) {
        ...
        if (restrictMode) {
                if (msgFile && strncmp(msgFile, "~", 1) != 0) {
                        if ((strcmp(users_home_dir, "/") != 0) &&
                            (strncmp(msgFile, users_home_dir,
...
DialogData *ViewDirectoryProc(char *root_dir, int restricted, Tt_message msg) {
        ...
        if (restrictMode) {
                if (root_dir && strncmp(root_dir, "~", 1) != 0) {
                        if ((strcmp(users_home_dir, "/") != 0) &&
                            (strncmp(root_dir, users_home_dir,
                                     strlen(users_home_dir) - 1) != 0)) {
                                char *tmpStr, *errTitle, *errMsg, *dmsg;
...
static void ViewToolsDirectoryHandler(Tt_message msg) {
        ...
        if (msgFile != NULL) {
                if (numArgs > 0) {
                        ...
                        for (i = 0; i < numArgs; i++) {
                                ...
                                } else if (strcmp(vtype,
                                                  "-common_link_point") == 0) {
                                        link_point = XtNewString(val);
                                } else if (strcmp(vtype,
                                                  "-user_install_point") == 0) {
                                        user_install_point = XtNewString(val);
                                }
                                if (strcmp(vtype, "-tree") == 0) {
                                        DtfileStringToTree(val,
                                                           &special_treeType);
                                } else if (strcmp(vtype, "-tree_files") == 0) {
                                        DtfileStringToTreeFiles(
                                            val, &special_treeFiles);
                                } else if (strcmp(vtype, VIEW_HEADER) == 0) {
                                        DtfileStringToView(val,
                                                           &special_viewType);
                                } else if (strcmp(vtype, "-order") == 0) {
                                        DtfileStringToOrder(val,
                                                            &special_orderType);
                                } else if (strcmp(vtype, "-direction") == 0) {
                                        DtfileStringToDirection(
                                            val, &special_directionType);
                                } else if (strcmp(vtype, "-grid") == 0) {
                                        DtfileStringToGrid(val,
                                                           &special_randomType);
                                } else if (strcmp(vtype, RESTRICTED_HEADER) ==
                                           0) {
                                        ;
                                }
                                tt_free(val);
                        ...
                        /* Due to the use of tooltalk messaging, app manager
                           objects that are dragged either to the desktop or to
                           the front panel are identified by their fully
                           resolved names (ie.
                           /usr/dt/appconfig/appmanager/$LANG/.... as opposed to
                           /var/dt/appconfig/appmanager/$DTUSERSESSION/....).
                           In order for the File Manager to treat these objects
                           as links existing in the /var/.... directory, we use
                           the following ugly code:

                             1. Search the incoming path for $LANG (if you can't
                           find $LANG, try C since this is the default).
                             2. Concatenate the path following $LANG to the path
                           for the root toolbox (this comes in with the tooltalk
                           message so that we don't have to hardcode it).
                             3. Call OpenDirectories with the new path that you
                           have created and with the root toolbox path as the
                           restricted directory.

                           This problem is further complicated by:
                             1. Users traversing into APPGROUPs.
                             2. Users opening the parent folders for workspace
                           APPGROUPs.

                           For situation 1, File Manager kicks the user into the
                           App Manager if he/she traverses into an APPGROUP.
                           BUT we don't want to make modifications to the path
                           as we do above. So, if there is nothing following
                           $LANG or if we can't find $LANG in the path, (ie.
                           /var/dt/appconfig/appmanager/$DTUSERSESSION), call
                           OpenDirectories with no parameter changes.

                           For situation 2, File Manager is unable to
                           distinguish between workspace objects dragged from
                           /usr/.... or /var/...., so the parent folder for all
                           workspace APPGROUPS is considered to be in the /var
                           path.  In addition to the OpenAppGroup action, there
                           is an OpenParentAppGroup action which also triggers
                           ViewToolsDirectoryHandler but sends in an additional
                           parameter (user_install_point).
                           If we know that this msg was generated by the
                           OpenParentAppGroup action AND nothing follows $LANG.
                             Modify the parameters as above.
                           OR we can't find $LANG but we can find the
                           user_install_point (user's personal appgroup path).
                             Concatenate the path following .dt/appmanager to
                           the path for the root toolbox. Call OpenDirectories
                           with the new path and the root toolbox as the
                           restricted directory.
                        */
                        if (root_toolbox) {
                                if (link_point) {
                                        ...
                                        if ((user_install_point) &&
                                            (ptr =
                                                 strstr(msgFile,
                                                        user_install_point))) {
                                                ptr +=
                                                    strlen(user_install_point);
                                                tool_dir = XtMalloc(
                                                    strlen(root_toolbox) +
                                                    strlen(ptr) + 1);
                                                sprintf(tool_dir, "%s%s",
                                                        root_toolbox, ptr);
                                                XtFree(special_restricted);
                                                special_restricted =
                                                    XtNewString(root_toolbox);
                                        } else {
                                                ptr =
                                                    strstr(msgFile, link_point);
        ...
        if (tmp_path == NULL) {
                ...
                if (stat(full_path, &stat_buf) == 0 && numArgs > 0) {
                        ...
                        restoreType = TOOL_RESTORE;
                        if (tool_dir)
                                RestoreSession(full_path, TOOL_RESTORE,
                                               tool_dir);
                        else
                                RestoreSession(full_path, TOOL_RESTORE,
                                               msgFile);
...
FileMgrData *ReturnDesktopPtr(Widget w) {
        ...
        for (i = 0; i < view_count; i++) {
                ...
                if (w == ((FileMgrRec *)file_mgr_data->file_mgr_rec)
                             ->file_window ||
...
PixmapData *CheckForOpenDirectory(FileViewData *order_list,
        ...
        for (i = 0; i < view_count; i++) {
                if (strcmp(real_dir_name, view_set[i]->directory_name) == 0) {
                        ...
                        new_file_type_name =
                            (char *)XtMalloc(strlen(file_type_name) +
                                             strlen(ICON_OPEN_PREFIX) + 1);
                        sprintf(new_file_type_name, "%s%s", ICON_OPEN_PREFIX,
                                file_type_name);
                        pixmapData = _DtRetrievePixmapData(
                            new_file_type_name, NULL, NULL,
        ...
        if (pixmapData == NULL || pixmapData->iconFileName == NULL) {
                ...
                pixmapData =
                    _DtRetrievePixmapData(logical_type, NULL, NULL,
                                          file_mgr_rec->file_window, icon_size);
        }
...
void CleanUp(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        /* CLEAN_UP_OP is not really a menu state.
           ...
           is being reread) not to reload icon positions from the .!dt<userid>
           file.
        */
        ...
        if ((file_mgr_data->show_type == SINGLE_DIRECTORY) &&
            ...
            (file_mgr_data->positionEnabled == RANDOM_ON)) {
                ((PreferencesData *)(file_mgr_data->preferences->data))
                    ->positionEnabled = RANDOM_ON;
        } else {
                /* Update the preferences dialog */
                ((PreferencesData *)(file_mgr_data->preferences->data))
                    ->positionEnabled = RANDOM_OFF;
        }
        ...
        for (j = 0; j < file_window->composite.num_children; j++) {
                if (XmIsTextField(file_window->composite.children[j]) &&
                    !file_window->composite.children[j]->core.being_destroyed) {
                        ...
                        XtGetValues(file_window->composite.children[j], args,
                                    1);

                        /* Find the associated icon data */
                        /* @@@ this won't work for tree mode! */
                        for (i = 0;
                             i < file_mgr_data->directory_set[0]->file_count;
                             i++) {
                                file_view_data = file_mgr_data->directory_set[0]
                                                     ->file_view_data[i];
                                if (strcmp(name, file_view_data->file_data
                                                     ->file_name) == 0) {
                                        ...
                                        x = textExtent.x;
                                        y = textExtent.y -
                                            (Dimension)(file_window->composite
                                                            .children[j]
                                                            ->core.height -
                                                        textExtent.height) /
                                                (Dimension)2;
...
#define OBJ_PLACE_TOP_STR (unsigned char *)"top"
        ...
        while (*pch && _DtNextToken(pch, &len, &pchNext)) {
                ...
                case 'B':
                case 'b':
                        if (_DtStringsAreEquivalent(
                                (char *)pch, (char *)OBJ_PLACE_BOTTOM_STR)) {
                                if (!fPrimarySet) {
                                        cval |= OBJ_PLACE_BOTTOM_PRIMARY;
                                        fPrimarySet = True;
                                } else if (!fSecondarySet) {
                                        if (!(cval & (OBJ_PLACE_BOTTOM_PRIMARY |
                                                      OBJ_PLACE_TOP_PRIMARY))) {
                                                cval |=
                                                    OBJ_PLACE_BOTTOM_SECONDARY;
                                                fSecondarySet = True;
                                        }
                                }
                        }
                ...
                case 'l':
                        if (_DtStringsAreEquivalent(
                                (char *)pch, (char *)OBJ_PLACE_LEFT_STR)) {
                                if (!fPrimarySet) {
                                        cval |= OBJ_PLACE_LEFT_PRIMARY;
                                        fPrimarySet = True;
                                } else if (!fSecondarySet) {
                                        if (!(cval &
                                              (OBJ_PLACE_LEFT_PRIMARY |
                                               OBJ_PLACE_RIGHT_PRIMARY))) {
                                                cval |=
                                                    OBJ_PLACE_LEFT_SECONDARY;
                                                fSecondarySet = True;
                                        }
                                }
                        }
                ...
                case 'r':
                        if (_DtStringsAreEquivalent(
                                (char *)pch, (char *)OBJ_PLACE_RIGHT_STR)) {
                                if (!fPrimarySet) {
                ...
                case 't':
                        if (_DtStringsAreEquivalent(
                                (char *)pch, (char *)OBJ_PLACE_TOP_STR)) {
                                if (!fPrimarySet) {
...
static unsigned char *_DtNextToken(unsigned char *pchIn, int *pLen,
        ...
        /* find end of word */
        {
                ...
                /* skip to next word */
#ifdef MULTIBYTE
        while (pchIn && (mblen((char *)pchIn, MB_CUR_MAX) == 1) &&
               isspace(*pchIn))
#else
...
static void DtfileCvtStringToDTIcon(XrmValue *args, Cardinal numArgs,
        ...
        if (_DtStringsAreEquivalent(in_str, "large"))
                ...
                i = LARGE; /* for now, eventually want to look at screen
                              width/height*/
        else
                i = LARGE;
        /*
...
static void SetupSendRequestArgs(ApplicationArgs application_args,
        ...
        if (application_args.title != NULL) {
                tt_message_arg_add(msg, TT_IN, "-title",
                                   application_args.title);
        }
...
static void BuildAndShowIconName(char *file_type_name, unsigned char view,
        ...
        if (view == BY_NAME_AND_ICON && show_type != MULTIPLE_DIRECTORY)
                pixmapData = _DtRetrievePixmapData(new_file_type_name, NULL,
                                                   NULL, widget, LARGE);
        else
                pixmapData = _DtRetrievePixmapData(new_file_type_name, NULL,
                                                   NULL, widget, SMALL);
        ...
        */
        ...
        _DtCheckAndFreePixmapData(new_file_type_name, widget,
                                  (DtIconGadget)widget, pixmapData);
...
void ForceMyIconClosed(char *host_name, char *directory_name) {
        ...
        for (i = 0; i < view_count; i++) {
                ...
                for (j = 0; j < file_mgr_data->directory_count; j++) {
                        if (strcmp(parent,
                                   file_mgr_data->directory_set[j]->name) ==
                            0) {
                                for (k = 0; k < file_mgr_data->directory_set[j]
                                                    ->file_count;
                                     k++) {
                                        file_view_data =
                                            file_mgr_data->directory_set[j]
                                                ->file_view_data[k];
                                        if (strcmp(file_view_data->file_data
                                                       ->file_name,
                                                   fname) == 0)
                                                goto search_done;
                                }
        ...
        if ((file_view_data) && (file_mgr_data->view != BY_NAME)) {
                if (file_mgr_data->view == BY_NAME_AND_ICON &&
                    file_mgr_data->show_type != MULTIPLE_DIRECTORY)
                        pixmapData = _DtRetrievePixmapData(
                            file_view_data->file_data->logical_type, fname,
...
static void ExitApp(XtPointer clientData, DtString words[], int num_fields)
