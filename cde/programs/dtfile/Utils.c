...
char *ResolveLocalPathName(char *hostname, char *directory_name,
        ...
        *tt_status = TT_OK;
         ...
         */
        cannon_name =
            (char *)tt_host_file_netfile(hostname, fully_qualified_name);
...
char *ResolveTranslationString(char *originalString, char *address)

{
        ...
        if (total == 0) {
                ...
                resolvedString = (char *)XtCalloc(
                    1, length + ((strlen(addressStr)) * total) + 1);
                if (resolvedString != NULL) {
                        i = 0;
                        j = 0;
                        while (originalString[i] != '\0') {
                                if (originalString[i] == '@') {
                                        for (k = 0; addressStr[k] != '\0';
                                             ++j, ++k)
                                                resolvedString[j] =
                                                    addressStr[k];
                                        ++i;
