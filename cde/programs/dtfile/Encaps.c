...
static DialogResource commonResources[] = {
    ...
    {"width", XmRHorizontalDimension, sizeof(Dimension),
     XtOffset(DialogInstanceDataPtr, width), (caddr_t)0, _DtDimensionToString},

    {"height", XmRVerticalDimension, sizeof(Dimension),
     XtOffset(DialogInstanceDataPtr, height), (caddr_t)0, _DtDimensionToString},
};
...

{
        ...
        timerId = XtAppAddTimeOut(
            ...
            initialTimeoutLength, (XtTimerCallbackProc)TimerEvent,
            (caddr_t)encap_parent_shell);
...

{
        ...
        if (num_classes == class_set_size) {
                ...
                class_set = (ClassSet *)XtRealloc(
                    (char *)class_set, sizeof(ClassSet) * class_set_size);
        }
        ...
        while (dialog != NULL) {
                if (dialog->dialog_data == dialog_data) {
                        ...
                        if (class_set[new_data->type].class->get_values) {
                                new_data->data =
                                    (*(class_set[new_data->type]
                                           .class->get_values))(dialog->dialog);
                        }
...

{
        ...
        dialog_data->data =
            (*(class_set[dialog_type].class->get_default_values))();
...

{
        ...
        dialog_data->type = dialog_type;
        dialog_data->data =
            (*(class_set[dialog_type].class->get_resource_values))(data_base,
                                                                   name_list);
...

{
        ...
        if (!ignoreCache) {
                ...
                /*
                 * In addition to looking for an available dialog in the cache
                 * to use, we also want to count up the number of unused dialogs
                 * in the cache. This lets us know it we need to restart the
                 * timer, to again build up the cache.
                 */
        ...
        if (dialog == NULL) {
                 ...
                 * (i.e icon_path, current_directory, status_line), now so that
                 * they don't get managed up front.  This is to make the
                 * Application Manager (toolbox) happy.
                 */
                ...
                if (class_set[dialog_type].class->install_change_callback)
                        (*(class_set[dialog_type]
                               .class->install_change_callback))(
                            dialog->dialog, DataChangeCallback,
                            (XtPointer)dialog);

                if (class_set[dialog_type].class->install_close_callback)
                        (*(class_set[dialog_type]
                               .class->install_close_callback))(
                            dialog->dialog, DataCloseCallback,
                            (XtPointer)dialog);
        ...
        if ((!ignoreCache) && (class_set[dialog_type].cache) &&
            (availableDialogCount < 1) && (timerState == TIMER_IDLE_STATE)) {
                /*
                 * We need to reset the cache building timer, so that it gets
                 * kicked off quickly, instead of after the longer idle delay.
                 */
                if (timerId)
        ...
        if (class_set[dialog_type].class->map) {
                ...
                (*(class_set[dialog_type].class->map))(map_parent,
                                                       dialog->dialog);
        }
        ...
        if (XtIsSubclass(XtParent(dialog->dialog_widget),
                ...
                if (XtIsRealized(XtParent(dialog->dialog_widget)) == False) {
                        if (instance_data->displayed == True) {
                                ...
                                XtSetValues(XtParent(dialog->dialog_widget),
                                            args, 1);
                        } else if ((instance_data->width != 0) &&
                        ...
                        XtSetMappedWhenManaged(XtParent(dialog->dialog_widget),
                                               False);
                        XtRealizeWidget(XtParent(dialog->dialog_widget));

                        /* Set the proper workspaces if needed */
                        _DtEncapSetWorkSpaceHints(
                            XtParent(dialog->dialog_widget), workspaces);

                        /* Set any application-specified class hints for the
                         * window */
                        if (classHints) {
                                XSetClassHint(
                                    XtDisplay(dialog->dialog_widget),
                        ...
                        if (instance_data->displayed == True) {
                                WMShellWidget wm = (WMShellWidget)XtParent(
                ...
                if (instance_data->displayed == True) {
                        ...
                        if (parent) {
                                ...
                                /*
                                 * Must be done after the set_values call, since
                                 * the dialog may get forced to a different
                                 * size.
                                 */
                 ...
                 * own workspace perperty set, if some workspaces have been
                 * requested.
                 */
        ...
        if (doCenter) {
                ...
                XtSetArg(args[1], XmNy,
                         (Dimension)(
                             HeightOfScreen(XtScreen(dialog->dialog_widget)) -
                ...
                XtSetArg(args[0], XmNx,
                         parent->core.x +
                             (Dimension)(parent->core.width -
                                         dialog->dialog_widget->core.width) /
                                 (Dimension)2);
                XtSetArg(args[1], XmNy,
                         parent->core.y +
                             (Dimension)(parent->core.height -
                                         dialog->dialog_widget->core.height) /
                                 (Dimension)2);
        ...
        XtAddEventHandler(XtParent(dialog->dialog_widget), StructureNotifyMask,
                          False, (XtEventHandler)DialogStructureNotifyHandler,
                          NULL);
}
        ...
        while (dialog != NULL) {
...

                {
                        ...
                        if (XtIsSubclass(XtParent(dialog->dialog_widget),
                                ...
                                /*
                                 * If we had been iconified, then our popped_up
                                 * flag will have been cleared by the vendor
                                 * shell.  When we call XtPopdown(), it will see
                                 * that we are no longer popped up, and will not
                                 * notify the window manager that our icon
                                 * should be removed; this can cause a
                                 * subsequent core dump if the user later tries
                                 * to deiconify the window. This fix should not
                                 * be necessary once the toolkit is fixed to
                                 * properly track the shell's state.
                                 */
                                shell_widget = (ShellWidget)XtParent(
                                    dialog->dialog_widget);
                                ...
                                /*
                                 * The following is for the condition described
                                 * above.  However, for a dialog shell, what
                                 * happens it that the 'managed' flag was set to
                                 * 'False' when the windows were iconified
                                 * (apparently by the dialog shell), and when we
                                 * tell the intrinsics to really unmanage the
                                 * child, it thinks it already has, so nothing
                                 * happens; as a result, the dialog shell is
                                 * left with its 'popped_up' flag set to 'True'.
                                 * The next time we try to post this dialog, the
                                 * intrinsics think that it is already up, so it
                                 * does nothing.
                                 */
                        ...
                        ((DialogInstanceData *)(dialog->dialog_data->data))
                            ->displayed = False;
                        ...
                        if (call_callbacks && dialog->close) {
                                ...
                                dialog->in_use = False;
                                if (new_data->data)
                                        ((DialogInstanceData *)(new_data->data))
                                            ->displayed = False;

                                (*(dialog->close))(dialog->close_client_data,
                                                   dialog->dialog_data,
                                                   new_data);
                        } else {
                                dialog->in_use = False;
                                ((DialogInstanceData *)(dialog->dialog_data
                                                            ->data))
                                    ->displayed = False;
                        }
...
{
        if (dialog_data != NULL) {
                (*(class_set[dialog_data->type].class->free_values))(
                    dialog_data->data);
...

{
        ...
        if (name_list != NULL) {
                while (name_list[nameCount] != NULL) {
                        xrmName[nameCount] =
                            XrmStringToQuark(name_list[nameCount]);
                        nameCount++;
                }
        }
        ...
        for (i = 0; i < resource_count; i++) {
                IntDialogGetResources(database, base, resources + i, xrmName,
                                      nameCount, stringQuark);
        }

        /*
         * Load the dialog specific resources. If no value found, use the
         * default.
         */
        for (i = 0; i < commonResourceCount; i++) {
                IntDialogGetResources(database, base, commonResources + i,
                                      xrmName, nameCount, stringQuark);
        }
}
...

{
...

        {
                ...
                if (XrmQGetResource(database, xrmName, xrmName, &repType,
                                    &value)) {
                        if (repType == stringQuark)
                                if (strcmp(resource->type, XmRString) != 0) {
                                        XtConvert(encap_parent_shell, XmRString,
                                                  &value, resource->type,
                                                  &convertedValue);
                                } else {
                                        *((char **)(base + resource->offset)) =
                                            (char *)value.addr;
                                        return;
                                }
                        else
                ...
                if (convertedValue.addr == NULL) {
                        if (resource->size == sizeof(char)) {
                                charVal =
                                    (char)(XtArgVal)resource->default_value;
                                convertedValue.addr = (caddr_t)&charVal;
                        } else if (resource->size == sizeof(short)) {
                                shortVal =
                                    (short)(XtArgVal)resource->default_value;
                                ...
                                longVal =
                                    (long)(XtArgVal)resource->default_value;
                ...
                if (resource->size == sizeof(char))
                        *((char *)(base + resource->offset)) =
                            *((char *)convertedValue.addr);
                else if (resource->size == sizeof(short))
                        *((short *)(base + resource->offset)) =
                            *((short *)convertedValue.addr);
                else if (resource->size == sizeof(int))
                        *((int *)(base + resource->offset)) =
                            *((int *)convertedValue.addr);
                else
                        *((long *)(base + resource->offset)) =
                            *((long *)convertedValue.addr);
        }
}
...

{
        ...
        if (workspaces) {
                ...
                /*
                 * Since no specific workspaces were specified, we will force
                 * the dialog to the current workspace.
                 */
...

{
        ...
        if (class_set[new_data->type].class->get_values) {
                new_data->data =
                    (*(class_set[new_data->type].class->get_values))(
                        dialog->dialog);
        }

        if (dialog->change)
                (*(dialog->change))(dialog->change_client_data,
                                    dialog->dialog_data, new_data, call_data);
}
...

{
        ...
        if (RecheckFlag(NULL,
                        widget)) /* cancel flag already set, just return */
                return;
...

{
        ...
        timerId = XtAppAddTimeOut(
            XtWidgetToApplicationContext(widget), idleTimeoutLength,
            (XtTimerCallbackProc)TimerEvent, (XtPointer)widget);
}
...
void _DtBooleanToString(int fd, Boolean *value, char *out_buf)
...
void _DtXmStringToString(int fd, XmString *value, char *out_buf)

{
        ...
        if (*value != NULL) {
                out_value =
                    (char *)_XmStringUngenerate(*value, XmFONTLIST_DEFAULT_TAG,
...
void _DtXmStringTableToString(int fd, XmStringTable *value, char *out_buf)

{
        ...
        if ((value != NULL) && (*value != NULL)) {
                i = 0;
                while ((*value)[i] != NULL) {
                        out_value = (char *)_XmStringUngenerate(
                            (*value)[i], XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT,
                            XmCHARSET_TEXT);
                        if (out_value != NULL) {
                                if (first) {
                                        (void)write(fd, out_buf,
                                                    strlen(out_buf));
                                        first = False;
                                } else
...
void _DtStringToString(int fd, char **value, char *out_buf)
...
#ifdef USE_XINERAMA
        if (!GetXineramaScreenDimensions(parent, &xOrg, &yOrg, &sWidth,
                                         &sHeight)) {
        ...
        if (event->type == MapNotify) {
                if (NumberOfDialogMapped == 0) {
                        ...
                        if (timeOut) {
                                ...
                                timerId = XtAppAddTimeOut(
                                    XtWidgetToApplicationContext(w), timeOut,
