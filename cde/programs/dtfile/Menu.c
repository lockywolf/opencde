...
Widget CreateMenu(Widget main, FileMgrRec *file_mgr_rec) {
        ...
        if (createSharedComponents) {
                mainMenu =
                    (MenuDesc *)XtCalloc(sizeof(MenuDesc) * menuItemCount, 1);
                j = 0;
                ...
                if (!restrictMode) {
                        ...
                        mainMenu[j].type = MENU_BUTTON;
                        mainMenu[j].label =
                            GETMESSAGE(20, 146, "Open Terminal");
                        mainMenu[j].label = XtNewString(mainMenu[j].label);
                        mainMenu[j].mnemonic = GETMESSAGE(20, 147, "O");
                        mainMenu[j].mnemonic =
                            XtNewString(mainMenu[j].mnemonic);
                        mainMenu[j].helpData = HELP_FILE_MENU_STR;
                ...
                if (openDirType == CURRENT) {
                        ...
                        mainMenu[j].type = MENU_BUTTON;
                        mainMenu[j].label =
                            GETMESSAGE(20, 138, "Open New View");
                        mainMenu[j].label = XtNewString(mainMenu[j].label);
                ...
                mainMenu[j].label =
                    GETMESSAGE(20, 142, "Save As Default Options...");
                mainMenu[j].label = XtNewString(mainMenu[j].label);
                ...
                mainMenu[j].label =
                    GETMESSAGE(20, 144, "Set Filter Options...");
                mainMenu[j].label = XtNewString(mainMenu[j].label);
                ...
                mainMenu[j].type = MENU_BUTTON;
                mainMenu[j].label =
                    GETMESSAGE(20, 157, "About Application Manager");
                mainMenu[j].label = XtNewString(mainMenu[j].label);
                mainMenu[j].mnemonic = GETMESSAGE(20, 116, "A");
                mainMenu[j].mnemonic = XtNewString(mainMenu[j].mnemonic);
                mainMenu[j].helpData = HELP_HELP_MENU_STR;
        ...
        menu_bar = _DtCreateMenuSystem(
            main, "mainMenu", HelpRequestCB, HELP_HELP_MENU_STR,
            createSharedComponents, mainMenu, menuItemCount, GetMainMenuData,
        ...
        if ((strcmp(file_mgr_data->selection_list[0]->file_data->file_name,
            ...
            (strcmp(file_mgr_data->selection_list[0]->file_data->file_name,
                ...
                file_mgr_rec->menuStates |=
                    RENAME | TRASH | MODIFY | PUT_ON_DESKTOP;
...
static void UnmanageAllActionItems(FileMgrRec *file_mgr_rec) {
        ...
        for (i = SELECTED_MENU_MAX; i < action_pane->composite.num_children;
             ++i) {
...
void ActivateMultipleSelect(FileMgrRec *file_mgr_rec) {
        ...
        file_mgr_rec->menuStates &=
            ~(RENAME | MOVE | DUPLICATE | LINK | MODIFY);
...
void TrashFiles(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (file_view_data)
                ...
                _DtBuildActionArgsWithSelectedFiles(
                    ...
                    file_mgr_data->selected_file_count, &action_args,
                    &arg_count);
...
static void NewView(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (file_mgr_data->restricted_directory != NULL) {
                ...
                if (file_mgr_data->toolbox)
                        ...
                        GetNewView(file_mgr_data->host,
                                   file_mgr_data->current_directory, NULL, NULL,
                                   0);
        } else
                GetNewView(file_mgr_data->host,
                           file_mgr_data->current_directory, NULL, NULL, 0);
...
void ChangeToParent(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (focus_widget) {
                if (focus_widget == file_mgr_rec->current_directory_text) {
                        XmTextFieldReplace(
                            focus_widget, (XmTextPosition)0,
                            XmTextFieldGetInsertionPosition(focus_widget), "");
                        return;
                } else if (strcmp(focus_widget->core.name, "nameChangeT") ==
                           0) {
                        XmTextReplace(
                            focus_widget, (XmTextPosition)0,
                            XmTextFieldGetInsertionPosition(focus_widget), "");
...
static void SetMenuSensitivity(Widget w, XtPointer client_data,
        ...
        if ((dialog_data = _DtGetInstanceData((XtPointer)file_mgr_rec)) !=
            NULL) {
                        ...
                        if (!(file_info.st_mode & S_IXUSR) ||
                            !(file_info.st_mode & S_IWUSR)) {
                                ...
                                _DtGlobalUpdateMenuItemState(
                                    *create_dataBtn, CREATE_FILE, False,
                                    &currentMenuStates);
                                _DtGlobalUpdateMenuItemState(
                                    *create_directoryBtn, CREATE_DIR, False,
                                    &currentMenuStates);
                                ...
                                file_mgr_rec->menuStates |=
                                    CREATE_DIR | CREATE_FILE;
                                _DtGlobalUpdateMenuItemState(
                                    *create_dataBtn, CREATE_FILE, True,
                                    &currentMenuStates);
                                _DtGlobalUpdateMenuItemState(
                                    *create_directoryBtn, CREATE_DIR, True,
                                    &currentMenuStates);
                        }
                }
                ...
                if (file_mgr_data->restricted_directory == NULL) {
                        if (strcmp(file_mgr_data->current_directory, "/") ==
                            0) {
                                file_mgr_rec->menuStates &= ~(MOVE_UP);
                                _DtGlobalUpdateMenuItemState(
                                    *upBarBtn, MOVE_UP, False,
                                    &currentMenuStates);
                        } else {
                                file_mgr_rec->menuStates |= MOVE_UP;
                                _DtGlobalUpdateMenuItemState(
                                    *upBarBtn, MOVE_UP, True,
                                    &currentMenuStates);
                        }
                        ...
                        if (homeDir != NULL &&
                            strcmp(homeDir, file_mgr_data->current_directory) !=
                                0) {
                                file_mgr_rec->menuStates |= HOME;
                                _DtGlobalUpdateMenuItemState(
                                    *homeBarBtn, HOME, True,
                                    &currentMenuStates);
                        } else {
                                file_mgr_rec->menuStates &= ~(HOME);
                                _DtGlobalUpdateMenuItemState(
                                    *homeBarBtn, HOME, False,
                                    &currentMenuStates);
                        }

                        file_mgr_rec->menuStates |= CHANGE_DIR;
                        _DtGlobalUpdateMenuItemState(*change_directoryBtn,
                                                     CHANGE_DIR, True,
                                                     &currentMenuStates);
                ...
                if (file_mgr_data->toolbox) {
                        if (geteuid() == root_user ||
                            access(file_mgr_data->current_directory,
                                ...
                                file_mgr_rec->menuStates &=
                                    ~(CREATE_DIR | CREATE_FILE | MOVE | LINK |
                                      RENAME | TRASH);
                                _DtGlobalUpdateMenuItemState(
                                    *create_dataBtn, CREATE_FILE, False,
                                    &currentMenuStates);
                                _DtGlobalUpdateMenuItemState(
                                    *create_directoryBtn, CREATE_DIR, False,
                                    &currentMenuStates);
                                _DtGlobalUpdateMenuItemState(
                                    *moveBtn, MOVE, False, &currentMenuStates);
                                _DtGlobalUpdateMenuItemState(
                                    *linkBtn, LINK, False, &currentMenuStates);
                                _DtGlobalUpdateMenuItemState(
                                    *renameBtn, RENAME, False,
                                    &currentMenuStates);
                        }
                }
        }
...
static void RestoreMenuSensitivity(Widget w, XtPointer client_data,
        ...
        if (restrictMode) {
                ...
                ptr = strrchr(users_home_dir, '/');
                *ptr = '\0';
                if (strcmp(file_mgr_data->current_directory, users_home_dir) ==
                    0) {
                        _DtGlobalUpdateMenuItemState(*upBarBtn, MOVE_UP, False,
                                                     &currentMenuStates);
                } else {
...
static void TerminalCWD(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        DtActionInvoke(file_mgr_rec->shell, "Terminal", NULL, 0, NULL, NULL,
                       pwd_dir, True, NULL, NULL);
