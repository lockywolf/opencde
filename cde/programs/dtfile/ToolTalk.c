...
Tt_message FileCallback(Tt_message msg, Tttk_op op, char *file,
        ...
        case TTDT_MOVED:
                ...
                ShowNewDirectory((FileMgrData *)dialog_data->data, local_host,
                                 to_file);

                /* Quit the old file, join the new */
                ttdt_file_quit(view->pats, 0);
                view->pats =
                    ttdt_file_join(to_file, TT_SESSION, 0, FileCallback, view);
...
void FinalizeToolTalkSession() {
        ...
        if (FileManagerToolTalkPattern &&
            tt_ptr_error(FileManagerToolTalkPattern) == TT_OK) {
                ttRc = ttdt_session_quit(NULL, FileManagerToolTalkPattern, 1);
...
void ToolTalkError(char *procName, Tt_status errId) {
        if (tt_is_err(errId)) {
                switch (errId) {
                case TT_ERR_NOMP:
                        XtRemoveInput(ProcessToolTalkInputId);
...
Tt_message SessionCallback(Tt_message msg, void *client_data,
                           Tt_message contract) {
...
Tt_message MediaCallback(Tt_message msg, void *client_data, Tttk_op op,
                         Tt_status diag, unsigned char *contents, int len,
        ...
        if (tt_message_contexts_count(msg) > 0) {
                ...
                if (!tt_is_err(tt_ptr_error(ceiling))) {
                        if ((ceiling == 0) || (strcmp(ceiling, ".") != 0)) {
                                tttk_message_fail(msg, TT_DESKTOP_ENOTSUP, 0,
                                                  1);
                                return 0;
                        }
                        restricted = True;
...
Tt_status InitializeToolTalkProcid(int *ttFd, Widget topLevel,
                                   Boolean sendStarted) {
        char *procId;
        Tt_status ttstat;
        ...
        procId =
            ttdt_open(ttFd, DTFILE_CLASS_NAME, VENDOR, VERSION, sendStarted);
        ...
        if ((ttstat = tt_ptr_error(procId)) == TT_OK) {
                ProcessToolTalkInputId = XtAppAddInput(
                    XtWidgetToApplicationContext(toplevel), *ttFd,
...
int InitializeToolTalkSession(Widget topLevel, int ttFd) {
        Tt_status ttstat;

        ttstat = ttmedia_ptype_declare("DT_File_Manager", 0, MediaCallback,
                                       (void *)0, 0);
        if (ttstat == TT_OK)
                ttstat = ttmedia_ptype_declare("DT_File_Manager", 1000,
                                               MediaCallback, (void *)0, 1);
        ...
        if (ttstat == TT_OK) {
                ...
                FileManagerToolTalkPattern = ttdt_session_join(
                    tt_default_session(), SessionCallback, topLevel, NULL, 1);
