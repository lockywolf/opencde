...
static Boolean IsMainWinDialog(DialogData *dialogData);
static XtPointer MapIconWidgetToFileType(Widget w, FileMgrData *fileMgrData);
static Boolean IsMenuWidget(Widget w, FileMgrRec *fileMgrRec,
                            FileMgrData *fileMgrData, String *aName,
                            String *ftName);
static Boolean IsFilterIcon(Widget w, FileMgrData *fileMgrData, String *ftName);
static Boolean ProcessItemHelp(Widget shell);
static void ReusePrimaryHelpWindow(DialogData *primaryHelpDialog,
                                   char *idString, char *volume,
                                   char *topicTitle, char *fileType,
                                   int helpType);
...
void HelpRequestCB(Widget w, XtPointer clientData, XtPointer callData)

{
        ...
        if (recordStruct = LocateRecordStructure(w)) {
                if ((dialogData = _DtGetInstanceData(recordStruct))) {
                        if (IsMainWinDialog(dialogData)) {
                                ...
                                FileMgrRec *fileMgrRec =
                                    (FileMgrRec *)recordStruct;

                                /* Check for item help */
                                if (clientData &&
                                    strcmp(clientData, HELP_HELP_MODE_STR) ==
                                        0) {
                                        if (!ProcessItemHelp(
                                                fileMgrRec->shell)) {
                                ...
                                if (clientData) {
                                        ...
                                        if (fileMgrData->selected_file_count >
                                            1) {
                                                Widget warn;
                                                String s;
                                                XmString xmstr;
                                        ...
                                        topicTitle = GetSharedMessage(
                                            ACTION_FT_HELP_TITLE);
                                        helpType = DtHELP_TYPE_DYNAMIC_STRING;
                                        fileType = NULL;

                                        /* Determine string, and set clientData
                                         * accordingly */
                                        if ((clientData =
                                                 MapIconWidgetToFileType(
                                                     w, fileMgrData)) == NULL) {
                                                /* Not a file icon; is it an
                                                 * action menu item? */
                                                if (!IsMenuWidget(w, fileMgrRec,
                                                                  fileMgrData,
                                                                  &strVal,
                                                                  &fileType)) {
                                                        if (!IsFilterIcon(
                                ...
                                /* Special check for 'Using Help'; required
                                 * different volume */
                                locId = (char *)clientData;
                                if ((w == *usingHelp) &&
                                    (strcmp(locId, HELP_HOME_TOPIC) == 0))
                                        vol = "Help4Help";
                                else
                                ...
                                if (fileMgrData->primaryHelpDialog) {
                                        ReusePrimaryHelpWindow(
                                            fileMgrData->primaryHelpDialog,
                                            clientData, vol, topicTitle,
                                            fileType, helpType);
                                } else {
                                        /* Create the main help window for this
                                         * view */
                                        ShowHelpDialog(fileMgrRec->shell,
                                                       (XtPointer)fileMgrRec,
                                                       MAIN_HELP_DIALOG, NULL,
                                                       clientData, vol,
                                                       topicTitle, fileType,
                                                       helpType);
                                }
                ...
                XtSetArg(args[i], DtNhelpType,
                         (unsigned char)DtHELP_TYPE_TOPIC);
                i++;
                XtSetArg(args[i], DtNhelpVolume, DTFILE_HELP_NAME);
                i++;
                XtSetArg(args[i], DtNlocationId, clientData);
                i++;
                XtSetArg(args[i], XmNtitle,
                         (GETMESSAGE(29, 1, "File Manager Help")));
                i++;
                if (!mainHelpDialog ||
                    (mainHelpDialog && !XtIsManaged(mainHelpDialog))) {

                        mainHelpDialog =
                            DtCreateHelpDialog(w, "mainHelpDialog", args, i);
                        XtAddCallback(mainHelpDialog, DtNcloseCallback,
                                      closeCB_mainHelpDialog, (XtPointer)NULL);
...
void DTHelpRequestCB(Widget w, XtPointer clientData, XtPointer callData)

{
        ...
        if (clientData) {
                ...
                if (XtParent(w) == desktop_data->popupMenu->popup) {
                        ...
                        fileType =
                            dtInfo->file_view_data->file_data->logical_type;
                } else {
...
void TrashHelpRequestCB(Widget w, XtPointer clientData, XtPointer callData)

{
        ...
        if (strcmp(clientData, HELP_HELP_MODE_STR) == 0) {
                if (!ProcessItemHelp(trashShell)) {
                        ...
                        _DtMessage(trashShell, title, msg, NULL, HelpRequestCB);
                        XtFree(title);
                        XtFree(msg);
                }
                return;
        ...
        if (primaryTrashHelpDialog) {
                ReusePrimaryHelpWindow(primaryTrashHelpDialog, clientData, vol,
                                       NULL, NULL, DtHELP_TYPE_TOPIC);
        } else {
                /* Create the main help window for this view */
                ShowTrashHelpDialog(trashShell, MAIN_HELP_DIALOG, NULL,
                                    clientData, vol);
        }
}
...

{
        ...
        for (i = directoryCount - 1; i >= 0; i--) {
                for (j = 0; j < fileMgrData->directory_set[i]->file_count;
                     j++) {
                        if (w == fileMgrData->directory_set[i]
                                     ->file_view_data[j]
                                     ->widget) {
                                ftName = fileMgrData->directory_set[i]
                                             ->file_view_data[j]
        ...
        for (i = 0; i < action_pane->composite.num_children; i++) {
                if (w == action_pane->composite.children[i]) {
                        ...
                        *aName = XtNewString(actionName);
                        ...
                        /* Get the filetype from the active item */
                        *ftName = XtNewString(fileMgrData->selection_list[0]
                                                  ->file_data->logical_type);
                        return (True);
                }
        }
...

{
        ...
        n = 0;
        XtSetArg(args[n], DtNhelpType, (unsigned char)helpType);
        n++;
...
void ObjectHelp(Widget w, XtPointer clientData, XtPointer callData)
