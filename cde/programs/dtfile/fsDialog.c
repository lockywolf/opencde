...

{
        ...
        if (strlen(fsType) == 0)
                ...
                readConfigFile(fsType, platform, fsDialogData,
                               &dialogAvailable);
                if (dialogAvailable) {
 ...
 *system the file defined by path is on. Return strings defining the file system
 *type and the platform. Any errors result in a fsType set to the null
 *string.
 *
        ...
        if (strlen(fsDialogData->fsDialogProgram) == 0) {
                *dialogAvailable = FALSE;
                if (strlen(fsDialogData->buttonLabel) != 0) {
                        ...
                        _DtSimpleError(application_name, DtError, NULL, msg1,
                                       NULL);
                        XtFree(msg1);
                }
        }
...

{
        ...
        static char fn[MAX_PATH] =
            ""; /* remember name once its been determined */
        ...
        for (i = 0; i < 4; i++) {
                ...
                case 0: /* file name $DTFSCONFIG */
                        if ((s = getenv("DTFSCONFIG")) !=
                            NULL) /* a convenience for debugging */
                ...
                case 1: /* $HOME/dtfile.config */
                        if ((s = getenv("HOME")) != NULL) {
                                strncpy(fn, s, MAX_PATH);
                                strcat(fn, "/");
                                strncat(fn, CONFIGFILENAME,
                                        MAX_PATH - strlen(fn) - 1);
        ...
        /* return the name of the factory default (case 3 above) which should
         * always be there */
        ...
        for (lineLength = readLine(fptr, line); lineLength != EOF;) {
                ...
                if (stricmp(id, fsID) == 0) {
                        ...
                        *dialogAvailable = TRUE;
                        token1 = strtok(NULL, " =");
                        s = strchr(token1, '\0') +
                            1; /* first character after token1 */
                        token2 = &s[strspn(
                            s, " =")]; /* first non-blank, non-= after token1 */
                        /* loop to get data for fields in fsDialogData */
                        while (strchr(token1, ':') == NULL &&
                               lineLength != EOF) {
                                if (stricmp(token1, "buttonlabel") == 0)
                                        strncpy(fsDialogData->buttonLabel,
                                                token2, MAXLINELENGTH);
                                else if (stricmp(token1, "warning") == 0) {
                                        strncpy(fsDialogData->warningMessage,
                                                token2, MAXLINELENGTH);
                                } else if (stricmp(token1, "fsdialog") == 0)
                                        strncpy(fsDialogData->fsDialogProgram,
                                                token2, MAXLINELENGTH);
                                else if (stricmp(token1, "dismiss") == 0)
                                        fsDialogData
                                            ->dismissStdPermissionDialog =
                        ...
                        /* the current line does not match fsID ... get next
                         * line */
                        lineLength = readLine(fptr, line);
                }
        } /* end for */
...

{
        ...
        while (TRUE) {
                if (fgets(myLine, MAXLINELENGTH, fptr) == NULL)
                        ...
                        s = strip(
                            myLine); /* remove leading & trailing whitespace */
                        if ((len = strlen(s)) != 0 && s[0] != commentChar) {
                                /* change imbedded white space characters to
                                 * spaces */
                                for (i = 0; i < len; i++)
                                        if (isspace(s[i]))
                                                s[i] = ' ';

                                /* change any imbedded "\n" to space followed by
                                 * newline */
                                while ((t = strstr(s, "\\n")) != NULL) {
                                        *t = ' ';        /* space */
                                        *(t + 1) = '\n'; /* new line */
