...
static void FilterChange(XtPointer client_data, DialogData *old_dialog_data,
                         DialogData *new_dialog_data, XtPointer call_data);
static void FilterClose(XtPointer client_data, DialogData *old_dialog_data,
...
void ShowFilterDialog(Widget w, XtPointer client_data, XtPointer callback) {
        ...
        _DtShowDialog(file_mgr_rec->shell, NULL, (XtPointer)file_mgr_rec,
                      ...
                      (XtPointer)file_mgr_rec, FilterClose,
        ...
        if (file_mgr_data->title != NULL &&
            strcmp(file_mgr_data->helpVol, DTFILE_HELP_NAME) != 0) {
                ...
                tmpStr =
                    (GETMESSAGE(13, 24, "File Manager - Set Filter Options"));
...
void NewFileTypeSelected(Widget w, XtPointer client_data, XtPointer callback) {
        ...
        if (callback_data->reason == XmCR_ACTIVATE) {
                ...
                if (user_data->selected) {
                        ...
                        if (background_color == white_pixel) {
                                ...
                                XtSetArg(args[4], XmNpixmapBackground,
                                         white_pixel);
...
void SelectAllFileTypes(Widget w, XtPointer client_data, XtPointer callback) {
        Arg args[6];
        FilterRec *filter_rec;
        Pixel background_color;
        XmManagerWidget mgr;
...
void UnselectAllFileTypes(Widget w, XtPointer client_data, XtPointer callback) {
        Arg args[6];
        FilterRec *filter_rec;
        Pixel background_color;
        Pixel foreground_color;
