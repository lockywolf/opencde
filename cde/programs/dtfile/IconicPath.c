...
#define MIN_WD(ip)                                                             \
        ((ip)->iconic_path.large_icons ? (ip)->iconic_path.large_min_width     \
...
#define DESIRED_WIDTH(ip)                                                      \
        (MWD(ip) + (ip)->iconic_path.dotdot_button->core.width + MWD(ip) +     \
         ((ip)->iconic_path.dropzone                                           \
              ? ((ip)->iconic_path.dropzone_icon->core.width + MWD(ip))        \
              : 0) +                                                           \
         ((ip)->iconic_path.status_msg                                         \
              ? ((ip)->iconic_path.status_label->core.width + MWD(ip))         \
              : 0))
...
static XtResource resources[] = {
    ...
    {XmNspacing, XmCSpacing, XmRVerticalDimension, sizeof(Dimension),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.spacing), XmRImmediate,
     (XtPointer)4},

    {DtNsmallMinWidth, XmCMinWidth, XmRHorizontalDimension, sizeof(Dimension),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.small_min_width),
     XmRImmediate, (XtPointer)25},

    {DtNlargeMinWidth, XmCMinWidth, XmRHorizontalDimension, sizeof(Dimension),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.large_min_width),
     XmRImmediate, (XtPointer)45},

    {DtNforceSmallIcons, DtCForceSmallIcons, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.force_small_icons),
     XmRImmediate, (XtPointer)False},

    {DtNforceLargeIcons, DtCForceLargeIcons, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.force_large_icons),
     XmRImmediate, (XtPointer)False},

    {"buttons", "Buttons", XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.buttons), XmRImmediate,
     (XtPointer)False},

    {"dropZone", "DropZone", XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.dropzone), XmRImmediate,
     (XtPointer)False},

    {"statusMsg", "StatusMsg", XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.status_msg), XmRImmediate,
     (XtPointer)True},

    {DtNfileMgrRec, DtCfileMgrRec, XmRString, sizeof(char *),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.file_mgr_rec),
     XmRImmediate, (XtPointer)NULL},

    {DtNcurrentDirectory, DtCCurrentDirectory, XmRString, sizeof(char *),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.current_directory),
     XmRImmediate, (XtPointer)NULL},

    {DtNlargeIcons, DtCLargeIcons, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtIconicPathRec, iconic_path.large_icons), XmRImmediate,
     (XtPointer)True},

    {DtNiconsChanged, DtCIconsChanged, XmRBoolean, sizeof(Boolean),
...
externaldef(dticonicpathwidgetclass)
    WidgetClass dtIconicPathWidgetClass = (WidgetClass)&dtIconicPathClassRec;

/*--------------------------------------------------------------------
 * Activate Callback for iconic path buttons and double click on icons
 *------------------------------------------------------------------*/

static void ButtonCallback(Widget w, XtPointer client_data,
                           XtPointer call_data) {
        ...
        if (XtClass(w) == dtIconGadgetClass) {
                if (((XmAnyCallbackStruct *)call_data)->reason !=
                    XmCR_DEFAULT_ACTION)
                        return;
        }
...
static int Update(DtIconicPathWidget ip, FileMgrRec *file_mgr_rec,
                  FileMgrData *file_mgr_data) {
...
#define INC_N_CHANGES()                                                        \
        if (n_changes++ == 0) { /* first change: unmanage all children and     \
                                   clear the window */                         \
                XtUnmanageChildren(ip->composite.children,                     \
                                   ip->composite.num_children);                \
...
#define MOVE_OBJECT(w, _x, _y)                                                 \
        if ((w)->core.x != (_x) || (w)->core.y != (_y)) {                      \
                INC_N_CHANGES();                                               \
                XmeConfigureObject(w, _x, _y, (w)->core.width,                 \
                                   (w)->core.height, (w)->core.border_width);  \
        } else

        DPRINTF2(("IconicPath.Update: cur_dir '%s', dir_shown '%s', "
                  "icons_changed %d\n",
        ...
        if (ip->iconic_path.dotdot_button == NULL) {
                ...
                n = 0;
                XtSetArg(args[n], XmNlabelString, xm_string);
                n++;
                ...
                if (ip->iconic_path.buttons) {
                        ...
                        ip->iconic_path.dotdot_button =
                            XmCreatePushButtonGadget(
                                (Widget)ip, "iconic_path_button", args, n);
                        XtAddCallback(ip->iconic_path.dotdot_button,
                                      XmNactivateCallback, ButtonCallback, ip);
                } else {
                        ip->iconic_path.dotdot_button = XmCreateLabelGadget(
                            (Widget)ip, "iconic_path_label", args, n);
                }
                XmStringFree(xm_string);
                if (ip->iconic_path.dotdot_button->core.width <
                    (Dimension)((Dimension)MIN_WD(ip) / (Dimension)2)) {
                        XmeConfigureObject(
                            ip->iconic_path.dotdot_button,
        ...
        if (ip->iconic_path.status_msg &&
            ip->iconic_path.status_label == NULL) {
                /* create StatuMsg */
                n = 0;
                XtSetArg(args[n], XmNmarginHeight, 0);
                n++;
        ...
        if (ip->iconic_path.dropzone && ip->iconic_path.dropzone_icon == NULL) {
                ...
                if (change_view_pixmap == XmUNSPECIFIED_PIXMAP) {
                        ...
                        background = ip->core.background_pixel;
                        change_view_pixmap =
                            _DtGetPixmap(XtScreen(ip), CHANGE_VIEW_ICON_M,
            ...
            (ip->iconic_path.directory_shown == NULL ||
             ...
             ip->iconic_path.icons_changed)) {
                ...
                for (;;) {
                        ...
                        /* don't show path components above a restricted
                         * directory */
                        path_len = strlen(path);
                        if (restricted_len && path_len <= restricted_len)
                                goto next_component;

                        /* in restricted mode: check if this component is above
                         * $HOME */
                        forbidden =
                            restrictMode &&
                        ...
                        if (i >= ip->iconic_path.num_components) {
                                ...
                                ip->iconic_path.components =
                                    (struct _IconicPathComponent *)XtRealloc(
                                        (char *)ip->iconic_path.components,
                                        (i + 1) *
                                            sizeof(
                                ...
                                pixmapData = GetPixmapData(
                                    file_mgr_rec, file_mgr_data, path,
                                ...
                                ip->iconic_path.components[i].icon =
                                    _DtCreateIcon((Widget)ip,
                                                  "iconic_path_icon", args, n);

                                _DtCheckAndFreePixmapData(
                                    GetDirectoryLogicalType(file_mgr_data,
                                                            path),
                                    file_mgr_rec->shell,
                                    (DtIconGadget)ip->iconic_path.components[i]
                                        .icon,
                                    pixmapData);
                                ...
                                if (ip->iconic_path.buttons) {
                                        ...
                                        ip->iconic_path.components[i].button =
                                            XmCreatePushButtonGadget(
                                                (Widget)ip,
                                                "iconic_path_button", args, n);
                                        XtAddCallback(
                                            ip->iconic_path.components[i]
                                                .button,
                                            XmNactivateCallback, ButtonCallback,
                                            ip);
                                } else {
                                        ip->iconic_path.components[i].button =
                                            XmCreateLabelGadget(
                                                (Widget)ip, "iconic_path_label",
                                                args, n);
                                }
                                XmStringFree(xm_string);

                        } else {
                                /* check if the existing component needs to be
                                 * updated */

                                /* check if the path has changed */
                                if (strcmp(ip->iconic_path.components[i].path,
                                           path) != 0 ||
                                    ip->iconic_path.large_shown !=
                                        ip->iconic_path.large_icons) {
                                        INC_N_CHANGES();
                                ...
                                XtSetValues(
                                    ip->iconic_path.components[i].button, args,
                                    2);
                                XmStringFree(xm_string);
                                ...
                                if (pixmapData) {
                                        ...
                                        XtFree(ip->iconic_path.components[i]
                                                   .icon_name);
                                        ip->iconic_path.components[i]
                                            .icon_name = NULL;
        ...
        for (i = 0; i < ip->iconic_path.num_components; i++) {
                ...
                twidth = MIN_WD(ip);
                if (ip->iconic_path.components[i].icon->core.width >
                    (Dimension)twidth)
                        twidth = ip->iconic_path.components[i].icon->core.width;
                if (ip->iconic_path.components[i].button->core.width >
                    (Dimension)twidth)
                        twidth =
                            ip->iconic_path.components[i].button->core.width;
        ...
        if (i == 1 &&
            (Dimension)(twidth - ip->iconic_path.dotdot_button->core.width -
                        SPC(ip)) <= ip->core.width) {
                i--;
        }
        ...
        manage = (Widget *)XtMalloc(
            (2 * (ip->iconic_path.num_components - i) + 2) * sizeof(Widget));
        nmanage = 0;

        /* position & manage the path components */
        x = MWD(ip);
        y = ip->core.height - MHT(ip) -
            ip->iconic_path.dotdot_button->core.height - SPC(ip);
        ...
        for (j = i; j < ip->iconic_path.num_components; j++) {
                MOVE_OBJECT(
                    ip->iconic_path.components[j].icon, x + SPC(ip),
                    y - ip->iconic_path.components[j].icon->core.height);
                MOVE_OBJECT(ip->iconic_path.components[j].button, x,
                            y + SPC(ip));
        ...
        if (ip->iconic_path.status_msg) {
                ...
                if (ip->iconic_path.status_label->core.x != x ||
                    ip->iconic_path.status_label->core.y != y) {
                        XmeConfigureObject(
                            ip->iconic_path.status_label, x, y,
                            ip->iconic_path.status_label->core.width,
                            ip->iconic_path.status_label->core.height,
        ...
        if (ip->iconic_path.dropzone) {
                ...
                if (x < 0)
                        x = 0;
                y = (y - (int)ip->iconic_path.dropzone_icon->core.height + 1) /
                    2;
                if (y < 0)
                        y = 0;
...
static void IconicPathRedraw(DtIconicPathWidget ip) {
        FileMgrRec *file_mgr_rec = (FileMgrRec *)ip->iconic_path.file_mgr_rec;
        DialogData *dialog_data;
        FileMgrData *file_mgr_data;
        int i;
        ...
        if (ip->iconic_path.gc == None) {
                ...
                ip->iconic_path.gc =
                    XCreateGC(XtDisplay(ip), XtWindow(ip),
        ...
        if (ip->iconic_path.left_component > 0) {
                XSetLineAttributes(XtDisplay(ip), ip->iconic_path.gc, 2,
        ...
        for (i = ip->iconic_path.left_component;
             i < ip->iconic_path.num_components - 1; i++) {
                if (ip->iconic_path.components[i].icon_name != NULL &&
                    ip->iconic_path.components[i + 1].icon_name != NULL) {
                        XDrawLine(
                            XtDisplay(ip), XtWindow(ip), ip->iconic_path.gc,
                            x + SPC(ip) +
                                ip->iconic_path.components[i].icon->core.width +
                                1,
                            y,
                            x + SPC(ip) + ip->iconic_path.components[i].width +
                                SPC(ip) - 1,
                            y);
                }
...
static void ClassInitialize(void) { baseClassExtRec.record_type = XmQmotif; }
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args) {
        ...
        FileMgrRec *file_mgr_rec =
            (FileMgrRec *)new_w->iconic_path.file_mgr_rec;
...
Widget _DtCreateIconicPath(Widget p, String name, ArgList args, Cardinal n) {
...
void DtUpdateIconicPath(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
                        Boolean icons_changed) {
        Arg args[8];

        XtSetArg(args[0], DtNfileMgrRec, file_mgr_rec);
        XtSetArg(args[1], DtNcurrentDirectory,
                 file_mgr_data->current_directory);
        XtSetArg(args[2], DtNlargeIcons,
