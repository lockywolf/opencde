...
static void RestoreVerifyCancel(Widget w, XtPointer client_data,
                                XtPointer call_data);
static int RestoreObject(Widget w, int mode, register char *source,
                         register char *target, Boolean isContainer,
                         void (*errorHandler)(), Boolean checkForBusyDir,
                         int type, Boolean CheckedAlready);
...
Boolean InitializeTrash(Boolean enableVerifyPrompt) {
        ...
        sacred_dir_list =
            (char **)XtMalloc(sizeof(char *) * sacred_dir_list_size);
                ...
                *ptr = '\0';
                if (sacred_dir_count >= sacred_dir_list_size) {
                        ...
                        sacred_dir_list = (char **)XtRealloc(
                            (char *)sacred_dir_list,
                            sizeof(char *) * sacred_dir_list_size);
                }
                sacred_dir_list[sacred_dir_count++] =
                    XtNewString(TrashInfoFileName);
                *ptr = '/';
                ptr++;
        }
...
void TrashCreateDialog(Display *display) {
        ...
        _DtBuildDialog(NULL, NULL, NULL, dialog_data, NULL, NULL, NULL, NULL,
                       NULL, False, special_view, title, NULL);
        TrashView = False;
...
static Boolean ReadTrashList(void) {
        ...
        while (fgets(trashEntry, bufSize, trashInfoFileId) != NULL) {
                ...
                if (sscanf(trashEntry, "%d %d", &extSize, &intSize) == 2) {
                        ...
                        /* extSize + intSize + 3 is a rough estimate of a trash
                           entry a trash entry looks something like the
                           following:
                           ...
                           so the len (the string length of the trash entry has
                           AT LEAST to be bigger than extSize + intSize + 3
                           separator spaces
                        */
                        {
                                char *tmpPtr;
                                ...
                                /* sscanf can be used to reduce the code.
                                   I'm not using it here because I don't have
                                   time to research. I just want it to work.
                                */
                                tmpPtr = DtStrchr(trashEntry, ' ') + 1;
                                tmpPtr = DtStrchr(tmpPtr, ' ') + 1;

                                /* Extract the full external and partial
                                 * internal file names */
                                ...
                                if (lstat(internal, &statbuf) < 0) {
                                        ...
                                        if (numTrashItems >= trashListSize) {
                                                trashListSize += 10;
                                                trashCan =
                                                    (TrashEntry *)XtRealloc(
                                                        (char *)trashCan,
                                                        sizeof(TrashEntry) *
                                                            trashListSize);
                                        }

                                        trashCan[numTrashItems].intNew =
                                            internal;
                                        trashCan[numTrashItems].intOrig =
                                            XtNewString(external);
                                        trashCan[numTrashItems].external =
                                            external;
                                        trashCan[numTrashItems].filename =
                                            intName;
                                        numTrashItems++;
                                } /* end if file exists */
                        } else {
...
static Boolean WriteTrashEntries(void) {
        ...
        if (newFile) {
                chmod(NewTrashInfoFileName, S_IRUSR | S_IWUSR | S_IRGRP |
                                                S_IWGRP | S_IROTH | S_IWOTH);

                /* Write all remaining entries */
                for (i = 0; i < numTrashItems; i++) {
                        if (WriteEntry(newFile, trashCan[i].external,
                                       trashCan[i].filename) < 0) {
                                fclose(newFile);
...
Widget CreateTrashMenu(Widget mainw, FileMgrRec *file_mgr_rec) {
        ...
        menu_bar = _DtCreateMenuSystem(
            mainw, "menu_bar_trash", TrashHelpRequestCB, HELP_TRASH_DIALOG_STR,
            True, trashMenu, trashMenuItemCount, NULL, Noop, Noop);
...
void TrashDisplayHandler(Tt_message msg) {
        ...
        if (trashFileMgrData->file_mgr_rec == 0) {
                ...
                TrashView = True;
                _DtShowBuiltDialog(NULL, NULL, trashDialogData, NULL, False,
                                   &classHints);
                TrashView = False;
                ...
                if (trash_icon == XmUNSPECIFIED_PIXMAP) {
                        ...
                        XmGetColors(XtScreen(file_mgr_rec->main), colormap,
                                    background, &foreground, &top_shadow,
                                    &bottom_shadow, &select);

                        pixmap = XmGetPixmap(XtScreen(file_mgr_rec->main),
                                             trashIcon, foreground, background);
                        if (pixmap != XmUNSPECIFIED_PIXMAP)
                                trash_icon = pixmap;

                        /* now let's get the mask for the File Manager */
                        pixmap =
                            _DtGetMask(XtScreen(file_mgr_rec->main), trashIcon);
                        if (pixmap != XmUNSPECIFIED_PIXMAP)
                                trash_mask = pixmap;
                ...
                if (DtWsmGetCurrentWorkspace(XtDisplay(trashShell), root,
                                             &current_ws) == Success)
                ...
                wmhints =
                    XGetWMHints(XtDisplay(trashShell), XtWindow(trashShell));
                wmhints->flags |= IconWindowHint;
                wmhints->initial_state = NormalState;
                XSetWMHints(XtDisplay(trashShell), XtWindow(trashShell),
                            wmhints);
                XFree(wmhints);
...
void TrashEmptyHandler(Tt_message msg) {
...

        {
                ...
                tmpStr = GETMESSAGE(
                    27, 84,
                    "When trash objects are shredded, they are "
                    "permanently\nremoved from your system.\n\nAre you "
                    "sure you want to shred?\n");
                msg = XtNewString(tmpStr);

                _DtMessageDialog(toplevel, title, msg, NULL, TRUE,
                                 ConfirmEmptyCancel, ConfirmEmptyOk, NULL,
...
void TrashRemoveHandler(Tt_message msg) {
         ...
         * re-entrance into TrashRemoveHandler. This will continue until the
         * last Dttrash_Remove msg. has been processed (and the recursive
        ...
        /* post message if file(s) from desktop */
        {
                ...
                str = (char *)IsAFileOnDesktop2(file_list, file_count, &number,
                                                &IsToolBox);
                /* IsToolBox is unused here, but is required to satisfy the
                 * prototype */
        }
        if (number == 0) {
                ...
                if (number == 1) {
                        if (widget_dragged)
                                template = (GETMESSAGE(
                                    27, 85,
                                    "The Workspace object you are deleting is "
                                    "a linked copy of\n   an "
                                    "object inside the File Manager at\n   "
                                    "%s\n   If you click OK, "
                                    "both the linked copy and the original "
                                    "object\n   will be "
                                    ...
                                    "From Workspace'\n         from the "
                                    "Workspace object's popup "
                                    "menu."));
                        else
                                template = (GETMESSAGE(
                                    27, 102,
                                    "A linked copy of the following object is "
                                    "on\nthe Workspace backdrop:\n%s\nMoving "
                                    "the "
                                    "object to the trash will remove it "
                                    "from\nboth "
                        ...
                        if (widget_dragged)
                                template = (GETMESSAGE(
                                    27, 86,
                                    "The Workspace objects you are deleting "
                                    "are linked copies of\n   "
                                    "objects inside the File Manager at\n%s\n  "
                                    " If you click OK, both "
                                    "the linked copies and the original "
                                    "objects\n   will be deleted.\n "
                                    "  If you just want to delete the "
                                    "Workspace objects,\n      1) "
                                    "click Cancel on this dialog\n      2) "
                                    "then select 'Remove From "
                                    "Workspace'\n         from the Workspace "
                                    "objects' popup menu."));
                        else
                                template = (GETMESSAGE(
                                    27, 103,
                                    "A linked copy of the following objects "
                                    "are also on\nthe Workspace "
                                    "backdrop:\n%s\nMoving the objects to the "
                ...
                _DtMessageDialog(toplevel, title, message, NULL, TRUE,
...
#else
                        {
                                ...
                                msg_cache = (Tt_msg_cache *)XtMalloc(
                                    sizeof(Tt_msg_cache));
                                msg_cache->msg_cnt = global_msg_cnt;

                                msg_cache->msg_list = (Tt_message *)XtMalloc(
                                    sizeof(Tt_message) * msg_cache->msg_cnt);
                                msg_cache->msg_list[msg_cache->msg_cnt] = NULL;
                                memcpy((char *)msg_cache->msg_list,
                                       (char *)global_msg_list,
                                       sizeof(Tt_message) * msg_cache->msg_cnt);
                                dlog = (Widget)_DtMessageDialog(
                                    toplevel, title, message, NULL, TRUE,
                                    RemoveCancelCB, RemoveOkCB, NULL,
                                    HelpRequestCB, False, QUESTION_DIALOG);
...
#else
                MessagesToFileList(current_msg_cache->msg_list,
                                   current_msg_cache->msg_cnt, &file_list,
                                   &file_count);
#endif /* SUN_PERF */
...
#else
                for (i = 0; i < current_msg_cache->msg_cnt; i++)
                        tttk_message_fail(current_msg_cache->msg_list[i],
                                          TT_DESKTOP_ECANCELED, 0, 1);
                XtFree((char *)current_msg_cache->msg_list);
                XtFree((char *)current_msg_cache);
#endif /* SUN_PERF */
}
...
static void MessageToFileList(Tt_message msg, char ***file_list,
        ...
        *file_list = 0;
        ...
        for (arg = 0; arg < args; arg++) {
                ...
                for (val = DtStrtok(val, white); val;
                     val = DtStrtok(0, white)) {
                        if (strcmp(val, "/") == 0) {
                                file = (char *)XtMalloc(dirlen + 1);
                                strcpy(file, dir);
                        } else {
                                file =
                                    (char *)XtMalloc(dirlen + strlen(val) + 1);
                                strcpy(file, dir);
                                strcat(file, val);
                        }
                        *file_list = (char **)XtRealloc((char *)(*file_list),
        ...
        for (num_msgs = 0; num_msgs < msg_cnt; num_msgs++) {
                ...
                if ((tt_is_err(tt_ptr_error(dir))) || (dir == 0)) {
                        ...
                        if (dir[strlen(dir)] != '/') {
                                ...
                                if (strcmp(dir, "/") == 0) {
                                        newdir =
                                            (char *)tt_malloc(strlen(dir) + 1);
                                        strcpy(newdir, dir);
                                } else {
                                        newdir =
                                            (char *)tt_malloc(strlen(dir) + 2);
                                        strcpy(newdir, dir);
                                        strcat(newdir, "/");
                                }
                                tt_free(dir);
                ...
                /* Invariant: dir can now be safely concatenated to form a valid
                 * path */
                dirlen = strlen(dir);
                for (arg = 0; arg < args; arg++) {
                        char *val;
...
void ConfirmRemove(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        _DtMessageDialog(toplevel, title, msg, NULL, TRUE, ConfirmCancel,
                         ConfirmOk, NULL, HelpRequestCB, False,
                         QUESTION_DIALOG);
...
void Remove(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (w != NULL) {
                ...
                if (client_data != NULL) {
                        ...
                        /* need to check to see if it is selected, if it is
                           restore all the selected files, else just restore
                           this file */

                        file_view_data = (FileViewData *)client_data;
                        for (i = 0; i < trashFileMgrData->selected_file_count;
                             i++) {
                                if (strcmp(file_view_data->file_data->file_name,
                                           trashFileMgrData->selection_list[i]
                                               ->file_data->file_name) == 0) {
                                        match = True;
                                }
                        }
                } else {
                ...
                deleteList =
                    (DeleteList *)XtMalloc(deleteCount * sizeof(DeleteList));
...
void Restore(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (client_data != NULL) {
                ...
                /* The object would have been deleted if the following condn is
                 * true */
                if (!file_view_data && !trashFileMgrData->selected_file_count)
                        return;
                trashFileMgrData->popup_menu_icon = NULL;
...
void DropOnTrashCan(int file_count, char **host_set, char **file_set,
        ...
        RepositionIcons(trashFileMgrData, file_set, file_count, drop_x, drop_y,
                        True);
}
...
void MoveOutOfTrashCan(FileMgrData *file_mgr_data, FileMgrRec *file_mgr_rec,
        ...
        RestoreFromTrash(file_list, file_count, target_host, target_dir, NULL,
                         False);
}
...
static void MoveToTrashProcess(int pipe_fd, char **file_list, int file_count,
        ...
        for (i = 0; i < file_count; i++) {
                ...
                if (baseName == NULL || path == NULL) {
                        ...
                        rc = BAD_FILE_ERROR;
                        DPRINTF(
                            ("MoveToTrashProcess: sending BAD_FILE_ERROR\n"));
                ...
                if (path && MatchesSacredDirectory(path)) {
                        ...
                        rc = BAD_FILE_SACRED;
                        DPRINTF(
                            ("MoveToTrashProcess: sending BAD_FILE_SACRED\n"));
                        write(pipe_fd, &pipe_msg, sizeof(short));
                        write(pipe_fd, &rc, sizeof(int));
                        continue;
                }
                ...
                baseName++;
                if (do_verify_checks) {
                        char *tmpstr = (file_list[i] == (baseName - 1))
                                           ? "/"
                                           : file_list[i];
                        struct stat s1;
                        ...
                        if (TrashError) {
                                XtFree(path);
                                pipe_msg = PIPEMSG_OTHER_ERROR;
                                DPRINTF(("MoveToTrashProcess: sending "
                                         "BAD_TRASH message\n"));
                                write(pipe_fd, &pipe_msg, sizeof(short));
                                write(pipe_fd, &rc, sizeof(int));
                                continue;
                        } else if (CheckAccess(path, W_OK) != 0 &&
                                   !S_ISLNK(s1.st_mode)) {
                ...
                success =
                    FileManip((Widget)(intptr_t)pipe_fd, MOVE_FILE, path, to,
                              TRUE, FileOpError, True, TRASH_DIRECTORY);
                if (success) {
...
static void MoveToTrashPipeCB(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        default:
                fprintf(
                    ...
                    pipe_msg);
                /* Don't know how it can get in here, but if it does, we'll try
                 * to simulate an error condition.  Without it, the SGI hung up
                 */
                    ...
                    cb_data->rc[i] == BAD_TRASH_FILE) {
                        if (badCount < 8) {
                                ...
                                if (cb_data->rc[i] == BAD_TRASH_DIRECTORY) {
                                        tmpmsg = GETMESSAGE(
                                            27, 113,
                                            "At least one object in the "
                                            "following Folder(s) "
                                            "or \nthe folder itself cannot be "
                                            "deleted.\nThe "
                                            "most likely cause is that you do "
                                            "not have "
                                            "permissions\nto delete that "
                                            "object.\n\n");
                                        if (dir_error) {
                                        ...
                                        tmpmsg = GETMESSAGE(
                                            27, 114,
                                            "The following file(s) cannot be "
                                            "deleted.  The "
                                            "most likely cause \nis that you "
                                            "do not have "
                                            "permissions to delete the "
                                            "file(s).\n\n");
                                        if (file_error) {
                        ...
                        if (badCount < 8) {
                                char *tmpmsg = GETMESSAGE(
                                    27, 115,
                                    "The following object(s) cannot be "
                                    "deleted.\nThe desktop cannot function "
                                    "properly without these object(s).\n\n");
                                if (sacred_error) {
                                        sacred_error = (char *)XtRealloc(
                                            sacred_error,
                                            strlen(sacred_error) +
                                                strlen(cb_data->file_list[i]) +
                                                5);
                                        strcat(sacred_error,
                                               cb_data->file_list[i]);
                                        strcat(sacred_error, "\n");
                        ...
                        tmpmsg = GETMESSAGE(
                            ...
                            "most likely cause is that these object(s)\nhave "
                        ...
                        if (verifyCount < 6)
                                AddString(&verifybuf, &verifybufsize,
                                          cb_data->file_list[i], NULL);
                        verifyCount++;

                        /*
                         * Keep track of whether we have directories only, files
                         * only, or a combination, so that we can display the
                         * appropriate label in the dialog.
                         */
                        ...
                        /*
                         * If the source file was referenced by a desktop
                         * object, we need to remove the destkop object
                         */
        ...
        if (verifybuf) {
                ...
                if (vfiles && vdirs)
                        header = GETMESSAGE(
                            27, 6,
                            "   Each of the following is either a non-empty "
                            "folder,   "
                ...
                dlog = (Widget)_DtMessageDialog(
                    ...
                    NULL, HelpRequestCB, False, WARNING_DIALOG);
                XtFree(title);
...
static void MoveToTrash(char **file_list, int file_count,
        ...
        if (global_msg_cnt > 0) {
                ...
                cb_data->msg_list = (Tt_message *)XtMalloc(sizeof(Tt_message) *
                                                           cb_data->msg_cnt);
        ...
        if (pid == 0) {
                ...
                MoveToTrashProcess(pipe_fd[1], file_list, file_count,
                                   do_verify_checks);
                close(pipe_fd[1]);
...
static void RestoreProcess(int pipe_fd, char **file_list, int file_count,
        ...
        if (target_dir) {
                Tt_status tt_status;
                full_dirname = ResolveLocalPathName(
                    target_host, target_dir, NULL, home_host_name, &tt_status);
                if (TT_OK != tt_status) {
                        /* send return codes back trough the pipe */
                        pipe_msg = PIPEMSG_DONE;
                        rc[0] = -1;
                        DPRINTF(("RestoreProcess: Unable to Resolve local path "
                                 "name\n"));
                        write(pipe_fd, &pipe_msg, sizeof(short));
                        write(pipe_fd, rc, sizeof(int));
                        return;
        ...
        for (i = 0; i < file_count; i++) {
                ...
                for (j = 0; j < numTrashItems; j++) {
                        /* file_list[i] may be a complete path or just a file
                         * name */
                ...
                if (target_dir == NULL) {
                        ...
                        if (j < numTrashItems) {
                                ...
                                /* can't do a restore if the file wasn't found
                                 * in the trash list */
                                from = to = NULL;
                        }
                } else {
                ...
                if (to != NULL) {
                        ...
                        status =
                            RestoreObject((Widget)(intptr_t)pipe_fd, MOVE_FILE,
                                          from, to, TRUE, FileOpError, False,
...
static void RestorePipeCB(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        for (i = 0; i < cb_data->file_count; i++) {
                ...
                for (j = 0; j < numTrashItems; j++) {
                        /* file_list[i] may be a complete path or just a file
                         * name */
                        if (strcmp(cb_data->file_list[i],
                                   trashCan[j].filename) == 0 ||
                            strcmp(cb_data->file_list[i], trashCan[j].intNew) ==
                                0) {
                                break;
                        }
                }
                ...
                if (cb_data->rc[i] == SKIP_FILE) {
                        ToRestoreList = (char **)XtRealloc(
                            (char *)ToRestoreList,
                            sizeof(char *) * (++RestoreIndex));
                        ToRestoreList[RestoreIndex - 1] =
                            XtNewString(trashCan[j].intOrig);
                        FromRestoreList =
                            (char **)XtRealloc((char *)FromRestoreList,
                                               sizeof(char *) * RestoreIndex);
                        FromRestoreList[RestoreIndex - 1] =
                            XtNewString(trashCan[j].intNew);
                        continue;
                }
                /* Check the return code from the restore */
                else if (cb_data->rc[i] == 0) {
                        /* restore was successful: remove the file from the
                         * trash list */
                        if (j < numTrashItems) {
                                /* arrange for the source directory to be
                                 * updated */
                        ...
                        if (j < numTrashItems && cb_data->target_dir == NULL)
                                AddString(&buf, &bufsize, trashCan[j].external,
                                          restore_header);
                        else
                                AddString(&buf, &bufsize, cb_data->file_list[i],
                                          restore_header);
        ...
        if (cb_data->target_dir)
                UpdateDirectory(NULL, cb_data->target_host,
                                cb_data->target_dir);
        ...
        if (ToRestoreList) {
                for (i = 0; i < RestoreIndex; i++) {
                        CreateRestoreDialog(FromRestoreList[i],
                                            ToRestoreList[i]);
...
static void RestoreFromTrash(char **file_list, int file_count,
        ...
        if (pid == 0) {
                ...
                RestoreProcess(pipe_fd[1], file_list, file_count, target_host,
                               target_dir, cb_data->rc, CheckedAlready);
                close(pipe_fd[1]);
        ...
        XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_fd[0],
                      (XtPointer)XtInputReadMask, RestorePipeCB,
                      (XtPointer)cb_data);
}
...
static void EmptyTrashPipeCB(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        if (cb_data->removeType == TRASH_FILE) {
                for (i = 0; i < cb_data->del_count; i++) {
                        /* Locate file in trash list */
                        for (j = 0; j < numTrashItems; j++)
                                if (strcmp(cb_data->del_list[i].trash,
                                           trashCan[j].intNew) == 0)
                                        break;

                        /* Check the return code from the erase */
                        if (cb_data->rc[i] == 0) {
                                /* erase was successful: remove the file from
                                 * the trash list */
                                if (j < numTrashItems) {
                                        /* Remove this entry from the trash list
                                         */
                                ...
                                if (itemCount < 8) {
                                        ...
                                        if (j < numTrashItems)
                                                ...
                                                file_name =
                                                    cb_data->del_list[i].trash;
                                        AddString(&buf, &bufsize, file_name,
                                                  msg);
                                        XtFree(msg);
                                        itemCount++;
                                }
                ...
                if (!WriteTrashEntries()) {
                        ...
                        title =
                            XtNewString(GetSharedMessage(TRASH_ERROR_TITLE));
                        tmpStr = XtNewString(GETMESSAGE(
                            27, 88,
                            "Cannot write to a temporary file.\nPerhaps "
                            "your file system is full.\n"));
                        _DtMessage(toplevel, title, tmpStr, NULL,
                                   HelpRequestCB);
                        XtFree(title);
                        XtFree(tmpStr);
                }
                ...
                if (cb_data->rc[0] == 0) {
                        /* arrange for the directory containing the file to be
                         * updated */
                        UpdateDirectoryOf(cb_data->del_list[0].trash);
                } else {
                        /*  put up an error message saying the file couldn't be
                         * removed */
                        char *buffer;
                        char *tmpStr;
...
#if defined(CSRG_BASED)
                if (!strcmp(statbuf.f_fstypename,
                            "nfs")) /* Root user and nfs */
#else
                        if (FileSysType(statbuf.st_dev) <
                            0) /* Root user and nfs */
#endif
                {
                        char *tmpfile;
                        tmpfile = tempnam(parentdir, "quang");
                        if (creat(tmpfile, O_RDONLY) <
                ...
                if (strcmp(dp->d_name, ".") != 0 &&
                    strcmp(dp->d_name, "..") != 0) {
                        if (first_file) {
                                 ...
                                 */
                                if (CheckAccess(destinationPath, W_OK | X_OK) <
                                    0)
                                        return -1;

                                /* append a '/' to the end of directory name */
                                fnamep =
                                    destinationPath + strlen(destinationPath);
                                *fnamep++ = '/';
