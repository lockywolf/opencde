...
static void Create(Display *display, Widget parent, Widget *shell,
        ...
        XtAddCallback(change_dir, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_CHANGEDIR_DIALOG_STR);
        ...
        XtAddCallback(XtParent(w), XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_CHANGEDIR_DIALOG_STR);
...
static void InstallClose(ChangeDirRec *change_dir_rec, XtCallbackProc callback,
        ...
        XmAddWMProtocolCallback(change_dir_rec->shell, delete_window_atom,
...
static XtPointer GetValues(ChangeDirRec *change_dir_rec) {
        ...
        n++;
        XtSetArg(args[n], XmNlistVisibleItemCount,
                 &change_dir_data->visible_count);
        n++;
        ...
        for (n = 0; n < change_dir_data->list_count; n++)
                change_dir_data->history_list[n] =
                    XmStringCopy(history_list[n]);
...
static XtPointer GetDefaultValues(void) {
        ...
        /*
           ...
           XmFONTLIST_DEFAULT_TAG); else change_dir_data->string_path = NULL;
        */
...
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list) {
        ...
        if (change_dir_data->list_count != 0) {
                new_table = (XmStringTable)XtMalloc(
                    sizeof(XmStringTable) * (change_dir_data->list_count + 1));
                ...
                /******************
                  ...
                  CONVERTER.  EITHER REALLOC THE RETURNED TABLE OR COPY AND FREE
                IT.
                ******************/
...
static void SetValues(ChangeDirRec *change_dir_rec,
        ...
        XtSetArg(args[1], XmNlistVisibleItemCount,
                 change_dir_data->visible_count);
        ...
        if (change_dir_data->string_path) {
                ...
                text = XmSelectionBoxGetChild(change_dir_rec->change_dir,
                                              XmDIALOG_TEXT);
...
static Boolean TryToChangeDir(Boolean isFromDialog, char *incoming_string,
        ...
        if ((new_directory) && (strcmp(new_directory, "") != 0)) {
                _DtPathFromInput(new_directory,
                                 file_mgr_data->current_directory, &host,
                                 &path);
        } else {
                ...
                if (isFromDialog)
                        _DtMessage(selection_box, title, msg, NULL,
                                   HelpRequestCB);
                else
        ...
        /* Stat the file and see if it is a valid directory.  (If the current
           view is restricted, make sure that the new directory doesn't violate
           the directory restrictions. If so, refilter the displayed file set to
           show the files in this directory.
        */
        if ((path) && (stat(path, &stat_buf) == 0) &&
            ((stat_buf.st_mode & S_IFMT) == S_IFDIR) &&
            (CheckRestrictedDir(file_mgr_rec, file_mgr_data, &path, False) ==
             0)) {
                ...
                if (CheckAccess(path, R_OK | X_OK)) {
                        ...
                        if (isFromDialog)
                                ...
                                _DtMessage(file_mgr_rec->current_directory_text,
                        ...
                        if (selection_box) {
                                /* Adjust the selection box elements to add the
                                 text item into the list.
                                */
                                list = XmSelectionBoxGetChild(selection_box,
                                                              XmDIALOG_LIST);
                        ...
                        */
                        ...
                        if (selection_box) {
                                ...
                                XtSetArg(args[0], XmNlabelString, host_string);
                                XtSetValues(
                                    XmSelectionBoxGetChild(selection_box,
                                                           XmDIALOG_LIST_LABEL),
                                    args, 1);
                                XtFree(message_buf);
                                XmStringFree(host_string);
                        ...
                        if (isFromDialog)
                                (*callback)(selection_box,
                                            apply_data->client_data, path);
                        else
                ...
                if (isFromDialog)
                        _DtMessage(selection_box, title, message_buf, NULL,
                                   HelpRequestCB);
                else
                        _DtMessage(file_mgr_rec->current_directory_text, title,
                                   message_buf, NULL, HelpRequestCB);
...
static void OkCallback(Widget selection_box, ChangeDirApply *apply_data,
        ...
        if (value == NULL) {
                tmpStr = GetSharedMessage(CHANGE_DIR_ERROR_TITLE);
                title = XtNewString(tmpStr);
                tmpStr = (char *)GETMESSAGE(2, 11,
                                            "Cannot get the new folder name.");
                msg = XtNewString(tmpStr);
        ...
        if (file_mgr_data->toolbox && strValue[0] != '/')
                result = CheckRestrictedDir(file_mgr_rec, file_mgr_data,
                                            &strValue, True);
        else
                result = CheckRestrictedDir(file_mgr_rec, file_mgr_data,
                                            &strValue, False);
        ...
        if (result == 0) {
                TryToChangeDir(True, (char *)strValue, file_mgr_data,
                               ...
                               apply_data, callback_data, apply_data->callback);
        }
        ...
        if (strncmp(*value, directory, len) != 0 ||
            (*value)[len] != '/' && (*value)[len] != '\0') {
                ...
                if (restrictMode)
                        tmpBuffer =
                            GETMESSAGE(2, 8,
                                       "You cannot switch to this folder.  You "
                                       "are\nallowed to "
                                       "view only folders beneath "
                                       "your\ncurrent folder. You "
                                       "cannot specify an absolute\npath to "
                                       "the new folder.");
                else
                        tmpBuffer = GETMESSAGE(
                            2, 9,
...
void ChangeToNewDir(Widget text_field, XtPointer client_data,
        ...
        /*
           ...
           HCI requests that if the switch is not successful, don't de-activate
           it so user has a change to go back and correct it without having to
           pop it up again.
...
void DropOnChangeView(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (numFiles > 0 && fileList->reason == DtCR_DND_DROP_ANIMATE) {
                ...
                full_path_name =
                    XtNewString(fileList->dropData->data.files[numFiles - 1]);
                ...
                if (CheckRestrictedDir(file_mgr_rec, file_mgr_data,
                        ...
                        TryToChangeDir(False, full_path_name, file_mgr_data,
                                       file_mgr_rec, change_dir_data,
                                       selection_box, NULL, NULL, GoToNewDir);
                }
...
static void GoToNewDir(FileMgrData *file_mgr_data, char *dir_name) {
        ...
        if (file_mgr_data->change_dir) {
                ...
                for (i = 0, found = False; i < change_dir_data->list_count;
                     i++) {
                        if (XmStringCompare(xm_dir_name,
                                            change_dir_data->history_list[i])) {
                                found = True;
                ...
                if (!found) {
                        ...
                        change_dir_data->history_list =
                            (XmStringTable)XtRealloc(
                                (char *)change_dir_data->history_list,
                                sizeof(XmStringTable) *
                                    (change_dir_data->list_count + 1));
                        change_dir_data
                            ->history_list[change_dir_data->list_count - 1] =
                            xm_dir_name;
                        change_dir_data
                            ->history_list[change_dir_data->list_count] = NULL;
                }

                /* Update view to display the new directory */
                ShowNewDirectory(file_mgr_data, change_dir_data->host_name,
                                 dir_name);
        }
}
...
static void SetFocus(ChangeDirRec *change_dir_rec,
        ...
        text =
            XmSelectionBoxGetChild(change_dir_rec->change_dir, XmDIALOG_TEXT);
                ...
                if (selection != NULL) {
                        if (callback_data->event == NULL)
                                ...
                                XmTextFieldClearSelection(
                                    text, callback_data->event->xkey.time);
                        callback_data->doit = False;
                } else if (callback_data->reason == XmCR_MODIFYING_TEXT_VALUE) {
                        /* see if the user is trying to back over the
                         * 'restricted' text, in this case users_home_dir
                         */
                        if (strcmp(homeDir, "") != 0 &&
                            strcmp(homeDir,
