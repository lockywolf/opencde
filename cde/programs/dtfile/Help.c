...

{
        ...
        _DtDialogGetResources(data_base, name_list, HELP_DIALOG,
                              (char *)helpData, resources,
...

{
...
#endif
                desc = DtDtsDataTypeToAttributeValue(
                    filetype, DtDTS_DA_DESCRIPTION, NULL);
        ...
        if (isAction) {
                tmpStr = (GETMESSAGE(
                    29, 5, "There is no help available for the %s action"));
                helpPattern = XtNewString(tmpStr);
        } else {
                tmpStr = (GETMESSAGE(
                    29, 6, "There is no help available for the %s filetype"));
                helpPattern = XtNewString(tmpStr);
        }
