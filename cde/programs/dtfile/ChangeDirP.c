...
static void CurrentDirChange(XtPointer client_data, DialogData *old_dialog_data,
                             DialogData *new_dialog_data, XtPointer call_data);
static void CurrentDirClose(XtPointer client_data, DialogData *old_dialog_data,
                            DialogData *new_dialog_data);
static void CheckCurrentDirectorySelect(FileMgrData *file_mgr_data);
static void TimerEvent(XtPointer client_data, XtIntervalId *id);
static void ResizeFastText(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
                           short columns);
static int get_textwidth(FileMgrData *fmd, char *str, int len);
static void draw_imagestring(Display *display, Drawable d, FileMgrData *fmd,
                             GC gc, int x, int y, char *text, int bytes);
...
static Boolean get_text_pieces(FileMgrData *file_mgr_data, int width,
                               char buf[], int *host_len_p, int *host_pixels_p,
                               int *prefix_len_p, int *prefix_pixels_p,
                               int *path_len_p, int *path_pixels_p) {
        ...
        *host_len_p = *host_pixels_p = 0;
        ...
        if (file_mgr_data->restricted_directory) {
                ...
                prefix_len = strlen("/...");
                prefix_pixels =
                    get_textwidth(file_mgr_data, "/...", prefix_len);
        ...
        if (prefix_pixels + path_pixels > width) {
                ...
                do {
                        ...
                        if (next_part == NULL) {
                                /* Got here only when the last directory is
                                   still too long to display
                                */
                                break;
                        }
                        ...
                        path_len = strlen(path_begin);
                        path_pixels =
                            get_textwidth(file_mgr_data, path_begin, path_len);
        ...
        if (NULL == next_part) {
                ...
                while (0 != len) {
                         ...
                         */
                        path_pixels =
                            get_textwidth(file_mgr_data, path_begin, len);
                        if (prefix_pixels + path_pixels < width) {
                                break;
                        }
                        --len;
...
/*ARGSUSED*/
void ShowChangeDirDialog(Widget w, XtPointer client_data, XtPointer callback) {
        ...
        _DtShowDialog(file_mgr_rec->shell, (Widget)NULL,
                      (XtPointer)file_mgr_rec, file_mgr_data->change_dir,
        ...
        file_mgr_rec->change_directoryBtn_child =
            XtParent(change_dir_rec->change_dir);
}
...
void CurrentDirSelected(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        /*  directory text.                                       */
        chopped = get_text_pieces(file_mgr_data, width - 2 * left_margin, buf,
                                  &host_len, &host_pixels, &prefix_len,
                                  &prefix_pixels, &path_len, &path_pixels);
        ...
        if (event->x < begin_x || event->x >= end_x) {
                ...
                new_select = NULL;
                   ...
                   event->x < begin_x + host_pixels + prefix_pixels &&
                       !chopped) {
        ...
        } else if (event->x < begin_x + host_pixels + prefix_pixels &&
                   chopped) {
        ...
        } else /* event->x >= begin_x + host_pixels + prefix_pixels */
        {
                ...
                /* if we have a restricted diretory and i == 5 ("/.../") then we
                   want the len to be the restricted directory */
                ...
                if (len == 0)
                        ...
                        memcpy(new_select, file_mgr_data->current_directory,
                               len);
                        new_select[len] = 0;
                }
        }
...
void CurrentDirDropCallback(Widget w, XtPointer client_data,
                            XtPointer call_data) {
        FileMgrRec *file_mgr_rec = (FileMgrRec *)client_data;
...
void CurrentDirIconCallback(Widget w, XtPointer client_data,
                            XtPointer call_data) {
        FileMgrRec *file_mgr_rec = (FileMgrRec *)client_data;
        XmAnyCallbackStruct *callback;
        XButtonEvent *event;
        ...
        if (callback->reason == XmCR_DRAG) {
                ...
                /* but since we're in the current directory icon we don't want
                   to process on the button up */
...
void CurrentDirectoryIconMotion(Widget w, XtPointer client_data,
                                XEvent *event) {
            ...
            (B2DragPossible && (event->xmotion.state & Button2Mask))) {
                ...
                if ((ABS(diffX) >= dragThreshold) ||
                    (ABS(diffY) >= dragThreshold)) {
                        dialog_data =
                            _DtGetInstanceData((XtPointer)file_mgr_rec);
                        file_mgr_data = (FileMgrData *)dialog_data->data;
...
void CurrentDirExposed(Widget w, XtPointer client_data, XtPointer call_data) {
        FileMgrRec *file_mgr_rec = (FileMgrRec *)client_data;
        DialogData *dialog_data;
        FileMgrData *file_mgr_data;
                        ...
                        for (j = 0;
                             ...
                             j++) {
                                if (file_view_data[j]->filtered &&
                                    (ts == tsAll ||
                                     file_view_data[j]->file_data->is_subdir)) {
                                        n_hidden++;
                                }
                        }
                }
...
void DrawCurrentDirectory(Widget w, FileMgrRec *file_mgr_rec,
                          FileMgrData *file_mgr_data) {
        Arg args[8];
        ...
        top_margin = (height > (Dimension)font_height)
                         ? (Dimension)(height - font_height + 1) / (Dimension)2
                         : 0;
        left_margin = highlight + shadow + margin;

        /* Ensure the area is cleared out. */
        XClearArea(XtDisplay(w), XtWindow(w), highlight, highlight,
                   width - 2 * highlight, height - 2 * highlight, False);
        ...
        draw_x = left_margin;
        if (draw_x < 0 ||
            (Dimension)draw_x > width) /* Make sure that it's in bound */
                draw_x = 0;

        draw_y = top_margin + font_yoffset;
        if (draw_y < 0 ||
            (Dimension)draw_y > height) /* Make sure that it's in bound */
        {
                draw_y = height - 5;
        }
        ...
        chopped = get_text_pieces(file_mgr_data, dir_width, buf, &host_len,
                                  &host_pixels, &prefix_len, &prefix_pixels,
        ...
        if (!file_mgr_data->show_iconic_path &&
            !file_mgr_data->show_status_line) {
                ...
                if (!file_mgr_data->fast_cd_enabled)
                        ...
                        XtGetValues(file_mgr_rec->current_directory_text, args,
                                    1);
                ...
                if ((Dimension)(draw_x + CUR_DIR_SPACING + msg_width +
                                left_margin) <= width) {
                        draw_x = width - left_margin - msg_width;
                        draw_imagestring(XtDisplay(w), XtWindow(w),
                                         file_mgr_data,
                                         file_mgr_data->cd_normal_gc, draw_x,
                                         draw_y, msg, strlen(msg));
                        draw_x -=
                            shadow + CUR_DIR_SPACING; /* right edge of shadow */
                        msg_drawn = True;
                } else
                        msg_drawn = False;
        }
        ...
        if (!file_mgr_data->fast_cd_enabled) {
                ...
                XmeDrawShadows(
                    XtDisplay(w), XtWindow(w), tf->primitive.top_shadow_GC,
                    tf->primitive.bottom_shadow_GC, highlight, highlight,
                    shadow_width, shadow_height, shadow, XmSHADOW_IN);
        }
}
...
static void CurrentDirChange(XtPointer client_data, DialogData *old_dialog_data,
        ...
        new_change_dir_data->host_name =
            XtNewString(old_change_dir_data->host_name);
...
static void CheckCurrentDirectorySelect(FileMgrData *file_mgr_data) {
        ...
        if (file_mgr_data == NULL || file_mgr_data->cd_select == NULL ||
            file_mgr_data->current_directory == NULL)
                return;

        /* get the true lengths of current_directory and
        ...
        while (1) {
                ...
                *ptr = '\0';
                ...
                if (length_cd > length_cd_s) {
                        XtFree(file_mgr_data->cd_select);
                        file_mgr_data->cd_select =
                            (char *)XtMalloc(strlen(str) + 1);
 ...
 *
 ***************************************************************************/
void ShowFastChangeDir(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data) {
        char *textString;
        Arg args[16];
        ...
        XtRemoveAllCallbacks(file_mgr_rec->current_directory,
                             XmNexposeCallback);
        ...
        if (file_mgr_data->restricted_directory == NULL) {
                if (strcmp(file_mgr_data->host, home_host_name) == 0)
                        ...
                        textString = DtCreateContextString(
                            file_mgr_data->host,
                            file_mgr_data->current_directory, NULL);
        } else /* is a toolbox, so put the subset of what the toolbox is in the
                  text*/
        {
                char *ptr;
        ...
        if (file_mgr_data->restricted_directory == NULL)
                ...
                XtGetValues(file_mgr_rec->current_directory_text, args, 2);
                chopped =
                    get_text_pieces(file_mgr_data, width - 2 * left_margin, buf,
                                    &host_len, &host_pixels, &prefix_len,
                                    &prefix_pixels, &path_len, &path_pixels);
