...
static void BranchListToString(int fd, char ***value, char *out_buf);
static void SelectionListToString(int fd, FileViewData ***value, char *out_buf);
...
static void Create(Display *display, Widget parent, Widget *return_widget,
        ...
        trans_table = XtParseTranslationTable(translations_sp_esc);
        {
                ...
                resolvedTranslationString = ResolveTranslationString(
                    translations_da, (char *)file_mgr_rec);
                trans_table1 =
                    XtParseTranslationTable(resolvedTranslationString);
        ...
        if (!first &&
            (special_view == True && special_title != NULL && !TrashView)) {
                file_mgr_rec->shell = shell = XtAppCreateShell(
                    special_title, DTFILE_CLASS_NAME,
                    applicationShellWidgetClass, display, args, 1);
                XtFree(special_title);
                special_title = NULL;
        } else
                file_mgr_rec->shell = shell = XtAppCreateShell(
                    application_name, DTFILE_CLASS_NAME,
                    applicationShellWidgetClass, display, args, 1);
        if (!TrashView)
                special_view = False;
        ...
        XmAddWMProtocolCallback(shell, delete_window,
                                (XtCallbackProc)SystemClose,
                                (XtPointer)file_mgr_rec);

        file_mgr_rec->main = mainWidget =
            XmCreateMainWindow(shell, "main", args, 1);
        XtManageChild(mainWidget);
        XtAddCallback(mainWidget, XmNhelpCallback,
        ...
        if (TrashView) {
                ...
                file_mgr_rec->menu = menu =
                    CreateTrashMenu(mainWidget, file_mgr_rec);
        } else {
                file_mgr_rec->menuStates =
                    (RENAME | MOVE | DUPLICATE | LINK | TRASH | MODIFY |
                     CHANGEDIR | PREFERENCES | FILTER | FIND | CREATE_DIR |
                     CREATE_FILE | SETTINGS | CLEAN_UP | MOVE_UP | HOME |
                     CHANGE_DIR | TERMINAL);

                file_mgr_rec->menu = menu =
                    CreateMenu(mainWidget, file_mgr_rec);
        }
        ...
        if (showFilesystem && !TrashView) {
                ...
                /* Create the separator between iconic path and current
                 * directory */
                n = 0;
                ...
                file_mgr_rec->header_separator = header_separator =
                    XmCreateSeparatorGadget(header_frame, "header_separator",
                                            args, n);
                ...
                XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
                n++;
                file_mgr_rec->current_directory_frame =
                    current_directory_frame = XmCreateForm(
                        header_frame, "current_directory_frame", args, n);
                ...
                XtAddEventHandler(current_directory_frame,
                                  ...
                                  (XtEventHandler)CurrentDirectoryIconMotion,
                                  file_mgr_rec);

                /* Create the change directory drop zone. */
                if (showDropZone) {
                        if (change_view_pixmap == XmUNSPECIFIED_PIXMAP) {
                                change_view_pixmap = _DtGetPixmap(
                                    XtScreen(current_directory_frame),
                        ...
                        file_mgr_rec->current_directory_drop =
                            current_directory_drop = _DtCreateIcon(
                                current_directory_frame, "drop_cd", args, n);
                ...
                file_mgr_rec->current_directory_icon = current_directory_icon =
                    _DtCreateIcon(current_directory_frame,
                                  "current_directory_icon", args, n);
                ...
                /*
                 * Make sure the font & highlight thickness of the text field is
                 * the same as the corresponding values for the current
                 * directory line.
                 */
                ...
                /*
                 * Calculate how high to make the current directory line by
                 * adding up the font height and the highlight and shadow
                 * thickness
                 */
                XtSetArg(args[0], XmNshadowThickness, &shadow_thickness);
                XtGetValues(file_mgr_rec->current_directory_text, args, 1);

                entry_font = XmFontListEntryGetFont(
                    FontListDefaultEntry(font_list), &type);
                ...
                curdir_height =
                    font_height + 2 * (highlight_thickness + shadow_thickness);
                ...
                XtSetArg(args[0], XmNheight,
                         curdir_height + 2 * cur_dir_offset);
                ...
                XtAddCallback(file_mgr_rec->current_directory_text,
                              XmNhelpCallback, (XtCallbackProc)HelpRequestCB,
                              HELP_FILE_MANAGER_VIEW_STR);
#ifdef FOO
                /* This code is here so the Text Path (used to be Fast Change
                   Dir text) will be checked if user is ever try to type on the
                   portion before his restricted path. It's no longer needed
                   because, there won't be anything since I'm tryting to treat
                   restrictMode the same way I treat restricted directory (i.e.
                   dtfile -restricted). Why I want to treat them the same way?
                   Because they are the same sort of things and it, also, solves
                   the iconic path problem. Note that user can double click on
                   one of the icon and to a directory above the restricted
                   directory.
                */
                if (restrictMode) {
        ...
        XmMainWindowSetAreas(mainWidget, menu, file_mgr_rec->header_frame, NULL,
                             NULL, work_frame);
        ...
        XtSetArg(args[1], XmNverticalScrollBar,
        ...
        n++;
        file_mgr_rec->file_window = XmCreateDrawingArea(
            file_mgr_rec->scroll_window, "file_window", args, n);
        ...
        if (fileMgrPopup.menu == NULL)
                ...
                XmAddToPostFromList(fileMgrPopup.menu,
        ...
        XtOverrideTranslations(file_mgr_rec->file_window,
                               (XtTranslations)((XmManagerClassRec *)XtClass(
                                                    file_mgr_rec->file_window))
...
static XtPointer GetDefaultValues(void) {
        ...
        file_mgr_data->preferences =
            _DtGetDefaultDialogData(preferences_dialog);
...
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list) {
        ...
        if (first) {
                ...
                XtAppAddConverter(XtWidgetToApplicationContext(toplevel),
                                  XmRString, SHOW_TYPE, StringToShowType, NULL,
                                  0);
                XtAppAddConverter(XtWidgetToApplicationContext(toplevel),
                                  XmRString, TREE_FILES, StringToTreeFiles,
                                  NULL, 0);
                XtAppAddConverter(XtWidgetToApplicationContext(toplevel),
                                  XmRString, VIEW, StringToView, NULL, 0);
                XtAppAddConverter(XtWidgetToApplicationContext(toplevel),
                                  XmRString, ORDER, StringToOrder, NULL, 0);
                XtAppAddConverter(XtWidgetToApplicationContext(toplevel),
                                  XmRString, DIRECTION_RESRC, StringToDirection,
                                  NULL, 0);
                XtAppAddConverter(XtWidgetToApplicationContext(toplevel),
                                  XmRString, RANDOM, StringToRandom, NULL, 0);
                XtAppSetTypeConverter(XtWidgetToApplicationContext(toplevel),
                                      XmRString, BRANCH_LIST,
                                      (XtTypeConverter)StringToBranchList, NULL,
                                      0, XtCacheNone, NULL);
                XtAppAddConverter(XtWidgetToApplicationContext(toplevel),
                                  XmRString, SELECTION_LIST,
                                  (XtConverter)StringToSelectionList, NULL, 0);
        }
        ...
        _DtDialogGetResources(data_base, name_list, FILEMGR,
                              (char *)file_mgr_data, resources,
        ...
        if (restoreType != NORMAL_RESTORE) {
                ...
                if (special_view && !TrashView) {
                        if (special_title != NULL)
                                file_mgr_data->title =
                                    XtNewString(special_title);
                        else
                                file_mgr_data->title =
                                    XtNewString(file_mgr_data->title);

                        if (special_helpVol != NULL)
                                file_mgr_data->helpVol =
                                    XtNewString(special_helpVol);
                        else
                                file_mgr_data->helpVol =
                                    XtNewString(file_mgr_data->helpVol);
                        ...
                        if (special_viewType != UNSET_VALUE) {
                                ...
                                if (file_mgr_data->show_type ==
                                    MULTIPLE_DIRECTORY)
                                        file_mgr_data->view_tree =
                                            special_viewType;
                                else
                                        file_mgr_data->view_single =
                                            special_viewType;
                        }
                        ...
                        if (special_directionType != UNSET_VALUE)
                                file_mgr_data->direction =
                                    special_directionType;

                        if (special_randomType != UNSET_VALUE)
                                file_mgr_data->positionEnabled =
                                    special_randomType;
                } else {
                        file_mgr_data->title =
                            XtNewString(file_mgr_data->title);
        ...
        if (restoreType == NORMAL_RESTORE &&
            file_mgr_data->positionEnabled == RANDOM_ON) {
                ...
                RestorePositionalData(data_base, name_list, file_mgr_data,
                                      FILEMGR);
        }

        /*  Get a copy of the name list to be used to build new name lists.  */

        _DtDuplicateDialogNameList(name_list, new_name_list, 20,
                                   &name_list_count);
        ...
        if (restoreType == NORMAL_RESTORE) {
                ...
                if (((DialogInstanceData *)(file_mgr_data->primaryHelpDialog
                                                ->data))
                        ->displayed == False) {
         ...
         */
        if (restoreType != NORMAL_RESTORE) {
                ((DialogInstanceData *)(file_mgr_data->change_dir->data))
                    ->displayed = False;

                ((DialogInstanceData *)(file_mgr_data->preferences->data))
                    ->displayed = False;
        }
        ...
        if (restoreType != NORMAL_RESTORE) {
                ((DialogInstanceData *)(file_mgr_data->filter_edit->data))
                    ->displayed = False;

                ((DialogInstanceData *)(file_mgr_data->find->data))->displayed =
                    False;
        }
...
static void SetValues(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data) {
        ...
        if (showFilesystem && !TrashView) {
                ...
                XtAddCallback(cd, XmNarmCallback, CurrentDirSelected,
                              file_mgr_rec);

                XtRemoveAllCallbacks(cd, XmNexposeCallback);
                XtAddCallback(cd, XmNexposeCallback, CurrentDirExposed,
                              file_mgr_rec);

                XtRemoveAllCallbacks(cdi, XmNcallback);
                XtAddCallback(cdi, XmNcallback, CurrentDirIconCallback,
                              file_mgr_rec);

                if (cdd) {
                        XtRemoveAllCallbacks(cdd, XmNcallback);
                        XtAddCallback(cdd, XmNcallback, CurrentDirDropCallback,
                                      file_mgr_rec);
                }
        ...
        if (file_mgr_data->cd_inited == False) {
                ...
                file_mgr_data->tree_dash_thin_gc = XtGetGC(
                    widg, gc_mask | GCLineStyle | GCDashList, &gc_values);
                file_mgr_data->tree_dash_thick_gc = XtGetGC(
                    widg, gc_mask | GCLineWidth | GCLineStyle | GCDashList,
                    &gc_values);
        }
        ...
        if (showFilesystem && !TrashView) {
                ...
                SetToNormalColors(file_mgr_rec->current_directory_icon,
                                  file_mgr_rec->file_window, file_mgr_rec->main,
                                  0);

                /*  Update the Change Directory host name  */
                change_dir_data =
                    (ChangeDirData *)file_mgr_data->change_dir->data;
                ...
                pixmapData =
                    GetPixmapData(file_mgr_rec, file_mgr_data,
        ...
        SetIconAttributes((FileMgrRec *)file_mgr_data->file_mgr_rec,
                ...
                while (file_mgr_data->selection_table[i] != NULL) {
                        for (j = 0; j < directory_count; j++) {
                                if (strncmp(directory_set[j]->name,
                                            ...
                                            strlen(directory_set[j]->name)) ==
                                    0) {
                                        file_name =
                                            strrchr(file_mgr_data
                                                        ->selection_table[i],
                                                    '/') +
                                            1;
                                        order_list =
                                            directory_set[j]->order_list;

                                        for (k = 0;
                                             k < directory_set[j]->file_count;
                                             k++) {
                                                if (strcmp(file_name,
                                                           order_list[k]
                                                               ->file_data
        ...
        if (((DialogInstanceData *)(file_mgr_data->change_dir->data))
                ->displayed == True) {
        ...
        if (((DialogInstanceData *)(file_mgr_data->preferences->data))
                ->displayed == True) {
        ...
        if (((DialogInstanceData *)(file_mgr_data->find->data))->displayed ==
            True) {
                ShowFindDialog((Widget)NULL, (XtPointer)file_mgr_rec,
                               (XtPointer)NULL);
        } else
                file_mgr_rec->menuStates |= FIND;

        if (((DialogInstanceData *)(file_mgr_data->filter_edit->data))
                ->displayed == True) {
                ShowFilterDialog((Widget)NULL, (XtPointer)file_mgr_rec,
                                 (XtPointer)NULL);
        } else
        ...
        for (i = 0; i < file_mgr_data->attr_dialog_count; i++) {
                if (((DialogInstanceData *)(file_mgr_data->attr_dialog_list[i]
                                                ->data))
                        ->displayed == False) {
                        continue;
                }
                ...
                loadOk =
                    LoadFileAttributes(attr_data->host, attr_data->directory,
                ...
                if (loadOk) {
                         ...
                         */
                        ((ModAttrData *)(file_mgr_data->attr_dialog_list[i]
                                             ->data))
                        ...
                        _DtShowDialog(file_mgr_rec->shell, NULL,
                                      ...
                                      ModAttrChange, (XtPointer)file_mgr_rec,
                                      ModAttrClose, (XtPointer)file_mgr_rec,
                        ...
                        if (file_mgr_data->title != NULL &&
                            strcmp(file_mgr_data->helpVol, DTFILE_HELP_NAME) !=
                                ...
                                tmpStr = (GETMESSAGE(
                                    21, 34, "File Manager - Permissions"));
                        ...
                        /* The object would have got deleted and hence we are
                           not able to find it */
                        if (attr_data)
        ...
        for (i = 0; i < file_mgr_data->secondaryHelpDialogCount; i++) {
                ShowHelpDialog(file_mgr_rec->shell, (XtPointer)file_mgr_rec,
                               HYPER_HELP_DIALOG,
                               file_mgr_data->secondaryHelpDialogList[i], NULL,
                               NULL, NULL, NULL, 0);
        }
        ...
        if (file_mgr_data->primaryHelpDialog &&
            ((DialogInstanceData *)(file_mgr_data->primaryHelpDialog->data))
                ...
                ShowHelpDialog(file_mgr_rec->shell, (XtPointer)file_mgr_rec,
                               MAIN_HELP_DIALOG,
                               file_mgr_data->primaryHelpDialog, NULL, NULL,
                               NULL, NULL, 0);
        }
}
...
static void WriteResourceValues(DialogData *values, int fd, char **name_list) {
        ...
        if (file_mgr_data->current_directory && file_mgr_data->toolbox) {
                ...
                ptr =
                    strstr(file_mgr_data->current_directory, user_session_str);
                if (ptr) {
                        *ptr = '\0';
                        file_mgr_data->host =
                            XtNewString(file_mgr_data->current_directory);
...
static void GetSessionDir(FileMgrData *file_mgr_data) {
        ...
        } else /* session restore */
        {
                if ((file_mgr_data->current_directory) &&
                    (file_mgr_data->toolbox) && (file_mgr_data->host) &&
                    (strcmp(file_mgr_data->host,
                        ...
                        user_session_str = getenv("DTUSERSESSION");
                        /* the restricted directory for any object is a parent
                         * of that * object; the restricted directory for
                         * toolboxes is the root  * toolbox; the current toolbox
                         * dir is identified by the path  * information past the
                         * restricted dir/root toolbox            */
...
static void SelectionListToString(int fd, FileViewData ***value,
                                  char *out_buf) {
        int i;
        Boolean first = True;
        ...
        if (*value != NULL) {
                ...
                while (file_view_data != NULL) {
                        ...
                        (void)write(fd, directory_set->name,
                                    strlen(directory_set->name));

                        if (strcmp(directory_set->name, "/") != 0)
                                (void)write(fd, "/", strlen("/"));

                        (void)write(
                            fd, file_view_data->file_data->file_name,
...
void SetSpecialMsg(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
        ...
        if (msg) {
                ...
                file_mgr_data->msg_timer_id = XtAppAddTimeOut(
                    ...
                    (XtTimerCallbackProc)MsgTimerEvent,
...
void UpdateHeaders(FileMgrRec *file_mgr_rec, FileMgrData *file_mgr_data,
        ...
        if ((file_mgr_data->show_iconic_path == 0) !=
            ...
            (file_mgr_data->show_current_dir == 0) !=
                ...
                DPRINTF(("UpdateHeaders: iconic_path %d, current_dir %d, "
                         "status_line %d\n",
                ...
                XtUnmanageChildren(((XmManagerWidget)file_mgr_rec->header_frame)
                                       ->composite.children,
                                   ((XmManagerWidget)file_mgr_rec->header_frame)
                ...
                if (file_mgr_data->show_current_dir) {
                        ...
                        /*
                           ...
                           XmATTACH_FORM);      n++;
                        */
                        XtSetValues(file_mgr_rec->current_directory_frame, args,
                                    n);
                        manage[nmanage++] =
                            file_mgr_rec->current_directory_frame;

                        /*
                         * If the iconic path is shown, show only the current
                         * directory on the current directory line; if the
                         * iconic path is not shown, also show the drop target
                         * and the icon representing the current directory.
                         */
                        cur_dir_nmanage = 1;
                        ...
                        if (file_mgr_data->show_iconic_path) {
                                ...
                                XtSetArg(args[n], XmNleftOffset, 5);
                                n++;
                                XtSetValues(file_mgr_rec->current_directory,
                                            args, n);
                        } else {
                                if (showDropZone) {
                                        n = 0;
                                ...
                                XtSetValues(
                                    file_mgr_rec->current_directory_icon, args,
                                    n);
                                cur_dir_manage[cur_dir_nmanage++] =
                ...
                if (file_mgr_data->show_iconic_path ||
                    file_mgr_data->show_current_dir) {
                        if (file_mgr_data->show_current_dir)
                                XtManageChildren(cur_dir_manage,
                                                 cur_dir_nmanage);
        ...
        if (file_mgr_data->show_current_dir) {
                if (icons_changed) {
                        ...
                        if (pixmapData)
                                ...
                                XtSetArg(args[0], XmNimageName, NULL);
                        XtSetValues(file_mgr_rec->current_directory_icon, args,
                                    1);
                        _DtCheckAndFreePixmapData(
...
void ShowNewDirectory(FileMgrData *file_mgr_data, char *host_name,
        ...
        if (((strcmp(tmp_type, LT_AGROUP) == 0) ||
            ...
            (!(file_mgr_data->toolbox))) {
                ...
                pwd_dir = XtNewString(file_mgr_data->current_directory);
                DtActionInvoke(
                    ((FileMgrRec *)file_mgr_data->file_mgr_rec)->shell, "Trash",
        ...
        if (openDirType == NEW) {
                ...
                if (fileMgrData != NULL) {
                        dialog_data =
                            _DtGetInstanceData(fileMgrData->file_mgr_rec);
                        CloseView(dialog_data);
                }
                ForceMyIconClosed(file_mgr_data->host,
                                  file_mgr_data->current_directory);
        }
...
void FileMgrBuildDirectories(FileMgrData *file_mgr_data, char *host_name,
        ...
        if (file_mgr_data->current_directory != NULL) {
                DirectoryChanged((XtPointer)file_mgr_data, file_mgr_data->host,
                                 host_name, file_mgr_data->current_directory,
                                 directory_name);
                FreeDirectoryData(file_mgr_data);
        }
        ...
        if (PositioningEnabledInView(file_mgr_data)) {
                ...
                while (top) {
                        ...
                        if (file_view_data->displayed &&
                            DropOnGadget(file_mgr_data, file_view_data->widget,
                                         drop_x, drop_y)) {
                                *directory_data =
                ...
                for (; i < directory_count; i++) {
                        ...
                        for (j = 0; j < dir_data->file_count; j++) {
                                ...
                                if (DropOnGadget(file_mgr_data,
                                                 file_view_data->widget, drop_x,
                                                 drop_y)) {
        ...
        if (dropInfo->reason != DtCR_DND_DROP_ANIMATE) {
                ...
                */
                if (dropInfo->dropData->protocol == DtDND_BUFFER_TRANSFER) {
                        if (file_mgr_data == trashFileMgrData &&
                            file_mgr_data != NULL) {
                                ...
                                DPRINTF(("ProcessDropOnFileWindow: Rejecting "
                                         "buffer drop on Trash Can\n"));
                                return;
                        }
                }
                ...
                /* if placement is 'Grid' */
                        ...
                        */
                        if (initiating_view) {
                                if ((((FileMgrData *)initiating_view)
                                         ->current_directory ==
                                     file_mgr_data->current_directory) &&
                                    (dropInfo->dropData->protocol ==
                                     DtDND_FILENAME_TRANSFER)) {
                                        /* we actually want to allow a copy or a
                                         * link to the same directory but not a
                                         * move.  If it's a copy or link, we
                                         * want the initiating view to be NULL
                                         * so that later we don't error out when
                                         * it checks to see if they are from the
                                         * same view.  We will fail out if the
                                         * operation is a MOVE (causing the zoom
                                         * back).
                                         */
                                        fileType = GetDirectoryLogicalType(
                                            file_mgr_data,
                                            file_mgr_data->current_directory);

                                        command = TypeToAction(
                                            dropInfo->operation, fileType);
                                        if (command) {
                                                if (strcmp(command,
                                                           "FILESYSTEM_MOVE") ==
                                                    0)
                ...
                /* set the complete move flag to False since the animate
                   callback handles the deletion of the original file on the
                   move
                */

                DPRINTF(("DropOnFileWindow: Transfer Callback - Setting "
                         "Complete move flag "
                         "to False\n"));
                ...
                if (dropInfo->dropData->protocol == DtDND_BUFFER_TRANSFER)
                        ...
                        /* set the complete move flag to False since we will be
                         * handling */
                ...
                if (command) {
                        ...
                        for (i = 0; i < directory_set->file_count; ++i) {
                                if (strcmp(directory_set->order_list[i]
                                               ->file_data->file_name,
                                           ".") == 0) {
                                        RunCommand(command, file_mgr_data,
...
static void ProcessDropOnObject(Widget w, DtDndDropCallbackStruct *dropInfo,
        ...
        DirectorySet *directory_data =
            (DirectorySet *)file_view_data->directory_set;
        FileMgrData *file_mgr_data =
            (FileMgrData *)directory_data->file_mgr_data;
        char *command = NULL;
        ...
        if (dropInfo->reason != DtCR_DND_DROP_ANIMATE) {
                ...
                if (command &&
                    (strncmp("FILESYSTEM_", command, strlen("FILESYSTEM_")) !=
                     0) &&
        ...
        /******************************************/
                ...
                command = TypeToAction(dropInfo->operation,
                                       file_view_data->file_data->logical_type);
                if (command) {
                        RunCommand(command, file_mgr_data, file_view_data, NULL,
                                   dropInfo, NULL);
...
void FileMgrPropagateSettings(FileMgrData *src_data, FileMgrData *dst_data) {
        ...
        dst_data->show_type = dst_preferences_data->show_type =
            src_data->show_type;
        ...
        dst_data->direction = dst_preferences_data->direction =
            src_data->direction;
        ...
        dst_filter_active_data->filter =
            XtNewString(src_filter_active_data->filter);
        dst_filter_active_data->show_hidden =
            src_filter_active_data->show_hidden;
        ...
        dst_filter_edit_data->filter =
            XtNewString(src_filter_edit_data->filter);
...
void UpdateBranchList(FileMgrData *file_mgr_data, DirectorySet *directory_set) {
        ...
        if (directory_set && file_mgr_data->branch_list) {
                for (i = 0; file_mgr_data->branch_list[i]; i++)
                        if (strcmp(file_mgr_data->branch_list[i] + 2,
                                   directory_set->name) == 0) {
                                /* just update the TreeShow value for this
                                 * branch and return */
                                file_mgr_data->branch_list[i][0] =
                                    '0' + directory_set->sub_root->ts;
                                return;
                        }
        }
        ...
        for (i = 0; i < file_mgr_data->directory_count; i++) {
                ...
                branch_entry[0] =
                    '0' + file_mgr_data->directory_set[i]->sub_root->ts;
                branch_entry[1] = ':';
...
static void GetFileData(FileMgrData *file_mgr_data, Boolean valid,
        ...
        for (i = 0; i < directory_count; i++) {
                ...
                for (j = 0; j < new_dir_count; j++)
                        if (strcmp(directory_set[i]->name,
                                   new_dir_set[j]->name) == 0)
                                break;
                ...
                for (j = 0; j < directory_set[i]->file_count; j++) {
                        ...
                        for (k = 0; k < new_file_count; k++) {
                                if (new_view_data[k]->file_data ==
                                    file_view_data->file_data) {
                                        ...
                                        if (file_mgr_data
                                                ->drag_file_view_data &&
                                            file_mgr_data->drag_file_view_data
                                                    ->file_data ==
                                                file_view_data->file_data)
                                                new_drag_file_view_data =
                                                    new_view_data[k];

                                        if (file_view_data ==
                                            file_mgr_data->renaming)
                                                new_renaming = new_view_data[k];

                                        /* re-use the old widgets */
                                        new_view_data[k]->widget =
                                            file_view_data->widget;
                                        new_view_data[k]->treebtn =
                                            file_view_data->treebtn;
                                        new_view_data[k]->registered =
                                            file_view_data->registered;

                                        /* preserve ndir, nfile counts */
                                        new_view_data[k]->ndir =
                                            file_view_data->ndir;
                                        new_view_data[k]->nfile =
                                        ...
                                        if (position_info) {
                                                new_view_data[k]
                                                    ->position_info =
                                                    position_info;
                                                position_info->file_view_data =
                                                    new_view_data[k];
                                        }

                                        /* preserve icon_mtime */
                                        new_view_data[k]->icon_mtime =
                        ...
                        /* if no file by the same name was found in the new file
                           list, the file must have gone away ... lets eliminate
                           the position infomation */
                        if (position_info && k == new_file_count) {
                                for (k = 0; k < file_mgr_data->num_objects;
                                     k++) {
                                        if (file_mgr_data
                                                ->object_positions[k] ==
                                            position_info) {
                        ...
                        if (ts == tsAll &&
                            file_mgr_data->tree_files == TREE_FILES_NEVER)
                                *tsp = tsDirs;
                                 ...
                                 file_mgr_data->tree_files == TREE_FILES_ALWAYS)
                                *tsp = tsAll;
                        else
                                *tsp = ts;
...
static DirectorySet *NewDirectorySet(char *name, FileViewData *ip,
        ...
        for (dp = ip->desc; dp; dp = dp->next)
                ds->file_count++;
        if (ds->file_count != 0)
                ds->file_view_data = (FileViewData **)XtMalloc(
                    ds->file_count * sizeof(FileViewData *));
        else
 ...
 */
{
        ...
        if (ip->file_data->is_subdir && ip->ts != tsNotRead) {
                ...
                directory_set[*index] =
                    NewDirectorySet(fullname, ip, file_mgr_data);
                (*index)++;
        }
...
static void ReadTreeDirectory(Widget w, char *host_name, char *directory_name,
        ...
        if (file_mgr_data->busy_status != not_busy) {
                /* No point in continuing any further, free up all 'ip' and
                 * return */
        ...
        for (dp = ip->desc; dp; dp = dp->next)
                if (strcmp(dp->file_data->file_name, ".") == 0) {
                        ip->file_data->physical_type =
                            dp->file_data->physical_type;
                        ip->file_data->logical_type = GetDirectoryLogicalType(
                            file_mgr_data, directory_name);
...
static void SelectVisible(FileMgrData *file_mgr_data) {
        ...
        for (i = 0; i < file_mgr_data->directory_count; i++)
                for (j = 0; j < file_mgr_data->directory_set[i]->file_count;
                     j++)
        ...
        else if (op == BRANCH_EXPAND) /* show more */
        {
                if (file_mgr_data->tree_files == TREE_FILES_NEVER) {
                        ...
                        if (ip->ndir == 0 && ip->nfile == 0) {
                                ...
                                ip->ts = tsNone;
                        else if (ip->ts == tsNone && ip->ndir > 0 &&
                                 ip->nfile > 0 &&
                ...
                if (showEmptyMsg && ip->ndir == 0 &&
                    ...
                    ip->ts == tsDirs) {
                        ...
                        if (ip->nfile == 0)
                                msg = GETMESSAGE(12, 18,
                                                 "The folder %s is empty.");
                        else
                                msg = GETMESSAGE(12, 19,
        ...
        if (expand) {
                ...
                if (new_branch) {
                        ...
                        if (ip->ts == tsError) {
                                if (file_mgr_data->busy_status == not_busy) {
                                        file_mgr_data->busy_detail = 0;
                                        file_mgr_data->busy_status =
                                            initiating_readdir;
                                }
                                RereadDirectory(file_mgr_rec->shell,
                                                file_mgr_data->host, path);
                                if (file_mgr_data->busy_status ==
                                    initiating_readdir)
                                        file_mgr_data->busy_status = not_busy;
                        }
                        rc = ReadDir(file_mgr_rec->shell, file_mgr_data,
                                     file_mgr_data->host, path, ip, level,
                                     level, NULL);

                        /* create new directory set entry */
                        directory_set =
                            NewDirectorySet(path, ip, file_mgr_data);
                        ...
                        if (ip->ts == tsNotRead) {
                                file_mgr_data->directory_set =
                                    ((DirectorySet **)XtRealloc(
                                        (char *)(file_mgr_data->directory_set -
                                                 1),
                                ...
                                for (i = 0; i < file_mgr_data->directory_count;
                                     i++)
                                        if (strcmp(
                                                file_mgr_data->directory_set[i]
                                                    ->name,
                                ...
                                if (file_mgr_data->directory_set[i]
                                        ->file_view_data != NULL)
                                        XtFree((char *)file_mgr_data
                                                   ->directory_set[i]
                                                   ->file_view_data);
                        ...
                        if (file_mgr_data->busy_status != busy_readdir) {
                                ...
                                file_mgr_data->newSize = True;
                                AddFileIcons(file_mgr_rec, file_mgr_data,
                                             directory_set);
                        }
                }
                ...
                UpdateBranchState(file_mgr_data, ip, BRANCH_EXPAND,
                                  new_branch && file_mgr_data->busy_status ==
                                                    busy_readdir);
        } else {
                /* collaps the branch */
...
static void FreeDirectorySet(DirectorySet **directory_set,
        ...
        for (i = -1; i < directory_count; i++) {
                ...
                for (j = 0; j < directory_set[i]->file_count; j++) {
                        if ((char *)directory_set[i]
                                ->file_view_data[j]
                                ->label) {
                                XtFree((char *)directory_set[i]
                                           ->file_view_data[j]
                                           ->label);
                                directory_set[i]->file_view_data[j]->label =
                                    NULL;
                        }
...
static void SetIconAttributes(FileMgrRec *file_mgr_rec,
        ...
        if (fileLabel =
                DtDtsFileToAttributeValue(directory_name, DtDTS_DA_LABEL))
                ptr = fileLabel;
        ...
        if (file_mgr_data->title) {
                if (file_mgr_data->toolbox) {
                        if (strcmp(directory_name,
                                ...
                                new_directory_name = (char *)XtMalloc(
                                    strlen(file_mgr_data->title) + 1);
...
void FreePositionInfo(FileMgrData *file_mgr_data) {
        ...
        for (i = 0; i < file_mgr_data->directory_count; i++) {
                file_view_data =
                    file_mgr_data->directory_set[i]->file_view_data;
                for (j = 0; j < file_mgr_data->directory_set[i]->file_count;
                     j++)
...
void InheritPositionInfo(FileMgrData *src_file_mgr_data,
        ...
        for (i = 0; i < dest_file_mgr_data->num_objects; i++) {
                if (dest_file_mgr_data->object_positions[i]->stacking_order ==
                    1) {
                        dest_file_mgr_data->object_positions[i]->next =
                            temp_stack[dest_file_mgr_data->object_positions[i]
                                           ->stacking_order];

                } else if (dest_file_mgr_data->object_positions[i]
                               ->stacking_order ==
...

{
        ...
        if (file_mgr_data->num_objects <= 0)
                ...
                position_info = (PositionInfo *)XtMalloc(
                    file_mgr_data->num_objects * sizeof(PositionInfo));
                for (i = 0; i < file_mgr_data->num_objects; i++) {
...
void CheckMoveType(FileMgrData *file_mgr_data, FileViewData *file_view_data,
        ...
        if (_DtSpacesInFileNames(file_set, file_count)) {
                char *tmpStr =
                    (GETMESSAGE(27, 94,
                                "The File Manager does not support "
                                "objects\nwith spaces in their "
                                "names, so we cannot delete them.\nTo delete "
                                "these objects:\n  - "
                                "select 'Open Terminal' from the File Manager "
                                "menu\n  - then issue the "
        ...
        /* Fix for the case, when you drop an object from App. Manager on to
           desktop and try to move it to File Manager.  It must copied for this
           operation (modifers == ControlMask). */
        {
                Boolean IsToolBox;

                Str = (char *)IsAFileOnDesktop2(file_set, file_count, &number,
                                                &IsToolBox);
                if (IsToolBox)
        ...
        if (number > 0 && modifiers != ControlMask && modifiers != ShiftMask &&
            widget_dragged != NULL) {
                ...
                default: /* view == NOT_DESKTOP */
                        ...
                        dd = (DirectorySet *)NULL;
                        to = (char *)XtMalloc(
                            strlen(file_mgr_data->current_directory) + 1);
                ...
                for (i = 0; i < file_count; i++) {
                        if (strcmp(file_set[i], to) == 0) {
                                ...
                                if (file_view_data == NULL) {
                                        ...
                                        FileOperationError(file_mgr_rec->shell,
                                                           msg, file_set[i]);
                                } else {
                                        FileOperationError(
                                            file_view_data->widget, msg,
                                            file_set[i]);
                                }
                                XtFree(msg);
                                XtFree(Str);
                ...
                if (number == 1) {
                        ...
                        if (strcmp(from, to) == 0) {
                                RemoveIconInWorkspace(file_set[0],
                                                      workspace_name);
                                XtFree(title);
                                XtFree(to);
                                XtFree(Str);
                        ...
                        for (i = 0; i < desktop_data->numWorkspaces; ++i) {
                                if (strcmp(
                                        workspace_name,
                                        desktop_data->workspaceData[i]->name) ==
                                    0) {
                                        workspace_num =
                                            desktop_data->workspaceData[i]
                                                ->number;
                                        break;
                                }
                        }
                        ...
                        DeselectAllDTFiles(
                            desktop_data->workspaceData[workspace_num - 1]);

                        {
                                char *ptr;

                                if (file_mgr_data && file_mgr_data->toolbox) {
                                        to = _DtResolveAppManPath(
                                            to, file_mgr_data
                                                    ->restricted_directory);
                                        for (i = 0; i < number; ++i) {
                                                char *ptr, *ptr1;

                                                from = (char *)XtNewString(
                                                    file_set[i]);
                                                ptr = strrchr(from, '/');
                                                *ptr = 0x0;
                                                from = _DtResolveAppManPath(
                                                    from,
                                                    file_mgr_data
                                                        ->restricted_directory);
        ...
        default: /* view == NOT_DESKTOP */
                ...
                G_dropy = drop_y;
                /*
                           RepositionIcons(file_mgr_data, file_set, file_count,
                   drop_x, drop_y, True);
                */
                value = FileMoveCopy(file_mgr_data, NULL, directory,
                                     target_host, host_set, file_set,
        ...
        if (number > 0 && value && modifiers != ControlMask &&
            modifiers != ShiftMask) {
                ...
                for (j = 0; j < file_count; j++) {
                         ...
                         * being operated on has a reference to a desktop
                         * object. If it does change the reference in the
                         * desktop object structure. NOTE:  if we find a desktop
                         * object the file references, after we've changed the
                         * DT object we can't break because there might
                         * be more than one DT object which references
                         * it.
                         */
                        for (i = 0; i < desktop_data->numIconsUsed; i++) {
                                desktopWin = desktop_data->desktopWindows[i];
                                fileName = ResolveLocalPathName(
                                    desktopWin->host, desktopWin->dir_linked_to,
                                    desktopWin->file_name, home_host_name,
                                    &tt_status);
                                if (TT_OK == tt_status &&
                                    strcmp(fileName, file_set[j]) == 0) {
                                        /*
                                         * if fileName == file_set[j] then the
                                         * file is on the Desktop so change the
                                         * dir_linked to reference.  What the
                                         * new reference is depends on which
                                         * view type is being exectuted.
                                         */
                                        XtFree(desktopWin->host);
                                        desktopWin->host =
                                            XtNewString(target_host);

                                        XtFree(desktopWin->dir_linked_to);
                                        desktopWin->dir_linked_to =
                                            XtNewString(directory);
...
static void CreateFmPopup(Widget w) {
        ...
        label_string = XmStringCreateLocalized(
            (GETMESSAGE(20, 150, "Change Permissions...")));
        XtSetArg(args[0], XmNlabelString, label_string);
        ...
        XtAddCallback(popupBtns[i], XmNactivateCallback, ShowFindDialog,
                      (XtPointer)FM_POPUP);
        XtAddCallback(popupBtns[i++], XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_FILE_MENU_STR);

        XmStringFree(label_string);

        /* Create 'Clean Up' option -- white space popup */
        label_string =
            XmStringCreateLocalized((GETMESSAGE(20, 99, "Clean Up")));
        XtSetArg(args[0], XmNlabelString, label_string);
        ...
        XtAddCallback(popupBtns[i], XmNactivateCallback, CleanUp,
                      (XtPointer)FM_POPUP);
        XtAddCallback(popupBtns[i++], XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_CLEAN_UP_COMMAND_STR);

        XmStringFree(label_string);

        /* Create 'Select All' option -- white space popup */
        label_string =
            XmStringCreateLocalized((GETMESSAGE(20, 48, "Select All")));
        XtSetArg(args[0], XmNlabelString, label_string);
        ...
        label_string =
            XmStringCreateLocalized((GETMESSAGE(20, 52, "Deselect All")));
        XtSetArg(args[0], XmNlabelString, label_string);
        ...
        label_string = XmStringCreateLocalized(
            (GETMESSAGE(20, 156, "Show Hidden Objects")));
        XtSetArg(args[0], XmNlabelString, label_string);
        mnemonic = ((char *)GETMESSAGE(20, 102, "H"));
        XtSetArg(args[1], XmNmnemonic, mnemonic[0]);

        fileMgrPopup.wsPopup[BTN_SHOWHIDDEN] = popupBtns[i] =
            XmCreateToggleButtonGadget(fileMgrPopup.menu, "showHiddenObjects",
                                       args, 2);
        ...
        fileMgrPopup.objPopup[BTN_PUTON] = popupBtns[i] =
            XmCreatePushButtonGadget(fileMgrPopup.menu, "putInWorkspace", args,
                                     2);
        XtAddCallback(popupBtns[i++], XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_POPUP_MENU_STR);
        ...
        label_string =
            XmStringCreateLocalized((GETMESSAGE(20, 151, "Put in Trash")));
        XtSetArg(args[0], XmNlabelString, label_string);
        ...
        XtAddCallback(popupBtns[i++], XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_HELP_MENU_STR);

        XmStringFree(label_string);

        /* Create 'Restore' option -- trash popup */
        label_string =
            XmStringCreateLocalized((GETMESSAGE(27, 24, "Put back")));
        XtSetArg(args[0], XmNlabelString, label_string);
        ...
        XtAddCallback(popupBtns[i++], XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_TRASH_DIALOG_STR);
        XmStringFree(label_string);
...
static void DoTheMove(int type) {
        ...
        case NOT_DESKTOP:
                /*
                           RepositionIcons(fm, file_set, file_count, dx, dy,
                   True);
                */
                result = FileMoveCopy(fm, NULL, fm->current_directory, fm->host,
                ...
                if (type == DESKTOP) {
                        for (j = 0; j < file_count; j++) {
                                /* remove the Desktop object which corresponds
                                 * to this move */
        ...
        default: {
                ...
                result =
                    FileMoveCopyDesktop(fv, directory, host_set, file_set,
                                        file_count, mod, dtWindow, NULL, NULL);
        } break;
        }
