        ...
        */
        ...
        if (action_count + menu_offset > num_children) {
                ...
                for (i = num_children; i < action_count + menu_offset; i++) {
                        child = XmCreatePushButtonGadget(
                            (Widget)action_pane, "action_button", args, 0);
                        if (type == DESKTOP) {
        ...
        for (i = 0; i < action_count; i++) {
                ...
                if (type != DESKTOP) {
                        ...
                        if (action_label != NULL)
                                ...
                                string =
                                    XmStringCreateLocalized(command_list[i]);

                        count = i + menu_offset;
                } else {
                        if (physical_type == DtDIRECTORY && i == 0) {
                                if ((action_label =
                                         DtActionLabel(openNewView)) != NULL)
                                ...
                                if (physical_type == DtDIRECTORY) {
                                        if (strcmp(command_list[i - 1],
                                                   openNewView) == 0 ||
                                            strcmp(command_list[i - 1],
                                                   openInPlace) == 0) {
                                                del_count++;
                                                XtUnmanageChild(
                                                    action_pane->composite
                                                        .children[num_children -
                                                                  del_count]);
                                                continue;
                                        }
                                        action_label =
                                            DtActionLabel(command_list[i - 1]);
                                        ...
                                        XtSetArg(
                                            args[1], XmNuserData,
                                            XtNewString(command_list[i - 1]));
                                        count = i + number - del_count;
                                } else {
                                        action_label =
                                            DtActionLabel(command_list[i]);
                ...
                if (type != NOT_DESKTOP) {
                        XtRemoveAllCallbacks(
                            action_pane->composite.children[count],
                            XmNactivateCallback);
...
static void ActionCallback(Widget w, XtPointer client_data,
        ...
        if (client_data == NULL) {
                ...
                if (!file_view_data) /* The object would have probably been
                                        delete */
                        return;
                file_mgr_data->popup_menu_icon =
                    NULL; /* Just to make it unuseful */
        } else {
        ...
        if (strcmp(command, openNewView) == 0) {
                ...
                if (popup)
                        ...
                        RunCommand(command, file_mgr_data,
                                   file_mgr_data->selection_list[0], NULL, NULL,
                                   NULL);
                ...
                if ((modifiers != 0) && ((modifiers & ControlMask) != 0)) {
                        ...
                        dialog_data =
                            _DtGetInstanceData(file_mgr_data->file_mgr_rec);
...

{
        if ((strcmp(command, openInPlace) == 0) ||
            (strcmp(command, openNewView) == 0)) {
                ...
                if (strcmp(file_view_data->file_data->logical_type,
                           LT_FOLDER_LOCK) == 0) {
                ...
                if (((strcmp(file_view_data->file_data->logical_type,
                    ...
                    (strcmp(file_view_data->file_data->logical_type,
                            LT_TRASH) == 0)) {
                        ProcessAction(
                            command, file_view_data, drop_parameters,
                            file_mgr_data->host,
                            file_mgr_data->current_directory,
                            file_mgr_data->restricted_directory,
                            ((FileMgrRec *)file_mgr_data->file_mgr_rec)->shell);
                } else {
                        ProcessNewView(command, file_mgr_data, file_view_data,
                                       position);
                }
        }
                ...
                if (drop_parameters->dropData->protocol ==
                    DtDND_FILENAME_TRANSFER)
                        ProcessMoveCopyLink(command, file_mgr_data,
                                            file_view_data, drop_parameters,
                                            drop_window);
                else if (drop_parameters->dropData->protocol ==
                         DtDND_BUFFER_TRANSFER)
                        ProcessBufferDropOnFolder(command, file_mgr_data,
                                                  file_view_data,
...
void ProcessNewView(char *command, FileMgrData *file_mgr_data,
        ...
        if (trashFileMgrData != NULL && file_mgr_data == trashFileMgrData) {
                ...
                tmpStr = GETMESSAGE(
                    ...
                    "object use 'Put Back' to return it to the\nFile "
                    "Manager then open it there.");
        ...
        if (strcmp(command, openInPlace) == 0) {
                ...
                (void)XmScrollBarGetValues(vb, &value, &size, &increment,
                                           &page);

                /* set scroll bar values changing its position */
                if (value != 0)
                        (void)XmScrollBarSetValues(vb, (int)0, size, increment,
                                                   page, True);
                ...
                if (file_mgr_data->restricted_directory == NULL) {
                        GetNewView(host_name, directory_name, NULL, position,
                                   0);
                } else {
                        special_view = True;
...

{
        ...
        if (drop_window) {
                ...
                if (file_mgr_data == trashFileMgrData && !trashFile) {
                        DPRINTF(("DropOnFileWindow:Dragging File(s) to Trash "
                                 "Can from NonTrash "
                                 "Window\n"));

                        DropOnTrashCan(numFiles, host_set, file_set,
                                       drop_parameters);
                }
                ...
                /****************************************************************/
                        ...
                        DPRINTF(
                            ("DropOnFileWindow: Drag from Within Trash Can\n"));
                }
                ...
                /****************************************************************/
                        ...
                        DPRINTF(("DropOnFileWindow: Dragging from Trash to "
                                 "Folder Window\n"));
                ...
                /****************************************************************/
                        ...
                        /*
                           ...
                           SINGLE_DIRECTORY) && (file_mgr_data->view !=
                           BY_ATTRIBUTES))
                        */
                        {
                                if (file_mgr_data ==
                                    (FileMgrData *)initiating_view) {
                                        DPRINTF(
                                            ("DropOnFileWindow: Dragging and "
                                             "Dropping File within same "
                                             "window\n"));
                                        ...
                                        RepositionIcons(
                                            file_mgr_data, file_set, numFiles,
                                            drop_parameters->x,
                                            drop_parameters->y, False);
                                        LayoutFileIcons(file_mgr_rec,
                                                        file_mgr_data, False,
                                                        True);
                                        XmDropSiteEndUpdate(
                                            file_mgr_rec->file_window);
                                        ...
                                        DPRINTF(
                                            ("DropOnFileWindow: Dragging "
                                             "file(s) and dropping from other "
                                             "folders\n"));

                                        CheckMoveType(
                                            file_mgr_data, (FileViewData *)NULL,
                                            (DirectorySet *)NULL,
                                            (DesktopRec *)NULL, file_set,
                                            host_set, modifiers, numFiles,
                                            drop_parameters->x,
                        ...
                        /*
                                 ...
                                 {
                                    DPRINTF(("DropOnFileWindow: Not Single
                           Directory View\n"));

                                    if (FileMoveCopy (file_mgr_data,
                                                      NULL,
                           file_mgr_data->current_directory,
                           file_mgr_data->host, host_set, file_set, numFiles,
                           modifiers, NULL, NULL))
                                    {
                                       ...
                                       directory_data =
                           file_mgr_data->directory_set[0]; for (i = 0; i <
                           numFiles; i++)
                                       {
                                          directory_name = DName (file_set[i]);
                                          for (j = 0; j <
                           directory_data->file_count; j++)
                                          {
                                             file_view_data =
                           directory_data->file_view_data[j]; if (
                           (file_view_data->filtered != True) &&
                                                  (strcmp(directory_name,
                                                          file_view_data->file_data->file_name)
                           == 0) )
                                             {
                                                SelectFile (file_mgr_data,
                           file_view_data); break;
                                             }
                                          }
                                       }

                                       PositionFileView(file_view_data,
                           file_mgr_data);
                                    }
                                 }
                        */
        ...
        /*****************************/
                ...
                CheckMoveType(file_mgr_data, file_view_data,
                              ...
                              (DesktopRec *)NULL, file_set, host_set, modifiers,
...

{
        ...
        if (drop_window) {
...

                {
                        ...
                        DPRINTF(("ProcessDropOnBufferFolder: Dropping buffers "
                                 "on single "
                                 "directory view: %s\n",
                                 file_mgr_data->current_directory));
                ...
                DPRINTF(("ProcessBufferDropOnFolder...Buffers dropped on "
                         "Folder icon %s\n",
                         file_view_data->file_data->file_name));
                ...
                MakeFilesFromBuffers(file_mgr_data, directory,
                                     file_mgr_data->host, file_set, host_set,
                                     buffer_set, num_of_buffers, NULL, NULL);
        }
...
Boolean InvalidTrashDragDrop(int drag_op, int trash_context, Widget w) {
        ...
        if ((drag_op == XmDROP_COPY) || (drag_op == XmDROP_LINK)) {
                ...
                case WITHIN_TRASH:
                        tmpStr = (GETMESSAGE(
                            18, 36,
                            "You can't copy or link a file or folder out of "
                            "the "
                            "Trash Can.\nMove the object out of the Trash and "
                            "put it into the File Manager.\nYou can then copy "
                            "or link it from there."));
                        break;
                case FROM_TRASH:
                        tmpStr = (GETMESSAGE(
                            18, 37,
...

{
        ...
        if (trashFileMgrData != NULL && file_mgr_data == trashFileMgrData) {
                ...
                tmpStr = GETMESSAGE(
                    27, 105,
                    "Default action of a trash object will not be "
                    "executed.\n\nTo execute the default action of this "
                    "object\n use 'Put Back' to return it to the File "
        ...
        if (drop_parameters) {
                if (drop_parameters->dropData->protocol ==
                    DtDND_FILENAME_TRANSFER)
                        _DtBuildActionArgsWithDroppedFiles(
                            first_arg, drop_parameters, &action_args,
                            &arg_count);
                else
                ...
                if (first_arg)
                        _DtBuildActionArgsWithSelectedFiles(
                            &first_arg, 1, &action_args, &arg_count);
        }
        ...
        /* Invoke action */
        DtActionInvoke(w, action, action_args, arg_count, NULL, NULL, pwd_dir,
                       True, NULL, NULL);
...
void MoveCopyLinkHandler(Tt_message ttMsg, int opType) {
         ...
         * happens if a file has a space in it: We parse on the spaces and if
         * the next char after a space is a '/' then we assume that is the end
         * of the file name.  What this implies is that this won't work for
         * files with spaces at the end of the name
         */
        while (1) {
                /* build the arrary of char pointer's */
                if (file_count == file_set_size) {
                        file_set_size += 10;
                        file_set = (char **)XtRealloc(
                            (char *)file_set, sizeof(char **) * file_set_size);
                }
                ...
                if (ptr == NULL) {
                        ...
                        /* Let's check if the next char is a '/'. If it is then
                         * we know it's the next file name, else the space found
        ...
        /*
         * Let's check make sure the Folder the operation is happening to
         * exists.
         */
        ...
        for (i = 0; i < file_count; i++) {
                if (stat(file_set[i], &fileInfo) != 0 &&
                    lstat(file_set[i], &fileInfo) !=
                        ...
                        _DtMessage(toplevel, dialogTitle, title, NULL,
                                   HelpRequestCB);
        ...
        /* Go do the Move/Copy/Link... this function will do proper error
         * checking */
