...
void ModAttrChange(XtPointer client_data, DialogData *old_dialog_data,
                   DialogData *new_dialog_data, XtPointer call_data);
void ModAttrClose(XtPointer client_data, DialogData *old_dialog_data,
...
void ShowModAttrDialog(Widget w, XtPointer client_data, XtPointer callback) {
        ...
        if (file_view_data == NULL) {
                directory_set = (DirectorySet *)file_mgr_data->selection_list[0]
                                    ->directory_set;

                /* Determine which file is selected */
                loadOk = LoadFileAttributes(
                ...
                /* Determine which file is selected */
                loadOk =
                    LoadFileAttributes(file_mgr_data->host, directory_set->name,
        ...
        if (loadOk) {
                ...
                _DtMessage(file_mgr_rec->shell, title, errorMsg, NULL,
                           HelpRequestCB);
                XtFree(title);
                XtFree(errorMsg);
        }
...
static int AttrChangeProcess(int pipe_fd, char *host, char *directory,
                             char *file, int uid, int gid, long mode) {
        char *path;
        ...
        path = ResolveLocalPathName(host, directory, NULL, home_host_name,
                                    &tt_status);
        if (TT_OK == tt_status) {
                ...
                path = ResolveLocalPathName(host, directory, file,
...
static void AttrChangePipeCB(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        if (pipe_msg == PIPEMSG_DONE) {
                ...
                fprintf(stderr,
                        "Internal error in AttrChangePipeCB: bad pipe_msg %d\n",
                        pipe_msg);
                rc = 0;
                modify_time = 0;
        }
        ...
        DPRINTF(
            ("AttrChangePipeCB: n %d, pipe_msg %d, rc %d\n", n, pipe_msg, rc));

        /* if error, post error message */
        if (rc) {
                err_msg = XtNewString(
                    GETMESSAGE(22, 3, "Cannot change properties for %s"));
...
void ModAttrChange(XtPointer client_data, DialogData *old_dialog_data,
        ...
        if (strcmp(current->owner, old->owner) == 0 &&
            ...
            current->setuidBits == old->setuidBits) {
                ...
                ModAttrClose((XtPointer)file_mgr_rec, old_dialog_data,
                             new_dialog_data);
                return;
        }
        ...
        if (pid == 0) {
                ...
                rc = AttrChangeProcess(pipe_fd[1], cb_data->host,
                                       cb_data->directory, cb_data->file, uid,
                                       gid, mode);
                close(pipe_fd[1]);
        ...
        XtAppAddInput(XtWidgetToApplicationContext(toplevel), pipe_fd[0],
                      (XtPointer)XtInputReadMask, AttrChangePipeCB,
                      (XtPointer)cb_data);
...
void ModAttrClose(XtPointer client_data, DialogData *old_dialog_data,
        ...
        _DtHideOneSubdialog(old_dialog_data, &file_mgr_data->attr_dialog_list,
                            &file_mgr_data->attr_dialog_count);
