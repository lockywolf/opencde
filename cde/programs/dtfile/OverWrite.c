...
/* local functions */
static void replaceError_ok_callback(Widget w, XtPointer client_data,
                                     XtPointer call_data);
static void renameError_ok_callback(Widget w, XtPointer client_data,
                                    XtPointer call_data);
static void multiReplaceError_ok_callback(Widget w, XtPointer client_data,
                                          XtPointer call_data);
static void multiRenameError_ok_callback(Widget w, XtPointer client_data,
                                         XtPointer call_data);
static char *getVariableMessage(VariableMessage message, int mode);
static Boolean IsReplaceable(char *name);

/*--------------------------------------------------------------------
 * Callback routines
 *------------------------------------------------------------------*/

void replace_rename_ok_callback(Widget w, XtPointer client_data,
                                XtPointer call_data) {
         ...
         * folder as itself.  So the existing files stay the same and the new
         * files being created take on the new names.
         */
        ...
        if (XmToggleButtonGetState(G_rename_toggle)) {
                ...
                if ((lstat(renameNewName, &buf)) == 0) {
                        sprintf(msg,
                                ...
                                renameNewName);
                        title = XtNewString(
                            GETMESSAGE(9, 37, "Object Rename - Error"));
                        _DtMessageDialog(G_dialogShell, title, msg, 0, FALSE,
                                         NULL, renameError_ok_callback, NULL,
                                         NULL, False, ERROR_DIALOG);
                        XtFree(title);
                ...
                if (!same) {
                        /* attempt the rename ... since rename function is being
                         * used, this*/
                        /* only allows renames within the same filesystem */

                        DirectoryFileModified(home_host_name, G_directory,
                                              G_file);
                        if (DtStrchr(newFile, '/') == NULL)
                                DirectoryFileModified(home_host_name,
                                                      G_directory, newFile);
                        else {
                                ...
                                strncpy(newFileDir, renameNewName,
                                        newFileDirLen);
                                newFileDir[newFileDirLen] = '\0';

                                DirectoryBeginModify(home_host_name,
                                                     newFileDir);
                                DirectoryFileModified(home_host_name,
                        ...
                        if (rename_rc != 0) {
                                ...
                                sprintf(msg,
                                        ...
                                        destinationName, renameNewName,
                                        strerror(saveError));
                                title = XtNewString(
                                    GETMESSAGE(9, 37, "Object Rename - Error"));
                                _DtMessageDialog(G_dialogShell, title, msg, 0,
                                                 FALSE, NULL,
                                                 renameError_ok_callback, NULL,
                                                 NULL, False, ERROR_DIALOG);
                                XtFree(title);
                                XtFree(renameNewName);
                                XtFree(destinationName);
        ...
        /* Replace was selected ... remove the offending target */
                ...
                if (saveError == 0) {
                        ...
                        title = XtNewString(
                            GETMESSAGE(9, 40, "Object Replace - Error"));

                        if (S_ISDIR(buf.st_mode)) {
                                strcpy(msg, getVariableMessage(
                                                vm_replaceCannotDeleteFolder,
                                                G_mode));
                        } else {
                                sprintf(msg,
                                        getVariableMessage(
                                            vm_replaceCannotDeleteFile, G_mode),
                                        destinationName);
                        }

                        _DtMessageDialog(G_dialogShell, title, msg, 0, FALSE,
                                         NULL, replaceError_ok_callback, NULL,
                                         NULL, False, ERROR_DIALOG);
                        XtFree(title);
                        XtFree(destinationName);
                        return;
...
void buffer_replace_rename_ok_callback(Widget w, XtPointer client_data,
                                       XtPointer call_data) {
        int rc = 0;
        String newFile, renameNewName, destinationName;
        struct stat buf;
                ...
                if (saveError == 0) {
                        ...
                        if (S_ISDIR(buf.st_mode)) {
                                ...
                                sprintf(
                                    msg,
                                    getVariableMessage(
                                        vm_replaceCannotDeleteFile, COPY_FILE),
                                    destinationName);
                        }
...
void replace_rename_cancel_callback(Widget w, XtPointer client_data,
                                    XtPointer call_data) {
...
static void replace_rename_radio_callback(Widget w, XtPointer client_data,
                                          XtPointer call_data) {
        ...
        if (state->set)
                ...
                case RADIO_REPLACE:
                        XtVaSetValues(G_rename_text, XmNsensitive, FALSE, NULL);
                        break;

                case RADIO_RENAME:
                        XtVaSetValues(G_rename_text, XmNsensitive, TRUE, NULL);
...
void replace_merge_ok_callback(Widget w, XtPointer client_data,
                               XtPointer call_data) {
        int rc;
...
void replace_merge_cancel_callback(Widget w, XtPointer client_data,
                                   XtPointer call_data) {
        const int rc = PIPEMSG_CANCEL;
...
void multicollide_cancel_callback(Widget w, XtPointer client_data,
                                  XtPointer call_data) {
        const int rc = PIPEMSG_CANCEL;
        int i;
...
void multicollide_ok_callback(Widget w, XtPointer client_data,
                              XtPointer call_data) {
        ...
        if (XmToggleButtonGetState(G_multi_rename_toggle)) {
                ...
                for (i = 0; i < G_nCollisions; i++) {
                        if (!same) {
                                ...
                                destinationPath =
                                    build_path(G_directory, G_fileList[i]);
                                DirectoryFileModified(
                                    home_host_name, G_directory, G_fileList[i]);
                                if (auto_rename(destinationPath) != 0) {
                                        saveError = -1;
                                        if (!names)
                                                names = (char *)XtNewString(
                                                    destinationPath);
                                        else {
                                                names = (char *)XtRealloc(
                                                    names,
                                                    strlen(names) +
                                                        strlen(
                                                            destinationPath) +
                                        ...
                                        saveError = 0;
                                write(G_filop_confirm_fd, &saveError,
                                      sizeof(int));
                                XtFree(destinationPath);
                        }
                ...
                if (!same) {
                        if (names) {
                                ...
                                buf = XtMalloc(
                                    strlen(GETMESSAGE(
                                        9, 138,
                                        "Failed to rename the following "
                                        "object(s)\n\n%s\n\nThe most likely "
                                        "cause is "
                                ...
                                sprintf(
                                    ...
                                    GETMESSAGE(9, 138,
                                               ...
                                               "object(s)"),
                                    names);
                                _DtMessage(toplevel, title, buf, NULL,
                                           HelpRequestCB);
                                XtFree(buf);
                        }
                        XtFree(names);
                ...
                title =
                    XtNewString(GETMESSAGE(9, 40, "Object Replace - Error"));
                write(G_filop_confirm_fd, &rc, sizeof(int));
                        ...
                        if (IsReplaceable(destinationPath)) {
                                if (DirectoryBusy(destinationPath)) {
                                        ...
                                        msg = GETMESSAGE(
                                            ...
                                            "folder%s.\nAll File "
                                            "Manager views displayed for a "
                                            "folder or its "
                                            "sub-folders\nmust be closed "
                                            "before a folder can be "
                                            "moved or renamed.");
                                        ...
                                        sprintf(errmsg, msg, destinationPath);
                                        _DtMessage(toplevel, title, errmsg,
                                                   NULL, HelpRequestCB);
                                        XtFree(errmsg);
                                        XtFree(destinationPath);
                                        continue;
                ...
                if (names) {
                        ...
                        buf = XtMalloc(
                            strlen(GETMESSAGE(
                                9, 139,
                                "Failed to replace the following "
                                "object(s)\n\n%s\n\nThe most likely cause "
                                "is that you do not have\npermission to "
                                "replace these object(s)")) +
                            strlen(names) + 10);
                        sprintf(buf,
                                GETMESSAGE(9, 139,
                                           "Failed to replace the following "
...
void buffer_multicollide_ok_callback(Widget w, XtPointer client_data,
                                     XtPointer call_data) {
        int rc = 0;
        int i;
        int saveError;
        ...
        if (XmToggleButtonGetState(G_multi_rename_toggle)) {
                ...
                for (i = 0; i < G_nCollisions; i++) {
                        ...
                        DPRINTF((
                            "buffer_multicollide_ok_callback: new file is %s\n",
                            newFile));
                        ...
                        if (saveError != 0) {
                                ...
                                if (S_ISDIR(buf.st_mode) && i != 1) {
                                        sprintf(
                                            msg,
                                            getVariableMessage(
                                                vm_multiCannotDeleteFolderMany,
                                                COPY_FILE),
                                            destinationPath, i);
                                ...
                                } else /*  ! S_ISDIR(buf.st_mode) && i == 1 ) */
                                {
                                        sprintf(msg,
                                                getVariableMessage(
                                                    vm_multiCannotDeleteFileOne,
                                                    COPY_FILE),
                                                destinationPath);
                                }
                                _DtMessageDialog(
                                    G_dialogShell, title, msg, 0, FALSE, NULL,
                                    multiReplaceError_ok_callback, NULL, NULL,
                                    False, ERROR_DIALOG);
                                XtFree(destinationPath);
                                return;
                        }

                        /* send return code through the pipe to the background
                         * proc */
                        rc = PIPEMSG_REPLACE_BUFFER;
                        write(G_filop_confirm_fd, &rc, sizeof(int));
...
void create_replace_rename_dialog(Widget parent_widget, int mode,
                                  String directory, String file,
                                  int filop_confirm_fd, ActionAreaDefn actions,
                                  Boolean Same) {
        Arg args[10];
        int n;
        Widget form;
        ...
        case MOVE_FILE:
                title = XtNewString(
                    GETMESSAGE(9, 34, "File Manager - Move Warning"));
        ...
        case COPY_FILE:
                title = XtNewString(
                    GETMESSAGE(9, 35, "File Manager - Copy Warning"));
                XtSetArg(args[n], XmNtitle, title);
                n++;
                actions.actionList[1].label =
        ...
        case LINK_FILE:
                title = XtNewString(
                    GETMESSAGE(9, 36, "File Manager - Link Warning"));
                XtSetArg(args[n], XmNtitle, title);
                n++;
                actions.actionList[1].label =
        ...
        default:
                title = XtNewString(
                    GETMESSAGE(9, 33, "File Manager - Move/Copy/Link Warning"));
                XtSetArg(args[n], XmNtitle, title);
                n++;
                break;
        }
        ...
        G_dialogShell = XmCreateDialogShell(parent_widget,
                                            "replace/rename dialog", args, n);

        /* Create the Manager Widget, form, for the copy dialog */
        form = XtVaCreateWidget(
            "dialog_form", xmFormWidgetClass, G_dialogShell, XmNverticalSpacing,
            VERTICAL_SPACING, XmNhorizontalSpacing, HORIZONTAL_SPACING,
        ...
        px = XmGetPixmapByDepth(XtScreen(form), "xm_question", foreground,
                                background, form->core.depth);
        ...
        icon = XtVaCreateManagedWidget(
            "question_icon", xmLabelWidgetClass, form, XmNlabelType, XmPIXMAP,
            XmNlabelPixmap, px, XmNtopAttachment, XmATTACH_FORM,
        ...
        msg_label = XtVaCreateManagedWidget(
            "msg_header", xmLabelWidgetClass, form, XmNlabelString, xs,
            XmNalignment, XmALIGNMENT_BEGINNING, XmNtopAttachment,
            XmATTACH_FORM, XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
            icon, XmNleftOffset, HORIZONTAL_SPACING, XmNrightAttachment,
            XmATTACH_FORM, NULL);
        XmStringFree(xs);
        XtFree(s);
        ...
        xs = XmStringCreateLocalized(
            GETMESSAGE(9, 20, "What do you want to do?"));
        action_label = XtVaCreateManagedWidget(
            "action-question", xmLabelWidgetClass, form, XmNlabelString, xs,
            XmNalignment, XmALIGNMENT_BEGINNING, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, msg_label, XmNtopOffset,
            3 * VERTICAL_SPACING, XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET,
            XmNleftWidget, msg_label, XmNleftOffset, 0, NULL);
        XmStringFree(xs);
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, action_label);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, action_label);
        n++;
        XtSetArg(args[n], XmNleftOffset, 2 * HORIZONTAL_SPACING);
        n++;
        ...
        if (mode == MAKE_BUFFER)
                ...
                /* if the object is have the operation done to the same folder
                   (i.e. being done to itself, we want the default to be to
                   create a new name for the new object.
                   */
                if (Same)
                        set = FALSE;
                else
        ...
        xs = XmStringCreateLocalized(
            GETMESSAGE(9, 21, "Replace existing object"));
        radioButton1 = XtVaCreateManagedWidget(
            "radio_toggle1", xmToggleButtonWidgetClass, radio, XmNalignment,
            XmALIGNMENT_BEGINNING, XmNlabelString, xs, XmNset, set, NULL);
        XmStringFree(xs);
        if (!Same) {
                XtSetSensitive(radioButton1, True);
                XtAddCallback(radioButton1, XmNvalueChangedCallback,
                              replace_rename_radio_callback,
                              (XtPointer)RADIO_REPLACE);
        } else
                XtSetSensitive(radioButton1, False);

        if (mode == MAKE_BUFFER) {
                xs = XmStringCreateLocalized(
                    GETMESSAGE(10, 48, "New Object Name:"));
                set = TRUE;
        } else {
                if (Same) {
                        xs = XmStringCreateLocalized(
                            GETMESSAGE(9, 140, "Rename new object to:"));
                        set = TRUE;
                } else {
                        xs = XmStringCreateLocalized(
                            GETMESSAGE(9, 22, "Rename existing object to:"));
                        set = FALSE;
                }
        }
        G_rename_toggle = XtVaCreateManagedWidget(
            "radio_toggle2", xmToggleButtonWidgetClass, radio, XmNalignment,
            XmALIGNMENT_BEGINNING, XmNlabelString, xs, XmNset, set, NULL);
        XmStringFree(xs);
        XtAddCallback(G_rename_toggle, XmNvalueChangedCallback,
                      replace_rename_radio_callback, (XtPointer)RADIO_RENAME);
        ...
        G_rename_text = XtVaCreateManagedWidget(
            "rename_text", xmTextFieldWidgetClass, form, XmNrightAttachment,
            XmATTACH_FORM, XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
            radio, XmNleftOffset, HORIZONTAL_SPACING, XmNbottomAttachment,
            XmATTACH_OPPOSITE_WIDGET, XmNbottomWidget, radio, XmNsensitive, set,
            XmNvalue, newFile, XmNcursorPosition, strlen(newFile), NULL);
        ...
        separator = XtVaCreateManagedWidget(
            ...
            XmATTACH_WIDGET, XmNtopWidget, radio, XmNbottomAttachment,
            XmATTACH_WIDGET, XmNbottomWidget, actionArea, XmNleftAttachment,
            XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, NULL);

        /* set initial keyborad focus to the action button area */
        XtVaSetValues(form, XmNinitialFocus, actionArea, NULL);
...
void create_replace_merge_dialog(Widget parent_widget, int mode,
                                 String directory, String file,
                                 int filop_confirm_fd, ActionAreaDefn actions) {
        Arg args[10];
        int n;
        ...
        /* Create the dialog shell for the dialog */
        G_dialogShell =
            XmCreateDialogShell(parent_widget, "replace/merge dialog", args, n);
        ...
        /* create the radio buttons ... default button is replace */
        xs = XmStringCreateLocalized(
            GETMESSAGE(9, 25, "Replace existing folder"));
        radioButton1 = XtVaCreateManagedWidget(
            "radio_toggle1", xmToggleButtonWidgetClass, radio, XmNalignment,
            XmALIGNMENT_BEGINNING, XmNlabelString, xs, XmNset, TRUE, NULL);
        XmStringFree(xs);

        xs = XmStringCreateLocalized(
            GETMESSAGE(9, 24, "Merge contents of the two folders"));
        G_merge_toggle = XtVaCreateManagedWidget(
            "radio_toggle2", xmToggleButtonWidgetClass, radio, XmNalignment,
            XmALIGNMENT_BEGINNING, XmNlabelString, xs, NULL);
        XmStringFree(xs);
...
void create_multicollide_dialog(Widget parent_widget, int mode, int nSelected,
                                int nCollisions, String destination,
                                String *fileList, int filop_confirm_fd,
        ...
        case COPY_FILE:
                ...
                eMsgMany =
                    XtNewString(GETMESSAGE(9, 130, "%d objects copied."));
                break;
        case LINK_FILE:
                title = XtNewString(
                    GETMESSAGE(9, 36, "File Manager - Link Warning"));
                ...
                eMsgMany =
                    XtNewString(GETMESSAGE(9, 132, "%d objects linked."));
                break;
        default:
                title = XtNewString(
                    GETMESSAGE(9, 33, "File Manager - Move/Copy/Link Warning"));
                ...
                eMsgMany =
                    XtNewString(GETMESSAGE(9, 134, "%d objects changed."));
                break;
        }

        /* Create the dialog shell for the dialog */
        G_dialogShell =
            XmCreateDialogShell(parent_widget, "multicollide dialog", args, n);
        ...
        if (nCollisions > 1) {
                ...
                for (i = 0; i < nCollisions; i++) {
                        ...
                        filemsg = XtRealloc(
                            filemsg, strlen(filemsg) + strlen(destination) +
                                         strlen(fileList[i]) + 3);
                        strcat(filemsg, "\n");
                ...
                tmpmsg = GETMESSAGE(9, 70,
                                    "%s\n%d objects have the same name as "
                                    "objects in the folder\n%s");
                s = (char *)XtMalloc(tmp_len + strlen(tmpmsg) +
                                     strlen(filemsg) + 10);
                ...
                s = (char *)XtMalloc(
                    tmp_len + 10 +
                    strlen(GETMESSAGE(9, 71,
                                      "%s\n1 object has the same name as an "
        ...
        /* create the radio buttons ... default button is replace */
        xs = XmStringCreateLocalized(
            GETMESSAGE(9, 56, "Replace existing objects"));
        ...
        if (Same) {
                XtSetSensitive(radioButton1, False);
                set = TRUE;
                xs = XmStringCreateLocalized(GETMESSAGE(
                    9, 141, "Rename new objects by appending a number"));
        } else {
                XtSetSensitive(radioButton1, True);
                set = FALSE;
                xs = XmStringCreateLocalized(GETMESSAGE(
                    9, 57, "Rename existing objects by appending a number"));
        }

        G_multi_rename_toggle = XtVaCreateManagedWidget(
            "radio_toggle2", xmToggleButtonWidgetClass, radio, XmNalignment,
            XmALIGNMENT_BEGINNING, XmNset, set, XmNlabelString, xs, NULL);
        XmStringFree(xs);
...
Widget Create_Action_Area(Widget parent_widget, ActionAreaDefn actions,
        ...
        if (actions.defaultAction < 0 ||
            actions.defaultAction > actions.numActions - 1)
                actions.defaultAction = 0;

        fractbase_value = (TIGHTNESS * actions.numActions) - 1;
        action_area = XtVaCreateWidget(
            "action_area", xmFormWidgetClass, parent_widget, XmNfractionBase,
            fractbase_value, XmNleftAttachment, XmATTACH_FORM,
            XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment,
            XmATTACH_FORM, NULL);

        for (i = 0; i < actions.numActions; i++) {
                xm_string = XmStringCreateLocalized(
                    GETMESSAGE(actions.actionList[i].msg_set,
                               actions.actionList[i].msg_num,
                               actions.actionList[i].label));
                widget = XtVaCreateManagedWidget(
                    actions.actionList[i].label, xmPushButtonWidgetClass,
                    action_area, XmNleftAttachment,
                    (i ? XmATTACH_POSITION : XmATTACH_FORM), XmNleftPosition,
                    (TIGHTNESS * i), XmNtopAttachment, XmATTACH_FORM,
                    XmNbottomAttachment, XmATTACH_FORM, XmNrightAttachment,
                    ((i != (actions.numActions - 1)) ? XmATTACH_POSITION
                                                     : XmATTACH_FORM),
                    XmNrightPosition, ((TIGHTNESS * i) + (TIGHTNESS - 1)),
                    XmNshowAsDefault, (i == actions.defaultAction),
                    XmNdefaultButtonShadowThickness, 1, XmNlabelString,
                    xm_string, NULL);
                XmStringFree(xm_string);

                if (actions.actionList[i].callback) {
                        XtAddCallback(widget, XmNactivateCallback,
                                      actions.actionList[i].callback,
                                      actions.actionList[i].data);
                }

                if (i == actions.defaultAction) {
                        XtArgVal heightptr;
                        Dimension height, h;
                        XtVaGetValues(action_area, XmNmarginHeight, &heightptr,
                                      NULL);
                        ...
                        XtVaSetValues(action_area, XmNdefaultButton, widget,
                                      XmNpaneMaximum, height, XmNpaneMinimum,
                                      height, NULL);
                }
...
static char *getVariableMessage(VariableMessage message, int mode) {
        ...
        case vm_replaceCannotDeleteFolder:
                ...
                case MOVE_FILE:
                        return GETMESSAGE(
                            9, 95,
                            "Move Canceled - the folder was not moved.\n\n\
...
are moving so it no longer has the same name.");
                ...
                case COPY_FILE:
                        return GETMESSAGE(
                            9, 96,
                            "Copy Canceled - the folder was not copied.\n\n\
The folder you want to replace could not be deleted\n\
because an object inside that folder could not be deleted.\n\
Some other objects inside that folder may have already been\n\
...
are copying so it no longer has the same name.");
                ...
                case LINK_FILE:
                        return GETMESSAGE(
                            9, 97,
                            "Link Canceled - the folder was not linked.\n\n\
The folder you want to replace could not be deleted\n\
because an object inside that folder could not be deleted.\n\
Some other objects inside that folder may have already been\n\
...
are linking so it no longer has the same name.");
                ...
                default:
                        return GETMESSAGE(
                            9, 98,
                            "Operation Canceled - the folder was not changed.\n\n\
The folder you want to replace could not be deleted\n\
because an object inside that folder could not be deleted.\n\
Some other objects inside that folder may have already been\n\
...
file you are changing so it no longer has the same name.");
        ...
        case vm_multiCannotRenameMany:
                ...
                case MOVE_FILE:
                        return GETMESSAGE(
                            9, 103,
                            "Move Canceled - None of the objects were moved.\n\n\
...
rename the object you are moving so it no longer has the same name.");
                ...
                case COPY_FILE:
                        return GETMESSAGE(
                            9, 104,
                            "Copy Canceled - None of the objects were copied.\n\n\
 The following object could not be automatically renamed\n\
%s\n\n\
...
rename the object you are copying so it no longer has the same name.");
                ...
                case LINK_FILE:
                        return GETMESSAGE(
                            9, 105,
                            "Link Canceled - None of the objects were linked.\n\n\
 The following object could not be automatically renamed\n\
%s\n\n\
...
rename the object you are linking so it no longer has the same name.");
                ...
                default:
                        return GETMESSAGE(
                            9, 106,
                            "Operation Canceled - None of the objects were changed.\n\n\
 The following object could not be automatically renamed\n\
%s\n\n\
