...

{
        ...
        if (desktop_data->numIconsUsed > 0 && checkBrokenLink != 0) {
                checkBrokenLinkTimerId =
                    XtAppAddTimeOut(app_context, checkBrokenLink * 1000,
                                    TimerEventBrokenLinks, NULL);
        } else {
...
static Directory *FindDirectory(char *host_name, char *directory_name) {
        ...
        for (i = 0; i < directory_count; i++) {
                if (strcmp(host_name, directory_set[i]->host_name) == 0 &&
                    strcmp(directory_name, directory_set[i]->directory_name) ==
                        0) {
        ...
        for (i = 0; i < directory_count; i++) {
                if (directory_set[i]->tt_path_name != NULL &&
                    ...
                    strcmp(directory_name, directory_set[i]->tt_path_name) ==
                        0) {
                        return directory_set[i];
                }
        }
...
FileData *ReadFileData(char *full_directory_name, char *file_name) {
        ...
        if (stat_result == 0 && (stat_buf.st_mode & S_IFMT) == S_IFLNK) {
                while ((link_len = readlink(link_file_name, link_path,
                                            MAX_PATH)) > 0) {
                        ...
                        for (i = 0; i < link_count; i++) {
                                if (strcmp(link_file_name, link_list[i]) == 0) {
                                         ...
                                         */
                                        strcpy(link_file_name,
                                               link_list[link_count - 1]);
        ...
        if (stat_result == 0) {
                ...
                if ((stat_buf.st_mode & S_IFMT) == S_IFLNK) {
                        ...
                        if (recursive_link_found)
                                file_data->logical_type =
                                    XtNewString(LT_RECURSIVE_LINK);
                        else
                                file_data->logical_type =
                                    XtNewString(LT_BROKEN_LINK);
                } else {
                        file_data->is_broken = False;
                        if (file_data->link)
                                file_data->logical_type =
                                    (char *)DtDtsDataToDataType(
                                        file_data->link, NULL, 0, &stat_buf,
                                        file_data->final_link, NULL, NULL);
                        else
                                file_data->logical_type =
                                    (char *)DtDtsDataToDataType(
                                        full_file_name, NULL, 0, &stat_buf,
                                        NULL, NULL, NULL);
#if defined(DATATYPE_IS_FIXED)
#else
                        /* The problem here is there isn't a way for user to
                           mask only the OWNER READ bit in the MODE field of
                           dtfile.dt file. If the MODE field set to d&!r Then
                           all READ permission (S_IRUSR, S_IRGRP and S_IROTH)
                           bits has to be off in order for the above data typing
                           to work. Also data typing is unable to detect when
                           the directory is not the owners and only has execute
                           permission by that owner. The work around is manually
                           checking it ourselves. When the data typing code is
                           fixed, please remove this check.
                        */
                        if (S_ISDIR(stat_buf.st_mode) &&
                            (strcmp(file_data->logical_type, LT_DIRECTORY) ==
                             0)) {
                                if (strcmp(file_name, "..") != 0 &&
                                    strcmp(file_name, ".") != 0) {
                                        ...
                                        if (access(fullPathName, R_OK) != 0) {
                                                ...
                                                file_data->logical_type =
                                                    XtNewString(LT_FOLDER_LOCK);
                                        } else if (access(fullPathName, W_OK) !=
                                                   0) {
                                                XtFree(file_data->logical_type);
                                                file_data->logical_type =
                                                    XtNewString(
                                                        LT_NON_WRITABLE_FOLDER);
                                        }
                                }
                        }
#endif
                ...
                if (DtActionExists(file_data->logical_type)) {
                        file_data->action_name =
                            (char *)DtActionLabel(file_data->file_name);
                } else {
                        char *ptr = DtDtsDataTypeToAttributeValue(
                            file_data->logical_type, DtDTS_DA_LABEL, NULL);
                        if (ptr) {
...
int ReadFileData2(FileData2 *file_data2, char *full_directory_name,
        ...
        if (stat_result == 0) {
                ...
                if ((stat_buf.st_mode & S_IFMT) == S_IFLNK) {
                        ...
                        if (link_buf[0] == NILL)
                                ptr = (char *)DtDtsDataToDataType(
                                    full_file_name, NULL, 0, &stat_buf, NULL,
                                    NULL, NULL);
                        else
                                ptr = (char *)DtDtsDataToDataType(
                                    link_buf, NULL, 0, &stat_buf,
                            ...
                            (strcmp(ptr, LT_DIRECTORY) == 0)) {
                                if (strcmp(file_name, "..") != 0 &&
                                    strcmp(file_name, ".") != 0) {
                                        ...
                                        if (access(fullPathName, R_OK) != 0) {
                                                free(ptr); /* Don't use XtFree.
                                                              This pointer is
                                                              being kept by
                                                              tooltalk */
                                                strcpy(logical_type_buf,
                                                       LT_FOLDER_LOCK);
                                        } else if (access(fullPathName, W_OK) !=
                                                   0) {
                                                free(ptr); /* Don't use XtFree.
                                        ...
                                        strcpy(logical_type_buf, ptr);
                                        free(ptr); /* Don't use XtFree. This
                                                    * pointer is being kept by
                                                    * tooltalk
                                                    */
                                }
                        } else {
                                strcpy(logical_type_buf, ptr);
                                free(ptr); /* Don't use XtFree. This pointer is
                                              being kept by tooltalk */
                        }
#endif
                }
                ...
                if (DtActionExists(logical_type_buf)) {
                        ...
                        char *ptr = DtDtsDataTypeToAttributeValue(
                            logical_type_buf, DtDTS_DA_LABEL, NULL);
                        if (ptr) {
                                strcpy(action_name_buf, ptr);
                                DtDtsFreeAttributeValue(ptr);
                        }
...
static int ReadDirectoryProcess(int pipe_fd, Directory *directory,
        ...
        for (;;) {
                ...
                full_directory_name = ResolveLocalPathName(
                    host_name, namePtr, NULL, home_host_name, &tt_status);
                if (TT_OK != tt_status)
                        break;

                DtEliminateDots(full_directory_name);
                path_logical_types =
                    (char **)XtRealloc((char *)path_logical_types,
...
#else
                {
                        if (stat(full_directory_name, &stat_buf) == 0) {
                                if (S_ISDIR(stat_buf.st_mode) &&
                                    (strcmp(path_logical_types[path_count],
                                            LT_DIRECTORY) == 0)) {
                                        if (access(full_directory_name, R_OK) !=
                                            0) {
                                                XtFree(path_logical_types
                                                           [path_count]);
                                                path_logical_types[path_count] =
                                                    XtNewString(LT_FOLDER_LOCK);
                                        } else if (access(full_directory_name,
                                                          W_OK) != 0) {
                                                XtFree(path_logical_types
                                                           [path_count]);
                                                path_logical_types[path_count] =
...
#endif
                DPRINTF2((
                    "ReadDirectoryProcess: path '%s', fullname '%s', type %s\n",
                    namePtr, full_directory_name,
                    path_logical_types[path_count]));
        ...
        /* get the full name of the current directory */
        {
                Tt_status tt_status;
                full_directory_name =
                    ResolveLocalPathName(host_name, directory_name, NULL,
        ...
        if (CheckAccess(full_directory_name, R_OK | X_OK) != 0 ||
            stat(full_directory_name, &stat_buf) != 0) {
                ...
                modify_time = 0;
                DPRINTF(
                    ("ReadDirectoryProcess: sending errno %d (stat failed)\n",
                     rc));
                write(pipe_fd, &pipe_msg, sizeof(short));
        ...
        if (dirp == NULL) {
                ...
                rc = errno;
                DPRINTF((
                    "ReadDirectoryProcess: sending errno %d (opendir failed)\n",
                    rc));
                write(pipe_fd, &pipe_msg, sizeof(short));
                write(pipe_fd, &rc, sizeof(int));
                write(pipe_fd, &modify_time, sizeof(long));
...
#define PIPEMSG_HDR_LEN (2 * sizeof(short) + sizeof(int))
        ...
        do {
                ...
                if ((dp = readdir(dirp)) != NULL) {
                        ...
                        /* get the info */
                        if (directory->directoryView &&
                            directory->directoryView->file_mgr_data)
                                IsToolBox = directory->directoryView
                ...
                /* check if 0.4 seconds have passed since the last status line
                 * update */
        ...
        printf("    finished reading: %s, time: %ld.%ld\n\n",
               full_directory_name, update_time_f.tv_sec - update_time_s.tv_sec,
        ...
        if ((fptr = fopen(file_name, "r")) != NULL) {
                ...
                for (i = 0; i < position_count; i++) {
                        ...
                        write(pipe_fd, &(position_info[i].stacking_order),
                              sizeof(int));
                }
...
static int UpdateAllProcess(int pipe_fd, Directory *directory,
        ...
        /* if modified list contains "." or ".." arrange for the
           path_logical_types to get updated */
        ...
        if (rc) {
                ...
                for (;;) {
                        ...
                        /* get logical type of next path component */
                        full_directory_name =
                            ResolveLocalPathName(host_name, namePtr, NULL,
                                                 home_host_name, &tt_status);
                        if (TT_OK != tt_status)
                                break;
                        ...
                        path_logical_types = (char **)XtRealloc(
                            ...
                            (path_count + 1) * sizeof(char *));
                        path_logical_types[path_count] =
                            (char *)DtDtsDataToDataType(full_directory_name,
                                                        NULL, 0, NULL, NULL,
                                                        NULL, NULL);
#if defined(DATATYPE_IS_FIXED)
#else
                        ...
                        DPRINTF2(("ReadDirectoryProcess: path '%s', fullname "
                                  "'%s', type %s\n",
                                  namePtr, full_directory_name,
                                  path_logical_types[path_count]));
        ...
        if (TT_OK != tt_status) {
                ...
                modify_time = 0;
                DPRINTF(
                    ("UpdateAllProcess: sending errno %d (tooltalk failed)\n",
                     rc));
                write(pipe_fd, &pipe_msg, sizeof(short));
                write(pipe_fd, &rc, sizeof(int));
                write(pipe_fd, &modify_time, sizeof(long));
        ...
        if (CheckAccess(full_directory_name, R_OK | X_OK) != 0 ||
            stat(full_directory_name, &stat_buf) != 0) {
                ...
                modify_time = 0;
                DPRINTF(
                    ("UpdateAllProcess: sending errno %d (stat failed)\n", rc));
                write(pipe_fd, &pipe_msg, sizeof(short));
                write(pipe_fd, &rc, sizeof(int));
                write(pipe_fd, &modify_time, sizeof(long));
                XtFree(full_directory_name);
        ...
        if (dirp == NULL) {
                ...
                rc = errno;
                DPRINTF(
                    ("UpdateAllProcess: sending errno %d (opendir failed)\n",
                     rc));
                write(pipe_fd, &pipe_msg, sizeof(short));
                write(pipe_fd, &rc, sizeof(int));
                write(pipe_fd, &modify_time, sizeof(long));
        ...
        while (dp = readdir(dirp)) {
                ...
                for (old_pp = &directory->file_data;
                     (old_data = *old_pp) != NULL; old_pp = &old_data->next) {
                        if (strcmp(dp->d_name, old_data->file_name) == 0) {
                                ...
                                /* check if this file appears on the modified
                                 * list */
                                for (i = 0; i < directory->modified_count; i++)
                                        if (strcmp(
                                                dp->d_name,
                                            ...
                                            0) {
                                                /*
                                                 * This file is on the modified
                                                 * list. Pretend we didn't find
                                                 * it to force a refresh of this
                                                 * file.
                                                 */
                                                old_data = NULL;
                                                break;
                ...
                if (old_data != NULL) {
                        /*
                         * We remove the file from the old file list; thus, when
                         * we are done, the files on the old file list will be
                         * onew that no longer exist.
                         */
                ...
                /* this is a new file */
                DPRINTF(
                    ("UpdateAllProcess: found new file \"%s\"\n", dp->d_name));
        ...
        for (old_data = directory->file_data; old_data;
...
static int UpdateSomeProcess(int pipe_fd, Directory *directory,
        ...
        /* get the full name of the current directory */
        {
                ...
                if (TT_OK != tt_status) {
                        ...
                        rc = -1;
                        modify_time = 0;
                        DPRINTF(("UpdateSomeProcess: sending errno %d (stat "
                                 "failed)\n",
                                 rc));
                        write(pipe_fd, &pipe_msg, sizeof(short));
                        write(pipe_fd, &rc, sizeof(int));
        ...
        for (i = 0; i < directory->modified_count; i++) {
                ...
                if (directory->directoryView &&
                    directory->directoryView->file_mgr_data)
                        IsToolBox =
                            directory->directoryView->file_mgr_data->toolbox;
                else
                        IsToolBox = False;
...
static void ReaddirPipeCallback(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        case PIPEMSG_PATH_LOGICAL_TYPES:
                ...
                if (directory->path_count != n) {
                        ...
                        directory->path_logical_types = (char **)XtRealloc(
                            (char *)directory->path_logical_types,
                            n * sizeof(char *));
                }
                ...
                for (i = 0; i < directory->numOfViews; i++) {
                        file_mgr_data =
                            directory->directoryView[i].file_mgr_data;
                        file_mgr_rec =
        ...
        case PIPEMSG_FILEDATA3:
                ...
                if (msg == PIPEMSG_FILEDATA) {
                        ...
                        if (n < sizeof(FileData2)) {
                                ...
                                fprintf(
                                    stderr,
                                    "PipeReadFileData2: n = %d, expected %ld\n",
                ...
                if (msg == PIPEMSG_FILEDATA3) {
                        ...
                        for (i = 0; i < file_data_count; i++) {
                                ...
                                new_data = FileData2toFileData(
                                    (FileData2 *)file_data_buf_ptr, &n);
                                file_data_buf_ptr += n;
        ...
        case PIPEMSG_POSITION_INFO:
                ...
                if (directory->position_count != n) {
                        ...
                        directory->position_info = (PositionInfo *)XtRealloc(
                            (char *)directory->position_info,
                            n * sizeof(PositionInfo));
                }
        ...
        default:
                if (whined_fd != *fd) {
                        whined_fd = *fd;
                        fprintf(stderr,
                                "ReaddirPipeCallback: badmsg=%d, ppid=%d "
                                "pid=%d fd=%d activ'y=%d\n",
        ...
        if (done) {
                     ...
                     new_nextp = &new_data->next) {
                        for (old_nextp = &directory->file_data;
                             ...
                             old_nextp = &old_data->next) {
                                if (strcmp(old_data->file_name,
                                           new_data->file_name) == 0) {
                                        *old_nextp = old_data->next;
                ...
                /*
                 * If this was a complete re-read, we free all FileData still
                 * left in the old list.  Otherwise, if this was just a partial
                 * update, we append the old data that's still left to the end
                 * of the new list.
                 */
                if (activity == activity_reading) {
                        /* This was a complete re-read: free all old data still
                         * left. */
                        ...
                        */
                        ...
                        /* Append the old list to the end of the new list
                           Replace the old list pointer with the new list
                           pointer
                        */
                        if (tmp_ptr != NULL) {
                ...
                for (new_data = directory->file_data; new_data;
                 ...
                 * This data will be read when querying this view's top
                 * directory, if the parent directory isn't already cached (tree
                 * mode)
                 */
                for (new_data = directory->file_data; new_data != NULL;
                     new_data = new_data->next)
                        if (strcmp(new_data->file_name, ".") == 0) {
                                ...
                                /*
                                 * Found current directory information, now we
                                 * make dir_data info from "." info
                                 */

                                /* If we already have allocated space for
                                 * dir_data free it */
                                ...
                                memcpy(directory->dir_data, new_data,
                                       sizeof(FileData));

                                /*
                                 * Doctor up some of the information fields so
                                 * that this doesn't seem to be a "." entry
                                 */
                                directory->dir_data->next = NULL;
                                directory->dir_data->file_name = XtNewString(
                                    DName(directory->directory_name));
                                directory->dir_data->action_name = NULL;
                                if (directory->path_count > 0) {
                                        directory->dir_data
                                            ->logical_type = XtNewString(
                                            directory->path_logical_types
                                                [directory->path_count - 1]);
                                } else
                                        directory->dir_data->logical_type =
                                            NULL;
                ...
                for (i = 0; i < directory->numOfViews; i++) {
                        ...
                        file_mgr_rec =
                            (FileMgrRec *)file_mgr_data->file_mgr_rec;
                        FileMgrRedisplayFiles(file_mgr_rec, file_mgr_data,
                                              False);
...
static Boolean ReadDirectory(Widget w, char *host_name, char *directory_name,
        ...
        if ((directory != NULL) &&
            (strcmp(directory_name, directory->directory_name) == 0)) {
                ...
                for (i = 0; i < directory->numOfViews; i++)
                        if (directory->directoryView[i].file_mgr_data ==
                            file_mgr_data)
                                break;
                ...
                if (directory->errmsg_needed &&
                    !directory->busy[activity_reading] && w != NULL) {
                        err_msg = XtNewString(
                            GetSharedMessage(CANNOT_READ_DIRECTORY_ERROR));
                ...
                directory->directoryView =
                    (DirectoryView *)XtMalloc(sizeof(DirectoryView));
 ...
 */
{
        ...
        busy_reading = ReadDirectory(w, host_name, directory_name, &file_data,
                                     &n, file_mgr_data);
        if (busy_reading) {
                file_mgr_data->busy_status = busy_readdir;
                return 0;
        ...
        for (i = 0, fp = file_data; i < n && fp; i++, fp = fp->next) {
                ...
                if (fp->is_subdir) {
                        ...
                        if (!QueryBranchList(file_mgr_data, branch_list,
                                             subdir_name, &ts))
                                /* not known: assume we shouldn't read this
                                 * subdir */
                                ts = tsNotRead;

                        if (level < read_level || ts != tsNotRead) {

                                rc = _ReadDir(w, file_mgr_data, host_name,
                                              subdir_name, ip, level,
                                ...
                                if (ip == NULL)
                                        ...
                                        ip->ts = ts;
                                         ...
                                         file_mgr_data->tree_show_level)
                                        ip->ts = tsNone;
                                else if (file_mgr_data->tree_files ==
                                         TREE_FILES_ALWAYS)
                                        ip->ts = tsAll;
                                else
                                        ip->ts = tsDirs;
...

{
        ...
        for (i = 0; i < directory_count; i++) {
                for (j = 0; j < directory_set[i]->numOfViews; j++) {
                        if (file_mgr_data ==
                            directory_set[i]->directoryView[j].file_mgr_data) {
                                directory_set[i]->directoryView[j].mapped =
                                    file_mgr_data->mapped;
                                break;
                        }
                }
        }
...
void DirectoryEndModify(char *host_name, char *directory_name) {
        ...
        if (directory != NULL) {
                directory->modify_begin--;
                DPRINTF(
                    ("   modify_begin %d, up_to_date %d, modified_count %d\n",
                     directory->modify_begin, directory->was_up_to_date,
                ...
                if (directory->modified_count > 0) {
                        ...
                        /*
                         * If any of the modifed files is a subdirectory that we
                         * have cached, schedule an activity_checking_dir to
                         * make sure that the subdirectory is still readable.
                         */
                        strcpy(subdir_name, directory_name);
                        ...
                        for (i = 0; i < directory->modified_count; i++) {
                                ...
                                if (subdir) {
                                        DPRINTF(("   schedule check for subdir "
                                                 "\"%s\"\n",
                                                 directory->modified_list[i]));
                                        subdir->busy[activity_checking_dir] =
                                            True;
...
void UpdateCachedDirectories(View **view_set, int view_count) {
         ...
         */
        for (i = 0; i < directory_count; i++) {
                if (!(strcmp(directory_set[i]->directory_name, trash_dir) ==
                      0)) {
        ...
        for (j = 0; j < view_count; j++) {
                ...
                for (k = 0; k < file_mgr_data->directory_count; k++) {
                        ...
                        /* we expect the directory to be found; if not,
                         * something is wrong */
                        if (directory == NULL) {
                                fprintf(stderr,
                                        "Warning: %s:%s not found in directory "
                                        "set.\n",
                                        view_set[j]->host_name,
                                        file_mgr_data->directory_set[k]->name);
                                continue;
                        ...
                        directory->directoryView[n].file_mgr_data =
                            file_mgr_data;
                        directory->directoryView[n].mapped =
                            file_mgr_data->mapped;
                        directory->numOfViews++;
                        directory->viewed = True;
                }
        ...
        while (i < directory_count) {
                if (directory_set[i]->numOfViews > 0 ||
                    strcmp(directory_set[i]->directory_name, trash_dir) == 0) {
                        ...
                        /* Delete the file data and remove from the directory
                         * set. */
...
char *GetLongName(FileData *file_data) {
        ...
        if (file_data->errnum == 0) {
                ...
                strftime(time_string, 100,
                         GetSharedMessage(DIRECTORY_DATE_FORMAT), tms);
#else
                ...
                strcpy(error_msg, "(");
                strncpy(error_msg + 1, strerror(file_data->errnum),
                        msg_len - 2);
...
#ifdef NLS16
        tms = localtime(&long_modify_time);
        strftime(time_string, 100, GetSharedMessage(DIRECTORY_DATE_FORMAT),
                 tms);
#else
        time_string = ctime((time_t *)&long_modify_time);
        time_string[strlen(time_string) - 1] = 0x0;
                ...
                if (!is_multibyte) {
                        ...
                        if (len > NAME_PRECISION) {
                                ...
                                sprintf(long_name,
                                        ...
                                        permission, usr_read_priv,
                                        usr_write_priv, usr_exec_priv,
                                        grp_read_priv, grp_write_priv,
                                        grp_exec_priv, oth_read_priv,
                        ...
                        int copy_len =
                            len > NAME_PRECISION
                                ? NAME_PRECISION - sizeof(ELLIPSIS) + 1
                                : len;
                        int byte_len = 0;
                        int count;

                        long_name[0] = '\0';
                        /* properly copy copy_len characters of the multibyte
                           string replacing invalid chars with '?' */
                        for (count = 0; (count < copy_len) &&
                                        *(file_data->file_name + byte_len);
                             count++) {
                                int chr_bytes =
                                    mblen(file_data->file_name + byte_len,
                                          MB_CUR_MAX);
                                if (chr_bytes > 0) {
                                        strncpy(long_name + byte_len,
                                                file_data->file_name + byte_len,
                                                chr_bytes);
                                        ...
                                        /* null-wide character, won't really
                                         * happen */
                                        break;
                                }
                                byte_len += chr_bytes;
                        ...
                        if (copy_len < len) {
                                ...
                                strncpy(long_name + byte_len, ELLIPSIS,
                                        sizeof(ELLIPSIS) - 1);
                                byte_len += sizeof(ELLIPSIS) - 1;
                        } else {
                                /* full name, pad it with spaces up to the
                                 * proper length */
                        ...
                        sprintf(
                            ...
                            usr_exec_priv, grp_read_priv, grp_write_priv,
                            grp_exec_priv, oth_read_priv, oth_write_priv,
...
Boolean DirectoryBusy(char *path) {
        ...
        for (i = 0; i < directory_count; i++) {
                /* check if this directory is equal to 'path' or a subdir of
                 * 'path' */
                    ...
                    (strcmp(directory_set[i]->path_name, path) == 0 ||
                     ...
                     directory_set[i]->tt_path_name != NULL &&
                         (strcmp(directory_set[i]->tt_path_name, path) == 0 ||
                          strncmp(directory_set[i]->tt_path_name, path, len) ==
                                  0 &&
                        ...
                        for (j = 0; j < directory_set[i]->numOfViews; j++) {
                                ...
                                /* find the dir in the directory set for this
                                 * view */
                                for (k = 0; k < file_mgr_data->directory_count;
                                     k++)
                                        if (strcmp(
                                                file_mgr_data->directory_set[k]
                                                    ->name,
                                                directory_set[i]
                                                    ->directory_name) == 0) {
                                                break;
                                ...
                                if (k == file_mgr_data->directory_count)
                                        continue; /* not found ... something
                                                     must be wrong! */

                                /*
                                 * Check if this directory is acutally visible.
                                 * If the directory is in a tree branch that is
                                 * not currently expanded, it is not visible and
                                 * would not be considered busy.
                                 */
                                ...
                                /* a subdir is considered busy if it is visible
                                 * and at least partially expanded */
                                sub_root =
                                    file_mgr_data->directory_set[k]->sub_root;
                                if (sub_root->displayed &&
                                    (sub_root->ts == tsDirs &&
                                         sub_root->ndir > 0 ||
                                     sub_root->ts == tsAll &&
                                         sub_root->ndir + sub_root->nfile >
                                             0)) {
                                        return True;
                                }
                        }
...
char *GetDirectoryLogicalType(FileMgrData *file_mgr_data, char *path) {
        ...
        if ((directory != NULL) && (strcmp(file_mgr_data->current_directory,
                ...
                DPRINTF2(("GetDirectoryLogicalType(%s): n %d, type %s\n", path,
                          n, directory->path_logical_types[n]));
...
static int WritePosInfoProcess(int pipe_fd, Directory *directory,
        ...
        fileName = ResolveLocalPathName(
            directory->host_name, directory->directory_name, positionFileName,
            home_host_name, &tt_status);
        ...
        if (position_count <= 0)
                ...
                if (f == NULL) {
                        ...
                        for (i = 0; i < position_count; i++) {
                                fprintf(f, "%s\n%d %d %d\n",
                                        position_info[i].name,
                                        position_info[i].x, position_info[i].y,
                                        position_info[i].stacking_order);
                        }
...
int SetDirectoryPositionInfo(char *host_name, char *directory_name,
                ...
                for (i = 0; i < position_count && unchanged; i++) {
                        for (j = 0; j < position_count; j++)
                                if (strcmp(position_info[i].name,
                                           directory->position_info[j].name) ==
                                    0) {
                                        break;
                                }
                        ...
                        if (j == position_count ||
                            position_info[i].x !=
                                directory->position_info[j].x ||
                            position_info[i].y !=
                                directory->position_info[j].y ||
                            position_info[i].stacking_order !=
        ...
        for (i = 0; i < position_count; i++) {
                directory->position_info[i].name =
                    XtNewString(position_info[i].name);
...
static int TimerEventProcess(int pipe_fd, Directory *directory,
        ...
        if (rc == 0 && activity == activity_checking_links) {
                ...
                for (file_data = directory->file_data;
                     file_data && !link_changed; file_data = file_data->next) {
                        ...
                        /* Check what kind of link this was the last time we
                         * looked: a normal link (1), a recursive link (2), or
                         * an otherwise broken link (3) */
                        if (strcmp(file_data->logical_type, LT_BROKEN_LINK) ==
                            0)
                                prev_link_kind = 3;
                        else if (strcmp(file_data->logical_type,
                                        LT_RECURSIVE_LINK) == 0)
                                prev_link_kind = 2;
                        else
                                prev_link_kind = 1;
...
static void TimerPipeCallback(XtPointer client_data, int *fd, XtInputId *id) {
        ...
        DPRINTF2(("TimerPipeCallback: rc %d (was %d), time %ld (was %ld), link "
                  "change %d\n",
                  rc, directory->errnum, modify_time,
                  (long)directory->modify_time, link_changed));
...
static int CheckDesktopProcess(int pipe_fd, Directory *directory,
        ...
        for (i = 0; i < desktop_data->numIconsUsed; i++) {
                ...
                if (lstat(full_path, &stat_buf) < 0) {
                        ...
                        DPRINTF2(("CheckDesktopProcess: sending "
                                  "PIPEMSG_DESKTOP_REMOVED for %s\n",
                                  full_path));
                        ...
                        if (new_data->physical_type !=
                                old_data->physical_type ||
                            strcmp(new_data->logical_type,
                                   old_data->logical_type) != 0) {
                                /* the type has changed */
                                DPRINTF2(("CheckDesktopProcess: sending "
                                          "PIPEMSG_DESKTOP_CHANGED for %s\n",
                                          full_path));
                                DPRINTF2(("  old type %d %s, new type %d %s\n",
                                          old_data->physical_type,
                                          old_data->logical_type,
                                          new_data->physical_type,
                                          new_data->logical_type));

                                pipe_msg = PIPEMSG_DESKTOP_CHANGED;
                                write(pipe_fd, &pipe_msg, sizeof(short));
                                PipeWriteString(pipe_fd, desktopWindow->host);
                                PipeWriteString(pipe_fd,
...
static void CheckDesktopPipeCallback(XtPointer client_data, int *fd,
        ...
        if (msg == PIPEMSG_DESKTOP_REMOVED || msg == PIPEMSG_DESKTOP_CHANGED) {
                ...
                DPRINTF2(("CheckDesktopPipeCallback: msg %d: host %s, dir %s, "
                          "name %s\n",
...
static int CheckListCmp(int *p1, int *p2) {
...
static void TimerEvent(XtPointer client_data, XtIntervalId *id) {
        ...
        if (dragActive) {
                 ...
                 *
                 * Reason: drag callbacks are called with a pointer to a
                 * FileViewData structure; if a directory is reread while a drag
                 * is active, the pointer would become invalid, causing
                 * unpredictable behavior.
                 *
                 * Schedule the next TimerEvent in 1/2 second, so that check
                 * will be done soon after the drag is finished.
                 */
        ...
        if (checkBrokenLink > 0 &&
            tick_count >= lastLinkCheckTick + ticksBetweenLinkChecks) {
                ...
                for (i = 0; i < directory_count; i++) {
                        ...
                        /* if a check is already in progress, don't start
                         * another one */
        ...
        if (directory_count > check_alloc) {
                ...
                check_list = (int *)XtRealloc((char *)check_list,
                                              check_alloc * sizeof(int));
        }
...
static void ScheduleDirectoryActivity(Directory *directory) {
        ...
        StickyProcDesc *p;
        int pipe_s2m_fd
            [2]; /* for msgs from backgroundnd proc (slave to master) */
        ...
        DPRINTF2(
            ("ScheduleActivity: activity %d, busy %c%c%c%c%c%c%c, dir %s\n",
             directory->activity,
        ...
        if (directory->numOfViews > 0) {
                ...
                for (i = 0; i < directory->numOfViews; i++) {
                        ...
                        /* see if the same view appears in the view list of a
                         * non-idle dir */
                        this_view_active = False;
                        for (j = 0; j < directory_count && !this_view_active;
                             j++) {
                                /* we are only interested in directories that
                                 * are not idle */
                                if (directory_set[j]->activity == activity_idle)
                                        continue;

                                /* see if the view appears in the view list */
                                for (k = 0; k < directory_set[j]->numOfViews;
                                     k++) {
                                        if (directory_set[j]
                                                ->directoryView[k]
                                                .file_mgr_data ==
         ...
         * time.  Hence, instead of exiting when it's done, the background
         * process is "sticky", i.e., it will stay around waiting for a message
         * on stdin, so it can be re-used the next time around.  A linked list
         * of sticky procs that are currently active is maintained in the
         * ActivityTable.
         */
        ...
        if (p) {
                ...
                if (PipeWriteString(p->pipe_m2s_fd, directory->path_name) < 0) {
                        ...
                        /* the pipe is broken, remove the old proc then start a
                         * new one */
                        for (d = ActivityTable[activity].sticky_procs; d && p;
                             d = d->next) {
                                if (d == p) {
                                        /* the proc listed 1st is dead, remove
                                         * it */
                                        ActivityTable[activity].sticky_procs =
                                            p->next;
                                        XtFree((void *)p);
                                        p = NULL;
                                } else if (d->next == p) {
        ...
        if (!p) {
                ...
                if (pid == -1) {
                        ...
                        fprintf(stderr,
                                "%s:  fork failed, ppid %d, pid %d, activity "
                                "%d: error %d=%s\n",
                                pname, getppid(), getpid(), activity, errno,
                                strerror(errno));
                        ...
                        close(pipe_s2m_fd[0]); /* child won't read from this
                                                  pipe */
                        close(pipe_s2m_fd[1]); /* parent won't write to this
                                                  pipe */
                        if (sticky) {
                                close(pipe_m2s_fd[1]); /* child won't write to
                                                          this pipe */
                                close(pipe_m2s_fd[0]); /* parent won't read from
                                                          this pipe */
                                p->pipe_s2m_fd = 0;
                                p->pipe_m2s_fd = 0;
                                p->idle = True;
                ...
                if (pid == 0) {
                         ...
                         */
                        for (;;) {
                                rc = (*ActivityTable[activity].main)(
                                    pipe_s2m_fd[1], directory, activity);
                                if (!sticky || rc != 0)
                                        break;
                                ...
                                DPRINTF2(
                                    ("StickyActivity:  activity %d, dir %s\n",
                                     activity, s));
                        }
                ...
                DBGFORK(
                    ("%s:  forked child<%d> for activity %d, s2m %d, m2s %d\n",
                ...
                if (sticky)
                        close(pipe_m2s_fd[0]); /* parent won't read from this
                                                  pipe */
        }
