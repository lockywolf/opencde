...
static void Create(Display *display, Widget parent, Widget *return_widget,
        ...
        Widget filterText, filterLabel, listLabel, scrolledList, dirName,
            dirLabel;
#if defined(__hpux) || defined(sun)
        ...
        label_string = XmStringCreateLocalized((GETMESSAGE(
            15, 47,
            "Fill in one or more fields to specify which items to find:")));
        n = 0;
        ...
        label_string = XmStringCreateLocalized(
            (GETMESSAGE(15, 31, "File or Folder Name: ")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, label_string);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        XmStringFree(label_string);
        XtAddCallback(filterLabel, XmNhelpCallback,
        ...
        XtAddCallback(filterText, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_FIND_DIALOG_STR);
        ...
        XmStringFree(label_string);
        XtAddCallback(contentLabel, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_FIND_DIALOG_STR);
        ...
        XtAddCallback(contentText, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_FIND_DIALOG_STR);
        ...
        XtAddCallback(followLink, XmNhelpCallback,
                      (XtCallbackProc)HelpRequestCB, HELP_FIND_DIALOG_STR);
        ...
        label_string = XmStringCreateLocalized(
            ((char *)GETMESSAGE(15, 42, "Search Folder: ")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, label_string);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        n++;
        outputSeparator =
            XmCreateSeparatorGadget(form, "outputSeparator", args, n);
        XtManageChild(outputSeparator);

        label_string =
            XmStringCreateLocalized(GetSharedMessage(FILES_FOUND_LABEL));
        n = 0;
        XtSetArg(args[n], XmNlabelString, label_string);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        */
        ...
        label_string =
            XmStringCreateLocalized((GETMESSAGE(15, 48, "Open Folder")));
        ...
        label_string =
            XmStringCreateLocalized(((char *)GETMESSAGE(15, 14, "Start")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, label_string);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
        ...
        label_string =
            XmStringCreateLocalized(((char *)GETMESSAGE(15, 15, "Stop")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, label_string);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
...
static void InstallClose(FindRec *find_rec, XtCallbackProc callback,
        ...
        XtAddCallback(find_rec->close, XmNactivateCallback, callback,
                      client_data);
        ...
        if (restrictMode &&
            strncmp(users_home_dir, dirbuf, strlen(users_home_dir)) != 0) {
                find_data->directories =
                    (char *)XtMalloc(strlen(users_home_dir) + 1);
...
static XtPointer GetResourceValues(XrmDatabase data_base, char **name_list) {
        ...
        _DtDialogGetResources(data_base, name_list, FIND_FILE,
                              (char *)find_data, resources,
                              findClass->resource_count);
...
static void SetValues(FindRec *find_rec, FindData *find_data) {
        ...
        if (find_data->file_mgr_data->restricted_directory) {
                XtAddCallback(find_rec->searchDirectory,
        ...
        if (find_data->num_matches > 0) {
                ...
                matches = (XmString *)XtMalloc(sizeof(XmString) *
                                               find_data->num_matches);

                for (i = 0; i < find_data->num_matches; i++) {
                        matches[i] =
                            XmStringCreateLocalized(find_data->matches[i]);
                }
...
static Boolean GetFindValues(FindRec *find_rec, FindData *find_data,
        ...
        if (find_data->directories)
                XtFree(find_data->directories);
        find_data->directories =
            XmTextFieldGetString(find_rec->searchDirectory);

        if (validate) {
                find_data->directories =
                    (char *)_DtStripSpaces(find_data->directories);
        ...
        if (find_data->num_matches == 0) {
                ...
                for (i = 0; i < find_data->num_matches; i++) {
                        find_data->matches[i] = (char *)_XmStringUngenerate(
                            stringTable[i], XmFONTLIST_DEFAULT_TAG,
...
static void InvalidFindMessage(FindRec *find_rec, int messageIndex,
        ...
        if (noExistanceMessage == NULL) {
                ...
                tmpStr = GETMESSAGE(
                    15, 50,
                    "Search Folder name or File Content argument is "
                    "missing\nType in the name of the folder where you "
                    "want the search to begin.\nOr type in the string that "
                    "you want to search.");
...
static void StopSearch(Widget w, FindRec *find_rec) {
        ...
        if (find_rec->popenId != NULL) {
                /* Abort the find process, and remove the alternate input
                 * handler */
...
static void StartSearch(Widget w, FindRec *find_rec) {
        ...
        if (!FindProcessStarted(find_rec, find_data)) {
                ...
                /* This event handlers will be called when user is moving his
                   mouse over the Find dialog's stop button.
                */
...
static Boolean ExecuteFind(FindRec *find_rec, FindData *find_data,
        ...
        if (strcmp(find_data->content, "") == 0) {
                label_string = XmStringCreateLocalized(
                    GetSharedMessage(FILES_FOUND_LABEL));
                ...
                tmpStr =
                    (GETMESSAGE(15, 39, "Files Found (by Name and Contents):"));
                label_string = XmStringCreateLocalized(tmpStr);
        }
        ...
        _DtPathFromInput(findptr, file_mgr_data->current_directory, &host,
                         &path);
    ...
    defined(linux) || defined(CSRG_BASED)
        ...
        if (strcmp(find_data->content, "") != 0) {
                command =
                    GrowBuffer(command, &commandLen, (int)strlen(TYPEDIR));
        ...
        if (strcmp(find_data->content, "") != 0) {
                find_rec->alternateInputId = XtAddInput(
                    fileno(find_rec->popenId), (XtPointer)XtInputReadMask,
...
static Boolean ExecuteGrep(FindRec *find_rec) {
        ...
        if (ptr2 == NULL) {
                ...
                if (item_count == 0) {
                        ...
                        _DtMessage(find_rec->shell, title, msg, NULL,
                                   HelpRequestCB);
                        XtFree(title);
                        XtFree(msg);
                } else {
                        XmListSelectPos(find_rec->matchList, 1, True);
                        XmProcessTraversal(find_rec->matchList,
                                           XmTRAVERSE_CURRENT);
                }
                ...
                *ptr2 = '\0';
                command =
                    GrowBuffer(command, &commandLen, (int)strlen(ptr) + 3);
...
static void AlternateInputHandler(XtPointer client_data, int *source,
        ...
        while (1) {
                while ((offset < bufSize - 1) &&
                       ((count = fread(&next, sizeof(char), 1,
                                       find_rec->popenId)) == 1) &&
                       (next != '\n')) {
                        buf[offset++] = next;
                }
                ...
                if (offset > 0) {
                        ...
                        if ((stat(buf, &stat_data) == 0) ||
                            (lstat(buf, &stat_data) == 0)) {
                                ...
                                if (findptr) {
                                        file_type = (char *)DtDtsDataToDataType(
                                            findptr, NULL, 0, NULL, NULL, NULL,
                                            NULL);
                ...
                if (count == 0) {
                        ...
                        XtRemoveEventHandler(
                            find_rec->stop, LeaveWindowMask, FALSE,
                            (XtEventHandler)LeaveStopBttn, find_rec);
                        XtRemoveEventHandler(
                            find_rec->stop, EnterWindowMask, FALSE,
                            (XtEventHandler)EnterStopBttn, find_rec);
                        find_rec->alternateInputId = 0;
                ...
                if (count == 0) {
                        ...
                        if (buffer != NULL)
                                ...
                                if (item_count == 0) {
                                        ...
                                        tmpStr =
                                            GetSharedMessage(FIND_ERROR_TITLE);
                                        title = XtNewString(tmpStr);
                                        tmpStr = GetSharedMessage(
                                            NO_FILES_FOUND_ERROR);
                                        msg = XtNewString(tmpStr);
                                        _DtMessage(find_rec->shell, title, msg,
                                                   NULL, HelpRequestCB);
                                        XtFree(title);
                                        XtFree(msg);
                                } else {
                                        XmListSelectPos(find_rec->matchList, 1,
                                                        True);
                                        XmProcessTraversal(find_rec->matchList,
                                                           XmTRAVERSE_CURRENT);
                                }
...
static Boolean ExtractDirectory(String host, String path, char **file_name) {
        ...
        realPath = (String)ResolveLocalPathName(host, path, NULL,
...
static void NewView(Widget widget, XtPointer client_data, XtPointer call_data) {
        ...
        if (host == NULL) {
                ...
                if (ExtractDirectory(host, path, &file_name)) {
                        ...
                        if (file_mgr_data->restricted_directory == NULL)
                                dialog_data =
                                    GetNewView(host, path, NULL, NULL, 0);
                        else {
                                special_view = True;
                                special_treeType = file_mgr_data->show_type;
                                special_viewType = file_mgr_data->view;
...
static void FindPutOnDesktop(Widget widget, XtPointer client_data,
        ...
        if (host == NULL)
                result =
                    ExtractDirectory(file_mgr_data->host, path, &file_name);
        else
...
FILE *findpopen(cmd, mode, childpid) char *cmd;
char *mode;
int *childpid;
{
        ...
        if ((*childpid = fork()) == 0) {
                ...
                if (read_or_write !=
                    childside) /* If what we got is already stdin */
