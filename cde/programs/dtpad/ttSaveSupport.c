 ...
 *
 ******************************************************************************/
Tt_message TTSaveContractCB(Tt_message m, void *clientData,
                            Tt_message contract) {
        Editor *pPad;
        int silent, force;
        Tt_status status;
        ...
        if (op == TTDT_SAVE) {
                        ...
                        for (pPad = pPadList; pPad != (Editor *)NULL;
                             pPad = pPad->pNextPad) {
                                if (pPad->inUse == True &&
                                    ...
                                    strcmp(pPad->ttEditReq.msg_id, messageID) ==
                                        0) {
                                        pPad->ttSaveReq.contract =
                                            m; /* process Save msg */
                                        pPad->ttSaveReq.messageID =
                                            strdup(messageID);
                                        FileSaveCB((Widget)NULL, (caddr_t)pPad,
                                                   (caddr_t)NULL);
                                        tt_release(mark);
                                        return (
                                            (Tt_message)0); /* handling Save */
                                }
                        }
                        /* no Edit window matching the messageID */
                        tttk_message_fail(m, TT_DESKTOP_ECANCELED, /* status */
                ...
                /* We're handling all Save requests.  If the request has not
                 * been failed already, pPad->ttSaveReq.contract will be
                 * non-NULL and the Save message will be responded to or failed
                 * later. */
                tt_release(mark);
 ...
 ******************************************************************************/
Tt_message _TT_pattern_contract(Tt_pattern pat) {
 ...
 ******************************************************************************/
static Tt_callback_action _TTpatternCB(Tt_message msg, Tt_pattern pat) {
 ...
 ******************************************************************************/
Tt_pattern _TTCreatePattern(Tt_category category, Tt_scope theScope,
                            int addDefaultSess, const char *file,
                            const char *op_string, Ttdt_contract_cb clientCB,
                            void *clientData) {
 ...
 ******************************************************************************/
char *_TTmessage_id(Tt_message msg) {
 ...
 ******************************************************************************/
Tt_pattern TTCreateSavePattern(Tt_message contract, Ttdt_contract_cb clientCB,
                               void *clientData, int register_it) {
        char *msgID;

        /* -----> Create pattern */
        Tt_pattern pat =
            _TTCreatePattern(TT_HANDLE,  /* catagory */
                             TT_SESSION, /* scope */
 ...
 ************************************************************************/
void TTresetSaveArgs(Editor *pPad) {
...
void TTfailPendingSave(Editor *pPad) {
        if (pPad->ttSaveReq.contract) {
                tttk_message_fail(
                    pPad->ttSaveReq.contract, /* original Save req */
