...
static void FileOpenOkCB(Widget w, caddr_t client_data, caddr_t call_data);
static void IncludeFile(Widget w, caddr_t client_data, caddr_t call_data);
static Boolean SaveUnsaved(Editor *pPad, void (*callingFunc)());
static Boolean FileExitWP(XtPointer client_data);
 ...
 ************************************************************************/
/* ARGSUSED */
void FileCascadingCB(Widget w, caddr_t client_data, caddr_t call_data) {
 ...
 ************************************************************************/
/* ARGSUSED */
void FileNewCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        ...
        if (pPad->ttEditReq.contract &&
            pPad->ttEditReq.op != TTME_INSTANTIATE) {
 ...
 ************************************************************************/
/* ARGSUSED */
void FileOpenCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        ...
        /* -----> obtain the name of the file to open (via a Xm file selection
         * box) and load its contents (via FileOpenOkCB) to the Editor widget */
        pStuff->pendingFileFunc = FileOpenOkCB; /* FSB XmNokCallback */
        pStuff->pendingFileHelpFunc =
            HelpOpenDialogCB; /* FSB XmNhelpCallback */
 ...
 ************************************************************************/
/* ARGSUSED */
void FileIncludeCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        FileStuff *pStuff = &pPad->fileStuff;
 ...
 *
 ************************************************************************/
void FileSaveCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        void (*pFunc)();
        ...
        if (pPad->fileStuff.fileName &&
            *pPad->fileStuff.fileName) { /* filename? */
                /* -----> if called directly from [Save] menu and word wrap is
                 * on, display Save dialog so that saveWithNewlines can be set
                 */
                if (pPad->xrdb.wordWrap && !pPad->fileStuff.pendingFileFunc &&
                    !pPad->ttSaveReq.contract) {
                        AskIfSave(pPad);
                ...
                if (pPad->ttEditReq.contract) {
                        if (pPad->ttEditReq.contents) {
                                if (TTmediaDepositContents(pPad) != 0) {
                                        if (pPad->fileStuff.pendingFileFunc ==
                                            FileExitCB) {
 ...
 ************************************************************************/
void FileSaveAsCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        ...
        if (!pSaveAs->saveAs_form) {
                ...
                /*
                 * XXX - Should be dealing with the FSB instead of the text
                 * field directly. Also, should be setting an XmString, rather
                 * than a char *
                 */
 ...
 ************************************************************************/
/* ARGSUSED */
void PrintActionCB(DtActionInvocationID actionID, XtPointer client_data,
                   DtActionArg *actionArgp, int argCount,
 ...
 ************************************************************************/
/* ARGSUSED */
void FilePrintCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        DtActionArg *actionArgp =
            (DtActionArg *)XtCalloc(2, sizeof(DtActionArg));
        ...
        if ((pr_name = GetTempFile()) == (char *)NULL) {
                ...
                Warning(pPad,
                        ((char *)GETMESSAGE(
                            4, 5, "Unable to create a temporary file.")),
                        XmDIALOG_ERROR);
                return;
        }
        ...
        errorCode = DtEditorSaveContentsToFile(
            pPad->editor, pr_name, True, /* overwrite existing file */
            addNewlines,                 /* replace soft line feeds? */
        ...
        actionID =
            DtActionInvoke(pPad->app_shell, "PRINT_DTPAD_TEMPFILE", /* action */
 ...
 ************************************************************************/
/* ARGSUSED */
void FileXpPrintCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
 ...
 ************************************************************************/
/* ARGSUSED */
void FileExitCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        ...
        if (SaveUnsaved(pPad, FileExitCB)) {
                 ...
                 * 2) an error has occurred (in which case we abort the exit
                 * operation, failing the TTDT_QUIT request if it initiated the
                 * exit).
                 */
                if (pPad->fileStuff.pendingFileFunc !=
                    FileExitCB) { /* error occurred */
                        TTfailPendingQuit(pPad);
                }
                return;
        }
...
static Boolean FileExitWP(XtPointer client_data) {
        ...
        if (pPad->ttEditReq.contract) {
                TTmediaReply(
                    pPad); /* reply/close ToolTalk Edit/Display request */
        }
        ...
        /* -----> Set app shell geo (pixels), resize hints, position & size
         * hints */
        SetWindowSize(pPad);
 ...
 ************************************************************************/
/* ARGSUSED */
void oldFileExitCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        Tt_status status;
 ...
 ************************************************************************/
void NoSaveCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        void (*pFunc)();
        ...
        if ((pFunc = pPad->fileStuff.pendingFileFunc) != (void (*)())NULL) {
                /* -----> don't clear the pending function if it calls
                 * SaveUnsaved() */
 ...
 ************************************************************************/
/* ARGSUSED */
void CancelFileSelectCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
...
static void FileOpenOkCB(Widget w, caddr_t client_data, caddr_t call_data) {
         ...
         * XXX - Eventually, it makes sense to store the name/etc. as an
         * XmString rather than convert everything to a string.  This will mean
         * changing the pPad.fileName type.
         * Additionally, we can get quit saving the text field ID and
         * deal only with the FSB. */

        name = (char *)_XmStringUngenerate(cb->value, NULL, XmMULTIBYTE_TEXT,
                                           XmMULTIBYTE_TEXT);
        ...
        ExtractAndStoreDir(pPad, name,
                           OPEN); /* store pPad->fileStuff.pathDir */
...
static void IncludeFile(Widget w, caddr_t client_data, caddr_t call_data) {
        ...
        ExtractAndStoreDir(pPad, name,
                           INCLUDE); /* store pPad->fileStuff.pathDir */
...
static Boolean SaveUnsaved(Editor *pPad, void (*callingFunc)()) {
        ...
        if (DtEditorCheckForUnsavedChanges(pPad->editor)) {
                ...
                /* -----> If handling a "silent" TTDT_QUIT request, don't
                 * AskIfSave and don't save any unsaved changes (TTDT_SAVE does
                 * this) */
                if (callingFunc == FileExitCB && pPad->ttQuitReq.contract &&
                    ...
                    (pPad->fileStuff.fileName && *pPad->fileStuff.fileName)) {
                        ...
                        if (errorCode != SUCCESS) {
                                ...
                                if (callingFunc == FileExitCB) {
                                        /* Set saveOnClose to False to force
                                         * user to explicitly choose to not save
                                         * changes in order to exit. */
                        ...
                        if (callingFunc == pPad->fileStuff.pendingFileFunc) {
                                /* We've already did AskIfSave but the user
                                 * responded "No" so lets not keep asking
                                 * (NoSaveCB does not clear
                                 * pPad->fileStuff.pendingFileFunc). */
                                pPad->fileStuff.pendingFileFunc =
                                    (void (*)())NULL;
                                pPad->ttEditReq.returnBufContents = False;
                        } else {
                                /* AskIfSave assigns either FileSaveAsCB or
                                 * FileSaveCB to the the o.k. dialog button that
                                 * it posts.  These callbacks execute
                                 * pPad->fileStuff.pendingFileFunc when done. */
 ...
 ************************************************************************/
/* ARGSUSED */
void SaveNewLinesCB(Widget w, XtPointer client_data, XtPointer call_data) {
        Editor *pPad = (Editor *)client_data;
 ...
 ************************************************************************/
void SaveAsOkCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        SaveAs *pSaveAs = &pPad->fileStuff.fileWidgets.saveAs;
        void (*pFunc)();
        Widget textField = XmFileSelectionBoxGetChild(
        ...
        if (!pPad->xrdb.nameChange &&
            pPad->fileStuff
                .fileName && /* allow for -noNameChange w/o a fileName */
            !overWrite) {
                markSaved = False;
        } else {
                markSaved = True;
        ...
        errorCode = DtEditorSaveContentsToFile(
            pPad->editor, pPad->fileStuff.savingName, overWrite,
            addNewlines, /* replace soft line feeds? */
            markSaved);  /* mark contents as saved? */
        if (errorCode ==
            DtEDITOR_WRITABLE_FILE) { /* file exists & not overwriting */
                PostAlreadyExistsDlg(
                    pPad); /* save handled in AlrdyExistsOkCB */
        ...
        if (errorCode != SUCCESS) {
                ...
                if (pPad->xrdb.nameChange == True) {
                        if (pPad->ttEditReq.contract &&
                            pPad->ttEditReq.op != TTME_INSTANTIATE) {
                                pPad->ttEditReq.returnBufContents =
                                    False;          /* drop chgs w/o notice */
                                TTmediaReply(pPad); /* reply/close ToolTalk
                                                       media request */
                        }
                        XtFree(pPad->fileStuff.fileName);
 ...
 ************************************************************************/
void AlrdyExistsOkCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
        void (*pFunc)();
        DtEditorErrorCode errorCode;
        Boolean addNewlines = pPad->xrdb.wordWrap == True &&
        ...
        if (!pPad->xrdb.nameChange &&
            pPad->fileStuff
                .fileName && /* allow for -noNameChange w/o a fileName */
            (strcmp(pPad->fileStuff.fileName, pPad->fileStuff.savingName) !=
             0)) {
                markSaved = False;
        } else {
                markSaved = True;
        ...
        errorCode = DtEditorSaveContentsToFile(
            pPad->editor, pPad->fileStuff.savingName,
            True,        /* overwrite existing file */
            addNewlines, /* replace soft line feeds? */
            markSaved);  /* mark contents as saved? */
 ...
 ************************************************************************/
/* ARGSUSED */
void SaveAsCancelCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
 ...
 ************************************************************************/
/* ARGSUSED */
void AlrdyExistsCancelCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;
 ...
 ************************************************************************/
/* ARGSUSED */
void AskIfSaveCancelCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Editor *pPad = (Editor *)client_data;

        XtUnmanageChild(
            (Widget)pPad->fileStuff.fileWidgets.select.save_warning);
