...
#define SAVE_CH_NONAME                                                         \
        ((char *)GETMESSAGE(                                                   \
            5, 2, "The current file has no name.\nDo you want to save it?"))
#define SAVE_CH_BUF ((char *)GETMESSAGE(5, 40, "Save changes?"))
#define SAVE_CH_DOC ((char *)GETMESSAGE(5, 41, "Save changes to \"%s?\""))
#define SAVE_DROP                                                              \
        ((char *)GETMESSAGE(5, 3,                                              \
                            "Do you want to save the current copy of "         \
                            "%s\nbefore you Edit %s?"))
#define MSG1                                                                   \
        ((char *)GETMESSAGE(                                                   \
            5, 5,                                                              \
            "The file %s already exists.\nDo you want to over "                \
            "write that file\nwith the contents within this edit "             \
            "session? "))

/************************************************************************
 *			Forward Declarations
 ************************************************************************/
static void CreateFileSelectionDialog(Editor *pPad);
static void CreateSaveWarning(Editor *pPad);
 ...
 ************************************************************************/
void PostAlreadyExistsDlg(Editor *pPad) {
        Arg args[10];
        int n;
        char *tmpMess;
        XmString tmpMessStr;
        ...
        if (!pSaveAs->alrdy_exist) {
                ...
                XtSetArg(args[n], XmNdialogTitle, tempStr);
                n++;
                XtSetArg(args[n], XmNokLabelString,
                         XmStringCreateLocalized((String)_DtOkString));
                n++;
                tempStr2 =
                    XmStringCreateLocalized((char *)GETMESSAGE(2, 1, "Help"));
                XtSetArg(args[n], XmNhelpLabelString, tempStr2);
                n++;
                XtSetArg(args[n], XmNcancelLabelString,
                         XmStringCreateLocalized((String)_DtCancelString));
                n++;
                pSaveAs->alrdy_exist =
                    XmCreateMessageDialog(pPad->app_shell, "Warn", args, n);
                ...
                n = 0;
                pixmap = XmGetPixmap(XtScreen(pSaveAs->alrdy_exist),
                                     "warn_image", foreground, background);
                XtSetArg(args[n], XmNsymbolPixmap, pixmap);
                n++;
                XtSetValues(pSaveAs->alrdy_exist, args, n);

                XtAddCallback(pSaveAs->alrdy_exist, XmNokCallback,
                              (XtCallbackProc)AlrdyExistsOkCB, (XtPointer)pPad);
        ...
        tmpMess = (char *)XtMalloc(strlen(MSG1) +
                                   strlen(pPad->fileStuff.savingName) + 1);
        sprintf(tmpMess, MSG1, pPad->fileStuff.savingName);
        tmpMessStr = XmStringCreateLocalized(tmpMess);
        n = 0;
        XtSetArg(args[n], XmNmessageString, tmpMessStr);
        n++;
        XtSetValues(pSaveAs->alrdy_exist, args, n);
        XtFree(tmpMess);
...
void SetSaveAsDirAndFile(Editor *pPad) {
        char dirbuf[MAX_DIR_PATH_LEN], *currentVal, *directoryVal, *lastSlash,
            *pColon;
        ...
        textField = XmFileSelectionBoxGetChild(
            pPad->fileStuff.fileWidgets.saveAs.saveAs_form, XmDIALOG_TEXT);
        ...
        if (currentVal != (char *)NULL && *currentVal != (char)'\0') {
                /* -----> If the file name begins with '/', it has a pathname
                 *and we can use it for the initial directory */
                if (currentVal[0] == '/') {
                        strcpy(dirbuf, currentVal);
                        /* Trim off everything behind the last slash */
                        if ((lastSlash = MbStrrchr(dirbuf, '/')) !=
                            (char *)NULL) {
                                *(lastSlash + 1) = (char)'\0';
                        }
                } else {
                        /* -----> The filename does not have a directory, so use
                         * the value from the directory field. */
                        n = 0;
                        XtSetArg(args[n], XmNdirectory, &dirString);
                        n++;
                        XtGetValues(
                        ...
                        directoryVal = (char *)XtMalloc(sizeof(char) *
                                                        (MAX_DIR_PATH_LEN + 1));
                        XmStringGetLtoR(dirString, XmFONTLIST_DEFAULT_TAG,
                                        &directoryVal);
                ...
                XtFree(currentVal);
                /* -----> No file name was not previously entered via a SaveAs
                 * FSB so use other defaults. */
        } else {
        ...
        n = 0;
        XtSetArg(args[n], XmNdirMask, dirString);
        n++;
 ...
 ************************************************************************/
static void CreateNewLineToggles(Editor *pPad, Widget parent,
        ...
        pToggleWidgets->newl_radio =
            XmCreateRadioBox(parent, "radioBox", args, n);

        /* -----> Create the toggle button for adding newlines. */
        label_string = XmStringCreateLocalized(((char *)GETMESSAGE(
            5, 44, "Add newline characters to the end of wrap-to-fit lines.")));
        n = 0;
        XtSetArg(args[n], XmNshadowThickness, 0);
        n++;
        ...
        pToggleWidgets->with_newl = XmCreateToggleButtonGadget(
            pToggleWidgets->newl_radio, "with_nl", args, n);
        ...
        label_string = XmStringCreateLocalized(((char *)GETMESSAGE(
            5, 45,
            "Do not add newlines.  Only line breaks created by "
            "[Return]\nwill be preserved.")));
        n = 0;
        XtSetArg(args[n], XmNshadowThickness, 0);
        n++;
        ...
        pToggleWidgets->without_newl = XmCreateToggleButtonGadget(
            pToggleWidgets->newl_radio, "without_nl", args, n);
        XmStringFree(label_string);
 ...
 * CreateSaveAsDialog
 ************************************************************************/
void CreateSaveAsDialog(Editor *pPad) {
        Arg args[20];
        register int n;
        XmString label_string;
        char buf[256];
        ...
        n = 0;
        XtSetArg(args[n], XmNdialogTitle, label_string);
        n++;
        ...
        n++;
        pSaveAs->saveAs_form = XmCreateFileSelectionDialog(
            pPad->app_shell, "save_dialog", args, n);
        XmStringFree(label_string);

        /* -----> Add the FSB cancel, ok, and help callbacks */
        XtAddCallback(pSaveAs->saveAs_form, XmNcancelCallback,
                      (XtCallbackProc)SaveAsCancelCB, (XtPointer)pPad);
        XtAddCallback(pSaveAs->saveAs_form, XmNokCallback,
                      (XtCallbackProc)SaveAsOkCB, (XtPointer)pPad);
        XtAddCallback(pSaveAs->saveAs_form, XmNhelpCallback,
                      (XtCallbackProc)HelpSaveAsDialogCB, (XtPointer)pPad);
        ...
        CreateNewLineToggles(pPad, pSaveAs->saveAs_form, /* parent widget */
 ...
 ************************************************************************/
void ExtractAndStoreDir(Editor *pPad, char *fileName, short type) {
        ...
        if (type == OPEN) {
                if (pFirstSlash != (char *)NULL) {
                        ...
                        pPad->fileStuff.pathDir[pLastSlash - pFirstSlash + 1] =
                            '\0';
                }
        } else {
                if (pFirstSlash != (char *)NULL) {
                        ...
                        strncpy(pPad->fileStuff.includePathDir, pFirstSlash,
                                pLastSlash - pFirstSlash + 1);
                        pPad->fileStuff
                            .includePathDir[pLastSlash - pFirstSlash + 1] =
                            '\0';
                        if (pPad->fileStuff.lastIncludeName != NULL)
                                XtFree(pPad->fileStuff.lastIncludeName);
                        pLastSlash++;
                        pPad->fileStuff.lastIncludeName =
                            XtNewString(pLastSlash);
                }
        }
}
 ...
 ************************************************************************/
void GetFileName(Editor *pPad, XmString title, short type) {
        ...
        XtAddCallback(pSelect->file_dialog, XmNokCallback,
                      (XtCallbackProc)pStuff->pendingFileFunc, (XtPointer)pPad);
        ...
        if (type == OPEN) {
                if (pStuff->pathDir[0] == (char)'\0') {
                        if (pStuff->fileName != NULL) {
                                /* set pStuff->pathDir*/
                                ExtractAndStoreDir(pPad, pStuff->fileName,
                                                   OPEN);
                        } else {
                                if (pStuff->pathDir[0] != (char)'\0')
                                ...
                                dirString = XmStringCreateLocalized(dirbuf);
                                ac = 0;
                                XtSetArg(al[ac], XmNdirMask, dirString);
                                ac++;
                                XtSetValues(
                                    pStuff->fileWidgets.select.file_dialog, al,
                                    ac);
                                XmStringFree(dirString);
                        }
                }
                ...
                if (pStuff->includePathDir[0] != (char)'\0') {
                        tmpStr =
                            XmStringCreateLocalized(pStuff->includePathDir);
                        XtSetArg(al[0], XmNdirectory, tmpStr);
                        XtSetValues(pSelect->file_dialog, al, 1);
                        XmStringFree(tmpStr);
                }
        ...
        if (type == INCLUDE) {
                if (pStuff->lastIncludeName != NULL) {
                        tmpStr =
                            XmStringCreateLocalized(pStuff->lastIncludeName);
                        XmListSelectItem(
                            XmFileSelectionBoxGetChild(pSelect->file_dialog,
                                                       XmDIALOG_LIST),
                            tmpStr, False);
        ...
        XtManageChild(pSelect->file_dialog);
        XmProcessTraversal(
            XmFileSelectionBoxGetChild(pSelect->file_dialog, XmDIALOG_TEXT),
            XmTRAVERSE_CURRENT);
...
static void CreateFileSelectionDialog(Editor *pPad) {
        ...
        XtSetArg(al[ac], XmNhelpLabelString, tmpStr);
        ac++;

        pSelect->file_dialog = XmCreateFileSelectionDialog(
            pPad->app_shell, "file_sel_dlg", al, ac);
        XmStringFree(tmpStr);
        ...
        pSelect->select_text =
            XmFileSelectionBoxGetChild(pSelect->file_dialog, XmDIALOG_TEXT);
        pSelect->file_list =
            XmFileSelectionBoxGetChild(pSelect->file_dialog, XmDIALOG_LIST);
}
...
static void CreateSaveWarning(Editor *pPad) {
        ...
        ac = 0;
        XtSetArg(al[ac], XmNapplyLabelString, tmpStr1);
        ac++;
        XtSetArg(al[ac], XmNokLabelString, tmpStr2);
        ac++;
        XtSetArg(al[ac], XmNdialogTitle, tmpStr3);
        ac++;
        XtSetArg(al[ac], XmNhelpLabelString, tmpStr4);
        ac++;
        XtSetArg(al[ac], XmNcancelLabelString, tmpStr5);
        ac++;
        XtSetArg(al[ac], XmNmwmFunctions, MWM_FUNC_MOVE | MWM_FUNC_CLOSE);
        ac++;
        XtSetArg(al[ac], XmNmwmDecorations, MWM_DECOR_BORDER | MWM_DECOR_TITLE);
        ac++;
        XtSetArg(al[ac], XmNallowShellResize, False);
        ac++;
        pSelect->save_warning =
            XmCreatePromptDialog(pPad->app_shell, "save_warn", al, ac);
        ...
        /* -----> Create Form to contain the prompt symbol and text (in a
         * RowColumn) and the "insert new lines?" toggles (in a RadioBox) */
        ac = 0;
        XtSetArg(al[ac], XmNshadowThickness, 0);
        ac++;
        pSelect->work_area =
            XmCreateForm(pSelect->save_warning, "wkarea", al, ac);
        ...
        ac++;
        pSelect->row_col =
            XmCreateRowColumn(pSelect->work_area, "rowCol", al, ac);
        ...
        ac = 0;
        pSelect->wkArea_textLabel =
            XmCreateLabelGadget(pSelect->row_col, "txt_lab", al, ac);
        ...
        ac++;
        XtSetArg(al[ac], XmNtopWidget, pSelect->row_col);
        ac++;
        ...
        ac++;
        pSelect->separator =
            XmCreateSeparator(pSelect->work_area, "sep", al, ac);
        if (pPad->xrdb.wordWrap == True)
                XtManageChild(pSelect->separator);

        /* -----> Create a radio box with two toggle buttons for either adding
         *        or not adding new lines if word wrap mode is on. */
        CreateNewLineToggles(pPad, pSelect->work_area, /* parent widget */
        ...
        XtSetArg(al[ac], XmNtopAttachment, XmATTACH_WIDGET);
        ac++;
        XtSetArg(al[ac], XmNtopWidget, pSelect->separator);
        ac++;
        XtSetArg(al[ac], XmNtopOffset, 5);
        ac++;
        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_FORM);
        ...
        w = XmSelectionBoxGetChild(pSelect->save_warning,
        ...
        ac = 0;
        kid[ac++] =
            XmSelectionBoxGetChild(pSelect->save_warning, XmDIALOG_TEXT);
        kid[ac++] = XmSelectionBoxGetChild(pSelect->save_warning,
                                           XmDIALOG_SELECTION_LABEL);
        XtUnmanageChildren(kid, ac);

        /* -----> set the SelectionBox PromptDialog [No] [Cancel] [Help]
         * callbacks (the [Yes] XmNokCallback is set in AskIfSave() based on
         * whether or not a file name has been specified among other
         * things) */
         ...
         *        processed */
        pSelect->cancelBtn = XmSelectionBoxGetChild(pSelect->save_warning,
                                                    XmDIALOG_APPLY_BUTTON);
 ...
 ************************************************************************/
void Warning(Editor *pPad, char *mess, unsigned char dialogType) {
        Arg al[10];
        int ac;
        ...
        if (!pFileWidgets->gen_warning) {
                ...
                XtSetArg(al[ac], XmNokLabelString,
                         XmStringCreateLocalized((String)_DtOkString));
                ac++;
                pFileWidgets->gen_warning =
                    XmCreateMessageDialog(pPad->app_shell, "Warn", al, ac);
        ...
        ac = 0;
        tempStr = XmStringCreateLocalized(buf);
        XtSetArg(al[ac], XmNdialogTitle, tempStr);
        ac++;
        XtSetArg(al[ac], XmNdialogType, dialogType);
        ac++;
        XtSetArg(al[ac], XmNmessageString, XmStringCreateLocalized(tmpMess));
        ac++;
 ...
 *	callbacks).
 ************************************************************************/
void PostSaveError(Editor *pPad, char *saveName, DtEditorErrorCode errorCode) {
        ...
        case DtEDITOR_UNWRITABLE_FILE:
                errorReason =
                    strdup((char *)GETMESSAGE(5, 46,
                                              "Either file is unwritable or "
                                              "path to\nfile does not exist."));
                break;
        case DtEDITOR_DIRECTORY:
                errorReason =
                    strdup((char *)GETMESSAGE(5, 35, "File is a directory."));
                break;
        case DtEDITOR_CHAR_SPECIAL_FILE:
                errorReason = strdup((char *)GETMESSAGE(
                    5, 36, "File is a character special device."));
                break;
        case DtEDITOR_BLOCK_MODE_FILE:
                errorReason = strdup(
                    (char *)GETMESSAGE(5, 37, "File is a block mode device."));
                break;
        case DtEDITOR_INSUFFICIENT_MEMORY:
                errorReason = strdup(
                    (char *)GETMESSAGE(5, 47,
                                       "Operation failed due to insufficient "
                                       "memory.\nTry increasing swap space."));
                break;
        case DtEDITOR_INVALID_TYPE: /* this should never occur */
                errorReason =
                    strdup((char *)GETMESSAGE(5, 38, "Invalid file format."));
                break;
        case DtEDITOR_SAVE_FAILED: /* this should catch everything else */
        default:                   /* - but just in case */
                errorReason = strdup((char *)GETMESSAGE(
                    5, 39, "Unknown reason - possibly not enough disk space."));
                break;
        }
...
void AskIfSave(Editor *pPad) {
        ...
        if (pPad->ttEditReq.contract && pPad->ttEditReq.docName &&
            *pPad->ttEditReq.docName) {
        ...
        XmProcessTraversal(
            XmSelectionBoxGetChild(pSelect->save_warning, XmDIALOG_OK_BUTTON),
            XmTRAVERSE_CURRENT);
}

char *DialogTitle(Editor *pPad) {
        if (pPad->dialogTitle == NULL)
                pPad->dialogTitle =
                    strdup((char *)GETMESSAGE(7, 2, "Text Editor - "));
