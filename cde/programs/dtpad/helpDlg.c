 ...
 ************************************************************************/
static Widget GetHelpDialog(Editor *pPad) {
        HelpStruct *pHelp;

        if (pPad->pHelpCache == (HelpStruct *)NULL) {
                pHelp = pPad->pHelpCache =
                    (HelpStruct *)XtMalloc(sizeof(HelpStruct));
 ...
 ************************************************************************/
void SetHelpVolAndDisplayHelp(Editor *pPad, char *locationId,
                              char *helpVolume) {
 ...
 ************************************************************************/
void DisplayHelp(Editor *pPad, char *helpVolume, char *locationId) {
 ...
 ************************************************************************/
Widget CreateHelpDialog(Editor *pPad) {
        Arg args[10];
        int n = 0;
        Widget dialog;
        ...
        XtAddCallback(dialog, DtNcloseCallback, (XtCallbackProc)HelpCloseCB,
                      pPad);
        return dialog;
}
 ...
 *	this pad.
 ************************************************************************/
void DisplayNewHelpWindow(Editor *pPad, char *helpVolume, char *locationId) {
 ...
 ************************************************************************/
void DisplayHelpDialog(Editor *pPad, Widget helpDialog, char *helpVolume,
                       char *locationId) {
        Arg args[10];
        int n = 0;
        ...
        titleStr =
            (char *)XtMalloc(strlen(helpStr) + strlen(DialogTitle(pPad)) + 1);
        sprintf(titleStr, "%s%s", DialogTitle(pPad), helpStr);
        n = 0;
        XtSetArg(args[n], XmNtitle, titleStr);
        n++;
