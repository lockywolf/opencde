 ...
 ************************************************************************/
void TTdisplayError(Editor *pPad, char *errorMsg, Tt_status status) {
 ...
 ******************************************************************************/
Tt_message TTdt_session_joinCB(Tt_message m, void *client_data,
                               Tt_message contract) {
        /* should never get here, if we do, notify sender with a fail */
        tttk_message_fail(m, TT_DESKTOP_ENOTSUP, /* status */
 ...
 *
 ******************************************************************************/
Tt_message TTdt_message_acceptCB(Tt_message m, void *client_data,
                                 Tt_message contract) {
        Editor *pPad;
        int silent, force;
        Tt_status status;
        ...
        if (op == TTDT_QUIT) {
                ...
                if (operation2Quit = tt_message_arg_val(m, 2)) {
                        for (pPad = pPadList; pPad != (Editor *)NULL;
                             pPad = pPad->pNextPad) {
                                if (pPad->inUse == True &&
                                    pPad->ttEditReq.msg_id &&
                                    strcmp(pPad->ttEditReq.msg_id,
                                           operation2Quit) == 0) {
                                        pPad->ttQuitReq.contract =
                                            m; /* process Quit msg */
                                        status =
                                            tt_message_arg_ival(m, 0, &silent);
                                        status =
                                            tt_message_arg_ival(m, 1, &force);
                                        pPad->ttQuitReq.silent =
                                            (Boolean)silent;
                                        pPad->ttQuitReq.force = (Boolean)force;
                                        pPad->ttQuitReq.operation2Quit =
                                            strdup(operation2Quit);
                                        FileExitCB((Widget)NULL, (caddr_t)pPad,
                                                   (caddr_t)NULL);
                                        tt_release(mark);
                                        return (
                                            (Tt_message)0); /* handling Quit */
                                }
                        }
                        /* no Edit window matching the operation2Quit */
                        tttk_message_fail(m, TT_DESKTOP_ENOMSG, /* status */
                                          (char *)NULL,         /* status str */
                                          1); /* destroy request */
                ...
                /* We're handling all Quit requests.  If the request has not
                 * been failed already, pPad->ttQuitReq.contract will be
                 * non-NULL and the Quit message will be responded to or failed
                 * later. */
                tt_release(mark);
 ...
 ******************************************************************************/
Tt_message TTmedia_ptype_declareCB(Tt_message m, void *client_data, Tttk_op op,
        ...
        if (diag != TT_OK &&
            !(op == TTME_INSTANTIATE &&
              diag == TT_DESKTOP_ENODATA)) { /* TTT - workaround for tttk bug */
                fprintf(
                    stderr,
                ...
                if (MbStrrchr(localPath, '/') != (char *)NULL) {
                        ...
                        *pathEnd = (char)'\0';
                        if ((pDir = opendir(dir)) != (DIR *)NULL) {
                                ...
                                XtFree(dir);
                                tttk_message_fail(
                                    m, TT_DESKTOP_ENOENT, /* status */
                                    (char *)NULL,         /* status str */
                                    1);                   /* destroy request */
                                tt_release(mark);
                                return ((Tt_message)0); /* Message handled by
                                                           this routine. */
                        }
                }
        }
        ...
        if (foundPad == False)
                foundPad =
                    FindOrCreatePad(&pPad); /* returns new Editor in pPad */
        ...
        if (tt_message_contexts_count(m)) {
                ...
                if (*(context = tt_message_context_val(m, "SESSION"))) {
                        ...
                        if (*(context =
                                  tt_message_context_val(m, "GEOMETRY"))) {
                        ...
                        if (context =
                                tt_message_context_val(m, "WINDOWWORDWRAP")) {
                                if (*context == 't')
                        ...
                        if (context =
                                tt_message_context_val(m, "SAVEONCLOSE")) {
                                if (*context == 't')
                        ...
                        if (context = tt_message_context_val(
                                m, "MISSINGFILEWARNING")) {
                                if (*context == 't')
                        ...
                        if (context =
                                tt_message_context_val(m, "NONAMECHANGE")) {
                                if (*context == 't')
                        ...
                        if (context =
                                tt_message_context_val(m, "WORKSPACELIST")) {
                                pPad->xrdb.workspaceList = strdup(context);
                        }

                        /* -----> client/server control options */
                        if (*(context =
                                  tt_message_context_val(m, "BLOCKING"))) {
        ...
        if (op == TTME_INSTANTIATE) {
                ...
                if (file) {
                        /* the mediaType of the first arg (contents) applies to
                         * either contents or a file */
                        pPad->ttEditReq.vtype =
                            strdup(tt_message_arg_type(m, 0));
                        pPad->ttEditReq.contents = False; /* editing a file */
                        pPad->ttEditReq.fileName = strdup(localPath);
                        pPad->fileStuff.fileName =
                            XtNewString(pPad->ttEditReq.fileName);
                } else {
                        pPad->ttEditReq.contents = True; /* editing a buffer */
                        pPad->xrdb.nameChange =
                            False; /* disallow switching to another
                                      file */
        ...
        /* -----> Accept the message.
         *
         * Registers patterns in the default session to handle
         *TT_HANDLER-addressed requests (created by specifying the "handler"
         *argument in tttk_message_create) in the following manner (based on
         *the shell and Ttdt_contract_cb args):
         *
         *    type	requests		handled
         *notes
         *    ----	--------------------	----------------------------
         *----- (1)	Get|Set_Geometry,	transparently
         *(a) Get|Set_Iconified, Get|Set_Mapped,
         *		Get|Set_XInfo,
         *		Raise, Lower
         *	  (2)	Pause, Resume		passed to Ttdt_contract_cb
         *(b) (3)	Quit, Get_Status	passed to Ttdt_contract_cb
         *(c)
         *
         *	NOTES:
         *	  (a)	type (1) requests are handled transparently because
         *the shell arg is specified (that is, a pattern is registered to handle
         ...
         *    (c)	type (3) request are handled in Ttdt_contract_cb because
         *it was specified; otherwise they fail with TT_DESKTOP_ENOTSUP (that
         *is, Ttdt_contract_cb is the pattern callback registered in
         *the pattern to handle these requests).
         */
        ttrc = ttdt_message_accept(m,                     /* Tt_message */
        ...
        if (pPad->ttEditReq.contents) {
                ...
                /* [SuG 5/18/95] Done
                   The NOT_SAMPLE warnings/errors need to be added to the
                   message catalog. Removed ifdefs.*/
                ...
                case DtEDITOR_NULLS_REMOVED:
                        Warning(pPad,
                                (char *)GETMESSAGE(
                                    6, 6, "Embedded Nulls stripped from file."),
                                XmDIALOG_WARNING);
                        break;
 ...
 ******************************************************************************/
Tt_status TTstartDesktopMediaExchange(Widget topLevelWithWmCommand,
                                      Editor *pPad) {
        char *my_procid = NULL;
        int my_ttfd = 0;
        ...
        if (!sess || (*sess == '\0')) {
                tt_default_session_set(tt_X_session(
                    XDisplayString(XtDisplay(topLevelWithWmCommand))));
        }
        ...
        if (pPad->xrdb.server == True) {
                 ...
                 * which in this case currently consist of the Instantialte,
                 * Display and Edit messages from the Media set, as defined in
                 * dtpad.ptypes. This will enable auto start of dtpad if any of
                 * the messages in the ptype are received.
                 */
                if ((status = ttmedia_ptype_declare(
                    ...
                    != TT_OK) {
                        fprintf(
                            ...
                            DTPAD_TOOL_CLASS, tt_status_message(status));
                        return (status);
                }

                /*
                 * Join the default sesssion,
                 * and register to deal with standard Desktop requests in the
                 *following manner (controlled by the Ttdt_contract_cb and shell
                 *arguments):
                 *
                 *    type	requests		handled
                 *notes
                 ...
                 *		Get|Set_Situation,
                 *		Signal, Get_Sysinfo
                 *	  (2)	Get|Set_Geometry,	fail with
                 *TT_DESKTOP_ENOTSUP	(b) Get|Set_Iconified, Get|Set_Mapped,
                 *		Raise, Lower,
                 *		Set_Xinfo,
                 *		Get_Xinfo		transparently
                 *	  (3)	Pause, Resume, Quit	passed to
                 *Ttdt_contract_cb	(c) (4)	Get_Status, Do_Command
                 *passed to Ttdt_contract_cb
                 *
                 *	NOTES:
                 *	  (b)	type (2) requests (except Get_Xinfo) fail
                 *because the shell arg is not a "realized mappedWhenManaged
                 *applicationShellWidget" (c)	type (3) requests are not
                 *handled transparently because Ttdt_contract_cb is specified
                 */
        ...
        XtAppAddInput(XtWidgetToApplicationContext(topLevelWithWmCommand),
                      my_ttfd, (XtPointer)XtInputReadMask,
                      tttk_Xt_input_handler, my_procid);
 ...
 ************************************************************************/
Boolean TTmediaDepositContents(Editor *pPad) {
        DtEditorErrorCode errorCode;
        DtEditorContentRec contentRec;
        ...
        contentRec.type = DtEDITOR_TEXT;
        errorCode =
            DtEditorGetContents(pPad->editor, &contentRec, addNewlines,
                                False); /* don't mark contents as saved yet */
        if (errorCode != DtEDITOR_NO_ERRORS) {
                Warning(pPad,
                        "Internal Error: Unable to get contents from Editor "
                        "Widget.",
                        XmDIALOG_ERROR);
                return (1);
        }
        ...
        if (status == TT_OK) {
                ...
                _DtTurnOffHourGlass(pPad->app_shell);
                TTdisplayError(
                    pPad, (char *)GETMESSAGE(13, 1, "Checkpoint save failed."),
                    status);
                return (1);
        }
        return (0);
 ...
 *	Returns: 0 if successful;  !0 if failure.
 ************************************************************************/
Boolean TTmediaReply(Editor *pPad) {
        DtEditorErrorCode errorCode;
        DtEditorContentRec contentRec;
        Tt_status status;
        Boolean addNewlines = pPad->xrdb.wordWrap == True &&
        ...
        if (!pPad->ttEditReq.contents) {
                ...
                _DtTurnOffHourGlass(pPad->app_shell);
                if (status == TT_OK) {
                        pPad->ttEditReq.contract =
                            0; /* request has been closed */
                        return (0);
                } else {
                        TTdisplayError(pPad,
                                       (char *)GETMESSAGE(
                                           13, 2, "Checkpoint reply failed."),
                                       status);
                        return (1);
                }
        }
        ...
        if (pPad->ttEditReq.returnBufContents) {
                ...
                errorCode = DtEditorGetContents(
                    pPad->editor, &contentRec, addNewlines,
                    False); /* don't mark contents as saved yet */
                if (errorCode != DtEDITOR_NO_ERRORS) {
                        Warning(pPad,
                                "Internal Error: Unable to get contents from "
                                "Editor Widget.",
                                XmDIALOG_ERROR);
                ...
                if (status == TT_OK) {
                        /* XXX - an inefficient way of marking contents as saved
                         * - need to use a global to keep track of unsaved
                         * changes and replace
                 ...
                 *        when the user responds "no" to saving unsaved
                 * contents).
                 *
                 *    NOTE: TT_DESKTOP_ECANCELED changes the "failure" into a
                 * "reply".
                 */
 ...
 ************************************************************************/
void TTresetQuitArgs(Editor *pPad) {
 ...
 *	specified
 ************************************************************************/
void TTfailPendingQuit(Editor *pPad) {
        if (pPad->ttQuitReq.contract) {
                tttk_message_fail(
                    pPad->ttQuitReq.contract, /* original Quit req */
                    TT_DESKTOP_ECANCELED,     /* status */
 ...
 ************************************************************************/
Tt_status TTdesktopOpen(int *ttFd, Editor *pPad, Boolean sendStarted) {
        char *procId;
        Tt_status status;
        char *vendor;
        ...
        ProcessToolTalkInputId =
            XtAppAddInput(pPad->app_context, *ttFd, (XtPointer)XtInputReadMask,
...
void TTwarning(Editor *pPad, Tt_status status, char *errsuff) {
        ...
        if (TT_OK == status || NULL == pPad)
                return;

        errfmt = GETMESSAGE(7, 13,
                            "Could not connect to ToolTalk service:\n%s\n%s\n");
        statmsg = tt_status_message(status);
        errmsg =
            XtMalloc(strlen(errfmt) + strlen(statmsg) + strlen(errsuff) + 2);
