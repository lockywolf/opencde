...
static char *_psGetResourceFileName(PrintSetup *pSetup);
static void _psSetupToSource(PrintSetup *pSetup);
static void _psSourceToSetup(PrintSetup *pSetup, Boolean wordWrap);
static void _psSetupToUi(PrintSetup *pSetup);
static void _psUiToSetup(PrintSetup *pSetup);
static Boolean _psUiSpecsAreValid(PrintSetup *pSetup);
static void _psAttachPrintSetupDialog(PrintSetup *pSetup, Editor *pPad);
static Widget _psCreatePrintSetupDialog(Widget parent, Editor *pPad);
static void _psPrintSetupCancelCB(Widget dtprintSetup, XtPointer client_data,
                                  XtPointer call_data);
static void _psPrintSetupDestroyCB(Widget dtprintSetup, XtPointer client_data,
                                   XtPointer call_data);
static void _psPrintSetupPrintCB(Widget dtprintSetup, XtPointer client_data,
                                 XtPointer call_data);
static void _psReleasePrintSetupDialog(PrintSetup *pSetup);
...
static ResourceValue _psHdrFtrValues[] = {
    {PRINT_NONE_STRING_TYPE_ENUM, PS_RESOURCE_VALUE_NONE, 15, 1, "None"},
    {PRINT_DATE_STRING_TYPE_ENUM, PS_RESOURCE_VALUE_DATE, 15, 2, "Date"},
    {PRINT_DOCNAME_STRING_TYPE_ENUM, PS_RESOURCE_VALUE_DOCNAME, 15, 3,
     "Document Name"},
    {PRINT_PAGE_NUMBER_STRING_TYPE_ENUM, PS_RESOURCE_VALUE_PAGE_NUMBER, 15, 4,
     "Page Number"},
    {PRINT_USER_NAME_STRING_TYPE_ENUM, PS_RESOURCE_VALUE_USER_NAME, 15, 5,
     "User Name"}};
...
static XtResource _psResources[] = {
    {
        ...
        XmRImmediate,
        (XtPointer)PRINT_NONE_STRING_TYPE_ENUM,
    },
    {
        ...
        XmRImmediate,
        (XtPointer)PRINT_PAGE_NUMBER_STRING_TYPE_ENUM,
    },
    {
    ...
    },
    {
        ...
        XmRImmediate,
        (XtPointer)PRINT_DOCNAME_STRING_TYPE_ENUM,
    },
    {
        ...
        XmRString,
        (XtPointer)PS_RESOURCE_VALUE_DFLT_MARGIN,
    },
    {
 ...
 ************************************************************************/
PrintSetup *PrintSetupCreate(Widget parent, char *docName, Boolean wordWrap,
                             Editor *pPad, XtCallbackProc cancelCB,
                             XtPointer cancelClosure,
                             XtCallbackProc closeDisplayCB,
                             XtPointer closeDisplayClosure,
 ...
 *     Class destructor.
 ************************************************************************/
void PrintSetupDestroy(PrintSetup *pSetup) {
        if (pSetup == NULL)
                return;
 ...
 ************************************************************************/
void PrintSetupDisplay(PrintSetup *pSetup) {
        if (pSetup == NULL)
                return;
 ...
 ************************************************************************/
void PrintSetupHide(PrintSetup *pSetup) {
        if (pSetup == NULL)
                return;
        if (pSetup->dtprintSetup == NULL)
                return;
 ...
 ************************************************************************/
PrintStringTypeEnum PrintSetupGetHdrFtrSpec(PrintSetup *pSetup,
                                            _DtPrintHdrFtrEnum which) {
        PrintStringTypeEnum rtn;

        if (pSetup == NULL)
                return PRINT_NONE_STRING_TYPE_ENUM;
        switch (which) {
 ...
 ************************************************************************/
char *PrintSetupGetMarginSpec(PrintSetup *pSetup, _DtPrintMarginEnum which) {
        char *rtn;

        if (pSetup == NULL)
                return _psDefaultMargin;
        switch (which) {
 ...
 ************************************************************************/
Boolean PrintSetupGetDefaultPrintData(PrintSetup *pSetup,
                                      DtPrintSetupData *pData) {
        if (pSetup == NULL || pData == NULL)
                return FALSE;
        return (DtPRINT_SUCCESS ==
                DtPrintFillSetupData(pSetup->dtprintSetup, pData));
}
 ...
 ************************************************************************/
Boolean PrintSetupUseWordWrap(PrintSetup *pSetup) {
        if (pSetup == NULL)
                return TRUE;
...
#define BUFFER_SIZE 256
        ...
        if (!DtSessionSavePath(topLevelWithWmCommand, &savePath, &saveFile)) {
                sprintf(buffer, "%s/%s/%s", getenv(PS_HOME_ENV_VARIABLE),
                        DtPERSONAL_TMP_DIRECTORY, PS_DEFAULT_RESOURCE_FILE);
                savePath = buffer;
        }
...
static void _psSourceToSetup(PrintSetup *pSetup, Boolean wordWrap) {
        ...
        XtGetApplicationResources(pSetup->parent, &pOption, _psResources,
                                  XtNumber(_psResources), NULL, 0);
...
static void _psSetupToSource(PrintSetup *pSetup) {
        ...
        fprintf(fp, PS_RESOURCE_HDRFTR_FORMAT_STRING,
                PS_RESOURCE_NAME_HEADER_LEFT, pOption->headerLeft);

        fprintf(fp, PS_RESOURCE_HDRFTR_FORMAT_STRING,
                PS_RESOURCE_NAME_HEADER_RIGHT, pOption->headerRight);

        fprintf(fp, PS_RESOURCE_HDRFTR_FORMAT_STRING,
                PS_RESOURCE_NAME_FOOTER_LEFT, pOption->footerLeft);

        fprintf(fp, PS_RESOURCE_HDRFTR_FORMAT_STRING,
                PS_RESOURCE_NAME_FOOTER_RIGHT, pOption->footerRight);

        fprintf(fp, PS_RESOURCE_MARGIN_FORMAT_STRING,
                PS_RESOURCE_NAME_MARGIN_TOP, pOption->marginTop);

        fprintf(fp, PS_RESOURCE_MARGIN_FORMAT_STRING,
                PS_RESOURCE_NAME_MARGIN_RIGHT, pOption->marginRight);

        fprintf(fp, PS_RESOURCE_MARGIN_FORMAT_STRING,
                PS_RESOURCE_NAME_MARGIN_BOTTOM, pOption->marginBottom);

        fprintf(fp, PS_RESOURCE_MARGIN_FORMAT_STRING,
                PS_RESOURCE_NAME_MARGIN_LEFT, pOption->marginLeft);
...
static void _psSetupToUi(PrintSetup *pSetup) {
        ...
        if (pSetup == NULL || pSetup->dtprintSetup == NULL ||
            pSetup->widgets == NULL)
                return;
        pOption = &pSetup->options;
        ...
        w = _DtPrintMarginFrameEnumToWidget(frame,
...
static Boolean _psUiSpecsAreValid(PrintSetup *pSetup) {
        ...
        if (marginSpec != NULL && strcmp("", marginSpec) != 0) {
                ...
                (void)XmConvertStringToUnits(XtScreenOfObject(pSetup->parent),
                                             marginSpec, XmVERTICAL, XmPIXELS,
                                             &parseError);
                ...
                if (parseError) {
                        ...
                        i18nMsg = GETMESSAGE(
                            15, 6, "Top Margin specifier is invalid:  ");
                        errMsg = (char *)XtMalloc(strlen(i18nMsg) +
                                                  strlen(marginSpec) + 1);
        ...
        if (marginSpec != NULL && strcmp("", marginSpec) != 0) {
                ...
                (void)XmConvertStringToUnits(XtScreenOfObject(pSetup->parent),
                                             marginSpec, XmHORIZONTAL, XmPIXELS,
                                             &parseError);
                ...
                if (parseError) {
                        ...
                        i18nMsg = GETMESSAGE(
                            15, 7, "Right Margin specifier is invalid:  ");
                        errMsg = (char *)XtMalloc(strlen(i18nMsg) +
                                                  strlen(marginSpec) + 1);
                        sprintf(errMsg, "%s%s", i18nMsg, marginSpec);
                        Warning(pSetup->pPad, (char *)errMsg, XmDIALOG_WARNING);
        ...
        if (marginSpec != NULL && strcmp("", marginSpec) != 0) {
                ...
                if (parseError) {
                        ...
                        i18nMsg = GETMESSAGE(
                            15, 8, "Bottom Margin specifier is invalid:  ");
                        errMsg = (char *)XtMalloc(strlen(i18nMsg) +
                                                  strlen(marginSpec) + 1);
                        sprintf(errMsg, "%s%s", i18nMsg, marginSpec);
                        Warning(pSetup->pPad, (char *)errMsg, XmDIALOG_WARNING);
        ...
        if (marginSpec != NULL && strcmp("", marginSpec) != 0) {
                ...
                if (parseError) {
                        ...
                        i18nMsg = GETMESSAGE(
                            15, 9, "Left Margin specifier is invalid:  ");
                        errMsg = (char *)XtMalloc(strlen(i18nMsg) +
                                                  strlen(marginSpec) + 1);
                        sprintf(errMsg, "%s%s", i18nMsg, marginSpec);
                        Warning(pSetup->pPad, (char *)errMsg, XmDIALOG_WARNING);
...
static void _psAttachPrintSetupDialog(PrintSetup *pSetup, Editor *pPad) {
        ...
        if (_psDefaultPrintSetupDialog != NULL) {
                /*
                 * Attempt to reuse the last print setup and print display
                 * connection.
                 */
...
#endif
        {
                ...
                pSetup->dtprintSetup =
                    _psCreatePrintSetupDialog(pSetup->parent, pPad);
                dirname = getenv("HOME");
                if (dirname) {
                        ...
                        XtVaSetValues(pSetup->dtprintSetup, DtNfileName, path,
                                      NULL);
                }
        }
         ...
         */
        XtVaGetValues(pSetup->dtprintSetup, XmNuserData, &pSetup->widgets,
                      NULL);

        /*
         * Add the setup-specific callbacks.
         */
        XtAddCallback(pSetup->dtprintSetup, DtNcancelCallback,
                      _psPrintSetupCancelCB, (XtPointer)pSetup);
        XtAddCallback(pSetup->dtprintSetup, DtNclosePrintDisplayCallback,
                      pSetup->closeDisplayCB,
                      (XtPointer)pSetup->closeDisplayClosure);
        XtAddCallback(pSetup->dtprintSetup, DtNprintCallback,
                      _psPrintSetupPrintCB, (XtPointer)pSetup);
        XtAddCallback(pSetup->dtprintSetup, DtNsetupCallback, pSetup->setupCB,
        ...
        if (NULL != dialog && XtIsShell(dialog)) {
                ...
                title =
                    (char *)XtMalloc(strlen(titleParent) + strlen(titlePrefix) +
                                     strlen(pSetup->docName) + 1);
                sprintf(title, "%s%s%s", titleParent, titlePrefix,
                        pSetup->docName);
...
static Widget _psCreatePrintSetupDialog(Widget parent, Editor *pPad) {
        ...
        XtVaSetValues(widgets->dtprintSetup, DtNworkAreaLocation,
                      DtWORK_AREA_BOTTOM, XmNuserData, widgets, NULL);

        XtAddCallback(widgets->dtprintSetup, XmNhelpCallback,
                      HelpPrintSetupDialogCB, (XtPointer)pPad);

        XtAddCallback(widgets->dtprintSetup, XmNdestroyCallback,
                      _psPrintSetupDestroyCB, (XtPointer)widgets);
        /*
         * Create the dtpad-specific setup dialog in the bottom work area
         */
        widgets->form = XtVaCreateManagedWidget("SetupForm", xmFormWidgetClass,
                                                widgets->dtprintSetup, NULL);

        XtAddCallback(widgets->form, XmNhelpCallback,
                      HelpPrintSetupAppSpecificCB, (XtPointer)pPad);
        ...
        widgets->docLabel = XtVaCreateManagedWidget(
            "DocumentLable", xmLabelGadgetClass, widgets->form,
        ...
        widgets->docNameLabel = XtVaCreateManagedWidget(
            "DocumentName", xmLabelGadgetClass, widgets->form,
        ...
        widgets->wordWrapTB = XtVaCreateManagedWidget(
            "WrapToFit", xmToggleButtonWidgetClass, widgets->form, XmNalignment,
            XmALIGNMENT_BEGINNING, XmNlabelString, xms, XmNleftAttachment,
            XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, widgets->docLabel, NULL);
        XmStringFree(xms);
        ...
        for (i = 0; i < nitems; i++) {
                ...
                strings[i] = GETMESSAGE(_psHdrFtrValues[i].set_id,
    ...
    XtManageChild(widgets->marginFrame);
         ...
         */
        widgets->marginFrame = _DtPrintCreateMarginFrame(widgets->form);
        XtVaSetValues(widgets->marginFrame, XmNtopAttachment, XmATTACH_WIDGET,
                      XmNtopWidget, widgets->wordWrapTB, XmNleftAttachment,
                      XmATTACH_FORM, NULL);
        XtManageChild(widgets->marginFrame);
        ...
        widgets->hdrftrFrame =
            _DtPrintCreateHdrFtrFrame(widgets->form, nitems, strings, data);
        XtVaSetValues(widgets->hdrftrFrame, XmNtopAttachment, XmATTACH_WIDGET,
                      XmNtopWidget, widgets->wordWrapTB, XmNtopWidget,
                      widgets->marginFrame, XmNleftAttachment, XmATTACH_FORM,
                      NULL);
        XtManageChild(widgets->hdrftrFrame);
#endif

        XtAddCallback(widgets->hdrftrFrame, XmNhelpCallback,
                      HelpPrintSetupPageHeadersFootersCB, (XtPointer)pPad);

        XtAddCallback(widgets->marginFrame, XmNhelpCallback,
                      HelpPrintSetupPageMargins, (XtPointer)pPad);
...
static void _psReleasePrintSetupDialog(PrintSetup *pSetup) {
         ...
         */
        XtRemoveCallback(pSetup->dtprintSetup, DtNcancelCallback,
                         _psPrintSetupCancelCB, (XtPointer)pSetup);
        XtRemoveCallback(pSetup->dtprintSetup, DtNclosePrintDisplayCallback,
                         pSetup->closeDisplayCB,
                         (XtPointer)pSetup->closeDisplayClosure);
        XtRemoveCallback(pSetup->dtprintSetup, DtNprintCallback,
                         _psPrintSetupPrintCB, (XtPointer)pSetup);
        XtRemoveCallback(pSetup->dtprintSetup, DtNsetupCallback,
                         pSetup->setupCB, (XtPointer)pSetup->setupClosure);
