...
void LoadFile(Editor *pPad, char *include) {
        ...
        if (localName && *localName) {
                ...
                if (include != (char *)NULL) {
                        errorCode =
                            DtEditorInsertFromFile(pPad->editor, localName);
                } else {
                        pPad->fileStuff.fileExists = True;
                        errorCode = DtEditorSetContentsFromFile(pPad->editor,
                                                                localName);
                }
                ...
                case DtEDITOR_DIRECTORY:
                        Warning(pPad,
                                (char *)GETMESSAGE(
                                    6, 3, "Unable to edit a Directory."),
                                XmDIALOG_ERROR);
                        fileLoadError = True;
                        break;
        ...
        if (fileLoaded) {
                ...
                if (errorCode != DtEDITOR_NO_ERRORS)
                        Warning(pPad,
                                "Internal Error: Unable to NULL Editor Widget.",
                                XmDIALOG_ERROR);
        ...
        for (pPad = pPadList; pPad != (Editor *)NULL; pPad = pPad->pNextPad) {
                ...
                if (pPad->inUse != True || pPad->editor == NULL ||
                    (!DtEditorCheckForUnsavedChanges(pPad->editor)))
                        continue;
                /* -----> Try to cook up a name for a file that doesn't already
                 * exist */
                ...
                if (stat(fileName, &statbuf) == 0) {
                        ...
                        if (stat(fileName, &statbuf) == 0) {
                                fileName = AddPound(fileName);
                                if (stat(fileName, &statbuf) == 0) {
                                        /* Give up. We've tried enough names.
                                         * The user loses. */
                                        continue;
                                }
                        }
