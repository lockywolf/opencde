...
static XtResource resources[] = {
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, statusLine),
        XmRImmediate,
        (XtPointer)False,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, wordWrap),
        XmRImmediate,
        (XtPointer)False,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, overstrike),
        XmRImmediate,
        (XtPointer)False,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, saveOnClose),
        XmRImmediate,
        (XtPointer)False,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, missingFileWarning),
        XmRImmediate,
        (XtPointer)False,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, readOnlyWarning),
        XmRImmediate,
        (XtPointer)True,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, nameChange),
        XmRImmediate,
        (XtPointer)True,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, viewOnly),
        XmRImmediate,
        (XtPointer)False,
    },
    {
        ...
        XmRString,
        sizeof(char *),
        XtOffset(ApplicationDataPtr, workspaceList),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
...

    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, standAlone),
        XmRImmediate,
        (XtPointer)False,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, blocking),
        XmRImmediate,
        (XtPointer)True,
    },
    {
    ...
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffset(ApplicationDataPtr, exitOnLastClose),
        XmRImmediate,
        (XtPointer)False,
    },
};
...
static XrmOptionDescRec option_list[] = {
    ...
    {"-missingFileWarning", "missingFileWarning", XrmoptionNoArg,
     (caddr_t) "TRUE"},
    {"-noReadOnlyWarning", "readOnlyWarning", XrmoptionNoArg,
     (caddr_t) "FALSE"},
...
static String fallbacks[] = {"Dtpad.Print.width:8.5in",
                             "Dtpad.Print.height:11.0in",
                             "Dtpad.Print*renderTable:-adobe-courier-medium-r-"
                             "normal--0-140-0-0-m-0-iso8859-1",
                             "Dtpad.Print*shadowThickness:0", NULL};
...
static Editor *CreateFirstPad(int *pArgc, char **argv);
static void SendMessageToServer(Editor *pPad, int argc, char **argv);
static void InitEditSessionAndFirstPad(Editor *pPad, int argc, char **argv);
static void SetStateFromResources(Editor *pPad);
static Tt_callback_action HostCB(Tt_message m, Tt_pattern p);
static void Usage(char **argv);
static int dtpadXErrorHandler(Display *display, XErrorEvent *xerr);
static int dtpadXIOErrorHandler(Display *display);
...
int main(int argc, char **argv) {
        ...
        if (DtInitialize(pPad->display, topLevelWithWmCommand, pPad->progname,
                ...
                XtError((char *)GETMESSAGE(7, 4,
                                           "Unable to initialize Desktop "
                                           "services ... exiting Text "
                                           "Editor."));
        }
        ...
        if (pPad->xrdb.standAlone) {
                ...
                if (_DtGetLock(pPad->display, DTPAD_CLASS_NAME) == 0) {
                        FileWidgets *pFileWidgets =
                            &pPad->fileStuff.fileWidgets;
                        Boolean validTT = True;
                        Boolean notDone = True;

                        /* we want to get the hostname of where the server is
                         * running. To do that, we need to open a ToolTalk
                         * connection, the use the Desktop request "Get_Sysinfo"
                        ...
                        while (validTT && notDone) {
                                ...
                                /* the first two variable returned by
                                 * Get_Sysinfo is all we need to get the
                                 * hostname.  hostname is the 2nd variable.
                                 * Let's register that we want them
                                 */
                                status = tt_message_arg_add(msg, TT_OUT,
                                                            "string", 0);
                                if (status != TT_OK) {
                                        validTT = False;
                                        break;
                                ...
                                /* block for 10 seconds or until the HostCB is
                                 * called */
                                tttk_block_while(pPad->app_context, &waitCB,
                                                 10000);
                                notDone = False;
                        }

                        /* If we sent a valid ToolTalk message and the Callback
                         * set value to something other then NULL, lets use it
                         * to print out the message.
                         */
                        if (validTT && value != NULL) {
                                tmpStr = (char *)XtMalloc(
                                    strlen(GETMESSAGE(7, 7,
                                                      "ERROR: A dtpad server "
                                                      "on %s\nis already "
                                                      "serving "
                                                      "this display.")) +
                                    strlen(value) + 10);
                                ...
                                sprintf(tmpStr,
                                        GETMESSAGE(7, 7,
                                                   "ERROR: A dtpad server on "
                                                   "%s\nis already serving "
                                                   "this display."),
                                        value);
                                Warning(pPad, tmpStr, XmDIALOG_ERROR);
                        } else
                                /* ToolTalk connection/message was bogus or
                                 * unable to get hostname in Callback.
                                 */
                                Warning(pPad,
                                        GETMESSAGE(7, 8,
                                                   "ERROR: A dtpad server is "
                                                   "already\nserving this "
                                                   "display."),
                                        XmDIALOG_ERROR);
...
static Editor *CreateFirstPad(int *pArgc, char **argv) {
        ...
        topLevelWithWmCommand =
            XtInitialize(argv[0], DTPAD_CLASS_NAME, option_list,
                         XtNumber(option_list), (int *)pArgc, argv);
	...
	if (*pArgc > 2) {
        ...
        ac = 0;
        XtSetArg(al[ac], XmNgeometry, &pPad->geometry);
        ac++;
        ...
        if (!(pPad->display = XtDisplay(topLevelWithWmCommand))) {
                XtError((char *)GETMESSAGE(
                    7, 5, "Can not open display ... exiting Text Editor."));
        }
        ...
        xa_WM_SAVE_YOURSELF =
            XInternAtom(pPad->display, "WM_SAVE_YOURSELF", False);
        XmAddWMProtocolCallback(topLevelWithWmCommand, xa_WM_SAVE_YOURSELF,
                                (XtCallbackProc)SaveSessionCB,
                                (XtPointer)pPadList);

        /* Create the application shell widget and obtain the application
         * context. This is also done in FindOrCreatePad() for each Editor pad
         * instance */
        ac = 0;
        XtSetArg(al[ac], XmNargv, &argv_l);
        ac++;
        XtSetArg(al[ac], XmNargc, &argc_l);
        ac++;
        XtGetValues(topLevelWithWmCommand, al, ac);
        ...
        if (pPad->xrdb.server) {
                ...
                initialServerResources.server =
                    False; /* so window will be mapped */
                initialServerGeometry = pPad->geometry;
                if (pPad->xrdb.workspaceList)
                        pPad->xrdb.workspaceList =
                            strdup(pPad->xrdb.workspaceList);
 ...
 ************************************************************************/
/* ARGSUSED */
void SetupLoadArgs(Tt_message m, Editor *pPad) {
        ...
        tt_message_context_set(m, "WINDOWWORDWRAP",
                               pArgs->wordWrap ? "t" : "f");
        tt_message_context_set(m, "OVERSTRIKE", pArgs->overstrike ? "t" : "f");
        tt_message_context_set(m, "SAVEONCLOSE",
                               pArgs->saveOnClose ? "t" : "f");
        tt_message_context_set(m, "MISSINGFILEWARNING",
                               pArgs->missingFileWarning ? "t" : "f");
 ...
 *
 ************************************************************************/
Tt_message pLoadMsgCB(Tt_message msg, void *clientData, Tttk_op op,
        ...
        if (state == TT_HANDLED) {
                ...
                InitEditSessionAndFirstPad(pStruct->pPad, pStruct->argc,
                                           pStruct->argv);
                return 0;
        } else {
                return 0;
 ...
 *
 ************************************************************************/
Tt_message pStatusCB(Tt_message msg, void *clientdata, Tt_message contract) {
        if (clientdata) {
                /* This is the blocking case, just return 0 (we'll exit in
                 * pLoadMsgCB when we get a Reply from the dtpad server when
                 * it's done handling the request).*/
                return 0;
        } else {
                /* This is the non-blocking case, exit if this is a Status
                 * notice */
 ...
 ************************************************************************/
static void SendMessageToServer(Editor *pPad, int argc, char **argv) {
        StartupStruct *pStruct =
            (StartupStruct *)XtMalloc(sizeof(StartupStruct));
        char *fileName, *pArgs;
        Tt_message msg;
        Tt_status status;
        Tttk_op op;
        ...
        if (status != TT_OK) {
                ...
                InitEditSessionAndFirstPad(pPad, argc,
                                           argv); /* start standAlone  */
                TTwarning(pPad, status,
         ...
         * We'll restore this to true iff the server sends a failure
         * notification.
         */
        pPad->inUse = False;
        ...
        msg = ttmedia_load(0, pLoadMsgCB, NULL, op, "C_STRING", NULL, 0,
                           fileName, NULL, 0);
...
static void InitEditSessionAndFirstPad(Editor *pPad, int argc, char **argv) {
        ...
        xa_WM_DELETE_WINDOW =
            XInternAtom(pPad->display, "WM_DELETE_WINDOW", False);
 ...
 *
 ************************************************************************/
void RealizeNewPad(Editor *pPad) {
        Arg al[5];       /* arg list */
        ...
        ac++;
        XtSetArg(al[ac], XmNgeometry, NULL);
        ac++;
        ...
        XmMainWindowSetAreas(pPad->mainWindow, pPad->menu_bar, (Widget)NULL,
                             (Widget)NULL, (Widget)NULL, pPad->editor);
 ...
 ************************************************************************/
void ManageOldPad(Editor *pPad, Boolean isCurrentlyVisible) {
 ...
 ************************************************************************/
/* ARGSUSED */
void StartDbUpdate(XtPointer clientData) {
 ...
 ************************************************************************/
/* ARGSUSED */
void exitCB(Widget w, XtPointer callData, XtPointer clientData) {
 ...
 *	menu bar.
 ************************************************************************/
static void GetAdjustedResizeHints(Editor *pPad, XSizeHints *pHints) {
        Arg al[2];       /* arg list */
        register int ac; /* arg count */
        Dimension MBheight;
 ...
 ************************************************************************/
void SetAppShellResizeHints(Editor *pPad) {
 ...
 ************************************************************************/
static int SetPadGeometry(Editor *pPad) {
        int geometryBits;
        int geometryX;
        int geometryY;
 ...
 *
 ************************************************************************/
void SetWindowSize(Editor *pPad) {
        Arg al[10];      /* arg list */
        register int ac; /* arg count */
        XSizeHints size_hints;
        XWMHints *wmhints;
        ...
        if (pPad->saveRestore == True) {
                ...
                XtSetArg(al[ac], XmNx, pPad->x);
                ac++;
                XtSetArg(al[ac], XmNy, pPad->y);
                ac++;
                XtSetArg(al[ac], XmNwidth, pPad->width);
                ac++;
                XtSetArg(al[ac], XmNheight, pPad->height);
                ac++;
                XtSetValues(pPad->app_shell, al, ac);
        } else {
                 ...
                 */
                geometryBits =
                    SetPadGeometry(pPad); /* parse geometry str to pixels */
                wm = (WMShellWidget)(pPad->app_shell);
        ...
        XGetWMSizeHints(pPad->display, XtWindow(pPad->app_shell), &size_hints,
                        &supplied_return, XA_WM_NORMAL_HINTS);
        wm->wm.size_hints.flags |=
            PMinSize | PResizeInc | PBaseSize; /* set previously*/
        if (pPad->saveRestore == True) {
        ...
        wmhints =
            XGetWMHints(XtDisplay(pPad->app_shell), XtWindow(pPad->app_shell));
        wmhints->flags |= IconWindowHint;
        if (pPad->iconic)
                wmhints->initial_state = IconicState;
        else
                wmhints->initial_state = NormalState;
        XSetWMHints(XtDisplay(pPad->app_shell), XtWindow(pPad->app_shell),
                    wmhints);
 ...
 ************************************************************************/
static void SetInitialMenuSensitivity(Editor *pPad) {
        ...
        if (pPad->editStuff.widgets.checkSpellingBtn != (Widget)NULL) {
                XtSetSensitive(pPad->editStuff.widgets.checkSpellingBtn,
                               editable);
        }
...
static void SetStateFromResources(Editor *pPad) {
        ...
        if (pPad->xrdb.overstrike ==
            XmToggleButtonGetState(pWidg->overstrikeBtn))
                XmToggleButtonSetState(pWidg->overstrikeBtn,
                                       !pPad->xrdb.overstrike, False);
        XmToggleButtonSetState(pWidg->overstrikeBtn, pPad->xrdb.overstrike,
                               True);

        if (pPad->xrdb.wordWrap == XmToggleButtonGetState(pWidg->wordWrapBtn))
                XmToggleButtonSetState(pWidg->wordWrapBtn, !pPad->xrdb.wordWrap,
                                       False);
        XmToggleButtonSetState(pWidg->wordWrapBtn, pPad->xrdb.wordWrap, True);

        if (pPad->xrdb.statusLine ==
            XmToggleButtonGetState(pWidg->statusLineBtn))
                XmToggleButtonSetState(pWidg->statusLineBtn,
                                       !pPad->xrdb.statusLine, False);
        XmToggleButtonSetState(pWidg->statusLineBtn, pPad->xrdb.statusLine,
                               True);
...
void UnmanageAllDialogs(Editor *pPad) {
        ...
        if (pPad->fileStuff.fileWidgets.select.save_warning != (Widget)NULL &&
            pPad->fileStuff.fileWidgets.select.save_warning->core.managed ==
                True) {
                XtUnmanageChild(
                    pPad->fileStuff.fileWidgets.select.save_warning);
        }
        ...
        if (pPad->fileStuff.fileWidgets.select.file_dialog != (Widget)NULL &&
            pPad->fileStuff.fileWidgets.select.file_dialog->core.managed ==
                True) {
        ...
        if (pPad->fileStuff.fileWidgets.saveAs.saveAs_form != (Widget)NULL &&
            pPad->fileStuff.fileWidgets.saveAs.saveAs_form->core.managed ==
                True) {
         ...
         */
        if (pPad->MainHelp != (Widget)NULL &&
            pPad->MainHelp->core.managed == True) {
 ...
 ************************************************************************/
void RestoreInitialServerResources(Editor *pPad) {
        ...
        if (initialServerResources.workspaceList != (char *)NULL)
                pPad->xrdb.workspaceList =
                    strdup(initialServerResources.workspaceList);
 ...
 *
 *****************************************************************************/
char *_DtpadGetMessage(int set, int number, char *string) {
        char *msg;
        nl_catd catopen();
        char *catgets();
        static int first = 1;
...
static void Usage(char **argv) {
        ...
        char *message_string1 =
            "'%s' is a bad option.\nUsage: %s...\n"
            "   -saveOnClose\n\t"
            "Dtpad automatically and silently saves the current text when "
            "there\n\t"
        ...
        char *message_string2 =
            "\tRestores the Text Editor to all text editing windows and\n\t"
            ...
            "See the dtpad man page for more details on any of these "
            "options.\n\n";
...
#define _DTPAD_BUFSIZE 1024
        ...
        if (PrintJobIsActivePrintDisplay(display) &&
            xerr->error_code == BadAlloc) {
