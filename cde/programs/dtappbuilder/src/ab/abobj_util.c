 ...
 * UI object hierarchy
 */
int abobj_instantiate_tree(ABObj root, BOOL manage_last) {
        int iRet = 0;
 ...
 */
int abobj_show_tree(ABObj root, BOOL manage_last) {
        int iRet = 0;
 ...
 */
int abobj_hide_tree(ABObj root) {
        int iRet = 0;
 ...
 */
int abobj_get_actual_width(ABObj obj) {
        Dimension width = 0;
        Widget widget;
 ...
 */
int abobj_get_actual_height(ABObj obj) {
        Dimension height = 0;
        Widget widget;
 ...
 */
int abobj_get_comp_width(ABObj obj) {
        ABObj szObj;
        int width;
 ...
 */
int abobj_get_comp_height(ABObj obj) {
        ABObj szObj;
        int height;
 ...
 * to the widget.
 */
int abobj_get_comp_x(ABObj obj) {
        int x;
        ABObj xyObj;
 ...
 * can be -1 for immovable objects.
 */
int abobj_get_actual_x(ABObj obj) {
        Position w_x = 0;
        Widget widget;
 ...
 * to the widget.
 */
int abobj_get_comp_y(ABObj obj) {
        int y;
        ABObj xyObj;
        ABObj pobj = obj_get_root(obj_get_parent(obj));
...
int abobj_get_actual_y(ABObj obj) {
        Position w_y = 0;
        Widget widget;
 ...
 */
int abobj_get_label_width(ABObj obj) {
        Dimension width;
        ABObj l_obj;
        ...
        if (obj_is_group(obj)) {
                ...
                if (l_obj != NULL)
                        XtVaGetValues(objxm_get_widget(l_obj), XmNwidth, &width,
                                      NULL);
                else /* No label */
                        width = 0;
        } else
                XtVaGetValues(objxm_get_widget(obj), XmNwidth, &width, NULL);

        return ((int)width);
}

void abobj_get_rect_for_objects(ABObj *list, int count, XRectangle *j_rect) {
        XRectangle r1;
        int i;
 ...
 */
void abobj_get_greatest_size(ABObj *list, int count, int *width, int *height,
                             ABObj *tallest_obj, ABObj *widest_obj) {
        XRectangle w_rect;
        int i;
...
int abobj_get_value_x(ABObj obj) {
        ABObj o_obj;
        ...
        if (o_obj != obj) {
                XtVaGetValues(objxm_get_widget(obj), XmNx, &x, NULL);
                XtVaGetValues(objxm_get_widget(o_obj), XmNx, &valuex, NULL);
                x += valuex;
        } else {
                XtVaGetValues(objxm_get_widget(obj), XmNx, &x, XmNwidth, &width,
                              NULL);
                x += width;
        }
...
BOOL abobj_has_attached_label(ABObj obj) {
        BOOL has_label;
 ...
 */
void abobj_force_dang_form_resize(ABObj obj) {
        /*
        REMIND: aim,1/12/95 - I believe this is no longer necessary, however
            ...
            {
                XtVaGetValues(parent, XmNwidth, &p_width, XmNheight, &p_height,
        NULL); XtVaSetValues(parent, XmNwidth, p_width+1,XmNheight,
        p_height+1,NULL); XtVaSetValues(parent, XmNwidth, p_width,  XmNheight,
        p_height,  NULL);
            }
        */
}
 ...
 */
STRING
abobj_alloc_moduled_name(STRING mod_name, STRING obj_name) {
 ...
 */
STRING
abobj_get_moduled_name(ABObj obj) {
        ABObj module;
        STRING obj_name;
        STRING mod_name;
        STRING moduled_name;
 ...
 */
int abobj_moduled_name_extract(STRING m_o_name, ABObj *module, ABObj *obj) {
        ...
        if (modobjname == NULL) {
                if (util_get_verbosity() > 0)
                        fprintf(stderr, "abobj_moduled_name_extract: NULL "
                                        "module-object name passed\n");
                iRet = -1;
        } else {
                ...
                if (modname != NULL && objname != NULL) {
                        if ((*module = obj_find_module_by_name(
                                 proj_get_project(), modname)) != NULL)
                                *obj =
                                    obj_scoped_find_by_name(*module, objname);
                        else {
                                if (util_get_verbosity() > 0)
                                        fprintf(stderr,
                        ...
                        if (util_get_verbosity() > 0)
                                fprintf(stderr, "abobj_moduled_name_extract: "
                                                "couldn't extract names for "
                                                "module or object\n");
                        iRet = -1;
                }
        }
 ...
 */
int abobj_comp_rename(ABObj root, STRING oldname, STRING newname) {
        AB_TRAVERSAL trav;
        ABObj subobj;
        STRING old_subname;
        STRING new_subname;
             ...
             (subobj = trav_next(&trav)) != NULL;) {
                if (subobj != root) {
                        ...
                        if (sfx_index < strlen(old_subname)) {
                                ...
                                new_subname =
                                    ab_ident_from_name_and_label(newname, sfx);
#ifdef DEBUG
                                util_dprintf(
                                    5, "changing subobj \"%s\" to \"%s\"\n",
...
int abobj_update_module_name(ABObj modobj) {
        int num_win, i;
        ...
        for (i = 0; i < num_win; i++) {
                ...
                if (statobj && objxm_get_widget(statobj) != NULL) {
                        ...
                        XtVaSetValues(objxm_get_widget(statobj), XmNlabelString,
                                      xmname, NULL);
                        XmStringFree(xmname);
                }
        }
...
int abobj_update_proj_name(ABObj proj_obj) {
        ...
        if (!util_strempty(proj_name)) {
                proj_win_title =
                    (STRING)util_malloc(strlen(proj_name) + strlen(".bip") + 1);
...
int abobj_update_palette_title(ABObj proj_obj) {
        STRING new_title = NULL;
        STRING proj_win_title = NULL;
        STRING proj_name = NULL;
        AB_TRAVERSAL trav;
        ...
        new_title = (STRING)util_malloc(
            ...
            +strlen(catgets(Dtb_project_catd, 100, 244, " (Save Needed)")) + 1);
...
ABObj abobj_dup(ABObj obj) {
        ...
        obj_tree_clear_flag(newObj, AttrChangedFlag | XmConfiguredFlag |
                                        InstantiatedFlag | MappedFlag |
                                        BuildActionsFlag);
 ...
 * somewhat ready for pasting.
 */
ABObj abobj_dup_tree(ABObj obj) {
        AB_TRAVERSAL trav;
        ABObj newObj = NULL, child, newChild;
 ...
 */
int abobj_init_pane_position(ABObj paneobj) {
        Dimension p_width, p_height;
        ABObj parent = obj_get_parent(paneobj);

        if (obj_is_layers(obj_get_root(parent))) {
                obj_set_attachment(paneobj, AB_CP_NORTH, AB_ATTACH_GRIDLINE, 0,
                                   0);
                obj_set_attachment(paneobj, AB_CP_WEST, AB_ATTACH_GRIDLINE, 0,
                                   0);
                obj_set_attachment(paneobj, AB_CP_EAST, AB_ATTACH_GRIDLINE,
                                   (void *)100, 0);
                obj_set_attachment(paneobj, AB_CP_SOUTH, AB_ATTACH_GRIDLINE,
                                   (void *)100, 0);
        } else {
                /* Set up Default Layout for Control Panel */
                XtVaGetValues(objxm_get_widget(parent), XmNwidth, &p_width,
                              XmNheight, &p_height, NULL);

                /* Snap x,y to edge of parent if already close */
                if (paneobj->x <= AB_grid_size)
                        paneobj->x = 0;
                else if ((paneobj->x + paneobj->width) >=
                         ((int)p_width - AB_grid_size))
                        paneobj->x = (int)p_width - (paneobj->width + 1);
                if (paneobj->y <= AB_grid_size)
                        paneobj->y = 0;
                else if ((paneobj->y + paneobj->height) >=
                         ((int)p_height - AB_grid_size))
                        paneobj->y = (int)p_height - (paneobj->height + 1);

                obj_set_attachment(paneobj, AB_CP_NORTH, AB_ATTACH_POINT, NULL,
                                   paneobj->y);
                obj_set_attachment(paneobj, AB_CP_WEST, AB_ATTACH_POINT, NULL,
                                   paneobj->x);
        }
...
static int reset_widget_colors(Widget widget, BOOL reset_bg, BOOL reset_fg) {
        static Widget dummy_shell = NULL;
        Widget dummy, dummy_parent;
        Pixel default_bg, default_fg;
        Arg args[2];
        int n = 0;
        ...
        if (XtIsShell(dummy_parent)) /* shell can only have 1 child */
        {
                 ...
                 */
                if (dummy_shell == NULL)
                        dummy_shell = XtCreatePopupShell(
                            "dummy_shell", transientShellWidgetClass,
                            AB_toplevel, NULL, 0);
         ...
         */
        dummy = XtCreateWidget("dummy", XtClass(widget), dummy_parent, NULL, 0);
        XtVaGetValues(dummy, XmNbackground, &default_bg, XmNforeground,
                      &default_fg, NULL);
        if (reset_bg) {
                XtSetArg(args[n], XmNbackground, default_bg);
                n++;
        }
        if (reset_fg) {
                XtSetArg(args[n], XmNforeground, default_fg);
                n++;
        }
...
int abobj_reset_colors(ABObj obj, BOOL reset_bg, BOOL reset_fg) {
        AB_TRAVERSAL trav;
        ABObj subobj;
        Widget widget;
 ...
 */
int abobj_tree_set_centering_handler(ABObj root) {
        AB_TRAVERSAL trav;
        ABObj obj;
...
DTB_CENTERING_TYPES
abobj_get_centering_type(ABObj obj) {
        AB_ATTACH_TYPE left_attach_type, top_attach_type;
 ...
 *       by this function
 */
int abobj_build_menus_array(ABObj root, ABObj **menus_ptr,
                            int *menu_count_ptr) {
        AB_TRAVERSAL trav;
        ABObj modules[256];
        int num_modules = 0;
        ...
        *menu_count_ptr = 0;
        ...
        if (*menu_count_ptr > 0) {
                ...
                *menus_ptr =
                    (ABObj *)util_malloc(*menu_count_ptr * sizeof(ABObj));
                k = 0;
 ...
 */
int abobj_project_name_extract(STRING app_name, ABObj *obj) {
        ...
        if (appobjname == NULL) {
                if (util_get_verbosity() > 0)
                        fprintf(stderr, "abobj_project_name_extract: NULL "
                                        "project-object name passed\n");
                iRet = -1;
        } else {
                ...
                if (objname != NULL) {
                        if (strcmp(obj_get_name(proj_get_project()), objname) ==
                            0)
