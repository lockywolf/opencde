...
**                                                                      **
**************************************************************************/
static void box_gc_init(Widget w, Display *display, Drawable drawable);
static void fullscreen_gc_init(Widget w, Display *display, Drawable drawable);
static Bool expose_count_predicate(Display *display, XEvent *event,
                                   XPointer arg);
 ...
 */
int x_load_cursor_bitmaps(Widget widget) {

        if (!XtIsRealized(widget)) {
                if (util_get_verbosity() > 0)
                        fprintf(
                            stderr,
                            "x_load_cursor_bitmaps: widget must be realized\n");
                return ERROR;
        }

        AB_cp_cursor_height = copy_cursor_bm_height;
        AB_cp_cursor_width = copy_cursor_bm_width;
        AB_cp_cursor_pixmap = XCreatePixmapFromBitmapData(
            XtDisplay(widget), XtWindow(widget), (char *)copy_cursor_bm_bits,
            AB_cp_cursor_width, AB_cp_cursor_height,
            BlackPixelOfScreen(XtScreen(widget)),
            WhitePixelOfScreen(XtScreen(widget)), 1);

        AB_mv_cursor_height = move_cursor_bm_height;
        AB_mv_cursor_width = move_cursor_bm_width;
        AB_mv_cursor_pixmap = XCreatePixmapFromBitmapData(
            XtDisplay(widget), XtWindow(widget), (char *)move_cursor_bm_bits,
            AB_mv_cursor_width, AB_mv_cursor_height,
            BlackPixelOfScreen(XtScreen(widget)),
            WhitePixelOfScreen(XtScreen(widget)), 1);
 ...
 * a stencil around it
 */
Cursor x_create_stencil_cursor(Widget widget, Pixmap pixmap,
        ...
        if (!XtIsRealized(widget)) {
                if (util_get_verbosity() > 0)
                        fprintf(stderr, "x_create_stencil_cursor: widget must "
                                        "be realized\n");
                return 0;
        }
        ...
        if (!c_gc) {
                ...
                values.foreground = 1;
                values.background = 0;
                c_gc = XCreateGC(dpy, cursor_pixmap,
        ...
        XCopyArea(dpy, pixmap, cursor_pixmap, c_gc, 0, 0, c_width, c_height, 1,
                  1);
        ...
        for (i = -1; i <= 1; i++) {
                for (j = -1; j <= 1; j++) {
                        x_graphics_op(dpy, mask_pixmap, c_gc, i, j, c_width,
                                      c_height, cursor_pixmap);
                }
        }

        cursor = XCreatePixmapCursor(dpy, cursor_pixmap, mask_pixmap, &fg_color,
                                     &bg_color, c_xhot, c_yhot);
...
void x_graphics_op(Display *dpy, Drawable dest, GC gc, int x, int y, int width,
                   int height, Pixmap src) {
        XGCValues orig, changes;
        unsigned long changes_mask =
...
void x_get_widget_rect(Widget widget, XRectangle *rect) {
 ...
 */
void x_get_rect_bounding(XRectangle *r1, XRectangle *r2) {
        XRectangle r;
        ...
        if (rect_isnull(r1))
                ...
                r.y = min(r1->y, r2->y);
                r.width =
                    max(r1->x + (short)r1->width, r2->x + (short)r2->width) -
                    r.x;
                r.height =
                    max(r1->y + (short)r1->height, r2->y + (short)r2->height) -
                    r.y;
        }
        *r1 = r;
}

void x_adjust_rect_margin(XRectangle *r, int m) {
        (r->x) -= m;
        (r->y) -= m;
        (r->width) += (m + m);
        (r->height) += (m + m);
 ...
 *  x_box
 */
void x_box(Widget widget, Drawable drawable, int x0, int y0, int x1, int y1) {
        Display *display;
        int tmp;
 ...
 *  x_box_r
 */
void x_box_r(Widget widget, XRectangle *r) {
        x_box(widget, XtWindow(widget), r->x, r->y, rect_right(r),
              rect_bottom(r));
}
 ...
 * x_dashed_box_r
 */
void x_dashed_box_r(Widget widget, Drawable drawable, XRectangle *r) {
        Display *display;
        ...
        XSetLineAttributes(display, box_gc, 0, LineOnOffDash, CapButt,
                           JoinMiter);

        XDrawRectangle(display, drawable, box_gc, r->x, r->y, r->width - 1,
                       r->height - 1);

        /* Restore back to solid lines */
        XSetLineAttributes(display, box_gc, 0, LineSolid, CapButt, JoinMiter);
}

void x_fullscreen_box(Widget w, Drawable rootwin, int x0, int y0, int x1,
                      int y1) {
        Display *display;
        int tmp;
        ...
        XDrawRectangle(display, rootwin, fullscreen_gc, x0, y0, x1 - x0,
                       y1 - y0);
}

void x_fullscreen_preview_box(Widget w, Drawable rootwin, int x0, int y0,
                              int x1, int y1) {
        Display *display;
        int tmp;
        ...
        XDrawRectangle(display, rootwin, fullscreen_gc, x0 - 1, y0 - 1,
                       x1 - x0 + 2, y1 - y0 + 2);
}
 ...
 */
unsigned int x_contrast_color(Widget widget) {
        XrmValue src, dst;
        float intensity;
        ...
        intensity = ((float)bg_color.red * 0.3 + (float)bg_color.green * 0.59 +
                     (float)bg_color.blue * 0.12) /
                    255.0;
        /*
        fprintf(stderr,"x_contrast_color: %s : red(%d) green(%d) blue(%d) =
        intensity(%f)\n", XtName(widget),bg_color.red, bg_color.green,
        bg_color.blue, intensity);
        */
...
Boolean x_widget_translate_xy(Widget src, XtPointer dst, int src_x, int src_y,
                              int *p_dst_x, int *p_dst_y) {
        Window src_win = XtWindow(src);
        Window dst_win =
            dst ? XtWindow((Widget)dst) : RootWindowOfScreen(XtScreen(src));
        Window child;

        return (XTranslateCoordinates(XtDisplay(src), src_win, dst_win, src_x,
                                      src_y, p_dst_x, p_dst_y, &child));
}

Boolean x_rootxy_inside_widget(Widget w, int rootx, int rooty) {
        ...
        if (XtIsRealized(w)) {
                ...
                XTranslateCoordinates(XtDisplay(w), XtWindow(w), root_win, 0, 0,
                                      &w_x, &w_y, &win_below);
 ...
 * box_gc_init
 */
static void box_gc_init(Widget widget, Display *display, Drawable drawable) {
        XGCValues gc_val;
 ...
 * fullscreen_gc_init
 */
static void fullscreen_gc_init(Widget widget, Display *display,
                               Drawable drawable) {
        XGCValues gc_val;
        unsigned long cc_pixel = x_contrast_color(widget);
        unsigned long bg_pixel;
 ...
 * on the RootWindow
 */
Window x_xwin_at_rootxy(Widget widget, int x, int y, int *dstx, int *dsty) {
        Display *dpy;
        Window child;
        ...
        if (XTranslateCoordinates(dpy, rootwin, rootwin, x, y, dstx, dsty,
                                  &child) == 0)
                return ((Window)0);
        ...
        for (;;) {
                if (XTranslateCoordinates(dpy, srcwin, dstwin, x, y, dstx, dsty,
                                          &child) == 0)
                        return ((Window)0);
...
void x_conn_fullscreen_init(Widget w, Drawable root_win) {
        Display *display;
        ...
        XSetLineAttributes(display, fullscreen_gc, 2, LineSolid, CapButt,
                           JoinMiter);
}
 ...
 */
void x_conn_fullscreen_chord(Widget w, Drawable rootwin, int x0, int y0, int x1,
                             int y1) {
        Display *display;
        int tmp;
...
void x_conn_fullscreen_cleanup(Widget w) {
        /* Restore back to solid lines */
        XSetLineAttributes(XtDisplay(w), fullscreen_gc, 0, LineSolid, CapButt,
                           JoinMiter);
}

Cursor x_create_cursor(Widget widget, Window root_win, unsigned char bits[],
                       unsigned short width, unsigned short height,
                       unsigned int xhot, unsigned int yhot) {
        Pixmap curs_pix;
        Cursor cursor;
        ...
        curs_pix = XCreateBitmapFromData(XtDisplay(widget), root_win,
 ...
 */
int x_get_num_pending_expose_events(Display *display) {
        int num_expose_events = 0;
        XEvent event;

        XCheckIfEvent(display, &event, expose_count_predicate,
                      (XPointer)&num_expose_events);
