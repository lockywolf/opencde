 ...
 * Methods
 */
static int textf_initialize(ABObj obj);
static Widget textf_prop_init(Widget parent, AB_PROP_TYPE type);
static int textf_prop_activate(AB_PROP_TYPE type, BOOL active);
static int textf_prop_clear(AB_PROP_TYPE type);
static int textf_prop_load(ABObj obj, AB_PROP_TYPE type, unsigned long loadkey);
static int textf_prop_apply(AB_PROP_TYPE type);
static BOOL textf_prop_pending(AB_PROP_TYPE type);
...
static Widget textf_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbTextfPropDialogInfo cgen =
            &dtb_textf_prop_dialog; /* Codegen structure */
        ...
        if (dtb_textf_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_REVOLVING)
                        XtVaSetValues(parent, XmNuserData, pts->current_obj,
                                      NULL);
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_textf_palitem, cgen->ok_button,
                ...
                n++;
                prop_options_init(
                    &(pts->label_type), cgen->labeltype_rbox_label,
                ...
                prop_label_field_init(&(pts->label), cgen->graphic_hint, item,
                                      n);
                ...
                /* Menu Name */
                /*
                        prop_menuname_init(&(pts->menuname), cgen->menu_mbutton,
                   cgen->menu_field, cgen->name_field, cgen->menu_cb,
                ...
                */
                ...
                prop_geomfield_init(&(pts->size), cgen->width_opmenu_label,
                                    NULL, NULL, NULL, NULL, cgen->w_field_label,
                                    cgen->w_field, NULL, NULL, cgen->width_cb);
                ...
                prop_options_init(&(pts->size_metric), cgen->width_opmenu_label,
                                  cgen->width_opmenu, cgen->width_opmenu_menu,
                                  n, item, (XtPointer *)item_val,
                                  cgen->width_cb);
...
static int textf_prop_clear(AB_PROP_TYPE type) {
        ...
        prop_options_set_value(&(pts->label_type), (XtPointer)AB_LABEL_STRING,
                               False);
        ...
        */
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pts->init_state), AB_STATE_VISIBLE, True,
                                False);
        prop_checkbox_set_value(&(pts->init_state), AB_STATE_ACTIVE, True,
                                False);
...
static int textf_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_options_set_value(&(pts->label_type),
                                       (XtPointer)obj->label_type, False);
                prop_options_set_value(&(pts->label_pos),
                                       (XtPointer)obj_get_label_position(obj),
                                       False);

                /* Load Label */
                prop_setup_label_field(&(pts->label), NULL, obj->label_type,
                                       obj_get_label(obj), AB_LINE_UNDEF);
                /* Load Operation */
                prop_radiobox_set_value(
                    &(pts->op), (XtPointer)obj_get_read_only(obj), False);

                /* Load Maximum Chars */
                prop_field_set_numeric_value(&(pts->max_chars),
                                             obj_get_max_length(obj), False);

                /* Load Initial Value */
                prop_field_set_value(&(pts->init_value),
                                     obj_get_initial_value_string(obj), False);
        ...
        if (load_all || loadkey & LoadSize) {
                ...
                {
                        prop_options_set_value(&(pts->size_metric),
                ...
                prop_geomfield_set_value(&(pts->size), GEOM_WIDTH, width,
                                         False);
        }
        ...
        if (prop_changed(pts->label.changebar) ||
            prop_changed(pts->label_type.changebar)) {
                ...
                abobj_set_label_position(
                    pts->current_obj, (AB_COMPASS_POINT)prop_options_get_value(
                                          &(pts->label_pos)));
                size_chg = True;
        }
        /*
        ...
        */
        ...
        if (prop_changed(pts->size.changebar)) {
                ...
                abobj_set_pixel_width(pts->current_obj,
                                      metric == SIZE_IN_PIXELS_KEY ? new_w : -1,
                                      0);
        ...
        if (prop_changed(pts->init_state.changebar)) {
                abobj_set_visible(pts->current_obj,
                                  prop_checkbox_get_value(&(pts->init_state),
                                                          AB_STATE_VISIBLE));
        ...
        if (prop_changed(pts->fg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pts->fg_color),
                                          obj_get_fg_color(pts->current_obj),
                                          False);
        }
        if (prop_changed(pts->bg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pts->bg_color),
                                          obj_get_bg_color(pts->current_obj),
                                          False);
        }
...
static BOOL textf_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_textf_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if ((prop_changed(pts->label_type.changebar) ||
             prop_changed(pts->label.changebar)) &&
            prop_options_get_value(&(pts->label_type)) ==
                (XtPointer)AB_LABEL_GLYPH &&
            !prop_graphic_filename_ok(pts->label.field, False))
                return False;

        if (prop_changed(pts->pos.changebar) &&
            (!prop_number_ok(pts->pos.x_field, (STRING)XFieldStr, -SHRT_MAX,
                             SHRT_MAX) ||
             !prop_number_ok(pts->pos.y_field, (STRING)YFieldStr, -SHRT_MAX,
                             SHRT_MAX)))
                return False;
        ...
        if (prop_changed(pts->fg_color.changebar) &&
            !prop_color_ok(pts->fg_color.field))
                return False;

        if (prop_changed(pts->bg_color.changebar) &&
            !prop_color_ok(pts->bg_color.field))
                return False;

        /*
            if (prop_changed(pts->menuname.changebar) &&
           !prop_obj_name_ok(pts->menuname.field,
                                obj_get_module(pts->current_obj), AB_TYPE_MENU,
           "Menu")) return False;
        */
