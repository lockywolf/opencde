...
static int format_dir_name_for_user(STRING ugly_dir, STRING pretty_dir,
                                    int pretty_dir_size);

static int format_dir_name_for_system(STRING user_dir_name, STRING system_dir,
                                      int system_dir_size);

static void set_cursor(Widget widget, Cursor cursor, BOOL on);

static void track_win_iconify(Widget widget, XtPointer client_data,
                              XEvent *event, Boolean *cont_disp);
static void track_win_leader_iconify(Widget widget, XtPointer client_data,
                                     XEvent *event, Boolean *cont_disp);
static void popdown_winCB(Widget widget, XtPointer client_data,
                          XtPointer call_data);
static void position_winCB(Widget widget, XtPointer client_data,
                           XtPointer call_data);
static void track_win_popupCB(Widget widget, XtPointer client_data,
                              XtPointer call_data);
static void track_win_popdownCB(Widget widget, XtPointer client_data,
                                XtPointer call_data);
static void track_win_destroyCB(Widget widget, XtPointer client_data,
                                XtPointer call_data);

static void set_windows_visibility(BOOL show);
static void set_window_objects_visibility(BOOL show);

static void exit_mboxCB(Widget widget, XtPointer client_data,
                        XtPointer call_data);

static void destroy_mboxCB(Widget widget, XtPointer client_data,
                           XtPointer call_data);

static void mwm_close_mboxCB(Widget widget, XtPointer client_data,
                             XtPointer call_data);
...
void ab_position_window(Widget widget, Widget ref_widget,
        ...
        if (widget == NULL || ref_widget == NULL || !XtIsRealized(ref_widget))
                return;
        ...
        XtVaGetValues(shell, XmNwidth, &shell_w, XmNheight, &shell_h, NULL);

        ref_shell = ui_get_ancestor_shell(ref_widget);
        XtVaGetValues(ref_shell, XmNx, &ref_x, XmNy, &ref_y, XmNwidth, &ref_w,
                      XmNheight, &ref_h, NULL);
        ...
        if ((int)shell_w < AB_min_dimension || (int)shell_h < AB_min_dimension)
                util_dprintf(
                    1, "ab_position_window: %s - %dx%d - size under minimum\n",
        ...
        XtVaSetValues(shell, XmNx, shell_x, XmNy, shell_y, NULL);
}

BOOL ab_window_leader_iconified() {
        return (AB_win_leader->state & WindowIconified);
}

void ab_register_window(Widget widget, AB_WIN_TYPE type,
                        unsigned long init_state, Widget init_pos_ref_widget,
        ...
        ui_add_window_close_callback(
            shell, close_callback ? close_callback : popdown_winCB,
            close_callback ? close_clientdata : NULL, XmDO_NOTHING);
        ...
        if (type == AB_WIN_LEADER) {
                ...
                if (AB_win_leader == NULL) {
                        ...
                        ui_add_window_iconify_handler(
                            shell, track_win_leader_iconify, (XtPointer)newwin);
                        return;
                } else
                        util_dprintf(1, "ab_register_window: can only have 1 "
                                        "Window Leader\n");
        }
        ...
        if (type == AB_WIN_WINDOW)
                ui_add_window_iconify_handler(shell, track_win_iconify,
                                              (XtPointer)newwin);
        ...
        /*
         * Set up general window resources to get popup/popdown to work
         * correctly We will not be using Motif's method of Managing the first
         * child of the shell to get the window to popup!
         */
...
void ab_show_window(Widget widget) {
        ABWindow *win;
        ...
        for (win = AB_win_head; win != NULL; win = win->next) {
                if (win->widget == widget) {
                        if (win->state & WindowUp) {
                                if (win->state & WindowIconified &&
                                    !(AB_win_leader->state & WindowIconified)) {
                                        /* Window is iconified, unmap it so we
                                         * can change its initial state and
                                         * remap it as Opened
                                         */
                        ...
                        XtSetMappedWhenManaged(
                            ui_get_ancestor_shell(win->widget), True);
                        switch (win->type) {
                        case AB_WIN_MODAL:
...
BOOL ab_window_is_open(Widget widget) {
        ABWindow *win;
...
static void track_win_iconify(Widget widget, XtPointer client_data,
        ...
        switch (event->type) {
        case MapNotify:
                if (InBuildMode && win->state & WindowIconified &&
                    !(AB_win_leader->state & WindowIconified)) {
                        win->state &= (~WindowIconified);
        ...
        case UnmapNotify:
                 ...
                 */
                if (InBuildMode && win->state & WindowUp &&
                    !(win->state & WindowIconified) &&
                    !(AB_win_leader->state & WindowIconified)) {
                        win->state |= WindowIconified;
...
static void popdown_winCB(Widget shell, XtPointer client_data,
                          XtPointer call_data) {
        ui_win_show(shell, False, XtGrabNone);
}

static void position_winCB(Widget shell, XtPointer client_data,
                           XtPointer call_data) {
...
static void track_win_popupCB(Widget shell, XtPointer client_data,
                              XtPointer call_data) {
        ABWindow *win = (ABWindow *)client_data;
        win->state |= WindowUp;
}

static void track_win_popdownCB(Widget shell, XtPointer client_data,
                                XtPointer call_data) {
        ABWindow *win = (ABWindow *)client_data;

        if (InBuildMode && !(AB_win_leader->state & WindowIconified)) {
                win->state &= (~WindowUp);
                if (win->state & WindowIconified) {
                        XtVaSetValues(shell, XtNinitialState, NormalState,
                                      NULL);
                        win->state &= (~WindowIconified);
                }
        }
...
static void track_win_destroyCB(Widget shell, XtPointer client_data,
                                XtPointer call_data) {
        ABWindow *win = (ABWindow *)client_data;
        ABWindow *prev = NULL;
        ABWindow *p;
...
void ab_takedown_windows() { set_windows_visibility(False); }

void ab_putback_windows() { set_windows_visibility(True); }
...
static void set_window_objects_visibility(BOOL show) {
        ...
        for (trav_open(&trav, project, AB_TRAV_WINDOWS);
             (winobj = trav_next(&trav)) != NULL;) {
                if (obj_is_base_win(winobj) && obj_has_flag(winobj, MappedFlag))
                        ui_win_show(objxm_get_widget(winobj), show, XtGrabNone);
        }
        trav_close(&trav);
}

int ab_update_stat_region(AB_STATUS_INFO type, String value) {
        ...
        case AB_STATUS_OBJ_TYPE:
                if (obj_type == NULL)
                        obj_type =
                            dtb_palette_ab_palette_main.object_type_field;
                widget = obj_type;
                break;
        case AB_STATUS_OBJ_NAME:
                if (obj_name == NULL)
                        obj_name =
                            dtb_palette_ab_palette_main.object_name_field;
                widget = obj_name;
                break;
        ...
        case AB_STATUS_CURS_POS:
                if (curs_pos == NULL)
                        curs_pos =
                            dtb_palette_ab_palette_main.cursor_position_field;
                widget = curs_pos;
                break;
        case AB_STATUS_CUR_MODULE:
                if (cur_module == NULL)
                        cur_module =
                            dtb_palette_ab_palette_main.editing_module_field;
        ...
        if (widget != NULL)
                XtVaSetValues(widget, XtVaTypedArg, XmNlabelString, XtRString,
 ...
 */
void ab_set_busy_cursor(BOOL on) {
        AB_TRAVERSAL trav;
        ABObj project = proj_get_project();
        ABWindow *win;
        ABObj module;
        ...
        if (busy_cursor == NULL)
                busy_cursor =
                    XCreateFontCursor(XtDisplay(AB_toplevel), XC_watch);
             ...
             (module = trav_next(&trav)) != NULL;) {
                ...
                for (i = 0; i < num_wins; i++) {
                        ...
                        if (obj_is_window(winobj) && winobj->ui_handle != NULL)
                                set_cursor((Widget)winobj->ui_handle,
                                           new_cursor, on);
                }
        }
        trav_close(&trav);
 ...
 */
int ab_cvt_image_file_to_pixmap(Widget widget, STRING file_name,
                                Pixmap *pixmap_out) {
 ...
 */
int ab_post_instantiate(ABObj obj) {
        if (obj && obj_is_group(obj) &&
            (obj_get_group_type(obj) != AB_GROUP_IGNORE)) {
                 ...
                 * layout. Unfortunately, some group members have invalid sizes
                 * prior to XtRealize(), so the group layout has to be
                 * recalculated after the group is realized or exposed in this
                 * case, since there is no realize callback.
                 */
                if (group_w) {
                        ABObj parent = NULL;
                        ...
                        if (parent_w) {
                                /*
                                 * If parent widget is not realized, register
                                 * expose handler
                                 */
 ...
 */
int ab_change_dir(STRING request_dir) {
        ...
        *pretty_dir = 0;
        ...
        if (request_dir == NULL) {
                 ...
                 * this.
                 */
                if (format_dir_name_for_user(request_dir, pretty_dir,
                                             MAXPATHLEN) >= 0) {
        ...
        if (dir_changed) {
                {
                        ...
                        if (format_dir_name_for_system(request_dir, sys_dir,
                                                       MAXPATHLEN + 1) >= 0) {
                                /*
                                 * We may have changed paths but still be in the
                                 * same actual directory. Only chdir if we've
                                 * actually moved.
                                 */
                ...
                {
                        format_dir_name_for_user(request_dir, pretty_dir,
                                                 MAXPATHLEN);
                }
...
ab_get_cur_dir(void) {
        ...
        *cur_dir = 0;
        ...
        if (initialized && (raw_current_dir != NULL) &&
        ...
        if (new_raw_dir == NULL) {
                ...
                format_dir_name_for_user(new_raw_dir, formatted_cur_dir,
                                         MAXPATHLEN);
...
static int format_dir_name_for_user(STRING ugly_dir, STRING pretty_dir,
        ...
        if (home != NULL) {
                ...
                *home_relative = 0;
                ...
                util_cvt_path_to_relative(ugly_dir, home, home_relative,
                                          MAXPATHLEN);
                *pretty_dir = 0;
                if (home_relative[0] == '/') {
                        ...
                        /* relative to home (but avoid "~/." if actually in home
                         * dir)*/
                        strcpy(pretty_dir, "~");
                        if (!util_streq(home_relative, ".")) {
                                strcat(pretty_dir, "/");
                                strncat(pretty_dir, home_relative,
                                        pretty_dir_size - 2);
                        }
                        pretty_dir[pretty_dir_size - 1] = 0;
                }
...
void ab_palette_set_active(BOOL active) {
                ...
                if (proj_check_unsaved_edits(proj_get_project())) {
                        ...
                        if (ab_window_leader_iconified()) {
                                shell = XtCreatePopupShell(
                                    "exit_dtbuilder", topLevelShellWidgetClass,
                                    dtb_get_toplevel_widget(), (ArgList)NULL,
                                    (Cardinal)0);

                                n = 0;
                                XtSetArg(args[n], XmNdialogType,
                                         dtb_palette_exit_msg.type);
                                n++;
                                XtSetArg(args[n], XmNmessageString,
                                         dtb_palette_exit_msg.message);
                                n++;
                                XtSetArg(args[n], XmNdialogTitle,
                                         dtb_palette_exit_msg.title);
                                n++;
                                XtSetArg(args[n], XmNokLabelString,
                                         dtb_palette_exit_msg.action1_label);
                                n++;
                                XtSetArg(args[n], XmNdefaultButtonType,
                                         XmDIALOG_CANCEL_BUTTON);
                                n++;
                                XtSetArg(args[n], XmNuserData, &answer);
                                n++;
                                mbox = XmCreateMessageBox(shell, "exit_mbox",
                                                          args, n);
                                XtManageChild(mbox);
                                ...
                                XtAddCallback(shell, XmNpopdownCallback,
                                              destroy_mboxCB, (XtPointer)NULL);

                                ui_add_window_close_callback(
                                    shell, mwm_close_mboxCB, (XtPointer)&answer,
                                    XmDO_NOTHING);
                                ...
                                /* Display the "unsaved edits" message DIALOG if
                                 * the "unsaved edits" message topLevelShell is
                                 * not already up.
                                 */
