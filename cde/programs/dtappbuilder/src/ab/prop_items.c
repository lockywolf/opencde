...
#define HasItems(o)                                                            \
        (obj_is_choice(o) || obj_is_list(o) || obj_is_menubar(o) ||            \
         obj_is_menu(o) || obj_is_combo_box(o) || obj_is_spin_box(o))

#define PointerToRealItem(o) (o)->ui_handle

#define HasActiveState(t)                                                      \
        ((t) == AB_ITEM_FOR_CHOICE || (t) == AB_ITEM_FOR_MENUBAR ||            \
         (t) == AB_ITEM_FOR_MENU)

#define HasSelectedState(t)                                                    \
        ((t) == AB_ITEM_FOR_CHOICE || (t) == AB_ITEM_FOR_LIST ||               \
         (t) == AB_ITEM_FOR_COMBO_BOX || (t) == AB_ITEM_FOR_SPIN_BOX)

#define HasLabelType(t)                                                        \
        ((t) == AB_ITEM_FOR_CHOICE || (t) == AB_ITEM_FOR_MENUBAR ||            \
         (t) == AB_ITEM_FOR_MENU)
...
#define RequiresSelection(p)                                                   \
        (p->item_type == AB_ITEM_FOR_COMBO_BOX ||                              \
         p->item_type == AB_ITEM_FOR_SPIN_BOX ||                               \
         (p->item_type == AB_ITEM_FOR_CHOICE &&                                \
          obj_get_choice_type(*(p->current_obj_ptr)) !=                        \
...
static void activate_item_settings(PropItemsSetting pis, BOOL active);
static void activate_menu_item_settings(PropItemsSetting pis, BOOL active);
static void copy_item_to_clipboard(ABObj iobj, ABObj *item_clipboard);
static int copy_item_fields(ABObj iobj, ABObj iobj_copy);
static BOOL delete_selected_item(PropItemsSetting pis);
static int get_current_item_pos(PropItemsSetting pis);
static ABObj get_selected_item_copy(Widget itemlist, int *ppos);
static STRING get_unique_default_label(Widget itemlist, STRING base);
static STRING get_graphic_namebase(STRING graphic_path);
static ABObj init_new_item(PropItemsSetting pis);
static void load_item(PropItemsSetting pis, ABObj iobj);
static void setup_item_label_setting(PropItemsSetting pis,
                                     AB_LABEL_TYPE label_type, STRING label,
                                     AB_LINE_TYPE label_line_style);
static void verify_and_set_item_selected(PropItemsSetting pis, BOOL selected);
/*
 * Callbacks
 */
static void add_itemCB(Widget widget, XtPointer client_data,
                       XtPointer call_data);
static void edit_itemCB(Widget widget, XtPointer client_data,
                        XtPointer call_data);
static void field_activateCB(Widget widget, XtPointer client_data,
                             XtPointer call_data);
static void select_itemCB(Widget widget, XtPointer client_data,
                          XtPointer call_data);
static void set_edit_menu_stateCB(Widget widget, XtPointer client_data,
                                  XtPointer call_data);
 ...
 */
void prop_item_editor_init(
    PropItemsSetting pis, AB_ITEM_TYPE type, Widget list, Widget changebar,
    Widget add_button, int add_count, WidgetList add_item, int *add_enum,
    int edit_count, WidgetList edit_item, int *edit_enum,
    PropFieldSetting label_pfs, PropOptionsSetting label_type_pos,
    Widget label_graphic_hint, PropFieldSetting mnemonic_pfs,
    PropFieldSetting accel_pfs, PropOptionsSetting line_style_pos,
    PropCheckboxSetting state_pcs, PropMenunameSetting menuname_pms,
    ABObj *current_obj_ptr) {
        ...
        XtVaSetValues(pis->item_list, XmNselectionPolicy, XmBROWSE_SELECT,
                      XmNautomaticSelection, TRUE, NULL);

        XtVaSetValues(XtParent(pis->item_list), XmNrightAttachment,
                      XmATTACH_FORM, XmNrightOffset, 100, NULL);

        XtAddCallback(pis->item_list, XmNbrowseSelectionCallback, select_itemCB,
                      (XtPointer)pis);

        XtAddCallback(pis->item_label_pfs->field, XmNactivateCallback,
                      field_activateCB, (XtPointer)pis);

        XtVaSetValues(pis->item_label_pfs->label, XmNuserData,
                      (XtArgVal)label_graphic_hint, XmNrecomputeSize, False,
                      NULL);

        /* Set Up Add Button */
        XtVaSetValues(add_button, XmNuserData, (XtArgVal)INSERT_AFTER, NULL);
        XtAddCallback(add_button, XmNactivateCallback, add_itemCB,
                      (XtPointer)pis);

        /* Set Up Edit MenuButton */
        for (i = 0; i < add_count; i++) {
                XtVaSetValues(add_item[i], XmNuserData, (XtArgVal)add_enum[i],
                              NULL);
                XtAddCallback(add_item[i], XmNactivateCallback, add_itemCB,
                              (XtPointer)pis);
        }
        for (i = 0; i < edit_count; i++) {
                XtVaSetValues(edit_item[i], XmNuserData, (XtArgVal)edit_enum[i],
                              NULL);
                XtAddCallback(edit_item[i], XmNactivateCallback, edit_itemCB,
                              (XtPointer)pis);
        }
...
void prop_item_editor_load(PropItemsSetting pis, ABObj obj) {
        AB_TRAVERSAL trav;
        ABObj pobj, iobj;
        ABObj *iobj_list;
        int num_items;
...
void prop_item_editor_clear(PropItemsSetting pis) {
        ABObj *iobj_list;
        int num_items;
        int i;

        /* Destroy dup'd array of item objs */
        XtVaGetValues(pis->item_list, XmNuserData, &iobj_list, XmNitemCount,
                      &num_items, NULL);
        ...
        if (num_items > 0) /* Clear List & Array of Item copies...*/
        {
                ...
                util_free(iobj_list);
                XtVaSetValues(pis->item_list, XmNuserData, (XtArgVal)NULL,
                              NULL);
        ...
        if (HasLabelType(pis->item_type))
                prop_options_set_value(pis->item_label_type_pos,
                                       (XtPointer)AB_LABEL_STRING, False);
        ...
        if (HasLabelLineStyle(pis->item_type))
                prop_options_set_value(pis->item_line_style_pos,
                                       (XtPointer)AB_LINE_ETCHED_OUT, False);
 ...
 */
void prop_item_editor_apply(PropItemsSetting pis) {
        ABObj current_obj;
        AB_TRAVERSAL trav;
        ABObj *iobj_list;
        ...
        if (num_items > 0) {
                ...
                case AB_ITEM_FOR_CHOICE:
                        real_widget =
                            (Widget *)util_malloc(num_items * sizeof(Widget));
                }
        }
        ...
        for (i = 0; i < num_items; i++) {
                ...
                if (real_widget) {
                        ...
                        abobjP_disable_build_actions(real_iobj[i],
                                                     real_widget[i]);
        ...
        for (i = 0; i < new_num_items; i++) {
                ...
                if (PointerToRealItem(iobj_list[i]) != NULL) {
                        ...
                        for (j = 0; iobj == NULL && j < num_items; j++)
                                if (PointerToRealItem(iobj_list[i]) ==
                                    real_iobj[j]) {
                                        iobj = real_iobj[j];
                ...
                abobj_set_label(iobj, iobj_list[i]->label_type,
                                obj_get_label(iobj_list[i]));
                ...
                if (i < num_items) {
                        if (real_widget) {
                                ...
                                objxm_store_obj_and_actions(real_widget[i],
                                                            iobj);
                                abobjP_enable_build_actions(iobj,
                                                            real_widget[i]);

                                if (item_transfer) {
                                        /* Resources may be set differently for
                                         * iobj than they were set for the
                                         * previous iobj which owned this widget
                                         */
                                        objxm_comp_set_ui_args(
                                            iobj, OBJXM_CONFIG_BUILD, True);
        ...
        for (i = 0; i < num_items; i++) {
                ...
                if (real_widget && real_widget[i] != NULL)
                        ...
                        if (parent && pis->item_type == AB_ITEM_FOR_LIST)
                                XmListDeletePos(parent,
                                                real_pos[i - del_count]);

                        else if (parent &&
                                 pis->item_type == AB_ITEM_FOR_COMBO_BOX)
                                DtComboBoxDeletePos(parent,
                                                    real_pos[i - del_count]);

                        else if (parent &&
                                 pis->item_type == AB_ITEM_FOR_SPIN_BOX)
                                DtSpinBoxDeletePos(parent,
                                                   real_pos[i - del_count]);
                        del_count++;
                }
...
void prop_item_change(PropItemsSetting pis, BOOL select_next) {
        ABObj current_obj;
        ...
        if (pis->current_item) {
                XtVaGetValues(pis->item_list, XmNuserData, &iobj_list,
                ...
                if (current_item_pos == 0) {
                        util_dprintf(
                            3,
                            "prop_item_change: could not find item in List\n");
                        return;
                }

                if (HasLabelType(pis->item_type)) {
                        new_label_type = (AB_LABEL_TYPE)prop_options_get_value(
                            pis->item_label_type_pos);
                        label_type_chg =
                            (new_label_type != pis->current_item->label_type);
                ...
                if (HasMnemonic(pis->item_type))
                        obj_set_mnemonic(
                            pis->current_item,
                            prop_field_get_value(pis->item_mnemonic_pfs));

                if (HasAccelerator(pis->item_type))
                        obj_set_accelerator(
                            pis->current_item,
                            prop_field_get_value(pis->item_accel_pfs));

                if (HasSelectedState(pis->item_type))
                        verify_and_set_item_selected(
                            pis, prop_checkbox_get_value(pis->item_state_pcs,
                                                         AB_STATE_SELECTED));

                if (HasActiveState(pis->item_type))
                        obj_set_is_initially_active(
                            pis->current_item,
                            prop_checkbox_get_value(pis->item_state_pcs,
                                                    AB_STATE_ACTIVE));

                if (HasHelpState(pis->item_type)) {
                        BOOL help_item;

                        if ((help_item = prop_checkbox_get_value(
                                 pis->item_state_pcs, HELP_ITEM_KEY)) == True) {
                                if (!prop_help_item_ok(pis->item_list,
                                                       pis->current_item)) {
                                        prop_checkbox_set_value(
                                            pis->item_state_pcs, HELP_ITEM_KEY,
                                            False, False);
                                        help_item = FALSE;
                                }
                        }
                ...
                if (HasSubmenu(pis->item_type)) {
                        ...
                        if (prop_submenu_name_ok(pis->item_menuname_pms->field,
                                                 current_obj)) {
                                menuname = ui_field_get_string(
                                    pis->item_menuname_pms->field);
                                obj_set_menu_name(
                                    pis->current_item,
                                    (util_strcmp(menuname, "") == 0
                                         ? NULL
                                         : menuname));
                                util_free(menuname);
                        }
                ...
                case AB_LABEL_SEPARATOR:
                        /*
                         * Separator Menu-items have a NULL label string, but
                         * they still require a string for identification in the
                         * itemlist as well as to base the item name
                         */
                        newlabel = NULL;
                        obj_set_line_style(pis->current_item,
                                           (AB_LINE_TYPE)prop_options_get_value(
                                               pis->item_line_style_pos));
                        prop_field_set_value(pis->item_label_pfs, "", False);
                        basename = get_unique_default_label(
                            pis->item_list, (STRING)SeparatorNamebase);
                        break;

                case AB_LABEL_GLYPH:
                        if (!prop_graphic_filename_ok(
                                pis->item_label_pfs->field, False))
                                /* Couldn't convert to Pixmap, so reset back to
                                 * type String */
                                pis->current_item->label_type = AB_LABEL_STRING;
                        else {
                                /* Strip off graphic filename path for item name
                                 */
                                graphic_path = newlabel =
                                    prop_field_get_value(pis->item_label_pfs);
                ...
                if (label_type_chg ||
                    !util_streq(obj_get_label(pis->current_item), newlabel)) {
                        ...
                        abobj_set_item_name(
                            pis->current_item, obj_get_module(current_obj),
                            obj_get_name(current_obj), basename);

                        xmitem = XmStringCreateLocalized(basename);
                        XmListReplacePositions(pis->item_list,
                                               &current_item_pos, &xmitem, 1);
                        XmStringFree(xmitem);
                }
                ...
                if (select_next) {
                        next_pos = current_item_pos == num_items
                                       ? 1
                                       : current_item_pos + 1;
...
void prop_item_edit(PropItemsSetting pis, AB_EDIT_TYPE etype) {
        ABObj iobj_copy;
        int pos;
        int new_item_pos;
        ...
        case AB_EDIT_CUT:
                ...
                PointerToRealItem(pis->clipboard_item) =
                    PointerToRealItem(iobj_copy);
        ...
        case AB_EDIT_PASTE:
                if (pis->clipboard_item != NULL) {
                        ...
                        new_item_pos =
                            prop_item_insert(pis, INSERT_AFTER, iobj_copy);
                        if (new_item_pos != -1)
                                ui_list_select_pos(pis->item_list, new_item_pos,
                                                   True);
                        else {
                                PointerToRealItem(iobj_copy) = NULL;
                                obj_destroy(iobj_copy);
                                util_dprintf(3, "prop_item_edit: could not add "
                                                "new item to List\n");
                        }
                }
                break;
...
STRING
prop_item_get_namebase(ABObj iobj) {
...
int prop_item_insert(PropItemsSetting pis, INSERT_TYPE itype, ABObj iobj) {
        ABObj *iobj_list, *new_iobj_list;
        int num_items;
        int select_pos;
        int pos;
        int i, j;

        XtVaGetValues(pis->item_list, XmNitemCount, &num_items, XmNuserData,
                      &iobj_list, NULL);
...
static int copy_item_fields(ABObj iobj, ABObj iobj_copy) {
        if (iobj != NULL && iobj_copy != NULL) {
                ...
                obj_set_is_initially_visible(iobj_copy,
                                             obj_is_initially_visible(iobj));
                obj_set_is_initially_active(iobj_copy,
                                            obj_is_initially_active(iobj));
...
static BOOL delete_selected_item(PropItemsSetting pis) {
        ...
        XtVaGetValues(pis->item_list, XmNuserData, &iobj_list, NULL);
        ...
        if (select_pos != -1) {
                ...
                if (num_items > 1) {
                        new_iobj_list = (ABObj *)util_malloc((num_items - 1) *
                                                             sizeof(ABObj));
                        if (new_iobj_list == NULL) {
                                util_dprintf(
                                    1, "delete_selected_item: malloc failed\n");
                                return False;
                        }
                }
                ...
                for (i = j = 0; i < num_items; i++) {
                        if (i == (select_pos -
                                  1)) /* Found item-object being deleted */
                        {
                                /* If the item-object being deleted is the one
                                 * which is selected and it's not a
                                 * Checkbox-item, then choose another
                                 * item-object to be selected.
                                 */
                                if (num_items > 1 &&
                                    obj_is_initially_selected(iobj_list[i]) &&
                                    RequiresSelection(pis)) {
                                        if (i + 1 < num_items)
                                                obj_set_is_initially_selected(
                                                    iobj_list[i + 1], True);
                                        else
                                                obj_set_is_initially_selected(
                                                    iobj_list[0], True);
                ...
                if (num_items > 1)
                        XmListSelectPos(pis->item_list,
                                        select_pos == num_items ? select_pos - 1
                                                                : select_pos,
                                        TRUE);
...
ABObj get_selected_item_copy(Widget list, int *ppos) {
        ABObj *iobj_list;
        int select_pos;

        select_pos = ui_list_get_selected_pos(list);
        XtVaGetValues(list, XmNuserData, &iobj_list, NULL);
...
static STRING get_unique_default_label(Widget itemlist, STRING base) {
        ...
        while (!unique) {
                ...
                if (ui_list_find_item(itemlist, label_buf, &pos) ==
                    ERR_NOT_FOUND)
                        unique = True;
        }
        return label_buf;
...
static ABObj init_new_item(PropItemsSetting pis) {
        ...
        newlabel = get_unique_default_label(
            pis->item_list, catgets(Dtb_project_catd, 100, 268, "Item"));
...
static void load_item(PropItemsSetting pis, ABObj iobj) {
        ...
        if (label_type != AB_LABEL_SEPARATOR) {
                ...
                setup_item_label_setting(pis, iobj->label_type, label,
                                         AB_LINE_UNDEF);
                ui_field_select_string(pis->item_label_pfs->field, True);
        } else /* AB_LABEL_SEPARATOR */
                setup_item_label_setting(pis, iobj->label_type, "",
                                         iobj ? obj_get_line_style(iobj)
                                              : AB_LINE_NONE);

        /* Load Mnenonic/Accelerator */
        if (HasMnemonic(pis->item_type))
                prop_field_set_value(pis->item_mnemonic_pfs,
                                     obj_get_mnemonic(iobj), False);

        if (HasAccelerator(pis->item_type))
                prop_field_set_value(pis->item_accel_pfs,
                                     obj_get_accelerator(iobj), False);

        /* Load Item State */
        if (HasActiveState(pis->item_type))
                prop_checkbox_set_value(
                    pis->item_state_pcs, AB_STATE_ACTIVE,
                    iobj ? obj_is_initially_active(iobj) : True, False);

        if (HasSelectedState(pis->item_type))
                prop_checkbox_set_value(
                    pis->item_state_pcs, AB_STATE_SELECTED,
                    iobj ? obj_is_initially_selected(iobj) : False, False);

        if (HasHelpState(pis->item_type))
                prop_checkbox_set_value(pis->item_state_pcs, HELP_ITEM_KEY,
                                        iobj ? obj_is_help_item(iobj) : False,
                                        False);

        /* Load Item Submenu */
        if (HasSubmenu(pis->item_type))
                prop_menuname_set_value(pis->item_menuname_pms,
                                        iobj ? obj_get_menu_name(iobj) : "",
                                        False);
}
...
static void activate_item_settings(PropItemsSetting pis, BOOL active) {
        ...
        if (HasLabelType(pis->item_type)) {
                ...
                XtVaGetValues(pis->item_label_pfs->label, XmNuserData,
                              &graphic_hint, NULL);
                if (graphic_hint)
...
static void setup_item_label_setting(PropItemsSetting pis,
        ...
        XtVaSetValues(pis->item_label_pfs->field, XmNcursorPositionVisible,
                      no_sep_label, NULL);
        ...
        case AB_ITEM_FOR_LIST:
                if (selected) {
                        if (pis->item_type != AB_ITEM_FOR_LIST ||
                            ((obj_get_selection_mode(current_obj) ==
                              AB_SELECT_SINGLE) ||
                             (obj_get_selection_mode(current_obj) ==
                              AB_SELECT_BROWSE))) {
                                /* make sure only 1 Item is selected at a
                                 * time...*/
                                for (i = 0; i < num_items; i++)
                                        if (iobj_list[i] != pis->current_item &&
                                            obj_is_initially_selected(
                                                iobj_list[i]) == True)
                                                obj_set_is_initially_selected(
                                                    iobj_list[i], False);
                        }
...
static void select_itemCB(Widget widget, XtPointer client_data,
        ...
        if (pis->current_item != NULL && pis->current_item != iobj_copy) {
...
static void set_edit_menu_stateCB(Widget widget, XtPointer client_data,
        ...
        XtVaGetValues(pis->item_menu, XmNnumChildren, &num_menu_items,
                      XmNchildren, &menu_items, NULL);
...
void prop_item_labeltypeCB(Widget item, XtPointer clientdata,
                           XtPointer call_data) {
        ...
        setup_item_label_setting(pis, (AB_LABEL_TYPE)value, NULL,
                                 AB_LINE_UNDEF);
