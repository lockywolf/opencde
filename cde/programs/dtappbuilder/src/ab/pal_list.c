 ...
 * Methods
 */
static int list_initialize(ABObj obj);
static Widget list_prop_init(Widget parent, AB_PROP_TYPE type);
static int list_prop_activate(AB_PROP_TYPE type, BOOL active);
static int list_prop_clear(AB_PROP_TYPE type);
static int list_prop_load(ABObj obj, AB_PROP_TYPE type, unsigned long loadkey);
static int list_prop_apply(AB_PROP_TYPE type);
static BOOL list_prop_pending(AB_PROP_TYPE type);
...
static int prop_list_obj_destroy_CB(ObjEvDestroyInfo destroyInfo);

/*
 * Xt Callbacks
 */
static void wth_policyCB(Widget widget, XtPointer clientdata,
...
static Widget list_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbListPropDialogInfo cgen =
            &dtb_list_prop_dialog; /* Codegen structure */
        ...
        if (dtb_list_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_REVOLVING)
                        XtVaSetValues(parent, XmNuserData, pls->current_obj,
                                      NULL);
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_list_palitem, cgen->ok_button,
                ...
                n++;
                prop_options_init(
                    &(pls->label_type), cgen->labeltype_rbox_label,
                ...
                prop_label_field_init(&(pls->label), cgen->graphic_hint, item,
                                      n);

                n = 0;
                item[n] = cgen->labelpos_rbox_items.Above_item;
                item_val[n] = AB_CP_NORTH;
                n++;
                item[n] = cgen->labelpos_rbox_items.Left_item;
                item_val[n] = AB_CP_WEST;
                n++;
                ...
                prop_radiobox_init(&(pls->wth_policy), cgen->wpolicy_rbox_label,
                                   cgen->wpolicy_rbox, n, item,
                                   (XtPointer *)item_val, cgen->wpolicy_cb);
                ...
                prop_geomfield_init(&(pls->wth), cgen->wpolicy_rbox_label, NULL,
                                    NULL, NULL, NULL, cgen->width_field_label,
                                    cgen->width_field, NULL, NULL,
                                    cgen->wpolicy_cb);

                /* Height */
                prop_geomfield_init(&(pls->hgt), cgen->hgt_opmenu_label, NULL,
                                    NULL, NULL, NULL, NULL, NULL,
                                    cgen->height_field_label,
                                    cgen->height_field, cgen->hgt_cb);

                n = 0;
                item[n] = cgen->hgt_opmenu_items.Lines_item;
                item_val[n] = SIZE_IN_CHARS_KEY;
                n++;
                item[n] = cgen->hgt_opmenu_items.Pixels_item;
                item_val[n] = SIZE_IN_PIXELS_KEY;
                n++;
                prop_options_init(&(pls->hgt_metric), cgen->hgt_opmenu_label,
                                  cgen->hgt_opmenu, cgen->hgt_opmenu_menu, n,
                                  item, (XtPointer *)item_val, cgen->hgt_cb);
                ...
                prop_menuname_init(&(pls->menuname), type, cgen->menu_label,
                                   ...
                                   &(pls->menu_title), &(pls->current_obj),
                                   True);
                ...
                prop_item_editor_init(
                    &(pls->items), AB_ITEM_FOR_LIST, cgen->itemlist,
                    cgen->itemlist_cb, cgen->item_add_button, n, item,
                    item_val,            /* Insert */
                    j, item2, item2_val, /* Edit */
                    &(pls->item_label), NULL /*label_type*/,
                    NULL /*graphic_hint*/, NULL /*mnemonic*/, NULL /*accel*/,
                    NULL /*line_style*/, &(pls->item_state), NULL /*submenu*/,
                    &(pls->current_obj));
...
static int list_prop_clear(AB_PROP_TYPE type) {
        ...
        prop_options_set_value(&(pls->label_type), (XtPointer)AB_LABEL_STRING,
                               False);
        ...
        prop_options_set_value(&(pls->sel_mode), (XtPointer)AB_SELECT_SINGLE,
                               False);
        ...
        /* Clear Width */
        prop_radiobox_set_value(&(pls->wth_policy), (XtPointer)SIZE_FIXED_KEY,
                                False);
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pls->init_state), AB_STATE_VISIBLE, True,
                                False);
        prop_checkbox_set_value(&(pls->init_state), AB_STATE_ACTIVE, True,
                                False);
...
static int list_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_options_set_value(&(pls->label_type),
                                       (XtPointer)obj->label_type, False);
                prop_options_set_value(&(pls->label_pos),
                                       (XtPointer)obj_get_label_position(obj),
                                       False);

                /* Load Label */
                prop_setup_label_field(&(pls->label), NULL, obj->label_type,
                                       obj_get_label(obj), AB_LINE_UNDEF);

                /* Load Selection Mode */
                prop_options_set_value(
                    &(pls->sel_mode),
                    (XtPointer)((AB_SELECT_TYPE)obj_get_selection_mode(obj)),
                    False);
                ...
                prop_field_set_value(&(pls->menu_title),
                                     obj_get_menu_title(obj), False);
        ...
        if (load_all || loadkey & LoadSize) {
                ...
                {
                        prop_options_set_value(&(pls->hgt_metric),
                                               (XtPointer)SIZE_IN_PIXELS_KEY,
                                               False);
                        height = abobj_get_comp_height(obj);
                } else /* Size in Chars */
                {
                        prop_options_set_value(&(pls->hgt_metric),
                                               (XtPointer)SIZE_IN_CHARS_KEY,
                                               False);
                        height = obj_get_num_rows(obj);
                }
                prop_geomfield_set_value(&(pls->hgt), GEOM_HEIGHT, height,
                                         False);
        }
...
static int list_prop_apply(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pls->label.changebar) ||
            prop_changed(pls->label_type.changebar)) {
                ...
                abobj_set_label_position(
                    pls->current_obj, (AB_COMPASS_POINT)prop_options_get_value(
                                          &(pls->label_pos)));
        ...
        if (prop_changed(pls->wth_policy.changebar)) {
                ...
                abobj_set_size_policy(pls->current_obj,
                                      width_policy == SIZE_FIXED_KEY);

                if (width_policy == SIZE_FIXED_KEY) {
                        new_w =
                            prop_geomfield_get_value(&(pls->wth), GEOM_WIDTH);
        ...
        if (prop_changed(pls->hgt.changebar)) {
                ...
                abobj_set_pixel_height(
                    pls->current_obj, metric == SIZE_IN_PIXELS_KEY ? new_h : -1,
                    0);
        ...
        if (prop_changed(pls->init_state.changebar)) {
                abobj_set_visible(pls->current_obj,
                                  prop_checkbox_get_value(&(pls->init_state),
                                                          AB_STATE_VISIBLE));
        ...
        if (prop_changed(pls->fg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pls->fg_color),
                                          obj_get_fg_color(pls->current_obj),
                                          False);
        }
        if (prop_changed(pls->bg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pls->bg_color),
                                          obj_get_bg_color(pls->current_obj),
                                          False);
        }
        if (prop_changed(pls->menuname.changebar)) {
                ...
                util_free(value);
                prop_menuname_set_value(&(pls->menuname),
                                        obj_get_menu_name(pls->current_obj),
                                        False);
        }
        ...
        if (size_chg) {
                ...
                if (obj_get_width(pls->current_obj) != -1) {
                        szObj = objxm_comp_get_subobj(pls->current_obj,
                                                      AB_CFG_SIZE_OBJ);
                        XtVaSetValues(objxm_get_widget(szObj), XmNwidth,
                                      obj_get_width(pls->current_obj), NULL);
                }
...
static BOOL list_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_list_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if ((prop_changed(pls->label_type.changebar) ||
             prop_changed(pls->label.changebar)) &&
            prop_options_get_value(&(pls->label_type)) ==
                (XtPointer)AB_LABEL_GLYPH &&
            !prop_graphic_filename_ok(pls->label.field, False))
                return False;

        if (prop_changed(pls->pos.changebar) &&
            (!prop_number_ok(pls->pos.x_field, (STRING)XFieldStr, -SHRT_MAX,
                             SHRT_MAX) ||
             !prop_number_ok(pls->pos.y_field, (STRING)YFieldStr, -SHRT_MAX,
                             SHRT_MAX)))
                return False;
        ...
        if (prop_changed(pls->fg_color.changebar) &&
            !prop_color_ok(pls->fg_color.field))
                return False;

        if (prop_changed(pls->bg_color.changebar) &&
            !prop_color_ok(pls->bg_color.field))
                return False;

        if (prop_changed(pls->menuname.changebar) &&
            !prop_obj_name_ok(pls->menuname.field,
                              obj_get_module(pls->current_obj), AB_TYPE_MENU,
                              "Menu"))
                return False;
 ...
 */
static int prop_list_install_obj_destroy_CB() {
...
static void wth_policyCB(Widget w, XtPointer clientdata,
                         XmToggleButtonCallbackStruct *state) {
        ...
        if (state->set) {
                ...
                ui_field_set_editable(pls->wth.w_field,
