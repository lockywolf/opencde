...
static int panedwin_initialize(ABObj obj);
static Widget panedwin_prop_init(Widget parent, AB_PROP_TYPE type);
static int panedwin_prop_load(ABObj obj, AB_PROP_TYPE type,
                              unsigned long loadkey);
static int panedwin_load_panes(ABObj obj, AB_PROP_TYPE type);
static int panedwin_prop_clear(AB_PROP_TYPE type);
static int panedwin_prop_clear_geom(AB_PROP_TYPE type);
static int panedwin_prop_activate(AB_PROP_TYPE type, BOOL active);
static int panedwin_prop_apply(AB_PROP_TYPE type);
static BOOL panedwin_prop_pending(AB_PROP_TYPE type);
...
static int position_compare(const void *leftEntry, const void *rightEntry);

static int east_compare(const void *leftEntry, const void *rightEntry);

static int west_compare(const void *leftEntry, const void *rightEntry);
...
static Widget panedwin_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbPanedwinEdDialogInfo cgen =
            &dtb_panedwin_ed_dialog; /* Codegen structure */
        ...
        if (dtb_panedwin_ed_dialog_initialize(cgen, parent) == 0) {
                 ...
                 */
                XtAddCallback(cgen->panelist, XmNbrowseSelectionCallback,
                              (XtCallbackProc)pw_panelist_selectCB,
                              (XtPointer)type);

                if (type == AB_PROP_REVOLVING)
                        XtVaSetValues(parent, XmNuserData, pws->current_pw_obj,
                                      NULL);

                /* Dialog/Object List */
                if (type == AB_PROP_FIXED) {
                        prop_fixed_dialog_init(ab_panedwin_palitem,
                                               cgen->dialog_shellform,
                                               cgen->objlist);
                        prop_activate_panel_init(
                            type, ab_panedwin_palitem, cgen->ok_button,
                ...
                n++;
                prop_checkbox_init(&(pws->init_state),
                                   cgen->init_state_cbox_label,
                                   cgen->init_state_cbox, n, item, item_val,
                                   cgen->init_state_cb);

                /* Pane Minimum/Maximum Setting */
                prop_geomfield_init(
                    &(pws->min_max_geo), cgen->pane_height_lbl, NULL, NULL,
                    NULL, NULL, cgen->min_height_field_label,
                    cgen->min_height_field, cgen->max_height_field_label,
                    cgen->max_height_field, cgen->height_cb);
...
static int panedwin_prop_clear(AB_PROP_TYPE type) {
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pws->init_state), AB_STATE_VISIBLE, True,
                                False);
        prop_checkbox_set_value(&(pws->init_state), AB_STATE_ACTIVE, True,
                                False);
...
static int panedwin_prop_clear_geom(AB_PROP_TYPE type) {
        ...
        XtVaSetValues(pws->width, XtVaTypedArg, XmNlabelString, XtRString,
                      "   0", strlen("   0") + 1, NULL);
        XtVaSetValues(pws->height, XtVaTypedArg, XmNlabelString, XtRString,
                      "   0", strlen("   0") + 1, NULL);
...
static int panedwin_pane_prop_load(ABObj obj, AB_PROP_TYPE type) {
        PropPanedWinSettingsRec *pws = &(prop_pw_settings_rec[type]);
        char width[MAXPATHLEN], height[MAXPATHLEN];
        *width = 0;
        *height = 0;
        ...
        XtVaSetValues(pws->width, XtVaTypedArg, XmNlabelString, XtRString,
                      width, strlen(width) + 1, NULL);
        XtVaSetValues(pws->height, XtVaTypedArg, XmNlabelString, XtRString,
                      height, strlen(height) + 1, NULL);
 ...
 */
int panedwin_prop_apply(AB_PROP_TYPE type) {
        PropPanedWinSettingsRec *pws = &(prop_pw_settings_rec[type]);
        STRING value;
        int geom;
        ...
        if (prop_changed(pws->init_state.changebar)) {
                abobj_set_visible(pws->current_pw_obj,
                                  prop_checkbox_get_value(&(pws->init_state),
                                                          AB_STATE_VISIBLE));
        ...
        if (prop_changed(pws->min_max_geo.changebar)) {
                geom =
                    prop_geomfield_get_value(&(pws->min_max_geo), GEOM_WIDTH);
                abobj_set_pane_min(pws->current_pane_obj, geom);
                geom =
                    prop_geomfield_get_value(&(pws->min_max_geo), GEOM_HEIGHT);
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pws->min_max_geo.changebar) &&
            (!prop_number_ok(pws->min_max_geo.w_field, "Pane Height Min Field",
                             1, SHRT_MAX) ||
             !prop_number_ok(pws->min_max_geo.h_field, "Pane Height Max Field",
                             1, SHRT_MAX)))
                return False;
                        ...
                        for (i = 0; i < AB_PROP_TYPE_NUM_VALUES; ++i) {
                                ...
                                if (pws->current_pane_obj == info->obj)
                                        panedwin_prop_clear_geom(
                                            (AB_PROP_TYPE)i);
                                ret = abobj_list_obj_reparented(
                                    pws->panelist, info, pw_child_test_func);
                        }
                }
        }
        return (ret);
...
void abobj_make_panedwin() {
        ...
        void *attach_val = NULL, *next_attach_val = NULL;
        int attach_offset = 0;
        ...
        obj_set_attachment(pw_obj, AB_CP_NORTH, attach_type, attach_val,
                           attach_offset);
        ...
        attach_offset =
            obj_get_attach_offset(sel.list[sel.count - 1], AB_CP_SOUTH);
        obj_set_attachment(pw_obj, AB_CP_SOUTH, attach_type, attach_val,
                           attach_offset);
        ...
        attach_offset =
            obj_get_attach_offset(sel.list[sel.count - 1], AB_CP_EAST);
        obj_set_attachment(pw_obj, AB_CP_EAST, attach_type, attach_val,
                           attach_offset);
        ...
        obj_set_attachment(pw_obj, AB_CP_WEST, attach_type, attach_val,
                           attach_offset);
        ...
        if (AttachTypesEqual) {
                ...
                for (i = 1; i < sel.count; i++) {
                        next_attach_val =
                            obj_get_attach_value(sel.list[i], AB_CP_EAST);
                        if (attach_val != next_attach_val) {
                                ...
                                dtb_show_message(
                                    ...
                                    &dtb_panedwin_ed_pw_east_attach_msg, NULL,
                                    NULL);
                                break;
                        }
                }
        ...
        if (AttachTypesEqual) {
                ...
                for (i = 1; i < sel.count; i++) {
                        next_attach_val =
                            obj_get_attach_value(sel.list[i], AB_CP_WEST);
                        if (attach_val != next_attach_val) {
                                /* Post Message */
                                dtb_panedwin_ed_pw_west_attach_msg_initialize(
                                    &dtb_panedwin_ed_pw_west_attach_msg);
                                dtb_show_message(
                                    (Widget)obj_parent->ui_handle,
                                    &dtb_panedwin_ed_pw_west_attach_msg, NULL,
                                    NULL);
                                break;
                        }
                }
...
static int east_compare(const void *leftEntry, const void *rightEntry) {
        ...
        int left_xpos =
            obj_get_x(*left_obj) + abobj_get_actual_width(*left_obj);
        int right_xpos =
            obj_get_x(*right_obj) + abobj_get_actual_width(*right_obj);
        return (left_xpos - right_xpos);
}
