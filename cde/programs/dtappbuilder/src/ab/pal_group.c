 ...
 * Methods
 */
static int group_initialize(ABObj obj);
static Widget group_prop_init(Widget parent, AB_PROP_TYPE type);

static int group_prop_activate(AB_PROP_TYPE type, BOOL active);
static int group_prop_load(ABObj obj, AB_PROP_TYPE type, unsigned long loadkey);
static int group_prop_clear(AB_PROP_TYPE type);
static int group_prop_apply(AB_PROP_TYPE type);

static BOOL group_prop_pending(AB_PROP_TYPE type);

static BOOL verify_props(AB_PROP_TYPE type);
static void turnoff_changebars(AB_PROP_TYPE type);
static void setup_grouptype_settings(AB_PROP_TYPE type, AB_GROUP_TYPE gtype);
static void set_new_group_size(ABObj obj);

static void group_objects(ABObj newgroup, ABObj *obj_list, int count);

static void ungroup_objects(ABObj *obj_list, int count);

static void group_align_tops(ABObj obj);

static void group_align_hcenters(ABObj obj, BOOL init);

static void group_align_horiz_same_size(ABObj obj);

static void group_align_bottoms(ABObj obj);

static void group_align_left(ABObj obj);

static void group_align_labels(ABObj obj);

static void group_align_vcenters(ABObj obj, BOOL init);

static void group_align_right(ABObj obj);

static void create_member_list(ABObj cobj, ABObj **member_list,
                               int *member_count);

static void get_cell_size(ABObj group, int *cell_width, int *cell_height);

static void get_row_col(ABObj group, int *rows, int *cols);

static void group_align_rows(ABObj obj, BOOL init);

static void group_align_cols(ABObj obj, BOOL init);

static void get_widest_label_obj(ABObj *list, int count, ABObj *widest_label,
                                 int *label_width);

static void get_widest_value_obj(ABObj *list, int count, ABObj *widest_value,
                                 int *value_width);

/*
 * Callbacks
 */
static void grouptypeCB(Widget widget, XtPointer clientdata,
                        XmToggleButtonCallbackStruct *state);

static void undo_group(ABUndoRec undo_rec);

static void undo_ungroup(ABUndoRec undo_rec);

static ABObj get_child(ABObj group, int x_pos, int y_pos);

static void group_expose_handler(Widget widget, XtPointer client_data,
 ...
 */
void abobj_group_selected_objects() {
        ...
        if (group_initialize(newgroup) == ERROR)
                fprintf(stderr,
                        "create_obj_action: couldn't initialize object\n");
...
static void group_objects(ABObj newgroup, ABObj *list, int count) {
        ABObj obj;
        ABSelectedRec sel;
        XRectangle current_rect;
        int i;
 ...
 */
void abobj_ungroup_selected_objects() {
        ABObj project = proj_get_project();
        ABSelectedRec sel;
...
static void ungroup_objects(ABObj *obj_list, int count) {
        ...
        for (i = 0; i < count; i++) {
                ...
                if (obj_is_group(group)) {
                        ABObj oobj = NULL, *member_list = NULL;
                        int num_children, cur_member = 0;
                        ...
                        member_list = (ABObj *)util_malloc(num_children *
                                                           sizeof(ABObj *));

                        /*
                         * Group members are all direct children of the
                         * AB_CFG_OBJECT_OBJ object
                         */
                        for (trav_open(&trav, oobj,
                                       AB_TRAV_CHILDREN | AB_TRAV_MOD_SAFE);
                             (member = trav_next(&trav)) != NULL;) {
                                ...
                                /*
                                 * Set attachments with 0 x and y. abobj_set_xy
                                 * will set the correct values.
                                 */
                                obj_init_attachments(member);
                                obj_set_attachment(member, AB_CP_NORTH,
                                                   AB_ATTACH_POINT, NULL, 0);
                                obj_set_attachment(member, AB_CP_WEST,
                                                   AB_ATTACH_POINT, NULL, 0);
                                 ...
                                 * We don't do it here because we don't want to
                                 * screw up the x_get_widget_rect() calls in the
                                 * current group.
                                 */
                         ...
                         */
                        for (cur_member = 0; cur_member < num_children;
                             ++cur_member) {
                                ABObj cur_obj = NULL;

                                /*
                                 * Clear instantiated flag in former group
                                 * member
                                 */
                                obj_clear_flag(member_list[cur_member],
                                               InstantiatedFlag);

                                /*
                                 * Clear instantiated flag in former group
                                 * member's subtree as well
                                 */
                                for (trav_open(&trav, member_list[cur_member],
                                               AB_TRAV_ALL);
                                     (cur_obj = trav_next(&trav)) != NULL;) {
                                        obj_clear_flag(cur_obj,
                                                       InstantiatedFlag);
                                }

                                /*
                                 * The manage_lat parameter used to be FALSE.
                                 * It caused list objects to get the wrong size
                                 * when ungroup was done. Changing to TRUE
                                 * somehow fixes this.
                                 */
...
static Widget group_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        */
        DtbGroupPropDialogInfo cgen =
            &dtb_group_prop_dialog; /* Codegen structure */
        ...
        if (dtb_group_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_REVOLVING)
                        XtVaSetValues(parent, XmNuserData, pgs->current_obj,
                                      NULL);

                /* Dialog/Object List */
                if (type == AB_PROP_FIXED) {
                        prop_fixed_dialog_init(ab_group_palitem,
                                               cgen->prop_dialog_shellform,
                                               cgen->objlist);
                        prop_activate_panel_init(
                            type, ab_group_palitem, cgen->ok_button,
                            cgen->apply_button, cgen->reset_button,
                            cgen->cancel_button, cgen->help_button);
                }

                /* Alternate Editor Buttons */
                prop_editors_panel_init(type, ab_group_palitem,
                                        cgen->attach_button, cgen->conn_button,
                                        cgen->helptxt_button);
                ...
                */
                ...
                item_val[n] = AB_ORIENT_HORIZONTAL;
                n++;
                item[n] = cgen->gridrowcol_rbox_items.Columns_item;
                item_val[n] = AB_ORIENT_VERTICAL;
                n++;
                prop_radiobox_init(&(pgs->grid_rowcol), NULL,
                                   cgen->gridrowcol_rbox, n, item,
                                   (XtPointer *)item_val, cgen->gridrowcol_cb);
                ...
                item_val[n] = AB_ALIGN_LEFT;
                n++;
                item[n] =
                    cgen->valign_opmenu_items.bitmaps_align_labels_xbm_item;
                item_val[n] = AB_ALIGN_LABELS;
                n++;
                item[n] =
                    cgen->valign_opmenu_items.bitmaps_align_vcenter_xbm_item;
                item_val[n] = AB_ALIGN_VCENTER;
                n++;
                item[n] =
                    cgen->valign_opmenu_items.bitmaps_align_right_xbm_item;
                item_val[n] = AB_ALIGN_RIGHT;
                n++;
                ...
                item_val[n] = AB_ALIGN_TOP;
                n++;
                item[n] =
                    cgen->halign_opmenu_items.bitmaps_align_hcenter_xbm_item;
                item_val[n] = AB_ALIGN_HCENTER;
                n++;
                item[n] =
                    cgen->halign_opmenu_items.bitmaps_align_bottom_xbm_item;
                item_val[n] = AB_ALIGN_BOTTOM;
                n++;
                ...
                prop_geomfield_init(&(pgs->pos), cgen->pos_label,
                                    ...
                                    cgen->y_field_label, cgen->y_field, NULL,
                                    NULL, NULL, NULL, cgen->pos_cb);

                /* Initial State */
                n = 0;
                item[n] = cgen->istate_ckbox_items.Visible_item;
                item_val[n] = AB_STATE_VISIBLE;
                n++;
                item[n] = cgen->istate_ckbox_items.Active_item;
                item_val[n] = AB_STATE_ACTIVE;
                n++;
...
static int group_prop_clear(AB_PROP_TYPE type) {
        ...
        /* Clear Layout Type */
        prop_radiobox_set_value(&(pgs->layout_type), (XtPointer)AB_GROUP_IGNORE,
                                False);

        /* Clear Grid Rows/Columns */
        prop_radiobox_set_value(&(pgs->grid_rowcol),
                                (XtPointer)AB_ORIENT_HORIZONTAL, False);
        prop_field_set_numeric_value(&(pgs->grid_rowcol_count), NUMROWCOL,
                                     False);
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pgs->init_state), AB_STATE_VISIBLE, True,
                                False);
        prop_checkbox_set_value(&(pgs->init_state), AB_STATE_ACTIVE, True,
                                False);
...
static int group_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_options_set_value(
                    &(pgs->frame), (XtPointer)obj_get_border_frame(obj), False);

                /* Load Layout Type */
                gtype = obj_get_group_type(obj);
                prop_radiobox_set_value(&(pgs->layout_type), (XtPointer)gtype,
                                        False);
                ...
                case AB_GROUP_ROWS:
                        prop_options_set_value(
                            &(pgs->halign), (XtPointer)obj_get_row_align(obj),
                            False);

                        prop_field_set_numeric_value(
                            &(pgs->hspacing), obj_get_hoffset(obj), False);
                        break;

                case AB_GROUP_COLUMNS:
                        prop_options_set_value(
                            &(pgs->valign), (XtPointer)obj_get_col_align(obj),
                            False);

                        prop_field_set_numeric_value(
                            &(pgs->vspacing), obj_get_voffset(obj), False);
                        break;
...
static int group_prop_apply(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pgs->layout_type.changebar)) {
                ...
                case AB_GROUP_ROWSCOLUMNS:
                        ...
                        num_rc = prop_field_get_numeric_value(
                            &(pgs->grid_rowcol_count));

                        if (prop_radiobox_get_value(&(pgs->grid_rowcol)) ==
                            AB_ORIENT_HORIZONTAL) {
                                abobj_set_num_rows(obj,
                                                   num_rc < 2 ? 2 : num_rc);
                                abobj_set_num_columns(obj, 0);
                                if (obj_get_group_type(obj) == AB_GROUP_IGNORE)
                                        abobj_sort_children(obj, YSORT);
                        } else {
                                abobj_set_num_columns(obj,
                                                      num_rc < 2 ? 2 : num_rc);
                                abobj_set_num_rows(obj, 0);
                                if (obj_get_group_type(obj) == AB_GROUP_IGNORE)
                                        abobj_sort_children(obj, XSORT);
        ...
        if (prop_changed(pgs->grid_rowcol_count.changebar)) {
                ...
                num_rc =
                    prop_field_get_numeric_value(&(pgs->grid_rowcol_count));
                if (prop_radiobox_get_value(&(pgs->grid_rowcol)) ==
                    AB_ORIENT_HORIZONTAL) {
        ...
        if (prop_changed(pgs->halign.changebar)) {
                ...
                abobj_set_row_alignment(
                    obj, (AB_ALIGNMENT)prop_options_get_value(&(pgs->halign)));
        ...
        if (prop_changed(pgs->valign.changebar)) {
                ...
                abobj_set_col_alignment(
                    obj, (AB_ALIGNMENT)prop_options_get_value(&(pgs->valign)));
        ...
        if (changed) {
                ...
                XtVaGetValues(w, XmNnumChildren, &num_children, XmNchildren,
                              &children_list, NULL);
        ...
        if (prop_changed(pgs->init_state.changebar)) {
                abobj_set_visible(pgs->current_obj,
                                  prop_checkbox_get_value(&(pgs->init_state),
                                                          AB_STATE_VISIBLE));
        ...
        if (prop_changed(pgs->bg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pgs->bg_color),
                                          obj_get_bg_color(pgs->current_obj),
                                          False);
        }
...
static BOOL group_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_group_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pgs->name.changebar) &&
            !prop_name_ok(pgs->current_obj, pgs->name.field))
                return False;

        if (prop_changed(pgs->bg_color.changebar) &&
            !prop_color_ok(pgs->bg_color.field))
                return False;

        if (prop_changed(pgs->grid_rowcol_count.changebar) &&
            !prop_number_ok(pgs->grid_rowcol_count.field,
                            (STRING)RowColFieldStr, 1, SHRT_MAX))
                return False;
        ...
        if (prop_changed(pgs->pos.changebar) &&
            (!prop_number_ok(pgs->pos.x_field, (STRING)XFieldStr, -SHRT_MAX,
                             SHRT_MAX) ||
             !prop_number_ok(pgs->pos.y_field, (STRING)YFieldStr, -SHRT_MAX,
                             SHRT_MAX)))
                return False;
...
static void grouptypeCB(Widget item, XtPointer clientdata,
                        XmToggleButtonCallbackStruct *state) {
...
static void undo_group(ABUndoRec undo_rec) {
         ...
         */
        abobj_set_undo(undo_rec->list, undo_rec->count, undo_ungroup,
                       AB_UNDO_UNGROUP);
...
static void undo_ungroup(ABUndoRec undo_rec) {
        ABObj newgroup, parent, *new_list, *member_list, project;
        int i, member_count;
        ...
        for (i = 0; i < undo_rec->count; ++i) {
                ...
                newgroup =
                    abobj_dup_tree(undo_info->info.ungroup.dup_old_group);
                ...
                if (abobj_show_tree(newgroup, False) == -1)
                        fprintf(stderr,
                                "create_obj_action: couldn't show object\n");
                else {
...
void set_new_group_size(ABObj obj) {
        ABObj oobj = objxm_comp_get_subobj(obj, AB_CFG_OBJECT_OBJ);
        int member_count;
        ABObj *member_list;
        int width, height;
        ...
        abobj_get_greatest_size(member_list, member_count, &width, &height,
...
void abobj_layout_group(ABObj obj, BOOL init) {
        ABObj pobj;
        ABObj child;
        AB_GROUP_TYPE type;
        int i;
...
void abobj_register_group_expose_handler(ABObj obj) {
        AB_GROUP_TYPE type;
        AB_ALIGNMENT row_alignment, col_alignment;
        Widget group_widget;
        ...
        if (register_expose) {
                ...
                XtAddEventHandler(group_widget, ExposureMask, False,
...
static void group_expose_handler(Widget widget, XtPointer client_data,
        ...
        XtRemoveEventHandler(widget, ExposureMask, False, group_expose_handler,
...
static void group_align_tops(ABObj obj) {
        ABAttachment attach;
        ABObj child, previous_child,
            oobj = objxm_comp_get_subobj(obj, AB_CFG_OBJECT_OBJ);
        int num_children = obj_get_num_children(oobj), num_columns, num_rows,
            cell_height, cell_width, i, j;
        ...
        for (j = 0; j < num_rows; j++) {
                for (i = 0; i < num_columns; i++) {
                        ...
                        if ((i == 0) && (j == 0)) {
                                ...
                                abobj_set_attachment(child, AB_CP_NORTH,
                                                     &attach);
...
static void group_align_hcenters(ABObj obj, BOOL init) {
        ...
        int num_children = obj_get_num_children(oobj), num_columns, num_rows,
            cell_width, cell_height, group_width, group_height, offset,
            gridline, i, j;
        ...
        for (j = 0; j < num_rows; j++) {
                ...
                for (i = 0; i < num_columns; i++) {
                        ...
                        if (init) {
                                ...
                                if (child_widget &&
                                    !XtIsSubclass(child_widget,
                                                  compositeWidgetClass)) {
                                        int height =
                                            abobj_get_actual_height(child);
...
static void group_align_horiz_same_size(ABObj obj) {
        ...
        for (i = num_children - 2; i >= 0; i--) {
                ...
                obj_set_attachment(sibling, AB_CP_NORTH, AB_ATTACH_OBJ,
                                   (void *)oobj, 0);

                obj_set_attachment(sibling, AB_CP_WEST, AB_ATTACH_OBJ,
                                   (void *)previous_sibling,
                                   obj_get_hoffset(oobj));

                obj_set_attachment(sibling, AB_CP_SOUTH, AB_ATTACH_OBJ,
                                   (void *)oobj, 0);
        ...
        obj_set_attachment(sibling, AB_CP_NORTH, AB_ATTACH_OBJ, (void *)oobj,
                           0);
        obj_set_attachment(sibling, AB_CP_WEST, AB_ATTACH_OBJ, (void *)oobj, 0);

        obj_set_attachment(sibling, AB_CP_SOUTH, AB_ATTACH_OBJ, (void *)oobj,
                           0);
...
static void group_align_bottoms(ABObj obj) {
        ...
        AB_GROUP_TYPE type = obj_get_group_type(obj);
        int num_children = obj_get_num_children(oobj), num_columns, num_rows,
            cell_height, cell_width, offset, i, j;
        ...
        for (j = 0; j < num_rows; j++) {
                for (i = 0; i < num_columns; i++) {
                        ...
                        if (child && previous_child) {
                                ...
                                abobj_set_attachment(child, AB_CP_SOUTH,
                                                     &attach);
                        }
                }
        }
        ...
        for (j = 0; j < num_rows; j++) {
                for (i = 0; i < num_columns; i++) {
                        ...
                        if ((i == 0) && (j == 0)) {
                                ...
                                abobj_set_attachment(child, AB_CP_WEST,
                                                     &attach);
                                continue;
                        }
...
static void group_align_labels(ABObj obj) {
        ...
        ABObj child, previous_child,
            oobj = objxm_comp_get_subobj(obj, AB_CFG_OBJECT_OBJ),
            *one_col = NULL, *child_list, previous_ref_obj = NULL;
        int num_children = 0, num_columns, num_rows, cell_width, cell_height,
            offset, max_label_width = 0, max_value_width = 0, i, j;
        ...
        get_widest_label_obj(child_list, num_children, &child,
                             &max_label_width);
        get_widest_value_obj(child_list, num_children, &child,
                             &max_value_width);
...
static void group_align_right(ABObj obj) {
        ...
        AB_GROUP_TYPE type = obj_get_group_type(obj);
        int num_children = obj_get_num_children(oobj), num_columns, num_rows,
            cell_width, cell_height, offset, i, j;
        ...
        for (j = 0; j < num_rows; j++) {
                for (i = 0; i < num_columns; i++) {
                        ...
                        if (previous_child) {
                                ...
                                abobj_set_attachment(child, AB_CP_EAST,
                                                     &attach);
                        }
                }
        }
...
static void create_member_list(ABObj cobj, ABObj **member_list,
        ...
        *member_count =
            trav_count(cobj, AB_TRAV_SALIENT_CHILDREN | AB_TRAV_MOD_SAFE);
...
static ABObj get_child(ABObj group, int x_pos, int y_pos) {
        AB_GROUP_TYPE type;
        ABObj ret_child = NULL, oobj;
        int num_children, num_rows, num_columns, i = -1;

        if (!group || !obj_is_group(group) || (x_pos < 0) || (y_pos < 0))
                return (NULL);
...
static void get_cell_size(ABObj group, int *cell_width, int *cell_height) {
        ...
        */
        if (1) {
                ...
                create_member_list(oobj, &member_list, &member_count);
                abobj_get_greatest_size(member_list, member_count, cell_width,
                                        cell_height, (ABObj *)NULL,
                                        (ABObj *)NULL);
