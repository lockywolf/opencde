 ...
 * Methods
 */
static int button_initialize(ABObj obj);
static Widget button_prop_init(Widget parent, AB_PROP_TYPE type);
static int button_prop_activate(AB_PROP_TYPE type, BOOL active);
static int button_prop_clear(AB_PROP_TYPE type);
static int button_prop_load(ABObj obj, AB_PROP_TYPE type,
                            unsigned long loadkey);
static int button_prop_apply(AB_PROP_TYPE type);
static BOOL button_prop_pending(AB_PROP_TYPE type);
...
static void setup_labeltype_setting(AB_PROP_TYPE type, AB_BUTTON_TYPE btype);
static void setup_label_field(AB_PROP_TYPE type, AB_LABEL_TYPE ltype);
/*
 * Callbacks
 */
static void typeCB(Widget widget, XtPointer clientdata,
                   XmToggleButtonCallbackStruct *state);
static void labeltypeCB(Widget widget, XtPointer clientdata,
                        XtPointer calldata);
static void size_policyCB(Widget widget, XtPointer clientdata,
                          XmToggleButtonCallbackStruct *state);
...
static int button_initialize(ABObj obj) {
        if (obj_get_subtype(obj) == AB_BUT_MENU) {
                ...
                obj_set_label(obj,
                              catgets(Dtb_project_catd, 6, 66, "Menu Button"));
        } else /* Push or Drawn */
        {
...
static Widget button_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        if (type == AB_PROP_REVOLVING) {
                ...
                XtVaSetValues(parent, XmNuserData, pbs->current_obj, NULL);
        } else /* AB_PROP_FIXED */
        ...
        if (dtb_button_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_REVOLVING)
                        XtVaSetValues(parent, XmNuserData, pbs->current_obj,
                                      NULL);
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_button_palitem, cgen->ok_button,
                ...
                item_val[n] = AB_BUT_PUSH;
                n++;
                item[n] = cgen->buttontype_rbox_items.Drawn_item;
                item_val[n] = AB_BUT_DRAWN;
                n++;
                item[n] = cgen->buttontype_rbox_items.Menu_item;
                item_val[n] = AB_BUT_MENU;
                n++;
                prop_radiobox_init(&(pbs->button_type),
                                   cgen->buttontype_rbox_label,
                                   cgen->buttontype_rbox, n, item,
                                   (XtPointer *)item_val, cgen->buttontype_cb);
                for (i = 0; i < n; i++)
                ...
                item_val[n] = AB_LABEL_ARROW_DOWN;
                n++;
                prop_options_init(
                    &(pbs->label_type), cgen->labeltype_rbox_label,
                    cgen->labeltype_rbox, cgen->labeltype_rbox_menu, n, item,
                    (XtPointer *)item_val, cgen->labeltype_cb);
                for (i = 0; i < n; i++)
                        XtAddCallback(item[i], XmNactivateCallback,
                                      (XtCallbackProc)labeltypeCB,
                                      (XtPointer)type);
                 ...
                 * we need to use a private labeltype callback (not
                 * propP_labeltypeCB). The following needs to be set in order
                 * for prop_setup_label_field() to work correctly.
                 */
                XtVaSetValues(cgen->label_field_label, XmNrecomputeSize, False,
                              XmNuserData, (XtArgVal)cgen->graphic_hint, NULL);
                ...
                item_val[n] = AB_ALIGN_CENTER;
                n++;
                item[n] = cgen->labelalign_opmenu_items.Right_item;
                item_val[n] = AB_ALIGN_RIGHT;
                n++;
                prop_options_init(
                    &(pbs->label_align), cgen->labelalign_opmenu_label,
                    cgen->labelalign_opmenu, cgen->labelalign_opmenu_menu, n,
                    item, (XtPointer *)item_val, cgen->labelalign_cb);

                /* Arrow Direction */
                n = 0;
                item[n] = cgen->arrowdir_opmenu_items.Up_item;
                item_val[n] = AB_LABEL_ARROW_UP;
                n++;
                item[n] = cgen->arrowdir_opmenu_items.Down_item;
                item_val[n] = AB_LABEL_ARROW_DOWN;
                n++;
                item[n] = cgen->arrowdir_opmenu_items.Right_item;
                item_val[n] = AB_LABEL_ARROW_RIGHT;
                n++;
                item[n] = cgen->arrowdir_opmenu_items.Left_item;
                item_val[n] = AB_LABEL_ARROW_LEFT;
                n++;
                prop_options_init(
                    &(pbs->arrow_dir), cgen->arrowdir_opmenu_label,
                    cgen->arrowdir_opmenu, cgen->arrowdir_opmenu_menu, n, item,
                    (XtPointer *)item_val, cgen->arrowdir_cb);

                /* Menu Name Setting */
                prop_menuname_init(&(pbs->menuname), type, cgen->menu_label,
                                   cgen->menu_mbutton, cgen->menu_field,
                                   cgen->name_field, cgen->menu_cb,
                                   NULL /*title*/, &(pbs->current_obj), True);

                /* Size Policy */
                n = 0;
                item[n] = cgen->sizepolicy_rbox_items.Size_of_Label_item;
                item_val[n] = SIZE_OF_CONTENTS_KEY;
                n++;
                item[n] = cgen->sizepolicy_rbox_items.Fixed_item;
                item_val[n] = SIZE_FIXED_KEY;
                n++;
                prop_radiobox_init(&(pbs->size_policy),
                                   cgen->sizepolicy_rbox_label,
                                   cgen->sizepolicy_rbox, n, item,
                                   (XtPointer *)item_val, cgen->sizepolicy_cb);

                for (i = 0; i < n; i++)
                        XtAddCallback(item[i], XmNvalueChangedCallback,
                                      (XtCallbackProc)prop_size_policyCB,
                                      (XtPointer) & (pbs->geometry));
                ...
                prop_geomfield_init(&(pbs->geometry), cgen->geom_label,
                                    ...
                                    cgen->height_field_label,
                                    cgen->height_field, cgen->geom_cb);
...
static int button_prop_clear(AB_PROP_TYPE type) {
        ...
        /* Clear Button Type*/
        prop_radiobox_set_value(&(pbs->button_type), (XtPointer)AB_BUT_PUSH,
                                False);

        /* Clear LabelType */
        prop_options_set_value(&(pbs->label_type), (XtPointer)AB_LABEL_STRING,
                               False);
        ...
        prop_options_set_value(&(pbs->arrow_dir),
                               (XtPointer)AB_LABEL_ARROW_DOWN, False);

        /* Clear Label Alignment */
        prop_options_set_value(&(pbs->label_align), (XtPointer)AB_ALIGN_CENTER,
                               False);
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pbs->init_state), AB_STATE_VISIBLE, True,
                                False);
        prop_checkbox_set_value(&(pbs->init_state), AB_STATE_ACTIVE, True,
                                False);
...
static int button_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
                            unsigned long loadkey) {
        ...
        if (load_all) {
                ...
                prop_radiobox_set_value(&(pbs->button_type), (XtPointer)btype,
                                        False);
                ...
                case AB_LABEL_ARROW_LEFT:
                        /* AB_LABEL_ARROW_* types are mapped into
                         * AB_LABEL_ARROW_DOWN */
                        prop_options_set_value(&(pbs->label_type),
                                               (XtPointer)AB_LABEL_ARROW_DOWN,
                                               False);
                        break;
                default:
                        prop_options_set_value(
                            &(pbs->label_type),
                            (XtPointer)obj_get_label_type(obj), False);
                        break;
                }
                setup_labeltype_setting(type, btype);

                /* Load Label */
                prop_setup_label_field(&(pbs->label), NULL, obj->label_type,
                                       obj_get_label(obj), AB_LINE_UNDEF);

                /* Load Label Alignment */
                prop_options_set_value(&(pbs->label_align),
                                       (XtPointer)obj_get_label_alignment(obj),
                                       False);
                ...
                prop_radiobox_set_value(&(pbs->size_policy),
                                        abobj_width_resizable(obj)
                                            ...
                                            : (XtPointer)SIZE_OF_CONTENTS_KEY,
                                        False);
...
int button_prop_apply(AB_PROP_TYPE type) {
        PropButtonSettingsRec *pbs = &(prop_button_settings_rec[type]);
        STRING value;
        BOOL size_chg = False;
        BOOL reset_bg = False;
        ...
        if (prop_changed(pbs->button_type.changebar)) {
                abobj_set_button_type(pbs->current_obj,
                                      (AB_BUTTON_TYPE)prop_radiobox_get_value(
                                          &(pbs->button_type)));
                size_chg = True;
        }
            ...
            prop_changed(pbs->arrow_dir.changebar)) {
                ...
                case AB_LABEL_ARROW_LEFT:
                        abobj_set_label(pbs->current_obj,
                                        (AB_LABEL_TYPE)prop_options_get_value(
                                            &(pbs->arrow_dir)),
                                        NULL);
                        break;
                default:
        ...
        if (prop_changed(pbs->geometry.changebar)) {
                if (abobj_width_resizable(pbs->current_obj)) {
                        new_w = prop_geomfield_get_value(&(pbs->geometry),
                                                         GEOM_WIDTH);
                        abobj_set_pixel_width(pbs->current_obj, new_w, 0);
                }
                if (abobj_height_resizable(pbs->current_obj)) {
                        new_h = prop_geomfield_get_value(&(pbs->geometry),
                                                         GEOM_HEIGHT);
        ...
        if (prop_changed(pbs->init_state.changebar)) {
                abobj_set_visible(pbs->current_obj,
                                  prop_checkbox_get_value(&(pbs->init_state),
                                                          AB_STATE_VISIBLE));
        ...
        if (prop_changed(pbs->fg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pbs->fg_color),
                                          obj_get_fg_color(pbs->current_obj),
                                          False);
        }
        if (prop_changed(pbs->bg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pbs->bg_color),
                                          obj_get_bg_color(pbs->current_obj),
                                          False);
        }
        if (prop_changed(pbs->menuname.changebar)) {
                ...
                util_free(value);
                prop_menuname_set_value(&(pbs->menuname),
                                        obj_get_menu_name(pbs->current_obj),
                                        False);
        }
        ...
        prop_geomfield_set_value(&(pbs->geometry), GEOM_HEIGHT,
                                 abobj_get_comp_height(pbs->current_obj),
                                 False);
...
static BOOL button_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_button_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pbs->name.changebar) &&
            !prop_name_ok(pbs->current_obj, pbs->name.field))
                return False;

        if ((prop_changed(pbs->label_type.changebar) ||
             prop_changed(pbs->label.changebar)) &&
            (AB_BUILTIN_ACTION)prop_options_get_value(&(pbs->label_type)) ==
                AB_LABEL_GLYPH &&
            !prop_graphic_filename_ok(pbs->label.field, False))
                return False;

        if (prop_changed(pbs->geometry.changebar) &&
            (!prop_number_ok(pbs->geometry.x_field, (STRING)XFieldStr,
                             -SHRT_MAX, SHRT_MAX) ||
             !prop_number_ok(pbs->geometry.y_field, (STRING)YFieldStr,
                             -SHRT_MAX, SHRT_MAX) ||
             !prop_number_ok(pbs->geometry.w_field, (STRING)WFieldStr, 1,
                             SHRT_MAX) ||
             !prop_number_ok(pbs->geometry.h_field, (STRING)HFieldStr, 1,
                             SHRT_MAX)))
                return False;

        if (prop_changed(pbs->fg_color.changebar) &&
            !prop_color_ok(pbs->fg_color.field))
                return False;

        if (prop_changed(pbs->bg_color.changebar) &&
            !prop_color_ok(pbs->bg_color.field))
                return False;

        if (prop_changed(pbs->menuname.changebar) &&
            !prop_obj_name_ok(pbs->menuname.field,
                              obj_get_module(pbs->current_obj), AB_TYPE_MENU,
                              "Menu"))
                return False;
...
static void typeCB(Widget item, XtPointer clientdata,
                   XmToggleButtonCallbackStruct *state) {
        ...
        if (state->set) {
                ...
                if ((btype == AB_BUT_DRAWN) || (btype == AB_BUT_MENU)) {
                        switch ((AB_LABEL_TYPE)prop_options_get_value(
                            &(pbs->label_type))) {
                        case AB_LABEL_ARROW_DOWN:
                        case AB_LABEL_ARROW_UP:
                        case AB_LABEL_ARROW_LEFT:
                        case AB_LABEL_ARROW_RIGHT:
                                prop_options_set_value(
                                    &(pbs->label_type),
                                    (XtPointer)AB_LABEL_STRING, True);
                                break;
                        default:
                                break;
                        }
...
static void setup_labeltype_setting(AB_PROP_TYPE type, AB_BUTTON_TYPE btype) {
        ...
        */
        prop_options_set_value(&(pbs->label_type),
                               prop_options_get_value(&(pbs->label_type)),
                               False);
        setup_label_field(
            type, (AB_LABEL_TYPE)prop_options_get_value(&(pbs->label_type)));
}
...
static void labeltypeCB(Widget item, XtPointer clientdata, XtPointer calldata) {
        AB_PROP_TYPE type = (AB_PROP_TYPE)clientdata;
        PropButtonSettingsRec *pbs = &(prop_button_settings_rec[type]);
...
static void size_policyCB(Widget w, XtPointer clientdata,
                          XmToggleButtonCallbackStruct *state) {
        AB_PROP_TYPE type = (AB_PROP_TYPE)clientdata;
        PropButtonSettingsRec *pbs = &(prop_button_settings_rec[type]);
        XtArgVal value;
        ...
        if (state->set) {
                ...
                ui_field_set_editable(pbs->geometry.w_field,
                                      value == SIZE_FIXED_KEY);
                ui_field_set_editable(pbs->geometry.h_field,
                                      value == SIZE_FIXED_KEY);
        }
}
