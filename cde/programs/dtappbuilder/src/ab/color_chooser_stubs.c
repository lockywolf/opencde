...
static void swatchCB(Widget w, XtPointer clientdata, XtPointer calldata);

static void swatch_resizeCB(Widget w, XtPointer clientdata, XtPointer calldata);
...
static void create_color_palette(Widget parent);
        ...
        if (first_time) {
                ...
                if (Shown_colors != Num_colors) {
                        update_color_palette(
                            dtb_color_chooser_dialog.swatch_panel);
                }
        }

        XtVaSetValues(dtb_color_chooser_dialog.dialog_shellform, XmNdialogStyle,
                      XmDIALOG_FULL_APPLICATION_MODAL, NULL);

        /* Clear out place-holder name in color_name field */
        label = XmStringCreateLocalized("");
        XtVaSetValues(dtb_color_chooser_dialog.color_name, XmNlabelString,
                      label, NULL);
        XmStringFree(label);
...
void create_color_chooser() {
        extern Widget AB_toplevel;
        XmString label;

        dtbColorChooserDialogInfo_clear(&dtb_color_chooser_dialog);
        dtb_color_chooser_dialog_initialize(&dtb_color_chooser_dialog,
                                            AB_toplevel);
...
void create_color_palette(Widget parent) {
        ...
        if (depth == 1) {
                ...
                for (Num_colors = 0; Color[Num_colors] != (char *)NULL;
                     Num_colors++)
                        ;
        ...
        Panel = XtVaCreateWidget(
            "color_panel", xmFormWidgetClass, parent, XmNfractionBase,
            Rows * Cols, XmNrightOffset, 10, XmNrightAttachment, XmATTACH_FORM,
            XmNleftOffset, 10, XmNleftAttachment, XmATTACH_FORM, XmNtopOffset,
            10, XmNtopAttachment, XmATTACH_FORM, XmNbottomOffset, 10,
            XmNbottomAttachment, XmATTACH_FORM, NULL);

        /* Create the dummy widget used to control color name-to-pixel caching
         */
        CacheAnchor = XtVaCreateWidget("CaChEaNcHoR", xmLabelWidgetClass,
                                       parent, XmNlabelString, label, NULL);

        for (i = 0, Shown_colors = 0; i < Num_colors; i++) {
                /* See if we can allocate a color */
                if ((color = name_to_pixel(CacheAnchor, Color[i])) ==
                    ((Pixel)-1)) {
                        continue;
                }
                /* The color is available, so create a swatch to show it */
                tile =
                    XCreatePixmap(XtDisplay(AB_toplevel), XtWindow(AB_toplevel),
                                  SWATCH_SZ, SWATCH_SZ, depth);
                gcv.foreground = color;
                gcm = GCForeground;
                gc = XtGetGC(AB_toplevel, gcm, &gcv);
                XFillRectangle(XtDisplay(AB_toplevel), tile, gc, 0, 0,
                               SWATCH_SZ, SWATCH_SZ);

                r = Shown_colors / Cols;
                c = Shown_colors % Cols;
                swatch = XtVaCreateWidget(
                    "swatch", xmDrawnButtonWidgetClass, Panel, XmNlabelType,
                    XmPIXMAP, XmNlabelPixmap, tile, XmNforeground, color,
                    XmNshadowType, XmSHADOW_OUT, XmNshadowThickness, SHADOW_SZ,
                    XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition,
                    c * Rows, XmNrightAttachment, XmATTACH_POSITION,
                    XmNrightPosition, (c + 1) * Rows, XmNtopAttachment,
                    XmATTACH_POSITION, XmNtopPosition, r * Cols,
                    XmNbottomAttachment, XmATTACH_POSITION, XmNbottomPosition,
                    (r + 1) * Cols, XmNuserData,
                    i, /* Save color index for later use */
                    NULL);
                XtAddCallback(swatch, XmNactivateCallback, swatchCB,
                              (XtPointer)0);
                XtAddCallback(swatch, XmNresizeCallback, swatch_resizeCB,
                              (XtPointer)0);
        ...
        XtVaSetValues(dtb_color_chooser_dialog.color_name, XmNuserData, 0,
                      NULL);
        ...
        if (Shown_colors != Num_colors) {
                dtb_color_chooser_message_initialize(
                    &dtb_color_chooser_message);
                dtb_show_message(parent, &dtb_color_chooser_message, NULL,
                                 NULL);
        }
}
...
static void swatchCB(Widget w, XtPointer clientdata, XtPointer calldata) {
        ...
        */
        ...
        XtVaSetValues(dtb_color_chooser_dialog.color_name, XmNuserData, w,
                      NULL);
}

void swatch_resizeCB(Widget w, XtPointer clientdata, XtPointer calldata) {
        Pixmap tile;
        Dimension wid, ht;
        int depth;
        XGCValues gcv;
        ...
        XtVaGetValues(w, XmNheight, &ht, XmNwidth, &wid, XmNlabelPixmap, &tile,
        ...
        ht -= 4 * SHADOW_SZ;
        depth = DefaultDepthOfScreen(XtScreen(AB_toplevel));
        tile = XCreatePixmap(XtDisplay(AB_toplevel), XtWindow(AB_toplevel), wid,
                             ht, depth);
        gcv.foreground = color;
        gcm = GCForeground;
        gc = XtGetGC(AB_toplevel, gcm, &gcv);
        XFillRectangle(XtDisplay(AB_toplevel), tile, gc, 0, 0, wid, ht);

        XtVaSetValues(w, XmNlabelPixmap, tile, NULL);
}

void update_color_palette(Widget parent) {
        extern Widget AB_toplevel;
        Widget swatch;
        XmString label = XmStringCreateLocalized(" ");
        ...
        for (i = 0, Shown_colors = 0; i < Num_colors; i++) {
                ...
                if (Shown_colors < old_shown_colors) {
                        ...
                        util_dprintf(
                            2, "reusing swatch widget #%d for color #%d (%s)\n",
                            Shown_colors, i, Color[i]);
                        /* Destroy old tile pixmap */
                        XtVaGetValues(swatch, XmNlabelPixmap, &oldtile, NULL);
                        XFreePixmap(XtDisplay(AB_toplevel), oldtile);
                } else {
                        /* Need to create a new swatch button */
                        swatch = XtVaCreateWidget(
                            "swatch", xmDrawnButtonWidgetClass, Panel, NULL);
                        Swatch[Shown_colors] = swatch;
                        util_dprintf(
                            2,
                            "creating swatch widget #%d for color #%d (%s)\n",
                            Shown_colors, i, Color[i]);
                }
                XtVaSetValues(
                    swatch, XmNlabelType, XmPIXMAP, XmNlabelPixmap, tile,
                    XmNforeground, color, XmNshadowType, XmSHADOW_OUT,
                    XmNshadowThickness, SHADOW_SZ, XmNleftAttachment,
                    XmATTACH_POSITION, XmNleftPosition, c * Rows,
                    XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition,
                    (c + 1) * Rows, XmNtopAttachment, XmATTACH_POSITION,
                    XmNtopPosition, r * Cols, XmNbottomAttachment,
                    XmATTACH_POSITION, XmNbottomPosition, (r + 1) * Cols,
...
Pixel name_to_pixel(Widget w, String color) {
        ...
        if (XAllocNamedColor(XtDisplay(AB_toplevel), cmap, color, &exact,
                             &actual) != 0) {
                return (actual.pixel);
        } else
                return ((Pixel)-1);
