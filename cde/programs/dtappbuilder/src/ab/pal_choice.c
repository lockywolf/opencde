 ...
 * Methods
 */
static int choice_initialize(ABObj obj);
static Widget choice_prop_init(Widget parent, AB_PROP_TYPE type);
static int choice_prop_activate(AB_PROP_TYPE type, BOOL active);
static int choice_prop_load(ABObj obj, AB_PROP_TYPE type,
                            unsigned long loadkey);
static int choice_prop_clear(AB_PROP_TYPE type);
static int choice_prop_apply(AB_PROP_TYPE type);
static BOOL choice_prop_pending(AB_PROP_TYPE type);

static BOOL verify_props(AB_PROP_TYPE type);
static void turnoff_changebars(AB_PROP_TYPE type);

static void load_item(ABObj iobj, AB_PROP_TYPE type);
static ABObj init_new_item(AB_PROP_TYPE type);
static void change_item(AB_PROP_TYPE type, ABObj iobj);

static void setup_type_settings(AB_PROP_TYPE type, AB_CHOICE_TYPE choice_type);
/*
 * Xt Callbacks
 */
static void typeCB(Widget widget, XtPointer clientdata, XtPointer calldata);
/*
 * ABObj callbacks
 */
static int prop_choice_install_obj_destroy_CB(void);

static int prop_choice_obj_destroy_CB(ObjEvDestroyInfo destroyInfo);
...
static int choice_initialize(ABObj obj) {
        ...
        case AB_CHOICE_NONEXCLUSIVE:
                ...
                obj_set_label(obj,
                              catgets(Dtb_project_catd, 100, 253, "Choice:"));
                break;
        case AB_CHOICE_OPTION_MENU:
                obj_set_unique_name(obj, "optionmenu");
                obj_set_label(obj,
                              catgets(Dtb_project_catd, 100, 254, "Options:"));
                break;
...
static Widget choice_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        if (dtb_choice_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_REVOLVING)
                        XtVaSetValues(parent, XmNuserData, pcs->current_obj,
                                      NULL);
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_choice_palitem, cgen->ok_button,
                ...
                n++;
                prop_options_init(
                    &(pcs->label_type), cgen->labeltype_rbox_label,
                ...
                prop_label_field_init(&(pcs->label), cgen->graphic_hint, item,
                                      n);
                ...
                item_val[n] = AB_CHOICE_EXCLUSIVE;
                n++;
                item[n] = cgen->choicetype_rbox_items.Check_Box_item;
                item_val[n] = AB_CHOICE_NONEXCLUSIVE;
                n++;
                item[n] = cgen->choicetype_rbox_items.Option_Menu_item;
                item_val[n] = AB_CHOICE_OPTION_MENU;
                n++;
                prop_options_init(
                    &(pcs->choice_type), cgen->choicetype_rbox_label,
                    cgen->choicetype_rbox, cgen->choicetype_rbox_menu, n, item,
                    (XtPointer *)item_val, cgen->choicetype_cb);

                for (i = 0; i < n; i++)
                        XtAddCallback(item[i], XmNactivateCallback, typeCB,
                                      (XtPointer)type);
                ...
                prop_radiobox_init(&(pcs->row_col), cgen->layout_rbox_label,
                                   ...
                                   (XtPointer *)item_val, cgen->layout_cb);

                prop_field_init(&(pcs->row_col_count), NULL, cgen->layout_field,
                                cgen->layout_cb);
                ...
                n++;
                prop_options_init(
                    &(pcs->item_label_type), cgen->itemlabel_type_label,
                    cgen->itemlabel_opmenu, cgen->itemlabel_opmenu_menu, n,
                ...
                prop_item_editor_init(
                    &(pcs->items), AB_ITEM_FOR_CHOICE, cgen->itemlist,
                    cgen->itemlist_cb, cgen->item_add_button, n, item,
                    item_val,            /* Insert */
                    j, item2, item2_val, /* Edit */
                    &(pcs->item_label), &(pcs->item_label_type),
                    cgen->graphic_hint3, NULL, NULL, NULL, &(pcs->item_state),
                    NULL, &(pcs->current_obj));
...
static int choice_prop_clear(AB_PROP_TYPE type) {
        ...
        prop_options_set_value(&(pcs->label_type), (XtPointer)AB_LABEL_STRING,
                               False);
        ...
        /* Clear Choice Type*/
        prop_options_set_value(&(pcs->choice_type),
                               (XtPointer)AB_CHOICE_EXCLUSIVE, False);

        /* Clear Row/Columns */
        prop_radiobox_set_value(&(pcs->row_col),
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pcs->init_state), AB_STATE_VISIBLE, True,
                                False);
        prop_checkbox_set_value(&(pcs->init_state), AB_STATE_ACTIVE, True,
                                False);
...
static int choice_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
                            unsigned long loadkey) {
        PropChoiceSettingsRec *pcs = &(prop_choice_settings_rec[type]);
        ...
        if (load_all) {
                ...
                prop_options_set_value(&(pcs->label_type),
                                       (XtPointer)obj->label_type, False);
                prop_options_set_value(&(pcs->label_pos),
                                       (XtPointer)obj_get_label_position(obj),
                                       False);

                /* Load Label */
                prop_setup_label_field(&(pcs->label), NULL, obj->label_type,
                                       obj_get_label(obj), AB_LINE_UNDEF);
                /* Load Choice Type */
                ctype = obj_get_choice_type(obj);
                prop_options_set_value(&(pcs->choice_type), (XtPointer)ctype,
                                       False);
                setup_type_settings(type, ctype);

                /* Load Row/Column*/
                prop_radiobox_set_value(&(pcs->row_col),
                                        (XtPointer)obj_get_orientation(obj),
                                        False);
                prop_field_set_numeric_value(&(pcs->row_col_count),
        ...
        if (prop_changed(pcs->label.changebar) ||
            prop_changed(pcs->label_type.changebar)) {
                ...
                abobj_set_label_position(
                    pcs->current_obj, (AB_COMPASS_POINT)prop_options_get_value(
                                          &(pcs->label_pos)));
        ...
        */
        ...
        if (prop_changed(pcs->init_state.changebar)) {
                abobj_set_visible(pcs->current_obj,
                                  prop_checkbox_get_value(&(pcs->init_state),
                                                          AB_STATE_VISIBLE));
        ...
        if (prop_changed(pcs->fg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pcs->fg_color),
                                          obj_get_fg_color(pcs->current_obj),
                                          False);
        }
        if (prop_changed(pcs->bg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pcs->bg_color),
                                          obj_get_bg_color(pcs->current_obj),
                                          False);
        }
        ...
        if (prop_changed(pcs->choice_type.changebar)) {
                abobj_set_choice_type(pcs->current_obj,
                                      (AB_CHOICE_TYPE)prop_options_get_value(
                                          &(pcs->choice_type)));
                size_chg = True;
        }
...
static BOOL choice_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_choice_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if ((prop_changed(pcs->label_type.changebar) ||
             prop_changed(pcs->label.changebar)) &&
            (AB_BUILTIN_ACTION)prop_options_get_value(&(pcs->label_type)) ==
                AB_LABEL_GLYPH &&
            !prop_graphic_filename_ok(pcs->label.field, False))
                return False;

        if (prop_changed(pcs->row_col.changebar) &&
            !prop_number_ok(pcs->row_col_count.field, "Row/Column Field", 1,
                            99))
                return False;

        if (prop_changed(pcs->pos.changebar) &&
            (!prop_number_ok(pcs->pos.x_field, (STRING)XFieldStr, -SHRT_MAX,
                             SHRT_MAX) ||
             !prop_number_ok(pcs->pos.y_field, (STRING)YFieldStr, -SHRT_MAX,
                             SHRT_MAX)))
                return False;

        if (prop_changed(pcs->fg_color.changebar) &&
            !prop_color_ok(pcs->fg_color.field))
                return False;

        if (prop_changed(pcs->bg_color.changebar) &&
            !prop_color_ok(pcs->bg_color.field))
                return False;
...
static void typeCB(Widget item, XtPointer clientdata, XtPointer call_data) {
 ...
 */
static int prop_choice_install_obj_destroy_CB() {
