...
static void process_chord_extend(Widget widget);
static void setup_drag(Widget widget, XButtonEvent *button_ev);
static void cleanup_drag(Widget widget, XButtonEvent *button_ev);
static DRAG_DIR find_drag_dirn(int start_x, int start_y, int new_x, int new_y);
static ABObj get_obj_at_point(ABObj project, Boolean in_browser, Window window,
                              int x, int y);
static ABObj ui_get_obj_at_point(ABObj project, Window window, int x, int y);
static Boolean is_win_decor(Widget win_wid, Window window);
static Boolean find_win_match(Display *display, Window top_win,
                              Window target_win, Window limit_win);
static void preview_object(ABObj obj, Boolean in_browser, Window win);
static void ui_preview_object(ABObj obj);

extern void conn_drag_chord(Widget widget, XEvent *event, String *params,
                            Cardinal num_params) {
        ...
        XTranslateCoordinates(display, window, root_win, button_ev->x,
                              button_ev->y, &start_x, &start_y, &child_win);
        ...
        source_obj =
            get_obj_at_point(project, in_browser, source_win, win_x, win_y);
        if (source_obj == NULL) {
                XBell(display, 0);
                return;
        }
        ...
        x_conn_fullscreen_chord(widget, root_win, start_x, start_y, cur_x,
                                cur_y);
                ...
                case MotionNotify:
                        if (new_event.xmotion.root != root_win ||
                            (XEventsQueued(display, QueuedAfterReading) > 0 &&
                             (XPeekEvent(display, &extra),
                              extra.type == MotionNotify)))
                                break;
                        else
        ...
        if (target_obj != NULL) {
                ...
                XBell(display, 0);
                fprintf(stderr,
                        "Connection chord not dropped on a valid target.\n");
        }
...
static void process_chord_extend(Widget widget) {
        ...
        if (!XQueryPointer(display, root_win, &dum_ret, &dum_ret, &new_x,
                           &new_y, &dum_x, &dum_y, &mask_ret))
                return;
        ...
        x_conn_fullscreen_chord(widget, root_win, start_x, start_y, new_x,
                                new_y);

        new_win = x_xwin_at_rootxy(widget, new_x, new_y, &win_x, &win_y);
        ...
        if (in_browser) {
                if (new_win != target_win) {
                        ...
                        if (new_obj != target_obj) {
                                preview_object(target_obj, in_browser,
                                               target_win);
                ...
                new_obj = get_obj_at_point(project, in_browser, new_win, win_x,
                                           win_y);
                if (new_obj != target_obj) {
...
static void setup_drag(Widget widget, XButtonEvent *button_ev) {
        ...
        if (XGrabPointer(display, root_win, True,
                         ...
                         GrabModeAsync, root_win, cur_cursor,
                         button_ev->time) != GrabSuccess) {
                fprintf(stderr, "Pointer grab failed.\n");
                return;
        }
        if (XGrabKeyboard(display, root_win, False, GrabModeAsync,
                          GrabModeAsync, button_ev->time) != GrabSuccess) {
...
static ABObj ui_get_obj_at_point(ABObj project, Window window, int x, int y) {
        ...
        return ui_obj;
             ...
             win_translate_xy(window, XtWindow(ui_obj->ui_handle), x, y, &newx,
             &newy);
             ...
             for (obj = G_tail_fn(G_itf); (obj && obj != win); obj = obj->previous)
             {
                 ...
                 {
                      * If it's a setting, also identify which particular setting
             item
                      * it's pointing at.
...
static Boolean is_win_decor(Widget win_wid, Window window) {
         ...
         *
         */
        for (; XQueryTree(display, query_win, &qroot, &parent, &children,
                          &num_children) &&
               parent != qroot;
             query_win = parent) {
...
static Boolean find_win_match(Display *display, Window top_win,
        ...
        ret_val = FALSE;
        if (XQueryTree(display, top_win, &qroot, &parent, &children,
                       &num_children)) {
                unsigned int i;

                for (i = 0; i < num_children; i++) {
                        if ((ret_val = find_win_match(display, children[i],
                                                      target_win, limit_win)) ==
                            TRUE)
                                break;
                }
                XFree((XPointer)children);
...
static void ui_preview_object(ABObj obj) {
        ...
        XTranslateCoordinates(dpy, XtWindow(parent), rootwin, p_rect.x,
                              p_rect.y, &trans_x, &trans_y, &win);
        p_rect.x = trans_x;
        p_rect.y = trans_y;
        x_fullscreen_preview_box(gwidget, rootwin, p_rect.x, p_rect.y,
                                 rect_right(&p_rect), rect_bottom(&p_rect));
}
