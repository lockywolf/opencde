...
static Widget ConnP_view_source_obj =
    NULL; /* View:'s "Source Object" PushButton */
...
static void init_exec_code_dialog(DtbConnExecCodeDialogInfo instance);

static void populate_src_menu(Widget widget, XtPointer client_data,
                              XtPointer call_data);
static void populate_target_menu(Widget widget, XtPointer client_data,
                                 XtPointer call_data);
static void populate_when_menu(Widget widget, XtPointer client_data,
                               XtPointer call_data);
static void populate_action_menu(Widget widget, XtPointer client_data,
                                 XtPointer call_data);
static void populate_view_menu(Widget widget, XtPointer client_data,
                               XtPointer call_data);
static void populate_list(Widget list, ABObj cur_selection, BOOL invoke_cb);
static void populate_connection_list(ABObj cur_action_obj);
static void change_src_type(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void change_target_type(Widget w, XtPointer client_data,
                               XtPointer call_data);
static void change_arg_sensitivity(Widget w, XtPointer client_data,
                                   XtPointer call_data);
static void set_view_filter(Widget w, XtPointer client_data,
                            XtPointer call_data);
static BOOL is_action_type(ABObj obj, int filter);
static void update_cur_src(Widget w, XtPointer client_data,
                           XtPointer call_data);
static void update_cur_target(Widget w, XtPointer client_data,
                              XtPointer call_data);
static void select_connection(Widget w, XtPointer client_data,
                              XtPointer call_data);

static void setup_source(AB_OBJECT_TYPE source_type, int source_subtype,
                         ABObj source);
static void setup_target(AB_OBJECT_TYPE target_type, int target_subtype,
                         ABObj target);
static int get_cur_when(void);
static AB_BUILTIN_ACTION get_cur_act(void);
static void set_cur_when(AB_WHEN new_when);
static void set_cur_act(AB_BUILTIN_ACTION new_act);
static ABObj get_cur_connection(void);
static void set_standard_action_type(void);
static void set_call_function_action_type(void);
static void set_execute_code_action_type(void);
static void set_on_item_help_action_type(void);
static void set_access_help_vol_action_type(void);

static void set_conn_controls(BOOL state);
static void hide_execute_code_win(void);
static BOOL verify_builtin_arg_string(AB_BUILTIN_ACTION c_act, ABObj c_target,
                                      Widget field);
static void update_on_src_type_change(AB_OBJECT_TYPE obj_type, int obj_subtype);
static void update_on_target_type_change(AB_OBJECT_TYPE obj_type,
                                         int obj_subtype);
static void update_conn_ed_controls(void);
static AB_FUNC_TYPE get_cur_func_type(void);

static BOOL src_list_test(ABObj test_obj);
static BOOL target_list_test(ABObj test_obj);
static void set_ctrls_for_src(BOOL state);
static void set_ctrls_for_target(BOOL state);
...
extern void conn_popup_dialog(Widget widget, XtPointer client_data,
        ...
        if (AB_conn_dialog == (Widget)NULL) {
                 ...
                 */
                ab_register_window(AB_conn_dialog, AB_WIN_DIALOG, WindowHidden,
                                   AB_toplevel, AB_WPOS_TILE_HORIZONTAL,
                                   conn_cancelCB,
                                   (XtPointer)&dtb_conn_conn_dialog);
...
static void init_exec_code_dialog(DtbConnExecCodeDialogInfo instance) {
        ...
        ab_register_window(exec_code_dialog, AB_WIN_DIALOG, WindowHidden,
                           AB_conn_dialog, AB_WPOS_STACK_CENTER, NULL, NULL);
}
...
static void populate_src_menu(Widget widget, XtPointer client_data,
        ...
        for (i = 0; i < ConnP_num_conn_objs; i++) {
                ...
                w = XtVaCreateManagedWidget(
                    "src_menu_push", xmPushButtonWidgetClass, widget,
                    XtVaTypedArg, XmNlabelString, XtRString,
                    ConnP_conn_objs[i].label,
                    strlen(ConnP_conn_objs[i].label) + 1, NULL);
        ...
        for (i = 0; i < ConnP_num_conn_objs; i++) {
                ...
                if ((ConnP_conn_objs[i].obj_type != AB_TYPE_MESSAGE) &&
                    ...
                    (ConnP_conn_objs[i].obj_type != AB_TYPE_MENU)) {
                        w = XtVaCreateManagedWidget(
                            "target_menu_push", xmPushButtonWidgetClass, widget,
                            XtVaTypedArg, XmNlabelString, XtRString,
                            ConnP_conn_objs[i].label,
                            strlen(ConnP_conn_objs[i].label) + 1, NULL);
                        XtAddCallback(w, XmNactivateCallback,
...
static void populate_when_menu(Widget widget, XtPointer client_data,
        ...
        if (num_labels > 0) {
                ...
                for (i = 0; i < num_labels; i++) {
                        ...
                        w = XtVaCreateWidget(
                            "when_menu_push", xmPushButtonWidgetClass,
                            when_pulldown, XtVaTypedArg, XmNlabelString,
                            XtRString, labels[i], strlen(labels[i]) + 1, NULL);
                        w_list[i] = w;
                }
                w_list[i] = NULL;
        }
...
static void populate_action_menu(Widget widget, XtPointer client_data,
        ...
        if (num_labels > 0) {
                ...
                for (i = 0; i < num_labels; i++) {
                        ...
                        w = XtVaCreateWidget(
                            "action_menu_push", xmPushButtonWidgetClass,
                            action_pulldown, XtVaTypedArg, XmNlabelString,
                            XtRString, labels[i], strlen(labels[i]) + 1, NULL);
                        XtAddCallback(w, XmNactivateCallback,
...
static void populate_view_menu(Widget widget, XtPointer client_data,
        ...
        w = XtVaCreateManagedWidget("view_menu_push", xmPushButtonWidgetClass,
                                    widget, XtVaTypedArg, XmNlabelString,
                                    XtRString, "Source Object",
                                    sizeof("Source Object"), NULL);
        XtAddCallback(w, XmNactivateCallback, set_view_filter, (XtPointer)-1);
        ConnP_view_source_obj = w;

        for (i = 0; i < ConnP_num_conn_objs; i++) {
                w = XtVaCreateManagedWidget(
                    "view_menu_push", xmPushButtonWidgetClass, widget,
                    XtVaTypedArg, XmNlabelString, XtRString,
                    ConnP_conn_objs[i].label,
                    strlen(ConnP_conn_objs[i].label) + 1, NULL);
                XtAddCallback(w, XmNactivateCallback, set_view_filter,
                              (XtPointer)i);
        }
}
...
static void setup_source(AB_OBJECT_TYPE source_type, int source_subtype,
        ...
        XtVaSetValues(XmOptionButtonGadget(src_menu), XtVaTypedArg,
                      XmNlabelString, XtRString, ConnP_conn_objs[i].label,
                      strlen(ConnP_conn_objs[i].label) + 1, NULL);
...
static void setup_target(AB_OBJECT_TYPE target_type, int target_subtype,
        ...
        XtVaSetValues(XmOptionButtonGadget(target_menu), XtVaTypedArg,
                      XmNlabelString, XtRString, ConnP_conn_objs[i].label,
                      strlen(ConnP_conn_objs[i].label) + 1, NULL);
...
static BOOL src_list_test(ABObj test_obj) {
        ...
        if ((cur_obj_type == AB_TYPE_PROJECT) && (obj_type == cur_obj_type)) {
                ret = TRUE;
        } else if ((mod != NULL) && obj_has_flag(mod, MappedFlag) &&
                   obj_is_defined(mod)) {
                if (connP_objtype_needs_subtype(cur_obj_type,
                                                cur_obj_subtype)) {
                        if (!obj_is_sub(test_obj) && obj_is_defined(test_obj) &&
                        ...
                        if (!obj_is_container(test_obj) &&
                            !obj_is_sub(test_obj) && obj_is_defined(test_obj) &&
                            obj_type == cur_obj_type) {
                                ret = TRUE;
                ...
                if (is_action_type(obj, ConnP_view_filter)) {
                        ...
                        if ((cur_action_obj == obj) ||
                            (selected_conn && (selected_conn == obj))) {
                                ui_list_select_item(connection_list, item_str,
                                                    FALSE);
...
static BOOL is_action_type(ABObj obj, int filter) {
        ...
        if (filter == -1) {
                ...
                if (obj_subtype != -1)
                        ret_val &= (obj_get_subtype(obj->info.action.from) ==
                                    obj_subtype);
        }
cret:
        return (ret_val);
                ...
                if (selected_obj) {
                        ...
                        if (obj_is_message(selected_obj)) {
                                if (obj_has_action1_button(selected_obj)) {
                                        XtVaSetValues(
                                            XmOptionButtonGadget(when_menu),
                                            XtVaTypedArg, XmNlabelString,
                                            XtRString,
                                            ConnP_conn_whens[0].label,
                                            strlen(ConnP_conn_whens[0].label) +
                                                1,
                                            NULL);
                                } else if (obj_has_action2_button(
                                               selected_obj)) {
                                        XtVaSetValues(
                                            XmOptionButtonGadget(when_menu),
                                            XtVaTypedArg, XmNlabelString,
                                            ...
                                            strlen(ConnP_conn_whens[1].label) +
                                                1,
                                            NULL);
                                } else if (obj_has_action3_button(
                                               selected_obj)) {
                                        XtVaSetValues(
                                            XmOptionButtonGadget(when_menu),
                                            XtVaTypedArg, XmNlabelString,
                                            ...
                                            strlen(ConnP_conn_whens[2].label) +
                                                1,
                                            NULL);
                                } else if (obj_has_cancel_button(
                                               selected_obj)) {
                                        XtVaSetValues(
                                            XmOptionButtonGadget(when_menu),
                                            XtVaTypedArg, XmNlabelString,
                                            ...
                                            strlen(ConnP_conn_whens[3].label) +
                                                1,
                                            NULL);
                                }
                        }
                         ...
                         * item, then set the "When" to be "Toggled"
                         * ["Activated" is made inactive in
                         * connP_update_when_menu()]. If the the item is an
                         * optionmenu item, then make the "When"
                         * ["Toggled" is made inactive in
                         * connP_update_when_menu()].
                         */
                        if (obj_is_choice_item(selected_obj)) {
                                if (obj_is_option_menu(obj_get_root(
                                        obj_get_parent(selected_obj))))
                                        XtVaSetValues(
                                            XmOptionButtonGadget(when_menu),
                                            XtVaTypedArg, XmNlabelString,
                                            XtRString,
        ...
        if (name = objxm_xmstr_to_str(list_cl->item)) {
                ...
                if (selected_obj) {
                        ...
                        connP_guess_when_action(
                            connP_get_source_type(), connP_get_source_subtype(),
                            connP_get_target_type(), connP_get_target_subtype(),
                            &g_when, &g_func, &g_act);
...
static void select_connection(Widget w, XtPointer client_data,
        ...
        if (connP_get_source_type() != obj_get_type(cur_info->from)) {
                if (connP_objtype_needs_subtype(
                        obj_get_type(cur_info->from),
                        obj_get_subtype(cur_info->from))) {
                        if (connP_get_source_subtype() !=
                            obj_get_subtype(cur_info->from)) {
                ...
                conn_set_source(cur_info->from);
                ui_list_select_item(
                    src_list, abobj_get_moduled_name(cur_info->from), FALSE);
        }
        ...
        case AB_FUNC_BUILTIN:
                ...
                if (connP_get_target_type() != obj_get_type(cur_info->to)) {
                        if (connP_objtype_needs_subtype(
                                obj_get_type(cur_info->to),
                                obj_get_subtype(cur_info->to))) {
                                if (connP_get_target_subtype() !=
                                    obj_get_subtype(cur_info->to)) {
                        ...
                        conn_set_target(cur_info->to);
                        ui_list_select_item(
                            target_list, abobj_get_moduled_name(cur_info->to),
                            FALSE);
                }
        ...
        case AB_FUNC_USER_DEF: {
                ...
                if (strcmp(cur_func_name, func_name) != 0) {
                        if (func_name != (STRING)NULL)
                                XtVaSetValues(arg_field, XmNvalue, func_name,
                                              NULL);
                }
                break;
        }
...
static void set_cur_when(AB_WHEN new_when) {
        ...
        if (i < ConnP_num_conn_whens) {
                XtVaSetValues(XmOptionButtonGadget(when_menu), XtVaTypedArg,
                              XmNlabelString, XtRString,
                              ConnP_conn_whens[i].label,
                              strlen(ConnP_conn_whens[i].label) + 1, NULL);
        }
}
static void set_cur_act(AB_BUILTIN_ACTION new_act) {
        register int i;
        ...
        if (i < ConnP_num_conn_acts) {
                XtVaSetValues(XmOptionButtonGadget(action_menu), XtVaTypedArg,
                              XmNlabelString, XtRString,
                              ConnP_conn_acts[i].label,
                              strlen(ConnP_conn_acts[i].label) + 1, NULL);
                change_arg_sensitivity(arg_field, (XtPointer)NULL,
                                       (XtPointer)NULL);
        }
}
...
static ABObj get_cur_connection(void) {
         ...
         */
        XtVaGetValues(connection_list, XmNselectedItemCount, &sel_count,
                      XmNselectedItems, &sel_items, XmNuserData, &obj_list,
                      NULL);
...
static void set_standard_action_type(void) {
        ...
        xm_label_str = XmStringCreateLocalized(
            catgets(Dtb_project_catd, 100, 54, ARGUMENT_LABEL));
...
static void set_call_function_action_type(void) {
        ...
        xm_label_str = XmStringCreateLocalized(
            catgets(Dtb_project_catd, 100, 55, FUNCTION_LABEL));
        XtVaSetValues(arg_label, XmNlabelString, xm_label_str, NULL);
        XmStringFree(xm_label_str);
...
static void set_execute_code_action_type(void) {
        ...
        xm_label_str = XmStringCreateLocalized(
            catgets(Dtb_project_catd, 100, 56, CODE_LABEL));
        XtVaSetValues(arg_label, XmNlabelString, xm_label_str, NULL);
        XmStringFree(xm_label_str);
...
extern int connP_update_on_obj_destroy(ObjEvDestroyInfo info) {
        ABObj obj = info->obj, project;
        ...
        if (AB_conn_dialog != (Widget)NULL) {
                ...
                abobj_list_obj_destroyed(target_list, info->obj,
                                         target_list_test);
                populate_connection_list(connP_get_connection());
...
extern int connP_update_on_obj_rename(ObjEvAttChangeInfo info) {
        ...
        if ((obj != NULL) && (AB_conn_dialog != (Widget)NULL)) {
                ab_type = obj_get_type(obj);
                if ((ab_type != AB_TYPE_UNDEF) &&
                    (ab_type != AB_TYPE_UNKNOWN) && !obj_is_project(obj)) {
        ...
        if ((obj != NULL) && (AB_conn_dialog != (Widget)NULL)) {
                ...
                if (obj_is_module(obj) && !obj_has_flag(obj, MappedFlag)) {
                        ...
                        if (conn = connP_get_connection()) {
                                cur_conn_src_mod =
                                    obj_get_module(obj_get_from(conn));
                                cur_conn_target_mod =
                                    obj_get_module(obj_get_to(conn));
...
BOOL connP_conn_is_possible() {
        BOOL ConnIsPossible = FALSE;
        ABObj source = NULL, target = NULL;
...
)
{
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
        ...
        STRING vol_value = NULL, loc_value = NULL;
        BOOL valid = False;
        ...
        case AB_FUNC_BUILTIN:
                if ((c_target = connP_get_target()) != NULL &&
                    connP_get_target_type() == obj_get_type(c_target)) {
                        ...
                        case AB_ARG_STRING:
                                if (!verify_builtin_arg_string(c_act, c_target,
                                                               arg_field))
                                        return;

                                XtVaGetValues(arg_field, XmNvalue, &str_value,
                                              NULL);
                                if (str_value != (char *)NULL)
                                        connP_set_conn_arg(ab_action,
                                                           AB_ARG_STRING,
                                                           str_value);
                                break;
                        ...
                        case AB_ARG_INT:
                                ...
                                if (str_value != (char *)NULL)
                                        connP_set_conn_arg(
                                            ab_action, AB_ARG_INT, str_value);
                                break;
                        ...
                        /* Set SaveNeeded flag */
                        if (obj_get_module(c_source) ==
                            obj_get_module(c_target)) {
                                abobj_set_save_needed(obj_get_module(c_source),
                                                      TRUE);
                        } else {
                                abobj_set_save_needed(obj_get_project(c_source),
                                                      TRUE);
        ...
        case AB_FUNC_CODE_FRAG:
                ...
                if (!util_strempty(str_value)) {
                        ab_action = obj_create(AB_TYPE_ACTION, NULL);
                        connP_make_code_frag_conn(ab_action, c_source, c_when,
                                                  str_value);
        ...
        case AB_FUNC_HELP_VOLUME:
                ...
                ab_action = obj_create(AB_TYPE_ACTION, NULL);
                connP_make_help_vol_conn(ab_action, c_source, c_when, vol_value,
                                         loc_value);
        ...
        if (ab_action != (ABObj)NULL) {
                if (is_cross_module) {
                        ...
                        obj_add_action(obj_is_project(c_source)
                                           ? c_source
                                           : obj_get_module(c_source),
                                       ab_action);
                }
                ...
                /*
                 * cmvc 4359 - make View: be Source Object, if it is not already
                 * or if the View: is not the Source: connection being made.
                 */
                if (!((ConnP_view_filter == -1) ||
                      (ConnP_view_filter ==
                       connP_get_obj_type_index(connP_get_source_type(),
                                                connP_get_source_subtype())))) {
                        ConnP_view_filter = -1;
                        XtVaSetValues(view_menu, XmNmenuHistory,
                                      ConnP_view_source_obj, NULL);
                }
                populate_connection_list(connP_get_connection());
        }
...
)
{
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
        ...
        char *str_value = (char *)NULL;
        STRING loc_value = (STRING)NULL, vol_value = (STRING)NULL;
        ISTRING istr_value = (ISTRING)NULL, istr_loc_value = (ISTRING)NULL,
        ...
        case AB_FUNC_BUILTIN:
                ...
                case AB_FUNC_BUILTIN:
                        ...
                        is_changed = connP_change_in_builtin(
                            ab_action, c_source, c_target, c_when,
                ...
                case AB_FUNC_USER_DEF:
                        is_changed = TRUE;
                        if (ab_action->info.action.func_value.func_name !=
                            (ISTRING)NULL)
                                istr_destroy(ab_action->info.action.func_value
                                                 .func_name);
                        connP_make_builtin_conn(ab_action, c_source, c_target,
                                                c_when, c_act);
                        break;
                ...
                case AB_FUNC_CODE_FRAG:
                        ...
                        if (ab_action->info.action.func_value.code_frag !=
                            (ISTRING)NULL)
                                istr_destroy(ab_action->info.action.func_value
                                                 .code_frag);
                        connP_make_builtin_conn(ab_action, c_source, c_target,
        ...
        case AB_FUNC_USER_DEF:
                ...
                case AB_FUNC_USER_DEF:
                        ...
                        if (is_changed) {
                                connP_make_user_def_conn(ab_action, c_source,
                                                         c_when, str_value);
                                abobj_set_save_needed(
                                    obj_is_project(c_source)
                                        ? c_source
        ...
        case AB_FUNC_CODE_FRAG:
                ...
                case AB_FUNC_CODE_FRAG:
                        ...
                        if (is_changed) {
                                connP_make_code_frag_conn(ab_action, c_source,
                                                          c_when, str_value);
                                abobj_set_save_needed(
                                    obj_is_project(c_source)
                                        ? c_source
        ...
        case AB_FUNC_ON_ITEM_HELP:
                ...
                case AB_FUNC_ON_ITEM_HELP:
                        if (c_when != obj_get_when(ab_action)) {
                                is_changed = TRUE;
                                connP_make_on_item_help_conn(ab_action,
                                                             c_source, c_when);
                                abobj_set_save_needed(
                                    obj_is_project(c_source)
                                        ? c_source
                ...
                case AB_FUNC_USER_DEF:
                        ...
                        if (ab_action->info.action.func_value.func_name !=
                            (ISTRING)NULL)
                                istr_destroy(ab_action->info.action.func_value
                                                 .func_name);
                        connP_make_on_item_help_conn(ab_action, c_source,
                                                     c_when);
                        abobj_set_save_needed(obj_is_project(c_source)
                                                  ? c_source
                                                  : obj_get_module(c_source),
        ...
        case AB_FUNC_HELP_VOLUME:
                ...
                case AB_FUNC_HELP_VOLUME:
                        ...
                        if (ab_action->info.action.volume_id !=
                            istr_vol_value) {
                                is_changed = TRUE;
                                istr_destroy(ab_action->info.action.volume_id);
                        }
...
)
{
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
        ...
        if (obj_get_module(source) == obj_get_module(target)) {
                abobj_set_save_needed(
                    obj_is_project(source) ? source : obj_get_module(source),
                    TRUE);
        } else /* This is a cross-module connection */
        {
                abobj_set_save_needed(
...
)
{
    ...
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
         ...
         */
        XtVaSetValues(widget, XmNsubMenuId, dtbSource->source_choices_menu,
                      NULL);
...
)
{
    ...
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
         ...
         *          does not set XmNsubMenuId if Option Menu has 0 items
         */
        XtVaSetValues(widget, XmNsubMenuId, dtbSource->target_choices_menu,
                      NULL);
...
)
{
    ...
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
        ...
        XtAddCallback(XtParent(dtbSource->when_choices_menu), XmNpopupCallback,
                      connP_update_when_menu, (XtPointer)NULL);
...
)
{
    ...
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
         ...
         *          does not set XmNsubMenuId if Option Menu has 0 items
         */
        XtVaSetValues(widget, XmNsubMenuId, dtbSource->predef_act_choices_menu,
                      NULL);
...
)
{
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
        ...
        if (!(dtbSource->initialized)) {
                dtb_conn_exec_code_dialog_initialize(
                    dtbSource, dtb_palette_ab_palette_main.ab_palette_main);
...
** target dialog;
*/
BOOL allow_show_help_connection(ABObj src_obj, ABObj target_obj) {
        AB_OBJECT_TYPE src_type;
        AB_OBJECT_TYPE target_type;
        int src_subtype;

        if ((src_obj == (ABObj)NULL) || (target_obj == (ABObj)NULL))
                return (FALSE);
        ...
        if ((target_type == AB_TYPE_DIALOG) && (src_type == AB_TYPE_BUTTON) &&
...
static void hide_execute_code_win(void) {
        ...
        if (!(instance->initialized)) {
                dtb_conn_exec_code_dialog_initialize(
                    instance, dtb_palette_ab_palette_main.ab_palette_main);
 ...
 */
void connP_update_when_menu(Widget widget, XtPointer clientData,
                            XtPointer callData) {
        ...
        case AB_TYPE_ITEM:
                if (obj_is_choice_item(cur_obj)) {
                        if (obj_is_option_menu(
                                obj_get_root(obj_get_parent(cur_obj)))) {
                                XtSetSensitive(
                                    w_list[ConnP_conn_whens[2].when_type - 1],
                                    True);
                                XtSetSensitive(
                                    w_list[ConnP_conn_whens[3].when_type - 1],
                                    False);
                        } else {
                                XtSetSensitive(
                                    w_list[ConnP_conn_whens[2].when_type - 1],
        ...
        case AB_TYPE_MESSAGE:
                if (cur_obj != NULL) {
                        if (!obj_has_action1_button(cur_obj))
                                XtSetSensitive(
                                    w_list[ConnP_conn_whens[0].when_type - 1],
                                    False);
                        else
                                XtSetSensitive(
                                    w_list[ConnP_conn_whens[0].when_type - 1],
                        ...
                        if (!obj_has_action2_button(cur_obj))
                                XtSetSensitive(
                                    w_list[ConnP_conn_whens[1].when_type - 1],
                                    False);
                        else
                                XtSetSensitive(
                                    w_list[ConnP_conn_whens[1].when_type - 1],
 ...
 * optionmenu.
 */
void connP_update_action_menu(Widget widget, XtPointer clientData,
                              XtPointer callData) {
...
void conn_override_default_when(AB_WHEN when) { DefaultWhen = when; }

void conn_reset_default_when(void) { DefaultWhen = AB_WHEN_UNDEF; }

void conn_override_default_action_type(AB_FUNC_TYPE func_type) {
        DefaultFuncType = func_type;
}

void conn_reset_default_action_type(void) { DefaultFuncType = AB_FUNC_UNDEF; }

static void update_on_src_type_change(AB_OBJECT_TYPE obj_type,
                                      int obj_subtype) {
        AB_WHEN g_when = AB_WHEN_UNDEF;
        AB_FUNC_TYPE g_func = AB_FUNC_UNDEF;
        AB_BUILTIN_ACTION g_act = AB_STDACT_UNDEF;
        ...
        (void)connP_guess_when_action(
            obj_type, obj_subtype, connP_get_target_type(),
            connP_get_target_subtype(), &g_when, &g_func, &g_act);
...
static void update_on_target_type_change(AB_OBJECT_TYPE obj_type,
        ...
        (void)connP_guess_when_action(connP_get_source_type(),
                                      connP_get_source_subtype(), obj_type,
                                      obj_subtype, &g_when, &g_func, &g_act);
...
static void update_conn_ed_controls(void) {
        ...
        XtVaSetValues(XmOptionButtonGadget(target_menu), XtVaTypedArg,
                      XmNlabelString, XtRString, ConnP_conn_objs[j].label,
                      strlen(ConnP_conn_objs[j].label) + 1, NULL);
...
BOOL connP_obj_part_of_conn(ABObj obj, ABObj conn_obj) {
        BOOL ret = FALSE;
