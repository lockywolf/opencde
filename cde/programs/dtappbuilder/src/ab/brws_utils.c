...
static ABBrowser get_browser_for_win(AB_OBJ *project, Window w);

static void select_node(VNode selected_node);

static void deselect_node(VNode selected_node);

static void toggle_select_node(VNode selected_node);

static int select_fn(VNode node);

static int collapsed_fn(VNode module);

static void r_deselect_all_nodes(VNode tree, int flag);

static void draw_area_snap(Vwr v);

static Boolean clipWindowWorkProc(XtPointer client_data);

static void clipwindowEventHandler(Widget widget, XtPointer client_data,
                                   XEvent *event, Boolean *cont_dispatch);

static void clipwindowResizeCB(Widget w, XtPointer client_data,
                               XtPointer call_data);
...
BOOL aob_is_browser_win(AB_OBJ *project, Window w) {
        return (get_browser_for_win(project, w) != NULL);
}

AB_OBJ *aob_get_object_from_xy(AB_OBJ *project, Window w, int x, int y) {
        ABBrowser cur_browser;
        VNode found_node;
 ...
 * passed Viewer
 */
ViewerNode *aob_find_bnode(AB_OBJ *obj, Viewer *v) {
        ViewerNode *bnodes, *found = NULL;
        ViewerMethods *m;
        ...
        for (found = bnodes; found && (found->browser != v);
             found = found->next)
                ;
 ...
 */
AB_OBJ *aob_project_from_browser(Viewer *b) {
 ...
 */
BrowserProperties *aob_browser_properties(Viewer *b) {
 ...
 */
void aob_copy_props(Viewer *from, Viewer *to) {
        BrowserProperties *from_prop, *to_prop;
        if (!from || !to)
                return;
 ...
 */
BrowserUiObjects *aob_ui_from_browser(Viewer *b) {
 ...
 */
Widget brws_draw_area(Viewer *b) {
        VMethods m;
 ...
 */
ViewerNodeElm *aob_bnode_elements(ViewerNode *bnode) {
        if (!bnode)
                return (NULL);
 ...
 */
unsigned long browser_get_elm_shown(Viewer *b) {
 ...
 */
int browser_num_elm_shown(Viewer *b) {
        int i, count = 0;
...
void brws_toggle_select(AB_OBJ *obj) {
        AB_OBJ *root_obj = obj;
        VNode selected_nodes;
        Widget draw_area;
...
void aob_deselect_all_objects(AB_OBJ *project) {
        ABBrowser browsers;
 ...
 * in needed or not.
 */
void aob_deselect_all_nodes(Viewer *b, int flag) {
 ...
 */
static void r_deselect_all_nodes(ViewerNode *tree, int flag) {
        VMethods m;
        VNode child;
        ...
        vwr_get_cond(tree, &selected_nodes, &num_selected, select_fn);
...
void brws_set_module_name(Vwr b) {
        BrowserUiObjects *ui;
        DtbBrwsMainwindowInfo instance;
        AB_OBJ *module_obj = NULL;
        Widget shell, project_label;
        XmString xmlabel;
 ...
 */
extern void aob_preview(ABObj obj, Window browser_window) {
#define AB_BROWSER_PREVIEW_MARGIN 2
        ...
        XTranslateCoordinates(dpy, browser_window, root, bnode->x, bnode->y,
        ...
        x_fullscreen_preview_box(
            draw_area, root, root_x, root_y,
            root_x + bnode->width + AB_BROWSER_PREVIEW_MARGIN +
                AB_BROWSER_PREVIEW_MARGIN,
            root_y + bnode->height + AB_BROWSER_PREVIEW_MARGIN +
                AB_BROWSER_PREVIEW_MARGIN);

#undef AB_BROWSER_PREVIEW_MARGIN
}

void brws_update_node(ABObj obj) {
        Vwr v;
 ...
 */
ABBrowser brws_get_browser_for_obj(ABObj obj) {
        ABObj module = NULL, proj = NULL;
        ABBrowser cur_b = NULL, b_list = NULL;
        ...
        if (obj) {
                ...
                b_list =
                    project ? (ABBrowser)project->info.project.browsers : NULL;
        }
 ...
 */
Widget brws_get_browser_shell_for_obj(ABObj obj) {
        ABObj module = NULL, proj = NULL;
        ABBrowser cur_b = NULL, b_list = NULL;
        ABObj project = proj_get_project();
...
int brwsP_select_fn(VNode vnode) {
        if (BRWS_NODE_STATE_IS_SET(vnode, BRWS_NODE_SELECTED))
                return (1);
 ...
 * is taken.
 */
void brwsP_sync_views(ABBrowser ab, short select_at_least_one) {
        Vwr toplevel, detailed;
        VNode toplevel_child, detailed_child;
        VMethods m;
        int num_child, num_selected = 0, i;
        ...
        for (i = 0, toplevel_child = (*m->get_child)(toplevel->current_tree, 0);
             ...
             toplevel_child = (*m->get_child)(toplevel->current_tree, ++i)) {
                ...
                if (BRWS_NODE_STATE_IS_SET(toplevel_child,
                                           BRWS_NODE_SELECTED)) {
                        /*
                         ...
                         * in the toplevel view selected and make the
                         * corresponding node in the detailed view visible.
                         */
                        if (select_at_least_one && !num_selected) {
                                BRWS_NODE_SET_STATE(toplevel_child,
                                                    BRWS_NODE_SELECTED);
                                BRWS_NODE_SET_STATE(detailed_child,
                                                    BRWS_NODE_VISIBLE);
                                ++num_selected;
                        } else {
                                /*
                                 ...
                                 */
                                BRWS_NODE_UNSET_STATE(detailed_child,
                                                      BRWS_NODE_VISIBLE);
                        }
                }
        }
 ...
 */
int brwsP_node_is_collapsed(VNode node) {
        VNode parent;
        VMethods m;
        Vwr v;
        ABObj obj;
 ...
 * Note: on error conditions, TRUE is returned.
 */
int brwsP_node_is_visible(VNode node) {
        VNode parent;
        VMethods m;
        Vwr v;
        ABObj obj;
 ...
 */
void brwsP_make_drawarea_snap(Vwr v, Widget draw_area) {
        Widget clipwindow;
        ...
        if (XmIsDrawingArea(clipwindow)) {
                XtAddCallback(clipwindow, XmNresizeCallback, clipwindowResizeCB,
                              (XtPointer)v);
        } else {
                XtAddEventHandler(clipwindow, StructureNotifyMask, False,
...
static void clipwindowResizeCB(Widget w, XtPointer client_data,
         ...
         */
        XtAppAddWorkProc(XtWidgetToApplicationContext(w), clipWindowWorkProc,
                         client_data);
}
 ...
 */
static void draw_area_snap(Vwr v) {
        Widget clipwindow = NULL, draw_area = NULL;
        Dimension cw_width = 0, cw_height = 0, da_borderWidth = 0, da_width = 0,
                  da_height = 0;
         ...
         */
        XtVaGetValues(clipwindow, XmNwidth, &cw_width, XmNheight, &cw_height,
                      NULL);
        XtVaGetValues(draw_area, XmNborderWidth, &da_borderWidth, XmNwidth,
                      &da_width, XmNheight, &da_height, NULL);
        ...
        if ((Dimension)cw_width > props->min_width) {
                ...
                if (cw_width > 2u * da_borderWidth) {
                        ...
                        if (cw_width > 2) {
                                XtSetArg(arg[num_args], XmNwidth,
                                         (cw_width - 2));
                                num_args++;
                        }
                }
        ...
        if ((Dimension)cw_height > props->min_height) {
                ...
                if (cw_height > 2u * da_borderWidth) {
                        ...
                        if (cw_height > 2) {
                                XtSetArg(arg[num_args], XmNheight,
                                         (cw_height - 2));
                                num_args++;
                        }
                }
...
void brwsP_collapse_selected(ABBrowser ab) {
        Vwr v = NULL;
        VNode *selected_nodes = NULL;
        VMethods m;
        ...
        for (i = 0; i < num_selected; ++i) {
                 ...
                 */
                if (BRWS_NODE_STATE_IS_SET(selected_nodes[i],
                                           BRWS_NODE_EXPANDED)) {
                        BRWS_NODE_UNSET_STATE(selected_nodes[i],
                                              BRWS_NODE_EXPANDED);
                        redraw = TRUE;
                }
        }
...
void brwsP_expand_selected(ABBrowser ab) {
        Vwr v = NULL;
        VNode *selected_nodes = NULL;
        VMethods m;
        int num_selected = 0, i;
        ...
        for (i = 0; i < num_selected; ++i) {
                ...
                if (!BRWS_NODE_STATE_IS_SET(selected_nodes[i],
                                            BRWS_NODE_EXPANDED)) {
                        BRWS_NODE_SET_STATE(selected_nodes[i],
                                            BRWS_NODE_EXPANDED);
                        redraw = TRUE;
                }
        }
...
void brwsP_expand_collapsed(ABBrowser ab) {
        Vwr v = NULL;
        VNode *collapsed_nodes = NULL;
        VMethods m;
        int num_collapsed = 0, i;
        short redraw = FALSE;
         ...
         */
        vwr_get_cond(v->current_tree, &collapsed_nodes, &num_collapsed,
                     collapsed_fn);
        ...
        for (i = 0; i < num_collapsed; ++i) {
                ...
                if (brwsP_node_is_visible(collapsed_nodes[i])) {
                        BRWS_NODE_SET_STATE(collapsed_nodes[i],
                                            BRWS_NODE_EXPANDED);
                        redraw = TRUE;
                }
        }
...
void brwsP_tear_off_selected(ABBrowser b) {
...
void brws_center_on_obj(ABBrowser ab, ABObj obj) {
        ...
        if (top_level_view && detailed_view) {
                ...
                int x, y, value, size, increment, page, vert_max, horiz_max,
                    vert_min, horiz_min;
                ...
                if (!brwsP_node_is_visible(detailed_node)) {
                        ...
                        /*
                         * Found top level ABObj - direct child of module which
                         * is the ancestor of 'obj'.
                         *
                         * Get the corresponding node in the top level view
                         */
                        top_level_node =
                            aob_find_bnode(top_level_obj, top_level_view);
                ...
                XtVaGetValues(instance->detailed_drawarea_scrolledwin,
                              ...
                              XmNhorizontalScrollBar, &horiz_sb, XmNwidth,
                              &scroll_width, XmNheight, &scroll_height, NULL);
                ...
                if (vert_sb) {
                         ...
                         */
                        XtVaGetValues(vert_sb, XmNminimum, &vert_min,
                                      XmNmaximum, &vert_max, NULL);
                        XmScrollBarGetValues(vert_sb, &value, &size, &increment,
                                             &page);

                        /*
                         * Don't adjust if for some reason if the node is off
                         * the browser
                         */
                        if (detailed_node->y < vert_max) {
                                /*
                                 * This adjusts the y position of the scrollbar
                                 * so that the node is centered
                                 */
                                y = detailed_node->y - ((int)scroll_height -
                                                        detailed_node->height) /
                                                           2;
                                ...
                                /*
                                 * Set the scrollbar with new 'centered' y
                                 * position
                                 */
                                XmScrollBarSetValues(vert_sb, (int)y, size,
                                                     increment, page, True);
                        }
                }
                ...
                if (horiz_sb) {
                         ...
                         * Get current scrollbar values
                         */
                        XtVaGetValues(horiz_sb, XmNminimum, &horiz_min,
                                      XmNmaximum, &horiz_max, NULL);
                        XmScrollBarGetValues(horiz_sb, &value, &size,
                                             &increment, &page);
                        ...
                        if (detailed_node->x < horiz_max) {
                                 ...
                                 */
                                x = detailed_node->x -
                                    ((int)scroll_width - detailed_node->width) /
                                        2;
                                 ...
                                 * position
                                 */
                                XmScrollBarSetValues(horiz_sb, (int)x, size,
                                                     increment, page, True);
                        }
                }
