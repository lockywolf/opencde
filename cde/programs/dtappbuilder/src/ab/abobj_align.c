...
**                                                                      **
**************************************************************************/
void abobj_align(unsigned long align_mask) {
        ABSelectedRec sel;
...
static void align_vcenters(ABObj *sel_list, int sel_count) {
        ...
        for (i = 0; i < sel_count; i++) {
                ...
                undo_record_move(obj);
                */
                abobj_set_xy(obj, x - abobj_get_actual_width(obj) / 2,
                             obj_get_y(obj));
                abobj_instantiate_changes(obj);
        }
}
...
static void align_right_edges(ABObj *sel_list, int sel_count) {
        ...
        for (i = 0; i < sel_count; i++) {
                ...
                undo_record_move(obj);
                */
                abobj_set_xy(obj, x - abobj_get_actual_width(obj),
                             obj_get_y(obj));
                abobj_instantiate_changes(obj);
        }
}
...
static void align_labels(ABObj *sel_list, int sel_count) {
        ...
        for (i = 0; i < sel_count; i++) {
                ...
                if (obj_get_y(obj) < y) {
                        new_label_x =
                            obj_get_x(obj) + abobj_get_label_width(obj);
                        y = obj_get_y(obj);
                }
        }
...
static void align_hcenters(ABObj *sel_list, int sel_count) {
        ...
        for (i = 0; i < sel_count; i++) {
                ...
                undo_record_move(obj);
                */
                abobj_set_xy(obj, obj_get_x(obj),
                             y - abobj_get_actual_height(obj) / 2);
                abobj_instantiate_changes(obj);
        }
}
 ...
 */
static void distribute_horizontal(ABObj *sel_list, int sel_count,
                                  int hspacing) {
        int i;
        ABObj obj;
        ABObj prev;
        ...
        for (i = 1; i < sel_count; i++) {
                ...
                */
                abobj_set_xy(obj,
                             obj_get_x(prev) + abobj_get_actual_width(prev) +
                                 hspacing,
                             obj_get_y(obj));
                abobj_instantiate_changes(obj);
        }
...
static void distribute_vertical(ABObj *sel_list, int sel_count, int vspacing) {
        ...
        for (i = 1; i < sel_count; i++) {
                ...
                */
                abobj_set_xy(obj, obj_get_x(obj),
                             obj_get_y(prev) + abobj_get_actual_height(prev) +
                                 vspacing);
                abobj_instantiate_changes(obj);
        }
}
