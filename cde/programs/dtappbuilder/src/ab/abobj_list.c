...
*/
int abobj_list_load(Widget list, ABObj root, ABObjTestFunc list_obj_func) {
        int return_value = 0;
...
int abobj_list_update(Widget list, ABObj root, ABObjTestFunc list_obj_func) {
        ABObj obj = NULL;
        AB_TRAVERSAL trav;
        STRING modname;
        int item_count = 0;
             ...
             (obj = trav_next(&trav)) != NULL;) {
                if (obj_get_name(obj) != NULL) {
                        ...
                        if ((*list_obj_func)(obj)) {
                                if (ui_list_find_item(list, modname,
                                                      &item_pos) ==
                                    ERR_NOT_FOUND) {
 ...
 */
int abobj_list_obj_created(Widget list, ABObj obj,
...
int abobj_list_obj_renamed(Widget list, ABObj obj, STRING old_name,
        ...
        if (obj_is_module(obj)) {
                ui_list_replace_item_prefix(list, old_name, obj_get_name(obj));
        } else if ((module != NULL) && list_obj_test(obj)) {

                new_modname = abobj_get_moduled_name(obj);
                old_modname =
                    abobj_alloc_moduled_name(obj_get_name(module), old_name);
 ...
 */
int abobj_list_obj_updated(Widget list, ObjEvUpdateInfo info,
                           ABObjTestFunc list_obj_test) {
...
int abobj_list_obj_reparented(Widget list, ObjEvReparentInfo info,
        ...
        STRING moduled_name = NULL, module = NULL, name = NULL;
        ABObj obj = NULL;
        int ret = 0;
