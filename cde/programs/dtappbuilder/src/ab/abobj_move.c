...
#define rect_passtoparent(a, b, rect)                                          \
        {                                                                      \
                (rect)->x = (unsigned short)(rect)->x + (a);                   \
                (rect)->y = (unsigned short)(rect)->y + (b);                   \
        }
...
static void undo_move(ABUndoRec undo_rec);
...
**                                                                      **
**************************************************************************/
int abobjP_move_object_outline(ABObj obj, XMotionEvent *mevent) {
        ...
        if (first_move) {
                ...
                if (obj_is_control(obj) || obj_is_group(obj) ||
                    obj_is_pane(obj)) {
                        abobj_get_selected(obj_get_root(obj_get_parent(obj)),
                                           False, False, &sel);
                } else {
                        sel.count = 1;
                ...
                if (xy_widget == NULL) {
                        if (util_get_verbosity() > 2)
                                fprintf(stderr,
                                        "abobjP_move_object_outline: %s :no "
                                        "POSITION widget\n",
                ...
                if (sel.count > 1) {
                        abobj_get_rect_for_objects(sel.list, sel.count,
                                                   &orig_rect);
                } else {
                        orig_rect = widget_rect;
                        XtVaGetValues(xy_widget, XtNborderWidth, &border_w,
                                      NULL);
                ...
                drag_init_rect.width = drag_init_rect.height =
                    2 * AB_drag_threshold;
         ...
         */
                 ...
                 */
                XTranslateCoordinates(dpy, XtWindow(xy_widget),
                                      XtWindow(parent), mevent->x, mevent->y,
                                      &trans_x, &trans_y, &win);
                ...
                if (move_rect.x < 0)
                        move_rect.x = 0;
                else if ((move_rect.x + (short)move_rect.width + 1) >=
                         (short)parent_rect.width)
                        move_rect.x = parent_rect.width - (move_rect.width + 1);

                if (move_rect.y < 0)
                        move_rect.y = 0;
                else if ((move_rect.y + (short)move_rect.height + 1) >=
                         (short)parent_rect.height)
                        move_rect.y =
                            parent_rect.height - (move_rect.height + 1);
 ...
 */
int abobj_move(ABObj obj, XEvent *event) {
        short x_delta = 0;
        short y_delta = 0;
        ...
        if (!first_move) {
                 ...
                 */
                (void)abobj_set_undo(sel.list, sel.count, undo_move,
                                     AB_UNDO_MOVE);

                abobj_nudge_selected(sel.list, sel.count, x_delta, y_delta,
                                     True);
...
void abobj_nudge_selected(ABObj *sel_list, int sel_count, short x_delta,
        ...
        for (i = 0; i < sel_count; i++) {
                ...
                /*
                        if (xyobj->attachments)
                            abobj_calculate_new_layout(xyobj, new_rect.x,
                   new_rect.y, new_rect.width, new_rect.height);
                */
...
void abobj_move_selected(ABObj *sel_list, int sel_count, XRectangle *start,
                         XRectangle *stop) {
        ABObj obj;
        XRectangle obj_rect;
        int i;
...
static void undo_move(ABUndoRec undo_rec) {
         ...
         */
        (void)abobj_set_undo(undo_rec->list, undo_rec->count, undo_move,
                             AB_UNDO_MOVE);
        ...
        for (i = 0; i < undo_rec->count; ++i) {
                ...
                if (xy_obj->attachments)
                        abobj_calculate_new_layout(
                            xy_obj, x, y, move_rect.width, move_rect.height);

                abobj_instantiate_changes(obj);
        }
}

BOOL abobj_is_movable(ABObj obj) {
        BOOL movable = True;
        ABObj pobj;
        ...
        if (obj_is_group(pobj) && obj_get_group_type(pobj) != AB_GROUP_IGNORE)
                movable = False;
