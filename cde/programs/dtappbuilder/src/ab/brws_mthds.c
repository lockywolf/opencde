 ...
 */
static ABObj first_module_of_project(ABObj project);

/*
 * Viewer methods
 */
static ViewerNode *init_vnode(Vwr b, void *obj_data);

static ViewerNode *init_toplevel_node(Vwr b, void *obj_data);

static ViewerNode *init_proj_vnode(Vwr b, void *obj_data);

static void init_glyph_elm(VNode vnode, VNodeElm elm, void *client_data);

static void init_name_elm(VNode vnode, VNodeElm elm, void *client_data);

static void init_class_elm(VNode vnode, VNodeElm elm, void *client_data);

static void init_wclass_elm(VNode vnode, VNodeElm elm, void *client_data);

static void free_str_elm(VNode vnode, VNodeElm elm);

static void init_elements(VNode bnode);

static void free_elements(VNode bnode);

static void free_prop(Vwr v);

static void free_node(VNode vnode, void *obj_data);

static void init_proj_props(Vwr viewer);

static void init_mod_props(Vwr viewer);

static void free_elements(VNode bnode);

static int insert_projview_tree(Vwr viewer, void *obj_data);
static int insert_proj_tree(Vwr viewer, void *obj_data);

static int insert_entire_tree(Vwr viewer, void *obj_data);

static VNode insert_node(Vwr viewer, void *obj_data);

static void remove_tree(Vwr viewer, void *obj_data);

static void remove_node(Vwr viewer, void *obj_data);

static VNode get_browser_data(void *obj_data);

static void set_browser_data(void *obj_data, VNode vnode);

static VNode get_projwin_data(void *obj_data);

static void set_projwin_data(void *obj_data, VNode vnode);

static ViewerNode *get_parent(VNode bnode);

static int get_num_salient_children(VNode bnode);

static int get_num_children_of_mod(VNode bnode);

static int get_num_children_of_proj(VNode bnode);

static VNode get_salient_child(VNode bnode, int which_child);

static VNode get_child_of_mod(VNode bnode, int which_child);

static VNode get_child_of_proj(VNode bnode, int which_child);

static int get_num_children_ui(VNode bnode);

static ViewerNode *get_child_ui(VNode bnode, int which_child);

static unsigned long get_toplevel_drawarea(Vwr v);

static unsigned long get_detailed_drawarea(Vwr v);

static unsigned long get_proj_drawarea(Vwr v);

static void compute_tree(Vwr b, int *end_x, int *end_y);

static void compute_matrix(Vwr b, int *end_x, int *end_y);

static void compute_subtree(VNode tree, int start_x, int start_y, int *max_x,
                            int *max_y);

static void compute_submatrix(VNode tree, int start_x, int start_y, int *max_x,
                              int *max_y);

static void compute_glyph_elm(VNode node, VNodeElm str_elm, int x, int y);

static void compute_str_elm(VNode node, VNodeElm str_elm, int x, int y);

static void compute_node(VNode node, int x, int y);

static void render_salient_tree(VNode tree);

static void render_mod_tree(VNode tree);

static void render_proj_tree(VNode tree);

static void render_glyph_elm(VNode node, VNodeElm glyph_elm, int hilite);

static void render_str_elm(VNode node, VNodeElm str_elm, int hilite);

static void render_node(VNode node, int hilite);

static void render_line(VNode parent, VNode child);

static VNode locate_node(VNode tree, int x, int y);

static unsigned long locate_elements(VNode vnode, int x, int y);

static void render_node_bbox(ViewerNode *node);

static void draw_zoom_in(Vwr b, VNode node);

static void draw_zoom_out(Vwr b);

static void draw_collapsed_feedback(VNode node);

static void compute_collapsed_feedback(VNode node, int *max_x, int *max_y);
...
static ViewerElmMethods glyphElmMethods = {init_glyph_elm, compute_glyph_elm,
                                           render_glyph_elm, NULL};

static ViewerElmMethods nameElmMethods = {init_name_elm, compute_str_elm,
                                          render_str_elm, free_str_elm};

static ViewerElmMethods classElmMethods = {init_class_elm, compute_str_elm,
                                           render_str_elm, free_str_elm};

static ViewerElmMethods wclassElmMethods = {init_wclass_elm, compute_str_elm,
                                            render_str_elm, free_str_elm};

static VElmMethods elmMethods[] = {&glyphElmMethods, &nameElmMethods,
                                   &classElmMethods, &wclassElmMethods};
 ...
 */
static ViewerNode *init_vnode(Viewer *b, void *obj_data) {
 ...
 */
static ViewerNode *init_toplevel_node(Viewer *b, void *obj_data) {
        AB_OBJ *obj = (AB_OBJ *)obj_data;
        ViewerNode *node, *cur_node;
        BrowserProps props;
        ViewerMethods *m;
 ...
 */
static ViewerNode *init_proj_vnode(Viewer *b, void *obj_data) {
        AB_OBJ *obj = (AB_OBJ *)obj_data;
        ViewerNode *node, *cur_node;
        BrowserProps props;
        ViewerMethods *m;
...
static void init_elements(ViewerNode *vnode) {
        AB_OBJ *obj;
        ViewerNodeElm *elm;
        ...
        if (!vnode->elements) {
                elm = (ViewerNodeElm *)calloc(BRWS_NUM_ELM,
                                              sizeof(ViewerNodeElm));
                vnode->num_elements = BRWS_NUM_ELM;
                vnode->elements = elm;

                for (i = 0; i < vnode->num_elements; ++i) {
                        elm[i].data = NULL;
                        elm[i].x = elm[0].y = elm[i].width = elm[i].height = -1;
                }
        } else
                elm = vnode->elements;
...
static void free_elements(ViewerNode *vnode) {
        AB_OBJ *obj;
        ViewerNodeElm *elm;
        VElmMethods *elm_methods;
        int i;

        if (!vnode || !vnode->elm_methods || !vnode->elements ||
            !vnode->elements)
                return;
...
static void init_proj_props(Vwr viewer) {
         ...
         *	BRWS_SHOW_TYPE, BRWS_SHOW_NAME, BRWS_SHOW_WIDGET_CLASS,
         *BRWS_SHOW_GLYPH
         */
 ...
 */
static int insert_projview_tree(Vwr viewer, void *obj_data) {
        ABObj root = (ABObj)obj_data, browsed_module = NULL,
              insert_module = NULL, cur_obj;
        Vwr b_list;
        AB_TRAVERSAL trav;
        VNode bnode, module_node;
        ViewerMethods *m;
        ...
        if (!obj_is_project(root) && !obj_is_module(root) &&
            !obj_is_window(root) && !obj_is_menu(root) &&
            !obj_is_message(root)) {
        ...
        if (obj_is_project(root)) {
                ...
                if (!browsed_module) {
                         ...
                         * Re-inserting will only have an effect if there are
                         * new objects in the tree that the browser does not
                         * know about yet.
                         */
...
static int insert_entire_tree(Viewer *viewer, void *obj_data) {
        ABObj root = (ABObj)obj_data, browsed_module = NULL,
              insert_module = NULL, cur_obj;
        Viewer *b_list;
        VNode bnode, module_node;
...
static ViewerNode *get_parent(ViewerNode *bnode) {
...
static int get_num_children_of_proj(ViewerNode *bnode) {
        AB_OBJ *obj;
...
static int get_num_children_of_mod(ViewerNode *bnode) {
        AB_OBJ *obj;
...
static int get_num_salient_children(ViewerNode *bnode) {
        AB_OBJ *obj;
...
static ViewerNode *get_child_of_mod(ViewerNode *bnode, int which_child) {
        AB_OBJ *obj, *child;
        Viewer *browser;
...
static ViewerNode *get_child_of_proj(ViewerNode *bnode, int which_child) {
        AB_OBJ *obj, *child;
        Viewer *browser;
        ViewerNode *child_bnode;
...
static ViewerNode *get_salient_child(ViewerNode *bnode, int which_child) {
        AB_OBJ *obj, *child;
        Viewer *browser;
        ViewerNode *child_bnode;
...
static int get_num_children_ui(ViewerNode *bnode) {
        AB_OBJ *obj;
...
static ViewerNode *get_child_ui(ViewerNode *bnode, int which_child) {
        AB_OBJ *obj, *child;
        Viewer *browser;
 ...
 * node
 */
static void node_elm_offset(ViewerNode *node, int x, int y) {
        Viewer *b;
        ViewerNodeElm *elm;
        int elm_shown;
        int i;
...
static unsigned long get_proj_drawarea(Vwr v) {
        ...
        return (proj_d ? (unsigned long)proj_d->module_drawarea
                       : (unsigned long)NULL);
}
...
static void compute_tree(Vwr b, int *end_x, int *end_y) {
        VNode top;
        AB_OBJ *obj;
        int i, num_child, max_x = BRWS_X_ORIGIN, max_y = BRWS_Y_ORIGIN;
        ...
        if (BRWS_NODE_STATE_IS_SET(top, BRWS_NODE_VISIBLE))
                compute_subtree(top, BRWS_X_ORIGIN, BRWS_Y_ORIGIN, end_x,
                                end_y);
}
...
static void compute_matrix(Vwr b, int *end_x, int *end_y) {
        ...
        *end_y = w_rect.height;
        ...
        compute_submatrix(top, BRWS_X_ORIGIN, BRWS_Y_ORIGIN, end_x, end_y);
}
 ...
 * It does not draw the tree.
 */
static void compute_subtree(ViewerNode *tree, int start_x, int start_y,
                            int *max_x, int *max_y) {
        ViewerMethods *m;
        AB_OBJ *obj;
        Viewer *b;
         ...
         */
        if ((num_child == 0) ||
            !(BRWS_NODE_STATE_IS_SET(tree, BRWS_NODE_EXPANDED))) {
                /*
                ...
                if (props->orientation == BRWS_VERTICAL) {
                        ...
                        for (i = 0, child = (*m->get_child)(tree, 0);
                             ...
                             child = (*m->get_child)(tree, ++i))
                                if (child && BRWS_NODE_STATE_IS_SET(
                                                 child, BRWS_NODE_VISIBLE))
                                        compute_subtree(child, *max_x, tmp,
                                                        max_x, max_y);

                        tmp =
                            (*max_x - start_x - BRWS_NODE_X_GAP - tree->width) /
                                2 +
                            start_x;
                        if (tmp < start_x) {
                                tree->x = start_x;
                        ...
                        /*
                         * If for some reason the the max_y (i.e. height) of the
                         * entire subtree was not incremented (error in
                         * compute_node of child), make it big enough for this
                         * node.
                         */
                        tmp = start_y + tree->height + BRWS_NODE_Y_GAP;
                        if (*max_y < tmp)
                        ...
                        for (i = 0, child = (*m->get_child)(tree, 0);
                             ...
                             child = (*m->get_child)(tree, ++i))
                                if (child && BRWS_NODE_STATE_IS_SET(
                                                 child, BRWS_NODE_VISIBLE))
                                        compute_subtree(child, tmp, *max_y,
                                                        max_x, max_y);

                        tmp = (*max_y - start_y - BRWS_NODE_Y_GAP -
                               tree->height) /
                                  2 +
                              start_y;
                        if (tmp < start_y) {
                        ...
                        /*
                         * If for some reason the the max_x (i.e. width) of the
                         * entire subtree was not incremented (error in
                         * compute_node of child), make it big enough for this
                         * node.
                         */
 ...
 * It does not draw the tree.
 */
static void compute_submatrix(ViewerNode *tree, int start_x, int start_y,
                              int *max_x, int *max_y) {
        ViewerMethods *m;
        AB_OBJ *obj;
        Vwr b;
        VNode child;
        BrowserProps props;
        int i, x = start_x, y = start_y, num_child, col_max_width = 0,
               row_max_height = 0;
 ...
 * a node
 */
static void compute_glyph_elm(VNode node, VNodeElm glyph_elm, int x, int y) {
        if (!node || !glyph_elm)
                return;
 ...
 * of the border.
 */
static void compute_node(ViewerNode *node, int x, int y) {
        Vwr b;
        ViewerNodeElm *elm;
        VElmMethods *elm_methods;
        int elm_shown;
        int cur_height = 0, cur_width = 0, cur_y_offset, i;
        BOOL first_elm = TRUE;
        ...
        for (i = 0; i < node->num_elements; ++i) {
                if (elm_shown & (1L << i)) {
                         ...
                         */
                        (*(elm_methods[i]->compute))(node, &elm[i], x,
                                                     cur_y_offset);

                        /*
                         * Increment node height, y offset by new element
                         * height, bottom margin and bottom border
                         */
                         ...
                         */
                        cur_width = (cur_width > elm[i].width) ? cur_width
                                                               : elm[i].width;
                }
        }
         ...
         */
        node->width =
            cur_width + (2 * (BRWS_ELM_BBOX_MARGIN + BRWS_ELM_BORDER_WIDTH));
 ...
 */
static void compute_collapsed_node(ViewerNode *node, int x, int y) {
        Viewer *b;
        BrowserUiObjects *ui;
        ViewerNodeElm *elm;
        XFontStruct *sm_font;
        int elm_shown;
        int font_ascent, font_descent, cur_height = 0, cur_width = 0,
                                       cur_x_offset, cur_y_offset, i;
        char *collapsed_str = ". . .";
        ...
        */
        ...
        XTextExtents(sm_font, collapsed_str, strlen(collapsed_str), &direction,
        ...
        if (obj_is_project(obj)) {
                ...
                for (i = 0, child = (*m->get_child)(tree, 0); (i < num_child);
                     child = (*m->get_child)(tree, ++i)) {
                         ...
                         */
                        if (child &&
                            !BRWS_NODE_STATE_IS_SET(child, BRWS_NODE_VISIBLE))
                                continue;
                ...
                if (BRWS_NODE_STATE_IS_SET(tree, BRWS_NODE_EXPANDED)) {
                        ...
                        for (i = 0, child = (*m->get_child)(tree, 0);
                             ...
                             child = (*m->get_child)(tree, ++i)) {
                                /*
                                 * If this child node is not supposed to be
                                 * seen, don't render it. The render_tree method
                                 * (this routine we're in at this moment)
                                 * actually checks for this, but ...
                                 */
                                if (child && !BRWS_NODE_STATE_IS_SET(
                                                 child, BRWS_NODE_VISIBLE))
                                        continue;
...
static void render_glyph_elm(VNode node, VNodeElm glyph_elm, int hilite) {
         ...
         * draw area
         */
        XCopyArea(dpy, image, XtWindow(draw_area), gc, 0, 0, glyph_elm->width,
                  glyph_elm->height, glyph_elm->x, glyph_elm->y);
}
...
static void render_str_elm(VNode node, VNodeElm str_elm, int hilite) {
         ...
         */
        XDrawImageString(dpy, XtWindow(draw_area), gc, str_elm->x,
                         str_elm->y + font_ascent, str, strlen(str));
}
 ...
 * hilited or not.
 */
static void render_node(ViewerNode *node, int hilite) {
        Display *dpy;
        Widget draw_area;
        Viewer *b;
        AB_OBJ *obj;
        ...
        VElmMethods *elm_methods;
        int elm_shown;
        int cur_height = 0, cur_width = 0, cur_x_offset = 0, cur_y_offset = 0,
            i;
        ...
        if (!hilite) {
                XClearArea(dpy, XtWindow(draw_area), node->x, node->y,
 ...
 */
static void render_line(ViewerNode *parent, ViewerNode *child) {
        BrowserUiObjects *ui;
        BrowserProperties *props;
        Viewer *b;
        ViewerMethods *m;
        ...
        XGCValues gcvalues;
        XPoint xp[3];
        int num_child, num_visible_siblings = 0, i;
         ...
         */
        if (child->x > 32766 || child->y > 32766 || parent->x > 32766 ||
            parent->y > 32766)
                return;
        ...
        for (i = 0, cur_child = (*m->get_child)(real_parent, 0);
             (i < num_child); cur_child = (*m->get_child)(real_parent, ++i)) {
                if (cur_child == child)
                        continue;
        ...
        if (props->orientation == BRWS_VERTICAL) {
                ...
                if (num_visible_siblings == 0) {
                        ...
                        xp[1].y =
                            parent->y + parent->height + BRWS_NODE_LINK_GAP;

                        XDrawLines(XtDisplay(draw_area), XtWindow(draw_area),
                                   line_gc, xp, 2, CoordModeOrigin);
                } else {
                        /*
                ...
                if (num_visible_siblings == 0) {
                        ...
                        xp[1].x =
                            parent->x + parent->width + BRWS_NODE_LINK_GAP;
...
static VNode locate_node(VNode tree, int x, int y) {
         ...
         * node
         */
        if (x >= tree->x && x <= tree->x + tree->width && y >= tree->y &&
...
static unsigned long locate_elements(VNode vnode, int x, int y) {
        ...
        if ((x < vnode->x) || (x > vnode->x + vnode->width) || (y < vnode->y) ||
        ...
        for (i = 0; i < num_elm; ++i) {
                if (!(elm_shown & (1L << i)) ||
                    (y <
                     elm[i].y - BRWS_ELM_BBOX_MARGIN - BRWS_ELM_BORDER_WIDTH) ||
                    (y > elm[i].y + elm[i].height + BRWS_ELM_BBOX_MARGIN +
 ...
 */
static void draw_zoom_out(Viewer *b) {
        ...
        XtVaGetValues(draw_area, XtNx, &x, XtNy, &y, XtNwidth, &width,
        ...
        while (width > 0 || height > 0) {
                ...
                XClearArea(XtDisplay(draw_area), XtWindow(draw_area), x - 4,
                           y - 4, width + 8, height + 8, FALSE);
                x += 4;
                y += 4;
                width -= 8;
                height -= 8;
 ...
 */
static void draw_zoom_in(Viewer *b, ViewerNode *node) {
        BrowserUiObjects *ui_handle;
        Widget draw_area;
        int x, y, width, height, canvas_x, canvas_y, canvas_width,
            canvas_height;
        ...
        XtVaGetValues(draw_area, XtNx, &canvas_x, XtNy, &canvas_y, XtNwidth,
                      &canvas_width, XtNheight, &canvas_height, NULL);
        ...
        while (width < canvas_width || height < canvas_height) {
                 ...
                 * Erase the rectangle.
                 */
                XClearArea(XtDisplay(draw_area), XtWindow(draw_area), x, y,
                           width, height, FALSE);
        }
}
...
static void render_node_bbox(ViewerNode *node) {
        ...
        XtVaGetValues(draw_area, XmNbackground, &bg, XmNhighlightColor, &hc,
                      XmNhighlightPixmap, &hp, NULL);
...
void erase_viewer(Viewer *v) {
        ...
        XClearArea(XtDisplay(draw_area), XtWindow(draw_area), 0, 0,
                   w_rect.width, w_rect.height, FALSE);
}

/*
 * Erase a node.
 */
static void erase_node(ViewerNode *node) {
        Viewer *b;
        Widget draw_area;
