...
static void verify_clipboard_space(int count);

static void resolve_clipboard_connections(ABObj *obj, int count,
                                          ABClipboardRec clipboard);

static void resolve_clipboard_attachments(ABObj *obj, int count,
                                          ABClipboardRec clipboard);

static void resolve_attach_one_obj(ABObj obj, ABObj origObj, ABObj *new_list,
                                   int new_count);

static void resolve_attach_one_attachment(AB_COMPASS_POINT dir, ABObj obj,
                                          ABObj origObj, ABObj *new_list,
                                          int new_count);

static void resolve_attach_parent(ABObj obj, ABObj new_parent);

static void resolve_attach_parent_dir(AB_COMPASS_POINT dir, ABObj obj,
                                      ABObj new_parent);

static void resolve_none_attachments(ABObj obj);

static void resolve_undo_rec_connections(ABObj *obj, int count,
                                         ABUndoRec undo_rec);

static void resolve_undo_rec_attachments(ABObj *obj, int count,
                                         ABUndoRec undo_rec);

static void init_undo_rec(ABUndoRec *undo_rec_ptr);

static void init_undo();

static void verify_undo_rec_space(ABUndoRec *undo_rec_ptr, int count);

static void verify_undo_space(int count);

static void remove_clipboard_descendants(ABSelectedRec *sel);

static BOOL any_object_at_my_position(ABObj obj, ABObj parent);

static void stack_objects(ABObj newObj, ABObj parent);

static int set_undo_rec(ABUndoRec *undo_rec_ptr, ABObj *obj, int count,
                        ABUndoFunc undo_func, AB_UNDO_TYPE undo_type);

static void clear_undo_rec(ABUndoRec *undo_rec_ptr);

static void undo_cut(ABUndoRec undo_rec);

static void undo_paste(ABUndoRec undo_rec);

static int edit_destroyOCB(ObjEvDestroyInfo info);

static ABObj get_ctrl_pane_or_group(ABObj paste_target);

static DTB_MODAL_ANSWER show_modal_msg_relative_initiator(
    ABObj parent_obj, AB_PASTE_INITIATOR_TYPE initiator, DtbMessageData mbr,
    XmString override_msg, DtbObjectHelpData override_help,
                        ...
                        if (clip_info->action_list) {
                                ...
                                for (trav_open(&trav, clip_info->action_list,
                                     ...
                                     (action_obj = trav_next(&trav)) != NULL;) {
                                        ...
                                        /*
                                         * If the object being destroyed is the
                                         * 'to' object OR it is a descendant of
                                         * the 'to' obj, delete the connection
                                         * object
                                         */
                                        if ((obj == to_obj) ||
                                            (obj_is_descendant_of(to_obj,
                                                                  obj))) {
                        ...
                        if (undo_info->info.cut.to_action_list) {
                                ...
                                for (trav_open(
                                     ...
                                     (action_obj = trav_next(&trav)) != NULL;) {
                                        ...
                                        /*
                                         * If the object being destroyed is the
                                         * 'from' object OR it is a descendant
                                         * of the 'from' obj, delete the
                                         * connection object
                                         */
                                        if ((obj == from_obj) ||
                                            (obj_is_descendant_of(from_obj,
                                                                  obj))) {
                                                obj_destroy(action_obj);
                                                ABClipboard->action_count--;
                                        }
...
static void verify_clipboard_space(int count) {
        ...
        ABClipboard->info_list = (ABClipbInfo)realloc(
            ABClipboard->info_list, (num_nodes * sizeof(AB_CLIPB_INFO)));
        ...
        if (ABClipboard->list) {
                 ...
                 * blocks untouched.
                 */
                memset((void *)&ABClipboard->list[ABClipboard->count], 0,
                       (remainder * sizeof(ABObj)));

                memset((void *)&ABClipboard->info_list[ABClipboard->count], 0,
 ...
 */
static void resolve_none_attachments(ABObj obj) {
        AB_ATTACH_TYPE north_type, south_type, east_type, west_type;
        Position x = 0, y = 0;
        ...
        if (objxm_get_widget(obj)) {
                XtVaGetValues(objxm_get_widget(obj), XmNx, &x, XmNy, &y, NULL);
        }
        ...
        if ((north_type == AB_ATTACH_NONE) && (south_type == AB_ATTACH_NONE)) {
                ...
                obj_set_attachment(obj, AB_CP_NORTH, AB_ATTACH_POINT, NULL,
                                   obj->y);
        }
        ...
        if ((east_type == AB_ATTACH_NONE) && (west_type == AB_ATTACH_NONE)) {
                ...
                obj_set_attachment(obj, AB_CP_WEST, AB_ATTACH_POINT, NULL,
                                   obj->x);
        }
}
...
static void resolve_clipboard_connections(ABObj *obj, int count,
        ...
        AB_TRAVERSAL trav;
        ABClipbInfo info;
        ABObj newObj, origObj, action_obj;
        int i, j;
        ...
        for (i = 0; i < clipboard->count; ++i) {
                ...
                for (trav_open(&trav, info->action_list, AB_TRAV_ACTIONS);
                     (action_obj = trav_next(&trav)) != NULL;) {
                        ...
                        /*
                         * Skip connections if the action type is not
                         * 'predefined' or if the source/target are the same.
                         * These connections should have the target field set up
                         * properly when they were first dup'd in
                         * abobj_clipboard_add()
                         */
                        if ((obj_get_func_type(action_obj) !=
                             AB_FUNC_BUILTIN) ||
                        ...
                        for (j = 0; j < clipboard->count; ++j) {
                                if ((orig_to_obj == clipboard->list[j]) ||
                                    (obj_is_descendant_of(
                                        orig_to_obj, clipboard->list[j]))) {
                                        to_obj_copied = TRUE;
                                        save_index = j;
                                        break;
                                }
                        ...
                        /*
                         * Search for the dup'd object. It may not be the copied
                         * object, itself, but a descendant of it.
                         *
                         * This depends on the fact that the dup'd tree
                         * maintains objs with same names
                         *
                         * This also depends on the fact that the 'info_list',
                         * and 'list' fields on the clipboard have identical
                         * indices.
                         */
                        new_to_obj = obj_find_by_name_and_type(
                            to_info->dup_obj, obj_get_name(orig_to_obj),
...
static void resolve_clipboard_attachments(ABObj *obj, int count,
        ...
        ABClipbInfo info;
        ABObj *new_list = NULL;
        ABObj newObj, origObj, childObj, action_obj;
        int i, j;
        ...
        for (i = 0; i < clipboard->count; ++i) {
                 ...
                 */
                resolve_attach_one_obj(newObj, origObj, new_list,
                                       clipboard->count);
                ...
                for (trav_open(&trav, newObj, AB_TRAV_SALIENT);
                     (childObj = trav_next(&trav)) != NULL;) {
                         ...
                         */
                        origChildObj = obj_find_by_name_and_type(
                            origObj, obj_get_name(childObj),
                            obj_get_type(childObj));

                        /*
                         * Resolve attachment for descendant object.
                         */
                        resolve_attach_one_obj(childObj, origChildObj, new_list,
                                               clipboard->count);
                }
                trav_close(&trav);
        }
...
static void resolve_attach_one_obj(ABObj obj, ABObj origObj, ABObj *new_list,
        ...
        resolve_attach_one_attachment(AB_CP_NORTH, obj, origObj, new_list,
                                      new_count);

        resolve_attach_one_attachment(AB_CP_SOUTH, obj, origObj, new_list,
                                      new_count);

        resolve_attach_one_attachment(AB_CP_EAST, obj, origObj, new_list,
                                      new_count);

        resolve_attach_one_attachment(AB_CP_WEST, obj, origObj, new_list,
                                      new_count);
}
...
static void resolve_attach_one_attachment(AB_COMPASS_POINT dir, ABObj obj,
        ...
        ABObj attachObj = NULL, parentObj = NULL, origAttachObj = NULL,
              origParentObj = NULL;
        int offset = 0;
        ...
        case AB_ATTACH_OBJ:
                ...
                if (origParentObj) {
                         ...
                         *	obj is a copy of a descendant of a clipboard
                         *object
                         */
                        ...
                        if (origAttachObj == origParentObj) {
                                ...
                                if (parentObj) {
                                         ...
                                         * This means that it is a top level
                                         * object on the new list/clipboard
                                         * Since the actual parent object will
                                         * not be known until paste time, set
                                         * this to NULL for now.
                                         */
                                        attachObj = NULL;
                                }
                        } else {
                                ...
                                if (parentObj) {
                                        /*
                                         * obj has a parent. This means that
                                         * it's parent (and all it's siblings
                                         * were copied onto the new list).
                                         * Search for corresponding sibling
                                         * object starting at the parent object.
                                         */
                                        attachObj = obj_find_by_name_and_type(
                                            parentObj,
                                            obj_get_name(origAttachObj),
                                            obj_get_type(origAttachObj));

                                        /*
                                         * Make sure the found object is a
                                         * sibling of obj If not, nullify
                                         * attachObj.
                                         */
                                        if (attachObj &&
                                            (obj_get_parent(attachObj) !=
                                             parentObj)) {
                                                attachObj = NULL;
                                        }

                                        /*
                                         * If attached sibling not found, change
                                         * attachment type to AB_ATTACH_NONE
                                         */
                                        if (attachObj == NULL) {
                                         ...
                                         * obj does not have a parent.
                                         * This means that it is a top level
                                         * object on the new list. Check if the
                                         * sibling object was also copied onto
                                         * the new list as a top level object.
                                         */
                         ...
                         * will be resolved later, during paste. For now, we
                         *will only worry about sibling attachments.
                         */
                        ...
                        if (origAttachObj != NULL) {
                                ...
                                for (j = 0; j < new_count; ++j) {
                                        if (istr_equal(obj_get_name_istr(
                                                           origAttachObj),
                                                       obj_get_name_istr(
                                                           new_list[j])) &&
                                            (obj_get_type(origAttachObj) ==
                                ...
                                /*
                                 * If the sibling was not found, change the
                                 * attachment type to AB_ATTACH_NONE.
                                 */
                                if (attachObj == NULL) {
                                        attachType = AB_ATTACH_NONE;
                 ...
                 */
                (void)obj_set_attachment(obj, dir, attachType,
                                         (void *)attachObj, offset);
...
static void resolve_undo_rec_connections(ABObj *obj, int count,
        ...
        AB_TRAVERSAL trav;
        ABUndoInfo undo_info;
        ABObj origObj, from_action_list, to_action_list, action_obj;
        int i, j;
        ...
        for (i = 0; i < undo_rec->count; ++i) {
                ...
                if (from_action_list) {
                        for (trav_open(&trav, from_action_list,
                                       AB_TRAV_ACTIONS);
                             (action_obj = trav_next(&trav)) != NULL;) {
                                ABUndoInfo to_info, from_info;
                                BOOL to_obj_copied = FALSE,
                                     from_obj_copied = FALSE;
                                ABObj orig_to_obj, new_to_obj, orig_from_obj,
                                    new_from_obj;
                                int from_save_index, to_save_index;
                ...
                if (to_action_list) {
                        for (trav_open(&trav, to_action_list, AB_TRAV_ACTIONS);
                             (action_obj = trav_next(&trav)) != NULL;) {
                                ...
                                /*
                                 * Check if the 'from' object was copied into
                                 * the clipboard
                                 */
                                ...
                                from_info =
                                    &undo_rec->info_list[from_save_index];

                                /*
                                 * This depends on the fact that the dup'd tree
                                 * maintains objs with same names
                                 */
                                new_from_obj = obj_find_by_name_and_type(
                                    from_info->info.cut.dup_obj,
...
static void resolve_undo_rec_attachments(ABObj *obj, int count,
        ...
        for (i = 0; i < undo_rec->count; ++i) {
                ...
                if ((undo_info->type != AB_UNDO_CUT) ||
                    (undo_info->type != AB_UNDO_DELETE))
                        util_dprintf(1, "resolve_undo_rec_attachments: Warning "
                                        "- undo type is not cut or delete!\n");
 ...
 */
static void init_undo() { init_undo_rec(&ABUndo); }
...
static void verify_undo_rec_space(ABUndoRec *undo_rec_ptr, int count) {
         ...
         */
        num_nodes =
            ABOBJ_UNDO_BLOCK_SIZE * ((count / ABOBJ_UNDO_BLOCK_SIZE) + 1);
        ...
        undo_rec->list = (ABObj *)realloc((void *)undo_rec->list,
                                          (num_nodes * sizeof(ABObj)));
        undo_rec->info_list = (ABUndoInfo)realloc(
            undo_rec->info_list, (num_nodes * sizeof(AB_UNDO_INFO)));
        ...
        if (undo_rec->list) {
                 ...
                 * blocks untouched.
                 */
                memset((void *)&undo_rec->list[undo_rec->count], 0,
                       (remainder * sizeof(ABObj)));

                memset((void *)&undo_rec->info_list[undo_rec->count], 0,
 ...
 */
static void remove_clipboard_descendants(ABSelectedRec *sel) {
        ABObj curObj, curAncestor, *buf;
        int i, j, numNullified = 0;
        ...
        for (i = 0; i < sel->count; ++i) {
                ...
                for (j = 0; j < sel->count; ++j) {
                        ...
                        if (obj_is_descendant_of(curObj, curAncestor)) {
                                ...
                                /*
                                 * Keep track of number of objects deleted from
                                 * list
                                 */
                                ++numNullified;
                                break;
...
static void stack_objects(ABObj newObj, ABObj parent) {
        ...
        if (xy_changed) {
                obj_set_attachment(newObj, AB_CP_NORTH, AB_ATTACH_POINT, NULL,
                                   newObj->y);
                obj_set_attachment(newObj, AB_CP_WEST, AB_ATTACH_POINT, NULL,
                                   newObj->x);
        }
}
...
static int set_undo_rec(ABUndoRec *undo_rec_ptr, ABObj *obj, int count,
        ...
        for (i = undo_rec->count, j = 0; i < undo_rec->count + count;
             ++i, ++j) {
                ABUndoInfo undo_info;
                AB_TRAVERSAL trav;
                ...
                case AB_UNDO_DELETE:
                         ...
                         * Search for all ACTIONS that have this object (or any
                         * of it's descendants) as the source/target.
                         */
                        for (trav_open(&trav, project, AB_TRAV_ACTIONS);
                             (action_obj = trav_next(&trav)) != NULL;) {
                                /*
                                 * Does this action have a source ('from') that
                                 * matches the cut object or any of it's
                                 * descendants ?
                                 */
                                match_from =
                                    ((action_obj->info.action.from == obj[j]) ||
                                     (obj_is_descendant_of(
                                         action_obj->info.action.from,
                                         obj[j])));
                                 ...
                                 * matches the cut object or any of it's
                                 * descendants ? Note: The 'to' field in actions
                                 * is relevant only for predefined action types.
                                 */
                                match_to =
                                    (obj_get_func_type(action_obj) ==
                                     AB_FUNC_BUILTIN) &&
                                ...
                                if (match_from || match_to) {
                                        ...
                                        if (match_from) {
                                                /*
                                                 * Create action list to hold
                                                 * all the actions
                                                 */
                                                if (!undo_info->info.cut
                                                         .from_action_list)
                                                        undo_info->info.cut
                                                            .from_action_list =
                                                            obj_create(
                                                ...
                                                /*
                                                 * Since the action's source
                                                 * ('from') is the object that
                                                 * is copied onto the undo
                                                 * record, it will need to be
                                                 ...
                                                 * the dup'd tree maintains objs
                                                 * with same names
                                                 */
                                                new_from_obj =
                                                    obj_find_by_name_and_type(
                                                        undo_info->info.cut
                                                            .dup_obj,
                                                        obj_get_name(
                                                            action_obj->info
                                                        ...
                                                        obj_get_type(
                                                            action_obj->info
                                                                .action.from));
                                                new_action->info.action.from =
                                                    new_from_obj;

                                                /*
                                                 * If connected to itself,
                                                 * reassign the target ('to')
                                                 * object as well.
                                                 */
                                                if (connect_to_self)
                                                        new_action->info.action
                                                            .to = new_from_obj;
                                        ...
                                        if (!connect_to_self && match_to) {
                                                ...
                                                /*
                                                 * Since the action's target
                                                 * ('to') is the object that is
                                                 * copied onto the undo record,
                                                 * it will need to be reassigned
                                                 * to the new (copied) 'to'
                                                 * object.
                                                 */
                        ...
                        /*
                        fprintf(stderr,
                        "abobj_set_undo(%s, AB_UNDO_CUT/DELETE): X=%d, Y=%d,
                        W=%d, H=%d\n", obj_get_name(obj[j]), obj_get_x(obj[j]),
                        obj_get_y(obj[j]), obj_get_width(obj[j]),
                        obj_get_height(obj[j]));
                        */
                        break;
                ...
                case AB_UNDO_MOVE:
                        ...
                        if (objxm_get_widget(obj[j])) {
                                XtVaGetValues(objxm_get_widget(obj[j]), XmNx,
                                              &x, XmNy, &y, NULL);
                        } else {
                ...
                case AB_UNDO_UNGROUP: {
                        ...
                        undo_info->info.ungroup.dup_old_group =
                            abobj_dup(obj[j]);

                        member_count =
                            trav_count(obj[j], AB_TRAV_SALIENT_CHILDREN |
                                                   AB_TRAV_MOD_SAFE);

                        member_list =
                            (ABObj *)malloc(member_count * sizeof(ABObj));

                        for (trav_open(&trav, obj[j],
                                       AB_TRAV_SALIENT_CHILDREN |
                                           AB_TRAV_MOD_SAFE),
                             member_index = 0;
                ...
                default:
                        fprintf(stderr,
                                "abobj_set_undo_rec: Bad undo type %d\n",
                                undo_type);
                }
        }
...
int abobj_cut() {
        ...
        if (!project)
                ...
                (void)abobj_set_undo(sel.list, sel.count, undo_cut,
                                     AB_UNDO_CUT);

                for (i = (sel.count - 1); i >= 0; i--) {
                        /* Set the dirty bit on the module */
                        abobj_set_save_needed(obj_get_module(sel.list[i]),
                                              TRUE);
...
int abobj_copy() {
        ABObj project = proj_get_project(), newObj;
        ABSelectedRec sel;
        int i;
        int iRet = 0;
...
int abobj_paste(AB_PASTE_INITIATOR_TYPE initiator) {
        ABObj project = proj_get_project(), newObj, root, newroot, parent;
        ABSelectedRec sel;
        ...
        if (!project)
                ...
                if (abobj_clipboard_is_empty() || (sel.count != 1))
                        ...
                        ABObj *new_list = NULL, *new_action_list = NULL, obj;
                        int action_index = 0;
                        ...
                        for (i = 0; i < ABClipboard->count && !Err; ++i) {
                                ...
                                if (obj_is_window(obj) || obj_is_menu(obj) ||
                                    obj_is_message(obj)) {
                                        if (root)
                                        ...
                                        if (!obj_is_base_win(winobj)) {
                                                Err = True;
                                                i18n_msg = catgets(
                                                    Dtb_project_catd, 100, 37,
                                                    "Menubar can be pasted to "
                                                    "a Main Window only.");

                                                if (errmsg != (STRING)NULL)
                                                        util_free(errmsg);

                                                errmsg = (STRING)util_malloc(
                                                    strlen(i18n_msg) + 1);
                                                strcpy(errmsg, i18n_msg);
                                        } else
                                        /* Check to make sure there isn't
                                         * already a Menubar for this window
                                         */
                                        {
                                                for (trav_open(&trav, winobj,
                                                               AB_TRAV_UI);
                                                     (nobj = trav_next(
                                                          &trav)) != NULL;) {
                                                        if (obj_is_menubar(
                                        ...
                                        if (root == NULL ||
                                            (!(obj_is_container(root) &&
                                               ...
                                               !obj_is_file_chooser(
                                                ...
                                                Err = True;
                                                i18n_msg = catgets(
                                                    Dtb_project_catd, 100, 28,
                                                    "Containers must be pasted "
                                                    "to\na Main Window or "
                                                    "Custom Dialog.");
                                        ...
                                        if (root == NULL ||
                                            (!(obj_is_control_panel(
                                                   obj_get_root(root)) ||
                                               obj_is_group(
                                                   obj_get_root(root))))) {
                                                Err = True;
                                                i18n_msg = catgets(
                                                    Dtb_project_catd, 100, 57,
                                                    "Groups must be pasted "
                                                    "to\na group or control "
                                                    "panel.");
                                             ...
                                             !obj_is_window(
                                                ...
                                                if (!obj_is_control_panel(
                                                    ...
                                                    obj_is_control_panel(
                                                         ...
                                                         * child of a Control
                                                         * Panel, then use the
                                                         * Control Panel as the
                                                         * actual parent instead
                                                         * of the pane.
                                                         */
                                                        if (obj_is_control_panel(
                                                                obj_get_root(obj_get_parent(
                                                                    obj_get_root(
                                                                        root))))) {
                                                                obj_parent =
                                                                    obj_get_parent(
                                         ...
                                         */
                                                ...
                                                Err = True;
                                                i18n_msg = catgets(
                                                    Dtb_project_catd, 100, 29,
                                                    "Panes must be pasted "
                                                    "to\na Main Window, Custom "
                                                    "Dialog or Container.");
                                        ...
                                        if (newroot == NULL ||
                                            (!obj_is_control_panel(
                                                 obj_get_root(newroot)) &&
                                             !obj_is_group(
                                                 obj_get_root(newroot)))) {
                                                Err = True;
                                                i18n_msg = catgets(
                                                    Dtb_project_catd, 100, 30,
                                                    "Controls must be pasted "
                                                    "to\na Control Panel or "
                                                    "Group.");
                        ...
                        if (Err) {
                                ...
                                dtb_palette_wrn_msg_initialize(
                                    &dtb_palette_wrn_msg);
                        ...
                        if (!CancelPaste) {
                                ...
                                /*
                                 * Allocate buffer to store ptrs to new objects
                                 * pasted for undo
                                 */
                                new_list = (ABObj *)malloc(ABClipboard->count *
                                                           sizeof(ABObj));

                                if (ABClipboard->action_count)
                                        new_action_list = (ABObj *)malloc(
                                            ABClipboard->action_count *
                                            sizeof(ABObj));
                                ...
                                /*
                                 * Perform the paste operation for all objects
                                 * on clipboard
                                 */
                                for (i = 0; i < ABClipboard->count; ++i) {
                                        ABClipbInfo info;
                                        ...
                                        /*
                                         * Get a handle to one object on the
                                         * clipboard (includes it's descendants)
                                         */
                                        info = &(ABClipboard->info_list[i]);
                                        ...
                                        /*
                                         * Check if this object has any
                                         * connections that we should also
                                         * duplicate
                                         */
                                        if (info->action_list) {
                                                AB_TRAVERSAL trav;
                                                ...
                                                for (trav_open(
                                                     ...
                                                     (action_obj = trav_next(
                                                         ...
                                                         * on clipboard
                                                         */
                                                        new_action = abobj_dup(
                                                            action_obj);

                                                        /*
                                                         * The connection that
                                                         * is duplicated has, as
                                                         * the source, an object
                                                         * that was on the
                                                         * clipboard. We need to
                                ...
                                /*
                                 * At this point, all the objects that need to
                                 * be pasted have been dupicated, but not
                                 * parented to the proper parent yet. Also, all
                                 * the connection objects have been created, but
                                 * similarly, not added to the proper
                                 * module/project.
                                 *
                                 * Before we do the above, we need to resolve
                                 * the targets of the duplicated connections. If
                                 * the target object was also on the clipboard,
                                 * then the target has to be changed to it's
                                 * corresponding pasted object. This needs to be
                                 * done, in fact before we parent the newly
                                 * created objects because once that is done,
                                 * the names of the object may change.
                                 * obj_append_child() makes sure the child names
                                 * are unique.
                                 *
                                 * Also, the attachments of the newly dup'd
                                 * objects still contain references to objects
                                 * on the clipboard. This needs to be resolved
                                 * as well.
                                 */
                                 ...
                                 */
                                for (i = 0; i < ABClipboard->action_count;
                                     ++i) {
                                        ABObj action, orig_to_obj;

                                        action = new_action_list[i];

                                        if (obj_get_func_type(action) !=
                                            AB_FUNC_BUILTIN)
                                                continue;
                                        ...
                                        /*
                                         * Check if the pasted connection target
                                         * ('to') is on the clipboard
                                         */
                                        for (j = 0; j < ABClipboard->count;
                                             ++j) {
                                                ABClipbInfo info;

                                                info = &(
                                                    ABClipboard->info_list[j]);
                                ...
                                /*
                                 * Resolve the pasted objects' (and their
                                 * descendants') attachments
                                 */
                                for (i = 0; i < ABClipboard->count; ++i) {
                                        ABClipbInfo info;
                                        AB_TRAVERSAL trav;
                                        ABObj origObj = NULL, childObj = NULL;

                                        info = &(ABClipboard->info_list[i]);
                                        origObj = info->dup_obj;

                                        resolve_attach_one_obj(
                                            new_list[i], origObj, new_list,
                                            ABClipboard->count);

                                        for (trav_open(&trav, new_list[i],
                                                       AB_TRAV_SALIENT);
                                             (childObj = trav_next(&trav)) !=
                                             NULL;) {
                                                ABObj origChildObj = NULL;

                                                /*
                                                 * Skip top level object - taken
                                                 * care of above
                                                 */
                                ...
                                for (i = 0; i < ABClipboard->count; ++i) {
                                        ...
                                        /*
                                         * If an object already exists at (x,
                                         * y), offset the new object.
                                         */
                                        stack_objects(new_list[i], parent);

                                        /*
                                         * Add obj to parent, ensure unique
                                         * names.
                                         */

                                        /*
                                         * If pasted object is menubar, paste to
                                         * proper config child of main window
                                         */
                                        if (obj_is_menubar(new_list[i])) {
                                                 ...
                                                 */
                                                while (
                                                    !(obj_is_window(winobj) &&
                                                      !obj_is_sub(winobj)))
                                                        winobj = obj_get_parent(
                                                            winobj);
                                                ...
                                                if (winobj)
                                                        proper_parent =
                                                            objxm_comp_get_subobj(
                                                                winobj,
                                                                AB_CFG_MENU_PARENT_OBJ);
                                        }

                                        /*
                                         * Resolve attachments where the
                                         * attached object is the parent object.
                                         * This is for top level clipboard
                                         * objects. The new parent object is
                                         * only known at paste time. Until now,
                                         * the value field of the attachment was
                                         * NULL.
                                         */
                                        resolve_attach_parent(new_list[i],
                                                              proper_parent);

                                        /*
                                         * Last for attchments - resolve NONE
                                         * attachments If 2 opposite sides have
                                         * NONE attachments, create an
                                         * AB_ATTACH_POINT attachment on one
                                         * side.
                                         */
                                        resolve_none_attachments(new_list[i]);

                                        obj_append_child(proper_parent,
                                                         new_list[i]);
                                        ...
                                        abobj_set_save_needed(
                                            obj_get_module(new_list[i]), TRUE);
...
int abobj_delete() {
        ABObj project = proj_get_project(), parent = NULL;
        ABSelectedRec sel;
        int i;
...
int abobj_set_undo(ABObj *obj, int count, ABUndoFunc undo_func,
...
int abobj_cancel_undo() {
        clear_undo_rec(&ABUndo);
        return 0;
}

BOOL abobj_undo_active() {
        BOOL i;
 ...
 */
BOOL abobj_in_clipboard(ABObj obj) {
        int i;
 ...
 */
void abobj_clipboard_add(ABObj *obj, int count) {
        int action_count = 0, i, j;

        verify_clipboard_space(count);

        for (i = ABClipboard->count, j = 0; i < ABClipboard->count + count;
             ++i, ++j) {
                ABObj project, action_obj;
                ABClipbInfo info;
                AB_TRAVERSAL trav;
                 ...
                 * the current object being added to the clipboard or any of
                 * it's descendants.
                 */
                for (trav_open(&trav, project, AB_TRAV_ACTIONS);
                     (action_obj = trav_next(&trav)) != NULL;) {
                        if (!action_obj->info.action.from) {
                                util_dprintf(1, "abobj_clipboard_add: "
                                                "action_obj->info.action.from "
                                                "= NIL!\n");
                                continue;
                        }
                        ...
                        if ((action_obj->info.action.from == obj[j]) ||
                            (obj_is_descendant_of(action_obj->info.action.from,
                                ...
                                /*
                                 * Create the ACTION_LIST that will hold all the
                                 * connections that we will store on the
                                 * clipboard for one particular object (and it's
                                 * descendants).
                                 */
                                if (!info->action_list)
                                        info->action_list = obj_create(
                                            AB_TYPE_ACTION_LIST, NULL);
                                ...
                                /*
                                 * OK, at this point we have duplicatd the
                                 * object tree and one connection that has it as
                                 * the source. The problem is the source, as
                                 * seen by the connection is the object that is
                                 * still part of the project. We need to make
                                 * the source be the dup'd object that is in the
                                 * clipboard.
                                 */

                                /*
                                 * Search for the dup'd object. It may not be
                                 * the copied object, but a descendant of it. We
                                 * search by name/type.
                                 *
                                 * This depends on the fact that the dup'd tree
                                 * maintains objs with same names
                                 ...
                                 * The target ('to') is important only if this
                                 * is a 'predefined' action type
                                 */
                                if ((obj_get_func_type(action_obj) ==
                                     AB_FUNC_BUILTIN) &&
                                    (action_obj->info.action.from ==
 ...
 */
void abobj_clipboard_set(ABObj *obj, int count) {
        int i, j;
...
static ABObj get_ctrl_pane_or_group(ABObj paste_target) {
        ABObj ret_obj = NULL, parent = paste_target;
 ...
 */
void abobj_setup_undo_cut_layer(ABObj layer, ABObj last_pane) {
        int i;
...
static void undo_cut(ABUndoRec undo_rec) {
        ...
        ABUndoInfo undo_info;
        ABObj *new_list = NULL, *new_from_action_list = NULL,
              *new_to_action_list = NULL;
        int i, j, from_action_count = 0, to_action_count = 0;
        ABObj dup_obj, parent, newObj;
        ...
        if (undo_rec->action_count > 0) {
                 ...
                 */
                new_from_action_list =
                    (ABObj *)malloc(undo_rec->action_count * sizeof(ABObj));
                new_to_action_list =
                    (ABObj *)malloc(undo_rec->action_count * sizeof(ABObj));
        }
        ...
        for (i = 0; i < undo_rec->count; ++i) {
                 ...
                 * These are actions that have the re-created object (or any of
                 * it's descendants) as the source ('from').
                 */
                if (undo_info->info.cut.from_action_list) {
                        AB_TRAVERSAL trav;
                        ABObj action_obj;
                        ...
                        for (trav_open(&trav,
                                       ...
                                       AB_TRAV_ACTIONS);
                             (action_obj = trav_next(&trav)) != NULL;) {
                                ABObj orig_from_obj, new_from_obj, new_action;

                                new_action = abobj_dup_tree(action_obj);

                                /*
                                 * Since the from object is re-created, we need
                                 * to reassign the 'from' field of the action to
                                 * the new 'from' object.
                                 */
                                ...
                                /*
                                 * The new 'from' object is searched for by
                                 * name/type The search is rooted at the object
                                 * that was re-created.
                                 */
                                new_from_obj = obj_find_by_name_and_type(
                                    newObj, obj_get_name(orig_from_obj),
                                    obj_get_type(orig_from_obj));
                                new_action->info.action.from = new_from_obj;

                                /*
                                 * If source == target, reassign the 'to' field
                                 * as well
                                 */
                                if (action_obj->info.action.from ==
                                    action_obj->info.action.to)
                                 ...
                                 */
                                new_from_action_list[from_action_count++] =
                                    new_action;
                        }
                        trav_close(&trav);
                }
                 ...
                 * These are actions that have the re-created object (or any of
                 * it's descendants) as the target ('to').
                 */
                if (undo_info->info.cut.to_action_list) {
                        AB_TRAVERSAL trav;
                        ABObj action_obj;
                        ...
                        for (trav_open(&trav,
                                       ...
                                       AB_TRAV_ACTIONS);
                             (action_obj = trav_next(&trav)) != NULL;) {
                                ABObj orig_to_obj, new_to_obj, new_action;

                                new_action = abobj_dup_tree(action_obj);

                                /*
                                 * Since the 'to' object is re-created, we need
                                 * to reassign the 'to' field of the action to
                                 * the new 'to' object.
                                 */
                                ...
                                /*
                                 * The new 'to' object is searched for by
                                 * name/type The search is rooted at the object
                                 * that was re-created.
                                 */
                                new_to_obj = obj_find_by_name_and_type(
                                    newObj, obj_get_name(orig_to_obj),
                                    obj_get_type(orig_to_obj));
                                 ...
                                 */
                                new_to_action_list[to_action_count++] =
                                    new_action;
                        }
                        trav_close(&trav);
                }
         ...
         * was dup'd onto the undo record (and re-created again in the first
         * loop above) . If this is the case, the action's target must be set to
                ...
                for (j = 0; j < undo_rec->count; ++j) {
                        ...
                        if ((orig_to_obj == dup_obj) ||
                            (obj_is_descendant_of(orig_to_obj, dup_obj))) {
                                 ...
                                 */
                                new_to_obj = obj_find_by_name_and_type(
                                    new_list[j], obj_get_name(orig_to_obj),
                                    obj_get_type(orig_to_obj));
         ...
         * was dup'd onto the undo record (and re-created again in the first
         * loop above) . If this is the case, the action's source must be set to
         * the recreated object.
         */
        for (i = 0; i < to_action_count; ++i) {
                ABObj action, orig_from_obj;
                ...
                for (j = 0; j < undo_rec->count; ++j) {
                        ...
                        if ((orig_from_obj == dup_obj) ||
                            (obj_is_descendant_of(orig_from_obj, dup_obj))) {
                                 ...
                                 */
                                new_from_obj = obj_find_by_name_and_type(
                                    new_list[j], obj_get_name(orig_from_obj),
                                    obj_get_type(orig_from_obj));
        ...
        for (i = 0; i < undo_rec->count; ++i) {
                ...
                resolve_attach_one_obj(new_list[i], origObj, new_list,
                                       undo_rec->count);
        ...
        for (i = 0; i < undo_rec->count; ++i) {
                ...
                if (parent) {
                        ...
                        if (obj_is_menubar(new_list[i])) {
                                ...
                                if (winobj)
                                        proper_parent = objxm_comp_get_subobj(
                                            winobj, AB_CFG_MENU_PARENT_OBJ);
                        }
                } else {
                        /*
                         * parent == NULL
                        ...
                        layer =
                            abobj_handle_layered_pane(new_list[i], last_pane);
                        proper_parent = layer;
                }
                 ...
                 * parent object is only known at paste time. Until now, the
                 * value field of the attachment was NULL.
                 */
                ...
                if (obj_is_layers(new_list[i]) || obj_is_group(new_list[i]) ||
                    obj_is_menu(new_list[i]))
                        manage_last = FALSE;
        ...
        for (i = 0; i < from_action_count; ++i) {
                ...
                is_cross_module =
                    (obj_get_func_type(action) == AB_FUNC_BUILTIN) &&
                    obj_is_cross_module(action);

                if (is_cross_module) {
                        obj_add_action(
                            obj_get_project(action->info.action.from), action);
                        abobj_set_save_needed(
                            obj_get_project(action->info.action.from), TRUE);
                } else {
                        obj_add_action(
                            obj_is_project(action->info.action.from)
                                ? action->info.action.from
                                : obj_get_module(action->info.action.from),
                            action);
                }
        }
         ...
         */
        (void)abobj_set_undo(new_list, undo_rec->count, undo_paste,
                             AB_UNDO_PASTE);
...
static void undo_paste(ABUndoRec undo_rec) {
        ...
        ABObj obj, dup_obj, parent_obj, newObj;
         ...
         */
        (void)abobj_set_undo(undo_rec->list, undo_rec->count, undo_cut,
                             AB_UNDO_CUT);
 ...
 *	delete
 */
void abobj_undo_cb(Widget widget, XtPointer client_data, XtPointer call_data) {
        (void)abobj_undo();
}

void abobj_cut_cb(Widget widget, XtPointer client_data, XtPointer call_data) {
        (void)abobj_cut();
}

void abobj_copy_cb(Widget widget, XtPointer client_data, XtPointer call_data) {
        (void)abobj_copy();
}

void abobj_paste_cb(Widget widget, XtPointer client_data, XtPointer call_data) {
        (void)abobj_paste((AB_PASTE_INITIATOR_TYPE)client_data);
}

void abobj_delete_cb(Widget widget, XtPointer client_data,
                     XtPointer call_data) {
        (void)abobj_delete();
}
...
static DTB_MODAL_ANSWER show_modal_msg_relative_initiator(
    ...
    Widget *modal_dlg_pane_out_ptr) {
        ...
        if ((winobj != NULL) && obj_has_flag(winobj, IconifiedFlag)) {
                /*
                ** The window is iconified, determine who to parent the dialog
                *off
                ** of.
                */
                switch (initiator) {
                case AB_PASTE_INITIATOR_OBJ_MENU:
                        /*
                        ** paste initiated from obj menu! The window shouldn't
                        *be iconified.
                ...
                /*
                ** The window parent_obj is on is not iconified, parent the
                *dialog
                ** off it.
                */
                parent = objxm_get_widget(parent_obj);
        }
        return (dtb_show_modal_message(parent, mbr, override_msg, override_help,
                                       modal_dlg_pane_out_ptr));
}
