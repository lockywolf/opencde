...
static void ttalk_applyCB(Widget widget, XtPointer client_data,
                          XtPointer call_data);

static void ttalk_resetCB(Widget widget, XtPointer client_data,
                          XtPointer call_data);

static void ttalk_cancelCB(Widget widget, XtPointer client_data,
                           XtPointer call_data);

static void list_add_msgs(Widget widget, XtPointer client_data,
                          XtPointer call_data);

static void reg_msg_setting_id(Widget widget, XtPointer client_data,
                               XtPointer call_data);

static void ttalk_mode_changedCB(Widget widget, XtPointer client_data,
                                 XmToggleButtonCallbackStruct *calldata);

static void ttalk_rcv_changedCB(Widget widget, XtPointer client_data,
                                XmToggleButtonCallbackStruct *calldata);

static char *message_strs[] = {
    "Do_Command",    "Get_Environment", "Get_Geometry",
    "Get_iconified", "Get_Locale",      "Get_Mapped",
    "Get_Mofified",  "Get_Situation",   "Get_Status",
    "Get_Sysinfo",   "Get_XInfo",       "Lower",
    "Quit",          "Raise",           "Revert",
    "Save",          "Set_Environment", "Set_Geometry",
    "Set_Iconified", "Set_Locale",      "Set_Mapped",
    "Set_Situation", "Set_XInfo",       "Signal"};

static Widget tt_msg_list;
static Widget tt_msg_setting;

void ttalk_show_dialog(Widget widget, XtPointer client_data,
                       XtPointer call_data) {
        ...
        if (AB_ttalk_dialog == (Widget)NULL) {
                ...
                /* These will go away when connections work */
                {
                        XtAddCallback(tooltalk_tooltalk_editor.ok_button,
                                      XmNactivateCallback,
                                      (XtCallbackProc)ttalk_applyCB,
                                      (XtPointer)NULL);
                        XtAddCallback(tooltalk_tooltalk_editor.cancel_button,
                                      XmNactivateCallback,
                                      (XtCallbackProc)ttalk_cancelCB,
                                      (XtPointer)NULL);
                }
...
static void list_add_msgs(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        XmString xmitem;
        int i;
...
static void ttalk_applyCB(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        util_dprintf(2, "ttalk_applyCB()\n");
        XtUnmanageChild(AB_ttalk_dialog);
}

static void ttalk_resetCB(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        util_dprintf(2, "ttalk_resetCB()\n");
}

static void ttalk_cancelCB(Widget w, XtPointer client_data,
                           XtPointer call_data) {
