...
static Widget appfw_editor_init(DtbAppfwDialogInfo appfw_cgen, Widget parent);
static int appfw_editor_load(ABObj project);
static int appfw_editor_load_gencode(ABObj project, AppfwEditorSettings afs);
static int appfw_editor_apply(ABObj project);
static void turnoff_changebars();
static BOOL rootwin_test_func(ABObj obj);
static BOOL gencode_arg_class_is_supported(AB_ARG_CLASS argClass);
static BOOL appfw_editor_pending();
static void appfw_editor_prevent_closeCB(Widget w, XtPointer client_data,
                                         XtPointer call_data);
/*
 * Object-Callbacks
 */
...
void appfw_show_dialog(ABObj project) {
        ...
        if (AB_appfw_dialog == (Widget)NULL) {
                AB_appfw_dialog = appfw_editor_init(&dtb_appfw_dialog,
...
static Widget appfw_editor_init(DtbAppfwDialogInfo appfw_cgen, Widget parent) {
        ...
        if (dtb_appfw_dialog_initialize(appfw_cgen, parent) == 0) {
                ...
                prop_field_init(&(afs->vendor), appfw_cgen->vendor_field_label,
                                appfw_cgen->vendor_field,
                                appfw_cgen->vendor_cb);

                /* Version */
                prop_field_init(
                    &(afs->version), appfw_cgen->version_field_label,
                ...
                prop_obj_options_init(
                    &(afs->rootwin), appfw_cgen->rootwin_label,
                    appfw_cgen->rootwin_opmenu, appfw_cgen->rootwin_opmenu_menu,
                    n, item, (XtPointer *)item_val, appfw_cgen->rootwin_cb,
                    True, /* Display module */
                ...
                n = 0;
                item[n] =
                    dtb_appfw_dialog.gencode_res_checkbox_items.Colors_item;
                item_val[n++] = (int)AB_ARG_CLASS_COLOR;
                item[n] =
                    dtb_appfw_dialog.gencode_res_checkbox_items.Geometry_item;
                item_val[n++] = (int)AB_ARG_CLASS_GEOMETRY;
                item[n] = dtb_appfw_dialog.gencode_res_checkbox_items
                              .Label_Strings_item;
                ...
                item[n] =
                    dtb_appfw_dialog.gencode_res_checkbox_items.Other_item;
                item_val[n++] = (int)AB_ARG_CLASS_OTHER;
                item[n] = dtb_appfw_dialog.gencode_res_checkbox_items
                              .Initial_Values_item;
                item_val[n++] = (int)AB_ARG_CLASS_VALUE;
                prop_checkbox_init(&(afs->gencode_res_file),
                                   appfw_cgen->gencode_label,
                                   appfw_cgen->gencode_res_checkbox, n, item,
                                   item_val, appfw_cgen->gencode_res_cb);
                ...
                item_val[n] = AB_SESSIONING_NONE;
                n++;
                item[n] =
                    appfw_cgen->ssmgmt_method_opmenu_items.Command_Line_item;
                item_val[n] = AB_SESSIONING_CMDLINE;
                n++;
                item[n] =
                    appfw_cgen->ssmgmt_method_opmenu_items.Session_File_item;
                item_val[n] = AB_SESSIONING_FILE;
                n++;
                item[n] = appfw_cgen->ssmgmt_method_opmenu_items.Both_item;
                item_val[n] = AB_SESSIONING_CMDLINE_AND_FILE;
                n++;
                prop_options_init(
                    &(afs->session_mgt), appfw_cgen->ssmgt_method_label,
                    appfw_cgen->ssmgmt_method_opmenu,
                    appfw_cgen->ssmgmt_method_opmenu_menu, n, item,
                    (XtPointer *)item_val, appfw_cgen->session_cb);
                ...
                item_val[n] = AB_TOOLTALK_NONE;
                n++;
                item[n] = appfw_cgen->tt_msg_handling_opmenu_items.Basic_item;
                item_val[n] = AB_TOOLTALK_DESKTOP_BASIC;
                n++;
                item[n] =
                    appfw_cgen->tt_msg_handling_opmenu_items.Advanced_item;
                item_val[n] = AB_TOOLTALK_DESKTOP_ADVANCED;
                n++;
                prop_options_init(
                    &(afs->tooltalk), appfw_cgen->tt_msg_handling_label,
                    appfw_cgen->tt_msg_handling_opmenu,
                    appfw_cgen->tt_msg_handling_opmenu_menu, n, item,
                    (XtPointer *)item_val, appfw_cgen->tt_msg_handling_cb);
                 ...
                 * protocol
                 */
                ab_register_window(appfw_cgen->dialog_shellform, AB_WIN_DIALOG,
                ...
                util_dprintf(3, "appfw_show_dialog: could not initialize "
                                "Application Framework Editor\n");
                return NULL;
        }
}

static int appfw_editor_load(ABObjPtr project) {
        ABObj rootwin;
        ...
        prop_options_set_value(&(afs->session_mgt),
                               (XtPointer)obj_get_sessioning_method(project),
                               False);
        ...
        prop_options_set_value(&(afs->tooltalk),
                               (XtPointer)obj_get_tooltalk_level(project),
                               False);
...
static int appfw_editor_load_gencode(ABObj project, AppfwEditorSettings afs) {
        ...
        for (i = ((int)AB_ARG_CLASS_UNDEF) + 1;
             i < (int)AB_ARG_CLASS_NUM_VALUES; ++i) {
                ...
                prop_checkbox_set_value(&(afs->gencode_res_file), argClass,
                                        argIsResFile, FALSE);
        }
        return 0;
}

static int appfw_editor_apply(ABObjPtr project) {
        ...
        if (prop_changed(afs->i18n.changebar)) {
                abobj_set_i18n_enabled(
                    project, (BOOL)prop_checkbox_get_value(&(afs->i18n),
                                                           APPFW_I18N_KEY));
        }
        ...
        if (prop_changed(afs->gencode_res_file.changebar)) {
                ...
                for (i = ((int)AB_ARG_CLASS_UNDEF) + 1;
                     i < (int)AB_ARG_CLASS_NUM_VALUES; ++i) {
                        ...
                        if (prop_checkbox_get_value(&(afs->gencode_res_file),
                                                    argClass)) {
                                resArgClasses |=
                                    util_arg_class_to_flag(argClass);
                        }
                }
        ...
        if (prop_changed(afs->session_mgt.changebar)) {
                abobj_set_sessioning_method(
                    project, (AB_SESSIONING_METHOD)prop_options_get_value(
                                 &(afs->session_mgt)));
        ...
        if (prop_changed(afs->tooltalk.changebar)) {
                ...
                new_level =
                    (AB_TOOLTALK_LEVEL)prop_options_get_value(&(afs->tooltalk));
                if (new_level != obj_get_tooltalk_level(project)) {
                        obj_set_tooltalk_level(project, new_level);
                        abobj_set_save_needed(project, True);

                        /* REMIND: Need to Check that Connections match Tooltalk
                         * level */
                }
        }
 ...
 * for the Application
 */
static BOOL rootwin_test_func(ABObj obj) { return (obj_is_base_win(obj)); }
 ...
 */
static void appfw_editor_prevent_closeCB(Widget widget, XtPointer client_data,
                                         XtPointer call_data) {
        ...
        if (appfw_editor_pending()) {
                if (dtb_app_resource_rec.implied_apply == True)
                        ...
                        XmString xm_buf;
                        char *msg = NULL, *help_text = NULL;
                        /*
                         * Initialize warning message object if necessary
                         */
                        dtb_appfw_appfw_pending_msg_initialize(
                            &dtb_appfw_appfw_pending_msg);
                        ...
                        xm_buf = XmStringCreateLocalized(
                            catgets(Dtb_project_catd, 100, 248,
                                    "Properties for your project have been "
                                    "modified on the\nApplication Framework "
                                    "Editor but not Applied.\nApply Changes or "
                                    "Cancel Close operation."));

                        help_data->help_volume = help_data->help_locationID =
                            NULL;
                        help_data->help_text = XtNewString(catgets(
                            Dtb_project_catd, 100, 249,
                            "Click Apply Changes to apply the changes \nand "
                            "close the Application Framework Editor.\n\nClick "
                            "Cancel if you don't want to apply the\nchanges "
                            "and want the Application Framework\nEditor to "
                            "remain displayed. You can then click\nReset to "
                            "undo the changes before closing\nthe Application "
                            "Framework Editor."));
                        ...
                        answer = dtb_show_modal_message(
                            AB_appfw_dialog, &dtb_appfw_appfw_pending_msg,
