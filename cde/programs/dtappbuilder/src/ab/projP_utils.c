...
static void save_as_bil_okCB(Widget widget, XtPointer client_data,
 ...
 *  and pops it up.
 ***********************************************************/
void projP_show_save_as_bil_chooser(Widget widget, ABObj obj) {
        Widget textf;
        ...
        title =
            XtNewString(catgets(Dtb_project_catd, 100, 242, "Save BIL File"));
        ...
        *mod_dir = 0;
        ...
        if (util_strempty(fullpath)) {
                ...
                (void)dtb_show_modal_message(dtb_get_toplevel_widget(),
                                             &dtb_proj_no_name_msg, NULL, NULL,
                                             NULL);
                return;
        }
        if (util_file_is_directory(fullpath)) {
                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 73,
                ...
                help_data->help_text = catgets(
                    Dtb_project_catd, 100, 87,
                    "The file name you specified for saving the\nmodule is a "
                    "directory. Type another file name\nfor the module.");
        ...
        if (iRet == 0) {
                if (write_OK) {
                        ...
                        if (proj_verify_name(mod_name, TRUE,
                                             &dtb_proj_save_as_mod_msg, widget,
                                             module_obj)) {
                                projP_save_export_bil(module_obj, fullpath,
                                                      FALSE);
                                abobj_set_save_needed(module_obj, FALSE);
                        }
                } else {
                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 76,
                        ...
                        help_data = (DtbObjectHelpData)util_malloc(
                            sizeof(DtbObjectHelpDataRec));
                        help_data->help_text =
                            catgets(Dtb_project_catd, 100, 88,
...
int projP_save_export_bil(ABObj module_obj, STRING fullpath, BOOL Exporting) {
        ...
        *new_filename = 0;
        ...
        if (!Exporting &&
            util_paths_are_same_file(old_filename, new_filename)) {
                ...
                if (write_OK) {
                        iRet = projP_write_out_bil_file(
                            module_obj, new_filename, NULL, Exporting);
                        if (iRet >= 0)
                                obj_clear_flag(module_obj, SaveNeededFlag);
                        XtUnmanageChild(AB_generic_chooser);
                } else {
                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 7,
                                        "%s is a read-only file."),
                                old_filename);
...
int projP_write_out_bil_file(ABObj obj, char *new_filename, STRING old_proj_dir,
                             BOOL Exporting) {
        ...
        *name = 0;
        ...
        if (!Exporting) {
                ...
                rel_path = proj_cvt_mod_file_to_rel_path(
                    new_filename, obj_get_file(proj_get_project()));
                ...
                if (util_strempty(rel_path) || util_strempty(name) ||
                    util_strempty(old_name)) {
                        ...
                        if (rc < 0) {
                                ...
                                /* Set the SaveNeededFlag on the project. Either
                                 * the directory where the module was stored or
                                 * its name (or both) has changed.
                                 */
                                if (!util_paths_are_same_file(old_file,
                                                              rel_path))
                                        abobj_set_save_needed(
                                            obj_get_project(obj), TRUE);
                        }
                }
 ...
 */
void projP_update_menu_items(int old_count, int new_count) {
        if (new_count == 0) {
