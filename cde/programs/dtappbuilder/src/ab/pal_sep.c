 ...
 * Methods
 */
static int separator_initialize(ABObj obj);
static Widget separator_prop_init(Widget parent, AB_PROP_TYPE type);
static int separator_prop_activate(AB_PROP_TYPE type, BOOL active);
static int separator_prop_clear(AB_PROP_TYPE type);
static int separator_prop_load(ABObj obj, AB_PROP_TYPE type,
                               unsigned long loadkey);
static int separator_prop_apply(AB_PROP_TYPE type);
static BOOL separator_prop_pending(AB_PROP_TYPE type);
...
static Widget separator_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbSepPropDialogInfo cgen =
            &dtb_sep_prop_dialog; /* Codegen structure */
        ...
        if (dtb_sep_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_separator_palitem, cgen->ok_button,
                ...
                n++;
                prop_options_init(
                    &(pss->line_style), cgen->linestyle_opmenu_label,
                    cgen->linestyle_opmenu, cgen->linestyle_opmenu_menu, n,
                    item, (XtPointer *)item_val, cgen->linestyle_cb);
...
static int separator_prop_clear(AB_PROP_TYPE type) {
        ...
        /* Clear Line Style */
        prop_options_set_value(&(pss->line_style),
...
static int separator_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
                               unsigned long loadkey) {
        PropSepSettingsRec *pss = &(prop_sep_settings_rec[type]);
        ...
        if (load_all) {
                ...
                prop_radiobox_set_value(
                    &(pss->orient), (XtPointer)obj_get_orientation(obj), False);

                /* Load Line Style */
                prop_options_set_value(&(pss->line_style),
                                       (XtPointer)obj_get_line_style(obj),
                                       False);
...
int separator_prop_apply(AB_PROP_TYPE type) {
        PropSepSettingsRec *pss = &(prop_sep_settings_rec[type]);
        STRING value;
        ...
        if (orient_chg) {
                ...
                prop_geomfield_set_value(&(pss->geometry), GEOM_X,
                                         abobj_get_comp_x(pss->current_obj),
                                         False);
                prop_geomfield_set_value(&(pss->geometry), GEOM_Y,
                                         abobj_get_comp_y(pss->current_obj),
                                         False);
                prop_geomfield_set_value(&(pss->geometry), GEOM_WIDTH,
                                         abobj_get_comp_width(pss->current_obj),
                                         False);
                prop_geomfield_set_value(
                    &(pss->geometry), GEOM_HEIGHT,
                    abobj_get_comp_height(pss->current_obj), False);
        }
        if (style_chg) {
                /* Workaround for XmSeparator bug where setting its
                 * XmNseparatorType resource changes its thickness; we want to
                 * maintain the specified width/height instead
                 */
                orient = obj_get_orientation(pss->current_obj);
                if (orient == AB_ORIENT_HORIZONTAL)
                        XtVaSetValues(objxm_get_widget(pss->current_obj),
                                      XmNheight,
                                      prop_geomfield_get_value(&(pss->geometry),
                                                               GEOM_HEIGHT),
                                      NULL);
...
static BOOL separator_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_sep_settings_rec[type].prop_sheet));
}
