 ...
 * Methods
 */
static int textp_initialize(ABObj obj);
static Widget textp_prop_init(Widget parent, AB_PROP_TYPE type);
static int textp_prop_activate(AB_PROP_TYPE type, BOOL active);
static int textp_prop_clear(AB_PROP_TYPE type);
static int textp_prop_load(ABObj obj, AB_PROP_TYPE type, unsigned long loadkey);
static int textp_prop_apply(AB_PROP_TYPE type);
static BOOL textp_prop_pending(AB_PROP_TYPE type);
...
static Widget textp_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbTextpPropDialogInfo cgen =
            &dtb_textp_prop_dialog; /* Codegen structure */
        ...
        if (dtb_textp_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_textp_palitem, cgen->ok_button,
                ...
                item_val[n] = AB_SCROLLBAR_NEVER;
                n++;
                item[n] = cgen->scroll_rbox_items.Always_item;
                item_val[n] = AB_SCROLLBAR_ALWAYS;
                n++;
                prop_radiobox_init(&(pts->scrolling), cgen->scroll_rbox_label,
                                   cgen->scroll_rbox, n, item,
                                   (XtPointer *)item_val, cgen->scroll_cb);
                ...
                prop_menuname_init(&(pts->menuname), type, cgen->menu_label,
                                   ...
                                   &(pts->menu_title), &(pts->current_obj),
                                   True);
                ...
                /* Size */
                prop_geomfield_init(&(pts->size), cgen->size_opmenu_label, NULL,
                                    NULL, NULL, NULL, cgen->width_field_label,
                                    cgen->width_field, cgen->height_field_label,
                                    cgen->height_field, cgen->size_cb);
                ...
                prop_options_init(&(pts->size_metric), cgen->size_opmenu_label,
                                  cgen->size_opmenu, cgen->size_opmenu_menu, n,
                                  item, (XtPointer *)item_val, cgen->size_cb);
...
static int textp_prop_load(ABObj obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_options_set_value(
                    &(pts->frame), (XtPointer)obj_get_border_frame(obj), False);
                ...
                prop_field_set_value(&(pts->menu_title),
        ...
        if (load_all || loadkey & LoadSize) {
                ...
                prop_geomfield_set_value(&(pts->size), GEOM_WIDTH, width,
                                         False);
                prop_geomfield_set_value(&(pts->size), GEOM_HEIGHT, height,
                                         False);
        }
...
int textp_prop_apply(AB_PROP_TYPE type) {
        PropTextpSettingsRec *pts = &(prop_textp_settings_rec[type]);
        ...
        if (prop_changed(pts->scrolling.changebar)) {
                abobj_set_scrollbar_state(
                    pts->current_obj,
                    (AB_SCROLLBAR_POLICY)prop_radiobox_get_value(
                        &(pts->scrolling)));
        ...
        if (prop_changed(pts->size.changebar)) {
                ...
                border = /* assumption! */
                    ((prop_options_get_value(&(pts->frame)) !=
                      (XtPointer)AB_LINE_NONE)
                         ? 4
                         : 0);
                ...
                if (obj_is_layers(pobj)) {
                        ...
                        abobj_set_num_rows(pts->current_obj,
                                           metric == SIZE_IN_CHARS_KEY ? new_h
                                                                       : -1);
        ...
        if (prop_changed(pts->wrap.changebar)) {
                abobj_set_word_wrap(pts->current_obj, prop_checkbox_get_value(
                                                          &(pts->wrap), True));
        }
        ...
        if (prop_changed(pts->menuname.changebar)) {
                ...
                util_free(value);
                prop_menuname_set_value(&(pts->menuname),
                                        obj_get_menu_name(pts->current_obj),
                                        False);
        }
...
static BOOL textp_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_textp_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pts->size.changebar) &&
            (!prop_number_ok(pts->size.w_field, (STRING)WFieldStr, 1,
                             SHRT_MAX) ||
             !prop_number_ok(pts->size.h_field, (STRING)HFieldStr, 1,
                             SHRT_MAX)))
                return False;
