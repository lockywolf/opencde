 ...
 * function.
 */
void vwr_destroy_node(ViewerNode *node) {
        free(node->elements);
        free(node);
}
 ...
 */
void vwr_reset(Viewer *b) { b->current_tree = b->tree; }

/*
 * Delete a tree node from a given tree.
 */
void vwr_delete_tree(Vwr v, VNode node) {}
 ...
 * aborts.
 */
ViewerNode *vwr_traverse_tree(ViewerNode *tree, int (*fn)(ViewerNode *)) {
        ViewerMethods *m;
        ViewerNode *child, *rtn_node = NULL, *tmp_node = NULL;
        int i, num_child;
 ...
 * Erase a node.
 */
static void erase_node(Viewer *b, ViewerNode *node, Pixmap pixmap) {
        Widget draw_area;
 ...
 */
static void recalc_matrix(b) Viewer *b;
{
 ...
 */
static void line_up_nodes(b, tree, fn, next) Viewer *b;
ViewerNode *tree;
int (*fn)();
ViewerNode **next;
{
 ...
 */
static void compute_matrix(b, head, start_x, start_y, x_limit, end_y) Viewer *b;
ViewerNode *head;
int start_x;
int start_y;
int x_limit;
 ...
 */
static void render_matrix(b, tree, pixmap, fn) Viewer *b;
ViewerNode *tree;
Pixmap pixmap;
int (*fn)();
{
 ...
 */
VNode vwr_locate_node(Vwr b, int x, int y) {
 ...
 */
static void draw_icon(pixmap, x, y, node) Pixmap pixmap;
int x;
int y;
ViewerNode *node;
{
        ...
        XDrawRectangle(dpy, pixmap, xor_gc,
                       ...
                       node->icon_y + y_delta,
                       node->icon_width - 2 * ICON_OFFSET, node->icon_height);
}

/*
 * Draw an up-pointing arrow at the given point.
 */
static void draw_arrow(pixmap, x, y) Pixmap pixmap;
int x;
int y;
{
#define pcount 7
        ...
        XFillPolygon(dpy, pixmap, xor_gc, plist, pcount, Convex,
                     CoordModeOrigin);
...
void vwr_get_cond(VNode tree, VNode **array, /* RETURN */
        ...
        VMethods m;
        VNode child;
        VNode *cur_array = NULL, *child_array;
        int child_num, cur_num = 0;
        int i, num_child;
             ...
             child = (*m->get_child)(tree, ++i)) {
                ...
                if (child_num > 0) {
                        ...
                        if (cur_num > 0) {
                                 ...
                                 */
                                new_array = (VNode *)calloc(
                                    sizeof(VNode), cur_num + child_num);
        ...
        if (!cond || (*cond)(tree)) {
                 ...
                 */
                new_array = (VNode *)calloc(sizeof(VNode), cur_num + 1);
                 ...
                 */
                if (cur_num > 0) {
                        memcpy(new_array, cur_array, (cur_num * sizeof(VNode)));
