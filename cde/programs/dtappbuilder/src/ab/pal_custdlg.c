 ...
 * Methods
 */
static int custdlg_initialize(ABObj obj);
static Widget custdlg_prop_init(Widget parent, AB_PROP_TYPE type);
static int custdlg_prop_activate(AB_PROP_TYPE type, BOOL active);
static int custdlg_prop_clear(AB_PROP_TYPE type);
static int custdlg_prop_load(ABObj obj, AB_PROP_TYPE type,
                             unsigned long loadkey);
static int custdlg_prop_apply(AB_PROP_TYPE type);
static BOOL custdlg_prop_pending(AB_PROP_TYPE type);

static BOOL verify_props(AB_PROP_TYPE type);
static void turnoff_changebars(AB_PROP_TYPE type);

static void custdlg_create_area(ABObj obj, AB_CONTAINER_TYPE area_type);
static void custdlg_create_buttons(ABObj obj, ABObj area);
static void custdlg_remove_area(ABObj obj, ABObj area);
static BOOL button_test_func(ABObj obj);
static BOOL winparent_test_func(ABObj obj);
...
static int custdlg_initialize(ABObj obj) {
        ...
        obj_set_label(obj,
                      catgets(Dtb_project_catd, 100, 256, "Custom Dialog"));
...
static Widget custdlg_prop_init(Widget parent, AB_PROP_TYPE type) {
        DtbCustdlgPropDialogInfoRec
            rev_custdlg_prop_dialog; /* Revolving Props */
        DtbCustdlgPropDialogInfo cgen =
            &dtb_custdlg_prop_dialog; /* Codegen structure */
        ...
        if (dtb_custdlg_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                prop_obj_options_init(
                    &(pcs->win_parent), cgen->winparent_opmenu_label,
                    cgen->winparent_opmenu, cgen->winparent_opmenu_menu, n,
                    item, (XtPointer *)item_val, cgen->winparent_cb,
                    True, /* Display module */
                ...
                item_val[n] = False;
                n++;
                prop_radiobox_init(&(pcs->resize_mode),
                                   cgen->resizemode_rbox_label,
                                   cgen->resizemode_rbox, n, item,
                                   (XtPointer *)item_val, cgen->resizemode_cb);

                /* Window Areas */
                n = 0;
                n = 0;
                item[n] = cgen->areas_ckbox_items.Button_Panel_item;
                item_val[n] = AB_CONT_BUTTON_PANEL;
                n++;
                item[n] = cgen->areas_ckbox_items.Footer_item;
                item_val[n] = AB_CONT_FOOTER;
                n++;
                ...
                n++;
                prop_obj_options_init(
                    &(pcs->default_but), cgen->defaultb_opmenu_label,
                    cgen->defaultb_opmenu, cgen->defaultb_opmenu_menu, n, item,
                    (XtPointer *)item_val, cgen->default_button_cb,
                    False, /* don't display module */
                ...
                n++;
                prop_obj_options_init(
                    &(pcs->help_but), cgen->helpb_opmenu_label,
                    cgen->helpb_opmenu, cgen->helpb_opmenu_menu, n, item,
                    (XtPointer *)item_val, cgen->help_button_cb,
                    False, /* don't display module */
                    &(pcs->current_obj), button_test_func);
                ...
                n++;
                prop_radiobox_init(&(pcs->size_policy),
                                   cgen->size_policy_rbox_label,
                                   cgen->size_policy_rbox, n, item,
                                   (XtPointer *)item_val, cgen->size_policy_cb);

                for (i = 0; i < n; i++)
                        XtAddCallback(item[i], XmNvalueChangedCallback,
                                      (XtCallbackProc)prop_size_policyCB,
                                      (XtPointer) & (pcs->geometry));

                /* Geometry */
                prop_geomfield_init(&(pcs->geometry), cgen->geom_label, NULL,
                                    NULL, NULL, NULL, cgen->width_field_label,
                                    cgen->width_field, cgen->height_field_label,
...
static int custdlg_prop_clear(AB_PROP_TYPE type) {
        ...
        /* Clear Window Areas */
        prop_checkbox_set_value(&(pcs->areas), AB_CONT_BUTTON_PANEL, False,
                                False);
        ...
        /* Clear Size Policy */
        prop_radiobox_set_value(&(pcs->size_policy), (XtPointer)SIZE_FIXED_KEY,
                                False);
...
static int custdlg_prop_load(ABObj obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_radiobox_set_value(&(pcs->resize_mode),
                                        (XtPointer)obj_get_resizable(obj),
                                        False);
                ...
                prop_obj_options_set_value(&(pcs->default_but),
                                           obj_get_default_act_button(obj),
                                           False);
...
int custdlg_prop_apply(AB_PROP_TYPE type) {
        PropCustdlgSettingsRec *pcs = &(prop_custdlg_settings_rec[type]);
        ABObj module;
        ABObj area;
        STRING value;
        ...
        if (prop_changed(pcs->title.changebar)) {
                ...
                abobj_set_label(pcs->current_obj, pcs->current_obj->label_type,
                                value);
                util_free(value);
        }
        ...
        if (prop_changed(pcs->areas.changebar)) {
                ...
                /* Button Panel */
                area = objxm_comp_custdlg_get_area(pcs->current_obj,
                                                   AB_CONT_BUTTON_PANEL);
                area_set = prop_checkbox_get_value(&(pcs->areas),
                                                   AB_CONT_BUTTON_PANEL);
                ...
                if (area != NULL && !area_set) /* Remove Button Panel */
                        ...
                        custdlg_create_area(pcs->current_obj,
                                            AB_CONT_BUTTON_PANEL);
                ...
                area = objxm_comp_custdlg_get_area(pcs->current_obj,
                                                   AB_CONT_FOOTER);
                area_set =
                    prop_checkbox_get_value(&(pcs->areas), AB_CONT_FOOTER);
...
static BOOL custdlg_prop_pending(AB_PROP_TYPE type) {
        return (prop_changebars_pending(
            prop_custdlg_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pcs->areas.changebar)) {
                ...
                if (area != NULL && !area_set) /* Remove Menubar */
                {
                        ...
                        if (obj_get_num_children(pobj) > 0) {
                                ...
                                answer = dtb_show_modal_message(
                                    pcs->areas.checkbox,
                                    &dtb_custdlg_remove_bpanel_msg, NULL, NULL,
                                    NULL);
                ...
                if (area != NULL && !area_set) /* Remove Footer */
                {
                        ...
                        if (obj_get_num_children(pobj) > 0) {
                                ...
                                answer = dtb_show_modal_message(
                                    pcs->areas.checkbox,
                                    &dtb_custdlg_remove_footer_msg, NULL, NULL,
                                    NULL);
                                if (answer == DTB_ANSWER_CANCEL)
                                        return False;
                        }
        ...
        if (prop_changed(pcs->geometry.changebar) &&
            (!prop_number_ok(pcs->geometry.w_field, (STRING)WFieldStr, 1,
                             SHRT_MAX) ||
             !prop_number_ok(pcs->geometry.h_field, (STRING)HFieldStr, 1,
                             SHRT_MAX)))
                return False;
...
static void custdlg_create_area(ABObj obj, AB_CONTAINER_TYPE area_type) {
        ...
        if (area_type == AB_CONT_FOOTER) {
                ...
                obj_set_label(label, catgets(Dtb_project_catd, 100, 264,
                                             "footer message"));
                obj_set_label_alignment(label, AB_ALIGN_LEFT);
                obj_set_unique_name(label, ab_ident_from_name_and_label(
                                               obj_get_name(obj), "label"));

                /*
                 * Workaround part1: MainWindow bug that causes MainWindow to
                 * shrink when a MessageWindow area is added
                 */
                XtVaGetValues(objxm_get_widget(workobj), XmNheight, &height,
                              NULL);
        ...
        /* Workaround part2 */
        if (area_type == AB_CONT_FOOTER)
                XtVaSetValues(objxm_get_widget(workobj), XmNheight, height,
                              NULL);
...
static void custdlg_create_buttons(ABObj obj, ABObj area) {
        ...
        for (i = 0; i < XtNumber(button_names); i++) {
                ...
                obj_set_unique_name(
                    bobj, ab_ident_from_name_and_label(obj_get_name(obj),
                                                       button_names[i]));
                obj_set_label(bobj, button_labels[i]);
                obj_set_is_initially_visible(bobj, True);
                obj_set_is_initially_active(bobj, True);
                startpos = endpos + 10;
                obj_set_attachment(bobj, AB_CP_WEST, AB_ATTACH_GRIDLINE,
                                   (void *)startpos, 0);
                endpos = startpos + 20;
                obj_set_attachment(bobj, AB_CP_EAST, AB_ATTACH_GRIDLINE,
                                   (void *)endpos, 0);
                obj_set_attachment(bobj, AB_CP_NORTH, AB_ATTACH_POINT,
                                   (void *)0, 5);
        }
}
 ...
 */
static BOOL winparent_test_func(ABObj obj) { return (obj_is_base_win(obj)); }
