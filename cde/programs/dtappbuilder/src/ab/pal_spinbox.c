 ...
 * Methods
 */
static int spinbox_initialize(ABObj obj);
static Widget spinbox_prop_init(Widget parent, AB_PROP_TYPE type);
static int spinbox_prop_activate(AB_PROP_TYPE type, BOOL active);
static int spinbox_prop_clear(AB_PROP_TYPE type);
static int spinbox_prop_load(ABObj obj, AB_PROP_TYPE type,
                             unsigned long loadkey);
static int spinbox_prop_apply(AB_PROP_TYPE type);
static BOOL spinbox_prop_pending(AB_PROP_TYPE type);

static BOOL verify_props(AB_PROP_TYPE type);
static void turnoff_changebars(AB_PROP_TYPE type);
static void setup_content_settings(AB_PROP_TYPE type,
...
static int spinbox_obj_destroy_CB(ObjEvDestroyInfo destroyInfo);
/*
 * Xt Callbacks
 */
static void spinbox_typeCB(Widget widget, XtPointer clientdata,
...
static Widget spinbox_prop_init(Widget parent, AB_PROP_TYPE type) {
        DtbSpinboxPropDialogInfoRec
            rev_spinbox_prop_dialog; /* Revolving Props */
        DtbSpinboxPropDialogInfo cgen =
            &dtb_spinbox_prop_dialog; /* Codegen structure */
        ...
        if (dtb_spinbox_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_REVOLVING)
                        XtVaSetValues(parent, XmNuserData, pss->current_obj,
                                      NULL);
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_spinbox_palitem, cgen->ok_button,
                ...
                item_val[n] = AB_TEXT_NUMERIC;
                n++;
                item[n] = cgen->spinboxtype_rbox_items.String_List_item;
                item_val[n] = AB_TEXT_DEFINED_STRING;
                n++;
                prop_radiobox_init(&(pss->spinbox_type),
                                   cgen->spinboxtype_rbox_label,
                                   cgen->spinboxtype_rbox, n, item,
                                   (XtPointer *)item_val, cgen->spinboxtype_cb);
                for (i = 0; i < n; i++)
                        XtAddCallback(item[i], XmNvalueChangedCallback,
                                      (XtCallbackProc)spinbox_typeCB,
                                      (XtPointer)type);
                ...
                n++;
                prop_options_init(
                    &(pss->label_type), cgen->labeltype_rbox_label,
                ...
                prop_label_field_init(&(pss->label), cgen->graphic_hint, item,
                                      n);
                ...
                item_val[n] = AB_ARROW_FLAT_BEGIN;
                n++;
                item[n] = cgen->arrowstyle_opmenu_items.Flat_End_item;
                item_val[n] = AB_ARROW_FLAT_END;
                n++;
                item[n] = cgen->arrowstyle_opmenu_items.Beginning_item;
                item_val[n] = AB_ARROW_BEGIN;
                n++;
                item[n] = cgen->arrowstyle_opmenu_items.End_item;
                item_val[n] = AB_ARROW_END;
                n++;
                item[n] = cgen->arrowstyle_opmenu_items.Split_item;
                item_val[n] = AB_ARROW_SPLIT;
                n++;
                prop_options_init(
                    &(pss->arrow_style), cgen->arrowstyle_opmenu_label,
                    cgen->arrowstyle_opmenu, cgen->arrowstyle_opmenu_menu, n,
                    item, (XtPointer *)item_val, cgen->arrowstyle_cb);
                ...
                prop_item_editor_init(
                    &(pss->items), AB_ITEM_FOR_SPIN_BOX, cgen->itemlist,
                    cgen->itemlist_cb, cgen->item_add_button, n, item, item_val,
                    j, item2, item2_val, &(pss->item_label),
                    NULL /*label_type*/, NULL /*graphic_hint*/,
                    NULL /*mnemonic*/, NULL /*accel*/, NULL /*line_style*/,
                    &(pss->item_state), NULL /*submenu*/, &(pss->current_obj));
...
static int spinbox_prop_clear(AB_PROP_TYPE type) {
        ...
        prop_options_set_value(&(pss->label_type), (XtPointer)AB_LABEL_STRING,
                               False);
        ...
        /* Clear Arrow Style*/
        prop_options_set_value(&(pss->arrow_style), (XtPointer)AB_ARROW_END,
                               False);
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pss->init_state), AB_STATE_VISIBLE, True,
                                False);
        prop_checkbox_set_value(&(pss->init_state), AB_STATE_ACTIVE, True,
                                False);
...
static int spinbox_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
                             unsigned long loadkey) {
        PropSpinboxSettingsRec *pss = &(prop_spinbox_settings_rec[type]);
        AB_TEXT_TYPE spinbox_type;
        ...
        if (load_all) {
                ...
                prop_options_set_value(&(pss->label_type),
                                       (XtPointer)obj->label_type, False);
                prop_options_set_value(&(pss->label_pos),
                                       (XtPointer)obj_get_label_position(obj),
                                       False);

                /* Load Label */
                prop_setup_label_field(&(pss->label), NULL, obj->label_type,
                ...
                /* Load Arrow Style */
                prop_options_set_value(&(pss->arrow_style),
                                       (XtPointer)obj_get_arrow_style(obj),
                                       False);

                /* Load Min/Max/Incr */
                prop_field_set_numeric_value(&(pss->min),
                                             obj_get_min_value(obj), False);
                prop_field_set_numeric_value(&(pss->max),
                                             obj_get_max_value(obj), False);
                prop_field_set_numeric_value(&(pss->incr),
                                             obj_get_increment(obj), False);

                /* Load Initial Value/Decimal Points  */
                prop_field_set_numeric_value(
                    &(pss->ivalue), obj_get_initial_value_int(obj), False);
                prop_field_set_numeric_value(
                    &(pss->decimal), obj_get_decimal_points(obj), False);
...
int spinbox_prop_apply(AB_PROP_TYPE type) {
        PropSpinboxSettingsRec *pss = &(prop_spinbox_settings_rec[type]);
        AB_TEXT_TYPE spinbox_type;
        STRING value;
        int new_w;
        ...
        if (prop_changed(pss->label.changebar) ||
            prop_changed(pss->label_type.changebar)) {
                ...
                abobj_set_label_position(
                    pss->current_obj, (AB_COMPASS_POINT)prop_options_get_value(
                                          &(pss->label_pos)));
                size_chg = True;
        }
        if (prop_changed(pss->spinbox_type.changebar)) {
                spinbox_type =
                    (AB_TEXT_TYPE)prop_radiobox_get_value(&(pss->spinbox_type));

                if (spinbox_type == AB_TEXT_NUMERIC &&
                    obj_get_text_type(pss->current_obj) ==
                        AB_TEXT_DEFINED_STRING) {
                        int num_items;
                        DTB_MODAL_ANSWER answer = DTB_ANSWER_NONE;

                        XtVaGetValues(pss->items.item_list, XmNitemCount,
                                      &num_items, NULL);
                        if (num_items > 0) {
                                ...
                                answer = dtb_show_modal_message(
                                    ...
                                    &dtb_spinbox_chg_type_msg, NULL, NULL,
                                    NULL);
                                if (answer == DTB_ANSWER_CANCEL)
                                         ...
                                         */
                                        prop_item_editor_clear(&(pss->items));
                                        prop_set_changebar(pss->items.changebar,
                                                           PROP_CB_ON);
                                }
                        }
                }
        ...
        if (prop_changed(pss->arrow_style.changebar)) {
                abobj_set_arrow_style(pss->current_obj,
                                      (AB_ARROW_STYLE)prop_options_get_value(
                                          &(pss->arrow_style)));
        }
        if (prop_changed(pss->min.changebar)) {
                abobj_set_min_max_values(
                    pss->current_obj, prop_field_get_numeric_value(&(pss->min)),
        ...
        if (prop_changed(pss->geometry.changebar)) {
                if (abobj_width_resizable(pss->current_obj)) {
                        new_w = prop_geomfield_get_value(&(pss->geometry),
                                                         GEOM_WIDTH);
        ...
        if (prop_changed(pss->init_state.changebar)) {
                abobj_set_visible(pss->current_obj,
                                  prop_checkbox_get_value(&(pss->init_state),
                                                          AB_STATE_VISIBLE));
        ...
        if (prop_changed(pss->fg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pss->fg_color),
                                          obj_get_fg_color(pss->current_obj),
                                          False);
        }
        if (prop_changed(pss->bg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pss->bg_color),
                                          obj_get_bg_color(pss->current_obj),
                                          False);
        }
...
static BOOL spinbox_prop_pending(AB_PROP_TYPE type) {
        return (prop_changebars_pending(
            prop_spinbox_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if ((prop_changed(pss->label_type.changebar) ||
             prop_changed(pss->label.changebar)) &&
            prop_options_get_value(&(pss->label_type)) ==
        ...
        if (!prop_number_ok(pss->ivalue.field, "Initial Value Field", min,
                            max) ||
            !prop_number_ok(pss->decimal.field, "Decimal Field", 0, 99))
                return False;

        if (prop_changed(pss->geometry.changebar) &&
            (!prop_number_ok(pss->geometry.x_field, (STRING)XFieldStr,
                             -SHRT_MAX, SHRT_MAX) ||
             !prop_number_ok(pss->geometry.y_field, (STRING)YFieldStr,
                             -SHRT_MAX, SHRT_MAX) ||
             !prop_number_ok(pss->geometry.w_field, (STRING)WFieldStr, 1,
                             SHRT_MAX)))
                return False;
...
static void spinbox_typeCB(Widget item, XtPointer clientdata,
                           XmToggleButtonCallbackStruct *state) {
        ...
        if (state->set) {
                ...
                if (value == AB_TEXT_DEFINED_STRING) {
                        /* If type is changed to StringList and there are no
                         * items, inactivate the item settings.
                         */
                        XtVaGetValues(pss->items.item_list, XmNitemCount,
                                      &num_items, NULL);
                        if (num_items < 1) {
                                ui_field_set_editable(pss->item_label.field,
                                                      False);
...
static int spinbox_install_obj_destroy_CB() {
