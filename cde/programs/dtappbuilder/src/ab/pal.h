 ...
 */
typedef int (*PalItemInitProc)(ABObj);

typedef Widget (*PalItemPropInitProc)(Widget, AB_PROP_TYPE);

typedef int (*PalItemPropClearProc)(AB_PROP_TYPE);

typedef int (*PalItemPropActiveProc)(AB_PROP_TYPE, BOOL);

typedef int (*PalItemPropLoadProc)(ABObj, AB_PROP_TYPE, unsigned long);

typedef int (*PalItemPropApplyProc)(AB_PROP_TYPE);

typedef BOOL (*PalItemPropPendingFunc)(AB_PROP_TYPE);

typedef BOOL (*PalItemIsaFunc)(ABObj);
...
typedef struct _PAL_ITEM_INFO {
        ...
        PalItemPropActiveProc
            prop_activate;               /* sets prop sheet active/inactive */
...
typedef BOOL (*PalEditableObjTestFunc)(PalEditableObjInfo *edobj_info);
...
extern void pal_register_item_info(Widget pwidget, PalItemInfo *palitem,
                                   int subtype, String subname, Pixmap pixmap);

extern void pal_enable_item_drag(Widget item_widget, int subtype);

extern PalItemInfo *pal_get_item_info(ABObj obj);

extern PalItemInfo *pal_get_type_item_info(AB_OBJECT_TYPE type, int subtype);

extern Pixmap pal_get_item_pixmap(ABObj obj, int subtype, int *p_width,
                                  int *p_height);

extern STRING pal_get_item_subname(ABObj obj, int subtype);

extern void pal_register_editable_obj(AB_OBJECT_TYPE type, int subtype,
                                      STRING name, PalItemInfo *palitem);

extern PalEditableObjInfo *pal_get_editable_obj_info(ABObj obj);

extern void pal_add_editable_obj_menu_items(Widget menu_pane,
                                            XtCallbackProc item_cb,
                                            PalEditableObjTestFunc test_func);

extern int pal_initialize_obj(ABObj obj);

extern int pal_create_menu(AB_PROP_TYPE type, ABObj module, STRING menu_name,
                           STRING menu_title);

extern void pal_set_File_menu(AB_CHOOSER_TYPE chooser_type, BOOL active);
