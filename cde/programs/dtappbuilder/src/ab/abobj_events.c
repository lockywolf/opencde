 ...
 * Widget Action Procs
 */
static void object_button_press(Widget w, XEvent *ev, String *params,
                                int nparams);
static void object_mouse_enter(Widget w, XEvent *ev, String *params,
                               int nparams);
static void object_mouse_leave(Widget w, XEvent *ev, String *params,
                               int nparams);
static void object_mouse_motion(Widget w, XEvent *ev, String *params,
                                int nparams);
static void object_select(Widget w, XEvent *ev, String *params, int nparams);
static void object_toggle_select(Widget w, XEvent *ev, String *params,
                                 int nparams);
static void object_invoke_props(Widget w, XEvent *ev, String *params,
                                int nparams);
static void object_popup_menu(Widget w, XEvent *ev, String *params,
                              int nparams);
static void objects_pixel_move(Widget w, XEvent *ev, String *params,
                               int nparams);

/*
 * Widget Event Handlers
 */
static void interpose_button_event(Widget w, XtPointer cd, XEvent *ev,
                                   Boolean *cont);
static void object_button_drag(Widget w, XtPointer cd, XEvent *ev,
                               Boolean *cont);
static void object_move_release(Widget w, XtPointer cd, XEvent *ev,
                                Boolean *cont);
static void object_resize_release(Widget w, XtPointer cd, XEvent *ev,
                                  Boolean *cont);
static void object_mselect(Widget widget, XEvent *event, XRectangle *rb_rect,
                           XtPointer client_data);
static void object_track_move(Widget widget, XtPointer cd, XEvent *ev,
                              Boolean *cont);
static void object_track_iconify(Widget widget, XtPointer cd, XEvent *ev,
                                 Boolean *cont);
static void prevent_closeCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void wait_for_double(XtPointer client_data, XtIntervalId timer_id);

/*
 * Private Internal Procs
 */
static int initiate_move(Widget widget, ABObj obj);

static int initiate_resize(Widget widget, ABObj obj, RESIZE_DIR dir);

static int initiate_mselect(Widget widget, ABObj obj);
 ...
 */
String base_translations = "<Enter>:			ObjectMouseEnter() \n\
...
String btn3_menu_translations = "<Btn3Down>:               ObjectPopupMenu()";

String btn2_menu_translations = "<Btn2Down>:		ObjectPopupMenu()";
 ...
 */
int abobj_tree_set_build_actions(ABObj root) {
        AB_TRAVERSAL trav;
        ABObj obj;
 ...
 */
void abobj_register_build_actions(XtAppContext app) {
...
static void enable_on_internal_widgets(Widget widget, ABObj obj, ABObj root) {
        WidgetList children;
        int num_children = 0;
        int i;
        ...
        for (i = 0; i < num_children; ++i) {
                ...
                XtVaSetValues(children[i], XtNtranslations, build_transtbl,
                              NULL);

                XtAddEventHandler(children[i],
...
static void disable_on_internal_widgets(Widget widget, ABObj obj, ABObj root) {
        WidgetList children;
        int num_children = 0;
        ...
        for (i = 0; i < num_children; ++i) {
                ...
                XtRemoveEventHandler(
                    children[i], Button1MotionMask | Button2MotionMask, False,
                    object_button_drag, (XtPointer)root);

                orig_trans = objxm_get_actions_from_widget(children[i]);

                if (orig_trans)
                        XtVaSetValues(children[i], XtNtranslations, orig_trans,
                                      NULL);
                else
 ...
 */
void abobjP_enable_build_actions(ABObj obj, Widget widget) {
        ABObj evObj;
        ...
        if (obj_is_window(obj) && XtIsSubclass(widget, shellWidgetClass)) {
                ...
                ui_add_window_close_callback(widget, prevent_closeCB, NULL,
                                             XmDO_NOTHING);

                /* Track when a window is iconified */
                if (obj_is_base_win(obj))
                        ui_add_window_iconify_handler(
                            widget, object_track_iconify, (XtPointer)obj);

                /* Track when the user moves the window so that if it is
                 * re-instantiated, it will appear in the position the user
                 * placed it in. NOTE: this does not set a permanent position
                 * attribute for a window!
                 */
                XtAddEventHandler(widget, StructureNotifyMask, False,
                                  (XtEventHandler)object_track_move,
                                  (XtPointer)rootObj);
                return;
        }
        ...
        XtVaSetValues(widget, XtNtranslations, build_transtbl, NULL);
        ...
        /* The DtMenuButton widget uses an internal event-handler to post its
         * menu, so our standard translations will not get called. Therefore,
         * we use an event-handler to get these buttons events before the widget
         * does.
         */
        if (XtIsSubclass(widget, dtMenuButtonWidgetClass))
                XtInsertEventHandler(widget,
                                     ButtonMotionMask | ButtonPressMask |
                                         ButtonReleaseMask,
                                     False, interpose_button_event,
                                     (XtPointer)rootObj, XtListHead);
 ...
 */
void abobjP_disable_build_actions(ABObj obj, Widget widget) {
        ABObj evObj;
        ABObj rootObj = obj_get_root(obj);
        XtTranslations orig_trans;
        ...
        if (XtIsSubclass(widget, dtMenuButtonWidgetClass))
                XtRemoveEventHandler(
                 ...
                 !(obj_is_menubar(rootObj)))
                XtRemoveEventHandler(
                    widget, Button1MotionMask | Button2MotionMask, False,
 ...
 */
static void object_mouse_enter(Widget widget, XEvent *event, String *params,
                               int num_params) {
        ABObj obj = NULL;
        ABObj rootObj;
        char pos_str[12];
        ...
        if (obj != NULL) {
                ...
                sprintf(sz_str, "%3d X %3d", abobj_get_actual_width(rootObj),
                        abobj_get_actual_height(rootObj));
                ab_update_stat_region(
                    AB_STATUS_OBJ_TYPE,
                    pal_get_item_subname(rootObj, obj_get_subtype(rootObj)));
 ...
 */
static void object_mouse_leave(Widget widget, XEvent *event, String *params,
                               int num_params) {
 ...
 * Action: pointer has 'left' widget
 */
static void object_mouse_motion(Widget widget, XEvent *event, String *params,
                                int num_params) {
        ...
        if (obj != NULL) {
                ...
                if (rootObj != obj)
                        x_widget_translate_xy(
                            ...
                            (XtPointer)objxm_get_widget(rootObj), mevent->x,
                            mevent->y, &rx, &ry);
                else {
                        rx = mevent->x;
                        ry = mevent->y;
 ...
 */
static void object_select(Widget widget, XEvent *event, String *params,
                          int num_params) {
        ABObj obj = NULL;
 ...
 */
static void object_toggle_select(Widget widget, XEvent *event, String *params,
                                 int num_params) {
        ABObj obj, rootObj;
        ABSelectedRec sel;
        BOOL allow = True;
 ...
 */
static void object_invoke_props(Widget widget, XEvent *event, String *params,
                                int num_params) {
        ABObj obj = NULL;
 ...
 */
static void object_popup_menu(Widget widget, XEvent *event, String *params,
                              int num_params) {
        ABObj obj = NULL;
        ABSelectedRec sel;
        ...
        if (event->type == ButtonPress)
                abobj_popup_menu(WIN_EDIT_MENU, widget, &sel,
                                 (XButtonEvent *)event);
 ...
 */
static void objects_pixel_move(Widget widget, XEvent *event, String *params,
                               int num_params) {
        XKeyEvent *kevent;
        ABObj obj = NULL;
        ABSelectedRec sel;
        ...
        if (event->type == KeyPress) {
                ...
                XtTranslateKeycode(kevent->display, kevent->keycode,
                                   kevent->state, &mod, &keysym);
...
static int initiate_move(Widget widget, ABObj obj) {
        ...
        XtAddEventHandler(widget, ButtonReleaseMask, False, object_move_release,
                          (XtPointer)rootObj);
...
static int initiate_resize(Widget widget, ABObj obj, RESIZE_DIR dir) {
        ...
        if (XtGrabPointer(widget, False,
                          ButtonReleaseMask | ButtonMotionMask |
                              PointerMotionMask,
                          GrabModeAsync, GrabModeAsync, NULL, resize_cursor,
 ...
 */
static void object_button_press(Widget widget, XEvent *event, String *params,
                                int num_params) {
        XButtonEvent *bevent;
        ABObj obj = NULL;
        ABObj rootObj;
        ...
        if (event->type == ButtonPress) {
                ...
                {
                        resize_dir =
                            abobjP_find_resize_direction(obj, widget, event);

                        if (resize_dir ==
                            MOVE) /* MOVE (pending subsequent drag event) */
                                potential_move = True;

                        else if (bevent->button == 1 && resize_dir == NONE &&
                                 obj_is_pane(rootObj)) {
                                if (obj_is_control_panel(rootObj)) {
                                        /* RUBBERBAND-SELECT */
                                        if (ui_initiate_rubberband(
                                                widget, False, object_mselect,
                                                (XtPointer)obj) == ERROR) {
                                                mselect_in_progress = False;
                                                util_dprintf(
                                                    0, "object_button_press: "
                                                       "couldn't begin "
                                                       "rubberband\n");
                                        } else {
                             ...
                             ShiftMask))) /* RUBBERBAND-ADJUST */
                {
                        if (obj_is_control_panel(rootObj)) {
                                if (ui_initiate_rubberband(
                                        ...
                                        (XtPointer)obj) == ERROR) {
                                        mselect_in_progress = False;
                                        util_dprintf(0, "object_button_press: "
                                                        "couldn't begin "
                                                        "rubberband\n");
                                } else
                                        mselect_in_progress = mselect_adjust =
                                            True;
                        }
                }
        }
 ...
 */
static void interpose_button_event(Widget widget, XtPointer client_data,
        ...
        if (event->type == ButtonPress || event->type == ButtonRelease) {
                ...
                *cont_dispatch = False;
                ...
                case 1:
                        if (event->type == ButtonPress) {
                                if (bevent->state &
                                    ControlMask) /* Connections Accelerator */
                                {
                                ...
                                if (d_click ==
                                    NULL) /* Init DoubleClick structure */
                                {
                                        d_click =
                                            (DoubleClickInfo *)util_malloc(
                                                sizeof(DoubleClickInfo));
                                        if (d_click == NULL)
                                                return; /* yikes! */
                                        d_click->widget = NULL;
                                        d_click->event = NULL;
                                        d_click->interval = XtGetMultiClickTime(
                                            XtDisplay(widget));
                                        d_click->waiting = False;
                                }

                                if (d_click
                                        ->waiting) /* double-click occurred */
                                {
                                        XtRemoveTimeOut(timer_id);
                                        d_click->waiting = False;
                                        object_invoke_props(widget, event, NULL,
                                                            0);
                                } else if (bevent->state == 0) /* Start timing
                                                                  for
                                                                  double-click
                                                                */
                                {
                        ...
                        /* ButtonRelease */
                                ...
                                object_resize_release(widget, client_data,
                                                      event, cont_dispatch);

                        else if (move_in_progress) /* ButtonRelease */
                                object_move_release(widget, client_data, event,
                                                    cont_dispatch);
                ...
                case 2:
                        if (event->type == ButtonPress &&
                            AB_btn1_transfer != True) /* BUTTON2 TRANSFER */
                                object_button_press(widget, event, NULL, 0);

                        else if (event->type == ButtonRelease) {
                                if (AB_btn1_transfer != True &&
                                    move_in_progress) /* BUTTON2 TRANSFER */
                                        object_move_release(widget, client_data,
                                                            event,
                                                            cont_dispatch);
                                else
                                        object_toggle_select(widget, event,
                                                             NULL, 0);
                        }
                        break;
                ...
                if (mevent->state & Button1Mask) {
                        /* Check to see if the drag occurred while we are
                         * waiting for hte double-click interval to expire. If
                         * so, then if the drag is outside the threshhold,
                         * cancel the double-click and initiate a move or
                         * resize.
                         */
                        if (d_click->waiting) {
                                if (abs(mevent->x - d_click->start_x) >
                                        AB_drag_threshold ||
                                    abs(mevent->y - d_click->start_y) >
                                        AB_drag_threshold) {
                                        XtRemoveTimeOut(timer_id);
                                        d_click->waiting = False;
                                        object_button_press(d_click->widget,
                                                            d_click->event,
                                                            NULL, 0);
                                }
                        } else /* call the generic drag event-handler to process
                                  the drag */
                                object_button_drag(widget, client_data, event,
                                                   cont_dispatch);
                } else if (mevent->state & Button2Mask &&
                           AB_btn1_transfer != True)
                        object_button_drag(widget, client_data, event,
                                           cont_dispatch);
 ...
 */
static void object_button_drag(Widget widget, XtPointer client_data,
                               XEvent *event, Boolean *cont_dispatch) {
        XMotionEvent *mevent;
        ABObj obj = (ABObj)client_data;
        ABObj moveObj;
        ...
        if (event->type == MotionNotify) {
                ...
                /* RESIZE, MOVE or RUBBERBAND-SELECT */
                {
                        if (resize_in_progress) {
                                abobjP_resize_object_outline(obj, event,
                                                             resize_dir);
                        } else if (potential_move) {
                                /* If moving a Control within a Group, move the
                                 * whole Group instead.
                                 */
                                moveObj = obj_get_root(obj);
                                while (obj_is_group(
                                    obj_get_root(obj_get_parent(moveObj))))
                                        moveObj = obj_get_root(
                                            obj_get_parent(moveObj));

                                if (moveObj !=
                                    obj_get_root(obj)) /* moveObj is translated
                                                          to Group */
                                {
                                        Widget orig_w;
                                        int trans_x, trans_y;
                                         ...
                                         */
                                        obj = objxm_comp_get_subobj(
                                            moveObj, AB_CFG_PARENT_OBJ);
                                        orig_w = widget;
                                        widget = objxm_get_widget(obj);

                                        /* Translate Control's x,y position
                                         * relative to Group 0,0 */
                                        XTranslateCoordinates(
                                            XtDisplay(orig_w), XtWindow(orig_w),
                                            XtWindow(widget), mevent->x,
                                            mevent->y, &trans_x, &trans_y,
                                            &win);

                                        mevent->x = trans_x;
                                        mevent->y = trans_y;
                                }
                                if (!move_in_progress) /* Initialize Move
                                                          operation */
                                {
                                        if (!obj_is_selected(moveObj))
                                                object_select(widget, NULL,
                                                              NULL, 0);

                                        if (initiate_move(widget, obj) ==
                                            ERROR) {
                                                move_in_progress = False;
                                                potential_move = False;
                                                util_dprintf(
                                                    0, "object_select_drag: "
                                                       "couldn't begin move\n");
                                        } else
 ...
 *         move the object!
 */
static void object_move_release(Widget widget, XtPointer client_data,
                                XEvent *event, Boolean *cont_dispatch) {
        ABObj obj = (ABObj)client_data;
 ...
 *          resize the object!
 */
static void object_resize_release(Widget widget, XtPointer client_data,
                                  XEvent *event, Boolean *cont_dispatch) {
        ABObj obj = (ABObj)client_data;
...
static void object_mselect(Widget widget, XEvent *event, XRectangle *rb_rect,
                           XtPointer client_data) {
        ABObj control, pobj, xyobj, obj = NULL;
        ...
        for (i = 0; i < num_controls; i++) {
                ...
                if (obj_is_salient(control)) {
                        xyobj =
                            objxm_comp_get_subobj(control, AB_CFG_POSITION_OBJ);

                        if (objxm_get_widget(xyobj) != NULL) {
                                x_get_widget_rect(objxm_get_widget(xyobj),
                                                  &c_rect);

                                if (rect_includesrect(rb_rect, &c_rect)) {
                                        /*
                                         * If RUBBERBAND-SELECT, deselect other
                                         * currently selected objects. If
                                         * RUBBERBAND-ADJUST, deselect only
                                         * currently selected NON-sibling
                                         * objects
                                         */
                                        if (old_sel.count > 0) {
                                                for (j = 0; j < old_sel.count;
                                                     j++) {
                                                        if (control !=
                                                                old_sel
                                                                    .list[j] &&
...
static void prevent_closeCB(Widget widget, XtPointer client_data,
                            XtPointer call_data) {
...
static void object_track_move(Widget widget, XtPointer client_data,
                              XEvent *event, Boolean *cont_disp) {
...
static void object_track_iconify(Widget widget, XtPointer client_data,
                                 XEvent *event, Boolean *cont_disp) {
        ABObj winobj = (ABObj)client_data;
        ...
        case MapNotify:
                ...
                else if (InBuildMode && obj_has_flag(winobj, IconifiedFlag) &&
                         !ab_window_leader_iconified()) {
                        obj_clear_flag(winobj, IconifiedFlag);
                        XtVaSetValues(widget, XtNinitialState, NormalState,
                                      NULL);
                }
                break;
        ...
        case UnmapNotify:
                 ...
                 */
                        ...
                        XtVaSetValues(widget, XtNinitialState, IconicState,
                                      NULL);
                }
                break;
        }
