...
**                                                                      **
**************************************************************************/
static void clear_layout(ABObj obj);
...
**                                                                      **
**************************************************************************/
int abobj_layout_changed(ABObj obj) {
 ...
 */
int abobj_calculate_new_layout(ABObj obj, int new_x, int new_y,
        ...
        if (obj->ui_handle == NULL) {
                if (util_get_verbosity() > 0)
                        fprintf(stderr,
                                "abobj_calculate_new_layout: %s: NULL widget\n",
                                obj_get_name(obj));
                return -1;
        }

        XtVaGetValues((Widget)obj->ui_handle, XmNx, &x, XmNy, &y, XmNwidth,
                      &width, XmNheight, &height, NULL);

        parent = obj_get_parent(obj);
        XtVaGetValues((Widget)parent->ui_handle, XmNwidth, &p_width, XmNheight,
                      &p_height, NULL);
 ...
 */
void abobj_sort_children(ABObj obj, int sort) {
        ABObj child1, child2;
 ...
 */
void clear_layout(ABObj obj) {
        obj_set_attachment(obj, AB_CP_NORTH, AB_ATTACH_POINT, NULL,
                           obj_get_y(obj));
        obj_set_attachment(obj, AB_CP_WEST, AB_ATTACH_POINT, NULL,
                           obj_get_x(obj));
        obj_set_attachment(obj, AB_CP_SOUTH, AB_ATTACH_NONE, NULL, 0);
        obj_set_attachment(obj, AB_CP_EAST, AB_ATTACH_NONE, NULL, 0);
}
 ...
 */
void abobj_clear_layout(ABObj obj, BOOL clear_children, BOOL init_attachments) {
