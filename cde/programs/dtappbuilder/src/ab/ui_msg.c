...
static Widget create_message_dlg(Widget parent, unsigned char dlg_type,
                                 STRING title, STRING msg, STRING ok_label,
                                 STRING cancel_label, STRING help_label,
...
static Widget create_prompt_dlg(Widget parent, STRING title, STRING label,
                                STRING ok_label, STRING cancel_label,
                                STRING help_label,
                                unsigned char default_button_type);

static void prompt_ok_modal_dlgCB(Widget widget, XtPointer client_data,
                                  XmSelectionBoxCallbackStruct *call_data);
static void prompt_cancel_modal_dlgCB(Widget widget, XtPointer client_data,
                                      XmSelectionBoxCallbackStruct *call_data);
static void prompt_close_modal_dlg(Widget widget, XtPointer client_data,
                                   XtPointer call_data);
 ...
 */
void ui_popup_info_message(Widget parent, STRING title, STRING msg) {
        ui_popup_message(parent, XmDIALOG_INFORMATION, title, msg, "OK", NULL,
                         NULL, NULL, "Help", NULL, XmDIALOG_OK_BUTTON);
}
 ...
 */
void ui_popup_message(Widget parent, unsigned char msg_type, STRING title,
                      STRING msg, STRING ok_label, XtCallbackProc ok_callback,
                      STRING cancel_label, XtCallbackProc cancel_callback,
                      STRING help_label, XtCallbackProc help_callback,
                      unsigned char default_button_type) {
        Widget msg_dlg;

        msg_dlg =
            create_message_dlg(parent, msg_type, title, msg, ok_label,
                               cancel_label, help_label, default_button_type);

        /* Add Callbacks if necessary */
        if (ok_callback != NULL)
                XtAddCallback(msg_dlg, XmNokCallback, ok_callback, NULL);
        if (cancel_callback != NULL)
                XtAddCallback(msg_dlg, XmNcancelCallback, cancel_callback,
                              NULL);
...
ui_popup_modal_message(Widget parent, unsigned char msg_type, STRING title,
                       STRING msg, STRING ok_label, STRING cancel_label,
        ...
        XtVaSetValues(
            modal_dlg_pane, XmNdialogStyle, XmDIALOG_FULL_APPLICATION_MODAL,
            XmNokCallback, &ok_callback, XmNcancelCallback, &cancel_callback,
            XmNhelpCallback, &help_callback, XmNuserData, &answer, NULL);
...
static Widget create_message_dlg(Widget parent, unsigned char msg_type,
                                 STRING title, STRING msg, STRING ok_label,
                                 STRING cancel_label, STRING help_label,
                                 unsigned char default_button_type) {
        Widget msg_dlg;
        ...
        msg_dlg = XmCreateMessageDialog(shell, "ab_message_dialog", NULL, 0);
        ...
        if (cancel_label == NULL) {
                ...
                XtSetArg(arg[n], XmNcancelLabelString, c_xmlabel);
                n++;
        }
        if (ok_label == NULL) {
                ...
                XtSetArg(arg[n], XmNokLabelString, ok_xmlabel);
                n++;
        }
        if (help_label == NULL) {
                ...
                XtSetArg(arg[n], XmNhelpLabelString, h_xmlabel);
                n++;
        }
        XtSetArg(arg[n], XmNmessageAlignment, XmALIGNMENT_CENTER);
        n++;
        XtSetArg(arg[n], XmNdialogType, msg_type);
        n++;
         ...
         */
        XtVaSetValues(msg_dlg, XtVaTypedArg, XmNdialogTitle, XtRString, title,
                      strlen(title) + 1, XtVaTypedArg, XmNmessageString,
                      XtRString, msg, strlen(msg) + 1, XmNmessageAlignment,
                      XmALIGNMENT_CENTER, XmNdefaultButtonType,
                      default_button_type, NULL);
...
static Widget create_prompt_dlg(Widget parent, STRING title, STRING label,
        ...
        XtSetArg(arg[n], XmNautoUnmanage, FALSE);
        n++;
        prompt_dlg = XmCreatePromptDialog(shell, "ab_prompt_dialog", arg, n);

        n = 0;
        /* We don't want a "Cancel" button */
        if (cancel_label == NULL) {
                button =
                    XmSelectionBoxGetChild(prompt_dlg, XmDIALOG_CANCEL_BUTTON);
                XtUnmanageChild(button);
        } else {
                c_xmlabel = XmStringCreateLocalized(cancel_label);
                XtSetArg(arg[n], XmNcancelLabelString, c_xmlabel);
        ...
        if (help_label == NULL) {
                button =
                    XmSelectionBoxGetChild(prompt_dlg, XmDIALOG_HELP_BUTTON);
                XtUnmanageChild(button);
        } else {
                h_xmlabel = XmStringCreateLocalized(help_label);
                XtSetArg(arg[n], XmNhelpLabelString, h_xmlabel);
        ...
        case XmDIALOG_OK_BUTTON:
                default_button =
                    XmSelectionBoxGetChild(prompt_dlg, XmDIALOG_OK_BUTTON);
                break;
         ...
         * Motif converter available in Varargs.
         */
        XtVaSetValues(prompt_dlg, XtVaTypedArg, XmNdialogTitle, XtRString,
                      title, strlen(title) + 1, XtVaTypedArg,
                      XmNselectionLabelString, XtRString, label, strlen(label),
 ...
 */
void ui_popup_prompt(Widget parent, STRING title, STRING label,
                     ...
                     XtCallbackProc help_callback, XtPointer help_clientData,
                     unsigned char default_button_type) {
        Widget prompt_dlg, textf;

        prompt_dlg =
            create_prompt_dlg(parent, title, label, ok_label, cancel_label,
                              help_label, default_button_type);

        /* Add Callbacks if necessary */
        if (ok_callback != NULL)
                XtAddCallback(prompt_dlg, XmNokCallback, ok_callback,
                              ok_clientData);
        if (cancel_callback != NULL)
                XtAddCallback(prompt_dlg, XmNcancelCallback, cancel_callback,
                              cancel_clientData);
        if (help_callback != NULL)
                XtAddCallback(prompt_dlg, XmNhelpCallback, help_callback,
                              help_clientData);
        /* destroy the dialog after popdown */
        XtAddCallback(XtParent(prompt_dlg), XtNpopdownCallback, destroyCB,
                      (XtPointer)1);
        ...
        if (initial_text != NULL) {
                ...
                XmTextFieldSetSelection(textf, 0, strlen(initial_text),
                                        CurrentTime);
...
ui_popup_modal_prompt(Widget parent, STRING title, STRING label,
        ...
        ab_register_window(modal_prompt, AB_WIN_MODAL, WindowUp, parent,
                           AB_WPOS_STACK_CENTER, prompt_close_modal_dlg,
                           (XtPointer)&answer);
        ...
        XtVaSetValues(modal_prompt, XmNdialogStyle,
                      XmDIALOG_PRIMARY_APPLICATION_MODAL, XmNokCallback,
                      &ok_callback, XmNcancelCallback, &cancel_callback,
                      XmNuserData, &prompt_info, NULL);
        if (!util_strempty(help_label) && (help_data != NULL)) {
                XtAddCallback(modal_prompt, XmNhelpCallback, dtb_help_dispatch,
                              help_data);
        }
