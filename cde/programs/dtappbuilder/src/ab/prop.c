...
static char *menu_names[] = {"none_item", "new_menu_item", "menus_item",
                             "edit_item"};
 ...
 */
static void revolv_dialog_init(Widget parent);
static void revolv_objectspane_init(DtbRevolvPropDialogInfo cgen);
static void revolv_basicframe_init(DtbRevolvPropDialogInfo cgen);
static PalItemInfo *revolv_set_prop_type(PalItemInfo *palitem, ABObj loadobj);
static void revolv_change_prop_frame(PalItemInfo *palitem);
static PalItemInfo *revolv_invoke_props(PalItemInfo *palitem, ABObj loadobj);

/* Fixed Dialog Functions
 */
static void fixed_invoke_props(PalItemInfo *palitem);

/* General Prop Manipulation Functions
 */
static int apply_props(AB_PROP_TYPE ptype, Widget dialog);
static int apply_prop_changes(AB_PROP_TYPE ptype, ABObj obj,
                              PalItemInfo *palitem);
static DTB_MODAL_ANSWER handle_auto_apply(AB_PROP_TYPE ptype,
                                          PropStateInfo *pstate,
                                          PalItemInfo *new_palitem,
                                          ABObj new_obj, PalItemInfo **result);
static PropStateInfo *get_prop_state_info(Widget widget);
static void init_prop_state_info(Widget dialog, PalItemInfo *palitem,
                                 Widget objlist, ABObj loaded_obj);
static void load_props(AB_PROP_TYPE ptype, PropStateInfo *pstate, ABObj obj);
static void create_obj_option_item(PropObjOptionsSetting pos, Widget item,
                                   ABObj obj);

/* Object List Manipulation Functions
 */
static int objlist_delete_obj(AB_PROP_TYPE type, PalItemInfo *palitem,
                              ObjEvDestroyInfo info);
static void objlist_ensure_selection(Widget objlist);
static void objlist_load(AB_PROP_TYPE type, PropStateInfo *pstate);
static int objlist_rename_obj(AB_PROP_TYPE type, PalItemInfo *palitem,
                              ObjEvAttChangeInfo info);
static int objlist_show_or_hide_obj(AB_PROP_TYPE type, PalItemInfo *palitem,
                                    ObjEvUpdateInfo info);
static BOOL objlist_test_func(ABObj test_obj);

/*
 * Object-Callbacks
 */
static int obj_renamedOCB(ObjEvAttChangeInfo info);
static int obj_shown_or_hiddenOCB(ObjEvUpdateInfo info);
static int obj_geom_changedOCB(ObjEvAttChangeInfo info);
static int obj_destroyedOCB(ObjEvDestroyInfo info);
 ...
 */
static void apply_propsCB(Widget w, XtPointer clientdata, XtPointer calldata);
static void cancel_propsCB(Widget w, XtPointer clientdata, XtPointer calldata);
static void change_objecttypeCB(Widget w, XtPointer clientdata,
                                XtPointer calldata);
static void close_propsCB(Widget w, XtPointer clientdata, XtPointer calldata);
static void invoke_attach_editorCB(Widget w, XtPointer cldata,
                                   XtPointer calldata);
static void invoke_connectionsCB(Widget w, XtPointer cldata,
                                 XtPointer calldata);
static void invoke_help_editorCB(Widget w, XtPointer cldata,
                                 XtPointer calldata);
static void objlist_selectCB(Widget widget, XtPointer client_data,
                             XmListCallbackStruct *listdata);
static void ok_propsCB(Widget w, XtPointer clientdata, XtPointer calldata);
static void reset_propsCB(Widget w, XtPointer clientdata, XtPointer calldata);
static void tearoff_propsCB(Widget w, XtPointer clientdata, XtPointer calldata);

/* Color Field Callbacks
 */
static void color_chooserCB(Widget widget, XtPointer clientdata,
                            XtPointer calldata);
static void color_noneCB(Widget widget, XtPointer clientdata,
                         XtPointer calldata);

/* Menu Name Setting Callbacks
 */
static void menu_newCB(Widget widget, XtPointer client_data,
                       XtPointer call_data);
static void menu_editCB(Widget widget, XtPointer client_data,
                        XtPointer call_data);
static void menu_edit_set_stateCB(Widget widget, XtPointer client_data,
                                  XtPointer call_data);
static void menulist_buildCB(Widget widget, XtPointer client_data,
                             XtPointer call_data);
static void menulist_destroyCB(Widget widget, XtPointer client_data,
                               XtPointer call_data);
static void menuname_clearCB(Widget widget, XtPointer clientdata,
                             XtPointer calldata);
static void menuname_setCB(Widget widget, XtPointer client_data,
                           XtPointer call_data);
static void menu_field_chgCB(Widget widget, XtPointer client_data,
                             XtPointer call_data);

static void obj_options_buildCB(Widget widget, XtPointer client_data,
                                XtPointer call_data);

/*
 * Misc Functions
 */
static void find_submenu_owners(ABObj submenu, ABObj *menulist, int menu_count,
                                ABObj *i_menulist, int *i_count_ptr);
static BOOL is_submenu_of(STRING submenu_name, ABObj menu);
static BOOL menu_in_list(ABObj *menulist, int menu_count, ABObj target_menu);
 ...
 */
void prop_init(Widget toplevel) {
        strings_init();
 ...
 */
void prop_show_dialog(Widget widget, XtPointer client_data,
                      XtPointer call_data) {
        ABSelectedRec sel;
 ...
 * Palette Item type.
 */
void prop_show_fixed(PalItemInfo *palitem) {
        PropStateInfo *pstate;
 ...
 */
void prop_show_menu_props(Widget w, XtPointer client_data, XtPointer calldata) {
 ...
 */
void prop_show_group_props(Widget w, XtPointer client_data,
                           XtPointer calldata) {
 ...
 */
void prop_load_obj(ABObj obj, AB_PROP_TYPE ptype) {
        PalItemInfo *palitem;
        ...
        if ((viz_palitem = palitem = pal_get_item_info(obj)) != NULL) {
                ...
                util_dprintf(1,
                             "prop_load_obj: %s : could not get Palette info\n",
...
void prop_fixed_dialog_init(PalItemInfo *palitem, Widget dialog,
                            Widget objlist) {
         ...
         */
        ab_register_window(dialog, AB_WIN_DIALOG, WindowHidden, NULL,
                           AB_WPOS_UNSPECIFIED, close_propsCB,
                           (XtPointer)AB_PROP_FIXED);

        /* Store current-object in List (init to NULL)  */
        XtVaSetValues(objlist, XmNselectionPolicy, XmBROWSE_SELECT, XmNuserData,
                      NULL, NULL);
...
void prop_activate_panel_init(AB_PROP_TYPE type, PalItemInfo *palitem,
                              Widget ok_button, Widget apply_button,
                              Widget reset_button, Widget cancel_button,
                              Widget help_button) {
        Widget dialog;
        ...
        XtVaSetValues(apply_button, XmNshowAsDefault, True,
                      XmNdefaultButtonShadowThickness, 1, NULL);
        XtVaSetValues(ok_button, XmNdefaultButtonShadowThickness, 1, NULL);
        XtVaSetValues(reset_button, XmNdefaultButtonShadowThickness, 1, NULL);
        XtVaSetValues(cancel_button, XmNdefaultButtonShadowThickness, 1, NULL);
        if (help_button != NULL)
                XtVaSetValues(help_button, XmNdefaultButtonShadowThickness, 1,
                              NULL);

        XtAddCallback(apply_button, XmNactivateCallback, apply_propsCB,
                      (XtPointer)type);
        XtAddCallback(reset_button, XmNactivateCallback, reset_propsCB,
                      (XtPointer)type);
        XtAddCallback(ok_button, XmNactivateCallback, ok_propsCB,
                      (XtPointer)type);
        XtAddCallback(cancel_button, XmNactivateCallback, cancel_propsCB,
                      (XtPointer)type);
}

void prop_editors_panel_init(AB_PROP_TYPE type, PalItemInfo *palitem,
                             Widget attach_ed_button, Widget conn_ed_button,
...
void prop_checkbox_init(PropCheckboxSetting pcs, Widget label, Widget checkbox,
                        int num_items, WidgetList items, int *item_keys,
                        Widget changebar) {
        int i;
        ...
        for (i = 0; i < num_items; i++) {
                XtVaSetValues(items[i], XmNuserData, (XtArgVal)item_keys[i],
                              NULL);
...
int prop_checkbox_set_value(PropCheckboxSetting pcs, int itemkey, BOOL value,
                            BOOL trip_changebar) {
        int num_children = 0;
        ...
        XtVaGetValues(pcs->checkbox, XtNnumChildren, &num_children, XtNchildren,
                      &children, NULL);
        ...
        for (i = 0; i < num_children; i++) {
                ...
                {
                        XmToggleButtonSetState(children[i], value,
                                               trip_changebar ? TRUE : FALSE);
                        return OK;
                }
        }
...
BOOL prop_checkbox_get_value(PropCheckboxSetting pcs, int itemkey) {
        int num_children = 0;
        WidgetList children = NULL;
        XtArgVal childkey;
        int i;
...
void prop_colorfield_init(PropColorSetting pcs, Widget menubutton,
                          Widget none_item, Widget chooser_item, Widget swatch,
                          Widget field, Widget changebar) {
        ...
        XtAddCallback(chooser_item, XmNactivateCallback, color_chooserCB,
                      (XtPointer)pcs);

        XtAddCallback(none_item, XmNactivateCallback, color_noneCB,
                      (XtPointer)pcs);
        ...
        XtVaGetValues(XtParent(changebar), XmNforeground, &contrast_pix, NULL);
        XtVaSetValues(swatch, XmNborderColor, (XtArgVal)contrast_pix,
                      XmNborderWidth, (XtArgVal)1, XmNuserData,
                      (XtArgVal)default_pix, XmNrecomputeSize, (XtArgVal)False,
                      NULL);
        ...
        XtAddCallback(field, XmNvalueChangedCallback, propP_field_chgCB,
                      (XtPointer)changebar);
}

STRING
prop_colorfield_get_value(PropColorSetting pcs) {
        STRING value;
...
int prop_colorfield_set_value(PropColorSetting pcs, STRING colorname,
                              BOOL trip_changebar) {
        Pixel pixel;

        if (util_strempty(colorname)) /* no color - swatch is 'invisible' */
                XtVaGetValues(XtParent(pcs->swatch), XmNbackground, &pixel,
                              NULL);

        else if (objxm_name_to_pixel(AB_toplevel, colorname, &pixel) != OK) {
                if (objxm_color_exists(
                        colorname)) /* color couldn't be allocated */
                {
                ...
                {
                        util_dprintf(1,
                                     "prop_colorfield_set_value: \"%s\" is not "
                                     "an existing color name.\n",
                                     colorname);
                        return ERR;
                }
        }
        XtVaSetValues(pcs->swatch, XmNbackground, (XtArgVal)pixel, NULL);

        if (!trip_changebar)
                /* Set state so changebar is not triggered */
                XtVaSetValues(pcs->field, XmNuserData, (XtArgVal)PROP_LOAD,
                              NULL);
        ...
        if (!trip_changebar)
                /* Reset state */
                XtVaSetValues(pcs->field, XmNuserData, (XtArgVal)PROP_EDIT,
                              NULL);

        return OK;
}

void prop_obj_combobox_init(PropObjComboboxSetting pcs, Widget label,
                            Widget combobox, Widget changebar,
...
ABObj prop_obj_combobox_get_value(PropObjComboboxSetting pcs) {
        ABObj obj;
        STRING name;
        XmStringTable selected;
        int select_count = 0;

        XtVaGetValues(pcs->list, XmNselectedItems, &selected,
                      XmNselectedItemCount, &select_count, NULL);

        if (select_count == 0) {
                util_dprintf(
                    1,
                    "prop_obj_combobox_get_value: No items select in List\n");
                return NULL;
        }
        name = objxm_xmstr_to_str(selected[0]);

        if (util_strcmp(name, NoneItem) != 0)
                obj = obj_find_by_name(obj_get_module(*pcs->current_obj_ptr),
                                       name);
        else
...
int prop_obj_combobox_set_value(PropObjComboboxSetting pcs, ABObj obj,
                                BOOL trip_changebar) {
        ui_combobox_select_item(pcs->combobox,
                                (STRING)(obj ? obj_get_name(obj) : NoneItem));
        prop_set_changebar(pcs->changebar,
                           trip_changebar ? PROP_CB_ON : PROP_CB_OFF);

        return 0;
}

void prop_obj_combobox_load(PropObjComboboxSetting pcs, ABObj root,
                     ...
                     (obj = trav_next(&trav)) != NULL;)
                        if ((*obj_test_func)(obj) == True)
                                ui_combobox_add_item(
                                    pcs->combobox, obj_get_name(obj), 0, False);

                trav_close(&trav);
        }
}

void prop_field_init(PropFieldSetting pfs, Widget label, Widget field,
                     Widget changebar) {
        /* Store Widget-IDs */
        pfs->label = label;
        pfs->field = field;
...
STRING
prop_field_get_value(PropFieldSetting pfs) {
        STRING value;
...
int prop_field_set_value(PropFieldSetting pfs, STRING value,
                         BOOL trip_changebar) {
        return (propP_field_set_value(pfs->field, value, trip_changebar));
}

int prop_field_get_numeric_value(PropFieldSetting pfs) {
        STRING value;
        int num;
...
int prop_field_set_numeric_value(PropFieldSetting pfs, int value,
                                 BOOL trip_changebar) {
        char valstr[8];

        if (!trip_changebar)
                /* Set state so changebar is not triggered */
                XtVaSetValues(pfs->field, XmNuserData, (XtArgVal)PROP_LOAD,
                              NULL);
        ...
        if (!trip_changebar)
                /* Reset state */
                XtVaSetValues(pfs->field, XmNuserData, (XtArgVal)PROP_EDIT,
                              NULL);

        return OK;
}

void prop_footer_init(PropFooterSetting pfs, Widget label, Widget checkbox,
                      int num_items, WidgetList items, int *item_keys,
                      Widget field, Widget changebar) {
        int i;
...
void prop_geomfield_init(PropGeometrySetting pgs, Widget label, Widget x_label,
                         Widget x_field, Widget y_label, Widget y_field,
                         Widget w_label, Widget w_field, Widget h_label,
                         Widget h_field, Widget changebar) {
        /* Store Widget-IDs */
        pgs->label = label;
        pgs->x_label = x_label;
        pgs->x_field = x_field;
        ...
        propP_changebar_init(
            changebar,
            x_field ? x_field
                    : (y_field ? y_field : (w_field ? w_field : h_field)));
...
int prop_geomfield_get_value(PropGeometrySetting pgs, GEOM_KEY gkey) {
        Widget field;
        STRING value;
        int gvalue = 0;
...
int prop_geomfield_set_value(PropGeometrySetting pgs, GEOM_KEY gkey, int gvalue,
                             BOOL trip_changebar) {
        Widget field;
...
void prop_load_obj_position(ABObj obj, PropGeometrySetting pgs) {
...
void prop_load_obj_size(ABObj obj, PropGeometrySetting pgs) {
        BOOL resizable;

        if (pgs->w_field) {
                prop_geomfield_set_value(pgs, GEOM_WIDTH,
        ...
        if (pgs->h_field) {
                prop_geomfield_set_value(pgs, GEOM_HEIGHT,
                                         abobj_get_comp_height(obj), False);
...
void prop_size_policyCB(Widget w, XtPointer clientdata, XtPointer calldata) {
...
void prop_menuname_init(PropMenunameSetting pms, AB_PROP_TYPE ptype,
                        Widget label, Widget menubutton, Widget field,
                        Widget owner_name_field, Widget changebar,
                        PropFieldSetting menu_title_pfs, ABObj *current_obj_ptr,
                        BOOL new_item_ok) {
        Widget submenu;
        Widget menu_items[4];
        int i;
        ...
        for (i = 0; i < XtNumber(menu_items); i++) {
                menu_items[i] = XtVaCreateManagedWidget(
                    menu_names[i], xmCascadeButtonWidgetClass, submenu,
                    XtVaTypedArg, XmNlabelString, XtRString, menu_strs[i],
                    strlen(menu_strs[i]) + 1, XmNuserData, pms, NULL);
        }

        XtAddCallback(menu_items[NO_MENU], XmNactivateCallback,
                      menuname_clearCB, (XtPointer)pms);

        if (!new_item_ok)
                XtDestroyWidget(menu_items[NEW_MENU]);
        else
                XtAddCallback(menu_items[NEW_MENU], XmNactivateCallback,
                              menu_newCB, (XtPointer)pms);
        ...
        if (menu_title_pfs) {
                XtAddCallback(field, XmNvalueChangedCallback, menu_field_chgCB,
                              (XtPointer)pms);
...
STRING
prop_menuname_get_value(PropMenunameSetting pms) {
        STRING value;
...
int prop_menuname_set_value(PropMenunameSetting pms, STRING value,
                            BOOL trip_changebar) {
        return (propP_field_set_value(pms->field, value, trip_changebar));
}

void prop_options_init(PropOptionsSetting pos, Widget label, Widget optionbox,
                       Widget menu, int num_items, WidgetList items,
                       XtPointer *item_values, Widget changebar) {
        int i;
        ...
        for (i = 0; i < num_items; i++) {
                XtVaSetValues(items[i], XmNuserData, (XtArgVal)item_values[i],
                              NULL);
...
XtPointer prop_options_get_value(PropOptionsSetting pos) {
        XtArgVal value = -1;
...
int prop_options_set_value(PropOptionsSetting pos, XtPointer value,
                           BOOL trip_changebar) {
        int num_children;
        WidgetList children;
        XtArgVal child_val;
        ...
        XtVaGetValues(pos->menu, XtNnumChildren, &num_children, XtNchildren,
                      &children, NULL);

        for (i = 0; i < num_children; i++) {
                XtVaGetValues(children[i], XmNuserData, &child_val, NULL);

                if (child_val == (XtArgVal)value) {
                        XtVaSetValues(pos->optionbox, XmNmenuHistory,
                                      children[i], NULL);
                        XtVaSetValues(pos->menu, XmNuserData,
                                      (XtArgVal)child_val, NULL);
 ...
 */
int prop_options_remove_value(PropOptionsSetting pos, XtPointer value,
                              BOOL trip_changebar) {
        int num_children;
        WidgetList children;
        XtArgVal child_val, reset_val;
        int i;
        ...
        for (i = 0; i < num_children; i++) {
                ...
                if (child_val == (XtArgVal)value) {
                         ...
                         *	- if current setting value is the same as the
                         *one we are deleting, set the setting value to be the
                         *previous item (if that is not the deleted one)
                         *	- unmanage item widget
                         */

                        if (cur_val == (XtArgVal)value) {
                                if ((i == 0) && (num_children != 1)) {
                                        XtVaGetValues(children[1], XmNuserData,
                                                      &reset_val, NULL);
                                        prop_options_set_value(
                                            pos, (XtPointer)reset_val,
                                            trip_changebar);
                                } else if (i != 0)
                                        prop_options_set_value(
                                            pos, (XtPointer)reset_val,
...
void prop_obj_options_init(PropObjOptionsSetting pos, Widget label,
                           Widget optionbox, Widget menu, int num_items,
                           WidgetList items, XtPointer *item_values,
                           Widget changebar, BOOL display_module,
                           ABObj *current_obj_ptr,
                           ABObjTestFunc obj_test_func) {
        Widget shell;

        prop_options_init(&(pos->options), label, optionbox, menu, num_items,
                          items, item_values, changebar);
        pos->display_module = display_module;
        pos->current_obj_ptr = current_obj_ptr;
        pos->obj_test_func = obj_test_func;
        pos->search_root = NULL;

        /* Setup callback to dynamically build Object list when Menu is popped
         * up */
        shell = ui_get_ancestor_shell(menu);
        XtAddCallback(shell, XtNpopupCallback, obj_options_buildCB,
                      (XtPointer)pos);
}

ABObj prop_obj_options_get_value(PropObjOptionsSetting pos) {
        return ((ABObj)prop_options_get_value(&(pos->options)));
}

int prop_obj_options_set_value(PropObjOptionsSetting pos, ABObj obj,
                               BOOL trip_changebar) {
        return (prop_options_set_value(&(pos->options), (XtPointer)obj,
                                       trip_changebar));
}

void prop_obj_options_load(PropObjOptionsSetting pos, ABObj search_root) {
        AB_TRAVERSAL trav;
        ABObj obj;
        int i, num_items;
        WidgetList items;
        ...
        XtVaGetValues(pos->options.menu, XmNchildren, &items, XmNnumChildren,
                      &num_items, NULL);
                     ...
                     (obj = trav_next(&trav)) != NULL;)
                        if ((*(pos->obj_test_func))(obj) == True) {
                                ...
                                create_obj_option_item(
                                    pos, i < num_items ? items[i] : NULL, obj);
                                i++;
                        }
                trav_close(&trav);
        }
        XtVaGetValues(pos->options.menu, XmNchildren, &items, NULL);
...
void prop_radiobox_init(PropRadioSetting prs, Widget label, Widget radiobox,
                        int num_items, WidgetList items, XtPointer *item_values,
                        Widget changebar) {
        int i;
        ...
        for (i = 0; i < num_items; i++) {
                ...
                if (i == 0) {
                        ...
                        XtVaSetValues(radiobox, XmNuserData,
                                      (XtArgVal)item_values[i], NULL);
                }
        }
}

int prop_radiobox_get_value(PropRadioSetting prs) {
        XtArgVal value;
...
int prop_radiobox_set_value(PropRadioSetting prs, XtPointer value,
                            BOOL trip_changebar) {
        int num_children = 0;
        WidgetList children = NULL;
        XtArgVal childval = 0;
        ...
        XtVaGetValues(prs->radiobox, XtNnumChildren, &num_children, XtNchildren,
                      &children, NULL);

        for (i = 0; i < num_children; i++) {
                XtVaGetValues(children[i], XmNuserData, &childval, NULL);
                XmToggleButtonSetState(
                    children[i], childval == (XtArgVal)value ? TRUE : FALSE,
                    FALSE);
                if (childval == (XtArgVal)value) {
                        found = TRUE;
                        XtVaSetValues(prs->radiobox, XmNuserData,
                                      (XtArgVal)value, NULL);
                        if (trip_changebar)
...
void prop_label_field_init(PropFieldSetting label_pfs, Widget graphic_hint,
                           WidgetList labeltype_items, int num_items) {
        int i;

        if (label_pfs->label != NULL && graphic_hint != NULL)
                XtVaSetValues(label_pfs->label, XmNuserData,
                              (XtArgVal)graphic_hint, XmNrecomputeSize, False,
                              NULL);

        for (i = 0; i < num_items; i++)
                XtAddCallback(labeltype_items[i], XmNactivateCallback,
                              (XtCallbackProc)propP_labeltypeCB,
                              (XtPointer)label_pfs);
}

void prop_setup_label_field(PropFieldSetting label_pfs,
                            PropOptionsSetting label_line_style_pos,
                            AB_LABEL_TYPE ltype, STRING field_val,
        ...
        if (field_val != NULL) {
                XtVaSetValues(label_pfs->field, XmNuserData,
                              (XtArgVal)PROP_LOAD, NULL);
                ui_field_set_string(label_pfs->field, field_val);
                XtVaSetValues(label_pfs->field, XmNuserData,
                              (XtArgVal)PROP_EDIT, NULL);
        }
        if (line_style != AB_LINE_UNDEF)
                prop_options_set_value(label_line_style_pos,
                                       (XtPointer)line_style, False);
 ...
 */
BOOL prop_changed(Widget changebar) {
        XtArgVal cb_state;
 ...
 */
BOOL prop_changebars_pending(Widget prop_sheet) {
 ...
 */
void prop_changebars_cleared(Widget prop_sheet) {
 ...
 */
void prop_set_changebar(Widget changebar, int mode) {
        Widget prop_sheet;
        Pixel bar_pixel;
        ...
        if (mode == PROP_CB_ON) {
                /*
                 REMIND: aim - workaround for Motif problem with Prop textfield
                        ...
                        {
                */
                XtVaGetValues(XtParent(changebar), XmNforeground, &bar_pixel,
                              NULL);

                XtVaSetValues(changebar, XmNbackground, bar_pixel,
                              XmNforeground, bar_pixel, XmNuserData,
                              (XtArgVal)PROP_CB_ON, NULL);
        ...
        } else /* PROP_CB_OFF */
        {
                /*
                        ...
                        {
                */
                XtVaGetValues(XtParent(changebar), XmNbackground, &bar_pixel,
                              NULL);
                XtVaSetValues(changebar, XmNbackground, bar_pixel,
                              XmNforeground, bar_pixel, XmNuserData,
                              (XtArgVal)PROP_CB_OFF, NULL);
                /*
                        }
                */
        }
 ...
 */
BOOL prop_color_ok(Widget field) {
        STRING colorname;
        BOOL valid = TRUE;
 ...
 */
BOOL prop_graphic_filename_ok(Widget field, BOOL empty_ok) {
        char filebase[512];
        STRING filename;
        STRING ext;
        Pixmap pixmap;
        ...
        if (util_strempty(filename)) {
                if (empty_ok)
                        ...
                        util_set_help_data(
                            catgets(Dtb_project_catd, 100, 115,
                                    "If Graphic is specified as the Label Type "
                                    "for\nthe object whose properties are "
                                    "being modified,\nthen a pixmap (.pm, "
                                    ".xpm) or bitmap (.bm, .xbm)\nfile name "
                                    "must be specified in the Graphic "
                                    "Filename\nfield."),
                            NULL, NULL);

                        propP_popup_message(field,
                                            catgets(Dtb_project_catd, 100, 114,
                                                    "The \"Graphic Filename\" "
                                                    "field cannot be empty."),
                                            False);
                }
        } else {
                strcpy(filebase, filename);
                /* Look to see if user typed in the full filename for the
                 * Graphic. If so, strip off the extension and mark stripped to
                 * be True.
                 */
                if (util_file_name_has_extension(filename, "pm") ||
                    ...
                    util_file_name_has_extension(filename, "xbm")) {
                        ...
                        util_set_help_data(
                            catgets(Dtb_project_catd, 100, 117,
                                    "The graphic file name extension should "
                                    "not be included\nwhen it is specified in "
                                    "the Graphic Filename field."),
                            NULL, NULL);

                        propP_popup_message(
                            field,
                            catgets(Dtb_project_catd, 100, 116,
                                    "\"Graphic Filename\" field expects the "
                                    "filename\nbase only (no extensions: .pm "
                                    ".xpm .bm .xbm).\nStripping off the "
                                    "extension."),
                            True);
...
BOOL prop_help_item_ok(Widget list, ABObj item_obj) {
        int num_items;
        ABObj *iobj_list;
        BOOL exists = FALSE;
        int i;
        ...
        XtVaGetValues(list, XmNitemCount, &num_items, XmNuserData, &iobj_list,
                      NULL);
        ...
        if (exists) {
                util_set_help_data(
                    catgets(Dtb_project_catd, 100, 119,
                            "Two Menubar items cannot both be specified as "
                            "the\nHelp cascade. To specify a different Menubar "
                            "item,\nyou must first clear the item that is "
                            "currently set\nto be the Help cascade, then "
                            "select the new Menubar\nitem to be the Help "
                            "cascade."),
                    NULL, NULL);

                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 118,
                                "There can only be one Help cascade per "
                                "Menubar.Item\n\"%s\" is already configured to "
                                "be the Help cascade,\ntherefore Item \"%s\" "
                                "cannot also be the Help cascade."),
 ...
 */
BOOL prop_identifier_ok(Widget field) {
 ...
 */
BOOL prop_name_ok(ABObj obj, Widget field) {
        ...
        if (!newname || !*newname) {
                util_set_help_data(
                    catgets(
                        Dtb_project_catd, 100, 121,
                        "A string must be entered in the Object Name field."),
                    NULL, NULL);
                propP_popup_message(
                    field,
                    catgets(Dtb_project_catd, 100, 120, "A name is required."),
                    False);
                valid = FALSE;
        ...
        if (valid && ((other = obj_find_by_name(module, newname)) != NULL)) {
                if (other != obj) {
                        util_set_help_data(
                            catgets(Dtb_project_catd, 100, 123,
                                    "The name specified in the Object Name "
                                    "field is not\na unique name in the "
                                    "module. You must specify a unique\nname "
                                    "for the object."),
                            NULL, NULL);

                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 122,
                                        "Another object in Module \"%s\"\nhas "
                                        "the name \"%s\".\nPlease enter a "
                                        "unique name."),
                                util_strsafe(obj_get_name(module)), newname);
                        propP_popup_message(field, Buf, False);
                        valid = FALSE;
                }
        }
...
BOOL prop_number_ok(Widget field, STRING field_name, int min_val, int max_val) {
        BOOL valid = TRUE;
        char *p;
        STRING string;
        char *s;
        ...
        if ((s != NULL) && (*s != '\0')) {
                ...
                if ((s == p) || ((s + strlen(s)) != p)) {
                        util_set_help_data(catgets(Dtb_project_catd, 100, 125,
                                                   "The field only accepts "
                                                   "integers. You must "
                                                   "enter\nan integer value."),
                                           NULL, NULL);
                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 124,
        ...
        if (valid) /* is an integer */
        {
                ...
                if (value < min_val || value > max_val) {
                        util_set_help_data(catgets(Dtb_project_catd, 100, 127,
                                                   ...
                                                   "not valid. Specify a value "
                                                   "in the given range."),
                                           NULL, NULL);
                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 126,
                                        "\"%s\" value (%d) out of\nvalid range "
                                        "[%d - %d]."),
 ...
 */
BOOL prop_obj_name_ok(Widget field, ABObj root, AB_OBJECT_TYPE objtype,
                      STRING objtype_name) {
        ABObj target;
        STRING objname;
        BOOL valid = FALSE;
        ...
        if (util_strcmp(objname, "") == 0)
                ...
                if (target->type == objtype)
                        ...
                        util_set_help_data(
                            catgets(Dtb_project_catd, 100, 129,
                                    "The object specified is not of the "
                                    "correct type.\nFor example, it is an "
                                    "error to specify a button\nas the Popup "
                                    "Menu for a Control Pane."),
                            NULL, NULL);
                        sprintf(msgbuf,
                                catgets(Dtb_project_catd, 100, 128,
                ...
                sprintf(msgbuf,
                        catgets(Dtb_project_catd, 100, 130,
...
BOOL prop_submenu_name_ok(Widget field, ABObj owner) {
        BOOL valid = False;
        ...
        if (prop_obj_name_ok(field, module, AB_TYPE_MENU, "Menu")) {
                ...
                /* Check that the submenu name specified will not cause a
                 * circular menu (if the submenu is one that is in the owner's
                 * ancestory)
                 */
                if (obj_is_menu(owner)) {
                        ...
                        /* Build list of any menus in the ancestory of "owner"
                         * menu since these are not valid submenus
                         */
                        ineligible_menus =
                            (ABObj *)util_malloc(menu_count * sizeof(ABObj));
                        ineligible_menus[ineligible_count++] = owner;
                        find_submenu_owners(owner, menus, menu_count,
                                            ineligible_menus,
                                            &ineligible_count);

                        /* Check to see if the specified submenu is in the
                         * ineligible list */
                        menu = obj_scoped_find_by_name(module, menu_name);
                        if (menu_in_list(ineligible_menus, ineligible_count,
                                         menu)) {
                                STRING fmtStr = NULL;
                                ...
                                fmtStr = XtNewString(catgets(
                                    Dtb_project_catd, 100, 132,
                                    "Menu \"%s\" is attached as a sub-menu\nto "
                                    "\"%s\". You cannot create a "
                                    "circular\nreference within menus."));

                                help_buf = (STRING)util_malloc(
                                    strlen(fmtStr) +
                                    strlen(obj_get_name(owner)) +
                                    strlen(menu_name) + 1);
                                sprintf(help_buf, fmtStr, obj_get_name(owner),
                                        menu_name);

                                util_set_help_data(help_buf, NULL, NULL);
                                sprintf(msgbuf,
                                        catgets(Dtb_project_catd, 100, 131,
                                                "Menu \"%s\" is an ancestor of "
                                                "\"%s\"\ntherefore it cannot "
                                                "be attached as a Sub-menu."),
                                        menu_name, obj_get_name(owner));
                                propP_popup_message(field, msgbuf, False);
...
int prop_str_to_int(STRING str) {
        int val;
        char *s = str;
        char *p;
 ...
 */
BOOL prop_string_ok(Widget field, const char *chars, BOOL display_notice) {
        STRING string;
        char *s;
        ...
        while (s && *s != '\0') {
                if (!isalnum(*s) && (strchr(chars, *s) == NULL)) {
                        if (display_notice) {
                                util_set_help_data(
                                    catgets(Dtb_project_catd, 10, 69,
                                            "Object names are used by the code "
                                            "generator to\ncreate C identifier "
                                            "names. C identifiers must\nbe "
                                            "composed of letters, digits, or "
                                            "underscores.\nTherefore, object "
                                            "names in App Builder must "
                                            "also\nfollow that rule."),
                                    NULL, NULL);
                                sprintf(Buf,
                                        catgets(Dtb_project_catd, 100, 133,
 ...
 */
static void set_changebar_size(Widget shell, XtPointer client_data,
                               XEvent *event, Boolean *cont_dispatch) {
        ...
        if (event->type == MapNotify) {
                ...
                XtVaGetValues(setting, XmNheight, &setting_h, XmNy, &setting_y,
                              NULL);
                ...
                XtVaSetValues(changebar, XmNheight, setting_h - 2, XmNy,
                              setting_y + 1, XmNtopOffset, setting_y + 1, NULL);
 ...
 */
void propP_changebar_init(Widget changebar, Widget setting) {
         ...
         */
        XtAddEventHandler(shell, StructureNotifyMask, False, set_changebar_size,
                          (XtPointer)wlist);
}
...
void propP_combobox_chgCB(Widget widget, XtPointer clientdata,
                          XtPointer calldata) {
...
void propP_field_chgCB(Widget widget, XtPointer clientdata,
                       XtPointer calldata) {
...
int propP_field_set_value(Widget field, STRING value, BOOL trip_changebar) {
        if (!trip_changebar)
                /* Set state so changebar is not triggered */
...
int propP_field_set_numeric_value(Widget field, int val, BOOL trip_changebar) {
        char valstr[8];
...
void propP_labeltypeCB(Widget item, XtPointer clientdata, XtPointer call_data) {
        ...
        prop_setup_label_field(label_pfs, NULL, label_type, NULL,
                               AB_LINE_UNDEF);
        ui_field_select_string(label_pfs->field, True);
}

void propP_options_itemCB(Widget item, XtPointer client_data,
                          XtPointer call_data) {
 ...
 */
void propP_popup_message(Widget parent, STRING msg, BOOL modal) {
        ...
        if (!util_strempty(help_text)) {
                help_data.help_text =
                    help_text ? (STRING)XtNewString(help_text) : NULL;
                help_data.help_volume =
                    help_vol ? (STRING)XtNewString(help_vol) : NULL;
                help_data.help_locationID =
                    help_loc ? (STRING)XtNewString(help_loc) : NULL;
        } else {
        ...
        if (!util_strempty(help_text)) {
                dtb_show_message(parent, &dtb_revolv_wrn_msg, xm_buf,
                                 &help_data);
                ...
                dtb_show_message(parent, &dtb_revolv_wrn_msg, xm_buf, NULL);
 ...
 */
void propP_setting_chgCB(Widget widget, XtPointer clientdata,
                         XtPointer calldata) {
        Widget changebar = (Widget)clientdata;
        XtArgVal cb_state;
 ...
 */
void propP_radiobox_itemCB(Widget item, XtPointer clientdata,
                           XtPointer calldata) {
        XmToggleButtonCallbackStruct *state =
            (XmToggleButtonCallbackStruct *)calldata;
...
static void revolv_dialog_init(Widget parent) {
        DtbRevolvPropDialogInfo cgen =
            &dtb_revolv_prop_dialog; /* Codegen structure */

        if (dtb_revolv_prop_dialog_initialize(cgen, parent) == 0) {
                AB_rev_prop_dialog = cgen->prop_dialog_shellform;

                XtVaSetValues(cgen->prop_dialog_form, XmNresizePolicy,
                              XmRESIZE_ANY, XmNallowResize, True,
                              XmNautoUnmanage, False, NULL);

                revolv_objectspane_init(cgen);
                revolv_basicframe_init(cgen);
                prop_activate_panel_init(
                    AB_PROP_REVOLVING, NULL, cgen->ok_button,
                    cgen->apply_button, cgen->reset_button, cgen->cancel_button,
                    cgen->help_button);

                init_prop_state_info(AB_rev_prop_dialog, NULL, cgen->objlist,
                                     NULL);

                ab_register_window(AB_rev_prop_dialog, AB_WIN_DIALOG,
                                   WindowHidden, AB_toplevel,
                                   AB_WPOS_TILE_HORIZONTAL, close_propsCB,
 ...
 */
static String get_prop_frame_name(PalItemInfo *palitem) {
        switch (palitem->type) {
...
static void revolv_basicframe_init(DtbRevolvPropDialogInfo cgen) {
        ...
        rev_form = XtVaCreateManagedWidget(
            "revolving_dialog_form", xmFormWidgetClass, cgen->prop_dialog_form,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
            cgen->objlist_panel, XmNrightAttachment, XmATTACH_FORM,
            XmNleftAttachment, XmATTACH_FORM, XmNbottomAttachment,
            XmATTACH_FORM, XmNresizePolicy, XmRESIZE_ANY, XmNheight, 600, NULL);

        /*
         * Create frame for each object on the palette...
         */
        for (i = 0; i < palette_item_cnt; i++) {
                rev_frame = cgen->attrs_ctrlpanel_frame = XtVaCreateWidget(
                    get_prop_frame_name(palette_item[i]), xmFrameWidgetClass,
                    rev_form, XmNshadowType, XmSHADOW_IN, XmNtopAttachment,
                    XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
                    XmNleftAttachment, XmATTACH_FORM, XmNbottomAttachment,
                    XmATTACH_FORM, NULL);

                (*(palette_item[i]->prop_initialize))(rev_frame,
                                                      AB_PROP_REVOLVING);
...
static BOOL editable_obj_test(PalEditableObjInfo *ed_obj_info) {
...
static void revolv_objectspane_init(DtbRevolvPropDialogInfo cgen) {
        ...
        XtVaSetValues(cgen->objlist_label2, XmNalignment, XmALIGNMENT_END,
                      XmNrightAttachment, XmATTACH_WIDGET, XmNrightWidget,
                      cgen->objlist_scrolledwin, XmNrightOffset, 4,
 ...
 */
static PalItemInfo *revolv_set_prop_type(PalItemInfo *new_palitem,
                                         ABObj load_obj) {
 ...
 */
static PalItemInfo *revolv_invoke_props(PalItemInfo *palitem, ABObj load_obj) {
        PalItemInfo *viz_palitem;
...
static void fixed_invoke_props(PalItemInfo *palitem) {
        ...
        if (palitem->fix_prop_dialog == NULL) {
                prop_dialog =
                    (*(palitem->prop_initialize))(AB_toplevel, AB_PROP_FIXED);
        ...
        if (prop_dialog != NULL) {
                ...
                util_dprintf(1,
                             "fixed_invoke_props: could not initialize "
                             "Properties for %s\n",
                             palitem->name);
}

static int apply_props(AB_PROP_TYPE type, Widget dialog) {
        PropStateInfo *pstate;
 ...
 */
static int apply_prop_changes(AB_PROP_TYPE type, ABObj obj,
                              PalItemInfo *palitem) {
 ...
 */
static DTB_MODAL_ANSWER handle_auto_apply(AB_PROP_TYPE prop_type,
                                          PropStateInfo *pstate,
                                          PalItemInfo *new_palitem,
                                          ABObj new_obj, PalItemInfo **result) {
        ...
        help_data =
            (DtbObjectHelpData)util_malloc(sizeof(DtbObjectHelpDataRec));
                ...
                if (new_obj == NULL) {
                        if (changing_types) {
                                sprintf(
                                    Buf,
                                    catgets(
                                        Dtb_project_catd, 100, 34,
                                        "Properties for \"%s\" have been modified but not Applied.\
		\nApply Changes or Cancel Change-ObjectType operation."),
                                    loadedObjName);

                                help_data->help_text = catgets(
                                    Dtb_project_catd, 100, 95,
                                    "Click Apply Changes to apply the changes "
                                    "to the\ncurrent object and display the "
                                    "new object type.\n\nClick Cancel if you "
                                    "don't want to apply the\nchanges to the "
                                    "current object. You can then\nclick Reset "
                                    "to undo the changes before changing\nto a "
                                    "different object type.");
                        } else {
                                sprintf(
                                    Buf,
                                    catgets(
                                        Dtb_project_catd, 100, 35,
                                        "Properties for \"%s\" have been modified but not Applied.\
		\nApply Changes or Cancel Close operation."),
                                    loadedObjName);

                                help_data->help_text = catgets(
                                    Dtb_project_catd, 100, 96,
                                    "Click Apply Changes to apply the changes "
                                    "to the\ncurrent object and close the "
                                    "Property Editor.\n\nClick Cancel if you "
                                    "don't want to apply the\nchanges to the "
                                    "current object and want the\nProperty "
                                    "Editor to remain displayed. You can\nthen "
                        ...
                        sprintf(
                            Buf,
                            catgets(
                                Dtb_project_catd, 100, 36,
                                "Properties for \"%s\" have been modified but not Applied.\
		\nApply Changes or Cancel Load operation for \"%s\"."),
                            loadedObjName, newObjName);

                        help_data->help_text = catgets(
                            Dtb_project_catd, 100, 93,
                            "Click Apply Changes to apply the changes to "
                            "the\ncurrent object and load the selected "
                            "object.\n\nClick Cancel if you don't want to "
                            "apply the\nchanges to the current object. You can "
                            "then\nclick Reset to undo the changes before "
                            "loading\nthe selected object.");
                }
        ...
        case DTB_ANSWER_ACTION1: /* Apply Changes */
                                  ...
                                  */
                if (apply_prop_changes(prop_type, pstate->loaded_obj,
                                       pstate->palitem) != OK) {
        ...
        case DTB_ANSWER_CANCEL: /* Cancel Load */
                if (changing_types)
                        ui_optionmenu_change_label(revolv_optionmenu,
                                                   pstate->palitem->name);
 ...
 */
static void load_props(AB_PROP_TYPE type, PropStateInfo *pstate, ABObj obj) {
...
static int objlist_delete_obj(AB_PROP_TYPE type, PalItemInfo *palitem,
        ...
        if (num_items == 0) /* Deleted Last one...*/
        {
                ...
                pstate->loaded_obj = NULL;
                   ...
                   pstate->loaded_obj) /* Deleted "Current" one...*/
        {
                /* If there are pending edits on the deleted object, first clear
                 * those edits via a reset before making another object
                 * "current" in props.
                 */
...
static void objlist_ensure_selection(Widget objlist) {
        ...
        XtVaGetValues(objlist, XmNitemCount, &num_items, XmNselectedItemCount,
                      &num_selected, NULL);
...
static int objlist_show_or_hide_obj(AB_PROP_TYPE type, PalItemInfo *palitem,
        ...
        ui_list_get_info(pstate->objlist, &orig_num_items, &first_vis,
                         &last_vis);
        ...
        if (num_items == 0) /* Hid Last one...*/
        {
                ...
                pstate->loaded_obj = NULL;
                     ...
                     obj_has_flag(obj_get_module(pstate->loaded_obj),
                                  MappedFlag)) ||
                   num_items == 1)
 ...
 */
static BOOL objlist_test_func(ABObj test_obj) {
        if ((obj_get_module(test_obj) != NULL) && obj_is_defined(test_obj) &&
            obj_is_salient_ui(test_obj) && obj_has_flag(test_obj, MappedFlag)) {
...
static void create_obj_option_item(PropObjOptionsSetting pos, Widget item,
        ...
        if (item == NULL) {
                /* Create new item */
                newitem = XtVaCreateWidget(
                    modname ? modname : name, xmPushButtonWidgetClass,
                    pos->options.menu, XmNlabelString, xmitem, XmNuserData,
                    (XtArgVal)obj, NULL);

                XtAddCallback(newitem, XmNactivateCallback, propP_setting_chgCB,
                              (XtPointer)pos->options.changebar);
                XtAddCallback(newitem, XmNactivateCallback,
                              propP_options_itemCB,
                              (XtPointer)pos->options.menu);
        } else /* Use existing item */
                XtVaSetValues(item, XmNlabelString, xmitem, XmNuserData,
                              (XtArgVal)obj, NULL);
...
static int obj_renamedOCB(ObjEvAttChangeInfo info) {
        ...
        if (!obj_is_salient(info->obj) || obj_get_parent(info->obj) == NULL ||
            obj_is_item(info->obj))
                return 0;

        util_dprintf(3, "obj_renamedOCB: from %s to %s\n",
                     istr_string_safe(info->old_name), obj_get_name(info->obj));
        ...
        if (obj_is_module(info->obj)) /* Module name changed */
        {
                ...
                for (i = 0; i < palette_item_cnt; i++) {
                        ...
                        if (palitem->rev_prop_frame == revolv_current_frame)
                                objlist_rename_obj(AB_PROP_REVOLVING, palitem,
                                                   info);

                        if (palitem->fix_prop_dialog != NULL)
                                objlist_rename_obj(AB_PROP_FIXED, palitem,
                                                   info);
                }
        } else if ((palitem = pal_get_item_info(info->obj)) !=
                   NULL) /* Object name changed */
        {
                /* If obj's type is currently loaded in Revolving props, update
                 * the list */
                if (AB_rev_prop_dialog != NULL &&
                    revolv_current_frame == palitem->rev_prop_frame)
...
static int obj_geom_changedOCB(ObjEvAttChangeInfo info) {
         ...
         */
        if (!obj_is_salient(info->obj) || obj_get_parent(info->obj) == NULL ||
            obj_is_item(info->obj) || apply_in_progress)
                return 0;
         ...
         */
        if (info->atts & OBJEV_ATT_POSITION && obj_is_layers(info->obj)) {
        ...
        if ((palitem = pal_get_item_info(info->obj)) != NULL) {
                util_dprintf(3,
                             "obj_geom_changedOCB: %s's geometry changed to "
                             "%d,%d %dx%d\n",
                             obj_get_name(info->obj), obj_get_x(info->obj),
                             obj_get_y(info->obj), obj_get_width(info->obj),
                             obj_get_height(info->obj));
                ...
                /* If modified obj is currently loaded in the Revolving prop
                 * editor, update only its geometry attributes in the prop sheet
                 */
                if (AB_rev_prop_dialog != NULL &&
                    revolv_current_frame == palitem->rev_prop_frame) {
                        pstate = get_prop_state_info(AB_rev_prop_dialog);
                        if (pstate->loaded_obj == info->obj)
                                (*(palitem->prop_load))(NULL, AB_PROP_REVOLVING,
                                                        geomflag);
                }

                /* If modified obj is currently loaded in its Fixed prop editor,
                 * update only its geometry attributes in the prop sheet
                 */
                if (palitem->fix_prop_dialog != NULL) {
                        pstate = get_prop_state_info(palitem->fix_prop_dialog);
                        if (pstate->loaded_obj == info->obj)
                                (*(palitem->prop_load))(NULL, AB_PROP_FIXED,
                                                        geomflag);
                }
        }
        return 0;
...
static int obj_destroyedOCB(ObjEvDestroyInfo info) {
        ...
        if ((palitem = pal_get_item_info(info->obj)) !=
            NULL) /* Object being destroyed */
        {
                /* If obj's type is currently loaded in Revolving props, update
                 * the list */
                if (AB_rev_prop_dialog != NULL &&
...
static int obj_shown_or_hiddenOCB(ObjEvUpdateInfo info) {
        ...
        if (obj_is_module(info->obj)) {
                ...
                for (i = 0; i < palette_item_cnt; i++) {
                        ...
                        if (palitem->rev_prop_frame == revolv_current_frame)
                                objlist_show_or_hide_obj(AB_PROP_REVOLVING,
                                                         palitem, info);

                        if (palitem->fix_prop_dialog != NULL)
                                objlist_show_or_hide_obj(AB_PROP_FIXED, palitem,
                                                         info);
                }
        } else if ((palitem = pal_get_item_info(info->obj)) !=
                   NULL) /* Object being Updated */
        {
                cur_list_palitem = palitem;
                ...
                if (AB_rev_prop_dialog != NULL &&
                    revolv_current_frame == palitem->rev_prop_frame)
                        objlist_show_or_hide_obj(AB_PROP_REVOLVING, palitem,
                                                 info);
 ...
 */
static void apply_propsCB(Widget widget, XtPointer client_data,
                          XtPointer calldata) {
 ...
 * Callback:
 */
static void cancel_propsCB(Widget widget, XtPointer client_data,
                           XtPointer calldata) {
        AB_PROP_TYPE type = (AB_PROP_TYPE)client_data;
        PropStateInfo *pstate;
        Widget dialog;
 ...
 */
static void change_objecttypeCB(Widget widget, XtPointer client_data,
                                XtPointer calldata) {
 ...
 */
static void close_propsCB(Widget widget, XtPointer client_data,
                          XtPointer calldata) {
        AB_PROP_TYPE type = (AB_PROP_TYPE)client_data;
        ...
        XtVaGetValues(widget, XmNchildren, &shell_child, NULL);
        ...
        if ((*(pstate->palitem->prop_pending))(type) == TRUE)
                ...
                answer = handle_auto_apply(type, pstate, pstate->palitem, NULL,
                                           &palitem);
 ...
 */
static void invoke_attach_editorCB(Widget widget, XtPointer client_data,
                                   XtPointer calldata) {
        AB_PROP_TYPE type = (AB_PROP_TYPE)client_data;
        PropStateInfo *pstate;
 ...
 */
static void invoke_connectionsCB(Widget widget, XtPointer client_data,
                                 XtPointer calldata) {
        AB_PROP_TYPE type = (AB_PROP_TYPE)client_data;
        PropStateInfo *pstate;
 ...
 */
static void invoke_help_editorCB(Widget widget, XtPointer client_data,
                                 XtPointer calldata) {
        AB_PROP_TYPE type = (AB_PROP_TYPE)client_data;
        PropStateInfo *pstate;
...
static void objlist_selectCB(Widget widget, XtPointer client_data,
        ...
        if (name) {
                ...
                if (selected_obj && selected_obj != pstate->loaded_obj) {
                        if ((*(pstate->palitem->prop_pending))(type) == TRUE)
                                /* An Object is loaded with pending changes...*/
                                handle_auto_apply(type, pstate, pstate->palitem,
                                                  selected_obj, &palitem);
                        else
 ...
 */
static void ok_propsCB(Widget widget, XtPointer client_data,
                       XtPointer calldata) {
        AB_PROP_TYPE type = (AB_PROP_TYPE)client_data;
        PropStateInfo *pstate;
        Widget dialog;
 ...
 */
static void reset_propsCB(Widget widget, XtPointer client_data,
                          XtPointer calldata) {
        AB_PROP_TYPE type = (AB_PROP_TYPE)client_data;
        PropStateInfo *pstate;
 ...
 * Callback:
 */
static void tearoff_propsCB(Widget widget, XtPointer client_data,
                            XtPointer calldata) {
        PropStateInfo *pstate;
...
static void color_chooserCB(Widget w, XtPointer clientdata,
                            XtPointer calldata) {
        ...
        if (color_choice[0] != 0) {
                ...
                if ((status = objxm_name_to_pixel(w, color_choice, &c)) == OK) {
                        XtVaSetValues(pcs->swatch, XmNbackground, c, NULL);
                        XmTextSetString(pcs->field, color_choice);
                }
        }
}

static void color_noneCB(Widget w, XtPointer clientdata, XtPointer calldata) {
        PropColorSetting pcs = (PropColorSetting)clientdata;
...
static void menu_newCB(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (pms->prop_type == AB_PROP_FIXED) {
                ...
                /* Try using the Revolving Prop Editor if either we are
                 * currently in the Menu Fixed Props, or if the Menu Fixed Props
                 * has edits pending...
                 */
                if (prop_dialog == ab_menu_palitem->fix_prop_dialog ||
                    (ab_menu_palitem->fix_prop_dialog &&
                     (*ab_menu_palitem->prop_pending)(AB_PROP_FIXED) == True)) {
                        ...
                        if (pstate->palitem &&
                            (*pstate->palitem->prop_pending)(
                                AB_PROP_REVOLVING) == False) {
...
static void menulist_buildCB(Widget widget, XtPointer client_data,
        ...
        abobj_build_menus_array(obj_get_module(current_obj), &menus,
                                &num_menus);

        if (num_menus > 0) {
                if (obj_is_menu(
                        current_obj)) /* Need to prevent circular menus */
                {
                        /* Build up a list of menus that belong to the current
                         * menu's ancestory (if it's a submenu) - these menus
                         * will not be eligible for selection as a submenu off
                         * the current menu.
                         */
                        ineligible_menus =
                            (ABObj *)util_malloc(num_menus * sizeof(ABObj));
                ...
                for (j = 0; j < num_menus; j++) {
                        if (menus_menu == NULL)
                                menus_menu = XmCreatePulldownMenu(
                                    submenu, "menus_menu", NULL, 0);

                        menu_name = obj_get_name(menus[j]);

                        menus_menuitem = XtVaCreateManagedWidget(
                            "menus_menuitem", xmCascadeButtonWidgetClass,
                            menus_menu, XtVaTypedArg, XmNlabelString, XtRString,
                            menu_name, strlen(menu_name) + 1, NULL);

                        /* If Menu is ineligible to be a Submenu, make it
                         * inactive */
                        if (menu_in_list(ineligible_menus, ineligible_count,
                                         menus[j]))
 ...
 */
static void menulist_destroyCB(Widget w, XtPointer client_data,
                               XtPointer calldata) {
 ...
 */
static void menuname_clearCB(Widget w, XtPointer client_data,
                             XtPointer calldata) {
        PropMenunameSetting pms = (PropMenunameSetting)client_data;
 ...
 */
static void menuname_setCB(Widget w, XtPointer client_data,
                           XtPointer calldata) {
...
static void menu_editCB(Widget w, XtPointer client_data, XtPointer call_data) {
        PropMenunameSetting pms = (PropMenunameSetting)client_data;
        STRING current_menuname = NULL;
        ABObj module;
        ABObj menu;
        ...
        if (!util_strempty(current_menuname)) {
                if ((menu = obj_find_by_name(module, current_menuname)) != NULL)
                        ...
                        fmtStr = XtNewString(
                            catgets(Dtb_project_catd, 100, 135,
                                    "The menu with the name \"%s\" does not "
                                    "exist in\nthe module \"%s\". Specify a "
                                    "valid menu name."));

                        help_buf = (STRING)util_malloc(
                            strlen(fmtStr) + strlen(current_menuname) +
                            strlen(obj_get_name(module)) + 1);
                        sprintf(help_buf, fmtStr, current_menuname,
                                obj_get_name(module));
...
static void menu_field_chgCB(Widget w, XtPointer client_data,
                             XtPointer call_data) {
        PropMenunameSetting pms = (PropMenunameSetting)client_data;
        STRING value;
...
int prop_geomfield_clear(PropGeometrySetting pgs, GEOM_KEY gkey) {
        Widget field = NULL;
...
void strings_init() {
        ...
        LabelForGraphic = XtNewString(
            catgets(Dtb_project_catd, 100, 216, "Graphic Filename:"));
        NoneItem = XtNewString(catgets(Dtb_project_catd, 100, 217, "None"));
        XFieldStr = XtNewString(catgets(Dtb_project_catd, 100, 218, "X Field"));
        YFieldStr = XtNewString(catgets(Dtb_project_catd, 100, 219, "Y Field"));
        WFieldStr =
        ...
        PercentageFieldStr = XtNewString(
            catgets(Dtb_project_catd, 100, 223, "Percentage Field"));
        menu_strs[0] = XtNewString(catgets(Dtb_project_catd, 100, 224, "None"));
        menu_strs[1] = XtNewString(
            catgets(Dtb_project_catd, 100, 225, "Create New Menu..."));
        menu_strs[2] =
            XtNewString(catgets(Dtb_project_catd, 100, 226, "Menus"));
        menu_strs[3] =
            XtNewString(catgets(Dtb_project_catd, 100, 227, "Edit Current..."));
        RowColFieldStr = XtNewString(
            catgets(Dtb_project_catd, 100, 250, "Rows/Columns Field"));
        VertSpacingFieldStr = XtNewString(
            catgets(Dtb_project_catd, 100, 251, "Vertical Spacing Field"));
        HorizSpacingFieldStr = XtNewString(
            catgets(Dtb_project_catd, 100, 252, "Horizontal Spacing Field"));
}
