 ...
 * Methods
 */
static int termp_initialize(ABObj obj);
static Widget termp_prop_init(Widget parent, AB_PROP_TYPE type);
static int termp_prop_activate(AB_PROP_TYPE type, BOOL active);
static int termp_prop_clear(AB_PROP_TYPE type);
static int termp_prop_load(ABObj obj, AB_PROP_TYPE type, unsigned long loadkey);
static int termp_prop_apply(AB_PROP_TYPE type);
static BOOL termp_prop_pending(AB_PROP_TYPE type);
...
static Widget termp_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbTermpPropDialogInfo cgen =
            &dtb_termp_prop_dialog; /* Codegen structure */
        ...
        if (dtb_termp_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_termp_palitem, cgen->ok_button,
...
static int termp_prop_load(ABObj obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                /* Load Process String */
                prop_field_set_value(&(pts->pstring),
                                     obj_get_process_string(obj), False);
...
int termp_prop_apply(AB_PROP_TYPE type) {
        PropTermpSettingsRec *pts = &(prop_termp_settings_rec[type]);
        ...
        if (prop_changed(pts->size.changebar)) {
                ...
                border = ((prop_options_get_value(&(pts->frame)) !=
        ...
        if (prop_changed(pts->pstring.changebar)) {
                ABObj o_obj =
                    objxm_comp_get_subobj(pts->current_obj, AB_CFG_OBJECT_OBJ);

                value = prop_field_get_value(&(pts->pstring));
                obj_set_process_string(pts->current_obj, value);
                objxm_obj_set_ui_arg(o_obj, AB_ARG_STRING, DtNsubprocessCmd,
                                     value);
...
static BOOL termp_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_termp_settings_rec[type].prop_sheet));
}
