...
**                                                                      **
**************************************************************************/
static void find_callback(Widget widget, XtPointer client_data,
...
void brwsP_create_find_box(ABBrowser b) {
        ...
        if (!ui->find_box) {
                ...
                XtSetArg(args[num_args], XmNautoUnmanage, FALSE);
                num_args++;
                ui->find_box =
                    XmCreatePromptDialog(ui->shell, "find_box", args, num_args);

                title = XtNewString(catgets(Dtb_project_catd, 100, 230,
                                            "Module Browser: Find Object"));
                XtVaSetValues(XtParent(ui->find_box), XmNtitle, title, NULL);
                XtFree(title);
                ...
                XtVaSetValues(ui->find_box, XmNselectionLabelString, sel_label,
                              XmNokLabelString, ok_label, XmNokCallback,
                              &find_callback_list, XmNcancelCallback,
                              &cancel_callback_list, XmNuserData, b, NULL);
