...
static Widget attch_editor_props_init(DtbAttchEdAttchEdDialogInfo attch_ed_cgen,
                                      Widget parent);

static int attch_editor_load(ABObj object);

static int attch_editor_apply(ABObj object);

static BOOL attch_edP_pending();

static int attch_editor_activate(BOOL active);

static BOOL editable_obj_test(PalEditableObjInfo *ed_obj_info);

static void load_attch_obj(ABObj obj, BOOL turnoff_cb);

static void create_children_list(ABObj obj);

static void update_attch_obj_parent(ABObj obj);

static void update_obj_type(ABObj obj);

static void update_objlist(ABObj obj);

static void create_obj_menu_dir(ABObj object, AB_COMPASS_POINT dir);

static void create_obj_menu(ABObj object);

static void delete_from_obj_menu(ABObj cur_obj, ABObj deleted_obj);

static void select_objlist(ABObj obj);

static void turnoff_changebars();

static void get_attach_info(ABObj obj, ABAttachment *attachment,
                            ATTCH_ED_ATTACH_TYPE *type, int *offset,
                            int *position, ABObj *attach_obj);

static void set_attach_info(ABObj obj, ABAttachment *attachment,
                            ATTCH_ED_ATTACH_TYPE type, int offset, int position,
                            ABObj attach_obj);

static void attch_ed_select_itemCB(Widget widget, XtPointer client_data,
                                   XtPointer call_data);

static void populate_obj_menu(Widget widget, ABObj object, AB_COMPASS_POINT dir,
                              Widget **item, XtPointer **item_val, int *n);

static void attch_change_objecttypePalCB(Widget widget, XtPointer client_data,
                                         XtPointer calldata);

static BOOL attch_objlist_test_func(ABObj test_obj);

static BOOL attch_childrenlist_test_func(ABObj test_obj);

static void attch_objlist_selectCB(Widget widget, XtPointer client_data,
                                   XmListCallbackStruct *listdata);

static int attch_obj_geom_changedCB(ObjEvAttChangeInfo evInfo);

static int attch_obj_renameCB(ObjEvAttChangeInfo evInfo);

static int attch_obj_destroyCB(ObjEvDestroyInfo evInfo);

static int attch_obj_reparentCB(ObjEvReparentInfo info);

static void handle_prop_sheet_obj_destroy(ObjEvDestroyInfo evInfo);

static void handle_actual_obj_destroy(ObjEvDestroyInfo evInfo);

static void handle_prop_sheet_obj_reparent(ObjEvReparentInfo evInfo);

static void handle_actual_obj_reparent(ObjEvReparentInfo evInfo);

static void set_widget_state(ATTCH_ED_ATTACH_TYPE attach_type, Widget objlist,
                             Widget offset, Widget position);

static void enable_attach_types(ABObj obj);

static void set_attach_values(ATTCH_ED_ATTACH_TYPE attach_type,
                              PropFieldSetting offset_setting,
                              PropFieldSetting position_setting,
                              PropOptionsSetting objlist_setting,
                              int offset_value, int position_value,
                              XtPointer objlist_value);

static void change_attach_type(ABObj obj, AB_COMPASS_POINT dir,
                               ATTCH_ED_ATTACH_TYPE attach_type);

static void change_opp_attach_type(ABObj obj, AB_COMPASS_POINT dir,
                                   ATTCH_ED_ATTACH_TYPE attach_type);

static void get_attach_values(ATTCH_ED_ATTACH_TYPE attach_type,
                              PropFieldSetting offset_setting,
                              PropFieldSetting position_setting,
                              PropOptionsSetting objlist_setting,
                              int *offset_value, int *position_value,
...
static void convert_offset_position(ABObj cur_obj, ABObj sibling_obj,
                                    AB_COMPASS_POINT dir,
                                    ATTCH_ED_ATTACH_TYPE attach_type,
                                    int *ret_offset, int *ret_position);

static void attach_obj_changed(Widget widget, XtPointer client_data,
                               XtPointer calldata);

static void print_attach_type(ATTCH_ED_ATTACH_TYPE attach_type);

static BOOL attch_ed_obj_is_target_type(ABObj obj);

static void attch_ed_change_objecttype(AB_OBJECT_TYPE type, int subtype);

static ABObj attch_ed_get_parent(ABObj obj);
...
static void attch_edP_prevent_closeCB(Widget w, XtPointer client_data,
                                      XtPointer call_data);

static void change_attype_from_sib_to_point(ABObj obj, ABObj attach_to_sib,
                                            AB_COMPASS_POINT dir,
                                            BOOL reparent);

static DTB_MODAL_ANSWER get_wrn_response(Widget w, ATTCH_ED_WRN_TYPE wrn_type,
                                         ABObj new_obj);

static BOOL attch_ed_verify_props();

static BOOL verify_one_attach(ATTCH_ED_ATTACH_TYPE attach_type,
                              PropFieldSetting offset_setting,
                              PropFieldSetting position_setting);
...
void attch_ed_show_dialog(ABObj cur_obj) {
        ...
        if (AB_attch_ed_dialog == (Widget)NULL) {
                AB_attch_ed_dialog = attch_editor_props_init(
                    &dtb_attch_ed_attch_ed_dialog, AB_toplevel);
        }
        ...
        if (AB_attch_ed_dialog != NULL) {
                ...
                if (answer == DTB_ANSWER_ACTION1) {
                        if (cur_obj)
                                ...
                                /* First time - start with First object type in
                                 * menu */
                                attch_ed_change_objecttype(AB_TYPE_BUTTON,
                                                           AB_NO_SUBTYPE);
...
static Widget attch_editor_props_init(DtbAttchEdAttchEdDialogInfo attch_ed_cgen,
        ...
        if (dtb_attch_ed_attch_ed_dialog_initialize(attch_ed_cgen, parent) ==
            0) {
                /*
                 ...
                 */
                XtDestroyWidget(
                    attch_ed_cgen->obj_type_opmenu_items.Button_item);

                /*
                 * Add choice items for every palette object type
                 */
                pal_add_editable_obj_menu_items(
                    attch_ed_cgen->obj_type_opmenu_menu,
                ...
                XtAddCallback(
                    attch_ed_cgen->objlist, XmNbrowseSelectionCallback,
                ...
                */
                 ...
                 */
                n = 0;
                item[n] = attch_ed_cgen->top_attach_items
                              .bitmaps_attach_top_form_xbm_item;
                item_val[n] = ATTCH_ED_PARENT;
                n++;
                item[n] = attch_ed_cgen->top_attach_items
                              .bitmaps_attach_top_opposite_form_xbm_item;
                item_val[n] = ATTCH_ED_OPPOSITE_PARENT;
                n++;
                item[n] = attch_ed_cgen->top_attach_items
                              .bitmaps_attach_top_widget_xbm_item;
                item_val[n] = ATTCH_ED_SIBLING;
                n++;
                item[n] = attch_ed_cgen->top_attach_items
                              .bitmaps_attach_top_opposite_widget_xbm_item;
                item_val[n] = ATTCH_ED_OPPOSITE_SIBLING;
                n++;
                item[n] = attch_ed_cgen->top_attach_items
                              .bitmaps_attach_top_position_xbm_item;
                item_val[n] = ATTCH_ED_GRIDLINE;
                n++;

                item[n] = attch_ed_cgen->top_attach_items
                              .bitmaps_attach_top_center_position_xbm_item;
                item_val[n] = ATTCH_ED_CENTER_GRIDLINE;
                n++;

                item[n] = attch_ed_cgen->top_attach_items
                              .bitmaps_attach_none_xbm_item;
                item_val[n] = ATTCH_ED_NONE;
                n++;
                prop_options_init(
                    &(ats->top_attach_type), NULL, attch_ed_cgen->top_attach,
                    attch_ed_cgen->top_attach_menu, n, item,
                    (XtPointer *)item_val, attch_ed_cgen->top_attach_cb);
                /*
                 * Top offset text field
                 */
                prop_field_init(
                    &(ats->top_attach_offset), attch_ed_cgen->top_offset_label,
                    attch_ed_cgen->top_offset, attch_ed_cgen->top_attach_cb);

                /*
                 * Top position text field
                 */
                prop_field_init(&(ats->top_attach_position),
                                attch_ed_cgen->top_position_label,
                 ...
                 **********************/
                n = 0;
                item[n] = attch_ed_cgen->bottom_attach_items
                              .bitmaps_attach_bottom_form_xbm_item;
                item_val[n] = ATTCH_ED_PARENT;
                n++;
                item[n] = attch_ed_cgen->bottom_attach_items
                              .bitmaps_attach_bottom_opposite_form_xbm_item;
                item_val[n] = ATTCH_ED_OPPOSITE_PARENT;
                n++;
                item[n] = attch_ed_cgen->bottom_attach_items
                              .bitmaps_attach_bottom_widget_xbm_item;
                item_val[n] = ATTCH_ED_SIBLING;
                n++;
                item[n] = attch_ed_cgen->bottom_attach_items
                              .bitmaps_attach_bottom_opposite_widget_xbm_item;
                item_val[n] = ATTCH_ED_OPPOSITE_SIBLING;
                n++;
                item[n] = attch_ed_cgen->bottom_attach_items
                              .bitmaps_attach_bottom_position_xbm_item;
                item_val[n] = ATTCH_ED_GRIDLINE;
                n++;

                item[n] = attch_ed_cgen->bottom_attach_items
                              .bitmaps_attach_bottom_center_position_xbm_item;
                item_val[n] = ATTCH_ED_CENTER_GRIDLINE;
                n++;
                ...
                prop_options_init(&(ats->bottom_attach_type), NULL,
                                  attch_ed_cgen->bottom_attach,
                                  attch_ed_cgen->bottom_attach_menu, n, item,
                                  (XtPointer *)item_val,
                 ...
                 */
                prop_field_init(&(ats->bottom_attach_offset),
                                attch_ed_cgen->bottom_offset_label,
                 ...
                 */
                prop_field_init(&(ats->bottom_attach_position),
                                attch_ed_cgen->bottom_position_label,
                 ...
                 **********************/
                n = 0;
                item[n] = attch_ed_cgen->left_attach_items
                              .bitmaps_attach_left_form_xbm_item;
                item_val[n] = ATTCH_ED_PARENT;
                n++;
                item[n] = attch_ed_cgen->left_attach_items
                              .bitmaps_attach_left_opposite_form_xbm_item;
                item_val[n] = ATTCH_ED_OPPOSITE_PARENT;
                n++;
                item[n] = attch_ed_cgen->left_attach_items
                              .bitmaps_attach_left_widget_xbm_item;
                item_val[n] = ATTCH_ED_SIBLING;
                n++;
                item[n] = attch_ed_cgen->left_attach_items
                              .bitmaps_attach_left_opposite_widget_xbm_item;
                item_val[n] = ATTCH_ED_OPPOSITE_SIBLING;
                n++;
                item[n] = attch_ed_cgen->left_attach_items
                              .bitmaps_attach_left_position_xbm_item;
                item_val[n] = ATTCH_ED_GRIDLINE;
                n++;
                item[n] = attch_ed_cgen->left_attach_items
                              .bitmaps_attach_left_center_position_xbm_item;
                item_val[n] = ATTCH_ED_CENTER_GRIDLINE;
                n++;
                item[n] = attch_ed_cgen->left_attach_items
                              .bitmaps_attach_none_xbm_item;
                ...
                n++;
                prop_options_init(
                    &(ats->left_attach_type), NULL, attch_ed_cgen->left_attach,
                    attch_ed_cgen->left_attach_menu, n, item,
                    (XtPointer *)item_val, attch_ed_cgen->left_attach_cb);

                /*
                 * Left offset text field
                 */
                prop_field_init(&(ats->left_attach_offset),
                                attch_ed_cgen->left_offset_label,
                 ...
                 */
                prop_field_init(&(ats->left_attach_position),
                                attch_ed_cgen->left_position_label,
                 ...
                 **********************/
                n = 0;
                item[n] = attch_ed_cgen->right_attach_items
                              .bitmaps_attach_right_form_xbm_item;
                item_val[n] = ATTCH_ED_PARENT;
                n++;
                item[n] = attch_ed_cgen->right_attach_items
                              .bitmaps_attach_right_opposite_form_xbm_item;
                item_val[n] = ATTCH_ED_OPPOSITE_PARENT;
                n++;
                item[n] = attch_ed_cgen->right_attach_items
                              .bitmaps_attach_right_widget_xbm_item;
                item_val[n] = ATTCH_ED_SIBLING;
                n++;
                item[n] = attch_ed_cgen->right_attach_items
                              .bitmaps_attach_right_opposite_widget_xbm_item;
                item_val[n] = ATTCH_ED_OPPOSITE_SIBLING;
                n++;
                item[n] = attch_ed_cgen->right_attach_items
                              .bitmaps_attach_right_position_xbm_item;
                item_val[n] = ATTCH_ED_GRIDLINE;
                n++;
                item[n] = attch_ed_cgen->right_attach_items
                              .bitmaps_attach_right_center_position_xbm_item;
                item_val[n] = ATTCH_ED_CENTER_GRIDLINE;
                n++;
                item[n] = attch_ed_cgen->right_attach_items
                              .bitmaps_attach_none_xbm_item;
                ...
                prop_options_init(&(ats->right_attach_type), NULL,
                                  attch_ed_cgen->right_attach,
                                  attch_ed_cgen->right_attach_menu, n, item,
                                  (XtPointer *)item_val,
                                  attch_ed_cgen->left_attach_cb);
                 ...
                 */
                prop_field_init(&(ats->right_attach_offset),
                                attch_ed_cgen->right_offset_label,
                 ...
                 */
                prop_field_init(&(ats->right_attach_position),
                                attch_ed_cgen->right_position_label,
                ...
                ab_register_window(attch_ed_cgen->attch_ed_dialog_shellform,
                                   AB_WIN_DIALOG, WindowHidden, AB_toplevel,
                                   AB_WPOS_TILE_HORIZONTAL,
                                   attch_edP_prevent_closeCB, NULL);

                ret_value =
                    dtb_attch_ed_attch_ed_dialog.attch_ed_dialog_shellform;

                /*
                 * Add callback to detect change in geometry
                 */
                obj_add_geometry_change_callback(attch_obj_geom_changedCB,
                                                 "ATTCH_ED");

                obj_add_rename_callback(attch_obj_renameCB, "ATTCH_ED");

        } else {
                util_dprintf(3, "attch_editor_props_init: could not initialize "
                                "Attachment Editor\n");
                ret_value = NULL;
        }
...
static int attch_editor_load(ABObjPtr object) {
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
...
static void load_attch_obj(ABObj object, BOOL turnoff_cb) {
        DtbAttchEdAttchEdDialogInfo attch_ed_cgen =
            &dtb_attch_ed_attch_ed_dialog;
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
        ABAttachment *top_attachment, *bottom_attachment, *left_attachment,
            *right_attachment;
        ...
        int top_offset = 0, top_position = 0, bottom_offset = 0,
            bottom_position = 0, left_offset = 0, left_position = 0,
            right_offset = 0, right_position = 0, cur_position = 1,
            child_count = 0, n = 0;
        ABObj top_obj = NULL, bottom_obj = NULL, left_obj = NULL,
              right_obj = NULL, *child_list = NULL;
         ...
         */
        get_attach_info(object, top_attachment, &top_attach_type, &top_offset,
                        &top_position, &top_obj);
        get_attach_info(object, bottom_attachment, &bottom_attach_type,
                        &bottom_offset, &bottom_position, &bottom_obj);
        get_attach_info(object, right_attachment, &right_attach_type,
                        &right_offset, &right_position, &right_obj);
        get_attach_info(object, left_attachment, &left_attach_type,
                        &left_offset, &left_position, &left_obj);
         ...
         */
        prop_options_set_value(&(ats->top_attach_type),
                               (XtPointer)top_attach_type, False);
        prop_options_set_value(&(ats->bottom_attach_type),
                               (XtPointer)bottom_attach_type, False);
        prop_options_set_value(&(ats->right_attach_type),
                               (XtPointer)right_attach_type, False);
        prop_options_set_value(&(ats->left_attach_type),
                               (XtPointer)left_attach_type, False);
         ...
         */
        set_widget_state(left_attach_type, attch_ed_cgen->left_attach_objmenu,
                         attch_ed_cgen->left_offset_rowcolumn,
                         attch_ed_cgen->left_position_rowcolumn);

        set_widget_state(right_attach_type, attch_ed_cgen->right_attach_objmenu,
                         attch_ed_cgen->right_offset_rowcolumn,
                         attch_ed_cgen->right_position_rowcolumn);

        set_widget_state(top_attach_type, attch_ed_cgen->top_attach_objmenu,
        ...
        /*
         * Set values for position/offset/objmenu depending on type of
         * attachment i.e. if attachment is of type ATTCH_ED_PARENT, don't set
         * the position value.
         */
        set_attach_values(left_attach_type, &(ats->left_attach_offset),
                          &(ats->left_attach_position), &(ats->left_attach_obj),
                          left_offset, left_position, (int)left_obj);

        set_attach_values(right_attach_type, &(ats->right_attach_offset),
                          &(ats->right_attach_position),
                          &(ats->right_attach_obj), right_offset,
                          right_position, (int)right_obj);

        set_attach_values(top_attach_type, &(ats->top_attach_offset),
                          &(ats->top_attach_position), &(ats->top_attach_obj),
                          top_offset, top_position, (int)top_obj);

        set_attach_values(bottom_attach_type, &(ats->bottom_attach_offset),
                          &(ats->bottom_attach_position),
                          &(ats->bottom_attach_obj), bottom_offset,
                          bottom_position, (int)bottom_obj);
...
static void create_children_list(ABObj obj) {
        DtbAttchEdAttchEdDialogInfo attch_ed_cgen =
            &dtb_attch_ed_attch_ed_dialog;
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
        ABObj parent, *child_list, cur_child;
        AB_TRAVERSAL trav;
        int child_count, i, cur_position = 0;
        ...
        if (1) {
                 ...
                 */
                XtVaGetValues(attch_ed_cgen->childrenlist, XmNuserData,
                              &child_list, NULL);
                ...
                if (child_list) {
                        ...
                        XtVaSetValues(attch_ed_cgen->childrenlist, XmNuserData,
                                      NULL, NULL);
                }
                ...
                for (i = 1, trav_open(&trav, parent, AB_TRAV_SALIENT_CHILDREN);
                     (cur_child = trav_next(&trav)) != NULL; ++i) {
                        ...
                        ui_list_add_item(attch_ed_cgen->childrenlist,
                                         obj_get_name(cur_child), 0);
                }

                /*
                 * Make current object selected, and visible in list
                 */
                ui_list_select_pos(attch_ed_cgen->childrenlist, cur_position,
                                   False);
                 ...
                 */
                XtAddCallback(attch_ed_cgen->childrenlist,
...
static void update_objlist(ABObj obj) {
        ...
        num_items = abobj_list_load(attch_ed_cgen->objlist, proj,
...
static int attch_editor_apply(ABObjPtr object) {
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
        ...
        if (prop_changed(ats->top_attach_type.changebar)) {
                attach_type = (ATTCH_ED_ATTACH_TYPE)prop_options_get_value(
                    &(ats->top_attach_type));

                get_attach_values(attach_type, &(ats->top_attach_offset),
                                  &(ats->top_attach_position),
                                  &(ats->top_attach_obj), &offset, &position,
                                  (XtPointer *)&obj);

                set_attach_info(object, &attachment, attach_type, offset,
                                position, obj);
        ...
        if (prop_changed(ats->bottom_attach_type.changebar)) {
                attach_type = (ATTCH_ED_ATTACH_TYPE)prop_options_get_value(
                    &(ats->bottom_attach_type));

                get_attach_values(attach_type, &(ats->bottom_attach_offset),
                                  &(ats->bottom_attach_position),
                                  &(ats->bottom_attach_obj), &offset, &position,
                                  (XtPointer *)&obj);
        ...
        if (prop_changed(ats->left_attach_type.changebar)) {
                attach_type = (ATTCH_ED_ATTACH_TYPE)prop_options_get_value(
                    &(ats->left_attach_type));

                get_attach_values(attach_type, &(ats->left_attach_offset),
                                  &(ats->left_attach_position),
                                  &(ats->left_attach_obj), &offset, &position,
                                  (XtPointer *)&obj);
        ...
        if (prop_changed(ats->left_attach_type.changebar)) {
                attach_type = (ATTCH_ED_ATTACH_TYPE)prop_options_get_value(
                    &(ats->right_attach_type));

                get_attach_values(attach_type, &(ats->right_attach_offset),
                                  &(ats->right_attach_position),
                                  &(ats->right_attach_obj), &offset, &position,
                                  (XtPointer *)&obj);
...
static void attch_ed_select_itemCB(Widget widget, XtPointer client_data,
        ...
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
        ABObj *child_list = NULL, new_obj = NULL;
        int num_items = 0, pos;

        XtVaGetValues(widget, XmNuserData, &child_list, XmNitemCount,
                      &num_items, NULL);
        ...
        if (new_obj != ats->cur_object) {
                ...
                /*
                 * Update object list
                 * If type of object did not change just select new object in
                 * list otherwise, reload the list
                 */
                if (old_obj && (pal_get_item_info(old_obj) ==
                                pal_get_item_info(new_obj))) {
...
static void create_obj_menu(ABObj object) {
        ...
        get_attach_info(object, attachment, &attach_type, &offset, &position,
                        &obj);
...
static void delete_from_obj_menu(ABObj cur_obj, ABObj deleted_obj) {
        ...
        get_attach_info(cur_obj, attachment, &attach_type, &offset, &position,
                        &obj);
        if (attch_ed_need_obj_menu(attach_type))
...
static void create_obj_menu_dir(ABObj object, AB_COMPASS_POINT dir) {
        ...
        Widget label = NULL, optionbox, menu, changebar, *item = NULL;
        int item_values = NULL, *item_val = NULL, n = 0;
        ...
        populate_obj_menu(menu, object, dir, &item, (XtPointer **)&item_val,
                          &n);
        prop_options_init(option_setting, NULL, optionbox, menu, n, item,
                          (XtPointer *)item_val, changebar);
         ...
         */
        if (n > 0)
                prop_options_set_value(option_setting, (XtPointer)item_val[0],
                                       False);
 ...
 */
static void populate_obj_menu(Widget widget, ABObj object, AB_COMPASS_POINT dir,
                              Widget **ret_item, XtPointer **ret_item_val,
                              int *n) {
        AB_TRAVERSAL trav;
        ABObj parent, cur_obj;
        Widget *item_list = NULL;
        WidgetList menu_children;
        XtPointer *item_val_list = NULL;
        int i, num_menu_children, num_items_needed, num_trav_children;
        ...
        if (first_time) {
                 ...
                 */
                XtAddCallback(
                    attch_ed_cgen->left_attach_objmenu_items.Object1_item,
                    XmNactivateCallback, attach_obj_changed,
                    (XtPointer)AB_CP_WEST);

                XtAddCallback(
                    attch_ed_cgen->right_attach_objmenu_items.Object1_item,
                    XmNactivateCallback, attach_obj_changed,
                    (XtPointer)AB_CP_EAST);

                XtAddCallback(
                    attch_ed_cgen->top_attach_objmenu_items.Object1_item,
                    XmNactivateCallback, attach_obj_changed,
                    (XtPointer)AB_CP_NORTH);

                XtAddCallback(
                    attch_ed_cgen->bottom_attach_objmenu_items.Object1_item,
                    XmNactivateCallback, attach_obj_changed,
                    (XtPointer)AB_CP_SOUTH);
         ...
         * count.
         */
        num_items_needed = (num_trav_children > 0) ? num_trav_children - 1 : 0;
        ...
        if (num_items_needed > 0) {
                item_list =
                    (Widget *)util_malloc(num_items_needed * sizeof(Widget));
                item_val_list = (XtPointer *)util_malloc(num_items_needed *
                                                         sizeof(XtPointer));
        }
         ...
         */
        XtVaGetValues(widget, XmNnumChildren, &num_menu_children, XmNchildren,
                      &menu_children, NULL);
        ...
        if (num_items_needed > num_menu_children) {
                 ...
                 */
                XtVaGetValues(widget, XmNnumChildren, &num_menu_children,
                              XmNchildren, &menu_children, NULL);
        }
        ...
        for (i = 0, trav_open(&trav, parent, AB_TRAV_SALIENT_CHILDREN);
             i < num_items_needed, (cur_obj = trav_next(&trav)) != NULL;) {
                 ...
                 */
                if (label)
                        XtVaSetValues(item_list[i], XtVaTypedArg,
                                      XmNlabelString, XtRString, label,
                                      strlen(label) + 1, NULL);
        ...
        if (num_items_needed == 0) {
                ...
                XtVaSetValues(none_item, XtVaTypedArg, XmNlabelString,
                              XtRString, "None", strlen("None") + 1,
                              XmNuserData, NULL, NULL);
...
static BOOL attch_objlist_test_func(ABObj test_obj) {
        ...
        if ((module != NULL) && obj_is_salient_ui(test_obj) &&
...
static void attch_ed_change_objecttype(AB_OBJECT_TYPE type, int subtype) {
        ...
        if (attch_edP_pending()) {
                answer = get_wrn_response(AB_attch_ed_dialog,
                                          ATTCH_ED_WRN_CHANGE_OBJTYPE, NULL);
...
static int attch_obj_destroyCB(ObjEvDestroyInfo info) {
...
static void handle_prop_sheet_obj_destroy(ObjEvDestroyInfo info) {
        DtbAttchEdAttchEdDialogInfo attch_ed_cgen =
            &dtb_attch_ed_attch_ed_dialog;
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
        ABObj obj = info->obj;
        ...
        if (!AB_attch_ed_dialog || !obj_is_salient(obj) || obj_is_item(obj) ||
            !ats->cur_object)
                return;
        ...
        ui_list_get_info(attch_ed_cgen->objlist, &num_items, &first_viz,
                         &last_viz);
        ...
        if (num_items == 0) {
                 ...
                 *
                 * NOTE: should check if the object deleted is an 'attached-to'
                object.
                 *
...
static void handle_actual_obj_destroy(ObjEvDestroyInfo info) {
        ABObjList refList;
        ABObj obj, refObj, project;
        void *voidRefType = NULL;
        AB_OBJ_REF_TYPE refType = AB_REF_UNDEF;
        ABAttachment *attachment;
        ATTCH_ED_ATTACH_TYPE attach_type;
        AB_COMPASS_POINT dir;
        int i, numRefs, offset, position;
        ...
        for (i = 0; i < numRefs; ++i) {
                ...
                if (attach_ref) {
                        /*
                         * If there are any attachments from this object that
                         * involve 'obj', change them to AB_ATTACH_POINT
                         */
                        change_attype_from_sib_to_point(refObj, obj,
                                                        AB_CP_NORTH, FALSE);
                        change_attype_from_sib_to_point(refObj, obj,
                                                        AB_CP_SOUTH, FALSE);
                        change_attype_from_sib_to_point(refObj, obj, AB_CP_EAST,
                                                        FALSE);
                        change_attype_from_sib_to_point(refObj, obj, AB_CP_WEST,
                                                        FALSE);
                }
        }
}

static void handle_prop_sheet_obj_reparent(ObjEvReparentInfo info) {
        DtbAttchEdAttchEdDialogInfo attch_ed_cgen =
            &dtb_attch_ed_attch_ed_dialog;
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
        ABObj obj = info->obj;
...
static void change_attype_from_sib_to_point(ABObj obj, ABObj attach_to_sib,
        ...
        if ((attach_type == AB_ATTACH_OBJ) ||
            (attach_type == AB_ATTACH_ALIGN_OBJ_EDGE)) {
                ...
                obj_set_attachment(obj, dir, AB_ATTACH_POINT, (void *)position,
                                   offset);
                objxm_obj_set_attachment_args(obj, OBJXM_CONFIG_BUILD);
        }
}

static void handle_actual_obj_reparent(ObjEvReparentInfo info) {
        ABObjList refList;
        ABObj obj, refObj, parent;
        void *voidRefType = NULL;
        AB_OBJ_REF_TYPE refType = AB_REF_UNDEF;
        ABAttachment *attachment;
        ATTCH_ED_ATTACH_TYPE attach_type;
...
static int attch_obj_renameCB(ObjEvAttChangeInfo info) {
        DtbAttchEdAttchEdDialogInfo attch_ed_cgen =
            &dtb_attch_ed_attch_ed_dialog;
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
        ABObj obj = info->obj, root_parent;
        ...
        if (obj_is_module(obj)) {
                if ((obj_get_module(ats->cur_object) == obj_get_module(obj))) {
                        abobj_list_obj_renamed(attch_ed_cgen->objlist, obj,
                 ...
                 */
                if ((obj == ats->cur_object) ||
                    obj_is_sibling(obj, ats->cur_object)) {
                        /*
...
static void enable_attach_types(ABObj obj) {
        ...
        parent = attch_ed_get_parent(obj);
        child_count = parent ? trav_count(parent, AB_TRAV_SALIENT_CHILDREN) : 0;
        ...
        /* left attachment */
        ui_set_active(attch_ed_cgen->left_attach_items
                          .bitmaps_attach_left_widget_xbm_item,
                      state);
        ui_set_active(attch_ed_cgen->left_attach_items
                          .bitmaps_attach_left_opposite_widget_xbm_item,
        ...
        /* right attachment */
        ui_set_active(attch_ed_cgen->right_attach_items
                          .bitmaps_attach_right_widget_xbm_item,
                      state);
        ui_set_active(attch_ed_cgen->right_attach_items
                          .bitmaps_attach_right_opposite_widget_xbm_item,
        ...
        /* top attachment */
        ui_set_active(
            attch_ed_cgen->top_attach_items.bitmaps_attach_top_widget_xbm_item,
            state);
        ui_set_active(attch_ed_cgen->top_attach_items
                          .bitmaps_attach_top_opposite_widget_xbm_item,
                      state);

        /* bottom attachment */
        ui_set_active(attch_ed_cgen->bottom_attach_items
                          .bitmaps_attach_bottom_widget_xbm_item,
                      state);
        ui_set_active(attch_ed_cgen->bottom_attach_items
                          .bitmaps_attach_bottom_opposite_widget_xbm_item,
...
static void set_attach_values(ATTCH_ED_ATTACH_TYPE attach_type,
        ...
        switch (attach_type) {
        case ATTCH_ED_PARENT:
                prop_field_set_numeric_value(offset_setting, offset_value,
                                             False);
                break;
        ...
        case ATTCH_ED_SIBLING:
                prop_field_set_numeric_value(offset_setting, offset_value,
                                             False);
                prop_options_set_value(objlist_setting,
                                       (XtPointer)objlist_value, False);
                break;
        ...
        case ATTCH_ED_GRIDLINE:
                prop_field_set_numeric_value(offset_setting, offset_value,
                                             False);
                prop_field_set_numeric_value(position_setting, position_value,
                                             False);
                break;
...
static void get_attach_values(ATTCH_ED_ATTACH_TYPE attach_type,
        ...
        case ATTCH_ED_GRIDLINE:
                *offset_value = prop_field_get_numeric_value(offset_setting);
                *position_value =
                    prop_field_get_numeric_value(position_setting);
                break;
...
static void convert_offset_position(ABObj cur_obj, ABObj sibling_obj,
                                    ...
                                    ATTCH_ED_ATTACH_TYPE attach_type,
                                    int *ret_offset, int *ret_position) {
        Widget cur_widget, sibling_widget, parent_widget;
        ABObj parent_obj;
        Position x = 0, y = 0, p_x = 0, p_y = 0, s_x = 0, s_y = 0;
        Dimension width = 0, height = 0, p_width = 0, p_height = 0, s_width = 0,
                  s_height = 0;
        int fraction_base = 100, offset_value = 0, position_value = 0;
        ...
        *ret_offset = *ret_position = 0;
        ...
        XtVaGetValues(cur_widget, XmNx, &x, XmNy, &y, XmNwidth, &width,
                      XmNheight, &height, NULL);

        XtVaGetValues(parent_widget, XmNx, &p_x, XmNy, &p_y, XmNwidth, &p_width,
                      XmNheight, &p_height, XmNfractionBase, &fraction_base,
                      NULL);
        ...
        if (attch_ed_need_obj_menu(attach_type)) {
                ...
                XtVaGetValues(sibling_widget, XmNx, &s_x, XmNy, &s_y, XmNwidth,
                              &s_width, XmNheight, &s_height, NULL);
        }
        ...
        case AB_CP_WEST:
                ...
                case ATTCH_ED_GRIDLINE:
                        position_value =
                            (fraction_base * (int)x) / (int)p_width;
                        break;

                case ATTCH_ED_CENTER_GRIDLINE:
                        position_value =
                            (fraction_base * ((int)x + ((int)width / 2))) /
                            (int)p_width;
                        break;
        ...
        case AB_CP_EAST:
                ...
                case ATTCH_ED_OPPOSITE_PARENT:
                        offset_value =
                            (int)(-((int)p_width - ((int)x + (int)width)));
                        break;
                ...
                case ATTCH_ED_OPPOSITE_SIBLING:
                        offset_value =
                            ((int)s_x + (int)s_width) - ((int)x + (int)width);
                        break;

                case ATTCH_ED_GRIDLINE:
                        position_value =
                            (fraction_base * ((int)x + (int)width)) /
                            (int)p_width;
                        break;
        ...
        case AB_CP_NORTH:
                ...
                case ATTCH_ED_GRIDLINE:
                        position_value =
                            (fraction_base * (int)y) / (int)p_height;
                        break;

                case ATTCH_ED_CENTER_GRIDLINE:
                        position_value =
                            (fraction_base * ((int)y + ((int)height / 2))) /
                            (int)p_height;
                        break;
        ...
        case AB_CP_SOUTH:
                ...
                case ATTCH_ED_OPPOSITE_SIBLING:
                        offset_value =
                            ((int)s_y + (int)s_height) - ((int)y + (int)height);
                        break;

                case ATTCH_ED_GRIDLINE:
                        position_value =
                            (fraction_base * ((int)y + (int)height)) /
                            (int)p_height;
                        break;
...
static void change_attach_type(ABObj obj, AB_COMPASS_POINT dir,
        ...
        PropFieldSetting offset_setting = NULL, position_setting = NULL;
        Widget objlist_w = NULL, offset_w = NULL, position_w = NULL;
         ...
         */
        set_widget_state(attach_type, objlist_w, offset_w, position_w);
         ...
         */
        convert_offset_position(obj, attach_obj, dir, attach_type, &offset,
                                &position);

        /*
         * Set the new offset/position/sibling object values
         */
        set_attach_values(attach_type, offset_setting, position_setting,
                          objlist_setting, offset, position,
                          (XtPointer)attach_obj);
}
...
static void change_opp_attach_type(ABObj obj, AB_COMPASS_POINT dir,
        ...
        PropOptionsSetting opp_attach_type_setting, opp_objlist_setting;
        PropFieldSetting opp_offset_setting, opp_position_setting;
        Widget opp_objlist_w, opp_offset_w, opp_position_w;
         ...
         * immediately
         */
        if (!opp_attach_type_setting || !opp_offset_setting ||
            !opp_position_setting || !opp_objlist_setting || !opp_objlist_w ||
            !opp_offset_w || !opp_position_w)
                return;

        /*
         * Get opposite side attachment type
         */
        opp_attach_type = (ATTCH_ED_ATTACH_TYPE)prop_options_get_value(
            opp_attach_type_setting);
        ...
        if ((attach_type == ATTCH_ED_CENTER_GRIDLINE) &&
            (opp_attach_type != ATTCH_ED_NONE)) {
                ...
                set_opp_attach = TRUE;
                   ...
                   (opp_attach_type == ATTCH_ED_NONE)) {
                /*
                 * If attachment type is now ATTCH_ED_NONE and the opposite
                 * side's attachment type is also ATTCH_ED_NONE, make the
                 * opposite side's attachment type ATTCH_ED_PARENT. You cannot
                 * set both the opposite sides to ATTCH_ED_NONE.
                 */
                 ...
                 * the opposite side's attachment type is
                 * ATTCH_ED_CENTER_GRIDLINE, make the opposite side's attachment
                 * type ATTCH_ED_NONE. ATTCH_ED_CENTER_GRIDLINE cannot be set on
                 * a side, if the opposite side is not ATTCH_ED_NONE.
                 */
                opp_attach_type = ATTCH_ED_NONE;
                set_opp_attach = TRUE;
        }
        ...
        if (set_opp_attach) {
                 ...
                 */
                set_widget_state(opp_attach_type, opp_objlist_w, opp_offset_w,
                                 opp_position_w);
                ...
                if (attch_ed_need_obj_menu(opp_attach_type)) {
                        ...
                        attach_obj =
                            (ABObj)prop_options_get_value(opp_objlist_setting);
                }

                /*
                 * Set the new attachment type
                 */
                prop_options_set_value(opp_attach_type_setting,
                                       (XtPointer)opp_attach_type, True);

                /*
                 * Figure out offset/position based on (new) attachment type
                 */
                convert_offset_position(obj, attach_obj, opp_dir,
                                        opp_attach_type, &offset, &position);

                /*
                 * Set the new offset/position/sibling object values
                 */
                set_attach_values(opp_attach_type, opp_offset_setting,
                                  opp_position_setting, opp_objlist_setting,
...
static void attach_obj_changed(Widget widget, XtPointer client_data,
        ...
        DtbAttchEdAttchEdDialogInfo attch_ed_cgen =
            &dtb_attch_ed_attch_ed_dialog;
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
        PropFieldSetting offset_setting, position_setting;
        PropOptionsSetting objlist_setting, attach_type_setting;
        ...
        attach_type =
            (ATTCH_ED_ATTACH_TYPE)prop_options_get_value(attach_type_setting);

        if (attch_ed_need_obj_menu(attach_type)) {
                int offset, position;

                convert_offset_position(ats->cur_object, attach_obj, dir,
                                        attach_type, &offset, &position);
                set_attach_values(attach_type, offset_setting, position_setting,
                                  objlist_setting, offset, position,
                                  (XtPointer)attach_obj);
        }
...
static void attch_ed_activate_parent_child_button() {
        ...
        DtbAttchEdAttchEdDialogInfo attch_ed_cgen =
            &dtb_attch_ed_attch_ed_dialog;
        ABObj cur_obj, parent;
        BOOL child_state = FALSE, parent_state = FALSE;
 ...
 */
static void attch_edP_prevent_closeCB(Widget widget, XtPointer client_data,
                                      XtPointer call_data) {
        AttchEditorSettingsRec *ats = &attch_editor_settings_rec;
        DtbAttchEdAttchEdDialogInfo attch_ed_cgen =
            &dtb_attch_ed_attch_ed_dialog;
         ...
         */
        if (attch_edP_pending()) {
                answer = get_wrn_response(widget, ATTCH_ED_WRN_CLOSE, NULL);
        }
...
static DTB_MODAL_ANSWER get_wrn_response(Widget w, ATTCH_ED_WRN_TYPE wrn_type,
        ...
        char wrn_string[512], *cur_name = NULL, *new_name = NULL;
        ...
        if (dtb_app_resource_rec.implied_apply == True)
                ...
                help_data->help_text = help_data->help_volume =
		    ...
		    Load operation for \"%s\"."),
                ...
                case ATTCH_ED_WRN_CHANGE_OBJTYPE:
                        help_data->help_text = XtNewString(catgets(
                            ...
                            "Click Apply Changes to apply the changes to "
                            "the\ncurrent object and display the new object "
                            "type.\n\nClick Cancel if you don't want to apply "
                            "the\nchanges to the current object. You can "
                            "then\nclick Reset to undo the changes before "
		    ...
		    'Change Object-Type' operation."),
                ...
                case ATTCH_ED_WRN_CLOSE:
                        help_data->help_text = XtNewString(catgets(
                            Dtb_project_catd, 100, 247,
                            "Click Apply Changes to apply the changes to "
                            "the\ncurrent object and close the Attachments "
                            "Editor.\n\nClick Cancel if you don't want to "
                            "apply the\nchanges to the current object and want "
                            "the\nAttachments Editor to remain displayed. You "
                            "can\nthen click Reset to undo the changes "
...
BOOL attch_ed_can_edit_attachments(ABObj obj) {
        ...
        if (module &&
            (!obj_has_flag(module, MappedFlag) || !obj_is_defined(module)))
                return (FALSE);

        if (!obj_is_ui(obj) || obj_is_window(obj) || obj_is_menubar(obj) ||
            obj_is_item(obj) || obj_is_sub(obj))
                return (FALSE);
...
static BOOL attch_ed_verify_props() {
        ...
        if (parent && obj_is_group(parent) &&
            (obj_get_group_type(parent) != AB_GROUP_IGNORE)) {
                ...
                XmString xm_buf;
                char *format_str, *obj_name, *parent_name, *msg_buf;
                 ...
                 */
                dtb_attch_ed_grp_member_wrn_initialize(
                    &dtb_attch_ed_grp_member_wrn);

                /*
                 * Fetch format string
                 */
                format_str = XtNewString(
                    catgets(Dtb_project_catd, 100, 245,
                            "Attachments for \"%s\"\ncannot be modified "
                 ...
                 */
                msg_buf =
                    (STRING)util_malloc(strlen(format_str) + strlen(obj_name) +
                                        strlen(parent_name) + 10);
                ...
                help_data->help_text = catgets(
                    Dtb_project_catd, 100, 246,
                    "Group objects have attributes that determine "
                    "what\nattachments their members will have, therefore "
                    "setting\nattachments for group members is not allowed. "
                    "This only\napplies to group objects that have layout type "
                    "Vertical,\nHorizontal, or Row-column. Click Cancel or "
                 ...
                 */
                answer = dtb_show_modal_message(AB_attch_ed_dialog,
                                                &dtb_attch_ed_grp_member_wrn,
                                                xm_buf, help_data, NULL);
        ...
        if (prop_changed(ats->top_attach_type.changebar)) {
                ...
                if (!verify_one_attach(attach_type, &(ats->top_attach_offset),
                                       &(ats->top_attach_position)))
                        return FALSE;
        }
        ...
        if (prop_changed(ats->left_attach_type.changebar)) {
                ...
                if (!verify_one_attach(attach_type, &(ats->left_attach_offset),
                                       &(ats->left_attach_position)))
                        return FALSE;
        }
        ...
        if (prop_changed(ats->left_attach_type.changebar)) {
                ...
                if (!verify_one_attach(attach_type, &(ats->right_attach_offset),
                                       &(ats->right_attach_position)))
                        return FALSE;
        }
...
static BOOL verify_one_attach(ATTCH_ED_ATTACH_TYPE attach_type,
        ...
        case ATTCH_ED_OPPOSITE_SIBLING:
                if (!prop_number_ok(offset_setting->field,
                                    (STRING)OffsetFieldStr, -SHRT_MAX,
                                    SHRT_MAX))
                        return FALSE;
                break;
        ...
        case ATTCH_ED_GRIDLINE:
                if (!prop_number_ok(offset_setting->field,
                    ...
                    !prop_number_ok(position_setting->field,
                                    (STRING)PercentageFieldStr, -SHRT_MAX,
                                    SHRT_MAX))
                        return FALSE;
                break;
