...
/*
 * brws.c
 * This file contains the API/callbacks for the CDE App Builder interface
 * browser. Currently, it is a direct port of the Devguide browser. The contents
 * of this file will change soon...
 */
 ...
 */
static void browser_repaint(Widget widget, XtPointer client_data,
                            XtPointer call_data);

static void browser_resize(Widget widget, XtPointer client_data,
                           XtPointer call_data);

static void browser_orientation(Widget widget, XtPointer client_data,
                                XtPointer call_data);

static void browser_show_view_glyph(Widget widget, XtPointer client_data,
                                    XtPointer call_data);

static void browser_show_view_name(Widget widget, XtPointer client_data,
                                   XtPointer call_data);

static void browser_show_view_type(Widget widget, XtPointer client_data,
                                   XtPointer call_data);

static void browser_show_view_wclass(Widget widget, XtPointer client_data,
                                     XtPointer call_data);

static void browser_find_obj(Widget widget, XtPointer client_data,
                             XtPointer call_data);

static void browser_tear_off(Widget widget, XtPointer client_data,
                             XtPointer call_data);
 ...
 */
static void browser_bpress(Widget w, XEvent *ev, String *params, int nparams);
static void browser_select(Widget w, XEvent *ev, String *params, int nparams);
static void browser_toggle_select(Widget w, XEvent *ev, String *params,
                                  int nparams);
static void browser_invoke_props(Widget w, XEvent *ev, String *params,
                                 int nparams);
static void browser_popup_menu(Widget w, XEvent *ev, String *params,
                               int nparams);

static void browser_rband(Widget widget, XEvent *event, XRectangle *rb_rect,
                          XtPointer client_data);

/*
 * Action Procs for project subwindow of the browser.
 */
static void browser_proj_select(Widget w, XEvent *ev, String *params,
                                int nparams);
static void browser_proj_toggle_select(Widget w, XEvent *ev, String *params,
                                       int nparams);
static void browser_proj_invoke_props(Widget w, XEvent *ev, String *params,
                                      int nparams);
static void browser_proj_popup_menu(Widget w, XEvent *ev, String *params,
                                    int nparams);

/*
 * Object notification callbacks
 */
static int brwsP_obj_renameOCB(ObjEvAttChangeInfo info);

static int brwsP_obj_destroyOCB(ObjEvDestroyInfo info);

static int brwsP_obj_reparentedOCB(ObjEvReparentInfo info);

static int brwsP_selectOCB(ObjEvAttChangeInfo info);

static int brwsP_obj_updateOCB(ObjEvUpdateInfo info);

/*
 * Private functions within this file
 */
static ABBrowser create_browser_struct();
static void destroy_browser_struct(ABBrowser b);

static void create_browser_ui_handles(ABBrowser b);

static void destroy_browser_ui_handles(ABBrowser b);

static BrowserProps aob_create_props();

static void aob_destroy_ui_obj(Vwr b);

static void brws_view_cascadeCB(Widget widget, XtPointer client_data,
                                XtPointer call_data);

static void aob_destroy_props(Vwr b);

static Widget get_main_window(Widget widget);

static int select_rband(VNode vnode);

static int select_fn(VNode vnode);

static void brwsP_drawarea_button_drag(Widget widget, XtPointer client_data,
...
static char *sm_font_name =
    "-*-courier-bold-r-normal--12-120-75-75-m-70-iso8859-1";
 ...
 */
static void browser_proj_select(Widget widget, XEvent *event, String *params,
                                int num_params) {
        ABBrowser ab = NULL;
        Vwr proj_b;
        Vwr mod_b;
        ...
        proj_node = vwr_locate_node(proj_b, event->xbutton.x, event->xbutton.y);
 ...
 */
static void browser_proj_toggle_select(Widget widget, XEvent *event,
                                       String *params, int num_params) {
        ABBrowser ab = NULL;
        Vwr proj_b;
        Vwr mod_b;
        VNode proj_node;
 ...
 */
static void browser_proj_invoke_props(Widget widget, XEvent *event,
                                      String *params, int num_params) {
        AB_OBJ *obj;
        ABBrowser ab = NULL;
        Vwr b;
        ...
        selected_node = vwr_locate_node(b, event->xbutton.x, event->xbutton.y);
 ...
 */
static void browser_proj_popup_menu(Widget widget, XEvent *event,
                                    String *params, int num_params) {
        AB_OBJ *obj;
        ABBrowser ab = NULL;
        Vwr b;
        ...
        if (selected_node) {
                ...
                if (event->type == ButtonPress)
                        abobj_popup_menu(BROWSER_EDIT_MENU, widget, &sel,
                                         (XButtonEvent *)event);
...
static int select_rband(VNode vnode) {
        ...
        if (((int)rband_rect->x <= vnode->x) &&
            ((int)(rband_rect->x + (short)rband_rect->width) >=
             (vnode->x + vnode->width)) &&
            ((int)rband_rect->y <= vnode->y) &&
            ((int)(rband_rect->y + (short)rband_rect->height) >=
             vnode->y + vnode->height))
                return (1);
...
static void browser_rband(Widget widget, XEvent *event, XRectangle *rb_rect,
         ...
         */
        vwr_get_cond(v->current_tree, &selected_nodes, &num_selected,
                     select_rband);
        ...
        for (i = 0; i < num_selected; ++i) {
                ...
                /*
                 * For each in current/old selected list (of the object's
                 * window)
                 */
                for (j = 0; j < old_sel.count; j++) {
                         ...
                         * Deselect object:
                         *
                         * If RUBBERBAND-SELECT, deselect all other currently
                         * selected objects. If RUBBERBAND-ADJUST, deselect only
                         * currently selected NON-sibling objects
                         */
                 ...
                 *
                 * Mark this object as selected only if it is a sibling of any
                 * object on the same window already on the list.
                 *
                ...
                if (new_sel.count == 0) {
                        ...
                        Bool added_to_list = False, window_found = False;

                        /*
                         * List is not empty.
                         * 2 things to check:
                         *	- does obj already have a sibling on the list?
                         *If yes, insert obj
                         *	- is there an object on the list that is on the
                         *same window as obj? Non-siblings can be selected
                         *together if they are not on the same window.
                         */
                        for (j = 0; j < new_sel.count; ++j) {
                                /*
                                 * Remember if an object on the same window was
                                 * found
                                 */
                                if (obj_get_window(obj) ==
                                    obj_get_window(new_sel.list[j]))
                                        window_found = True;
                        ...
                        /*
                         * Double check: Add obj to list if there is no object
                         * in the same window on the list yet.
                         */
 ...
 */
static void browser_bpress(Widget widget, XEvent *event, String *params,
                           int num_params) {
        XButtonEvent *bevent;
        ...
        if (event->type == ButtonPress) {
                ...
                if (bevent->state == 0 &&
                    bevent->button == 1) /* RUBBERBAND SELECT */
                {
                        if (ui_initiate_rubberband(widget, True, browser_rband,
                                                   (XtPointer)NULL) == ERROR) {
                                if (util_get_verbosity() > 0)
                                        fprintf(stderr, "browser: couldn't "
                                                        "begin "
                                                        "rubberbanding\n");
                        } else
 ...
 * Action: caused the browser object to become "selected"
 */
static void browser_select(Widget widget, XEvent *event, String *params,
                           int num_params) {
        ABBrowser ab = NULL;
        Vwr b;
        VNode selected_node;
        ...
        if (selected_node) {
                ...
                elm_selected = (*m->locate_elements)(
                    selected_node, event->xbutton.x, event->xbutton.y);
                ...
                if (util_get_verbosity() > 0) {
                        ...
                        if (obj->ui_handle) {
                                ...
                                util_dprintf(1, "Widget %s has %d children\n",
                                             XtName(objxm_get_widget(obj)),
                                             num_children);
 ...
 * Action: toggles the select-state of the browser object
 */
static void browser_toggle_select(Widget widget, XEvent *event, String *params,
                                  int num_params) {
        AB_OBJ *obj;
        ABBrowser ab = NULL;
        Vwr b;
        ...
        if (selected_node) {
                ...
                if (obj_is_selected(obj_get_root(obj))) {
                         ...
                         * objects
                         */
                        abobj_get_selected(obj_get_window(obj), True, False,
                                           &sel);
 ...
 * Action: invokes the property sheet for the browser object
 */
static void browser_invoke_props(Widget widget, XEvent *event, String *params,
                                 int num_params) {
        AB_OBJ *obj;
        ABBrowser ab = NULL;
        Vwr b;
 ...
 * Action: popup the build menu for the browser object
 */
static void browser_popup_menu(Widget widget, XEvent *event, String *params,
                               int num_params) {
        AB_OBJ *obj;
        ABBrowser ab = NULL;
        Vwr b;
 ...
 */
void brws_register_actions(XtAppContext app) {
 ...
 */
static void enable_proj_actions(Widget widget) {
        if (!orig_trans_set) {
                XtVaGetValues(widget, XtNtranslations, &orig_draw_area_trans,
                              NULL);
                orig_trans_set = True;
        }
        ...
        if (browser_proj_transtbl == NULL) {
                ...
                /* First time only.
                 * Build up translation table based on the number of mouse
                 * buttons and the value of the display resource,
                 * "enableBtn1Transfer"
                 */
                menu_translations =
                    (AB_BMenu == Button3 ? btn3_menu_translations
                                         : btn2_menu_translations);

                len = strlen(proj_base_translations) +
                      strlen(menu_translations) + 1;
        ...
        XtVaSetValues(widget, XtNtranslations, browser_proj_transtbl, NULL);
}

/*
 * Enable browser actions for module subwindow
 */
static void enable_actions(Widget widget) {
        if (!orig_trans_set) {
                XtVaGetValues(widget, XtNtranslations, &orig_draw_area_trans,
                              NULL);
                orig_trans_set = True;
        ...
        /* enable build mode behavior */
        XtVaSetValues(widget, XtNtranslations, browser_transtbl, NULL);
}

/*
 * Disable browser actions for project subwindow
 */
static void disable_proj_actions(Widget widget) {
        if (!orig_trans_set) {
                XtVaGetValues(widget, XtNtranslations, &orig_draw_area_trans,
                              NULL);
                orig_trans_set = True;
        ...
        /* enable build mode behavior */
        XtVaSetValues(widget, XtNtranslations, orig_draw_area_trans, NULL);
}

/*
 * Disable browser actions for module subwindow
 */
static void disable_actions(Widget widget) {
        if (!orig_trans_set) {
                XtVaGetValues(widget, XtNtranslations, &orig_draw_area_trans,
                              NULL);
                orig_trans_set = True;
...
static void brwsP_drawarea_button_drag(Widget widget, XtPointer client_data,
        ...
        if (event->type == MotionNotify) {
                if (((XMotionEvent *)event)->state &
                    Button1Mask) /* RUBBERBAND SELECT */
                {
                        ui_button_drag(widget, event, client_data);
                } else if (((XMotionEvent *)event)->state &
                           Button2Mask) /* RUBBERBAND ADJUST */
                {
                        ui_button_drag(widget, event, client_data);
                }
        }
...
static void setup_callbacks(Widget browser_widget, ABBrowser browser) {
        BrowserUiObj proj_ui, mod_ui;
        ...
        XtVaSetValues(instance->toplevel_drawarea, XmNuserData,
                      (XtPointer)browser, NULL);
        XtVaSetValues(instance->detailed_drawarea, XmNuserData,
                      (XtPointer)browser, NULL);
        XtVaSetValues(instance->menubar_items.View_item, XmNuserData,
                      (XtPointer)browser, NULL);
        XtVaSetValues(
            instance->menubar_View_item_view_pulldown_items.Horizontal_item,
            XmNuserData, (XtPointer)browser, NULL);
        XtVaSetValues(instance->menubar_View_item_view_pulldown_items
                          .Hide_Object_Glyph_item,
                      XmNuserData, (XtPointer)browser, NULL);
        XtVaSetValues(instance->menubar_View_item_view_pulldown_items
                          .Hide_Object_Name_item,
                      XmNuserData, (XtPointer)browser, NULL);
        XtVaSetValues(instance->menubar_View_item_view_pulldown_items
                          .Show_Object_Type_item,
                      XmNuserData, (XtPointer)browser, NULL);
        XtVaSetValues(
            instance->menubar_View_item_view_pulldown_items.Collapse_item,
            XmNuserData, (XtPointer)browser, NULL);
        XtVaSetValues(
            instance->menubar_View_item_view_pulldown_items.Expand_item,
            XmNuserData, (XtPointer)browser, NULL);
        XtVaSetValues(
            instance->menubar_View_item_view_pulldown_items.Expand_All_item,
            XmNuserData, (XtPointer)browser, NULL);

#ifdef BRWS_WIDGET_CLASS
        XtVaSetValues(instance->obj_wclass, XmNuserData, (XtPointer)browser,
                      NULL);
#endif
        ...
        XtVaSetValues(instance->menubar_View_item_view_pulldown_items.Find_item,
                      XmNuserData, (XtPointer)browser, NULL);
        XtVaSetValues(instance->menubar_View_item_view_pulldown_items
                          .Tear_Off_Browser_item,
                      XmNuserData, (XtPointer)browser, NULL);

        XtVaSetValues(
            instance->menubar_View_item_view_pulldown_items.Module_item,
            XmNuserData, (XtPointer)browser, NULL);

        XtVaSetValues(instance->menubar_items.Edit_item, XmNuserData,
                      (XtPointer)browser, NULL);

        XtVaSetValues(instance->menubar_View_item_view_pulldown, XmNuserData,
                      (XtPointer)browser, NULL);
        ...
        XtAddCallback(instance->menubar_items.View_item, XmNcascadingCallback,
                      brws_view_cascadeCB,
                      (XtPointer)instance->menubar_View_item_view_pulldown_items
                          .Module_item);
...
void brws_add_objects_to_browser(ABBrowser ab, AB_OBJ *obj) {
        ViewerMethods *m;
        ABObj module;
...
void brws_delete_objects_from_browser(ABBrowser ab, AB_OBJ *obj) {
        ViewerMethods *m;
...
void brws_add_objects(AB_OBJ *obj) {
        AB_OBJ *project, *module;
...
void brws_delete_objects(AB_OBJ *obj) {
        AB_OBJ *project;
...
static void create_browser_ui_handles(ABBrowser b) {
        ...
        instance =
            (DtbBrwsMainwindowInfo)malloc(sizeof(DtbBrwsMainwindowInfoRec));
        ...
        XtVaSetValues(instance->menubar, XmNmenuHelpWidget,
                      instance->menubar_items.Help_item, NULL);

        XtAddCallback(project_ui->shell, XtNdestroyCallback, browser_destroyCB,
                      (XtPointer)b);
         ...
         * window protocol
         */
        ab_register_window(browser_main, AB_WIN_WINDOW, WindowHidden,
                           AB_toplevel, AB_WPOS_TILE_LEFT, NULL, NULL);
 ...
 */
void setup_vwr_graphics(Viewer *b) {
        ...
        if (!ui_handle->normal_gc) {
                ...
                XtVaGetValues(draw_area, XtNbackground, &ui_handle->bg_color,
                              XtNforeground, &ui_handle->fg_color, NULL);
                ...
                ui_handle->normal_gc =
                    XCreateGC(dpy, XtWindow(draw_area),
        ...
        if (!ui_handle->sm_font) {
                ui_handle->sm_font =
                    (XFontStruct *)XLoadQueryFont(dpy, sm_font_name);
 ...
 */
void aob_free_graphics(Viewer *b) {
 ...
 * big enough.
 */
static void reallocate_drawarea(Viewer *v, int width, int height) {
        ...
        XtVaGetValues(draw_area, XtNwidth, &(w_rect.width), XtNheight,
                      &(w_rect.height), NULL);
 ...
 */
void recompute_viewer(Viewer *v) {
        ViewerMethods *m;
        int end_y = BRWS_Y_ORIGIN, end_x = BRWS_X_ORIGIN, max_x = BRWS_X_ORIGIN,
            max_y = BRWS_Y_ORIGIN;
 ...
 */
void browser_show_view_elements(Viewer *b, unsigned long mask, Widget widget,
                                char *set_str, char *unset_str) {
 ...
 *	selected
 */
void aob_proj_init_state(ABBrowser ab) {
        Vwr proj_b;
        Vwr mod_b;
        VNode top, child;
        ViewerMethods *m;
             ...
             child = (*m->get_child)(top, ++i)) {
                ...
                if (!first_found) {
                        if (child) {
                                ...
                                BRWS_NODE_SET_STATE(module_node,
                                                    BRWS_NODE_VISIBLE);
                        ...
                        if (child) {
                                BRWS_NODE_UNSET_STATE(module_node,
                                                      BRWS_NODE_VISIBLE);
                        }
                }
        }
 ...
 */
BrowserUiObj aob_create_ui_obj() {
        BrowserUiObj ui;
 ...
 */
static void aob_destroy_ui_obj(Vwr v) {
        if (!v && !v->ui_handle)
                return;
 ...
 *	Destroys browser
 */
static void browser_destroyCB(Widget widget, XtPointer client_data,
                              XtPointer call_data) {
        ABBrowser b = (ABBrowser)client_data, prev, next;
        ABObj project = NULL;
...
Vwr aob_proj_or_module(ABBrowser b, Widget w) {
...
int aob_is_widget_proj_browser(ABBrowser b, Widget w) {
        Widget proj_draw_area, draw_area;
...
int aob_is_proj_browser(ABBrowser ab, Vwr b) {
        if (b == ab->project)
                return (TRUE);
 ...
 */
int number_of_selected(VNode tree) {
        ViewerMethods *m;
        VNode child;
         ...
         */
        for (i = 0, child = (*m->get_child)(tree, 0); (i < num_child);
             child = (*m->get_child)(tree, ++i)) {
                total = number_of_selected(child) + total;
        }
        return total +
               (BRWS_NODE_STATE_IS_SET(tree, BRWS_NODE_SELECTED) ? 1 : 0);
}

/*
 * Return the first node selected.
 */
VNode node_selected(VNode tree) {
        ViewerMethods *m;
        VNode child;
        VNode selected_node;
        int i, num_child;
 ...
 */
void aob_set_mode(AB_OBJ *project) {
        ABBrowser new_b, cur_b, b_list = NULL;
...
void brws_switch_module(Widget widget, XtPointer client_data,
                        XtPointer call_data) {
        ABBrowser ab = NULL;
...
void brws_edit_cascadeCB(Widget widget, XtPointer client_data,
                         XtPointer call_data) {
        ...
        if (sel.count == 0) {
                XtVaSetValues(
                    instance->menubar_Edit_item_edit_pulldown_items.Cut_item,
                    XmNsensitive, False, NULL);
                XtVaSetValues(
                    instance->menubar_Edit_item_edit_pulldown_items.Copy_item,
                    XmNsensitive, False, NULL);
                XtVaSetValues(
                    instance->menubar_Edit_item_edit_pulldown_items.Paste_item,
                    XmNsensitive, False, NULL);
                XtVaSetValues(
                    instance->menubar_Edit_item_edit_pulldown_items.Delete_item,
                    XmNsensitive, False, NULL);
        } else {
                XtVaSetValues(
                    instance->menubar_Edit_item_edit_pulldown_items.Cut_item,
                ...
                if (abobj_clipboard_is_empty())
                        XtVaSetValues(
                            instance->menubar_Edit_item_edit_pulldown_items
                                .Paste_item,
                            XmNsensitive, False, NULL);
                else
                        XtVaSetValues(
        ...
        if (abobj_undo_active())
                XtVaSetValues(
                    instance->menubar_Edit_item_edit_pulldown_items.Undo_item,
                    XmNsensitive, True, NULL);
        else
                XtVaSetValues(
                    instance->menubar_Edit_item_edit_pulldown_items.Undo_item,
...
static Widget get_main_window(Widget widget) {
        Widget browser_main = NULL, cur_widget = widget;
 ...
 *  view_cascadeCB().
 *****************************************************************/
void brws_build_module_menu(Widget pulldown, XtCallbackProc callback) {
        ABObj project = proj_get_project();
             ...
             (module = trav_next(&trav)) != NULL;) {
                ...
                if (!util_strempty(name)) {
                        ...
                        mpb = XtVaCreateManagedWidget(
                            ...
                            XmNlabelString, label, XmNuserData, ab, NULL);
        ...
        if (!shown_modules_exist) {
                label = XmStringCreateLocalized(catgets(
                    Dtb_project_catd, 100, 229, "No modules to browse"));
                mpb = XtVaCreateManagedWidget(name, xmPushButtonWidgetClass,
                                              pulldown, XmNlabelString, label,
                                              XmNuserData, ab, NULL);
                XmStringFree(label);
        }
}
 ...
 */
void brws_show_browser(Widget widget, XtPointer client_data,
                       XtPointer call_data) {
 ...
 */
static int brwsP_obj_reparentedOCB(ObjEvReparentInfo info) {
        ABObj obj = info->obj, old_parent = info->old_parent, new_parent;
...
static int brwsP_obj_updateOCB(ObjEvUpdateInfo info) {
        ABObj obj = info->obj, project = NULL, module = NULL;
        ABBrowser b_list = NULL, cur_b;
