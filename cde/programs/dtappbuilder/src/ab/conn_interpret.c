...
typedef void (*INTERPRET_HANDLER)(AB_ACTION_INFO *, XtCallbackProc when_cb);
typedef void (*ACTION_HANDLER)(AB_ACTION_INFO *);
...
static ACTION_HANDLER get_std_action_handler(AB_BUILTIN_ACTION std_act);

static void attach_interpretation(ABObj action);
static void attach_for_single_refs(BOOL for_from, ABObj *item_refs,
                                   AB_ACTION_INFO *info,
                                   INTERPRET_HANDLER handler,
                                   XtCallbackProc when_callback);
static void attach_for_both_refs(ABObj *from_refs, ABObj *to_refs,
                                 AB_ACTION_INFO *info,
                                 INTERPRET_HANDLER handler,
                                 XtCallbackProc when_callback);
static void detach_interpretation(ABObj action);
static void tree_set_initial_state(ABObj obj, BOOL ignore_win_visibility);
static void tree_set_obj_initial_state(ABObj obj);
static void tree_restore_build_state(ABObj obj);
static void set_obj_initial_state(ABObj obj);
static void set_initial_state(ABObj trg_obj, BOOL ignore_win_visibility);
static void restore_build_state(ABObj trg_obj);
static int setup_window_for_test_mode(ABObj win);
static void deiconify_wins(void *cl_data);
static void make_win_iconic(ABObj win, BOOL to_iconify);
static void remove_win_close(void *cl_data);
static void destroy_menus(void *cl_data);
static void sensitize_objects(void *cl_data);
static void free_infos(void *cl_data);
static void reset_layer(void *cl_data);
static void manage_objs(void *cl_data);

static ACTION_HANDLER get_action_handler(AB_ACTION_INFO *notify_info);

static void activate_attach_handler(AB_ACTION_INFO *notify_info,
                                    XtCallbackProc when_cb);
static void activate_detach_handler(AB_ACTION_INFO *notify_info,
                                    XtCallbackProc when_cb);
static void post_menu_attach_handler(AB_ACTION_INFO *notify_info,
                                     XtCallbackProc when_cb);
static void post_menu_detach_handler(AB_ACTION_INFO *notify_info,
                                     XtCallbackProc when_cb);

static void activate_when_cb(Widget w, XtPointer client_data,
                             XtPointer call_data);
static void post_menu_when_cb(Widget w, XtPointer client_data,
                              XtPointer call_data);

static void user_def_fn_handler(AB_ACTION_INFO *action_info);
static void code_frag_handler(AB_ACTION_INFO *action_info);
static void on_item_help_handler(AB_ACTION_INFO *action_info);
static void help_volume_handler(AB_ACTION_INFO *action_info);
static void show_action_handler(AB_ACTION_INFO *action_info);
static void hide_action_handler(AB_ACTION_INFO *action_info);
static void enable_action_handler(AB_ACTION_INFO *action_info);
static void disable_action_handler(AB_ACTION_INFO *action_info);
static void set_text_action_handler(AB_ACTION_INFO *action_info);
static void set_value_action_handler(AB_ACTION_INFO *action_info);
static void set_label_action_handler(AB_ACTION_INFO *action_info);
static void show_help_action_handler(AB_ACTION_INFO *action_info);

static void tree_enable_help(ABObj root);

static void tree_disable_help(ABObj root);

static void win_closeCB(Widget w, XtPointer client_data, XtPointer call_data);
...
static INTERPRET_HANDLER get_when_attach_handler(AB_WHEN when) {
        ...
        case AB_WHEN_ACTIVATED:
                return activate_attach_handler;
...
static INTERPRET_HANDLER get_when_detach_handler(AB_WHEN when) {
        ...
        case AB_WHEN_UNDEF:
                return (INTERPRET_HANDLER)NULL;
        case AB_WHEN_ACTIVATED:
                return activate_detach_handler;
        case AB_WHEN_AFTER_CREATED:
                return (INTERPRET_HANDLER)NULL;
        case AB_WHEN_DESTROYED:
                return (INTERPRET_HANDLER)NULL;
...
static ACTION_HANDLER get_std_action_handler(AB_BUILTIN_ACTION std_act) {
        ...
        case AB_STDACT_DISABLE:
                return disable_action_handler;
        case AB_STDACT_ENABLE:
                return enable_action_handler;
        case AB_STDACT_HIDE:
                return hide_action_handler;
        case AB_STDACT_SET_LABEL:
                return set_label_action_handler;
        case AB_STDACT_SET_TEXT:
                return set_text_action_handler;
        case AB_STDACT_SET_VALUE:
                return set_value_action_handler;
        case AB_STDACT_SHOW:
                return show_action_handler;
...
static ABObj *get_menu_item_refs(ABObj menu_item);
 ...
 */
extern void conn_enable_action_interpret(ABObj project, BOOL to_test_project) {
 ...
 */
extern void conn_disable_action_interpret(ABObj project) {
        ABObj obj;
        AB_TRAVERSAL trav;
        ...
        if (obj_is_menu_item(obj)) {
                ...
                if (ref_list != NULL) {
                        for (cur_obj = ref_list; *cur_obj != NULL; cur_obj++)
                                set_initial_state(*cur_obj,
                                                  ignore_win_visibility);
...
static void set_obj_initial_state(ABObj obj) {
        ...
        if (!obj_is_initially_active(obj)) {
                ...
                if (obj_is_list(obj))
                        XtSetSensitive(objxm_get_widget(objxm_comp_get_subobj(
                                           obj, AB_CFG_OBJECT_OBJ)),
                                       FALSE);
...
static void set_initial_state(ABObj trg_obj, BOOL ignore_win_visibility) {
        ...
        if (obj_is_window(trg_obj)) {
                ...
                if (ignore_win_visibility) {
                        if (setup_window_for_test_mode(trg_obj) == -1)
                                goto cret;

                        /*
                        ** For TestShownModules, if the object is currently
                        *invisible
                        ** show it.
                        */
                        if (!obj_has_flag(trg_obj, MappedFlag)) {
                                if (objxm_tree_map(trg_obj, TRUE) == -1)
                                        goto cret;
                        }

                        /*
                        ** If the object is supposed to be iconic (and visible)
                        ** make it iconic, but remember that we have to
                        *deiconify
                        ** it we go back to Build mode
                        */
                        if (obj_is_initially_iconic(trg_obj)) {
                                make_win_iconic(trg_obj, TRUE);
                                util_llist_insert_after(ConnP_wins_deiconify,
                                                        trg_obj);
                        }
                } else {
                        /*
                        ...
                        */
                        if (is_visible) /* The object should be visible */
                        {
                                ...
                                */
                                ...
                                /*
                                ** If the object is supposed to be iconic (and
                                *visible)
                                ** make it iconic, but remember that we have to
                                *deiconify
                                ** it we go back to Build mode
                                */
                                if (obj_is_initially_iconic(trg_obj)) {
                                        ...
                                        util_llist_insert_after(
                                            ConnP_wins_deiconify, trg_obj);
                                }
                        } else /* !is_visible */
                        {
                                /*
                                ** Object is supposed to be invisible.  If it is
                                *currently
                                ** visible and is a real widget, make invisible.
                                */
                                if (obj_has_flag(trg_obj, MappedFlag) &&
                                    objxm_get_widget(trg_obj))
...
static void restore_build_state(ABObj trg_obj) {
        ...
        if (obj_is_window(trg_obj) && !obj_get_resizable(trg_obj))
                ui_win_set_resizable(trg_w, True,
                                     obj_has_flag(trg_obj, MappedFlag));
}
...
static int setup_window_for_test_mode(ABObj win) {
        ...
        */
        if (obj_is_base_win(win) || obj_is_popup_win(win) ||
            obj_is_file_chooser(win)) {
                /* Windows, Dialogs and FSB's need a window-close callback to
                 * handle "manual" close as if it were a 'hide' action.
                 */
                ui_add_window_close_callback(objxm_get_widget(win), win_closeCB,
                                             (XtPointer)win, XmUNMAP);
                ...
                if (obj_get_resizable(win) == False) {
                        ...
                        /*
                         * Since we need to unmap/remap the window in order to
                         * get the decorations to change, we need to mark this
                         * window object so that when the iconify_tracker event
                         * handler is called, it doesn't interpret the Unmap
                         * event as an iconify. (this is an unfortunate hack to
                         * get around the mysterious problem of the Unmap event
                         * occurring AFTER we return to BUILD mode).
                         */
...
static void disable_modules(void *cl_data) {
...
static void sensitize_objects(void *cl_data) {
        ...
        if (obj != NULL &&
            !tmodeP_obj_has_flags(obj_get_window(obj),
                                  TestModeFlagResizedWindow) &&
...
static void attach_interpretation(ABObj action) {
        ...
        if (info->func_type == AB_FUNC_BUILTIN) { /* connection is a builtin one
                                                     and needs a target */
        ...
        if (needs_from_refs == TRUE) {
                ...
                if (needs_to_refs == TRUE) {
                        ...
                        attach_for_both_refs(from_refs, to_refs, info, handler,
                                             get_when_callback(c_when));
                        XtFree((char *)to_refs);
                } else
                        attach_for_single_refs(TRUE, from_refs, info, handler,
                                               get_when_callback(c_when));
                XtFree((char *)from_refs);
                ...
                attach_for_single_refs(FALSE, to_refs, info, handler,
                                       get_when_callback(c_when));
                XtFree((char *)to_refs);
        } else
                (*handler)(info, get_when_callback(c_when));
...
static void attach_for_both_refs(ABObj *from_refs, ABObj *to_refs,
        ...
        for (trav_from = from_refs; *trav_from != NULL; trav_from++) {
                ...
                for (trav_to = to_refs; *trav_to != NULL; trav_to++) {
                        ...
                        new_info =
                            (AB_ACTION_INFO *)XtMalloc(sizeof(AB_ACTION_INFO));
                        ...
                        /* add info to list */
                        util_llist_insert_after(ConnP_menu_item_infos,
                                                new_info);
...
static void attach_for_single_refs(BOOL for_from, ABObj *item_refs,
        ...
        for (trav_item = item_refs; *trav_item != NULL; trav_item++) {
                ...
                /* create new info structure and copy old one */
                new_info = (AB_ACTION_INFO *)XtMalloc(sizeof(AB_ACTION_INFO));
                memmove((void *)new_info, (void *)info, sizeof(AB_ACTION_INFO));
                        ...
                        if (obj_is_window(trg)) {
                                ...
                                default:
                                        if (!obj_has_flag(trg, MappedFlag))
                                                ...
                                                handler =
                                                    get_std_action_handler(
                                                        notify_info->func_value
                                                            .builtin);
                                        break;
                                }
                        } else {
...
static void activate_attach_handler(AB_ACTION_INFO *notify_info,
                ...
                case AB_ITEM_FOR_CHOICE:
                        ...
                        if (attach_w != NULL) {
                                if (obj_is_option_menu(
                                        ...
                                        XtAddCallback(
                                            attach_w, XmNactivateCallback,
                                            when_cb, (XtPointer)notify_info);
                                else
                                        XtAddCallback(
                                            attach_w, XmNvalueChangedCallback,
                                            when_cb, (XtPointer)notify_info);
                        }
                        break;
                }
        ...
        case AB_TYPE_BUTTON:
                attach_w = objxm_get_widget(c_from);
                if (attach_w != NULL)
                        XtAddCallback(attach_w, XmNactivateCallback, when_cb,
                                      (XtPointer)notify_info);
                break;
        default:
                break;
...
static void activate_detach_handler(AB_ACTION_INFO *notify_info,
        ...
        case AB_TYPE_ITEM:
                ...
                case AB_ITEM_FOR_MENU: {
                        if (obj_has_menu(c_from)) {
                                if (objxm_get_widget(c_from) != NULL)
                                        XtRemoveCallback(
                                            objxm_get_widget(c_from),
                                            XmNcascadingCallback, when_cb,
                                            (XtPointer)notify_info);
                        } else {
                                if (objxm_get_widget(c_from) != NULL)
                                        XtRemoveCallback(
                                            objxm_get_widget(c_from),
                                            XmNactivateCallback, when_cb,
                                            (XtPointer)notify_info);
                        }
                } break;
        ...
        case AB_TYPE_BUTTON:
                attach_w = objxm_get_widget(c_from);
                if (attach_w != NULL)
                        XtRemoveCallback(attach_w, XmNactivateCallback, when_cb,
                                         (XtPointer)notify_info);
                break;
        default:
                break;
...
static void activate_when_cb(Widget w, XtPointer client_data,
        ...
        case AB_TYPE_ITEM:
                if (obj_get_subtype(notify_info->from) ==
                    (int)AB_ITEM_FOR_CHOICE) {
                        if (!obj_is_option_menu(obj_get_root(
                                obj_get_parent(notify_info->from)))) {
                                XmToggleButtonCallbackStruct *tog_data =
                                    (XmToggleButtonCallbackStruct *)call_data;

                                if (!tog_data ||
                                    tog_data->reason != XmCR_VALUE_CHANGED ||
                                    !tog_data->set)
                                        return;
                        }
                }
...
static void show_help_action_handler(AB_ACTION_INFO *action_info) {
        ...
        if (obj_has_help_data(c_to) == True) {
                ...
                if (help_obj != NULL) {
                        XtCallCallbacks(objxm_get_widget(help_obj),
                                        XmNhelpCallback, (XtPointer)NULL);
                }
        }
}
...
static void show_action_handler(AB_ACTION_INFO *action_info) {
        ...
        if (obj_is_window(c_to)) {
                ...
                if (objxm_get_widget(c_to) &&
                    !XtIsManaged(objxm_get_widget(c_to))) {
                        if (!obj_is_menu_item(c_to)) {
                                /* Layers */
                                if (obj_is_layers(obj_get_parent(c_to))) {
                                        layer = obj_get_parent(c_to);
                                        /*
                                         * If there is only one pane, this one
                                         * must be it. Thus, do nothing; else
                                         * manage it.
                                         */
                                        if (obj_get_num_salient_children(
                                                layer) > 1) {
                                                ui_set_visible(
                                                    objxm_get_widget(c_to),
                                                    TRUE);

                                                /* add layer to clean up when we
                                                 * go back to Build Mode */
                                                util_aset_add(ConnP_layer,
                                                              layer);
                                        }
                                } else {
                                        ui_set_visible(objxm_get_widget(c_to),
                                                       TRUE);
                                }
                        }
                }
        ...
        if (obj_is_window(c_to)) {
                ...
                if (XtIsManaged(objxm_get_widget(c_to))) {
                        if (!obj_is_menu_item(c_to)) {
                                ...
                                /* Show this object when we go back to Build
                                 * mode */
...
static void set_text_action_handler(AB_ACTION_INFO *action_info) {
        ...
        if (obj_is_text(c_to)) {
                ...
                tmodeP_obj_set_flags(obj_get_window(c_to),
                                     TestModeFlagDirtyWindow);
        }
}
...
static void set_value_action_handler(AB_ACTION_INFO *action_info) {
        ...
        if (obj_is_spin_box(c_to) || obj_is_scale(c_to)) {
                ...
                if (obj_is_spin_box(c_to))
                        XtVaSetValues(objxm_get_widget(subObj), DtNposition,
                                      val_int, NULL);
                else
                        XtVaSetValues(objxm_get_widget(subObj), XmNvalue,
                                      val_int, NULL);
...
static ABObj *get_menu_item_refs(ABObj menu_item) {
        ...
        for (i = 0, trav_open(&trav, parent_menu, AB_TRAV_CHILDREN);
             (item_child = trav_next(&trav)) != NULL; i++) {
                if (obj_get_type(item_child) == AB_TYPE_ITEM &&
                    util_streq(obj_get_name(item_child),
                               obj_get_name(menu_item))) {
                        item_index = i;
                        break;
                }
        }
        ...
        for (trav_open(&trav, this_module, AB_TRAV_ALL);
             (child = trav_next(&trav)) != NULL;) {
                if (obj_is_menu_ref(child) &&
                    obj_get_actual_obj(child) == parent_menu) {
                        ABObj item_ref;

                        /* Find our item by index in each tree rooted at a
                         * menu-reference */
 ...
 */
extern void conn_test_mode_initialize(ABObj project) {
        AB_TRAVERSAL trav;
        ABObj obj;
 ...
 */
extern void conn_test_mode_cleanup(ABObj project) {
...
static void make_win_iconic(ABObj win, BOOL to_iconify) {
        ...
        event.xclient.message_type =
            XInternAtom(XtDisplay(top_level), "WM_CHANGE_STATE", False);
...
static void tree_enable_help(ABObj obj) {
        ABObj child;
        AB_TRAVERSAL trav;

        if (!obj_is_virtual(obj))
                help_test_enable(obj);
...
static void tree_disable_help(ABObj obj) {
        ABObj child;
        AB_TRAVERSAL trav;

        if (!obj_is_virtual(obj))
                help_test_disable(obj);
...
static void remove_win_close(void *cl_data) {
        ...
        ui_remove_window_close_callback(objxm_get_widget(obj), win_closeCB,
                                        (XtPointer)obj);
}
