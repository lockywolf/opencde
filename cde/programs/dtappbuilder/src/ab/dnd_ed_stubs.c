...
static void dnd_change_objecttype_palCB(Widget widget, XtPointer client_data,
                                        XtPointer call_data);
static void change_objtype(Widget widget, AB_OBJECT_TYPE newtype,
                           int newsubtype);
static void update_objecttype_menu_from_obj(Widget menu, ABObj obj);
static Widget dnd_ed_editor_init(DtbDndEdDialogInfo dnd_ed, Widget parent);
static DTB_MODAL_ANSWER do_auto_apply(Widget list, ABObj old_obj,
                                      ABObj new_obj);
static void clear_editor_fields(void);

static BOOL dnd_editable_obj_test(PalEditableObjInfo *ed_obj_info);
static int dnd_ed_editor_load(ABObj project);
...
static int dnd_ed_update_objecttype_menu(Widget menu_pane);

static BOOL dnd_list_test(ABObj test_obj);

static int dnd_store_attrs(ABObj obj);
static void dnd_fetch_attrs(ABObj obj);

static BOOL dnd_obj_is_target_type(ABObj obj);

static int dnd_obj_was_deleted(ObjEvDestroyInfo info);
static int dnd_obj_was_renamed(ObjEvAttChangeInfo info);
static int dnd_obj_was_updated(ObjEvUpdateInfo info);

static void dnd_select_instanceCB(Widget widget, XtPointer client_data,
                                  XmListCallbackStruct *listdata);

static void dnd_prevent_closeCB(Widget widget, XtPointer client_data,
                                XtPointer call_data);
...
void dnd_ed_show_dialog() {
        /* If there is no DragAndDrop Editor, create it */
        if (AB_dnd_dialog == (Widget)NULL) {
                AB_dnd_dialog =
                    dnd_ed_editor_init(&dtb_dnd_ed_dialog, AB_toplevel);
        }
...
static Widget dnd_ed_editor_init(DtbDndEdDialogInfo dnd_ed, Widget parent) {
...
#ifdef DEBUG
                util_dprintf(1, "dnd_ed_show_dialog: could not initialize Drag "
                                "and Drop Editor\n");
                return NULL;
#endif /* DEBUG */
        }
        ...
        prop_checkbox_init(
            &(dds->drag_ops_checkbox), dnd_ed->drag_op_checkbox_label,
            dnd_ed->drag_op_checkbox, n, item, item_val, dnd_ed->drag_op_cb);

        /* drag icon text */
        prop_field_init(&(dds->drag_icon_text), dnd_ed->drag_icon_field_label,
                        dnd_ed->drag_icon_field, dnd_ed->drag_icon_cb);
        prop_field_init(
            &(dds->drag_icon_mask_text), dnd_ed->drag_icon_mask_field_label,
            dnd_ed->drag_icon_mask_field, dnd_ed->drag_icon_mask_cb);
        ...
        prop_radiobox_init(&(dds->drag_types_radiobox),
                           dnd_ed->drag_data_checkbox_label,
                           dnd_ed->drag_data_checkbox, n, item,
                           (XtPointer *)item_val, dnd_ed->drag_data_cb);
        ...
        item_val[n++] = (int)ABDndOpLink;
        prop_checkbox_init(
            &(dds->drop_ops_checkbox), dnd_ed->drop_op_checkbox_label,
            dnd_ed->drop_op_checkbox, n, item, item_val, dnd_ed->drop_op_cb);
        ...
        prop_checkbox_init(&(dds->drop_types_checkbox),
                           dnd_ed->drop_data_checkbox_label,
                           dnd_ed->drop_data_checkbox, n, item, item_val,
                           dnd_ed->drop_data_cb);
        ...
        prop_checkbox_init(&(dds->drop_child_checkbox),
                           ...
                           dnd_ed->drop_child_checkbox, n, item, item_val,
...
static int dnd_ed_update_objecttype_menu(Widget menu_pane) { return 0; }
        ...
        */
        if ((dds->curObj != NULL) && prop_changebars_pending(dds->propSheet)) {
                ...
                if (answer == DTB_ANSWER_CANCEL) {
                        update_objecttype_menu_from_obj(dds->objTypeMenu,
                                                        dds->curObj);
                        return;
                }
        }
...
static void update_objecttype_menu_from_obj(Widget menu, ABObj obj) {
        ...
        if ((menu_selection = XmOptionButtonGadget(menu)) == NULL)
                return;
...
static int dnd_ed_editor_load(ABObj obj) {
        DndEditorSettings dds = &dndEdInfo;
...
static void dnd_turnoff_changebars() {
        DndEditorSettings dds = &dndEdInfo;
...
static void dnd_select_instanceCB(Widget widget, XtPointer client_data,
        ...
        if ((listdata->selected_item_count == 0) || (name == NULL)) {
                        ...
                        */
                        if ((dds->curObj != NULL) &&
                            prop_changebars_pending(dds->propSheet) == True) {
                                (void)do_auto_apply(widget, dds->curObj,
                                                    selected_obj);
                        }
        ...
        if (util_get_verbosity() > 0)
                fprintf(stderr, "Dnd Editor::dnd_select_instanceCB: could not "
                                "get object in list\n");
}
...
static void clear_editor_fields() {
        ...
        prop_checkbox_set_value(&(dds->drag_ops_checkbox), ABDndOpCopy, False,
                                False);
        prop_checkbox_set_value(&(dds->drag_ops_checkbox), ABDndOpLink, False,
                                False);
        prop_checkbox_set_value(&(dds->drag_ops_checkbox), ABDndOpMove, False,
                                False);
        prop_field_set_value(&(dds->drag_icon_text), Util_empty_string, False);
        prop_field_set_value(&(dds->drag_icon_mask_text), Util_empty_string,
                             False);
        prop_radiobox_set_value(&(dds->drag_types_radiobox),
                                (XtPointer)ABDndTypeText, False);

        prop_checkbox_set_value(&(dds->drop_ops_checkbox), ABDndOpCopy, False,
                                False);
        prop_checkbox_set_value(&(dds->drop_ops_checkbox), ABDndOpLink, False,
                                False);
        prop_checkbox_set_value(&(dds->drop_ops_checkbox), ABDndOpMove, False,
                                False);
        prop_checkbox_set_value(&(dds->drop_types_checkbox), ABDndTypeText,
                                False, False);
        prop_checkbox_set_value(&(dds->drop_types_checkbox), ABDndTypeFilename,
                                False, False);
        prop_checkbox_set_value(&(dds->drop_types_checkbox), ABDndTypeUserDef,
                                False, False);
        prop_checkbox_set_value(&(dds->drop_child_checkbox), 0, False, False);
...
static void dnd_fetch_attrs(ABObj obj) {
        ...
        util_dprintf(2,
                     "Load object dnd attributes into editor display fields\n");
        ...
        prop_checkbox_set_value(&(dds->drag_ops_checkbox), ABDndOpCopy,
                                ((dragOps & ABDndOpCopy) != 0), False);
        prop_checkbox_set_value(&(dds->drag_ops_checkbox), ABDndOpLink,
                                ((dragOps & ABDndOpLink) != 0), False);
        prop_checkbox_set_value(&(dds->drag_ops_checkbox), ABDndOpMove,
                                ((dragOps & ABDndOpMove) != 0), False);
        prop_field_set_value(&(dds->drag_icon_text), obj_get_drag_cursor(obj),
                             False);
        ...
        prop_checkbox_set_value(&(dds->drop_ops_checkbox), ABDndOpCopy,
                                ((dropOps & ABDndOpCopy) != 0), False);
        prop_checkbox_set_value(&(dds->drop_ops_checkbox), ABDndOpLink,
                                ((dropOps & ABDndOpLink) != 0), False);
        prop_checkbox_set_value(&(dds->drop_ops_checkbox), ABDndOpMove,
                                ((dropOps & ABDndOpMove) != 0), False);
        prop_checkbox_set_value(&(dds->drop_types_checkbox), ABDndTypeText,
                                ((dropTypes & ABDndTypeText) != 0), False);
        prop_checkbox_set_value(&(dds->drop_types_checkbox), ABDndTypeFilename,
                                ((dropTypes & ABDndTypeFilename) != 0), False);
        prop_checkbox_set_value(&(dds->drop_types_checkbox), ABDndTypeUserDef,
                                ((dropTypes & ABDndTypeUserDef) != 0), False);

        prop_checkbox_set_value(&(dds->drop_child_checkbox), 0, dropOnChildren,
                                False);
...
static int dnd_store_attrs(ABObj obj) {
        ...
        if (prop_checkbox_get_value(&(dds->drop_types_checkbox),
                                    ABDndTypeText)) {
                dropTypes |= ABDndTypeText;
        }
        if (prop_checkbox_get_value(&(dds->drop_types_checkbox),
        ...
        dropOnChildren =
            prop_checkbox_get_value(&(dds->drop_child_checkbox), 0);
...
static int check_active(void) {
        ...
        if (dds->curObj != NULL) {
                ...
                XtSetSensitive(dnd_ed->drag_icon_mask_field_rowcolumn,
                               sensitive);
                ...
                XtSetSensitive(dnd_ed->drop_child_checkbox_rowcolumn,
                               sensitive);
...
static BOOL dnd_obj_is_target_type(ABObj obj) {
        ...
        if ((dds->objType == AB_TYPE_ITEM) ||
            (dds->objType == AB_TYPE_CONTAINER)) {
                if ((type == dds->objType) && (subtype == dds->objSubtype)) {
                        return (True);
                } else
                        return (False);
        }
...
static int dnd_obj_was_renamed(ObjEvAttChangeInfo info) {
        return abobj_list_obj_renamed(dndEdInfo.objList, info->obj,
...
)
{
    ...
    /*** DTB_USER_CODE_START vvv Add C code below vvv ***/
        if (dndEdInfo.curObj != NULL) {
                ...
                conn_popup_dialog(dtb_get_toplevel_widget(), (XtPointer)0,
                                  NULL);
