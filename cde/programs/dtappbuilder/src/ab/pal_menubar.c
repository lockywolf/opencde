 ...
 * Methods
 */
static int menubar_initialize(ABObj obj);
static Widget menubar_prop_init(Widget parent, AB_PROP_TYPE type);
static int menubar_prop_activate(AB_PROP_TYPE type, BOOL active);
static int menubar_prop_load(ABObj obj, AB_PROP_TYPE type,
                             unsigned long loadkey);
static int menubar_prop_clear(AB_PROP_TYPE type);
static int menubar_prop_apply(AB_PROP_TYPE type);
static BOOL menubar_prop_pending(AB_PROP_TYPE type);
 ...
 * ABObj callbacks
 */
static int prop_menubar_obj_destroy_CB(ObjEvDestroyInfo destroyInfo);
...
static Widget menubar_prop_init(Widget parent, AB_PROP_TYPE type) {
        DtbMenubarPropDialogInfoRec
            rev_menubar_prop_dialog; /* Revolving Props */
        DtbMenubarPropDialogInfo cgen =
            &dtb_menubar_prop_dialog; /* Codegen structure */
        ...
        if (dtb_menubar_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                n++;
                prop_options_init(
                    &(pms->item_label_type), cgen->item_labeltype_label,
                ...
                /* Item Menu Name */
                prop_menuname_init(&(pms->item_menuname), type,
                                   cgen->menu_label, cgen->item_menu_mbutton,
                ...
                prop_item_editor_init(
                    &(pms->items), AB_ITEM_FOR_MENUBAR, cgen->itemlist,
                    cgen->itemlist_cb, cgen->item_add_button, n, item,
                    item_val,            /* Insert */
                    j, item2, item2_val, /* Edit */
                    &(pms->item_label), &(pms->item_label_type),
                    cgen->graphic_hint, &(pms->item_mnemonic), NULL /*accel*/,
                    NULL /*line_style*/, &(pms->item_state),
                    &(pms->item_menuname), &(pms->current_obj));
...
static int menubar_prop_clear(AB_PROP_TYPE type) {
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pms->init_state), AB_STATE_ACTIVE, True,
                                False);
...
static int menubar_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
                             unsigned long loadkey) {
        PropMenubarSettingsRec *pms = &(prop_menubar_settings_rec[type]);
...
static int menubar_prop_apply(AB_PROP_TYPE type) {
        ...
        XtVaSetValues(objxm_get_widget(mb_obj), XmNmenuHelpWidget,
                      help_item ? objxm_get_widget(help_item) : NULL, NULL);
...
static BOOL menubar_prop_pending(AB_PROP_TYPE type) {
        return (prop_changebars_pending(
            prop_menubar_settings_rec[type].prop_sheet));
}
 ...
 */
static int prop_menubar_install_obj_destroy_CB() {
