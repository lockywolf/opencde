 ...
 * Methods
 */
static int menu_initialize(ABObj obj);
static Widget menu_prop_init(Widget parent, AB_PROP_TYPE type);
static int menu_prop_activate(AB_PROP_TYPE type, BOOL active);
static int menu_prop_clear(AB_PROP_TYPE type);
static int menu_prop_load(ABObj obj, AB_PROP_TYPE type, unsigned long loadkey);
static int menu_prop_apply(AB_PROP_TYPE type);
static BOOL menu_prop_pending(AB_PROP_TYPE type);
 ...
 */
static void edit_item_init(Widget widget, AB_PROP_TYPE ptype,
                           AB_EDIT_TYPE etype);
static void copy_menu_to_clipboard(ABObj mobj, ABObj *menu_clipboard);

/*
 * ABObj callbacks
 */
static int prop_menu_obj_destroy_CB(ObjEvDestroyInfo destroyInfo);
static int prop_menu_install_obj_destroy_CB(void);

/*
 * Xt Callbacks
 */
static void menu_createCB(Widget widget, XtPointer client_data,
                          XtPointer call_data);
...
static int menu_initialize(ABObj obj) {
        ...
        for (i = 0; i < XtNumber(items); i++) {
                ...
                abobj_set_item_name(iobj, obj_get_module(obj),
                                    obj_get_name(obj), items[i]);
...
static Widget menu_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbMenuPropDialogInfo cgen =
            &dtb_menu_prop_dialog; /* Codegen structure */
        ...
        if (dtb_menu_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_REVOLVING)
                        XtVaSetValues(parent, XmNuserData, pms->current_obj,
                                      NULL);
                ...
                edit_item_init(
                    cgen->menuedit_mbutton_menu_editmenu_items.Cut_item, type,
                    AB_EDIT_CUT);
                edit_item_init(
                    cgen->menuedit_mbutton_menu_editmenu_items.Copy_item, type,
                    AB_EDIT_COPY);
                edit_item_init(
                    cgen->menuedit_mbutton_menu_editmenu_items.Paste_item, type,
                    AB_EDIT_PASTE);
                edit_item_init(
                    cgen->menuedit_mbutton_menu_editmenu_items.Delete_item,
                    type, AB_EDIT_DELETE);
                ...
                prop_radiobox_init(&(pms->tearoff), cgen->tearoff_rbox_label,
                                   cgen->tearoff_rbox, n, item,
                                   (XtPointer *)item_val, cgen->tearoff_cb);

                /* Color */
                prop_colorfield_init(
                    &(pms->bg_color), cgen->bg_mbutton,
                ...
                item_val[n] = AB_LABEL_STRING;
                n++;
                item[n] = cgen->itemlabeltype_opmenu_items.Graphic_item;
                item_val[n] = AB_LABEL_GLYPH;
                n++;
                item[n] = cgen->itemlabeltype_opmenu_items.Separator_item;
                item_val[n] = AB_LABEL_SEPARATOR;
                n++;
                prop_options_init(
                    &(pms->item_label_type), cgen->itemlabeltype_opmenu_label,
                ...
                for (i = 0; i < n; i++)
                        XtAddCallback(item[i], XmNactivateCallback,
                                      (XtCallbackProc)prop_item_labeltypeCB,
                                      (XtPointer) & (pms->items));

                /* Item Mnemonic */
                prop_field_init(&(pms->item_mnemonic),
                                cgen->mnemonic_field_label,
                ...
                item_val[n] = AB_LINE_ETCHED_IN_DASH;
                n++;
                item[n] = cgen->linestyle_opmenu_items.Etched_Out_Dash_item;
                item_val[n] = AB_LINE_ETCHED_OUT_DASH;
                n++;
                item[n] = cgen->linestyle_opmenu_items.Single_Line_item;
                item_val[n] = AB_LINE_SINGLE_LINE;
                n++;
                item[n] = cgen->linestyle_opmenu_items.Double_Line_item;
                item_val[n] = AB_LINE_DOUBLE_LINE;
                n++;
                item[n] = cgen->linestyle_opmenu_items.Single_Dashed_Line_item;
                item_val[n] = AB_LINE_SINGLE_DASHED_LINE;
                n++;
                item[n] = cgen->linestyle_opmenu_items.Double_Dashed_Line_item;
                item_val[n] = AB_LINE_DOUBLE_DASHED_LINE;
                n++;
                prop_options_init(
                    &(pms->item_line_style), cgen->linestyle_opmenu_label,
                    cgen->linestyle_opmenu, cgen->linestyle_opmenu_menu, n,
                    item, (XtPointer *)item_val, cgen->itemlist_cb);
                prop_options_set_value(&(pms->item_line_style),
                ...
                n++;
                prop_checkbox_init(&(pms->item_state),
                                   cgen->itemstate_ckbox_label,
...
static int menu_prop_activate(AB_PROP_TYPE type, BOOL active) {
        /*
         * Since the "Add Menu" button must remain active at all times, we
         * cannot make the entire prop sheet inactive;  Therefore, we set
         * sensitivity on more specific controls in the prop sheet (not the
         * entire Form).
         */
        ...
        ui_set_active(prop_menu_settings_rec[type].item_list_button_group,
                      active);
        ...
        ui_set_active(prop_menu_settings_rec[type].item_label_type.label,
                      active);
        ui_set_active(prop_menu_settings_rec[type].item_label_type.optionbox,
                      active);
        ...
        ui_set_active(prop_menu_settings_rec[type].item_line_style.label,
                      active);
        ui_set_active(prop_menu_settings_rec[type].item_line_style.optionbox,
                      active);
        ...
        ui_set_active(prop_menu_settings_rec[type].item_menuname.menubutton,
                      active);
...
static int menu_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_radiobox_set_value(&(pms->tearoff),
                                        (XtPointer)(obj->info.menu.tear_off),
                                        False);
...
static int menu_prop_apply(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pms->tearoff.changebar)) {
                abobj_set_tearoff(pms->current_obj,
                                  prop_radiobox_get_value(&(pms->tearoff)));
        }
        if (prop_changed(pms->fg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pms->fg_color),
                                          obj_get_fg_color(pms->current_obj),
                                          False);
        }
        if (prop_changed(pms->bg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pms->bg_color),
                                          obj_get_bg_color(pms->current_obj),
                                          False);
        }
...
static BOOL menu_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_menu_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pms->fg_color.changebar) &&
            !prop_color_ok(pms->fg_color.field))
                return False;

        if (prop_changed(pms->bg_color.changebar) &&
            !prop_color_ok(pms->bg_color.field))
                return False;
...
static void edit_item_init(Widget widget, AB_PROP_TYPE ptype,
        ...
        XtAddCallback(widget, XmNactivateCallback, menu_editCB,
                      (XtPointer)ptype);
}
...
static void set_edit_menu_stateCB(Widget widget, XtPointer client_data,
        ...
        XtVaGetValues(pms->edit_menu, XmNnumChildren, &num_menu_items,
...
int pal_create_menu(AB_PROP_TYPE type, ABObj module, STRING menu_name,
                    STRING menu_title) {
        PropMenuSettingsRec *pms = &(prop_menu_settings_rec[type]);
        ABObj newmenu;
...
static void menu_createCB(Widget widget, XtPointer client_data,
        ...
        name = obj_get_unique_name_for_child(proj_get_cur_module(), "menu",
                                             5000, nameBuf);
...
static void menu_editCB(Widget widget, XtPointer client_data,
        ...
        case AB_EDIT_CUT:
                if (pms->current_obj != NULL) {
                        copy_menu_to_clipboard(pms->current_obj,
                                               &(pms->clipboard_menu));
        ...
        case AB_EDIT_PASTE:
                if (pms->clipboard_menu != NULL) {
                        /* We need to use some trickery here in order to give
                         * the new copy of the menu an incremented/unique name
                         * based on the clipboard menu's name.  We must
                         * temporarily re-name the clipboard menu so that the
                         * dup'd copy won't get an identicle name as the
                         * clipboard menu (which in turn will cause the prop
                         * sheet mechanism to think the clipboard menu is being
                         * renamed, rather than a new menu being added). Trust
                         * me - this works.
                         */

                        /* store off original clipboard menu name and
                         * temporarily set it to a dummy string.
                         */
                        ...
                        /* Change name to be based on clipboard menu name
                         * instead of "MENU_DUMMY"; Note that abobj_set_name()
                         * will also take care of updating the menu-item names.
                         */
 ...
 */
static int prop_menu_install_obj_destroy_CB() {
...
static int prop_menu_obj_destroy_CB(ObjEvDestroyInfo destroyInfo) {
        ...
        if (!obj_is_menu_item(doomedObj) && !obj_is_menu(doomedObj)) {
                return 0;
        }
        ...
        if (obj_is_menu(doomedObj) && obj_get_parent(doomedObj) != NULL) {
                 ...
                 * the menu being destroyed.  If so, set their menu-name
                 * attributes to NULL.
                 */
                for (trav_open(&trav, obj_get_module(doomedObj),
                               AB_TRAV_SALIENT);
                     (obj = trav_next(&trav)) != NULL;)
                        if (util_strcmp(obj_get_menu_name(obj),
                                        doomed_menu_name) == 0)
                ...
                for (i = 1; i < AB_PROP_TYPE_NUM_VALUES; i++) {
                        ...
                        /* Look to see if any of the clipboard menu item's refer
                         * to the menu being destroyed.  If so, set their
                         * menu-name attributes to NULL.
                         */
                        if (pms->clipboard_menu != NULL) {
                                for (trav_open(&trav, pms->clipboard_menu,
                                               AB_TRAV_ITEMS);
                                     (item = trav_next(&trav)) != NULL;)
                                        if (util_strcmp(obj_get_menu_name(item),
                                                        doomed_menu_name) == 0)
