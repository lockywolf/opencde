...
{
        XRectangle
            rect; /* win dimension. saved on entry, compaired with on exit */
} TestModeAnyDataRec;
 ...
 */
#define tmodeP_obj_data(obj) (obj->test_mode_data)

#define tmodeP_obj_has_data(obj) ((obj)->test_mode_data != NULL)

extern int tmodeP_obj_create_data(ABObj obj);
extern int tmodeP_obj_destroy_data(ABObj obj);
extern int tmodeP_obj_construct_data(ABObj obj);

/*
 * Geometry
 */
#define tmodeP_obj_geometry(obj) (tmodeP_obj_data(obj)->rect)

#define tmodeP_obj_set_geometry(obj, _rect) (tmodeP_obj_geometry(obj) = _rect)

#define tmodeP_obj_get_width(obj) (tmodeP_obj_data(obj)->rect.width)

#define tmodeP_obj_set_width(obj, _width)                                      \
        (tmodeP_obj_data(obj)->rect.width = _width)

#define tmodeP_obj_get_height(obj) (tmodeP_obj_data(obj)->rect.height)

#define tmodeP_obj_set_height(obj, _height)                                    \
        (tmodeP_obj_data(obj)->rect.height = _height)

#define tmodeP_obj_get_x(obj) (tmodeP_obj_data(obj)->rect.x)

#define tmodeP_obj_set_x(obj, _x) (tmodeP_obj_data(obj)->rect.x = _x)

#define tmodeP_obj_get_y(obj) (tmodeP_obj_data(obj)->rect.y)

#define tmodeP_obj_set_y(obj, _y) (tmodeP_obj_data(obj)->rect.y = _y)

/*
 * Window List manipulation
 */
extern int tmodeP_window_list_create(ABObj project);
extern int tmodeP_window_list_construct(ABObj project);
extern int tmodeP_window_list_destruct(void);
extern int tmodeP_window_list_destroy(void);
extern void tmodeP_window_list_iterate(ABObjListIterFn fn);
extern int tmodeP_window_list_add_handler(EventMask event_mask,
                                          Boolean nonmaskable,
                                          XtEventHandler event_handler);
extern int tmodeP_window_list_remove_handler(EventMask event_mask,
                                             Boolean nonmaskable,
                                             XtEventHandler event_handler);
