...
static void select_feedback(ABObj selObj, BOOL on);
static void select_footer(ABObj obj, BOOL on);
static void turnon_select_feedback(ABObj selObj, BOOL resizable);

static void turnoff_select_feedback(ABObj selObj, BOOL resizable);

static void draw_select_feedback(ABObj selObj, BOOL resizable);
/*
 * Event Handlers
 */
static void monitor_cursor(Widget w, XtPointer clientdata, XEvent *event,
                           Boolean *cont_dispatch);

static void select_feedback_redraw(Widget w, XtPointer clientdata,
...
void abobj_select(ABObj obj) {
        ...
        if (selObj == NULL) {
                if (util_get_verbosity() > 0)
                        fprintf(stderr,
                                "abobj_select: %s :no selection object\n",
        ...
        if (obj_is_window(rootObj))
                ...
                turnon_select_feedback(selObj,
                                       abobj_is_directly_resizable(rootObj));
}

/*
 * remove "select" feedback from object
 */
void abobj_deselect(ABObj obj) {
        AB_CONTAINER_TYPE cont_type;
        ABObj rootObj = obj_get_root(obj);
        ABObj selObj;
        ...
        if (selObj == NULL) {
                if (util_get_verbosity() > 0)
                        fprintf(stderr,
                                "abobj_deselect: %s :no selection object\n",
                                obj_get_name(rootObj));
                return;
        }
        ...
        if (obj_is_window(rootObj))
                ...
                turnoff_select_feedback(selObj,
                                        abobj_is_directly_resizable(rootObj));
}

void abobj_deselect_all(ABObj root) {
        ABObj obj = root;
        AB_TRAVERSAL trav;
 ...
 */
int abobj_get_selected(ABObj root, BOOL include_root, BOOL include_items,
                       ABSelectedRec *sel) {
        ABObj obj = root;
        AB_TRAVERSAL trav;
 ...
 */
void abobj_sort_sel_list(ABObj *sel_list, int sel_count, int sort) {
        int i, j;
        int x1, y1, x2, y2;
        ABObj tmp_obj;
...
static void select_feedback(ABObj selObj, BOOL on) {
        ...
        if (selWidget == NULL) {
                if (util_get_verbosity() > 0)
                        fprintf(stderr,
                                "select_feedback: %s :no selection widget\n",
                                obj_get_name(selObj));
                return;
        }
        ...
        if (on) {
                ...
                XtVaSetValues(selWidget, XmNborderWidth, (XtArgVal)2,
                              XmNborderColor, contrast, NULL);
        } else
                XtVaSetValues(selWidget, XmNborderWidth, (XtArgVal)0, NULL);
}

static void select_footer(ABObj selObj, BOOL on) {
        Widget selWidget;
        Pixel contrast;
        XmString xmstr;
        ...
        if (on) {
                ...
                XtVaSetValues(selWidget, XmNlabelString, xmstr, XmNforeground,
                              contrast, NULL);
                XmStringFree(xmstr);
        } else /* off */
        {
                xmstr = XmStringCreateLocalized("         ");
                XtVaSetValues(selWidget, XmNlabelString, xmstr, NULL);
                XmStringFree(xmstr);
        }
}
...
static void turnon_select_feedback(ABObj selObj, BOOL resizable) {
        ...
        XtAddEventHandler(selWidget, PointerMotionMask, FALSE, monitor_cursor,
                          (XtPointer)selObj);
}
...
static void turnoff_select_feedback(ABObj selObj, BOOL resizable) {
        ...
        XtRemoveEventHandler(selWidget, StructureNotifyMask | ExposureMask,
                             FALSE, select_feedback_redraw,
                             (XtPointer)resizable);
...
static void draw_select_feedback(ABObj selObj, BOOL resizable) {
        ...
        if ((obj_is_drawing_area(obj) || obj_is_text_pane(obj)) &&
            (obj_has_hscrollbar(obj) || obj_has_vscrollbar(obj))) {
                ...
                if (cwidget) {
                        XtVaGetValues(cwidget, XtNwidth, &width, XtNheight,
                                      &height, NULL);

                        XtVaGetValues(selWidget, XtNbackground, &bg, NULL);
                }
        }

        if (height == 0 || width == 0)
                XtVaGetValues(selWidget, XtNwidth, &width, XtNheight, &height,
        ...
        */
        ...
        XDrawRectangle(display, drawable, AB_grab_handle_gc, x0, y0,
        ...
        if (resizable) {
                ...
                XFillRectangle(display, drawable, AB_grab_handle_gc, x1, y1,
                               grabbox_size, grabbox_size);
                /* North */
                XFillRectangle(display, drawable, AB_grab_handle_gc,
                               (x0 + x1) / 2, y0, grabbox_size, grabbox_size);
                /* South */
                XFillRectangle(display, drawable, AB_grab_handle_gc,
                               (x0 + x1) / 2, y1, grabbox_size, grabbox_size);
                /* West */
                XFillRectangle(display, drawable, AB_grab_handle_gc, x0,
                               (y0 + y1) / 2, grabbox_size, grabbox_size);
                /* East */
                XFillRectangle(display, drawable, AB_grab_handle_gc, x1,
                               (y0 + y1) / 2, grabbox_size, grabbox_size);
        }
        XFlush(display);
}
 ...
 */
static void monitor_cursor(Widget selWidget, XtPointer clientdata,
                           XEvent *event, Boolean *cont_dispatch) {
        ...
        if (dir == NONE) {
                ...
                if ((resize_cursor =
                         abobjP_get_resize_cursor(selWidget, dir)) != NULL) {
 ...
 */
static void select_feedback_redraw(Widget selWidget, XtPointer clientdata,
                                   XEvent *event, Boolean *cont_dispatch) {
        ABObj selObj = NULL;
