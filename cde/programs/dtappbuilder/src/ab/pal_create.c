...
**                                                                      **
**************************************************************************/
static void palitem_update_type(Widget w, XtPointer cd, XEvent *ev,
                                Boolean *cont);
static void palitem_select_event(Widget w, XtPointer cd, XEvent *ev,
                                 Boolean *cont);
static void palitem_drag_action(Widget w, int subtype, XEvent *event);
static void palitem_release_event(Widget w, XtPointer cd, XEvent *ev,
                                  Boolean *cont);
static void create_obj_action(Widget w, int subtype, XEvent *event);
static char *locate_obj_parent(ABObj obj, Widget widget, int x, int y,
                               ABObj *parentptr, BOOL *ModuleCreated);
static Boolean initiate_drag(Widget w, int subtype, int *x, int *y, int *xhot,
                             int *yhot);
static XImage *build_item_image(Widget w);
static void image_copy(Widget w, XImage *from, XImage *to, int width, int gx,
                       int px);
ABObj locate_obj_at_rootxy(int x, int y, int *p_wx, int *p_wy);
 ...
 */
void pal_enable_item_drag(Widget widget, int subtype) {
        static int st_subtype;

        st_subtype = subtype;
        XtInsertEventHandler(widget, ButtonPressMask, FALSE,
                             palitem_select_event, (XtPointer)&st_subtype,
                             XtListHead);
        XtInsertEventHandler(widget, ButtonReleaseMask, FALSE,
                             palitem_release_event, (XtPointer)&st_subtype,
                             XtListHead);
        ...
        if (!palette_widget) {
                if ((palette_widget =
                         dtb_palette_ab_palette_main.palette_cpanel) == NULL) {
                        if (util_get_verbosity() > 2)
                                fprintf(stderr,
                                        "pal_enable_item_drag: couldn't find "
                                        "\"%s\"\n",
                                        XtName(dtb_palette_ab_palette_main
                                                   .palette_cpanel));
                        palette_widget = XtParent(XtParent(widget));
                }
        }
}
static void palitem_update_type(Widget w, XtPointer clientData, XEvent *event,
 ...
 */
static void palitem_select_event(Widget w, XtPointer clientData, XEvent *event,
                                 Boolean *cont_dispatch) {
        XButtonEvent *bevent;
        int subtype = *((int *)clientData);
 ...
 */
static void palitem_drag_action(Widget widget, int subtype, XEvent *event) {
        int x, y;
        ...
        if (event->type == ButtonPress) {
                ...
                if (!initiate_drag(widget, subtype, &x, &y, &drag_cursor_xhot,
                                   &drag_cursor_yhot)) {
                        drag_on = FALSE;
                        if (util_get_verbosity() > 2)
                                fprintf(stderr, "palitem_drag_action: couldn't "
                                                "begin drag\n");
                } else
                        drag_on = TRUE;
        }
 ...
 */
static void palitem_release_event(Widget w, XtPointer clientData, XEvent *event,
                                  Boolean *cont_dispatch) {
        XButtonEvent *bevent;
        int subtype = *((int *)clientData);
 ...
 */
static void create_obj_action(Widget widget, int subtype, XEvent *event) {
        ...
        if (event->type == ButtonRelease) {
                ...
                if (errmsg = locate_obj_parent(obj, widget, bevent->x_root,
                                               bevent->y_root, &obj_parent,
                                               &ModuleCreated)) {
                        obj_destroy(obj);
                        if (!util_streq(errmsg, "")) {
                                xm_buf = XmStringCreateLocalized(errmsg);
                                dtb_palette_error_msg_initialize(
                                    &dtb_palette_error_msg);
                        ...
                        if (pal_initialize_obj(obj) == ERROR)
                                fprintf(stderr, "create_obj_action: couldn't "
                                                "initialize object\n");
                        else if (abobj_show_tree(obj, TRUE) == -1)
                                fprintf(stderr, "create_obj_action: couldn't "
                                                "show object\n");
                        else {
                                /* set the initial visiblity of new layers to
                                 * false */
                                if (obj_is_layers(obj_parent))
                                        obj_set_is_initially_visible(obj,
                                                                     FALSE);

                                /* Deselect any objects that happen to be
                                 * selected */
                                ...
                                abobj_set_save_needed(obj_get_module(obj),
                                                      TRUE);

                                /* If the window is the first one dragged out
                                 * and its parent (the module) has not been
                                 * named yet, popup the module name dialog.  The
                                 * only time a module will have not been named
                                 * by the user is when it is created at the time
                                 * the first window is dragged from the palette.
                                 */
                                if (obj_is_window(obj) && ModuleCreated) {
                                        /* Pop up the name dialog */
                                        proj_show_name_dlg(
                                            obj_parent, objxm_get_widget(obj));
                                }

                                /*
                                 * Once an object is successfully dragged out
                                 * from the palette, disable undo. Object
                                 * creation is not undo-able.
                                 */
                                abobj_cancel_undo();
                        }
                }
...
static Boolean initiate_drag(Widget w, int subtype, int *x, int *y, int *xhot,
        ...
        if (max_c_width == 0) /* Need to Query server's Cursor constraints */
        {
                ...
                if (strcmp(server_vendor, "Sun Microsystems, Inc.") == 0) {
                        /*
                         * On a Sun X server, a call to XQueryBestSize() will
                         * return a limit based on a *hardware* cursor;  this is
                         * misleading because the server actually supports a
                         * much larger software cursor (only limited by the
                         * screen size); therefore set the limit based on screen
                         * size.
                         */
                        ...
                        XQueryBestSize(XtDisplay(w), CursorShape, XtWindow(w),
                                       1000, 1000, &max_c_width, &max_c_height);
        ...
        XtVaGetValues(w, XmNuserData, &palitem, XtNwidth, &(w_rect.width),
                      XtNheight, &(w_rect.height), XtNx, &(w_rect.x), XtNy,
                      &(w_rect.y), NULL);
        /*
            ...
            (palitem->type == AB_TYPE_SCALE &&
             !XtIsSubclass(w, xmScaleWidgetClass))) {
                ...
                *y += w_rect.y;
                ...
                XtVaGetValues(item_widget, XmNuserData, (XtArgVal)&palitem,
                              XtNwidth, (XtArgVal) & (w_rect.width), XtNheight,
                              (XtArgVal) & (w_rect.height), NULL);
        }
        ...
        if (!p_gc) /* First-time thru: Create Cursor pixmaps & GC */
        {
                ...
                p_gc = XCreateGC(dpy, item_pixmap, GCForeground | GCBackground,
                                 &values);
        }
        ...
        if (palitem_subptr == NULL) {
                util_dprintf(0, "initiate_drag: could not find palette object "
                                "subtype info\n");
                return FALSE;
        }
        ...
        if (!palitem_subptr->pixmap) {
                ...
                palitem_subptr->pixmap =
                    XCreatePixmap(dpy, win, palitem_subptr->pmwidth,
        ...
        x_graphics_op(dpy, item_pixmap, p_gc, *xhot, *yhot, cc_width, cc_height,
                      AB_cp_cursor_pixmap);
        ...
        drag_cursor = x_create_stencil_cursor(item_widget, item_pixmap, c_width,
                                              c_height, *xhot, *yhot);

        if (XGrabPointer(dpy, win, FALSE, ButtonReleaseMask | PointerMotionMask,
 ...
 */
static XImage *build_item_image(Widget widget) {
        XRectangle w_rect;
        Window win;
        Display *dpy;
        XImage *get_image, *new_image;
         ...
         */
        get_image = XGetImage(dpy, win, 0, 0, w_rect.width, w_rect.height,
                              AllPlanes, XYPixmap);
         ...
         */
        new_image_data =
            (char *)XtMalloc((w_rect.width * w_rect.height) * sizeof(char));
        new_image =
            XCreateImage(dpy, attr.visual, 1, XYBitmap, 0, new_image_data,
                         w_rect.width, w_rect.height, 8, 0);
 ...
 */
static void image_copy(Widget widget, XImage *from_image, XImage *to_image,
                       int image_width, int get_x, int put_x) {
         ...
         */
        XtVaGetValues(widget, XtNbackground, &background, XtNforeground,
                      &foreground, XmNbottomShadowColor, &bshadow,
                      XmNtopShadowColor, &tshadow, NULL);
        ...
        for (i = 0; i < from_image->height; i++) {
                for (j = get_x, k = put_x; j < image_width; j++, k++) {
                        ...
                        if ((pix == foreground || pix == bshadow ||
                             pix == tshadow || pix == black || pix == white) &&
                            pix != background)
                ...
                if (obj_is_menubar(obj)) {
                        ...
                        if (obj_parent != NULL) {
                                ...
                                if (!obj_is_base_win(winobj))
                                        obj_parent = NULL;
                                else /* Check to make sure there isn't already a
                                      ...
                                      */
                                {
                                        ...
                                        obj_parent = objxm_comp_get_subobj(
                                            winobj, AB_CFG_MENU_PARENT_OBJ);
                                }
                        }
                        if (obj_parent == NULL) {
                                i18n_msg = catgets(Dtb_project_catd, 100, 15,
                                                   "Menubars must be dropped "
                                                   "on a Main Window.");

                                /* If we have an old buffer lying around, free
                                 * it */
                                if (errmsg != (STRING)NULL)
                                        util_free(errmsg);

                                errmsg =
                                    (STRING)util_malloc(strlen(i18n_msg) + 1);
                        ...
                        if (obj_parent == NULL ||
                            (!(obj_is_container(obj_parent) &&
                               ...
                               !obj_is_file_chooser(obj_parent)))) {
                                i18n_msg =
                                    catgets(Dtb_project_catd, 100, 12,
                                            "Containers must be dropped on\na "
                                            "Main Window or Custom Dialog.");
                                /* If we have an old buffer lying around, free
                                 * it */
                                if (errmsg != (STRING)NULL)
                                        util_free(errmsg);
                        ...
                        if (obj_parent != NULL &&
                            (obj_is_pane(obj_parent) &&
                             !obj_is_window(obj_get_root(obj_parent)))) {
                                ...
                                grandparent =
                                    obj_get_parent(obj_get_root(obj_parent));
                                great_grandparent = obj_get_parent(grandparent);

                                /* The child is a textpane, termpane, or drawing
                                 * area and the parent (object being dropped
                                 * upon) is a control pane that is not part of a
                                 * panedWindow.
                                 */
                                if (!obj_is_control_panel(obj) &&
                                    obj_is_control_panel(obj_parent) &&
                                    !obj_is_paned_win(grandparent)) {
                                        /* If the parent of obj_parent is a
                                         * layered pane then we have to check if
                                         * the layered pane is a child of a
                                         * panedwindow. If so, then we should
                                         * ask the user whether he wants to make
                                         * the pane a child of the control pane,
                                         * a new layer, or a new pane in the
                                         * paned window.
                                         */
                                        if (obj_is_layers(grandparent) &&
                                            obj_is_paned_win(
                                                ...
                                                answer = dtb_show_modal_message(
                                                    (Widget)
                                                        obj_parent->ui_handle,
                                                    &dtb_palette_chld_layr_pw_msg,
                                                    NULL, NULL, NULL);
                                                switch (answer) {
                                                /* Create as a child of the
                                                 * control pane */
                                                ...
                                                answer = dtb_show_modal_message(
                                                    ...
                                                    NULL, NULL, NULL);
                                                switch (answer) {
                                                case DTB_ANSWER_ACTION1: /* As
                                                                            Child
                                                                          */
                                                        break;

                                                case DTB_ANSWER_ACTION2: /* As
                                                                            Layered
                                                                            Pane
                                                                          */
                                                        obj_parent =
                                 ...
                                 */
                                        ...
                                        /* If Pane was dropped on a Pane which
                                         * is a NORMAL child of a Control Pane,
                                         * then use the Control Pane as the
                                         * actual parent instead of the pane.
                                         */
                                        if (obj_is_control_panel(
                                                obj_get_root(grandparent))) {
                                                obj_parent = grandparent;
                                        }

                                        /* Check if the grandparent is a layered
                                         * pane. If so, then we have to check it
                                         * is part of a panedwindow. If it is,
                                         * then we need to ask the user whether
                                         * to create the pane as a layered pane
                                         * or a new pane in the paned window.
                                         */
                                        if (obj_is_layers(grandparent) &&
                                            obj_is_paned_win(
                                                great_grandparent)) {
                                                ...
                                                answer = dtb_show_modal_message(
                                                    ...
                                                    NULL, NULL, NULL);
                                                switch (answer) {
                                                case DTB_ANSWER_ACTION1: /* Layered
                                                                            Pane
                                                                          */
                                                        obj_parent =
                                                            abobj_handle_layered_pane(
                                                ...
                                                case DTB_ANSWER_ACTION2: /* PanedWindow
                                                                            Pane
                                                                          */
                                                        obj_parent =
                                                            grandparent;
                                                ...
                                                answer = dtb_show_modal_message(
                                                    ...
                                                    NULL, NULL, NULL);
                                                switch (answer) {
                                                case DTB_ANSWER_ACTION2: /* Layered
                                                                            Pane
                                                                          */
                                                        obj_parent =
                                                            abobj_handle_layered_pane(
                                ...
                                /* A termpane, textpane, or draw area is being
                                 * dropped on another pane (not control pane)
                                 * and that pane is inside a paned window.
                                 *
                                 * OR
                                 *
                                 * The object being dragged is a control pane
                                 * and the parent is part of a panedWindow.
                                 */
                                 ...
                                 * and the parent of obj_parent is a
                                 * panedWindow.
                                 */
                                else {
                                        dtb_palette_chld_layr_pw_msg_initialize(
                                        ...
                                        answer = dtb_show_modal_message(
                                            (Widget)obj_parent->ui_handle,
                                            &dtb_palette_chld_layr_pw_msg, NULL,
                                            NULL, NULL);
                                        switch (answer) {
                                        ...
                                        /* Create as a new pane in the paned
                                         * window */
                                        case DTB_ANSWER_ACTION3:
                                                obj_parent =
                                                    obj_get_parent(obj_parent);
                                                break;
                         ...
                         */
                                 ...
                                 obj_is_group(obj_parent)) {
                                if (obj_is_control_panel(obj)) {
                                        i18n_msg = catgets(
                                            Dtb_project_catd, 100, 59,
                                            "Control Panes must be dropped on "
                                            "a Main Window,\nCustom Dialog, or "
                                            "another pane.");

                                        /* If we have an old buffer lying
                                         * around, free it */
                                ...
                                i18n_msg = catgets(Dtb_project_catd, 100, 13,
                                                   "Panes must be dropped on a "
                                                   "Main Window,\nCustom "
                                                   "Dialog, or another pane.");
                        ...
                        if (obj_parent == NULL ||
                            (!obj_is_control_panel(obj_get_root(obj_parent)) &&
                             !obj_is_group(obj_get_root(obj_parent)))) {
                                i18n_msg =
                                    catgets(Dtb_project_catd, 100, 14,
                                            "Controls must be dropped on\na "
                                            "Control Pane or Group.");
                                /* If we have an old buffer lying around, free
                                 * it */
                                if (errmsg != (STRING)NULL)
                                        util_free(errmsg);
...
ABObj locate_obj_at_rootxy(int x, int y, int *p_wx, int *p_wy) {
        ...
        for (trav_open(&trav, obj, AB_TRAV_UI);
             (obj = trav_next(&trav)) != NULL;) {
                ...
                if (widget != NULL && XtIsRealized(widget) &&
        ...
        if (aob_is_browser_win(project, window)) {
                 ...
                 */
                browser_obj =
                    aob_get_object_from_xy(project, window, *p_wx, *p_wy);
                ...
                if (browser_obj) {
                        ...
                        *p_wy = AB_DEFAULT_Y + drag_cursor_yhot;
                        return (objxm_comp_get_subobj(browser_obj,
                                                      AB_CFG_PARENT_OBJ));
                }
        }
