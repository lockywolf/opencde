...
static STRING dtcodegenCmdList[] = {
    ...
    "dtcodegen", "/usr/dt/bin/dtcodegen", NULL};

static STRING makeCmdList[] = {"make", "/usr/ccs/bin/make",
                               "/usr/dist/exe/make", "/usr/dist/local/exe/make",
                               NULL};
...
static int term_execute_command(CG_SUBCOMMAND cmd_code, STRING cmd,
                                STRING argv[]);
...
static int write_to_status_pipe(CG_STATUS status_code, CG_SUBCOMMAND cmd_code,
                                void *status_data);
static int read_from_status_pipe(CG_STATUS *status_code_out,
...
static int do_user_action(CG_GOAL goal, CG_SUBCOMMAND cmd);
...
static int add_obj_file_name(StringList fileNames, ABObj obj, STRING suffix);
...
static STRING cvt_type_to_ident(STRING type, STRING identBuf, int identBufSize);
static Boolean path_is_executable(char *path, uid_t euid, gid_t egid);

static void popdown_cgen_window(Widget widget, XtPointer client_data,
                                XtPointer call_data);

static void pipe_data_ready_proc(XtPointer client_data, int *fid,
                                 XtInputId *id);
...
#define util_fdclose(fd)                                                       \
        ((fd) < 0 ? 0 : ((close(fd) == 0) ? (((fd) = -1), 0) : -1))

/*
 * Exit cmd_code and exit_code are the command that was previously run
 * and its exit status.
 */
static int exec_next_command(CG_SUBCOMMAND cmd_code, int exit_code);
static int exec_next_command_for_gen_code(CG_SUBCOMMAND cmd_code,
                                          int exit_code);
static int exec_next_command_for_make(CG_SUBCOMMAND cmd_code, int exit_code);
static int exec_next_command_for_run(CG_SUBCOMMAND cmd_code, int exit_code);
static int exec_next_command_for_build_and_run(CG_SUBCOMMAND cmd_code,
                                               int exit_code);
...
void cgen_show_codegen_win(void) {
        ...
        if (AB_cgen_win == (Widget)NULL) {
                ...
                dtb_cgen_win_mainwindow_initialize(&dtb_cgen_win_mainwindow,
                ...
                make_run_item =
                    dtb_cgen_win_mainwindow
                        .menubar_File_item_file_pulldown_items.Make_Run_item;
                gen_code_item = dtb_cgen_win_mainwindow
                                    .menubar_File_item_file_pulldown_items
                                    .Generate_Code_item;
                make_item =
                    dtb_cgen_win_mainwindow
                        .menubar_File_item_file_pulldown_items.Make_item;
                run_item = dtb_cgen_win_mainwindow
                               .menubar_File_item_file_pulldown_items.Run_item;
                abort_item =
                    dtb_cgen_win_mainwindow
                        .menubar_File_item_file_pulldown_items.Abort_item;
                cgen_props_item =
                    dtb_cgen_win_mainwindow
                        .menubar_Options_item_options_pulldown_items
                        .Generator_item;
                ...
                /*
                 * Make sure the application being run can find its resource
                 * file By setting this via cgenP_put_env_var(), the user has
                 * the option of resetting it to its original value.
                 */
        ...
        DtTermDisplaySend(output_termWidget, (unsigned char *)"[?25l",
                          strlen("[?25l"));
}
...
int cgen_notify_new_project(ABObj project) {
...
int cgen_notify_new_directory(STRING dir) {
        public_entry_point();
...
static void cgen_abort_at_exit(void) { cgen_abort(); }

int cgen_notify_props_new_proj(ABObj project) {
        int ret = 0;

        ret =
            set_props_proj_name(project == NULL ? NULL : obj_get_name(project));
        return ret;
}

void cgenP_init_props_module_list(Widget mod_list) {
...
void cgen_gen_code(CG_SUBCOMMAND cmd) { do_user_action(CG_GOAL_GEN_CODE, cmd); }

void cgen_make(CG_SUBCOMMAND cmd) { do_user_action(CG_GOAL_MAKE, cmd); }

void cgen_run(CG_SUBCOMMAND cmd) { do_user_action(CG_GOAL_RUN, cmd); }

void cgen_make_run(CG_SUBCOMMAND cmd) {
 ...
 */
static int send_output_to_term() {
        ...
        if (watchdog_pid == (pid_t)-1) {
                ...
                if (actual_process_pid == INVALID_PID) {
                        ...
                        fprintf(stderr, "Could not create subprocess: %s\n",
                                strerror(errno));
                        write_to_status_pipe(CG_STATUS_ERROR, cmd_code,
                                             (void *)0);
                        subprocess_exit(1);
                } else if (actual_process_pid == 0) {
                        /* grandchild - becomes the program we are actually
                         * running */
                        ...
                        write_to_status_pipe(CG_STATUS_STARTED, cmd_code,
                                             (void *)getpgrp());

                        for (i = 0; i < num_env_vars; ++i) {
                                var_value = NULL;
                                var_name = strlist_get_str(user_env_vars, i,
                                                           (void **)&var_value);
                                if (!util_strempty(var_value)) {
                                        putenv_var_size = strlen(var_name) +
                                                          strlen(var_value) + 2;
                                        /* +2 for "=" and NULL */
                                        putenv_var = (STRING)util_malloc(
                                            putenv_var_size);
                                        sprintf(putenv_var, "%s=%s", var_name,
                                                var_value);
                                } else {
                        ...
                        rc_pid =
                            waitpid(actual_process_pid, &grandchild_status, 0);
                        /*printf("frontline: child done!\n");*/
                        if (rc_pid == INVALID_PID) {
                                /* damn! an error occured... */
                                /*printf("frontline: error waiting for child!
                        ...
                        if (status_pipe_write >= 0) {
                                write_to_status_pipe(status_code, cmd_code,
                                                     (void *)exit_code);
                                util_fdclose(status_pipe_write);
                        }
 ...
 */
static int get_slave_device_name() {
        ...
        if (termSlaveDeviceName == NULL) {
                XtVaGetValues(output_termWidget, DtNtermSlaveName, &deviceName,
                              NULL);
                if (deviceName != NULL) {
...
static int exec_generate_specific_files() {
        int returnValue = 0;
        int rc = 0; /* return code */
        ABObj project = proj_get_project();
        STRING project_name = NULL;
        ...
        argv[arg_count] = "-p";
        arg_count++;
        argv[arg_count] = project_name;
        arg_count++;

        num_args = arg_count + num_mods;
        for (i = arg_count, n = 0; i < num_args; i++, n++) {
                argv[i] = strlist_get_str(CodeGenOptions.module_list, n,
                                          (void **)NULL);
        }
        argv[num_args] = NULL;
...
static int exec_generate_specific_files_and_main() {
        int returnValue = 0;
        int rc = 0; /* return code */
        ABObj project = proj_get_project();
        STRING project_name = NULL;
...
static int exec_next_command(CG_SUBCOMMAND cmd_code, int exit_code) {
        ...
        case CG_GOAL_GEN_CODE:
                return_value =
                    exec_next_command_for_gen_code(cmd_code, exit_code);
                break;
        ...
        case CG_GOAL_MAKE_AND_RUN:
                return_value =
                    exec_next_command_for_build_and_run(cmd_code, exit_code);
                break;
...
static int exec_first_build_and_run_command() {
        int return_value = 0;
...
static int wait_for_child(void) {
        ...
        for (i = 0; i < XtNumber(child_pid_list); ++i) {
                ...
                if (((rcPid = waitpid(childPid, (int *)0, WNOHANG)) ==
                     childPid) ||
                    (kill(childPid, 0) == -1)) {
                        child_pid_list[i] = INVALID_PID;
                        ++numExited;
...
static int add_child_to_list(pid_t childPid) {
        int i = 0;
        BOOL added = FALSE;
        int attemptCount = 0;
...
static int print_internal_err_message() {
...
static int print_success_message() {
...
static int print_death_message(void) {
        char msg[256];
        sprintf(msg, "****> Program died a horrible, unnatural death, due to "
                     "an uncaught signal\n");
        print_to_term(msg);
        user_goal = CG_GOAL_UNDEF;
...
static int print_exit_message(int exitCode) {
        char msg[256];
...
static int print_abort_message() {
        print_to_term("\n====> Command aborted.\n");
        user_goal = CG_GOAL_UNDEF;
        goto_ready_state();
        return 0;
...
static int create_status_pipe(void) {
        ...
        if ((status_pipe_read < 0) || (status_pipe_write < 0)) {
                ...
                if (pipe(fds) == 0) {
                        pipeOpen = TRUE;
                        status_pipe_read = fds[0];
                        fds[0] = -1;
                        status_pipe_write = fds[1];
                        fds[1] = -1;
                }

                if ((input_proc_id == -1) && (status_pipe_read >= 0) &&
                    (output_termWidget != NULL)) {
                        XtAppContext app_context =
                            XtWidgetToApplicationContext(output_termWidget);
                        input_proc_id =
                            XtAppAddInput(app_context, status_pipe_read,
                                          (XtPointer)XtInputReadMask,
                                          pipe_data_ready_proc, NULL);
                }
        }
...
static int careful_kill_group(pid_t pgid) {
        ...
        if (!killed) {
                ...
                for (waitcount = 0; (!killed) && (waitcount < 5); ++waitcount) {
                        if (kill(kill_pgrp_id, 0) ==
                            -1) /* sig 0 checks pid only */
                        {
                                /* kill failed, so group doesn't exist, any more
                                 */
                                killed = TRUE;
...
static int cgen_set_title(STRING projectName) {
        ...
        XtVaSetValues(XtParent(AB_cgen_win), XmNtitle, newTitle, NULL);
...
static int cgen_set_project_dir(STRING dir) {
        ...
        XtVaSetValues(cur_dir_text, XmNlabelString, xmlabel, NULL);
 ...
 * projectName may be NULL (signifies no project)
 */
static int set_props_proj_name(STRING projectName) {
        char newProj[256];
        ...
        XtVaSetValues(dtb_cgen_props_cgen_props_dlg.proj_name, XtVaTypedArg,
                      XmNlabelString, XtRString, newProj, strlen(newProj) + 1,
                      NULL);
        ...
        if (AB_cgen_prop_dialog != NULL) {
                ...
                if ((obj_is_project(info->obj)) &&
                    ((info->atts & OBJEV_ATT_NAME) != 0)) {
                        /* the project's name changed.  Update prop sheet
                         * proj_name */
                        ...
                        XtVaGetValues(AB_cgen_prop_dialog, XmNuserData, &list,
                                      NULL);

                        /* A new module was created and named */
                        if (info->old_name != NULL) {
                                ui_list_replace_item(
                                    list, istr_string(info->old_name),
                                    mod_name);
                        }
                }
        }
...
static int obj_updateOCB(ObjEvUpdateInfo info) {
        ...
        if (AB_cgen_prop_dialog != NULL) {
                ...
                if (CodeGenOptions.module_list !=
                    NULL) /* the list that is applied */
                {
...
int cgenP_sync_up_dir() {
        ...
        if (!util_strempty(request_dir)) {
                ...
                strcat(cmd, "\n");
                DtTermSubprocSend(input_termWidget, (unsigned char *)cmd,
                                  strlen(cmd));
        }
        return 0;
}
...
static int do_user_action(CG_GOAL goal, CG_SUBCOMMAND cmd) {
        ...
        if ((goal != CG_GOAL_RUN) && proj_check_unsaved_edits(project)) {
                ...
                if (all_files_exist(project)) {
                        ...
                        answer = dtb_show_modal_message(
                            AB_cgen_win,
                            &dtb_cgen_win_query_save_or_gen_old_msg, NULL, NULL,
                            &dlg);
                        switch (answer) {
                        ...
                        answer = dtb_show_modal_message(
                            AB_cgen_win, &dtb_cgen_win_query_save_or_abort_msg,
                            NULL, NULL, &dlg);
                        switch (answer) {
                        case DTB_ANSWER_ACTION1: /* save */
                                doSave = TRUE;
...
static int check_path() {
        ...
        if (keepAsking) {
                ...
                static STRING ccCmdList[] = {"cc", "/opt/SUNWspro/bin/cc",
                                             "/usr/dist/exe/cc",
                                             "/usr/dist/local/exe/cc", NULL};
                if ((rc = check_path_to_cmd(ccCmdList, &allowWarnUserAboutCc)) <
                    0) {
                        keepAsking = FALSE;
                }
        }
        ...
        if (keepAsking) {
                ...
                static STRING shCmdList[] = {
                    ...
                    "/sbin/sh", "/usr/dist/exe/sh", "/usr/dist/local/exe/sh",
                    NULL};
                if ((rc = check_path_to_cmd(shCmdList, &allowWarnUserAboutSh)) <
                    0) {
                        keepAsking = FALSE;
                }
        }
        ...
        if (keepAsking) {
                ...
                static STRING rmCmdList[] = {"rm", "/bin/rm", "/usr/bin/rm",
                                             NULL};
                if ((rc = check_path_to_cmd(rmCmdList, &allowWarnUserAboutRm)) <
                    0) {
                        keepAsking = FALSE;
                }
        }
                ...
                if (!util_streq(foundCmd, nopathCmd)) {
                        ...
                        *buffer = 0;
                        ...
                        if (slashPtr != NULL) {
                                ...
                                sprintf(buffer,
                                        catgets(Dtb_project_catd, 100, 52,
                                                "Your PATH does not contain "
                                                "the command %s.\n"
                                                "In order to access this "
                                                "command, may I append this\n"
                                ...
                                answer = dtb_show_modal_message(
                                    AB_cgen_win, &dtb_cgen_win_modify_path_msg,
                                    xmMsg, NULL, &dlg);
                                XmStringFree(xmMsg);
                                xmMsg = NULL;
...
static int cgenP_put_env_var(STRING varName, STRING varValue) {
        ...
        if (strIndex >= 0) {
                ...
                strlist_remove_index(user_env_vars, strIndex);
                strIndex = -1;
        }
...
static int check_makefile(BOOL *continueOutPtr) {
        ...
        *continueOutPtr = TRUE;
         ...
         */
        if (!makefileExists) {
                rc = cgenP_makefile_is_for_project("Makefile",
                                                   proj_get_project());
                if (rc >= 0) {
                        makefileExists = makefileIsOK = TRUE;
                } else if (rc == ERR_OPEN) {
         ...
         */
        if (!makefileExists) {
                dtb_cgen_win_no_makefile_msg_initialize(
                ...
                answer = dtb_show_modal_message(
                    AB_cgen_win, &dtb_cgen_win_wrong_makefile_msg, NULL, NULL,
                    &dlg);
                switch (answer) {
                case DTB_ANSWER_ACTION1: /* Yes */
...
static int move_file_to_backup(STRING fileName) {
        ...
        if ((fileNameLen >= 4) &&
            (strcmp(fileName + fileNameLen - 4, ".BAK") == 0)) {
...
#define fast_strneq(s1, s2, n)                                                 \
        ...
        *buf = 0;
        ...
        /*
         * Convert strings list to arrays, to avoid 750,000 calls to
         * istr_string()
         */
        ...
        if ((stringsArray == NULL) || (stringsLenArray == NULL) ||
            (stringExists == NULL)) {
                goto epilogue;
        }
        ...
        while ((c = fgetc(file)) != EOF) {
                if (bufLen >= maxBufLen) {
                        ...
                        memmove(buf, buf + maxBufLen - maxFileNameLen,
                                maxFileNameLen);
                ...
                for (i = 0; i < numStrings; ++i) {
                        ...
                        if (fast_strneq(stringStart, stringsArray[i],
                                        stringsLenArray[i])) {
                                stringExists[i] = TRUE;
                                if (++numStringsFound >= numStrings) {
                                        /*
                                         * Instead of a loop-control variable
                                         * that must get checked on each
                                         * iteration, we use a goto to speed
...
static int select_command(STRING *cmdList, STRING *cmdOutPtr) {
        int i = 0;
        int cmdIndex = -1;
        STRING path = NULL;
 ...
 * other file.
 */
static STRING cvt_type_to_ident(STRING type, STRING identBuf,
                                int identBufSize) {
        int typeOff = 0;
        int identOff = 0;
        int typeChar = -1;
        ...
        for (typeOff = 0; (typeOff < typeLen) && (identOff < (identMaxLen - 1));
             ++typeOff) {
                typeChar = type[typeOff];
                if (isupper(typeChar)) {
                        if ((lastIdentChar != '_') && (!lastTypeCharWasUpper) &&
                            (lastIdentChar != -1)) {
                                lastIdentChar = identBuf[identOff++] = '_';
                        }
                        if (identOff < (identMaxLen - 1)) {
                                lastIdentChar = identBuf[identOff++] =
                                    tolower(typeChar);
                        }
