 ...
 * Methods
 */
static int fchooser_initialize(ABObj obj);
static Widget fchooser_prop_init(Widget parent, AB_PROP_TYPE type);
static int fchooser_prop_activate(AB_PROP_TYPE type, BOOL active);
static int fchooser_prop_clear(AB_PROP_TYPE type);
static int fchooser_prop_load(ABObj obj, AB_PROP_TYPE type,
                              unsigned long loadkey);
static int fchooser_prop_apply(AB_PROP_TYPE type);
static BOOL fchooser_prop_pending(AB_PROP_TYPE type);
...
static int fchooser_initialize(ABObj obj) {
        ...
        obj_set_label(
            obj, catgets(Dtb_project_catd, 100, 257, "File Selection Dialog"));
...
static Widget fchooser_prop_init(Widget parent, AB_PROP_TYPE type) {
        DtbFchooserPropDialogInfoRec
            rev_fchooser_prop_dialog; /* Revolving Props */
        DtbFchooserPropDialogInfo cgen =
            &dtb_fchooser_prop_dialog; /* Codegen structure */
        ...
        if (dtb_fchooser_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_fchooser_palitem, cgen->ok_button,
                            cgen->apply_button, cgen->reset_button,
                            cgen->cancel_button, cgen->help_button);
                }

                /* Alternate Editor Buttons */
                prop_editors_panel_init(type, ab_fchooser_palitem, NULL,
                                        cgen->conn_button,
                                        cgen->help_txt_button);
                ...
                item_val[n] = AB_FILE_REGULAR;
                n++;
                item[n] = cgen->pattern_type_rbox_items.Directories_item;
                item_val[n] = AB_FILE_DIRECTORY;
                n++;
                item[n] = cgen->pattern_type_rbox_items.Both_item;
                item_val[n] = AB_FILE_ANY;
                n++;
                prop_radiobox_init(
                    &(pcs->pattern_type), cgen->pattern_type_rbox_label,
                ...
                n = 0;
                item[n] =
                    cgen->popdown_checkbox_items.AutomaticallyDismiss_item;
                item_val[n] = DISMISS_ITEM_KEY;
                n++;
                prop_checkbox_init(&(pcs->auto_dismiss),
                                   cgen->popdown_checkbox_label,
                                   cgen->popdown_checkbox, n, item, item_val,
                                   cgen->popdown_cb);
...
static int fchooser_prop_clear(AB_PROP_TYPE type) {
        ...
        prop_checkbox_set_value(&(pcs->auto_dismiss), DISMISS_ITEM_KEY, True,
                                False);
...
static int fchooser_prop_load(ABObj obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_field_set_value(&(pcs->init_dir), obj_get_directory(obj),
                                     False);

                /* Load Search Pattern */
                prop_field_set_value(&(pcs->pattern),
                                     obj_get_filter_pattern(obj), False);

                /* Load Ok Button Label */
                prop_field_set_value(&(pcs->ok_label), obj_get_ok_label(obj),
                                     False);

                /* Load Search Pattern Type */
                prop_radiobox_set_value(&(pcs->pattern_type),
                                        (XtPointer)obj_get_file_type_mask(obj),
                                        False);
...
int fchooser_prop_apply(AB_PROP_TYPE type) {
        PropFchooserSettingsRec *pcs = &(prop_fchooser_settings_rec[type]);
        STRING value;
        BOOL reset_bg = False;
        BOOL reset_fg = False;
        ...
        if (prop_changed(pcs->win_parent.changebar)) {
                ...
                if (util_strcmp(value, "") == 0) {
                        ...
                        abobj_set_win_parent(
                            pcs->current_obj,
                            obj_scoped_find_by_name(proj_get_project(),
                                                    (STRING)value));
                        util_free(value);

                        /* Ensure that the object is displayed in the form
                         * "module :: obj" */
                        value = abobj_get_moduled_name(
                            obj_get_win_parent(pcs->current_obj));
        ...
        if (prop_changed(pcs->pattern_type.changebar)) {
                abobj_set_pattern_type(
                    pcs->current_obj,
                    (AB_FILE_TYPE_MASK)prop_radiobox_get_value(
                        &(pcs->pattern_type)));
        }
        ...
        if (prop_changed(pcs->auto_dismiss.changebar)) {
                abobj_set_auto_dismiss(
                    pcs->current_obj,
                    prop_checkbox_get_value(&(pcs->auto_dismiss),
                                            DISMISS_ITEM_KEY));
        }
...
static BOOL fchooser_prop_pending(AB_PROP_TYPE type) {
        return (prop_changebars_pending(
            prop_fchooser_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pcs->win_parent.changebar) &&
            !prop_obj_name_ok(pcs->win_parent.field, proj_get_project(),
                              AB_TYPE_BASE_WINDOW, "Main Window"))
                return False;
