...
**                                                                          **
*****************************************************************************/
static ABObjList tmodeP_list_create(void);
static int tmodeP_list_destroy(ABObjList list);
...
int tmodeP_obj_create_data(ABObj obj) {
        if (!obj)
                return ERROR;
        ...
        tmodeP_obj_data(obj) =
            (TestModeData)util_malloc(sizeof(TestModeDataRec));
...
int tmodeP_obj_destroy_data(ABObj obj) {
        if (!obj)
                return ERROR;
...
int tmodeP_obj_construct_data(ABObj obj) {
...
int tmodeP_window_list_create(ABObj project) {
        if (!project)
                return ERROR;
...
int tmodeP_window_list_construct(ABObj project) {
        ABObj obj;
        AB_TRAVERSAL trav;
        int status;
             ...
             (obj = trav_next(&trav)) != NULL;) {
                ...
                if (status == 0) {
                        ...
                        if (tmodeP_obj_create_data(obj) != OK)
                                util_dprintf(1, "tmodeP_window_list_construct: "
                                                "Unable to create data.\n");
                } else if (status == ERR_DUPLICATE_KEY)
                        util_dprintf(1, "tmodeP_window_list_construct: "
                                        "Duplicate window in obj structure.\n");
                else
                        util_dprintf(1, "tmodeP_window_list_construct: Unable "
                                        "to add window to list.\n");
        }
        trav_close(&trav);
...
int tmodeP_window_list_destruct(void) {
        int i;
        ABObj obj = NULL;
        ...
        for (i = 0; i < objlist_get_num_objs(wlist); i++) {
                ...
                if (!obj) {
                        util_dprintf(1,
                                     "tmodeP_window_list_destruct: %d window's "
                                     "obj is missing.\n",
                                     i);
                        continue;
                }
                ...
                if (tmodeP_obj_has_data(obj)) {
                        if (tmodeP_obj_destroy_data(obj) == ERROR) {
                                util_dprintf(1,
                                             ...
                                             "Can't free test mode data for %d "
                                             "window.\n",
                                             i);
                                continue;
                        }
...
int tmodeP_window_list_destroy(void) {
        if (!wlist)
                return OK;

        /* clean the list */
        if (tmodeP_window_list_destruct() != OK)
                util_dprintf(1, "tmodeP_window_list_destroy: Can't destroy "
                                "some test mode data.\n");
...
*/
void tmodeP_window_list_iterate(ABObjListIterFn fn) {
        int i;
        ABObj obj = NULL;
...
**         client_data = obj.
*/
int tmodeP_window_list_add_handler(EventMask event_mask, Boolean nonmaskable,
        ...
        for (i = 0; i < objlist_get_num_objs(wlist); i++) {
                ...
                if (!obj) {
                        util_dprintf(1,
                                     "tmodeP_window_list_add_handler: %d "
                                     "window is missing obj.\n",
                                     i);
                        continue;
                }
                ...
                if (!obj->ui_handle) {
                        util_dprintf(2,
                                     "tmodeP_window_list_add_handler: no "
                                     "ui_handle %d window.\n",
                                     i);
                        continue;
                }
...
**         client_data = obj.
*/
int tmodeP_window_list_remove_handler(EventMask event_mask, Boolean nonmaskable,
                                      XtEventHandler event_handler) {
        int i;
        Widget w;
        ABObj obj = NULL;
        ...
        for (i = 0; i < objlist_get_num_objs(wlist); i++) {
                ...
                if (!obj) {
                        util_dprintf(1,
                                     "tmodeP_window_list_remove_handler: %d "
                                     "window is missing obj.\n",
                                     i);
                        continue;
                }
                ...
                if (!obj->ui_handle) {
                        util_dprintf(2,
                                     "tmodeP_window_list_remove_handler: no "
                                     "ui_handle %d window.\n",
                                     i);
                        continue;
                }
