...
static int print_load_stats(ABObj tree, ABProfiledInterval totalTime,
 ...
 */
int ab_load_bil_file(STRING fileName, FILE *inFile, BOOL BufferDrop) {
                ...
                for (trav_open(&trav, newProject, AB_TRAV_MODULES);
                     (mod = trav_next(&trav)) != NULL;) {
                        ...
                        if (!write_OK && !util_strempty(modfile)) {
                                ...
                                /*
                                ** Is this the first read-only file we've
                                *encountered?
                                ...
                                */
                                if (NoWrite == 0) {
                                        ...
                                        /* Now create a new buffer of the proper
                                         * size */
                                        len = strlen(init_msg) +
                                              strlen(modfile) + 3;
                                        ...
                                        /*
                                        ** Put the general text plus name of the
                                        *first
                                        ** read-only file into the notice
                                        *message.
                                        */
                                        sprintf(errmsg, "%s\n\n%s", init_msg,
                                                modfile);
        ...
        if (!util_strempty(errmsg)) {
                util_set_help_data(catgets(Dtb_project_catd, 100, 92,
                                           "Any changes made to a read-only "
                                           "module cannot be saved,\nunless "
                                           "you save the module to a different "
                                           "file name."),
                                   NULL, NULL);
                util_puts(errmsg);
                util_free(errmsg);
        ...
        if (obj_get_num_children(newProject) == 0) {
                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 23,
                                "%s: Empty project file loaded."),
                        fileName);
                xm_buf = XmStringCreateLocalized(Buf);
                dtb_palette_empty_proj_msg_initialize(
                    &dtb_palette_empty_proj_msg);
...
int ab_import_bil_file(STRING fileName, FILE *inFile, BOOL ImportByCopy) {
        int return_value = 0;
        AB_TRAVERSAL trav;
        ...
        if (loaded_module != NULL) {
                ...
                } else /* Import by reference */
                {
                         ...
                         */
                        file = proj_cvt_mod_file_to_rel_path(
                            fileName, obj_get_file(project));
...
static int get_start_times(ABProfiledInterval interval) {
        return get_cur_times(&(interval->realTime.startSeconds),
                             &(interval->CPUTime.startSeconds));
}

static int get_end_times(ABProfiledInterval interval) {
        return get_cur_times(&(interval->realTime.endSeconds),
                             &(interval->CPUTime.endSeconds));
}

static int calc_elapsed_times(ABProfiledInterval interval) {
 ...
 */
int ab_check_and_open_bip(STRING fileName) {
        ...
        if (read_OK) {
                ...
                } else /* Read-only file */
                {
                        ...
                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 17,
                        ...
                        help_data->help_text = catgets(
                            Dtb_project_catd, 100, 90,
                            "Opening a read-only project means that any "
                            "changes that\naffect the project (.bip) file, "
                            "such as creation of a\nnew module or creation of "
                            "a cross-module connection,\ncannot be saved, "
                            "unless you save the project to a\ndifferent file "
                            "name. You have the option of continuing\nwith the "
                            "open operation or cancelling it.");
                        help_data->help_volume = "";
                        help_data->help_locationID = "";

                        answer = dtb_show_modal_message(
                            dtb_get_toplevel_widget(),
                            &dtb_palette_open_ro_proj_msg, xm_buf, help_data,
                            NULL);
...
int ab_load_project(STRING fileName, FILE *inFile, BOOL BufferDrop) {
        int ret = 0;
 ...
 */
int ab_check_and_import_bil(STRING fileName, BOOL ImportByCopy) {
        BOOL read_OK, write_OK;
        DTB_MODAL_ANSWER answer = DTB_ANSWER_NONE;
        ABObj project = proj_get_project();
        int iRet = 0;
        ...
        if (read_OK) {
                ...
                {
                        if (write_OK) {
                                /* The file is readable and writable, can be
                                 * opened, and is a valid module file, so import
                                 * it.
                                 */
                                ...
                                sprintf(
                                    Buf,
                                    catgets(Dtb_project_catd, 100, 21,
                                            "The file %s is a read-only file.\n\
			    ...
			    the operation."),
                                ...
                                help_data = (DtbObjectHelpData)util_malloc(
                                    sizeof(DtbObjectHelpDataRec));
                                help_data->help_text = catgets(
                                    Dtb_project_catd, 100, 91,
                                    "Importing a read-only module means that "
                                    "any changes\nthat affect the module "
                                    "cannot be saved, unless you\nsave the "
                                    "module to a different file name. You "
                                    "have\nthe option of continuing with the "
                                    "import operation\nor cancelling it.");
                                ...
                                answer = dtb_show_modal_message(
                                    dtb_get_toplevel_widget(),
                                    &dtb_palette_import_ro_msg, xm_buf,
                                    help_data, NULL);
                                ...
                                switch (answer) {
                                case DTB_ANSWER_ACTION1:
                                        iRet = ab_import_module(fileName, NULL,
                                                                ImportByCopy);
                                        break;
                ...
                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 9,
                                "%s does not have read permission."),
                        fileName);
                xm_buf = XmStringCreateLocalized(Buf);
                dtb_palette_error_msg_initialize(&dtb_palette_error_msg);
...
int ab_import_module(STRING fileName, FILE *inFile, BOOL ImportByCopy) {
        int iRet = 0;
