 ...
 * Methods
 */
static int combo_box_initialize(ABObj obj);
static Widget combo_box_prop_init(Widget parent, AB_PROP_TYPE type);
static int combo_box_prop_activate(AB_PROP_TYPE type, BOOL active);
static int combo_box_prop_clear(AB_PROP_TYPE type);
static int combo_box_prop_load(ABObj obj, AB_PROP_TYPE type,
                               unsigned long loadkey);
static int combo_box_prop_apply(AB_PROP_TYPE type);
static BOOL combo_box_prop_pending(AB_PROP_TYPE type);
...
static int combobox_obj_destroy_CB(ObjEvDestroyInfo destroyInfo);
...
static Widget combo_box_prop_init(Widget parent, AB_PROP_TYPE type) {
        DtbComboboxPropDialogInfoRec
            rev_combobox_prop_dialog; /* Revolving Props */
        DtbComboboxPropDialogInfo cgen =
            &dtb_combobox_prop_dialog; /* Codegen structure */
        ...
        if (dtb_combobox_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_combo_box_palitem, cgen->ok_button,
                ...
                n++;
                prop_radiobox_init(
                    &(pcs->combobox_type), cgen->comboboxtype_rbox_label,
                ...
                prop_geomfield_init(&(pcs->wth), cgen->wpolicy_rbox_label, NULL,
                                    NULL, NULL, NULL, cgen->w_field_label,
                                    cgen->w_field, NULL, NULL,
                                    cgen->wpolicy_cb);
...
static int combo_box_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
                               unsigned long loadkey) {
        PropComboboxSettingsRec *pcs = &(prop_combobox_settings_rec[type]);
        BOOL editable;
...
int combo_box_prop_apply(AB_PROP_TYPE type) {
        PropComboboxSettingsRec *pcs = &(prop_combobox_settings_rec[type]);
        STRING value;
        ...
        if (prop_changed(pcs->wth_policy.changebar)) {
                ...
                if (width_policy == SIZE_FIXED_KEY) {
                        new_w =
                            prop_geomfield_get_value(&(pcs->wth), GEOM_WIDTH);
...
static BOOL combo_box_prop_pending(AB_PROP_TYPE type) {
        return (prop_changebars_pending(
            prop_combobox_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if ((prop_changed(pcs->label_type.changebar) ||
            ...
            prop_options_get_value(&(pcs->label_type)) ==
...
static int combobox_install_obj_destroy_CB() {
...
static void wth_policyCB(Widget w, XtPointer clientdata,
        ...
        if (state->set) {
                ...
                ui_field_set_editable(pcs->wth.w_field,
