 ...
 */
void pal_register_item_info(Widget pwidget, PalItemInfo *palitem, int subtype,
                            String subname, Pixmap subpixmap) {
        int i;
        ...
        if (!exists) {
                if (palette_item_cnt < MAX_PALETTE_ITEMS) {
                        palitem->subinfo =
                            (PalSubtypeInfo *)XtMalloc(sizeof(PalSubtypeInfo));
                        ...
                        if (subtype != AB_NO_SUBTYPE)
                                palitem->subinfo[0].subname =
                                    XtNewString(subname);
                        else
                                palitem->subinfo[0].subname = palitem->name;
                } else {
                        if (util_get_verbosity() > 0)
                                fprintf(stderr, "pal_register_item_info: "
                                                "palette item table full\n");
                        return;
                }

                /* Register High Level Object Type */
                pal_register_editable_obj(palitem->type,
                                          palitem->subinfo[0].subtype,
        ...
        } else /* Make room for additional Subtype information */
        {
                ...
                if (!found) {
                        ...
                        (palitem->num_subinfo)++;
                        newsub = (PalSubtypeInfo *)XtMalloc(
                            sizeof(PalSubtypeInfo) * palitem->num_subinfo);
 ...
 */
PalItemInfo *pal_get_item_info(ABObj obj) {
        int i;
 ...
 */
PalItemInfo *pal_get_type_item_info(AB_OBJECT_TYPE type, int subtype) {
        int i;
        PalItemInfo *palitem = NULL;
        ABObj vobj;
 ...
 */
int pal_initialize_obj(ABObj obj) {
        PalItemInfo *palitem;
...
Pixmap pal_get_item_pixmap(ABObj obj, int subtype, int *p_width,
                           int *p_height) {
        PalItemInfo *palitem;
...
STRING
pal_get_item_subname(ABObj obj, int subtype) {
        PalItemInfo *palitem;
        PalSubtypeInfo *sub = NULL;
        int i;
...
void pal_register_editable_obj(AB_OBJECT_TYPE type, int subtype, STRING name,
                               PalItemInfo *palitem) {
        PalEditableObjInfo *ed_obj_info;
        PalEditableObjInfo *prev, *cur;

        ed_obj_info =
            (PalEditableObjInfo *)util_malloc(sizeof(PalEditableObjInfo));
...
PalEditableObjInfo *pal_get_editable_obj_info(ABObj obj) {
        ...
        for (peo = pal_ed_obj_head; peo != NULL; peo = peo->next) {
                /* If a palitem structure is registered, use the "is_a" test
                 *func in that structure to determine if the obj is of that type
                 *of "editable" object, else directly compare the type &
                 *subtype.
                 *
                 * Note: the palitem structure test has precedence in order to
                 *       accomodate the high-level types which map to multiple
                 *	 subtypes. i.e. If we have a "Radio Box", we want to map
                 *to the "Choice" editable object 'super' type.
                 */
                if (peo->palitem) {
...
void pal_add_editable_obj_menu_items(Widget menu_pane,
        ...
        for (eop = pal_ed_obj_head; eop != NULL; eop = eop->next) {
                if ((*test_func)(eop) == True) {
                        xmlabel =
                            XmStringCreateLocalized(istr_string(eop->name));
                        button = XtVaCreateManagedWidget(
                            "object_menu_item", xmPushButtonWidgetClass,
                            menu_pane, XmNlabelString, xmlabel, XmNuserData,
                            (XtPointer)eop->palitem, NULL);
