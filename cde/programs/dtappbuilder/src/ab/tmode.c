...
**                                                                          **
*****************************************************************************/
static void win_get_geometry(ABObj obj);
static void track_win_focus(Widget widget, XtPointer client_data, XEvent *event,
                            Boolean *cont_disp);
static void hide_windows(ABObj obj);
static void check_win_for_resize(ABObj obj);
static void cleanup_window(ABObj obj);
...
*/
void tmode_enable(ABObj project, BOOL test_project) {
        ...
        if (tmodeP_window_list_create(project) != OK) {
                util_dprintf(1, "tmode_enable: Unable to create window list "
                                "for Test Mode.\n");
                goto cret;
        }
        ...
        */
        if (tmodeP_window_list_add_handler(EnterWindowMask, False,
                                           track_win_focus) != OK) {
                util_dprintf(
                    1, "tmode_enable: Unable to add window focus handler.\n");
                goto cret;
        }
...
*/
void tmode_disable(ABObj project) {
        ABObj mod;
        AB_TRAVERSAL trav;
        ...
        tmodeP_window_list_iterate(
            check_win_for_resize);                /* process for resize */
        ...
        tmodeP_window_list_iterate(
            cleanup_window); /* re-instantiate/set_ui_args */
        ...
        if (tmodeP_window_list_remove_handler(EnterWindowMask, False,
                ...
                util_dprintf(
                    1,
                    "tmode_disable: Unable to remove window focus handler.\n");
                goto cret;
        }
        ...
        if (tmodeP_window_list_destroy() != OK)
                util_dprintf(
                    1, "tmode_disable: Problems destroying window list.\n");
...
static void win_get_geometry(ABObj obj) {
        ...
        if (!tmodeP_obj_has_data(obj)) {
                if (tmodeP_obj_create_data(obj) != OK) {
                        util_dprintf(2, "win_get_geometry: Unable to create "
                                        "test mode data.\n");
                        return;
                }
        }
...
static void hide_windows(ABObj obj) {
        ...
        if (tmodeP_obj_has_flags(obj, TestModeFlagDirtyWindow |
        ...
        if (hide) {
                 ...
                 * object so that when the iconify_tracker,
                 * object_track_iconify(), event handler is called, it doesn't
                 * interpret the Unmap event as an iconify. (this is an
                 * unfortunate hack to get around the mysterious problem of the
                 * Unmap event occurring AFTER we return to BUILD mode).
                 */
