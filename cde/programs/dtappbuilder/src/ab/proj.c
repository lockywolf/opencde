...
static void create_proj_ui_handles(Vwr v);

/*
 * Action Procs for project window.
 */
static void select_module(Widget w, XEvent *ev, String *params, int nparams);

static void toggle_select_module(Widget w, XEvent *ev, String *params,
                                 int nparams);

static void show_module(Widget w, XEvent *ev, String *params, int nparams);

static void enable_proj_actions(Widget widget);

static void disable_proj_actions(Widget widget);

static void no_module_selected(DtbProjProjMainInfo proj_d);

static void one_module_selected(DtbProjProjMainInfo proj_d);

static void mult_module_selected(DtbProjProjMainInfo proj_d);

static void update_menu_items(int old_count, int new_count);

static void pointer_motion_proc(Widget widget, XEvent *event, String *params,
                                int num_params);

static void popdown_proj_window(Widget widget, XtPointer client_data,
                                XtPointer call_data);

static void import_uil(STRING uil_file_name);

/*
 * Object notification callbacks
 */
static int projP_obj_renameOCB(ObjEvAttChangeInfo info);

static int projP_obj_destroyOCB(ObjEvDestroyInfo info);

static int projP_obj_updateOCB(ObjEvUpdateInfo info);

/*
 * Functions for supporting rubber banding
 */
static void project_bpress(Widget widget, XEvent *event, String *params,
                           int num_params);

static int project_select_rband(VNode vnode);

static void projP_drawarea_button_drag(Widget widget, XtPointer client_data,
 ...
 */
void projP_store_viewer(Widget widget, XtPointer client_data,
                        XtPointer call_data) {
...
static void create_proj_ui_handles(Vwr v) {
        ...
        ab_register_window(AB_proj_window, AB_WIN_WINDOW, WindowHidden,
                           AB_toplevel, AB_WPOS_TILE_VERTICAL,
         ...
         */
        XtVaSetValues(dtb_proj_proj_main.menubar, XmNmenuHelpWidget,
                      dtb_proj_proj_main.menubar_items.Help_item, NULL);
         ...
         */
        XtAddEventHandler(draw_area, Button1MotionMask | Button2MotionMask,
                          False, projP_drawarea_button_drag, (XtPointer)NULL);
...
void proj_add_objects(AB_OBJ *obj) {
        AB_OBJ *project;
        Vwr viewer;
        ViewerMethods *m;
...
void proj_delete_objects(AB_OBJ *obj) {
        AB_OBJ *project;
        Vwr viewer;
        ViewerMethods *m;
 ...
 */
void proj_register_actions(XtAppContext app) {
...
static void enable_proj_actions(Widget widget) {
        if (proj_transtbl == NULL) {
                ...
                if (AB_btn1_transfer ==
                    True) /* Button2 can be used for ADJUST */
        ...
        XtVaSetValues(widget, XtNtranslations, proj_transtbl, NULL);
}
 ...
 */
static void select_module(Widget widget, XEvent *event, String *params,
                          int num_params) {
        Vwr v = NULL;
        VNode selected_node;
        VMethods m;
        ...
        selected_node = vwr_locate_node(v, event->xbutton.x, event->xbutton.y);
 ...
 */
static void toggle_select_module(Widget widget, XEvent *event, String *params,
                                 int num_params) {
        Vwr v = NULL;
        VNode selected_node;
        VMethods m;
 ...
 */
static void show_module(Widget widget, XEvent *event, String *params,
                        int num_params) {
        Vwr v = NULL;
        VNode selected_node;
        VMethods m;
        ...
        if (selected_node) {
                ...
                if (obj_has_flag(obj, MappedFlag)) {
                        ...
                        if (util_get_verbosity() > 0)
                                fprintf(
                                    stderr,
                                    "show_module: error in abobj_show_tree\n");
                } else {
                        proj_set_cur_module(obj);
                }
 ...
 * relevant menu items
 */
static void no_module_selected(DtbProjProjMainInfo proj_w) {
 ...
 */
static void one_module_selected(DtbProjProjMainInfo proj_w) {
        ProjModMenubuttonMenuItems menu_rec;
        ChooserInfo info = NULL;
         ...
         */
        if ((AB_generic_chooser == NULL) || !XtIsManaged(AB_generic_chooser)) {
 ...
 * saves at the moment...
 */
static void mult_module_selected(DtbProjProjMainInfo proj_w) {
        ProjModMenubuttonMenuItems menu_rec;
 ...
 */
int proj_destroy_project(ABObj proj) {
...
int proj_set_project(ABObj proj) {
        if (proj != NULL) {
                /*
...
ABObj proj_get_project() { return AB_project; }

int proj_set_cur_module(ABObj module) {
        if ((module != NULL) && (obj_get_type(module) != AB_TYPE_MODULE))
                return -1;

        AB_cur_module = module;
        if (module != NULL)
                ab_update_stat_region(AB_STATUS_CUR_MODULE,
                                      obj_get_name(module));
        else
...
ABObj proj_get_cur_module() { return AB_cur_module; }

int proj_rename_module(ABObj module, STRING name) {
...
static void pointer_motion_proc(Widget widget, XEvent *event, String *params,
        ...
        node_under_pointer = vwr_locate_node(v, mevent->x, mevent->y);
        if (node_under_pointer != NULL) {
                ...
                if (!util_strempty(mod_path)) {
                        util_dprintf(
                            2, "\tupdating the project window status region\n");
...
void proj_open_proj_okCB(Widget widget, XtPointer client_data,
        ...
        if (util_strempty(proj_filename)) {
                ...
                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 8,
                                "The file %s does not exist."),
                        proj_filename);
                util_printf_err(Buf);
        } else if (!util_file_is_regular_file(proj_filename)) {
                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 69,
                ...
                help_data->help_text =
                    catgets(Dtb_project_catd, 100, 89,
...
void proj_cancelCB(Widget widget, XtPointer client_data,
                   XmSelectionBoxCallbackStruct *call_data) {
...
void proj_import_okCB(Widget widget, XtPointer client_data,
                      XmSelectionBoxCallbackStruct *call_data) {
        ...
        if (util_strempty(file_name)) {
                ...
                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 70,
                                "Cannot import %s.\n%s is not a regular file."),
                        file_name, file_name);
                xm_buf = XmStringCreateLocalized(Buf);
                dtb_proj_error_msg_initialize(&dtb_proj_error_msg);
...
static int projP_obj_destroyOCB(ObjEvDestroyInfo info) {
        ...
        if (obj->projwin_data) {
                ...
                /*
                 * If the project is being destroyed, skip if the object passed
                 * in is not the project itself. We do this so that
                 * proj_delete_objects() is not called for every module in the
                 * project. Instead, it is called just once for the project.
                 */
                if (obj_has_flag(proj, BeingDestroyedFlag) &&
                    !obj_is_project(obj))
                        return (0);
...
static void project_bpress(Widget widget, XEvent *event, String *params,
        ...
        if (event->type == ButtonPress) {
                ...
                {
                        if (ui_initiate_rubberband(widget, True, project_rband,
                                                   (XtPointer)NULL) == ERROR) {
                                if (util_get_verbosity() > 0)
                                        fprintf(stderr, "project organizer: "
                                                        "couldn't begin "
                                                        "rubberbanding\n");
                        } else
...
static void project_rband(Widget widget, XEvent *event, XRectangle *rb_rect,
        ...
        XRectangle tmp_rect;
        int num_selected = 0, num_cur_selected = 0, i;
...
void proj_unmap_chooserCB(Widget chooser, XtPointer clientData,
                          XtPointer callData) {
        ChooserInfo info = NULL;
