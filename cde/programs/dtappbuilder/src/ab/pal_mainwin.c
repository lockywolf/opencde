 ...
 * Methods
 */
static int mainwin_initialize(ABObj obj);
static Widget mainwin_prop_init(Widget parent, AB_PROP_TYPE type);
static int mainwin_prop_activate(AB_PROP_TYPE type, BOOL active);
static int mainwin_prop_clear(AB_PROP_TYPE type);
static int mainwin_prop_load(ABObj obj, AB_PROP_TYPE type,
                             unsigned long loadkey);
static int mainwin_prop_apply(AB_PROP_TYPE type);

static BOOL mainwin_prop_pending(AB_PROP_TYPE type);

static BOOL verify_props(AB_PROP_TYPE type);
static void turnoff_changebars(AB_PROP_TYPE type);

static void mainwin_create_area(ABObj obj, AB_CONTAINER_TYPE area_type);

static void mainwin_remove_area(ABObj obj, ABObj area);
...
static Widget mainwin_prop_init(Widget parent, AB_PROP_TYPE type) {
        DtbMainwinPropDialogInfoRec
            rev_mainwin_prop_dialog; /* Revolving Props */
        ...
        if (dtb_mainwin_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                item_val[n] = False;
                n++;
                prop_radiobox_init(&(pms->resize_mode),
                ...
                item_val[n] = AB_CONT_MENU_BAR;
                n++;
                item[n] = cgen->areas_ckbox_items.Toolbar_item;
                item_val[n] = AB_CONT_TOOL_BAR;
                n++;
                ...
                n++;
                prop_radiobox_init(&(pms->size_policy),
                ...
                for (i = 0; i < n; i++)
                        XtAddCallback(item[i], XmNvalueChangedCallback,
                                      prop_size_policyCB,
                                      (XtPointer) & (pms->geometry));

                /* Geometry */
                prop_geomfield_init(&(pms->geometry), cgen->geom_label, NULL,
...
static int mainwin_prop_clear(AB_PROP_TYPE type) {
        ...
        /* Clear Size Policy */
        prop_radiobox_set_value(&(pms->size_policy), (XtPointer)SIZE_FIXED_KEY,
                                False);
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pms->init_state), AB_STATE_VISIBLE, True,
                                False);
        prop_checkbox_set_value(&(pms->init_state), AB_STATE_ICONIC, False,
                                False);
...
static int mainwin_prop_load(ABObj obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_field_set_value(&(pms->icon_mask), obj_get_icon_mask(obj),
                                     False);

                /* Load Icon Label */
                prop_field_set_value(&(pms->icon_label),
                                     obj_get_icon_label(obj), False);
...
int mainwin_prop_apply(AB_PROP_TYPE type) {
        PropMainwinSettingsRec *pms = &(prop_mainwin_settings_rec[type]);
        ABObj module;
        ABObj area;
        BOOL area_set;
        ...
        if (prop_changed(pms->title.changebar)) {
                ...
                abobj_set_label(pms->current_obj, pms->current_obj->label_type,
                                value);
                util_free(value);
        }
        ...
        if (prop_changed(pms->areas.changebar)) {
                ...
                /* Menubar */
                area = objxm_comp_mainwin_get_area(pms->current_obj,
                                                   AB_CONT_MENU_BAR);
                area_set =
                    prop_checkbox_get_value(&(pms->areas), AB_CONT_MENU_BAR);
                ...
                area = objxm_comp_mainwin_get_area(pms->current_obj,
                                                   AB_CONT_TOOL_BAR);
                area_set =
                    prop_checkbox_get_value(&(pms->areas), AB_CONT_TOOL_BAR);
                ...
                area = objxm_comp_mainwin_get_area(pms->current_obj,
                                                   AB_CONT_FOOTER);
                area_set =
                    prop_checkbox_get_value(&(pms->areas), AB_CONT_FOOTER);
        ...
        if (prop_changed(pms->init_state.changebar)) {
                abobj_set_visible(pms->current_obj,
                                  prop_checkbox_get_value(&(pms->init_state),
                                                          AB_STATE_VISIBLE));
...
static BOOL mainwin_prop_pending(AB_PROP_TYPE type) {
        return (prop_changebars_pending(
            prop_mainwin_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pms->areas.changebar)) {
                ...
                if (area != NULL && !area_set) /* Remove Menubar */
                {
                        ...
                        if (obj_get_num_children(pobj) > 0) {
                                ...
                                answer = dtb_show_modal_message(
                                    pms->areas.checkbox,
                                    &dtb_mainwin_remove_mbar_msg, NULL, NULL,
                                    NULL);
                ...
                if (area != NULL && !area_set) /* Remove Toolbar */
                {
                        ...
                        if (obj_get_num_children(pobj) > 0) {
                                ...
                                answer = dtb_show_modal_message(
                                    pms->areas.checkbox,
                                    &dtb_mainwin_remove_tbar_msg, NULL, NULL,
                                    NULL);
                                if (answer == DTB_ANSWER_CANCEL)
                                        return False;
                        }
                ...
                if (area != NULL && !area_set) /* Remove Footer */
                {
                        ...
                        if (obj_get_num_children(pobj) > 0) {
                                ...
                                answer = dtb_show_modal_message(
                                    pms->areas.checkbox,
                                    &dtb_mainwin_remove_footer_msg, NULL, NULL,
                                    NULL);
                                if (answer == DTB_ANSWER_CANCEL)
                                        return False;
                        }
        ...
        if (prop_changed(pms->geometry.changebar) &&
            (!prop_number_ok(pms->geometry.w_field, (STRING)WFieldStr, 1,
                             SHRT_MAX) ||
             !prop_number_ok(pms->geometry.h_field, (STRING)HFieldStr, 1,
                             SHRT_MAX)))
                return False;
