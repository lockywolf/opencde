...
static void textTransferCB(Widget widget, XtPointer clientData,
                           XtPointer callData);

static void textAnimateCB(Widget widget, XtPointer clientData,
                          XtPointer callData);

static int dnd_load_file(STRING fileName);

static int dnd_load_buffer(DtDndBuffer dropBuf);
 ...
 */
void ab_dnd_register_drop_file(Widget dropFile) {
...
static void textTransferCB(Widget widget, XtPointer clientData,
        ...
        case DtDND_FILENAME_TRANSFER:
                newString =
                    XtNewString((char *)transferInfo->dropData->data.files[0]);
...
static int dnd_load_buffer(DtDndBuffer dropBuf) {
        ...
        dataType = (char *)DtDtsBufferToDataType(dropBuf.bp, dropBuf.size,
                                                 dropBuf.name);

        inFile = util_create_tmp_file((char *)dropBuf.bp);
        if (inFile == NULL) {
                (void)dtb_show_modal_message(dtb_get_toplevel_widget(),
                                             &dtb_palette_drop_err_msg, NULL,
                                             NULL, NULL);
                iRet = -1;
                goto epilogue;
        }

        fileName = dropBuf.name ? dropBuf.name
                                : catgets(Dtb_project_catd, 100, 104, "NoName");
        fileName = XtNewString(fileName);

        if ((strcmp(dataType, BIX) == 0) || (strcmp(dataType, BIP) == 0)) {
                ...
                if (proj_check_unsaved_edits(project)) {
                        answer = dtb_show_modal_message(
                            ...
                            &dtb_proj_open_proj_warn_msg, NULL, NULL, NULL);
                ...
                case DTB_ANSWER_ACTION1: /* Load as Project */
                        if (proj_check_unsaved_edits(project)) {
                                answer = dtb_show_modal_message(
                                    dtb_get_toplevel_widget(),
                                    &dtb_proj_open_proj_warn_msg, NULL, NULL,
                                    NULL);
                                ...
                                case DTB_ANSWER_ACTION1:
                                         ...
                                         */
                                        proj_destroy_project(project);
                                        iRet = ab_load_project(fileName, inFile,
                                                               TRUE);
                                        break;
                ...
                if ((strcmp(dataType, BIP) == 0) ||
                    (strcmp(dataType, BIX) == 0)) {
                        ...
                        fmtStr = XtNewString(catgets(
                            Dtb_project_catd, 100, 103,
                            "Cannot determine file type for %s.\nDo you want "
                            "to load the file as a project or a module?"));
                        msg = (STRING)util_malloc(strlen(fmtStr) +
                                                  strlen(fileName) + 1);
                        sprintf(msg, fmtStr, fileName);
                        xm_str = XmStringCreateLocalized(msg);
                        ...
                        case DTB_ANSWER_ACTION1: /* Load as Project */
                                if (proj_check_unsaved_edits(project)) {
                                        answer = dtb_show_modal_message(
                                            dtb_get_toplevel_widget(),
                                            &dtb_proj_open_proj_warn_msg, NULL,
                                            NULL, NULL);

                                        switch (answer) {
                                        case DTB_ANSWER_ACTION1:
                                                /* Go ahead and destroy the old
                                                 * project.  */
                                                proj_destroy_project(project);
                                                iRet = ab_check_and_open_bip(
                                                    fileName);
                                                break;
                        ...
                        case DTB_ANSWER_ACTION2: /*Load as Module */
                                answer = dtb_show_modal_message(
                                    dtb_get_toplevel_widget(),
                                    &dtb_palette_import_by_msg, NULL, NULL,
                                    NULL);
                                switch (answer) {
                                case DTB_ANSWER_ACTION1: /* Copy */
                                        iRet = ab_check_and_import_bil(fileName,
                                                                       TRUE);
                                        break;
 ...
 * must be called prior to calling this function.
 */
void ab_dnd_unregister_drop(Widget dropFile) {
        if (!dropFile)
                return;
