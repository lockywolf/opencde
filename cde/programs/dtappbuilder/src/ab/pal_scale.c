 ...
 * Methods
 */
static int scale_initialize(ABObj obj);
static Widget scale_prop_init(Widget parent, AB_PROP_TYPE type);
static int scale_prop_activate(AB_PROP_TYPE type, BOOL active);
static int scale_prop_clear(AB_PROP_TYPE type);
static int scale_prop_load(ABObj obj, AB_PROP_TYPE type, unsigned long loadkey);
static int scale_prop_apply(AB_PROP_TYPE type);
static BOOL scale_prop_pending(AB_PROP_TYPE type);
...
static void setup_direction_setting(AB_PROP_TYPE type, AB_ORIENTATION orient);

/*
 * Xt Callbacks
 */
static void orientCB(Widget widget, XtPointer clientdata,
...
static int scale_initialize(ABObj obj) {
        if (obj_get_read_only(obj) == False) {
                ...
                obj_set_label(obj,
                              catgets(Dtb_project_catd, 100, 260, "Scale:"));
        } else /* Gauge */
        {
                obj_set_unique_name(obj, "gauge");
                obj_set_label(obj,
                              catgets(Dtb_project_catd, 100, 261, "Gauge:"));
        }
        ...
        /* REMIND:  Hack to get around Motif XmNtitleString bug (when set to
         * NULL) */
...
static Widget scale_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbScalePropDialogInfo cgen =
            &dtb_scale_prop_dialog; /* Codegen structure */
        ...
        if (dtb_scale_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_scale_palitem, cgen->ok_button,
                ...
                n++;
                prop_options_init(
                    &(pss->label_type), cgen->labeltype_opmenu_label,
                    cgen->labeltype_opmenu, cgen->labeltype_opmenu_menu, n,
                    item, (XtPointer *)item_val, cgen->labeltype_cb);
                ...
                n++;
                prop_options_init(
                    &(pss->label_pos), cgen->labelpos_opmenu_label,
                    cgen->labelpos_opmenu, cgen->labelpos_opmenu_menu, n, item,
                ...
                item_val[n] = True;
                n++;
                prop_radiobox_init(&(pss->scale_type),
                                   cgen->scaletype_rbox_label,
                                   cgen->scaletype_rbox, n, item,
                                   (XtPointer *)item_val, cgen->scaletype_cb);
...
static int scale_prop_clear(AB_PROP_TYPE type) {
        ...
        /* Clear Orientation */
        prop_radiobox_set_value(&(pss->orient), (XtPointer)AB_ORIENT_HORIZONTAL,
                                False);

        /* Clear Direction */
        prop_options_set_value(&(pss->dir), (XtPointer)AB_DIR_LEFT_TO_RIGHT,
                               False);
...
static int scale_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_radiobox_set_value(&(pss->scale_type),
                                        (XtPointer)obj_get_read_only(obj),
                                        False);

                /* Load Orientation */
                orient = obj_get_orientation(obj);
                prop_radiobox_set_value(&(pss->orient), (XtPointer)orient,
                                        False);
                setup_direction_setting(type, orient);

                /* Load Direction */
                prop_options_set_value(
                    &(pss->dir), (XtPointer)obj_get_direction(obj), False);
...
int scale_prop_apply(AB_PROP_TYPE type) {
        PropScaleSettingsRec *pss = &(prop_scale_settings_rec[type]);
        STRING value;
        int new_w, new_h;
        BOOL size_chg = False;
        ...
        if (prop_changed(pss->ivalue.changebar)) {
                ...
                abobj_set_show_value(pss->current_obj,
                                     prop_checkbox_get_value(&(pss->show_value),
                                                             SHOW_VALUE_KEY));
        }
        if (prop_changed(pss->geometry.changebar)) {
                ...
                if (abobj_height_resizable(pss->current_obj)) {
                        new_h = prop_geomfield_get_value(&(pss->geometry),
                                                         GEOM_HEIGHT);
...
static BOOL scale_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_scale_settings_rec[type].prop_sheet));
}
        ...
        if (prop_changed(pss->geometry.changebar) &&
            (!prop_number_ok(pss->geometry.x_field, (STRING)XFieldStr,
             ...
             !prop_number_ok(pss->geometry.y_field, (STRING)YFieldStr,
                             -SHRT_MAX, SHRT_MAX) ||
             !prop_number_ok(pss->geometry.h_field, (STRING)HFieldStr, 1,
                             SHRT_MAX) ||
...
static void orientCB(Widget item, XtPointer clientdata,
                     XmToggleButtonCallbackStruct *state) {
        ...
        if (state->set) {
                ...
                if (value == AB_ORIENT_HORIZONTAL)
                        prop_options_set_value(
                            &(pss->dir), (XtPointer)AB_DIR_LEFT_TO_RIGHT, True);
                else
                        prop_options_set_value(
                            &(pss->dir), (XtPointer)AB_DIR_BOTTOM_TO_TOP, True);
        }
}
