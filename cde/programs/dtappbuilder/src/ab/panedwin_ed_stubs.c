...
void panedwin_ed_show_dialog() {
        ...
        if (AB_panedWinEd_dialog == (Widget)NULL) {
                ...
                XtVaSetValues(
                    XtParent(dtb_panedwin_ed_dialog.panelist_scrolledwin),
                    XmNrightAttachment, XmATTACH_FORM, XmNrightOffset, 100,
                    NULL);
                 ...
                 */
                XtAddCallback(
                    dtb_panedwin_ed_dialog.objlist, XmNbrowseSelectionCallback,
                    (XtCallbackProc)pw_objlist_selectCB, (XtPointer)NULL);

                /* Add Callback to load panes when a paned window obj is
                 * selected from list. */
                XtAddCallback(
                    dtb_panedwin_ed_dialog.panelist, XmNbrowseSelectionCallback,
                ...
                ab_register_window(AB_panedWinEd_dialog, AB_WIN_DIALOG,
                                   ...
                                   AB_WPOS_TILE_HORIZONTAL, NULL, NULL);
        }
...
static Widget panedwin_editor_init(DtbPanedwinEdDialogInfo pw_cgen,
        ...
        if (dtb_panedwin_ed_dialog_initialize(pw_cgen, parent) == 0) {
                ...
                prop_geomfield_init(
                    &(pws->geometry), pw_cgen->geo_label, NULL, NULL, NULL,
                    NULL, pw_cgen->width_field_label, pw_cgen->width_field,
                ...
                prop_geomfield_init(
                    &(pws->min_max_geo), pw_cgen->pane_height_lbl, NULL, NULL,
                    NULL, NULL, pw_cgen->min_height_field_label,
                    pw_cgen->min_height_field, pw_cgen->max_height_field_label,
...
static void pw_objlist_selectCB(Widget widget, XtPointer client_data,
        ...
        if (name) {
                ...
                if (selected_obj) {
                        ...
                        /* REMIND: Need to add code to deal with pending
                         * changes.*/
                        ...
                        abobj_list_update(panelist, selected_obj,
                                          pw_child_test_func);
...
static void pw_panelist_selectCB(Widget widget, XtPointer client_data,
        ...
        if (name) {
                ...
                if (module != NULL) {
                        ui_set_active(dtb_panedwin_ed_dialog.width_field_label,
                                      True);
                        ui_set_active(dtb_panedwin_ed_dialog.width_field, True);
                        ui_set_active(dtb_panedwin_ed_dialog.height_field_label,
                                      True);
                        ui_set_active(dtb_panedwin_ed_dialog.height_field,
                                      True);
                        ui_set_active(
                            dtb_panedwin_ed_dialog.min_height_field_label,
                            True);
                        ui_set_active(dtb_panedwin_ed_dialog.min_height_field,
                                      True);
                        ui_set_active(
                            dtb_panedwin_ed_dialog.max_height_field_label,
                            True);
                        ui_set_active(dtb_panedwin_ed_dialog.max_height_field,
                                      True);
...
static int pw_obj_destroyedOCB(ObjEvDestroyInfo info) {
        ...
        if (AB_panedWinEd_dialog != NULL) {
                if (!obj_is_project(info->obj) &&
                    !obj_is_paned_win(info->obj) && !obj_is_pane(info->obj) &&
                ...
                if (obj_is_project(info->obj)) {
                        ...
                        ui_set_active(dtb_panedwin_ed_dialog.width_field_label,
                                      False);
                        ui_set_active(dtb_panedwin_ed_dialog.width_field,
                                      False);
                        ui_set_active(dtb_panedwin_ed_dialog.height_field_label,
                                      False);
                        ui_set_active(dtb_panedwin_ed_dialog.height_field,
                        ...
                        if (sel_item != NULL) {
                                ...
                                if (strcmp(name, objxm_xmstr_to_str(
                                                     sel_item[0])) == 0)
                                        panedwin_editor_clear();
                        }
                ...
                } else /* Object is a pane OR a layer */
                {
                        ...
                        if (((parent = obj_get_parent(info->obj)) != NULL) &&
                            obj_is_paned_win(parent)) {
                                XtVaGetValues(panelist, XmNselectedItems,
                                              &sel_item, NULL);
                                if (sel_item != NULL) {
                                        name =
                                            abobj_get_moduled_name(info->obj);
                                        if (strcmp(name, objxm_xmstr_to_str(
                                                             sel_item[0])) == 0)
                                                panedwin_editor_clear_geom();
                                }
                                ret = abobj_list_obj_destroyed(
                                    panelist, info->obj, pw_child_test_func);
                        }
                }
        }
        return (ret);
        ...
        if (AB_panedWinEd_dialog != NULL) {
                ...
                } else /* obj is a pane OR a layer */
                {
                        if (info->old_name == NULL) {
                                ...
                                if ((parent != NULL) &&
                                    obj_is_paned_win(parent)) {
                                        XtVaGetValues(list, XmNselectedItems,
                                                      &sel_item, NULL);
                                        if (sel_item != NULL) {
...
static int pw_obj_reparentedOCB(ObjEvReparentInfo info) {
        ...
        if (AB_panedWinEd_dialog != NULL) {
                ...
                if (obj_get_name(info->obj) != NULL) {
                         ...
                         * need to update the panes list because that will
                         * happen when the new paned window obj is selected.
                         */
                        if ((info->old_parent != NULL) &&
                            obj_is_paned_win(info->old_parent)) {
                                ...
                                ret = abobj_list_obj_reparented(
                                    panelist, info, pw_child_test_func);
                        }
                }
        }
        return (ret);
