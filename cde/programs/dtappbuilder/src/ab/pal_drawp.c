 ...
 * Methods
 */
static int drawp_initialize(ABObj obj);
static Widget drawp_prop_init(Widget parent, AB_PROP_TYPE type);
static int drawp_prop_activate(AB_PROP_TYPE type, BOOL active);
static int drawp_prop_clear(AB_PROP_TYPE type);
static int drawp_prop_load(ABObj obj, AB_PROP_TYPE type, unsigned long loadkey);
static int drawp_prop_apply(AB_PROP_TYPE type);
static BOOL drawp_prop_pending(AB_PROP_TYPE type);

static BOOL verify_props(AB_PROP_TYPE type);
static void turnoff_changebars(AB_PROP_TYPE type);

static void setup_scroll_settings(AB_PROP_TYPE type,
                                  AB_SCROLLBAR_POLICY sb_policy);
/*
 * Callbacks
 */
static void scroll_stateCB(Widget widget, XtPointer client_data,
                           XtPointer call_data);
...
static Widget drawp_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbDrawpPropDialogInfo cgen =
            &dtb_drawp_prop_dialog; /* Codegen structure */
        ...
        if (dtb_drawp_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_REVOLVING)
                        XtVaSetValues(parent, XmNuserData, pds->current_obj,
                                      NULL);
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_drawp_palitem, cgen->ok_button,
                ...
                item_val[n] = AB_SCROLLBAR_WHEN_NEEDED;
                n++;
                prop_radiobox_init(&(pds->scrolling),
                                   cgen->scrolldisp_rbox_label,
                                   cgen->scrolldisp_rbox, n, item,
                                   (XtPointer *)item_val, cgen->scroll_cb);

                for (i = 0; i < n; i++)
                        XtAddCallback(item[i], XmNvalueChangedCallback,
                                      (XtCallbackProc)scroll_stateCB,
                                      (XtPointer)type);
                ...
                prop_menuname_init(&(pds->menuname), type, cgen->menu_label,
                                   ...
                                   &(pds->menu_title), &(pds->current_obj),
                                   True);
                ...
                /* Canvas Size */
                prop_geomfield_init(&(pds->size), cgen->tsize_label, NULL, NULL,
                                    NULL, NULL, cgen->twidth_field_label,
                                    cgen->twidth_field,
                                    cgen->theight_field_label,
                                    cgen->theight_field, cgen->tsize_cb);
...
static int drawp_prop_clear(AB_PROP_TYPE type) {
        ...
        /* Clear Initial State */
        prop_checkbox_set_value(&(pds->init_state), AB_STATE_VISIBLE, True,
                                False);
        prop_checkbox_set_value(&(pds->init_state), AB_STATE_ACTIVE, True,
                                False);
...
static int drawp_prop_load(ABObj obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_radiobox_set_value(&(pds->scrolling), (XtPointer)sb_policy,
                                        False);
                setup_scroll_settings(type, sb_policy);

                /* Load Border Frame */
                prop_options_set_value(
                    &(pds->frame), (XtPointer)obj_get_border_frame(obj), False);

                /* Load Menu Name/Title */
                prop_menuname_set_value(&(pds->menuname),
                                        obj_get_menu_name(obj), False);
                prop_field_set_value(&(pds->menu_title),
                ...
                /* Load Color */
                prop_colorfield_set_value(&(pds->bg_color),
                                          obj_get_bg_color(obj), False);
                prop_colorfield_set_value(&(pds->fg_color),
...
int drawp_prop_apply(AB_PROP_TYPE type) {
        PropDrawpSettingsRec *pds = &(prop_drawp_settings_rec[type]);
        ...
        if (prop_changed(pds->menuname.changebar)) {
                ...
                util_free(value);
                prop_menuname_set_value(&(pds->menuname),
                                        obj_get_menu_name(pds->current_obj),
                                        False);
        }
        ...
        if (prop_changed(pds->geom.changebar)) {
                ...
                border = /* assumption! */
                    ((prop_options_get_value(&(pds->frame)) !=
        ...
        if (prop_changed(pds->init_state.changebar)) {
                abobj_set_visible(pds->current_obj,
                                  prop_checkbox_get_value(&(pds->init_state),
                                                          AB_STATE_VISIBLE));
        ...
        if (prop_changed(pds->fg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pds->fg_color),
                                          obj_get_fg_color(pds->current_obj),
                                          False);
        }
        if (prop_changed(pds->bg_color.changebar)) {
                ...
                util_free(value);
                prop_colorfield_set_value(&(pds->bg_color),
                                          obj_get_bg_color(pds->current_obj),
                                          False);
        }
...
static BOOL drawp_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_drawp_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pds->geom.changebar) &&
            (!prop_number_ok(pds->geom.x_field, (STRING)XFieldStr, -SHRT_MAX,
                             SHRT_MAX) ||
             !prop_number_ok(pds->geom.y_field, (STRING)YFieldStr, -SHRT_MAX,
                             SHRT_MAX) ||
             !prop_number_ok(pds->geom.w_field, (STRING)WFieldStr, 1,
                             SHRT_MAX) ||
             !prop_number_ok(pds->geom.h_field, (STRING)HFieldStr, 1,
                             SHRT_MAX)))
                return False;

        if (prop_changed(pds->size.changebar) &&
            (!prop_number_ok(pds->size.w_field, "DrawArea Width Field", 1,
                             SHRT_MAX) ||
             !prop_number_ok(pds->size.h_field, "DrawArea Height Field", 1,
                             SHRT_MAX)))
                return False;

        if (prop_changed(pds->fg_color.changebar) &&
            !prop_color_ok(pds->fg_color.field))
                return False;

        if (prop_changed(pds->bg_color.changebar) &&
            !prop_color_ok(pds->bg_color.field))
                return False;

        if (prop_changed(pds->menuname.changebar) &&
            !prop_obj_name_ok(pds->menuname.field,
                              obj_get_module(pds->current_obj), AB_TYPE_MENU,
                              "Menu"))
                return False;
