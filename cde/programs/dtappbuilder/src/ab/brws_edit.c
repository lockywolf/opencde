...
static void browser_textf_activate(Widget widget, XtPointer client_data,
                                   XtPointer call_data);
...
void brwsP_create_textf(Vwr v) {
        ...
        if (!ui->textf) {
                ...
                font_list_entry = XmFontListEntryCreate(
                    XmFONTLIST_DEFAULT_TAG, XmFONT_IS_FONT, ui->sm_font);

                font_list = XmFontListAppendEntry(NULL, font_list_entry);

                ui->textf = XtVaCreateWidget(
                    "change_name", xmTextFieldWidgetClass, draw_area,
                    XmNfontList, font_list, XmNshadowThickness, 0,
                    XmNhighlightThickness, 0, XmNmarginWidth,
                    BRWS_ELM_BBOX_MARGIN, XmNmarginHeight, BRWS_ELM_BBOX_MARGIN,
                    XmNmaxLength, 80, XmNbackground,
                    BlackPixelOfScreen(XtScreen(ui->shell)), XmNforeground,
                    WhitePixelOfScreen(XtScreen(ui->shell)), NULL);
...
void brwsP_destroy_textf(Vwr v) {
        BrowserUiObjects *ui;
 ...
 */
void brwsP_show_textf(VNode vnode, int elm_pos) {
        Vwr v;
        BrowserUiObj ui;

        if (!vnode || (elm_pos < 0) || !vnode->num_elements ||
        ...
        if (ui->textf) {
                ...
                XtVaSetValues(ui->textf, XmNvalue, (char *)elm[elm_pos].data,
                              XmNx, vnode->x + BRWS_ELM_BORDER_WIDTH, XmNy,
                              elm[elm_pos].y - BRWS_ELM_BBOX_MARGIN, XmNwidth,
                              vnode->width - (2 * BRWS_ELM_BORDER_WIDTH),
                              XmNheight,
                              elm[elm_pos].height + (2 * BRWS_ELM_BBOX_MARGIN),
                              XmNuserData, vnode, NULL);
