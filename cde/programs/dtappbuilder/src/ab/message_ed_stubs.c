...
**                                                             **
*****************************************************************/
static Widget msg_editor_init(DtbMessageEdDialogInfo msg_cgen, Widget parent);
static int msg_editor_load(ABObj project);
...
static void setup_button_fields(AB_MESSAGE_TYPE mtype);
static void setup_btn_lbl_field(int button, BOOL value);
static void set_msg_textp_label(AB_MESSAGE_TYPE mtype);
static void msgEdP_init();

static int msgEdP_msgpane_init(DtbMessageEdDialogInfo cgen);
static DTB_MODAL_ANSWER msgEdP_do_auto_apply(Widget list, ABObj old_obj,
                                             ABObj new_obj);
static BOOL apply_changes(ABObj obj);
static void objlist_load(Widget list);
static BOOL objlist_test_func(ABObj test_obj);
static int build_module_opmenu(DtbMessageEdDialogInfo cgen);
...
static int update_mod_opmenu(MOD_OP op, STRING item_str, STRING new_item_str,
                             Widget *RetItem, int *ModItemCount);
 ...
 * Callbacks
 */
static void msgEdP_prevent_closeCB(Widget widget, XtPointer client_data,
                                   XtPointer call_data);
static void typeCB(Widget widget, XtPointer clientdata, XtPointer calldata);
static void buttonCB(Widget widget, XtPointer clientdata,
...
static void change_msg_moduleCB(Widget widget, XtPointer clientdata,
                                XtPointer calldata);
...
void msgEd_show_dialog() {
        ...
        if (AB_msgEd_dialog == (Widget)NULL) {
                ...
                XtVaSetValues(dtb_message_ed_dialog.dialog_form,
                              XmNresizePolicy, XmRESIZE_ANY, XmNallowResize,
                              True, NULL);
                /*
                 * Setup dialog to participate in dtbuilder window protocol
                 */
                ab_register_window(AB_msgEd_dialog, AB_WIN_DIALOG, WindowHidden,
        ...
        if (AB_msgEd_dialog != NULL) {
                ...
                if (MessageModule != current_module) {
                        ...
                        ui_optionmenu_change_label(
                            MsgModOpmenu, obj_get_name(current_module));
...
static Widget msg_editor_init(DtbMessageEdDialogInfo msg_cgen, Widget parent) {
        ...
        if (dtb_message_ed_dialog_initialize(msg_cgen, parent) == 0) {
                ...
                item_val[n] = AB_MSG_ERROR;
                n++;
                item[n] =
                    msg_cgen->type_opmenu_items.bitmaps_xm_information_xbm_item;
                item_val[n] = AB_MSG_INFORMATION;
                n++;
                item[n] =
                    msg_cgen->type_opmenu_items.bitmaps_xm_question_xbm_item;
                item_val[n] = AB_MSG_QUESTION;
                n++;
                item[n] =
                    msg_cgen->type_opmenu_items.bitmaps_xm_warning_xbm_item;
                item_val[n] = AB_MSG_WARNING;
                n++;
                item[n] =
                    msg_cgen->type_opmenu_items.bitmaps_xm_working_xbm_item;
                item_val[n] = AB_MSG_WORKING;
                n++;
                prop_options_init(&(mes->msg_type), msg_cgen->type_opmenu_label,
                                  msg_cgen->type_opmenu,
                                  msg_cgen->type_opmenu_menu, n, item,
                                  (XtPointer *)item_val, msg_cgen->msg_cb);
                /*
                ...
                item_val[n] = ACTION1_ITEM_KEY;
                n++;
                prop_checkbox_init(&(mes->action1_button),
                                   msg_cgen->action1_cbox_label,
                                   msg_cgen->action1_cbox, n, item, item_val,
                                   msg_cgen->action1_cb);
                ...
                item_val[n] = ACTION2_ITEM_KEY;
                n++;
                prop_checkbox_init(&(mes->action2_button),
                                   msg_cgen->action2_cbox_label,
                                   msg_cgen->action2_cbox, n, item, item_val,
                                   msg_cgen->action2_cb);
                ...
                item_val[n] = ACTION3_ITEM_KEY;
                n++;
                prop_checkbox_init(&(mes->action3_button),
                                   msg_cgen->action3_cbox_label,
                                   msg_cgen->action3_cbox, n, item, item_val,
                                   msg_cgen->action3_cb);
                ...
                item_val[n] = CANCEL_ITEM_KEY;
                n++;
                prop_checkbox_init(&(mes->cancel_button),
                                   msg_cgen->cancel_cbox_label,
                                   msg_cgen->cancel_cbox, n, item, item_val,
                                   msg_cgen->cancel_cb);
                ...
                util_dprintf(
                    3,
                    "msg_editor_init: could not initialize Message Editor\n");
                return NULL;
        }
}
...
static int msg_editor_clear() {
        ...
        /* Clear Message Type */
        prop_options_set_value(&(mes->msg_type), (XtPointer)AB_MSG_ERROR,
                               False);
        ...
        prop_checkbox_set_value(&(mes->action1_button), ACTION1_ITEM_KEY, False,
                                False);
        ...
        prop_checkbox_set_value(&(mes->action2_button), ACTION2_ITEM_KEY, True,
                                False);

        /* Clear Action2 Label */
        prop_field_set_value(&(mes->action2_label),
                             msgEd_strings[MSG_ED_RETRY_STR], False);

        /* Clear Action3 Button */
        prop_checkbox_set_value(&(mes->action3_button), ACTION3_ITEM_KEY, False,
                                False);
        ...
        /* Clear Cancel Button */
        prop_checkbox_set_value(&(mes->cancel_button), CANCEL_ITEM_KEY, True,
                                False);

        /* Clear Help Button */
        prop_checkbox_set_value(&(mes->help_button), HELP_ITEM_KEY, True,
                                False);

        /* Clear Default Button */
        prop_options_set_value(&(mes->default_btn), (XtPointer)ACTION2_ITEM_KEY,
                               False);
...
static int msg_editor_load(ABObj obj) {
        STRING name, msg;
        ...
        prop_field_set_value(&(mes->action1_label), obj_get_action1_label(obj),
                             False);
        ...
        prop_field_set_value(&(mes->action2_label), obj_get_action2_label(obj),
                             False);
        ...
        prop_field_set_value(&(mes->action3_label), obj_get_action3_label(obj),
                             False);
...
static BOOL msg_editor_pending() {
...
static void buttonCB(Widget item, XtPointer clientdata,
                     XmToggleButtonCallbackStruct *state) {
        int button;
        MsgEditorSettingsRec *mes = &msg_editor_settings_rec;
...
static void typeCB(Widget item, XtPointer clientdata, XtPointer calldata) {
        int value;
        STRING label = NULL;
        MsgEditorSettingsRec *mes = &msg_editor_settings_rec;
        ...
        case AB_MSG_ERROR:
                ...
                if ((XtPointer)prop_options_get_value(&(mes->default_btn)) !=
                    AB_DEFAULT_BTN_CANCEL) {
                        prop_options_set_value(&(mes->default_btn),
                                               (XtPointer)AB_DEFAULT_BTN_CANCEL,
                                               True);
                }
                break;
...
static int obj_updateOCB(ObjEvUpdateInfo info) {
        ...
        if (AB_msgEd_dialog != NULL) {
                if (!obj_is_defined(info->obj) ||
                    (!obj_is_project(info->obj) && !obj_is_module(info->obj))) {
                        return iRet;
                }
                ...
                if (obj_is_module(info->obj)) {
                        ...
                        if (obj_has_flag(info->obj, MappedFlag)) {
                                ...
                                if (!XtIsSensitive(
                                        dtb_message_ed_dialog.ctrl_panel)) {
                                        ui_set_active(
                                            dtb_message_ed_dialog.ctrl_panel,
                                            TRUE);
                                }

                                iRet = update_mod_opmenu(
                                    MOD_ADD, obj_get_name(info->obj), NULL,
                                    &item, &numModItems);
                                if (item != NULL) {
                                        ...
                                        /* If this is the first module to be
                                         * shown, created, or imported, then set
                                         * it to be the current message module.
                                         */
                        ...
                        {
                                iRet = update_mod_opmenu(
                                    MOD_DELETE, obj_get_name(info->obj), NULL,
                                    &item, &numModItems);
                                ...
                                if (numModItems == 0) {
                                        ...
                                        ui_set_active(
                                            ...
                                            False);
                                        ui_set_active(
                                            dtb_message_ed_dialog.ctrl_panel2,
                                            False);
                                        ui_set_active(
                                            dtb_message_ed_dialog.ok_button,
                                            False);
                                        ui_set_active(
                                            dtb_message_ed_dialog.apply_button,
                                            False);
                                        ui_set_active(
                                            dtb_message_ed_dialog.reset_button,
                                            False);
...
static int obj_destroyedOCB(ObjEvDestroyInfo info) {
        ...
        if (AB_msgEd_dialog != NULL) {
                ...
                if (obj_is_project(info->obj)) {
                        ...
                        ui_set_active(dtb_message_ed_dialog.ctrl_panel2, False);
                        ui_set_active(dtb_message_ed_dialog.ok_button, False);
                        ui_set_active(dtb_message_ed_dialog.apply_button,
                                      False);
                        ui_set_active(dtb_message_ed_dialog.reset_button,
                                      False);
                        MessageModule = (ABObj)NULL;
                        XtManageChild(NoModulesItem);
                } else if (obj_is_module(info->obj)) {
                        iRet = update_mod_opmenu(MOD_DELETE,
                                                 obj_get_name(info->obj), NULL,
                                                 &item, &numModItems);
                        /* Check if deleting the last module in the
                         * module optionmenu.
                         */
                        if (numModItems == 0) {
                                msg_editor_clear();
                                ui_set_active(dtb_message_ed_dialog.ctrl_panel,
                                              False);
                                ui_set_active(dtb_message_ed_dialog.ctrl_panel2,
                                              False);
                                ui_set_active(dtb_message_ed_dialog.ok_button,
                                              False);
                                ui_set_active(
                                    dtb_message_ed_dialog.apply_button, False);
                                ui_set_active(
                                    dtb_message_ed_dialog.reset_button, False);
                                XtManageChild(NoModulesItem);
                                ui_optionmenu_change_label(
                                    MsgModOpmenu,
...
static int obj_renamedOCB(ObjEvAttChangeInfo info) {
        ...
        if (AB_msgEd_dialog != NULL) {
                if (!obj_is_defined(info->obj) ||
                    (!obj_is_module(info->obj) && !obj_is_message(info->obj))) {
                        return iRet;
                }
                ...
                if (obj_is_module(info->obj)) {
                        ...
                        if (info->old_name != NULL) {
                                iRet = update_mod_opmenu(
                                    MOD_REPLACE, istr_string(info->old_name),
                                    mod_name, &item, &numModItems);
                        }
                } else {
        ...
        for ((i = 0, trav_open(&trav, project, AB_TRAV_MODULES));
             (mod = trav_next(&trav)) != NULL; i++) {
                if (obj_has_flag(mod, MappedFlag) && obj_is_defined(mod)) {
                        ...
                        iRet = update_mod_opmenu(MOD_ADD, name, NULL, &item,
                                                 &numModItems);
                        if (iRet != -1) {
                                XtAddCallback(item, XmNactivateCallback,
                                              change_msg_moduleCB,
                ...
                if (selected_obj) {
                        ...
                        if ((mes->current_obj != NULL) &&
                            prop_changebars_pending(mes->prop_sheet)) {
                                (void)msgEdP_do_auto_apply(
                                    widget, mes->current_obj, selected_obj);
                        }
...
static BOOL objlist_test_func(ABObj test_obj) {
        ...
        if (module && obj_is_defined(module) &&
            obj_has_flag(module, MappedFlag) && obj_is_message(test_obj)) {
                should_add = TRUE;
        }
        return (should_add);
}
        ...
        case AB_DEFAULT_BTN_CANCEL:
                if (!prop_checkbox_get_value(&(mes->cancel_button),
                        ...
                        (void)dtb_show_modal_message(
                            ...
                            &dtb_message_ed_cancel_def_btn_msg, NULL, NULL,
                            NULL);
                        validButton = FALSE;
                }
                break;
...
static BOOL verify_label() {
        ...
        if (prop_checkbox_get_value(&(mes->action1_button), ACTION1_ITEM_KEY)) {
                label = ui_field_get_string(mes->action1_label.field);
                if (util_strempty(label)) {
                        dtb_message_ed_act1_lbl_msg_initialize(
                            &dtb_message_ed_act1_lbl_msg);
                        (void)dtb_show_modal_message(
                            mes->action1_label.field,
                            &dtb_message_ed_act1_lbl_msg, NULL, NULL, NULL);
                        validLabel = FALSE;
                }
        }

        if (prop_checkbox_get_value(&(mes->action2_button), ACTION2_ITEM_KEY)) {
                label = ui_field_get_string(mes->action2_label.field);
                if (util_strempty(label)) {
                        dtb_message_ed_act2_lbl_msg_initialize(
                            &dtb_message_ed_act2_lbl_msg);
                        (void)dtb_show_modal_message(
                            mes->action2_label.field,
                            &dtb_message_ed_act2_lbl_msg, NULL, NULL, NULL);
                        validLabel = FALSE;
                }
        }

        if (prop_checkbox_get_value(&(mes->action3_button), ACTION3_ITEM_KEY)) {
                label = ui_field_get_string(mes->action3_label.field);
                if (util_strempty(label)) {
        ...
        if (!prop_checkbox_get_value(&(mes->action1_button),
            ...
            !prop_checkbox_get_value(&(mes->help_button), HELP_ITEM_KEY)) {
                ...
                (void)dtb_show_modal_message(AB_msgEd_dialog,
                                             &dtb_message_ed_no_btns_msg, NULL,
                                             NULL, NULL);
                hasButtons = FALSE;
        }
...
)
{
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
        ...
        if ((mes->current_obj != NULL) &&
            prop_changebars_pending(mes->prop_sheet)) {
                answer = msgEdP_do_auto_apply(MsgObjList, mes->current_obj,
                                              (ABObj)NULL);
        ...
        ret =
            abobj_list_obj_created(cgen->msg_list, msg_obj, objlist_test_func);
...
)
{
    ...
    /*** DTB_USER_CODE_START vvv Add C code below vvv ***/
        ...
        if ((mes->current_obj == NULL) || !obj_is_message(mes->current_obj)) {
                return;
        }
        ...
        /* Free data */
        if (mbr.title != NULL)
                XmStringFree(mbr.title);
        if (mbr.message != NULL)
                XmStringFree(mbr.message);
        if (mbr.action1_label != NULL)
                XmStringFree(mbr.action1_label);
        if (mbr.action2_label != NULL)
                XmStringFree(mbr.action2_label);
        if (mbr.action3_label != NULL)
                XmStringFree(mbr.action3_label);
...
static BOOL apply_changes(ABObj obj) {
        ...
        if (!verify_name() || !verify_default_btn() || !verify_label() ||
            !verify_buttons()) {
                return (FALSE);
        }
        ...
        if (prop_changed(mes->msg_type.changebar)) {
                if (obj_get_msg_type(obj) !=
                    (AB_MESSAGE_TYPE)prop_options_get_value(&(mes->msg_type))) {
                        obj_set_msg_type(
                            obj, (AB_MESSAGE_TYPE)prop_options_get_value(
                                     &(mes->msg_type)));
                }
                abobj_set_save_needed(module, TRUE);
        }
        ...
        if (prop_changed(mes->cancel_button.changebar)) {
                obj_set_cancel_button(
                    obj, prop_checkbox_get_value(&(mes->cancel_button),
                                                 CANCEL_ITEM_KEY));
                abobj_set_save_needed(module, TRUE);
        }
        ...
        if (prop_changed(mes->default_btn.changebar)) {
                obj_set_default_btn(obj,
                                    (AB_DEFAULT_BUTTON)prop_options_get_value(
                                        &(mes->default_btn)));
                abobj_set_save_needed(module, TRUE);
        }
        turnoff_changebars();
...
static DTB_MODAL_ANSWER msgEdP_do_auto_apply(Widget list, ABObj old_obj,
        ...
        DTB_MODAL_ANSWER answer = DTB_ANSWER_NONE;
        char buffer[256];
        STRING old_name = NULL, new_name = NULL;
        BOOL ChangingObjects = FALSE, ChangesApplied = FALSE;
        ...
        if (dtb_app_resource_rec.implied_apply == True)
                ...
                for \"%s\"."),
                        ...
                        help_data->help_text = catgets(
                            Dtb_project_catd, 100, 98,
                            "Click Apply Changes to apply the changes to "
                            "the\ncurrent message and load the selected "
                            "message.\n\nClick Cancel if you don't want to "
                            "apply the\nchanges to the current message. You "
                ...
                Applied.\nApply Changes or Cancel Close operation."),
                                    old_name);
                                help_data->help_text = catgets(
                                    Dtb_project_catd, 100, 99,
                                    "Click Apply Changes to apply the changes "
                                    "to the\ncurrent message and close the "
                                    "Message Editor.\n\nClick Cancel if you "
                                    "don't want to apply the\nchanges to the "
                                    "current message and want the\nMessage "
                                ...
                                sprintf(
                                    buffer,
                                    catgets(
                                        Dtb_project_catd, 100, 39,
                                        "Properties for \"%s\" have been modified but not\
                Applied.\nApply Changes or Cancel Add-Message\
                operation."),
                                    old_name);
                                help_data->help_text = catgets(
                                    Dtb_project_catd, 100, 100,
                                    "Click Apply Changes to apply the changes "
                                    "to the\ncurrent message and add a new "
                                    "message to the\nmodule.\n\nClick Cancel "
                                    "if you don't want to apply the\nchanges "
                                    "to the current message. You can "
                                    "then\nclick Reset to undo the changes "
                                    "before adding\na new message.");
                        }
                }
...
static int update_mod_opmenu(MOD_OP op, STRING item_str, STRING new_item_str,
        ...
        case MOD_ADD:
                if (item_str != NULL) {
                        ...
                        if (item == NULL) {
                                item = ui_optionmenu_add_item(MsgModOpmenu,
                                                              item_str);
                                if (item != NULL)
                                        NumModItems++;
                                else
        ...
        case MOD_DELETE:
                if (item_str != NULL) {
                        iRet =
                            ui_optionmenu_delete_item(MsgModOpmenu, item_str);
                        if (iRet == 0)
                                NumModItems--;
                } else {
                        iRet = -1;
        ...
        case MOD_REPLACE:
                if ((item_str != NULL) && (new_item_str != NULL)) {
                        item = ui_optionmenu_replace_item(
                            MsgModOpmenu, item_str, new_item_str);
                } else {
                        iRet = -1;
                }
                break;
