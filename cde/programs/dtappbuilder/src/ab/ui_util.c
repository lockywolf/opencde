...
static void rubberband_finish(Widget widget, XEvent *event,
                              XtPointer client_data);

static void rubberband_release(Widget widget, XtPointer client_data,
                               XEvent *event, Boolean *cont_dispatch);

static void rubberband_draw(Widget widget, XEvent *event,
                            XtPointer client_data);
...
**                                                                      **
**************************************************************************/
void ui_win_front(Widget widget) {
        Widget p_shell;
...
void ui_win_show(Widget widget, BOOL show, XtGrabKind grab_kind) {
        Widget shell;
        Widget dialog;
...
void ui_win_set_resizable(Widget widget, BOOL resizable, BOOL remap) {
        Widget shell;
        int decor = 0;
        int new_decor = 0;
        int func = 0;
        ...
        XtVaGetValues(shell, XmNmwmDecorations, &decor, XmNmwmFunctions, &func,
                      NULL);
        ...
        if (new_func != func || new_decor != decor) {
                ...
                XtVaSetValues(shell, XmNmwmDecorations, new_decor,
                              XmNmwmFunctions, new_func, NULL);
...
void ui_field_set_string(Widget field, STRING valuestr) {

        XtVaSetValues(field, XmNvalue, (XtArgVal)(valuestr ? valuestr : ""),
                      XmNcursorPosition,
                      (XtArgVal)valuestr ? strlen(valuestr) : 0, NULL);
}

STRING
ui_field_get_string(Widget field) {
        char *string = NULL;
...
void ui_field_select_string(Widget field, BOOL assign_focus) {
        STRING valuestr = NULL;

        valuestr = ui_field_get_string(field);

        if (XtIsSubclass(field, xmTextFieldWidgetClass))
                XmTextFieldSetSelection(field, 0, strlen(valuestr),
                                        CurrentTime);
        else if (XtIsSubclass(field, xmTextWidgetClass))
...
void ui_field_set_editable(Widget field, BOOL editable) {
        XtVaSetValues(field, XmNeditable, editable, XmNcursorPositionVisible,
                      editable, NULL);
}

void ui_set_active(Widget widget, BOOL state) {
        ...
        if (XtIsComposite(widget)) {
                ...
                for (i = 0; i < num_children; i++) {
                        ...
                        XtVaGetValues(children[i], XmNnumChildren,
                                      &num_children_children, XmNchildren,
                                      &children_children, NULL);

                        if (num_children_children > 0) {
                                for (c = 0; c < num_children_children; c++)
                                        XtSetSensitive(children_children[c],
                                                       (Boolean)state);
                        }
                }
        }
...
void ui_set_visible(Widget widget, BOOL viz) {
...
void ui_set_label_string(Widget widget, STRING string) {
        XmString xmlabel;
...
void ui_set_label_glyph(Widget widget, STRING fileName) {
        Pixmap labelPixmap = NULL;
        Pixmap labelInsensitivePixmap = NULL;

        XtVaGetValues(widget, XmNlabelPixmap, &labelPixmap,
                      XmNlabelInsensitivePixmap, &labelInsensitivePixmap, NULL);
...
Widget ui_get_ancestor_shell(Widget widget) {
        Widget shell = widget;
...
Widget ui_get_ancestor_dialog(Widget widget) {
...
Widget ui_build_menu(Widget parent, int menu_type, int num_columns,
                     char *menu_title, char *menu_name, MenuItem *menu_items) {
        ...
        if (num_columns > 1) {
                ...
                n++;
                XtSetArg(args[n], XmNnumColumns, num_columns);
                n++;
        }
        if (menu_type == XmMENU_PULLDOWN)
                menu = XmCreatePulldownMenu(parent, "pulldown_menu", args, n);
        else {
                XtSetArg(args[n], XmNwhichButton, AB_BMenu);
                n++;
        ...
        if (menu_type == XmMENU_PULLDOWN) {
                cascade = XtVaCreateManagedWidget(
                    menu_name, xmCascadeButtonWidgetClass, parent, XmNsubMenuId,
                    menu, NULL);
        ...
        for (i = 0; menu_items[i].label != NULL; i++) {
                if (menu_items[i].subitems) {
                        widget = ui_build_menu(
                            menu, XmMENU_PULLDOWN, menu_items[i].num_columns,
                            menu_items[i].label, menu_items[i].name,
                            (MenuItem *)menu_items[i].subitems);

                        XtVaSetValues(widget, XmNuserData,
                                      (XtArgVal)menu_items[i].user_data, NULL);
                } else {
                        widget = XtVaCreateManagedWidget(
                            menu_items[i].name, *menu_items[i].wclass, menu,
                        ...
                        if (menu_items[i].label != NULL) {
                                xmlabel = XmStringCreateLocalized(
                                    menu_items[i].label);
                                XtVaSetValues(widget, XmNlabelString, xmlabel,
                                              NULL);
                                XmStringFree(xmlabel);
                        }
...
void ui_populate_pulldown_menu(Widget menu, int num_columns,
                               MenuItem *menu_items) {
        Widget widget;
        Arg args[4];
        int i;
...
void ui_size_to_row_col(Widget text, unsigned short width,
                        unsigned short height, int *row_ptr, int *col_ptr) {
        ...
        if (XtIsSubclass(text, dtTermWidgetClass))
                XtVaGetValues(text, XmNmarginWidth, &margin_w, XmNmarginHeight,
                              &margin_h, DtNuserFont, &fontlist, NULL);
        else
                XtVaGetValues(text, XmNmarginWidth, &margin_w, XmNmarginHeight,
                              &margin_h, XmNfontList, &fontlist, XmNlistSpacing,
                              &text_spacing, NULL);

        parent = XtParent(text);

        if (XtIsSubclass(parent, xmScrolledWindowWidgetClass)) {
                XtVaGetValues(parent, XmNhorizontalScrollBar, &hsb,
                              XmNverticalScrollBar, &vsb, XmNspacing, &spacing,
                              NULL);
                if (hsb)
                ...
                XtVaGetValues(parent, XmNmarginWidth, &p_margin_w,
                              XmNmarginHeight, &p_margin_h, NULL);
        ...
        lineheight =
            font->max_bounds.ascent + font->max_bounds.descent + text_spacing;
...
int ui_set_busy_cursor(Window window, BOOL on) {
...
static void init_obj_pixmaps() {
        ...
        for (i = 0; (object_pixmaps[i].type != AB_TYPE_UNKNOWN); ++i) {
                 ...
                 * Create pixmap
                 */
                status = dtb_cvt_image_file_to_pixmap(
                    AB_toplevel, object_pixmaps[i].filename, &tmp);

                if (!status) {
                        /*
                         * Get width/height of pixmap
                         */
                        if (XGetGeometry(dpy, tmp, &root, &x, &y, &w, &h, &bw,
                                         &d)) {
         ...
         */
        status = dtb_cvt_image_file_to_pixmap(AB_toplevel, "DtABdfl",
                                              &default_pixmap);
        ...
        if (default_pixmap) {
                 ...
                 */
                if (XGetGeometry(dpy, default_pixmap, &root, &x, &y, &w, &h,
                                 &bw, &d)) {
 ...
 */
void ui_get_obj_pixmap(AB_OBJ *obj, Pixmap *pixmap, /* RETURN */
...
void ui_add_window_close_callback(Widget shell, XtCallbackProc delete_callback,
                                  XtPointer client_data,
        ...
        XmAddWMProtocolCallback(shell, WM_DELETE_WINDOW, delete_callback,
                                client_data);
}

void ui_remove_window_close_callback(Widget shell,
        ...
        WM_DELETE_WINDOW =
            XmInternAtom(XtDisplay(shell), "WM_DELETE_WINDOW", False);
        XmRemoveWMProtocolCallback(shell, WM_DELETE_WINDOW, delete_callback,
                                   client_data);
}

void ui_add_window_iconify_handler(Widget shell, XtEventHandler iconify_proc,
                                   XtPointer client_data) {
        XtAddEventHandler(shell, StructureNotifyMask, False, iconify_proc,
                          (XtPointer)client_data);
}

void ui_remove_window_iconify_handler(Widget shell, XtEventHandler iconify_proc,
                                      XtPointer client_data) {
        XtRemoveEventHandler(shell, StructureNotifyMask, False, iconify_proc,
                             (XtPointer)client_data);
}

void ui_refresh_widget_tree(Widget widget) {
        WidgetList children;
        int num_children = 0;
        int i;
...
static Bool event_is_expose(XEvent *event) {
...
static void sync_timeout_proc(XtPointer clientData,
        ...
        done = ((syncData->timeout_ticks - syncData->last_expose_ticks) >= 5);
...
int ui_sync_display_of_widget(Widget widget) {
        int return_value = 0;
...
#define timeout_ticks (syncData.timeout_ticks)
        ...
        syncData.timerId = XtAppAddTimeOut(appContext, 100, sync_timeout_proc,
                                           (XtPointer)&syncData);
        ...
        XtRemoveTimeOut(syncData.timerId);
        syncData.timerId = 0;
        return return_value;
 ...
 * rubber band func.
 */
int ui_initiate_rubberband(Widget widget, Boolean confine_to_window,
                           UiRubberBandFunc rb_func, XtPointer client_data) {

        XtAddEventHandler(widget, ButtonReleaseMask, False, rubberband_release,
                          client_data);
        ...
        if (XtGrabPointer(widget, False,
                          ...
                          confine_to_window ? XtWindow(widget) : None, NULL,
 ...
 */
void ui_button_drag(Widget widget, XEvent *event, XtPointer client_data) {
...
Widget ui_optionmenu_add_item(Widget opmenu, STRING item_str) {
        XmString xmitem;
        ...
        if ((cascade_btn = XmOptionButtonGadget(opmenu)) != NULL) {
                ...
                if (menu) {
                        ...
                        mpb = XtVaCreateManagedWidget(
                            item_str, xmPushButtonWidgetClass, menu,
                            XmNlabelString, xmitem, NULL);
                        XmStringFree(xmitem);
                }
        }
...
int ui_optionmenu_delete_item(Widget opmenu, STRING item_str) {
        Widget item = NULL;
        int iRet = 0;
...
Widget ui_optionmenu_replace_item(Widget opmenu, STRING old_item_str,
                                  STRING new_item_str) {
        Widget item = NULL;
        ...
        if (item != NULL) {
                ...
                if (XmStringCompare(xmstr, old_item_xmstr)) {
                        XtVaSetValues(opmenu_label, XmNlabelString,
                                      new_item_xmstr, NULL);
                }
...
Widget ui_optionmenu_find_item(Widget opmenu, STRING item_str) {
        Widget cascade_btn = NULL;
        Widget menu = NULL;
        ...
        if ((cascade_btn = XmOptionButtonGadget(opmenu)) != NULL) {
                ...
                if (menu) {
                        ...
                        XtVaGetValues(menu, XmNnumChildren, &numChildren,
                                      XmNchildren, &children, NULL);
                        for (i = 0; i < numChildren && !Found; i++) {
                                XtVaGetValues(children[i], XmNlabelString,
                                              &child, NULL);
...
int ui_optionmenu_num_items(Widget opmenu) {
        Widget cascade_btn = NULL;
        Widget menu = NULL;
        int numChildren = 0;
        ...
        if ((opmenu != NULL) &&
            ((cascade_btn = XmOptionButtonGadget(opmenu)) != NULL)) {
                XtVaGetValues(cascade_btn, XmNsubMenuId, &menu, NULL);
                if (menu) {
                        XtVaGetValues(menu, XmNnumChildren, &numChildren, NULL);
                }
        }
        return (numChildren);
}

void ui_optionmenu_change_label(Widget opmenu, STRING new_str) {
        Widget opmenu_label = NULL;
        XmString new_xmstr;
        ...
        if ((opmenu_label != NULL) && (new_str != NULL)) {
                ...
                if (!XmStringCompare(old_xmstr, new_xmstr)) {
                        XtVaSetValues(opmenu_label, XmNlabelString, new_xmstr,
                                      NULL);
                }
...
void ui_optionmenu_change_label_pixmap(Widget opmenu, Pixmap pixmap) {
        Widget opmenu_label;

        opmenu_label = XmOptionButtonGadget(opmenu);

        if (opmenu_label != NULL) {
                XtVaSetValues(opmenu_label, XmNlabelPixmap, pixmap,
...
*/
void ui_obj_set_label_string(ABObj obj, STRING label) {
        ABObj labelObj = NULL;
        ...
        case AB_TYPE_ITEM:
                ...
                case AB_ITEM_FOR_CHOICE:
                        labelObj = objxm_comp_get_subobj(obj, AB_CFG_LABEL_OBJ);
                        if (labelObj == NULL ||
                            objxm_get_widget(labelObj) == NULL)
                                return;
        ...
        case AB_TYPE_FILE_CHOOSER:
                ...
                XtVaSetValues(objxm_get_widget(labelObj), XmNtitle, label,
                              NULL);
                break;
...
*/
void ui_obj_set_label_glyph(ABObj obj, STRING fileName) {
        ABObj labelObj = NULL;
        ...
        case AB_TYPE_ITEM:
                ...
                case AB_ITEM_FOR_MENUBAR:
                case AB_ITEM_FOR_CHOICE:
                        ui_set_label_glyph(objxm_get_widget(labelObj),
                                           fileName);
                        break;
...
*/
void ui_obj_set_label(ABObj obj, STRING label) {
        if (obj == NULL)
                return;
