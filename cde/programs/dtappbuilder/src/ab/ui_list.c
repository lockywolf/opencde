...
Widget ui_combobox_get_list_widget(Widget combobox) {
        Widget list;
        list = XtNameToWidget(combobox, "*List");
        return (list);
}

Widget ui_combobox_get_field_widget(Widget combobox) {
        Widget textf;
        textf = XtNameToWidget(combobox, "*Text");
        return (textf);
}

int ui_combobox_clear(Widget combobox) {
        Widget list;
        int i, num_items;

        list = ui_combobox_get_list_widget(combobox);
        XtVaGetValues(list, XmNitemCount, &num_items, NULL);
...
int ui_combobox_add_item(Widget combobox, STRING item, int pos, BOOL unique) {
        XmString xmitem;
...
int ui_combobox_select_item(Widget combobox, STRING item) {
        XmString xmitem;
...
int ui_list_add_item(Widget list, STRING item_str, int pos) {
        XmString xmitem;
 ...
 */
int ui_list_delete_item(Widget list, STRING item_str) {
        int return_value = 0;
 ...
 */
int ui_list_replace_item(Widget list, STRING old_item_str,
                         STRING new_item_str) {
        int rc = 0; /* return code */
        int item_pos = 0;
...
/*
 */
int ui_list_replace_item_prefix(Widget list, STRING old_item_prefix,
        ...
        XtVaGetValues(list, XmNitemCount, &num_items, XmNitems, &items, NULL);
        ...
        for (i = 0; i < num_items; ++i) {
                ...
                if (strncmp(item_str, old_item_prefix, old_prefix_len) == 0) {
                        ...
                        new_xmitem_array[0] =
                            XmStringCreateLocalized(new_item_buf);
 ...
 */
int ui_list_find_item(Widget list, STRING item_str, int *pos_out) {
        int return_value = 0;
        XmString xmitem;
        int item_pos;
...
int ui_list_get_info(Widget list, int *num_items_out, int *first_visible_out,
                     int *last_visible_out) {
        int num_items = 0;
        int top_pos = 0;
        int items_visible = 0;

        XtVaGetValues(list, XmNitemCount, &num_items, XmNtopItemPosition,
                      &top_pos, XmNvisibleItemCount, &items_visible, NULL);
...
int ui_list_select_item(Widget list, STRING item_str, BOOL invoke_callbacks) {
        int item_pos = 0;
        int iRet = 0;
...
int ui_list_select_pos(Widget list, int item_pos, BOOL invoke_callbacks) {
        int num_items = 0;
        int first_vis = 0;
        int last_vis = 0;
        int items_vis = 0;
        ...
        if (item_pos > last_vis) {
                 ...
                 */
                XtVaSetValues(list, XmNtopItemPosition,
                              (item_pos - items_vis) + 1, NULL);

        } else if (item_pos < first_vis) {
                /* Item is above the first item showing -
                 * so make the Item the first one showing
                 */
                XtVaSetValues(list, XmNtopItemPosition, item_pos, NULL);
        }
 ...
 */
int ui_list_get_selected_pos(Widget list) {
        int sel_item_count;
        int *sel_item_num;
        int select_pos = -1;
