...
static void help_dispatchCB(Widget widget, XtPointer client_data,
                            XtPointer call_data);

static void more_help_dispatch(Widget widget, XtPointer clientData,
                               XtPointer callData);

static void help_back_hdlr(Widget widget, XtPointer clientData,
                           XtPointer callData);

static void help_attr_chgCB(Widget widget, XtPointer client_data,
                            XtPointer call_data);

static void select_instanceCB(Widget widget, XtPointer client_data,
                              XmListCallbackStruct *listdata);

static int object_was_created(ObjEvCreateInfo info);

static int object_was_deleted(ObjEvDestroyInfo info);

static int object_was_renamed(ObjEvAttChangeInfo info);

static int object_was_updated(ObjEvUpdateInfo info);

static void clear_editor_fields();

static void fetch_help_attrs(ABObj obj);

static void update_help_attrs(ABObj obj);

static DTB_MODAL_ANSWER do_auto_apply(Widget w, ABObj old_obj, ABObj new_obj);

static BOOL help_list_test(ABObj obj);

static BOOL help_obj_is_target_type(ABObj obj);

static void help_test_onitem_help();

static void verify_closeCB(Widget w, XtPointer client_data,
                           XtPointer call_data);

static void change_objecttype_palCB(Widget w, XtPointer client_data,
                                    XtPointer call_data);

static void change_objtype(Widget w, AB_OBJECT_TYPE type, int subtype);

static void update_object_menu_from_obj(Widget menu, ABObj obj);
...
extern void ab_popup_help(Widget widget, XtPointer client_data,
        ...
        if (AB_help_dialog == (Widget)NULL) {
                ...
                /* Build object type menu (but first must destroy dummy option
                 * item) */
                ...
                */
                ...
                prop_field_init(&(hes->locID), help_ed->help_location_label,
                                help_ed->help_location,
                                help_ed->loc_change_bar);
        }
        ...
        */
        if (HelpObj == NULL) {
                ...
                if (sel.count > 0) {
                        ...
                        ** If this fails, there must really not be any salient
                        *UI objects
                        */
                 ...
                 */
                if (HelpObj &&
                    (ed_obj_info = pal_get_editable_obj_info(HelpObj))) {
                        if (editable_obj_test(ed_obj_info)) {
                                HelpObjTypeInfo.type = obj_get_type(HelpObj);
                                HelpObjTypeInfo.subtype =
                                    obj_get_subtype(HelpObj);
                        }
                }
        }
        ...
        if (HelpObj != NULL) {
                /*
                 ** Having figured out what (if any) object type we should be
                 *targeting,
...
void ab_set_help_obj(ABObj obj) {
        HelpObj = obj;
...
*/
static void help_attr_chgCB(Widget w, XtPointer client_data,
                            XtPointer call_data) {
...
static void select_instanceCB(Widget widget, XtPointer client_data,
                        ...
                        */
                        if (HelpObj != NULL &&
                            prop_changebars_pending(hes->prop_sheet) == True) {
                                (void)do_auto_apply(widget, HelpObj,
                                                    selected_obj);
                        }
        ...
        if (util_get_verbosity() > 0)
                fprintf(stderr, "Help Editor::select_instanceCB: could not get "
                                "object in list\n");
}
...
*/
static DTB_MODAL_ANSWER do_auto_apply(Widget list, ABObj old_obj,
                                      ABObj new_obj) {
        ...
        if (dtb_app_resource_rec.implied_apply == True)
                ...
                if (changing_objects) {
                        sprintf(buffer,
                                catgets(Dtb_project_catd, 100, 31,
		...
		Load operation for \"%s\"."),
                        ...
                        if (new_obj != (ABObj)NULL) {
                                sprintf(buffer,
                                        catgets(Dtb_project_catd, 100, 32,
                                                "Help properties for \"%s\"\n\
		have been modified but not Applied.\n\n\
		You can Apply the Changes or Cancel the\n\
		Close operation."),
                                ...
                                help_data->help_text = catgets(
                                    Dtb_project_catd, 100, 94,
                                    "Click Apply Changes to apply the changes "
                                    "to the\ncurrent object and close the Help "
                                    "Editor.\n\nClick Cancel if you don't want "
                                    "to apply the\nchanges to the current "
                                    "object and want the Help\nEditor to "
                                    "remain displayed.  You can then "
                                    "click\nReset to undo the changes before "
                                    "closing the\nHelp Editor.");
                        } else {
                                sprintf(buffer,
                                        catgets(Dtb_project_catd, 100, 33,
                                                "Help properties for \"%s\"\n\
		have been modified but not Applied.\n\n\
		You can Apply the Changes or Cancel the\n\
...
static void fetch_help_attrs(ABObj obj) {
        ...
        util_dprintf(
            2, "Load object help attributes into editor display fields\n");
...
** Editor's list.
*/
static BOOL help_list_test(ABObj test_obj) {
        /* Is this an object we'd want to display? */
        if (obj_is_salient_ui(test_obj) && obj_is_defined(test_obj) &&
            ...
            (obj_has_flag(test_obj, MappedFlag) ||
             (obj_is_message(test_obj)))) {
                /* Yes it is.  If we're looking for a current type, do they
                 * match ? */
...
static BOOL help_obj_is_target_type(ABObj obj) {
        ...
        /*
        ** Otherwise we only need to check object type, and in fact don't want
        *to
...
static int object_was_renamed(ObjEvAttChangeInfo info) {
        return abobj_list_obj_renamed(HelpObjList, info->obj,
...
void help_test_enable(ABObj obj) {
         ...
         */
        if (obj_has_help_data(obj) == False)
                return;

        /*
        ** Composite objects have a specifically-identified help object
        ** to which we should attach the help callback.  For other objects
        *types,
        ...
        */
        if (obj_is_popup_win(obj)) {
                if ((help_btn = obj_get_help_act_button(obj)) != (ABObj)NULL) {
                        XtAddCallback(
                            (Widget)help_btn->ui_handle, XmNactivateCallback,
                            dtb_call_help_callback, help_obj->ui_handle);
                }
        }
}

void help_test_disable(ABObj obj) {
        ABObj help_obj, help_btn;
        ...
        */
        ...
        /*
        util_dprintf(2,"    help deactivated on
        %s\n",abobj_get_moduled_name(obj));
        */
...
*/
static void help_dispatchCB(Widget widget, XtPointer clientData,
                            XtPointer callData) {
        ...
        */
        if (help_volume == 0 || *(help_volume) == NULL || help_location == 0 ||
            *(help_location) == NULL) {
                buffer[0] = '\0';
        } else {
        ...
        */
        if (Quick_help_dialog == (Widget)NULL) {
                ...
                i++;
                XtSetArg(wargs[i], DtNstringData, help_text);
                i++;
                XtSetArg(wargs[i], DtNhelpVolume, buffer);
                i++;
                Quick_help_dialog =
                    DtCreateHelpQuickDialog(AB_toplevel, "Help", wargs, i);
                ...
                */
                ...
                XtAddCallback(MoreButton, XmNactivateCallback,
                              more_help_dispatch, (XtPointer)Quick_help_dialog);
                if (buffer[0] == '\0')
...
static void help_test_onitem_help() {
        ...
        if ((obj = objxm_get_obj_from_widget(target)) == NULL)
                return;
        ...
        if (obj_has_help_data(obj) == True) {
                ...
                if (help_obj != NULL) {
                        ...
                        XtCallCallbacks((Widget)help_obj->ui_handle,
        ...
        */
                ...
                /*
                ** Do conditional traversal, where the condition is our
                *obj_has_help
                ** function.  Combined with TRAV_PARENTS, this means that a
                *single
                ...
                */
                ...
                if (root_obj == (ABObj)NULL) {
                         ...
                         */
                        help_obj =
                            objxm_comp_get_subobj(root_obj, AB_CFG_HELP_OBJ);
                        if (help_obj != NULL) {
                                obj_get_help_data(obj, &help_vol, &help_id,
                                                  &help_text);
                                util_dprintf(2, "Object help text:\n%s\n",
                                             help_text);
                                XtCallCallbacks((Widget)help_obj->ui_handle,
...
** window menu.
*/
static void verify_closeCB(Widget widget, XtPointer client_data,
                           XtPointer call_data) {
        DTB_MODAL_ANSWER answer;
        HelpEditorSettingsRec *hes = &Help_Editor_Settings_Rec;
...
static void change_objecttype_palCB(Widget widget, XtPointer client_data,
                                    XtPointer call_data) {
...
static void change_objtype(Widget widget, AB_OBJECT_TYPE newtype,
                           int newsubtype) {
        DTB_MODAL_ANSWER answer;
        int i;
        HelpEditorSettingsRec *hes = &Help_Editor_Settings_Rec;
        ...
        */
        ...
        /* Update current-objecttype (from menu selection) & clear current
         * object */
        HelpObj = (ABObj)NULL;
         ...
         */
        XtVaGetValues(HelpObjList, XmNitemCount, &i, NULL);
        if (i > 0) {
                /*
...
static void update_object_menu_from_obj(Widget menu, ABObj obj) {
        ...
        if ((palitem = pal_get_item_info(obj)) == (PalItemInfo *)NULL)
                return;
        ...
        XtVaSetValues(menu_selection, XmNlabelString, xmlabel, NULL);
...
static ABObj find_toplevel_obj() {
        ...
        for (trav_open(&trav, proj_get_project(), AB_TRAV_WINDOWS);
             (obj = trav_next(&trav)) != NULL;) {
                ...
                if (obj_is_popup_win(obj) && (pwobj == (ABObj)NULL))
                        pwobj = obj;
                if (obj_is_file_chooser(obj) && (fcobj == (ABObj)NULL))
                        fcobj = obj;
        }
        trav_close(&trav);
...
)
{
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
        ...
        /*
        ** Callback invoked when the user hits the Cancel button on the Help
        *Editor
        ** dialog.
        */
...
)
{
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
        ...
        /*
        ** Callback invoked when the user hits the Apply button on the Help
        *Editor
        ** dialog.
        */
...
)
{
    /*** DTB_USER_CODE_START vvv Add C variables and code below vvv ***/
        ...
        /*
        ** Callback invoked when the user hits the Reset button on the Help
        *Editor
        ** dialog.
        */
        if (HelpObj != (ABObj)NULL)
...
static void more_help_dispatch(Widget widget, XtPointer clientData,
        ...
        */
        ...
        if (GeneralHelpDialog == (Widget)NULL) {
                ...
                i++;
                XtSetArg(wargs[i], DtNcolumns, 100);
                i++;
                XtSetArg(wargs[i], DtNrows, 20);
                i++;
