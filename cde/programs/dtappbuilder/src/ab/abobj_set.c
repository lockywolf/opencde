...
static void get_size_params(ABObj obj, ABObj *szObjPtr, ABObj *bdObjPtr,
                            Dimension *border_w_ptr);
static void set_height_attrs(ABObj obj, ABObj szObj, ABObj bdObj,
                             int border_frame_w, Dimension border_w);
static void set_width_attrs(ABObj obj, ABObj szObj, ABObj bdObj,
                            int border_frame_w, Dimension border_w);
 ...
 */
void abobj_instantiate_changes(ABObj obj) {
        BOOL selected = FALSE;
 ...
 */
void abobj_tree_instantiate_changes(ABObj root) {
        ABSelectedRec sel;
        static BOOL new_widgets;
        int i;
...
void abobj_set_name(ABObj obj, STRING name) {
        AB_TRAVERSAL trav;
        ...
        if (util_strcmp(name, oldname) != 0) {
                if (obj_is_module(obj)) {
                        if (obj_find_by_name_and_type(project, name,
                                                      AB_TYPE_MODULE) != NULL) {
                                util_dprintf(1,
                                             "abobj_set_name: %s: another "
                                             "module already has name \"%s\"\n",
                                             obj_get_name(obj), name);
                                return;
                        }
                } else if (obj_find_by_name(module, name) != NULL) {
                        util_dprintf(1,
                                     "abobj_set_name: %s: another object "
                                     "already has name \"%s\"\n",
                                     obj_get_name(obj), name);
                        return;
                }
                 ...
                 */
                if (obj_is_module(obj) || obj_get_root_window(project) == obj)
                        abobj_set_save_needed(project, TRUE);

                if (obj_is_module(obj)) {
                        /* Since the window-parent relationship between a dialog
                         * and a main-window is stored in the main-window's
                         * module (as a list of dialog names in the form
                         * "module.dialog"), we need to check all the dialogs in
                         * this module to see if any of their window-parent's
                        ...
                        for (trav_open(&trav, obj, AB_TRAV_WINDOWS);
                             (ui_obj = trav_next(&trav)) != NULL;)
                                if (obj_is_popup_win(ui_obj)) {
                                        wp_module = obj_get_module(
                                            obj_get_win_parent(ui_obj));
                                        if (wp_module != NULL &&
                                            wp_module != obj)
                                                abobj_set_save_needed(wp_module,
                                                                      TRUE);
                                }
                        trav_close(&trav);
                } else if (obj_is_popup_win(obj)) {
                        /* If this dialog's window-parent is in a different
                         * module than this dialog, then mark that module as
                         * dirty also.
                         */
                        wp_module = obj_get_module(obj_get_win_parent(obj));
                        if (wp_module != NULL && wp_module != module)
                                abobj_set_save_needed(wp_module, TRUE);
                } else if (obj_is_menu(obj)) /* We must change menu-name field
                                                for all owners */
                {
                        AB_TRAVERSAL trav;
                        ABObj ui_obj;
                        ...
                        for (trav_open(&trav, module, AB_TRAV_SALIENT);
                             (ui_obj = trav_next(&trav)) != NULL;)
                                if (util_strcmp(obj_get_menu_name(ui_obj),
                                                oldname_copy) == 0)
...
STRING
abobj_construct_item_name(STRING prefix, STRING namebase, STRING suffix) {
        STRING name;
        STRING itemname;
...
void abobj_set_item_name(ABObj iobj, ABObj module, STRING prefix,
                         STRING namebase) {
...
void abobj_set_choice_type(ABObj obj, AB_CHOICE_TYPE type) {
        ...
        if (type != oldtype) {
                ...
                if (oldtype == AB_CHOICE_OPTION_MENU ||
                    type == AB_CHOICE_OPTION_MENU) {
                        ...
                        objxm_tree_configure(
                            obj, OBJXM_CONFIG_BUILD); /* Sets all args */
                ...
                } else /* EXCLUSIVE || NON_EXCLUSIVE */
                {
                        ...
                        for (i = 0; i < obj_get_num_children(pObj); i++) {
                                ...
                                objxm_obj_set_ui_arg(iobj, AB_ARG_LITERAL,
...
void abobj_set_menu_name(ABObj obj, STRING menuname) {
        ABObj menu_ref = NULL;
        ABObj subObj;
...
void abobj_set_menu_title(ABObj obj, STRING menu_title) {
...
void abobj_set_accelerator(ABObj obj, STRING accel) {
        ABObj subObj;
        ...
        if (util_strcmp(obj_get_accelerator(obj), new_accel) != 0) {
                ...
                objxm_obj_set_ui_arg(subObj, AB_ARG_STRING, XmNaccelerator,
                                     new_accel);
                /* REMIND: acceleratorText should be converted to different
                 * format */
                objxm_obj_set_ui_arg(
                    subObj, AB_ARG_XMSTRING, XmNacceleratorText,
                    new_accel ? (XtArgVal)XmStringCreateLocalized(accel)
                              : (XtArgVal)NULL);
                obj_set_flag(subObj, AttrChangedFlag);
        }
}
void abobj_set_mnemonic(ABObj obj, STRING mnemonic) {
        ABObj subObj;
        ...
        if (util_strcmp(obj_get_mnemonic(obj), new_mnemonic) != 0) {
                ...
                objxm_obj_set_ui_arg(
                    subObj, AB_ARG_MNEMONIC, XmNmnemonic,
                    new_mnemonic ? (XtArgVal)XStringToKeysym(new_mnemonic)
                                 : (XtArgVal)NULL);
                obj_set_flag(subObj, AttrChangedFlag);
        }
}

void abobj_set_group_type(ABObj obj, AB_GROUP_TYPE type) {
...
void abobj_set_row_alignment(ABObj obj, AB_ALIGNMENT align) {
...
void abobj_set_col_alignment(ABObj obj, AB_ALIGNMENT align) {
...
void abobj_set_row_attach_type(ABObj obj, AB_ATTACH_TYPE type) {
...
void abobj_set_col_attach_type(ABObj obj, AB_ATTACH_TYPE type) {
...
void abobj_set_row_offset(ABObj obj, int offset) {
...
void abobj_set_col_offset(ABObj obj, int offset) {
...
void abobj_set_button_type(ABObj obj, AB_BUTTON_TYPE type) {
...
void abobj_set_label_alignment(ABObj obj, AB_ALIGNMENT align) {
...
void abobj_set_label_position(ABObj obj, AB_COMPASS_POINT pos) {
...
void abobj_set_label(ABObj obj, AB_LABEL_TYPE ltype, STRING label) {
        ...
        if (util_strcmp(label, old_label) != 0 || type_change) {
                                ...
                                if (pos <= num_items) {
                                        if (obj_is_list_item(obj) ||
                                            obj_is_combo_box_item(obj))
                                                ...
                                                DtSpinBoxAddItem(parent, xmitem,
                                                                 pos);
                                        }
                                }
                                XmStringFree(xmitem);
                ...
                /* If a Label is being Added or Removed from a Menu,List,Choice
                 * or ComboBox, SpinBox, Scale or TextField, ReConfigure the Obj
                 */
                        ...
                        objxm_obj_configure(
                            obj, OBJXM_CONFIG_BUILD,
                            TRUE); /* Takes care of setting label args */
                        obj_tree_clear_flag(obj, InstantiatedFlag);
                } else {
                        obj_set_label(obj, label ? label : "");
                        objxm_comp_set_label_args(obj, OBJXM_CONFIG_BUILD);

                        /* If label-type changes for buttons, might need to
                         * Re-instantiate with new class
                         */
                        if (obj_is_button(obj) && type_change) {
                                lblObj = objxm_comp_get_subobj(
                                    obj, AB_CFG_LABEL_OBJ);

                                if ((ltype == AB_LABEL_STRING ||
                                     ltype == AB_LABEL_GLYPH) &&
                                    ab_label_is_arrow(old_ltype)) {
                                        obj_set_class_name(
                                            lblObj,
                                            obj_get_subtype(obj) == AB_BUT_DRAWN
                                                ? _xmDrawnButton
                                                : _xmPushButton);
                                        obj_clear_flag(lblObj,
                                                       InstantiatedFlag);
                                } else if (ab_label_is_arrow(ltype) &&
                                           !ab_label_is_arrow(old_ltype)) {
                                        obj_set_class_name(lblObj,
                                                           _xmArrowButton);
                                        obj_clear_flag(lblObj,
                                                       InstantiatedFlag);
                                }
...
void abobj_set_border_frame(ABObj obj, AB_LINE_TYPE btype) {
        ...
        if (old_btype != btype) {
                    ...
                    btype == AB_LINE_NONE) /* Adding or Removing border */
                {
                        ...
                        objxm_obj_configure(
                            obj, OBJXM_CONFIG_BUILD,
                            TRUE); /* Takes care of setting border args */
                        obj_tree_clear_flag(obj, InstantiatedFlag);
...
void abobj_set_line_style(ABObj obj, AB_LINE_TYPE type) {
        ABObj subObj;
...
void abobj_set_arrow_style(ABObj obj, AB_ARROW_STYLE astyle) {
        ABObj subObj;
...
void abobj_set_icon(ABObj obj, STRING icon, STRING icon_mask,
                    STRING icon_label) {
        BOOL chg = False;
        ...
        if (util_strcmp(icon_mask, obj_get_icon_mask(obj)) != 0) {
                ...
                obj_set_icon_mask(obj,
                                  util_strempty(icon_mask) ? NULL : icon_mask);
        }
 ...
 */
void abobj_set_initial_state(ABObj obj, AB_OBJECT_STATE istate) {
#ifdef DEBUG
        util_dprintf(1, "OBSOLETE ROUTINE abobj_set_initial_state() called!\n");
        util_dprintf(1,
                     "    (This works, for now, but it should be removed!)\n");
#endif /* DEBUG */
...
void abobj_set_initial_value(ABObj obj, STRING strval, int intval) {
        ABObj subObj; /* object */
        STRING old_strval;
        ...
        if (obj_is_text(obj)) {
                ...
                if (util_strcmp(old_strval, new_strval) != 0) {
                        /* A change has occurred in the module so set the save
                         * flag */
                        abobj_set_save_needed(obj_get_module(obj), TRUE);
                ...
                if (obj_get_initial_value_int(obj) != intval) {
                        ...
                        if (obj_is_spin_box(obj))
                                objxm_obj_set_ui_arg(subObj, AB_ARG_INT,
                                                     DtNposition, intval);
                        else
                                objxm_obj_set_ui_arg(subObj, AB_ARG_INT,
                                                     XmNvalue, intval);
...
void abobj_set_orientation(ABObj obj, AB_ORIENTATION orient) {
        ABObj subObj;
        ABObj lblObj;
        ...
        if (obj_get_orientation(obj) != orient) {
                ...
                if (obj_is_choice(obj)) {
                        ...
                        if (lblObj && orient == AB_ORIENT_VERTICAL) {
                                objxm_obj_set_ui_arg(
                                    subObj, AB_ARG_LITERAL,
                                    XmNentryVerticalAlignment,
                                    XmALIGNMENT_BASELINE_BOTTOM);
                                objxm_obj_set_ui_arg(lblObj, AB_ARG_INT,
                                                     XmNmarginHeight, 8);
...
void abobj_set_num_columns(ABObj obj, int num_cols) {
        ABObj subObj; /* object */
        ABObj pObj;
        ABObj lblObj;
        int old_num_cols;
        ...
        if (num_cols != old_num_cols) {
                ...
                if (num_cols != -1 &&
                    (obj_is_text(obj) || obj_is_term_pane(obj))) {
                        objxm_obj_set_ui_arg(subObj, AB_ARG_INT, XmNcolumns,
                                             num_cols);
                        /*
                        ...
                        */
                        ...
                        objxm_obj_set_ui_arg(pObj, AB_ARG_INT, XmNnumColumns,
                                             num_cols);
                        obj_set_flag(pObj, AttrChangedFlag);

                        if (num_cols > 1)
                                objxm_obj_set_literal_ui_arg(
                                    subObj, OBJXM_CONFIG_BUILD, XmNpacking,
                                    XmPACK_COLUMN);
                        else
                                objxm_obj_set_literal_ui_arg(
                                    subObj, OBJXM_CONFIG_BUILD, XmNpacking,
...
void abobj_set_num_rows(ABObj obj, int num_rows) {
        ABObj subObj; /* object */
        int old_num_rows;
...
void abobj_set_scrollbar_state(ABObj obj, AB_SCROLLBAR_POLICY scrolling) {
        int value;
        ...
        if (scrolling != obj_get_vscrollbar_policy(obj)) {
                ...
                /* If text wordwrap is ON, then NO Horizontal scrollbar is
                 * necessary */
                if (obj_is_text_pane(obj) && scroll_flag &&
        ...
        if (obj_is_drawing_area(obj) && changed) {
                ...
                objxm_obj_set_ui_arg(swObj, AB_ARG_LITERAL,
                                     XmNscrollBarDisplayPolicy, value);
                obj_set_flag(swObj, AttrChangedFlag);
        }
}

void abobj_set_show_value(ABObj obj, BOOL show_val) {
        ABObj subObj;

        if (obj_is_scale(obj) && obj_get_show_value(obj) != show_val) {
                obj_set_show_value(obj, show_val);

                subObj = objxm_comp_get_subobj(obj, AB_CFG_OBJECT_OBJ);
                objxm_obj_set_ui_arg(subObj, AB_ARG_BOOLEAN, XmNshowValue,
                                     show_val);
                obj_set_flag(subObj, AttrChangedFlag);
        }
}

void abobj_set_size_policy(ABObj obj, BOOL fixed_size) {
        BOOL selected = False;
        ABObj subObj;
        ...
        if (obj_get_width(obj) != -1 && !fixed_size ||
            obj_get_width(obj) == -1 && fixed_size) {
                ...
                if (!fixed_size) /* size NOT set by user */
                {
                         ...
                         * select grab-handles to be removed (so the object
                         * cannot be resized by direct manipulation).  It is
                         * necessary to remove the grab-handle event handling
                         * HERE (before size gets set to -1), else grab-handles
                         * are never removed.
                         */
                        if (obj_is_selected(
                                obj)) /* Temporarily turn-off selection */
                        {
                                selected = True;
                                abobj_deselect(obj);
                        }
                ...
                case AB_TYPE_DIALOG:
                        objxm_obj_set_literal_ui_arg(
                            subObj, OBJXM_CONFIG_BUILD, XmNresizePolicy,
                            fixed_size ? XmRESIZE_GROW : XmRESIZE_ANY);
                        /* fall through..*/
                case AB_TYPE_CONTAINER:
                        /* If being changed to shrink-to-fit, must
                         * re-instantiate in order for it to size around it's
                         * contents
                         */
                        if (!fixed_size)
                                obj_clear_flag(obj, InstantiatedFlag);
                ...
                case AB_TYPE_LIST:
                        ...
                        objxm_obj_set_literal_ui_arg(
                            subObj, OBJXM_CONFIG_BUILD, XmNlistSizePolicy,
                            fixed_size ? XmCONSTANT : XmVARIABLE);
...
void abobj_set_max_length(ABObj obj, int len) {
        ABObj subObj; /* Text object */

        if (obj_is_text(obj) && len != obj_get_max_length(obj)) {
                /* A change has occurred in the module so set the save flag */
                abobj_set_save_needed(obj_get_module(obj), TRUE);
...
void abobj_set_background_color(ABObj obj, STRING colorname) {
        ...
        if (util_strcmp(oldcolor, newcolor) != 0) {
                ...
                objxm_comp_set_color_args(obj, OBJXM_CONFIG_BUILD,
                                          ObjxmBackground);
...
void abobj_set_foreground_color(ABObj obj, STRING colorname) {
...
void abobj_set_read_only(ABObj obj, BOOL readonly) {
...
void abobj_set_resize_mode(ABObj obj, BOOL resizable) {
...
static void get_size_params(ABObj obj, ABObj *szObjPtr, ABObj *bdObjPtr,
        ...
        *bdObjPtr = objxm_comp_get_subobj(obj, AB_CFG_BORDER_OBJ);
        ...
        if (*szObjPtr != NULL && widget != NULL)
                XtVaGetValues(widget, XmNborderWidth, border_w_ptr, NULL);
}
...
static void set_width_attrs(ABObj obj, ABObj szObj, ABObj bdObj,
        ...
        if (width != -1) {
                ...
                if (bdObj) {
                        objxm_obj_set_ui_arg(bdObj, AB_ARG_INT, XmNwidth,
                                             width);
...
void abobj_set_pixel_width(ABObj obj, int width, int border_frame_w) {
...
static void set_height_attrs(ABObj obj, ABObj szObj, ABObj bdObj,
        ...
        if (height != -1) {
                ...
                if (bdObj) {
                        objxm_obj_set_ui_arg(bdObj, AB_ARG_INT, XmNheight,
                                             height);
                        obj_set_flag(bdObj, AttrChangedFlag);
                }
                if (obj_is_control_panel(obj) &&
                    (obj_get_container_type(obj) == AB_CONT_BUTTON_PANEL ||
                     obj_get_container_type(obj) == AB_CONT_FOOTER)) {
                        objxm_obj_set_ui_arg(bdObj ? bdObj : szObj, AB_ARG_INT,
...
void abobj_set_pixel_height(ABObj obj, int height, int border_frame_w) {
        ABObj szObj; /* Size object */
        ABObj bdObj = NULL;
        Dimension border_w;
...
void abobj_set_pixel_size(ABObj obj, int width, int height,
                          int border_frame_w) {
        ABObj szObj; /* Size object */
        ABObj bdObj = NULL;
        Dimension border_w;
...
void abobj_set_text_size(ABObj obj, int width, int height) {
        ABObj tObj;
        ...
        ui_size_to_row_col(objxm_get_widget(tObj), width, height, &num_rows,
                           &num_cols);
 ...
 */
void abobj_set_drawarea_size(ABObj obj, int width, int height) {
 ...
 */
void abobj_set_win_parent(ABObj obj, ABObj win_parent) {
        ...
        if ((old_win_parent = obj_get_win_parent(obj)) != win_parent) {
                ...
                /* If either win_parent value was NULL, then the module will be
                 * NULL
                 */
                if (old_mod)
...
void abobj_set_word_wrap(ABObj obj, BOOL wrap) {
        ABObj subObj, swObj;
...
void abobj_set_xy(ABObj obj, int x, int y) {
        ...
        if (x != -1 && y != -1 && obj->attachments != NULL) {
                ...
                if (widget != NULL) {
                        XtVaGetValues(widget, XmNwidth, &obj_width, XmNheight,
                                      &obj_height, NULL);

                        abobj_calculate_new_layout(
                            obj, x, y, obj->width < 0 ? obj_width : obj->width,
...
void abobj_set_active(ABObj obj, BOOL active) {
...
void abobj_set_visible(ABObj obj, BOOL visible) {
...
void abobj_set_iconic(ABObj obj, BOOL iconic) {
...
void abobj_set_selected(ABObj obj, BOOL selected) {
        if (obj_is_initially_selected(obj) != selected) {
                ...
                if (obj_is_choice_item(obj)) {
                        ...
                        if (obj_get_choice_type(chobj) !=
                            AB_CHOICE_OPTION_MENU) {
                                objxm_obj_set_ui_arg(obj, AB_ARG_BOOLEAN,
                                                     XmNset, selected);
                                obj_set_flag(obj, AttrChangedFlag);
                        }
                }
...
void abobj_set_selection_mode(ABObj obj, AB_SELECT_TYPE select) {
        ABObj subObj;
...
void abobj_set_tearoff(ABObj obj, BOOL tearoff) {
...
void abobj_set_text_type(ABObj obj, AB_TEXT_TYPE ttype) {
        ABObj subObj;
        ...
        if (obj_get_text_type(obj) != ttype) {
                ...
                if (obj_is_spin_box(obj)) {
                        objxm_obj_set_literal_ui_arg(
                            subObj, OBJXM_CONFIG_BUILD, DtNspinBoxChildType,
                            ttype == AB_TEXT_NUMERIC ? DtNUMERIC : XmSTRING);
...
void abobj_set_decimal_points(ABObj obj, int dec_points) {
        ABObj subObj;
        ...
        if (dec_points != obj_get_decimal_points(obj) &&
            (obj_is_scale(obj) || obj_is_spin_box(obj))) {
                ...
                objxm_obj_set_ui_arg(subObj, AB_ARG_INT, XmNdecimalPoints,
                                     dec_points);
                obj_set_flag(subObj, AttrChangedFlag);
        }
}

void abobj_set_default_act_button(ABObj obj, ABObj button) {
...
void abobj_set_direction(ABObj obj, AB_DIRECTION dir) {
        ABObj subObj;

        if (dir != obj_get_direction(obj) && obj_is_scale(obj)) {
                /* A change has occurred in the module so set the save flag */
                abobj_set_save_needed(obj_get_module(obj), TRUE);
...
void abobj_set_help_act_button(ABObj obj, ABObj button) {
...
void abobj_set_increment(ABObj obj, int incr) {
        ABObj subObj;
        ...
        if (incr != obj_get_increment(obj) &&
            (obj_is_scale(obj) || obj_is_spin_box(obj))) {
                ...
                if (obj_is_spin_box(obj))
                        objxm_obj_set_ui_arg(subObj, AB_ARG_INT,
                                             DtNincrementValue, incr);
                else if (obj_is_scale(obj))
                        objxm_obj_set_ui_arg(subObj, AB_ARG_INT,
                                             XmNscaleMultiple, incr);

                obj_set_flag(subObj, AttrChangedFlag);
        }
}

void abobj_set_min_max_values(ABObj obj, int min, int max) {
        ABObj subObj;
        ...
        if (min != obj_get_min_value(obj) ||
            max != obj_get_max_value(obj) &&
                ...
                if (obj_is_spin_box(obj)) {
                        ...
                        objxm_obj_set_ui_arg(subObj, AB_ARG_INT, XmNminimum,
                                             min);
                        objxm_obj_set_ui_arg(subObj, AB_ARG_INT, XmNmaximum,
                                             max);
                }
                obj_set_flag(subObj, AttrChangedFlag);
        }
...
int abobj_set_save_needed(ABObj obj, BOOL set) {
        ABObj module = NULL;
...
void abobj_disable_save_needed() { save_needed_enabled = FALSE; }

void abobj_enable_save_needed() { save_needed_enabled = TRUE; }

BOOL abobj_save_needed_enabled() { return (save_needed_enabled); }

void abobj_set_pattern_type(ABObj obj, AB_FILE_TYPE_MASK fmtype) {
        ABObj subObj;
        unsigned char value;
        ...
        if (old_fmtype != fmtype) {
                ...
                objxm_obj_set_ui_arg(subObj, AB_ARG_LITERAL, XmNfileTypeMask,
                                     value);
                obj_set_flag(subObj, AttrChangedFlag);
        }
}

void abobj_set_directory(ABObj obj, STRING val) {
        ABObj subObj; /* object */
        STRING oldval;
        STRING newval = NULL;
        ...
        if (obj_is_file_chooser(obj)) {
                ...
                if (!util_streq(oldval, newval)) {
                        ...
                        objxm_obj_set_ui_arg(
                            subObj, AB_ARG_XMSTRING, XmNdirectory,
                            newval ? (XtArgVal)XmStringCreateLocalized(newval)
                                   : (XtArgVal)NULL);
...
void abobj_set_filter_pattern(ABObj obj, STRING val) {
        ABObj subObj; /* object */
        STRING oldval;
        STRING newval = NULL;
        ...
        if (obj_is_file_chooser(obj)) {
                ...
                if (!util_streq(oldval, newval)) {
                        ...
                        objxm_obj_set_ui_arg(
                            subObj, AB_ARG_XMSTRING, XmNpattern,
                            newval ? (XtArgVal)XmStringCreateLocalized(newval)
                                   : (XtArgVal)NULL);
                        obj_set_flag(subObj, AttrChangedFlag);
                }
...
void abobj_set_ok_label(ABObj obj, STRING val) {
        ABObj subObj; /* object */
        STRING oldval;
        STRING newval = NULL;
        ...
        if (obj_is_file_chooser(obj)) {
                ...
                if (!util_streq(oldval, newval)) {
                        ...
                        objxm_obj_set_ui_arg(
                            subObj, AB_ARG_XMSTRING, XmNokLabelString,
                            newval ? (XtArgVal)XmStringCreateLocalized(newval)
                                   : (XtArgVal)NULL);
                        obj_set_flag(subObj, AttrChangedFlag);
                }
...
void abobj_set_auto_dismiss(ABObj obj, BOOL val) {
        ABObj subObj; /* object */
        BOOL oldval;
...
void abobj_set_sessioning_method(ABObj proj, AB_SESSIONING_METHOD ss_mthd) {
...
void abobj_set_attachment(ABObj obj, AB_COMPASS_POINT dir,
                          ABAttachment *attach) {
        ...
        obj_set_attachment(obj, dir, attach->type, attach->value,
                           attach->offset);
        objxm_obj_set_attachment_args(obj, OBJXM_CONFIG_BUILD);
...
void abobj_set_pane_min(ABObj obj, int value) {
        ABObj xyObj = NULL;
...
void abobj_set_pane_max(ABObj obj, int value) {
        ABObj xyObj = NULL;
...
void abobj_set_i18n_enabled(ABObj proj, BOOL i18n_enabled) {
