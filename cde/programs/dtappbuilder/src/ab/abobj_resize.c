...
**                                                                      **
**************************************************************************/
static void make_rect(XRectangle *new_r, XRectangle *r, int x, int y,
                      RESIZE_DIR dir);

static void undo_resize(ABUndoRec undo_rec);
void resize_in_pixels(ABObj obj, int width, int height, int border_w);

static void subtract_attached_label(ABObj obj, XRectangle *r_rect);
 ...
 */
RESIZE_DIR
abobjP_find_resize_direction(ABObj obj, Widget widget, XEvent *event) {
        ABObj rootObj;
        XRectangle w_rect;
        XRectangle temp;
        int half_handle;
        ...
        if ((obj_is_drawing_area(obj) || obj_is_text_pane(obj)) &&
            (obj_has_hscrollbar(obj) || obj_has_vscrollbar(obj))) {
                ...
                XtVaGetValues((Widget)swobj->ui_handle, XmNclipWindow, &cwidget,
                              NULL);
                ...
                XTranslateCoordinates(XtDisplay(widget), XtWindow(widget),
                                      XtWindow(objxm_get_widget(rootObj)),
                                      orig_x, orig_y, &x, &y, &win);
 ...
 */
Cursor abobjP_get_resize_cursor(Widget w, RESIZE_DIR dir) {
        static Cursor resize_cursor[10] = {NULL, NULL, NULL, NULL, NULL,
                                           NULL, NULL, NULL, NULL, NULL};
        unsigned int shape;

        if (!resize_cursor[dir]) {
                if (dir == MOVE) {
                        resize_cursor[MOVE] = x_create_stencil_cursor(
                            w, AB_mv_cursor_pixmap, AB_mv_cursor_width,
                        ...
                        resize_cursor[dir] =
                            XCreateFontCursor(XtDisplay(w), shape);
                }
        }
 ...
 */
int abobjP_resize_object_outline(ABObj obj, XEvent *event, RESIZE_DIR dir) {
        ABObj s_obj;
        ...
        if (first_move) {
                ...
                if (xy_widget == NULL) {
                        if (util_get_verbosity() > 0)
                                fprintf(stderr,
                                        "abobjP_resize_object_outline: %s :no "
                                        "SIZE widget\n",
                ...
                if (obj_has_border_frame(
                        obj)) /* We have a border-frame to deal with */
                {
                        XRectangle pane_r;
                ...
                XTranslateCoordinates(dpy, XtWindow(parent), rootwin, orig_x,
                                      orig_y, &trans_x, &trans_y, &win);
        ...
        } else /* erase previous outline */
        {
                ...
                x_fullscreen_box(xy_widget, rootwin, resize_rect.x,
                                 resize_rect.y, rect_right(&resize_rect),
                                 rect_bottom(&resize_rect));
        }

        make_rect(&resize_rect, &r, x, y, dir);
        x_fullscreen_box(xy_widget, rootwin, resize_rect.x, resize_rect.y,
                         rect_right(&resize_rect), rect_bottom(&resize_rect));

        sprintf(buf, "%3.3dx%3.3d", resize_rect.width, resize_rect.height);
 ...
 */
int abobj_resize(ABObj obj, XEvent *event) {
        Widget parent;
        Display *dpy;
        XRectangle p_rect;
        XRectangle adj_rect;
        ...
        if (!first_move) /* resize occurred */
        {
                 ...
                 */
                XTranslateCoordinates(dpy, rootwin, XtWindow(parent), orig_x,
                                      orig_y, &trans_x, &trans_y, &win);
                ...
                if (trans_x < 0) {
                        ...
                        if (obj_is_pane(obj) ||
                            obj_is_separator(
                                obj)) /* If a pane, attach to parent's edge */
                                obj_set_attachment(xy_obj, AB_CP_WEST,
                                                   AB_ATTACH_OBJ,
                                                   obj_get_parent(xy_obj), 0);
                } else {
                        resize_rect.x = trans_x;

                        /* Break edge-attachment */
                        if ((obj_is_pane(obj) || obj_is_separator(obj)) &&
                            (obj_get_attach_type(xy_obj, AB_CP_WEST) ==
                                 AB_ATTACH_OBJ &&
                             obj_get_attach_value(xy_obj, AB_CP_WEST) ==
                                 (void *)obj_get_parent(xy_obj)))
                                obj_set_attachment(xy_obj, AB_CP_WEST,
                                                   AB_ATTACH_POINT, (void *)0,
                                                   trans_x);
                }
                ...
                if (trans_y < 0) {
                        ...
                        if (obj_is_pane(obj) ||
                            obj_is_separator(
                                obj)) /* If a pane, attach to parent's edge */
                                obj_set_attachment(xy_obj, AB_CP_NORTH,
                                                   AB_ATTACH_OBJ,
                                                   obj_get_parent(xy_obj), 0);
                } else {
                        ...
                        /* Break edge-attachment */
                        if ((obj_is_pane(obj) || obj_is_separator(obj)) &&
                            (obj_get_attach_type(xy_obj, AB_CP_NORTH) ==
                                 AB_ATTACH_OBJ &&
                             obj_get_attach_value(xy_obj, AB_CP_NORTH) ==
                                 (void *)obj_get_parent(xy_obj)))
                                obj_set_attachment(xy_obj, AB_CP_NORTH,
                                                   AB_ATTACH_POINT, (void *)0,
                                                   trans_y);
                ...
                if (resize_rect.x + (short)resize_rect.width >=
                    (short)p_rect.width) {
                        resize_rect.width =
                            (short)p_rect.width - resize_rect.x - 1;

                        if (obj_is_pane(obj) ||
                            obj_is_separator(
                                obj)) /* If a pane, attach to parent's edge */
                                obj_set_attachment(xy_obj, AB_CP_EAST,
                                                   AB_ATTACH_OBJ,
                                                   obj_get_parent(xy_obj), 0);
                } else {
                        /* Break edge-attachment */
                        if ((obj_is_pane(obj) || obj_is_separator(obj)) &&
                            (obj_get_attach_type(xy_obj, AB_CP_EAST) ==
                                 AB_ATTACH_OBJ &&
                             obj_get_attach_value(xy_obj, AB_CP_EAST) ==
                                 (void *)obj_get_parent(xy_obj)))
                                obj_set_attachment(xy_obj, AB_CP_EAST,
                                                   AB_ATTACH_NONE, (void *)0,
                                                   0);
                }

                if (resize_rect.y + (short)resize_rect.height >
                    (short)p_rect.height) {
                        resize_rect.height =
                            (short)p_rect.height - resize_rect.y - 1;

                        if (obj_is_pane(obj) ||
                            obj_is_separator(
                                obj)) /* If a pane, attach to parent's edge */
                                obj_set_attachment(xy_obj, AB_CP_SOUTH,
                                                   AB_ATTACH_OBJ,
                                                   obj_get_parent(xy_obj), 0);
                } else {
                        /* Break edge-attachment */
                        if ((obj_is_pane(obj) || obj_is_separator(obj)) &&
                            (obj_get_attach_type(xy_obj, AB_CP_SOUTH) ==
                                 AB_ATTACH_OBJ &&
                             obj_get_attach_value(xy_obj, AB_CP_SOUTH) ==
                                 (void *)obj_get_parent(xy_obj)))
                                obj_set_attachment(xy_obj, AB_CP_SOUTH,
                                                   AB_ATTACH_NONE, (void *)0,
                                                   0);
                ...
                if (obj_is_drawing_area(obj) && obj_has_scrollbar(obj)) {
                        ...
                        /* If the overall DrawArea dimension is now larger than
                         * the internal canvas dimension, resize the canvas
                         * dimension to be at least as large as the overall
                         * DrawArea.
                         */
                        ...
                        if (new_d_w != -1 || new_d_h != -1)
                                abobj_set_drawarea_size(
                                    obj, new_d_w != -1 ? new_d_w : d_w,
                                    new_d_h != -1 ? new_d_h : d_h);
                }
                 ...
                 */
                if (obj_is_layers(pobj))
                        abobj_layer_set_size(pobj, resize_rect.width,
                                             resize_rect.height);

                else if ((obj_is_text(obj) || obj_is_term_pane(obj)) &&
                         obj_get_num_columns(obj) != -1)
                        /* TextPane, TermPane, TextField with Character-based
                         * Sizing */
                        abobj_set_text_size(obj, (int)adj_rect.width,
                                            (int)adj_rect.height);
                        ...
                        abobj_set_text_size(obj, (int)adj_rect.width,
                                            (int)adj_rect.height);
                        abobj_set_pixel_width(obj, (int)adj_rect.width,
                                              border_w);
                } else /* else size is Pixel-based...*/
                        resize_in_pixels(obj, (int)adj_rect.width,
                                         (int)adj_rect.height, border_w);

                abobj_set_xy(moveobj, resize_rect.x, resize_rect.y);

                if (xy_obj->attachments)
                        abobj_calculate_new_layout(
                            xy_obj, resize_rect.x, resize_rect.y,
                ...
                /* WORKAROUND for Motif XmForm bug (it ignores child resize
                 * request if x,y have not also changed). So, we have to force
                 * it.
                 */
        ...
        if (lblObj != NULL && lbl_widget != NULL) {
                ...
                case AB_CP_WEST:
                        ...
                        XtVaGetValues(XtParent(lbl_widget), /* RowColumn */
                                      XmNmarginWidth, &margin, XmNspacing,
                                      &spacing, NULL);
                ...
                case AB_CP_NORTH:
                        ...
                        XtVaGetValues(XtParent(lbl_widget), /* RowColumn */
                                      XmNmarginHeight, &margin, XmNspacing,
                                      &spacing, NULL);
...
void abobjP_track_external_resizes(Widget widget, XtPointer client_data,
        ...
        if (event->type == ConfigureNotify) {
                XConfigureEvent *cfg_event =
                    (XConfigureEvent *)&event->xconfigure;

                obj_get_size(obj, &old_w, &old_h);

                if (old_w != -1)
                        new_w = (int)cfg_event->width +
                                (2 * cfg_event->border_width);

                if (old_h != -1)
                        new_h = (int)cfg_event->height +
                                (2 * cfg_event->border_width);
...
static void undo_resize(ABUndoRec undo_rec) {
         ...
         */
        (void)abobj_set_undo(undo_rec->list, undo_rec->count, undo_resize,
                             AB_UNDO_RESIZE);
        ...
        for (i = 0; i < undo_rec->count; ++i) {
                 ...
                 */
                if (obj->ui_handle) {
                        XtVaGetValues(objxm_get_widget(obj), XmNx, &x, XmNy, &y,
                                      NULL);
                } else {
 ...
 */
BOOL abobj_is_directly_resizable(ABObj obj) {
        BOOL resizable = True;
 ...
 */
BOOL abobj_height_resizable(ABObj obj) {
        BOOL resizable = True;
 ...
 */
BOOL abobj_width_resizable(ABObj obj) {
        BOOL resizable = True;
