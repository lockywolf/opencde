...
static void set_pane_attachments(ABObj pane);
...
static AB_COMPASS_POINT attach_dirs[] = {AB_CP_NORTH, AB_CP_WEST, AB_CP_EAST,
                                         AB_CP_SOUTH};
...
ABObj abobj_handle_layered_pane(ABObj newobj, ABObj pane) {
        ...
        if (!obj_is_layers(pane_parent)) /* Create New Layered pane */
        {
                ...
                if (!obj_is_paned_win(pane_parent)) {
                        ...
                        for (i = 0; i < XtNumber(attach_dirs); i++) {
                                atype =
                                    obj_get_attach_type(pane, attach_dirs[i]);
                                aval =
                                    obj_get_attach_value(pane, attach_dirs[i]);
                                aoffset =
                                    obj_get_attach_offset(pane, attach_dirs[i]);
                                obj_set_attachment(layers, attach_dirs[i],
                                                   atype, aval, aoffset);
                        }
                } else {
                ...
                /* Reset pane's attachment's to be to all sides of the Layers
                 * object */
...
int abobj_layer_show_next(ABObj layers) {
        AB_TRAVERSAL trav;
        ABObj pane;
        ABObj old_top = NULL;
        ABObj new_top = NULL;
...
void abobj_layer_set_size(ABObj layers, int new_width, int new_height) {
        ABObj pane;
        ABObj szObj, xyObj;
        ...
        for (trav_open(&trav, layers, AB_TRAV_SALIENT_CHILDREN);
             (pane = trav_next(&trav)) != NULL;) {
                if (obj_has_border_frame(pane)) {
                        ...
                        xyObj =
                            objxm_comp_get_subobj(pane, AB_CFG_POSITION_OBJ);
                ...
                /* If Pane has "Fit Contents" size-policy, then don't set new
                 * size */
                else if (obj_get_width(pane) != -1 &&
                         obj_get_height(pane) != -1)
                        abobj_set_pixel_size(pane, new_width, new_height,
                                             border_w);
        }
        trav_close(&trav);
}
 ...
 */
int abobj_layer_manage_visible(ABObj layer) {
        AB_TRAVERSAL trav;
        ABObj pane;
...
static void set_pane_attachments(ABObj pane) {
        ...
        obj_set_attachment(pane, AB_CP_EAST, AB_ATTACH_GRIDLINE, (void *)100,
                           0);
        obj_set_attachment(pane, AB_CP_SOUTH, AB_ATTACH_GRIDLINE, (void *)100,
                           0);
}
...
static int obj_destroyedOCB(ObjEvDestroyInfo info) {
        ...
        if (obj_is_layers(layers)) {
                ...
                if (obj_get_num_salient_children(layers) == 1) {
                        /* Since there is only 1 pane left, we no longer need
                         * the layers. So, reparent the last pane to the layer's
                         * parent and destroy the layers object.
                         */
                        last_pane = obj_get_child(layers, 0);

                        /*
                         * Update undo buffer to depend on this last pane for
                         * undo instead of the parent (layer) which is about to
                         * be nuked
                         */
                        abobj_setup_undo_cut_layer(layers, last_pane);

                        /* Reparenting the last layer causes the obj_notify
                         * function to flush its queue, which in turn causes
                         * this function to be called again with the same object
                         * to destroy.  Therefore we need to track when we are
                         * in the reparent phase so that we don't get into an
                         * infinite loop...
                         */
                        obj_unparent(last_pane);
                        obj_append_child(obj_get_parent(layers), last_pane);

                        /* Copy layers object's attachments back to the last
                         * pane so that it remains positioned in the same spot.
                         */
                        for (i = 0; i < XtNumber(attach_dirs); i++) {
                                atype =
                                    obj_get_attach_type(layers, attach_dirs[i]);
                                aval = obj_get_attach_value(layers,
                                                            attach_dirs[i]);
                                aoffset = obj_get_attach_offset(layers,
                                                                attach_dirs[i]);
                                obj_set_attachment(last_pane, attach_dirs[i],
                                                   atype, aval, aoffset);
                        }
                        obj_destroy(layers);
