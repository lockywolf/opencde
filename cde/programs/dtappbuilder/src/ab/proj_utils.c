...
**                                                                      **
**************************************************************************/
static int select_fn(VNode module);

static void save_proj_as_okCB(Widget widget, XtPointer client_data,
                              XmSelectionBoxCallbackStruct *call_data);

static void export_bil_okCB(Widget widget, XtPointer client_data,
                            XmSelectionBoxCallbackStruct *call_data);

static void name_module(ABObj new_module, STRING new_module_name);

static Boolean verify_name(Widget prompt_text, XtPointer client_data);

static Widget create_save_encap_cbox(Widget parent, BOOL *EncapProj);

static Widget create_export_format_cbox(Widget parent, ABObj module_obj);

static void save_as_encap(Widget widget, XtPointer clientData,
                          XtPointer callData);

static void export_format_changed(Widget widget, XtPointer clientData,
                                  XtPointer callData);

static int save_proj_as_bip(STRING file);

static int save_proj_as_bix(STRING file);

static int proj_get_file_basename(STRING filename, STRING basename);

static Boolean export_uil_file(STRING fullpath, ABObj module_obj);

static int overwrite_bip_file(ABObj proj, STRING file, STRING old_proj_dir);

static void notify_proj_save_as_cancelCB(Widget widget, XtPointer client_data,
                                         XtPointer call_data);

static void notify_proj_save_as_okCB(Widget widget, XtPointer client_data,
                                     XtPointer call_data);
...
void proj_select(AB_OBJ *module) {
        VNode selected_mod;
        Vwr v;
        VMethods m;
...
void proj_deselect(AB_OBJ *module) {
        VNode selected_mod;
        Vwr v;
        VMethods m;
...
void proj_toggle_select(AB_OBJ *module) {
        VNode selected_mod;
        Vwr v;
        VMethods m;
...
void proj_update_node(ABObj obj) {
        Vwr v;
        VNode vnode;
...
int proj_update_stat_region(PROJ_STATUS_INFO type, STRING value) {
        Vwr proj_vwr = NULL;
        ...
        XtVaGetValues(dtb_proj_proj_main.module_drawarea, XmNuserData,
                      &proj_vwr, NULL);
        ...
        if (path_value != NULL) {
                ...
                XtVaSetValues(path_value, XtVaTypedArg, XmNlabelString,
                              XtRString, value, strlen(value) + 1, NULL);
        }

        return 0;
}

void proj_show_save_proj_as_chooser(Widget widget,
                                    ProjSaveCallbackFunc saveDoneCB) {
        Widget textf = NULL, encap_cbox = NULL;
        ...
        if (AB_generic_chooser == NULL) {
                AB_generic_chooser = XmCreateFileSelectionDialog(
                    widget, "AB_generic_chooser", (ArgList)NULL, (Cardinal)0);

                info = (ChooserInfo)calloc(sizeof(ChooserInfoRec), 1);
                XtVaSetValues(AB_generic_chooser, XmNuserData, (XtPointer)info,
                              NULL);
        ...
        encap_cbox = create_save_encap_cbox(AB_generic_chooser,
                                            &(info->writeEncapsulated));

        info->child = (Widget)encap_cbox;
        info->chooser_type = AB_SAVE_PROJ_AS_CHOOSER;

        ok_label = XmStringCreateLocalized(
            catgets(Dtb_project_catd, 100, 237, "Save"));
        XtVaSetValues(AB_generic_chooser, XmNautoUnmanage, FALSE, XtVaTypedArg,
                      XmNpattern, XtRString, "*.bip", sizeof("*.bip"),
                      XmNokLabelString, ok_label, XmNokCallback, &ok_callback,
                      XmNcancelCallback, &cancel_callback, NULL);
        XmStringFree(ok_label);

        title =
            XtNewString(catgets(Dtb_project_catd, 100, 238, "Save Project"));
        XtVaSetValues(XtParent(AB_generic_chooser), XmNtitle, title,
                      XmNpopdownCallback, &popdown_callback, NULL);
        XtFree(title);
        textf = XmFileSelectionBoxGetChild(AB_generic_chooser, XmDIALOG_TEXT);
...
static void save_proj_as_okCB(Widget widget, XtPointer client_data,
        ...
        */
        if (util_strempty(proj_filename)) {
                ...
                (void)dtb_show_modal_message(widget, &dtb_proj_no_name_msg,
                                             NULL, NULL, NULL);
                return;
        }
        if (util_file_is_directory(proj_filename)) {
                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 71,
                ...
                help_data = (DtbObjectHelpData)util_malloc(
                    sizeof(DtbObjectHelpDataRec));
                help_data->help_text = catgets(
                    Dtb_project_catd, 100, 85,
                    "The file name you specified for saving the project\nis a "
                    "directory. Type another file name for the project.");
                help_data->help_volume = "";
                help_data->help_locationID = "";

                (void)dtb_show_modal_message(widget, &dtb_proj_error_msg,
        ...
        if (iRet == 0) {
                if (write_OK) {
                        ...
                        if (ab_c_ident_is_ok(proj_name)) {
                                XtVaGetValues(widget, XmNuserData, &chooserInfo,
                                              NULL);
                                ...
                                /* If the project name is illegal (i.e. x.y.z),
                                 * then post a message informing the user and
                                 * leave the chooser up to give him/her another
                                 * try.
                                 */
                                dtb_proj_save_proj_as_msg_initialize(
                                    &dtb_proj_save_proj_as_msg);
                                (void)dtb_show_modal_message(
                                    widget, &dtb_proj_save_proj_as_msg, NULL,
                                    NULL, NULL);
                        }
                } else {
                        ...
                        if (chooserInfo->writeEncapsulated) {
                                sprintf(Buf,
                                        catgets(Dtb_project_catd, 100, 77,
                                                "Cannot save encapsulated "
                                                "project to \"%s\".\n\"%s\" "
                                                "does not have write "
                                                "permission."),
                                        proj_dir, proj_dir);
                                ...
                                sprintf(Buf,
                                        catgets(Dtb_project_catd, 100, 74,
                                                "Cannot save project to "
                                                "\"%s\".\n\"%s\" does not have "
                                                "write permission."),
                                        proj_dir, proj_dir);
                        }
                        ...
                        util_set_help_data(
                            catgets(Dtb_project_catd, 100, 83,
                                    "The directory you specified for saving is "
                                    "not\nwritable by you. Therefore, the "
                                    "project cannot\nbe saved.  Try saving the "
...
#define MSG                                                                    \
        "You are trying to save the new project file (%s)\nin the same "       \
        "directory as the old project file (%s).\nThis will cause the same "   \
        "modules to be used in both the old\nproject and in the new project. " \
        "Click OK to continue with\nthis option.\n\nTo create a separate "     \
        "version of the project %s and its\nmodules, you must save to a "      \
        "different directory. Click\nCancel now, then select a different "     \
        "directory in the file\nselection dialog."

#define HLP_MSG                                                                \
        "When choosing Save Project As, you must save the project\nfile in a " \
        "new directory if you want to save a separate\nversion of the "        \
        "project file (.bip) and each module file\n(.bil). Otherwise, the "    \
        "modules from the old version will\nbe overwritten with any changes "  \
        "from the new version. Both\nproject files will reference the same "   \
        "module files.\n\nApp Builder recommends one project per directory. "  \
        "When code\nis generated for the project, a Makefile is produced. "    \
        "There\ncan be only one Makefile per directory. If there are "         \
        "multiple\nprojects in one directory, the Makefile will be "           \
        ...
        *new_proj_dir = 0;
        ...
        if (ProjHasBeenSaved && !changed_dir && !EncapProj) {
                ...
                *old_file = 0;
                *new_file = 0;
                util_get_file_name_from_path(obj_get_file(project), old_file,
                                             MAXPATHLEN);
                util_get_file_name_from_path(new_filename, new_file,
                                             MAXPATHLEN);
                new_name_len = strlen(new_file);
                old_name_len = strlen(old_file);
                msg_len = strlen(catgets(Dtb_project_catd, 100, 101, MSG));
                msg = (STRING)util_malloc((2 * old_name_len) + new_name_len +
                                          msg_len + 1);

                help_data.help_text =
                    catgets(Dtb_project_catd, 100, 102, HLP_MSG);
                ...
                if (msg) {
                        ...
                        answer = dtb_show_modal_message(
                            AB_generic_chooser, &dtb_proj_wrn_msg, xm_msg,
                            &help_data, NULL);
                        util_free(msg);
                        XmStringFree(xm_msg);
                } else {
                        util_dprintf(1, "save_proj_as_bip: could not malloc "
                                        "space for message.");
        ...
        if (answer == DTB_ANSWER_ACTION1) {
                ...
                iRet =
                    proj_save_exploded(project, new_filename, TRUE, EncapProj);
...
void proj_show_name_dlg(ABObj module, Widget parent) {
        ...
        if (module) {
                ...
                init_name =
                    obj_alloc_unique_name_for_child(project, "module", -1);
        }
        ...
        if (parent != AB_toplevel) {
                ...
                XtSetSensitive(dtb_palette_ab_palette_main.build_test_choice1,
                               FALSE);
        ...
        help_data->help_text = XtNewString(catgets(
            Dtb_project_catd, 100, 41,
            "Type in a name for the new module. A valid name\ncan consist of "
            "letters, underscore (_), and digits,\nhowever the name must begin "
            "with either a letter or\nan underscore."));
        help_data->help_volume = "";
        help_data->help_locationID = "";

        dialog_title = XtNewString(
            catgets(Dtb_project_catd, 100, 40, "Application Builder"));
        textf_lbl =
            XtNewString(catgets(Dtb_project_catd, 100, 111, "Module Name:"));
        ...
        answer = ui_popup_modal_prompt(parent, dialog_title, textf_lbl,
                                       init_name, btn1_lbl, btn2_lbl, btn3_lbl,
                                       help_data, XmDIALOG_OK_BUTTON, &seln_str,
        ...
        case UI_ANSWER_CANCEL:
                if (parent != AB_toplevel) {
                        ...
                        XtSetSensitive(
                            dtb_palette_ab_palette_main.build_test_choice1,
                            TRUE);
...
void proj_show_export_bil_chooser(Widget widget, ABObj obj) {
        Widget textf;
        ...
        ok_label = XmStringCreateLocalized(
            catgets(Dtb_project_catd, 100, 243, "Export"));
        XtVaSetValues(AB_generic_chooser, XmNautoUnmanage, FALSE, XtVaTypedArg,
                      XmNpattern, XtRString, "*.bil", sizeof("*.bil"),
                      XmNokLabelString, ok_label, XmNokCallback, &ok_callback,
                      XmNcancelCallback, &cancel_callback, NULL);
        XmStringFree(ok_label);

        title =
            XtNewString(catgets(Dtb_project_catd, 100, 239, "Export Module"));
        XtVaSetValues(XtParent(AB_generic_chooser), XmNtitle, title,
                      XmNpopdownCallback, &popdown_callback, NULL);
        XtFree(title);
        textf = XmFileSelectionBoxGetChild(AB_generic_chooser, XmDIALOG_TEXT);
...
static void export_bil_okCB(Widget widget, XtPointer client_data,
        ...
        */
        ...
        if (util_file_is_directory(fullpath)) {
                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 72,
                ...
                help_data = (DtbObjectHelpData)util_malloc(
                    sizeof(DtbObjectHelpDataRec));
                help_data->help_text = catgets(
                    Dtb_project_catd, 100, 84,
                    "The file name you specified for exporting the\nmodule is "
                    "a directory. Type another file name\nfor the module.");
                help_data->help_volume = "";
                help_data->help_locationID = "";
                ...
                if (write_OK) {
                        if (ExportAsUIL) {
                                if (export_uil_file(fullpath, module_obj)) {
                                        /* Export succeeded.  Take down the
                                         * chooser */
                                        XtUnmanageChild(AB_generic_chooser);
                                }
                        } else {
                                ...
                                if (ab_c_ident_is_ok(mod_name)) {
                                         ...
                                         * the chooser.
                                         */
                                        projP_save_export_bil(module_obj,
                                                              fullpath, TRUE);
                                } else {
                                        /* If the module name is illegal (i.e.
                                         * x.y.z), then post a message informing
                                         * the user and leave the chooser up to
                                         * give him/her another try.
                                         */
                                        dtb_proj_save_as_mod_msg_initialize(
                                            &dtb_proj_save_as_mod_msg);
                                        (void)dtb_show_modal_message(
                                            widget, &dtb_proj_save_as_mod_msg,
                                            NULL, NULL, NULL);
                                }
                        }
                } else {
                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 78,
                                        "Cannot export module to "
                                        "\"%s\".\n\"%s\" does not have write "
                                        "permission."),
                                mod_dir, mod_dir);
                        xm_buf = XmStringCreateLocalized(Buf);
                        dtb_proj_error_msg_initialize(&dtb_proj_error_msg);
                        ...
                        help_data = (DtbObjectHelpData)util_malloc(
                            sizeof(DtbObjectHelpDataRec));
                        help_data->help_text =
                            catgets(Dtb_project_catd, 100, 86,
                                    "The directory you specified for exporting "
                                    "is not\nwritable by you. Therefore, the "
                                    "module cannot be\nexported. Try exporting "
                                    "the module to another\ndirectory.");
                        help_data->help_volume = "";
                        help_data->help_locationID = "";
 ...
 */
int proj_overwrite_existing_file(char *file, ABObj obj, BOOL encapsulating,
                                 BOOL exporting, BOOL changed_dir,
                                 STRING old_proj_dir,
        ...
        if (!write_OK) {
                if (!encapsulating) {
                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 3,
                                        "File \"%s\" exists and\nis a "
                                        "read-only file. You can save "
                                        "your\nwork by saving to a different "
                                        "filename."),
                                file);
                } else {
                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 53,
                                        "File \"%s\" exists and\nis a "
                                        "read-only file. To save the "
                                        "encapsulated file,\nyou must save it "
                ...
                util_set_help_data(
                    catgets(Dtb_project_catd, 100, 79,
                            "The file name you specified for saving already "
                            "exists\nand cannot be overwritten, since it is "
                            "not writable by\nyou. Your edits can be saved if "
                            "you type a different\nfile name."),
                    NULL, NULL);
        ...
        sprintf(
            Buf,
            catgets(Dtb_project_catd, 100, 4, "File \"%s\" exists.  You can:"),
            file);
        xm_buf = XmStringCreateLocalized(Buf);

        dtb_proj_overwrite_msg_initialize(&dtb_proj_overwrite_msg);
        *answer =
            dtb_show_modal_message(dtb_get_toplevel_widget(),
        ...
        case DTB_ANSWER_ACTION1:
                ...
                if (obj_is_project(obj)) {
                        if (encapsulating)
                                ...
                                iRet =
                                    overwrite_bip_file(obj, file, old_proj_dir);
                } else
...
void proj_save_needed(ProjSaveCallbackFunc saveDoneCB) {
        ABObj project = proj_get_project();
	...
	It will be saved as a normal project, with one file per module."),
        ...
        if (!projHasBeenSaved || encapFile) {
                 ...
                 */
                proj_show_save_proj_as_chooser(dtb_get_toplevel_widget(),
                                               saveDoneCB);
        } else {
                /* Check if we're allowed to write to the directory
                 * specified for the project file.
                ...
                if (iRet == 0) {
                        if (write_OK) {
                                ...
                                if (saveDoneCB != NULL)
                                        saveDoneCB(iRet >= 0 ? AB_SAVE_NORMAL
                                                             : iRet);
                        } else {
                                sprintf(Buf,
                                        catgets(Dtb_project_catd, 100, 74,
                                ...
                                util_set_help_data(
                                    catgets(Dtb_project_catd, 100, 83,
                                            ...
                                            "saved. Try saving the project to "
                                            "another directory."),
                                    NULL, NULL);
 ...
 */
int proj_set_files_exploded(ABObj project) {
        AB_TRAVERSAL trav;
        ABObj module = NULL;
...
int proj_unset_files_exploded(ABObj project) {
        AB_TRAVERSAL trav;
        ABObj module = NULL;
        STRING projFileName = obj_get_file(project);
...
BOOL proj_is_exploded(ABObj project) {
        AB_TRAVERSAL trav;
        ABObj module = NULL;
        STRING projFileName = obj_get_file(project);
...
int proj_save_encapsulated(ABObj obj, STRING file_name) {
        ...
        *proj_dir = 0;
        ...
        if (rc == 0) {
                if (write_OK) {
                        ...
                        sprintf(Buf,
                                catgets(Dtb_project_catd, 100, 77,
                                        "Cannot save encapsulated project to "
                                        "\"%s\".\n\"%s\" does not have write "
                                        "permission."),
                                proj_dir, proj_dir);
...
int proj_save_exploded(ABObj project, char *new_filename, BOOL save_as,
                       BOOL encap_file) {
        int rc = 0; /* return code */
        ...
        DTB_MODAL_ANSWER answer = DTB_ANSWER_NONE;
        BOOL DiffFile = TRUE;
        char tmp_path[MAXPATHLEN], mod_exp_file[MAXPATHLEN];
        ABObj module;
        AB_TRAVERSAL trav;
        int iRet = 0;
        XmString xm_buf = (XmString)NULL;
        ...
        *mod_exp_file = 0;
        ...
        if (obj_get_name(project) != NULL) {
                old_name =
                    (STRING)util_malloc(strlen(obj_get_name(project)) + 1);
                strcpy(old_name, obj_get_name(project));
        }
        if (obj_get_file(project) != NULL) {
                old_file =
                    (STRING)util_malloc(strlen(obj_get_file(project)) + 1);
                strcpy(old_file, obj_get_file(project));
                old_proj_dir =
                    (STRING)util_malloc(strlen(obj_get_file(project)));
                util_get_dir_name_from_path(obj_get_file(project), old_proj_dir,
                                            strlen(obj_get_file(project)));
        }
         ...
         */
        for (trav_open_cond(&trav, project, AB_TRAV_MODULES,
                ...
                if ((obj_get_file(module) == NULL) || encap_file) {
                        ...
                        if (!util_file_exists(mod_exp_file)) {
                                if (errno == ENOENT) {
                                        ...
                                        /* Update all the UI components, such as
                                         * the the project window and  browser.
                                         */
                                        proj_rename_module(
                                            module, obj_get_name(module));
                                } else /* An error occurred with stat */
                                {
                                        obj_set_name(project, old_name);
                                        sprintf(Buf, "%s: %s", mod_exp_file,
                                                strerror(errno));
                                        util_printf_err(Buf);
                                        if (old_name != NULL)
                                                util_free(old_name);
                         ...
                         */
                                ...
                                iRet = proj_overwrite_existing_file(
                                    mod_exp_file, module, FALSE, FALSE, FALSE,
                                    old_proj_dir, &answer);
                                if ((iRet < 0) ||
                                    (answer != DTB_ANSWER_ACTION1)) {
                                        obj_set_name(project, old_name);
                                        if (encap_file) {
                                                proj_unset_files_exploded(
                                                    project);
                                        }
                                        XtUnmanageChild(AB_generic_chooser);
                                        if (old_name != NULL)
                        ...
                        if (obj_has_flag(module, SaveNeededFlag) || save_as) {
                                if (save_as) {
                                        ...
                                        *dir = 0;
                                        ...
                                        /* We need to get the relative path of
                                         * the bil file because the project
                                         * directory (and thus the current
                                         * directory) may have changed.
                                         * Therefore the file field can change
                                         * for the module obj.
                                         */
                                        rel_path =
                                        ...
                                        /* If the module was stored to a
                                         * subdirectory, make the same
                                         * subdirectory hierarchy underneath the
                                         * new project directory, if necessary.
                                         */
                                        util_get_dir_name_from_path(
                                            obj_get_file(module), dir,
                                            MAXPATHLEN);
                                        if (!util_path_is_absolute(dir) &&
                                            !ab_is_cur_dir(dir)) {
                                                if ((iRet = util_mkdir_hier(
                                                         dir)) < 0) {
                                                        obj_set_name(project,
                                                                     old_name);
                                                        sprintf(
                                                            Buf, "%s: %s", dir,
                                                            strerror(errno));
                                 ...
                                 * directory) even if the module doesn't need
                                 * saving because it could have been saved out
                                 * first to the current directory and thereby
        ...
        if (obj_has_flag(project, SaveNeededFlag) || save_as) {
                ...
                if (!util_paths_are_same_file(old_file, new_filename) &&
                    util_file_exists(new_filename)) {
                         ...
                         */
                        iRet = proj_overwrite_existing_file(
                            new_filename, project, FALSE, FALSE, FALSE,
                            old_proj_dir, &answer);
...
BOOL proj_check_unsaved_edits(ABObj root) {
        ABObj obj = NULL;
        AB_TRAVERSAL trav;
...
static void name_module(ABObj new_module, STRING new_module_name) {
        ...
        if (new_module == NULL) {
                ...
                obj_ensure_unique_name(new_module, obj_get_project(new_module),
                                       -1);
...
void proj_create_new_proj(STRING proj_name) {
        ABObj new_proj = NULL;
...
int proj_show_proj_dir(void) {
        ...
        if (util_get_dir_name_from_path(projPath, dir_buf, MAXPATHLEN) ==
            NULL) {
                return -1;
        }
...
void proj_name_proj() {
        ...
        dialog_title = XtNewString(
            catgets(Dtb_project_catd, 100, 40, "Application Builder"));
        textf_lbl =
            XtNewString(catgets(Dtb_project_catd, 100, 107, "Project Name:"));
        ...
        answer = ui_popup_modal_prompt(
            dtb_get_toplevel_widget(), dialog_title, textf_lbl, def_name,
            btn1_lbl, btn2_lbl, btn3_lbl, help_data, XmDIALOG_OK_BUTTON,
        ...
        if (util_strempty(seln_str)) {
                if (ObjIsModule) {
                        ...
                        dtb_show_message(prompt_text,
                                         &dtb_proj_vrfy_mod_name_msg, NULL,
                                         NULL);
                } else {
                        ...
                        dtb_show_message(prompt_text,
                                         &dtb_proj_vrfy_proj_name_msg, NULL,
                                         NULL);
                }
                retval = False;
        } else {
...
static Widget create_save_encap_cbox(Widget parent, BOOL *EncapProj) {
        ...
        label_xmstring = XmStringCreateLocalized(catgets(
            Dtb_project_catd, 100, 240, "Save As Encapsulated Project"));
        encap_toggle = XtVaCreateManagedWidget(
            "encap_toggle", xmToggleButtonWidgetClass, parent, XmNlabelString,
            label_xmstring, XmNspacing, 0, XmNuserData, (XtPointer)parent,
            NULL);
        XtAddCallback(encap_toggle, XmNvalueChangedCallback, save_as_encap,
                      (XtPointer)EncapProj);
...
static Widget create_export_format_cbox(Widget parent, ABObj obj) {
        Widget format_toggle = NULL;
        XmString label_xmstring = NULL;

        label_xmstring = XmStringCreateLocalized(
            catgets(Dtb_project_catd, 100, 241, "Save As UIL"));
        format_toggle = XtVaCreateManagedWidget(
            "format_toggle", xmToggleButtonWidgetClass, parent, XmNlabelString,
            label_xmstring, XmNspacing, 0, XmNuserData, (XtPointer)obj, NULL);
        XtAddCallback(format_toggle, XmNvalueChangedCallback,
                      export_format_changed, (XtPointer)parent);
        XmStringFree(label_xmstring);
        label_xmstring = NULL;
...
static void save_as_encap(Widget widget, XtPointer clientData,
        ...
        Widget fchooser = NULL, textf = NULL;
        char init_name[MAXPATHLEN];
...
static void export_format_changed(Widget widget, XtPointer clientData,
        ...
        ** exported.  If a filename is specified, it might be either .bil or
        *.uil.
        */
        if (util_strempty(filename))
        ...
        if (abio_expand_bil_encapsulated_path(bix_file) != -1) {
                if (!util_file_exists(bix_file)) {
                        if (errno == ENOENT) {
                                ...
                                XtUnmanageChild(AB_generic_chooser);
                                iRet = proj_save_encapsulated(
                                    proj_get_project(), bix_file);
                        } else /* An error occurred with stat */
                        {
                                sprintf(Buf, "%s: %s", bix_file,
                                        strerror(errno));
                                xm_buf = XmStringCreateLocalized(Buf);
                                dtb_proj_error_msg_initialize(
                                    &dtb_proj_error_msg);
                                (void)dtb_show_modal_message(
                ...
                {
                        iRet = proj_overwrite_existing_file(
                            bix_file, proj_get_project(), TRUE, TRUE, FALSE,
                            NULL, &answer);
...
static Boolean export_uil_file(STRING fullpath, ABObj module_obj) {
        ...
        if (!util_file_exists(fullpath)) {
                if (errno == ENOENT) {
                        ...
                        if ((fp = util_fopen_locked(fullpath, "w")) !=
                            (FILE *)NULL) {
                ...
                */
                sprintf(Buf,
                        catgets(Dtb_project_catd, 100, 4,
                                "File \"%s\" exists.  You can:"),
                        fullpath);
                xm_buf = XmStringCreateLocalized(Buf);
                dtb_proj_overwrite_msg_initialize(&dtb_proj_overwrite_msg);
                answer = dtb_show_modal_message(dtb_get_toplevel_widget(),
                                                &dtb_proj_overwrite_msg, xm_buf,
                                                NULL, NULL);
                XmStringFree(xm_buf);
 ...
 */
STRING
proj_cvt_mod_file_to_rel_path(STRING mod_path, STRING proj_path) {
        char mod_name[MAXPATHLEN];
        char mod_dir[MAXPATHLEN];
        char proj_dir[MAXPATHLEN];
        ...
        *rel_dir = 0;
        ...
        if (proj_path != NULL) {
                util_get_dir_name_from_path(proj_path, proj_dir, MAXPATHLEN);
                iRet = util_cvt_path_to_relative(mod_dir, proj_dir, rel_dir,
                                                 MAXPATHLEN);
                if (iRet == ERR) {
                        return NULL;
                }
        ...
        if (util_streq(rel_dir, ".")) {
                ...
                rel_path =
                    (char *)util_malloc(strlen(mod_name) + strlen(rel_dir) + 5);
                sprintf(rel_path, "%s%s.bil", rel_dir, mod_name);
        } else {
                rel_path =
                    (char *)util_malloc(strlen(mod_name) + strlen(rel_dir) + 6);
                sprintf(rel_path, "%s/%s.bil", rel_dir, mod_name);
        }
...
void proj_set_menus(AB_CHOOSER_TYPE chooser_type, BOOL active) {
        Vwr proj_vwr = NULL;
        VNode *selected_nodes = NULL;
        int num_selected = 0;
        Widget open_proj_item =
            dtb_proj_proj_main.menubar_Project_item_proj_menubutton_menu_items
                .Open_item;
        ...
        Widget save_proj_item =
            dtb_proj_proj_main.menubar_Project_item_proj_menubutton_menu_items
                .Save_item;
        Widget import_item =
            dtb_proj_proj_main.menubar_Module_item_mod_menubutton_menu_items
                .Import_item;
        Widget save_mod_as_item =
            dtb_proj_proj_main.menubar_Module_item_mod_menubutton_menu_items
                .Save_As_item;
        Widget save_mod_item =
            dtb_proj_proj_main.menubar_Module_item_mod_menubutton_menu_items
                .Save_item;
        Widget export_item =
            dtb_proj_proj_main.menubar_Module_item_mod_menubutton_menu_items
         ...
         */
        XtVaGetValues(open_proj_item, XmNuserData, &proj_vwr, NULL);
        if (!proj_vwr)
                return;
         ...
         */
        vwr_get_cond(proj_vwr->current_tree, &selected_nodes, &num_selected,
                     select_fn);
...
Boolean proj_verify_name(STRING new_name, BOOL ObjIsModule,
                         DtbMessageData msgData, Widget parent, ABObj newObj) {
        Boolean retval = TRUE;
        XmString xm_buf = (XmString)NULL;
        ABObj obj = NULL;
        ...
        {
                if (ObjIsModule) {
                        obj = obj_find_by_name_and_type(
                            proj_get_project(), new_name, AB_TYPE_MODULE);
                        ...
                        if ((obj != NULL) && (obj != newObj)) {
                                sprintf(Buf,
                                        catgets(Dtb_project_catd, 100, 6,
                                                "%s is not a unique name for "
                                                "the module."),
                                        new_name);
                                xm_buf = XmStringCreateLocalized(Buf);
