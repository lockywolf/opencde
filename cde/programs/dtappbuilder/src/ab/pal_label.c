 ...
 * Methods
 */
static int label_initialize(ABObj obj);
static Widget label_prop_init(Widget parent, AB_PROP_TYPE type);
static int label_prop_activate(AB_PROP_TYPE type, BOOL active);
static int label_prop_clear(AB_PROP_TYPE type);
static int label_prop_load(ABObj obj, AB_PROP_TYPE type, unsigned long loadkey);
static int label_prop_apply(AB_PROP_TYPE type);
static BOOL label_prop_pending(AB_PROP_TYPE type);
...
static Widget label_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbLabelPropDialogInfo cgen =
            &dtb_label_prop_dialog; /* Codegen structure */
        ...
        if (dtb_label_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_label_palitem, cgen->ok_button,
                ...
                /* Label */
                prop_field_init(&(pls->label), cgen->label_label,
                                cgen->label_textpane, cgen->label_cb);
                ...
                n++;
                prop_options_init(
                    &(pls->label_align), cgen->labelalign_opmenu_label,
                ...
                n++;
                prop_radiobox_init(&(pls->size_policy),
                                   cgen->szpolicy_rbox_label,
                                   cgen->szpolicy_rbox, n, item,
                                   (XtPointer *)item_val, cgen->szpolicy_cb);

                for (i = 0; i < n; i++)
                        XtAddCallback(item[i], XmNvalueChangedCallback,
                                      (XtCallbackProc)prop_size_policyCB,
                                      (XtPointer) & (pls->geometry));
...
static int label_prop_clear(AB_PROP_TYPE type) {
        ...
        prop_options_set_value(&(pls->label_align), (XtPointer)AB_ALIGN_CENTER,
                               False);
...
static int label_prop_load(ABObjPtr obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_options_set_value(&(pls->label_align),
...
int label_prop_apply(AB_PROP_TYPE type) {
        PropLabelSettingsRec *pls = &(prop_label_settings_rec[type]);
        STRING value;
        BOOL size_chg = False;
        BOOL reset_bg = False;
        ...
        if (prop_changed(pls->geometry.changebar)) {
                if (abobj_width_resizable(pls->current_obj)) {
                        new_w = prop_geomfield_get_value(&(pls->geometry),
                                                         GEOM_WIDTH);
                        abobj_set_pixel_width(pls->current_obj, new_w, 0);
                }
                if (abobj_height_resizable(pls->current_obj)) {
                        new_h = prop_geomfield_get_value(&(pls->geometry),
                                                         GEOM_HEIGHT);
        ...
        prop_geomfield_set_value(&(pls->geometry), GEOM_HEIGHT,
                                 abobj_get_comp_height(pls->current_obj),
                                 False);
...
static BOOL label_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_label_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pls->geometry.changebar) &&
            (!prop_number_ok(pls->geometry.x_field, (STRING)XFieldStr,
                             -SHRT_MAX, SHRT_MAX) ||
             !prop_number_ok(pls->geometry.y_field, (STRING)YFieldStr,
                             -SHRT_MAX, SHRT_MAX) ||
             !prop_number_ok(pls->geometry.w_field, (STRING)WFieldStr, 1,
                             SHRT_MAX) ||
             !prop_number_ok(pls->geometry.h_field, (STRING)HFieldStr, 1,
                             SHRT_MAX)))
                return False;
...
static void size_policyCB(Widget w, XtPointer clientdata,
        ...
        if (state->set) {
                ...
                ui_field_set_editable(pls->geometry.w_field,
                                      value == SIZE_FIXED_KEY);
                ui_field_set_editable(pls->geometry.h_field,
                                      value == SIZE_FIXED_KEY);
        }
}
