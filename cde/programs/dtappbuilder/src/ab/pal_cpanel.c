 ...
 * Methods
 */
static int cpanel_initialize(ABObj obj);
static Widget cpanel_prop_init(Widget parent, AB_PROP_TYPE type);
static int cpanel_prop_activate(AB_PROP_TYPE type, BOOL active);
static int cpanel_prop_clear(AB_PROP_TYPE type);
static int cpanel_prop_load(ABObj obj, AB_PROP_TYPE type,
                            unsigned long loadkey);
static int cpanel_prop_apply(AB_PROP_TYPE type);
static BOOL cpanel_prop_pending(AB_PROP_TYPE type);
...
static int cpanel_initialize(ABObj obj) {
        ...
        default: /* Standard Control Pane */
                ...
                /* If control pane was added as a layer, the size has already
                 * been set by the size of the layer.
                 */
...
static Widget cpanel_prop_init(Widget parent, AB_PROP_TYPE type) {
        ...
        DtbCpanelPropDialogInfo cgen =
            &dtb_cpanel_prop_dialog; /* Codegen structure */
        ...
        if (dtb_cpanel_prop_dialog_initialize(cgen, parent) == 0) {
                ...
                if (type == AB_PROP_FIXED) {
                        ...
                        prop_activate_panel_init(
                            type, ab_cpanel_palitem, cgen->ok_button,
                ...
                prop_menuname_init(&(pcs->menuname), type, cgen->menu_label,
                                   ...
                                   &(pcs->menu_title), &(pcs->current_obj),
                                   True);
...
static int cpanel_prop_load(ABObj obj, AB_PROP_TYPE type,
        ...
        if (load_all) {
                ...
                prop_options_set_value(
                    &(pcs->frame), (XtPointer)obj_get_border_frame(obj), False);

                /* Load Menu Name/Title */
                prop_menuname_set_value(&(pcs->menuname),
                                        obj_get_menu_name(obj), False);
                prop_field_set_value(&(pcs->menu_title),
                ...
                /* For now we will not allow the user to manipulate the "Size
                 * Policy" of specialized Control Panels; they will be "Fit
                 * Contents".
                 */
...
int cpanel_prop_apply(AB_PROP_TYPE type) {
        ABObj pobj;
        PropCPanelSettingsRec *pcs = &(prop_cpanel_settings_rec[type]);
        ...
        if (prop_changed(pcs->menuname.changebar)) {
                ...
                util_free(value);
                prop_menuname_set_value(&(pcs->menuname),
                                        obj_get_menu_name(pcs->current_obj),
                                        False);
        }
        ...
        if (prop_changed(pcs->geometry.changebar)) {
                ...
                if (obj_is_layers(pobj) && (new_h != -1 || new_w != -1))
                        abobj_layer_set_size(pobj, new_w, new_h);
                else {
                        if (new_w != -1)
                                abobj_set_pixel_width(pcs->current_obj, new_w,
                                                      0);
                        if (new_h != -1)
                                abobj_set_pixel_height(pcs->current_obj, new_h,
                                                       0);
                }
...
static BOOL cpanel_prop_pending(AB_PROP_TYPE type) {
        return (
            prop_changebars_pending(prop_cpanel_settings_rec[type].prop_sheet));
}
...
static BOOL verify_props(AB_PROP_TYPE type) {
        ...
        if (prop_changed(pcs->geometry.changebar) &&
            (!prop_number_ok(pcs->geometry.x_field, (STRING)XFieldStr,
                             -SHRT_MAX, SHRT_MAX) ||
             !prop_number_ok(pcs->geometry.y_field, (STRING)YFieldStr,
                             -SHRT_MAX, SHRT_MAX) ||
        ...
        if (prop_changed(pcs->menuname.changebar) &&
            !prop_obj_name_ok(pcs->menuname.field,
                              obj_get_module(pcs->current_obj), AB_TYPE_MENU,
                              "Menu"))
                return False;
