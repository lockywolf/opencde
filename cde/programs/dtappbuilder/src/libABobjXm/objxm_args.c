...
int objxmP_set_attachment_arg(ABObj obj, OBJXM_CONFIG_TYPE ctype,
                              ABObj parent_obj, AB_COMPASS_POINT dir);
 ...
 * starting at root
 */
int objxm_tree_set_ui_args(ABObj root, OBJXM_CONFIG_TYPE ctype, BOOL replace) {
        AB_TRAVERSAL trav;
        ABObj obj;
 ...
 * starting at root
 */
int objxm_tree_remove_ui_args(ABObj root, OBJXM_CONFIG_TYPE ctype) {
        AB_TRAVERSAL trav;
        ABObj obj;
 ...
 */
int objxm_comp_set_ui_args(ABObj obj, OBJXM_CONFIG_TYPE ctype, BOOL replace) {
        ...
        util_dprintf(3, "Building Resource List for %s\n",
        ...
        if (obj->width >= 0 && s_obj != NULL)
                objxm_obj_set_ui_arg(s_obj, AB_ARG_INT, XmNwidth,
                                     (Dimension)obj->width);

        if (obj->height >= 0)
                objxm_obj_set_ui_arg(s_obj, AB_ARG_INT, XmNheight,
                                     (Dimension)obj->height);

        /* If widget is already instantiated, set colors now, else wait until
         * the post-instantiate phase so that the correct widget-id gets
         * associated with the allocated colors in the Xt resource cache.
         */
        if (objxm_get_widget(obj) != NULL)
                objxm_comp_set_color_args(obj, ctype,
                                          ObjxmBackground | ObjxmForeground);
        ...
        if (ctype == OBJXM_CONFIG_CODEGEN) {
                if (!obj_is_initially_active(obj)) {
                        objxm_obj_set_ui_arg(obj, AB_ARG_BOOLEAN, XmNsensitive,
                                             False);
                }
        }
...
int objxm_tree_set_post_inst_args(ABObj root, OBJXM_CONFIG_TYPE ctype) {
        AB_TRAVERSAL trav;
        ABObj obj;
...
int objxm_obj_set_post_inst_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        if (obj_is_salient(obj)) {
                objxmP_comp_type_post_instantiate(obj, ctype);

                /* Want to ensure allocated color is associated with correct
                 * widget-id in Xt string-to-pixel converter cache, so need to
                 * set color resources AFTER widget is instantiated.
                 */
                objxm_comp_set_color_args(obj, ctype,
                                          ObjxmBackground | ObjxmForeground);
        }
...
Arg *objxm_get_arg(ArgList arglist, char *name) {
...
int objxm_set_literal_arg(OBJXM_CONFIG_TYPE ctype, ArgList *p_arglist,
...
int objxm_set_widget_arg(OBJXM_CONFIG_TYPE ctype, ArgList *p_arglist,
...
int objxm_set_arg(ArgList *p_arglist, AB_ARG_TYPE argtype, String name,
                  XtArgVal value) {
        Arg *oldarg;
        Arg newarg;
 ...
 */
int objxm_set_arg_if_new(ArgList *p_arglist, AB_ARG_TYPE type, STRING name,
                         XtArgVal value) {
        Arg *old_arg;
...
void objxm_remove_all_args(ArgList *p_arglist, OBJXM_CONFIG_TYPE ctype) {
        int num_args;
        ArgList args;
        AB_ARG_TYPE type;
        ISTRING istr;
...
int objxm_get_num_args(ArgList arglist) {
        register ArgList args = arglist;
        register int n = 0;

        for (; (args != (ArgList)NULL) && (args->name != NULL); args++, n++)
                ;
 ...
 */
int objxm_remove_arg(ArgList *p_arglist, STRING name) {
        register int i;
 ...
 */
objxm_comp_set_attachment_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        AB_TRAVERSAL trav;
...
int objxm_comp_set_color_args(ABObj obj, OBJXM_CONFIG_TYPE ctype,
                              unsigned int flag) {
        ...
        if (rootObj == NULL) /* First time */
        {
                ...
                if (!cgen_args)
                        widget = (objxm_get_widget(obj) ? objxm_get_widget(obj)
                                                        : ObjxmP_toplevel);
                ...
                if (flag & ObjxmBackground) {
                        if (util_strempty(obj_get_bg_color(rootObj)))
                                ...
                                if (!cgen_args) {
                                        status = objxm_name_to_pixel(
                                            widget, obj_get_bg_color(rootObj),
                                            &pixel);

                                        if (status == ERR) /* Color couldn't be
                                                              allocated! */
                                        {
                                                obj_get_safe_name(rootObj, name,
                                                                  256);
                                                util_printf_err(
                                                    catgets(
                                                        OBJXM_MESSAGE_CATD,
...
Object's Background will default to white."),
                                                    name,
                                                    obj_get_bg_color(rootObj));
                                                pixel = WhitePixelOfScreen(
                                                    XtScreen(ObjxmP_toplevel));
                                        }
                                        bgvalue = (XtArgVal)pixel;
                                } else /* Generating Code */
                                        bgvalue = (XtArgVal)istr_create(
                                            obj_get_bg_color(rootObj));
                        }
                }
                if (flag & ObjxmForeground) {
                        if (util_strempty(obj_get_fg_color(obj)))
                                ...
                                if (!cgen_args) {
                                        status = objxm_name_to_pixel(
                                            widget, obj_get_fg_color(rootObj),
                                            &pixel);
...
Object's Foreground will default to black."),
                                                    name,
                                                    obj_get_fg_color(rootObj));
                                                pixel = BlackPixelOfScreen(
                                                    XtScreen(ObjxmP_toplevel));
                                        }
                                        fgvalue = (XtArgVal)pixel;
                                } else /* Generating Code */
                                        fgvalue = (XtArgVal)istr_create(
                                            obj_get_fg_color(rootObj));
                        }
                }
...
int objxm_comp_set_default_button_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        AB_TRAVERSAL trav;
        ...
        /* If Dialog has a Button Panel, set all it's button
                ...
                for (trav_open(&trav, button_panel, AB_TRAV_SALIENT_UI);
                     (child = trav_next(&trav)) != NULL;) {
                        if (obj_is_button(child)) {
                                button = objxm_comp_get_subobj(
                                    child, AB_CFG_OBJECT_OBJ);
...
int objxm_comp_set_label_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        ...
        case AB_LABEL_STRING:
                if (obj_is_window(obj))
                        objxm_obj_set_ui_arg(subObj, AB_ARG_STRING, XmNtitle,
                                             label);
                else {
                        XtArgVal valstr;
                        ...
                        /* If in Build-mode, convert label to XmString, else
                         * (CodeGen-mode) set value to label
                         */
                        if (cgen_args)
                                valstr = (XtArgVal)istr_create(label);
                        else {
                                /* NOTE:This XmString is freed in
                                 * objxm_remove_all_args() */
                                util_dprintf(3, "%s : allocating XmString\n",
                                             XmNlabelString);
                                valstr = (XtArgVal)objxm_str_to_xmstr(
                                    ObjxmP_toplevel, label);
                        }
                        objxm_obj_set_ui_arg(subObj, AB_ARG_XMSTRING,
        ...
        case AB_LABEL_GLYPH:
                if (!util_strempty(label)) {
                        if (!cgen_args) {
                                status = objxm_filebase_to_pixmap(
                                    ObjxmP_toplevel, label, &label_pm);
                                if (status != OK)
                                        util_printf_err(
                        ...
                        if (status == OK) {
                                objxm_obj_set_literal_ui_arg(
                                    subObj, ctype, XmNlabelType, XmPIXMAP);
                                objxm_obj_set_ui_arg(
                                    subObj, AB_ARG_PIXMAP, XmNlabelPixmap,
                                    cgen_args ? (XtArgVal)istr_create(
                                                    obj_get_label(obj))
                                              : (XtArgVal)label_pm);
                        }
                }
        ...
        case AB_LABEL_ARROW_DOWN:
                ...
                objxm_obj_set_literal_ui_arg(subObj, ctype, XmNarrowDirection,
                                             XmARROW_DOWN);
                break;
...
int objxm_comp_set_lbl_align_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        ABObj subObj = objxm_comp_get_subobj(obj, AB_CFG_LABEL_OBJ);
        unsigned char value;
...
int objxm_comp_set_lbl_pos_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        ...
        case AB_CP_WEST:
                objxm_obj_set_literal_ui_arg(subObj, ctype, XmNorientation,
                                             XmHORIZONTAL);
                objxm_obj_set_literal_ui_arg(subObj, ctype, XmNentryAlignment,
                                             XmALIGNMENT_END);
                break;
        case AB_CP_NORTH:
                objxm_obj_set_literal_ui_arg(subObj, ctype, XmNorientation,
...
int objxm_comp_set_icon_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        ...
        if (ctype == OBJXM_CONFIG_CODEGEN)
                objxm_obj_set_literal_ui_arg(
                    shobj, ctype, XmNinitialState,
                    obj_is_initially_iconic(obj) ? IconicState : NormalState);

        /* Setup Icon Pixmap */
        if ((icon = obj_get_icon(obj)) != NULL) {
                if (ctype != OBJXM_CONFIG_CODEGEN) {
                        status = objxm_filebase_to_pixmap(ObjxmP_toplevel, icon,
                                                          &icon_pm);
                        if (status != OK)
                                util_printf_err(
                                    objxm_pixmap_conversion_error_msg(obj, icon,
                                                                      status));
                }
                if (status == OK)
                        objxm_obj_set_ui_arg(shobj, AB_ARG_PIXMAP,
                                             XmNiconPixmap,
                                             ctype == OBJXM_CONFIG_CODEGEN
                                                 ? (XtArgVal)istr_create(icon)
                                                 : icon_pm);
        ...
        if ((mask = obj_get_icon_mask(obj)) != NULL) {
                if (ctype != OBJXM_CONFIG_CODEGEN) {
                        status = objxm_filebase_to_pixmap(ObjxmP_toplevel, mask,
                                                          &mask_pm);
                        if (status != OK)
                                util_printf_err(
                                    objxm_pixmap_conversion_error_msg(obj, mask,
                                                                      status));
                }
                if (status == OK)
                        objxm_obj_set_ui_arg(shobj, AB_ARG_PIXMAP, XmNiconMask,
                                             ctype == OBJXM_CONFIG_CODEGEN
                                                 ? (XtArgVal)istr_create(mask)
                                                 : mask_pm);
        } else if (ctype == OBJXM_CONFIG_BUILD)
        ...
        if ((icon_label = obj_get_icon_label(obj)) != NULL)
                objxm_obj_set_ui_arg(shobj, AB_ARG_STRING, XmNiconName,
                                     icon_label);
...
int objxm_comp_set_read_only_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        ABObj subObj;
        BOOL read_only;
...
int objxm_comp_set_border_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        ABObj b_obj;
        int bstyle;
...
int objxm_comp_set_msgbox_button_arg(ABObj obj, OBJXM_CONFIG_TYPE ctype,
...
int objxm_obj_set_arrow_style_arg(ABObj subObj, OBJXM_CONFIG_TYPE ctype) {
        unsigned char value;
...
int objxm_obj_set_attachment_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        ABObj parent_obj;
...
int objxm_obj_set_direction_arg(ABObj subObj, OBJXM_CONFIG_TYPE ctype) {
        unsigned char value;
        ...
        objxm_obj_set_literal_ui_arg(subObj, ctype, XmNprocessingDirection,
                                     value);
        obj_set_flag(subObj, AttrChangedFlag);
...
int objxm_obj_set_line_style_arg(ABObj subObj, OBJXM_CONFIG_TYPE ctype) {
        unsigned char value;

        value =
            objxm_linestyle_to_enum(obj_get_line_style(obj_get_root(subObj)));
...
int objxm_obj_set_orientation_arg(ABObj subObj, OBJXM_CONFIG_TYPE ctype) {
        unsigned char value;
...
int objxm_obj_set_selection_arg(ABObj subObj, OBJXM_CONFIG_TYPE ctype) {
        unsigned char value;
...
STRING
objxm_obj_get_default_motif_class(ABObj obj) {
        STRING cn = NULL;
 ...
 *************************************************************************/
int objxm_dump_arglist(ABObj obj, ArgList args, int num_args) {
        char name[256];
...
int objxm_dump_arglist_indented(ABObj obj, ArgList args, int num_args,
                                int spaces) {
        int space_count = 0;
        int i = 0;
        String valstr = NULL;
        ...
        for (i = 0; i < num_args; i++) {
                ...
                case AB_ARG_PIXEL:
                        if (cgen_args) {
                                ...
                                util_dprintf(0, "(Pixel)%ld ",
                                             (long)args[i].value);
                        }
                        break;
                case AB_ARG_XMSTRING:
                        if (cgen_args)
                                valstr = (String)istr_string(
                                    (ISTRING)(args[i].value));
                        else
                                valstr = arg->value
                                             ? (String)objxm_xmstr_to_str(
                                                   (XmString)arg->value)
                                             : "NULL";
                ...
                case AB_ARG_LITERAL:
                        if (cgen_args)
                                ...
                                valstr = istr_string(objxm_get_enum_strdef(
                                    args[i].name,
                ...
                util_dprintf(0, "%s\n",
                             objxm_res_value_is_default(
                                 args[i].name,
                                 objxm_get_class_ptr(obj_get_class_name(obj)),
                                 args[i].value)
                                 ? "(Default)"
                                 : "");
        }
...
int objxm_dump_widget_geometry(Widget w) {
        Position x, y;
        ...
        if (parent_is_form) {
                XtVaGetValues(
                    w, XmNx, &x, XmNy, &y, XmNwidth, &width, XmNheight, &height,
                    XmNtopAttachment, &(attach[A_TOP]), XmNbottomAttachment,
                    &(attach[A_BOTTOM]), XmNleftAttachment, &(attach[A_LEFT]),
                    XmNrightAttachment, &(attach[A_RIGHT]), XmNtopWidget,
                    &(widget[A_TOP]), XmNbottomWidget, &(widget[A_BOTTOM]),
                    XmNleftWidget, &(widget[A_LEFT]), XmNrightWidget,
                    &(widget[A_RIGHT]), XmNtopPosition, &(position[A_TOP]),
                    XmNbottomPosition, &(position[A_BOTTOM]), XmNleftPosition,
                    &(position[A_LEFT]), XmNrightPosition, &(position[A_RIGHT]),
                    XmNtopOffset, &(offset[A_TOP]), XmNbottomOffset,
                    &(offset[A_BOTTOM]), XmNleftOffset, &(offset[A_LEFT]),
                    XmNrightOffset, &(offset[A_RIGHT]), NULL);

        } else {
                XtVaGetValues(w, XmNx, &x, XmNy, &y, XmNwidth, &width,
        ...
        if (parent_is_form) {
                for (i = 0; i < 4; i++) {
                        ...
                        case XmATTACH_FORM:
                                util_dprintf(0,
                                             "XmATTACH_FORM, Form=%s, %s=%d\n",
                                             XtName(XtParent(w)), offset_str[i],
                                             offset[i]);
                                break;
                        case XmATTACH_WIDGET:
                                util_dprintf(0,
                                             "XmATTACH_WIDGET, %s=%s, %s=%d\n",
                                             widget_str[i], XtName(widget[i]),
                                             offset_str[i], offset[i]);
                                break;
                        case XmATTACH_POSITION:
                                util_dprintf(
                                    0, "XmATTACH_POSITION, %s=%d, %s=%d\n",
                                    pos_str[i], position[i], offset_str[i],
                                    offset[i]);
                                break;
                        }
                }
 ...
 *
 */
int objxmP_merge_arg(ArgList *p_arglist, Arg *arg) {
        ArgList result;
        int nargs = 0;

        if ((nargs = objxm_get_num_args(*p_arglist)) ==
            0) { /* first time ever */
                result = objxmP_merge_arglists(arg, 1, (ArgList)NULL, 0);
                *p_arglist = result;
        } else { /* merge with existing args */
...
ArgList objxmP_merge_arglists(ArgList args1, int num_args1, ArgList args2,
                              int num_args2) {
        ArgList result, args;
        Arg nullarg = {NULL, 0};
...
int objxmP_set_attachment_arg(ABObj obj, OBJXM_CONFIG_TYPE ctype,
        ...
        case AB_ATTACH_POINT:
                objxm_obj_set_literal_ui_arg(obj, ctype, att_side_resource,
                                             XmATTACH_FORM);
                objxm_obj_set_ui_arg(obj, AB_ARG_INT, att_offset_resource,
                                     offset);
                break;
        case AB_ATTACH_OBJ:
        case AB_ATTACH_ALIGN_OBJ_EDGE:
                att_obj = (ABObj)value;
                if (att_obj) {
                        /* If has an attachment to a widget which isn't
                         * generated in code(window status area), then move the
                         * attachment to its parent
                         */
                        if (ctype == OBJXM_CONFIG_CODEGEN &&
                            obj_has_flag(att_obj, NoCodeGenFlag))
                                att_obj = parent_obj;

                        if (att_obj == parent_obj)
                                objxm_obj_set_literal_ui_arg(
                                    obj, ctype, att_side_resource,
                                    (type == AB_ATTACH_OBJ
                                         ? XmATTACH_FORM
                                ...
                                objxm_obj_set_widget_ui_arg(
                                    obj, ctype, att_widget_resource, att_obj);
                        }
                        objxm_obj_set_ui_arg(obj, AB_ARG_INT,
                                             att_offset_resource, offset);
                } else if (util_get_verbosity() > 0) {
                        char name[256];
                        obj_get_safe_name(obj, name, 256);
                        util_printf_err(
                            catgets(OBJXM_MESSAGE_CATD, OBJXM_MESSAGE_SET, 3,
