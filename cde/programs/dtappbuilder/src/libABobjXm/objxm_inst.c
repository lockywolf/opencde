...
**                                                                      **
**************************************************************************/
static int xm_instantiate_tree(ABObj root, BOOL manage);
static int xm_instantiate_obj(ABObj obj, BOOL manage);
static int post_instantiate_tree(ABObj root);
static int post_instantiate_obj(ABObj obj);
static int xm_manage_tree(ABObj obj);
static Widget instantiate_menu(ABObj obj, ABObj menu_ref, Widget owner,
                               Widget parent);
static Widget instantiate_widget(ABObj obj, Widget parent, BOOL manage);
static int xm_map_obj(ABObj obj);
static int xm_unmap_obj(ABObj obj);
static WidgetClass xm_get_class(ISTRING class_name);

static int formlist_append(Widget newform);
 ...
 * Event Handlers
 */
static void xm_popup_menu(Widget w, XtPointer client_data, XEvent *ev,
                          Boolean *cont);

/*
 * Actions
 */
static void popup_menu(Widget widget, XEvent *event, String *params,
                       int num_params);
...
static XtActionsRec menu_actions[] = {{"PopupMenu", (XtActionProc)popup_menu}};

static String menu_trans = "<Btn3Down>:        PopupMenu()";
 ...
 * starting at root
 */
int objxm_tree_instantiate(ABObj root, BOOL manage_last) {
        /* Create all widgets
         */
 ...
 * starting at root
 */
int objxm_tree_map(ABObj root, BOOL map) {
        AB_TRAVERSAL trav;
        ABObj child = NULL;
                ...
                if (obj_is_module(obj) || obj_is_project(obj)) {
                        ...
                        if (XtIsSubclass(widget, applicationShellWidgetClass) ||
                            XtIsSubclass(widget, topLevelShellWidgetClass))
                                ...
                                XtManageChild(
                                    objxm_get_widget(obj_get_child(obj, 0)));
                                XtPopup(widget, XtGrabNone);
                        }
                }
...
static int xm_manage_tree(ABObj root) {
        AB_TRAVERSAL trav;
                    ...
                    !XtIsSubclass(widget, xmDialogShellWidgetClass)) {
                        ...
                        if (util_get_verbosity() > 3) {
                                ...
                                for (i = 0; i < num_wchildren; i++) {
                                        XtVaGetValues(wchildren[i], XmNwidth,
                                                      &w, XmNheight, &h, NULL);
                                        fprintf(stderr, "%s(%dx%d) ",
                                                XtName(wchildren[i]), w, h);
                                }
                                fprintf(stderr, ".\n");
                        }
                }
        ...
        if (manage_root == root) {
                if (widget != NULL && !XtIsSubclass(widget, shellWidgetClass) &&
                    !XtIsSubclass(XtParent(widget), xmDialogShellWidgetClass)) {
                        ...
                        if (util_get_verbosity() > 3) {
                                Dimension w, h;
                                XtVaGetValues(widget, XmNwidth, &w, XmNheight,
                                              &h, NULL);
...
static int xm_instantiate_obj(ABObj obj, BOOL manage) {
        ...
        if (obj_is_combo_box_item(obj) || obj_is_list_item(obj) ||
            obj_is_spin_box_item(obj)) {
                ...
                if ((parent = objxm_get_widget(obj_get_parent(obj))) == NULL) {
                        ...
                        util_dprintf(1,
                                     "Instantiation ERROR: %s: widget-ID for "
                                     "parent is NULL\n",
                                     obj_name);
                        return ERROR;
                }
                ...
                if (objxm_is_menu_widget(widget) &&
                    XtIsSubclass(widget, xmRowColumnWidgetClass)) {
                        ...
                        if (obj_is_choice(robj)) {
                                ABObj o_obj = objxm_comp_get_subobj(
                                    robj, AB_CFG_OBJECT_OBJ);
                                objxm_obj_set_ui_arg(o_obj, AB_ARG_WIDGET,
                                                     XmNsubMenuId, widget);
                        }
                }
...
static Widget instantiate_widget(ABObj obj, Widget parent, BOOL manage) {
        ...
        if (name == NULL) {
                util_dprintf(
                    1, "Instantiation ERROR: Object must have a valid name\n");
                return NULL;
        }
        if (parent == NULL) {
                ...
                util_dprintf(
                    1,
                    "Instantiation ERROR: %s: widget-ID for parent is NULL\n",
                    obj_name);
                return NULL;
        }
        ...
        if (!obj_has_flag(obj,
                          CreateAttrsFlag)) /* Make sure Resources are set */
                objxm_comp_set_ui_args(obj_get_root(obj), OBJXM_CONFIG_BUILD,
                                       TRUE);
        ...
        if (util_streq(obj_get_class_name(obj), _applicationShell)) {
                ...
                if (util_get_verbosity() > 3) {
                        util_dprintf(2,
                                     "INSTANTIATING(Shell): %s part_of(%s), "
                                     "parent=%s, class=%s.\n",
                                     name,
                                     obj->part_of ? util_strsafe(obj_get_name(
                                                        obj->part_of))
                                                  : name,
                ...
                widget = XtCreatePopupShell(name, topLevelShellWidgetClass,
                                            parent, all_args, num_args);
                XtVaSetValues(widget, XtNwindowGroup, XtWindow(ObjxmP_toplevel),
                              NULL);
        ...
        } else /* Non TopLevel */
        {
                ...
                if (util_streq(obj_get_class_name(obj), _xmFrame) ||
                    XtIsSubclass(parent, xmFrameWidgetClass)) {
                        ...
                        widget = XtCreateManagedWidget(name, xmclass, parent,
                                                       all_args, num_args);
                } else {
                        xmcreate_func = objxm_obj_get_xmcreate_func(obj);

                        if (xmcreate_func != NULL)
                                widget = (*xmcreate_func)(parent, name,
                                                          all_args, num_args);
                        else {
                                xmclass = objxm_get_class_ptr(
                                    obj_get_class_name(obj));
                                if (xmclass != NULL)
                                        widget = XtCreateWidget(
                                            name, xmclass, parent, all_args,
                                            num_args);
                                else {
                                        char *obj_name =
                                            util_strsafe(obj_get_name(obj));
                                        util_dprintf(1,
                                                     "Instantiation ERROR: %s: "
                                                     "Object must have widget "
                                                     "class\n",
                                                     obj_name);
                                        return NULL;
                                }
                ...
                if (util_get_verbosity() > 3) {
                        fprintf(stderr,
                                "INSTANTIATING: %s=%p part_of(%s), parent=%s, "
                                "class=%s.\n",
                                name, widget,
                                obj->part_of
                                    ? util_strsafe(obj_get_name(obj->part_of))
                                    : name,
                                XtName(parent), obj_get_class_name(obj));
                        objxm_dump_arglist(obj, all_args, num_args);
                }
...
static int post_instantiate_obj(ABObj obj) {
        ...
        if (!obj_is_window(obj)) {
                ...
                /* This is a workaround for an extremely strange problem where
                 * [sometimes] Control Panes do not resize large enough to
                 * display all of their children.
                 */
                if (XtIsSubclass(objxm_get_widget(parent_obj),
                                 xmFormWidgetClass))
 ...
 */
int objxmP_destroy(ObjEvDestroyInfo info) {
        ABObj obj = info->obj;
        ...
        if (widget != NULL) {
                ...
                if (obj_is_combo_box(obj) ||
                    (obj_is_control_panel(obj) &&
                     obj_get_subtype(obj) == AB_CONT_FOOTER)) {
                        XtUnrealizeWidget(widget);
                } else {
                        util_dprintf(
                            2, "WIDGET DESTROY(CB): %s=%p part_of(%s)\n",
 ...
 */
int objxm_tree_destroy(ABObj root) {
 ...
 */
int objxm_tree_uninstantiate(ABObj root, BOOL topmost) {
        Widget widget;
        AB_TRAVERSAL trav;
        ABObj child;
        ...
        if (obj_is_combo_box(root) ||
            (obj_is_control_panel(root) &&
             obj_get_subtype(root) == AB_CONT_FOOTER)) {
                XtUnrealizeWidget(widget);
        } else {
                util_dprintf(2, "WIDGET DESTROY: %s=%p part_of(%s)\n",
                             util_strsafe(obj_get_name(root)), widget,
                             root->part_of
                                 ? util_strsafe(obj_get_name(root->part_of))
 ...
 * starting at root
 */
int objxm_tree_instantiate_changes(ABObj root, BOOL *new_widgets) {
 ...
 */
int objxm_comp_instantiate_changes(
    ABObj obj, BOOL *new_widgets /* return if new widgets created */
) {
         ...
         */
        else if (widget != NULL && obj_has_flag(obj, AttrChangedFlag) &&
                 !obj_has_flag(obj, BeingDestroyedFlag)) {
                args = (ArgList)obj->ui_args;
                num_args = objxm_get_num_args(args);
                ...
                */
                 ...
                 */
                /*
                REMIND: aim,2/8/94 - revisit why this prevents foreground from
                being changed later...
        ...
        {
                /* Ensure any Non-widget/OptionMenu type items get "selected" if
                 * necessary */
                if (obj_is_item(obj))
 ...
 */
int objxm_comp_set_item_selection(ABObj obj) {
        ABObj pobj;
        Widget parent;
        XmString xmitem;
        AB_ITEM_TYPE itype;
        ...
        if (itype == AB_ITEM_FOR_CHOICE && selected &&
            (obj_get_choice_type(obj_get_root(pobj)) ==
             AB_CHOICE_OPTION_MENU)) {
                Widget optionbox = objxm_get_widget(objxm_comp_get_subobj(
                    obj_get_root(pobj), AB_CFG_OBJECT_OBJ));
 ...
 */
int objxm_store_obj_and_actions(Widget w, ABObj obj) {
 ...
 */
int objxm_free_obj_and_actions(Widget w) {
        ObjxmWidgetInfo *winfo = NULL;
 ...
 */
ABObj objxm_get_obj_from_widget(Widget w) {
        ObjxmWidgetInfo *winfo = NULL;
 ...
 */
XtTranslations objxm_get_actions_from_widget(Widget w) {
        ObjxmWidgetInfo *winfo = NULL;
...
WidgetClass objxm_get_class_ptr(STRING class_name) {
...
XmCreateFunc objxm_obj_get_xmcreate_func(ABObj obj) {
        ABObj robj = obj_get_root(obj);
        ...
        if (istr_equal(class_name, istr_const(_xmArrowButton)))
                ...
                if (obj_is_menubar(obj))
                        ...
                        AB_CHOICE_INFO *info =
                            (AB_CHOICE_INFO *)&(robj->info.choice);

                        if (objxm_comp_get_subobj(robj, AB_CFG_OBJECT_OBJ) ==
                            obj) {
                                switch (info->type) {
...
static int formlist_force_resize() {
        ...
        for (i = form_list_count - 1; i >= 0; i--) {
                 ...
                 */
                XtVaGetValues(form_list[i], XmNbackground, &bgpixel, NULL);

                xmlabel = XmStringCreateLocalized(" ");
                util_dprintf(2, "\n Creating phantom child of %s.\n",
                             XtName(form_list[i]));
                phantom = XtVaCreateManagedWidget(
                    "phantom", xmLabelWidgetClass, form_list[i], XmNlabelString,
                    xmlabel, XmNbackground, bgpixel, NULL);
...
int objxm_create_popup_menu_title(ABObj menu_ref, OBJXM_CONFIG_TYPE ctype,
                                  STRING title) {
        ABObj tobj;
        ABObj menu;
        ...
        obj_set_unique_name(
            tobj, ab_ident_from_name_and_label(obj_get_name(menu), "title"));
        obj_set_label(tobj, title);
        objxm_obj_configure(tobj, ctype, True);

        if (ctype == OBJXM_CONFIG_BUILD)
                tobj->ui_handle =
                    instantiate_widget(tobj, objxm_get_widget(menu_ref), True);

        tobj = obj_create(AB_TYPE_SEPARATOR, NULL);
        obj_insert_child(menu_ref, tobj, 1);
        obj_set_unique_name(tobj, ab_ident_from_name_and_label(
                                      obj_get_name(menu), "title_sep"));
 ...
 */
int objxm_instantiate_menus(ABObj root) {
        AB_TRAVERSAL trav;
        ABObj obj;
        ABObj m_parent;
        ABObj menu_ref;
        ...
        for (trav_open(&trav, root, AB_TRAV_UI);
             (obj = trav_next(&trav)) != NULL;)
                 ...
                 */
                if (obj_is_salient(obj) && obj_get_menu_name(obj) != NULL &&
                    !obj_is_menu_item(obj)) {
                        m_parent =
                ...
                if (menu != NULL && parent != NULL) {
                        if (obj_is_button(obj) &&
                            obj_get_subtype(obj) == AB_BUT_MENU) {
                                ...
                                if (obj_is_menu_item(obj) ||
                                    obj_is_menubar_item(obj)) {
                                        m_type = AB_MENU_PULLDOWN;
                                        obj_set_class_name(menu,
                                                           _xmPulldownMenu);
                                } else {
                                ...
                                if (m_type == AB_MENU_PULLDOWN)
                                        XtVaSetValues(owner, XmNsubMenuId,
                                                      menu_w, NULL);
                                ...
                                else /* PopupMenu */
                                {
                                        ...
                                        XtInsertEventHandler(
                                            parent, ButtonPressMask, False,
                                            xm_popup_menu, (XtPointer)obj,
                                            XtListHead);
                                        /*
                                         REMIND: aim, 10/7/94 - In trying to
                                         migrate to using translations, it
                                                 messed up the event handling
                                         when we returned to BUILD mode. For
                                         now, going back to event-handling.
                                         ...
                                         XtNumber(menu_actions));
                                                                popup_menu_trans
                                         = XtParseTranslationTable(menu_trans);
                                                            }
                                                            menu_p =
                                         objxm_get_obj_from_widget(parent);
                                                            objxm_store_obj_and_actions(parent,
                                         menu_p); XtOverrideTranslations(parent,
                                         popup_menu_trans);
                                        */

                                        if (!util_strempty(
                                                (title =
                                                     obj_get_menu_title(obj))))
                                                objxm_create_popup_menu_title(
                                                    menu_ref,
                                                    OBJXM_CONFIG_BUILD, title);
                                }
                        }
                        ...
                        for (trav_open(&trav, menu, AB_TRAV_CHILDREN);
                             (item = trav_next(&trav)) != NULL;) {
                                ...
                                if ((sub_menu_ref = objxm_comp_get_subobj(
                                         item, AB_CFG_MENU_OBJ)) != NULL) {
                                        /* Create TEMP copy of submenu(Menu-ref)
                                         * under temp Item obj */
                                        tmp_menuref = obj_create_ref(
                                            obj_get_actual_obj(sub_menu_ref));
                                        tmp_menuref->part_of = tmp_itemobj;
                                        obj_append_child(tmp_itemobj,
                                                         tmp_menuref);
                        ...
                        char *obj_name = util_strsafe(obj_get_name(obj));
                        util_dprintf(1,
                                     "Instantiation ERROR: %s: could not "
                                     "instantiate menu\n",
                                     obj_name);
                        return NULL;
                }
 ...
 * objects inside a tree starting at root.
 */
int objxm_destroy_menus(ABObj root) {
        AB_TRAVERSAL trav;
        ABObj obj;
        ABObj menu;
        AB_MENU_TYPE m_type;
                ...
                if (obj_is_salient(obj) && !obj_is_menu_item(obj) &&
                    (menu_ref = objxm_comp_get_subobj(obj, AB_CFG_MENU_OBJ)) !=
                        ...
                        if (menu_ref->ui_handle != NULL) {
                                ...
                                /* Destroy Temporary Item objs created ONLY for
                                 * Test-mode */
                                num_items = obj_get_num_children(menu_ref);
                                item_array = (ABObj *)util_malloc(
                                    num_items * sizeof(ABObj));
                                for (i = 0; i < num_items; i++)
                                        item_array[i] =
                                            obj_get_child(menu_ref, i);
                                ...
                                /* Unless the obj is a MenuButton, disconnect &
                                 * Destroy the Menu
                                 */
                                if (!(obj_is_button(obj) &&
                                      obj_get_subtype(obj) == AB_BUT_MENU)) {
                                                              ...
                                                              pops up menu */
                                        {
                                                ...
                                                menu_p = objxm_comp_get_subobj(
                                                    obj,
                                                    AB_CFG_MENU_PARENT_OBJ);
                                                XtRemoveEventHandler(
                                                    (Widget)menu_p->ui_handle,
                                                    ButtonPressMask, False,
                                                    xm_popup_menu,
                                                    (XtPointer)obj);
                                                /*
                                                 REMIND: aim, 10/7/94 - In
                                                 trying to migrate to using
                                                 translations, it messed up the
                                                 event handling when we returned
                                                 to BUILD mode. For now, going
                                                 back to event-handling.

                                                                        parent =
                                                 objxm_get_widget(menu_p);
                                                                        orig_trans
                                                 =
                                                 objxm_get_actions_from_widget(parent);
...
static void xm_popup_menu(Widget widget, XtPointer client_data, XEvent *event,
                          Boolean *cont) {
        ...
        if (bevent->button == 3) {
                ...
                if (menu_ref != NULL &&
                    (menu = (Widget)menu_ref->ui_handle) != NULL) {
