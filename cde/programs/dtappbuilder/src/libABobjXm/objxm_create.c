...
**                                                                      **
**************************************************************************/
static int xm_instantiate_tree(ABObj root, BOOL map);
static int xm_instantiate_obj(ABObj obj, BOOL map);
static int xm_instantiate_menu(ABObj obj, Widget owner, Widget parent,
                               BOOL submenu);
static int post_instantiate_tree(ABObj root, BOOL map);
static int post_instantiate_obj(ABObj obj, BOOL map);
static Widget instantiate_widget(ABObj obj, Widget parent, BOOL map);
...
int objxm_fully_instantiate_tree(ABObj root, BOOL map) {
...
int objxm_map_tree(ABObj root, BOOL map) {
        AB_TRAVERSAL trav;
        ABObj obj;
...
static int xm_map_obj(ABObj obj) {
        ...
        if (!obj_has_flag(obj, InstantiatedFlag)) {
                if (util_get_verbosity() > 2)
                        fprintf(stderr,
                                "xm_map_obj: %s must be instantiated first\n",
                                obj_get_name(obj));
                return -1;
        }
        if (!obj_has_flag(obj, MappedFlag)) {
                /* If the object has an associated widget and is NOT a
                 * menu-pane, then Manage it...
                 */
...
static int xm_instantiate_obj(ABObj obj, BOOL map) {
        ...
        if (obj_is_item(obj) && obj->info.item.type == AB_ITEM_FOR_LIST) {
                ...
                if ((parent = (Widget)(obj_get_parent(obj)->ui_handle)) !=
                    NULL) {
        ...
        if (parent == NULL) {
                if (util_get_verbosity() > 2)
                        fprintf(stderr,
                                "xm_instantiate_obj: %s: ui_handle for parent "
                                "is NULL\n",
                ...
                if (objxm_is_menu_widget(widget) &&
                    XtIsSubclass(widget, xmRowColumnWidgetClass)) {
                        ...
                        if (obj_is_choice(robj)) {
                                ABObj o_obj = objxm_get_config_obj(
                                    robj, AB_CFG_OBJECT_OBJ);
                                objxm_set_ui_arg(o_obj, AB_ARG_WIDGET,
                                                 XmNsubMenuId, widget);
                        }
                }
...
int objxm_instantiate_menus(ABObj root) {
        ...
        for (trav_open(&trav, root, AB_TRAV_UI);
             (obj = trav_next(&trav)) != NULL;)
                 ...
                 * actually instantiated using recursion in
                 * xm_instantiate_menu(), thus we do not call
                 * xm_instantiate_menu() for them directly.
                 */
                if (obj_is_salient(obj) && obj_get_menu_name(obj) != NULL &&
                    !obj_is_menu_item(obj)) {
                        m_parent =
                            objxm_get_config_obj(obj, AB_CFG_MENU_PARENT_OBJ);
        ...
        if ((menu_ref = objxm_get_config_obj(obj, AB_CFG_MENU_OBJ)) != NULL) {
                ...
                if (menu != NULL && parent != NULL) {
                        ...
                        if (m_type == AB_MENU_PULLDOWN)
                                XtVaSetValues(owner, XmNsubMenuId, menu_w,
                                              NULL);
                        else /* PopupMenu */
                        ...
                        for (trav_open(&trav, menu, AB_TRAV_CHILDREN);
                             (item = trav_next(&trav)) != NULL;) {
                                ...
                                if (obj_get_menu_name(item) != NULL)
                                        xm_instantiate_menu(item, item_w,
                                                            menu_w, TRUE);
                        }
                        trav_close(&trav);
                        ...
                        if (util_get_verbosity() > 2)
                                fprintf(stderr,
                                        "xm_instantiate_menu: could not "
                                        "instantiate menu for %s\n",
                                        istr_string_safe(obj->name));
                        return -1;
                }
        }
                ...
                if (obj_is_salient(obj) && !obj_is_menu_item(obj) &&
                    (menu_ref = objxm_get_config_obj(obj, AB_CFG_MENU_OBJ)) !=
                        ...
                        if (menu_ref->ui_handle != NULL) {
                                ...
                                if (m_type == AB_MENU_POPUP) /* Remove
                                                                EventHandler
                                                                that pops up
                                                                menu */
                                {
                                        menu_p = objxm_get_config_obj(
                                            obj, AB_CFG_MENU_PARENT_OBJ);
...
static Widget instantiate_widget(ABObj obj, Widget parent, BOOL map) {
        ...
        if (name == NULL) {
                if (util_get_verbosity() > 2)
                        fprintf(stderr,
                                "instantiate_widget: object must have name\n");
                return NULL;
        }
        ...
        if (istr_equal(obj->class_name, istr_const(_topLevelShell))) {
                all_args = objxmP_merge_arglists(
                    (ArgList)obj->ui_args,
        ...
        {
                objxm_set_ui_arg(obj, AB_ARG_BOOLEAN, XtNmappedWhenManaged,
                                 map);
                ...
                name, obj->part_of? istr_string_safe(obj->part_of->name) :
                "null",obj_get_class_name(obj)); objxm_dump_arglist(obj,
                all_args, num_args);
                */
                ...
                if (xmcreate_func != NULL)
                        widget =
                            (*xmcreate_func)(parent, name, all_args, num_args);

                else {
                        xmclass = (WidgetClass)xm_get_class(obj->class_name);
                        widget = XtCreateWidget(name, xmclass, parent, all_args,
                                                num_args);
                }
...
static int post_instantiate_obj(ABObj obj, BOOL map) {
        ...
        if (!obj_is_window(obj)) {
                ...
                if (XtIsSubclass((Widget)parent_obj->ui_handle,
                                 xmFormWidgetClass))
        ...
        if (obj_has_flag(obj, AttrChangedFlag)) {
                ...
                istr_string_safe(obj->name)); objxm_dump_arglist(obj, args,
                num_args);
                */
...
int objxm_destroy_tree(ABObj root) {
...
int objxm_uninstantiate_tree(ABObj root, BOOL topmost) {
        AB_TRAVERSAL trav;
        ABObj child;
...
XmCreateFunc objxm_get_xmcreate_func(ABObj obj) {
        ABObj robj = obj_get_comp_rootobj(obj);
        ...
        if (istr_equal(class_name, istr_const(_xmArrowButton)))
                ...
                AB_COMPOUND_INFO *info =
                    (AB_COMPOUND_INFO *)&(obj->info.compound);
                switch (info->type) {
                ...
                if (obj_is_menubar(obj))
                        ...
                        if (objxm_get_config_obj(robj, AB_CFG_OBJECT_OBJ) ==
                            obj) {
                                switch (info->type) {
...
static int formlist_force_resize() {
        ...
        for (i = form_list_count - 1; i >= 0; i--) {
                /* UGLY! */
                XtVaGetValues(form_list[i], XmNwidth, &f_width, XmNheight,
                              &f_height, NULL);
                XtVaSetValues(form_list[i], XmNwidth, f_width + 1, XmNheight,
                              f_height + 1, NULL);
                XtVaSetValues(form_list[i], XmNwidth, f_width, XmNheight,
                              f_height, NULL);
                form_list[i] = NULL;
        }
        form_list_count = 0;
