...
**                                                                      **
**************************************************************************/
static int xm_configure_tree(ABObj root, OBJXM_CONFIG_TYPE ctype);
static int xm_configure_tree_attachments(ABObj root);

static int config_attachment(ABObj obj, ABObj parent_obj, AB_COMPASS_POINT dir);
 ...
 */
void objxm_register_config_info(XmConfigInfo cfginfo) {
        int i;
        ...
        if (!exists) {
                if (config_info_count < MAX_XMCONFIG_INFOS)
                        ...
                        if (util_get_verbosity() > 0)
                                fprintf(stderr, "objxm_register_config_info: "
                                                "config_info table full\n");
                        return;
                }
        }
}
 ...
 */
XmConfigInfo objxmP_get_config_info(ABObj obj) {
        int i;
 ...
 */
int objxm_obj_configure(ABObj obj, OBJXM_CONFIG_TYPE ctype, BOOL set_args) {
        int iReturn = OK;
 ...
 * UnConfigured object
 */
int objxm_obj_unconfigure(ABObj obj) {
        int iReturn = -1;
        XmConfigInfo cfginfo;
 ...
 */
int objxm_tree_configure(ABObj root, OBJXM_CONFIG_TYPE ctype) {
        /* Phase I */
 ...
 * UnConfigured state
 */
int objxm_tree_unconfigure(ABObj root) {
        int iRetVal = 0;
        AB_TRAVERSAL trav;
        ABObj obj = NULL;
...
static int xm_configure_tree(ABObj root, OBJXM_CONFIG_TYPE ctype) {
        ...
        for (trav_open(&trav, root,
                       AB_TRAV_SALIENT | AB_TRAV_MOD_PARENTS_FIRST |
                           AB_TRAV_MOD_SAFE);
             (obj = trav_next(&trav)) != NULL;) {
                ++configCount;
 ...
 */
int objxmP_comp_set_type_args(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        XmConfigInfo cfginfo;
 ...
 * to an object-type
 */
int objxmP_comp_type_post_instantiate(ABObj obj, OBJXM_CONFIG_TYPE ctype) {
        XmConfigInfo cfginfo;
 ...
 */
int objxmP_obj_config_attachments(ABObj obj) {
...
static int config_attachment(ABObj obj, ABObj parent_obj,
        ...
        if (attachment->type == AB_ATTACH_OBJ) {
                ...
                if (attobj == obj_get_root(parent_obj)) {
                        attobj =
                            objxm_comp_get_subobj(attobj, AB_CFG_PARENT_OBJ);
 ...
 * POSITION, etc)
 */
ABObj objxm_comp_get_subobj(ABObj obj, AB_CFG_OBJ_TYPE type) {
        XmConfigInfo cfginfo;
        ABObj cfgobj = NULL;
        ...
        if (obj_has_flag(obj, XmConfiguredFlag)) {
                ...
                if (cfginfo != NULL && cfginfo->get_config_obj != NULL)
                        cfgobj = ((*cfginfo->get_config_obj)(obj_get_root(obj),
                                                             type));
                ...
                fprintf(stderr,
                        catgets(OBJXM_MESSAGE_CATD, OBJXM_MESSAGE_SET, 9,
                                "ERROR: objxm_comp_get_subobj - object not "
                                "xmconfigured: %s\n"),
                        name);
        }
...
ABObj objxmP_create_direct_subobj(ABObj root, AB_OBJECT_TYPE type,
                                  STRING subname) {
        ABObj subobj;
        ...
        */
        obj_set_unique_name(
            subobj, ab_ident_from_name_and_label(obj_get_name(root), subname));
 ...
 */
ABObj objxm_comp_config_border_frame(ABObj obj) {
        ABObj subobj = NULL;
        ...
        if (obj_has_border_frame(obj)) {
                ...
                subobj =
                    objxmP_create_direct_subobj(obj, AB_TYPE_CONTAINER, "obj");
        }
        return subobj;
}
 ...
 */
int objxm_comp_unconfig_border_frame(ABObj obj) {
        ABObj subobj;
        ...
        if (obj_has_border_frame(obj)) {
                ...
                if (subobj) {
                        ...
                        fprintf(stderr,
                                "objxm_comp_unconfig_border_frame: %s : can't "
                                "find SubObj\n",
                                util_strsafe(obj_get_name(obj)));
                }
        }
...
int objxm_comp_config_labeled_obj(ABObj obj, AB_OBJECT_TYPE obj_type,
                                  STRING obj_suffix, STRING obj_classname) {
        ABObj lbl_obj = NULL;
        ABObj real_obj;
        ...
        if (obj_has_label(obj)) {
                ...
                obj_set_unique_name(
                    real_obj, ab_ident_from_name_and_label(obj_get_name(obj),
                                                           obj_suffix));
...
int objxm_comp_unconfig_labeled_obj(ABObj obj) {
        ABObj lbl_obj;
        ABObj real_obj;
 ...
 */
ABObj objxm_comp_config_menu_ref(ABObj obj) {
        ABObj menu_obj;
        ABObj menu_ref = NULL;
        ...
        if (menuname != NULL) {
                ...
                if ((menu_obj = obj_find_menu_by_name(obj_get_module(obj),
                                                      menuname)) != NULL) {
                        /* ALWAYS make Menu Reference FIRST child! */
                        menu_ref = obj_create_ref(menu_obj);
                        obj_insert_child(obj, menu_ref, 0);
                        /*
                         REMIND: aim, 9/13/94 - The Menu Ref should not really
                         be a "part of" an object But, leaving this in
                         (commented out) for now in case change causes problems
                        ...
                        */
                        ...
                        obj_set_unique_name(menu_ref,
                                            ab_ident_from_name_and_label(
                                                obj_get_name(obj), menuname));

                        /*
                        ** Set the object class of the Menu Reference to be that
                        *of the
                        ** virtual menu object to which it refers.  This makes
                        *it easier
                        ** later on to figure out some of the aspects of
                        *handling the Menu
                        ** Reference.
                        */
 ...
 */
int objxm_comp_unconfig_menu_ref(ABObj obj) {
        ABObj menu_ref;
