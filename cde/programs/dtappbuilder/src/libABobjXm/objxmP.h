 ...
 */
typedef XtArgVal (*ObjxmResDefaultFunc)(WidgetClass);
...
typedef struct _OBJXM_RES {
        ...
        ObjxmResDefaultFunc
            default_func; /* default value calculation function */
} ObjxmRes;
 ...
 * destroyed
 */
extern int objxmP_destroy(ObjEvDestroyInfo info);

/* Function which returns XmConfigInfo
 * ptr for an object
 */
extern XmConfigInfo objxmP_get_config_info(ABObj obj);

/* Create a SubObj directly under the root and
 * move the root's children under the new SubObj
 */
extern ABObj objxmP_create_direct_subobj(ABObj root, AB_OBJECT_TYPE type,
                                         STRING subname);

/* Function which re-configures attachments after
 * objects have been xmconfigured
 */
extern int objxmP_obj_config_attachments(ABObj obj);

extern int objxmP_obj_set_attachment_arg(ABObj obj, OBJXM_CONFIG_TYPE ctype,
                                         ABObj parent_obj,
                                         AB_COMPASS_POINT dir);

extern int objxmP_comp_set_type_args(ABObj obj, OBJXM_CONFIG_TYPE ctype);

extern int objxmP_comp_type_post_instantiate(ABObj obj,
                                             OBJXM_CONFIG_TYPE ctype);

extern int objxmP_merge_arg(ArgList *p_arglist, Arg *arg);

extern ArgList objxmP_merge_arglists(ArgList args1, int num_args1,
                                     ArgList args2, int num_args2);
