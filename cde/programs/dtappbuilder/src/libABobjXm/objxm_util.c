...
static int indent(FILE *file, int spaces);

static int convert_filebase_to_pixmap(Widget widget, STRING filebase,
                                      Pixmap *pixmap_p);
static int convert_file_to_pixmap(Widget widget, STRING filename,
                                  Pixmap *pixmap_p);
static int load_pixmap(Widget widget, STRING filebase, STRING ext,
                       Pixmap *pixmap_p);
 ...
 */
BOOL objxm_color_exists(String colorname) {
        ...
        return (
            XLookupColor(display, colormap, colorname, &exact_color, &color));
}
 ...
 * destroyed.
 */
int objxm_name_to_pixel(Widget widget, String colorname, Pixel *pixel_p) {
        XrmValue src, dst;
...
int objxm_filebase_to_pixmap(Widget widget, STRING filebase, Pixmap *pixmap_p) {
        ...
        *pixmap_p = 0;
        ...
        if (ObjxmP_filename_to_pixmap_cb != NULL) {
                return (
                    ObjxmP_filename_to_pixmap_cb(widget, filebase, pixmap_p));
        } else /* Use internal conversion routine */
                return (convert_filebase_to_pixmap(widget, filebase, pixmap_p));
}

STRING
objxm_pixmap_conversion_error_msg(ABObj obj, STRING filebase, int status) {
        ...
        if (obj != NULL)
                sprintf(buf,
                        catgets(OBJXM_MESSAGE_CATD, 203, 14, "Warning: %s :\n"),
                        util_strsafe(obj_get_name(obj)));
        else
                strcpy(buf, "");

        sprintf(buf2,
                catgets(OBJXM_MESSAGE_CATD, 203, 15,
        ...
        if (status == ERR_BAD_PARAM2)
                ...
                strcat(buf, catgets(OBJXM_MESSAGE_CATD, 203, 17,
                                    "None of the following files could be "
                                    "converted to a Pixmap:\n"));

                if ((int)(strlen(buf) + (4 * (strlen(filebase) + 4))) >
                    LG_BUF_SIZE)
        ...
        util_set_help_data(
            catgets(OBJXM_MESSAGE_CATD, 203, 20,
                    "The graphic file name specified could not be "
                    "converted\ninto a pixmap. App Builder searches for a file "
                    "of the\nform Filename.pm, Filename.xpm, Filename.bm, "
                    "and\nFilename.xbm.\n\nEither none of those files exists "
                    "in the directory specified\n(via the Graphic Filename "
                    "field or if no path was specified,\nthen the current "
                    "working directory), or a file exists but may\nbe "
                    "corrupt."),
            NULL, NULL);
...
static int convert_filebase_to_pixmap(Widget widget, STRING filebase,
        ...
        {
                if ((status = load_pixmap(widget, filebase, ".pm", pixmap_p)) !=
                    OK)
                        if ((status = load_pixmap(widget, filebase, ".xpm",
                                                  pixmap_p)) != OK)
                                if ((status = load_pixmap(widget, filebase,
                                                          ".bm", pixmap_p)) !=
                                    OK)
                                        status = load_pixmap(widget, filebase,
                                                             ".xbm", pixmap_p);
        } else /* Monochrome - Try Bitmap First */
        {
                if ((status = load_pixmap(widget, filebase, ".bm", pixmap_p)) !=
                    OK)
                        if ((status = load_pixmap(widget, filebase, ".xbm",
                                                  pixmap_p)) != OK)
                                if ((status = load_pixmap(widget, filebase,
                                                          ".pm", pixmap_p)) !=
                                    OK)
...
XFontStruct *objxm_fontlist_to_font(XmFontList fontlist) {
        ...
        if (XmFontListGetNextFont(context, &charset, &font)) {
                ...
                util_puts_err(
                    "objxm_fontlist_to_font: could not convert fontlist\n");
        }
...
XmString objxm_str_to_xmstr(Widget widget, String string) {
        XrmValue src, dst;
        XmString xmstr;
...
STRING
objxm_xmstr_to_str(XmString xmstr) {
        ...
        if (XmStringInitContext(&context, xmstr)) {
                while (XmStringGetNextSegment(context, &seg, &char_set_tag,
                                              &dir, &sep)) {
        ...
        if (util_get_verbosity() > 0) {
                util_puts_err(
                    "objxm_xmstr_to_str: could not convert xmstring\n");
        }
        return NULL;
}
 ...
 * accelerator
 */
String objxm_accel_to_acceltext(String accelstr) {
        char buf[128];
...
unsigned char objxm_linestyle_to_enum(AB_LINE_TYPE linestyle) {
        unsigned char value;
...
BOOL objxm_is_menu_widget(Widget widget) {
