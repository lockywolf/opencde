...
static void expand_path(char *nm, char *buf);
 ...
 * and returns ERROR.
 */
int abio_expand_path(char *path) {
        char buf[MAXPATHLEN];
 ...
 */
int abio_gil_expand_path(char *path) {
...
int abio_expand_bil_module_path(char *path) {
 ...
 */
int abio_gil_expand_proj_path(char *path) {
...
int abio_expand_bil_proj_path(char *path) {
...
#else
    abio_is_gil_path(path) char *path;
#endif
{
        return (strcmp(path + strlen(path) - strlen(GIL_SUFFIX), GIL_SUFFIX) ==
                0);
}

BOOL util_file_name_is_bil_module(STRING path) {
...
BOOL util_file_name_is_bil_proj(STRING path) {
        return util_file_name_has_extension(path, BIL_PROJ_SUFFIX);
}

BOOL util_file_name_is_bil_encapsulated(char *path) {
