...
int util_set_verbosity(int newVerbosity) {
        ...
        if (((oldVerbosity >= 3) || (newVerbosity >= 3)) &&
            (oldVerbosity != newVerbosity)) {
                ...
                if (newDebugLevel > 0) {
                        util_dprintf(0, " [ Debugging level %d ]",
                                     newDebugLevel);
                }
                util_dprintf(0, "\n");
                utilP_verbosity = newVerbosity;
...
int util_set_program_name(STRING new_program_name) {
...
util_get_program_name(void) {
        return (program_name == NULL ? default_program_name
...
extern "C" {
#endif
#if defined(__uxp__) || defined(__hpux) ||                                     \
...
#endif
int util_putenv(STRING string) { return putenv(string); }
...
#define IS_MATCH(a, b, c, d, e, f, g, h, i, j)                                 \
        ...
        if (IS_MATCH(lnxIndex, aixIndex, hpIndex, sunIndex, osf1Index,
                     unixwareIndex, uxpIndex, fbsdIndex, nbsdIndex,
                     obsdIndex)) {
                return AB_OS_LNX;
        }
