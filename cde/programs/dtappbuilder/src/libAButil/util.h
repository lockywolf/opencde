...
int util_flock(FILE *file, BOOL wait, int lockType, off_t offset, off_t length);
...
int util_cvt_path_to_relative(/* NULL from = cwd */
                              STRING path, STRING from, char *buf,
                              int buf_size);
...
STRING util_get_file_name_from_path(STRING path, STRING fileNameBuf,
                                    int fileNameBufSize);

STRING util_get_dir_name_from_path(STRING path, STRING dirNameBuf,
                                   int dirNameBufSize);
...
#define util_fclose(fp)                                                        \
        ((fp) == NULL ? 0                                                      \
                      : (util_funlock(fp, 0, 0), fclose(fp), (fp) = NULL, 0))
