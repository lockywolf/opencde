...
static int strlistP_build_user_data_array(StringList list, int index,
                                          void *data);
...
int strlist_construct(StringList list) {
        list->num_strings = 0;
        list->strings_size = 0;
        list->strings = NULL;
 ...
 */
int strlist_destroy(StringList list) {
        strlist_destruct(list);
        free(list);
        return 0;
}

int strlist_destruct(StringList list) { return strlist_make_empty(list); }

STRLIST_SORT_ORDER
strlist_get_sort_order(StringList list) { return list->sort_order; }

int strlist_set_sort_order(StringList list, STRLIST_SORT_ORDER new_order) {
        int return_value = 0;
...
int strlist_make_empty(StringList list) {
        int return_value = 0;
...
int strlist_is_empty(StringList list) { return (list->num_strings < 1); }

int strlist_set_is_unique(StringList list, BOOL unique) {
        list->unique = unique;
        return 0;
}

BOOL strlist_get_is_unique(StringList list) { return list->unique; }
 ...
 * exist.
 */
BOOL strlist_istr_exists(StringList list, ISTRING istring) {
        return (strlist_get_istr_index(list, istring) >= 0);
}

long strlist_set_istr_data(StringList list, ISTRING istring, void *data) {
...
void *strlist_get_istr_data(StringList list, ISTRING istring) {
 ...
 */
long strlist_get_istr_index(StringList list, ISTRING string) {
        long index = -1;
        int i = 0;
 ...
 * already in list
 */
int strlist_add_istr(StringList list, ISTRING istring, void *clientData) {
        int return_value = 0;
        int rc = 0;
...
long strlist_remove_istr(StringList list, ISTRING istring) {
        long index = strlist_get_istr_index(list, istring);
        if (index < 0) {
                return 0;
        }
...
int strlist_remove_index(StringList list, int doomedIndex) {
...
int strlist_get_num_strs(StringList list) {
        if (list == NULL) {
                return 0;
        }
...
strlist_get_istr(StringList list, int whichString, void **clientDataOut) {
        ...
        if (clientDataOut != NULL) {
                *clientDataOut =
                    (list->user_datas == NULL ? NULL
                                              : list->user_datas[whichString]);
        }
 ...
 */
BOOL strlist_max_size_is_soft(StringList list) { return TRUE; }

int strlist_set_max_size_soft(StringList list, int maxSize) {
        int return_value = 0;
...
int strlist_get_max_size(StringList list) { return list->strings_size; }
 ...
 */
int strlist_shrink_mem_to_fit(StringList list) {
        int return_value = 0;
...
int strlist_add_str(StringList list, STRING string, void *clientData) {
        int return_value = 0;
...
#ifdef UNIMPLEMENTED
int strlist_add_index_str(StringList list, int index, STRING string,
                          void *clientData) {
        int return_value = 0;
        ISTRING istring = istr_create(string);
...
BOOL strlist_str_exists(StringList list, STRING string) {
        BOOL doesIt = FALSE;
...
long strlist_get_str_index(StringList list, STRING string) {
        long index = -1;
        ISTRING istring = istr_create(string);
        if (istring != NULL) {
...
long strlist_remove_str(StringList list, STRING string) {
        long return_value = 0;
        ISTRING istring = istr_dup_existing(string);
        if (istring != NULL) {
...
long strlist_set_str_data(StringList list, STRING string, void *data) {
        long return_value = 0;
        ISTRING istring = istr_dup_existing(string);
        if (istring != NULL) {
...
void *strlist_get_str_data(StringList list, STRING string) {
...
StringList strlist_dup(StringList list) {
        int i;
        ...
        for (i = 0; i < list->num_strings; i++) {
                strlist_add_istr(new_list, list->strings[i],
                                 list->user_datas[i]);
        }
        return (new_list);
}
...
int strlistP_shrink_array(StringList list, int sizeDiff) {
        int return_value = 0;
        ...
        if (new_strings_size > 0) {
                ...
                util_free(list->strings);
                list->strings = NULL;
                util_free(list->user_datas);
                list->user_datas = NULL;
        }
...
static int strlistP_grow_array(StringList list, int sizeDiff) {
        ...
        new_strings = (ISTRING *)realloc(list->strings,
                                         new_strings_size * sizeof(ISTRING));
        new_user_datas = (void **)realloc(list->user_datas,
                                          new_strings_size * sizeof(void *));
...
int strlist_dump(StringList list) {
        ...
        for (i = 0; i < list->strings_size; ++i) {
                ...
                util_dprintf(0, "%d[%s]: '%s'", i,
                             (i >= list->num_strings ? " " : "X"), string);
