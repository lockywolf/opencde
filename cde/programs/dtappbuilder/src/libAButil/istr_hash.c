...
int insert(HASH *hashtable, ISTRING istring, void *str) {
        int hashval;
        void *str_exists;
        HASH *new_bucket;
        int i, j;
        ...
        if (istring == NULL) {
                fprintf(stderr, "ISTR: can't create empty string\n");
                return NULL;
        }
...
int delete (HASH *hashtable, ISTRING istring) {
...
static void *
hash_lookup(HASH *hashtable,
            ISTRING istr) { /* search hash table for existing strings */
        HASH entry;
        int i;
        unsigned val;
