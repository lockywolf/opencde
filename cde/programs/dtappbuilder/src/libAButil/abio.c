...
static int abioP_build_indent_string(FILE *out_file, int indent_char,
 ...
 */
int abio_close_input_impl(FILE **pFile) {
#define file (*pFile)
 ...
 */
int abio_close_output_impl(FILE **pOutFile) {
 ...
 */
STRING
abio_comment_string(void) { return "//"; }

/*
 * Return the boolean false string.
 */
STRING
abio_bil_false_string(void) { return ":false"; }

/*
 * Return the file begin string.
 */
STRING
abio_file_begin_string(void) { return ":bil-version"; }

/*
 * Return the file end string.
 */
STRING
abio_file_end_string(void) { return ")"; }

/*
 * Get a boolean from the input file.  Returns TRUE if successful.
 */
int abio_bil_get_boolean(FILE *inFile, BOOL *valueOut) {
 ...
 */
int abio_get_handler(FILE *inFile, ISTRING *stringOutPtr) {
 ...
 * Get a boolean from the input file.  Returns TRUE if successful.
 */
int abio_gil_get_boolean(FILE *inFile, BOOL *valueOut) {
        STRING p = get_token(inFile);
 ...
 */
STRING
abio_gil_false_string(void) { return "nil"; }

/*
 * Return the boolean true string.
 */
STRING
abio_gil_true_string(void) { return "t"; }

/*
 * Get an integer from the input file.  Returns TRUE if successful.
 */
int abio_get_integer(FILE *inFile, int *i) {
        STRING p = get_token(inFile);
        STRING q;
 ...
 */
int abio_get_keyword(FILE *inFile, ISTRING *s) {
 ...
 */
int abio_get_list(FILE *inFile, ISTRING *stringOutPtr) {
#define stringOut (*stringOutPtr)
#define INC 32
        int ch;
        int len; /* string length */
 ...
 */
int abio_gil_get_name(FILE *inFile, ISTRING *stringOutPtr) {
#define stringOut (*stringOutPtr)
        STRING p = get_token(inFile);
        stringOut = NULL;
 ...
 */
int abio_bil_get_full_name(FILE *inFile, ISTRING *parentOutPtr,
                           ISTRING *nameOutPtr) {
 ...
 */
int abio_gil_get_full_name(FILE *inFile, ISTRING *interfaceStringOut,
                           ISTRING *parentStringOut, ISTRING *objStringOut,
 ...
 */
int abio_get_proj_full_name(FILE *inFile, STRING *interface, STRING *parent,
 ...
 */
int abio_get_string(FILE *inFile, ISTRING *istringOutPtr) {
 ...
 */
STRING
abio_keyword_string(STRING s) { return s; }

/*
 * Return the list begin string.
 */
STRING
abio_list_begin_string(void) { return "("; }

/*
 * Return the list end string.
 */
STRING
abio_list_end_string(void) { return ")"; }

/*
 * Return the string representation of a name.
 */
STRING
abio_name_string(STRING s) { return s ? s : abio_bil_false_string(); }

/*
 * Return the object begin string.
 */
STRING
abio_gil_object_begin_string(void) { return "("; }

/*
 * Return the object end string.
 */
STRING
abio_gil_object_end_string(void) { return ")"; }
...
STRING
abio_check_bil_version(float ver, STRING name) {
#define MAXVER 10
        STRING errmsg = NULL;
        ...
        fileName =
            name ? name
                 : catgets(UTIL_MESSAGE_CATD, UTIL_MESSAGE_SET, 35, "NoName");
        ...
        if (ver > Util_major_version) {
                ...
                help_msg =
                    catgets(UTIL_MESSAGE_CATD, UTIL_MESSAGE_SET, 33,
                            "The version of the BIL file %s (%3.2f)\nis not "
                            "supported in this version of App Builder.");

                help_buf = (STRING)util_malloc(strlen(help_msg) +
                                               strlen(fileName) + MAXVER + 1);
                ...
                help_msg =
                    catgets(UTIL_MESSAGE_CATD, UTIL_MESSAGE_SET, 31,
                            "The file %s does not appear to be in BIL\nformat. "
                            " Either a BIL header (:bil-version) was\nnot "
                            "found or the header is corrupt.");
 ...
 * message.
 */
int abio_open_bil_output(STRING outfile, STRING old_file, FILE **pOutFile) {
#define outFile (*pOutFile)
        ...
        *backup = 0;
         ...
         */
        fprintf(outFile, "%s\t%d %d\n", AB_VERSION_PREFIX, Util_major_version,
                Util_minor_version);
 ...
 */
STRING
abio_bil_object_begin_string(void) { return "("; }

/*
 * Return the object end string.
 */
STRING
abio_bil_object_end_string(void) { return ")"; }
 ...
 */
int abio_printf(FILE *outFile, STRING fmt, ...) {
        va_list args;
...
int abio_print_line(FILE *outFile, STRING fmt, ...) {
        int iReturn = 0;
        va_list args;
 ...
 * are on a new line.
 */
int abio_putc(FILE *outFile, char c) {
        if (Newline)
                fputs(Indent, outFile);
        fputc(c, outFile);
        Newline = (c == '\n');
 ...
 * are on a new line.
 */
int abio_puts(FILE *outFile, STRING s) {
        if (Newline)
                fputs(Indent, outFile);
        fputs(s, outFile);
 ...
 */
int abio_bil_put_boolean(FILE *outFile, BOOL value) {
 ...
 * Write an integer to the output file.
 */
int abio_put_float(FILE *outFile, double d) {
        sprintf(Buf, "%f", d);
 ...
 * Write an integer to the output file.
 */
int abio_put_integer(FILE *outFile, int i) {
        sprintf(Buf, "%d", i);
        return abio_puts(outFile, Buf);
}

int abio_put_keyword_name(FILE *outFile, STRING name) {
 ...
 */
int abio_put_keyword(FILE *outFile, AB_OBJECT_TYPE abo_type) {
        switch (abo_type) {
 ...
 */
int abio_put_handler(FILE *outFile, STRING s) {
        STRING newstr;
        STRING p;
 ...
 */
int abio_gil_put_name(FILE *outFile, STRING s) {
 ...
 */
int abio_put_full_name(FILE *outFile, STRING parent, STRING name, STRING item) {
 ...
 */
int abio_put_proj_full_name(FILE *outFile, STRING interface, STRING parent,
                            STRING name, STRING item) {
        abio_puts(outFile, abio_list_begin_string());
 ...
 * (e.g., "\"hi\"", "newline:\n")
 */
int abio_put_string(FILE *outFile, STRING s) {
 ...
 * Write a string to the output file.
 */
int abio_put_string_to_file(FILE *outFile, STRING string) {
        register STRING ptr;
 ...
 */
int abio_indent(FILE *out_file) {
        return abioP_build_indent_string(
            out_file, Indent_char, Indent_chars_per_level, Indent_level + 1);
}

int abio_outdent(FILE *out_file) {
        return abioP_build_indent_string(
            out_file, Indent_char, Indent_chars_per_level, Indent_level - 1);
}
 ...
 */
int abio_get_indent(FILE *out_file) {
        out_file = out_file; /* avoid warning */
        return Indent_level;
}

int abio_set_indent(FILE *out_file, int indent) {
        return abioP_build_indent_string(out_file, Indent_char,
                                         Indent_chars_per_level, indent);
}

/*
 * Set indent char
 */
int abio_set_indent_char(FILE *out_file, int indent_char) {
        return abioP_build_indent_string(out_file, indent_char,
                                         Indent_chars_per_level, Indent_level);
}

int abio_get_indent_char(FILE *out_file) {
        out_file = out_file; /* avoid warning */
        return Indent_char;
}

int abio_set_indent_chars_per_level(FILE *out_file, int chars_per_level) {
        return abioP_build_indent_string(out_file, Indent_char, chars_per_level,
                                         Indent_level);
}

int abio_get_indent_chars_per_level(FILE *out_file) {
        out_file = out_file; /* avoid warning */
 ...
 */
static int abioP_build_indent_string(FILE *out_file, int new_indent_char,
 ...
 */
STRING
abio_string_begin_string(void) { return "\""; }

/*
 * Return the string end string.
 */
STRING
abio_string_end_string(void) { return "\""; }
 ...
 */
STRING
abio_bil_true_string(void) { return ":true"; }
 ...
 * at end-of-file.
 */
static STRING get_token(FILE *inFile) {
        int ch = 0;
 ...
 */
static int gil_version(FILE *fp) {
 ...
 */
static void skip_comments(FILE *inFile) {
        long pos;
 ...
 */
static int debug_unbuffer_file(FILE *file) {
...
int util_set_output_handler(UtilOutputHandler new_output_handler) {
        output_handler = new_output_handler;
        return 0;
}

int util_set_err_output_handler(UtilOutputHandler new_err_output_handler) {
 ...
 */
int abio_access_file(STRING name, BOOL *read_OK, BOOL *write_OK) {
        struct stat statbuf;
        uid_t userid;
        gid_t groupid;
        int ret = 0;
 ...
 */
STRING
abio_module_string(void) { return ":module"; }

/*
 * Return the project keyword.
 */
STRING
abio_project_string(void) { return ":project"; }
 ...
 */
int abio_put_string_to_buffer(STRING string, STRING outBuf, int size) {
#define INC 32
        register STRING ptr;
        char strEndChar = *(abio_string_end_string());
        char curChar = 0;
 ...
 */
void util_set_help_data(STRING msg, STRING vol, STRING locID) {
        if (help_text) {
                util_free(help_text);
                help_text = NULL;
        }
 ...
 */
int util_get_help_data(STRING *msg, STRING *vol, STRING *locID) {
        if (msg)
                *msg = help_text;
        if (vol)
                *vol = help_volume;
