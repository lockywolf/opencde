...
static STRING convert_int_to_string(int value, ISTRING *table, int num_values,
                                    ISTRING default_value) {
...
static int convert_string_to_int(STRING s_value, ISTRING *table, int num_values,
                                 int default_value) {
        int i;
...
static int convert_fill_table(ISTRING *table, int num_values, ISTRING value) {
        int i;
        for (i = 0; i < num_values; ++i) {
                table[i] = value;
        }
...
static int builtin_action_table_init() {
#define bat builtin_action_table
        builtin_action_table_inited = TRUE;
        convert_fill_table(builtin_action_table, AB_BUILTIN_ACTION_NUM_VALUES,
                           istr_const("?when?"));
...
util_string_to_builtin_action(STRING action) {
        return (AB_BUILTIN_ACTION)convert_string_to_int(
            action, builtin_action_table, AB_BUILTIN_ACTION_NUM_VALUES,
            AB_STDACT_UNDEF);
}
...
util_builtin_action_to_string(AB_BUILTIN_ACTION action) {
        ...
        return convert_int_to_string(action, builtin_action_table,
...
static int obj_type_table_init() {
#define ott obj_type_table
        obj_type_table_inited = TRUE;
        convert_fill_table(obj_type_table, AB_OBJECT_TYPE_NUM_VALUES,
                           istr_const("?type?"));
...
util_string_to_object_type(STRING string_type) {
        ...
        return (AB_OBJECT_TYPE)convert_string_to_int(
            string_type, obj_type_table, AB_OBJECT_TYPE_NUM_VALUES,
            AB_TYPE_UNKNOWN);
}
...
util_object_type_to_string(AB_OBJECT_TYPE type) {
        check_obj_type_table_init();
        return convert_int_to_string(type, obj_type_table,
                                     AB_OBJECT_TYPE_NUM_VALUES,
                                     istr_const("?type?"));
}
...
static int browser_obj_type_table_init() {
#define bott browser_obj_type_table
        browser_obj_type_table_inited = TRUE;
        convert_fill_table(browser_obj_type_table, AB_OBJECT_TYPE_NUM_VALUES,
                           istr_const("?type?"));
...
util_browser_string_to_object_type(STRING string_type) {
        ...
        return (AB_OBJECT_TYPE)convert_string_to_int(
            string_type, browser_obj_type_table, AB_OBJECT_TYPE_NUM_VALUES,
            AB_TYPE_UNKNOWN);
}
...
util_object_type_to_browser_string(AB_OBJECT_TYPE type) {
        check_browser_obj_type_table_init();
        return convert_int_to_string(type, browser_obj_type_table,
                                     AB_OBJECT_TYPE_NUM_VALUES,
                                     istr_const("?type?"));
}
...
util_string_to_when(STRING when_string) {
        ...
        return (AB_WHEN)convert_string_to_int(
            when_string, when_table, AB_WHEN_NUM_VALUES, AB_WHEN_UNDEF);
}

STRING
util_when_to_string(AB_WHEN when) {
        check_when_table_init();
        return convert_int_to_string(when, when_table, AB_WHEN_NUM_VALUES,
                                     istr_const("?when?"));
}
...
util_string_to_alignment(STRING align_string) {
        ...
        return (AB_ALIGNMENT)convert_string_to_int(align_string, align_tab,
                                                   ARRAY_SIZE(align_tab), -1);
}

STRING
util_alignment_to_string(AB_ALIGNMENT align) {
        align_tab_check_init();
        return convert_int_to_string(align, align_tab, ARRAY_SIZE(align_tab),
                                     istr_const(NULL));
}
...
static int label_type_tab_init() {
...
util_string_to_label_type(STRING type_string) {
        ...
        return (AB_LABEL_TYPE)convert_string_to_int(
            type_string, label_type_tab, ARRAY_SIZE(label_type_tab), -1);
}

STRING
util_label_type_to_string(AB_LABEL_TYPE label_type) {
        label_type_tab_check_init();
        return convert_int_to_string(label_type, label_type_tab,
                                     ARRAY_SIZE(label_type_tab), NULL);
}
...
static int button_type_tab_init() {
...
util_string_to_button_type(STRING type_string) {
        ...
        return (AB_BUTTON_TYPE)convert_string_to_int(
            type_string, button_type_tab, ARRAY_SIZE(button_type_tab), -1);
}

STRING
util_button_type_to_string(AB_BUTTON_TYPE type) {
        button_type_tab_check_init();
        return convert_int_to_string(type, button_type_tab,
                                     ARRAY_SIZE(button_type_tab), NULL);
}
...
util_string_to_arg_class(STRING arg_class_string) {
        ...
        return (AB_ARG_CLASS)convert_string_to_int(
            arg_class_string, arg_class_tab, ARRAY_SIZE(arg_class_tab), -1);
}

STRING
util_arg_class_to_string(AB_ARG_CLASS arg_class) {
        arg_class_tab_check_init();
        return convert_int_to_string(arg_class, arg_class_tab,
                                     ARRAY_SIZE(arg_class_tab), NULL);
}
...
#define arg_type_tab_check_init()                                              \
        (arg_type_tab_inited ? 0 : arg_type_tab_init())

static int arg_type_tab_init() {
...
util_string_to_arg_type(STRING arg_type_string) {
        ...
        return (AB_ARG_TYPE)convert_string_to_int(arg_type_string, arg_type_tab,
                                                  ARRAY_SIZE(arg_type_tab), -1);
}

STRING
util_arg_type_to_string(AB_ARG_TYPE arg_type) {
        arg_type_tab_check_init();
        return convert_int_to_string(arg_type, arg_type_tab,
                                     ARRAY_SIZE(arg_type_tab), NULL);
}

/******************* AB_TEXT_TYPE **************************************/
AB_TEXT_TYPE
util_string_to_text_type(STRING s_text_type) { return AB_TEXT_ALPHANUMERIC; }
...
/******************* AB_CHOICE_TYPE **************************************/
AB_CHOICE_TYPE
util_string_to_choice_type(STRING s_choice_type) { return AB_CHOICE_EXCLUSIVE; }
...
/******************* AB_ORIENTATION **************************************/
AB_ORIENTATION
util_string_to_orientation(STRING s_orient) { return AB_ORIENT_HORIZONTAL; }
...
/******************* AB_MENU_TYPE **************************************/
AB_MENU_TYPE
util_string_to_menu_type(STRING s_menu_type) { return AB_MENU_PULLDOWN; }
...
/******************* AB_GROUP_TYPE **************************************/
AB_GROUP_TYPE
util_string_to_group_type(STRING s_group) { return AB_GROUP_IGNORE; }
...
/******************* AB_OBJECT_STATE **************************************/
AB_OBJECT_STATE
util_string_to_object_state(STRING s_state) { return AB_STATE_ACTIVE; }
...
#define container_tab_check_init()                                             \
        (container_tab_inited ? 0 : container_tab_init())

static int container_tab_init() {
...
util_string_to_container_type(STRING container_string) {
        ...
        return (AB_CONTAINER_TYPE)convert_string_to_int(
            container_string, container_tab, ARRAY_SIZE(container_tab), -1);
}
...
util_container_type_to_string(AB_CONTAINER_TYPE type) {
        ...
        return convert_int_to_string(
            type, container_tab, ARRAY_SIZE(container_tab), istr_const(NULL));
}
...
util_string_to_item_type(STRING item_string) {
        item_tab_check_init();
        return (AB_ITEM_TYPE)convert_string_to_int(item_string, item_tab,
                                                   ARRAY_SIZE(item_tab), -1);
}

STRING
util_item_type_to_string(AB_ITEM_TYPE type) {
        item_tab_check_init();
        return convert_int_to_string(type, item_tab, ARRAY_SIZE(item_tab),
                                     istr_const(NULL));
}
