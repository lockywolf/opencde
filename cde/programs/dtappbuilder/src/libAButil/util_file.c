 ...
 */
int util_unbuffer_file(FILE *fp) {
        int iReturn = 0;
 ...
 *
 */
long util_file_size(STRING fileName) {
        struct stat fileInfo;
 ...
 * has no extension
 */
BOOL util_file_name_has_extension(STRING fileName, STRING extension) {
...
BOOL util_file_name_has_ab_extension(STRING fileName) {
        BOOL hasExtension = FALSE;
        char *dotPtr = strrchr(fileName, '.');
...
STRING
util_get_file_name_from_path(STRING path, STRING fileNameBuf,
                             int fileNameBufSize) {
 ...
 */
STRING
util_get_dir_name_from_path(STRING path, STRING dirNameBuf,
                            int dirNameBufSize) {
        char *slashPtr = strrchr(path, '/');
...
#endif
int util_fdtruncate(int filedes, off_t length) {
        return ftruncate(filedes, length);
}

BOOL util_directory_exists(STRING dir_name) {
        BOOL exists = FALSE;
 ...
 */
int util_cvt_path_to_relative(STRING path_in, STRING from_dir, char *buf,
                              int buf_size) {
 ...
 */
int util_mkdir_hier(STRING path) {
...
BOOL util_path_is_absolute(STRING dir) {
        BOOL val = FALSE;
 ...
 */
int util_derive_name_from_path(char *fullpath, char *objname) {
        char *filename, *name;
        int len = 0;
 ...
 */
int util_check_name(STRING name, STRING new_name) {
        int len = 0;
 ...
 *
 */
FILE *util_fopen_locked(const char *filename, const char *accessType) {
        FILE *file = NULL;
        char char1 = accessType[0];
        BOOL charPlus = ((accessType[1] != 0) &&
...
int util_flock(FILE *file, BOOL wait, int lockType, off_t offset,
               off_t length) {
        struct flock lock;
...
int util_funlock(FILE *file, off_t offset, off_t length) {
        struct flock lock;
