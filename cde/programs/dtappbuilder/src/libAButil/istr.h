 ...
 *	 string
 */
extern ISTRING istr_create_const(STRING string);

/*
 * Identical to istr_create_const (shortcut)
 */
extern ISTRING istr_const(STRING string);
 ...
 *	 string
 */
extern ISTRING istr_create_alloced(STRING string);
 ...
 *	 the string.
 */
extern ISTRING istr_create(STRING string);
 ...
 */
extern int istr_destroy(ISTRING istring);

/*
 * Return the string value, and increment counter
 * return -1 if error
 */
extern ISTRING istr_dup(ISTRING istring);

/*
 * Return (STRING) for istring, return NULL if error
 */
extern STRING istr_string(ISTRING istring);
 ...
 */
extern STRING istr_string_safe(ISTRING istring);
 ...
 */
extern ISTRING istr_dup_existing(STRING s);
 ...
 * always returns 0
 */
extern int istr_move(ISTRING to, ISTRING from);

/*
 * Returns the length of the string, returns NULL if the
 * string is empty
 */
extern int istr_len(ISTRING s);

/*
 * Compare the two strings, returns the same as strcmp
 */
extern int istr_cmp(ISTRING s1, ISTRING s2);

/*
 * Compare the two istrings, returns TRUE if equal
 */
extern BOOL istr_equal(ISTRING s1, ISTRING s2);

/*
 * Compare the two strings, returns the same as strcmp
 */
extern int istr_cmpstr(ISTRING s1, STRING s2);

/*
 * Compare the two strings, returns TRUE if equal
 */
extern BOOL istr_equalstr(ISTRING s1, STRING s2);
...
#define istr_destroy(s) (istrP_destroy_impl518283652PrivF(&(s)))

#define istr_create_alloced(s) (istrP_create_alloced_impl9726039350PrivF(&(s)))
...
#define istr_string(istring)                                                   \
        ((!debugging()) ? istrP_get_string_fast3718930164PrivF(istring)        \
                        : istrP_get_string_verify4521632085PrivF(istring))
#else
#define istr_string(istring) (istrP_get_string_fast3718930164PrivF(istring))
#endif /* DEBUG */
