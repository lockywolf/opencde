...
static unsigned hashing(char *p);

static int insert_array(int flag, int val, char *string);

static int hash_and_lookupstr(char *string, int *hash_val_out_ptr);
...
istr_create(STRING string) {
        ...
        if ((str_exists = hash_and_lookupstr(string, &hashval)) ==
            -1) /* new entry */
        {
                ...
                for (i = 0; i < BUCKETSIZE; i++) {
                        if (entry->values[i] == -1) {
                                ...
                                if (freecount == 0) {
                                        ...
                                        if (insert_array(1, num_count,
                                                         string) == -1) {
                                                fprintf(
                                                    stderr,
                                                    catgets(UTIL_MESSAGE_CATD,
                                                            UTIL_MESSAGE_SET, 1,
                                                            "ISTR: error in "
                                                            "allocating to int "
                                                            "array\n"));
                                        ...
                                        num_count++;
                                        hash_count++;
                                        return istrP_int_to_client(num_count -
                                                                   1);
                                } else { /* take int value from freelist */
                                        if (int_array[freelist[freecount - 1]]
                                                .refcount != 0) {
                                                fprintf(
                                                    stderr,
                                                    catgets(UTIL_MESSAGE_CATD,
                ...
                /* if get this far, then no space is available in existing
                ...
                if (freecount == 0) {
                        ...
                        if (int_array[freelist[freecount - 1]].refcount != 0) {
                                fprintf(stderr,
                                        catgets(UTIL_MESSAGE_CATD,
                                                UTIL_MESSAGE_SET, 2,
                                                "ISTR: error in allocating "
                                                "space for string\n"));
                                return NULL;
                        }
                        new_bucket->values[0] = freelist[freecount - 1];
                        entry->next = new_bucket;
                        if (insert_array(1, freelist[freecount - 1], string) ==
                            -1) {
                                fprintf(stderr,
                                        catgets(UTIL_MESSAGE_CATD,
                                                UTIL_MESSAGE_SET, 1,
 ...
 */
ISTRING
istrP_create_alloced_impl(STRING *string) {
        int hashval;
        long str_exists;
        Bucket new_bucket;
        int i;
            ...
            -1) /* new entry */
        {
                ...
                if (freecount == 0) {
                        ...
                        new_bucket->values[0] = freelist[freecount - 1];
                        entry->next = new_bucket;
                        if (insert_array(3, freelist[freecount - 1], *string) ==
                            -1) {
                                fprintf(stderr,
                                        catgets(UTIL_MESSAGE_CATD,
                                                UTIL_MESSAGE_SET, 1,
 ...
 */
ISTRING
istr_create_const(STRING string) {
        int hashval;
        long str_exists;
        Bucket new_bucket;
        int i;
            ...
            -1) /* new entry */
        {
                ...
                for (i = 0; i < BUCKETSIZE; i++) {
                        if (entry->values[i] == -1) {
                                ...
                                if (freecount ==
                                    0) { /* no numbers on freelist */
                                        entry->values[i] = num_count;
                                        if (insert_array(2, num_count,
                                                         string) == -1) {
                                                fprintf(
                                                    stderr,
                                                    catgets(UTIL_MESSAGE_CATD,
                ...
                if (freecount == 0) {
                        ...
                        new_bucket->values[0] = freelist[freecount - 1];
                        entry->next = new_bucket;
                        if (insert_array(2, freelist[freecount - 1], string) ==
                            -1) {
                                fprintf(stderr,
                                        catgets(UTIL_MESSAGE_CATD,
                                                UTIL_MESSAGE_SET, 1,
        ...
        {
                if (int_array[str_exists].read_const ==
                    0) { /* change read only string to const string */
                        if ((string != int_array[str_exists].str) &&
                            (&string != &(int_array[str_exists].str))) {
 ...
 */
ISTRING
istr_dup_existing(STRING string) {
        int hashval;
        long str_exists;
 ...
 */
int istrP_destroy_impl(ISTRING *istringClientPtr) {
        ...
        int_array[istring].refcount--;
        if (int_array[istring].refcount == 0) { /* remove from hash table */
                ...
                if (free_num == 0) {
                        ...
                        if (freelist == NULL) {
                                fprintf(
                                    stderr,
                                    catgets(
                                        UTIL_MESSAGE_CATD, UTIL_MESSAGE_SET, 3,
                                        "ISTR: error in allocating memory\n"));
                                return 0;
                        }
                        free_num = 1;
                ...
                {
                        free_num++;
                        freelist = (long *)realloc(
                            freelist, (ARRAYSIZE * free_num) * sizeof(long));
 ...
 */
ISTRING
istr_dup(ISTRING istringClientVal) {
 ...
 */
STRING
istrP_get_string_verify(ISTRING istringClientVal) {
        long istring = istrP_client_to_int(istringClientVal);
 ...
 * hash function
 */
static unsigned hashing(STRING string) {
...
static int insert_array(int flag, int val, char *string) {
        ...
        {
                array_num++;
                int_array_set((STRN *)realloc(
                    int_array, (ARRAYSIZE * array_num) * sizeof(STRN)));
 ...
 */
int istr_verify(ISTRING istringClientVal) {
 ...
 */
int istr_verify_all(void) { return 0; }
