...
#define ab_label_is_arrow(lt)                                                  \
        ((lt) == AB_LABEL_ARROW_DOWN || (lt) == AB_LABEL_ARROW_UP ||           \
         (lt) == AB_LABEL_ARROW_RIGHT || (lt) == AB_LABEL_ARROW_LEFT)
/*
 * Scale subtypes.
 */
typedef enum {
...
AB_ARG_TYPE util_string_to_arg_type(STRING arg_type);
STRING util_arg_type_to_string(AB_ARG_TYPE arg_type);

AB_ARG_CLASS util_string_to_arg_class(STRING arg_class_string);
STRING util_arg_class_to_string(AB_ARG_CLASS arg_class);
AB_ARG_CLASS_FLAGS util_arg_class_to_flag(AB_ARG_CLASS arg_class);
AB_ARG_CLASS util_flag_to_arg_class(AB_ARG_CLASS_FLAGS arg_flag);

AB_LABEL_TYPE util_string_to_label_type(STRING label_type);
STRING util_label_type_to_string(AB_LABEL_TYPE label_type);

AB_OBJECT_STATE util_string_to_object_state(STRING state);
STRING util_object_state_to_string(AB_OBJECT_STATE state);

AB_OBJECT_TYPE util_string_to_object_type(STRING type);
STRING util_object_type_to_string(AB_OBJECT_TYPE type);

AB_ORIENTATION util_string_to_orientation(STRING orientation);
STRING util_orientation_to_string(AB_ORIENTATION orientation);

AB_MENU_TYPE util_string_to_menu_type(STRING menu_type);
STRING util_menu_type_to_string(AB_MENU_TYPE menu_type);

AB_BUTTON_TYPE util_string_to_button_type(STRING button_type);
STRING util_button_type_to_string(AB_BUTTON_TYPE button_type);

AB_ALIGNMENT util_string_to_alignment(STRING alignment);
STRING util_alignment_to_string(AB_ALIGNMENT alignment);

AB_GROUP_TYPE util_string_to_group_type(STRING group_type);
STRING util_group_type_to_string(AB_GROUP_TYPE group_type);

AB_CHOICE_TYPE util_string_to_choice_type(STRING choice_type);
STRING util_choice_type_to_string(AB_CHOICE_TYPE choice_type);

AB_TEXT_TYPE util_string_to_text_type(STRING text_type);
STRING util_text_type_to_string(AB_TEXT_TYPE text_type);

AB_BUILTIN_ACTION util_string_to_builtin_action(STRING action);
STRING util_builtin_action_to_string(AB_BUILTIN_ACTION action);

AB_WHEN util_string_to_when(STRING when);
STRING util_when_to_string(AB_WHEN when);

STRING util_object_type_to_browser_string(AB_OBJECT_TYPE type);
AB_OBJECT_TYPE util_browser_string_to_object_type(STRING string_type);

AB_CONTAINER_TYPE util_string_to_container_type(STRING container_string);
STRING util_container_type_to_string(AB_CONTAINER_TYPE type);

AB_ITEM_TYPE util_string_to_item_type(STRING item_string);
STRING util_item_type_to_string(AB_ITEM_TYPE type);
