...
static ABObjPtr uil_tree_into_ab_project(sym_entry_type *uil_parse_tree,
                                         ABObjPtr ab_project);
static ABObjPtr root_into_project(sym_entry_type *uil_root,
                                  ABObjPtr ab_project);
static ABObjPtr module_hdr_to_module(sym_entry_type *uil_module_hdr);
static ABObjPtr sections_into_module(ABObjPtr ab_module,
                                     sym_entry_type *uil_sections);
static ABObjPtr object_into_module(ABObjPtr ab_module,
                                   sym_entry_type *uil_section_entries);
static ABObjPtr object_child_into_module(ABObjPtr ab_module,
                                         sym_entry_type *uil_section_entries);
static ABObjPtr widget_into_module(ABObjPtr ab_module, ABObjPtr ab_parent,
                                   sym_widget_entry_type *uil_widget);
static ABObjPtr gadget_into_module(ABObjPtr ab_module, ABObjPtr ab_parent,
                                   sym_widget_entry_type *uil_gadget);
static ABObjPtr
arguments_into_ab_widget(ABObjPtr ab_widget,
                         sym_list_entry_type *uil_widget_arguments);
static ABObjPtr
callbacks_into_ab_module(ABObjPtr ab_widget,
                         sym_list_entry_type *uil_widget_callbacks);
static STRING ab_name_for_uil_widget(sym_widget_entry_type *uil_widget);
static STRING resource_name_for_uil_arg(sym_argument_entry_type *uil_arg);
static XtPointer resource_value_for_uil_arg(sym_argument_entry_type *uil_arg,
                                            AB_ARG_TYPE *p_res_type);
static STRING *
ab_action_handler_list_for_uil_callback(sym_callback_entry_type *uil_cb,
                                        AB_WHEN *p_ab_action_when);
static sym_entry_type *parse_uil(char *uil_file_name);
static Uil_continue_type message_cb(int *message_user_data, int msg_number,
                                    int msg_severity, char *msg_text,
                                    char *src_text, char *ptr_text,
                                    char *loc_text, int message_count[]);
static Uil_continue_type status_cb(char *status_user_data, int percent_complete,
                                   int lines_processed, char *current_file,
                                   int message_count[]);
static void init_sym_stacks(void);
static void cleanup_sym_stacks(void);
static void sym_stack_push(void);
static void sym_stack_pop(void);
static void uil_sym_push(sym_entry_type *sym_entry);
static sym_entry_type *uil_sym_pop(void);

static void init_unresolved_refs(void);
static void resolve_unresolved_refs(void);
static void object_for_widget(void *client_data);

static void free_uil_tree(sym_entry_type *node_entry);
/*
 * Recursively goes through the uil parse tree, freeing each node
 * accessible from the root node in postfix order.
 */
static void free_uil_node(sym_entry_type *node_entry) {
...
extern ABObjPtr abuil_get_uil_file(STRING uil_file_path, ABObjPtr ab_project) {
        ...
        if (uil_parse_tree != NULL) {
                ...
                ab_module =
                    uil_tree_into_ab_project(uil_parse_tree, ab_project);
...
static ABObjPtr root_into_project(sym_entry_type *uil_root,
        ...
        if (ab_project != NULL && obj_is_project(ab_project)) {
                ...
                /*
                obj_set_name(ab_project,
                strdup(ab_ident_from_file_name(root_node->file_name)));
                 */

                ab_module = module_hdr_to_module(
                    (sym_entry_type *)root_node->module_hdr);

                if (ab_module != NULL) {
                        if (sections_into_module(
                                ab_module,
                                (sym_entry_type *)root_node->sections) !=
                            NULL) {
...
static ABObjPtr module_hdr_to_module(sym_entry_type *uil_module_hdr) {
        ...
        if (ab_module != NULL) {
                ...
                obj_set_name(
                    ab_module,
                    strdup(ab_ident_from_file_name(
...
static ABObjPtr sections_into_module(ABObjPtr ab_module,
        ...
        while (!done) {
                if ((section_node = (sym_section_entry_type *)uil_sym_pop()) ==
                    NULL)
                        break;

                switch (section_node->header.b_type) {
                case sym_k_object_section:
                        if ((ret_val = object_into_module(
                                 ab_module, section_node->entries)) == NULL)
                                done = 1;
                        break;
                case 0:
                        if ((ret_val = object_child_into_module(
                                 ab_module, section_node->entries)) == NULL)
                                done = 1;
                        break;
                }
        ...
        for (section_trav = section_node;
             ...
             section_trav = (sym_section_entry_type *)section_trav->next) {
                if (section_trav->header.b_type != 0)
                        continue;
                if ((ret_val = object_child_into_module(
                         ab_module, section_trav->entries)) == NULL)
                        break;
        }
        return (ret_val);
}
...
static ABObjPtr widget_into_module(ABObjPtr ab_module, ABObjPtr ab_parent,
        ...
        if (ab_type != AB_TYPE_UNKNOWN) {
                 ...
                 */
                if (arguments_into_ab_widget(
                        ab_widget, uil_widget->az_arguments) == NULL ||
                    callbacks_into_ab_module(
                        ab_widget, uil_widget->az_callbacks) == NULL) {
        ...
        if (ab_map_entry->object_proc != NULL) {
                ...
                ret_widget = (*ab_map_entry->object_proc)(
                    ab_map_entry->widget_name, ab_parent, ab_widget);
                if (ret_widget == NULL) {
        ...
        for (a_control = (sym_control_entry_type *)controls->obj_header.az_next;
             a_control != NULL;
             a_control =
                 (sym_control_entry_type *)a_control->obj_header.az_next) {
                uil_child = a_control->az_con_obj;

                /*
                 *  If the reference field is set, this is a reference to a
                 * control defined elsewhere.  Otherwise it is an inline
                 * definition.
                 */
                while (uil_child->obj_header.az_reference != NULL)
                        uil_child = (sym_widget_entry_type *)
                                        uil_child->obj_header.az_reference;
        ...
        while ((uil_child = (sym_widget_entry_type *)uil_sym_pop()) != NULL) {
                ...
                if (ab_map_entry->child_proc != NULL) {
                        ...
                        ret_child = (*ab_map_entry->child_proc)(
                            ab_map_entry->widget_name, ab_widget, ab_child);
                        if (ret_child == NULL)
...
arguments_into_ab_widget(ABObjPtr ab_widget,
        ...
        for (widget_args =
                 (sym_list_entry_type *)widget_args->obj_header.az_next;
             widget_args != NULL;
             widget_args =
                 (sym_list_entry_type *)widget_args->obj_header.az_next) {
                ...
                if (!abuilP_store_attr_in_abobj(ab_widget, res_name, res_type,
                                                res_value)) {
...
callbacks_into_ab_module(ABObjPtr ab_widget,
        ...
        for (widget_cbs = (sym_list_entry_type *)widget_cbs->obj_header.az_next;
             widget_cbs != NULL;
             widget_cbs =
                 (sym_list_entry_type *)widget_cbs->obj_header.az_next) {
                /*
                 * REMIND: All this stuff should be pushed into a function
                 */
                ...
                ab_handler_list = ab_action_handler_list_for_uil_callback(
                    uil_cb, &ab_action_when);
                for (ab_handler_trav = ab_handler_list;
                     ab_handler_trav != (STRING *)NULL; ab_handler_trav++) {
...
static XtPointer resource_value_for_uil_arg(sym_argument_entry_type *uil_arg,
        ...
        *p_res_type = AB_ARG_VOID_PTR;
        ...
        if (resource_value->header.b_tag == sym_k_widget_entry) {
                ...
                /*
                 *  The reference field will be set; this is a reference to a
        ...
        case sym_k_integer_value:
                /*
                 * No special action if value is not an integer enumeration
                 * value. Else; value must be from enumeration set
                 */
        ...
        case sym_k_compound_string_value:
                ret_val = _XmStringUngenerate(
                    (XmString)resource_value->value.c_value,
                    XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, XmCHARSET_TEXT);
                if (ret_val != NULL) {
                        *p_res_type = AB_ARG_STRING;
                }
        ...
        case sym_k_fontset_value:
                fprintf(stderr, "font_value or fontset_value for argument: "
                                "UNIMPLEMENTED\n");
        ...
        case sym_k_string_table_value:
                fprintf(stderr,
                        "string_table_value for argument: UNIMPLEMENTED\n");
                goto common_table;

        case sym_k_font_table_value:
                fprintf(stderr,
                        "font_table_value for argument: UNIMPLEMENTED\n");
                goto common_table;

        case sym_k_trans_table_value:
                fprintf(stderr,
                        "trans_table_value for argument: UNIMPLEMENTED\n");
        ...
        case sym_k_color_table_value:
                fprintf(stderr,
                        "color_table_value for argument: UNIMPLEMENTED\n");
                break;
        ...
        case sym_k_keysym_value:
                /* fprintf(stderr, "keysym_value for argument:
                 * UNIMPLEMENTED\n"); */
                ret_val = (XtPointer)resource_value->value.c_value;
                *p_res_type = AB_ARG_STRING;
                break;
...
static sym_entry_type *parse_uil(char *uil_file_name) {
        ...
        compile_stat = Uil(&command_desc, &compile_desc, (UIL_CB)message_cb,
                           (char *)&user_mess_data, (UIL_CB)status_cb,
                           (char *)&user_stat_data);

        fprintf(stderr, "\nUIL Compiler Version %d, ",
                compile_desc.compiler_version);
        fprintf(stderr,
                "Data Structures Version %d: ", compile_desc.data_version);
...
message_cb(int *message_user_data, /* user message number */
...
static Uil_continue_type status_cb(char *status_user_data, int percent_complete,
                                   ...
                                   int message_count[]) {
        /*
           fprintf(stderr, "Status #%d -- Lines: %d, File: %s, Percentage:
           %d%%\n", *status_user_data, lines_processed, current_file,
           percent_complete);
         */
...
static void init_sym_stacks(void) {
        ...
        stack_stack =
            (UilSymStack *)malloc(sizeof(UilSymStack) * stack_stack_size);
...
static void sym_stack_push(void) {
        ...
        new_stack->stack_entries = (sym_entry_type **)malloc(
            sizeof(sym_entry_type *) * new_stack->stack_size);
        ...
        if (stack_stack_ptr == stack_stack_size) {
                ...
                stack_stack = (UilSymStack *)realloc((void *)stack_stack,
                                                     sizeof(UilSymStack) *
                                                         stack_stack_size);
                if (stack_stack == (UilSymStack *)NULL) {
...
static void uil_sym_push(sym_entry_type *sym_entry) {
        ...
        if (cur_stack->stack_ptr == cur_stack->stack_size) {
                ...
                cur_stack->stack_entries = (sym_entry_type **)realloc(
                    (void *)cur_stack->stack_entries,
...
extern void abuil_add_unresolved_ref(void *entry) {
...
static void object_for_widget(void *client_data) {
        ...
        field_widget =
            (sym_widget_entry_type *)obj_get_attach_value(rr->obj, rr->dir);
