...
static int install_action(ABObj obj, ABObj module, ABObj action);
#ifdef BOGUS
static ABObj find_or_create_target(ABObj obj, ABObj module, ISTRING parent_name,
                                   ISTRING name);
#endif /* BOGUS */
...
bilP_load_att_from(BIL_TOKEN valueToken) {
        ...
        if (valueToken == AB_BIL_APPLICATION) {
                ...
                newObj = obj_scoped_find_or_create_undef(
                    bilP_load.module, stringValue, AB_TYPE_UNKNOWN);
...
bilP_load_att_to(BIL_TOKEN valueToken) {
        ...
        if (valueToken == AB_BIL_VALUE_IDENT) {
                 ...
                 * bilP_load_att_name() in bil_loadatt.c.
                 */
                toObj = obj_scoped_find_or_create_undef(
                    bilP_load.module, stringValue, AB_TYPE_UNKNOWN);
        ...
        *target_name = 0;
        ...
        if (*target_parent_name == 0) {
                ...
                if (istr_equalstr(obj->name, (STRING)target_parent_name)) {
                        ...
                        target_parent = obj_scoped_find_or_create_undef(
                            module, (STRING)target_parent_name,
                            AB_TYPE_UNKNOWN);
                }
        }
        ...
        if (istr_equalstr(obj->name, (STRING)target_name)) {
                ...
                target = obj_scoped_find_or_create_undef(
                    target_parent, (STRING)target_name, AB_TYPE_UNKNOWN);
        }
        return target;
}
#endif /* BOGUS */
...
bilP_load_att_arg_value(BIL_TOKEN valueToken) {
        ...
        else if (valueToken == AB_BIL_VALUE_FLOAT) {
                float fval;
