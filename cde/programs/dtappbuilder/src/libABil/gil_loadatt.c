...
#else
#define util_dassert(_debugLevel, _boolExpr) /* ignore this */;
#endif
#endif
#ifndef util_dabort
#ifdef DEBUG
#define util_dabort(_lvl)                                                      \
        {                                                                      \
                if (debug_level() >= (_lvl)) {                                 \
                        abort();                                               \
                }                                                              \
 ...
 */
static int find_or_create_members(ABObj group, ABObj root,
                                  ISTRING_ARRAY *names) {
        ...
        for (i = 0; i < names->count; ++i) {
                ...
                member = obj_scoped_find_or_create_undef(
                    root, istr_string(member_name), AB_TYPE_UNDEF);
 ...
 */
static int gilP_obj_set_initial_state(ABObj obj,
                                      AB_OBJECT_STATE initial_state) {
...
static int set_children_accelerators(ABObj obj, ISTRING_ARRAY *strings) {
        int return_value = 0;
        int rc = 0;
        ...
        for (trav_open(&trav, obj, AB_TRAV_CHILDREN), i = 0;
             (child_obj = trav_next(&trav)) != NULL; ++i) {
                ...
                if ((int)istr_len(strings->strings[i]) > 0) {
                        obj_set_accelerator(child_obj,
                                            istr_string(strings->strings[i]));
                }
        }
        trav_close(&trav);
        return return_value;
...
static int set_children_defaults(ABObj obj, BOOL_ARRAY *defaults) {
        int return_value = 0;
        int rc = 0;
...
static int set_children_fgcolors(ABObj obj, ISTRING_ARRAY *colors) {
...
static int set_children_initially_selected(ABObj obj, BOOL_ARRAY *selects) {
...
static int set_item_labels(ABObj obj, ISTRING_ARRAY *strings) {
        int num_strings = strings->count;
        int i = 0;
        ABObj item = NULL;
        AB_TRAVERSAL trav;
...
static int set_item_label_types(ABObj obj, LABEL_TYPE_ARRAY *label_types) {
        ...
        for (trav_open(&trav, obj, AB_TRAV_ITEMS), i = 0;
             (item = trav_next(&trav)) != NULL; ++i) {
                ...
                if (obj_get_label_type(item) != AB_LABEL_SEPARATOR) {
                        /* if it's already a separator, it has been set
                         * elsewhere */
...
static int set_children_menu_names(ABObj obj, ISTRING_ARRAY *strings) {
        int num_strings = strings->count;
        int i;
        ABObj child_obj;
        AB_TRAVERSAL trav;
 ...
 */
static int load_events(FILE *inFile, AB_WHEN **p) {
...
int gilP_load_handlers(FILE *inFile, ISTRING_ARRAY *handlers) {
        ...
        for (i = 0; (!abio_get_list_end(inFile)) && (i < AB_OBJ_MAX_CHILDREN);
             ++i) {
                if ((rc = gilP_load_handler(inFile, &(handlers->strings[i]))) <
                    0) {
                        return_value = rc;
                        break;
                }
...
int gilP_load_name(FILE *inFile, ISTRING *name) {
        ...
        if ((string_name != NULL) &&
            (!ab_c_ident_is_ok(istr_string(string_name)))) {
...
int gilP_load_file_names(FILE *inFile, ISTRING_ARRAY *names) {
        ...
        for (i = 0; (!abio_get_list_end(inFile)) && (i < AB_OBJ_MAX_CHILDREN);
             ++i) {
                if ((rc = gilP_load_file_name(inFile, &(names->strings[i]))) <
                    0) {
                        return_value = rc;
                        break;
                }
 ...
 * message.
 */
int gilP_load_label_types(FILE *inFile, LABEL_TYPE_ARRAY *label_types) {
        int i;           /* array count */
 ...
 * error message.
 */
int gilP_load_initial_states(FILE *inFile, INITIAL_STATE_ARRAY *states) {
        int i;
        ISTRING keyword;
        ...
        for (i = 0; (!abio_get_list_end(inFile)) && (i < AB_OBJ_MAX_CHILDREN);
             ++i) {
                ...
                states->states[i] =
                    gilP_string_to_object_state(istr_string(keyword));
        }
        states->count = i;
...
int gilP_load_strings(FILE *inFile, ISTRING_ARRAY *strings) {
        ...
        for (i = 0; (!abio_get_list_end(inFile)) && (i < AB_OBJ_MAX_CHILDREN);
             ++i) {
                ...
                strings->strings[i] =
                    (string == NULL ? istr_create("") : string);
        }
        strings->count = i;
...
/*
 * */
static int load_att_(ABObj obj, ABObj root_obj) { return 0; }

static int load_att_anchor_object(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        ISTRING anchor = NULL;
...
static int load_att_anchor_point(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING compass_str;
...
static int load_att_background_color(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING bg_color;
...
/*
 * */
static int load_att_busy_drop_glyph(FILE *inFile, ABObj obj, ABObj root_obj) {
        int rc = 0;
        ISTRING glyph = NULL;
...
static int load_att_button_type(FILE *inFile, ABObj obj, ABObj root_obj) {
        ...
        if ((button_type = gilP_string_to_button_type(
                 istr_string(button_type_string))) == ERROR) {
                abil_print_load_err(ERR_UNKNOWN);
                return -1;
        } else {
 ...
 * for choice labels
 */
static int load_att_choices(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
 ...
 * for choice colors
 */
static int load_att_choice_colors(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
...
/*
 * */
static int load_att_choice_defaults(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        BOOL_ARRAY defaults;
 ...
 */
static int load_att_choice_label_types(FILE *inFile, ABObj obj,
                                       ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
...
static int load_att_col_alignment(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING align_string;

        if (!abio_get_keyword(inFile, &align_string))
                return (abil_print_load_err(ERR_WANT_KEYWORD), -1);
        if ((obj->info.container.col_align = gilP_string_to_alignment(
                 istr_string(align_string))) == ERROR) {
 ...
 */
static int load_att_columns(FILE *inFile, ABObj obj, ABObj root_obj) {
        int num_columns;
 ...
 */
static int load_att_constant_width(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_default_drop_site(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_dnd_accept_cursor(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_dnd_accept_cursor_xhot(FILE *inFile, ABObj obj,
                                           ABObj root_obj) {
        int xhot;
...
static int load_att_dnd_accept_cursor_yhot(FILE *inFile, ABObj obj,
                                           ABObj root_obj) {
        int yhot;
...
static int load_att_dnd_cursor(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING dnd_cursor;
...
static int load_att_dnd_cursor_xhot(FILE *inFile, ABObj obj, ABObj root_obj) {
        int xhot;
        if (!abio_get_integer(inFile, &xhot))
                return (abil_print_load_err(ERR_WANT_INTEGER), -1);
        return 0;
...
static int load_att_dnd_cursor_yhot(FILE *inFile, ABObj obj, ABObj root_obj) {
        int yhot;
        if (!abio_get_integer(inFile, &yhot))
                return (abil_print_load_err(ERR_WANT_INTEGER), -1);
        return 0;
...
static int load_att_draggable(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_drawing_model(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_drop_target_width(FILE *inFile, ABObj obj, ABObj root_obj) {
        int width;
...
static int load_att_droppable(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
...
static int load_att_events(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        AB_WHEN *events = NULL;
...
static int load_att_file_chooser_filter_pattern(FILE *inFile, ABObj obj,
                                                ABObj root_obj) {
...
static int load_att_file_chooser_match_glyph(FILE *inFile, ABObj obj,
                                             ABObj root_obj) {
        ISTRING tmp_str = NULL;
        if (!abio_get_string(inFile, &tmp_str))
                return (abil_print_load_err(ERR_WANT_STRING), -1);
...
static int load_att_file_chooser_match_glyph_mask(FILE *inFile, ABObj obj,
                                                  ABObj root_obj) {
        ISTRING tmp_str = NULL;
        if (!abio_get_string(inFile, &tmp_str))
                return (abil_print_load_err(ERR_WANT_STRING), -1);
...
static int load_att_file_chooser_type(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_foreground_color(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_group_type(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;

        if (!abio_get_keyword(inFile, &tmp_str))
                return (abil_print_load_err(ERR_WANT_KEYWORD), -1);
        if ((obj->info.container.group_type =
                 gilP_string_to_group_type(istr_string(tmp_str))) == ERROR) {
                return (abil_print_load_err(ERR_UNKNOWN), -1);
        }
...
static int load_att_height(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int;
...
static int load_att_help(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_hoffset(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_hscroll(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
...
static int load_att_hspacing(FILE *inFile, ABObj obj, ABObj root_obj) {
 ...
 */
static int load_att_icon_file(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_icon_label(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_icon_mask_file(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_initial_list_glyphs(FILE *inFile, ABObj obj,
                                        ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        ABObj child = NULL;
...
static int load_att_initial_list_values(FILE *inFile, ABObj obj,
                                        ABObj root_obj) {
        return load_att_choices(inFile, obj, root_obj);
}

static int load_att_initial_selections(FILE *inFile, ABObj obj,
                                       ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        BOOL_ARRAY bools;
...
static int load_att_initial_state(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
 ...
 */
static int load_att_interfaces(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
 ...
 */
static int load_att_initial_value(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int;
        ISTRING tmp_str;
...
static int load_att_label(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_label_bold(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
...
static int load_att_label_type(FILE *inFile, ABObj obj, ABObj root_obj) {
        ...
        if (!abio_get_keyword(inFile, &tmp_str))
                return (abil_print_load_err(ERR_WANT_KEYWORD), -1);
        if ((label_type = gilP_string_to_label_type(istr_string(tmp_str))) ==
            ERROR)
                return (abil_print_load_err(ERR_UNKNOWN), -1);
 ...
 */
static int load_att_layout_type(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_mapped(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
...
static int load_att_max_tick_string(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_max_value(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int;
...
static int load_att_max_value_string(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_members(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
 ...
 *
 */
static int load_att_menu_item_accelerators(FILE *inFile, ABObj obj,
                                           ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
...
static int load_att_menu_item_colors(FILE *inFile, ABObj obj, ABObj root_obj) {
        return load_att_choice_colors(inFile, obj, root_obj);
}

/*
 * */
static int load_att_menu_item_defaults(FILE *inFile, ABObj obj,
                                       ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        BOOL_ARRAY bools;
...
/*
 * */
static int load_att_menu_item_labels(FILE *inFile, ABObj obj, ABObj root_obj) {
        return load_att_choices(inFile, obj, root_obj);
}

/*
 * */
static int load_att_menu_item_label_types(FILE *inFile, ABObj obj,
                                          ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
...
/*
 * */
static int load_att_menu_item_states(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
...
/*
 * */
static int load_att_menu_item_menus(FILE *inFile, ABObj obj, ABObj root_obj) {
        int rc = 0; /* r turn code */
        ISTRING_ARRAY names;
 ...
 * to be attached
 */
static int load_att_menu(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
...
static int load_att_menu_title(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_menu_type(FILE *inFile, ABObj obj, ABObj root_obj) {
        ...
        if (!abio_get_keyword(inFile, &tmp_str))
                return (abil_print_load_err(ERR_WANT_KEYWORD), -1);
        if ((tmp_menu_type = gilP_string_to_menu_type(istr_string(tmp_str))) <
            0) {
...
static int load_att_min_tick_string(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_min_value(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int;
...
static int load_att_min_value_string(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
static int load_att_multiple_selections(FILE *inFile, ABObj obj,
                                        ABObj root_obj) {
 ...
 * this one.
 */
static int load_att_name(FILE *inFile, ABObj *pobj, ABObj root_obj) {
#define obj (*pobj)
        int return_value = 0;
        int rc = 0; /* r turn code */
        ISTRING name = NULL;
...
static int load_att_normal_drop_glyph(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
 ...
 */
static int load_att_orientation(FILE *inFile, ABObj obj, ABObj root_obj) {
        AB_ORIENTATION orientation;
        ISTRING tmp_str = NULL;
...
static int load_att_owner(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        ...
        parent_obj = obj_scoped_find_or_create_undef(
            root_obj, istr_string(owner_name), AB_TYPE_UNDEF);
 ...
 * for menus
 */
static int load_att_pinnable(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL pinnable = FALSE;
 ...
 * for pop-up windows
 */
static int load_att_pinned(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
 ...
 */
static int load_att_read_only(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
...
static int load_att_reference_point(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;

        if (!abio_get_keyword(inFile, &tmp_str))
                return abil_print_load_err(ERR_WANT_KEYWORD);

        return 0;
}

static int load_att_resizable(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
...
static int load_att_row_alignment(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;

        if (!abio_get_keyword(inFile, &tmp_str))
                return (abil_print_load_err(ERR_WANT_KEYWORD), -1);
        if ((obj->info.container.row_align =
                 gilP_string_to_alignment(istr_string(tmp_str))) == ERROR)
                return (abil_print_load_err(ERR_UNKNOWN), -1);
 ...
 * for projects
 */
static int load_att_root_window(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
        ...
        if (root_window_name != NULL) {
                root_window = obj_scoped_find_or_create_undef(
                    root_obj, istr_string(root_window_name),
                    AB_TYPE_BASE_WINDOW);
                if (root_window == NULL) {
                        abil_print_load_err(ERR_WANT_NAME);
                        return_value = -1;
 ...
 */
static int load_att_rows(FILE *inFile, ABObj obj, ABObj root_obj) {
        int rows = -1;
 ...
 */
static int load_att_scrollable_height(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int;
 ...
 * this is for stub widget - drawing area scrollbar - skip at the moment
 */
static int load_att_scrollable_width(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int;
...
static int load_att_selection_required(FILE *inFile, ABObj obj,
                                       ABObj root_obj) {
        BOOL tmp_bool = FALSE;
...
static int load_att_setting_type(FILE *inFile, ABObj obj, ABObj root_obj) {
        AB_CHOICE_TYPE choice_type;
        ISTRING tmp_str;

        if (!abio_get_keyword(inFile, &tmp_str))
                return (abil_print_load_err(ERR_WANT_KEYWORD), -1);
        if ((choice_type = gilP_string_to_choice_type(istr_string(tmp_str))) ==
            ERROR)
                return (abil_print_load_err(ERR_UNKNOWN), -1);
 ...
 * says don't
 */
static int load_att_show_border(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
 ...
 */
static int load_att_show_endboxes(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
 ...
 */
static int load_att_show_footer(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_show_range(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL footer_present = FALSE;
...
static int load_att_show_value(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
...
static int load_att_scale_width(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int;
...
static int load_att_stored_length(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int = 0;
 ...
 * for text-field
 */
static int load_att_max_length(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int;
...
static int load_att_text_type(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
 ...
 */
static int load_att_ticks(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_title(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
...
/*
 * */
static int load_att_type(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
 ...
 */
static int load_att_user_data(FILE *inFile, ABObj obj, ABObj root_obj) {
        ISTRING tmp_str = NULL;
 ...
 */
static int load_att_value_length(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int;
...
static int load_att_value_underlined(FILE *inFile, ABObj obj, ABObj root_obj) {
        int return_value = 0;
...
static int load_att_value_x(FILE *inFile, ABObj obj, ABObj root_obj) {
        int valx;
...
static int load_att_value_y(FILE *inFile, ABObj obj, ABObj root_obj) {
        int valy;
...
static int load_att_voffset(FILE *inFile, ABObj obj, ABObj root_obj) {
...
static int load_att_vscroll(FILE *inFile, ABObj obj, ABObj root_obj) {
        BOOL tmp_bool = FALSE;
...
static int load_att_vspacing(FILE *inFile, ABObj obj, ABObj root_obj) {
 ...
 * CURRENT
 */
static int load_att_width(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int = 0;
...
static int load_att_x(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int = 0;
...
static int load_att_y(FILE *inFile, ABObj obj, ABObj root_obj) {
        int tmp_int = 0;
 ...
 */
int gilP_load_attribute_value(FILE *inFile, ABObj obj, AB_GIL_ATTRIBUTE g_attr,
                              ABObj root_obj) {
        int return_value = 0;
        int rc = 0; /* r turn code */
