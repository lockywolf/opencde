...
int main(int argc, char *argv[]) {
        ...
        if (params.import) {
                ...
                newProject =
                    bil_load_file(params.fileName, NULL, NULL, &fileObjList);
                if (newProject != NULL) {
                        for (trav_open(&trav, newProject, AB_TRAV_FILES);
                             (file = trav_next(&trav)) != NULL;) {
                                bil_load_file(obj_get_name(file), (FILE *)NULL,
                                              newProject, &fileObjList);
        ...
        if (params.save) {
                ...
                if (params.save_encapsulated) {
                        ...
                        iRC =
                            bil_save_tree(newProject, obj_get_file(newProject),
                                          BIL_SAVE_FILE_PER_MODULE);
                        if (iRC >= 0) {
                                for (trav_open(&moduleTrav, newProject,
                                               AB_TRAV_MODULES);
                                     (module = trav_next(&moduleTrav)) !=
                                     NULL;) {
...
static int parse_args(int argc, char *argv[], PARAMETERS *params) {
        ...
        if (!(params->import || params->dump_tree || params->save)) {
