 ...
 */
ABObj gil_load_file(STRING gil_file, FILE *inFile, ABObj project) {
        int return_value = 0;
         ...
         */
        interface = obj_scoped_find_or_create_undef(
            project, ab_ident_from_file_name(gil_file), AB_TYPE_MODULE);
        if (interface == NULL) {
        ...
        if (inFile != NULL) {
                ...
                if ((errmsg = abio_open_gil_input(gil_file, &gilInFile)) !=
                    NULL) {
                        util_error(errmsg);
                        return_value = -1;
                        goto epilogue;
 ...
 * THE PROJECT FILE.
 */
int gil_load_project_file(STRING project_file, FILE *inFile,
                          ABObj *projectOutPtr) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        STRING errmsg = NULL;
        ...
        util_dprintf(1, "gil_load_project_file(%s)\n",
        ...
        if (inFile != NULL) {
                ...
                if ((errmsg = abio_open_gil_input(project_file, &gilInFile)) !=
                    NULL) {
                        util_error(errmsg);
                        return_value = -1;
                        goto epilogue;
...
int gil_load_project_file_and_resolve_all(STRING project_file, FILE *inFile,
                                          ABObj *projectOutPtr) {
        int return_value = 0;
        int rc = 0;
        ABObj project = NULL;
        ...
        for (trav_open(&trav, project, AB_TRAV_MODULES | AB_TRAV_MOD_SAFE);
             (interface = trav_next(&trav)) != NULL;) {
                if (!obj_is_defined(interface)) {
                        if (gil_load_file(obj_get_file(interface), NULL,
                                          project) == NULL) {
                                return_value = -1;
                                goto epilogue;
                        }
