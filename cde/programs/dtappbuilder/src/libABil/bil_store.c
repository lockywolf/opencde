...
#ifdef BOGUS
static int store_project_tree(ABObj root, STRING filename,
                              BIL_SAVE_TYPE save_type);
#endif /* BOGUS */

static BIL_TOKEN *get_object_attrs(ABObj obj);
static int store_object(FILE *outFile, ABObj obj, BIL_SAVE_TYPE save_type);
static int store_attribute(FILE *outFile, ABObj obj, BIL_TOKEN attr,
                           BIL_SAVE_TYPE save_type);
...
static int store_action(FILE *outFile, ABObj obj, BIL_SAVE_TYPE save_type);
static STRING create_bil_file_list(ABObj proj);
static int bilP_write_tree(FILE *outFile, ABObj root, BIL_SAVE_TYPE save_type);
#ifdef BOGUS
static int save_cross_module_actions(FILE *outFile, ABObj root,
                                     BIL_SAVE_TYPE save_type);
#endif /* BOGUS */
static BOOL attr_is_undef(BIL_TOKEN attr, ABObj obj);
static BOOL skip_attr(BIL_TOKEN attr, ABObj obj, BIL_SAVE_TYPE save_type);

static int bilP_write_tree_ui_objs(FILE *outFile, ABObj root,
                                   BIL_SAVE_TYPE save_type);
static int bilP_write_tree_conn_objs(FILE *outFile, ABObj root,
                                     BIL_SAVE_TYPE save_type);
...
static BOOL attachment_is_ok(ABObj obj, AB_COMPASS_POINT attach_dir);
static int check_attachments(ABObj obj);
static int bilP_obj_verify(ABObj obj);
#endif /* DEBUG */
        ...
        *buf = 0;
        ...
        case AB_BIL_PACKING:
                if (obj_get_packing(obj) == AB_PACK_UNDEF) {
                        util_error(catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET,
        ...
        case AB_BIL_INITIAL_VALUE: {
                ...
                if (type == AB_TYPE_SCALE || type == AB_TYPE_SPIN_BOX)
                        return (False);
                else if (type == AB_TYPE_TEXT_FIELD ||
                         type == AB_TYPE_TEXT_PANE)
                        return (
                            util_strempty(obj_get_initial_value_string(obj)));
        }
        ...
        case AB_BIL_WEST_ATTACHMENT: {
                ...
                switch (attr) {
                case AB_BIL_NORTH_ATTACHMENT:
                        attach_dir = AB_CP_NORTH;
                        break;
                case AB_BIL_EAST_ATTACHMENT:
                        attach_dir = AB_CP_EAST;
                        break;
                case AB_BIL_SOUTH_ATTACHMENT:
                        attach_dir = AB_CP_SOUTH;
                        break;
                case AB_BIL_WEST_ATTACHMENT:
                        attach_dir = AB_CP_WEST;
                        break;
                }
...
static BOOL skip_attr(BIL_TOKEN attr, ABObj obj, BIL_SAVE_TYPE save_type) {
        ...
        if (obj_is_group(obj)) {
                if ((obj_get_group_type(obj) == AB_GROUP_ROWS) &&
                    (attr == AB_BIL_COLUMN_ALIGNMENT ||
                     attr == AB_BIL_NUM_ROWS || attr == AB_BIL_NUM_COLUMNS ||
                     attr == AB_BIL_VOFFSET || attr == AB_BIL_VATTACH_TYPE))
                        return TRUE;

                if ((obj_get_group_type(obj) == AB_GROUP_COLUMNS) &&
                    (attr == AB_BIL_ROW_ALIGNMENT || attr == AB_BIL_NUM_ROWS ||
                     attr == AB_BIL_NUM_COLUMNS || attr == AB_BIL_HOFFSET ||
        ...
        if (parent && obj_is_group(parent) &&
            ...
            (attr == AB_BIL_X || attr == AB_BIL_Y ||
             attr == AB_BIL_NORTH_ATTACHMENT ||
             attr == AB_BIL_SOUTH_ATTACHMENT ||
             attr == AB_BIL_EAST_ATTACHMENT || attr == AB_BIL_WEST_ATTACHMENT))
                return TRUE;
...
STRING
bil_get_filename(ABObj proj, ABObj module) {
        static char proj_dir[MAXPATHLEN] = "", module_dir[MAXPATHLEN] = "",
        ...
        if (proj != NULL && module != NULL) {
                ...
                if (proj_fullpath != NULL && mod_fullpath != NULL) {
                        util_get_dir_name_from_path(proj_fullpath, proj_dir,
                                                    MAXPATHLEN);
                        util_get_dir_name_from_path(mod_fullpath, module_dir,
                                                    MAXPATHLEN);
                        util_get_file_name_from_path(mod_fullpath, module_file,
                                                     MAXPATHLEN);
        ...
        if (type == AB_TYPE_ITEM) {
                ...
                case AB_ITEM_FOR_CHOICE: {
                        static BIL_TOKEN attrs[] = {
                            AB_BIL_TYPE,        AB_BIL_ITEM_TYPE,
                            AB_BIL_LABEL_TYPE,  AB_BIL_LABEL,
                            AB_BIL_ACTIVE,      AB_BIL_SELECTED,
                            AB_BIL_BG_COLOR,    AB_BIL_FG_COLOR,
                            AB_BIL_HELP_VOLUME, AB_BIL_HELP_LOCATION,
                            AB_BIL_HELP_TEXT,   -1,
                        };
                ...
                case AB_ITEM_FOR_MENUBAR: {
                        static BIL_TOKEN attrs[] = {
                            AB_BIL_TYPE,         AB_BIL_ITEM_TYPE,
                            AB_BIL_LABEL_TYPE,   AB_BIL_LABEL,
                            AB_BIL_IS_HELP_ITEM, AB_BIL_MNEMONIC,
                            AB_BIL_MENU,         AB_BIL_BG_COLOR,
                            AB_BIL_FG_COLOR,     AB_BIL_ACTIVE,
                            AB_BIL_HELP_VOLUME,  AB_BIL_HELP_LOCATION,
                            AB_BIL_HELP_TEXT,    -1,
                        };
                ...
                case AB_TYPE_ACTION: {
                        static BIL_TOKEN attrs[] = {
                            AB_BIL_FROM,          AB_BIL_TO,
                            AB_BIL_WHEN,          AB_BIL_ACTION_TYPE,
                            AB_BIL_ACTION,        AB_BIL_ARG_TYPE,
                            AB_BIL_ARG_VALUE,     AB_BIL_HELP_VOLUME,
                            AB_BIL_HELP_LOCATION, -1,
                        };
                ...
                case AB_TYPE_FILE_CHOOSER: {
                        static BIL_TOKEN attrs[] = {
                            AB_BIL_TYPE,        AB_BIL_X,
                            AB_BIL_Y,           AB_BIL_BG_COLOR,
                            AB_BIL_FG_COLOR,    AB_BIL_VISIBLE,
                            AB_BIL_LABEL,       AB_BIL_FILTER_PATTERN,
                            AB_BIL_OK_LABEL,    AB_BIL_AUTO_DISMISS,
                            AB_BIL_DIRECTORY,   AB_BIL_FILE_TYPE_MASK,
                            AB_BIL_HELP_VOLUME, AB_BIL_HELP_LOCATION,
                            AB_BIL_HELP_TEXT,   -1,
                        };
                        returnAttrs = attrs;
                ...
                case AB_TYPE_MESSAGE: {
                        static BIL_TOKEN attrs[] = {
                            AB_BIL_TYPE,          AB_BIL_MESSAGE_TYPE,
                            AB_BIL_LABEL,         AB_BIL_MESSAGE,
                            AB_BIL_ACTION1_LABEL, AB_BIL_ACTION2_LABEL,
                            AB_BIL_ACTION3_LABEL, AB_BIL_CANCEL_BUTTON,
                            AB_BIL_HELP_BUTTON,   AB_BIL_DEFAULT_BUTTON,
                            AB_BIL_HELP_VOLUME,   AB_BIL_HELP_LOCATION,
                            AB_BIL_HELP_TEXT,     -1,
                        };
                        returnAttrs = attrs;
...
static int store_attribute(FILE *outFile, ABObj obj, BIL_TOKEN attr,
        ...
        case AB_BIL_TYPE:
                abio_puts(outFile,
                          util_object_type_to_string(obj_get_type(obj)));
                break;
        ...
        case AB_BIL_DRAG_ENABLED:
                abio_puts(outFile, abio_bil_boolean_string(
                                       obj_get_drag_initially_enabled(obj)));
                break;
        ...
        case AB_BIL_DRAG_TO_ROOT_ALLOWED:
                abio_puts(outFile, abio_bil_boolean_string(
                                       obj_get_drag_to_root_allowed(obj)));
                break;
        ...
        case AB_BIL_DRAWAREA_WIDTH:
                abio_puts(outFile,
                          abio_integer_string(obj_get_drawarea_width(obj)));
                break;

        case AB_BIL_DRAWAREA_HEIGHT:
                abio_puts(outFile,
                          abio_integer_string(obj_get_drawarea_height(obj)));
                break;

        case AB_BIL_DROP_ENABLED:
                abio_puts(outFile, abio_bil_boolean_string(
                                       obj_get_drop_initially_enabled(obj)));
                break;

        case AB_BIL_DROP_SITE_CHILD_ALLOWED:
                abio_puts(outFile, abio_bil_boolean_string(
                                       obj_drop_on_children_is_allowed(obj)));
                break;
        ...
        case AB_BIL_LABEL_POSITION:
                abio_puts(outFile, bilP_compass_point_to_string(
                                       obj_get_label_position(obj)));
                break;
                /*
                        case AB_BIL_LABEL_STYLE:
                        {
                            ...
                            {
                                abio_puts(outFile,
                   bilP_label_style_to_string(lbl_style));
                            }
                            break;
                        }
                */
        ...
        case AB_BIL_FILES: {
                ...
                if ((bil_files = create_bil_file_list(obj)) != NULL) {
                        ...
                        util_dprintf(1,
                                     "malloc or copy failed for bil file list");
                break;
        }

        case AB_BIL_WIN_CHILDREN: {
                ABObj refObj = NULL, objModule = NULL, refModule = NULL;
        ...
        case AB_BIL_WIN_PARENT: {
                ...
                if (win_parent == NULL) {
                        abio_puts(outFile,
                                  bilP_token_to_string(AB_BIL_UNDEF_KEYWORD));
                        break;
                }
        ...
        case AB_BIL_BORDER_FRAME:
                abio_puts(outFile, bilP_border_frame_to_string(
                                       obj_get_border_frame(obj)));
                break;

        case AB_BIL_PACKING:
                abio_puts(outFile,
                          bilP_packing_to_string(obj_get_packing(obj)));
                break;
        ...
        case AB_BIL_ROW_ALIGNMENT:
                abio_puts(outFile,
                          bilP_alignment_to_string(obj_get_row_align(obj)));
                break;

        case AB_BIL_COLUMN_ALIGNMENT:
                abio_puts(outFile,
                          bilP_alignment_to_string(obj_get_col_align(obj)));
                break;
         ...
         */
                /*
                        case AB_BIL_HEIGHT_RESIZABLE:
                            abio_puts(outFile,
                   abio_bil_boolean_string(obj_get_height_resizable(obj)));
                            break;

                        case AB_BIL_WIDTH_RESIZABLE:
                            abio_puts(outFile,
                   abio_bil_boolean_string(obj_get_width_resizable(obj)));
                            break;
                */
        case AB_BIL_RESIZABLE:
                abio_puts(outFile,
                          abio_bil_boolean_string(obj_get_resizable(obj)));
                break;

        case AB_BIL_CONTAINER_TYPE:
                abio_puts(outFile, bilP_container_type_to_string(
                                       obj_get_container_type(obj)));
                break;
        ...
        case AB_BIL_MENU_TITLE:
                ...
                /*
                        case AB_BIL_MENU_TYPE:
                            abio_puts(outFile,
                   bilP_menu_type_to_string(obj_get_menu_type(obj))); break;
                */

        case AB_BIL_GROUP_TYPE:
                abio_puts(outFile,
                          bilP_group_type_to_string(obj_get_group_type(obj)));
                break;
        ...
        case AB_BIL_LABEL_ALIGNMENT:
                ...
                case AB_TYPE_BUTTON:
                case AB_TYPE_LABEL:
                        abio_puts(outFile, bilP_alignment_to_string(
                                               obj_get_label_alignment(obj)));
                        break;
                }
                break;
        ...
        case AB_BIL_NUM_COLUMNS:
                abio_puts(outFile,
                          abio_integer_string(obj_get_num_columns(obj)));
                break;

        case AB_BIL_HATTACH_TYPE:
                abio_puts(outFile,
                          bilP_attachment_to_string(obj_get_hattach_type(obj)));
                break;

        case AB_BIL_VATTACH_TYPE:
                abio_puts(outFile,
                          bilP_attachment_to_string(obj_get_vattach_type(obj)));
                break;
        ...
        case AB_BIL_REFERENCE_POINT:
                abio_puts(outFile,
                          bilP_compass_point_to_string(obj_get_ref_point(obj)));
                break;
        ...
        case AB_BIL_BUTTON_TYPE:
                abio_puts(outFile,
                          bilP_button_type_to_string(obj_get_button_type(obj)));
                break;

        case AB_BIL_ORIENTATION:
                abio_puts(outFile,
                          bilP_orientation_to_string(obj_get_orientation(obj)));
                break;

        case AB_BIL_DIRECTION:
                abio_puts(outFile,
                          bilP_direction_to_string(obj_get_direction(obj)));
                break;

        case AB_BIL_SELECTION_REQUIRED:
                abio_puts(outFile, abio_bil_boolean_string(
                                       obj_get_selection_required(obj)));
                break;
        ...
        case AB_BIL_IS_HELP_ITEM:
                abio_puts(outFile,
                          abio_bil_boolean_string(obj_is_help_item(obj)));
                break;

        case AB_BIL_ITEM_TYPE:
                abio_puts(outFile,
                          bilP_item_type_to_string(obj_get_item_type(obj)));
                break;
        ...
        case AB_BIL_TEAR_OFF:
                abio_puts(outFile,
                          abio_bil_boolean_string(obj_get_tearoff(obj)));
                break;

        case AB_BIL_EXCLUSIVE:
                abio_puts(outFile,
                          abio_bil_boolean_string(obj_get_exclusive(obj)));
                break;

        case AB_BIL_DECIMAL_POINTS:
                abio_puts(outFile,
                          abio_integer_string(obj_get_decimal_points(obj)));
                break;
        ...
        case AB_BIL_TEXT_TYPE:
                abio_puts(outFile,
                          bilP_text_type_to_string(obj_get_text_type(obj)));
                break;

        case AB_BIL_MAX_LENGTH:
                abio_puts(outFile,
                          abio_integer_string(obj_get_max_length(obj)));
                break;
        ...
        case AB_BIL_READ_ONLY:
                abio_puts(outFile,
                          abio_bil_boolean_string(obj_get_read_only(obj)));
                break;
        ...
        case AB_BIL_ACTION_TYPE:
                abio_puts(outFile,
                          bilP_func_type_to_string(obj_get_func_type(obj)));
                break;
        ...
        case AB_BIL_FROM: {
                ...
                if (obj_is_project(obj->info.action.from)) {
                        abio_puts(outFile,
                                  bilP_token_to_string(AB_BIL_APPLICATION));
                } else {
                        type = obj_get_func_type(obj);
                        switch (type) {
                        case AB_FUNC_BUILTIN: {
        ...
        case AB_BIL_ARG_TYPE:
                abio_puts(outFile,
                          bilP_arg_type_to_string(obj_get_arg_type(obj)));
                break;
                ...
                if ((type = obj_get_arg_type(obj)) != 0) {
                        ...
                        case AB_ARG_STRING:
                                if (obj_get_arg_string(obj)) {
                                        abio_put_string(
                                            outFile, obj_get_arg_string(obj));
                                }
                                break;
                        }
                }
        ...
        case AB_BIL_CHOICE_TYPE:
                abio_puts(outFile,
                          bilP_choice_type_to_string(obj_get_choice_type(obj)));
                break;

        case AB_BIL_SELECTED:
                abio_puts(outFile, abio_bil_boolean_string(
                                       obj_is_initially_selected(obj)));
                break;

        case AB_BIL_SELECTION_MODE:
                abio_puts(outFile, bilP_selection_mode_to_string(
                                       obj_get_selection_mode(obj)));
                break;

        case AB_BIL_DEFAULT:
                abio_puts(outFile,
                          abio_bil_boolean_string(obj_is_default(obj)));
                break;
        ...
        case AB_BIL_NORTH_ATTACHMENT: {
                ...
                if (attach_type != AB_ATTACH_UNDEF)
                        abio_puts(outFile,
                                  bilP_attachment_to_string(attach_type));
                abio_putc(outFile, ' ');
                switch (attach_type) {
                case AB_ATTACH_OBJ:
                case AB_ATTACH_ALIGN_OBJ_EDGE:
                        abio_puts(outFile, obj_get_name(obj_get_root(
                                               (ABObj)obj_get_attach_value(
                                                   obj, AB_CP_NORTH))));
                        break;
                case AB_ATTACH_GRIDLINE:
                case AB_ATTACH_CENTER_GRIDLINE:
                        abio_put_integer(outFile, (long)obj_get_attach_value(
                                                      obj, AB_CP_NORTH));
                        break;
                ...
                if (obj->attachments != NULL)
                        abio_put_integer(
                            outFile, obj_get_attach_offset(obj, AB_CP_NORTH));
                else
                        abio_put_integer(outFile, 0);
                abio_putc(outFile, ')');
                break;
        ...
        case AB_BIL_SOUTH_ATTACHMENT: {
                ...
                if (obj->attachments != NULL)
                        abio_put_integer(
                            outFile, obj_get_attach_offset(obj, AB_CP_SOUTH));
                else
                        abio_put_integer(outFile, 0);
                abio_putc(outFile, ')');
                break;
        ...
        case AB_BIL_EAST_ATTACHMENT: {
                ...
                case AB_ATTACH_ALIGN_OBJ_EDGE:
                        abio_puts(
                            outFile,
                            obj_get_name(obj_get_root(
                                (ABObj)obj_get_attach_value(obj, AB_CP_EAST))));
                        break;
                case AB_ATTACH_GRIDLINE:
                case AB_ATTACH_CENTER_GRIDLINE:
                        abio_put_integer(outFile, (long)obj_get_attach_value(
                ...
                if (obj->attachments != NULL)
                        abio_put_integer(
                            outFile, obj_get_attach_offset(obj, AB_CP_EAST));
                else
                        abio_put_integer(outFile, 0);
                abio_putc(outFile, ')');
                break;
        ...
        case AB_BIL_WEST_ATTACHMENT: {
                ...
                case AB_ATTACH_ALIGN_OBJ_EDGE:
                        abio_puts(
                            outFile,
                            obj_get_name(obj_get_root(
                                (ABObj)obj_get_attach_value(obj, AB_CP_WEST))));
                        break;
                case AB_ATTACH_GRIDLINE:
                case AB_ATTACH_CENTER_GRIDLINE:
                        abio_put_integer(outFile, (long)obj_get_attach_value(
                ...
                if (obj->attachments != NULL)
                        abio_put_integer(
                            outFile, obj_get_attach_offset(obj, AB_CP_WEST));
                else
                        abio_put_integer(outFile, 0);
                abio_putc(outFile, ')');
                break;
        ...
        case AB_BIL_HELP_LOCATION:
                if (!obj_is_action(obj))
                        ...
                        abio_put_string(outFile,
                                        obj_get_func_help_location(obj));
                break;
        ...
        case AB_BIL_WORD_WRAP:
                abio_puts(outFile,
                          abio_bil_boolean_string(obj_get_word_wrap(obj)));
                break;
        ...
        case AB_BIL_SHOW_VALUE:
                abio_puts(outFile,
                          abio_bil_boolean_string(obj_get_show_value(obj)));
                break;
        ...
        case AB_BIL_AUTO_DISMISS:
                abio_puts(outFile,
                          abio_bil_boolean_string(obj_get_auto_dismiss(obj)));
                break;
        ...
        case AB_BIL_FILE_TYPE_MASK:
                abio_puts(outFile, bilP_file_type_mask_to_string(
                                       obj_get_file_type_mask(obj)));
                break;

        case AB_BIL_MESSAGE_TYPE:
                abio_puts(outFile,
                          bilP_msg_type_to_string(obj_get_msg_type(obj)));
                break;
        ...
        case AB_BIL_SESSION_MGMT: {
                ...
                store_attribute(outFile, obj, AB_BIL_SESSIONING_METHOD,
                                save_type);
        ...
        case AB_BIL_SESSIONING_METHOD:
                abio_puts(outFile, bilP_sessioning_method_to_string(
                                       obj_get_sessioning_method(obj)));
                break;
        ...
        case AB_BIL_TOOLTALK: {
                ...
                store_attribute(outFile, obj, AB_BIL_TOOLTALK_DESKTOP_LEVEL,
                                save_type);
        ...
        case AB_BIL_TOOLTALK_DESKTOP_LEVEL:
                abio_puts(outFile, bilP_tt_desktop_level_to_string(
                                       obj_get_tooltalk_level(obj)));
                break;
        ...
        case AB_BIL_DEFAULT_BUTTON:
                if (obj_is_popup_win(obj)) {
                        ...
                        if (default_b == NULL) {
                                abio_puts(outFile, bilP_token_to_string(
                                                       AB_BIL_UNDEF_KEYWORD));
                                break;
                        } else
                                abio_puts(outFile, obj_get_name(default_b));
                } else
                        abio_puts(outFile, bilP_default_button_to_string(
                                               obj_get_default_btn(obj)));
                break;
...
static int store_res_file_types(FILE *outFile, ABObj obj) {
        AB_ARG_CLASS_FLAGS resFileArgClasses =
            obj_get_res_file_arg_classes(obj);
...
static int store_dnd_ops(FILE *outFile, ABDndOpFlags opFlags) {
        int flagCount = 0;
        int numWritten = 0;
...
static int store_dnd_types(FILE *outFile, ABDndTypeFlags typeFlags) {
        int flagCount = 0;
        int numWritten = 0;
        ...
        for (flagCount = 0; flagCount < 4; ++flagCount) {
                switch (flagCount) {
                case 0:
                        oneFlag = ABDndTypeText;
                        break;
                case 1:
                        oneFlag = ABDndTypeFilename;
                        break;
                case 2:
                        oneFlag = ABDndTypeUserDef;
                        break;
                case 3:
                        oneFlag = ABDndTypeAny;
                        break;
                }
...
static int bilP_write_tree_ui_objs(FILE *outFile, ABObj root,
        ...
        if (!obj_is_sub(root) && obj_is_defined(root) &&
            (obj_is_ui(root) || obj_is_module(root) || obj_is_project(root))) {
             ...
             (action_obj = trav_next(&trav)) != NULL;) {
                ...
                case AB_FUNC_BUILTIN:
                        ...
                        if (!util_xor(obj_is_cross_module(action_obj),
                                      saveCrossModule)) {
                                /*is_cross_module and saveCrossModule are
                                 * logically same*/
                ...
                for (trav_open(&act_trav, mod_obj, AB_TRAV_ACTIONS);
                     (action_obj = trav_next(&act_trav)) != NULL;) {
                        ...
                        if (type == AB_FUNC_BUILTIN) {
                                ...
                                if (obj_get_module(
                                        action_obj->info.action.to) != mod_obj)
                                        store_action(outFile, action_obj,
                                                     save_type);
                        }
                }
        }
...
static STRING create_bil_file_list(ABObj proj) {
        ...
        *bil_list = 0;
        for (trav_open_cond(&trav, proj, AB_TRAV_MODULES,
...
int bil_save_tree(ABObj root, STRING file_name, STRING old_proj_dir,
                  BIL_SAVE_TYPE save_type) {
        int return_value = OK;
        STRING errmsg = NULL;
...
#endif /* BOGUS */
        {
                ...
                if (old_proj_dir != NULL) {
                         ...
                         */
                        if ((old_file != NULL) &&
                            !util_path_is_absolute(old_file)) {
                                old_file_len = strlen(old_file);
                                old_file = (STRING)NULL;
                                old_file = (STRING)util_malloc(
                                    old_file_len + strlen(old_proj_dir) + 2);
                                sprintf(old_file, "%s/%s", old_proj_dir,
                                        obj_get_file(root));
                        }
                ...
                return_value =
                    abio_open_bil_output(file_name, old_file, &bilOutFile);
                if (return_value != OK) {
                        goto epilogue;
                }
                 ...
                 */
                if ((save_type != BIL_SAVE_FILE_PER_MODULE) &&
                    (!obj_is_module(root))) {
                        ABObj module = NULL;
                        AB_TRAVERSAL trav;
...
static int store_project_tree(ABObj root, STRING filename,
        ...
        if ((root != NULL) && (filename != NULL)) {
                ...
                util_printf_err(catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET, 45,
                                        "Cannot save project; Either nothing "
                                        "to store or NULL filename."));
        }
...
static int bad_obj_msg(ABObj obj, STRING fieldName, int return_value,
        ...
        case AB_TYPE_CONTAINER:
                if ((obj_get_container_type(obj) == AB_CONT_GROUP) ||
                    ...
                    (obj_get_container_type(obj) == AB_CONT_RELATIVE)) {
                        ...
                        if (obj_get_container_type(obj) == AB_CONT_GROUP) {
                                if ((obj_get_row_align(obj) ==
                                     AB_ALIGN_UNDEF) ||
                                    (obj_get_col_align(obj) ==
                                     AB_ALIGN_UNDEF)) {
...
static int util_dabort_impl(int debug_abort_level) {
