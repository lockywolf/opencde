...
static int get_action(FILE *inFile, ABObj obj, ABObj module, ABObj action);
static int get_action_attribute(FILE *inFile, ABObj obj, ABObj module,
                                ABObj action, AB_GIL_ATTRIBUTE attr);
static int get_when(FILE *inFile, ABObj obj, ABObj action);
static int get_from(FILE *inFile, ABObj obj, ABObj module, ABObj action);
static int get_to(FILE *inFile, ABObj obj, ABObj module, ABObj action);
...
static int add_user_handler(ABObj obj, ABObj module, ISTRING handler,
                            AB_WHEN when);
static int install_action(ABObj obj, ABObj module, ABObj action);
static ABObj find_or_create_target(ABObj obj, ABObj module,
                                   ISTRING interface_name, ISTRING parent_name,
                                   ISTRING obj_name, ISTRING item_label);
...
/*
 * */
int gilP_load_att_done_handler(FILE *inFile, ABObj obj, ABObj module) {
        int return_value = 0;
...
int gilP_load_att_event_handler(FILE *inFile, ABObj obj, ABObj module) {
        int return_value = 0;
        int rc = 0; /* r turn code */
...
/*
 * */
int gilP_load_att_menu_handler(FILE *inFile, ABObj obj, ABObj module) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        ISTRING handler = NULL;
...
/*
 * */
int gilP_load_att_menu_item_handlers(FILE *inFile, ABObj obj, ABObj module) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        ...
        for (trav_open(&trav, obj, AB_TRAV_ITEMS_FOR_OBJ), i = 0;
             (item = trav_next(&trav)) != NULL; ++i) {
                add_user_handler(item, module, handlers.strings[i],
                                 AB_WHEN_ACTIVATED);
...
/*
 * */
int gilP_load_att_notify_handler(FILE *inFile, ABObj obj, ABObj module) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        ISTRING handler = NULL;
...
/*
 * */
int gilP_load_att_repaint_proc(FILE *inFile, ABObj obj, ABObj module) {
        int return_value = 0;
        int rc = 0; /* r turn code */
 ...
 */
int gilP_load_att_actions(FILE *inFile, ABObj obj, ABObj module) {
        int return_value = 0;
        int rc = 0; /* r turn code */
        ABObj action = NULL;
        ...
        while (!abio_get_list_end(inFile)) {
                ...
                if ((rc = get_action(inFile, obj, module, action)) < 0) {
                        ...
                        if (obj_get_when(action) == AB_WHEN_UNDEF) {
                                /* unsupported :when value - ignore it and
                                 * continue */
                                obj_destroy(action);
                        } else {
...
static int get_action(FILE *inFile, ABObj obj, ABObj module, ABObj action) {
        ...
        while (!abio_get_list_end(inFile)) {
                ...
                if ((rc = get_action_attribute(inFile, obj, module, action,
                                               action_attr)) < 0) {
                        return_value = rc;
                        break;
                }
...
static int get_from(FILE *inFile, ABObj obj, ABObj module, ABObj action) {
        ...
        if ((abio_gil_get_full_name(inFile, &interface_name, &parent_name,
                                    &name, &item_name) < 0) ||
            (name == NULL)) {
        ...
        from = find_or_create_target(obj, module, interface_name, parent_name,
                                     name, item_name);
...
static int get_to(FILE *inFile, ABObj obj, ABObj module, ABObj action) {
        ...
        to = find_or_create_target(obj, module, interface_name, parent_name,
                                   name, item_name);
...
static int get_operation(FILE *inFile, ABObj action) {
        ...
        case AB_FUNC_BUILTIN: /* predefined function */
                ...
                obj_set_func_builtin(
                    action, gilP_string_to_builtin_action(istr_string(name)));
...
static int get_when(FILE *inFile, ABObj obj, ABObj action) {
        ...
        while (!abio_get_list_end(inFile)) {
                ...
                if (when == AB_WHEN_UNDEF) {
                        if (util_be_verbose()) {
                                util_printf(
                                    "WARNING: ignoring unsupported when: %s\n",
                                    istr_string(string));
                        }
                } else {
                        ...
                        ++numWhens;
                        if (numWhens > 1) {
                                util_printf_err("WARNING: multiple :when "
...
static int get_arg_type(FILE *inFile, ABObj action) {
        ...
        while (!abio_get_list_end(inFile)) {
                ...
                obj_set_arg_type(
                    action, gilP_string_to_arg_type(istr_string(type_name)));
        }
...
static int add_user_handler(ABObj obj, ABObj module, ISTRING handler,
        ...
        /*
         * With "callbacks," the to field is irrelevant.  The target is
         * generally whatever widget calls the callback, so we set the "to"
         * field to NULL, so that this action may be used by multiple widgets.
         * I.e., if the function type, name and to fields match, an action will
         * be shared.
         */
        action = obj_create(AB_TYPE_ACTION, NULL);
        obj_set_func_type(action, AB_FUNC_USER_DEF);
        ...
        *target_item_label = 0;
        ...
        if (obj_is_project(module)) {
                ...
                if (interface_name != NULL) {
                        util_strncpy(target_interface_file,
                                     istr_string(interface_name),
                                     GIL_MAX_NAME_SIZE);
                        if (parent_name != NULL) {
                                util_strncpy(target_parent_name,
                                             istr_string(parent_name),
                                             GIL_MAX_NAME_SIZE);
                        }
                ...
                strcpy(target_interface_name, target_interface_file);
                {
                        ...
                        if ((len >= 2) &&
                            (util_streq(&(target_interface_name[len - 2]),
                                        ".G") ||
                             util_streq(&(target_interface_name[len - 2]),
                                        ".P"))) {
         ...
         */
        /*util_dprintf(3, "finding: module:'%s' parent:'%s' obj:'%s'
        ...
        if (util_strempty(target_interface_name)) {
                ...
                for (trav_open(&trav, target_project, AB_TRAV_MODULES);
                     (target_module = trav_next(&trav)) != NULL;) {
                        if ((target_module != target_project) &&
                            (util_streq(obj_get_file(target_module),
                ...
                if (target_module == NULL) {
                        target_module =
                            obj_create(AB_TYPE_MODULE, target_project);
        ...
        if (util_strempty(target_parent_name)) {
                ...
                for (trav_open(&trav, target_module,
                     ...
                     (target_parent = trav_next(&trav)) != NULL;) {
                        if ((target_parent != target_module) &&
                            (util_streq(obj_get_name(target_parent),
                                        target_parent_name))) {
                                break;
                        }
                }
                ...
                if (target_parent == NULL) {
                        target_parent =
                            obj_create(AB_TYPE_UNDEF, target_module);
