 ...
 *
 * @(#)abuil_print.c	1.30 19 Oct 1994
...
static int indent(FILE *fp, int spaces);
static int print(ABObjPtr obj);
static int print_indented(FILE *fp, ABObjPtr obj);
static int print_tree(FILE *fp, ABObjPtr obj);
static int print_tree_indented(FILE *fp, ABObjPtr obj);
static int verify_tree(ABObjPtr root);

static int output_ui_arg(FILE *fp, ABObj obj, Arg *arg);

static int handle_dtterm(FILE *fp, ABObj obj);

static int handle_combo_box(FILE *fp, ABObj obj);

static int handle_spin_box(FILE *fp, ABObj obj);

static int handle_menu_button(FILE *fp, ABObj obj);
 ...
 */
extern void abuil_obj_print_uil(FILE *fp, ABObjPtr ab_proj) {
        /*
...
static STRING fix_prefix_suffix(STRING widget_class_name) {
        static char s[1024];
        STRING m;
        STRING p;
        int n;
...
static STRING suffixed(STRING widget_instance_name) {
...
static int print_indented(FILE *fp, ABObjPtr obj) {
        ...
        */
        if (obj_is_message(obj) || (obj_is_menu(obj) && !obj_is_ref(obj)) ||
            obj_is_combo_box_item(obj) || obj_is_list_item(obj) ||
        ...
        ** special menu-reference objects as children of each object that uses
        *the
        ** sharable menu.  (The menu-reference objects each point to the real
        *menu
        ...
        */
        if (obj_is_menu_ref(obj)) {
                ...
                obj = obj_get_actual_obj(
                    menu_ref); /* substitute the actual menu */
        }
        ...
        /*
        ** Special case handling for Option Menus.  The actual OptionMenu
        *subobject
        ** in the composite option menu ABObj must have its XmNsubMenuId
        *resource
        ...
        */
        ...
        if (obj_is_option_menu(root_obj)) {
                ...
                if (obj_obj == obj) {
                        child_obj =
                            objxm_comp_get_subobj(root_obj, AB_CFG_PARENT_OBJ);
        ...
        if ((child_obj = trav_next(&trav)) != NULL) {
                ...
                do {
                        ...
                        if (obj_is_term_pane(child_obj)) {
                                child_class = "XmText";
                        } else if (util_streq(obj_get_class_name(child_obj),
                                              _dtSpinBox)) {
                                ...
                                child_class = fix_prefix_suffix(
                                    obj_get_class_name(child_obj));
...
static int output_ui_arg(FILE *fp, ABObj obj, Arg *arg) {
        ...
        case AB_ARG_WIDGET: /* istring (object name, not widget) */
                ...
                if ((attachObjName = istr_string((ISTRING)arg->value)) ==
                    NULL) {
                        fprintf(fp, "!Couldn't get attach obj name for %s\n",
                                arg->name);
                        break;
                }
                if ((attachObj = obj_scoped_find_by_name(
                         module, attachObjName)) == NULL) {
        ...
        case AB_ARG_VOID_PTR:
                ...
                if (!util_streq(arg->name, XmNentryClass))
                        break;
...
static int handle_dtterm(FILE *fp, ABObj obj) {
        ...
        **	    XmNverticalScrollBar, so we shouldn't output it.   Also, we
        *need to *	    make sure XmText is multi-line instead of the
        *default single line.
        ...
        */
        ...
        for (arg = (Arg *)argList; (arg != NULL) && (arg->name != NULL);
             arg++) {
                ...
                if (util_streq(arg->name, DtNsubprocessCmd))
                        continue;

                /* Skip XmNverticalScrollBar resource */
                if (util_streq(arg->name, XmNverticalScrollBar))
                        continue;
...
static int handle_combo_box(FILE *fp, ABObj obj) {
        ...
        child_name =
            obj_get_unique_name(obj, child_base_name, 64, child_name_buf);
        ...
        for (arg = (Arg *)argList; (arg != NULL) && (arg->name != NULL);
             arg++) {
                if (util_streq(arg->name, DtNcomboBoxType))
                        continue;
        ...
        */
        ...
        if ((child_obj = trav_next(&trav)) != NULL) {
                do {
                        fprintf(fp, "\n! DtComboBox list item transformed into "
                                    "menu item\n");
                        fprintf(fp, "object\n");
                        ...
                        fprintf(fp, "arguments {\n");
                        indent(fp, DBLSP);
                        fprintf(fp, "XmNlabelString = \"%s\" ;\n",
                                obj_get_label(child_obj));
                        indent(fp, DBLSP);
                        fprintf(fp, "XmNlabelType = XmSTRING ;\n");
                        indent(fp, DBLSP);
                        fprintf(fp, "};\n");
...
static int handle_spin_box(FILE *fp, ABObj obj) {
        ...
        for (arg = (Arg *)argList; (arg != NULL) && (arg->name != NULL);
             arg++) {
                if (util_streq(arg->name, DtNspinBoxChildType))
                        continue;
                if (util_streq(arg->name, DtNminimumValue))
                        continue;
                if (util_streq(arg->name, DtNmaximumValue))
                        continue;
                if (util_streq(arg->name, DtNposition))
                        continue;
                if (util_streq(arg->name, DtNincrementValue))
                        continue;
                if (util_streq(arg->name, DtNarrowLayout))
                        continue;
                if (util_streq(arg->name, XmNdecimalPoints))
                        continue;
