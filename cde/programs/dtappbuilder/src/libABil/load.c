...
abil_loadmsg(STRING userMessage) {
        ...
        *tmpMsg = 0;
        ...
        if (*File != 0) {
                sprintf(tmpMsg,
                        catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET, 27,
                                "file: %s"),
                        File);
                strcat(msg, tmpMsg);
        }
        if (lineNumber > 0) {
                sprintf(tmpMsg,
                        catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET, 28,
                                ", line: %d"),
                        lineNumber);
                strcat(msg, tmpMsg);
        }
        if (*Current_Object != 0) {
                sprintf(tmpMsg,
                        catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET, 29,
                                ", object: %s"),
                        Current_Object);
                strcat(msg, tmpMsg);
        }
...
static int move_string(STRING to, STRING from) {
        if (from == NULL) {
                *to = 0;
        } else {
                strcpy(to, from);
...
int abil_loadmsg_set_file(STRING filename) {
        return move_string(File, filename);
}

BOOL abil_loadmsg_err_printed(void) {
        int cur_err_line = bilP_load_get_line_number();

        if ((abilP_err_line_number == cur_err_line) || (cur_err_line == -1))
                return TRUE;
        else
                return FALSE;
}

int abil_loadmsg_set_err_printed(BOOL printed) {
...
*/
int abil_print_load_err(int errmsg) {
        ...
        */
        ...
        if (!abil_loadmsg_err_printed()) {
                if (Errormsg[errmsg].msg_id < 0) {
                        strcat(msg, catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET,
                                            33, "syntax error"));
                } else {
                        strcat(msg, catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET,
...
static int abil_load_errmsg_table() {
        ...
        blet(ERR_NOT_IMPL_IGNOR, 7,
             "currently not implemented - ignoring object and its attributes");
        blet(ERR_NO_MEMORY, 8, "out of memory");
        blet(ERR_UNKNOWN, 9, "unknown value");
        blet(ERR_UNKNOWN_ATTR, 10, "unknown attribute");
        blet(ERR_UNKNOWN_MENU_TYPE, 11,
             "unknown menu type - defaulting to Command Menu");
