...
STRING
bilP_load_att_children_end(void) { return NULL; }
...
STRING
bilP_load_att_east_attachment_end(void) { return NULL; }
...
STRING
bilP_load_att_west_attachment_end(void) { return NULL; }
...
STRING
bilP_load_att_north_attachment_end(void) { return NULL; }
...
STRING
bilP_load_att_south_attachment_end(void) { return NULL; }
...
bilP_load_att_class(BIL_TOKEN valueToken) {
        ...
        case AB_BIL_PROJECT:
                if (bilP_load.project == NULL) {
                        /*
                         * MUST SET BOTH "project" and "module" TO BE THE
                         * PROJECT This is so that objects that belong to the
                         * project (e.g., cross-module connections) can be
                         * handled like any other object.
                         */
...
bilP_load_att_container_type(BIL_TOKEN valueToken) {
        ...
        if (containerType == AB_CONT_ABSOLUTE) {
                util_dprintf(1, "%s",
                             abil_loadmsg("AB_CONT_ABSOLUTE is obsolete!"));
...
bilP_load_att_menu(BIL_TOKEN valueToken) {
        ...
        if (valueToken == AB_BIL_VALUE_IDENT) {
                newObj = obj_scoped_find_or_create_undef(
                    bilP_load.module, stringValue, AB_TYPE_MENU);
...
STRING
bilP_load_att_res_file_types_end(void) { return NULL; }
...
bilP_load_att_multiple_selections(BIL_TOKEN valueToken) {
        ...
        */
        util_dprintf(
            1, "%s",
            abil_loadmsg("multiple-selections attribute is obsolete!"));
...
#define load_project (bilP_load.project)
        ...
        if (load_type == AB_TYPE_MODULE) {
                ...
                if (load_project != NULL)
                        forward_obj =
                            obj_find_module_by_name(load_project, string_name);
        } else {
        ...
        if (forward_obj != NULL) {
                if (!obj_is_defined(forward_obj)) {
                         ...
                         * the name, so we can just add the additional
                         * attributes into the forward reference.
                         */
                        AB_OBJECT_TYPE obj_type = AB_TYPE_UNKNOWN;
                        ABObj doomed_obj = NULL;

                        /*
                         * Set the "current" object to be the "forward"
                         * reference
                         */
                        load_obj = forward_obj;

                        /*
                         * Mark the current obj as being defined (we're at the
                         * beginning of its definition, now).
                         */
                        ...
                        if (obj_is_module(load_obj)) {
                                if (load_project != NULL)
                                        obj_ensure_unique_name(
                                            load_obj, load_project, -1);
                                load_module = load_obj;
                                objlist_add_obj(load_compObjsRead, load_module,
                                                NULL);
                                if (load_project != NULL) {
                                        obj_append_child(load_project,
                                                         load_module);
                                }
                        } else {
                                obj_ensure_unique_name(load_obj, load_module,
                                                       -1);
                        }
                        ...
                        sprintf(msg,
                                catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET, 36,
                                        "Duplicate name.  Changed to "
                                        "%s.\n\t(This may be a conflict with "
                                        "the interface or project name)\n"),
                                obj_name);
        ...
        } else /* forward_obj == NULL */
        {
                ...
                case AB_TYPE_PROJECT:
                        if (load_project == NULL) {
                                /*
                                 * MUST SET BOTH "project" and "module" TO BE
                                 * THE PROJECT This is so that objects that
                                 * belong to the project (e.g., cross-module
                                 * connections) can be handled like any other
                                 * object.
                                 */
...
bilP_load_att_initial_value(BIL_TOKEN valueToken) {
        ...
        if (valueToken == AB_BIL_VALUE_INT) {
                ...
                obj_set_initial_value_string(bilP_load.obj,
                                             istr_string(istr_value));
        }
...
bilP_load_att_type(BIL_TOKEN valueToken) {
        ...
        if ((obj_get_type(obj) != AB_TYPE_UNDEF) &&
            ...
            (obj_type != AB_TYPE_UNDEF) && (obj_type != AB_TYPE_UNKNOWN) &&
            (obj_get_type(obj) != obj_type)) {
                ...
                *msg = 0;
                ...
                conflict with the interface or project name)\n"), obj_name);
               */
...
#define loadObj (bilP_load.obj)
        ...
        case AB_BIL_DRAG_OPS:
                ...
                obj_set_drag_ops(loadObj,
                                 obj_get_drag_ops(loadObj) | dndOpFlag);
                break;
        ...
        case AB_BIL_DRAG_TYPES:
                ...
                if (obj_get_drag_types(loadObj) == ABDndTypeNone)
                        ...
                        util_dprintf(
                            1, "%s",
                            abil_loadmsg(
                                catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET, 48,
                                        "only 1 drag-type allowed - discarding "
                                        "additional drag-type")));
                break;

        case AB_BIL_DROP_OPS:
                dndOpFlag = bilP_token_to_dnd_op_flag(token);
                obj_set_drop_ops(loadObj,
                                 obj_get_drop_ops(loadObj) | dndOpFlag);
                break;
        ...
        case AB_BIL_WIN_CHILDREN:
                newObj = obj_scoped_find_or_create_undef(
                    bilP_load.module, stringValue, AB_TYPE_DIALOG);
...
int bilP_load_set_current_att(BIL_TOKEN att) {
 ...
 * return NULL; }
 *
 * STRING bilP_load_att_north_value(BIL_TOKEN valueToken) { ABObj	target=
 * NULL; STRING	name= NULL; int		value;
 *
 ...
 * return NULL; }
 *
 * STRING bilP_load_att_south_value(BIL_TOKEN valueToken) { ABObj	target=
 * NULL; STRING	name= NULL; int		value;
 *
 ...
 * return NULL; }
 *
 * STRING bilP_load_att_east_value(BIL_TOKEN valueToken) { ABObj	target=
 * NULL; STRING	name= NULL; int		value;
 *
 ...
 * return NULL; }
 *
 * STRING bilP_load_att_west_value(BIL_TOKEN valueToken) { ABObj	target=
 * NULL; STRING	name= NULL; int		value;
 *
...
bilP_load_attachment_type(BIL_TOKEN valueToken) {
        ...
        util_dprintf(3, "bilP_load_attachment_type: %d/%s\n\t%d/%s\n",
                     valueToken, str_safe(bilP_token_to_string(valueToken)),
                     bilP_load.att,
        ...
        if (valueToken != AB_BIL_NIL) {
                ...
                case AB_BIL_EAST_ATTACHMENT:
                        obj_set_attach_type(bilP_load.obj, AB_CP_EAST,
                                            obj_type);
                        break;
                case AB_BIL_WEST_ATTACHMENT:
                        obj_set_attach_type(bilP_load.obj, AB_CP_WEST,
                                            obj_type);
                        break;
                case AB_BIL_NORTH_ATTACHMENT:
                        obj_set_attach_type(bilP_load.obj, AB_CP_NORTH,
                                            obj_type);
                        break;
                case AB_BIL_SOUTH_ATTACHMENT:
                        obj_set_attach_type(bilP_load.obj, AB_CP_SOUTH,
                                            obj_type);
                        break;
                }
        }
 ...
 */
STRING
bilP_load_att_sb_always_visible(BIL_TOKEN valueToken) { return NULL; }
 ...
 */
STRING
bilP_load_att_win_parent(BIL_TOKEN valueToken) { return NULL; }
...
bilP_load_att_help_button(BIL_TOKEN valueToken) {
        ...
        if (obj_is_message(bilP_load.obj)) {
                ...
                if (bilP_load_get_value_type() == AB_BIL_VALUE_IDENT) {
                        ...
                        if (strValue != NULL) {
                                buttonObj = obj_scoped_find_or_create_undef(
                                    bilP_load.module, strValue,
                                    AB_TYPE_UNKNOWN);

                                obj_set_help_act_button(bilP_load.obj,
                                                        buttonObj);
                        }
                } else {
                        abil_print_load_err(ERR_WANT_STRING);
