 ...
 *
 */
ABObj bil_load_file(char *bil_file, FILE *inFile, ABObj project,
...
#define compObjsOut (*compObjsOutPtr)
        ...
        if (compObjsOutPtr != NULL) {
                ...
                for (i = 0; i < numObjs; ++i) {
                        objlist_add_obj(
                            compObjsOut,
                            objlist_get_obj(bilP_load.compObjsRead, i, NULL),
                            NULL);
                }
        }
...
epilogue:
        ...
        if (LoadErr) {
                ...
                if ((bilP_load.module != NULL) &&
                    (bilP_load.module != bilP_load.project)) {
 ...
 * project
 */
int bil_load_file_and_resolve_all(STRING path, FILE *inFile,
                                  ABObj *newProjectPtr) {
...
int bilP_load_end_of_file() {
        bilP_reset_token_text();
        return 0;
}

int bilP_load_end_of_data() {
