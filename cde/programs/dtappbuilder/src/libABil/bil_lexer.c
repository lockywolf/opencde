...
int AByylex(void) { return get_token(AByyin); }
...
static int get_keyword(FILE *file, int lastChar) {
        ...
        if (keywordToken == AB_BIL_UNDEF) {
                /*
                 * This will not be executed if the actual :undef keyword is
                 * seen in the BIL file.  :undef is AB_BIL_UNDEF_KEYWORD, and is
                 * a valid keyword.
                 *
...
static int get_number(FILE *file, int lastChar) {
        ...
        while (((c = fgetc(file)) != EOF) && (isalnum(c) || (c == '.'))) {
                if (c == '.') {
 ...
 */
void AByyerror(const char *message) {
        char tokenMsg[1024] = "";
        char errMsg[1024] = "";

        if (strlen(tokenText) > 0) {
                sprintf(tokenMsg,
                        catgets(ABIL_MESSAGE_CATD, ABIL_MESSAGE_SET, 37,
                                ", near '%s'"),
                        tokenText);
        }
        sprintf(errMsg, "%s%s\n", message, tokenMsg);
...
int bilP_load_get_token() { return last_token; }

int bilP_load_get_value_type(void) { return last_value_type; }

STRING
bilP_load_get_value(void) { return tokenText; }

int bilP_load_get_length() { return tokenTextLen; }

int bilP_load_get_line_number() { return line_number; }

int bilP_load_set_line_number(int lineNumber) {
        line_number = lineNumber;
        return 0;
}

void allprint(char c) { printf("'%c'", c); }

void sprint(char *s) { printf("\"%s\"", s); }

void bilP_reset_token_text(void) { sprintf(tokenText, ""); }
