...
static void trav_uil_node(sym_entry_type *node_entry);
static void print_uil_symbol(sym_entry_type *az_symbol_entry);
static void print_uil_widget(sym_widget_entry_type *az_widget_entry);
static void print_uil_argument(sym_argument_entry_type *az_argument_entry);
static void print_uil_control(sym_control_entry_type *az_control_entry);
static void print_uil_callback(sym_callback_entry_type *az_callback_entry);
static void print_uil_list(sym_list_entry_type *az_list_entry);
static void print_uil_name(sym_name_entry_type *az_name_entry);
static void print_uil_module(sym_module_entry_type *az_module_entry);
static void
print_uil_color_item(sym_color_item_entry_type *az_color_item_entry);
static void
print_uil_parent_list_item(sym_parent_list_type *az_parent_list_item);
static void
print_uil_external_def(sym_external_def_entry_type *az_external_def_entry);
static void print_uil_proc_def(sym_proc_def_entry_type *az_proc_def_entry);
static void print_uil_proc_ref(sym_proc_ref_entry_type *az_proc_ref_entry);
static void
print_uil_forward_ref(sym_forward_ref_entry_type *az_forward_ref_entry);
static void print_uil_value(sym_value_entry_type *az_value_entry);
static void output_text(int length, char *text);
static void print_uil_source_info(sym_entry_header_type *hdr);
static void print_uil_obj_header(sym_obj_entry_type *az_obj_entry);
static void
print_uil_include_file(sym_include_file_entry_type *az_symbol_entry);
static void print_uil_section(sym_section_entry_type *az_symbol_entry);
static void print_uil_object_variant(sym_def_obj_entry_type *az_symbol_entry);
static void print_uil_root_entry(sym_root_entry_type *az_symbol_entry);
static char *get_sym_section_text(int b_type);
static char *get_tag_text(int b_tag);
static char *get_object_text(int b_type);
static char *get_value_text(int b_type);
static char *get_charset_text(int b_type);
...
static void trav_uil_node(sym_entry_type *node_entry) {
        ...
        case sym_k_list_entry:
                /*
                 * Sublists (nested lists) are not processed recursively to pick
                 * up definitions, since all named lists are picked up in their
                 * list sections. We assume no list of interest to us can
                 * possibly be encountered only in a nested list reference.
                 */
...
static void print_uil_widget(sym_widget_entry_type *az_widget_entry) {
        ...
        if (az_widget_entry->obj_header.az_name != NULL)
                wid_name =
                    (char *)&(az_widget_entry->obj_header.az_name->c_text);
        ...
        if (az_widget_entry->obj_header.b_flags & sym_m_obj_is_gadget)
                printf("GADGET ");
        printf("%s %s(0x%lx) name: %s\n",
               get_object_text(az_widget_entry->header.b_type),
               get_tag_text(az_widget_entry->header.b_tag), az_widget_entry,
               wid_name);

        printf("  controls: 0x%lx  callbacks: 0x%lx  arguments: 0x%lx  "
               "parent_list: 0x%lx\n",
...
static void print_uil_argument(sym_argument_entry_type *az_argument_entry) {
        ...
        if (resource_name->obj_header.b_flags & sym_m_builtin) {
                ...
                key_entry =
                    (key_keytable_entry_type *)resource_name->value.l_integer;
...
static void print_uil_control(sym_control_entry_type *az_control_entry) {
        ...
        while (control_obj->obj_header.az_reference != NULL)
                control_obj = (sym_widget_entry_type *)
                                  control_obj->obj_header.az_reference;
...
static void print_uil_callback(sym_callback_entry_type *az_callback_entry) {
        ...
        if (reason_name->obj_header.b_flags & sym_m_builtin) {
                ...
                key_entry =
                    (key_keytable_entry_type *)reason_name->value.l_integer;
                name_str = key_entry->at_name;
        } else
                name_str = (char *)reason_name->value.c_value;

        printf("  CALLBACK name: %s  proc ref: 0x%lx  proc ref list: 0x%lx\n",
               name_str, az_callback_entry->az_call_proc_ref,
...
static void print_uil_list(sym_list_entry_type *az_list_entry) {
        ...
        printf("%s LIST(0x%lx)  count: %d  gadget count: %d\n",
               get_tag_text(az_list_entry->header.b_type), az_list_entry,
...
static void print_uil_name(sym_name_entry_type *az_name_entry) {
        printf("NAME(0x%lx)  size: %d  next name: 0x%lx  object: 0x%lx",
               ...
               az_name_entry->az_next_name_entry, az_name_entry->az_object);
...
static void print_uil_module(sym_module_entry_type *az_module_entry) {
        printf("%s MODULE(0x%lx)  version: 0x%lx \n",
               az_module_entry->obj_header.az_name->c_text, az_module_entry,
...
static void
print_uil_color_item(sym_color_item_entry_type *az_color_item_entry) {
        printf("COLOR ITEM(0x%lx)  size: %d  letter: %c  index: %d  color: "
               "0x%lx  next: 0x%lx\n",
...
print_uil_parent_list_item(sym_parent_list_type *az_parent_list_item) {
        printf("PARENT LIST(0x%lx)  parent: 0x%lx  next: 0x%lx \n",
               az_parent_list_item, az_parent_list_item->parent,
...
print_uil_external_def(sym_external_def_entry_type *az_external_def_entry) {
        printf("EXTERNAL DEF(0x%lx)  next external: 0x%lx  object: 0x%lx \n",
               az_external_def_entry, az_external_def_entry->az_next_object,
...
static void print_uil_proc_def(sym_proc_def_entry_type *az_proc_def_entry) {
        ...
        printf("PROC DEF(0x%lx)  name: 0x%lx %s%s%s%s  count: %d  %s\n",
               az_proc_def_entry, az_proc_def_entry->obj_header.az_name,
               checking_flag, private_flag, exported_flag, imported_flag,
...
static void print_uil_proc_ref(sym_proc_ref_entry_type *az_proc_ref_entry) {
        ...
        printf("PROC REF(0x%lx)  proc def: 0x%lx  value: 0x%lx\n",
               az_proc_ref_entry, az_proc_ref_entry->az_proc_def,
...
static void
print_uil_forward_ref(sym_forward_ref_entry_type *az_forward_ref_entry) {
        printf("FORWARD REF(0x%lx)  next ref: 0x%lx  location: 0x%lx  %s  "
               "parent: 0x%lx\n",
               az_forward_ref_entry, az_forward_ref_entry->az_next_ref,
...
static void print_uil_value(sym_value_entry_type *az_value_entry) {
        ...
        printf("  VALUE(0x%lx)  size: %d  name: 0x%lx  %s%s%s%s",
               az_value_entry, az_value_entry->header.w_node_size,
               az_value_entry->obj_header.az_name, builtin_flag, private_flag,
               exported_flag, imported_flag);
        ...
        case sym_k_icon_value:
                printf("  icon  width: %d  height: %d  colors: 0x%lx  rows: "
                       "0x%lx \n",
 ...
 */
static void print_uil_obj_header(sym_obj_entry_type *az_obj_entry) {
        printf("%s(0x%lx)", get_tag_text(az_obj_entry->header.b_tag),
               az_obj_entry);
...
static void print_uil_section(sym_section_entry_type *az_symbol_entry) {
        printf("%s SECTION(0x%lx)  entries: 0x%lx\n",
               ...
               az_symbol_entry, az_symbol_entry->entries);
}

static void print_uil_object_variant(sym_def_obj_entry_type *az_symbol_entry) {
        printf("DEFAULT OBJ VAR(0x%lx)  next: 0x%lx  object info: %d, "
               "variant_info: %d\n",
               az_symbol_entry, az_symbol_entry->next,
               az_symbol_entry->b_object_info, az_symbol_entry->b_variant_info);
}
