...
static ABObj find_first_child_of_type(ABObj obj, ABTraversal trav,
                                      AB_OBJECT_TYPE type);
...
int travP_open(ABTraversal trav, ABObj root, unsigned travType,
 ...
 */
ABObj travP_goto(ABTraversal trav, int node_num) {
        int i;
...
ABObj travP_next(ABTraversal trav) {
        ...
        while (!found_next) {
                ...
                if ((trav->curObj == NULL) || (travP_qualifies(trav))) {
                        found_next = TRUE;
                }
        } /* while !found_next */
...
#define curObj (trav->curObj)
        ...
        case AB_TRAV_MODULES:
                curObj = obj_is_module(rootObj)
                             ? rootObj
                             : find_first_child_of_type(rootObj, trav,
                                                        AB_TYPE_MODULE);
                break;
        ...
        case AB_TRAV_SIBLINGS: {
                ...
                curObj = rootObj;
                while ((prevSibling = travP_obj_prev_sibling(curObj, trav)) !=
                       NULL) {
                        curObj = prevSibling;
                }
        } break;
...
#define curObj (trav->curObj)
        ...
        if (travP_is_parents_first(trav)) {
                ...
                if (curObj != NULL) {
                        while ((parent = travP_obj_parent(curObj, trav)) !=
                               NULL) {
                                curObj = parent;
                        }
                }
...
static int travP_find_next_parents_first(ABTraversal trav) {
        ...
        if (curObj == trav->rootObj) {
                ...
                if ((tmpObj = travP_obj_next_sibling(curObj, trav)) != NULL) {
                        ...
                        if (tmpObj == NULL) {
                                tmpObj = find_first_ancestor_sibling_child(
                                    curObj, trav);
                        }
                        curObj = tmpObj;
                }
 ...
 * it, if necessary.
 */
static int travP_verify_type(ABTraversal trav, BOOL warn) {
        int iRet = 0;
 ...
 */
static ABObj find_first_child_of_type(ABObj parent, ABTraversal trav,
                                      AB_OBJECT_TYPE type) {
        ABObj child = NULL;
        ...
        for (child = travP_obj_first_child(parent, trav); child != NULL;
             child = travP_obj_next_sibling(child, trav)) {
                if (child->type == type) {
                        break;
                }
        }
...
static ABObj find_first_action(ABObj obj, ABTraversal trav) {
