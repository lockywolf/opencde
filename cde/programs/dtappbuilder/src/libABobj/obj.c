...
static int objP_or_tree_update_clients_with_data(ABObj obj, BOOL update_subtree,
                                                 int update_code,
                                                 void *update_data,
 ...
 */
ABObj obj_get_menubar_help_item(ABObj obj) {
        AB_TRAVERSAL trav;
 ...
 */
int obj_get_child_num(ABObj obj) {
        int child_num = -1;
        ABObj child;
        ...
        for (child = obj->parent->first_child; child != NULL;
...
ABObj obj_get_container_child(ABObj obj) {
        ABObj child;
 ...
 */
int obj_get_item_num(ABObj obj) {
        int item_num = -1;
        ABObj item;
        for (item = obj->parent->first_child; item != NULL;
...
ABObj obj_get_pane_child(ABObj obj) {
        ABObj child;
        for (child = obj->first_child; child != NULL;
             child = child->next_sibling) {
...
int obj_move_children(ABObj to, ABObj from) {
         ...
         */
        for (child = from->first_child; child != NULL;
             child = child->next_sibling) {
 ...
 */
int obj_ensure_unique_name(ABObj obj, ABObj root, int maxlen) {
        BOOL unique = FALSE;
        BOOL error = FALSE;
        ...
        while ((!unique) && (!error)) {
                ...
                for (trav_open(&trav, root, trav_type);
                     (other_obj = trav_next(&trav)) != NULL;) {
                        if ((other_obj != obj) && (other_obj->name != NULL) &&
                            (strncmp(istr_string(other_obj->name), obj_name,
                                     maxlen) == 0)) {
                                unique = FALSE;
                                name_changed = TRUE;
...
int obj_set_unique_name(ABObj obj, STRING strName) {
        int return_value = 0;
...
STRING
obj_alloc_unique_name(ABObj obj, STRING name, int maxNameLen) {
        ABObj parent = obj->parent;
        if ((parent == NULL) || (obj_find_by_name(parent, name) == obj)) {
                return strdup(name);
        }
...
ISTRING
obj_alloc_unique_name_istr(ABObj obj, ISTRING name, int maxNameLen) {
        ABObj parent = obj->parent;
        if ((parent == NULL) ||
...
STRING
obj_get_unique_name(ABObj obj, STRING name, int maxNameLen, STRING nameOutBuf) {
        ABObj parent = obj->parent;
        if ((parent == NULL) || (obj_find_by_name(parent, name) == obj)) {
                return name;
        }
        return obj_get_unique_name_for_child(parent, name, maxNameLen - 1,
                                             nameOutBuf);
}
 ...
 */
STRING
obj_alloc_unique_name_for_child(ABObj obj, STRING name, int maxlen) {
        char nameBuf[8192];
...
ISTRING
obj_alloc_unique_name_istr_for_child(ABObj obj, ISTRING name, int maxlen) {
        char nameBuf[8192];
...
#endif
        unique_name = istr_create(obj_get_unique_name_for_child(
            obj, istr_string(name), maxlen - 1, nameBuf));

        return unique_name;
}

STRING
obj_get_unique_name_for_child(ABObj obj, STRING name, int maxlen,
                              STRING nameOutBuf) {
 ...
 */
int obj_set_name_from_label(ABObj obj, STRING parent_name_in) {
        ...
        parent_name =
            (parent_name_in == NULL
                 ? (obj->parent == NULL ? NULL : istr_string(obj->parent->name))
                 : parent_name);
        if (parent_name == NULL) {
                return -1;
        }
        ...
        obj_set_unique_name(
            obj, ab_ident_from_name_and_label(parent_name, item_label));

        return 0;
}

int obj_set_name_from_parent(ABObj obj, STRING suffix) {
        STRING parent_name;
        STRING new_name = NULL;
        ...
        obj_set_unique_name(obj,
                            ab_ident_from_name_and_label(parent_name, suffix));
...
int obj_update_clients(ABObj obj) {
        return objP_notify_send_update(obj, FALSE);
}

int obj_tree_update_clients(ABObj tree)

{
        return objP_notify_send_update(tree, TRUE);
}

int obj_update_clients_with_data(ABObj obj, int update_code, void *update_data,
                                 UpdateDataFreeFunc free_func) {
        return objP_or_tree_update_clients_with_data(obj, FALSE, update_code,
                                                     update_data, free_func);
}

int obj_tree_update_clients_with_data(ABObj obj, int update_code,
                                      void *update_data,
                                      UpdateDataFreeFunc free_func) {
        return objP_or_tree_update_clients_with_data(obj, TRUE, update_code,
                                                     update_data, free_func);
}
