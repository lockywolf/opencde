...
static ABArgList MergeArgLists(ABArgList args1, int num_args1, ABArgList args2,
                               int num_args2);
 ...
 *
 */
int obj_set_arg(ABObj obj, AB_ARG_TYPE type, STRING name, void *value) {
        ABArgList merged_list = NULL;
        int nargs = 0;
        ABArg oldarg;
        AB_ARG newarg;

        if ((type == AB_ARG_STRING) || (type == AB_ARG_XMSTRING) ||
            (type == AB_ARG_LITERAL)) {
         ...
         */
        if ((nargs = obj_get_num_args(obj)) == 0) { /* first time ever */
 ...
 */
int obj_set_arg_if_new(ABObj obj, AB_ARG_TYPE type, STRING name, void *value) {
...
void obj_remove_all_args(ABObj obj) { util_free(obj->args); }

void obj_remove_all_typed_args(ABObj obj) { util_free(obj->typed_args); }
 ...
 *
 */
int obj_set_typed_arg(ABObj obj, AB_ARG_TYPE type, STRING name,
                      STRING value_type, int value_size, void *value) {
        AB_TYPED_ARG newarg;
        ABTypedArg merged_list = NULL;
        int nargs = 0;

        ABSetTypedArg(&newarg, type, istr_create(name), istr_create(value_type),
                      value_size, value);
        ...
        if ((nargs = obj_get_num_args(obj)) == 0) { /* first time ever */
                ...
                merged_list =
                    MergeTypedArgLists(&newarg, 1, obj->typed_args, nargs);
 ...
 *  returns 0 if the arg is set, negative if not.
 */
int obj_set_typed_arg_if_new(ABObj obj, AB_ARG_TYPE type, STRING name,
                             STRING value_type, int value_size, void *value) {
        ...
        return obj_set_typed_arg(obj, type, name, value_type, value_size,
                                 value);
}
 ...
 */
ABTypedArg obj_get_typed_arg(ABObj obj, char *name) {
 ...
 * returns an index to it. If unable to find it returns ERROR(-1).
 */
ABArg obj_get_arg(ABObj obj, char *name) {
        register int num_args;
        register int i;
...
int obj_remove_typed_arg(ABObj obj, STRING targ_name) {
 ...
 */
int obj_remove_arg(ABObj obj, STRING name) {
        register int i;
 ...
 *
 */
int obj_merge_arglist(ABObj obj, ABArg arg) {
        ABArgList result;
        int nargs = 0;
 ...
 *
 */
int abo_merge_typed_args(ABObj obj, ABTypedArg arg) {
        ABTypedArg result;
        int nargs = 0;

        if ((nargs = obj_get_num_typed_args(obj)) == 0) { /* first time ever */
                result = MergeTypedArgLists(arg, 1, (ABTypedArgList)NULL, 0);
                obj->typed_args = result;
        } else { /* merge with existing args */
                result = MergeTypedArgLists(arg, 1, obj->typed_args, nargs);
                (void)free((STRING)obj->typed_args);
                obj->typed_args = result;
        }
        return 0;
 ...
 */
int obj_get_num_args(ABObj obj) {
        register ABArgList args = obj->args;
        register int n = 0;

        for (; (args != (ABArgList)NULL) && (args->name != NULL); args++, n++)
                ;
 ...
 */
int obj_get_num_typed_args(ABObj obj) {
