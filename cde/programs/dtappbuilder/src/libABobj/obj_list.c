...
static int objlistP_build_user_data_array(ABObjList list, int index,
                                          void *data);
...
#define objlistP_get_user_data(_list, _index)                                  \
        (((_list)->user_datas) == NULL ? NULL : ((_list)->user_datas[(_index)]))
...
int objlist_construct(ABObjList list) {
        list->num_objs = 0;
        list->objs_size = 0;
        list->objs = NULL;
...
int objlist_destruct(ABObjList list) { return objlist_make_empty(list); }

OBJLIST_SORT_ORDER
objlist_get_sort_order(ABObjList list) { return list->sort_order; }

int objlist_set_sort_order(ABObjList list, OBJLIST_SORT_ORDER new_order) {
        int return_value = 0;
...
int objlist_make_empty(ABObjList list) {
        int return_value = 0;
...
int objlist_is_empty(ABObjList list) { return (list->num_objs < 1); }

int objlist_set_is_unique(ABObjList list, BOOL unique) {
        list->unique = unique;
        return 0;
}

BOOL objlist_get_is_unique(ABObjList list) { return list->unique; }
...
void *objlist_get_obj_data(ABObjList list, ABObj obj) {
 ...
 */
int objlist_add_obj(ABObjList list, ABObj obj, void *clientData) {
        int return_value = 0;
        int rc = 0;
...
int objlist_remove_obj(ABObjList list, ABObj obj) {
        int index = objlist_get_obj_index(list, obj);
        if (index < 0) {
                return 0;
        }
...
int objlist_remove_index(ABObjList list, int doomedIndex) {
...
int objlist_get_num_objs(ABObjList list) {
        if (list == NULL) {
                return 0;
        }
...
ABObj objlist_get_obj(ABObjList list, int whichObj, void **clientDataOut) {
        if ((list == NULL) || (whichObj >= list->num_objs) || (whichObj < 0)) {
                return NULL;
        }
 ...
 */
BOOL objlist_max_size_is_soft(ABObjList list) {
        list = list; /* avoid cc warning */
        return TRUE;
}

int objlist_set_max_size_soft(ABObjList list, int maxSize) {
        int return_value = 0;
...
int objlist_get_max_size(ABObjList list) { return list->objs_size; }
 ...
 */
int objlist_shrink_mem_to_fit(ABObjList list) {
        int return_value = 0;
...
int objlist_iterate(ABObjList list, ABObjListIterFn fn) {
        int i;
...
ABObjList objlist_dup(ABObjList list) {
        int i;
        ...
        for (i = 0; i < list->num_objs; i++) {
                objlist_add_obj(new_list, list->objs[i],
                                objlistP_get_user_data(list, i));
        }
        return (new_list);
}
...
objlistP_shrink_array(ABObjList list, int sizeDiff) {
        ...
        if (new_objs_size > 0) {
                new_objs =
                    (ABObj *)realloc(list->objs, new_objs_size * sizeof(ABObj));
...
static int objlistP_grow_array(ABObjList list, int sizeDiff) {
        ...
        new_objs = (ABObj *)realloc(list->objs, new_objs_size * sizeof(ABObj));
        if (user_datas_valid) {
                new_user_datas = (void **)realloc(
                    list->user_datas, new_objs_size * sizeof(void *));
        }
        ...
        *objName = 0;
        ...
        for (i = 0; i < list->objs_size; ++i) {
                ...
                util_dprintf(0, "%d[%s]: %s", i,
                             (i >= list->num_objs ? " " : "X"), objName);

                if (list->user_datas != NULL) {
                        util_dprintf(0, " 0x%08lx",
                                     objlistP_get_user_data(list, i));
                }
                util_dprintf(0, "\n");
        }
