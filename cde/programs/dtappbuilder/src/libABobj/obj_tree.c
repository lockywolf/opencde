...
static int add_to_ref_list(ABObjList *refListInOutPtr, ABObj refObj,
                           AB_OBJ_REF_TYPE refType);

static int comp_change_flag(ABObj root, unsigned flags, BOOL clear);

static int update_names_lists(ABObj obj, StringList oldNamesList,
                              StringList newNamesList);

/*
 * Test to see if a reparent will be allowed
 */
int obj_test_reparent(ABObj obj, ABObj new_parent) {
        int iReturn = 0;
        verify_obj_read(obj);
 ...
 */
int obj_insert_child(ABObj obj, ABObj new_child, int new_child_num) {
        int iReturn = 0;
        int iRC; /* return code */
        int child_num = 0;
...
int obj_append_child(ABObj obj, ABObj new_child_obj) {
        ...
        for (child_obj = obj->first_child;
             ...
             child_obj = child_obj->next_sibling) { /* null */
        }
...
ABObj obj_get_parent(ABObj obj) {
        ABObj parent = NULL;
        verify_obj_read(obj);
...
ABObj obj_get_child(ABObj obj, int child_num) {
        ABObj child_obj = NULL;
        AB_TRAVERSAL trav;
        verify_obj_read(obj);
 ...
 */
int obj_get_num_salient_children(ABObj obj) {
        verify_obj_read(obj);
        return trav_count(obj, AB_TRAV_SALIENT_CHILDREN);
}

int obj_get_num_salient_ui_children(ABObj obj) {
        verify_obj_read(obj);
 ...
 */
ABObj obj_get_salient_child(ABObj obj, int child_num) {
        AB_TRAVERSAL trav;
        ABObj child = NULL;
        verify_obj_read(obj);
 ...
 */
ABObj obj_get_salient_ui_child(ABObj obj, int child_num) {
        AB_TRAVERSAL trav;
        ABObj child = NULL;
        verify_obj_read(obj);
 ...
 *
 */
ABObj obj_get_child_cond(ABObj obj, int child_num, BOOL (*fn)(ABObj)) {
        int i;
        ABObj child_obj;
        verify_obj_read(obj);
 ...
 * destroyed
 */
int obj_tree_destroy_flagged_impl(ABObj *rootPtr) {
#define root (*rootPtr)
        AB_TRAVERSAL trav;
        ABObj obj = NULL;
...
int obj_tree_set_flag(ABObj root, unsigned flags) {
        AB_TRAVERSAL trav;
        ABObj obj = NULL;
        for (trav_open(&trav, root, AB_TRAV_ALL);
             (obj = trav_next(&trav)) != NULL;) {
...
int obj_tree_clear_flag(ABObj root, unsigned flags) {
        AB_TRAVERSAL trav;
        ABObj obj = NULL;
        for (trav_open(&trav, root, AB_TRAV_ALL);
             (obj = trav_next(&trav)) != NULL;) {
 ...
 */
int obj_comp_set_flag(ABObj root, unsigned flags) {
 ...
 */
int obj_comp_clear_flag(ABObj root, unsigned flags) {
 ...
 */
int obj_prepend_sibling(ABObj obj, ABObj newsib) {
        ABObj parent = obj->parent;

        return ((parent == NULL)
                    ? -1
                    : obj_insert_child(parent, newsib, obj_get_child_num(obj)));
}

/*
 * Inserts newsib after obj in obj->parent's child list
 */
int obj_append_sibling(ABObj obj, ABObj newsib) {
        ABObj parent = obj->parent;

        return (
            (parent == NULL)
                ? -1
                : obj_insert_child(parent, newsib, obj_get_child_num(obj) + 1));
}
        ...
        for (i = 0; i < num_objs; ++i) {
                ...
                if (obj_is_popup_win(refObj)) {
                        if (obj_get_win_parent(refObj) == obj) {
                                add_to_ref_list(&refList, refObj,
                                                AB_REF_WIN_PARENT);
                        }
                ...
                if (refObj->attachments != NULL) {
                        if ((refObj->attachments->north.type ==
                             AB_ATTACH_OBJ) &&
                            (refObj->attachments->north.value == (void *)obj)) {
                                add_to_ref_list(&refList, refObj,
                                                AB_REF_ATTACH_NORTH);
                        }
                        if ((refObj->attachments->south.type ==
                             AB_ATTACH_OBJ) &&
...
static int update_names_lists(ABObj obj, StringList oldNamesList,
        ...
        if (obj->name != NULL) {
                ...
                if (!istr_equal(newName, obj->name)) {
                        ...
                        /* The name has not changed, so set_name() would ignore
                         * us */
