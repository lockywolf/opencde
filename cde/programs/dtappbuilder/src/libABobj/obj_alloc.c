...
static int objP_dup_or_ref_child_trees(ABObj dupRoot, ABObj orgRoot,
                                       BOOL createRef);

static int objP_remove_all_refs_to(ABObj obj);

ABObj obj_create(AB_OBJECT_TYPE obj_type, ABObj parent) {
...
static ABObj objP_create_dup_or_ref(ABObj obj, BOOL createRef) {
        ...
        *newObj = *obj;
        ...
        if (obj_is_action(newObj)) {
                ...
                newObj->info.module.stubs_file =
                    istr_dup(obj->info.module.stubs_file);
                newObj->info.module.ui_file =
                    istr_dup(obj->info.module.ui_file);
        } else if (obj_is_item(newObj)) {
                newObj->info.item.accelerator =
                    istr_dup(newObj->info.item.accelerator);
                newObj->info.item.mnemonic =
                    istr_dup(newObj->info.item.mnemonic);
        } else if (obj_is_project(newObj)) {
                newObj->info.project.file = istr_dup(obj->info.project.file);
                newObj->info.project.stubs_file =
                    istr_dup(obj->info.project.stubs_file);
                newObj->info.project.root_window =
                    obj->info.project.root_window;
                ...
                newObj->info.window.icon_label =
                    istr_dup(obj->info.window.icon_label);
        }
...
static int objP_dup_or_ref_child_trees(ABObj dupRoot, ABObj orgRoot,
        ...
        for (orgChild = orgRoot->first_child; orgChild != NULL;
             orgChild = orgChild->next_sibling) {
        ...
        for (orgChild = orgRoot->first_child, dupChild = dupRoot->first_child;
             ...
             orgChild = orgChild->next_sibling,
            dupChild = dupChild->next_sibling) {
...
ABObj obj_dup(ABObj obj) { return objP_create_dup_or_ref(obj, FALSE); }

ABObj obj_tree_dup(ABObj tree) {
...
ABObj obj_create_ref(ABObj obj) { return objP_create_dup_or_ref(obj, TRUE); }

ABObj obj_tree_create_ref(ABObj tree) {
        return objP_tree_create_dup_or_ref(tree, TRUE);
}

int obj_create_ref_subtrees(ABObj to_root, ABObj from_root) {
...
#define obj (*objPtr)
        ...
        if (debugging()) {
                if (obj_verify(obj) < 0) {
                        util_dprintf(0,
                                     "WARNING: Corrupt obj detected in "
                                     "obj_destroy (%s:d).\n",
                                     __FILE__, __LINE__);
                        if (debug_level() >= 3) {
                                abort();
                ...
                /*be sure to verify if anybody accesses this now-invalid hunk o'
                 * mem*/
        ...
        /*
         * It's officially destroyed, at this point. It just doesn't know it,
         * yet. It won't show up in traversal, find_by_name(), et cetera
         */
        if (obj != NULL) {
 ...
 */
int obj_destroy_impl(ABObj *treePtr) {
#define tree (*treePtr)
        int iReturn = 0;
...
static int objP_remove_all_refs_to(ABObj obj) { return obj_replace(obj, NULL); }
