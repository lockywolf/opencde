...
int obj_construct(ABObj obj, AB_OBJECT_TYPE type,
                  ABObj parent);          /* init to defaults*/
...
int obj_update_clients_with_data(ABObj, int update_code, void *update_data,
                                 UpdateDataFreeFunc update_data_free_func);

int obj_tree_update_clients_with_data(ABObj, int update_code, void *update_data,
                                      UpdateDataFreeFunc update_data_free_func);
...
int obj_tree_ensure_unique_names_in_modules(ABObj root, int maxnamelen);
int obj_tree_ensure_unique_names(ABObj root, int maxnamelen);
int obj_ensure_unique_name(ABObj, ABObj root, int maxnamelen);

STRING obj_alloc_unique_name(ABObj obj, STRING name, int maxNameLen);
ISTRING obj_alloc_unique_name_istr(ABObj obj, ISTRING name, int maxNameLen);
STRING obj_get_unique_name(ABObj obj, STRING name, int maxnamelen,
                           STRING nameOutBuf);

STRING obj_alloc_unique_name_for_child(ABObj obj, STRING name, int maxNameLen);
ISTRING obj_alloc_unique_name_istr_for_child(ABObj obj, ISTRING name,
                                             int maxnamelen);

STRING obj_get_unique_name_for_child(ABObj obj, STRING name, int maxnamelen,
                                     STRING nameOutBuf);
...
int obj_get_geometry(ABObj, int *ptr_x, int *ptr_y, int *ptr_width,
                     int *ptr_height);
...
int obj_test_set_geometry(ABObj, int x, int y, int width, int height);
...
int obj_set_help_data(ABObj, STRING help_volume, STRING help_location,
                      STRING help_text);
int obj_set_help_volume(ABObj, STRING help_volume);
int obj_set_help_location(ABObj, STRING help_location);
int obj_set_help_text(ABObj, STRING help_text);
int obj_get_help_data(ABObj, STRING *help_volume, STRING *help_location,
                      STRING *help_text);
...
ABObj obj_scoped_find_or_create_undef(ABObj parent, STRING scoped_name,
                                      AB_OBJECT_TYPE type);
ABObj obj_get_container_child(ABObj);
ABObj obj_get_child(ABObj obj, int which_child);
ABObj obj_get_child_cond(ABObj obj, int which_child, BOOL (*fn)(ABObj));
...
int obj_insert_child(ABObj obj, ABObj newchild, int newchildnum);
...
int obj_scoped_name_split(/* _buf params may be NULL */
                          STRING scoped_name, STRING module_name_buf,
...
#define obj_is_choice_item(obj)                                                \
        (obj->type == AB_TYPE_ITEM && obj->info.item.type == AB_ITEM_FOR_CHOICE)
#define obj_is_combo_box_item(obj)                                             \
        (obj->type == AB_TYPE_ITEM &&                                          \
         obj->info.item.type == AB_ITEM_FOR_COMBO_BOX)
#define obj_is_list_item(obj)                                                  \
        (obj->type == AB_TYPE_ITEM && obj->info.item.type == AB_ITEM_FOR_LIST)
#define obj_is_menu_item(obj)                                                  \
        (obj->type == AB_TYPE_ITEM && obj->info.item.type == AB_ITEM_FOR_MENU)
#define obj_is_menubar_item(obj)                                               \
        (obj->type == AB_TYPE_ITEM &&                                          \
         obj->info.item.type == AB_ITEM_FOR_MENUBAR)
#define obj_is_spin_box_item(obj)                                              \
        (obj->type == AB_TYPE_ITEM &&                                          \
...
#define obj_get_root(obj)                                                      \
        (ABObj)((obj)->part_of == NULL ? (obj) : (obj)->part_of)
...
#define obj_set_flag(obj, f) (((obj)->flags) |= ((unsigned int)(f)))

#define obj_clear_flag(obj, f) (((obj)->flags) &= ~((unsigned int)(f)))

#define obj_has_flag(obj, f) ((((obj)->flags) & ((unsigned int)(f))) != 0)
