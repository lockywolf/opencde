 ...
 */
#ifdef DEBUG
static int member_error_impl(ABObj obj, STRING member_name, STRING file,
                             int line);
...
int obj_set_has_border(ABObj obj, BOOL has_border) {
        verify_for_write(obj);
        switch (obj->type) {
...
int obj_get_increment(ABObj obj) {
        verify_for_read(obj);
...
int obj_set_increment(ABObj obj, int incr) {
        verify_for_write(obj);
        if (obj_is_spin_box(obj)) {
...
int obj_set_is_default(ABObj obj, BOOL is_default) {
        verify_for_write(obj);
...
int obj_set_is_defined(ABObj obj, BOOL is_defined) {
        verify_for_write(obj);
        if (is_defined) {
...
BOOL obj_is_defined(ABObj obj) {
        verify_for_read(obj);
        return obj_has_impl_flags(obj, ObjFlagIsDefined);
}

int obj_set_is_help_item(ABObj obj, BOOL is_help_item) {
        verify_for_write(obj);
        if (obj_is_item(obj)) {
...
BOOL obj_is_help_item(ABObj obj) {
        verify_for_read(obj);
        if (obj_is_item(obj)) {
...
int obj_set_accelerator(ABObj obj, STRING accel) {
        verify_for_write(obj);
        if (!obj_is_item(obj)) {
...
int obj_set_bg_color(ABObj obj, STRING bg_color) {
        verify_for_write(obj);
...
int obj_set_fg_color(ABObj obj, STRING fg_color) {
        verify_for_write(obj);
...
int obj_set_icon(ABObj obj, STRING icon) {
        verify_for_write(obj);
...
int obj_set_icon_label(ABObj obj, STRING icon_label) {
        verify_for_write(obj);
        if (obj_is_window(obj) && !obj_is_file_chooser(obj)) {
...
int obj_set_icon_mask(ABObj obj, STRING icon_mask) {
        verify_for_write(obj);
        if (obj_is_window(obj) && !obj_is_file_chooser(obj)) {
...
int obj_set_initial_value_string(ABObj obj, STRING val) {
        verify_for_write(obj);
        if (obj_is_text(obj)) {
 ...
 * 	 invalid).
 */
int obj_set_name_istr(ABObj obj, ISTRING new_name) {
        ISTRING old_name = NULL;
...
#ifdef DEBUG
                                util_dprintf(
                                    1, "ERROR: duplicate name rejected: %s\n",
...
int obj_set_name(ABObj obj, STRING strName) {
        int return_value = 0;
...
int obj_set_num_columns(ABObj obj, int num_columns) {
        int old_num_cols;
        int iRet = -1;
...
int obj_get_num_columns(ABObj obj) {
        verify_for_read(obj);
        switch (obj->type) {
        case AB_TYPE_CONTAINER:
...
int obj_set_num_rows(ABObj obj, int num_rows) {
        int old_num_rows;
        int iRet = -1;
...
int obj_get_num_rows(ABObj obj) {
        verify_for_read(obj);
        switch (obj->type) {
        case AB_TYPE_CONTAINER:
...
int obj_set_orientation(ABObj obj, AB_ORIENTATION orientation) {
        verify_for_write(obj);
        switch (obj->type) {
...
int obj_set_label(ABObj obj, STRING label) {
        verify_for_write(obj);
...
int obj_set_label_type(ABObj obj, AB_LABEL_TYPE label_type) {
        verify_for_write(obj);
...
int obj_set_label_alignment(ABObj obj, AB_ALIGNMENT type) {
        verify_for_write(obj);
...
int obj_set_label_position(ABObj obj, AB_COMPASS_POINT pos) {
        verify_for_write(obj);
...
int obj_set_menu_name(ABObj obj, STRING menu_name) {
        verify_for_write(obj);
...
int obj_set_menu_title(ABObj obj, STRING menu_title) {
        verify_for_write(obj);
...
int obj_set_mnemonic(ABObj obj, STRING mnem) {
        verify_for_write(obj);
        if (!obj_is_item(obj)) {
 ...
 */
int obj_cvt_to_ref(ABObj obj, ABObj ref_obj) {
        if (obj == NULL) {
...
ABObj obj_get_actual_obj(ABObj obj) {
        if (obj == NULL) {
                return NULL;
        }
        verify_for_read(obj);
...
int obj_set_min_value(ABObj obj, int minval) {
        int iRet = 0;
        verify_for_write(obj);
...
int obj_get_min_value(ABObj obj) {
        int iValue = 0;
        verify_for_read(obj);
...
int obj_set_max_value(ABObj obj, int maxval) {
        int iRet = 0;
        verify_for_write(obj);
...
int obj_get_max_value(ABObj obj) {
        int iValue = 0;
        verify_for_read(obj);
...
int obj_set_res_file_arg_classes(ABObj obj, AB_ARG_CLASS_FLAGS argClasses) {
        if (obj->type == AB_TYPE_PROJECT) {
...
int obj_set_resizable(ABObj obj, BOOL resizable) {
        verify_for_write(obj);
        switch (obj->type) {
...
BOOL obj_get_resizable(ABObj obj) {
        verify_for_read(obj);
        switch (obj->type) {
        case AB_TYPE_BASE_WINDOW:
        case AB_TYPE_DIALOG:
...
int obj_set_selection_required(ABObj obj, BOOL selreq) {
        int iRet = 0;
        verify_for_write(obj);
...
BOOL obj_get_selection_required(ABObj obj) {
        BOOL bRet = FALSE;
        verify_for_read(obj);
...
int obj_set_type(ABObj obj, AB_OBJECT_TYPE new_type) {
        verify_for_write(obj);
...
STRING
obj_get_process_string(ABObj obj) {
        verify_for_read(obj);
...
int obj_set_process_string(ABObj obj, STRING subproc) {
        verify_for_write(obj);
        if (obj->type == AB_TYPE_TERM_PANE) {
...
int obj_set_was_written(ABObj obj, BOOL was_written) {
        verify_for_write(obj);
        if (was_written)
...
BOOL obj_was_written(ABObj obj) {
        verify_for_read(obj);
...
int obj_set_write_me(ABObj obj, BOOL write_me) {
        verify_for_write(obj);
        if (write_me) {
...
BOOL obj_get_write_me(ABObj obj) {
        verify_for_read(obj);
        return obj_has_impl_flags(obj, ObjFlagWriteMe);
}

int obj_set_value_x(ABObj obj, int valx) {
        int iRet = 0;
        verify_for_write(obj);
...
int obj_set_value_y(ABObj obj, int valy) {
        int iRet = 0;
        verify_for_write(obj);
...
int obj_set_height_is_resizable(ABObj obj, BOOL resizable) {
        verify_for_write(obj);
        if (resizable)
...
BOOL obj_get_height_is_resizable(ABObj obj) {
        verify_for_read(obj);
...
int obj_get_voffset(ABObj obj) {
        ...
        if (obj_is_container(obj)) {
                ...
                /*
                 * REMIND: Not sure what to return when an ABObj is not a
                 * container type.
                 */
                return (0);
        }
...
obj_get_ref_point(ABObj obj) {
        ...
        if (obj_is_container(obj)) {
                ...
                /*
                 * REMIND: Not sure if I should use obj_get_label_position() or
                 * if I should do my own switch statement.
                 */
...
int obj_set_hattach_type(ABObj obj, AB_ATTACH_TYPE hattach_type) {
        verify_for_write(obj);
        if (obj_is_container(obj)) {
...
int obj_set_vattach_type(ABObj obj, AB_ATTACH_TYPE vattach_type) {
        verify_for_write(obj);
        if (obj_is_container(obj)) {
...
int obj_set_item_type(ABObj obj, AB_ITEM_TYPE item_type) {
        verify_for_write(obj);
        if (obj_is_item(obj)) {
...
int obj_set_group_type(ABObj obj, AB_GROUP_TYPE group_type) {
        verify_for_write(obj);
        if (obj_is_container(obj)) {
...
int obj_set_class_name(ABObj obj, STRING cname) {
        verify_for_write(obj);
...
int obj_set_col_align(ABObj obj, AB_ALIGNMENT col_align) {
        verify_for_write(obj);
        if (obj_is_container(obj)) {
...
int obj_set_container_type(ABObj obj, AB_CONTAINER_TYPE cont_type) {
        verify_for_write(obj);
        if (obj_is_container(obj)) {
...
int obj_get_decimal_points(ABObj obj) {
        verify_for_read(obj);
        if (obj_is_spin_box(obj))
...
int obj_set_decimal_points(ABObj obj, int dec_points) {
        verify_for_write(obj);
        if (obj_is_spin_box(obj)) {
...
int obj_set_default_act_button(ABObj obj, ABObj defaultb) {
        if (!obj_is_window(obj))
                return -1;
...
ABObj obj_get_default_act_button(ABObj obj) {
        ABObj defaultb = NULL;
...
int obj_set_direction(ABObj obj, AB_DIRECTION dir) {
        verify_for_write(obj);
        if (obj_is_scale(obj)) {
...
int obj_set_drawarea_width(ABObj obj, int width) {
        verify_for_write(obj);
...
int obj_set_drawarea_height(ABObj obj, int height) {
        verify_for_write(obj);
        if (obj_is_drawing_area(obj)) {
...
int obj_get_drawarea_width(ABObj obj) {
        verify_for_read(obj);
        if (obj_is_drawing_area(obj))
...
int obj_get_drawarea_height(ABObj obj) {
        verify_for_read(obj);
        if (obj_is_drawing_area(obj))
...
int obj_set_is_initially_active(ABObj obj, BOOL active) {
        verify_for_write(obj);
        if (active)
...
BOOL obj_is_initially_active(ABObj obj) {
        verify_for_read(obj);
        return obj_has_impl_flags(obj, ObjFlagIsInitiallyActive);
}

int obj_set_is_initially_iconic(ABObj obj, BOOL iconic) {
        int return_value = 0;
        verify_for_write(obj);
...
BOOL obj_is_initially_iconic(ABObj obj) {
        verify_for_read(obj);
        if (obj_is_window(obj) && !obj_is_file_chooser(obj)) {
...
int obj_set_is_initially_selected(ABObj obj, BOOL selected) {
        int return_value = 0;
        verify_for_write(obj);
...
int obj_set_is_initially_visible(ABObj obj, BOOL visible) {
        verify_for_write(obj);
        if (visible)
...
BOOL obj_is_initially_visible(ABObj obj) {
        verify_for_read(obj);
...
int obj_set_to(ABObj obj, ABObj to) {
        verify_for_write(obj);
...
int obj_set_from(ABObj obj, ABObj from) {
        verify_for_write(obj);
        if (!obj_is_action(obj)) {
...
int obj_set_func_type(ABObj obj, AB_FUNC_TYPE new_func_type) {
        verify_for_write(obj);
        if (!obj_is_action(obj)) {
...
int obj_set_func_name(ABObj obj, STRING func_name) {
        verify_for_write(obj);
        if (!(obj_is_action(obj) &&
...
int obj_set_func_name_suffix(ABObj obj, STRING suffix) {
        verify_for_write(obj);
        if (!obj_is_action(obj)) {
...
int obj_set_func_code(ABObj obj, STRING code) {
        verify_for_write(obj);
        if (!(obj_is_action(obj) &&
...
obj_get_func_builtin(ABObj obj) {
        verify_for_read(obj);
        if (!(obj_is_action(obj) &&
              (obj_get_func_type(obj) == AB_FUNC_BUILTIN))) {
                return AB_STDACT_UNDEF;
        }
        return (obj->info.action.func_value.builtin);
}

int obj_set_func_builtin(ABObj obj, AB_BUILTIN_ACTION act) {
        verify_for_write(obj);
...
int obj_set_when(ABObj obj, AB_WHEN when) {
        verify_for_write(obj);
...
int obj_set_width_is_resizable(ABObj obj, BOOL resizable) {
        verify_for_write(obj);
        if (resizable)
...
BOOL obj_get_width_is_resizable(ABObj obj) {
        verify_for_read(obj);
        return obj_has_impl_flags(obj, ObjFlagWidthIsResizable);
}

int obj_set_arg_type(ABObj obj, AB_ARG_TYPE arg_type) {
        verify_for_write(obj);
        if (!obj_is_action(obj)) {
...
int obj_set_arg_float(ABObj obj, double float_val) {
        verify_for_write(obj);
...
int obj_set_arg_int(ABObj obj, int int_value) {
        verify_for_write(obj);
        if (!(obj_is_action(obj) &&
              (obj->info.action.arg_type == AB_ARG_INT))) {
...
int obj_set_arg_string(ABObj obj, STRING argString) {
        verify_for_write(obj);
        if (!(obj_is_action(obj) &&
              (obj->info.action.arg_type == AB_ARG_STRING))) {
...
ABAttachment *obj_get_attachment(ABObj obj, AB_COMPASS_POINT dir) {
        ...
        default:
                if (util_get_verbosity() >= 3)
                        fprintf(stderr,
                                "obj_get_attachment: invalid direction\n");
                attach = NULL;
                break;
        }
...
AB_ATTACH_TYPE
obj_get_attach_type(ABObj obj, AB_COMPASS_POINT dir) {
        verify_for_read(obj);
        if (obj->attachments == NULL)
        ...
        default:
                if (util_get_verbosity() > 0)
                        fprintf(stderr,
                                "obj_get_attach_type: invalid direction\n");
                return AB_ATTACH_UNDEF;
        }
}
...
void *obj_get_attach_value(ABObj obj, AB_COMPASS_POINT dir) {
        ...
        default:
                if (util_get_verbosity() > 0)
                        fprintf(stderr,
                                "obj_get_attach_value: invalid direction\n");
                return NULL;
        }
}
...
int obj_get_attach_offset(ABObj obj, AB_COMPASS_POINT dir) {
        ...
        default:
                if (util_get_verbosity() > 0)
                        fprintf(stderr,
                                "obj_get_attach_offset: invalid direction\n");
                return 0;
        }
}

int obj_set_attachment(ABObj obj, AB_COMPASS_POINT dir, AB_ATTACH_TYPE type,
                       void *value, int offset) {
...
int obj_set_attach_type(ABObj obj, AB_COMPASS_POINT dir, AB_ATTACH_TYPE type) {
        verify_for_write(obj);
        if (obj->attachments == NULL)
                obj_init_attachments(obj);
...
int obj_set_attach_value(ABObj obj, AB_COMPASS_POINT dir, void *value) {
        verify_for_write(obj);
        if (obj->attachments == NULL)
                obj_init_attachments(obj);
...
int obj_set_attach_offset(ABObj obj, AB_COMPASS_POINT dir, int offset) {
        verify_for_write(obj);
        if (obj->attachments == NULL)
                obj_init_attachments(obj);
...
int obj_set_help_act_button(ABObj obj, ABObj helpb) {
        if (!obj_is_window(obj))
                return -1;
...
ABObj obj_get_help_act_button(ABObj obj) {
        ABObj defaultb = NULL;
...
*/
BOOL obj_has_help_data(ABObj obj) {
...
int obj_set_help_data(ABObj obj, STRING help_volume, STRING help_location,
                      STRING help_text) {
        verify_for_write(obj);
...
int obj_set_help_volume(ABObj obj, STRING help_volume) {
        verify_for_write(obj);
        istr_destroy(obj->help_volume);
        obj->help_volume = istr_create(help_volume);
...
int obj_set_help_location(ABObj obj, STRING help_location) {
        verify_for_write(obj);
        istr_destroy(obj->help_location);
        obj->help_location = istr_create(help_location);
...
int obj_set_help_text(ABObj obj, STRING help_text) {
        verify_for_write(obj);
        istr_destroy(obj->help_text);
        obj->help_text = istr_create(help_text);
...
int obj_get_help_data(ABObj obj, STRING *help_volume, STRING *help_location,
                      STRING *help_text) {
        verify_for_read(obj);
...
STRING
obj_get_help_location(ABObj obj) {
...
int obj_set_user_data(ABObj obj, STRING user_data) {
        verify_for_write(obj);
...
static int verify(ABObj obj, int min_debug, STRING file, int line) {
        int rc = 0;
...
static int member_error_impl(ABObj obj, STRING member_name, STRING file,
        ...
        *obj_name = 0;
        ...
        util_dprintf(0,
                     "ERROR(%s:%d): Bad attribute access '%s' in object
        '%s'.\n", file, line, member_name, obj_name); return 0;
        */
}
...
int obj_set_tooltalk_level(ABObj obj, AB_TOOLTALK_LEVEL tt_level) {
        verify_for_write(obj);
        if (obj_is_project(obj)) {
...
int obj_set_read_only(ABObj obj, BOOL is_read_only) {
        verify_for_write(obj);
        if (is_read_only) {
...
BOOL obj_get_read_only(ABObj obj) {
        verify_for_read(obj);
...
int obj_set_drag_cursor(ABObj obj, STRING filename) {
        verify_for_write(obj);
...
int obj_set_drag_cursor_mask(ABObj obj, STRING filename) {
        verify_for_write(obj);
...
int obj_set_drag_initially_enabled(ABObj obj, BOOL enabled) {
...
BOOL obj_get_drag_initially_enabled(ABObj obj) {
        return ((obj->impl_dnd_flags & ObjDndFlagDragEnabled) != 0);
}

int obj_set_drag_to_root_allowed(ABObj obj, BOOL allowed) {
...
BOOL obj_drop_on_children_is_allowed(ABObj obj) {
