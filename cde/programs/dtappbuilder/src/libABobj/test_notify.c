 ...
 *
 *	@(#)test_notify.c	1.16 14 Feb 1994
...
int main(int argc, char *argv[]) {
        ...
        obj_add_allow_reparent_callback(allow_reparent_callback1,
                                        "allow_reparent_callback1");
        obj_add_reparent_callback(reparent_callback1, "reparent_callback1");
        obj_add_reparent_callback(reparent_count_callback,
                                  "reparent_count_callback");
        obj_add_create_callback(rename_obj_create_callback,
                                "rename_obj_create_callback");
        obj_add_create_callback(composite_obj_create_callback,
                                "composite_obj_create_callback");
        obj_add_create_callback(mod2_obj_create_callback,
                                "mod2_obj_create_callback");
        obj_add_rename_callback(rename_callback1, "rename_callback1");
        obj_add_selected_change_callback(exclusive_select_callback,
                                         "obj_add_selected_change_callback");
        obj_add_selected_change_callback(count_select_callback,
                                         "count_select_callback");
        obj_add_selected_change_callback(composite_select_callback,
                                         "composite_select_callback");
        obj_add_geometry_change_callback(grid_geometry_callback,
                                         "grid_geometry_callback");
        ...
        obj_add_allow_geometry_change_callback(
            ...
            "composite_geometry_allow_callback");
        obj_add_allow_geometry_change_callback(bogus_geometry_allow_callback,
                                               "bogus_geometry_allow_callback");
        obj_add_destroy_callback(composite_destroy_callback,
                                 "composite_destroy_callback");
        obj_add_destroy_callback(count_destroy_callback,
                                 "count_destroy_callback");
        obj_add_update_callback(count_update_callback, "count_update_callback");
        obj_add_update_callback(composite_update_callback,
                                "composite_update_callback");
        obj_add_update_with_data_callback(
            composite_update_with_data_callback,
            "composite_update_with_data_callback");
        obj_add_update_with_data_callback(count_update_with_data_callback,
        ...
        if (objCount != allObjectsArrayCount) {
                fprintf(stderr, "Count mismatch (%d,%d) %s:%d\n", objCount,
                        allObjectsArrayCount, __FILE__, __LINE__);
                exit(1);
        }
         ...
         */
        {
                ...
                obj_reparent(slider1, container3);
                ++reparentCount;
                verify_count(project, objCount);
                 ...
                 */
                obj_reparent(slider1, container5);
                ++reparentCount;
                obj_reparent(list3, container3);
                ++reparentCount;
                obj_reparent(container5, container3);
                ++reparentCount;
                verify_count(project, objCount);
        }
...
static int test_update(ABObj tree) {
         ...
         */
        obj_update_clients_with_data(slider1, UPDATE_MSG1,
                                     (void *)&update_data1, free_update_data);
        updateCount += 3;
        ++data1_free_count;
        if (updateCount != updateEventCount) {
        ...
        numObjects = trav_count(module1, AB_TRAV_ALL);
        obj_tree_update_clients_with_data(
            module1, UPDATE_MSG2, (void *)&update_data2, free_update_data);
        ...
        obj_tree_update_clients_with_data(
            item2, UPDATE_MSG2, (void *)&update_data2, free_update_data);
        ++updateCount;
        ++data2_free_count;
...
#define objCount (*objCountPtr)
        ...
        iRC = obj_destroy_one(container5);
        --objCount;
        ++destroyCount;
        if (iRC < 0) {
        ...
        iRC = obj_destroy_one(module1);
        objCount -= 8;
        destroyCount += 8;
        verify_count(tree, objCount);

        printf("destroying complex subtree\n");
        iRC = obj_destroy(slider1);
        objCount -= 4;
        destroyCount += 4;
        verify_count(tree, objCount);

        verify_count(tree, objCount); /* be sure objCount is up-to-date */
        iRC = obj_destroy(tree);
        destroyCount += objCount;
        objCount = 0;
        tree = NULL;
        verify_count(tree, objCount);
...
static int verify_count_impl(ABObj tree, int count, STRING file, int line) {
        int localCount = 0;
...
static ABObj create_test_project(void) {
        ...
        if (module1 != NULL) {
                obj_set_name(module1, "module1");
                ++renameCount;
        }
        if (module2 != NULL) {
                obj_set_name(module2, "module2");
                ++renameCount;
        }
        if (module3 != NULL) {
                obj_set_name(module3, "module3");
                ++renameCount;
        }
...
static int set_name(ABObj obj) {
        ...
        ++(typeCounts[obj->type]);
        sprintf(name, "%s%d", util_object_type_to_string(obj->type) + 1,
                typeCounts[obj->type]);
...
static int composite_create_list(ABObj list) {
             ...
             (subobj = trav_next(&trav)) != NULL;) {
                if ((subobj != obj) && (subobj->part_of == obj)) {
                        obj_update_clients_with_data(
                            subobj, info->update_code, info->update_data,
...
static int free_update_data(int update_code, void *void_update_data) {
