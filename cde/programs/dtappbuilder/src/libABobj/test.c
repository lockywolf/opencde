...
static ABObj create_one(ABObj parent, AB_OBJECT_TYPE type, ABObj compositeRoot);
...
static int obj_print_salient_tree_indented(ABObj obj, int spaces, int verbosity,
                                           int numPrinted);
...
int main(int argc, STRING argv[]) {
        ...
        */
        ...
        if (((module1 == NULL) ||
             (!util_streq(obj_get_name(module1), "module1"))) ||
            ((module2 == NULL) ||
             (!util_streq(obj_get_name(module2), "module2"))) ||
            ((module3 == NULL) ||
             (!util_streq(obj_get_name(module3), "module3")))) {
                printf("error %s:%d\n", __FILE__, __LINE__);
                exit(1);
        }
...
static int check_scoped_searches(ABObj root) {
        ...
        printf("(module1, slider1): ");
        obj_print(obj);

        obj = obj_scoped_find_by_name(module1, "module2::slider2");
        printf("(module1, module2::slider2): ");
        obj_print(obj);
        if (obj == NULL) {
        ...
        printf("(module3, module2 :: slider2): ");
        obj_print(obj);
...
static int check_parents_first(ABObj root) {
        ...
        for (i = 0; i < objArraySize; ++i) {
                if (!visited[i]) {
                        fprintf(stderr,
                                "PARENTS_FIRST didn't visit all nodes! %s:%d\n",
                                __FILE__, __LINE__);
                        exit(1);
                }
        }
...
static int show_salient_ui_tree(ABObj tree) {
        int salientCount = 0;
        int printCount = 0;
        int i = 0;
...
static ABObj build_test_project() {
        ABObj project = NULL;
...
static ABObj create_one(ABObj parent, AB_OBJECT_TYPE type,
                        ABObj compositeRoot) {
        ABObj obj = NULL;
 ...
 */
static int obj_print_salient_tree(ABObj obj) {
        int iRet = 0;
        int printCount = 0;

        printf("***** Object tree *****\n");

        printCount = obj_print_salient_tree_indented(
            obj, 0, util_get_verbosity(), printCount);
...
static int obj_print_salient_tree_indented(ABObj obj, int spaces, int verbosity,
        ...
        for (trav_open(&trav, obj, travMod | AB_TRAV_SALIENT_CHILDREN);
             (child = trav_next(&trav)) != NULL;) {
                numPrinted = obj_print_salient_tree_indented(
                    child, child_spaces, verbosity, numPrinted);
        }
        trav_close(&trav);
