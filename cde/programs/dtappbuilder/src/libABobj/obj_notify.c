 ...
 *
 *	@(#)obj_notify.c	1.41 02 Feb 1995
 *cde_app_builder/src/libABobj
 *
...
**                                                                      **
**************************************************************************/
static int call_callbacks_in_list(NamedCallbackList list, void *data,
                                  ObjEvent event);
...
#define should_print_callbacks() ((forcePrintCallbacks) || (debug_level() >= 3))
...
int obj_add_allow_geometry_change_callback(ObjAllowGeometryChangeCallback cb,
                                           STRING debug_name) {
...
int obj_add_allow_reparent_callback(ObjAllowReparentCallback cb,
                                    STRING debug_name) {
...
int obj_add_create_callback(ObjCreateCallback cb, STRING debug_name) {
        return add_to_callback_list(&callbacksForCreate, (GenericCallback)cb,
                                    OBJEV_ATT_NONE, istr_create(debug_name));
}

int obj_add_destroy_callback(ObjDestroyCallback cb, STRING debug_name) {
        return add_to_callback_list(&callbacksForDestroy, (GenericCallback)cb,
                                    OBJEV_ATT_NONE, istr_create(debug_name));
}

int obj_add_geometry_change_callback(ObjAttChangeCallback cb,
                                     STRING debug_name) {
        return add_to_callback_list(&callbacksForAttChange, (GenericCallback)cb,
                                    OBJEV_ATT_GEOMETRY | OBJEV_ATT_POSITION |
                                        OBJEV_ATT_SIZE,
                                    istr_create(debug_name));
}

int obj_add_rename_callback(ObjAttChangeCallback cb, STRING debug_name) {
        return add_to_callback_list(&callbacksForAttChange, (GenericCallback)cb,
                                    OBJEV_ATT_NAME, istr_create(debug_name));
}

int obj_add_reparent_callback(ObjReparentCallback cb, STRING debug_name) {
        return add_to_callback_list(&callbacksForReparent, (GenericCallback)cb,
                                    OBJEV_ATT_NONE, istr_create(debug_name));
}

int obj_add_selected_change_callback(ObjAttChangeCallback cb,
                                     STRING debug_name) {
        return add_to_callback_list(&callbacksForAttChange, (GenericCallback)cb,
                                    OBJEV_ATT_SELECTED,
                                    istr_create(debug_name));
...
int obj_add_update_callback(ObjUpdateCallback cb, STRING debug_name) {
        return add_to_callback_list(&callbacksForUpdate, (GenericCallback)cb,
                                    OBJEV_ATT_NONE, istr_create(debug_name));
}

int obj_add_update_with_data_callback(ObjUpdateWithDataCallback cb,
                                      STRING debug_name) {
...
int objP_notify_set_mode(OBJ_EVENT_MODE newMode) {
        modeStack[modeStackIndex] |= newMode;
        return 0;
}

int objP_notify_clear_mode(OBJ_EVENT_MODE deadMode) {
...
OBJ_EVENT_MODE
objP_notify_get_mode() { return current_notify_mode(); }
...
int objP_notify_send_allow_geometry_change(ABObj obj, int x, int y, int w,
                                           int h) {
        OBJ_EVENT event;
...
int objP_notify_send_allow_reparent(ABObj obj, ABObj newParent) {
        OBJ_EVENT event;
...
int objP_notify_send_att_change(ABObj obj, OBJEV_ATT_FLAGS atts) {
        OBJ_EVENT event;
...
int objP_notify_send_create(ABObj obj) {
        OBJ_EVENT event;
...
int objP_notify_send_destroy(ABObj obj) {
        OBJ_EVENT event;
...
int objP_notify_send_geometry_change(ABObj obj, int oldX, int oldY,
...
int objP_notify_send_rc_geometry_change(ABObj obj) {
        OBJ_EVENT event;
...
int objP_notify_send_name_change(ABObj obj, ISTRING old_name) {
        OBJ_EVENT event;
        event.type = OBJEV_ATT_CHANGE;
...
int objP_notify_send_reparent(ABObj obj, ABObj oldParent) {
        OBJ_EVENT event;
...
int objP_notify_send_selected_change(ABObj obj) {
        OBJ_EVENT event;
...
int objP_notify_send_update(ABObj obj, BOOL update_subtree) {
        OBJ_EVENT event;
...
int objP_notify_send_update_with_data(
    ABObj obj, BOOL update_subtree, int update_code, void *update_data,
...
static int objP_dispatch_event(OBJ_EVENT *event) {
        int iReturn = 0;
        ...
        if (ignoreEvents) {
                 ...
                 * pretend everything is hunky-dory and ignore dispatch
                 * requests.
                 */
                iReturn = OK;
                goto epilogue;
...
static int force_flush_queue(void) {
        ...
        if (debugging()) {
                ...
                fprintf(stderr, "\n\nAPPARENT EVENT CYCLE DETECTED.  STARTING "
                                "EVENT TRACING.\n\n");
                fflush(stderr);
                sleep(3);
        }
 ...
 */
static int event_destruct(ObjEvent event, BOOL wasBatched) {
        int return_value = 0;
...
static int event_print(OBJ_EVENT *event, FILE *outFile, BOOL addNewline,
                       STRING name) {
        ABObj obj = NULL;
        char eventName[1024];
        ...
        *buf2 = 0;
        ...
        case OBJEV_ATT_CHANGE:
                ...
                case OBJEV_ATT_GEOMETRY: {
                        ...
                        sprintf(eventParamsPtr, "x:%d  y:%d  w:%d  h:%d", x, y,
                                w, h);
                } break;
                ...
                case OBJEV_ATT_HSCROLL:
                        ...
                        sprintf(eventParamsPtr, "%s",
                                util_cvt_bool_to_string(obj_has_hscrollbar(obj),
                                                        buf1, 256));
                        break;
                ...
                case OBJEV_ATT_SELECTED:
                        ...
                        sprintf(eventParamsPtr, "%s",
                                util_cvt_bool_to_string(obj_is_selected(obj),
                                                        buf1, 256));
                        break;
                ...
                case OBJEV_ATT_VSCROLL:
                        ...
                        sprintf(eventParamsPtr, "%s",
                                util_cvt_bool_to_string(obj_has_vscrollbar(obj),
                                                        buf1, 256));
                        break;
                }
                strcat(eventParams, ")");
        ...
        case OBJEV_REPARENT:
                ...
                sprintf(eventParams, "(%s old_parent:%s)",
                        obj_get_safe_name(obj, buf1, 256),
                        obj_get_safe_name(event->info.reparent.old_parent, buf2,
                                          256));
                break;
...
static int add_to_callback_list(NamedCallbackList *ppsList,
                                GenericCallback callback, OBJEV_ATT_FLAGS atts,
...
#define list (*ppsList)
        ...
        if (list == NULL) {
                list =
                    (NamedCallbackList)util_malloc(1 * sizeof(NAMED_CALLBACK));
                if (list == NULL) {
                        return ERR_NO_MEMORY;
                }
        ...
        iNumEntries += 2;
        newList = (NamedCallbackList)realloc(list, iNumEntries *
                                                       sizeof(NAMED_CALLBACK));
        if (newList == NULL) {
                return ERR_NO_MEMORY;
        }
        list = newList;
...
static int call_callbacks_in_list(NamedCallbackList list, void *data,
        ...
        for (i = 0; list[i].callback != NULL; ++i) {
                ...
                if (should_print_callbacks()) {
                        event_print(event, stderr, FALSE,
                                    istr_string(list[i].name));
                }
...
static BOOL events_equal(ObjEvent event1, ObjEvent event2) {
        ...
        switch (event1->type) {
        case OBJEV_ATT_CHANGE: {
                if ((event1->info.att_change.obj !=
                     event2->info.att_change.obj) ||
                    (event1->info.att_change.atts !=
                     event2->info.att_change.atts)) {
                        break;
                }
        ...
        case OBJEV_REPARENT:
                equal =
                    (event1->info.reparent.obj == event2->info.reparent.obj);
                break;
