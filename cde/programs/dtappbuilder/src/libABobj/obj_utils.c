...
ABObj obj_scoped_find_or_create_undef(ABObj parent, STRING scopedName,
        ...
        if (obj == NULL) {
                ...
                if (moduleName == NULL) {
                        ...
                        if (project != NULL) {
                                module = obj_find_module_by_name(project,
                                                                 moduleName);
                                if (module == NULL) {
                                        /* damn! module don't exist!! */
                                        module =
                                            obj_create(AB_TYPE_MODULE, project);
                                        obj_set_is_defined(module, FALSE);
                                        obj_set_name(module, moduleName);
                                }
                        }
                        if (module != NULL) {
                                /* actually create the object under the correct
                                 * module */
                 ...
                 * reference to an undefined object). Set the attributes that we
...
ab_make_valid_c_ident(STRING label) {
        ...
        *newPtr = 0; /* terminate the string */
        ...
        if (newPtr == name) {
                ...
                /*
                 * The entire label is composed of illegal characters, in that
                 * case, return a generated name.
                 */
        ...
        if (module_name_buf != NULL) {
                if (module_name_ptr == NULL) {
                        ...
                        *module_name_buf = 0;
                        ...
                        strncpy(module_name_buf, complex_name,
        ...
        *namebuf = 0;
        ...
        if (obj_is_action(obj)) {
                ...
                case AB_FUNC_USER_DEF:
                        sprintf(namebuf, "func:%s",
                                istr_string_safe(
                                    obj->info.action.func_value.func_name));
                        break;
        ...
        if (verbosity >= 4) {
                ...
                if (obj_is_action(obj)) {
                        ...
                        util_dprintf(0, "to: ");
                        if ((thatModule != NULL) &&
                            (thatModule != thisModule)) {
                                util_dprintf(0, "%s.",
                                             obj_get_safe_name(thatModule,
                                                               namebuf, 256));
                        }
                        ...
                        if (obj->ref_to != NULL) {
                                ...
                                util_dprintf(0, "[ref_to: %s]\n",
                                             obj_get_safe_name(obj->ref_to,
                                                               namebuf, 256));
                        }
                        if (obj->part_of != NULL) {
                                indent(spaces);
                                util_dprintf(0, "[part_of: %s]\n",
                                             obj_get_safe_name(obj->part_of,
                                                               namebuf, 256));
                        }
                        indent(spaces);
                        util_dprintf(0, "x: %d\n", obj->x);
...
#define check_str(obj, _field)                                                 \
        ...
        if (!objP_is_accessible(obj)) {
                ...
                return_value = -1;
                util_dprintf(
                    0, "ERROR - bad object ptr in obj_verify(): %#lx\n", obj);
                goto epilogue;
        }
        ...
        if (obj->impl_flags == ObjFlagAlreadyFreedValue) {
                ...
                util_dprintf(0, "\n");
                sleep(5);
                util_dprintf(0, "        (Actually, it's just a reference to a "
        ...
        if ((obj->type != AB_TYPE_PROJECT) && (obj->name != NULL)) {
                namesList = NULL;
                for (parent = obj->parent; parent != NULL;
                ...
                if (parent != NULL) {
                        ...
                        if (namesList == NULL) {
                                util_dprintf(
                                    1, "No names index found containing %s\n",
                                    obj_name);
                                return_value = -1;
                                goto epilogue;
                        } else {
                                /*
                                 * A destroyed object must *not* be in the
                                 * index, but any other object *must* be in the
                                 * index.
                                 */
                                tmpObj = (ABObj)strlist_get_istr_data(
                                    namesList, obj->name);
...
#ifdef DEBUG
        if (objIsAccessible) {
                obj->debug_last_verify_time =
                    time(NULL); /*only exists in debug build*/
        }
#endif /* DEBUG */
...
static int verify_the_silly_index(ABObj nameScopeObj, ABObj *objArray,
        ...
        *nameBuf3 = 0;
        ...
        for (nameCount = 0; nameCount < numNames; ++nameCount) {
                namedObjFound = FALSE;
                curName =
                    strlist_get_istr(names, nameCount, (void **)&namedObj);
                for (objCount = 0;
                     (!namedObjFound) && (objCount < objArraySize);
                     ++objCount) {
                        /*
                        ...
                        /*
                         * We've found the object that goes with this name.
                         * Check to see if it is in the right scope.
                         */
                        curNames = NULL;
                        ...
                        if (curNames == NULL) {
                                util_dprintf(
                                    1,
                                    "No names list found for object %s in "
                                    "module %s!\n",
                                    obj_get_safe_name(curObj, nameBuf1, 1024),
                                    obj_get_safe_name(obj_get_module(curObj),
                                                      nameBuf2, 1024));
                                ...
                                util_dprintf(
                                    1,
                                    "Object %s should be in scope for %s, but "
                                    "is in scope for %s\n",
                                    obj_get_safe_name(curObj, nameBuf1, 1024),
                                    obj_get_safe_name(curScopeObj, nameBuf2,
                                                      1024),
                                    obj_get_safe_name(nameScopeObj, nameBuf3,
                                                      1024));
                                return_value = ERR_INTERNAL;
                                goto epilogue;
                                ...
                                util_dprintf(
                                    1,
                                    "Object %s has incorrect reference in "
                                    "index!\n",
                                    obj_get_safe_name(curObj, nameBuf1, 1024));
                        } else {
                                namedObjFound = TRUE;
...
#ifdef DEBUG
                        util_dprintf(1,
                                     "Name is in index '%s', but no such "
                                     "object exists!\n",
...
#define objArraySize (*objArraySizePtr)
                ...
                *name = 0;
                for (i = 0; i < objArraySize; ++i) {
                        if (objArray[i] == root) {
                                util_dprintf(
                                    1,
                                    "INTERNAL ERROR: some sort of cycle "
                                    "detected involving %s\n",
        ...
        ++objArraySize;
        newObjArray =
            (ABObj *)realloc(objArray, objArraySize * sizeof(ABObj *));
        if (newObjArray == NULL) {
        ...
        for (child = root->first_child; child != NULL;
             child = child->next_sibling) {
                if ((rc = build_obj_array(&objArray, &objArraySize, child)) <
                    0) {
                        return rc;
                }
        }
