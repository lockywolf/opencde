...
static int merge_files_by_segment(File oldFile, CSegArray oldSegs,
                                  STRING oldFileName, File newFile,
                                  CSegArray newSegs, STRING newFileName,
                                  File *mergedFileOutPtr,
                                  File *deltaFileOutPtr);

static int match_all_segments(CSegArray newSegs, CSegArray oldSegs);

static CSeg match_segment(CSeg oldSeg, CSegArray oldSegArray,
...
int abmfP_merge_c_files(File oldFile, STRING oldFileName, File newFile,
                        STRING newFileName, File *mergedFileOutPtr,
                        File *deltaFileOutPtr) {
        int return_value = 0;
        ...
        */
        ...
        rc = merge_files_by_segment(oldFile, oldSegs, oldFileName, newFile,
                                    newSegs, newFileName, &mergedFile,
                                    &deltaFile);
        if (rc < 0) {
                return_value = rc;
        } else {
        ...
        *deltaFileOutPtr = NULL;
        ...
        while ((c = fgetc(oldFile)) != EOF) {
                ...
                if (oldFileOffset == (nextModOffset + 1)) {
                        ...
                        oldUserSeg = oldUserSeg->next;
                        while ((oldUserSeg != NULL) &&
                               (oldUserSeg->clientData == NULL)) {
                                oldUserSeg = oldUserSeg->next;
                        }
                        nextModOffset =
                            (oldUserSeg == NULL ? -1 : oldUserSeg->offset);
                } else {
                        fputc(c, mergedFile);
                }
        }
 ...
 */
static int match_all_segments(CSegArray oldSegs, CSegArray newSegs) {
        CSeg oldSeg = NULL;
        CSeg newSeg = NULL;
        ...
        for (i = 0; i < oldSegs->numSegs; ++i) {
                ...
                if (newSeg != NULL) {
                        for (userSegCount = 0;
                             ...
                             ++userSegCount) {
                                if (!util_streq(
                                        oldSeg->userSegs.segs[userSegCount]
                                            .text,
...
static CSeg match_segment(CSeg oldSeg, CSegArray oldSegArray,
        ...
        if (oldSeg->type == CSEG_GLOBAL) {
                ...
                assert(oldSegNum >= 0);
                assert((oldSegIndex >= 0) &&
                       (oldSegIndex < oldSegArray->numSegs));
                ...
                for (i = 0; i < newSegArray->numSegs; ++i) {
                        if (util_streq(oldSeg->name,
                                       newSegArray->segs[i].name)) {
...
int main(int argc, char *argv[]) {
         ...
         * Merge 'em!
         */
        rc = abmfP_merge_c_files(oldFile, oldFileName, newFile, newFileName,
                                 &mergedFile, &deltaFile);
