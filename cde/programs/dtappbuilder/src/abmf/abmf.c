...
static int mark_modules_to_load_and_write(ABObj project, BOOL loadAllModules,
                                          BOOL genAllModules, BOOL genNoModules,
...
static BOOL proj_file_has_modules(STRING projFileName, StringList fileNames);
static int replace_string_shorter(STRING buf, STRING subStr, STRING replaceStr);
...
int main(int argc, STRING *argv) {
        ...
        if (cmdline->use_default_project) {
                ...
                if (cmdline->proj_file == NULL) {
                        if (!strlist_is_empty(cmdline->files)) {
                                ...
                                if (!util_be_silent()) {
                                        util_printf(catgets(
                                            Dtb_project_catd, 1, 1,
                                            "No project file that references "
                                            "module(s) - using defaults\n"));
                                }
                        } else if (numProjFiles == 0) {
                                util_printf(catgets(Dtb_project_catd, 1, 2,
                                                    "No files specified, and "
                                                    "no project file found\n"));
                                abmf_usage();
                        } else {
                                /* assume find_proj_file() complained
                                 * sufficiently */
                                exit(1);
                        }
                }
        ...
        if (strlist_is_empty(cmdline->files)) {
                ...
                if (cmdline->write_main) {
                        ...
                        if (useDefaultProject) {
                                /* dtcodegen -np module.bil -main : only gen
                                 * main */
                                genMainOnly = TRUE;
                        }
                } else {
        ...
        util_dprintf(1,
                     "main:%d mainonly:%d all:%d defaultproj:%d showall:%d\n",
        ...
        if (cmdline->proj_file != NULL) {
                /* project = bil_load_file(istr_string(cmdline->proj_file),
                NULL, project); assert(project != NULL); */
        ...
        if (useDefaultProject) {
                ...
                *projNameBuf = 0;
                ...
                for (i = 0; i < numModuleFiles; ++i) {
                        moduleFileName =
                            strlist_get_str(cmdline->files, i, NULL);
        ...
        mark_modules_to_load_and_write(project, cmdline->force_load_all,
                                       genAllFiles, genMainOnly,
                                       cmdline->files);
        ...
        if (!write_required(project)) {
                ...
                fprintf(stderr,
                        catgets(Dtb_project_catd, 1, 5, "%s: Nothing to do!\n"),
                        prog_name_string);
                exit(1);
        }
        ...
        /*
         * The tree has been loaded, examined, mangled, and generally messed
         * with. Now, do what the user has requested.
         */
        ...
        if (cmdline->dump_tree) {
                ...
                AB_ARG_CLASS_FLAGS dumpedRes =
                    obj_get_res_file_arg_classes(project);
                 ...
                 * Generate the code!
                 */
                iRC = abmf_generate_code(project, cmdline->cgen_restriction,
                                         cmdline->merge_files, i18n_method,
                                         cmdline->prototype_funcs, dumpedRes);
                if (iRC < 0) {
                        exitValue = 1;
                }
        }
...
static int parse_args(int argc, char *argv[], CmdlineArgs cmdline) {
        int iReturn = 0;
        int argCount = 0;
                ...
                if (*arg == '-') {
                        if (strcmp(arg, "-s") == 0 ||
                            strcmp(arg, "-silent") == 0) {
                                ...
                                if ((argCount + 1) >= argc) {
                                        util_error(
                                            catgets(Dtb_project_catd, 1, 6,
                                                    "Missing project name for "
                                                    "-p option"));
                                        abmf_usage();
                                } else {
                                        ...
                                        if (util_file_name_has_extension(
                                                projArg, "bip") ||
                                            util_file_name_has_extension(
                                                projArg, "bix")) {
                                                cmdline->proj_file =
                                                    istr_const(projArg);
                                        } else {
                                                char projFile[MAX_PATH_SIZE];
                                                sprintf(projFile, "%s.bip",
                                                        projArg);
                                                if (!util_file_exists(
                                                        projFile)) {
                                                        char encapsFile
        ...
        if (cmdline->use_default_project && (cmdline->proj_file != NULL)) {
                ...
                util_printf_err(catgets(Dtb_project_catd, 1, 9,
                                        "ignoring -noproject (-np) because "
                                        "project file was specified: %s\n"),
                                proj_file_name);
                cmdline->use_default_project = FALSE;
        }
        ...
        *newModuleName = 0;
        ...
        for (trav_open(&fileTrav, project, AB_TRAV_FILES | AB_TRAV_MOD_SAFE);
             (file = trav_next(&fileTrav)) != NULL;) {
                ...
                util_get_file_name_from_path(istr_string(fileName),
                                             newModuleName, 1024);
                ...
                if (module != NULL) {
                        /* this module already exists, probably because of a
                         * forward */
                        /* reference */
                        obj_destroy(file);
                } else {
                        ...
                        mfobj_set_flags(module,
                                        CGenFlagLoadMe | CGenFlagIsReferenced);
                }
         ...
         */
        {
                ...
                *explodedModuleFile = 0;
                ...
                for (trav_open(&moduleTrav, project,
                               AB_TRAV_MODULES | AB_TRAV_MOD_SAFE);
                     (module = trav_next(&moduleTrav)) != NULL;) {
                        sprintf(explodedModuleFile, "%s.bil",
                                obj_get_name(module));

                        if ((genAllModules && obj_is_defined(module)) ||
                            (strlist_str_exists(fileNames,
                                                explodedModuleFile) &&
                             (!genNoModules))) {
                                if (obj_is_defined(
         ...
         */
        {
                ...
                for (i = 0; i < numFiles; ++i) {
                        fileName =
                            strlist_get_str(fileNames, i, &wasFoundVoidPtr);
                        assert(fileName != NULL);
                        wasFound = (BOOL)(unsigned long)wasFoundVoidPtr;
                        if (!wasFound) {
                                util_printf_err(
                                    "module not found in project: %s\n",
                                    fileName);
                                errOccurred = TRUE;
                        }
                }
                ...
                for (trav_open(&allTrav, project,
                     ...
                     (obj = trav_next(&allTrav)) != NULL;) {
                        ...
                        case AB_TYPE_BASE_WINDOW:
                                if (obj_is_defined(obj) && obj_is_popup(obj) &&
                                    obj_get_write_me(obj_get_module(obj))) {
                                        if ((winParent = obj_get_win_parent(
                                                 obj)) != NULL) {
                                                mfobj_set_flags(
                                                    obj_get_module(winParent),
                                                    CGenFlagIsReferenced);
                                        }
                                }
                        ...
                        case AB_TYPE_ACTION:
                                ...
                                if (((obj1 != NULL) && obj_is_defined(obj1) &&
                                    ...
                                    ((obj2 != NULL) && obj_is_defined(obj2) &&
                                     obj_get_write_me(obj_get_module(obj2)))) {
                                        /*
                                         * At least one object will be written,
                                ...
                                /*
                                 * to/from of cross-module conns must be
                                 * available to write main
                                 */
                                if (obj_get_write_me(project) &&
                                    obj_is_defined(obj) &&
...
static int abmf_usage(void) {
        ...
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 13,
                        "Code is generated for each module specified on the "
                        "command line, or for\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 14,
                        "all modules in the project, if no modules are "
                        "specified. If no project\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 15,
                        "file is specified, a project file containing the "
                        "specified module(s) is\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 16,
                        "searched for in the current directory.\n\n"));

        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 17,
                        "Files with extension .bip are assumend to be BIL "
                        "project files, files with\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 18,
                        ".bix extenstion are assumed to be encapsulated BIL "
                        "files, and files\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 19,
                        "With a .bil extension are assumed to be BIL module "
                        "files.\n\n"));

        fprintf(
            stderr, "%s",
            catgets(
                Dtb_project_catd, 1, 20,
                "Options (* = default, + = default with no project file):\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 21,
                        "  -help (-h)        Print out this help message\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 22,
                        "  -main             Write file containing main()\n"));
        fprintf(
            stderr, "%s",
            catgets(
                Dtb_project_catd, 1, 23,
                "  -changed          Only generate files that have changed\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 24,
                        "* -merge	     Merge generated _stubs.c files "
                        "with previous version\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 25,
                        "  -nomerge          Don't merge existing and new "
                        "stubs file\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 26,
                        "* -project (-p)     Specify a project to generate "
                        "code for\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 27,
                        "  -noproject (-np)  Use default project settings, "
                        "ignore project file\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 28,
                        "+ -showall          Application shows (maps) all "
                        "windows at startup\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 29,
                        "* -noshowall        Application shows (maps) only "
                        "initially-visible windows\n"));
        fprintf(
            stderr, "%s",
            catgets(Dtb_project_catd, 1, 30,
                    "  -silent (-s)      Silent mode, no messages written\n"));
        fprintf(stderr, "%s",
                catgets(Dtb_project_catd, 1, 31,
                        "  -verbose (-v)     Verbose mode, detailed progress "
                        "messages\n\n"));
 ...
 *
 */
static int find_proj_file(CmdlineArgs cmdline) {
        DIR *dir = NULL;
        ...
        if (strlist_get_num_strs(&projFiles) == 1) {
                cmdline->proj_file =
                    istr_create(strlist_get_str(&projFiles, 0, NULL));
        } else if (strlist_get_num_strs(&projFiles) > 1) {
                int i = 0;
                char *prog_name_string = util_get_program_name();

                fprintf(stderr,
                        catgets(Dtb_project_catd, 1, 33,
                                "%s: Please specify project file (e.g."),
                        prog_name_string);
                for (i = 0; i < strlist_get_num_strs(&projFiles); ++i) {
                        fprintf(stderr, ", %s",
                                strlist_get_str(&projFiles, i, NULL));
                }
                fprintf(stderr, "%s", catgets(Dtb_project_catd, 1, 34, ")\n"));
                if (!strlist_is_empty(cmdline->files)) {
                        fprintf(stderr,
                                catgets(Dtb_project_catd, 1, 35,
                                        "%s: More than one project contains "
                                        "specified module(s)\n"),
                                prog_name_string);
                }
        }
...
static BOOL proj_file_has_modules(STRING projFileName,
                                  StringList moduleFileNames) {
                        ...
                        if ((fgets(word, 6, file) != NULL) &&
                            (strncmp(word, "files", 5) == 0)) {
                                ...
                                while (!filesDone) {
                                   ...
                                   (strncmp(word, "e", 1) == 0)) {
                                /* :module <module-name */
                                /* REMIND: we need to have this for .bix files
                                 * to work! */
                        } else {
                                fsetpos(file, &fpos);
                        }
...
static int abmf_init(void) {
        ...
        replace_string_shorter(abmfP_lib_default_dragCB->proto,
                               templateFuncName, "%s");
        replace_string_shorter(abmfP_lib_default_dragCB->def, templateFuncName,
                               "%s");

        templateFuncName = "dtb_default_dropCB";
        replace_string_shorter(abmfP_lib_default_dropCB->proto,
                               templateFuncName, "%s");
        replace_string_shorter(abmfP_lib_default_dropCB->def, templateFuncName,
                               "%s");
 ...
 */
static int replace_string_shorter(STRING buf, STRING subStr,
                                  STRING replaceStr) {
        int numReplaced = 0;
        char *bufPtr = buf;
        ...
        while ((subStrPtr = strstr(bufPtr, subStr)) != NULL) {
                memmove(subStrPtr + replaceStrLen, subStrPtr + subStrLen,
        ...
        for (trav_open(&moduleTrav, project, AB_TRAV_MODULES);
             (module = trav_next(&moduleTrav)) != NULL;) {
                ...
                for (trav_open(&uiTrav, module,
                               AB_TRAV_UI | AB_TRAV_MOD_PARENTS_FIRST |
                                   AB_TRAV_MOD_SAFE);
                                        ...
                                        if (!(obj_is_item(obj) ||
                                              ...
                                              obj_is_message(obj))) {
                                                ...
                                                util_dprintf(
                                                    0,
                                                    "WARNING: "
                                                    "Object has no class name "
                                                    "(NoCodeGenFlag not set): "
                                                    "%s\n",
                                                    obj_get_safe_name(obj, name,
                                                                      256));
                                        }
                                }
#endif /* DEBUG */
...
static int munge_liberate_menu(ABObj menu) {
        ...
        }
        {
                ...
                if ((compRoot != menu) && ((compType == AB_TYPE_ITEM) ||
                     ...
                     (menu = trav_next(&menuTrav)) != NULL;) {
                        if (obj_is_ref(menu)) {
                                ...
                                if (obj_is_popup(menu)) {
                                        STRING title = obj_get_menu_title(
                                            obj_get_parent(menu));
                                        if (!util_strempty(title))
                                                objxm_create_popup_menu_title(
                                                    menu, OBJXM_CONFIG_CODEGEN,
                                                    title);
                                }
                        }
                }
...
static int print_tree(ABObj root, int indent) {
        ...
        if ((travType == AB_TRAV_CHILDREN) || (obj_is_salient(root))) {
        ...
        if (travType == AB_TRAV_SALIENT_CHILDREN) {
                for (trav_open(&trav, root, AB_TRAV_CHILDREN);
                     (child = trav_next(&trav)) != NULL;) {
                        if ((!obj_is_salient(child)) &&
                            (obj_get_root(child) != root)) {
        ...
        for (trav_open(&allTrav, project,
             ...
             (obj = trav_next(&allTrav)) != NULL;) {
                ...
                if (obj_is_action(obj) &&
                    (obj_get_func_type(obj) == AB_FUNC_USER_DEF)) {
                        /*
                         * we have a user-defined callback. decide where to put
                         * it
                         */
                        funcName = obj_get_func_name(obj);
                        callbackScopeObj =
                            abmfP_find_callback_scope(project, funcName);
                        if (callbackScopeObj == NULL) {
                                /* doesn't exist */
                                if (obj_is_intra_module(obj) &&
                                    !obj_is_project(obj_get_from(obj))) {
                                        strlist_add_str(
                                            mfobj_get_module_data(module)
                                                ->callbacks,
                                            funcName, (void *)obj);
                                } else {
                                        strlist_add_str(
                                            mfobj_get_proj_data(project)
                                                ->callbacks,
                                            funcName, (void *)obj);
                                ...
                                if (obj_is_module(callbackScopeObj) &&
                                    (callbackScopeObj != module)) {
                                        /*
                                         * it's in more than one module - put it
                                         * in the project
                                         */
                                        int str_index = 0;
                                        ABObj originalConn = NULL;
                                        StringList moduleCallbacks =
                                            mfobj_get_module_data(
                                                callbackScopeObj)
                                                ->callbacks;

                                        str_index = strlist_get_str_index(
                                            moduleCallbacks, funcName);
                                        strlist_get_str(moduleCallbacks,
                                                        str_index,
                                                        (void **)&originalConn);
                                        ...
                                        strlist_remove_index(moduleCallbacks,
                                                             str_index);
                                        strlist_add_str(
                                            mfobj_get_proj_data(project)
                                                ->callbacks,
                                        ...
                                        mfobj_set_flags(
                                            obj, CGenFlagWriteDefToProjFile);
                                }

                                /*
                                 * Mark this as a duplicate, so we only write it
                                 * once
                                 */
                        ...
                        /*
                         * For some reason, menu titles are of type menu and not
                         * label
                         */
                        ...
                        if ((obj_is_menu(obj) || (obj_is_menu_ref(obj))) &&
                            (obj_get_class_name(obj) == NULL)) {
                                obj_set_class_name(
                                    obj, istr_string(abmfP_xmMenuShell));
                        }
                } /* obj_get_write_me() */
                if ((!(mfobj_get_proj_data(project)->has_ui_obj)) &&
                    obj_is_ui(obj)) {
...
static BOOL abmfP_obj_has_no_module(ABObj obj) {
...
static int abmfP_create_obj_data_for_module(ABObj objInModule) {
        ...
        if ((objInModule == NULL) || (mfobj_has_data(objInModule)) ||
...
static int ensure_data_for_module_obj(ABObj module) {
        ...
        if (mfobj_get_module_data(module) == NULL) {
                moduleData =
                    (CGenModuleData)calloc(sizeof(CGenModuleDataRec), 1);
