...
static int find_func(BFile file, long *funcOffPtr, long *funcLenPtr,
                     long *bodyOffPtr, long *bodyLenPtr) {
...
static int find_func_proto(BFile file, long funcOff, long nameOff, long bodyOff,
                           long *protoOffPtr, long *protoLenPtr) {
...
static int get_func(FILE *file, FileFuncInfo funcInfo) {
        ...
        while ((!abort) && (!foundFunc) && (!bfile_eof(bfile))) {
                ...
                if (!err) {
                        rc = find_func(bfile, &funcOff, &funcLen, &bodyOff,
                                       &bodyLen);
                        if (rc < 0) {
                                err = TRUE;
                                abort = TRUE;
                ...
                if (!err) {
                        rc = find_func_name(bfile, funcOff, bodyOff, &nameOff,
                                            &nameLen);
                        if (rc < 0) {
                                err = TRUE;
                        }
                ...
                if (!err) {
                        rc = trim_opening_comment(bfile, &funcOff, &funcLen,
                                                  protoOff);
                        if (rc < 0) {
                                err = TRUE;
                        }
        ...
        if (foundFunc) {
                grab_string_from_file(&(funcInfo->funcName), bfile, nameOff,
                                      nameLen);
                format_func_name(&(funcInfo->funcName));
                grab_string_from_file(&(funcInfo->funcProto), bfile, protoOff,
                                      protoLen);
                format_func_proto(&(funcInfo->funcProto));
                grab_string_from_file(&(funcInfo->funcCode), bfile, funcOff,
                                      funcLen);
...
static int format_func_name(STRING *namePtr) {
#define name (*namePtr)
        return 0;
#undef name
}

static int format_func_proto(STRING *protoPtr) {
...
static int format_func_def(STRING *defPtr) {
#define def (*defPtr)
        return 0;
#undef def
}
...
static COMMENT_TYPE find_comment_start(BFile file, long *commentOffOutPtr) {
