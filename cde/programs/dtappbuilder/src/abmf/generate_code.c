...
static int log_add_entry(GenLog, STRING file_name, time_t mod_date,
                         BOOL merged);
...
static int write_make_file(GenCodeInfo genCodeInfo, GenLog genLog,
                           ABObj project, BOOL merge_files, BOOL check_dates,
                           BOOL *makeFileChangedPtr);

static int write_project_files(GenCodeInfo genCodeInfo, GenLog genLog,
                               ABObj project, BOOL merge_files,
                               BOOL check_dates, BOOL force_write_c_file,
                               BOOL *headerFileChangedPtr,
                               BOOL *stubsCFileChangedPtr);

static int write_module_files(GenCodeInfo genCodeInfo, GenLog genLog,
                              ABObj module, BOOL merge_files, BOOL check_dates,
                              STRING resFileName, BOOL *headerFileChangedPtr,
                              BOOL *uiCFileChangedPtr,
                              BOOL *stubsCFileChangedPtr);

static int write_utils_files(GenCodeInfo genCodeInfo, GenLog genLog,
                             ABObj project, BOOL check_dates,
                             BOOL *headerFileChangedPtr, BOOL *CFileChangedPtr);

static int write_app_resource_file(GenCodeInfo genCodeInfo, GenLog genLog,
                                   ABObj project, BOOL mergeFiles,
                                   BOOL checkDates, BOOL *fileChangedOut);
static int write_msg_file(GenCodeInfo genCodeInfo, GenLog genLog, ABObj project,
                          BOOL checkDates, BOOL *msgFileChangedOut);
static int print_progress_message(int verbosity, STRING message,
                                  STRING fileName);
static int print_backing_up_message(STRING fromFile, STRING toFile);
static int print_skipping_message(STRING fileName, ABMF_SKIP_WHY why);
static int count_possible_substruct_fields(ABObj obj);

static int compare_file_times(STRING input_file, STRING output_file,
                              GenLog genLog);

static int should_write_file(STRING output_file, STRING input_file,
                             GenLog genLog, BOOL merging, BOOL check_dates);

static int check_and_merge_c_file(BOOL needMerge, File *codeFileInOut,
                                  STRING codeFileName,
                                  ABMF_SKIP_WHY *fileSkipReasonInOut,
                                  File *deltaFileOut);
static int check_and_replace_file(STRING fileName, GenLog genLog, File newFile,
                                  BOOL check_for_changes, BOOL fileWasMerged,
...
#define print_writing_message(_fname)                                          \
        {                                                                      \
                STRING progress_msg;                                           \
                progress_msg =                                                 \
                    XtNewString(catgets(Dtb_project_catd, 1, 52, "writing"));  \
 ...
 */
int abmf_generate_code(ABObj project, ABMF_CGEN_RESTRICTION restriction,
                       BOOL merge_files, ABMF_I18N_METHOD i18n_method_in,
        ...
        GenLogRec genLogRec, *genLog = &(genLogRec);
        ...
        *resFileName = 0;
        ...
        if (genCodeInfo->i18n_method == ABMF_I18N_XPG4_API) {
                if (!abmfP_initialize_msg_file(genCodeInfo, project)) {
                        util_dprintf(
                            0, "Cannot generate message source file: %s\n",
        ...
        for (trav_open(&trav, project, AB_TRAV_MODULES);
             (module = trav_next(&trav)) != NULL;) {
                if (!obj_get_write_me(module) || !obj_is_defined(module)) {
                        continue;
                }
                 ...
                 */
                abmfP_get_intermediate_res_file_name(module, resFileName,
                                                     MAX_PATH_SIZE);
                if (genCodeInfo->dumped_resources == AB_ARG_CLASS_FLAGS_NONE) {
                        ...
                        genCodeInfo->resource_file = abmfP_res_file_open(
                            resFileName, resFileName, module, TRUE);
                        if (genCodeInfo->resource_file == NULL) {
                                exit(1);
                        }
                }

                iRC = write_module_files(genCodeInfo, genLog, module,
                                         merge_files, check_dates, resFileName,
                                         &headerFileChanged, &uiCFileChanged,
                                         &stubsCFileChanged);
         ...
         */
        if (obj_get_write_me(project)) {
                abmfP_get_intermediate_res_file_name(project, resFileName,
                                                     MAX_PATH_SIZE);
                if (genCodeInfo->dumped_resources == AB_ARG_CLASS_FLAGS_NONE) {
                        unlink(resFileName);
                } else {
                        ...
                        genCodeInfo->resource_file = abmfP_res_file_open(
                            resFileName, resFileName, project, TRUE);
                        if (genCodeInfo->resource_file == NULL) {
                                exit(1);
                        }
                }

                iRC = write_project_files(
                    genCodeInfo, genLog, project, merge_files, check_dates,
                    forceWriteProjectCFile, &headerFileChanged,
                    &stubsCFileChanged);
                return_if_err(iRC, iRC);
                abmfP_res_file_close(genCodeInfo->resource_file);
        }
         ...
         */
        iRC = write_app_resource_file(genCodeInfo, genLog, project, merge_files,
                                      check_dates, &resFileChanged);
        return_if_err(iRC, iRC);
        ...
        if (obj_get_write_me(project)) {
                iRC =
                    write_utils_files(genCodeInfo, genLog, project, check_dates,
                                      &headerFileChanged, &stubsCFileChanged);
                return_if_err(iRC, iRC);

                iRC = write_make_file(genCodeInfo, genLog, project, merge_files,
                                      check_dates, &makeFileChanged);
                return_if_err(iRC, iRC);
        }

        iRC = write_msg_file(genCodeInfo, genLog, project, check_dates,
                             &msgFileChanged);
        return_if_err(iRC, iRC);
        ...
        if (!appResFileComplete) {
                 ...
                 */
                if (debugging()) {
                        util_dprintf(1, "Warning: Application resource file "
                                        "incomplete, because some modules\n");
                        util_dprintf(1,
...
#define codeFile (genCodeInfo->code_file)
        ...
        *stubsBakFileName = 0;
        ...
        if (!should_write_file(uiHeaderFileName, obj_get_file(module), genLog,
                               FALSE, check_dates)) {
                ...
                iRC = abmfP_write_ui_header_file(
                    genCodeInfo, module, uiHeaderFileName, uiHeaderDefineName);
                return_if_err(iRC, iRC);

                iRC = check_and_replace_file(
                    curFileName, genLog, codeFile, check_dates, FALSE,
                    &headerFileChanged, &curFileSkipReason);
                return_if_err(iRC, iRC);
        ...
        curFileSkipReason = ABMF_SKIP_UNDEF;
        if (!should_write_file(uiCFileName, obj_get_file(module), genLog, FALSE,
                               check_dates)) {
                curFileSkipReason = ABMF_SKIP_UP_TO_DATE;
        } else {
                ...
                if (!util_be_silent()) {
                        if ((genCodeInfo->resource_file != NULL) &&
                            (resFileName != NULL)) {
                                char *prog_name_string =
                                    util_get_program_name();
                                fprintf(stderr,
                                        catgets(Dtb_project_catd, 1, 57,
                                                "%s: writing resources for %s "
                                                "into %s\n"),
                                        prog_name_string, uiCFileName,
                                        resFileName);
                        }
                }
                if ((errmsg = abio_open_output(NULL, &codeFile)) != NULL) {
                ...
                iRC = check_and_replace_file(
                    curFileName, genLog, codeFile, check_dates, FALSE,
                    &uiCFileChanged, &curFileSkipReason);
                return_if_err(iRC, iRC);
        ...
        if (!should_write_file(stubsFileName, obj_get_file(module), genLog,
                ...
                if (curFileSkipReason == ABMF_SKIP_UNDEF) {
                        ...
                        errmsg =
                            abio_open_output(NULL, &codeFile); /* tmp file */
                        if (errmsg != NULL) {
                                util_printf_err("%s\n", errmsg);
                                return_code(ERR_OPEN);
                        }
                        ...
                        iRC = abmfP_write_stubs_c_file(genCodeInfo,
                                                       stubsFileName, module);
                        return_if_err(iRC, iRC);
                        ...
                        iRC = check_and_merge_c_file(
                            needStubsMerge, &codeFile, curFileName,
                            &curFileSkipReason, &deltaFile);
                        return_if_err(iRC, iRC);
                         ...
                         */
                        iRC = check_and_replace_file(
                            curFileName, genLog, codeFile, check_dates,
                            needStubsMerge, &stubsCFileChanged,
                            &curFileSkipReason);
                        return_if_err(iRC, iRC);

                        if ((stubsCFileChanged) && (deltaFile != NULL)) {
                                char deltaFileName[MAXPATHLEN];
                                sprintf(deltaFileName, "%s.delta",
                                        stubsFileName);
...
#define codeFile (genCodeInfo->code_file)
        ...
        strcpy(headerDefineName,
               abmfP_get_define_from_file_name(headerFileName));
        ...
        if (!should_write_file(headerFileName, obj_get_file(project), genLog,
                ...
                iRC = abmfP_write_project_header_file(genCodeInfo, project,
                                                      headerFileName);
                return_if_err(iRC, iRC);
                 ...
                 * Merge the file
                 */
                iRC = check_and_merge_c_file(needHeaderMerge, &codeFile,
                                             curFileName, &curFileSkipReason,
                                             &deltaFile);
                return_if_err(iRC, iRC);
                 ...
                 */
                iRC = check_and_replace_file(
                    curFileName, genLog, codeFile, check_dates, needHeaderMerge,
                    &headerFileChanged, &curFileSkipReason);
                return_if_err(iRC, iRC);
        } /* compare_file_times */
        if (curFileSkipReason != ABMF_SKIP_UNDEF) {
        ...
        if (!(force_write_c_file ||
              should_write_file(stubsFileName, obj_get_file(project), genLog,
                                merge_files, check_dates))) {
                curFileSkipReason = ABMF_SKIP_UP_TO_DATE;
        } else {
                if (util_be_verbose()) {
                ...
                iRC = abmfP_write_project_c_file(genCodeInfo, stubsFileName,
                                                 needStubsMerge, project);
                return_if_err(iRC, iRC);

                /*
                 * Merge the file
                 */
                iRC = check_and_merge_c_file(needStubsMerge, &codeFile,
                                             curFileName, &curFileSkipReason,
                                             &deltaFile);
                return_if_err(iRC, iRC);
                 ...
                 */
                iRC = check_and_replace_file(
                    curFileName, genLog, codeFile, check_dates, needStubsMerge,
                    &stubsCFileChanged, &curFileSkipReason);
                return_if_err(iRC, iRC);
...
#define makeFile (genCodeInfo->code_file)
        ...
        for (i = intOSMin; i <= intOSMax; ++i) {
                ...
                sprintf(curMakeFileName, "%s.%s", actualMakeFileName,
                        curOSTypeString);

                if (util_paths_are_same_file(curMakeFileName,
                                             actualMakeFileName)) {
                ...
                if (writeCurMakeFile) {
                        ...
                        if ((errmsg = abio_open_output(curMakeFileName,
                                                       &makeFile)) != NULL) {
                                util_printf_err("%s\n", errmsg);
                                goto epilogue;
                        }
                        abmfP_write_make_file(genCodeInfo, project, curOSType,
                                              FALSE);
        ...
        if (actualOSType == AB_OS_UNDEF) {
                util_printf_err(
                    catgets(Dtb_project_catd, 1, 60,
                            "Could not determine OS type of this machine\n"));
                goto epilogue;
        }

        sprintf(curMakeFileName, "%s.%s", actualMakeFileName,
                util_os_type_to_ident(actualOSType));
...
#define codeFile (genCodeInfo->code_file)
         ...
         * Replace the old version with the new
         */
        iRC = check_and_replace_file(curFileName, genLog, codeFile, check_dates,
                                     FALSE, &headerFileChanged,
                                     &curFileSkipReason);
        return_if_err(iRC, iRC);
        if (curFileSkipReason != ABMF_SKIP_UNDEF) {
                print_skipping_message(curFileName, curFileSkipReason);
        ...
        curFileSkipReason = ABMF_SKIP_UNDEF;
        {
                ...
                iRC =
                    abmfP_write_utils_c_file(genCodeInfo, curFileName, project);
                return_if_err(iRC, iRC);

                /*
                 * Replace the old version with the new
                 */
                iRC = check_and_replace_file(curFileName, genLog, codeFile,
                                             check_dates, FALSE, &CFileChanged,
                                             &curFileSkipReason);
                return_if_err(iRC, iRC);
...
static int write_app_resource_file(GenCodeInfo genCodeInfo, GenLog genLog,
                                   ABObj project, BOOL merge_files,
                                   BOOL check_dates, BOOL *fileChangedOut) {
        int return_value = 0;
        int rc = 0; /* return code */
        ...
        *appResFileName = 0;
        ...
        abio_printf(newResFile, "! All CDE applications should include the "
        ...
        if (rc < 0) {
                ...
                if (!util_be_silent()) {
                        /*
                         * We don't want to merge this file. Its pointless,
                         * since it's incomplete, and it may make us overwrite
                         * an existing complete resource file.
                         */
        ...
        if (merge_files) {
                ...
                if (oldResFile != NULL) {
                        ...
                        abmfP_res_file_merge(newResFile, oldResFile,
                                             &mergedResFile);
                        assert(rc >= 0);
         ...
         */
        rc = check_and_replace_file(appResFileName, genLog, newResFile,
                                    check_dates, merge_files, fileChangedOut,
                                    &curFileSkipReason);
        return_if_err(rc, rc);
        if (curFileSkipReason != ABMF_SKIP_UNDEF) {
                print_skipping_message(curFileName, curFileSkipReason);
...
static int write_msg_file(GenCodeInfo genCodeInfo, GenLog genLog, ABObj project,
                          BOOL check_for_changes, BOOL *msgFileChangedOutPtr) {
        int return_value = 0;
        int rc = 0; /* return code */
        ...
        if (genCodeInfo->i18n_method == ABMF_I18N_XPG4_API) {
                ...
                rc = check_and_replace_file(
                    curFileName, genLog, newMsgFile, check_for_changes, TRUE,
                    msgFileChangedOutPtr, &msgFileSkipReason);
                return_if_err(rc, rc);
        }
 ...
 */
static int should_write_file(STRING output_file, STRING input_file,
                             GenLog genLog, BOOL merge_files,
                             BOOL check_times) {
         ...
         * that should cause the file to be regenerated. Unfortunately, it's
         gotten
...
static int check_and_merge_c_file(BOOL needMerge, File *codeFileInOut,
        ...
        if (debugging()) {
                deltaFilePtr =
                    &deltaFile; /* delta file is primarily debugging */
        }
#endif /* DEBUG */

        *deltaFileOut = NULL;

        if (((*fileSkipReasonInOut) != ABMF_SKIP_UNDEF) || (!needMerge)) {
                return 0;
        }
        ...
        rewind(codeFile);
        rewind(orgFile);
        rc = abmfP_merge_c_files(codeFile, "new file", orgFile, codeFileName,
...
#define fileSkippedReasonOut (*fileSkippedReasonOutPtr)
        ...
        if (check_for_changes && util_file_exists(fileName)) {
                ...
                if (orgFile == NULL) {
                        ...
                        util_printf_err(
                            catgets(Dtb_project_catd, 1, 65, "%s: %s\n"),
        ...
        if (fileChanged) {
                ...
                if (util_file_exists(fileName)) {
                        ...
                        if (file == NULL) {
                                fileSkippedReasonOut = ABMF_SKIP_ERR_OCCURRED;
                                util_printf_err(catgets(Dtb_project_catd, 1, 65,
                                                        "%s: %s\n"),
                                                fileName, strerror(errno));
                                return_code(ERR_OPEN);
                        }
...
static int move_file(STRING fileName, STRING newFileName, BOOL force) {
        int rc = 0;
...
static int print_backing_up_message(STRING fileFromName, STRING fileToName) {
        if (util_be_verbose()) {
...
static int compare_file_times(STRING input_file, STRING output_file,
        ...
        if (logEntry == NULL) {
                /* we don't have any record of output file, so input file is
                 * newer */
                return 1;
        }
 ...
 * write our own.
 */
static time_t mkgmtime(struct tm *localTimeStruct) {
...
static int log_construct(GenLog log) {
        log->log_file = NULL;
        log->num_entries = 0;
        log->entries_size = 0;
        log->entries = NULL;
...
static int log_destruct(GenLog log) {
        logP_write(log);
        logP_release_data(log);
        return 0;
}

static int logP_release_data(GenLog log) {
        int i;
...
static GenLogEntry log_find_entry_by_name(GenLog log, STRING file_name) {
 ...
 * time.
 */
static int log_add_entry(GenLog log, STRING file_name, time_t mod_time,
                         BOOL merged) {
        int return_value = 0;
        ...
        new_entries = (GenLogEntry)realloc(
            log->entries, new_entries_size * sizeof(GenLogEntryRec));
...
static int log_dump(GenLog log) {
#ifndef DEBUG
        return 0;
#else
        int i = 0;
        ...
        for (i = 0; i < log->num_entries; ++i) {
                ...
                util_dprintf(0, "%s %ld merged:%d\n",
                             istr_string_safe(entry->file_name),
                             (long)(entry->mod_time), entry->merged);
        }
        util_dprintf(0, "===================\n");
        return 0;
#endif /* DEBUG */
...
static int logP_read(GenLog log) {
        int return_value = 0;
        ...
        log->entries = (GenLogEntry)util_malloc(log->entries_size *
                                                sizeof(GenLogEntryRec));
        if (log->entries == NULL) {
                log->entries_size = 0;
                return_code(ERR_NO_MEMORY);
        }
...
static int logP_write(GenLog log) {
        int return_value = 0;
        ...
        for (i = 0; i < log->num_entries; ++i) {
                ...
                fprintf(file, "%s %04d.%02d.%02d.%02d.%02d.%02d", file_name,
