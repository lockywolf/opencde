...
static int write_all_obj_clear_procs(GenCodeInfo genCodeInfo, ABObj rootObj);
static int write_all_obj_create_procs(GenCodeInfo genCodeInfo, ABObj rootObj);
static int write_call_create_proc(GenCodeInfo genCodeInfo, ABObj obj);
static int write_manage_widgets(GenCodeInfo genCodeInfo, ABObj obj);
static int write_manage_one_tree(GenCodeInfo genCodeInfo, ABObj obj);
static int write_manage_descendants(GenCodeInfo genCodeInfo, ABObj obj);
...
static int write_action_target_includes(GenCodeInfo genCodeInfo, ABObj module);

static int write_call_all_user_post_create_procs(GenCodeInfo genCodeInfo,
                                                 ABObj obj);
static int write_call_user_post_create_procs(GenCodeInfo genCodeInfo,
                                             ABObj obj);
static int write_msg_init_proc(GenCodeInfo genCodeInfo, ABObj parent);
static int write_msg_clear_proc(GenCodeInfo genCodeInfo, ABObj module);

static int abmfP_obj_get_num_conns_by_when(ABObj obj, AB_WHEN when);

static ABObj abmfP_obj_get_conn_by_when(ABObj obj, AB_WHEN when,
                                        int which_conn);

static StringList abmfP_get_msg_action_list(ABObj msg_obj);

static STRING strip_spaces_and_dots(char *mname) {
 ...
 */
static int write_all_shared_var_decls(GenCodeInfo genCodeInfo, ABObj module) {
...
static int write_call_create_proc(GenCodeInfo genCodeInfo, ABObj obj) {
         ...
         */
        {
                ...
                for (submenu_num = 0; submenu_num < num_submenus;
                     ++submenu_num) {
                        submenuObj =
                            abmfP_get_cp_submenu_param_obj(obj, submenu_num);
                        if (submenuObj == NULL) {
                                break;
                        }
                        abio_puts(codeFile, ",\n");
...
static int write_all_call_create_procs(GenCodeInfo genCodeInfo, ABObj root) {
        AB_TRAVERSAL trav;
        ABObj obj;
        int inst = 0;
...
static int write_add_widget_ref_resources(GenCodeInfo genCodeInfo, ABObj root) {
        ...
        for (trav_open(&trav, root, AB_TRAV_UI | AB_TRAV_MOD_PARENTS_FIRST);
             (obj = trav_next(&trav)) != NULL;) {
                ...
                if (abmfP_get_num_args_of_classes(
                        obj, ABMF_ARGCLASS_WIDGET_REF) < 1) {
                        continue;
                }
                ...
                abmfP_obj_spew_args(genCodeInfo, obj, ABMF_ARGCLASS_WIDGET_REF,
                                    ABMF_ARGFMT_VA_LIST);
...
static int write_manage_widgets(GenCodeInfo genCodeInfo, ABObj tree) {
        AB_TRAVERSAL trav;
        ABObj obj = NULL;
...
static int write_manage_one_tree(GenCodeInfo genCodeInfo, ABObj tree) {
...
static int write_manage_descendants(GenCodeInfo genCodeInfo, ABObj obj) {
        ...
        if ((!(ObjWClassIsDialogShell(obj) &&
            ...
            (ObjWClassIsCompositeSubclass(obj)) && (abmfP_obj_has_field(obj)) &&
            (obj_has_child(obj))) {
                if (abmfP_obj_has_unmanaged_child(obj) ||
                    ObjWClassIsCommand(obj) ||
                    ObjWClassIsFileSelectionBox(obj) ||
                    ObjWClassIsMainWindow(obj) || ObjWClassIsPanedWindow(obj) ||
                    ObjWClassIsScrolledWindow(obj)) {
                        /* This object has one or more unmanaged children.
                         * Manage each child individually.
                         *
                         * Or, the widget class has a complex private structure
                         * (e.g., ScrolledWindow), and we can't rely on
                         * XmNchildren.
                         */
                        ...
                        for (trav_open(&unmanagedTrav, obj, AB_TRAV_CHILDREN);
                             (child = trav_next(&unmanagedTrav)) != NULL;) {
                                if (obj_should_be_managed_when_initialized(
                                        child)) {
                        ...
                        abio_puts(genCodeInfo->code_file,
                                  "XmNchildren, &children, XmNnumChildren, "
                                  "&numChildren, NULL);\n");
...
static int write_out_mainwin_set_areas(GenCodeInfo genCodeInfo, ABObj obj) {
        ...
        if (menubar || toolbar) {
                ...
                abio_printf(genCodeInfo->code_file, "%s,\n",
                            menubar ? abmfP_get_c_name(genCodeInfo, menubar)
                                    : "NULL");

                abio_printf(genCodeInfo->code_file, "%s,\n",
                            toolbar ? abmfP_get_c_name(genCodeInfo, toolbar)
                                    : "NULL");
                ...
                abio_printf(genCodeInfo->code_file, "%s);\n",
                            workarea ? abmfP_get_c_name(genCodeInfo, workarea)
                                     : "NULL");
        ...
        if (footer) {
                ...
                abio_printf(genCodeInfo->code_file, "XmNmessageWindow, %s,\n",
...
static int write_set_fixed_pane(GenCodeInfo genCodeInfo, ABObj pane) {
        ...
        abio_printf(genCodeInfo->code_file, "XmNpaneMinimum,   pane_height,\n");
        abio_printf(genCodeInfo->code_file, "XmNpaneMaximum,   pane_height,\n");
        abio_printf(genCodeInfo->code_file, "NULL);\n\n");
...
static int write_out_dialog_set_pane_height(GenCodeInfo genCodeInfo,
                                            ABObj obj) {
...
static BOOL abmfP_obj_has_unmanaged_child(ABObj obj) {
        ...
        for (trav_open(&trav, obj, AB_TRAV_CHILDREN);
             (child = trav_next(&trav)) != NULL;) {
                /* menus don't show up in XmNchildren list, so we need to ignore
                 * them
                 */
                if (abmfP_obj_has_field(child) && (!obj_is_menu(child)) &&
 ...
 */
static int write_obj_clear_proc(GenCodeInfo genCodeInfo, ABObj obj) {
        File file = genCodeInfo->code_file;
        abmfP_write_clear_proc_begin(genCodeInfo, obj);
        abio_printf(file, "memset((void *)(%s), 0, sizeof(*%s));\n",
                    abmfP_instance_ptr_var_name, abmfP_instance_ptr_var_name);
 ...
 */
static int write_obj_init_proc(GenCodeInfo genCodeInfo, ABObj parent) {
        int return_value = 0;
        File codeFile = genCodeInfo->code_file;
        ABObj obj;
             ...
             (obj = trav_next(&trav)) != NULL;) {
                if (abmfP_obj_needs_align_handler(obj)) {
                        ...
                        abio_printf(genCodeInfo->code_file,
                                    ...
                                    abmfP_obj_get_col_align_type(obj), 0,
                                    obj_get_num_rows(obj),
                                    obj_get_num_columns(obj),
                                    obj_get_hoffset(obj), obj_get_voffset(obj));
                }
        }
        trav_close(&trav);
        ...
        if (obj_is_popup_win(parent)) {
                ABObj pwObj =
                    objxm_comp_get_subobj(parent, AB_CFG_WINDOW_PW_OBJ);

                if (pwObj) {
                        abmfP_write_c_comment(genCodeInfo, FALSE,
                                              "Turn off traversal for "
                                              "invisible sash in dialog's "
                                              "PanedWindow");
                        abio_printf(codeFile, "%s(%s);\n",
        ...
        for (trav_open(&trav, parent, AB_TRAV_UI);
             (obj = trav_next(&trav)) != NULL;) {
                if (((abmfP_get_window_parent(obj) == parent) ||
                     (obj == parent)) &&
                    (abmfP_write_add_callbacks_and_actions(genCodeInfo, obj) <
                     0)) {
                        return_code(-1);
                }
        }
...
static int write_all_obj_init_procs(GenCodeInfo genCodeInfo, ABObj module) {
        ABObj obj;
        AB_TRAVERSAL trav;
...
static int write_create_proc_decls(GenCodeInfo genCodeInfo, ABObj module) {
        File codeFile = genCodeInfo->code_file;
        AB_TRAVERSAL trav;
        ABObj obj;
        char *parent = NULL;
...
static void set_parent_for_submenu(ABObj obj) {
        ABObj child = NULL;
...
static int item_has_children(ABObj obj) {
        return (obj_is_menu_item(obj) && obj_has_child(obj));
}

static void check_for_submenus(ABObj obj) {
        AB_TRAVERSAL trav;
        ABObj child;
...
static void assign_parent_to_submenus(ABObj project) {
        AB_TRAVERSAL trav;
        ABObj obj;
...
int write_obj_create_proc(GenCodeInfo genCodeInfo, ABObj obj) {
        File codeFile = genCodeInfo->code_file;
        ABObj top_obj = NULL;
        ...
        if (abmfP_obj_create_proc_has_parent_param(obj)) {
                abmfP_parent_param(genCodeInfo) =
                    istr_const(abmfP_parent_param_name);
                abmfP_parent_param_has_value(genCodeInfo) = TRUE;
        }
...
static int write_call_all_user_post_create_procs(GenCodeInfo genCodeInfo,
        ...
        for (trav_open(&itemTrav, obj, AB_TRAV_ITEMS_FOR_OBJ);
             (item = trav_next(&itemTrav)) != NULL;) {
                total_post_create_procs += abmfP_obj_get_num_conns_by_when(
                    item, AB_WHEN_AFTER_CREATED);
        }
...
static int write_call_user_post_create_procs(GenCodeInfo genCodeInfo,
        ...
        ABObj sourceObj =
            objxm_comp_get_source_for_when(compObj, AB_WHEN_AFTER_CREATED);
        ...
        *sourceName = 0;
        ...
        if (num_post_create_procs > 0) {
                ...
                for (i = 0; i < num_post_create_procs; ++i) {
                         ...
                         * source widget
                         */
                        util_strncpy(sourceName,
                                     abmfP_get_c_name(genCodeInfo, sourceObj),
                                     MAX_NAME_SIZE);
                        ...
                        if (targetObj == NULL) {
                                ...
                                util_strncpy(targetStructPtrName,
                                             abmfP_get_c_struct_ptr_name(
                                                 genCodeInfo, targetObj),
                                             MAX_NAME_SIZE);
                        }

                        abio_printf(genCodeInfo->code_file,
                                    "%s(%s, (XtPointer)%s, (XtPointer)%s);\n",
                                    abmfP_get_action_name(action), sourceName,
                                    targetStructPtrName,
                                    abmfP_get_c_struct_ptr_name(genCodeInfo,
                                                                sourceObj));
                }
        }
        return return_value;
 ...
 *
 */
static int write_includes(GenCodeInfo genCodeInfo, ABObj module) {
        File codeFile = genCodeInfo->code_file;
        int num_written = 0;
        AB_TRAVERSAL trav;
        ABObj obj = NULL;
                ...
                for (trav_open(&trav, module, AB_TRAV_SALIENT_UI);
                     (obj = trav_next(&trav)) != NULL;) {
                        ...
                        if (abmfP_obj_has_glyph_label(obj)) {
                                ...
                                if (!strlist_str_exists(glyphIncludes,
                                                        glyphFile)) {
                                        strlist_add_str(glyphIncludes,
                                                        glyphFile, NULL);
                                        abmfP_write_c_local_include(genCodeInfo,
                                                                    glyphFile);
                                }
                ...
                if ((fromModule != NULL) && (fromModule == module) &&
                    (toModule != NULL) && (toModule != module)) {
                        ...
                        if (!strlist_str_exists(&targetIncludes, headerFile)) {
                                strlist_add_str(&targetIncludes, headerFile,
                                                NULL);
                                abmfP_write_c_local_include(genCodeInfo,
                                                            headerFile);
                        }
 ...
 */
int abmfP_write_ui_c_file(GenCodeInfo genCodeInfo, STRING codeFileName,
                          ABObj module) {
        File codeFile = genCodeInfo->code_file;
        ...
        *moduleName = 0;
        ...
        abmfP_write_file_header(
            genCodeInfo, codeFileName, FALSE, moduleName,
            util_get_program_name(), ABMF_MODIFY_NOT,
...
static BOOL action_from_pred(ABObj obj) {
...
static int write_msg_init_proc(GenCodeInfo genCodeInfo, ABObj parent) {
        ...
        STRING okCB = (STRING)NULL, action2CB = (STRING)NULL,
               action3CB = (STRING)NULL, cancelCB = (STRING)NULL;
        AB_WHEN when = AB_WHEN_UNDEF;
        int i = 0;
        STRING help_text = (STRING)NULL, help_volume = (STRING)NULL,
        ...
        if (!util_strempty(str)) {
                if (genCodeInfo->i18n_method == ABMF_I18N_XPG4_API) {
                        i18n_str =
                            abmfP_catgets_prefix_str(genCodeInfo, parent, str);
                        abio_printf(codeFile, "XmStringCreateLocalized(%s",
                                    i18n_str);
        ...
        if (callback_funcs != NULL) {
                for (i = 0; i < strlist_get_num_strs(callback_funcs); i++) {
                        callback_func =
                            strlist_get_str(callback_funcs, i, (void **)&when);
                        switch (when) {
        ...
        abio_printf(codeFile,
                    "%s->action1_label = ", abmfP_instance_ptr_var_name);
        ...
        abio_printf(codeFile,
                    "%s->action1_callback = ", abmfP_instance_ptr_var_name);
        abio_printf(codeFile, "%s;\n", okCB ? okCB : "(XtCallbackProc) NULL");

        /* Set the Action2 button label (Extra button) */
        abio_printf(codeFile,
                    "%s->action2_label = ", abmfP_instance_ptr_var_name);
        ...
        abio_printf(codeFile,
                    "%s->action2_callback = ", abmfP_instance_ptr_var_name);
        abio_printf(codeFile, "%s;\n",
                    action2CB ? action2CB : "(XtCallbackProc) NULL");

        /* Set the Action3 button label (Extra button) */
        abio_printf(codeFile,
                    "%s->action3_label = ", abmfP_instance_ptr_var_name);
        ...
        abio_printf(codeFile,
                    "%s->default_button = ", abmfP_instance_ptr_var_name);
        abio_printf(codeFile, "%s;\n", default_btn);
