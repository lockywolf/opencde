...
int abmfP_ui_header_file_init(void) {
...
int abmfP_comp_get_widget_specific_includes(StringList includesList,
                                            ABObj obj) {
        int return_value = 0;
        ...
        for (trav_open(&trav, obj, AB_TRAV_COMP_SUBOBJS);
             (subobj = trav_next(&trav)) != NULL;) {
                rc = abmfP_obj_get_widget_specific_includes(includesList,
                                                            subobj);
                return_if_err(rc, rc);
        }
        trav_close(&trav);
 ...
 */
int abmfP_obj_get_widget_specific_includes(StringList includesList, ABObj obj) {
        int return_value = 0;
        int num_includes = 0;
 ...
 */
int abmfP_tree_write_widget_specific_includes(File codeFile, ABObj module) {
        int return_value = 0;
        int num_includes = 0;
        AB_TRAVERSAL trav;
        ABObj obj = NULL;
...
static int write_struct_def(GenCodeInfo genCodeInfo, ABObj structObj) {
         ...
         */
        for (trav_open(&uiTrav, structObj,
                       AB_TRAV_UI | AB_TRAV_MOD_PARENTS_FIRST);
             (fieldObj = trav_next(&uiTrav)) != NULL;) {
                /* ref objs do not have their own type (they take the menu's
                 * type */
        ...
        for (trav_open(&salientTrav, structObj,
                       AB_TRAV_UI | AB_TRAV_MOD_PARENTS_FIRST);
             (salientObj = trav_next(&salientTrav)) != NULL;
             lastNumFields = numFields, lastSeparateObj = separateObj) {
                if (!(obj_is_salient(salientObj) ||
                      obj_is_menu_ref(salientObj))) {
                        continue;
                }
                item = obj_get_item(salientObj, 0);
                objHasItems = ((item != NULL) && !obj_is_list_item(item) &&
                               !obj_is_combo_box_item(item) &&
                               !obj_is_spin_box_item(item));
                numFields = obj_get_num_comp_subobjs(salientObj) +
                            (objHasItems ? 1 : 0);
                thisFieldNum = -1;
                     ...
                     (fieldObj = trav_next(&uiTrav)) != NULL; ++thisFieldNum) {
                        ...
                        if ((typeName != NULL) && (fieldName != NULL)) {
                                ...
                                if (obj_is_menu_ref(fieldObj)) {
                                        /* be sure we don't write the items,
                                         * since they're */
                                        /* in the substructure */
                                        abmfP_obj_set_items_written(fieldObj,
                                                                    TRUE);
                                }
                                abio_printf(codeFile, "%s\t%s;", typeName,
                                            fieldName);
                                if (separateObj && (thisFieldNum == 0)) {
                                        STRING userObjName = NULL;
                                        if (obj_is_ref(salientObj)) {
                                                userObjName = obj_get_name(
                                                    obj_get_actual_obj(
                                                        salientObj));
                                        } else {
                                                userObjName =
                                                    obj_get_name(salientObj);
                                        }
                                        abio_puts(codeFile, "\t");
 ...
 */
static int write_all_struct_defs(GenCodeInfo genCodeInfo, ABObj module) {
        AB_TRAVERSAL trav;
        ...
        for (trav_open(&trav, module, AB_TRAV_MENUS);
             (menuObj = trav_next(&trav)) != NULL;) {
                ...
                if ((!obj_is_ref(menuObj)) &&
                    (abmfP_obj_is_substruct_obj(menuObj))) {
                        ++numWritten;
 ...
 */
static int write_shared_var_decl(GenCodeInfo genCodeInfo, ABObj structObj) {
        File codeFile = genCodeInfo->code_file;
 ...
 */
static int write_action_decls(GenCodeInfo genCodeInfo, ABObj module,
                              BOOL auto_named) {
        AB_TRAVERSAL trav;
        ABObj action = NULL;
 ...
 */
static int write_clear_proc_decls(GenCodeInfo genCodeInfo, ABObj module) {
        AB_TRAVERSAL trav;
        ABObj obj = NULL;
 ...
 */
static int write_init_proc_decls(GenCodeInfo genCodeInfo, ABObj module) {
        AB_TRAVERSAL trav;
        ABObj obj = NULL;
 ...
 */
static STRING remove_dots(STRING s) {
 ...
 */
int abmfP_write_ui_header_file(GenCodeInfo genCodeInfo, ABObj module,
                               STRING headerName, STRING headerDefine) {
        File codeFile = genCodeInfo->code_file;
        ...
        *moduleName = 0;
        ...
        abmfP_write_file_header(genCodeInfo, headerName, TRUE, moduleName,
                                util_get_program_name(), ABMF_MODIFY_NOT,
                                " * Contains: Declarations of module objects, "
                                "user create procedures,\n"
                                " *           and callbacks.");
...
int abmfP_write_action_func_decl(GenCodeInfo genCodeInfo, ABObj action) {
        ...
        case AB_WHEN_TOOLTALK_PAUSE_RESUME:
                abmfP_write_tooltalk_callback_decl(genCodeInfo, FALSE,
                                                   funcName);
                break;

        case AB_WHEN_SESSION_SAVE:
                abmfP_write_session_save_callback_decl(genCodeInfo, FALSE,
                                                       funcName);
                break;
        case AB_WHEN_SESSION_RESTORE:
                abmfP_write_session_restore_callback_decl(genCodeInfo, FALSE,
                                                          funcName);
                break;
