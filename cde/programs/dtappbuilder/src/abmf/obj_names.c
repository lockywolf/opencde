...
static STRING abmfP_build_instance_prefix(ABObj obj, STRING prefixBuf,
                                          int prefixBufSize);
...
#define return_c_ident(ident) return (ident)
        ...
        *fieldNameBuf = 0;
        ...
        substructObj = abmfP_obj_get_substruct_obj(obj);
        {
                ...
                if ((substructObj != NULL) && (fieldNameObj != obj)) {
                        abmfP_get_c_field_name(
                            fieldNameObj); /* we set field_name */
        ...
        if ((substructObj != NULL) && (compRootName != NULL)) {
                ...
                if ((fieldnamelen > subnamelen) &&
                    (strncmp(fieldNameBuf, compRootName, subnamelen) == 0)) {
                        ...
                        if (fieldnamelen > 0) {
                                memmove(fieldNameBuf, fieldNameBuf + subnamelen,
                                        fieldnamelen + 1);
                        }
...
abmfP_get_c_name_in_subinst(ABObj obj) {
        ...
        sprintf(name, "%s->%s", abmfP_instance_ptr_var_name, fieldName);
        return name;
}
...
abmfP_get_c_name(GenCodeInfo genCodeInfo, ABObj obj) {
        ...
        if (genCodeInfo->writing_func) {
                ...
                if (ipObj == NULL) {
                        ...
                        if (createObj != NULL) {
                                /* we're in a create proc.  Create procs take
                                 * struct ptr */
                                ...
                                if (abmfP_objs_in_same_struct(ipObj, obj)) {
                                        /* we can get the variable from the
                                         * instance pointer */
                                        name = abmfP_get_c_name_in_inst(obj);
                                }
                        }
 ...
 *
 */
static STRING abmfP_get_c_struct_or_ptr_name(GenCodeInfo genCodeInfo, ABObj obj,
                                             BOOL wantPtr) {
        static char nameBuf[MAX_NAME_SIZE] = "";
        STRING name = NULL;
...
abmfP_get_c_substruct_name(GenCodeInfo genCodeInfo, ABObj obj) {
        ...
        if (genCodeInfo->writing_func) {
                ...
                ipObjIsSubstruct =
                    ((createObj != NULL) && (createObj == ipObj));
        }
        ...
        *nameBuf = 0;
        ...
        if (ObjWClassIsShellSubclass(obj) ||
            ObjWClassIsShellSubclass(abmfP_parent(obj))) {
                sprintf(nameBuf, "dtb_%s_%s", obj_get_name(obj_get_module(obj)),
                ...
                ABObj objectObj =
                    objxm_comp_get_subobj(compRoot, AB_CFG_OBJECT_OBJ);
...
abmfP_get_widget_name_for_res_file(ABObj obj) {
        ...
        *name = 0;
        ...
        if (mfobj_has_flags(obj, CGenFlagTreatAsAppShell)) {
                 ...
                 * from XtVaAppInitialize(). To actually set a resource on just
                 * that widget, the class name (only!) is used in the resource
                 * file.
                 */
                /* name=Class */
                assert(obj_is_base_win(obj));
                util_strncpy(name, abmfP_get_app_class_name(obj),
                             MAX_NAME_SIZE);
        } else if ((ObjWClassIsShellSubclass(obj)) ||
                   (ObjWClassIsShellSubclass(abmfP_parent(obj)))) {
                /* name=Class*widget */
                sprintf(name, "%s*%s", abmfP_get_app_class_name(obj),
...
static int ensure_unique_comp_field_names(ABObj obj) {
        ...
        if ((objectObj != NULL) && (objectObj != compRoot)) {
                util_dprintf(2, "changing field name %s -> %s\n",
                             util_strsafe(obj_get_name(objectObj)),
                             compRootName);
        ...
        if ((objectObjRenamed) && (compRoot != objectObj)) {
                ...
                }
                {
                        ...
                        *newObjName = 0;
                        ...
                        util_dprintf(2, "changing field name %s -> %s\n",
                                     util_strsafe(obj_get_name(compRoot)),
                                     newObjName);
                        mfobj_data(compRoot)->field_name =
                            istr_create(newObjName);
                }
        } /* compRoot != objectObj */
...
static STRING cvt_ident_to_type(STRING ident) {
        ...
        for (nextCharOff = 0; nextCharOff < identLen; ++nextCharOff) {
                ...
                if (nextChar == '_') {
                        ...
                        ident[prevCharOff++] =
                            capitalizeNext ? toupper(nextChar) : nextChar;
