 ...
 */
STRING
abmfP_boolean_string(int i) { return i ? "True" : "False"; }

/*
 * Check a string to see if contains a legal identifier.
 */
int abmfP_identifier_ok(STRING s) { return ab_c_ident_is_ok(s); }
...
BOOL abmfP_items_are_homogeneous(ABObj obj) {
        ...
        if (item != NULL) {
                ...
                for (; (item = trav_next(&trav)) != NULL;)
                        if (util_strcmp(ref_classname,
                                        obj_get_class_name(item)) != 0) {
                                same = FALSE;
                                break;
                        }
        }
 ...
 */
int abmfP_obj_is_duplicate(ABObj obj, ABObj dupObj) { return (obj == dupObj); }

BOOL abmfP_parent_is_class(ABObj obj, STRING classname) {
        if ((abmfP_parent(obj) == NULL) || (obj_get_class_name(obj) == NULL) ||
 ...
 */
ABObj abmfP_get_actual_control(ABObj parent) {
 ...
 */
ABObj abmfP_get_whole_object(ABObj obj) { return obj_get_root(obj); }
 ...
 */
ABObj abmfP_find_callback_scope(ABObj obj, STRING callbackName) {
        ABObj scopeObj = NULL;
        ...
        if ((scopeObj == NULL) && (objProject != NULL)) {
                for (trav_open(&trav, objProject, AB_TRAV_MODULES);
                     (scopeObj == NULL) &&
                     ((curModule = trav_next(&trav)) != NULL);) {
                        if (mfobj_get_module_data(curModule) != NULL) {
                                callbackList =
                                    mfobj_get_module_data(curModule)->callbacks;
                                if (strlist_str_exists(callbackList,
                                                       callbackName)) {
                                        scopeObj = curModule;
                                }
                        }
                ...
                if (scopeObj == NULL) {
                        callbackList =
                            mfobj_get_proj_data(objProject)->callbacks;
                        if (strlist_str_exists(callbackList, callbackName)) {
                                scopeObj = NULL;
                        }
                }
...
int find_longest_label(STRING *label) {
        int len1, len2, longest, i = 0;
 ...
 */
ABObj abmfP_get_window_pane(ABObj obj) {
        if (abmfP_parent(obj) == NULL) {
                return ((ABObj)NULL);
        }
        if (obj_is_top_level(obj))
                return NULL;
        else if (obj_is_pane(abmfP_parent(obj)) ||
                 obj_is_top_level(abmfP_parent(obj)))
                return abmfP_parent(obj);
 ...
 */
int abmfP_assign_widget_names(ABObj root) {
        int iReturn = 0;
        ABObj win_obj;
        static int i = 0;
        char Buf[64];
        ...
        for (trav_open(&trav, root, AB_TRAV_WINDOWS), i = 1;
             (win_obj = trav_next(&trav)) != NULL;) {
                if (obj_get_name(win_obj) == NULL) { /* Only assign once */
...
STRING
abmfP_cvt_file_name_to_bitmap_data_vars(STRING path, STRING *widthVarPtr,
                                        STRING *heightVarPtr,
                                        STRING *bitsVarPtr) {
...
int abmfP_obj_set_subobjs_written(ABObj obj, BOOL written) {
        AB_TRAVERSAL trav;
        ABObj subObj = NULL;

        for (trav_open(&trav, obj, AB_TRAV_ALL);
             (subObj = trav_next(&trav)) != NULL;) {
                if ((subObj == obj) || (obj_get_root(subObj) != obj)) {
                        continue;
                }
...
BOOL abmfP_obj_has_create_proc(ABObj obj) {
        if (obj_is_menu(obj)) {
                 ...
                 */
                return (obj_is_ref(obj) &&
                        (abmfP_obj_get_struct_obj(obj) != NULL));
        }
...
BOOL abmfP_obj_create_proc_has_instance_param(ABObj obj) { return TRUE; }

BOOL abmfP_obj_create_proc_has_name_param(ABObj obj) { return FALSE; }
 ...
 */
BOOL abmfP_obj_create_proc_has_parent_param(ABObj obj) { return TRUE; }

int abmfP_obj_get_num_create_proc_return_widgets(ABObj obj) {
        int numWidgets = 0;
...
BOOL abmfP_obj_create_proc_has_submenu_params(ABObj obj) {
        return FALSE;
        /* return (abmfP_obj_has_menu(obj) ||
         * abmfP_obj_has_item_with_menu(obj)); */
}

int abmfP_get_num_cp_submenu_params(ABObj obj) {
        return 0;
        /*
        AB_TRAVERSAL	trav;
        ABObj		item;
...
ABObj abmfP_get_cp_submenu_param_obj(ABObj obj, int whichParam) {
        int curParam = -1;
        ABObj menu = NULL;
        ABObj paramObj = NULL;
        AB_TRAVERSAL trav;
...
BOOL abmfP_obj_has_menu(ABObj obj) { return (obj_get_menu(obj) != NULL); }

BOOL abmfP_obj_has_item_with_menu(ABObj obj) {
