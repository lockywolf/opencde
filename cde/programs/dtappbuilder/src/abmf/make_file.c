...
static int write_os_params(File makeFile, AB_OS_TYPE osType, AbmfLibs libs);
...
static int write_file_header(GenCodeInfo genCodeInfo, ABObj project,
                             AB_OS_TYPE osType);
static int write_ab_params(GenCodeInfo genCodeInfo, ABObj project);
static int write_targets(GenCodeInfo genCodeInfo, ABObj project,
                         AB_OS_TYPE osType);

static int write_local_libraries(File makeFile, AbmfLibs libs,
                                 AB_OS_TYPE osType);

static int write_codegen_macro(File makeFile);
...
int abmfP_write_make_file(GenCodeInfo genCodeInfo, ABObj project,
        ...
        if (osType == AB_OS_SUNOS) {
                abio_printf(genCodeInfo->code_file,
                            "\n"
                            ".NO_PARALLEL: $(TARGETS.h) $(TARGETS.h.merged) "
                            "$(TARGETS.c) $(TARGETS.c.merged)\n");
        }
...
static int determine_libs(AbmfLibs libs, ABObj project, AB_OS_TYPE osType) {
        ...
        if (projData->has_ui_obj) {
                lib_add(libs, LibDtWidget, ABMF_LIB_APPEND,
                        ABMF_LIB_REJECT_DUP);
...
static int determine_aix_libs(AbmfLibs libs, ABObj project) { return 0; }
...
static int determine_sunos_libs(AbmfLibs libs, ABObj project) { return 0; }

static int determine_uxp_libs(AbmfLibs libs, ABObj project) { return 0; }

static int determine_osf1_libs(AbmfLibs libs, ABObj project) { return 0; }

static int determine_unixware_libs(AbmfLibs libs, ABObj project) { return 0; }

static int determine_lnx_libs(AbmfLibs libs, ABObj project) { return 0; }

static int determine_fbsd_libs(AbmfLibs libs, ABObj project) { return 0; }

static int determine_nbsd_libs(AbmfLibs libs, ABObj project) { return 0; }

static int determine_obsd_libs(AbmfLibs libs, ABObj project) { return 0; }

static STRING get_string_for_lib(AbmfLibDesc lib, AB_OS_TYPE osType) {
...
static int write_file_header(GenCodeInfo genCodeInfo, ABObj project,
        ...
        abio_printf(
            ...
            "##################################################################"
            "#########\n",
            util_get_program_name(), obj_get_file(project), osName);
...
static int write_aix_stuff(File makeFile, AbmfLibs libs) {
        ...
        abio_printf(makeFile,
                    "##########################################################"
                    "#################\n"
                    "# These are the %s-dependent configuration parameters "
                    "that must be\n"
...
static int write_hpux_stuff(File makeFile, AbmfLibs libs) {
        ...
        abio_puts(
            ...
            "        INCLUDES = -I/usr/dt/include -I/X11/include\n"
            "\n"
            "        STD_DEFINES = -DSYSV -DNLS16 -DMALLOC_0_RETURNS_NULL "
            "-DMERGE -DNDEBUG"
...
static int write_uxp_params(File makeFile, AbmfLibs libs) {
        ...
        abio_puts(makeFile,
                  ...
                  "\n"
                  "        CDEBUGFLAGS = -O\n"
                  "        SYS_LIBRARIES = -lnsl -lsocket -lm -ldl -lgen "
                  "-lresolv -lw\n"
                  "        CDE_LIBPATH = /usr/dt/lib\n"
                  "        CDE_LDFLAGS = -L$(CDE_LIBPATH)\n"
                  "        ALLX_LIBPATH = /X11/lib\n"
        ...
        abio_puts(makeFile,
                  "        CFLAGS = $(CDEBUGFLAGS) -D_POSIX_SOURCE=1 "
                  "$(INCLUDES) $(STD_DEFINES)"
                  " $(ANSI_DEFINES)\n"
                  "        LDLIBS = $(SYS_LIBRARIES)\n"
                  "        LDOPTIONS = $(CDE_LDFLAGS) $(ALLX_LDFLAGS)\n"
                  "\n");
...
static int write_osf1_stuff(File makeFile, AbmfLibs libs) {
        ...
        abio_puts(makeFile,
                  ...
                  "        ALLX_LDFLAGS = -L$(ALLX_LIBPATH)\n"
                  "        EXTRA_LOAD_FLAGS = "
                  "-Wl,-rpath,$(CDE_LIBPATH):$(ALLX_LIBPATH)\n");
...
static int write_ab_params(GenCodeInfo genCodeInfo, ABObj project) {
        File makeFile = genCodeInfo->code_file;
        int numModules =
            trav_count_cond(project, AB_TRAV_MODULES, obj_is_defined_module);
        int thisModuleNum = 0;
        AB_TRAVERSAL trav;
        ABObj module = NULL;
        ...
        abio_printf(makeFile,
                    ...
                    "##########################################################"
                    "#################\n"
                    "#  These parameters are generated by %s according to the "
                    "structure\n"
        ...
        for (thisModuleNum = 0, trav_open(&trav, project, AB_TRAV_MODULES);
             (module = trav_next(&trav)) != NULL; ++thisModuleNum) {
...
static BOOL obj_is_module_func(ABObj obj) { return obj_is_module(obj); }
...
static int write_targets(GenCodeInfo genCodeInfo, ABObj project,
        ...
        *modName = 0;
        ...
        if (!(obj_is_default(project) && (firstModuleFileName == NULL))) {
                ...
                abio_printf(makeFile, "\t$(%s) %s -main", CODEGEN_MACRO,
                            projFlag);
                if (obj_is_default(project)) {
                ...
                abio_printf(makeFile,
                            "%s: %s %s $(TARGETS.h) $(TARGETS.h.merged)\n",
                            cvt_to_obj(abmfP_get_project_c_file_name(project)),
                            abmfP_get_project_c_file_name(project),
                            obj_get_file(project));
        }
        ...
        for (trav_open(&moduleTrav, project, AB_TRAV_MODULES);
             (module = trav_next(&moduleTrav)) != NULL;) {
                ...
                abio_printf(makeFile, "%s: %s %s %s\n",
                            cvt_to_obj(abmfP_get_ui_c_file_name(module)),
                            abmfP_get_ui_c_file_name(module),
                            abmfP_get_ui_header_file_name(module), modFile);
                abio_printf(makeFile, "%s: %s %s %s\n",
                            cvt_to_obj(abmfP_get_stubs_c_file_name(module)),
                            abmfP_get_stubs_c_file_name(module),
                            abmfP_get_ui_header_file_name(module), modFile);
        ...
        abio_puts(
            ...
            "	      Makefile.osf1 Makefile.uxp Makefile.unixware "
            "Makefile.linux \\\n"
