...
static int find_func_name(BFile file, long funcOff, long bodyOff,
                          long *nameOffPtr, long *nameLenPtr);
static int find_func_proto(BFile file, long funcOff, long nameOff, long bodyOff,
                           long *protoOff, long *protoLen);

static int trim_opening_comment(BFile file, long *funcOffPtr, long *funcLenPtr,
                                long protoOff);

static int grab_string_from_file(STRING *stringPtr, BFile file, long stringOff,
                                 long stringLen);
static int find_seg(

    BFile file, CSEG_TYPE *csegTypePtr, char *namePtr, long *segOffPtr,
    long *segLenPtr, long *bodyOffPtr, long *bodyLenPtr, CUserSegs userSegs);
...
int abmfP_parse_c_file(FILE *cFile, CSegArray *segmentsOutPtr) {
        int return_value = 0;
        ...
        while ((rc = get_seg(bfile, &seg)) >= 0) {
                ...
                {
                    int		i = 0;
                    util_dprintf(1, "Seg: %s  lines:%d-%d length:%ld
                userSegs:%d\n", seg.type == CSEG_GLOBAL? "[GLOBAL]":
                util_strsafe(seg.name), cvt_offset_to_line(bfile, seg.offset),
                    ...
                    {
                        STRING	string = NULL;
                        util_dprintf(1, "    =>UserSeg: %d-%ld [%ld+%ld
                bytes]\n", cvt_offset_to_line(bfile,
                seg.userSegs.segs[i].offset), cvt_offset_to_line(bfile,
                seg.userSegs.segs[i].offset +
                                        + seg.userSegs.segs[i].length
                                        + (seg.userSegs.segs[i].length<1?
                0:-1)), seg.userSegs.segs[i].offset,
                            seg.userSegs.segs[i].length);
                ...
                */
                ...
                ++numSegs;
                segArray->segs =
                    (CSeg)realloc(segArray->segs, numSegs * sizeof(CSegRec));
        ...
        for (segCount = 0; segCount < segArray->numSegs; ++segCount) {
                ...
                for (userSegCount = 0; userSegCount < curSeg->userSegs.numSegs;
                     ++userSegCount) {
                        ...
                        curUserSeg->line = -1;
                        curUserSeg->line =
                            cvt_offset_to_line(file, curUserSeg->offset);
...
static int find_seg(BFile file, CSEG_TYPE *segTypePtr, char *namePtr,
                    long *segOffPtr, long *segLenPtr, long *bodyOffPtr,
                    long *bodyLenPtr, CUserSegs userSegs) {
        int return_value = 0;
        long segOff = 0;
        long segLen = 0;
                ...
                while ((openingBraceOff < 0) && (!bfile_eof(file))) {
                        ...
                        if (iChar == '{') /* } vi hack */
                        {
                                ...
                                if (userSegs->numSegs > 0) {
                                        /* we found global segs before this
                                         * object */
                                        segType = CSEG_GLOBAL;
                                        bfile_backup(file, 1);
                                        segLen =
                                            bfile_get_off(file) - segOff + 1;
                                        goto epilogue;
                                }
                        }
                ...
                while (lookForIdent) {
                        ...
                        if (iChar == EOF) {
                                ...
                                if (openingParenOff >= 0) {
                                        ...
                                        /* if a ; is seen between () and {}, it
                                         * should be an old-style parameter list
                                         * declaration
                                         */
                                        identLen = 0;
                                }
                                ...
                                /* no ident - look for () */
                                segType = CSEG_FUNC;
                                if (find_matching(file, '(', &openingParenOff,
                                                  NULL)
                                    /* ) vi hack */
                                    >= 0) {
                                        segType = CSEG_FUNC;
                                        ...
                                        util_printf_err(
                                            "Bad function definition at or "
                                            "near line %d\n",
                                            cvt_offset_to_line(
                                                file, openingBraceOff - 2));
                                        lookForIdent = FALSE;
                                        identLen = 0;
                                        return_code(-1);
                                }
                        } else if (iChar == ']') {
                                if (find_matching(file, '[', NULL, NULL) >=
                                    0) /* ] vi hack */
                                {
                                        ...
                                        util_printf_err(
                                            "Unbalanced '[' at or near line "
                                            "%d\n",
                                            cvt_offset_to_line(
                                                file, bfile_get_off(file)));
                                        /* ] vi hack */
                                        lookForIdent = FALSE;
                                        identLen = 0;
                                        return_code(-1);
                ...
                if (identLen < 1) {
                        ...
                        *ident = 0;
                        if (find_ident(file, ident, 1024) < 0) {
                                util_printf_err(
                                    "Syntax error at or near line %d\n",
                ...
                if ((identLen > 0) && (segType == CSEG_UNDEF)) {
                        ...
                        if (util_streq(ident, "struct") ||
                            ...
                            util_streq(ident, "class")) {
                                ...
                                /*
                                 * This could be a variable assignment or an
                                 * aggregate type. The ident is the proper name
                                 * for this one.
                                 */
                                segType = CSEG_AGGREGATE_TYPE;
                        }
...
epilogue:
        ...
        if ((return_value >= 0) && ((segType == CSEG_UNDEF) ||
                                    ((*segOffPtr) < 0) || ((*segLenPtr) < 0))) {
                return_value = -1;
        }
...
static int find_ident(BFile file, char *buf, int bufSize) {
        ...
        *buf = 0;
        ...
        while (TRUE) {
                ...
                if ((iChar == EOF) ||
                    (!char_is_legal_for_ident(iChar, FALSE))) {
                        break;
...
get_seg(BFile file, CSeg cseg) {
        ...
        rc = find_seg(file, &segType, segName, &segOff, &segLen, &bodyOff,
                      &bodyLen, &userSegs);
        return_if_err(rc, rc);
        ...
        for (i = 0; i < cseg->userSegs.numSegs; ++i) {
                grab_string_from_file(&(cseg->userSegs.segs[i].text), file,
        ...
        while ((!quit) && (!bfile_eof(file))) {
                ...
                if (userSegs != NULL) {
                        ...
                        case MAGIC_CMT_USER_START:
                                if ((userSegs->numSegs > 0) &&
                                    (!user_seg_is_valid(
                                        &(userSegs->segs[(userSegs->numSegs) -
                                                         1])))) {
                                        ...
                                        if (find_user_seg_end(file, userSegs) <
                                            0) {
                                                return_code(-1);
                                        }
                                }
                        ...
                        case MAGIC_CMT_USER_END:
                                /*
                                 * This should have been found by
                                 * find_user_seg_end().
                                 */
                                util_printf_err(
                                    "Bad DTB_USER segment at line %d\n",
                                    cvt_offset_to_line(file,
                                                       bfile_get_off(file)) -
                                        1);
                                return_code(-1);
                                break;
                ...
                if (iChar == matchChar) {
                        ...
                        case '{':
                                if (find_matching(file, '}', NULL, userSegs) <
                                    0) {
                                        util_printf_err(
                                            "Unbalanced '{' at line %d.\n",
...
epilogue:
        if (foundMatching) {
                ...
                if (matchOffPtr != NULL) {
                        matchOff =
                            bfile_get_off(file) + (file->reverse ? 1 : -1);
...
static BOOL user_seg_is_valid(CUserSeg userSeg) {
        return ((userSeg->offset > -1) && (userSeg->length > -1));
}
        ...
        while (!done) {
                ...
                case '\n':
                        if (bfile_is_forward(file)) {
                                ...
                                if (skip_slash_slash_comment(
                                        file, magicCommentTypeOutPtr) < 0) {
                                        /* no comment */
                                        done = TRUE;
                                }
                        }
...
static int skip_comment(BFile file, COMMENT_TYPE commentType,
                        MAGIC_COMMENT_TYPE *magicCommentTypeOutPtr) {
        int return_value = -1;
...
skip_slash_slash_comment(BFile file,
        ...
        if (bfile_is_forward(file)) {
                return_value =
                    skip_slash_slash_comment_fwd(file, magicCommentTypeOutPtr);
        } else {
                return_value =
                    skip_slash_slash_comment_bwd(file, magicCommentTypeOutPtr);
        }
        return return_value;
}
...
static int bfile_construct(BFile file, FILE *stream) {
        file->stream = NULL;
        file->reverse = FALSE;
...
static int bfile_destruct(BFile file) { return bfile_set_file(file, NULL); }

static BOOL bfile_eof(BFile file) { return file->eof; }

static BOOL bfile_is_forward(BFile file) { return (!(file->reverse)); }

static BOOL bfile_is_reverse(BFile file) { return file->reverse; }

static int bfile_get_char(BFile file) {
        int iChar = EOF;
...
static int bfile_backup(BFile file, long numBytes) {
        int returnValue = 0;
        long posChange = 0;
...
static int bfile_set_file(BFile file, FILE *stream) {
        file->stream = stream;
        file->reverse = FALSE;
        bfileP_set_eof(file);
        return 0;
...
static int bfile_reverse(BFile file) {
        file->reverse = TRUE;
        bfileP_set_eof(file);
        return 0;
}

static int bfile_forward(BFile file) {
        file->reverse = FALSE;
        bfileP_set_eof(file);
        return 0;
}

static long bfile_get_off(BFile file) {
        long off = -1;
...
static int bfile_set_off(BFile file, long off) {
        int returnValue = 0;
...
static int bfile_save_state(BFile file, BFileState state) {
...
static int bfile_restore_state(BFile file, BFileState state) {
        if (!(state->forward)) {
                bfile_reverse(file);
        } else {
                bfile_forward(file);
...
int cseg_arrayP_destroy_impl(CSegArray *arrayInOutPtr) {
...
int main(int argc, char *argv[]) {
        ...
        for (segIndex = 0; segIndex < numSegs; ++segIndex) {
                ...
                util_dprintf(
                    1, "Seg: %s  offset:%ld length:%ld userSegs:%d\n",
                    cseg->type == CSEG_GLOBAL ? "[GLOBAL]"
                                              : util_strsafe(cseg->name),
                    cseg->offset, cseg->length, cseg->userSegs.numSegs);
                if (cseg->text != NULL) {
                        printf("|||%s|||\n", cseg->text);
                }
                ...
                if (cseg->userSegs.numSegs < 1) {
                        ...
                        for (usegIndex = 0; usegIndex < cseg->userSegs.numSegs;
                             ++usegIndex) {
                                printf(
                                    "--- user seg %d [line %ld] ---\n%s---\n",
                                    usegIndex,
                                    ...
                                    util_strsafe(
                                        cseg->userSegs.segs[usegIndex].text));
                        }
                }
        }
        printf("\n");
