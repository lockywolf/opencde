...
static BOOL msgFile_load(MsgFile that, char *filename);
static BOOL msgFile_read_top_comment(MsgFile that);
static BOOL msgFile_read_msg_set(MsgFile that);
static MsgSet msgFile_get_msg_set_by_num(MsgFile that, int set_num);
static MsgSet msgFile_add_msg_set(MsgFile that, char *module_name,
                                  int set_number, BOOL is_generated);
static int msgFile_max_set_number(MsgFile that);
static MsgSetRec *msgSet_create_new(int set_number, char *name, char *comment,
                                    BOOL is_generated);
static char *msgSet_get_name(MsgSet that);
static BOOL msgSet_is_generated(MsgSet that);
static void msgSet_output(MsgSet that, FILE *fp);
static Msg msgSet_add_msg(MsgSet that, char *string, char *comment,
                          int msg_num);
static int msgSet_max_msg_number(MsgSet that);
static Msg msg_create(char *string, char *comment, int number);
static void msg_destroy(Msg that);
static char *msg_get_string(Msg that);
static int msg_get_number(Msg that);
static void msg_set_is_referenced(Msg that, BOOL is_referenced);
static BOOL msg_is_referenced(Msg that);
static void msg_output(Msg that, FILE *fp);

static void out_str_puts(char *string, FILE *fp);
static char *in_str_dup(char *string);

static BOOL get_set_number_and_name(FILE *fp, int *set_num, char **set_name);
static char *get_comment(FILE *fp);
static BOOL get_msg_and_comment(FILE *fp, int *msg_num, char **msg_str,
                                char **msg_cmt);
static BOOL get_message(FILE *fp, int *msg_num, char **msg_str);
static char *extract_bil_name(char *set_line_comment);
static char *jump_backslash_gets(FILE *fp);
static void jump_backslash_ungets(char *buf);

/* These are obsolete functions and should go away eventually */
static char *nl_dup(char *string);
static void nl_puts(char *string, FILE *fp);
...
int MsgSet_set_allow_msg_delete(MsgSet msgSet, BOOL allow_delete) {
        msgSet->allow_msg_delete = allow_delete;
        return 0;
}

BOOL MsgSet_get_allow_msg_delete(MsgSet msgSet) {
...
int MsgSet_sure_find_msg(/* adds it, if not already there */
                         MsgSet that, char *string) {
        int ret_val = -1;
        Msg msg;
...
int MsgSet_set_is_referenced(MsgSet msgSet, BOOL set_is_referenced) {
        msgSet->is_referenced = set_is_referenced;
        return 0;
}

int MsgSet_set_msg_is_referenced(MsgSet that, int msg_num,
...
MsgFile MsgFile_create(char *project_name,
        ...
        if ((new_msg_file->msg_sets = (MsgSet *)malloc(
                 sizeof(MsgSet) * new_msg_file->alloced_slots)) ==
            (MsgSet *)NULL) {
        ...
        if (MsgFile_sure_find_msg_set(
                new_msg_file, strdup(proj_name_suffixed)) == (MsgSet)NULL) {
                MsgFile_destroy(new_msg_file);
                ret_val = (MsgFile)NULL;
                goto cret;
        }
...
int MsgFile_save(MsgFile that, FILE **msgFileOutPtr) {
        int ret_val = 0;
        FILE *fp = NULL;
        register int i;
        ...
        *msgFileOutPtr = NULL;
        ...
        if (that->file_comment == (char *)NULL) {
                fputs("$ "
                      ...
                      "==========\n",
                      fp);
                fputs("$ = Project Message Source File: generated by "
                      "'dtcodegen'\n",
                      fp);
                fputs("$ = Generated message sets have \'", fp);
                fputs(DTB_GEN_MSG_SET_PREFIX, fp);
                fputs("<name>\' as their comment\n", fp);
                fputs("$ = PLEASE READ SET COMMENTS ON HOW TO EDIT GENERATED "
                      "MESSAGE SETS \n",
                      fp);
                fputs("$ "
                      "========================================================"
...
MsgSet MsgFile_obj_sure_find_msg_set(MsgFile msgFile, ABObj obj) {
...
MsgSet MsgFile_just_find_msg_set(MsgFile that, STRING set_name) {
...
MsgSet MsgFile_sure_find_msg_set(/* adds one, if not already there */
                                 MsgFile that, char *module_name) {
        MsgSet ret_val = (MsgSet)NULL;

        if (that == (MsgFile)NULL)
                goto cret;

        if ((ret_val = MsgFile_just_find_msg_set(that, module_name)) !=
            (MsgSet)NULL)
                goto cret;
...
static BOOL msgFile_read_msg_set(MsgFile that) {
        ...
        if (that->fp == (FILE *)NULL)
                goto cret;
        if (get_set_number_and_name(that->fp, &set_number, &set_line_cmt) ==
            FALSE)
                goto cret;
        ...
        if ((msg_set = msgFile_get_msg_set_by_num(that, set_number)) == NULL) {
                ...
                if ((msg_set = msgFile_add_msg_set(
                         that, (is_generated) ? bil_name : set_line_cmt,
                         set_number, is_generated)) == (MsgSet)NULL)
                        goto cret;
        }
        msg_set->set_comment = get_comment(that->fp);

        while (get_msg_and_comment(that->fp, &msg_num, &msg_str, &msg_cmt)) {
                if (msgSet_add_msg(msg_set, msg_str, msg_cmt, msg_num) ==
                    (Msg)NULL)
                        goto cret;
        }
...
static MsgSet msgFile_add_msg_set(MsgFile that, char *module_name, int set_num,
                                  BOOL is_generated) {
        MsgSet ret_val = (MsgSet)NULL;
        MsgSet new_set;
        ...
        if (that->num_msg_sets == that->alloced_slots) {
                ...
                if ((new_sets = (MsgSet *)realloc(
                         (void *)that->msg_sets,
                         sizeof(MsgSet) * that->alloced_slots)) == NULL) {
                        that->alloced_slots -= INIT_ALLOCED_SLOTS;
                        goto cret;
                }
...
static MsgSetRec *msgSet_create_new(int set_number, char *name, char *comment,
        ...
        if (!is_generated) {
                ...
                if ((new_msg_set->msgs = (Msg *)malloc(
                         sizeof(Msg) * new_msg_set->alloced_slots)) ==
                    (Msg *)NULL) {
...
static void msgSet_output(MsgSet that, FILE *fp) {
        ...
        if (that->set_comment != (char *)NULL)
                ...
                fputs("$\t** DO NOT EDIT MESSAGES OR THE $set LINE ABOVE.\n",
                      fp);
                fputs("$\t** DO NOT USE THESE MESSAGES IN YOUR APPLICATION "
                      "CODE.\n",
                      fp);
                fputs("$\t** You can edit this comment and add comments below "
                      "each message.\n",
                      fp);
        }
...
static Msg msgSet_add_msg(MsgSet that, char *string, char *comment,
        ...
        if (that->num_msgs == that->alloced_slots) {
                ...
                that->alloced_slots += INIT_ALLOCED_SLOTS;
                if ((new_msgs = (Msg *)realloc(
                         (void *)that->msgs,
                         sizeof(Msg) * that->alloced_slots)) == NULL) {
                        that->alloced_slots -= INIT_ALLOCED_SLOTS;
                        goto cret;
                }
...
static void msg_set_is_referenced(Msg that, BOOL msg_is_referenced) {
        if (that == (Msg)NULL)
                goto cret;
...
static char *extract_bil_name(char *set_line_comment) {
        ...
        if (strncmp(set_line_comment, DTB_GEN_MSG_SET_PREFIX,
...
static void jump_backslash_ungets(char *buf) { ungets_buf = buf; }
