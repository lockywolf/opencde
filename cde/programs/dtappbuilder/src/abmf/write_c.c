...
**                                                                      **
**************************************************************************/
static int write_func_def_params(File file, BOOL use_prototypes,
                                 va_list va_params);
...
int abmfP_write_c_comment(GenCodeInfo genCodeInfo, BOOL oneLiner,
                          STRING comment) {
...
int abmfP_write_c_func_decl(GenCodeInfo genCodeInfo, BOOL is_static,
                            STRING return_type, STRING func_name, ...) {
...
int abmfP_write_c_func_begin(GenCodeInfo genCodeInfo, BOOL is_static,
                             STRING return_type, STRING func_name, ...) {
#define va_start_params() (va_start(params, func_name))
        va_list params;
        File file = genCodeInfo->code_file;
 ...
 */
int abmfP_write_c_func_end(GenCodeInfo genCodeInfo, STRING return_value) {
        File file = genCodeInfo->code_file;
 ...
 * writes #include X
 */
int abmfP_write_c_include(GenCodeInfo genCodeInfo, STRING fileName) {
 ...
 */
int abmfP_write_c_local_include(GenCodeInfo genCodeInfo, STRING fileName) {
        File codeFile = genCodeInfo->code_file;
        abio_puts(codeFile, "#include ");
        abio_put_string(codeFile, fileName);
        abio_puts(codeFile, "\n");
 ...
 * writes #include <blah.h>
 */
int abmfP_write_c_system_include(GenCodeInfo genCodeInfo, STRING fileName) {
        File codeFile = genCodeInfo->code_file;
...
#define va_end_params() (0)
        ...
        if (!use_prototypes) {
                ...
                while (!paramsDone) {
                        ...
                        abio_printf(file, "\t%s %s", curParamType,
                                    curParamName);
                        ++num_params_written;
                        if (num_params_written < num_params) {
                                abio_puts(file, ",");
...
int abmfP_write_xm_callback_decl(GenCodeInfo genCodeInfo, BOOL is_static,
                                 STRING func_name) {
        return abmfP_write_c_func_decl(
            genCodeInfo, is_static, abmfP_str_void, func_name, abmfP_str_widget,
            abmfP_widget_var_name, abmfP_str_xtpointer,
            abmfP_client_data_var_name, abmfP_str_xtpointer,
            abmfP_call_data_var_name, NULL);
}

int abmfP_write_xm_callback_begin(GenCodeInfo genCodeInfo, BOOL is_static,
                                  STRING func_name) {
...
int abmfP_write_tooltalk_callback_decl(GenCodeInfo genCodeInfo, BOOL is_static,
                                       STRING func_name) {
        return abmfP_write_c_func_decl(
            genCodeInfo, is_static, abmfP_str_bool, func_name, abmfP_str_tt_msg,
            abmfP_tt_msg_var_name, abmfP_str_voidpointer,
            abmfP_call_data_var_name, NULL);
}

int abmfP_write_session_save_callback_decl(GenCodeInfo genCodeInfo,
                                           BOOL is_static, STRING func_name) {
        return abmfP_write_c_func_decl(
            genCodeInfo, is_static, abmfP_str_bool, func_name, abmfP_str_widget,
            abmfP_widget_var_name, abmfP_str_string,
            abmfP_ss_session_file_var_name, abmfP_str_argvpointer, "argv",
            abmfP_str_intpointer, "argc", NULL);
}

int abmfP_write_session_restore_callback_decl(GenCodeInfo genCodeInfo,
                                              BOOL is_static,
                                              STRING func_name) {
        return abmfP_write_c_func_decl(genCodeInfo, is_static, abmfP_str_bool,
                                       func_name, abmfP_str_widget,
                                       abmfP_widget_var_name, abmfP_str_string,
                                       abmfP_ss_session_file_var_name, NULL);
}

int abmfP_write_tooltalk_callback_begin(GenCodeInfo genCodeInfo, BOOL is_static,
                                        STRING func_name) {
        return abmfP_write_c_func_begin(
            genCodeInfo, is_static, abmfP_str_bool, func_name, abmfP_str_tt_msg,
            abmfP_tt_msg_var_name, abmfP_str_voidpointer,
...
abmfP_write_clear_proc_decl(GenCodeInfo genCodeInfo, ABObj obj) {
        return abmfP_write_c_func_decl(
...
int abmfP_write_create_proc_begin_or_decl(GenCodeInfo genCodeInfo, ABObj obj,
                                          BOOL declaration) {
#define MAX_CREATE_PARAMS 11 /* need lots for widget return params*/
        ...
        if (abmfP_obj_create_proc_has_instance_param(obj)) {
                ...
                ++num_params;
                if (!declaration) {
                        /* 	The ipObj field is set, so we don't need to
                         * save the var. */
                }
        }
        ...
        if (num_widgets > 0) {
                ...
                for (i = 0;
                     (i < num_widgets) && (num_params < MAX_CREATE_PARAMS);
                     ++i) {
                        param_types[num_params] = "Widget *";
                        param_names[num_params] =
                            strdup(abmfP_obj_get_create_proc_return_widget_name(
                                obj, i));
                        ++num_params;
                }
        }
        ...
        if (declaration) {
                return_value = abmfP_write_c_func_decl(
                    genCodeInfo, TRUE, abmfP_str_int,
                    abmfP_get_create_proc_name(obj), param_types[0],
                    param_names[0], param_types[1], param_names[1],
                    param_types[2], param_names[2], param_types[3],
                    param_names[3], param_types[4], param_names[4],
                    param_types[5], param_names[5], param_types[6],
                    param_names[6], param_types[7], param_names[7],
                    param_types[8], param_names[8], param_types[9],
                    param_names[9], NULL);
        } else {
                return_value = abmfP_write_c_func_begin(
                    genCodeInfo, TRUE, abmfP_str_int,
                    abmfP_get_create_proc_name(obj), param_types[0],
                    param_names[0], param_types[1], param_names[1],
                    param_types[2], param_names[2], param_types[3],
        ...
        if (first_widget_return_param >= 0) {
                for (i = first_widget_return_param; i < MAX_CREATE_PARAMS;
                     ++i) {
                        if (param_names[i] != NULL) {
                                util_free(param_names[i]);
                                param_names[i] = NULL;
                        }
                }
        }
...
int abmfP_write_create_proc_decl(GenCodeInfo genCodeInfo, ABObj obj) {
        return abmfP_write_create_proc_begin_or_decl(genCodeInfo, obj, TRUE);
}

int abmfP_write_create_proc_begin(GenCodeInfo genCodeInfo, ABObj obj) {
        return abmfP_write_create_proc_begin_or_decl(genCodeInfo, obj, FALSE);
}

abmfP_write_clear_proc_begin(GenCodeInfo genCodeInfo, ABObj obj) {
        return abmfP_write_c_func_begin(
            genCodeInfo, FALSE,                    /* Is static     */
            abmfP_str_int,                         /* return type  */
            abmfP_get_clear_proc_name(obj),        /* function name */
...
abmfP_write_init_proc_decl(GenCodeInfo genCodeInfo, ABObj obj) {
...
abmfP_write_init_proc_begin(GenCodeInfo genCodeInfo, ABObj obj) {
        if (obj_is_message(obj)) {
                return abmfP_write_c_func_begin(
                    genCodeInfo, FALSE,            /* Is static */
                    abmfP_str_int,                 /* return type */
...
int abmfP_write_file_header(GenCodeInfo genCodeInfo, STRING fileName,
                            BOOL openIfdef, STRING gennedFrom, STRING gennedBy,
                            ABMF_MODIFY_TYPE modifyType, STRING description) {
        File codeFile = genCodeInfo->code_file;
        STRING comment = NULL;
        ...
        case ABMF_MODIFY_NOT:
                comment = " *    ** DO NOT MODIFY BY HAND - ALL MODIFICATIONS "
                          "WILL BE LOST **\n";
                break;

        case ABMF_MODIFY_USER_SEGS:
                comment = " * Any text may be added between the "
                          "DTB_USER_CODE_START and\n"
                          " * DTB_USER_CODE_END comments (even non-C code). "
                          "Descriptive comments\n"
                          " * are provided only as an aid.\n"
                          " *\n"
                          " *  ** EDIT ONLY WITHIN SECTIONS MARKED WITH "
                          "DTB_USER_CODE COMMENTS.  **\n"
...
int abmfP_write_file_footer(GenCodeInfo genCodeInfo, STRING fileName,
                            BOOL closeIfdef) {
        if (closeIfdef) {
...
abmfP_write_msg_clear_proc_decl(GenCodeInfo genCodeInfo, ABObj module) {
        return abmfP_write_c_func_decl(
            genCodeInfo, FALSE,                    /* Is static */
            abmfP_str_int,                         /* return type */
