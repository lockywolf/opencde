 ...
 */
typedef Boolean (*DtbClientSessionSaveCB)(Widget, char *, char ***, int *);

/*
 * Function type for Application-defined ToolTalk Message callbacks
 */
typedef Boolean (*DtbTTMsgHandlerCB)(Tt_message msg, void *calldata);

/*
 * Function type for client session restore callback
 */
typedef Boolean (*DtbClientSessionRestoreCB)(Widget, char *);
typedef struct {
        char *help_text;
        char *help_volume;
 ...
 */
int dtb_cvt_file_to_pixmap(String fileName, Widget widget,
                           Pixmap *pixmapReturnPtr);
int dtb_set_label_pixmaps(Widget widget, Pixmap labelPixmap,
                          Pixmap labelInsensitivePixmap);
int dtb_set_label_from_bitmap_data(Widget widget, int width, int height,
                                   unsigned char *bitmapData);
Boolean dtb_file_has_extension(String fileName, String extension);
int dtb_cvt_filebase_to_pixmap(Widget widget, String fileBase, String extension,
                               Pixmap *pixmap_ptr);

int dtb_cvt_image_file_to_pixmap(Widget widget, String fileName,
                                 Pixmap *pixmap);
int dtb_set_label_from_image_file(Widget widget, String fileName);
unsigned long dtb_cvt_resource_from_string(Widget parent, String res_type,
                                           unsigned int size_of_type,
                                           String res_str_value,
                                           unsigned long error_value);
Pixmap dtb_create_greyed_pixmap(Widget widget, Pixmap pixmap);
void dtb_save_toplevel_widget(Widget toplevel);

Widget dtb_get_toplevel_widget();

void dtb_remove_sash_focus(Widget panedwindow);

void dtb_save_command(char *command);

char *dtb_get_command();

void dtb_help_dispatch(Widget widget, XtPointer clientData, XtPointer callData);
void dtb_help_back_hdlr(Widget widget, XtPointer clientData,
                        XtPointer callData);
void dtb_more_help_dispatch(Widget widget, XtPointer clientData,
                            XtPointer callData);
void dtb_do_onitem_help();
void dtb_show_help_volume_info(char *volume_name, char *location_id);
Widget dtb_create_message_dlg(Widget parent, DtbMessageData mbr,
                              String override_msg);
static void destroyCB(Widget widget, XtPointer client_data,
                      XtPointer call_data);
Widget dtb_MessageBoxGetActionButton(Widget msg_dlg, DTB_BUTTON which_btn);
void dtb_show_message(Widget parent, DtbMessageData mbr, String override_msg,
                      DtbObjectHelpData override_help);
DTB_MODAL_ANSWER dtb_show_modal_message(Widget parent, DtbMessageData mbr,
                                        String override_msg,
                                        DtbObjectHelpData override_help,
                                        Widget *modal_dlg_pane_out_ptr);
static void modal_dlgCB(Widget widget, XtPointer client_data,
                        XtPointer call_data);
void dtb_children_center(Widget form, DTB_CENTERING_TYPES type);
void dtb_children_uncenter(Widget form, DTB_CENTERING_TYPES type);
void dtb_center(Widget form_child, DTB_CENTERING_TYPES type);
void dtb_uncenter(Widget form_child, DTB_CENTERING_TYPES type);
void dtb_session_save(Widget widget, XtPointer clientData, XtPointer callData);
DtbClientSessionSaveCB dtb_get_client_session_saveCB();

void dtb_set_client_session_saveCB(DtbClientSessionSaveCB session_saveCB);
void dtb_session_restore(Widget widget, char *session_file);
...
void dtb_set_tt_msg_quitCB(DtbTTMsgHandlerCB msg_quitCB);
void dtb_set_tt_msg_do_commandCB(DtbTTMsgHandlerCB msg_do_commandCB);
void dtb_set_tt_msg_get_statusCB(DtbTTMsgHandlerCB msg_get_statusCB);
void dtb_set_tt_msg_pause_resumeCB(DtbTTMsgHandlerCB msg_pause_resumeCB);
Tt_message dtb_tt_contractCB(Tt_message msg, void *client_data,
                             Tt_message contract);

void dtb_tt_close();

#define dtb_cvt_string_to_pixel(parent, str)                                   \
        ((Pixel)dtb_cvt_resource_from_string((parent), XtRPixel,               \
                                             sizeof(Pixel), (str), 0))
 ...
 */
static void center_widget(Widget form_child, DTB_CENTERING_TYPES type);
static void uncenter_widget(Widget form_child, DTB_CENTERING_TYPES type);
static void centering_handler(Widget widget, XtPointer client_data,
...
static void get_widest_label(WidgetList list, int count, Widget *child_widget,
                             Dimension *label_width);

static Position get_offset_from_ancestor(Widget ancestor, Widget w);

static void get_widest_value(WidgetList list, int count, Widget *child_widget,
                             Dimension *value_width);

static void get_widget_rect(Widget widget, XRectangle *rect);

static void get_greatest_size(Widget *list, int count, int *width, int *height,
                              Widget *tallest, Widget *widest);

static void get_group_cell_size(Widget parent, DtbGroupInfo *group_info,
                                int *cell_width, int *cell_height);

static void get_group_row_col(Widget parent, DtbGroupInfo *group_info,
                              int *rows, int *cols);
...
#define pixmapReturn (*pixmapReturnPtr)
         ...
         */
        XtVaGetValues(widget, XmNforeground, &fgPixel, XmNbackground, &bgPixel,
                      NULL);
...
Pixmap dtb_create_greyed_pixmap(Widget widget, Pixmap pixmap) {
        ...
        if (!XGetGeometry(dpy, pixmap, &root, &x, &y, &width, &height, &bw,
                          &depth)) {
                return ((Pixmap)NULL);
        }
        ...
        XCopyArea(dpy, pixmap, insensitive_pixmap, gc, 0, 0, width, height, 0,
                  0);
        ...
        } else /* Append extensions to locate best graphic match */
        {
                ...
                if (depth > 1) /* Look for Color Graphics First */
                {
                        rc = dtb_cvt_filebase_to_pixmap(widget, fileName, ".pm",
                                                        &tmpPixmap);
                        if (rc < 0)
                                rc = dtb_cvt_filebase_to_pixmap(
                                    widget, fileName, ".xpm", &tmpPixmap);
                        if (rc < 0)
                                rc = dtb_cvt_filebase_to_pixmap(
                                    widget, fileName, ".bm", &tmpPixmap);
                        if (rc < 0)
                                rc = dtb_cvt_filebase_to_pixmap(
                                    widget, fileName, ".xbm", &tmpPixmap);
...
int dtb_set_label_from_bitmap_data(Widget widget, int width, int height,
        ...
        if ((widget == NULL) || (width < 1) || (height < 1) ||
            (bitmapData == NULL)) {
                return -1;
        }
         ...
         */
        {
                ...
                XtVaGetValues(XtIsSubclass(widget, xmGadgetClass)
                                  ? XtParent(widget)
                                  : widget,
                              XmNbackground, &widgetBg, XmNforeground,
                              &widgetFg, XmNdepth, &widgetDepth, NULL);
                bgPixel = widgetBg;
                fgPixel = widgetFg;
                depth = widgetDepth;
        }
         ...
         */
        labelPixmap =
            XCreatePixmapFromBitmapData(display, window, (char *)bitmapData,
                                        width, height, fgPixel, bgPixel, depth);
        if (labelPixmap == NULL) {
                return -1;
        }
 ...
 * Sets both the sensitive and insensitve pixmaps
 */
int dtb_set_label_from_xpm_data(Widget widget, char *xpmData[]) {
        int status = 0;
        Display *display = NULL;
        Screen *screen = NULL;
        Drawable window = NULL;
        ...
        status =
            _DtXpmCreatePixmapFromData(display, window, xpmData, &labelPixmap,
...
unsigned long dtb_cvt_resource_from_string(Widget parent, String res_type,
        ...
        if (XtConvertAndStore(parent, XtRString, &source, res_type, &dest) !=
            0) {
...
Widget dtb_get_toplevel_widget() { return (dtb_project_toplevel_widget); }
...
*/
void dtb_remove_sash_focus(Widget widget) {
        WidgetList children;
        int numChildren, i;
        ...
        XtVaGetValues(widget, XmNchildren, &children, XmNnumChildren,
                      &numChildren, NULL);
...
char *dtb_get_command() { return (dtb_save_command_str); }
...
void dtb_help_dispatch(Widget widget, XtPointer clientData,
                       XtPointer callData) {
        ...
        */
        if (help_data->help_volume == 0 || *(help_data->help_volume) == NULL ||
            help_data->help_locationID == 0 ||
            *(help_data->help_locationID) == NULL) {
                buffer[0] = '\0';
        } else {
                sprintf(buffer, "%s/%s", help_data->help_volume,
        ...
        */
        if (Quick_help_dialog == (Widget)NULL) {
                ...
                i = 0;
                XtSetArg(wargs[i], XmNtitle, "Application Help");
                i++;
                XtSetArg(wargs[i], DtNhelpType, DtHELP_TYPE_DYNAMIC_STRING);
                i++;
                XtSetArg(wargs[i], DtNstringData, help_data->help_text);
                i++;
                XtSetArg(wargs[i], DtNhelpVolume, buffer);
                i++;
                Quick_help_dialog = DtCreateHelpQuickDialog(
                    dtb_get_toplevel_widget(), "Help", wargs, i);

                /*
                ** Fetch out the Dialog's More button child and hook the 'more
                *help'
                ** handler to its activateCallback.  Set it's current status to
                ** indicate whether this object has supplemental help data.
                */
                MoreButton = DtHelpQuickDialogGetChild(
                    Quick_help_dialog, DtHELP_QUICK_MORE_BUTTON);
                ...
                /*
                ** Fetch out the Dialog's Backtrack button child & hook a
                *callback
                ** that will control button sensitivity based on the presence of
                *more
                ** help data.
                */
                back_button = DtHelpQuickDialogGetChild(
                    Quick_help_dialog, DtHELP_QUICK_BACK_BUTTON);
                XtAddCallback(back_button, XmNactivateCallback,
                              dtb_help_back_hdlr, (XtPointer)Quick_help_dialog);
        }
        /* Otherwise the dialog already exists so we just set the attributes. */
                ...
                */
                ...
                XtVaSetValues(Quick_help_dialog, DtNhelpType,
                              DtHELP_TYPE_DYNAMIC_STRING, DtNhelpVolume, buffer,
                              DtNstringData, help_data->help_text, NULL);
        }
...
void dtb_help_back_hdlr(Widget widget, XtPointer clientData,
         ...
         */
        XtVaGetValues(help_dialog, DtNhelpVolume, &buffer, DtNstringData, &text,
                      NULL);

        /* Get a handle to the "More..." button */
        more_button =
            DtHelpQuickDialogGetChild(help_dialog, DtHELP_QUICK_MORE_BUTTON);
        /*
...
void dtb_more_help_dispatch(Widget widget, XtPointer clientData,
        ...
        /* Fetch the saved volume/locationID information from the dialog widget
         */
        XtVaGetValues(help_dialog, DtNhelpVolume, &buffer, NULL);
        ...
        */
        ...
        if (GeneralHelpDialog == (Widget)NULL) {
                ...
                XtSetArg(wargs[i], XmNtitle, "Application Help");
                i++;
                XtSetArg(wargs[i], DtNhelpType, DtHELP_TYPE_TOPIC);
                i++;
                XtSetArg(wargs[i], DtNhelpVolume, vol);
                i++;
                XtSetArg(wargs[i], DtNlocationId, loc);
                i++;

                GeneralHelpDialog = DtCreateHelpDialog(
                    dtb_get_toplevel_widget(), "GeneralHelp", wargs, i);
        } else {
                i = 0;
                XtSetArg(wargs[i], DtNhelpType, DtHELP_TYPE_TOPIC);
                i++;
...
void dtb_do_onitem_help() {
        ...
        */
        while (target != (Widget)NULL) {
                if (XtHasCallbacks(target, XmNhelpCallback) ==
                    XtCallbackHasSome) {
                        XtCallCallbacks(target, XmNhelpCallback,
                                        (XtPointer)NULL);
                        return;
                } else {
...
*/
int dtb_show_help_volume_info(char *volume_name, char *location_id) {
        int i;
        Arg wargs[10];
        static Widget GeneralHelpDialog = (Widget)NULL;
        ...
        if (GeneralHelpDialog == (Widget)NULL) {
                ...
                XtSetArg(wargs[i], DtNhelpType, DtHELP_TYPE_TOPIC);
                i++;
                XtSetArg(wargs[i], DtNhelpVolume, volume_name);
                i++;
                XtSetArg(wargs[i], DtNlocationId, location_id);
                i++;
 ...
 */
void dtb_session_save(Widget widget, XtPointer clientData, XtPointer callData) {
        int new_argc, client_argc = 0, new_argc_counter, i;
        char **new_argv, **client_argv = NULL, *session_file_path,
                         *session_file_name, *app_name = NULL;
        ...
        if (session_saveCB)
                 ...
                 */
                status = session_saveCB(widget, session_file_path, &client_argv,
                                        &client_argc);
         ...
         */
        XSetCommand(XtDisplay(widget), XtWindow(widget), new_argv, new_argc);
...
Widget dtb_create_message_dlg(Widget parent, DtbMessageData mbr,
        ...
        if (mbr->action1_label == (XmString)NULL) {
                ...
                XtSetArg(arg[n], XmNokLabelString, mbr->action1_label);
                n++;
        }
        ...
        if (mbr->action2_label != (XmString)NULL) {
                button = XtVaCreateManagedWidget(
                    ...
                    XmNlabelString, mbr->action2_label, XmNuserData,
                    DTB_ACTION2_BUTTON, NULL);
        }
        /* Create an extra button for the MessageBox */
        if (mbr->action3_label != (XmString)NULL) {
                button = XtVaCreateManagedWidget(
                    "action3_button", xmPushButtonWidgetClass, msg_dlg,
                    XmNlabelString, mbr->action3_label, XmNuserData,
                    DTB_ACTION3_BUTTON, NULL);
        }

        XtSetArg(arg[n], XmNdialogType, mbr->type);
        n++;
        ...
        if (mbr->default_button == DTB_ACTION2_BUTTON) {
                action_button =
                    dtb_MessageBoxGetActionButton(msg_dlg, DTB_ACTION2_BUTTON);
        } else if (mbr->default_button == DTB_ACTION3_BUTTON) {
                action_button =
                    dtb_MessageBoxGetActionButton(msg_dlg, DTB_ACTION3_BUTTON);
        }

        if (action_button != (Widget)NULL) {
                XtVaSetValues(action_button, XmNdefaultButtonShadowThickness, 2,
...
Widget dtb_MessageBoxGetActionButton(Widget msg_dlg, DTB_BUTTON which_btn) {
        ...
        XtVaGetValues(msg_dlg, XmNnumChildren, &numChildren, XmNchildren,
                      &children, NULL);
 ...
 */
void dtb_show_message(Widget parent, DtbMessageData mbr, XmString override_msg,
                      DtbObjectHelpData override_help) {
        DtbObjectHelpData help_data_copy = (DtbObjectHelpData)NULL;
        Widget msg_dlg = (Widget)NULL, action_btn = (Widget)NULL;

        if (override_help != (DtbObjectHelpData)NULL) {
                help_data_copy =
                    (DtbObjectHelpData)XtMalloc(sizeof(DtbObjectHelpDataRec));

                help_data_copy->help_text =
                    override_help->help_text
                        ? XtNewString(override_help->help_text)
                        : NULL;
                help_data_copy->help_volume =
                    override_help->help_volume
                        ? XtNewString(override_help->help_volume)
                        : NULL;
                help_data_copy->help_locationID =
                    override_help->help_locationID
                        ? XtNewString(override_help->help_locationID)
                        : NULL;
        }

        msg_dlg =
            dtb_create_message_dlg(parent, mbr, override_msg, help_data_copy);
        ...
        if (mbr->help_button) {
                if (help_data_copy != (DtbObjectHelpData)NULL) {
                        XtAddCallback(msg_dlg, XmNhelpCallback,
                                      dtb_help_dispatch,
 ...
 */
DTB_MODAL_ANSWER
dtb_show_modal_message(Widget parent, DtbMessageData mbr, XmString override_msg,
                       DtbObjectHelpData override_help,
                       Widget *modal_dlg_pane_out_ptr) {
        XtAppContext app;
        ...
        XtVaSetValues(modal_dlg_pane, XmNdialogStyle,
                      XmDIALOG_FULL_APPLICATION_MODAL, XmNokCallback,
                      &ok_callback, XmNcancelCallback, &cancel_callback,
                      XmNuserData, &answer, NULL);

        action2_button =
            dtb_MessageBoxGetActionButton(modal_dlg_pane, DTB_ACTION2_BUTTON);
        if (action2_button != (Widget)NULL) {
                XtVaSetValues(action2_button, XmNuserData, (XtPointer)&answer,
                              NULL);
                XtAddCallback(action2_button, XmNactivateCallback, modal_dlgCB,
                              (XtPointer)DTB_ANSWER_ACTION2);
        }

        action3_button =
            dtb_MessageBoxGetActionButton(modal_dlg_pane, DTB_ACTION3_BUTTON);
        if (action3_button != (Widget)NULL) {
                XtVaSetValues(action3_button, XmNuserData, (XtPointer)&answer,
                              NULL);
                XtAddCallback(action3_button, XmNactivateCallback, modal_dlgCB,
                              (XtPointer)DTB_ANSWER_ACTION3);
        }

        if (mbr->help_button) {
                if (help_data_copy != (DtbObjectHelpData)NULL) {
                        XtAddCallback(modal_dlg_pane, XmNhelpCallback,
                                      dtb_help_dispatch,
                                      (XtPointer)help_data_copy);
                } else if (mbr->help_data.help_text != (char *)NULL) {
                        XtAddCallback(modal_dlg_pane, XmNhelpCallback,
...
void dtb_children_center(Widget form, DTB_CENTERING_TYPES type) {
         ...
         * Get children list
         */
        XtVaGetValues(form, XmNnumChildren, &num_children, XmNchildren,
                      &children_list, NULL);
...
void dtb_center(Widget form_child, DTB_CENTERING_TYPES type) {
        ...
        XtAddEventHandler(form_child, StructureNotifyMask, False,
...
void dtb_uncenter(Widget form_child, DTB_CENTERING_TYPES type) {
        ...
        XtRemoveEventHandler(form_child, StructureNotifyMask, False,
                             centering_handler, (XtPointer)type);
}
...
static void center_widget(Widget form_child, DTB_CENTERING_TYPES type) {
        ...
        int center_offset;
        unsigned char left_attach = XmATTACH_NONE, top_attach = XmATTACH_NONE;
        ...
        XtVaGetValues(form_child, XmNwidth, &width, XmNheight, &height,
                      XmNleftAttachment, &left_attach, XmNtopAttachment,
                      &top_attach, NULL);
        ...
        case DTB_CENTER_POSITION_VERT:
                ...
                XtVaSetValues(form_child, XmNleftOffset, center_offset, NULL);
        ...
        case DTB_CENTER_POSITION_HORIZ:
                ...
                XtVaSetValues(form_child, XmNtopOffset, center_offset, NULL);
                break;
        ...
        case DTB_CENTER_POSITION_BOTH: {
                ...
                XtVaSetValues(form_child, XmNleftOffset, left_offset,
                              XmNtopOffset, top_offset, NULL);
        } break;
        }
}
...
static void uncenter_widget(Widget form_child, DTB_CENTERING_TYPES type) {
        ...
        XtVaGetValues(form_child, XmNleftAttachment, &left_attach,
                      XmNtopAttachment, &top_attach, NULL);
        ...
        case DTB_CENTER_POSITION_VERT:
                ...
                XtVaSetValues(form_child, XmNleftOffset, 0, NULL);
        ...
        case DTB_CENTER_POSITION_HORIZ:
                ...
                XtVaSetValues(form_child, XmNtopOffset, 0, NULL);
                break;
        ...
        case DTB_CENTER_POSITION_BOTH:
                ...
                XtVaSetValues(form_child, XmNleftOffset, 0, XmNtopOffset, 0,
                              NULL);
                break;
        }
}
 ...
 */
static Widget get_label_widget(Widget widget) {
        WidgetList children_list;
        Widget label_widget = NULL;
        int i, num_children = 0;
        char *subobj_name = NULL, *label_name = NULL;
        ...
        label_name =
            (char *)XtMalloc(1 + strlen(subobj_name) + strlen("_label") + 5);
        label_name[0] = '*';
        ...
        if (XtIsSubclass(widget, xmFormWidgetClass) ||
            XtIsSubclass(widget, xmFrameWidgetClass))
                XtVaGetValues(widget, XmNnumChildren, &num_children,
                              XmNchildren, &children_list, NULL);
...
static Dimension get_label_width(Widget widget) {
        WidgetList children_list;
        ...
        if (lbl_widget) {
                ...
                XtVaGetValues(lbl_widget, XmNwidth, &lbl_width, NULL);
...
static void get_widget_rect(Widget widget, XRectangle *rect) {
        ...
        XtVaGetValues(widget, XtNwidth, (XtArgVal) & (rect->width), XtNheight,
                      (XtArgVal) & (rect->height), XtNx, (XtArgVal) & (rect->x),
                      XtNy, (XtArgVal) & (rect->y), NULL);
}
...
static void get_group_cell_size(Widget parent, DtbGroupInfo *group_info,
         ...
         * Get children list
         */
        XtVaGetValues(parent, XmNnumChildren, &num_children, XmNchildren,
                      &children_list, NULL);

        get_greatest_size(children_list, num_children, cell_width, cell_height,
...
static void get_group_row_col(Widget parent, DtbGroupInfo *group_info,
                              int *rows, int *cols) {
        WidgetList children_list = NULL;
        int num_rows, num_cols, num_children;
        ...
        if (num_cols <= 0)
                num_cols = (num_children / num_rows) +
                           ((num_children % num_rows) ? 1 : 0);

        if (num_rows <= 0)
                num_rows = (num_children / num_cols) +
                           ((num_children % num_cols) ? 1 : 0);

        *rows = num_rows;
        *cols = num_cols;
}

static Widget get_group_child(Widget parent, DtbGroupInfo *group_info,
                              int x_pos, int y_pos) {
        DTB_GROUP_TYPES group_type;
        WidgetList children_list = NULL;
        Widget ret_child = NULL;
        int num_children = 0, num_rows, num_columns, i = -1;

        if (!parent || !group_info || (x_pos < 0) || (y_pos < 0))
                return (NULL);
...
void dtb_children_align(Widget parent, DTB_GROUP_TYPES group_type,
                        DTB_ALIGN_TYPES row_align, DTB_ALIGN_TYPES col_align,
                        int margin, int num_rows, int num_cols, int hoffset,
                        int voffset) {
        ...
        case DTB_GROUP_ROWS:
                if (group_info->row_align == DTB_ALIGN_HCENTER)
                        XtAddEventHandler(parent, ExposureMask, False,
                                          expose_handler,
                                          (XtPointer)group_info);
                break;
        ...
        XtAddCallback(parent, XtNdestroyCallback, free_group_info,
                      (XtPointer)group_info);
}

static void align_children(Widget parent, DtbGroupInfo *group_info,
                           Boolean init) {
        if (!parent || !group_info)
                return;
...
static void align_handler(Widget widget, XtPointer client_data, XEvent *event,
         ...
         * Get children list
         */
        XtVaGetValues(widget, XmNnumChildren, &num_children, XmNchildren,
                      &children_list, NULL);
        ...
        if (event->type == ConfigureNotify) {
                ...
                if (xcon->window != xcon->event) {
                        resized_child =
                            XtWindowToWidget(XtDisplay(widget), xcon->window);
                        ...
                        case DTB_GROUP_COLUMNS:
                                if ((group_info->col_align ==
                                     DTB_ALIGN_LABELS) ||
                                    (group_info->col_align ==
                                     DTB_ALIGN_VCENTER))
                                        relayout_all = True;
                                break;

                        case DTB_GROUP_ROWSCOLUMNS:
                                if ((group_info->row_align ==
                                     DTB_ALIGN_HCENTER) ||
                                    (group_info->col_align ==
                                     DTB_ALIGN_LABELS) ||
                                    (group_info->col_align ==
        ...
        if (event->type == DestroyNotify) {
                ...
                destroyed_child =
                    XtWindowToWidget(XtDisplay(widget), xdestroy->window);
        ...
        XtAddEventHandler(widget, SubstructureNotifyMask, False, align_handler,
                          (XtPointer)client_data);
}

static void expose_handler(Widget widget, XtPointer client_data, XEvent *event,
                           Boolean *cont_dispatch) {
        DtbGroupInfo *group_info = (DtbGroupInfo *)client_data;
        WidgetList children_list;
        int i, num_children = 0;
        Boolean relayout_all = False, register_align_handler = False;
        ...
        XtRemoveEventHandler(widget, ExposureMask, False, expose_handler,
                             (XtPointer)client_data);
...
static void free_group_info(Widget widget, XtPointer client_data,
                            XtPointer call_data) {
        DtbGroupInfo *group_info = (DtbGroupInfo *)client_data;
        ;
...
static void align_rows(Widget parent, DtbGroupInfo *group_info, Boolean init) {
        if (!parent || !group_info ||
            (group_info->group_type == DTB_GROUP_COLUMNS))
                return;
...
static void align_cols(Widget parent, DtbGroupInfo *group_info, Boolean init) {
        if (!parent || !group_info ||
            (group_info->group_type == DTB_GROUP_ROWS))
                return;
...
static void align_left(Widget parent, DtbGroupInfo *group_info) {
        WidgetList children_list;
        Widget child, previous_child;
        int num_children = 0, num_columns, num_rows, cell_width, cell_height, i,
            j;
        ...
        for (j = 0; j < num_rows; j++) {
                for (i = 0; i < num_columns; i++) {
                        ...
                        if (j == 0) {
                                ...
                                previous_child = get_group_child(
                                    parent, group_info, i - 1, j);
                                ...
                                if (group_type == DTB_GROUP_ROWSCOLUMNS) {
                                        ...
                                        XtVaGetValues(previous_child, XmNwidth,
                                                      &width, NULL);
                        ...
                        previous_child =
                            get_group_child(parent, group_info, i, j - 1);
...
static void align_right(Widget parent, DtbGroupInfo *group_info) {
        WidgetList children_list;
        Widget child, previous_child;
        int num_children = 0, num_columns, num_rows, cell_width, cell_height,
            offset, i, j;
                        ...
                        if (j == 0) {
                                ...
                                if (group_info->group_type ==
                                    DTB_GROUP_ROWSCOLUMNS) {
                                        ...
                                        XtVaGetValues(child, XmNwidth, &width,
                                                      NULL);
                                        offset += (cell_width - width);
                                }
...
static void align_labels(Widget parent, DtbGroupInfo *group_info) {
        WidgetList children_list = NULL, one_col;
        Widget previous_child = NULL, child, ref_widget,
               previous_ref_widget = NULL;
        Dimension ref_lbl_width = 0, max_label_width = 0, max_value_width = 0;
        int num_children = 0, num_rows, num_columns, cell_width, cell_height,
            offset, i, j, ref_x;
...
static void align_vcenter(Widget parent, DtbGroupInfo *group_info,
                          Boolean init) {
        WidgetList children_list;
        Widget child, previous_child;
        DTB_GROUP_TYPES group_type;
        int num_children = 0, num_columns, num_rows, cell_width, cell_height,
            group_width, group_height, offset, gridline, i, j;
        ...
        if (group_type == DTB_GROUP_ROWSCOLUMNS) {
                group_width =
                    (num_columns * cell_width) + ((num_columns - 1) * offset);
        }

        for (i = 0; i < num_columns; i++) {
                if (group_type == DTB_GROUP_ROWSCOLUMNS)
                        gridline =
                            (((i * (cell_width + offset)) + (cell_width / 2)) *
                             100) /
                            group_width;
                else
                        gridline = 50;
                ...
                for (j = 0; j < num_rows; j++) {
                        ...
                        if (init) {
                                ...
                                XtSetArg(args[n], XmNleftAttachment,
                                         XmATTACH_POSITION);
                                n++;
                                XtSetArg(args[n], XmNleftPosition, gridline);
                                n++;
...
static void align_top(Widget parent, DtbGroupInfo *group_info) {
        WidgetList children_list;
        Widget previous_child = NULL, child, cur_child;
        int num_children = 0, num_columns, num_rows, cell_width, cell_height, i,
            j;
        ...
        for (j = 0; j < num_rows; j++) {
                for (i = 0; i < num_columns; i++) {
                        ...
                        previous_child =
                            get_group_child(parent, group_info, i - 1, j);
...
static void align_bottom(Widget parent, DtbGroupInfo *group_info) {
        WidgetList children_list;
        Widget child, previous_child;
        DTB_GROUP_TYPES group_type;
        int num_children = 0, num_columns, num_rows, cell_height, cell_width,
            offset, i, j;
                ...
                for (i = 0; i < num_columns; i++) {
                        ...
                        if (i == 0) {
                                ...
                                if (previous_child) {
                                        ...
                                        XtVaGetValues(child, XmNheight, &height,
                                                      NULL);
...
static void align_hcenter(Widget parent, DtbGroupInfo *group_info,
                          Boolean init) {
        WidgetList children_list = NULL;
        Widget child, previous_child;
        DTB_GROUP_TYPES group_type;
        ...
        if (group_type == DTB_GROUP_ROWSCOLUMNS) {
                group_height =
                    (num_rows * cell_height) + ((num_rows - 1) * offset);
        }

        for (j = 0; j < num_rows; j++) {
                if (group_type == DTB_GROUP_ROWSCOLUMNS)
                        gridline = (((j * (cell_height + offset)) +
                                     (cell_height / 2)) *
                                    100) /
                                   group_height;
                else
                ...
                for (i = 0; i < num_columns; i++) {
                        ...
                        if (init) {
                                ...
                                XtSetArg(args[n], XmNtopAttachment,
                                         XmATTACH_FORM);
                                n++;
                                XtSetArg(args[n], XmNtopOffset, init_offset);
                                n++;
                                XtSetArg(args[n], XmNbottomAttachment,
                                         XmATTACH_NONE);
                                n++;
                                ...
                                XtSetArg(args[n], XmNtopAttachment,
                                         XmATTACH_POSITION);
                                n++;
                                XtSetArg(args[n], XmNtopPosition, gridline);
                                n++;
 ...
 * from.
 */
String dtb_get_exe_dir(void) { return dtb_exe_dir; }
...
static int determine_exe_dir_from_path(char *argv0, char *buf, int bufSize) {
        ...
        while ((!foundDir) && (moreDirs)) {
                ...
                for (i = iCurrentPathStart;
                     (i < iPathVarLen) && (szPathVar[i] != ':');) {
                        ++i;
                }
...
static Boolean path_is_executable(char *path, uid_t euid, gid_t egid) {
        ...
        if (stat(path, &sStat) == 0) {
                ...
                if (!bDetermined) {
                        if ((euid == 0) && (((sStat.st_mode & S_IXOTH) != 0) ||
 ...
 */
static DtbTTMsgHandlerCB dtb_get_tt_msg_quitCB() { return (dtb_tt_msg_quitCB); }
 ...
 */
static Boolean dtb_tt_msg_quit(Tt_message msg) {
 ...
 * Call Application-defined callback if it exists.
 */
static Boolean dtb_tt_msg_do_command(Tt_message msg) {
 ...
 * Call Application-defined callback if it exists.
 */
static Boolean dtb_tt_msg_get_status(Tt_message msg) {
 ...
 * Call Application-defined callback if it exists.
 */
static Boolean dtb_tt_msg_pause_resume(Tt_message msg, Boolean sensitive) {
 ...
 */
void dtb_tt_close() { ttdt_close(NULL, NULL, True); }
...
*/
void dtb_call_help_callback(Widget widget, XtPointer clientData,
                            XtPointer callData) {
...
static void dtb_drag_button1_motion_handler(Widget dragInitiator,
                                            XtPointer clientData,
                                            XEvent *event);
static int dtb_drag_start(DtbDragSite dragSite, XEvent *event);
static void dtb_drag_convertCB(Widget dragContext, XtPointer clientData,
                               XtPointer callData);
static void dtb_drag_to_rootCB(Widget dragContext, XtPointer clientData,
                               XtPointer callData);
static void dtb_drag_finishCB(Widget dragContext, XtPointer clientData,
                              XtPointer callData);
...
int dtb_popup_menu_register(Widget popupMenu, Widget parent) {
        ...
        if (popupMenuTrans == NULL) {
                ...
                popupMenuTrans =
                    XtParseTranslationTable("<Btn3Down>: DtbPopupMenu()");
        }
        ...
        if (!foundEntry) {
                ...
                DtbMenuRef newPopupMenus = (DtbMenuRef)realloc(
                    popupMenus, sizeof(DtbMenuRefRec) * (numPopupMenus + 1));
        ...
        if (foundEntry) {
                ...
                popupMenus[i].menu = popupMenu;
                XtAddCallback(popupMenus[i].widget, XmNdestroyCallback,
                              dtb_popup_menu_destroyCB, (XtPointer)NULL);
                XtAddCallback(popupMenus[i].menu, XmNdestroyCallback,
                              dtb_popup_menu_destroyCB, (XtPointer)NULL);
        }
...
static void dtb_popup_menu(Widget widget, XEvent *event, String *params,
                           Cardinal *num_params) {
        int i = 0;
        Widget menu = NULL;
 ...
 */
static void dtb_popup_menu_destroyCB(Widget widget, XtPointer clientData,
                                     XtPointer callData) {
        int i = 0;
        for (i = 0; i < numPopupMenus; ++i) {
                if ((popupMenus[i].widget == widget) ||
 ...
 */
int dtb_drag_site_register(Widget widget, DtbDndDragCallback callback,
                           DtDndProtocol protocol, unsigned char operations,
                           Boolean bufferIsText, Boolean allowDropOnRootWindow,
                           Pixmap cursor, Pixmap cursorMask,
        ...
        if (dragSite != NULL) {
                ...
                XtAddEventHandler(
                    widget, Button1MotionMask, False,
...
int dtb_drop_site_register(Widget widget, DtbDndDropCallback callback,
                           DtDndProtocol protocols, unsigned char operations,
        ...
        if ((callback != NULL) && ((dropSite = (DtbDropSite)XtCalloc(
                                        1, sizeof(DtbDropSiteRec))) != NULL)) {
                ...
                DtDndVaDropRegister(
                    ...
                    dropSite->transferCBRec, DtNregisterChildren,
                    regInfo.respondToDropsOnChildren, DtNtextIsBuffer,
                    regInfo.textIsBuffer,
#ifdef DtNpreserveRegistration
                    DtNpreserveRegistration,
                    regInfo.preservePreviousRegistration,
#endif
                    DtNdropAnimateCallback, dropSite->animateCBRec, NULL);
        }
...
static int dtb_drag_start(DtbDragSite dragSite, XEvent *event) {
        ...
        if (dragSite->callback != NULL) {
                dragSite->callback(DTB_DND_DRAG_START, &dragStart, NULL, NULL,
                                   NULL, NULL);
        }

        n = 0;
        if (dragStart.cursor != NULL) {
                XtSetArg(args[n], DtNsourceIcon, dragStart.cursor);
                ++n;
        }
        if ((dragStart.bufferIsText) &&
            (dragStart.protocol == DtDND_BUFFER_TRANSFER)) {
                XtSetArg(args[n], DtNbufferIsText, True);
                ++n;
        }
        if (dragStart.allowDropOnRootWindow) {
                XtSetArg(args[n], DtNdropOnRootCallback,
                         (XtPointer)dragSite->dragToRootCBRec);
                ++n;
        }
        if (DtDndDragStart(dragSite->widget, event, dragStart.protocol,
                           dragStart.numItems, dragStart.operations,
...
static void dtb_drag_convertCB(Widget dragContext, XtPointer clientData,
        ...
        case DtCR_DND_CONVERT_DATA:
                ...
                dragSite->callback(DTB_DND_CONVERT, NULL, convert, NULL, NULL,
                                   NULL);
                break;

        case DtCR_DND_CONVERT_DELETE:
                dragSite->callback(DTB_DND_DELETE, NULL, NULL, NULL, convert,
                                   NULL);
                break;
        }
}
...
static void dtb_drag_to_rootCB(Widget dragContext, XtPointer clientData,
        ...
        dragSite->callback(DTB_DND_DROPPED_ON_ROOT_WINDOW, NULL, NULL,
                           &dropInfo, NULL, NULL);
}
...
void dtb_default_dragCB(DTB_DND_REQUEST request, DtbDndDragStartInfo dragStart,
 ...
 */
void dtb_default_dropCB(Widget widget, DTB_DND_REQUEST request,
        ...
        case DTB_DND_REGISTER:
                 ...
                 ** options chosen in the Application Builder, with default
                 *values
...
static void dtb_drop_animateCB(Widget dragContext, XtPointer clientData,
                               XtPointer callData) {
        DtbDropSite dropSite = (DtbDropSite)clientData;
        DtDndDropAnimateCallback animateInfo =
            (DtDndDropAnimateCallback)callData;
        dropSite->callback(dropSite->widget, DTB_DND_ANIMATE, NULL, NULL,
                           animateInfo);
}

static void dtb_drop_transferCB(Widget dragContext, XtPointer clientData,
                                XtPointer callData) {
        DtbDropSite dropSite = (DtbDropSite)clientData;
        DtDndTransferCallback transferInfo = (DtDndTransferCallback)callData;

        dropSite->callback(dropSite->widget, DTB_DND_RECEIVE_DATA, NULL,
                           transferInfo, NULL);
}
