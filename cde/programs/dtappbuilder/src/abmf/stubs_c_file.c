 ...
 */
typedef enum { IMMED_TYPE, INT_TYPE, STRING_TYPE } ARG_TYPES;
...
static int write_instance_ptr_var(GenCodeInfo genCodeInfo, ABObj ipObj,
                                  STRING instanceVarName, STRING paramName,
                                  BOOL castValue, STRING comment);

static int write_builtin_action_for_ref(GenCodeInfo genCodeInfo, ABObj action,
                                        ABObj toObj, BOOL setUpVars,
                                        AB_BUILTIN_ACTION builtin_action,
                                        STRING actionTKResource,
                                        STRING actionResource);

static int write_builtin_action(GenCodeInfo genCodeInfo, ABObj action,
                                ABObj toObj, AB_BUILTIN_ACTION builtin_action,
                                STRING actionTKResource, STRING actionResource);
 ...
 * that do).
 */
static ABObj get_actual_from_obj(ABObj actionOrObj) {
...
static BOOL cwd_is_dtbuilder_src_dir(void) {
...
static STRING get_to_var_name(void) {
        return (cwd_is_dtbuilder_src_dir() ? instanceVarName : toVarName);
}

static STRING get_from_var_name(void) { return fromVarName; }
...
static int printf_setval(GenCodeInfo genCodeInfo, ABObj obj, ...) {
        ...
        while ((resName = va_arg(paramList, STRING)) != NULL) {
                ...
                case AB_ARG_WIDGET:
                        util_dprintf(1, "Ignoring unsupported connection "
 ...
 */
static int printf_getval(GenCodeInfo genCodeInfo, ABObj obj, int nres, ...) {
        int return_value = 0;
        File codeFile = genCodeInfo->code_file;
        va_list ap;
        STRING resource;
...
static void set_up_user_type_variables(GenCodeInfo genCodeInfo, ABObj toObj) {
        File codeFile = genCodeInfo->code_file;
        ...
        *winParentName = 0;
        ...
        if (cwd_is_dtbuilder_src_dir()) {
                write_instance_ptr_var(genCodeInfo, toObj, instanceVarName,
                                       abmfP_client_data_var_name, TRUE, NULL);
                abio_puts(genCodeInfo->code_file, nlstr);
        } else {
                write_instance_ptr_var(genCodeInfo, toObj, toVarName,
                                       abmfP_client_data_var_name, TRUE, NULL);
                write_instance_ptr_var(genCodeInfo, toObj, instanceVarName,
                                       toVarName, FALSE, "/* obsolete */");
                abio_puts(genCodeInfo->code_file, nlstr);
        }
        abio_printf(codeFile, "if (!(%s->initialized))\n", get_to_var_name());
        abmfP_write_c_block_begin(genCodeInfo);
        abio_printf(codeFile, "%s(%s, %s);\n",
                    abmfP_get_init_proc_name(structObj), get_to_var_name(),
                    winParentName);
...
static int write_instance_ptr_var(GenCodeInfo genCodeInfo, ABObj ipObj,
        ...
        abio_printf(genCodeInfo->code_file,
                    "%s\t%s = ", abmfP_get_c_struct_ptr_type_name(ipObj),
                    instanceVarName);
        if (castValue) {
...
static int write_tooltalk_cb_vars(GenCodeInfo genCodeInfo, ABObj action) {
        ...
        case AB_WHEN_TOOLTALK_DO_COMMAND:
                abio_puts(codeFile,
                          "Tt_status\t\tstatus = TT_DESKTOP_ENOTSUP;\n");
...
static int write_tooltalk_cb_body1(GenCodeInfo genCodeInfo, ABObj action) {
        ...
        case AB_WHEN_TOOLTALK_QUIT:
                ...
                abio_puts(codeFile, "Process Quit request->\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(
                    codeFile,
                    "if \"silent\" == 1, then the Quit should occur without\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "notifying the user.  If \"force\" == 1, "
                                    "then the user may\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(
                    codeFile,
                ...
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "request may be cancelled, in which case "
                                    "\"cancel\" should\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(
                    codeFile,
                ...
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "application exit should not occur until "
                                    "after the request\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "has been replied-to.\n");
        ...
        case AB_WHEN_TOOLTALK_GET_STATUS:
                abio_puts(codeFile, abmfP_comment_begin);
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "Process Get Status request->\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "The application should assign "
                                    "\"status_string\" an appropriate\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(
                    codeFile,
        ...
        case AB_WHEN_TOOLTALK_PAUSE_RESUME:
                abio_puts(codeFile, abmfP_comment_begin);
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "Process Pause/Resume request->\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "If \"sensitive\" == False (Pause), then "
                ...
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "\"sensitive\" == True (Resume), then the "
                                    "application should\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile,
                          "set the sensitivity on all mapped shells to True.n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "If the application is already in the "
                                    "requested state of\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "sensitivity, then \"already_set\" should "
                                    "be set to True.\n");
                abio_puts(codeFile, abmfP_comment_end);
                break;
        case AB_WHEN_TOOLTALK_DO_COMMAND:
                ...
                abio_puts(codeFile, abmfP_comment_begin);
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "Process Do_Command request->\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "The application should execute the code "
                                    "defined by \"command\".\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "If execution is successful, then "
                                    "\"status\" should be set to TT_OK,\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile,
                          "else set to the appropriate Tt_status value.\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "If the command returns a result, then "
                                    "\"result\" should be set\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "to the appropriate result string.\n");
                abio_puts(codeFile, abmfP_comment_end);
                break;
        default:
                break;
...
static int write_tooltalk_cb_body2(GenCodeInfo genCodeInfo, ABObj action) {
        ...
        case AB_WHEN_TOOLTALK_QUIT:
                ...
                abmfP_write_c_comment(genCodeInfo, FALSE,
                                      "Now that the Message has been replied, "
        ...
        case AB_WHEN_TOOLTALK_GET_STATUS:
                abio_puts(codeFile,
                          "tt_message_arg_val_set(msg, 0, status_string);\n");
                write_tooltalk_msg_reply(codeFile);
                break;
        case AB_WHEN_TOOLTALK_PAUSE_RESUME:
        ...
        case AB_WHEN_TOOLTALK_DO_COMMAND:
                ...
                abio_indent(codeFile);
                abio_puts(codeFile,
                          "tt_message_arg_val_set(msg, 1, result);\n");
                abio_outdent(codeFile);
                write_tooltalk_msg_reply(codeFile);
                abio_outdent(codeFile);
                abio_puts(codeFile, "}\n");
...
static int write_ss_cb_body1(GenCodeInfo genCodeInfo, ABObj action) {
        ...
        case AB_WHEN_SESSION_RESTORE:
                ...
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "Use contents of \"session file\" to bring "
                                    "the application\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "to the desired state.\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile,
        ...
        case AB_WHEN_SESSION_SAVE:
                ...
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "command line vector.\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "Session File:\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "    Open the file \"session_file\"\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile,
                          "    Write application state information into it\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "    Close the file\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "    Set \"ret_value\" to True\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "    (Setting \"ret_value\" to True is "
                                    "important if you are using\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "    session files! Do not set it to True "
                                    "if you are not using\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "    session files.)\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "Command line vector:\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "    Allocate an argv vector\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile,
                ...
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile, "    \t*argv = my_new_argv;\n");
                abio_puts(codeFile, abmfP_comment_continue);
                abio_puts(codeFile,
...
static int write_action_functions(GenCodeInfo genCodeInfo, ABObj obj) {
        File codeFile = genCodeInfo->code_file;
        static char msg[256], *s;
        AB_TRAVERSAL trav;
        ABObj action = NULL;
         ...
         */
        for (trav_reset(&trav); (action = trav_next(&trav)) != NULL; ++i) {
                if (mfobj_has_flags(action, CGenFlagIsDuplicateDef) ||
                    mfobj_has_flags(action, CGenFlagWriteDefToProjFile)) {
                        continue;
                }
 ...
 */
int abmfP_write_stubs_c_file(GenCodeInfo genCodeInfo, STRING codeFileName,
                             ABObj module) {
        File codeFile = genCodeInfo->code_file;
        STRING errmsg = NULL;
        ABObj win_obj = NULL;
        ...
        abmfP_write_file_header(
            ...
            util_get_program_name(), ABMF_MODIFY_USER_SEGS,
        ...
        if (genCodeInfo->i18n_method == ABMF_I18N_XPG4_API)
                abmfP_write_c_local_include(
                    genCodeInfo, abmfP_get_project_header_file_name(project));
...
int abmfP_write_action_function(GenCodeInfo genCodeInfo, ABObj action) {
        ...
        case AB_WHEN_AFTER_CREATED:
                ...
                write_instance_ptr_var(genCodeInfo, actualFromObj,
                                       get_from_var_name(), "callData", TRUE,
                                       NULL);
                abio_puts(genCodeInfo->code_file, nlstr);
        ...
        case AB_WHEN_DRAGGED_FROM: {
                abio_printf(
                    ...
                    actionName, actionName, actionName, actionName, actionName,
                    actionName);
                abio_puts(genCodeInfo->code_file, "\n\n");
        ...
        if (!topUserSegWritten) {
                STRING contents =
                    (actionPrintfWritten ? NULL
                                         : (isTTCB ? actionPrintf : NULL));
        ...
        if (isTTCB) {
                ...
                funcBodyWritten = TRUE;
                bottomUserSegWritten = TRUE;
        } else if (!funcBodyWritten)
                switch (obj_get_func_type(action)) {
                case AB_FUNC_BUILTIN:
                        rc = abmfP_write_builtin_action(genCodeInfo, action,
                                                        TRUE);
                        return_if_err(rc, rc);
                        funcBodyWritten = TRUE;
                        break;
                ...
                case AB_FUNC_USER_DEF:
                        abmfP_write_user_start_comment(
                            genCodeInfo, "vvv Add C code below vvv");
                        abmfP_write_user_end_comment(
                            genCodeInfo, "^^^ Add C code above ^^^");
                        bottomUserSegWritten = TRUE;
                        funcBodyWritten = TRUE;
                        break;
                ...
                default: {
                        ...
                        util_printf_err(catgets(Dtb_project_catd, 1, 78,
                                                "unknown function type for "
                                                "action from object, %s"),
                                        obj_name_string);
                        return_code(ERR);
                } break;
                }
...
int abmfP_write_builtin_action(GenCodeInfo genCodeInfo, ABObj action,
                               BOOL setUpVars) {
        int return_value = 0;
        File codeFile = genCodeInfo->code_file;
        ABObj toObj = NULL;
        ...
        if (toObj != NULL) {
                toObj = objxm_comp_get_target_for_builtin_action(
         ...
         */
        actionTKResource =
            objxm_get_resource_for_builtin_action(toObj, builtin_action);
        ...
        if (abmfP_get_c_name(genCodeInfo, toObj) == NULL) {
                return_value = write_builtin_action_for_ref(
                    genCodeInfo, action, toObj, setUpVars, builtin_action,
                    actionTKResource, actionResource);
        } else {
                if (setUpVars) {
                        set_up_user_type_variables(genCodeInfo, toObj);
                }

                return_value = write_builtin_action(
                    genCodeInfo, action, toObj, builtin_action,
                    actionTKResource, actionResource);
        }
...
*/
static int
write_builtin_action_for_ref(GenCodeInfo genCodeInfo, ABObj action, ABObj toObj,
                             BOOL setUpVars, AB_BUILTIN_ACTION builtin_action,
        ...
        if ((refList != NULL) && (numRefs > 0)) {
                ...
                for (i = 0; i < numRefs; ++i) {
                        ...
                        if (abmfP_get_c_name(genCodeInfo, refObj) != NULL) {
                                return_value = write_builtin_action(
                                    genCodeInfo, action, refObj, builtin_action,
                                    actionTKResource, actionResource);
                        }
                }
        } else {
...
#define IVAL(action) ((int)obj_get_arg_int(action))
        ...
        case AB_STDACT_SET_LABEL:
                ...
                if (obj_get_label_type(obj_get_root(toObj)) == AB_LABEL_GLYPH) {
                        ...
                        printf_setval(genCodeInfo, toObj, actionTKResource,
                                      SVAL(action), NULL);
                }
                break;
