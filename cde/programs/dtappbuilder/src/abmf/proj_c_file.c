...
static int write_main_register_save_yourself(GenCodeInfo genCodeInfo,
                                             ABObj project, char *atom_name);
...
static int write_main_tooltalk_local_vars(GenCodeInfo genCodeInfo,
                                          ABObj project);
static int write_main_tooltalk_init(GenCodeInfo genCodeInfo, ABObj project);
static int write_map_window(GenCodeInfo genCodeInfo, ABObj window, BOOL show);
...
static char *Includes[] = {
    ...
    "<sys/param.h>",     "<sys/types.h>",     "<sys/types.h>",
    "<sys/stat.h>",      "<X11/Intrinsic.h>", "<Xm/Xm.h>",
    "<Xm/MwmUtil.h>",    "<Xm/Protocols.h>",  "<Dt/Help.h>",
    "<Dt/HelpQuickD.h>", "<Dt/HelpDialog.h>", NULL,
};
...
int abmfP_write_project_c_file(GenCodeInfo genCodeInfo, STRING codeFileName,
        ...
        *projectName = 0;
        ...
        abmfP_write_file_header(
            genCodeInfo, codeFileName, FALSE, projectName,
...
static int write_app_resources(GenCodeInfo genCodeInfo, ABObj project) {
        ...
        abio_indent(codeFile);
        abio_puts(codeFile, "XtOffsetOf(DtbAppResourceRec, session_file), "
...
static int write_main(GenCodeInfo genCodeInfo, ABObj project) {
        ...
        mainWindowHasIcon = ((main_window != NULL) &&
        ...
        /*
         * Create a parent shell for every other shell and don't realize it.
         * This way, we can be consistent with our hierarchy.
         */
         ...
         */
        abio_puts(codeFile, "\n");
        abio_puts(codeFile, "XtGetApplicationResources(toplevel, "
                            "(XtPointer)&dtb_app_resource_rec,\n");
        abio_indent(codeFile);
        ...
        if (main_window != NULL) {
                ...
                if (obj_get_icon_mask(main_window) != NULL) {
                        ...
                        abio_put_string(codeFile,
                                        obj_get_icon_mask(main_window));
                        abio_printf(codeFile, ", &%s);\n",
                                    istr_string(abmfP_icon_mask_pixmap_var(
                                        genCodeInfo)));
                        abio_outdent(codeFile);
                        abmfP_icon_mask_pixmap_var_has_value(genCodeInfo) =
                            TRUE;
                }

                if (abmfP_get_num_args_of_classes(main_window,
                                                  ABMF_ARGCLASS_ALL) > 0) {
                        abmfP_xt_va_list_open_setvalues(genCodeInfo,
                                                        main_window);
                        abmfP_obj_spew_args(genCodeInfo, main_window,
                                            ABMF_ARGCLASS_ALL,
                                            ABMF_ARGFMT_VA_LIST);
                        if (!obj_get_resizable(main_window)) {
                                abio_printf(codeFile, "XmNmwmDecorations, "
                                                      "MWM_DECOR_ALL | "
                                                      "MWM_DECOR_RESIZEH,\n");
                                abio_printf(codeFile, "XmNmwmFunctions, "
                                                      "MWM_FUNC_ALL | "
                                                      "MWM_FUNC_RESIZE,\n");
                        }
         ...
         */
        write_main_register_save_yourself(genCodeInfo, project,
                                          "save_yourself_atom");
        ...
        abmfP_write_user_long_seg(
            ...
            "     *** realized. Set resources on widgets, or perform other "
            "operations\n"
...
static int write_main_session_restore(GenCodeInfo genCodeInfo, ABObj project) {
        ...
        if (abmfP_proj_needs_session_restore(project)) {
                ...
                abio_printf(codeFile,
                            ...
                            "DtbClientSessionRestoreCB)%s);\n",
                            ss_restore_CB_name ? ss_restore_CB_name : "NULL");

                abio_puts(codeFile, "    (void)dtb_session_restore(toplevel, "
                                    "dtb_app_resource_rec.session_file);\n");
                abio_puts(codeFile, "}\n");
...
static int write_main_i18n_local_vars(GenCodeInfo genCodeInfo, ABObj project) {
        ...
        if (genCodeInfo->i18n_method == ABMF_I18N_XPG4_API) {
                ...
                abio_puts(codeFile, "XtPointer\t\tlanguage_proc_client_data = "
                                    "(XtPointer)NULL;\n");
        }
        return 0;
}
...
static int write_main_msg_i18n(GenCodeInfo genCodeInfo, ABObj project) {
        ...
        abio_puts(codeFile, "Dtb_project_catd = catopen(DTB_PROJECT_CATALOG, "
                            "NL_CAT_LOCALE);\n");
        abio_puts(codeFile, "if (Dtb_project_catd == (nl_catd)-1)\n");
        abmfP_write_c_block_begin(genCodeInfo);
        abio_puts(codeFile, "fprintf(stderr, \"WARNING: Could not open message "
                            "catalog: %s. Messages will be defaults.\\n\",\n");
        abio_indent(codeFile);
...
static int write_main_tooltalk_init(GenCodeInfo genCodeInfo, ABObj project) {
        ...
        */
        abmfP_write_c_block_begin(genCodeInfo);
        abio_puts(codeFile, "fprintf(stderr,\"ttdt_open(): %s\\n\", "
        ...
        */
        ...
        abio_outdent(codeFile);
        abio_printf(
            codeFile, "tt_session_pattern = ttdt_session_join(NULL, %s,\n",
            tt_level == AB_TOOLTALK_DESKTOP_ADVANCED ? "dtb_tt_contractCB"
                                                     : "NULL");
        abio_indent(codeFile);
