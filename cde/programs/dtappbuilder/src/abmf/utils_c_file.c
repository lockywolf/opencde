...
static int write_save_toplevel_widget_var(GenCodeInfo genCodeInfo,
                                          ABObj project);
...
static int write_centering_static_func_decls(GenCodeInfo genCodeInfo,
                                             ABObj project);
static int write_msg_static_func_decls(GenCodeInfo genCodeInfo, ABObj project);
static int write_align_static_func_decls(GenCodeInfo genCodeInfo,
                                         ABObj project);
...
int abmfP_write_utils_c_file(GenCodeInfo genCodeInfo, STRING fileName,
        ...
        abmfP_write_file_header(
            genCodeInfo, fileName, FALSE, projName, util_get_program_name(),
            ABMF_MODIFY_NOT,
...
static int write_session_CB_vars(GenCodeInfo genCodeInfo, ABObj project) {
        ...
        abio_puts(codeFile, " */\n");
        abio_puts(codeFile, "static "
                            "DtbClientSessionSaveCB\t\tdtb_client_session_"
                            "saveCB = NULL;\n");

        if (abmfP_proj_needs_session_restore(project)) {
                abio_puts(codeFile, "/*\n");
                abio_puts(codeFile, " * Variable for storing client session "
                                    "restore callback\n");
                abio_puts(codeFile, " */\n");
                abio_puts(codeFile, "static "
                                    "DtbClientSessionRestoreCB\tdtb_client_"
                                    "session_restoreCB = NULL;\n");
        }
...
static int write_dnd_vars(GenCodeInfo genCodeInfo, ABObj project) {
        File codeFile = genCodeInfo->code_file;
        project = project; /* avoid warning */
        abio_puts(codeFile, "#include <Dt/Dnd.h>"
                            "\n"
                            "\n"
