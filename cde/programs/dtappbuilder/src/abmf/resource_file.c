...
static int write_header(File file, STRING fileName, ABObj project,
                        BOOL isIntermediateFile);

static int write_footer(File file, BOOL isIntermediateFile);

static int abmfP_res_file_append_intermediate_files(File appResFile,
                                                    ABObj project);

static int append_intermediate_res_file(File toFile, File intFile);
 ...
 */
int abmfP_write_app_res_file(File file, ABObj project, STRING resFileName) {
        int return_value = 0;
        int rc = 0;
 ...
 */
File abmfP_res_file_open(STRING filePath, STRING fileDescName, ABObj obj,
                         BOOL isIntermediateFile) {
        File fp = NULL;
...
int abmfP_res_file_close_impl(File *pfile) {
...
int abmfP_res_file_merge(File vanillaFile, File modifiedFile,
                         File *mergedFileOut) {
        int return_value = 0;
        int rc = 0;
        long userSegOff = 0;
        ...
        while ((iChar = fgetc(vanillaFile)) != EOF) {
                ...
                if ((curVanillaSeg != NULL) &&
                    (vanillaOffset >= curVanillaSeg->offset)) {
                        ...
                        fseek(vanillaFile,
                              curVanillaSeg->offset + curVanillaSeg->len,
                              SEEK_SET);
                        vanillaOffset = ftell(vanillaFile) - 1;

                        if (++curSegIndex < 2) {
                                curVanillaSeg =
                                    &(vanillaUserSegs.segs[curSegIndex]);
                                curModifiedSeg =
                                    &(modifiedUserSegs.segs[curSegIndex]);
                        } else {
                                curVanillaSeg = NULL;
                                curModifiedSeg = NULL;
                        }
...
static int find_user_segs(File file, UserSegs segs) {
        ...
        if (segs->numSegs != 2) {
                util_printf_err("There are %d DTB_USER segments in the file "
                                "(there should be two)\n",
                                segs->numSegs);
...
static int find_user_seg(File file, UserSeg seg) {
        long userSegStart = -1;
        long userSegEnd = -1;
...
static long find_user_seg_begin(File modifiedResFile) {
        long userSegOff = -1;
        char lineBuf[256];
        int lineOff = 0;
        char magicLine[256];
        ...
        *magicLine = 0;
        ...
        while ((userSegOff < 0) && ((iChar = fgetc(modifiedResFile)) != EOF)) {
                if (iChar == '\n') {
                        lineOff = 0;
...
STRING
abmfP_get_intermediate_res_file_name(ABObj obj, char *fileNameOut,
                                     int nameMax) {
...
STRING
abmfP_get_app_res_file_name(ABObj obj, char *fileNameOut, int nameMax) {
...
static int write_header(File file, STRING fileName, ABObj obj,
                        BOOL isIntermediateFile) {
        ...
        if (isIntermediateFile) {
                abio_printf(file, "! Intermediate resource file for %s\n",
                            get_name_for_comment(obj, buf, MAXPATHLEN));
                abio_puts(file, "!\n"
                                "! ** DO NOT EDIT BY HAND. ALL MODIFICATIONS "
                                "WILL BE LOST!\n");
        } else {
                assert(obj_is_project(obj));
                abio_printf(file, "! Resource file for application %s\n",
                            obj_get_name(obj));
                abio_printf(
                    file, "! This file was generated by %s, from project %s\n",
                ...
                abio_printf(file,
                            ...
                            "DTB_USER_ COMMENTS.      **\n"
                            "!  ** ALL OTHER MODIFICATIONS WILL BE "
                            "OVERWRITTEN. DO NOT MODIFY OR  **\n"
...
static int write_footer(File file, BOOL isIntermediate) {
        if (isIntermediate) {
        ...
        abio_printf(file,
                    ...
                    "!!!!!!!!!!!!!!!!\n"
                    "!\n"
                    "! The remainder of this file (following this comment) may "
                    "be modified.\n"
...
static int abmfP_res_file_append_intermediate_files(File appResFile,
        ...
        *partResFileName = 0;
         ...
         */
        abmfP_get_intermediate_res_file_name(project, partResFileName,
                                             MAX_PATH_SIZE);
        ...
        for (trav_open(&trav, project, AB_TRAV_MODULES);
             (module = trav_next(&trav)) != NULL;) {
                abmfP_get_intermediate_res_file_name(module, partResFileName,
                                                     MAX_PATH_SIZE);
                partResFile = util_fopen_locked(partResFileName, "r");
