        ...
        if (action->info.action.auto_named && (from != NULL)) {
                ...
                if (obj_is_project(from)) {
                        ...
                        parent_name = (STRING)util_malloc(
                            strlen(obj_get_name(parent)) + APP_PREFIX_LEN + 1);
                ...
                if (!util_strempty(parent_name) &&
                    !util_strempty(obj_get_name(from))) {
                        sprintf(name, "%s_%s%s", parent_name,
                                obj_get_name(from),
 ...
 *
 */
int abmfP_tree_set_action_names(ABObj rootObj) {
        int return_value = 0;
                ...
                for (trav_open(&moduleTrav, rootObj, AB_TRAV_MODULES);
                     (module = trav_next(&moduleTrav)) != NULL;) {
                        if (obj_get_write_me(module)) {
                                abmfP_set_action_names_for_obj(
                                    module); /* each module */
                        }
                }
        ...
        *headerFilename = 0;
        ...
        for (trav_open(&trav, projOrModule, AB_TRAV_ACTIONS_FOR_OBJ);
             (action = trav_next(&trav)) != NULL;) {
                ...
                /*
                 * Check to see if the popup's window parent is in another
                 * module
                 */
                ...
                if (popup_win == NULL) {
                        popup_win =
                            obj_get_parent_of_type(compRoot, AB_TYPE_DIALOG);
                }
                if (popup_win == NULL) {
                        popup_win = obj_get_parent_of_type(compRoot,
                                                           AB_TYPE_BASE_WINDOW);
                }
                ...
                if (popup_win != NULL) {
                        ...
                        if (win_parent != NULL) {
                                ...
                                if (toModule != NULL) {
                                        sprintf(headerFilename, "\"%s\"",
                                                abmfP_get_ui_header_file_name(
                                                    toModule));
                                        strlist_add_str(includeFiles,
                                                        headerFilename, NULL);
                                }
                        }
                }
                 ...
                 * functions and resource strings that exist in the header
                 * files.
                 */
