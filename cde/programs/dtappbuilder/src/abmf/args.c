...
static int abmfP_obj_spew_one_class_args(GenCodeInfo genCodeInfo, ABObj obj,
                                         ABMF_ARG_CLASS argClass,
                                         ABMF_ARG_LIST_FORMAT argFormat);

static int spew_extra_item_strings_args(GenCodeInfo genCodeInfo,
                                        ABObj first_item, int num_items,
                                        STRING strings_res_name,
                                        STRING strings_count_res_name);

static int spew_extra_choice_args(GenCodeInfo genCodeInfo, ABObj obj,
                                  ABMF_ARG_CLASS argClasses,
                                  ABMF_ARG_LIST_FORMAT argFormat);

static int spew_extra_list_args(GenCodeInfo genCodeInfo, ABObj obj,
                                ABMF_ARG_CLASS argClasses);

static int spew_extra_spinbox_args(GenCodeInfo genCodeInfo, ABObj obj,
                                   ABMF_ARG_CLASS argClasses);

static BOOL get_wclass_and_res_value(ABObj obj, STRING argName,
...
#define addit(s)                                                               \
        (istr_const(s), strlist_add_str(list_i18nl3_resources, (s), NULL))
...
#define dump_int_resource(inst, name, value)                                   \
        (wroteResource = TRUE, fprintf(fp, "%s.%s:\t%d\n", inst, name, value))
...
#define dump_str_resource(inst, name, value)                                   \
        ...
        if (strval2 != ((STRING)value)) {                                      \
                util_free(strval2)                                             \
        };                                                                     \
        strval2 = NULL;

#define write_str_resource(name, value)                                        \
        (wroteResource = TRUE, abio_printf(fp, "%s, ", name),                  \
         (value == NULL ? abio_puts(fp, nullCStr)                              \
...
#define write_str_literal_resource(name, value)                                \
        (wroteResource = TRUE, abio_printf(fp, "%s, %s", name, (STRING)(value)))
 ...
 */
int abmfP_write_arg_val(GenCodeInfo genCodeInfo,
                        BOOL dumpResource, /* put in res. file format */
                        STRING argName, void *argValue,
        ...
        FILE *fp = (dumpResource ? (genCodeInfo->resource_file)
        ...
        *strval = 0;
        ...
        case AB_ARG_STRING:
                if (dumpResource) {
                        ...
                        /*
                         * We try not to call catgets() on a per-resource basis
                         * because it might lead to cascaded catgets() calls.
                         * However, not doing that means we have to be smart
                         * enough to have a local variable to hold the fetched
                         * string.  We'll handle all those cases we know about
                         * explicitly, and then fall through to using catgets
                         * just to make sure we don't miss something.  Still, we
                         * should try and make sure there are local variables
                         * for all string-valued resources...
                         */

                        /* Local variable case #1: XmNtitle */
                        if (util_streq(argName, XmNtitle) &&
                            (abmfP_title_str_var_has_value(genCodeInfo))) {
                                write_istr_literal_resource(
                                    c_arg_name,
                        ...
                        /* Local variable case #2: XmNvalue (for text objects
                         * *only*) */
        ...
        case AB_ARG_VOID_PTR:
                if (util_streq(argName, XmNentryClass)) {
                        ...
                        if (wclass_name != NULL) {
                                if (dumpResource) {
                                        dump_str_resource(dumpWidgetName,
                                                          argName, wclass_name);
                                } else {
        ...
        case AB_ARG_XMSTRING:
                if (dumpResource) {
                        ...
                        if (util_streq(argName, XmNlabelString)) {
                                if (abmfP_label_xmstr_var_has_value(
                                        genCodeInfo)) {
                                        write_istr_literal_resource(
                                            c_arg_name,
                                ...
                                if (abmfP_acceltext_xmstr_var_has_value(
                                        genCodeInfo)) {
                                        write_istr_literal_resource(
                                            c_arg_name,
                                            abmfP_acceltext_xmstr_var(
                                                genCodeInfo));
                                        wroteResource = TRUE;
                                }
                        } else if (util_streq(argName, XmNokLabelString)) {
                                if (abmfP_ok_label_xmstr_var_has_value(
                                        genCodeInfo)) {
                                        write_istr_literal_resource(
                                            c_arg_name,
                                            abmfP_ok_label_xmstr_var(
                                                genCodeInfo));
                                        wroteResource = TRUE;
                                }
                        } else if (util_streq(argName, XmNdirectory)) {
                                if (abmfP_directory_xmstr_var_has_value(
                                        genCodeInfo)) {
                                        write_istr_literal_resource(
                                            c_arg_name,
                                            abmfP_directory_xmstr_var(
                                                genCodeInfo));
                                        wroteResource = TRUE;
                                }
                        } else if (util_streq(argName, XmNpattern)) {
                                if (abmfP_pattern_xmstr_var_has_value(
                                        genCodeInfo)) {
                                        write_istr_literal_resource(
                                            c_arg_name, abmfP_pattern_xmstr_var(
                                                            genCodeInfo));
                                        wroteResource = TRUE;
                                }
                        }
                        if (!wroteResource) {
                                if (argValue == NULL) {
                                         ...
                                         *items won't work with catgets: need to
                                         *fix that
                                         */
                                        /*
                ...
                if (dumpResource) {
                        ...
                        if (obj_is_item(obj) &&
                            abmfP_mnemonic_array_var(genCodeInfo) != NULL) {
                                sprintf(strval, "%s[i]",
                                        istr_string(abmfP_mnemonic_array_var(
                                            genCodeInfo)));
                                abio_printf(fp,
                                            "%s? (KeySym)XStringToKeysym(%s) : "
                                            "(KeySym)NULL",
                                            strval, strval);
                        } else {
                                sprintf(strval, "\"%s\"",
        ...
        case AB_ARG_PIXMAP:
                if (dumpResource) {
                        ...
                        if (util_streq(argName, XmNlabelPixmap)) {
                                if (abmfP_label_pixmap_var_has_value(
                                        genCodeInfo)) {
                                        write_str_literal_resource(
                                            c_arg_name,
                                            istr_string(abmfP_label_pixmap_var(
                                                genCodeInfo)));
                                }
                        } else if (util_streq(argName, XmNiconPixmap)) {
                                if (abmfP_icon_pixmap_var_has_value(
                                        genCodeInfo)) {
                                        write_str_literal_resource(
                                            c_arg_name,
                                            istr_string(abmfP_icon_pixmap_var(
                                                genCodeInfo)));
                                }
                        } else if (util_streq(argName, XmNiconMask)) {
                                if (abmfP_icon_mask_pixmap_var_has_value(
                                        genCodeInfo)) {
                                        write_str_literal_resource(
                                            c_arg_name,
                                            istr_string(
                                                abmfP_icon_mask_pixmap_var(
                                                    genCodeInfo)));
                                }
                        }
                }
        ...
        case AB_ARG_BOOLEAN:
                if (dumpResource) {
                        ...
                        if (obj_is_item(obj) && (util_streq(argName, XmNset))) {
                                ...
                                if (genCodeInfo->i18n_method !=
                                    ABMF_I18N_XPG4_API)
                                        abio_printf(
                                            fp, "%s, %s[%s]", c_arg_name,
                                            abmfP_get_c_array_of_selected_bools_name(
                                                obj),
                                            istr_string(abmfP_counter_var(
                                                genCodeInfo)));
                                else
                                        abio_printf(
                                            fp, "%s, %s", c_arg_name,
                                ...
                                abio_printf(fp, "%s, %s", c_arg_name,
                                            (((Boolean)(unsigned long)argValue)
                                                 ? "True"
                                                 : "False"));
                        }
                }
        ...
        case AB_ARG_WIDGET: {
                ...
                if (module != NULL) {
                        ...
                        attachObj =
                            obj_scoped_find_by_name(module, attachObjName);
                }
                assert(attachObj != NULL);
                assert(!dumpResource); /* can't dump widget resources */

                write_str_literal_resource(
                    c_arg_name, abmfP_get_c_name(genCodeInfo, attachObj));
        } break;
#ifdef DEBUG
        default:
 ...
 */
int abmfP_write_arg_val_line(GenCodeInfo genCodeInfo,
                             BOOL dumpResource, /* put in res. file format */
                             STRING argName, void *argValue,
                             ABObj obj, /* obj the arg belongs to */
                             ABMF_ARG_LIST_FORMAT argFormat) {
...
int abmfP_write_resource(GenCodeInfo genCodeInfo,
                         BOOL dumpResource, /* put in res. file format */
                         Arg *arg,          /* argument to format/write*/
                         ABObj obj          /* obj the arg belongs to */
) {
        return abmfP_write_arg_val(genCodeInfo, dumpResource, arg->name,
                                   (void *)(arg->value), obj);
}
 ...
 */
int abmfP_obj_spew_args(GenCodeInfo genCodeInfo, ABObj obj,
                        ABMF_ARG_CLASS argClasses,
                        ABMF_ARG_LIST_FORMAT argFormat) {
        int return_value = 0;
        ...
        if ((argClasses & ABMF_ARGCLASS_OTHER) != 0) {
                iRC = abmfP_obj_spew_one_class_args(
                    genCodeInfo, obj, ABMF_ARGCLASS_OTHER, argFormat);
                return_if_err(iRC, iRC);
        }

        if ((argClasses & ABMF_ARGCLASS_SENSITIVITY) != 0) {
                iRC = abmfP_obj_spew_one_class_args(
                    genCodeInfo, obj, ABMF_ARGCLASS_SENSITIVITY, argFormat);
                return_if_err(iRC, iRC);
        }

        if ((argClasses & ABMF_ARGCLASS_TYPED) != 0) {
                iRC = abmfP_obj_spew_one_class_args(
                    genCodeInfo, obj, ABMF_ARGCLASS_TYPED, argFormat);
                return_if_err(iRC, iRC);
        }

        if ((argClasses & ABMF_ARGCLASS_WIDGET_REF) != 0) {
                iRC = abmfP_obj_spew_one_class_args(
                    genCodeInfo, obj, ABMF_ARGCLASS_WIDGET_REF, argFormat);
                return_if_err(iRC, iRC);
        }
        ...
        for (objCount = 0; objCount < 2; ++objCount) {
                if (objCount == 0) {
                        ...
                        if ((actualObj == NULL) || (actualObj == obj)) {
                                ...
                                /* we have a reference to another obj - dump its
                                 * resources as if they were on this obj.
                                 */
                                argList = objxm_obj_get_ui_args(actualObj);
                        }
                }

                for (arg = (Arg *)argList; (arg != NULL) && (arg->name != NULL);
                     ++arg) {
                        switch (argClass) {
                        ...
                        if (abmfP_arg_is_dumped(genCodeInfo, arg->name, obj)) {
                                abmfP_write_resource(genCodeInfo, TRUE, arg,
                                                     obj);
                        } else if (abmfP_arg_is_written(genCodeInfo, arg->name,
                                                        obj)) {
                                    ...
                                    (argFormat == ABMF_ARGFMT_ARRAY)) {
                                        ...
                                        if ((!abmfP_label_pixmap_var_has_value(
                                            ...
                                            ((util_streq(arg->name,
                                             ...
                                             (util_streq(arg->name,
                                                         XmNlabelType)))) {
                                                /* Don't write label resources -
                                                 * a dtb_ convenience function
                                                 * will be written out that sets
                                                 * the label.
                                                 */
                                        } else
                                                switch (argFormat) {
                                                case ABMF_ARGFMT_VA_LIST:
                                                        if (abmfP_write_resource(
                                                                genCodeInfo,
                                                                FALSE, arg,
                                                                obj) >= 0) {
                                                                abio_printf(
        ...
        if (abmfP_arg_counter_var(genCodeInfo) &&
            abmfP_get_c_array_of_xmstrings_name(first_item)) {
                ...
                abio_printf(codeFile,
                            ...
                            "%s, (XtArgVal)%d); %s++;\n",
                            istr_string(abmfP_arg_counter_var(genCodeInfo)),
                            strings_count_res_name, num_items,
                            istr_string(abmfP_arg_counter_var(genCodeInfo)));
        }
        return 0;
}
        ...
        if (((argClasses & ABMF_ARGCLASS_OTHER) != 0) && (numItems > 0)) {
                ...
                if (menuObj != NULL) {
                        ...
                        abmfP_write_arg_val_line(
                            ...
                            argName, objName, menuObj, argFormat);
...
static int spew_extra_list_args(GenCodeInfo genCodeInfo, ABObj obj,
         ...
         */
        if (obj_is_actual_obj && (obj_get_root(obj) == obj_get_root(parent)) &&
            (ObjWClassIsScrolledWindow(parent))) {
                abmfP_obj_spew_args(genCodeInfo, parent, argClasses,
                                    ABMF_ARGFMT_ARRAY);

                if ((num_items > 0) &&
                    ((argClasses & ABMF_ARGCLASS_OTHER) != 0))
                        spew_extra_item_strings_args(genCodeInfo, first_item,
                                                     num_items, "XmNitems",
                                                     "XmNitemCount");
        }
        return 0;
...
static STRING fmt_strval_for_res_file(STRING string) {
        ...
        if (!modified) {
                ...
                if ((newStr = (STRING)util_malloc((newStrLen + 1) *
                                                  sizeof(char))) == NULL) {
                        return NULL;
                }
                newStrI = 0;
 ...
 * not stored anywhere.
 */
BOOL abmfP_arg_is_spewed(GenCodeInfo genCodeInfo, STRING argName, ABObj obj) {
        /*
         ...
         */
        return (obj_is_item(obj) || (!arg_has_default_value(obj, argName)));
}

/*
 * TRUE if the arg is dumped to the resource file
 */
BOOL abmfP_arg_is_dumped(GenCodeInfo genCodeInfo, STRING argName, ABObj obj) {
 ...
 */
BOOL abmfP_arg_is_written(GenCodeInfo genCodeInfo, STRING argName, ABObj obj) {
...
BOOL abmfP_arg_is_widget_ref(STRING argName) {
         ...
         * layout.
         */
        return ((abmfP_get_res_type(argName, NULL) == AB_ARG_WIDGET) &&
                (!util_streq(argName, XmNsubMenuId)));
}
 ...
 */
int abmfP_get_num_args_of_classes(ABObj obj, ABMF_ARG_CLASS argClasses) {
        int num_args = 0;
        ABObj actualObj = obj_get_actual_obj(obj);
        BOOL countSensitivity = ((argClasses & ABMF_ARGCLASS_SENSITIVITY) != 0);
        ...
        if (actualObj != obj) {
                num_args +=
                    abmfP_get_num_args_of_classes(actualObj, argClasses);
        }
...
int abmfP_xt_va_list_open(GenCodeInfo genCodeInfo) {
        abio_indent(genCodeInfo->code_file);
        return 0;
}

int abmfP_xt_va_list_open_setvalues(GenCodeInfo genCodeInfo, ABObj obj) {
...
int abmfP_xt_va_list_close(GenCodeInfo genCodeInfo) {
...
int abmfP_comp_get_num_args_of_classes(ABObj obj, ABMF_ARG_CLASS argClasses) {
        int num_args = 0;
...
static BOOL arg_has_default_value(ABObj obj, STRING argName) {
        ...
        if (util_streq(argName, XmNradioBehavior)) {
                if (obj_is_choice(compRoot)) {
                        /* return TRUE, if the boolean values are logically
                         * equal */
