...
static int write_define_var(GenCodeInfo genCodeInfo, ISTRING *struct_field_ptr,
                            STRING type, ISTRING var, STRING value);
        ...
        if (obj_get_num_items(menu) != 0) {
                if (abmfP_items_have_accelerators(menu))
                        ...
                        if (genCodeInfo->i18n_method != ABMF_I18N_XPG4_API) {
                                ...
                                write_items_strings_array_def(genCodeInfo,
                                                              menu);
                                write_define_counter_var(genCodeInfo);
                                if (abmfP_items_have_mnemonics(menu))
                                        write_items_mnemonics_array_def(
                                            genCodeInfo, menu);
                                write_items_widgets_array_def(genCodeInfo,
                                                              menu);
                        }
                }
        }
...
static int write_combobox_defs(GenCodeInfo genCodeInfo, ABObj combobox) {
        File codeFile = genCodeInfo->code_file;
...
int write_file_chooser_defs(GenCodeInfo genCodeInfo, ABObj fchooser) {
...
#define struct_field (*struct_field_ptr)
        ...
        abio_printf(codeFile, "%s\t%s", (type), istr_string(var));
        if (value != NULL) {
...
static int write_help_defs(GenCodeInfo genCodeInfo, ABObj obj) {
        ...
        if (obj_has_help_data(obj)) {
                obj_get_help_data(obj, &help_volume, &help_location,
                                  &help_text);

                /*
                ** If this is an I18N-ized app, we can't compile in the help
                *text,
                ** but must instead load it at runtime via catgets().  We still
                *need
                ** the static HelpData info, but we just force the help text
                *part to
                ...
                */
                if (genCodeInfo->i18n_method == ABMF_I18N_XPG4_API) {
                        abio_puts(
                            codeFile,
                            "static DtbObjectHelpDataRec\thelp_data = {\n");
                        abio_indent(codeFile);
                        abio_puts(codeFile, "\"\",\n");
