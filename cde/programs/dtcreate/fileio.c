...
void SetCookie(FILE *fp) {
        ...
        if (!keystr) {
#ifdef DEBUG
                printf("No '$KEY' keyword in file. Magic Cookie not written to "
                       "file.\n");
#endif
                return;
        }
...

{
...
#endif
                msgPtr =
                    GETMESSAGE(5, 55, "Could not create the following file:");
                fmtPtr = "%s\n   %s";
                errPtr = XtMalloc(
                    (strlen(msgPtr) + strlen(fmtPtr) + strlen(pszFile) + 1) *
                    sizeof(char));
        ...
        printf("CA flags = %X\n", pAD->fsFlags);
#endif
        if (pAD->fsFlags & CA_WT_TERM)
                fprintf(fp, "     WINDOW_TYPE   TERMINAL\n");
        else if (pAD->fsFlags & CA_WT_PERMTERM)
                fprintf(fp, "     WINDOW_TYPE   PERM_TERMINAL\n");
        else if (pAD->fsFlags & CA_WT_SHELLTERM)
                fprintf(fp, "     WINDOW_TYPE   SHELL_TERMINAL\n");
        else
                fprintf(fp, "     WINDOW_TYPE   NO_STDIO\n");

        if ((pAD->papFiletypes) && (pAD->fsFlags & CA_DF_ONLYFTFILES)) {
                fprintf(fp, "     ARG_TYPE      %s",
                        (pAD->papFiletypes[0])->pszName);
...

{
        ...
        fprintf(fp, "#\n");
        fprintf(fp,
                "#  $TOG: fileio.c /main/9 1998/10/26 17:18:19 mgreess $\n");
        fprintf(fp, "#\n");
        fprintf(fp, "################################################\n");
        fprintf(fp, "\n");
        fprintf(fp, "# This file represents a CDE Dt action.  The contents of "
                    "the file do\n");
        fprintf(
            fp,
            "# not matter; however, the file must have executable permission "
            "and the\n");
        fprintf(fp,
                "# name of the file must match an entry in the action database "
                "(*.dt files).\n");
        fprintf(fp, "#\n");
        fprintf(
            fp,
...

{
        ...
        if (CheckCookie(fp)) {
                ...
                if (rc) {
                        msgPtr = GETMESSAGE(
                            5, 90, "Could not parse the following file:");
                        fmtPtr = "%s\n   %s";
...
/*                                                                           */
/*****************************************************************************/
char *CreateIconName(char *pszDirName, char *pszBaseName,
                     enum icon_size_range enumSize, char *pszType,
                     Boolean bIsMask) {
        char *pszName;
        ...
        if (pszDirName) {
                pszName = XtMalloc(strlen(pszDirName) + strlen(pszBaseName) +
                                   strlen(pszSize) + strlen(pszType) + 1);
                if (pszName) {
                        sprintf(pszName, "%s%s%s%s", pszDirName, pszBaseName,
                                pszSize, pszType);
                }
        } else {
                pszName = XtMalloc(strlen(pszBaseName) + strlen(pszSize) +
                                   strlen(pszType) + 1);
                if (pszName) {
                        sprintf(pszName, "%s%s%s", pszBaseName, pszSize,
                                pszType);
                }
        }
        return (pszName);
...

{
        ...
        if (fdSource == -1) {
                ...
                fmtPtr = "%s\n   %s";
                errPtr = XtMalloc(
                    (strlen(msgPtr) + strlen(fmtPtr) + strlen(pszSource) + 1) *
                    sizeof(char));
                ...
                if ((rlen = read(fdSource, buffer, len)) != len) {
                        ...
                        if (fdDest == -1) {
                                ...
                                fmtPtr = "%s\n   %s";
                                errPtr =
                                    XtMalloc((strlen(msgPtr) + strlen(fmtPtr) +
                                              strlen(pszDest) + 1) *
                                             sizeof(char));
                                ...
                                if ((rlen = write(fdDest, buffer, len)) !=
                                    len) {
                                        /***************************************************/
...
char *CreateDirName(void) {
        ...
        if (pszEnvVar && strlen(pszEnvVar)) {
                pszDirName =
                    XtMalloc(strlen(pszEnvVar) + strlen("/.dt/icons/") + 1);
...

{
        ...
        if ((!bHaveError) && (pIconData)) {
                pszIconName = CreateIconName(
                    pszDirName, pszNewName, pIconData->size, PIXMAP_EXT, FALSE);
                if ((!pszIconName) ||
                    (CopyFile(pIconData->pmFileName, pszIconName))) {
                        bHaveError = True;
                }
                if (pszIconName)
                        XtFree(pszIconName);
        ...
        if ((!bHaveError) && (pIconData)) {
                pszIconName = CreateIconName(
                    pszDirName, pszNewName, pIconData->size, BITMAP_EXT, FALSE);
                if ((!pszIconName) ||
                    (CopyFile(pIconData->bmFileName, pszIconName))) {
                        bHaveError = True;
                }
                if (pszIconName)
                        XtFree(pszIconName);
...
#endif
                                pszIconName = CreateIconName(
                                    pszDirName, pszNewName, pIconData->size,
                                    BITMAP_EXT, TRUE);
                                if ((!pszIconName) ||
                                    (CopyFile(pszMaskName, pszIconName))) {
                                        bHaveError = True;
                                }
                                if (pszIconName)
                                        XtFree(pszIconName);
...
Boolean FiletypeHasIcon(FiletypeData *pFtD) {
        if ((pFtD->pszMedPmIcon) || (pFtD->pszMedBmIcon) ||
            (pFtD->pszTinyPmIcon) || (pFtD->pszTinyBmIcon)) {
                return (True);
        } else {
                return (False);
        }
...
#define CIF_SUCCESS 0
        ...
        if (ActionHasIcon()) {
                if (!bHaveError &&
                    (CopyFilesFromWid(pAD, CA_LRG_IconGadget) != 0)) {
                        bHaveError = TRUE;
                }
                if (!bHaveError &&
                    (CopyFilesFromWid(pAD, CA_MED_IconGadget) != 0)) {
                        bHaveError = TRUE;
                }
                if (!bHaveError &&
                    (CopyFilesFromWid(pAD, CA_TINY_IconGadget) != 0)) {
                        bHaveError = TRUE;
                }
        }
                ...
                for (i = 0; i < pAD->cFiletypes; i++) {
                        ...
                        if (FiletypeHasIcon(pFtD)) {
                                FIND_ICONGADGET_ICON(pFtD->pszIcon,
                                                     pszOldIconName, DtMEDIUM);
                                if (pszOldIconName) {
                                        ptr =
                                            strstr(pszOldIconName, MEDIUM_EXT);
                                        if (ptr)
                                                *ptr = '\0';
                                }
                                ...
                                /* Medium Pixmap */
                                /*****************************************************************/
                                pszNewIconName = CreateIconName(
                                    pszDirName, pFtD->pszName, Medium_Icon,
                                    PIXMAP_EXT, FALSE);
                                if (pszNewIconName) {
                                        if (pFtD->pszMedPmIcon) {
                                                CopyFile(pFtD->pszMedPmIcon,
                                                         pszNewIconName);
                                        } else {
                                                ptr = CreateIconName(
                                                    (char *)NULL,
                                                    pszOldIconName, Medium_Icon,
                                                    PIXMAP_EXT, FALSE);
                                                if (ptr) {
                                                        CopyFile(
                                                            ptr,
                                ...
                                pszNewIconName = CreateIconName(
                                    pszDirName, pFtD->pszName, Medium_Icon,
                                    BITMAP_EXT, FALSE);
                                pszNewIconMask = CreateIconName(
                                    pszDirName, pFtD->pszName, Medium_Icon,
                                    BITMAP_EXT, TRUE);
                                if (pszNewIconName) {
                                        if (pFtD->pszMedBmIcon) {
                                                CopyFile(pFtD->pszMedBmIcon,
                                                         pszNewIconName);
                                                /****************************/
                                ...
                                /*****************************************************************/
                                pszNewIconName = CreateIconName(
                                    pszDirName, pFtD->pszName, Tiny_Icon,
                                    PIXMAP_EXT, FALSE);
                                if (pszNewIconName) {
                                        if (pFtD->pszTinyPmIcon) {
                                                CopyFile(pFtD->pszTinyPmIcon,
                                                         pszNewIconName);
                                        } else {
                                                ptr = CreateIconName(
                                                    (char *)NULL,
                                                    pszOldIconName, Tiny_Icon,
                                                    PIXMAP_EXT, FALSE);
                                                if (ptr) {
                                                        CopyFile(
                                                            ptr,
                                ...
                                pszNewIconName = CreateIconName(
                                    ...
                                    BITMAP_EXT, FALSE);
                                pszNewIconMask =
                                    CreateIconName(pszDirName, pFtD->pszName,
                                                   Tiny_Icon, BITMAP_EXT, TRUE);
                                if (pszNewIconName) {
                                        if (pFtD->pszTinyBmIcon) {
                                                CopyFile(pFtD->pszTinyBmIcon,
                                                         pszNewIconName);
                                                /****************************/
                                                /* Now do the mask file.    */
                                                /****************************/
