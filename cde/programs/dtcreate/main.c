...
void GetSessionInfo(void) {
        ...
        /************************************************************************/
        /*  UxTopLevel is used because CreateActionAppShell isn't created
         * yet...*/
        /*  okay because it only uses it to get a display, not a window */
        /************************************************************************/
        if (DtSessionRestorePath(UxTopLevel, &path,
                                 sessiondata.pszSessionFile) == False)
        ...
        if ((db = XrmGetFileDatabase(path)) == NULL) {
                msgPtr =
                    GETMESSAGE(5, 162, "Could not restore session from file:");
                fmtPtr = "%s\n %s\n";
                errPtr = XtMalloc(
                    (strlen(msgPtr) + strlen(fmtPtr) + strlen(path) + 1) *
                    sizeof(char));
        ...
        if (sessiondata.width < DEFAULT_WIDTH) sessiondata.width =
        DEFAULT_WIDTH;
        */
        ...
        if (sessiondata.height < DEFAULT_HEIGHT) sessiondata.height =
        DEFAULT_HEIGHT;
        */
...
void SaveSession(void) {
        ...
        if (DtWsmGetWorkspacesOccupied(display, XtWindow(CreateActionAppShell),
                ...
                for (j = 0; j < num_workspaces; j++) {
                        ...
                        if (j == 0)
                                (void)strcpy(all_workspace_names,
                                             workspace_name);
                        else {
        ...
        XSetCommand(XtDisplay(CreateActionAppShell),
                    XtWindow(CreateActionAppShell), xa_CommandStr, n);
        XtFree((char *)path);
        XtFree((char *)name);
        ...
        for (i = 0; i < ICON_NUMBER; i++) {
                if ((IconDataList[i]->pmDirtyBit) &&
                    ...
                    (strlen(IconDataList[i]->pmFileName))) {
                        ...
                        /* This is bogus.  But just in case the user specified a
                         * bitmap */
                        /* rather than a pixmap during the edit process, this
                         * will make */
                        /* sure that file gets removed.  The chance of this
                         * occurring   */
                        /* is very unlikely. */
...
Tt_callback_action IconEdit_tt_handler(Tt_message m, Tt_pattern p) {
        ...
        if (!strcmp(op, "Edit")) {
                ...
                case TT_FAILED:
                        ...
                        fmtPtr = "%s = %p";
                        errPtr =
                            XtMalloc((strlen(msgPtr) + strlen(fmtPtr) + 20) *
                                     sizeof(char));
                ...
                if ((pIconData) && (!strcmp(op, "Saved"))) {
                        tt_message_arg_bval(m, 0, (unsigned char **)&ttbuf,
                                            &blen);
                        ...
                        if (bIsNewFile) {
                                tmpfd =
                                    open(tmpIconFile,
                                         O_CREAT | O_WRONLY | O_NDELAY, 0666);
                        } else {
                                tmpfd =
                                    open(tmpIconFile,
                                         O_TRUNC | O_WRONLY | O_NDELAY, 0666);
                        }
                        if (tmpfd == -1) {
                                msgPtr = GETMESSAGE(
                                    5, 60,
                                ...
                                errPtr =
                                    XtMalloc((strlen(msgPtr) + strlen(fmtPtr) +
                                              strlen(tmpIconFile) + 1) *
                                             sizeof(char));
                                sprintf(errPtr, fmtPtr, msgPtr, tmpIconFile);
                                display_error_message(XtParent(pIconData->wid),
                                                      errPtr);
                                XtFree(errPtr);
                                tt_release(ttMark);
                        ...
                        /*******************************************************************/
                        /* Determine if this is a pixmap or bitmap.  If it is a
                         * pixmap,    */
                        /* handle as a one file buffer.  If a bitmap, then it
                         * may be       */
                        ...
                        if ((ttbuf) && (!strncmp(ttbuf, "/* XPM */", 9))) {
                                ...
                                if ((wlen = write(tmpfd, ttbuf, tmplen)) ==
                                    tmplen) {
                                        if (tmpfd > -1)
                                                close(tmpfd);
                                } else {
                                        /* INTERNATIONALIZE */
                                        msgPtr = "Could not write to the "
                                                 "following file:";
                                        fmtPtr = "%s\n   %s";
                                        errPtr = XtMalloc(
                                            (strlen(msgPtr) + strlen(fmtPtr) +
                                             strlen(tmpIconFile) + 1) *
                                            sizeof(char));
                                        sprintf(errPtr, fmtPtr, msgPtr,
                                                tmpIconFile);
                                        display_error_message(
                                            XtParent(pIconData->wid), errPtr);
                                        XtFree(errPtr);
                                        tt_release(ttMark);
                                        if (tmpfd > -1)
                                                close(tmpfd);
                                ...
                                /****************************************************************/
                                /* Now determine if there is a second bitmap and
                                 * its size.      */
                                /****************************************************************/
                                if (tmp) {
                                        tmp1 = strstr(tmp, "};");
                                ...
                                if (tmp1) {
                                        ...
                                        */
                                        ...
                                        printf("Troy tmplen = %d\n",
                                               tmplen); /* debug */
#endif
                                        if ((wlen = write(tmpfd, tmp,
                                                          tmplen)) == tmplen) {
                                                if (tmpfd > -1)
                                                        close(tmpfd);
                                        } else {
                                                /* INTERNATIONALIZE */
                                                msgPtr = "Could not write to "
                                                         "the following file:";
                                                fmtPtr = "%s\n   %s";
                                                errPtr = XtMalloc(
                                                    (strlen(msgPtr) +
                                        ...
                                        /****************************************************************/
                                        /* Must not have a mask, so lets clean
                                         * up any old mask file.    */
                                        /****************************************************************/
                                } else {
#ifdef DEBUG
                                        printf("Remove maskFile = %s\n",
                        ...
                        /****************************************************************/
                        /* Set the icon gadget with new icon, and flag the dirty
                         * bit.   */
                        /****************************************************************/
                        if (((pIconData->pmMsgID) &&
                             (!strcmp(pIconData->pmMsgID, msgID)) &&
                            ...
                            ((pIconData->bmMsgID) &&
                             ...
                             (!bShowPixmaps))) {
                                ...
                                printf("Change icon image for widget = %p\n",
                                       pIconData->wid); /* debug */
#endif
                                SET_ICONGADGET_ICON(pIconData->wid,
                                                    tmpIconFile);
                        }
                        if ((pIconData->pmMsgID) &&
                            (!strcmp(pIconData->pmMsgID, msgID))) {
...
void UxDoEditPixmap(Widget wid, char *fname) {
        ...
        if (fname) {
                ...
                if (fd1 == -1) {
                        ...
                        msgPtr = GETMESSAGE(
                            5, 60, "Could not open the following file:");
                        fmtPtr = "%s\n   %s";
                        ...
                        /*************************************************************/
                        /* Read first few bytes to determine what kind of file
                         * it is */
                        /*************************************************************/
                        read(fd1, tmpbuf, 10);
                        (void)lseek(fd1, 0, SEEK_SET);
                        ...
                        /******************************************************************/
                        /* If the file is a bitmap, then check for its mask file
                         * and add. */
                        /******************************************************************/
...
#endif
                        buffer = (unsigned char *)XtMalloc(
                            (Cardinal)(lenFile1 + lenFile2 + 1));
                        memset(buffer, 0, (lenFile1 + lenFile2 + 1));
                        if (buffer) {
                                /***************************************/
                                /* Read file1 into buffer.             */
                                /***************************************/
                                if ((rlenFile1 = read(fd1, buffer, lenFile1)) !=
                                    lenFile1) {
                                        ...
                                        fmtPtr = "%s\n   %s";
                                        errPtr = XtMalloc((strlen(msgPtr) +
                                                           strlen(fmtPtr) +
                                                           strlen(fname) + 1) *
                                                          sizeof(char));
                                        sprintf(errPtr, fmtPtr, msgPtr, fname);
                                        display_error_message(XtParent(wid),
                                                              errPtr);
                                        XtFree(errPtr);
                                        if (mname)
                                                XtFree(mname);
...
#endif
                                        if ((rlenFile2 =
                                                 read(fd2, ptr, lenFile2)) !=
                                            lenFile2) {
                                                /* didn't read whole file! */

                                                /* INTERNATIONALIZE */
                                                msgPtr = "Could not read the "
                                                         "entire file:";
                                                fmtPtr = "%s\n   %s";
                                                errPtr = XtMalloc(
                                                    (strlen(msgPtr) +
                ...
                tt_message_barg_add(m, TT_INOUT, "XPM",
 ...
 *----------------------------------------------------------*/
                ...
                for (i = 1; i < argc; i++) {
                        if ((argv[i][0] == '-') && (argv[i][1] == '?')) {
                                ...
                                printf(
                                    ...
                                    GETMESSAGE(
                                        5, 110,
                                        "Usage: dtcreate [<filename>] [-?]"));
                                exit(0);
                        } else {
                                if (argv[i][0] == '-') {
                                        if (!strcmp(argv[i], "-s")) {
                                                i += 1;
                                                sessiondata.pszSessionFile =
                                                    (char *)XtMalloc(
                                                        strlen(argv[i]) + 1);
                                                strcpy(
                                                    sessiondata.pszSessionFile,
        ...
        UxTopLevel = XtAppInitialize(&UxAppContext, CLASS_NAME, NULL, 0, &argc,
                                     argv, NULL, NULL, 0);
        ...
        /*------------------------------------------------------------------*/
        xa_WM_SAVE_YOURSELF = XInternAtom(XtDisplay(CreateActionAppShell),
