...
void putCAactionOpensText(struct _ActionData *ca_struct) {
        PutWidgetTextString(CA_XprtActionOpenText,
                            CA_ActionOpensText(ca_struct));
        return;
}
...
void putCAwindowType(struct _ActionData *ca_struct) {
        if (ca_struct->fsFlags & CA_WT_XWINDOWS) {
                ...
                */
                XtVaSetValues(CA_WindowTypeArea, XmNmenuHistory, CA_WinTypeX,
                              NULL);
        } else if (ca_struct->fsFlags & CA_WT_TERM) {
                XtVaSetValues(CA_WindowTypeArea, XmNmenuHistory,
                              CA_WinTypeAutoClose, NULL);
        } else if (ca_struct->fsFlags & CA_WT_PERMTERM) {
                XtVaSetValues(CA_WindowTypeArea, XmNmenuHistory,
                              CA_WinTypeManualClose, NULL);
        } else {
                /*XtVaSetValues(CA_WindowTypeArea,XmNmenuHistory,CA_WinTypeNoOutput,NULL);*/
                XtVaSetValues(CA_WindowTypeArea, XmNmenuHistory, CA_WinTypeX,
                              NULL);
...
void set_ca_dialog_height(void) {
        ...
        if (!showExpert) {
                ...
                XtVaSetValues(CA_Expand, XmNbottomAttachment, XmATTACH_NONE,
                              NULL);
                XtUnmanageChild(XprtOptionForm);

                if (!sessiondata.useSession) {
                        /* give window new height */
                        XtVaSetValues(CreateActionAppShell, XmNheight,
                                      newheight, NULL);
                }

                XtVaSetValues(CA_Expand, XmNbottomAttachment, XmATTACH_FORM,
                              NULL);
        } else {
                XtVaSetValues(
                    CA_Expand,
...
void activateCB_FileOpen(Widget wid, XtPointer cdata, XtPointer cbstruct) {
        ...
        XtVaSetValues(OpenFile, RES_CONVERT(XmNdirMask, "~/.dt/types/*.dt"),
                      ...
                      RES_CONVERT(XmNdirectory, "~/.dt/types/"), NULL);
...
void activateCB_FileQuit(Widget wid, XtPointer cdata, XtPointer cbstruct) {
        ...
        /**************************************************************************/
        /* Check if anything has changed.  If so ask if they really want to
         * quit. */
        /**************************************************************************/
        if (compareAD(&AD, pMyCopyAD)) {
                msgPtr =
                    GETMESSAGE(5, 150, "Your current work has not been saved.");
                msgPtr = XtNewString(msgPtr);

                /* CDExc19439 - Save GETMESSAGE results to buffers since it can
                 */
                /* return an internal buffer which can be overwritten on each
                 * call. */
                ...
                display_question_message(CreateActionAppShell, msgPtr, savePtr,
                                         (XtCallbackProc)saveCB_QuestionDialog,
...
void activateCB_ExpertOption(Widget wid, XtPointer client_data,
        ...
        if (!XtIsManaged(XprtOptionForm)) {
                ...
                XtVaSetValues(CA_Expand, XmNbottomAttachment, XmATTACH_NONE,
                              NULL);
                XtVaSetValues(XprtOptionForm, XmNbottomAttachment,
                              XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM,
                              XmNrightAttachment, XmATTACH_FORM, XmNwidth,
                              ca_width, NULL);
                XtManageChild(XprtOptionForm);
                XtVaSetValues(
                    CA_Expand, XmNbottomAttachment, XmATTACH_WIDGET,
                    XmNbottomWidget, XprtOptionForm, XmNbottomOffset, 10,
                    /*XmNlabelString, GETMESSAGE(3, 52, "Basic"),*/
                    RES_CONVERT(XmNlabelString, GETMESSAGE(3, 52, "Basic")),
                    NULL);
                XtVaSetValues(CA_HelpTextWindow, XmNheight, wintypeheight,
                              NULL);
                /*XtVaSetValues (CA_WindowTypeArea, XmNheight, wintypeheight,
                 * NULL);*/
        } else {
                /**********************************************************************/
                ...
                XtUnmanageChild(XprtOptionForm);
                XtVaSetValues(
                    CA_Expand, XmNbottomAttachment, XmATTACH_FORM,
                    XmNbottomOffset, 15,
...
void valueChangedCB_ColorMonoOption(Widget wid, XtPointer client_data,
        ...
        for (i = 0; i < 3; i++) {
                ...
                if (pIconData) {
                        if (bShowPixmaps) {
                                SET_ICONGADGET_ICON(widIcons[i],
...
void activateCB_action_icon(Widget wid, XtPointer client_data,
        ...
        if (cbs->reason == XmCR_ACTIVATE) {
                ...
                if (last_action_pushed && (last_action_pushed != wid)) {
                        XtVaSetValues(XtParent(last_action_pushed),
...
void activateCB_add_filetype(Widget wid, XtPointer client_data,
        ...
        if (!CreateActionAppShellCheckFields()) {
                ...
                pFtD->pszName = (char *)XtMalloc(
                    strlen(ptr) + strlen("_FILE_") + strlen(tmpbuf) + 1);
                ...
                */
                ...
                XtVaSetValues(AddFiletype, RES_CONVERT(XmNdialogTitle, title),
                              XmNuserData, pFtD, NULL);
...
'Datatypes That Use This Action' list."));
                ...
                fmtPtr = "%s\n%s";
                len = (strlen(msgPtr1) + strlen(msgPtr2) + strlen(fmtPtr) + 3);
                errPtr = XtMalloc(len);
...
Boolean CreateActionAppShellCheckFields(void) {
        ...
        if (!bError) {
                ...
                if (!ptr) {
                        msgPtr =
                            GETMESSAGE(5, 10, "The Action Name is missing.\n\
Please enter a name in the 'Action Name' field.");
                        errPtr = XtNewString(msgPtr);
                        display_error_message(CreateActionAppShell, errPtr);
                        XtFree(errPtr);
                        XmProcessTraversal(CA_ActionNameTextField,
                                           XmTRAVERSE_CURRENT);
                        bError = TRUE;
                } else {
                        XtFree(ptr);
...
#endif
                if (!ptr) {
                        msgPtr = GETMESSAGE(5, 15,
                                            "The execute command is missing.\n\
...
'Command To Execute When Double-clicked' field.");
                        ...
                        /********************************************************************/
                        /* Now check to see if there is a prompt string.  If so,
                         * make sure  */
                        ...
                        if (ptrPrompt) {
                                if (!strchr(ptr, '$')) {
                                        msgPtr = GETMESSAGE(
                                            5, 30,
                                            "A variable name is missing in the\n\
'Command To Execute When Double-clicked' field.\n\
...
Valid variable names are $*, $1, $2, ..., $9.");
                                        ...
                                        display_error_message(
                                            CreateActionAppShell, errPtr);
                                        XtFree(errPtr);
                                        XmProcessTraversal(CA_DblClkText,
                                                           XmTRAVERSE_CURRENT);
                                        bError = TRUE;
                                }
                                XtFree(ptrPrompt);
...
or add at least one datatype to the list.");
                        ...
                        display_error_message(CreateActionAppShell, errPtr);
                        XtFree(errPtr);
                        XmProcessTraversal(CA_AllFiletypesToggle,
                                           XmTRAVERSE_CURRENT);
                        bError = TRUE;
                }
        }
...
ActionData *copyAD(ActionData *pAD) {
        ...
        if (pAD->papFiletypes) {
                pcopyAD->papFiletypes = (FiletypeData **)XtMalloc(
                    sizeof(FiletypeData *) * (pAD->cFiletypes + 1));
                for (i = 0; i < pAD->cFiletypes; i++) {
                        pcopyAD->papFiletypes[i] =
                            copyFtD(pAD->papFiletypes[i]);
                }
        }
...
Boolean compareAD(ActionData *pAD1, ActionData *pAD2) {
        ...
        if (pAD1->papFiletypes) {
                for (i = 0; i < pAD1->cFiletypes; i++) {
                        if (compareFtD(pAD1->papFiletypes[i],
                                       pAD2->papFiletypes[i])) {
                                return (NOT_EQUAL);
                        }
                }
