...
/*                                                                         */
/***************************************************************************/
int myscandir(char *pszDirName, struct dirent ***NameList, int Select(),
              int Compare())
...
#define AINCREMENT ((int)512)
        ...
        if (!(dirptr = opendir(pszDirName))) {
                ...
                fmtPtr = "%s\n   %s";
                errPtr = XtMalloc(
                    (strlen(msgPtr) + strlen(fmtPtr) + strlen(pszDirName) + 1) *
                    sizeof(char));
...

{
        ...
        *filecount = 0;
        ...
        if (dirptr) {
                ...
                *filecount =
                    myscandir(dirname, &namelist, new_name_select, NULL);
                ...
                fmtPtr = "%s\n   %s";
                errPtr = XtMalloc(
                    (strlen(msgPtr) + strlen(fmtPtr) + strlen(dirname) + 1) *
                    sizeof(char));
...
char *update_filter(char *filter) {
...
#ifdef DEBUG
        if (!filter_end)
                printf("Error callocing memory for filter_end in "
                       "update_filter.\n");
#endif
        ...
        if (fe_ptr = strrchr(filter_end, EXT_DELIMITER)) {
                ...
                if (bShowPixmaps) {
                        ...
                        if ((ffg_ptr) &&
                            ((int)strlen(ffg_ptr) >= (int)strlen(PIXMAP_EXT))) {
                        ...
                        if ((ffg_ptr) &&
                            ((int)strlen(ffg_ptr) >= (int)strlen(BITMAP_EXT))) {
        ...
        /******************************************************************/
        new_filter = (char *)calloc(strlen(filter) + strlen(filter_end) + 1,
                                    sizeof(char));
#ifdef DEBUG
        if (!new_filter)
                printf("Error callocing memory for new_filter in "
                       "update_filter.\n");
#endif
...
void update_container_contents(char *filter) {
        ...
        if (!path)
                printf(
                    "Calloc error for path, in update_container_contents.\n");
#endif
        ...
        n = 0;
        /*
        XtSetArg(args[n], XmNimageName,
        "/usr/dt/appconfig/icons/C/Dtactn.m.pm"); n++;
        */
        ...
        if (count > icon_count) {
                ...
                /*memset(&(icons_in_container[icon_count]), 0,
                 * sizeof(DtIconGadget *));*/
                for (lcv = old_count; lcv < icon_count; lcv++) {
                        icons_in_container[lcv] =
                            /*(DtIconGadget *)XtCreateManagedWidget
                               (filelist[lcv]->d_name,*/
                            (DtIconGadget *)XtCreateManagedWidget(
                                "IconGadget", dtIconGadgetClass,
                                (Widget)icon_scrolled_container, args, n);
                        XtAddCallback(
                            (Widget)icons_in_container[lcv], XmNcallback,
                            (XtCallbackProc)selectionCB_icon_scrolled_container,
                            (XtPointer)NULL);

#ifdef DEBUG
                        if (!icons_in_container[lcv])
                                printf("Error creating icon gadget, in "
...
if (namecount) {
        ...
        for (lcv = 0; lcv < count; lcv++) {
                ...
                XtVaSetValues((Widget)icons_in_container[lcv], XmNstring,
                              xmstring, XmNimageName, iconfile, NULL);
...
static void calc_bottom_attachment(Widget UxWidget, XtPointer UxClientData,
            ...
            (_UxCicon_selection_dialog *)UxGetContext(UxWidget);
        {
                if (numberOfTopButtons > 0) {
                        XtVaSetValues(UxWidget, XmNbottomWidget,
                                      top_button_form, NULL);
                } else if (use_icon_name_field) {
                        XtVaSetValues(UxWidget, XmNbottomWidget,
                                      icon_name_textfield_label, NULL);
                } else
                        XtVaSetValues(UxWidget, XmNbottomWidget,
                                      separatorGadget1, NULL);
        }
        return;
}
...

{
        ...
        if (bottom_button_one_cb) {
                ...
                bottom_button_one_cb(UxWidget, (XtPointer)basename,
                                     UxCallbackArg);
                if (basename)
                        XtFree(basename);
        }
...
void display_nofindicon_message(Widget parent) {
        ...
        XtVaSetValues(widChild, RES_CONVERT(XmNlabelString, "Change Name"),
                      NULL);
        ...
        strcpy(buffer, "There is no set of icons with that filename in the "
                       "icon folders.\n");
        strcat(buffer,
               "Move the icons into one of the icon folders, then select "
               "\"Name OK\".\n");
        strcat(buffer, "Or, to select a different set of icons, select "
                       "\"Change Name\".\n");
        strcat(buffer, "\nIf you are a software developer creating a "
...
static void activateCB_bottom_button1(Widget UxWidget, XtPointer UxClientData,
            ...
            (_UxCicon_selection_dialog *)UxGetContext(UxWidget);
        {
                ...
                if (basename) {
                        if (strchr(basename, '/')) {
                                /**** WARNING - this needs to be in a message
...
Enter only the name of the icon without the path or extensions.";
                                ...
                                FIND_ICONGADGET_ICON(basename, fullname,
                                                     DtMEDIUM);
                                if ((fullname) &&
                                    (check_file_exists(fullname))) {
                                        ...
                                        display_nofindicon_message(
                                            IconSelector);
                                }
                        }
                } else {
                        msgPtr = GETMESSAGE(11, 50,
                                            "The Icon Filename is invalid.\n\
...
static void activateCB_bottom_button3(Widget UxWidget, XtPointer UxClientData,
        ...
        UxIcon_selection_dialogContext = UxContext =
            (_UxCicon_selection_dialog *)UxGetContext(UxWidget);
        {
                DisplayHelpDialog(UxWidget, (XtPointer)HELP_ICONSELECTOR,
                                  UxCallbackArg);
        }
        return;
}
...
#define TIGHTNESS 20
        ...
        _UxParent = XtVaCreatePopupShell(
            "icon_selection_dialog_shell", xmDialogShellWidgetClass, _UxParent,
            XmNx, 18, XmNy, 116, XmNwidth, 610, XmNheight, 534,
            XmNshellUnitType, XmPIXELS, XmNkeyboardFocusPolicy, XmEXPLICIT,
            XmNtitle, "icon_selection_dialog", NULL);

        UxTmp0 = dialog_title ? (char *)dialog_title : "Icon Selection Window";

        icon_selection_dialog = XtVaCreateWidget(
            "icon_selection_dialog", xmFormWidgetClass, _UxParent,
            XmNdialogStyle, XmDIALOG_FULL_APPLICATION_MODAL, XmNwidth, 550,
            XmNheight, 534, XmNunitType, XmPIXELS,
            RES_CONVERT(XmNdialogTitle, UxTmp0), XmNautoUnmanage, FALSE,
            XmNdefaultPosition, FALSE, NULL);
        UxPutContext(icon_selection_dialog,
        ...
        bottom_button_form = XtVaCreateManagedWidget(
            ...
            ((TIGHTNESS * ntotalbuttons) - 1),

            XmNleftOffset, 10, XmNleftAttachment, XmATTACH_FORM, XmNrightOffset,
            10, XmNrightAttachment, XmATTACH_FORM, XmNbottomOffset, 20,
            XmNbottomAttachment, XmATTACH_FORM,

            NULL);
        UxPutContext(bottom_button_form,
                     (char *)UxIcon_selection_dialogContext);
        ...
        bottom_button3 = XtVaCreateManagedWidget(
            ...
            RES_CONVERT(XmNlabelString, UxTmp0), XmNuserData,
        ...
        separatorGadget1 = XtVaCreateManagedWidget(
            "separatorGadget1", xmSeparatorGadgetClass, icon_selection_dialog,
            XmNx, 0, XmNy, 490, XmNrightOffset, 1, XmNrightAttachment,
            XmATTACH_FORM, XmNleftOffset, 1, XmNleftAttachment, XmATTACH_FORM,
            XmNbottomOffset, 10, XmNbottomWidget, bottom_button_form,
        ...
        icon_name_text_field = XtVaCreateManagedWidget(
            "icon_name_text_field", xmTextFieldWidgetClass,
            icon_selection_dialog, XmNx, 9, XmNy, 440, XmNbottomOffset, 10,
            XmNbottomWidget, separatorGadget1, XmNbottomAttachment,
            XmATTACH_WIDGET, XmNrightOffset, 10, XmNrightAttachment,
            XmATTACH_FORM, XmNleftOffset, 10, XmNleftAttachment, XmATTACH_FORM,
            NULL);
        UxPutContext(icon_name_text_field,
                     (char *)UxIcon_selection_dialogContext);
        ...
        icon_name_textfield_label = XtVaCreateManagedWidget(
            "icon_name_textfield_label", xmLabelWidgetClass,
            icon_selection_dialog, XmNx, 9, XmNy, 410,
            RES_CONVERT(XmNlabelString, UxTmp0), XmNbottomOffset, 0,
            XmNbottomWidget, icon_name_text_field, XmNbottomAttachment,
            XmATTACH_WIDGET, XmNleftOffset, 0, XmNleftWidget,
            icon_name_text_field, XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET,
        ...
        filter_textfield_label = XtVaCreateManagedWidget(
            "filter_textfield_label", xmLabelWidgetClass, icon_selection_dialog,
            XmNx, 10, XmNy, 10, RES_CONVERT(XmNlabelString, UxTmp0),
            XmNalignment, XmALIGNMENT_BEGINNING, XmNtopOffset, 10,
            XmNtopAttachment, XmATTACH_FORM, XmNleftOffset, 10,
            XmNleftAttachment, XmATTACH_FORM, NULL);
        UxPutContext(filter_textfield_label,
                     (char *)UxIcon_selection_dialogContext);

        /* Creation of filter_text_field */
        filter_text_field = XtVaCreateManagedWidget(
            "filter_text_field", xmTextFieldWidgetClass, icon_selection_dialog,
            XmNx, 10, XmNy, 40, XmNtopOffset, 0, XmNtopWidget,
            filter_textfield_label, XmNtopAttachment, XmATTACH_WIDGET,
        ...
        directory_list_label = XtVaCreateManagedWidget(
            "directory_list_label", xmLabelWidgetClass, icon_selection_dialog,
            XmNx, 0, XmNy, 90, XmNlabelType, XmSTRING,
            RES_CONVERT(XmNlabelString, UxTmp0), XmNalignment,
            XmALIGNMENT_BEGINNING, XmNtopOffset, 10, XmNtopWidget,
            filter_text_field, XmNtopAttachment,
            use_filter_field ? XmATTACH_WIDGET : XmATTACH_FORM, XmNleftOffset,
            10, XmNleftAttachment, XmATTACH_FORM, NULL);
        UxPutContext(directory_list_label,
                     (char *)UxIcon_selection_dialogContext);
        ...
        icon_container_label = XtVaCreateManagedWidget(
            "icon_container_label", xmLabelWidgetClass, icon_selection_dialog,
            XmNx, 281, XmNy, 90, XmNlabelType, XmSTRING,
            RES_CONVERT(XmNlabelString, UxTmp0), XmNalignment,
            XmALIGNMENT_BEGINNING, XmNtopOffset, 10, XmNtopWidget,
            filter_text_field, XmNtopAttachment, XmATTACH_WIDGET,
            XmNleftAttachment, XmATTACH_POSITION, XmNleftPosition, 55, NULL);
        UxPutContext(icon_container_label,
                     (char *)UxIcon_selection_dialogContext);
        ...
        top_button_form = XtVaCreateManagedWidget(
            ...
            XmNresizePolicy, XmRESIZE_NONE, XmNx, 10, XmNy, 290,
            ...
            XmNrightAttachment, XmATTACH_FORM, XmNleftOffset, 1,
            XmNleftAttachment, XmATTACH_FORM, NULL);
        UxPutContext(top_button_form, (char *)UxIcon_selection_dialogContext);

        UxTmp0 =
            top_button_one_label ? (char *)top_button_one_label : "No_Label";

        /* Creation of top_button1 */
        top_button1 = XtVaCreateManagedWidget(
            "top_button1", xmPushButtonWidgetClass, top_button_form, XmNx, 40,
            XmNy, 0, XmNleftOffset, 0, XmNbottomAttachment, XmATTACH_FORM,
            XmNleftAttachment, XmATTACH_POSITION, XmNtopOffset, 0,
        ...
        UxTmp0 =
            top_button_two_label ? (char *)top_button_two_label : "No_Label";

        /* Creation of top_button2 */
        top_button2 = XtVaCreateManagedWidget(
            "top_button2", xmPushButtonWidgetClass, top_button_form, XmNx, 220,
            XmNy, 0, XmNleftOffset, 0, XmNtopOffset, 0, XmNleftPosition,
        ...
        UxTmp0 = top_button_three_label ? (char *)top_button_three_label
                                        : "No_Label";

        /* Creation of top_button3 */
        top_button3 = XtVaCreateManagedWidget(
            "top_button3", xmPushButtonWidgetClass, top_button_form, XmNx, 310,
            XmNy, 0, RES_CONVERT(XmNlabelString, UxTmp0), XmNleftPosition, 72,
        ...
        icon_scrolled_win = XtVaCreateManagedWidget(
            "icon_scrolled_win", xmScrolledWindowWidgetClass,
            icon_selection_dialog, XmNscrollingPolicy, XmAUTOMATIC,
            ...
            0, XmNtopWidget, icon_container_label, XmNtopAttachment,
            XmATTACH_WIDGET, XmNleftOffset, 0, XmNleftWidget,
            icon_container_label, XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET,
            XmNbottomOffset, 10, XmNbottomWidget, None, XmNbottomAttachment,
        ...
        icon_scrolled_container = XtVaCreateManagedWidget(
            "icon_scrolled_container", xmRowColumnWidgetClass,
            icon_scrolled_win, XmNnavigationType, XmTAB_GROUP,
            /*XmNborderWidth, 1,*/
            /*XmNnumColumns, 2,*/
            XmNnumColumns, 1, XmNorientation, XmVERTICAL, XmNpacking,
            XmPACK_COLUMN, XmNshadowThickness, 1, NULL);

        XtVaSetValues(icon_selection_dialog, XmNcancelButton, bottom_button2,
                      NULL);
...
Widget create_icon_selection_dialog(
    ...
    void (*_Uxtop_button_one_cb)(), unsigned char *_Uxtop_button_two_label,
    void (*_Uxtop_button_two_cb)(), unsigned char *_Uxtop_button_three_label,
    void (*_Uxtop_button_three_cb)(), int _Uxuse_icon_name_field,
...

{
        ...
        if (!file_filter_global)
                printf("Calloc error for file_filter_global, in "
                       "create_icon_selection_dialog.\n");
#endif

        if (file_filter[0] == '*')
                file_filter_global =
                    strcpy(file_filter_global, (char *)&file_filter[1]);
        else
                file_filter_global =
                    strcpy(file_filter_global, (char *)file_filter);
...
#ifdef DEBUG
        if (!main_filter)
                printf("Calloc error for main_filter, in "
                       "create_icon_selection_dialog.\n");
#endif
