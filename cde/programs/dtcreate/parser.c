...
char keywordDB[][30] = {
    "ACTION",      "ICON",     "EXEC_STRING", "DESCRIPTION", "TYPE",
    "WINDOW_TYPE", "ARG_TYPE", "LABEL",       '\0',
};
...
int GetActionData(FILE *fp, ActionData *ActionDataptr) {
        ...
        if (ActionDataptr)
                memset(ActionDataptr, 0, sizeof(ActionData));
        else {
#ifdef DEBUG
        ...
        while (fgets(linebuf, sizeof(linebuf) - 1, fp)) {
                ...
                if (linebuf[0] == '{' || linebuf[0] == '#')
                        continue;
                /* If the first character is '}' reached the end of Action
                 * Stanza so quit */
                if (linebuf[0] == '}')
                        break;
                len = strlen(linebuf);
                ...
                */
                if ((wordPairs = GetKeywordValuePairs(linebuf, &fldid,
                                                      ACTION_TABLE)) != NULL) {
                        if (first && strcmp(wordPairs[0], "ACTION")) {
#ifdef DEBUG
                                printf(
                                    "first && strcmp(wordPairs[0],ACTION)\n");
#endif
                                return 1;
                        } else
                                first = FALSE;
                        ...
                        /* if linebuf does not contain the keyword then
                        everything in wordPairs[0],else, wordPairs[0] contains
                        the keyword and wordpairs[1] contains the value.
                        */
                        if (wordPairs && wordPairs[0] && !wordPairs[1]) {
                                /* fldid is returned by the GetKeywordValuePairs
                                function. This id determines which field of the
                                ActionDataptr will have the value. Please see
                                KeywordDB array in parser.h for more details.
                                */
                                switch (fldid) {
        ...
        if (manflds != 3) {
                ...
                ActionDataptr->papFiletypes = (FiletypeData **)GetFiletypeData(
                    fp, (char *)(ActionDataptr->pszCmd),
                    (short *)&(ActionDataptr->cFiletypes));
                if (!ActionDataptr->papFiletypes &&
                    ActionDataptr->cFiletypes > 0) {
#ifdef DEBUG
                        printf("!ActionDataptr->papFiletypes && "
                               "ActionDataptr->cFiletypes > 0)\n");
#endif
...
FiletypeData **GetFiletypeData(FILE *fp, char *pszOpenCmd, short *nftypes) {
        ...
        while (fgets(linebuf, sizeof(linebuf) - 1, fp)) {
                ...
                /* If end of a stanza check if all the mandatory fields are
                 * there */
                if (linebuf[0] == '}') {
                        /* Check for atleast one field in DATA_ATTRIBUTES stanza
                         */
                        if (previous == DATA_ATTRIBUTES && manflds != 1) {
                                printf("Error in DATA_ATTR stanza of the "
                                       "FiletypeData\n");
                                return NULL;
                        }
                         ...
                         */
                                ...
                                printf("Error in DATA_CRITERIA stanza of the "
                                       "FiletypeData\n");
                                return NULL;
                        }
                        /* Check for atleast two fields in ACTION_OPEN or
                           ACTION_PRINT stanza */
                        else if ((previous == ACTION_OPEN ||
                                  previous == ACTION_PRINT)) {
                                if (manflds != 4) {
                                        printf("Error in ACTION_OPEN/PRINT "
                                               "stanza of the FiletypeData\n");
                                        return NULL;
                                }
                        }
                        /* Check for atleast three fields in ACTION_PRINT_FTYPE
                           stanza */
                        else if (previous == ACTION_PRINT_FTYPE &&
                                 manflds != 3) {
                                printf("Error in ACTION_PRINT_FTYPE stanza of "
                                       "the FiletypeData\n");
                                return NULL;
                        }
                        continue;
                ...
                if ((wordPairs = GetKeywordValuePairs(
                         linebuf, &fldid, FILETYPE_TABLE)) != NULL) {
                        if (!strcmp(wordPairs[0], "DATA_ATTRIBUTES")) {
                                /* first filetype */
                                if (previous == 0) {
                                        /* Allocate a filetypedata pointer to an
                                         * array of filetypedata records
                                         */
                                        if (!ppFiletypeData) {
                                                if ((ppFiletypeData =
                                                         (FiletypeData **)calloc(
                                        ...
                                        ppFiletypeData[nfiletypes]->pszOpenCmd =
                                            pszOpenCmd;
                                        previous = DATA_ATTRIBUTES;
                                        manflds = 0;
                                } else if (previous == ACTION_OPEN ||
                                           previous == ACTION_PRINT) {
                                        /* New filetypedata started so allocate
                                         * a new filetypedata ptr */
                                        if ((ppnewFiletypeData =
                                                 (FiletypeData **)realloc(
                                                     (FiletypeData *)
                                                         ppFiletypeData,
                                                     nfiletypes +
                                                         2 * sizeof(FiletypeData
                                                                        *))) ==
                                            NULL) {
                                                printf("\n Cannot allocate "
                                                       "memory\n");
                                                 ...
                                                 * process the exec_string */
                                                if (!(execstr = ProcessExecString(
                                                          (char *)ppFiletypeData
                                                              [nfiletypes]
                                                                  ->pszPrintCmd)))
                                                        ppFiletypeData
                                                ...
                                                ppFiletypeData[nfiletypes]
                                                    ->pszOpenCmd = pszOpenCmd;
                                                previous = DATA_ATTRIBUTES;
                                                manflds = 0;
                                        }
                        ...
                        } else if (!strcmp(wordPairs[0], "ACTION") &&
                                   !strcmp(wordPairs[1], "Open")) {
                ...
                if (wordPairs && !wordPairs[1] && wordPairs[0]) {
                        ...
                        case 0: /* next line for filetype_name */
                                ppFiletypeData[nfiletypes]->pszName =
                                    wordPairs[0];
                                break;

                        case 1: /* next line for icon_name */
                                ppFiletypeData[nfiletypes]->pszIcon =
                                    wordPairs[0];
                                break;

                        case 2: /* next line for help_text */
                                ppFiletypeData[nfiletypes]->pszHelp =
                                    wordPairs[0];
                                break;

                        case 3: /* next line for print_cmd */
                                ppFiletypeData[nfiletypes]->pszPrintCmd =
                                    wordPairs[0];
                                manflds++;
                                break;
                        case 4: /* next line for name_pattern or path_pattern */
                                ppFiletypeData[nfiletypes]->pszPattern =
                                    wordPairs[0];
                                break;
                        ...
                        case 6: /* next line for mode */
                                ppFiletypeData[nfiletypes]->pszPermissions =
                                    wordPairs[0];
                                break;
                        case 7: /* next line for contents */
                                ppFiletypeData[nfiletypes]->pszContents =
                                    wordPairs[0];
                                break;
        ...
        if ((previous == ACTION_OPEN && manflds != 4) ||
            (previous == ACTION_PRINT && manflds != 4))
                ...
                if (ProcessContents(
                        (FiletypeData *)ppFiletypeData[nfiletypes]) < 0) {
                        printf("\n Error in Contents Data\n");
                        return NULL;
                }
                /* Everything looks right so process the exec_string */
                if (!(execstr = ProcessExecString(
                          (char *)ppFiletypeData[nfiletypes]->pszPrintCmd)))
...
char **GetKeywordValuePairs(char *s, int *id, int table) {
        char *wordStart;
        static int fldid = -1;
        int idx;
        ...
        if (table == ACTION_TABLE) {
                ...
                while ((strcmp(keywordDB[idx], "")) &&
                       (strcmp(args[0], keywordDB[idx])))
                        idx++;
                if (idx >= 0 && idx < MAX_NUM_ACTION_FIELDS)
                        fldid = idx;
        } else if (table == FILETYPE_TABLE) {
                /* Check for keyword in FiletypekeywordDB defined in parser.h */
                while ((strcmp(FiletypekeywordDB[idx], "")) &&
                       (strcmp(args[0], FiletypekeywordDB[idx])))
                        idx++;
        ...
        */
        if ((idx == MAX_NUM_ACTION_FIELDS && table == ACTION_TABLE) ||
            (idx == MAX_NUM_FILETYPE_FIELDS && table == FILETYPE_TABLE)) {
                ...
                if ((temp4 = (char *)realloc(
                         args[0], strlen(args[0]) + strlen(args[1]) + 2)) ==
                    NULL) {
                ...
                if (IsLastSingle(args[0]) &&
                    args[0][strlen(args[0]) - 1] == '\\')
                        args[0][strlen(args[0]) - 1] = '\0';
                if ((temp4 = (char *)realloc(
                         fline, flinesize + strlen(args[0]) + 1)) == NULL) {
                        printf("Cannot Allocate memory\n");
                        return NULL;
                }
                strcat(temp4, args[0]);
...
char **ProcessExecString(char *cmd) {
                ...
                if (s3 && *(s3 + 1) != '"') {
                        ...
                        if (!promptfound) {
                                if ((tmp = strchr(s3, '"'))) {
                                        promptfound = TRUE;
                                        exec_args[1] =
                                            (char *)calloc(1, (tmp - s3) + 1);
                                        strncpy(exec_args[1], s3, (tmp - s3));
                                        exec_args[1][strlen(exec_args[1])] =
                                            '\0';
                                }
                        }
                }
...
int ProcessContents(FiletypeData *pFtD) {
