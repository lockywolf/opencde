...
double max_fix[4] = {6.871947674e+10, 3.245185537e+32, 1.000000000e+36,
                     2.230074520e+43};
double min_fix0[4] = {2.500000000e-1, 3.750000000e-1, 1.000000000e-1,
                      4.375000000e-1};
double min_fix1[4] = {1.250000000e-1, 4.687500000e-2, 1.000000000e-2,
                      2.734375000e-2};
double min_fix2[4] = {6.250000000e-2, 5.859375000e-3, 1.000000000e-3,
                      1.708984375e-3};
double min_fix3[4] = {3.125000000e-2, 7.324218750e-4, 1.000000000e-4,
                      1.068115234e-4};
double min_fix4[4] = {1.562500000e-2, 9.155273437e-5, 1.000000000e-5,
                      6.675720215e-6};
double min_fix5[4] = {7.812500000e-3, 1.144409180e-5, 1.000000000e-6,
                      4.172325134e-7};
double min_fix6[4] = {6.906250000e-3, 1.430511475e-6, 1.000000000e-7,
                      2.607703209e-8};
double min_fix7[4] = {1.953125000e-3, 1.788139343e-7, 1.000000000e-8,
                      1.629814506e-9};
double min_fix8[4] = {9.765625000e-4, 2.235174179e-8, 1.000000000e-9,
                      1.018634066e-10};
double min_fix9[4] = {4.882812500e-4, 2.793967724e-9, 1.000000000e-10,
                      6.366462912e-12};
...
void do_dtcalc(int argc, char **argv) {
        char *ptr;
...
int get_bool_resource(enum res_type rtype, int *boolval) {
        ...
        len = strlen(tempstr);
        for (n = 0; n < len; n++)
                if (isupper(tempstr[n]))
                        tempstr[n] = tolower(tempstr[n]);
        if (EQUAL(tempstr, vstrs[(int)V_TRUE]))
                *boolval = TRUE;
        else
                *boolval = FALSE;
        return (1);
...
int get_int_resource(enum res_type rtype, int *intval) {
        char *val;
...
void get_key_val(char *val, char *str) { *val = str[0]; }

void get_label(int n) {
        char *temp;
...
void get_options(int argc, char *argv[]) {
        ...
        while (argc > 0) {
                if (argv[0][0] == '-' || argv[0][0] == '+') {
                        ...
                        case 'D':
                                v->MPdebug =
                                    TRUE; /* MP debug info. to stderr. */
                                break;
                        case 'a':
                                INC;
                                getparam(next, MAXLINE, argv,
                                         opts[(int)O_ACCVAL]);
                                v->accuracy = atoi(next);
                                if (v->accuracy < 0 || v->accuracy > 9) {
                                        msg_size =
                                            strlen(opts[(int)O_ACCRANGE]) + 3;
                                        msg = (char *)XtMalloc(msg_size);
                                        snprintf(msg, msg_size, "%s",
                                                 opts[(int)O_ACCRANGE]);
                                        _DtSimpleError(v->appname, DtWarning,
                                                       NULL, msg);
                                        XtFree(msg);
                                        v->accuracy = 2;
                                }
                        ...
                        case 'm':
                                INC;
                                msg_size = strlen(opts[(int)O_MODE]) +
                                           strlen(next) + 3;
                                msg = (char *)XtMalloc(msg_size);
                                snprintf(msg, msg_size, opts[(int)O_MODE],
                                         next);
                        ...
                        case 'b':
                                ...
                                break;
                        case 'n':
                                if (strcmp(&argv[0][1], "notation") == 0) {
                                        INC;
                                        msg_size =
                                            strlen(opts[(int)O_DISPLAY]) +
                                            strlen(next) + 3;
                                        msg = (char *)XtMalloc(msg_size);
                                        snprintf(msg, msg_size,
                                                 opts[(int)O_DISPLAY], next);
                                        getparam(next, MAXLINE, argv, msg);
                                        XtFree(msg);
                                        strlcpy(strval, next, MAXLINE);
                                        len = strlen(strval);
                                        ...
                                        if (strncmp(strval, "FIX", 3) == 0)
                                                ...
                                                msg_size =
                                                    ...
                                                    strlen(next) + 3;
                                                msg =
                                                    (char *)XtMalloc(msg_size);
                                                snprintf(msg, msg_size,
                                                         opts[(int)O_DISPLAY],
                                                         next);
                                                _DtSimpleError(v->appname,
                                                               DtWarning, NULL,
                                                               msg);
                                                XtFree(msg);
                                                v->dtype = FIX;
                                        ...
                                        break;
                                } else if (strcmp(&argv[0][1], "no_menu_bar") ==
                                           0) {
                                        INC;
                        ...
                        case 't':
                                if (strcmp(&argv[0][1], "trig") == 0) {
                                        INC;
                                        msg_size = strlen(opts[(int)O_TRIG]) +
                                                   strlen(next) + 3;
                                        msg = (char *)XtMalloc(msg_size);
                                        snprintf(msg, msg_size,
                                                 opts[(int)O_TRIG], next);
                                        getparam(next, MAXLINE, argv, msg);
                                        XtFree(msg);
                                        strlcpy(strval, next, MAXLINE);
                                        len = strlen(strval);
                                        ...
                                        if (strncmp(strval, "DEG", 3) == 0)
                                                ...
                                                v->ttype = RAD;
                                        else if (strncmp(strval, "GRAD", 4) ==
                                                 0)
                                                v->ttype = GRAD;
                                        else {
                                                msg_size =
                                                    strlen(opts[(int)O_TRIG]) +
                                                    strlen(next) + 3;
                                                msg =
                                                    (char *)XtMalloc(msg_size);
                                                snprintf(msg, msg_size,
...
void getparam(char *s, int s_size, char *argv[], char *errmes) {
        char *msg;
        int msg_size;
                        ...
                        if (line[1] >= '0' && line[1] <= '9' &&
                            line[2] == ' ') {
                                ...
                                if (isval == 'c') {
                                        ...
                                        SSCANF(&line[3], "%s", tmp);
                                        strlcpy(v->fun_vals[i], convert(tmp),
                                                MAXLINE);
                                } else if (isval == 'r') {
                                        n = sscanf(&line[3], "%lf", &cval);
                                        if (n == 1)
                                                MPstr_to_num(&line[3], DEC,
                                                             v->MPmvals[i]);
                                        continue;
                                }
                                len = strlen(line);
...
int get_str_resource(enum res_type rtype, char *strval, int strval_size) {
        char *val;
        int i, len;
...
void grey_buttons(enum base_type base) {
        char val;
...
void handle_menu_selection(int n, int item) {
        if (item != -1) {
                if (IS_KEY(v->pending,
                           KEY_LPAR)) /* Are we inside parentheses? */
                {
                        ...
                        v->current = item;
                        v->ismenu =
                            1; /* To prevent grey buttons being redrawn. */
                        do_pending();
                        v->ismenu = 0;
                }
        }
...
char *make_eng_sci(int *MPnumber) {
...
char *make_fixed(int *MPnumber, int cmax) {
        char *optr;
...
char *make_number(int *MPnumber, BOOLEAN mkFix) {
        double number, val;

        /*  NOTE: make_number can currently set v->error when converting to a
         * double. This is to provide the same look&feel as V3 even though
         * dtcalc now does internal arithmetic to "infinite" precision.
         *
...
void MPstr_to_num(char *str, enum base_type base, int *MPval) {
        char *optr;
...
void paren_disp(char c) {
        int i, n;

        /*  If the character is a Delete, clear the whole line, and exit
         * parenthesis processing.
         *
         *  If the character is a Back Space, remove the last character. If the
         * last character was a left parenthesis, decrement the parentheses
         * count. If the parentheses count is zero, exit parenthesis
         * processing.
         *
         *  If the character is a control character (not Ctrl-h), then append
         * ^(char).
         *
        ...
        case KEYBOARD_DOWN:
                if (v->pending) {
                        ...
                        if (ival < TITEMS - EXTRA) {
                                if ((v->modetype == FINANCIAL && ival == 23) ||
                                    (v->modetype == LOGICAL &&
                                     (ival == 21 || ival == 22 || ival == 23)))
                                        break;
                                else
                                        /* go arm the button */
                        ...
                        if ((v->modetype == FINANCIAL && ival == 23) ||
                            (v->modetype == LOGICAL &&
                             (ival == 21 || ival == 22 || ival == 23)))
                                ...
                                draw_button(ival, v->curwin, v->row, v->column,
                                            FALSE);
                }
                /* go disarm the button */
                draw_button(ival, v->curwin, v->row, v->column, FALSE);
                break;
        }
}

void process_item(int n) {
        int i, isvalid;
...
void process_stack(int startop, int startnum, int n) {
...
void process_str(char *str, enum menu_type mtype) {
        int i, len;
        char save[80];
...
void read_rcfiles(
    void) /* Read .dtcalcrc's from home and current directories. */
{
        ...
        for (n = 0; n < MAXREGS; n++) {
                ...
                snprintf(name, MAXPATHLEN + 50, "%1d: %s [%s]", n, tmp,
                         v->con_names[n]);

                strlcpy(v->con_names[n], name, MAXLINE);
                strlcpy(v->fun_vals[n], "",
                        MAXLINE); /* Initially empty function strings. */
        }
        ...
        snprintf(name, MAXPATHLEN, "%s/%s", getcwd(pathname, MAXPATHLEN + 1),
                 RCNAME);
        get_rcfile(name); /* Read .dtcalcrc file from current directory. */
}

void show_display(int *MPval) {
        if (!v->error) {
...
void write_rcfile(enum menu_type mtype, int exists, int cfno, char *val,
                  char *comment) {
        char *home;                 /* Pathname for users home directory. */
        ...
        rcexists = 0;
        snprintf(rcname, MAXPATHLEN, "%s/%s", getcwd(pathname, MAXPATHLEN),
                 RCNAME);
...
void write_resources(char *filename) {
        char intval[5];
        int MPtemp[MP_SIZE];
