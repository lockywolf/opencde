...
int main(int argc, char **argv) {
        ...
        X->kframe =
            XtVaAppInitialize(&X->app, DTCALC_CLASS_NAME, /* app class */
                              NULL,                       /* options list */
                              0,                          /* num options */
        ...
        if (DtInitialize(X->dpy, X->kframe, argv[0], DTCALC_CLASS_NAME) ==
            False) {
        ...
        XtGetApplicationResources(X->kframe, &application_args, resources, 6,
                                  NULL, 0);
...
/* #ifdef hpux */
        ...
        if (colorSrv && !BlackWhite) {
                ...
                pixmap =
                    XmGetPixmap(DefaultScreenOfDisplay(X->dpy), CALC_ICON_NAME,
                ...
                pixmap =
                    _DtGetMask(DefaultScreenOfDisplay(X->dpy), CALC_ICON_NAME);
                if (pixmap != XmUNSPECIFIED_PIXMAP)
                        X->iconmask = pixmap;
                else
                ...
                /* first get the Calculator's Icon */
                pixmap =
                    XmGetPixmap(DefaultScreenOfDisplay(X->dpy),
                                CALC_ICON_NAME_BM, white_pixel, black_pixel);
                if (pixmap != XmUNSPECIFIED_PIXMAP)
                        X->icon = pixmap;
                else
                        X->icon = XmUNSPECIFIED_PIXMAP;
                ...
                /* now let's get the mask for the Calculator */
                pixmap = _DtGetMask(DefaultScreenOfDisplay(X->dpy),
                                    CALC_ICON_NAME_BM);
                if (pixmap != XmUNSPECIFIED_PIXMAP)
                        X->iconmask = pixmap;
                else
...
static Widget button_create(Widget owner, int row, int column, int maxrows,
        ...
        if (colorSrv) {
                ...
                button = XtVaCreateManagedWidget(
                    buttons[n].resname, xmPushButtonWidgetClass, owner,
                    XmNtopAttachment, XmATTACH_POSITION, XmNtopPosition,
                    row * maxcols, XmNleftAttachment, XmATTACH_POSITION,
                    XmNleftPosition, column * maxrows, XmNrightAttachment,
                    XmATTACH_POSITION, XmNrightPosition, (column + 1) * maxrows,
                    XmNbottomAttachment, XmATTACH_POSITION, XmNbottomPosition,
                    (row + 1) * maxcols, XmNlabelString, lstr, XmNbackground,
                    bg, XmNtraversalOn, TRUE, XmNalignment, XmALIGNMENT_CENTER,
                ...
                button = XtVaCreateManagedWidget(
                    ...
                    XmATTACH_POSITION, XmNrightPosition, (column + 1) * maxrows,
                    XmNbottomAttachment, XmATTACH_POSITION, XmNbottomPosition,
                    (row + 1) * maxcols, XmNlabelString, lstr, XmNtraversalOn,
                    TRUE, XmNalignment, XmALIGNMENT_CENTER, XmNrecomputeSize,
                    False, XmNnavigationType, XmNONE, NULL);
        ...
        XtAddEventHandler(button, KeyPressMask | KeyReleaseMask, FALSE,
                          event_proc, NULL);

        if (funBtn == NULL &&
            strcmp(v->pstr, GETMESSAGE(3, 5, "Functions")) == 0)
                funBtn = button;
                ...
                accBtn = button;
                 ...
                 strcmp(v->pstr, GETMESSAGE(3, 13, "Store")) == 0)
                stoBtn = button;
        else if (rclBtn == NULL &&
                 strcmp(v->pstr, GETMESSAGE(3, 14, "Recall")) == 0)
                rclBtn = button;
...
static void dtcalc_initialize_rframe(Widget owner, int type) {
        ...
        if (type == MEM) {
                ...
                XtVaSetValues(form, XmNmarginWidth, 5, XmNmarginHeight, 5,
                              XmNallowShellResize, TRUE, XmNdefaultPosition,
                              FALSE, NULL);

                snprintf(str, MAXLINE, "register%1d", 0);
                X->registers[0] = XtVaCreateManagedWidget(
                    str, xmLabelWidgetClass, form, XmNtopAttachment,
                    XmATTACH_FORM, XmNtopOffset, 2, XmNleftAttachment,
                    XmATTACH_FORM, XmNleftOffset, 2, XmNmarginWidth, 5,
                ...
                for (i = 1; i < MAXREGS; i++) {
                        ...
                        X->registers[i] = XtVaCreateManagedWidget(
                            str, xmLabelWidgetClass, form, XmNtopAttachment,
                            XmATTACH_WIDGET, XmNtopWidget, X->registers[i - 1],
                            XmNtopOffset, 2, XmNleftAttachment, XmATTACH_FORM,
                            XmNleftOffset, 2, XmNmarginWidth, 5,
                            XmNmarginHeight, 1, NULL);
                }
                ...
                label_string =
                    XmStringCreateLocalized(GETMESSAGE(2, 32, "Close"));
                ...
                set_title(FCP_FIN_REG,
                          GETMESSAGE(2, 18, lstrs[(int)L_FINMEMT]));
                ...
                for (i = 1; i < FINREGS; i++) {
                        ...
                        X->fregisters[i] = XtVaCreateManagedWidget(
                            str, xmLabelWidgetClass, form, XmNtopAttachment,
                            XmATTACH_WIDGET, XmNtopWidget, X->fregisters[i - 1],
                            XmNtopOffset, 2, XmNleftAttachment, XmATTACH_FORM,
                            XmNleftOffset, 2, XmNmarginWidth, 5,
                            XmNmarginHeight, 1, NULL);
                }
                ...
                X->fregistersvals[0] = XtVaCreateManagedWidget(
                    str, xmLabelWidgetClass, form, XmNtopAttachment,
                    XmATTACH_FORM, XmNtopOffset, 2, XmNleftAttachment,
                    XmATTACH_WIDGET, XmNleftWidget, X->fregisters[0],
                    XmNleftOffset, 0, XmNrightAttachment, XmATTACH_FORM,
                    XmNrightOffset, 2, XmNmarginWidth, 5, XmNmarginHeight, 1,
                    NULL);
                ...
                for (i = 1; i < FINREGS; i++) {
                        ...
                        X->fregistersvals[i] = XtVaCreateManagedWidget(
                            ...
                            X->fregistersvals[i - 1], XmNtopOffset, 2,
                            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
                            X->fregisters[i], XmNleftOffset, 0,
...
static void dtcalc_kkeyboard_create(Widget owner) {
        ...
        buttonFrame = XtVaCreateManagedWidget(
            ...
            XmNmarginWidth, 0, XmNmarginHeight, 0, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, X->modeFrame, XmNrightAttachment,
        ...
        for (row = 0; row < BROWS; row++)
                for (column = 0; column < BCOLS; column++) {
                        X->kbuttons[row][column] = button_create(
                            X->kkeyboard, row, column, BROWS, BCOLS);
...
static void dtcalc_kpanel_create(Widget owner) {
        ...
        X->mainWin = XtVaCreateManagedWidget("mainWin", xmMainWindowWidgetClass,
                                             owner, NULL);
        ...
        X->kFrame = XtVaCreateManagedWidget(
            ...
            XmNshadowType, XmSHADOW_OUT, XmNmarginWidth, 5, XmNmarginHeight, 5,
            NULL);
        ...
        X->textFrame = XtVaCreateManagedWidget(
            ...
            XmNleftAttachment, XmATTACH_FORM, XmNnavigationType, XmTAB_GROUP,
            NULL);
        ...
        X->modevals[(int)DISPLAYITEM] = XtVaCreateManagedWidget(
            ...
            XmNbottomAttachment, XmATTACH_FORM, XmNresizeWidth, TRUE,
            XmNshadowThickness, 0, XmNhighlightThickness, 0, XmNeditable, FALSE,
            XmNverifyBell, FALSE, XmNbackground, tmp_pixelbg, XmNforeground,
            tmp_pixelfg, NULL);
        XtAddCallback(X->modevals[(int)DISPLAYITEM], XmNhelpCallback,
                      HelpRequestCB, (XtPointer)HELP_DISPLAY);
        ...
        X->modeFrame = XtVaCreateManagedWidget(
            ...
            XmNmarginWidth, 0, XmNmarginHeight, 0, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, X->textFrame, XmNrightAttachment,
            XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, XmNnavigationType,
            XmTAB_GROUP, NULL);
        XtAddCallback(X->modeFrame, XmNhelpCallback, HelpRequestCB,
                      (XtPointer)HELP_MODELINE);

        X->modeline =
            XtVaCreateManagedWidget("modeline", xmFormWidgetClass, X->modeFrame,
        ...
        X->modevals[i] = XtVaCreateManagedWidget(
            ...
            XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNrecomputeSize,
            False, XmNalignment, XmALIGNMENT_CENTER, XmNlabelString,
            label_string, NULL);
        ...
        X->modevals[i] = XtVaCreateManagedWidget(
            ...
            XmNtopAttachment, XmATTACH_FORM, XmNrecomputeSize, False,
            XmNalignment, XmALIGNMENT_CENTER, XmNlabelString, label_string,
            NULL);
        XtAddCallback(X->modevals[i], XmNhelpCallback, HelpRequestCB,
                      (XtPointer)HELP_MODELINE);
...

        {
                i = (int)MODEITEM;
                modePulldown =
                    XmCreatePulldownMenu(X->modeline, "modePD", args, 0);
                ...
                label_string =
                    XmStringCreateLocalized(GETMESSAGE(2, 9, "Financial"));
                XtSetArg(args[2], XmNlabelString, label_string);
                XtSetArg(args[3], XmNuserData, val);
                modeArry[0] =
                    XmCreatePushButtonGadget(modePulldown, "fin", args, 4);
                XmStringFree(label_string);
                XtAddCallback(modeArry[0], XmNactivateCallback,
                              modelineValueChanged, (XtPointer)M_MODE);

                val = (v->curwin << 16) +
                      menu_entries[cmenus[(int)M_MODE].mindex + 2].val;
                label_string =
                    XmStringCreateLocalized(GETMESSAGE(2, 10, "Logical"));
                XtSetArg(args[2], XmNlabelString, label_string);
                XtSetArg(args[3], XmNuserData, val);
                modeArry[1] =
                    XmCreatePushButtonGadget(modePulldown, "logic", args, 4);
                XmStringFree(label_string);
                XtAddCallback(modeArry[1], XmNactivateCallback,
                              modelineValueChanged, (XtPointer)M_MODE);

                val = (v->curwin << 16) +
                      menu_entries[cmenus[(int)M_MODE].mindex + 3].val;
                label_string =
                    XmStringCreateLocalized(GETMESSAGE(2, 11, "Scientific"));
                XtSetArg(args[2], XmNlabelString, label_string);
                XtSetArg(args[3], XmNuserData, val);
                modeArry[2] =
                    XmCreatePushButtonGadget(modePulldown, "Sci", args, 4);
                XmStringFree(label_string);
                XtAddCallback(modeArry[2], XmNactivateCallback,
                              modelineValueChanged, (XtPointer)M_MODE);
                ...
                n++;
                X->modevals[i] =
                    XmCreateOptionMenu(X->modeline, "mode", args, n);
                ...
                i = (int)BASEITEM;
                basePulldown =
                    XmCreatePulldownMenu(X->modeline, "basePD", args, 0);
                ...
                XtSetArg(args[3], XmNuserData, val);
                X->baseWidgArry[0] =
                    XmCreatePushButtonGadget(basePulldown, "bin", args, 4);
                XmStringFree(label_string);
                ...
                XtSetArg(args[3], XmNuserData, val);
                X->baseWidgArry[1] =
                    XmCreatePushButtonGadget(basePulldown, "oct", args, 4);
                XmStringFree(label_string);
                ...
                XtSetArg(args[3], XmNuserData, val);
                X->baseWidgArry[2] =
                    XmCreatePushButtonGadget(basePulldown, "dec", args, 4);
                XmStringFree(label_string);
                ...
                XtSetArg(args[3], XmNuserData, val);
                X->baseWidgArry[3] =
                    XmCreatePushButtonGadget(basePulldown, "hex", args, 4);
                XmStringFree(label_string);
                ...
                XtSetArg(args[n], XmNmenuHistory,
                         X->baseWidgArry[(int)v->base]);
                n++;
                X->modevals[i] =
                    XmCreateOptionMenu(X->modeline, "base", args, n);
                XtManageChild(X->modevals[i]);
                XtAddCallback(X->modevals[i], XmNhelpCallback, HelpRequestCB,
                              (XtPointer)HELP_BASE);
                ...
                i = (int)NUMITEM;
                numPulldown =
                    XmCreatePulldownMenu(X->modeline, "numPD", args, 0);
                ...
                XtSetArg(args[3], XmNuserData, val);
                X->numWidgArry[0] =
                    XmCreatePushButtonGadget(numPulldown, "eng", args, 4);
                XmStringFree(label_string);
                XtAddCallback(X->numWidgArry[0], XmNactivateCallback,
                              modelineValueChanged, (XtPointer)M_NUM);
                ...
                XtSetArg(args[3], XmNuserData, val);
                X->numWidgArry[1] =
                    XmCreatePushButtonGadget(numPulldown, "fix", args, 4);
                XmStringFree(label_string);
                XtAddCallback(X->numWidgArry[1], XmNactivateCallback,
                              modelineValueChanged, (XtPointer)M_NUM);
                ...
                XtSetArg(args[3], XmNuserData, val);
                X->numWidgArry[2] =
                    XmCreatePushButtonGadget(numPulldown, "sci", args, 4);
                XmStringFree(label_string);
                XtAddCallback(X->numWidgArry[2], XmNactivateCallback,
                              modelineValueChanged, (XtPointer)M_NUM);
                ...
                n++;
                XtSetArg(args[n], XmNmenuHistory,
                         X->numWidgArry[(int)v->dtype]);
                n++;
                X->modevals[i] =
                    XmCreateOptionMenu(X->modeline, "num", args, n);
                XtManageChild(X->modevals[i]);
                XtAddCallback(X->modevals[i], XmNhelpCallback, HelpRequestCB,
                ...
                i = (int)TTYPEITEM;
                trigPulldown =
                    XmCreatePulldownMenu(X->modeline, "trigPD", args, 0);
                ...
                n++;
                XtSetArg(args[n], XmNmenuHistory,
                         X->ttypeWidgArry[(int)v->ttype]);
                n++;
                X->modevals[i] =
                    XmCreateOptionMenu(X->modeline, "trig", args, n);
                XtManageChild(X->modevals[i]);
                XtAddCallback(X->modevals[i], XmNhelpCallback, HelpRequestCB,
                              (XtPointer)HELP_TRIG);
...
static void create_cfframe(void) /* Create auxiliary frame for CON/FUN key. */
{
        ...
        XtVaSetValues(X->CFframe, XmNdialogTitle, tstr, XmNautoUnmanage, FALSE,
                      XmNallowShellResize, FALSE, XmNdefaultPosition, FALSE,
                      NULL);
        XmStringFree(tstr);
        ...
        j++;
        XtSetArg(args[j], XmNmwmDecorations,
                 MWM_DECOR_BORDER | MWM_DECOR_TITLE);
        j++;
...
static void create_menu(enum menu_type mtype, Widget button, int n) {
        ...
        X->menus[(int)mtype] = menu =
            XmCreatePopupMenu(button, "menu", NULL, 0);

        X->mrec[(int)mtype] = n;

        XtCreateManagedWidget(cmenus[(int)mtype].title, xmLabelWidgetClass,
                              menu, NULL, 0);
        XtCreateManagedWidget("separator", xmSeparatorWidgetClass, menu, NULL,
                              0);
        XtCreateManagedWidget("separator", xmSeparatorWidgetClass, menu, NULL,
                              0);
        ...
        for (i = 0; i < cmenus[(int)mtype].total; i++) {
                ...
                if (!invalid) {
                        ...
                        if (strcmp(tmpStr, ".") != 0) {
                                /* if it's not a "." let's go change it to what
                                 * it should be */
                                ptr = DtStrchr(mstr, '.');
                                if (ptr != NULL)
                                        ptr[0] = tmpStr[0];
                        }
                        mi = XtCreateManagedWidget(
                            mstr, xmPushButtonWidgetClass, menu, NULL, 0);
                        val = (v->curwin << 16) +
                              menu_entries[cmenus[(int)mtype].mindex + i].val;
                        XtAddCallback(mi, XmNactivateCallback, menu_proc,
                                      (XtPointer)val);
                }
        }
        if (!isAFunction && mtype == M_FUN) {
                mi = XtCreateManagedWidget(
                    GETMESSAGE(2, 35, "No Functions Defined"),
...
static void do_continue_notice(Widget parent, char *str) {
        ...
        X->notice =
            (Widget)XmCreateInformationDialog(X->kframe, "continue", NULL, 0);
        ...
        XtUnmanageChild(
            (Widget)XmMessageBoxGetChild(X->notice, XmDIALOG_OK_BUTTON));
        XtUnmanageChild(
...
static void do_confirm_notice(Widget parent, char *str) {
        ...
        message = XmStringCreateLocalized(str);
        XtVaSetValues(X->notice, XmNautoUnmanage, FALSE, XmNcancelLabelString,
                      cancel, XmNokLabelString, confirm, XmNmessageString,
                      message, XmNdialogStyle, XmDIALOG_APPLICATION_MODAL,
                      NULL);
...
static void event_proc(Widget widget, XtPointer client_data, XEvent *event,
        ...
        /* If the ASCII convert dialog, or the Help dialog
           is up and managed, we don't want the keystrokes to go to the
           calculator main, just those dialogs if ((X->Aframe != NULL &&
           XtIsManaged(X->Aframe)) || (X->helpDialog != NULL && X->helpMapped))
           {
           return;
           }
        */
        ...
        if (strcmp(vstrs[(int)V_CLR], str) == 0 &&
            (v->cur_ch != KEY_CLR && v->cur_ch != KEY_QUIT)) {
                if (v->event_type == KEYBOARD_DOWN)
                        ...
                        if (index != TITEMS)
                                draw_button(index, v->curwin, v->row, v->column,
                                            FALSE);
                }
                return;
        }
        ...
        */
        if (X->CFframe != NULL && XtIsManaged(X->CFframe)) {
                ...
                if (w == X->CFpi_butOK) {
                        if (v->cur_ch == KEY_EQ) {
                                XtCallCallbacks(X->CFpi_butOK, XmNarmCallback,
                                                (XtPointer)NULL);
                                XtCallCallbacks(X->CFpi_butOK,
                                                XmNactivateCallback,
                                                (XtPointer)NULL);
                        }
                        ...
                        if (v->cur_ch == KEY_EQ) {
                                XtCallCallbacks(X->CFpi_butClose,
                                                XmNarmCallback,
                                                (XtPointer)NULL);
                                XtCallCallbacks(X->CFpi_butClose,
                                                XmNactivateCallback,
                        ...
                        if (v->cur_ch == KEY_EQ) {
                                XtCallCallbacks(X->CFpi_butHelp, XmNarmCallback,
                                                (XtPointer)NULL);
                                XtCallCallbacks(X->CFpi_butHelp,
                                                XmNactivateCallback,
        ...
        if (v->pending == 0) {
                /* if the key is an "inactive" key (i.e. desensitized) then we
                   want to ignore it. */
                switch (v->base) {
                case BIN:
                        if (v->cur_ch == KEY_2 || v->cur_ch == KEY_3 ||
                            v->cur_ch == KEY_4 || v->cur_ch == KEY_5 ||
                            v->cur_ch == KEY_6 || v->cur_ch == KEY_7)
                                return;
                case OCT:
                        if (v->cur_ch == KEY_8 || v->cur_ch == KEY_9)
                                return;
                case DEC:
                        if (v->cur_ch == KEY_A || v->cur_ch == KEY_B ||
                            v->cur_ch == KEY_C || v->cur_ch == KEY_D ||
                            v->cur_ch == KEY_E || v->cur_ch == KEY_F)
                                return;
        ...
        */
        if (v->event_type == F4_PRESS || v->event_type == ARROW ||
            ...
            v->event_type == ESCAPE || v->event_type == META ||
            v->event_type == ALT)
                return;
...
static int get_next_event(Widget widget, int ev_action, XEvent *xevent) {
        ...
        if (nextc == KeyPress || nextc == KeyRelease) {
                ...
                /*
                  ...
                  When you press ctrl-space, the input method converts your
                  composed input to a Kanji character and sends a keyboard event
                  to dtcalc with a keycode of 0.  The keycode of 0 is the X
                  convention that the application should call XmbLookupString()
                  to get the Kanji character from the input method.

                  dtcalc calls XLookupString() instead though, which being a
                  much older interface, doesn't know about this convention.
                  XLookupString() isn't able to map the keycode to anything and
                  just returns 0.

                  The fix catches this and returns immediately.  Otherwise, the
                  code would go on to call event_is_keypad() to see if the event
                  was a keypad event like '+', '=', etc.  That function would
                  core dump at the following line because of the negative array
                  index:

                        return(X->kparray[xevent->xkey.keycode - X->kcmin] > 0)
                  ;

                  Returning immediately from get_next_event() avoids the core
                  dump and doesn't hurt anything because dtcalc doesn't actually
                  use any Kanji characters for its keypad characters, just ASCII
                  ones.

                  An alternative fix would be to use XmbLookupString() instead
                  of XLookupString().  That fix would be more complex though,
                  and as CDExc15419 points out, XLookupString() is fine here (as
                  long as you check its return value).
                  */
...
/* #ifdef hpux */
#ifdef HP_EXTENSIONS
                else if (ksym == XK_BackTab && down)
                        return (SHIFT);
                else if (ksym == XK_BackTab && up)
                        return (SHIFT);
                else if (ksym == hpXK_DeleteChar && down)
                        cval = 127;
                else if (ksym == hpXK_DeleteChar && up)
                        cval = 127;
#endif
...
#ifdef _AIX
                /*
                 * In keypad_keysym(), KeySym associated with
                 * xevent->xkey.keycode is extracted by calling
                 * XKeycodeToKeysym(). But the current implementation doesn't
                 * care of the modifires state. In this moment, we can still use
                 * ksym which is got in the above XLookupString(). Sun's
                 * implementation seems to need an attention for this. But AIX
                 * doesn't.
        ...
        if ((nextc == KeyPress || nextc == KeyRelease) && cval >= 0 &&
            cval <= 127) {

                /*  If this is a '*' or Return key press, then map to their
                 * better known equivalents, so that button highlighting works
                 * correctly.
                 */
...
void key_init(void) {
         ...
         *  keypad keysym, we know this key is on the keypad.  Mark it as such
         * in kparray[].
         */
...
void make_frames(void) {
        ...
        if (v->titleline == NULL) {
                ...
                snprintf(tool_label, tool_label_size, "%s",
                         lstrs[(int)L_UCALC]);
        } else
...
void make_registers(int type) {
        ...
        if (type == MEM) {
                for (i = 0; i < MAXREGS; i++) {
                        ...
                        XtVaSetValues(X->registers[i], XmNlabelString, str,
                                      NULL);
                        XmStringFree(str);
                }
        } else {
                for (i = 0; i < FINREGS; i++) {
                        ...
                        */
                        ...
                        XtVaSetValues(X->fregisters[i], XmNlabelString, str,
                                      NULL);
                        XtVaSetValues(X->fregistersvals[i], XmNlabelString,
                                      numStr, XmNalignment, XmALIGNMENT_END,
                                      NULL);
                        XmStringFree(str);
                        XmStringFree(numStr);
                }
...
static void new_cf_value(Widget widget, XtPointer client_data,
        ...
        if (mtype == M_CON) {
                ...
                XtAddCallback(X->CFpi_butHelp, XmNactivateCallback,
                              HelpRequestCB, (XtPointer)HELP_CONSTANT);
        } else {
                X->CFtype = M_FUN;
                ...
                XtRemoveAllCallbacks(X->CFpi_butHelp, XmNactivateCallback);
                XtAddCallback(X->CFpi_butHelp, XmNactivateCallback,
                              HelpRequestCB, (XtPointer)HELP_FUNCTION);
        }
...
void put_resource(enum res_type rtype, char *value) {
        ds_put_resource(&(X->dtcalcDB), v->appname, calc_res[(int)rtype],
                        value);
}
...
void redraw_buttons(void) {
        ...
        for (row = 0; row < BROWS; row++)
                for (column = 0; column < BCOLS; column++) {
                        ...
                        get_label(n);
                        lstr = XmStringCreateLocalized(v->pstr);
                        XtVaSetValues(X->kbuttons[row][column], XmNlabelString,
                                      lstr, NULL);
                        XmStringFree(lstr);
                }
        v->curwin = scurwin;
...
void save_resources(char *filename) {
        ...
        if (reply)
                _DtSimpleError(v->appname, DtWarning, NULL,
                               vstrs[(int)V_NORSAVE]);
}
        ...
        if (itemno == DISPLAYITEM) {
                if (!NoDisplay) {
                        if (str != NULL && (strcmp(str, "") != 0)) {
                                /* Let's get the decimal point, in some
                                 * languages it's a , */
                                strlcpy(displayStr, str, 50);
                                tmpStr = GETMESSAGE(3, 45, ".");
                                if (strcmp(tmpStr, ".") != 0) {
                                        /* if it's not a "." let's go change it
                ...
                if (str != NULL) {
                        ...
                        XtVaSetValues(X->modevals[(int)itemno], XmNlabelString,
                                      cstr, NULL);
                        XmStringFree(cstr);
                }
        }
...
void show_ascii_frame(void) /* Display ASCII popup. */
{
        ...
        if (X->Aframe == NULL) {
                X->Aframe =
                    (Widget)XmCreateFormDialog(X->kframe, "aframe", NULL, 0);
                tstr = XmStringCreateLocalized(pstrs[(int)P_ASCIIT]);
                XtVaSetValues(X->Aframe, XmNdialogTitle, tstr, XmNautoUnmanage,
                              FALSE, XmNallowShellResize, TRUE,
                ...
                label_string =
                    XmStringCreateLocalized(GETMESSAGE(2, 33, "Apply"));
                ...
                XtSetArg(args[9], XmNbottomOffset, 5);
                X->Api_butOK =
                    XmCreatePushButton(X->Aframe, "button", args, 10);
                XmStringFree(label_string);
                ...
                X->Api_butClose =
                    XmCreatePushButton(X->Aframe, "button", args, 11);
                XmStringFree(label_string);
                XtManageChild(X->Api_butClose);

                label_string =
                    XmStringCreateLocalized(GETMESSAGE(2, 18, "Help"));
                XtSetArg(args[0], XmNmarginHeight, 2);
                XtSetArg(args[1], XmNmarginWidth, 10);
                XtSetArg(args[2], XmNlabelString, label_string);
                XtSetArg(args[3], XmNtopAttachment, XmATTACH_WIDGET);
...
void start_tool(void) {
        ...
        XmAddWMProtocolCallback(X->kframe, saveatom, save_state,
                                (XtPointer)NULL);

        v->started = 1;
        XSetErrorHandler((int (*)())xerror_interpose);
        XtAddEventHandler(X->kframe, KeyPressMask | KeyReleaseMask, FALSE,
                          event_proc, NULL);
...
static void update_cf_value(void) {
        ...
        char cur_op, current, display[MAXLINE], old_cal_value,
            fnum[MAX_DIGITS + 1];
        ...
        case M_CON:
                ...
                if (n != 1 || ptr != NULL || v->error == TRUE) {
                        snprintf(message, MAXLINE, "%s\n%s",
                                 vstrs[(int)V_INVCON], vstrs[(int)V_NOCHANGE]);
                        do_continue_notice(X->CFframe, message);
                        set_item(OPITEM, "");
                        if (v->cur_op != '?')
                                set_item(OPITEM,
                                         buttons[get_index(v->cur_op)].str);
                        v->error = FALSE;
                        return;
                }
        ...
        for (i = 0; i < NOBUTTONS; i++)
                if (buttons[i].mtype == X->CFtype)
                        create_menu(X->CFtype,
                                    X->kbuttons[i / BCOLS][i % BCOLS], i);
        switch (X->CFtype) {
        case M_CON:
...
void write_cf_value(Widget widget, XtPointer client_data, XtPointer call_data) {
        ...
        /*
        if(focus_widget != X->CFpi_vtext->textfield && focus_widget !=
        X->CFpi_butOK) return;
        */
        ...
        if ((strcmp(X->cfval, "") == 0) || X->cfval[0] < '0' ||
            X->cfval[0] > '9' || X->cfno < 0 || X->cfno > 9) {
...
static void create_menu_bar(Widget parent) {
        ...
        child =
            (Widget)XmCreateSeparatorGadget(lastMenuPane, "separator", args, 0);
        XtManageChild(child);
        ...
        n++;
        WidgList[count++] =
            XmCreatePushButton(helpPulldown, "reference", args, n);
        XtAddCallback(WidgList[count - 1], XmNactivateCallback,
...
static void init_colors(void) {
        ...
        XmeGetPixelData(X->screen, &colorUse, pixels, &act, &inact, &prim,
                        &second);
...
void move_cf(Widget widget, XtPointer client_data, XtPointer call_data) {
        ...
        if (widget == X->CFpi_cftext->textfield) {
                ...
                if (strcmp(input, "") != 0)
                        XmProcessTraversal(X->CFpi_dtext->textfield,
                                           XmTRAVERSE_CURRENT);
                ...
                XmProcessTraversal(X->CFpi_vtext->textfield,
                                   XmTRAVERSE_CURRENT);
                value = 1;
...
static void create_popup(Widget parent) {
        ...
        XtCreateManagedWidget("separator", xmSeparatorWidgetClass, X->popupMenu,
                              NULL, 0);
        XtCreateManagedWidget("separator", xmSeparatorWidgetClass, X->popupMenu,
                              NULL, 0);
        ...
        X->asciiConv = XtVaCreateManagedWidget(
            ...
            XmStringCreateLocalized(GETMESSAGE(2, 16, "ASCII Convert")),
            XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 41, "A")), NULL);
        XtAddCallback(X->asciiConv, XmNactivateCallback,
                      (XtCallbackProc)do_ascii, (XtPointer)NULL);
        ...
        XtAddCallback(X->enterFun, XmNactivateCallback,
                      (XtCallbackProc)new_cf_value, (XtPointer)(int)M_FUN);
        ...
        dummyHelp1 = XtVaCreatePopupShell("dummyHelp1", xmMenuShellWidgetClass,
                                          X->popupMenu, XmNwidth, 10, XmNheight,
                                          10, NULL);
        ...
        XtCreateManagedWidget("separator", xmSeparatorWidgetClass, dummyHelp2,
                              NULL, 0);
        ...
        helpR = XtVaCreateManagedWidget(
            "reference", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString,
            label, XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 23, "R")), NULL);
        ...
        helpO = XtVaCreateManagedWidget(
            "onItem", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString,
            label, XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 25, "O")), NULL);
        ...
        helpU = XtVaCreateManagedWidget(
            "useHelp", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString,
            label, XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 27, "U")), NULL);
        ...
        helpV = XtVaCreateManagedWidget(
            "version", xmPushButtonGadgetClass, dummyHelp2, XmNlabelString,
            label, XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 29, "A")), NULL);
        ...
        XtVaCreateManagedWidget("help", xmCascadeButtonGadgetClass,
                                X->popupMenu, XmNsubMenuId, dummyHelp2,
        ...
        X->Close = XtVaCreateManagedWidget(
            "close", xmPushButtonWidgetClass, X->popupMenu, XmNlabelString,
            label, XmNmnemonic, XStringToKeysym(GETMESSAGE(2, 44, "X")), NULL);
        XmStringFree(label);
        ...
        if (event->type == ButtonPress && event->xbutton.button == Button3) {
                if (bevent->x >= funBtn->core.x &&
                    ...
                    bevent->y <= funBtn->core.y + funBtn->core.height +
                                     ...
                                     X->textForm->core.height) {
                        menu_handler(widget, (XtPointer)M_FUN, event,
                                     continue_to_dispatch);
                } else if (bevent->x >= constBtn->core.x &&
                           bevent->x <=
                               constBtn->core.x + constBtn->core.width &&
                           bevent->y >= constBtn->core.y +
                                            X->modeline->core.height +
                                            X->textForm->core.height &&
                           bevent->y <= constBtn->core.y +
                                            constBtn->core.height +
                                            X->modeline->core.height +
                                            X->textForm->core.height) {
                        menu_handler(widget, (XtPointer)M_CON, event,
                                     continue_to_dispatch);
                           ...
                           bevent->y <= accBtn->core.y + accBtn->core.height +
                                            X->modeline->core.height +
                                            X->textForm->core.height) {
                        menu_handler(widget, (XtPointer)M_ACC, event,
                                     continue_to_dispatch);
                           ...
                           bevent->y <= rclBtn->core.y + rclBtn->core.height +
                                            X->modeline->core.height +
                                            X->textForm->core.height) {
                        menu_handler(widget, (XtPointer)M_RCL, event,
                                     continue_to_dispatch);
                           ...
                           bevent->y <= stoBtn->core.y + stoBtn->core.height +
                                            X->modeline->core.height +
                                            X->textForm->core.height) {
                        menu_handler(widget, (XtPointer)M_STO, event,
                                     continue_to_dispatch);
                           ...
                           bevent->y <= exchBtn->core.y + exchBtn->core.height +
                                            X->modeline->core.height +
                                            X->textForm->core.height) {
                        menu_handler(widget, (XtPointer)M_EXCH, event,
                                     continue_to_dispatch);
                } else {
                        XmMenuPosition(X->popupMenu,
...
static void save_state(Widget widget, XtPointer client_data,
        ...
        if (status)
                ...
                snprintf(full_path, full_path_size, "%s/%s", dt_path,
                         DTCALC_CLASS_NAME);
        ...
        if (first == True) {
                first = False;
                XGetCommand(X->dpy, XtWindow(X->kframe), &start_argv,
                            &start_argc);
        }
...
static void SaveSession(char *path, char *file_name) {
        ...
        XTranslateCoordinates(XtDisplay(X->kframe), XtWindow(X->kframe),
                              RootWindowOfScreen(XtScreen(X->kframe)), 0, 0,
                              &t_x, &t_y, &junkWindow);
        x = (Position)t_x;
        y = (Position)t_y;
...
void RestoreSession(void) {
        ...
        if (get_str_resource(R_DISPLAY, str, MAXLINE)) {
                ...
                if (i == MAXDISPMODES) {
                        msg_size =
                            strlen(opts[(int)O_DISPLAY]) + strlen(str) + 3;
                        msg = (char *)XtMalloc(msg_size);
                        snprintf(msg, msg_size, opts[(int)O_DISPLAY], str);
                        _DtSimpleError(v->appname, DtWarning, NULL, msg);
                        XtFree(msg);
...
static int GetKeyboardID(void) {
        ...
        for (i = 0, list = slist; i < ndevices; i++, list++) {
                if (list->type != KEYBOARD && strcmp(list->name, PS2_DIN_NAME))
                        continue;
                if (list->detailed_id &
                    (HP_ITF_KBD | HP_HIL) == (HP_ITF_KBD | HP_HIL)) {
                        kbd = HIL_ITF_KBD;
                        return (kbd);
                } else if (list->detailed_id &
                           (PC101_KBD | HP_HIL) == (PC101_KBD | HP_HIL)) {
                        kbd = HIL_PC101_KBD;
                        return (kbd);
                } else if (list->detailed_id &
                           (PC101_KBD | SERIAL) == (PC101_KBD | SERIAL)) {
