...
double mods[] = {1.0,     1.0e-1,  1.0e-2,  1.0e-3,  1.0e-4,  1.0e-5,  1.0e-6,
...
void do_accuracy(void) /* Set display accuracy. */
{
        int i;
...
void do_ascii(void) /* Convert ASCII value. */
{
        show_ascii_frame();
}

void do_base(void) /* Change the current base setting. */
{
        if (v->current == BASE_BIN)
                v->base = BIN;
        else if (v->current == BASE_OCT)
                v->base = OCT;
        else if (v->current == BASE_DEC)
                v->base = DEC;
        else if (v->current == BASE_HEX)
                v->base = HEX;
        else
                return;
...
void set_base(enum base_type base) {
        grey_buttons(v->base);
...
void do_business(void) /* Perform special business mode calculations. */
{
        Boolean need_show = TRUE;
        char *display_number = NULL;
        int MPbv[MP_SIZE], MP1[MP_SIZE], MP2[MP_SIZE], MP3[MP_SIZE],
            MP4[MP_SIZE];
        ...
        if (IS_KEY(v->current, KEY_CTRM)) {
                 ...
                 */
                if (v->MPfvals[1] == 0.0 || v->MPfvals[2] == 0.0 ||
                    v->MPfvals[4] == 0.0) {
                        char *errorMsg, *tmp;
                        ...
                        errorMsg = GETMESSAGE(
                            5, 7,
                            "Can't calculate 'Compound Term'\nwithout a non "
                            "zero interest rate,\na non zero Present value, "
                        ...
                        v->error = False;
                        result = log(v->MPfvals[4] / v->MPfvals[2]) /
                                 log(1.0 + (v->MPfvals[1] / 1200));
                        if (!v->error) {
                 ...
                 *          MEM3 = period  (time period for depreciation
                 * allowance).
                 *
                 *          bv = 0.0 ;
                 *          for (i = 0; i < MEM3; i++)
                ...
                if (v->MPfvals[0] == 0.0 || v->MPfvals[1] == 0.0 ||
                    (v->MPfvals[2] == 0.0 && v->MPfvals[3] == 0.0) ||
                    v->funstate == 0) {
                        if (v->funstate == 1) {
                                v->funstate = 0;
                        ...
                        if ((w = 1.0 + v->MPfvals[1] /
                                           (v->MPfvals[5] * 100.0)) == 1.0)
                                result = -(v->MPfvals[2] +
                                           v->MPfvals[0] * v->MPfvals[3]);
                        else
                                result =
                                    -(v->MPfvals[2] * pow(w, v->MPfvals[0]) +
                                      v->MPfvals[3] *
                                          (pow(w, v->MPfvals[0]) - 1.0) *
                                          pow(w, 0.0) / (w - 1.0));
                        mpcdm(&result, v->MPdisp_val);
                }
                 ...
                 *          FMEM2 = n    (term).
                 *
                 *          RESULT = FMEM0 * (FMEM1 / (1 - pow(FMEM1 + 1, -1 *
                 * FMEM2)))
                 */

                if (v->MPfvals[0] == 0.0 || v->MPfvals[1] == 0.0 ||
                    (v->MPfvals[2] == 0.0 && v->MPfvals[4] == 0.0) ||
                    v->funstate == 0) {
                        if (v->funstate == 1) {
                                v->funstate = 0;
                                doerr(GETMESSAGE(5, 2, "ERROR: No Solution"));
                        ...
                        if ((w = 1.0 + v->MPfvals[1] /
                                           (v->MPfvals[5] * 100.0)) == 1.0)
                                result = -(v->MPfvals[4] + v->MPfvals[2]) /
                                         v->MPfvals[0];
                        else
                                result =
                                    -(v->MPfvals[2] * pow(w, v->MPfvals[0]) +
                 ...
                 *          FMEM2 = n   (term).
                 *
                 *          RESULT = FMEM0 * (1 - pow(1 + FMEM1, -1 * FMEM2)) /
                 * FMEM1
                 */
                ...
                if (v->MPfvals[0] == 0.0 || v->MPfvals[1] == 0.0 ||
                    v->MPfvals[3] == 0.0 || v->funstate == 0) {
                        ...
                        if ((w = 1.0 + v->MPfvals[1] /
                                ...
                                result =
                                    -(v->MPfvals[4] / pow(w, v->MPfvals[0]) +
                                      v->MPfvals[3] *
                                          (pow(w, v->MPfvals[0]) - 1.0) *
                                          pow(w, 0.0 - v->MPfvals[0]) /
                                          (w - 1.0));
                        mpcdm(&result, v->MPdisp_val);
                }
                if (strcmp(v->display, GETMESSAGE(3, 364, "Error")) != 0)
                ...
                if (v->MPfvals[0] == 0.0 ||
                    ...
                    v->funstate == 0) {
                        if (v->funstate == 1) {
                                ...
                                v->accuracy = 2;
                                display_number =
                                    make_number(v->MPdisp_val, FALSE);
                                MPstr_to_num(display_number, DEC,
                                             v->MPdisp_val);
                                /* set RATE register */
                 ...
                 *          MEM1 = salvage (salvage value of the asset).
                 *          MEM2 = life    (useful life of the asset).
                 *          MEM3 = period  (period for which depreciation is
                 * computed).
                 *
                 ...
                 *
                 *          RESULT = log(1 + (FMEM1 * FMEM2 / FMEM0)) / log(1 +
                 * FMEM2)
                 */

                if (v->MPfvals[1] == 0.0 ||
                    (v->MPfvals[2] == 0.0 && v->MPfvals[4] == 0) ||
                    v->MPfvals[3] == 0.0 || v->funstate == 0) {
                        if (v->funstate == 1) {
                                v->funstate = 0;
                                doerr(GETMESSAGE(5, 2, "ERROR: No Solution"));
                        ...
                        if ((w = 1.0 + v->MPfvals[1] /
                                ...
                                result =
                                    log((v->MPfvals[3] * wdb / (w - 1.0) -
                                         v->MPfvals[4]) /
                                        (v->MPfvals[2] * pow(w, 0.0) +
                                         v->MPfvals[3] * wdb / (w - 1.0))) /
                                    log(w);
                        }

                        if (strcmp(v->display, GETMESSAGE(3, 364, "Error")) !=
                            0)
                                mpcdm(&result, v->MPdisp_val);
                }
                if (strcmp(v->display, GETMESSAGE(3, 364, "Error")) != 0)
...
void do_calc(void) /* Perform arithmetic calculation and display result. */
{
        double dval, dres;
        int MP1[MP_SIZE];
        ...
        if (!(v->opsptr &&
              !v->show_paren)) { /* Don't do if processing parens. */
                if (IS_KEY(v->current, KEY_EQ) &&
                    IS_KEY(v->old_cal_value, KEY_EQ)) {
                        if (v->new_input) {
...
void do_clear(void) /* Clear the calculator display and re-initialise. */
{
        clear_display();
        if (v->error)
                set_item(DISPLAYITEM, "");
        initialise();
}
...
void do_delete(void) /* Remove the last numeric character typed. */
{
        if (strlen(v->display))
                v->display[strlen(v->display) - 1] = '\0';

        /*  If we were entering a scientific number, and we have backspaced over
         *  the exponent sign, then this reverts to entering a fixed point
         * number.
         */
...
void do_exchange(void) /* Exchange display with memory register. */
{
...
void do_expno(void) /* Get exponential number. */
{
        /* the financial state is false - last key was not a fin. key */
        v->funstate = 0;
...
void do_factorial(int *MPval,
                  int *MPres) /* Calculate the factorial of MPval. */
{
        double val;
...
void do_frame(void) /* Exit dtcalc. */
{
        exit(0);
}

void do_function(void) /* Perform a user defined function. */
{
...
void do_immed(void) {
        ...
        else if (IS_KEY(v->current, KEY_32)) /* &32 */
        {
                ...
                dval2 = ibool2(dval);
                if (dval2 == 0)
                        doerr(GETMESSAGE(5, 6,
                                         "ERR:Num too large for operation"));
                else {
...
void do_keys(void) /* Display/undisplay the dtcalc key values. */
{
        v->tstate = !v->tstate;
        redraw_buttons();
}

void do_mode(void) /* Set special calculator mode. */
{
        if (v->current == MODE_FIN)
                v->modetype = FINANCIAL;
        else if (v->current == MODE_LOG)
                v->modetype = LOGICAL;
        else if (v->current == MODE_SCI)
...
void do_none(void) /* Null routine for empty buttons. */
{}
...
void do_numtype(void) /* Set number type (engineering, fixed or scientific). */
{
        if (v->current == DISP_ENG)
                v->dtype = ENG;
        else if (v->current == DISP_FIX)
                v->dtype = FIX;
        else if (v->current == DISP_SCI)
                v->dtype = SCI;
        else
                return;
...
void do_paren(void) {
        ...
        /*  Check to see if this is the first outstanding parenthesis. If so,
         * and their is a current operation already defined, then add the
         * current operation to the parenthesis expression being displayed.
         *  Increment parentheses count, and add the open paren to the
         * expression.
         */

        if (IS_KEY(v->current, KEY_LPAR)) {
                if (v->noparens == 0) {
                        /* if not in default state, put the operand between the
                           display value and the paren, else just put the paren
                         */
                        if (!v->defState) {
                                /* there is no paren, and there is no current
                                   operand ... Let's make the current operand
                                   into a "x" */
                                if (v->cur_op == '?') {
                                        v->current = 'x';
                                        do_calc();
                                ...
                                /* if the current op is an '=' and the result in
                                   the display is zero, we want to ignore the
                                   display */
                                if (v->cur_op == '=') {
        ...
        /*  If we haven't had any left brackets yet, and this is a right
         * bracket, then just ignore it. Decrement the bracket count. If the
         * count is zero, then process the parenthesis expression.
         */
...
void do_pending(void) {
        ...
        /*  Certain pending operations which are half completed, force the
         * numeric keypad to be reshown (assuming they already aren't).
         *
        ...
        if (IS_KEY(v->pending, KEY_BASE))
                do_base(); /* Base */
        else if (IS_KEY(v->pending, KEY_DISP))
                do_numtype(); /* Disp */
        else if (IS_KEY(v->pending, KEY_TRIG))
                do_trigtype(); /* Trig */
        else if (IS_KEY(v->pending, KEY_CON))
                do_constant(); /* Con */
        else if (IS_KEY(v->pending, KEY_EXCH))
                do_exchange(); /* Exch */
        else if (IS_KEY(v->pending, KEY_FUN))
                do_function();                  /* Fun */
                 ...
                 IS_KEY(v->pending, KEY_RCL))   /* Rcl */
        {
                ...
                if (IS_KEY(v->pending_op, KEY_ADD) ||
                    ...
                    IS_KEY(v->pending_op, KEY_DIV))
                        return;
        } else if (IS_KEY(v->pending, KEY_LSFT) || /* < */
                   IS_KEY(v->pending, KEY_RSFT))
                do_shift(); /* > */
        else if (IS_KEY(v->pending, KEY_ACC))
                do_accuracy(); /* Acc */
        else if (IS_KEY(v->pending, KEY_MODE))
                do_mode();                     /* Mode */
        ...
        if (v->error)
                ...
                set_item(OPITEM, ""); /* Redisplay pending op. (if any). */

        v->pending = 0;
        if (!v->ismenu)
                grey_buttons(
                    v->base); /* Just show numeric keys for current base. */
}

void do_point(void) /* Handle numeric point. */
{
        /* the financial state is false - last key was not a fin. key */
        v->funstate = 0;
...
void do_shift(void) /* Perform bitwise shift on display value. */
{
        ...
        if (IS_KEY(v->pending, KEY_LSFT))
                temp = temp << shift;
        else if (IS_KEY(v->pending, KEY_RSFT))
                temp = temp >> shift;
...
void do_sto_rcl(void) /* Save/restore value to/from memory register. */
{
        int i, MPn[MP_SIZE], n;
        ...
        for (i = MEM_START; i <= MEM_END; i++)
                if (v->current == menu_entries[i].val) {
                        ...
                        {
                                mpstr(v->MPmvals[char_val(v->current)],
                                      v->MPdisp_val);
                                v->new_input = 1;
                        } else if (IS_KEY(v->pending, KEY_STO)) /* Sto */
                        {
                                ...
                                if (IS_KEY(v->pending_op, KEY_ADD)) /* + */
                                {
                                        ...
                                        mpadd(MPn, v->MPdisp_val,
                                              v->MPmvals[n]);
                                } else if (IS_KEY(v->pending_op,
                                                  KEY_SUB)) /* - */
                                {
                                        mpstr(v->MPmvals[n], MPn);
                                        mpsub(MPn, v->MPdisp_val,
                                              v->MPmvals[n]);
                                } else if (IS_KEY(v->pending_op,
                                                  KEY_MUL)) /* x */
                                {
                                        mpstr(v->MPmvals[n], MPn);
                                        mpmul(MPn, v->MPdisp_val,
                                              v->MPmvals[n]);
                                } else if (IS_KEY(v->pending_op,
                                                  KEY_DIV)) /* / */
                                {
                                        mpstr(v->MPmvals[n], MPn);
                                        mpdiv(MPn, v->MPdisp_val,
                                              v->MPmvals[n]);
                                } else
...
void do_trig(void) /* Perform all trigonometric functions. */
{
...
void do_trigtype(void) /* Change the current trigonometric type. */
{
        if (v->current == TRIG_DEG)
                v->ttype = DEG;
        else if (v->current == TRIG_GRA)
                v->ttype = GRAD;
        else if (v->current == TRIG_RAD)
                v->ttype = RAD;
        else
                return;

        if (IS_KEY(v->cur_op, KEY_COS) || IS_KEY(v->cur_op, KEY_SIN) ||
...
void process_parens(char current) {
        int i;
        int last_lpar; /* Position in stack of last left paren. */
        int last_num;  /* Position is numeric stack to start processing. */

        /*  Check to see if this is the first outstanding parenthesis. If so,
         * and their is a current operation already defined, then push the
         * current result on the numeric stack, and note it on the op stack,
         * with a -1, which has this special significance. Zeroise current
         * display value (in case of invalid operands inside the parentheses.
        ...
        /*  If we haven't had any left brackets yet, and this is a right
         * bracket, then just ignore it. Decrement the bracket count. Add a
         * equals to the op stack, to force a calculation to be performed for
         * two op operands. This is ignored if the preceding element of the op
         * stack was an immediate operation. Work out where the preceding left
         * bracket is in the stack, and then process the stack from that point
         * until this end, pushing the result on the numeric stack, and setting
         * the new op stack pointer appropriately. If there are no brackets
         * left unmatched, then clear the pending flag, clear the stack
         * pointers and current operation, and show the display.
         */
                ...
                process_stack(last_lpar + 1, last_num,
                              v->opsptr - last_lpar - 1);
                if (!v->noparens) {
                        if (v->opsptr > 1) {
                                push_op(KEY_EQ);
                        ...
                        if (v->error) {
                                ...
                                set_item(OPITEM, vstrs[(int)V_CLR]);
                                strlcpy(v->display, vstrs[(int)V_ERROR],
                                        MAXLINE);
                        } else {
                                show_display(v->MPdisp_val);
                                mpstr(v->MPdisp_val, v->MPlast_input);
...
void push_num(int *MPval) /* Try to push value onto the numeric stack. */
{
        if (v->numsptr < 0)
                return;
...
void push_op(int val) /* Try to push value onto the operand stack. */
{
        if (v->opsptr < 0)
                return;
...
void save_pending_values(int val) {
        int n;
...
double do_round(double result, int ndigits) {
        ...
        for (;;) {
                ...
                if (v->MPfvals[3] != 0.0) {
                        if (w == 1.0) {
                                ...
                                term_prime =
                                    (v->MPfvals[0] *
                                         pow(w, (0.0 + v->MPfvals[0] - .01)) +
                                     (wn - 1.0) * (0.0) * pow(w, (0.0 - 1.0))) /
                                        (w - 1.0) -
                                    (wn - 1.0) * wdb / ((w - 1.0) * (w - 1.0));
                        }
                        if (v->MPfvals[3] > 0.0) {
                ...
                case 1:
                        f = lsp - lsn;
                        f_prime =
                            sum_pos_prime / sum_pos - sum_neg_prime / sum_neg;
                        break;
                case 2:
                        f = lsp / lsn - 1.0;
