...
void mpadd2(int *x, int *y, int *z, int *y1, int *trunc) {
        int i__1, i__2;
...
void mpadd3(int *x, int *y, int *s, int *med, int *re) {
        int i__1;
...
void mpaddi(int *x, int *iy, int *z) {
...
void mpaddq(int *x, int *i, int *j, int *y) {
        ...
        *dz = 0.;
        ...
        for (i = 1; i <= i__1; ++i) {
                *dz = db * *dz + (double)((float)x[i + 2]);
                ...
                /*  TEST BELOW NOT ALWAYS EQUIVALENT TO - IF (DZ2.LE.DZ) GO TO
                 * 20, FOR EXAMPLE ON CYBER 76.
                 */
                if (dz2 - *dz <= 0.)
                        goto L20;
        }
...
L20:
        ...
        *dz *= mppow_di(&db, &i__1);
        ...
        if ((d__1 = (double)((float)x[2]) -
                    (log(*dz) / log((double)((float)MP.b)) + .5),
             C_abs(d__1)) > .6)
                goto L30;
...
void mpdivi(int *x, int *iy, int *z) {
        int i__1, i__2;
...
L80:
        ...
        MP.t = ts;
        mpadd2(&MP.r[i3 - 1], &MP.r[i2 - 1], &MP.r[i3 - 1], &MP.r[i2 - 1],
               &c__0);
        if (MP.r[i2 - 1] != 0)
                goto L80;
...
L110:
        if (dabs(rx) > (float)10.0)
                return;

        mpcmr(&y[1], &ry);
        if ((r__1 = ry - exp(rx), dabs(r__1)) < ry * (float)0.01)
                return;
...
L40:
        ...
        q = (int)(sqrt((float)MP.t * (float).48 * rlb) +
                  (float)x[2] * (float)1.44 * rlb);
...
void mpmlp(int *u, int *v, int *w, int *j) {
        int i__1;
...
void mpmul2(int *x, int *iy, int *z, int *trunc) {
        int i__1, i__2;
...
void mpmuli(int *x, int *iy, int *z) {
...
void mpmulq(int *x, int *i, int *j, int *y) {
        int i__1;
...
void mpnzr(int *rs, int *re, int *z, int *trunc) {
        int i__1;
...
void mppi(int *x) {
        ...
        mpcmr(&x[1], &rx);
        if ((r__1 = rx - (float)3.1416, dabs(r__1)) < (float)0.01)
                return;

        /* FOLLOWING MESSAGE MAY INDICATE THAT B**(T-1) IS TOO SMALL */

        if (v->MPerrors)
                _DtSimpleError(v->appname, DtWarning, NULL, mpstrs[(int)MP_PI]);

        mperr();
        return;
}

void mppwr(int *x, int *n, int *y) {
        static int i2, n2, ns;
...
L110:
        ...
        r__1 =
            exp(((float)(np * ex - xes) * log((float)MP.b) - log((dabs(rx)))) /
                (float)np);
        mpcrm(&r__1, &MP.r[i2 - 1]);
...
void mpset(int *idecpl, int *itmax2, int *maxdr) {
        int i__1;
...
void mpsin1(int *x, int *y, int *is) {
        int i__1;
...
double mppow_di(double *ap, int *bp) {
        double pow, x;
        int n;
...
double mppow_ri(float *ap, int *bp) {
        double pow, x;
        int n;
