...
void Help(char *helpVolume, char *locationId) {
        Arg args[10];
        Position newX, newY;
        int n;
        ...
        if (X->helpDialog == NULL) {
                ...
                XtSetArg(args[n], XmNtitle,
                         GETMESSAGE(4, 1, "Calculator - Help"));
                n++;
                X->helpDialog =
                    DtCreateHelpDialog(X->mainWin, "helpDlg", args, n);

                DtWsmRemoveWorkspaceFunctions(
                    X->dpy, XtWindow(XtParent(X->helpDialog)));
...
void HelpCloseCB(Widget widget, caddr_t client_data, caddr_t call_data) {

        HelpStruct *pHelpCache = (HelpStruct *)client_data;
        HelpStruct *pHelp;

        for (pHelp = pHelpCache;
             pHelp != (HelpStruct *)NULL && pHelp->dialog != widget;
             pHelp = pHelp->pNext)
                ;
...
void HelpHyperlinkCB(Widget widget, caddr_t client_data, caddr_t call_data) {
...
void HelpRequestCB(Widget widget, caddr_t client_data, caddr_t call_data) {
        ...
        ignore_event = True;
        timerId =
            XtAppAddTimeOut(XtWidgetToApplicationContext(X->kframe), 300,
                            (XtTimerCallbackProc)TimerEvent, (XtPointer)NULL);
}

void HelpModeCB(Widget w, caddr_t client_data, caddr_t call_data) {
        Widget widget;
        char *errorMsg, *tmp;

        switch (
            DtHelpReturnSelectedWidgetId(X->mainWin, (Cursor)NULL, &widget)) {
        ...
        case DtHELP_SELECT_INVALID:
                errorMsg = GETMESSAGE(
                    4, 2, "You must select an item within Calculator");
...
static Widget GetHelpDialog(void) {
        ...
        if (pHelpCache == (HelpStruct *)NULL) {
                ...
                XtSetArg(args[0], XmNtitle,
                         GETMESSAGE(4, 1, "Calculator - Help"));
                pHelp->dialog =
                    DtCreateHelpDialog(X->mainWin, "helpDlg", args, 1);

                DtWsmRemoveWorkspaceFunctions(
                    X->dpy, XtWindow(XtParent(pHelp->dialog)));
                ...
                for (pHelp = pHelpCache; pHelp != (HelpStruct *)NULL;
                     pHelp = pHelp->pNext) {
...
void ErrDialog(char *errString, Widget visualParent) {
        ...
        if (X->errDialog == NULL) {
                ...
                X->errDialog =
                    XmCreateErrorDialog(X->mainWin, "ErroNotice", args, n);

                XtAddCallback(X->errDialog, XmNokCallback, UnmanageCB, NULL);
                XtAddCallback(X->errDialog, XmNmapCallback, CenterMsgCB, NULL);

                XtUnmanageChild(
                    XmMessageBoxGetChild(X->errDialog, XmDIALOG_CANCEL_BUTTON));

                XtUnmanageChild(
                    XmMessageBoxGetChild(X->errDialog, XmDIALOG_HELP_BUTTON));
                ...
                n++;
                XtSetArg(args[n], XmNtitle, GETMESSAGE(4, 3, "Error"));
                n++;
...
static void UnmanageCB(Widget widget, XtPointer client_data,
                       XtPointer call_data) {
        XtUnmanageChild(widget);
}

void CenterMsgCB(Widget widget, XtPointer client_data, XtPointer call_data) {
        int n;
        Position newX, newY;
        Arg args[4];
        Widget shell;
