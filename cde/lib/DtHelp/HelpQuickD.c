...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
                         Cardinal *num_args);
static void CloseQuickCB(Widget w, XtPointer clientData, XtPointer callData);
static void HelpButtonCB(Widget w, XtPointer clientData, XtPointer callData);
static void PrintQuickHelpCB(Widget w, XtPointer clientData,
                             XtPointer callData);
static void VariableInitialize(DtHelpQuickDialogWidget nw);
static void FreeQuickHelpInfo(Widget nw, int cleanUpKind);
static void CatchClose(Widget widget);
static void SetupTopic(Widget nw, int updateKind);
static void SetupDisplayType(Widget nw, int updateKind);
static void ProcessJumpReuse(Widget nw, DtHelpHyperTextStruct *hyperData);
static void ProcessBackCB(Widget w, XtPointer clientData, XtPointer callData);
static void UpdateJumpList(char *topicInfo, int topicType, Widget nw);
static void ResizeQuickDialogCB(XtPointer clientData);
...
static XtResource resources[] = {
    ...
    {DtNcloseLabelString, DtCCloseLabelString, XmRXmString, sizeof(XmString),
     XtOffset(DtHelpQuickDialogWidget, qhelp_dialog.qhelp.closeLabelString),
     XmRString, NULL},

    {DtNmoreLabelString, DtCMoreLabelString, XmRXmString, sizeof(XmString),
     XtOffset(DtHelpQuickDialogWidget, qhelp_dialog.qhelp.moreLabelString),
     XmRString, NULL},

    {DtNbackLabelString, DtCBackLabelString, XmRXmString, sizeof(XmString),
     XtOffset(DtHelpQuickDialogWidget, qhelp_dialog.qhelp.backLabelString),
     XmRString, NULL},

    {DtNhelpLabelString, DtCHelpLabelString, XmRXmString, sizeof(XmString),
     XtOffset(DtHelpQuickDialogWidget, qhelp_dialog.qhelp.helpLabelString),
     XmRString, NULL},

    {DtNprintLabelString, DtCPrintLabelString, XmRXmString, sizeof(XmString),
...
externaldef(dthelpquickdialogwidgetclassrec)
    DtHelpQuickDialogWidgetClassRec dtHelpQuickDialogWidgetClassRec = {
        {
            /* core_class fields  */
...
externaldef(dthelpquickdialogwidgetclass)
...
static void VariableInitialize(DtHelpQuickDialogWidget nw) {
        ...
        if (qw->qhelp_dialog.display.locationId != NULL)
                qw->qhelp_dialog.display.locationId =
                    XtNewString(qw->qhelp_dialog.display.locationId);
        ...
        if (qw->qhelp_dialog.display.manPage != NULL)
                qw->qhelp_dialog.display.manPage =
                    XtNewString(qw->qhelp_dialog.display.manPage);
        if (qw->qhelp_dialog.display.stringData != NULL)
                qw->qhelp_dialog.display.stringData =
                    XtNewString(qw->qhelp_dialog.display.stringData);
        if (qw->qhelp_dialog.display.helpFile != NULL)
                qw->qhelp_dialog.display.helpFile =
                    XtNewString(qw->qhelp_dialog.display.helpFile);
        ...
        if (qw->qhelp_dialog.display.topicTitleStr != NULL)
                qw->qhelp_dialog.display.topicTitleLbl =
 ...
 *****************************************************************************/
static int FilterExecCmdCB(void *clientData, const char *cmdStr,
        ...
        return _DtHelpFilterExecCmd(
            (Widget)qw, cmdStr, qw->qhelp_dialog.display.executionPolicy, True,
            &qw->qhelp_dialog.help, ret_filteredCmdStr, hv_path);
}
...
static void Initialize(Widget rw, Widget nw, ArgList args_init,
        ...
        /* Now remove BulletinBoard Unmanage callback from apply and help
         * buttons. */
        XtRemoveAllCallbacks(qw->qhelp_dialog.qhelp.helpButton,
                             XmNactivateCallback);
        /* and add ours */

        pHelpInfo = _DtHelpListAdd(DtHELP_quickHelpBtn_STR, (Widget)qw,
        ...
        /* Shell help */
        pHelpInfo = _DtHelpListAdd(DtHELP_quickHelpShell_STR, (Widget)qw,
                                   &qw->qhelp_dialog.help,
                                   &qw->qhelp_dialog.help.pHelpListHead);
        XtAddCallback((Widget)qw, XmNhelpCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
        ...
        qw->qhelp_dialog.help.pDisplayArea = _DtHelpCreateDisplayArea(
            (Widget)qw->qhelp_dialog.qhelp.displayAreaFrame, "DisplayArea",
            ((short)qw->qhelp_dialog.display.scrollBarPolicy),
            ((short)qw->qhelp_dialog.display.scrollBarPolicy), False,
            ((int)qw->qhelp_dialog.display.textRows),
            ((int)qw->qhelp_dialog.display.textColumns),
            _DtHelpQuickDialogHypertextCB, ResizeQuickDialogCB, FilterExecCmdCB,
            (XtPointer)qw, defaultList);
        ...
        XtAddCallback(XtParent(qw), XmNpopupCallback,
                      (XtCallbackProc)InitialPopupCB, (XtPointer)qw);
...
static void InitialPopupCB(Widget w, XtPointer clientData, XtPointer callData) {
        ...
        XtRemoveCallback(XtParent(qw), XmNpopupCallback,
                         (XtCallbackProc)InitialPopupCB, (XtPointer)qw);
}
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
        ...
        if ((current->qhelp_dialog.display.textRows !=
             qw->qhelp_dialog.display.textRows) ||
            (current->qhelp_dialog.display.textColumns !=
             qw->qhelp_dialog.display.textColumns)) {
        ...
        if (current->qhelp_dialog.display.helpVolume !=
            qw->qhelp_dialog.display.helpVolume) {
                qw->qhelp_dialog.display.helpVolume =
                    XtNewString(qw->qhelp_dialog.display.helpVolume);
        ...
        if (current->qhelp_dialog.display.locationId !=
            qw->qhelp_dialog.display.locationId) {
                qw->qhelp_dialog.display.locationId =
                    XtNewString(qw->qhelp_dialog.display.locationId);
        ...
        _DtHelpPrintSetValues(
            &current->qhelp_dialog.print, &qw->qhelp_dialog.print,
            &qw->qhelp_dialog.display, &qw->qhelp_dialog.help);

        /* Check DtNmanPage resource for change */
        if (current->qhelp_dialog.display.manPage !=
            qw->qhelp_dialog.display.manPage) {
                qw->qhelp_dialog.display.manPage =
                    XtNewString(qw->qhelp_dialog.display.manPage);
        ...
        if (current->qhelp_dialog.display.stringData !=
            qw->qhelp_dialog.display.stringData) {
                qw->qhelp_dialog.display.stringData =
                    XtNewString(qw->qhelp_dialog.display.stringData);
        ...
        if (current->qhelp_dialog.display.helpFile !=
            qw->qhelp_dialog.display.helpFile) {
                qw->qhelp_dialog.display.helpFile =
                    XtNewString(qw->qhelp_dialog.display.helpFile);
        ...
        if ((current->qhelp_dialog.display.helpType !=
             qw->qhelp_dialog.display.helpType) ||
            (updateRequest)) {
 ...
 *****************************************************************************/
/*ARGSUSED*/
static void SeparatorFix(XmGeoMatrix geoSpec, int action,
 ...
 *
 *****************************************************************************/
XmGeoMatrix _DtHelpQuickDialogWidgetGeoMatrixCreate(Widget wid,
                                                    Widget instigator,
                                                    XtWidgetGeometry *desired) {
 ...
 *
 *****************************************************************************/
Widget DtCreateHelpQuickDialog(Widget parent, char *name, ArgList al,
                               Cardinal ac) {
        Widget w;
        _DtHelpWidgetToAppContext(parent);

        _DtHelpAppLock(app);
        w = XmeCreateClassDialog(dtHelpQuickDialogWidgetClass, parent, name, al,
                                 ac);
 ...
 *
 *****************************************************************************/
Widget DtHelpQuickDialogGetChild(Widget widget, unsigned char child) {
 ...
 *
 ****************************************************************************/
void _DtHelpQuickDialogHypertextCB(XtPointer pDisplayAreaStruct,
                                   XtPointer clientData,
                                   DtHelpHyperTextStruct *hyperData) {
        ...
        case _DtCvLinkType_CrossLink:
                ...
                case _DtCvWindowHint_NewWindow:
                        ...
                        if (qw->qhelp_dialog.display.hyperLinkCallback !=
                            NULL) {
                                ...
                                if (pTempAccessPath !=
                                    NULL) /* Use the New one */
                                        ...
                                        callData.helpVolume =
                                            XtNewString(qw->qhelp_dialog.display
                                                            .helpVolume);
                                ...
                                XtCallCallbackList(
                                    (Widget)qw,
                                    qw->qhelp_dialog.display.hyperLinkCallback,
                                    &callData);
        ...
        case _DtCvLinkType_Execute:
                _DtHelpExecFilteredCmd((Widget)qw, hyperData->specification,
...
static void SetupDisplayType(Widget nw, int updateKind) {
        ...
        if (qw->qhelp_dialog.backtr.pJumpListHead != NULL) {
                ...
                tmpPtr->scrollPosition = _DtHelpGetScrollbarValue(
                    qw->qhelp_dialog.help.pDisplayArea);
        }
        ...
        case DtHELP_TYPE_STRING:
                ...
                status = _DtHelpFormatAsciiString(
                    qw->qhelp_dialog.help.pDisplayArea,
                    qw->qhelp_dialog.display.stringData, &topicHandle);
                if (status >= VALID_STATUS) {
                        _DtHelpDisplayAreaSetList(
                            qw->qhelp_dialog.help.pDisplayArea, topicHandle,
                            FALSE, qw->qhelp_dialog.backtr.scrollPosition);
                        ...
                        _DtHelpDisplayFormatError(
                            qw->qhelp_dialog.help.pDisplayArea, (Widget)qw,
        ...
        case DtHELP_TYPE_FILE:
                ...
                status = _DtHelpFormatAsciiFile(
                    qw->qhelp_dialog.help.pDisplayArea,
                    qw->qhelp_dialog.display.helpFile, &topicHandle);
                if (status >= VALID_STATUS) {
                        _DtHelpDisplayAreaSetList(
                            qw->qhelp_dialog.help.pDisplayArea, topicHandle,
                            FALSE, qw->qhelp_dialog.backtr.scrollPosition);
        ...
        default:
                ...
                _DtHelpDisplayFormatError(qw->qhelp_dialog.help.pDisplayArea,
                                          (Widget)qw, tmpError,
                                          (char *)QHDMessage7);
...
static void SetupTopic(Widget nw, int updateKind) {
        ...
        if (qw->qhelp_dialog.display.helpVolume != NULL) {
                validPath = _DtHelpExpandHelpVolume(
                    (Widget)qw, &qw->qhelp_dialog.display,
                    &qw->qhelp_dialog.help, &qw->qhelp_dialog.print);
        }
        ...
        if (validPath) {
                ...
                if (qw->qhelp_dialog.display.locationId != NULL) {
                        ...
                        status = _DtHelpFormatTopic(
                            ...
                            qw->qhelp_dialog.display.locationId, True,
                            &topicHandle);
        ...
        if (!validPath) {
                if (qw->qhelp_dialog.display.helpVolume == NULL) {
                        ...
                        /* We cannot find our HelpVolume so display the proper
                         * error message in the help dialog and continue.
                         */
                        tmpMsg = XtNewString((char *)_DTGETMESSAGE(
                            2, 58,
                            "The requested online help is either not installed "
                            "or not in the "
                            "proper help search path.  For information on "
                            "installing online "
                            "help, consult the documentation for the "
                            "product.\n"));
                        locTitle = XtNewString(
                            (char *)_DTGETMESSAGE(2, 62, "Location ID:"));
                        volumeTitle = XtNewString(
                            (char *)_DTGETMESSAGE(2, 61, "Help Volume:"));
                        int tmpError_size =
                            strlen(tmpMsg) + strlen(tmpMsg) +
                            strlen(volumeTitle) +
                            strlen(qw->qhelp_dialog.display.locationId) +
                            strlen(qw->qhelp_dialog.display.helpVolume) + 4;
                        tmpError = XtMalloc(tmpError_size);

                        strlcpy(tmpError, tmpMsg, tmpError_size);
                        strlcat(tmpError, volumeTitle, tmpError_size);
                        strlcat(tmpError, " ", tmpError_size);
                        strlcat(tmpError, qw->qhelp_dialog.display.helpVolume,
                                tmpError_size);
                        strlcat(tmpError, "\n", tmpError_size);
                        strlcat(tmpError, locTitle, tmpError_size);
                        strlcat(tmpError, " ", tmpError_size);
                        strlcat(tmpError, qw->qhelp_dialog.display.locationId,
                                tmpError_size);
                ...
                if (qw->qhelp_dialog.display.locationId == NULL) {
                        tmpMsg =
                            _DTGETMESSAGE(2, 59, "No location ID specified.");
                        userErrorStr = XtNewString(tmpMsg);
                } else {
                        tmpMsg =
                            _DTGETMESSAGE(2, 55, "Nonexistent location ID:");
...
static void FreeQuickHelpInfo(Widget nw, int cleanUpKind) {
        ...
        if (cleanUpKind == DtCLEAN_FOR_DESTROY) {
                ...
                _DtHelpFreePrintStuff(&qw->qhelp_dialog.print,
                                      DtCLEAN_FOR_DESTROY);
...
static void UpdateJumpList(char *topicInfo, int topicType, Widget nw) {
        ...
        _DtHelpTopicListAddToHead(topicInfo, NULL, topicType, 999,
                                  ...
                                  &qw->qhelp_dialog.backtr.totalJumpNodes, -1);
...
static void ProcessBackCB(Widget w, XtPointer clientData, XtPointer callData) {
        ...
        if (qw->qhelp_dialog.backtr.totalJumpNodes <= 1) {
                ...
                XmProcessTraversal(qw->qhelp_dialog.qhelp.closeButton,
                                   XmTRAVERSE_HOME);
        } else
                XtSetSensitive(qw->qhelp_dialog.qhelp.backButton, TRUE);
...
static void CloseQuickCB(Widget w, XtPointer clientData, XtPointer callData) {
        ...
        if (qw->qhelp_dialog.qhelp.closeCallback != NULL) {
                ...
                XtCallCallbackList((Widget)qw,
                                   qw->qhelp_dialog.qhelp.closeCallback,
                                   &callDataInfo);
        } else
