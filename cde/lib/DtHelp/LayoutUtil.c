...
static const _DtCvTraversalInfo DefTravData = {
    _DtCvFALSE /* active   */, _DtCvTraversalNone /* type     */,
    -1 /* idx      */,         0 /* x_pos    */,
    0 /* y_pos    */,          0 /* width    */,
    0 /* height   */,          NULL /* *seg_ptr */
};
 ...
 *****************************************************************************/
static _DtCvValue IsTrueMultiByte(wchar_t wc_char) {
        char buf[MB_LEN_MAX];
 ...
 *			the MultiCantEndList.
 *****************************************************************************/
static _DtCvValue CheckList(wchar_t wc_char, const wchar_t *list) {
        /*
 ...
 *
 *****************************************************************************/
static int CompareTraversalPos(const void *a, const void *b) {
 ...
 *
 *****************************************************************************/
void _DtCvInitLayoutInfo(_DtCanvasStruct *canvas, _DtCvLayoutInfo *layout) {
        *layout = DefLayInfo;
 ...
 *
 *****************************************************************************/
int _DtCvGetTraversalWidth(_DtCanvasStruct *canvas, _DtCvSegmentI *p_seg,
                           int lst_hyper) {
        int value = 0;
        int lnkBefore = 0;
        int lnkAfter = 0;
        ...
        if (lst_hyper != p_seg->link_idx) {
                ...
                if (_DtCvIsSegALink(p_seg)) {
                         ...
                         * leaving room for the traversal and link end
                         * indicators
                         */
                        if (lst_hyper != -1)
                                value += (canvas->traversal_info.space_after +
                                          lnkAfter);
 ...
 *****************************************************************************/
void _DtCvAddSpace(_DtCvUnit number, _DtCvUnit *ret_y) {
        /*
         * anything to do?
         */
        if (0 >= number)
 ...
 *			the OneByteCantEndList.
 *****************************************************************************/
_DtCvValue _DtCvCheckOneByteCantEndList(char c, char *cant_end_list) {
        int i;
 ...
 *			the OneByteCantBeginList.
 *****************************************************************************/
_DtCvValue _DtCvCheckOneByteCantBeginList(char c, char *cant_begin_list) {
        int i;
 ...
 *
 *****************************************************************************/
_DtCvValue _DtCvCheckLineSyntax(_DtCanvasStruct *canvas, _DtCvSegmentI *pSeg,
                                int start, int str_len,
        ...
        if (_DtCvIsSegString(pSeg)) {
                ...
                pChar =
                    _DtCvStrPtr(_DtCvStringOfStringSeg(pSeg), wcFlag, start);
        ...
        if (myStrLen > str_len) {
                ...
                if (False == skip_hypen_ck && wcFlag &&
                    CheckList(nextChar, canvas->locale.cant_begin_chars) ==
                        False &&
                    True == nxtCharMb)
                        return True;
         ...
         */
        if (False == lstCharMb &&
            _DtCvCheckOneByteCantEndList((char)lastChar, OneByteCantEndList) ==
                True)
                return False;
        ...
        if (True == lstCharMb && False == nxtCharMb &&
            _DtCvCheckOneByteCantBeginList((char)nextChar,
                                           OneByteCantBeginList) == True)
                return False;
 ...
 *
 *****************************************************************************/
int _DtCvGetNextWidth(_DtCanvasStruct *canvas, int old_type, int lst_hyper,
                      _DtCvSegmentI *pSeg, int start, _DtCvSegmentI *prev_seg,
                      _DtCvSegmentI **nextSeg, int *nextStart,
                      int *widthCount) {
        int result;
        int len = 0;
        int tLen;
         ...
         */
                ...
                if (0 == wcFlag) {
                        ...
                        do {
                                ...
                                good_len = True;
                                result =
                                    _DtCvStrcspn((void *)tChar, " -", 0, &tLen);
                                len += tLen;

                                /*
                                 * check for '-'. Some of the possible
                                 * combinations are: -text
                                 *    - text
                                 *    -/text/
                                 *    text/-text/
                                 *    text-text
                                 *    text text
                                 *
                                 * if it is the first character to check and
                                 * there is no previous segment, then it is
                                 * starting a line and can not be broken on.
                                 *
                                 * _DtCvStrcpn return 0 if one of the characters
                                 * in the test string was found.
                                 */
                                if (0 == result && '-' == tChar[tLen] &&
                                    0 == len && NULL == prev_seg &&
                                    _DtCvCheckLineSyntax(canvas, pSeg, start, 1,
                                                         True) == False) {
                                        len++;
                                        tLen++;
                                        tChar += tLen;
                         ...
                         */
                        if (result == 0 ||
                            _DtCvCheckLineSyntax(canvas, pSeg, start, len,
                                                 False) == True) {
                                if (nextSeg != NULL)
                                        *nextSeg = pSeg;
                                if (nextStart != NULL)
                 ...
                 */
                        ...
                        while (len < myLength) {
                                len++;
                                if (_DtCvCheckLineSyntax(canvas, pSeg, start,
                                                         len, False) == True) {
                                        pChar = _DtCvStrPtr(
                                            _DtCvStringOfStringSeg(pSeg),
                                            _DtCvIsSegWideChar(pSeg), start);
                                        curWidth =
                                            _DtCvGetStringWidth(canvas, pSeg,
                                                                pChar, len) +
                        ...
                        /*
                         * Didn't find a smaller segment that satisfied the
                         * requirements. Determine the length of the current
                         * segment.
                         */
                        curWidth =
                            _DtCvGetStringWidth(canvas, pSeg, pChar, len) +
        ...
        if (pSeg != NULL) {
                start = 0;
                curWidth += _DtCvGetNextWidth(
                    canvas, _DtCvPrimaryTypeOfSeg(prev_seg), lst_hyper, pSeg,
                    start, prev_seg, nextSeg, nextStart, &nextLen);
        }
 ...
 *****************************************************************************/
void _DtCvSaveInfo(_DtCanvasStruct *canvas, _DtCvLayoutInfo *layout,
                   _DtCvUnit max_width, _DtCvUnit r_margin,
        ...
        while (pSeg != NULL && count > 0) {
                ...
                if (_DtCvIsSegString(pSeg)) {
                        _DtCvFontMetrics(canvas, _DtCvFontOfStringSeg(pSeg),
                                         &fontAscent, &fontDescent, NULL, NULL,
                                         NULL);
                        ...
                        if (-1 == _DtCvAscentOfRegionSeg(pSeg)) {
                                if (maxRegion < _DtCvHeightOfRegionSeg(pSeg))
                                        maxRegion =
                                            _DtCvHeightOfRegionSeg(pSeg);
                        } else {
                                fontAscent = _DtCvAscentOfRegionSeg(pSeg);
                                fontDescent =
                                    _DtCvHeightOfRegionSeg(pSeg) - fontAscent;
                        }
                }
                ...
                if (_DtCvIsSegSuperScript(pSeg)) {
                        ...
                        if (_DtCvIsSegRegion(pSeg) &&
                            -1 == _DtCvAscentOfRegionSeg(pSeg) &&
                            maxRegion < _DtCvHeightOfRegionSeg(pSeg) + superY)
                                maxRegion =
                                    _DtCvHeightOfRegionSeg(pSeg) + superY;
                ...
                } else /* not a subscript or superscript */
                {
                        ...
                        if (_DtCvIsSegString(pSeg))
                                _DtCvFontMetrics(
                                    canvas, _DtCvFontOfStringSeg(pSeg), NULL,
                                    NULL, NULL, &superY, &subY);
                        else if (_DtCvIsSegRegion(pSeg)) {
        ...
        if (txt_justify == _DtCvJUSTIFY_RIGHT ||
            _DtCvJUSTIFY_CENTER == txt_justify) {
                /*
                 * justify the line.
                 */
                _DtCvUnit workWidth =
                    max_width - layout->text_x_pos - r_margin - layout->cur_len;
 ...
 *
 *****************************************************************************/
void _DtCvCheckAddHyperToTravList(_DtCanvasStruct *canvas, _DtCvSegmentI *p_seg,
                                  _DtCvValue flag, _DtCvValue *lst_vis,
                                  int *lst_hyper, _DtCvUnit *cur_len) {
        int nxtHyper;
        int prevIdx;
        _DtCvValue junk;
        ...
        if (_DtCvIsSegALink(p_seg)) {
                ...
                if (-1 == nxtHyper)
                        /*
                         * NOTE....should this routine return a value?
                         * If (re)alloc error occurs, this simply ignores the
                         * problem.
                         */
                        return;
                ...
                if (prevIdx < 0 ||
                    ...
                    p_seg->link_idx !=
                        canvas->trav_lst[prevIdx].seg_ptr->link_idx) {
                        /*
                         * save this hypertext link in the traversal list
                         */
                        _DtCvSetTravEntryInfo(canvas, nxtHyper,
                                              _DtCvTraversalLink, p_seg,
        ...
        *lst_vis = _DtCvModifyXpos(canvas->link_info, p_seg, junk, *lst_vis,
                                   *lst_hyper, &retLen);
        /*
        ...
        (void)_DtCvModifyXpos(canvas->traversal_info, p_seg, junk,
                              ((_DtCvValue)True), *lst_hyper, &retLen);
 ...
 *****************************************************************************/
int _DtCvProcessStringSegment(_DtCanvasStruct *canvas,
                              _DtCvLayoutInfo *lay_info, _DtCvUnit max_width,
                              _DtCvUnit l_margin, _DtCvUnit r_margin,
                              _DtCvSegmentI *cur_seg, unsigned int *cur_start,
                              _DtCvFrmtOption txt_justify,
        ...
        if (NULL != _DtCvStringOfStringSeg(cur_seg)) {
                ...
                if (TRUE == lay_info->align_flag) {
                        ...
                        if (0 != stringLen)
                                textWidth = _DtCvGetStringWidth(
                                    canvas, cur_seg, pChar, stringLen);
                        ...
                        _DtCvCheckAddHyperToTravList(
                            canvas, cur_seg, _DtCvTRUE, &(lay_info->lst_vis),
                            &(lay_info->lst_hyper), &(lay_info->cur_len));

                        /*
                         * update the length and position information
                         * to skip past the characters before the alignment
                         * character.
                         */
                        ...
                        *cur_start += stringLen;
                        ...
                        /*
                         * if we didn't find the character, check to see if this
                         * forces a newline - honor it if it does. We'll check
                         * the next string segment for the alignment character.
                         */
                        if (1 == nWidth && _DtCvIsSegNewLine(cur_seg) &&
                            lay_info->line_bytes) {
                                ...
                                while (lay_info->delayed_search_saves > 0) {
                                        _DtCvSetSearchEntryInfo(
                                            canvas, canvas->txt_cnt - 1);
                                        lay_info->delayed_search_saves--;
                                }
                                return 0;
                        }
                        ...
                        pChar = _DtCvStrPtr(_DtCvStringOfStringSeg(cur_seg),
                                            ...
                                            *cur_start);
                        textWidth =
                            _DtCvGetStringWidth(canvas, cur_seg, pChar, 1) +
                            _DtCvGetTraversalWidth(canvas, cur_seg,
                                                   lay_info->lst_hyper);
                        /*
                         * is this the second or more align position?
                         * if so, need to shift the character to align with
                         * others.
                         */
                        if (lay_info->align_pos > lay_info->text_x_pos +
                                                      lay_info->cur_len +
                                                      textWidth / 2)
                                lay_info->text_x_pos = lay_info->align_pos -
                                                       lay_info->cur_len -
                                                       textWidth / 2;
                        /*
                         * otherwise, does this exceed the previous alignments?
                         * if so, the table processing should catch that we've
                         * changed the alignment position and re-format the
                         * others.
                         */
                        else if (lay_info->align_pos < lay_info->text_x_pos +
                                                           lay_info->cur_len +
                                                           textWidth / 2)
                                lay_info->align_pos = lay_info->text_x_pos +
                                                      lay_info->cur_len +
                                                      textWidth / 2;
                         ...
                         * if can't end the line, force a join for the next
                         * segment or for the rest of this segment.
                         */
                        if (False == _DtCvCheckLineSyntax(canvas, cur_seg,
                                                          *cur_start, 1, False))
                                lay_info->join = True;
                ...
                while (1) {
                        ...
                        pChar = _DtCvStrPtr(_DtCvStringOfStringSeg(cur_seg),
                                            ...
                                            *cur_start);
                        stringLen =
                            _DtCvStrLen(pChar, _DtCvIsSegWideChar(cur_seg));

                        /*
                         * get the pixel width of the text string.
                         */
                        textWidth = _DtCvGetStringWidth(canvas, cur_seg, pChar,
                                                        stringLen) +
                                    _DtCvGetTraversalWidth(canvas, cur_seg,
                                                           lay_info->lst_hyper);
                        /*
                        ...
                        if (stat_flag == True || textWidth <= workWidth) {
                                /*
                                 * Yes, this segment or part of a segment can
                                 * fit in the current width. But can the last
                                 * character of this segment end a line and can
                                 * the beginning of the next segment start a new
                                 * line?
                                 */
                                if (stat_flag == True ||
                                    _DtCvCheckLineSyntax(canvas, cur_seg,
                                                         *cur_start, stringLen,
                                                         False) == TRUE) {
                                        /*
                                         * check to see if this a hypertext that
                                         * needs to be remembered.
                                         */
                                        _DtCvCheckAddHyperToTravList(
                                            canvas, cur_seg, _DtCvFALSE,
                                            &(lay_info->lst_vis),
                                            &(lay_info->lst_hyper),
                                            &(lay_info->cur_len));
                                        ...
                                        /*
                                         * Check to see if this segment forces
                                         * an end
                                         */
                                        if (_DtCvIsSegNewLine(cur_seg) &&
                                            lay_info->line_bytes) {
                                                _DtCvSaveInfo(
                                                    canvas, lay_info, max_width,
                                                    r_margin, txt_justify);
                                                while (
                                                    lay_info
                                                        ->delayed_search_saves >
                                                    0) {
                                                        _DtCvSetSearchEntryInfo(
                                                            canvas,
                                                            canvas->txt_cnt -
                                ...
                                /*
                                 * CheckLineSyntax says that either this line
                                 * couldn't end a line or the next segment
                                 * couldn't start a line. Therefore, find out
                                 * how much of the next segment or segments we
                                 * need to incorporate to satisfy the Line
                                 * Syntax rules.
                                 */
                                nWidth = _DtCvGetNextWidth(
                                    canvas, oldType, lay_info->lst_hyper,
                                    cur_seg->next_seg, 0, cur_seg, &retSeg,
                                    &retStart, &retCount);
                                /*
                        ...
                        while (!done) {
                                nWidth = _DtCvGetNextWidth(
                                    canvas, oldType, lay_info->lst_hyper,
                                    cur_seg, *cur_start, NULL, &retSeg,
                                    &retStart, &retCount);

                                if (retSeg == cur_seg &&
                                    textWidth + nWidth <= workWidth) {
                                        /*
                                         * check to see if this a hypertext that
                                         * needs to be remembered.
                                         */
                                        ...
                                        *cur_start = retStart;
                                        ...
                                        /*
                                         * take into account a space if that is
                                         * where it breaks.
                                         */
                                        pChar = _DtCvStrPtr(
                                            _DtCvStringOfStringSeg(cur_seg),
                                            _DtCvIsSegWideChar(cur_seg),
                                            *cur_start);
                                        if ((_DtCvIsSegWideChar(cur_seg) &&
                                             (' ' == *((wchar_t *)pChar))) ||
                                        ...
                                        /*
                                         * Done trying to find a segment that
                                         * will fit in the size given
                                         */
                                        done = True;
                                }
                        }
                        ...
                        if (lay_info->join == True ||
                            lay_info->line_bytes == 0) {
                                /*
                                 * This line would be empty if we followed the
                                 * rules. Or it would break a line improperly.
                                 * Force this onto the line.
                                 * check to see if this a hypertext that needs
                                 * to be remembered.
                                 */
                                _DtCvCheckAddHyperToTravList(
                                    canvas, cur_seg, _DtCvTRUE,
                                    &(lay_info->lst_vis),
                                    &(lay_info->lst_hyper),
                                    &(lay_info->cur_len));
                                ...
                                stringLen = _DtCvStrLen(
                                    pChar, _DtCvIsSegWideChar(cur_seg));
                                if (retCount > 0 && retCount < stringLen)
                                        stringLen = retCount;

                                lay_info->line_bytes += stringLen;
                                lay_info->cur_len +=
                                    (_DtCvGetStringWidth(canvas, cur_seg, pChar,
                                                         stringLen) +
                                     _DtCvGetTraversalWidth(
                                         canvas, cur_seg, lay_info->lst_hyper));

                                _DtCvSetJoinInfo(lay_info, False, -1);

                                /*
                                 * If we had to do a bigger segment,
                                 * then we're done processing the target
                                 * segment.
                                 */
                                if (stringLen ==
                                    _DtCvStrLen(pChar,
                                                _DtCvIsSegWideChar(cur_seg))) {
                                        if (_DtCvCheckLineSyntax(
                                                canvas, cur_seg, *cur_start,
                                                stringLen, False) == False)
                                                _DtCvSetJoinInfo(lay_info, True,
                                                                 -1);
                                ...
                                *cur_start = retStart;
                                ...
                                /*
                                 * If a space was included as the last
                                 * character, remove it now.
                                 */
                                lay_info->line_bytes--;
                         ...
                         */
                        _DtCvSaveInfo(canvas, lay_info, max_width, r_margin,
                                      txt_justify);
                        if (*cur_start == 0 &&
                            (cur_seg->type & _DtCvSEARCH_FLAG))
                                lay_info->delayed_search_saves--;
 ...
 *
 *****************************************************************************/
void _DtCvSetJoinInfo(_DtCvLayoutInfo *lay_info, _DtCvValue flag, int txt_ln) {
        lay_info->join = flag;
 ...
 *
 *****************************************************************************/
int _DtCvGetNextTravEntry(_DtCanvasStruct *canvas) {
 ...
 *              list.
 *****************************************************************************/
int _DtCvSetTravEntryInfo(_DtCanvasStruct *canvas, int entry,
                          _DtCvTraversalType type, _DtCvSegmentI *p_seg,
                          int line_idx, _DtCvValue inc) {
        int result = -1;
...
int _DtCvGetNextSearchEntry(_DtCanvasStruct *canvas) {
        if (canvas->search_cnt >= canvas->search_max) {
                canvas->search_max += GROW_SIZE;

                if (canvas->searchs)
                        canvas->searchs = (_DtCvSearchData *)realloc(
                            (void *)canvas->searchs,
                            canvas->search_max * sizeof(_DtCvSearchData));
                else
                        canvas->searchs = (_DtCvSearchData *)malloc(
                            canvas->search_max * sizeof(_DtCvSearchData));
        }
...
void _DtCvSetSearchEntryInfo(_DtCanvasStruct *canvas, int line_idx) {
        int search_idx;
 ...
 *
 *****************************************************************************/
int _DtCvSetTravEntryPos(_DtCanvasStruct *canvas, int entry, _DtCvUnit x,
 ...
 *
 *****************************************************************************/
int _DtCvCalcMarkPos(_DtCanvasStruct *canvas, int entry, _DtCvUnit *ret_x,
                     _DtCvUnit *ret_y, _DtCvUnit *ret_width,
        ...
        if (-1 != entry && entry <= canvas->mark_cnt) {
                ...
                if (-1 != mark->beg.line_idx && -1 != mark->end.line_idx) {
                        _DtCvDspLine *line =
                            &(canvas->txt_lst[mark->beg.line_idx]);
                        ...
                        *ret_y = mark->beg.y - line->ascent;
                        ...
                        if (mark->beg.line_idx == mark->end.line_idx)
                                ...
                                *ret_width =
                                    canvas->txt_lst[mark->beg.line_idx].max_x -
                                    *ret_x;
 ...
 *
 *****************************************************************************/
void _DtCvSortTraversalList(_DtCanvasStruct *canvas, _DtCvValue retain) {
        ...
        if (1 < canvas->trav_cnt) {
                 ...
                 * sort the items.
                 */
                qsort(canvas->trav_lst, canvas->trav_cnt,
                      sizeof(_DtCvTraversalInfo), CompareTraversalPos);
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCvCvtSegsToPts(_DtCanvasStruct *canvas, _DtCvSegPtsI **segs,
                              _DtCvSelectData *beg, _DtCvSelectData *end,
                              _DtCvUnit *ret_y1, _DtCvUnit *ret_y2,
        ...
        *end = DefSelectData;
        ...
        while (NULL != *segs) {
                ...
                /*
                 * now skip the segments on this line that aren't in the data
                 * pt.
                 */
                while (NULL != pSeg && pSeg != (*segs)->segment) {
                        ...
                        /*
                         * we know that this is not the segment we are looking
                         * for, so go past it.
                         */
                        ...
                        /*
                         * skip the segment's width, decrease the overall length
                         * by the segment's count, reset the character start
                         * point and go to the next segment.
                         */
                        startX += segWidth;
                        length -= cnt;
                        start = 0;
                 ...
                 */
                startX = _DtCvAdvanceXOfLine(canvas, pSeg, startX, &linkIdx,
                                             &lastVisLnk);
                /*
                ...
                segWidth = 0;
                if (0 < count)
                        _DtCvGetWidthOfSegment(canvas, pSeg, start, count, &cnt,
                                               &segWidth, NULL);
                /*
                ...
                while (count > length) {
                        ...
                        do {
                                 ...
                                 */
                                if (minY == -1 ||
                                    minY > lines[lineIdx].baseline -
                                               lines[lineIdx].ascent)
                                        minY = lines[lineIdx].baseline -
                                               lines[lineIdx].ascent;
                ...
                while (0 < count) {
                        /*
                         * findout how many characters are in the next segment,
                         * and its width.
                         */
                        _DtCvGetWidthOfSegment(canvas, pSeg, start, count, &cnt,
                                               &segWidth, NULL);
                        ...
                        /*
                         * there are less than in the count, go to the next
                         * segment.
                         */
                        if (cnt < count) {
                                pSeg = pSeg->next_disp;
        ...
        if (-1 != bReg.x) {
                 ...
                 */
                         ...
                         (bReg.line_idx == beg->line_idx ||
                          /*
                           * Or if the region is 'standalone' (a bullet of a
                           * list, a graphic to wrap around, etc.)  then check
                           * to see if it straddles the other information and is
                           * before the text.  If it does, take it's x value.
                           */
                          _DtCvStraddlesPt(
                              beg->y, bReg.y - lines[bReg.line_idx].ascent,
        ...
        if (-1 != eReg.x) {
                 ...
                 */
                         ...
                         (eReg.line_idx == end->line_idx ||
                           ...
                           */
                          _DtCvStraddlesPt(
                              end->y, eReg.y - lines[eReg.line_idx].ascent,
 ...
 *
 *****************************************************************************/
int _DtCvAddToMarkList(_DtCanvasStruct *canvas, _DtCvPointer client_data,
                       _DtCvValue flag, _DtCvSelectData *beg,
                       _DtCvSelectData *end) {
        ...
        if (canvas->mark_cnt >= canvas->mark_max) {
                ...
                if (NULL == canvas->marks)
                        ...
                        canvas->marks = (_DtCvMarkData *)realloc(
                            (void *)canvas->marks,
                            sizeof(_DtCvMarkData) * canvas->mark_max);
