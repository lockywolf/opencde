...
/*********************/
static void _gif_error(char *format, ...) {
        va_list args;
...
/*********************/
static void _gif_message(char *format, ...) {
        va_list args;
...
/*********************/
static pixel **_gif_allocarray(int cols, int rows, int size) {
        char **its;
        int i;
...
/*********************/
static int GetDataBlock(byte **inbuf, unsigned char *buf, GifState *g) {
        unsigned char count;
...
/*********************/
static int ReadColorMap(byte **inbuf, int number,
...
/*********************/
static int DoExtension(byte **inbuf, int label, GifState *g) {
        char *str;
        char *buf = g->ext_buf;
...
/*********************/
static int GetCode(byte **inbuf, int code_size, int flag, GifState *g) {
        int i, j, ret;
        unsigned char count;
...
/*********************/
static int LWZReadByte(byte **inbuf, int flag, int input_code_size,
                       GifState *g) {
        int code, incode;
        register int i;
...
#ifdef DEBUG
                        if (count != 0)
                                _gif_message("missing EOD in data stream "
                                             "(common occurence)");
#endif
                        return -2;
                }
...
/*********************/
pixel **ReadImage(byte **inbuf, int len, int height,
                  unsigned char cmap[3][MAXCOLORMAPSIZE], int interlace,
                  int ignore, GifState *g) {
        unsigned char c;
        int v;
...
#ifdef DEBUG
        _gif_message("reading %d by %d%s GIF image", len, height,
...
enum _DtGrLoadStatus InitGifObject(GifObj *go, Display *dpy, Drawable drawable,
                                   Screen *screen, int depth, Colormap colormap,
                                   Visual *visual, GC gc,
        ...
        */
        ...
        visualList =
            XGetVisualInfo(go->f_dpy, VisualScreenMask | VisualDepthMask,
        ...
        for (r = 0; r < go->colors_per_pixel; r++)
                for (g = 0; g < go->colors_per_pixel; g++)
                        for (b = 0; b < go->colors_per_pixel; b++) {
                                go->GifCMap[i].red =
                                    ((r * 65535) / (go->colors_per_pixel - 1));
                                go->GifCMap[i].green =
                                    ((g * 65535) / (go->colors_per_pixel - 1));
                                go->GifCMap[i].blue =
                                    ((b * 65535) / (go->colors_per_pixel - 1));
                                i++;
                        }
...
int allocate_colors(GifObj *g) {
        ...
        for (i = 0; i < g->total_colors; i++) {
                ...
                if (!XAllocColor(g->f_dpy, g->f_cmap, &color)) {
                        ...
                        /*puts ("FAILED!!!"); */
                        colors =
                            (unsigned long *)malloc(sizeof(unsigned long) * i);
                        for (j = 0; j < i; j++)
...
int allocate_greys(GifObj *g) {
        ...
        for (i = 0; i < g->total_colors; i++) {
                ...
                debug1(cerr, 0.299 * GifCMap[i].red + 0.587 * GifCMap[i].green +
                                                         0.114 *
                GifCMap[i].blue); debug1(cerr, GifCMap[i].grey);
                */

                g->GifCMap[i].grey =
                    (unsigned short)(0.299 * g->GifCMap[i].red +
                ...
                */
                ...
                if (i == g->f_total_greys) {
                        ...
                        */
                        ...
                        g->f_color_map_constructed = 1;
                        g->f_do_visual = DO_GREY;
                        /* If greyscape was downgraded to bitonal, record the
                         * fact */
                        if ((g->f_total_greys == 2) &&
                            (g->f_init_total_greys > 2))
                                g->f_color_reduction_used = TRUE;
                        return 0;
                }
        }
...
void free_raw_image(pixel **image) {
        free((char *)image[0]);
        free((char *)image);
}
...
pixel **create_raw_image(byte *inbuf, unsigned int buflen, int *width,
                         int *height, int imageNumber) {
        unsigned char *buf;
        ...
        if (BitSet(buf[4], LOCALCOLORMAP)) { /* Global Colormap */
                if (ReadColorMap(&inbuf, g.GifScreen.BitPixel,
                                 g.GifScreen.ColorMap)) {
...
#ifdef DEBUG
                _gif_message("warning - non-square pixels; to fix do a "
                             "'pnmscale -%cscale %g'",
        ...
        for (;;) {
...
#ifdef DEBUG
                        _gif_message("bogus character 0x%02x, ignoring",
                                     (int)c);
#endif
                        continue;
                }
                ...
                *height = LM_to_uint(buf[6], buf[7]);
                ...
                if (!useGlobalColormap) {
                        ...
                        image =
                            ReadImage(&inbuf, *width, *height, localColorMap,
                                      BitSet(buf[8], INTERLACE),
                                      imageCount != imageNumber, &g);
                } else {
                        image = ReadImage(&inbuf, *width, *height,
                                          g.GifScreen.ColorMap,
                                          BitSet(buf[8], INTERLACE),
                                          imageCount != imageNumber, &g);
                }
...
static int dither_8X8[8][8] = {
    ...
    {42, 44, 4, 36, 14, 46, 6, 38}, {60, 28, 52, 20, 62, 30, 54, 22},
...
Pixmap create_pixmap(GifObj *g, pixel **image, int width, int height, Pixel fg,
                     Pixel bg, float ratio) {
        ...
        if (g->f_nplanes == 1)
                g->f_ximage =
                    XCreateImage(g->f_dpy, g->f_visual, g->f_nplanes, XYPixmap,
        ...
        /* non-mono display */
                ...
                g->f_ximage =
                    XCreateImage(g->f_dpy, g->f_visual, g->f_nplanes, ZPixmap,
                                 0, (char *)ximData, width, height, 32, 0);
        ...
        for (y = 0; y < height; y++) {
                for (x = 0; x < width; x++) {
                         ...
                         * g->GifCMap[index].grey, g->GifCMap[index].grey /
                         * spacing);*/
                        ...
                        case DO_GREY:
                                ...
                                case 2:
                                        ...
                                        /*cerr << "index=" << index << "\n"; */
                                        /*cerr << "GifCMap[index].grey" <<
                                         * GifCMap[index].grey << "\n"; */
                                        /*cerr << "GifCMap[index].grey/spacing"
                                         * << GifCMap[index].grey / spacing <<
                                         * "\n"; */

                                        if (dither_8X8[x % dither_matrix_sz]
                                                      [y % dither_matrix_sz] <
                                            g->GifCMap[index].grey / spacing) {
                                                XPutPixel(g->f_ximage, x, y,
                                                          g->f_white);
                                        } else {
                                                XPutPixel(g->f_ximage, x, y,
                                                          g->f_black);
                                ...
                                default:
                                        ...
                                        XPutPixel(
                                            g->f_ximage, x, y,
                                            g->GifGMap[g->GifCMap[index].grey /
                                                       spacing]);
                                }
                        ...
                        case DO_COLOR:
                                ...
                                fprintf(stderr,
                                        ...
                                        (short)PPM_GETR(pixval),
                                        (short)PPM_GETG(pixval),
                                        (short)PPM_GETB(pixval),
                                        g->GifCMap[index].red >> 8,
                                        g->GifCMap[index].green >> 8,
                                        g->GifCMap[index].blue >> 8);
                                if ((short)PPM_GETR(pixval) !=
                                        (GifCMap[index].red >> 8) ||
                                    (short)PPM_GETG(pixval) !=
                                        (GifCMap[index].green >> 8) ||
                                    (short)PPM_GETB(pixval) !=
                                        (GifCMap[index].blue >> 8))
                                        puts(" *");
                                else
                                        puts(" -");
#endif
                                ...
                                XPutPixel(g->f_ximage, x, y,
                                          g->GifCMap[index].pixel);
                                /*
                                         switch ( g->f_visual_class ) {
                                          case StaticColor:
                                          case PseudoColor:
                                             XPutPixel(g->f_ximage,x,y,
                                   g->GifCMap[index].pixel); break;
        ...
        if (scaledHeight == 0)
                scaledHeight = 1;
        pm = XCreatePixmap(g->f_dpy, g->f_drawable, scaledWidth, scaledHeight,
                           g->f_nplanes);
        ...
        _XmPutScaledImage(g->f_dpy, pm, g->f_gc, g->f_ximage, 0, 0, 0, 0, width,
...
Pixmap gif_to_pixmap(GifObj *g, byte *inbuf, unsigned int buflen, Dimension *w,
                     Dimension *h, Pixel fg, Pixel bg, float ratio) {
        Pixmap pixmap;
        pixel **raw_image;
        int width, height;
