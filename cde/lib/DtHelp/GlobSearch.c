...
static Boolean VolumeHasIndexP(_DtHelpGlobSrchSources srchSource, int helpType,
                               char *baseName);
static void DeleteListContents(_DtHelpGlobSearchStuff *srch);
static void StatusLabelUpdate(DtHelpDialogWidget hw, ResultsStatus status,
                              Boolean forceUpdate, int intArg);
static void ResultsListUpdate(DtHelpDialogWidget hw, _DtHelpFileEntry newFile);
static void StopSearchCB(Widget w, XtPointer clientData, XtPointer callData);
static void VolNameDisplay(DtHelpDialogWidget hw, _DtHelpFileEntry file,
                           Boolean insertVol);
static int VolHitsDisplay(DtHelpDialogWidget hw, _DtHelpFileEntry file);
static void UpdateSearchVolumesCB(Widget widget, XtPointer clientData,
                                  XtPointer callData);
static void UpdateSearchStartStatusCB(Widget w, XtPointer clientData,
                                      XtPointer callData);
...
#define PREFIX_FONT_SPEC                                                       \
        "-dt-interface user-bold-r-normal-m*-*-*-*-*-m-*-iso8859-1"
#define PREFIX_FONT_SPEC1                                                      \
        "-dt-application-bold-r-normal-*-*-140-*-*-m-*-iso8859-1"
#define PREFIX_FONT_SPEC2                                                      \
        "-dt-interface user-bold-r-normal-m*-*-*-*-*-m-*-iso8859-1"
...
static char *s_GlobSrchDlgBtnStrs[] = {
    "Start Search", "Continue Search", "Stop Search", "Close", "Help", NULL};
/* catalog index numbers for the strings */
static unsigned char s_GlobSrchDlgBtnCatNum[] = {12, 13, 14, 15, 16};
...
/* static char defaultMgrTranslations[] = "<Key>Return:
 ...
 *
 *****************************************************************************/
static void MergeFontListIntoWidgetFonts(Widget widget,
        ...
        for (entry = XmFontListNextEntry(context); NULL != entry;
 ...
 *
 *****************************************************************************/
static Boolean UpdateCurVolBtnSens(DtHelpDialogWidget hw,
     ...
     if (selectVolBtn) UpdateSearchVolumesCB(sourceBtn,hw,&status);
        ...
        hw->help_dialog.srch.curVolRadBtnSens = curState;
        if (selectVolBtn)
                UpdateSearchVolumesCB(sourceBtn, (XtPointer)hw,
                                      (XtPointer)&status);
#endif
 ...
 *
 ************************************************************************/
static void LoadPrefixFont(DtHelpDialogWidget hw) {
        XmFontListEntry entry;
        ...
        if (hw->help_dialog.srch.hitsFontLoaded)
                return; /* RETURN */
 ...
 ************************************************************************/
static Boolean
GetVolumeInfoCB(XtPointer pDisplayArea, /* in: display area in use */
                ...
                Boolean *ret_mod)        /* out: has font list been changed */
{
        ...
        /* open volume */
        if (_DtHelpOpenVolume(volumePath, &vol) !=
            0) { /* if can't open, do best possible */
                if (ret_titleXmStr)
                        *ret_titleXmStr = XmStringCreateLocalized(baseName);
                if (ret_title)
                        *ret_title = XtNewString(baseName);
                if (ret_nameKey)
        ...
        if (ret_titleXmStr) {
                ...
                /* locTitle is owned by caller */
                _DtHelpFormatVolumeTitle(pDisplayArea, vol, &locTitleXmStr,
                                         io_fontList, ret_mod);
        ...
        if (ret_docId || ret_timeStamp) {
                ...
                /* locDocId & locTimeStamp will point to private memory; do not
                 * modify */
                ...
                if (ret_timeStamp)
                        *ret_timeStamp =
                            locTimeStamp ? locTimeStamp : XtNewString(&empty);
        }
 ...
 *
 ************************************************************************/
static void AdjustPositionValues(_DtHelpFileList fileList, int adjStartAmount,
                                 int adjNextAmount, int adjustThisFile) {
        ...
        if (False == adjustThisFile) {
                ...
                for (fileList = _DtHelpFileListGetNext(
                         NULL, fileList); /* begin with next */
                     NULL != fileList;
                     fileList = _DtHelpFileListGetNext(NULL, fileList)) {
                        register _DtHelpGlobSrchVol *vol =
                            (_DtHelpGlobSrchVol *)fileList->clientData;
                        if (vol->showVolInList &&
                            (vol->hitCnt > 0 || vol->zeroHitsOk))
                                break; /* BREAK */
 ...
 ************************************************************************/
static int HitLoadTopics(DtHelpDialogWidget hw, _DtHelpFileEntry file,
        ...
        topicCnt =
            _DtHelpCeFindKeyword(volHandle, hit->indexEntry, &origTopicIdList);
        if (topicCnt <= 0)
                return -1; /* RETURN: error */
        ...
        for (i = 0; i < topicCnt; i++) {
                 ...
                 * once already.  Save a pointer to it so we can free the font
                 * list if _DtHelpFormatTopicTitle copies it again.
                 */
                lastFontList = fontList;

                valid = _DtHelpFormatTopicTitle(
                    hw->help_dialog.help.pDisplayArea, volHandle,
                    origTopicIdList[i], &titleStr, &fontList, &mod);

                if (mod && NULL != lastFontList)
                        XmFontListFree(lastFontList);
                lastFontList = NULL;
                ...
                idList = (char **)_DtHelpCeAddPtrToArray(
                    (void **)idList, (void *)XtNewString(origTopicIdList[i]));
                fileList = (char **)_DtHelpCeAddPtrToArray(
                    (void **)fileList, (void *)XtNewString(file->fullFilePath));
 ...
 ************************************************************************/
static int HitNameDisplay(DtHelpDialogWidget hw, _DtHelpFileEntry file,
                          _DtHelpGlobSrchHit *hit, int hitPos,
                          Boolean insertHit) {
        char *expandPrefix;
        char *contractPrefix;
        char *gotoPrefix;
        ...
        if (hit->topicCnt > 1) {
                if (hit->topicsDisplayed) /* topics currently displayed */
                        ...
                        tmpStr = expandPrefix;
                snprintf(buf, 30, "%s%3d  ", tmpStr,
                         hit->topicCnt); /* 4d too spacy */
                tmpStr = buf;
        }
        LoadPrefixFont(hw);
        prefixString =
            XmStringGenerate(tmpStr, s_PrefixFontListTag, XmCHARSET_TEXT, NULL);
 ...
 ************************************************************************/
static int HitTopicsDisplay(DtHelpDialogWidget hw, _DtHelpFileEntry file,
                            _DtHelpGlobSrchHit *hit,
        ...
        if (hit->topicCnt > 0) {
                ...
                for (i = 0, pTopicString = hit->topicTitles;
                     NULL != *pTopicString; pTopicString++, i++)
 ...
 ************************************************************************/
static int HitTopicsUndisplay(DtHelpDialogWidget hw, _DtHelpFileEntry file,
                              _DtHelpGlobSrchHit *hit,
                              int firstTopicListPosition) {
        int topPos;
        Arg args[5];
 ...
 ************************************************************************/
static _DtHelpGlobSrchHit *HitFree(_DtHelpGlobSrchHit *hit,
        ...
        for (nextStr = hit->topicTitles; NULL != nextStr && NULL != *nextStr;
             nextStr++)
 ...
 ************************************************************************/
static void HitListFree(_DtHelpGlobSrchVol *vol, Boolean freeHitsThemselves) {
        register _DtHelpGlobSrchHit *hit;

        if (NULL == vol)
                return; /* RETURN */

        for (hit = vol->hitListHead; NULL != hit;
             hit = HitFree(hit, freeHitsThemselves)) { /* nothing */
        }
 ...
 ************************************************************************/
static int HitListGetNth(_DtHelpFileList volListHead, int position,
                         _DtHelpFileEntry *ret_file,
                         _DtHelpGlobSrchHit **ret_hit, char **ret_locationId,
                         char **ret_helpFile) {
        ...
        for (curFile = volListHead; NULL != curFile;
             curFile = _DtHelpFileListGetNext(curFile, curFile)) {
                /* it is impt to use > test so that multiple volumes with the
                   same start position are properly handled.  This occurs when
                   they have no hits in them. */
     ...
     if (curVol->hitCnt > 0) posFile = curFile;      /* recent with hits */
#else
                if (curVol->showVolInList &&
                    curVol->nextVolPosition > position) {
                        posFile = curFile;
                        break; /* BREAK */
                }
#endif
        ...
        if (curPos < position) { /* a hit inside the volume */
                if (curVol->hitsDisplayed) {
                        for (curPos++, hit = curVol->hitListHead;
                             ...
                             curPos++, hit = hit->next) {
                                ...
                                /* walk all displayed topics to see if the
                                 * position is here */
                                for (locationIdList = hit->topicIdList,
                                    fileNameList = hit->topicFileList;
                                     NULL != locationIdList[0];
                                     locationIdList++, fileNameList++) {
                                        /* use GOTO to escape with all pointers
                                           correct and without introducing an
                                           additional flag variable */
                                        if (++curPos == position)
                                                goto done; /* GOTO */
                                } /* for all locations of a hit before the
                                     position */
                        } /* for all members of the hit list before the position
                           */
                }         /* if hits are currently shown */
                else
                        posFile = NULL; /* an error occurred */
        ...
        if (ret_locationId) {
                if (locationIdList)
                        *ret_locationId = locationIdList[0];
                else
                        *ret_locationId = NULL;
        }
        if (ret_helpFile) {
                if (fileNameList)
                        *ret_helpFile = fileNameList[0];
                else
                        *ret_helpFile = NULL;
        }
...
#ifdef __LP64__
        return (0 ==
                ((int64_t)hit | (int64_t)posFile | (int64_t)locationIdList))
                   ? -1
                   : 0;
#else
 ...
 ************************************************************************/
static int HitListAddFound(_DtHelpFileEntry curFile, XmString indexTitle,
                           char *indexEntry, Boolean insertSorted,
        ...
        {
                /* walk along list */
                for (next = vol->hitListHead; NULL != next;
                     prev = next, next = next->next) {
                        register int ret;

                        /* do a NLS case insensitive compare using NLS collating
                         */
                        if ((ret = (*strcollfn)(next->indexEntry,
                                                indexEntry)) >= 0) {
                                if (0 == ret) {
                                        if (ret_hit)
                                                *ret_hit = next;
                ...
                for (next = vol->hitListHead; NULL != next;
                     prev = next, next = next->next) {
                        if (newKey == next->indexKey /* quick compare */
                            && strcmp(indexEntry, next->indexEntry) ==
                                   0) /* long compare */
                        {
                                if (ret_hit)
                                        *ret_hit = next;
        ...
        if (prev)
                ...
                vol->hitListHead = srcHit;
        if (!next)
                vol->hitListTail = srcHit;
...
static _DtHelpFileEntry GetNextSearchFileAndDisplayCompleted(
    DtHelpDialogWidget hw, _DtHelpFileList listHead, _DtHelpFileEntry curFile) {
        /* get first file needing work */
        for (curFile = _DtHelpFileListGetNext(listHead, curFile);
             NULL != curFile; curFile = _DtHelpFileListGetNext(NULL, curFile)) {
                register _DtHelpGlobSrchVol *vol;
                ...
                if (vol->searchThisVolume && vol->searchCompleted &&
                    ...
                    (vol->hitCnt > 0 || vol->zeroHitsOk)) {
                        ...
                        if (vol->showHitsWithVol ||
                            hw->help_dialog.srch.srchSources ==
                                _DtHelpGlobSrchCurVolume) {
                                VolHitsDisplay(hw, curFile);

                                /* update the volume label to show state;
                                 * False--dont insert */
                ...
                if (vol->searchThisVolume && False == vol->searchCompleted &&
                    vol->showVolInList) /* don' search unless results will be
                                           shown */
                        break;
        }
 ...
 *
 ************************************************************************/
static void AddVolInfoToFile(_DtHelpFileEntry file, Boolean initialSearchStatus,
                             Boolean displayStatus, Boolean selectedFilesOnly) {
        _DtHelpGlobSrchVol *vol;
        ...
        if (NULL == vol) {
                vol = (_DtHelpGlobSrchVol *)XtCalloc(
                    1, sizeof(_DtHelpGlobSrchVol));
                file->clientData = (XtPointer)vol;
                if (NULL == vol)
                        return; /* RETURN:  memory alloc error */
 ...
 *
 ************************************************************************/
static void AddVolInfoToList(_DtHelpFileList listHead,
                             Boolean initialSearchStatus, Boolean displayStatus,
                             Boolean selectedFilesOnly) {
        /* walk the files and allocate the vol info as needed */
        for (/*nothing*/; NULL != listHead;
             listHead = _DtHelpFileListGetNext(listHead, listHead)) {
                AddVolInfoToFile(listHead, initialSearchStatus, displayStatus,
                                 selectedFilesOnly);
        }
}
 ...
 *
 ************************************************************************/
static void SetVolStatus(_DtHelpFileList listHead, Boolean searchThisVolume,
                         Boolean showVolInList, Boolean zeroHitsOk,
                         Boolean selectedFilesOnly) {
        /* walk the files and set the status */
        for (/*nothing*/; NULL != listHead;
             listHead = _DtHelpFileListGetNext(listHead, listHead)) {
                _DtHelpGlobSrchVol *vol =
                    (_DtHelpGlobSrchVol *)listHead->clientData;

                if (NULL == vol)
                        continue;

                if ((False == selectedFilesOnly) ||
                    (True == listHead->fileSelected &&
                     True == selectedFilesOnly)) {
...
static void VolNameDisplay(DtHelpDialogWidget hw, _DtHelpFileEntry file,
        ...
        if (vol->hitCnt <= 0)
                tmpStr = (char *)_DTGETMESSAGE(GSSET, 25, " ");
        else if (vol->hitsDisplayed)
                tmpStr = (char *)_DTGETMESSAGE(GSSET, 27, "-");
        else
                tmpStr = (char *)_DTGETMESSAGE(GSSET, 26, "+"); /* >0 hits */
        snprintf(buf, 40, "%s%3d ", tmpStr, vol->hitCnt); /* 4d too spacy */

        LoadPrefixFont(hw);
        prefixString =
            XmStringGenerate(buf, s_PrefixFontListTag, XmCHARSET_TEXT, NULL);
        ...
        for (hit = vol->hitListHead, itemCnt = 0; NULL != hit;
             hit = hit->next) {
                ...
                if (hit->showTopicsWithHit) {
                        int newItems;
                        newItems = HitTopicsDisplay(hw, file, hit,
                                                    listPos + itemCnt + 1);
 ...
 ************************************************************************/
static int VolHitsUndisplay(DtHelpDialogWidget hw, _DtHelpFileEntry file) {
        _DtHelpGlobSrchVol *vol = (_DtHelpGlobSrchVol *)file->clientData;
        _DtHelpGlobSrchHit *hit;
        ...
        if (nonVisibleItems) {           /* delete all items with a position >
                                            topNonVisPos */
                ...
                for (hit = vol->hitListHead, itemCnt = 0; NULL != hit;
                     hit = hit->next) {
                        ...
                        if (curDelPos < topNonVisPos) {
                                ...
                                if (hit->topicsDisplayed)
                                        curDelPos += hit->topicCnt;
                                continue; /* CONTINUE */
                        }
                        /* we're in non-visible region--undisplay hit */
                        XmListDeletePos(hw->help_dialog.srch.resultList,
                                        curDelPos);
                        /* undisplay any topics */
                        if (hit->topicsDisplayed)
                                itemCnt += HitTopicsUndisplay(hw, file, hit,
                                                              curDelPos);
                }
 ...
 *
 ************************************************************************/
static void VolListFree(_DtHelpGlobSearchStuff *srch) {
        ...
        while (NULL != nextFile) {
                ...
                /* no need to free indexEntriesList because these aren't owned
                 * by vol */
                ...
                for (nextStr = vol->indexXmStrsList;
                     NULL != nextStr && NULL != *nextStr; nextStr++)
                        XmStringFree(*nextStr);
                ...
                _DtHelpFileFreeEntry(
                    tmpFile); /* also frees the clientData (vol) */
        }                     /* walk all volumes */
 ...
 *
 ************************************************************************/
static void ScanStatusCB(int count, XtPointer clientData) {
 ...
 *
 ************************************************************************/
static void VolListBuild(DtHelpDialogWidget hw, Boolean searchStatus,
                         Boolean displayStatus, Boolean selectedVolumesOnly) {
        Boolean mod;
        ...
        _DtHelpFileListScanPaths(
            &hw->help_dialog.srch.volListHead,
            &hw->help_dialog.srch.volTitlesFontList, &mod, DtHelpVOLUME_TYPE,
            _DtHelpFileSuffixList, False, GetVolumeInfoCB,
            hw->help_dialog.help.pDisplayArea,
            /* sysPathCompare */ _DtHELP_FILE_NAME,
            /*otherPathCompare*/ _DtHELP_FILE_NAME | _DtHELP_FILE_TIME |
                _DtHELP_FILE_IDSTR,
        ...
        if (mod) {
                ...
                if (origFontList)
                        XmFontListFree(origFontList);
        }

        /* Add on the Vol info and (T,F) allow search on all volumes */
        AddVolInfoToList(hw->help_dialog.srch.volListHead, searchStatus,
                         displayStatus, selectedVolumesOnly);
 ...
 *
 ************************************************************************/
static Boolean SearchContinuePossible(DtHelpDialogWidget hw, char *srchWord) {
        char *normSrchWord;
 ...
 *
 ************************************************************************/
static Boolean SearchOnSameCriteria(DtHelpDialogWidget hw, char *srchWord) {
        char *normSrchWord;
        ...
        /* comparison with srchNormWordStr is correct, given calls to this fn */
        if (hw->help_dialog.srch.fullIndex ||
            (NULL != hw->help_dialog.srch.normWordStr && NULL != normSrchWord &&
             strcmp(normSrchWord, hw->help_dialog.srch.normWordStr) == 0))
                sameCrit = True;
 ...
 *
 ************************************************************************/
static void UpdateActionButtonLabel(DtHelpDialogWidget hw, char *srchWord,
        ...
        _DtHelpProcessLock();
        if (startContAction) {
                if (SearchContinuePossible(
                        hw, srchWord)) { /* then continue search */
                        textString = (char *)_DTGETMESSAGE(
                            GSSET, CONT_SEARCH_CAT, CONT_SEARCH_STR);
                        labelString = XmStringCreateLocalized(textString);
                        /*mnemonic = ((char *)_DTGETMESSAGE(GSSET,
                         * 22,CONT_SEARCH_MNEM));*/
                        ...
                        labelString = XmStringCreateLocalized(textString);
                        /*mnemonic = ((char *)_DTGETMESSAGE(GSSET,
                         * 20,START_SEARCH_MNEM));*/
                }
        } else /* action label is to stop */
        {
                textString = (char *)_DTGETMESSAGE(GSSET, STOP_SEARCH_CAT,
                                                   STOP_SEARCH_STR);
                labelString = XmStringCreateLocalized(textString);
                /*mnemonic = ((char *)_DTGETMESSAGE(GSSET,
                 * ??,STOP_SEARCH_MNEM));*/
 ...
 *
 *****************************************************************************/
static Boolean CheckSearchWord(DtHelpDialogWidget hw, char *srchWord,
 ...
 *
 *****************************************************************************/
static void PrepSearchSourceData(DtHelpDialogWidget hw, char *srchWord) {
        ...
        if (hw->help_dialog.srch.fullIndex ||
            CheckSearchWord(hw, srchWord, True) == False) {
                ...
                /* else we're searching on the same word, but completed
                   ...
                   according to the current settings of the Volumes radio
                   buttons, but leave the results intact.
                */
        ...
        if (_DtHelpGlobSrchSelectedVolumes ==
            hw->help_dialog.srch.srchSources) {
                /* volume list already built when opening selection dialog */
                /* (F,F,F,F): disable search,disable display,no 0 hits,for all
                 * vols */
                SetVolStatus(hw->help_dialog.srch.volListHead, False, False,
                             False, False);
                /* (T,T,T,T): enable search, enable display,
                       zero hits ok, only for selected volumes */
                SetVolStatus(hw->help_dialog.srch.volListHead, True, True, True,
                             True);
        } /* if selected volumes button */
        else if (_DtHelpGlobSrchAllVolumes ==
                 hw->help_dialog.srch.srchSources) {
                ...
                /* (T,T,F,F): enable search,enable display,no 0 hits,for all
                 * vols */
                SetVolStatus(hw->help_dialog.srch.volListHead, True, True,
                             False, False);
        ...
        else /*** current volume button ***/
        {
                ...
                path = _DtHelpFileLocate(DtHelpVOLUME_TYPE,
                                         hw->help_dialog.display.helpVolume,
                                         _DtHelpFileSuffixList, False, R_OK);
                cur = _DtHelpFileListGetMatch(
                    hw->help_dialog.srch.volListHead, path, GetVolumeInfoCB,
                    _DtHELP_FILE_NAME, hw->help_dialog.help.pDisplayArea);

                if (NULL == cur &&
                    NULL != path) /*file was located but isn't in list*/
                {                 /* add it to the list */
                        Boolean mod;
                        XmFontList origFontList;
                        ...
                        _DtHelpFileListAddFile(
                            &hw->help_dialog.srch.volListHead,
                            &hw->help_dialog.srch.volTitlesFontList, &mod, path,
                            NULL, GetVolumeInfoCB, _DtHELP_FILE_NAME,
                            _DtHELP_FILE_TITLE,
                            hw->help_dialog.help.pDisplayArea);
                        if (mod) {
                                MergeFontListIntoWidgetFonts(
                        ...
                        /* (F,F,F): disable search, disable display, for all
                         * volumes */
                        AddVolInfoToList(hw->help_dialog.srch.volListHead,
                                         False, False, False);
                        cur = _DtHelpFileListGetMatch(
                            hw->help_dialog.srch.volListHead, path,
                ...
                if (cur) /* file present in list */
                {
                        ...
                        _DtHelpGlobSrchVol *vol =
                            (_DtHelpGlobSrchVol *)cur->clientData;

                        vol->searchThisVolume = True;
                        vol->showVolInList = True;
                        vol->zeroHitsOk =
                            True; /* show zero hits for search of cur vol */
                } else            /* file not found */
                {
 ...
 *
 ************************************************************************/
static void CloseSearchCB(Widget w, XtPointer clientData, XtPointer callData) {
...
static void DeleteListContents(_DtHelpGlobSearchStuff *srch) {
        ...
        for (curFile = srch->volListHead; NULL != curFile;
             curFile = _DtHelpFileListGetNext(NULL, curFile)) {
                register _DtHelpGlobSrchVol *vol =
                    (_DtHelpGlobSrchVol *)curFile->clientData;
                if (NULL == vol)
                        continue; /* CONTINUE */
                ...
                if (vol->hitsDisplayed &&
                    ((vol->nextVolPosition - vol->startPosition) !=
                     vol->hitCnt)) {
                        register _DtHelpGlobSrchHit *hit;
                        for (hit = vol->hitListHead; NULL != hit;
                             hit = hit->next) {
   ...
   if (status == hw->help_dialog.srch.statusLineUsage && !forceUpdate)
      return;
        ...
        case SCANNING_STATUS:
                labelMsg = (char *)_DTGETMESSAGE(
                    GSSET, 17, "Scanning for volumes... Found %d");
                snprintf(buf, 100, labelMsg, intArg);
                labelMsg = buf;
                break; /* BREAK */
        case WORKING_STATUS:
                labelMsg = (char *)_DTGETMESSAGE(
                    GSSET, 18, "Searching... Volumes remaining: %d");
                /* we need +1 because of when this message is used */
                snprintf(buf, 100, labelMsg,
                         hw->help_dialog.srch.volLeftCnt + 1);
                labelMsg = buf;
                break; /* BREAK */
        case SEARCH_RESULTS_STATUS:
                /* if no hits found */
                if (hw->help_dialog.srch.hitsFound ==
                    False) /*dont use == True*/
                {
                        if (hw->help_dialog.srch.fullIndex) {
                                if (hw->help_dialog.srch.srchSources ==
                                    _DtHelpGlobSrchAllVolumes)
                                        ...
                                        labelMsg = (char *)_DTGETMESSAGE(
                                            GSSET, 50,
                                            "No index entries found.  Try All "
                                            "Volumes.");
                        } else /* searched on a pattern */
                        {
                                labelMsg = (char *)_DTGETMESSAGE(
                                    GSSET, 51,
                                    "No index entries found.  Try another "
                                    "word.");
                        }
                        /* change focus to the search word.  Traversal is
                           ignored if srchWord is desensitized (Contains btn not
                           selected) */

                        if (XtIsSensitive(hw->help_dialog.srch.wordField)) {
                                /*
                                 * make sure the text field is editable before
                                 * sending the traversal there.
                                 */
                                XtSetArg(args[0], XmNeditable, True);
                                XtSetValues(hw->help_dialog.srch.wordField,
                                            args, 1);
                                XmProcessTraversal(
                ...
                } else /* if hits found */
                {
                        ...
                        if (hw->help_dialog.srch.fullIndex) {
                                labelMsg = (char *)_DTGETMESSAGE(
                                    GSSET, 20, "Complete Index");

                        } else {
                                labelMsg = (char *)_DTGETMESSAGE(
                                    GSSET, 21, "Entries with \"%s\"");
                                snprintf(buf, 100, labelMsg,
                                         hw->help_dialog.srch.rawWordStr);
                                labelMsg = buf;
                        }
                        /* change focus to the results list */
                        XmProcessTraversal(hw->help_dialog.srch.resultList,
                                           XmTRAVERSE_CURRENT);
        ...
        case NO_VOL_STATUS:
                labelMsg =
                    (char *)_DTGETMESSAGE(GSSET, 22, "No volumes selected");
                break; /* BREAK */
        case FIRST_PROMPT_STATUS:
                labelMsg = (char *)_DTGETMESSAGE(GSSET, 23,
                                                 "Select search options above");
                break; /* BREAK */
        case BLANK_STATUS:
        default:
                /* DBG: when an empty string, form constraints are wrong; make a
                 * space */
                labelMsg = " ";
                break; /* BREAK */
        }
 ...
 *****************************************************************************/
static Boolean SearchForPattern(char *string, char *pattern, int patternLen) {
 ...
 *****************************************************************************/
static Boolean OpenVolForSearch(DtHelpDialogWidget hw, char *fullFilePath,
                                _DtHelpGlobSrchVol *curVol) {
        ...
        if (NULL == curVol->volHandle) {
                ...
                if (curVol->gotLocale == False) {
                        curVol->stdLocale =
                            _DtHelpGetVolumeLocale(curVol->volHandle);
                        /* get the op-specific locales; the strings are default
                         * values */
                        _DtHelpCeXlateStdToOpLocale(
                            DtLCX_OPER_ICONV3, curVol->stdLocale, "iso88591",
                            &curVol->iconv3Codeset);
                        curVol->gotLocale = True;
                }
                openedVolume = True;

                /* get the codeset of the application's locale, if haven't
                 * gotten it */
                if (NULL == hw->help_dialog.srch.iconv3Codeset) {
                        ...
                        /* get the op-specific locales; the strings are default
                         * values */
                        /* impt: XlateStdToOpLocale() call requires a locale,
                         * not a codeset */
                        _DtHelpCeXlateStdToOpLocale(
                            DtLCX_OPER_ICONV3, locale, "iso88591",
                            &hw->help_dialog.srch.iconv3Codeset);
                        XtFree(locale);
                }
                ...
                if (_DtHelpCeIconvContextSuitable(
                        hw->help_dialog.srch.iconv3Context,
                        curVol->iconv3Codeset,
                        hw->help_dialog.srch.iconv3Codeset) == False) {
                        /* close old, open new */
                        _DtHelpCeIconvClose(
                            &hw->help_dialog.srch.iconv3Context);
        ...
        if (openedVolume) {
                /* we just share pointers; when freeing localeWordStr, we need
                   to test whether they are different before freeing */
                hw->help_dialog.srch.localeWordStr =
                    hw->help_dialog.srch.normWordStr;
        }
 ...
 *
 *****************************************************************************/
static void CloseVolAfterSearch(DtHelpDialogWidget hw,
                                _DtHelpGlobSrchVol *curVol) {
        ...
        if (NULL != curVol->volHandle) {
                ...
                if (hw->help_dialog.srch.localeWordStr !=
                    hw->help_dialog.srch.normWordStr) {
 ...
 *
 *****************************************************************************/
static void SearchTopic(DtHelpDialogWidget hw, _DtHelpFileEntry curFile,
                        _DtHelpGlobSrchVol *curVol, char *srchWord,
                        int srchWordLen, int numSearches) {
#if 0
    int i;
 ...
 *
 *****************************************************************************/
static void SearchIndex(DtHelpDialogWidget hw, _DtHelpFileEntry curFile,
                        _DtHelpGlobSrchVol *curVol,
                        _DtHelpCeIconvContext iconvContext, char *srchWord,
                        int srchWordLen, int numSearches) {
        int i;
        ...
        if (NULL == curVol->indexXmStrsList) {
                ...
                /* work with copy, because FontListAppendEntry() destroys input
                 * FL */
                fontList = XmFontListCopy(fontList);
#endif
                ...
                _DtHelpFormatIndexEntries(
                    hw->help_dialog.help.pDisplayArea, curVol->volHandle,
                    &numEntries, &curVol->indexXmStrsList, &fontList, &mod);
        ...
        if (NULL == curVol->indexEntriesList) {
                ...
                _DtHelpCeGetKeywordList(curVol->volHandle,
                                        &curVol->indexEntriesList);
        ...
        /* get a index entry */
        for (i = 0; i < numSearches && NULL != curVol->indexEntriesList[0];
             i++, curVol->indexEntriesList++, curVol->curIndexXmStr++) {
                ...
                if (NULL == srchWord ||
                    (_DtHelpCeIconvStr(iconvContext,
                                       curVol->indexEntriesList[0], &indexEntry,
                                       &entryLen, indexEntry, entryLen) == 0 &&
                     SearchForPattern(indexEntry, srchWord, srchWordLen) ==
                         True)) {
                        _DtHelpGlobSrchHit *hit;
                        char **topicIdList;

                        HitListAddFound(curFile, curVol->curIndexXmStr[0],
                                        curVol->indexEntriesList[0], True,
                                        &hit);

                        /* topicIdList is set but not allocated & need not be
                         * freed */
                        hit->topicCnt = _DtHelpCeFindKeyword(
                            curVol->volHandle, hit->indexEntry, &topicIdList);
                }
 ...
 *
 *****************************************************************************/
static void SearchVolume(DtHelpDialogWidget hw, _DtHelpFileEntry curFile,
                         _DtHelpGlobSrchVol *curVol, int numSearches) {
        XmString labelString;
        Arg args[10];
        char buf[50];
        ...
        */
        if (curVol->indexSearchInProgress) {
                ...
                if (hw->help_dialog.srch.fullIndex)
                        srchWord = NULL;
                SearchIndex(hw, curFile, curVol,
                            hw->help_dialog.srch.iconv3Context, srchWord,
                            hw->help_dialog.srch.wordFieldLen, numSearches);
        }
 ...
 *
 *****************************************************************************/
static void SearchFile(DtHelpDialogWidget hw, _DtHelpFileEntry file) {
        register _DtHelpGlobSrchVol *vol;
        ...
        if (NULL == vol)
                AddVolInfoToFile(file, True, True, False);
        vol = (_DtHelpGlobSrchVol *)file->clientData;
        if (NULL == vol)
                return;
 ...
 *
 *****************************************************************************/
static Boolean DoBackgroundSearchWP(XtPointer clientData) {
        DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
        _DtHelpFileEntry curFile;
...
#define NUM_SEARCHES_PER_CALL 30
        ...
        if (curVol->searchCompleted) {
                ...
                if (curVol->hitCnt > 0 || curVol->zeroHitsOk) {
                        ...
                        XmUpdateDisplay(
                            (Widget)hw->help_dialog.srch.resultList);
                }
...
searchComplete : {
        ...
        StopSearchCB(hw->help_dialog.srch.actionBtn, (XtPointer)hw,
                     (XtPointer)&status);
 ...
 *****************************************************************************/
static void StartSearchCB(Widget w, /* dont use */
                          XtPointer clientData,
...
/* 9/26/94: these two XtSetArgs (and following SetValues) cause a core dump
   ...
   HP-UX 9.05. */
                ...
                /* process the source selection and retain in widget srchWord
                 * memory */
                PrepSearchSourceData(hw, srchWord);

                /* activate search thread; do it even with a 0 count, because
                   the process reactivates the dialog after search is completed
                 */
                appCntx = XtWidgetToApplicationContext((Widget)hw);
                hw->help_dialog.srch.workProcId = XtAppAddWorkProc(
                    appCntx, DoBackgroundSearchWP, (XtPointer)hw);
                ...
                _DtHelpTurnOffHourGlass(
                    XtParent(hw->help_dialog.srch.srchForm));
                if (hw->help_dialog.srch.selectionDlg)
                        _DtHelpTurnOffHourGlass(
                            XtParent(hw->help_dialog.srch.selectionDlg));
        } else {
 ...
 *
 *****************************************************************************/
static void StartSelectedVolumeSearchCB(Widget w, XtPointer clientData,
                                        XtPointer callData) {
        DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
        ...
        if (hw->help_dialog.srch.fullIndex == False) {
                ...
                /* test the word ; False=dontUpdate--do this in
                 * PrepSearchSourceData() */
                if (CheckSearchWord(hw, srchWord, False) ==
                    True) { /* word has changed since last search; do a whole
                               new search */
        ...
        for (file = hw->help_dialog.srch.volListHead; NULL != file;
             file = _DtHelpFileListGetNext(NULL, file)) {
                ...
                /* test for contradictory flags; fileSelected reflects dialog
                 * list */
                if (file->fileSelected != vol->showVolInList) {
                        /* file has been deselected */
                        if (False == file->fileSelected) {
                                if (vol->showVolInList) {
                                        /* update list: delete hits, topics, and
                                         * volume title */
                                        if (vol->hitsDisplayed)
        ...
        /* update state */
        if (noSelectedFiles)
                StatusLabelUpdate(hw, NO_VOL_STATUS, True, 0);
}
 ...
 *
 *****************************************************************************/
static void ActionButtonCB(Widget w, XtPointer clientData, XtPointer callData) {
        DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
 ...
 *
 *****************************************************************************/
static void GotoResultCB(Widget w, XtPointer clientData, XtPointer callData) {
        DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
        char *helpFile = NULL;
        ...
        status = XmListGetSelectedPos(hw->help_dialog.srch.resultList,
                                      &topicPosList, &i);
        ...
        if (status && topicPosList) {
                ...
                /* Get the entry */
                HitListGetNth(hw->help_dialog.srch.volListHead, topicPosition,
                              &file, &entry, &locationId, &helpFile);
                ...
                hw->help_dialog.ghelp.volumeFlag =
                    False; /* force search for volume */
                hw->help_dialog.display.helpType = DtHELP_TYPE_TOPIC;
 ...
 *
 *****************************************************************************/
static void GotoLocation(DtHelpDialogWidget hw, char *helpVolume,
                         char *locationId) {
 ...
 *
 *****************************************************************************/
static void ProcessResultSelectionCB(Widget w, XtPointer clientData,
                                     XtPointer callData) {
        DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
        int *topicPosList = NULL;
        int topicPosition;
        ...
        if (status && topicPosList) {
                ...
                if (HitListGetNth(hw->help_dialog.srch.volListHead,
                                  topicPosition, &file, &hit, &locationId,
                                  &helpFile) == 0) {
                        _DtHelpTurnOnHourGlass(
                            XtParent(hw->help_dialog.srch.srchForm));

                        /* deactivate it */
                        XmListDeselectPos(hw->help_dialog.srch.resultList,
                                          topicPosition);

                        /* was a volume item selected? */
                        if (NULL == hit) {
                                _DtHelpGlobSrchVol *vol =
                                    (_DtHelpGlobSrchVol *)file->clientData;
                                if (vol->hitCnt > 0) {
                                        if (False == vol->hitsDisplayed) {
                                                VolHitsDisplay(
                                                    hw,
                                        ...
                                        /* update the volume label to show the
                                         * state */
                        ...
                        /* was an expandable index selected */
                                ...
                                if (False == hit->topicsDisplayed) {
                                        topicCnt = HitTopicsDisplay(
                                            hw, file, hit, topicPosition + 1);
                                } else {
                                        topicCnt = -HitTopicsUndisplay(
                                            hw, file, hit, topicPosition + 1);
                                        hit->showTopicsWithHit = False;
                                }
                                /* update the hit label to show the state;
                                 * False--dont insert */
                                HitNameDisplay(hw, file, hit, topicPosition,
                                               False);

                                /* add to all the following */
                                if (topicCnt != 0) {
                                        /* adjust count beginning with next file
                                         * with hits */
                                        AdjustPositionValues(file, 0, topicCnt,
                                                             True);
                                }
                        }
                        /* a location or index with one topic was selected */
                                ...
                                if (hit->topicCnt == 1) {
                                        if (HitLoadTopics(hw, file, hit) == 0) {
                                                GotoLocation(
                                                    hw, file->fullFilePath,
                                                    hit->topicIdList[0]);
                                                HitFree(
                                                    hit,
                                                    False); /*release what was
                                                               loaded 3 lines
                                                               up*/
                                        } else { /* volume couldn't be opened or
                                                    some other error */
                                                 /* FIX: error message */
                                        }
                                }      /* if an index with a single topic */
                                else { /* a location from an open multi-topic
                                          index entry */
                                        GotoLocation(hw, file->fullFilePath,
                                                     locationId);
                                }
 ...
 *
 *****************************************************************************/
static void CloseVolSelDialogCB(Widget widget, XtPointer clientData,
                                XtPointer callData) {
        DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
        ...
        if (hw->help_dialog.srch.hitsFound == True)
                ...
                XmProcessTraversal(hw->help_dialog.srch.allVolRadBtn,
                                   XmTRAVERSE_CURRENT);
}
 ...
 *
 *****************************************************************************/
static void CreateVolSelDialog(DtHelpDialogWidget hw) {
        Widget selDlg;
        ...
        selDlg = _DtHelpFileListCreateSelectionDialog(
            ...
            (char *)_DTGETMESSAGE(GSSET, 40, "Help - Search Volume Selection"),
            &hw->help_dialog.srch.volTitlesFontList,
        ...
        XtAddCallback(dlgChld.form, XmNunmapCallback, CloseVolSelDialogCB,
                      (XtPointer)hw);
        ...
        wm_delete_window =
            XmInternAtom(XtDisplay(dlgChld.shell), "WM_DELETE_WINDOW", FALSE);
        XmAddWMProtocolCallback(dlgChld.shell, wm_delete_window,
                                (XtCallbackProc)CloseVolSelDialogCB,
                                (XtPointer)hw);
 ...
 *
 *****************************************************************************/
static void OpenSelectDialogCB(Widget widget, XtPointer clientData,
                               XtPointer callData) {
        DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
        ...
        /* FIX: the modal behaviour doesn't seem to be set; is it
         * desired/needed? */
...
static void UpdateSearchVolumesCB(Widget widget, XtPointer clientData,
                                  XtPointer callData) {
        XmToggleButtonCallbackStruct *status =
        ...
        if (False == status->set) {
                XtSetArg(args[0], XmNset,
                         hw->help_dialog.srch.allVolRadBtn == widget);
                ...
                XtSetArg(args[0], XmNset,
                         hw->help_dialog.srch.curVolRadBtn == widget);
        ...
        if (XmCR_VALUE_CHANGED == status->reason && True == status->set &&
                                       ...
                                       button is pressed. */
        {
                 ...
                 * ***/
                hw->help_dialog.srch.volLeftCnt =
                    0; /* change invalidates prior search */
                ...
                if (hw->help_dialog.srch.selVolRadBtn == widget) {
                        ...
                        if (False == hw->help_dialog.srch.volScanDone ||
                            NULL == hw->help_dialog.srch.volListHead) {
                                /* Add on the Vol info; (T,T,T) search, display,
                                 * selected only */
                                VolListBuild(hw, True, True, True);
                        }

                        /* (F,F,F,F): disable search,disable display,no 0
                         * hits,for all vols */
                        SetVolStatus(hw->help_dialog.srch.volListHead, False,
                                     False, False, False);
                        /* (T,T,T,T): enable search, enable display,
                                    zero hits ok, only for selected volumes */
                        /* Set these here so that the CountSelectedVolumes()
                         * works right */
                        SetVolStatus(hw->help_dialog.srch.volListHead, True,
                                     True, True, True);

                        /* count all volumes selected, including those already
                         * searched */
                        if (CountSelectedVolumes(
                                hw->help_dialog.srch.volListHead, True) == 0) {
                                StatusLabelUpdate(hw, NO_VOL_STATUS, False, 0);
                                DeleteListContents(&hw->help_dialog.srch);
                                CreateVolSelDialog(hw);
                                /* NOTE: don't start search here, start it from
                                 * the callback */
                                /* set state of start button correctly */
                                UpdateSearchStartStatusCB(NULL, (XtPointer)hw,
                                                          NULL);
                ...
                /*** all volumes button ***/
                        ...
                        if (hw->help_dialog.srch.selectionDlg)
                                XtUnmanageChild(
                                    hw->help_dialog.srch.selectionDlg);
                ...
                /* do this last, so that while searching for volumes
                   (VolListBuild), the button is not active. */
                XtSetSensitive(
 ...
 *
 *****************************************************************************/
static void UpdateIndexSelectionCB(Widget widget, XtPointer clientData,
                                   XtPointer callData) {
        XmToggleButtonCallbackStruct *status =
            (XmToggleButtonCallbackStruct *)callData;
        DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
                                       ...
                                       button is pressed. */
        {
                ...
                if (hw->help_dialog.srch.searchInProgress ||
                ...
                n = 0;
                XtSetArg(args[n], XmNeditable, containsState);
                n++;
                XtSetArg(args[n], XmNcursorPositionVisible, containsState);
                n++;
                XtSetValues(hw->help_dialog.srch.wordField, args, n);

                /* set widget status here; do this way bec. srchFullIndex is a
                 * bit flag */
                hw->help_dialog.srch.fullIndex =
                    (containsState ? False : True); /*flag*/

                /* set state of start button correctly */
                UpdateSearchStartStatusCB(NULL, (XtPointer)hw, NULL);

                if (containsState) { /* if changing to "Containing" */
                        /* change focus to the search word */
                        XmProcessTraversal(hw->help_dialog.srch.wordField,
                                           XmTRAVERSE_CURRENT);
                        /* user will need to hit CR to initiate search */
                } else { /* if changing to full index */
                        /* indicate that no contains word is valid by deleting
                         * it */
                        /* FIX: I'm concerned that normWordStr & localeWordStr
                         * aren't in synch */
                        XtFree(hw->help_dialog.srch.normWordStr);
 ...
 *
 *****************************************************************************/
static void ContainsDisarmCB(Widget widget, XtPointer clientData,
                             XtPointer callData) {
        XmToggleButtonCallbackStruct *status =
            (XmToggleButtonCallbackStruct *)callData;
        DtHelpDialogWidget hw = (DtHelpDialogWidget)clientData;
 ...
 *
 *****************************************************************************/
static Boolean CreateCurVolBtnLabel(DtHelpDialogWidget hw,
   ...
   return False;
        ...
        if (hw->help_dialog.display.helpType == DtHELP_TYPE_TOPIC) {
                ...
                curVolAvail =
                    VolumeHasIndexP(hw->help_dialog.srch.srchSources,
                                    hw->help_dialog.display.helpType,
                                    hw->help_dialog.display.helpVolume);
                ...
                if (path) {
                        ...
                        GetVolumeInfoCB(hw->help_dialog.help.pDisplayArea, path,
                                        NULL, &volTitleString, NULL, NULL, NULL,
                                        &fontList, &mod);
                        ...
                        if (mod) {
                                ...
                                XtSetArg(args[n], XmNfontList, fontList);
                                n++;
                                XtSetValues(hw->help_dialog.srch.curVolRadBtn,
                                            args, n);
                                if (fontList)
                                        XmFontListFree(fontList);
                        }
        ...
        else { /* if helpType != TOPIC */
                curVolAvail = False;
                switch (hw->help_dialog.display.helpType) {
                case DtHELP_TYPE_STRING:
                case DtHELP_TYPE_DYNAMIC_STRING:
                        volTitleString = XmStringCreateLocalized(
                            ((char *)_DTGETMESSAGE(GSSET, 32, "Help Message")));
                        break;
                case DtHELP_TYPE_FILE:
                        volTitleString = XmStringCreateLocalized(
                            ((char *)_DTGETMESSAGE(GSSET, 33, "Help File")));
                        break;
                case DtHELP_TYPE_MAN_PAGE:
                        volTitleString = XmStringCreateLocalized(
                            ((char *)_DTGETMESSAGE(GSSET, 34, "Manual Page")));
                        break;
                default:
                        volTitleString = XmStringCreateLocalized(((
                            char *)_DTGETMESSAGE(GSSET, 35, "Unknown Format")));
                        break;
                } /* switch on helpType */
        }         /* if helpType != TOPIC */
        ...
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(GSSET, 3, "Current")));
        ...
        /* set the out values */
        if (out_curVolAvailable)
                *out_curVolAvailable = curVolAvail;
        if (out_mnemonic)
                *out_mnemonic = mnemonic;
        if (out_labelString)
                *out_labelString = labelString;
        else
                XmStringFree(labelString);
 ...
 *
 *****************************************************************************/
static void CreateGlobSrchDialog(Widget nw, char *searchWord) {
        Widget parentForm;
        Widget separator;
        Widget srchShell;
        /* Widget        topWidget; */
        ...
        curVolAvailable = VolumeHasIndexP(hw->help_dialog.srch.srchSources,
                                          hw->help_dialog.display.helpType,
                                          hw->help_dialog.display.helpVolume);
        ...
        title = XtNewString(
            ((char *)_DTGETMESSAGE(GSSET, 1, "Help - Index Search")));
        n = 0;
   ...
   XtSetValues (srchShell, args, n);
        ...
        wm_delete_window =
            XmInternAtom(XtDisplay(srchShell), "WM_DELETE_WINDOW", FALSE);
        XtSetArg(args[0], XmNdeleteResponse, XmDO_NOTHING);
        XmAddWMProtocolCallback(srchShell, wm_delete_window, CloseSearchCB,
                                (XtPointer)&hw->help_dialog.srch);
        XtSetValues(srchShell, args, 1);

        /* set the callback that positions the dialog when popped up */
        XtAddCallback(srchShell, XmNpopupCallback, (XtCallbackProc)_DtHelpMapCB,
        ...
        labelString =
            XmStringCreateLocalized((_DTGETMESSAGE(GSSET, 2, "Search")));
        n = 0;
        ...
        n++;
        frameTitle =
            XmCreateLabelGadget(volumesFrame, "volFrameTitle", args, n);
        ...
        n++;
        hw->help_dialog.srch.curVolRadBtn = XmCreateToggleButtonGadget(
            volumesForm, "srchCurVolRadBtn", args, n);
        XtAddCallback(hw->help_dialog.srch.curVolRadBtn,
                      XmNvalueChangedCallback, UpdateSearchVolumesCB,
                      (XtPointer)hw);
        /* ??? XtSetSensitive (hw->help_dialog.srch.curVolRadBtn,False); */
        ...
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(GSSET, 5, "All Volumes")));
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, hw->help_dialog.srch.curVolRadBtn);
        n++;
        ...
        XtSetArg(args[n], XmNhighlightOnEnter, True);
        n++;
        hw->help_dialog.srch.allVolRadBtn = XmCreateToggleButtonGadget(
            volumesForm, "srchAllVolRadBtn", args, n);
        XtAddCallback(hw->help_dialog.srch.allVolRadBtn,
                      XmNvalueChangedCallback, UpdateSearchVolumesCB,
                      (XtPointer)hw);
        ...
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(GSSET, 6, "Selected")));
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, hw->help_dialog.srch.allVolRadBtn);
        n++;
        XtSetArg(args[n], XmNtopOffset, 1);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNhighlightOnEnter, True);
        n++;
        hw->help_dialog.srch.selVolRadBtn = XmCreateToggleButtonGadget(
            volumesForm, "srchSelVolRadBtn", args, n);
        XtAddCallback(hw->help_dialog.srch.selVolRadBtn,
                      XmNvalueChangedCallback, UpdateSearchVolumesCB,
                      (XtPointer)hw);
        ...
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(GSSET, 7, "Select Volumes...")));
        ...
        XtSetArg(args[n], XmNbottomOffset, 2);
        n++;
        XtSetArg(args[n], XmNmarginTop, 0);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, volumesFrame);
        n++;
        XtSetArg(args[n], XmNtopOffset, 5);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
        ...
        /* create the frame title */
        labelString =
            XmStringCreateLocalized((_DTGETMESSAGE(GSSET, 8, "Show")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        XtSetArg(args[n], XmNtraversalOn, False);
        ...
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(GSSET, 9, "Complete Index")));
        ...
        XtSetArg(args[n], XmNhighlightOnEnter, True);
        n++;
        hw->help_dialog.srch.fullIndexRadBtn = XmCreateToggleButtonGadget(
            showForm, "srchFullIndexRadBtn", args, n);
        XtAddCallback(hw->help_dialog.srch.fullIndexRadBtn,
                      XmNvalueChangedCallback, UpdateIndexSelectionCB,
                      (XtPointer)hw);
        ...
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(GSSET, 10, "Entries with:")));
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, hw->help_dialog.srch.fullIndexRadBtn);
        n++;
        XtSetArg(args[n], XmNtopOffset, 1);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        XtAddCallback(hw->help_dialog.srch.containsRadBtn,
                      XmNvalueChangedCallback, UpdateIndexSelectionCB,
                      (XtPointer)hw);
        ...
        n++;
        XtSetArg(args[n], XmNleftWidget, hw->help_dialog.srch.containsRadBtn);
        n++;
        XtSetArg(args[n], XmNleftOffset, 5);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, showFrame);
        n++;
        ...
        StatusLabelUpdate(hw, FIRST_PROMPT_STATUS, True,
                          0); /* installs a label */
        ...
        XtAddCallback(hw->help_dialog.srch.resultList,
                      XmNsingleSelectionCallback, ProcessResultSelectionCB,
                      (XtPointer)hw);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, hw->help_dialog.srch.statusLabel);
        n++;
        XtSetArg(args[n], XmNtopOffset, 5);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNbottomOffset, 70);
        n++;
        ...
        XtSetArg(args[n], XmNtopWidget,
                 XtParent(hw->help_dialog.srch.resultList));
        n++;
        XtSetArg(args[n], XmNtopOffset, 5);
        n++;
        separator = XmCreateSeparatorGadget(parentForm, "separator", args, n);
        ...
        _DtHelpProcessLock();
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(GSSET, START_SEARCH_CAT, START_SEARCH_STR)));
        _DtHelpProcessUnlock();
        ...
        _DtHelpProcessLock();
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(GSSET, CLOSE_BTN_CAT, CLOSE_BTN_STR)));
        _DtHelpProcessUnlock();
        ...
        XtAddCallback(hw->help_dialog.srch.closeBtn, XmNactivateCallback,
        ...
        _DtHelpProcessLock();
        labelString = XmStringCreateLocalized(
            (char *)_DTGETMESSAGE(GSSET, HELP_BTN_CAT, HELP_BTN_STR));
        _DtHelpProcessUnlock();
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_srchIndexHelpBtn_STR, (Widget)hw,
                                   &hw->help_dialog.help,
        ...
        n = 0;
        XtSetArg(args[n], XmNcancelButton, hw->help_dialog.srch.closeBtn);
        n++;
...
#define NUMSTRS 5
                ...
                float scale = 0.0;
                float posList
                    [11]; /* need 11 due to algorithm, which uses index 10 */
                XmFontList fontList = NULL;
                int i;
...
#define BETBUTSPACE 5                   /* pixels */
                ...
                for (i = 0; i < NUMSTRS; i++) {
                        ...
                        Dimension width;
                        _DtHelpProcessLock();
                        s_GlobSrchDlgBtnStrs[i] = (char *)_DTGETMESSAGE(
                            GSSET, s_GlobSrchDlgBtnCatNum[i],
                            s_GlobSrchDlgBtnStrs[i]);
                        labelString =
                            XmStringCreateLocalized(s_GlobSrchDlgBtnStrs[i]);
                        _DtHelpProcessUnlock();
#define MARGINS 4 /* pixel margins: 2=L, 2=R */
                        width = XmStringWidth(fontList, labelString) +
                                borderWidth + MARGINS;
                        if (i <= 2)
                                maxWidth = max(width, maxWidth);
                        if (i == 2)
                                width =
                                    maxWidth; /* 1st three labels go in 1st
                                                 button */
                ...
                n++;
                XtSetArg(args[n], XmNleftPosition,
                         (Dimension)(posList[6] * scale));
                n++;
                XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
                n++;
                XtSetArg(args[n], XmNrightPosition,
                         (Dimension)(posList[7] * scale));
                n++;
                ...
                n++;
                XtSetArg(args[n], XmNleftPosition,
                         (Dimension)(posList[8] * scale));
                n++;
                XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
                n++;
                XtSetArg(args[n], XmNrightPosition,
                         (Dimension)(posList[9] * scale));
                n++;
        ...
        { /* set the proper offset between the bottom of the results list
             ...
             button bar. */
                ...
                /* first calc offset of list to form bottom based on earlier
                 * sizes */
#define KNOWN_OFFSETS_BELOW_LIST 20 /* actually only 15, but 15 fails */
                n = 0;
                XtSetArg(args[n], XmNborderWidth, &widgetBorderHeight);
                n++;
                XtSetArg(args[n], XmNheight, &widgetHeight);
                n++;
         ...
         */
        pHelpInfo = _DtHelpListAdd(DtHELP_srchIndexShell_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(parentForm, XmNhelpCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
}
 ...
 *
 *****************************************************************************/
static int updateDisplay(DtHelpDialogWidget hw) {
        ...
        XmUpdateDisplay(
            XtParent((Widget)hw->help_dialog.srch.srchForm)); /*DBG*/
        return 0;
}
 ...
 *
 *****************************************************************************/
void _DtHelpGlobSrchDisplayDialog(Widget w, char *searchWord, char *curVolume) {
   ...
   int           n;
        ...
        if (NULL == hw->help_dialog.srch.srchForm) {
                ...
                XmUpdateDisplay(
                    XtParent((Widget)hw->help_dialog.srch.srchForm));
                XmUpdateDisplay(
                    XtParent((Widget)hw->help_dialog.srch.srchForm));
                XmUpdateDisplay(
                    XtParent((Widget)hw->help_dialog.srch.srchForm));
                ...
                /* set full index if current vol has an index, contains word if
                 * not */
                if (hw->help_dialog.srch
                        .curVolRadBtnSens) /* set in UpdateCurVolBtnSens*/
                ...
                status.set = True;
                status.event =
                    (XEvent *)1; /* thwart == NULL test in Update...() */
                UpdateIndexSelectionCB(sourceBtn, (XtPointer)hw,
                                       (XtPointer)&status);
                StatusLabelUpdate(hw, FIRST_PROMPT_STATUS, False, 0);
      ...
      XtSetValues(XtParent((Widget)hw->help_dialog.srch.srchForm), args, n);
        ...
        } else /* properly update dialog to support new word and cur vol */
        {
                ...
                if (NULL != searchWord) {
                        ...
                        if (CheckSearchWord(hw, srchWord, True) ==
                            True) { /* words are different */
                                /* free all hit-related data and reset flags */
                                HitListFreeAllVolHits(
                                    hw, True); /*True=free everything*/
                        }
                }
 ...
 *
 *****************************************************************************/
void _DtHelpGlobSrchUpdateCurVol(Widget w) {
        String path;
        DtHelpDialogWidget hw = (DtHelpDialogWidget)w;
        ...
        if (hw->help_dialog.display.helpVolume)
                path = _DtHelpFileLocate(
                    DtHelpVOLUME_TYPE,
                    /* FIX: will helpVolume have matching path? */
                    hw->help_dialog.display.helpVolume, _DtHelpFileSuffixList,
                    False, R_OK);

        if (_DtHelpGlobSrchCurVolume == hw->help_dialog.srch.srchSources &&
            hw->help_dialog.display.helpType == DtHELP_TYPE_TOPIC &&
            NULL != path && NULL != hw->help_dialog.srch.curVolPath &&
        ...
        /* a different current volume or change of topic */
        {
                ...
                /* if still searching previous cur volume, stop any search in
                 * progress */
                if (_DtHelpGlobSrchCurVolume ==
                        hw->help_dialog.srch.srchSources &&
                    hw->help_dialog.srch.workProcId) {
                        /* cancel the search */
                        ...
                        /* assumption is, that even though the search was
                           incomplete, nothing bad will happen if we don't free
                           the hit data, and the search could resume where left
                           off, if necessary. */
                }
                ...
                if (_DtHelpGlobSrchCurVolume ==
                    hw->help_dialog.srch.srchSources) {
                        ...
                        if (hw->help_dialog.display.helpType ==
                            DtHELP_TYPE_TOPIC)
                                StartSearchCB(NULL, (XtPointer)hw, NULL);
                        else /* if current isn't a volume, clear the display &
                                update status */
                        {
                                DeleteListContents(&hw->help_dialog.srch);
                                StatusLabelUpdate(hw, NO_VOL_STATUS, False, 0);
...
void _DtHelpGlobSrchInitVars(_DtHelpGlobSearchStuff *srch) {
        ...
        srch->iconv3Codeset =
            NULL; /* iconv(3)-compatible code set of current locale */
...
void _DtHelpGlobSrchCleanAndClose(_DtHelpGlobSearchStuff *srch,
        ...
        if (destroy) {
                 ...
                 * The XtDestroyWidget that called me will have called the
                 * destroy callback of the widgets. So I don't have to do
                 * anything except make sure I know they are destroyed.
                 */
                srch->selectionDlg = NULL;
                srch->srchForm = NULL;
                srch->resultList = NULL;
