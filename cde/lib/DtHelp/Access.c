...
static int VolumeUnload(_DtHelpVolume vol) {
        ...
        if (vol != NULL) {
                ...
                if (vol->keywordTopics != NULL) {
                        for (topicList = vol->keywordTopics; *topicList != NULL;
                             topicList++)
...
static int GetTopicTitleAndAbbrev(_DtHelpVolume volume, char *id,
        ...
        if (_DtHelpCeFindId(volume, id, lockInfo.fd, &filename, &offset) !=
            True)
                result = -2;
        ...
        if (result == 0) {
                if (0 == _DtHelpCeGetVolumeFlag(volume)) {
                        ...
                        if (result == 0) {
                                result = -1;
                                if (_DtHelpCeReadBuf(file, buffer, BUFF_SIZE) !=
                                    -1) {
                                        result = 0;
                                        bufPtr = buffer;
                                        if (_DtHelpCeGetCcdfTopicAbbrev(
                                                NULL, file, buffer, &bufPtr,
                                                BUFF_SIZE, MB_CUR_MAX, ret_name,
                                                NULL, ret_abbrev) != 0)
                                                result = -3;
                                }
                        ...
                        _DtHelpProcessLock();
                        if (_DtHelpCeFrmtSDLTitleToAscii(
                                volume, offset, ret_name, ret_abbrev) != 0)
                                result = -3;
                        _DtHelpProcessUnlock();
                }
        }
                        ...
                        default:
                                ...
                                if (i < MY_NUM) {
                                        if (mySubs[i].substitution != NULL) {
                                                subString =
                                                    mySubs[i].substitution;
                                                if (((int)(ptr - pathName +
                                                           strlen(subString))) >
                                                    MAXPATHLEN) {
                                                        errno =
                                                            CEErrorExceedMaxSize;
                                                        return NULL;
                                                }
                                        ...
                                        while (i < num && subs &&
                                               subs[i].match != *spec)
                                                i++;
                                        /*
                                         * If the substitution character is not
                                         * found include the character onto the
                                         * final string.
                                         */
                                        if (i >= num)
                                                *ptr++ = *spec;
...
int _DtHelpCeGetLangSubParts(char *lang, char **subLang, char **subTer,
        ...
        if (lang != NULL && *lang != '\0') {
                ...
                if (ptr) {
                        ...
                        /*
                         * if it was in the form lang_ter.codeset, sLang will
                         * non-null
                         */
                        if (sLang != NULL) {
                                /*
                                ...
                                /*
                                 * don't wan to save, but make sure we mark the
                                 * territory as being found (non-null).
                                 */
                                else
                                        sTer = lang;
                        }
                        ...
                        /*
                         * didn't want to save the lang portion, but mark as
                         * found.
                         */
                        else
                                sLang = lang;
                ...
                if (ptr && *ptr == '.') {
                        ...
                        /*
                         * don't save the code set, but make sure we mark as
                         * found
                         */
                        else
                                sCode = ptr;
                ...
                /*
                 * otherwise this is the territory of the language. Save if
                 * desired
                 */
                else if (subTer != NULL) {
                        sTer = strdup(lang);
...
int _DtHelpCeCompressPathname(char *basePath) {
        ...
        do {
                /*
                 * for multi-byte environments, check how far single bytes
                 * extend.
                 */
                char1 = False;
                char2 = False;
                ...
                if (MB_CUR_MAX == 1 || mblen(&ptr[1], MB_CUR_MAX) == 1) {
                        char1 = True;
                        if (MB_CUR_MAX == 1 ||
                            mblen(&ptr[2], MB_CUR_MAX) == 1) {
                                char2 = True;
                                if (MB_CUR_MAX == 1 ||
                                    mblen(&ptr[3], MB_CUR_MAX) == 1)
                                        char3 = True;
                        }
                }
                 ...
                 */
                        ...
                        if (prevPtr == NULL)
                                 ...
                                 */
                                *ptr = '\0';
                                result = _DtHelpCeStrrchr(basePath, Slash,
                                                          MB_CUR_MAX, &prevPtr);
                                if (result == -1)
                                        return -1;

                                /*
                                 * if there is no previous slash, set the
                                 * pointer to indicate that we're at the top of
                                 * the path (NULL).
                                 */
                                if (result != 0)
                                        prevPtr = NULL;
...
char *_DtHelpCeTracePathName(char *path) {
        ...
        do {
                ...
                *ptr = c;
                ...
                if (result == -1) {
                        /*
                         * if this was NOT a symbolic link, errno should be
                         * EINVAL
                         */
                        if (errno != EINVAL)
                                return NULL;
                        ...
                        /*
                         * put the null byte on the end of the symbolic link
                         * string.
                         */
                        linkPath[result] = '\0';
                        ...
                        if (*linkPath == '/')
                                 ...
                                 * prev is looking at the first character of
                                 * this directory. replace with the link.
                                 */
                                strlcpy(prev, linkPath,
                                        (2 * MAXPATHLEN + 2) - strlen(newPath) +
                                            strlen(prev));
                        }
...
char *_DtHelpCeTraceFilenamePath(char *file_path) {
        ...
        do {
                 ...
                 */
                result =
                    _DtHelpCeStrrchr(workName, Slash, MB_CUR_MAX, &oldName);
                if (result == -1)
                        return NULL;
                ...
                *oldName = '/';
                ...
                if (result == -1) {
                        ...
                        if (*linkName == '/')
                                 ...
                                 */
                                strlcpy((namePlace + 1), linkName,
                                        MAXPATHLEN + 2 - strlen(newName) - 1);
                        }
...
int _DtHelpCeFileOpenAndSeek(char *name, int offset, int fd,
        ...
        if (!*fileMagic) { /* started with compressed file magic number */
                 ...
                 */
                myInfo->fd = tmpFd;
                myInfo->size = (((fileMagic[1] * 256) + fileMagic[2]) * 256) +
                               fileMagic[3];
                ...
                *ret_file = _DtHelpCeBufFilePushZ(inputRaw);
                if (*ret_file == NULL) {
                        _DtHelpCeBufFileClose(inputRaw,
                                              (fd == -1 ? True : False));
                        return -1;
                }
        } else {
        ...
        if (vol == NULL || ret_idString == NULL ||
            CheckVolList(vol, NULL) == -1)
                ...
                if (vol->sdl_flag == False)
                        ...
                        *ret_idString =
                            _DtHelpCeGetSdlHomeTopicId((_DtHelpVolumeHdl)vol);
...
int _DtHelpCeFindId(_DtHelpVolumeHdl volume, char *target_id, int fd,
        ...
        if (vol->sdl_flag == False) {
                ...
                result = _DtHelpCeFindSdlId(vol, target_id, fd, ret_name,
                                            ret_offset);
        }
...
int _DtHelpCeGetTopicChildren(_DtHelpVolumeHdl volume, char *topic_id,
        ...
        if (vol->sdl_flag == False) {
                result =
                    _DtHelpCeGetCcdfTopicChildren(volume, topic_id, retTopics);
        } else {
                result =
                    _DtHelpCeGetSdlTopicChildren(volume, topic_id, retTopics);
        }
