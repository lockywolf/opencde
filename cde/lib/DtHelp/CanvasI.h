 ...
 * segment type access
 */
#define _DtCvIsTypeNoop(x) ((((x)&_DtCvPRIMARY_MASK) == _DtCvNOOP) ? 1 : 0)

/*
 * Is a flag set
 */
#define _DtCvIsSegVisibleLink(x) _DtCvIsSegHyperText(x)
#define _DtCvIsSegALink(x) (_DtCvIsSegHyperText(x) || _DtCvIsSegGhostLink(x))
 ...
 * remove masks
 */
#define _DtCvRemoveBeginFlags(x)                                               \
        x &= ~(_DtCvTRAVERSAL_BEGIN | _DtCvLINK_BEGIN | _DtCvMARK_BEGIN |      \
               _DtCvSEARCH_BEGIN)
#define _DtCvSetSearchEnd(x, seg) x |= ((seg)->type & _DtCvSEARCH_END)
#define _DtCvSetSearchBegin(x, seg) x |= ((seg)->type & _DtCvSEARCH_BEGIN)
#define _DtCvClearSearchFlags(x)                                               \
        x &= ~(_DtCvSEARCH_FLAG | _DtCvSEARCH_BEGIN | _DtCvSEARCH_END)

#define _DtCvClearLinkFlags(x)                                                 \
        x &= ~(_DtCvLINK_FLAG | _DtCvLINK_BEGIN | _DtCvLINK_END |              \
               _DtCvLINK_POP_UP | _DtCvLINK_NEW_WINDOW)
...
extern _DtCvUnit
_DtCvAdjustForSuperSub(_DtCanvasStruct *canvas, _DtCvSegmentI *p_seg,
                       _DtCvUnit start_x, _DtCvUnit *script_x,
                       _DtCvUnit *super_width, _DtCvUnit *super_y,
                       _DtCvUnit *sub_width, _DtCvUnit *sub_y,
                       _DtCvValue *super_flag, _DtCvValue *sub_flag);
extern _DtCvUnit _DtCvAdvanceXOfLine(_DtCanvasStruct *canvas,
                                     _DtCvSegmentI *p_seg, _DtCvUnit x_pos,
                                     int *link_idx, _DtCvValue *link_flag);
extern _DtCvStatus _DtCvCheckInfringement(_DtCvUnit tst_top, _DtCvUnit tst_bot,
                                          _DtCvUnit obj_top, _DtCvUnit obj_bot);
extern void _DtCvCheckLineMarks(_DtCanvasStruct *canvas, int line_idx,
                                int char_idx, int length, _DtCvUnit dst_x,
                                _DtCvFlags check_flags, int *ret_len,
                                _DtCvFlags *ret_old, _DtCvFlags *ret_new);
extern void _DtCvClearInternalUse(_DtCvSegmentI *list, _DtCvStatus flag);
extern _DtCvUnit _DtCvDrawSegments(
    _DtCanvasStruct *canvas, _DtCvDspLine line, _DtCvSegmentI *p_seg,
    int start_char, int count, int *prev_lnk, _DtCvUnit txt_x, _DtCvUnit sel_x,
    _DtCvUnit *scriptX, _DtCvUnit *super_width, _DtCvUnit *super_y,
    ...
    _DtCvFlags new_flag, _DtCvElemType trav_flag, _DtCvPointer trav_data);
extern int _DtCvGetCharIdx(_DtCanvasStruct *canvas, _DtCvDspLine line,
                           _DtCvUnit find_x);
extern _DtCvUnit _DtCvGetStartXOfLine(_DtCvDspLine *line, _DtCvSegmentI **pSeg);
extern void _DtCvGetWidthOfSegment(_DtCanvasStruct *canvas,
                                   _DtCvSegmentI *p_seg, int start, int max_cnt,
                                   int *ret_cnt, _DtCvUnit *ret_w,
                                   _DtCvValue *ret_trimmed);
extern _DtCvValue _DtCvModifyXpos(_DtCvSpaceMetrics info, _DtCvSegmentI *seg,
                                  _DtCvValue tst_result, _DtCvValue old_result,
                                  int idx, _DtCvUnit *x_pos);
extern void _DtCvSkipLineChars(_DtCanvasStruct *canvas, _DtCvSegmentI *p_seg,
                               int start, int max_cnt, int use_len,
                               int *ret_start, _DtCvSegmentI **ret_seg);
