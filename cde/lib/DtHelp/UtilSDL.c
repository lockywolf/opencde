 ...
 *
 *****************************************************************************/
static int SkipWhiteSpace(BufFilePtr f) {
        char c;
 ...
 *
 *****************************************************************************/
static int CheckSDLAttributeNameStart(char c) {
        if (('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z'))
                return 0;
 ...
 *
 *****************************************************************************/
static int GetSDLElement(BufFilePtr f, int max_allowed, char *string) {
 ...
 *
 *****************************************************************************/
int _DtHelpCeMatchSdlElement(BufFilePtr f, const char *element_str,
                             int sig_chars) {
        int len;
        int different = False;
 ...
 *
 *****************************************************************************/
int _DtHelpCeGetSdlAttribute(BufFilePtr f, int max_len, char *attribute_name) {
        int len = 0;
        char c;
        ...
        } while (c != BUFFILEEOF && !IsWhiteSpace(c) && c != '=' &&
                 len < max_len);
 ...
 *
 *****************************************************************************/
int _DtHelpCeReturnSdlElement(BufFilePtr f, const SDLElementInfo *el_list,
                              _DtCvValue cdata_flag,
                              enum SdlElement *ret_el_define, char **ret_data,
        ...
        *ret_end_flag = False;
        ...
        do {
                ...
                /*
                 * lower case the element. String dup it first incase this is
                 * data and not an element. Then skip the leading white space.
                 */
                ptr = nextEl;
                while (IsWhiteSpace(*ptr) == True)
                        ptr++;
        ...
        if (*ptr == '<') {
                ...
                while (different && el_list->element_str != NULL) {
                        if (endFlag == True &&
                            el_list->element_end_str != NULL &&
                            lowerChar == ((int)(el_list->element_end_str[2])) &&
                            _DtHelpCeStrNCaseCmpLatin1(
                                &(el_list->element_end_str[2]), ptr,
                                el_list->end_sig_chars - 2) == 0) {
                                different = False;
                                *ret_end_flag = True;
                        } else if (endFlag == False &&
                                   lowerChar ==
                                       ((int)(el_list->element_str[1])) &&
                                   _DtHelpCeStrNCaseCmpLatin1(
                                       &(el_list->element_str[1]), ptr,
 ...
 *
 *****************************************************************************/
int _DtHelpCeGetSdlCdata(BufFilePtr f, SdlOption type, int max_mb,
                         _DtCvValue nl_to_space, _DtCvValue *cur_space,
                         _DtCvValue *cur_nl, _DtCvValue *cur_mb,
                         char *non_break_char, char **string, int *str_max) {
...
#define ESC_STR_LEN 4
        ...
        do {
                ...
                if (c == BUFFILEEOF || (escaped == False && strMB == 1 &&
                        ...
                        if (lastWasNl == True) {
                                ptr = " ";
                                if (_DtHelpCeAddCharToBuf(&ptr, string,
                                                          &strSize, str_max,
                                                          32) == -1)
                                        return -1;
                                lastWasSpace = True;
                        ...
                        if (escaped == True) {
                                if (c == '#') {
                                        /*
                                         * process the SGML numeric character
                                         * reference entity.  '&#d[d[d]][;]' -
                                         * 'ddd' represent characters of '0' to
                                         * '9'.  The semi-colon is required iff
                                         * the next character is a numeric
                                         * character of '0' to '9'.  Otherwise
                                         * it is optional.
                                         */
                                        int value;

                                        len =
                                            0; /* len should be zero here, but
                                                  let's make sure */
                                        do {
                                                /*
                                        ...
                                        } while (len < ESC_STR_LEN &&
                                                 c >= '0' && c <= '9');

                                        /*
                                         * the ending character of the entity
                                         * has to be a non-numeric character.
                                         */
                                        if (c >= '0' && c <= '9')
                                                return -1;
                                        /*
                                         * if the optional semi-colon is not
                                         * present, replace the terminating
                                         * character back in the input buffer to
                                        ...
                                        /*
                                         * get the integer value. It has to be
                                         * less than 256.
                                         */
                                        value = atoi(escStr);
                                        if (value > 255)
                                        ...
                                        /*
                                         * transfer the value to the character
                                         * variable and zero the index for the
                                         * general buffer.
                                         */
                                        c = value;
                                        len = 0;
                                ...
                                /*
                                 * clear the escaped flag, so that the character
                                 * gets put in the output buffer.
                                 */
                                escaped = False;
                        } else if (c == '\n') {
                        ...
                        if (escaped == False && ignore == False) {
                                ...
                                /*
                                 * as long as we aren't processing a non break
                        ...
                        if (ignore == False) {
                                ...
                                ptr = buf;
                                if (_DtHelpCeAddStrToBuf(&ptr, string, &strSize,
                                                         str_max, len,
                                                         32) == -1)
                                        return -1;
                        }
 ...
 *
 *****************************************************************************/
int _DtHelpCeGetSdlAttributeCdata(BufFilePtr f, _DtCvValue limit,
                                  char **string) {
        _DtCvValue ignore = False;
        _DtCvValue done = False;
        ...
        do {
                ...
                if (firstChar) {
                        ...
                        if (limit == True && strSize >= DTD_NAMELEN) {
                                ...
                                if (strSize == 0)
                                        myStr =
                                            (char *)malloc(sizeof(char) * 65);
                                else
                                        myStr = (char *)realloc(
                                            myStr,
                                            (sizeof(char) * (strMax + 1)));
 ...
 *
 *****************************************************************************/
int _DtHelpCeSkipCdata(BufFilePtr f, _DtCvValue flag) {
        _DtCvValue done = False;
 ...
 *
 *****************************************************************************/
int _DtHelpCeGetSdlId(BufFilePtr f, char **id_value) {
        *id_value = NULL;
 ...
 *
 *****************************************************************************/
int _DtHelpCeGetSdlNumber(BufFilePtr f, char *number_str) {
 ...
 *
 *****************************************************************************/
int _DtHelpCeFindSkipSdlElementEnd(BufFilePtr f) {
        char c;
 ...
 *
 ******************************************************************************/
void _DtHelpCeMergeSdlAttribInfo(_DtCvSegment *src_seg, _DtCvSegment *dst_seg,
                                 _DtHelpFontHints *dst_fonts, void *el_info,
                        ...
                        if ((pAttr->struct_type == SdlFontSpecific ||
                            ...
                            !(ATTRS_EQUAL(pAttr->data_type,
                              ...
                              ATTRS_EQUAL(pAttr->data_type,
                                ...
                                if (pAttr->struct_type == SdlFontSpecific) {
                                        src = ((
                                            char *)(&(_SdlTossInfoPtrFontSpecs(
                                            tossInfo))));
                                        dst = ((char *)dst_fonts);
                                } else /* if (pAttr->struct_type ==
                                          SdlContainerSpecific) */
                                {
                                        src = ((char *)_SdlContainerPtrOfSeg(
                                            src_seg));
                                        dst = ((char *)_SdlContainerPtrOfSeg(
                                            dst_seg));
                                }

                                switch (
                                    SDLAttrFlagType(pAttr->sdl_attr_define)) {
                                case ENUM_VALUE:
                                        dstEnum =
                                            (SdlOption *)(((char *)dst) +
                                                          pAttr->field_ptr);
                                ...
                                case STRING1_VALUE:
                                case STRING2_VALUE:
                                        dstStr = (SDLCdata *)(((char *)dst) +
                                                              pAttr->field_ptr);
                                        srcStr = (SDLCdata *)(((char *)src) +
                                                              pAttr->field_ptr);
                                        *dstStr = *srcStr;
                                        break;
                                }
 ...
 *
 ******************************************************************************/
void _DtHelpCeAddSegToList(_DtCvSegment *seg_ptr, _DtCvSegment **seg_list,
                           _DtCvSegment **last_seg) {
        if (*seg_list == NULL)
                *seg_list = seg_ptr;
        else
 ...
 *
 ******************************************************************************/
int _DtHelpCeAllocSegment(int malloc_size, int *alloc_size,
                          _DtCvSegment **next_seg, _DtCvSegment **seg_ptr) {
        int cnt = 0;
 ...
 *
 ******************************************************************************/
int _DtHelpCeSaveString(_DtCvPointer client_data, _DtCvSegment **seg_list,
                        _DtCvSegment **last_seg, _DtCvSegment **prev_seg,
                        char *string, _DtHelpFontHints *font_specs,
                        int link_index, int multi_len, int flags,
                        void (*load_font)(), _SdlFontMode resolve_font,
                        _DtCvValue newline) {
        ...
        if (*ptr == '\0') {
                if (newline == True) {
                        if (_DtHelpCeAllocSegment(1, &junk, &junkSeg, &pSeg) !=
                            0)
                                return -1;
                        addToList = True;
                }
                ...
                while (*ptr != '\0') {
                        ...
                        if (_SdlFontModeSave == resolve_font) {
                                fontSpec = (_DtHelpFontHints *)malloc(
                                    sizeof(_DtHelpFontHints));
                                if (NULL == fontSpec) {
                                        free(pSeg);
                                        return -1;
                                }
                        ...
                        if (multi_len != 1) {
                                ...
                                if (1 != len)
                                        pSeg->type =
                                            _DtCvSetTypeToWideChar(pSeg->type);
                        } else
                                size = strlen(ptr);
                        ...
                        if (link_index != -1) {
                                pSeg->type =
                                    _DtCvSetTypeToHyperText(pSeg->type);
                        ...
                        if (_DtCvIsSegWideChar(pSeg)) {
                                ...
                                /*
                                 * include the terminating byte in the
                                 * conversion.
                                 */
                                numChars++;
                                 ...
                                 * malloc the memory
                                 */
                                pwcs = (wchar_t *)malloc(sizeof(wchar_t) *
                                                         numChars);
                                if (NULL != pwcs) {
                                        /* convert */
                                        convSize = mbstowcs(pwcs, string,
                                                            ((size_t)numChars));

                                        /* check to see if it converted
                                         * everything */
 ...
 *
 *****************************************************************************/
_DtCvSegment *_DtHelpCeMatchSemanticStyle(_DtCvSegment *toss, SdlOption clan,
                                          int level, char *ssi) {
        _DtCvSegment *pSeg;
        SDLTossInfo *pEl;
        ...
        do {
                 ...
                 */
                while (
                    pClassStyle->start != SdlOptionBad &&
                    !(pClassStyle->start <= clan && clan <= pClassStyle->end))
                        pClassStyle++;

                if (pClassStyle->start != SdlOptionBad) {
                        /*
                         * skip those not matching the style this class belongs
                         * to. But don't go past the ones after, because they
                         * might match too.
                         */
                        while (
                            pSeg != NULL &&
                        ...
                        /*
                         * the class matches the style, look for a specific
                         * class, ssi and level match.
                         */
                        while (pSeg != NULL && _SdlSegPtrToTossType(pSeg) ==
                                                   pClassStyle->style) {
...
/******************************************************************************
 * Function:    int _DtHelpCeAllocateFontStruct (int num, _DtHelpFontHints
 **ret_ptr)
 *
 * Parameters:
 *
 ...
 *
 *****************************************************************************/
int _DtHelpCeAllocateFontStruct(int size, _DtHelpFontHints **ret_ptr) {
