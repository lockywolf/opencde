...
static void CloseOnHelpCB(Widget w, XtPointer client_data, XtPointer call_data);
static void SetupHelpDialog(Widget parent, char *helpOnHelpVolume,
                            Widget *helpWidget, char *locationId);
 ...
 *
 ****************************************************************************/
static void CloseOnHelpCB(Widget w, XtPointer client_data,
                          XtPointer call_data) {
 ...
 *
 ***************************************************************************/
void _DtHelpCB(Widget widget, XtPointer clientData, XtPointer callData) {
        char *errorStr;
        ...
        if (help->helpOnHelpVolume != NULL) {
                SetupHelpDialog((Widget)pHelpInfo->widget,
                                help->helpOnHelpVolume, &(help->onHelpDialog),
                                pHelpInfo->locationId);
        } else {
                /* We have no help available for the help stuff so generate an
                 * error */
                /* Do NOT EVER provide a help button */
                /* ??? ERROR */
                errorStr = XtNewString(((char *)_DTGETMESSAGE(
                    6, 4, "Help On Help not provided for this application!")));
 ...
 *****************************************************************************/
static void SetupHelpDialog(Widget parent, char *helpOnHelpVolume,
                            Widget *helpDialog, char *locationId) {
        Arg args[10];
        int n;
        ...
        if (*helpDialog == NULL) {
                ...
                title =
                    XtNewString(((char *)_DTGETMESSAGE(6, 1, "Help On Help")));
                printString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(6, 2, "Print ...")));
                closeString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(6, 3, "Close")));
                backString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(6, 5, "Backtrack")));

                helpString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(6, 6, "Help ...")));
                ...
                n++;
                XtSetArg(args[n], DtNhelpLabelString, helpString);
                n++;
                XtSetArg(args[n], DtNhelpVolume, helpOnHelpVolume);
                n++;
                ...
                *helpDialog = DtCreateHelpQuickDialog((Widget)parent,
                ...
                XtAddCallback(*helpDialog, DtNcloseCallback, CloseOnHelpCB,
                              (XtPointer)NULL);
 ...
 *
 *****************************************************************************/
void _DtHelpListFree(DtHelpListStruct **pHead) {
 ...
 *
 *****************************************************************************/
DtHelpListStruct *_DtHelpListAdd(char *locationId, Widget widget,
        ...
        if (*pHead == NULL) {
                *pHead =
                    (DtHelpListStruct *)XtMalloc((sizeof(DtHelpListStruct)));
        ...
        } else { /* We have a list so add the new Help item to the top */
                pTemp =
                    (DtHelpListStruct *)XtMalloc((sizeof(DtHelpListStruct)));
