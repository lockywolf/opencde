...
Boolean _DtHelpSetScrollBars(XtPointer client_data, Dimension new_width,
        ...
        do {
                ...
                if (new_height != pDAS->dispHeight) {
                        ...
                        pDAS->dispUseHeight =
                            new_height - 2 * pDAS->decorThickness;
                        if (((int)new_height) < 2 * ((int)pDAS->decorThickness))
                                pDAS->dispUseHeight = 0;

                        pDAS->visibleCount =
                            ((int)pDAS->dispUseHeight) / pDAS->lineHeight;
                ...
                if (new_width != pDAS->dispWidth) {
                        ...
                        pDAS->dispUseWidth =
                            new_width - 2 * pDAS->decorThickness;
                ...
                if (NULL != pDAS->vertScrollWid && NeedVertScrollbar(pDAS)) {
                        ...
                        if (pDAS->maxYpos <= ((int)pDAS->dispUseHeight)) {
                                /*
                                 * If the window has not already been
                                 * 'stretched' by the width of the scrollbar and
                                 * if the scrollbar is mapped, grow the window
                                 * width.
                                 */
                                if (0 == changeWidth && pDAS->vertIsMapped) {
                                        /*
                                         * set the flag to indicate that the
                                         * window width has grown. This can
                                         * occur only if the window has its
                                         * vertical scrollbar already mapped.
                                         */
                                        changeWidth = 1;
                                        new_width += vertWidth;
                                }
                                /*
                                 * Else check to see if the window has been
                                 * shrunk. If so, then stretch it back out to
                                 * its original width.
                                 *
                                 * This can occur only if
                                 *    a) The vertical scrollbar is not mapped.
                                 *    b) The previous pass through this code
                                 * indicates that the window could not hold the
                                 * length of the information, and
                                 * therefore calculations and flags were
                                 * set to map the vertical scrollbar. But
                                 * a _DtCanvasResize() now indicates that
                                 * a vertical scrollbar is not needed.  So we
                                 * don't need the vertical scrollbar and
                                 * have to nuliify the calculation to
                                 * include it.
                                 */
                         ...
                         */
                                 ...
                                 * accomidate the vertical scrollbar (and the
                                 * scrollbar is not already mapped) then set the
                                 * flags and shrink the width.
                                 */
                                if (False == pDAS->vertIsMapped &&
                                    0 == changeWidth) {
                                        /*
                                         * set the flag to indicate that the
                                         * window width has shrunk (and that
                                         * consequently, the vertical scrollbar
                                         * is not mapped).
                                         */
                                        changeWidth = -1;
                                ...
                                /*
                                 * Else check to see if the window has been
                                 * streched. If so, then shrink it back out to
                                 * its original width.
                                 *
                                 * This can occur only if
                                 *    a) The vertical scrollbar is mapped.
                                 *    b) The previous pass through this code
                                 * indicates that the window could hold the
                                 * length of the information, and
                                 * therefore calculations and flags were
                                 * set to unmap the vertical scrollbar.
                                 *       But a _DtCanvasResize() now indicates
                                 * that a vertical scrollbar is needed.  So we
                                 * need the vertical scrollbar and have to
                                 * nuliify the calculation to eliminate
                                 * it.
                                 */
                                else if (1 == changeWidth) {
                                        changeWidth = 0;
                ...
                if (NULL != pDAS->horzScrollWid && NeedHorzScrollbar(pDAS)) {
                        ...
                        if (pDAS->maxX <= ((int)pDAS->dispUseWidth)) {
                                /*
                                 * If the window has not already been
                                 * 'stretched' by the height of the scrollbar
                                 * and if the scrollbar is mapped, grow the
                                 * window.
                                 */
                                if (0 == changeHeight && pDAS->horzIsMapped) {
                                        /*
                                         * set the flag to indicate that the
                                         * window has grown. This can occur only
                                         * if the window has its horizontal
                                         * scrollbar already mapped.
                                         */
                                        changeHeight = 1;
                                ...
                                /*
                                 * Else check to see if the window has been
                                 * shrunk. If so, stretch it back to its
                                 * original height.
                                 *
                                 * This can occur only if
                                 *   a) The horizontal scrollbar is not mapped.
                                 *   b) The previous pass through the code
                                 * indicated that the information exceeded the
                                 * right border and therefore calculations
                                 * and flags were set to map the horizontal
                                 * scrollbar. But a _DtCanvasResize() now
                                 * indicates that the horizontal scrollbar
                                 * is not needed. So we have to nullify the
                                 * calculations and flags set to include
                                 * it.
                                 */
                        ...
                        /*
                         * The information exceeds the right border of the
                         * window.
                         */
                        else {
                                /*
                                 ...
                                 * scrollbar is not already mapped), then set
                                 * the flags and shrink the height.
                                 */
                                if (False == pDAS->horzIsMapped &&
                                    0 == changeHeight) {
                                        /*
                                         * set the flag to indicate that the
                                         * window height has shrunk (and that
                                         * the scrollbar needs mapping).
                                         */
                                        changeHeight = -1;
                                 ...
                                 *   b) The previous pass through the code
                                 * indicated that the information did not
                                 * exceeded the right border and therefore
                                 * calculations and flags were set to unmap
                                 * the horizontal scrollbar. But a
                                 *      _DtCanvasResize() now indicates that the
                                 *      horizontal scrollbar is needed. So we
                                 *      have to nullify the calculations and
                                 * flags set to exclude it.
                                 */
                                else if (1 == changeHeight) {
                                        changeHeight = 0;
                                        new_height -= horzHeight;
        ...
        if (oldWidth != pDAS->dispWidth || oldHeight != pDAS->dispHeight) {
                ...
                XSetClipRectangles(XtDisplay(pDAS->dispWid), pDAS->normalGC, 0,
                                   0, rectangle, 1, Unsorted);
                XSetClipRectangles(XtDisplay(pDAS->dispWid), pDAS->invertGC, 0,
                                   0, rectangle, 1, Unsorted);
        ...
        if (pDAS->vertScrollWid && pDAS->vertIsMapped && doVert != DoUnmanage) {
                if (pDAS->horzScrollWid && pDAS->horzIsMapped &&
                    doHorz != DoUnmanage)
                        XtSetArg(args[0], XmNbottomOffset,
                                 (marginHeight + horzHeight));
                else
        ...
        if (pDAS->horzScrollWid && pDAS->horzIsMapped && doHorz != DoUnmanage) {
                if (pDAS->vertScrollWid && pDAS->vertIsMapped &&
                    doVert != DoUnmanage)
                        XtSetArg(args[0], XmNrightOffset,
                                 (marginWidth + vertWidth));
                else
...
void _DtHelpDisplayAreaSetList(XtPointer client_data, XtPointer topic_handle,
        ...
        _DtCanvasSetTopic(pDAS->canvas, (_DtCvTopicPtr)topic_handle,
                          pDAS->honor_size, &width, &height, &scrollY);
        ...
        (void)_DtHelpSetScrollBars(client_data, pDAS->dispWidth,
                                   pDAS->dispHeight);
        if (XtIsRealized(pDAS->dispWid))
...
int _DtHelpUpdatePath(DtHelpDispAreaStruct *pDAS,
         ...
         */
        result = _DtHelpFormatToc(pDAS, volume_handle, loc_id, &realId,
                                  &topic_handle);
        if (-1 == result)
                return -1;
         ...
         */
        if (1 != result) {
                (void)_DtCanvasMoveTraversal(
                    pDAS->canvas, _DtCvTRAVERSAL_ID, False,
                    ((_DtCvStatus)XtIsRealized(pDAS->dispWid)), realId, NULL,
                    &(pDAS->toc_y), &(pDAS->toc_base), &(pDAS->toc_height));
