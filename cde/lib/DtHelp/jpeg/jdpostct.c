...
start_pass_dpost(j_decompress_ptr cinfo, J_BUF_MODE pass_mode) {
        ...
        case JBUF_PASS_THRU:
                if (cinfo->quantize_colors) {
                         ...
                         * starting a 2-pass color quantization; in that case,
                         * jinit_d_post_controller did not allocate a strip
                         * buffer.  Use the virtual-array buffer as workspace.
                         */
                        if (post->buffer == NULL) {
                                post->buffer =
...
jinit_d_post_controller(j_decompress_ptr cinfo, boolean need_full_buffer) {
        ...
        if (cinfo->quantize_colors) {
                 ...
                 * typically an efficient number of rows for upsampling to
                 * return. (In the presence of output rescaling, we might want
                 * to be smarter?)
                 */
