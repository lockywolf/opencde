        ...
        if (length >= JFIF_LEN) {
                ...
                if (b[0] == 0x4A && b[1] == 0x46 && b[2] == 0x49 &&
                    b[3] == 0x46 && b[4] == 0) {
                         ...
                         * incompatible change. We used to treat this as an
                         * error, but now it's a nonfatal warning, because some
                         * bozo at Hijaak couldn't read the spec. Minor version
                         * should be 0..2, but process anyway if newer.
                         */
                        if (b[5] != 1)
                        ...
                        TRACEMS3(cinfo, 1, JTRC_JFIF, cinfo->X_density,
                                 cinfo->Y_density, cinfo->density_unit);
                        if (b[12] | b[13])
                                TRACEMS2(cinfo, 1, JTRC_JFIF_THUMBNAIL, b[12],
                                         b[13]);
...
next_marker(j_decompress_ptr cinfo) {
        ...
        for (;;) {
                 ...
                 * valid file. We sync after each discarded byte so that a
                 * suspending data source can discard the byte from its buffer.
                 */
                while (c != 0xFF) {
                 ...
                 * legal as pad bytes, so don't count them in discarded_bytes.
                 * We assume there will not be so many consecutive FF bytes as
                 * to overflow a suspending data source's input buffer.
                 */
                do {
                        INPUT_BYTE(cinfo, c, return FALSE);
                } while (c == 0xFF);
...
read_markers(j_decompress_ptr cinfo) {
        ...
        for (;;) {
                 ...
                 * and the input point is just past the marker proper, but
                 * before any parameters. A suspension will cause us to return
                 * with this state still true.
                 */
                ...
                default: /* must be DHP, EXP, JPGn, or RESn */
                         ...
                         * errors since they are likely to be used to signal
                         * incompatible JPEG Part 3 extensions. Once the JPEG 3
                         * version-number marker is well defined, this code
                         * ought to change!
                         */
