 ...
 *
 *****************************************************************************/
void _DtHelpBuildPathArea(Widget parent, DtHelpDialogWidget nw) {
        ...
        labelString =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(9, 1, "Volume: ")));
        n = 0;
        ...
        n++;
        pathLabel = XmCreateLabelGadget(_pathForm, "pathLabel", args, n);
        ...
        n++;
        XtSetArg(args[n], XmNleftOffset, 1);
        n++;
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNtopOffset, 0);
        ...
        /* Build a container for our toc to sit in.  We do this because we
         * cannot set form constraints on our toc, but can on the container.
         */
        ...
        XtSetArg(args[n], XmNtopOffset, 0);
        n++;
        XtSetArg(args[n], XmNtopWidget, _pathForm);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNleftWidget, tocFrame);
        n++;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, tocFrame);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNentryVerticalAlignment, XmALIGNMENT_CENTER);
        n++;
        ...
        /* Backtrack button */
        labelString =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(9, 2, "Backtrack")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        hw->help_dialog.browser.btnBoxBackBtn =
            XmCreatePushButtonGadget(btnBox, "btnBoxBackBtn", args, n);
        XtManageChild(hw->help_dialog.browser.btnBoxBackBtn);
        XmStringFree(labelString);
        XtAddCallback(hw->help_dialog.browser.btnBoxBackBtn,
                      XmNactivateCallback, _DtHelpDisplayBackCB, (XtPointer)hw);
        XtSetSensitive(hw->help_dialog.browser.btnBoxBackBtn, FALSE);

        /* History Button */
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(9, 3, "History...")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        hw->help_dialog.browser.btnBoxHistoryBtn =
            XmCreatePushButtonGadget(btnBox, "btnBoxHistoryBtn", args, n);
        XtManageChild(hw->help_dialog.browser.btnBoxHistoryBtn);
        XmStringFree(labelString);
        XtAddCallback(hw->help_dialog.browser.btnBoxHistoryBtn,
                      XmNactivateCallback, _DtHelpDisplayHistoryCB,
                      (XtPointer)hw);
        ...
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        hw->help_dialog.browser.btnBoxIndexBtn =
            XmCreatePushButtonGadget(btnBox, "btnBoxIndexBtn", args, n);
        XtManageChild(hw->help_dialog.browser.btnBoxIndexBtn);
        XmStringFree(labelString);
        XtAddCallback(hw->help_dialog.browser.btnBoxIndexBtn,
                      XmNactivateCallback, _DtHelpDisplayIndexCB,
                      (XtPointer)hw);
        ...
        if (hw->help_dialog.browser.showTopLevelBtn == TRUE) {
                labelString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(9, 5, "Top Level")));
                n = 0;
                XtSetArg(args[n], XmNlabelString, labelString);
                n++;
                hw->help_dialog.browser.btnBoxTopLevelBtn =
                    XmCreatePushButtonGadget(btnBox, "btnBoxTopLevelBtn", args,
                                             n);
                XtManageChild(hw->help_dialog.browser.btnBoxTopLevelBtn);
                XmStringFree(labelString);
                XtAddCallback(hw->help_dialog.browser.btnBoxTopLevelBtn,
                              XmNactivateCallback, _DtHelpDisplayBrowserHomeCB,
                              (XtPointer)hw);
        }
        ...
        hw->help_dialog.browser.pTocArea = _DtHelpCreateDisplayArea(
            tocFrame, "TocArea",
            ((short)hw->help_dialog.display.scrollBarPolicy),
            ((short)hw->help_dialog.display.scrollBarPolicy), True,
            ((int)hw->help_dialog.browser.visiblePathCount), 1,
            _DtHelpDialogHypertextCB, NULL /*ResizeHelpDialogCB*/,
        ...
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
        n++;
        XtSetArg(args[n], XmNrightOffset, maxBtnWidth + 10);
        n++;
...
void _DtHelpUpdatePathArea(char *locationId, DtHelpDialogWidget nw) {
        ...
        if (locationId != NULL) {
                retState = _DtHelpUpdatePath(
                    hw->help_dialog.browser.pTocArea,
                ...
                /* We have most likely entered an error state and must set our
                 * path area to NULL values while we display and error message
                 * in the display area
                 */
                _DtHelpDisplayAreaClean(hw->help_dialog.browser.pTocArea);

                /* We set our volumeFlag back to false so we are sure to create
                 * a new toc next time we enter this function (i.e. recovering
                 * from our error state.
                 */
