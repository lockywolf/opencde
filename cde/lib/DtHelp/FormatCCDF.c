...
enum _processState { NormalState, LabelBody };
 ...
 ******************************************************************************/
static _DtCvSegment *AllocateSegments(_DtCvSegment *p_seg, int original_size,
                                      int new_size);
static int AppendCharToInfo(FormatVariables *cur_vars, char **src);
static int AppendOctalToInfo(FormatVariables *cur_vars, char *src);
static int AppendSpaceToInfo(FormatVariables *cur_vars, char **src,
                             _DtCvFrmtOption type);
static int AppendToInfo(FormatVariables *cur_vars, char **src,
                        const char *scan_string);
static int ChangeFont(int whichOne, unsigned long seg_flags,
                      _DtCvFrmtOption frmt_type, _DtHelpFontHints *font_attr,
                      int linkspec, FormatVariables *cur_vars,
                      ProcessState cur_state, int flags, int ret_on_nl,
                      int fnt_flag);
static int CheckIdString(FormatVariables *cur_vars, ProcessState cur_state,
                         unsigned long seg_flags, _DtCvFrmtOption frmt_type,
                         _DtHelpFontHints *font_attr, int linkspec, int flags,
                         int ret_on_nl, int fnt_flag);
static int CheckSaveSegment(unsigned long seg_flags,
                            _DtHelpFontHints *font_attr, int link_spec,
                            FormatVariables *cur_vars);
static int CreateSaveGraphic(FormatVariables *cur_vars, int type,
                             char *file_name, int link_spec);
static int CheckSegList(FormatVariables *cur_vars);
static int Format(FormatVariables *cur_vars, _FrmtUiInfo *ui_info,
                  char *id_string, _DtCvTopicPtr *ret_handle);
static int GetNextBuffer(FormatVariables *cur_vars);
static int GetStringParameter(FormatVariables *cur_vars, _DtCvValue flag,
                              _DtCvValue eat_escape, _DtCvValue ignore_quotes,
                              _DtCvValue less_test, char **ret_string);
static int GetValueParameter(FormatVariables *cur_vars, _DtCvValue flag,
                             int *ret_value);
static int InitStructure(FormatVariables *cur_vars, _FrmtUiInfo *ui_info,
                         char *rd_buf, int rd_size);
static int Parse(int cur_cmd, ProcessState cur_state, FormatVariables *cur_vars,
                 unsigned long seg_flags, _DtCvFrmtOption frmt_type,
                 _DtHelpFontHints *font_attr, int linkspec,
                 const char *scan_string, int allowed, int ret_on_nl,
                 int fnt_flag);
static int ProcessFigureCmd(FormatVariables *cur_vars, ProcessState cur_state,
                            _DtCvFrmtOption frmt_type,
                            _DtHelpFontHints *font_attr, int ret_on_nl,
                            int fnt_flag);
static int ProcessHypertext(FormatVariables *cur_vars, ProcessState cur_state,
                            unsigned long seg_flags, _DtCvFrmtOption frmt_type,
                            _DtHelpFontHints *font_attr, int flags,
                            int ret_on_nl, int fnt_flag);
static int ProcessInLine(FormatVariables *cur_vars, int seg_type,
                         int link_spec);
static int ProcessLabelCmd(FormatVariables *cur_vars, unsigned long seg_flags,
                           _DtCvFrmtOption frmt_type,
                           _DtHelpFontHints *font_attr, int link_spec,
                           int flags, int ret_on_nl, int fnt_flag,
                           _DtCvSegment **ret_list);
static int ProcessParagraph(FormatVariables *cur_vars, ProcessState cur_state,
                            int fnt_flag);
static int SaveNewLine(FormatVariables *cur_vars, int cur_cmd,
                       _DtHelpFontHints *font_attr, int link_spec);
static int SaveStringAsSegments(FormatVariables *cur_vars,
                                unsigned long seg_flags,
                                _DtHelpFontHints *font_attr, int link_spec);
static int SkipToNextToken(FormatVariables *cur_vars, _DtCvValue flag);
static void TerminateSegList(SegList *list, int flag);
static int SegmentSave(FormatVariables *cur_vars, unsigned long seg_type,
 ...
 *
 *****************************************************************************/
static int CreateMarker(FormatVariables *cur_vars, char *id_string) {
        int result;
 ...
 *
 *****************************************************************************/
static void GetMbLen(FormatVariables *cur_vars, char *font_str, char **lang_ptr,
                     char **set_ptr) {
        int i;
        char *ptr;
        char *langPart = NULL;
         ...
         */
                ...
                _DtHelpCeXlateOpToStdLocale(DtLCX_OPER_CCDF, font_str, NULL,
                                            &langPart, &codePart);
                /*
                ...
                if (strcmp(langPart, "?") == 0) {
                        ...
                        else if (_DtHelpCeStrrchr(langPart, "_", MB_CUR_MAX,
                                                  &ptr) == 0)
                                *ptr = '\0';
                        else if (_DtHelpCeStrrchr(langPart, Period, MB_CUR_MAX,
                                                  &ptr) == 0)
                                *ptr = '\0';
                }
        }
 ...
 * Purpose:
 *****************************************************************************/
static int ReturnLinkWinHint(int ccdf_type) {
 ...
 * Purpose:
 *****************************************************************************/
static int ReturnLinkType(int ccdf_type, char *spec) {
...
static int CheckSegList(FormatVariables *cur_vars) {
        if (cur_vars->my_list.cnt >= cur_vars->my_list.max) {
                cur_vars->my_list.list = AllocateSegments(
                    cur_vars->my_list.list, cur_vars->my_list.max,
...
static _DtCvSegment *AllocateSegments(_DtCvSegment *p_seg, int original_size,
        ...
        if (NULL != p_seg) {
                 ...
                 */
                p_seg = (_DtCvSegment *)realloc(
                    (void *)p_seg, (sizeof(_DtCvSegment) * new_size));
                /*
                 * reallocate the private formatting information.
                 */
                priv = p_seg->client_use;
                priv = (FrmtPrivateInfo *)realloc(
                    (void *)priv, sizeof(FrmtPrivateInfo) * new_size);
                if (NULL == priv)
                        return NULL;
 ...
 *
 *****************************************************************************/
static int CheckList(SegList *seg_list, int count, int grow) {
        /*
        ...
        if (count > seg_list->max) {
                 ...
                 */
                seg_list->list =
                    AllocateSegments(seg_list->list, seg_list->max, grow);
        ...
        if (str_size) {
                ...
                if (seg_type & _DtCvWIDE_CHAR) {
                        ...
                        if (NULL != pwcs) {
                                /* convert */
                                str_size =
                                    mbstowcs(pwcs, string, ((size_t)num_chars));
                ...
                if (NULL != cur_vars->ui_info->load_font)
                        (*(cur_vars->ui_info->load_font))(
                            ...
                            _DtHelpFontHintsCharSet(*font_attr), *font_attr,
...
/******************************************************************************
 * Function: int SaveStringAsSegments (FormatVariables, int type, char
...
static int SaveStringAsSegments(FormatVariables *cur_vars,
        ...
        if (cur_vars->cur_mb_max != 1 && cur_vars->fmt_size > 1) {
                ...
                do {
                        ...
                        /*
                         * How many of the following characters are of the same
                         * size?
                         */
                         ...
                         * and set to wide char if not doing multi-byte
                         * sequence.
                         */
                        ptr[size] = '\0';
                        if (1 != len)
                        ...
                        if (CheckSegList(cur_vars) == -1 ||
                            SegmentSave(cur_vars, seg_flags, font_attr, ptr,
                                        size, numChars, link_spec) == -1)
                                return -1;
...
static int CheckSaveSegment(unsigned long seg_flags,
        ...
        if (cur_vars->fmt_size)
                result = SaveStringAsSegments(cur_vars, seg_flags, font_attr,
                                              link_spec);
 ...
 *
 *****************************************************************************/
static void TerminateSegList(SegList *seg_list, int flag) {
        int count;
...
static int AppendToInfo(FormatVariables *cur_vars, char **src,
        ...
        spnResult =
            _DtHelpCeStrcspn(*src, scan_string, cur_vars->cur_mb_max, &size);
        if (0 < size) {
                if (_DtHelpCeAddStrToBuf(
                        src, &(cur_vars->fmt_buf), &(cur_vars->fmt_size),
...
static int AppendOctalToInfo(FormatVariables *cur_vars, char *src) {
        ...
        if (_DtHelpCeAddOctalToBuf(src, &(cur_vars->fmt_buf),
                                   &(cur_vars->fmt_size),
...
static int AppendSpaceToInfo(FormatVariables *cur_vars, char **src,
        ...
        if (!cur_vars->last_was_space || _DtCvLITERAL == type) {
                ...
                result = _DtHelpCeAddCharToBuf(
                    &space, &(cur_vars->fmt_buf), &(cur_vars->fmt_size),
                    &(cur_vars->fmt_buf_max), INFO_GROW);
 ...
 *
 *****************************************************************************/
static int FindEndMarker(FormatVariables *cur_vars) {
...
static int GetNextBuffer(FormatVariables *cur_vars) {
        cur_vars->rd_flag =
            _DtHelpCeGetNxtBuf(cur_vars->my_file, cur_vars->rd_buf,
                               &(cur_vars->rd_ptr), cur_vars->rd_size);
 ...
 *
 *****************************************************************************/
static int SaveNewLine(FormatVariables *cur_vars, int cur_type,
                       _DtHelpFontHints *font_attr, int link_spec) {
        int result = 0;
        _DtCvSegment *pSeg;
         ...
         */
        if (cur_vars->fmt_size)
                result = SaveStringAsSegments(cur_vars,
                                              _DtCvSetTypeToNewLine(cur_type),
                                              font_attr, link_spec);
        /*
         ...
         */
                ...
                result = SaveStringAsSegments(cur_vars,
                                              _DtCvSetTypeToNewLine(_DtCvNOOP),
                                              font_attr, link_spec);
        /*
         ...
         */
                ...
                if (_DtCvIsSegNewLine(pSeg))
                        result = SaveStringAsSegments(
                            cur_vars, _DtCvSetTypeToNewLine(_DtCvNOOP),
                            font_attr, link_spec);
                else
...
static int CreateSaveGraphic(FormatVariables *cur_vars, int type,
        ...
        if (*file_name == '/')
                ...
                int fullname_size =
                    strlen(cur_vars->my_path) + strlen(file_name) + 2;
        ...
        if (NULL != cur_vars->ui_info->load_graphic &&
            (*(cur_vars->ui_info->load_graphic))(
                cur_vars->ui_info->client_data, cur_vars->vol_name,
                cur_vars->topic_id, fullName, NULL, NULL,
...
static int ChangeFont(int whichOne, unsigned long seg_flags,
        ...
        if (!result)
                result = Parse(CCDF_FONT_CMD, cur_state, cur_vars, seg_flags,
                               frmt_type, font_attr, linkspec, Specials, flags,
                               ret_on_nl, fnt_flag);
        /*
 ...
 *****************************************************************************/
static int SkipToNextToken(FormatVariables *cur_vars, _DtCvValue flag) {
        return (_DtHelpCeSkipToNextCcdfToken(
            cur_vars->my_file, cur_vars->rd_buf, cur_vars->rd_size, 1,
            &(cur_vars->rd_ptr), flag));
}
...
static int GetStringParameter(FormatVariables *cur_vars, _DtCvValue flag,
                              _DtCvValue eat_escape, _DtCvValue ignore_quotes,
                              _DtCvValue less_test, char **ret_string) {
        return (_DtHelpCeGetCcdfStrParam(
            cur_vars->my_file, cur_vars->rd_buf, cur_vars->rd_size,
            cur_vars->cur_mb_max, &(cur_vars->rd_ptr), flag, eat_escape,
            ignore_quotes, less_test, ret_string));
}
...
static int GetValueParameter(FormatVariables *cur_vars, _DtCvValue flag,
                             int *ret_value) {
        return (_DtHelpCeGetCcdfValueParam(
            cur_vars->my_file, cur_vars->rd_buf, cur_vars->rd_size,
            &(cur_vars->rd_ptr), flag, cur_vars->cur_mb_max, ret_value));
}
 ...
 *
 *****************************************************************************/
static int GetParagraphParameters(FormatVariables *cur_vars, _DtCvSegment *para,
                                  _DtCvFrmtOption *frmt_type,
                                  _DtCvFrmtOption *gpos, char **glink,
                                  int *glinktype, _DtCvUnit *gspace,
                                  char **ret_label, char **file_name,
                                  char **description) {
        int done = False;
        int result = 0;
        int optionCount = 0;
        ...
        *gspace = 0;
        ...
        while (False == done && result != -1) {
                 ...
                 */
                case 'a':
                        if (GetValueParameter(cur_vars, _DtCvFALSE, &value) ==
                            -1)
                                return -1;

                        _DtCvContainerBMarginOfSeg(para) = 0;
                        if (value > 0 && 0 < cur_vars->ui_info->line_height)
                                _DtCvContainerBMarginOfSeg(para) =
                                    value *
                ...
                case 'b':
                        ...
                        if (value > 0 && 0 < cur_vars->ui_info->line_height)
                                _DtCvContainerTMarginOfSeg(para) =
                                    value *
                                    (cur_vars->ui_info->line_height / 2);
                        break;
                ...
                case 'd':
                        result = GetStringParameter(cur_vars, _DtCvTRUE,
                                                    _DtCvTRUE, _DtCvFALSE,
                                                    _DtCvFALSE, description);
                        break;
                ...
                case 'g':
                         ...
                         */
                        case 'l':
                                result = GetStringParameter(
                                    cur_vars, _DtCvTRUE, _DtCvTRUE, _DtCvFALSE,
                                    _DtCvFALSE, glink);
                                break;
                        ...
                        case 'p':
                                result = GetStringParameter(
                                    cur_vars, _DtCvTRUE, _DtCvTRUE, _DtCvFALSE,
                                    _DtCvFALSE, &tmpString);

                                if (-1 != result &&
                                    _DtCvToLower(*tmpString) == 'r')
                         ...
                         */
                        case 's':
                                if (GetValueParameter(cur_vars, _DtCvFALSE,
                                                      &value) == -1 ||
                                    value < 0)
                                        return -1;
                ...
                case 'l':
                        ...
                        if (_DtCvToLower(*(cur_vars->rd_ptr)) == 'a') {
                                 ...
                                 * label
                                 */
                                if (GetStringParameter(cur_vars, _DtCvTRUE,
                                                       _DtCvFALSE, _DtCvFALSE,
                                                       _DtCvFALSE,
                                                       &tmpString) == -1)
                                        return -1;
         ...
         */
        _DtCvContainerFMarginOfSeg(para) =
            _DtCvContainerLMarginOfSeg(para) - _DtCvContainerFMarginOfSeg(para);
        if (-1 == result)
                optionCount = 0;
 ...
 *
 *****************************************************************************/
static int FlowingParagraph(FormatVariables *cur_vars, ProcessState cur_state,
                            _DtCvFrmtOption frmt_flags, int allowed,
                            int link_idx, int ret_on_nl, int fnt_flag,
                            _DtCvFrmtOption gpos, _DtCvUnit gspace,
                            char *file_name, _DtHelpFontHints *font_attr) {
        int type = 0;
        int result = -1;
        ...
        if (0 == CheckList(&(flowList), 2, 2)) {
                 ...
                 */
                if (0 ==
                    CreateSaveGraphic(cur_vars, type, file_name, link_idx)) {
                        /*
                         * so the first segment in cur_vars is a region.
                         * attach it to the graphic container.
                         */
                        _DtCvContainerListOfSeg(graphSeg) =
                            cur_vars->my_list.list;
                        ...
                        if (-1 != Parse(CCDF_PARAGRAPH_CMD, cur_state, cur_vars,
                                        0, frmt_flags, font_attr, -1, Specials,
                                        allowed, ret_on_nl, fnt_flag)) {
                                /*
                                 ...
                                 */
                                if (NULL != cur_vars->my_list.list) {
                                        _DtCvContainerListOfSeg(bodySeg) =
                                            cur_vars->my_list.list;
                                        flowList.cnt++;
                                }
                                result = 0;
...
static int ProcessParagraph(FormatVariables *cur_vars, ProcessState cur_state,
         ...
         */
        result = GetParagraphParameters(cur_vars, paraSeg, &frmtType, &gpos,
                                        &glinkSpec, &glinktype, &gspace, &label,
        ...
        if (-1 != result) {
                ...
                if (NULL != label) {
                        ...
                        result = Parse(CCDF_LABEL_CMD, NormalState, cur_vars, 0,
                                       frmtType, &fontAttrs, -1, Specials,
                                       (CCDF_FONT_CMD | CCDF_OCTAL_CMD), False,
                                       fnt_flag);

                        if (result != -1)
                                result = CheckSaveSegment(
                                    _DtCvSTRING, &fontAttrs, -1, cur_vars);
                 ...
                 */
                else if (0 == _DtHelpCeCheckNextCcdfCmd(
                                  "lab", cur_vars->my_file, cur_vars->rd_buf,
                                  cur_vars->rd_size, 1, &(cur_vars->rd_ptr))) {
                        result = ProcessLabelCmd(cur_vars, 0, frmtType,
                                                 &fontAttrs, -1, allowed, False,
                                                 fnt_flag, &labelSeg);
                        labelFnd = True;
                }
                ...
                if (-1 != result && True == labelFnd) {
                        ...
                        if (-1 != result) {
                                /*
                                 * set the top and bottom margins on both the
                                 * child containers (so they are the same) to
                                 * reflect the list's before/after values.
                                 */
                                /*
                                ...
                                col1->type =
                                    _DtCvSetTypeToContainer(col1->type);
                                ...
                                _DtCvContainerRMarginOfSeg(col1) =
                                    cur_vars->ui_info->avg_char;
                                tableList.cnt++;
                                ...
                                col2->type =
                                    _DtCvSetTypeToContainer(col2->type);
                                 ...
                                 */
                                snprintf(numChar, 16, "%d",
                                         cur_vars->cell_cnt++);
                                int col1_size =
                                    strlen("&CCDF_RES_") + strlen(numChar) + 1;
                                _DtCvContainerIdOfSeg(col1) =
                                    (char *)malloc(col1_size);
                                if (NULL != _DtCvContainerIdOfSeg(col1)) {
                                        /*
                                         * copy over the id and attach the list.
                                         */
                                        strlcpy(_DtCvContainerIdOfSeg(col1),
                                                "&CCDF_RES_", col1_size);
                                        strlcat(_DtCvContainerIdOfSeg(col1),
                                                numChar, col1_size);
                                        _DtCvContainerListOfSeg(col1) =
                                            labelSeg;
                                        ...
                                        snprintf(numChar, 16, "%d",
                                                 cur_vars->cell_cnt++);
                                        int col2_size = strlen("&CCDF_RES_") +
                                                        strlen(numChar) + 1;
                                        _DtCvContainerIdOfSeg(col2) =
                                            (char *)malloc(col2_size);
                                        if (NULL !=
                                            _DtCvContainerIdOfSeg(col2)) {
                                                strlcpy(
                                                    _DtCvContainerIdOfSeg(col2),
        ...
        if (-1 != result) {
                /*
                 * initialize the segment list for the children of the
                 * paragraph.
                 */
                cur_vars->my_list = InitList;
                ...
                if (NULL != glinkSpec) {
                        ...
                        if (NULL != fileName && strlen(glinkSpec)) {
                                linkIndex = _DtLinkDbAddLink(
                                    cur_vars->my_links, NULL, glinkSpec,
                                    ReturnLinkType(glinktype, glinkSpec),
                                    ReturnLinkWinHint(glinktype), description);
                ...
                if (0 == result) {
                        if (NULL != fileName)
                                result = FlowingParagraph(
                                    cur_vars, myState, frmtType, allowed,
                                    linkIndex, False, fnt_flag, gpos, gspace,
                                    fileName, &fontAttrs);
                        else {
                                result =
                                    Parse(CCDF_PARAGRAPH_CMD, myState, cur_vars,
                                          0, frmtType, &fontAttrs, -1, Specials,
                                          allowed, False, fnt_flag);
                                if (-1 != result) {
                                        /*
                                         * establish the links
                                         */
                                        TerminateSegList(&(cur_vars->my_list),
                                                         True);
                                }
                        }
                }
                ...
                if (-1 != result) {
                        ...
                        if (True == labelFnd) {
                                ...
                                char *colW2 =
                                    "99,0,98"; /* default bulletted list */
                                char *ids;

                                /*
                                 * set the body of the label in it's container.
                                 */
                                _DtCvContainerListOfSeg(col2) =
                                    cur_vars->my_list.list;

                                /*
                                 * figure out which column widths should be
                                 * used.
                                 */
                                 ...
                                 */
                                colW = NULL;
                                colW = (char **)_DtCvAddPtrToArray(
                                    (void **)colW, ((void *)strdup(colW1)));
                                if (NULL != colW)
                                        colW = (char **)_DtCvAddPtrToArray(
                                            (void **)colW,
                                            ((void *)strdup(colW2)));
                                 ...
                                 */
                                colJ = (_DtCvFrmtOption *)malloc(
                                    sizeof(_DtCvFrmtOption) * 2);
                                if (NULL != colJ) {
                                ...
                                /*
                                 * set this segment's type to a table and fill
                                ...
                                _DtCvCellIdsOfTableSeg(paraSeg) =
                                    (char **)_DtCvAddPtrToArray(
                                        (void **)_DtCvCellIdsOfTableSeg(
                                            paraSeg),
                                        (void *)ids);
                                ...
                                if (NULL != _DtCvCellsOfTableSeg(paraSeg))
                                        _DtCvCellsOfTableSeg(paraSeg) =
                                            (_DtCvSegment **)_DtCvAddPtrToArray(
                                                (void **)_DtCvCellsOfTableSeg(
                                                    paraSeg),
                                                (void *)col2);

                                if (NULL == colW || NULL == colJ ||
                                    NULL == ids ||
                                 ...
                                 */
                                _DtCvContainerListOfSeg(paraSeg) =
                                    cur_vars->my_list.list;

                                /*
                                 * consolidate any table children I have.
                                 */
                                for (pSeg = cur_vars->my_list.list,
                                    labelSeg = NULL;
                                     - 1 != result && NULL != pSeg;
                                     pSeg = pSeg->next_seg) {
...
static int ProcessHypertext(FormatVariables *cur_vars, ProcessState cur_state,
        ...
        if (0 == result) {
                 ...
                 */
                result =
                    GetStringParameter(cur_vars, _DtCvFALSE, _DtCvTRUE,
                                       _DtCvFALSE, _DtCvFALSE, &description);
                if (result == 1)
                        description = NULL;

                result =
                    _DtLinkDbAddLink(cur_vars->my_links, NULL, hyperlink,
                                     ReturnLinkType(hyperType, hyperlink),
                                     ReturnLinkWinHint(hyperType), description);
                if (NULL != description)
                        free(description);
        ...
        if (result != -1) {
                ...
                if (!result) {
                        ...
                        result =
                            Parse(CCDF_LINK_CMD, cur_state, cur_vars, seg_flags,
                                  frmt_type, font_attr, hyperIndex, Specials,
                                  flags, ret_on_nl, fnt_flag);
                }
        }
...
static int ProcessLabelCmd(FormatVariables *cur_vars, unsigned long seg_flags,
        ...
        cur_vars->my_list = InitList;
        if (Parse(CCDF_LABEL_CMD, NormalState, cur_vars, seg_flags, frmt_type,
                  font_attr, link_spec, Specials, flags, ret_on_nl,
                  fnt_flag) == -1)
                return -1;
...
static int ProcessFigureCmd(FormatVariables *cur_vars, ProcessState cur_state,
        ...
        while (!done && result != -1) {
                ...
                case 'c':
                        ...
                        if (result != -1) {
                                /*
                                 * look at the next charager to determine the
                                 * token.
                                 */
                                switch (_DtCvToLower(*(cur_vars->rd_ptr))) {
                ...
                case 'i':
                         ...
                         */
                        if (-1 != result && NULL != idString &&
                            '\0' != *idString) {
                                /*
                                 * create the marker. If there is an error, free
                                 * the string.
                                 */
                ...
                case 'l':
                        ...
                        if (result != -1) {
                                ...
                                case 'i':
                                        segType =
                                            _DtCvSetTypeToHyperText(segType);
                                        result = GetStringParameter(
                                            cur_vars, _DtCvTRUE, _DtCvTRUE,
                                            _DtCvFALSE, _DtCvFALSE, &linkspec);
                                        break;
                 ...
                 */
                case 't':
                        if (GetValueParameter(cur_vars, _DtCvTRUE, &linkType) ==
                            -1)
                                result = -1;
                        else if (linkType < 0) {
                                errno = CEErrorHyperType;
        ...
        if (result != -1) {
                ...
                if (IsTypeHyperText(segType)) {
                        linkIndex = _DtLinkDbAddLink(
                            cur_vars->my_links, NULL, linkspec,
                            ReturnLinkType(linkType, linkspec),
                            ReturnLinkWinHint(linkType), description);
                        if (linkIndex == -1)
                                result = -1;
                }
        }
        ...
        if (result != -1) {
                 ...
                 */
                result = Parse(CCDF_FIGURE_CMD, cur_state, cur_vars, 0,
                               frmt_type, font_attr, -1, Specials,
                ...
                if (-1 != result) {
                        ...
                        if (0 != capList.cnt) {
                                ...
                                _DtCvContainerVOrientOfSeg(pSeg) =
                                    vertOrientCap;
                        ...
                        result = CreateSaveGraphic(cur_vars, segType, filename,
                                                   linkIndex);

                        /*
                         * if no problems, attach the figure (and caption) to
                        ...
                        if (-1 != result) {
                                ...
                                /*
                                 * attach the figure (and caption) to the
                                 * wrapper
                                 */
                                _DtCvContainerListOfSeg(figContainer) =
                                    cur_vars->my_list.list;

                                /*
                                 * set the justify to the correct value to act
                                 * on the figure.
                                 */
                                _DtCvContainerJustifyOfSeg(figContainer) =
                                    justifyGraphic;
...
static int ProcessInLine(FormatVariables *cur_vars, int seg_type,
        ...
        if (result != -1) {
                 ...
                 */
                result =
                    CreateSaveGraphic(cur_vars, _DtCvSetTypeToInLine(seg_type),
                                      filename, link_spec);
                cur_vars->last_was_space = False;
        }
...
static int CheckIdString(FormatVariables *cur_vars, ProcessState cur_state,
         ...
         */
        if (-1 != GetStringParameter(cur_vars, _DtCvTRUE, _DtCvTRUE, _DtCvFALSE,
                                     _DtCvFALSE, &ptr) &&
            -1 != CheckSaveSegment(seg_flags, font_attr, linkspec, cur_vars)) {
                ...
                if (0 == result && -1 != FindEndMarker(cur_vars) &&
                    -1 != Parse(CCDF_ID_CMD, cur_state, cur_vars, seg_flags,
                                frmt_type, font_attr, linkspec, Specials, flags,
                                ret_on_nl, fnt_flag))
                        result = 0;
        }
...
static int Parse(int cur_cmd, ProcessState cur_state, FormatVariables *cur_vars,
        ...
        while (!done) {
                if (cur_vars->cur_mb_max != 1)
                        charSize =
                            mblen(cur_vars->rd_ptr, cur_vars->cur_mb_max);
                ...
                if (charSize == 1) {
                        ...
                        case '<':
                                 ...
                                 * determine the cmd
                                 */
                                cmdType = _DtHelpCeGetCcdfCmd(
                                    cur_cmd, cur_vars->rd_buf,
                                    &(cur_vars->rd_ptr), cur_vars->my_file,
                                    cur_vars->rd_size, allowed);
                                switch (cmdType) {
                                /*
                                 * <figure>
                                 */
                                case CCDF_FIGURE_CMD:
                                        if (CheckSaveSegment(
                                                seg_flags, font_attr, linkspec,
                                                cur_vars) == -1 ||
                                            ProcessFigureCmd(
                                                cur_vars, cur_state, frmt_type,
                                                font_attr, ret_on_nl,
                                                fnt_flag) == -1)
                                                return -1;
                                        break;
                                 ...
                                 */
                                case CCDF_FONT_CMD:
                                        fontType = _DtHelpCeGetCcdfFontType(
                                            cur_vars->rd_ptr);
                                ...
                                case CCDF_ID_CMD:
                                            ...
                                            cur_vars, cur_state, seg_flags,
                                            frmt_type, font_attr, linkspec,
                                            allowed, ret_on_nl, fnt_flag);
                                        break;
                                ...
                                case CCDF_NEWLINE_CMD:
                                        if (SaveNewLine(cur_vars, seg_flags,
                                                        font_attr,
                                                        linkspec) == -1 ||
                                            FindEndMarker(cur_vars) == -1)
                                                done = -1;
                                ...
                                case CCDF_OCTAL_CMD:
                                        if (AppendOctalToInfo(
                                                cur_vars, cur_vars->rd_ptr) ==
                                                -1 ||
                                            FindEndMarker(cur_vars) == -1)
                                                done = -1;
                                        break;
                        ...
                        case '\n':
                                 ...
                                 */
                                if (_DtCvLITERAL == frmt_type ||
                                    True == ret_on_nl) {
                                        done = SaveStringAsSegments(
                                            cur_vars,
                        ...
                        case '\t':
                                ...
                                /*
                                 * append 1-8 characters on the end of the
                                 * buffer.
                                 */
                                _DtHelpCeCountChars(cur_vars->fmt_buf,
                                                    MB_CUR_MAX, &leftOver);
                                leftOver = leftOver % 8;
                                ptr = ((char *)SpaceString) + leftOver;
                                done = _DtHelpCeAddStrToBuf(
                                    &ptr, &(cur_vars->fmt_buf),
                                    &(cur_vars->fmt_size),
                                    &(cur_vars->fmt_buf_max), (8 - leftOver),
                                    INFO_GROW);
                                break;
                        ...
                        case '\\':
                                 ...
                                 * escaped character.
                                 */
                                done = AppendCharToInfo(cur_vars,
                                                        &(cur_vars->rd_ptr));
                                break;
                        ...
                        case ' ':
                                 ...
                                 */
                                done = AppendSpaceToInfo(
                                    cur_vars, &(cur_vars->rd_ptr), frmt_type);
                                break;
                        ...
                        /*
                         * we have either invalid characters or part of a
                         * multi-byte character. Read the next buffer for more
                         * info.
                         */
                        ...
                        if (leftOver < ((int)MB_CUR_MAX)) {
                                ...
                                /*
                                 * In trouble brothers and sisters. We have
                                 * garbage in the buffer - BAIL OUT!
                                 */
                                done = -1;
                        }
                }
 ...
 *
 *****************************************************************************/
static int ParseTitle(FormatVariables *cur_vars, int cont_flag, int skip_abbrev,
                      int fnt_flag, _DtHelpFontHints *fontAttrs) {
        int result = -1;
        SegList oldList = cur_vars->my_list;
         ...
         */
        if (1 == Parse(CCDF_TITLE_CMD, NormalState, cur_vars, 0, _DtCvDYNAMIC,
                ...
                result = 0;
                if (skip_abbrev)
                        result = _DtHelpCeSkipCcdfAbbrev(
                            cur_vars->my_file, cur_vars->rd_buf,
                            &(cur_vars->rd_ptr), cur_vars->rd_size,
                            cur_vars->cur_mb_max);
                ...
                if (result != -1 && cur_vars->my_list.list) {
                        ...
                        if (NULL != list) {
                                list[cnt].type =
                                    _DtCvSetTypeToContainer(list[cnt].type);
                                if (True == cont_flag)
                                        list[cnt].type =
                                            _DtCvSetTypeToController(
                                                list[cnt].type);

                                /*
                                 * attach the title body to the controller
                                 * container.
                                 */
                                _DtCvContainerListOfSeg(&list[cnt]) =
                                    cur_vars->my_list.list;
                        ...
                        /*
                         * otherwise, a container hasn't been created for this
                         * list. but we don't want to loose the information.
                         */
                        else
 ...
 *****************************************************************************/
static int Format(FormatVariables *cur_vars, _FrmtUiInfo *ui_info,
                  char *id_string, _DtCvTopicPtr *ret_topic) {
        int result = 0;
        int done = 0;
        char *charSet = NULL;
         ...
         */
        cur_vars->rd_flag = _DtHelpCeReadBuf(
            cur_vars->my_file, cur_vars->rd_buf, cur_vars->rd_size);
        ...
        while (0 == result && !done) {
                ...
                if (0 == done)
                        do {
                                done = Parse(CCDF_PARAGRAPH_CMD, NormalState,
                                             cur_vars, 0, _DtCvDYNAMIC,
                                             &fontAttrs, -1, Specials,
                                             ~(CCDF_TOPIC_CMD | CCDF_TITLE_CMD |
                                               CCDF_ABBREV_CMD),
                                             False, 0);
                        } while (0 == done);
                ...
                if (done != -1) {
                        ...
                        if (cur_vars->my_list.list) {
                                 ...
                                 */
                                list->type =
                                    _DtCvSetTypeToContainer(list->type);

                                /*
                                 * attach the information to the container
                                 */
                                _DtCvContainerListOfSeg(list) =
                                    cur_vars->my_list.list;
         ...
         */
        if (result == -1)
                _DtHelpFreeSegments(myList.list, _DtCvFALSE,
 ...
 *
 *****************************************************************************/
static int FormatCCDFTitle(FormatVariables *cur_vars,
                           _DtHelpFontHints fontAttrs, char *filename,
                           int offset, int level, int fnt_flags,
                           int want_abbrev) {
        char *strPtr;
        char *charSet = NULL;
        int result = 0;
        ...
        if (_DtHelpCeFileOpenAndSeek(filename, offset, -1, &(cur_vars->my_file),
                                     NULL) != 0)
                return -1;
        ...
        _DtCvContainerLMarginOfSeg(titleSeg) =
            level * 2 * cur_vars->ui_info->avg_char;
        ...
        if (NULL == _DtCvContainerListOfSeg(titleSeg) || want_abbrev) {
                ...
                result = _DtHelpCeGetCcdfAbbrevCmd(
                    cur_vars->my_file, cur_vars->rd_buf, cur_vars->rd_size,
                    cur_vars->cur_mb_max, &(cur_vars->rd_ptr),
                    &(cur_vars->fmt_buf));
                ...
                if (result != -1) {
                        ...
                        if (cur_vars->fmt_buf != NULL) {
                                ...
                                if (NULL != _DtCvContainerListOfSeg(titleSeg))
                                        _DtHelpFreeSegments(
                                            _DtCvContainerListOfSeg(titleSeg),
                                            _DtCvFALSE,
                                ...
                                titleSeg->type =
                                    _DtCvSetTypeToContainer(titleSeg->type);
                                _DtCvContainerListOfSeg(titleSeg) =
                                    cur_vars->my_list.list;
                                titleList.cnt++;
                        }
                ...
                result = _DtHelpCeSkipCcdfAbbrev(
                    cur_vars->my_file, cur_vars->rd_buf, &(cur_vars->rd_ptr),
         ...
         */
        for (list = _DtCvContainerListOfSeg(titleSeg); NULL != list;
             list = list->next_seg) {
                if (_DtCvIsSegNewLine(list)) {
                        ...
                        if (NULL != list->next_disp && _DtCvIsSegString(list) &&
                            _DtCvIsSegRegChar(list)) {
                                 ...
                                 * if so do nothing.
                                 */
                                len = strlen((char *)_DtCvStringOfStringSeg(
                                          list)) -
                                      1;
                                if (' ' != ((char *)_DtCvStringOfStringSeg(
                                               list))[len]) {
                                        /*
                                         * is the next segment a one byte
                                         * string? and if so, is the first
                                         * character a space? if so, do nothing.
                                         */
                                        if (_DtCvIsSegString(list->next_disp) &&
                                            ...
                                            ' ' !=
                                                *((char *)
                                                      _DtCvStringOfStringSeg(
                                                          list->next_disp))) {
                                                /*
                                                 * need to add a space to one of
                                                 * these strings.
                                                 */
                                                len += 3;
 ...
 *
 ******************************************************************************/
static int FormatEntryInToc(_DtHelpVolumeHdl volume, char *loc_id,
                            FormatVariables *cur_vars,
                            _DtHelpFontHints font_attrs, int level) {
        int result = -1;
        int tocLen = 0;
        int offset;
        int num;
        ...
        if (True == _DtHelpCeFindId(volume, loc_id, -1, &strPtr, &offset) &&
            -1 != FormatCCDFTitle(cur_vars, font_attrs, strPtr, offset, level,
                                  ((1 << _CEFONT_SIZE) | (1 << _CEFONT_WEIGHT) |
                                   (1 << _CEFONT_ANGLE) | (1 << _CEFONT_TYPE)),
                                  False)) {
                /*
                ...
                if (num != -1) {
                        ...
                        if (NULL != pSeg) {
                                while (NULL != pSeg) {
                                        /*
                                         * If this segment has a hypertext link,
                                         * remove it.
                                         */
                                        if (pSeg->link_idx != -1)
                                                _DtLinkDbRemoveLink(
                                        ...
                                        /*
                                         * change the link index and flag to the
                                         * ghost link.
                                         */
                                        pSeg->link_idx = num;
                                        pSeg->type &= ~(_DtCvHYPER_TEXT);
                                        pSeg->type =
                                            _DtCvSetTypeToGhostLink(pSeg->type);
                                ...
                                /*
                                 * no title or abbreviation, so use the location
                                 * id. allocate room for it and put it between
                                 * parens.
                                 */
                                ...
                                strlcpy(cur_vars->fmt_buf, "...(",
                                        cur_vars->fmt_buf_max);
                                strlcat(cur_vars->fmt_buf, loc_id,
                                        cur_vars->fmt_buf_max);
                                strlcat(cur_vars->fmt_buf, ")...",
                                        cur_vars->fmt_buf_max);
                                ...
                                /*
                                 * save the string off and then attach to the
                                 * title container.
                                 */
                                result = SaveStringAsSegments(cur_vars,
                                                              _DtCvGHOST_LINK,
 ...
 * Purpose:
 ******************************************************************************/
static int FormatExpandedToc(_DtHelpVolumeHdl volume, char *cur_id,
                             char **path_lst, FormatVariables *cur_vars,
                             _DtHelpFontHints font_attrs, int level) {
        int result = 0;
        ...
        if (*path_lst != NULL) {
                ...
                if (result != -1) {
                        char **children;
                        char **childLst;
                        int count = _DtHelpCeGetCcdfTopicChildren(
                            volume, *path_lst, &children);
                        ...
                        while (result != -1 && count > 0) {
                                 ...
                                 */
                                if (*path_lst != NULL &&
                                    _DtCvStrCaseCmpLatin1(*path_lst,
                                                          *childLst) == 0)
                                        result = FormatExpandedToc(
                                            volume, cur_id, path_lst, cur_vars,
                                            font_attrs, level);
                                else
                                        result = FormatEntryInToc(
                                            volume, *childLst, cur_vars,
                                            font_attrs, level);
 ...
 *
 ******************************************************************************/
VarHandle __DtHelpCeSetUpVars(char *lang, char *code_set,
                              _FrmtUiInfo *ui_info) {
 ...
 *
 ******************************************************************************/
int __DtHelpCeProcessString(VarHandle var_handle, BufFilePtr my_file,
                            _DtCvFrmtOption frmt_type, char *scan_string,
                            char *in_string, int in_size, int fnt_flag,
        ...
        while (!done) {
                ...
                done = Parse(CCDF_FONT_CMD, NormalState, myVars, 0, frmt_type,
                             font_attr, -1, scan_string,
                             (CCDF_FONT_CMD | CCDF_OCTAL_CMD), ret_on_nl,
                             fnt_flag);
                ...
                if (done != -1 && True == ret_on_nl) {
                        ...
                        if (NULL != myVars->my_list.list) {
                                ...
                                if (-1 != done) {
                                        _DtCvSegment *newSeg =
                                            NextAvailSeg(oldList);

                                        newSeg->type = _DtCvSetTypeToContainer(
                                            newSeg->type);
                                        _DtCvContainerListOfSeg(newSeg) =
                                            myVars->my_list.list;
                                        _DtCvContainerTypeOfSeg(newSeg) =
                                            _DtCvDYNAMIC;
 ...
 *
 *****************************************************************************/
int __DtHelpCeGetParagraphList(VarHandle var_handle, int make_cont,
                               _DtCvFrmtOption type,
        ...
        if (topicHandle != NULL) {
                ...
                if (True == make_cont) {
                        ...
                        if (NULL != myCont) {
                                myCont->type =
                                    _DtCvSetTypeToContainer(myCont->type);
                                _DtCvContainerTypeOfSeg(myCont) = type;
                                _DtCvContainerListOfSeg(myCont) =
                                    myVars->my_list.list;
...
/******************************************************************************
 * Function:	int _DtHelpCeFrmtCcdfTopic (_DtHelpVolumeHdl volume, char
 **filename, int offset, char *id_string, _DtCvTopicPtr *ret_handle)
 *
 * Parameters:
 ...
 *
 ******************************************************************************/
int _DtHelpCeFrmtCcdfTopic(_DtHelpVolumeHdl volume, char *filename, int offset,
                           char *id_string, _FrmtUiInfo *ui_info,
                           _DtCvTopicPtr *ret_handle) {
        int result = -1;
        char *ptr;
         ...
         * Check the parameters
         */
        if (volume == NULL || filename == NULL || offset < 0 ||
            ret_handle == NULL) {
                errno = EINVAL;
                return -1;
        }
        ...
        if (volume) {
                ...
                result = _DtHelpCeStrrchr(variables.my_path, Slash, MB_CUR_MAX,
                                          &ptr);
                if (result == -1)
 ...
 *
 ******************************************************************************/
int _DtHelpCeFrmtCcdfPathAndChildren(_DtHelpVolumeHdl volume, char *loc_id,
                                     _FrmtUiInfo *ui_info,
                                     _DtCvTopicPtr *ret_handle) {
        int result = 0;
        int pathCnt;
        ...
        if (result == 0) {
                ...
                if (-1 ==
                    _DtHelpCeStrrchr(myVars.my_path, Slash, MB_CUR_MAX, &ptr))
                        ptr = strrchr(myVars.my_path, '/');

                if (ptr)
                        *ptr = '\0';

                result = FormatExpandedToc(volume, loc_id, pathHead, &myVars,
                                           fontAttrs, 0);
                _DtCvFreeArray((void **)pathHead);
        }

        if (result != -1)
                result = __DtHelpCeGetParagraphList(&myVars, False,
                                                    _DtCvDYNAMIC, ret_handle);
 ...
 *
 ******************************************************************************/
int _DtHelpCeGetCcdfTitleChunks(_DtHelpVolumeHdl volume, char *loc_id,
        ...
        *ret_chunks = NULL;
        ...
        result = FormatCCDFTitle(&myVars, fontAttrs, fileName, offset, 0,
                                 (1 << _CEFONT_SIZE), True);
        free(fileName);
        ...
        if (result != -1 && NULL != list) {
                ...
                while (-1 != result && NULL != pSeg) {
                        ...
                        if (_DtCvIsSegString(pSeg)) {
                                *ret_chunks = (void **)_DtCvAddPtrToArray(
                                    *ret_chunks, (void *)type);
                                if (NULL != *ret_chunks)
                                        *ret_chunks =
                                            (void **)_DtCvAddPtrToArray(
                                                *ret_chunks,
                                                (void *)_DtCvFontOfStringSeg(
                                                    pSeg));
                ...
                if (NULL != idStr) {
                        ...
                        *ret_chunks = (void **)_DtCvAddPtrToArray(
                        ...
                        if (NULL != *ret_chunks)
                                *ret_chunks = (void **)_DtCvAddPtrToArray(
                                    *ret_chunks, (void *)strdup(buf));
                        if (NULL != *ret_chunks)
                                *ret_chunks = (void **)_DtCvAddPtrToArray(
                                    *ret_chunks, (void *)idStr);
                }
        }

        if (NULL != *ret_chunks)
                *ret_chunks = (void **)_DtCvAddPtrToArray(
                    *ret_chunks, (void *)DT_HELP_CE_END);
 ...
 *
 ******************************************************************************/
int _DtHelpCeGetCcdfVolTitleChunks(_DtHelpVolumeHdl volume,
                                   _FrmtUiInfo *ui_info, void ***ret_chunks) {
        char *charSet;
        char *titleStr;
        ...
        if (titleStr != NULL) {
                *ret_chunks = (void **)malloc(sizeof(void *) * 4);
                ...
                free(charSet);
                if (_DtHelpCeGetCcdfTitleChunks(volume, "_title", ui_info,
                                                ret_chunks) != 0 &&
                    _DtHelpCeGetCcdfTitleChunks(volume, "_hometopic", ui_info,
                                                ret_chunks) != 0)
                        return -1;
        }
