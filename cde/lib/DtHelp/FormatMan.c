 ...
 *
 *****************************************************************************/
enum State { Char, Bold, BoldDone, Italic, BoldItalic, BoldItalicDone };
...
static int FormatManPage(VarHandle my_vars, BufFilePtr in_file, char *in_buf,
                         int in_size, _DtHelpFontHints *font_attr,
                         char **out_buf, int *out_size, int *out_max);
 ...
 *
 *********************************************************************/
static enum State GetNextState(char c, wchar_t wide_c, int len, char *str,
                               char *ret_c, wchar_t *ret_wc, int *ret_wc_len,
                               char **ret_str_ptr, Boolean *ret_flag) {
        register int i = len;
        ...
        if (*str == '+') {
                if (str[7] == '\010' && str[8] == '_' && str[9] == '\010' &&
                    str[10] == '_' && str[11] == '\010' && str[12] == '_' &&
                    str[13] == '\010' && str[14] == '_') {
                        *ret_flag = TRUE;
                }
        }
        ...
        if (TRUE == *ret_flag) {
                if (c == '+' && str[0] == '\010' && str[1] == '_' &&
                    str[2] == '\010' && str[3] == '_' && str[4] == '\010' &&
                    str[5] == '_' && str[6] == '\010' && str[7] == '_') {
                        *ret_flag = FALSE;
                        *ret_str_ptr = str + 8;
                        return Bold;
                }
         ...
         */
        if (len == 1 && c == '_' && mblen(str, MB_CUR_MAX) == 1 &&
            *str == '_') {
                str++;
                i++;
        }
 ...
 ******************************************************************************/
static void RemoveToken(int size, char **out_buf, int *out_size) {
 ...
 * Function: WriteToken
 *********************************************************************/
static int WriteToken(char *token, int size, char **out_buf, int *out_size,
                      int *out_max) {
        char *ptr = token;

        return (
            _DtHelpCeAddStrToBuf(&ptr, out_buf, out_size, out_max, size, 128));
}

/*********************************************************************
 * Function: WriteOutChar
 *********************************************************************/
static int WriteOutChar(int last_len, wchar_t last_wc, char c, char **out_buf,
                        int *out_size, int *out_max, Boolean flag) {
        char *ptr;
        char temp[10];
...
static int FormatManPage(VarHandle my_vars, BufFilePtr in_file, char *in_buf,
        ...
        do {
                ...
                while (result != -1 && cread > 0) {
                        ...
                        /*
                         * if we hit a null character before we've run out of
                         * characters, we've got corrupt data.
                         */
                        if (checkLen == 0)
                                return -1;
                        ...
                        if (checkLen > 0) {
                                ...
                                if (checkLen == 1 && *rloc == '\n') {
                                        ...
                                        case BoldItalicDone:
                                                /*
                                                 * get the character and wide
                                                 * character representation of
                                                 * the next character.
                                                 */
                                                c = *rloc;
                                                lastLen = mbtowc(&lastWC, rloc,
                                                                 MB_CUR_MAX);
                                                ...
                                                /*
                                                 * Check ahead for bold or
                                                 * italic sequences
                                                 */
                                                newState = GetNextState(
                                                    c, lastWC, lastLen, rloc,
                                                    &retC, &retWC, &retWCLen,
                                                    &retStrPtr, &flag);
                                                ...
                                                result = WriteOutChar(
                                                    lastLen, lastWC, c, out_buf,
                                                    out_size, out_max, flag);
                                                break;
                                 ...
                                 * But be careful.
                                 * If this is the start of a multi-byte
                                 *character, I must save it and try again on the
                                 *next read.
                                 **/
                ...
                if (result != -1 && !feof(FileStream(in_file))) {
                        if (_DtHelpCeGetNxtBuf(in_file, in_buf, &rloc,
                                               in_size) == -1)
                                result = -1;
 ...
 *
 *********************************************************************/
int _DtHelpFormatManPage(XtPointer client_data, char *man_spec,
        ...
        _FrmtUiInfo myUiInfo = {NULL, NULL, NULL, NULL, NULL, NULL,
                                0,    0,    0,    1,    False};
         ...
         */
        int ptr_size =
            sizeof(manString) + strlen(man_spec) + sizeof(devNullString) - 1;
        ...
        _DtHelpCeXlateOpToStdLocale(DtLCX_OPER_SETLOCALE,
                                    ...
                                    &(fontAttr.language), &(fontAttr.char_set));

        myVars = __DtHelpCeSetUpVars(fontAttr.language, fontAttr.char_set,
                                     &myUiInfo);
        if (myVars == NULL) {
        ...
        if (result > 0)
                result =
                    FormatManPage(myVars, myBufFile, readBuf, BUFSIZ, &fontAttr,
                                  &writeBuf, &writeBufSize, &writeBufMax);

        if ((result != -1) && writeBufSize)
                result = __DtHelpCeProcessString(
                    myVars, NULL, _DtCvLITERAL, ScanString, writeBuf,
                    writeBufSize, 0, False, &fontAttr);
        ...
        if ((topic != NULL && NULL == topicStruct->seg_list) ||
            NULL == _DtCvContainerListOfSeg(topicStruct->seg_list)) {
                _DtHelpFreeSegments(topicStruct->seg_list, _DtCvFALSE, NULL,
                                    (_DtCvPointer)pDAS);
