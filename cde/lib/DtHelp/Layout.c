...
#define CheckAddToHyperList(a, b)                                              \
        _DtCvCheckAddHyperToTravList(a, b, _DtCvTRUE, &(layout->info.lst_vis), \
 ...
 *****************************************************************************/
static void AdjustForBorders(_DtCanvasStruct *canvas, LayoutInfo *layout,
                             _DtCvFrmtOption brdr, _DtCvUnit line_width,
                             _DtCvUnit *ret_bot, _DtCvUnit *ret_right);
static void AdjustHeadPosition(_DtCanvasStruct *canvas, LayoutInfo *layout,
                               _DtCvSegmentI *p_seg, TopDims *top_bot,
                               SideDims *side, CornerDims *corner,
                               FlowDims *flow, LayFrmtInfo *info,
                               _DtCvUnit base_y, _DtCvUnit base_left,
                               _DtCvUnit block_width, _DtCvUnit left_margin,
                               _DtCvUnit right_margin);
static void AdjustObjectPosition(_DtCanvasStruct *canvas, LayoutInfo *layout,
                                 _DtCvFrmtOption justify, int start_txt,
                                 int start_ln, int start_brk, int end_txt,
                                 int end_ln, int end_brk, int brdr_cnt,
                                 _DtCvUnit height_adj, _DtCvUnit x_adj,
                                 _DtCvUnit y_adj, _DtCvUnit internal_y);
static void CheckSaveInfo(_DtCanvasStruct *canvas, LayoutInfo *layout,
                          _DtCvSegmentI *new_seg, int start);
static void DetermineFlowConstraints(LayoutInfo *layout, FlowDims flow_dims,
                                     _DtCvUnit left_margin,
                                     _DtCvUnit right_margin, _DtCvUnit start_y,
                                     DataPoint *left_pt, DataPoint *right_pt);
static void DetermineHeadPositioning(TopDims *top_bot, SideDims *side,
                                     CornerDims *corner, FlowDims *flow,
                                     _DtCvUnit start_y, _DtCvUnit max_top,
                                     _DtCvUnit block_size,
                                     _DtCvUnit *ret_side_size);
static void DetermineMaxDims(TopDims *top_bot, CornerDims *corner,
                             _DtCvUnit left_margin, _DtCvUnit right_margin,
                             _DtCvUnit *top_height, _DtCvUnit *bot_height,
                             _DtCvUnit *max_width);
static int DrawBorders(_DtCanvasStruct *canvas, LayoutInfo *layout,
                       _DtCvFrmtOption brdr, _DtCvPointer data,
                       _DtCvUnit line_width, _DtCvUnit top_y, _DtCvUnit bot_y,
                       _DtCvUnit left_x, _DtCvUnit right_x);
static void FormatCell(_DtCanvasStruct *canvas, LayoutInfo *layout,
                       _DtCvSegmentI *cell_seg, _DtCvUnit span_width,
                       _DtCvUnit min_height, DataPoint base_pt, int *ret_ln,
                       _DtCvUnit *ret_width, _DtCvUnit *ret_height,
                       _DtCvValue *ret_tab_flag);
static void InitDimArrays(TopDims *top_bot, SideDims *side, CornerDims *corner,
                          FlowDims *flow);
static _DtCvStatus LayoutCanvasInfo(_DtCanvasStruct *canvas, LayoutInfo *layout,
                                    _DtCvUnit divisor, char *target_id);
static _DtCvValue LinesMayChange(_DtCanvasStruct *canvas, int start_ln,
                                 int end_ln, int brdr_cnt);
static _DtCvUnit MoveGroup(_DtCanvasStruct *canvas, LayoutInfo *layout,
                           GrpInfo *tst_grp, _DtCvUnit needed);
static _DtCvUnit MoveLines(_DtCanvasStruct *canvas, LayoutInfo *layout, int idx,
                           _DtCvUnit top_y, _DtCvUnit bot_y, _DtCvUnit needed);
static _DtCvUnit MoveText(_DtCanvasStruct *canvas, LayoutInfo *layout, int idx,
                          _DtCvUnit top_y, _DtCvUnit bot_y, _DtCvUnit needed);
static void ProcessContainer(_DtCanvasStruct *canvas, LayoutInfo *layout,
                             _DtCvSegmentI *con_seg, _DtCvUnit min_y,
                             _DtCvUnit *ret_width, _DtCvUnit *ret_max_x,
                             int *ret_cnt);
static void ProcessSegmentList(_DtCanvasStruct *canvas, LayoutInfo *layout,
                               _DtCvSegmentI *cur_seg, _DtCvUnit min_y,
                               _DtCvUnit *ret_width, _DtCvUnit *ret_max_x,
                               int **ret_vert);
 ...
 * offsets indicated.
 *****************************************************************************/
static void AdjustTextPositions(_DtCanvasStruct *canvas, int beg, int end,
                                _DtCvUnit x_offset, _DtCvUnit y_offset) {
 ...
 *****************************************************************************/
static void AdjustLinePositions(_DtCanvasStruct *canvas, int beg, int end,
                                _DtCvUnit x_offset, _DtCvUnit y_offset) {
        if (0 != y_offset || 0 != x_offset) {
                while (beg < end) {
 ...
 *****************************************************************************/
static void AdjustPgBrk(_DtCanvasStruct *canvas, int beg, int end,
                        _DtCvUnit y_offset) {
        if (0 != y_offset) {
                while (beg < end)
 ...
 *
 *****************************************************************************/
static _DtCvUnit MaxXOfLine(_DtCanvasStruct *canvas, _DtCvDspLine *line) {
        ...
        while (pSeg != NULL && count > 0) {
                xPos = _DtCvAdvanceXOfLine(canvas, pSeg, xPos, &lnkInd,
                                           &lastLinkVisible);

                _DtCvGetWidthOfSegment(canvas, pSeg, start, count, &len,
                                       &tmpWidth, NULL);
                xPos += tmpWidth;
                count -= len;
                start = 0;
 ...
 *
 *****************************************************************************/
static _DtCvValue GetLinkInfo(_DtCvHandle canvas_handle, int indx,
        ...
        *ret_height =
            canvas->txt_lst[line].ascent + canvas->txt_lst[line].descent;
        ...
        while (count > 0 && !found && pSeg != NULL) {
                 ...
                 */
                         ...
                         */
                        pChar =
                            _DtCvStrPtr(_DtCvStringOfStringSeg(pSeg),
                         ...
                         */
                        endX = startX +
                               _DtCvGetStringWidth(canvas, pSeg, pChar, len);
                }
 ...
 *****************************************************************************/
static void CheckId(LayoutInfo *layout, char *id) {
 ...
 *****************************************************************************/
static void CheckSetLineStart(LayoutInfo *layout, _DtCvSegmentI *cur_seg) {
 ...
 *****************************************************************************/
static int CheckForPageBreak(_DtCanvasStruct *canvas, _DtCvSegmentI *cur_seg,
                             _DtCvUnit position) {
        /*
 ...
 *****************************************************************************/
static void SetBeginCounts(_DtCanvasStruct *canvas, CntInfo *cnt_info) {
        /*
         * text counts
         */
 ...
 *****************************************************************************/
static void SetEndCounts(_DtCanvasStruct *canvas, CntInfo *cnt_info,
                         int end_ln) {
        /*
         * text counts
         */
 ...
 * Returns:
 *****************************************************************************/
static void SkipToNumber(const char **string) {
        if (string != NULL) {
 ...
 * Returns:
 *****************************************************************************/
static int GetValueFromString(const char **string, int def_num) {
        int value = def_num;
 ...
 * Returns:
 *****************************************************************************/
static void PushDataPoint(LayoutInfo *layout, DataPoint *data_pt) {
        data_pt->x_units = 0;
 ...
 * Returns:
 *****************************************************************************/
static void InsertDataPoint(LayoutInfo *layout, DataPoint *data_pt) {
        DataPoint *lastPt = NULL;
        DataPoint *nextPt = layout->data_pts;

        while (nextPt != NULL && nextPt->y_pos != _CEFORMAT_ALL &&
               (data_pt->y_pos == _CEFORMAT_ALL ||
                nextPt->y_pos < data_pt->y_pos)) {
                lastPt = nextPt;
 ...
 * Returns:
 *****************************************************************************/
static void RemoveDataPoint(LayoutInfo *layout, DataPoint *data_pt) {
        DataPoint *lastPt = NULL;
 ...
 * Returns:
 *****************************************************************************/
static void GetCurrentDataPoint(LayoutInfo *layout, DataPoint *data_pt) {
        data_pt->left = 0;
        data_pt->right = 0;
        data_pt->y_pos = _CEFORMAT_ALL;
 ...
 *****************************************************************************/
static void SetMargins(LayoutInfo *layout) {
        layout->lmargin = 0;
        layout->rmargin = 0;
 ...
 *	    position value.
 *****************************************************************************/
static void SetTextPosition(LayoutInfo *layout, _DtCvValue first) {
 ...
 *****************************************************************************/
static void CheckFormat(LayoutInfo *layout, _DtCvValue first) {
 ...
 *****************************************************************************/
static void SaveLine(_DtCanvasStruct *canvas, LayoutInfo *layout, int direction,
                     _DtCvPointer data, _DtCvUnit line_width, _DtCvUnit x,
                     _DtCvUnit y, _DtCvUnit length) {
 ...
 *****************************************************************************/
static void SaveInfo(_DtCanvasStruct *canvas, LayoutInfo *layout,
                     _DtCvSegmentI *new_seg, int start) {
        _DtCvSaveInfo(canvas, &(layout->info), layout->max_width,
                      layout->rmargin, layout->txt_justify);
 ...
 *
 *****************************************************************************/
static void ProcessStringSegment(_DtCanvasStruct *canvas, LayoutInfo *layout,
        ...
        while (_DtCvProcessStringSegment(
                   canvas, &(layout->info), layout->max_width, layout->lmargin,
                   layout->rmargin, cur_seg, &(layout->cur_start),
 ...
 *****************************************************************************/
static _DtCvUnit ResolveHeight(RowSpec *row_info, CellInfo *cell_info,
                               int max_cols, int row, int span) {
        int i;
        int col;
        int cell;
        int topRow;
        ...
        for (col = 0, cell = row * max_cols; col < max_cols; col++, cell++) {
                 ...
                 * the height using it.
                 */
                if (cell_info[cell].row_spn == -1 &&
                    cell_info[cell].col_spn != -1) {
                        /*
                         ...
                         * allowd number of row heights were zeroed out (the
                         * ones we are looking for). go ahead and calculate a
                         * new height. Otherwise, it may be tried later.
                         */
                        if (zeroed > 0) {
                                /*
                                ...
                                if (cell_info[topCell].info.height > total) {
                                        total = cell_info[topCell].info.height -
                                                total;
                                        total = total / span +
                                                (total % span ? 1 : 0);
                                }
 ...
 *          allow.
 *****************************************************************************/
static void AdjustHeight(CellInfo *cell_info, RowSpec *row_specs, int max_cols,
                         int row, int col) {
        ...
        if (total < cell_info[cell].info.height) {
                 ...
                 * So check for this and set total to the number of rows
                 * spanned.
                 */
                if (0 == total)
                ...
                for (i = 0; i < cell_info[cell].row_spn; i++) {
                        value = (((row_specs[row + i].height * 100) / total) *
                                 adjustValue) /
                                100;
                ...
                if (totalUsed < adjustValue) {
                        ...
                        for (i = 0, j = cell_info[cell].row_spn;
                             adjustValue > 0 && i < cell_info[cell].row_spn;
                             i++, j--) {
                                value =
                                    adjustValue / j + (adjustValue % j ? 1 : 0);
                                row_specs[row + i].height += value;
                                adjustValue -= value;
                                totalUsed += value;
                        }
 ...
 *****************************************************************************/
static void ReFormatCell(_DtCanvasStruct *canvas, LayoutInfo *layout,
                         CellInfo *this_cell, ColumnSpec *col_specs, int col,
                         _DtCvUnit new_height, _DtCvUnit new_y) {
        int i;
         ...
         * re-format the cell
         */
        FormatCell(canvas, layout, this_cell->cell_seg, cellWidth, new_height,
...
static void FormatCell(_DtCanvasStruct *canvas, LayoutInfo *layout,
        ...
        if (cell_seg != NULL && &BlankTableCell != cell_seg) {
                ProcessContainer(canvas, layout, cell_seg, minY, ret_width,
                                 &maxX, ret_ln);
                ...
                *ret_tab_flag = True;
                while (True == *ret_tab_flag && NULL != nextSeg) {
                        ...
                        else if (!(_DtCvIsSegTable(nextSeg) ||
                                   _DtCvIsSegNoop(nextSeg)))
                                *ret_tab_flag = False;
 ...
 *
 *****************************************************************************/
static void AdjustFrmtTxtOption(_DtCvSegmentI *p_seg, _DtCvFrmtOption option) {
 ...
 *
 *****************************************************************************/
static _DtCvValue ResolveCell(_DtCanvasStruct *canvas, LayoutInfo *layout,
                              _DtCvSegmentI *table, ColumnSpec *col_specs,
                              RowSpec *row_specs, int col, int row,
                              int max_cols, int max_rows, CellInfo *ret_info) {
        register int i;
        char *id;
        char *idRefs;
        char *ptr;
        ...
        if (thisCell->cell_seg == NULL) {
                        ...
                        *ptr = '\0';
                        while (f_data != NULL && NULL != *f_data &&
                               _DtCvStrCaseCmpLatin1(
                                   _DtCvContainerIdOfSeg(*f_data), id) != 0)
                                f_data++;
                        /*
                         * make sure to break the link to the next segment.
                         * Otherwise, the formatting routines will format too
                         * much for the cell.
                         */
                        ...
                        } else /* there is no id for this cell, use a blank
                                  container */
                                thisCell->cell_seg =
                                    (struct _dtCvSegment *)&BlankTableCell;
                ...
                for (done = False, count = 1, i = row + 1;
                     0 < len && i < max_rows && False == done; i++) {
                        ...
                        if (_DtCvStrNCaseCmpLatin1(id, row_specs[i].next_id,
                                                   len) == 0 &&
                            (row_specs[i].next_id[len] == ' ' ||
                             row_specs[i].next_id[len] == '\0')) {
                                 ...
                                 */
                                for (k = 0; k < thisCell->col_spn &&
                                            k + myCol < max_cols;
                                     k++) {
                                        ret_info[i * max_cols + myCol + k]
                                            .col_spn = -1;
                                        ret_info[i * max_cols + myCol + k]
                                            .row_spn = -1;
                                }
                                ...
                                do {
                                         ...
                                         * cycle if so. quit if the end of
                                         * string or not a match.
                                         */
                                } while (*idRefs != '\0' &&
                                         _DtCvStrNCaseCmpLatin1(id, idRefs,
                                                                len) == 0 &&
                                         (idRefs[len] == ' ' ||
                                          idRefs[len] == '\0'));

                                /*
                                 * the next non-spanned column in this row will
                                 * use this id.
                                 */
        ...
        FormatCell(canvas, layout, thisCell->cell_seg, cellWidth,
                   row_specs[row].height, basePt, &brdCnt, &retWidth,
                   &retHeight, &retTabFlag);
        ...
        if (retWidth > cellWidth) {
                ...
                _DtCvUnit slop =
                    retWidth - cellWidth; /* the amount of room
                 ...
                 */
                for (j = myCol + thisCell->col_spn, maxSlop = 0; j < max_cols;
                     j++)
                        maxSlop =
                            maxSlop + col_specs[j].actual - col_specs[j].min;

                /*
                 * If the slop demanded is larger than available,
                 * simply reduced the other column specifications to their
                 * smallest values.
                 */
                if (slop >= maxSlop) {
                        ...
                        /*
                         * Is it allowed for this column to 'hang over' the
                         * others?
                         *
                         * And is it the first one? I.e. don't allow more than
                         * one cell per row to overhang it's neighbors.
                         *
                         * And only allow it if the vertical text justification
                         * places the text at the top of the cell.
                         */
                        if (_DtCvTRUE == col_specs[myCol].hanger &&
                            0 == row_specs[row].y_adj &&
                            _DtCvJUSTIFY_TOP ==
                                TxtVertJustify(thisCell->cell_seg)) {
                                /*
                                 * now set the other columns to start a little
                                 * lower in their objects. Remove the bottom
                                 * margin from the adjustment. One would hope
                                 * that each of the containers for the columns
                                 * in the row have the same bottom margin so
                                 * that the overhung cell will push the next row
                                 * down by the appropriate amount.
                                 */
                                row_specs[row].y_adj =
                                    retHeight - _DtCvContainerBMarginOfSeg(
                                                    thisCell->cell_seg);
                        ...
                        do {
                                slop -= slopUsed;
                                for (j = myCol + thisCell->col_spn,
                                    slopUsed = 0;
                                     slop > slopUsed && j < max_cols; j++) {
                                        if (col_specs[j].actual >
                                            col_specs[j].min) {
                                                col_specs[j].actual--;
                                                slopUsed++;
                                        }
                ...
                if (thisCell->col_spn == 1) {
                        ...
                        if (slop < 0) {
                                /*
                                 * set the desired to the max and calculate the
                                 * leftover slop and the maximum desired size.
                                 */
                                ...
                                slopUsed = 0;
                                for (j = myCol; j < max_cols &&
                                                j < myCol + thisCell->col_spn;
                                     j++) {
                                        percent = col_specs[j].max -
                                                  col_specs[j].actual;
                                        value = slop * percent / maxSlop;
                                        slopUsed += value;
                                        col_specs[j].actual += value;
                                ...
                                do {
                                        slop -= slopUsed;
                                        for (j = myCol, slopUsed = 0;
                                             slop > slopUsed && j < max_cols &&
                                             j < myCol + thisCell->col_spn;
                                             j++) {
                                                if (col_specs[j].actual <
                                                    col_specs[j].max) {
 ...
 *
 *****************************************************************************/
static void ProcessTable(_DtCanvasStruct *canvas, LayoutInfo *layout,
                         _DtCvSegmentI *table, _DtCvUnit min_y) {
        int a;
        int b;
        int c;
        int col;
        ...
        if (maxRows != 1 || maxCols != 1) {
                cellInfo =
                    (CellInfo *)malloc(sizeof(CellInfo) * maxCols * maxRows);
                if (cellInfo == NULL) {
                        if (maxCols > 1)
                                free(colSpecs);
                        if (maxRows > 1)
        ...
        for (col = 0, divisor = 0; col < maxCols; col++) {
                 ...
                 */
                a = GetValueFromString(&nxtWidth, 1);
                if (a < 1)
                        a = 1;
                b = GetValueFromString(&nxtWidth, 0);
                if (b < 0)
                        b = 0;
                c = GetValueFromString(&nxtWidth, b);
                if (c > a)
                        c = a;
        ...
        for (col = 0; col < maxCols; col++) {
                ...
                colSpecs[col].actual =
                    workWidth * colSpecs[col].actual / divisor;
                ...
                if (_DtCvJUSTIFY_CHAR == layout->txt_justify) {
                        ...
                        if (NULL != alignCharacters &&
                            '\0' != alignCharacters) {
                                ...
                                /*
                                 * are there more characters? If so increment
                                 * for the next column that may have
                                 * JUSTIFY_CHAR. Otherwise, leave alone and
                                 * re-use for other columns.
                                 */
                ...
                do {
                        ...
                        for (row = 0, redo = False, cell = col;
                             ...
                             row++, cell += maxCols) {
                                ...
                                /*
                                 * set the alignment flag for each column in the
                                 * row.
                                 */
                                ...
                                /*
                                 * layout all the cells jammed to the top of the
                                 * table. later, they get moved down to their
                                 * position.
                                 */
                                ...
                                /*
                                 * check to see if the specified anchor has been
                                 * found in this row. If so, save some
                                 * information for later use.
                                 */
                                if (anchorRow == -1 &&
                                    oldFound != layout->id_found)
                                        anchorRow = row;

                                /*
                                 * check to see if the alignment position has
                                 * changed. for all but the first row!
                                 */
                                if (0 != row &&
                                    oldAlignPos != layout->info.align_pos)
                                        redo = True;
                        ...
                        if (True == redo) {
                                for (a = 0; a < row; a++) {
                                        /*
                                         * restore the old row heights for this
                                         * column
                                         */
                                        rowSpecs[a].height =
                                            rowSpecs[a].lst_height;
        ...
        if (maxRowSpn > 1) {
                ...
                for (a = 1, redo = True; redo && a < maxRowSpn; a++) {
                        ...
                        for (row = 0; row < maxRows; row++) {
                                if (rowSpecs[row].height == 0 &&
                                    ResolveHeight(rowSpecs, cellInfo, maxCols,
                                                  row, a) == 0)
                                        redo = True;
                        }
                }
                 ...
                 * line height.
                 *
                 * But only do this if the first cell *does not* span all the
                 * rows and columns.
                 */
                if (redo && cellInfo[0].row_spn != maxRows &&
                    cellInfo[0].col_spn != maxCols) {
                        for (row = 0; row < maxRows; row++) {
                                if (rowSpecs[row].height == 0)
                                        rowSpecs[row].height =
                                            canvas->metrics.line_height;
                        }
                }
                ...
                for (row = 0; row < maxRows; row++)
                        for (col = 0; col < maxCols; col++)
                                AdjustHeight(cellInfo, rowSpecs, maxCols, row,
                                             col);
        }
                ...
                for (col = 0, layout->left = saveLeft; col < maxCols;
                     layout->left += colSpecs[col++].actual, cell++) {
                        if (cellInfo[cell].cell_seg != NULL) {
                                ...
                                /*
                                 * if the heights (and/or width for spanning
                                 * columns) are different, check to see if the
                                 * cell contains lines that may be affected by
                                 * the height adjustment.
                                 *
                                 * It is strongly assumed that if a table
                                 * specifies that a cell can hang over it's
                                 * neighbors that it will *NOT* have borders
                                 * (whereby LinesMayChange is false) and
                                 * newWidth will not be greater than the cell's
                                 * width.
                                 */
                                if ((newHeight > cellInfo[cell].info.height &&
                                    ...
                                    (1 < cellInfo[cell].col_spn &&
                                     newWidth > cellInfo[cell].info.width))
                                        ReFormatCell(
                                            canvas, layout, &cellInfo[cell],
                                            colSpecs, col, newHeight,
                                            layout->info.y_pos + tableYpos);

                                else
                                        /* adjust the cell rather than
                                         * reformatting */
                                        AdjustObjectPosition(
                                            ...
                                            newHeight -
                                                cellInfo[cell].info.height,
                                            0, tableYpos, workWidth);
                        }
                }
        }
 ...
 *
 *****************************************************************************/
static void UpdateDimensionArrays(_DtCvSegmentI *p_seg, _DtCvUnit width,
                                  _DtCvUnit height, TopDims *top_bot,
                                  SideDims *side, CornerDims *corner,
                                  FlowDims *flow, _DtCvUnit *max_left,
                                  _DtCvUnit *max_right) {
        int i;
        int j;
        int *marginPtr;
...
static void DetermineHeadPositioning(TopDims *top_bot, SideDims *side,
        ...
        for (i = DIMS_LM; i <= DIMS_RM; i++) {
                ...
                (*top_bot)[DIMS_BOTTOM][i][DIMS_YPOS] =
                    start_y + max_top + sideHeight;
        }
        ...
        (*side)[DIMS_CENTER][DIMS_LEFT] =
            start_y + max_top +
            (sideHeight - (*side)[DIMS_CENTER][DIMS_LEFT]) / 2;
        (*side)[DIMS_CENTER][DIMS_RIGHT] =
            start_y + max_top +
            (sideHeight - (*side)[DIMS_CENTER][DIMS_RIGHT]) / 2;

        (*side)[DIMS_BOTTOM][DIMS_LEFT] =
            start_y + max_top + sideHeight - (*side)[DIMS_BOTTOM][DIMS_LEFT];
        (*side)[DIMS_BOTTOM][DIMS_RIGHT] =
            start_y + max_top + sideHeight - (*side)[DIMS_BOTTOM][DIMS_RIGHT];
...
static void AdjustHeadPosition(_DtCanvasStruct *canvas, LayoutInfo *layout,
         ...
         */
        AdjustLinePositions(canvas, info->cnt.beg_ln, info->cnt.end_ln, adjustX,
                            adjustY);
 ...
 *****************************************************************************/
static LayFrmtInfo *ProcessController(_DtCanvasStruct *canvas,
                                      LayoutInfo *layout,
                                      _DtCvSegmentI *cur_seg) {
        int getLn;
        int saveTravCnt = canvas->trav_cnt;
        ...
        *frmtInfo = DefLayFrmtInfo;
         ...
         */
        myMaxWidth = layout->max_width - basePt.left - basePt.right - saveLeft -
                     saveRight;
        if (myMaxWidth < 0)
                myMaxWidth = 0;
        ...
        do {
                 ...
                 */
                ProcessContainer(canvas, layout, cur_seg, -1, &maxWidth,
                                 &maxXPos, &getLn);
                 ...
                 */
                if (maxXPos + _DtCvContainerRMarginOfSeg(cur_seg) >
                    myMaxWidth) {
                        redo = True;
                        myMaxWidth =
                            maxXPos + _DtCvContainerRMarginOfSeg(cur_seg);
                }
        } while (True == redo);
...
static void AdjustForBorders(_DtCanvasStruct *canvas, LayoutInfo *layout,
        ...
        if (brdr == _DtCvBORDER_FULL || brdr == _DtCvBORDER_HORZ ||
            brdr == _DtCvBORDER_TOP || brdr == _DtCvBORDER_TOP_LEFT ||
            brdr == _DtCvBORDER_TOP_RIGHT)
                layout->info.y_pos += line_width;

        *ret_bot = 0;
        if (brdr == _DtCvBORDER_FULL || brdr == _DtCvBORDER_HORZ ||
            brdr == _DtCvBORDER_BOTTOM || brdr == _DtCvBORDER_BOTTOM_LEFT ||
            brdr == _DtCvBORDER_BOTTOM_RIGHT)
                *ret_bot = line_width;

        if (brdr == _DtCvBORDER_FULL || brdr == _DtCvBORDER_VERT ||
            brdr == _DtCvBORDER_LEFT || brdr == _DtCvBORDER_TOP_LEFT ||
            brdr == _DtCvBORDER_BOTTOM_LEFT)
                layout->left += line_width;

        *ret_right = 0;
        if (brdr == _DtCvBORDER_FULL || brdr == _DtCvBORDER_VERT ||
            brdr == _DtCvBORDER_RIGHT || brdr == _DtCvBORDER_TOP_RIGHT ||
            brdr == _DtCvBORDER_BOTTOM_RIGHT) {
...
static int DrawBorders(_DtCanvasStruct *canvas, LayoutInfo *layout,
        ...
        if (brdr != _DtCvBORDER_NONE) {
                ...
                case _DtCvBORDER_BOTTOM_RIGHT:
                        SaveLine(canvas, layout, _DtCvLINE_HORZ, data,
                                 line_width, left_x, bot_y, width);
                        mod = -1;
                ...
                case _DtCvBORDER_TOP_LEFT:
                        SaveLine(canvas, layout, _DtCvLINE_VERT, data,
                                 line_width, left_x, top_y, bot_y - top_y);
                ...
                case _DtCvBORDER_TOP_RIGHT:
                        SaveLine(canvas, layout, _DtCvLINE_VERT, data,
                                 line_width, right_x - line_width, top_y,
                                 bot_y - top_y);
                }
        }
...
static void AdjustObjectPosition(_DtCanvasStruct *canvas, LayoutInfo *layout,
        ...
        if (0 != height_adj) {
                while (start_ln < end_ln) {
                         ...
                         */
                        else if (canvas->line_lst[start_ln].dir ==
                                 _DtCvLINE_VERT)
                                canvas->line_lst[start_ln].max_y += height_adj;
...
static void ProcessContainer(_DtCanvasStruct *canvas, LayoutInfo *layout,
         ...
         * first margins.
         */
        if (canvas->metrics.horiz_pad_hint >
            _DtCvContainerLMarginOfSeg(con_seg))
        ...
        if (canvas->metrics.horiz_pad_hint >
            _DtCvContainerRMarginOfSeg(con_seg))
        ...
        if (canvas->metrics.horiz_pad_hint >
            _DtCvContainerFMarginOfSeg(con_seg))
        ...
        if (NotJoining(layout)) {
                 ...
                 */
                AdjustForBorders(canvas, layout, Border(con_seg),
                                 BrdWidth(con_seg), &yPad, &xPad);
                ...
                if (_DtCvINHERIT != _DtCvContainerJustifyOfSeg(con_seg)) {
                        layout->txt_justify =
                            _DtCvContainerJustifyOfSeg(con_seg);
                        if (_DtCvJUSTIFY_NUM == layout->txt_justify)
                                layout->info.align_char = PeriodStr;
                        else if (_DtCvJUSTIFY_CHAR == layout->txt_justify) {
                                layout->info.align_char =
                                    _DtCvContainerJustifyCharOfSeg(con_seg);
                                /*
        ...
        if (0 < min_y && layout->info.y_pos < min_y) {
                AdjustObjectPosition(canvas, layout, TxtVertJustify(con_seg),
                                     ...
                                     frmtInfo.cnt.beg_brk, frmtInfo.cnt.end_txt,
                                     frmtInfo.cnt.end_ln, frmtInfo.cnt.end_brk,
                                     0, min_y - layout->info.y_pos - yPad, 0, 0,
                                     0);
        ...
        getLn =
            DrawBorders(canvas, layout, Border(con_seg), BrdData(con_seg),
                        BrdWidth(con_seg), saveYpos, layout->info.y_pos - yPad,
                        curPt.left, curPt.left + maxWidth);
         ...
         */
        if (_DtCvUSE_BOUNDARY_MOVE == canvas->constraint &&
            True == layout->brdr_flag && False == saveBrdr &&
            False == layout->table_flag) {
                GrpInfo *info = (GrpInfo *)malloc(sizeof(GrpInfo));
...
static void ProcessSegmentList(_DtCanvasStruct *canvas, LayoutInfo *layout,
         ...
         */
        while (NULL != cur_seg) {
                if (_DtCvIsSegContainer(cur_seg) &&
                    _DtCvIsSegController(cur_seg)) {
                        /*
                         * want to clear this once and only once. Then any join
                         * directives will survive, though if two or more
                         * controllers have the directive set, the 'last' one
                         * will win out.
                         */
                         ...
                         */
                        UpdateDimensionArrays(cur_seg, nxtHead->width,
                                              nxtHead->height, &topBot,
                                              &sideDims, &cornerDims, &flowDims,
        ...
        if (NULL != headInfo) {
                ...
                DetermineFlowConstraints(layout, flowDims, basePt.left,
                                         basePt.right, layout->info.y_pos,
                                         &leftPt, &rightPt);
                ...
                if (layout->max_width < maxWidth + basePt.left + basePt.right +
                                            layout->left + layout->right)
                        layout->max_width = maxWidth + basePt.left +
                                            basePt.right + layout->left +
                                            layout->right;
                ...
                if (JoinSet(layout)) {
                        ...
                        layout->info.text_x_pos =
                            canvas->txt_lst[joinLine].text_x + layout->lmargin;
                        layout->info.cur_len = 0;
                         ...
                         */
                        while (pSeg != NULL && count > 0) {
                                _DtCvGetWidthOfSegment(canvas, pSeg, start,
                                                       count, &cnt, &tmpWidth,
                                                       NULL);
        ...
        while (NULL != cur_seg) {
                ...
                case _DtCvREGION:
                        ...
                        if (_DtCvIsSegInLine(cur_seg)) {
                                 ...
                                 */
                                nWidth = _DtCvGetTraversalWidth(
                                    canvas, cur_seg, layout->info.lst_hyper);

                                /*
                                 * check to see if this region can end a line
                                 */
                                flag = _DtCvCheckLineSyntax(canvas, cur_seg, 0,
                                                            0, False);

                                /*
                                 * if this can't end a line, get the length up
                                 * to the next segment that can and base whether
                                 * to save pased on that.
                                 */
                                tempLen = 0;
                                if (False == flag) {
                                        tempLen = _DtCvGetNextWidth(
                                            canvas, _DtCvSTRING,
                                            layout->info.lst_hyper,
                                            cur_seg->next_seg, 0, cur_seg, NULL,
                                            NULL, NULL);
                                        /*
                                         * if the next width is zero, reset the
                                         * flag.
                                         */
                                        if (tempLen <= 0)
                                                flag = True;
                                 ...
                                 */
                                if (NotJoining(layout) &&
                                    _DtCvWidthOfRegionSeg(cur_seg) + tempLen >
                                        width)
                                        CheckSaveInfo(canvas, layout, cur_seg,
                                                      0);
                ...
                case _DtCvLINE:
                        ...
                        if (_DtCvIsSegBlockLine(cur_seg)) {
                                ...
                                width =
                                    layout->max_width - tempX - layout->rmargin;
                        }
                 ...
                 */
                if (leftPt.y_pos > 0 && leftPt.x_units > layout->max_width -
                                                             leftPt.left -
                                                             leftPt.right) {
                        layout->max_width =
                            leftPt.x_units + leftPt.left + leftPt.right;
                        redo = True;
                }
                if (rightPt.y_pos > 0 && rightPt.x_units > layout->max_width -
                                                               rightPt.left -
                                                               rightPt.right) {
                        layout->max_width =
                            rightPt.x_units + rightPt.left + rightPt.right;
                        redo = True;
                }
                ...
                if (redo == True) {
                        ...
                        if (rightPt.y_pos > 0) {
                                RemoveDataPoint(
                                    layout, &rightPt); /* make sure its gone */
                                InsertDataPoint(layout, &rightPt);
                        }
                        if (leftPt.y_pos > 0) {
                                RemoveDataPoint(
                                    layout, &leftPt); /* make sure its gone */
                        ...
                        if (NULL != headInfo) {
                                ...
                                if (JoinSet(layout)) {
                                        int cnt;
                                        int joinLine = layout->info.join_line;
                                        int start = canvas->txt_lst[joinLine]
                                                        .byte_index;
                                        int count =
                                            canvas->txt_lst[joinLine].length;
                                        _DtCvSegmentI *pSeg =
                                            canvas->txt_lst[joinLine].seg_ptr;
                                        _DtCvUnit tmpWidth;

                                        /*
                                         * change the starting location of the
                                         * following text. take into account the
                                         * left margin that *hasn't* been added
                                         * to the controlling container.
                                         */
                                        layout->info.text_x_pos =
                                            canvas->txt_lst[joinLine].text_x +
                                            layout->lmargin;
                                        layout->info.cur_len = 0;
        ...
        if (NULL != headInfo) {
                 ...
                 */
                DetermineHeadPositioning(&topBot, &sideDims, &cornerDims,
                                         &flowDims, saveYpos, topHeight,
                ...
                if (layout->max_width < layout->info.cur_max_x + layout->right)
                        layout->max_width =
                            layout->info.cur_max_x + layout->right;
                ...
                while (cur_seg != NULL) {
                        if (_DtCvIsSegContainer(cur_seg) &&
                            _DtCvIsSegController(cur_seg)) {
                                AdjustHeadPosition(
                                    canvas, layout, cur_seg, &topBot, &sideDims,
                                    &cornerDims, &flowDims, nxtHead, 0,
                                    basePt.left + layout->left - leftMargin,
                                    blockWidth, leftMargin, rightMargin);
                                /*
 ...
 *****************************************************************************/
static void MaxOfGroup(GrpInfo *group, _DtCvDspLine *text, _DtCvLineSeg *lines,
                       _DtCvUnit max_x, _DtCvUnit max_y) {
        int i;
 ...
 *
 *****************************************************************************/
static _DtCvStatus TestSpacing(_DtCvUnit tst_top, _DtCvUnit tst_bot,
                               _DtCvUnit tst_min, _DtCvUnit obj_top,
                               _DtCvUnit obj_bot, _DtCvUnit obj_max,
                               _DtCvUnit needed, _DtCvUnit min_space,
        ...
        if (obj_max < tst_min &&
            True ==
                _DtCvCheckInfringement(tst_top, tst_bot, obj_top, obj_bot) &&
 ...
 *
 *****************************************************************************/
static void MoveLeft(_DtCanvasStruct *canvas, int beg_txt, int end_txt,
                     int beg_ln, int end_ln, _DtCvUnit space) {
        int i;
 ...
 *
 *****************************************************************************/
static _DtCvUnit CheckSpacing(_DtCanvasStruct *canvas, LayoutInfo *layout,
                              GrpInfo *tst_grp, int txt_idx, int line_idx,
                              _DtCvUnit top_y, _DtCvUnit bot_y, _DtCvUnit min_x,
                              _DtCvUnit needed) {
        int i;
        _DtCvUnit space;
        _DtCvUnit maxSpace;
        ...
        while (NULL != nxtGrp) {
                ...
                if (nxtGrp != tst_grp) {
                        /*
                         * is this group before(x wise) the test group,
                         * infringing upon the test group's top and bottom
                         * positioning (y) and is the amount of space (x wise)
                         * between them smaller than the current smallest space
                         * found?
                         */
                        if (True == TestSpacing(top_y, bot_y, min_x,
                                                ...
                                                &space)) {
                                space += MoveGroup(canvas, layout, nxtGrp,
                                                   needed - space);
                                if (maxSpace > space)
                                        maxSpace = space;
                        }
        ...
        for (i = 0; i < canvas->txt_cnt; i++) {
                ...
                if (i != txt_idx && _DtCvIsNotProcessed(canvas->txt_lst[i])) {
                        topY = canvas->txt_lst[i].baseline -
                               canvas->txt_lst[i].ascent;
                        botY = canvas->txt_lst[i].baseline -
                               canvas->txt_lst[i].descent;
                        if (True == TestSpacing(top_y, bot_y, min_x, topY, botY,
                                                canvas->txt_lst[i].max_x,
                                                needed,
                                                canvas->metrics.horiz_pad_hint,
                                                &space)) {
                                space += MoveText(canvas, layout, i, topY, botY,
                                                  needed - space);
                                if (maxSpace > space)
                                        maxSpace = space;
                        }
        ...
        for (i = 0; i < canvas->line_cnt; i++) {
                ...
                if (i != line_idx && _DtCvIsNotProcessed(canvas->line_lst[i])) {
                        ...
                        if (True == TestSpacing(top_y, bot_y, min_x, topY, botY,
                                                canvas->line_lst[i].max_x,
                                                needed,
                                                canvas->metrics.horiz_pad_hint,
                                                &space)) {
                                space += MoveLines(canvas, layout, i, topY,
                                                   botY, needed - space);
                                if (maxSpace > space)
                                        maxSpace = space;
                        }
...
static _DtCvUnit MoveGroup(_DtCanvasStruct *canvas, LayoutInfo *layout,
         ...
         * there is.
         */
        space = CheckSpacing(canvas, layout, tst_grp, -1, -1, tst_grp->top_y,
                             tst_grp->bot_y, tst_grp->min_x, needed);
        /*
         * now move the group
         */
...
static _DtCvUnit MoveText(_DtCanvasStruct *canvas, LayoutInfo *layout, int idx,
         ...
         * there is.
         */
        space = CheckSpacing(canvas, layout, NULL, idx, -1, top_y, bot_y,
                             canvas->txt_lst[idx].text_x, needed);
        /*
         * now move the group
         */
...
static _DtCvUnit MoveLines(_DtCanvasStruct *canvas, LayoutInfo *layout, int idx,
         ...
         * there is.
         */
        space = CheckSpacing(canvas, layout, NULL, -1, idx, top_y, bot_y,
                             canvas->line_lst[idx].pos_x, needed);
 ...
 *
 *****************************************************************************/
static void CheckMoveInfo(_DtCanvasStruct *canvas, LayoutInfo *layout) {
        int i;
        _DtCvUnit topY;
        _DtCvUnit botY;
         ...
         */
        for (nxtGrp = layout->grp_lst; NULL != nxtGrp;
             nxtGrp = nxtGrp->next_info) {
                /*
                 * find the maximum of the group
                 */
        ...
        for (nxtGrp = layout->grp_lst; NULL != nxtGrp;
             nxtGrp = nxtGrp->next_info) {
                ...
                if (maxWidth < nxtGrp->max_x)
                        (void)MoveGroup(canvas, layout, nxtGrp,
                                        nxtGrp->max_x - maxWidth);
        }
 ...
 *
 *****************************************************************************/
static int CompareUnits(const void *a, const void *b) {
 ...
 *
 *****************************************************************************/
static int CompareSearchs(const void *a, const void *b) {
...
static _DtCvStatus LayoutCanvasInfo(_DtCanvasStruct *canvas, LayoutInfo *layout,
        ...
        *layout = DefInfo;
        ...
        ProcessSegmentList(canvas, layout, canvas->element_lst, -1, &maxWidth,
                           &maxXPos, NULL);
        ...
        for (i = 0; i < canvas->txt_cnt; i++)
                canvas->txt_lst[i].max_x =
                    MaxXOfLine(canvas, &(canvas->txt_lst[i]));
 ...
 *
 *****************************************************************************/
static _DtCvStatus LayoutCanvas(_DtCanvasStruct *canvas, LayoutInfo *layout,
                                char *target_id) {
        _DtCvUnit divisor = 1;
        _DtCvValue redo;
        _DtCvStatus result;
                ...
                if (_DtCvSTATUS_OK == result &&
                    ...
                    layout->info.max_x_pos >= canvas->metrics.width) {
                        if (_DtCvUSE_BOUNDARY_MOVE == canvas->constraint) {
                                ...
                                /*
                                 * clear the processed flag from all the
                                 * text/region lines.
                                 */
                                for (i = 0; i < canvas->txt_cnt; i++)
        ...
        if (0 != canvas->search_cnt)
                qsort(canvas->searchs, canvas->search_cnt,
                      sizeof(_DtCvSearchData), CompareSearchs);
        /*
 ...
 *
 *****************************************************************************/
static void SortTraversal(_DtCanvasStruct *canvas) {
        int i;
 ...
 *
 *****************************************************************************/
static _DtCvStatus ProcessMarks(_DtCanvasStruct *canvas,
        ...
        while (NULL != mark_lst && NULL != *mark_lst) {
                 ...
                 */
                if (_DtCvSTATUS_BAD ==
                    _DtCvCvtSegsToPts(canvas, (*mark_lst)->segs, &beg, &end,
                                      NULL, NULL, &firstSeg))
                        /*
                 ...
                 */
                        ...
                        markIdx =
                            _DtCvAddToMarkList(canvas, (*mark_lst)->client_data,
                                               _DtCvFALSE, &beg, &end);
                        /*
                         * now put the mark in the traversal list, but don't
                         * sort or fill out position and dimension information.
                         * SortTraversal() will do that.
                         *
                         * bail here if system failure indicated.
                         */
                        if (-1 == markIdx ||
                            0 != _DtCvSetTravEntryInfo(
                                     canvas, _DtCvGetNextTravEntry(canvas),
                                     _DtCvTraversalMark, firstSeg, markIdx,
                                     _DtCvTRUE))
...
_DtCvStatus _DtCanvasResize(_DtCvHandle canvas_handle, _DtCvValue force,
        ...
        (*(canvas->virt_functions.get_metrics))(
            canvas->client_data, _DtCvCANVAS_TYPE, &(canvas->metrics));

        if (canvas->metrics.width != oldWidth || _DtCvTRUE == force) {
                /*
                 * remember the current selection.
                 */
                selectStatus = _DtCanvasGetSelectionPoints(
                    canvas, &(selPt.segs), NULL, NULL);
                /*
                 * remember the marks
                 */
                ...
                if (_DtCvSTATUS_OK == selectStatus) {
                        _DtCanvasActivatePts(canvas, _DtCvACTIVATE_SELECTION,
                                             &selPt, NULL, NULL);
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCanvasSetTopic(_DtCvHandle canvas_handle, _DtCvTopicPtr topic,
                              _DtCvValue honor_size, _DtCvUnit *ret_width,
