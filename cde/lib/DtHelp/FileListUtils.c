...
static char *GetExtension(char *filename) {
        char *ext;
        if (_DtHelpCeStrrchr(filename, PeriodStr, MB_CUR_MAX, &ext) == 0)
                return ext;
        else
                return ""; /* do NOT return a NULL*/
}
 ...
 *
 *****************************************************************************/
static int SpecialStrcmp(const char *str1, const char *str2) {
        if (NULL == str1) {
                if (NULL == str2)
                        return 0; /* str1 == str2 */
                return -1;        /* str1 < str2 */
 ...
 *
 *****************************************************************************/
static Boolean FileInfoMatchesP(_DtHelpFileEntry file1, _DtHelpFileEntry file2,
                                int compareFlags) {
 ...
 *
 *****************************************************************************/
Boolean _DtHelpFileIsSameP(char *fileName1, char *fileName2,
                           _DtHelpFileInfoProc infoProc, int compareFlags,
        ...
        if (_DtHelpCeStrrchr(fileName1, DirSlashStr, MB_CUR_MAX, &fileName) ==
            0)
                fileName1 = fileName + 1;
        if (_DtHelpCeStrrchr(fileName2, DirSlashStr, MB_CUR_MAX, &fileName) ==
            0)
        ...
        (*infoProc)(pDisplayArea, fileName1, NULL, NULL, &file1.docId,
                    &file1.timeStamp, &file1.nameKey, NULL, NULL);
        (*infoProc)(pDisplayArea, fileName2, NULL, NULL, &file2.docId,
                    &file2.timeStamp, &file2.nameKey, NULL, NULL);
 ...
 *
 *****************************************************************************/
static int ScanDirForFiles(char *dirpath, const char *suffixList[],
                           _DtHelpFileList *in_out_list,
                           XmFontList *io_fontList, Boolean *ret_mod,
                           int foundFilesCnt, _DtHelpFileInfoProc infoProc,
                           XtPointer pDisplayArea, int compareFlags,
                           int sortFlags, _DtHelpFileScanProcCB scanProc,
        ...
        while ((result = _XReaddir(pDir, dirEntryBuf)) != NULL) {
                ...
                if (NULL != matchedSuffix) {
                        ...
                        if (_DtHelpFileListAddFile(in_out_list, io_fontList,
                                                   &mod, fullName, ptr,
                                                   infoProc, compareFlags,
                                                   sortFlags, pDisplayArea)) {
                                count++;
                                if (scanProc)
                                        (*scanProc)(count + foundFilesCnt,
                                                    clientData);
                        }
 ...
 *
 *****************************************************************************/
_DtHelpFileList _DtHelpFileListGetMatch(_DtHelpFileList fileListHead,
                                        char *fullFilePath,
        ...
        if (NULL == fileListHead || NULL == fullFilePath)
                return NULL; /* RETURN */

        /* get filename without path */
        file.fileName = fullFilePath;
        if (_DtHelpCeStrrchr(fullFilePath, DirSlashStr, MB_CUR_MAX,
                             &file.fileName) == 0)
                file.fileName++;

        /* if an info proc, use it, otherwise make due */
        if (infoProc)
                (*infoProc)(pDisplayArea, fullFilePath, NULL, NULL, &file.docId,
                            &file.timeStamp, &file.nameKey, NULL, NULL);
        else
 ...
 *
 *****************************************************************************/
_DtHelpFileList _DtHelpFileListGetNext(_DtHelpFileList fileListHead,
 ...
 *
 *****************************************************************************/
void _DtHelpFileFreeEntry(_DtHelpFileEntry entry) {
        if (NULL == entry)
                return;
 ...
 *
 *****************************************************************************/
Boolean _DtHelpFileListAddFile(_DtHelpFileList *in_out_list,
                               XmFontList *io_fontList, Boolean *ret_mod,
                               char *fullFilePath, char *fileName,
                               _DtHelpFileInfoProc infoProc, int compareFlags,
                               int sortFlags, XtPointer pDisplayArea) {
        _DtHelpFileList next;
        ...
        /* if an info proc, use it, otherwise make due */
        if (infoProc) {
                (*infoProc)(pDisplayArea, fullFilePath, &addFile.fileTitle,
                            &addFile.fileTitleXmStr, &addFile.docId,
                            &addFile.timeStamp, &addFile.nameKey, io_fontList,
                            ret_mod);
        } else {
        ...
        newList->timeStamp =
            (addFile.timeStamp == &empty ? NULL : addFile.timeStamp);
 ...
 *		EINVAL
 *
 * Purpose:	Scans all paths of given type looking for files with the
 *suffixes
 *
 *****************************************************************************/
int _DtHelpFileListScanPaths(
    _DtHelpFileList *in_out_list, XmFontList *io_fontList, Boolean *ret_mod,
    char *type, const char *suffixList[], Boolean searchHomeDir,
    _DtHelpFileInfoProc infoProc, XtPointer pDisplayArea,
    int sysPathCompareFlags, int otherPathCompareFlags, int sortFlags,
    _DtHelpFileScanProcCB scanProc, XtPointer clientData) {
        char *loc;
        char *ptr;
        char *pathName;
        struct stat status;
        ...
        if (NULL == loc || EOS == loc[0])
                loc = strdup(LANG_C_STR);

        /* zero found */
        if (scanProc)
                (*scanProc)(0, clientData);

        /* outer loop is once for each path */
        foundFilesCnt = 0;
        for (curPathIndex = 0; curPathIndex < _DtHELP_FILE_NUM_PATHS;
             curPathIndex++) {
                ...
                do {
                        char *slashptr;
                        /* look for next subpath separator and insert and EOS if
                         * found */
                        if (_DtHelpCeStrchr(curPath, PathSeparator, MB_CUR_MAX,
                                            &ptr) == 0)
                                *ptr = EOS;

                        /* generate the (directory) path using all the variables
                           and fix it up to remove the unwanted stuff involving
                           the filename*/
                        pathName = _DtHelpCeExpandPathname(curPath, NULL, type,
                                                           NULL, loc, NULL, 0);
                        if (pathName &&
                            _DtHelpCeStrrchr(pathName, DirSlashStr, MB_CUR_MAX,
                                             &slashptr) == 0)
                                slashptr[0] = EOS;
                            ...
                            S_ISDIR(status.st_mode)) /* a dir */
                        {
                                ...
                                for (i = 0; i < scannedPathsCnt; i++)
                                        if (strcmp(scannedPaths[i], pathName) ==
                                            0)
                                                break;
                                ...
                                if (i == scannedPathsCnt) {
                                        foundFilesCnt += ScanDirForFiles(
                                            pathName, suffixList, in_out_list,
                                            io_fontList, ret_mod, foundFilesCnt,
                                            infoProc, pDisplayArea,
                                            compareFlags, sortFlags, scanProc,
                                            clientData);

                                        /* add to list of scanned */
                                        scannedPaths = (char **)XtRealloc(
                                            (char *)scannedPaths,
                                            (scannedPathsCnt + 1) *
                                                sizeof(char *));
                                        scannedPaths[scannedPathsCnt++] =
                                            strdup(pathName);
        ...
        while (scannedPathsCnt-- > 0)
                XtFree(scannedPaths[scannedPathsCnt]);
