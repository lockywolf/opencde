...
static void RenderSubSet(_DtCanvasStruct *canvas, _DtCvDspLine *lines, int cnt,
        ...
        for (i = 0; NULL != lines && i < cnt; lines++, i++) {
                ...
                if (_DtCvIsNotProcessed(*lines) && maxY >= y1 && maxY <= y2) {
                         ...
                         */
                        RenderSubSet(canvas, canvas->txt_lst, cnt, minY, y2,
                                     last_y);
                }
        }
}
...
static _DtCvStatus CheckAround(_DtCvDspLine *lines, int cnt, int idx,
        ...
        while (i < cnt && _DtCvTRUE != result) {
                if (_DtCvIsNotProcessed(lines[i])) {
                        /*
                         * calculate the minimum and maximum y positions for the
                         * line.
                         */
                        ...
                        if (maxY > topY && minY < botY &&
                            _DtCvTRUE == _DtCvCheckInfringement(topY, botY,
                                                                minY, maxY)) {
                                /*
                                ...
                                /*
                                 * Does it have something else infringing one
                                 * it? Or does it hang down below the test line?
                                 */
                                if (maxY > y2 ||
                                    _DtCvTRUE == CheckAround(lines, cnt, i, y2))
                                        result = _DtCvTRUE;
                        }
                }
...
static void DrawCanvasLines(_DtCanvasStruct *canvas, _DtCvUnit x1, _DtCvUnit y1,
        ...
        if (canvas->line_lst != NULL && canvas->line_cnt &&
            NULL != canvas->virt_functions.render_elem) {
                 ...
                 */
                for (i = 0, pLS = canvas->line_lst; i < canvas->line_cnt;
                     i++, pLS++) {
                        ...
                        /*
                         * does it fit on the page? Take into account the
                         * rendering type.
                         */
                         ...
                         */
                        else if (lnInfo.x2 >= x1 && pLS->pos_x <= x2 &&
                                 pLS->max_y > newY2 &&
...
static _DtCvUnit DrawText(_DtCanvasStruct *canvas, _DtCvDspLine *line,
         ...
         */
        xPos = _DtCvAdjustForSuperSub(canvas, pSeg, xPos, &scriptX, &superWidth,
        ...
        while (NULL != pSeg && 0 < count) {
                 ...
                 */
                xPos = _DtCvDrawSegments(
                    canvas, *line, pSeg, start, len, &lastLnk, xPos, xPos,
                    &scriptX, &superWidth, &superY, &subWidth, &subY,
                    &lastWasSub, &lastWasSuper, &lastLnkVis, old_flag, new_flag,
                    _DtCvBAD_TYPE, NULL);
                /*
                ...
                curIdx += len;
                if (0 < count)
                        _DtCvSkipLineChars(canvas, pSeg, start, count + len,
                                           len, &start, &pSeg);
        }
        ...
        *ret_len = length;
        ...
        if (line_idx == start.line_idx) {
                ...
                if (start.char_idx > char_idx) {
                         ...
                         * never return a value larger than the inspection
                         * length!
                         */
...
static void DrawTraversalIndicator(_DtCanvasStruct *canvas, _DtCvValue render,
        ...
        if (_DtCvTraversalMark == canvas->trav_lst[travIdx].type) {
                ...
                if (True == render) {
                        ...
                        _DtCvDrawAreaWithFlags(
                            canvas, canvas->marks[markIdx].beg,
                            canvas->marks[markIdx].end, oldFlag, newFlag,
                            _DtCvMARK_TYPE, canvas->marks[markIdx].client_data);
                }

                if (ret_height)
                        *ret_height =
                            canvas->marks[markIdx].end.y -
                            canvas->marks[markIdx].beg.y +
                            canvas->txt_lst[canvas->marks[markIdx].end.line_idx]
                                .descent +
                            canvas->txt_lst[canvas->marks[markIdx].beg.line_idx]
                                .ascent;
        ...
        while (pSeg->link_idx != linkIndex) {
                 ...
                 * get the corrected x
                 */
                dstX = _DtCvAdvanceXOfLine(canvas, pSeg, dstX, &lstLnk,
                                           &lstLnkVis);
        ...
        height = 0;
        if (True == render) {
                while (txtLine < canvas->txt_cnt &&
                       linkIndex == pSeg->link_idx) {
                        /*
                         * get the corrected x
                         */
                        dstX = _DtCvAdvanceXOfLine(canvas, pSeg, dstX, &lstLnk,
                        ...
                        /*
                         * move the text x position base on if this is a super
                         * or sub script - or not.
                         */
                        dstX = _DtCvAdjustForSuperSub(
                            canvas, pSeg, dstX, &scriptX, &superWidth, &superY,
                            &subWidth, &subY, &lastWasSuper, &lastWasSub);
                        ...
                        while (count > 0 && pSeg != NULL &&
                               pSeg->link_idx == linkIndex) {
                                /*
                                ...
                                /*
                                 * if there is mark/selected text, determine,
                                 * how much
                                 */
                                _DtCvCheckLineMarks(
                                    canvas, txtLine, curIdx, count, dstX,
                                    (_DtCvSELECTED_FLAG | _DtCvMARK_FLAG), &len,
                                    &oldFlag, &newFlag);
                                /*
                                 * if this is the last segment(s) of the
                                 * traversal set the end flags.
                                 */
                                if (len == count) {
                                        newFlag |= (_DtCvTRAVERSAL_END |
                                                    _DtCvLINK_END);
                                        oldFlag |= (_DtCvTRAVERSAL_END |
                                                    _DtCvLINK_END);
                                }
                                ...
                                dstX = _DtCvDrawSegments(
                                    canvas, canvas->txt_lst[txtLine], pSeg,
                                    start, len, &lstLnk, dstX, dstX, &scriptX,
                                    &superWidth, &superY, &subWidth, &subY,
                                    &lastWasSub, &lastWasSuper, &lstLnkVis,
                                    oldFlag, newFlag, _DtCvLINK_TYPE, NULL);

                                count -= len;
                                curIdx += len;
                                if (count > 0) {
                                        _DtCvSkipLineChars(canvas, pSeg, start,
                                                           count + len, len,
                                                           &start, &pSeg);
                        ...
                        if (txtLine < canvas->txt_cnt) {
                                 ...
                                 * get the correct x
                                 */
                                dstX = _DtCvGetStartXOfLine(
                                    &(canvas->txt_lst[txtLine]), &pSeg);
                        }
                }
        }
...
void _DtCvCheckLineMarks(_DtCanvasStruct *canvas, int line_idx, int char_idx,
         ...
         * check the selection
         */
        if ((check_flags & _DtCvSELECTED_FLAG) &&
            canvas->select_start.y != -1) {
                ...
                *ret_new &= ~(_DtCvSELECTED_FLAG);
                ...
                if (IsLineSpecial(start, end, canvas->txt_lst[line_idx],
                                  line_idx, char_idx, length, dst_x, &length,
                                  NULL)) {
                        /*
        ...
        if ((check_flags & _DtCvMARK_FLAG) && 0 < canvas->mark_cnt) {
                 ...
                 */
                *ret_old &= ~(_DtCvMARK_FLAG | _DtCvMARK_BEGIN | _DtCvMARK_END |
                              _DtCvMARK_ON);
                *ret_new &= ~(_DtCvMARK_FLAG | _DtCvMARK_BEGIN | _DtCvMARK_END |
                              _DtCvMARK_ON);
                ...
                for (i = 0; i < canvas->mark_cnt; i++) {
                        if (IsLineSpecial(
                                canvas->marks[i].beg, canvas->marks[i].end,
                                canvas->txt_lst[line_idx], line_idx, char_idx,
                                length, dst_x, &length, &flag)) {
                                /*
                                 * A false return from IsLineSpecial means that
                                 * 'length' is outside this mark.
                                 *
                                 * When true, it means that some part of this
                                 * mark will be rendered on the call. Therefore
                                 * set the mark flag and any other flags
                                 * returned and check for mark 'on'.
                                 */
...
void _DtCvSkipLineChars(_DtCanvasStruct *canvas, _DtCvSegmentI *p_seg,
        ...
        while (use_len > 0) {
                 ...
                 */
                _DtCvGetWidthOfSegment(canvas, p_seg, start, max_cnt, &len,
                                       NULL, NULL);
                /*
...
void _DtCvClearInternalUse(_DtCvSegmentI *list, _DtCvStatus flag) {
        while (NULL != list) {
                ...
                if (_DtCvIsSegContainer(list))
                        _DtCvClearInternalUse(_DtCvContainerListOfSeg(list),
                                              flag);
                ...
                if (xPos < find_x) {
                                ...
                                */
                        {
                                if (_DtCvIsSegString(pSeg)) {
                                        ...
                                        len = _DtCvStrLen(
                                            pChar, _DtCvIsSegWideChar(pSeg));

                                        if (len > count)
                                                len = count;

                                        count -=
                                            FindChar(canvas, pSeg, pChar, len,
                                                     find_x - xPos, NULL);
                                }
                                done = True;
                        }
        ...
        *ret_cnt = 0;
        ...
        if (!(_DtCvIsSegNoop(p_seg))) {
                if (_DtCvIsSegRegion(p_seg)) {
                        ...
                        *ret_cnt =
                            _DtCvStrLen(pChar, _DtCvIsSegWideChar(p_seg));
                        if (*ret_cnt > max_cnt) {
                                *ret_cnt = max_cnt;
                                if (ret_trimmed != NULL)
                                        *ret_trimmed = True;
                         ...
                         */
                        if (ret_w != NULL)
                                *ret_w = _DtCvGetStringWidth(canvas, p_seg,
                                                             pChar, *ret_cnt);
                }
        }
}
...
_DtCvUnit _DtCvDrawSegments(
    ...
    _DtCvFlags new_flag, _DtCvElemType trav_type, _DtCvPointer trav_data) {
        ...
        while (p_seg != NULL && count > 0) {
                 ...
                 */
                txt_x = _DtCvAdvanceXOfLine(canvas, p_seg, txt_x, prev_lnk,
                                            last_link_vis);
                 ...
                 * adjust text x positioning accordingly.
                 */
                txt_x = _DtCvAdjustForSuperSub(
                    canvas, p_seg, txt_x, script_x, super_width, super_y,
                    sub_width, sub_y, last_was_super, last_was_sub);
                if (sel_x > txt_x)
                        sel_x = txt_x;
                ...
                if (_DtCvIsSegVisibleLink(p_seg)) {
                        ...
                        /*
                         * is this the start of a new link? If so, set the begin
                         * flag.
                         */
                         ...
                         */
                        linkType = _DtLinkDbGetLinkType(canvas->link_data,
                                                        p_seg->link_idx);
                        switch (_DtLinkDbGetHint(canvas->link_data,
                                                 p_seg->link_idx)) {
                ...
                *prev_lnk = p_seg->link_idx;
                ...
                case _DtCvSTRING:
                         ...
                         */
                        pChar =
                            _DtCvStrPtr(_DtCvStringOfStringSeg(p_seg),
                        ...
                        /*
                         * now get the width of the string to update the x
                         * positions
                         */
                        segWidth =
                            _DtCvGetStringWidth(canvas, p_seg, pChar, len);
                ...
                if (hasElemType) {
                        ...
                        /*
                         * if the item (string) was not cropped, set the Search
                         * end flag accordingly.
                         */
                        if (_DtCvFALSE == cropped) {
                                _DtCvSetSearchEnd(old_flag, p_seg);
                                _DtCvSetSearchEnd(new_flag, p_seg);

                                if (new_flag & _DtCvSEARCH_END &&
                                    new_flag & _DtCvAPP_FLAG2 &&
                        ...
                        if (NULL != canvas->virt_functions.render_elem)
                                (*(canvas->virt_functions.render_elem))(
                                    canvas->client_data, elemType, txt_x, yPos,
                                    linkType, old_flag, new_flag, trav_type,
                                    trav_data, &posInfo);

                        if (cropped == _DtCvFALSE &&
                            new_flag & _DtCvSEARCH_END &&
                            new_flag & _DtCvAPP_FLAG2 &&
                            !(new_flag & _DtCvSELECTED_FLAG))
...
void _DtCanvasLoadMetrics(_DtCvHandle handle) {
        ...
        (*(canvas->virt_functions.get_metrics))(
            canvas->client_data, _DtCvLINK_TYPE, &(canvas->link_info));
        (*(canvas->virt_functions.get_metrics))(canvas->client_data,
                                                _DtCvTRAVERSAL_TYPE,
                                                &(canvas->traversal_info));
        if (MB_CUR_MAX > 1)
                (*(canvas->virt_functions.get_metrics))(
             ...
             NULL != lines && i < canvas->txt_cnt; lines++, i++) {
                ...
                if (_DtCvIsNotProcessed(*lines) && maxY >= y1 && minY <= y2 &&
                    (_DtCvRENDER_PARTIAL == flag ||
                     (maxY <= y2 &&
                      _DtCvTRUE !=
                          (sideCk = CheckAround(canvas->txt_lst,
                                                canvas->txt_cnt, i, y2))))) {
                        (void)DrawText(canvas, lines, i, 0, 0);
                        ...
                        /*
                         * if doing complete printing, get any other lines that
                         * exist next to this one, but don't fit the [y1,y2]
                         * pair. This will catch scrolling problems using
                         * _DtCvRENDER_COMPLETE.
                         *
                         * The previous CheckAround() call will have set sideCk
                         * to _DtCvFALSE if there are other items to the side,
                         * but these items did not violate the maximum y.
                         *
                         * sideCk will be _DtCvSTATUS_NONE if there is nothing
                         * to the side for _DtCvRENDER_COMPLETE or if flag is
                         * _DtCvRENDER_PARTIAL.
                         */
                        if (_DtCvFALSE == sideCk)
                                RenderSubSet(canvas, canvas->txt_lst,
                                             canvas->txt_cnt, minY, maxY,
                                             &lastY);
...
_DtCvStatus _DtCanvasMoveTraversal(_DtCvHandle canvas_handle,
                        ...
                        while (False == found && newIndex < canvas->trav_cnt) {
                                if (_DtCvTRAVERSAL_ID == cmd &&
                                    _DtCvTraversalLink ==
                                        ...
                                        idx = canvas->trav_lst[newIndex]
                                                  .seg_ptr->link_idx;
                                        lnkId = _DtLinkDbGetLinkSpec(
                                            canvas->link_data, idx);

                                        if (_DtCvStrCaseCmpLatin1(lnkId, rid) ==
                                            0)
                                                found = True;
                                } else if (_DtCvTRAVERSAL_MARK == cmd &&
                                           _DtCvTraversalMark ==
                                               canvas->trav_lst[newIndex]
                                                   .type) {
                                        idx = canvas->trav_lst[newIndex].idx;
                                        if (rid ==
                                            canvas->marks[idx].client_data)
                                                found = True;
                                }
        ...
        if (cmd == _DtCvTRAVERSAL_OFF) {
                if (-1 != canvas->cur_trav)
                        DrawTraversalIndicator(canvas, render, False, NULL,
                                               NULL, NULL, NULL);
         ...
         */
                ...
                DrawTraversalIndicator(canvas, render, True, ret_x, ret_y,
                                       ret_baseline, ret_height);
                ...
                if (topY <= y1 && y1 <= botY && startX <= x1 &&
                    x1 <= canvas->txt_lst[line].max_x) {
                        ...
                        while (count > 0 && _DtCvSTATUS_NONE == found) {
                                /*
                                 * adjust the starting position by the link
                                 * space
                                 */
                                junk = _DtCvIsSegVisibleLink(pSeg);
                                lstVisible = _DtCvModifyXpos(
                                    canvas->link_info, pSeg, junk, lstVisible,
                                    lnkIndx, &startX);
                                /*
                                 * adjust the starting position by the traversal
                                 * space
                                 */
                                junk = _DtCvIsSegALink(pSeg);
                                (void)_DtCvModifyXpos(
                                    canvas->traversal_info, pSeg, junk,
                                 ...
                                 */
                                        ...
                                        pChar = _DtCvStrPtr(
                                            _DtCvStringOfStringSeg(pSeg),
                                            _DtCvIsSegWideChar(pSeg),
                                            startChar);

                                        /*
                                         * get the length of the current string.
                                         * If it is longer than the line count
                                         * indicates, it must be wrapped to the
                                         * next line. We are only interested in
                                         * in the part of the line that is on
                                         * the line selected.
                                         */
                                        len = _DtCvStrLen(
                                            pChar, _DtCvIsSegWideChar(pSeg));
                                        ...
                                        /*
                                         * calculate the ending pixel postion
                                         * for this string segment.
                                         */
                                        endX = startX +
                                ...
                                /*
                                 * test to see if the selected segment was this
                                 * segment.
                                 */
                                if (x1 >= startX && x1 < endX && x2 >= startX &&
                                    x2 < endX) {
...
_DtCvStatus _DtCanvasGetCurTraversal(_DtCvHandle canvas_handle,
        ...
        if (NULL != ret_data &&
            _DtCvTraversalMark == canvas->trav_lst[curTrav].type) {
                *ret_data =
                    canvas->marks[canvas->trav_lst[curTrav].idx].client_data;
