...
static void GetUnderLineInfo(Display *dpy, DtHelpDispAreaStruct *pDAS,
        ...
        if (idx < 0) {
                ...
                if (get_fontsetproperty(
                        (XFontSet)myFont, XA_UNDERLINE_POSITION,
                        ((unsigned long *)&(pDAS->underLine))) == FALSE) {
                        extents = XExtentsOfFontSet((XFontSet)myFont);
                        pDAS->underLine =
                            ((int)(extents->max_ink_extent.height +
                                   extents->max_ink_extent.y)) /
                            2;
                }
        } else {
                myFont = (XtPointer)__DtHelpFontStructGet(pDAS->font_info, idx);
                if (XGetFontProperty(
                        (XFontStruct *)myFont, XA_UNDERLINE_POSITION,
                        ((unsigned long *)&(pDAS->underLine))) == FALSE)
                        pDAS->underLine =
                            ((XFontStruct *)myFont)->max_bounds.descent / 2;
        }
        ...
        if (idx < 0) {
                if (get_fontsetproperty(
                        (XFontSet)myFont, XA_UNDERLINE_THICKNESS,
                        ((unsigned long *)ret_underThick)) == FALSE)
                        *ret_underThick = pDAS->lineHeight * 15 / 100;
        } else {
                if (XGetFontProperty(
                        (XFontStruct *)myFont, XA_UNDERLINE_THICKNESS,
                        ((unsigned long *)ret_underThick)) == FALSE)
                        *ret_underThick = pDAS->lineHeight * 15 / 100;
        }
         ...
         * an example of Hewlett-Packard's implementation is given for
         * reference.
         */
 ...
 *
 *********************************************************************/
static XtPointer HelpCreateDA(Widget parent, char *name, short vert_flag,
                              short horz_flag, Boolean traversal_flag,
                              _DtCvValue honor_size,
                              _DtCvRenderType render_type, int rows,
                              int columns, unsigned short media_resolution,
                              void (*hyperTextCB)(), void (*resizeCB)(),
        ...
        if (default_list != NULL &&
            XmFontListInitFontContext(&fontContext, default_list)) {
                ...
                if (fontEntry != NULL)
                        default_font =
                            XmFontListEntryGetFont(fontEntry, &fontType);
        ...
        if (n < 0)
                value = get_fontsetproperty(
                    __DtHelpFontSetGet(pDAS->font_info, n), xa_ave_width,
                    ((unsigned long *)&(pDAS->charWidth)));
        else
                value = XGetFontProperty(
                    __DtHelpFontStructGet(pDAS->font_info, n), xa_ave_width,
                    ((unsigned long *)&(pDAS->charWidth)));
        ...
        if (False == value || 0 == pDAS->charWidth) {
                ...
                if (n < 0)
                        ...
                        len += XTextWidth(
                            __DtHelpFontStructGet(pDAS->font_info, n), "1", 1);
        ...
        if (vert_flag != _DtHelpNONE) {
                ...
                ++n;
                pDAS->vertScrollWid = XmCreateScrollBar(
                    form, "DisplayDtHelpVertScrollBar", args, n);

                XtManageChild(pDAS->vertScrollWid);
                if (vert_flag != _DtHelpSTATIC)
                        pDAS->neededFlags = _DtHelpSET_AS_NEEDED(
                            pDAS->neededFlags, _DtHelpVERTICAL_SCROLLBAR);
                ...
                XtAddEventHandler(pDAS->vertScrollWid, ButtonPressMask, True,
                                  (XtEventHandler)_DtHelpMoveBtnFocusCB,
                                  (XtPointer)pDAS);
        }
        ...
        if (horz_flag != _DtHelpNONE) {
                ...
                if (horz_flag != _DtHelpSTATIC)
                        pDAS->neededFlags = _DtHelpSET_AS_NEEDED(
                            pDAS->neededFlags, _DtHelpHORIZONTAL_SCROLLBAR);
       ...
       XtSetArg(args[n], XmNtranslations	   , DrawnBTrans);
       ++n;
        */
        if (DrawnBTrans != NULL)
        ...
        if (_XmGetFocusPolicy(parent) == XmPOINTER) {
                XtAddEventHandler(pDAS->dispWid, EnterWindowMask, True,
                                  (XtEventHandler)_DtHelpEnterLeaveCB,
                                  (XtPointer)pDAS);
        ...
        gcMask = (GCFunction | GCPlaneMask | GCForeground | GCBackground |
                  GCLineWidth | GCLineStyle | GCClipXOrigin | GCClipYOrigin |
                  GCClipMask);
        ...
        pDAS->normalGC =
            XCreateGC(dpy, RootWindowOfScreen(XtScreen(pDAS->dispWid)), gcMask,
                      &gcValues);
         ...
         */
        pDAS->dispUseWidth =
            ((int)(char_width / 10 + (char_width % 10 ? 1 : 0))) +
            2 * pDAS->marginWidth;
 ...
 *
 *********************************************************************/
XtPointer _DtHelpCreateDisplayArea(Widget parent, char *name, short vert_flag,
                                   short horz_flag, Boolean traversal_flag,
                                   int rows, int columns, void (*hyperTextCB)(),
                                   void (*resizeCB)(), int (*exec_ok_routine)(),
                                   XtPointer client_data,
                                   XmFontList default_list) {
        return HelpCreateDA(parent, name, vert_flag, horz_flag, traversal_flag,
                            _DtCvIGNORE_BOUNDARY, _DtCvRENDER_PARTIAL, rows,
                            columns, 100, hyperTextCB, resizeCB,
                            exec_ok_routine, client_data, default_list);
}
...
XtPointer _DtHelpCreateOutputArea(
    ...
    XtPointer client_data, XmFontList default_list) {
        ...
        pDAS = HelpCreateDA(parent, name, vert_flag, horz_flag, traversal_flag,
                            honor_size, render_type, 1, 1, media_resolution,
                            hyperTextCB, resizeCB, exec_ok_routine, client_data,
                            default_list);
