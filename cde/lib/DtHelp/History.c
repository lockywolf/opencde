...
static void CloseHistoryCB(Widget w, XtPointer client_data,
                           XtPointer call_data);
static void ProcessTopicSelectionCB(Widget w, XtPointer client_data,
                                    XtPointer call_data);
static void ProcessVolumeSelectionCB(Widget w, XtPointer client_data,
                                     XtPointer call_data);
static void CreateHistoryDialog(Widget nw);
static DtHistoryListStruct *AddItemToHistoryList(DtHistoryListStruct **pHead,
                                                 XmString title, int topicType,
                                                 Boolean *duplicateItem);
static DtTopicListStruct *PullTopicListFromSelVolumeList(Widget nw);
static void UpdateTopicList(DtHistoryListStruct *pHistoryList, Widget nw,
                            int topicType);
static Boolean IsTopicInHistoryList(DtTopicListStruct *pTopicList,
                                    XmString topicTitle);
 ...
 *
 *****************************************************************************/
void _DtHelpDisplayHistoryInfo(Widget nw) {
        ...
        if (XtIsManaged(hw->help_dialog.history.historyWidget) == False) {
                ...
                XtMapWidget(
                    XtParent((Widget)hw->help_dialog.history.historyWidget));
...
static Boolean IsTopicInHistoryList(DtTopicListStruct *pTopicList,
        ...
        while (!done && (pTopicList != NULL)) {
                if (XmStringCompare(topicTitle, pTopicList->topicTitleLbl) ==
                    True)
                        done = TRUE;
...
static void UpdateTopicList(DtHistoryListStruct *pHistoryList, Widget nw,
        ...
        items =
            (XmString *)XtMalloc(sizeof(XmString) * pHistoryList->totalNodes);
        ...
        case DtHELP_TYPE_TOPIC:
                labelString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(3, 8, "Help Topics Visited:")));
                break;

        case DtHELP_TYPE_FILE:
                labelString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(3, 9, "Help Files Visited:")));
                break;

        case DtHELP_TYPE_MAN_PAGE:
                labelString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(3, 10, "Man Pages Visited:")));
                break;

        case DtHELP_TYPE_STRING:
        case DtHELP_TYPE_DYNAMIC_STRING:
                labelString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(3, 11, "Help Messages Visited:")));
                break;
...
static void ProcessVolumeSelectionCB(Widget w, XtPointer client_data,
        ...
        if (False == XmListGetSelectedPos(w, &topicPosList, &topicCnt) &&
            NULL == topicPosList) {
                XmListSelectPos(w, topicPosition,
                                False); /* False=dont notify */
                return;                 /* RETURN */
        }
        ...
        if (_DtHelpFormatTopicTitle(hw->help_dialog.help.pDisplayArea,
            ...
            NULL == labelString) {
                labelString =
                    XmStringCreateLocalized(hw->help_dialog.display.locationId);
        }

        if (True == mod) {
                /* must set the fontlist, otherwise this will cause a dangle
                 * later. */
                XtSetArg(args[0], XmNfontList, fontList);
...
static void ProcessTopicSelectionCB(Widget w, XtPointer client_data,
        ...
        if (pTemp == NULL) {
                /*FIX: We have a problem, this should never happen; decide what
                 * to do*/
                return;
        }
        ...
        case DtHELP_TYPE_TOPIC:
                /* Look and see if we need to update our helpVolue to a new
                 * value */
                if (pTemp->helpVolume != NULL &&
                    (hw->help_dialog.display.helpVolume == NULL ||
                     strcmp(hw->help_dialog.display.helpVolume,
                            pTemp->helpVolume) != 0)) {
...
static void CreateHistoryDialog(Widget nw) {
        ...
        title = XtNewString(
            ((char *)_DTGETMESSAGE(3, 1, "Help - History Browser")));
        n = 0;
        ...
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(3, 2, "Help Volumes Visited:")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, volumeListLabel);
        n++;
        ...
        XtSetArg(args[n], XmNtopWidget,
                 XtParent(hw->help_dialog.history.volumeList));
        n++;
        XtSetArg(args[n], XmNtopOffset, 10);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        ...
        XtAddCallback(hw->help_dialog.history.topicList,
                      XmNsingleSelectionCallback,
                      (XtCallbackProc)ProcessTopicSelectionCB, (XtPointer)hw);
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget,
                 XtParent(hw->help_dialog.history.topicList));
        n++;
        ...
        labelString =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(3, 3, "Close")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        ...
        n++;
        /*XtSetArg (args[n], XmNbottomAttachment, XmATTACH_FORM);
         *n++; XtSetArg (args[n], XmNbottomOffset, 3);
         *n++;
         */
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_historyHelpBtn_STR, (Widget)hw,
                                   &hw->help_dialog.help,
        ...
        n = 0;
        XtSetArg(args[n], XmNmwmFunctions, MWM_FUNC_MOVE | MWM_FUNC_RESIZE);
        n++;
        ...
        /* calculate and set the buttons' positions on the form */
        {
                ...
                unsigned long avgWidth =
                    73; /* default size, in 10ths of pixel */
                Dimension formWidth;
...
#define BETW_BTN_SPC 5
                ...
                xa_ave_width =
                    XmInternAtom(XtDisplay(cancelBtn), "AVERAGE_WIDTH", False);
                _DtHelpXmFontListGetPropertyMax(fontList, xa_ave_width,
                                                &avgWidth);

                /* xa_ave_width is given in 10ths of a pixel; convert to pixels
                 * by div thru 10 */
                /* we want a 25 column minimum width */
                formWidth = avgWidth * 25 / 10;

                _DtHelpSetButtonPositions(btnList, 2, formWidth, BTN_MARGINS,
                                          BETW_BTN_SPC);
        }
        ...
        n = 0;
        XtSetArg(args[n], XmNheight, &btnHeight);
        n++;
        ...
        /* make the bottom attachment for the seporator such that things will
         * fit, and the dialog will size properly.
         */
        n = 0;
        XtSetArg(args[n], XmNbottomAttachment, XmATTACH_FORM);
        n++;
        ...
        XtAddCallback(historyShell, XmNpopupCallback,
                      (XtCallbackProc)_DtHelpMapCB, (XtPointer)XtParent(hw));

        /* Add our help callback to the shell of the dialog  */
        pHelpInfo = _DtHelpListAdd(DtHELP_historyShell_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(historyForm, XmNhelpCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
 ...
 *
 *****************************************************************************/
void _DtHelpUpdateHistoryList(char *locationId, int topicType,
        ...
        case DtHELP_TYPE_TOPIC:
                ...
                allocTopic = True;
                mod = False;
                if (_DtHelpFormatTopicTitle(
                        hw->help_dialog.help.pDisplayArea,
                        hw->help_dialog.display.volumeHandle, locationId,
                        &topicTitle, &fontList, &mod) != 0 ||
                    NULL == topicTitle)
         ...
         */
        pCurrentHistoryList =
            AddItemToHistoryList(&(hw->help_dialog.history.pHistoryListHead),
                                 currentItemTitle, topicType, &dupItem);

        if (dupItem != TRUE) {
                /* We can skip this if it already exists in our history
                 * list,else we add the new history item  to the top of the
                 * scrolled window
                 */
                XmListAddItem(hw->help_dialog.history.volumeList,
                              currentItemTitle, 1);
                XmListSelectPos(hw->help_dialog.history.volumeList, 1, FALSE);
        } else /* dupItem == True */
        {
                /* We changed to a diferent volume or help type so force it
                 * selected */
        ...
        if (pCurrentHistoryList != NULL) {
                ...
                if (!IsTopicInHistoryList(pCurrentHistoryList->pTopicHead,
                                          topicTitle)) {
                        /* Add the new topic to the history top of the history
                         * list */
                        _DtHelpTopicListAddToHead(
                            locationId, topicTitle, topicType,
                            DtHISTORY_LIST_MAX,
                            hw->help_dialog.display.helpVolume,
                        ...
                        UpdateTopicList(pCurrentHistoryList, (Widget)hw,
                                        topicType);

                        /* Force the top item to be selected */
                        XmListSelectPos(hw->help_dialog.history.topicList, 1,
                                        FALSE);
                } else {
                        if (changedSelectedVolume) {
                                if (TRUE == vol_changed)
                                        UpdateTopicList(pCurrentHistoryList,
                                                        (Widget)hw, topicType);
                                XmListSelectItem(
                                    hw->help_dialog.history.topicList,
                                    topicTitle, FALSE);
...
static DtHistoryListStruct *AddItemToHistoryList(DtHistoryListStruct **pHead,
        ...
        *duplicateItem = False;
        ...
        if (pTemp == NULL) {
                 ...
                 * our current list.
                 */
                pTemp = (DtHistoryListStruct *)XtMalloc(
                    (sizeof(DtHistoryListStruct)));
