...
static Boolean ConvertSelectionCB(Widget widget, Atom *selection, Atom *target,
        ...
        retStatus =
            XmbTextListToTextProperty(XtDisplay(widget), &testString, 1,
        ...
        if (*target == TARGETS) {
                ...
                *format = 8;
                ...
                _DtCanvasGetSelection(
                    pDAS->canvas, (_DtCvSELECTED_TEXT | _DtCvSELECTED_REGION),
                    (_DtCvPointer *)(&tmpString));
                retStatus = -1;
                if (tmpString != NULL && *tmpString != '\0')
                        retStatus = XmbTextListToTextProperty(
                            XtDisplay(widget), &tmpString, 1,
                          ...
                          XtIntervalId *id)      /*  timer id                */
{
        ...
        if (pDAS->scr_timer_data.horizontal_reason == XmCR_NONE)
                ...
                if (pDAS->scr_timer_data.horizontal_reason == XmCR_INCREMENT) {
                ...
                } else if (pDAS->scr_timer_data.horizontal_reason ==
                           XmCR_DECREMENT) {
                        diffX = -((int)(pDAS->charWidth / 10));
                        x = 0;
                        ...
                        fprintf(
                            stderr,
                            "ScrollTimerCB: impossible condition, aborting");
                        exit(1);
                }
        }

        _DtCanvasProcessSelection(
            pDAS->canvas, (x + diffX + pDAS->virtualX - pDAS->decorThickness),
...
void _DtHelpSearchMoveTraversal(XtPointer client_data, int search_hit_index) {
        ...
        if (_DtCvGetSearchLineMetrics(pDAS->canvas, search_hit_index, &baseline,
                                      &descend, &ascend) == 0) {
...
Boolean _DtHelpCancelSelection(XtPointer client_data) {
        ...
        if (pDAS->select_state == _DtHelpSelectingText &&
            pDAS->primary == True) {
                selActive = True;
...
void _DtHelpVertScrollCB(Widget widget, XtPointer clientData,
         ...
         * use XCopyArea to move the visible lines and draw the cleared out
         * line.
         */
        if (!pDAS->maxYpos ||
            (pDAS->neededFlags & (1 << (VisibilityFullyObscured + 3))))
        ...
        if (reason == XmCR_PAGE_INCREMENT || reason == XmCR_PAGE_DECREMENT ||
            reason == XmCR_TO_TOP || reason == XmCR_TO_BOTTOM)
                ...
                XCopyArea(dpy, win, win, pDAS->normalGC, pDAS->decorThickness,
                          srcY, pDAS->dispUseWidth,
                          (pDAS->dispUseHeight - diff), pDAS->decorThickness,
                          dstY);
                ...
                if (pDAS->neededFlags &
                    (1 << (VisibilityPartiallyObscured + 3))) {
                        /*
                         ...
                         */
                        absTop =
                            clearY + pDAS->firstVisible - pDAS->decorThickness;
                        absBot = absTop + diff;

                        _DtCanvasRender(pDAS->canvas, 0, absTop,
                                        pDAS->virtualX + pDAS->dispWidth,
                                        absBot, _DtCvRENDER_PARTIAL, _DtCvFALSE,
                                        NULL, NULL);
                        /*
                         * if the toc exists within this area, draw it.
                         */
...
void _DtHelpHorzScrollCB(Widget widget, XtPointer clientData,
         ...
         * View_DtHelpExposeCB to redisplay the text. For increment and
         * decrement, use XCopyArea to move the visible lines and draw the
         * cleared out line.
         */
        if (reason == XmCR_PAGE_INCREMENT || reason == XmCR_PAGE_DECREMENT ||
            reason == XmCR_TO_TOP || reason == XmCR_TO_BOTTOM)
                ...
                XCopyArea(dpy, win, win, pDAS->normalGC, srcX,
                          pDAS->decorThickness, pDAS->dispUseWidth - diff,
                          pDAS->dispUseHeight, dstX, pDAS->decorThickness);
 ...
 *****************************************************************************/
void _DtHelpEndSelectionCB(Widget w,              /*  widget id           */
        ...
        if (pDAS->select_state == _DtHelpCopyOrLink) {
                if (abs(pDAS->timerX - newX) <= pDAS->moveThreshold &&
                    abs(pDAS->timerY - newY) <= pDAS->moveThreshold) {
                         ...
                         * which means a key was pressed, so we ignore this
                         * call.
                         */
                        if (callback->event)
                                /*
                                ...
                                _DtHelpProcessHyperSelection(
                                    client_data, pDAS->timerX, pDAS->timerY,
                                    callback->event);
                } else
...
void _DtHelpMouseMoveCB(Widget widget, XtPointer client_data, XEvent *event) {
        ...
        if (newY < ((int)pDAS->decorThickness) && pDAS->firstVisible)
                pDAS->scr_timer_data.vertical_reason = XmCR_DECREMENT;
                 ...
                 (pDAS->maxYpos >
                  (((int)pDAS->firstVisible) + ((int)pDAS->dispUseHeight))))
        ...
        _DtCanvasProcessSelection(pDAS->canvas, newX, newY,
 ...
 *
 *****************************************************************************/
static void StartSelection(Widget widget,         /*  widget id               */
                           XtPointer client_data) /*  data from applicaiton   */
{
        DtHelpDispAreaStruct *pDAS = (DtHelpDispAreaStruct *)client_data;
...
void _DtHelpLoseSelectionCB(Widget widget, Atom *selection) {
        ...
        if (pDAS != NULL && pDAS->dispWid == widget &&
...
void _DtHelpClearSelection(XtPointer client_data) {
        ...
        if (pDAS->primary == True) {
                if (NULL != pDAS->canvas) {
                        _DtCanvasMoveTraversal(pDAS->canvas, _DtCvTRAVERSAL_OFF,
                                               False, True, NULL, NULL, NULL,
                                               NULL, NULL);
                        _DtCanvasProcessSelection(pDAS->canvas, 0, 0,
                                                  _DtCvSELECTION_CLEAR);
                        _DtCanvasMoveTraversal(pDAS->canvas, _DtCvTRAVERSAL_ON,
                                               False, True, NULL, NULL, NULL,
                                               NULL, NULL);
                }
                pDAS->select_state = _DtHelpNothingDoing;
        ...
        if (oldFlag != newFlag) {
                if (newFlag == False) {
                        _DtCanvasMoveTraversal(
                            pDAS->canvas, _DtCvTRAVERSAL_OFF, False,
                            (XtIsRealized(widget) ? True : False), NULL, NULL,
                            NULL, NULL, NULL);
                        pDAS->neededFlags =
                        ...
                        pDAS->neededFlags =
                            pDAS->neededFlags | _DT_HELP_FOCUS_FLAG;
                        _DtCanvasMoveTraversal(
                            pDAS->canvas, _DtCvTRAVERSAL_ON, False,
                            (XtIsRealized(widget) ? True : False), NULL, NULL,
                            NULL, NULL, NULL);
                }
        }
...
void _DtHelpVisibilityCB(Widget widget, XtPointer client_data, XEvent *event) {
        ...
        /*
         * save the scrollbar and focus flags while clearing the visibility
         * flags.
         */
        pDAS->neededFlags =
         ...
         */
        pDAS->neededFlags =
            pDAS->neededFlags | (1 << (event->xvisibility.state + 3));
        ...
        if (pDAS->primary == True && pDAS->text_selected == True) {
                ...
                if (tmpString != NULL) {
                        ...
                        status = XmClipboardStartCopy(
                            dpy, win, clipLabel, XtLastTimestampProcessed(dpy),
                        ...
                        status = XmbTextListToTextProperty(
                            ...
                            (XICCEncodingStyle)XStdICCTextStyle, &tmpProp);
                        /*
                         * free the original copy of the string and check the
                         * results.
                         */
                        free(tmpString);
                        ...
                        status = XmClipboardCopy(dpy, win, itemId, atomName,
                                                 (XtPointer)tmpProp.value,
                                                 tmpProp.nitems, 0, &dataId);
