 ...
 * Purpose:
 *****************************************************************************/
static int FindLink(_DtCvLinkDb link_db, char *id, int *ret_no, int *ret_hint) {
        int result = -1;
        int i;
 ...
 * Purpose:
 *****************************************************************************/
static int ResolveSwitch(_DtCvLinkDb link_db, int link_no, int iterations,
                         int (*filter)(), void *client_data, int *ret_link,
                         int *ret_hint) {
        int result = -1;
        char *valueData = NULL;

        if (iterations < 100) {
                result = _DtCvRunInterp(
                    filter, client_data,
                    link_db->list[link_no].info.swtch.interp,
                    link_db->list[link_no].info.swtch.cmd, &valueData);

                if (result == 0) {
                        int value = atoi(valueData);
                        char *idRefs =
                            link_db->list[link_no].info.swtch.branches;
                        char *rid;
                        ...
                        /*
                         * if the value returned is more than the id list or we
                         * hit the end, back up to the first idref
                         */
                        if (value > 0 || *idRefs == '\0')
                                idRefs =
                                    link_db->list[link_no].info.swtch.branches;
                        ...
                        *idRefs = '\0';
                        ...
                        if (0 == result &&
                            _DtLinkTypeSwitch == link_db->list[link_no].type)
                                result = ResolveSwitch(
                                    link_db, link_no, iterations, filter,
 ...
 * Purpose:
 *****************************************************************************/
static int GetNextEntry(_DtCvLinkDb link_db) {
        int i = 0;
        ...
        if (i >= link_db->max) {
                ...
                if (link_db->list != NULL)
                        link_db->list = (_DtCvLinkEntry *)realloc(
                            link_db->list,
 ...
 *
 *****************************************************************************/
int _DtLinkDbGetLinkType(_DtCvLinkDb link_db, int link_index) {
        int value = -1;
 ...
 *
 *****************************************************************************/
int _DtLinkDbGetHint(_DtCvLinkDb link_db, int link_index) {
        int value = -1;
 ...
 *
 *****************************************************************************/
char *_DtLinkDbGetLinkSpec(_DtCvLinkDb link_db, int link_index) {
        char *ptr = NULL;
 ...
 *
 *****************************************************************************/
int _DtLinkDbGetLinkInfo(_DtCvLinkDb link_db, int link_index, int (*filter)(),
                         void *client_data, _DtCvLinkInfo *ret_info) {
        int hint;
        int result = -1;
        char *spec;
                ...
                if (_DtLinkTypeSwitch == link_db->list[link_index].type ||
                    (0 == FindLink(link_db, spec, &link_index, &hint) &&
                     _DtLinkTypeSwitch == link_db->list[link_index].type))
                        result = ResolveSwitch(link_db, link_index, 0, filter,
                                               client_data, &link_index, &hint);
 ...
 *
 *****************************************************************************/
_DtCvLinkDb _DtLinkDbCreate(void) {
 ...
 *
 *****************************************************************************/
void _DtLinkDbDestroy(_DtCvLinkDb link_db) {
        int i;
                ...
                for (i = 0; i < link_db->max; i++) {
                        if (_DtLinkTypeLink == link_db->list[i].type) {
                                ...
                                if (NULL != link_db->list[i].info.link.descrip)
                                        free(
                                            link_db->list[i].info.link.descrip);
 ...
 *
 *****************************************************************************/
int _DtLinkDbAddLink(_DtCvLinkDb link_db, char *id, char *spec, int type,
 ...
 *
 *****************************************************************************/
void _DtLinkDbRemoveLink(_DtCvLinkDb link_db, int link_index) {
        if (link_index < link_db->max &&
            _DtLinkTypeNone != link_db->list[link_index].type) {
                ...
                if (_DtLinkTypeLink == link_db->list[link_index].type) {
                        ...
                        if (NULL != link_db->list[link_index].info.link.descrip)
                                free(link_db->list[link_index]
                                         .info.link.descrip);
                } else if (_DtLinkTypeSwitch ==
                           link_db->list[link_index].type) {
 ...
 *
 *****************************************************************************/
int _DtLinkDbAddSwitch(_DtCvLinkDb link_db, char *id, char *interp, char *cmd,
                       char *branches) {
        int i = GetNextEntry(link_db);
