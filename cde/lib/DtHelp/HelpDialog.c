...
static char *HelpTypeNames[] = {"help_type_topic", "help_type_string",
                                "help_type_man_page", "help_type_file",
...
static void ClassInitialize(void) {
        ...
        if (checkId == XmREP_TYPE_INVALID) {
                ...
                XmRepTypeRegister(DtRDtScrollBarPolicy, ScrollBarValueNames,
                                  NULL, NUM_NAMES(ScrollBarValueNames));
                XmRepTypeRegister(DtRDtExecutionPolicy, ExecutionValueNames,
                                  NULL, NUM_NAMES(ExecutionValueNames));
...
static void VariableInitialize(DtHelpDialogWidget nw) {
        ...
        if (hw->help_dialog.display.topicTitleStr != NULL)
                ...
                hw->help_dialog.display.topicTitleStr = XtNewString(
                    (char *)_DTGETMESSAGE(2, 70, "Nonexistent Topic Title."));
        /*
        ...
        if (hw->help_dialog.print.printer != NULL)
                hw->help_dialog.print.printer =
                    XtNewString(hw->help_dialog.print.printer);
...
static void Initialize(Widget rw, Widget nw, ArgList args_init,
        ...
        if (hw->help_dialog.browser.pathArea != (Widget)NULL)
                XmProcessTraversal(hw->help_dialog.browser.pathArea,
                                   XmTRAVERSE_HOME);
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_dialogShell_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback((Widget)hw, XmNhelpCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);

        /* Just for fun, lets make sure our sizes are correct */
        XtAddCallback(XtParent(hw), XmNpopupCallback,
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
        ...
        if (current->help_dialog.display.helpVolume !=
            hw->help_dialog.display.helpVolume) {
                ...
                /* Update our volumeFlag so we know to resolve our new help
                 * volume */
                hw->help_dialog.ghelp.volumeFlag = FALSE;
        ...
        _DtHelpPrintSetValues(&current->help_dialog.print,
                              &hw->help_dialog.print, &hw->help_dialog.display,
                              &hw->help_dialog.help);
        ...
        if (current->help_dialog.display.manPage !=
            hw->help_dialog.display.manPage) {
                hw->help_dialog.display.manPage =
                    XtNewString(hw->help_dialog.display.manPage);
        ...
        if (hw->help_dialog.display.topicTitleStr != NULL &&
            current->help_dialog.display.topicTitleStr !=
                ...
                if (NULL != hw->help_dialog.display.topicTitleStr) {
                        snprintf(hw->help_dialog.display.topicTitleStr,
                                 len + 10, "%s%d", str,
        ...
        if (newTitle == TRUE) {
                ...
                if (current->help_dialog.display.topicTitleLbl != NULL)
                        XmStringFree(
                            current->help_dialog.display.topicTitleLbl);
        }
...
void _DtHelpDuplicateWindowCB(Widget w, XtPointer clientData,
        ...
        if (hw->help_dialog.display.hyperLinkCallback != NULL) {
                ...
                XtCallCallbackList((Widget)hw,
                                   hw->help_dialog.display.hyperLinkCallback,
                                   &callDataStruct);
...
void _DtHelpDisplayBrowserHomeCB(Widget w, XtPointer client_data,
        ...
        /* ??? Check to see if help volume is always fully expanded at this
         * point
         * ??? if so, we will never have a match.
         */
        if (hw->help_dialog.display.helpVolume == NULL ||
            strstr(hw->help_dialog.display.helpVolume, DtBROWSER_NAME) ==
                NULL) {
                XtFree(hw->help_dialog.display.helpVolume);
                hw->help_dialog.display.helpVolume =
                    XtNewString(DtBROWSER_NAME);
...
static void BuildDisplayArea(Widget parent, DtHelpDialogWidget nw) {
        ...
        hw->help_dialog.help.pDisplayArea = _DtHelpCreateDisplayArea(
            ...
            _DtHelpDialogHypertextCB, ResizeHelpDialogCB,
            _DtHelpFilterExecCmdCB, (XtPointer)hw, defaultList);
...
void _DtHelpSetupDisplayType(DtHelpDialogWidget nw, Boolean vol_changed,
        ...
        /* Here we need to store away our current scroll position as the
         * currentlly displayed item is about replaced w/a new item. We already
         * have placed this item in the jump stack, and now are just adding the
         * proper value for our scrollPosition.
         */
        ...
        case DtHELP_TYPE_TOPIC:
                ...
                if ((hw->help_dialog.ghelp.volumeFlag) &&
                    (hw->help_dialog.display.volumeHandle != NULL))
                        ...
                        validAccessPath = _DtHelpExpandHelpVolume(
                            (Widget)hw, &hw->help_dialog.display,
                            &hw->help_dialog.help, &hw->help_dialog.print);

                /* If the developer gave us a valid Access Path then we can
                 * continue else, we will do nothing
                 */
                if (validAccessPath) {
#if 0
             ...
             /* hw->help_dialog.ghelp.volumeFlag         = TRUE;*/
                        ...
                        _DtHelpUpdateDisplayArea(templocationId, hw,
                                                 vol_changed, updateType,
                                                 DtHELP_TYPE_TOPIC);
                        ...
                        if (hw->help_dialog.display.helpVolume == NULL) {
                                ...
                                tmpError = XtNewString((char *)_DTGETMESSAGE(
                                    2, 60, "No help volume specified."));
                                _DtHelpDisplayFormatError(
                                    hw->help_dialog.help.pDisplayArea,
                                    (Widget)hw, tmpError, NULL);

                                _DtHelpUpdateJumpList(
                                    tmpError, DtHELP_TYPE_DYNAMIC_STRING,
                                    (Widget)hw);
                                hw->help_dialog.display.helpType =
                                    DtHELP_TYPE_DYNAMIC_STRING;
                                hw->help_dialog.display.stringData =
                                    XtNewString(tmpError);
                                XtFree(tmpError);
                        } else {
                                /* We cannot find our HelpVolume so display the
                                 * proper error message in the help dialog and
                                 * continue.
                                 */
                                didWeHaveAnError = TRUE;
                                tmpMessage = XtNewString((char *)_DTGETMESSAGE(
                                    2, 58,
                                    "The requested online help is either not "
                                    "installed "
                                    "or not in the proper help search path.  "
                                    "For "
                                    "information on installing online help, "
                                    "consult "
                                    "the documentation for the product.\n"));
                                locTitle = XtNewString((char *)_DTGETMESSAGE(
                                    2, 62, "Location ID:"));
                                volumeTitle = XtNewString((char *)_DTGETMESSAGE(
                                    2, 61, "Help Volume:"));
                                int tmpError_size =
                                    strlen(tmpMessage) + strlen(locTitle) +
                                    strlen(volumeTitle) +
                                    strlen(hw->help_dialog.display.locationId) +
                                    strlen(hw->help_dialog.display.helpVolume) +
                                    4;
                                ...
                                /* Set current path area to null by giving it a
                                 * null id */
        ...
        case DtHELP_TYPE_STRING:
                ...
                status = _DtHelpFormatAsciiString(
                    hw->help_dialog.help.pDisplayArea,
                    hw->help_dialog.display.stringData, &topicHandle);
                if (status >= VALID_STATUS) {
                        ...
                        _DtHelpUpdateHistoryList(
                            hw->help_dialog.display.stringData,
                            DtHELP_TYPE_STRING, TRUE, (Widget)hw);

                        _DtHelpDisplayAreaSetList(
                            hw->help_dialog.help.pDisplayArea, topicHandle,
                            FALSE, hw->help_dialog.backtr.scrollPosition);
                } else {
                        didWeHaveAnError = TRUE;
                        tmpError = XtNewString((char *)_DTGETMESSAGE(
                            2, 50, "String data could not be formatted."));

                        _DtHelpDisplayFormatError(
                            hw->help_dialog.help.pDisplayArea, (Widget)hw,
                            tmpError, (char *)HDMessage0);

                        _DtHelpUpdateJumpList(
                            tmpError, DtHELP_TYPE_DYNAMIC_STRING, (Widget)hw);
                        hw->help_dialog.display.helpType =
                            DtHELP_TYPE_DYNAMIC_STRING;
                        hw->help_dialog.display.stringData =
                            XtNewString(tmpError);
        ...
        case DtHELP_TYPE_MAN_PAGE:
                ...
                /* Set the string to the current help dialog */
                status = _DtHelpFormatManPage(hw->help_dialog.help.pDisplayArea,
                                              hw->help_dialog.display.manPage,
                                              &topicHandle);
                ...
                if (status >= VALID_STATUS) {
                        ...
                        tmpError = XtNewString((char *)_DTGETMESSAGE(
                            2, 52,
                            "Man Page could not be formatted. The requested "
                            "Man Page is either "
        ...
        case DtHELP_TYPE_FILE:
                ...
                if (status >= VALID_STATUS) {
                        ...
                        _DtHelpUpdateHistoryList(
                            hw->help_dialog.display.helpFile, DtHELP_TYPE_FILE,
                            TRUE, (Widget)hw);
                        _DtHelpDisplayAreaSetList(
                            hw->help_dialog.help.pDisplayArea, topicHandle,
                            FALSE, hw->help_dialog.backtr.scrollPosition);
                        ...
                        tmpError = XtNewString((char *)_DTGETMESSAGE(
                            2, 53,
                            "Text file data could not be formatted. The "
                            "requested text file is "
        ...
        default:
                ...
                didWeHaveAnError = TRUE;
                tmpError = XtNewString((char *)_DTGETMESSAGE(
                    2, 80, "The specified help type is invalid."));
...
void _DtHelpUpdateDisplayArea(char *locationId, DtHelpDialogWidget nw,
        ...
        if (locationId != NULL) {
                ...
                status =
                    _DtHelpFormatTopic(hw->help_dialog.help.pDisplayArea,
        ...
        if (status == 0) /* success */
        {
                if (hw->help_dialog.ghelp.volumeFlag == FALSE) {
                        ...
                        XtGetValues(hw->help_dialog.browser.volumeLabel, args,
                                    1);
                        ...
                        _DtHelpFormatVolumeTitle(
                            hw->help_dialog.help.pDisplayArea,
                            hw->help_dialog.display.volumeHandle, &labelString,
                            &fontList, &mod);
                        ...
                        if (NULL != labelString) {
                                ...
                                XtSetValues(hw->help_dialog.browser.volumeLabel,
                                            args, n);
                                XmStringFree(labelString);
                        }
                        ...
                        /* Update our volumeFlag because we know that its we
                         * have just, opened a new volume. It will get set to
                         * false when we need to process a different help
                         * volume, and remain true while processing the current
                         * help volume.
                         */
                        hw->help_dialog.ghelp.volumeFlag = TRUE;
                } /* if volumeFlag == False */
                ...
                if (listUpdateType == DtHISTORY_AND_JUMP) {
                        ...
                        _DtHelpUpdateJumpList(locationId, topicUpdateType,
                                              (Widget)hw);
                ...
                if (hw->help_dialog.browser.showTopLevelBtn == TRUE) {
                        if (strstr(hw->help_dialog.display.helpVolume,
                                   DtBROWSER_NAME) != NULL) {
                                /* FIX: get top topic of the current volume &
                                 * compare with locationId */
                                if (strcmp(hw->help_dialog.display.locationId,
                                           DtBROWSER_HOME) == 0)
                                        XtSetSensitive(hw->help_dialog.browser
                                                           .btnBoxTopLevelBtn,
                                                       FALSE);
                                else
                                        XtSetSensitive(hw->help_dialog.browser
                                ...
                                XtSetSensitive(
                                    hw->help_dialog.browser.btnBoxTopLevelBtn,
                                    TRUE);
                } /* if showTopLevelBtn */
                ...
                _DtHelpDisplayAreaSetList(
                    hw->help_dialog.help.pDisplayArea, topicHandle, FALSE,
        ...
        if (status < 0) {
                ...
                if (hw->help_dialog.history.pHistoryListHead == NULL) {
                        ...
                        XtSetSensitive(hw->help_dialog.browser.btnBoxHistoryBtn,
                                       FALSE);
                } else {
                ...
                if (hw->help_dialog.backtr.pJumpListHead != NULL) {
                        ...
                        XtSetSensitive(hw->help_dialog.browser.btnBoxBackBtn,
                                       TRUE);
                }
                ...
                if (!validlocationId) {
                        ...
                        if (locationId == NULL) {
                                tmpMsg = (char *)_DTGETMESSAGE(
                                    2, 59, "No location ID specified.");
                                userErrorStr = XtNewString(tmpMsg);
                        } else {
                                tmpMsg = (char *)_DTGETMESSAGE(
                                    2, 55, "Nonexistent location ID:");
                                int userErrorStr_size =
                                    strlen(tmpMsg) +
                                    strlen(hw->help_dialog.display.locationId) +
                                    2;
                                userErrorStr = XtMalloc(userErrorStr_size);
                                strlcpy(userErrorStr, tmpMsg,
                                        userErrorStr_size);
                                strlcat(userErrorStr, " ", userErrorStr_size);
                                strlcat(userErrorStr,
                                        hw->help_dialog.display.locationId,
                                        userErrorStr_size);
                        ...
                        tmpMsg = (char *)_DTGETMESSAGE(
                            2, 54, "Help topic could not be formatted.");
                        userErrorStr = XtNewString(tmpMsg);
...
void _DtHelpUpdateJumpList(char *topicInfo, int topicType, Widget nw) {
        ...
        /* We add a -1 for the scrollbar position value, and will replace it
         * with the actual value just prior to changing the window to the new
         * topic.
         */
        _DtHelpTopicListAddToHead(
            topicInfo, hw->help_dialog.display.topicTitleLbl, topicType,
            DtJUMP_LIST_MAX, hw->help_dialog.display.helpVolume,
...
static void ProcessJumpBack(DtHelpDialogWidget nw) {
        ...
        if (NULL != pTemp->helpVolume &&
            strcmp(hw->help_dialog.display.helpVolume, pTemp->helpVolume) !=
                0) {
                XtFree(hw->help_dialog.display.helpVolume);
                hw->help_dialog.display.helpVolume =
                    XtNewString(pTemp->helpVolume);
        ...
        /* Assign our scrollbar value to our instance structure  so we jump to
         * the proper location
         */
        ...
        /* reset our scrollPosition back to its default here so in any other
         * case, we jump to the top of the topic
         */
        hw->help_dialog.backtr.scrollPosition = -1;
}
 ...
 *
 ****************************************************************************/
void _DtHelpDialogHypertextCB(XtPointer pDisplayAreaStruct,
                              XtPointer clientData,
                              DtHelpHyperTextStruct *hyperData) {
        ...
        case _DtCvLinkType_CrossLink:
                ...
                case _DtCvWindowHint_NewWindow:
                        ...
                        if (hw->help_dialog.display.hyperLinkCallback != NULL) {
                                pTempAccessPath = _DtHelpParseAccessFile(
                                    hyperData->specification);

                                if (pTempAccessPath !=
                                    NULL) /* Use the New one */
                                        callData.helpVolume = pTempAccessPath;
                                else /* Use the old one */
                                        callData.helpVolume = XtNewString(
                                            hw->help_dialog.display.helpVolume);
                                ...
                                XtCallCallbackList(
                                    ...
                                    hw->help_dialog.display.hyperLinkCallback,
                                    &callData);
                        } else { /* The application did not register a hypertext
                                  * callback so we must generate the proper
        ...
        case _DtCvLinkType_TextFile:
                ...
                if (hw->help_dialog.display.hyperLinkCallback != NULL) {
                        ...
                        if (hyperData->hyper_type == DtHELP_LINK_APP_DEFINE) {
                                ...
                                tmpErrorMsg = XtNewString((char *)_DTGETMESSAGE(
                                    2, 57,
                                    "Links to Man Pages are not supported by "
                                    "this application."));
                                _DtHelpErrorDialog(XtParent(hw), tmpErrorMsg);
                        }
                        XtFree(tmpErrorMsg);
...
static void CleanUpHelpDialog(Widget nw, int cleanUpKind) {
        ...
        /* Depending on weither we are just closing this help dialog or
         * destroying it we will do the following!
         */

        if (cleanUpKind == DtCLEAN_FOR_DESTROY) {

                /* Free any remaining char * values we malloc'ed in our help
                 * dialog */
                XtFree(hw->help_dialog.display.locationId);
                XtFree(hw->help_dialog.display.helpVolume);
                ...
                if (hw->help_dialog.history.volumeList != NULL) {
                        ...
                        XtSetValues(hw->help_dialog.history.volumeList, args,
                                    2);
                }
                if (hw->help_dialog.history.topicList != NULL) {
                        XtSetArg(args[0], XmNitems, NULL);
...
static void CloseHelpCB(Widget w, XtPointer clientData, XtPointer callData) {
         ...
         * flush out any info currently in the help, history, and search
         * dialogs.
         */
        ...
        if (hw->help_dialog.ghelp.closeCallback != NULL) {
                ...
                XtCallCallbackList((Widget)hw,
                                   hw->help_dialog.ghelp.closeCallback,
...
static Widget SetupHelpDialogMenus(Widget parent) {
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_FileMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        ...
        /* Print button */
        labelStr =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 1, "Print...")));
        ...
        if (hw->help_dialog.menu.showDupBtn == True) {
                ...
                labelStr = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(2, 5, "New Window...")));
                ...
                c[1] = hw->help_dialog.menu.newWindowBtn =
                    XmCreatePushButtonGadget(menupane1, "newWindowBtn", args,
                                             n);
                XtAddCallback(hw->help_dialog.menu.newWindowBtn,
                              XmNactivateCallback, _DtHelpDuplicateWindowCB,
                              (XtPointer)hw);
                XmStringFree(labelStr);

                /* Exit button */
                labelStr = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(2, 3, "Close")));
                ...
                XtAddCallback(hw->help_dialog.menu.closeBtn,
                              XmNactivateCallback, CloseHelpCB, (XtPointer)hw);
                XmStringFree(labelStr);

                /* set the cancel button (for KCancel) */
                hw->bulletin_board.cancel_button =
                    hw->help_dialog.menu.closeBtn;
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_EditMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        ...
        /* Edit button */
        labelStr =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 7, "Copy")));
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_SearchMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        ...
        /* Search button */
        labelStr =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 9, "Index...")));
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_NavigateMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        ...
        /* Back Button */
        labelStr = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(2, 13, "Backtrack")));
        ...
        /* Top Button */
        labelStr = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(2, 11, "Home Topic")));
        ...
        /* History Button */
        labelStr = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(2, 15, "History...")));
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_HelpMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        ...
        n++;
        c[0] = helpBtn1 =
            XmCreatePushButtonGadget(menupane5, "usingHelp", args, n);
        pHelpInfo = _DtHelpListAdd(DtHELP_onHelpMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(helpBtn1, XmNactivateCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_onTableOfContenseMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(helpBtn2, XmNactivateCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
        ...
        labelStr =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 21, "Tasks")));
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_onTasksMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(helpBtn3, XmNactivateCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
        XmStringFree(labelStr);

        /* On Reference button */
        labelStr = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(2, 23, "Reference")));
        ...
        n++;
        c[4] = helpBtn4 =
            XmCreatePushButtonGadget(menupane5, "reference", args, n);
        pHelpInfo = _DtHelpListAdd(DtHELP_onReferenceMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(helpBtn4, XmNactivateCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
        XmStringFree(labelStr);
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_onMouseAndKeysMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(helpBtn5, XmNactivateCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
        XmStringFree(labelStr);
        ...
        /* On Version */
        labelStr = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(2, 27, "About Help")));
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_onVersionMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(helpBtn6, XmNactivateCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
        XmStringFree(labelStr);
        ...
        /* File Menu */
        labelStr =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 29, "File")));
        ...
        /* Edit Menu */
        labelStr =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 37, "Edit")));
        ...
        /* Search Menu */
        labelStr =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 31, "Search")));
        ...
        /* Help Menu */
        labelStr =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(2, 35, "Help")));
...
static Widget CreatePopupMenu(Widget nw) {
        ...
        char *mnemonic;
        int numButtons =
            XGetPointerMapping(XtDisplay(nw), (unsigned char *)NULL, 0);
        ...
        XtAddEventHandler(
            _DtHelpDisplayAreaWidget(hw->help_dialog.help.pDisplayArea),
            ButtonPressMask, FALSE, (XtEventHandler)ProcessBMenuBackCB, hw);

        /* Add Help callback for Popup Menu */
        pHelpInfo = _DtHelpListAdd(DtHELP_PopupMenu_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        ...
        /* Popup Label*/
        labelStr =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(3, 4, "Help")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelStr);
        n++;
