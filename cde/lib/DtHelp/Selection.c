...
static void AdjustSelection(_DtCanvasStruct *canvas, _DtCvSelectData next);
static int GetSelectedText(_DtCanvasStruct *canvas, _DtCvSelectData start,
                           _DtCvSelectData end, unsigned int mask,
 ...
 *****************************************************************************/
static _DtCvUnit StartXOfLine(_DtCanvasStruct *canvas, _DtCvDspLine line) {
        ...
        return (
            _DtCvAdvanceXOfLine(canvas, pSeg, xPos, &lnkInd, &lastLinkVisible));
 ...
 *
 *****************************************************************************/
static void SearchForClosestLine(_DtCanvasStruct *canvas, _DtCvUnit target_x,
                                 _DtCvUnit target_y, _DtCvSelectData *next) {
        int i;
        int maxI;
        _DtCvUnit lineY;
        _DtCvUnit endX;
        ...
        *next = defaultSelect;
        for (i = 0; i < canvas->txt_cnt; i++) {
                ...
                if (_DtCvStraddlesPt(
                        target_y, lines[i].baseline - lines[i].ascent, lineY)) {
                        /*
                         * Is this the minimum y of all the maximum y values of
                         * the line straddling the target y?
                         */
                        ...
                        if (_DtCvStraddlesPt(target_x, begX, endX)) {
                                next->line_idx = i;
                                next->char_idx =
                                    _DtCvGetCharIdx(canvas, lines[i], target_x);
                        }
                }
        }
 ...
 *
 *****************************************************************************/
static void MarkLinesOutsideBoundary(_DtCanvasStruct *canvas, _DtCvUnit top_y,
                                     _DtCvUnit top_x, _DtCvUnit bot_y,
                                     _DtCvUnit bot_x) {
        int i;
        _DtCvUnit maxY;
        _DtCvUnit minY;
...
static void AdjustSelection(_DtCanvasStruct *canvas, _DtCvSelectData next) {
        ...
        if (!(Equal(next, end))) {
                if (next.line_idx != -1 &&
                    next.line_idx == canvas->select_end.line_idx &&
                    next.char_idx != -1 &&
                    next.char_idx == canvas->select_end.char_idx)
                        return;

                if (GreaterThan(next, end)) {
                        if (LessThanEq(start, end))
                                _DtCvDrawAreaWithFlags(canvas, end, next, 0,
                                                       _DtCvSELECTED_FLAG,
                                                       _DtCvBAD_TYPE, NULL);
                        ...
                        else /* end < start < next */
                        {
                                _DtCvDrawAreaWithFlags(canvas, end, start,
                                                       _DtCvSELECTED_FLAG, 0,
                                                       _DtCvBAD_TYPE, NULL);
                                _DtCvDrawAreaWithFlags(canvas, start, next, 0,
                                                       _DtCvSELECTED_FLAG,
                                                       _DtCvBAD_TYPE, NULL);
                        }
                } else /* if (next < end) */
                {
                        if (LessThanEq(start, next))
                                ...
                                _DtCvDrawAreaWithFlags(canvas, next, end, 0,
                                                       _DtCvSELECTED_FLAG,
                                                       _DtCvBAD_TYPE, NULL);
                        ...
                        else /* next < start < end */
                        {
                                _DtCvDrawAreaWithFlags(canvas, start, end,
                                                       _DtCvSELECTED_FLAG, 0,
                                                       _DtCvBAD_TYPE, NULL);
                                _DtCvDrawAreaWithFlags(canvas, next, start, 0,
                                                       _DtCvSELECTED_FLAG,
                                                       _DtCvBAD_TYPE, NULL);
                        }
                }
 ...
 *
 *****************************************************************************/
static void SkipOtherLines(_DtCvDspLine *lines, int max_cnt, int idx,
                           _DtCvUnit target_y, int *ret_idx) {
 ...
 *
 *****************************************************************************/
static void CheckAndSwitchPoints(_DtCvSelectData *pt1, _DtCvSelectData *pt2) {
 ...
 *
 *****************************************************************************/
static _DtCvUnit AddSegmentToData(_DtCanvasStruct *canvas, unsigned int mask,
                                  _DtCvUnit start_x, int line_idx, int char_idx,
                                  int copy_cnt, _DtCvFlags end_flag,
                                  _DtCvUnit *ret_y, _DtCvPointer *ret_data) {
        ...
        while (_DtCvSTATUS_OK == result && pSeg != NULL && copy_cnt > 0) {
                ...
                case _DtCvSTRING:
                        ...
                        if (canvas->virt_functions.build_selection != NULL)
                                result =
                                    (*(canvas->virt_functions.build_selection))(
                                        canvas->client_data, _DtCvSTRING_TYPE,
                                        mask, ret_data, xPos - start_x,
                                        segWidth, flag, (_DtCvPointer)&strInfo);
                ...
                case _DtCvREGION:
                        ...
                        if (canvas->virt_functions.build_selection != NULL)
                                result =
                                    (*(canvas->virt_functions.build_selection))(
                                        canvas->client_data, _DtCvREGION_TYPE,
                                        mask, ret_data, xPos - start_x,
                                        _DtCvWidthOfRegionSeg(pSeg), flag,
 ...
 *
 *****************************************************************************/
static int BuildLine(_DtCanvasStruct *canvas, unsigned int mask,
                     _DtCvUnit prev_y, _DtCvUnit target_x, int line_idx,
                     int char_idx, int copy_cnt, _DtCvFlags end_flag,
        ...
        if (topY > prev_y && canvas->virt_functions.build_selection != NULL) {
                ...
                while (newLines > 0) {
                        ...
                        if ((*(canvas->virt_functions.build_selection))(
                                canvas->client_data, _DtCvSTRING_TYPE, mask,
                                ret_data, 0, 0, _DtCvEND_OF_LINE,
 ...
 *
 *****************************************************************************/
static int FindMinX(_DtCvDspLine *lines, int txt_cnt, _DtCvUnit target_y,
                    int *ret_line) {
        int i;
        int cnt = 0;
        _DtCvUnit curMin = -1;
 ...
 *
 *****************************************************************************/
static _DtCvValue FindNextMinY(_DtCvDspLine *lines, int max_cnt,
                               _DtCvUnit target_y, _DtCvUnit *ret_y) {
        int i = 0;
        _DtCvUnit maxY;
 ...
 *
 *****************************************************************************/
static int GetSelectedText(_DtCanvasStruct *canvas, _DtCvSelectData next,
                           _DtCvSelectData end, unsigned int mask,
                           _DtCvPointer *ret_data) {
        _DtCvUnit maxY;
        _DtCvUnit botY;
        ...
        while (processing == True && result == 0) {
                ...
                do {
                        ...
                        if (next.line_idx == end.line_idx)
                                cpyCnt = cpyCnt - lines[next.line_idx].length +
                                         end.char_idx;
                        else if (lineCnt == 1)
                                endFlag = _DtCvEND_OF_LINE;

                        result = BuildLine(canvas, mask, maxY, next.x,
                                           next.line_idx, next.char_idx, cpyCnt,
                                           endFlag, &next.x, &botY, ret_data);
                        ...
                        next.char_idx = 0;
                        lineCnt =
                            FindMinX(lines, txtCnt, next.y, &next.line_idx);
                ...
                if (result == 0) {
                        ...
                        if (processing == True)
                                lineCnt = FindMinX(lines, txtCnt, next.y,
                                                   &next.line_idx);
                }
        }
 ...
 *
 *****************************************************************************/
static _DtCvStatus GetSegsInArea(_DtCanvasStruct *canvas, _DtCvSelectData *beg,
                                 _DtCvSelectData *end, _DtCvSegPts ***ret_segs,
        ...
        *ret_segs = NULL;
        ...
        while (processing == True && result == 0) {
                ...
                while (result == 0 && lineCnt > 0) {
                        ...
                        while (0 == result && NULL != pSeg && 0 < length) {
                                /*
                                 * start with error condition. If the malloc
                                 * works the error result gets reset to valid.
                                 */
                                result = -1;
                                newPt =
                                    (_DtCvSegPts *)malloc(sizeof(_DtCvSegPts));
                                if (NULL != newPt) {
                                        /*
                                        ...
                                        _DtCvGetWidthOfSegment(
                                            canvas, pSeg, start, length, &cnt,
                                            NULL, NULL);

                                        /*
                                         * now set the segment point information
                                         * and add it to the array of segment
                                         * points.
                                         */
                                        newPt->offset = start;
                                        newPt->len = cnt;
                                        ...
                                        *ret_segs =
                                            (_DtCvSegPts **)_DtCvAddPtrToArray(
                                                (void **)*ret_segs,
                                                (void *)newPt);
                                        if (NULL == *ret_segs)
                                                result = -1;
                         ...
                         */
                        botY = lines[next.line_idx].baseline +
                               lines[next.line_idx].descent;
                        if (botY > maxY)
                                maxY = botY;
                ...
                if (result == 0) {
                        next.x = 0;
                        processing =
                            FindNextMinY(lines, canvas->txt_cnt, -1, &next.y);
                        if (processing == True)
                                lineCnt = FindMinX(lines, canvas->txt_cnt,
                                                   next.y, &next.line_idx);
                }
        }
        ...
        if (0 != result) {
                if (NULL != *ret_segs) {
                        for (lineCnt = 0; NULL != (*ret_segs)[lineCnt];
                             lineCnt++)
 ...
 *
 *****************************************************************************/
void _DtCvDrawAreaWithFlags(_DtCanvasStruct *canvas, _DtCvSelectData start,
                            _DtCvSelectData end, _DtCvFlags old_flags,
                            _DtCvFlags new_flags, _DtCvElemType trav_type,
        ...
        for (i = 0; i < canvas->txt_cnt; i++) {
                ...
                if (InRegion(topY, botY, start.y, end.y)) {
                         ...
                         */
                        dstX = _DtCvAdvanceXOfLine(canvas, pSeg, dstX, &lnkInd,
                                                   &lstLinkVis);
                        /*
                         * take into account super/sub scripting
                         */
                        dstX = _DtCvAdjustForSuperSub(
                            canvas, pSeg, dstX, &scriptX, &superWidth, &superY,
                            &subWidth, &subY, &lstWasSuper, &lstWasSub);

                        /*
                         * set this flag so that the first pass of 'while (cnt >
                         * 0)' doesn't do it again.
                         */
                        trimmed = True;
                        ...
                        if (_DtCvStraddlesPt(start.y, topY, botY)) {
                                /*
                                 * skip this item?
                                 * I.E. is this line before the start or after
                                 * the end?
                                 */
                                ...
                                /*
                                 * otherwise this line is after the line that
                                 * starts the mark/selection. Stick with its
                                 * start x.
                                 */
                        }
                        ...
                        if (_DtCvStraddlesPt(end.y, topY, botY)) {
                                /*
                                 * does this start after the end of the
                                 * mark/selection? if so, skip.
                                 */
                                if (end.x <= dstX)
                                        continue;

                                /*
                                 * Does this segment end after the end of the
                                 * mark/selection? If so, trim how much gets
                                 * highlighted.
                                 */
                                if (canvas->txt_lst[i].max_x > end.x)
                                        count -=
                                            (lines[i].length - end.char_idx);
                        }
                        ...
                        while (count > 0) {
                                 ...
                                 */
                                _DtCvCheckLineMarks(
                                    canvas, i, startChar - lines[i].byte_index,
                                    count, dstX, flagMask, &len, &old_flags,
                                    &new_flags);

                                /*
                                 * if this is the last segment(s) of the
                                 * (un)mark/selection set the end flags.
                                 */
                                if (len == count) {
                                        new_flags |= (endFlag | _DtCvLINK_END |
                                                      _DtCvMARK_END);
                                        old_flags |= (endFlag | _DtCvLINK_END |
                                                      _DtCvMARK_END);
                                }
                                ...
                                dstX = _DtCvDrawSegments(
                                    canvas, lines[i], pSeg, startChar, len,
                                    &lnkInd, dstX, dstX, &scriptX, &superWidth,
                                    &superY, &subWidth, &subY, &lstWasSub,
                                    &lstWasSuper, &lstLinkVis, old_flags,
                                    new_flags, trav_type, trav_data);
                                /*
                                ...
                                /*
                                 * did this do the entire length? If not, set
                                 * the indexes ahead and do again.
                                 */
                                if (count > 0)
                                        _DtCvSkipLineChars(canvas, pSeg,
                                                           startChar,
                                                           count + len, len,
                                                           &startChar, &pSeg);
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCvGetMarkSegs(_DtCanvasStruct *canvas,
        ...
        *ret_info = NULL;
        ...
        for (i = 0; i < canvas->mark_cnt; i++) {
                ...
                if (_DtCvSTATUS_BAD ==
                    GetSegsInArea(canvas, &(canvas->marks[i].beg),
                                  &(canvas->marks[i].end), &(nxtInfo->segs),
                                  NULL, NULL))
                        return _DtCvSTATUS_BAD;

                *ret_info = (_DtCvPointInfo **)_DtCvAddPtrToArray(
                    (void **)*ret_info, (void *)nxtInfo);
                if (NULL == *ret_info)
                        return _DtCvSTATUS_BAD;
        }
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCanvasGetSelection(_DtCvHandle canvas_handle, unsigned int mask,
        ...
        *ret_select = NULL;
        return (GetSelectedText(canvas, canvas->select_start,
                                canvas->select_end, mask, ret_select));
}
 ...
 *
 *****************************************************************************/
void _DtCanvasProcessSelection(_DtCvHandle canvas_handle, _DtCvUnit x,
                               _DtCvUnit y, _DtCvSelectMode mode) {
        _DtCanvasStruct *canvas = (_DtCanvasStruct *)canvas_handle;
        _DtCvSelectData temp;
        ...
        case _DtCvSELECTION_START:
                _DtCvDrawAreaWithFlags(canvas, canvas->select_start,
                                       canvas->select_end, _DtCvSELECTED_FLAG,
                                       0, _DtCvBAD_TYPE, NULL);

                canvas->select_start = defaultSelect;
                if (mode == _DtCvSELECTION_START)
                        SearchForClosestLine(canvas, x, y,
                                             &(canvas->select_start));
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCanvasGetSelectionPoints(_DtCvHandle canvas_handle,
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCanvasActivatePts(_DtCvHandle canvas_handle, unsigned int mask,
                                 _DtCvPointInfo *info, _DtCvUnit *ret_y1,
                                 _DtCvUnit *ret_y2) {
        int markIdx;
        _DtCanvasStruct *canvas = (_DtCanvasStruct *)canvas_handle;
...
#define REQUIRE_SEGS (_DtCvACTIVATE_MARK | _DtCvACTIVATE_SELECTION)
        /*
        ...
        if (((mask & _DtCvDEACTIVATE) && NULL == info->client_data) ||
            (mask & REQUIRE_SEGS)) {
                if (NULL == info || NULL == info->segs ||
                    _DtCvSTATUS_BAD ==
                        _DtCvCvtSegsToPts(canvas, info->segs, &startSel,
                                          &endSel, ret_y1, ret_y2, &firstSeg))
                        return _DtCvSTATUS_BAD;
        }
        ...
        if (mask & _DtCvACTIVATE_SELECTION) {
                _DtCanvasProcessSelection(canvas_handle, 0, 0,
                                          _DtCvSELECTION_CLEAR);

                canvas->select_start = startSel;
                canvas->select_end = endSel;

                _DtCvDrawAreaWithFlags(canvas, startSel, endSel, 0,
                                       _DtCvSELECTED_FLAG, _DtCvBAD_TYPE, NULL);
        }
        ...
        if (mask & _DtCvACTIVATE_MARK) {
                ...
                markIdx = _DtCvAddToMarkList(canvas, info->client_data,
                                             _DtCvIsMarkMaskOn(mask), &startSel,
                                             &endSel);
                if (-1 == markIdx)
                        return _DtCvSTATUS_BAD;
                ...
                if (-1 == travIdx ||
                    0 != _DtCvSetTravEntryInfo(canvas, travIdx,
                                               ...
                                               markIdx, _DtCvTRUE) ||
                    0 != _DtCvCalcMarkPos(canvas, markIdx, &x, &y, &width,
                                          &height) ||
                    0 != _DtCvSetTravEntryPos(canvas, travIdx, x, y, width,
                                              height))
                        return _DtCvSTATUS_BAD;
                ...
                _DtCvDrawAreaWithFlags(canvas, startSel, endSel, 0, flag,
                                       _DtCvBAD_TYPE, NULL);
        }
         ...
         */
                 ...
                 */
                        ...
                        while (markIdx < canvas->mark_cnt &&
                               startSel.line_idx !=
                                   canvas->marks[markIdx].beg.line_idx &&
                               startSel.char_idx !=
                                   canvas->marks[markIdx].beg.char_idx &&
                               endSel.line_idx !=
                                   canvas->marks[markIdx].end.line_idx &&
                               endSel.line_idx !=
                                   canvas->marks[markIdx].end.char_idx)
                                markIdx++;
                }
                 ...
                 */
                for (travIdx = 0;
                     _DtCvTraversalMark != canvas->trav_lst[travIdx].type ||
                     ...
                     travIdx++) {
                         ...
                         */
                        if (_DtCvTraversalMark ==
                                canvas->trav_lst[travIdx].type &&
                            markIdx < canvas->trav_lst[travIdx].idx)
                                canvas->trav_lst[travIdx].idx--;
                }

                /*
                 * move the list of traversal entries to eliminate the mark
                 * entry.
                 */
                while (travIdx + 1 < canvas->trav_cnt) {
                        canvas->trav_lst[travIdx] =
                            canvas->trav_lst[travIdx + 1];

                        /*
                         * is this a mark after the one being removed?
                         * if so, decrease it's index because it's about to
                         * move.
                         */
                        if (_DtCvTraversalMark ==
                                canvas->trav_lst[travIdx].type &&
                ...
                /*
                 * update the traversal count and back up to the previous
                 * traversal if the mark was at the end of the traversal list.
                 */
                canvas->trav_cnt--;
                ...
                if (NULL != info && NULL != info->client_data) {
                        ...
                        /*
                         * are there any traversals? Is the current one sitting
                         * on a mark?
                         */
                        if (0 == canvas->trav_cnt || -1 == canvas->cur_trav ||
                            _DtCvTraversalMark !=
                                canvas->trav_lst[canvas->cur_trav].type)
                                return _DtCvSTATUS_BAD;
                ...
                /*
                 * is this different than what it is set at now? If not, do
                 * nothing.
                 */
 ...
 *
 *****************************************************************************/
_DtCvStatus _DtCanvasGetMarkPositions(_DtCvHandle canvas_handle,
        ...
        *ret_pos = NULL;
        ...
        for (i = 0; i < canvas->mark_cnt; i++, markLst++) {
                ...
                nextPos->baseline1 =
                    canvas->txt_lst[markLst->beg.line_idx].baseline;
                nextPos->baseline2 =
                    canvas->txt_lst[markLst->end.line_idx].baseline;
                ...
                *ret_pos = (_DtCvMarkPos **)_DtCvAddPtrToArray(
                    (void **)*ret_pos, (void *)nextPos);
                if (NULL == *ret_pos)
                        return _DtCvSTATUS_BAD;
        }
