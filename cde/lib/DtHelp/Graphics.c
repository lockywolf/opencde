...
/*
static char rcs_id[]="$XConsortium: Graphics.c /main/23 1996/12/06 11:12:54
cde-hp $";
*/
...
static char *GreyScaleColors[MAX_GREY_COLORS] = {
    "#212121212121", "#424242424242", "#636363636363", "#737373737373",
    "#949494949494", "#adadadadadad", "#bdbdbdbdbdbd", "#dededededede",
};
 ...
 */
static char *img_extensions[] = {"xpm",  "PM",   "pm",   "PM",   "gif", "GIF",
                                 "jpeg", "JPEG", "jpg",  "JPEG", "xbm", "BM",
                                 "bm",   "BM",   "tiff", "TIFF", "tif", "TIFF",
                                 "xwd",  "XWD",  "cgm",  "CGM",  NULL};

/*****************************************************************************/
/*		      Private declarations				     */
/*****************************************************************************/

static int Do_Direct(Display *dpy, int screen, XWDFileHeader *header,
                     Colormap colormap, int ncolors, XColor *colors,
                     enum _DtGrColorModel force, XImage *in_image,
                     XImage *out_image, unsigned long **ret_colors,
                     int *ret_number);
static int Do_Pseudo(Display *dpy, int screen, Colormap colormap, int ncolors,
                     XColor *colors, enum _DtGrColorModel force,
                     XImage *in_image, XImage *out_image,
                     unsigned long **ret_colors, int *ret_number);
static int GreyScale(Display *dpy, int screen, Colormap cmap, XImage *in_image,
                     XImage *out_image, XColor *colors, int ncolors,
                     enum _DtGrColorModel force, int rshift, int gshift,
                     int bshift, Pixel rmask, Pixel gmask, Pixel bmask);
static unsigned int Image_Size(XImage *image);
static void Perform_Dither(Display *dpy, int screen, XImage *image,
                           int *greyscale);
static enum _DtGrLoadStatus
processBitmap(_DtGrStream *image, Screen *screen, int depth, Colormap colormap,
...
#ifndef STUB
static enum _DtGrLoadStatus
processTiff(_DtGrStream *stream, Screen *screen, int depth, Colormap colormap,
            Visual *visual, Pixel foreground, Pixel background, GC gc,
            enum _DtGrColorModel color_model, Boolean allow_reduced_colors,
            Dimension *in_out_width, Dimension *in_out_height,
            unsigned short media_resolution, Pixmap *ret_pixmap,
            Pixmap *ret_mask, Pixel **ret_colors, int *ret_num_colors,
            _DtGrContext *context);
static void destroyTiffContext(_DtGrContext *context);
#endif
static enum _DtGrLoadStatus
processXwd(_DtGrStream *stream, Screen *screen, int depth, Colormap colormap,
           Visual *visual, Pixel foreground, Pixel background, GC gc,
           enum _DtGrColorModel color_model, Boolean allow_reduced_colors,
           Dimension *in_out_width, Dimension *in_out_height,
           unsigned short media_resolution, Pixmap *ret_pixmap,
...
static int XwdFileToPixmap(Display *dpy, int screen, int depth, Pixmap pixmap,
                           Colormap colormap, Visual *visual, GC gc,
                           enum _DtGrColorModel color_model, int src_x,
                           int src_y, int dst_x, int dst_y, int width,
                           int height, _DtGrStream *stream,
                           unsigned long **ret_colors, int *ret_number);
static enum _DtGrLoadStatus
processXpm(_DtGrStream *stream, Screen *screen, int depth, Colormap colormap,
           Visual *visual, Pixel foreground, Pixel background, GC gc,
           enum _DtGrColorModel color_model, Boolean allow_reduced_colors,
           Dimension *in_out_width, Dimension *in_out_height,
           unsigned short media_resolution, Pixmap *ret_pixmap,
           ...
           _DtGrContext *context);
static enum _DtGrLoadStatus
processGIF(_DtGrStream *image, Screen *screen, int depth, Colormap colormap,
           Visual *visual, Pixel foreground, Pixel background, GC gc,
           enum _DtGrColorModel color_model, Boolean allow_reduced_colors,
           Dimension *in_out_width, Dimension *in_out_height,
           unsigned short media_resolution, Pixmap *ret_pixmap,
           ...
           _DtGrContext *context);
static enum _DtGrLoadStatus
processJPEG(_DtGrStream *image, Screen *screen, int depth, Colormap colormap,
            Visual *visual, Pixel foreground, Pixel background, GC gc,
            enum _DtGrColorModel color_model, Boolean allow_reduced_colors,
            Dimension *in_out_width, Dimension *in_out_height,
            unsigned short media_resolution, Pixmap *ret_pixmap,
            ...
            _DtGrContext *context);
static enum _DtGrLoadStatus
DetermineImageType(_DtGrStream *stream, char *image_type, int image_type_size);
static enum _DtGrLoadStatus
GetConverterAndDestructor(char *image_type, _DtGrLoadProc *converter,
...
static _DtGrRegistryRec registry[] = {
    ...
    {"BM", processBitmap, NULL},

    {"GIF", processGIF, NULL},

    {"JPEG", processJPEG, NULL},

    {"TIFF", processTiff, destroyTiffContext},
...
static int GreyScale(Display *dpy, int screen, Colormap cmap, XImage *in_image,
        ...
        for (j = 0, x = 0; j < height; j++)
                for (i = 0; i < width; i++, x++) {
                        ...
                        if (rshift) {
                                ...
                                if (ncolors) {
                                        ret_color.red =
                                            colors[ret_color.red].red;
                                        ret_color.green =
                                            colors[ret_color.green].green;
                                        ret_color.blue =
                                            colors[ret_color.blue].blue;
                                } else {
                                        ret_color.red =
                                            (((Pixel)ret_color.red) * 65535) /
                                            rmask;
                                        ret_color.green =
                                            (((Pixel)ret_color.green) * 65535) /
                                            gmask;
                                        ret_color.blue =
                                            (((Pixel)ret_color.blue) * 65535) /
                                            bmask;
                                }

                                value = (((int)(ret_color.red * 299) +
                                          (int)(ret_color.green * 587) +
                                          (int)(ret_color.blue * 114)) /
                                         1000) >>
                                        8;
                        } else
                                value = (((int)(colors[n].red * 299) +
                                          (int)(colors[n].green * 587) +
                                          (int)(colors[n].blue * 114)) /
                                         1000) >>
                                        8;
                        grey_scale[x] = value;
                        valueArray[value]++;
        ...
        if (!GreyAllocated && force != _DtGrBITONAL) {
                for (i = 0; !GreyAllocated && i < MAX_GREY_COLORS; i++) {
                        if (!XParseColor(dpy, cmap, GreyScaleColors[i],
                                         &ret_color) ||
                            !XAllocColor(dpy, cmap, &ret_color)) {
                                /*
                                 * Have a problem allocating one of the
                                 * pre-defined grey colors. Free the already
                                 * allocated pixels and set the flag.
                                 */
                                if (i)
                                        XFreeColors(dpy, cmap, GreyScalePixels,
                                                    i, 0);
                                GreyAllocated = -1;
                        } else
         ...
         */
        for (i = 0, n = 0, j = 0; n < 256 && count && i < MAX_GREY_COLORS;
             n++) {
                if (valueArray[n]) {
                        _DtHelpProcessLock();
                        valueArray[n] =
                            GreyScalePixels[i * MAX_GREY_COLORS / inc];
                        _DtHelpProcessUnlock();
                        j++;
                        if (j >= count / (MAX_GREY_COLORS - i)) {
                                count -= j;
        ...
        for (j = 0; j < height; j++)          /* rows */
                for (i = 0; i < width; i++) { /* columns */
                        ...
                        if (i + 1 < width) /*--- G [j] [i+1] ---*/
                                greyscale[(j * width) + i + 1] +=
                                    (error * 8) / 42;
                        if (i + 2 < width) /*--- G [j] [i+2] ---*/
                                greyscale[(j * width) + i + 2] +=
                                    (error * 4) / 42;

                        if (j + 1 < height) {
                                if (i - 2 >= 0) /*--- G [j+1] [i-2] ---*/
                                        greyscale[((j + 1) * width) + i - 2] +=
                                            (error * 2) / 42;
                                if (i - 1 >= 0) /*--- G [j+1] [i-1] ---*/
                                        greyscale[((j + 1) * width) + i - 1] +=
                                            (error * 4) / 42;
                                /*--- G [j+1] [i] ---*/
                                greyscale[((j + 1) * width) + i] +=
                                    (error * 8) / 42;
                        ...
                        if (j + 2 < height) {
                                if (i - 2 >= 0) /*--- G [j+2] [i-2] ---*/
                                        greyscale[((j + 2) * width) + i - 2] +=
                                            error / 42;
                                if (i - 1 >= 0) /*--- G [j+2] [i-1] ---*/
                                        greyscale[((j + 2) * width) + i - 1] +=
                                            (error * 2) / 42;
                                /*--- G [j+2] [i] ---*/
                                greyscale[((j + 2) * width) + i] +=
                                    (error * 4) / 42;
...
static int Do_Pseudo(Display *dpy, int screen, Colormap colormap, int ncolors,
        ...
        *ret_number = 0;
        ...
        if (!result) {
                ...
                result = GreyScale(dpy, screen, colormap, in_image, out_image,
                                   colors, ncolors, force, 0, 0, 0, 0, 0, 0);
        } else if (colorCount) {
                result = GR_SUCCESS;
                *ret_colors =
                    (unsigned long *)malloc(sizeof(unsigned long) * colorCount);
                if (*ret_colors == NULL) {
                        colorCount = 0;
                        result = GR_ALLOC_ERR;
                }
...
static int Do_Direct(Display *dpy, int screen, XWDFileHeader *header,
                ...
                for (x = 0; color.flags && x < in_image->width; x++) {
                        ...
                        if (i == pixI) {
                                ...
                                if (!XAllocColor(dpy, colormap, &color))
                                        ...
                                        if (pixI >= pixMax) {
                                                pixMax += 128;
                                                oldPixels = (Pixel *)realloc(
                                                    (void *)oldPixels,
                                                    (sizeof(Pixel) * pixMax));
                                                newPixels = (Pixel *)realloc(
                                                    (void *)newPixels,
                                                    (sizeof(Pixel) * pixMax));
        ...
        if (color.flags) {
                ...
                *ret_number = pixI;
                ...
                result = GreyScale(dpy, screen, colormap, in_image, out_image,
                                   colors, ncolors, force, rshift, gshift,
                                   bshift, rmask, gmask, bmask);
        }
...
static int XwdFileToPixmap(Display *dpy, int screen, int depth, Pixmap pixmap,
...
#ifdef __alpha
                /* Use XWDColor instead of XColor. Byte-swapping if it is
                 * necessary. Move values back into Xcolor structure.
                 */
                for (i = 0; i < ncolors; i++) {
                        if (_DtGrRead((char *)&xwd_color, sizeof(XWDColor), 1,
                                      stream) != 1) {
                                XFree((char *)colors);
                                return GR_FILE_ERR;
                        }
                        ...
                        if (*(char *)&swaptest) {
                                _swaplong((char *)&xwd_color.pixel,
                                          sizeof(xwd_color.pixel));
                                _swapshort((char *)&xwd_color.red,
                                           3 * sizeof(xwd_color.red));
                        }
...
#else
                if (_DtGrRead((char *)colors, sizeof(XColor), ncolors,
                              stream) != ncolors) {
                        XFree((char *)colors);
                        return GR_FILE_ERR;
                }
        ...
        if (rawbits) {
                ...
                out_image = XCreateImage(
                    dpy, visual, depth,
                    (depth == 1) ? XYBitmap : in_image.format, in_image.xoffset,
                    NULL, in_image.width, in_image.height, XBitmapPad(dpy), 0);
                ...
                if ((header.visual_class == TrueColor) ||
                    (header.visual_class == DirectColor))
                        result =
                            Do_Direct(dpy, screen, &header, colormap, ncolors,
                                      colors, color_model, &in_image, out_image,
                                      ret_colors, ret_number);
                else
                        result = Do_Pseudo(dpy, screen, colormap, ncolors,
                                           colors, color_model, &in_image,
                                           out_image, ret_colors, ret_number);
        }

        if (result != GR_ALLOC_ERR)

                _XmPutScaledImage(dpy, pixmap, gc, out_image, src_x, src_y,
                                  dst_x, dst_y, in_image.width, in_image.height,
                                  width, height);
 ...
 ***********/
static enum _DtGrLoadStatus
processBitmap(_DtGrStream *stream, Screen *screen, int depth, Colormap colormap,
              Visual *visual, Pixel foreground, Pixel background, GC gc,
              enum _DtGrColorModel color_model, Boolean allow_reduced_colors,
              Dimension *in_out_width, Dimension *in_out_height,
              unsigned short media_resolution, Pixmap *ret_pixmap,
        ...
        result = _DtGrReadBitmapStreamData(stream, &width, &height, &data,
                                           &junk, &junk);
...
processTiff(_DtGrStream *stream, Screen *screen, int depth, Colormap colormap,
        ...
        if (*tiff_xwc == NULL) {
                ...
                *tiff_xwc =
                    ilCreateXWC(IlContext, dpy, visual, colormap, gc, 0, 0);
                if (*tiff_xwc == NULL) {
                        _DtHelpProcessUnlock();
                        return (_DtGrCONVERT_FAILURE);
                }
        ...
        if (inFile) {
                ...
                if (inImage) {
                        int image_resolution =
                            100; /* assume that if not in file */

                        if (inImage->xRes != 0) {
                                image_resolution =
                                    inImage->xRes; /* dpi from the file */
                        }

                        if (*in_out_width == 0 && *in_out_height == 0)
                                ratio = ComputeRatio(media_resolution,
                                                     image_resolution);
                        ...
                        *in_out_height = inImage->height * ratio + 0.5;
                        ...
                        if (pixmap) {
                                ...
                                if (inPipe) {
                                        ...
                                        result = 0;
                                        if (!ilReadFileImage(inPipe, inImage,
                                                             (ilRect *)NULL, 0))
                                                result = -1;

                                        if (result == 0 &&
                                            color_model != _DtGrCOLOR &&
                                            ilConvert(inPipe, inDes,
                                                      ((ilImageFormat *)NULL),
                                                      0, NULL) != True)
                                                result = -1;
...
processXwd(_DtGrStream *stream, Screen *screen, int depth, Colormap colormap,
        ...
        *in_out_height = header.pixmap_height * ratio + 0.5;
        ...
        pixmap = XCreatePixmap(dpy, drawable, (*in_out_width), (*in_out_height),
                               depth);

        if (pixmap) {
                if ((result = XwdFileToPixmap(
                         dpy, screen_num, depth, pixmap, colormap, visual, gc,
                         color_model, 0, 0, 0, 0, (*in_out_width),
                         (*in_out_height), stream, ret_colors,
                         ret_num_colors)) == GR_SUCCESS) {
                        *ret_pixmap = pixmap;
                        return (_DtGrSUCCESS);
 ...
 ***********************************/
static int myXpmReadFileToPixmap(Display *display, Screen *screen, Drawable d,
                                 _DtGrStream *stream, Pixmap *pixmap_return,
                                 Pixmap *shapemask_return,
                                 XpmAttributes *attributes, GC gc, Pixel bg,
                                 Pixel fg, int depth, float ratio) {
        XImage *image = NULL;
        XImage **imageptr = 0;
        ...
        if (stream->type == _DtGrFILE) {
                if (stream->source.file.uncompressed_filename != NULL)
                        ErrorStatus = _DtXpmReadFileToImage(
                            display, stream->source.file.uncompressed_filename,
                            imageptr, shapeimageptr, attributes);
                else
                        ErrorStatus = _DtXpmReadFileToImage(
                            display, stream->source.file.filename, imageptr,
                            shapeimageptr, attributes);
        } else if (stream->type == _DtGrBUFFER) {
                ErrorStatus = XpmCreateImageFromBuffer(
                    display, (char *)stream->source.buffer.base, imageptr,
                    shapeimageptr, attributes);
        } else
        ...
        if (imageptr && image) {
                ...
                *pixmap_return =
                    XCreatePixmap(display, d, scaledWidth, scaledHeight, depth);
                if (image->depth == 1)
                ...
                _XmPutScaledImage(display, *pixmap_return, gc, image, 0, 0, 0,
                                  0, image->width, image->height, scaledWidth,
                                  scaledHeight);
                if (switchFlag) {
...
processXpm(_DtGrStream *stream, Screen *screen, int depth, Colormap colormap,
        ...
        do {
                ...
                result = myXpmReadFileToPixmap(
                    DisplayOfScreen(screen), screen, RootWindowOfScreen(screen),
                    stream, ret_pixmap, ret_mask, &xpmAttr, gc, background,
                    foreground, depth, ratio);
                done = True;
                /*
        ...
        if (result == XpmSuccess || result == XpmColorError) {
                ...
                *ret_num_colors = xpmAttr.npixels;
                 ...
                 */
                i = 0;
                while (i < xpmAttr.npixels &&
                       xpmAttr.pixels[i] != colorSymbol.pixel)
                        i++;
 ...
 *****************************************************************************/
static enum _DtGrLoadStatus
processGIF(_DtGrStream *stream, Screen *screen, int depth, Colormap colormap,
           Visual *visual, Pixel foreground, Pixel background, GC gc,
           enum _DtGrColorModel color_model, Boolean allow_reduced_colors,
           Dimension *in_out_width, Dimension *in_out_height,
           unsigned short media_resolution, Pixmap *ret_pixmap,
        ...
        */
        ...
        if (*ret_pixmap != 0) {
                if (g.f_do_visual == DO_COLOR) {
                        ...
                        *ret_colors =
                            (Pixel *)malloc(*ret_num_colors * sizeof(Pixel));
 ...
 *****************************************************************************/
static enum _DtGrLoadStatus
processJPEG(_DtGrStream *stream, Screen *screen, int depth, Colormap colormap,
            Visual *visual, Pixel foreground, Pixel background, GC gc,
            enum _DtGrColorModel color_model, Boolean allow_reduced_colors,
            Dimension *in_out_width, Dimension *in_out_height,
            unsigned short media_resolution, Pixmap *ret_pixmap,
        ...
        */
        status =
            jpeg_to_ximage(stream, screen, visual, in_out_width, in_out_height,
                           &in_image, &colors, &ncolors, &xres);
        ...
        if (status == _DtGrSUCCESS) {
                if (in_image->depth == 1) {
                        ...
                        */
                        ...
                        /*
                        ** Create an output image and have Do_Pseudo allocate
                        *the needed
                        ** pixels and store image data that uses these pixels
                        *into
                        ** the output image.
                        */
                        out_image = XCreateImage(
                            ...
                            in_image->xoffset, NULL, in_image->width,
                            in_image->height, XBitmapPad(dpy), 0);

                        out_image->data = (char *)malloc(Image_Size(out_image));

                        result =
                            Do_Pseudo(dpy, screen_num, colormap, ncolors,
                                      colors, color_model, in_image, out_image,
                                      ret_colors, ret_num_colors);
                }

                /*
                ** If we've got a good XImage, go ahead and make a pixmap out of
                *it
                */
                if (result != GR_ALLOC_ERR) {
                        /*
                        ...
                        *ret_pixmap = XCreatePixmap(dpy, drawable, scaledWidth,
                        ...
                        if (*ret_pixmap) {
                                /*
                                ** Copy the XImage into the pixmap and set the
                                *other
                                ** return parameters.
                                */
                                _XmPutScaledImage(
                                    dpy, *ret_pixmap, gc, out_image, 0, 0, 0, 0,
...
DetermineImageType(_DtGrStream *stream, char *image_type, int image_type_size) {
        ...
        if (stream->type == _DtGrFILE) {
                ...
                if (stream->source.file.filename != NULL) {
                        ...
                        if (_DtHelpCeStrrchr(stream->source.file.filename, ".",
                                ...
                                while (img_extensions[i] != NULL) {
                                        if (strcmp(ext, img_extensions[i]) ==
                                            0) {
                                                /* Found a match */
                                                strlcpy(image_type,
                                                        img_extensions[++i],
                                        ...
                                        /* Skip two ahead to the next file
                                         * extension name */
                                        i += 2;
                                }
                        }
                ...
                /*
                 * Didn't work, see if CDE datatyping can determine the image
                 * type. First try the filename, then try the uncompressed
                 * temporary filename, if it exists.
                 */
                if ((ext = DtDtsFileToDataType(stream->source.file.filename)) ==
                    NULL)
                        if (stream->source.file.uncompressed_filename != NULL)
                ...
                /* See if the CDE datatyping mechanism can determine the image
                 * type */
                ext = DtDtsBufferToDataType(stream->source.buffer.base,
                                            stream->source.buffer.size, NULL);
...
Pixmap _DtHelpProcessGraphic(
    ...
    Dimension *width, Dimension *height, Pixmap *ret_mask, Pixel **ret_colors,
    int *ret_number) {
        ...
        *ret_mask = None;
        ...
        if (filename != NULL) {
                ...
                if (result != 0)
                        ...
                        _DtGrLoad(&stream, NULL, scrptr, depth, colormap,
                                  visual, fore_ground, back_ground, gc,
                                  ForceColor, TRUE, &pWidth, &pHeight,
                                  media_resolution, &pix, &mask, ret_colors,
                                  ret_number, context);
                        pixWidth = pWidth;
                        pixHeight = pHeight;
        ...
        if (pix == 0) {
                 ...
                 */
                if (*def_pix == 0) {
                        *def_pix = XmGetPixmap(scrptr, "Dthgraphic",
                        ...
                        if (*def_pix != XmUNSPECIFIED_PIXMAP) {
                                ...
                                if (XGetGeometry(dpy, *def_pix, &root_id, &x,
                                                 &y, &pixWidth, &pixHeight,
                                                 &border, &depth) == 0) {
                        ...
                        /*
                         * couldn't get a localized pixmap, go with a build in
                         * default
                         */
                        if (*def_pix == 0) {
                                pixWidth = Missing_bm_width;
                                pixHeight = Missing_bm_height;
                                *def_pix = XCreatePixmapFromBitmapData(
                                    dpy, drawable, (char *)Missing_bm_bits,
                                    pixWidth, pixHeight, fore_ground,
                                    back_ground, depth);
                        }
...
_DtGrLoad(_DtGrStream *stream, char **image_type, Screen *screen, int depth,
          Colormap colormap, Visual *visual, Pixel foreground, Pixel background,
          GC gc, enum _DtGrColorModel color_model, Boolean allow_reduced_colors,
          Dimension *in_out_width, Dimension *in_out_height,
          unsigned short media_resolution, Pixmap *ret_pixmap, Pixmap *ret_mask,
          Pixel **ret_colors, int *ret_num_colors, _DtGrContext *context) {
        enum _DtGrLoadStatus status;
        ...
        if ((image_type != NULL) && (*image_type != NULL))
                ...
                if ((status = DetermineImageType(stream, buf, 20)) !=
                    _DtGrSUCCESS)
                        return (status); /* Return failure if image type is
                                            unknown */
                itype = &buf[0];
        }

        /* Look up the proper converter for this image type */
        if ((status = GetConverterAndDestructor(itype, &converter,
                                                &destructor)) == _DtGrSUCCESS) {
                /* Call the converter */
                if (converter != NULL) {
                        status = (*converter)(
                            stream, screen, depth, colormap, visual, foreground,
                            background, gc, color_model, allow_reduced_colors,
                            in_out_width, in_out_height, media_resolution,
                            ret_pixmap, ret_mask, ret_colors, ret_num_colors,
                            context);
                } else
 ...
 *
 *****************************************************************************/
void _DtGrDestroyContext(_DtGrContext *context) {
        _DtGrLoadProc converter;
        _DtGrDestroyContextProc destructor;
 ...
 *
 *****************************************************************************/
void _DtGrRegisterConverter(char *image_type, _DtGrLoadProc convert_proc,
                            _DtGrDestroyContextProc destroy_context_proc,
        ...
        for (i = 0; i < registry_count; i++) {
                if (strcmp(registry[i].image_type, image_type) == 0) {
                        ...
                        if (current_convert_proc != NULL)
                                *current_convert_proc =
                                    registry[i].convert_proc;
                        if (current_destroy_proc != NULL)
                                *current_destroy_proc =
                                    registry[i].destroy_context_proc;
        ...
        for (i = 0; i < new_registry_count; i++) {
                if (strcmp(new_registry[i].image_type, image_type) == 0) {
                        ...
                        if (current_convert_proc != NULL)
                                *current_convert_proc =
                                    new_registry[i].convert_proc;
                        if (current_destroy_proc != NULL)
                                *current_destroy_proc =
                                    new_registry[i].destroy_context_proc;
        ...
        new_registry = (_DtGrRegistryRec *)XtRealloc(
            (char *)new_registry,
 ...
 *
 *****************************************************************************/
int _DtGrOpenFile(_DtGrStream *stream, const char *path) {
        char *fname = NULL;
 ...
 *
 *****************************************************************************/
int _DtGrOpenBuffer(_DtGrStream *stream, const char *buffer, int buffer_size) {
 ...
 *
 *****************************************************************************/
int _DtGrCloseStream(_DtGrStream *stream) {
        int status;
 ...
 *
 *****************************************************************************/
size_t _DtGrRead(void *buffer, size_t size, size_t num_items,
                 _DtGrStream *stream) {
        unsigned nleft;
        int n;
        ...
        if (stream->type == _DtGrFILE) {
                return (fread(buffer, size, num_items,
                              stream->source.file.fileptr));
        } else if (stream->type == _DtGrBUFFER) {
                ...
                for (nleft = num_items * size;;) {
                        ...
                        buffer =
                            (char *)memcpy(
                                buffer, (void *)stream->source.buffer.current,
                                (size_t)n) +
                            n;
 ...
 *
 *****************************************************************************/
int _DtGrSeek(_DtGrStream *stream, long offset, int whence) {
        long newpos;
 ...
 *
 *****************************************************************************/
int _DtGrGetChar(_DtGrStream *stream) {
        if ((stream == NULL) || (stream->type == _DtGrNONE))
                return (EOF);
        ...
        if (stream->type == _DtGrFILE) {
                ...
                if (stream->source.buffer.current > stream->source.buffer.end)
                        ...
                        return (
                            (unsigned char)*(stream->source.buffer.current++));
        } else
                return (EOF);
}
 ...
 *
 *****************************************************************************/
char *_DtGrGetString(char *buffer, int num_bytes, _DtGrStream *stream) {
        char *p, *save = buffer;
        int i;
