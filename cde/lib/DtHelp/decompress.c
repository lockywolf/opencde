...
BufFilePtr _DtHelpCeBufFilePushZ(BufFilePtr f) {
        int code;
        int maxbits;
        int hsize;
        CompressedFile *file;
        ...
        extra =
            (1 << maxbits) * sizeof(char_type) + hsize * sizeof(unsigned short);
        ...
        file->tab_prefix =
            (unsigned short *)(file->tab_suffix + file->maxmaxcode);
        /*
        ...
        return _DtHelpCeBufFileCreate((char *)file, BufCompressedFill,
            ...
            file->free_ent > file->maxcode) {
                ...
                if (file->free_ent > file->maxcode) {
                        ...
                        if (file->n_bits == file->maxbits)
                                file->maxcode =
                                    file->maxmaxcode; /* won't get any bigger
                                                         now */
                        else
...
int _DtHelpCeUncompressFile(char *infile, char *outfile) {
