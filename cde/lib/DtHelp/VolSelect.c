 ...
 *
 *****************************************************************************/
static void CloseDialogCB(Widget w, XtPointer clientData, XtPointer callData) {
        FLSelDlgRec *selDlgData = (FLSelDlgRec *)clientData;
 ...
 *
 *****************************************************************************/
static void SetSelectionCB(Widget w, XtPointer clientData, XtPointer callData) {
        FLSelDlgRec *selDlgData = (FLSelDlgRec *)clientData;
        int *selList = NULL;
        int selCnt = 0;
        int setCnt = 0;
        ...
        if (NULL ==
            selList) { /* no more items selected ==> deselected last item */
        ...
        for (file = selDlgData->fileList, pos = 1, setCnt = 0;
             ...
             file = _DtHelpFileListGetNext(NULL, file), pos++) {
                if (setCnt < selCnt /* fixes bug due to valid value at
                                       selList[selCnt] */
                    && pos == selList[setCnt]) {
 ...
 *
 *****************************************************************************/
static Widget CreateFileSelDialog(DtHelpDialogWidget hw, Widget parent,
                                  Boolean modalDialog, char *dlgTitle,
...
#define FSWIDGETS selDlgData->dlgWidgets /* short hand */
        ...
        if (NULL == dlgTitle)
                dlgTitle =
                    (char *)_DTGETMESSAGE(FUSET, 1, "Help - Volume Selection");

        dlgTitle = XtNewString(dlgTitle);
        n = 0;
        XtSetArg(args[n], XmNtitle, dlgTitle);
        n++;
        XtSetArg(args[n], XmNallowShellResize, TRUE);
        n++;
        FSWIDGETS.shell =
            (Widget)XmCreateDialogShell(parent, "fileSelectShell", args, n);
        XtFree(dlgTitle);
        XtAddCallback(FSWIDGETS.shell, XmNdestroyCallback, DestroyDialogCB,
                      (XtPointer)selDlgData);
        ...
        n++;
        XtSetArg(args[n], XmNuserData, selDlgData);
        n++;
        /* XtSetArg (args[n], XmNdialogStyle, (modalDialog ?
                 XmDIALOG_PRIMARY_APPLICATION_MODAL : XmDIALOG_MODELESS));
           n++;*/
        FSWIDGETS.form =
            (Widget)XmCreateForm(FSWIDGETS.shell, "selectForm", args, n);

        /* put the list label in the form */
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(FUSET, 2, "Help Volumes")));
        n = 0;
        ...
        n++;
        FSWIDGETS.label =
            (Widget)XmCreateLabelGadget(FSWIDGETS.form, "listLabel", args, n);
        ...
        n++;
        XtSetArg(args[n], XmNdoubleClickInterval, 10); /*basically disables*/
        n++;
        XtSetArg(args[n], XmNhighlightOnEnter, True);
        n++;
        FSWIDGETS.list =
            (Widget)XmCreateScrolledList(FSWIDGETS.form, "selList", args, n);
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, FSWIDGETS.label);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNbottomOffset, 100);
        n++;
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, XtParent(FSWIDGETS.list));
        n++;
        XtSetArg(args[n], XmNtopOffset, 5);
        n++;
        FSWIDGETS.prompt =
            (Widget)XmCreateLabelGadget(FSWIDGETS.form, "prompt", args, n);
        ...
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, FSWIDGETS.prompt);
        n++;
        XtSetArg(args[n], XmNtopOffset, 5);
        n++;
        ...
        labelString =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(FUSET, 4, "Close")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        ...
        n++;
        XtSetArg(args[n], XmNrightPosition, 35);
        n++;
        XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNtopWidget, FSWIDGETS.separator);
        n++;
        ...
        labelString =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(FUSET, 5, "Help")));
        n = 0;
        XtSetArg(args[n], XmNlabelString, labelString);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_volSelectHelpBtn_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(FSWIDGETS.helpBtn, XmNactivateCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);

        /* set the forms default and cancel button (for KCancel) */
        n = 0;
        XtSetArg(args[n], XmNcancelButton, FSWIDGETS.closeBtn);
        n++;
        XtSetArg(args[n], XmNdefaultButton, FSWIDGETS.closeBtn);
        n++;
...
#define KNOWN_OFFSETS_BELOW_LIST 24 /* 2*(4+3) + 2*5 */
                ...
                offset += widgetHeight + 2 * widgetBorderHeight;
                XtGetValues(XtParent(FSWIDGETS.list), args,
                            1); /* get only first arg */
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_volSelectShell_STR, (Widget)hw,
                                   &hw->help_dialog.help,
                                   &hw->help_dialog.help.pHelpListHead);
        XtAddCallback(FSWIDGETS.form, XmNhelpCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);

        if (out_struct)
                *out_struct = FSWIDGETS;
...
Widget _DtHelpFileListCreateSelectionDialog(
    DtHelpDialogWidget hw, Widget parent, Boolean modalDialog, char *dlgTitle,
    XmFontList *io_titlesFontList, _DtHelpFileList in_out_list,
    _DtHelpFileDlgChildren *out_widgets) {
        ...
        /* create the dialog */
        selDlg = CreateFileSelDialog(hw, parent, modalDialog, dlgTitle,
                                     in_out_list, &widgets);
                    ...
                    XmFontListInitFontContext(&context, fontList) == True) {
                        ...
                        for (entry = XmFontListNextEntry(context);
                             ...
                             entry = XmFontListNextEntry(context)) {
                                *io_titlesFontList = XmFontListAppendEntry(
                                    *io_titlesFontList, entry);
                        }
        ...
        for (file = in_out_list; NULL != file;
             file = _DtHelpFileListGetNext(NULL, file)) {
                ...
                if (file->fileSelected)
                        XmListSelectPos(widgets.list, 0, False);
        }
