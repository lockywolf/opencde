...
static boolean fill_input_buffer(j_decompress_ptr cinfo) {
        ...
        ** we set nbytes to zero, allocate a new buffer to hold a fake EOI
        *marker
        ...
        */
        if (src->stream->type == _DtGrBUFFER) {
                ...
                src->buffer = (JOCTET *)(*cinfo->mem->alloc_small)(
                    (j_common_ptr)cinfo, JPOOL_IMAGE, 2 * SIZEOF(JOCTET));
        } else /* _DtGrFILE, read the next chunk of data */
                nbytes =
                    _DtGrRead(src->buffer, 1, src->input_buf_size, src->stream);

        if (nbytes <= 0) {
                if (src
                        ->start_of_file) /* Treat empty input file as fatal
                                            error */
 ...
 *
 *****************************************************************************/
static void jpeg_stream_src(j_decompress_ptr cinfo, _DtGrStream *stream) {
        stream_src_ptr src;
        ...
        */
        if (stream->type == _DtGrFILE) {
                ...
                src->pub.bytes_in_buffer =
                    0; /* forces fill_input_buffer call */
 ...
 *
 *****************************************************************************/
static void copy_pixels(j_decompress_ptr cinfo, djpeg_dest_ptr dinfo,
        ...
        for (col = 0; col < cinfo->output_width; col++) {
                ...
                XPutPixel(dest->ximage, col, cinfo->output_scanline - 1,
                          pixval);
        }
}
 ...
 *
 *****************************************************************************/
static void start_output_ximg(j_decompress_ptr cinfo, djpeg_dest_ptr dinfo) {
        return;
}
 ...
 *
 *****************************************************************************/
djpeg_dest_ptr init_jpeg_dest_mgr(j_decompress_ptr cinfo) {
        ximg_dest_ptr dest;
        ...
        */
        dest = (ximg_dest_ptr)(*cinfo->mem->alloc_small)(
            (j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(ximg_dest_struct));
 ...
 *
 *****************************************************************************/
enum _DtGrLoadStatus jpeg_to_ximage(_DtGrStream *stream, Screen *screen,
                                    Visual *visual, Dimension *in_out_width,
                                    Dimension *in_out_height, XImage **ximage,
                                    XColor **xcolors, int *ncolors, int *xres) {
        ...
        */
        if (setjmp(jerr.setjmp_buffer)) {
                ...
                *to
                ** free memory, clean up the JPEG object, and return a failure
                *code.
                */
                if (*ximage != NULL)
        ...
        */
        ...
        if (nplanes > 8)
                ximData =
                    (unsigned char *)malloc(ximWidth * cinfo.output_height * 4);
        else
                ximData =
                    (unsigned char *)malloc(ximWidth * cinfo.output_height);
        ...
        *ximage = XCreateImage(
            display, visual, nplanes, (nplanes == 1) ? XYPixmap : ZPixmap, 0,
            (char *)ximData, cinfo.output_width, cinfo.output_height, 32, 0);
        ...
        */
        ...
        while (cinfo.output_scanline < cinfo.output_height) {
                num_scanlines =
                    jpeg_read_scanlines(&cinfo, (JSAMPARRAY)dest_mgr->buffer,
                ...
                for (i = 0; i < cinfo.actual_number_of_colors; i++) {
                        if (cinfo.out_color_space == JCS_GRAYSCALE) {
                                colors[i].red = colors[i].green =
                                    colors[i].blue = INTERP_TO_XCOLORSPACE(
                                        cinfo.colormap[0][i]);
                        } else /* JCS_RGB */
                        {
                                colors[i].red =
                                ...
                                colors[i].green =
                                    INTERP_TO_XCOLORSPACE(cinfo.colormap[1][i]);
                                colors[i].blue =
                                    INTERP_TO_XCOLORSPACE(cinfo.colormap[2][i]);
                        }
                }
