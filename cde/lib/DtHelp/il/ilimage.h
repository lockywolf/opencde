...
#define IL_NPLANES(_pImage)                                                    \
        (((_pImage)->format.sampleOrder == IL_SAMPLE_PIXELS)                   \
             ? 1                                                               \
             : (_pImage)->des.nSamplesPerPixel)

/*  Definition of IL internal and client images.  Notes:

        o               object header - MUST BE FIRST.
                        Not meaningful for temporary pipe images, which are
   owned by /ilc/ilimage.c and have an object type of IL_NULL_OBJECT.
    ...
    Next three fields used for compressed images only:
        ...
        pStripOffsets   ptr to an array of byte offsets to each strip within the
                        ...
                        pStripOffsets[i+1] - pStripOffsets[i].  There are
   nStrips+1 elements in array so this works for the last strip.
...
*/
IL_EXTERN ilError _ilMallocImagePixels(ilImagePtr pImage);
...
/*  Destroy() function for client and internal image objects.
    ...
    Free the pixels of the image only if the IL owns them.
        NOTE: handles uncompressed images only!
*/
IL_EXTERN void _ilFreeImagePixels(ilImagePtr pImage);
...
*/
IL_EXTERN ilBool _ilAllocStripOffsets(register ilImagePtr pImage,
                                      long stripHeight);
