 ...
 */
ILJPG_PUBLIC
iljpgError iljpgEncodeInit(register iljpgDataPtr pData,
        ...
        pPriv =
            (iljpgEncodePrivPtr)ILJPG_MALLOC_ZERO(sizeof(iljpgEncodePrivRec));
        if (!pPriv)
        ...
        */
        ...
        for (i = 0; i < 4; i++) {
                if (pData->QTables[i]) {
                        if (!(pPriv->DCTScaleTables[i] =
                                  (float *)ILJPG_MALLOC(sizeof(float) * 64)))
                                return ILJPG_ERROR_ENCODE_MALLOC;
                        if (error = _iljpgEnDCTScale(pData->QTables[i],
                                                     pPriv->DCTScaleTables[i]))
                                return error;
                }
        }
        ...
        for (comp = 0, pCompData = pData->comp, pComp = pPriv->compData;
             comp < pData->nComps; comp++, pCompData++, pComp++) {
                ...
                pComp->width =
                    pData->width * pCompData->horiFactor / pData->maxHoriFactor;
 ...
 */
ILJPG_PUBLIC
iljpgError iljpgEncodeCleanup(iljpgPtr pPrivate) {
        register int i;
        register iljpgEncodePrivPtr pPriv;
        iljpgError error;
...
*/
ILJPG_PUBLIC
iljpgError iljpgEncodeExecute(iljpgPtr pPrivate, ILJPG_ENCODE_STREAM stream,
                              long nSrcLines, iljpgPtr pSrcPixels[],
        ...
        /*  Encode "interleaved" JPEG data, where all components are grouped
           together in Minimum Coded Unit (MCU) size, = 8 * max hori(width) or
           vert(height) factor.  The "factors" are the inverse of IL subsample
           factors.  For example, if component 0 is not subsampled and 1 and 2
           are subsampled by 2, then hori/vertFactor for comp 0 is 2, for
           comps 1..2 is 1, and max hori/vert factor is 2 (there are 4x as many
           comp 0 pixels as comps 1 or 2). So: loop over y, and over x within y,
           and decode one MCU (all components), advancing x by mcuWidth and y by
           mcuHeight.
        */
        mcuWidth = 8 * pData->maxHoriFactor;
        mcuHeight = 8 * pData->maxVertFactor;

        /*  Reset temp vars in comp data in private; "height" is size of one
         * strip */
        for (comp = 0, pComp = pPriv->compData; comp < pData->nComps;
             comp++, pComp++) {
                pComp->height =
                    nSrcLines * pComp->vertFactor / pData->maxVertFactor;
                pComp->x = 0;
                pComp->y = 0;
                pComp->lastDC = 0; /* new strip; clear as if a reset/restart */
        }

        /*  Loop over y, and over x within y, and encode one MCU (all
           components), advancing x by mcuWidth and y by mcuHeight.
        */
        for (mcuMaxY = 0; mcuMaxY < nSrcLines; mcuMaxY += mcuHeight) {
                for (mcuMaxX = 0; mcuMaxX < pData->width; mcuMaxX += mcuWidth) {

                        /*  Encode one MCU, all components, to (mcuX, mcuY). For
                           each component there are horiFactor * vertFactor 8x8
                           blocks that go across then down.
                        */
                        for (comp = 0, pComp = pPriv->compData;
                             comp < pData->nComps; comp++, pComp++) {
                                nBytesPerRow = nSrcBytesPerRow[comp];
                                pPixels = pSrcPixels[comp];

                                for (v = 0, bY = pComp->y;
                                     v < pComp->vertFactor; v++, bY += 8) {
                                        for (h = 0, bX = pComp->x;
                                             h < pComp->horiFactor;
                                             h++, bX += 8) {

                                                /*  Extract one 8x8 block from
                                                   position (bX, bY).  If
                                                    clipped replicate pixel or
                                                   above scan line out to 8x8.
                                                */
                                                {
                                                        int nBytesInit;
                                                        register int nLines,
                                                ...
                                                /*  Do DCT encoding, from
                                                 * "pixels" back into "pixels"
                                                 */
                                                _iljpgEnDCT(pixels,
                                                            pComp->pScale);

                                                /*  Subtract previous DC from
                                                 * this one, save this one for
                                                 * next */
                                                {
                                                        int dc;
                                                        dc = pixels[0];
                                ...
                                pComp->x += pComp->mcuXInc; /* move component
                                                               one MCU to right
                                                             */
                        } /* END one component */
                }         /* END one hori MCU */
