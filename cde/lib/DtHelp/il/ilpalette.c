...
/*  AddElement() function, called by ilConvert() after the pipe element is
   added. Merely copy the given pPalette into private.
*/
static ilError ilAddElementPaletteToRGBByte(ilPaletteToRGBPrivPtr pPriv,
...
static ilError ilInitPaletteToRGBByte(ilPaletteToRGBPrivPtr pPriv,
...
static void ilInitPackedPalette(ilPaletteToRGBPrivPtr pPriv) {
        register int i;
        register unsigned long *pPacked, packed;
        register unsigned short *pRed, *pGreen, *pBlue, temp;

        /*  Pack the upper 8 bits of the rgb value, with blue in the high order
         * bits. */
...
*/
static ilError ilExecutePaletteToRGBByte(ilExecuteData *pData, long dstLine,
        ...
        /*  Setup src,dst line ptrs and rowBytes, and nLines/PixelsM1 ("minus
           1"); exit if no lines or pixels.
        */
        ...
        /*  For each src byte: get a long from the cvt'd palette table, then
           take the low eight bits to get r,g,b, shifting down by 8 to get next
           component.
        */
