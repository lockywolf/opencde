...
static unsigned long ilSupportedCompressions[IL_MAX_TYPE + 1] = {
    /* IL_BITONAL */ ((1 << IL_UNCOMPRESSED) | (1 << IL_G3) | (1 << IL_G4) |
                      (1 << IL_LZW) | (1 << IL_PACKBITS)),
    /* IL_GRAY */
    ((1 << IL_UNCOMPRESSED) | (1 << IL_LZW) | (1 << IL_PACKBITS) |
     (1 << IL_JPEG)),
    /* IL_PALETTE */
    ((1 << IL_UNCOMPRESSED) | (1 << IL_PACKBITS) | (1 << IL_LZW)),
    /* IL_RGB */ ((1 << IL_UNCOMPRESSED) | (1 << IL_LZW) | (1 << IL_JPEG)),
    /* IL_YCRCB */ ((1 << IL_UNCOMPRESSED) | (1 << IL_LZW) | (1 << IL_JPEG))};
...
*/
IL_PRIVATE ilError _ilValidateDesFormat(ilBool allowPrivateTypes,
...
#define MAX_BITS_PER_SAMPLE 24 /* maximum # of bits/sample supported */
        ...
        /*  unused bits in flags must = 0; if dithered palette bit set, must be
           a palette image with levels whose product is <= # levels.
        */
        ...
        */
        if (pFormat) {
                ...
                /*  NOTE: currently only pixel sample order supported if
                 * compressed */
        ...
        /*  Format and des are valid, excluding image type considerations.
            ...
            an error will occur - but client pipe elements could work on that
           image.
        */
        switch (pDes->type) {
        case IL_BITONAL:
        ...
        case IL_RGB:
                ...
                /*  Validate YCbCr data; subsamples must be 1, 2 or 4, refWhite
                   > refBlack. Image must be planar, 8 bits per sample.
                */
        case IL_YCBCR:
                if (nSamplesPerPixel != 3)
                        return IL_ERROR_SAMPLES_PER_PIXEL;
...
/*  Public function: see spec.
    ...
    format "*pFormat".
*/
void ilGetBytesPerRow(register const ilImageDes *pDes,
                      register const ilImageFormat *pFormat, long width,
        ...
        /*  Calculate nBytesPerRow: # bits * width, rounded up by rowBitAlign.
            For 8 < bits/pixel <= 16, use word/pixel; <= 32, use long (TIFF
           std). Allocation size for each plane is then nBytesPerRow * height of
           image. If a YCBCrImage, take horizontal subsampling into account.
        */
        if (pFormat->sampleOrder == IL_SAMPLE_PIXELS) {

                /*  Pixel order: one row contains all of the bits for each
                   pixel, in sample order.  Sum the bits for one pixel; always
                   start on a byte boundary for samples > 8 bits.
                */
                nBits = 0;
                while (nSamplesPerPixel-- > 0) {
                        ...
                        if (i > 8) {
                                nBits = ((nBits + 7) / 8) *
                                        8; /* round nBits to byte boundary */
                                if (i <= 16)
                                        i = 16;
                                else
                                        i = 32;
                ...
                *pnBytesPerRow =
                    ((nBits * width + rowBitAlign - 1) / rowBitAlign) *
                    (rowBitAlign / 8);
        } else {
                register const ilYCbCrSampleInfo *pYCbCr;

                /*  Planar order: each plane is independent; return
                   nSamplesPerPixel rowbytes. Take into account subsampling if
                   YCbCr image.
                */
                pYCbCr = (pDes->type == IL_YCBCR) ? pDes->typeInfo.YCbCr.sample
                                                  : (ilYCbCrSampleInfo *)NULL;
                ...
                while (nSamplesPerPixel-- > 0) {
                        ...
                        if (pYCbCr) {
                                i = width >>
                                    _ilSubsampleShift[pYCbCr->subsampleHoriz];
                                if (i <= 0)
                                        i = 1;
                                pYCbCr++;
                        } else
                                i = width;
                        *pnBytesPerRow++ =
                            ((nBits * i + rowBitAlign - 1) / rowBitAlign) *
                            (rowBitAlign / 8);
                }
        }
}
...
*/
IL_PRIVATE ilError _ilMallocImagePixels(register ilImagePtr pImage) {
        ...
        ilGetBytesPerRow(&pImage->des, &pImage->format, pImage->i.width,
                         rowBytesPerPlane);

        /*  Alloc each plane; on failure, backtrack and free those already
           created. Don't allocate buffer for compressed image! If YCbCr image
           point pYCbCr and calc maxVertSubsample (1,2,4) and MCU (Minimum Coded
           Unit) = 8 * maxVertSubsample.
        */
        nPlanes = (pImage->format.sampleOrder == IL_SAMPLE_PIXELS)
                      ? 1
        ...
        if (pImage->des.type == IL_YCBCR) {
                ...
                MCUMinus1 =
                    MCUMinus1 * 8 - 1; /* e.g. if max = 4 MCUMinus1 is 31 */
        } else
                pYCbCr = (ilYCbCrSampleInfo *)NULL;
        ...
        for (plane = 0; plane < nPlanes; plane++) {
                ...
                if (pImage->des.compression != IL_UNCOMPRESSED)
                        ...
                        /*  If YCbCr image, alloc smaller buffer (divide by
                           subsample) but first rounded up to the MCU.  For
                           example, if max subsample is 2, an make unsubsampled
                           buffer a mult of 16; a 2 subsampled mult of 8.
                            "allocHeight" is height rounded up to MCU /
                           vertSubsample.
                        */
                        if (pYCbCr) {
                                allocHeight =
                                    ((pImage->i.height + MCUMinus1) &
                                     ~MCUMinus1) >>
                                    _ilSubsampleShift[pYCbCr->subsampleVert];
                                pYCbCr++;
                        } else
                                allocHeight = pImage->i.height;
                ...
                if (!nBytesToAlloc)
                        ...
                        if (!pPixels) { /* error; free those already created */
                                while ((plane != 0)) {
                                        plane--;
                                        if ((pPixels = pImage->i.plane[plane]
                                                           .pPixels))
                                                IL_FREE(pPixels);
                                }
                                pImage->i.clientPixels =
                                    TRUE; /* so IL does not deallocate them */
...
*/
IL_PRIVATE void _ilFreeImagePixels(register ilImagePtr pImage) {
        int plane;
        ilPtr pPixels;
        ...
        */
        ...
        /*  If own comp data and it is non-null free it and associated (JPEG)
         * tables */
...
/*  For compressed images only: allocate strip offsets array (pStripOffsets)
    using the given "stripHeight", or use the default strip height if
   "stripHeight" is zero (0).  Locks image to that strip height.
*/
IL_PRIVATE ilBool _ilAllocStripOffsets(register ilImagePtr pImage,
                                       register long stripHeight) {
        register long nStrips;

        if (stripHeight <= 0)
                stripHeight =
                    ilRecommendedStripHeight(pImage->i.pDes, pImage->i.pFormat,
        ...
        pImage->pStripOffsets =
            (long *)IL_MALLOC_ZERO((nStrips + 1) * sizeof(long));
        if (!pImage->pStripOffsets) {
...
/*  Public function: see spec.
 */
ilInternalImage ilCreateInternalImage(ilContext context, long width,
                                      long height, ilImageDes *pDes,
        ...
        if ((context->error =
                 _ilValidateDesFormat(FALSE, pDes, (ilImageFormat *)NULL)))
                return (ilInternalImage)NULL;

        /*  Create the object and add it to context. */
        pImage = (ilImagePtr)_ilCreateObject(
            context, IL_INTERNAL_IMAGE, _ilFreeImagePixels, sizeof(ilImageRec));
        if (!pImage)
                return (ilInternalImage)NULL;

        /*  Fill in the image structure, excluding object header (.o).
            Init the format to std format, based on image type, which is not
           private type. For bits per sample (other than bitonal): use 4 bits if
           levels/sample <= 16, else use 8 bits.  Could use other values (e.g.
           min # bits for levels/sample), but at this point there are no
           conversions other than for 4<->8 bits.
        */
        pImage->des = *pDes;
        switch (pDes->type) {
        case IL_BITONAL:
        ...
        case IL_RGB:
                ...
                if ((pDes->nLevelsPerSample[0] <= 16) &&
                    ...
                    (pDes->nLevelsPerSample[2] <= 16))
                        pImage->format.nBitsPerSample[0] =
                            pImage->format.nBitsPerSample[1] =
                                pImage->format.nBitsPerSample[2] = 4;
                break;

                /*  YCbCr: always use 8 bits / pixel.  If uncompressed use
                   planar format, but if compressed use pixel (e.g. JPEG
                   interleaved format).
                */
        case IL_YCBCR:
                pImage->format = (pDes->compression == IL_UNCOMPRESSED)
                                     ? *IL_FORMAT_3BYTE_PLANE
                                     : *IL_FORMAT_3BYTE_PIXEL;
                break;
        ...
        if (pDes->type == IL_PALETTE) {
                pImage->i.pPalette = (unsigned short *)IL_MALLOC_ZERO(
        ...
        /*  If raw JPEG compressed, point pCompData to allocated JPEG info
           block, else init pCompData to null, owned by client (i.e. not freed
           when destroyed).
        */
        if ((pDes->compression == IL_JPEG) &&
            (pDes->compInfo.JPEG.reserved & IL_JPEGM_RAW)) {
                if (!(pImage->i.pCompData =
                          (ilPtr)IL_MALLOC_ZERO(sizeof(ilJPEGData)))) {
                        ilDestroyObject((ilObject)pImage);
                        context->error = IL_ERROR_MALLOC;
                        return (ilInternalImage)NULL;
                }
                pImage->i.clientCompData =
                    FALSE; /* IL must now destroy comp data */
        }
...
/*  Public function: see spec.
 */
ilBool ilQueryInternalImage(ilInternalImage image, long *pWidth, /* RETURNED */
...
/*  Public function: see spec.
 */
ilClientImage ilCreateClientImage(ilContext context, ilImageInfo *pInfo,
                                  unsigned long mustBeZero) {
        register ilImagePtr pImage;
        ilError error;
        int plane, nPlanes;
        ...
        if ((context->error =
                 _ilValidateDesFormat(TRUE, pInfo->pDes, pInfo->pFormat)))
                return (ilClientImage)NULL;

        /*  Create the object and add it to context. */
        pImage = (ilImagePtr)_ilCreateObject(
            context, IL_CLIENT_IMAGE, _ilFreeImagePixels, sizeof(ilImageRec));
        if (!pImage)
                return (ilClientImage)NULL;

        /*  Fill in the image structure, excluding object header (.o).
            Copy caller's des, and if "raw" JPEG force to non-raw.  Client must
           be able to access image, and non-raw (JIF) format only meaningful way
           to do so.
        */
        ...
        pImage->i.pPalette =
            (unsigned short *)NULL; /* assume no palette or comp data */
        pImage->i.clientPalette = TRUE;
        pImage->i.pCompData = (ilPtr)NULL;
        pImage->i.clientCompData = TRUE;
        pImage->i.clientPixels = pInfo->clientPixels;

        /*  If a palette image, point to the one from the caller if pPalette not
           null; if null, create one here and the IL owns it.
        */
        ...
        /*  If caller providing pixels (clientPixels == true), point to them,
            otherwise allocate buffer space; if failure delete object and return
           error.
        */
        if (pInfo->clientPixels) {
                for (plane = 0; plane < nPlanes; plane++)
                        pImage->i.plane[plane] = pInfo->plane[plane];
        } else { /*  no pixels from client; allocate them here if uncompressed
                    image */
                if ((error = _ilMallocImagePixels(pImage))) {
                        ilDestroyObject((ilObject)pImage);
                        context->error = error;
        ...
        /*  Compressed client images stored as one big strip; allocate strip
           offset array. If client pixels, offset[0] = 0, offset[1] = nBytes -
           all data in 1 strip.
        */
        if (pImage->des.compression != IL_UNCOMPRESSED) {
                ...
                if (pInfo->clientPixels) {
                        ...
                        pImage->pStripOffsets[1] =
                            pImage->i.plane[0].bufferSize;
                }
        }
...
/*  Public function: see spec.
 */
ilBool ilQueryClientImage(ilClientImage image,
