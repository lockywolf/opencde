...
/*  Defines for values of tag IL_TAG_PRIVATE_0 (34209).  This is a private tag
    read and (sometimes) written by the IL.  The tag is extensible; it is
   variable length, and the data in the tag depends on the flags.  The values
   are always an array of shorts, as follows: IL_TAG_P0_FLAGS         array of
   16 bits.  Only bit 0 is defined as follows; all other bits must be zero:
        IL_TAG_P0_FLAG_DITHER_LEVELS ignored if the image is not a palette
   image; else image was dithered / error diffused.  The red/green/blue levels
   are stored in offsets 1..3; tag must be at least
...
/*  Private definition of object ilFileImage.
    ...
    it is the header for a file image: # of tags, list of tags, offset to next
   IFD.
*/
typedef struct {
