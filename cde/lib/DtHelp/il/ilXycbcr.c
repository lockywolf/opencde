...
/*  Number of bits of Y and Cb/Cr used to dither before a table lookup to
    ...
    1<<DYCBCR_NBITS_Y/CBCR.  The YCbCr->RGB lookup table is "nLY * nLCbCr *
   nLCbCr" longs in size.
*/
...
typedef struct {
        unsigned long pixels[1 << (DYCBCR_NBITS_Y + DYCBCR_NBITS_CBCR +
                                   DYCBCR_NBITS_CBCR)];
 ...
 */
static ilError ilFastYCbCrDitherExecute(ilExecuteData *pData, long dstLine,
                                        long *pNLines) {
        ...
        */
        ...
        halfWidth =
            pData->pSrcImage->width >> 1; /* 1/2 # of lines, pixels per line */
        ...
        */
        while (nLinesDiv2-- > 0) {
                ...
                while (nPixelsDiv2-- > 0) {
                        ...
                        CbCr <<= DYCBCR_NBITS_Y; /* CbCr now in upper bits w/
                                                    room for Y */
 ...
 */
static ilError ilFastYCbCrDitherDoubleExecute(ilExecuteData *pData,
        ...
        *pNLines <<= 1; /* write 2x # of src lines */
        ...
        /*  Do 4 src pixels at a time, using 4 Ys for each Cb,Cr pair: get Cb,Cr
           into upper bits of "CbCr", then add in each Y and use result to index
           into table that yields 4 dithered pixels: <left><right><below><below
           right>. To double, output those 4 pixels as shown above; 1 src pixel
           becomes 4 dst.
        */
        while (nLinesDiv2-- > 0) {
                pY = pYLine;
                ...
                pCrLine += CrRowBytes;
                pDst = pDstLine;
                pDstLine += dstRowShorts
                            << 2; /* skip 4 lines; write 4 lines each loop */
                nPixelsDiv2 = halfWidth;
                while (nPixelsDiv2-- > 0) {
                        temp0 = *pCb++;
                        CbCr = *pCr++;
                        ...
                        pDst[dstRowShortsTimes2] =
                            (unsigned short)(temp0 >> 16);
                        temp1 = CbCr + (temp1 >> (8 - DYCBCR_NBITS_Y));
                        temp1 = pTable[temp1];
 ...
 */
static void ilSetupYCbCrDitherTables(register ilYCbCrInfo *pYCbCr, int nBitsR,
                                     int nBitsG, int nBitsB, ilPtr pMapPixels,
...
#define YCBCR_DEBUG_TABLE 1
        ...
        /*  Build the YCbCr to dither-RGB lookup table.  The table is indexed by
            ...
            in size and <Cb><Cr> are each "DYCBCR_NBITS_CBCR" in size.  Indexed
           that way the table yields either four dithered pixels or a long 24 X
           pixel. Each table entry is calculated by converting the YCbCr value
           to its RGB equivalent and either dithering to form 4 pixels or a
           single long X pixel.
        */
        ...
        for (refCb = 0; refCb < nLevelsCbCr; refCb++) {
                ...
                for (refCr = 0; refCr < nLevelsCbCr; refCr++) {
                        ...
                        for (refY = 0; refY < nLevelsY; refY++) {
                                ...
                                /*  Store 4 dithered pixels, the result of
                                   dithering RGB with a 2x2 kernel.  See
                                   /ilc/ildither.c for equivalent code.
                                */
#define DITHER
#ifdef COLORSLAM
                                CVT_TO_RGB
...
*/
IL_PRIVATE ilBool _ilFastYCbCrDither(ilPipe pipe, ilImageDes *pDes,
        ...
        /*  Add a filter to convert subsampled YCbCr to a dithered palette
           image. Force a single strip; image is at least 2 by 2
        */
        ...
        pPriv = (ilDitherYCbCrPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilDitherYCbCrPrivRec), 0, &srcData,
            &dstData, IL_NPF, IL_NPF, IL_NPF,
            (doDouble) ? ilFastYCbCrDitherDoubleExecute
                       : ilFastYCbCrDitherExecute,
            0);
        if (!pPriv)
                return FALSE;
        ...
        ilSetupYCbCrDitherTables(&pDes->typeInfo.YCbCr, nBitsRed, nBitsGreen,
                                 nBitsBlue, pMapPixels, pPriv->pixels);
