...
static ilError ilMirrorInit(ilMirrorPrivptr pPrivate, ilImageInfo *pSrcImage,
...

{
        ...
        bzero((char *)pdstline,
              (pplane->nBytesPerRow * pPrivate->ilMirrorheight));
...
/* =============================================================================================================================
      ...
      ilMirrorXBitonalExecute  -  Vertical mirroring for images with bit per
   pixel format .

   =============================================================================================================================
 */
static ilError ilMirrorXBitonalExecute(register ilExecuteData *pData,
        ...
        /* Mirror vertically  map (x,y) to (x, -y) about its center  [ x,
         * height-y ]  */
        ...
        for (y = pPriv->illinecount; y <= (lastcount + *pNLines);
             y++, pPriv->illinecount++) {
                ...
                for (x = 0; x < srcnbytes; x++)
                        *pdst++ = *psrc++;
                psrcline += srcnbytes;
        }

        /* No. of lines written is the destination height */
        if (pPriv->illinecount <= pPriv->ilMirrorheight)
                *pNLines = 0;
        else
                *pNLines = pPriv->ilMirrorheight;
...
/* =============================================================================================================================
      ...
      ilMirrorYBitonalExecute  -  Horizontal mirroring for images with bit per
   pixel format .

   =============================================================================================================================
 */
static ilError ilMirrorYBitonalExecute(register ilExecuteData *pData,
                                       unsigned long dstLine,
                                       unsigned long *pNLines) {
        ...
        /* Mirror horizontally  map (x,y) to (-x, y) about its center  [
         * width-x,  y ]  */
...
/* =============================================================================================================================
      ...
      ilMirrorX3ByteExecute  -  Vertical mirroring for images with 24 bits per
   pixel format .

   =============================================================================================================================
 */
static ilError ilMirrorX3ByteExecute(register ilExecuteData *pData,
                                     unsigned long dstLine,
                                     unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilMirrorY3ByteExecute  -  Horizontal mirroring for images with 24 bits per
   pixel format .

   =============================================================================================================================
 */
static ilError ilMirrorY3ByteExecute(register ilExecuteData *pData,
                                     unsigned long dstLine,
                                     unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilMirrorXByteExecute  -  Vertical mirroring for images with byte per pixel
   format .

   =============================================================================================================================
 */
static ilError ilMirrorXByteExecute(register ilExecuteData *pData,
                                    unsigned long dstLine,
                                    unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilMirrorYByteExecute  -  Horizontal mirroring for images with byte per
   pixel format .

   =============================================================================================================================
 */
static ilError ilMirrorYByteExecute(register ilExecuteData *pData,
                                    unsigned long dstLine,
                                    unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilMirror -  Add a mirror filter to an existing pipe  - checking for format
   types and doing an explicit conversion if necessary.  Direction parameter
...
ilBool ilMirror(ilPipe pipe, int direction)

{
        ...
        case 3:
                switch (direction) {
                case IL_MIRRORX:
                        pPriv = (ilMirrorPrivptr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilMirrorPriv), 0,
                            (ilSrcElementData *)NULL, &dstdata, ilMirrorInit,
                            IL_NPF, IL_NPF, ilMirrorX3ByteExecute, 0);
                        break;
                case IL_MIRRORY:
                        pPriv = (ilMirrorPrivptr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilMirrorPriv), 0,
                            (ilSrcElementData *)NULL, &dstdata, ilMirrorInit,
                            IL_NPF, IL_NPF, ilMirrorY3ByteExecute, 0);
                }
                break;
                ...
                if (bitonal) {
                        ...
                        case IL_MIRRORX:
                                pPriv = (ilMirrorPrivptr)ilAddPipeElement(
                                    ...
                                    ilMirrorInit, IL_NPF, IL_NPF,
                                    ilMirrorXBitonalExecute, 0);
                                break;
                        case IL_MIRRORY:
                                pPriv = (ilMirrorPrivptr)ilAddPipeElement(
                                    pipe, IL_FILTER, sizeof(ilMirrorPriv), 0,
