...
#define GET_VALUE_LSB(value, base, offset, mask)                               \
        {                                                                      \
                if (offset > pPriv->maxSrcPos)                                 \
                        return IL_ERROR_COMPRESSED_DATA;                       \
                value =                                                        \
                    (READ32_LSB((base) + ((offset) >> 3)) >> ((offset)&7) &    \
                     (mask));                                                  \
        }
...
#define GET_VALUE_MSB(value, base, offset, mask)                               \
        {                                                                      \
                if (offset > pPriv->maxSrcPos)                                 \
                        return IL_ERROR_COMPRESSED_DATA;                       \
                value =                                                        \
                    (READ32_MSB((base) + ((offset) >> 3)) >> ((offset)&7) &    \
                     (mask));                                                  \
        }
...
/* define to find out the value of the bit (1 or 0) in the image line      */
#define PIXEL_LSB(buf, ix)                                                     \
        ((ilBitReverseTable[((buf)[(ix) >> 3])] >> (7 - ((ix)&7))) & 1)
...
IL_EXTERN const ilDecompG4HuffTableRec
    ilArFax2DDecodeTable[G4K_DecodeTableSize];

IL_EXTERN ilError _ilDecompG4Line(register ilDecompG3G4PrivPtr pPriv,
                                  ilPtr pRefLine, ilPtr dstImageP);
