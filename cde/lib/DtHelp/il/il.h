...
extern const ilImageDes ilBitonal0WhiteImageDes, ilGray256ImageDes,
    ilRGB256ImageDes;
...
extern ilError ilInternalCreateContext(
    ...
    int versionCheck, ilContext *pContextReturn, /* RETURNED */
    unsigned long mustBeZero
#endif
);

#define IL_CREATE_CONTEXT(_pContext, _zero)                                    \
        (ilInternalCreateContext(IL_INTERNAL_VERSION, (_pContext), (_zero)))
...
#if NeedFunctionPrototypes
    ilContext context, long width, long height, ilImageDes *pImageDes,
    unsigned long mustBeZero
#endif
);
...
#if NeedFunctionPrototypes
    ilInternalImage image, long *pWidth, /* RETURNED */
...
#if NeedFunctionPrototypes
    ilContext context, ilImageInfo *pInfo, unsigned long mustBeZero
#endif
);

extern ilBool ilQueryClientImage(
#if NeedFunctionPrototypes
    ilClientImage image, ilImageInfo **ppInfo, /* RETURNED */
    unsigned long mustBeZero
#endif
);
...
#if NeedFunctionPrototypes
    ilContext context, unsigned long mustBeZero
#endif
);
...
#if NeedFunctionPrototypes
    ilPipe pipe, long *pWidth, /* RETURNED */
    long *pHeight,             /* RETURNED */
    ilImageDes *pImageDes      /* RETURNED */
#endif
);
...
#if NeedFunctionPrototypes
    ilPipe pipe, long nStrips, float ratio
#endif
);

extern ilBool ilReadImage(
#if NeedFunctionPrototypes
    ilPipe pipe, ilObject image
#endif
);
...
#if NeedFunctionPrototypes
    ilPipe pipe, long dstWidth, long dstHeight, int option, void *pOptionData
#endif
);

extern ilBool ilRotate90(
#if NeedFunctionPrototypes
    ilPipe pipe, int factor
#endif
);

extern ilBool ilMirror(
#if NeedFunctionPrototypes
    ilPipe pipe, int direction
#endif
);

extern ilBool ilCrop(
#if NeedFunctionPrototypes
    ilPipe pipe, ilRect *pRect
#endif
);

extern ilBool ilMap(
#if NeedFunctionPrototypes
    ilPipe pipe, ilClientImage mapImage
#endif
);

extern ilBool ilConvert(
#if NeedFunctionPrototypes
    ilPipe pipe, const ilImageDes *pDes, const ilImageFormat *pFormat,
    int option, void *pOptionData
#endif
);

extern ilBool ilCompress(
#if NeedFunctionPrototypes
    ilPipe pipe, unsigned int compression, ilPtr pData, long dstStripHeight,
    unsigned long mustBeZero
#endif
);
...
#if NeedFunctionPrototypes
    ilPipe pipe, ilObject image, long height, long stripHeight,
...
#if NeedFunctionPrototypes
    ilPipe pipe, long start, long nLines, long nCompBytes,
    unsigned long mustBeZero
#endif
);
...
#if NeedFunctionPrototypes
    ilPipe pipe, unsigned long mustBeZero, void *mustBeNull
#endif
);
