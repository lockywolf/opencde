 ...
 */
ILJPG_PUBLIC_EXTERN
iljpgError iljpgDecodeInit(register iljpgDataPtr pData,
        ...
        *pPrivate = (iljpgPtr)pPriv;
        ...
        for (comp = 0, pCompData = pData->comp, pComp = pPriv->compData;
             comp < pData->nComps; comp++, pCompData++, pComp++) {
                pComp->pRevScale =
                    pPriv->DCTRevScaleTables[pCompData->QTableIndex];
 ...
 */
ILJPG_PUBLIC_EXTERN
iljpgError iljpgDecodeCleanup(iljpgPtr pPrivate) {
...
*/
ILJPG_PUBLIC_EXTERN
iljpgError iljpgDecodeExecute(iljpgPtr pPrivate, ILJPG_DECODE_STREAM stream,
                              int doReset, long nDstLines,
                              iljpgPtr pDstPixels[], long nDstBytesPerRow[]) {
        ...
        /*  Decode "interleaved" JPEG data, where all components are grouped
        ...
        */
        ...
        for (comp = 0, pComp = pPriv->compData; comp < pData->nComps;
             comp++, pComp++) {
                pComp->height =
                    nDstLines * pComp->vertFactor / pData->maxVertFactor;
                pComp->x = 0;
                pComp->y = 0;
        }
        ...
        /*  Loop over y, and over x within y, and decode one MCU (all
        ...
        */
        for (mcuMaxY = 0; mcuMaxY < nDstLines; mcuMaxY += mcuHeight) {
                for (mcuMaxX = 0; mcuMaxX < pData->width; mcuMaxX += mcuWidth) {
                        ...
                        /*  Decode one MCU, all components, to (mcuX, mcuY). For
                           each component there are horiFactor * vertFactor 8x8
                           blocks that go across then down. If a restart
                           interval and mcu count >, do a reset/restart.
                        */
                        if (restartInterval &&
                            (pPriv->mcuRestartCount >= restartInterval)) {
                                if ((error = _iljpgDehuffReset(pPriv)))
                                        return error;
                                pPriv->mcuRestartCount = 0;
                        }
                                     ...
                                     v < pComp->vertFactor; v++, bY += 8) {
                                        for (h = 0, bX = pComp->x;
                                             ...
                                             h++, bX += 8) {
                                                ...
                                                /*  Add previous DC to this one,
                                                 * save away for next */
                                                dc = outHuff[0] + pComp->lastDC;
                                                pComp->lastDC = dc;

                                                /*  If an 8x8 block fits in
                                                   output buffer, decode the DCT
                                                    (based on return from
                                                   Huffman decode) directly into
                                                    the output buffer; otherwise
                                                   decode into "outDCT" and copy
                                                   top-left part of it into
                                                   buffer below.
                                                */
                                                pDstLine = pPixels +
                                                           (bY * nBytesPerRow) +
                        ...
                        pPriv->mcuRestartCount++; /* inc count of mcus since
                                                     restart */
                }                                 /* END one hori MCU */
