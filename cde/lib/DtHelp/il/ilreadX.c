 ...
 */
static ilError ilReadXInit(ilReadXPrivPtr pPriv, ilImageInfo *pSrcImage,
...
#define NRGBS 256
        ...
        /*  Init the palette if present by querying the colormap, with # entries
           = pPriv->colormapSize (limit to NCOLORS; may be less, so init unused
           to black).
        */
        if (pPriv->pPalette) {
                int i;
                ...
                XQueryColors(pPriv->display, pPriv->colormap, cells,
                             (pPriv->colormapSize > NCOLORS)
                                 ? NCOLORS
                                 : pPriv->colormapSize);
                for (i = 0, pPalette = pPriv->pPalette, pColor = cells;
                     i < NCOLORS; i++, pPalette++, pColor++) {
        ...
        /*  If a grayMapImage, reading from grayscale: query the colormap and
           store the upper 8 bits of the green value into grayMapImage, which is
           used by an ilMap() filter which follows the ilReadXDrawable() (added
           there).
        */
                ...
                XQueryColors(pPriv->display, pPriv->colormap, cells,
                             (pPriv->colormapSize > NGRAYS)
                                 ? NGRAYS
                                 : pPriv->colormapSize);
                for (i = 0, pByte = pPriv->pGrayMapPixels, pColor = cells;
                     i < NGRAYS; i++, pColor++)
                        *pByte++ = pColor->green >> 8;
        }

        /*  If an rgbMapImage, reading from True/DirectColor: query color - RGBs
           are independent.  Init the mapImage which is 3 byte (rgb) per pixel.
        */
                ...
                for (i = 0, pColor = cells; i < NRGBS; i++, pColor++) {
                        ...
                        if (!(pPriv->SlowMode)) {
                                ...
                                pColor->red = pColor->green = pColor->blue = 0;
                                pColor->pixel = ((i << pPriv->nRedZeros) |
                                                 (i << pPriv->nGreenZeros) |
                                                 i << pPriv->nBlueZeros);
                        }
                }
                XQueryColors(pPriv->display, pPriv->colormap, cells,
                             (pPriv->colormapSize > NRGBS)
                                 ? NRGBS
                                 : pPriv->colormapSize);
                for (i = 0, pByte = pPriv->pRGBMapPixels, pColor = cells;
                     i < NRGBS; i++, pColor++) {
                        *pByte++ = pColor->red >> 8;
                        *pByte++ = pColor->green >> 8;
                        *pByte++ = pColor->blue >> 8;
        ...
        if (pPriv->copyPixmapDepth) {
                pPriv->copyPixmap = XCreatePixmap(
                    pPriv->display, pPriv->drawable, pPriv->rect.width,
...
*/
static ilError ilReadXCleanup(ilReadXPrivPtr pPriv) {
        if (pPriv->copyPixmap) {
...
*/
static ilError ilReadXDestroy(ilReadXPrivPtr pPriv) {
        if (pPriv->pPalette)
 ...
 */
static ilError ilReadXExecute(ilExecuteData *pData, long dstLine,
                              long *pNLines) {
        ...
        */
        ...
        /*  If "copyPixmap" non-null, blt from drawable into it at (0,0), and
           set src (X,Y) to (0,0) - always GetImage() from left-top of it.  Then
           XGetImage() "nLines" from readDrawable (drawable or pixmap), and copy
           the bits into pDstImage's buffer at (0, "dstLine").
        */
        if (pPriv->copyPixmap) {
                XCopyArea(pPriv->display, pPriv->drawable, pPriv->copyPixmap,
                          pPriv->copyGC, pPriv->rect.x,
                          pPriv->rect.y + pData->srcLine, pPriv->rect.width,
                          nLines, 0, 0);
        ...
        Ximage = XGetImage(pPriv->display, readDrawable, srcX, srcY,
                           pPriv->rect.width, nLines, ~0, ZPixmap);
        if (!Ximage)
                return IL_ERROR_X_GET_IMAGE;

        /*  Bump srcLine by # of lines gotten, and copy that many lines to
           pDstImage. Set nBytesToCopy to lesser of src and dstRowBytes.
        */
        *pNLines = nLines;
        ...
        /*  If a long/pixel image, must extract IL RGB bytes from X long/pixel
         * image.
         */
        if (pPriv->isLongImage) {
                ...
                while (nLines-- > 0) {
                        ...
                        nLongsM1 = pPriv->rect.width -
                                   1; /* width > 0, from ilReadXDrawable() */
                        do {
                                temp =
                                    *pSrc++; /* long = <unused,r,g,b> each 8
                                                bits */
                                *(pDst + 2) = temp; /* red */
                                temp >>= 8;
                                *(pDst + 1) = temp; /* green */
                ...
                /*  Byte or bit/pixel: if 1 bit/pixel image and LSBFirst bit
                   order, reverse the bits using lookup table, else copy: if
                   bytes/row same for X/IL images, copy buffer, else one line at
                   a time.
                */
                if ((Ximage->depth == 1) &&
                    (Ximage->bitmap_bit_order == LSBFirst)) {
                        ...
                        if (nBytesToCopy > 0)
                                while (nLines-- > 0) {
                                        ...
                                        do {
                                                *pDst++ =
                                                    ilBitReverseTable[*pSrc++];
                                        } while (--nBytesM1 >= 0);
                                }
                } else {
                        if (srcRowBytes == dstRowBytes)
                                bcopy((char *)pSrcLine, (char *)pDstLine,
                                      nLines * srcRowBytes);
                        else {
                                while (nLines-- > 0) {
                                        bcopy((char *)pSrcLine,
                                              (char *)pDstLine, nBytesToCopy);
                                        pSrcLine += srcRowBytes;
                                        pDstLine += dstRowBytes;
                                }
                        }
...
/*  Execute() function for ilReadXDrawable() pipe elements.
 */
static ilError ilReadXExecuteSlow(ilExecuteData *pData, long dstLine,
                                  long *pNLines) {
        register ilReadXPrivPtr pPriv;
        ilReadXPrivRec priv;
        Drawable readDrawable;
                    ...
                    ((pPriv->nBlueOnes <= 8) && (pPriv->nBlueOnes >= 1))) {
                        ...
                        for (Y = 0; Y < nLines; Y++) {
                                ...
                                for (X = 0; X < pPriv->rect.width; X++) {
                                        ...
                                        /* Red */
                                        *pDst++ = (pixel & pPriv->OrgRedMask) >>
                                                  pPriv->nRedZeros;
                                        /* Green */
                                        *pDst++ =
                                            (pixel & pPriv->OrgGreenMask) >>
                                            pPriv->nGreenZeros;
                                        /* Blue */
                                        *pDst++ =
                                            (pixel & pPriv->OrgBlueMask) >>
                                            pPriv->nBlueZeros;
                                }
                        }
                }
 ...
 */
ilBool ilReadXDrawable(ilPipe pipe, Display *display, Drawable drawable,
                       Visual *visual, Colormap colormap, ilBool blackIsZero,
                       ilRect *pSrcRect, ilBool copyToPixmap,
                       unsigned long flags) {
        ...
        */
        ...
        /*  Init priv with what we have so far. Set "priv.rect" to rectangle to
           read from drawable: bounds of drawable, intersected with pSrcRect if
           present. Null priv. objects - call ilReadXDestroy() on failure to
           free non-nulls.
        */
        ...
        /*  Do type-specific setup: set pixelSize based on des.type; set des and
           format. If no visual or colormap: if not depth 1, error; else a
           bitmap: handle here.
        */
        if (!visual || !colormap) {
                if (depth != 1) /* not a bitmap */
                        return ilDeclarePipeInvalid(pipe,
                                                    IL_ERROR_X_COLORMAP_VISUAL);
                ...
                /*  Not a bitmap: get info from "visual"; depths must match.
                    Set "supported" true if this visual handled, else break
                   (becomes error).
                */
                if (!colormap || !visual)
                        return ilDeclarePipeInvalid(pipe,
                ...
                pVisualInfo =
                    XGetVisualInfo(display, VisualIDMask, &template, &notUsed);
                if (!pVisualInfo)
                        return ilDeclarePipeInvalid(pipe, IL_ERROR_X_RESOURCE);
                if (pVisualInfo->depth != depth)
                        return ilDeclarePipeInvalid(
                            pipe, IL_ERROR_UNSUPPORTED_VISUAL);
                priv.colormapSize = pVisualInfo->colormap_size;

                switch (pVisualInfo->class) {

                        /*  Support 1 and 8 bit gray scale.  For 1 bit, query
                           color map to determine blackIsZero (assume is if
                           pixel 0 = rgb of 0,0,0.) For 8 bit, add an ilMap()
                           element using pPriv->grayMapImage, which is setup in
                           ilReadXInit() with the gray value for each X pixel.
                ...
                case StaticGray:
                        if (depth == 1) {
                                ...
                                if (!(flags & IL_READ_X_RAW_MODE)) {
                                        ...
                                        priv.grayMapImage = ilCreateClientImage(
                                            pipe->context, &imageInfo, 0);
                                        if (!priv.grayMapImage)
                                                return FALSE;
                                        ilQueryClientImage(priv.grayMapImage,
                                                           &pImageInfo, 0);
                                        priv.pGrayMapPixels =
                                            pImageInfo->plane[0].pPixels;
                                }
                                des = *IL_DES_GRAY;
                        ...
                        /*  Support 8 bit Pseudo/StaticColor as palette image;
                         * alloc palette. */
                case StaticColor:
                case PseudoColor:
                        if (depth > 8)
                        ...
                        format = *IL_FORMAT_BYTE;
                        pixelSize = 8;
                        if (!(priv.pPalette = (unsigned short *)IL_MALLOC_ZERO(
                        ...
                        /*  Support True/DirectColor only if format = "<unused
                         * 8><8R><8G><8B>". */
                case DirectColor:
                case TrueColor:
                        if ((depth == 24) &&
                            ...
                            (pVisualInfo->blue_mask == 0xff)) {
                                ...
                                if (!(flags & IL_READ_X_RAW_MODE)) {
                                        ...
                                        imageInfo.height = 1;
                                        imageInfo.clientPixels = FALSE;
                                        priv.rgbMapImage = ilCreateClientImage(
                                            pipe->context, &imageInfo, 0);
                                        if (!priv.rgbMapImage)
                                                return FALSE;
                                        ilQueryClientImage(priv.rgbMapImage,
                                                           &pImageInfo, 0);
                                        priv.pRGBMapPixels =
                                            pImageInfo->plane[0].pPixels;
                                }
                        ...
                        /*suport for Gacko and 12 bit display depth */
                                ...
                                /*counting the number of ones and zeros in each
                                  red, green and blue mask */
                                /* calculating Number of zeros and ones for
                                 * red_mask */
                                /******************************************************/
         ...
         */
        priv.stripHeight = ilRecommendedStripHeight(
            &des, &format, priv.rect.width, priv.rect.height);

        /*  Create a GC if copyToPixmap (set copyPixmapDepth != 0).  Null ptrs
            out and call ilReadXDestroy() if failure - it will free non-null
           objects.
        */
        ...
        if (copyToPixmap) {
                ...
                values.subwindow_mode =
                    IncludeInferiors; /* get subwindow contents */
                priv.copyGC = XCreateGC(priv.display, priv.drawable,
        ...
        /*  Add a producer element to read from the X drawable; copy priv into
         * *pPriv. */
        ...
        pPriv = (ilReadXPrivPtr)ilAddPipeElement(
            pipe, IL_PRODUCER, sizeof(ilReadXPrivRec), 0,
            (ilSrcElementData *)NULL, &dstData, ilReadXInit, ilReadXCleanup,
            ilReadXDestroy, ((SlowMode) ? ilReadXExecuteSlow : ilReadXExecute),
            0);

        /*
            pPriv = (ilReadXPrivPtr)ilAddPipeElement (pipe, IL_PRODUCER, sizeof
           (ilReadXPrivRec), 0, (ilSrcElementData *)NULL, &dstData, ilReadXInit,
           ilReadXCleanup, ilReadXDestroy, ilReadXExecuteSlow, 0);
