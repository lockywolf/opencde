...
static ilError ilComppackbitsInit(ilCompPBPrivPtr pPrivate,
...
static ilError ilComppackbitsExecute(ilExecuteData *pData,
                                     unsigned long dstLine,
                                     unsigned long *pNLines) {

#define PB_MAX_BUFFER_WRITE                                                    \
        140 /* max bytes written after check for room in compressed buffer */

        unsigned long nBytes =
            0; /* Number of source image bytes to unpack for current strip */
        ...
        ilPtr pDstBufferEnd; /* Ptr to byte "PB_MAX_BUFFER_WRITE" from end of
                                compressed buffer  */
        ...
        long offset; /* difference between current dst image address pointer and
                    the start address of the dst image buffer
                  */
        ...
        pDstBufferEnd =
            pDstPlane->pPixels + (pDstPlane->bufferSize - PB_MAX_BUFFER_WRITE);
        ...
        while (nLines-- > 0) {
                ...
                while (j <= nBytesM1) {
                        ...
                        if (pSrcByte[j] == pSrcByte[j + 1]) { /*   repeat run */
                                ...
                                while ((pSrcByte[j] == pSrcByte[j + 1]) &&
                                       (j < nBytesM1) && ((j - start) < 127))
                                        j++;

                                /*    output buffer size handling code */
                                if (pDstByte >= pDstBufferEnd) {
                                        offset = pDstByte - pDstPlane->pPixels;
                                        if (!_ilReallocCompressedBuffer(
                                                pData->pDstImage, 0,
                                                offset + PB_MAX_BUFFER_WRITE))
                                                return IL_ERROR_MALLOC;
                                        pDstByte = pDstPlane->pPixels + offset;
                                        pDstBufferEnd = pDstPlane->pPixels +
                                                        (pDstPlane->bufferSize -
                                                         PB_MAX_BUFFER_WRITE);
                                }
...
IL_PRIVATE ilBool _ilCompressPackbits(ilPipe pipe, ilPipeInfo *pinfo,
                                      ilImageDes *pimdes,
        ...
        /*  Validate that image is bit/pixel, or if not bit, convert to byte if
         * fits. */
        ...
        if (pimformat->nBitsPerSample[0] == 1)
                ...
                if (pimformat->nBitsPerSample[0] != 8)
                        if (!ilConvert(pipe, (ilImageDes *)NULL, IL_FORMAT_BYTE,
                                       0, (ilPtr)NULL))
                                return FALSE;
        }
        ...
        pPriv = (ilCompPBPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilCompPBPrivRec), 0, pSrcData, &dstdata,
        ...
        pPriv->widthbytes =
            (bitPerPixel) ? (pPriv->width + 7) / 8 : pPriv->width;
