...
typedef struct {
        unsigned long R[256], G[256],
            B[256];       /* RGB->gray multiply lookup table */
...
static ilError ilInitRGBGrayConversions(ilRGBToGrayRecPtr pPriv,
...
/* =============================================================================================================================
        ...
        Called only for RGB->gray conversion: init RGB lookup/multiply tables in
   pPriv
   =============================================================================================================================
 */

static ilError
ilAddElementRGBGrayConversions(ilRGBToGrayRecPtr pPriv,
                               unsigned short *pPalette, /* not used */
                               void *pOptionData         /* not used */
) {
        register int i;

        /*  Init tables, index by R/G/B, which effectively multiply each
           component (0..255) by the NTSC conversion formula: 0.30 * R + 0.587 *
           G + 0.114 * B, except that each value is * 65536 for extra accuracy:
           the result is shifted right by 16 before storing. Note that "19660"
           would probably be more accurate at "19661", but the old code used
           19660 and for that little difference it is silly to invalidate the
           old checksum tests.  Note also that if R/G/B are all = 255 the result
           is still <= 255 after the shift right.
        */
        for (i = 0; i < 256; i++) {
                pPriv->R[i] = 19660 * i;
                pPriv->G[i] = 38469 * i;
...
/* =============================================================================================================================
            ilExecuteRGBToGray    -    Execute() function: convert the given #
   of src lines.
...
static ilError ilExecuteRGBToGray(ilExecuteData *pData, long dstLine,
                                  long *pNLines) {
        ...
        while (nLines-- > 0) {
                ...
                /*  --    grayscale from:     Y  =   0.30red  + 0.587green +
                 * 0.114blue    */
                while (nPixels-- > 0) {
                        i = pPriv->R[*psrc++];
                        i += pPriv->G[*psrc++];
...
/* =============================================================================================================================
          ilRGBToGray          -    Table exported to ilConvert(), declared in
   /ilc/ilconvert.h
   =============================================================================================================================
 */
...
static ilError ilExecuteGrayToRGB(ilExecuteData *pData, long dstLine,
                                  long *pNLines) {
...
/* =====================================================================================
          ilGrayToRGB  -    Table exported to ilConvert(), declared in
   /ilc/ilconvert.h
   ====================================================================================
 */
