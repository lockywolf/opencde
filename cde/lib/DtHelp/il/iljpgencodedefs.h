...
*/
IL_EXTERN iljpgError _ilReallocJPEGEncode(ilJPEGEncodeStreamPtr pStream,
                                          long nBytes);
...
#define ILJPG_ENCODE_PUT_BYTE(_stream, _byte, _error)                          \
        (((_stream)->pDst >= (_stream)->pPastEndBuffer)                        \
             ? ((_error = _ilReallocJPEGEncode(_stream, ILJPG_REALLOC_EXTRA))  \
                    ? 0                                                        \
                    : (*(_stream)->pDst++ = _byte, 1))                         \
             : (*(_stream)->pDst++ = _byte, 1))

/*  Macro to return position within dst buffer, like ftell() */
#define ILJPG_ENCODE_OFFSET(_stream) ((_stream)->pDst - (_stream)->pBuffer)
