...
/*  ========================== Fast Code ================================= */
/*  This code handles the upsampling cases where the vertical and horizontal
   subsample factors are equal (generally the case).  It is faster than the
   general case code.
*/

/*  Private for fast upsampling filters. "subsampleShift" is the shift value
   (0,1,2) for how that sample was subsampled; "shift" is the shift for
   upsampling that sample.  They are the same unless "double";
   shift[i]=subsampleShift[i]+1 then.
*/
typedef struct {
 ...
 */
static void ilUpsampleQuadruple(long nSrcLines, long nSrcBytes,
                                long srcRowBytes, ilPtr pSrcLine,
                                register long dstRowBytes, ilPtr pDstLine) {
        ...
        while (nSrcLines-- > 0) {        /* double and interpolate next line */
                ...
                while (TRUE) {
                        ...
                        *((unsigned long *)pDst) = aLong;
                        ...
                        if (nSrcBytesM1 ==
                            0) { /* next-to-last pixel; back up src */
                                pSrc--;
                                pSrcBelow--;
                        }
...
/*  -------------------------- ilExecuteFastUpsample
 * ------------------------------ */
/*  Execute(): upsample as necessary "pPriv->nSamples" planes of the source
   image, where the hori/vertical upsampling factors are equal for each plane.
*/
static ilError ilExecuteFastUpsample(ilExecuteData *pData, long dstLine,
                                     long *pNLines) {
        ...
        *pNLines =
            nLines << pPriv->scaleFactor; /* # of dst lines after any scaling */
        if (nLines <= 0)
                return IL_OK;
        ...
        */
        ...
        for (sample = 0; sample < pPriv->nSamples;
             sample++, pSrcPlane++, pDstPlane++) {
                subsampleShift = pPriv->subsampleShift[sample];

                srcRowBytes = pSrcPlane->nBytesPerRow;
                pSrcLine = pSrcPlane->pPixels +
                           (pData->srcLine >> subsampleShift) * srcRowBytes;
                dstRowBytes = pDstPlane->nBytesPerRow;
                pDstLine = pDstPlane->pPixels + dstLine * dstRowBytes;

                /*  Handle tiny image: if subsampling left nothing, fill with
                 * zeros. */
                ...
                if ((nSrcLines <= 0) || (nSrcBytes <= 0)) {
                        ...
                        case 0:
                                ...
                                /*  Doubling: loop while there are a pair of src
                                   lines, replicating and interpolating into
                                   dst.  For each pixel across: do left with
                                    interpolated pixel between.  For each pixel
                                   pair, interpolate between hori and
                                   vertically.  Replicate last pixel.  For last
                                   line, point "pSrcBelow" to last src line:
                                   replication of last line occurs.
                                */
                        case 1:
                                pSrc = pSrcLine;
                                pDst = pDstLine;
                                while (
                                    nSrcLines-- >
                                    0) { /* double and interpolate next line */
                                        pSrc = pSrcLine;
                                        if (nSrcLines >
                                            0) /* else use last for below line
                                                */
                                                pSrcLine += srcRowBytes;
                                        pSrcBelow = pSrcLine;
                                        pDst = pDstLine;
                                        pDstLine +=
                                            dstRowBytes
                                            << 1; /* skip down 2 dst lines */

                                        leftBelow =
                                            *pSrcBelow++; /* do first (left)
                                                             byte */
                                        left = *pSrc++;
                                        pDst[dstRowBytes] =
                                            (left + leftBelow) >> 1;
                        ...
                        case 2:
                                ilUpsampleQuadruple(nSrcLines, nSrcBytes,
                                                    srcRowBytes, pSrcLine,
...
/*  ---------------------------- _ilFastUpsample -----------------------------
 */
/*  Attempt to upsample and scale based on "scaleFactor" (0 = no scaling; 1 =
   double; 2 = 4x), or return false if it cannot be done with the given pipe
   image (in which case pipe->context->error == 0) or if error occurs (error !=
   0).  Pipe image must be decompressed before calling this function. Note: when
   scaling up by 2x or 4x, always yields even/ *4 width/height. Thus, *cannot*
   be used to upsample odd width/height YCbCr images.
*/
IL_PRIVATE ilBool _ilFastUpsample(ilPipe pipe, ilPipeInfo *pInfo,
        ...
        case IL_GRAY:
                if (!scaleFactor)
                        return TRUE; /* no scaling is noop; return */
        ...
        case IL_RGB:
                if (!scaleFactor)
                        return TRUE; /* no scaling is noop; return */
                if (!ilConvert(pipe, IL_DES_RGB, IL_FORMAT_3BYTE_PLANE, 0,
                               NULL))
                        return FALSE;
                upPriv.nSamples = 3; /* double three planes */
                upPriv.subsampleShift[0] = upPriv.subsampleShift[1] =
                    upPriv.subsampleShift[2] = 0;
                upPriv.shift[0] = upPriv.shift[1] = upPriv.shift[2] =
                    scaleFactor;
                ilGetPipeInfo(pipe, FALSE, pInfo, pDes, pFormat);
                break;

                /*  YCbCr: hori/vert subsample each plane must be equal and must
                 * be planar */
        case IL_YCBCR:
                if (pFormat->sampleOrder != IL_SAMPLE_PLANES)
                        return FALSE;
                upPriv.nSamples = 3;
                for (i = 0, pSample = pDes->typeInfo.YCbCr.sample; i < 3;
                     i++, pSample++) {
                        if (pSample->subsampleHoriz != pSample->subsampleVert)
                                return FALSE;
                        j = _ilSubsampleShift[pSample->subsampleHoriz];
                        upPriv.subsampleShift[i] =
                            j;            /* shift due to subsampling */
                        j += scaleFactor; /* plus that due to scaling */
                        if (j > 2)        /* more than 4x scale; can't do */
                                return FALSE;
                        upPriv.shift[i] =
                            j; /* shift due to subsampling and doubling */
                }
        ...
        /*  Set format: 32 bit alignment required if any scales up by 4x.
            Don't mark as such if width multiple of 4, as other filters may
           check it.
        */
        ...
        /*  Use current strip height rather than allowing image to be split into
            smaller strips; there is a copied rather than smoothed line at the
           end of each strip, so should do min number of strips.  Could force no
           strips for better quality, but performance penalty probably not worth
           it.
        */
        ...
        pUpPriv = (ilUpFastPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilUpFastPrivRec), 0, &srcData, &dstData,
            IL_NPF, IL_NPF, IL_NPF, ilExecuteFastUpsample, NULL, 0);
        if (!pUpPriv)
                return FALSE;
        *pUpPriv = upPriv;
 ...
 * ================================= */
/*  This code handles the upsampling cases where the vertical and horizontal
   subsample factors are not the same.
*/
...
typedef struct {
        ...
        ilUpsampleShiftRec
            shift[IL_MAX_SAMPLES]; /* upsample mul as shift value */
...
static void ilUpsampleHorizontal(
    ...
    ilPtr pDstLine      /* ptr to first line of dst image */
        ...
        case 0:
                ...
                /*  1: doubling; The image for this plane is 1/2 "width".  Copy
                   the first pixel, interpolate pixels between previous pixels
                   and the last pixel, then replicate last pixel to fill out
                   width.  Example: width == 9, source width = 4 (0..3): A
                   B                       C: <0> | <0+1/2> <1> <1+2/2> <2>
                   <2+3/2> <3> | <3> <3> A: write first src pixel (pixel <0>);
                   set into "prevPixel" B: write (interpolated pixel, src pixel)
                   pair "nMidPixels" times, leaving srcPixel in prevPixel C:
                   replicate last pixel (prevPixel) "nEndPixels" times
                */
        case 1:
                ...
                while (nLines-- > 0) {
                        ...
                        pDstLine += dstRowInc;
                        prevPixel = *pSrc++;
                        *pDst++ = prevPixel;  /* prevPixel = first src pixel;
                                                 copy to dst */
                ...
                /*  1: quadrupling; The image for this plane is 1/4 "width".
                   Copy the first pixel, interpolate pixels between previous
                   pixels and the last pixel, then replicate last pixel to fill
                   out width.  To interpolate: add "delta" = pixel-prevPixel to
                   "temp" (which starts at prevPixel*4) then divide temp by 4
                   before storing. For example, if prevPixel = 3 and pixel = 6,
                   delta = 3, pixels are: 3 (prevPixel) | (12+3)/4=3
                   (15+3)/4=4   (18+3)/4=5   6 (src pixel) |
                */
        case 2: {
...
/*  -------------------------- ilUpsampleVertical ------------------------------
 */
/*  Do vertical upsampling as necessary, after the horizontal upsampling is
   done. Vertical upsampling is done in the dst buffer: pLine points to first
   line (which contains an already upsampled line); rowBytes is bytes/row of
   buffer.
*/
static void ilUpsampleVertical(
    ...
    ilPtr pLine             /* ptr to first line of src/dst image */
        ...
        case 0:
                ...
                /*  1: doubling.  Interpolate between pairs of lines, starting
                   with the first and third lines, filling in the second with
        ...
        case 1: {
                ...
                while (nLines >=
                       2) { /* two src lines, line between to interpolate */
                        nLines -= 2;
                        pSrc1 = pLine;
                        pLine += rowBytes;
                        pDst = pLine;
                ...
                /*  1: quadrupling.  Interpolate between quadruples of lines,
                   starting with lines 0 and 4, interpolating 1..3 - see
        ...
        case 2: {
                ...
                nLines--; /* dont count first line already in buffer */
                while (nLines >=
                       4) { /* four src lines, line between to interpolate */
                        nLines -= 4;
                        pSrc = pLine;
                        ...
                        do {
                                pDst = pSrc;
                                pixel = *pSrc++; /* pixel from top src line;
                                                    next pixel */
                                delta = *(pDst + rowBytesTimes4) -
                                        pixel; /* delta = bottom - top */
                                pixel <<= 2;   /* work in pixels * 4, /4 before
                                                  storing */
                                pixel += delta;
                                pDst += rowBytes;
 ...
 * image.
 */
static ilError ilExecuteUpsample(ilExecuteData *pData, long dstLine,
        ...
        for (pSrcPlane = pData->pSrcImage->plane,
            ...
            nSamples = pPriv->nSamples;
             nSamples-- > 0; pSrcPlane++, pDstPlane++, pShift++) {
                ...
                srcRowBytes = pSrcPlane->nBytesPerRow;
                pSrcLine = pSrcPlane->pPixels +
                           (pData->srcLine >> pShift->vert) * srcRowBytes;
                dstRowBytes = pDstPlane->nBytesPerRow;
                pDstLine = pDstPlane->pPixels + dstLine * dstRowBytes;

                /*  Upsample horizontal "height >> pShift->vert" src lines (e.g.
                   half the lines for shift = 1).  Write the results starting at
                   pDstLine, each line "dstRowBytes << pShift->vert" bytes after
                   the other, e.g. if pShift->vert is 1 (double), leave one line
                   gap between each dst line. But first, handle very small
                   image: if subsampling left nothing, fill with zeros.  Note
                   that small strips will not cause this to happen, because
                   stripHeight checked when element added.
                */
                ...
                if ((nLines <= 0) || ((width >> pShift->horiz) <= 0)) {
                        ...
                        ilUpsampleHorizontal(
                            pShift->horiz, width, nLines, srcRowBytes, pSrcLine,
                            dstRowBytes << pShift->vert, pDstLine);

                        /*  Upsample vertically, interpolating between the lines
                         * just written. */
                        ilUpsampleVertical(pShift->vert, width, height,
                                           dstRowBytes, pDstLine);
                }
        }
...
/*  ---------------------------- ilUpsampleYCbCr -----------------------------
 */
/*  Upsample and / or convert to gray the pipe image which must be a YCbCr
   image. If "toGray" is true, the Y plane only will be upsampled (or copied)
   resulting in a gray image; else a planar YCbCr image will result. If
   "upSample" is true must upsample; "upSample" and/or "toGray" must be true.
    pFormat points to the source format; on return, *pFormat is updated
    to the dst format, *pDes to the dst descriptor.
*/
IL_PRIVATE ilBool _ilUpsampleYCbCr(ilPipe pipe, ilPipeInfo *pInfo,
                                   ilImageDes *pDes, ilImageFormat *pFormat,
                                   ilBool toGray, ilBool upSample) {
        ...
        /*  Only 8 bit planar YCbCr can currently be upsampled; if not that and
           upsampling needed, error; if no upsampling (only convert to gray)
           convert to planar. NOTE: can't ilConvert() to planar if upsampling
           needed as ilConvert() will call this function back to upsample,
           thereby recursing "forever".
        */
        if ((pFormat->sampleOrder != IL_SAMPLE_PLANES) ||
            ...
            (pFormat->nBitsPerSample[2] != 8)) {
                if (upSample)
                        return ilDeclarePipeInvalid(pipe,
                                                    IL_ERROR_NOT_IMPLEMENTED);
                if (!ilConvert(pipe, (ilImageDes *)NULL, IL_FORMAT_3BYTE_PLANE,
                               0, NULL))
                        return FALSE;
    ...
    if (!toGray) {
        ...
        /*  Demand constant strips, a multiple of maximum subsample (4). To
           avoid the problem of having the last strip have 3 lines or less (and
           therefore for * 4 upsampling require that the previous line be copied
           - but no previous line available), bump stripHeight by 4 until
           stripHeight mod 4 > 3, or until requiring whole image as one strip.
        */
        i = 8;
        while (TRUE) {
                j = dstData.height % i;
        ...
        /*  Add a filter to upsample; if "toGray" upsample/copy one plane and
         * done. */
        if (toGray) {
                *pDes = *IL_DES_GRAY;
        ...
        pUpPriv = (ilUpsamplePrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilUpsamplePrivRec), 0, &srcData, &dstData,
            IL_NPF, IL_NPF, IL_NPF, ilExecuteUpsample, NULL, 0);
        if (!pUpPriv)
                return FALSE;
        ...
        for (sample = 0; sample < pUpPriv->nSamples;
             sample++, pShift++, pSample++) {
