...
static ilEFSPrivatePtr ilInitEFS(ilContext context) {
        ...
        */
        context->error = IL_OK;
        pPriv = (ilEFSPrivatePtr)((ilContextPtr)context)
                    ->pAlloc[IL_CONTEXT_ALLOC_EFS];
        if (!pPriv) {
                ...
                ((ilContextPtr)context)->pAlloc[IL_CONTEXT_ALLOC_EFS] =
                    (ilPtr)pPriv;

                /*  Init file type list to null. */
                pPriv->fileTypeHead.pNext = pPriv->fileTypeHead.pPrev =
                    (ilEFSFileTypePtr)&pPriv->fileTypeHead;
                pPriv->replacedFileTypeHead.pNext =
                    pPriv->replacedFileTypeHead.pPrev =
...
*/
static ilEFSFileTypePtr ilFindFileType(char *name, ilEFSFileTypePtr pListHead) {
...
/*  Add the file type pointed to by "pFileType" to the list of file types whose
    ...
    checkOrder.
*/
static void ilAddFileTypeToList(register ilEFSFileTypePtr pFileType,
        ...
        LINK_FILE_TYPE(pFileType,
                       pListFileType) /* insert in front of pListFileType */
}
...
static void ilDestroyFileType(register ilEFSFileTypePtr pFileType) {
        ilEFSPrivatePtr pPriv;
        register ilEFSFileTypePtr pReplaced;

        pPriv = (ilEFSPrivatePtr)((ilContextPtr)(pFileType->h.o.p.context))
                    ->pAlloc[IL_CONTEXT_ALLOC_EFS];

        /*  Remove file type from its current list.  Search the "replaced" list
           for a file type of same name; if found, move it from replaced to
           active list.
        */
        UNLINK_FILE_TYPE(pFileType)
        pReplaced =
            ilFindFileType(pFileType->info.name,
                           (ilEFSFileTypePtr)&pPriv->replacedFileTypeHead);
        if (pReplaced) {
                UNLINK_FILE_TYPE(pReplaced)
                ilAddFileTypeToList(pReplaced,
                                    (ilEFSFileTypePtr)&pPriv->fileTypeHead);
        }
}

ilEFSFileType ilEFSAddFileType(ilContext context,
...
#define OPEN_MODE_MASKS                                                        \
        (1 << IL_EFS_READ | 1 << IL_EFS_READ_SEQUENTIAL | 1 << IL_EFS_WRITE)
#define ATTRIBUTES_MASKS                                                       \
        (IL_EFS_MULTI_PAGE_READS | IL_EFS_MULTI_PAGE_WRITES |                  \
        ...
        /*  Validate *pInfo: strings present, unused mask bits zero, functions
         * right. */
        if ((strlen(pInfo->name) > (IL_EFS_MAX_NAME_CHARS - 1)) ||
            (strlen(pInfo->displayName) >
             (IL_EFS_MAX_DISPLAY_NAME_CHARS - 1)) ||
            (pInfo->nExtensions > IL_EFS_MAX_EXTENSIONS))
                goto badFileTypeInfo;

        for (i = 0; i < pInfo->nExtensions; i++)
                if (strlen(pInfo->extensions[i]) >
                    (IL_EFS_MAX_EXTENSION_CHARS - 1))
                        goto badFileTypeInfo;
        ...
        pFileType = (ilEFSFileTypePtr)_ilCreateObject(context, IL_EFS_FILE_TYPE,
                                                      ilDestroyFileType,
                                                      sizeof(ilEFSFileTypeRec));
        if (!pFileType)
                return (ilEFSFileType)NULL;
        pFileType->info = *pInfo;

        /*  Search the active list for file type with same name.  If found,
           remove it from active list and add to replaced list, in front of any
           with same name.
        */
...
/*  Public function: see spec.
 */
ilBool ilEFSGetFileTypeInfo(ilEFSFileType fileType, ilEFSFileTypeInfo *pInfo) {
        register ilEFSFileTypePtr pFileType;
...
/*  Public function: see spec.
 */
ilBool ilEFSListFileTypes(ilContext context, int *pNFileTypes,
        ...
        *pfileTypes = (ilEFSFileType *)NULL;
        ...
        /*  Malloc space for returned file types - use malloc(); caller uses
           free(). If no file types, still malloc something to return non-null
           ptr.
        */
        pfileType = (ilEFSFileType *)malloc(
            (nFileTypes > 0) ? nFileTypes * sizeof(ilEFSFileType) : 4);
        if (!pfileType) {
                context->error = IL_ERROR_MALLOC;
                return FALSE;
        }
...
static void ilDestroyFile(ilEFSFilePtr pFile) {
        register ilEFSFileTypePtr pFileType;

        /*  Get ptr to file type; if null, file not actually open yet; skip
         * Close() */
...
ilEFSFile ilEFSOpen(ilContext context, char *fileName, unsigned int openMode,
                    unsigned long searchOptions, char *typeName,
                    void *pOptions) {
        ilEFSPrivatePtr pPriv;
        ...
        /*  Add a file type object - goto openError to destroy it if an error
         * later. */
        pFile = (ilEFSFilePtr)_ilCreateObject(
            context, IL_EFS_FILE, ilDestroyFile, sizeof(ilEFSFileRec));
        if (!pFile)
                return (ilEFSFile)NULL;

        /*  Find pFileType for this file, searches enabled by mask in
           searchOptions. First try typeName, if non-null.  When found at any
           point: if openMode not supported, error - except for checking mode -
           else call Open() for the file type.  If it returns any error other
           than "not mine" abort.
        */
        ...
        if (typeName && (searchOptions & IL_EFS_BY_TYPE_NAME)) {
                ...
                if (pFileType) { /* file type found */
                        ...
                        pOpenPriv = (*pFileType->info.Open)(pFileType, fileName,
                                                            openMode, &nPages);
                        if (!pOpenPriv) {
                                if (context->error == IL_ERROR_EFS_NOT_MINE)
                                        pFileType = (ilEFSFileTypePtr)
                                            NULL; /* try next search method */
                                else
                                        goto openError;
                        }
        ...
        if (!pFileType && (searchOptions & IL_EFS_BY_EXTENSION)) {
                ...
                if (pExtension) {     /* is a "." in fileName */
                        ...
                        bcopy(pExtension, extension,
                              nChars); /* extract "extension" */
                        extension[nChars] = 0;

                        /* Search list for extension match until pFileType found
                         * or list done */
                        pSearch = pListHead->h.pNext;
                        while (!pFileType && (pSearch != pListHead)) {
                                register int nExtensions =
                                    pSearch->info.nExtensions;
                                while (nExtensions-- > 0)
                                        if (STRING_EQUAL(
                                                extension,
                                                pSearch->info
                        ...
                        if (pFileType) {
                                if (!(pFileType->info.openModes &
                                      (1 << openMode))) {
                                        context->error = IL_ERROR_EFS_OPEN_MODE;
                                        goto openError;
                                }
                                pOpenPriv = (*pFileType->info.Open)(
                                    pFileType, fileName, openMode, &nPages);
                                if (!pOpenPriv) {
                                        if (context->error ==
                                            IL_ERROR_EFS_NOT_MINE)
                                                pFileType =
                                                    (ilEFSFileTypePtr)NULL;
                                        else
                                                goto openError;
                                }
                        }
        ...
        /*  If not found, search by checking if a read openMode.   For each file
           type, try open if enabled (checkOrder != 0) and openMode supported
           for file type.
        */
        if (!pFileType && readOpen && (searchOptions & IL_EFS_BY_CHECKING)) {
                ...
                while (pSearch != pListHead) {
                        if (pSearch->info.checkOrder &&
                            (pSearch->info.openModes & (1 << openMode))) {
                                pOpenPriv = (*pSearch->info.Open)(
                                    pSearch, fileName, openMode, &nPages);
                                if (pOpenPriv) {
                                        pFileType =
                                            pSearch; /* found right file type;
                                                        break */
                                        pFile->info.howFound =
                                            IL_EFS_BY_CHECKING;
                                        break;
        ...
        /*  If not found above, error: can't find file type handler for this
         * file. */
        if (!pFileType) {
        ...
        /*  File type found.  Fill in info for the file, return pFile.  The
           object's pPrivate is set to the ptr returned from file type's Open().
        */
        ...
        /*  openError: goto here on error after file object created. The Open()
           was not successful, so set fileType to null so ilDestroyFile() does
           not call Close().
        */
openError : {
        ilError error;
...
/*  Public function: see spec.
 */
ilBool ilEFSGetFileInfo(ilEFSFile file, ilEFSFileInfo *pInfo /* RETURNED */
) {
        register ilEFSFilePtr pFile;
...
/*  Public function: see spec.
 */
ilBool ilEFSSeek(ilEFSFile file, long page, void *pOptions) {
        register ilEFSFilePtr pFile;
        ...
        if (!(pFile->info.attributes & IL_EFS_MULTI_PAGE_READS))
                ...
                return (*pFile->pFileType->info.Seek)(file, page);
}

/*  ------------------------ ilEFSGetPageInfo ---------------------------- */
/*  Public function: see spec.
 */
ilBool ilEFSGetPageInfo(ilEFSFile file, ilEFSPageInfo *pInfo) {
        register ilEFSFilePtr pFile;
...
/*  Public function: see spec.
 */
ilBool ilEFSReadImage(ilPipe pipe, ilEFSFile file, unsigned int readMode,
                      long width, long height, void *pOptions) {
        register ilEFSFilePtr pFile;
        ...
        if ((pipe->objectType != IL_PIPE) ||
            (pFile->o.p.objectType != IL_EFS_FILE)) {
                pFile->o.p.context->error = IL_ERROR_OBJECT_TYPE;
                return FALSE;
        }
        ...
        /*  If pipe element added successfully inc file's refCount to mark in
           use, and add file to list of objects to be destroyed when pipe
           emptied.
        */
        pFile->o.p.context->error = IL_OK;
        if ((*pFile->pFileType->info.ReadImage)(pipe, file, readMode, width,
                                                height)) {
                pFile->o.refCount++;
...
/*  Public function: see spec.
 */
ilBool ilEFSWriteImage(ilPipe pipe, ilEFSFile file, long xRes, long yRes,
                       ilClientImage maskImage, void *pOptions) {
        register ilEFSFilePtr pFile;
        ...
        /*  If pipe element added successfully inc file's refCount to mark in
           use, and add file/maskImage to list of objects to be destroyed when
           pipe emptied.
        */
        pFile->o.p.context->error = IL_OK;
        if ((*pFile->pFileType->info.WriteImage)(pipe, file, xRes, yRes,
                                                 maskImage)) {
                if (maskImage) {
