...
typedef struct {
        ...
        unsigned short
            *pPalette;      /* ptr to palette or null if not palette image */
 ...
 */
static ilError ilWriteImageInit(ilImagePrivPtr pPriv, ilImageInfo *pSrcImage,
...
*/
static ilError ilWriteImageDestroy(ilImagePrivPtr pPriv) {
 ...
 */
static ilError ilWriteCompressedCleanup(ilImagePrivPtr pPriv, ilBool aborting) {
        ...
        /*  If # of strips written by pipe is not equal to # of strips in image,
         * error */
        ...
        if (aborting) {
                ...
                /*  Success: realloc (shrink) the image down to the written size
                   (offset to strip past last strip), to free up extra
                   unused-but-allocated space.
                */
                pImagePlane->bufferSize =
                    pImage->pStripOffsets[pImage->nStrips];
                if (pImagePlane->pPixels)
                        pImagePlane->pPixels = (ilPtr)IL_REALLOC(
                            pImagePlane->pPixels, pImagePlane->bufferSize);
        }
 ...
 */
static ilError ilWriteCompressedExecute(register ilExecuteData *pData,
        ...
        /*  If first strip: copy non-null pipe palette (pPriv->pPalette) and
           comp data (NOTE: assumed to be JPEG!), after freeing old tables, to
           the image.
        */
        if (pData->srcLine <= 0) {
                if (pPriv->pPalette)
                        bcopy((char *)pPriv->pPalette,
                              (char *)pPriv->pImage->i.pPalette,
        ...
        /*  If # lines written != strip size and not last strip, inconstant
         * strips */
        if ((*pNLines != pImage->stripHeight) &&
            (pData->srcLine != pImage->i.height))
                return IL_ERROR_MALFORMED_IMAGE_WRITE;

        /*  Store offset to this and next strip; bump offset for next write */
        pImage->pStripOffsets[pPriv->nStripsWritten++] =
...
/*  Called by ilWriteImage() to add a consumer to "pipe" which writes compressed
    data to the given "image".  "pipe" and "image" are guaranteed to be the
   correct object types and to belong to the same context.  The pipe is
   guaranteed to be in the correct state (IL_PIPE_FORMING) and "image" is
   guaranteed to be the correct size (size of the pipe image).
*/
static ilBool ilWriteCompressedImage(ilPipe pipe, ilObject image) {
        register ilImagePtr pImage;
        ilPipeInfo info;
        ilImageDes des;
        ...
        if ((des.type != pImage->des.type) ||
            ...
            (des.nSamplesPerPixel != pImage->des.nSamplesPerPixel))
                ...
                for (plane = 0; plane < des.nSamplesPerPixel; plane++)
                        if ((des.nLevelsPerSample[plane] !=
                             pImage->des.nLevelsPerSample[plane]) ||
                            (format.nBitsPerSample[plane] !=
                             pImage->format.nBitsPerSample[plane])) {
                                mustConvert = TRUE;
                                break;
                        }
        }
        if (mustConvert) {
                ...
                tempDes.compression =
                    IL_UNCOMPRESSED; /* ilConvert() error otherwise */
                if (!ilConvert(pipe, &tempDes, (ilImageFormat *)NULL, 0,
                               (ilPtr)NULL))
                        return FALSE;
                des.compression = IL_UNCOMPRESSED; /* force recompress below */
        }

        /*  Compress to compression type of image, including type-specific
           (G3/JPEG) compression data.  Compress to image's strip height if is
           established (pStripOffsets non-null), else set "stripHeight" to 0 and
           use default.
        */
        ...
        if (!ilCompress(pipe, pImage->des.compression, pCompData, stripHeight,
                        0))
                return FALSE;
        ilGetPipeInfo(pipe, FALSE, &info, (ilImageDes *)NULL,
                      (ilImageFormat *)NULL);

        /*  If image does not yet have a strip height, set it to pipe's strip
         * height */
        if (!stripHeight)
        ...
        pPriv = (ilImagePrivPtr)ilAddPipeElement(
            pipe, IL_CONSUMER, sizeof(ilImagePrivRec), 0, &srcData,
            (ilDstElementData *)NULL, ilWriteImageInit,
            ilWriteCompressedCleanup, ilWriteImageDestroy,
            ilWriteCompressedExecute, 0);
        if (!pPriv)
                return FALSE;

        pPriv->pImage = pImage;
        pPriv->pPalette = (pImage->des.type == IL_PALETTE)
                              ? info.pPalette
                              : (unsigned short *)NULL;
 ...
 */
static ilError ilWriteImageExecute(register ilExecuteData *pData, long dstLine,
                                   long *pNLines) {
        ilImagePrivPtr pPriv;

        pPriv = (ilImagePrivPtr)pData->pPrivate;

        /*  If first strip, copy pipe palette (pPriv->pPalette) to the image
         * palette. */
...
*/
ilBool ilWriteImage(ilPipe pipe, ilObject image) {
        register ilImagePtr pImage;
        register ilImagePrivPtr pPriv;
        ...
        /*  Validate that pipe and image are such, and that they have the same
         * context. */
        pImage = (ilImagePtr)image;
        ...
        /*  Get pipe info; if pipe not in IL_PIPE_FORMING state: error.
            If producerObject is the given image, error: cant read/write same
           image.
        */
        if (ilGetPipeInfo(pipe, FALSE, &info, (ilImageDes *)NULL,
         ...
         */
        if ((info.width != pImage->i.width) ||
            (info.height != pImage->i.height))
        ...
        /*  Convert des and/or format if not same as the images format; exit if
         * error. */
        if (!ilConvert(pipe, pImage->i.pDes, pImage->i.pFormat, 0, NULL))
                return FALSE;
        ilGetPipeInfo(pipe, TRUE, &info, (ilImageDes *)NULL,
                      (ilImageFormat *)NULL);

        /*  Add a consumer element which writes to the given image.
            If writing a palette image, supply an Init() function to copy the
           palette.
        */
        srcData.consumerImage = (ilObject)pImage;
        srcData.stripHeight = 0;
        ...
        pPriv = (ilImagePrivPtr)ilAddPipeElement(
            pipe, IL_CONSUMER, sizeof(ilImagePrivRec), 0, &srcData,
            (ilDstElementData *)NULL, ilWriteImageInit, IL_NPF,
