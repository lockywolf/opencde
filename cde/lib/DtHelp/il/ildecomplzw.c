...
static ilError ilDecompLZWInit(ilDecompPrivPtr pPriv, ilImageInfo *pSrcImage,
        ...
        pPriv->denodes =
            (ilDenodePtr)IL_MALLOC(sizeof(ilDenodeRec) * MAX_NODES);
        if (!pPriv->denodes) {
...
static ilError ilDecompLZWCleanup(ilDecompPrivPtr pPriv, ilBool aborting) {
        if (pPriv->pString)
                IL_FREE(pPriv->pString);
        if (pPriv->denodes)
...
static int ilCurrentBitsRead(register ilDecompPrivPtr pPriv) {
        int retval;
        int node;
...

{
        ...
        if (code > 255)
                node = code - 2; /* remove for extra codes */
        else
                node = code;
...
static void
ilAddStringToTable(ilDecompPrivPtr pPriv,
...

{
        ...
        if (code < 256)
                denodes[pPriv->nextNode].previous = code;
        else
                denodes[pPriv->nextNode].previous = code - 2;
        pPriv->nextNode++;
}
...
static ilError ilInitializeDecodeTable(register ilDecompPrivPtr pPriv) {
        unsigned int i;
...
static ilError ilDecompLZWExecute(ilExecuteData *pData, unsigned long dstLine,
...
/*  Macro which returns whether a clear code exists at *_pByte: a clear code
    is 9 bits, = 256, or 0x80 in first byte and a zero in the upper bit of the
   next. Each strip must begin with a clear code; at end of strip, a
   search-ahead is done for the next clear code.
*/
#define IS_CLEAR_CODE(_pByte)                                                  \
        ((*(_pByte) == 0x80) && ((*(_pByte + 1) & 0x80) == 0))

        /* My defines */
        unsigned long dstNBytes; /* Number of destination image bytes per row */
        long nLines;             /* Number of lines per source image strip             */
        long nBytesToGo; /* Number of source image bytes left to unpack for
                            current strip */
        long
            nDstLineBytesToGo; /* Number of bytes left to write to this line of
                                  destination     */
        register ilPtr pSrcByte; /* Pointer to source image data */
        register ilPtr
            pDstByte;    /* Pointer to destination image data first byte    */
        ilPtr pSrcStart; /* Pointer to initial src (comp) byte */
        ilPtr
            pDstLine; /* Pointer to destination image data first byte of each
                         scanline */
        ...
        pPriv->length = 0;
        pPriv->nextNode = 256; /* set to keep ref in ReadCode() call to
                                  CurrentBits2() from bombing */
        bits = 0;
        bitCount = 0;
        ...
        while (nLines-- > 0) {
                ...
                while (nDstLineBytesToGo > 0) {
                        ...
                        if (stringCount == 0) { /* get another string */
                                ...
                                /*  BEGIN ReadCode */
                                if (bitCount < 16) { /* Get next byte(s) of
                                ...
                                if (code == CLEAR_CODE) {
                                        if ((status = ilInitializeDecodeTable(
                                                 pPriv)) != IL_OK)
                                                goto xit;

                                        /*  BEGIN ReadCode */
                                        if (bitCount <
                                            16) { /* Get next byte(s) of encoded
                                                     source image data */
                                                if (--nBytesToGo < 0)
                                                        return IL_ERROR_COMPRESSED_DATA;
                                                if (--nBytesToGo < 0) {
                                        ...
                                        if (code == END_OF_INFORMATION)
                                                goto xit;
                                        ilStringFromCode(pPriv, code, pString,
                                                         &stringCount);
                                        oldCode = code;
                                } else {
                                        ...
                                        if ((code < 256) ||
                                            ((code - 2) <
                                             pPriv->nextNode)) { /* code is in
                                                                    table */
                                                ilStringFromCode(pPriv, code,
                                                                 pString,
                                                                 &stringCount);
                                                ilAddStringToTable(
                                                    pPriv, oldCode,
                                                    pString[stringCount - 1]);
                                                oldCode = code;
                                        }

                                        else {
                                                ilStringFromCode(pPriv, oldCode,
                                                                 pString,
                                                                 &stringCount);
...
IL_PRIVATE ilBool _ilDecompLZW(ilPipe pipe, ilPipeInfo *pinfo,
...

{
        ...
        pPriv = (ilDecompPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilDecompPrivRec), 0,
            (ilSrcElementData *)NULL, &dstdata, ilDecompLZWInit,
            ilDecompLZWCleanup, IL_NPF, ilDecompLZWExecute, NULL, 0);
        if (!pPriv)
                return FALSE;

        /*  Store info in *pPriv.  For nDstLineBytes, need minimum # of bytes
