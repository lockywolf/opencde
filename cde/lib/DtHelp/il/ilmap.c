 ...
 * reference.
 */
static ilError ilMapInit(ilPtr pPrivate, ilImageInfo *pSrcImage,
...
*/
static ilError ilMapByteExecute(ilExecuteData *pData, long dstLine,
                                long *pNLines) {
        ilMapPrivPtr pPriv;
 ...
 */
static ilError ilMap3ByteExecute(ilExecuteData *pData, long dstLine,
                                 long *pNLines) {
        ilMapPrivPtr pPriv;
        long srcRowBytes, dstRowBytes, nRowsM1, nLinesM1;
        ilPtr pSrcLine, pDstLine;
        ...
        /*  For each pixel, lookup the samples independently.  The src, dst and
           map images are all 3 components in sample order - must multiply * 3
           to index.
        */
        do {
                pSrc = pSrcLine;
...
*/
static ilError ilMapDestroy(ilPtr pPrivate) {
        ilMapPrivPtr pPriv = (ilMapPrivPtr)pPrivate;
...
*/
ilBool ilMap(ilPipe pipe, ilClientImage mapImage) {
        ...
        /*  Query mapImage; must be uncompressed, 256x1, # samples = same as
           pipe image, each 8 bits/sample, or error.
        */
        ...
        /*  Handle 1 and 3 byte (pixel format) cases; set "executeProc"
         * accordingly.
         */
        if (des.nSamplesPerPixel == 1) {
                ...
                if ((format.nBitsPerSample[0] != 8) ||
                    ...
                    (format.sampleOrder != IL_SAMPLE_PIXELS))
                        if (!ilConvert(pipe, (ilImageDes *)NULL,
                                       IL_FORMAT_3BYTE_PIXEL, 0, NULL))
                                return FALSE;
                executeProc = ilMap3ByteExecute;
        } else
                return ilDeclarePipeInvalid(pipe, IL_ERROR_NOT_IMPLEMENTED);

        /*  Add a filter to do the map, copy data to private.  Inc refCount for
           mapImage; see notes for ilMapDestroy().
        */
        pPriv = (ilMapPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilMapPrivRec), 0, (ilSrcElementData *)NULL,
            (ilDstElementData *)NULL, ilMapInit, IL_NPF, ilMapDestroy,
            executeProc, NULL, 0);
        if (!pPriv)
                return FALSE;
