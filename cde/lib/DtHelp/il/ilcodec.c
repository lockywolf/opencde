...
*/
IL_PRIVATE ilBool _ilReallocCompressedBuffer(ilImageInfo *pImage,
                                             unsigned int plane,
        ...
        pPlane->bufferSize =
            minNewSize + 10000; /* A GUESS - DO SOMETHING SMARTER !!!!! */

        if (!pPlane->pPixels)
                pPlane->pPixels = (ilPtr)IL_MALLOC(pPlane->bufferSize);
        else
                pPlane->pPixels =
                    (ilPtr)IL_REALLOC(pPlane->pPixels, pPlane->bufferSize);
        if (!pPlane->pPixels) {
                pPlane->bufferSize = 0;
                return FALSE;
        }
...
/*  Execute() function for ilInsertCompressedCopyFilter() to copy compressed
   images. Copies one strip of compressed data.
*/
static ilError ilCopyCompressedExecute(register ilExecuteData *pData,
        ...
        dstOffset =
            *pData->compressed.pDstOffset; /* byte offset into dst buffer */
        pDstPlane = &pData->pDstImage->plane[0];
        requiredBufferSize =
            nBytes + dstOffset; /* # bytes needed in dst buffer */
        ...
        if (requiredBufferSize > pDstPlane->bufferSize) {
                pDstPlane->pPixels = (pDstPlane->pPixels)
                                         ? (ilPtr)IL_REALLOC(pDstPlane->pPixels,
                                                             requiredBufferSize)
...
/*  Insert a "filter" which copies the compressed pipe image to the dest image.
   ...
   images.
*/
IL_PRIVATE ilBool _ilInsertCompressedCopyFilter(ilPipe pipe) {
        /*  Add a filter which copies src to dst.  No private needed; the
           Execute() function does all necessary setup.
        */
        return (ilAddPipeElement(pipe, IL_FILTER, 0, 0,
                                 (ilSrcElementData *)NULL,
                                 (ilDstElementData *)NULL, IL_NPF, IL_NPF,
                                 IL_NPF, ilCopyCompressedExecute, NULL, 0))
                   ? TRUE
                   : FALSE;
}
