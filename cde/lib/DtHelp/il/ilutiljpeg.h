...
*/
IL_EXTERN void _ilJPEGDataIn(const ilImageDes *pDes, long width, long height,
                             iljpgDataPtr pDst);

/*  Copy table info from IL format into IL JPG package format.
    Note: the restartInterval is also copied.
*/
IL_EXTERN void _ilJPEGTablesIn(ilJPEGData *pSrc, iljpgDataPtr pDst);

/*  Free the tables in the given IL JPEG data block, which is not freed.
 */
IL_EXTERN void _ilJPEGFreeTables(ilJPEGData *pData);
...
*/
IL_EXTERN ilBool _ilJPEGCopyData(ilJPEGData *pSrc, ilJPEGData *pDst);
