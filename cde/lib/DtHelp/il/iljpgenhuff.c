...
*/
ILJPG_PRIVATE
iljpgError _iljpgBuildEnhuffTable(register iljpgPtr pTableIn,
        ...
        pTable =
            (iljpgEnhuffTablePtr)ILJPG_MALLOC_ZERO(sizeof(iljpgEnhuffTableRec));
        if (!pTable)
        ...
        */
        for (i = 1, nValues = 0; i <= 16; i++) {
                value = *pTableIn++;
                if ((value + nValues) >
                    256) /* sum of first 16 bytes too large */
        ...
        /*  Reorder values created above into final tables; fig C.3 (C-4) in
         * spec */
 ...
 */
ILJPG_PRIVATE
iljpgError _iljpgEnhuffInit(iljpgEncodePrivPtr pPriv) {
        ...
        */
        for (i = 0; i < 4; i++) {
                if (pData->DCTables[i]) {
                        if (error = _iljpgBuildEnhuffTable(
                                pData->DCTables[i], &pHuffPriv->DCTables[i]))
                                return error;
                }
                if (pData->ACTables[i]) {
                        if (error = _iljpgBuildEnhuffTable(
                                pData->ACTables[i], &pHuffPriv->ACTables[i]))
                                return error;
                }
        }
        ...
        for (i = 0; i < pData->nComps; i++) {
                pHuffPriv->compDCTables[i] =
                    pHuffPriv->DCTables[pData->comp[i].DCTableIndex];
                pHuffPriv->compACTables[i] =
                    pHuffPriv->ACTables[pData->comp[i].ACTableIndex];
        }
 ...
 */
ILJPG_PRIVATE
iljpgError _iljpgEnhuffCleanup(iljpgEncodePrivPtr pPriv) {
...
*/
static iljpgError _iljpgPackHuffman(iljpgEnhuffPtr pHuffPriv,
                                    register int *pHuff, register int nHuff,
        ...
        /*  For each (size, value) pair, pack the lower "size" bits from "value"
           and write to the output stream.  Bits are packed with MSB first
           packing. "nHuffBits" (0..7!) bits are left from before in "huffBits",
           in the low-order bits. After any 0xff, write a 0, as per JPEG spec
           F.1.2.3 (pg F-7).
        */
        ...
        while (nHuff-- > 0) {
                size = *pHuff++;
                value = *pHuff++;
                while ((nHuffBits + size) >=
                       8) { /* while a byte or more worth */
                        nHuffBits =
                            8 -
                            nHuffBits; /* now # bits that fit in this byte */
                        huffBits <<=
                            nHuffBits; /* move existing bits up to make room */
                        size -=
                            nHuffBits; /* # bits from value -= # bits written */
                        huffBits |=
                            (value >> size) & /* or in upper unwritten bits */
                            sizeMasks[nHuffBits];
                        ...
                        if (((iljpgByte)huffBits) == ((iljpgByte)0xff))
                                if (!ILJPG_ENCODE_PUT_BYTE(stream, 0, error))
                                        return error; /* write a 0 after any
                                                         0xff */
                ...
                if (size > 0) { /* bits (if any) fit in less than byte */
                        ...
                        huffBits |=
                            value & sizeMasks[size]; /* add lower "size" bits
                                                        from value */
...
*/
ILJPG_PRIVATE
iljpgError _iljpgEnhuffExecute(iljpgEncodePrivPtr pPriv, int comp, int *pSrc,
                               ILJPG_ENCODE_STREAM stream) {
        iljpgEnhuffPtr pHuffPriv;
        register iljpgEnhuffTablePtr pTable;
...
#define ENHUFF(_value)                                                         \
        ...
        /*  Build a list of (size, value) pairs to Huffman encode in "huff", two
            per DC or AC coefficient, possibly less for AC due to zero
           run-lengths. Use pHuff to index thru it.  When done, go back thru
           list and encode it. This is done so that encoding can be done without
           a function call per encode, and without having to replicate long code
           all over the place.
        */
        pHuffPriv = (iljpgEnhuffPtr)pPriv->pHuffPriv;
        pHuff = huff;

        /*  Encode the first (DC) coefficient, which must already be the
           difference from the previous DC.  See section F.1.2.1 (pg F-3) of the
           JPEG spec. Write Huff code for size of DC, then Huff for DC value, -1
           if value < 0.
        */
        pTable =
            pHuffPriv->compDCTables[comp]; /* use DC table for this component */
        value = *pSrc++;
        if (value < 0) {
                size = -value;
                value -= 1;
        ...
        *pHuff++ = value;
        ...
        /*  Encode 63 ACs.  See section F.1.2.2 (pg F-4) of JPEG spec.
            ...
            from the previous non-zero AC - i.e. the # of zero ACs before this
           one. 0xf0 = 16 zero ACs; 0 = end of block (EOB), meaning rest of ACs
           are zero. SSSS is the "size" of the AC.  For each AC (excluding
           zeros), Huff encode the 8 bit representation, then write "size" bits
           as in DC encoding.
        */
        pTable = pHuffPriv->compACTables[comp]; /* use AC table for all other
                                                   components */
        nACLeft = 63;
        nZeros = 0;
        while (nACLeft-- > 0) {
                ...
                if (value == 0)
                        ...
                        while (nZeros >=
                               16) { /* AC != 0: flush out runs of 16 or more */
                                nZeros -= 16;
                                ENHUFF(0xf0)
                        }
                        ...
                        if (size < 256)
                                ...
                                size = iljpgBitsNeeded[size >> 8] + 8;
                        nZeros = (nZeros << 4) +
                                 size;   /* now have RRRRSSSS in "nZeros" */
 ...
 */
ILJPG_PRIVATE
iljpgError _iljpgEnhuffFlush(iljpgEncodePrivPtr pPriv,
                             ILJPG_ENCODE_STREAM stream) {
        register iljpgEnhuffPtr pHuffPriv;
        ...
        */
        ...
        if (nHuffBits > 0) {
                huffBits = pHuffPriv->huffBits;
                huffBits <<=
                    (8 - nHuffBits); /* move bits to upper bits of byte */
                huffBits |= (1 << (8 - nHuffBits)) -
                            1;            /* or in all 1's in unused bits */
