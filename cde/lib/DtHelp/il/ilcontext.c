...
*/
IL_EXTERN ilBool _ilObjectInitContext(ilContextPtr pContext);

/*  Called by ilDestroyContext() to destroy object data in the given context.
 */
IL_EXTERN void _ilObjectDestroyContext(ilContextPtr pContext);
...
ilError ilInternalCreateContext(int versionCheck,
                                ilContext *pContextReturn, /* RETURNED */
                                unsigned long mustBeZero) {
        register ilContextPtr pContext;

        /*  If internal version # > the version # the library was built with,
           then error; if <, old IL program using new library: supported.
        */
...
*/
IL_PRIVATE void *_ilMallocAndInitWithGarbage(unsigned long nBytes) {
...
unsigned int ilGetPrivateType(ilContext context) {
        register ilContextPtr pContext;
...
ilBool ilDestroyContext(ilContext context) {
        register ilContextPtr pContext;
        int i;
...
/*  Intersect the rectangle "*pSrcRect" with the rect "*pDstRect",
    storing the result in "*pDstRect".
        Not in this file for any particular reason; no other logical place for
   it.
*/
IL_PRIVATE void _ilIntersectRect(register ilRect *pSrcRect,
