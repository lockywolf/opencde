...
#define X_COLOR_MAX                                                            \
        65535 /* max value for X colormap RGB                                  \
                 value */
...
*/
#define MAX_IMAGE_BUFFER_SIZE                                                  \
        100000                       /* force strips if larger than            \
                                        this ... */
...
/*  Codes for view of X , in "ilXWCRec.i.visualType".  When the visual is
    ...
    for black/white.
*/
#define IL_X_MAX_VISUAL_TYPE                                                   \
        3 /* max value of IL_XWC_BITONAL                                       \
             et al*/
...
*/
static void ilFreeColorData(register ilXWCPtr pXWC, unsigned long freeMask) {
        int i;
        XColor *pColor;
        ...
        */
        if (freeMask & IL_FREE_XCOLORS) {
                if (pXWC->pColors) {
                        for (i = 0, pColor = pXWC->pColors; i < pXWC->nColors;
                             i++) {
                                if (pColor->pad)
                                        XFreeColors(pXWC->i.display,
                                                    pXWC->i.colormap,
                                                    &pColor->pixel, 1, 0L);
                                pColor++;
                        }
        ...
        if (freeMask & IL_FREE_XGRAYS) {
                if (pXWC->pGrays) {
                        for (i = 0, pColor = pXWC->pGrays; i < pXWC->nGrays;
                             i++) {
                                if (pColor->pad)
                                        XFreeColors(pXWC->i.display,
                                                    pXWC->i.colormap,
...
*/
static void ilDestroyXWC(ilPtr pPrivate) {
...
/*  Returns a value from 0..scaleValue, given "value", from
    ...
    (gamma = 1.0).
*/
static int ilGammaCorrect(long scaleValue, int value, int maxValue) {
...
*/
static ilBool ilAllocateXDitherColors(ilContext context,
                                      register ilXWCPtr pXWC) {
        ...
        for (red = 0; red < NRED; red++)
                for (green = 0; green < NGREEN; green++)
                        for (blue = 0; blue < NBLUE; blue++) {
                                pColor->red =
                                    ilGammaCorrect(X_COLOR_MAX, red, NRED);
                                pColor->green =
                                    ilGammaCorrect(X_COLOR_MAX, green, NGREEN);
                                pColor->blue =
                                    ilGammaCorrect(X_COLOR_MAX, blue, NBLUE);
                                if (!XAllocColor(pXWC->i.display,
                                                 pXWC->i.colormap, pColor)) {
                                        ilFreeColorData(pXWC, IL_FREE_XCOLORS);
                                        return FALSE;
                                }
                                pTranslate[red * (NGREEN * NBLUE) +
                                           green * NBLUE + blue] =
                                    pColor->pixel;
...
*/
static ilBool ilAllocateXGrays(ilContext context, register ilXWCPtr pXWC,
                               int nGrays) {
        int i, index;
        XColor *pColor;
        ilPtr pTranslate;
...
/*  Allocate "nLevels" levels of each of RGB for a DirectColor or a
    ...
    levels not alloc'd.
*/
static ilBool ilAllocateXDirectColors(register ilXWCPtr pXWC, int nLevels) {
...
#define MIN_DIRECT_LEVELS 32 /* min # of acceptable levels */
        ...
        */
        ...
        pXWC->pColors =
            (XColor *)IL_MALLOC_ZERO(pXWC->nColors * sizeof(XColor));
        if (!pXWC->pColors) {
                ilDestroyObject(pXWC->colorMapImage);
                pXWC->colorMapImage = (ilClientImage)NULL;
                context->error = IL_ERROR_MALLOC;
        ...
        for (i = 0, pColor = pXWC->pColors; i < nLevels; i++, pColor++) {
                ...
                if (!XAllocColor(pXWC->i.display, pXWC->i.colormap, pColor)) {
                        ...
                        if (nLevels <= MIN_DIRECT_LEVELS)
                                ...
                                return ilAllocateXDirectColors(pXWC,
                                                               nLevels / 2);
                }
                pColor->pad = TRUE; /* mark as alloc'd */
                pixel = pColor->pixel;
                ...
                if ((red != i) || (green != i) || (blue != i))
                        pXWC->mapDirect = TRUE;
                for (j = 0; j < nReplicate;
                     j++) { /* replicate into map image */
                        *pTranslate++ = red;
                        *pTranslate++ = green;
                        *pTranslate++ = blue;
                }
 ...
 */
ilBool ilChangeXWC(ilXWC XWC, unsigned int code, void *pData) {
        register ilXWCPtr pXWC;
        ...
        switch (code) {
                ...
                /*  Allocate grays: allocate grays if gray or pseudo-color, else
                    return true if gray display supported (e.g. to
                   True/DirectColor).
                */
...
/*  Public function; see spec.
 */
ilBool ilQueryXWC(ilXWC XWC, ilXWCInfo *pInfo) {
        register ilXWCPtr pXWC;
...
ilXWC ilCreateXWC(ilContext context, Display *display, Visual *visual,
                  Colormap colormap, GC gc, unsigned long mustBeZero1,
...
*/
static ilError ilXDestroy(ilPtr pPrivate) {
...
*/
static ilError ilWriteXInit(ilPtr pPrivate, ilImageInfo *pSrcImage,
        ...
        /*  Create an XImage that will be pointed to each buffer (strip) to
            write. */
        pXWC = pPriv->pXWC;
        pXImage = XCreateImage(
            pXWC->i.display, pXWC->i.visual, pPriv->imageDepth,
        ...
        if (!pXImage)
                ...
                /*  Set bit/byte order to MSBFirst.
                    NOTE: byte order should be set to byte order of client
                   machine!!!
                */
...
/*  Cleanup() function for ilWriteXDrawable() for all src image types.
    ...
    Init() below.
*/
static ilError ilWriteXCleanup(ilPtr pPrivate, ilBool aborting) {
...
*/
static ilError ilWriteXExecute(register ilExecuteData *pData, long dstLine,
                               long *pNLines, float ratio) {
        ...
        pXImage->data =
            (char *)(pData->pSrcImage->plane[0].pPixels +
         ...
         */
        scaledHeight = ((nSrcLines + pPriv->linesDone) * ratio + 0.5) - scaledY;

        _XmPutScaledImage(pXWC->i.display, pPriv->drawable, pXWC->i.gc, pXImage,
                          0, 0, scaledX, scaledY, pXImage->width, nSrcLines,
...
*/
static ilError ilWriteXOptLongExecute(register ilExecuteData *pData,
                                      long dstLine, long *pNLines,
                                      float ratio) {
        ilXPrivPtr pPriv;
        ...
        pSrcLine =
            (ilPtr)(pSrcImage->plane[0].pPixels + pData->srcLine * srcRowBytes);
        pXImage = pPriv->pXImage;
        pXImage->height = nSrcLines;
...
*/
ilBool ilConvertForXWrite(ilPipe pipe, ilXWC XWC, ilBool mapPixels,
                          unsigned long mustBeZero, void *mustBeNull) {
        ilPipeInfo info;
        ilImageDes des;
        ...
        */
        ...
        /*  Get pipe info and decompress; if pipe not in IL_PIPE_FORMING state,
            error. */
        if (ilGetPipeInfo(pipe, TRUE, &info, &des, &format) !=
            IL_PIPE_FORMING) {
        ...
        */
        if (pXWC->i.rawMode) {
                if ((des.nSamplesPerPixel == 1) &&
                    (format.nBitsPerSample[0] == 1))
                        ...
                        writeType = rawWriteMethodTable[pXWC->i.visualType];
        } else {
                ...
                writeType =
                    writeTypeTable[des.type * (IL_X_MAX_VISUAL_TYPE + 1) +
                                   pXWC->i.visualType];
                if (writeType == IL_XWC_WRITE_COLOR_DITHER) {
                        if (!pXWC->colorMapImage)
                                ilChangeXWC((ilXWC)pXWC,
                                            IL_XWC_ALLOC_DITHER_COLORS,
                                            (ilPtr)NULL);
                        if (!pXWC->colorMapImage)
                ...
                if (writeType == IL_XWC_WRITE_GRAY) {
                        if (!pXWC->grayMapImage)
                                ilChangeXWC((ilXWC)pXWC, IL_XWC_ALLOC_GRAYS,
                                            (ilPtr)NULL);
                        if (!pXWC->grayMapImage)
                                writeType = IL_XWC_WRITE_BITONAL;
                }
        ...
        switch (writeType) {
                ...
                /*  Bitonal: do an XPutImage() with XYBitmap, which renders
                   image 0's with GC background color, 1's with foreground, as
                   defined by IL spec. If rawMode, must be bit/pixel.
                */
                        ...
                        if ((des.nSamplesPerPixel != 1) ||
                            (format.nBitsPerSample[0] != 1))
                                return ilDeclarePipeInvalid(pipe,
                                                            IL_ERROR_RAW_MODE);
                } else if (!ilConvert(pipe, IL_DES_BITONAL, IL_FORMAT_BIT, 0,
                                      NULL))
                        return FALSE;
                break;

                /*  Gray: do ilMap() which maps 256 gray shades to the
                   equivalent X pixel values, possibly using fewer shades of
                   gray (slam; not dithered to lesser # grays).
                */
        case IL_XWC_WRITE_GRAY:
                if (pXWC->i.rawMode) {
                        ...
                        if (!ilConvert(pipe, IL_DES_GRAY, IL_FORMAT_BYTE, 0,
                                       NULL))
                                return FALSE;
                        if (mapPixels)
                ...
                /*  Dithered RGB: if raw mode, accept if byte/pixel;
                    if a pre-dithered palette image in pipe, ilMap() to map
                   colors to X pixels; otherwise dither cvt to palette w/ map to
        ...
        case IL_XWC_WRITE_COLOR_DITHER:
                if (pXWC->i.rawMode) {
                        ...
                        if ((format.nBitsPerSample[0] != 8) &&
                            !ilConvert(pipe, (ilImageDes *)NULL, IL_FORMAT_BYTE,
                                       0, NULL))
                                return FALSE;
                        if (mapPixels)
                        ...
                        cvt.kernelSize = 8;
                        if (mapPixels) {
                                cvt.dstType = IL_GRAY; /* any type other than
                                                          IL_PALETTE */
...
/*  Public function; see spec.
 */
ilBool ilWriteXDrawable(ilPipe pipe, Drawable drawable, ilXWC XWC,
                        ilRect *pSrcRect, int dstX, int dstY,
        ...
        */
        ilError (*executeFunction)(register ilExecuteData *, long, long *,
                                   float);
        ...
        */
        if (pXWC->i.writeType == IL_XWC_WRITE_COLOR) {
                ...
                if (bufferSize > MAX_IMAGE_BUFFER_SIZE) {
                        ...
                        srcData.stripHeight =
                            BEST_IMAGE_BUFFER_SIZE / (4 * info.width);
        ...
        */
        pPriv = (ilXPrivPtr)ilAddPipeElement(
            pipe, IL_CONSUMER, sizeof(ilXPrivRec), 0, pSrcData,
            (ilDstElementData *)NULL, ilWriteXInit, ilWriteXCleanup, ilXDestroy,
            NULL, executeFunction, 0);
        if (!pPriv)
                return FALSE; /* EXIT */
