...
static ilError ilInitBiDiffusion(ilBiDiffusionPrivPtr pPriv,
        ...
        /*  Create error accumulator buffers, 2 larger than width because the
           pixels above to the left and right are examined.
        */
        pPriv->pError1 =
            (short *)IL_MALLOC_ZERO(sizeof(short) * (pPriv->width + 2));
        pPriv->pError2 =
            (short *)IL_MALLOC_ZERO(sizeof(short) * (pPriv->width + 2));
...
static ilError ilCleanupBiDiffusion(ilBiDiffusionPrivPtr pPriv) {
        if (pPriv->pError1)
...
static ilError ilExecuteBiDiffusion(ilExecuteData *pData, long dstLine,
...
#define SHIFT_MASK(m) ((m) >>= 1)
        ...
        /*  Set "invert" to 0 if blackIsZero, else to 0xff, and xor each src
           pixel with invert.  Thus if 0 is white, the xor turns 0 into 255, 1
           into 254, etc.
        */
        invert = (pPriv->blackIsZero) ? 0 : 0xff;

        /*  Dither: do Floyd-Steinberg dither (error diffusion) requiring src to
           be 256 gray levels.  True error diffusion would map:
                (srcLevels-1)/(dstLevels-1) = 255/1
            but we will use 256/1 to make muls/divs easy.  Use the following:
                    1/16    5/16    3/16
                    7/16     x
            where the fractions indicate what portion of the error from the
           surrounding pixels should be added to this pixels value (x) to
           determine dst pixel to write at "x".
        */
        do {
                pSrc = pSrcLine;
                ...
                /*  Point to error accumulator buffers and switch, so *pError
                   becomes *pPrevError for next line.  Bump ptrs by 1: prev scan
                   line error is examined above to left and right: need zero
                   extra value there to handle beginning/end of line.
                */
                ...
                do {
                        ...
                        errorAcc +=
                            *(pPrevError - 1); /* 1 * error above left */
                        errorAcc += 5 * *pPrevError++; /* 5 * error above */
                        errorAcc += 3 * *pPrevError; /* 3 * error above right */
                        errorAcc >>= 4;              /* /16 */
                        errorAcc += *pSrc++ ^
                                    invert; /* + "x"; w/ blackIsZero handled */
                        if (errorAcc >=
                            128) /* output white: dont set out bit */
                                errorAcc -=
                                    255; /* sub value of white from errorAcc */
                ...
                if (mask !=
                    SPECIAL_MASK_BIT) { /* bits left in outLong; write them */
                        flip_bits((ilPtr)&outLong, sizeof(outLong));
                        *pDst++ = outLong;
                }
...
*/
static ilError
ilAddElementThreshold(ilThresholdPrivPtr pPriv,
...
*/
static ilError ilInitThreshold(ilThresholdPrivPtr pPriv, ilImageInfo *pSrcImage,
                               ilImageInfo *pDstImage) {
        pPriv->width = pSrcImage->width;
        pPriv->srcRowBytes = pSrcImage->plane[0].nBytesPerRow;
        pPriv->pSrcPixels = pSrcImage->plane[0].pPixels;
...
/*  Execute() function: dither and pack the given # of src lines.
 */
static ilError ilExecuteThreshold(ilExecuteData *pData, long dstLine,
                                  long *pNLines /* ignored on input */
) {
#ifdef LSB_BIT_ORDER
#define SPECIAL_MASK_BIT 0x00000001 /* for LSB bit order */
...
#define SHIFT_MASK(m) ((m) >>= 1)
        ...
        */
        do {
                ...
                /*  If mask != left bit on, some bits in outLong; output them.
                 * Next line.
                 */
