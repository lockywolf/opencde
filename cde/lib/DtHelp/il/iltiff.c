...
static void ilFreeFileList(ilFilePtr pFile) {
        register ilFileImagePtr pFileImage, pAltFileImage, pFileImageTemp;

        /*  Run thru the list of file images; for each: if a mask image destroy
           it (only one); if child images, destroy them - they are linked by
           pChild.
        */
 ...
 */
ilPtr ilReadFileTags(ilFileImage fileImage, int nTagsInit,
                     unsigned short *pTagNumbers, ilFileTag **ppTags,
                     unsigned long mustBeZero) {
#define RFT_FATAL_ERROR(_error)                                                \
        {                                                                      \
                error = (_error);                                              \
                goto RFTFatalError;                                            \
        }
        ...
        */
        ...
        while (nFileTags-- > 0) {
                ...
                /*  NOTE: declare fatal error if tag type out of range, BUT
                   TIFF 6.0 says we should ignore these tags; ALSO, there are
                   new tag types for 6.0.
                */
        ...
        /*  For each of caller's tags: find *pTagNumber in the pFileTags list.
            If not found, set corresponding "ppTags" entry to null, else point
           it at the found entry in pFileTags list, add its data size (rounded
           up to multiple of 4 - long-align), and the size of one ilFileTag, to
           "mallocSize".
        */
        mallocSize = 0;
        ppTag = ppTags;
        ...
        while (nTags-- > 0) {
                ...
                if (tagFound) {
                        mallocSize +=
                            (sizeof(ilFileTag) +
                             (pFileTag->length *
                                  _ilTagTypeItemSizes[pFileTag->type] +
                              3)) &
                            ~3;
        ...
        /*  If no tags found; exit, else allocate "mallocSize" bytes, and for
           each found tag, point corresponding "ppTags" to spot in pTagAlloc
           that has the ilFileTag data, followed by the data itself, then long
           aligned. What is returned will look like: ppTags [0] == null if tag
           not present, else ->: tag data:                               byte
           offset number (same value as corresponding       0 entry in
           pTagsNumbers) type                                      2 nItems
           4 pItems (points to <items>, i.e. one       8 long past this spot, or
           null if "nItems" is zero (0). <items> - "nItems" occurrences of data
           12 depending on "type". ppTags [1]->      <next tag, at offset 12 +
           ppTag[0]->nItems
        ...
        */
        ...
        pTagAlloc =
            (ilPtr)malloc(mallocSize); /* caller uses free() to free this */
        if (!pTagAlloc)
                RFT_FATAL_ERROR(IL_ERROR_MALLOC)
        ...
        while (nTags-- > 0) {
                if (!*ppTag)
                        ...
                        pFileTag = (ilTIFFTagPtr)*ppTag; /* points to internal
                                                            tag data */
                        *ppTag++ = pTag; /* point to converted data, next tag */
                        pTag->number =
                            pFileTag
                                ->number; /* copy from pFileTags to pTagAlloc */
                        ...
                        if (nItems == 0) {
                                ...
                                pTag = (ilFileTag *)pTagData; /* point pTag to
                                                                 spot for next
                                                                 tag */
                        } else {
                                pTag->pItems =
                                    pTagData; /* data follows the tag */
                                pTag = (ilFileTag *)(pTagData + /* next spot =
                                                                   past data,
                                                                   long-aligned
                                                                 */
                                                     ((nItems *
                                ...
                                /*  Byte flip the tag data to the spot pointed
                                   to by "pTagData", i.e. right after the 12
                                   bytes for the tag.  In TIFF, the data is in
                                   the "offset" field tag if it fits in 4 bytes,
                                   else offset points to the data in the file -
                                   seek and read it in that case.
                                */
                                switch (tagType) {
                                case IL_TAG_BYTE:
                                case IL_TAG_ASCII:
                                        if (nItems <= 4)
                                                bcopy(pFileTag->data.chars,
                                                      (char *)pTagData, nItems);
                                        else {
                                                IL_FLIP_LONG(
                                                    bigEndian,
                                                    pFileTag->data.chars,
                                ...
                                case IL_TAG_RATIONAL:
                                        IL_FLIP_LONG(bigEndian,
                                                     pFileTag->data.chars,
                                                     offset)
                                        if (!IL_SEEK(pFile, offset) ||
                                            !IL_READ(pFile, nItems << 3,
                                                     pTagData))
                                                RFT_FATAL_ERROR(
                                                    IL_ERROR_FILE_IO)
                                        while (nItems-- > 0) {
        ...
        /*  Tags read succesfully: free pFileTags (local buffer); return
         * pTagAlloc.
         */
        IL_FREE(pFileTags);
        pFileImage->context->error = IL_OK;
        ...
        /*  Fatal error: free pFileTags, pTagAlloc if not null, return error
         * code, null.
         */
RFTFatalError:
        if (pFileTags)
...
*/
static void ilAddImageToList(ilFilePtr pFile,
                             register ilFileImagePtr pFileImage,
                             ilFileImagePtr pPrevFileImage) {
        pFileImage->p.pNext = pFileImage->p.pPrev = pFileImage->p.pChild =
            pFileImage->p.pMask = (ilFileImage)NULL;
        ...
        case childImage:
                if (pPrevFileImage->imageType == childImage)
                        ...
                        pPrevFileImage->p.pChild = (ilFileImage)pFileImage;
                break;
        ...
        case maskImage:
                ...
                /*  Main image: if page # present, add to list in page order,
                 * else add to end */
        case mainImage:
                if (pFileImage->p.nPages <
                    0) { /* no page #; add to end of list */
                        if ((pFileImage->p.pPrev =
                                 (ilFileImage)pFile->pFileImageTail))
                                pFileImage->p.pPrev->pNext =
                                    (ilFileImage)pFileImage;
                        else
                                pFile->pFileImageHead = pFileImage;
                } else {
                        /*  Have page #.  If list is empty or page # <= that of
                           first element in list, add pFileImage to front of
                           list; otherwise find place in list based on page #
                           and insert image in that place.
                        */
                        pPrevFileImage = pFile->pFileImageHead;
                        if (!pPrevFileImage ||
                            (pFileImage->p.page <= pPrevFileImage->p.page)) {
                                pFileImage->p.pNext = (ilFileImage)
                                    pPrevFileImage; /* add to front */
                                pFileImage->p.pPrev = (ilFileImage)NULL;
                                pFile->pFileImageHead = pFileImage;
                        } else { /* Add after pPrevFileImage w/ highest page #
                                    <= image's page # */
                                while (pPrevFileImage->p.pNext &&
                                       (pPrevFileImage->p.pNext->page <=
                                        pFileImage->p.page)) {
                                        pPrevFileImage =
                                            (ilFileImagePtr)
                                                pPrevFileImage->p.pNext;
                                }
                                pFileImage->p.pNext = pPrevFileImage->p.pNext;
                                pFileImage->p.pPrev =
                                    (ilFileImage)pPrevFileImage;
                                pPrevFileImage->p.pNext =
                                    (ilFileImage)pFileImage;
                        }
                }

                /*  Point next in list, or tail if this image is last, back to
                 * this image */
                if (pFileImage->p.pNext)
                        pFileImage->p.pNext->pPrev = (ilFileImage)pFileImage;
                else
                        pFile->pFileImageTail = pFileImage;
                break;
        }
}
 ...
 */
/*  Look at the tag values in the given file image "*pFileImage", and if the
   tags indicate a supported image, add it to the list of file images in
   "*pFile". If image is not supported (e.g unknown compression type) the image
   is freed. ppPrevFileImage must point to the ptr to the previous image read
   from the file, or null if this is the first image; used for child and mask
   images. Return IL_OK if image valid, even if not supported; an error means
    the image is malformed (e.g. missing required tag).
*/
static ilError ilReadTagsAndAddImageToList(ilFilePtr pFile,
                                           ilFileImagePtr *ppPrevFileImage,
                                           register ilFileImagePtr pFileImage) {
/*  Indexes into "tagNumbers" below. NOTE: order is important, as tags are
   checked in the below order, and some tags default to values which must be
   before them. Also NOTE: check for type other than short/long and # items done
   below!
*/
...
#define PHOTOMETRIC_INTERPRETATION                                             \
        15                       /* follow COMP, BITS_PS, SAMPLES_PP, NEW_SFT */
...
#define PRIVATE_0 22             /* follow PHOTO_INT */
        ...
        static unsigned short tagNumbers[] = {IL_TAG_NEW_SUBFILE_TYPE,
                                              IL_TAG_IMAGE_WIDTH,
                                              IL_TAG_IMAGE_LENGTH,
...
#define AIL_FATAL_ERROR(_error)                                                \
         ...
         */
        pTagAlloc = ilReadFileTags((ilFileImage)pFileImage, AI_NTAGS,
                                   tagNumbers, tag, 0);
        if (!pTagAlloc)
                return (pFileImage->context->error) ? pFileImage->context->error
                                                    : IL_ERROR_FILE_MISSING_TAG;

        /*  Loop thru the tags; set "present" true if tag found, set "value" to
           integer value of tag (long or short allowed - exceptions checked
           explicitly - else fatal error). Fix bug #0169: ignore tags with #
           items <= 0.
        */
        for (tagIndex = 0; tagIndex < AI_NTAGS; tagIndex++) {
                if ((pTag = tag[tagIndex]) && (pTag->nItems > 0)) {
                        if (pTag->type == IL_TAG_SHORT)
                                ...
                                /* compatibility problem with long and unsigned
                                 * long data fields */
                        ...
                        if (pTag->nItems != 1) {
                                if ((tagIndex != BITS_PER_SAMPLE) &&
                                    ...
                                    (tagIndex != PRIVATE_0))
                                        AIL_FATAL_ERROR(
                                            IL_ERROR_FILE_MALFORMED_TAG);
                        }
                        present = TRUE;
                } else
                        present = FALSE;

                /*  Parse the tags and fill in pFileImage->format, des.  Init
                   des first. See page 8 in the TIFF v5.0 spec for info on each
                   tag type.
                */
                switch (tagIndex) {
                ...
                case IMAGE_WIDTH:
                        if (!present)
                                AIL_FATAL_ERROR(IL_ERROR_FILE_MISSING_TAG);
                ...
                case IMAGE_LENGTH:
                        ...
                        /*  Resolution returned as dpi * 2, or "0" if not
                           present.  The TIFF spec says should default to 300
                           dpi if not present, but we let the caller make that
                           assumption.
                        */
                ...
                case Y_RESOLUTION: {
                        ...
                        if (tagIndex == X_RESOLUTION)
                                ...
                                pFileImage->p.yRes = value;
                } break;

                case ROWS_PER_STRIP:
                        if (!present || (((long)value) < 0) ||
                            (value > pFileImage->p.height))
                                pFileImage->p.stripHeight =
                                    pFileImage->p.height;
                        else
                                pFileImage->p.stripHeight = value;
                        break;
                ...
                case SAMPLES_PER_PIXEL:
                        ...
                        /*  Planar configuration: default to pixels unless more
                           than one sample, and value is present and is 2.
                        */
                case PLANAR_CONFIGURATION:
                        pFileImage->p.format.sampleOrder =
                        ...
                        /*  bits per sample: default to 1, or to first item (in
                           "value") if fewer items than samples per pixel.
                        */
                case BITS_PER_SAMPLE:
                        for (i = 0; i < pFileImage->p.des.nSamplesPerPixel; i++)
                                pFileImage->p.format.nBitsPerSample[i] =
                                    (!present || (pTag->nItems < 1))
                                        ? 1
                                        : ((i > pTag->nItems - 1)
                                               ? value
                                               : *((unsigned short *)
                        ...
                        /*  FillOrder: used for class F only (TIFF compression
                         * "3")
                         */
                case FILL_ORDER:
                        fillOrder = (present && (value == 2)) ? IL_LSB_FIRST
                                                              : IL_MSB_FIRST;
                        break;

                        /*  Parse group 3 TIFF options, assumed to be 0 if not
                           present. As per TIFF spec, dont support if any
                           unrecognized bits are not 0. Note that
                           "group3CompData" is set to 0 below if comp = 2 (TIFF
                           G3).
                        */
                case GROUP_3_OPTIONS:
                        group3CompData = IL_G3M_EOL_MARKERS; /* EOL present if
                                                                not "TIFF" G3 */
                        if (fillOrder == IL_LSB_FIRST)
                                group3CompData |= IL_G3M_LSB_FIRST;
                        if (present) {
                                if (value & ~0x7)
                                        goto UnsupportedImage; /* non-zero
                                                                  unhandled
                                                                  bits;
                                                                  unsupported */
                                if (value & 1)
                        ...
                        /*  Group 4: set LSB first if FillOrder tag specifies
                           it.  If Group4Options tag defined and any bit other
                           than zero set (bit 0 is undefined) than consider
                           image unsupported.  Bit one (1) specifies
                ...
                case COMPRESSION:
                        ...
                        switch (value) {
                        case 1:
                                pFileImage->p.des.compression = IL_UNCOMPRESSED;
                                break;

                                /*  Comp codes 2 and 3 are both Group 3 - the
                                   difference is only in what *pCompData
                                   describes.  For code 2 ("TIFF" group 3), the
                                   comp data is 0 - the bits were defined that
                                   way.  For code 3, use what was
                                   deciphered/defaulted by the group 3 options
                                   tag.
                                */
                        case 2:
                                group3CompData = 0; /* "TIFF" G3 - default
                                                       options, set to 0 */
                        case 3:                     /* "true" G3 */
                                pFileImage->p.des.compression = IL_G3;
                                pFileImage->p.des.compInfo.g3.flags =
                                    group3CompData;
                                break;

                        case 4:
                                pFileImage->p.des.compression = IL_G4;
                                pFileImage->p.des.compInfo.g4.flags =
                                    group4CompData;
                                break;
                        case 5:
                                pFileImage->p.des.compression = IL_LZW;
                                break;
                        case 6:
                                pFileImage->p.des.compression = IL_JPEG;
                                pFileImage->p.des.compInfo.JPEG.reserved =
                                    IL_JPEGM_RAW;
                                pFileImage->p.des.compInfo.JPEG.process = 0;
                                break;
                        case 32773:
                                pFileImage->p.des.compression = IL_PACKBITS;
                                break;
                        default:
                                goto UnsupportedImage;
                        }
                ...
                case PREDICTOR:
                        ...
                        /*  If no photometric interpration, default based on
                           (assumed) image type: 1 sample: 1 bit/pixel: bitonal,
                           white=0 if fax comp, else 1; > 1 bit/pixel: gray,
                           black=0. 3 samples: RGB; else error.
                        */
                case PHOTOMETRIC_INTERPRETATION:
                        pYCbCr = (ilYCbCrInfo *)NULL; /* unless a YCbCr image */
                        if (!present) {
                                if (pFileImage->p.des.nSamplesPerPixel == 1) {
                                        if (pFileImage->p.format
                                                .nBitsPerSample[0] == 1) {
                                                pFileImage->p.des.type =
                                                    IL_BITONAL;
                                                switch (pFileImage->p.des
                                                            .compression) {
                                                case IL_G3:
                                                case IL_G4:
                                                        pFileImage->p.des
                                                ...
                                                pFileImage->p.des.type =
                                                    IL_GRAY;
                                                pFileImage->p.des.blackIsZero =
                                                    TRUE;
                                        }
                                } else if (pFileImage->p.des.nSamplesPerPixel ==
                                           3)
                                ...
                                switch (value) { /* photo interpretation
                                                    present; handle it */

                                        /*  Bitonal or gray; 0 = white is 0,
                                         * else black is zero. */
                                case 0:
                                case 1:
                                        if (pFileImage->p.des
                ...
                case PAGE_NUMBER:
                        if (present) {
                                if ((pTag->nItems != 2) ||
                                    (pTag->type != IL_TAG_SHORT))
                                        AIL_FATAL_ERROR(
                                            IL_ERROR_FILE_MALFORMED_TAG);
                                pFileImage->p.page = value;
                                pFileImage->p.nPages =
                                    *((unsigned short *)pTag->pItems + 1);
                        } else
                                pFileImage->p.page = pFileImage->p.nPages = -1;
                        break;
                        ...
                        if (pFileImage->p.des.compression == IL_JPEG) {
                                if (!present)
                                        AIL_FATAL_ERROR(
                                            IL_ERROR_FILE_MISSING_TAG);
                                if (value !=
                                    1) /* only baseline sequential supported! */
                                        goto UnsupportedImage;
                        }
                        break;
                ...
                case YCBCR_COEFFICIENTS:
                        if (pYCbCr) {
                                if (!present) {
                                        ...
                                        /* compatibility problem with long or
                                         * unsigned long data fields */
                                        register INT32 *pLong =
                                            (INT32 *)pTag->pItems;
                                        double num, denom;
                                        if ((pTag->nItems != 3) ||
                                            (pTag->type != IL_TAG_RATIONAL))
                                                AIL_FATAL_ERROR(
                                                    IL_ERROR_FILE_MALFORMED_TAG);
                                        num = *pLong++;
                                        denom = *pLong++;
                ...
                case YCBCR_SUBSAMPLING:
                        if (pYCbCr) {
                                if (!present)
                                        pYCbCr->sample[1].subsampleHoriz =
                                            pYCbCr->sample[1].subsampleVert = 2;
                                else {
                                        if ((pTag->nItems != 2) ||
                                            (pTag->type != IL_TAG_SHORT))
                                                AIL_FATAL_ERROR(
                                        ...
                                        pYCbCr->sample[1].subsampleHoriz =
                                            *((short *)pTag->pItems);
                                        pYCbCr->sample[1].subsampleVert =
                                            *((short *)pTag->pItems + 1);
                                }
                                /* Set Y subsampling to (1,1); copy Cr to Cb:
                                 * defined by TIFF */
                                pYCbCr->sample[0].subsampleHoriz =
                                    pYCbCr->sample[0].subsampleVert = 1;
                                pYCbCr->sample[2].subsampleHoriz =
                                    pYCbCr->sample[1].subsampleHoriz;
                                pYCbCr->sample[2].subsampleVert =
                                    pYCbCr->sample[1].subsampleVert;
                        }
                        break;
                ...
                case YCBCR_POSITIONING:
                        ...
                        /*  ReferenceBlackWhite: if present, allow tag type to
                           be "long" or "rational". "rational" is the correct
                           tag type; "long" was written by Image 1 (the first
                           Developer's Kit that supported JPEG), so support the
                           old files.
                        */
                case REFERENCE_BLACK_WHITE:
                        if (pYCbCr) {
                                ...
                                if (!present) {
                                        ...
                                        /* compatibility problem with long and
                                         * unsigned long data fields */
                                        register INT32 *pLong =
                                            (INT32 *)pTag->pItems;
                                        if (pTag->nItems != 6)
                                        ...
                                        if (pTag->type ==
                                            ...
                                            IL_TAG_RATIONAL) { /* correct
                                                ...
                                                AIL_FATAL_ERROR(
                                                    IL_ERROR_FILE_MALFORMED_TAG); /* bad tag type */
                                }
                        }
                        break;

                        /*  HP IL private tag.  See /ilc/iltiffint.h for
                           definition. If "dithered palette" mask on, set levels
                           and bit in flags in des.
                        */
                case PRIVATE_0:
                        if (present) {
                                if (value & IL_TAG_P0_FLAG_DITHER_LEVELS) {
                                        if (pFileImage->p.des.type ==
                                            IL_PALETTE) {
                                                register short *pLevel =
                                                    ((short *)pTag->pItems + 1);
                                                if ((pTag->nItems < 4) ||
                                                    (pTag->type !=
                                                     IL_TAG_SHORT))
                                                        AIL_FATAL_ERROR(
                                                            IL_ERROR_FILE_MALFORMED_TAG);
                                                pFileImage->p.des.flags |=
                                                    IL_DITHERED_PALETTE;
                                                pFileImage->p.des.typeInfo
                                                    .palette.levels[0] =
                                                    *pLevel++;
                                                pFileImage->p.des.typeInfo
                                                    .palette.levels[1] =
        ...
        /*  All the tags are parsed and valid so far.  Finish filling in
           *pFileImage; # levels is inferred from # bits; only supporting
           byte/sample for now.
        */
        for (i = 0; i < pFileImage->p.des.nSamplesPerPixel; i++) {
                nBits = pFileImage->p.format.nBitsPerSample[i];
                if (nBits <= 0)
                        AIL_FATAL_ERROR(IL_ERROR_FILE_TAG_VALUE); /* error;
                                                                     invalid #
                                                                     bits */
                if (nBits > 8)
        ...
        /*  If a child image and no previous image, make a main image and add
         * it. */
        ...
        /*  Error: free pTagAlloc, but not pFileImage - caller does that. Return
           error. Use free(), not IL_FREE(), to free pTagAlloc, just like caller
           should.
        */
AILFatalError:
        if (pTagAlloc)
        ...
        /*  Image is not supported; not an error, but don't add to list - free
         * image.
         */
UnsupportedImage:
        IL_FREE(pFileImage);
...
/*  Public function: see spec.
 */
ilFileImage ilListFileImages(ilFile file, unsigned long mustBeZero) {
        ilError error;
        ...
        /*  Dont have image list - read file and build the list.  When done,
            pFile->IFDTailPtrOffset points to the "next IFD" ptr which is null,
           i.e. the next IFD ptr for the last image in the file.  For a file
           with no images that would be the ptr in the file header. First read
           header to get ptr to first image in file.
        */
#ifdef DTLIB
        /* Used when this module is compiled by the DtHelp library build */
        ...
        /*  Overview: read the tag count, allocate room for tags
           (pBeginTagBuffer) and read the unflipped tags into it.  Allocate
           space for converted tags (pFile->pTags) and copy/flip unflipped tags
           into it. After the tags comes the offset to next IFD; read and flip
           it.
        */
        while (IFDOffset) {
                imageSequenceNumber++;
                pFileImage =
                    (ilFileImagePtr)IL_MALLOC_ZERO(sizeof(ilFileImageRec));
                if (!pFileImage) {
                ...
                /*  Read and flip the tagCount, @IFDOffset.  Point tagOffset at
                 * next location.
                 */
                ...
                /*  Add file image to list, if supported.  An error is returned
                   only if the file image is malformed; not if it is just
                   unsupported.
                */
                if ((error = ilReadTagsAndAddImageToList(pFile, &pPrevFileImage,
                                                         pFileImage)))
                        goto LFIFatalError; /* fatal error returned; EXIT */

                /*  Read and flip offset to next IFD, at end of tags; save in
                 * pFile. */
        ...
        /*  Fatal error: dispose file list and pFileImage if non-null, return
           "error", and sequence # of image in file that provoked the error.
        */
LFIFatalError:
        if (pFileImage)
                IL_FREE(pFileImage);
...
ilFile ilConnectFile(
    ...
    FILE *stream,
#endif /* DTLIB */
    long offset, unsigned long mustBeZero) {
        ilBool bigEndian;
        ilFilePtr pFile;
        ilTIFFHeader header;
        ...
        if (mustBeZero != 0) {
                ...
                /*  Read what should be a TIFF file header at "offset" within
                   "file". Verify that byte order is "MM" (bigEndian) or "II"
                   (!bigEndian) or error. Verify the answer to life, the
        ...
        if (!(bigEndian && (header.version[0] == 0) &&
              (header.version[1] == 42)) &&
            !(!bigEndian && (header.version[0] == 42) &&
              (header.version[1] == 0))) {
                context->error = IL_ERROR_FILE_NOT_TIFF;
                return (ilFile)NULL;
        }

        /*  File seems to be a valid TIFF file.  Create an ilFileRec object,
           init it and return ptr to it.
        */
        pFile = (ilFilePtr)_ilCreateObject(context, IL_FILE, ilFreeFileList,
                                           sizeof(ilFileRec));
        if (!pFile)
