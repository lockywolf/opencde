...
*/
static ilError ilExecuteBitAlign(ilExecuteData *pData, long dstLine,
                                 long *pNLines) {
...
*/
static ilError ilExecute8Bit3PlaneToPixel(ilExecuteData *pData, long dstLine,
                                          long *pNLines) {
        ilImagePlaneInfo *pImagePlane;
...
*/
static ilError ilExecute8Bit3PixelToPlane(ilExecuteData *pData, long dstLine,
                                          long *pNLines) {
        ilImagePlaneInfo *pImagePlane;
...
*/
static ilError ilExecute4To8Bit(ilExecuteData *pData, long dstLine,
                                long *pNLines) {
        ...
        /*  Set "nBytesM1Init" to whole # of src bytes to do (2 nibbles each),
            ...
            Copy each src byte (2 nibbles) to two dst bytes; do optional last
           nibble.
        */
...
/*  Pack pipe image which is byte/pixel into nibble/pixel.
    Handles any number of samples, but pixel order only if multi-sample.
*/
static ilError ilExecute8To4Bit(ilExecuteData *pData, long dstLine,
                                long *pNLines) {
        ilImageInfo *pSrcImage;
        ilImagePlaneInfo *pImagePlane;
        long srcRowBytes, dstRowBytes, nBytesM1Init, nLinesM1;
        ...
        /*  Set "nBytesM1Init" to whole # of dst bytes to do (2 nibbles each),
            where the # of nibbles is width * nSamplesPerPixel.
            Set "oddNumberOfNibbles" true if width is odd (do one last nibble).
            Pack each pair of src bytes into one dst nibble; do optional last
           nibble.
        */
        nBytesM1Init = pSrcImage->width * pSrcImage->pDes->nSamplesPerPixel;
        oddNumberOfNibbles = ((nBytesM1Init & 1) != 0);
...
/*  Execute() function: convert the given # of src lines, changing nibbles
    ...
    of the bytes, thus scaling from 0..15 to 0..255.
        Handles any number of samples, but pixel order only if multi-sample.
*/
static ilError ilExecute4To8BitScaled(ilExecuteData *pData, long dstLine,
                                      long *pNLines) {
        ilImageInfo *pSrcImage;
        ilImagePlaneInfo *pImagePlane;
        long srcRowBytes, dstRowBytes, nBytesM1Init, nLinesM1;
        ...
        */
        ...
        /*  Set "nBytesM1Init" to whole # of src bytes to do (2 nibbles each),
            where the # of nibbles is width * nSamplesPerPixel.
            Set "oddNumberOfNibbles" true if width is odd (do one last nibble).
            Make from each src byte (2 nibbles) two dst bytes; do optional last
           nibble. Replicate each src nibble to upper/lower nibble of dst byte,
           thus scaling to new # of levels (from 16 to 256): 0 => 0, 1 => 17, 15
           => 255.
        */
        nBytesM1Init = pSrcImage->width * pSrcImage->pDes->nSamplesPerPixel;
        oddNumberOfNibbles = ((nBytesM1Init & 1) != 0);
...
/*  Execute() function: convert the given # of src lines, changing bytes from
    ...
    order nibble, thus scaling from 0..15 to 0..255.
        Handles any number of samples, but pixel order only if multi-sample.
*/
static ilError ilExecuteByte16To256Level(ilExecuteData *pData, long dstLine,
                                         long *pNLines) {
        ilImageInfo *pSrcImage;
        ilImagePlaneInfo *pImagePlane;
        long srcRowBytes, dstRowBytes, nBytesM1Init, nLinesM1;
        ...
        */
        ...
        /*  Set "nBytesM1Init" to whole # of src bytes to do, = width * #
           samples/pixel (note that each sample must be a byte!) Replicate
           low-order nibble to upper nibble of dst byte, thus scaling to new #
           of levels (from 16 to 256): 0 => 0, 1 => 17, 15 => 255.
        */
...
*/
IL_PRIVATE ilBool _ilAddLevelConversionFilter(ilPipe pipe, ilPipeInfo *pInfo,
                                              ilImageDes *pPipeDes,
        ...
        /*  Determine if we support requested conversion.
            Currently only support: 4 or 8 bit, 16 level => 8 bit, 256 level,
           any # of samples/pixel, but pixel mode only if # samples > 1.
        */
        ...
        /*  All src # levels == 16, dst == 256.  If all src formats are 4 bit or
           8 bit, can handle, else error.  pFormat->nBitsPerSample now == 8 for
           all samples.
        */
        if (all4Bit)
        ...
        /*  Update *pPipeDes, *pFormat and add filter or error.
            Update pInfo after successful add; pPipeDes and pFormat already
           updated.
        */
        ...
        pPriv = ilAddPipeElement(pipe, IL_FILTER, 0, 0,
                                 (ilSrcElementData *)NULL, &dstData, IL_NPF,
                                 IL_NPF, IL_NPF, executeFunction, NULL, 0);
        if (!pPriv)
                return FALSE; /* EXIT */
