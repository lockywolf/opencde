...
*/
ilBool ilCompress(ilPipe pipe, unsigned int compression, ilPtr pCompData,
                  long dstStripHeight, unsigned long mustBeZero) {
        unsigned int state;
        ilPipeInfo info;
        ilImageDes des;
        ...
        /*  Check current pipe image and force decompression if wrong
           compression, or inconstant strip height, or not requested (non-zero)
           strip height.
        */
        if ((des.compression != compression) ||
            (dstStripHeight && (info.stripHeight != dstStripHeight)) ||
            !info.constantStrip)
                ilGetPipeInfo(pipe, TRUE, &info, &des,
                              &format); /* decompress */

        /*  Specify the input (and therefore the output) strip height: if
           dstStripHeight is zero, take default, otherwise requested, always
           constant strips.
        */
        if (dstStripHeight <= 0)
                dstStripHeight = ilRecommendedStripHeight(
                    &des, &format, info.width, info.height);
        srcData.consumerImage = (ilObject)NULL;
        srcData.minBufferHeight = 0;
        srcData.stripHeight = dstStripHeight;
        srcData.constantStrip = TRUE;

        /*  Pipe image is now correct compression or is uncompressed.  For
           compression types which have no variants, exit if already requested
           compression; for types with variant, call compression-specific code
           to check variant.
        */
        switch (compression) {
        ...
        case IL_G3: /* check G3 variants */
                return _ilCompressG3(pipe, &info, &des, &format, &srcData,
                                     pCompData);
                break;

        case IL_JPEG: /* check JPEG variants */
                return _ilCompressJPEG(pipe, &info, &des, &format, &srcData,
                                       pCompData);
                break;

        case IL_G4: /* check G4 variants */
                return _ilCompressG4(pipe, &info, &des, &format, &srcData,
                                     pCompData);
                break;

        case IL_LZW:
                if (des.compression != IL_LZW)
                        return _ilCompressLZW(pipe, &info, &des, &format,
                                              &srcData);
                break;

        case IL_PACKBITS:
                if (des.compression != IL_PACKBITS)
                        return _ilCompressPackbits(pipe, &info, &des, &format,
                                                   &srcData);
                break;
