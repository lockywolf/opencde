 ...
 */
static ilError ilDecompInit(ilJPEGPrivPtr pPriv, ilImageInfo *pSrcImage,
 ...
 */
static ilError ilDecompCleanup(ilJPEGPrivPtr pPriv, ilBool aborting) {
        ilError error;
 ...
 */
static ilError ilDecompRawExecute(ilExecuteData *pData, unsigned long dstLine,
        ...
        if (pPriv->firstStrip) {
                _ilJPEGTablesIn((ilJPEGData *)pSrcImage->pCompData,
                                &pPriv->jpgData);
                if ((error =
                         iljpgDecodeInit(&pPriv->jpgData, &pPriv->pJPEGPriv)))
                        return error;
        ...
        streamRec.pData =
            pSrcImage->plane[0].pPixels + pData->compressed.srcOffset;
        ...
        for (i = 0, pPlane = pData->pDstImage->plane;
             i < pData->pDstImage->pDes->nSamplesPerPixel; i++, pPlane++) {
                pPixels[i] = pPlane->pPixels +
                             (dstLine * pPriv->jpgData.comp[i].vertFactor /
 ...
 */
static ilError ilDecompJIFExecute(ilExecuteData *pData, unsigned long dstLine,
                                  unsigned long *pNLines) {
        register ilJPEGPrivPtr pPriv;
        ilError error;
        int i;
        ...
        /*  If firstStrip decode header, determine # of strips and init for
         * decode */
        ...
        if (firstStrip) {
                ...
                if ((error =
                         iljpgDecodeJIF(&pPriv->streamRec, &pPriv->pJPEGData)))
                        return error;
                if ((error =
                         iljpgDecodeInit(pPriv->pJPEGData, &pPriv->pJPEGPriv)))
                        return error;
        }
        ...
        if ((error =
                 iljpgDecodeExecute(pPriv->pJPEGPriv, &pPriv->streamRec, FALSE,
                                    *pNLines, pPixels, nBytesPerRow)))
                return error;

        /*  Handle based on whether firstStrip and/or last strip:
                first?  last?
                YES     YES     normal return; image is one strip, no call backs
           needed. YES     NO      return "call back" error code: want this
           filter to be called again until another return cancels it NO      YES
           cancel call back; don't want to be called again NO      NO
           normal return; more strips to decompress
        */
        pPriv->nLinesWritten += pPriv->stripHeight;
        if (firstStrip)
                if (pPriv->nLinesWritten >=
                    pPriv->jpgData.height) /* last strip */
                        return IL_OK;
                else
                        return IL_ERROR_ELEMENT_AGAIN;
        else if (pPriv->nLinesWritten >= pPriv->jpgData.height)
...
/*  -------------------- _ilDecompJPEG -------------------------- */
/*  Called by ilDecompress() when pipe image is JPEG compressed.  Add a filter
   to decompress the pipe image.
*/
ilBool _ilDecompJPEG(ilPipe pipe, ilPipeInfo *pInfo, ilImageDes *pDes) {
        ...
        */
        ...
        if (pDes->nSamplesPerPixel == 1)
                ...
                format.nBitsPerSample[0] = format.nBitsPerSample[1] =
        ...
        /*  If "raw" JPEG, decode in strips, the size of those presented to this
         * filter */
        if (pDes->compInfo.JPEG.reserved & IL_JPEGM_RAW) {
                dstData.stripHeight =
                    pInfo->stripHeight; /* decode strips as they come in */
                dstData.constantStrip = pInfo->constantStrip;
                pPriv = (ilJPEGPrivPtr)ilAddPipeElement(
                    pipe, IL_FILTER, sizeof(ilJPEGPrivRec), 0,
                    (ilSrcElementData *)NULL, &dstData, ilDecompInit,
                    ilDecompCleanup, IL_NPF, ilDecompRawExecute, NULL, 0);
        } else {
                /*  JIF: a single data stream (strip) with tables in the stream:
                   decompress in IL's default strip size for this image, made a
                   multiple of MCUs.
                */
                long stripHeight, mcuHeight;
                stripHeight =
                    ilRecommendedStripHeight(dstData.pDes, dstData.pFormat,
                                             dstData.width, dstData.height);
                mcuHeight = 8 * jpgData.maxVertFactor;
                if (mcuHeight == 0)
                        mcuHeight = 32; /* ? error, default to max possible */
                stripHeight =
                    ((stripHeight + mcuHeight - 1) / mcuHeight) * mcuHeight;
                ...
                pPriv = (ilJPEGPrivPtr)ilAddPipeElement(
                    pipe, IL_FILTER, sizeof(ilJPEGPrivRec), 0,
                    (ilSrcElementData *)NULL, &dstData, ilDecompInit,
                    ilDecompCleanup, IL_NPF, ilDecompJIFExecute, NULL, 0);
        }
