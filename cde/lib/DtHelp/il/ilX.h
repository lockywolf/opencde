...
***    file:           ilX.h
***
***    description:    Public declarations and functions for accessing X
*drawables with IL
***
***
...
extern ilBool ilReadXDrawable(
    ...
    ilPipe pipe, Display *display, Drawable drawable, Visual *visual,
    Colormap colormap, ilBool blackIsZero, ilRect *pSrcRect,
    ilBool copyToPixmap, unsigned long flags
#endif
);

extern ilBool ilConvertForXWrite(
#if NeedFunctionPrototypes
    ilPipe pipe, ilXWC XWC, ilBool mapPixels, unsigned long mustBeZero,
    void *mustBeNull
#endif
);
...
#if NeedFunctionPrototypes
    ilPipe pipe, Drawable drawable, ilXWC XWC, ilRect *pSrcRect, int dstX,
    int dstY, unsigned long mustBeZero
#endif
);

extern ilXWC ilCreateXWC(
#if NeedFunctionPrototypes
    ilContext context, Display *display, Visual *visual, Colormap colormap,
    GC gc, unsigned long mustBeZero1, unsigned long mustBeZero2
#endif
);

extern ilBool ilChangeXWC(
#if NeedFunctionPrototypes
    ilXWC XWC, unsigned int code, void *pData
#endif
);

extern ilBool ilQueryXWC(
#if NeedFunctionPrototypes
    ilXWC XWC, ilXWCInfo *pInfo
#endif
);
