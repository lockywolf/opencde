...
*/
static ilBool ilAddConversionFilter(ilPipe pipe, ilPipeInfo *pInfo,
                                    ilImageDes *pSrcDes,
        ...
        /*  Check src format against code for src format demanded by this
           filter. If not the same, do ilConvert() to that format (recurse!),
           exit if error.
        */
                ...
                if ((pSrcFormat->rowBitAlign != 32) ||
                    (pSrcFormat->nBitsPerSample[0] != 1)) {
                        *pSrcFormat = *IL_FORMAT_BIT;
                        ilConvert(pipe, (ilImageDes *)NULL, pSrcFormat, 0,
                                  NULL);
                        if (pipe->context->error)
                                return FALSE;
                        ilGetPipeInfo(pipe, FALSE, pInfo, (ilImageDes *)NULL,
                                      (ilImageFormat *)NULL);
                }
                break;
        ...
        /*  Add a filter, from in *pCvtData.  Only the des and format may
           change; other info (width, stripHeight, etc.) stays the same.
        */
        ...
        pPriv = ilAddPipeElement(pipe, IL_FILTER, pCvtData->nBytesPrivate, 0,
                                 ...
                                 pCvtData->Init, pCvtData->Cleanup,
                                 pCvtData->Destroy, pCvtData->Execute, NULL, 0);
        if (!pPriv)
                return FALSE; /* EXIT */

        /*  If successful: call converters init function if non-null, return
         * true.
         */
        if (pCvtData->AddElement) {
                error = (*pCvtData->AddElement)(pPriv, pInfo->pPalette,
                                                pOptionData);
                if (error) {
...
*/
static ilBool ilAddFormatFilter(ilPipe pipe, ilPipeInfo *pInfo,
        ...
        */
        ...
        pPriv = ilAddPipeElement(pipe, IL_FILTER, pData->nBytesPrivate, 0,
                                 (ilSrcElementData *)NULL, &dstData,
                                 pData->Init, pData->Cleanup, pData->Destroy,
                                 pData->Execute, NULL, 0);
        if (!pPriv)
                return FALSE; /* EXIT */
...
/*  Public function; see spec.
    ...
    efficient."
        If the current pipe image already fits the bill, no pipe element is
   added. Note: compression is ignored in "pDes" - call ilCompress() to
   compress.
*/
ilBool ilConvert(ilPipe pipe, const ilImageDes *pDes,
                 const ilImageFormat *pFormat, int option, void *pOptionData) {
        ilPipeInfo info;
        ilImageDes pipeDes;
        ilImageFormat pipeFormat;
        unsigned int sample;
        ilBool doConvert, doSubsample;
        static unsigned long defaultThreshold =
            128; /* dflt threshold for gray->bitonal cvt */
#define ADD_CVT_FILTER(_filter)                                                \
        (ilAddConversionFilter(pipe, &info, &pipeDes, &pipeFormat,             \
                               pOptionData, (_filter)))

        /*  Macro to determine if the given YCbCr must be upsampled (any bits !=
         * 1) */
#define MUST_UPSAMPLE(_des)                                                    \
        ...
        /*  Overall approach: if pDes specified, try to convert image descriptor
            if different, by adding filters with ilAddConversionFilter(): the
           des converting filter may add a reformat filter before it, and may
           change the format - the new des and format are returned if changed.
            ...
            ilAddPipeElement() will do that and return an error if they are
           incompatible (e.g. des.type = IL_RGB with des.nSamplesPerPixel != 3).
        */

        /*  Get pipe info and decompress! if not ready for a filter, exit.
         */
        if (ilGetPipeInfo(pipe, TRUE, &info, &pipeDes, &pipeFormat) !=
            IL_PIPE_FORMING) {
                if (!pipe->context->error)
        ...
        /*  If no pDes, point pDes to pipe des for format check below, else
           check for descriptor conversions.
        */
        ...
        if (!pDes)
                ...
                /*  ilConvert() does not compress - use ilCompress() to
                 * compress. */
                if (pDes->compression != IL_UNCOMPRESSED)
                        return ilDeclarePipeInvalid(
                            pipe, IL_ERROR_CONVERT_COMPRESSION);

                if (pipeDes.type == pDes->type) /* cvt within type */
                        switch (pipeDes.type) {

                                /*  Within YCbCr: check for changes to luma or
                                   refBlack/White.  If any, convert to RGB;
                                   below cvt back to YCbCr will handle luma and
                                   ref.
                                */
                        case IL_YCBCR: {
                                const ilYCbCrSampleInfo *pSrcSample,
                                    *pDstSample;
                                ilBool doCvtYCbCr;

                                /* If must convert refBlack/White or
                                   lumaRed/Green/Blue, convert to RGB and back
                                   to YCbCr; only those conversions handle this.
                                */
                                if ((pipeDes.typeInfo.YCbCr.lumaRed !=
                                     pDes->typeInfo.YCbCr.lumaRed) ||
                                    (pipeDes.typeInfo.YCbCr.lumaGreen !=
                                     pDes->typeInfo.YCbCr.lumaGreen) ||
                                    (pipeDes.typeInfo.YCbCr.lumaBlue !=
                                     pDes->typeInfo.YCbCr.lumaBlue))
                                        doCvtYCbCr = TRUE;
                                else {
                                        for (pSrcSample =
                                                 pipeDes.typeInfo.YCbCr.sample,
                                            pDstSample =
                                                 pDes->typeInfo.YCbCr.sample,
                                            doCvtYCbCr = FALSE, sample = 0;
                                             sample < 3; sample++, pSrcSample++,
                                            pDstSample++)
                                                if ((pSrcSample->refBlack !=
                                                     pDstSample->refBlack) ||
                                                    (pSrcSample->refWhite !=
                                                     pDstSample->refWhite))
                                                        doCvtYCbCr = TRUE;
                                ...
                                if (doCvtYCbCr) { /* cvt to RGB */
                                        if (!ilConvert(pipe, IL_DES_RGB,
                                                       (ilImageFormat *)NULL, 0,
                                                       NULL))
                                                return FALSE;
                                        ilGetPipeInfo(pipe, FALSE, &info,
                                                      &pipeDes, &pipeFormat);
                                }
                                break;
                        }

                                /*  Within palette: if option is zero (0), or if
                                   option says to dither and image already
                                   dithered to requested levels: fine, done;
                                   otherwise cvt first to RGB, so below
                                   conversion to palette will be enabled.
                                */
                        case IL_PALETTE:
                                if (option == 0)
                                ...
                                if (option == IL_CONVERT_TO_PALETTE) {
                                        ...
                                        if ((pipeDes.flags &
                                            ...
                                            (pipeDes.typeInfo.palette
                                                 .levels[2] ==
                                             pData->levels[2]))
                                                break; /* dithered to requested
                                                          levels; fine */
                                }
                                if (!ADD_CVT_FILTER(
                                        &_ilPaletteToRGBByte)) /* cvt up to RGB
                                                                */
                                        return FALSE;
                                break;
                        } /* END cvt within type */

                /*  Loop converting image type until desired type reached.  May
                   take some iterations, e.g pal->bitonal = pal->rgb; rgb->gray;
                   gray->bitonal. For conversion to YCbCr, cvt to RGB first. For
                   conversion to palette, cvt to RGB first.
                */
                while (pipeDes.type != pDes->type) {
                        switch (pipeDes.type) {

                                /*  Bitonal: cvt to gray, then to other types as
                                 * necessary. */
                        case IL_BITONAL:
                                switch (pDes->type) {
                                case IL_GRAY:
                                ...
                                case IL_RGB: {
                                        if (!ilScale(
                                                pipe, info.width, info.height,
                                                IL_SCALE_BITONAL_TO_GRAY, NULL))
                                                return FALSE;
                                        ilGetPipeInfo(pipe, FALSE, &info,
                                                      &pipeDes, &pipeFormat);
                                        break;
                        ...
                        case IL_GRAY:
                                ...
                                /*  If not 256 levels/sample, cvt to 256 if
                                 * possible. */
                                if (pipeDes.nLevelsPerSample[0] != 256)
                                        if (!_ilAddLevelConversionFilter(
                                                pipe, &info, &pipeDes,
                                                IL_DES_GRAY, &pipeFormat))
                                                return FALSE;
                                        ...
                                        if (option == IL_CONVERT_THRESHOLD) {
                                                if (!pOptionData)
                                                        pOptionData =
                                                            &defaultThreshold;
                                                if (!ADD_CVT_FILTER(
                                                        &_ilThresholdGrayToBitonal))
                                                        return FALSE; /* error,
                                                                         EXIT */
                                        } else if (
                                            !ADD_CVT_FILTER(
                                                &_ilDiffuseGrayToBitonal))
                                ...
                                /*  Palette to anything (except self - but that
                                   case wont get here) is currently done by
                                   converting to RGB, then to des type if
                                   necessary.
                                */
                        case IL_PALETTE:
                                ...
                                /*  RGB goes to gray for gray or bitonal; handle
                                   conversion to YCbCr. If not 256
                                   levels/sample, cvt to 256 if possible.
                                */
                        case IL_RGB:
                                if ((pipeDes.nLevelsPerSample[0] != 256) ||
                                ...
                                case IL_YCBCR:
                                        if (!_ilConvertRGBToYCbCr(
                                                pipe, &info, &pipeDes, pDes,
                                                &pipeFormat))
                                                return FALSE;
                                        break;

                                        /*  Cvt RGB to palette: set pipeDes.type
                                           to IL_PALETTE to terminate loop:
                                           private type may actually have been
                                           created.
                                        */
                                case IL_PALETTE:
                                        if (!_ilConvertRGBToPalette(
                                                pipe, &info, &pipeDes,
                                                &pipeFormat, option,
                                                (ilConvertToPaletteInfo *)
                                                    pOptionData))
                                ...
                                /*  YCbCr (aka LUV): convert based on dst type
                                   and set "convertDone" to true, else convert
                                   to RGB, and then loop again until desired
                                   type.
                                */
                        case IL_YCBCR: {
                                ...
                                switch (pDes->type) {
                                case IL_BITONAL:
                                case IL_GRAY: /* extract Y plane if Y range
                                                 0..255 */
                                        if ((pipeDes.typeInfo.YCbCr.sample[0]
                                                 .refBlack == 0) &&
                                            (pipeDes.typeInfo.YCbCr.sample[0]
                                                 .refWhite == 255)) {
                                                if (!_ilUpsampleYCbCr(
                                                        pipe, &info, &pipeDes,
                                                        &pipeFormat, TRUE,
                                        ...
                                        break;
                                case IL_PALETTE: /* try direct YCbCr to palette
                                                    conversion */
                                        if (_ilDitherYCbCr(
                                                pipe, &info, &pipeDes,
                                                &pipeFormat, option,
                                ...
                                if (!convertDone) { /* no conversion; convert
                                                       YCbCr to RGB */
                                        if (upSample &&
                                            !_ilUpsampleYCbCr(
                                                pipe, &info, &pipeDes,
                                                ...
                                                return FALSE;
                                        if (!_ilConvertYCbCrToRGB(pipe, &info,
                                                                  &pipeDes,
                                                                  &pipeFormat))
                                                return FALSE;
                                }
                ...
                /*  Check levels/sample, but not for palette images, where the
                   levels/sample must be in range 2 .. (1<<nbits) - 1, but is
                   otherwise ignored.
                */
                for (sample = 0; sample < pDes->nSamplesPerPixel; sample++)
                        if (pipeDes.nLevelsPerSample[sample] !=
                            pDes->nLevelsPerSample[sample]) {
                                if ((pipeDes.type != IL_PALETTE) &&
                                    !_ilAddLevelConversionFilter(pipe, &info,
                                                                 &pipeDes, pDes,
                                                                 &pipeFormat))
                                        return FALSE;
                                break;
                ...
                switch (pipeDes.type) {
                        ...
                        /*  "blackIsZero" applies only to bitonal and gray scale
                           images. If different then invert bits if not "soft"
                           invert (which means leave bits alone); if same,
                           invert bits if "hard" invert (which means invert
                           bits).
                        */
                case IL_BITONAL:
                case IL_GRAY:
                        if (pipeDes.blackIsZero != pDes->blackIsZero) {
                                ...
                                if (option == IL_CONVERT_SOFT_INVERT)
                                        _ilSetPipeDesFormat(
                                            pipe, &pipeDes,
                                            (ilImageFormat *)NULL);
                                else if (!_ilAddInvertFilter(pipe, &pipeDes,
                                                             &info))
                                        goto CantConvert;
                        }
                        break;

                        /*  YCbCr: check sub/upsampling; up/subsample as
                         * necessary */
                case IL_YCBCR: {
                        ...
                        for (pSrcSample = pipeDes.typeInfo.YCbCr.sample,
                            ...
                            doUpsample = FALSE, sample = 0;
                             sample < 3; sample++, pSrcSample++, pDstSample++) {
                                if (pSrcSample->subsampleHoriz <
                                    pDstSample->subsampleHoriz)
                                        doSubsample = TRUE;
                                else if (pSrcSample->subsampleHoriz >
                                         pDstSample->subsampleHoriz)
                                        doUpsample = TRUE;
                                if (pSrcSample->subsampleVert <
                                    pDstSample->subsampleVert)
                                        doSubsample = TRUE;
                                else if (pSrcSample->subsampleVert >
                                         pDstSample->subsampleVert)
                                        doUpsample = TRUE;
                        }
                        if (doUpsample)
                                if (!_ilUpsampleYCbCr(pipe, &info, &pipeDes,
                                                      &pipeFormat, FALSE, TRUE))
                                        return FALSE;
                        if (doSubsample) {
                                pipeDes = *pDes; /* pass subsample pars; des
                                                    convert done */
                                if (!_ilSubsampleYCbCr(pipe, &info, &pipeDes,
                                                       &pipeFormat))
                                        return FALSE;
                        }
                } /* END YCbCr */
        ...
        /*  If pFormat given, convert to requested format.
            WARNING !!!!! - ilCheckPipeFormat() below also does format
           conversions - May have to change it when adding new format filters
           !!!!
        */
        if (pFormat) {

                /*  Check if bits/sample needs conversion and if that conversion
                   supported. Currently supported: 4 to 8 bit, pixel mode only
                   if multi-sample.
                */
                doConvert = FALSE;
                for (sample = 0; sample < pDes->nSamplesPerPixel; sample++)
                        if (pipeFormat.nBitsPerSample[sample] !=
                            pFormat->nBitsPerSample[sample]) {
                                doConvert = TRUE;
                                break;
                        }

                /*  If any format conversions and YCbCr, upsample if necessary.
                    However, if subsampling was done before, error: cant
                   convert.
                */
                if ((pipeDes.type == IL_YCBCR) && MUST_UPSAMPLE(pipeDes) &&
                    (doConvert ||
                     (pipeFormat.sampleOrder != pFormat->sampleOrder) ||
                     (pipeFormat.rowBitAlign != pFormat->rowBitAlign))) {
                        if (doSubsample)
                                goto CantConvert; /* cant upsample - subsampled
                                                     before */
                        if (!_ilUpsampleYCbCr(pipe, &info, &pipeDes,
                                              &pipeFormat, FALSE, TRUE))
                                return FALSE;
                ...
                /*  If doConvert, then pipe image does not have desired # of
                   bits/sample. Check if all planes have same # src, dst bits;
                   else can't convert. Currently can convert 4->8 or 8->4.
                */
                if (doConvert) {
                        int nSrcBits, nDstBits;
                        if ((pipeDes.nSamplesPerPixel > 1) &&
                            (pFormat->sampleOrder != IL_SAMPLE_PIXELS))
                                goto CantConvert;
                        nSrcBits = pipeFormat.nBitsPerSample[0];
                        nDstBits = pFormat->nBitsPerSample[0];
                        for (sample = 1; sample < pDes->nSamplesPerPixel;
                             sample++)
                                if ((nSrcBits !=
                                     pipeFormat.nBitsPerSample[sample]) ||
                                    (nDstBits !=
                                     pFormat->nBitsPerSample[sample]))
                                        goto CantConvert; /* not same # src,dst
                                                             bits each plane */

                        if ((nSrcBits == 4) && (nDstBits == 8)) {
                                if (!ilAddFormatFilter(pipe, &info, pFormat,
                                                       &_ilFormat4To8Bit))
                                        return FALSE;
                ...
                if (pipeFormat.sampleOrder != pFormat->sampleOrder) {
                        ...
                        if (pipeFormat.sampleOrder == IL_SAMPLE_PIXELS) {
                                if (!ilAddFormatFilter(
                                        pipe, &info, pFormat,
                                        &_ilFormat8Bit3PixelToPlane))
                                        return FALSE;
                        } else if (!ilAddFormatFilter(
                                       pipe, &info, pFormat,
                                       &_ilFormat8Bit3PlaneToPixel))
                                return FALSE;
                }

                if (pipeFormat.rowBitAlign != pFormat->rowBitAlign) {
                        if (!ilAddFormatFilter(pipe, &info, pFormat,
                                               &_ilFormatRowBitAlign))
                                return FALSE;
                }
        }
...
IN PROGRESS - NOT DONE YET !!!!!!!!!!
    ...
    */
    unsigned int
    ilCheckPipeFormat(ilPipe pipe, register unsigned long formatMask,
         ...
         * error. */
        if (ilGetPipeInfo(pipe, TRUE, pInfo, pDes, pFormat) !=
            IL_PIPE_FORMING) {
                if (!pipe->context->error)
                        ilDeclarePipeInvalid(pipe, IL_ERROR_PIPE_STATE);
                return IL_CPF_ERROR;
        ...
        /*  Look at des, format and try to match/convert to format(s) caller
         * accepts */
        if (pDes->nSamplesPerPixel == 1) {

                /*  1 sample/pixel: if bit/pixel and caller accepts, check
                 * rowBitAlign */
                if ((formatMask & IL_CPF_BIT) &&
                    (pFormat->nBitsPerSample[0] == 1)) {
                        if (pFormat->rowBitAlign != 32) {
                                pFormat->rowBitAlign = 32;
                                if (!ilAddFormatFilter(pipe, pInfo, pFormat,
                                                       &ilFormatRowBitAlign))
                                        return IL_CPF_ERROR;
                                return IL_CPF_BIT; /* match found: bit/pixel */
                        }
                } else {
                        /*  1 sample, bit not accepted; error if byte not
                         * accepted, make byte */
                        ...
                        if (pFormat->nBitsPerSample[0] != 8) {
                                if (pFormat->nBitsPerSample[0] == 4) {
                                        ...
                                        return IL_CPF_ERROR; /* NOTE!! currently
                                                                only 4-8
                                                                supported!!! */
                        }
