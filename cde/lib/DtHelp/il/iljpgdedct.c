...
static void
_il_fwft_rev_scale(iljpgPtr q, /* pointer to quantization matrix */
        ...
        /* Note: given Q table is zigzag'd, as in JIF stream.  De-zigzag *qptr.
           Also: if an entry in the Q table is zero, then: if it is the first
           entry, store 16 (?) in the Q table, otherwise store the previous Q
           table value.
        */
...
iljpgError _iljpgDeDCTInit(iljpgDecodePrivPtr pPriv) {
        iljpgDataPtr pData;
        register int i;
        ...
        for (i = 0; i < 4; i++) {
                if (pData->QTables[i]) {
                        ...
                        _il_fwft_rev_scale(pData->QTables[i],
                                           pPriv->DCTRevScaleTables[i]);
                }
        }
 ...
 */
ILJPG_PRIVATE_EXTERN
iljpgError _iljpgDeDCTCleanup(iljpgDecodePrivPtr pPriv) {
        register int i;
...
*/
ILJPG_PRIVATE_EXTERN
void _iljpgDeDCTFull(register int *pSrc, long nBytesPerRow,
        ...
        /* Constants needed by the 16 point Winograd Fourier Transform for inv.
         * DCT */
        ...
        /* do 1-D inverse DCT along columns. setup input data, which is out of
         * order */
        for (i = 0; i < 8; i++, oxaddr++, zptr++, pRevScale++, tmp = 0.0) {

                in0 = *(pSrc + zptr[0]) * pRevScale[0] +
                      tmp; /* Add 128.0 to first DCT only */
...
*/
ILJPG_PRIVATE_EXTERN
void _iljpgDeDCT4x4(register int *pSrc, long nBytesPerRow,
                    iljpgPtr ix, /* RETURNED */
                    register float *pRevScale) {
        register int i;
        register int *zptr;
        ...
        /* descale-dequantize dct coefficients - need to do this for 4x4
         * submatrix */
...
*/
ILJPG_PRIVATE_EXTERN
void _iljpgDeDCTDCOnly(int *pSrc, long nBytesPerRow,
