...
/*  One element in the pipe, added by ilAddElement().  Notes:
        ...
        pNext, pPrev    forward/back ptrs to next/prev element in list.  First
   element in list is *ilPipePtr->elementHead.pNext; last is *pPrev.

        elementType     One of: IL_PRODUCER, IL_FILTER, IL_CONSUMER.  Not needed
   or used, but very handy for debugging.

        flags           flag values, e.g. IL_ADD_PIPE_NO_DST

        exec            par block to Execute() for this element: ptrs to src/dst
   image src/dst lines to read.

        Init()
        Cleanup()
        Destroy()
        Execute()       functions passed to ilAddPipeElement(); see spec for
   details
...
typedef struct _ilElementRec {
        ...
        ilError (*ExecuteFour)(register ilExecuteData *, long, long *, float);
...
/*  Actual contents of a ilPipe.   Notes:
        ...
        state           current state of the pipe as returned by
   ilGetPipeInfo(); one of:

            IL_PIPE_INVALID     an error occurred in forming the pipe, and the
   pipe cannot be used until ilEmptyPipe() is done.  No more elements should be
   added.  The pipe is in fact empty, only the state is different, to prevent
   other elements from being added.  This state is set by calling
                                ilDeclarePipeInvalid().

            IL_PIPE_EMPTY       pipe is empty; ready to add a producer (read)

            IL_PIPE_FORMING     pipe has a producer and zero or more filters;
   ready to add filters or a consumer (write)

            IL_PIPE_COMPLETE    pipe has a consumer (write); ilExecutePipe() can
   now be called on it (executable, from caller's view). Error if an attempt to
   add an element.

            IL_PIPE_EXECUTING   one or more, but not all, strips have been
   processed.
        ...
        elementHead     head/tail of list of ilElementRec, the elements in the
   pipe

        hookHead        head/tail of list of IL_HOOK ilElementRec, the
   pseudo-elements in the pipe of type "hook" - not in elementHead list.

        stackIndex      index of top of "execStack"; see below.

        execStack       stack of ptrs to elements that returned
   IL_ERROR_ELEMENT_AGAIN "stackIndex" points to the top of stack. [0] contains
   the first element in the list.

        nDestroyObjects # of elements in "destroyObjects" array; see below.

        destroyObjects  an array of "nDestroyObjects" objects which must be
   destroyed (by calling ilDestroyObject()) when the pipe is emptied.  This
                        array is added to by calling ilAddPipeDestroyObject().

        lastStrip       inited to false when pipe started, set true when "last
   strip" error returned from an element.  Pipe execution is complete when
   lastStrip && stackIndex <= 0 (> 0 => last strip has still not made it all the
   way through the pipe).

        needProducerThrottle true iff ilAddProducerImage() just called: pipe
   began with an ilReadImage() and no filters added yet.
        ...
        imageHead       head/tail of list of ilImageRec, the temporary images
                        associated with this pipe.  The images object prev/next
   ptrs are used to maintain this list.  Note that the images in this list are
   not official objects; they must be deleted when then the pipe is emptied.
   The images have buffers only when state is IL_PIPE_EXECUTING; the list is
   null when state is IL_PIPE INVALID or EMPTY.

        pSrcPipeImage   if non-null, points the source (input) pipe image to the
   next element to be added.  If null, must allocate a src image for the next
   element unless it is a ilWriteImage().

        image           describes the current pipe image.  The values in this
   struct meaningful only if pipe forming (state == IL_PIPE_FORMING). The fields
   are as returned by ilGetPipeInfo().
*/
...
/*  Default optimal strip size, in bytes.  Used by ilRecommendedStripHeight().
        NOTE: currently the same as TIFF strip size, "IL_WRITE_TIFF_STRIP_SIZE"
   in /ilc/iltiffwrite.c .  The two are not directly coupled, but making them
   different may adversely affect performance, e.g. cause de/recompression!
*/
...
*/
void _ilSetDefaultStripSize(long stripSize) { ilDefaultStripSize = stripSize; }
...
*/
static void ilFreeTempImageBuffers(register ilPipePtr pPipe) {
...
*/
static ilImagePtr ilAllocTempImage(register ilPipePtr pPipe, ilPipeInfo *pInfo,
                                   ilImageDes *pDes, ilImageFormat *pFormat) {
        register ilImagePtr pImage;
...
*/
static ilError ilCleanupRunningPipe(register ilPipePtr pPipe, ilBool aborting) {
        register ilElementPtr pElement, pElementHead;
        ilError error, prevError;

        /*  Run thru the list of elements and call Cleanup function if non-null,
           then do same for "hook" list.  Pass given aborting flag to Cleanup().
            If an error return, switch to "aborting" mode and return first error
           returned.
        */
        prevError = IL_OK;
        ...
        while (TRUE) {
                ...
                while (pElement != pElementHead) {
                        if (pElement->Cleanup) {
                                error = (*pElement->Cleanup)(
                                    (ilPtr)pElement->exec.pPrivate, aborting);
                                if (error) {
                                        if (prevError == IL_OK)
                                                prevError = error;
                                        aborting = TRUE; /* error; now
                                                            considered aborted
                                                          */
                                }
                        }
...
*/
ilBool ilEmptyPipe(ilPipe pipe) {
        ...
        /*  Action to empty a pipe depends on current state.  Transition from
           running (abort) to complete (empty element list) to invalid (mark as
           empty). FALL THRU (no break) from each state.
        */
        ...
        case IL_PIPE_FORMING:  /* forming is same as complete */
                ...
                /*  Run thru list of elements and free them.  Call optional
                   Destroy function if non-null.  Make list head empty.
                */
                pElementHead = &pPipe->elementHead;
                while (TRUE) {
                        pElement = pElementHead->pNext;
                        ...
                        if (pElementHead == &pPipe->hookHead)
                                break; /* both lists scanned; done */
                        pElementHead =
                            &pPipe->hookHead; /* run thru hook list */
                }
                pPipe->elementHead.pNext = pPipe->elementHead.pPrev =
                    &pPipe->elementHead;
                pPipe->hookHead.pNext = pPipe->hookHead.pPrev =
                    &pPipe->hookHead;

                /*  Destroy all temp images associated with this pipe.  The
                   buffers are already gone, because they only exist when state
                   is IL_PIPE_EXECUTING, and ilCleanupRunningPipe() would be
                   called in that case.
                */
                pImage = (ilImagePtr)pPipe->imageHead.pNext;
                while (pImage != (ilImagePtr)&pPipe->imageHead) {
                ...
                pPipe->imageHead.pNext = pPipe->imageHead.pPrev =
                    (ilPtr)&pPipe->imageHead;
...
ilPipe ilCreatePipe(ilContext context, unsigned long mustBeZero) {
        register ilPipePtr pPipe;
        ...
        pPipe = (ilPipePtr)_ilCreateObject(
            context, IL_PIPE, ((void (*)())ilEmptyPipe), sizeof(ilPipeRec));
        if (!pPipe)
                return (ilPipe)NULL;

        /*  Declare pipe empty, set element and temp image lists to empty state.
            The rest of the pipe state must be set by ilAddPipeElement() on each
           element. Init some of elementHead, which itself is an element, and
           will be referenced as the current element in a pipe with no elements.
        */
        pPipe->state = IL_PIPE_EMPTY;
        pPipe->elementHead.pNext = pPipe->elementHead.pPrev =
            &pPipe->elementHead;
...
*/
ilBool ilDeclarePipeInvalid(ilPipe pipe, ilError error) {
        register ilPipePtr pPipe;
...
*/
unsigned int ilQueryPipe(ilPipe pipe, long *pWidth, /* RETURNED */
...
/*  ------------------------ ilGetPipeInfo ----------------------------- */
/*  Public function; see spec.
    If pipe state is "forming": if "forceDecompress" and image is compressed,
   make it uncompressed, then return description of current pipe state.
*/
unsigned int ilGetPipeInfo(ilPipe pipe, ilBool forceDecompress,
                           ilPipeInfo *pInfo, ilImageDes *pDes,
        ...
        if (pPipe->state == IL_PIPE_FORMING) {
                ...
                /*  Decompress image if it is compressed and caller asked for
                 * decompressed.
                 */
                if (forceDecompress &&
...
/*  Public function; see spec.
 */
long ilRecommendedStripHeight(const ilImageDes *pDes,
                              const ilImageFormat *pFormat, long width,
                              long height) {
        long bytesPerRow[IL_MAX_SAMPLES];
        register long stripHeight, nBytes;
        int i;

        /*  Get the bytes/row, per plane.  If pixel format, all bytes are in
           plane 0, else in pDes->nSamplesPerPixel planes.
        */
        if (height <= 0)
                stripHeight = 0;
        else {
...
*/
static void ilChangeStripHeight(register ilPipePtr pPipe) {
        ...
        if (stripHeight > pPipe->image.info.height)
                pPipe->image.info.stripHeight = stripHeight =
                    pPipe->image.info.height;
        else if (stripHeight <= 0)
                pPipe->image.info.stripHeight = stripHeight = 1;

        /*  If uncompressed image, calc recommendedStripHeight else current
         * stripHeight */
        if (pPipe->image.des.compression == IL_UNCOMPRESSED) {
                register long i;
                i = ilRecommendedStripHeight(
                    &pPipe->image.des, &pPipe->image.format,
                    pPipe->image.info.width, pPipe->image.info.height);
                pPipe->image.info.recommendedStripHeight =
                    (i > stripHeight) ? stripHeight : i;
        } else
                pPipe->image.info.recommendedStripHeight = stripHeight;
}
...
/*  IL internal function, declared in /ilc/ilpipeint.h .
    ...
    NOTE: a hard limit exists on the # of objects which can be added; not
   intended for unlimited use, e.g. by filters! Sets context->error to error
   code, returns true if success.
*/
IL_PRIVATE ilBool _ilAddPipeDestroyObject(ilPipe pipe, ilObject object) {
        register ilPipePtr pPipe;

        pPipe = (ilPipePtr)pipe;
        if (pPipe->nDestroyObjects >= IL_MAX_DESTROY_OBJECTS) {
                pipe->context->error =
                    IL_ERROR_MALLOC; /* should not happen, return phony */
                return FALSE;
        }
...
*/
IL_PRIVATE ilBool _ilAddProducerImage(ilPipe pipe, register ilImagePtr pImage,
                                      unsigned int producerCode, long height,
                                      long stripHeight, ilBool constantStrip,
                                      ilBool needProducerThrottle) {
        register ilPipePtr pPipe;
        ...
        pPipe->image.info.producerObject = (ilObject)pImage;
        pPipe->image.info.pPalette =
            pImage->i.pPalette; /* point to image palette */
        pPipe->image.info.pCompData =
            pImage->i.pCompData; /* point to comp data */
        pPipe->pSrcPipeImage = &pImage->i;
        pPipe->state = IL_PIPE_FORMING;
        pPipe->feedDone = FALSE; /* ilSetFeedPipeData not done yet. */

        /*  Signal ilAddPipeElement() to add a throttle before next filter is
           added, and that a copy filter must be added if ilWriteImage() is
           called next.
        */
        pPipe->needProducerThrottle = needProducerThrottle;
        pPipe->copyToConsumerImage = TRUE;

        /*  Inc pImage->refCount, and add as destroyObject for when pipe
         * destroyed. */
        pImage->o.refCount++;
...
*/
IL_PRIVATE ilBool _ilSetFeedPipeData(ilPipe pipe, long start, long nLines,
                                     long nCompBytes) {
        register ilPipePtr pPipe;
        register ilImagePtr pImage;
        ...
        /*  If uncompressed image, starting line plus # lines must be <= height,
           and "start" is starting line; if compressed "start" is starting byte
           offset.
        */
...
*/
IL_PRIVATE void _ilSetPipeDesFormat(ilPipe pipe, ilImageDes *pDes,
                                    ilImageFormat *pFormat) {
        register ilPipePtr pPipe;
...
ilPtr ilAddPipeElement(
    ilPipe pipe, int elementType,
    /* Use portable type for sizeof() operator, bug report OSF_QAR# 32082 */
    size_t nBytesPrivate, unsigned long flags, ilSrcElementData *pSrcData,
    ilDstElementData *pDstData, ilError (*Init)(), ilError (*Cleanup)(),
    ilError (*Destroy)(),
    /*
    ...
    */
    ...
    unsigned long mustBeZero) {
        ...
        /*  Validate elementType and pipe state: "empty" for producer, else
           "forming". Set newState to state pipe becomes after this element is
           added. If a "hook" element, add hook element and EXIT. For filters
           and consumers: set stripHeight to source (input) strip height
            required by this element; set constantStrip true if every strip
           (except last) must be stripHeight lines high.  Default if !pSrcData
           or stripHeight == 0: !constantStrip; filters: recommended strip
           height; consumers: whole strip. Set some flags, explained when used
           below.
        */
        ...
        case IL_PRODUCER:
                if (pPipe->state != IL_PIPE_EMPTY) {
                        ilDeclarePipeInvalid((ilPipe)pPipe,
                                             IL_ERROR_PIPE_STATE);
                        return (ilPtr)NULL;
                }
        ...
        case IL_CONSUMER:
                ...
                if (!pSrcData || !pSrcData->stripHeight) {
                        stripHeight =
                            pPipe->image.info
                                .stripHeight; /* accept strip height */
                        constantStrip = FALSE;
                } else {
                        stripHeight = pSrcData->stripHeight;
                        constantStrip = pSrcData->constantStrip;
                ...
                /*  A "hook" element: invalid if pipe executing or invalid, else
                   add element to hook list with Init/Cleanup/Destroy functions
                   copied.
                */
        case IL_HOOK:
                if ((pPipe->state == IL_PIPE_INVALID) ||
                ...
                if (ExecuteThree || ExecuteFour) {
                        ilDeclarePipeInvalid((ilPipe)pPipe,
                                             IL_ERROR_PAR_NOT_ZERO);
                        return (ilPtr)NULL;
                }
                pElement = (ilElementPtr)IL_MALLOC_ZERO(sizeof(ilElementRec) +
                                                        nBytesPrivate);
                if (!pElement) {
        ...
        /*  If pipe begins with an ilReadImage() and this is first filter, add
            a throttle to entire strip height - this throttle knows to read
           whole strip and return IL_ERROR_LAST_STRIP - otherwise
           ilExecutePipe() would not stop. Note: ilAddThrottlePipeElement()
           calls this function: recursion! Also, note that no pipe state has
           been changed up to this point. BUT, set needProducerThrottle to false
           or below check causes infinite recursion.
        */
        if ((elementType != IL_PRODUCER) && pPipe->needProducerThrottle) {
                pPipe->needProducerThrottle = FALSE; /* prevent recursion */
                if (!_ilAddThrottlePipeElement(
                        (ilPipe)pPipe, stripHeight, constantStrip,
                        pPipe->producerCode, &pPipe->image.info.stripHeight,
                        &pPipe->image.info.constantStrip)) {
                        ilDeclarePipeInvalid((ilPipe)pPipe,
                                             pPipe->o.p.context->error);
                        return (ilPtr)NULL;
                }
        }

        /*  If writing consumerImage (e.g. ilWriteImage()) or inserting a
           throttle requiring minBufferHeight, insert copy filter if flagged:
           e.g. previous element was "no dst" filter or ilReadImage(); else
           possibly add throttle: If not a producer and pipe image is not
           compressed, check "stripHeight", the desired strip height, set above.
                If != current strip height, or caller requires a constant strip
            and current is not constant, add a filter which makes it so.
                If throttle added successfully, set image info to what was asked
           for, but first save stripHeight - it is the size of the src buffer to
           create, if any.
        */
        srcBufferHeight = pPipe->image.info.stripHeight;
        if (insertCopyFilter) {
                if (pPipe->image.des.compression == IL_UNCOMPRESSED) {
                        if (!_ilInsertCopyFilter((ilPipe)pPipe,
                                                 pPipe->image.info.stripHeight))
                                return (ilPtr)NULL;
                ...
                if (!_ilAddThrottlePipeElement(
                        (ilPipe)pPipe, stripHeight, constantStrip,
                        IL_PIPE_NOT_IMAGE, &pPipe->image.info.stripHeight,
                        &pPipe->image.info.constantStrip)) {
                        ilDeclarePipeInvalid((ilPipe)pPipe,
                                             pPipe->o.p.context->error);
                        return (ilPtr)NULL;
        ...
        /*  Allocate and zero element struct and add to end of list; exit if
         * error. */
        pElement =
            (ilElementPtr)IL_MALLOC_ZERO(sizeof(ilElementRec) + nBytesPrivate);
        if (!pElement) {
                ilDeclarePipeInvalid((ilPipe)pPipe, IL_ERROR_MALLOC);
                return (ilPtr)NULL; /* EXIT */
        }
        ...
        /*  Copy passed data into the element struct and init element.
            Each element's pNextSrcLine points to the next one's srcLine, and
           compressed. pOffset/pNBytesWritten point to next one's
           srcOffset/nBytesToRead. Set "Destroy" to null for now, so it will not
           be called if we fail here. Set it to passed function only when sure
           of success.
        */
        ...
        pPrevElement->exec.compressed.pDstOffset =
            &pElement->exec.compressed.srcOffset;
        pPrevElement->exec.compressed.pNBytesWritten =
            &pElement->exec.compressed.nBytesToRead;
        ...
        /*  If a producer, must have valid dst data and pDes/Format, or error.
            Skip allocation of src image for producers - they get src on their
           own; set pSrcPipeImage to NULL.  Init # src lines to read to strip
           height. Set ilPipeInfo.pPalette to callers if palette image, else to
           null
        */
        if (elementType == IL_PRODUCER) {
                if (!pDstData || !pDstData->pDes || !pDstData->pFormat) {
                        ilDeclarePipeInvalid((ilPipe)pPipe,
                                             IL_ERROR_PRODUCER_DATA);
                        return (ilPtr)NULL; /* EXIT */
                }
                if ((pDstData->width <= 0) || (pDstData->height <= 0)) {
                        ilDeclarePipeInvalid((ilPipe)pPipe,
                                             IL_ERROR_ZERO_SIZE_IMAGE);
                        return (ilPtr)NULL; /* EXIT */
                }
                error = _ilValidateDesFormat(TRUE, pDstData->pDes,
                                             pDstData->pFormat);
                if (error) {
                ...
                pPipe->producerCode =
                    IL_PIPE_NOT_IMAGE; /* not a read from a (feed) image */
                ...
                pPipe->image.info.pPalette =
                    (pPipe->image.des.type == IL_PALETTE)
                        ? pDstData->pPalette
                        : (unsigned short *)NULL;
                ...
                /*  Not a producer.  Create src image to this element, unless a
                   consumer which specifies its source image (e.g.
                   ilWriteImage()), or src image specified as output from last
                   element = a producer (e.g. ilReadImage()): pSrcImage.
                */
                if (pPipe->pSrcPipeImage) {
                        pElement->exec.pSrcImage = pPipe->pSrcPipeImage;
                        pPipe->pSrcPipeImage = (ilImageInfo *)NULL;
                } else if (haveConsumerImage) {
                        pElement->exec.pSrcImage =
                            &((ilImagePtr)pSrcData->consumerImage)->i;
                } else {
                        /*  Src image not provided; must create a temp and add
                           to list of temps. Set image height = max (minHeight
                           if present, srcBufferHeight).
                        */
                        ilImagePtr pImage;
                        pImage = ilAllocTempImage(pPipe, &pPipe->image.info,
                                                  &pPipe->image.des,
                                                  &pPipe->image.format);
                        if (!pImage) {
                                ilDeclarePipeInvalid((ilPipe)pPipe,
                        ...
                        if (pSrcData &&
                            (pSrcData->minBufferHeight > srcBufferHeight))
                ...
                /*  Src image to this element is dst image of previous element.
                    If no dst from this element, then src to this element is src
                   to next, and must copy if next element is ilWriteImage();
                   otherwise, src to next is necessarily a temp image, and don't
                   need to copy.
                */
                ...
                /*  If not a consumer and have pDstData: if non-null des/format
                   ptr, copy data from it into the pipe image area - element
                   changes des or format. Set rest of data from pDstData:
                   stripHeight if not zero. Set ilPipeInfo.pPalette to null if
                   not a palette image, or to callers value if it is not null.
                    Validate pDes and pFormat together if either non-null (else
                   no change).
                */
                if (pDstData && (elementType != IL_CONSUMER)) {
                        ...
                        if (pDstData->stripHeight != 0) {
                                pPipe->image.info.constantStrip =
                                    pDstData->constantStrip;
                                pPipe->image.info.stripHeight =
                                    pDstData->stripHeight;
                        }
                        ...
                        if (pDes || pDstData->pFormat) {
                                error = _ilValidateDesFormat(
                                    TRUE /* allow private types */,
                                    &pPipe->image.des, &pPipe->image.format);
                                if (error) {
                                        ilDeclarePipeInvalid((ilPipe)pPipe,
                                                             error);
                        ...
                        ilChangeStripHeight(
                            pPipe); /* reset (recommended) strip height */
...
*/
ilBool ilAbortPipe(ilPipe pipe) {
        register ilPipePtr pPipe;
        ...
        if (pPipe->state == IL_PIPE_EXECUTING)
                return ((pPipe->o.p.context->error =
                             ilCleanupRunningPipe(pPipe, TRUE)) == IL_OK);
        else {
                pPipe->o.p.context->error = IL_OK;
                return TRUE;
        }
...
/*  Public function: see spec.
 */
int ilExecutePipe(ilPipe pipe, long nStrips, float ratio) {
        register ilPipePtr pPipe;
        ...
        /*  Execute based on pipe state.  Error if pipe invalid, empty or
           forming. Otherwise: abort if requested, cleanup if in progress.
           Compile pipe if complete, or executable and producer changed.  If in
           progress and producer changed, cleanup and return error.
        */
        pExecHead =
            &pPipe->elementHead; /* head of "executable" (non-hook) list */
        switch (pPipe->state) {
        ...
        case IL_PIPE_COMPLETE:
                ...
                /*  Call elements' Init() functions if non-null. If Init()
                   returns error: call Cleanup() of previous elements, whose
                   Init()s were already called, and free image buffers allocated
                   above, and return error in errorInfo. Init all srcLines and
                   srcOffsets to 0; always start at line/byte 0. Do above first
                   for "hook" list, then for regular element list.
                */
                ...
                while (TRUE) {
                        ...
                        while (pElement != pElementHead) {
                                ...
                                if (pElement->Init) {
                                        ilError error = (*pElement->Init)(
                                            (ilPtr)pElement->exec.pPrivate,
                                            pElement->exec.pSrcImage,
                                            pElement->exec.pDstImage);
                                        if (error) {
                                                while (TRUE) {
                                                        while (
                                                            pElement->pPrev !=
                                                ...
                                                ilFreeTempImageBuffers(pPipe);
                                                pPipe->o.p.context->errorInfo =
                                                    error;
                                                pPipe->o.p.context->error =
                                                    (error < 0)
                                                        ? IL_ERROR_USER_PIPE_ELEMENT
                                                        : IL_ERROR_PIPE_ELEMENT;
                                                return IL_EXECUTE_ERROR; /* EXIT
                                                                          */
                                        }
                                }
                ...
                /*  Now executing; FALL THRU to execute one pass.  Point top of
                   execStack to first element; it changes to top of execStack on
                   a push.
                */
                pPipe->stackIndex = 0;
                ...
                /*  Pipe being executed, possibly first time. Execute nStrips
                   strips (or all if !nStrips). dstLine for each element is
                   "srcLine" of next element. Start with top element of
                   "execStack". "nLines" on entry to Execute() = # of src lines
                   to read; on exit = # of dst lines written (== 0, skip rest of
                   pipe). If error: check for pseudo-error, else abort.
                   Pseudo-errors: IL_ERROR_LAST_STRIP: finish processing this
                   strip, then done IL_ERROR_ELEMENT_AGAIN: element states that
                   it needs to be recalled (e.g. a throttle with more left in
                   its input buffer). Push pElement onto execStack; exit if
                   overflow. IL_ERROR_ELEMENT_COMPLETE: element states that it
                   previously issued "AGAIN" and is now done.  Pop stack; exit
                   if underflow.
                */
        case IL_PIPE_EXECUTING:
                if (pPipe->producerCode == IL_PIPE_FEED_IMAGE) {
                        ...
                        if (!pPipe->feedDone) { /* pipe executed directly
                                                   instead of fed */
                                ilCleanupRunningPipe(pPipe, TRUE);
                                pPipe->o.p.context->error =
                                    IL_ERROR_PIPE_NOT_FED;
                        ...
                        pFirstElement->exec.compressed.srcOffset =
                            pPipe->feedCompOffset;
                        pFirstElement->exec.compressed.nBytesToRead =
                            pPipe->feedCompNBytes;
                } else
                        initNLines =
                            pPipe->image.info
                                .height; /* first element reads whole image */
                ...
                while (TRUE) { /* until all or nStrips done */
                        ...
                        while (pElement != pExecHead) {
                                /*
                                ** Added another execute function for passing in
                                *a fourth
                                ** parameter which is a floating pointing.  So
                                *check to see
                                ** which execute function is not null and call
                                *that one.
                                */
                                if (pElement->ExecuteThree) {
                                        error = (*pElement->ExecuteThree)(
                                            &pElement->exec,
                                            pElement->pNext->exec.srcLine,
                                            &nLines);
                                } else {
                                        error = (*pElement->ExecuteFour)(
                                            &pElement->exec,
                                            pElement->pNext->exec.srcLine,
                                            &nLines, ratio);
                                }
                                        ...
                                        if (error == IL_ERROR_ELEMENT_AGAIN) {
                                                if (++pPipe->stackIndex >=
                                                    IL_MAX_EXEC_STACK) {
                                                        ...
                                                        pPipe->o.p.context
                                                            ->error =
                                                            IL_ERROR_EXECUTE_STACK_OVERFLOW;
                                                        return IL_EXECUTE_ERROR; /* EXIT */
                                                }
                                                pPipe->execStack
                                                    [pPipe->stackIndex] =
                                                   ...
                                                   IL_ERROR_ELEMENT_COMPLETE) {
                                                if (--pPipe->stackIndex < 0) {
                                                        ilCleanupRunningPipe(
                                                            pPipe, TRUE);
                                                        pPipe->o.p.context
                                                            ->error =
                                                            IL_ERROR_EXECUTE_STACK_UNDERFLOW;
                                                        return IL_EXECUTE_ERROR; /* EXIT */
                                                }
                                        } else if (error == IL_ERROR_LAST_STRIP)
                                                pPipe->lastStrip = TRUE;
                                        else {
                                                ilCleanupRunningPipe(pPipe,
                                                                     TRUE);
                                                pPipe->o.p.context->errorInfo =
                                                    error;
                                                pPipe->o.p.context->error =
                                ...
                                if (nLines <=
                                    0) /* no lines written; skip rest of pipe */
                                        break;
                                pElement = pElement->pNext;
                        }

                        /*  If stack is empty (if not empty, still pushing the
                           current strip thru the pipe): if last strip, do
                           cleanup and exit, else downcount nStrips (if != 0 =>
                           all strips) and return if 0.
                        */
                        if (pPipe->stackIndex <= 0) {
                                if (pPipe->lastStrip) {
                                        ilError error =
                                            ilCleanupRunningPipe(pPipe, FALSE);
                                        if (error == IL_OK)
