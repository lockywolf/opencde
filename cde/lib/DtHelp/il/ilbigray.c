...
#define IL_FILTER_SQUARE                                                       \
        (IL_FILTER_SIZE * IL_FILTER_SIZE) /* square size of filter */
...
*/
static void ilInit9BitFilter(long srcWidth, long srcHeight, long dstWidth,
                             long dstHeight, unsigned long nLevels,
                             ilBool blackIsZero, ilByte *pTable) {
        double filter[IL_FILTER_SQUARE];
        double fraction, widthScale, heightScale, totalArea, widthSmall,
            heightSmall;
        long value, nLevelsM1;
        int i;

        /*  Setup "filter" with the fractional (0..1) contribution of each pixel
           to the whole, in the bit order described above (bit 0 is the top-left
           pixel). The sum of all values should ~= 1.0.  The portion of each
           pixel selected is based on a square of size "srcWidth/dstWidth" by
           "srcHeight/dstHeight", mapped over the source pixels, centered on
           pixel 4 (above).  If the scale factor is greater than 3.0 in either
           direction, make it 3.0 (all pixels contribute equally in that
           direction.
        */
        ...
        /*  The center pixel is a 1x1 square.
            The four corner pixels are overlayed by a rect of width or height
           (wh) = (wh - 1) / 2: the whole side is "wh", - 1 for center, / 2
           cause there are 2. The four side pixels have width/height of corner
           pixels, but other dim = 1.
        */
        ...
        /*  Convert the filter to an array indexed by a 9 bit number: the filter
           bits as shown above, in order "876543210".  If whitePixel is 0, add
           the filter fraction if bit is off. If whitePixel not zero than add
           the filter fraction if bit is on.
        */
        nLevelsM1 = nLevels - 1;
        if (!blackIsZero) {
                for (i = 0; i < 512; i++) {
...
/*  --------------------- ilBiGrayInit -------------------------- */
/*  Init() function: copy values from given images to private for fast
 * reference.
 */
static ilError ilBiGrayInit(ilBiGrayPrivPtr pPriv, ilImageInfo *pSrcImage,
 ...
 */
static ilError ilBiGrayExecute(ilExecuteData *pData, long dstLine,
                               long *pNLines) {
#define ULPTR CARD32 *
#ifdef LSB_BIT_ORDER
...
#define SHIFT_MASK(m) ((m) >>= 1)
        ...
        */
        ...
        if (pPriv->srcBufferHeight)
                ...
                bottomLine = pData->srcLine + nLines - 1;
        ...
        while (TRUE) {
                ...
                /*  Algorithm: skip src bits, using basic scale algorithm.  To
                   form dst bit, form 9 bit value from 3x3, with top bits in
                   high order bits.  Lookup the 9 bit value in table of correct
                   gray scale.  "bitNumber" is the # of the left bit of 3x3; 31
                   = left bit of long.  If bitNumber is < 2, the bits cross a
                   long boundary: get bits from bytes, low order of this long
                   and high order byte of next long. pSrc will point to the
                   middle line of 3x3; the line above is
                    "-srcBytesAbove" away; the line below is "srcBytesBelow"
                   away.  If the 3x3 is outside of the buffer being read, set
                   "srcBytesAbove/Below" to 0, causing the middle line to be
                   re-read and replicated to replace the unaccessible line.
                */
                srcBytesAbove = (line < 0) ? 0 : pPriv->srcRowBytes;
                srcBytesBelow =
                    ((line + 2) > bottomLine) ? 0 : pPriv->srcRowBytes;
                nDstBits = pPriv->dstWidth;

                /*  Set pSrc and bitNumber to point to the rightmost bit of the
                   long before the first long.  The "colAcc > 0" check below
                   will be true, causing a bump to the next bit (first bit in
                   row), _except_ when "widthDiff" is 0 (no hori scaling).  In
                   this case, set colAcc to 1, forcing the first colAcc>0 check
                   to be true and bumping past first bit - the rest proceeds
                   normally.
                */
                ...
                /*  If bitonal output, logically produce a gray pixel, then
                   output a 1 if gray is < threshold.
                */
                if (pPriv->bitonalOutput) {
                        ULPTR pDst;
                        CARD32 mask, outLong;
                        ...
                        while (TRUE) {
                                ...
                                /*  If done and mask != left bit on, output bits
                                 * in "outLong". */
                                if (nDstBits <= 0) {
                                        if (mask != SPECIAL_MASK_BIT) {
                                                flip_bits((ilPtr)&outLong,
                                                          sizeof(outLong));
                                                *pDst++ = outLong;
                                        }
                                ...
                                if (--nDstBits <= 0) {
                                        INT32 rightBit =
                                            (((ilPtr)pSrc - pSrcLine) << 3) +
                                            34 - bitNumber;
                                        if ((rightBit > pPriv->srcWidth) &&
                                            (pPriv->srcWidth > 3))
                                                if (++bitNumber > 31) {
                                                        bitNumber = 0;
                                                        pSrc--;
                                ...
                                if (bitNumber >= 2) {
                                        ...
                                        ilPtr pSrcTemp =
                                            (ilPtr)pSrc - srcBytesAbove + 3;
                                        shift = 6 + bitNumber;
                                        index = ((CARD32)((*pSrcTemp << 8) |
                                                          *(pSrcTemp + 1)) >>
                                                 shift) &
                                                7;
                                        index <<= 3;
                ...
                /*  Line done; next src, dst lines; inc line index and # lines
                 * written.
                 */
                pSrcLine += pPriv->srcRowBytes;
                line++;
 ...
 */
/*  Called by ilScale().
    Adds an element to "pipe" to scale the pipe image to "dstWidth" by
   "dstHeight" which both must be > 0.  The input pipe image must be a bitonal,
    ...
    is the number of levels of gray.  The output image is a gray image.
        Only a scale down in both directions is allowed; the following must be
   true: srcWidth must be >= dstWidth and srcHeight must be >= dstHeight!
*/
IL_PRIVATE void _ilScaleBitonalToGray(ilPipe pipe, unsigned long dstWidth,
                                      unsigned long dstHeight,
                                      unsigned long nGrayLevels,
                                      ilBool blackIsZero, ilPipeInfo *pInfo) {
        register ilBiGrayPrivPtr pPriv;
        ...
        dstData.stripHeight =
            ((dstHeight * pInfo->recommendedStripHeight) / pInfo->height) + 1;
        dstData.constantStrip = FALSE;
        dstData.pPalette = (unsigned short *)NULL;
        pPriv = (ilBiGrayPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilBiGrayPrivRec), IL_ADD_PIPE_HOLD_SRC,
            (ilSrcElementData *)NULL, &dstData, ilBiGrayInit, IL_NPF, IL_NPF,
            ilBiGrayExecute, NULL, 0);
        if (!pPriv)
                return;

        /*  Pipe element added, init pPriv.  If not a tempImage, reading
           directly from a permanent image: set srcHeight to image height; else
           set to 0.
        */
        ...
        if (!pInfo->tempImage)
                ...
                pPriv->srcBufferHeight = 0;

        /*  Init the filter table in private based on scale factors and # gray
         * levels.
         */
        ilInit9BitFilter(pPriv->srcWidth, pPriv->srcHeight, pPriv->dstWidth,
                         pPriv->dstHeight, nGrayLevels, blackIsZero,
                         pPriv->filterValues);
...
/*  Called by ilScale().
    Adds an element to "pipe" to scale the pipe image to "dstWidth" by
   "dstHeight" which both must be > 0.  The input pipe image must be a bitonal,
    bit-per-pixel image, with _any_ "blackIsZero".  The resulting image is of
   the same type. "pInfo" points to the pipe info, "dstWidth/Height" is the (>
   0) size to scale to. Only a scale down in both directions is allowed; the
   following must be true: srcWidth must be >= dstWidth and srcHeight must be >=
   dstHeight! Logically the same as ilScaleBitonalToGray(): a gray byte is
   produced for each destination pixel, using 256 levels of gray.  If that byte
   is
    >= "*pGrayThreshold"; a white pixel is written; else a black pixel is
   written. NOTE: the value at "*pGrayThreshold" is read each time the pipe is
   executed (during Init()); the caller change its value and get different
   results without recreating the pipe.  If "pGrayThreshold" is NULL, a default
   value is used.
*/
IL_PRIVATE void _ilAreaScaleBitonal(ilPipe pipe, unsigned long dstWidth,
                                    unsigned long dstHeight,
                                    int *pGrayThreshold, ilPipeInfo *pInfo) {
        register ilBiGrayPrivPtr pPriv;
        ilDstElementData dstData;
#define DEFAULT_GRAY_THRESHOLD 128 /* assumes 256 levels */
        ...
        */
        ...
        /*  Init the filter table in private based on scale factors and 256 gray
           levels. Note that "blackIsZero" is passed as FALSE below, because we
           want to output the same blackIsZero as input.  We output a zero when
           the gray is
