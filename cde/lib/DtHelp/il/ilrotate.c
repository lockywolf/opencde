...
/* =============================================================================================================================
     ...
     /ilc/ilrotate.c : Images Library rotation routines.
        ...
        --------    -----------
   ------------------------------------------------------------------- 07/2/91
   larsson     Initial Coding. 11/4/91    voegelin    moved ilBitReverseTable to
   ildata.c
...
typedef struct {
        ...
        int ilRotateDstheight; /* destination height value saved to avoid strip
                                  sizes  */
        int ilRotateSrcheight; /* src height value saved to avoid strip sizes */
        unsigned long dstMask; /* destination bit mask for 90 degree rotations
                                  (bitonal) */
        int rtype;             /* rotation type used for setting dstMask */
        ilBool Lastbit; /* Flag indicating leftmost bit of current word is
                           written */
...
static ilError ilRotateInit(ilRotatePrivptr pPrivate, ilImageInfo *pSrcImage,
...

{
        ...
        if (pPrivate->rtype == ROT90)
                pPrivate->dstMask = 1 << bitoff;
        else
                pPrivate->dstMask = LEFT_BIT_ON;

        if ((long)pPrivate->dstMask < 0)
                pPrivate->Lastbit = TRUE;
        else
                pPrivate->Lastbit = FALSE;
...
/* =============================================================================================================================
      ...
      ilRotate90BitonalExecute  -  90 degree cw Rotation for images with bit per
   pixel format .

   =============================================================================================================================
 */
static ilError ilRotate90BitonalExecute(register ilExecuteData *pData,
        ...
        psrcline =
            (unsigned long *)(pplane->pPixels) + pData->srcLine * srcnwords;
        ...
        /* Rotate cw 90 degrees  map (x,y) to (-y,x) about its center  [
         * width-y,  x ]  */
        psrc = psrcline;
        ...
        shortwidth = (((srcnwords * WORDPOS) - pData->pSrcImage->width) > 0)
                         ? TRUE
                         : FALSE;

        lastcount = pPriv->illinecount - 1;
        for (y = pPriv->illinecount; y <= (lastcount + *pNLines);
             y++, pPriv->illinecount++) {
        ...
        if (pPriv->illinecount <= pPriv->ilRotateSrcheight)
                *pNLines = 0;
        else
                *pNLines = pPriv->ilRotateDstheight;
...
/* =============================================================================================================================
      ...
      ilRotate180BitonalExecute  -  180 degree Rotation for images with bit per
   pixel format .

   =============================================================================================================================
 */
static ilError ilRotate180BitonalExecute(register ilExecuteData *pData,
                                         unsigned long dstLine,
                                         unsigned long *pNLines) {
        ...
        /* Rotate 180 degrees  map (x,y) to (-x,-y)  about its center [ width-x,
         * height-y ]  */
        ...
        for (y = pPriv->illinecount; y <= (lastcount + *pNLines);
             y++, pPriv->illinecount++) {
                ...
                if (loffset == 0)
                        ...
                        for (x = 0; x < widthbytes; x++) {
                                if (psrcbefore < psrcline)
                                        srcbyte = (*psrc >> roffset);
                                else
                                        srcbyte = (*psrcbefore << loffset) |
                                                  (*psrc >> roffset);
...
/* =============================================================================================================================
      ...
      ilRotate270BitonalExecute  -  270 degree cw Rotation for images with bit
   per pixel format .

   =============================================================================================================================
 */
static ilError ilRotate270BitonalExecute(register ilExecuteData *pData,
                                         unsigned long dstLine,
                                         unsigned long *pNLines) {
        ...
        /* Rotate cw 270 degrees  map (x,y) to (y,-x) about its center  [ y,
         * height-x ]  */
        psrc = psrcline;
...
/* =============================================================================================================================
      ...
      ilRotate903ByteExecute  -  90 degree cw Rotation for images with 24 bits
   per pixel format .

   =============================================================================================================================
 */
static ilError ilRotate903ByteExecute(register ilExecuteData *pData,
                                      unsigned long dstLine,
                                      unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilRotate1803ByteExecute  -  180 degree Rotation for images with 24 bits
   per pixel format .

   =============================================================================================================================
 */
static ilError ilRotate1803ByteExecute(register ilExecuteData *pData,
                                       unsigned long dstLine,
                                       unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilRotate2703ByteExecute  -  270 degree cw Rotation for images with 24 bits
   per pixel format .

   =============================================================================================================================
 */
static ilError ilRotate2703ByteExecute(register ilExecuteData *pData,
                                       unsigned long dstLine,
                                       unsigned long *pNLines) {
        ...
        for (y = pPriv->illinecount; y <= (lastcount + *pNLines);
             y++, pPriv->illinecount++) {
                ...
                for (x = 1; x <= pData->pSrcImage->width; x++) {
                        pdst = pdstline +
                               (dstnbytes * (pPriv->ilRotateDstheight - x)) +
                               xoffset;
                        *pdst++ = *psrc++;
                        *pdst++ = *psrc++;
                        *pdst = *psrc++;
...
/* =============================================================================================================================
      ...
      ilRotate90ByteExecute  -  90 degree cw Rotation for images with byte per
   pixel format .

   =============================================================================================================================
 */
static ilError ilRotate90ByteExecute(register ilExecuteData *pData,
                                     unsigned long dstLine,
                                     unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilRotate180ByteExecute  -  180 degree Rotation for images with byte per
   pixel format .

   =============================================================================================================================
 */
static ilError ilRotate180ByteExecute(register ilExecuteData *pData,
                                      unsigned long dstLine,
                                      unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilRotate270ByteExecute  -  270 degree cw Rotation for images with byte per
   pixel format .

   =============================================================================================================================
 */
static ilError ilRotate270ByteExecute(register ilExecuteData *pData,
                                      unsigned long dstLine,
                                      unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilRotate90   - Add a rotate filter to an existing pipe for 90 degree
   rotations - checking for format types and doing an explicit conversion if
   necessary. Positive factors cause clockwise rotations - negative counter
   clockwise.

   =============================================================================================================================
 */

ilBool ilRotate90(ilPipe pipe, int factor)
...
#define PIPE_FLAGS
        ...
        case 0:
                pipe->context->error =
                    IL_OK; /* 0 Rotation - nothing required. */
                return TRUE;
        case 1:
        ...
        case 3:
                switch (rtype) {
                case ROT90:
                        pPriv = (ilRotatePrivptr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilRotatePriv),
                            IL_ADD_PIPE_HOLD_DST, (ilSrcElementData *)NULL,
                            &dstdata, ilRotateInit, IL_NPF, IL_NPF,
                            ilRotate903ByteExecute, 0);
                        break;
                case ROT180:
                        pPriv = (ilRotatePrivptr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilRotatePriv),
                ...
                if (bitonal) {
                        ...
                        case ROT90:
                                pPriv = (ilRotatePrivptr)ilAddPipeElement(
                                    ...
                                    ilRotateInit, IL_NPF, IL_NPF,
                                    ilRotate90BitonalExecute, 0);
                                break;
                        case ROT180:
                                pPriv = (ilRotatePrivptr)ilAddPipeElement(
                                    pipe, IL_FILTER, sizeof(ilRotatePriv),
