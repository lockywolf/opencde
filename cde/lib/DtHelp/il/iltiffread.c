...
typedef struct {
        ...
        unsigned long
            nLastStripBytes; /* if !pByteCounts: # bytes in last strip */
...
ilReadJPEGTags(ilFilePtr pFile, /* file to read */
        ...
        /*  Use sample/component index as table index; load tables based on
         * sample */
        for (c = 0, pSample = pCompData->sample; c < IL_MAX_SAMPLES;
             c++, pSample++) {
        ...
        for (c = 0; c < nComps; c++, pQOffsets++, pDCOffsets++, pACOffsets++) {
                ...
                /*  Read DC table: first 16 bytes are # of codes for each length
                   (1..16); the codes follow in bytes, "size" = sum of values of
                   first 16 bytes. Malloc space and read in "size" + 16 bytes.
                */
                ...
                if (!IL_SEEK(pFile, *pDCOffsets) ||
                    !IL_READ(pFile, nBytes, pTable))
                        return IL_ERROR_FILE_IO;
...
*/
static ilError ilReadFileInit(ilPtr pPrivate, ilImageInfo *pSrcImage,
        ...
        /*  Data for tags to read: "numbers" is the array of tag ids, "nTags" in
           length.
...
#define ADD_TAG(_index, _tagid)                                                \
        ...
        /*  If this function has already been called, exit.  All of the tag data
           is read in here and kept around, so if pipe is being re-executed
           there is no need to read it again - merely reset current strip ptrs
           to start of data.
        */
        if (pPriv->initDone) {
        ...
        /*  Read the strip tags, which reads in the actual strips.  Offsets
           (file offsets to strips) are required; read the byte counts only if
           the image is compressed. Read ColorMap tag if a palette image; read
           JPEG tags if JPEG compressed.
        */
        ...
        pPriv->pTagAlloc =
            ilReadFileTags((ilFileImage)pFileImage, nTags, numbers, tags, 0);
        if (pFileImage->context->error)
                return pFileImage->context->error;

        pPriv->pDstImagePlane = pDstImage->plane;

        /*  Set pOffsets, ptr to strip offsets (required); determine if
         * short/long. */
        ...
        /*  If image is uncompressed, ignore strip byte counts - can calculate
           them, and it is safer (have seen uncompressed files with bad strip
           byte counts, and perhaps they should be rejected, but we can read
           them, so why not?): set nStripBytes based on dst row bytes,
           nLastStripBytes for last strip. If compress: Strip byte counts must
           exist and have same # items as offsets.
        */
        if (pPriv->compression == IL_UNCOMPRESSED) {
        ...
        /*  ColorMap (palette) must be present if a palette image
           (pPriv->pPalette). Copy TIFF palette (size = 2 ** nBits)  to
           pPriv->pPalette (size = 3 * 256).
        */
        if (pPriv->pPalette) {
                ...
                nPaletteEntries =
                    1 << pPriv->pFileImage->p.format.nBitsPerSample[0];
        ...
        /*  If a JPEG file read JPEG tags and load Q/DC/AC tables into
           pCompData. JPEG table tags must be present and # items = #
           components/samples.
        */
        if (pPriv->compression == IL_JPEG) {
                ...
                if (!pQTag || !pDCTag || !pACTag)
                        return IL_ERROR_FILE_MISSING_TAG;
                if ((pQTag->nItems != nSamples) ||
                    (pQTag->type != IL_TAG_LONG) ||
                    (pDCTag->nItems != nSamples) ||
                    (pDCTag->type != IL_TAG_LONG) ||
                    (pACTag->nItems != nSamples) ||
                    (pACTag->type != IL_TAG_LONG))
                        return IL_ERROR_FILE_MALFORMED_TAG;

                if ((error = ilReadJPEGTags(
                         pPriv->pFile, nSamples, (long *)pQTag->pItems,
                         (long *)pDCTag->pItems, (long *)pACTag->pItems,
                         (ilJPEGData *)pPriv->pCompData)))
                        return error;

                /*  If restartInterval tag present, store value else default to
                 * 0 */
                if ((pTag = tags[restartIndex]) && (pTag->type == IL_TAG_SHORT))
                        ((ilJPEGData *)pPriv->pCompData)->restartInterval =
                            *((short *)pTag->pItems);
                else
                        ((ilJPEGData *)pPriv->pCompData)->restartInterval = 0;

                /*  Handle 2 bugs in older versions of IL (v2.0 and earlier).
                   These versions always wrote in "JIF mode", as a single strip.
                   The read code always read using the JIF tag instead of the
                   strip offsets/byte counts, because the decompression code
                   could only handle JIF data. "count" bug: in versions 2.0 and
                   earlier. The (single) strip byte count was too small; usually
                   by 8 for a single sample image (gray) or 14 for multi-sample
                   (RGB or YCbCr); larger for some baselevels. Then work around
                   the count bug by reading until the EOI marker is found. The
                   data length is then all bytes up but not including the EOI
                   marker. "offset" bug: the strip offset is 3 bytes too small.
                   Fix by adding 3 to the strip offset. The bugs are in files
                   written by IL versions 1.1 (Image Developer's Kit release;
                   first to support JPEG) or 2.0 (Image 2). So if the first "n"
                   chars of the software tag are: "HP IL v1.1"  (n = 10)  count
                   and offset bugs "HP IL v2.0"  (n = 10)  count bug "HP IL
                   v 2.0" (n = 11)  count bug
                */
                {
                        ...
                        if ((pSoftwareTag = tags[softwareIndex])) {
                                ...
                                if ((pSoftwareTag->nItems >= 7) &&
                                    (pName[0] == 'H') && (pName[1] == 'P') &&
                                    (pName[2] == ' ') && (pName[3] == 'I') &&
                                    (pName[4] == 'L') && (pName[5] == ' ') &&
                                    (pName[6] == 'v')) {
                                        if ((pSoftwareTag->nItems >= 10) &&
                                            (pName[7] == '1') &&
                                            (pName[8] == '.') &&
                                            (pName[9] == '1'))
                                                hasCountBug = hasOffsetBug =
                                                    TRUE;
                                                 ...
                                                 (pName[9] == '0'))
                                                hasCountBug = TRUE;
                                        else if ((pSoftwareTag->nItems >= 11) &&
                                                 (pName[7] == ' ') &&
                                                 (pName[8] == '2') &&
                                                 (pName[9] == '.') &&
                                                 (pName[10] == '0'))
...
#define JPEGM_EOI 0xD9        /* End Of Image */
                                ...
                                if (!IL_SEEK(pPriv->pFile,
                                             *pPriv->pOffsets + nStripBytes))
                                        return (IL_ERROR_COMPRESSED_DATA);
                                do { /* get a marker */
                                        do {
                                                if (!IL_READ(pPriv->pFile, 1,
                                                             &value))
                                                        return IL_ERROR_COMPRESSED_DATA;
                                                nStripBytes++;
                                ...
                                /*  nStripBytes now includes marker; subtract 2
                                 * to remove marker */
                                nStripBytes -= 2;
...
/*  Destroy() function for ilReadFileImage().  Calls ilDestroyObject() on the
    ...
    from being destroyed until the pipe element is destroyed.
        Free the palette space, compression data and tag malloc space if
   present.
*/
static ilError ilReadFileDestroy(ilPtr pPrivate) {
...
*/
static ilError ilReadFileExecute(ilExecuteData *pData, long dstLine,
                                 long *pNLines) {
        ...
        /*  Get "offset", position of strip in file, and "nBytes" to read: from
           strip byte counts if present (must be if compressed), else use
           calculated values.
        */
        ...
        if (pPriv->shortOffsets) {
                ...
                offset = *pPriv->pOffsets++;
        if (pPriv->pByteCounts) {
                if (pPriv->shortByteCounts) {
                        nBytes = *((unsigned short *)pPriv->pByteCounts);
                        pPriv->pByteCounts =
                            (CARD32 *)((ilPtr)pPriv->pByteCounts + 2);
                } else
                        nBytes = *pPriv->pByteCounts++;
        } else
                nBytes = (pPriv->nStripsLeft <= 0) ? pPriv->nLastStripBytes
                                                   : pPriv->nStripBytes;

        /*  If reading a compressed image, read into "pPixels + srcOffset"
           (ignore dstLine); make sure room for "nBytes"; (re)alloc buffer if
           not room. Pass "nBytes" written on to next pipe element.
        */
        ...
        if (pPriv->compression != IL_UNCOMPRESSED) {
                ...
                *pData->compressed.pNBytesWritten = nBytes;
                ...
                pDst = pDstImagePlane->pPixels +
                       dstLine * pDstImagePlane->nBytesPerRow;
        ...
        /*  Set # of lines written - strip height unless last strip (return
         * flag). */
...
*/
ilBool ilReadFileImage(ilPipe pipe, ilFileImage fileImage, ilRect *pSrcRect,
        ...
        /*  If a palette image, allocate space for it (must pass ptr to it when
           adding pipe element), but don't read it until Init() called.
        */
        if (pFileImage->p.des.type == IL_PALETTE) {
                pPalette = (unsigned short *)IL_MALLOC(sizeof(unsigned short) *
                                                       3 * 256);
                if (!pPalette)
        ...
        pPriv = (ilReadFilePrivPtr)ilAddPipeElement(
            pipe, IL_PRODUCER, sizeof(ilReadFilePrivRec), 0,
            (ilSrcElementData *)NULL, &dstData, ilReadFileInit, IL_NPF,
            ilReadFileDestroy, ilReadFileExecute, NULL, 0);
        if (!pPriv) {
                if (pPalette)
                        IL_FREE(pPalette);
                if (pCompData)
        ...
        /*  Init private. Inc refCount for this file - dec'd in
         * ilReadFileDestroy(). */
