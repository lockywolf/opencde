...
ILJPG_PRIVATE
int _iljpgZigzagTable[64] = {0,  1,  5,  6,  14, 15, 27, 28, 2,  4,  7,  13, 16,
...
#define VALID_FACTOR(_f) (((_f) == 1) || ((_f) == 2) || ((_f) == 4))
        ...
        if ((pData->nComps <= 0) || (pData->nComps > ILJPG_MAX_COMPS) ||
            (pData->nComps > ILJPG_MAX_COMPS) ||
            !VALID_FACTOR(pData->maxHoriFactor) ||
            !VALID_FACTOR(pData->maxVertFactor) || (pData->width <= 0) ||
            (pData->height <= 0))
                return FALSE;

        for (comp = 0, pCompData = pData->comp; comp < pData->nComps;
             comp++, pCompData++) {
