...
/* =============================================================================================================================
     ...
     /ilc/ilhplrotate.c : Images Library arbitrary rotation routines.   using
   HP-LABs image rotation code
   =============================================================================================================================
 */
...
static ilError ilRotateInit2(ilRotatePrivptr p, ilImageInfo *pSrcImage,
...

{
        ...
        if (p->restart) {
                data_in = _il_Rotate_Image_Begin(
                    p->in_w, p->in_h, p->in_nSPP, p->mode, p->degrees, p->bg_r,
                    p->bg_g, p->bg_b, &out_w, &out_h);
...
/* =============================================================================================================================
   ...
   =============================================================================================================================
 */
static ilError ilRotateCleanup(register ilRotatePrivptr pPriv,
                               ilBool aborting) {
        free(pPriv->buf);
...
/* =============================================================================================================================
   ...
   =============================================================================================================================
 */
static ilError ilRotateExecute(register ilExecuteData *pData,
                               unsigned long dstLine, unsigned long *pNLines) {
        ...
        for (nlines = *pNLines; nlines > 0; nlines--) {
                ...
                for (nrows = _il_Rotate_Send_Row(&pPriv->data_in); nrows > 0;
                     --nrows) {
                        _il_Rotate_Get_Row(pPriv->buf);
                        bcopy((char *)pPriv->buf, (char *)pdstline,
                              (int)dstnbytes);
                        pdstline += dstnbytes;
                        nlinesout++;
                }
...
/* =============================================================================================================================
      ...
      ilRotate - Add a rotate filter to an existing pipe for 0 - 90 degree
...
ilBool ilRotate(ilPipe pipe, int degrees, int interpMode,
...
#define PIPE_FLAGS
         ...
         */
        if (!ilRotate90(pipe, (degrees + 89) / 90))
                return FALSE;
        ilGetPipeInfo(pipe, FALSE, &info, (ilImageDes *)NULL,
                      (ilImageFormat *)NULL);
        ...
        data_in = _il_Rotate_Image_Begin(
            (short)info.width, (short)info.height,
            (short)imdes.nSamplesPerPixel, (short)interpMode, (float)degrees,
            (unsigned char)bg_color[0], (unsigned char)bg_color[1],
            (unsigned char)bg_color[2], &out_w, &out_h);
        ...
        pPriv = (ilRotatePrivptr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilRotatePriv), IL_ADD_PIPE_HOLD_DST,
            (ilSrcElementData *)NULL, &dstdata, ilRotateInit2, ilRotateCleanup,
            IL_NPF, ilRotateExecute, 0);
