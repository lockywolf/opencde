...
/*  Private structure for all throttle elements.
    "Init()" are fields that Init() must init, "Add()" when pipe element is
   added, else temp values used during pipe execution.
*/
typedef struct {
 ...
 */
static ilError ilThrottleInit(ilThrottlePrivPtr pPriv, ilImageInfo *pSrcImage,
...
/*  Execute() for a filter inserted by ilAddThrottlePipeElement() to be the
    ...
    error on last one.
*/
static ilError ilProducerThrottleExecute(register ilExecuteData *pData,
        ...
        /*  Set the next elements srcLine to our current line.  Bump srcLine by
            stripHeight (# lines just "written"), but if that is to/past end of
           image, return last strip error, else "ok", to do more strips.
        */
...
*/
static ilError ilFeedProducerThrottleExecute(register ilExecuteData *pData,
                                             long dstLine, /* ignored */
                                             long *pNLines) {
        register ilThrottlePrivPtr pPriv;
        long nLines;

        /*  Point next element to the feed data, set into pData by
           ilExecutePipe(): srcLine (for uncompressed) and srcOffset,
           nBytesToRead for compressed data. If all lines received (count in
           nLinesReceived), return "last strip", else ok.
        */
        pPriv = (ilThrottlePrivPtr)pData->pPrivate;
        *pData->pNextSrcLine = pData->srcLine;
 ...
 */
static ilError ilNonConstantThrottleExecute(register ilExecuteData *pData,
                                            long dstLine, /* ignored */
                                            long *pNLines) {
        register ilThrottlePrivPtr pPriv;

        /*  Not from a producer, but !constantStrip; easy: caller just wants
           strips of size "stripHeight" or smaller.  For each strip written to
           this throttle, break it into "stripHeight" or smaller pieces;
           completely process each strip. Move srcLine thru image; always have
           next element read from srcLine. If not beingRecalled, then previous
           filter actually wrote something to us: store how much was written
           into nBufferLines; exit if nothing.
        */
        *pData->pNextSrcLine = pData->srcLine;
        pPriv = (ilThrottlePrivPtr)pData->pPrivate;
        ...
        /*  Write "pPriv->stripHeight" sized strips.  If that or less in buffer,
           write it and cancel the recall if on; reset srcLine to be beginning
           of buffer.
        */
        ...
        /*  More than pPriv->stripHeight lines in buffer: write stripHeight
           lines, bump srcLine by that amount, and if not beingRecalled, make it
           so.
        */
        *pNLines = pPriv->stripHeight;
        pData->srcLine += pPriv->stripHeight;
 ...
 */
static ilError ilConstantThrottleExecute(register ilExecuteData *pData,
                                         long dstLine, /* ignored */
                                         long *pNLines) {
        register ilThrottlePrivPtr pPriv;
        ...
        /*  If have (at least) a strip's worth in buffer, "write" it and request
           a recall.  If beingRecalled (i.e. this code was done on the previous
           call), bump srcLine by stripHeight (# lines done before), else set
           srcLine to zero to start at the beginning of the buffer.
        */
        if (pPriv->nBufferLines >= pPriv->stripHeight) {
                ...
                /*  Dont have a strip's worth in buffer.  If beingRecalled, then
                   some lines have been read out from beginning of buffer by
                   next element.  Copy the the lines above that point down to
                   the beginning of the buffer, and set srcLine to the line past
                   them. If not being recalled, bump srcLine by # lines received
                   this strip.
                */
                if (pPriv->beingRecalled) {
                        int nPlanes;
                        ...
                        if (pPriv->nBufferLines > 0) {
                                nPlanes =
                                    (pData->pSrcImage->pFormat->sampleOrder ==
                                     IL_SAMPLE_PIXELS)
                                        ? 1
                                        : pDes->nSamplesPerPixel;
                                pPlane = pData->pSrcImage->plane;

                                /*  If a YCbCr image, copy subsampled buffers
                                 * properly. */
                                if (pDes->type == IL_YCBCR) {
                                        ...
                                        while (nPlanes-- > 0) {
                                                vertShift = _ilSubsampleShift
                                                    [pSample->subsampleVert];
                                                pSample++;
                                                bcopy(
                                                    (char
                                                         *)(pPlane->pPixels +
                ...
                /*  Dont have strip's worth, need more lines: if all lines read,
                   write a short final strip, otherwise write 0 lines and wait
                   for more lines to come in, written to pData->srcLine.
                */
                if (pPriv->nLinesReceived >= pPriv->imageHeight) {
                        nLinesToWrite =
                            pPriv->nBufferLines; /* write last (short) strip */
                        pPriv->nBufferLines = 0;
                        pData->srcLine = 0;
                } else
                        nLinesToWrite = 0;

                /*  Cancel recall if one in effect, mark that we are not in a
                 * recall. */
                returnCode =
                    (pPriv->beingRecalled) ? IL_ERROR_ELEMENT_COMPLETE : IL_OK;
                pPriv->beingRecalled = FALSE;
        } /* END need more lines */

        /*  Tell next element to read "nLinesWrite" starting at
         * "pData->srcLine". */
...
*/
static ilError ilCopyImageExecute(register ilExecuteData *pData, long dstLine,
                                  long *pNLines) {
        ...
        /*  If a YCbCr image point pSample to first plane's sample info; else
         * null */
        if (pSrcImage->pDes->type == IL_YCBCR)
                pSample = pSrcImage->pDes->typeInfo.YCbCr.sample;
        else
                pSample = (ilYCbCrSampleInfo *)NULL;

        /*  Loop thru and copy, plane by plane: 1 plane if pixel order.  If
           YCbCr image (pSample) divide nLines and src/dstStart by subsample
           factor (1,2 or 4).
        */
        nPlanes = (pSrcImage->pFormat->sampleOrder == IL_SAMPLE_PIXELS)
                      ? 1
                      : pSrcImage->pDes->nSamplesPerPixel;
        while (nPlanes-- > 0) {
                lineCount = nLines;
                ...
                if (nBytes <= 0)
                        continue; /* no bytes (? perhaps subsampled ?), next
                                     plane */
...
*/
IL_PRIVATE ilBool _ilInsertCopyFilter(ilPipe pipe, long height) {
        ilSrcElementData srcData;

        /*  Add a filter which copies src to dst.  No private needed; the
           Execute() function ilCopyImageExecute() does all necessary setup.
           Specify src stripHeight of whole image, to do whole copy in one
           strip.
        */
        ...
        if (!ilAddPipeElement(pipe, IL_FILTER, 0, 0, &srcData,
                              (ilDstElementData *)NULL, IL_NPF, IL_NPF, IL_NPF,
                              ilCopyImageExecute, NULL, 0))
                return FALSE;
...
/*  Called by ilAddPipeElement to add a "throttle" pipe element to feed
    strips of height "stripHeight" or zero (0) to the next element.
        For uncompressed images only: if "constantStrip" is true, the strips out
   of this filter must all be "stripHeight" or zero lines high, except for the
   last strip;  if false, the height of each strip must be <= "stripHeight". If
   "producerCode" is not IL_PIPE_NOT_IMAGE, then the throttle is a logical
   producer: the pipe began with an ilReadImage() or ilFeedFromImage() and must
   signal IL_ERROR_LAST_STRIP; else the throttle is not the producer. This flag
   is used in place of "tempImage" in pipe info which IS INCORRECT -
...
*/
IL_PRIVATE ilBool _ilAddThrottlePipeElement(ilPipe pipe, long stripHeight,
        ...
        switch (producerCode) {
                ...
                /*  If reading directly from whole image, always output constant
                 * strips. */
        case IL_PIPE_IMAGE:
                pPriv = (ilThrottlePrivPtr)ilAddPipeElement(
                    pipe, IL_FILTER, sizeof(ilThrottlePrivRec),
                    IL_ADD_PIPE_NO_DST, &srcData, (ilDstElementData *)NULL,
                    ilThrottleInit, IL_NPF, IL_NPF, ilProducerThrottleExecute,
                    NULL, 0);
                constantStrip = TRUE;
                break;

                /*  If feeding pipe from an image, first add producer filter to
                   "write" the number of lines passed to ilFeedPipe().  If
                   stripHeight written by feed (info.stripHeight) is < requested
                   strip height and !constantStrip, or if equal and feed gives
                   constant strips, done; else insert a filter to copy the feed
                   image to output buffer (else throttle will never get a
                   buffer), fall thru and treat same as if pipe not started with
                   a read from an image.
                */
        case IL_PIPE_FEED_IMAGE:
                pPriv = (ilThrottlePrivPtr)ilAddPipeElement(
                    pipe, IL_FILTER, sizeof(ilThrottlePrivRec),
                    IL_ADD_PIPE_NO_DST, &srcData, (ilDstElementData *)NULL,
                    ilThrottleInit, IL_NPF, IL_NPF,
                ...
                /*  If not constant strips are not required, simple, else
                   tougher: Must buffer those passed to us, possibly requesting
                   a recall when we have more than one strip's worth in the
                   buffer.  Need space in buffer for worst case: we have 1 line
                   less than needed for (requested) "stripHeight", and then
                   previous element then writes "info.stripHeight" lines.
                */
        case IL_PIPE_NOT_IMAGE:
                if (!constantStrip) {
                        pPriv = (ilThrottlePrivPtr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilThrottlePrivRec),
                            IL_ADD_PIPE_NO_DST, &srcData,
                            (ilDstElementData *)NULL, ilThrottleInit, IL_NPF,
                            IL_NPF, ilNonConstantThrottleExecute, NULL, 0);
                } else {
                        srcData.minBufferHeight =
                            stripHeight + info.stripHeight - 1;
                        ...
                        pPriv = (ilThrottlePrivPtr)ilAddPipeElement(
                            ...
                            IL_ADD_PIPE_NO_DST, &srcData,
                            (ilDstElementData *)NULL, ilThrottleInit, IL_NPF,
                            IL_NPF, ilConstantThrottleExecute, NULL, 0);
                }
                break;
        }
