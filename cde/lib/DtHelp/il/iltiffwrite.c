...
/*  Tag info summary, private to ilWriteFileImage().  Encapsulates the user and
   std tag info.  Contains ptrs to data which is created when the pipe element
   is added and freed when pipe emptied (Destroy() function). "pTagOffsets"
   points to an array of offsets, in the same order as the tags at
    "pTagWriteData" + 2 (bytes).  For each tag, if the offset is zero (0), the
   tag data is in the tag, already byte-flipped.  If non-zero, it is the offset
   of the tag data from pTagWriteData.  The file offset where the data at
   pTagWriteData will be written must be added to this offset and inserted into
   the tag byte-flipped. See ilMergeWriteTags() for details.
*/
typedef struct {
        ...
        unsigned short
            *pJPEGRestart;        /* (JPEG only): restart interval in memory */
 ...
 */
static ilError ilWriteFileInit(register ilWriteFilePrivPtr pPriv,
                               ilImageInfo *pSrcImage, ilImageInfo *pDstImage) {
 ...
 */
static ilError ilWriteFileCleanup(register ilWriteFilePrivPtr pPriv,
                                  ilBool aborting) {
        ...
        /*  If aborting pipe, just exit.  The net effect is that if
            some image strips are written and the pipe aborted before
           completion, the image strips are there but not pointed to by
           anything.
        */
        if (aborting)
                return IL_OK; /* EXIT */

        /*  Make sure the proper # of lines and strips have been written or
         * error. */
        ...
        /*  Read "offset to next" that will be pointed to this image; set as
           "offset to next" for this image (IFDOffset), following tag info; see
           ilMergeWriteTags().
        */
        if (!IL_SEEK(pFile, pPriv->tailPtrOffset) ||
            !IL_READ(pFile, 4, nextBytes))
                return IL_ERROR_FILE_IO;
        pBytes = pPriv->tagInfo.pTagWriteData +
                 pPriv->tagInfo.tagCount * IL_TAG_SIZE + 2;
        *pBytes++ = nextBytes[0];
        *pBytes++ = nextBytes[1];
        *pBytes++ = nextBytes[2];
        *pBytes++ = nextBytes[3];

        /*  Position to EOF; save file position in "position".  Update tags
           whose values did not fit in the tag; they have a non-zero value in
           the array @pTagOffsets; the value is the offset from the beginning of
           the data to be written to the tag data.
        */
        if (fseek(pFile->stream, 0,
                  2)) /* seek to EOF (2); return != 0 is error */
                return IL_ERROR_FILE_IO;
        position = ftell(pFile->stream) - pFile->offset;

        pTagOffsets = pPriv->tagInfo.pTagOffsets;
        pTag = pPriv->tagInfo.pTagWriteData + 2;
        for (i = 0; i < pPriv->tagInfo.tagCount;
             i++, pTagOffsets++, pTag += IL_TAG_SIZE)
                if (*pTagOffsets) {
                        l = position + *pTagOffsets;
                        pBytes =
                            pTag + 8; /* point to offset field of the tag */
                        IL_WRITE_FLIP_LONG(pPriv->bigEndian, l, pBytes)
                }

        /*  Write out the tag data to add the image to the file.  Patch location
           at "tailPtrOffset" in file to point to this image to link image into
           list.
        */
        if (fwrite((char *)pPriv->tagInfo.pTagWriteData,
                   pPriv->tagInfo.tagWriteSize, 1, pFile->stream) != 1)
                return IL_ERROR_FILE_IO;
        pBytes = fourBytes;
        /* compatibility problem with long or unsigned long data fields */
        IL_WRITE_FLIP_LONG(pPriv->bigEndian, (CARD32)position, pBytes)
        if (!IL_SEEK(pFile, pPriv->tailPtrOffset) ||
            !IL_WRITE(pFile, 4, fourBytes))
                return IL_ERROR_FILE_IO;
        fflush(pFile->stream); /* flush all buffers; image now in file */

        /*  If this image is now last in file (its next offset is null), update
           file's offset to last image's offset to next, and copy of same in
           private.
        */
        if (!nextBytes[0] && !nextBytes[1] && !nextBytes[2] && !nextBytes[3]) {
                pFile->IFDTailPtrOffset =
                    position + pPriv->tagInfo.tagCount * IL_TAG_SIZE + 2;
                pPriv->tailPtrOffset = pFile->IFDTailPtrOffset;
        }

        /*  Mark file image list as invalid, so this image will show up next
         * time. */
 ...
 */
static ilError ilWriteFileDestroy(ilWriteFilePrivPtr pPriv) {
...
/*  Called by Execute() function before writing first strip to copy the palette
    into the tag memory buffer, so it will be written out by
   ilWriteFileCleanup().
*/
static void ilCopyPalette(register ilWriteFilePrivPtr pPriv,
        ...
        /*  Copy palette into the in-memory tag area, byte-flipped.
            It will be written out along with all the tags and tag data during
           Cleanup(). TIFF palettes are the same as IL: all the red, then green,
           then blue values, each a short, except in TIFF each is "2 ** nBits"
           entries (pPriv->nPaletteColors) in length; in the IL they are always
           256 entries.
        */
        ...
        if (pDstPalette) {
                ...
                for (i = 0; i < pPriv->tagInfo.nPaletteColors;
                     i++, pSrcPalette++, pDstPalette++) {
 ...
 */
static ilError ilWriteFileExecute(ilExecuteData *pData, long dstLine,
                                  long *pNLines) {
        ...
        if ((pPriv->nStripsSoFar > pPriv->nStrips) ||
            (pPriv->nLinesSoFar > pPriv->fileImage.p.height) ||
            ((pPriv->nLinesSoFar !=
              pPriv->fileImage.p.height) /* not last strip */
             && (*pNLines != pPriv->fileImage.p.stripHeight)))
                return IL_ERROR_MALFORMED_FILE_WRITE;

        /*  Write one strip of data to the end of the file, after saving
         * "position". */
        if (fseek(pFile->stream, 0,
                  2)) /* seek to EOF (2); return != 0 is error */
        ...
        if (pPriv->compression == IL_UNCOMPRESSED) {
                ...
                pBytes = pData->pSrcImage->plane[0].pPixels +
                         pData->srcLine * rowBytes;
        } else {
                nBytes = pData->compressed.nBytesToRead;
                pBytes = pData->pSrcImage->plane[0].pPixels +
                         pData->compressed.srcOffset;
        }
...
*/
static ilError ilWriteJIFData(register ilWriteFilePrivPtr pPriv,
                              ilFilePtr pFile, ilImageInfo *pImage) {
        ...
        /*  Init streamRec so output is written into malloc'd memory, and encode
            a JIF image into that memory.  For restartInterval value: if a
           single strip use value from raw data (== 0 if no restart markers),
           otherwise calculate it from strip size.  If multi-strip and non-zero
           restartMarker in raw data a JIF image cannot be created - see below.
        */
        pCompData = (ilJPEGData *)pImage->pCompData;
        _ilJPEGDataIn(pImage->pDes, pPriv->fileImage.p.width,
                      pPriv->fileImage.p.height, &data);
        ...
        if (pPriv->nStrips != 1) { /* else data.restartInterval = raw already */
                ...
                data.restartInterval = /* # of mcus across * # down */
                    ...
                    ((pPriv->fileImage.p.stripHeight + mcuHeight - 1) /
                     mcuHeight);
        }

        streamRec.pBuffer = streamRec.pDst = streamRec.pPastEndBuffer =
            (ilPtr)NULL;
        ...
        /*  Update in-memory values for tags that point to JIF data and to
            ...
            then there must be only one strip in image or it cannot be encoded
           as as a JIF image, because restart markers need to be written between
           the strips and would not mesh with restart markers already in the
           data; so in that case write JIF pointer marker = 0 => not JIF. The
           resulting file will be able to be read by IL2.1 and later but not by
        ...
        */
        ...
        IL_WRITE_FLIP_SHORT(pPriv->bigEndian, (short)pCompData->restartInterval,
                            pBytes)
        ...
        /*  Write offsets to Q tables for each component: table offset for each
            table, indexed by component table index, plus file to offset to JIF
           hdr.
        */
 ...
 */
static ilError ilWriteJPEGExecute(ilExecuteData *pData, long dstLine,
                                  long *pNLines) {
        register ilWriteFilePrivPtr pPriv;
        ilFilePtr pFile;
        ilError error;
        ...
        /*  The overall approach is to write a JIF image into the file plus
           strips. The data written will be the JIF "header", followed by strips
           of JPEG data, separated by restart markers.  Each strip will pointed
           to by the normal TIFF strip offset/byte counts.  The resulting data
           can then be read either as a JIF image (using the
           JPEGInterchangeFormat/Length tags - how IL2.0 and earlier versions
           read it) or as a "raw" image using the other JPEG tags (how later
           versions read it).
        */
        pPriv = (ilWriteFilePrivPtr)pData->pPrivate;
        pFile = pPriv->pFile;
        ...
        */
        ...
        /*  Write one strip of data to end of the TIFF file, after saving
         * "position". */
        if (fseek(pFile->stream, 0,
                  2)) /* seek to EOF (2); return != 0 is error */
                return IL_ERROR_FILE_IO;
        ...
        nBytes = pData->compressed.nBytesToRead;
        pBytes =
            pData->pSrcImage->plane[0].pPixels + pData->compressed.srcOffset;
        if (fwrite((char *)pBytes, nBytes, 1, pFile->stream) != 1)
                return IL_ERROR_FILE_IO;
        pPriv->nBytesSoFar +=
            nBytes + 2; /* # bytes in strip + 2 for marker below */
        ...
        */
        ...
        /*  If last strip, add an EOI marker to end and store JIF length tag
           value; otherwise add a restart marker, with strip count modulo 8 in
           it.
        */
        marker[0] = 0xff;
        if (pPriv->nStripsSoFar >= pPriv->nStrips) {
                ...
                /* compatibility problem with long or unsigned long data fields
                 */
                IL_WRITE_FLIP_LONG(pPriv->bigEndian, (INT32)pPriv->nBytesSoFar,
                                   pBytes)
        } else
                marker[1] = ILJPGM_RST0 | ((pPriv->nStripsSoFar - 1) & 7);
...
*/
static ilError ilMergeWriteTags(ilBool bigEndian, /* byte order to be written */
                                ...
                                ilTagInfoPtr pInfo)   /* returned tag info */
{
        ...
        /*  Allocate and null an array of ptrs to tags, size = # user + std
           tags, + 1 so that there is at least one guaranteed null tag ptr, for
           "end of array".
        */
        tagArraySize = nStdTags + nUserTags + 1;
        ppTagsInit =
            (ilFileTag **)IL_MALLOC(tagArraySize * sizeof(ilFileTag **));
        if (!ppTagsInit)
        ...
        /*  Fill in array with ptrs to tags, sorted in ascending numeric order
           (required by TIFF!), with duplicates eliminated.  Do the user tags,
           then the std tags, so that the std tag values overwrite the user if
           duplicates present. Sort by insertion sort: find tag with number >=
           this tag's number, or end of list (null ptr).  If greater, shift all
           values up one; then insert this tag
        */
        ...
        /*  Set "nTags" to the resulting number of tags, and "tagDataSize" to
           the size of tag data which does not fit in the tag itself (rounded to
           mult of 2).
        */
        for (tagDataSize = 0, nTags = 0, ppTag = ppTagsInit; *ppTag;
             nTags++, ppTag++) {
                pTag = *ppTag;
                tagType = pTag->type;
                ...
                if (pTag->nItems > _ilTagTypeItemsThatFit[tagType])
                        tagDataSize +=
                            (_ilTagTypeItemSizes[tagType] * pTag->nItems + 1) &
                            ~1;
        }

        /*  Alloc room on heap for: an array of offsets to tag data for each
           tag, or 0 if tag data fits in tag; the # of tags (2 bytes); the tags
           themselves; the next IFD offset (4 bytes); and the tag data that does
           not fit in the tags. pInfo->pAlloc will point to it all:
                    -----------------------------
                    | offset to data for each   |   <- pTagOffsets (not written)
                    | tag or 0 if data in tag   |  (rest is written to TIFF, and
           is |                           |   "tagWriteSize" bytes in size
                    -----------------------------
        ...
        */
        ...
        pInfo->pAlloc =
            (ilPtr)IL_MALLOC_ZERO(nTags * sizeof(INT32) + pInfo->tagWriteSize);
        if (!pInfo->pAlloc) {
                IL_FREE(ppTagsInit);
                return IL_ERROR_MALLOC;
        }
        ...
        /*  Write the # of tags byte-flipped, point "pFileTags" to tag data
           write area. For byte/ascii data: "nItems" is length of string,
           including trailing null for ascii.  Round up to even byte boundary
           (TIFF requirement).
        */
        pFileTags = (ilPtr)pInfo->pTagWriteData;
        IL_WRITE_FLIP_SHORT(bigEndian, (unsigned short)nTags, pFileTags)

        for (ppTag = ppTagsInit, pTagOffsets = pInfo->pTagOffsets; *ppTag;
             ppTag++) {
                ilPtr pWrite;
                ...
                /*  Copy the data at "*pTag->pItems" to "pWrite", with
                   byte-flip. pWrite points to 4 byte area in tag if items fit,
                   else to area in pTagData. NOTE: handle strip offsets/byte
                   counts specially: pItems is ignored.
                */
                ...
                if (tagNumber == IL_TAG_STRIP_OFFSETS)
                        ...
                        case IL_TAG_BYTE:
                        case IL_TAG_ASCII:
                                bcopy((char *)pTag->pItems, (char *)pWrite,
                                      pTag->nItems);
                                break;
                        case IL_TAG_SHORT:
                                for (i = 0,
                                    pShort = (unsigned short *)pTag->pItems;
                                     i < pTag->nItems; i++, pShort++)
                                        IL_WRITE_FLIP_SHORT(bigEndian, *pShort,
                                                            pWrite)
                                break;
                        case IL_TAG_LONG:
                                /* compatibility problem with long or unsigned
                                 * long data fields */
                                for (i = 0, pLong = (CARD32 *)pTag->pItems;
                                     i < pTag->nItems; i++, pLong++)
                                        IL_WRITE_FLIP_LONG(bigEndian, *pLong,
                                                           pWrite)
                                break;
...
*/
static ilBool ilFormatPipeImage(ilPipe pipe, unsigned int compression,
                                ilPtr pCompData, long *pStripHeight,
...
#define IL_WRITE_TIFF_STRIP_SIZE (16 * 1024) /* write 16KB strips as default   \
        ...
        /*  If compression is "use current compression" (which makes it easier
           for caller to copy images), then set compression to current pipe
           compression, and point pCompData to compression-specific current pipe
           data.
        */
        ...
        /*  Compare the levels/sample to bits/sample, and scale up if levels !=
           2**bits. This is because the IL separates bits and levels, but TIFF
           assumes # levels = 2**bits.  A 16 level 8 bit gray image must be
           level-scaled up to 256 or else the reader will assume 256 level and
           display it too dark. However, bitonal must be bit/pixel, and for
           palette, levels is ignored. NOTE: the right thing to do is to convert
           the format (e.g. change 16 level 8 bit gray to 4 bit gray), but
           ilConvert() does not handle this yet!
        */
        switch (pDes->type) {
        case IL_BITONAL:
                if (pFormat->nBitsPerSample[0] != 1) {
                        pFormat->nBitsPerSample[0] = 1;
                        if (!ilConvert(pipe, (ilImageDes *)NULL, pFormat, 0,
                                       NULL))
                                return FALSE;
                }
                break;
        ...
        default: /* continuous tone: match levels to bits */
                for (i = 0, mustConvert = FALSE; i < pDes->nSamplesPerPixel;
                     i++) {
        ...
        /*  Convert to pixel order and reget pipe info if multi-sample and
           planar. However, leave planar if JPEG compression - keep YCbCr
           subsampled if so.
        */
        if (pDes->nSamplesPerPixel == 1)
                pFormat->sampleOrder = IL_SAMPLE_PIXELS;
        else if ((pFormat->sampleOrder != IL_SAMPLE_PIXELS) &&
                 (compression != IL_JPEG)) {
                pFormat->sampleOrder = IL_SAMPLE_PIXELS;
        ...
        /*  If writing uncompressed, force rowBitAlign of 8 (required by TIFF);
            ...
            If stripHeight is 0 use default TIFF write size, based on
           uncompressed size.
        */
        stripHeight = *pStripHeight;

        if (compression == IL_UNCOMPRESSED) {
                ilGetPipeInfo(pipe, TRUE, pInfo, pDes,
                              pFormat); /* force decompression */
                ...
                if (stripHeight <= 0) {
                        ...
                        ilGetBytesPerRow(pDes, pFormat, pInfo->width,
                                         bytesPerRow);
                ...
                *pStripHeight =
                    stripHeight; /* write with desired strip height */
        } else {
                /*  If compressing JPEG, force "raw" format; get caller's Q
                   (dflt = 0). Compress with desired strip height, unless
                   already JPEG compressed, in which case use 0 strip height:
                   accept current strip height.
                */
                if (compression == IL_JPEG) {
                        jpegEncode.mustbezero = IL_JPEGM_RAW;
                        if (pCompData)
                                jpegEncode.Q =
                                    ((ilJPEGEncodeControl *)pCompData)->Q;
                        else
                                jpegEncode.Q = 0;
                        pCompData = (ilPtr)&jpegEncode;
                        if (pDes->compression == IL_JPEG)
                                stripHeight =
                                    0; /* accept current strip height */
                }
                if (!ilCompress(pipe, compression, pCompData, stripHeight, 0))
                        return FALSE;
                ilGetPipeInfo(pipe, FALSE, pInfo, pDes, pFormat);
                *pStripHeight = pInfo->stripHeight; /* strip height determined
                                                       by compression */
        }
...
IL_EXTERN char
    _ilVersionString[]; /* in /ilc/ilversion.c ; written for "software" tag */

ilBool ilWriteFileImage(ilPipe pipe, ilFile file, unsigned int method,
                        ilFileImage fileImage, unsigned int compression,
                        ilPtr pCompData, long stripHeight, long xRes, long yRes,
                        short page, short nPages, int nUserTags,
                        ilFileTag *pUserTags, unsigned long mustBeZero) {
...
#define TW_ADD_TAG(_number, _type, _nItems, _pItems)                           \
        ...
        /*  NOTE: below data types (sizes) FIXED! - tied to TIFF type
         * (short/long) */
...
#define VERSION_LENGTH 18 /* length includes trailing null */
        ...
        /*  Force the file image list to be formed if not already; exit on
         * error. */
        ...
        case IL_WRITE_MAIN: /* no parent, add as last image */
                ...
                /*  Transparency mask: error if no parent, or parent already has
                   or is a mask, or not bitonal image.
                */
        case IL_WRITE_MASK:
                if (!fileImage /* no parent */
                    ...
                    || (format.nBitsPerSample[0] != 1))
                        return ilDeclarePipeInvalid(pipe,
                                                    IL_ERROR_FILE_WRITE_METHOD);
                writeMethod = maskImage;
                newSubFileType = 4; /* TIFF transparency mask */
                break;

                /*  Child image: must have non-mask parent fileImage with same
                   file or error. Point to last image associated with parent:
                   find last child (its mask if it has one), so that child
                   images are in order written, and masks "bind" closest to
                   image that they are a mask of.
                */
        case IL_WRITE_CHILD:
                ...
                if (fileImage->pMask)
                        fileImage =
                            fileImage->pMask; /* write after the mask image */
                break;
        ...
        /*  Reformat and compress the pipe image as necessary.  Return the
           stripHeight to use, and the new pipe info/des/format.
        */
        ...
        /*  Write the 1st part of the IL version string as the "software" tag,
           starting 4 chars into string (past '@(#)') - remember length includes
           trailing null!
        */
        ...
        TW_ADD_TAG(IL_TAG_BITS_PER_SAMPLE, IL_TAG_SHORT, des.nSamplesPerPixel,
                   bitsPerSample)

        /*  Determine TIFF compression tag based on pipe image compression and
           possibly des.compInfo.  Ensure pCompData null if not used.
        */
        ...
        case IL_UNCOMPRESSED:
                ...
                /*  Group 3: use des.compInfo.g3.flags.
                    If 0, write TIFF G3 (code 2), else code 3: must have
                   EOL_MARKERS; init group3Options to be written below, and
                   write FillOrder tag (required for Class F; not recommended
                   for other uses in TIFF). Add group 3 options tag only if
                   "true" (not TIFF) G3 (TIFFcompression == 3)
                */
        case IL_G3: {
                ...
                if (userOptions == 0)
                        ...
                        if (userOptions & ~0x3f) /* unhandled bits set: error */
                                return ilDeclarePipeInvalid(
                                    pipe, IL_ERROR_COMPRESSION);
                        group3Options = 0;
                        fillOrder = (userOptions & IL_G3M_LSB_FIRST) ? 2 : 1;
                        TW_ADD_TAG(IL_TAG_FILL_ORDER, IL_TAG_SHORT, 1,
                                   &fillOrder)
                        ...
                        if (!(userOptions & IL_G3M_EOL_UNALIGNED))
                                group3Options |= 4;
                        TW_ADD_TAG(IL_TAG_GROUP_3_OPTIONS, IL_TAG_LONG, 1,
                                   &group3Options)
                        TIFFcompression = 3;
                }
        } break;
        ...
        case IL_G4: {
                ...
                group4Options = 0;
                TW_ADD_TAG(IL_TAG_GROUP_4_OPTIONS, IL_TAG_LONG, 1,
                           &group4Options)
                TIFFcompression = 4;
                break;
        ...
        case IL_LZW:
                ...
                /*  JPEG: write JIF with restart markers between strips.
                    For JIF-in-TIFF: add tags for a file ptr and length of the
                   JIF image, and the restart interval, but dont know value
                   until JIF image received.
                */
        case IL_JPEG:
                ...
                TW_ADD_TAG(IL_TAG_JPEG_INTERCHANGE_FORMAT_LENGTH, IL_TAG_LONG,
                           1, NULL)
                TW_ADD_TAG(IL_TAG_JPEG_RESTART_INTERVAL, IL_TAG_SHORT, 1, NULL)
                TW_ADD_TAG(IL_TAG_JPEG_Q_TABLES, IL_TAG_LONG,
                           des.nSamplesPerPixel, NULL)
                TW_ADD_TAG(IL_TAG_JPEG_DC_TABLES, IL_TAG_LONG,
                           des.nSamplesPerPixel, NULL)
                TW_ADD_TAG(IL_TAG_JPEG_AC_TABLES, IL_TAG_LONG,
                           des.nSamplesPerPixel, NULL)
        ...
        /*  Image type-specific code: set PhotometricInterpretation: to 4
           (transparency mask) if a maskImage, else based on image type.
        */
        if (writeMethod == maskImage)
                ...
                case IL_RGB:
                        ...
                        /*  YCbCr subsampling currently supported only if
                         * JPEG/JIF compressed. */
                case IL_YCBCR:
                        /* Add the YCbCr-specific tags: coefficients as
                         * fractions of 10000 */
                        ...
                        TW_ADD_TAG(IL_TAG_YCBCR_COEFFICIENTS, IL_TAG_RATIONAL,
                                   3, coeff)

                        /*  TIFF only allows subsampling as follows: Y = 1,
                           Cr/Cb = 1,2,4 but Cr and CB equal to each other; vert
                           <= horiz.
                        */
                        {
                                register ilYCbCrSampleInfo *pSample =
                                    des.typeInfo.YCbCr.sample;

                                if ((pSample[0].subsampleHoriz != 1) ||
                                    (pSample[0].subsampleVert != 1) ||
                                    (pSample[1].subsampleHoriz !=
                                     pSample[2].subsampleHoriz) ||
                                    (pSample[1].subsampleVert !=
                                     pSample[2].subsampleVert) ||
                                    (pSample[1].subsampleVert >
                                     pSample[2].subsampleHoriz))
                                        return ilDeclarePipeInvalid(
                                            pipe, IL_ERROR_YCBCR_TIFF);
                                subsample[0] = pSample[1].subsampleHoriz;
                                subsample[1] = pSample[1].subsampleVert;
                                TW_ADD_TAG(IL_TAG_YCBCR_SUBSAMPLING,
                                           IL_TAG_SHORT, 2, subsample)

                                /*  Set positioning - ? is centered correct ? */
                                positioning = 1;
                                TW_ADD_TAG(IL_TAG_YCBCR_POSITIONING,
                                           IL_TAG_SHORT, 1, &positioning)

                                refBW[0] = pSample[0].refBlack;
                                refBW[1] = 1;
                                refBW[2] = pSample[0].refWhite;
                                refBW[3] = 1;
                                refBW[4] = pSample[1].refBlack;
                                refBW[5] = 1;
                                refBW[6] = pSample[1].refWhite;
                                refBW[7] = 1;
                                refBW[8] = pSample[2].refBlack;
                                refBW[9] = 1;
                                refBW[10] = pSample[2].refWhite;
                                refBW[11] = 1;
                                TW_ADD_TAG(IL_TAG_REFERENCE_BLACK_WHITE,
                                           IL_TAG_RATIONAL, 6, refBW)
                        }
                        photo = 6;
                        break;
        ...
        /*  Write # strips stripOffsets/ByteCounts, but supply no data - called
            function handles these specially and creates blank space, filled in
           later.
        */
        ...
        /*  As per TIFF spec, write planarConfiguration tag only if # samples
         * > 1. */
        if (des.nSamplesPerPixel > 1) {
                planarConfiguration =
                    (format.sampleOrder == IL_SAMPLE_PIXELS) ? 1 : 2;
                TW_ADD_TAG(IL_TAG_PLANAR_CONFIGURATION, IL_TAG_SHORT, 1,
                           &planarConfiguration)
        }
        ...
        */
        if (des.type == IL_PALETTE) {
                ...
                TW_ADD_TAG(IL_TAG_COLOR_MAP, IL_TAG_SHORT,
                           3 * tagInfo.nPaletteColors, NULL)
                ...
                if (des.flags & IL_DITHERED_PALETTE) {
                        ...
                        TW_ADD_TAG(IL_TAG_PRIVATE_0, IL_TAG_SHORT,
                                   IL_TAG_P0_DITHER_LEVELS_LENGTH, ditherLevels)
                }
        } else
                tagInfo.pPalette = (unsigned short *)NULL;

        /*  Merge the user tags with the tags formed above, convert the data to
            the proper byte order and return the allocated info to "tagInfo" or
           error.
        */
        if (error = ilMergeWriteTags(pFile->bigEndian, nTags, tags, nUserTags,
                                     pUserTags, &tagInfo))
                return ilDeclarePipeInvalid(pipe, error);

        /*  Add consumer pipe element, forcing constant "stripHeight" - ignored
           if pipe image is compressed - stripHeight was forced above by call to
           ilCompress(). Use separate function for when writing JPEG.
        */
        srcData.consumerImage = (ilObject)NULL;
        srcData.stripHeight = stripHeight;
        srcData.constantStrip = TRUE;
        srcData.minBufferHeight = 0;
        pPriv = (ilWriteFilePrivPtr)ilAddPipeElement(
            pipe, IL_CONSUMER, sizeof(ilWriteFilePrivRec), 0, &srcData,
            (ilDstElementData *)NULL, ilWriteFileInit, ilWriteFileCleanup,
            ilWriteFileDestroy,
            (des.compression == IL_JPEG) ? ilWriteJPEGExecute
                                         : ilWriteFileExecute,
            0);
        if (!pPriv)
                return FALSE;

        /*  Fill in private (*pPriv), including template for file image to be
         * added. */
        pPriv->pFile = pFile;
        ...
        /*  Point pPriv->tailPtrOffset to location in file to point to this
           image: "ptr to next" of last image in file (if main image) else ptr
           to next of parent.
        */
        if (fileImage) /* have parent: child or mask image */
                pPriv->tailPtrOffset =
                    ((ilFileImagePtr)fileImage)->tagOffset +
        ...
        pPriv->fileImage.p.pNext = pPriv->fileImage.p.pPrev =
            pPriv->fileImage.p.pChild = pPriv->fileImage.p.pMask =
                (ilFileImage)NULL;
...
*/
ilBool ilInitFile(ilContext context, FILE *stream, long offset,
                  ilBool MSBIsFirst, unsigned long mustBeZero) {
        ilBool bigEndian;
        ilFilePtr pFile;
        ilByte *pHeader;
