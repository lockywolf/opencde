 ...
 */
static ilError ilJIFToRawDestroy(ilJIFToRawPrivPtr pPriv) {
 ...
 */
static ilError ilJIFToRawExecute(ilExecuteData *pData, unsigned long dstLine,
        ...
        for (i = 0; i < ILJPG_MAX_COMPS; i++) {
                pCompData->sample[i].QTableIndex =
                    pJPEGData->comp[i].QTableIndex;
                pCompData->sample[i].DCTableIndex =
                    pJPEGData->comp[i].DCTableIndex;
                pCompData->sample[i].ACTableIndex =
                    pJPEGData->comp[i].ACTableIndex;
        }

        /*  Copy restartInterval from JIF info; it will be zero (0) if no DRI
           marker, implying no restart markers.
        */
        pCompData->restartInterval = pJPEGData->restartInterval;

        /*  Free decode data but not tables; they will be freed by
         * ilJIFToRawDestroy() */
        IL_FREE(pJPEGData);

        /*  Write rest of data (actual JPEG data, after tables etc.) to next
           filter. Exclude the EOI marker if present (which it really should
           be).
        */
        ...
        if (pDstPlane->bufferSize < (nBytes + dstOffset))
                if (!_ilReallocCompressedBuffer(pData->pDstImage, 0,
                                                (nBytes + dstOffset)))
                        return IL_ERROR_MALLOC;
        bcopy((char *)streamRec.pData, (char *)(pDstPlane->pPixels + dstOffset),
              nBytes);
...
*/
static ilBool ilJIFToRaw(ilPipe pipe, ilPipeInfo *pInfo, ilImageDes *pDes,
        ...
        pPriv = (ilJIFToRawPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilJIFToRawPrivRec), 0,
            (ilSrcElementData *)NULL, &dstData, IL_NPF, IL_NPF,
            ilJIFToRawDestroy, ilJIFToRawExecute, 0);
        if (!pPriv)
                return FALSE;
 ...
 */
static ilError ilRawToJIFInit(ilRawToJIFPrivPtr pPriv, ilImageInfo *pSrcImage,
 ...
 */
static ilError ilRawToJIFExecute(ilExecuteData *pData, unsigned long dstLine,
                                 unsigned long *pNLines) {
        ...
        /*  Setup stream to point to dst image buffer; set size, offset also.
            ...
            bytes written on each strip.  When done, it is size of JIF image
           written.
        */
        pImage = pData->pDstImage;
        streamRec.pBuffer = pImage->plane[0].pPixels;
        streamRec.pDst = streamRec.pBuffer + pPriv->dstOffset;
        streamRec.pPastEndBuffer =
            streamRec.pBuffer + pImage->plane[0].bufferSize;
        startOffset = ILJPG_ENCODE_OFFSET(
            &streamRec);           /* stream offset before any writes */
        ...
        if (pPriv->nStripsWritten == 0) {
                ...
                /*  Set restart interval: if a single strip, it is the restart
                   interval in the raw data (already set from above).  If not a
                   single strip, then the raw data must not have a restart
                   interval or an error - can't code both of them!  Otherwise
                   restart interval is determined by strip size.
                */
                if (pPriv->stripHeight !=
                    pPriv->height) { /* not single strip */
                        long mcuWidth, mcuHeight;
                        if (data.restartInterval)
                                return IL_ERROR_COMPRESSED_DATA; /* have
                                                                    restartInterval;
                                                                    error */
                ...
                if (error = iljpgEncodeJIF(&streamRec, &data,
                                           (iljpgJIFOffsetsPtr)NULL))
                        return error;
        }

        /*  Write data from this strip to output buffer, reallocating if
           necessary. Allocate 2 extra bytes for EOI or RSTn marker.
        */
        ...
        /*  If last strip, write 2 byte EOI marker else write 2 byte restart
         * marker. */
        pPriv->nLinesWritten += *pNLines;
        lastStrip = (pPriv->nLinesWritten >= pPriv->height);
        if (lastStrip) {
                *streamRec.pDst++ =
                    ILJPGM_FIRST_BYTE; /* last strip; write EOI marker */
        ...
        pImage->plane[0].bufferSize =
            streamRec.pPastEndBuffer - streamRec.pBuffer;
        pPriv->dstOffset += ILJPG_ENCODE_OFFSET(&streamRec) - startOffset;

        /*  If last strip, pass # bytes written to next filter (= current dst
           offset; note that dst offset set to 0 on first strip), and return #
           lines = height of image, as JIF is one big strip.  If not last strip,
           return # lines as zero and next filter will not be called.
        */
        if (lastStrip) {
...
*/
static ilBool ilRawToJIF(ilPipe pipe, ilPipeInfo *pInfo, ilImageDes *pDes,
                         ilImageFormat *pFormat) {
        ilRawToJIFPrivPtr pPriv;
        ilDstElementData dstData;

        dstData.producerObject = (ilObject)NULL;
        pDes->compInfo.JPEG.reserved &=
            ~IL_JPEGM_RAW; /* now writing JIF, not raw JPEG */
        dstData.pDes = pDes;
        dstData.pFormat = pFormat;
        dstData.width = pInfo->width;
        dstData.height = pInfo->height;
        ...
        /*  Add the filter.  "Hold dst" flag set because filter will write to
            output buffer, returning # lines = 0 until last strip written, after
           which whole output buffer will be passed to next filter, # lines =
           height.
        */
        pPriv = (ilRawToJIFPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilRawToJIFPrivRec), IL_ADD_PIPE_HOLD_DST,
            (ilSrcElementData *)NULL, &dstData, ilRawToJIFInit, IL_NPF, IL_NPF,
            ilRawToJIFExecute, 0);
        if (!pPriv)
                return FALSE;
 ...
 */
static ilError ilCompressJPEGInit(ilCompressJPEGPrivPtr pPriv,
 ...
 */
static ilError ilCompressJPEGCleanup(ilCompressJPEGPrivPtr pPriv,
                                     ilBool aborting) {
        ilError error;
 ...
 */
static ilError ilCompressJPEGDestroy(ilCompressJPEGPrivPtr pPriv) {
        register int i;
        ilPtr pTable;
 ...
 */
static ilError ilCompressJPEGExecute(ilExecuteData *pData,
        ...
        /*  Encode one strip, writing output to dst buffer (pointed to by
           streamRec). Input is from src buffer, starting at srcLine, divided by
           subsample factor (which is "inverse" of JPEG factors; = max factor /
           component factor).
        */
        ...
        for (i = 0; i < pPriv->jpgData.nComps; i++, pSrcPlane++) {
                pSrcPixels[i] =
                    pSrcPlane->pPixels +
                    (pData->srcLine * pPriv->jpgData.comp[i].vertFactor /
                     pPriv->jpgData.maxVertFactor) *
        ...
        /*  Set stream data back into dst image structure.  Inform next filter
           of the # bytes written = current stream offset - start offset.
        */
        pImage->plane[0].pPixels = streamRec.pBuffer;
        pImage->plane[0].bufferSize =
            streamRec.pPastEndBuffer - streamRec.pBuffer;
        *pData->compressed.pNBytesWritten =
            ILJPG_ENCODE_OFFSET(&streamRec) - startOffset;
...
*/
static ilPtr
_ilScaleQTable(register ilPtr pSrc, /* ptr to 64 byte Q table to scale */
        ...
        /*  Scale src to dst (pTable) by: dst = src * Qfactor / 50.
            I don't know where this came from, but it is how the old code
           worked. The old code however only allowed a max Q of 255; larger
           values became 255. Here we will allow the maximum value of a short.
        */
...
*/
IL_PRIVATE ilBool _ilCompressJPEG(ilPipe pipe, ilPipeInfo *pInfo,
        ...
        */
        if (!pEncodeData)
                ...
                pEncode = (ilJPEGEncodeControl *)pEncodeData;
        if (pEncode->mustbezero & ~IL_JPEGM_RAW)
                return ilDeclarePipeInvalid(pipe, IL_ERROR_PAR_NOT_ZERO);

        /*  If already JPEG-compressed cvt to requested format (JIF/raw) if
         * necessary */
        if (pDes->compression == IL_JPEG) {
                if (pDes->compInfo.JPEG.reserved &
                    IL_JPEGM_RAW) /* src is raw */
        ...
        /*  Image is not JPEG; must be uncompressed.  Check for supported image
           type. Set maxVert to max vert subsampling; luminanceMask to sample #.
        */
        switch (pDes->type) {
        case IL_GRAY:
                maxVert = 1;
        ...
        case IL_RGB:
                ...
                luminanceMask =
                    ~0; /* use luminance tables for all components */
                break;
        case IL_YCBCR:
                maxVert = 1;
        ...
        stripHeight =
            ((pSrcData->stripHeight + maxVert - 1) / maxVert) * maxVert;
        if (stripHeight <= 0)
                stripHeight = maxVert;
        ...
        pPriv = (ilCompressJPEGPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilCompressJPEGPrivRec), 0, pSrcData,
            &dstData, ilCompressJPEGInit, ilCompressJPEGCleanup,
            ilCompressJPEGDestroy, ilCompressJPEGExecute, 0);
        if (!pPriv)
                return FALSE;

        /*  Init tables to use: default luminance (index 0) or chrominance
           (index 1). If "Q" not zero, scale up Q tables; must destroy them in
           Destroy().
        */
        pPriv->pCompData = pCompData;
        ...
        for (i = 0; i < pDes->nSamplesPerPixel; i++) {
                ...
                if (!pCompData->QTables[tableIndex]) { /* no Q table here yet;
                                                          make one */
                        if (pEncode->Q) /* Q != 0; scale copy of Q table */
                                if (!(pTable =
                                          _ilScaleQTable(pTable, pEncode->Q)))
                                        return ilDeclarePipeInvalid(
                                            pipe, IL_ERROR_MALLOC);
        ...
        /*  Init iljpg package data in private, based on IL view of JPEG comp
         * data */
         ...
         */
        if (!(pEncode->mustbezero & IL_JPEGM_RAW)) {
                ilGetPipeInfo(pipe, FALSE, pInfo, pDes,
                              pFormat); /* update pipe info */
                return ilRawToJIF(pipe, pInfo, pDes, pFormat);
        }
