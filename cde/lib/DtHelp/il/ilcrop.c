 ...
 * ================================= */
/*  This code crops compressed pipe images by ignoring strips that are outside
 ...
 */
static ilError ilCropCompInit(ilCropCompPrivPtr pPriv, ilImageInfo *pSrcImage,
 ...
 */
static ilError ilCropCompExecute(ilExecuteData *pData, long dstLine,
                                 long *pNLines) {
        register ilCropCompPrivPtr pPriv;

        /*  If this strip is out of range of strips to be written, return 0 for
            # lines written (crops the strip); otherwise pass strip on to next
           filter.
        */
        pPriv = (ilCropCompPrivPtr)pData->pPrivate;
        if ((pPriv->stripIndex < pPriv->topStrip) ||
            (pPriv->stripIndex > pPriv->bottomStrip)) {
                *pNLines = 0; /* skip this strip */
                *pData->compressed.pDstOffset =
                    0; /* next filter should not be called anyway */
...
/*  Called by ilCrop() when the pipe image is compressed.  "pRect" must point to
    a non-null rectangle which is the crop rectangle intersect with the image
   bounds. "pInfo" must point to the current pipe info.
*/
static ilBool ilCropCompressed(ilPipe pipe, ilRect *pRect, ilPipeInfo *pInfo) {
        ilRect rect;
        ...
        /*  Skip strips above and below the strips that intersect the crop rect.
            Can only do that if strips are constant, because we must declare the
           size of the image coming out of the filter added here.  Set
           "top/bottomStrip" to the index (from 0) of the top/bottom
           (first/last) strip within crop rect.
        */
        ...
        if (pInfo->constantStrip && (stripHeight > 0)) {
                ...
                /*  Add a "no dst" filter to crop only if some strips would be
                 * skipped */
                if ((topStrip > 0) ||
                    (bottomStrip < ((pInfo->height - 1) / stripHeight))) {
                        ...
                        topLine =
                            topStrip *
                            stripHeight; /* first line output from filter */
                        dstData.height =
                            (bottomStrip + 1) * stripHeight - topLine;
                        ...
                        pPriv = (ilCropCompPrivPtr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilCropCompPrivRec),
                            IL_ADD_PIPE_NO_DST, (ilSrcElementData *)NULL,
                            &dstData, ilCropCompInit, IL_NPF, IL_NPF,
                            ilCropCompExecute, NULL, 0);
                        if (!pPriv)
                                return FALSE;
                        pPriv->topStrip = topStrip;
                        pPriv->bottomStrip = bottomStrip;

                        /*  Adjust the top of new crop rect by lines skipped by
                         * this filter. */
                        rect.y -= topLine;
                }
        }

        /*  Decompress the pipe image and call ilCrop() recursively to crop the
           bits that remain after this code has (possibly) skipped unnecessary
           strips.
        */
        ilGetPipeInfo(pipe, TRUE, (ilPipeInfo *)NULL, (ilImageDes *)NULL,
                      (ilImageFormat *)NULL);
         ...
         * =============================== */
        /*  This code crops uncompressed pipe images by copying only the
         * relevant pixels.
         */
...
typedef struct {
        ...
        int ilCropDstheight; /* destination height value saved to avoid strip
                                sizes  */
...
static ilError ilCropInit(ilCropPrivptr pPrivate, ilImageInfo *pSrcImage,
                          ilImageInfo *pDstImage)
...
/* =============================================================================================================================
      ...
      ilCropBitonalExecute  -  Crop processing for images with bit per pixel
   format .

   =============================================================================================================================
 */
static ilError ilCropBitonalExecute(register ilExecuteData *pData,
        ...
        for (y = lastcount; y < (lastcount + *pNLines);
             y++, pPriv->illinecount++) {
                ...
                if ((y >= pPriv->ilCropYoff) && (y < yextent)) { /* process */
                        ...
                        for (x = 0; x < firstword; x++, psrc++) {
                        } /* skip long words up to the offset */
...
/* =============================================================================================================================
      ...
      ilCrop3ByteExecute  -  Crop processing for images with 24 bits per pixel
   format .

   =============================================================================================================================
 */
static ilError ilCrop3ByteExecute(register ilExecuteData *pData,
                                  unsigned long dstLine,
                                  unsigned long *pNLines) {
...
/* =============================================================================================================================
      ...
      ilCropByteExecute  -  Crop processing for images with byte per pixel
   format .

   =============================================================================================================================
 */
static ilError ilCropByteExecute(register ilExecuteData *pData,
                                 unsigned long dstLine,
                                 unsigned long *pNLines) {
        register unsigned char *psrc, *pdst, *psrcline, *pdstline;
        register unsigned long srcnbytes, dstnbytes, nlines;
...
/* =============================================================================================================================
      ...
      ilCrop   -  Public function.  Insert a pipe element which crops to the
   given rectangle. Check for format types and do an explicit conversion if
   necessary.

   =============================================================================================================================
 */
ilBool ilCrop(ilPipe pipe, ilRect *pRect) {
        unsigned int state;
        ilPipeInfo info;
        register ilCropPrivptr pPriv;
        ilDstElementData dstdata;
        ...
        /* If crop rect = image rect; a noop - return.  If null crop rect,
         * error. */
        if ((Srcrect.height == Dstrect.height) &&
            (Srcrect.width == Dstrect.width) && (Dstrect.x == 0) &&
            (Dstrect.y == 0))
                return TRUE;
        ...
        /* If the pipe image is compressed, handle separately.  Code will crop
           compressed data by ignoring strips outside of the range of the crop
           rect. It will then force decompression of the pipe image and call
           ilCrop() to crop within the strips.
        */
        ...
        switch (imdes.nSamplesPerPixel) {
        case 3:
                pPriv = (ilCropPrivptr)ilAddPipeElement(
                    pipe, IL_FILTER, sizeof(ilCropPriv), 0,
                    (ilSrcElementData *)NULL, &dstdata, ilCropInit, IL_NPF,
                    IL_NPF, ilCrop3ByteExecute, NULL, 0);
                break;
        ...
        case 1:
                if (bitonal)
                        pPriv = (ilCropPrivptr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilCropPriv), 0,
                            (ilSrcElementData *)NULL, &dstdata, ilCropInit,
                            IL_NPF, IL_NPF, ilCropBitonalExecute, NULL, 0);
                else
                        pPriv = (ilCropPrivptr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilCropPriv), 0,
                            (ilSrcElementData *)NULL, &dstdata, ilCropInit,
                            IL_NPF, IL_NPF, ilCropByteExecute, NULL, 0);
        }
