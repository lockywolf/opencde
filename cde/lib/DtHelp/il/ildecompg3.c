...
static ilError _ilDecompG3Init(ilDecompG3G4PrivPtr pPriv,
...
static ilError _ilDecompG3Cleanup(ilDecompG3G4PrivPtr pPriv,
...
static ilError _ilDecompG3Line(register ilDecompG3G4PrivPtr pPriv,
                               ilPtr dstImageP) {
        ...
        /*	int secondRun;              no. of consecutive pixels for
         * M(a1a2) in  Horiz mode */
        ...
        register ilDecompG4HuffTablePtrConst pDecodeWhite =
            ilArFax1DDecodeWhite;
        register ilDecompG4HuffTablePtrConst pDecodeTemp;
        register ilDecompG4HuffTablePtrConst pDecodeBlack =
            ilArFax1DDecodeBlack;
        register ilPtr sByte;
        register int no_of_ones;
        register int startPixel;

        static const unsigned char fillmasks[] = {0,    0x80, 0xc0, 0xe0, 0xf0,
                                                  0xf8, 0xfc, 0xfe, 0xff};

        /*  Deccompression Procedure  ....

    From the compressed data from Source Image .. retrieve a long bit; use this
    as index to determine the number of White Pixels and then the number of
   ...
   */
        ...
        do { /* till a complete Image line is DeCompressed */
                ...
                if (color == pPriv->white) { /* white is run is expected first
                                                always... */
                        firstRun = 0;
                        do {
                                if (Is_Lsb_First)
                                        GET_VALUE_LSB(bits, ImageP, srcpos,
                                                      G4M_WhiteRun)
                                else
                                        GET_VALUE_MSB(bits, ImageP, srcpos,
                                                      G4M_WhiteRun)
                                pDecodeTemp = pDecodeWhite + bits;

                                /* if invalid data is found .... */
                                if ((pDecodeTemp->length == 0) &&
                                    (pDecodeTemp->value == 0) &&
                                    (pDecodeTemp->type == 0))
                ...
                if ((color) && (firstRun > 0)) /* fill the dst image with 1's */
                {
                        ...
                        /* the whole block was written as a function, In-lined
                           later to speedup performance in Hpux     ..
                         */
                        ...
                        if (startPixel &= 7) { /* align to byte boundary */
                                if (no_of_ones < 8 - startPixel) {
                                        *sByte++ |=
                                            fillmasks[no_of_ones] >> startPixel;
                                        goto putones_done;
                                }
...
static ilError _ilDecompG3Execute(ilExecuteData *pData, unsigned long dstLine,
   ...
   ======================================================================== */
        ...
        if (*pNLines <= 0)
                return IL_OK;
        if (pData->compressed.nBytesToRead <= 0)
                return IL_OK;
        nLines = *pNLines;

        /* Exit if pointer to pPixels is NULL */
        pSrcPlane = &pData->pSrcImage->plane[0];
        if (!pSrcPlane->pPixels)
                return IL_ERROR_NULL_COMPRESSED_IMAGE;
        pSrcLine = pSrcPlane->pPixels +
                   pData->compressed.srcOffset; /* image location pointer */

        pDstPlane = &pData->pDstImage->plane[0];

        /* The destination image line pointer gets updated at the beginning of
         * each strip */
   ...
   ======================================================================== */
        ...
        /* this pPriv->maxSrcPos is set to the bits that could be read, to
           prevent the program from reading beyond the compressed bytes; this
           check is done by the macro Get_value_msb & lsb
         */
       ...
       Compressed file... so check for that
     */
        ...
        /* till the Destination Image is filled up , call the decompress from
       the source line by line.. If any error in reading the compressed data,
       i.e invalid Code words or invalid Modes are found will retrun the error
       ...
       Destination image size and Pointer are suitably decremented &
       incremented.
    */

        while (nLines-- > 0) {

                /*  If EOLs are present in the compressed Image, (they will be
           there at the begining of Each line, not at the end contrary to the
           meaning of EOL !! ), they have to be skipped... ; This loop takes
           care of this whether the EOL's are aligned or Not..  */

                if (Is_EOLs) {

                        do {
                                if (pPriv->Is_Lsb_First)
                                        GET_VALUE_LSB(bits, pPriv->ImageP,
                                                      pPriv->srcpos,
                                                      G4M_WhiteRun)
                                else
                                        GET_VALUE_MSB(bits, pPriv->ImageP,
                                                      pPriv->srcpos,
                                                      G4M_WhiteRun)
                                pDecodeTemp = pDecodeWhite + bits;

                                if ((pDecodeTemp->length == 12) &&
                                    (pDecodeTemp->value == 1) &&
                                    (pDecodeTemp->type == G4K_CodetypeEol)) {
                                        pPriv->srcpos += 1;
                                        continue;
                                }
                        } while ((pDecodeTemp->length == 12) &&
                                 (pDecodeTemp->value == 1) &&
                                 (pDecodeTemp->type == G4K_CodetypeEol));
                ...
                /*  If 2D compressed Image, then, after EOL look for the tag
           bit, which indicates the mode of compression for the next line;
           PIXEL(...), retrieves the next bit i.e tag bit, and if it is equal to
           1, then next line sholud be decompressed 1 dimesionally...; so, call
           _ilDecompG3Line(...); else, next line sholud be decompressed 2
           dimesionally...and call _ilDecompG4Line(...);

           If not 2D compressed Image, and if Not EOLs, then, the next row of
           image, will be from the start of the next byte, so check for that and
           skip to the next byte...
                ...
                if (Is_2DCoding) {
                        ...
                        if (pPriv->Is_Lsb_First)
                                tag_bit =
                                    PIXEL_LSB(pPriv->ImageP, pPriv->srcpos);
                        else
                        ...
                        if (tag_bit) {
                                ...
                                pPriv->srcpos++;
                                if ((error = _ilDecompG4Line(pPriv, pRefLine,
                                                             dstImageP)))
                                        return error;
                        }
...
IL_PRIVATE
ilBool _ilDecompG3(ilPipe pipe, ilPipeInfo *pinfo, ilImageDes *pimdes) {
    ...
    */
        ...
        pPriv = (ilDecompG3G4PrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilDecompG3G4PrivRec), 0,
            (ilSrcElementData *)NULL, &dstdata, _ilDecompG3Init,
            _ilDecompG3Cleanup, IL_NPF, _ilDecompG3Execute, NULL, 0);
