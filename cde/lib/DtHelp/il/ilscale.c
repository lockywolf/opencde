...
typedef struct {
        long
            ilScaleAccDown; /* accumulator value used in scale down algorithm */
        long ilScaleAccUp;  /* accumulator value used in scale  up  algorithm */
        long
            ilScaleDstheight;  /* destination height value saved to avoid strip
                                  sizes  */
...
/* =============================================================================================================================
        ilScaleInit  -  Initialization function for a scale filter performed
   with the method of replication/decimation.  This gets called when the pipe is
   executed for the first time.
   =============================================================================================================================
 */

static ilError ilScaleInit(ilScalePrivptr pPrivate, ilImageInfo *pSrcImage,
                           ilImageInfo *pDstImage)

{
        pPrivate->ilScaleAccDown =
            pPrivate->ilScaleSrcheight - pPrivate->ilScaleDstheight;
        pPrivate->ilScaleAccUp =
            pPrivate->ilScaleDstheight - pPrivate->ilScaleSrcheight;

        return IL_OK;
}

/* =============================================================================================================================
        ilswbitdown  -  Routine to scale down the width of an image by
   decimation. Operates on bitonal images.
   =============================================================================================================================
 */

static void ilswbitdown(register CARD32 *psrcline, register CARD32 *pdstline,
...

{
        ...
        if (copy == TRUE) { /*  degenerate case optimized  */
                ...
                while (nbits--) {
                        ...
                        SHIFT_MASK(srcMask);
                        if (!srcMask) { /* after 32 bits reset mask and get new
                                           srcline */
                                srcMask = SPECIAL_MASK_BIT;
                                srcLong = *psrcline++;
                        }
                ...
                while (nbits--) {
                        ...
                        for (; acc > 0; acc -=
                                        dstwidth) { /*  skip bits according to
                                                       scale ratio */

                                SHIFT_MASK(srcMask);

                                if (!srcMask) { /* after 32 bits reset mask and
                                                   get new srcline */
                                        srcMask = SPECIAL_MASK_BIT;
                                        srcLong = *psrcline++;
                                }
...
static void ilswbitup(register CARD32 *psrcline, register CARD32 *pdstline,
                      unsigned long srcwidth, unsigned long dstwidth)
...
/* =============================================================================================================================
        ilsw3bytedown  -  Routine to scale down the width of an image by
   decimation. Operates on 24 bit images.
   =============================================================================================================================
 */

static void ilsw3bytedown(register unsigned char *psrcline,
...
/* =============================================================================================================================
        ilsw3byteup  -  Routine to scale up the width of an image by
   replication. Operates on 24 bit images.
   =============================================================================================================================
 */

static void ilsw3byteup(register unsigned char *psrcline,
                        register unsigned char *pdstline,
                        unsigned long srcwidth, unsigned long dstwidth)
...
/* =============================================================================================================================
        ilswbytedown  -  Routine to scale down the width of an image by
   decimation. Operates on 8 bit images.
   =============================================================================================================================
 */

static void ilswbytedown(register unsigned char *psrcline,
                         register unsigned char *pdstline,
                         unsigned long srcwidth, unsigned long dstwidth)
...
static void ilswbyteup(register unsigned char *psrcline,
                       register unsigned char *pdstline, unsigned long srcwidth,
...
/* =============================================================================================================================
      ...
      ilScaleBitonalExecute  -  Scale processing for images with bit per pixel
   format . This function that will get called at least once when the pipe is
   executed.  This routine loops through the number of lines, scaling the height
   and calls the appropriate function to scale the width.
   =============================================================================================================================
 */
static ilError ilScaleBitonalExecute(register ilExecuteData *pData,
        ...
        if (pPriv->ilScaleSrcheight ==
            pPriv->ilScaleDstheight) { /*   copy case optimized  */

                while (nlines-- > 0) {
                        if (srcwidth >= dstwidth)
                                ilswbitdown(psrcline, pdstline, srcwidth,
                                            dstwidth);
                        else if (srcwidth < dstwidth)
                                ilswbitup(psrcline, pdstline, srcwidth,
                                          dstwidth);
        ...
        } else if (pPriv->ilScaleSrcheight >
                   pPriv->ilScaleDstheight) { /* scale height down */

                while (TRUE) {
                        while (pPriv->ilScaleAccDown > 0) {
                                if (nlines-- <= 0)
                                        break;
                                pPriv->ilScaleAccDown -=
                                    pPriv->ilScaleDstheight;
                                psrcline += srcnwords;
                        }
                        if (nlines-- <= 0)
                                break;
                        ...
                        pPriv->ilScaleAccDown +=
                            (pPriv->ilScaleSrcheight - pPriv->ilScaleDstheight);
                   ...
                   pPriv->ilScaleDstheight) { /* scale height up */

                while (nlines-- > 0) {

                        for (; pPriv->ilScaleAccUp >= 0;
                             pPriv->ilScaleAccUp -= pPriv->ilScaleSrcheight) {

                                if (srcwidth >= dstwidth)
                                        ilswbitdown(psrcline, pdstline,
                                                    srcwidth, dstwidth);
                                else if (srcwidth < dstwidth)
                                        ilswbitup(psrcline, pdstline, srcwidth,
                                                  dstwidth);
...
/* =============================================================================================================================
      ...
      ilScale3ByteExecute  -  Scale processing for images with 24 bits per pixel
   format . This function that will get called at least once when the pipe is
   executed.  This routine loops through the number of lines, scaling the height
   and calls the appropriate function to scale the width.
   =============================================================================================================================
 */
static ilError ilScale3ByteExecute(register ilExecuteData *pData,
                                   unsigned long dstLine,
                                   unsigned long *pNLines) {
        ...
        psrcline =
            (unsigned char *)(pplane->pPixels) + pData->srcLine * srcnbytes;
        ...
        if (pPriv->ilScaleSrcheight ==
            pPriv->ilScaleDstheight) { /*   copy case optimized  */
                ...
                while (nlines-- > 0) {
                        if (srcwidth >= dstwidth)
                                ilsw3bytedown(psrcline, pdstline, srcwidth,
                                              dstwidth);
                        else if (srcwidth < dstwidth)
                                ilsw3byteup(psrcline, pdstline, srcwidth,
                                            dstwidth);
                ...
                while (nlines-- > 0) {
                        ...
                        for (; pPriv->ilScaleAccUp >= 0;
                             pPriv->ilScaleAccUp -= pPriv->ilScaleSrcheight) {
                                ...
                                if (srcwidth >= dstwidth)
                                        ilsw3bytedown(psrcline, pdstline,
                                                      srcwidth, dstwidth);
                                else if (srcwidth < dstwidth)
                                        ilsw3byteup(psrcline, pdstline,
                                                    srcwidth, dstwidth);
...
/* =============================================================================================================================
      ...
      ilScaleByteExecute  -  Scale processing for images with byte per pixel
   format . This function that will get called at least once when the pipe is
   executed.  This routine loops through the number of lines, scaling the height
   and calls the appropriate function to scale the width.
   =============================================================================================================================
 */
static ilError ilScaleByteExecute(register ilExecuteData *pData,
                                  unsigned long dstLine,
                                  unsigned long *pNLines) {
        register unsigned char *psrcline, *pdstline;
        register unsigned long srcnbytes, dstnbytes;
        ...
        if (pPriv->ilScaleSrcheight ==
            pPriv->ilScaleDstheight) { /*   copy case optimized  */
                ...
                while (nlines-- > 0) {
                        if (srcwidth >= dstwidth)
                                ilswbytedown(psrcline, pdstline, srcwidth,
                                             dstwidth);
                        else if (srcwidth < dstwidth)
                                ilswbyteup(psrcline, pdstline, srcwidth,
                                           dstwidth);
                ...
                while (nlines-- > 0) {
                        ...
                        for (; pPriv->ilScaleAccUp >= 0;
                             pPriv->ilScaleAccUp -= pPriv->ilScaleSrcheight) {
                                ...
                                if (srcwidth >= dstwidth)
                                        ilswbytedown(psrcline, pdstline,
                                                     srcwidth, dstwidth);
                                else if (srcwidth < dstwidth)
                                        ilswbyteup(psrcline, pdstline, srcwidth,
                                                   dstwidth);
...
/* =============================================================================================================================
   ...
   =============================================================================================================================
 */
ilBool ilScale(ilPipe pipe, long dstWidth, long dstHeight, int option,
               void *pOptionData)

{
        ...
        if (option == IL_SCALE_SAMPLE) {
                int scaleFactor;
                if (((info.width << 1) == dstWidth) &&
                    ((info.height << 1) == dstHeight))
                        scaleFactor = 1;
                else if (((info.width << 2) == dstWidth) &&
                         ((info.height << 2) == dstHeight))
                        scaleFactor = 2;
                else
                        scaleFactor = 0;
                if (scaleFactor) {
                        if (_ilFastUpsample(pipe, &info, &imdes, &imformat,
                                            scaleFactor))
        ...
        case IL_SCALE_SAMPLE:
                ...
                if ((info.width < dstWidth) || (info.height < dstHeight))
                        break;
                ...
                if (imdes.type == IL_PALETTE) {
                        if (!ilConvert(pipe, IL_DES_RGB, IL_FORMAT_3BYTE_PIXEL,
                                       0, (ilPtr)NULL))
                                return FALSE;
                ...
                if (bitonal) {
                        ...
                        if (!pOptionData)
                                pOptionData = &defaultBitonalThreshold;
                        _ilAreaScaleBitonal(pipe, dstWidth, dstHeight,
                                            (int *)pOptionData, &info);
                } else
                        _ilScaleSample(pipe, imdes.nSamplesPerPixel, dstWidth,
                                       dstHeight, 256, imdes.blackIsZero,
                                       &info);
                return (pipe->context->error) ? FALSE : TRUE;

        case IL_SCALE_BITONAL_TO_GRAY:

                /*  Error condition if not scaling down (no demotion) or if the
                   image is not bitonal.  If pOptionData is null, default to 256
                   levels of gray.
                */
                if (!pOptionData)
                        pOptionData = &defaultNGrays;
                if ((info.width < dstWidth) || (info.height < dstHeight) ||
                    !bitonal)
                        return ilDeclarePipeInvalid(
                            pipe, IL_ERROR_SCALE_BITONAL_TO_GRAY);

                _ilScaleBitonalToGray(pipe, dstWidth, dstHeight,
                                      *((unsigned long *)pOptionData),
                                      imdes.blackIsZero, &info);
                return (pipe->context->error) ? FALSE : TRUE;
        ...
        dstdata.stripHeight =
            (dstHeight * info.recommendedStripHeight) / info.height + 1;
        dstdata.constantStrip = FALSE;

        switch (imdes.nSamplesPerPixel) {
        case 3:
                pPriv = (ilScalePrivptr)ilAddPipeElement(
                    pipe, IL_FILTER, sizeof(ilScalePriv), 0,
                    (ilSrcElementData *)NULL, &dstdata, ilScaleInit, IL_NPF,
                    IL_NPF, ilScale3ByteExecute, NULL, 0);
                break;
        ...
        case 1:
                if (bitonal)
                        pPriv = (ilScalePrivptr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilScalePriv), 0,
                            (ilSrcElementData *)NULL, &dstdata, ilScaleInit,
                            IL_NPF, IL_NPF, ilScaleBitonalExecute, NULL, 0);
                else
                        pPriv = (ilScalePrivptr)ilAddPipeElement(
                            pipe, IL_FILTER, sizeof(ilScalePriv), 0,
                            (ilSrcElementData *)NULL, &dstdata, ilScaleInit,
                            IL_NPF, IL_NPF, ilScaleByteExecute, NULL, 0);
        }
