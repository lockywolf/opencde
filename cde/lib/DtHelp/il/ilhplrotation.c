...
static char rcsid[] =
    "$XConsortium: ilhplrotation.c /main/3 1995/10/23 15:48:35 rswiston $";
static char version[] =
    "$XConsortium: ilhplrotation.c /main/3 1995/10/23 15:48:35 rswiston $";
static char vdate[] =
    "$XConsortium: ilhplrotation.c /main/3 1995/10/23 15:48:35 rswiston $";
...
#define PIXEL_UNIT                                                             \
        16384            /* pixel width, used to convert floating point        \
...
#define PIXEL_UNIT                                                             \
        1073741824L      /* pixel width, used to convert floating point        \
                            operations into integer operations */
 ...
 */
static short Build_Mult_Table(table) OPERAND *table[INTER_LEVELS];
{
        short i, j;
        OPERAND value, *p;
        ...
        for (i = 0; i < INTER_LEVELS; i++) {
                if ((table[i] = (OPERAND *)malloc(511 * sizeof(OPERAND))) ==
                    NULL) {
                        for (j = 0; j < i; j++) {
                                free(table[j]);
                                table[j] = NULL;
        ...
        /* Fill in Data */
        for (i = 0; i < INTER_LEVELS; i++) {
                for (p = table[i], value = -255 * i, j = -255; j < 256;
                     j++, value += i) {
                        *p++ = value >> INTER_BITS;
                }
        }
 ...
 */
static void Free_Mult_Table(table) OPERAND *table[INTER_LEVELS];
{
        short i;
...
static OPERAND First_Pixel(row_num) OPERAND row_num;
{
        if ((r_angle == 0.0) || (r_angle == 90.0) || (r_angle == 180.0) ||
            (r_angle == 270.0)) {
                return (0);
        }
        ...
        if (r_rotation_mode == 0) {
                if (row_num <= r_first_mark) {
                        return (r_input_w * r_cosA - (row_num + 0.5) * r_cotA +
                                0.5);
                } else {
                        return ((row_num + 0.5 - r_input_w * r_sinA) * r_tanA +
                                0.5);
                }
        } else {
                if (row_num < r_first_mark) {
...
static OPERAND Last_Pixel(row_num) OPERAND row_num;
{
        ...
        if (r_rotation_mode == 0) {
                if (row_num <= r_last_mark) {
                        return (r_input_w * r_cosA + (row_num + 0.5) * r_tanA -
                                0.5);
                } else {
...
DATA_IN_PTR _il_Rotate_Image_Begin(
    ...
    short *output_h         /* Returned val. for output image height*/
        ...
        if (rotation_mode == 0) {
                ...
                /* Add 2 pix above, below, to the left & right of the orig.
                 * image */
                r_buf = (DATA_IN_PTR)malloc((size_t)(input_w + 4) *
                                            (size_t)(input_h + 4) *
                                            input_bytes * sizeof(DATA_IN));
        ...
        if (r_angle == 0.0)
                r_quadrant = Q_0_180;
        else if (r_angle < 90.0)
                r_quadrant = Q_FIRST;
        else if (r_angle == 90.0)
                r_quadrant = Q_90_270;
        else if (r_angle < 180.0)
                r_quadrant = Q_SECOND;
        else if (r_angle == 180.0)
                r_quadrant = Q_0_180;
        else if (r_angle < 270.0)
                r_quadrant = Q_THIRD;
        else if (r_angle == 270.0)
                r_quadrant = Q_90_270;
        else
        ...
        *output_h = r_output_h = Output_Height();
        ...
        if (rotation_mode != 0) {
                ...
                top = r_buf;
                bot = r_buf + ((input_h + 2) * (input_w + 4) - 2) * input_bytes;
                if (input_bytes == 1) {
 ...
 * IR_Rotate_Send_Row
 */
short _il_Rotate_Send_Row(DATA_IN_PTR *input_data) {
        DATA_IN_PTR data;
        short i;
 ...
 */
static void _il_Rotate_Get_Row_Mode0(out) DATA_OUT_PTR out;
{
        ...
        */
        ...
        if (r_image_bytes == 3) {
                ...
                data = r_data +
                       (size_t)3 * (((long)y - 1) * r_input_w + ((long)x));
 ...
 */
static void _il_Rotate_Get_Row_Mode1(out) DATA_OUT_PTR out;
{
        OPERAND first_pix, last_pix, col;
        float Y, x, y;
        OPERAND ex, ey, dx, dy;
        ...
        */
        ...
        if (r_image_bytes == 3) {
                ...
                if (r_mul_tbls_valid) {
                        ...
                        data = r_data +
                               (((size_t)floor(y - 0.5) - 1) * (r_input_w + 4) +
                                (size_t)floor(x - 0.5)) *
                                   r_image_bytes;
                        ...
                        for (; col <= last_pix; col++, ex += dx, ey += dy) {
                                ...
                                x1 = *data++;
                                val1 =
                                    *(tex + ((size_t)(*(data + 2)) - x1)) + x1;
                                x1 = *data2++;
                                val2 =
                                    *(tex + ((size_t)(*(data2 + 2)) - x1)) + x1;
                        ...
                        for (; col <= last_pix; col++, ex += dx, ey += dy) {
                                ...
                                x1 = PIXEL_UNIT - ex;
                                y1 = PIXEL_UNIT - ey;
                                val1 = (*(data + 3) * ex + *(data)*x1) >>
                                       PIXEL_BITS;
                                val2 = (*(data2 + 3) * ex + *(data2)*x1) >>
                                       PIXEL_BITS;
                                *out++ = (val2 * ey + val1 * y1) >> PIXEL_BITS;
                                val1 = (*(data + 4) * ex + *(data + 1) * x1) >>
                                       PIXEL_BITS;
                                val2 =
                                    (*(data2 + 4) * ex + *(data2 + 1) * x1) >>
                                    PIXEL_BITS;
                                *out++ = (val2 * ey + val1 * y1) >> PIXEL_BITS;
                                val1 = (*(data + 5) * ex + *(data + 2) * x1) >>
                                       PIXEL_BITS;
                                val2 =
                                    (*(data2 + 5) * ex + *(data2 + 2) * x1) >>
                                    PIXEL_BITS;
                                *out++ = (val2 * ey + val1 * y1) >> PIXEL_BITS;
                        }
#else
                        ...
                        for (; col <= last_pix; col++, ex += dx, ey += dy) {
                                ...
                                y1 = ey >> PIX_SHIFT;
                                t = *data++;
                                val1 = ((((OPERAND)(*(data + 2)) - t) * x1) >>
                                        INTER_BITS) +
                                       t;
                                t = *data2++;
                                val2 = ((((OPERAND)(*(data2 + 2)) - t) * x1) >>
                                        INTER_BITS) +
                                       t;
                                *out++ =
                                    (((val2 - val1) * y1) >> INTER_BITS) + val1;
                                t = *data++;
                                val1 = ((((OPERAND)(*(data + 2)) - t) * x1) >>
                                        INTER_BITS) +
                                       t;
                ...
                if (r_mul_tbls_valid) {
                        ...
                        data = r_data +
                               ((size_t)floor(y - 0.5) - 1) * input_row_size +
                               (size_t)floor(x - 0.5);
                        data2 = data + input_row_size;
                        for (; col <= last_pix; col++, ex += dx, ey += dy) {
                                if (ex < 0) {
                        ...
                        for (; col <= last_pix; col++, ex += dx, ey += dy) {
                                ...
                                x1 = ex >> PIX_SHIFT;
                                t = *data++;
                                val1 = ((((OPERAND)(*data) - t) * x1) >>
                                        INTER_BITS) +
                                       t;
                                t = *data2++;
                                val2 = ((((OPERAND)(*data2) - t) * x1) >>
                                        INTER_BITS) +
                                       t;
                                *out++ = (((val2 - val1) * (ey >> PIX_SHIFT)) >>
                                          INTER_BITS) +
                                         val1;
                        }
#endif
 ...
 * IR_Rotate_Get_Row
 */
void _il_Rotate_Get_Row(DATA_OUT_PTR out) {
        if (r_rotation_mode == 0)
                _il_Rotate_Get_Row_Mode0(out);
        else
                _il_Rotate_Get_Row_Mode1(out);
}

void _il_Rotate_Image_End() { Free_All_Buffers(); }
