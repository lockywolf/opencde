...
/*  /ilc/ilycbcr.c : Code for handling IL_YCBCR images, including
    conversions to/from RGB, and subsampling - upsampling is in
   /ilc/ilupsample.c
*/
#include <stdlib.h>
...
typedef struct {
        ...
        ilSubsampleShiftRec
            shift[NSUBSAMPLES]; /* upsample mul as shift value */
...
/*  -------------------------- ilExecuteSubsample
 * ------------------------------- */
/*  Execute(): subsample as necessary "pPriv->nSamples" planes of the source
   image. The source image must be planar, byte/pixel.
*/
static ilError ilExecuteSubsample(ilExecuteData *pData, long dstLine,
        ...
        /*  Loop on samples (components), upsample/translate each plane
           separately. Note that "srcLine" is shifted - indexing into plane
           based on vert subsample.
        */
        for (pSrcPlane = pData->pSrcImage->plane,
            pDstPlane = pData->pDstImage->plane, pShift = pPriv->shift,
            nSamples = pPriv->nSamples;
             nSamples-- > 0; pSrcPlane++, pDstPlane++, pShift++) {
                ...
                /*  Point to src/dst; note dstLine scaled down by vert subsample
                   shift. Set srcRowInc to srcRowBytes scaled up by vertical
                   shift.
                */
                ...
                pDstLine = pDstPlane->pPixels +
                           (dstLine >> pShift->vert) * dstRowBytes;

                switch (
                    pShift->horiz) { /* NOTE: vert shift must be the same! */
                ...
                case 0:
                        ...
                        /*  1 = 1/4 of pixels (half in each dimension). Take the
                           average of 4 pixels: (x,y), (x+1,y), (x,y+1),
                           (x+1,y+1), for each even x,y.
                        */
                case 1:
                        do {
                                pSrc = pSrcLine;
                                ...
                                do {
                                        ...
                                        *pDst++ =
                                            pixel >>
                                            2; /* store 1/4 sum of 4 pixels */
                                } while (--nDstPixelsM1 >= 0);
                        } while (--nDstLinesM1 >= 0);
                        break;

                        /*  2 = 1/16 of pixels (1/4 in each dimension). Take the
                           average of 16 pixels, a block below and to the right
                           of (x,y), x,y = multiple of 4.
                        */
                case 2:
                        do {
                                pSrc = pSrcLine;
                                ...
                                do {
                                        ...
                                        *pDst++ =
                                            pixel >>
                                            4; /* store 1/16 sum of 16 pixels */
                                        pSrc += -srcRowInc + 4; /* back to top
                                                                   line, 4 to
                                                                   right */
                                } while (--nDstPixelsM1 >= 0);
                        } while (--nDstLinesM1 >= 0);
...
/*  Subsample the pipe image which must be a fully upsampled YCbCr image.
    ...
    to the dst format.  *pDes to the _destination_ des, i.e. has subsample
   values.
*/
IL_PRIVATE ilBool _ilSubsampleYCbCr(ilPipe pipe, ilPipeInfo *pInfo,
        ...
        /*  Determine subsample values: only 1 (no subsampling), 2 (1/4 = 1/2 in
           each dimension) or 4 (1/16) allowed; same both vertical/horizontal.
        */
        ...
        for (sample = 0; sample < NSUBSAMPLES; sample++, pShift++, pSample++) {
                ...
                pShift->vert =
                    pShift->horiz; /* same subsampling both dimensions */
        }
        ...
        pSubPriv = (ilSubsamplePrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilSubsamplePrivRec), 0, &srcData, &dstData,
            IL_NPF, IL_NPF, IL_NPF, ilExecuteSubsample, NULL, 0);
        if (!pSubPriv)
                return FALSE;
...
/*  Lookup tables for YCbCr to RGB conversion.  Each table is indexed by Y, Cb
   or Cr, and contains two values; one the upper short (1st) the other in the
   lower (2nd); the upper short values are used to compute green. Y   G cvt: Y
   factor = (1 - Lb - Lr)/Lg      scale for refBlack/White Cb  G cvt: Cb factor
   = -Lb*(2 - 2*Lb)/Lg    Cb scale * (2 - 2 * Lr) Cr  G cvt: Cr factor = -Lr*(2
   - 2*Lr)/Lg    Cr scale * (2 - 2 * Lb)
*/
typedef struct {
...
*/
static ilError ilDestroyTransTable(ilYCbCrToRGBPrivPtr pPriv) {
...
/*  Execute(): convert _planar_ src YCbCr to _pixel_ dst RGB.
    NOTE: the YCbCr image must have been upsampled, i.e. thru
   ilExecuteUpsample().
*/
static ilError ilExecutePlanarYCbCrToRGB(ilExecuteData *pData, long dstLine,
                                         long *pNLines) {
        ...
        /*  Point pY/Cb/CrLine to 1st line in src planes; pDstLine to 1st line
           in dst. Get bytes/row for each of them.
        */
                ...
                plRClipped:
                        *pDst++ = pixel; /* store red */
                        ...
                        if (pixel >> 8)
                                goto plClipG;
                plGClipped:
                        *pDst++ = pixel; /* store green */
                        pixel = Y + ((short)CbTrans);
                        if (pixel >> 8)
                                goto plClipB;
                plBClipped:
                        *pDst++ = pixel; /* store blue */
                } while (--nPixelsM1 >= 0);
        } while (--nLinesM1 >= 0);

        return IL_OK;

        /*  goto point for when R, G or B out of range: clamp pixel and go back
           and continue. This is done to avoid taking a branch in the most
           common case: not out of range.
        */
plClipR:
        if (pixel > 255)
                pixel = 255;
...
/*  Execute(): convert _pixel_ src YCbCr to _pixel_ dst RGB.
   ...
   ilExecuteUpsample().
*/
static ilError ilExecutePixelYCbCrToRGB(ilExecuteData *pData, long dstLine,
                                        long *pNLines) {
        ilPtr pSrcLine, pDstLine;
        register ilRGBTransPtr pTrans;
        register ilPtr pSrc, pDst;
...
*/
static ilRGBTransPtr ilGetYCbCrConvertTable(ilImageDes *pDes) {
        register ilRGBTransPtr pTrans;
        ...
        /*  If luma and refBlack/White info same as std IL_DES_YCBCR, then reuse
            the table at _ilStdYCbCrTransPtr; create it here if does not exist
           yet.
        */
        ...
        /*  Build translate tables.  First, from pg 3 of Appendix O of TIFF
           spec: R = Cr * (2 - 2 * Lr) + Y B = Cb * (2 - 2 * Lb) + Y G = (Y - Lb
           * B - Lr * R) / Lg Get green in terms of Cb and Cr: G = (Y - Lb*B -
           Lr*R) / Lg G = (Y - Lb*(Cb * (2 - 2*Lb) + Y) - Lr*(Cr * (2 - 2*Lr) +
           Y)) / Lg G = (Y - Lb*Cb*(2 - 2*Lb) - Lb*Y - Lr*Cr*(2 - 2*Lr) - Lr*Y)
           / Lg G = (Y - Lb*Y - Lr*Y - Cb*Lb*(2 - 2*Lb) - Cr*Lr*(2 - 2*Lr)) / Lg
                G = (Y * (1 - Lb - Lr) - Cb*Lb*(2 - 2*Lb) - Cr*Lr*(2 - 2*Lr)) /
           Lg G = Y * (1 - Lb - Lr)/Lg + Cb * -Lb*(2 - 2*Lb)/Lg + Cr * -Lr*(2 -
           2*Lr)/Lg
        */
        if (!pTrans) { /* non-std or std table not build yet */
                ...
                /*  The Y, Cb and Cr components must be scaled up based on
                   refBlack/White. Factor that scaling in everywhere Y/Cb/Cr are
                   looked up. First get the scale and refBlack for each sample.
                */
                for (i = 0; i < 3; i++) {
                        sample[i].refBlack = pYCbCr->sample[i].refBlack;
                        sample[i].scale = ((i == 0) ? 255 : 127) /
                                          (pYCbCr->sample[i].refWhite -
                                           pYCbCr->sample[i].refBlack);
                }

                /*  Calc the Cb/Cr lookup tables, factoring in their scale
                 * factors. */
                Lr = ((double)pYCbCr->lumaRed / (double)10000);
                Lg = ((double)pYCbCr->lumaGreen / (double)10000);
                Lb = ((double)pYCbCr->lumaBlue / (double)10000);

                /*  Y translation: lower is scaled Y, upper is value added to
                 * get green */
                gFactor = (1 - Lb - Lr) / Lg;
                for (i = 0; i < 256; i++) {
                ...
                /*  Cb: lower is added to Y to get blue, upper added to get
                 * green */
                ...
                /*  If creating "std" conversion table for first time, set ptr
                 * to it */
                if (isStd)
...
IL_PRIVATE ilBool _ilConvertYCbCrToRGB(ilPipe pipe, ilPipeInfo *pInfo,
                                       ilImageDes *pDes,
        ...
        /*  Handle planar/pixel src format with different executeFunction, but
           in either case output RGB in 3 byte pixel format.
        */
        ...
        destroyFunction =
            (pTrans == _ilStdYCbCrTransPtr) ? IL_NPF : ilDestroyTransTable;
        ...
        *pDes = *IL_DES_RGB;
        ...
        dstData.constantStrip = FALSE;
        dstData.pPalette = (unsigned short *)NULL;
        pPriv = (ilYCbCrToRGBPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilYCbCrToRGBPrivRec), 0,
            (ilSrcElementData *)NULL, &dstData, IL_NPF, IL_NPF, destroyFunction,
        ...
        /*  Defines for # bits of RGB to dither to. NOTE! however that these are
           not arbitrary; # bits of red must = # bits of green, because the
           pre-dither-mul'd value of "Y" is the same for both.
        */
...
static ilError ilYCbCr2DitherInit(ilYCbCr2DitherPrivPtr pPriv,
...
static ilError ilYCbCr2DitherDestroy(ilYCbCr2DitherPrivPtr pPriv) {
        if (pPriv->pPalette)
...
*/
static ilError ilYCbCr2DitherExecute(ilExecuteData *pData, long dstLine,
                                     long *pNLines) {
...
#define YCBCR2_KERNEL_SIZE 8 /* size of dither kernel used */
        ...
        /*  This filter handles a pipe image of YCbCr subsampled by 2 in Cb/Cr
           only. The # of lines of Cb/Cr is therefore 1/2 the # of lines of Y.
            Note: can only handle images with even width/height; checked
           elsewhere.
        */
        pPlane = pData->pSrcImage->plane;
        YRowBytes = pPlane->nBytesPerRow;
        ...
        /*  Do 4 pixels at a time, using 4 Ys (below L, L, below R, R) for each
           Cb,Cr pair. For each Y, do one pixel: get "comp" (RGB) as done in
           other YCbCr->RGB filters, except that all values are pre-multiplied
           by what the dither tables would yield if the lookup to _ilMul4/8[]
           was done.  "pixel" is then used as an index into pTranslate table to
           get pixel to write to dst. See comments in ilGetYCbCrDitherTable()
           for details.
        */
        while (nLinesDiv2-- > 0) {
                pY = pYLine;
                pYLine += YRowBytes
                          << 1; /* skip 2 lines; read 2 lines each loop */
                pCb = pCbLine;
                pCbLine += CbRowBytes;
                pCr = pCrLine;
                pCrLine += CrRowBytes;
                pDst = pDstLine;
                pDstLine += dstRowBytes
                            << 1; /* skip 2 lines; write 2 lines each loop */
                nPixelsDiv2M1 = halfWidthM1;

                /*  Point to even row within kernel based on "y"; point to end
                 * of row */
                ...
                do {
                        ...
                        CbTrans >>= 16;
                        CbTrans +=
                            CrTrans; /* add Cb, Cr contributions to green */
                        ...
                        pixel = (Y + Cr) >> 8;
                        if (pixel >> YCBCR2_NR_BITS)
                                goto d0ClipR;
                d0RClipped:
                        comp = ((YTrans >> 16) + CbTrans + kernel) >> 8;
                        if (comp >> YCBCR2_NG_BITS)
                                goto d0ClipG;
                d0GClipped:
                        pixel = (pixel << YCBCR2_NG_BITS) + comp;
                        comp = (Y + Cb) >> 8;
                        if (comp >> YCBCR2_NB_BITS)
                                goto d0ClipB;
                d0BClipped:
                        pixel = (pixel << YCBCR2_NB_BITS) + comp;
                ...
                d3BClipped:
                        ...
                        *pDst++ = pTranslate[pixel];
                        ...
                        /*  If pKernel off end of row, reset back (-8) to
                         * beginning of row */
        ...
        */
d0ClipR:
        if (pixel > YCBCR2_MAX_R)
                pixel = YCBCR2_MAX_R;
        else
                pixel = 0;
        goto d0RClipped;
d0ClipG:
        if (comp > YCBCR2_MAX_G)
                comp = YCBCR2_MAX_G;
        else
                comp = 0;
        goto d0GClipped;
d0ClipB:
        if (comp > YCBCR2_MAX_B)
                comp = YCBCR2_MAX_B;
        else
                comp = 0;
        goto d0BClipped;
...
*/
static void ilSetupYCbCrDitherTable(register ilRGBTransPtr pTrans,
                                    ilImageDes *pDes) {
        ilYCbCrInfo *pYCbCr;
        double Lr, Lg, Lb, scaledY;
        ...
        /*  This function sets up the given translation table, similar to
            ilGetYCbCrConvertTable(); see that function for more details.  The
           difference is that the multiplications done for dithering (i.e. by
           table lookup into _ilMul4/8[]) are done into the values stored in the
           table. For example, red is obtained from Y and Cr by: red =
           (short)YTrans[Y] + (short)CrTrans[Cr]         (1) then red is
           dithered down to 4 levels by: red = (pMul4[red] + kernel) >> 8;
           (2) where "pMul4[i]" = i * (nLevels(4) - 1) * 256 / 255"; see
           /ilc/ildither.c But if the values in the low-order short of YTrans
           and CrTrans are premultiplied by "(nLevels - 1) * 256 / 255" then the
           lookup into pMul4[] can be skipped, and step (2) above can be
           skipped.
        */
        pYCbCr = &pDes->typeInfo.YCbCr;
...
/*  Does conversions of some forms of YCbCr to dithered IL_PALETTE.
    Returns "true" if conversion handled, else "false" if not handled or if
   error. The pipe image must be an uncompressed YCbCr image.
*/
IL_PRIVATE ilBool _ilDitherYCbCr(ilPipe pipe, ilPipeInfo *pInfo,
                                 ilImageDes *pDes, ilImageFormat *pFormat,
                                 int option, ilConvertToPaletteInfo *pData) {
        ilSrcElementData srcData;
        ilDstElementData dstData;
        ilPtr pTranslate;
        ilYCbCr2DitherPrivPtr pPriv;
        ...
        /*  Check for case handled here: 8x8 area dither, levels 484 specified;
           pipe image is 8 bit/comp YCbCr, subsampled by 2, even width and
           height.
        */
        if (((option != IL_CONVERT_TO_PALETTE) && (option != 0)) || !pData ||
            (pData->method != IL_AREA_DITHER) || (pData->levels[0] != 4) ||
            (pData->levels[1] != 8) || (pData->levels[0] != 4) ||
            (pData->kernelSize != 8) ||
        ...
        /*  If mapImage given, validate it and point to its pixels.  If not
           given, pTranslate becomes null and will point to "identity" mapImage
           below.
        */
        ...
        /*  alloc pPalette unless not palette dst; init with ramp if not
         * choosing */
        ...
        if (pData->dstType == IL_PALETTE) {
                ...
                pPalette = (unsigned short *)IL_MALLOC_ZERO(
                    3 * 256 * sizeof(unsigned short));
                if (!pPalette)
                        return ilDeclarePipeInvalid(pipe, IL_ERROR_MALLOC);
                pPal = pPalette;
                for (red = 0; red < pData->levels[0]; red++) {
                        ...
                        for (green = 0; green < pData->levels[1]; green++) {
                                greenLevel =
                                    65535 * green / (pData->levels[1] - 1);
                                for (blue = 0; blue < pData->levels[2];
                                     blue++) {
                                        blueLevel = 65535 * blue /
                                                    (pData->levels[2] - 1);
                                        pPal[0] = redLevel;
                                        pPal[256] = greenLevel;
                                        pPal[512] = blueLevel;
        ...
        */
        ...
        pPriv = (ilYCbCr2DitherPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilYCbCr2DitherPrivRec), 0, &srcData,
            &dstData, ilYCbCr2DitherInit, IL_NPF, ilYCbCr2DitherDestroy,
        ...
        /*  # of bits of fixed point precision for interim values during
         * conversion */
#define PR2Y 16

/*  Private for RGB to YCbCr filter. For each component to be calculated
   (Y/Cb/Cr), a table indexed by the src component (R/G/B) which gives the
   contribution for that src component, in fixed point: <32-PR2Y bits>.<PR2Y
   bits>
*/
typedef struct {
 ...
 */
static ilError ilExecuteRGBToYCbCr(ilExecuteData *pData, long dstLine,
                                   long *pNLines) {
        ...
        */
        ...
        /*  Load component refBlack values, plus 1/2 shift (precision) amt for
           rounding into values, shifted up so it can be added before right
           shift.
        */
        ...
        do {
                ...
                do {
                        ...
                        pixel = (pPriv->YR[R] + pPriv->YG[G] + pPriv->YB[B] +
                                 YRefBlack) >>
                                PR2Y;
                        if (pixel >> 8)
                                goto YClip;
                YClipped:
                        *pY++ = pixel;
                        ...
                        pixel = (pPriv->CbR[R] + pPriv->CbG[G] + pPriv->CbB[B] +
                                 CbRefBlack) >>
                                PR2Y;
                        if (pixel >> 8)
                                goto CbClip;
                CbClipped:
                        *pCb++ = pixel;
                        ...
                        pixel = (pPriv->CrR[R] + pPriv->CrG[G] + pPriv->CrB[B] +
                                 CrRefBlack) >>
                                PR2Y;
                        if (pixel >> 8)
                                goto CrClip;
        ...
        /*  goto point for when Y/Cb/Cr out of range: clamp pixel and go back
           and continue. This is done to avoid taking a branch in the most
           common case: not out of range.
        */
YClip:
...
/*  Convert from source type (pDes->type) == RGB to YCbCr.
    pFormat points to the source format; on return, *pFormat is updated
    to the dst format. pNewDes points to the dst dest; *pDes to the src (pipe)
   des; on return it is updated to the new descriptor.
*/
IL_PRIVATE ilBool _ilConvertRGBToYCbCr(ilPipe pipe, ilPipeInfo *pInfo,
        ...
        if ((pFormat->nBitsPerSample[0] != 8) ||
            ...
            (pFormat->nBitsPerSample[2] != 8) ||
            (pFormat->sampleOrder != IL_SAMPLE_PIXELS)) {
                if (!ilConvert(pipe, (ilImageDes *)NULL, IL_FORMAT_3BYTE_PIXEL,
                               0, NULL))
                        return FALSE;
        }

        /*  Add filter to write 3 planar YCbCr format image.  Update pDes: std
           YCbCr but with caller's sample refBlack/White and lumaRed/Green/Blue.
        */
        *pDes = *IL_DES_YCBCR;
        pDes->typeInfo.YCbCr.sample[0].refBlack =
            pNewDes->typeInfo.YCbCr.sample[0].refBlack;
        pDes->typeInfo.YCbCr.sample[0].refWhite =
            pNewDes->typeInfo.YCbCr.sample[0].refWhite;
        pDes->typeInfo.YCbCr.sample[1].refBlack =
            pNewDes->typeInfo.YCbCr.sample[1].refBlack;
        pDes->typeInfo.YCbCr.sample[1].refWhite =
            pNewDes->typeInfo.YCbCr.sample[1].refWhite;
        pDes->typeInfo.YCbCr.sample[2].refBlack =
            pNewDes->typeInfo.YCbCr.sample[2].refBlack;
        pDes->typeInfo.YCbCr.sample[2].refWhite =
            pNewDes->typeInfo.YCbCr.sample[2].refWhite;
        ...
        *pFormat = *IL_FORMAT_3BYTE_PLANE;
        ...
        dstData.constantStrip = FALSE;
        dstData.pPalette = (unsigned short *)NULL;
        pPriv = (ilRGBToYCbCrPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilRGBToYCbCrPrivRec), 0,
            (ilSrcElementData *)NULL, &dstData, IL_NPF, IL_NPF, IL_NPF,
            ilExecuteRGBToYCbCr, NULL, 0);
        if (!pPriv)
                return FALSE;
        ...
        /*  See Appendix O pg 3 of TIFF spec.  Use fixed point (16.16) for
            ...
            by multiplying the scaling range into each RGB factor.  The refBlack
           is then added (not shown above).  Each of the values multiplied times
           R/G/B is called "Y/Cb/Cr|R/G/B".
        */
        pYCbCr = &pDes->typeInfo.YCbCr;
        ...
        range =
            (1 << PR2Y) *
            (double)(pYCbCr->sample[0].refWhite - pYCbCr->sample[0].refBlack) /
            255;
        YR = range * Lr + 0.5;
        YG = range * Lg + 0.5;
        YB = range * Lb + 0.5;
        ...
        range =
            (1 << PR2Y) *
            (double)(pYCbCr->sample[1].refWhite - pYCbCr->sample[1].refBlack) /
            127;
        ...
        range =
            (1 << PR2Y) *
            (double)(pYCbCr->sample[2].refWhite - pYCbCr->sample[2].refBlack) /
            127;
        ...
        /*  Build lookup table which is the result of multiplying the above by
           0..255, i.e. the source RGB values.  Using this lookup table is
           equivalent to multiply but a whole lot faster.
        */
        for (i = 0; i < 256; i++) {
                pPriv->YR[i] = YR * i;
                pPriv->YG[i] = YG * i;
