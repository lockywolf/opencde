...
typedef struct _ilFileImage {
        ilFile file;
        struct _ilFileImage *pNext, *pPrev, *pMask, *pChild;
        long width, height;
        long xRes, yRes;
        short page, nPages;
        ilImageDes des;
...
extern ilBool ilInitFile(
    ...
    ilContext context, FILE *stream, long offset, ilBool MSBIsFirst,
...
#if NeedFunctionPrototypes
    ilFile file, unsigned long mustBeZero
#endif
);

extern ilPtr ilReadFileTags(
#if NeedFunctionPrototypes
    ilFileImage fileImage, int nTags, unsigned short *pTagNumbers,
    ilFileTag **ppTags, unsigned long mustBeZero
#endif
);

extern ilBool ilReadFileImage(
#if NeedFunctionPrototypes
    ilPipe pipe, ilFileImage fileImage, ilRect *pSrcRect,
    unsigned long mustBeZero
#endif
);
...
#if NeedFunctionPrototypes
    ilPipe pipe, ilFile file, unsigned int method, ilFileImage fileImage,
    unsigned int compression, ilPtr pCompressionData, long stripHeight,
    long xRes, long yRes, short page, short nPages, int nTags, ilFileTag *pTags,
    unsigned long mustBeZero
#endif
);
