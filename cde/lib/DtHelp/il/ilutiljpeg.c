...
*/
IL_PRIVATE void _ilJPEGDataIn(register const ilImageDes *pDes, long width,
                              long height, register iljpgDataPtr pDst) {
        int maxSubsample;
        ...
        /*  Set IL subsample info into iljpg Hori/VertFactor info, which is
           essentially the inverse of IL subsample data: for each sample/comp,
           hori/vert: iljpgFactor[comp] = maxILSubsample/ILSubsample[comp] NOTE:
           currently only YCbCr data in IL is subsampled!
        */
        if (pDes->type == IL_YCBCR) {
                register const ilYCbCrSampleInfo *pSample =
                ...
                pDst->comp[0].horiFactor =
                    maxSubsample / pSample[0].subsampleHoriz;
                pDst->comp[1].horiFactor =
                    maxSubsample / pSample[1].subsampleHoriz;
                pDst->comp[2].horiFactor =
                    maxSubsample / pSample[2].subsampleHoriz;
                ...
                pDst->comp[0].vertFactor =
                    maxSubsample / pSample[0].subsampleVert;
                pDst->comp[1].vertFactor =
                    maxSubsample / pSample[1].subsampleVert;
                pDst->comp[2].vertFactor =
                    maxSubsample / pSample[2].subsampleVert;
        } else {
...
*/
IL_PRIVATE void _ilJPEGTablesIn(register ilJPEGData *pSrc,
 ...
 */
IL_PRIVATE void _ilJPEGFreeTables(register ilJPEGData *pData) {
        register int i;
...
*/
static ilPtr _ilJPEGCopyHuffmanTable(register ilPtr pSrc) {
        register int i, nBytes;
        register ilPtr pDst;

        /*  Huffman tables are: 16 bytes of # occurrences each # bits, followed
           by bytes for each of those # occurrences.  Size of table = 16 +
           sum(0..15).
        */
...
*/
IL_PRIVATE ilBool _ilJPEGCopyData(register ilJPEGData *pSrc,
        ...
        /*  For each non-null table in *pSrc, malloc space, copy contents, set
         * pDst */
        for (i = 0; i < 4; i++) {
                if (pSrc->QTables[i]) {
                        if (!(pDst->QTables[i] = (ilPtr)IL_MALLOC(64)))
                                goto cdMallocError;
                        bcopy((char *)pSrc->QTables[i],
                              (char *)pDst->QTables[i], 64);
                }
                if (pSrc->DCTables[i])
                        if (!(pDst->DCTables[i] =
                                  _ilJPEGCopyHuffmanTable(pSrc->DCTables[i])))
                                goto cdMallocError;
                if (pSrc->ACTables[i])
                        if (!(pDst->ACTables[i] =
                                  _ilJPEGCopyHuffmanTable(pSrc->ACTables[i])))
                                goto cdMallocError;
        }
...
*/
IL_PRIVATE iljpgError
_ilReallocJPEGEncode(register ilJPEGEncodeStreamPtr pStream, long nBytes) {
        register long offset;

        /*  Allocate space so exactly "nBytes" more bytes will fit in output
           buffer. There are "offset" bytes in the buffer now so allocate offset
           + nBytes.
        */
        if (pStream->pBuffer) {
                offset = pStream->pDst - pStream->pBuffer;
                pStream->pBuffer =
                    (ilPtr)IL_REALLOC(pStream->pBuffer, offset + nBytes);
        } else {
        ...
        /*  Successful realloc: set end of buffer to pBuffer + new size; set
           pDst to pBuffer + offset and return success.
        */
