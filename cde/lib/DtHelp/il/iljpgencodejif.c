...
*/
static iljpgError iljpgPut2Bytes(int value, ILJPG_ENCODE_STREAM stream) {
        ilByte abyte;
        iljpgError error;
 ...
 */
static iljpgError iljpgEncodePutBytes(iljpgPtr pBytes, int nBytes,
...
*/
static iljpgError iljpgWriteHuffmanTable(iljpgPtr pTable, int Th,
                                         ILJPG_ENCODE_STREAM stream) {
        ...
        */
        ...
        /*  Write length = 2 (length) + 1 (Th) + nBytes (16 bytes + variable
         * length) */
...
*/
ILJPG_PUBLIC iljpgError iljpgEncodeJIF(ILJPG_ENCODE_STREAM stream,
                                       iljpgDataPtr pData,
...
#define PUT_2BYTES(_value)                                                     \
        ...
        /*  Write a single frame, single scan JIF image.  The format will be:
            ...
            Note that the order of the tables could be different but this is how
           JFIF files have the tables and other readers may require it. When
           writing length, remember that the length includes the size of the
        ...
        */
        for (index = 0; index < 4; index++) {
                if (pTable = pData->QTables[index]) {
                        ...
                        if (pOffsets)
                                pOffsets->QTables[index] =
                                    ILJPG_ENCODE_OFFSET(stream) - startOffset;
        ...
        /*  Write per-component frame data: id is arbitrary unique id (0..255).
            ...
            component ids be 1..3 for samples 0..2.  So use use "index" + 1;
           also below. Write hori/vert sample factor, Q table selector
           (QTableIndex).
        */
        for (index = 0, pComp = pData->comp; index < pData->nComps;
             index++, pComp++) {
        ...
        /*  Write DHT and Huffman tables for all non-null tables.
            ...
            Add "5" to offsets; 5 bytes (marker, length, Th) written before code
           lengths.
        */
        for (index = 0; index < 4; index++) {
                if (pTable = pData->DCTables[index]) {
                        if (pOffsets)
                                pOffsets->DCTables[index] =
                                    ILJPG_ENCODE_OFFSET(stream) - startOffset +
                                    5;
                        if (error =
                                iljpgWriteHuffmanTable(pTable, index, stream))
                                goto JIFError;
                }
        }
        ...
        for (index = 0; index < 4; index++) {
                if (pTable = pData->ACTables[index]) {
                        if (pOffsets)
                                pOffsets->ACTables[index] =
                                    ILJPG_ENCODE_OFFSET(stream) - startOffset +
                                    5;
                        if (error = iljpgWriteHuffmanTable(pTable, 16 + index,
                                                           stream))
                                goto JIFError;
                }
        }
        ...
        /*  Write per-component frame data: scan component selector, = id in
            frame, = index+1; Td/Ta = DC/AC table selector = DC/AC table index
           (0..1).
        */
        for (index = 0, pComp = pData->comp; index < pData->nComps;
             index++, pComp++) {
