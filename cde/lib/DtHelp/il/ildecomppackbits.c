...
static ilError ilDecompPackbitsExecute(ilExecuteData *pData,
        ...
        register long
            nBytesToGo; /* Number of source image bytes left to unpack for
                           current strip */
        register long nDstLineBytesToGo; /* Number of bytes left to write to
                                            this line of destination     */
        register ilPtr pSrcByte;         /* Pointer to source image data         */
        register ilPtr
            pDstByte;   /* Pointer to destination image data first byte   */
        ilPtr pSrcLine; /* Pointer to source image data first byte of each
                           scanline      */
        ilPtr
                ...
                while (nDstLineBytesToGo > 0) {
                        ...
                        if (count >= 0) {
                                ...
                                do {
                                   ...
                                   128) { /* 128 is a noop for Packbits */
                                count =
                                    -count; /* count is now -1 of real count */
...
/* =============================================================================================================================
        ...
        -------------------- ilDecompPackbits() --------------------
    Main body of code for Packbits decompression.  This includes image
   descriptor parameter error checking and function calls for: strip handler
   initialization, adding the filter element to the pipe, pipe initialization
    and execution, decompression algorithm.....

   =============================================================================================================================
 */

IL_PRIVATE ilBool _ilDecompPackbits(ilPipe pipe, ilPipeInfo *pinfo,
                                    ilImageDes *pimdes,
        ...
        /*  If image has more than one sample/pixel or is not bit or byte/pixel,
         * error */
        ...
        pPriv = (ilDecompPBPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilDecompPBPrivRec), 0,
            (ilSrcElementData *)NULL, &dstdata, IL_NPF, IL_NPF, IL_NPF,
            ilDecompPackbitsExecute, NULL, 0);
        if (!pPriv)
                return FALSE;
