 ...
 */
static ilError ilRCIInit(ilRCIPrivPtr pPriv, ilImageInfo *pSrcImage,
 ...
 */
static ilError ilRCIExecute(register ilExecuteData *pData,
        ...
        *pData->compressed.pNBytesWritten =
            pImage->pStripOffsets[pPriv->strip] - offset;
...
*/
ilBool ilReadImage(ilPipe pipe, ilObject image) {
        register ilImagePtr pImage;
        register ilRCIPrivPtr pPriv;

        /*  Validate that pipe and image are such, have same context and pipe
         * empty */
        ...
        /*  If uncompressed, add producer image and exit.  stripHeight = image
           height, need producer throttle filter before next element is added.
        */
        if (pImage->des.compression == IL_UNCOMPRESSED)
                return _ilAddProducerImage(pipe, pImage, IL_PIPE_IMAGE,
                                           pImage->i.height, pImage->i.height,
                                           TRUE, TRUE);

        /*  Image is compressed: if no stripHeight yet, allocate strip offsets
           array using default strip height.  Add producer image: constant
           strips the size of the image's strip height.  Add producer throttle
           to feed each strip.
        */
        ...
        pPriv = (ilRCIPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilRCIPrivRec), IL_ADD_PIPE_NO_DST,
            (ilSrcElementData *)NULL, (ilDstElementData *)NULL, ilRCIInit,
            IL_NPF, IL_NPF, ilRCIExecute, 0);
        if (!pPriv)
                return FALSE;
        pPriv->pImage = pImage;
