...
#define LZW_MAX_BUFFER_WRITE                                                   \
        8 /* max # of dst bytes written after buffer check */
 ...
 */
static ilError ilCompLZWInit(ilCompLZWPrivPtr pPriv, ilImageInfo *pSrcImage,
        ...
        pPriv->ennodes =
            (ilEnnodePtr)IL_MALLOC(sizeof(ilEnnodeRec) * MAX_NODES);
        if (!pPriv->ennodes) {
 ...
 */
static ilError ilCompLZWCleanup(ilCompLZWPrivPtr pPriv, ilBool aborting) {
        if (pPriv->pString)
                IL_FREE(pPriv->pString);
        if (pPriv->ennodes)
 ...
 */
static void NewEnnode(ilEnnodePtr ennodes, unsigned int node,
                      unsigned int value) {
...
*/
static void ilInitEncodeTable(register ilCompLZWPrivPtr pPriv) {
        register int i;
...
*/
static ilBool ilStringInTable(ilCompLZWPrivPtr pPriv, ilPtr string,
                              unsigned int count,
        ...
        if (count == 1) { /* single character string, always first 256 entries
                             in table */
                *pCurrentNode = byte;
                return TRUE;
        } else {
                if (ennodes[*pCurrentNode].next ==
                    -1) { /* if no next char entry */
                        ennodes[*pCurrentNode].next = pPriv->nextNode;
                        *pCurrentNode = pPriv->nextNode;
                        NewEnnode(ennodes, *pCurrentNode,
                                  byte); /* pPriv->nextNode is always>255 */
                        pPriv->nextNode++;
                        return FALSE;
                } else {
                        *pCurrentNode = ennodes[*pCurrentNode].next;
                        while (TRUE) {
                                ...
                                } else { /* byte must be greater than the
                                            current value */
                                        if (ennodes[*pCurrentNode].right != -1)
                                                *pCurrentNode =
                                                    ennodes[*pCurrentNode]
                                                        .right;
                                        else { /* create new right node */
                                                ennodes[*pCurrentNode].right =
                                                    pPriv->nextNode;
                                                *pCurrentNode = pPriv->nextNode;
                                                NewEnnode(ennodes,
                                                          *pCurrentNode, byte);
...
*/
static ilError ilWriteCode(register ilCompLZWPrivPtr pPriv, unsigned int code) {
        CARD32 newBits, nBits;

        if (pPriv->nextNode <= (512 - 2))
                nBits = 9;
        else if (pPriv->nextNode <= (1024 - 2))
                nBits = 10;
        else if (pPriv->nextNode <= (2048 - 2))
                nBits = 11;
        else
                nBits = 12;
        ...
        if (pPriv->bitCount > 16) {
                ...
                /*  Output 2 bytes; check for room in buffer; realloc if not
                 * room.
                 */
 ...
 */
static ilError ilCompLZWExecute(ilExecuteData *pData, unsigned long dstLine,
        ...
        */
        ...
        while (nLines-- > 0) {
                ...
                while (nSrcBytes-- > 0) {
                        ...
                        if (!ilStringInTable(pPriv, string, stringCount,
                                             &currentNode)) {
                                codeIndex =
                                    (lastNode < 256) ? lastNode : lastNode + 2;
        ...
        /*  Pass the number of bytes written (ptr to next byte - start) to next
         * filter.
         */
        *pData->compressed.pNBytesWritten =
            pPriv->pDst - (pPriv->pDstImage->plane[0].pPixels +
...
*/
IL_PRIVATE ilBool _ilCompressLZW(ilPipe pipe, ilPipeInfo *pInfo,
        ...
        /*  Only pixel order supported - if planar order, try to convert to
         * pixel order.
         */
        des = *pDes;
        format = *pFormat;
        if ((pDes->nSamplesPerPixel > 1) &&
            (pFormat->sampleOrder != IL_SAMPLE_PIXELS)) {
                ...
                if (!ilConvert(pipe, (ilImageDes *)NULL, &format, 0,
                               (ilPtr)NULL))
                        return FALSE;
                ilGetPipeInfo(pipe, FALSE, pInfo, &des, &format);
        }

        /*  Add pipe element: no change to des except now compressed; same
           format. NOTE: pSrcData->constantStrip should be TRUE, from
           ilCompress().
        */
        ...
        pPriv = (ilCompLZWPrivPtr)ilAddPipeElement(
            pipe, IL_FILTER, sizeof(ilCompLZWPrivRec), 0, pSrcData, &dstData,
            ilCompLZWInit, ilCompLZWCleanup, IL_NPF, ilCompLZWExecute, 0);
        if (!pPriv)
                return FALSE;

        /*  Store info in *pPriv.  For nSrcLineBytes, need minimum # of bytes
           uncompressed dst data would take - get bytes/row using rowBitAlign of
           8 (packed).
        */
