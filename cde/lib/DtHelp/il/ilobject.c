...
IL_PRIVATE ilObjectPtr _ilCreateObject(
    ...
    size_t sizeInBytes /* size of object to create */
        ...
        /*  Fill in object header, init refCount to 1 => no one attached to it
         * yet.
         */
...
ilBool ilDestroyObject(ilObject object) {
        register ilObjectPtr pObject;

        /*  Downcount refCount, exit if not zero - object is still attached to
           others. Call object-specific destroy function, free client private if
           present, remove object from the linked list, free the object.
        */
        ...
        if (pObject->p.pPrivate)
                free(pObject->p.pPrivate); /* note: caller used malloc, not
                                              IL_MALLOC() */
...
IL_PRIVATE ilBool _ilObjectInitContext(ilContextPtr pContext) {

        /*  pCOD->head points to the list of objects; init list to null (point
         * to self).
         */
        pContext->objectHead.p.context = (ilContext)pContext;
        pContext->objectHead.pPrev = pContext->objectHead.pNext =
            (ilPtr)&pContext->objectHead;
        return TRUE;
}
...
IL_PRIVATE void _ilObjectDestroyContext(ilContextPtr pContext) {
        register ilObjectPtr pObject, pNextObject, pObjectHead;

        /*  Loop thru and destroy all objects until list points back to head
         * (done).
         */
