...
*/
ilBool ilFeedFromImage(ilPipe pipe, ilObject image, long height,
        ...
        return _ilAddProducerImage(pipe, pImage, IL_PIPE_FEED_IMAGE, height,
                                   stripHeight, constantStrip, TRUE);
}

/*  ------------------------ ilFeedPipe ---------------------------------- */
/*  Public function: see spec.
 */
int ilFeedPipe(ilPipe pipe, long start, long nLines, long nCompBytes,
...
/*  Init() function for element added by ilTapPipe() to the tapped pipe.
    ...
    image, so the offshoot pipe will read directly from the bits in the main
   pipe.
*/
static ilError ilTapInit(ilTapPrivPtr pPriv, ilImageInfo *pSrcImage,
...
*/
static ilError ilTapDestroy(ilTapPrivPtr pPriv) {
...
*/
static ilError ilTapExecute(register ilExecuteData *pData, long dstLine,
                            long *pNLines) {
        ...
        /*  Point feedImage directly to the src data in the pipe.  This was done
           in ilTapInit(), but must be done again in case the pipe image is
           compressed, in which case pPixels for example could change on each
           strip.
        */
        ...
        /*  Feed the src lines or bytes if compressed down the offshoot pipe.
           This is a "no dst" filter, so point next filter to our data: lines or
           bytes (compressed)
        */
        ...
        if (ilFeedPipe(pPriv->pipe, start, *pNLines, nCompBytes, 0) ==
            IL_EXECUTE_ERROR)
...
/*  Public function: see spec.
 */
ilPipe ilTapPipe(register ilPipe pipe, unsigned long mustBeZero,
                 void *mustBeNull) {
        ilContext context;
        ilPipe newPipe;
        ...
        /*  If pipe image is "raw" JPEG, convert to JIF format before adding
           tap. This is because we use the pipe image as a client image, but
           only JIF format client images are supported.
        */
        if ((des.compression == IL_JPEG) &&
            (des.compInfo.JPEG.reserved & IL_JPEGM_RAW)) {
                if (!ilCompress(pipe, IL_JPEG, (ilPtr)NULL, 0, 0))
                        goto tpError;
        }

        /*  Create newPipe, to returned by this function, into which bits will
           be fed as they stream down this pipe.  Create feedImage with which to
           feed newPipe, with des/format of pipe image, but with no buffer yet.
           Feed into newPipe.
        */
        ...
        /*  Add an element to the pipe to be tapped to feed the offshoot pipe;
           init *pPriv. Get a ptr into the feed image data structure, to make
        ...
        */
        ...
        if (!(pPriv = (ilTapPrivPtr)ilAddPipeElement(
                  pipe, IL_FILTER, sizeof(ilTapPrivRec), IL_ADD_PIPE_NO_DST,
                  &srcData, (ilDstElementData *)NULL, ilTapInit, IL_NPF,
                  ilTapDestroy, ilTapExecute, 0)))
                goto tpError;
        ...
        /*  Goto point if error - free objects, post "context->error" to pipe,
         * return null. */
tpError:
        error =
            context->error; /* save error; ilDestroyObject() will overwrite */
        if (newPipe)
