...
const char *_DtHelpFileSuffixList[3] = {DtHelpSDL_VOL_SUFFIX,
                                        DtHelpCCDF_VOL_SUFFIX, NULL};
 ...
 ***********************************************************************/
Boolean _DtHelpFileTraceLinks(char **pPathName) {
        int result = 0;
        char curBuf;
        char *linkPath;
        char *filePath;
        ...
        while (result >= 0) {
                ...
                if (result == -1) {
                        ...
                        /* filePath is not a sym link ==> a real file or
                         * directory */
                        /* so return filePath in caller-owned memory */
                        if (curBuf !=
                            1) /* curBuf == 1 when pPathName is a file */
                        {
                                /*
                                 * pPathName had memory allocated before this
                                 * function was called. only increase the memory
                                 * if needed.
                                 */
                                int pPathName_size = strlen(*pPathName);
                                if (pPathName_size < strlen(filePath))
                                        pPathName_size = strlen(filePath) + 1;
                                *pPathName = (char *)realloc((void *)*pPathName,
                                                             pPathName_size);
                        ...
                        if (linkPath[0] != DIR_SLASH) {
                                ...
                                if (_DtHelpCeStrrchr(filePath, DirSlashStr,
                                                     MB_CUR_MAX, &slash) ==
                                    0) { /* there is a path comonent in
                                          * filePath; use it with linkPath
                                          */
                                        int remaining_size =
                                            MAXPATHLEN + 2 -
                                            (strlen(filePath) -
                                             (strlen(slash) + 1));
                                        strlcpy(++slash, linkPath,
                                                remaining_size);
                                        strlcpy(linkPath, filePath,
                                                MAXPATHLEN +
                                                    2); /* leave result in
                                                           linkPath */
                                }
 ...
 ***********************************************************************/
Boolean _DtHelpFileTraceToFile(char **pPathName, int accessMode,
                               char **pFoundPath) {
        struct stat status;
        ...
        *pFoundPath = NULL;
        ...
        if (access(pathName, accessMode) == 0 && stat(pathName, &status) == 0 &&
 ...
 *
 *****************************************************************************/
void _DtHelpFileGetSearchPaths(char *paths[], Boolean searchHomeDir) {
        ...
        if (searchHomeDir)
                paths[_DtHELP_FILE_HOME_PATH] =
                    pathsSet[_DtHELP_FILE_HOME_PATH];
        else
 ...
 *****************************************************************************/
char *_DtHelpFileLocate(char *type, char *filespec, const char *suffixList[],
                        Boolean searchCurDir, int accessMode) {
        char *loc;
        char *ptr;
        char *pathName;
        char *curPath;
...
#define NUM_BUGS 1
            ...
            && *filespec == '/')               /* and its a / */
        {
                ...
                _DtHelpCeCompressPathname(
                    pathName); /* compress out relative paths */
                ...
                for (pSuffix = suffixList; NULL != *pSuffix; pSuffix++) {
                        strlcpy(eos, (char *)*pSuffix,
                                pathName_size - strlen(pathName));
                        /*recall: _DtHelpFileTraceToFile() requires pathName to
                         * be malloc'd*/
                        if (_DtHelpFileTraceToFile(&pathName, accessMode,
                                                   &foundPath))
        ...
        { /* we're looking at a cwd-relative path; ignore others */
                /*** this is monstrously inefficient--but it shouldn't get
                 * called often ***/

                /* get user's current working directory */
                /* JET - CERT VU#575804 */
                if (getcwd(tmpPath, MAXPATHLEN - 1) == NULL)
                ...
                /* recall: _DtHelpFileTraceToFile() requires pathName to be
                 * malloc'd */
                if (_DtHelpFileTraceToFile(&pathName, accessMode, &foundPath))
                        return foundPath; /* RETURN: found */
        ...
        for (curPathIndex = 0;
             ...
             curPathIndex++) {
                ...
                if (NULL != curPath)
                        do {
                                /* look for next subpath separator and insert
                                 * and EOS if found */
                                if (_DtHelpCeStrchr(curPath, PathSeparator,
                                                    MB_CUR_MAX, &ptr) == 0)
                                        *ptr = EOS;
                                ...
                                for (pSuffix = suffixList, foundPath = NULL;
                                     ...
                                     pSuffix++) {
                                        /* generate the (directory) path using
                                           all the variables and fix it up to
                                           remove the unwanted stuff involving
                                           the filename */
                                        pathName = _DtHelpCeExpandPathname(
                                            curPath, base, type,
                                            (char *)*pSuffix, loc, bugFixSubs,
                                            NUM_BUGS);

                                        if (_DtHelpFileTraceToFile(
                                                &pathName, accessMode,
                                                &foundPath) == False &&
                                            NULL != pathName)
                                                free(pathName);
                                ...
                                /* restore the subpath separator and advance
                                 * past it */
                                if (ptr)
                                        *ptr++ = *PathSeparator;
 ...
 *
 *****************************************************************************/
int _DtHelpCeCheckAndCacheDir(char *dir) {
        int result = ENOTDIR;
        ...
        if (curDir != NULL)
                ...
                result = ENOMEM;
                curDir =
                    (_DtHelpCeDirStruct *)malloc(sizeof(_DtHelpCeDirStruct));
                if (curDir != NULL) {
                        /*
                         * initialize the new entry. I.E. type starts out
                         * ENOTDIR.
                         */
                        *curDir = DefCacheDir;
 ...
 *
 *****************************************************************************/
_DtHelpCeDirStruct *_DtHelpCeGetCachedDirs(void) { return CachedDirs; }
#endif /* not_done */
