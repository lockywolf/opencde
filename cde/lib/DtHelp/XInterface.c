...
static void GetTocIndicator(DtHelpDispAreaStruct *pDAS) {
        ...
        if (_DtHelpDAResolveSpc(pDAS, "C", IsoString, DefFontAttrs, "[rArr  ]",
                ...
                _DtHelpDAResolveSpc(pDAS, "C", IsoString, DefFontAttrs,
                                    "[ndash ]", &(pDAS->toc_indicator),
                                    &(pDAS->toc_width), &(pDAS->toc_height),
                                    &ascent);
}
...
static void ClearTocMarker(DtHelpDispAreaStruct *pDAS, _DtCvFlags flag) {
        ...
        _DtCvUnit clearY =
            pDAS->toc_y + pDAS->decorThickness - pDAS->firstVisible;
        ...
        if (clearWidth > 0 && clearHeight > 0) {
                if (IsSelected(flag))
                        XFillRectangle(XtDisplay(pDAS->dispWid),
                                       XtWindow(pDAS->dispWid), pDAS->invertGC,
                                       clearX, clearY, clearWidth, clearHeight);
                else
                        XClearArea(XtDisplay(pDAS->dispWid),
                                   XtWindow(pDAS->dispWid), clearX, clearY,
                                   clearWidth, clearHeight, False);
        }
}
...
static void DrawTocIndicator(DtHelpDispAreaStruct *pDAS, _DtCvFlags flag) {
        ...
        if (XtIsRealized(pDAS->dispWid) && TocIsOn(pDAS->toc_flag) &&
            ((int)(pDAS->toc_y + pDAS->toc_height)) >=
                ((int)pDAS->firstVisible) &&
            ((int)pDAS->toc_y) <
                ((int)(pDAS->firstVisible + pDAS->dispUseHeight)) &&
            pDAS->toc_indicator) {
                _DtHelpDARegion *regInfo =
                    (_DtHelpDARegion *)pDAS->toc_indicator;

                DADrawSpc(((_DtCvPointer)pDAS), regInfo->handle, 0,
                          pDAS->toc_base, 0, pDAS->toc_y, pDAS->toc_height, 0,
                          0, 0);
        }
}
...
static void DrawTraversal(DtHelpDispAreaStruct *pDAS, _DtCvUnit dst_x,
        ...
        if (pDAS->neededFlags & _DT_HELP_FOCUS_FLAG) {
                ...
                XGetGCValues(dpy, pDAS->normalGC, GCLineStyle | GCForeground,
                             &saved);
                gc_values.line_style = saved.line_style;
                if (HasTraversal(flag)) {
                        if (flag & _DtCvAPP_FLAG2) {
                                XSetForeground(dpy, pDAS->normalGC,
                                               pDAS->searchColor);
                    ...
                    lineX2 + onRightBot - 1 > 0) {
                        ...
                        if (copyY1 < ((int)pDAS->dispHeight) &&
                            copyY1 + ((int)pDAS->lineThickness) >
                                ...
                                XDrawLine(dpy, win, pDAS->normalGC, lineX1,
                                          lineY1, lineX2, lineY1);

                        /*
                         * draw the bottom line
                         */
                        if (copyY2 - ((int)pDAS->lineThickness) <
                                ((int)pDAS->dispHeight) &&
                            copyY2 > ((int)pDAS->decorThickness))
                                XDrawLine(dpy, win, pDAS->normalGC, lineX1,
                                          lineY2, lineX2, lineY2);
                }
                ...
                if (IsTraversalBegin(flag) &&
                    ...
                    copyY2 > ((int)pDAS->decorThickness))
                        XDrawLine(dpy, win, pDAS->normalGC, lineX1, copyY1,
                                  lineX1, copyY2);
                ...
                if (IsTraversalEnd(flag) &&
                    ...
                    copyY1 < ((int)pDAS->dispHeight) &&
                    copyY2 > ((int)pDAS->decorThickness))
                        XDrawLine(dpy, win, pDAS->normalGC, lineX2, copyY1,
                                  lineX2, copyY2);
...
static void MyDrawString(Display *dpy, Drawable d, GC gc, long font_index,
        ...
        if (font_index < 0) {
                ...
                if (wc) {
                         ...
                         * because we need to use a font set to render the
                         * string.
                         */
                        if (image)
                                XmbDrawImageString(dpy, d, mySet, gc, x, y,
                                                   string, length);
                        else
                                XmbDrawString(dpy, d, mySet, gc, x, y, string,
                                              length);
                }
        } else {
                XFontStruct *myFont;
...
static int ResolveFont(_DtCvPointer client_data, char *lang,
        ...
        if (font_attr.weight == _DtHelpFontWeightBold) {
                ...
                if (xrmList[_DT_HELP_FONT_ANGLE] ==
                    XrmStringToQuark("italic")) {
        ...
        if (NULL == xlfdSpec ||
            _DtHelpGetExactFontIndex(pDAS, lang, charset, xlfdSpec, ret_idx) !=
                0)
...
static void DADrawString(_DtCvPointer client_data, _DtCvPointer data,
        ...
        if (byte_len)
                strWidth =
                    _DtHelpDAGetStrWidth(client_data, _DtCvSTRING_TYPE, data);
        ...
        if (strWidth && pDAS->dtinfo &&
            ((new_flags & _DtCvSEARCH_FLAG) ||
             (NULL != pSCD && (pSCD->fg_color || pSCD->bg_color)))) {
                ...
                /*
                 * indicate that we are going to change the foreground and
                 * background.
                 */
                 ...
                 */
                /*
                 * is the search flag set? Set the foreground to the search
                 * color.
                 */
                 ...
                 */
                        ...
                        if (pSCD->fg_pixel == (unsigned long)-1) {
                                if (XAllocNamedColor(dpy, pDAS->colormap,
                                                     pSCD->fg_color, &screen,
                                                     &exact)) {
                                        XSetForeground(dpy, drawGC,
                                                       screen.pixel);
                                        XSetBackground(dpy, fillGC,
                                                       screen.pixel);
                ...
                if (pSCD->bg_color) {
                        ...
                        if (pSCD->bg_pixel == (unsigned long)-1) {
                                if (XAllocNamedColor(dpy, pDAS->colormap,
                                                     pSCD->bg_color, &screen,
                                                     &exact)) {
                                        XSetBackground(dpy, drawGC,
                                                       screen.pixel);
                                        XSetForeground(dpy, fillGC,
                                                       screen.pixel);
                                } else /* the allocation went badly */
                                {
                                        mask1 &= ~GCBackground;
        ...
        if (IsMarkOn(new_flags) && None == pDAS->stipple)
                pDAS->stipple =
                    XCreateBitmapFromData(dpy, win, root_weave_bits,
        ...
        if (image_flag) {
                 ...
                 */
                if (pDAS->dtinfo && IsMarkOn(new_flags) &&
                    None != pDAS->stipple) {
                        /*
                ...
                XFillRectangle(dpy, win, fillGC, eff_x, (int)box_y, eff_width,
                               box_height);
        }

        if (strWidth) {
                MyDrawString(dpy, win, drawGC, (long)font_ptr, (long)x, (long)y,
                             string, byte_len, wc, False, pDAS->font_info);

                if (pDAS->dtinfo && NULL != pSCD) {
                        _DtCvUnit line_y;
                        if (pSCD->hilite_type & hilite_underline) {
                                line_y = box_y + box_height -
                                         pDAS->lineThickness / 2;
                                XDrawLine(dpy, win, drawGC, x, line_y,
                                          x + strWidth, line_y);
                        }
...
static void DADrawSpc(_DtCvPointer client_data, _DtCvPointer spc_handle,
        ...
        _DtHelpDAGetFontMetrics(client_data,
                                pDAS->spc_chars[spcLstIdx].font_ptr, &ascent,
        ...
        do {
                ...
                strInfo.wc = 0;
                strInfo.font_ptr =
                    (_DtCvPointer)(pDAS->spc_chars[spcLstIdx].font_ptr);

                DADrawString(client_data, (_DtCvPointer)&strInfo, x, y, spcStr,
                             i, 0, pDAS->spc_chars[spcLstIdx].font_ptr, NULL,
                             box_x, box_y, box_height, old_flags, new_flags);
...
static void DADrawGraphic(_DtCvPointer client_data, _DtCvPointer graphic_ptr,
        ...
        if (width > 0) {
                ...
                if (y + height >
                    ((int)(pDAS->dispHeight - pDAS->decorThickness)))
         ...
         */
        if (height > 0 && width > 0)
                XCopyArea(dpy, pGS->pix, win, pDAS->normalGC, srcX, srcY, width,
                          height, x, y);
        ...
        if (HasTraversal(new_flags))
                ...
                XDrawLine(dpy, win, pDAS->normalGC, x, y, x,
                          y + box_height - 4);
...
static _DtCvStatus DABuildSelectedString(_DtCvPointer client_data,
        ...
        if (spaceLong > 0)
                spaceNum =
                    (int)(spaceLong / pDAS->charWidth +
                          ((spaceLong % pDAS->charWidth) < pDAS->charWidth / 2
                               ? 0
                               : 1));
        ...
        totalLen =
            byte_len * mbLen + spaceNum + 1 + (IsEndOfLine(flags) ? 1 : 0);
        ...
        if (myString == NULL)
                ...
                myString = (char *)realloc(
                    (void *)myString, (sizeof(char) * (stringLen + totalLen)));
        }
        ...
        if (0 != wc) {
                ...
                for (j = 0; 0 < byte_len; byte_len--) {
                        ...
                        /*
                         * if a valid character, increase the destination
                         * pointer
                         */
                        if (0 <= mbLen)
                                i += mbLen;
        ...
        if (result == 0) {
                ...
                if (spcLstIdx == pDAS->cur_spc) {
                        if (pDAS->cur_spc >= pDAS->max_spc) {
                                ...
                                if (pDAS->cur_spc > 0)
                                        pDAS->spc_chars =
                                            (DtHelpSpecialChars *)realloc(
                                                (void *)pDAS->spc_chars,
                                                (sizeof(DtHelpSpecialChars) *
                                                 pDAS->max_spc));
                                else
                                        pDAS->spc_chars =
                                            (DtHelpSpecialChars *)malloc(
                                                (sizeof(DtHelpSpecialChars) *
                                                 pDAS->max_spc));
                ...
                *ret_width = 0;
                ...
                /*
                 * get the maximum ascent and descent for the font. Set the
                 * return ascent.
                 */
                ...
                *ret_ascent = ascent;
                ...
                do {
                        ...
                        /*
                         * set the height if it is zero or if this is a
                         * multi-height special character.
                         */
                        ...
                        maxWidth =
                            _DtHelpDAGetStrWidth(client_data, _DtCvSTRING_TYPE,
                ...
                result = _DtHelpDAResolveSpc(client_data, lang, charset,
                                             font_attr, DefaultStr, ret_handle,
                                             ret_width, ret_height, ret_ascent);
        return result;
}
...
_DtCvStatus _DtHelpDALoadGraphic(_DtCvPointer client_data, char *vol_xid,
        ...
        pGS->pix = _DtHelpProcessGraphic(
            ...
            &(pDAS->def_pix_height), pDAS->context, pDAS->colormap,
            pDAS->visual, pDAS->foregroundColor, pDAS->backgroundColor,
            fileName, pDAS->media_resolution, &(pGS->width), &(pGS->height),
            &(pGS->mask), &(pGS->pixels), &(pGS->num_pixels));
...
void _DtHelpDADestroyGraphic(_DtCvPointer client_data,
        ...
        if (pGS->num_pixels) {
                ...
                /*
                 * This will only be set to -1 if the original graphic was an X
                 * Pixmap, <filename>.pm or <filename>.xpm.
                 */
                if (pGS->used != -1)
                        XFreeColors(dpy, colormap, pGS->pixels, pGS->num_pixels,
                                    0);
...
void _DtHelpDAGetFontMetrics(_DtCvPointer client_data, _DtCvPointer font_ptr,
        ...
        __DtHelpFontMetrics(pDAS->font_info, (long)font_ptr, ret_ascent,
                            ret_descent, ret_width, ret_super, ret_sub);
}
...
_DtCvUnit _DtHelpDAGetStrWidth(_DtCvPointer client_data,
        ...
        if (font_index < 0) {
                ...
                length = XTextWidth(
                    __DtHelpFontStructGet(pDAS->font_info, font_index),
                    (char *)strInfo->string, strInfo->byte_len);
...
void _DtHelpDARenderElem(_DtCvPointer client_data, _DtCvElemType elem_type,
        ...
        if (_DtCvSTRING_TYPE == elem_type) {
                ...
                DADrawLine(client_data, x, y, lnInfo->x2, lnInfo->y2,
                           lnInfo->width);
                ...
                if (_DtHelpDAGraphic == regInfo->type) {
                        DADrawGraphic(client_data, regInfo->handle,
                                      ...
                                      posInfo->box_width, posInfo->box_height,
                                      old_flags, new_flags);
                } else if (_DtHelpDASpc == regInfo->type) {
                        DADrawSpc(client_data, regInfo->handle, x, y,
                                  posInfo->box_x, posInfo->box_y,
                                  posInfo->box_height, link_type, old_flags,
                                  new_flags);
                }
        }
}
...
_DtCvStatus _DtHelpDABuildSelection(_DtCvPointer client_data,
        ...
        if (_DtCvSTRING_TYPE == elem_type) {
                ...
                result = DABuildSelectedString(
                    client_data, prev_info, strInfo->string, strInfo->byte_len,
                    strInfo->wc, strInfo->font_ptr, space, flags);
        } else if (_DtCvREGION_TYPE == elem_type) {
                _DtHelpDARegion *regInfo = (_DtHelpDARegion *)data;

                if (_DtHelpDASpc == regInfo->type)
                        result =
                            DABuildSelectedSpc(client_data, prev_info,
