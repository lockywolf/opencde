...
static void _DtMessageClose(Widget w, XtPointer client_data, XEvent *event);
static void CloseDefBoxCB(Widget w, XtPointer client_data, XtPointer call_data);
...
static Boolean trusted(char *hv_path) /* Full path to the help volume */
{
        ...
        */
        ...
        if (buf.st_uid != ROOT_USER && buf.st_uid != BIN_USER &&
            buf.st_uid != SYS_USER) {
                return False;
        }
         ...
         *-----------------------------------------------------------------------
         **
         **  The help volume MUST not be writable by group or others to be
         *trusted.
         **
         *-----------------------------------------------------------------------
        */
        ...
        writable =
            (((buf.st_mode & S_IWGRP) == 0) && (buf.st_mode & S_IWOTH) == 0)
                ? True
                : False;
 ...
 *
 ************************************************************************/
static void ExecuteContextCB(Widget w, XtPointer client_data,
                             XtPointer callData) {
 ...
 *
 *****************************************************************************/
static Widget CreateErrorDialog(Widget parent, char *message) {
        Widget button;
        ...
        ok_string =
            XmStringCreateLocalized(((char *)_DTGETMESSAGE(HUSET, 2, "Close")));
        label_string = XmStringCreateLocalized(message);
        title_string =
            XtNewString((char *)_DTGETMESSAGE(HUSET, 5, "Help Error"));

        n = 0;
        XtSetArg(args[n], XmNmessageString, label_string);
        n++;
        XtSetArg(args[n], XmNtitle, title_string);
        n++;
        XtSetArg(args[n], XmNcancelLabelString, ok_string);
        n++;
        XtSetArg(args[n], XmNdefaultButtonType, XmDIALOG_CANCEL_BUTTON);
        n++;
        messageDialog = XmCreateErrorDialog(parent, "errorDialog", args, n);
        ...
        XtAddEventHandler(XtParent(messageDialog), StructureNotifyMask, True,
                          (XtEventHandler)_DtMessageClose,
                          (XtPointer)messageDialog);
 ...
 * Called by:
 *****************************************************************************/
static Widget CreateExecErrorDlg(Widget helpWidget, const char *cmdStr,
                                 Boolean invalidAlias,
                                 _DtHelpCommonHelpStuff *pHelpStuff,
                                 int condition, char *current_hd) {
        ...
        if (invalidAlias) {
                ...
                msg = (char *)_DTGETMESSAGE(
                    HUSET, 14,
                    "The help volume wanted to execute a command as the root "
                    "user, but "
                    "the\n"
                    "home directory of  \"%s\"  ($HOME) does not match the "
                    "root\n"
                    "user's home directory.  This could result in executing "
                    "unexpected\n"
                    "commands.\n\n"
                ...
                msg = (char *)_DTGETMESSAGE(
                    ...
                    "For security reasons, automatic command execution is "
                    "turned off.\n"
                    "The command is:  %s");
        }
        ...
        if (fullmsg) {
                if (condition == MISMATCHING_HOME_DIRS)
                        snprintf(fullmsg, fullmsg_size, msg, current_hd,
                                 cmdStr);
                else
        ...
        pHelpInfo = _DtHelpListAdd(DtHELP_ExecutionPolicy_STR, helpWidget,
                                   pHelpStuff, &pHelpStuff->pHelpListHead);
        XtAddCallback(btn, XmNactivateCallback, _DtHelpCB,
                      (XtPointer)pHelpInfo);
...
Boolean _DtHelpFilterExecCmdStr(
    Widget helpWidget, unsigned char executionPolicy, const char *commandStr,
    char **ret_cmdStr, Boolean *ret_invalidAlias, Boolean *ret_execPermitted,
    Boolean *ret_queryNeeded, char *hv_path) /* Path to the Help Volume */
{
        char *token;
        char *tokenEnd;
        char tokenEndChar;
...
#define ExecAliasCmd "DtHelpExecAlias"
        ...
        *ret_queryNeeded = False;
        ...
        if (DtHELP_EXECUTE_QUERY_UNALIASED == executionPolicy) {
                if (!(trusted(hv_path)))
                        *ret_queryNeeded =
                            True; /* Query ALL non-trusted help volumes */
        } else
                *ret_queryNeeded =
        ...
        if (NULL == token ||
            _DtHelpCeStrCaseCmpLatin1(token, ExecAliasCmd) != 0) {
        ...
        *tokenEnd = tokenEndChar; /* restore the string */
        ...
        if (token) {
                ...
                snprintf(rsrc_name, 200, "%s.%s.%s", appclass, RN_execAlias,
                         token);
                /* e.g. App.ExecutionAlias.<alias> */
                snprintf(rsrc_class, 200, "%s.%s.%s", appclass, RC_execAlias,
                         token);

                /* Get alias command */
                if (XrmGetResource(appDb, rsrc_name, rsrc_class, &reptype,
                                   &value) == True)
                        aliasCommand = value.addr;

                /* build alias resource name and resource */
                /* e.g. app.executionAlias.<alias> */
                snprintf(rsrc_name, 200, "%s.%s.%s", appname, RN_execAlias,
                         token);
                /* e.g. app.ExecutionAlias.<alias> */
                snprintf(rsrc_class, 200, "%s.%s.%s", appname, RC_execAlias,
                         token);

                /* Get alias command and override class with instance, if
                 * defined */
                if (XrmGetResource(appDb, rsrc_name, rsrc_class, &reptype,
                                   &value) == True)
                        aliasCommand = value.addr;
        ...
        if (aliasCommand) {
                *ret_cmdStr = strdup(aliasCommand);
                /* see if query needed; is not if policy is query_unaliased or
                 * all */
                *ret_queryNeeded =
                    !(DtHELP_EXECUTE_QUERY_UNALIASED == executionPolicy ||
        ...
        } else /* the alias wasn't defined */
        {
                ...
                if (token ==
                    tokenEnd) { /* alias wasn't defined and no default command
                                 */
                        *ret_cmdStr = strdup(aliasToken);
                        *ret_invalidAlias = True;
                        *ret_queryNeeded =
                            False; /* no query needed on invalid alias, ever */
                        *ret_execPermitted =
                            False; /* can't exec an invalid alias */
                        ret = False;
                } else { /* alias wasn't defined but a default command */
...
void _DtHelpCeWaitAndProcessEvents(Widget w, _DtHelpCeWaitProc waitProc,
        ...
        do {
                ...
                {
                        /* Get the XEvent - we know it's there! Note that
                           XtAppNextEvent would also process timers/alternate
                           inputs.
                         */
                        XtAppNextEvent(
                            app,
                            &event); /* No blocking, since an event is ready */
                        XtDispatchEvent(&event);
                } else /* Not a XEvent, it's an alternate input/timer event */
                {
                        XtAppProcessEvent(
                            app,
                            mask); /* No blocking, since an event is ready */
                }
#endif /* XTHREADS */
 ...
 *****************************************************************************/
static Boolean WaitForBtnActivatedCB(Widget w, void *clientData) {
 ...
 *
 *****************************************************************************/
static void IdentifyActivatedBtnCB(Widget w, XtPointer clientData,
                                   XtPointer callData) {
        ModalMsgDlgCBStruct *pMd = (ModalMsgDlgCBStruct *)clientData;

        /* w must be a XmMessageDialog widget */
        if (pMd->okBtn == w) {
                pMd->activatedBtnId = XmDIALOG_OK_BUTTON;
                return; /* RETURN */
        }
        if (pMd->cancelBtn == w) {
                pMd->activatedBtnId = XmDIALOG_CANCEL_BUTTON;
                return; /* RETURN */
        }
        if (pMd->helpBtn == w) {
                pMd->activatedBtnId = XmDIALOG_HELP_BUTTON;
                return; /* RETURN */
        }
 ...
 *    useQueryDialog:   use a dialog to query user whether to exec, if not
 *allowed pHelpStuff:       ptr to the HelpStuff structure of the help widget
...
int _DtHelpFilterExecCmd(Widget helpWidget, const char *commandStr,
        ...
        goodCmd = _DtHelpFilterExecCmdStr(
            helpWidget, executionPolicy, commandStr, &filteredCmdStr,
            &invalidAlias, &execPermitted, &queryNeeded, hv_path);
        ...
        msgDlg = CreateExecErrorDlg(helpWidget, filteredCmdStr, invalidAlias,
                                    pHelpStuff, NO_CONDITION, "");
        ...
        labelString = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(HUSET, 10, "Execute Anyway")));
        /* give the right title to the Cancel button */
        labelString2 = XmStringCreateLocalized(
            ((char *)_DTGETMESSAGE(HUSET, 11, "Don't Execute")));
        n = 0;
        XtSetArg(args[n], XmNokLabelString, labelString);
        n++;
        XtSetArg(args[n], XmNcancelLabelString, labelString2);
        n++;
        ...
        /* We put an activate callback on the DontExecute and ExecuteAnyway
           buttons and wait until a button is pressed.  */
        noexecBtn = XmMessageBoxGetChild(msgDlg, XmDIALOG_CANCEL_BUTTON);
        XtAddCallback(noexecBtn, XmNactivateCallback, IdentifyActivatedBtnCB,
                      (XtPointer)&msgDlgCBStruct);

        execBtn = XmMessageBoxGetChild(msgDlg, XmDIALOG_OK_BUTTON);
        XtManageChild(execBtn); /* re-manage the button */
        XtAddCallback(execBtn, XmNactivateCallback, IdentifyActivatedBtnCB,
                      (XtPointer)&msgDlgCBStruct);
        ...
        _DtHelpCeWaitAndProcessEvents(msgDlg, WaitForBtnActivatedCB,
                                      &msgDlgCBStruct.activatedBtnId);
...
void _DtHelpExecFilteredCmd(Widget helpWidget, char *commandStr,
        ...
        Boolean diff_home_dirs = False; /* True ==> $HOME is different from */
                                        ...
                                        /*
                                           getpw{uid,nam}_r routines fail on IBM platform when search password
                                           info                                 via NIS (yellow pages). However, in the case of root, we'll
                                           assume that                                 the password info is in /etc/passwd. If this is not the
                                           case, the                                 following code can fail on IBM platform when XTHREADS and
                                           XUSE_MTSAFE_API                                 are defined.
                                        */
                                        /*
        ...
        hv_path =
            _DtHelpFileLocate(DtHelpVOLUME_TYPE, pDisplayStuff->helpVolume,
                              _DtHelpFileSuffixList, False, R_OK);

        /* The desired and intended behaviour is to use
           _DtHelpFilterExecCmdStr(), but the other code is left here, should a
           change be wished. */
#if 1
        /* This function runs a filter for policy and alias but posts no dialog
         */
        goodCmd =
            _DtHelpFilterExecCmdStr(helpWidget, pDisplayStuff->executionPolicy,
        ...
        execPermitted =
            (goodCmd == 0); /* convert an error int into a Boolean */
        queryNeeded =
            ((pDisplayStuff->executionPolicy == DtHELP_EXECUTE_QUERY_ALL) ||
             (pDisplayStuff->executionPolicy ==
        ...
        if (execPermitted && False == queryNeeded && diff_home_dirs == False) {
                (void)_DtHelpExecProcedure(pHelpStuff->pDisplayArea,
                                           filteredCmdStr);
                free(filteredCmdStr);
                return; /* RETURN */
        }
        ...
        if (diff_home_dirs == True)
                msgDlg = CreateExecErrorDlg(helpWidget, filteredCmdStr,
                                            invalidAlias, pHelpStuff,
                                            MISMATCHING_HOME_DIRS, home_dir);
        else
                msgDlg =
                    CreateExecErrorDlg(helpWidget, filteredCmdStr, invalidAlias,
                                       pHelpStuff, NO_CONDITION, "");

        /*** setup ExecuteAnyway and Help buttons ***/

        if ((diff_home_dirs == True) || (queryNeeded && execPermitted)) {
                /* give the right title to the buttons */
                labelString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(HUSET, 10, "Execute Anyway")));
                /* give the right title to the Cancel button */
 ...
 * Called by:
 *****************************************************************************/
static Widget LocateWidgetId(Display *dpy, int screen, int *statusRet,
                             Widget shellWidget, Cursor cursorIn) {
...
#else
        {
                ...
                if (0 == DfltOnItemCursor) {
                        ...
                        pixmap = XCreateBitmapFromData(
                            dpy, RootWindowOfScreen(XtScreen(shellWidget)),
                            bits, width, height);

                        maskPixmap = XCreateBitmapFromData(
                            dpy, RootWindowOfScreen(XtScreen(shellWidget)),
                            maskBits, width, height);

                        xcolors[0].pixel =
                            BlackPixelOfScreen(ScreenOfDisplay(dpy, screen));
                        xcolors[1].pixel =
                            WhitePixelOfScreen(ScreenOfDisplay(dpy, screen));

                        XQueryColors(dpy,
                                     DefaultColormapOfScreen(
                                         ScreenOfDisplay(dpy, screen)),
                                     xcolors, 2);

                        DfltOnItemCursor = XCreatePixmapCursor(
                            dpy, pixmap, maskPixmap, &(xcolors[0]),
                            &(xcolors[1]), xHotspot, yHotspot);
                ...
                /* Grab the pointer using target cursor, letting it roam all
                 * over */
                status = XtGrabPointer(
                    shellWidget, TRUE, ButtonPressMask | ButtonReleaseMask,
                    GrabModeAsync, GrabModeAsync, None, cursor, CurrentTime);
        if (status != GrabSuccess) {
                XmeWarning(
                    shellWidget,
                    (char *)_DTGETMESSAGE(HUSET, 3,
                                          "Internal Error: Could not grab the "
                                          "mouse\nDtHelpReturnSelectedWidget "
                                          "aborted.\n"));
        ...
        status = XtGrabKeyboard(shellWidget, False, GrabModeAsync,
                                GrabModeAsync, CurrentTime);
        if (status != GrabSuccess) {

                XtUngrabPointer(shellWidget, CurrentTime);
                XmeWarning(shellWidget,
                           (char *)_DTGETMESSAGE(
                               HUSET, 4,
                               "Internal Error: Could not grab the "
                               "keyboard\nDtHelpReturnSelectedWidget() "
        ...
        while (notDone) {
                ...
                if (!(mask & XtIMXEvent)) /* Not a XEvent, it's an alternate
                                             input/timer event */
                        XtAppProcessEvent(
                            app,
                            mask); /* No blocking, since an event is ready */
...
XTranslateCoordinates(dpy, parent, target_win, x, y, &new_x, &new_y, &sub);
x = new_x;
y = new_y;
 ...
 *
 *****************************************************************************/
static Boolean RememberDir(String path) {
        int result = 0;
        char *ptr;
        struct stat buf;
        ...
        if (result == 0 && access(path, R_OK) == 0 && stat(path, &buf) == 0 &&
            S_ISREG(buf.st_mode))
                return True;
 ...
 *
 *****************************************************************************/
Boolean _DtHelpResolvePathname(Widget widget, char **io_fileName,
        ...
        if (newPath != NULL) /* We have a valid volume file so open it */
        {
                ...
                if (_DtHelpOpenVolume(newPath, io_volumeHandle) >= 0) {
                        ...
                        *io_fileName = newPath;
                         ...
                         * We used to set it to null here now we just return
                         * what came in
                         */
                        /* later NOTE: this seems strange, since we have closed
                           the old volume, invalidating io_fileName */
                        XtFree(newPath);
        ...
        {
                /* ERROR; leave io_fileName untouched on error
                 * We used to set it to null here now we just return what came
                 * in
                 */
 ...
 *
 *****************************************************************************/
Boolean _DtHelpExpandHelpVolume(Widget w, _DtHelpDisplayWidgetStuff *display,
        ...
        validPath = _DtHelpResolvePathname(
            (Widget)w, &print->printVolume, &display->volumeHandle,
            help->sysVolumeSearchPath, help->userVolumeSearchPath);
        ...
        if (!validPath)
                ...
                validTopic =
                    _DtHelpCeGetTopTopicId(display->volumeHandle, &topLevelId);
 ...
 *
 *****************************************************************************/
char *_DtHelpParseIdString(char *specification) {
 ...
 *
 *****************************************************************************/
char *_DtHelpParseAccessFile(char *specification) {
        char *pAccessFile = NULL;
        char *tmpSpec = NULL;
        char *returnStr = NULL;
        char *strtok_ptr;
 ...
 *
 *****************************************************************************/
int DtHelpReturnSelectedWidgetId(Widget parent, Cursor cursor, Widget *widget) {
 ...
 *
 *****************************************************************************/
void _DtHelpTopicListAddToHead(char *locationId, XmString topicTitle,
                               int topicType, int maxNodes, char *accessPath,
                               DtTopicListStruct **pHead,
                               DtTopicListStruct **pTale, int *totalNodes,
                               int scrollPosition) {
        ...
        if (*pHead == NULL) {
                ...
                /* Make sure or totalNodes counter is correct, e.g. force it to
                 * 1 */
                *totalNodes = 1;
        } else { /* We have a list so add the new topic to the top */
        ...
        /* If we have reached our maxNodes remove a node from the end of our
         * list */
 ...
 *
 *****************************************************************************/
void _DtHelpTopicListDeleteHead(DtTopicListStruct **pHead,
                                DtTopicListStruct **pTale, int *totalNodes) {
        DtTopicListStruct *pTemp = NULL;
 ...
 *
 *****************************************************************************/
XtCallbackProc _DtHelpMapCB(Widget w, XtPointer client_data,
                            XtPointer call_data) {
        Arg args[2];
        Widget parent;
 ...
 *
 *****************************************************************************/
XtCallbackProc _DtHelpMapCenteredCB(Widget w, XtPointer client_data,
                                    XtPointer call_data) {
        Arg args[2];
        Widget parent;
 ...
 *
 ****************************************************************************/
void _DtHelpDisplayDefinitionBox(Widget parent, Widget **definitionBox,
        ...
        if (*definitionBox == NULL) {
                ...
                /* Create the QuickHelpDialog widget to use as the definition
                 * box */
                closeString = XmStringCreateLocalized(
                    ((char *)_DTGETMESSAGE(HUSET, 2, "Close")));
                n = 0;
                XtSetArg(args[n], DtNhelpVolume, path);
                n++;
                XtSetArg(args[n], DtNhelpType, DtHELP_TYPE_TOPIC);
                n++;
                XtSetArg(args[n], DtNlocationId, locationId);
                n++;
                XtSetArg(args[n], DtNcloseLabelString, closeString);
                n++;
                XtSetArg(args[n], XmNtitle, title);
                n++;
                *definitionBox = (Widget *)DtCreateHelpQuickDialog(
                    parent, "definitionBox", args, n);
                ...
                printWidget = DtHelpQuickDialogGetChild(
                    (Widget)*definitionBox, DtHELP_QUICK_PRINT_BUTTON);
                XtUnmanageChild(printWidget);

                /* We do not want a help button for now so we unmap it */
                helpWidget = DtHelpQuickDialogGetChild(
                ...
                backWidget = DtHelpQuickDialogGetChild(
                    (Widget)*definitionBox, DtHELP_QUICK_BACK_BUTTON);
 ...
 *
 ****************************************************************************/
static void CloseDefBoxCB(Widget w, XtPointer client_data,
                          XtPointer call_data) {
...
void _DtHelpDisplayFormatError(XtPointer displayArea, Widget widget,
        ...
        (void)_DtHelpFormatAsciiStringDynamic(displayArea, userError,
                                              &topicHandle);
...
void _DtHelpSetButtonPositions(
    Widget btnList[], int numBtns, Dimension minFormWidth, Dimension btnMargins,
    Dimension minBetweenBtnSpace) { /* position buttons */
        ...
        /* assumption: the fontList that is returned is not owned by me; don't
         * free */
        ...
        for (i = 0; i < numBtns && NULL != btnList[i]; i++) {
                ...
                btnWidth = XmStringWidth(fontList, labelString) + borderWidth +
                           btnMargins;
        ...
        if (minFormWidth > minWidthWithSpace)
                minFormWidth = minWidthWithSpace;
        ...
        for (entry = XmFontListNextEntry(context); NULL != entry;
             entry = XmFontListNextEntry(context)) {
                ...
                if (XmFONT_IS_FONT == type) {
                        ...
                        if (XGetFontProperty(fontStruct, atom, &value) ==
                            TRUE) {
                                if (gotValue == False) /* haven't gotten any
                                                          prop value yet */
                                {
                                        *ret_propertyValue = value;
                                        gotValue = True;
                                } else /* have a good prop value already...get
                                          the max one */
                                {
                ...
                } else    /* XmFONT_IS_FONTSET */
                {
                        ...
                        numfont =
                            XFontsOfFontSet(fontSet, &font_list, &name_list);
                        for (i = 0; i < numfont; i++) {
                                if (XGetFontProperty(font_list[i], atom,
                                                     &value) == TRUE) {
                                        if (gotValue ==
                                            False) /* haven't gotten any prop
                                                      value yet */
                                        {
                                                *ret_propertyValue = value;
                                                gotValue = True;
                                        } else /* have a good prop value
                                                  already...get the max one */
                                        {
                                                if (value > *ret_propertyValue)
                                                        *ret_propertyValue =
                                                            value;
