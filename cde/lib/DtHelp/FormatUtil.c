 ...
 *
 *****************************************************************************/
int _DtHelpCeAddOctalToBuf(char *src, char **dst, int *dst_size, int *dst_max,
                           int grow_size) {
        char tmp;
        char *dstPtr;
        unsigned long value;

        if (src == NULL || dst == NULL || dst_size == NULL || dst_max == NULL ||
        ...
        if ((value == ULONG_MAX && errno == ERANGE) || value > 255 ||
            value < 1) {
 ...
 *
 *****************************************************************************/
int _DtHelpCeAddCharToBuf(char **src, char **dst, int *dst_size, int *dst_max,
                          int grow_size) {
        char *srcPtr;
        char *dstPtr;

        if (src == NULL || *src == NULL || dst == NULL || dst_size == NULL ||
            dst_max == NULL || (*dst == NULL && (*dst_size || *dst_max))) {
                errno = EINVAL;
                return -1;
        }
 ...
 *
 *****************************************************************************/
int _DtHelpCeAddStrToBuf(char **src, char **dst, int *dst_size, int *dst_max,
                         int copy_size, int grow_size) {
        char *srcPtr;
        char *dstPtr;
 ...
 *
 *****************************************************************************/
int _DtHelpCeGetNxtBuf(BufFilePtr file, char *dst, char **src, int max_size) {
        int leftOver;
        int result;
        ...
        result =
            _DtHelpCeReadBuf(file, &(dst[leftOver]), (max_size - leftOver));
 ...
 *
 *****************************************************************************/
int _DtHelpCeReadBuf(BufFilePtr file, char *buffer, int size) {
        int flag;
 ...
 *
 *****************************************************************************/
char *_DtHelpGetNxtToken(char *str, char **retToken) {
        int len = 1;
        char *start;
        char *token;
        short quote = False;
        ...
        if (retToken)
                *retToken = NULL; /* tested in caller code */
        ...
        if (*str == '\0')
                ...
                if ((MB_CUR_MAX == 1 || mblen(str, MB_CUR_MAX) == 1) &&
                    *str == '\"') {
                        /*
 ...
 *
 *****************************************************************************/
_DtCvSegment *_DtHelpAllocateSegments(int malloc_size) {
        int i;
        _DtCvSegment *newSeg;
        FrmtPrivateInfo *p;
        ...
        if (NULL != newSeg) {
                /*
                 * now allocate the same number of private information
                 * structures
                 */
                p = (FrmtPrivateInfo *)calloc(sizeof(FrmtPrivateInfo),
                                              malloc_size);
                if (NULL != p) {
                        /*
                         * mark the first item as the top block.  Since it will
                         * be attached to the first segment, it marks both the
                         * segment and the private information blocks for later
                         * frees.
                         */
                        p->top_block = True;
                        for (i = 0; i < malloc_size; i++) {
                                /*
                                 * invalidate the link index and attach a
                                 * private information structure to each segment
                                 */
                                newSeg[i].link_idx = -1;
                         ...
                         * free the new segment list and return NULL as an
                         * error.
                         */
                        free(newSeg);
                        newSeg = NULL;
 ...
 *
 *****************************************************************************/
int _DtHelpFmtFindBreak(char *ptr, int mb_len, int *num_chars) {
        int len = 0;
        int numChars = 0;
        int mySize;
        short done = 0;
 ...
 *
 *****************************************************************************/
void _DtHelpLoadMultiInfo(wchar_t **cant_begin_chars, wchar_t **cant_end_chars,
                          short *nl_to_space) {
