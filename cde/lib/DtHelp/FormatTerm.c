...
static void TermMetrics(_DtCvPointer client_data, _DtCvElemType elem_type,
                        _DtCvPointer ret_metrics);
static _DtCvUnit TermStrWidth(_DtCvPointer client_data, _DtCvElemType elem_type,
                              _DtCvPointer data);
static void TermStrDraw(_DtCvPointer client_data, _DtCvUnit x, _DtCvUnit y,
                        const void *string, int byte_len, int wc,
                        _DtCvPointer font_ptr, _DtCvUnit box_x, _DtCvUnit box_y,
                        _DtCvUnit box_height, int link_type,
                        _DtCvFlags old_flags, _DtCvFlags new_flags);
#ifdef XTAG
static _DtCvStatus TermFindGraphic(_DtCvPointer client_data, char *vol_xid,
                                   char *topic_xid, char *file_xid,
                                   char *format, char *method,
                                   _DtCvUnit *ret_width, _DtCvUnit *ret_height,
                                   _DtCvPointer *ret_graphic);
#endif
static void TermGetFont(_DtCvPointer client_data, char *lang, char *charset,
                        _DtHelpFontHints font_attr, _DtCvPointer *ret_font);
static void TermFontMetrics(_DtCvPointer client_data, _DtCvPointer font_ptr,
                            _DtCvUnit *ret_ascent, _DtCvUnit *ret_descent,
                            _DtCvUnit *char_width, _DtCvUnit *ret_super,
                            _DtCvUnit *ret_sub);
static _DtCvStatus TermResolveSpc(_DtCvPointer client_data, char *lang,
                                  char *charset, _DtHelpFontHints font_attr,
                                  char *spc_symbol, _DtCvPointer *ret_handle,
                                  _DtCvUnit *ret_width, _DtCvUnit *ret_height,
                                  _DtCvUnit *ret_ascent);
static void TermRenderElem(_DtCvPointer client_data, _DtCvElemType elem_type,
...
static void TermStrDraw(_DtCvPointer client_data, _DtCvUnit x, _DtCvUnit y,
        ...
        if (wcStr == NULL) {
                ...
                if (pTerm->lines[y] != NULL) {
                        ...
                        /*
                         * this will leave a hole that will be plugged by the
                         * next mbstowc call.
                         */
                        wcStr[length] = 0;
                ...
                if (length > pTerm->wc_num[y]) {
                        pTerm->lines[y] = (wchar_t *)realloc(
                            wcStr, (sizeof(wchar_t) * (length + 1)));
                        if (pTerm->lines[y] != NULL) {
                                register int i;
                                wcStr = pTerm->lines[y];
                                _DtHelpProcessLock();
...
static void TermRenderElem(_DtCvPointer client_data, _DtCvElemType elem_type,
        ...
        if (_DtCvSTRING_TYPE == elem_type) {
                ...
                TermStrDraw(client_data, x, y, strInfo->string,
                            strInfo->byte_len, strInfo->wc, strInfo->font_ptr,
                            posInfo->box_x, posInfo->box_y, posInfo->box_height,
 ...
 *
 *****************************************************************************/
static DtHelpHyperLines *AddHyperToArray(DtHelpHyperLines *array_ptr, int value,
                                         int win_hint, char *link_spec,
                                         char *title) {
        ...
        if (array_ptr) {
                ...
                if (((num + 1) % GROW_SIZE) == 0)
                        array_ptr = (DtHelpHyperLines *)realloc(
                            (void *)array_ptr,
 ...
 *
 *****************************************************************************/
static void DeallocateHyperArray(DtHelpHyperLines *array_ptr) {
 ...
 *
 *****************************************************************************/
static int AddHyperToList(_DtHelpVolumeHdl volume, _DtCvHandle canvas, int i,
                ...
                if (hyperInfo.description == NULL) {
                        ...
                        case _DtCvLinkType_CrossLink:
                                ...
                                if (spec != NULL) {
                                        volName = spec;
                                        id = NULL;
                                        _DtHelpCeStrchr(spec, " ", MB_CUR_MAX,
                                                        &id);
                                        if (id != NULL) {
                                                *id = '\0';
                                                id++;

                                                /* find the volume (volName is
                                                 * malloc'd) */
                                                allocName = _DtHelpFileLocate(
                                                    DtHelpVOLUME_TYPE, volName,
                                                    _DtHelpFileSuffixList,
                                                    False, R_OK);
                                                if (allocName == NULL)
                                                        result = -2;

                                                if (_DtHelpOpenVolume(
                                                        allocName, &newVol) ==
                                                    0)
                                                        useVol = newVol;
                                                else
                        ...
                        case _DtCvLinkType_Execute:
                                ...
                                snprintf(title, title_size, "Execute \"%s\"",
                                         id);
                                break;
                        ...
                        case _DtCvLinkType_ManPage:
                                ...
                                snprintf(title, title_size, "Man Page \"%s\"",
                                         id);
                                break;
        ...
        if (result == -2)
                *ret_list = AddHyperToArray(
                    *ret_list, -(hyperInfo.hyper_type + 1), hyperInfo.win_hint,
                    hyperInfo.specification, strdup("Invalid Link"));
        else if (result >= 0)
                *ret_list = AddHyperToArray(*ret_list, hyperInfo.hyper_type,
                                            hyperInfo.win_hint,
                                            hyperInfo.specification, title);
 ...
 *
 ******************************************************************************/
static TerminalInfo *GetTermInfo(_DtCvHandle canvasHandle) {
 ...
 ******************************************************************************/
/******************************************************************************
 * Function: int _DtHelpTermCreateCanvas (int maxColumns,_DtCvHandle *
 *ret_canvas)
 *
 * Parameters:
...
int _DtHelpTermGetTopicData(_DtCvHandle canvasHandle,
                            _DtHelpVolumeHdl volHandle, char *locationId,
                            char ***helpList, DtHelpHyperLines **hyperList) {
        int result = -1;
        int offset;
        _DtCvUnit maxRows;
        _DtCvUnit maxWidth;
        ...
        if (_DtHelpCeFindId(volHandle, locationId, lockInfo.fd, &fileName,
                            &offset) == 0) {
        ...
        if (result == 1)
                result =
                    _DtHelpCeParseSdlTopic(volHandle, &myUiInfo, lockInfo.fd,
                                           offset, NULL, True, &topic);
        else if (result == 0)
                result = _DtHelpCeFrmtCcdfTopic(volHandle, fileName, offset,
                                                NULL, &myUiInfo, &topic);
        _DtHelpProcessUnlock();
        ...
        if (result != -1) {
                ...
                if (NULL != helpList) {
                        _DtCanvasSetTopic(canvasHandle, topic,
                                          _DtCvIGNORE_BOUNDARY, &maxWidth,
                                          &maxRows, NULL);

                        /*
                         * The 'maxRows' variable is really misnamed; it's
                         * really the 'maxY', and is 0-based.  Thus, the 'lines'
                         * and 'wc_num' arrays need to be 'maxRows+1', in order
                         * to hold all the entries.  Likewise, 'strList' must be
                         * 'maxRows+2', because it also needs to be NULL
                         * terminated.
                         */
                        termInfo->lines = (wchar_t **)malloc(sizeof(wchar_t *) *
                                                             (maxRows + 1));
                        termInfo->wc_num =
                            (size_t *)malloc(sizeof(size_t) * (maxRows + 1));
                        strList =
                            (char **)malloc(sizeof(char *) * (maxRows + 2));
                        if (termInfo->lines == NULL ||
                            termInfo->wc_num == NULL || strList == NULL) {
                                free(fileName);
                                if (termInfo->lines != NULL)
                                        free(termInfo->lines);
                        ...
                        _DtCanvasRender(canvasHandle, 0, 0, maxWidth, maxRows,
                                        _DtCvRENDER_PARTIAL, _DtCvFALSE, NULL,
                                        NULL);
                        ...
                        for (i = 0; i <= maxRows; i++, wcList++, strList++) {
                                if (*wcList == NULL) {
                                        ...
                                        **strList = '\0';
                                        ...
                                        len = (termInfo->wc_num[i] + 1) *
                                              MB_CUR_MAX;
                                        *strList =
                                            (char *)malloc(sizeof(char) * len);
                                        if (*strList != NULL)
                                                wcstombs(*strList, *wcList,
                                                         len);
                                }
                        }
                        ...
                        *strList = NULL;
                         ...
                         */
                        for (i = 0, wcList = termInfo->lines; i <= maxRows;
                             i++, wcList++)
                                if (*wcList != NULL)
                                        free(*wcList);
                        free(termInfo->lines);
                ...
                if (NULL != hyperList) {
                        *hyperList = NULL;
                        for (i = 0;
                             result != -1 && i < myCStruct->link_data->max; i++)
                                result = AddHyperToList(volHandle, canvasHandle,
                                                        i, hyperList);
                }
        } /* if successfully formatted topic */
 ...
 ******************************************************************************/
int _DtHelpGetTopicDataHandles(_DtCvHandle *ret_canvasHandle,
 ...
 *
 *****************************************************************************/
int _DtHelpGetTopicData(char *helpVolume, char *locationId, int maxColumns,
                        char ***helpList, DtHelpHyperLines **hyperList) {
        int result = -1;
        ...
        result = _DtHelpTermGetTopicData(MyCanvas, MyVolume, locationId,
                                         helpList, hyperList);
...
/*****************************************************************************
 * Function: int _DtHelpProcessLinkData (char * ref_volume,DtHelpHyperLines
 **hyperLine, char **ret_helpVolume, char **ret_locationId)
 *
 * Parameters:
 ...
 *
 *****************************************************************************/
int _DtHelpProcessLinkData(char *ref_volume, DtHelpHyperLines *hyperLine,
        ...
        /* check params */
        if (ref_volume == NULL || hyperLine == NULL || ret_helpVolume == NULL ||
            ret_locationId == NULL) {
                errno = EINVAL;
                return -1;
        }
        ...
        *ret_locationId = NULL;
        ...
        {
                volumeName =
                    linkSpec; /* posit that first token is the volume */
                ...
                {
                        /* separate the tokens and advance idToken to first
                         * valid char */
                        *idToken = EOS;
                        idToken++;
                         ...
                         */
                        volumeName = _DtHelpFileLocate(
                            DtHelpVOLUME_TYPE, volumeName,
                            _DtHelpFileSuffixList, False, R_OK);
                        if (volumeName == NULL)
                ...
                if (idToken && volumeName) {
                        *ret_helpVolume = volumeName; /* already allocd by
                                                         _DtHelpFileLocate */
 ...
 *
 *****************************************************************************/
void _DtHelpFreeTopicData(char **helpList, DtHelpHyperLines *hyperList) {
        if (helpList != NULL)
 ...
 *
 *****************************************************************************/
int _DtHelpGetTopicChildren(char *helpVolume, char *topic_id,
                            char ***ret_children) {
        int result = -1;
        _DtHelpVolumeHdl volume = NULL;
        char *path;
        ...
        *ret_children = NULL;
        ...
        if (MyCanvas == NULL) {
                _DtHelpTermCreateCanvas(
                    72, &MyCanvas); /* 72: arbitary value for max columns */
                if (MyCanvas == NULL) {
                        free(path);
                        _DtHelpProcessUnlock();
