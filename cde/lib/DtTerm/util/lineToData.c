...
static char rcs_id[] =
    "$XConsortium: lineToData.c /main/1 1996/04/21 19:20:58 drk $";
...
static void DumpChar(FILE *f, int charNum, charType *charList, int charListMax,
                     rect *rectList, int rectListMax, line *lineList,
                     int lineListMax, line *stippleList, int stippleListMax);
static void DumpGlyphs(FILE *f, char *prefix, GlyphInfo glyphList,
                       int glyphListMax);
static int parseLine(char **bufPtr, line *line);
static int parseRectangle(char **bufPtr, rect *rectangle);
static void order(char *p1, char *p2, signed char *offset1,
                  signed char *offset2);
...
char **argv;
{
        ...
        while (fgets(buffer, sizeof(buffer), f)) {
                ...
                if (!strcmp(c, "char")) {
                        ...
                        if (charListMax > 0) {
                                (void)DumpChar(
                                    stdout, charCount++, charList, charListMax,
                                    rectList, rectListMax, lineList,
                                    lineListMax, stippleList, stippleListMax);
                                lineListMax = 0;
                                rectListMax = 0;
                                stippleListMax = 0;
                        }
                        ...
                        if (glyphListMax + 1 > glyphListSize) {
                                glyphListSize += 10;
                                glyphList = (GlyphInfo)realloc(
                                    glyphList,
                                    glyphListSize * sizeof(GlyphInfoRec));
                        }
                        /* bump count.  We need to remember that this is one
                         * high when we use it as an index...
                         */
                        ...
                        charListMax = 0;
                        charListSize = 10;
                        charList = (charType *)malloc(
                            (unsigned)(charListSize * sizeof(charType)));

                        while (*bufPtr) {
                                /* extend the charList if we need to (the 2 is 1
                                 * for this entry and 1 for the terminating 0...
                                 */
                                if (charListMax + 2 > charListSize) {
                                        charListSize += 10;
                                        charList = (charType *)realloc(
                                            charList,
                                            charListSize * sizeof(charType));
                                }

                                if (0 == (charList[charListMax++] =
                                              ParseChar(&bufPtr))) {
                                        (void)fprintf(stderr,
                                                      "invalid \"char\" "
                                                      "specification:\n");
                                        (void)displayErrorString(stderr, orig,
                                                                 bufPtr);
                                        (void)exit(1);
                                }
                        ...
                        if (rectListMax + 1 > rectListSize) {
                                rectListSize += 10;
                                rectList = (rect *)realloc(
                                    rectList, rectListSize * sizeof(rect));
                        }

                        if (parseRectangle(&bufPtr, &rectList[rectListMax])) {
                                (void)fprintf(
                                    stderr,
                                    "invalid \"rect\" specification:\n");
                        ...
                        if (lineListMax + 1 > lineListSize) {
                                lineListSize += 10;
                                lineList = (line *)realloc(
                                    lineList, lineListSize * sizeof(line));
                        }
                        ...
                        glyphList[glyphListMax - 1].numStipples =
                            stippleListMax;
...
static int parseLine(char **bufPtr, line *line) {
        ...
        /* parse off x1... */
        error = parseCoord(bufPtr, &(line->x1), &(line->x1Offset));

        /* parse off y1... */
        if (!error)
                error = parseCoord(bufPtr, &(line->y1), &(line->y1Offset));

        /* parse off x2... */
        if (!error)
                error = parseCoord(bufPtr, &(line->x2), &(line->x2Offset));

        /* parse off y2... */
        if (!error)
                error = parseCoord(bufPtr, &(line->y2), &(line->y2Offset));

        /* parse off lineWidth... */
        if (!error)
                error = parseCoord(bufPtr, &(line->width), &dummyChar);
        ...
        (void)order(&(line->x1), &(line->x2), &(line->x1Offset),
                    &(line->x2Offset));
        (void)order(&(line->y1), &(line->y2), &(line->y1Offset),
                    &(line->y2Offset));
...
static int parseRectangle(char **bufPtr, rect *rectangle) {
        ...
        /* parse off x1... */
        error = parseCoord(bufPtr, &(rectangle->x1), &(rectangle->x1Offset));
        ...
        /* order x and y coords... */
        (void)order(&(rectangle->x1), &(rectangle->x2), &(rectangle->x1Offset),
                    &(rectangle->x2Offset));
        (void)order(&(rectangle->y1), &(rectangle->y2), &(rectangle->y1Offset),
                    &(rectangle->y2Offset));
...
static void DumpChar(FILE *f, int charCount, charType *charList,
                     int charListMax, rect *rectList, int rectListMax,
                     line *lineList, int lineListMax, line *stippleList,
                     int stippleListMax) {
        int i;
