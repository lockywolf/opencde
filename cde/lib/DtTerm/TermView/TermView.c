...
static char rcs_id[] =
    "$XConsortium: TermView.c /main/1 1996/04/21 19:20:13 drk $";
...
#define ourHeightOffset(w)                                                     \
        ((w)->manager.shadow_thickness + (w)->termview.marginHeight)
#define ourWidthOffset(w)                                                      \
        ((w)->manager.shadow_thickness + (w)->termview.marginWidth)
#define ourHeightAdjustment(w)                                                 \
        (2 * ((w)->manager.shadow_thickness + (w)->termview.marginHeight))
#define ourWidthAdjustment(w)                                                  \
        (2 * ((w)->manager.shadow_thickness + (w)->termview.marginWidth))
...
static void GetBlinkRate(Widget w, int resource_offset, XtArgVal *value) {
        ...
        ac = 0;
        (void)XtSetArg(al[ac], DtNblinkRate, &blinkRate);
        ac++;
        (void)XtGetValues(tw->termview.term, al, ac);
        *value = (XtArgVal)blinkRate;
}

static void GetCharCursorStyle(Widget w, int resource_offset, XtArgVal *value) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg al[1];
        int ac;
        ...
        ac = 0;
        (void)XtSetArg(al[ac], DtNcharCursorStyle, &charCursorStyle);
        ac++;
        (void)XtGetValues(tw->termview.term, al, ac);
        *value = (XtArgVal)charCursorStyle;
}

static void GetVisualBell(Widget w, int resource_offset, XtArgVal *value) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg al[1];
        int ac;
        Boolean visualBell;
        ...
        ac = 0;
        (void)XtSetArg(al[ac], DtNvisualBell, &visualBell);
        ac++;
        (void)XtGetValues(tw->termview.term, al, ac);
        *value = (XtArgVal)visualBell;
}

static void GetMarginBell(Widget w, int resource_offset, XtArgVal *value) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg al[1];
        int ac;
        Boolean marginBell;
        ...
        ac = 0;
        (void)XtSetArg(al[ac], DtNmarginBell, &marginBell);
        ac++;
        (void)XtGetValues(tw->termview.term, al, ac);
        *value = (XtArgVal)marginBell;
}

static void GetNMarginBell(Widget w, int resource_offset, XtArgVal *value) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg al[1];
        int ac;
        int nMarginBell;
        ...
        ac = 0;
        (void)XtSetArg(al[ac], DtNnMarginBell, &nMarginBell);
        ac++;
        (void)XtGetValues(tw->termview.term, al, ac);
        *value = (XtArgVal)nMarginBell;
}

static void GetJumpScroll(Widget w, int resource_offset, XtArgVal *value) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg al[1];
        int ac;
        Boolean jumpScroll;
        ...
        ac = 0;
        (void)XtSetArg(al[ac], DtNjumpScroll, &jumpScroll);
        ac++;
        (void)XtGetValues(tw->termview.term, al, ac);
        *value = (XtArgVal)jumpScroll;
}

static void GetRows(Widget w, int resource_offset, XtArgVal *value) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg al[1];
        int ac;
        short rows;
        ...
        ac = 0;
        (void)XtSetArg(al[ac], DtNrows, &rows);
        ac++;
        (void)XtGetValues(tw->termview.term, al, ac);
        *value = (XtArgVal)rows;
}

static void GetColumns(Widget w, int resource_offset, XtArgVal *value) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg al[1];
        int ac;
        short columns;
        ...
        ac = 0;
        (void)XtSetArg(al[ac], DtNcolumns, &columns);
        ac++;
        (void)XtGetValues(tw->termview.term, al, ac);
...
#ifdef NOTDEF
static XmImportOperator SetRows(Widget w, int resource_offset,
                                XtArgVal *value) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg al[1];
        int ac;
...
static XmImportOperator SetColumns(Widget w, int resource_offset,
                                   XtArgVal *value) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg al[1];
        int ac;
...
static XmSyntheticResource synResources[] = {
    {DtNblinkRate, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.blinkRate), GetBlinkRate, NULL},
    ...
    {DtNvisualBell, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.visualBell), GetVisualBell,
     NULL},
    {DtNmarginBell, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.marginBell), GetMarginBell,
     NULL},
    {DtNnMarginBell, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.nMarginBell), GetNMarginBell,
     NULL},
    {DtNjumpScroll, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.jumpScroll), GetJumpScroll,
     NULL},
    {DtNrows, sizeof(short), XtOffsetOf(struct _DtTermViewRec, termview.rows),
     GetRows, /*SetRows*/ NULL},
    {DtNcolumns, sizeof(short),
     XtOffsetOf(struct _DtTermViewRec, termview.columns), GetColumns,
     /*SetColumns*/ NULL},
};
...
static XtResource resources[] = {
    {DtNemulationId, DtCEmulationId, XmRString, sizeof(char *),
     XtOffsetOf(struct _DtTermViewRec, termview.emulationId), XtRImmediate,
     (XtPointer) "dtterm"},
    {DtNcharCursorStyle, DtCCharCursorStyle, DtRDtTermCharCursorStyle,
     sizeof(unsigned char),
     XtOffsetOf(struct _DtTermViewRec, termview.charCursorStyle), XmRImmediate,
     (XtPointer)DtTERM_CHAR_CURSOR_BOX},
    {DtNblinkRate, DtCBlinkRate, XmRInt, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.blinkRate), XmRImmediate,
     (XtPointer)250},
    {DtNvisualBell, DtCVisualBell, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.visualBell), XmRImmediate,
     (XtPointer)False},
    {DtNmarginBell, DtCMarginBell, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.marginBell), XmRImmediate,
     (XtPointer)False},
    {DtNnMarginBell, DtCNMarginBell, XmRInt, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.nMarginBell), XmRImmediate,
     (XtPointer)10},
    {DtNjumpScroll, DtCJumpScroll, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.jumpScroll), XmRImmediate,
     (XtPointer)True},
    {DtNbaseWidth, DtCBaseWidth, XmRInt, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.baseWidth), XmRImmediate,
     (XtPointer)0},
    {DtNbaseHeight, DtCBaseHeight, XmRInt, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.baseHeight), XmRImmediate,
     (XtPointer)0},
    {DtNwidthInc, DtCWidthInc, XmRInt, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.widthInc), XmRImmediate,
     (XtPointer)0},
    {DtNheightInc, DtCHeightInc, XmRInt, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.heightInc), XmRImmediate,
     (XtPointer)0},
    {DtNtermDevice, DtCTermDevice, XmRInt, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.pty), XmRImmediate,
     (XtPointer)-1},
    {DtNtermDeviceAllocate, DtCTermDeviceAllocate, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.ptyAllocate), XmRImmediate,
     (XtPointer)True},
    {DtNtermSlaveName, DtCTermSlaveName, XmRString, sizeof(char *),
     XtOffsetOf(struct _DtTermViewRec, termview.ptySlaveName), XmRImmediate,
     (XtPointer)NULL},
    {DtNrows, DtCRows, XmRShort, sizeof(short),
     XtOffsetOf(struct _DtTermViewRec, termview.rows), XmRImmediate,
     (XtPointer)0},
    {DtNcolumns, DtCColumns, XmRShort, sizeof(short),
     XtOffsetOf(struct _DtTermViewRec, termview.columns), XmRImmediate,
     (XtPointer)0},
    {XmNtraversalOn, XmCTraversalOn, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, manager.traversal_on), XtRImmediate,
     (XtPointer)True},
    {DtNshadowType, DtCShadowType, XmRShadowType, sizeof(unsigned char),
     XtOffsetOf(struct _DtTermViewRec, termview.shadowType), XmRImmediate,
     (XtPointer)DtSHADOW_OUT},
    {XmNshadowThickness, XmCShadowThickness, XmRHorizontalDimension,
     sizeof(Dimension),
     XtOffsetOf(struct _DtTermViewRec, manager.shadow_thickness), XmRImmediate,
     (XtPointer)1},
    {DtNmarginWidth, DtCMarginWidth, XmRHorizontalDimension, sizeof(Dimension),
     XtOffsetOf(struct _DtTermViewRec, termview.marginWidth), XmRImmediate,
     (XtPointer)7},
    {DtNmarginHeight, DtCMarginHeight, XmRVerticalDimension, sizeof(Dimension),
     XtOffsetOf(struct _DtTermViewRec, termview.marginHeight), XmRImmediate,
     (XtPointer)7},
    {DtNspacing, DtCSpacing, XmRVerticalDimension, sizeof(Dimension),
     XtOffsetOf(struct _DtTermViewRec, termview.spacing), XmRImmediate,
     (XtPointer)4},
    {DtNloginShell, DtCLoginShell, XmRBoolean, sizeof(Boolean),
    ...
    {DtNnewCallback, DtCCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffsetOf(struct _DtTermViewRec, termview.newCallback), XmRPointer,
     (XtPointer)NULL},
    {DtNsubprocessPid, DtCSubprocessPid, XmRInt, sizeof(int),
     XtOffsetOf(struct _DtTermViewRec, termview.subprocessPid), XtRImmediate,
     (XtPointer)-1},
    {DtNsubprocessExec, DtCSubprocessExec, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.subprocessExec), XmRImmediate,
     (XtPointer)True},
    ...
    {DtNsubprocessCmd, DtCSubprocessCmd, XmRString, sizeof(char *),
     XtOffsetOf(struct _DtTermViewRec, termview.subprocessCmd), XtRImmediate,
     (XtPointer)NULL},
    {DtNsubprocessArgv, DtCSubprocessArgv, XtRStringArray, sizeof(String *),
     XtOffsetOf(struct _DtTermViewRec, termview.subprocessArgv), XmRImmediate,
     (XtPointer)NULL},
    {DtNscrollBar, DtCScrollBar, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.scrollBarVisible), XmRImmediate,
     (XtPointer)True},
    {DtNmenuBar, DtCMenuBar, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.menuBarVisible), XmRImmediate,
     (XtPointer)True},
    {DtNmenuPopup, DtCMenuPopup, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermViewRec, termview.menuPopupVisible), XmRImmediate,
     (XtPointer)True},
    {DtNsizeList, DtCSizeList, DtRDtTermTerminalSizeList,
     sizeof(DtTermTerminalSizeList),
     XtOffsetOf(struct _DtTermViewRec, termview.sizeList), XmRString,
     (XtPointer) "80x24 132x24"},
    {DtNuserFont, DtCUserFont, XmRFontList, sizeof(XmFontList),
     XtOffsetOf(struct _DtTermViewRec, termview.fontList), XmRImmediate,
     (XtPointer)NULL},
    {DtNuserFontList, DtCUserFontList, XmRString, sizeof(char *),
     XtOffsetOf(struct _DtTermViewRec, termview.userFontList), XmRImmediate,
     (XtPointer)_DtTermViewMenuDefaultFonts},
};
...
externaldef(dttermviewwidgetclass)
    WidgetClass dtTermViewWidgetClass = (WidgetClass)&dtTermViewClassRec;
...
static Boolean SetSizeInfo(Widget w, int width, int height) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg arglist[20];
         ...
         */
        i = 0;
        (void)XtSetArg(arglist[i], DtNrows, &tw->termview.rows);
        i++;
        (void)XtSetArg(arglist[i], DtNcolumns, &tw->termview.columns);
        i++;
        (void)XtSetArg(arglist[i], DtNwidthInc, &tw->termview.widthInc);
        i++;
        (void)XtSetArg(arglist[i], DtNheightInc, &tw->termview.heightInc);
        i++;
        ...
        if (XtIsShell(XtParent(w))) {
                i = 0;
                (void)XtSetArg(arglist[i], XmNheight, &shellHeight);
                i++;
                XtGetValues(XtParent(w), arglist, i);
                if (shellHeight > 0) {
                        tw->termview.baseHeight =
                            shellHeight -
                ...
                if (height > 0) {
                        tw->termview.baseHeight =
                            height - tw->termview.rows * tw->termview.heightInc;
                }
                /* sanity check on baseHeight... */
        ...
        if (change && XtIsShell(XtParent(w))) {
                ...
                (void)XtSetArg(arglist[i], DtNwidthInc, tw->termview.widthInc);
                i++;
                (void)XtSetArg(arglist[i], DtNheightInc,
                               tw->termview.heightInc);
                i++;
                (void)XtSetArg(arglist[i], DtNbaseWidth,
                               tw->termview.baseWidth);
                i++;
                (void)XtSetArg(arglist[i], DtNbaseHeight,
                               tw->termview.baseHeight);
                i++;
...
static Boolean ResetSizeInfo(Widget w, int width, int height) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Arg arglist[20];
        Boolean change = False;
        int i;
...
/*ARGSUSED*/
static void Initialize(Widget rw, Widget w, Arg *args, Cardinal *num_args) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        int i;
        Arg arglist[20];
        ...
        if ((tw->termview.rows <= 0) || (tw->termview.columns <= 0)) {
                if (XtIsShell(XtParent(w))) {
                        ...
                        i = 0;
                        (void)XtSetArg(arglist[i], XmNgeometry, &geometry);
                        i++;
                        ...
                        if (geometry && *geometry) {
                                ...
                                flags = XWMGeometry(
                                    ...
                                    geometry, "80x24", 0, &hints, &x, &y,
                                    &width, &height, &winGravity);
        ...
        if (tw->termview.menuBarVisible) {
                Debug('T',
                      timeStamp(
                          "DtTermView Initialize() menubar create starting"));
                ...
                Debug('T', timeStamp(
                               "DtTermView Initialize() managing the menubar"));
                ...
                Debug('T',
                      timeStamp(
                          "DtTermView Initialize() menubar create finished"));
        } else {
        ...
        i = 0;
        tw->termview.scrolledWindowFrame = XtCreateManagedWidget(
            "dtTermScrolledWindowFrame", xmFrameWidgetClass, w, arglist, i);
        Debug(
            'T',
            timeStamp(
         ...
         */
        i = 0;
        (void)XtSetArg(arglist[i], XmNscrollingPolicy, XmAPPLICATION_DEFINED);
        i++;
        (void)XtSetArg(arglist[i], XmNvisualPolicy, XmVARIABLE);
        i++;
        (void)XtSetArg(arglist[i], XmNscrollBarDisplayPolicy, XmSTATIC);
        i++;
        tw->termview.scrolledWindow = XtCreateManagedWidget(
            "dtTermScrolledWindow", xmScrolledWindowWidgetClass,
        ...
        /* char *emulationId						*/
        (void)XtSetArg(arglist[i], DtNemulationId, tw->termview.emulationId);
        i++;

        /* unsigned char charCursorStyle */
        (void)XtSetArg(arglist[i], DtNcharCursorStyle,
                       tw->termview.charCursorStyle);
        i++;

        /* int blinkRate */
        (void)XtSetArg(arglist[i], DtNblinkRate, tw->termview.blinkRate);
        i++;

        /* Boolean visualBell		set via setvalues		*/
        (void)XtSetArg(arglist[i], DtNvisualBell, tw->termview.visualBell);
        i++;

        /* Boolean marginBell		set via setvalues		*/
        (void)XtSetArg(arglist[i], DtNmarginBell, tw->termview.marginBell);
        i++;
        ...
        (void)XtSetArg(arglist[i], DtNjumpScroll, tw->termview.jumpScroll);
        i++;
        ...
        /* int baseWidth			computed by DtTermView and
         * xterm*/
        /* int baseHeight			computed by DtTermView and
         * xterm*/
        ...
        tw->termview.term =
            DtCreateTerm(tw->termview.scrolledWindow, "dtTerm", arglist, i);
        Debug('T',
              timeStamp("DtTermView Initialize() DtCreateTerm() finished"));
        ...
        if (tw->termview.menuPopupVisible) {
                Debug('T',
                      timeStamp(
                          "DtTermView Initialize() popupmenu create started"));
                i = 0;
                (void)_DtTermViewCreatePopupMenu(w, tw->termview.term, arglist,
                                                 i);
                Debug('T',
                      timeStamp(
                          "DtTermView Initialize() popupmenu create finished"));
        }
         ...
         */
        (void)XtAddEventHandler(w, (EventMask)EnterWindowMask | LeaveWindowMask,
...
static void Resize(Widget w) {
        ...
        if (tw->termview.menuBar.widget)
                tw->termview.menuBar.managed =
                    XtIsManaged(tw->termview.menuBar.widget);
        if (tw->termview.scrollBar.widget)
                tw->termview.scrollBar.managed =
                    XtIsManaged(tw->termview.scrollBar.widget);
        ...
        if (tw->termview.menuBar.widget &&
            (tw->termview.menuBar.managed =
                ...
                if (XtQueryGeometry(tw->termview.menuBar.widget, &desired,
                                    &preferred) != XtGeometryYes) {
                        /* set the border width and the height to that returned
                         * by the widget XtQueryGeometry() request...
                         */
                 ...
                 */
                (void)XtResizeWidget(tw->termview.menuBar.widget, desired.width,
                                     desired.height, desired.border_width);
         ...
         * border)...
         */
        configWidth = ourWidth(tw) -
                      2 * tw->termview.scrolledWindowFrame->core.border_width;
        configHeight = ourHeight(tw) -
                       2 * tw->termview.scrolledWindowFrame->core.border_width -
                       menuBarHeight;
        if (configWidth <= 0) {
                configWidth = 1;
...
static void Redisplay(Widget w, XEvent *event, Region region) {
        ...
        if (XtIsRealized(w)) {
                ...
                (void)XClearArea(XtDisplay(w), XtWindow(w), 0, 0,
                                 tw->core.width, tw->core.height, False);
                if (tw->termview.menuBar.managed) {
                        (void)XmeDrawShadows(
                            XtDisplay(w), XtWindow(w),
                            tw->manager.top_shadow_GC,
                            tw->manager.bottom_shadow_GC, 0,
                        ...
                        (void)XmeDrawShadows(XtDisplay(w), XtWindow(w),
                                             tw->manager.top_shadow_GC,
                                             tw->manager.bottom_shadow_GC, 0, 0,
...
static Boolean SetValues(Widget cur_w, Widget ref_w, Widget w, ArgList args,
        ...
        if (tw->termview.scrollBarVisible !=
            cur_tw->termview.scrollBarVisible) {
                if (tw->termview.scrollBarVisible) {
                        /* make it visible... */
                        if (!tw->termview.scrollBar.widget) {
        ...
        if (tw->termview.menuBarVisible != cur_tw->termview.menuBarVisible) {
                if (tw->termview.menuBarVisible) {
                        ...
                        if (!tw->termview.menuBar.widget) {
                                /* need to create it first... */
                                tw->termview.menuBar.widget =
                                    _DtTermViewCreatePulldownMenu(w, w, NULL,
                                                                  0);
                                (void)XtManageChild(
                                    tw->termview.menuBar.widget);
                                tw->termview.menuBar.managed = True;
        ...
        if (tw->termview.nMarginBell != cur_tw->termview.nMarginBell) {
                (void)XtSetArg(al[ac], DtNnMarginBell,
                               tw->termview.nMarginBell);
                ac++;
        ...
        if ((tw->termview.rows != cur_tw->termview.rows) ||
            (tw->termview.columns != cur_tw->termview.columns)) {
                ...
                if (tw->termview.sizeDefault.columns <= 0) {
                        tw->termview.sizeDefault.columns =
                            cur_tw->termview.columns;
                }

                (void)XtSetArg(al[ac], DtNcolumns, tw->termview.columns);
                ac++;
                (void)XtSetArg(al[ac], DtNrows, tw->termview.rows);
                ac++;
        ...
        if (tw->termview.fontList != cur_tw->termview.fontList) {
                (void)XtSetArg(al[ac], DtNuserFont, tw->termview.fontList);
                ac++;

                /*DKS: we should change the font for the status line here as
                 * well...
                 */
        }
...
Widget DtCreateTermView(Widget parent, char *name, ArgList arglist,
                        Cardinal argcount) {
        Widget w;

        Debug('T', timeStamp("DtCreateTermView() starting"));
        w = XtCreateWidget(name, dtTermViewWidgetClass, parent, arglist,
                           argcount);
        Debug('T', (void)timeStamp("DtCreateTermView() started"));
        return (w);
}

static XtGeometryResult GeometryManager(Widget child, XtWidgetGeometry *desired,
                                        XtWidgetGeometry *allowed) {
        DtTermViewWidget tw;
        Dimension width;
        Dimension height;
        ...
        if (child == tw->termview.scrolledWindowFrame) {
                ...
                if ((desired->request_mode & CWX) &&
                    (desired->x != child->core.x)) {
                        allowed->x = child->core.x;
                        result = XtGeometryAlmost;
                }
                if ((desired->request_mode & CWY) &&
                    (desired->y != child->core.y)) {
                        allowed->y = child->core.y;
                        result = XtGeometryAlmost;
                }

                /* if we are still in XtGeometryYes mode, then make width and
                 * height changes... DKS: we should probably confirm this with
                 * our parent first!!!...
                 */
                if ((result == XtGeometryYes) &&
                    !(desired->request_mode & XtCWQueryOnly)) {
                        ...
                        /* we need to update our widthInc, heightInc, baseWidth,
                         * and baseHeight before we change our size so that the
                         * window manager will honour them...
                         */
                        ...
                        case XtGeometryAlmost:
                                ...
                                (void)XtMakeResizeRequest(
                                    (Widget)tw, widthReturn, heightReturn,
                                    &ignoreReturn, &ignoreReturn);

                                /* we aren't going to be allowed to resize the
                                 * DtTermView window to the extent that we need
                                 * to, so let's figure out what we can allow...
                                 */
                                if ((desired->request_mode & CWHeight) &&
                                    (height != heightReturn)) {
                                        /* height request was denied.  Back the
                                         * allowed height out...
                                         */
                                        if (tw->termview.menuBar.managed)
                                                heightReturn -= widgetHeight(
                                                    tw->termview.menuBar
                                                        .widget);
                                        allowed->height =
                                            heightReturn -
                                            ourHeightAdjustment(tw);
                                        result = XtGeometryAlmost;
                                }

                                if ((desired->request_mode & CWWidth) &&
                                    (width != widthReturn)) {
                                        allowed->width = widthReturn -
                                                         ourWidthAdjustment(tw);
                                        result = XtGeometryAlmost;
                                }
                                break;
                        ...
                        /* if we are still in XtGeometryYes mode, it is safe to
                ...
                /* if we had ourself resized and get a XtGeometryYes reply, we
                 * need to call our resize method...
                 */
                if (callResize) {
...
static void ChangeManaged(Widget w) {
        ...
        if (tw->termview.menuBar.widget && tw->termview.menuBar.managed) {
                (void)XtConfigureWidget(
                    tw->termview.menuBar.widget, 0, 0,
                    width + ourWidthAdjustment(tw) -
                        2 * tw->termview.menuBar.widget->core.border_width,
        ...
        (void)XtConfigureWidget(
            tw->termview.scrolledWindowFrame, ourWidthOffset(tw),
         ...
         * before we change our size so that the window manager will honour
         * them...
         */
        width += ourWidthAdjustment(tw);
        height += ourHeightAdjustment(tw);
        ...
        switch (XtMakeResizeRequest(w, width, height, &widthReturn,
                                    &heightReturn)) {
        case XtGeometryAlmost:
                /* we didn't get our requested size, so we mey need to
                 * recalculate lines and or columns...
                 */
...
static void CreateScrollBar(Widget w) {
        ...
        tw->termview.scrollBar.widget =
            XtCreateManagedWidget("dtTermScrollBar", xmScrollBarWidgetClass,
                                  tw->termview.scrolledWindow, arglist, i);
        (void)XmScrolledWindowSetAreas(tw->termview.scrolledWindow, (Widget)0,
                                       tw->termview.scrollBar.widget,
                                       tw->termview.term);
        tw->termview.scrollBar.managed = True;
        ...
        *childWidth = widgetWidth(tw->termview.scrolledWindow) -
        ...
        if (tw->termview.scrollBar.widget && tw->termview.scrollBar.managed) {
                ...
                (void)XtSetArg(arglist[i], DtNscrollBarPlacement,
                               &scrollBarPlacement);
                i++;
                (void)XtSetArg(arglist[i], DtNspacing, &scrolledWindowSpacing);
                i++;
                ...
                *childWidth -= scrolledWindowSpacing +
                ...
                if ((XmTOP_LEFT == scrollBarPlacement) ||
                    (XmBOTTOM_LEFT == scrollBarPlacement)) {
 ...
 */
/*ARGSUSED*/
static void SubprocessTerminationCallback(Widget wid, XtPointer client_data,
                                          XtPointer call_data) {
        ...
        if (tw->termview.subprocessTerminationCallback) {
                ...
                /* termviewCB.reason = ?; DKS: this needs to be implemented some
                 * day... */
                termviewCB.pid = termCB->pid;
                termviewCB.status = termCB->status;

                (void)XtCallCallbackList(
                    (Widget)tw, tw->termview.subprocessTerminationCallback,
                    &termviewCB);
        }
}
...
Boolean DtTermViewGetCloneEnabled(Widget w) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
...
/*ARGSUSED*/
void DtTermViewCloneCallback(Widget w, XtPointer client_data,
                             XtPointer call_data) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        ...
        (void)XtSetArg(arglist[i], DtNshadowType, tw->termview.shadowType);
        i++;

        /* XmFontList fontList */
        (void)XtSetArg(arglist[i], XmNfontList, tw->termview.fontList);
        i++;
        ...
        (void)XtSetArg(arglist[i], DtNmarginHeight, tw->termview.marginHeight);
        i++;

        /* Dimension marginWidth */
        (void)XtSetArg(arglist[i], DtNmarginWidth, tw->termview.marginWidth);
        i++;

        /* Dimension spacing */
        (void)XtSetArg(arglist[i], XmNspacing, tw->termview.spacing);
        i++;
        ...
        (void)XtSetArg(arglist[i], DtNscrollBar, tw->termview.scrollBarVisible);
        i++;

        /* Boolean menuBarVisible */
        (void)XtSetArg(arglist[i], DtNmenuBar, tw->termview.menuBarVisible);
        i++;
...
/*ARGSUSED*/
static void closeHelp(Widget w, XtPointer client_data, XtPointer call_data) {
        (void)XtUnmanageChild(w);
}

static void destroyHelp(Widget w, XtPointer client_data, XtPointer call_data) {
        (void)XtDestroyWidget(w);
}

static void hyperlinkCallback(Widget w, XtPointer clientData,
                              XtPointer callData) {
        Arg al[10];
        int ac;
        ...
        case DtHELP_LINK_MAN_PAGE:
                ac = 0;
                (void)XtSetArg(al[ac], DtNhelpType, DtHELP_TYPE_MAN_PAGE);
                ac++;
                (void)XtSetArg(al[ac], DtNmanPage, hyperData->specification);
                ac++;
                (void)XtSetArg(al[ac], DtNcolumns, 80);
                ac++;
                (void)XtSetArg(al[ac], DtNrows, 20);
                ac++;
                (void)XtSetArg(
                    al[ac], XmNtitle,
                    GETMESSAGE(NL_SETN_View, 4, "Terminal - Man Page"));
                ac++;
                quickHelpDialog =
                    DtCreateHelpQuickDialog(w, "helpDialog", al, ac);
#ifdef HPVUE
                (void)XtAddCallback(quickHelpDialog, XmNokCallback, destroyHelp,
                                    (XtPointer)0);
#else  /* HPVUE */
...
void _DtTermViewMapHelp(Widget w, char *volume, char *id) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Widget sw;
        Arg arglist[10];
        int i;
        ...
        i = 0;
        (void)XtSetArg(arglist[i], DtNhelpType, DtHELP_TYPE_TOPIC);
        i++;
        (void)XtSetArg(arglist[i], DtNlocationId, id);
        i++;
        (void)XtSetArg(arglist[i], DtNhelpVolume, volume);
        i++;
        (void)XtSetArg(arglist[i], XmNtitle,
                       GETMESSAGE(NL_SETN_View, 3, "Terminal - Help"));
        i++;
        if (tw->termview.helpWidget) {
                (void)XtSetValues(tw->termview.helpWidget, arglist, i);
        } else {
                tw->termview.helpWidget =
                    DtCreateHelpDialog(w, "helpDialog", arglist, i);
#ifdef BOGUS
#ifdef HPVUE
                ...
                (void)XtAddCallback(tw->termview.helpWidget, DtNcloseCallback,
                                    closeHelp, (XtPointer)0);
                (void)XtAddCallback(tw->termview.helpWidget,
                                    DtNhyperLinkCallback, hyperlinkCallback,
                                    (XtPointer)0);
        }
...
Widget _DtTermViewGetChild(Widget w, _DtTermViewChildType child) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
