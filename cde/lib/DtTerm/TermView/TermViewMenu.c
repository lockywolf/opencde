...
static char rcs_id[] =
    "$XConsortium: TermViewMenu.c /main/4 1996/09/26 16:42:48 samborn $";
...
static void menuBarToggleCallback(Widget w, XtPointer client_data,
                                  XtPointer call_data);
static void cloneCallback(Widget w, XtPointer client_data, XtPointer call_data);
#ifdef HPVUE
...
static void tokenCallback(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        if (client_data) {
...
/*ARGSUSED*/
static void postMenu(Widget w, XtPointer client_data, XEvent *event,
                     Boolean *cont) {
...
static Widget createMenuWidget(WidgetClass widgetClass, char *callbackName,
                               Widget parent, Widget subMenuId, char *label,
                               KeySym mnemonic, char *accelerator,
                               char *acceleratorText, XtCallbackProc callback,
        ...
        (void)XtSetArg(arglist[i], XmNlabelString, string);
        i++;

        /* if a mnemonic was specified, set the mnemonic and mnemonic charset...
         */
        if (mnemonic != NoSymbol) {
                (void)XtSetArg(arglist[i], XmNmnemonic, mnemonic);
                i++;
                (void)XtSetArg(arglist[i], XmNmnemonicCharSet,
        ...
        if (accelerator && *accelerator) {
                (void)XtSetArg(arglist[i], XmNaccelerator, accelerator);
                i++;
        }
        ...
        if (acceleratorText && *acceleratorText) {
                ...
                (void)XtSetArg(arglist[i], XmNacceleratorText,
                               acceleratorString);
                i++;
        }
        ...
        if (subMenuId) {
                (void)XtSetArg(arglist[i], XmNsubMenuId, subMenuId);
                i++;
        }
        ...
        if (!strcmp(callbackName, XmNactivateCallback)) {
                (void)XtAddCallback(w, callbackName, tokenCallback,
                                    strdup(label));
        }
#endif /* TOKEN_CALLBACK */
...
Widget _DtTermViewCreateCascadeButton(Widget parent, Widget subMenuId,
                                      char *label, KeySym mnemonic,
                                      char *accelerator, char *acceleratorText,
                                      XtCallbackProc callback,
                                      XtPointer clientData) {
        return (createMenuWidget(xmCascadeButtonGadgetClass,
                                 XmNactivateCallback, parent, subMenuId, label,
                                 mnemonic, accelerator, acceleratorText,
                                 callback, clientData));
}

Widget _DtTermViewCreatePushButton(Widget parent, char *label, KeySym mnemonic,
                                   char *accelerator, char *acceleratorText,
                                   XtCallbackProc callback,
                                   XtPointer clientData) {
        return (createMenuWidget(xmPushButtonGadgetClass, XmNactivateCallback,
                                 parent, NULL, label, mnemonic, accelerator,
                                 acceleratorText, callback, clientData));
}

Widget _DtTermViewCreateToggleButton(Widget parent, char *label,
                                     KeySym mnemonic, char *accelerator,
                                     char *acceleratorText,
                                     XtCallbackProc callback,
                                     XtPointer clientData) {
        return (createMenuWidget(xmToggleButtonGadgetClass,
                                 XmNvalueChangedCallback, parent, NULL, label,
                                 mnemonic, accelerator, acceleratorText,
                                 callback, clientData));
}

Widget _DtTermViewCreateLabel(Widget parent, char *label) {
        return (createMenuWidget(xmLabelGadgetClass, NULL, parent, NULL, label,
                                 0, NULL, NULL, NULL, NULL));
}

Widget _DtTermViewCreateSeparator(Widget parent, char *label) {
        return (createMenuWidget(xmSeparatorGadgetClass, NULL, parent, NULL,
                                 label, 0, NULL, NULL, NULL, NULL));
}

static Widget createPulldown(Widget parent, char *name, Arg *arglist,
                             int argcnt) {
        Widget w;
...
static char *mallocGETMESSAGE(int msgset, int msg, char *defaultString) {
        char *c1;
        char *c2;
        int c1_size;
...
static void createSizeMenu(Widget w, Widget menu) {
        ...
        ac = 0;
        (void)XtSetArg(al[ac], XmNradioBehavior, True);
        ac++;
...
#ifdef WINDOW_SIZE_TOGGLES
        windowSizeToggles = (Widget *)XtMalloc(
            (1 + tw->termview.sizeList.numSizes) * sizeof(Widget));
#endif /* WINDOW_SIZE_TOGGLES */
        for (i1 = 0; i1 < tw->termview.sizeList.numSizes; i1++) {
                *buffer = '\0';
                ...
                windowSizeToggles[i1] = _DtTermViewCreateToggleButton(
                    submenu, buffer, 0, NULL, NULL, sizeChangeCallback,
                    (XtPointer)i1);
#else  /* WINDOW_SIZE_TOGGLES */
                button = _DtTermViewCreatePushButton(submenu, buffer, 0, NULL,
                                                     NULL, sizeChangeCallback,
                                                     (XtPointer)i1);
#endif /* WINDOW_SIZE_TOGGLES */
        }
...
#ifdef WINDOW_SIZE_TOGGLES
        windowSizeToggles[i1] = _DtTermViewCreateToggleButton(
            submenu, buffer, 0, NULL, NULL, defaultSizeCallback, NULL);
#else  /* WINDOW_SIZE_TOGGLES */
        button = _DtTermViewCreatePushButton(submenu, buffer, 0, NULL, NULL,
                                             defaultSizeCallback, NULL);
#endif /* WINDOW_SIZE_TOGGLES */

        ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 3, "W"));
        (void)_DtTermViewCreateCascadeButton(
            menu, submenu, (GETMESSAGE(NL_SETN_ViewMenu, 2, "Window Size")), ks,
            NULL, NULL, NULL, NULL);
}
...
static void createFontMenu(Widget w, Widget menu) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
        Widget submenu;
        Widget button;
        long i1;
        ...
        dpi = HeightOfScreen(XtScreen(w)) /
              (HeightMMOfScreen(XtScreen(w)) / 25.4);
                ...
                *c2++ = '\0';
                ...
                if ((c2 = strchr(fontArray[i1].labelName, '/'))) {
                        ...
                        *c2++ = '\0';
                        ...
                        (void)strlcpy(fontName, fontArray[i1].labelName,
                                      BUFSIZ);
                        ...
                        if (fontName[strlen(fontName) - 1] == ':') {
                                /* let's turn in into an iso8859-1 name for the
                                 * query... */
                                fontName[strlen(fontName) - 1] = '\0';
                                /* strip off a '-' before the ':' (should not
                                 * have been one, but)...
                                 */
                                if (fontName[strlen(fontName) - 1] == '-') {
                                        fontName[strlen(fontName) - 1] = '\0';
                                }
                                (void)strlcat(fontName,
                                              (GETMESSAGE(NL_SETN_ViewMenu, 4,
                                                          "-iso8859-1")),
                                              BUFSIZ);
                        }
                        if ((fontNames = XListFonts(XtDisplay(w), fontName, 1,
                                                    &fontNameCount))) {
                                c2 = *fontNames;
                                for (i2 = 0; i2 < 7; i2++) {
                                        while (*c2 && (*c2 != '-')) {
                                ...
                                if (i2 == 7) {
                                        pointSize =
                                            ((float)strtol(c2, (char **)0, 0)) *
                                            72 / dpi;
                                        /* this was taken from the style
                                         * manager... */
                                        if (dpi <= 72) {
                                                /* whole points... */
                                                (void)snprintf(
                                        ...
                                        fontArray[i1].labelName =
                                            XtMalloc(labelName_size);
                                        (void)strlcpy(fontArray[i1].labelName,
                                                      fontName, labelName_size);
                                }
        ...
        *mnemonics = '\0';
        ...
        fontSizeToggles =
            (Widget *)XtMalloc((fontArrayCount + 1) * sizeof(Widget));
        ...
        for (i1 = 0; i1 < fontArrayCount; i1++) {
                ...
                fontSizeToggles[i1] = _DtTermViewCreateToggleButton(
                    submenu, fontArray[i1].labelName, 0, NULL, NULL,
                    fontChangeCallback, (XtPointer)i1);
        }

        (void)strlcpy(buffer, (GETMESSAGE(NL_SETN_ViewMenu, 9, "Default")),
                      BUFSIZ);
        fontSizeToggles[i1] = _DtTermViewCreateToggleButton(
            submenu, buffer, 0, NULL, NULL, defaultFontCallback, NULL);
        fontSizeTogglesDefault = i1;

        ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 11, "F"));
        (void)_DtTermViewCreateCascadeButton(
            menu, submenu, (GETMESSAGE(NL_SETN_ViewMenu, 10, "Font Size")), ks,
            NULL, NULL, NULL, NULL);
}
...
static Widget CreateMenu(Widget termView, Widget parent, Boolean menuBar,
        ...
        if (menuBar) {
                /* we will create the menubar unmanaged so that our parent can
                 * mange it when it wants to...
                 */
                menu = XmCreateMenuBar(parent, "menu_pulldown", newArglist, i);
        } else {
                if (firstPopup) {
                        popupMenu = XmCreatePopupMenu(topLevel, "menu_popup",
                                                      newArglist, i);
                }
                menu = popupMenu;
                button = _DtTermViewCreateLabel(
                    menu, (GETMESSAGE(NL_SETN_ViewMenu, 83, "Terminal")));
        ...
        if (first) {
                ...
                newButton = _DtTermViewCreatePushButton(
                    pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 12, "New")), ks,
                    NULL, NULL, cloneCallback, NULL);
#ifdef NOTDEF
                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 15, "P"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 14, "Print")),
                    ks, NULL, NULL, NULL, NULL);
                (void)XtSetSensitive(button, False);
                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 17, "r"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 16, "Print...")), ks, NULL,
                    NULL, NULL, NULL);
                (void)XtSetSensitive(button, False);
#endif /* NOTDEF */
                ...
                acceleratorText =
                    mallocGETMESSAGE(NL_SETN_ViewMenu, 62, "Alt+F4");
                button = _DtTermViewCreatePushButton(
                    pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 18, "Close")),
                    ks, accelerator, acceleratorText, exitCallback, NULL);
        ...
        cascade = _DtTermViewCreateCascadeButton(
            menu, pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 20, "Window")),
            ks, NULL, NULL, NULL, NULL);
#else  /* PULLDOWN_ACCELERATORS */
                cascade = _DtTermViewCreateCascadeButton(
                    menu, pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 20, "Window")), NoSymbol,
                    NULL, NULL, NULL, NULL);
        ...
        if (first) {
                ...
                accelerator = mallocGETMESSAGE(NL_SETN_ViewMenu, 24,
                                               "Ctrl <Key>osfInsert");
                acceleratorText =
                    mallocGETMESSAGE(NL_SETN_ViewMenu, 25, "Ctrl+Insert");
                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 23, "C"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 22, "Copy")),
                    ks, accelerator, acceleratorText, copyClipboardCallback,
                    NULL);
                (void)XtFree(accelerator);
                (void)XtFree(acceleratorText);
                ...
                accelerator = mallocGETMESSAGE(NL_SETN_ViewMenu, 28,
                                               "Shift <Key>osfInsert");
                acceleratorText =
                    mallocGETMESSAGE(NL_SETN_ViewMenu, 29, "Shift+Insert");
                button = _DtTermViewCreatePushButton(
                    pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 26, "Paste")),
                    ks, accelerator, acceleratorText, pasteClipboardCallback,
                    NULL);
                (void)XtFree(accelerator);
                (void)XtFree(acceleratorText);
        }
        ...
        cascade = _DtTermViewCreateCascadeButton(
            menu, pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 30, "Edit")), ks,
            NULL, NULL, NULL, NULL);
#else  /* PULLDOWN_ACCELERATORS */
                cascade = _DtTermViewCreateCascadeButton(
                    menu, pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 30, "Edit")), NoSymbol, NULL,
                    NULL, NULL, NULL);
#endif /* PULLDOWN_ACCELERATORS */
        ...
        if (first) {
                ...
                menuBarToggle = _DtTermViewCreateToggleButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 32, "Menu Bar")), ks, NULL,
                    NULL, menuBarToggleCallback, NULL);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 35, "S"));
                scrollBarToggle = _DtTermViewCreateToggleButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 34, "Scroll Bar")), ks, NULL,
                    NULL, scrollBarToggleCallback, NULL);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 37, "G"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 36, "Global...")), ks, NULL,
                    NULL, globalOptionsCallback, NULL);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 39, "T"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 38, "Terminal...")), ks, NULL,
                    NULL, terminalOptionsCallback, NULL);
                ...
                button = _DtTermViewCreatePushButton(
                    submenu, (GETMESSAGE(NL_SETN_ViewMenu, 40, "Soft Reset")),
                    ks, NULL, NULL, softResetCallback, NULL);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 43, "H"));
                button = _DtTermViewCreatePushButton(
                    submenu, (GETMESSAGE(NL_SETN_ViewMenu, 42, "Hard Reset")),
                    ks, NULL, NULL, hardResetCallback, NULL);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 45, "R"));
                (void)_DtTermViewCreateCascadeButton(
                    pulldown[pc], submenu,
                    (GETMESSAGE(NL_SETN_ViewMenu, 44, "Reset")), ks, NULL, NULL,
                    NULL, NULL);
        }
        if (menuBar || (!menuBar && firstPopup))
#ifdef PULLDOWN_ACCELERATORS
        ...
        cascade = _DtTermViewCreateCascadeButton(
            menu, pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 46, "Options")),
            ks, NULL, NULL, NULL, NULL);
#else  /* PULLDOWN_ACCELERATORS */
                cascade = _DtTermViewCreateCascadeButton(
                    menu, pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 46, "Options")), NoSymbol,
                    NULL, NULL, NULL, NULL);
#endif /* PULLDOWN_ACCELERATORS */
        ...
        if (first) {
                ...
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 48, "Overview")), ks, NULL,
                    NULL, helpIntroCallback, NULL);

                button = _DtTermViewCreateSeparator(pulldown[pc], "Separator");

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 51, "T"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 50, "Tasks")),
                    ks, NULL, NULL, helpTasksCallback, NULL);
                (void)XtSetSensitive(button, False);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 53, "R"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 52, "Reference")), ks, NULL,
                    NULL, helpReferenceCallback, NULL);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 55, "O"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 54, "On Item")),
                    ks, NULL, NULL, NULL, NULL);
                (void)XtSetSensitive(button, False);
                ...
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 56, "Using Help")), ks, NULL,
                    NULL, helpOnHelpCallback, NULL);
                ...
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 82, "About Terminal")), ks,
        ...
        cascade = _DtTermViewCreateCascadeButton(
            menu, pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 60, "Help")), ks,
            NULL, NULL, NULL, NULL);
#else  /* PULLDOWN_ACCELERATORS */
                cascade = _DtTermViewCreateCascadeButton(
                    menu, pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 60, "Help")), NoSymbol, NULL,
                    NULL, NULL, NULL);
#endif /* PULLDOWN_ACCELERATORS */
                ...
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 64, "Overview")), ks, NULL,
                    NULL, helpOverviewCallback, NULL);
                ...
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 68, "Table Of Contents")), ks,
                    NULL, NULL, helpTableOfContentsCallback, NULL);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 71, "T"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 70, "Tasks")),
                    ks, NULL, NULL, helpTasksCallback, NULL);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 73, "R"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 72, "Reference")), ks, NULL,
                    NULL, helpReferenceCallback, NULL);

                ks = XStringToKeysym(GETMESSAGE(NL_SETN_ViewMenu, 75, "K"));
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 74, "Keyboard")), ks, NULL,
                    NULL, helpKeyboardCallback, NULL);
                ...
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 76, "Using Help")), ks, NULL,
                    NULL, helpUsingHelpCallback, NULL);
                ...
                button = _DtTermViewCreatePushButton(
                    pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 83, "About Terminal")), ks,
        ...
        cascade = _DtTermViewCreateCascadeButton(
            menu, pulldown[pc], (GETMESSAGE(NL_SETN_ViewMenu, 80, "Help")), ks,
            NULL, NULL, NULL, NULL);
#else  /* PULLDOWN_ACCELERATORS */
                cascade = _DtTermViewCreateCascadeButton(
                    menu, pulldown[pc],
                    (GETMESSAGE(NL_SETN_ViewMenu, 80, "Help")), NoSymbol, NULL,
                    NULL, NULL, NULL);
#endif /* PULLDOWN_ACCELERATORS */
#endif /* HPVUE */
        if (menuBar) {
                ...
                i = 0;
                (void)XtSetArg(arglist[i], XmNmenuHelpWidget, cascade);
                i++;
...
Widget _DtTermViewCreatePulldownMenu(Widget termView, Widget parent,
                                     Arg menuArglist[], int menuArgcount) {
        Widget w;
...
Widget _DtTermViewCreatePopupMenu(Widget termView, Widget parent,
                                  Arg menuArglist[], int menuArgcount) {
        static Widget popupMenu = (Widget)0; /* popup widget...	*/

        _DtTermProcessLock();
        if (!popupMenu) {
                popupMenu = CreateMenu(termView, parent, False, menuArglist,
                                       menuArgcount);
        }
        _DtTermProcessUnlock();

        /* set up a handler to post the menu... */
        (void)XtAddEventHandler(parent, ButtonPressMask, False, postMenu,
                                (XtPointer)popupMenu);
        ...
        (void)XtAddCallback(parent, XmNdestroyCallback,
                            removeFromPostFromListCallback,
                            (XtPointer)popupMenu);
        return (popupMenu);
}
...
static void setContext(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        i = 0;
        (void)XtSetArg(arglist[i], DtNscrollBar, &scrollBarState);
        i++;
        (void)XtSetArg(arglist[i], DtNmenuBar, &menuBarState);
        i++;
#ifdef WINDOW_SIZE_TOGGLES
        (void)XtSetArg(arglist[i], DtNrows, &rows);
        i++;
        (void)XtSetArg(arglist[i], DtNcolumns, &columns);
        i++;
#endif /* WINDOW_SIZE_TOGGLES */
        (void)XtGetValues(w, arglist, i);

        /* set the toggles... */
        (void)XmToggleButtonGadgetSetState(scrollBarToggle, scrollBarState,
                                           False);
        ...
        if ((tw->termview.currentFontToggleButtonIndex >=
            ...
            (tw->termview.currentFontToggleButtonIndex < 0)) {
                (void)XmToggleButtonGadgetSetState(fontSizeToggles[i], True,
                                                   False);
        } else {
                (void)XmToggleButtonGadgetSetState(fontSizeToggles[i], False,
                                                   False);
        }
        ...
        if (((tw->termview.sizeDefault.rows <= 0) ||
            ...
            ((tw->termview.sizeDefault.columns <= 0) ||
             (tw->termview.sizeDefault.columns == columns))) {
                (void)XmToggleButtonGadgetSetState(windowSizeToggles[i], True,
                                                   False);
        } else {
                (void)XmToggleButtonGadgetSetState(windowSizeToggles[i], False,
                                                   False);
        }
#endif /* WINDOW_SIZE_TOGGLES */
...
static void exitCallback(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        (void)XtCallCallbacks(w, DtNsubprocessTerminationCallback,
                              (XtPointer)0);
...
static void raiseDialog(Widget w) {
        while (w && !XtIsShell(w)) {
                w = XtParent(w);
        }
...
static void defaultSizeCallback(Widget w, XtPointer client_data,
        ...
        if ((tw->termview.sizeDefault.columns > 0) &&
            (tw->termview.sizeDefault.rows > 0)) {
                ...
                ac++;
                (void)XtSetArg(al[ac], DtNrows, tw->termview.sizeDefault.rows);
                ac++;
                (void)XtSetValues((Widget)tw, al, ac);
        }
}
...
static void fontChangeCallback(Widget w, XtPointer client_data,
        ...
        if (tw->termview.fontList != fontArray[i1].fontList) {
                ac = 0;
                (void)XtSetArg(al[ac], DtNuserFont, fontArray[i1].fontList);
                ac++;
                (void)XtSetValues((Widget)tw, al, ac);
        }
...
static void defaultFontCallback(Widget w, XtPointer client_data,
        ...
        if (tw->termview.fontList != tw->termview.defaultFontList) {
                ac = 0;
                (void)XtSetArg(al[ac], DtNuserFont,
                               tw->termview.defaultFontList);
                ac++;
                (void)XtSetValues((Widget)tw, al, ac);
        }
...
static void softResetCallback(Widget w, XtPointer client_data,
        ...
        (void)_DtTermFuncSoftReset(
            _DtTermViewGetChild((Widget)tw, DtTermTERM_WIDGET), 1, fromMenu);
}
...
static void copyClipboardCallback(Widget w, XtPointer client_data,
        ...
        (void)_DtTermPrimSelectCopyClipboard(
            tw, XtLastTimestampProcessed(XtDisplay(tw)));
}
...
void _DtTermViewSetUserFontListIndex(Widget w, long i1) {
        DtTermViewWidget tw = (DtTermViewWidget)w;
