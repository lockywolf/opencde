...
static void GetAutoWrap(Widget wid, int offset, XtArgVal *value);
static XmImportOperator SetAutoWrap(Widget wid, int offset, XtArgVal *value);
static void GetReverseWrap(Widget wid, int offset, XtArgVal *value);
static XmImportOperator SetReverseWrap(Widget wid, int offset, XtArgVal *value);
static void GetAppCursorMode(Widget wid, int offset, XtArgVal *value);
static XmImportOperator SetAppCursorMode(Widget wid, int offset,
                                         XtArgVal *value);
static void GetAppKeypadMode(Widget wid, int offset, XtArgVal *value);
static XmImportOperator SetAppKeypadMode(Widget wid, int offset,
                                         XtArgVal *value);
...
static XtResource resources[] = {
    ...
    {DtNc132, DtCC132, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermRec, vt.c132), XtRImmediate, (XtPointer)False},
    {DtNappCursorDefault, DtCAppCursorDefault, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermRec, vt.appCursorMode), XtRImmediate,
     (XtPointer)False},
    {DtNappKeypadDefault, DtCAppKeypadDefault, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermRec, vt.appKeypadMode), XtRImmediate,
     (XtPointer)False},
#ifdef DKS
    {DtNstickyPrevCursor, DtCStickyCursor, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermRec, vt.stickyPrevCursor), XtRImmediate,
     (XtPointer)False},
    {DtNstickyNextCursor, DtCStickyCursor, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(struct _DtTermRec, vt.stickyNextCursor), XtRImmediate,
     (XtPointer)False},
    {DtNstatusChangeCallback, DtCCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffsetOf(struct _DtTermRec, vt.statusChangeCallback), XmRPointer,
     (XtPointer)NULL},
    {DtNverticalScrollBar, DtCVerticalScrollBar, XmRWidget, sizeof(Widget),
     XtOffsetOf(struct _DtTermRec, vt.verticalScrollBar), XmRImmediate,
     (XtPointer)NULL},
#endif /* DKS */
};
...
static XmSyntheticResource GetResources[] = {
    {DtNautoWrap, sizeof(Boolean), XtOffsetOf(struct _DtTermRec, vt.autoWrap),
     GetAutoWrap, SetAutoWrap},
    {DtNreverseWrap, sizeof(Boolean),
     XtOffsetOf(struct _DtTermRec, vt.reverseWrap), GetReverseWrap,
     SetReverseWrap},
    {DtNappCursorDefault, sizeof(Boolean),
     XtOffsetOf(struct _DtTermRec, vt.appCursorMode), GetAppCursorMode,
     SetAppCursorMode},
    {DtNappKeypadDefault, sizeof(Boolean),
     XtOffsetOf(struct _DtTermRec, vt.appKeypadMode), GetAppKeypadMode,
     SetAppKeypadMode},
...
externaldef(vtwidgetclass)
    WidgetClass dtTermWidgetClass = (WidgetClass)&dtTermClassRec;

/* Class Initialize...
 */
static void ClassInitialize(void) { return; }
...
*/
static void initializeModes(DtTermData td) {
...
unsigned char _DtTermGetRenderFontIndex(Widget w, int id) {
...
static void Initialize(Widget ref_w, Widget w, Arg *args, Cardinal *num_args) {
        ...
        /* malloc Vt dataspace... */
        tw->vt.td = (DtTermData)XtMalloc(sizeof(DtTermDataRec));
        td = tw->vt.td;
        ...
        td->renderFonts[1].termFont = _DtTermPrimRenderLineDrawCreate(
            w,                 /* Widget		*/
        ...
        */
        ...
        if (!strcmp(tpw->term.termId, "vt100"))
                td->terminalId = 100;
        else if (!strcmp(tpw->term.termId, "vt101"))
                td->terminalId = 101;
        else if (!strcmp(tpw->term.termId, "vt102"))
                td->terminalId = 102;
        else
                td->terminalId = 220; /* vt200  */
        if (td->terminalId < 200)
                td->compatLevel = 1;
        else
                td->compatLevel = 2;
...
/* ARGSUSED */
static XmImportOperator SetReverseWrap(Widget wid, int offset,
                                       XtArgVal *value) {
        DtTermWidget tw = (DtTermWidget)wid;
...
static Boolean SetValues(Widget cur_w, Widget ref_w, Widget w, ArgList args,
        ...
        if (tw->term.fontList != cur_tw->term.fontList) {
                ...
                (void)_DtTermPrimDestroyFont(
                    w, tw->vt.td->renderFonts[1].termFont);

                /* replace the font... */
                tw->vt.td->renderFonts[1].termFont =
                    _DtTermPrimRenderLineDrawCreate(
                        w, _DtTermGlyphs, _DtTermNumGlyphs, tw->term.widthInc,
                        tw->term.ascent, tw->term.heightInc - tw->term.ascent);

                /* replace the bold font.  If the base font is the default, then
                 * use the default bold font that we generated.  Else, null it
                 * out and we will fake by overstriking...
                 */
...
static void Destroy(Widget w) {
        ...
        /* remove the DtTermData structure contents, followed by the
         * structure...
         */
        if (tw->vt.td) {
                /*  free up any fonts we allocated for this widget... */
                /*  don't free the others because they're done in TermPrim
                 * Destroy()*/
                (void)_DtTermPrimDestroyFont(
                    w, tw->vt.td->renderFonts[1].termFont);
...
static void StatusChangeNotify(Widget w, unsigned long mask) {
        ...
        if (mask & TermSTATUS_CHANGE_NOTIFY_CAPS_LOCK) {
                ...
                (void)XtCallCallbackList(w, tw->vt.statusChangeCallback,
                                         (XtPointer)&cb);
        }
...
Widget DtCreateTerm(Widget parent, char *name, ArgList arglist,
                    Cardinal argcount) {
        Widget w;
        ...
        if (td->singleShiftPending) {
                ...
                *td->GL = td->singleShiftFont;
                (void)_DtTermPrimBufferSetEnhancement(
                    ...
                    tpd->cursorColumn, enhFont, *td->GL);
