...
static char rcs_id[] =
    "$XConsortium: TermEnhance.c /main/1 1996/04/21 19:15:41 drk $";
...
void _DtTermEnhProc(Widget w, enhValues values, TermEnhInfo info) {
        ...
        if (!td->colorPairs[ourColorPair].initialized) {
                (void)_DtTermColorInitializeColorPair(
                    w, &td->colorPairs[ourColorPair]);
        }
        if (!td->colorPairs[ourBgColor].initialized) {
                (void)_DtTermColorInitializeColorPair(
                    w, &td->colorPairs[ourBgColor]);
        }
        ...
        if (IS_BOLD(ourVideo)) {
                if (ourFont == FONT_NORMAL) {
                        if (td->renderFonts[RENDER_FONT_BOLD].termFont) {
                                ...
                                info->font =
                                    td->renderFonts[RENDER_FONT_BOLD].termFont;
                        } else {
                        ...
                        info->font =
                            td->renderFonts[RENDER_FONT_LINEDRAW].termFont;
                }
        } else {
