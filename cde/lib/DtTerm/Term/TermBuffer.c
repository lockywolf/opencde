...
static char rcs_id[] =
    "$TOG: TermBuffer.c /main/2 1997/04/17 18:04:41 samborn $";
...
*/
static void clearEnhancements(TermBuffer tb, short row, short startCol,
                              short count);

static void insertEnhancements(TermBuffer tb, short row, short col,
                               short insertCount);

static void deleteEnhancement(TermBuffer tb, short row, short col);

static void _DtTermClearEnhancements(TermBuffer tb, short row, short col,
                                     short count);

static int _DtTermSetEnhancement(TermBuffer tb, short row, short col,
                                 unsigned char id, enhValue value);

static Boolean _DtTermGetEnhancement(const TermBuffer tb, const short row,
                                     const short col, enhValue **enhancements,
                                     short *count, const countSpec countWhich);

static termChar *_DtTermGetCharacterPointer(TermBuffer tb, short row,
                                            short col);

static void _DtTermInsertEnhancements(const TermBuffer tb, const short row,
                                      const short col, short insertCount,
                                      const Boolean insertFlag);

static Boolean _DtTermBufferCreateEnhancement(TermBuffer tb, short row,
                                              short col);

static void _DtTermBufferResize(TermBuffer newTb, short *newRows,
                                short *newCols);

static void _DtTermDeleteEnhancement(TermBuffer tb, short row, short col,
                                     short width);

static short
_DtTermInsert(TermBuffer tb, short row, short col, termChar *newChars,
              short numChars,
...
static Boolean _DtTermSetLineLength(TermBuffer tb, short row, short newLength);

static short _DtTermGetLineLength(TermBuffer tb, short row);

static Boolean _DtTermClearLine(TermBuffer tb, short row, short newLength);

#if (defined(TEST) || defined(__CODECENTER__) || defined(VALIDATE_ENH))
static void validateEnhancements(TermBuffer tb, short row);
...
*/
TermBuffer _DtTermBufferCreateBuffer(const Widget w, const short rows,
        ...
        newTB = _DtTermPrimBufferCreateBuffer(w, rows, cols, sizeOfBuffer,
                                              sizeOfLine, sizeOfEnh);
        ...
        if (newTB) {
                ...
                for (lines = DT_LINES(newTB);
                     lines < DT_LINES(newTB) + ROWS(newTB); lines++) {
...
*/
static void _DtTermBufferResize(TermBuffer tb, short *newRows, short *newCols) {
        short i;
        DtEnh enh;
        DtLine *lines;
        ...
        if (*newCols > MAX_COLS(tb)) {
                ...
                */
                ...
                for (i = 0; i < MAX_ROWS(tb); i++) {
                        ...
                        */
                        if (DT_ENH(lines[i])) {
                                enh = (DtEnh)malloc(*newCols *
                                                    sizeof(DtTermEnhPart));

                                if (enh) {
                                        /*
                                        ** copy the enhancment info for all
                                        *characters on
                                        ...
                                        */
                                        ...
                                        /*
                                        ** the malloc failed, revert back to
                                        *MAX_COLS
                                        */
                                        *newCols = MAX_COLS(tb);
                        ...
                        newLineBuffer = (termChar *)malloc((unsigned)*newCols *
                                                           BYTES_PER_CHAR(tb));

                        if (newLineBuffer == NULL) {
                                /*
                                ** line buffer malloc failed, we can only
                                *increase the
                        ...
                        memcpy(newLineBuffer, BUFFER(lines[i]),
                               LENGTH(lines[i]) * BYTES_PER_CHAR(tb));
                        free(BUFFER(lines[i]));
        ...
        if (*newRows > MAX_ROWS(tb)) {
                ...
                */
                ...
                if (lines != NULL) {
                        ...
                        ** free it...
                        */
                        memcpy(lines, DT_LINES(tb),
                               sizeof(DtLine) * MAX_ROWS(tb));
                        free(DT_LINES(tb));
                        ...
                        **
                        ** NOTE:
                        **     If we are unable to malloc any part of a line,
                        *adjust
                        **     "newRows" and break.
                        */
                        for (i = MAX_ROWS(tb); i < *newRows; i++) {
                                ...
                                if (lines[i]) {
                                        DT_ENH(lines[i]) = NULL;
                                        BUFFER(lines[i]) = (termChar *)malloc(
                                            (unsigned)COLS(tb) *
                                            BYTES_PER_CHAR(tb));
                                        if (BUFFER(lines[i])) {
                                                LENGTH(lines[i]) = 0;
                        ...
                        /*
                        ** the malloc for the row buffer failed, revert back to
                        *MAX_ROWS
                        */
                        *newRows = MAX_ROWS(tb);
                }
...
*/
void _DtTermBufferFreeBuffer(const TermBuffer tb) {
        DtLine *lines;
...
*/
static void clearEnhancements(TermBuffer tb, short row, short col,
                              short count) {
        DtEnh enh;
        int i;
...
static void _DtTermInsertEnhancements(const TermBuffer tb, const short row,
        ...
        */
        if ((col < WIDTH(line)) && ((DT_ENH_DIRTY(tb)) || (enh != NULL))) {
                ...
                /*
                ** get a copy of the current enhancement (we'll insert
                *'copyCount'
                ...
                */
                ...
                if (insertFlag) {
                        /*
                        ** we're in insert mode, move any existing
                        *enhancements...
                        */
...
#ifdef NOCODE
                /*
                ** insert insertCount copies of fillEnh into the enhancement
                *buffer
...
void _DtTermBufferDelete(TermBuffer tb, short *row, short *col,
...
static void _DtTermDeleteEnhancement(TermBuffer tb, short row, short col,
        ...
        */
        ...
        memcpy(enh + col, enh + col + width, copyCount * sizeof(DtTermEnhPart));
}
...
static Boolean _DtTermBufferCreateEnhancement(TermBuffer tb, short row,
        ...
        */
        if (*enh == NULL) {
                /*
                ** Otherwise, allocate and initialize a row of enhancement
                *blocks.
                */
...
*/
Boolean _DtTermBufferFreeEnhancement(TermBuffer tb, short row, short col) {
        return (True);
}
        ...
        */
        if (DT_ENH(line) == NULL) {
                ...
                */
                ...
                case countNew:
                        /*
                        **  For Vt220 emulation, countNew is the same as
                        *countAll...
                        **  JRM 08/30/93
                        */
                case countAll:
                        ...
                        */
                        for (i = 0; i < (WIDTH(line) - col); i++) {
                                if (((*values)[(int)enhVideo] !=
                                     (enh->video & VIDEO_MASK)) ||
                                    ((*values)[(int)enhField] !=
                                     (enh->field & FIELD_MASK)) ||
                                    ((*values)[(int)enhFgColor] !=
                                     (enh->fgColor & COLOR_MASK)) ||
                                    ((*values)[(int)enhBgColor] !=
                                     (enh->bgColor & COLOR_MASK)) ||
                                    ((*values)[(int)enhFont] !=
                                     (enh->font & FONT_MASK))) {
                                        /*
...
*/
static Boolean _DtTermClearLine(TermBuffer tb, short row, short newWidth) {
        DtLine line;
        DtEnh enh;
...
*/
void _DtTermBufferErase(TermBuffer tb, short row, short col, short count,
        ...
        case eraseCharCount:
                ...
                */
                startCol = col;
                lastCol =
                    MIN(col + count - 1, WIDTH(DT_LINE_OF_TBUF(tb, row)) - 1);
                break;
...
#if (defined(TEST) || defined(__CODECENTER__) || defined(VALIDATE_ENH))
static void _DtTermValidateEnhancements(TermBuffer tb, short row) {
                ...
                for (col = 0, thisEnh = DT_ENH(thisLine);
                     ...
                     col++, thisEnh++) {
                        ...
                        if (thisEnh->fieldStart) {
                                ...
                                fprintf(stderr,
                                        "Field enhancements don't match:");
                                fprintf(stderr,
                                        "    row : %3.3d, col : %3.3d\n", row,
                                        col);
                        ...
                        if (thisEnh->colorStart) {
                                ...
                                fprintf(stderr,
                                        "Color enhancements don't match:");
                                fprintf(stderr,
                                        "    row : %3.3d, col : %3.3d\n", row,
                                        col);
                        ...
                        if (thisEnh->font <= 2) {
                                if (thisEnh->fontStart) {
                                        ...
                                        fprintf(
                                            stderr, "    thisEnh->font: %d\n",
                                            (int)(thisEnh->font & FONT_MASK));
                                        validatePassed = False;
                                }
                        } else {
                                fprintf(stderr,
                                        "Font enhancement out of range:");
                                fprintf(stderr,
                                        "    row : %3.3d, col : %3.3d\n", row,
                                        col);
...
#ifdef NOCODE
static void _printLine(TermBuffer tb, short row) {
        DtLine line;
        termChar *pChar;
        short j;
...
static int _DtTermPrintEnhancement(TermBuffer tb, short row, short col) {
        enhValue enhancements[NUM_ENHANCEMENT_FIELDS];
        short enhCount;

        _DtTermPrimBufferGetEnhancement(tb, row, col, enhancements, &enhCount,
                                        countNew);
        printf("    col  : %d\n", col);
        printf("    Count: %d\n", enhCount);
        printf("        Video: '_");
...
static void printEnh(DtEnh enh) {
        printf(" video  : %d\n", enh->video);
        printf(" field  : %d\n", enh->field);
        printf(" fgColor: %d\n", enh->fgColor);
        printf(" bgColor: %d\n", enh->bgColor);
...
*/
static void _DtTermPrintBuffer(DtTermBuffer tb) {
        short i;
        short j;
        short k;
