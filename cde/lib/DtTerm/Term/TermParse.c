...
static char rcs_id[] =
    "$TOG: TermParse.c /main/3 1999/10/15 12:25:13 mgreess $";
...
#define FLOOR(w)                                                               \
        ((w)->vt.td->originMode ? (w)->term.tpd->scrollLockBottomRow           \
                                : BOTTOMROW(w))
#define CEILING(w)                                                             \
        ((w)->vt.td->originMode ? (w)->term.tpd->scrollLockTopRow : TOPROW(w))
#define KILLWRAP(w)                                                            \
        if ((w)->term.tpd->cursorColumn > LASTCOLUMN(w))                       \
...
void _DtTermClearParam(Widget w) {
...
void _DtTermEnterNum(Widget w) {
        ParserContext context;
        context = GetParserContext(w);
        _DtTermPrimParserEnterNum(w);
        if (!PCOUNT(context))
                PCOUNT(context)++; /* increment for first number ONLY*/
}

void _DtTermParsePushNum(Widget w) {
        ParserContext context;
        context = GetParserContext(w);
        /* parmN(w,PCOUNT(c)) ;  Should  use this but not global   */
        if (context->workingNumIsDefault !=
            True) { /* did we actually parse a num*/
...
void _DtTermSaveChar(Widget w) {
        ...
        if ((int)context->stringParms[0].length < STR_SIZE && iswprint(c)) {
                strncpy(((char *)&context->stringParms[0]
                             .str[context->stringParms[0].length]),
                        (char *)context->inputChar, context->inputCharLen);
                context->stringParms[0].length += context->inputCharLen;
        }
}

void _DtTermPrintState(Widget w) {
        ParserContext context;
        context = GetParserContext(w);
...
static void sendEscSequence(Widget w, char *transmitString) {
...
void _DtTermDeviceStatus(Widget w) /* DSR CSI?pn  */
{
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
...
void _DtTermDeviceStatusAnsi(Widget w) /* DSR CSIpn  */
{
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermData vtd = ((DtTermWidget)w)->vt.td;
        ParserContext context;
...
void _DtTermPModeSet(Widget w) /*  DECSET CSI?ph */
{
        ParserContext context;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        ...
        for (i = 1; i <= cnt; i++) {
                ...
                case 3:                     /* Column (132) */
                        if (vtw->vt.c132) { /* xterm allow 132 mode */
                                ...
                                vtw->term.tpd->scrollLockBottomRow =
                                    BOTTOMROW(vtw);
                ...
                case 5: /* Screen (Reverse) */
                        ...
                        (void)XClearArea(XtDisplay(w), XtWindow(w), 0, 0, 0, 0,
                                         True);
                        break;
...
void _DtTermPModeReset(Widget w) /* DECRST CSI?pl */
{
        ParserContext context;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermWidget vtw = (DtTermWidget)w;
...
void _DtTermSetMode(Widget w) /* SM CSIph */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        int i, cnt;
...
void _DtTermResetMode(Widget w) /* RM CSIpl  */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        int i, cnt;
...
void _DtTermCursorPos(Widget w) /* CUP CSIp;pH */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        int row, col;
        ...
        if (vtw->vt.td->originMode)
                row += vtw->term.tpd->scrollLockTopRow;
        if (row > FLOOR(vtw))
                row = FLOOR(vtw);
        if (col > LASTCOLUMN(vtw))
                col = LASTCOLUMN(vtw);
        vtw->term.tpd->cursorRow = row;
        vtw->term.tpd->cursorColumn = col;
}

void _DtTermEraseDisplay(Widget w) /* ED CSIpJ */
{
        int cnt;
        DtTermWidget vtw = (DtTermWidget)w;
        ParserContext context;
...
void _DtTermEraseChars(Widget w) /* ECH  CSIpX */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        int row, col, cnt;
        ...
        if (!(cnt))
                cnt = 1;
        else if (cnt > LASTCOLUMN(vtw) - col + 1)
                cnt = LASTCOLUMN(vtw) - col + 1;

        _DtTermFuncEraseCharacter(w, cnt, fromParser);
}

void _DtTermInsertChars(Widget w) /* ICH  CSIp@ */
{
        DtTermWidget vtw = (DtTermWidget)w;
        ParserContext context;
        int row, col, cnt;
...
void _DtTermCursorUp(Widget w) /* CUU  CISpA */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        ...
        if (row < tmargin && row + cnt >= tmargin)
                row = tmargin;
        _DtTermPrimCursorMove(w, row, vtw->term.tpd->cursorColumn);
}

void _DtTermCursorDown(Widget w) /* CUD CISpB */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        ...
        if (row > bmargin && row - cnt <= bmargin)
                row = bmargin;
        /* did we start below bottom margin and cross bottom row?   */
        if (row > brow && bmargin < brow)
                row = brow;
        _DtTermPrimCursorMove(w, row, vtw->term.tpd->cursorColumn);
}

void _DtTermCursorForward(Widget w) /* CUF CISpC */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        int col, lcol, cnt;
...
void _DtTermCursorBack(Widget w) /* CUB CISpD */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
...
void _DtTermCursorToLineUp(Widget w) /* CPL  CSIpF */
{
        DtTermWidget vtw = (DtTermWidget)w;
        ParserContext context;
...
void _DtTermCursorToCol(Widget w) /* CHA CSIpG */
{
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        ParserContext context;
        context = GetParserContext(w);
        ...
        if (!context->parms[1])
                context->parms[1] = 1;
        if (context->parms[1] > tw->term.columns)
                context->parms[1] = tw->term.columns;
        tw->term.tpd->cursorColumn = context->parms[1] - BASE;
}

void _DtTermEraseInLine(Widget w) /* EL  ESC[pK */
{
        DtTermWidget vtw = (DtTermWidget)w;
        ParserContext context;
        DtEraseMode eraseMode;
...
void _DtTermInsertLines(Widget w) /* IL CSIpL */
{
        DtTermWidget vtw = (DtTermWidget)w;
        ParserContext context;
        int lines, cnt;
        ...
        if (vtw->term.tpd->cursorRow < TOPMARGIN(vtw) ||
            vtw->term.tpd->cursorRow >
                BOTTOMMARGIN(vtw)) /*outside scrolling region*/
                return;
        context = GetParserContext(w);
...
void _DtTermDeleteLines(Widget w) /* DL CSIpM */
{
        DtTermWidget vtw = (DtTermWidget)w;
        ParserContext context;
        int cnt;
...
void _DtTermDeleteChars(Widget w) /* DCH CSIpP */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        int cnt;
...
void _DtTermNextLine(Widget w) /* NEL ESCE */
{
        DtTermWidget vtw = (DtTermWidget)w;
        int brow;
...
void _DtTermReverseIndex(Widget w) /* RI ESCM */
{
        DtTermWidget vtw = (DtTermWidget)w;
...
void _DtTermIndex(Widget w) /* IND  ESCD */
{
        DtTermWidget vtw = (DtTermWidget)w;
        int bmargin, brow;
...
void _DtTermScrollingRegion(Widget w) /* DECSTBM  CSIp;pr */
{
        DtTermWidget vtw = (DtTermWidget)w;
        ParserContext context;
        int row1, row2;
        ...
        if (row1 < row2) {
                ...
                if (vtw->vt.td->originMode)
                        _DtTermPrimCursorMove(w, row1, 0);
                else
                        _DtTermPrimCursorMove(w, 0, 0);
        }
}

void _DtTermCharAttributes(Widget w) /* SGR CSIpm */
{
        ParserContext context;
        int i, cnt;
...
void _DtTermDeviceAttributes(Widget w) /* DA CSIpc */
{
        DtTermWidget vtw = (DtTermWidget)w;
        ParserContext context;
        int cnt;
        ...
        case 0:
                ...
                case 102:
                        ...
                        break;
                case 220:
                        /*  class 2 terminal (62), 132 columns (1), printer port
                         * (2), selective erase (6), DRCS (7), UDK (8), national
                         * replacement char sets
                         */
                        snprintf(buf, 50, "\033[?%d;%d;%d;%d;%d;%d;%dc", 62, 1,
                                 2, 6, 7, 8, 9);
                        break;
                }
                sendEscSequence(w, buf);
...
void _DtTermChangeTextParam(Widget w) /* xterm  CSIp;pcCtrl-G  */
{
        ParserContext context;
        DtTermWidget tw = (DtTermWidget)w;
        ...
        for (sw = w; !XtIsShell(sw); sw = XtParent(sw))
                ;
        /*
         *  context->parms[1] is inited to 0, so if no numeric param was
         * entered, we default appropriately to case 0.
         */
        switch (context->parms[1]) {
        case 0: /* change Icon name and Window title */
                XtSetArg(arg[i], XmNtitle, context->stringParms[0].str);
                i++;
                XtSetArg(arg[i], XmNiconName, context->stringParms[0].str);
                i++;
        ...
        case 3: /* change current working directory */
                ...
                tw->term.subprocessCWD =
                    XtRealloc(tw->term.subprocessCWD, size);
...
void _DtTermTabClear(Widget w) /*  TBC CSIpg */
{
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        ...
        switch (type) {
        case 0:
                (void)_DtTermPrimBufferClearTab(tb, tw->term.tpd->cursorColumn);
                break;
        case 3:
                (void)_DtTermPrimBufferClearAllTabs(tb);
                break;
        }
}

void _DtTermRequestParam(Widget w) /* DECREQTPARM CSIpx */
{
        ParserContext context;
        int row;
        char buf[100];
        ...
        if (row < 2) {
                 ...
                 */
                snprintf(buf, 100, "\033[%d;%d;%d;%d;%d;%d;%dx", row, 1, 1, 112,
                         112, 1, 0);
                sendEscSequence(w, buf);
        }
}

void _DtTermTabSet(Widget w) /* HTS ESCH */
{
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermBuffer tb = tw->term.tpd->termBuffer;
...
void _DtTermSaveCursor(Widget w) /* DECSC ESC7 */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
...
void _DtTermRestoreCursor(Widget w) /* DECRC ESC8 */
{
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        ...
        (void)_DtTermPrimBufferSetEnhancement(
            tpd->termBuffer, tpd->topRow + tpd->cursorRow, tpd->cursorColumn,
            enhVideo, vtd->enhVideoState);
        (void)_DtTermPrimBufferSetEnhancement(
            tpd->termBuffer, tpd->topRow + tpd->cursorRow, tpd->cursorColumn,
            enhField, vtd->enhFieldState);
        (void)_DtTermPrimBufferSetEnhancement(
            tpd->termBuffer, tpd->topRow + tpd->cursorRow, tpd->cursorColumn,
            enhFont, *(vtd->GL));
        (void)_DtTermPrimBufferSetEnhancement(
            tpd->termBuffer, tpd->topRow + tpd->cursorRow, tpd->cursorColumn,
            enhBgColor, vtd->saveCursor.enhBgColorState);
        (void)_DtTermPrimBufferSetEnhancement(
            tpd->termBuffer, tpd->topRow + tpd->cursorRow, tpd->cursorColumn,
            enhFgColor, vtd->saveCursor.enhFgColorState);
        if (vtd->originMode) {
...
void _DtTermAppKeypad(Widget w) /* DECPAM  ESC= */
{
        DtTermWidget vtw = (DtTermWidget)w;
...
void _DtTermNormalKeypad(Widget w) /* DECPNM ESC> */
{
        DtTermWidget vtw = (DtTermWidget)w;
...
void _DtTermS8C1T(Widget w) /* S8C1T  ESCG  */
{
        DtTermWidget vtw = (DtTermWidget)w;
...
void _DtTermS7C1T(Widget w) /* S7C1T  ESCF */
{
        DtTermWidget vtw = (DtTermWidget)w;
        Debug('P', fprintf(stderr, ">>In func _vt8CIT\n"));
...
void _DtTermSetCompatLevel(Widget w) /* DECSCL CSI p;p"p (last p literal) */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
...
void _DtTermInvokeG3(Widget w) /* LS3R  ESC| */
{
        ParserContext context;
        int cnt;
...
void _DtTermInvokeG2(Widget w) /* LS3R  ESC} */
{
        ParserContext context;
        int cnt;
...
void _DtTermScrollUp(Widget w) /* SU CSIpS */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        int row, col, cnt;
...
#define EXIT_IF_OUTSIDE_SR(w)                                                  \
        if ((w)->term.tpd->scrollLockTopRow > (w)->term.tpd->cursorRow ||      \
            (w)->term.tpd->scrollLockBottomRow < (w)->term.tpd->cursorRow)     \
                return;
...
void _DtTermScrollDown(Widget w) /* SD CSIpT */
{
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        int row, col, cnt;
...
void _DtTermRestoreModeValues(
    Widget w) /* xterm - Restore DEC mode values CSI?pr */
{
        ParserContext context;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermWidget vtw = (DtTermWidget)w;
        ...
        for (i = 1; i <= cnt; i++) {
                ...
                case 4: /* Scroll Mode ( jump/smooth ) */
                        ...
                        break;
                case 5: /* Screen mode */
                        vtw->term.reverseVideo =
                            vtd->saveDECMode.inverseVideoMode;
                        break;
                ...
                case 25: /* Text cursor enable (Off) */
                        _DtTermPrimSetCursorVisible(
                            w, vtd->saveDECMode.cursorVisible);
                        break;
...
void _DtTermSaveModeValues(Widget w) /* xterm - Save DEC mode values CSI?ps  */
{
        ParserContext context;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermWidget vtw = (DtTermWidget)w;
...
void _DtTermAlignTest(Widget w) /* DECALN ESC#8 */
{
        DtTermWidget vtw = (DtTermWidget)w;
        int tlrow, blrow, cnt;
        ...
        while (cnt--)
                _DtTermPrimInsertText(w, (unsigned char *)"E", 1);
...
void _DtTermInvokeG1(Widget w) /* ESC~ESC */
{
        ParserContext context;
        int cnt;
...
void _DtTermSelEraseInLine(Widget w) /* DECSEL ESC?pK */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermBuffer tb = tw->term.tpd->termBuffer;
        ...
        while (c <= col2) {
                (void)_DtTermPrimBufferGetEnhancement(tb, tpd->cursorRow, c,
                                                      &evalues, &cnt, countAll);
                if (!cnt)
                        break;
                if (evalues[(int)enhField] == FIELD_UNPROTECT) {
                        (void)_DtTermPrimBufferSetEnhancement(
                            tb, tpd->cursorRow, c, enhVideo,
                            evalues[(int)enhVideo]);
                        (void)_DtTermPrimBufferSetEnhancement(
                            tb, tpd->cursorRow, c, enhFgColor,
                            evalues[(int)enhFgColor]);
                        (void)_DtTermPrimBufferSetEnhancement(
                            tb, tpd->cursorRow, c, enhBgColor,
                            evalues[(int)enhBgColor]);
                        while ((c <= col2) && cnt--) {
                                tpd->cursorColumn = c;
                                (void)_DtTermPrimInsertText(
                                    w, (unsigned char *)" ", 1);
                                c++;
                        }
                } else {
...
void _DtTermSelEraseInDisplay(Widget w) /* DECSED ESC?pJ */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermBuffer tb = tw->term.tpd->termBuffer;
        ...
        switch (context->parms[1]) {
        case 0:
                col1 = vtw->term.tpd->cursorColumn;
                row1 = vtw->term.tpd->cursorRow;
                break;
        case 1:
                col2 = vtw->term.tpd->cursorColumn;
...
void _DtTermSingleShiftG2(Widget w) /* SS2  ESCN */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
...
void _DtTermSingleShiftG3(Widget w) /* SS3  ESCO */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
...
void _DtTermLoadCharSet(Widget w) /* vt220 DCS - download char set ESCPpST  */
{
 ...
 */
void _DtTermParseUserKeyClear(Widget w) /* vt220 DECUDK clear function keys*/
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        ParserContext context;
...
void _DtTermParseUserKeyLoad(Widget w) /* vt220 DECUDK load function string */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        ParserContext context;
        ...
        STORELASTARG(context);
        if (!vtd->userKeysLocked) {
                context->stringParms[0]
                    .str[context->stringParms[0].length / 2] =
                    (unsigned char)0; /* Null terminate */
                _DtTermFunctionKeyStringStore(
                    w, context->parms[1], (char *)context->stringParms[0].str);
        }
        _DtTermClearParam(w);
}

void _DtTermParseUserKeyLoadLast(
    Widget w) /* vt220 DECUDK load function string */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        _DtTermParseUserKeyLoad(w);
        if (vtd->needToLockUserKeys)
                vtd->userKeysLocked = True;
        vtd->needToLockUserKeys = False;
}

void _DtTermParseHexDigit(Widget w) /* vt220 DECUDK store hex digit*/
{
        ParserContext context;
        int ci, left_byte, i;
        char c, cbuf[2];
...
void _DtTermReset(Widget w) /* RIS  ESCc */
{
        ParserContext context;
        int cnt;
...
void _DtTermSelectG2(Widget w) /* LS2  ESCn */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
...
void _DtTermSelectG3(Widget w) /* LS3 ESCo */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
...
void _DtTermParseStatus(Widget w) /* DECID (same as DA)  ESCZ */
{
        _DtTermDeviceAttributes(w);
}

void _DtTermSetCharEraseMode(Widget w) /* DECSCA ESCp"q */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        ...
        case 2:
                ...
                (void)_DtTermPrimBufferSetEnhancement(
                    tpd->termBuffer, tpd->topRow + tpd->cursorRow,
                    tpd->cursorColumn, enhField, vtw->vt.td->enhFieldState);
                break;
        case 1:
...
void _DtTermSingleWide(Widget w) /* DECSWL ESC#5 */
{
        ParserContext context;
        int cnt;
...
void _DtTermDoubleWide(Widget w) /* DECDWL ESC#6  */
{
        ParserContext context;
        int cnt;
...
void _DtTermDoubleHigh(Widget w) /* DECWHL ESC#4 */
{
        ParserContext context;
        int cnt;
...
void _DtTermParseFontG0(Widget w) {
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
...
void _DtTermParseFontG1(Widget w) {
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
...
void _DtTermParseFontG2(Widget w) {
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
...
void _DtTermParseFontG3(Widget w) {
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
...
void _DtTermParseFontShift(Widget w) /* Ctrl-E  Ctrl-F  */
{
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData vtd = vtw->vt.td;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
...
void _DtTermParseTab(Widget w) /* Crtl-I */
{
        DtTermWidget vtw = (DtTermWidget)w;
        ...
        if (vtw->term.tpd->cursorColumn > LASTCOLUMN(vtw)) {
                if (vtw->vt.td->fixCursesMode) {
                        ...
                        /* we already tabbed to the end of the line, so do
                         * nothing... */
                }
        } else {
                (void)_DtTermFuncTab(w, 1, fromParser);
...
void _DtTermParseLF /* LineFeed (LF) or newline (NL) Ctrl-J,
...
static void GetWindowOffsets(Widget w, int *xOffset, int *yOffset,
        ...
        *heightOffset = 0;
        ...
        if (XmIsMotifWMRunning(w)) {
                 ...
                 */
                (void)XTranslateCoordinates(display, XtWindow(w), topWindow, 0,
                                            0, &destXReturn, &destYReturn,
                                            &childReturn);
                *xOffset = destXReturn;
                *yOffset = destYReturn;
                ...
                (void)XGetGeometry(display, topWindow, &rootWindow,
                                   ...
                                   &topHeightReturn, &borderWidthReturn,
                                   &depthReturn);

                (void)XGetGeometry(display, XtWindow(w), &rootWindow,
                                   &destXReturn, &destYReturn, &widthReturn,
                                   &heightReturn, &borderWidthReturn,
                                   &depthReturn);
...
void _DtTermParseSunMisc /*  Misc sun esc seqs  */
    (Widget w) {
        ParserContext context;
        DtTermWidget vtw = (DtTermWidget)w;
        ...
        case 3:
                ...
                (void) XMoveWindow(display, sh_win,
                        context->parms[2] + xOffset, context->parms[3] +
                yOffset); DKS */
                (void)XtVaSetValues(sw, XmNx, context->parms[2] + xOffset, XmNy,
                                    context->parms[3] + yOffset, NULL);
                break;
        case 4:
                (void)GetWindowOffsets(sw, &xOffset, &yOffset, &widthOffset,
                                       &heightOffset);
                (void)XtVaSetValues(sw, XmNwidth,
                                    context->parms[3] - widthOffset, XmNheight,
                                    context->parms[2] - heightOffset, NULL);
                break;
        case 5:
        ...
        case 8:
                ...
                (void)XtVaSetValues(w, DtNrows, rows, DtNcolumns, columns,
                                    NULL);
                break;
        case 11:
        ...
        case 13:
                ...
                (void)snprintf(buf, BUFSIZ, "\033[3;%d;%dt", x - xOffset,
                               y - yOffset);
        ...
        case 14:
                (void)GetWindowOffsets(sw, &xOffset, &yOffset, &widthOffset,
                                       &heightOffset);
                (void)XtVaGetValues(sw, XtNheight, &height, XtNwidth, &width,
                                    NULL);
                (void)snprintf(buf, BUFSIZ, "\033[4;%d;%dt",
                               height + heightOffset, width + widthOffset);
                (void)sendEscSequence(w, buf);
                break;
        case 18:
                (void)XtVaGetValues(w, DtNrows, &rows, DtNcolumns, &columns,
                                    NULL);
...
void _DtTermParserSunWindowIcon(Widget w) /* sun set icon label */
{
        ParserContext context;
        Widget sw;
        context = GetParserContext(w);
...
void _DtTermParserSunWindowTitle /* Sun set title name */
    (Widget w) {
        ParserContext context;
        Widget sw;
        context = GetParserContext(w);
...
void _DtTermParserSunIconFile(Widget w) /* Sun set icon to pixmap in file */
{
        DtTermBuffer tb =
        ...
        icon = XmGetPixmap(XtScreen(sw), (char *)context->stringParms[0].str,
                           fg, bg);
