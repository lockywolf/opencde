...
static char rcs_id[] =
    "$XConsortium: TermColor.c /main/2 1996/05/08 13:31:00 ageorge $";
...
#ifndef BBA
#define InitColor(p, r, g, b)                                                  \
        (p)->red = r ? 0xffff : 0;                                             \
        (p)->green = g ? 0xffff : 0;                                           \
        (p)->blue = b ? 0xffff : 0
#else /* BBA */
static void InitColor(XColor *p, int r, int g, int b) {
        p->red = r ? 0xffff : 0;
        p->green = g ? 0xffff : 0;
        p->blue = b ? 0xffff : 0;
}
...
#define DebugIsColorAvailable()                                                \
        (!debugColors || (debugColors && (debugColorsAvailable > 0)))
...
void _DtTermColorDestroy(Widget w) {
        ...
        for (i = 0; i < 8; i++) {
                if (td->colorPairs[i].initialized) {
                        ...
                        if (j > 0) {
                                (void)XFreeColors(XtDisplay(w),
                                                  w->core.colormap, pixels, j,
                                                  0);
                                _DtTermProcessLock();
...
void _DtTermColorInitializeColorPair(Widget w, VtColorPair colorPair) {
        ...
        if (!colorPair->fgCommon) {
                if (DebugIsColorAvailable() &&
                    XAllocColor(XtDisplay(w), w->core.colormap,
                                &colorPair->fg)) {
                        fgValid = True;
        ...
        if (!fgValid || !bgValid) {
                /* we were unable to allocate a foreground/background pair.
                 * Let's use the base colorpair pair[0]...
                 */
                i = 0;
                ...
                if (i > 0) {
                        (void)XFreeColors(XtDisplay(w), w->core.colormap,
                                          pixels, i, 0);
                ...
                /* since this is the base pair which is always common (since it
                 * is owned either by Motif or CDE), we don't need to re-alloc
                 * the pixels to maintain the correct usage count...
                 */
        }
         ...
         */
        if (colorPair->fgCommon) {
                (void)XQueryColor(XtDisplay(w), w->core.colormap,
                                  &colorPair->fg);
        }
        if (colorPair->bgCommon) {
                (void)XQueryColor(XtDisplay(w), w->core.colormap,
         ...
         */
        if ((0 == colorPair->hb.red) && (0 == colorPair->hb.green) &&
            (0 == colorPair->hb.blue)) {
