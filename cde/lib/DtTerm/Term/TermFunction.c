...
static char rcs_id[] =
    "$TOG: TermFunction.c /main/2 1998/03/16 14:41:27 mgreess $";
...
static void termFuncErase(Widget w, int count, DtEraseMode eraseSwitch,
 ...
 *   ####    ####   #    #   ####   ######  ######
 */
#define EXIT_IF_MARGINS_SET(w)                                                 \
        if ((w)->term.tpd->scrollLockTopRow != 0 ||                            \
            (w)->term.tpd->scrollLockBottomRow != (w)->term.rows - 1)          \
                return;

static void doScroll(Widget w, short lines) {
        ...
        if (newtop > 0)
                newtop = 0;
        if (newtop < -tpd->lastUsedHistoryRow)
                newtop = -tpd->lastUsedHistoryRow;
        /* perform scroll... */
        (void)_DtTermPrimScrollTextTo(w, newtop);
        (void)_DtTermPrimScrollComplete((Widget)tw, True);
}

void _DtTermFuncScroll(Widget w, int count, FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
...
void _DtTermFuncBeginningOfBuffer(Widget w, int count,
                                  FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
...
void _DtTermFuncEndOfBuffer(Widget w, int count,
                            FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
...
static void clearToEndOfMemory(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        int i;
        ...
        if (tpd->cursorRow + tpd->topRow < tpd->lastUsedRow) {
                ...
                for (i = tpd->topRow + tpd->cursorRow + 1; i < tpd->lastUsedRow;
                     i++)
                        (void)_DtTermPrimBufferClearLine(tpd->termBuffer, i, 0);

                /* render the current line from cursor to end of line... */
                (void)_DtTermPrimRefreshText(w, tpd->cursorColumn,
                                             tpd->cursorRow, tw->term.columns,
                                             tpd->cursorRow);
                ...
                if (tpd->cursorRow > tpd->memoryLockRow) {
                        if (tpd->cursorColumn == 0) {
                                /* don't count the current line -- lastUsedRow
                                 * is the line above...
                                 */
...
static void termFuncErase(Widget w, int count, DtEraseMode eraseSwitch,
        ...
        case eraseFromRow0Col0:
                ...
                */
                ...
                tpd->cursorColumn = 0;
                for (tpd->cursorRow = tpd->topRow; tpd->cursorRow < row;
                     tpd->cursorRow++) {
                        _DtTermFuncClearLine(w, 1, functionSource);
                }
                tpd->cursorRow = row;
                ...
                /*
                ** now clear all characters from col 0 to the current cursor
                *position
                ** on the current line...
                */
                _DtTermBufferErase(
                    ((DtTermPrimitiveWidget)w)->term.tpd->termBuffer,
                    tpd->cursorRow + tpd->topRow, tpd->cursorColumn, count,
                    eraseFromCol0);
        ...
        case eraseCharCount:
                ...
                (void)_DtTermPrimRefreshText(
                    w, tpd->cursorColumn - (tpd->mbCurMax > 1 ? 1 : 0),
                    tpd->cursorRow,
...
void _DtTermFuncEraseInDisplay(Widget w, int count,
                               FunctionSource functionSource) {
        termFuncErase(w, 0, (DtEraseMode)count, functionSource);
}

void _DtTermFuncEraseInLine(Widget w, int count,
                            FunctionSource functionSource) {
        termFuncErase(w, 0, (DtEraseMode)count, functionSource);
}

void _DtTermFuncEraseCharacter(Widget w, int count,
                               FunctionSource functionSource) {
        termFuncErase(w, count, eraseCharCount, functionSource);
}

void _DtTermFuncClearToEndOfBuffer(Widget w, int count,
                                   FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        short trow;
...
void _DtTermFuncClearBuffer(Widget w, int count,
                            FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermWidget vtw = (DtTermWidget)w;
        struct termData *tpd = tw->term.tpd;
...
void _DtTermFuncClearLine(Widget w, int count, FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        int lineWidth;
        ...
        if (tpd->topRow + tpd->cursorRow < tpd->lastUsedRow) {
                ...
                lineWidth = _DtTermPrimBufferGetLineWidth(
                    tpd->termBuffer, tpd->topRow + tpd->cursorRow);
                /* clear line... */
                (void)_DtTermPrimBufferClearLine(
                    tpd->termBuffer, tpd->topRow + tpd->cursorRow, 0);
...
void _DtTermFuncClearToEndOfLine(Widget w, int count,
                                 FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        int lineWidth;
        ...
        if (tpd->topRow + tpd->cursorRow < tpd->lastUsedRow) {
                ...
                if (lineWidth > tpd->cursorColumn) {
                        (void)_DtTermPrimBufferClearLine(
                            tpd->termBuffer, tpd->topRow + tpd->cursorRow,
                            tpd->cursorColumn);
                        /* render the line.  We only need to refresh the old
                         * line width...
                         */
                        (void)_DtTermPrimRefreshText(
                            w, tpd->cursorColumn - (tpd->mbCurMax > 1 ? 1 : 0),
                            tpd->cursorRow, lineWidth, tpd->cursorRow);
                }
...
*/
void _DtTermFuncDeleteChar(Widget w, int count, FunctionSource functionSource) {
        ...
        if (row < tpd->lastUsedRow && count > 0) {
                ...
                (void)_DtTermPrimRefreshText(
                    w, col, row, ((DtTermPrimitiveWidget)w)->term.columns, row);
        }
}

void _DtTermFuncDeleteLine(Widget w, int count, FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        ...
        if (tpd->topRow + tpd->cursorRow < tpd->lastUsedRow) {
                 ...
                 */
                (void)_DtTermPrimScrollCompleteIfNecessary(
                    w, tpd->cursorRow, tw->term.rows - 1, count);

                /* clip the line count... */
                if (count >
                    tpd->scrollLockBottomRow - tpd->scrollLockTopRow + 1)
                        count = tpd->scrollLockBottomRow -
                                tpd->scrollLockTopRow + 1;
#ifdef BOGUS
...
#endif /* BOBUS */
                if (tpd->topRow + tpd->cursorRow + count >
                ...
                if (((tpd->lastUsedRow == 1) && (tpd->cursorRow == 0)) ||
                    (tpd->cursorRow + 1 == tpd->lastUsedRow)) {
                        /* we need to clear the line, re-render it, and we are
                         * done...
                         */
                        (void)_DtTermPrimBufferClearLine(
                            tpd->termBuffer, tpd->topRow + tpd->cursorRow, 0);
                         ...
                         */
                        (void)_DtTermPrimBufferDeleteLine(
                            tpd->termBuffer, tpd->topRow + tpd->cursorRow,
                            count, tpd->lastUsedRow);
                        ...
                        /* insert lines at the bottom of the scrolling region
                         * into the selection...
                         */
                        (void)_DtTermPrimSelectInsertLines(
                            w,
                            tpd->lastUsedHistoryRow + tpd->topRow +
                                tpd->scrollLockBottomRow - count + 1,
                            count);

                        /* and scroll the text on the screen... */
                        (void)_DtTermPrimScrollTextArea(
                            w, tpd->cursorRow,
                            tpd->scrollLockBottomRow + 1 - tpd->cursorRow,
                            count);
                }
        }
 ...
 *  #    #  ######   ####   ######     #
 */
void _DtTermFuncHardReset(Widget w, int count, FunctionSource functionSource) {
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
...
void _DtTermFuncSoftReset(Widget w, int count, FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        DtTermWidget vtw = (DtTermWidget)w;
        ...
        */
        vtw->vt.td->reverseWrapMode =
            False; /* this is inconsistant with xterm*/
...
void _DtTermFuncInsertLine(Widget w, int count, FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        int insertedRows = 0;
        int moveCount;
        ...
        if (tpd->lastUsedRow < tpd->bufferRows) {
                 ...
                 */
                (void)_DtTermPrimBufferInsertLineFromTB(
                    tpd->termBuffer, tpd->topRow + tpd->cursorRow, moveCount,
                    insertFromBottom);
                /* inform the selection code that the selection has been
                 * moved... */
                (void)_DtTermPrimSelectInsertLines(
                    w, tpd->lastUsedHistoryRow + tpd->topRow + tpd->cursorRow,
                    moveCount);
        ...
        if (count > 0) {
                ...
                if (moveCount > 0) {
                        /* insert them into the buffer before we modify the
                         * selecction...
                         */
                        (void)_DtTermPrimBufferInsertLineFromTB(
                            tpd->termBuffer,
                            tpd->topRow + tpd->cursorRow - moveCount, moveCount,
                            insertFromTop);

                        /* delete the lines at the top of the buffer from the
                         * selection...
                         */
                        (void)_DtTermPrimSelectDeleteLines(
                            w, tpd->lastUsedHistoryRow, moveCount);
                ...
                if (count > 0) {
                        /* DKS: we need to clip here so we don't self
                         * destruct... */
                        moveCount = count;
                        ...
                        if ((tpd->cursorRow >= tpd->scrollLockTopRow) &&
                            (tpd->cursorRow <= tpd->scrollLockBottomRow)) {
                                /* scroll within the scroll region.  So, take
                                 * memory from the bottom of the scroll region
                                 * instead of the bottom of memory...
                                 */
                                /* insert lines into the selection... */
                                (void)_DtTermPrimBufferInsertLine(
                                    tpd->termBuffer,
                                ...
                                /* we don't need to delete lines from the bottom
                                 * since that doesn't move the selection...
                                 */
                                /* insert lines into the selection... */
                                (void)_DtTermPrimBufferInsertLineFromTB(
                                    tpd->termBuffer,
        ...
        if ((tpd->cursorRow >= tpd->scrollLockTopRow) &&
            (tpd->cursorRow <= tpd->scrollLockBottomRow)) {
                (void)_DtTermPrimScrollTextArea(w, tpd->cursorRow,
                                                tpd->scrollLockBottomRow + 1 -
                                                    tpd->cursorRow,
                                                -insertedRows);
        } else {
                (void)_DtTermPrimScrollTextArea(w, tpd->cursorRow,
                                                tw->term.rows - tpd->cursorRow,
                                                -insertedRows);
        }
}
...
void _DtTermFuncTab(Widget w, int count, FunctionSource functionSource) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
...
void _DtTermVideoEnhancement(Widget w, int value) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData td = vtw->vt.td;
        ...
        case 0:
                ...
                (void)_DtTermPrimBufferSetEnhancement(
                    tpd->termBuffer, tpd->topRow + tpd->cursorRow,
                    tpd->cursorColumn, enhFgColor, td->enhFgColorState);
                td->enhBgColorState = 0;
                (void)_DtTermPrimBufferSetEnhancement(
                    tpd->termBuffer, tpd->topRow + tpd->cursorRow,
                    tpd->cursorColumn, enhBgColor, td->enhBgColorState);
                break;
...
void _DtTermFontEnhancement(Widget w, int value) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        DtTermWidget vtw = (DtTermWidget)w;
        DtTermData td = vtw->vt.td;
...
/* This are utility functions for the terminal dialog to use to effect the
   function key lock
*/
void _DtTermSetUserKeyLock(Widget w, Boolean lock_state) {
...
Boolean _DtTermGetUserKeyLock(Widget w) {
        Boolean lock_state;
        _DtTermWidgetToAppContext(w);
...
void _DtTermSetAutoLineFeed(Widget w, Boolean alf_state) {
        _DtTermWidgetToAppContext(w);
...
Boolean _DtTermGetAutoLineFeed(Widget w) {
        Boolean alf_state;
        _DtTermWidgetToAppContext(w);
...
void _DtTermSetCurrentWorkingDirectory(Widget w, char *cwd) {
        DtTermWidget vtw = (DtTermWidget)w;
...
char *_DtTermGetCurrentWorkingDirectory(Widget w) {
        DtTermWidget vtw = (DtTermWidget)w;
        char *cwd;
        _DtTermWidgetToAppContext(w);
