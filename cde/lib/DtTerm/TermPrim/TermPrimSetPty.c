...
static char rcs_id[] =
    "$TOG: TermPrimSetPty.c /main/2 1998/04/03 17:11:24 mgreess $";
...
static int parseTtyModes(char *modeString, ttyMode *modeList) {
        ttyMode *pMode;
        int c, i;
        int modeCount = 0;
        ...
        */
        ...
        while (1) {
                ...
                ** return.
                */
                while (*modeString && isascii(*modeString) &&
                       isspace(*modeString)) {
                        modeString++;
                }
                ...
                */
                ...
                if (!pMode->name) {
                        DebugF('p', 2,
                               fprintf(stderr, ">>parseTtyModes() no match\n"));
                        return (-1);
                }
...
*/
static void parseCSWidth(char *cp, eucioc_t *wp) {
        int x;
        int i;
        ...
        */
        ...
        DebugF('p', 4,
               fprintf(stderr, ">>parseCSWidth(): csWidthString %s\n", cp));
        ...
        DebugF('p', 4,
               fprintf(stderr, "    eucw[1] : %d, scrw[1] : %d\n", wp->eucw[1],
                       wp->scrw[1]));
        DebugF('p', 4,
               fprintf(stderr, "    eucw[2] : %d, scrw[2] : %d\n", wp->eucw[2],
                       wp->scrw[2]));
        DebugF('p', 4,
               fprintf(stderr, "    eucw[3] : %d, scrw[3] : %d\n", wp->eucw[3],
                       wp->scrw[3]));
}
#endif /* (USE_CSWIDTH) */
...
void _DtTermPrimPtyGetDefaultModes() {
        ...
        _DtTermProcessLock();
        if (!refValid) {
                /* see if we can get a reference tty to get our base reference
                 * from...
                 */
...
void _DtTermPrimPtyInit(int pty, char *modeString, char *csWidthString) {
        ...
        */
        ...
        if (refValid) {
                 ...
                 */
                /* we already got the termios structure.  No need to get
                 * again... */
                DebugF(
                    'p', 3,
                    fprintf(stderr, ">>_DtTermPrimPtyInit() using refTio\n"));
                tio = refTio;
...
#define TMODE(ind, var)                                                        \
        if (_DtTermPrimTtyModeList[ind].set)                                   \
                var = _DtTermPrimTtyModeList[ind].value;

        _DtTermProcessLock();
        if (modeString) {
                if (parseTtyModes(modeString, _DtTermPrimTtyModeList) < 0) {
                        /*
                        ** NOTE: should we prepend the program name to this
                        *string?
                        */
                        ...
                        {
                                /* With HP-UX 9.0 (and earlier) we need to set
                                 * dsuspc via the ltchars array.  In addition,
                                 * we have no support for rprnt, flush, weras,
                                 * and lnext...
                                 */
                                struct ltchars ltc;
                ...
                if (st.st_uid == getuid()) {
                        ...
                        if (!access(CONSOLE_DEVICE, R_OK | W_OK)) {
                                ...
                                if (ioctl(pty, TIOCCONS, &one)) {
                                        ...
                                        (void)fprintf(stderr, "attempt to make "
                                                              "tty the console "
                                                              "failed\n");
                                }
