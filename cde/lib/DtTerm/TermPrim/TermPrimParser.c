...
static char rcs_id[] =
    "$TOG: TermPrimParser.c /main/2 1999/10/15 12:23:41 mgreess $";
...
** in the start state.
*/
Boolean _DtTermPrimParse(Widget w, unsigned char *parseChar, int parseCharLen) {
...
#pragma BBA_IGNORE
        ...
        */
        ...
        if (thisPreParseEntry) {
                ...
                ** current state.  This is for parse entries that do not break
                *us
                ** out of the current parse thread.  If we need to bail out of
                *the
...
**
*/
void _DtTermPrimParserClrStrParm(Widget w) {
        ParserContext context = GetParserContext(w);
...
**
*/
void _DtTermPrimParserNextState(Widget w) {}
...
**
*/
void _DtTermPrimParserClearParm(Widget w) {
        ParserContext context = GetParserContext(w);
        int i;
...
**
*/
void _DtTermPrimParserEnterNum(Widget w) {
        ParserContext context = GetParserContext(w);

        if (context->workingNum<MAXINT>> 4)
                context->workingNum =
                    context->workingNum * 10 + (*context->inputChar - '0');
        context->workingNumIsDefault = False;
}

void _DtTermPrimParserSaveSign(Widget w) {
        ParserContext context = GetParserContext(w);

        SetSign(context, (*GetInputChar(context) == '-')
...
**
*/
void _DtTermPrimParserNumParmPush(Widget w, int parmNum) {
        ParserContext context = GetParserContext(w);
...
*/
void _DtTermPrimParserInitContext(Widget w) {
...
testFunction(_termCR) testFunction(_termLF) testFunction(_termTab) testFunction(
    _termBackspace) testFunction(RecordDC1Rcvd) testFunction(_termWriteChar)
    testFunction(_DtTermPrimBell) testFunction(_termShiftOut) testFunction(
        _termShiftIn) testFunction(set_tab) testFunction(clear_tab)
        testFunction(clr_all_tab) testFunction(set_left_marg) testFunction(
            set_right_marg) testFunction(clear_marg) testFunction(curs_up)
            testFunction(curs_down) testFunction(curs_right) testFunction(
                curs_left) testFunction(home_down) testFunction(home_up)
                testFunction(clr_display) testFunction(clear_line) testFunction(
                    insert_line) testFunction(delete_line) testFunction(delete_char)
                    testFunction(insert_char) testFunction(
                        off_insert_char) testFunction(roll_up)
                        testFunction(roll_down) testFunction(next_page) testFunction(
                            prev_page) testFunction(format_mode_on)
                            testFunction(format_mode_off) testFunction(
                                display_func) testFunction(term_status)
                                testFunction(rel_curs_pos) testFunction(
                                    abs_curs_pos) testFunction(enable_key)
                                    testFunction(disable_key) testFunction(
                                        enter_line) testFunction(back_tab)
                                        testFunction(user_key_menu_on) testFunction(
                                            user_key_menu_off)
                                            testFunction(second_status) testFunction(
                                                mlock_on) testFunction(mlock_off)
                                                testFunction(start_unprotect)
                                                    testFunction(stop_field)

                                                        void _parserPrintContext(
                                                            ParserContext
                                                                context) {
        printf("    inputChar  : ");
...
void parseString(Widget w; unsigned char *string) {
        ParserContext context = GetParserContext(w);
        int i;
