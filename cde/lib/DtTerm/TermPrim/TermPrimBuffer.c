...
static char rcs_id[] =
    "$XConsortium: TermPrimBuffer.c /main/1 1996/04/21 19:16:49 drk $";
...
*/
TermBuffer _DtTermPrimBufferCreateBuffer(const Widget w, const short rows,
                                         const short cols,
        ...
        */
        for (i = 0; i < rows; i++) {
                ...
                if (newTL[i]) {
                        BUFFER(newTL[i]) = (termChar *)malloc(
                            (unsigned)cols * BYTES_PER_CHAR(newTB));
...
void _DtTermPrimBufferFreeBuffer(const TermBuffer tb) {
        int i;
...
*/
void _DtTermPrimBufferResizeBuffer(TermBuffer *oldTB, short *newRows,
                                   short *newCols) {
        int i;
        short thisTabWidth;
        short tabWidth;
        ...
        */
        if (*newCols > MAX_COLS(*oldTB)) {
                ...
                if (tabs) {
                        ...
                        (void)memcpy(tabs, TABS(*oldTB),
                                     COLS(*oldTB) * sizeof(Boolean));
                        free(TABS(*oldTB));
                        TABS(*oldTB) = tabs;
                        ...
                        */
                        tabWidth = 0;
                        for (i = 0, thisTabWidth = 0; i < COLS(*oldTB);
                             i++, thisTabWidth++) {
                                if (TABS(*oldTB)[i]) {
                                        if (tabWidth == 0) {
                                                /* first tab stop, set
                        ...
                        if (tabWidth > 0) {
                                /*
                                ** we have consistent tab stops.  Extend the
                                *buffer...
                                */
                                ...
                                /*
                                ** we don't have consistent tab stops, so clear
                                *the rest...
                                */
                                (void)memset(&(TABS(*oldTB)[COLS(*oldTB)]),
                                             '\0',
                                             (*newCols - COLS(*oldTB)) *
                                                 sizeof(Boolean));
                        }
                        ...
                        /*
                        ** the malloc failed, adjust newCols and newRows and
                        *leave...
                        */
        ...
        */
        if (BUFFER_RESIZE(*oldTB)) {
                ...
                **    than the current column width.  It is the responsibility
                *of
                **    the rendering code to make sure that two column characters
                **    are handled properly if the second column falls past the
                *last
                ...
                */
                if (*newCols > MAX_COLS(*oldTB)) {
                        ...
                        /*
                        ** now extend the line buffers for all lines, (even
                        *lines that
                        ** are not being used at the moment (ROWS <
                        *MAX_ROWS))...
                        */
                        newTL = LINES(*oldTB);
                        for (i = 0; i < MAX_ROWS(*oldTB); i++) {
                                newLineBuffer =
                                    (termChar *)malloc((unsigned)*newCols *
                                                       BYTES_PER_CHAR(*oldTB));

                                if (newLineBuffer == NULL) {
                                        /*
                                        ** line buffer malloc failed, we can
                                        *only increase the
                                        ** width to the current maximum...
                                        */
                                        *newCols = MAX_COLS(*oldTB);
                                        *newRows =
                                            MIN(*newRows, MAX_ROWS(*oldTB));
                                        break;
                                }
                                memcpy(newLineBuffer, BUFFER(newTL[i]),
                                       LENGTH(newTL[i]));
                                free(BUFFER(newTL[i]));
                ...
                */
                if (*newRows > MAX_ROWS(*oldTB)) {
                        ...
                        */
                        ...
                        if (newTL != NULL) {
                                /*
                                ** the malloc succeeded, copy the old
                                *information, and
                                ** then free it...
                                */
                                memcpy(newTL, LINES(*oldTB),
                                       sizeof(TermLine) * ROWS(*oldTB));
                                free(LINES(*oldTB));
                                LINES(*oldTB) = newTL;
                                ...
                                */
                                for (i = ROWS(*oldTB); i < *newRows; i++) {
                                        ...
                                        if (newTL[i]) {
                                                ...
                                                WIDTH(newTL[i]) = 0;
                                                WRAPPED(newTL[i]) = False;
                                                START_SELECTION(newTL[i]) =
                                                    NON_SELECTION_COL;
                                                END_SELECTION(newTL[i]) =
                                                    NON_SELECTION_COL;
                                        } else {
                                                /*
                                                ** line malloc failed, lets
                                                *limit the
                                                ** number of rows
                                                */
                                                *newRows = i;
                                ...
                                /*
                                ** its conceivable that MAX_ROWS could actually
                                *decrease if
                                ** we are low on memory...
                                */
                                MAX_ROWS(*oldTB) =
                                    MIN(MAX_ROWS(*oldTB), *newRows);
                        } else {
                                /*
                                ** the row buffer malloc failed, revert back to
                                *MAX_ROWS
                                */
...
*/
/*ARGSUSED*/
Boolean _DtTermPrimBufferCreateEnhancement(const TermBuffer tb, const short row,
                                           const short col) {
        return (True);
}
...
*/
static void clearEnhancement(const TermBuffer tb, const short row,
                             const short col) {}
#endif /* NOCODE */
...
*/
static void insertEnhancements(const TermBuffer tb, const short row,
                               const short col, const short insertCount,
...
*/
Boolean _DtTermPrimBufferFreeEnhancement(const TermBuffer tb, const short row,
                                         const short col) {
        return (True);
}
...
#pragma BBA_IGNORE
#endif /*BBA*/
static void RememberTermBuffer(const TermBuffer tb, TermLine **referenceLines,
...
#pragma BBA_IGNORE
#endif /*BBA*/
static void CheckTermBuffer(const TermBuffer tb, TermLine *referenceLines,
                            int referenceLineCount) {
        int i1;
        int i2;
        int matchedLine;
        ...
        if (referenceLineCount != ROWS(tb)) {
                for (i1 = 0; i1 < ROWS(tb); i1++) {
                        ...
                        if (matchedLine == -1) {
                                fprintf(stderr,
                                        ">>CheckTermBuffer: dropped pointer "
                                        "src=%d\n",
                                        i1);
                        }
                }
...
short _DtTermPrimBufferGetText(const TermBuffer tb, const short row,
                               const short col, const short width, char *buffer,
        ...
        if (colWidth > 0) {
                ...
                _DtTermPrimGetCharacterInfo(tb, row, col + colWidth - 1,
                                            &charInfoLast);
                ...
                if ((BYTES_PER_CHAR(tb) > 1) && !needWideChar) {
                        ...
                        */
                        ...
                        for (pwc = charInfoFirst.u.pwc;
                             pwc <= charInfoLast.u.pwc; pwc++) {
                                mbLen = wctomb(pChar, *pwc);
                                if (mbLen > 0) {
                                        pChar += mbLen;
                                        retLen += mbLen;
                                } else {
                                        fprintf(stderr, "_DtTermPrimBufferGetTe"
                                                        "xt: invalid wide "
                                                        "char.\n");
                                }
                        }
...
short _DtTermPrimBufferGetRows(const TermBuffer tb) { return (ROWS(tb)); }

short _DtTermPrimBufferGetCols(const TermBuffer tb) { return (COLS(tb)); }

void _DtTermPrimBufferSetLinks(const TermBuffer tb, const TermBuffer prev,
                               const TermBuffer next) {
        PREV_BUFFER(tb) = prev;
        NEXT_BUFFER(tb) = next;
}

void _DtTermPrimBufferSetSelectInfo(const TermBuffer tb,
...
** function.
*/
int _DtTermPrimBufferSetEnhancement(const TermBuffer tb, const short row,
                                    const short col, const unsigned char id,
...
*/
Boolean _DtTermPrimBufferGetEnhancement(const TermBuffer tb, const short row,
                                        const short col, enhValue **values,
                                        short *count,
                                        const countSpec countWhich) {
        if (GET_ENH(tb)) {
                return (
                    (*GET_ENH(tb))(tb, row, col, values, count, countWhich));
        } else {
...
*/
termChar *_DtTermPrimBufferGetCharacterPointer(const TermBuffer tb,
                                               const short row,
                                               const short col) {
...
**      enhancements
*/
Boolean _DtTermPrimGetCharacterInfo(TermBuffer tb, short row, short col,
                        ...
                        */
                        ...
                        for (pwc = (wchar_t *)BUFFER(line);
                             ...
                             pwc++) {
                                ...
                                if (col < totalWidth) {
                                        /*
                                        ** we've found the character, now fill
                                        *in the info...
                                        */
                                        charInfo->u.pwc = pwc;
                                        charInfo->idx =
...
*/
short _DtTermPrimBufferInsert(
    const TermBuffer tb, const short row, const short col,
    const termChar *newChars, short numChars,
        ...
        if (BYTES_PER_CHAR(tb) > 1) {
                ...
                */
                _DtTermPrimBufferInsertWc(tb, row, col, (wchar_t *)newChars,
                                          numChars, insertFlag, returnChars,
                                          returnCount);
                return 0;
        }
        ...
        */
        ...
        if (insertFlag == False) {
                ...
                *returnCount = MAX(0, (WIDTH(line) + numChars) - COLS(tb));
                if (*returnCount > 0) {
                        /*
                        ** we'll have some overflow, figure out how many (if
                        *any)
                        ...
                        */
                        memcpy(*returnChars + insertOverflow,
                               BUFFER(line) +
                                   (WIDTH(line) -
...
*/
void _DtTermPrimBufferDelete(
    TermBuffer tb, short *row, short *col, short *width,
...
*/
void _DtTermPrimBufferErase(TermBuffer tb, short row, short startCol,
                            short stopCol) {
        TermCharInfoRec charInfo;
        char *pchar;
...
*/
Boolean _DtTermPrimBufferSetLineWidth(const TermBuffer tb, const short row,
                                      short newWidth) {
        TermLine line;
        ...
        */
        ...
        if (BYTES_PER_CHAR(tb) == 1) {
                /*
                ** in single byte locales, we go ahead and set the line length
                *too,
                ** it is the responsibility of wide-character routines to set
                *the
...
*/
short _DtTermPrimBufferGetLineLength(const TermBuffer tb, const short row) {
        short retLen;
        short width;
        TermCharInfoRec charInfo;
        ...
        if (VALID_ROW(tb, row)) {
                ...
                if (width > 0) {
                        ...
                        (void)_DtTermPrimGetCharacterInfo(tb, row, width - 1,
                                                          &charInfo);

                        /*
                        ** a line can not end on column 1 of 2.  Note that we
                        *must
                        ** add 1 to the column index to make it the line
                        *length...
                        */
                        if ((charInfo.width == 2) &&
                            (charInfo.startCol != width - 2)) {
                                /* only half of charInfo.idx fits in the line,
                                 * so drop it...
                                 */
                                retLen = charInfo.idx;
                        } else {
                                /* single column character or double column
...
** (in the case that the buffer shrank after the line was full)
*/
short _DtTermPrimBufferGetLineWidth(const TermBuffer tb, const short row) {
        short width = 0;
        TermCharInfoRec charInfo;
 ...
 *  Notes:
 */
void _DtTermPrimBufferMoveLockArea(const TermBuffer tb, const short dest,
                                   const short src, const short length) {
        ...
        /* destination... */
        if ((dest > 0) &&
            IS_IN_SELECTION(LINE_OF_TBUF(tb, dest - 1), MIN_SELECTION_COL,
                            MAX_SELECTION_COL) &&
            IS_IN_SELECTION(LINE_OF_TBUF(tb, dest), MIN_SELECTION_COL,
                            MAX_SELECTION_COL)) {
                (void)_DtTermPrimSelectDisown(WIDGET(tb));
        }
        ...
        */
        ...
        if (distance > length) {
                ...
                if (dest > src) {
                        ...
                        /* restore the area under (that is now over) the lock
                         * area... */
                        destPtr = &(LINE_OF_TBUF(tb, src));
                        srcPtr = holdLines;
                        for (i = 0; i < distance; i++) {
                        ...
                        /* restore the area over (that is now under) the lock
                         * area... */
 ...
 *  Notes:
 */
void _DtTermPrimBufferInsertLine(const TermBuffer tb, const short dest,
                                 const short length, const short src) {
        TermLine lineCache[10];
        TermLine *holdLines = lineCache;
        TermLine *destPtr;
        TermLine *srcPtr;
        ...
        if (dest > src) {
                ...
                srcPtr = &(LINE_OF_TBUF(tb, src));
                for (i = 0; i < length; i++) {
                        /* these lines will be cleared, so we don't need to
                         * track movement of them...
                         */
                        *destPtr++ = *srcPtr++;
                }
                ...
                for (i = 0; i < length; i++) {
                        *destPtr = *srcPtr;
                        /* clear the line... */
                        _DtTermPrimBufferClearLine(tb, dest - length + 1 + i,
                                                   0);
                        *destPtr++;
                        *srcPtr++;
                }
 ...
 *  Notes:
 */
void _DtTermPrimBufferInsertLineFromTB(const TermBuffer tb, const short dest,
                                       const short length,
 ...
 *  Notes:
 */
int _DtTermPrimBufferScrollToHistory()(const TermBuffer tb,
                                       const short length, ) {
        if (PREV_BUFFER(tb)) {
        }
#endif /* NOTDONE */
        ...
        /**************************************************************************
         *  Function:
         *	_DtTermPrimBufferDeleteLine():  deletes one or more lines of
         *text from the deletion poing and sticks them at the end of the line
         *buffer.  It does not clear the line.  That is normally done when the
         *line is inserted.
         *
         *  Parameters:
         *	TermBuffer tb: term buffer to insert end/begining into
         ...
         *  Notes:
         */
        void _DtTermPrimBufferDeleteLine(const TermBuffer tb,
                                         const short source, const short length,
                ...
                DebugF('B', 1,
                       RememberTermBuffer(tb, &refLines, &refLineCount));

                /* don't bother if there is we are being asked to do nothing...
                 */
                if (length <= 0)
                        return;
                /* if we are being asked to move the last usedLines, we don't
                ...
                /* if we are moving more lines than will fit in the lineCache,
                 * we need to malloc (and free) storage for the termLineRecs...
                 */
                if (length > (sizeof(lineCache) / sizeof(lineCache[0]))) {
                        holdLines =
                            (TermLine *)XtMalloc(length * sizeof(TermLine));
                }
...
#ifdef USE_MEMCPY
                (void)memcpy(holdLines, &(LINE_OF_TBUF(tb, source)),
        ...
        for (i = 0; i < length; i++) {
                *destPtr++ = *srcPtr++;
                ...
                /* ripple up the lines... */
                copyLength =
                    MAX(0, MIN(ROWS(tb), lastUsedRow) - source - length);
                if (copyLength > 0) {
#ifdef USE_MEMCPY
...
#ifdef USE_MEMCPY
                (void)memcpy(&(LINE_OF_TBUF(tb, source + copyLength)),
                             holdLines, length * sizeof(TermLine));
#else  /* USE_MEMCPY */
        ...
        /*
        ** Pad the requested row from the current width to 'newWidth' with
        *spaces...
        */
        void _DtTermPrimBufferPadLine(const TermBuffer tb, const short row,
                ...
                /* if this line is part of the selection, disown the
                 * selection... */
                ...
                (void)memset(BUFFER(line) + WIDTH(line), 0x20,
                             (newWidth - WIDTH(line)) * BYTES_PER_CHAR(tb));
                if (CLEAR_ENH(tb)) {
                        (*CLEAR_ENH(tb))(tb, row, WIDTH(line),
                                         newWidth - WIDTH(line));
                }
                _DtTermPrimBufferSetLineWidth(tb, row, newWidth);
        }
        ...
        */
        Boolean _DtTermPrimBufferClearLine(const TermBuffer tb, const short row,
                                           short newWidth) {
                TermLine line;
                ...
                */
                if (BYTES_PER_CHAR(tb) > 1) {
                        return (
                            _DtTermPrimBufferClearLineWc(tb, row, newWidth));
                }
                ...
                **
                ** (We take the direct approach because
                ...
                */
                if (newWidth < WIDTH(line)) {
                        ...
                        */
                        ...
                        WIDTH(line) = newWidth;
                        LENGTH(line) =
                            newWidth; /* this works in single-byte locales */
                }
                return (True);
        }

        short _DtTermPrimBufferGetNextTab(const TermBuffer tb, short col) {
                if (!VALID_COL(tb, col)) {
                        return (0);
                }
        ...
        short _DtTermPrimBufferGetPreviousTab(const TermBuffer tb, short col) {
                ...
                /* we can just return col.  If ther was not another tab stop, it
                 * will be -1...
                 */
                return (col);
        }

        Boolean _DtTermPrimBufferSetTab(const TermBuffer tb, const short col) {
                if (VALID_COL(tb, col)) {
                        TABS(tb)[col] = True;
                        return (True);
                }
        ...
        Boolean _DtTermPrimBufferClearTab(const TermBuffer tb,
                                          const short col) {
                if (VALID_COL(tb, col)) {
                        TABS(tb)[col] = False;
                        return (True);
        ...
        Boolean _DtTermPrimBufferClearAllTabs(const TermBuffer tb) {
        ...
        */
        void _DtTermPrimBufferSetLineWrapFlag(TermBuffer tb, short row,
                                              Boolean state) {
                if (VALID_ROW(tb, row)) {
        ...
        */
        Boolean _DtTermPrimBufferTestLineWrapFlag(TermBuffer tb, short row) {
                if (VALID_ROW(tb, row)) {
        ...
        */
        void _DtTermPrimBufferSetInSelectionFlag(TermBuffer tb, short row,
                                                 TermLineSelection state) {
                if (VALID_ROW(tb, row)) {
                        if (state) {
                                START_SELECTION(LINE_OF_TBUF(tb, row)) =
                                    state->selectionStart;
                                END_SELECTION(LINE_OF_TBUF(tb, row)) =
                                    state->selectionEnd;
                        } else {
                                START_SELECTION(LINE_OF_TBUF(tb, row)) =
                                    NON_SELECTION_COL;
        ...
        */
        TermLineSelection _DtTermPrimBufferGetInSelectionFlag(TermBuffer tb,
                                                              short row) {
                if (VALID_ROW(tb, row)) {
                        if ((START_SELECTION(LINE_OF_TBUF(tb, row)) !=
                             NON_SELECTION_COL) &&
                            (END_SELECTION(LINE_OF_TBUF(tb, row)) !=
                             NON_SELECTION_COL)) {
        ...
        void _DtTermPrimBufferSetSelectLines(TermBuffer tb, short beginRow,
                                             short beginCol, short endRow,
                                             short endCol) {
                int i1;
                ...
                for (i1 = 0; i1 < ROWS(tb); i1++) {
                        if ((i1 >= beginRow) && (i1 <= endRow)) {
                                if (i1 == beginRow) {
                                        START_SELECTION(LINE_OF_TBUF(tb, i1)) =
                                            beginCol;
                                } else {
                                        START_SELECTION(LINE_OF_TBUF(tb, i1)) =
                                            MIN_SELECTION_COL;
                                ...
                                if (i1 == endRow) {
                                        END_SELECTION(LINE_OF_TBUF(tb, i1)) =
                                            endCol;
                                } else {
                                        END_SELECTION(LINE_OF_TBUF(tb, i1)) =
                                            MAX_SELECTION_COL;
        ...
        static void _termBufferPrintLine(const TermBuffer tb, const short row) {
                TermLine line;
                termChar *pChar;
                short j;
    ...
    if (LENGTH(line) > 0)
    {
        ...
        */
        static void _termBufferPrintBuffer(const TermBuffer tb) {
                short i;
                short j;
                short k;
