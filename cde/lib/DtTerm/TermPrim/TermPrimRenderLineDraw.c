...
static char rcs_id[] =
    "$XConsortium: TermPrimRenderLineDraw.c /main/1 1996/04/21 19:18:59 drk $";
...
static void LineDrawRenderFunction(Widget w, TermFont font, Pixel fg, Pixel bg,
                                   unsigned long flags, int x, int y,
                                   unsigned char *rawString, int len) {
        ...
        if (tpd->mbCurMax > 1) {
                ...
                for (ptr = string, wPtr = (wchar_t *)rawString, i1 = 0;
                     i1 < len; i1++, wPtr++) {
                        switch (wcwidth(*wPtr)) {
                        case 1:
...
#pragma BBA_IGNORE
                 ...
                 * be displayed...
                 */
                (void)XFillRectangle(XtDisplay(w), XtWindow(w),
                                     tpd->renderGC.gc, x, y,
                                     tpd->cellWidth * len, tpd->cellHeight);
...
static void LineDrawDestroyFunction(Widget w, TermFont font) {
        LineDrawFont lineDrawFont = (LineDrawFont)font->fontInfo;
...
static void LineDrawExtentsFunction(Widget w, TermFont font,
                                    unsigned char *string, int len,
                                    int *widthReturn, int *heightReturn,
                                    int *ascentReturn) {
        LineDrawFont lineDrawFont = (LineDrawFont)font->fontInfo;
...
TermFont _DtTermPrimRenderLineDrawCreate(Widget w, GlyphInfo glyphInfo,
                                         int numGlyphs, int width, int ascent,
                                         int descent) {
        TermFont termFont;
        ...
        termFont->fontInfo = (XtPointer)_DtTermPrimLineDrawCreateFont(
            w, glyphInfo, numGlyphs, width, ascent, descent);
        return (termFont);
}
