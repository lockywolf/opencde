...
static char rcs_id[] =
    "$TOG: TermPrimSubproc.c /main/11 1998/04/20 12:45:57 mgreess $";
...
/*ARGSUSED*/
static void InvokeCallbacks(XtPointer client_data, XtSignalId *id) {
        subprocInfo *subprocTmp = (subprocInfo *)client_data;

        if (subprocTmp->w && subprocTmp->proc)
                (subprocTmp->proc)(subprocTmp->w, subprocTmp->pid,
                                   &subprocTmp->stat_loc);
}

_termSubprocId _DtTermPrimAddSubproc(Widget w, pid_t pid, _termSubprocProc proc,
                                     XtPointer client_data) {
...
void _DtTermPrimSubprocRemoveSubproc(Widget w, _termSubprocId id) {
...
void _DtTermPrimSetChildSignalHandler(void) {
...
void DtTermSubprocReap(pid_t pid, int *stat_loc) {
        /*
...
pid_t _DtTermPrimSubprocExec(Widget w, char *ptyName, Boolean consoleMode,
                             char *cwd, char *cmd, char **argv,
                             Boolean loginShell) {
         ...
         *	    - from $SHELL,
         *	    - from the /etc/passwd entry for the /etc/utmp entry for
         *this terminal,
        ...
        if (!cmd || !*cmd) {
                if (!defaultCmd) {
                        ...
                        /* if not valid, try the /etc/passwd entry for the
                         * username associated with the /etc/utmp entry for this
                         * tty...
                         */
                        if (!c || !*c) {
                                /* get the /etc/passwd entry for the username
                                 * associated with /etc/utmp...
                                 */
                                if ((namebuf = _XGetlogin(login_buf)) != NULL) {
                                        /* get the user's passwd entry... */
                                        pw = _XGetpwnam(namebuf, pw_buf);
                                        /* if we weren't able to come up with
                        ...
                        /* if not valid, try the /etc/passwd entry for the
                         * username associate with the real uid...
                         */
                        if (!c || !*c) {
                                /* if we weren't able to come up with one for
                                 * the userid... */
                        ...
                        /* malloc space for this string.  It will be free'ed in
...
#pragma BBA_IGNORE
    ...
    defined(LINUX_ARCHITECTURE)
                ...
                if (ioctl(pty, TIOCSCTTY, (char *)NULL) < 0) {
                        (void)close(pty);
                        (void)perror("Error acquiring pty slave as controlling "
                                     "terminal");
                        /* exit the subprocess */
                        _exit(1);
                }
...
#ifdef MOVE_FDS
                /* that should have open'ed into fd 0.  Dup it into fd's 1
                 * and 2... */
                (void)dup(pty);
                (void)dup(pty);
#else  /* MOVE_FDS */
                ...
                */
                ...
                /* set our utmp entry... */
                (void)_DtTermPrimUtmpEntryCreate(
                    w, getpid(), ((DtTermPrimitiveWidget)w)->term.tpd->utmpId);
