...
static char rcs_id[] =
    "$XConsortium: TermPrimRenderFont.c /main/1 1996/04/21 19:18:47 drk $";
...
static void FontRenderFunction(Widget w, TermFont font, Pixel fg, Pixel bg,
...
static void FontDestroyFunction(Widget w, TermFont font) {
        (void)XtFree((char *)font);
}

static void FontExtentsFunction(Widget w, TermFont font, unsigned char *string,
                                int len, int *widthReturn, int *heightReturn,
                                int *ascentReturn) {
        XFontStruct *fontStruct = (XFontStruct *)font->fontInfo;
...
TermFont _DtTermPrimRenderFontCreate(Widget w, XFontStruct *fontStruct) {
        TermFont termFont;
