...
static char rcs_id[] =
    "$XConsortium: TermPrimCursor.c /main/1 1996/04/21 19:17:13 drk $";
...
static void timeoutCallback(XtPointer client_data, XtIntervalId *id) {
        ...
        if (tw->term.blinkRate > 0) {
                tpd->cursorTimeoutId = XtAppAddTimeOut(
                    XtWidgetToApplicationContext((Widget)tw),
...
void _DtTermPrimCursorChangeFocus(Widget w) {
        ...
        if (tw->term.hasFocus) {
                ...
                if ((!tpd->IMHasFocus) ||
                    ...
                    (tpd->IMCursorRow != tpd->cursorRow)) {
                        ...
                        point.x =
                            tpd->IMCursorColumn * tpd->cellWidth + tpd->offsetX;
                        point.y = tpd->IMCursorRow * tpd->cellHeight +
                                  tpd->offsetY + tpd->ascent;
                        DebugF('F', 1,
                               fprintf(stderr, "%s() %s calling %s\n",
                                       "_DtTermPrimCursorChangeFocus",
                                       "hasFocus", "XmImVaSetFocusValues()"));
                        (void)XmImVaSetFocusValues(w, XmNspotLocation, &point,
                                                   NULL);
                }
                ...
                if (tpd->cursorVisible && (!tpd->cursorTimeoutId) &&
                    ...
                    (tpd->cursorState != CURSORoff)) {
                        ...
                        tpd->cursorTimeoutId = XtAppAddTimeOut(
                            XtWidgetToApplicationContext(w), tw->term.blinkRate,
                            timeoutCallback, (XtPointer)w);
                }
        } else {
                if (tpd->IMHasFocus) {
                ...
                if (tpd->cursorTimeoutId && (tpd->cursorState != CURSORoff)) {
                        ...
                        Debug('F',
                              fprintf(stderr, ">>removing the timeout...\n"));
                        (void)XtRemoveTimeOut(tpd->cursorTimeoutId);
                        tpd->cursorTimeoutId = (XtIntervalId)0;
                } else {
                        if (tw->term.blinkRate > 0) {
                                Debug('F',
                                      fprintf(stderr, ">>we lost focus, but "
                                                      "cursor is not on and "
                                                      "blinking...\n"));
                        }
                }
        }
...
static void cursorToggle(Widget w) {
        ...
        tpd->cursorState =
            (tpd->cursorState == CURSORon) ? CURSORblink : CURSORon;

        return;
}

void _DtTermPrimCursorOn(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        XPoint point;
        ...
        if (alreadyActive) {
                ...
                fprintf(stderr, "tell Dave _DtTermPrimCursorOn has "
                                "alreadyActive == True\n");
                /*DKS!!!
                 * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ */
        ...
        if (tw->term.jumpScroll && tpd->scroll.jump.scrolled) {
                ...
                alreadyActive = True;
                Debug('t', fprintf(stderr, ">>_DtTermPrimCursorOn() calling "
        ...
        if ((tpd->IMCursorColumn != tpd->cursorColumn) ||
            (tpd->IMCursorRow != tpd->cursorRow)) {
                ...
                point.y = tpd->cursorRow * tpd->cellHeight + tpd->offsetY +
                          tpd->ascent;
                DebugF('F', 1,
                       fprintf(stderr, "%s() %s calling %s\n",
                               "_DtTermPrimCursorOn", "dont care",
                               "XmImVaSetValues()"));
                (void)XmImVaSetValues(w, XmNspotLocation, &point, NULL);
        }
#ifdef NOT_NEEDED
        if (!tw->term.hasFocus) {
                (void)fprintf(stderr, "%s() %s calling %s\n",
                              "_DtTermPrimCursorOn", "!hasFocus",
                              "XmImUnsetFocus()");
                (void)XmImUnsetFocus(w);
        }
#endif /* NOT_NEEDED */
        ...
        if (!((tpd->lastUsedRow <= tpd->topRow + tpd->cursorRow) ||
              (_DtTermPrimBufferGetLineWidth(tpd->termBuffer,
               ...
               MIN(tpd->cursorColumn, tw->term.columns - 1)))) {
                /* get the current enhancement to determine the color pair to
                 * use...
                 */
                ...
                if (ENH_PROC(tpd->termBuffer)) {
                        (void)(*(ENH_PROC(tpd->termBuffer)))(w, enhancements,
                                                             &enhInfo);
                }
        }
        ...
        if (valueMask) {
                (void)XChangeGC(XtDisplay(w), tpd->cursorGC.gc, valueMask,
                                &values);
        }
...
void _DtTermPrimCursorUpdate(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        Arg arglist[10];
        int i;
...
#define NO_SCROLL_REGION_HISTORY_SCROLL
                        ...
                        value = tpd->lastUsedRow +
                                (tpd->useHistoryBuffer ? tpd->lastUsedHistoryRow
                                                       : 0) -
                                tpd->scrollLockTopRow -
                                (tw->term.rows - 1 - tpd->scrollLockBottomRow);

                        /* add in any non-exisstent rows below the last used
                         * row...
                         */
                ...
                if (tpd->useHistoryBuffer) {
                        ...
                        value = tw->term.rows - tpd->scrollLockTopRow -
                                (tw->term.rows - 1 - tpd->scrollLockBottomRow);
                }
                ...
                /* verticalScrollBarPageIncrement =
                 * verticalScrollBarSliderSize... */
                ...
                Debug('b', if (newMaximum || newValue) {
                        fprintf(stderr,
                                ">>_DtTermPrimCursorUpdate: sb size=%d  min=%d "
                                " max=%d  value=%d  pginc=%d\n",
                ...
                if (newMaximum) {
                        (void)XtSetArg(arglist[i], XmNmaximum,
                                       tw->term.verticalScrollBarMaximum);
                        i++;
                        (void)XtSetValues(tw->term.verticalScrollBar, arglist,
                                          i);
                } else if (newValue) {
                        /* only need to worry about setting value, slidersize,
                         * and page increment...
                         */
                        (void)XmScrollBarSetValues(
                            tw->term.verticalScrollBar,
                            tw->term.verticalScrollBarValue,
                            /* value			*/
                            tw->term.verticalScrollBarSliderSize,
                            /* slider_size			*/
...
void _DtTermPrimCursorOff(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
...
*/
void _DtTermPrimCursorMove(Widget w, int row, int col) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
...
void _DtTermPrimSetCursorVisible(Widget w, Boolean visible) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
...
Boolean _DtTermPrimGetCursorVisible(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
