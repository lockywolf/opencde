...
static char rcs_id[] =
    "$XConsortium: TermPrimRenderFontSet.c /main/1 1996/04/21 19:18:53 drk $";
...
static void FontSetRenderFunction(Widget w, TermFont font, Pixel fg, Pixel bg,
        ...
        /* Text height may be smaller than cellHeight (happens when using font
           sets). In this case we need to fill background manually and then use
           X*DrawString instead of X*DrawImageString */
        ...
        if (fixExtents) {
                ...
                (void)XFillRectangle(XtDisplay(w), XtWindow(w),
                                     tpd->renderGC.gc, x, y, escapement,
                                     tpd->cellHeight);
        }
...
#pragma BBA_IGNORE
        ...
        if (tpd->mbCurMax == 1) {
                /* select right function, we do not want text background be
                 * drawn twice */
...
static void FontSetDestroyFunction(Widget w, TermFont font) {
        (void)XtFree((char *)font->fontInfo);
        (void)XtFree((char *)font);
}

static void FontSetExtentsFunction(Widget w, TermFont font,
                                   unsigned char *string, int len,
...
TermFont _DtTermPrimRenderFontSetCreate(Widget w, XFontSet fontSet) {
        TermFont termFont;
