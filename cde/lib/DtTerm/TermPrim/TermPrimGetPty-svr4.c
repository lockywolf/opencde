...
static char rcs_id[] =
    "$XConsortium: TermPrimGetPty-svr4.c /main/1 1996/04/21 19:17:39 drk $";
...
#pragma BBA_IGNORE
        ...
        if ((ptyFd = open(*ptyMaster, O_RDWR, 0)) >= 0) {
                ...
                /* use grantpt to prevent other processes from grabbing the tty
                 * that goes with the pty master we have opened.  It is a
                 * mandatory step in the SVR4 pty-tty initialization.  Note that
                 * /dev must be mounted read/write...
                 */
                ...
                /* Unlock the pty master/slave pair so the slave can be opened
                 * later */
                ...
                Debug('T',
                      timeStamp("_DtTermPrimGetPty() unlockpt() finished"));

#ifdef USE_STREAMS_BUFMOD
                if (ioctl(ptyFd, I_PUSH, "bufmod") == -1) {
                        (void)perror("I_PUSH bufmod");

                        /* We can't push bufmod.  This means that we're probably
                        ...
                        /* Note that we're not using SB_SEND_ON_WRITE |
                         * SB_DEFER_CHUNK. Turns out the shell (or someone down
                         * the pty) does an ioctl when sending out each prompt.
                         * Since this flushes any partially filled chunk
                         * automatically, we really don't need to do this...
                         */
                        ...
                        if (ioctl(ptyFd, I_STR, &cmd) < 0) {
                                ...
                                /* If we pushed bufmod, but this ioctl fails, it
                                 * means we're most likely running on a system
                                 * with old bufmod (i.e., for released OSs this
                                 * must be Jupiter).  We treat this error
                                 * silently so developers and users of the Mars
                                 * trees don't get confused.  Treat it like
                                 * bufmod wasn't there at all...
                                 */
                                goto backoff;
                        }
                        ...
                        if (ioctl(ptyFd, I_STR, &cmd) < 0) {
                                /* These are legit errors.  If we have new
                        ...
                        /* I have made the chunk size the same as the buffer
                         * used in the ttysw.  One could experiment here, but
                         * this works...
                         */
                        {
                                /* struct cbuf *sizeit; */
                                ...
                                chunk =
                                    2048; /* taken from xview source def of
                                             cbuf... */
                        }
                ...
                if (c = ptsname(ptyFd)) {
                        *ptySlave = malloc(strlen(c) + 1);
...
#ifdef NOTDEF
                        {
                                ...
                                if ((slaveFd = open(*ptySlave, O_RDWR, 0)) <
                                    0) {
                                        (void)perror(*ptySlave);
                                        (void)fprintf(stderr, "it failed!\n");
                                        (void)snprintf(buffer, BUFSIZ,
                                                       "ls -l %s", *ptySlave);
                                        (void)system(buffer);
                                } else {
                                        (void)close(slaveFd);
                        ...
                        /* ptsname on the pty master failed.  This should not
                         * happen!... */
                        (void)perror("ptsname");
                        (void)close(ptyFd);
                }
...
static int SetupPty(char *ptySlave, int ptyFd) {
/*
