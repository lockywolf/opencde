...
static char rcs_id[] =
    "$XConsortium: TermPrimDebug.c /main/4 1996/11/21 19:58:15 drk $";
...
#pragma BBA_IGNORE
#endif /*BBA*/
int isDebugFSet(int c, int f) {
        int i;

        _DtTermProcessLock();
        i = ((f < _TERM_MAX_DEBUG_FLAG)
                 ? debugLevel[c % 256] &&
                       (debugLevel[c % 256][__TERM_DEBUG_BYTE(f)] &
                        __TERM_DEBUG_BIT(f))
                 : 0);
                                ...
                                if (!debugLevel[(int)*c2]) {
                                        debugLevel[(int)*c2] =
                                            (unsigned char *)malloc(
                                                _TERM_DEBUG_NUM_BYTES);
                                        memset(debugLevel[(int)*c2], '\0',
                                               _TERM_DEBUG_NUM_BYTES);
                                }
                                for (i1 = 0; i1 < _TERM_DEBUG_NUM_BYTES; i1++) {
                                        debugLevel[(int)*c2][i1] |=
                                            theseFlags[i1];
                                }
                        }
                }
...
#endif /*BBA*/
/*ARGSUSED*/
static void ding(int sig) { timedOut = True; }

#ifdef BBA
#pragma BBA_IGNORE
#endif /*BBA*/
void shortSleep(int msec) {
...
void timeStamp(char *msg) {
        ...
        if (!timeStampFile) {
                ...
                if (!(c = getenv("timeStampFileName"))) {
                        (void)snprintf(buffer, BUFSIZ, "timeStamp-%d",
                                       getpid());
                        c = buffer;
                }
...
static char *enumToName(enumName *list, int value) {
        int i1;
        char buffer[BUFSIZ];
