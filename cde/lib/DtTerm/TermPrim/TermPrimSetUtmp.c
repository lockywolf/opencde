...
static char rcs_id[] =
    "$TOG: TermPrimSetUtmp.c /main/10 1998/04/03 17:11:42 mgreess $";
...
static void DeleteUtmpInfo(char *utmpLine) {
        utmpInfo *utmpInfoTmp;
        sigset_t newSigs;
        sigset_t oldSigs;
...
*/
        ...
        if (!_DtTermPrim_XA_UtmpLine) {
                _DtTermPrim_XA_UtmpLine =
                    XInternAtom(XtDisplay(w), TermUtmpIdString, False);
        }
...
static char *UtmpEntryCreate(Widget w, pid_t pid, char *utmpLine) {
        ...
        if (sizeof(utPtr->UT_HOST) < strlen(displayName)) {
                ...
                /* back up through the display name.  Each time we hit a '.'
                 * that signals the start of a new domain chunk, see if we can
                 * fit the display name (less these domain chunks) plus the seat
                 * number into the structure...
                 */
                ...
                for (; c >= utPtr->UT_HOST; c--) {
                        ...
                        if ((*c == '.') && (c - utPtr->UT_HOST + strlen(seat) <
                                ...
                                /* everything left of the dot plus the seat will
                                 * fit!... */
                                break;
                        }
                }
                if (c >= utPtr->UT_HOST) {
                        ...
                        (void)strncpy(c, seat,
                                      sizeof(utPtr->UT_HOST) -
                                          (c - utPtr->UT_HOST));
                        if ((c - utPtr->UT_HOST) + strlen(seat) <
                            sizeof(utPtr->UT_HOST)) {
                                /* null out the end of the host name... */
                                utPtr->UT_HOST[c - utPtr->UT_HOST +
                                               strlen(seat)] = '\0';
#ifdef UT_HOST_LEN
                                utPtr->UT_HOST_LEN =
                                    c - utPtr->UT_HOST + strlen(seat) + 1;
#endif /* UT_HOST_LEN */
                        }
                } else {
                        /* we can't fit even a single full chunk from the
                         * domain. since it is more important to get the seat
                         * number in (the host can be obtained from the addr),
                         * truncate the host.
                         */
...
static void UtmpEntryDestroy(Widget w, char *utmpLine) {
 ...
 * previous function setuid root...
 */
void _DtTermPrimUtmpEntryDestroy(Widget w, char *utmpLine) {
        /* this function needs to be suid root... */
        (void)_DtTermPrimToggleSuidRoot(True);
...
void _DtTermPrimUtmpCleanup() {
        DebugF('s', 10,
               fprintf(stderr, ">>_DtTermPrimUtmpCleanup() starting\n"));
        _DtTermProcessLock();
        while (utmpInfoHead->next && utmpInfoHead->next->utmpLine) {
                DebugF('s', 10,
                       fprintf(stderr, ">>resetting utmp for id \"%s\"\n",
        ...
        _DtTermProcessUnlock();
        DebugF('s', 10,
               fprintf(stderr, ">>_DtTermPrimUtmpCleanup() finished\n"));
}
