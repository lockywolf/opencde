...
static char rcs_id[] =
    "$XConsortium: TermPrimBufferWc.c /main/1 1996/04/21 19:16:59 drk $";
...
static void _termBufferValidateLineWc(const TermBuffer tb, const short row);

static void _patchUpChar(TermBuffer tb, short row, short *col,
                         TermCharInfo charInfo);

static void _countWidth(wchar_t *wcBuffer, short length, short maxWidth,
                        short *retCount, short *retWidth);

static void _primBufferInsertWc(
    const TermBuffer tb, const short row, short *col, wchar_t *newChars,
    short numChars, short *lengthInc, short *widthInc,
...
static void _primBufferOverwriteWc(
    const TermBuffer tb, const short row, short *col, wchar_t *newChars,
    short numChars, short *lengthInc, short *widthInc,
    short *widthInsert,    /* width of inserted characters           */
    termChar *returnChars, /* pointer to overflow buffer             */
...
**  A small workaround for systems that don't have wcwidth...
*/
int usl_wcwidth(const wchar_t wc) { return (1); }
#endif /*  USE_USL_WCWIDTH_PATCH */

short _DtTermPrimBufferGetTextWc(const TermBuffer tb, const short row,
...
static void _patchUpChar(TermBuffer tb, short row, short *col,
        ...
        */
        ...
        if ((charInfo->width == 2) && (charInfo->startCol == *col - 1)) {
                /*
...
*/
static void
_countWidth(wchar_t *wcBuffer, /* buffer of wide characters */
        ...
        *width = 0;
        for (pwc = wcBuffer; pwc < wcBuffer + wcLen; pwc++) {
                ...
                case -1:
                        /*
                        ** invalid character, (but this should have been handled
                        *earlier)
                        */
                ...
                case 0:
                        /*
                        ** its a null character, but is still has a width for
                        *our
                        ** purposes...
                        */
                        charWidth == 1;
    ...
    short *returnLength    /* count of characters in overflow buffer */
        ...
        */
        ...
        if (WIDTH(line) + *widthInsert <= COLS(tb)) {
                /*
                ** there is no overflow, we can insert all "lengthInsert"
                *characters...
                */
                        ...
                        *widthInc = *widthInsert - overflowWidth;
                        ...
                        for (pwc = ((wchar_t *)BUFFER(line)) + LENGTH(line) - 1;
                             pwc >= charInfo.u.pwc; pwc--) {
                                ...
                                if (overflowWidth <= 0) {
                                        /*
                                        ** we've removed enough characters, pwc
                                        *points to the
                        ...
                        /*
                        ** final adjustment to widthInc (at this point
                        *overflowWidth
                        ** is either 0 (we removed exactly "overflowWidth" worth
                        *of
                        ** characters) or -1 (we removed "overflowWidth + 1"
                        *worth of
                        ** characters because we removed some 2 column
                        *characters)
                        */
    ...
    short *returnLength    /* count of characters in overflow buffer */
        ...
        */
        if (localCol == WIDTH(line)) {
                ...
                *lengthInc = lengthInsert;
                pStart =
                    (char *)BUFFER(line) + (LENGTH(line) * sizeof(wchar_t));
        } else {
                /*
                ...
                */
                if (localCol + *widthInsert < WIDTH(line)) {
                        /*
                        ** the line width will remain constant, but the length
                        *may
                        ** change...
                        */
                        *widthInc = 0;
                        ...
                        /*
                        ** make sure we are not trying to overwrite the first
                        *column
                        ** of a two column character.
                        */
                        _DtTermPrimGetCharacterInfo(
                            tb, row, localCol + *widthInsert, &charInfo);
                        if ((charInfo.width == 2) &&
                            (charInfo.startCol ==
                             localCol + *widthInsert - 1)) {
                                /*
                                ** We are about to overwrite column 1 of a 2
                                *column
                                ** character.  Replace it with a space before we
                                *proceed
                                ** (we make adjustments later to make it look
                                *like we
                                ...
                                *charInfo.u.pwc = L' ';
                                ...
                                /*
                                ** now update the charInfo (since we replaced
                                *the 2 column
                        ...
                        */
                        *lengthInc = lengthInsert -
                                     (charInfo.u.pwc - startCharInfo.u.pwc);

                        if (*lengthInc != 0) {
                                memmove(charInfo.u.pwc + *lengthInc,
                                        charInfo.u.pwc,
                                        (LENGTH(line) - charInfo.idx) *
                                            sizeof(wchar_t));
                        }
                } else {
                        /*
                        ...
                        *lengthInc =
                            startCharInfo.idx + lengthInsert - LENGTH(line);
                }
...
*/
short _DtTermPrimBufferInsertWc(
    const TermBuffer tb, const short row, const short col, wchar_t *newChars,
    short numChars, Boolean insertFlag, /* if TRUE, insert, else overwrite */
    termChar **returnChars, /* pointer to overflow buffer             */
    short *returnLength     /* count of characters in overflow buffer */
) {
...
void _DtTermPrimBufferDeleteWc(
    ...
    short *returnCount      /* count of bytes in delete buffer */
        ...
        */
        ...
        (void)_DtTermPrimGetCharacterInfo(
            tb, localRow, MIN(WIDTH(line), localCol + *width), &stopCharInfo);
        ...
        */
        if (returnChars != NULL) {
                *returnCount =
                    (stopCharInfo.idx - startCharInfo.idx) * sizeof(wchar_t);
...
*/
void _DtTermPrimBufferPadLineWc(const TermBuffer tb, const short row,
                                const short width) {
        short i;
        short widthInc;
        TermLine line;
...
*/
Boolean _DtTermPrimBufferClearLineWc(const TermBuffer tb, const short row,
                                     short newWidth) {
        TermLine line;
        TermCharInfoRec charInfo;
        short newLength;
        ...
        */
        ...
        if (newWidth < WIDTH(line)) {
                if (newWidth == 0) {
                        ...
                        /*
                        ** handle the case of clearing the second column of a
                        *two column
                        ** character...
                        */
                        _DtTermPrimGetCharacterInfo(
                            tb, row, MAX(0, newWidth - 1), &charInfo);

                        if ((charInfo.width == 2) &&
                            (charInfo.startCol == MAX(0, newWidth - 1))) {
                                /*
                                ** we are clearing column 2 of 2, replace column
                                *1 of 1 with
                                ** a space...
                                */
                                *charInfo.u.pwc = L' ';
...
void _DtTermPrimBufferEraseWc(TermBuffer tb, short row, short startCol,
        ...
        */
        ...
        if ((stopCharInfo.width == 2) && (stopCharInfo.startCol == stopCol)) {
#ifdef NOTDEF
                /*
        ...
        */
        ...
        lengthInc =
            lengthErase - (stopCharInfo.u.pwc - startCharInfo.u.pwc + 1);

        if (lengthInc != 0) {
                /*
                ** the length will have to change, make the necessary
                *adjustments
                */
        ...
        */
        for (pwchar = startCharInfo.u.pwc;
             pwchar < startCharInfo.u.pwc + lengthErase; pwchar++) {
                *pwchar = L' ';
        }
}
...
static void _termBufferValidateLineWc(const TermBuffer tb, const short row) {
        ...
        for (pwc = (wchar_t *)BUFFER(line);
             pwc < (wchar_t *)BUFFER(line) + LENGTH(line); pwc++) {
                if (wcwidth(*pwc) == -1) {
                        fprintf(
                            stderr,
                            "_termBufferValidateLineWc: invalid wide char\n");
                        /* replace the character with a space... */
                        *pwc = L' ';
                        break;
                }
