...
static char rcs_id[] =
    "$XConsortium: TermPrimScroll.c /main/1 1996/04/21 19:19:17 drk $";
...
static void waitOnCopyArea(Widget w) {
        ...
        while (tpd->scrollInProgress) {
                ...
                case Expose:
                        ...
                        (void)_DtTermPrimExposeText(
                            w, ev->xexpose.x, ev->xexpose.y, ev->xexpose.width,
                            ev->xexpose.height, True);
                        break;

                case GraphicsExpose:
                        /* refresh the exposed area and, if this is the last
                         * graphics exposure, we are done and can fall through
                         * to the noExpose code...
                         */
                        Debug('e', fprintf(stderr,
                                           ">>_DtTermPrimScrollWait() "
                                           "GraphicsExpose event received, "
                                           "count=%d\n",
                                           ev->xgraphicsexpose.count));
                        Debug('e',
                              fprintf(stderr, ">>_waitOnCopyArea() exposing "
                                              "GraphicsExpose area\n"));
                        Debug('e', fprintf(stderr,
                                           ">>                  x=%d  y=%d  "
                                           "width=%d  height=%d\n",
                        ...
                        (void)_DtTermPrimExposeText(
                            w, ev->xgraphicsexpose.x, ev->xgraphicsexpose.y,
                            ev->xgraphicsexpose.width,
                            ev->xgraphicsexpose.height, False);
                ...
                case NoExpose:
                        /* we are done... */
                        Debug('e',
                              fprintf(stderr, ">>_waitOnCopyArea() NoExpose "
                                              "event received\n"));
...
void _DtTermPrimScrollWait(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        int i;
        int exposeY;
        ...
        if (tpd->scroll.jump.scrollLines != 0) {
                ...
                if (tpd->scroll.jump.scrollLines > 0) {
                        ...
                        Debug('s', fprintf(stderr,
                                           ">>_DtTermPrimScrollWait() up,  "
                                           "scrollLines=%d  cellHeight=%d\n",
                                           tpd->scroll.jump.scrollLines,
                                           tpd->cellHeight));
                        if (tpd->scroll.jump.scrollLines >=
                            tpd->scrollBottomRow - tpd->scrollTopRow + 1) {
                                /* a full screen or more -- we can avoid the
                                 * copy area... */
                                tpd->scrollHeight = 0;
                                exposeY = tpd->offsetY +
                                          tpd->cellHeight * (tpd->scrollTopRow);
                                exposeHeight =
                                    tpd->cellHeight * (tpd->scrollBottomRow -
                                                       tpd->scrollTopRow + 1);
                                Debug('s', fprintf(stderr,
                                                   ">>_DtTermPrimScrollWait()  "
                                                   "height=%d    expose Y=%d  "
                                                   "height=%d\n",
                                                   tpd->scrollHeight, exposeY,
                                                   exposeHeight));
                        } else {
                                tpd->scrollHeight =
                                    tpd->cellHeight *
                                    (tpd->scrollBottomRow - tpd->scrollTopRow +
                                     1 - tpd->scroll.jump.scrollLines);
                                tpd->scrollSrcY =
                                    tpd->offsetY +
                                    tpd->cellHeight *
                                        (tpd->scrollTopRow +
                                ...
                                Debug('s',
                                      fprintf(stderr,
                                              ">>_DtTermPrimScrollWait()  "
                                              "height=%d  SrcY=%d  DestY=%d    "
                                              "expose Y=%d  height=%d\n",
                                              tpd->scrollHeight,
                                              tpd->scrollSrcY, tpd->scrollDestY,
                                              exposeY, exposeHeight));
                        }
                } else {
                        /* scrolling memory down... */
                        Debug('s', fprintf(stderr,
                                           ">>_DtTermPrimScrollWait() down,  "
                                           "scrollLines=%d  cellHeight=%d\n",
                                           tpd->scroll.jump.scrollLines,
                                           tpd->cellHeight));
                        if (tpd->scroll.jump.scrollLines <=
                ...
                Debug(
                    ...
                    fprintf(stderr,
                            ...
                            tpd->offsetX, exposeY,
                            tpd->cellWidth * tw->term.columns, exposeHeight));
                 ...
                 */
                for (i = 0; i < (exposeY - tpd->offsetX) / tpd->cellHeight;
                     i++) {
                        if (tpd->scrollRefreshRows[i]) {
                                (void)_DtTermPrimRefreshText(
                                    w, 0, i, tw->term.columns, i);
                        }
                }
                /* expose the refresh expose area... */
                (void)_DtTermPrimExposeText(w, tpd->offsetX, exposeY,
                 ...
                 * scrollRefreshRows flag set...
                 */
                for (i = (exposeY - tpd->offsetX + exposeHeight) /
                         tpd->cellHeight;
                     i < tw->term.rows; i++) {
                        if (tpd->scrollRefreshRows[i]) {
                                (void)_DtTermPrimRefreshText(
...
static void doActualScroll(Widget w, int lines) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        int exposeY;
        int exposeHeight;
        ...
        if (lines > 0) {
                ...
                if (lines >= tpd->scrollBottomRow - tpd->scrollTopRow + 1) {
                        /* we are scrolling a full screen or more, so there is
                         * nothing to copy...
                         */
                        tpd->scrollHeight = 0;
                        exposeY =
                        ...
                        tpd->scrollHeight =
                            tpd->cellHeight * (tpd->scrollBottomRow -
                                               tpd->scrollTopRow + 1 - lines);
                        tpd->scrollSrcY =
                            tpd->offsetY +
                            tpd->cellHeight * (tpd->scrollTopRow + lines);
                        tpd->scrollDestY =
                            tpd->offsetY +
                            tpd->cellHeight * (tpd->scrollTopRow);
...
void _DtTermPrimScrollText(Widget w, short lines) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        short oldTopRow;
        short newTopRow;
        ...
        if (tpd->scrollLockTopRow > 0) {
                (void)_DtTermPrimBufferMoveLockArea(
                    tpd->termBuffer, tpd->topRow + lines, tpd->topRow,
        ...
        if (tpd->scrollLockBottomRow < (tw->term.rows - 1)) {
                ...
                if ((lines > 0) &&
                    (tpd->bufferRows > (tpd->topRow + tw->term.rows))) {
                        ...
                        if (lines2 >
                            (tpd->bufferRows - (tpd->topRow + tw->term.rows))) {
                                lines2 = tpd->bufferRows -
                                         (tpd->topRow + tw->term.rows);
                        }
        ...
        if (tw->term.jumpScroll) {
                ...
                /* NOTE: we loose the refresh flag for all rows that are
                 * scrolled off.  The result of this is that if we do a scroll
                 * up followed by a scroll down, we will (at a minimum) refresh
                 * the top and bottom lines.  One workaround would be to tripple
                 * the buffer and keep the lines that get scrolled off the top
                 * or bottom. This would probably break something, since there
                 * are times that the scrolled off line gets modified or even
                 * cleared (such as delete line off of the top of the display),
                 * so this might not be a very good idea.
                 */
                if (lines > 0) {
                        /* scroll them up... */
                ...
                if (tpd->scrollInProgress) {
                        ...
                        if (tpd->scroll.nojump.pendingScrollLines > 1)
                                (void)fprintf(
                                    stderr,
                                    "tell Dave _DtTermPrimScrollText has "
                                    "pendingScrollLines=%d\n",
                                    tpd->scroll.nojump.pendingScrollLines);
                        /*DKS!!! ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ ^ */
                        tpd->scroll.nojump.pendingScroll = True;
                        tpd->scroll.nojump.pendingScrollTopRow =
                            tpd->scrollLockTopRow;
        ...
        if (lines > 0 && tpd->scrollLockBottomRow < (tw->term.rows - 1)) {
                _DtTermPrimSelectMoveLines(
                    w, tpd->scrollLockTopRow + lines, tpd->scrollLockTopRow,
...
void _DtTermPrimScrollTextTo(Widget w, short topRow) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        int oldTopRow;
...
void _DtTermPrimScrollTextArea(Widget w, short scrollStart, short scrollLength,
...
void _DtTermPrimScrollComplete(Widget w, Boolean flush) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        ...
        if (tw->term.jumpScroll) {
                ...
                if (flush) {
                         ...
                         */
                        /* In jump scroll, all we need to do is to invoke the
                         * scroll. _DtTermPrimScrollWait() will perform the
                         * actual copy area and wait for the generated graphics
                         * expose / no expose events...
                         */
                        if (tpd->scroll.jump.scrolled) {
                                Debug('s', fprintf(stderr, ">>_"
                                                           "DtTermPrimScrollCom"
                                                           "plete() calling "
                                                           "_DtTermPrimScrollWa"
                                                           "it()\n"));
                ...
                if (flush && tpd->scrollInProgress) {
                        /* if we have a pending scroll, we need to wait for it
                         * to complete.  Normally, we would service this out of
                         * the XtMainLoop() dispatch loop, but for some reason
                ...
                if (!tw->term.jumpScroll && tpd->scroll.nojump.pendingScroll) {
                        ...
                        tpd->scrollBottomRow =
                            tpd->scroll.nojump.pendingScrollBottomRow;
                        (void)doActualScroll(
                            w, tpd->scroll.nojump.pendingScrollLines);

                        /* no lines pending, but there is a scroll in
                         * progress... */
                        ...
                        if (flush) {
                                ...
                                /* If there is any pending input or if we need
                                 * to reinstall the input select, force a pty
                                 * read...
                                 */
                                (void)XtAppAddTimeOut(
                                    XtWidgetToApplicationContext(w), 0,
                                    _DtTermPrimForcePtyRead, (XtPointer)w);
                        }
                }
        }
}

void _DtTermPrimScrollCompleteIfNecessary(Widget w, short scrollTopRow,
        ...
        if (tw->term.jumpScroll) {
                maxJumpScrollLines =
                    tpd->scrollBottomRow - tpd->scrollTopRow + 1;
                if ((lines + tpd->scroll.jump.scrollLines >
                     maxJumpScrollLines) ||
                    (lines + tpd->scroll.jump.scrollLines <
                     -maxJumpScrollLines))
                        (void)_DtTermPrimScrollComplete(w, True);
