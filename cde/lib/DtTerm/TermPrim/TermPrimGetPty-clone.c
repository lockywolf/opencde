...
static char rcs_id[] =
    "$TOG: TermPrimGetPty-clone.c /main/7 1998/04/03 17:11:08 mgreess $";
        ...
        if ((ptyFd = open(*ptyMaster, O_RDWR, 0))) {
                ...
                if (c = _XTtyname(ptyFd, tty_buf)) {
                        ...
                        *ptySlave = malloc(strlen(c) + 1);
                        ...
                        if ((ttyFd = open(*ptySlave, O_RDWR | O_NOCTTY, 0)) <
                            0) {
                                /* failure... */
                                ...
                                /* pty master and slave names are already set.
                        ...
                        /* ttyname on the pty master failed.  This should not
                         * happen!... */
                        (void)perror("ttyname");
                        (void)close(ptyFd);
                }
...
void _DtTermPrimPtyCleanup() { return; }
