...
static char rcs_id[] =
    "$TOG: TermPrimSelect.c /main/6 1999/10/14 16:22:53 mgreess $";
...
static XmTextScanType defaultScanArray[] = {XmSELECT_POSITION, XmSELECT_WORD,
                                            XmSELECT_LINE, XmSELECT_ALL};
...
*/
static Time getServerTime(Widget w) {
        XEvent event;
        EventMask shellMask;
        ...
        XChangeProperty(XtDisplay(w), XtWindow(w), XA_WM_HINTS, XA_WM_HINTS, 32,
...
static void setScanType(Widget w, XEvent *event) {
...
*/
XmTextPosition rowColToPos(DtTermPrimitiveWidget tw, short row, short col) {
        DtTermPrimData tpd = tw->term.tpd;

        return (
            ((tpd->selectInfo->columns + 1) * (row + tpd->lastUsedHistoryRow)) +
            col);
}
...
** getSelection
*/
Boolean _DtTermPrimSelectGetSelection(Widget w, XmTextPosition *begin,
                                      XmTextPosition *end) {
        TermSelectInfo selectInfo =
            ((DtTermPrimitiveWidget)w)->term.tpd->selectInfo;

        if (selectInfo->ownPrimary && (selectInfo->begin <= selectInfo->end) &&
            selectInfo->begin >= 0) {
...
*/
static XmTextPosition xyToPos(DtTermPrimitiveWidget tw, int x, /* pixel */
                              int y                            /* pixel */
) {
        DtTermPrimData tpd = tw->term.tpd;
        ...
        */
        yPos =
            MIN(yPos, MIN(tw->term.rows, tpd->lastUsedRow - tpd->topRow) - 1) +
            tpd->topRow;
        ...
        */
        if (tpd->useHistoryBuffer) {
                if (yPos < 0) {
                        /*
                        ** yPos is not in the history buffer (order is
                        *important,
        ...
        if (MB_CUR_MAX > 1) /* check if xPos splits a 2 col char */
        {
                ...
                if (_DtTermPrimGetCharacterInfo(tb, row, xPos, &charInfoRec)) {
                        if (charInfoRec.width == 2 &&
                            charInfoRec.startCol != xPos) {
 ...
 */
static void posToBufferRowCol(DtTermPrimitiveWidget tw, XmTextPosition pos,
                              TermBuffer *pb, short *row, short *col) {
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
        short lrow, lcol;
 ...
 */
static XmTextPosition bufferRowColToPos(DtTermPrimitiveWidget tw, TermBuffer pb,
                                        short row, short col) {
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
        short lrow, lcol;
        ...
        if (tpd->useHistoryBuffer && pb == tpd->termBuffer)
                pos +=
                    (tpd->selectInfo->columns + 1) * (tpd->lastUsedHistoryRow);
        return (pos);
}

static XmTextPosition scan(DtTermPrimitiveWidget tw, XmTextPosition scanStart,
                           XmTextScanType scanType, TermScanDirection scanDir,
                           int count, Boolean inclusive) {
        int i;
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
                ...
                if (MB_CUR_MAX > 1) {
                        ...
                        case scanLeft:
                                _DtTermPrimGetCharacterInfo(
                                    pb, row, col ? --col : 0, &charInfoRec);
                                while (
                                    !iswspace(*(wchar_t *)charInfoRec.u.pwc) &&
                                    ((col = charInfoRec.startCol - 1) >= 0)) {
                                        _DtTermPrimGetCharacterInfo(
                                            pb, row, col, &charInfoRec);
                                }
                                col++;
                                break;
                        case scanRight:
                                _DtTermPrimGetCharacterInfo(pb, row, col,
                                                            &charInfoRec);
                                while (++col <= width &&
                                       !iswspace(*(wchar_t *)charInfoRec.u.pwc))
                                        _DtTermPrimGetCharacterInfo(
                        ...
                        case scanLeft:
                                _DtTermPrimBufferGetText(
                                    pb, row, col ? --col : 0, 1, pbuf, False);
                                while (!isspace(*pbuf) && --col >= 0)
                                        _DtTermPrimBufferGetText(
                                            pb, row, col, 1, pbuf, False);
                                col++;
                                break;
                        case scanRight:
                                _DtTermPrimBufferGetText(pb, row, col, 1, pbuf,
                                                         False);
...
*/
void _DtTermPrimRenderRefreshTextLinear(Widget w, XmTextPosition start,
                                        XmTextPosition stop) {
        ...
        */
        if (startRow == stopRow) {
                _DtTermPrimRefreshText((Widget)tw, startCol, startRow, stopCol,
                                       startRow);
                return;
        }
...
static void setSelection(DtTermPrimitiveWidget tw, XmTextPosition begin,
                         XmTextPosition end, Time selectTime,
        ...
        if (selectInfo->ownPrimary == False && begin > end) {
        ...
        if (selectInfo->ownPrimary) {
                ...
                */
                        ...
                        */
                        if ((end <= selectInfo->begin) ||
                            (begin >= selectInfo->end)) {
                                /*
                                ** The two areas don't intersect, simply clear
                                *the old
                                ** area...
                                */
                                Debug('c',
                                      fprintf(stderr,
                                              "    new & old are disjoint\n"));
                                selectInfo->ownPrimary = False;
                                _DtTermPrimRenderRefreshTextLinear(
                                    (Widget)tw, selectInfo->begin,
                                    selectInfo->end - 1);
                                selectInfo->ownPrimary = True;
                                disJoint = True;
                        } else {
                                /*
                                ** There is some intersection, save the current
                                *begin
                ...
                /*
                ** we don't own the selection (yet), come up with some
                *reasonable
                ** defaults
                */
                disJoint = True;
        ...
        if (begin <= end) {
                if (selectInfo->ownPrimary == False) {
                        if (!XtOwnSelection((Widget)tw, XA_PRIMARY, selectTime,
                                ...
                                /*
                                ** XtOwnSelection failed, make a dummy call to
                                *setSelection
                ...
                */
                if (selectInfo->ownPrimary) {
                        if (disJoint == True) {
                                /*
                                ** the selections are disjoint, simply draw the
                                *new one
                                */
                                _DtTermPrimRenderRefreshTextLinear(
                                    (Widget)tw, begin, end - 1);
                        } else {
                                if (begin != oldBegin) {
                                        if (begin < oldBegin) {
                                                /*
                                                ** refresh from the new
                                                *beginning to the old
                                                ** beginning
                                                */
                                                _DtTermPrimRenderRefreshTextLinear(
                                                    (Widget)tw, begin,
                                                ...
                                                /*
                                                ** refresh from the old
                                                *beginning to the new
                                                ** beginning
                                                **
                                                ** NOTE: in this case we want to
                                                *unhighlight
                                                **       previously selected
                                                *text, so we
                                                **       temporarily set
        ...
        DebugF('c', 1,
               fprintf(stderr, "set selection units: %d-%d  lines: %d-%d\n",
                       ...
                       selectLineBegin, selectLineEnd));
        if (tw->term.tpd->useHistoryBuffer &&
            tw->term.tpd->lastUsedHistoryRow > 0) {
                if (selectLineEnd > tw->term.tpd->lastUsedHistoryRow) {
                        (void)_DtTermPrimBufferSetSelectLines(
                            tw->term.tpd->historyBuffer, selectLineBegin,
                            selectColBegin,
                            tw->term.tpd->lastUsedHistoryRow - 1,
                            selectInfo->columns);
                } else {
                        (void)_DtTermPrimBufferSetSelectLines(
                            tw->term.tpd->historyBuffer, selectLineBegin,
                            selectColBegin, selectLineEnd, selectColEnd);
                }
        ...
        if (selectLineEnd > tw->term.tpd->lastUsedRow) {
                (void)_DtTermPrimBufferSetSelectLines(
                    tw->term.tpd->termBuffer, selectLineBegin, selectColBegin,
...
static void handleSelection(DtTermPrimitiveWidget tw, int x, int y,
                            Time selectTime) {
        ...
        newBegin = scan(tw, position, selectInfo->scanType, scanLeft, 1, False);
        newEnd = scan(tw, position, selectInfo->scanType, scanRight, 1,
                      selectInfo->scanType == XmSELECT_LINE);

        setSelection(tw, newBegin, newEnd, selectTime, False);

        if ((position - newBegin) < (newEnd - position)) {
...
/* ARGSUSED */
static Boolean CheckTimerScrolling(Widget w, XEvent *event) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
        XmScrollBarWidget vsb = (XmScrollBarWidget)tw->term.verticalScrollBar;
        ...
        if ((event->xmotion.y > (int)tpd->offsetY) &&
            (event->xmotion.y <
             (int)(tpd->offsetY + tw->term.rows * tpd->cellHeight))) {
                ...
                if (!selectInfo->selectID)
                        selectInfo->selectID = XtAppAddTimeOut(
                            XtWidgetToApplicationContext(w), interval,
                            browseScroll, (XtPointer)w);
                return True;
        }
        return False;
...
*/
TermSelectInfo _DtTermPrimSelectCreate(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo;
        int i;
        ...
        selectInfo->scanArray = (XmTextScanType *)XtMalloc(
            selectInfo->scanArraySize * sizeof(XmTextScanType));
...
void _DtTermPrimSelectInitBtnEvents(Widget w) {
        Boolean btn1_transfer = False;
        XtVaGetValues((Widget)XmGetXmDisplay(XtDisplay(w)),
                      "enableBtn1Transfer", &btn1_transfer, NULL);
        if (btn1_transfer)
                XtOverrideTranslations(
                    w, XtParseTranslationTable(_DtTermEventBindingsCDE));
        if (btn1_transfer == True) /* for btn2 extend case */
                XtOverrideTranslations(
                    w, XtParseTranslationTable(_DtTermEventBindingsCDEBtn2));
}

void _DtTermPrimSelectDisown(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
...
void _DtTermPrimSelectDestroy(Widget w, TermSelectInfo selectInfo) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
...
**     buffer
*/
Boolean _DtTermPrimSelectIsInSelection(Widget w, int row, short startCol,
        ...
        if ((begin >= endPosition) || (end <= position)) {
                ...
                */
                ...
                /*
                ** we're in the selection range, clip endPosition as
                *necessary...
                */
                if (position < begin) {
                        /*
...
/*ARGSUSED*/
void _DtTermPrimSelectDoSelection(Widget w, XEvent *event, String *params,
                                  Cardinal *paramCount) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;

        Debug('c',
              fprintf(stderr, ">>_DtTermPrimSelectDoSelection() starting\n"));
...
/*ARGSUSED*/
void _DtTermPrimSelectSetHint(Widget w, XEvent *event, String *params,
                              Cardinal *paramCount) {
        TermSelectInfo selectInfo =
            ((DtTermPrimitiveWidget)w)->term.tpd->selectInfo;
...
/*ARGSUSED*/
void _DtTermPrimSelectStart(Widget w, XEvent *event, String *params,
                            Cardinal *paramCount) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermSelectInfo selectInfo = tw->term.tpd->selectInfo;
...
/*ARGSUSED*/
void _DtTermPrimSelectGrabFocus(Widget w, XEvent *event, String *params,
                                Cardinal *paramCount) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
        ...
        /* setDebugFlags("c") ; */
        Debug('c',
              fprintf(stderr, ">>_DtTermPrimSelectGrabFocus() starting\n"));
        ...
        */
        ...
        if (btnEvent->y <= (int)tpd->offsetY) {
                ...
                btnEvent->y = (int)(tpd->offsetY + 1);
                       ...
                       ((int)(tpd->offsetY + ((tpd->lastUsedRow - tpd->topRow) *
                                              tpd->cellHeight))) >=
                   selectInfo->threshold) {
                /* below */
                btnEvent->y =
                    (int)(tpd->offsetY +
                          ((tpd->lastUsedRow - tpd->topRow) * tpd->cellHeight) -
                          1);
        }
...
static Boolean dragged(TermSelectionHint hint, XEvent *event, int threshold) {
...
static void doExtendedSelection(Widget w, Time eventTime) {
        ...
        if (!(_DtTermPrimSelectGetSelection(w, &begin, &end)) ||
            (begin == end)) {
                ...
                if (selectInfo->anchor < 0)
                        selectInfo->anchor = position;
                selectInfo->origBegin = selectInfo->anchor;
                selectInfo->origEnd = selectInfo->anchor;
                midPoint = (float)selectInfo->anchor;
        } else {
                midPoint = (float)(((float)(selectInfo->origEnd -
                                            selectInfo->origBegin) /
                                    2.0) +
        ...
        */
        if ((selectInfo->extendDir == scanRight &&
            ...
            (selectInfo->extendDir == scanLeft &&
             position > selectInfo->anchor)) {
                selectInfo->extendDir =
                    (selectInfo->extendDir == scanRight) ? scanLeft : scanRight;
        ...
        if (selectInfo->extendDir == scanRight) {
                cursorPos = scan(tw, position, selectInfo->scanType, scanRight,
                                 1, selectInfo->scanType == XmSELECT_LINE);
                end = cursorPos;
                begin = selectInfo->anchor;
        } else {
                cursorPos = scan(tw, position, selectInfo->scanType, scanLeft,
                                 1, FALSE);
                begin = cursorPos;
                end = selectInfo->anchor;
                if (selectInfo->scanType == XmSELECT_WORD &&
                    (int)tw->term.tpd->cellWidth > 1) {
                        if (position == scan(tw, begin, selectInfo->scanType,
                                             scanRight, 1, FALSE)) {
...
void _DtTermPrimSelectExtendStart(Widget w, XEvent *event, String *params,
                                  Cardinal *num_params) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermSelectInfo selectInfo = tw->term.tpd->selectInfo;
...
/*ARGSUSED*/
void _DtTermPrimSelectExtend(Widget w, XEvent *event, String *params,
                             Cardinal *paramCount) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermSelectInfo selectInfo = tw->term.tpd->selectInfo;
        ...
        if ((selectInfo->hint.x > 0) || (selectInfo->hint.y > 0)) {
                if (dragged(selectInfo->hint, event, selectInfo->threshold)) {
                        ...
                        */
                        handleSelection(tw, selectInfo->hint.x,
                                        selectInfo->hint.y,
                                        event->xbutton.time);
                        selectInfo->hint.x = 0;
                        selectInfo->hint.y = 0;
...
/*ARGSUSED*/
void _DtTermPrimSelectExtendEnd(Widget w, XEvent *event, String *params,
                                Cardinal *paramCount) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermSelectInfo selectInfo = tw->term.tpd->selectInfo;

        Debug('c',
              fprintf(stderr, ">>_DtTermPrimSelectExtendEnd() starting\n"));
...
/*ARGSUSED*/
static void doHandleTargets(Widget w, XtPointer closure, Atom *seltype,
            ...
            *type == XA_STRING) {
                ...
                */
                if (numVals && (status == Success || status > 0)) {
                        ...
                        total_tmp_value[0] = '\0';
                        for (i = 0; i < numVals; i++) {
                                strlcat(total_tmp_value, tmp_value[i],
                                        malloc_size + 1);
                        }
                ...
                block.length =
                    (int)*length; /* NOTE: this causes a truncation on
                       some architectures */
                block.format = XmFMT_8_BIT;
        }
...
*/
/*ARGSUSED*/
static void handleTargets(Widget w, XtPointer closure, Atom *selType,
                          Atom *type, XtPointer value, unsigned long *length,
                          int *format) {
        Atom CS_OF_LOCALE;
        Atom COMPOUND_TEXT;
        ...
        */
        ...
        tmpProp.value = NULL;
        status =
            XmbTextListToTextProperty(XtDisplay(w), &abcString, 1,
        ...
        primSelect = (_TermSelectPrimaryRec *)XtMalloc(
            (unsigned)sizeof(_TermSelectPrimaryRec));
...
static char *getString(Widget w, XmTextPosition begin, XmTextPosition end,
...
*/
static void getTargets(Widget w, XEvent *event, String *params,
                       Cardinal *paramCount) {
        _TermSelectRec *tmp;
...
Boolean _DtTermPrimSelectConvert(Widget w, Atom *selection, Atom *target,
        ...
        tmpProp.value = NULL;
        status =
            XmbTextListToTextProperty(XtDisplay(widget), &tmpString, 1,
                                      (XICCEncodingStyle)XTextStyle, &tmpProp);
        if (status == Success) {
                CS_OF_LOCALE = tmpProp.encoding;
        } else {
        ...
        if (*selection == XA_PRIMARY || *selection == MOTIF_DROP) {
                ownPrimary =
                    _DtTermPrimSelectGetSelection(widget, &begin, &end);
        } else {
                return (False);
        }
        ...
        if (*target == TARGETS) {
                Atom *targets =
                    (Atom *)XtMalloc((unsigned)(maxTargets * sizeof(Atom)));
                ...
                *type = XA_ATOM;
                *length = (targetCount * sizeof(Atom)) >>
                          2; /* convert to work count */
                *format = 32;
        } else if (!ownPrimary) {
                return (False);
                ...
                if (status == Success || status > 0) {
                        /*
                        ** NOTE: casting tmpProp.nitems could result in a
                        *truncated long.
                        */
                        if (tmpProp.nitems > 0)
...
void _DtTermPrimSelectLoseSelection(Widget w, Atom *selection) {
        ...
        Debug('c',
              fprintf(stderr, ">>_DtTermPrimSelectLoseSelection() starting\n"));
        ...
        if (*selection == XA_PRIMARY && selectInfo->ownPrimary) {
                ...
                */
                ...
                setSelection(tw, 1, -99, XtLastTimestampProcessed(XtDisplay(w)),
                             True);
        ...
        Debug('c',
              fprintf(stderr, ">>_DtTermPrimSelectLoseSelection() exiting\n"));
}
/*ARGSUSED*/
void _DtTermPrimSelectBDragRelease(Widget w, XEvent *event, String *params,
                                   Cardinal *paramCount) {
        TermSelectInfo selectInfo =
            ((DtTermPrimitiveWidget)w)->term.tpd->selectInfo;
        XButtonEvent *btnEvent = (XButtonEvent *)event;
        ...
        Debug('c',
              fprintf(stderr, ">>_DtTermPrimSelectBDragRelease() starting\n"));
        ...
        */
        ...
        if (selectInfo->sel_start)
                getTargets(w, event, params, paramCount);
}

/*ARGSUSED*/
void _DtTermPrimSelectInsert(Widget w, XEvent *event, String *params,
                             Cardinal *paramCount) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermSelectInfo selectInfo = tw->term.tpd->selectInfo;
...
Boolean _DtTermPrimSelectIsAboveSelection(Widget w, short row, short col) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
...
void _DtTermPrimSelectResize(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
...
extern void _DtTermPrimSelectInsertLines(Widget w, short src, short len) {
        ...
        /* if the beginning of the selection is at or after the source, we need
...
void _DtTermPrimSelectAll(Widget w, XEvent *event, String *params,
                          Cardinal *paramCount) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
        ...
        begin = scan(tw, (XmTextPosition)0, XmSELECT_ALL, scanLeft, 1, False);
        end = scan(tw, (XmTextPosition)0, XmSELECT_ALL, scanRight, 1,
                   selectInfo->scanType == XmSELECT_LINE);
...
void _DtTermPrimSelectPage(Widget w, XEvent *event, String *params,
                           Cardinal *paramCount) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        TermSelectInfo selectInfo = tpd->selectInfo;
...
static void DropTransferCallback(Widget w, XtPointer closure, Atom *seltype,
                                 Atom *type, XtPointer value,
                                 unsigned long *length, int *format) {
        _DtTermDropTransferRec *transfer_rec =
            (_DtTermDropTransferRec *)closure;
...
static void SetDropContext(Widget w) {
        ...
        XSaveContext(display, (Window)screen, _DtTermDNDContext, (XPointer)w);
}
...
static void DropDestroyCB(Widget w, XtPointer clientData, XtPointer callData) {
        ...
        if (ptr) {
                if (ptr->transfer_rec)
                        XtFree((char *)ptr->transfer_rec);
                if (ptr->dropDestroyCB)
                        XtFree((char *)ptr->dropDestroyCB);
                XtFree((char *)ptr);
        }
}

static void HandleDrop(Widget w, XmDropProcCallbackStruct *cb) {
        ...
        clientData = (dropDestroyCBClientData *)XtMalloc(
            sizeof(dropDestroyCBClientData));
        dropDestroyCB = dd_cb =
            (XtCallbackRec *)XtMalloc(2 * sizeof(XtCallbackRec));
        ...
        n = 0;
        XtSetArg(args[n], XmNsourceWidget, &initiator);
        n++;
...

        {
                ...
                Atom CS_OF_LOCALE;
                char *tmp_string =
                    "ABC"; /* these are characters in XPCS, so... safe */
                ...
                status = XmbTextListToTextProperty(
                    XtDisplay(w), &tmp_string, 1, (XICCEncodingStyle)XTextStyle,
                    &tmp_prop);
                if (status == Success)
                        CS_OF_LOCALE = tmp_prop.encoding;
                else
                        CS_OF_LOCALE =
                            99999; /* XmbTextList... should never fail for XPCS
                ...
                transfer_rec = (_DtTermDropTransferRec *)XtMalloc(
                    sizeof(_DtTermDropTransferRec));
                clientData->transfer_rec = transfer_rec;
                transfer_rec->widget = w;
                /*  don't actually need all of this for dtterm - it was from
                 *Text widget transfer_rec->insert_pos = insert_pos;
                ...
                for (n = 0; n < numExportTargets; n++) {
                        ...
                        if (exportTargets[n] == COMPOUND_TEXT)
                                c_text_found = True;
                        if (exportTargets[n] == XA_STRING)
                                string_found = True;
                        if (exportTargets[n] == TEXT)
                                text_found = True;
                }

                n = 0;
                if (locale_found || c_text_found || string_found ||
                    text_found) {
                        if (!locale_found) {
                                if (c_text_found)
                                        transferEntries[0].target =
                ...
                n++;
                XtSetArg(args[n], XmNtransferProc, DropTransferCallback);
                n++;
        }
        SetDropContext(w);
...
static void RegisterDropSite(Widget w) {
        ...
        tmp_prop.value = NULL;
        status =
            XmbTextListToTextProperty(XtDisplay(w), &tmp_string, 1,
        ...
        n++;
        XtSetArg(args[n], XmNnumImportTargets, 4);
        n++;
...
static void StartDrag(Widget w, XEvent *event, String *params,
        ...
        if (status == Success)
                ...
                targets[num_targets++] =
                    99999; /* XmbTextList...  should never fail
        ...
        n++;
        XtSetArg(args[n], XmNcursorForeground, tw->primitive.foreground);
        n++;
        XtSetArg(args[n], XmNsourceCursorIcon, drag_icon);
        n++;
        XtSetArg(args[n], XmNexportTargets, targets);
        n++;
        XtSetArg(args[n], XmNnumExportTargets, num_targets);
        n++;
        XtSetArg(args[n], XmNconvertProc, _DtTermPrimSelectConvert);
        n++;
        XtSetArg(args[n], XmNclientData, w);
        n++;
...
static Position GetXFromPos(Widget w, XmTextPosition pos) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        TermBuffer pb;
        short row, col;
...
/* ARGSUSED */
void _DtTermPrimSelectProcessBDrag(Widget w, XEvent *event, char **params,
                                   Cardinal *num_params) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermSelectInfo selectInfo =
            ((DtTermPrimitiveWidget)w)->term.tpd->selectInfo;
...
Boolean _DtTermPrimSelectCopyClipboard(Widget w, Time copy_time) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        XmTextPosition begin;
        XmTextPosition end;
        ...
        if (selected_string != NULL) {
                ...
                status = XmClipboardStartCopy(display, window, clip_label,
                                              copy_time, w, NULL, &item_id);
                ...
                status = XmbTextListToTextProperty(
                    display, &selected_string, 1,
                    (XICCEncodingStyle)XStdICCTextStyle, &tmp_prop);
                ...
                status = XmClipboardCopy(display, window, item_id, atom_name,
                                         (XtPointer)tmp_prop.value,
                                         tmp_prop.nitems, 0, &data_id);
...
void _DtTermPrimSelectCopyClipboardEventIF(Widget w, XEvent *event,
                                           String *params,
                                           Cardinal *num_params) {
 ...
 */
Boolean _DtTermPrimSelectPasteClipboard(Widget w) {
        ...
        if (!get_ct)
                ...
                tmp_prop.encoding =
                    XmInternAtom(display, "COMPOUND_TEXT", False);
...
void _DtTermPrimSelectPasteClipboardEventIF(Widget w, XEvent *event,
                                            String *params,
                                            Cardinal *num_params) {
 ...
 */
static Bool LookForButton(Display *display, XEvent *event, XPointer arg) {
...
/* ARGSUSED */
static Boolean XmTestInSelection(Widget w, XEvent *event) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermSelectInfo selectInfo = tw->term.tpd->selectInfo;
        XmTextPosition position, left, right;
        ...
        if ((!(_DtTermPrimSelectGetSelection(w, &left, &right) &&
             ...
             (position == left && event->xbutton.x > GetXFromPos(w, left)) ||
             (position == right && event->xbutton.x < GetXFromPos(w, right))) ||
            ...
            (event->xbutton.time > selectInfo->lastTime &&
             event->xbutton.time - selectInfo->lastTime <
                ...
                /* The determination of whether this is a transfer drag cannot
                   be made until a Motion event comes in.  It is not a drag as
                   soon as a ButtonUp event happens or the MultiClickTimeout
                   expires. */
                XEvent new;

                XPeekIfEvent(XtDisplay(w), &new, LookForButton,
                             (XPointer)event);
                switch (new.type) {
                case MotionNotify:
                        return (True);
...
/* ARGSUSED */
void _DtTermPrimSelect2ButtonMouse(Widget w, XEvent *event, char **params,
                                   Cardinal *num_params) {
...
void _DtTermPrimSelectProcessCancel(Widget w, XEvent *event, String *params,
                                    Cardinal *num_params) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        TermSelectInfo selectInfo = tw->term.tpd->selectInfo;
