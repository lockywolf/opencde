...
static char rcs_id[] =
    "$XConsortium: TermPrimLineDraw.c /main/1 1996/04/21 19:17:48 drk $";
...
static void ScaleCharacter(
#ifdef USE_PIXMAPS
    GC gc, Pixmap bitmap, ScaledBitmapInfo renderInfo, GlyphInfo glyph,
    ...
    ScaledCharInfo renderInfo, GlyphInfo glyph, int width, int height
        ...
        if (numLines > 0) {
                ...
                renderInfo->segs = (XSegment *)XtRealloc(
                    (void *)renderInfo->segs, numLines * sizeof(XSegment));
        }
        ...
        if (glyph->numRects > 0) {
                ...
                renderInfo->rects = (XRectangle *)XtRealloc(
                    (void *)renderInfo->rects,
        ...
        if (glyph->numStipples > 0) {
                ...
                renderInfo->stipples = (XRectangle *)XtRealloc(
                    (void *)renderInfo->stipples,
        ...
        for (j = 0; j < glyph->numLines; j++) {
                ...
                DebugF('l', 0,
                       fprintf(stderr,
                               ">> line data:  x1=%d%+d  y1=%d%+d  x2=%d%+d  "
                               "y2=%d%+d width=%d",
                               glyph->lines[j].x1, glyph->lines[j].x1Offset,
                               glyph->lines[j].y1, glyph->lines[j].y1Offset,
                               glyph->lines[j].x2, glyph->lines[j].x2Offset,
                               glyph->lines[j].y2, glyph->lines[j].y2Offset,
                               glyph->lines[j].width));
                ...
                if (x1 == x2) {
                        ...
                        x2 += glyph->lines[j].width - 1 -
                              (glyph->lines[j].width - 1) / 2;
                }

                /* add in the line width... */
                if (y1 == y2) {
                        y1 -= (glyph->lines[j].width - 1) / 2;
                        y2 += glyph->lines[j].width - 1 -
                              (glyph->lines[j].width - 1) / 2;
                }

                DebugF('l', 0,
                       fprintf(stderr,
                               ">> line:  x=%d  y=%d  width=%d  height=%d", x1,
                               y1, x2 - x1 + 1, y2 - y1 + 1));
                        ...
                        for (k = 0; k < glyph->lines[j].width; k++) {
                                renderInfo->segs[numLines + k].y1 =
                                    renderInfo->segs[numLines + k].y2 =
                                        y2 - (glyph->lines[j].width / 2) + k;
                                renderInfo->segs[numLines + k].x1 =
                                    x1 - glyph->lines[j].width / 2;
                                renderInfo->segs[numLines + k].x2 =
                                    x2 + glyph->lines[j].width / 2;
                        }
                }
        ...
        static short *GetLineDrawFontIndex(GlyphInfo glyphInfo, int numGlyphs) {
                ...
                /* no match.  Insert one at the head of the list and fill it
                 * out...
                 */
                lineDrawFontData =
                    (LineDrawFontData)XtMalloc(sizeof(LineDrawFontDataRec));
                ...
                for (i = 0; i < (sizeof(lineDrawFontData->lineDrawIndex) /
                                 sizeof(lineDrawFontData->lineDrawIndex[0]));
                     i++) {
                ...
                for (i = 0; i < numGlyphs; i++) {
                        for (j = 0; glyphInfo[i].chars[j] > 0; j++) {
                                lineDrawFontData
                                    ->lineDrawIndex[glyphInfo[i].chars[j] %
                                                    256] = i;
                        }
                }
                return (lineDrawFontData->lineDrawIndex);
        ...
        LineDrawFont _DtTermPrimLineDrawCreateFont(
            Widget w, GlyphInfo glyphInfo, int numGlyphs, int width, int ascent,
            int descent) {
                ...
                for (lineDrawFont = lineDrawFontHead; lineDrawFont;
                     lineDrawFont = lineDrawFont->next) {
                        if ((lineDrawFont->refCount == 0) &&
                            ...
                            (lineDrawFont->numGlyphs == numGlyphs)) {
                                /* found a free slot... */
                                /* free any valid but unused font in this
                                 * slot... */
                                if (lineDrawFont->fontValid) {
#ifdef USE_PIXMAPS
                                        (void)XFreePixmap(
                                            lineDrawFont->display,
                                            /* Display		*/
                ...
                if (!lineDrawFont) {
                        ...
                        lineDrawFont =
                            (LineDrawFont)XtMalloc(sizeof(LineDrawFontRec));
                        (void)memset(lineDrawFont, '\0',
                                     sizeof(LineDrawFontRec));
                ...
                lineDrawFont->glyphInfo = glyphInfo;
                lineDrawFont->numGlyphs = numGlyphs;
                lineDrawFont->glyphIndex =
                    GetLineDrawFontIndex(glyphInfo, numGlyphs);

#ifdef USE_PIXMAPS
                /* malloc the cell position for this font (if necessary)... */
                if (!lineDrawFont->scaledBitmapInfo) {
                        lineDrawFont->scaledBitmapInfo =
                            (ScaledBitmapInfo)XtMalloc(
                                lineDrawFont->numGlyphs *
                                sizeof(ScaledBitmapInfoRec));
                        /* initialize it... */
                        (void)memset(lineDrawFont->scaledBitmapInfo, '\0',
                                     lineDrawFont->numGlyphs *
                                         sizeof(scaledBitmapInfo));
                } else {
                        /* mark all the characters in the old font as
                         * unscaled... */
                        for (i = 0; i < lineDrawFont->numGlyphs; i++) {
                                lineDrawFont->scaledBitmapInfo[i].scaled =
                                    False;
                        }
                }
#else  /* USE_PIXMAPS */
        ...
        void _DtTermPrimLineDrawImageString(
            Display * display, Drawable d, LineDrawFont lineDrawFont, GC gc,
            GC clearGC, int x, int y, unsigned char *string, int width) {
                int glyph;
#ifndef USE_PIXMAPS
                XSegment segs[20];
                XRectangle rects[20];
                ...
                for (; width > 0; width--, string++) {
                        ...
                        /* if it is still invalid, then let's skip this
                         * character... */
                        if (glyph < 0) {
                ...
                if (!lineDrawFont->scaledCharInfo[glyph].scaled) {
                        ...
                        (void)ScaleCharacter(
                            &(lineDrawFont->scaledCharInfo[glyph]),
                            &(lineDrawFont->glyphInfo[glyph]),
                            lineDrawFont->width, lineDrawFont->height);
                }

                if (lineDrawFont->scaledCharInfo[glyph].numSegs > 0) {
                        for (i = 0;
                             i < lineDrawFont->scaledCharInfo[glyph].numSegs;
                             i++) {
                                segs[i].x1 =
                                    x + lineDrawFont->scaledCharInfo[glyph]
                                            .segs[i]
                                            .x1;
                                segs[i].x2 =
                                    x + lineDrawFont->scaledCharInfo[glyph]
                ...
                if (lineDrawFont->scaledCharInfo[glyph].numRects > 0) {
                        for (i = 0;
                             i < lineDrawFont->scaledCharInfo[glyph].numRects;
                             i++) {
                                rects[i].x =
                                    x + lineDrawFont->scaledCharInfo[glyph]
                                            .rects[i]
