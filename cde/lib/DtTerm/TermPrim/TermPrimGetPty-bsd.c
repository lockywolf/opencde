...
static char rcs_id[] =
    "$XConsortium: TermPrimGetPty-bsd.c /main/4 1996/11/21 19:58:32 drk $";
...
static void AddPtyInfo(char *ptyName) {
        ptyInfo *ptyInfoTmp;
        sigset_t newSigs;
        sigset_t oldSigs;
...
static void DeletePtyInfo(char *ptyName) {
        ptyInfo *ptyInfoTmp;
        sigset_t newSigs;
        sigset_t oldSigs;
                ...
                for (first = 1, char_1 = pty_dirs_ptr->char_1;
                     (first || !pty_dirs_ptr->fast) && *char_1; char_1++) {
                        for (char_2 = pty_dirs_ptr->char_2;
                             (first || !pty_dirs_ptr->fast) && *char_2;
                             char_2++) {
                                for (char_3 = pty_dirs_ptr->char_3;
                                     (first || !pty_dirs_ptr->fast);) {
                                        (void)snprintf(ttyDev, ttyDev_size,
                                                       char_3 ? "%s/tty%c%c%c"
                                                              : "%s/tty%c%c",
                                                       pty_dirs_ptr->pty_dir,
                                                       *char_1, *char_2,
                                                       char_3 ? *char_3 : 0);
                                        (void)snprintf(ptyDev, ptyDev_size,
                                                       char_3 ? "%s/pty%c%c%c"
                                                              : "%s/pty%c%c",
                                        ...
                                        /* If we either were able to open the
                                         * pty master (i.e., slave open failed),
                                         * or the device was busy, keep going...
                                         */
...
static void ReleasePty(char *ptySlave) {
