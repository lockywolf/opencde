...
static char rcs_id[] =
    "$TOG: TermPrimUtil.c /main/3 1997/04/17 18:25:29 samborn $";
...
static void PointerMoved(Widget w, XtPointer closure, XEvent *event,
                         Boolean *cont);
...
#define whiteW 16
#define whiteH 16
static char whiteBits[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

static void InitPointerBlank(Widget w) {
        ...
        */
        ...
        tpd->noPointer = XCreatePixmapCursor(
            XtDisplay(tw), noPointerBitmap, /* source bitmap    */
                ...
                if (tpd->pointerOn) {
                        ...
                        */
                        if (tw->term.pointerBlankDelay) {
                                ...
                                */
                                tpd->pointerTimeoutID = XtAppAddTimeOut(
                                    XtWidgetToApplicationContext((Widget)tw),
                                    (unsigned long)1000 *
                                        tw->term.pointerBlankDelay,
...
void _DtTermPrimPointerOff(Widget w, XtIntervalId *id) {
        ...
        if (tpd->pointerOn) {
                ...
                */
                (void)XDefineCursor(XtDisplay(tw), XtWindow(tw),
                                    tpd->noPointer);
...
void _DtTermPrimPointerOn(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
...
void _DtTermPrimPointerFreeze(Widget w, Boolean freeze) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
...
void _DtTermPrimRecolorPointer(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
...
static void AddLogFileEntry(FILE *logFile) {
        logInfo *logInfoTmp;
        sigset_t newSigs;
        sigset_t oldSigs;
...
static void DeleteLogFileEntry(FILE *logFile) {
        logInfo *logInfoTmp;
        sigset_t newSigs;
        sigset_t oldSigs;
...
void _DtTermPrimStartLog(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
        ...
        if (tw->term.log_on || tw->term.logInhibit) {
                return;
        }
        ...
        if ('|' == *tw->term.logFile) {
            ...
            */ for (i = 3; i < _NFILE; i++) {
                        ...
                        */
                        ...
                        _bA_dump();
#endif /* BBA */
                        (void)execl(DEFAULT_SHELL, DEFAULT_SHELL_ARGV0, "-c",
                                    &tw->term.logFile[1], NULL);
...
static void ForceCloseLog(DtTermPrimitiveWidget tw) {
        DtTermPrimData tpd = tw->term.tpd;
...
void _DtTermPrimCloseLog(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        DtTermPrimData tpd = tw->term.tpd;
...
void _DtTermPrimWriteLog(DtTermPrimitiveWidget tw, char *buffer, int cnt) {
        DtTermPrimData tpd = tw->term.tpd;
...
void _DtTermPrimToggleSuidRoot(Boolean root) {
        (void)suidInit();
...
void _DtTermPrimLogFileCleanup(void) {
        ...
        DebugF('s', 10,
               fprintf(stderr, ">>_DtTermPrimLogFileCleanup() starting\n"));
        ...
        for (logInfoTmp = logInfoHead->next; logInfoTmp;
             logInfoTmp = logInfoTmp->next) {
                DebugF('s', 10,
                       fprintf(stderr, ">>flushing logfile 0x%lx\n",
                               (unsigned long)logInfoTmp->logFile));
                (void)fflush(logInfoTmp->logFile);
        }
        _DtTermProcessUnlock();
        DebugF('s', 10,
               fprintf(stderr, ">>_DtTermPrimLogFileCleanup() finished\n"));
}
