...
static char rcs_id[] =
    "$XConsortium: TermPrimRenderMb.c /main/1 1996/04/21 19:19:05 drk $";
...
void _DtTermPrimRefreshTextWc(Widget w, short startColumn, short startRow,
        ...
        DebugF('t', 0,
               fprintf(stderr, ">>_DtTermPrimRefreshTextWc() starting\n"));
        DebugF('t', 0,
               fprintf(stderr,
                       ">>_DtTermPrimRefreshTextWc() startCol=%hd  "
                       "startRow=%hd  endCol=%hd  endRow=%hd\n",
        ...
        */
        if (tw->term.jumpScroll && tpd->scroll.jump.scrolled) {
                ...
                DebugF('t', 0,
                       fprintf(stderr, ">>_DtTermPrimRefreshTextWc() jump "
                                       "scroll in progress, no render\n"));
                return;
        }
        ...
        for (; startRow <= endRow; startRow++) {
                ...
                if (lineNum >= tpd->lastUsedRow) {
                            ...
                            (-lineNum <= tpd->lastUsedHistoryRow)) {
                                ...
                                if ((startColumn < lineWidth) &&
                                    _DtTermPrimGetCharacterInfo(
                                        ...
                                        thisStartColumn =
                                            startCharInfo.startCol;
                                } else {
                                        /* past the last character in this
                                         * line... */
                                        thisStartColumn = startColumn;
                                }

                                /* get the line width and a pointer to the
                                 * data... */
                                thisEndColumn = endColumn;
                                lineWidth = MAX(
                                    0, MIN(thisEndColumn - thisStartColumn + 1,
                        ...
                        /* adjust startColumn to point to the first column for
                         * the first character...
                         */
                        lineWidth =
                            _DtTermPrimBufferGetLineWidth(tBuffer, lineNum);
                        if ((startColumn < lineWidth) &&
                            _DtTermPrimGetCharacterInfo(tBuffer, lineNum,
                                                        startColumn,
                                                        &startCharInfo)) {
                        ...
                        thisEndColumn = endColumn;
                        lineWidth =
                            MAX(0, MIN(thisEndColumn - thisStartColumn + 1,
                ...
                while (lineWidth > 0) {
                         ...
                         * _DtTermPrimSelectIsInSelection clips the chunkWidth
                         * to the end of the selection if we are...
                         */
                        ...
                        /* adjust for a multi-column character that spans the
                         * line boundary...
                         */
...
#pragma BBA_IGNORE
        ...
        DebugF('t', 0,
               fprintf(stderr, ">>_DtTermPrimRefreshTextWc() finished\n"));
}
...
void _DtTermPrimExposeTextMb(Widget w, int x, int y, int width, int height,
        ...
        DebugF('t', 0,
               fprintf(stderr,
                       ">>exposeText() calling _DtTermPrimRefreshTextWc()\n"));
        (void)_DtTermPrimRefreshTextWc(
...
static short DoInsertWc(Widget w, wchar_t *wcBuffer, int wcBufferLen,
                        Boolean *wrapped) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        TermBuffer tBuffer = tpd->termBuffer;
        ...
        */
        ...
        newWidth = _DtTermPrimBufferInsertWc(
            tBuffer,                      /* TermBuffer	    */
            ...
            tpd->insertCharMode !=
                DtTERM_INSERT_CHAR_OFF, /* insert flag	    */
        ...
        *wrapped = True;
        ...
        /*
        ** Copy the returned characters to a temporary buffer so we don't have
        *to
        ** worry about _DtTermPrimBufferInsertWc tromping over its overflow
        *buffer...
        */
...
*/
int _DtTermPrimInsertTextWc(Widget w,
        ...
        if (insertCharCount > 0) {
                ...
                **    character...
                */
                if (insertCharCount == 1 && thisCharWidth == 2 &&
        ...
        if (needToRender) {
                ...
                DebugF('t', 0,
                       fprintf(stderr, ">>termInsertText() calling "
                                       "_DtTermPrimRefreshTextWc()\n"));
                (void)_DtTermPrimRefreshTextWc(
                    w, renderStartX - 1, tpd->cursorRow,
