...
static char rcs_id[] =
    "$XConsortium: TermPrimRepType.c /main/1 1996/04/21 19:19:11 drk $";
...
static void CvtStringToTerminalSizeDestroy(XtAppContext app, XrmValue *to,
                                           XtPointer converter_data,
                                           XrmValue *args, Cardinal *num_args);

static void CvtStringToTerminalSizeListDestroy(XtAppContext app, XrmValue *to,
                                               XtPointer converter_data,
                                               XrmValue *args,
                                               Cardinal *num_args);

static Boolean CvtStringToTerminalSize(Display *display, XrmValue *args,
                                       Cardinal *num_args, XrmValue *from,
                                       XrmValue *to, XtPointer *converter_data);

static Boolean CvtStringToTerminalSizeList(Display *display, XrmValue *args,
                                           Cardinal *num_args, XrmValue *from,
                                           XrmValue *to,
                                           XtPointer *converter_data);
...
void _DtTermPrimInitRepTypes(void) {
        ...
        /* register our resource converters... */
                ...
                (void)XtSetTypeConverter(
                    XmRString, DtRDtTermTerminalSize, CvtStringToTerminalSize,
                    NULL, 0, XtCacheNone, CvtStringToTerminalSizeDestroy);
#endif /* NotDefined */
                (void)XtSetTypeConverter(XmRString, DtRDtTermTerminalSizeList,
                                         CvtStringToTerminalSizeList, NULL, 0,
                                         XtCacheNone,
                                         CvtStringToTerminalSizeListDestroy);
                (void)XmRepTypeRegister(
                    DtRDtTermEmulationMode, TermEmulationModeStrings, NULL,
                    (unsigned char)XtNumber(TermEmulationModeStrings));
                (void)XmRepTypeRegister(
                    DtRDtTermCharCursorStyle, CharCursorStyleStrings, NULL,
                    (unsigned char)XtNumber(CharCursorStyleStrings));

                _DtTermPrimEmulationMode =
                    XmRepTypeGetId(DtRDtTermEmulationMode);
                _DtTermPrimCharCursorStyle =
                    XmRepTypeGetId(DtRDtTermCharCursorStyle);
...
static Boolean CvtStringToTerminalSizeList(Display *display, XrmValue *args,
        ...
        sizesSize += 10;
        sizes = (DtTermTerminalSize *)XtRealloc(
            (char *)sizes, sizesSize * sizeof(DtTermTerminalSize));
        ...
        for (numSizes = 0, c1 = str; c1 && *c1; numSizes++) {
                ...
                if (*c1 != 'x') {
                        /* parse off columns... */
                        if ((*c1 >= '0') && (*c1 <= '9')) {
                                sizes[numSizes].columns =
                                    (short)strtol(c1, &c1, 10);
                        } else {
        ...
        sizeList->sizes = (DtTermTerminalSize *)XtMalloc(
            numSizes * sizeof(DtTermTerminalSize));
        (void)memcpy(sizeList->sizes, sizes,
                     numSizes * sizeof(DtTermTerminalSize));
