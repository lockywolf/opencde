...
static char rcs_id[] =
    "$TOG: TermPrimRender.c /main/3 1999/07/20 17:34:54 mgreess $";
...
#ifdef DKS
void _termSetRenderFont(Widget w, TermFont *termFont) {
         ...
         *
         *	    The following opens up a memory leak that needs to be
         *closed. we need to free up both the list and any created entries...
         */

        /* do initialization... */
        if (!tpd->renderTermFontsNum) {
                tpd->renderTermFontsNum = 4;
                tpd->renderTermFonts = (TermFont **)malloc(
                    (unsigned)tpd->renderTermFontsNum * sizeof(TermFont *));
        ...
        for (i = 0; i < tpd->renderTermFontsNum; i++) {
                if (tpd->renderTermFonts[i] &&
                    (tpd->renderTermFonts[i]->id == termFont->id))
        ...
        if (i >= tpd->renderTermFontsNum) {
                ...
                if (i >= tpd->renderTermFontsNum) {
                        (void)fprintf(stderr,
                                      "setRenderFont: list full can't add font "
                                      "to widget 0x%lx\n",
                                      w);
                }
        ...
        if (tw->term.visualBell) {
                ...
                if (tpd->renderGC.foreground !=
                    (tw->primitive.foreground ^ tw->core.background_pixel)) {
                        tpd->renderGC.foreground = tw->primitive.foreground ^
...
void _DtTermPrimRefreshText(Widget w, short startColumn, short startRow,
        ...
        DebugF('t', 0,
               fprintf(stderr, ">>_DtTermPrimRefreshText() starting\n"));
        DebugF('t', 0,
               fprintf(stderr,
                       ">>_DtTermPrimRefreshText() startCol=%hd  startRow=%hd  "
                       "endCol=%hd  endRow=%hd\n",
                       startColumn, startRow, endColumn, endRow));

        if (tpd->mbCurMax > 1) {
                _DtTermPrimRefreshTextWc(w, startColumn, startRow, endColumn,
                                         endRow);
                return;
        }
        ...
        */
        if (tw->term.jumpScroll && tpd->scroll.jump.scrolled) {
                ...
                DebugF('t', 0,
                       fprintf(stderr, ">>_DtTermPrimRefreshText() jump scroll "
                                       "in progress, no render\n"));
                return;
        }
        ...
        if (tpd->clearGC.foreground != tw->term.reverseVideo
                ? tw->core.background_pixel
                : tw->primitive.foreground) {
                values.foreground = tw->term.reverseVideo
                                        ? tw->primitive.foreground
        ...
        if (tpd->clearGC.background != tw->term.reverseVideo
                ? tw->primitive.foreground
                : tw->core.background_pixel) {
                values.background = tw->term.reverseVideo
                                        ? tw->core.background_pixel
        ...
        if (valueMask) {
                (void)XChangeGC(XtDisplay(w), tpd->clearGC.gc, valueMask,
                                &values);
        }
        ...
        for (; startRow <= endRow; startRow++) {
                ...
                if (tpd->useHistoryBuffer) {
                        if (_DtTermPrimSelectGetSelection(w, &begin, &end) &&
                            (begin < end) &&
                            (lineNum >=
                             (begin / (tpd->selectInfo->columns + 1)) -
                ...
                if (lineNum >= tpd->lastUsedRow) {
                        /* we are pointing to empty screen space below the last
                         * used line of the display...
                         */
                        lineWidth = 0;
                        linePtr = NULL;
                } else if (lineNum < 0) {
                        if ((tpd->useHistoryBuffer) &&
                            (-lineNum <= tpd->lastUsedHistoryRow)) {
                                ...
                                lineWidth = MAX(
                                    0,
                                    MIN(endColumn - startColumn + 1,
                                        _DtTermPrimBufferGetLineWidth(
                                            tpd->historyBuffer,
                                            tpd->lastUsedHistoryRow + lineNum) -
                                            startColumn));
                                linePtr = _DtTermPrimBufferGetCharacterPointer(
                                    tpd->historyBuffer,
                                    tpd->lastUsedHistoryRow + lineNum,
                                    startColumn);
                        } else {
                                /* we are above the history buffer.  Should not
                                 * happen, but...
                                 */
                                lineWidth = 0;
                                linePtr = NULL;
                ...
                while (lineWidth > 0) {
                         ...
                         */
                        if (lineNum >= 0) {
                                (void)_DtTermPrimBufferGetEnhancement(
                                    tBuffer,
                                    /* TermBuffer		*/
                                    lineNum,          /* row			*/
                        ...
                        if (ENH_PROC(tBuffer)) {
                                (void)(*(ENH_PROC(tBuffer)))(w, enhancements,
                                                             &enhInfo);
                        }
                        ...
                        if (checkSelection && _DtTermPrimSelectIsInSelection(
                                                  w, lineNum, chunkStartColumn,
                        ...
                        if (TermIS_SECURE(enhInfo.flags)) {
                                ...
                                if (tpd->renderReverseGC.foreground !=
                                    enhInfo.bg) {
                                        tpd->renderReverseGC.foreground =
                                            enhInfo.bg;
                                ...
                                (void)XFillRectangle(
                                    ...
                                    tpd->renderReverseGC.gc,
                                    chunkStartColumn * tpd->cellWidth +
                                        tpd->offsetX,
                                    startRow * tpd->cellHeight + tpd->offsetY,
                                ...
                                if (TermIS_UNDERLINE(enhInfo.flags)) {
                                        ...
                                        (void)XDrawLine(
                                            ...
                                            chunkStartColumn * tpd->cellWidth +
                                                tpd->offsetX,
                                            /* X1			*/
                                            startRow * tpd->cellHeight +
                                                tpd->offsetY + tpd->cellHeight -
                                                1,
                                            /* Y1			*/
                ...
                /* clear any extra space in the line.  chunkStartColumn now
                 * points to the end of the line, and lineWidth == 0...
                 */
...
#pragma BBA_IGNORE
#endif /*BBA*/
                                /* Fill in the clear area so we can see what is
                                 * going to be displayed...
                                 */
                                (void)XFillRectangle(
                                    XtDisplay(w), XtWindow(w), tpd->renderGC.gc,
                                    chunkStartColumn * tpd->cellWidth +
                                        tpd->offsetX,
                                    startRow * tpd->cellHeight + tpd->offsetY,
...
void _DtTermPrimExposeText(Widget w, int x, int y, int width, int height,
        ...
        DebugF('e', 0,
               fprintf(stderr,
                       ">>             offsetX=%d  offsetY=%d  cellHeight=%d  "
                       "cellWidth=%d\n",
                       tpd->offsetX, tpd->offsetY, tpd->cellHeight,
                       tpd->cellWidth));
        ...
        if (isExposeEvent && tpd->scrollInProgress) {
                ...
                bothY2 =
                    MIN(tpd->scrollSrcY + tpd->scrollHeight, y + height - 1);

                if ((bothX2 > bothX1) && (bothY2 > bothY1)) {
                        (void)_DtTermPrimRefreshText(
                            w,
                            (x - tpd->offsetX + tpd->scrollDestX -
                             tpd->scrollSrcX) /
                                tpd->cellWidth,
                            (y - tpd->offsetY + tpd->scrollDestY -
                             tpd->scrollSrcY) /
                                tpd->cellHeight,
        ...
        DebugF('t', 0,
               fprintf(stderr,
                       ">>exposeText() calling _DtTermPrimRefreshText()\n"));
        (void)_DtTermPrimRefreshText(
            w, (x - tpd->offsetX) / tpd->cellWidth,
...
void _DtTermPrimFillScreenGap(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        TermBuffer tBuffer = tpd->termBuffer;
        int linesNeeded;
        ...
        if (tpd->topRow + tpd->cursorRow >= tpd->lastUsedRow) {
                ...
                for (; (tpd->lastUsedRow < tpd->bufferRows) &&
                       (tpd->topRow + tpd->cursorRow >= tpd->lastUsedRow);) {
                        ...
                        _DtTermPrimBufferClearLine(tBuffer, tpd->lastUsedRow++,
                                                   0);
                }
                        ...
                        if (tpd->memoryLockMode == SCROLL_LOCKprotect) {
                                if (!tpd->warningDialogMapped) {
                                        (void)_DtTermPrimWarningDialog(
                                            w, GETMESSAGE(NL_SETN_PrimRend, 1,
                                                          "MEMORY FULL\nPress "
                                                          "OK to clear"));
                                }
                        } else {
                                 ...
                                 * using a history buffer, we need to transfer
                                 * them into the history buffer...
                                 */
                                if (tpd->useHistoryBuffer &&
                                    (tpd->scrollLockTopRow == 0) &&
                                    (tpd->scrollLockBottomRow >=
                                     (tw->term.rows - 1))) {
                                        /* do we have enough lines in the
                                         * history buffer, or do we need to
                                         * steal some from the top?...
                                         */
                                        historyLinesNeeded =
                                            linesNeeded -
                                        ...
                                        if ((historyLinesNeeded > 0) &&
                                            (tpd->historyBufferRows > 0)) {
                                                ...
                                                /* we are effectively deleting
                                                 * the lines from the top of the
                                                 * combined buffers...
                                                 */
                                                (void)
                                                ...
                                                (void)
                                                    _DtTermPrimBufferInsertLineFromTB(
                                                        ...
                                                        historyLinesNeeded,
                                                        insertFromTop);
                                                /* the lines are now freed,
                                                 * adjust the used count... */
                                                tpd->lastUsedHistoryRow -=
                                                    historyLinesNeeded;
                                                if (tpd->lastUsedHistoryRow <
                                                    0) {
                                                        historyLinesNeeded +=
                                                            tpd->lastUsedHistoryRow;
                                                        tpd->lastUsedHistoryRow =
                                                ...
                                                */
                                                if (length > 0) {
                                                        ...
                                                        for (eCol = 0;
                                                             eCol < length;
                                                             eCol += eCount) {
                                                                if (_DtTermPrimBufferGetEnhancement(
                                                                        tBuffer,
                                                                        i1,
                                                                        eCol,
                                                                        &eValues,
                                                ...
                                                /* clear the inselection flag
                                                 * before we scroll the lines
                                                 * off or we will end up
                                ...
                                /* take them from the top.  If we are about to
                                 * take lines from the top without first copying
                                 * them into the history buffer, we need to
                                 * adjust the selection position...
                                 */
                                if (linesNeeded > linesCopied) {
                                        (void)_DtTermPrimSelectDeleteLines(
                                            w,
                                            tpd->lastUsedHistoryRow +
                                                linesCopied,
                                ...
                                (void)_DtTermPrimBufferInsertLineFromTB(
                                    tBuffer, tpd->bufferRows - 1, linesNeeded,
                                    insertFromTop);
                                /* adjust everything... */
...
static short DoInsert(Widget w, unsigned char *buffer, int length,
                      Boolean *wrapped) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        TermBuffer tBuffer = tpd->termBuffer;
        ...
        newWidth = _DtTermPrimBufferInsert(
            tBuffer,                      /* TermBuffer		*/
        ...
        if ((tpd->insertCharMode != DtTERM_INSERT_CHAR_ON_WRAP) ||
            (returnCount <= 0)) {
...
int _DtTermPrimInsertText(Widget w, unsigned char *buffer, int length) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        TermBuffer tBuffer = tpd->termBuffer;
        int i;
...
#ifdef NOCODE
                /*
                ** It would be nice if the calling function supplied us with a
                *count
                ** of the number of mb characters in the buffer, then we
                *wouldn't
                ** have to count them again.
                */
                /*
                ** we could use this if the multi-byte buffer was null
                *terminated
                */
                wcBufferLen = mbstowcs((wchar_t *)NULL, (char *)buffer, length);
                        ...
                        pmb = (char *)buffer;
                        /*
                        ** we should never need more than length *
                        *sizeof(wchar_t)
                        ** bytes to store the wide char equivalent of the
                        *incoming mb string
                        */
                        wcBuffer =
                            (wchar_t *)XtMalloc(length * sizeof(wchar_t));
                        pwc = wcBuffer;
                        wcBufferLen = 0;
                        while (i < length) {
                                switch (mbLen = mbtowc(pwc, pmb,
                                                       MIN(((int)MB_CUR_MAX),
                                                           length - i))) {
                                case -1:
                                        if ((int)MB_CUR_MAX <= length - i) {
                                                /* we have a bogus multi-byte
                                                 * character.  Throw away the
                                                 * first byte and rescan (TM
                                                 * 12/14/93)...
                                                 */
                                                /*
                                                ** in this case, we move the
                                                *remaining length - i - 1
                                                ** bytes one byte to the left
                                                *(to overwrite the bogus
                                                ** byte)
                                                */
                                                memmove(pmb, pmb + 1,
                                                        length - i - 1);
                                ...
                                case 0:
                                        /*
                                        ** treat null character same as any
                                        *other character...
                                        */
                                        mbLen = 1;
                                default:
                ...
                while (i < wcBufferLen) {
                        switch (mbLen = mblen(
                                    pmb, MIN(((int)MB_CUR_MAX), length - i))) {
                        case -1:
                        case 0:
                                /*
                                ** treat null character same as any other
                                *character...
                                */
                                mbLen = 1;
                        default:
        ...
        /* we support two different types of autowrap.  The HP style one is
         * where you display the character and then wrap if you are at the end
         * of the line.  The ANSI style one is where you insert the character at
         * the end of the line and don't autowrap until you try to insert
         * another character...
         */
        ...
        for (i = 0; (i < length) && tpd->ptyInputId; i++) {
                ...
                if (((tpd->cursorColumn + insertCharCount) >=
                     tw->term.columns) ||
                    ((tpd->cursorColumn + insertCharCount) ==
                     tpd->rightMargin + 1)) {
                        if (tpd->autoWrapRight) {
                                ...
                                if (insertCharCount) {
                                        newWidth =
                                            DoInsert(w, &buffer[insertStartX],
                                                     insertCharCount, &wrapped);
                                        tpd->cursorColumn += insertCharCount;
                                        insertCharCount = 0;
                                        if (tpd->insertCharMode !=
                                            DtTERM_INSERT_CHAR_OFF) {
                                                renderEndX = newWidth;
                                        } else {
                                                renderEndX =
                                                    MAX(renderEndX,
                                ...
                                if (needToRender) {
                                        DebugF('t', 0,
                                               fprintf(stderr, ">>"
                                                               ...
                                                               "eshText()\n"));
                                        (void)_DtTermPrimRefreshText(
                                            w, renderStartX, tpd->cursorRow,
                                            wrapped ? tw->term.columns
                                                    : MAX(renderEndX, 0),
                                            tpd->cursorRow);
                                        if (wrapped && (tpd->cursorRow + 1 <
                                                        tw->term.rows)) {
                                                (void)_DtTermPrimRefreshText(
                                                    w, 0, tpd->cursorRow + 1,
                                                    renderEndX,
                                ...
                                _DtTermPrimBufferSetLineWrapFlag(
                                    tBuffer, tpd->topRow + tpd->cursorRow,
                                    True);

                                if (tpd->cursorRow ==
                                    tpd->scrollLockBottomRow) {
                                        /* scroll at the bottom of the lock
                                         * area... */
                                        (void)_DtTermPrimScrollText(w, 1);
                                        (void)_DtTermPrimFillScreenGap(w);
                                } else if (++tpd->cursorRow >= tw->term.rows) {
                                        /* we are at the bottom row of the
                                         * locked region. Wrap to the beginning
                                         * of this line...
                                         */
                                        tpd->cursorRow = tw->term.rows - 1;
                                } else {
                                        /* we are not at the bottom row.  We
                                         * already have wrapped down one line...
                                         */
                                }
                                ...
                                if (tpd->scroll.nojump.pendingScroll) {
                                        /* If we wrapped the screen, bail out
                                         * now and we will take care of this
                                         * character when we finish the scroll.
                                         * If we are in wrap after, then we need
                                         * to skip past this character so that
                                         * it doesn't get processed twice...
                                         */
                                        if (tpd->wrapRightAfterInsert)
                                                (void)i++;
                                        break;
                                ...
                                /* overwrite the last character(s) on the
                                 * line... */
                                if (insertCharCount > 0) {
                                        newWidth =
                                            DoInsert(w, &buffer[insertStartX],
        ...
        if (insertCharCount > 0) {
                newWidth = DoInsert(w, &buffer[insertStartX], insertCharCount,
                                    &wrapped);
                tpd->cursorColumn += insertCharCount;
                insertCharCount = 0;
        ...
        if (needToRender) {
                renderEndX = MAX(renderEndX, tpd->cursorColumn);
                DebugF('t', 0,
                       fprintf(stderr, ">>termInsertText() calling "
                                       "_DtTermPrimRefreshText()\n"));
...
static void buildDangleBuffer(unsigned char *buffer, int bufferLen,
                              unsigned char *mbPartialChar,
                              int *mbPartialCharLen, int writeLen,
        ...
        *dangleBuffer = (unsigned char *)XtMalloc(*dangleBufferLen);
        ...
        if (*mbPartialCharLen) {
                (void)memmove(*dangleBuffer + bufferLen - writeLen,
                              mbPartialChar, *mbPartialCharLen);
                *mbPartialCharLen = 0;
        }
        return;
}

Boolean _DtTermPrimParseInput(Widget w, unsigned char *buffer, int len,
                              unsigned char **dangleBuffer,
                              int *dangleBufferLen) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        ...
        *dangleBuffer = (unsigned char *)0;
        ...
        if (tpd->mbPartialCharLen > 0) {
                tmpBuffer =
                    (unsigned char *)XtMalloc(len + tpd->mbPartialCharLen);
                (void)memcpy(tmpBuffer, tpd->mbPartialChar,
        ...
        for (i = 0; (i < len) && tpd->ptyInputId;) {
                if (tpd->mbCurMax > 1) {
                        ...
                        case -1:
                                if ((int)MB_CUR_MAX <= len - i) {
                                        /* we have a bogus multi-byte character.
                                         * Throw away the first byte and rescan
                                         * (TM 12/14/93)...
                                         */
                                        /* dump what we know we want to
                                         * insert... */
                                        if (insertByteCount > 0) {
                                                returnLen =
                                                    (*(termClassPart
                                                           ->term_insert_proc))(
                                                        w, &buffer[insertStart],
                                                        insertByteCount);
                                                if (returnLen !=
                                                    insertByteCount) {
                                                        (void)buildDangleBuffer(
                                                            buffer, len,
                                                            tpd->mbPartialChar,
                                                            &tpd->mbPartialCharLen,
                                                            insertStart +
                                        ...
                                        /* skip over the bogus char's first
                                         * byte... */
                                        (void)i++;
                                        insertStart = i;
                                        continue;
                                } else {
                                        /* we have a dangling partial multi-byte
                                         * character... */
                                        (void)memmove(tpd->mbPartialChar,
                                                      &buffer[i], len - i);
                                        tpd->mbPartialCharLen = len - i;
                                        /* remove the partial char from the
                                         * buffer and adjust the buffer len...
                                         */
                                        len = i;
                                        continue;
                                }
                ...
                if (((mbCharLen == 1) && preParseTable[buffer[i]]) ||
                    tpd->parserNotInStartState) {
                        /*
                        ** It's either a control code or we are not in the start
                        *state.
                        ** If we have any text to insert, insert
                        ** the text, display any added text, then send it down
                        *through
                        ** the parser.
                        */
                        if (insertByteCount > 0) {
                                returnLen =
                                    (*(termClassPart->term_insert_proc))(
                                        w, &buffer[insertStart],
                                        insertByteCount);
                                if (returnLen != insertByteCount) {
                                        (void)buildDangleBuffer(
                                            buffer, len, tpd->mbPartialChar,
                                            &tpd->mbPartialCharLen,
                        ...
                        tpd->parserNotInStartState =
                            _DtTermPrimParse(w, &buffer[i], mbCharLen);
                        i += mbCharLen;
                        insertStart = i;
                } else {
        ...
        if (insertByteCount > 0) {
                returnLen = (*(termClassPart->term_insert_proc))(
                    w, &buffer[insertStart], insertByteCount);
                if (returnLen != insertByteCount) {
                        (void)buildDangleBuffer(buffer, len, tpd->mbPartialChar,
                                                &tpd->mbPartialCharLen,
                                                insertStart + returnLen,
                                                dangleBuffer, dangleBufferLen);
                }
...
*/
void _DtTermPrimRenderPadLine(Widget w) {
        DtTermPrimitiveWidget tw = (DtTermPrimitiveWidget)w;
        struct termData *tpd = tw->term.tpd;
        TermBuffer tBuffer = tpd->termBuffer;
        short currentWidth;
        ...
        currentWidth = _DtTermPrimBufferGetLineWidth(
            tBuffer, tpd->topRow + tpd->cursorRow);
        if (tpd->cursorColumn >= currentWidth) {
                ...
                */
                if (tpd->mbCurMax > 1) {
                        _DtTermPrimBufferPadLineWc(tBuffer,
                                                   tpd->topRow + tpd->cursorRow,
                                                   tpd->cursorColumn + 1);
                } else {
                        _DtTermPrimBufferPadLine(tBuffer,
                                                 tpd->topRow + tpd->cursorRow,
                                                 tpd->cursorColumn + 1);
                }
                _DtTermPrimRefreshText(w, currentWidth, tpd->cursorRow,
                                       tpd->cursorColumn, tpd->cursorRow);
        }
}

void _DtTermPrimDestroyFont(Widget w, TermFont font) {
        if (font)
                (void)(*(font->destroyFunction))(w, font);
        return;
}

void _DtTermPrimRenderText(Widget w, TermFont font, Pixel fg, Pixel bg,
                           unsigned long flags, int x, int y,
                           unsigned char *string, int len) {
        (void)(*(font->renderFunction))(w, font, fg, bg, flags, x, y, string,
                                        len);
        return;
}
