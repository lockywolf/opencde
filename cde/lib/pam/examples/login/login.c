 ...
 */
static char *localeenv[] = {"LANG",        "LC_CTYPE",   "LC_NUMERIC",
                            "LC_TIME",     "LC_COLLATE", "LC_MONETARY",
                            "LC_MESSAGES", "LC_ALL",     0};
...
static char *newenv[10 + MAXARGS] = {home, path, logname, hertz, term, 0, 0};
...
void main(int argc, char *argv[], char **renvp) {
        ...
        if ((pam_start(progname, user_name, &pam_conv, &pamh)) != PAM_SUCCESS)
                login_exit(1);
        ...
        if (pwd->pw_uid == 0)
                if (remote_host[0] && dosyslog)
                        syslog(LOG_NOTICE, "ROOT LOGIN %s FROM %.*s", ttyn,
                               HMAX, remote_host);
                else if (dosyslog)
...
/*ARGSUSED*/
static void catch (int sig) { ++intrupt; }
#endif
...
static char **getargs(char *in_line) {
...
static char *illegal[] = {"SHELL=",  "HOME=", "LOGNAME=",
#ifndef NO_MAIL
                          "MAIL=",
#endif
                          "CDPATH=", "IFS=",  "PATH=",    0};
...
static int get_options(int argc, char *argv[]) {
        ...
        while ((c = getopt(argc, argv, "f:h:r:pad:")) != -1) {
                ...
                case 'h':
                        if (hflag || rflag) {
                                (void)fprintf(
                                    stderr, "Only one of -r and -h allowed\n");
                                login_exit(1);
                        }
                        hflag++;
...
static void validate_account() {
        ...
        if ((error = pam_acct_mgmt(pamh, flag)) != PAM_SUCCESS) {
                if (error == PAM_AUTHTOKEN_REQD) {
                        ...
                        (void)printf(incorrectmsg);
                        if (dosyslog)
                                syslog(LOG_CRIT, "login account failure: %s",
                                       pam_strerror(pamh, error));
                        login_exit(1);
                }
        }
...
static void login_authenticate() {
        ...
        if (cnt >= MAXTRYS) {
                ...
                if (remote_host[0] && dosyslog) {
                        syslog(LOG_CRIT,
                               "REPEATED LOGIN FAILURES ON %s FROM %.*s", ttyn,
                               HMAX, remote_host);
                } else if (dosyslog) {
                        syslog(LOG_CRIT, "REPEATED LOGIN FAILURES ON %s", ttyn);
                }
                (void)sleep(DISABLETIME);
                exit(1);
        }
                ...
                if ((u->ut_type == INIT_PROCESS ||
                    ...
                    ((sublogin &&
                     ...
                     u->ut_pid == utmp.ut_pid)) {
                        ...
                        (void)memcpy(utmp.ut_id, u->ut_id, sizeof(utmp.ut_id));
...
static int process_chroot_logins(void) {
        ...
        if (*pwd->pw_shell == '*') {
                ...
                (void)execle("/usr/bin/login", "login", (char *)0, &envinit[0]);
...
static void establish_user_environment(char **renvp) {
        ...
        while (*lenvp++)
                ;
        envinit =
            (char **)calloc(lenvp - environ + 10 + MAXARGS, sizeof(char *));
        if (envinit == NULL) {
...
#else
        (void)printf(
            "UNIX System V Release %s AT&T %s\n%s\n"
            "Copyright (c) 1984, 1986, 1987, 1988 AT&T\nAll Rights Reserved\n",
            un.release, un.machine, un.nodename);
#endif /* i386 */
#endif /* DOBANNER */
