...
static int dce_changepw(char *account_name, char *old_pass, char *new_pass);

static char *get_passwd(pam_handle_t *pamh, char *prompt);
...
int pam_sm_chauthtok(pam_handle_t *pamh, int flags, int argc,
        ...
        if (use_mapped_pass || try_mapped_pass) {
                ...
                if ((err = pam_get_item(pamh, PAM_AUTHTOK, (void **)&newpass)) <
                    0)
                        return (err);
                ...
                got_mapped =
                    xfn_get_mapped_password(0, user, DCE_XFN_PASS_ATTR, oldpass,
                                            dcepass, sizeof(dcepass));

                if (got_mapped) {

                        updated_mapped = xfn_update_mapped_password(
                            0, user, DCE_XFN_PASS_ATTR, newpass, dcepass);
        ...
        if (try_first_pass || use_first_pass) {
                ...
                if (result == PAM_SUCCESS) {
                        /* might need to update PAM_OLDAUTHTOK and
                         * PAM_NEWAUTHTOK */
                        goto out;
                }
                ...
                oldpass =
                    get_passwd(pamh, PAM_MSG(pamh, 20, "Old DCE password: "));
        ...
        if (firstpass == NULL && !(try_first_pass || try_mapped_pass))
                newpass =
                    get_passwd(pamh, PAM_MSG(pamh, 21, "New DCE password: "));
        else
                newpass =
                    get_passwd(pamh, PAM_MSG(pamh, 21, "New DCE password: "));
        ...
        if (firstpass == NULL && !(try_first_pass || try_mapped_pass))
                vnewpass = get_passwd(
                    pamh, PAM_MSG(pamh, 22, "Re-enter new DCE password: "));
        else
                vnewpass = get_passwd(
                    pamh, PAM_MSG(pamh, 22, "Re-enter new DCE password: "));
...
static int dce_changepw(char *account_name, char *old_pass, char *new_pass) {
        ...
        passwd_rec.key.tagged_union.plain = (idl_char *)strdup(old_pass);
        ...
        login_valid = sec_login_validate_identity(
            login_context, &passwd_rec, &reset_pass, &auth_src, &status);
        ...
        sec_rgy_acct_lookup(rgy_handle, &name_key, &account_cursor,
        ...
        sec_rgy_acct_passwd(rgy_handle, &name_result, &caller_key, &new_key,
                            sec_passwd_des, &new_key_version, &status);
