...
static int attempt_dce_login(void *pamh, dce_module_data_t *dsd,
                             error_status_t *st, char *user, char *dce_pass);

void dce_cleanup(pam_handle_t *pamh, void *data, int pam_status);
...
int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc,
        ...
        if (!sec_login_setup_identity((unsigned char *)user, sec_login_no_flags,
        ...
        if (firstpass == NULL) {
                ...
                result = attempt_dce_login(pamh, dsd, &st, user, firstpass);

                if (result == PAM_SUCCESS || st == sec_login_s_acct_invalid ||
                    use_first_pass) {
                        goto out;
                }
        }
        ...
        if (firstpass == NULL &&
            !(try_first_pass || try_mapped_pass || invalid_user))
                (void)sprintf(messages[0],
                              (const char *)PAM_MSG(pamh, 10, "Password: "));
        else
                (void)sprintf(messages[0], (const char *)PAM_MSG(
                                               pamh, 11, "DCE Password: "));
        ...
        if (try_mapped_pass && result == PAM_SUCCESS && firstpass) {
                ...
                xfn_update_mapped_password(debug ? XFN_MAP_DEBUG : 0, user,
                                           DCE_XFN_PASS_ATTR, firstpass,
                                           password);
...
static int attempt_dce_login(void *pamh, dce_module_data_t *dsd,
        ...
        login_valid = sec_login_valid_and_cert_ident(
            dsd->login_context, &passwd_rec, &dsd->reset_passwd, &dsd->auth_src,
            st);
        ...
        if (*st == sec_login_s_acct_invalid && dsd->warn) {
                ...
                sprintf(messages[0],
                        PAM_MSG(pamh, 12,
                                "Error: Your DCE Account has expired.\n"));
