...
static void do_warn(pam_handle_t *pamh, time_t cur, time_t t, char *desc);

static void do_warn_passwd(pam_handle_t *pamh);
...
int pam_sm_acct_mgmt(pam_handle_t *pamh, int flags, int argc,
        ...
        for (i = 0; i < argc; i++) {
                if (strcmp(argv[i], "debug") == 0)
                        ...
                        syslog(LOG_ERR, "illegal DCE acct_mgmt option %s",
                               argv[i]);
        }
        ...
        if (debug)
                syslog(LOG_DEBUG, "DCE pam_sm_acct_mgmt");
        ...
        if (warn)
                do_warn(pamh, curtime, (time_t)net_info.acct_expiration_date,
                        "account");
