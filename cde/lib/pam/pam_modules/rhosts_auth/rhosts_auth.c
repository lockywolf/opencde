...
int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc,
        ...
        if (debug) {
                syslog(LOG_DEBUG, "rhosts authenticate: user = %s, host = %s",
                       lusername, host);
        }

        if (getpwnam_r(lusername, &pwd, pwd_buffer, sizeof(pwd_buffer)) == NULL)
        ...
        return (ruserok(host, is_superuser, rusername, lusername) == -1
                    ? PAM_AUTH_ERR
                    : PAM_SUCCESS);
}
