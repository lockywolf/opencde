...
#ifdef PAM_NISPLUS
static int get_nispluscred(pam_handle_t *, char *, char *, int, struct passwd *,
                           nis_result **);
#endif

static int verify_old_passwd(pam_handle_t *, int, int, char *, struct spwd *,
                             char *, char *, char *, int, void *, uid_t, int *,
                             int *, int, int);
static int turn_on_default_aging(pam_handle_t *, int, char *, int);
static int get_newpasswd(pam_handle_t *, char *, int, char *, int, uid_t, int,
                         int, int, int);
static int circ(char *, char *);
static int triviality_checks(pam_handle_t *, uid_t, char *, char *, int, int,
                             int);
...
int __update_authtok(pam_handle_t *pamh, int flags, int repository,
                     char *domain, int argc, const char **argv) {
 ...
 */
        ...
        for (j = 1; j <= 3; j++) {
                ...
                retcode =
                    ck_perm(pamh, re, (char *)domain, &pwd, &shpwd, &privileged,
                ...
                if (IS_NISPLUS(repository)) {
                         ...
                         */
                        retcode = get_nispluscred(pamh, domain, prognamep,
                                                  repository, pwd, &cred_res);
                ...
                retcode = verify_old_passwd(
                    pamh, re, repository, (char *)domain, shpwd, opwbuf, orpcpw,
                    curcryptsecret, privileged, (void *)cred_res, uid,
                ...
                if (!newpass) {
                        retcode = get_newpasswd(pamh, pwbuf, re, opwbuf,
                                                privileged, uid, try_first_pass,
                ...
                if (IS_NIS(re) && (pwd != NULL)) {
                        retcode =
                            update_authtok_nis(pamh, "passwd", &pw, opwbuf,
                                               pwbuf, pwd, privileged, nowarn);
                        if (retcode != PAM_SUCCESS) {
                ...
                if (IS_NISPLUS(re) && (pwd != NULL)) {
                        ...
                        retcode = update_authtok_nisplus(
                            pamh, (char *)domain, "passwd", &pw, opwbuf, orpcpw,
                            pwbuf, IS_OPWCMD(re) ? 1 : 0, pwd, curcryptsecret,
                            privileged, passwd_res, cred_res, debug, nowarn);
                        if (retcode != PAM_SUCCESS) {
                                free_passwd_structs(pwd, shpwd);
                                goto out;
                        }
                ...
                if (IS_FILES(re) && (pwd != NULL)) {
                        retcode = update_authtok_file(pamh, "passwd", &pw, pwd,
                                                      privileged, nowarn);
                        if (retcode != PAM_SUCCESS) {
                                free_passwd_structs(pwd, shpwd);
                                goto out;
...
static int verify_old_passwd(pamh, repository, real_rep, domain, shpwd, opwbuf,
                             orpcpw, curcryptsecret, privileged, cred_res, uid,
                             try_first_pass, use_first_pass, debug,
                             nowarn) pam_handle_t *pamh;
...
int nowarn;
{
        ...
        if (debug) {
                syslog(LOG_DEBUG, "verify_old_passwd(): repository is %s",
                       repository_to_string(repository));
                syslog(LOG_DEBUG,
         ...
         */
        if (shpwd->sp_pwdp[0] &&
            ((uid != 0 && IS_FILES(repository)) || IS_NIS(repository) ||
             IS_NISPLUS(repository))) {
                ...
                while (!done) {
                        if (*try_first_pass || *use_first_pass) {
                                ...
                                retcode = __pam_get_authtok(
                                    pamh, PAM_PROMPT, PAM_OLDAUTHTOK,
                                    PASSWORD_LEN, prompt, &pswd);
                                if (retcode != PAM_SUCCESS)
                                        return (retcode);
                        }
                ...
                if ((privileged == 0) && IS_NISPLUS(repository) &&
                    ...
                    ((nis_result *)cred_res)->status == NIS_SUCCESS) {
                         ...
                         * about seeing if it can decrypt the secretkey in cred
                         * table. If it does, attempt to decrypt the old
                         * secretkey with it. This only makes sense when users
                         * have credentials.
                         */
                            ...
                            FALSE) {
                                ...
                                (void)sprintf(messages[0],
                                              PAM_MSG(pamh, 67,
                                                      ...
                                                      "from your secure RPC "
                                                      "password."));
                                (void)__pam_display_msg(pamh, PAM_TEXT_INFO, 1,
                                                        messages, NULL);

                                if ((retcode = __pam_get_authtok(
                                         pamh, PAM_PROMPT, 0, PASSWORD_LEN,
                                         PAM_MSG(pamh, 68,
                                                 "Please enter your old Secure "
                                                 "RPC password: "),
                                         &tmpkeybuf)) != PAM_SUCCESS) {
                                        return (retcode);
                                }

                                if (verify_rpc_passwd(
                                        tmpkeybuf, curcryptsecret) == FALSE) {
                                        (void)sprintf(
                                            messages[0],
                                            PAM_MSG(pamh, 69,
                                                    "This password does not "
                                                    "decrypt your secure RPC "
...
aging:
        ...
        if (shpwd->sp_max != -1 && shpwd->sp_lstchg != 0) {
                ...
                if (shpwd->sp_lstchg == 0 && shpwd->sp_max > 0 ||
                    shpwd->sp_min > 0) {
                        ...
                        case 0: /* child */
                                (void)seteuid(0);
                                retcode = turn_on_default_aging(
                                    pamh, repository, domain, debug);
                                exit(retcode);
                        default:
                                /* wait for child */
...
static int turn_on_default_aging(pamh, repository, domain,
                                 debug) pam_handle_t *pamh;
int repository;
char *domain;
int debug;
{
        ...
        if (debug)
                syslog(LOG_DEBUG, "turn: maxdate == %d, mindate == %d\n",
                       maxdate, mindate);
...
static int get_newpasswd(pamh, pwbuf, re, opwbuf, privileged, uid,
                         try_first_pass, use_first_pass, debug,
                         nowarn) pam_handle_t *pamh;
...
int nowarn;
{
        ...
        while (insist < MAX_CHANCES && count < MAX_CHANCES && !done) {
                ...
                if ((try_first_pass || use_first_pass) && insist == 0 &&
                    count == 0) {
                        /*
                         ...
                         */
                        if ((retcode = __pam_get_authtok(
                                 pamh, PAM_HANDLE, PAM_AUTHTOK, PASSWORD_LEN, 0,
                                 &pswd)) != PAM_SUCCESS) {
                                return (retcode);
                        }
                } else {
                        if ((retcode = __pam_get_authtok(
                                 pamh, PAM_PROMPT, PAM_AUTHTOK, PASSWORD_LEN,
                                 PAM_MSG(pamh, 74, "New password: "), &pswd)) !=
                            PAM_SUCCESS) {
                                return (retcode);
                        }
                }
                ...
                if ((retcode = triviality_checks(pamh, uid, opwbuf, pwbuf, re,
                                                 privileged, nowarn)) !=
                    PAM_SUCCESS) {
                ...
                if (try_first_pass || use_first_pass) {
                         ...
                         */
                        if ((retcode = __pam_get_authtok(
                                 pamh, PAM_HANDLE, PAM_AUTHTOK, PASSWORD_LEN,
                                 NULL, &pswd)) != PAM_SUCCESS) {
                                /* clear bad "new password" if saved */
                                if (passwd_already_set == 0)
                                        pam_set_item(pamh, PAM_AUTHTOK, NULL);
                                return (retcode);
        ...
        if (insist >= MAX_CHANCES) {
                ...
                (void)sprintf(
                    messages[0],
                    PAM_MSG(pamh, 78, "%s%s: Too many failures - try later."),
                    prognamep, UNIX_MSG);
...
static int triviality_checks(pam_handle_t *pamh, uid_t uid, char *opwbuf,
                             char *pwbuf, int re, int privileged, int nowarn) {
        ...
        if (!((uid == 0 && IS_FILES(re)) || (privileged && IS_NISPLUS(re))) &&
            circ(usrname, pwbuf)) {
                if (!nowarn) {
                        (void)sprintf(messages[0],
                                      PAM_MSG(pamh, 80,
                                              "%s%s: Password cannot be "
                                              "circular shift of logonid."),
                                      prognamep, UNIX_MSG);
                        retcode = __pam_display_msg(pamh, PAM_ERROR_MSG, 1,
                                                    messages, NULL);
                        if (retcode != PAM_SUCCESS) {
 ...
 */
static bool_t verify_rpc_passwd(oldpass, curcryptsecret) char *oldpass;
char *curcryptsecret;
{
        ...
        if (xdecrypt(oldsecret, oldpass) &&
            (memcmp(oldsecret, &(oldsecret[HEXKEYBYTES]), KEYCHECKSUMSIZE) ==
             0))
 ...
 */
static int get_nispluscred(pamh, nisdomain, prognamep, repository, nisplus_pwd,
                           cred_res) pam_handle_t *pamh;
char *nisdomain;
char *prognamep;
int repository;
...
nis_result **cred_res;
{
         ...
         */
        if ((50 + strlen(nisdomain) + PKTABLELEN) > (size_t)NIS_MAXNAMELEN) {
        ...
        local_res =
            nis_list(buf, USE_DGRAM + FOLLOW_LINKS + FOLLOW_PATH + MASTER_ONLY,
                     NULL, NULL);
        ...
        if (local_cname == NULL) {
                ...
                if (IS_OPWCMD(repository)) {
                        (void)sprintf(
                            messages[0],
                            PAM_MSG(pamh, 85, "%s: invalid LOCAL credential"),
                            prognamep);
                        (void)__pam_display_msg(pamh, PAM_ERROR_MSG, 1,
                                                messages, NULL);
                        *cred_res = NULL;
         ...
         */
        if ((25 + strlen(local_cname) + strlen(cred_domain) + PKTABLELEN) >
        ...
        (void)sprintf(buf, "[cname=%s,auth_type=DES],%s.%s", local_cname,
                      PKTABLE, cred_domain);
