 ...
 */
bool_t npd_makeclnthandle(domain, clnt, srv_pubkey, keysize) char *domain;
...
int keysize;      /* size of buffer */
{
        ...
        if (srvs == NULL) {
                ...
                syslog(LOG_ERR, "can't get a list of servers for %s domain",
                       domain);
                return (FALSE);
        }
 ...
 * returns TRUE on success and FALSE on failure.
 */
bool_t __get_cmnkey(public, secret, deskey) char *public;
char *secret;
des_block *deskey;
{
        MINT *pub;
 ...
 */
nispasswd_status nispasswd_auth(user, domain, oldpass, u_pubkey, deskey, clnt,
                                ident, randval, err) char *user; /* user name */
char *domain;                                                    /* domain */
...
int *err;                /* error code, returned */
{
        ...
        if ((user == NULL || *user == '\0') ||
            ...
            (deskey == (des_block *)NULL) || (clnt == (CLIENT *)NULL)) {
                *err = NPD_INVALIDARGS;
                return (NPD_FAILED);
        }
        ...
        cryptstat = cbc_crypt((char *)deskey, (char *)xpass, _NPD_PASSMAXLEN,
        ...
        req_arg.user_pub_key.user_pub_key_len = strlen((char *)u_pubkey) + 1;
        ...
        if (clnt_call(clnt, NISPASSWD_AUTHENTICATE, (xdrproc_t)xdr_npd_request,
                      (caddr_t)&req_arg, (xdrproc_t)xdr_nispasswd_authresult,
 ...
 * clear.
 */
int nispasswd_pass(clnt, ident, randval, deskey, newpass, gecos, shell, err,
                   errlst) CLIENT *clnt; /* client handle to rpc.nispasswdd */
...
nispasswd_error **errlst; /* error list on partial success, returned */
{
        ...
        if ((clnt == (CLIENT *)NULL) || (deskey == (des_block *)NULL) ||
        ...
        if (!__npd_cbc_crypt(&randval, newpass, strlen(newpass), &cryptbuf,
                             _NPD_PASSMAXLEN, DES_ENCRYPT, deskey)) {
        ...
        if (clnt_call(clnt, NISPASSWD_UPDATE, (xdrproc_t)xdr_npd_update,
                      (caddr_t)&send_arg, (xdrproc_t)xdr_nispasswd_updresult,
                      (caddr_t)&result, TIMEOUT) != RPC_SUCCESS) {
        ...
        case NPD_PARTIALSUCCESS:
                ...
                *errlst = (nispasswd_error *)calloc(1, sizeof(nispasswd_error));
                ...
                for (p = *errlst; errl != NULL; errl = errl->next) {
                        ...
                        if (errl->next != NULL) {
                                p->next = (nispasswd_error *)calloc(
                                    1, sizeof(nispasswd_error));
                                p = p->next;
                        } else
...
void __npd_free_errlist(list) nispasswd_error *list;
{
        nispasswd_error *p;
