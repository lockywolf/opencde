...
int establish_key(uid, password, reestablish, netnamebuf) uid_t uid;
char *password;
int reestablish;
char *netnamebuf;
{
...
int ck_perm(pamh, repository, domain, pwd, shpwd, privileged, passwd_res, uid,
            debug, nowarn) pam_handle_t *pamh;
int repository;
char *domain;
struct passwd **pwd;
struct spwd **shpwd;
...
int nowarn;
{
        ...
        if (debug)
                syslog(LOG_DEBUG, "ck_perm() called: repository=%s",
        ...
        if (repository == PAM_REP_FILES) {
                ...
                while ((local_pwdp = fgetpwent_r(pwfp, &local_pwd, pwdbuf,
                        ...
                        if (strcmp(local_pwd.pw_name, usrname) == 0)
                                break;
                while ((local_shpwdp = fgetspent_r(spfp, &local_shpwd, shpwdbuf,
                                                   sizeof(shpwdbuf))) != NULL)
                ...
                return (
                    copy_passwd_structs(pwd, local_pwdp, shpwd, local_shpwdp));
        }
        ...
        if (repository == PAM_REP_NISPLUS) {
                ...
                if ((9 + strlen(usrname) + strlen(domain) + PASSTABLELEN) >
                    (size_t)NIS_MAXNAMELEN) {
                        sprintf(messages[0],
                                PAM_MSG(pamh, 140, "%s%s: Permission denied"),
                 ...
                 */
                *passwd_res = (void *)nis_list(
                    buf, USE_DGRAM + FOLLOW_LINKS + FOLLOW_PATH + MASTER_ONLY,
                    NULL, NULL);
                ...
                if (strcmp(nis_leaf_of(pwd_domain), "org_dir") == 0) {
                        pwd_domain = nis_domain_of(
                            NIS_RES_OBJECT((nis_result *)(*passwd_res))
                                ->zo_domain);
                }
                *privileged =
                    __nis_isadmin(local_principal, "passwd", pwd_domain);
...
char *attr_match(s1, s2) register char *s1, *s2;
{
        while (*s1 == *s2++)
                if (*s1++ == '=')
                        return (s2);
...
int attr_find(s1, s2) register char *s1, *s2[];
{
        int i;
        char *sa, *sb;
...
void free_setattr(setattr) char *setattr[];
{
        int i;
...
void setup_attr(dest_attr, k, attr, value) char *dest_attr[];
int k;
char attr[];
char value[];
{
...
#ifdef PAM_NISPLUS
static char *spskip(p) register char *p;
{
        while (*p && *p != ':' && *p != '\n')
                ++p;
        if (*p == '\n')
...
void nisplus_populate_age(enobj, sp) struct nis_object *enobj;
struct spwd *sp;
{
        char *oldage, *p, *end;
        long x;
 ...
 */
char *getloginshell(pamh, oldshell, privileged, nowarn) pam_handle_t *pamh;
char *oldshell;
int privileged;
int nowarn;
{
        ...
        sprintf(messages[0], PAM_MSG(pamh, 144, "Old shell: %s"), oldshell);
        (void)__pam_display_msg(pamh, PAM_TEXT_INFO, 1, messages, NULL);
        sprintf(messages[0], PAM_MSG(pamh, 145, "New shell: "));
        (void)__pam_get_input(pamh, PAM_PROMPT_ECHO_ON, 1, messages, NULL,
                              &ret_resp);
        ...
        if (newshell[0] == '\0' || (strcmp(newshell, oldshell) == 0)) {
                if (!nowarn) {
                        sprintf(messages[0],
                                PAM_MSG(pamh, 146, "Login shell unchanged."));
                        (void)__pam_display_msg(pamh, PAM_ERROR_MSG, 1,
                                                messages, NULL);
                }
                return (NULL);
 ...
 * Get name.
 */
char *getfingerinfo(pamh, old_gecos, nowarn) pam_handle_t *pamh;
char *old_gecos;
int nowarn;
{
        ...
        if (!nowarn) {
                ...
                (void)__pam_display_msg(pamh, PAM_TEXT_INFO, 3, messages, NULL);
        }
        ...
        do {
                sprintf(messages[0], " ");
                (void)__pam_display_msg(pamh, PAM_TEXT_INFO, 1, messages, NULL);
                sprintf(messages[0], PAM_MSG(pamh, 152, "Name [%s]: "),
                        old_gecos);
                (void)__pam_get_input(pamh, PAM_PROMPT_ECHO_ON, 1, messages,
                                      NULL, &ret_resp);
 ...
 * Get Home Dir.
 */
char *gethomedir(pamh, olddir, nowarn) pam_handle_t *pamh;
char *olddir;
int nowarn;
{
        ...
        do {
                ...
                (void)__pam_display_msg(pamh, PAM_TEXT_INFO, 1, messages, NULL);
                sprintf(messages[0],
                        PAM_MSG(pamh, 154, "Home Directory [%s]: "), olddir);
                (void)__pam_get_input(pamh, PAM_PROMPT_ECHO_ON, 1, messages,
                                      NULL, &ret_resp);
...
char *repository_to_string(int repository) {
 ...
 */
static int illegal_input(pamh, input_str, nowarn) pam_handle_t *pamh;
char *input_str;
int nowarn;
{
        char *ptr;
        ...
        if (strchr(input_str, ':')) {
                if (!nowarn) {
                        sprintf(messages[0],
                                PAM_MSG(pamh, 156, "':' is not allowed."));
                        (void)__pam_display_msg(pamh, PAM_ERROR_MSG, 1,
                                                messages, NULL);
                }
                error_flag = 1;
 ...
 *  in these two cases.
 */
static int special_case(str, default_str) char *str, *default_str;
{
...
static int copy_passwd_structs(struct passwd **pwd, struct passwd *local_pwd,
                               struct spwd **shpwd, struct spwd *local_shpwd) {

        /* copy the passwd information */
        if ((*pwd = (struct passwd *)calloc(1, sizeof(struct passwd))) == NULL)
                return (PAM_BUF_ERR);
        ...
        if ((*shpwd = (struct spwd *)calloc(1, sizeof(struct spwd))) == NULL)
                goto out;
        **shpwd = *local_shpwd;
...
void free_passwd_structs(struct passwd *pwd, struct spwd *shpwd) {
        if (pwd) {
                if (pwd->pw_name)
                        free(pwd->pw_name);
                if (pwd->pw_passwd) {
