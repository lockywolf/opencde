...
int pam_sm_setcred(pam_handle_t *pamh, int flags, int argc, const char **argv) {
        ...
        if (flags && !(flags & PAM_ESTABLISH_CRED) &&
            !(flags & PAM_REINITIALIZE_CRED) && !(flags & PAM_REFRESH_CRED) &&
            !(flags & PAM_DELETE_CRED) && !(flags & PAM_SILENT)) {
        ...
        if ((flags & PAM_REINITIALIZE_CRED) || (flags & PAM_REFRESH_CRED)) {
                ...
                if (geteuid() == 0) {
                        if (!(flags & PAM_SILENT)) {
                                ...
                                sprintf(messages[2],
                                        PAM_MSG(pamh, 52,
                                                "root may use keylogout -f to "
                                                "do this (at your own risk)!"));
                                __pam_display_msg(pamh, PAM_ERROR_MSG, 3,
                                                  messages, NULL);
                        }
                        err = PAM_PERM_DENIED;
                        goto out;
