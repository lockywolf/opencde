...
int __get_authtoken_attr(pam_handle_t *pamh, char ***ga_getattr, int repository,
                         const char *domain, int argc, const char **argv) {
        register int k;
        ...
        if (debug)
                syslog(LOG_DEBUG, "__get_authtoken_attr(): repository=%s",
        ...
        if (usrname == NULL || *usrname == NULL) {
                ...
                if (!IS_NISPLUS(repository)) {
                        sprintf(
                            messages[0],
                            PAM_MSG(pamh, 42, "Unable to retrieve username."));
                ...
                while ((psp = _np_getspent()) != NULL) {
                        ...
                        if (psp->sp_pwdp == NULL) {
                                sprintf(messages[0], "%s NP  ", value);
                        } else if ((int)strlen(psp->sp_pwdp) < NUMCP) {
                                sprintf(messages[0], "%s LK  ", value);
                        } else {
                                sprintf(messages[0], "%s PS  ", value);
                        }
                        ...
                        if (psp->sp_max != -1) {
                                ...
                                if (psp->sp_lstchg) {
                                        ...
                                        sprintf(messages[0], "%s 00/00/00  ",
                                                value);

                                strcpy(value, messages[0]);
                                if ((psp->sp_min >= 0) && (psp->sp_warn > 0)) {
                                        sprintf(messages[0], "%s %d  %d  %d ",
                                                value, psp->sp_min, psp->sp_max,
                                                psp->sp_warn);
                                } else if (psp->sp_min >= 0) {
                                        sprintf(messages[0], "%s %d  %d  ",
                                                value, psp->sp_min,
                                                psp->sp_max);
        ...
        retcode =
            ck_perm(pamh, repository, (char *)domain, &pwd, &shpwd, &privileged,
        ...
        if (shpwd->sp_max != -1) {
                ...
                setup_attr(get_attributes, k++, "AUTHTOK_LASTCHANGE=", value);
        ...
        *ga_getattr = &get_attributes[0];
        ...
        /*
         * XXX Our private version of the switch frontend for getspent.  We want
         * to search just the nisplus sp file, so we want to bypass normal
         * nsswitch.conf based processing.  This implementation compatible with
         * version 2 of the name service switch.
         */
...
static void _np_nss_initf_shadow(p) nss_db_params_t *p;
{
...
static struct spwd *_np_getspent_r(result, buffer, buflen) struct spwd *result;
char *buffer;
int buflen;
{
        nss_XbyY_args_t arg;
