 ...
 */
#ifdef PAM_NIS
static void nss_nis_passwd(p) nss_db_params_t *p;
{
...
static void nss_nis_shadow(p) nss_db_params_t *p;
{
...
#ifdef PAM_NISPLUS
static void nss_nisplus_passwd(p) nss_db_params_t *p;
{
        p->name = NSS_DBNAM_PASSWD;
        p->flags |= NSS_USE_DEFAULT_CONFIG;
...
static void nss_nisplus_shadow(p) nss_db_params_t *p;
{
        p->name = NSS_DBNAM_SHADOW;
        p->config_name = NSS_DBNAM_PASSWD; /* Use config for "passwd" */
        p->flags |= NSS_USE_DEFAULT_CONFIG;
...
static char *gettok(nextpp) char **nextpp;
{
        char *p = *nextpp;
        char *q = p;
        char c;
 ...
 */
static int str2passwd(const char *instr, int lenstr, void *ent, char *buffer,
                      int buflen) {
...
static bool_t /* 1 means success and more input, 0 means error or no more */
    getfield(nextp, limit, uns, valp) constp *nextp;
constp limit;
int uns;
void *valp;
{
 ...
 * need be. instring and buffer should be separate areas.
 */
int str2spwd(instr, lenstr, ent, buffer, buflen) const char *instr;
int lenstr;
...
int buflen;
{
        ...
        limit = p + lenstr;
        if ((p = memchr(instr, ':', lenstr)) == 0 || ++p >= limit ||
...
struct passwd *getpwnam_from(name, rep) const char *name;
int rep;
{
...
struct spwd *getspnam_from(name, rep) const char *name;
int rep;
{
...
#ifdef PAM_NIS
        case PAM_REP_NIS:
                nss_search(&spdb_root, nss_nis_shadow, NSS_DBOP_SHADOW_BYNAME,
                           &arg);
                break;
#endif
        default:
...
static void pr_config(pamh) pam_handle_t *pamh;
{
 ...
 */
int get_ns(pamh, o_rep, debug, nowarn) pam_handle_t *pamh;
int o_rep;
int debug;
int nowarn;
{
        ...
        if (conf == NULL) {
                if (!nowarn) {
                        ...
                        __pam_display_msg(pamh, PAM_ERROR_MSG, 1, messages,
                                          NULL);
                }
                ...
                if (IS_NISPLUS(o_rep)) {
                        if (!nowarn) {
                                ...
                                __pam_display_msg(pamh, PAM_ERROR_MSG, 1,
                                                  messages, NULL);
                        }
                        return (-1);
