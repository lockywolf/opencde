...
static int check_for_login_inactivity(struct passwd pwd, struct spwd shpwd) {
        int fdl;
        struct lastlog ll;
        int retval;
        long long offset;
...
static int new_password_check(pwd, shpwd, flags) struct passwd *pwd;
struct spwd *shpwd;
int flags;
{
        int now = DAY_NOW;
...
static int perform_passwd_aging_check(pam_handle_t *pamh, struct spwd shpwd,
                                      int flags) {
        int now = DAY_NOW;
        int Idleweeks = -1;
        char *ptr;
        ...
        if ((shpwd.sp_lstchg == 0) ||
            ((shpwd.sp_max >= 0) && (now > (shpwd.sp_lstchg + shpwd.sp_max)) &&
             (shpwd.sp_max >= shpwd.sp_min))) {
                if ((Idleweeks == 0) ||
                    ((Idleweeks > 0) &&
                        ...
                        if (!(flags & PAM_SILENT)) {
                                ...
                                strcpy(messages[1],
                                       PAM_MSG(pamh, 21,
                                               ...
                                               "administrator"));
                                __pam_display_msg(pamh, PAM_ERROR_MSG, 2,
                                                  messages, NULL);
                        }
...
static void warn_user_passwd_will_expire(pam_handle_t *pamh,
                                         struct spwd shpwd) {
        int now = DAY_NOW;
        char messages[PAM_MAX_NUM_MSG][PAM_MAX_MSG_SIZE];
        int days;
...
int pam_sm_acct_mgmt(pam_handle_t *pamh, int flags, int argc,
         ...
         */
        if (shpwd.sp_expire > 0 && shpwd.sp_expire < DAY_NOW) {
