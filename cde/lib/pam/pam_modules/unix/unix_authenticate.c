...
static void unix_cleanup(pam_handle_t *pamh, void *data, int pam_status) {
...
int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc,
        ...
        if (debug)
                syslog(LOG_DEBUG,
                       "unix pam_sm_authenticate(%s %s), flags = %x ", service,
        ...
        if (try_first_pass) {
                ...
                retcode = attempt_authentication(user, password, shpwd.sp_pwdp,
                                                 pwd.pw_uid, flags, debug);
...
prompt:
        ...
        if ((password != NULL && password[0] != '\0') || try_first_pass) {
                ...
                if ((retcode = __pam_get_authtok(
                         ...
                         PAM_MSG(pamh, 30, "UNIX Password: "), &password)) !=
                    PAM_SUCCESS) {
                        if (err != PAM_USER_UNKNOWN)
                                err = retcode;
                        goto out;
                ...
                if ((retcode = __pam_get_authtok(
                         pamh, PAM_PROMPT, PAM_AUTHTOK, PASSWORD_LEN,
                         PAM_MSG(pamh, 31, "Password: "), &password)) !=
                    PAM_SUCCESS) {
                        if (err != PAM_USER_UNKNOWN)
                                err = retcode;
                        goto out;
...
post_prompt:
         ...
         */
        uid = pwd.pw_uid;
        if (password != NULL && password[0] != '\0' && uid != 0) {
                /*
...
static int attempt_authentication(char *user, char *password, char *enc_passwd,
...
#ifdef PAM_SECURE_RPC
                        estkey_stat = establish_key(uid, password, 1, NULL);
...
credentials to login.",
                        ...
                        memset(pwd_buf, 0, sizeof(pwd_buf));
                        memset(shpwd_buf, 0, sizeof(shpwd_buf));
                        if (getpwnam_r(user, &pwd, pwd_buf, sizeof(pwd_buf)) ==
                                NULL ||
                        ...
                        if ((shpwd.sp_pwdp != 0) &&
                            (strcmp(shpwd.sp_pwdp, "*NP*") == 0)) {
                                syslog(LOG_ALERT, "Permissions on the password "
                                                  "database may be too "
                                                  "restrictive");
                                return (PAM_AUTH_ERR);
                        }
                ...
                if (strcmp(crypt(password, enc_passwd), enc_passwd) != 0) {
                        return (PAM_AUTH_ERR);
                }
                /* success! */
                return (PAM_SUCCESS);
