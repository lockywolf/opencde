...
#ifdef PAM_NISPLUS
static int update_attr(pam_handle_t *, char *, char **, char *, int, char **,
                       char **, struct passwd *, int, int, nis_result *, int);
static int talk_to_npd(pam_handle_t *, char *, char **, char *, char *, char *,
                       char *, struct passwd *, int, int, char **, char **,
                       nis_result *, int, int);
static char *reencrypt_secret(char *, char *, char *);
static nis_error revert2oldpasswd(char *, nis_result *);

int update_authtok_nisplus(pam_handle_t *pamh, char *domain, char *field,
                           ...
                           struct passwd *nisplus_pwd, char *curcryptsecret,
                           int privileged, nis_result *passwd_res,
                           nis_result *cred_res, int debug, int nowarn) {
        ...
        if (opwcmd == FALSE) {
                ...
                if (passwd_res == NULL || passwd_res->status != NIS_SUCCESS) {
                        ...
                        rc = talk_to_npd(pamh, field, data, domain, usrname,
                                         old, new, nisplus_pwd, failover,
                                         privileged, &shell, &gecos, passwd_res,
                                         debug, nowarn);
                } else {
        ...
        if (strcmp(field, "passwd") == 0) {
                ...
                /* same user check? */
                if ((!privileged) &&
                    (newcryptsecret = reencrypt_secret(tmpcryptsecret, oldrpc,
                                                       new)) == NULL) {
                        sprintf(messages[0], " ");
                        sprintf(
                            messages[1],
                ...
                if (privileged) {
                        ...
                        sprintf(messages[1],
                                PAM_MSG(pamh, 113,
                                        "The NIS+ credential information for "
                                        "%s will not be changed."),
                                usrname);
                        sprintf(messages[2],
                                PAM_MSG(pamh, 114,
                                        "User %s must do the following to "
                                        "update his/her"),
                                usrname);
                        sprintf(messages[3],
                                PAM_MSG(pamh, 115, "credential information:"));
                        sprintf(messages[4],
                                PAM_MSG(pamh, 116,
                                        "Use NEW passwd for login and OLD "
                                        "passwd for keylogin."));
                        sprintf(messages[5],
                                PAM_MSG(pamh, 117,
                                        "Use \"chkey -p\" to reencrypt the "
                                        "credentials with the"));
                        sprintf(messages[6],
                                PAM_MSG(pamh, 118, "new login passwd."));
                        sprintf(messages[7],
                                PAM_MSG(pamh, 119,
                                        "The user must keylogin explicitly "
                ...
                if (mres->status != NIS_SUCCESS) {
                        ...
                        if (niserr != NIS_SUCCESS) {
                                ...
                                sprintf(messages[1],
                                        PAM_MSG(pamh, 120,
                                                "WARNING: Could not reencrypt "
                                                "NIS+ credentials for %s;"),
                                        usrname);
                                sprintf(messages[2],
                                        PAM_MSG(pamh, 121,
                                                "login and keylogin passwords "
                                                "differ."));
                                sprintf(messages[3],
                                        PAM_MSG(pamh, 122,
                                                "Use NEW passwd for login and "
                                                "OLD passwd for keylogin."));
                                sprintf(messages[4],
                                        PAM_MSG(pamh, 117,
                                                "Use \"chkey -p\" to reencrypt "
                                ...
                                sprintf(messages[6],
                                        PAM_MSG(pamh, 123,
                                                "You must keylogin explicitly "
                                                "after your next login."));
                                sprintf(messages[7], "");
                                (void)__pam_display_msg(pamh, PAM_ERROR_MSG, 8,
                                                        messages, NULL);
                        ...
                        sprintf(messages[1],
                                PAM_MSG(pamh, 125,
                                        "Reason: failed to update the cred "
                                        "table with reencrypted credentials."));
                        sprintf(
                            messages[2],
                            PAM_MSG(
                                pamh, 126,
 ...
 * passwd daemon.
 */
static int talk_to_npd(pam_handle_t *pamh, char *field, char **data,
                       char *domain, char *user, char *oldpass, char *newpass,
                       struct passwd *nisplus_pwd, int failover, int privileged,
                       char **shell, char **gecos, nis_result *passwd_res,
                       int debug, int nowarn) {
        CLIENT *clnt = NULL;
         ...
         */
        retcode =
            update_attr(pamh, field, data, NULL, 0, NULL, NULL, nisplus_pwd,
                        failover, privileged, passwd_res, nowarn);
        ...
        retcode =
            update_attr(pamh, field, data, user, 0, shell, gecos, nisplus_pwd,
                        failover, privileged, passwd_res, nowarn);
        if (retcode != PAM_SUCCESS)
                goto out;
...
again:
        ...
        if (status == NPD_FAILED) {
                switch (error) {
                case NPD_NOTMASTER:
                        syslog(LOG_ALERT, "Password update daemon is not "
                                          "running with NIS+ master server");
                        retcode = PAM_AUTHTOK_RECOVERY_ERR;
                        goto out;
                case NPD_SYSTEMERR:
                ...
                case NPD_CKGENFAILED:
                        syslog(LOG_ALERT, "Couldn't generate a common DES key");
                        retcode = PAM_AUTHTOK_RECOVERY_ERR;
                        goto out;
                case NPD_NOPASSWD:
        ...
        if (status == NPD_TRYAGAIN) {
                ...
                if (debug)
                        syslog(LOG_DEBUG,
                               "status=tryagain; ident=%ld, randval=%ld", ident,
                               randval);
                ...
                sprintf(
                    messages[0],
                    PAM_MSG(pamh, 127, "NIS+ Password incorrect: try again"));
                (void)__pam_display_msg(pamh, PAM_ERROR_MSG, 1, messages, NULL);
        ...
        if (status == NPD_SUCCESS) {
                ...
                if (debug)
                        syslog(LOG_DEBUG,
                               "status=success; ident=%ld, randval=%ld", ident,
                               randval);
                if (newpass == NULL) {
                        /*
                         * This is possible from unix_set_authtokattr().
                ...
                status = nispasswd_pass(clnt, ident, randval, &deskey, newpass,
                                        *gecos, *shell, &error, &errlist);
...
static int update_attr(pam_handle_t *pamh, char *field, char **data,
                       char *usrname, int opwcmd, char **sh_p, char **gecos_p,
                       struct passwd *nisplus_pwd, int failover, int privileged,
        ...
        if (strcmp(field, "attr") == 0) {
                ...
                if (flag && opwcmd) {
                        ...
                        if (sp.sp_expire == -1) {
                                sprintf(shadow, "%ld:%ld:%ld:%ld:%ld::%lu",
                                        sp.sp_lstchg, sp.sp_min, sp.sp_max,
                                        sp.sp_warn, sp.sp_inact, sp.sp_flag);
                        } else {
                                sprintf(shadow, "%ld:%ld:%ld:%ld:%ld:%ld:%lu",
                                        sp.sp_lstchg, sp.sp_min, sp.sp_max,
                                        sp.sp_warn, sp.sp_inact, sp.sp_expire,
                                        sp.sp_flag);
                        }
                        ecol[7].ec_value.ec_value_val = shadow;
                        ecol[7].ec_value.ec_value_len = strlen(shadow) + 1;
                ...
                if (opwcmd || failover) {
                        eobj =
                            nis_clone_object(NIS_RES_OBJECT(passwd_res), NULL);
                        if (eobj == NULL) {
                        ...
                        if (mres->status != NIS_SUCCESS) {
                                sprintf(messages[0],
                                        PAM_MSG(pamh, 133,
                                                "NIS+ password information "
                                                "update failed (update_attr)"));
                                (void)__pam_display_msg(pamh, PAM_ERROR_MSG, 1,
                                                        messages, NULL);
 ...
 */
static char *reencrypt_secret(char *oldsecret, char *oldpass, char *newpass) {
 ...
 */
static nis_error revert2oldpasswd(char *usrname, nis_result *passwd_res) {
        entry_col ecol[8];
        nis_object *eobj;
        nis_result *mres;
        char mname[NIS_MAXNAMELEN];
