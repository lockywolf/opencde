...
static int update_spent(pam_handle_t *, char *, char **, struct passwd *,
                        struct spwd *, int *, int, int);
static int process_passwd(pam_handle_t *, char *, char *, struct passwd *, int);
...
int update_authtok_file(pamh, field, data, unix_pwd, privileged,
...
int nowarn;
{
        ...
        if (lckpwdf() != 0) {
                if (!nowarn) {
                        sprintf(messages[0],
                                PAM_MSG(pamh, 90,
                                        "%s%s: Password database busy. Try "
                                        "again later."),
                                prognamep, UNIX_MSG);
        ...
        if (stat(SHADOW, &buf) < 0) {
                syslog(LOG_ERR, "%s: stat of shadow file failed", prognamep);
                (void)ulckpwdf();
        ...
        if ((tsfp = fopen(SHADTEMP, "w")) == NULL) {
                if (!nowarn) {
                        sprintf(messages[0],
                                PAM_MSG(pamh, 91,
                                        "%s%s: Unexpected failure. Password "
                                        "database unchanged."),
                                prognamep, UNIX_MSG);
                        (void)__pam_display_msg(pamh, PAM_ERROR_MSG, 1,
                                                messages, NULL);
        ...
        while (fgetspent_r(spfp, &unix_sp, spbuf, sizeof(spbuf)) != NULL) {
                if (strcmp(unix_sp.sp_namp, usrname) == 0) {
                        ...
                        if (passwd_flag) {
                                ...
                                if ((retcode = process_passwd(
                                         pamh, prognamep, usrname, unix_pwd,
                                         nowarn)) != PAM_SUCCESS) {
                                        fclose(tsfp);
                                        fclose(spfp);
                                        goto err;
                ...
                if (putspent(&unix_sp, tsfp) != 0) {
                        if (!nowarn) {
                                sprintf(messages[0],
                                        PAM_MSG(pamh, 91,
                                                "%s%s: Unexpected failure. "
                                                "Password database unchanged."),
                                        prognamep, UNIX_MSG);
                                (void)__pam_display_msg(pamh, PAM_ERROR_MSG, 1,
                                                        messages, NULL);
                        }
...
static int update_spent(pamh, field, data, unix_pwd, unix_sp, passwd_flag,
                        privileged, nowarn) pam_handle_t *pamh;
char *field;
char **data;
struct passwd *unix_pwd;
struct spwd *unix_sp;
...
int nowarn;
{
        ...
        if (strcmp(field, "attr") == 0) {
                while (*data != NULL) {
                        ...
                        if ((value = attr_match("AUTHTOK_DEL", *data)) !=
                            NULL) {
                                if (strcmp(value, "1") == 0) {
                                         ...
                                         */
                                        if (attr_find("AUTHTOK_EXP", data_p) ==
                                            0)
                        ...
                        if ((value = attr_match("AUTHTOK_HOMEDIR", *data)) !=
                            NULL) {
                                ...
                                unix_pwd->pw_dir =
                                    gethomedir(pamh, unix_pwd->pw_dir, nowarn);
                                if (tmp_pwd_entry)
                                        free(tmp_pwd_entry);
                        ...
                        if ((value = attr_match("AUTHTOK_GECOS", *data)) !=
                            NULL) {
                                ...
                                unix_pwd->pw_gecos = getfingerinfo(
                                    pamh, unix_pwd->pw_gecos, nowarn);
                                if (tmp_pwd_entry)
                                        free(tmp_pwd_entry);
 ...
 * after shadow file.
 */
static int process_passwd(pamh, prognamep, usrname, unix_pwd,
                          nowarn) pam_handle_t *pamh;
char *prognamep;
char *usrname;
struct passwd *unix_pwd;
int nowarn;
{
        ...
        while ((unix_p = fgetpwent_r(pwfp, &unix_tmp, buf, 4 * BUFSIZ)) !=
               NULL) {
        ...
        if (unlink(OPASSWD) && access(OPASSWD, 0) == 0) {
                if (!nowarn) {
                        sprintf(messages[0], PAM_MSG(pamh, 91, "%s%s: %s"),
                                prognamep, UNIX_MSG);
                        (void)__pam_display_msg(pamh, PAM_ERROR_MSG, 1,
                                                messages, NULL);
                }
