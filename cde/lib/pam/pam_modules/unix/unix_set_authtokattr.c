...
int __set_authtoken_attr(pam_handle_t *pamh, const char **sa_setattr,
                         int repository, const char *domain, int argc,
                         const char **argv) {
        register int i;
        int retcode;
        char *usrname;
        ...
        if (IS_NIS(repository) && (pwd != NULL)) {
                retcode =
                    update_authtok_nis(pamh, "attr", (char **)sa_setattr, NULL,
                                       NULL, pwd, privileged, nowarn);
            ...
            if (IS_NISPLUS(repository) && (pwd != NULL)) {
                ...
                retcode = update_authtok_nisplus(
                    pamh, (char *)domain, "attr", (char **)sa_setattr, NULL,
                    NULL, NULL, IS_OPWCMD(repository) ? 1 : 0, pwd, NULL,
                    privileged, (nis_result *)passwd_res, NULL, debug, nowarn);
                free_passwd_structs(pwd, shpwd);
                return (retcode);
        } else
#endif
            if (IS_FILES(repository) && (pwd != NULL)) {
                retcode = update_authtok_file(pamh, "attr", (char **)sa_setattr,
