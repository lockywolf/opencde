...
static void reencrypt_secret(pam_handle_t *, char *, char *, char *, int);
static int update_nisattr(pam_handle_t *, char *, char **, struct passwd *, int,
                          int);

int update_authtok_nis(pam_handle_t *pamh, char *field,
                                   ...
                                   /* take the same protocol */
{
        ...
        if (strcmp(field, "passwd") == 0) {
                ...
                if ((retcode = update_nisattr(pamh, field, data, nis_pwd,
                                              privileged, nowarn)) !=
                    PAM_SUCCESS) {
        ...
        if (yp_get_default_domain(&domain) != 0) {
                syslog(LOG_ERR, "%s%s: can't get domain", prognamep, NIS_MSG);
        ...
        if (client == NULL) {
                syslog(LOG_ERR, "%s%s: couldn't create client to YP master",
                       prognamep, NIS_MSG);
                retcode = PAM_SYSTEM_ERR;
                goto out;
        }
        ...
        sprintf(messages[0],
                PAM_MSG(pamh, 102, "NIS(YP) passwd/attributes changed on %s"),
                master);
...
static int update_nisattr(pam_handle_t *pamh, char *field, char **data,
                ...
                while (*data != NULL) {
                        ...
                        if ((value = attr_match("AUTHTOK_GECOS", *data)) !=
                            NULL) {
                                ...
                                nis_pwd->pw_gecos = getfingerinfo(
                                    pamh, nis_pwd->pw_gecos, nowarn);
 ...
 * Otherwise, be quiet.
 */
static void reencrypt_secret(pam_handle_t *pamh, char *domain, char *oldpass,
        ...
        if (yp_update(domain, PKMAP, YPOP_STORE, who, strlen(who), pkent,
                      strlen(pkent)) != 0) {
        ...
        sprintf(messages[0],
                PAM_MSG(pamh, 107, "%s: secret key reencrypted for %s on %s"),
                NIS_MSG, who, master);
