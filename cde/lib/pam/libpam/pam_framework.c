...
int pam_start(const char *service, const char *user,
        ...
        if (pam_debug)
                syslog(LOG_DEBUG, "pam_start(%s %s)", service,
                       (user) ? user : "no-user");
...
int pam_get_user(pam_handle_t *pamh,          /* PAM handle */
                 char **user,                 /* User Name */
                 const char *prompt_override) /* Prompt */
{
        ...
        for (;;) {
                status = __pam_get_input(pamh, PAM_PROMPT_ECHO_ON, 1, messages,
                                         NULL, &ret_resp);
 ...
 */
pam_set_data(pam_handle_t *pamh,           /* PAM handle */
...
char *pam_strerror(pam_handle_t *pamh, int errnum) {
        if (errnum < 0 || errnum >= PAM_TOTAL_ERRNUM)
                ...
                return (
                    PAM_MSG(pamh, errnum, (char *)pam_error_strings[errnum]));
}
...
int pam_authenticate(pam_handle_t *pamh, int flags) {
        ...
        if ((error = load_modules(pamh, PAM_AUTH_MODULE,
        ...
        while (modulep) {
                ...
                if (authp && authp->pam_sm_authenticate) {
                        error = authp->pam_sm_authenticate(
                        ...
                        default:
                                ...
                                syslog(LOG_DEBUG, "pam_authenticate: error %s",
        ...
        while (modulep) {
                ...
                if (authp && authp->pam_sm_setcred) {
                        ...
                        default:
                                ...
                                syslog(LOG_DEBUG, "pam_setcred: error %s",
                                       pam_strerror(pamh, error));
                                break;
                        }
                }
...
int pam_acct_mgmt(pam_handle_t *pamh, int flags) {
        ...
        if ((error = load_modules(pamh, PAM_ACCOUNT_MODULE,
                                  PAM_SM_ACCT_MGMT)) != PAM_SUCCESS) {
                if (pam_debug)
                        syslog(LOG_DEBUG, "pam_acct_mgmt: load_modules failed");
                return (error);
        }
        ...
        while (modulep) {
                ...
                if (accountp && accountp->pam_sm_acct_mgmt) {
                        error = accountp->pam_sm_acct_mgmt(
                            pamh, flags, modulep->module_argc,
                            (const char **)modulep->module_argv);
                        ...
                        default:
                                ...
                                syslog(LOG_DEBUG, "pam_acct_mgmt: error %s",
                                       pam_strerror(pamh, error));
                                break;
                        }
                }
...
int pam_open_session(pam_handle_t *pamh, int flags) {
        ...
        while (modulep) {
                ...
                if (sessionp && sessionp->pam_sm_open_session) {
                        error = sessionp->pam_sm_open_session(
                            pamh, flags, modulep->module_argc,
                            (const char **)modulep->module_argv);
                        ...
                        default:
                                ...
                                syslog(LOG_DEBUG, "pam_open_session: error %s",
                                       pam_strerror(pamh, error));
                                break;
                        }
                }
        ...
        while (modulep) {
                ...
                if (sessionp && sessionp->pam_sm_close_session) {
                        ...
                        default:
                                ...
                                syslog(LOG_DEBUG, "pam_close_session: error %s",
                                       pam_strerror(pamh, error));
                                break;
                        }
                }
...
int pam_chauthtok(pam_handle_t *pamh, int flags) {
        ...
        if ((error = load_modules(pamh, PAM_PASSWORD_MODULE,
                                  PAM_SM_CHAUTHTOK)) != PAM_SUCCESS) {
                if (pam_debug)
                        syslog(LOG_DEBUG, "pam_chauthtok: load_modules failed");
                return (error);
        }
        ...
        for (i = 1; i <= 2; i++) {
                ...
                while (modulep) {
                        passwdp =
                            (struct password_module *)(modulep->function_ptr);
                        if (passwdp && passwdp->pam_sm_chauthtok) {
                                error = passwdp->pam_sm_chauthtok(
                                    pamh, sm_flags, modulep->module_argc,
                                    (const char **)modulep->module_argv);
                                            ...
                                            PAM_SUFFICIENT) {
                                                if (!(flags &
                                                    ...
                                                    !required_module_failed) {
                                                        /*
                                                         * If updating
                                                         * passwords, the module
                                                         * is sufficient, and no
                                                         * required modules have
                                                         * failed, then memset
                                                         * the passwd memory to
                                                         * 0 and return
                                                         */
                                                        pam_set_item(
                                                            pamh, PAM_AUTHTOK,
...
static int load_modules(pam_handle_t *pamh, int type, char *function_name) {
        ...
        while (pam_entry != NULL) {
                ...
                case PAM_AUTH_MODULE:
                        ...
                        loading_functions = 1;
                        authp = (struct auth_module *)calloc(
                            1, sizeof(struct auth_module));
                        if (authp == NULL)
                                return (PAM_BUF_ERR);

                        /* if open_module fails, continue */
                        if ((mh = open_module(pam_entry->module_path)) == NULL)
                                syslog(LOG_ERR,
                ...
                case PAM_ACCOUNT_MODULE:
                        ...
                        loading_functions = 1;
                        accountp = (struct account_module *)calloc(
                            1, sizeof(struct account_module));
                        if (accountp == NULL)
                                return (PAM_BUF_ERR);
                ...
                case PAM_SESSION_MODULE:
                        ...
                        loading_functions = 1;
                        sessionp = (struct session_module *)calloc(
                            1, sizeof(struct session_module));
                        if (sessionp == NULL)
                                return (PAM_BUF_ERR);
                ...
                case PAM_PASSWORD_MODULE:
                        ...
                        loading_functions = 1;
                        passwdp = (struct password_module *)calloc(
                            1, sizeof(struct password_module));
                        if (passwdp == NULL)
                                return (PAM_BUF_ERR);
                ...
                default:
                        if (pam_debug) {
                                syslog(LOG_DEBUG,
                                       "load_modules: unsupported type %d",
                                       type);
                        }
                        break;
                }
...
#ifdef sun
static void *open_module(char *module_so) {
#endif
#ifdef hpV4
        static shl_t open_module(char *module_so) {
#endif
                struct stat stb;
                char *errmsg;
#ifdef sun
                ...
                if (stb.st_mode & S_IWOTH) {
                        syslog(LOG_ALERT,
                               "open_module: module %s writable by world",
                               module_so);
                        return (NULL);
                }
 ...
 */
#ifdef sun
        static int load_function(void *lfd, char *name, int (**func)()) {
#endif
#ifdef hpV4
                static int load_function(shl_t lfd, char *name,
                                         int (**func)()) {
#endif
                        char *errmsg = NULL;
                        ...
                        if (lfd == NULL)
                                ...
                                /*
                                 * The APIs for opening the shared objects are
                                 * palatform dependent and hence the ifdef
                                 * platform
                                 */
#ifdef sun
                        *func = (int (*)())dlsym(lfd, name);
                        ...
                        stat =
                            shl_findsym(&lfd, name, TYPE_PROCEDURE, proc_addr);

                        *func = (int (*)())proc_addr;

                        if (stat) {
                                if (pam_debug) {
                                        strerror_r(errno, errmsg,
                                                   MAX_ERRMESSAGE_LENGTH);
                                        syslog(
                                            LOG_DEBUG,
                                            "shl_findsym failed %s: error %s",
                        ...
                        if (pam_debug) {
                                syslog(LOG_DEBUG,
                                       "load_function: successful load of %s",
                                       name);
                        }
                        return (PAM_SUCCESS);
                }
                ...
                static int open_pam_conf(struct pam_fh * *pam_fh) {
                        struct stat stb;
                        ...
                        if (stb.st_uid != (uid_t)0) {
                                syslog(LOG_ALERT,
                                       "open_pam_conf: Owner of %s is not root",
                                       PAM_CONFIG);
                                return (0);
                        }
                        ...
                        if (stb.st_mode & S_IWGRP) {
                                syslog(LOG_ALERT,
                                       "open_pam_conf: %s writable by group",
                                       PAM_CONFIG);
                                return (0);
                        }
                        if (stb.st_mode & S_IWOTH) {
                                syslog(LOG_ALERT,
                                       "open_pam_conf: %s writable by world",
                                       PAM_CONFIG);
                                return (0);
                        }
                ...
                static void close_pam_conf(struct pam_fh * pam_fh) {
                ...
                static int read_pam_conf(pam_handle_t * pamh) {
                        ...
                        if ((error = pam_get_item(pamh, PAM_SERVICE,
                                                  (void **)&service)) !=
                            PAM_SUCCESS)
                                goto out;
                        ...
                        while (get_pam_conf_entry(pam_fh, &pamentp)) {
                                ...
                                if (other_needed[pamentp->pam_type] == 1 &&
                                    !strcasecmp(pamentp->pam_service,
                                                service)) {

                                        /*
                                         * We found a service match.  Purge the
                                         * "other" entries we have stored.
                                         */
                                        new_head =
                                            pamh->pam_conf_info[pamentp
                                                                    ->pam_type];
                                        pament_traverse = new_head;
                                        while (
                                            new_head != NULL &&
                                            !strcasecmp(new_head->pam_service,
                                                        "other")) {
                                        ...
                                        /* "other" entries no longer needed for
                                         * this type */
                ...
                static int get_pam_conf_entry(struct pam_fh * pam_fh,
                                              pamtab * *pam) {
                        char *cp, *arg;
                        int argc;
                        char *tmp, *tmp_free;
                        ...
                        while (1) {
                                if ((*pam = (pamtab *)calloc(
                                         1, sizeof(pamtab))) == NULL)
                                        return (0);
                                ...
                                if (strcasecmp(arg, "required") == 0)
                                        ...
                                        /* error */
                                        syslog(LOG_CRIT, "invalid flag: %s",
                                               arg);
                                        free_pamconf(*pam);
                                        continue;
                                }
                ...
                /*
                 * read_next_token - skip tab and space characters and return
                 * the next token
                 */

                static char *read_next_token(char **cpp) {
                ...
                static char *nextline(struct pam_fh * pam_fh) {
                        char *cp;
                        char *ll;
                         ...
                         */
                        while (
                            (ll = fgets(pam_fh->line, sizeof(pam_fh->line),
                                        pam_fh->fconfig)) &&
                            (*(pam_fh->line) == '\n' || *(pam_fh->line) == '#'))
                                ;
                ...
                static void clean_up(pam_handle_t * pamh) {
                        int i;
                ...
                static void free_pamconf(pamtab * cp) {
                        int i;
                ...
                /*
                 * free_pam_conf_info - free memory used to store all pam.conf
                 *info under the pam handle
                 */

                static void free_pam_conf_info(pam_handle_t * pamh) {
