...
void __pam_free_resp(int num_msg, struct pam_response *resp) {
        int i;
...
int __pam_display_msg(pamh, msg_style, num_msg, messages,
                      conv_apdp) pam_handle_t *pamh;
int msg_style;
int num_msg;
...
int __pam_get_input(pamh, msg_style, num_msg, messages, conv_apdp,
                    ret_respp) pam_handle_t *pamh;
int msg_style;
int num_msg;
char messages[PAM_MAX_NUM_MSG][PAM_MAX_MSG_SIZE];
void *conv_apdp;
...
static int __pam_input_output(pamh, msg_style, num_msg, messages, conv_apdp,
                              ret_respp) pam_handle_t *pamh;
int msg_style;
int num_msg;
char messages[PAM_MAX_NUM_MSG][PAM_MAX_MSG_SIZE];
...
struct pam_response **ret_respp;
{
        ...
        msg = (struct pam_message *)calloc(num_msg, sizeof(struct pam_message));
        if (msg == NULL) {
                return (PAM_BUF_ERR);
        }
        m = msg;
 ...
 *			already set.
 */
int __pam_get_authtok(pam_handle_t *pamh, int source, int type, int len,
        ...
        case PAM_HANDLE:
                ...
                case PAM_OLDAUTHTOK:
                        ...
                        break;
                default:
                        syslog(LOG_ERR, "__pam_get_authtok() invalid type: %d",
                               type);
                        free(*authtok);
                        *authtok = NULL;
        ...
        case PAM_PROMPT:
                ...
                if ((error = __pam_get_input(pamh, PAM_PROMPT_ECHO_OFF, 1,
                ...
                break;
        default:
                syslog(LOG_ERR, "__pam_get_authtok() invalid source: %d",
                       source);
                free(*authtok);
                *authtok = NULL;
                return (PAM_SYMBOL_ERR);
...
static void __pam_msg_cleanup(pam_handle_t *pamh, void *data, int pam_status) {
...
static void __pam_msg_destroy(void *tsd) {
        if (tsd)
                free((char *)tsd);
}

static char *__pam_thread_backup(char *msg) {
        char *data_buffer = 0;
...
char *__pam_get_i18n_msg(pam_handle_t *pamh, char *filename, int set, int n,
                         char *s) {
        char *msg;
        char *output_msg;
        nl_catd nlmsg_fd;
        ...
        if (pamh != NULL) {
                ...
                if (status == PAM_NO_MODULE_DATA) {
                        ...
                        if ((msg_data = (_pam_msg_data *)calloc(
                                 1, sizeof(_pam_msg_data))) == NULL) {
                        ...
                        pam_set_data(pamh, filename, msg_data,
                                     __pam_msg_cleanup);
...
int __setutmp_mgmt(char *user,  /* user */
                   char *ttyn,  /* ttyn */
 ...
 */
int __reset_utmp_mgmt(char **user,  /* user */
                      char **ttyn,  /* tty name */
...
int __setutmp_mgmt(char *user, char *ttyn, char *rhost, int flags, int type,
                   char id[]) {
        int tmplen;
...
int __reset_utmp_mgmt(char **user, char **ttyn, char **rhost, int flags,
                      int status, char id[]) {
        struct utmpx *up;
        struct utmpx ut;
        int err = 0;
...
int __setproc_cred(char *user, int flags, uid_t uid, gid_t gid, int ngroups,
                   gid_t *grouplist) {
        ...
        if (flags & __SETEGID) {
                if (setegid(gid) == -1) {
                        sprintf(messages[0],
                                PAM_MSG(NULL, 29, "pam_sm_setcred: %s\n"),
                                strerror(errno));
                        return (__BAD_GID);
                }
        }
        ...
        if (flags & __SETGID) {
                if (setgid(gid) == -1) {
                        sprintf(messages[0],
                                PAM_MSG(NULL, 30, "setproc_cred: %s\n"),
                                strerror(errno));
                        return (__BAD_GID);
                }
        }
