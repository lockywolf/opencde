...
void _DtEditorResetUndo(DtEditorWidget editor) {
        ...
        if (M_insertionLength(pPriv) != 0) {
                ...
                if ((M_deletedText(pPriv) != (char *)NULL) &&
                    (strlen(M_deletedText(pPriv)) != 0)) {
                        /*
                         * If there is a current deletion, save it before
                         * removing the insertion so we can put it back (i.e.
                         * undo the deletion).
                         */
                         ...
                         */
                        XmTextInsert(M_text(pPriv), oldDeleteStart,
                                     oldDeletion);
                ...
                XmTextInsert(M_text(pPriv), M_deletionStart(pPriv),
                             oldDeletion);
 ...
 */
static void SetUndoDeletionState(DtEditorWidget pPriv,
        ...
        pDeletedText = (char *)_XmStringSourceGetString(
            (XmTextWidget)M_text(pPriv), cb->startPos, cb->endPos, False);
        ...
        if (M_deletedText(pPriv) != (char *)NULL &&
            ...
            (cb->startPos == M_deletionStart(pPriv) ||
             cb->endPos == M_deletionStart(pPriv))) {
                /*
                 * Continuation of the current deletion.  For a continuation,
                 * there must have been no intervening insertions, and we must
                 * be deleting from the same point, either forward or backward.
                 */
 ...
 * first one).
 */
static void SetUndoInsertionState(DtEditorWidget pPriv,
                                  XmTextVerifyCallbackStruct *cb) {
        if (M_insertionLength(pPriv) == 0) {
                /*
                ...
                M_insertStart(pPriv) = cb->startPos;
                M_insertionLength(pPriv) =
                    _DtEditor_CountCharacters(cb->text->ptr, cb->text->length);
        } else {
                /*
                ...
                if (cb->startPos ==
                    (M_insertStart(pPriv) + M_insertionLength(pPriv)))
                        ...
                        /*
                         * We're starting a new insertion context, so invalidate
                         * any existing deletion context, and reset the
                         * insertion context.
                         */
                        _DtEditorResetUndo(pPriv);
                        M_insertStart(pPriv) = cb->startPos;
...
/* ARGSUSED */
void _DtEditorModifyVerifyCB(Widget w, caddr_t client_data, caddr_t call_data) {
        ...
        if (M_loadingAllNewData(editor) == True) {
                ...
                /*
                 * Adding additional data, rather than replacing all of the
                 * contents.
                 *
                 * Mark that the contents have been modified since the last time
                 * the application requested a copy.
                 */
                M_unreadChanges(editor) = True;

                /*
                 * First, account for any data which will be removed by the new
                 * data. If text is being deleted, then grab a copy for later
                 * undo's.
                 */
                ...
                /*
                 * If text is being inserted, then change the undo insertion
                 * state.
                 */
                if (cb->text->length > 0)
