...
static DtEditorErrorCode Check4EnoughMemory(int numBytes);

static DtEditorErrorCode StripEmbeddedNulls(char *stringData, int *length);

static DtEditorErrorCode LoadFile(Widget w, char *fileName,
 ...
 */
static int strcasecmp(s1, s2) register char *s1, *s2;
{
        register int c1, c2;
...
static DtEditorErrorCode Check4EnoughMemory(int numBytes) {
        ...
        if (numBytes > 0) {
                char *tmpString =
                    (char *)malloc((2 * numBytes) + (numBytes / 10));
...
static DtEditorErrorCode StripEmbeddedNulls(char *stringData, int *length) {
        ...
        if (strlen(stringData) != *length) {
                ...
                while ((firstNull = strlen(stringData)) != *length) {
                        ...
                        memcpy(&stringData[firstNull],
                               &stringData[lastNull + 1], *length - lastNull);
...
void DtEditorSetInsertionPosition(Widget widget, XmTextPosition position) {
        DtEditorWidget editor = (DtEditorWidget)widget;
        _DtWidgetToAppContext(widget);
        _DtAppLock(app);
...
static DtEditorErrorCode setStringValue(DtEditorWidget editor, char *data) {
        /*
...
static DtEditorErrorCode setDataValue(DtEditorWidget widget, void *rawData,
                                      int length) {
        ...
        if (rawData != (void *)NULL) {
                ...
                /*
                 * Check to see if we have a valid buffer size & enough memory
                 * to load the buffer into the text widget.  This is only an
                 * estimate of our needs. Check4EnoughMemory() returns
                 * DtEDITOR_NO_ERRORS, DtEDITOR_ILLEGAL_SIZE, or
                 * DtEDITOR_INSUFFICIENT_MEMORY.
                 */
                ...
                if (status == DtEDITOR_NO_ERRORS) {
                        ...
                        /*
                         * Convert the data buffer into a string & insert into
                         * the widget
                         */
                        ...
                        /*
                         * Strip out any embedded NULLs because the text widget
                         * will only accept data up to the first NULL.
                         *
...
static DtEditorErrorCode setWcharValue(DtEditorWidget editor, wchar_t *data) {
...
static DtEditorErrorCode insertStringValue(DtEditorWidget editor, char *data,
        ...
        case INSERT_DATA: {
                beginInsert = endInsert =
                    XmTextGetInsertionPosition(M_text(editor));
                break;
        }
...
static DtEditorErrorCode insertDataValue(DtEditorWidget widget, void *rawData,
                                         int length,
                                         LoadActionType typeOfInsert,
                                         XmTextPosition beginInsert,
                                         XmTextPosition endInsert) {
        ...
        if (rawData != (void *)NULL) {
                ...
                /*
                 * Check to see if we have a valid buffer size & enough memory
                 * to insert the buffer into the text widget.  This is only an
                 * estimate of our needs. status will be set to
                 * DtEDITOR_NO_ERRORS, DtEDITOR_ILLEGAL_SIZE, or
                 * DtEDITOR_INSUFFICIENT_MEMORY.
                 */
                status = Check4EnoughMemory(length);
                if (status == DtEDITOR_NO_ERRORS) {
                         ...
                         */
                        loadError =
                            insertStringValue(widget, textData, typeOfInsert,
                                              beginInsert, endInsert);
...
static DtEditorErrorCode insertWcharValue(DtEditorWidget editor, wchar_t *data,
                                          LoadActionType typeOfInsert,
                                          XmTextPosition beginInsert,
                                          XmTextPosition endInsert) {
        ...
        status = insertStringValue(editor, mb_value, typeOfInsert, beginInsert,
                                   endInsert);
        XtFree(mb_value);
...
extern DtEditorErrorCode DtEditorSetContents(Widget widget,
        ...
        default: { error = DtEDITOR_INVALID_TYPE; }
        } /* end switch */
...
extern DtEditorErrorCode DtEditorSetContentsFromFile(Widget widget,
                                                     char *fileName) {
...
extern DtEditorErrorCode DtEditorAppend(Widget widget,
                                        DtEditorContentRec *data) {
        DtEditorErrorCode error = DtEDITOR_INVALID_TYPE;
        DtEditorWidget editor = (DtEditorWidget)widget;
        _DtWidgetToAppContext(widget);
        ...
        case DtEDITOR_DATA: {
                error =
                    insertDataValue(editor, data->value.data.buf,
                                    data->value.data.length, APPEND_DATA, 0, 0);
                break;
        }
        case DtEDITOR_WCHAR: {
                error = insertWcharValue(editor, data->value.wchar, APPEND_DATA,
                                         0, 0);
                break;
        }
        default: { error = DtEDITOR_INVALID_TYPE; }
...
extern DtEditorErrorCode DtEditorAppendFromFile(Widget widget, char *fileName) {
        DtEditorErrorCode result;
        _DtWidgetToAppContext(widget);
        _DtAppLock(app);
...
extern DtEditorErrorCode DtEditorInsert(Widget widget,
                                        DtEditorContentRec *data) {
        DtEditorErrorCode error = DtEDITOR_INVALID_TYPE;
        DtEditorWidget editor = (DtEditorWidget)widget;
        _DtWidgetToAppContext(widget);
        ...
        case DtEDITOR_WCHAR: {
                error = insertWcharValue(editor, data->value.wchar, INSERT_DATA,
                                         0, 0);
                break;
        }
        default: { error = DtEDITOR_INVALID_TYPE; }
...
extern DtEditorErrorCode DtEditorInsertFromFile(Widget widget, char *fileName) {
        DtEditorErrorCode result;
        _DtWidgetToAppContext(widget);
        _DtAppLock(app);
...
extern DtEditorErrorCode DtEditorReplace(Widget widget, XmTextPosition startPos,
        ...
        if (startPos > endPos) {
                ...
                case DtEDITOR_TEXT: {
                        error =
                            insertStringValue(editor, data->value.string,
                ...
                case DtEDITOR_WCHAR: {
                        error =
                            insertWcharValue(editor, data->value.wchar,
                                             REPLACE_DATA, startPos, endPos);
                        break;
                }
                default: { error = DtEDITOR_INVALID_TYPE; }
...
extern DtEditorErrorCode DtEditorReplaceFromFile(Widget widget,
        ...
        if (startPos > endPos) {
                ...
                result =
                    LoadFile(widget, fileName, REPLACE_DATA, startPos, endPos);
        }
...
extern DtEditorErrorCode _DtEditorValidateFileAccess(char *fileName,
                                                     int accessType) {
        ...
        if (fileName && *fileName) {
                ...
                if (access(fileName, F_OK) != 0)
                        ...
                        if (stat(fileName, &statbuf) != 0)
                                ...
                                /* if its a character special device - can't
                                 * save */
                                ...
                                /*
                                 * We now know that it's a regular file so check
                                 * to whether we can read or write to it, as
                                 * appropriate.
                                 */
...
LoadFile(Widget w, char *fileName, LoadActionType action,
         XmTextPosition startReplace, XmTextPosition endReplace)

{
        ...
        if (fileName && *fileName) {
                ...
                if (returnVal == DtEDITOR_NO_ERRORS) {
                        /*
                         * Open the file for reading.  If we can read/write,
                         * then we're cool, otherwise we might need to tell the
                         * user that the file's read-only, or that we can't even
                         * read from it.
                         */
                        if ((fp = fopen(fileName, "r+")) == NULL) {
                                /*
                                 * We can't update (read/write) the file so try
                                 * opening read- only
                                 */
                                if ((fp = fopen(fileName, "r")) == NULL) {
                                        /*
                                        ...
                                        /*
                                         * Tell the application that the file's
                                         * read-only. Becareful not to overwrite
                                         * this value with one of the calls to
                                         * set the widget's contents.
                                         */
        ...
        if (fp) {
                ...
                /*
                 * Check to see if we have enough memory to load the file
                 * contents into the text widget.  This is only an estimate of
                 * our needs. Check4EnoughMemory() returns DtEDITOR_NO_ERRORS,
                ...
                if (loadError == DtEDITOR_INSUFFICIENT_MEMORY)
                        ...
                        /*
                         * Read the file contents (with room for null) & convert
                         * to a string.  We want to use a string because the
                         * DtEditorSetContents/Append/Insert/... functions
                         * create another copy of the data before actually
                         * putting it into the widget.
                         */
                         ...
                         */
                        loadError =
                            StripEmbeddedNulls(file_string, &file_length);
                        if (loadError != DtEDITOR_NO_ERRORS)
                                returnVal = loadError;

                        /*
                         * Insert it as a string, otherwise the following
                         * DtEditor*() functions will make another copy of the
                         * data.
                         */
                        ...
                        /*
                         * Load, insert, append, or attach the file, as
                         * specified
                         */
                        switch (action) {
                        case LOAD_DATA: {
                        ...
                        case REPLACE_DATA: {
                                loadError = DtEditorReplace(w, startReplace,
                                                            endReplace, &cr);
                                break;
                        }
...
static char *StringAdd(char *destination, char *source, int number) {
...
static char *CopySubstring(XmTextWidget widget, XmTextPosition startPos,
                           XmTextPosition endPos, char *buf,
                           Boolean addNewlines) {
        ...
        if (addNewlines == False) {
                ...
                numToCopy =
                    pLastChar - pCurrChar + mblen(pLastChar, MB_CUR_MAX);
                ...
                mb_str_loc =
                    (int *)XtMalloc(sizeof(int) * ((endPos - startPos) + 1));
                if (NULL == mb_str_loc) {
                        /* Should figure out some way to pass back an error
                         * code. */
                        buf =
                            CopySubstring(widget, startPos, endPos, buf, False);
                        return buf;
                }
                ...
                mb_str_loc[0] = 0;
                for (total = 0, bptr = pString, z = 1; z <= (endPos - startPos);
                     bptr += siz, z++) {
                        if (MB_CUR_MAX > 1) {
                ...
                do {
                        if (startPos >
                            (XmTextPosition)line_table[currLine].start_pos)
                                pCurrChar = pString + mb_str_loc[0];
                        else {
                                z = line_table[currLine].start_pos;
                                pCurrChar = pString + mb_str_loc[z - startPos];
                        }
                        ...
                        if (currLine >= (widget->text.total_lines - 1))
                                pLastChar =
                                    pString + mb_str_loc[endPos - startPos];
                        else if ((XmTextPosition)line_table[currLine + 1]
                                     .start_pos <= endPos) {
                                z = line_table[currLine + 1].start_pos - 1;
                                pLastChar = pString + mb_str_loc[z - startPos];
                        } else
                                pLastChar =
                                    pString + mb_str_loc[endPos - startPos];

                        numToCopy = pLastChar - pCurrChar +
                                    mblen(pLastChar, MB_CUR_MAX);
...
static char *_DtEditorCopyDataOut(XmTextWidget tw, char *buf,
                                  Boolean addNewlines) {
...
static DtEditorErrorCode getStringValue(DtEditorWidget editor, char **buf,
        ...
        if (DtEDITOR_NO_ERRORS != returnVal)
                return returnVal;
...
static DtEditorErrorCode getDataValue(DtEditorWidget editor, void **buf,
                                      unsigned int *size,
                                      Boolean insertNewlines) {
...
static DtEditorErrorCode getWcharValue(DtEditorWidget editor, wchar_t **data,
                                       Boolean insertNewlines) {
        DtEditorErrorCode error;
        char *mb_value;
        ...
        if (error == DtEDITOR_NO_ERRORS) {
                 ...
                 */
                num_char =
                    _DtEditor_CountCharacters(mb_value, strlen(mb_value)) + 1;
                 ...
                 */
                result = mbstowcs(pWchar_value, mb_value,
                                  num_char * sizeof(wchar_t));
                if (result < 0)
                        pWchar_value[0] = 0L;
                *data = pWchar_value;
...
extern DtEditorErrorCode DtEditorGetContents(Widget widget,
                                             DtEditorContentRec *data,
        ...
        /*
         * If there were no errors, mark there are now no unsaved changes
...
extern DtEditorErrorCode DtEditorSaveContentsToFile(
    Widget widget, char *fileName, Boolean overwriteIfExists,
    Boolean hardCarriageReturns, Boolean markContentsAsSaved) {
        ...
        if (fileName && *fileName) {
                ...
                if (error == DtEDITOR_NO_ERRORS ||
                    ...
                    error == DtEDITOR_WRITABLE_FILE) {
                        /*
                         * Don't overwrite an existing file if we've been told
                         * not to
                         */
                        if (error == DtEDITOR_WRITABLE_FILE &&
                        ...
                        if ((pFile = fopen(fileName, "w")) == NULL) {
                                ...
                                /*
                                 * Save the unsaved changes flag so we can
                                 * restore it if the write to the file fails.
                                 */
                                Boolean saved_state = M_unreadChanges(editor);

                                /*
                                 * Now, get the contents of the widget and write
                                 * it to the file, depending upon the format
                                 * requested.
                                 */

                                cr.type = DtEDITOR_DATA;
                                error = DtEditorGetContents(
                                    widget, &cr, hardCarriageReturns,
                                    markContentsAsSaved);
                                ...
                                if (error == DtEDITOR_NO_ERRORS) {
                                         ...
                                         */
                                        size_t size_written =
                                            fwrite(cr.value.data.buf, 1,
                                                   cr.value.data.length, pFile);

                                        if (cr.value.data.length !=
                                            size_written)
                                ...
                                if (error == DtEDITOR_SAVE_FAILED) {
                                        /*
                                         * Restore the unsaved changes flag
 ...
 */
char *_DtEditorGetPointer(char *pString, int startChar) {
        char *bptr;
