...
extern Widget _DtCreateControl(Widget parent, String name, ArgList arglist,
                               int argcount);
...
static void ClickTimeout(XtPointer client_data, XtIntervalId *id);
static void BusyTimeout(XtPointer client_data, XtIntervalId *id);
static void DateTimeout(XtPointer client_data, XtIntervalId *id);
static void PushAnimationTimeout(XtPointer client_data, XtIntervalId *id);
static void DropAnimationTimeout(XtPointer client_data, XtIntervalId *id);
static void FileCheckTimeout(XtPointer client_data, XtIntervalId *id);
static void CheckFile(DtControlGadget g);
...
static void GetSize(DtControlGadget g, Dimension *w, Dimension *h);
static void GetPositions(DtControlGadget g, Position w, Position h,
                         Dimension h_t, Dimension s_t, Position *pix_x,
                         Position *pix_y, Position *str_x, Position *str_y);
static void Draw(DtControlGadget g, Drawable drawable, Position x, Position y,
                 Dimension w, Dimension h, Dimension h_t, Dimension s_t,
                 unsigned char s_type, unsigned char fill_mode);
static void CallCallback(DtControlGadget g, XtCallbackList cb, int reason,
                         XEvent *event);
static void UpdateGCs(DtControlGadget g);
static void ReplaceJPDate(char *date, char *jpstr, int wday);
...
**	Resource List
*/
#define R_Offset(field) XtOffset(DtControlGadget, control.field)
static XtResource resources[] = {
    {"_do_update", XmCBoolean, XmRBoolean, sizeof(Boolean),
     R_Offset(_do_update), XmRImmediate, (XtPointer)FALSE},
    {XmNuseEmbossedText, XmCBoolean, XmRBoolean, sizeof(Boolean),
     R_Offset(use_embossed_text), XmRImmediate, (XtPointer)TRUE},
    {XmNpushFunction, XmCPushFunction, XmRFunction, sizeof(XtPointer),
     R_Offset(push_function), XmRImmediate, (XtPointer)NULL},
    {XmNpushArgument, XmCPushArgument, XmRPointer, sizeof(XtPointer),
     R_Offset(push_argument), XmRImmediate, (XtPointer)NULL},
    {XmNdropAction, XmCDropAction, XmRString, sizeof(String),
     R_Offset(drop_action), XmRImmediate, (XtPointer)NULL},
    {"pushAction", "PushAction", XmRString, sizeof(String),
     R_Offset(push_action), XmRImmediate, (XtPointer)NULL},
    {XmNformat, XmCFormat, XmRString, sizeof(String), R_Offset(format),
     XmRImmediate, (XtPointer) "%b%n%e"},
    {XmNformatJP, XmCFormatJP, XmRString, sizeof(String), R_Offset(format_jp),
     XmRImmediate, (XtPointer)NULL},
    {XmNsubpanel, XmCSubpanel, XmRWidget, sizeof(Widget), R_Offset(subpanel),
     XmRImmediate, (XtPointer)NULL},
    {XmNcontrolType, XmCControlType, XmRControlType, sizeof(unsigned char),
     R_Offset(control_type), XmRImmediate, (XtPointer)XmCONTROL_NONE},
    {XmNfileName, XmCString, XmRString, sizeof(XmString), R_Offset(file_name),
     XmRImmediate, (XtPointer)NULL},
    {XmNalternateImage, XmCAlternateImage, XmRString, sizeof(String),
     R_Offset(alt_image), XmRImmediate, (XtPointer)NULL},
    {XmNchime, XmCBoolean, XmRBoolean, sizeof(Boolean), R_Offset(chime),
     XmRImmediate, (XtPointer)FALSE},
    {XmNclientTimeoutInterval, XmCInterval, XmRInt, sizeof(int),
     R_Offset(max_blink_time), XmRImmediate, (XtPointer)10000},
    {XmNwaitingBlinkRate, XmCInterval, XmRInt, sizeof(int),
     R_Offset(blink_time), XmRImmediate, (XtPointer)500},
    {XmNmonitorTime, XmCInterval, XmRInt, sizeof(int), R_Offset(monitor_time),
     XmRImmediate, (XtPointer)30000},
    {XmNpushButtonClickTime, XmCInterval, XmRInt, sizeof(int),
     R_Offset(click_time), XmRImmediate, (XtPointer)1000},
    {XmNuseLabelAdjustment, XmCBoolean, XmRBoolean, sizeof(Boolean),
...
static void FileCheckTimeout(XtPointer client_data, XtIntervalId *id) {
        ...
        G_MonitorTimer(g) = XtAppAddTimeOut(app_context, G_MonitorTime(g),
...
static void CheckFile(DtControlGadget g) {
        ...
        if (file_changed != G_FileChanged(g)) {
                ...
                (*call_callback)((DtIconGadget)g, G_Callback(g), XmCR_MONITOR,
                                 NULL);
...
static void BusyTimeout(XtPointer client_data, XtIntervalId *id) {
        ...
        if ((G_Busy(g) > 0) && (G_BlinkElapsed(g) < G_MaxBlinkTime(g))) {
                G_Set(g) = !G_Set(g);
                G_BlinkTimer(g) = XtAppAddTimeOut(app_context, G_BlinkTime(g),
...
static void DateTimeout(XtPointer client_data, XtIntervalId *id) {
        ...
        if (s1 != NULL) {
                if (s2 == NULL) {
                        ...
                        G_String(g) =
                            XmStringCreate(s1, XmFONTLIST_DEFAULT_TAG);
                        ...
                        XmStringFree(G_String(g));
                        G_String(g) =
                            XmStringCreate(s2, XmFONTLIST_DEFAULT_TAG);
                        XmStringExtent(G_FontList(g), G_String(g),
                                       &(G_StringWidth(g)),
                                       &(G_StringHeight(g)));
                }
         ...
         */
        tilMidnight =
            1000 * (86400 - ((3600 * timeptr->tm_hour) +
                             (60 * timeptr->tm_min) + (timeptr->tm_sec)));

        if (G_Format(g) != NULL) {
                G_DateTimer(g) = XtAppAddTimeOut(app_context, tilMidnight,
...
static void Initialize(Widget request_w, Widget new_w) {
        ...
        XtAppContext app_context = XtWidgetToApplicationContext(new_w);
        String str;
        ...
        if (((G_ControlType(new) == XmCONTROL_MONITOR) ||
            ...
            (G_AltImage(new) != NULL)) {
                        ...
                        if (G_FileName(new) == NULL) {
                                if (G_ControlType(new) == XmCONTROL_MAIL) {
                                        if ((str = getenv("MAIL")) == NULL) {
                                                str = getenv("LOGNAME");
                                                G_FileName(new) =
                                                    XtMalloc(strlen(MAIL_DIR) +
                                                             strlen(str) + 1);
                                                strcpy(G_FileName(new),
                                                       MAIL_DIR);
                                                strcat(G_FileName(new), str);
                                        } else {
                                                G_FileName(new) =
                                                    XtMalloc(strlen(str) + 1);
                 ...
                 */
                G_AltPix(new) = XmGetPixmap(XtScreen(new), G_AltImage(new),
                                            G_PixmapForeground(new),
                                            G_PixmapBackground(new));
                G_AltMask(new) = _DtGetMask(XtScreen(new), G_AltImage(new));
                G_AltImage(new) = NULL;
...
static Boolean SetValues(Widget current_w, Widget request_w, Widget new_w) {
	...
	Boolean		redraw_flag =	False;
        ...
        if (G_ImageName(new_c) &&
            (G_ImageName(current_c) != G_ImageName(new_c)))
                return True;
        ...
        case XmPIXMAP_LEFT:
                ...
                /*
                                        if (p_w && width > p_w + spacing + s_w)
                                                p_x += (width - p_w - spacing -
                   s_w)/2; if (s_w && width > p_w + spacing + s_w) s_x += (width
                   - p_w - spacing - s_w)/2;
                */
                break;
        case XmPIXMAP_RIGHT:
...
static void Draw(DtControlGadget g, Drawable drawable, Position x, Position y,
        ...
        if (G_Pixmap(g)) {
                 ...
                 */
                         ...
                         ((G_ControlType(g) == XmCONTROL_BUSY) &&
                          (G_AltPix(g) != XmUNSPECIFIED_PIXMAP) && G_Busy(g) &&
                          G_Set(g))) {
                        pix = G_AltPix(g);
                        mask = G_AltMask(g);
                }
        ...
        if (bClearShadow) {
                ...
                XFillRectangle(d, drawable, gc, x + h_t, y + h_t, w - 2 * h_t,
                               s_t);
                XFillRectangle(d, drawable, gc, x + h_t, y + h_t + s_t, s_t,
                               h - 2 * (h_t + s_t));
                XFillRectangle(d, drawable, gc, x + w - h_t - s_t,
                               y + h_t + s_t, s_t, h - 2 * (h_t + s_t));
        ...
        _DtProcessUnlock();
        (*get_positions)((DtIconGadget)g, w, h, h_t, s_t, &p_x, &p_y, &s_x,
                         &s_y);
        ...
        if (G_Pixmap(g)) {
                ...
                if ((width > 0 && height > 0) && (gc != NULL) &&
                    (pix != XmUNSPECIFIED_PIXMAP))
                        XCopyArea(d, pix, drawable, gc, 0, 0, width, height,
                                  x + p_x, y + p_y);
        }
        ...
        if ((G_String(g)) && (clip.width > 0 && clip.height > 0)) {
                if (G_ControlType(g) == XmCONTROL_DATE) {
                        ...
                        if (bMono) {
                                XmStringDrawImage(
                                    d, drawable, G_FontList(g), G_String(g),
                        ...
                        if (bMono) {
                                XmStringDrawImage(
                                    d, drawable, G_FontList(g), G_AltString(g),
                                    G_NormalGC(g), x + s_x, y + s_y, clip.width,
                                    XmALIGNMENT_BEGINNING,
                                    XmSTRING_DIRECTION_L_TO_R, &clip);
                        } else {
                                if (G_UseEmbossedText(g))
                                        XmStringDraw(
                                            d, drawable, G_FontList(g),
                                            G_AltString(g), G_BottomShadowGC(g),
                                            x + s_x + 1, y + s_y + 1,
                                            clip.width, XmALIGNMENT_BEGINNING,
                                            XmSTRING_DIRECTION_L_TO_R, &clip);
                                XmStringDraw(d, drawable, G_FontList(g),
                                             G_AltString(g), G_NormalGC(g),
                                             x + s_x, y + s_y, clip.width,
                                             XmALIGNMENT_BEGINNING,
                                             XmSTRING_DIRECTION_L_TO_R, &clip);
                        }
                        ...
                        if (bMono) {
                                ...
                                XmStringDrawImage(
                                    ...
                                    XmALIGNMENT_BEGINNING,
                                    XmSTRING_DIRECTION_L_TO_R, &clip);
                                if ((s_x + G_StringWidth(g) + 2U) <=
                                    (G_Width(g) - h_t - s_t))
                                        XFillRectangle(
                                            d, drawable, G_ArmedBackgroundGC(g),
        ...
        case XmCONTROL_CLIENT:
                 ...
                 */
                XmeDrawShadows(d, drawable, gcTS, gcBS, x + left - s_t,
                               y + top - s_t, w - left - right + (2 * s_t),
                               h - top - bottom + (2 * s_t), s_t,
                               XmSHADOW_ETCHED_OUT);
                break;
        ...
        case XmCONTROL_MAIL:
                ...
                case XmICON_BUTTON:
                        if (G_Armed(g))
                                XmeDrawShadows(d, drawable, M_TopShadowGC(mgr),
                                               M_BottomShadowGC(mgr), x + h_t,
                                               y + h_t, w - 2 * h_t,
                                               h - 2 * h_t, s_t, XmSHADOW_IN);
                        break;
                case XmICON_TOGGLE:
        ...
        case XmCONTROL_SWITCH:
                ...
                XmeDrawShadows(d, drawable, gcTS, gcBS, x + h_t, y + h_t,
                               w - 2 * h_t, h - 2 * h_t, 1, XmSHADOW_OUT);
                ...
                if (s_t > 0) {
                        ...
                        XmeDrawShadows(d, drawable, gcTS, gcBS, x + h_t,
...
static void PushAnimationTimeout(XtPointer client_data, XtIntervalId *id) {
        ...
        if ((G_PushImagePosition(g) > 0) &&
            (G_PushImagePosition(g) <= G_NumPushImages(g))) {
                G_PushAnimationTimer(g) = XtAppAddTimeOut(
                    app_context, G_PushDelays(g)[G_PushImagePosition(g) - 1],
...
**-------------------------------------------------------------
*/
void _DtControlDoPushAnimation(Widget w) {
        DtControlGadget g = (DtControlGadget)w;
...
**-------------------------------------------------------------
*/
void _DtControlAddPushAnimationImage(Widget w, String image, int delay) {
        DtControlGadget g = (DtControlGadget)w;
        int i;
        ...
        if (G_NumPushImages(g) == G_MaxPushImages(g)) {
                ...
                G_PushDelays(g) = (int *)XtRealloc(
                    (char *)G_PushDelays(g), G_MaxPushImages(g) * sizeof(int));
                G_PushPixmaps(g) =
                    (Pixmap *)XtRealloc((char *)G_PushPixmaps(g),
                                        G_MaxPushImages(g) * sizeof(Pixmap));
                G_PushMasks(g) =
                    (Pixmap *)XtRealloc((char *)G_PushMasks(g),
                                        G_MaxPushImages(g) * sizeof(Pixmap));
        }
        ...
        G_PushPixmaps(g)[i] = XmGetPixmap(
            XtScreen(g), image, G_PixmapForeground(g), G_PixmapBackground(g));
...
static void DropAnimationTimeout(XtPointer client_data, XtIntervalId *id) {
        ...
        if ((G_DropImagePosition(g) > 0) &&
            (G_DropImagePosition(g) <= G_NumDropImages(g))) {
                G_DropAnimationTimer(g) = XtAppAddTimeOut(
                    app_context, G_DropDelays(g)[G_DropImagePosition(g) - 1],
...
**-------------------------------------------------------------
*/
void _DtControlDoDropAnimation(Widget w) {
        DtControlGadget g = (DtControlGadget)w;
...
**-------------------------------------------------------------
*/
void _DtControlAddDropAnimationImage(Widget w, String image, int delay) {
        DtControlGadget g = (DtControlGadget)w;
        int i;
        ...
        if (G_NumDropImages(g) == G_MaxDropImages(g)) {
                ...
                G_DropDelays(g) = (int *)XtRealloc(
                    (char *)G_DropDelays(g), G_MaxDropImages(g) * sizeof(int));
                G_DropPixmaps(g) =
                    (Pixmap *)XtRealloc((char *)G_DropPixmaps(g),
                                        G_MaxDropImages(g) * sizeof(Pixmap));
                G_DropMasks(g) =
                    (Pixmap *)XtRealloc((char *)G_DropMasks(g),
                                        G_MaxDropImages(g) * sizeof(Pixmap));
        }
...
**-------------------------------------------------------------
*/
void _DtControlSetFileChanged(Widget w, Boolean b) {
        DtControlGadget g = (DtControlGadget)w;
        XtExposeProc expose;
...
**-------------------------------------------------------------
*/
void _DtControlSetBusy(Widget w, Boolean b) {
        DtControlGadget g = (DtControlGadget)w;
        ...
        if (b) {
                ...
                if (G_Busy(g) == 1) {
                        ...
                        call_callback = C_CallCallback(XtClass(g));
                        _DtProcessUnlock();
                        (*call_callback)((DtIconGadget)g, G_Callback(g),
                                         XmCR_BUSY_START, NULL);
...
**-------------------------------------------------------------
*/
Widget _DtCreateControl(Widget parent, String name, ArgList arglist,
                        int argcount) {
        return (XtCreateWidget(name, dtControlGadgetClass, parent, arglist,
                               argcount));
}
...
static void ReplaceJPDate(char *date, char *jpstr, int wday) {
        ...
        strcpy(s, jpstr);
        for (p = _XStrtok(s, ",", strtok_buf), i = 0; p != NULL && i < wday;
...
char _DtControlGetMonitorState(Widget w) {
        DtControlGadget g = (DtControlGadget)w;
