...
static void VariableInitialize(DtEditorWidget new) {
        ...
        M_unreadChanges(new) =
            False; /* There have not been any changes since */
...
static void ValidateResources(DtEditorWidget new, DtEditorWidget request) {
        ...
        /*
         * Make local copies of all resource strings assigned by the
         * application.
         */
        ...
        if (E_format_dialogTitle(request) != (XmString)NULL)
                E_format_dialogTitle(new) =
                    XmStringCopy(E_format_dialogTitle(request));
        else
                E_format_dialogTitle(new) =
                    XmStringCreateLocalized(FORMAT_SETTINGS);

        if (E_infoDialogTitle(request) != (XmString)NULL)
                E_infoDialogTitle(new) =
                    XmStringCopy(E_infoDialogTitle(request));
        else
...
static Widget CreateText(DtEditorWidget parent) {
         ...
         * with the real value.
         */
        XtSetArg(al[ac], XmNautoShowCursorPosition,
                 M_autoShowCursorPos(parent));
        ac++;
        ...
        if (M_scrollHorizontal(parent)) /* XXX Word Wrap workaround */
                FixWordWrap(text,
                            M_wordWrap(parent)); /* XXX Word Wrap workaround */
...
static void extractFontMetrics(DtEditorWidget w, XmFontList fontlist,
        ...
        *width = *height = 0;
        if (XmFontListInitFontContext(&context, fontlist)) {
                ...
                /*
                 * Look through the font list for a fontset with the default
                 * tag. If we do not find it, use the first font set we came to,
                 * otherwise, use the first font struct we came to.
                 */
                do {
                        ...
                        if (next_entry) {
                                ...
                                tmp_font = XmFontListEntryGetFont(next_entry,
                                                                  &type_return);
                                if (type_return == XmFONT_IS_FONTSET) {
                                         ...
                                         */
                                        font_tag =
                                            XmFontListEntryGetTag(next_entry);

                                        if (!have_font_set) {

                                                /*
                                                 * Save the first fontset found
                                                 * in case we don't find a
                                                 * default tag font set.
                                                 */
                                                have_font_set =
                                                    True; /* we have a font set.
                                                           */
                                                use_font_set = True;
                                                font = (XFontStruct *)tmp_font;

                                                /*
                                                 * We have a font set, so no
                                                 * need to consider future font
                                                 * structs
                                                 */
                                        ...
                                        /*
                                         * Save the first one in case no font
                                         * set is found
                                         */
                                        font = (XFontStruct *)tmp_font;
                                        use_font_set = False;
                ...
                if (use_font_set) {
                        ...
                        if (XmDirectionMatch(w->manager.string_direction,
                                ...
                                tmp_width =
                                    (unsigned long)
                                        fs_extents->max_ink_extent.width;
                        } else {
                                tmp_width =
                                    (unsigned long)
                                        fs_extents->max_logical_extent.width;
                        }
                        ...
                        if ((!XGetFontProperty(font, XA_QUAD_WIDTH,
                                               &tmp_width)) ||
                            tmp_width == 0) {
                                if (font->per_char &&
                ...
                if (tmp_width <= 0)
                        tmp_width = 1;
                *width = (int)
                    tmp_width; /* This assumes there will be no truncation */
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw) {
        ...
        if (XmStringCompare(M_spellTitle(request), M_spellTitle(current)) ==
            False) {
        ...
        if (resetSpellTitle == True) {
                /*
                 * No need to do anything because the Spell and Find/Change
                 * dialog titles are recomputed every time.
                 */
                redisplay_flag = True;
        }
         ...
         */
        if (M_status_showStatusLine(request) !=
            M_status_showStatusLine(current)) {
                SetStatusLine(new, M_status_showStatusLine(request));
                redisplay_flag = True;
        }
        ...
        if (M_status_statusArea(current) != (Widget)NULL) {
                ...
                if (newLabelFontList == True) {
                        ...
                        XtSetValues((Widget)M_status_totalLabel(current), al,
                                    1);
                        XtSetValues((Widget)M_status_totalText(current), al, 1);
                        XtSetValues((Widget)M_status_overstrikeWidget(current),
                                    al, 1);
                }
        ...
        if (M_gen_warning(current) != (Widget)NULL) {
                ...
                if (newButtonFontList == True) {
                        ...
                        XtSetValues(XmMessageBoxGetChild(M_gen_warning(current),
                                                         XmDIALOG_OK_BUTTON),
                                    al, 1);
                }
                ...
                if (newLabelFontList == True) {
                        XtSetArg(al[0], XmNfontList, E_labelFontList(request));
                        XtSetValues(
                            XmMessageBoxGetChild(M_gen_warning(current),
        ...
        if (M_search_dialog(current) != (Widget)NULL) {
                if (newButtonFontList == True) {
                        ...
                        XtSetValues((Widget)M_search_replaceBtn(current), al,
                                    1);
                        XtSetValues((Widget)M_search_replaceAllBtn(current), al,
                                    1);
                ...
                if (newLabelFontList == True) {
                        ...
                        XtSetValues((Widget)M_search_replaceLbl(current), al,
                                    1);
                }
        ...
        if (M_format_dialog(current) != (Widget)NULL) {
                ...
                if (newLabelFontList == True) {
                        ...
                        XtSetValues((Widget)M_format_rightLabel(current), al,
                                    1);
                }

                if (newTextFontList == True) {
                        XtSetArg(al[0], XmNfontList, E_textFontList(request));
                        XtSetValues((Widget)M_format_leftMarginField(current),
                                    al, 1);
                        XtSetValues((Widget)M_format_rightMarginField(current),
                                    al, 1);
                }
        ...
        /*
         * Check for any changes which affect the edit area (scrolled text
         * widget)
         */
        ...
        if (M_cursorPosVisible(request) != M_cursorPosVisible(current)) {
                XtSetArg(al[ac], XmNcursorPositionVisible,
                         M_cursorPosVisible(request));
                ac++;
        }
        ...
        if (M_editable(request) != M_editable(current)) {
                ...
                if (M_editable(request)) {
                        ...
                        /*
                         * ...turn back on the Change To field in the
                         * Find/Change dialog
                         */
                        if (M_search_dialog(current) != (Widget)NULL) {
                                XtSetSensitive(M_search_replaceLbl(current),
                                               True);
        ...
        if (NeedToFixWordWrap) /*XXX Word Wrap workaround*/
                FixWordWrap(M_text(new),
                            M_wordWrap(request)); /*XXX Word Wrap workaround*/

        if (ac)
                XtSetValues((Widget)M_text(new), al, ac);

        if (E_textTranslations(request) != E_textTranslations(current)) {
                XtOverrideTranslations(M_text(new),
                                       E_textTranslations(request));
        }
...
static void _DtEditorGetFormatParagraphButtonLabel(Widget wid,
        ...
        if (E_format_formatParagraphButtonLabel(editor) !=
            (XmString)DtUNSPECIFIED)
                data =
                    XmStringCopy(E_format_formatParagraphButtonLabel(editor));
        else if (M_format_dialog(editor) != (Widget)NULL) {
                XtSetArg(al[0], XmNlabelString, &data);
...
static void BackwardWord(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "backward-word", event, params,
                         *num_params);
}
...
static void CutClipboard(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "cut-clipboard", event, params,
                         *num_params);
}
...
static void DeselectAll(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "deselect-all", event, params,
                         *num_params);
}
...
static void EndOfFile(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "end-of-file", event, params,
                         *num_params);
}
...
static void EndOfLine(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "end-of-line", event, params,
                         *num_params);
}
...
static void ForwardWord(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "forward-word", event, params,
                         *num_params);
}
...
static void GoToLine(Widget w, XEvent *event, char **params,
        ...
        if (M_status_statusArea(editor) != (Widget)NULL &&
            XtIsManaged(M_status_statusArea(editor)) == True) {
                ...
                XmProcessTraversal(M_status_lineText(editor),
                                   XmTRAVERSE_CURRENT);
                ...
                XtCallActionProc((Widget)M_status_lineText(editor),
                                 "select-all", event, (String *)NULL, 0);
        }
...
static void InsertString(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "insert-string", event, params,
                         *num_params);
}
...
static void KeySelect(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "key-select", event, params,
                         *num_params);
}
...
static void NextPage(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "next-page", event, params,
                         *num_params);
}
...
static void PageLeft(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "page-left", event, params,
                         *num_params);
}
...
static void PageRight(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "page-right", event, params,
                         *num_params);
}
...
static void PreviousPage(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "previous-page", event, params,
                         *num_params);
}
...
static void ProcessDown(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "process-down", event, params,
                         *num_params);
}
...
static void ProcessUp(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "process-up", event, params,
                         *num_params);
}
...
static unsigned int GetModeSwitchModifier(Display *dpy) {
        ...
        for (mapIndex = 3 * pMap->max_keypermod; mapIndex < mapSize;
             mapIndex++) {
                /* look only at the first 4 columns of key map */
                for (keyCol = 0; keyCol < 4; keyCol++) {
                        keySym = XKeycodeToKeysym(
                            dpy, pMap->modifiermap[mapIndex], keyCol);
                        if (keySym == XK_Mode_switch)
                                modeSwitchModMask |=
                                    1 << (mapIndex / pMap->max_keypermod);
                }
        }
...
static void QuoteNextChar(Widget widget, XEvent *event, char **params,
        ...
        if (!ExtendcharMask) {
                ...
                if (!ExtendcharMask)
                        ExtendcharMask =
                            GetModeSwitchModifier(M_display(editor));
                _DtProcessUnlock();
        }
        ...
        for (;;) {
                ...
                if (mask & XtIMXEvent) {
                        ...
                        if ((next.type == KeyPress) &&
                            (!IsModifierKey(
                                XLookupKeysym((XKeyEvent *)&next, 0)))) {
                                if (next.xkey.state &
                                    (ControlMask | ExtendcharMask))
                                        XtCallActionProc(M_text(editor),
                                                         "self-insert", &next,
                                                         NULL, 0);
                                else
                        ...
                        /* Some (non-X) event is pending, so this should not
                         * block. */
...
static void SelectAll(Widget w, XEvent *event, char **params,
        ...
        XtCallActionProc(M_text(editor), "select-all", event, params,
                         *num_params);
}
...
static void AnimateCallback(Widget w, XtPointer client_data,
        ...
        case DtDND_FILENAME_TRANSFER: {
                ...
                for (ii = 0; ii < numItems; ii++) {
                        error = DtEditorInsertFromFile(
                            (Widget)editor,
                            animateInfo->dropData->data.files[ii]);

                        /*
                         * If the file was not inserted successfully, then quit
                         * (don't care if the file is read only or if it
                         * contained nulls that were stripped out).
                         */
        ...
        case DtDND_BUFFER_TRANSFER: {
                ...
                for (ii = 0; ii < numItems; ii++) {
                        ...
                        /*
                         * If the buffer was not inserted successfully, then
                         * quit (don't care if the file contained nulls that
                         * were stripped out).
                         */
                        if (error != DtEDITOR_NO_ERRORS &&
                            error != DtEDITOR_NULLS_REMOVED) {
...
void _DtEditorWarning(DtEditorWidget editor, char *mess,
        ...
        if (M_gen_warning(editor) == (Widget)NULL) {
                ...
                M_gen_warning(editor) = (Widget)XmCreateMessageDialog(
                    M_topLevelShell(editor), "Warn", al, ac);
                ...
                XtSetValues(XmMessageBoxGetChild(M_gen_warning(editor),
                                                 XmDIALOG_OK_BUTTON),
                            al, 1);
...
void _DtEditorUpdateLineDisplay(DtEditorWidget editor, int currentLine,
        ...
        if (M_status_showStatusLine(editor) == True) {
                 ...
                 * Current line
                 */
                if (M_status_currentLine(editor) != currentLine ||
                    forceUpdate) {
                        sprintf(tmpChars, "%d", currentLine);
                        XmTextFieldSetString(M_status_lineText(editor),
                                             tmpChars);
            ...
            M_status_overstrikeWidget(ew) != (Widget)NULL) {
                ...
                if (overstrikeOn == True) {
                        XtSetArg(al[ac], XmNlabelString,
                                 M_status_overstrikeLabel(ew));
                        ac++;
                } else {
                        XtSetArg(al[ac], XmNlabelString,
                                 M_status_insertLabel(ew));
                        ac++;
                }
...
static void SetStatusLine(DtEditorWidget ew, Boolean statusLineOn) {
        ...
        if (statusLineOn == True) {
                ...
                cursorPos =
                    (XmTextPosition)DtEditorGetInsertionPosition((Widget)ew);
                currentLine =
                    _DtEditorGetLineIndex((XmTextWidget)M_text(ew), cursorPos) +
                    1;
                ...
                XtAddCallback(M_text(ew), XmNmotionVerifyCallback,
                              (XtCallbackProc)SetCursorPosStatus,
                              (XtPointer)ew);
...
static Widget CreateStatusLine(DtEditorWidget parent) {
        ...
        ac++;
        M_status_lineText(parent) =
            XmCreateTextField(container, "lineText", al, ac);
...
static void Center(FormatData *data, wint_t *in_line,
                   register wint_t *inlinelim);
             ...
             byteNum += mblen(&pLine[byteNum], mbCurMax), charNum++) {
                ...
                if (iswcntrl(wc)) {
                        if (mblen(&pLine[byteNum], mbCurMax) == 1) {
                                ...
                                /*
                                 * I don't really know what to do with weird
                                 * embedded chars. This is just a guess.  In
                                 * non-ascii locales this could screw up, but I
                                 * don't know how else to deal with weird
...
static void fixLeftMarginAndNewlines(DtEditorWidget editor, FILE *fp,
         ...
         */
        lineIndex =
            _DtEditorGetLineIndex((XmTextWidget)M_text(editor), startPos);
        ...
        for (; lineIndex < lastIndex; lineIndex++) {
                ...
                if (newPara == True) {
                         ...
                         * paragraph, then the proper indent is
                         * adjustStuff.left_margin.
                         */
                        ...
                        pNext = _XmStringSourceGetString(
                            (XmTextWidget)M_text(editor),
                            lineTable[lineIndex + 1].start_pos,
                            lineTable[lineIndex + 1].start_pos + nextLen,
                            False);

                        if (lineBlank(pNext,
                                      nextLen)) /* single line paragraph */
                                ...
                                if (offset >= 0) {
                                        /* second line is closer to the left
                                         * margin */

                                        /*
                                         * Note, the first line will still be
                                         * indented even if the remaining words
                                         * in the paragraph can all fit on the
                                         * first line.  The end result is a one
                                         * line indented paragraph.  I am
                                         * mentioning this because some may
        ...
        if ((total_lines - lineIndex) == 1) {
                ...
                if (lineLen > 0) {
                        ...
                        pLastChar = _DtEditorGetPointer(pLine, lineLen - 1);
                        lineByteLen =
                            pLastChar - pLine + mblen(pLastChar, mbCurMax);
                        if (lineBlank(pLine, lineLen)) {
                                fwrite(pLine, sizeof(char), lineByteLen, fp);
                        } else {
                                pFirstNonBlank = findNonBlank(pLine, lineLen);
                                fwrite(pFirstNonBlank, sizeof(char),
                                       lineByteLen - (pFirstNonBlank - pLine),
                                       fp);
                        }
                        XtFree(pLine);
                }
...

{
         ...
         */
        if ((data.wordbuf = malloc(sizeof(wint_t) * LINESIZE * 3)) ==
            (wint_t *)NULL)
                return 0;
...
static int DoLine(FormatData *data,
                   ...
                   register wint_t *inlinelim) /* end of line + 1	*/
{
        ...
        for (incp = in_line; (incp < inlinelim) && (outcp < outlinelim);
             incp++) {
                ...
                if (iswspace(ch)) {
                        needsp =
                            ((ch != L'\t') && (iswblank(ch)))
                                ? 1
                                : data->tabsize - ((indent + outcp - outline) %
                                                   data->tabsize);
         ...
         */
                 ...
                 *	even	left	even	exact centering (due to
                 *"extra" char to right)
                 *
                 * When centermiddle is default or given by user, it is the same
                 *as the first two lines above (middle exactly specified).
                 */
                ...
                if (data->centerstartpara) /* start of paragraph */
                {
                        ...
                        if (data->m0flag) /* set automargin */
                                /* 1 */ data->centermiddle =
                                    (indent + textwidth + extwidth + 1) / 2;
                }

                /* 2 */ PrintIndent(data, data->centermiddle -
                                              ((textwidth + extwidth + 1) / 2));
                {
                        int i;
                 ...
                 register wint_t *inlinelim) /* end of line + 1	*/
{
        ...
        for (incp = in_line; incp < inlinelim; incp++) {
                ...
                /*
                 * AT WHITE SPACE; FINISH PREVIOUS WORD if any, then skip white
                 * space:
                 */
                 ...
                 */
                        ...
                        if (!inword) /* start new word */
                        {
                                ...
                                if (data->wordcp >
                                    data->wordcpwrap) /* past wrap point   */
                                        data->wordcp =
                                            data->wordbuf; /* wraparound buffer
                                                            */
                                ...
                                data->wordat->cp =
                                    data->wordcp; /* note start of word */
                                data->wordat->inlinenum =
                                    data->inlinenum; /* note input line    */
                                data->wordat->incolumn =
                                    data->incolumn; /* note input column  */
                                data->wordat->wclen = 0;
        ...
        } else                                  /* have text on line */
        {
                if (inword) /* line ends in word */
                        FillWord(data);
                if (data->pendchars >
                    data->fillmargin) {    /* time to dump line	*/
                        Dump(data, False); /* not end of paragraph	*/
                }

                if (data->m0flag &&
                    (data->inlinenum == 1)) /* need to note right margin */
                        data->fillmargin =
                            data->wordat->incolumn + data->wordat->length - 1;
        }
} /* Fill */
...
static void Dump(FormatData *data, int endpara) /* end of paragraph? */
{
        ...
        while ((data->pendchars > data->fillmargin) /* line is full */
               || (endpara &&
                   (data->wordfirst != data->wordnext))) /* more to dump */
        {
                startpara = (data->outlinenum < 2);
                indent = (startpara || (!haveindent2)) ? data->indent1
                                                       : data->indent2;
                ...
                if (startpara && haveindent2 &&
                    (data->indent1 < data->indent2)) {
                         ...
                         */
                                ...
                                WORD *wordat =
                                    data->wordfirst; /* local value */

                                while (wordat !=
                                       wordpast) /* decrement pendchars */
                                {
                                        data->pendchars -=
                                            wordat->length + wordat->blanks;
                ...
                outneed =
                    data->fillmargin - indent; /* always greater than zero */
                outchars = 0;
                ...
                */
                ...
                if (data->pAdj->rflag) /* right-justify only */
                {
                        ...
                        {
                                int blanks =
                                    data->fillmargin - outchars - indent;

                                while (blanks-- >
                                       0) /* can't use PrintIndent()*/
                        ...
                        if (data->pAdj->jflag &&
                            !(endpara && (wordpast == data->wordnext)))
...
static void PrintIndent(FormatData *data, int indent) /* leading indentation */
{
        if (indent > 0) /* indentation needed */
        {
                ...
                fprintf(data->pAdj->outfp, "%*s", indent,
                        ""); /*[remaining] blanks */
        }
} /* PrintIndent */
...
static void Justify(FormatData *data,
                    ...
                    WORD *wordpast)      /* past last to print */
{
         ...
         *
         * March backwards through the words on line, starting with next to
         * last.
         */
         ...
         *
         * Each pass through the sorted list adds one trailing blank to each
         * word not already past the current level.  Thus all one-blank words
         * are brought up to two; then all twos up to three; etc.
         */
...
static int CompareWords(const void *p1, const void *p2) {
        ...
        **	wordfirst	- beginning the word list (buffer of pending
        *lines) *	wordnext	- next available spot in the pending
        *buffer (i.e. *			  end of the buffer).
        **
...
static int EkinBekinProc(FormatData *data, WORD **wordpast) {
        ...
        if (ekinclass) {
                ...
                if (ekinflag) {
                        ...
                        if (curword != data->wordfirst) {
                                /*
                                 * We found a non-ekinsoku after we found the
                                 * ekinsoku. Move the end of line to just after
                                 * the non-ekinsoku.
                                 */
                                ...
                                *wordpast = curword;
                                ...
                                /*
                                 * Reached the beginning of the buffer without
                                 * finding a non-ekinsoku.  If the last word in
                                 * the line can begin a line (i.e. non-bkinsoku)
                                 * make it the first word of the next line.
                                 */
                                curword = WORDPREV(data, compword);
        ...
        if (bekinclass) {
                ...
                if (curword != data->wordnext && curword != compword) {
                        /*
                         * The first word of the second line is bekinsoku so
                         * search backwards from end of first line until we are:
                         * 1) not at the first word of the buffer, and
                         * 2) not in between either an ekinsoku or bekinsoku
                         * character.
                         */
                        ...
                        if (curword != data->wordfirst) {
                                /*
                                 * Did not reach the beginning of the buffer so
                                 * we are between two non-ekinsoku &
                                 * non-bekinsoku characters.  Move the end of
                                 * the first line here.
                                 */
...
static int EkinBekinProc(FormatData *data, WORD **wordpast) {
         ...
         */
        if (prevword->length > data->fillmargin ||
            (prevword == data->wordfirst))
                return False;
        ...
        while ((curword != data->wordfirst) &&
               (wdbindf(*(prevword->cp + prevword->wclen - 1), *curword->cp,
                        1) > 4)) {
                curword = prevword;
                prevword = WORDPREV(data, curword);
        }
        ...
        if (curword != data->wordfirst) {
                ...
                *wordpast = curword;
                 ...
                 * wordnext points to the next available space in the buffer
                ...
                while (nextword != data->wordnext) {
                        ...
                        if (wdbindf(*(curword->cp + curword->wclen - 1),
                                    *nextword->cp, 1) < 5)
                                break;
                        margin += (curword->length + curword->blanks);
                        if (margin >= data->fillmargin)
                ...
                if (nextword != data->wordnext) {
                         ...
                         * two non-ekinsoku & non-bekinsoku characters.  Move
                         * the end of the first line here.
                         */
                        *wordpast = nextword;
                        return True;
                }
        ...
        if (leftMargin >= 0 && rightMargin > 0 && rightMargin < 1025 &&
            leftMargin <= rightMargin) {
                ...
                */
                ...
                default: { returnVal = DtEDITOR_INVALID_TYPE; }
                ...
                if ((adjRec.infp = fopen(tempName1, "w+")) != (FILE *)NULL) {
                        ...
                        if ((adjRec.outfp = fopen(tempName2, "w")) !=
                            (FILE *)NULL) {

                                /*
                                 * Successfully opened the second temporary
                                 * file, so do the formatting.
                                 */
                                returnVal = DtEDITOR_NO_ERRORS;

                                fixLeftMarginAndNewlines(
                                    editor, adjRec.infp, leftMargin,
                                ...
                                fclose(adjRec.outfp);
                                returnVal = DtEditorReplaceFromFile(
                                    (Widget)editor, start, end, tempName2);
                                unlink(tempName2);

                        } else {
                                /*
                                 * Could not open second temporary file, so
                                 * clean up first one
                                 */
                                fclose(adjRec.infp);
                                unlink(tempName1);
...
static int ComputeRightMargin(DtEditorWidget editor) {
        ...
        if (M_fontWidth(editor) != 0)
                rightMargin =
                    ((int)text_width -
                     (2 * ((int)highlight_thickness + (int)shadow_thickness +
                           (int)margin_width))) /
                    M_fontWidth(editor);
        else
...
static void CreateFormatDialog(DtEditorWidget editor) {
        ...
        if (E_format_leftMarginFieldLabel(editor) != (XmString)DtUNSPECIFIED) {
                 ...
                 */
                tempString =
                    XmStringCopy(E_format_leftMarginFieldLabel(editor));
        ...
        XtAddCallback(M_format_leftLabel(editor), XmNhelpCallback,
                      (XtCallbackProc)HelpFormatLeftMarginCB,
                      (XtPointer)editor);
        ...
        if (E_format_rightMarginFieldLabel(editor) != (XmString)DtUNSPECIFIED) {
                 ...
                 */
                tempString =
                    XmStringCopy(E_format_rightMarginFieldLabel(editor));
                E_format_rightMarginFieldLabel(editor) =
                    (XmString)DtUNSPECIFIED;
        } else {
                /*
                 * The resource has not been set so use its default value
        ...
        M_format_rightMarginField(editor) = (Widget)XmCreateTextField(
            M_format_dialog(editor), "right_text", al, ac);
        ...
        if (E_format_leftAlignToggleLabel(editor) != (XmString)DtUNSPECIFIED) {
                 ...
                 */
                tempString =
                    XmStringCopy(E_format_leftAlignToggleLabel(editor));
        ...
        if (E_format_rightAlignToggleLabel(editor) != (XmString)DtUNSPECIFIED) {
                 ...
                 */
                tempString =
                    XmStringCopy(E_format_rightAlignToggleLabel(editor));
        ...
        if (E_format_formatParagraphButtonLabel(editor) !=
            (XmString)DtUNSPECIFIED) {
                ...
                tempString =
                    XmStringCopy(E_format_formatParagraphButtonLabel(editor));
                E_format_formatParagraphButtonLabel(editor) =
                    (XmString)DtUNSPECIFIED;
        } else {
                /*
                 * The resource has not been set so use its default value
        ...
        if (E_format_formatAllButtonLabel(editor) != (XmString)DtUNSPECIFIED) {
                 ...
                 */
                tempString =
                    XmStringCopy(E_format_formatAllButtonLabel(editor));
...
static void GetAdjustSettings(DtEditorWidget editor,
        ...
        if (M_format_dialog(editor) != (Widget)NULL) {
                ...
                else if (XmToggleButtonGadgetGetState(
                             M_format_rightJust(editor)))
                        formatSettings->alignment = DtEDITOR_ALIGN_RIGHT;
                else if (XmToggleButtonGadgetGetState(
                             M_format_bothJust(editor)))
                ...
                /*
                 * The Format Settings dialog box has not been created so use
                 * the default values.
                 */
...
Widget DtCreateEditor(Widget parent, char *name, ArgList arglist,
                      Cardinal argcount) {
        return (XtCreateWidget(name, dtEditorWidgetClass, parent, arglist,
                               argcount));
}
...
DtEditorErrorCode DtEditorFormat(Widget widget,
        ...
        case DtEDITOR_FORMAT_PARAGRAPH: {
                ...
                if (end != XmTextGetLastPosition(M_text(editor))) {
                        /*
                         * If we're not at the end, then we need to back up to
                         * the start of this line, otherwise we'll disturb the
                         * first line of the following paragraph.  If we are at
                         * the end, then "end" must point to the end of the
                         * line, or we'll leave the last line along with the
                         * formatted text.
                         */
        ...
        default: { error = DtEDITOR_INVALID_RANGE; }
...
void DtEditorGoToLine(Widget widget, int lineNumber) {
         ...
         * not be updated.
         */
        if (lineNumber > tw->text.total_lines ||
            lineNumber == DtEDITOR_LAST_LINE) {
         ...
         */
        if (newPos < tw->text.top_character ||
            newPos >= tw->text.bottom_position) {
                Arg al[5];
                int ac;
                Dimension height;
                short rows;
...
void DtEditorGetSizeHints(Widget widget, XSizeHints *pHints) /* Return */
{
         ...
         */
        textRegionWidth =
            (int)Twidth - (2 * ((int)highlightThickness + (int)shadowThickness +
                                (int)marginWidth));
        textRegionHeight =
            (int)Theight - (2 * ((int)highlightThickness +
        ...
        pHints->min_height =
            pHints->base_width + pHints->height_inc + statusHeight;
...
void DtEditorReset(Widget widget) {
         ...
         * Note, the current & total line displays are reset by
         * DtEditorSetContents()
         */
        if (M_status_statusArea(editor) != (Widget)NULL) {
