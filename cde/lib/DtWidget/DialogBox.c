...
extern Widget _DtCreateDialogBox(Widget parent, char *name, ArgList arglist,
                                 Cardinal argcount);
extern Widget __DtCreateDialogBoxDialog(Widget ds_p, String name,
                                        ArgList db_args, Cardinal db_n);
extern Widget _DtDialogBoxGetButton(Widget w, Cardinal pos);
extern Widget _DtDialogBoxGetWorkArea(Widget w);
...
static void ChangeManaged(Widget manager);
...
static void GetSize(Widget manager, Dimension wa_w, Dimension wa_h,
                    Dimension bp_w, Dimension bp_h, Dimension *w, Dimension *h);
static void CreateChildren(DtDialogBoxWidget new, Widget top_widget,
                           Widget bottom_widget);
static void ButtonCallback(Widget g, XtPointer client_data,
                           XtPointer call_data);
 ...
 */
#define DB_Offset(field) XtOffset(DtDialogBoxWidget, dialog_box.field)
...
static XtResource resources[] = {
    ...
    {XmNbuttonCount, XmCButtonCount, XmRCardinal, sizeof(Cardinal),
     DB_Offset(button_count), XmRImmediate, (XtPointer)4},
    {XmNbuttonLabelStrings, XmCButtonLabelStrings, XmRXmStringTable,
     sizeof(XtPointer), DB_Offset(button_label_strings), XmRStringTable, NULL},
    {XmNcallback, XmCCallback, XmRCallback, sizeof(XtCallbackList),
     DB_Offset(callback), XmRImmediate, (XtPointer)NULL},
    {XmNshadowThickness, XmCShadowThickness, XmRHorizontalDimension,
     sizeof(Dimension),
     XtOffsetOf(struct _XmManagerRec, manager.shadow_thickness), XmRCallProc,
...
static void ClassPartInitialize(WidgetClass wc) {
        ...
        DtDialogBoxWidgetClass super = (DtDialogBoxWidgetClass)C_Superclass(wc);
...
static void Initialize(Widget request_w, Widget new_w) {
        ...
        if (M_ButtonCount(new) > 0)
                M_Button(new) =
                    (Widget *)XtMalloc(sizeof(Widget) * M_ButtonCount(new));
        else
                M_Button(new) = NULL;
        ...
        _DtProcessLock();
        create_children =
            C_CreateChildren((DtDialogBoxWidgetClass)XtClass(new));
        _DtProcessUnlock();
...
static void ChangeManaged(Widget manager) {
        ...
        Dimension w = M_Width(mgr), h = M_Height(mgr);
        ...
        if (w != M_Width(mgr) || h != M_Height(mgr)) {
                ...
                case XtGeometryAlmost:
                        XtMakeResizeRequest((Widget)mgr, w, h, NULL, NULL);
                case XtGeometryYes:
                case XtGeometryNo:
                default:
                        break;
...
*/
static void InsertChild(Widget child) {
        DtDialogBoxConstraint constraint =
            (DtDialogBoxConstraint)M_DialogBoxConstraint(child);
...
*/
static void GetSize(Widget manager, Dimension wa_w, Dimension wa_h,
                    Dimension bp_w, Dimension bp_h, Dimension *w,
                    Dimension *h) {
        DtDialogBoxWidget mgr = (DtDialogBoxWidget)manager;
        Widget work_area = M_WorkArea(mgr), separator = M_Separator(mgr);
        Widget *button = M_Button(mgr);
        register int button_count = M_ButtonCount(mgr), i;
        Boolean min_btns = M_MinimizeButtons(mgr);
        Dimension sep_h = 0, offset = OFFSET, pad, max_w = 0, max_h = 0,
...
static void CreateChildren(DtDialogBoxWidget new, Widget top_widget,
                           Widget bottom_widget) {
        Widget ref_widget, sep, btn;
        Widget *button = M_Button(new);
        int count = M_ButtonCount(new), i, l_p, r_p, off_p, b_delta, offset = 0;
        ...
        for (i = 0; i < count; i++) {
                ac = 0;
                XtSetArg(al[ac], XmNdefaultButtonShadowThickness,
                         Max(2U, s_t) / 2U);
                ac++;
                XtSetArg(al[ac], XmNshadowThickness, s_t);
                ac++;
                XtSetArg(al[ac], XmNhighlightThickness, s_t);
                ac++;
                if (!min_btns) {
                        XtSetArg(al[ac], XmNleftAttachment, XmATTACH_POSITION);
                        ac++;
                        XtSetArg(al[ac], XmNleftPosition, l_p);
                        ac++;
                        XtSetArg(al[ac], XmNleftOffset, offset);
                        ac++;
                        XtSetArg(al[ac], XmNrightAttachment, XmATTACH_POSITION);
                        ac++;
                        XtSetArg(al[ac], XmNrightPosition, r_p);
                        ac++;
                        XtSetArg(al[ac], XmNrightOffset, offset);
                        ac++;
                } else {
                        if (i == 0) {
                ...
                ac++;
                XtSetArg(al[ac], XmNbottomOffset, OFFSET + offset);
                ac++;
                if (string) {
                        XtSetArg(al[ac], XmNlabelString, string[i]);
                        ac++;
                }
                XtSetArg(al[ac], XmNchildType, XmBUTTON);
                ac++;
                sprintf(button_name, "%s_%d", M_Name(new), i + 1);
                btn =
                    XmCreatePushButtonGadget((Widget) new, button_name, al, ac);
                XtManageChild(btn);
        ...
        XtSetArg(al[ac], XmNbottomAttachment, XmATTACH_WIDGET);
        ac++;
        XtSetArg(al[ac], XmNbottomWidget, ref_widget);
        ac++;
        XtSetArg(al[ac], XmNbottomOffset, OFFSET);
        ac++;
        XtSetArg(al[ac], XmNhighlightThickness, 0);
        ac++;
        XtSetArg(al[ac], XmNchildType, XmSEPARATOR);
        ac++;
...
static void ButtonCallback(Widget g, XtPointer client_data,
        ...
        XmAnyCallbackStruct *b_cb_data = (XmAnyCallbackStruct *)call_data;
...
**-------------------------------------------------------------
*/
Widget _DtCreateDialogBox(Widget parent, char *name, ArgList arglist,
                          Cardinal argcount) {
        return (XtCreateWidget(name, dtDialogBoxWidgetClass, parent, arglist,
                               argcount));
}
...
**-------------------------------------------------------------
*/
Widget __DtCreateDialogBoxDialog(Widget ds_p, String name, ArgList db_args,
                                 Cardinal db_n) {
         ...
         */
        ds_n = 0;
        XtSetArg(ds_args[ds_n], XmNallowShellResize, True);
        ds_n++;
        ds = XtCreatePopupShell(name, xmDialogShellWidgetClass, ds_p, ds_args,
                                ds_n);

        /*  create DialogBox, free args, return
         */
        db = XtCreateWidget(name, dtDialogBoxWidgetClass, ds, db_args, db_n);
...
**-------------------------------------------------------------
*/
Widget _DtDialogBoxGetButton(Widget w, Cardinal pos) {
        DtDialogBoxWidget mgr = (DtDialogBoxWidget)w;
        int index = pos - 1;
        Widget *button = M_Button(mgr);
        Widget rtn_w = NULL;
...
**-------------------------------------------------------------
*/
Widget _DtDialogBoxGetWorkArea(Widget w) {
        DtDialogBoxWidget mgr = (DtDialogBoxWidget)w;
