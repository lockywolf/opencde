...
#define MgrParent(g)                                                           \
        ((XmManagerWidget)(XmIsManager(XtParent(g))                            \
                               ? (XtParent(g))                                 \
                               : (XtParent(XtParent(g)))))

/********    Private Function Declarations    ********/

extern void _DtIconRegisterDropsite(Widget w);

/********    End Private Function Declarations    ********/

/********    Static Function Declarations    ********/

static int IconCacheCompare(XtPointer A, XtPointer B);

static void ReCacheIcon_r(DtIconCacheObjPart *cache, DtIconGadget g);

static void GetDefaultBackground(Widget g, int offset, XrmValue *value);
static void GetDefaultForeground(Widget g, int offset, XrmValue *value);
static void GetDefaultFillMode(Widget w, int offset, XrmValue *value);
static void GetSpacing(Widget w, int offset, XtArgVal *value);
static void GetString(Widget w, int offset, XtArgVal *value);
static void IconEventHandler(Widget w, XtPointer client_data,
                             XButtonEvent *event);
static void ClickTimeout(Widget w, XtIntervalId *id);
static void IconArm(Widget w, XEvent *event);
static void IconDisarm(Widget w, XEvent *event);
static void IconActivate(Widget w, XEvent *event);
static void IconDrag(Widget w, XEvent *event);
static void IconPopup(Widget w, XEvent *event);
static void IconEnter(Widget w, XEvent *event);
static void IconLeave(Widget w, XEvent *event);
static void ClassInitialize(void);
static void SecondaryObjectCreate(Widget req, Widget new_w, ArgList args,
                                  Cardinal *num_args);
static void InitializePosthook(Widget req, Widget new, ArgList args,
                               Cardinal *num_args);
...
static void GetValuesPrehook(Widget newParent, ArgList args,
                             Cardinal *num_args);
static void GetValuesPosthook(Widget new, ArgList args, Cardinal *num_args);
static Boolean SetValuesPosthook(Widget current, Widget req, Widget new,
                                 ArgList args, Cardinal *num_args);
static void QualifyIconLocalCache(DtIconGadget g, DtIconCacheObjPart *cache);
static void Initialize(Widget request_w, Widget new_w);
static void Destroy(Widget w);
static void Resize(Widget w);
static void Redisplay(Widget w, XEvent *event, Region region);
static Boolean SetValues(Widget current_w, Widget request_w, Widget new_w);
static void BorderHighlight(DtIconGadget g);
static void BorderUnhighlight(DtIconGadget g);
static void ArmAndActivate(Widget w, XEvent *event);
static void InputDispatch(Widget w, XButtonEvent *event, Mask event_mask);
static Boolean VisualChange(Widget w, Widget current_w, Widget new_w);
static void GetSize(DtIconGadget g, Dimension *w, Dimension *h);
static void GetPositions(DtIconGadget g, Position w, Position h, Dimension h_t,
                         Dimension s_t, Position *pix_x, Position *pix_y,
                         Position *str_x, Position *str_y);
static void Draw(DtIconGadget g, Drawable drawable, Position x, Position y,
                 Dimension w, Dimension h, Dimension h_t, Dimension s_t,
                 unsigned char s_type, unsigned char fill_mode);
static void CallCallback(DtIconGadget g, XtCallbackList cb, int reason,
                         XEvent *event);
static void UpdateGCs(DtIconGadget g);
static Cardinal GetIconClassSecResData(WidgetClass class,
                                       XmSecondaryResourceData **data_rtn);
static XtPointer GetIconClassResBase(Widget widget, XtPointer client_data);
static Boolean LoadPixmap(DtIconGadget new, String pixmap);
...
extern Cardinal _XmSecondaryResourceData(XmBaseClassExt,
                                         XmSecondaryResourceData **, XtPointer,
                                         String, String, XmResourceBaseProc);
extern XtPointer _XmCachePart(XmCacheClassPartPtr cp, XtPointer cpart,
                              size_t size);

/*-------------------------------------------------------------
**	Resource List
*/
#define I_Offset(field) XtOffset(DtIconGadget, icon.field)

#define I_Cache_Offset(field) XtOffset(DtIconCacheObject, icon_cache.field)

static XtResource resources[] = {
    {XmNset, XmCSet, XmRBoolean, sizeof(Boolean), I_Offset(set), XmRImmediate,
     (XtPointer)False},
    {XmNshadowType, XmCShadowType, XmRShadowType, sizeof(unsigned char),
     I_Offset(shadow_type), XmRImmediate, (XtPointer)XmSHADOW_ETCHED_OUT},
    {XmNborderType, XmCBorderType, XmRBorderType, sizeof(unsigned char),
     I_Offset(border_type), XmRImmediate, (XtPointer)DtRECTANGLE},
    ...
    {XmNimageName, XmCString, XmRString, sizeof(XmString), I_Offset(image_name),
     XmRImmediate, (XtPointer)NULL},
    {XmNpixmap, XmCPixmap, XmRPixmap, sizeof(Pixmap), I_Offset(pixmap),
     XmRImmediate, (XtPointer)XmUNSPECIFIED_PIXMAP},
    {XmNstring, XmCXmString, XmRXmString, sizeof(XmString), I_Offset(string),
     XmRImmediate, (XtPointer)XmUNSPECIFIED_STRING},
    {XmNpixmapForeground, XmCForeground, XmRPixel, sizeof(Pixel),
     I_Offset(pixmap_foreground), XmRCallProc, (XtPointer)GetDefaultForeground},
    {XmNpixmapBackground, XmCBackground, XmRPixel, sizeof(Pixel),
     I_Offset(pixmap_background), XmRCallProc, (XtPointer)GetDefaultBackground},
    {XmNmaxPixmapWidth, XmCMaxWidth, XmRHorizontalDimension, sizeof(Dimension),
     I_Offset(max_pixmap_width), XmRImmediate, (XtPointer)0},
    {XmNmaxPixmapHeight, XmCMaxHeight, XmRVerticalDimension, sizeof(Dimension),
     I_Offset(max_pixmap_height), XmRImmediate, (XtPointer)0},
    {XmNunderline, XmCUnderline, XmRBoolean, sizeof(Boolean),
     I_Offset(underline), XmRImmediate, (XtPointer)False},
    {XmNdropSiteOperations, XmCDropSiteOperations, XmRDropSiteOperations,
     sizeof(unsigned char), I_Offset(operations), XmRImmediate,
     (XtPointer)XmDROP_NOOP},
    {XmNdropCallback, XmCDropCallback, XmRCallback, sizeof(XtCallbackList),
     I_Offset(drop_callback), XmRImmediate, (XtPointer)NULL}};

static XtResource cache_resources[] = {
    {XmNbehavior, XmCBehavior, XmRBehavior, sizeof(unsigned char),
     I_Cache_Offset(behavior), XmRImmediate, (XtPointer)XmICON_BUTTON},
    {XmNrecomputeSize, XmCRecomputeSize, XmRBoolean, sizeof(Boolean),
     I_Cache_Offset(recompute_size), XmRImmediate, (XtPointer)True},
    {"drawShadow", "DrawShadow", XmRBoolean, sizeof(Boolean),
    ...
    {XmNforeground, XmCForeground, XmRPixel, sizeof(Pixel),
     I_Cache_Offset(foreground), XmRCallProc, (XtPointer)GetDefaultForeground},
    {XmNbackground, XmCBackground, XmRPixel, sizeof(Pixel),
     I_Cache_Offset(background), XmRCallProc, (XtPointer)GetDefaultBackground},
    {XmNarmColor, XmCArmColor, XmRPixel, sizeof(Pixel),
     I_Cache_Offset(arm_color), XmRCallProc, (XtPointer)_XmSelectColorDefault},
    {XmNspacing, XmCSpacing, XmRHorizontalDimension, sizeof(Dimension),
     I_Cache_Offset(spacing), XmRImmediate, (XtPointer)UNSPECIFIED_DIMENSION},
    {XmNmarginHeight, XmCMarginHeight, XmRVerticalDimension, sizeof(Dimension),
     I_Cache_Offset(margin_height), XmRImmediate,
     (XtPointer)UNSPECIFIED_DIMENSION},
    {XmNmarginWidth, XmCMarginWidth, XmRHorizontalDimension, sizeof(Dimension),
     I_Cache_Offset(margin_width), XmRImmediate,
     (XtPointer)UNSPECIFIED_DIMENSION},
    {XmNpixmapPosition, XmCPixmapPosition, XmRPixmapPosition,
     sizeof(unsigned char), I_Cache_Offset(pixmap_position), XmRImmediate,
     (XtPointer)UNSPECIFIED_CHAR},
    {XmNstringPosition, XmCStringPosition, XmRStringPosition,
     sizeof(unsigned char), I_Cache_Offset(string_position), XmRImmediate,
     (XtPointer)UNSPECIFIED_CHAR},
    {XmNalignment, XmCAlignment, XmRAlignment, sizeof(unsigned char),
     I_Cache_Offset(alignment), XmRImmediate, (XtPointer)XmALIGNMENT_BEGINNING},
    {XmNfillMode, XmCFillMode, XmRFillMode, sizeof(unsigned char),
     I_Cache_Offset(fill_mode), XmRCallProc, (XtPointer)GetDefaultFillMode}};

static XmSyntheticResource syn_resources[] = {
    {XmNstring, sizeof(XmString), I_Offset(string), GetString,
     (XmImportProc)NULL},
};

static XmSyntheticResource cache_syn_resources[] = {
    {XmNspacing, sizeof(Dimension), I_Cache_Offset(spacing), GetSpacing,
     (XmImportProc)NULL},
    {XmNmarginWidth, sizeof(Dimension), I_Cache_Offset(margin_width),
...
**	Class Record
*/
externaldef(dticonclassrec) DtIconClassRec dtIconClassRec = {
    /*	Core Part
     */
    {
...
externaldef(dticongadgetclass)
    WidgetClass dtIconGadgetClass = (WidgetClass)&dtIconClassRec;
...
static void HighlightBorder(Widget w) {
        ...
        if (XmIsPrimitive(w)) {
                _DtProcessLock();
                border_highlight =
                    xmPrimitiveClassRec.primitive_class.border_highlight;
                _DtProcessUnlock();
                (*border_highlight)(w);
        } else if (XmIsGadget(w)) {
                _DtProcessLock();
                border_highlight =
                    xmGadgetClassRec.gadget_class.border_highlight;
                _DtProcessUnlock();
                (*border_highlight)(w);
        }
}
...
static void UnhighlightBorder(Widget w) {
        ...
        if (XmIsPrimitive(w)) {
                _DtProcessLock();
                border_unhighlight =
                    xmPrimitiveClassRec.primitive_class.border_unhighlight;
                _DtProcessUnlock();
                (*border_unhighlight)(w);
        } else if (XmIsGadget(w)) {
                _DtProcessLock();
                border_unhighlight =
                    xmGadgetClassRec.gadget_class.border_unhighlight;
                _DtProcessUnlock();
                (*border_unhighlight)(w);
        }
}
...
*/
static GC GetMaskGC(DtIconGadget g, Position x, Position y) {
        if (G_Mask(g) != XmUNSPECIFIED_PIXMAP) {

                XSetClipOrigin(XtDisplay(g), G_ClipGC(g), x, y);
                return G_ClipGC(g);
        } else {
                return G_NormalGC(g);
        }
...
static void ClickTimeout(Widget w, XtIntervalId *id) {
         ...
         * fix for bug# 4504
         */
        if (G_ClickEvent(g) == NULL)
                return;

        end_time =
            G_ClickEvent(g)->time + (Time)XtGetMultiClickTime(XtDisplay(g));
        ...
        if ((last_time < end_time) && G_Sync(g)) {
                ...
                G_ClickTimerID(g) = XtAppAddTimeOut(
                    XtWidgetToApplicationContext((Widget)g), (unsigned long)50,
                    (XtTimerCallbackProc)ClickTimeout, (XtPointer)g);
        }
...
static void IconArm(Widget w, XEvent *event) {
        ...
        if ((G_Behavior(g) == XmICON_DRAG) && (G_ShadowThickness(g) == 0)) {
                /* Do nothing */
        } else {
                XtExposeProc expose;
...
static void IconActivate(Widget w, XEvent *event) {
        ...
        if (G_Behavior(g) == XmICON_BUTTON) {
                ...
                if (G_ClickTimerID(g)) {
                        ...
                        G_ClickEvent(g) = NULL;
                        G_Armed(g) = False;
                        (*call_callback)(g, G_Callback(g), XmCR_DEFAULT_ACTION,
                                         event);
                } else {
                        if (event->type == KeyPress) {
                                ...
                                delay = (unsigned long)XtGetMultiClickTime(
                                    XtDisplay(g));
                                G_ClickEvent(g) = (XButtonEvent *)XtMalloc(
                                    sizeof(XButtonEvent));
...
static void IconEnter(Widget w, XEvent *event) {
        ...
        _XmEnterGadget(w, (XEvent *)event, (String *)NULL, (Cardinal *)0);
...
static void IconLeave(Widget w, XEvent *event) {
        ...
        _XmLeaveGadget(w, (XEvent *)event, (String *)NULL, (Cardinal *)0);
        if (G_Armed(g)) {
                if ((G_Behavior(g) == XmICON_BUTTON) ||
                    (G_Behavior(g) == XmICON_TOGGLE)) {
                        XtExposeProc expose;
...
static void SecondaryObjectCreate(Widget req, Widget new_w, ArgList args,
        ...
        XtGetSubresources(new_w, newSec, NULL, NULL, resources, num_resources,
                          args, *num_args);
...
static void InitializePosthook(Widget req, Widget new, ArgList args,
        ...
        _DtProcessLock();
        Icon_Cache(lw) = (DtIconCacheObjPart *)_XmCachePart(
            Icon_ClassCachePart(lw), (XtPointer)Icon_Cache(lw),
...
static Boolean SetValuesPrehook(Widget oldParent, Widget refParent,
        ...
        memcpy(&(newSec->icon_cache), Icon_Cache(newParent),
               sizeof(DtIconCacheObjPart));
        ...
        XtSetSubvalues((XtPointer)newSec, resources, num_resources, args,
                       *num_args);
...
static void GetValuesPrehook(Widget newParent, ArgList args,
        ...
        XtGetSubvalues((XtPointer)newSec, resources, num_resources, args,
                       *num_args);
...
static Boolean SetValuesPosthook(Widget current, Widget req, Widget new,
        ...
        {
                _XmCacheDelete(
                    (XtPointer)Icon_Cache(current)); /* delete the old one */
                Icon_Cache(new) = (DtIconCacheObjPart *)_XmCachePart(
                    Icon_ClassCachePart(new), (XtPointer)Icon_Cache(new),
                    sizeof(DtIconCacheObjPart));
        } else
...
*/
static void QualifyIconLocalCache(DtIconGadget g,
...
static void ReCacheIcon_r(DtIconCacheObjPart *local_cache, DtIconGadget g) {
        if (!IconCacheCompare((XtPointer)local_cache,
                              (XtPointer)Icon_Cache(g))) {
                _DtProcessLock();
                _XmCacheDelete(
                    (XtPointer)Icon_Cache(g)); /* delete the old one */
                Icon_Cache(g) = (DtIconCacheObjPart *)_XmCachePart(
                    Icon_ClassCachePart(g), (XtPointer)local_cache,
                    sizeof(DtIconCacheObjPart));
                _DtProcessUnlock();
        }
}
 ...
 *
 *********************************************************************/
static void GetParentBackgroundGC(DtIconGadget g) {
        XGCValues values;
        XtGCMask valueMask;
        ...
        if (XmIsManager(parent))
                ...
                values.background =
                    ((XmManagerWidget)parent)->manager.foreground;
        else
                values.background =
                    ((XmPrimitiveWidget)parent)->primitive.foreground;
...
static void Initialize(Widget request_w, Widget new_w) {
        ...
        Window root;
        int int_x = 0, int_y = 0;
        unsigned int int_w = 0, int_h = 0, int_bw, depth;
        Dimension w, h;
        EventMask mask;
        String name = NULL;
         ...
         */
        G_EventMask(new) =
            (XmARM_EVENT | XmACTIVATE_EVENT | XmMULTI_ARM_EVENT |
             XmMULTI_ACTIVATE_EVENT | XmHELP_EVENT | XmFOCUS_IN_EVENT |
             XmKEY_EVENT | XmFOCUS_OUT_EVENT | XmENTER_EVENT | XmLEAVE_EVENT);
        ...
        if (G_ImageName(new) || G_Pixmap(new)) {
                if (G_ImageName(new)) {
                         ...
                         */
                        G_Pixmap(new) = XmGetPixmap(
                            XtScreen(new), G_ImageName(new),
                            G_PixmapForeground(new), G_PixmapBackground(new));
                        if (G_Pixmap(new) != XmUNSPECIFIED_PIXMAP) {
                ...
                if (G_Pixmap(new)) {
                        XGetGeometry(XtDisplay(new), G_Pixmap(new), &root,
                                     &int_x, &int_y, &int_w, &int_h, &int_bw,
                                     &depth);
                }
                if (name) {
        ...
        if (G_ShadowType(new) != XmSHADOW_IN &&
            ...
            G_ShadowType(new) != XmSHADOW_ETCHED_OUT) {
                ...
                if (G_Behavior(new) == XmICON_BUTTON)
                        ...
                        G_ShadowType(new) = (G_Set(new)) ? XmSHADOW_ETCHED_IN
                                                         : XmSHADOW_ETCHED_OUT;
        }
        ...
        if (G_UnitType(new) != XmPIXELS) {
                G_MarginWidth(new) = XmeToHorizontalPixels(
                    (Widget) new, G_UnitType(new),
                    (XtArgVal *)((long)G_MarginWidth(new)));
                G_MarginHeight(new) = XmeToVerticalPixels(
                    (Widget) new, G_UnitType(new),
        ...
        if (G_Spacing(new) == UNSPECIFIED_DIMENSION) {
                ...
                G_Spacing(new) = (G_PixmapPosition(new) == XmPIXMAP_LEFT ||
                                  G_PixmapPosition(new) == XmPIXMAP_RIGHT)
...
static Boolean SetValues(Widget current_w, Widget request_w, Widget new_w) {
        ...
        unsigned int int_w = 0, int_h = 0, int_bw, depth;
        Dimension w, h;
        Boolean new_image_name = False, redraw_flag = False,
                draw_pixmap = False, draw_string = False, draw_shadow = False;
        Dimension h_t = G_HighlightThickness(new), s_t = G_ShadowThickness(new),
                  p_x, p_y, s_x, s_y;
        String name = NULL;
         ...
         */
        if (G_ImageName(current) != G_ImageName(new) && G_ImageName(new) &&
            G_ImageName(current) &&
        ...
        if (G_PixmapForeground(current) != G_PixmapForeground(new) ||
            G_PixmapBackground(current) != G_PixmapBackground(new)) {
                if (G_Pixmap(current) == G_Pixmap(new) &&
                    (G_ImageName(new) != NULL) && (!new_image_name)) {
                        draw_pixmap = True;
        ...
        if (new_image_name) {
                ...
                G_Pixmap(new) = XmGetPixmap(XtScreen(new), G_ImageName(new),
                                            G_PixmapForeground(new),
                                            G_PixmapBackground(new));

                if (G_Pixmap(new) != XmUNSPECIFIED_PIXMAP) {
                        G_Mask(new) =
                            (Pixmap)_DtGetMask(XtScreen(new), G_ImageName(new));
                        XGetGeometry(XtDisplay(new), G_Pixmap(new), &root,
                                     &int_x, &int_y, &int_w, &int_h, &int_bw,
                                     &depth);
                        name = G_ImageName(new);
         ...
         */
                ...
                if (G_Pixmap(new) && (G_PixmapWidth(new) == w) &&
                    (G_PixmapHeight(new) == h)) {
                        draw_pixmap = True;
                } else {
                        redraw_flag = True;
         ...
         */
                ...
                if ((draw_pixmap && G_Pixmap(new)) ||
                    (draw_string && G_String(new))) {
                        ...
                        _DtProcessUnlock();
                        (*get_positions)(new, G_Width(new), G_Height(new), h_t,
                                         G_ShadowThickness(new),
                ...
                if (draw_pixmap && G_Pixmap(new) &&
                    G_Pixmap(new) != XmUNSPECIFIED_PIXMAP) {
                        w = ((unsigned)(p_x + s_t + h_t) >= G_Width(new))
                                ? 0
                                : Min((unsigned)G_PixmapWidth(new),
                                      G_Width(new) - p_x - s_t - h_t);
                        h = ((unsigned)(p_y + s_t + h_t) >= G_Height(new))
                                ? 0
                                : Min((unsigned)G_PixmapHeight(new),
                                      G_Height(new) - p_y - s_t - h_t);
                        if (w > 0 && h > 0) {
                                XCopyArea(XtDisplay(new), G_Pixmap(new),
                                          XtWindow(new),
                                          GetMaskGC(new, G_X(new) + p_x,
                                                    G_Y(new) + p_y),
                                          0, 0, w, h, G_X(new) + p_x,
                                          G_Y(new) + p_y);
                        }
                }
                /*	Draw string with normal or armed background; clip if
                 * necessary.
                 */
                if (draw_string && G_String(new)) {
                        ...
                        clip.x = G_X(new) + s_x;
                        clip.y = G_Y(new) + s_y;
                        clip.width = (s_x + s_t + h_t >= (unsigned)G_Width(new))
                                         ? 0
                                         : Min((unsigned)G_StringWidth(new),
                                               G_Width(new) - s_x - s_t - h_t);
                        clip.height =
                            (s_y + s_t + h_t >= (unsigned)G_Height(new))
                                ? 0
                                : Min((unsigned)G_StringHeight(new),
                                      G_Height(new) - s_y - s_t - h_t);
                        if (clip.width > 0 && clip.height > 0) {
                                XmStringDrawImage(
                                    XtDisplay(new), XtWindow(new),
                                    G_FontList(new), G_String(new), gc,
                                    G_X(new) + s_x, G_Y(new) + s_y, clip.width,
                                    XmALIGNMENT_CENTER,
...
static void BorderHighlight(DtIconGadget g) {
        ...
        if (g->gadget.highlight_thickness == 0)
                return;
...
static Boolean VisualChange(Widget w, Widget current_w, Widget new_w) {
        ...
        /*	If foreground or background was the same as the parent, and
         *parent *	foreground or background has changed, then update gcs
         *and pixmap.
         */
        ...
        if (update) {
                ...
                if (G_ImageName(g) != NULL) {
                        ...
                        XmDestroyPixmap(XtScreen(w), G_Pixmap(g));
                        G_Pixmap(g) = XmGetPixmap(XtScreen(g), G_ImageName(g),
                                                  G_PixmapForeground(g),
                                                  G_PixmapBackground(g));
                        if (G_Pixmap(g) != XmUNSPECIFIED_PIXMAP)
                                G_Mask(g) = (Pixmap)_DtGetMask(XtScreen(g),
                                                               G_ImageName(g));
                        return (True);
                } else
                        return (False);
...
static void GetPositions(DtIconGadget g, Position w, Position h, Dimension h_t,
                         Dimension s_t, Position *pix_x, Position *pix_y,
                         Position *str_x, Position *str_y) {
        Dimension p_w = G_PixmapWidth(g), p_h = G_PixmapHeight(g),
                  s_w = G_StringWidth(g), s_h = G_StringHeight(g),
                  m_w = G_MarginWidth(g), m_h = G_MarginHeight(g),
                  spacing = G_Spacing(g), h_pad = s_t + h_t + m_w,
                  v_pad = s_t + h_t + m_h, width = w - 2 * h_pad,
                  height = h - 2 * v_pad;
        Position p_x = h_pad, p_y = v_pad, s_x = h_pad, s_y = v_pad;
...
static void Draw(DtIconGadget g, Drawable drawable, Position x, Position y,
        ...
        if ((fill_mode == XmFILL_SELF) || (fill_mode == XmFILL_PARENT))
                XFillRectangle(d, drawable, gc, x + h_t, y + h_t, w - 2 * h_t,
                               h - 2 * h_t);
        ...
        if (G_Pixmap(g)) {
                ...
                if (width > 0 && height > 0) {
                        if (fill_mode == XmFILL_TRANSPARENT) {
                                ...
                                case XmPIXMAP_BOTTOM:
                                        XFillRectangle(
                                            ...
                                            x + p_x - adj_x, y + p_y - adj_y,
                                            width + (2 * adj_y),
                                            height + (2 * adj_x) - (s_t + h_t));
                                        break;
                                case XmPIXMAP_LEFT:
                                case XmPIXMAP_RIGHT:
                                        XFillRectangle(
                        ...
                        XCopyArea(d, G_Pixmap(g), drawable,
                                  GetMaskGC(g, x + p_x, y + p_y), 0, 0, width,
                                  height, x + p_x, y + p_y);
                }
        }
        ...
        if (G_String(g)) {
                ...
                case XmPIXMAP_BOTTOM:
                        clip.width = (s_x + s_t + h_t >= (unsigned)G_Width(g))
                                         ? 0
                                         : Min((unsigned)G_StringWidth(g),
                                               G_Width(g) - s_x);
                        clip.height = (s_y + s_t + h_t >= (unsigned)G_Height(g))
                                          ? 0
                                          : Min((unsigned)G_StringHeight(g),
                ...
                if (clip.width > 0 && clip.height > 0) {
                        if (fill_mode == XmFILL_TRANSPARENT) {
                                ...
                                case XmPIXMAP_LEFT:
                                        ...
                                        rec_width = clip.width + (2 * adj_y) -
                                                    (s_t + h_t);
                                        if (G_PixmapPosition(g) ==
                                                XmPIXMAP_LEFT &&
                                            begin > (int)(x + p_x) &&
                                            begin < (int)(x + p_x + width)) {
                                                /*
                                                 * XmPIXMAP_LEFT -- the
                                                 * rectangle starts inside the
                                                 * pixmap
                                                 */
                                                diff = x + p_x + width - begin;
                                                begin += diff;
                                                rec_width -= diff;
                                        } else if (G_PixmapPosition(g) ==
                                                       XmPIXMAP_RIGHT &&
                                                   (rec_width + begin) >
                                                       (x + p_x)) {
                                                /*
                                                 * PIXMAP_RIGHT -- rectangle
                                                 * drawn into the pixmap
                                                 */
                                                diff = (rec_width + begin) -
                                                       (x + p_x);
                                                rec_width -= diff;
                                        }
                                        ...
                                        XFillRectangle(
                                            d, drawable,
                                            G_ParentBackgroundGC(g), begin,
                                            clip.y - adj_y, rec_width,
                                            clip.height + (2 * adj_x));
                                        break;
                                case XmPIXMAP_MIDDLE:
                                        XFillRectangle(
                        ...
                        if (G_Underline(g)) {
                                XmStringDrawUnderline(
                                    d, drawable, G_FontList(g), G_String(g), gc,
                                    x + s_x, y + s_y, clip.width,
        ...
        if (G_ShadowThickness(g) > 0 && G_DrawShadow(g)) {
                if (G_BorderType(g) == DtRECTANGLE || !G_Pixmap(g)) {
                        ...
                        XmeDrawShadows(d, drawable, M_TopShadowGC(MgrParent(g)),
                                       M_BottomShadowGC(MgrParent(g)), x + h_t,
                                       y + h_t, w - 2 * h_t, h - 2 * h_t, s_t,
                                       shadow_type);
                } else {
                        CallCallbackProc call_callback;
                        _DtProcessLock();
...
static Cardinal GetIconClassSecResData(WidgetClass class,
        ...
        arrayCount = _XmSecondaryResourceData(
            bcePtr, data_rtn, client_data, resource_name, resource_class,
...
#ifndef SVR4
                bcopy((char *)(Icon_Cache(widget)), (char *)cp,
                      icon_cache_size);
#else
                memmove((char *)cp, (char *)(Icon_Cache(widget)),
                        icon_cache_size);
#endif
        }
...
**-------------------------------------------------------------
*/
Widget _DtCreateIcon(Widget parent, String name, ArgList arglist,
                     Cardinal argcount) {
        return (
            XtCreateWidget(name, dtIconGadgetClass, parent, arglist, argcount));
}
...
**-------------------------------------------------------------
*/
Boolean _DtIconGetState(Widget w) {
        DtIconGadget g = (DtIconGadget)w;
...
**-------------------------------------------------------------
*/
void _DtIconSetState(Widget w, Boolean state, Boolean notify) {
        DtIconGadget g = (DtIconGadget)w;
        CallCallbackProc call_callback;
        XtExposeProc expose;
...
**-------------------------------------------------------------
*/
Drawable _DtIconDraw(Widget widget, Drawable drawable, Position x, Position y,
                     Boolean fill) {
        DtIconGadget g = (DtIconGadget)widget;
        Dimension h_t = G_HighlightThickness(g), w = G_Width(g) - 2 * h_t,
                  h = G_Height(g) - 2 * h_t;
        unsigned char fill_mode;
        DrawProc draw;

        if (!drawable || drawable == XmUNSPECIFIED_PIXMAP)
                drawable = (Drawable)XCreatePixmap(
                    XtDisplay(g), RootWindowOfScreen(XtScreen(g)), w, h,
                    DefaultDepthOfScreen(XtScreen(g)));
        ...
        draw = C_Draw(XtClass(g));
        _DtProcessUnlock();
        (*draw)(g, drawable, x, y, w, h, 0, G_ShadowThickness(g),
                G_ShadowType(g), fill_mode);
...

{
        ...
        XmeGetPixmapData(s, pm, NULL, NULL, NULL, NULL, NULL, NULL, &int_w,
                         &int_h);
        G_PixmapWidth(new) = Limit((Dimension)int_w, G_MaxPixmapWidth(new));
...
Widget _DtDuplicateIcon(Widget parent, Widget widget, XmString string,
                        String pixmap, XtPointer user_data, Boolean underline) {
        DtIconGadget gadget;
        int size;
        DtIconGadget new;
        Dimension h, w;
        ...
        _DtProcessLock();
        insert_child = ((CompositeWidgetClass)XtClass(parent))
...
Boolean _DtIconSelectInTitle(Widget widget, Position pt_x, Position pt_y)
 ...
 */
void _DtIconGetTextExtent_r(Widget widget, XRectangle *clip) {
        DtIconGadget g = (DtIconGadget)widget;
        Position x, y;
        Dimension w, h, h_t, s_t;
        Position p_x, p_y, s_x, s_y;
...
XRectangle *_DtIconGetTextExtent(Widget widget)
...
*/
void _DtIconGetIconRects(DtIconGadget g, unsigned char *flags,
                         XRectangle *rect1, XRectangle *rect2)

{
        ...
        get_positions = C_GetPositions(XtClass(g));
        _DtProcessUnlock();
        (*get_positions)(g, G_Width(g), G_Height(g), h_t, s_t, &p_x, &p_y, &s_x,
                         &s_y);
        *flags = 0;
        ...
        if (G_String(g)) {
                width = (s_x + s_t + h_t >= (unsigned)G_Width(g))
                            ? 0
                            : Min((unsigned)G_StringWidth(g),
                                  G_Width(g) - s_x - s_t - h_t);
                height = (s_y + s_t + h_t >= (unsigned)G_Height(g))
                             ? 0
                             : Min((unsigned)G_StringHeight(g),
                                   G_Height(g) - s_y - s_t - h_t);
                if (width > 0 && height > 0) {
 ...
 * been completed.
 */
static void AnimateCallback(Widget w, XtPointer clientData,
                            XtPointer callData) {
        DtIconGadget g = (DtIconGadget)w;
...
/* ARGSUSED */
static void TransferCallback(Widget w, XtPointer clientData,
                             XtPointer callData) {
...
*/
void _DtIconRegisterDropsite(Widget w)

{
        ...
        n++;
        XtSetArg(args[n], DtNtextIsBuffer, True);
        n++;
        XtSetArg(args[n], DtNdropAnimateCallback, animateCB);
        n++;

        DtDndDropRegister(w, DtDND_FILENAME_TRANSFER | DtDND_BUFFER_TRANSFER,
                          G_Operations(g), transferCB, args, n);
}
