...
#include <Xm/XmPrivate.h>
    ...
    /* From MenuUtilP.h */
    extern int
    _XmGrabKeyboard(Widget widget, int owner_events, int pointer_mode,
...
static void Initialize(DtComboBoxWidget request, DtComboBoxWidget new,
...
static void _ComboBoxKbdCancel(Widget w, XEvent *event, char **params,
                               Cardinal *num_params);
static void _ComboBoxPrevTabGroup(Widget w, XEvent *event, char **params,
                                  Cardinal *num_params);
static void _ComboBoxNextTabGroup(Widget w, XEvent *event, char **params,
                                  Cardinal *num_params);
...
static void Redisplay(DtComboBoxWidget w, XEvent *event, Region region);
...
static Boolean SetValues(DtComboBoxWidget current, DtComboBoxWidget request,
...
static void text_activate_cb(Widget w, XtPointer client_data,
                             XtPointer call_data);
static void text_focus_cb(Widget w, XtPointer client_data, XtPointer call_data);
static void SyncWithList(DtComboBoxPart *combo_p);
static XmImportOperator _XmSetSyntheticResForChild(Widget widget, int offset,
                                                   XtArgVal *value);
/* Converter */
...
void _DtComboBoxGetArrowSize(Widget w, int resource_offset, XtArgVal *value);
void _DtComboBoxGetListMarginHeight(Widget w, int resource_offset,
                                    XtArgVal *value);
void _DtComboBoxGetListMarginWidth(Widget w, int resource_offset,
                                   XtArgVal *value);
void _DtComboBoxGetListSpacing(Widget w, int resource_offset, XtArgVal *value);
...
#define LabelMaxLength(w)                                                      \
        XmField(w, ipot, DtComboBox, label_max_length, Dimension)
#define LabelMaxHeight(w)                                                      \
        XmField(w, ipot, DtComboBox, label_max_height, Dimension)

#define MaxShellWidth(w)                                                       \
        XmField(w, ipot, DtComboBox, max_shell_width, Dimension)
#define MaxShellHeight(w)                                                      \
        XmField(w, ipot, DtComboBox, max_shell_height, Dimension)
...
#define FocusCallback(w)                                                       \
        XmField(w, ipot, DtComboBox, focus_callback, XtCallbackList)
#define HorizontalSpacing(w)                                                   \
        XmField(w, ipot, DtComboBox, horizontal_spacing, Dimension)
...
#define ListMarginHeight(w)                                                    \
        XmField(w, ipot, DtComboBox, list_margin_height, Dimension)
#define ListMarginWidth(w)                                                     \
        XmField(w, ipot, DtComboBox, list_margin_width, Dimension)
#define ListSpacing(w) XmField(w, ipot, DtComboBox, list_spacing, Dimension)
#define LosingFocusCallback(w)                                                 \
        XmField(w, ipot, DtComboBox, losing_focus_callback, XtCallbackList)
#define TextMaxLength(w)                                                       \
        XmField(w, ipot, DtComboBox, text_max_length, unsigned int)
#define MenuPostCallback(w)                                                    \
        XmField(w, ipot, DtComboBox, menu_post_callback, XtCallbackList)
...
#define VerticalSpacing(w)                                                     \
        XmField(w, ipot, DtComboBox, vertical_spacing, Dimension)
#define VisibleItemCount(w)                                                    \
        XmField(w, ipot, DtComboBox, visible_item_count, int)
...
static XmPartResource resources[] = {
    {XmNshadowThickness, XmCShadowThickness, XmRHorizontalDimension,
     sizeof(Dimension), offset(manager.shadow_thickness), XmRImmediate,
     (XtPointer)TEXT_FIELD_SHADOW},

    /*
     * ComboBox specific resources
     */
    {DtNactivateCallback, DtCCallback, XmRCallback, sizeof(XtCallbackList),
     DtOffset(activate_callback), XmRCallback, (XtPointer)NULL},
    {DtNalignment, DtCAlignment, XmRAlignment, sizeof(unsigned char),
     DtOffset(alignment), XmRImmediate, (XtPointer)DtALIGNMENT_END},
    {DtNarrowSpacing, DtCArrowSpacing, XmRHorizontalDimension,
     sizeof(Dimension), DtOffset(arrow_spacing), XmRImmediate, (XtPointer)0},
    {DtNarrowType, DtCArrowType, DtRArrowType, sizeof(unsigned char),
     DtOffset(arrow_type), XmRImmediate, (XtPointer)DtMOTIF},
    {DtNcolumns, DtCColumns, XmRShort, sizeof(short), DtOffset(text_columns),
     XmRImmediate, (XtPointer)20},
    {DtNfocusCallback, DtCCallback, XmRCallback, sizeof(XtCallbackList),
     DtOffset(focus_callback), XmRCallback, (XtPointer)NULL},
    {DtNhorizontalSpacing, DtCHorizontalSpacing, XmRHorizontalDimension,
     sizeof(Dimension), DtOffset(horizontal_spacing), XmRImmediate,
     (XtPointer)INVALID_DIMENSION},
    {DtNitemCount, DtCItemCount, XmRInt, sizeof(int), DtOffset(item_count),
    ...
    {DtNlist, DtCList, XmRWidget, sizeof(Widget), DtOffset(list), XmRImmediate,
     (XtPointer)NULL},
    ...
    {DtNlistMarginHeight, DtCListMarginHeight, XmRVerticalDimension,
     sizeof(Dimension), DtOffset(list_margin_height), XmRImmediate,
     (XtPointer)MARGIN},
    {DtNlistMarginWidth, DtCListMarginWidth, XmRHorizontalDimension,
     sizeof(Dimension), DtOffset(list_margin_width), XmRImmediate,
     (XtPointer)MARGIN},
    ...
    {DtNlosingFocusCallback, DtCCallback, XmRCallback, sizeof(XtCallbackList),
     DtOffset(losing_focus_callback), XmRCallback, (XtPointer)NULL},
    ...
    {DtNpoppedUp, DtCPoppedUp, XmRBoolean, sizeof(Boolean), DtOffset(popped_up),
    ...
    {DtNtextField, DtCTextField, XmRWidget, sizeof(Widget), DtOffset(text),
    ...
    {DtNverticalSpacing, DtCVerticalSpacing, XmRVerticalDimension,
     sizeof(Dimension), DtOffset(vertical_spacing), XmRImmediate,
     (XtPointer)INVALID_DIMENSION},
};
...
static XmSyntheticResource syn_resources[] = {
    ...
    {DtNitems, sizeof(XmStringTable), DtOffset(items), _DtComboBoxGetListItems,
    ...
    {DtNlistMarginHeight, sizeof(Dimension), DtOffset(list_margin_height),
...
externaldef(dtcomboBoxwidgetclass)
...
static void Initialize(DtComboBoxWidget request, DtComboBoxWidget new,
        ...
        DtComboBoxPart *combo_p =
            (DtComboBoxPart *)&(XmField(new, ipot, DtComboBox, arrow, Widget));
        ...
        if (Type(new) == DtDROP_DOWN_COMBO_BOX) {
                n = 0;
                XtSetArg(args[n], XmNcolumns, TextColumns(new));
                n++;
                XtSetArg(args[n], XmNmaxLength, TextMaxLength(new));
                n++;
                XtSetArg(args[n], XmNmarginWidth, TEXT_CONTEXT_MARGIN);
                n++;
                XtSetArg(args[n], XmNmarginHeight, 2);
                n++;
                /* Resolution Independent */
                if (unit_type != XmPIXELS) {
                        XtSetArg(args[n], XmNunitType, XmPIXELS);
                        n++;
                }
                Text(new) = XtCreateManagedWidget(
                    "Text", xmTextFieldWidgetClass, (Widget) new, args, n);
                XtAddCallback(Text(new), XmNlosingFocusCallback,
                              text_losing_focus_cb, (XtPointer) new);
                XtAddCallback(Text(new), XmNactivateCallback, text_activate_cb,
                              (XtPointer) new);
                XtAddCallback(Text(new), XmNfocusCallback, text_focus_cb,
                              (XtPointer) new);
                ...
                n = 0;
                XtSetArg(args[n], XmNalignment, Alignment(new));
                n++;
                XtSetArg(args[n], XmNrecomputeSize, FALSE);
                n++;
                XtSetArg(args[n], XmNlabelString, InitLabel);
                n++;
                XtSetArg(args[n], XmNmarginLeft, LABEL_PADDING);
                n++;
                XtSetArg(args[n], XmNmarginRight, LABEL_PADDING);
                n++;
                XtSetArg(args[n], XmNmarginWidth, TEXT_CONTEXT_MARGIN);
                n++;
                XtSetArg(args[n], XmNmarginHeight, 0);
                n++;
                XtSetArg(args[n], XmNstringDirection, new_direction);
                n++;
                /* Resolution Independent */
                if (unit_type != XmPIXELS) {
                        XtSetArg(args[n], XmNunitType, XmPIXELS);
                ...
                Label(new) = XtCreateManagedWidget("Label", xmLabelWidgetClass,
                                                   (Widget) new, args, n);
        ...
        if (ArrowType(new) == DtMOTIF) {
                XtSetArg(args[n], XmNarrowDirection, XmARROW_DOWN);
                n++;
                XtSetArg(args[n], XmNforeground, BackgroundPixel(new));
                n++;
         ...
         */
        n = 0;
        XtSetArg(args[n], XtNoverrideRedirect, TRUE);
        n++;
        ...
        n++;
        Shell(new) =
            XtCreatePopupShell("ComboBoxMenuShell", topLevelShellWidgetClass,
                               (Widget) new, args, n);

        n = 0;
        Frame(new) = XtCreateManagedWidget(
            "ComboBoxRowColumn", xmFrameWidgetClass, Shell(new), args, n);

        n = 0;
        /* Store combo widget in list for later use */
        XtSetArg(args[n], XmNuserData, (XtPointer) new);
        n++;
        if (ListFontList(new)) {
                XtSetArg(args[n], XmNfontList, ListFontList(new));
                n++;
        }
        /* to disable double click */
        XtSetArg(args[n], XmNdoubleClickInterval, 0);
        n++;
        XtSetArg(args[n], XmNitemCount, ItemCount(new));
        n++;
        XtSetArg(args[n], XmNitems, Items(new));
        n++;
        XtSetArg(args[n], XmNlistMarginHeight, ListMarginHeight(new));
        n++;
        XtSetArg(args[n], XmNlistMarginWidth, ListMarginWidth(new));
        n++;
        XtSetArg(args[n], XmNlistSpacing, ListSpacing(new));
        n++;
        {
                XmStringDirection new_direction =
                    XmDirectionToStringDirection(LayoutDirection(new));
        ...
        XtSetArg(args[n], XmNtopItemPosition, TopItemPosition(new));
        n++;
        XtSetArg(args[n], XmNvisibleItemCount, VisibleItemCount(new));
        n++;
        ...
        n = 0;
        XtSetArg(args[n], XmNshadowThickness, (XtArgVal)0);
        n++;
        ...
        if (SelectedItem(new) && XmeStringIsValid(SelectedItem(new))) {
                ...
                if (SelectedPosition(new) < 0 ||
                    SelectedPosition(new) >= ItemCount(new))
                        SelectedPosition(new) = 0;
                if (ItemCount(new))
                        XmListSelectPos(List(new), SelectedPosition(new) + 1,
                                        FALSE);
        }
        if (unit_type != XmPIXELS) {
                XtSetArg(args[0], XmNunitType, unit_type);
...
static void CheckResources(DtComboBoxWidget combo) {
        ...
        if ((Orientation(combo) != DtLEFT) && (Orientation(combo) != DtRIGHT)) {
...
static void SetComboBoxSize(DtComboBoxWidget combo) {
        Widget text_holder =
            ((Type(combo) == DtDROP_DOWN_COMBO_BOX) ? Text(combo)
                                                    : Label(combo));
        ...
        ResizeResult = XtMakeResizeRequest(
            (Widget)combo,
            arrow_width + sep_width + ArrowSpacing(combo) + text_width +
                shadow + h_spacing + (COMBO_MARGIN_W(combo) * 2),
            text_height + shadow + v_spacing + (COMBO_MARGIN_H(combo) * 2),
            NULL, NULL);
...
static void ForceChildSizes(DtComboBoxWidget combo) {
        ...
        if (Type(combo) == DtDROP_DOWN_LIST) { /** label **/
                ...
                if ((available_width != tmp_width) ||
                    (available_height != tmp_height))
                        XtResizeWidget(Label(combo), available_width,
                                       available_height, tmp_borderwidth);
                /* Motif BC */
                XtGetValues((Widget)Sep(combo), args, 3);
                if (full_available_height != tmp_height)
                        XtResizeWidget(Sep(combo), tmp_width,
                                       full_available_height, tmp_borderwidth);
        } else {
                /* Motif BC */
                XtGetValues((Widget)Text(combo), args, 3);
                if (Width(((Widget)Text(combo))) !=
                    available_width) /** TextField **/
                        XtResizeWidget(Text(combo), available_width, tmp_height,
                                       tmp_borderwidth);
        }
        ...
        if ((arrow_width != tmp_width) || (tmp_height != available_height)) {
                available_height = (available_height < ARROW_MIN)
                                       ? ARROW_MIN
                                       : available_height;
...
static void LayoutChildren(DtComboBoxWidget combo) {
        ...
        if (Orientation(combo) == DtLEFT) {
                ...
                start_x += tmp_width;
                if (Type(combo) == DtDROP_DOWN_LIST) {
                        XtMoveWidget(Sep(combo), start_x,
                                     start_y - COMBO_V_SPACING(combo));
                        start_x += sep_width;
                }
...
static Boolean SetValues(DtComboBoxWidget current, DtComboBoxWidget request,
        ...
        if (MUnitType(new) != XmPIXELS) {
                ...
                XtSetArg(args[0], XmNunitType, XmPIXELS);
                if (Arrow(new))
                        XtSetValues(Arrow(new), args, 1);
                if (List(new))
                        XtSetValues(List(new), args, 1);
                if (Shell(new))
                        XtSetValues(Shell(new), args, 1);
                if (Label(new))
                        XtSetValues(Label(new), args, 1);
                if (Text(new))
                        XtSetValues(Text(new), args, 1);
        ...
        if (MUnitType(current) != XmPIXELS) {
                ...
                XtSetArg(args[0], XmNunitType, XmPIXELS);
                if (Arrow(current))
                        XtSetValues(Arrow(current), args, 1);
                if (List(current))
                        XtSetValues(List(current), args, 1);
                if (Shell(current))
                        XtSetValues(Shell(current), args, 1);
                if (Label(current))
                        XtSetValues(Label(current), args, 1);
                if (Text(current))
                        XtSetValues(Text(current), args, 1);
                if (Sep(current))
                        XtSetValues(Sep(current), args, 1);
        }
        ...
        if (ArrowType(new) != ArrowType(current)) {
                ...
                if (ArrowType(new) == DtMOTIF) {
                        ...
                        XtSetArg(args[n], XmNforeground, BackgroundPixel(new));
                        n++;
                        Arrow(new) = XtCreateManagedWidget(
                            "ComboBoxArrow", xmArrowButtonWidgetClass,
                            (Widget) new, args, n);
                } else {
                        Arrow(new) = XtCreateManagedWidget(
                            "ComboBoxArrow", xmDrawnButtonWidgetClass,
                            (Widget) new, args, n);
                        XtAddCallback(Arrow(new), XmNexposeCallback,
                                      arrow_expose_cb, (XtPointer) new);
                }
                XtAddCallback(Arrow(new), XmNactivateCallback, activate_cb,
                              (XtPointer) new);
        ...
        if (Type(new) != Type(current)) {
                if (Type(new) == DtDROP_DOWN_COMBO_BOX) {
                        if (Text(new) == NULL) {
                                ...
                                XtSetArg(args[n], XmNmarginHeight, 2);
                                n++;
                                Text(new) = XtCreateWidget(
                                    "ComboBoxTextField", xmTextFieldWidgetClass,
                                    (Widget) new, args, n);
                                XtAddCallback(Text(new), XmNlosingFocusCallback,
                                              text_losing_focus_cb,
                                              (XtPointer) new);
                                ...
                                if (HorizontalSpacing(new) ==
                                    HorizontalSpacing(current))
                                        HorizontalSpacing(new) = 0;
                                if (VerticalSpacing(new) ==
                                    VerticalSpacing(current))
                                        VerticalSpacing(new) = 0;
                        }
                        ...
                        if (Label(new) == NULL) {
                                ...
                                XtSetArg(args[n], XmNstringDirection,
                                         new_direction);
                                n++;
                                Label(new) = XtCreateWidget(
                                    "ComboBoxLabel", xmLabelWidgetClass,
                                    (Widget) new, args, n);
                                XtOverrideTranslations((Widget)Label(new),
                                                       label_trans);
                                if (HorizontalSpacing(new) ==
                                    HorizontalSpacing(current))
                                        HorizontalSpacing(new) = 1;
                                ...
                                XtSetArg(args[n], XmNorientation, XmVERTICAL);
                                n++;
                                Sep(new) = XtCreateWidget(
                                    "ComboBoxSeparator", xmSeparatorWidgetClass,
                                    (Widget) new, args, n);
                        } else if (LayoutDirection(new) !=
                                   LayoutDirection(current)) {
                                XmStringDirection new_direction =
                                    XmDirectionToStringDirection(
                                        LayoutDirection(new));
                ...
                if (COMBO_SHADOW(new) == COMBO_SHADOW(current))
                        COMBO_SHADOW(new) =
                            ((Type(new) == DtDROP_DOWN_COMBO_BOX)
                                 ? TEXT_FIELD_SHADOW
                                 : LABEL_SHADOW);
        }
        ...
        if ((Items(new) != ListItems(current)) ||
            ...
            (XmStringByteCompare(SelectedItem(new), SelectedItem(current)) !=
             True) ||
            (Label(new) != Label(current)) ||
                ...
                if (SelectedItem(new) && XmeStringIsValid(SelectedItem(new)) &&
                    (XmStringByteCompare(SelectedItem(new),
                        ...
                        if (SelectedPosition(new) < 0 ||
                            SelectedPosition(new) >= ItemCount(new))
                                SelectedPosition(new) = 0;
                        if ((ItemCount(new)) && (SelectedPosition(new) !=
                                                 SelectedPosition(current))) {

                                XmListSelectPos(List(new),
                                                SelectedPosition(new) + 1,
                                                FALSE);
                                if (SelectedItem(current)) {
                                ...
                                SyncWithList(new_p);
                                SelectedItem(new) = XmStringCopy(
                                    (ListItems(new))[SelectedPosition(new)]);
                        }

                        /* Else, same item selected as last time */
                        else {
                                SelectedItem(new) =
                                    XmStringCopy(SelectedItem(new));
                                if ((SelectedItem(new) != NULL) &&
                                    (ListItems(new) != NULL))
                                        DtComboBoxSelectItem((Widget) new,
                                                             SelectedItem(new));
                                if (SelectedItem(current)) {
                                        XmStringFree(SelectedItem(current));
                                        SelectedItem(current) = NULL;
        ...
        if ((shadow > 0) && XtIsRealized((Widget)w)) {
                if (all) {
                        XClearArea(XtDisplayOfObject((Widget)w),
                                   ...
                                   margin_h, OldWidth(w) - (margin_w * 2),
                                   shadow, FALSE);
                        XClearArea(XtDisplayOfObject((Widget)w),
                                   XtWindowOfObject((Widget)w), margin_w,
                                   margin_h, shadow,
                ...
                XClearArea(XtDisplayOfObject((Widget)w),
                           ...
                           OldWidth(w) - margin_w - shadow, margin_h, shadow,
                           OldHeight(w) - (margin_h * 2), FALSE);
        }
...
static void DrawShadow(DtComboBoxWidget w) {
        ...
        if ((shadow > 0) && XtIsRealized((Widget)w)) {
                XmeDrawShadows(
                    XtDisplayOfObject((Widget)w), XtWindowOfObject((Widget)w),
                    TopShadowGC(w), BottomShadowGC(w), margin_w, margin_h,
                    Width(w) - (margin_w * 2), Height(w) - (margin_h * 2),
                    shadow, XmSHADOW_OUT);
        }
...
static void SetTextFieldData(DtComboBoxPart *combo_p, XmString item) {
        ...
        if (item_count && item_count > combo_p->selected_position && !item &&
            list_items) {
        ...
        if (!item) {
                ...
                while (!done) {
                        type = XmStringGetNextComponent(context, &text,
                                                        &charset, &direction,
                                                        &unknown_tag, &ul, &uv);
                        switch (type) {
...
static void SetMaximumLabelSize(DtComboBoxPart *combo_p) {
        ...
        /* Resolution Independent */
        XtSetArg(args[0], XmNunitType, &unit_type);
        XtGetValues(combo_p->list, args,
                    1); /* Assume list/Combo has same uniType*/
        if (unit_type != XmPIXELS) {
                XtSetArg(args[0], XmNunitType, XmPIXELS);
        ...
        if (item_count && item_count >= combo_p->item_count && list_items &&
            combo_p->update_label) {
                /*
                 * Loop through all the items to find the biggest dimensions
                 */
                for (i = 0; i < combo_p->item_count; i++) {
                        XmStringExtent(font_list, list_items[i], &width,
                                       &height);
                        longest = (width > longest) ? width : longest;
                        highest = (height > highest) ? height : highest;
                }
        } else {
                XmStringExtent(font_list, combo_p->label_string, &longest,
                               &highest);
        }

        combo_p->label_max_length =
...
static void SetLabelData(DtComboBoxPart *combo_p, XmString item,
        ...
        if (force_label_string || (combo_p->update_label == FALSE))
                item =
                    combo_p->label_string ? combo_p->label_string : InitLabel;
        else {
                if (!item) {
                        /* MotifBc
                         */
...
static void select_cb(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        if (ShellPoppedUp(((ShellWidget)Shell(combo_w))) == FALSE) {
                 ...
                 *     1. XmList's XmNdefaultActionCallback which is activated
                 *by KSelect, will first pop down the shell (see above) and and
                 *then get here.
                 *     2. BSelect on XmList, which first call the
                 *list_event_handler() (for the handler is registered as
                 *LIST_HEAD) to pop down the shell, and then call
                 *XmList's XmNbrowseSelectionCallback which in turn get
                 *here
                 */
...
static void shell_event_handler(Widget widget, XtPointer client_data,
        ...
        XmScrolledWindowWidget sw =
            (XmScrolledWindowWidget)XtParent(List(combo_w));
         ...
         *             the cursor is outside the scrollbar(horizontal or
         * vertical) action:     call action routine "Release" of ScrollBar to
         * release possible pressed slider arrow (up/down)
         */
        if (event->type == ButtonRelease) {
                if (ScrollBarVisible((Widget)h_scrollbar) &&
                    window != XtWindowOfObject((Widget)h_scrollbar))
                        XtCallActionProc((Widget)h_scrollbar, "Release", event,
                                         NULL, 0);
                if (ScrollBarVisible((Widget)v_scrollbar) &&
                    window != XtWindowOfObject((Widget)v_scrollbar))
                        XtCallActionProc((Widget)v_scrollbar, "Release", event,
                                         NULL, 0);
        }
}
 ...
 */
static void
list_event_handler(Widget widget, XtPointer client_data, XEvent *event,
                   Boolean *dispatch) /* call remaining event handlers */
{
        ...
        case ButtonRelease:
                 ...
                 * XmNbrowseSelectionCallback is the last one in this popup
                 * session and must treat it as a DtNselectionCallback
                 */
                input_ungrab(combo_w, GRAB_KEYBOARD);

                /* condition : ButtonRelease event
                 *             cursor is outside the scrollbar(horizontal or
                 * vertical) action:     call action routine "Release" of
                 * ScrollBar to release possible pressed slider arrow (up/down)
                 */
                /* MotifBc */
                XtSetArg(args[0], XmNverticalScrollBar, &v_scrollbar);
                XtSetArg(args[1], XmNhorizontalScrollBar, &h_scrollbar);
        ...
        case FocusOut:
                 ...
                 */
                if (ShellPoppedUp(((ShellWidget)(Shell(combo_w))))) {
                        _XmGrabKeyboard(widget, False, GrabModeAsync,
        ...
        case EnterNotify:
                /* call ListBeginSelect() action directly to simulate Btn1Down*/
                {
                        ...
                        XQueryPointer(disp, win, &root, &child, &root_x,
                                      &root_y, &win_x, &win_y, &keys_buttons);
                        if (keys_buttons & Button1Mask) {
                                item_pos = XmListYToPos(List(combo_w),
                                                        event->xbutton.y);
                                list = (XmListWidget)List(combo_w);
                                /* MotifBc
                                 */
                                num_args = 0;
                                XtSetArg(args[num_args], XmNtopItemPosition,
                                         &top_item_position);
                                num_args++;
                                XtSetArg(args[num_args], XmNvisibleItemCount,
                                         &visible_item_count);
                                num_args++;
                                XtSetArg(args[num_args], XmNitemCount,
                                         &item_count);
                                num_args++;
                                XtGetValues((Widget)list, args, num_args);
                                /* Adjust top_item_position value to its'
                                 * internal form */
...
static void TextFieldActivate(DtComboBoxPart *combo_p, XtPointer call_data) {
        ...
        /* Only send callback if both are not NULL */
                ...
                /* MotifBc
                 */
                if (XtHasCallbacks((Widget)w, XmNactivateCallback) ==
                    XtCallbackHasSome)
...
static void activate_cb(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        XtTranslateCoords((Widget)combo_w, X(combo_w), Y(combo_w), &root_x,
                          &root_y);
        root_y0 = root_y;
        ...
        if (width < Width(combo_w)) {
                ...
                Widget sb;
                n = 0;
                XtSetArg(args[n], XmNverticalScrollBar, &sb);
                n++;
                ...
                if (sb && XtIsWidget(sb)) {
                        n = 0;
                        XtSetArg(args[n], XmNwidth, &sb_width);
                        n++;
                        XtGetValues(sb, args, n);
                        /* Resolution Independent Resource */
                        if (unit_type != XmPIXELS) {
                                n = XmConvertUnits(sb, XmHORIZONTAL,
                                                   (int)unit_type, sb_width,
                                                   XmPIXELS);
                                sb_width = n;
                        }
                } else
                ...
                width = Width(combo_w);
                list_width =
                    (ScrollBarVisible(sb) ? (width - sb_width - spacing)
                                          : (width - spacing));

                if (unit_type != XmPIXELS) {
                        n = XmConvertUnits(sb, XmHORIZONTAL, XmPIXELS,
                                           list_width, (int)unit_type);
                        list_width = n;
                        n = 0;
                }
                XtSetArg(args[n], XmNwidth, list_width);
                n++;
        ...
        n = 0;
        XtSetArg(args[n], XmNx, root_x);
        n++;
        XtSetArg(args[n], XmNy, root_y);
        n++;
        ...
        _XmGrabPointer(Shell(combo_w), True,
                       ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
                       GrabModeAsync, None, XmGetMenuCursor(disp), CurrentTime);
...
static void text_losing_focus_cb(Widget w, XtPointer client_data,
        ...
        xmStr =
            (XmString)XmStringCreateLocalized(text_string ? text_string : "");
        if (text_string)
                XtFree(text_string);
...
static void text_activate_cb(Widget w, XtPointer client_data,
        ...
        if (ActivateCallback(combo))
                XtCallCallbackList((Widget)combo, ActivateCallback(combo),
                                   (XtPointer)call_data);
...
static void SyncWithList(DtComboBoxPart *combo_p) {
         ...
         */
        num_args = 0;
        XtSetArg(args[num_args], XmNitems, &(combo_p->list_items));
        num_args++;
        XtSetArg(args[num_args], XmNitems, &(combo_p->items));
        num_args++;
        XtSetArg(args[num_args], XmNitemCount, &(combo_p->item_count));
        num_args++;
        XtSetArg(args[num_args], XmNfontList, &(combo_p->list_font_list));
        num_args++;
        XtSetArg(args[num_args], XmNlistMarginHeight,
                 &(combo_p->list_margin_height));
        num_args++;
        XtSetArg(args[num_args], XmNlistMarginWidth,
                 &(combo_p->list_margin_width));
        num_args++;
        XtSetArg(args[num_args], XmNlistSpacing, &(combo_p->list_spacing));
        num_args++;
        XtSetArg(args[num_args], XmNtopItemPosition,
                 &(combo_p->top_item_position));
        num_args++;
        XtSetArg(args[num_args], XmNvisibleItemCount,
                 &(combo_p->visible_item_count));
        num_args++;
        XtGetValues((Widget)list, args, num_args);
 ...
 */
Widget DtCreateComboBox(Widget parent, char *name, ArgList arglist,
                        Cardinal num_args) {
        return (XtCreateWidget(name, dtComboBoxWidgetClass, parent, arglist,
                               num_args));
}

void DtComboBoxAddItem(Widget combow, XmString item, int pos, Boolean unique) {
...
void DtComboBoxDeletePos(Widget combow, int pos) {
        DtComboBoxWidget combo = (DtComboBoxWidget)combow;
        DtComboBoxPart *combo_p = (DtComboBoxPart *)&(
            XmField(combo, ipot, DtComboBox, arrow, Widget));
        ...
        if (pos == (combo_p->selected_position + 1)) {
                ...
                if (pos == combo_p->item_count &&
                    combo_p->selected_position > 0) {
                        combo_p->selected_position--;
                        selection_changed++;
                }
                XmListSelectPos(List(combo), combo_p->selected_position + 1,
                                True);
        ...
        if (selection_changed) {
                ...
                combo_p->selected_item =
                    ItemCount(combo)
                        ? XmStringCopy(
                              combo_p->list_items[combo_p->selected_position])
                        : (XmString)NULL;
        }
...
void DtComboBoxSetItem(Widget combow, XmString item) {
        DtComboBoxWidget combo = (DtComboBoxWidget)combow;
        DtComboBoxPart *combo_p = (DtComboBoxPart *)&(
            XmField(combo, ipot, DtComboBox, arrow, Widget));
        XmStringTable list_items;
...
void DtComboBoxSelectItem(Widget combow, XmString item) {
        DtComboBoxWidget combo = (DtComboBoxWidget)combow;
        DtComboBoxPart *combo_p = (DtComboBoxPart *)&(
            XmField(combo, ipot, DtComboBox, arrow, Widget));
        XmStringTable list_items;
...
void _DtComboBoxGetLabelString(Widget w, int resource_offset, XtArgVal *value) {
        DtComboBoxWidget combo = (DtComboBoxWidget)w;
...
void _DtComboBoxGetListItemCount(Widget w, int resource_offset,
                                 XtArgVal *value) {
        DtComboBoxWidget combo = (DtComboBoxWidget)w;
        int data;
        Arg arg;
...
void _DtComboBoxGetListItems(Widget w, int resource_offset, XtArgVal *value) {
        DtComboBoxWidget combo = (DtComboBoxWidget)w;
        XmStringTable data;
        Arg arg;
...
void _DtComboBoxGetListFontList(Widget w, int resource_offset,
                                XtArgVal *value) {
        DtComboBoxWidget combo = (DtComboBoxWidget)w;
        XmFontList data;
        Arg arg;
...
void _DtComboBoxGetListTopItemPosition(Widget w, int resource_offset,
                                       XtArgVal *value) {
        DtComboBoxWidget combo = (DtComboBoxWidget)w;
        int data;
        Arg arg;
...
void _DtComboBoxGetListVisibleItemCount(Widget w, int resource_offset,
                                        XtArgVal *value) {
        DtComboBoxWidget combo = (DtComboBoxWidget)w;
        int data;
        Arg arg;
