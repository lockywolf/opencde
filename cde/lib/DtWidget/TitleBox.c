...
extern Widget _DtCreateTitleBox(Widget parent, char *name, ArgList arglist,
                                Cardinal argcount);
extern Widget _DtTitleBoxGetTitleArea(Widget w);
extern Widget _DtTitleBoxGetWorkArea(Widget w);
...
static void ConfigureChildren(DtTitleBoxWidget manager);
static void GetSize(DtTitleBoxWidget manager, Dimension ta_w, Dimension ta_h,
                    Dimension wa_w, Dimension wa_h, Dimension *w, Dimension *h);
static void GetTitleString(DtTitleBoxWidget manager, XrmQuark resource,
                           XtArgVal *value);
static void ClassInitialize(void);
static void ClassPartInitialize(WidgetClass wc);
static void Initialize(DtTitleBoxWidget request, DtTitleBoxWidget new);
static void Destroy(DtTitleBoxWidget manager);
static void Resize(DtTitleBoxWidget manager);
static void Redisplay(DtTitleBoxWidget manager, XEvent *event, Region region);
static Boolean SetValues(DtTitleBoxWidget current, DtTitleBoxWidget request,
                         DtTitleBoxWidget new);
static XtGeometryResult QueryGeometry(DtTitleBoxWidget manager,
                                      XtWidgetGeometry *request,
                                      XtWidgetGeometry *reply);
static XtGeometryResult GeometryManager(Widget kid, XtWidgetGeometry *request,
                                        XtWidgetGeometry *reply);
static void ChangeManaged(DtTitleBoxWidget manager);
 ...
 */
#define TB_Offset(field) XtOffset(DtTitleBoxWidget, title_box.field)
...
static XtResource resources[] = {
    {XmNshadowThickness, XmCShadowThickness, XmRShort, sizeof(short),
     XtOffset(DtTitleBoxWidget, manager.shadow_thickness), XmRImmediate,
     (XtPointer)UNSPECIFIED_DIMENSION},
    {XmNmarginWidth, XmCMarginWidth, XmRHorizontalDimension, sizeof(Dimension),
     TB_Offset(margin_width), XmRImmediate, (XtPointer)UNSPECIFIED_DIMENSION},
    {XmNmarginHeight, XmCMarginHeight, XmRVerticalDimension, sizeof(Dimension),
     TB_Offset(margin_height), XmRImmediate, (XtPointer)UNSPECIFIED_DIMENSION},
    {XmNshadowType, XmCShadowType, XmRShadowType, sizeof(unsigned char),
     TB_Offset(shadow_type), XmRImmediate, (XtPointer)UNSPECIFIED_CHAR},
    {XmNtitleSpacing, XmCTitleSpacing, XmRHorizontalDimension,
     sizeof(Dimension), TB_Offset(title_spacing), XmRImmediate,
     (XtPointer)UNSPECIFIED_DIMENSION},
    {XmNtitlePosition, XmCTitlePosition, XmRTitlePosition,
     sizeof(unsigned char), TB_Offset(title_position), XmRImmediate,
     (XtPointer)XmTITLE_TOP},
    {XmNtitleAlignment, XmCAlignment, XmRAlignment, sizeof(unsigned char),
     TB_Offset(title_alignment), XmRImmediate,
     (XtPointer)XmALIGNMENT_BEGINNING},
    {XmNtitleString, XmCXmString, XmRXmString, sizeof(XmString),
     TB_Offset(title_string), XmRImmediate, (XtPointer)XmUNSPECIFIED_STRING},
...
static XmSyntheticResource syn_resources[] = {
    {XmNtitleString, sizeof(XmString), TB_Offset(title_string),
     (XmExportProc)GetTitleString, (XmImportProc)NULL},
    {XmNmarginWidth, sizeof(Dimension), TB_Offset(margin_width),
...
static void ConfigureChildren(DtTitleBoxWidget manager) {
        ...
        Dimension ta_w = 0, ta_h = 0, ta_bw = 0, wa_w = 0, wa_h = 0, wa_bw = 0,
                  w = M_Width(manager), h = M_Height(manager),
                  s_t = M_ShadowThickness(manager),
                  m_w = M_MarginWidth(manager), m_h = M_MarginHeight(manager),
        ...
        if (work_area) {
                ...
                XmeConfigureObject(work_area, wa_x, wa_y, wa_w, wa_h, wa_bw);
        }
}
...
*/
static void GetSize(DtTitleBoxWidget manager, Dimension ta_w, Dimension ta_h,
                    Dimension wa_w, Dimension wa_h, Dimension *w,
                    Dimension *h) {
        Widget title_area = M_TitleArea(manager),
               work_area = M_WorkArea(manager);
...
*/
static void ClassInitialize(void) { _DtRegisterNewConverters(); }
...
static void Initialize(DtTitleBoxWidget request, DtTitleBoxWidget new) {
        ...
        if (M_Width(new) == 0) {
                M_Width(new) =
                    2 * (M_ShadowThickness(new) + M_MarginWidth(new));
                if (M_Width(new) == 0)
                        M_Width(new) = 10;
        }
        if (M_Height(new) == 0) {
                M_Height(new) =
                    2 * (M_ShadowThickness(new) + M_MarginHeight(new));
                if (M_Height(new) == 0)
                        M_Height(new) = 10;
        }
        ...
        ac = 0;
        XtSetArg(al[ac], XmNchildType, XmTITLE_AREA);
        ac++;
        ...
        ac++;
        XtSetArg(al[ac], XmNlabelString, M_TitleString(new));
        ac++;
        if (M_FontList(new)) {
...
static void Resize(DtTitleBoxWidget manager) {
        Widget title_area = M_TitleArea(manager);
        Dimension s_t = M_OldShadowThickness(manager), w = M_OldWidth(manager),
        ...
        if (XtIsRealized((Widget)manager)) {
                ...
                if ((M_TitlePosition(manager) == XmTITLE_BOTTOM) &&
                    (M_Height(manager) > h)) {
                        ...
                        XClearArea(XtDisplay(manager), XtWindow(manager), 0, y,
                                   M_Width(manager), M_Height(manager) - y,
                                   False);
                }
        }
...
static void Redisplay(DtTitleBoxWidget manager, XEvent *event, Region region) {
        Widget title_area = M_TitleArea(manager),
               work_area = M_WorkArea(manager);
        Position title_x = M_X(title_area), title_y = M_Y(title_area);
        Dimension title_width = M_Width(title_area),
                  title_height = M_Height(title_area);

        Display *d = XtDisplay(manager);
        Dimension w = M_Width(manager), h = M_Height(manager),
                  s_t = M_ShadowThickness(manager);

        Position x = 0, y = title_height / 2 - s_t / 2;
        ...
        if (M_ShadowThickness(manager) > 0) {
                ...
                if (M_TitlePosition(manager) == XmTITLE_BOTTOM)
                        y = 0;
                XmeDrawShadows(d, XtWindow(manager), M_TopShadowGC(manager),
                               M_BottomShadowGC(manager), x, y, w, h, s_t,
                               M_ShadowType(manager));
        }

        /*	Redisplay title area.
         */
        if (title_area && XtIsManaged(title_area)) {
                XClearArea(d, XtWindow(manager), title_x, title_y, title_width,
                           title_height, False);
...
static Boolean SetValues(DtTitleBoxWidget current, DtTitleBoxWidget request,
        ...
        Boolean redisplay_flag = False, new_title = False, new_font = False;
        Arg al[10];      /*  arg list		*/
        register int ac; /*  arg count		*/
            ...
            M_ShadowThickness(new) != M_ShadowThickness(current)) {
                ...
                if (M_Width(new) == M_Width(current) &&
                    M_Height(new) == M_Height(current)) {
                        GetSize(new, 0, 0, 0, 0, &(M_Width(new)),
                                &(M_Height(new)));
...
static XtGeometryResult QueryGeometry(DtTitleBoxWidget manager,
                                      XtWidgetGeometry *request,
                                      XtWidgetGeometry *reply) {
        Dimension w = M_Width(manager), h = M_Height(manager), new_w = 0,
                  new_h = 0;
...
static XtGeometryResult GeometryManager(Widget kid, XtWidgetGeometry *request,
        ...
        Widget title_area = M_TitleArea(manager);
        Dimension ta_w = 0, ta_h = 0, ta_bw = 0, wa_w = 0, wa_h = 0,
                  w = M_Width(manager), h = M_Height(manager);
        Boolean query_only = request->request_mode & XtCWQueryOnly,
                ...
                height_req = request->request_mode & CWHeight,
                bw_req = request->request_mode & CWBorderWidth, almost = False;
        ...
        if (w != M_Width(manager) || h != M_Height(manager)) {
                ...
                parent_result = XtMakeGeometryRequest(
                    (Widget)manager, &parent_req, &parent_reply);
        ...
        case XtGeometryYes:
                if (query_only)
                        ...
                        if (kid == title_area)
                                XmeConfigureObject(
                                    title_area, title_area->core.x,
                                    title_area->core.y, ta_w, ta_h, ta_bw);
                        _DtProcessLock();
...
static void ChangeManaged(DtTitleBoxWidget manager) {
        ...
        if (w != M_Width(manager) || h != M_Height(manager)) {
                ...
                case XtGeometryAlmost:
                        XtMakeResizeRequest((Widget)manager, w, h, NULL, NULL);
                case XtGeometryYes:
                case XtGeometryNo:
                default:
                        break;
...
*/
static void InsertChild(Widget child) {
        DtTitleBoxConstraint constraint =
            (DtTitleBoxConstraint)M_TitleBoxConstraint(child);
        DtTitleBoxWidget w;
...
**-------------------------------------------------------------
*/
Widget _DtCreateTitleBox(Widget parent, char *name, ArgList arglist,
                         Cardinal argcount) {
        return (XtCreateWidget(name, dtTitleBoxWidgetClass, parent, arglist,
                               argcount));
}
...
**-------------------------------------------------------------
*/
Widget _DtTitleBoxGetTitleArea(Widget w) {
...
**-------------------------------------------------------------
*/
Widget _DtTitleBoxGetWorkArea(Widget w) {
        DtTitleBoxWidget mgr = (DtTitleBoxWidget)w;
