...
static void Initialize(DtSpinBoxWidget request, DtSpinBoxWidget new,
                       ArgList given_args, Cardinal *num_args);
static XmNavigability WidgetNavigable(DtSpinBoxWidget spin);
static void _SpinBoxFocusIn(DtSpinBoxWidget spin, XEvent *event, char **params,
                            Cardinal *num_params);
static void _SpinBoxFocusOut(DtSpinBoxWidget spin, XEvent *event, char **params,
                             Cardinal *num_params);
static void DrawHighlight(DtSpinBoxWidget spin, Boolean clear);
static void _SpinBoxUp(DtSpinBoxWidget spin, XEvent *event, char **params,
                       Cardinal *num_params);
static void _SpinBoxDown(DtSpinBoxWidget spin, XEvent *event, char **params,
                         Cardinal *num_params);
static void _SpinBoxLeft(DtSpinBoxWidget spin, XEvent *event, char **params,
                         Cardinal *num_params);
static void _SpinBoxRight(DtSpinBoxWidget spin, XEvent *event, char **params,
                          Cardinal *num_params);
static void _SpinBoxBeginLine(DtSpinBoxWidget spin, XEvent *event,
                              char **params, Cardinal *num_params);
static void _SpinBoxEndLine(DtSpinBoxWidget spin, XEvent *event, char **params,
                            Cardinal *num_params);
static void _SpinBoxGetFocus(DtSpinBoxWidget spin, XEvent *event, char **params,
                             Cardinal *num_params);
static void _SpinBoxPrevTabGroup(DtSpinBoxWidget spin, XEvent *event,
                                 char **params, Cardinal *num_params);
static void _SpinBoxNextTabGroup(DtSpinBoxWidget spin, XEvent *event,
                                 char **params, Cardinal *num_params);
...
static void Redisplay(DtSpinBoxWidget w, XEvent *event, Region region);
...
static void CalculateSizes(DtSpinBoxWidget spin, Dimension *pwidth,
                           Dimension *pheight, Dimension *parrow_width);
static void LayoutChildren(DtSpinBoxWidget spin);
static Boolean SetValues(DtSpinBoxWidget current, DtSpinBoxWidget request,
                         DtSpinBoxWidget new);
static void ClearShadow(DtSpinBoxWidget w, Boolean all);
static void DrawShadow(DtSpinBoxWidget w);
static void StoreResourceInfo(DtSpinBoxPart *spin_p, DtSpinBoxPart *old_p,
                              Boolean do_items);
...
static Boolean SendCallback(DtSpinBoxWidget spin, XEvent *event,
                            Boolean value_changed, int position, float current,
                            Boolean crossed);
static void FinishUpDown(DtSpinBoxWidget spin, XtPointer arrow_call_data,
                         int new_position, float new_current, Boolean crossed);
...
static void disarm_cb(Widget w, XtPointer client_data, XtPointer call_data);
static void grab_leave_cb(Widget w, XtPointer client_data, XEvent *event,
                          Boolean *dispatch);
...
#define FloatFormat(w)                                                         \
        (String) & (XmField(w, ipot, DtSpinBox, float_format, char *))
...
#define LabelMaxLength(w)                                                      \
        XmField(w, ipot, DtSpinBox, label_max_length, Dimension)
#define LabelMaxHeight(w)                                                      \
        XmField(w, ipot, DtSpinBox, label_max_height, Dimension)

#define ArrowSensitivity(w)                                                    \
        XmField(w, ipot, DtSpinBox, arrow_sensitivity, unsigned char)
...
#define ActivateCallback(w)                                                    \
        XmField(w, ipot, DtSpinBox, activate_callback, XtCallbackList)
...
#define FocusCallback(w)                                                       \
        XmField(w, ipot, DtSpinBox, focus_callback, XtCallbackList)
#define InitialDelay(w) XmField(w, ipot, DtSpinBox, initial_delay, unsigned int)
#define LosingFocusCallback(w)                                                 \
        XmField(w, ipot, DtSpinBox, losing_focus_callback, XtCallbackList)
...
#define ModifyVerifyCallback(w)                                                \
        XmField(w, ipot, DtSpinBox, modify_verify_callback, XtCallbackList)
...
#define ValueChangedCallback(w)                                                \
        XmField(w, ipot, DtSpinBox, value_changed_callback, XtCallbackList)
...
static XmPartResource resources[] = {
    ...
    {DtNalignment, DtCAlignment, XmRAlignment, sizeof(unsigned char),
     DtOffset(alignment), XmRImmediate, (XtPointer)DtALIGNMENT_BEGINNING},
    {DtNarrowLayout, DtCArrowLayout, DtRArrowLayout, sizeof(unsigned char),
     DtOffset(arrow_layout), XmRImmediate, (XtPointer)DtARROWS_END},
    {DtNarrowSensitivity, DtCArrowSensitivity, DtRArrowSensitivity,
     sizeof(unsigned char), DtOffset(arrow_sensitivity), XmRImmediate,
     (XtPointer)DtARROWS_SENSITIVE},
    {DtNspinBoxChildType, DtCSpinBoxChildType, DtRSpinBoxChildType,
     sizeof(unsigned char), DtOffset(child_type), XmRImmediate,
     (XtPointer)DtSTRING},
    {DtNcolumns, DtCColumns, XmRShort, sizeof(short), DtOffset(text_columns),
     XmRImmediate, (XtPointer)DEFAULT_COL},
    {DtNdecimalPoints, DtCDecimalPoints, XmRShort, sizeof(short),
     DtOffset(decimal_points), XmRImmediate, (XtPointer)0},
    {DtNeditable, DtCEditable, XmRBoolean, sizeof(Boolean), DtOffset(editable),
    ...
    {DtNmaximumValue, DtCMaximumValue, XmRInt, sizeof(int), DtOffset(maximum),
     XmRImmediate, (XtPointer)10},
    {DtNmaxLength, DtCMaxLength, XmRInt, sizeof(int), DtOffset(text_max_length),
     XmRImmediate, (XtPointer)MAXINT},
    {DtNminimumValue, DtCMinimumValue, XmRInt, sizeof(int), DtOffset(minimum),
     XmRImmediate, (XtPointer)0},
    {DtNmodifyVerifyCallback, DtCCallback, XmRCallback, sizeof(XtCallbackList),
     DtOffset(modify_verify_callback), XmRCallback, (XtPointer)NULL},
    {DtNposition, DtCPosition, XmRInt, sizeof(int), DtOffset(position),
     XmRImmediate, (XtPointer)0},
    ...
    {DtNvalueChangedCallback, DtCCallback, XmRCallback, sizeof(XtCallbackList),
     DtOffset(value_changed_callback), XmRCallback, (XtPointer)NULL},
    {DtNwrap, DtCWrap, XmRBoolean, sizeof(Boolean), DtOffset(wrap),
     XmRImmediate, (XtPointer)TRUE},
};
...
externaldef(dtspinboxwidgetclass)
    WidgetClass dtSpinBoxWidgetClass = (WidgetClass)&dtSpinBoxClassRec;
...
static void ClassInitialize(void) {
        ...
        child_trans_label =
            XtParseTranslationTable(SpinBoxLabelTranslationTable);
        child_trans_text = XtParseTranslationTable(SpinBoxTextTranslationTable);
        child_trans_arrow =
            XtParseTranslationTable(SpinBoxArrowTranslationTable);
 ...
 */
static void Initialize(DtSpinBoxWidget request, DtSpinBoxWidget new,
                       ArgList given_args, Cardinal *num_args) { /* MotifBc */
        DtSpinBoxPart *spin_p =
            (DtSpinBoxPart *)&(XmField(new, ipot, DtSpinBox, label, Widget));
        char *widget_name;
        Arg args[20];
        int n;
        ...
        if (Editable(new)) {
                ...
                Text(new) = XtCreateManagedWidget(
                    widget_name, xmTextFieldWidgetClass, (Widget) new, args, n);
                ...
                if (unit_type != XmPIXELS) {
                        XtSetArg(args[n], XmNunitType, unit_type);
                        n++;
                }
                ...
                Label(new) = XtCreateManagedWidget(
                    widget_name, xmLabelWidgetClass, (Widget) new, args, n);
                /* USL */
        ...
        if (ArrowLayout(new) == DtARROWS_SPLIT) {
                XtSetArg(args[n], XmNarrowDirection, XmARROW_RIGHT);
                n++;
        }
        ...
        n++;
        XtSetArg(args[n], XmNforeground, CBgPixel(new));
        n++;
        UpArrow(new) = XtCreateManagedWidget(
            widget_name, xmArrowButtonWidgetClass, (Widget) new, args, n);
        XtOverrideTranslations((Widget)UpArrow(new), child_trans_arrow);

        sprintf(widget_name, "%s_Down", XtName((Widget) new));
        if (ArrowLayout(new) == DtARROWS_SPLIT) {
                XtSetArg(args[n], XmNarrowDirection, XmARROW_LEFT);
                n++;
        } else {
                XtSetArg(args[n], XmNarrowDirection, XmARROW_DOWN);
                n++;
        }
        DownArrow(new) = XtCreateManagedWidget(
            widget_name, xmArrowButtonWidgetClass, (Widget) new, args, n);
        ...
        XtAddCallback(DownArrow(new), XmNdisarmCallback, disarm_cb,
                      (XtPointer) new);
        XtAddEventHandler(DownArrow(new), LeaveWindowMask, FALSE, grab_leave_cb,
                          (XtPointer) new);
        ...
        /* Store Converter for DtNmaximumValue, DtNminimumValue,
         * DtNincrementValue,
         */
        XtSetTypeConverter(XmRString, DtRMaximumValue, XtCvtStringToInt, NULL,
                           0, XtCacheAll, NULL);
        XtSetTypeConverter(XmRString, DtRMinimumValue, XtCvtStringToInt, NULL,
                           0, XtCacheAll, NULL);
        XtSetTypeConverter(XmRString, DtRIncrementValue, XtCvtStringToInt, NULL,
                           0, XtCacheAll, NULL);
        XtSetTypeConverter(XmRString, DtRNumValues, XtCvtStringToInt, NULL, 0,
                           XtCacheAll, NULL);
        /*
         * this is so these resources can be set uniformly through
         * XtCreateWidget or through defaults file as *spin.backgound
         */

        n = 0;
        XtSetArg(args[n], XmNbackground, CBgPixel(new));
        n++;
        XtSetArg(args[n], XmNbackgroundPixmap, CBgPixmap(new));
        n++;
        XtSetArg(args[n], XmNforeground, MFgPixel(new));
        n++;
        if (Text(new))
                XtSetValues(Text(new), args, n);
        if (Label(new))
...
static XmNavigability WidgetNavigable(DtSpinBoxWidget spin) {
        XmNavigationType nav_type =
            ((XmManagerWidget)spin)->manager.navigation_type;
...
static void _SpinBoxBeginLine(DtSpinBoxWidget spin, XEvent *event,
        ...
        if (SendCallback(spin, event, FALSE, new_position, new_current,
                         FALSE) == TRUE) {
...
static void CheckResources(DtSpinBoxWidget spin) {
        ...
        if ((ChildType(spin) == DtSTRING) &&
            ((Position(spin) < 0) ||
             ((Position(spin) >= ItemCount(spin)) && (ItemCount(spin) > 0)))) {
 ...
 */
static XtGeometryResult GeometryManager(Widget w, /* child */
...
static void SetSpinBoxSize(DtSpinBoxWidget spin) {
        ...
        (void)XtMakeResizeRequest(
            (Widget)spin,
            arrow_width + width + shadow + (SPIN_MARGIN_W(spin) * 2),
            height + shadow + (SPIN_MARGIN_H(spin) * 2), NULL, NULL);
        ...
        if (Editable(spin) == FALSE) { /** label **/
                ...
                if ((available_width != (Label(spin))->core.width) ||
                    (available_height != (Label(spin))->core.height))
                        XtResizeWidget(Label(spin), available_width,
                                       available_height,
                                       (Label(spin))->core.border_width);
        } else if ((Text(spin))->core.width !=
...
static void CalculateSizes(DtSpinBoxWidget spin, Dimension *pwidth,
        ...
        if ((Editable(spin) == FALSE) && (height > LabelMaxHeight(spin)))
...
static void LayoutChildren(DtSpinBoxWidget spin) {
        ...
        case DtARROWS_FLAT_END:
                 ...
                 */
                start_x =
                    (spin->core.width - start_x - (UpArrow(spin))->core.width -
...
static Boolean SetValues(DtSpinBoxWidget current, DtSpinBoxWidget request,
        ...
        if (Editable(new) != Editable(current)) {
                if (Editable(new)) {
                        ...
                        if (Text(new) == NULL) {
                                widget_name =
                                    XtMalloc(strlen(XtName((Widget) new)) + 10);
                                sprintf(widget_name, "%s_TF",
                                        XtName((Widget) new));
                                n = 0;
                                XtSetArg(args[n], XmNcolumns, TextColumns(new));
                                n++;
                                ...
                                n++;
                                if (unit_type != XmPIXELS) {
                                        XtSetArg(args[n], XmNunitType,
                                                 XmPIXELS);
                                        n++;
                                }
                                Text(new) = XtCreateManagedWidget(
                                    widget_name, xmTextFieldWidgetClass,
                                    (Widget) new, args, n);
                                XtAddCallback(Text(new), XmNlosingFocusCallback,
                        ...
                        if (Label(new) == NULL) {
                                ...
                                Label(new) = XtCreateManagedWidget(
                                    widget_name, xmLabelWidgetClass,
                                    (Widget) new, args, n);
                ...
                if (SPIN_SHADOW(new) == SPIN_SHADOW(current))
                        SPIN_SHADOW(new) =
                            (Editable(new)) ? TEXT_FIELD_SHADOW : LABEL_SHADOW;
        }
        ...
        if (Label(new) && (store_info || (Label(new) != Label(current)) ||
                           (ChildType(new) != ChildType(current)))) {
                SetMaximumLabelSize(new_p);
                label_size_changed = TRUE;
        }

        if (store_info || (Alignment(new) != Alignment(current)) ||
...
static void DrawShadow(DtSpinBoxWidget w) {
        ...
        if ((shadow > 0) && XtIsRealized((Widget)w)) {
                XmeDrawShadows(
                    ...
                    w->manager.top_shadow_GC, w->manager.bottom_shadow_GC,
                    margin_w, margin_h, w->core.width - (margin_w * 2),
                    w->core.height - (margin_h * 2), shadow, XmSHADOW_OUT);
        }
...
static void StoreResourceInfo(DtSpinBoxPart *spin_p, DtSpinBoxPart *old_p,
        ...
        sprintf((char *)(spin_p->float_format + 1), ".%df",
                spin_p->decimal_points);
}
...
static void SetMaximumLabelSize(DtSpinBoxPart *spin_p) {
        ...
        if (spin_p->child_type == DtNUMERIC) {
                 ...
                 */
                for (i = 0; i < spin_p->item_count; i++) {
                        XmStringExtent(font_list, spin_p->items[i], &width,
                                       &height);
                        longest = (width > longest) ? width : longest;
                        highest = (height > highest) ? height : highest;
                }
        ...
        spin_p->label_max_length =
            ((Dimension)(longest + ((LABEL_PADDING + TEXT_CONTEXT_MARGIN) *
                                    2)) > (Dimension)Width(spin_p->label))
                ? (longest + ((LABEL_PADDING + TEXT_CONTEXT_MARGIN) * 2))
                : Width(spin_p->label);

        spin_p->label_max_height =
            highest > Height(spin_p->label) ? highest : Height(spin_p->label);
...
static void SetLabelData(DtSpinBoxWidget spin) {
        ...
        if (ChildType(spin) == DtNUMERIC) {
                ...
                /*
                 * If the item is not empty, get the current item from the list,
                 * else use InitLabel.
                 */
                xm_string =
                    (ItemCount(spin) > 0) ? (Items(spin))[index] : InitLabel;
                XtSetArg(arg, XmNlabelString, xm_string);
                XtSetValues(Label(spin), &arg, 1);
        }
}
...
static Boolean SendCallback(DtSpinBoxWidget spin, XEvent *event,
        ...
        if (ChildType(spin) == DtNUMERIC) {
                ...
                xm_string =
                    (ItemCount(spin) > 0) ? (Items(spin))[position] : InitLabel;
...
static void FinishUpDown(DtSpinBoxWidget spin, XtPointer arrow_call_data,
        ...
        Timer(spin) =
            XtAppAddTimeOut(XtWidgetToApplicationContext((Widget)spin),
         ...
         */
        if (SendCallback(spin, last_event, FALSE, new_position, new_current,
                         crossed) == FALSE) {
                XtRemoveTimeOut(Timer(spin));
                Timer(spin) = (XtIntervalId)NULL;
                InitCb(spin) = TRUE;
...
static void up_cb(Widget w, XtPointer client_data, XtPointer call_data) {
        ...
        DtSpinBoxPart *spin_p =
            (DtSpinBoxPart *)&(XmField(spin, ipot, DtSpinBox, label, Widget));
        ...
        FinishUpDown(spin, call_data, new_position, new_current,
                     crossed_boundary);
}
...
static void grab_leave_cb(Widget w, XtPointer client_data, XEvent *event,
        ...
        if ((int)Timer(spin) && ((event->xcrossing.mode == NotifyGrab) ||
...
static void text_losing_focus_cb(Widget w, XtPointer client_data,
        ...
        if (LosingFocusCallback(spin))
                XtCallCallbackList((Widget)spin, LosingFocusCallback(spin),
                                   (XtPointer)call_data);
}
...
static void text_activate_cb(Widget w, XtPointer client_data,
        ...
        if (ActivateCallback(spin))
                XtCallCallbackList((Widget)spin, ActivateCallback(spin),
                                   (XtPointer)call_data);
}
...
static void text_focus_cb(Widget w, XtPointer client_data,
        ...
        if (FocusCallback(spin))
                XtCallCallbackList((Widget)spin, FocusCallback(spin),
                                   (XtPointer)call_data);
}
...
void _DtSpinBoxGetArrowSize(Widget w, int resource_offset, XtArgVal *value) {
...
Widget DtCreateSpinBox(Widget parent, char *name, Arg *arglist,
                       Cardinal num_args) {
        return (XtCreateWidget(name, dtSpinBoxWidgetClass, parent, arglist,
                               num_args));
}

void DtSpinBoxAddItem(Widget spinw, XmString item, int pos) {
...
void DtSpinBoxDeletePos(Widget spinw, int pos) {
        DtSpinBoxWidget spin = (DtSpinBoxWidget)spinw;
        DtSpinBoxPart *spin_p;
        int total_items;
        _DtWidgetToAppContext(spinw);
        ...
        if (total_items > 0)
                Items(spin) = (XmString *)XtRealloc(
                    (char *)Items(spin), (sizeof(XmString) * total_items));
        else {
                XtFree((char *)Items(spin));
 ...
 * text-field or label.
 */
void DtSpinBoxSetItem(Widget spinw, XmString item) {
        DtSpinBoxWidget spin = (DtSpinBoxWidget)spinw;
        int i;
        _DtWidgetToAppContext(spinw);
        _DtAppLock(app);
