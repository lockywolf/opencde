...
static Boolean DoReplace(DtEditorWidget pPriv, char *replace_string, Time time);
static int SearchForString(DtEditorWidget pPriv, XmTextPosition startLocation,
                           char *searchString);
static Boolean DoSearch(DtEditorWidget pPriv, char *search_string, Time time);
...
static void DestroyThisWidgetCB(Widget w, XtPointer client, XtPointer call);
...
DtEditorErrorCode DtEditorInvokeSpellDialog(Widget widget) {
        ...
        if (!IsValidFilter(pPriv)) {
                ...
                if ((fp = fopen(fileName, "w")) != (FILE *)NULL) {
                        /*
                         * Temporary file created sucessfully so write out
                         * contents of widget in preparation of feeding it to
                         * the 'spell' filter.
                         */
                        ...
                        XtFree(string);
                        /*
                         * Tack on a final newline (\n) cuz spell(1) does not
                         * spell-check lines which do not terminate with a
                         * newline.
                         */
                        ...
                        if (fp == (FILE *)NULL)
                                 ...
                                 * Initialize the Spell dialog and get ready to
                                 * receive the list of mispelled words.
                                 */
                                 ...
                                 */
                                maxLen = 50;
                                line = (char *)XtMalloc(sizeof(char) *
                                                        (maxLen + 1));
                                len = 0;
        ...
        if (error != DtEDITOR_NO_ERRORS) {
                ...
                buf = XtMalloc(
                    (strlen(BAD_FILTER2) + strlen(M_spellFilter(pPriv)) + 1) *
                    sizeof(char));

                sprintf(buf, BAD_FILTER2, M_spellFilter(pPriv));
                msg1 = XmStringGenerate(BAD_FILTER, XmFONTLIST_DEFAULT_TAG,
                                        XmCHARSET_TEXT, NULL);

                msg2 = XmStringSeparatorCreate();
                msg3 = XmStringConcatAndFree(msg1, msg2);
                msg1 = XmStringGenerate(buf, XmFONTLIST_DEFAULT_TAG,
                                        XmCHARSET_TEXT, NULL);
                ...
                ac = 0;
                XtSetArg(al[ac], XmNdialogTitle, title);
                ac++;
                XtSetArg(al[ac], XmNdialogType, XmDIALOG_ERROR);
                ac++;
                XtSetArg(al[ac], XmNmessageString, msg2);
                ac++;
                dialog =
                    XmCreateMessageDialog((Widget)pPriv, "Spell Error", al, ac);
                ...
                XtVaSetValues(XtParent(dialog), XmNdeleteResponse, XmDESTROY,
                              NULL);
...
/* ARGSUSED */
Boolean DtEditorFind(Widget widget, char *find) {
        ...
        if (find != (char *)NULL)
                ...
                /*
                 * If there is no value from the dialog, then post the
                 * Find/Change dialog to get one.
                 */
                if (!M_search_string(editor))
                        _DtEditorSearch(editor, False, False);
                else
                        foundIt = DoSearch(editor, M_search_string(editor),
                                           CurrentTime);
        }
 ...
 */
int _DtEditor_CountCharacters(char *str, int numBytes) {
        char *bptr;
        int count = 0;
        int char_size = 0;
...
static int SearchForString(DtEditorWidget pPriv, XmTextPosition startLocation,
        ...
        while ((Boolean)XmTextFindString(M_text(pPriv), searchAnchor,
               ...
               ((Boolean)XmTextFindString(M_text(pPriv), topAnchor,
                ...
                int length;
                /*
                 * Do some extra work for the Spell case, so we find only
                 * "words" and not strings. Get the word with the leading &
                 * trailing characters. It's a word if it's bounded by: a.
                 * 16-bit characters b. Spaces c. Punctuation
                 */
                 ...
                 *		 of the word.  Does not include leading
                 *character. endPos - Position in the document of the last
                 *character of the word + 1. Points to the trailing character,
                 *		 if any.
                 *
                 *	word   - The string we have matched. Includes the
                 *leading & trailing characters, if any. word[0] - Leading
                 *character, if any, otherwise first character of the matched
                 *string. word[length] - Trailing character, if any,
                 *otherwise last character of the matched string.
                 *
                 */

                endPos = pos + 1 +
                         _DtEditor_CountCharacters(searchString,
                                                   strlen(searchString));
                 ...
                 * last character of the word is the last character in the
                 * document there is no trailing character.
                 */
                if (pos > 0) {
                        /*
                        ...
                        if (endPos <= lastPosition) {
                                ...
                                word = (char *)_XmStringSourceGetString(
                                    (XmTextWidget)M_text(pPriv), pos - 1,
                                    endPos, False);
                        ...
                        if (endPos <= lastPosition) {
                                 ...
                                 */
                                word = (char *)_XmStringSourceGetString(
                                    (XmTextWidget)M_text(pPriv), 0, endPos,
                                    False);
                                length = strlen(word) - 1;
                                trailingChar = word[length];
                        } else {
...
static Boolean DoSearch(DtEditorWidget widget, char *search_string, Time time) {
        int stringPosition;
        Boolean foundIt = False;

        stringPosition = SearchForString(
            widget, XmTextGetInsertionPosition(M_text(widget)), search_string);
        ...
        if (stringPosition == -1) {
                ...
                /*
                 * The string was found so highlight the word and scroll the
                ...
                XmTextSetSelection(
                    M_text(widget), pos,
                    pos + _DtEditor_CountCharacters(search_string,
                 ...
                 */
                if (pos < tw->text.top_character ||
                    pos >= tw->text.bottom_position) {
                        Arg al[5];
                        int ac;
                        Dimension height;
                        short rows;
                        ...
                        if (XmTextPosToXY(M_text(widget), pos, &x, &y) ==
                            True) {
                                int offset = (int)((y - height / 2) * rows) /
                                             (int)height;
 ...
 */
static Boolean DoReplace(DtEditorWidget pPriv, char *replace_string,
                         Time time) {
        ...
        if (XmTextGetSelectionPosition(M_text(pPriv), &first, &last) &&
            first != last) {
                ...
                XmTextSetSelection(
                    M_text(pPriv), first,
                    first + _DtEditor_CountCharacters(replace_string,
...
static Boolean ReplaceAll(DtEditorWidget widget, char *search_string,
                          char *replace_string) {
        ...
        if (search_string && *search_string) {
                 ...
                 */
                replacementLength = _DtEditor_CountCharacters(
                ...
                lastOccurrence = -1;
                while (
                    ((thisOccurrence = SearchForString(
                          widget,
                          (lastOccurrence > 0) ? (XmTextPosition)lastOccurrence
                                               : 0,
                          search_string)) != -1) &&
                    thisOccurrence >= lastOccurrence) {
                        XmTextReplace(
                            M_text(widget), (XmTextPosition)thisOccurrence,
 ...
 */
Boolean DtEditorChange(Widget widget, DtEditorChangeValues *findChangeStrings,
        ...
        case DtEDITOR_NEXT_OCCURRENCE: {
                ...
                if (findChangeStrings != (DtEditorChangeValues *)NULL)
                        returnVal =
                            DtEditorFind(widget, findChangeStrings->find);
                else
        ...
        case DtEDITOR_CURRENT_SELECTION: {
                ...
                if (findChangeStrings != (DtEditorChangeValues *)NULL)
                        returnVal = DoReplace(
                            editor, findChangeStrings->changeTo, CurrentTime);
                else
                        returnVal = DoReplace(editor, M_replace_string(editor),
                                              CurrentTime);
...
/* ARGSUSED */
void _DtEditorSearchMapCB(Widget w, caddr_t client_data, caddr_t call_data) {
        int ac;
        Arg al[4];
        Widget parent;
        ...
        ac = 0;
        XtSetArg(al[ac], XmNx, newX);
        ac++;
        XtSetArg(al[ac], XmNy, newY);
        ac++;
        XtSetValues(w, al, ac);
}
...
/* ARGSUSED */
void _DtEditorDialogSearchCB(Widget w, caddr_t client_data, caddr_t call_data) {
        ...
        if (M_search_dialogMode(pPriv) == REPLACE) {
                ...
                M_search_string(pPriv) =
                    XmTextFieldGetString(M_findText(pPriv));
                ...
                if (DtEditorFind((Widget)pPriv, M_search_string(pPriv))) {
                        /*
                         * If the string was found then enable the Change
                         * button. It will be disabled when it is pressed or a
                         * new Find string is entered.
                         */
                        _DtEditorSetReplaceSensitivity(pPriv, True);
                } else {
                        /*
                         * Post a dialog informing the user the string was not
                         * found.
                         */

                        char *tempStr = (char *)XtMalloc(
                            strlen(NO_FIND) + strlen(M_search_string(pPriv)) +
                            1);
                ...
                M_misspelled_found(pPriv) =
                    DtEditorFind((Widget)pPriv, M_misspelled_string(pPriv));
                /*
                 * If the word was found & there is a Change To string then
                 * enable the Change button.  If there is no Change To string,
                 * Change will be enabled when a string is entered if
                 * M_misspelled_found is True (see
...
/* ARGSUSED */
void _DtEditorDialogReplaceCB(Widget w, caddr_t client_data,
                              caddr_t call_data) {
        DtEditorWidget pPriv = (DtEditorWidget)client_data;
        ...
        if (M_search_dialogMode(pPriv) == REPLACE) {
                ...
                M_replace_string(pPriv) =
                    XmTextFieldGetString(M_replaceText(pPriv));
                ...
                if (newWord.changeTo != (char *)NULL) {
                        /* This field ignored when changing the current
                         * selection */
                        newWord.find = (char *)NULL;
                        DtEditorChange((Widget)pPriv, &newWord,
                                       DtEDITOR_CURRENT_SELECTION);
...
/* ARGSUSED */
void _DtEditorDialogReplaceAllCB(Widget w, caddr_t client_data,
                                 caddr_t call_data) {
        DtEditorWidget pPriv = (DtEditorWidget)client_data;
        ...
        if (M_search_dialogMode(pPriv) == REPLACE) {
                /*
                 * Find/Change mode
                 * Free any existing search string before getting the current
                 * one.
                 */
                if (M_search_string(pPriv))
                        XtFree(M_search_string(pPriv));
                ...
                if (!DtEditorChange((Widget)pPriv, (DtEditorChangeValues *)NULL,
                        ...
                        /*
                         * If the replace failed, post a dialog informing the
                         * user the string was not found.
                         */
                ...
                changeValues.changeTo =
                    XmTextFieldGetString(M_replaceText(pPriv));
                DtEditorChange((Widget)pPriv, &changeValues,
                               DtEDITOR_ALL_OCCURRENCES);
...
/* ARGSUSED */
void _DtEditorDialogFindCancelCB(Widget w, caddr_t client_data,
                                 caddr_t call_data) {
        DtEditorWidget pPriv = (DtEditorWidget)client_data;
 ...
 */
/* ARGSUSED */
void _DtEditorMisspelledSelectCB(Widget w, caddr_t client_data,
                                 caddr_t call_data) {
 ...
 */
/* ARGSUSED */
void _DtEditorMisspelledDblClickCB(Widget w, caddr_t client_data,
                                   caddr_t call_data) {
...
void _DtEditorSetFindSensitivity(DtEditorWidget widget, Boolean sensitivity) {
        XtSetSensitive(M_search_findBtn(widget), sensitivity);
}

void _DtEditorSetReplaceSensitivity(DtEditorWidget editor,
                                    Boolean sensitivity) {
        /*
...
void _DtEditorSetReplaceAllSensitivity(DtEditorWidget editor,
                                       Boolean sensitivity) {
        /*
...
/* ARGSUSED */
void _DtEditorFindTextChangedCB(Widget w, caddr_t client_data,
                                caddr_t call_data) {
        Arg al[10]; /* arg list */
...
/* ARGSUSED */
void _DtEditorReplaceTextChangedCB(Widget w, caddr_t client_data,
                                   caddr_t call_data) {
        char *pString;
...
static Boolean IsExecutable(struct stat statbuf) {
         ...
         */
        if (ingroup && (statbuf.st_mode & S_IXGRP) &&
         ...
         * return true
         */
        if ((statbuf.st_mode & S_IXOTH) && (statbuf.st_uid != geteuid()) &&
            !ingroup) {
                return (True);
        }
...
static Boolean IsValidFilter(DtEditorWidget pPriv) {
        ...
        while (pathtok != NULL) {
                tmp = (char *)XtMalloc(
                    (strlen(pathtok) + strlen(M_spellFilter(pPriv)) + 2) *
                    sizeof(char));
                strcpy(tmp, pathtok);
                strcat(tmp, "/");
