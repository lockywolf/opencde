 ...
 * MotifBc
 */
#define CascadingCallback(w)                                                   \
        XmField(w, ipot, DtMenuButton, cascading_callback, XtCallbackList)
...
#define MB_PVT_SMENU(mb)                                                       \
        XmField(mb, ipot, DtMenuButton, private_submenu, Boolean)
#define MB_LAST_TIMESTAMP(mb)                                                  \
        XmField(mb, ipot, DtMenuButton, last_timestamp, Time)
...
#define MB_GLYPH_X(mb)                                                         \
        (XmField(mb, ipot, DtMenuButton, menu_rect, XRectangle)).x
#define MB_GLYPH_Y(mb)                                                         \
        (XmField(mb, ipot, DtMenuButton, menu_rect, XRectangle)).y
#define MB_GLYPH_WIDTH(mb)                                                     \
        (XmField(mb, ipot, DtMenuButton, menu_rect, XRectangle)).width
#define MB_GLYPH_HEIGHT(mb)                                                    \
        (XmField(mb, ipot, DtMenuButton, menu_rect, XRectangle)).height
...
static void AdjustMenuButtonSize(DtMenuButtonWidget menubtn,
                                 Boolean adjustWidth, Boolean adjustHeight);
static void Arm(DtMenuButtonWidget mb);
static void ArmAndActivate(Widget wid, XEvent *event, String *params,
                           Cardinal *num_params);

static void CalculateMenuGlyphSize(DtMenuButtonWidget menubtn);
static void CallCascadingCallbacks(Widget w, XEvent *event);

static void Destroy(Widget wid);
static void Disarm(DtMenuButtonWidget mb, Boolean pop_down);
static void DrawArrow(Widget wid);
static void Draw3DShadows(DtMenuButtonWidget mb);
static void DrawMenuGlyph(DtMenuButtonWidget mb);

static void GetGC(DtMenuButtonWidget mb);

static void Initialize(Widget w_req, Widget w_new, ArgList args,
                       Cardinal *num_args);

static void LocateMenuGlyph(DtMenuButtonWidget menubtn);

static void MenuButtonHandler(Widget wid, XtPointer cd, XEvent *event,
                              Boolean *cont_to_dispatch);

static void Popup(DtMenuButtonWidget mb, XEvent *event, Boolean call_cbacks);

static void Redisplay(Widget mb, XEvent *event, Region region);
static void Resize(Widget mb);

static void Select(Widget wid, XEvent *event, String *param,
                   Cardinal *num_param);
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
                         Cardinal *num_args);
static void GetTopManager(Widget w, Widget *topManager);
...
static XtActionsRec menuButton_actions[] = {{"Select", Select}};
...
static XmPartResource resources[] = {
    {DtNcascadingCallback, DtCCallback, XmRCallback, sizeof(XtCallbackList),
     DtOffset(cascading_callback), XmRCallback, NULL},
    {DtNsubMenuId, DtCMenuWidget, /* submenu */
     XmRMenuWidget, sizeof(Widget), DtOffset(submenu), XmRMenuWidget,
     (XtPointer)NULL},
    {DtNcascadePixmap, DtCPixmap, XmRPrimForegroundPixmap, sizeof(Pixmap),
     DtOffset(menu_pixmap), XmRImmediate, (XtPointer)XmUNSPECIFIED_PIXMAP},
    {XmNshadowThickness, XmCShadowThickness, XmRHorizontalDimension,
     sizeof(Dimension), XmPrimOffset(shadow_thickness), XmRImmediate,
     (XtPointer)2},
    {XmNtraversalOn, XmCTraversalOn, XmRBoolean, sizeof(Boolean),
     XmPrimOffset(traversal_on), XmRImmediate, (XtPointer)TRUE},
    {XmNhighlightThickness, XmCHighlightThickness, XmRHorizontalDimension,
     sizeof(Dimension), XmPrimOffset(highlight_thickness), XmRImmediate,
     (XtPointer)2},
    {XmNmarginWidth, XmCMarginWidth, XmRHorizontalDimension, sizeof(Dimension),
     XmLabelOffset(margin_width), XmRImmediate, (XtPointer)6},
};

externaldef(xmmenubuttonclassrec) DtMenuButtonClassRec dtMenuButtonClassRec = {
    {
        /* core class record */
...
externaldef(dtmenubuttonwidgetclass)
    WidgetClass dtMenuButtonWidgetClass = (WidgetClass)&dtMenuButtonClassRec;
...
static void Draw3DShadows(DtMenuButtonWidget mb) {
        if (XtIsRealized((Widget)mb))
                XmeDrawShadows(
                    XtDisplay(mb), XtWindow(mb), mb->primitive.top_shadow_GC,
                    mb->primitive.bottom_shadow_GC,
                    mb->primitive.highlight_thickness,
                    mb->primitive.highlight_thickness,
                    mb->core.width - 2 * mb->primitive.highlight_thickness,
                    mb->core.height - 2 * mb->primitive.highlight_thickness,
...
static void DrawMenuGlyph(DtMenuButtonWidget mb) {
        if ((MB_GLYPH_WIDTH(mb) != 0)) {
                if (MB_PIXMAP(mb) != XmUNSPECIFIED_PIXMAP)
                        XCopyArea(XtDisplay(mb), MB_PIXMAP(mb), XtWindow(mb),
                                  LNormalGC(mb), 0, 0, MB_GLYPH_WIDTH(mb),
                                  MB_GLYPH_HEIGHT(mb), MB_GLYPH_X(mb),
                                  MB_GLYPH_Y(mb));
                else
                        DrawArrow((Widget)mb);
        }
...
static void ArmAndActivate(Widget wid, XEvent *event, String *params,
        ...
        if (MB_SMENU(mb) == (Widget)NULL || event == (XEvent *)NULL)
                return;
...
static void Disarm(DtMenuButtonWidget mb, Boolean pop_down) {
        if (MB_ARMED(mb) == TRUE) {
                ...
                if (pop_down == TRUE && MB_SMENU(mb) != (Widget)NULL)
...
static void PreMenuButtonHandler(Widget wid, XtPointer cd, XEvent *event,
                                 Boolean *cont_to_dispatch) {
        DtMenuButtonWidget mb = (DtMenuButtonWidget)wid;

        if (MB_SMENU(mb) == (Widget)NULL || event->xany.type != ButtonPress ||
...
static void CallCascadingCallbacks(Widget w, XEvent *event) {
        ...
        XtCallCallbackList((Widget)mb, CascadingCallback(mb), &cback);
}

static void PopdownCallback(Widget w, XtPointer client_data,
                            XtPointer call_data) {
...
static void PopupCallback(Widget w, XtPointer client_data,
                          XtPointer call_data) {
        DtMenuButtonWidget mb = (DtMenuButtonWidget)client_data;
...
static void Popup(DtMenuButtonWidget mb, XEvent *event, Boolean call_cbacks) {
        ...
        if ((MB_SMENU(mb) == (Widget)NULL) || (MB_POPPED_UP(mb) == TRUE))
                return;

        if (call_cbacks)
                CallCascadingCallbacks((Widget)mb, event);

        x = (Position)((int)XtWidth((Widget)mb) - (int)XtWidth(MB_SMENU(mb))) /
            2;
...
static void CalculateMenuGlyphSize(DtMenuButtonWidget menubtn) {
        ...
        if (MB_PIXMAP(menubtn) != XmUNSPECIFIED_PIXMAP) {
                XGetGeometry(XtDisplay(menubtn), MB_PIXMAP(menubtn), &rootwin,
                ...
                side = Max((text_height * 2 / 3) + 2 * (ht + st),
                           (2 * (ht + (st - 1) + 1)) + 1);
                MB_GLYPH_WIDTH(menubtn) = MB_GLYPH_HEIGHT(menubtn) = side;
        }
}
...
static void LocateMenuGlyph(DtMenuButtonWidget menubtn) {
        ...
        MB_GLYPH_X(menubtn) =
            XtWidth(menubtn) - menubtn->primitive.highlight_thickness -
            menubtn->primitive.shadow_thickness - Lab_MarginWidth(menubtn) -
        ...
        MB_GLYPH_Y(menubtn) = (Position)(
            (int)buffer + (((int)XtHeight(menubtn) - 2 * (int)buffer) -
                           (int)MB_GLYPH_HEIGHT(menubtn)) /
                              2);
}
        ...
        if ((int)(MB_GLYPH_WIDTH(menubtn) + GLYPH_PIX_SPACE) >
            (int)Lab_MarginRight(menubtn)) {
                ...
                if (adjustWidth)
                        ...
                        else if (LAlignment(menubtn) == XmALIGNMENT_CENTER)
...
static void Destroy(Widget wid) {
        ...
        if (submenu != (XmRowColumnWidget)NULL) {
                ...
                XtRemoveCallback(shell, XtNpopupCallback, PopupCallback,
                                 (XtPointer)mb);
                XtRemoveCallback(shell, XtNpopdownCallback, PopdownCallback,
                                 (XtPointer)mb);
        }
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
        ...
        if (MB_SMENU(new_w) != (Widget)NULL) {
                XtVaGetValues(MB_SMENU(new_w), XmNrowColumnType, &rowcol_type,
                              NULL);
        ...
        if ((LRecomputeSize(new_w)) || (requested->core.width <= 0))
                adjustWidth = TRUE;

        if ((LRecomputeSize(new_w)) || (requested->core.height <= 0))
                adjustHeight = TRUE;
...
static void Initialize(Widget w_req, Widget w_new, ArgList args,
        ...
        if ((XmIsRowColumn(parent))) {
                XtVaGetValues(parent, XmNrowColumnType, &rowcol_type, NULL);
        ...
        if (MB_SMENU(new_w) != (Widget)NULL) {
                XtInsertEventHandler((Widget)new_w, ButtonPressMask, False,
                                     PreMenuButtonHandler, MB_SMENU(new_w),
                                     XtListHead);
                XtAddEventHandler((Widget)new_w, ButtonPressMask, FALSE,
                                  MenuButtonHandler, MB_SMENU(new_w));
...
static void DrawArrow(Widget wid) {
        ...
        /* armed arrow */
        if (MB_ARMED(mb) == TRUE) {
                XFillRectangle(dpy, win, gc, MB_GLYPH_X(mb), MB_GLYPH_Y(mb),
                               MB_GLYPH_WIDTH(mb), MB_GLYPH_HEIGHT(mb));
                XmeDrawArrow(
                    dpy, win, bsGC, tsGC, gc, MB_GLYPH_X(mb) + ht + st - 1,
                    MB_GLYPH_Y(mb) + ht + st - 1,
                    MB_GLYPH_WIDTH(mb) - 2 * (ht + st - 1),
                    MB_GLYPH_HEIGHT(mb) - 2 * (ht + st - 1), st, XmARROW_DOWN);
        } else {
                ...
                XFillRectangle(dpy, win, gc, MB_GLYPH_X(mb), MB_GLYPH_Y(mb),
                               MB_GLYPH_WIDTH(mb), MB_GLYPH_HEIGHT(mb));
                XmeDrawArrow(
                    dpy, win, tsGC, bsGC, gc, MB_GLYPH_X(mb) + ht + st - 1,
                    MB_GLYPH_Y(mb) + ht + st - 1,
                    MB_GLYPH_WIDTH(mb) - 2 * (ht + st - 1),
                    MB_GLYPH_HEIGHT(mb) - 2 * (ht + st - 1), st, XmARROW_DOWN);
        }
...
Widget DtCreateMenuButton(Widget parent, char *name, ArgList al, Cardinal ac) {
