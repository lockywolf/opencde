...
static int SHM_FLAG =
    IPC_CREAT | S_IRUSR | S_IWUSR | S_IWGRP | S_IRGRP | S_IROTH | S_IWOTH;
...
void release_shm_mem(void) {
        if (global_memory_ptr != NULL) {
                if (shmdt(global_memory_ptr) == -1) {
                        DtSearchAddMessage(catgets(
                            dtsearch_catd, MS_vestatis, 104,
...
static int init_global_memory(DtSrINT32 addrs, DtSrINT32 r_addrs) {
        ...
        i = DtSrMAX_STEMCOUNT * ((addrs >> 3) + 1) * 2 + addrs * sizeof(int) +
            sizeof(DB_ADDR) * r_addrs;
        ...
        for (;;) {
                ...
                while (right > left) {
                        if ((right - left) > MED_3_VALUE) {
                                /*
                                 * compute value for the median-of-three
                                 * partitioning
                                 */
                                mid3 = (left + right) >> 1;
                                /*
                                ...
                                temp = (stat_array + mid3)->wght;
                                (stat_array + mid3)->wght =
                                    (stat_array + pvidx)->wght;
                                (stat_array + pvidx)->wght = temp;
                                dba = (stat_array + mid3)->dba;
                                (stat_array + mid3)->dba =
                                    (stat_array + pvidx)->dba;
                        ...
                        for (;;) {
                                ...
                                (stat_array + scan_r)->wght =
                                    (stat_array + scan_l)->wght;
                                (stat_array + scan_l)->wght = temp;
                                dba = (stat_array + scan_r)->dba;
                                (stat_array + scan_r)->dba =
                                    (stat_array + scan_l)->dba;
...
void get_next_memory_block(size_t node_size) {
        ...
        if (cur_mem_ptr == NULL) {
                ...
                if (mem_start == NULL) {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_vestatis, 310,
                 ...
                 * list
                 */
                memory_blocks = (CNCRD_MEMORY_AREA_LIST *)malloc(
                    sizeof(CNCRD_MEMORY_AREA_LIST) + 2);
...
static int build_bin_tree(char *cur_word) {
        ...
        for (this_link = &root_node; *this_link != NULL;) {
                ...
                this_link =
                    (i < 0) ? &(*this_link)->llink : &(*this_link)->rlink;
...
static int inv_index_bin_tree(void) {
        ...
        for (cptr = dblk->parser(&parg); cptr; cptr = dblk->parser(NULL)) {
...
void load_ditto_str(void) {
        ...
        if (begin_load_ditto) {
                 ...
                 * the smaller the number the closer the document is to the
                 * query.
                 *
                 ...
                 * value that the first doc is 'distant' from perfection (1.0 or
                 * 100%). For example, if the normalized weight of the first
                 * record is .931 then the proximity will be 7 (100% - 93% =
                 * 7%).  He does this now because he's going to use this first
                 * proximity (sum2) as a scaling factor to stretch out all the
                 * subsequent proximities so they look reasonable.
                 */
                sum = 0.0;
                ...
                ditto_llist =
                    (DtSrResult *)austext_malloc(dittosz, PROGNAME "449", NULL);
                j32 = 0;
                i32_start = 0;
                d0024 = OR_D00 << 24;
        ...
        for (i32 = i32_start; i32 < num_hits; i32++) {
                ...
                dba1 =
                    ((stat_array + i32)->dba * slot_d00 - dba_offset) | d0024;
                ...
                if (db_status < 0) {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_vestatis, 437,
                                        "%s: db_status = %d, dba = %d:%ld "
                                        "(x'%08.8lx'), vistano = %d\n"),
                                PROGNAME "437", db_status,
                                (dba1 & 0xff000000) >> 24, dba1 & 0xffffff,
                                dba1, saveusr.vistano);
                        OE_flags |= OE_PERMERR;
                        usrblk.retncode = OE_ABORT;
                        release_shm_mem();
                        return;
                ...
                if (j32 == 0) /* first ditto node already allocated */
                        ...
                        if (cur_ditto_mem == NULL) {
                                fputs(
                                    catgets(dtsearch_catd, MS_vestatis, 504,
                                            PROGNAME
                ...
                cur_ditto_mem->abstractp =
                    (char *)cur_ditto_mem + sizeof(DtSrResult);
                ...
                if (abstrsz > 0) {
                        ...
                        while (db_status == S_OKAY) {
                                ...
                                if (targ >= targend)
                                        break;
                                FINDNM(PROGNAME "545", OR_OBJ_MISCS,
                                       saveusr.vistano);
                        } /* end while-loop */
                }         /* endif: (abstrsz > 0) */
        ...
        if (num_hits >= maxhits) {
                if (!(usrblk.flags & USR_NO_INFOMSGS)) {
                        sprintf(vestat_msgbuf,
                                catgets(dtsearch_catd, MS_vestatis, 421,
                                        "$s Total Number Hits = %ld. Discarded "
                                        "hits beyond "
...
static void stat_search(void) {
        ...
        if (begin_sort) {
                ...
                /*
                 * For every query stem, read d99.  For every dba in d99 for
                 * each stem, update object's stat array node with rec count and
                 * a weight based on the IDF for this stem. (IDF is described
                ...
                for (qs = qs_start; qs < saveusr.stemcount; qs++) {
                         ...
                         * not in database, why not take the next stem in
                         * query_stems array, if any? ]
                         */
                        if (db_status != S_OKAY)
                                word1.or_hwaddrs = 0;
                        else
                        ...
                        if (word1.or_hwaddrs > 0) {
                                ...
                                fread(word_addrs, sizeof(DB_ADDR),
                                      (size_t)word1.or_hwaddrs,
                                      usrblk.dblk->iifile);

                                /*
                                 * Calculate IDF (inverse document frequency)
                                 * for this word. The IDF is a statistical ratio
                                 * of the number of documents containing the
                                 * word and the total number of documents in the
                                 * entire corpus. It is calculated here on the
                                 * fly to save space in the database. IDF = {log
                                 * (totnumdocs / numdocswithword) / log(2)} + 1.
                                 * Note that an IDF of 1 means the word occurs
                                 * in every doc (it's meaningless).  An IDF of
                                 * 19 means the word occurs once in every
                                 * 300,000 recs. Note that by dividing by log(2)
                                 * the IDF also tells us how many binary digits
                                 * are necessary to discriminate the word.
                                 * Finally I think 1.0 was added to prevent it
                                 * ever becoming zero when converted to integer.
                                 */
                                idf = (log((double)real_num_rec /
                                           (double)word1.or_hwaddrs) /
                                       LOG2) +
                                      1.0;

                                /*
                                 * WEIGHT PASS #1:
                                 * Update the stat array node for each doc (ie
                                 * dba) which includes this stem.  Specifically,
                                 * sum the product of the IDF and word-doc
                                 * weight into the 'wght' bucket, and update the
                                 * number of query words this doc contains. Note
                                 * that the d99 dba format is slot# in hi 3
                                 * bytes, word-doc weights in lo byte.
                                 */
                                for (j32 = 0; j32 < word1.or_hwaddrs; j32++) {
                                        NTOHL(word_addrs[j32]);
                                        temp1 =
                                            *(word_addrs + j32); /* d99 dba */
                                        cur_weight =
                                            (double)(temp1 &
                                                     0xFF); /* lo byte */
                                        temp = temp1 >> 8;  /* slot# */
                                        ...
                                        ((stat_array + temp)->wght) +=
                                            (float)(cur_weight * idf);
                                }
                        } /* end if (word1.or_hwaddrs > 0), ie
                           * query word exists */

                        /*
                         * If the query words were common, the last double loop
                         * may have taken a long time.  If so, return now to the
                         * user interface to allow the gui to respond to button
                         * clicks (like CANCEL buttons).
                         */
                        time_dif = difftime(time(NULL), start_time);
                        if ((time_dif > TIME_ITERATION ||
                             usrblk.debug & USRDBG_ITERATE) &&
                 ...
                 * the prelim hitlist, prior to sort and truncation to user's
                 * maxhits.
                 *
                 * WEIGHT PASS #2:
                 * While we're at it, finalize the accumulated 'wght' field,
                 * which will be our sort field, by multiplying it by the ratio
                 * of the number of query words in the document divided by the
                 * number of words in the query. Thus the final sort field for
                 * each doc is the sum over all the query words in the doc of 3
                 * factors: 1) IDF (relative weight of each query word in
                 * corpus), times 2) d99wght (relative weight of each query word
                 * in doc), times 3) weight based on number of different query
                 * words in this doc.
                 */
                num_hits = 0;
                for (int32 = 0; int32 < total_num_addrs; int32++) {
                        if (stat_array[int32].wght > 0) {
                                ...
                                (stat_array + num_hits)->wght =
                                    (stat_array + int32)->wght *
                                    ((double)(stat_array + int32)
                                         ->num_word_hits /
                                     (double)num_diff_words);
                                (stat_array + num_hits)->dba =
                                    (stat_array + int32)->dba;
                                num_hits++;
                        }
                }
                        ...
                        if (!mes_search_box) {
                                DtSearchAddMessage(
                                    catgets(dtsearch_catd, MS_vestatis, 990,
                                            PROGNAME "990 The system is now "
                                                     "sorting. Please wait."));
                        }
                        usrblk.retncode = OE_SEARCHING;
                        usrblk.workproc = stat_search;
...
void ve_statistical(void) {
        ...
        total_num_addrs =
            (usrblk.dblk->dbrec.or_maxdba - (dba & 0x00FFFFFF) + 1) / slot_d00 +
            1;
        /* stat_array size = 1 node for every possible object */

        if (usrblk.query[0] == 0) {
                DtSearchAddMessage(catgets(dtsearch_catd, MS_vestatis, 677,
                                           PROGNAME
        ...
        query_stems = (QUERY_STEM_STR *)austext_malloc(sizeof(QUERY_STEM_STR) *
                                                           (num_diff_words + 1),
                                                       PROGNAME " 371", NULL);
        ...
        /*
         * Copy first DtSrMAX_STEMCOUNT stems into the saveusr.stems. [So no
         * more than DtSrMAX_STEMCOUNT will be used in search or hiliting!]
         */
        ...
        word_addrs =
            (DB_ADDR *)(global_memory_ptr + total_num_addrs * sizeof(STAT_STR));
        for (int32 = 0; int32 < total_num_addrs; int32++) {
