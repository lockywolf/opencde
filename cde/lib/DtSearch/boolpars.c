...
void add_syntax_errmsg(int msgno) {
        ...
        case 1:
                ...
                sprintf(msgbuf,
                        catgets(dtsearch_catd, MS_boolpars, 2,
        ...
        case 3:
                sprintf(msgbuf,
                        catgets(dtsearch_catd, MS_boolpars, 6,
        ...
        default:
                sprintf(msgbuf,
                        catgets(dtsearch_catd, MS_boolpars, 8,
...
static TRUTHTAB *creatett(int stemno, int pmsz, unsigned char *permutes) {
        TRUTHTAB *newtt =
            austext_malloc(sizeof(TRUTHTAB) + pmsz + 4, PROGNAME "140", NULL);
...
static TRUTHTAB *get_stem_truthtab(char *newstem, char *origword) {
        ...
        if (stemno == saveusr.stemcount) {
                if (++saveusr.stemcount > DtSrMAX_STEMCOUNT) {
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_boolpars, 9,
                ...
                if (origword) {
                        strncpy(usrblk.stems[stemno], origword,
                                DtSrMAXWIDTH_HWORD);
...
TRUTHTAB *boolyac_NOT(TRUTHTAB *oldtt) {
        ...
        for (candidate = 0; candidate < 256; candidate++) {
                if (oldpm >= oldtt->pmsz ||
                    candidate < oldtt->permutes[oldpm]) {
...
TRUTHTAB *boolyac_COLLOC(TRUTHTAB *word1tt, int colloc_val, TRUTHTAB *word2tt) {
        ...
        sprintf(wordbuf, COLLOC_STEM_FORMAT, word1tt->stemno, word2tt->stemno,
                colloc_val);
        ...
        if (debugging_boolpars) {
                fprintf(aa_stderr, " COLLOC: exprs=%p,%p-->expr=%p pmsz=%d\n",
                        (void *)word1tt, (void *)word2tt, (void *)newtt,
                        newtt->pmsz);
                fflush(aa_stderr);
        }
        return newtt;
                ...
                if (DtSearchHasMessages())
                        ...
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_boolpars, 1,
                                        ...
                                        "boolean query.  Please reformulate "
                                        "and try again."),
                                PROGNAME "001");
                        DtSearchAddMessage(msgbuf);
                }
...
GET_ANOTHER_TOKEN:
        ...
        case '@': /* COLLOCATION operator */
                ...
                if ((usrblk.dblk->dbrec.or_dbaccess & ORA_BLOB) == 0) {
                        ...
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_boolpars, 10,
                                        "%s Collocation searches not available "
                                        "for database '%s'."),
                ...
                if (yylval.int_val <= 0) {
                        ...
                        sprintf(
                            msgbuf,
                            catgets(
                                dtsearch_catd, MS_boolpars, 11,
                                "%s Collocation operator '%.*s' is invalid.\n"
                                "Correct format is '@n' where n is greater "
                                "than zero."),
        ...
        default:
                ...
                if (stembufp == NULL) {
                        ...
                        if (!DtSearchHasMessages()) {
                                sprintf(msgbuf,
                                        catgets(dtsearch_catd, MS_boolpars, 13,
                                                "%s Word '%.*s' is invalid."),
                                        PROGNAME "315", DtSrMAXWIDTH_HWORD,
                                        yytext);
                                DtSearchAddMessage(msgbuf);
                        }
                        break;
                ...
                if (strlen(stembufp) != strlen(yytext)) {
                        ...
                        sprintf(
                            msgbuf,
                            catgets(dtsearch_catd, MS_boolpars, 14,
...
DELIVER_TOKEN:
        if (debugging_boolpars) {
                fprintf(
                    ...
                    last_token_was_boolop, paren_count, retn_token,
                    (void *)yylval.truthtab,
...
int boolean_parse(void) {
        ...
        if (debugging_boolpars || (usrblk.debug & USRDBG_SRCHCMPL)) {
                fprintf(aa_stderr,
                        ...
                        "  query: '%s'\n",
                        (usrblk.request == OE_SRCH_STEMS), qry_is_all_ANDs,
                        qry_has_no_NOTs, usrblk.query);
                fflush(aa_stderr);
        }
...
int main(int argc, char *argv[]) {
        ...
        fprintf(aa_stderr, "  lang=%d minwdsz=%d maxwdsz=%d.\n",
                dblk.dbrec.or_language, dblk.dbrec.or_minwordsz,
        ...
        while (fgets(linebuf, sizeof(linebuf), stdin) != NULL) {
                ...
                if (!boolean_parse())
                        puts(PROGNAME "707 boolean_parse() returned FALSE "
                                      "(OE_BAD_QUERY).");
                if (DtSearchHasMessages()) {
                        printf("mmmmm Messages returned to user "
                               "mmmmmmmmmmmmmmmmmm\n"
                               "%s\nmmmmm End of messages to user "
