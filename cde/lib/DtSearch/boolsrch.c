...
static struct or_hwordrec *or_wordrecs = NULL;
        ...
        BAD_DBA:
                if (debugging_boolsrch) {
                        fprintf(aa_stderr,
                                ...
                                objrecdba, recno, recno >> 3, 1 << (recno % 8),
                                db_status);
                        fflush(aa_stderr);
                }
                return FALSE;
        ...
        for (i = 0; i < saveusr.stemcount; i++) {
                if (or_wordrecs[i].or_hwaddrs == 0 ||
                    or_wordrecs[i].or_hwordkey[0] == '@')
                        ...
                        /* ln(2) = 0.693147181 */
                        dbl = (double)or_reccount /
                              (double)or_wordrecs[i].or_hwaddrs;
                        idf[i] = log(dbl) / 0.693147181 + 1.0;
                        if (debugging_boolsrch)
                                fprintf(aa_stderr,
                                        PROGNAME
                                        "733 IDF[%d]  numdocs=%5ld  idf=%lf\n",
                                        i, (long)or_wordrecs[i].or_hwaddrs,
                                        idf[i]);
                }
        }
        return;
...
static int load_or_wordrecs(void) {
        ...
        if (or_wordrecs)
                free(or_wordrecs);
        or_wordrecs =
            austext_malloc(saveusr.stemcount * sizeof(struct or_hwordrec) + 16,
                           PROGNAME "782", NULL);
        ...
        for (stemno = 0; stemno < saveusr.stemcount; stemno++) {
                ...
                if (saveusr.stems[stemno][0] == '@') {
                        ...
                        sscanf(saveusr.stems[stemno], COLLOC_STEM_FORMAT, &i,
                               &j, &k);
                ...
                if (db_status == S_OKAY) {
                        ...
                        if (db_status != S_OKAY) {
                                ...
                                sprintf(
                                    msgbuf,
                                    catgets(dtsearch_catd, MS_boolsrch, 6,
                                            "%s Database Error.  Word '%s' is\n"
                                            "listed in database '%s' but has "
                                            "no index record."),
                                    PROGNAME "295", usrblk.stems[stemno],
                                    usrblk.dblk->label);
                        ...
                        if (wordrec->or_hwaddrs > OE_words_hitlimit) {
                                sprintf(
                                    msgbuf,
                                    catgets(dtsearch_catd, MS_boolsrch, 14,
                                            "%s '%s' has more than %ld hits.\n"
                                            "Please remove it from the query "
                                            "or raise the WHITLIM\n"
                                            "value in the search engine "
                                            "configuration file."),
                                    PROGNAME "1444", wordrec->or_hwordkey,
                                    OE_words_hitlimit);
                                DtSearchAddMessage(msgbuf);
...
static int get_proximity(long recno) {
        ...
        for (i = 0; i < proxwtct; i++)
                if (proxwts[i].byteno == byteno &&
                    proxwts[i].bitmask == bitmask)
                        break;
        if (i >= proxwtct)
                return -1;
 ...
 */
static void stuff_DtSrResult(DtSrResult *new, long recno) {
        int m;
        int fzkey_remaining;
        char *src, *targ, *targend;
        static struct or_miscrec miscrecbuf;
        ...
        if (or_abstrsz > 0) {
                ...
                while (db_status == S_OKAY) {
                        ...
                        if (miscrecbuf.or_misctype == ORM_FZKABS) {
                                ...
                                for (m = 0; m < sizeof(miscrecbuf.or_misc);
                                     m++) {
...
static void weights_filter_WK(void) {
        ...
        proxwts =
            austext_malloc(proxwtct * sizeof(PROXWT) + 4, PROGNAME "429", NULL);
        ...
        for (recno = 1; recno < tot_addr_count; recno++) {
                ...
                if (proxwts[smallest].wt > 0.0) {
                        ...
                        RESET_BIT(result_bitvec, smallest_byteno,
                                  smallest_bitmask);
                        result_count--;
                }
        ...
        if (debugging_boolsrch) {
                fprintf(
                    aa_stderr,
                    PROGNAME
                    "489 FINAL PROXWTS proxwtct=%d bigwt=%.2f scalefac=%.2lf\n",
                ...
                fprintf(aa_stderr,
                        PROGNAME "499 WEIGHT RESULTS resultct=%ld  bv=\n",
                        result_count);
                for (i = 0; i < 22; i++) {
                        if (i >= bitveclen)
                                break;
...
static void dbread_filter_WK(void) {
        ...
        for (recno = 1; recno < tot_addr_count; recno++) {
                ...
                if (!all_key_types) {
                        if (strchr(saveusr.ktchars, objrec.or_objkey[0]) ==
                            NULL) {
                                RESET_BIT(result_bitvec, byteno, bitmask);
                                result_count--;
                                if (debugging_boolsrch) {
        ...
        if (debugging_boolsrch) {
                ...
                fprintf(aa_stderr,
                        PROGNAME
                        "857 DBREAD RESULTS discards=%ld resultct=%ld  bv=\n",
        ...
        if (result_count <= 0) {
                ...
                if (debugging_boolsrch)
                        fprintf(aa_stderr, PROGNAME
                                "931 No sorting by statistical weights.\n");
                usrblk.retncode = OE_SEARCHING;
                usrblk.workproc = load_DtSrResults_WK;
        }
        return;
...
static void calc_result_bitvec_WK(void) {
        ...
        if (saveusr.stemcount < DtSrMAX_STEMCOUNT) {
                ...
                if (debugging_boolsrch) {
                        fprintf(aa_stderr,
                                PROGNAME "565 Minimize truth table, "
                                         "pmsz=%d-->%d\n  permutes=",
                                final_truthtab.pmsz, my_pmsz);
                        for (i = 0; i < 16; i++) {
                                if (i >= my_pmsz)
                                        break;
                                fprintf(aa_stderr, " %02x",
                                        (int)my_permutes[i]);
                        }
                        fputc('\n', aa_stderr);
                        fflush(aa_stderr);
        ...
        if (debugging_boolsrch) {
                fprintf(aa_stderr,
                        PROGNAME "621 PRELIM RESULTS resultct=%ld  bv=\n",
                        result_count);
                for (i = 0; i < 22; i++) {
                        if (i >= bitveclen)
                                break;
        ...
        if (result_count <= 0) {
                ...
                if (debugging_boolsrch)
                        fprintf(
                            aa_stderr, PROGNAME
                            "948 No db reads necessary for date or keytype.\n");
                usrblk.retncode = OE_SEARCHING;
                usrblk.workproc = weights_filter_WK;
        } else {
                if (debugging_boolsrch)
                        fprintf(aa_stderr, PROGNAME
                                "625 No filtering: no sort and no db reads.\n");
                usrblk.retncode = OE_SEARCHING;
                usrblk.workproc = load_DtSrResults_WK;
        }
        return;
...
static DB_ADDR read_d99(struct or_hwordrec *wordrec) {
        ...
        if (bufptr >= endbuf) {
                ...
                if (fread(readbuf, sizeof(DB_ADDR), request_read, fptr) !=
                    request_read) {
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_boolsrch, 28,
...
static int get_colloc_bitvec(void) {
        ...
        for (recno = 1; recno < tot_addr_count; recno++) {
                ...
                if ((bloblist = ve_getblobs(dba, vistano)) == NULL) {
                        if (debugging_boolsrch) {
                                fprintf(aa_stderr,
                                        PROGNAME "126 No blobs for recno=%ld "
                                                 "byteno=%ld mask%02x\n",
                ...
                for (a = 0; a < hitwcount_A; a++) {
                        ...
                        threshold_range =
                            offset_A + hitwords_A[a].length + range;
                        for (; b < hitwcount_B; b++) {
...
static void read_stem_bitvec_WK(void) {
        ...
        for (d99recno = read_d99(&or_wordrecs[save_stemno]); d99recno;
             d99recno = read_d99(NULL)) {
                ...
                if ((byteno = d99recno >> 3) >= bitveclen) {
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_boolsrch, 32,
                                        "%s Database Error: %s '%s'\n"
                                        "in database '%s' has invalid d99 "
                                        "record number %ld."),
                                PROGNAME "394",
                                (usrblk.search_type == 'W')
                                    ? catgets(dtsearch_catd, MS_boolsrch, 33,
                                              "Word")
                                    : catgets(dtsearch_catd, MS_boolsrch, 34,
                                              "Stem of"),
                                usrblk.stems[save_stemno], usrblk.dblk->label,
                                d99recno);
                        DtSearchAddMessage(msgbuf);
...
void boolean_search(void) {
        ...
        /* Sanity checks */
        if (saveusr.stemcount <= 0 || final_truthtab.pmsz <= 0 ||
            final_truthtab.pmsz >= 256) {
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_boolsrch, 35,
                                "%s Program Error: stemct=%d pmsz=%d\n"),
                        PROGNAME "1404", saveusr.stemcount,
                        final_truthtab.pmsz);
                DtSearchExit(14);
        }
        ...
        ********/
        ...
        if (debugging_boolsrch) {
                fprintf(
                    ...
                    usrblk.search_type, need_zero_permute, do_stat_sort,
                    aa_maxhits, (long)usrblk.dblk->dbrec.or_maxdba, or_reccount,
                    or_recslots, tot_addr_count, bitveclen, all_key_types,
                    saveusr.ktchars);
                fflush(aa_stderr);
        }
        ...
        if (debugging_boolsrch)
                fprintf(aa_stderr,
                        PROGNAME "430 "
                                 "bitvecs[] alloc needed=%lu (bvln=%ld "
                                 "stems=%d+1), have=%lu.\n",
                        (unsigned long)allocsz_needed, bitveclen,
                        saveusr.stemcount, (unsigned long)bitvec_allocsz);
        if (bitvec_allocsz < allocsz_needed) {
                if (bitvec_allocp)
                        free(bitvec_allocp);
                bitvec_allocp =
                    austext_malloc(allocsz_needed + 16, PROGNAME "508", NULL);
                if (debugging_boolsrch)
                        fprintf(aa_stderr,
                                PROGNAME "432 bitvecs[] realloc %lu-->%lu.\n",
                                (unsigned long)bitvec_allocsz,
                                (unsigned long)allocsz_needed);
