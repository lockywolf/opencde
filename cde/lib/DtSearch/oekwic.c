...
static void make_one_kwic(char *abstract) {
        ...
        /* Move text beyond the word until end of input or end of abstract
         * buffer */
...
void oe_ditto2kwic(void) {
         ...
         * Until workproc/mainloop is done, the static variable
         * 'saveusr.dittolist' will always indicate where the last execution of
         * the loop ended.
         */
...
void kwic_workproc(void) {
        ...
        for (iter = saveusr.iterations; iter > 0;) {
                ...
                if ((bloblist = ve_getblobs(dit->dba, saveusr.vistano)) ==
                    NULL) {
                        sprintf(sprintbuf,
                                PROGNAME
                                "149 "
        ...
        if (!entire_list_done) {
                ...
                if (time_dif < 1.)
                        ...
                        saveusr.iterations =
                            (double)saveusr.iterations / time_dif;
                ...
                saveusr.dittolist =
                    dit; /* temp save curr loc in usrblk.dittolist */
...
void oe_findstr_hitl(void) {
        ...
        if (strlen(usrblk.query) >= DtSrMAXWIDTH_HWORD) {
                sprintf(msgbuf,
                        PROGNAME "516 "
                                 "No more than %d characters are allowed in "
                                 "the search string.",
        ...
        if (usrblk.dittocount <= 0 || usrblk.dittolist == NULL) {
                DtSearchAddMessage(PROGNAME
                                   "317 Hitlist empty.  Nothing to search.");
        ...
        *ptr = 0;
        ...
        /* initialize saveusr loop status stuff maintained between workproc
         * calls */
        saveusr.vistano = usrblk.dblk->vistano;
         ...
         * Until workproc/mainloop is done, the static variable
         * 'saveusr.dittolist' will always indicate where the last execution of
         * the loop ended. The ditto node for each record which is found to
         * contain the string will be removed from dittolist and added to
         * newditlist.
         */
        usrblk.flags &= ~USR_STOPSRCH; /* init stop button to OFF */
 ...
 */
        ...
        if (saveusr.dittocount > 0) {
                ...
                while (dit != NULL) {
                        /* On a hit, advance the pointers, advance new
                         * dittocount. Break the loop on the last hit.
                         */
                        if (dit->flags & DIT_FINDSTR) {
#ifdef DEBUG_FINDSTR
                                printf("#%ld HIT %s, \"%s\"\n",
                                       newdittocount + 1, dit->reckey,
                                       dit->abstract);
#endif
                                lastlink = &dit->link;
                                dit = dit->link;
                ...
                while (dit != NULL) {
                        if (!(usrblk.flags & USR_STOPSRCH) /* never stopped */
                            || !(dit->flags &
                                 DIT_STOP)) /* stopped somewhere ahead */
                        {
#ifdef DEBUG_FINDSTR
        ...
        printf("LEAVING RESTORE now real dittocount = %ld, stemcount = %d\n"
               "   first stem = '%s'\n",
               usrblk.dittocount, usrblk.stemcount, usrblk.stems[0]);
#endif
        return;
} /* restore_findstr_hitl() */
        ...
        for (iter = saveusr.iterations; iter > 0;) {
                ...
                if ((bloblist = ve_getblobs(dit->dba, saveusr.vistano)) ==
                    NULL) {
                        DtSearchAddMessage(
                            PROGNAME
                            "390 Corrupted database address on hitlist.");
                        restore_findstr_hitl();
                        usrblk.retncode = OE_BAD_HITLIST;
                        return;
                }
