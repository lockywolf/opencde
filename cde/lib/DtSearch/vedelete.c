...
#define MAX_REC_READ                                                           \
        128 /* Max number of addresses to be read from                         \
...
static int delete_addrs(void) {
        ...
        while (not_done) {
                ncopy = 0;
                /*
                 * read only a single block of max recs, or fewer dba's if this
                 * is the last block
                 */
                ...
                if (fread(word_addrs, sizeof(DB_ADDR), num_reads,
                          dtbs_addr_fp) != num_reads) {
                        puts("\n\r" PROGNAME "98 "
                             "AusText is shutting down due to corrupted "
                             "k2x/d99 links.");
                        DtSearchExit(99);
                }
                ...
                for (i = 0; i < num_reads; i++) {
                        ...
                        if (dba_eq != NULL) {
                                /*
                                 * HIT! the passed word links to this dba. Don't
                                 * copy the dba to copy array.
                                 */
                                num_found++;
                                addrs_found = TRUE;
                        } else {
                                /* MISS! Keep the dba by copying it to the copy
                                 * array */
                ...
                if (addrs_found) {
                        /*
                         * Once we start deleting dba's: save the curr 'read'
                         * loc, write the shorter 'copy' array to the output
                         * file at the curr 'write' loc, update the 'write' loc
                         * ptr, restore the 'read' loc.
                         */
                        read_offset = ftell(dtbs_addr_fp);
                        if (read_offset <= 0L) {
                                printf(PROGNAME
                                       "169 ftell failed on '%s%s.d99'.\n",
                                       usrblk.dblk->path, usrblk.dblk->name);
                                DtSearchExit(99);
                        }
                        if (fseek(dtbs_addr_fp, write_offset, SEEK_SET)) {
                                printf(
                                    PROGNAME
                                    "175 fseek to %ld failed on '%s%s.d99'.\n",
                                    write_offset, usrblk.dblk->path,
                                    usrblk.dblk->name);
                                DtSearchExit(99);
                        }
                        num_writes = fwrite(copy_addrs, sizeof(DB_ADDR), ncopy,
                                            dtbs_addr_fp);
                        if (num_writes != ncopy) {
                                printf(
                                    PROGNAME
                                    "283 fwrite at pos %ld failed on "
                                    "'%s%s.d99'.\n"
                        ...
                        if (fseek(dtbs_addr_fp, read_offset, SEEK_SET)) {
                                printf(
                                    PROGNAME
                                    "198 fseek to %ld failed on '%s%s.d99'.\n",
                                    read_offset, usrblk.dblk->path,
                                    usrblk.dblk->name);
                                DtSearchExit(99);
                        }
                } /* end if (addrs_found) */
                else {
                        /* [do nothing, leave this block as is, just incr write
                         * ptr] */
                        write_offset = ftell(dtbs_addr_fp);
                        if (write_offset <= 0L) {
                                printf(PROGNAME
                                       "208 ftell failed on '%s%s.d99'.\n",
                                       usrblk.dblk->path, usrblk.dblk->name);
                                DtSearchExit(99);
                        }
                }
        ...
        if (addrs_found) {
                ...
                if (got_hword.or_hwaddrs < 0) {
                        printf(PROGNAME
                               "220 duplicate dbas for '%s' in '%s%s.d99'.\n",
                ...
                if (got_hword.or_hwaddrs == 0) {
                        ...
                        /*
                         * @@@ fill out newly freed dba positions with FF's as
                         * debugging aid?
                         */
                        updated_words++;
                        return UPDATE_WORD;
...
static void print_counters(char *wordtype) {
        ...
        if (wordtype == NULL) {
                ...
                sprintf(msgbuf,
                        ...
                        totupdwords + totdelwords + totskipwords,
                        totupdwords + totskipwords, totfreedbytes);
                ...
                sprintf(msgbuf,
                        ...
                        updated_words + deleted_words + skipped_words, wordtype,
                ...
                updated_words = deleted_words = skipped_words = freed_bytes =
                    0L;
        }
        return;
} /* print_counters() */
...
void ve_delete(void) {
        ...
        if (debugging) {
                ...
                if (verbose_debugging)
                        for (i = 0; i < dbacount; i++) {
                                ...
                                CRREAD(PROGNAME "179", OR_OBJKEY, charbuf,
                                       vistano);
                                fprintf(aa_stderr,
                                        " #%d\tdba=%ld:%ld  key='%s'\n", i,
                                        dba >> 24, dba & 0xffffff, charbuf);
                        }
        }
        ...
        for (i = 1; i < dbacount; i++) {
                ...
                dba = usrblk.dbatab[i];
                sprintf(charbuf,
                        PROGNAME "370 Duplicate dba = %ld:%ld ignored.",
        ...
        if (debugging) {
                ...
                if (verbose_debugging)
                        for (i = 0; i < dbacount; i++) {
                                ...
                                CRREAD(PROGNAME "359", OR_OBJKEY, charbuf,
                                       vistano);
                                fprintf(aa_stderr,
                                        " #%d\tdba=%ld:%ld  key='%s'\n", i,
                                        (long)dba >> 24,
                                        (long)(dba & 0x00ffffff), charbuf);
                        }
        }
         ...
         * process, only those loops will be executed as determined by
         * database's maxwordsz. However delete_addrs() function always uses the
        ...
        fprintf(aa_stderr,
                PROGNAME
                "470 All words processed.  Now deleting %d objects...\n",
                dbacount);
        for (i = 0; i < dbacount; i++) {
                dba = usrblk.dbatab[i];
                ...
                if (db_status != 0) {
                        ...
                        fprintf(aa_stderr,
                                PROGNAME "153 Abort: "
                                         "vistano=%d, dbatab #%d (out of %d), "
                                         "dba=%ld:%ld.\n",
                                vistano, i, dbacount, (long)dba >> 24,
                                (long)(dba & 0x00ffffff));
                        fflush(aa_stderr);
                        DtSearchExit(53);
                }
