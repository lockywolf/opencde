...
void oe_initialize(void) {
        ...
        if (usrblk.debug & USRDBG_RARE)
                fprintf(aa_stderr,
                        PROGNAME "555  Entered Engine Initialization.\n");
         ...
         * OE_sitecnfg_fname is initialized by ausapi or loadocf and never
         * reset. OE_sitecnfg_mtime is initialized by oe_initialize() and never
         * reset.
         */
                ...
                if (!is_compatible_version(usrblk.query, SCHEMA_VERSION) ||
                    !is_compatible_version(usrblk.query, PROTOCOL_VERSION)) {
                        if (usrblk.query != NULL)
                                ...
                                ptr = catgets(dtsearch_catd, MS_misc, 6,
                                              "<unknown>");
                        sprintf(
                            sprintbuf,
        ...
        if (!load_ocf()) {
                sprintf(sprintbuf,
                        catgets(dtsearch_catd, MS_oeinit, 202,
                                "%s Initialization failed due to errors in "
                                "configuration file."),
                        PROGNAME "202");
                DtSearchAddMessage(sprintbuf);
                usrblk.retncode = OE_NOTAVAIL;
        ...
        DELETE_DB:
                /*
                 * One or more language or semantic files could not be loaded
                 * for this dblk.  Unlink it and don't increment pointers.
                 */
                bad_db = db; /* temp save */
                *lastlink = db->link;
                db = db->link;
        ...
        if (usrblk.debug & USRDBG_RARE) {
                ...
                if (OE_sitecnfg_mtime != 0) {
                        time_ptr =
                            _XLocaltime(&OE_sitecnfg_mtime, localtime_buf);
...
void oe_uninitialize(void) {
        ...
        while (db != NULL) {
                ...
                /* Save next dblk so we'll know where to go after freeing this
                 * one */
                nextdb = db->link;
