        ...
        for (d = usrblk.dblist; d != NULL; d = d->link) {
                ...
                *ptr = 0;
                printf("DBLK at %p link=%p name='%s' max=%d\n"
                       "    keytypes='%s', path='%s'\n",
                       d, d->link, d->name, d->maxhits, typestring,
                       NULLORSTR(d->path));
        }
        ...
        if (strlen(token) > 8) {
                sprintf(sprintbufp,
                        catgets(dtsearch_catd, MS_loadocf, 184,
                                "%s %s: More than 8 characters in Database "
                                "name '%.16s'.\n"),
...
static void set_fileio(_Xstrtokparams *strtok_buf) {
        ...
        if (token[0] != '-') {
                OE_fileio =
                    austext_malloc(strlen(token) + 2, PROGNAME "592", NULL);
...
static void read_rest_of_line(char *keyword, char **passed_ptr,
        ...
        if ((token = _XStrtok(NULL, "\n", *strtok_buf)) == NULL) {
                sprintf(sprintbufp,
                        catgets(dtsearch_catd, MS_loadocf, 1007,
...
static void read_filepath(_Xstrtokparams *strtok_buf) {
        ...
        if (oef_table[tabx].previously_specified) {
                errp = catgets(dtsearch_catd, MS_loadocf, 166,
        ...
        if ((token = _XStrtok(NULL, DELIMITERS, *strtok_buf)) == NULL) {
                sprintf(sprintbufp,
                        catgets(dtsearch_catd, MS_loadocf, 221,
                                "%s %s: Database '%s', No PATH specified, "
                                "statement ignored.\n"),
...
static void obsolete_keyword(char *keyword) {
        sprintf(sprintbufp,
                catgets(dtsearch_catd, MS_loadocf, 1,
...
static int add_a_keytype(char ktchar) {
        ...
        sprintf(kt[newindex].name,
                catgets(dtsearch_catd, MS_loadocf, 457, "'%c' Records"),
                ktchar);
        ...
        if (((ascii_charmap[ktchar] & 0xff) != ktchar) && OE_uppercase_keys) {
                sprintf(
                    ...
                    catgets(dtsearch_catd, MS_loadocf, 1011,
                            ...
                            "  Set UPPERKEYS = FALSE if lowercase keys are "
                            "intended."),
...
static void read_keytypes(int how_many, _Xstrtokparams *strtok_buf) {
        ...
        while ((token = _XStrtok(NULL, DELIMITERS, *strtok_buf)) != NULL) {
                ...
                /*
                 * Token is a name string.  If the last token was NOT a ktchar
                 * in single quotes, or it was an invalid ktchar, then create a
                 * new keytypes entry with the ktchar equal to the first char of
                 * this label. If any err, don't update the name string.
                 */
                if (last_ktchar < 0) {
                        if (how_many <= 0)
                                break; /* don't exceed our quota */
...
static int ocfopen(char *prefix, char *fname, FILE **stream) {
        ...
        if (prefix == NULL) {
                if (debug_ocf)
                        fprintf(aa_stderr,
                                " --> open not attempted: null prefix.\n");
                return FALSE;
        }
...
int load_ocf(void) {
        ...
        if (OE_sitecnfg_fname) {
                if (debug_ocf)
                        fprintf(aa_stderr,
                                PROGNAME
                                "1446 ocfopen '%s', prespecified file name.\n",
                                OE_sitecnfg_fname);
        ...
        if (debug_ocf)
                fprintf(aa_stderr,
                        PROGNAME "1753 ocfopen '%s'. DTSROCFPATH='%s'.\n",
        ...
        sprintf(
            ...
            (p) ? p
                : catgets(dtsearch_catd, MS_misc, 16,
            ...
            (OE_inittab_dir)
                ? OE_inittab_dir
                : catgets(dtsearch_catd, MS_misc, 17, "<no server daemon>"),
            (OE_server_dir)
                ? OE_server_dir
                : catgets(dtsearch_catd, MS_misc, 17, "<no server daemon>"));
        ...
        *inbuf = '\0';
        while (fgets(inbuf, sizeof(inbuf) - 1, stream) != NULL) {
                ...
                if (strcmp(uprtoken, "USERNOTES") == 0) {
                        set_boolint(&OE_enable_usernotes, uprtoken,
                                    &strtok_buf);
                        continue;
                }
                ...
                if (strcmp(uprtoken, "NOITERATE") == 0) {
                        set_boolbit(&OE_flags, OE_NO_ITERATE, uprtoken,
                                    &strtok_buf);
                        continue;
                }
                ...
                if (strcmp(uprtoken, "LABEL") == 0) {
                        if (read_database(uprtoken, &strtok_buf))
                                read_rest_of_line(uprtoken, &db->label,
                                                  &strtok_buf);
                        continue;
                }
        ...
        if (usrblk.dblist == NULL) {
                sprintf(sprintbufp,
                        catgets(dtsearch_catd, MS_loadocf, 1869,
                                "%s %s: Configuration file must specify at "
                                "least one database."),
                ...
                for (db = usrblk.dblist; db != NULL; db = db->link) {
                        if (db->ktcount <= 0) {
                                sprintf(sprintbufp,
                                        catgets(dtsearch_catd, MS_loadocf, 986,
                                                "%s %s: KEYTYPES missing for "
                                                "database '%s'."),
                                        PROGNAME "986", cfgfname, db->name);
                                DtSearchAddMessage(sprintbufp);
                                cutnode_llist((LLIST *)db,
                                              (LLIST **)&usrblk.dblist);
                                free(db);
                                fatal_error = TRUE;
                        }
