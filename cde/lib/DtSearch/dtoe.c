...
static void expired(char *sprintbuf) {
        sprintf(
            sprintbuf,
            catgets(dtsearch_catd, MS_oe, 71, PROGNAME "71 %s has expired."),
            OE_prodname);
...
int oe_unblob(LLIST *bloblist) {
        ...
        while (lptr != NULL) {
                ...
                blobclearlen =
                    bptr->or_bloblen; /* len of cleartext in curr blob */
...
void oe_write_audit_rec(long numhits) {
        ...
        {
                sprintf(sprintbuf,
                        catgets(dtsearch_catd, MS_misc, 1596,
...
static int no_keytypes(void) {
        ...
        sprintf(
            ...
            catgets(dtsearch_catd, MS_oe, 440,
                    PROGNAME
                    "440 No record keytypes were selected in database '%s'."),
            usrblk.dblk->label);
        DtSearchAddMessage(sprintbuf);
...
void Opera_Engine(void) {
        ...
        if (usrblk.debug != 0L) {
                ...
                fprintf(aa_stderr,
                        ...
                        "  user='%s', usrblk.flags=%ld(x%04lx), "
                        "usrblk.debug=%ld(x%04lx).\n"
                        "  OE_flags=%ld(x%04lx), exp=%s, sitecnfg=%s.\n",
                        usrblk.request, request_str(usrblk.request), sprintbuf,
                        usrblk.userid, usrblk.flags, usrblk.flags, usrblk.debug,
                        usrblk.debug, OE_flags, OE_flags, sprintbuf + 100,
                        sprintbuf + 200);
                i = 0;
        ...
        if (OE_flags & OE_PERMERR) {
                sprintf(sprintbuf,
                        catgets(dtsearch_catd, MS_oe, 490,
                                PROGNAME "490 %s Engine permanently disabled."),
                        OE_prodname);
                DtSearchAddMessage(sprintbuf);
                usrblk.retncode = OE_ABORT;
                goto ENGINE_RETURN;
        ...
        if ((usrblk.request != OE_INITIALIZE) && !(OE_flags & OE_INITOK)) {
                DtSearchAddMessage(
                    catgets(dtsearch_catd, MS_oe, 523,
        ...
        if (usrblk.request != OE_INITIALIZE) {
                ...
                for (db = usrblk.dblist, OE_dbn = 0; db != NULL;
                     db = db->link, OE_dbn++)
        ...
        case OE_GETREC_DIC:
                 ...
                 */
                        ...
                        switch (usrblk.request) {
                        case OE_GETREC_WORDS:
                                hilite_cleartext('W', (char *)usrblk.stems,
                                                 usrblk.stemcount);
                                break;

                        case OE_GETREC_STEMS:
                                hilite_cleartext('S', (char *)usrblk.stems,
                                                 usrblk.stemcount);
                                break;
                ...
                if (OE_flags & OE_PERMERR)
                        ...
                        usrblk.retncode =
                            ((usrblk.clearlen) ? OE_OK : OE_NOTAVAIL);
                        if (usrblk.debug & USRDBG_RETRVL)
