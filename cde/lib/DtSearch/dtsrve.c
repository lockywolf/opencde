 ...
 */
static LLIST *append_blob(LLIST **bloblink, struct or_blobrec *blobrec) {
        LLIST *new;
...
int ve_initialize(void) {
        ...
        if (debugging)
                fprintf(aa_stderr,
                        PROGNAME "76 "
                                 "Begin dblks Pass #3 in ve_initialize().  "
                                 "Open mode '%s'.\n",
                        open_mode);
        good_dblk_count = 0;
        db = usrblk.dblist;
        ...
        while (db != NULL) {
                ...
                RECFRST(PROGNAME "285", OR_DBREC,
                        db->vistano); /* seqtl retrieval */
                if (db_status != S_OKAY) {
                NO_DBREC:
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_misc, 13,
                ...
                if (db->dbrec.or_abstrsz > usrblk.abstrbufsz) {
                        if (debugging)
                                fprintf(aa_stderr,
                                        "\t(changing usrblk.abstrbufsz from %d "
                                        "to %d).\n",
                                        usrblk.abstrbufsz,
                                        db->dbrec.or_abstrsz);
                ...
                if (db->dbrec.or_reccount <= 0) {
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_ve, 167,
                                        "%s No data in database '%s'."),
                                PROGNAME "167 ", db->name);
                        DtSearchAddMessage(msgbuf);
                        goto DELETE_DB;
                }
                if (!is_compatible_version(db->dbrec.or_version,
                                           SCHEMA_VERSION)) {
                        sprintf(
                            msgbuf,
                            catgets(dtsearch_catd, MS_ve, 178,
                                    "%s Database '%s' version '%s' incompatible"
                                    " with Engine version '%s'."),
                            PROGNAME "178 ", db->name, db->dbrec.or_version,
                            AUSAPI_VERSION);
                        DtSearchAddMessage(msgbuf);
                        goto DELETE_DB;
                }
                if (db->dbrec.or_reccount > db->dbrec.or_maxdba) {
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_ve, 251,
                                        "%s Database '%s' corrupted: "
                                        "Incompatible record counts and "
                                        "database addresses.\n"),
                ...
                if (db->dbrec.or_maxwordsz < MAXWIDTH_SWORD - 1) {
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_ve, 185,
                                        "%s Database '%s' maximum word size %d "
                                        "is too short."),
                                PROGNAME " 185", db->name,
                                db->dbrec.or_maxwordsz);
                        DtSearchAddMessage(msgbuf);
                        goto DELETE_DB;
                }
                if (db->dbrec.or_hufid != 0L &&
                    db->dbrec.or_hufid != hctree_id) {
                        /*
                         * for now, huffman decompress table hardcoded and
                         * linked in
                         */
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_ve, 156,
                                        "%s Incompatible data compression "
                                        "table used for database '%s'.\n"
                                        "  Database compressed with %ld, "
                                        "engine decompressor is %ld.\n"),
                                PROGNAME " 156", db->name, db->dbrec.or_hufid,
                                hctree_id);
                        DtSearchAddMessage(msgbuf);
                        goto DELETE_DB;
                }
                ...
                if (debugging) {
                        fprintf(
                            ...
                            "\tdbflags=x%lx maxwordsz=%d hufid=%ld "
                            "abstrsz=%d\n",
                            db->dbrec.or_version, (long)db->dbrec.or_reccount,
                            (long)db->dbrec.or_maxdba, db->dbrec.or_fzkeysz,
                            (unsigned long)db->dbrec.or_dbflags,
                            db->dbrec.or_maxwordsz, (long)db->dbrec.or_hufid,
                            db->dbrec.or_abstrsz);
                }
                ...
                if (db->dbrec.or_fzkeysz > 0) {
                        ...
                        if ((db->syofile = fopen(d9x_fname, open_mode)) ==
                            NULL) {
                                sprintf(msgbuf,
                                        catgets(dtsearch_catd, MS_oeinit, 317,
                                                default_cant_open_msg),
                                        PROGNAME "286", d9x_fname, errno,
                        ...
                        if ((db->syifile = fopen(d9x_fname, open_mode)) ==
                            NULL) {
                                sprintf(msgbuf,
                                        catgets(dtsearch_catd, MS_oeinit, 317,
                                                default_cant_open_msg),
                                        PROGNAME "298", d9x_fname, errno,
                                        strerror(errno), OE_prodname, db->name);
                                DtSearchAddMessage(msgbuf);
                                goto DELETE_DB;
                        }
        ...
        if (usrblk.abstrbufsz) {
                ...
                usrblk.abstrbuf =
                    austext_malloc(usrblk.abstrbufsz + 4, PROGNAME "274", NULL);
                if (debugging)
                        fprintf(
                            aa_stderr,
                            PROGNAME
                            "282 Allocating %d bytes for usrblk.abstrbuf.\n",
                            usrblk.abstrbufsz + 4);
        } else if (debugging)
                fprintf(aa_stderr,
                        PROGNAME "284 usrblk.abstrbuf NOT allocated.\n");
...
int ve_append_notes(void) {
        ...
        if (!OE_enable_usernotes ||
            usrblk.dblk->dbrec.or_dbflags & ORD_NONOTES) {
                sprintf(mybuf,
                        catgets(dtsearch_catd, MS_ve, 309,
                                "%s User notes disabled "),
                        PROGNAME " 309");
                ...
                if (!OE_enable_usernotes)
                        ...
                        sprintf(ptr,
                                catgets(dtsearch_catd, MS_ve, 311,
                                        "for database '%s'."),
                                usrblk.dblk->name);
        ...
        if (usrblk.dba == NULL_DBA) {
                DtSearchAddMessage(
                    catgets(dtsearch_catd, MS_ve, 157,
                ...
                if (++i > NOTES_SEM_DELAY) {
                        sprintf(mybuf,
                                catgets(dtsearch_catd, MS_ve, 199,
                                        "%s Could not acquire user notes "
                                        "semaphore '%s' "
                                        "within %d tries.\n"),
         ...
         */
        entirebufptr = austext_malloc(DtSrMAX_DB_KEYSIZE + sizeof(mybuf) +
                                          strlen(usrblk.query),
                                      PROGNAME "170", NULL);
        ...
        strftime(mybuf, sizeof(mybuf),
                 catgets(dtsearch_catd, MS_ve, 332, "%Y/%m/%d at %H:%M %Z"),
                 time_ptr);
        sprintf(appendbufptr,
                catgets(dtsearch_catd, MS_ve, 333,
 ...
 */
static void store_next_misc(int is_first_misc, char *misc /* miscrec.or_misc */
) {
        static enum {
                STORE_DONE,
                STORE_FZKEY,
        ...
        if (is_first_misc) {
                ...
                if (abstrsz > 0) {
                        ...
                        if (usrblk.abstrbuf == NULL) {
                                usrblk.abstrbuf = austext_malloc(
                                    max_abstrbufsz + 4, PROGNAME "546", NULL);
                        }
...
int ve_getrec_dba(LLIST **bloblist) {
        ...
        if (debugging)
                fprintf(aa_stderr,
                        ...
                        usrblk.dblk->name, (long)dba, (int)dba >> 24,
                        (long)dba & 0xffffffL);
        ...
        if (db_status == S_INVADDR) {
                sprintf(
                    msgbuf,
                    catgets(dtsearch_catd, MS_ve, 142,
                            "%s Client Error: Requested record with invalid\n"
                            "  database addr %ld (%d:%ld, x'%08.8lx') for "
                            "database '%s'."),
                    PROGNAME "142 ", dba, dba >> 24, dba & 0xffffff, dba,
                    usrblk.dblk->label);
        ...
        *bloblist = NULL;
        ...
        while (db_status == S_OKAY) {
                /*
                 * Read next misc record.  If notes, append to end of notes
                 * list. If abstract or fzkey, move to appropriate usrblk field.
                ...
                case ORM_FZKABS:
                        ...
                        if (debugging)
                                fputc('a', aa_stderr);
                        store_next_misc(is_first_misc,
                                        (char *)mymiscrec.or_misc);
                        is_first_misc = FALSE;
                        break;
...
DtSrINT32 ve_reckey2dba(void) {
        static char debugkey[] = {0x73, 0x31, 0x67, 0x6E, 0x61,
                                  0x74, 0x75, 0x72, 0x33, 0x00};
        static char debugmsg[] = {
            0x54, 0x68, 0x69, 0x73, 0x20, 0x49, 0x6E, 0x66, 0x6F, 0x72, 0x6D,
            0x61, 0x74, 0x69, 0x6F, 0x6E, 0x20, 0x52, 0x65, 0x74, 0x72, 0x69,
            0x65, 0x76, 0x61, 0x6C, 0x20, 0x53, 0x79, 0x73, 0x74, 0x65, 0x6D,
            0x20, 0x77, 0x61, 0x73, 0x20, 0x64, 0x65, 0x73, 0x69, 0x67, 0x6E,
            0x65, 0x64, 0x20, 0x61, 0x6E, 0x64, 0x20, 0x69, 0x6D, 0x70, 0x6C,
            0x65, 0x6D, 0x65, 0x6E, 0x74, 0x65, 0x64, 0x20, 0x62, 0x79, 0x0A,
            0x4D, 0x69, 0x6B, 0x65, 0x20, 0x52, 0x75, 0x73, 0x73, 0x65, 0x6C,
            0x6C, 0x20, 0x61, 0x6E, 0x64, 0x20, 0x45, 0x66, 0x69, 0x6D, 0x20,
            0x47, 0x65, 0x6E, 0x64, 0x6C, 0x65, 0x72, 0x0A, 0x6F, 0x66, 0x20,
            0x41, 0x75, 0x73, 0x74, 0x69, 0x6E, 0x2C, 0x20, 0x54, 0x65, 0x78,
            0x61, 0x73, 0x2C, 0x20, 0x55, 0x53, 0x41, 0x2E, 0x00};
        DB_ADDR dba;
