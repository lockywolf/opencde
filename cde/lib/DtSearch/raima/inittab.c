...
#define TS_ENABLE 0
...
#endif
        size_ft = size_rt = size_st = size_mt = size_srt = size_fd = size_kt =
            0;
        ...
        for (dbt_lc = 0, curr_db_table = &db_table[old_no_of_dbs];
             dbt_lc < no_of_dbs; ++dbt_lc, ++curr_db_table) {
                ...
                if (con_dbd(dbfile, dbname, get_element(dbdpath, dbt_lc)) !=
                    S_OKAY)
        ...
        goodver:
                ...
                if (debugging_inittab) {
                        printf(__FILE__ "152 sizes: pg=%d ft=%d rt=%d fd=%d\n"
                               ...
                               (int)DB_REF(Page_size), (int)DB_REF(Size_ft),
                               (int)DB_REF(Size_rt), (int)DB_REF(Size_fd),
                               (int)DB_REF(Size_st), (int)DB_REF(Size_mt),
                               (int)DB_REF(Size_srt), (int)DB_REF(Size_kt));
                        fflush(stdout);
                }
#endif
             ...
             dbt_lc < no_of_dbs; ++dbt_lc, ++curr_db_table) {
...
#ifdef DEBUG_INITTAB
                        if (dump_init_tables) {
                                printf(" FILE#%d: ty=%c slts=%2d slsz=%3d "
                                       "pgsz=%d '%s'\n",
                                       (int)i, file_ptr->ft_type,
...
#ifdef DEBUG_INITTAB
                        if (dump_init_tables) {
                                printf(" REC #%d: fil=%d len=%3d data=%2d "
                                       "fld1=%2d flds=%2d\n",
                                       (int)i, (int)rec_ptr->rt_file,
                                       (int)rec_ptr->rt_len,
                                       (int)rec_ptr->rt_data,
                ...
                for (i = 0, fld_ptr = &field_table[ORIGIN(fd_offset)];
                     i < DB_REF(Size_fd); i++, fld_ptr++) {
                        ...
                        if (dump_init_tables) {
                                if (i == 0)
                                        puts("         key typ len kfil key# "
                                             "ofs rec# flg dims");
                                printf(" FLD#%2d  %c   %c  %3d  %d    %d   %3d "
                                       " %d    %x",
                                       (int)i, fld_ptr->fd_key,
                                       fld_ptr->fd_type, (int)fld_ptr->fd_len,
...
#ifdef DEBUG_INITTAB
                        if (dump_init_tables) {
                                printf(" SET #%d: ord=%c owner=%d ownofs=%2d "
                                       "mem1=%d mems=%d\n",
                                       (int)i, (char)set_ptr->st_order,
...
#ifdef DEBUG_INITTAB
                        if (dump_init_tables) {
                                printf(" MEM #%d: rec=%d ofs=%d sort1=%d "
                                       "sorts=%d\n",
                                       (int)i, (int)mem_ptr->mt_record,
                        ...
                        for (i = 0, file_ptr = &file_table[ORIGIN(ft_offset)];
                             i < DB_REF(Size_ft); ++i, ++file_ptr) {
                                ...
                                if (DB_REF(db_path[0]))
                                        ...
                                        dbname[0] = '\0';
                                if (strlen(dbname) + strlen(DB_REF(db_name)) >=
                                    FILENMLEN + 4)
                                        return (dberr(S_NAMELEN));
                                strcat(dbname, DB_REF(db_name));
                                if (con_dbf(dbfile, file_ptr->ft_name, dbname,
                                            get_element(dbfpath, dbt_lc)) !=
                                    S_OKAY)
                                        return (dberr(db_status));
                ...
                for (i = ORIGIN(rt_offset),
                    rec_ptr = &record_table[ORIGIN(rt_offset)];
                     i < ORIGIN(rt_offset) + DB_REF(Size_rt); ++i, ++rec_ptr) {
#ifndef ONE_DB
                ...
                for (key_count = 0, i = ORIGIN(fd_offset),
                    fld_ptr = &field_table[ORIGIN(fd_offset)];
                     i < ORIGIN(fd_offset) + DB_REF(Size_fd); ++i, ++fld_ptr) {
#ifndef ONE_DB
                ...
                for (i = ORIGIN(st_offset),
                    set_ptr = &set_table[ORIGIN(st_offset)];
                     i < ORIGIN(st_offset) + DB_REF(Size_st); ++i, ++set_ptr) {
#ifndef ONE_DB
...
static int alloc_dict() {
        ...
        if (ALLOC_TABLE(&db_global.File_table, new_size, old_size,
                        "file_table") != S_OKAY) {
                return (db_status);
        }
        ...
        if (ALLOC_TABLE(&db_global.Record_table, new_size, old_size,
                        "record_table") != S_OKAY) {
                return (db_status);
        }
        ...
        if (ALLOC_TABLE(&db_global.Field_table, new_size, old_size,
                        "field_table") != S_OKAY) {
                return (db_status);
        }
        ...
        if (size_st) {
                ...
                if (ALLOC_TABLE(&db_global.Set_table, new_size, old_size,
                                "set_table") != S_OKAY) {
                        return (db_status);
                }
        } else
        ...
        if (size_mt) {
                ...
                if (ALLOC_TABLE(&db_global.Member_table, new_size, old_size,
                                "member_table") != S_OKAY) {
                        return (db_status);
                }
        } else
        ...
        if (size_srt) {
                ...
                if (ALLOC_TABLE(&db_global.Sort_table, new_size, old_size,
                                "sort_table") != S_OKAY) {
                        return (db_status);
                }
        } else
        ...
        if (size_kt) {
                ...
                if (ALLOC_TABLE(&db_global.Key_table, new_size, old_size,
                                "key_table") != S_OKAY) {
                        return (db_status);
                }
        } else
...
static int initcurr() {
        ...
        if (size_st) {
                ...
                if (ALLOC_TABLE(&db_global.Curr_own, new_size, old_size,
                                "curr_own") != S_OKAY) {
                        return (db_status);
                }
                if (ALLOC_TABLE(&db_global.Curr_mem, new_size, old_size,
                                "curr_mem") != S_OKAY) {
                        return (db_status);
                }
#ifndef NO_TIMESTAMP
                ...
                if (db_tsrecs) {
                        if (ALLOC_TABLE(&db_global.Co_time, new_size, old_size,
                                        "co_time") != S_OKAY) {
                                return (db_status);
                        }
                        if (ALLOC_TABLE(&db_global.Cm_time, new_size, old_size,
                                        "cm_time") != S_OKAY) {
                                return (db_status);
                        }
                }
                if (db_tssets) {
                        if (ALLOC_TABLE(&db_global.Cs_time, new_size, old_size,
                                        "cs_time") != S_OKAY) {
                                return (db_status);
                        }
                }
#endif
                        /* for each db make system record as curr_own of its
                         * sets */
#ifndef ONE_DB
                for (dbt_lc = no_of_dbs,
                            ...
                            rec_ptr = &record_table[ORIGIN(rt_offset)];
                             ...
                             ++rec, ++rec_ptr) {
                                if (rec_ptr->rt_fdtot == -1) {
                                        /* found system record */
                                        curr_rec = ((FILEMASK &
                                                     NUM2EXT(rec_ptr->rt_file,
                                                             ft_offset))
                                                    << FILESHIFT) |
                                                   1L;
                                        /* make system record current of sets it
                                         * owns */
                                        for (i = ORIGIN(st_offset),
                                            set_ptr =
                                                 &set_table[ORIGIN(st_offset)],
                                            co_ptr =
                                                 &curr_own[ORIGIN(st_offset)];
                                             i < ORIGIN(st_offset) +
                                                     DB_REF(Size_st);
                                             ++i, ++set_ptr, ++co_ptr) {
 ...
 * -f/usr/users/master/config/nonwin inittab.c */
