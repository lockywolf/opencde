...
static int sortcmp(P1(SET_ENTRY FAR *) Pi(char FAR *) Pi(char FAR *));
 ...
 */
int d_connect(set TASK_PARM DBN_PARM) int set; /* Set constant */
TASK_DECL
DBN_DECL /* Database number */
{
        ...
        /* read owner record */
        if ((stat = dio_read(*co_ptr, (char FAR *FAR *)&orec, PGHOLD)) !=
            S_OKAY) {
        ...
        while (TRUE)
                ...
                case DESCENDING:
                        /* perform a linked insertion sort on set -
                           position the current member to the proper place and
                           then switch to order NEXT */
                        for (mdba = cosp.first; TRUE; mdba = cmmp.next) {
                                /* read member record and get member pointer
                                   from member record */
                                if (((stat =
                                          dio_read(mdba, (char FAR *FAR *)&mrec,
                                                   NOPGHOLD)) != S_OKAY) ||
                                    ((stat = r_gmem(set, mrec,
                                                    (char FAR *)&cmmp)) !=
                                     S_OKAY))
                                        goto quit;

                                /* compare sort fields of current record with
                                 * member record */
                                compare = sortcmp(set_ptr, crec, mrec);
                                if ((ordering == ASCENDING) ? (compare < 0)
                                                            : (compare > 0)) {
                                        /* found insertion position -
                                           make previous member the current
                                           member of set and switch to next
                                           order processing */
                                        *cm_ptr = cmmp.prev;
                                        ordering = NEXT;
                                        break;
                ...
                case FIRST:
                        ...
                        /* read current owner's first member and get first
                           member's member pointer
                        */
                        mdba = cosp.first;
                        if (((stat = dio_read(mdba, (char FAR *FAR *)&mrec,
                                              PGHOLD)) != S_OKAY) ||
                            ((stat = r_gmem(set, mrec, (char FAR *)&cmmp)) !=
                             S_OKAY))
                                goto quit;

                        /* set current member's previous, and current owner's
                           first, to current record */
                ...
                case NEXT:
                        ...
                        /* read current member record and get member pointer
                           from current member
                        */
                        if (((stat = dio_read(mdba, (char FAR *FAR *)&mrec,
                                              PGHOLD)) != S_OKAY) ||
                        ...
                        /* read next member record and member pointer from next
                         * member */
                        ndba = crmp.next;
                        if (((stat = dio_read(ndba, (char FAR *FAR *)&nrec,
                                              PGHOLD)) != S_OKAY) ||
                            ((stat = r_gmem(set, nrec, (char FAR *)&nmmp)) !=
                             S_OKAY))
                                goto quit;
...
inserted:
        ...
        if (set_ptr->st_flags & TIMESTAMPED) {
                file = NUM2INT((FILE_NO)((*co_ptr >> FILESHIFT) & FILEMASK),
                               ft_offset);
 ...
 */
static int sortcmp(set_ptr, mem1,
                   mem2) SET_ENTRY FAR *set_ptr; /* set table entry */
 ...
 * -f/usr/users/master/config/nonwin connect.c */
