 ...
 */
int r_chkfld(field, fld_ptr, rec,
             data) INT field; /* field_table entry number */
...
CONST char FAR *data; /* pointer to data area containing field contents */
{
        ...
        if (fld_ptr->fd_type == COMKEY) {
                ...
                /* build compound key value. NOTE: cflag MUST be the same here
                   as for the call to key_bldcom in recwrite, which calls this
                   function. */
        ...
        */
        if (fld_ptr->fd_key == UNIQUE) {
                ...
                if ((!(fld_ptr->fd_flags & OPTKEYMASK) ||
                    ...
                    (d_keyfind(fld, data CURRTASK_PARM CURR_DB_PARM) ==
                     S_OKAY)) {
                        /* another record is already using this key value */
                        db_status = S_DUPLICATE;
                }
                curr_rec = dba;
                if (db_status == S_DUPLICATE)
                        return (db_status);
        }
 ...
 */
int r_delrec(rt, db_addr) INT rt;
DB_ADDR db_addr;
{
        ...
        for (fld = rec_ptr->rt_fields, fld_ptr = &field_table[fld];
             (fld < size_fd) && (fld_ptr->fd_rec == rt); ++fld, ++fld_ptr) {
                if (fld_ptr->fd_key != NOKEY) {
                        if (fld_ptr->fd_type == COMKEY) {
                                key_bldcom(fld, rec + rec_ptr->rt_data, ckey,
                                           TRUE);
                                fptr = ckey;
                        } else {
                                fptr = rec + fld_ptr->fd_ptr;
 ...
 */
int r_gfld(fld_ptr, rec, data) FIELD_ENTRY FAR *fld_ptr;
 ...
 */
int r_gmem(set, rec, mem_addr) int set; /* set table entry number */
char FAR *rec;                          /* pointer to record */
 ...
 */
int r_gset(set, rec, setptr) int set; /* set table entry number */
char FAR *rec;                        /* pointer to record */
 ...
 */
int r_pfld(field, fld_ptr, rec, data,
           db_addr) INT field; /* field_table entry number */
FIELD_ENTRY FAR *fld_ptr;      /* corresponds to field */
...
DB_ADDR FAR *db_addr;
{
        ...
        if ((fld_ptr->fd_key != NOKEY) &&
            (!(fld_ptr->fd_flags & OPTKEYMASK) || r_tstopt(fld_ptr, rec))) {
                ...
                if (key_delete(field, fptr, dba) == S_OKAY) {
                        ...
                        return (db_status == S_NOTFOUND ? dberr(S_KEYERR)
                                                        : db_status);
        }
        ...
        if (struct_key_chk && fld_ptr->fd_flags & STRUCTFLD) {
                for (strfld = field - 1, sfld_ptr = &field_table[strfld];
                     sfld_ptr->fd_type != GROUPED; --strfld, --sfld_ptr)
                        ; /* find struct field */
                if ((sfld_ptr->fd_key != NOKEY) &&
                    /* make sure it is stored */
                    (!(sfld_ptr->fd_flags & OPTKEYMASK) ||
                     r_tstopt(sfld_ptr, rec))) {
                        /* delete the old struct key */
                        if (key_delete(strfld, rec + sfld_ptr->fd_ptr, dba) !=
                            S_OKAY)
                                return (db_status);
                } else
                        strfld = -1;
        ...
        case GROUPED:
                if (!fld_ptr->fd_dim[0]) {
                        ...
                        for (i = field + 1, sfld_ptr = fld_ptr + 1;
                             ...
                             ++i, ++sfld_ptr) {
                                tfptr =
                                    data + sfld_ptr->fd_ptr - fld_ptr->fd_ptr;
                                if (r_pfld(i, sfld_ptr, rec, tfptr, &dba) !=
                                    S_OKAY)
                                        break;
                        }
                        struct_key_chk = 1;
        ...
        if (fld_ptr->fd_flags & SORTFLD) {
                for (s = 0, srt_ptr = sort_table; s < size_srt;
                     ++s, ++srt_ptr) {
                        if (srt_ptr->se_fld == field) {
                                sn = srt_ptr->se_set;
                                if (r_gmem(sn, rec, memp) != S_OKAY)
                                        return (db_status);
                                if (!null_dba(memp + MP_OWNER)) {
                                        ...
                                        /* set current owner and member to
                                         * sorted set */
                                        bytecpy(co_ptr, memp + MP_OWNER,
                                                DB_ADDR_SIZE);
                                        *cm_ptr = dba;
                                        ...
                                        /* disconnect from prior order set and
                                         * reconnect in new order */
                                        d_discon(
                                            set CURRTASK_PARM CURR_DB_PARM);
                                        d_connect(
                                            set CURRTASK_PARM CURR_DB_PARM);
 ...
 */
int r_pmem(set, rec, mem_addr) int set; /* set table entry number */
char FAR *rec;                          /* pointer to record */
char FAR *mem_addr;                     /* pointer to member pointer */
{
        INT rt;
 ...
 */
int r_pset(set, rec, setptr) int set; /* set table entry number */
char FAR *rec;                        /* pointer to record */
char FAR *setptr;                     /* pointer to set pointer */
{
        INT rt;
 ...
 */
int r_smem(db_addr, set) DB_ADDR FAR *db_addr;
INT set;
{
#ifndef NO_TIMESTAMP
        int nset;
...
char FAR *rec;                /* Pointer to record */
{
        ...
        offset = (record_table[fld_ptr->fd_rec].rt_flags & TIMESTAMPED)
                     ? (RECHDRSIZE + 2 * sizeof(LONG))
                     : RECHDRSIZE;

        /* extract the index into the bit map of this key */
        keyndx =
            (((fld_ptr->fd_flags & OPTKEYMASK) >> OPTKEYSHIFT) & OPTKEYNDX) - 1;
        if (keyndx < 0)
 ...
 * -f/usr/users/master/config/nonwin recfcns.c */
