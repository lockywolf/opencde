 ...
 */
int d_mapchar(
    inchar, outchar, sort_str,
    subsort
        TASK_PARM) unsigned char inchar; /* value of character to be mapped */
        ...
        if ((map_fd = open_b(ctb_name, O_RDONLY)) != -1) {
                while (bgets(loc_buf, 20, map_fd), *loc_buf) {
                        if (strcmp(loc_buf, "ignorecase") == 0) {
                                if (d_on_opt(IGNORECASE CURRTASK_PARM) !=
                                    S_OKAY)
                                        break;
                        } else {
                                sscanf(loc_buf, "%c,%c,%hd,%2s", &inchar,
                                       &outchar, &subs_i, &sortas[0]);
                                subsort = (unsigned char)subs_i;
                                if (d_mapchar(inchar, outchar, sortas,
                                              subsort CURRTASK_PARM) != S_OKAY)
                                        break;
                        }
                }
                close(map_fd);
...
static void bgets(s, len, fd) char *s;
int len, fd;
{
        int c;
 ...
 */
int ctbl_alloc() {
        if ((db_global.country_tbl.ptr = (CNTRY_TBL FAR *)ALLOC(
                 &db_global.country_tbl, 256 * sizeof(CNTRY_TBL) + 1,
                 "country_tbl")) == NULL)
 ...
 * -f/usr/users/master/config/nonwin mapchar.c */
