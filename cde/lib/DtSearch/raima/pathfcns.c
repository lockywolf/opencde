...
*/
int con_dbd(path_str, dbname, dbdpath) char FAR
    *path_str;     /* (output) string to receive the full path to the
                      dictionary */
        ...
        */
        ...
        if (isabs(dbname)) {
                ...
                if (dbdpath) {
                        ...
                        if (strlen(path_str) + strlen(remdrv(dbname)) >=
                            FILENMLEN - 4)
                                return (db_status = S_NAMELEN);
...
*/
int con_dbf(
    path_str, dbfile, dbname,
    dbfpath) char FAR *path_str; /* (output) receives full path to dictionary */
...
char FAR *dbfpath; /* one element from DBFPATH, or NULL for no path */
{
        ...
        */
        ...
        if ((dbname = remfile(dbname)) != NULL) {
                ...
                strcat(filespec,
                       remdrv(path_str));   /* Construct dbname + schema */
...
*/
char FAR *get_element(
    dbxpath,
    num) char FAR *dbxpath; /* DBxPATH, elements separated by semicolons
...
int num;                    /* Element to extract (0 = first) */
{
        ...
        */
        ...
        dbxpath = element - 1;
        for (i = 0; i < num; i++)
                if ((dbxpath = strchr(dbxpath + 1, ';')) == NULL)
                        return (NULL);
        dbxpath++;
        if ((save = strchr(dbxpath + 1, ';')) != NULL)
                *save = '\0';
...
*/
static int isabs(path_str) char FAR
    *path_str; /* path to test, NULL causes iabs to return FALSE */
{
        /*
...
*/
static char FAR *
    remdrv(path_str) char FAR *path_str; /* path to remove drive from */
{
        /*
...
*/
static char FAR *
    remfile(path_str) char FAR *path_str; /* Path to remove filename from */
{
        /*
        ...
        */
        ...
        if ((path_str = strrchr(dirpath, DIRCHAR)) == NULL)
                if ((path_str = strrchr(dirpath, ':')) == NULL)
                        return (NULL);
 ...
 */
int d_ctbpath(ctb TASK_PARM) CONST char FAR *ctb;
TASK_DECL {
        int i;
 ...
 * -f/usr/users/master/config/nonwin pathfcns.c */
