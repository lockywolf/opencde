 ...
 */
int d_setkey(field, fldvalue TASK_PARM DBN_PARM) long field;
...
DBN_DECL {
        ...
        while ((sk_ptr = (SK_P FAR *)ll_next(&sk_list)) != NULL) {
                if (sk_ptr->ptr->sk_fld == fld) {
                        ...
                        if (fld_ptr->fd_type != CHARACTER || fld_ptr->fd_dim[1])
                                bytecpy(sk_ptr->ptr->sk_val.ptr, fldvalue,
                                        fld_ptr->fd_len);
                        else if (fld_ptr->fd_dim[0])
                                strncpy(sk_ptr->ptr->sk_val.ptr, fldvalue,
                                        fld_ptr->fd_len);
                        else
        ...
        sk_p.ptr =
            (struct sk FAR *)ALLOC(&sk_p, sizeof(struct sk), "sk_ptr->ptr");
        if (sk_p.ptr == NULL)
 ...
 */
int d_makenew(nrec TASK_PARM DBN_PARM) int nrec;
TASK_DECL
DBN_DECL {
#ifndef NO_TIMESTAMP
        ULONG timestamp;
        ...
        if (nrec_check(nrec, &nrec, (RECORD_ENTRY FAR * FAR *)&rec_ptr) !=
            S_OKAY)
                RETURN(db_status);
             ...
             fld < fldtot; ++fld, ++fld_ptr) {
                if ((fld_ptr->fd_key == UNIQUE) &&
                    !(fld_ptr->fd_flags & OPTKEYMASK)) {
                        ...
                        MEM_LOCK(&sk_ptr->ptr->sk_val);
                        d_keyfind(FLDMARK * (long)recnum +
                                      (long)(fld - rec_ptr->rt_fields),
                    ...
                    !(fld_ptr->fd_flags & OPTKEYMASK)) {
                        ...
                        while (sk_ptr != NULL) {
                                if (sk_ptr->ptr->sk_fld == fld) {
                                        MEM_LOCK(&sk_ptr->ptr->sk_val);
                                        if ((stat = key_insert(
                                                 fld, sk_ptr->ptr->sk_val.ptr,
                                                 db_addr)) != S_OKAY) {
                                                dio_write(db_addr, NULL,
                                                          PGFREE);
                                        ...
                                        if ((fld_ptr->fd_type != CHARACTER) ||
                                            fld_ptr->fd_dim[0])
                                                bytecpy(ptr + fld_ptr->fd_ptr,
                                                        sk_ptr->ptr->sk_val.ptr,
                                                        fld_ptr->fd_len);
                                        else
                                                strncpy(ptr + fld_ptr->fd_ptr,
                                                        sk_ptr->ptr->sk_val.ptr,
                                                        fld_ptr->fd_len);
                                        MEM_UNLOCK(&sk_ptr->ptr->sk_val);
                                        break;
 ...
 * -f/usr/users/master/config/nonwin makenew.c */
