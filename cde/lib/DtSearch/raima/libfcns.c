...
static int rec_okay(P1(int) Pi(int *) Pi(RECORD_ENTRY FAR *FAR *));
static int ctblcmp(P1(CONST unsigned char FAR *) Pi(CONST unsigned char FAR *)
                       Pi(int));

#ifndef ONE_DB
/* Check for valid db number and set curr_db, curr_db_table and curr_rn_table
 */
int dbn_check(dbn) int dbn;
{
        if (!dbopen)
...
*/
int nset_check(nset, set, set_ptr) register int nset;
int *set;
...
*/
int nfld_check(nfld, rec, fld, rec_ptr, fld_ptr) long nfld;
int *rec;
int *fld;
...
FIELD_ENTRY FAR *FAR *fld_ptr;
{
        ...
        if (!rec_okay(trec = (int)(nfld / FLDMARK), rec,
                      (RECORD_ENTRY FAR * FAR *)rec_ptr) ||
...
/* Check for valid (external) record number and return (internal) record
   number and pointer.
*/
int nrec_check(nrec, rec, rec_ptr) int nrec;
int *rec;
RECORD_ENTRY FAR *FAR *rec_ptr;
{
 ...
 */
static int rec_okay(nrec, rec, rec_ptr) register int nrec;
int *rec;
RECORD_ENTRY FAR *FAR *rec_ptr;
{
 ...
 */
int fldcmp(fld_ptr, f1, f2) FIELD_ENTRY FAR *fld_ptr;
...
*/
{
        ...
        entries = 1;
        for (i = 0, dim_ptr = fld_ptr->fd_dim; (i < MAXDIMS) && *dim_ptr;
             ++i, ++dim_ptr)
                entries *= *dim_ptr;
        ...
        case REGINT:
                for (result = elt = 0; result == 0 && elt < entries; ++elt) {
                        if (fld_ptr->fd_flags & UNSIGNEDFLD) {
                                bytecpy(&ui1, f1 + (elt * sizeof(int)),
                                        sizeof(int));
                                bytecpy(&ui2, f2 + (elt * sizeof(int)),
                                        sizeof(int));
                                if (ui1 < ui2)
                                        result = -1;
                                else if (ui1 > ui2)
                                ...
                                bytecpy(&i1, f1 + (elt * sizeof(int)),
                                        sizeof(int));
                                bytecpy(&i2, f2 + (elt * sizeof(int)),
                                        sizeof(int));
                                if (i1 < i2)
                                        result = -1;
                                else if (i1 > i2)
        ...
        case LONGINT:
                for (result = elt = 0; result == 0 && elt < entries; ++elt) {
                        if (fld_ptr->fd_flags & UNSIGNEDFLD) {
                                bytecpy(&ul1, f1 + (elt * sizeof(long)),
                                        sizeof(long));
                                bytecpy(&ul2, f2 + (elt * sizeof(long)),
                                        sizeof(long));
                                if (ul1 < ul2)
                                        result = -1;
                                else if (ul1 > ul2)
                                ...
                                bytecpy(&l1, f1 + (elt * sizeof(long)),
                                        sizeof(long));
                                bytecpy(&l2, f2 + (elt * sizeof(long)),
                                        sizeof(long));
                                if (l1 < l2)
                                        result = -1;
                                else if (l1 > l2)
        ...
        case SHORTINT:
                for (result = elt = 0; result == 0 && elt < entries; ++elt) {
                        if (fld_ptr->fd_flags & UNSIGNEDFLD) {
                                bytecpy(&us1, f1 + (elt * sizeof(short)),
                                        sizeof(short));
                                bytecpy(&us2, f2 + (elt * sizeof(short)),
                                        sizeof(short));
                                if (us1 < us2)
                                        result = -1;
                                else if (us1 > us2)
                                ...
                                bytecpy(&s1, f1 + (elt * sizeof(short)),
                                        sizeof(short));
                                bytecpy(&s2, f2 + (elt * sizeof(short)),
                                        sizeof(short));
                                if (s1 < s2)
                                        result = -1;
                                else if (s1 > s2)
        ...
        case DOUBLE:
                for (result = elt = 0; result == 0 && elt < entries; ++elt) {
                        bytecpy(&d1, f1 + (elt * sizeof(double)),
                                sizeof(double));
                        bytecpy(&d2, f2 + (elt * sizeof(double)),
                                sizeof(double));
                        if (d1 < d2)
                                result = -1;
                        else if (d1 > d2)
        ...
        case DBADDR:
                for (result = elt = 0; result == 0 && elt < entries; ++elt) {
                        result = ADDRcmp(
                            (DB_ADDR FAR *)(f1 + (elt * sizeof(DB_ADDR))),
        ...
        case GROUPED:
                ...
                for (i = 0, cur_len = 0; i < entries; ++i, cur_len += len) {
                        for (sfld_ptr = fld_ptr + 1;
                             (sfld_ptr < fld_max) &&
                             (sfld_ptr->fd_flags & STRUCTFLD);
                             ++sfld_ptr) {
                                sub_len = cur_len + sfld_ptr->fd_ptr -
                                          fld_ptr->fd_ptr;
                                if ((k = fldcmp(sfld_ptr, f1 + sub_len,
                                                f2 + sub_len)))
                                        return (k);
                        }
                }
        ...
        case COMKEY:
                for (kt_lc = size_kt - fld_ptr->fd_ptr,
                    key_ptr = &key_table[fld_ptr->fd_ptr];
                     (--kt_lc >= 0) &&
                     (&field_table[key_ptr->kt_key] == fld_ptr);
                     ++key_ptr) {
                        i = key_ptr->kt_ptr;
                        if ((k = fldcmp(&field_table[key_ptr->kt_field], f1 + i,
                                        f2 + i)))
                                return (k);
                }
                return (0);
 ...
 */
int INTcmp(i1, i2) CONST char FAR *i1, FAR *i2;
{
        INT I1, I2;
 ...
 */
int ADDRcmp(d1, d2) CONST DB_ADDR FAR *d1, FAR *d2;
{
        DB_ADDR a1, a2;
        FILE_NO f1, f2;
        F_ADDR r1, r2;
 ...
 */
int null_dba(db_addr) CONST char FAR *db_addr;
{
        DB_ADDR dba;
 ...
 */
static int ctblcmp(s, t, n) CONST unsigned char FAR *s; /* String 1 */
        ...
        for (; (n && *s && *t); n--) {
                ...
                if (x1 && x2) { /* We have an entry for char. of both strings */
                        ...
                        if (x1 || x2) { /* Only sort_as2 value for one string */
                                if (x1) {
                                        *t++; /* Compare with next character in
                                                 string 2 */
                                        if (db_global.country_tbl.ptr[*t]
                                                .sort_as1)
                                                f2 = db_global.country_tbl
                                                         .ptr[*t]
                                                         .sort_as1;
                                        else
                                                f2 = *t;
                                ...
                                if (x2) {
                                        *s++; /* Compare with next character in
                                                 string 1 */
                                        if (db_global.country_tbl.ptr[*s]
                                                .sort_as1)
                                                f1 = db_global.country_tbl
                                                         .ptr[*s]
                                                         .sort_as1;
                                        else
                                                f1 = *s;
 ...
 * -f/usr/users/master/config/nonwin libfcns.c */
