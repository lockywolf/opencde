 ...
 */
int d_delete(TASK_DBN_ONLY) TASK_DECL DBN_DECL {
        ...
        */
        for (st_lc = size_st, set_ptr = set_table; --st_lc >= 0; ++set_ptr) {
                if ((set_ptr->st_own_rt == rt) &&
                    (bytecmp(ptr + set_ptr->st_own_ptr + SP_FIRST, &zap_dba,
                             DB_ADDR_SIZE) != 0))
        ...
        for (mt_lc = size_mt, mem_ptr = member_table; --mt_lc >= 0; ++mem_ptr) {
                if ((mem_ptr->mt_record == rt) &&
                    (bytecmp(ptr + mem_ptr->mt_mem_ptr + MP_OWNER, &zap_dba,
                             DB_ADDR_SIZE) != 0))
        ...
        /* nullify any currency containing deleted record */
                ...
                for (i = 0, co_ptr = curr_own, cm_ptr = curr_mem,
                    cots_ptr = co_time, cmts_ptr = cm_time, sts_ptr = cs_time;
                     i < size_st; ++i, ++co_ptr, ++cm_ptr, ++cots_ptr,
                    ++cmts_ptr, ++sts_ptr) {
#else
                for (i = 0, co_ptr = curr_own, cm_ptr = curr_mem; i < size_st;
                     ++i, ++co_ptr, ++cm_ptr) {
#endif
 ...
 * -f/usr/users/master/config/nonwin delete.c */
