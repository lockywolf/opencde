 ...
 */
char FAR *d_calloc(i, s) unsigned i, s;
{ return (I_CALLOC(cp, i, s)); }

/* Allocate b bytes of memory
 */
char FAR *d_alloc(b) unsigned b;
{ return (I_MALLOC(cp, b)); }

/* Free memory
 */
void d_free(p) CHAR_P FAR *p;
{
 ...
 */
BOOLEAN ll_access(ll) llist FAR *ll;
{
        ll->curr = NULL;
 ...
 */
int ll_append(ll, data) llist FAR *ll;
CHAR_P FAR *data;
{
        LL_P item;
        LL_P hold;
 ...
 */
CHAR_P FAR *ll_first(ll) llist FAR *ll;
{
        if (ll->head.ptr == NULL) {
                return (NULL);
        }
 ...
 */
CHAR_P FAR *ll_next(ll) llist FAR *ll;
{
        LL_P FAR *next;
 ...
 */
int ll_prepend(ll, data) llist FAR *ll;
CHAR_P FAR *data;
{
        LL_P item;
 ...
 */
static int NewInit(new) LL_P FAR *new;
{
 ...
 * -f/usr/users/master/config/nonwin alloc.c */
