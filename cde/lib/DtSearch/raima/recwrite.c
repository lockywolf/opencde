 ...
 */
int d_recwrite(rec TASK_PARM DBN_PARM) CONST
...
DBN_DECL {
        ...
        for (fld = rec_ptr->rt_fields, fld_ptr = &field_table[fld];
             (fld < size_fd) && (fld_ptr->fd_rec == rt); ++fld, ++fld_ptr) {
                ...
                /* Build compound key for new data supplied by user.  Note:
                   cflag must be the same here as in the 1st key_bldcom for
                   r_chkfld */
                if (fld_ptr->fd_type == COMKEY) {
                        key_bldcom(fld, (char FAR *)rec, nkey, FALSE);
                        fptr = nkey;
                } else
                        fptr = (char FAR *)rec + fld_ptr->fd_ptr -
                               rec_ptr->rt_data;

                if (!(fld_ptr->fd_flags & STRUCTFLD)) {
                        if ((stat = r_chkfld(fld, fld_ptr, crloc, fptr)) !=
                            S_OKAY) {
                ...
                if (fld_ptr->fd_type == COMKEY) {
                        ...
                        if ((fldcmp(fld_ptr, ckey, nkey) != 0) &&
                            ...
                            (!(fld_ptr->fd_flags & OPTKEYMASK) ||
                             r_tstopt(fld_ptr, crloc))) {
                                /* delete the old key */
                                if (key_delete(fld, ckey, curr_rec) == S_OKAY) {
                                        /* insert the new one */
                                        if (key_insert(fld, nkey, curr_rec) !=
                                            S_OKAY)
                                                RETURN(db_status);
                                } else
 ...
 * -f/usr/users/master/config/nonwin recwrite.c */
