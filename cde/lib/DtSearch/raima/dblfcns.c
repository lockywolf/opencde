...
----- --------- --- -----------------------------------------------------
  ...
  571 27-Jan-89 RSC Remove defn of taf_close - General lockmgr lattice port
issue 420 14-Feb-89 WLW Corrected KEYMARK handling in d_lock and lock_files 637
08-Mar-89 RSC Should not alloc file_refs in exclusive access, wasn't Freeing
fl_list of key_locks 713 08-May-89 WLW Make external recovery work for
single-user and one-user 656 08-May-89 WLW Eliminate assignment to unallocated
memory with gen lm.
...
#ifdef MULTI_TASK
DB_TASK Currtask = {POINTER_ASSIGN((TASK FAR *)&db_global),
                    POINTER_ASSIGN((char FAR *)NULL)};
#endif
 ...
 */
d_retries(num TASK_PARM) int num;
TASK_DECL {
 ...
 */
d_timeout(secs TASK_PARM) int secs;
TASK_DECL {
#ifdef SINGLE_USER
        return (db_status = S_OKAY);
#else
 ...
 */
int d_open(dbnames, opentype TASK_PARM) CONST char FAR *dbnames;
...
TASK_DECL {
        ...
        if (db_lockmgr) { /* [637] Only alloc file_refs for shared open */
                ...
                if (ALLOC_TABLE(&db_global.File_refs, size_ft * sizeof(FILE_NO),
                                old_size_ft * sizeof(FILE_NO),
                                "file_refs") != S_OKAY) {
                        RETURN(db_status);
                }
        }
        ...
        if (debugging_dopen) {
                printf(__FILE__
                       "392 d_open before key_open. pgsz=%hd lrgst=%hd\n",
        ...
        if (debugging_dopen) {
                printf(__FILE__
                       "404 d_open after dio_init. pgsz=%hd lrgst=%hd\n",
                       page_size, largest_page);
                fflush(stdout);
        }
#endif
 ...
 */
int initdbt(dbnames) CONST char FAR *dbnames;
{
        ...
        if (dbdpath[0]) {
                ...
                if (strrchr(dbdpath, ';') !=
                    NULL) /* Is dbdpath single element */
                        if (get_element(dbdpath, no_of_dbs) !=
                            NULL) /* Too many? */
                                return (dberr(S_BADBDPATH));
        }
        if (dbfpath[0]) {
                ...
                if (strrchr(dbfpath, ';') !=
                    NULL) { /* Is dbfpath single element */
                        if (get_element(dbfpath, no_of_dbs) !=
                            NULL) /* Too many? */
                                return (dberr(S_BADBFPATH));
                }
        }
        ...
        if ((ALLOC_TABLE(&db_global.Db_table, no_of_dbs * sizeof(DB_ENTRY),
                         old_no_of_dbs * sizeof(DB_ENTRY),
                         "db_table") != S_OKAY) ||
            (ALLOC_TABLE(&db_global.Rn_table, no_of_dbs * sizeof(RN_ENTRY),
                         old_no_of_dbs * sizeof(RN_ENTRY),
                         "rn_table") != S_OKAY)) {
                return (db_status);
        }
...
static int initses() {
        ...
        if ((net_status = nw_addnm(dbuserid, (int *)NULL)) != N_OKAY)
                if (net_status == N_DUPNAME) {
                        /* It is okay to reuse this name, but first be sure that
                           all sessions are hung up.
                        */
                        nw_cleanup(dbuserid);
                } else
                        return (neterr());
...
#ifdef GENERAL
             --ft_lc >= 0;
             fi_ptr += strlen(file_ptr->ft_name) + 1, ++file_ptr) {
#else
...
send_open:
        ...
        if (db_lockmgr) {
                for (ft_lc = size_ft - old_size_ft,
                    fref_ptr = &file_refs[old_size_ft],
...
#define file_used File_used.ptr
        ...
        File_used.ptr = NULL;
        /* Macro references must be on one line for some compilers */
        if ((ALLOC_TABLE(&db_global.App_locks, new_size, old_size,
                         "app_locks") != S_OKAY) ||
            (ALLOC_TABLE(&db_global.Excl_locks, new_size, old_size,
                         "excl_locks") != S_OKAY) ||
            (ALLOC_TABLE(&db_global.Kept_locks, new_size, old_size,
                         "kept_locks") != S_OKAY) ||
        ...
        if ((ALLOC_TABLE(&db_global.Rec_locks, new_size, old_size,
                         "rec_locks") != S_OKAY)) {
                return (db_status);
        }
        ...
        if (size_st) {
                ...
                /* Macro references must be on one line for some compilers */
                if (ALLOC_TABLE(&db_global.Set_locks, new_size, old_size,
                                "set_locks") != S_OKAY) {
                        return (db_status);
                }
        }
        ...
        for (rec = old_size_rt, rec_ptr = &record_table[old_size_rt],
            ld_ptr = rec_locks;
             rec < size_rt; ++rec, ++rec_ptr, ++ld_ptr) {
                ld_ptr->fl_type = 'f';
                ...
                for (fd_lc = size_fd - rec_ptr->rt_fields,
                    fld_ptr = &field_table[rec_ptr->rt_fields];
                     (--fd_lc >= 0) && (fld_ptr->fd_rec == rec); ++fld_ptr) {
                ...
                ld_ptr->fl_list.ptr =
                    /* Macro references must be on one line for some compilers
                     */
                    (FILE_NO FAR *)ALLOC(&ld_ptr->fl_list,
                                         fl_cnt * sizeof(FILE_NO), db_avname);
                if (ld_ptr->fl_list.ptr == NULL)
                        return (dberr(S_NOMEMORY));
        ...
        if (size_st) {
                for (st_lc = size_st - old_size_st,
                    set_ptr = &set_table[old_size_st], ld_ptr = set_locks;
                     --st_lc >= 0; ++set_ptr, ++ld_ptr) {
                        /* add owner's data file */
                        file_used[record_table[set_ptr->st_own_rt].rt_file] =
                            TRUE;
                        ld_ptr->fl_type = 'f';
                        ld_ptr->fl_prev = 'f'; /*[367] init to free */
                        ld_ptr->fl_kept = FALSE;
                        ...
                        fl_cnt = 1; /* count of used files */
                        for (mem = set_ptr->st_members,
                            memtot = mem + set_ptr->st_memtot,
                            mem_ptr = &member_table[mem];
                             mem < memtot; ++mem, ++mem_ptr) {
                                fu_ptr =
                                    &file_used[record_table[mem_ptr->mt_record]
                                                   .rt_file];
                                if (!*fu_ptr) {
                                        *fu_ptr = TRUE;
                                        ++fl_cnt;
                        ...
                        ld_ptr->fl_cnt = fl_cnt;
                        ld_ptr->fl_list.ptr =
                            /* Macro references must be on one line for some
                               compilers */
                            (FILE_NO FAR *)ALLOC(&ld_ptr->fl_list,
                                                 fl_cnt * sizeof(FILE_NO),
                                                 db_avname);
                        if (ld_ptr->fl_list.ptr == NULL)
                                return (dberr(S_NOMEMORY));
                        fl_ptr = ld_ptr->fl_list.ptr;
                        for (i = 0, fu_ptr = file_used; i < size_ft;
                             ++i, ++fu_ptr) {
                                if (*fu_ptr) {
                                        *fu_ptr = FALSE;
                                        *fl_ptr++ = i;
        ...
        if (keyl_cnt) {
                ...
                /* Macro references must be on one line for some compilers */
                if (ALLOC_TABLE(&db_global.Key_locks, new_size, old_size,
                                "key_locks") != S_OKAY) {
                        return (db_status);
                }
                for (fd_lc = size_fd - old_size_fd,
                    fld_ptr = &field_table[old_size_fd], ld_ptr = key_locks;
                     --fd_lc >= 0; ++fld_ptr) {
                        if (fld_ptr->fd_key != NOKEY) {
                                ld_ptr->fl_type = 'f';
                                ld_ptr->fl_prev = 'f'; /*[367] init to free */
                                ld_ptr->fl_kept = FALSE;
                                ld_ptr->fl_cnt = 1;
                                ld_ptr->fl_list.ptr = (FILE_NO FAR *)ALLOC(
                                    &ld_ptr->fl_list,
                                    ld_ptr->fl_cnt * sizeof(FILE_NO),
                                    "fl_list");
                                if (ld_ptr->fl_list.ptr == NULL)
                                        return (dberr(S_NOMEMORY));
        ...
        lock_pkt =
            (LM_LOCK FAR *)ALLOC(&db_global.Lock_pkt, lp_size, "lock_pkt");
        free_pkt =
            (LM_FREE FAR *)ALLOC(&db_global.Free_pkt, fp_size, "free_pkt");
        if (!lock_pkt || !free_pkt)
                return (dberr(S_NOMEMORY));
        lock_pkt->fcn = L_LOCK;
        free_pkt->fcn = L_FREE;
...
termses() {
        ...
        if (session_active) {
                ...
                send_pkt =
                    (LM_DBCLOSE FAR *)ALLOC(&Send_pkt, send_size, "send_pkt");
                if (send_pkt == NULL)
                        return (dberr(S_NOMEMORY));
...
void termfree() {
        ...
        if (key_locks) {
                for (i = 0, ld_ptr = key_locks; i < keyl_cnt;
                     ++i, ++ld_ptr) { /*[637]*/
                        MEM_UNLOCK(&ld_ptr->fl_list);
                        FREE(&ld_ptr->fl_list);
                }
 ...
 */
d_reclock(rec, lock_type TASK_PARM DBN_PARM) int rec;
char FAR *lock_type;
TASK_DECL
DBN_DECL {
#ifdef SINGLE_USER
 ...
 */
d_setlock(set, lock_type TASK_PARM DBN_PARM) int set;
char FAR *lock_type;
TASK_DECL
DBN_DECL {
#ifdef SINGLE_USER
...
/* Lock key file
 */
d_keylock(key, lock_type TASK_PARM DBN_PARM) long key; /* field number of key */
char FAR *lock_type;
TASK_DECL
DBN_DECL {
#ifdef SINGLE_USER
        ...
        if (nfld_check(key, &rec, &fld, (RECORD_ENTRY FAR * FAR *)&rec_ptr,
 ...
 */
d_reclstat(rec, lstat TASK_PARM DBN_PARM) int rec;
char FAR *lstat;
TASK_DECL
DBN_DECL {
#ifdef SINGLE_USER
 ...
 */
d_setlstat(set, lstat TASK_PARM DBN_PARM) int set;
char FAR *lstat;
TASK_DECL
DBN_DECL {
#ifdef SINGLE_USER
 ...
 */
d_keylstat(key, lstat TASK_PARM DBN_PARM) long key;
char FAR *lstat;
TASK_DECL
DBN_DECL {
#ifdef SINGLE_USER
 ...
 */
d_lock(count, lrpkt TASK_PARM DBN_PARM) int count;
...
DBN_DECL {
        ...
        lock_pkt->nfiles = 0;
        for (i = 0, lrpkt_ptr = lrpkt; (db_status == S_OKAY) && (i < count);
             ++i, ++lrpkt_ptr) {
        ...
        if (db_status != S_OKAY) {
                ...
                for (i = 0, lrpkt_ptr = lrpkt; i < count; ++i, ++lrpkt_ptr) {
                         ...
                         */
                        if (lrpkt_ptr->item >= KEYMARK) {
                                item = field_table[lrpkt_ptr->item - KEYMARK]
                                           .fd_keyno;
                                ld_ptr = &key_locks[item];
                        } else if (lrpkt_ptr->item >= SETMARK) {
                                item = NUM2INT(lrpkt_ptr->item - SETMARK,
                                               st_offset);
                                ld_ptr = &set_locks[item];
                        } else if (lrpkt_ptr->item >= RECMARK) {
                                item = NUM2INT(lrpkt_ptr->item - RECMARK,
                                               rt_offset);
 ...
 */
static process_lock(ld_ptr, type) struct lock_descr FAR *ld_ptr;
char type;
{
                ...
                for (fl_lc = ld_ptr->fl_cnt, fl_ptr = FL_LIST_ACCESS(ld_ptr);
                     --fl_lc >= 0; ++fl_ptr) {
                        ...
                        if (i < lock_pkt->nfiles) {
                                ...
                                if (lockreq_ptr->type == 'r' ||
                                    ld_ptr->fl_type == 'x')
                                        lockreq_ptr->type = ld_ptr->fl_type;
                        } else if (!excl_locks[fno] &&
                                   (!app_locks[fno] ||
                                    (ld_ptr->fl_type == 'w' &&
                                     app_locks[fno] > 0) ||
                                    (ld_ptr->fl_type == 'x'))) {
...
/* Lock database files
 */
static lock_files(count, lrpkt) int count;
LOCK_REQUEST FAR *lrpkt;
{
#ifndef SINGLE_USER
        register int fl_lc; /* loop control */
...
skip_send:
        ...
        case L_OKAY:
                ...
                for (l = 0, lrpkt_ptr = lrpkt; l < count; ++l, ++lrpkt_ptr) {
                        ...
                        for (fl_lc = ld_ptr->fl_cnt,
                            fl_ptr = FL_LIST_ACCESS(ld_ptr);
                             --fl_lc >= 0; ++fl_ptr) {
...
static int send_lock() {
        ...
        if (lock_pkt->nfiles) {
                /* send lock request */
                send_size = sizeof(LM_LOCK) +
                            (lock_pkt->nfiles - 1) * sizeof(LM_LOCKREQ);
        ...
        req_locks:
                ...
                if (nw_rcvmsg(lsn, (MESSAGE FAR *)&lock_reply, sizeof(LR_LOCK),
                              &recv_size))
                        return (neterr());
#ifdef MONITOR
                ...
                if (lock_reply.status == L_RECOVER) {
                        ...
                        /* tell lock mgr we're done */
                        trend_pkt.fcn = L_RECDONE;
                        if (nw_send(lsn, (MESSAGE FAR *)&trend_pkt,
                                    sizeof(LM_TREND)))
                                return (neterr());
...
/* Free key lock
 */
d_keyfree(key TASK_PARM DBN_PARM) long key;
TASK_DECL
DBN_DECL {
#ifdef SINGLE_USER
        return (db_status = S_OKAY);
 ...
 */
static keep_locks(ld_ptr) struct lock_descr FAR *ld_ptr; /* Lock descriptor */
{
        register int fl_lc; /* loop control */
...
/* Free record lock
 */
d_recfree(rec TASK_PARM DBN_PARM) int rec;
TASK_DECL
DBN_DECL {
#ifdef SINGLE_USER
        return (db_status = S_OKAY);
...
/* Free set lock
 */
d_setfree(set TASK_PARM DBN_PARM) int set;
TASK_DECL
DBN_DECL {
#ifdef SINGLE_USER
        return (db_status = S_OKAY);
 ...
 */
static int free_files(ld_ptr) struct lock_descr FAR *ld_ptr;
{
        register int fl_lc; /* loop control */
        FILE_NO fno;
             ...
             --fl_lc >= 0; ++fl_ptr) {
                ...
                if (ld_ptr->fl_type == 'r' && *appl_ptr > 0) {
                        ...
                        if (*appl_ptr > 0) {
                                ...
                                lockreq_ptr =
                                    &lock_pkt->locks[lock_pkt->nfiles++];
                                lockreq_ptr->type = 'r';
                                lockreq_ptr->fref = fref;
                        } else {
                                /* free excl-lock */
...
d_freeall(TASK_ONLY) TASK_DECL {
        ...
        free_pkt->nfiles = 0;
        for (i = 0, fref_ptr = file_refs, appl_ptr = app_locks; i < size_ft;
             ++i, ++fref_ptr, ++appl_ptr) {
                if (*appl_ptr) {
                        *appl_ptr = FALSE;
                        if (!excl_locks[i])
...
static int send_free() {
        ...
        if (free_pkt->nfiles) {
                send_size =
                    sizeof(LM_FREE) + (free_pkt->nfiles - 1) * sizeof(INT);
...
#ifndef SINGLE_USER
        file =
            NUM2INT((FILE_NO)((curr_rec >> FILESHIFT) & FILEMASK), ft_offset);
...
d_rlbclr(TASK_ONLY) TASK_DECL {
        ...
        if (dbopen == 1 && trans_id && app_locks[file] >= 0 &&
            !excl_locks[file])
                RETURN(dberr(S_NOTLOCKED));
...
/* Begin transaction
 */
d_trbegin(tid TASK_PARM) CONST char FAR *tid;
TASK_DECL {
        DB_ENTER(NO_DB_ID TASK_ID LOCK_SET(LOCK_IO));
...
d_trend(TASK_ONLY) TASK_DECL {
        ...
        if ((dboptions & TRLOGGING) && use_ovfl) {
                ...
                if (nw_send(lsn, (MESSAGE FAR *)&trcom_pkt,
                            sizeof(LM_TRCOMMIT)))
                        RETURN(neterr());
#endif
                trcommit = TRUE;
...
#ifndef GENERAL
                if (taf_add(dblog) != S_OKAY)
                        RETURN(db_status); /* after nw_send */
#endif
...
#ifndef GENERAL
                if (taf_del(dblog) != S_OKAY)
                        RETURN(db_status); /* before nw_send */
#endif
        }
#ifndef SINGLE_USER
        ...
        if (dbopen == 1) {
                ...
                lock_pkt->nfiles = free_pkt->nfiles = 0;
                for (ft_lc = size_ft, fref_ptr = file_refs,
                    appl_ptr = app_locks, keptl_ptr = kept_locks,
                    excl_ptr = excl_locks;
                     --ft_lc >= 0;
                     ++fref_ptr, ++appl_ptr, ++keptl_ptr, ++excl_ptr) {
                        if (*excl_ptr)
                                *appl_ptr = *keptl_ptr;
                        else if (*appl_ptr == -1) {
                                if ((*appl_ptr = *keptl_ptr) > 0) {
                                        lockreq_ptr =
                                            &lock_pkt
                                                 ->locks[lock_pkt->nfiles++];
                                        lockreq_ptr->type = 'r';
                                        lockreq_ptr->fref = *fref_ptr;
                                } else
                                        free_pkt->frefs[free_pkt->nfiles++] =
                                            *fref_ptr;
...
d_trabort(TASK_ONLY) TASK_DECL {
        ...
        if (dbopen == 1) {
                ...
                for (i = 0, keptl_ptr = kept_locks; i < size_ft;
                     ++i, ++keptl_ptr)
                        *keptl_ptr = 0;
                for (i = 0, ld_ptr = rec_locks; i < size_rt; ++i, ++ld_ptr)
                        ld_ptr->fl_kept = FALSE;
...
int alloc_table(Table, new_size, old_size) CHAR_P FAR *Table;
 ...
 * -f/usr/users/master/config/nonwin dblfcns.c */
