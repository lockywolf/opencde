...
void swab_page(char *pgbuf, FILE_ENTRY *file_ptr, SWABDIR direction) {
        ...
        if (file_ptr->ft_type == DATA) {
                ...
                if (debugging_dbswab) {
                        printf(
                            __FILE__
                            "073 %s DATA page '%s': slct=%d slsz=%d pgsz=%d\n",
                ...
                for (slotno = 0, slotptr = pgbuf + 4; /* over timestamp */
                     slotno < slots; slotno++, slotptr += slsize) {
                        ...
                        memcpy(&align_LONG, slotptr + sizeof(INT),
                               sizeof(LONG));
                        align_LONG = HTONL(align_LONG);
                        memcpy(slotptr + sizeof(INT), &align_LONG,
                               sizeof(LONG));
                        ...
                        for (curr_offset = (recno == MISCREC_RECNO) ? 7 : 6;
                             ...
                             curr_offset += sizeof(LONG)) {
                                memcpy(&align_LONG, slotptr + curr_offset,
                                       sizeof(LONG));
                                align_LONG = HTONL(align_LONG);
                                memcpy(slotptr + curr_offset, &align_LONG,
                                       sizeof(LONG));
                        }
         ...
         */
                ...
                if (debugging_dbswab || debugging_key_swabs) {
                        printf(__FILE__ "124 %s KEY page '%s': slct=%d used=%d "
                                        "slsz=%d pgsz=%d\n",
                               (direction == HTON) ? "WRITE" : "READ",
                               file_ptr->ft_name, (int)slots, (int)used_count,
                 ...
                 * Slots start at pg offset 10: timestamp + used_slots + orphan
                 * ptr. Note that if used_count == 0, we won't process this
                 * deleted node any further.
                 */
                for (slotno = 0, slotptr = pgbuf + 10; slotno < used_count;
                     slotno++, slotptr += slsize) {
                        ...
                        if (debugging_key_swabs)
                                printf("  slot #%02d: prefix=%d", (int)slotno,
                                       (int)key_prefix);
#endif
                        /* database address.
