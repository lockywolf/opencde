...
static int node_search(P1(CONST char FAR *) Pi(DB_ADDR FAR *) Pi(NODE FAR *)
                           Pi(int *) Pi(int *) Pi(F_ADDR *));
static int keycmp(P1(CONST char FAR *) Pi(KEY_SLOT FAR *) Pi(DB_ADDR FAR *));
...
int key_open() {
        ...
        if (no_of_keys) {
                key_info =
                     ...
                     */
                    (KEY_INFO FAR *)ALLOC(&db_global.Key_info,
                                          no_of_keys * sizeof(KEY_INFO),
                                          "key_info");
                if (!key_info)
                        return (dberr(S_NOMEMORY));
                for (i = 0, fld_ptr = &field_table[old_size_fd]; i < size_fd;
                     ++i, ++fld_ptr) {
                        if (fld_ptr->fd_key != NOKEY) {
                                ...
                                ki_ptr->keyval =
                                    /* Macro references must be on one line for
                                       some compilers */
                                    ALLOC(&ki_ptr->Keyval, file_ptr->ft_slsize,
                                          db_avname);
                                if (!ki_ptr->keyval)
                                        return (dberr(S_NOMEMORY));
                                MEM_UNLOCK(&ki_ptr->Keyval);
                                /* compute maximum possible levels */
                                for (t = file_ptr->ft_slots, l = 1;
                                     t < MAXRECORDS; ++l)
                                        t *= file_ptr->ft_slots;
                                ki_ptr->max_lvls = ++l;
                                ki_ptr->node_path = (NODE_PATH FAR *)ALLOC(
                                    &ki_ptr->Node_path, l * sizeof(NODE_PATH),
                                    db_avname);
                                if (!ki_ptr->node_path)
                                        return (dberr(S_NOMEMORY));
                                byteset(ki_ptr->node_path, 0,
                                        l * sizeof(NODE_PATH));
 ...
 */
int key_init(field) int field; /* field number to be processed */
{
...
int key_reset(fno) FILE_NO fno;
{
        ...
        for (i = 0, ki_ptr = key_info; i < no_of_keys; ++i, ++ki_ptr) {
                if (((fno == size_ft) || (ki_ptr->keyfile == fno)) &&
                    ((ki_ptr->lstat == KEYFOUND) ||
                     (ki_ptr->lstat == KEYNOTFOUND)))
 ...
 */
int key_locpos(key_val, dba) CONST char FAR *key_val; /* key search value */
DB_ADDR FAR *dba; /* database address of located key */
{
        ...
        for (curkey->level = 0, np_ptr = curkey->node_path, pg = ROOT_ADDR;
             TRUE; ++curkey->level, ++np_ptr, pg = child) {
                /* read in next node */
                ...
                if (curkey->level == 0 && node->used_slots == 0) {
                        ...
                        return (dberr(
                            S_SYSERR)); /* non-root nodes can't be empty */

                /* search node for key */
                stat = node_search(key_val, ((*dba == NULL_DBA) ? NULL : dba),
                                   node, &slot, &slot_pos, &child);
                np_ptr->slot = slot;
 ...
 */
static int node_search(key_val, dba, node, slotno, slot_offset, child) CONST
    char FAR *key_val; /* key being searched */
...
F_ADDR *child;         /* child ptr of located key */
{
        ...
        while (u >= l) {
                ...
                if ((cmp = keycmp(key_val, (KEY_SLOT FAR *)node_slot_ptr,
                                  dba)) < 0)
                        u = i - 1;
                else if (cmp > 0)
                        l = i + 1;
                        ...
                        while (
                            keycmp(key_val,
                                   (KEY_SLOT FAR *)(node_slot_ptr -= slot_len),
                                   dba) == 0) {
                                slot_pos -= slot_len;
                                if (--i == 0)
                                        goto have_slot;
...
/* Compare key value
 */
static int keycmp(key_val, slot, dba) CONST char FAR *key_val; /* key value */
...
DB_ADDR FAR *dba;   /* database address included in comparison if not null */
{
        ...
        */
        ...
        if (((cmp = INTcmp((char FAR *)&prefix, (char FAR *)&slot->keyno)) ==
             0) &&
            ((cmp = fldcmp(cfld_ptr, key_val, slot->data)) == 0) && dba)
...
/* Scan thru key field
 */
int key_scan(fcn, dba) int fcn; /* next or prev */
DB_ADDR *dba;                   /* db address of scanned record */
{
        ...
        case KEYEOF:
                return (
                    key_boundary(((fcn == KEYNEXT) ? KEYFRST : KEYLAST), dba));
        case KEYREPOS:
                MEM_LOCK(&curkey->Keyval);
        ...
        if (child == NULL_NODE) {
                ...
                while (
                    ((fcn == KEYNEXT) && (np_ptr->slot >= node->used_slots)) ||
                ...
                do { /* move down to leaf node */
                        if (dio_get(child, (char FAR *FAR *)&node, NOPGHOLD) !=
                            S_OKAY) {
                                MEM_UNLOCK(&curkey->Node_path);
                                return (db_status);
                        }
                        ...
                        if (fcn == KEYNEXT) {
                                ...
                                node_slot_ptr =
                                    &node->slots[np_ptr->slot * slot_len];
                        }
                        bytecpy(&child, node_slot_ptr, sizeof(F_ADDR));
 ...
 */
static void key_found(dba) DB_ADDR *dba;
{
        MEM_LOCK(&curkey->Keyval);
        /* save key value and database address for possible repositioning */
        bytecpy(curkey->keyval, key_type.ks.data, key_len);
...
/* Find key boundary
 */
int key_boundary(fcn, dba) int fcn; /* KEYFRST or KEYLAST */
                ...
                if (fcn == KEYFRST) {
                        ...
                        for (slot = node->used_slots - 1,
                            node_slot_ptr = &node->slots[slot * slot_len];
                             slot >= 0; --slot, node_slot_ptr -= slot_len) {
                                if ((cmp = INTcmp(
                                         (char FAR *)&prefix,
                                         (char FAR *)(node_slot_ptr +
                                                      sizeof(F_ADDR)))) >= 0)
 ...
 */
int key_insert(fld, key_val, dba) int fld; /* key field number */
...
DB_ADDR dba;                               /* record's database address */
{
        ...
        if (key_locpos(key_val, &dba) == S_NOTFOUND) {
                ...
                if ((stat = expand(key_val, dba, NULL_NODE)) == S_OKAY) {
                        /* save key value and database address for possible
                         * repositioning */
                        MEM_LOCK(&curkey->Keyval);
 ...
 */
static int expand(key_val, dba, brother) CONST
    char FAR *key_val; /* key value */
DB_ADDR dba;           /* record's database address */
F_ADDR brother;        /* page number of brother node */
{
        ...
        if (dio_get(pg = np_ptr->node, (char FAR *FAR *)&node, PGHOLD) !=
            S_OKAY) {
                MEM_UNLOCK(&curkey->Node_path);
                return (db_status);
        }
 ...
 */
static int split_node(l_pg, l_node) F_ADDR l_pg; /* left node's page number */
...
/* Split root node
 */
static int split_root(node) NODE FAR *node;
{
        F_ADDR l_pg, r_pg;
 ...
 */
int key_delete(fld, key_val, dba) int fld;
...
static int delete () {
        ...
        if (r_pg != NULL_NODE) {
                ...
                ++np_ptr->slot;
                do {
                        if (dio_get(r_pg, (char FAR *FAR *)&r_node, NOPGHOLD) !=
                            S_OKAY) {
                                MEM_UNLOCK(&curkey->Node_path);
                                return (db_status);
                        }
                ...
                bytecpy(node_slot_ptr, r_node_slot_ptr,
                        slot_len - sizeof(F_ADDR));
                dio_touch(pg);
                ...
                slot_pos = 0;
                node_slot_ptr = node->slots;
                if (dio_get(pg = np_ptr->node, (char FAR *FAR *)&node,
                            PGHOLD) != S_OKAY) {
                        MEM_UNLOCK(&curkey->Node_path);
                        return (db_status);
                }
...
shrink: /* delete key from leaf (shrink node ) */
        ...
        if ((curkey->level > 0) && (node->used_slots < mid_slot)) {
                ...
                if ((np_ptr - 1)->slot == p_node->used_slots) {
                        ...
                        if (dio_get(l_pg, (char FAR *FAR *)&l_node, PGHOLD) !=
                            S_OKAY) {
                                MEM_UNLOCK(&curkey->Node_path);
                                return (db_status);
                        }
                        ...
                        if (dio_get(r_pg, (char FAR *FAR *)&r_node, PGHOLD) !=
                            S_OKAY) {
                                MEM_UNLOCK(&curkey->Node_path);
                                return (db_status);
                        }
                ...
                if ((l_node->used_slots + r_node->used_slots + 1) < max_slots) {
                        ...
                        if ((curkey->level == 1) && (p_node->used_slots == 1)) {
                                ...
                                bytecpy(p_node_slot_ptr, r_node->slots,
                                        NODE_WIDTH(r_node));
                                ...
                                bytecpy(p_node->slots, l_node->slots,
                                        NODE_WIDTH(l_node));
                                l_node->used_slots = 0;
                                dio_touch(l_pg);
                        ...
                        /* open space for l_node->used_slots+1 slots in right
                         * node */
                        ...
                        r_node_slot_ptr += amt;
                        p_node_slot_ptr =
                            &p_node->slots[slot_pos + sizeof(F_ADDR)];
                        bytecpy(r_node_slot_ptr, p_node_slot_ptr,
                                slot_len - sizeof(F_ADDR));
                ...
                if ((l_node->used_slots + 1) < r_node->used_slots) {
                        ...
                        p_node_slot_ptr =
                            &p_node->slots[slot_pos + sizeof(F_ADDR)];
                        bytecpy(l_node_slot_ptr, p_node_slot_ptr,
                                slot_len - sizeof(F_ADDR));
                        ...
                        r_node_slot_ptr = r_node->slots;
                        bytecpy(l_node_slot_ptr, r_node_slot_ptr,
                                sizeof(F_ADDR));

                        /* move slot 0 of right node to parent */
                        r_node_slot_ptr += sizeof(F_ADDR);
                        bytecpy(p_node_slot_ptr, r_node_slot_ptr,
                                slot_len - sizeof(F_ADDR));
                        ...
                        l_node_slot_ptr =
                            &l_node->slots[NODE_WIDTH(l_node) - slot_len];
                        bytecpy(p_node_slot_ptr, l_node_slot_ptr,
                                slot_len - sizeof(F_ADDR));
 ...
 */
static void open_slots(node, slot_pos, n) NODE FAR *node;
int slot_pos;
int n;
{
 ...
 */
static void close_slots(node, slot_pos, n) NODE FAR *node;
int slot_pos;
int n;
{
        register char FAR *dst, FAR *src;
 ...
 */
int d_keyread(key_val TASK_PARM) char FAR *key_val;
TASK_DECL {
        ...
        if ((curkey->lstat != KEYFOUND) && (curkey->lstat != KEYNOTFOUND) &&
                     ...
                     (--kt_lc >= 0) && (key_ptr->kt_key == fldno); ++key_ptr) {
                        ...
                        if (key_ptr->kt_sort == 'd') {
                                ...
                                case CHARACTER:
                                        ...
                                        if (fld_ptr->fd_dim[0] > 1 &&
                                            fld_ptr->fd_dim[1] == 0) {
                                                /* make sure a null byte is at
                                                 * the end */
                                                kptr[fld_ptr->fd_len - 1] =
                                                    '\0';
                                        }
                                        break;
                                default:
 ...
 */
int key_bldcom(fld, rec, key, cflag) int fld; /* compound key field number */
             ...
             (--kt_lc >= 0) && (key_ptr->kt_key == fld); ++key_ptr) {
                ...
                if (cflag && (key_ptr->kt_sort == 'd')) {
                        ...
                        case FLOAT:
                                bytecpy(&fv, fptr, sizeof(float));
                                fv = (float)0.0 - fv;
                                bytecpy(&key[key_ptr->kt_ptr], &fv,
                                        sizeof(float));
                                break;
                        case DOUBLE:
                                bytecpy(&dv, fptr, sizeof(double));
                                dv = 0.0 - dv;
                                bytecpy(&key[key_ptr->kt_ptr], &dv,
                                        sizeof(double));
                                break;
#endif
                        case CHARACTER:
                                key_cmpcpy(key + key_ptr->kt_ptr, fptr,
                                           kfld_ptr->fd_len);
                                if (kfld_ptr->fd_dim[0] > 1 &&
                                    kfld_ptr->fd_dim[1] == 0) {
 ...
 */
void key_cmpcpy(s1, s2, n) register char FAR *s1;
 ...
 * -f/usr/users/master/config/nonwin keyfcns.c */
