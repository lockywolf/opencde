 ...
 */
int d_setoo(
    nsett, nsets TASK_PARM
               DBN_PARM) int nsett; /* set table entry number of target owner */
...
DBN_DECL /* database number */
{
        ...
        if ((nset_check(nsett, &sett, (SET_ENTRY FAR * FAR *)&sett_ptr) !=
             S_OKAY) ||
            (nset_check(nsets, &sets, (SET_ENTRY FAR * FAR *)&sets_ptr) !=
             S_OKAY))
                RETURN(db_status);
 ...
 * -f/usr/users/master/config/nonwin setoo.c */
