...
static void chk_desc_key(P1(int) Pi(FIELD_ENTRY FAR *) Pi(CONST char FAR *)
                             Pi(char FAR *));

/* Find record thru key field
 */
int d_keyfind(field, fldval TASK_PARM DBN_PARM) long field; /* field constant */
 ...
 */
static void chk_desc_key(fld, fld_ptr, fldval, ckey) int fld;
FIELD_ENTRY FAR *fld_ptr;
 ...
 * -f/usr/users/master/config/nonwin keyfind.c */
