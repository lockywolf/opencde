...
static void cache_init(P1(int) Pi(LOOKUP_ENTRY FAR *) Pi(PAGE_ENTRY FAR *)
                           Pi(int));
...
#else
static int dio_in(P1(PAGE_ENTRY FAR *) Pi(LOOKUP_ENTRY FAR *) Pi(BOOLEAN));
#endif
 ...
 */
int d_setpages(dbpgs, ixpgs) int dbpgs; /* # of db cache pages */
...
#define tempbuff Tempbuff.ptr
        ...
        if (dbpg_table) {
                ...
                if (page_size > largest_page) {
                        if ((tempbuff = ALLOC(&Tempbuff, page_size,
                                              "tempbuff")) == NULL)
                ...
                if (debugging_dio_init) {
                        printf(
                            __FILE__
                            "323 dio_init: pzinited ok. pgsz=%d largest=%d\n",
        ...
        used_files =
            ...
            (int FAR *)ALLOC(&Used_files, (size_ft + 1) * sizeof(int),
                             "used_files");
        db_lookup =
            /* Macro references must be on one line for some compilers */
            (LOOKUP_ENTRY FAR *)ALLOC(
                &Db_lookup, db_pgtab_sz * sizeof(LOOKUP_ENTRY), "db_lookup");
        dbpg_table =
            /* Macro references must be on one line for some compilers */
            (PAGE_ENTRY FAR *)ALLOC(
                &Dbpg_table, db_pgtab_sz * sizeof(PAGE_ENTRY), "dbpg_table");
#ifdef DEBUG_DIO
        if (debugging_dio_init) {
                printf(__FILE__ "345 dio_init: usedfls=%p lookup=%p pgtab=%p\n",
                       (void *)used_files, (void *)db_lookup,
                       (void *)dbpg_table);
                fflush(stdout);
        }
#endif
        ...
        if (use_ovfl) {
                ix_lookup =
                     ...
                     */
                    (LOOKUP_ENTRY FAR *)ALLOC(
                        &Ix_lookup, ix_pgtab_sz * sizeof(LOOKUP_ENTRY),
                        "ix_lookup");
                ixpg_table =
                    /* Macro references must be on one line for some compilers
                     */
                    (PAGE_ENTRY FAR *)ALLOC(&Ixpg_table,
                                            ix_pgtab_sz * sizeof(PAGE_ENTRY),
                                            "ixpg_table");
                ...
                if ((dbpgbuff = ALLOC(&Dbpgbuff, page_size, "dbpgbuff")) ==
                    NULL)
                        return (dberr(S_NOMEMORY));
...
static void cache_init(pg_cnt, lu_ptr, pg_ptr, pgsize) int pg_cnt;
...
int pgsize;
{
        ...
        if (debugging_dio_init) {
                printf(__FILE__
                       "400 cache_init: pgcnt=%d lu=%p pgp=%p pgsz=%d\n",
        ...
        for (pg_no = 0; pg_no < pg_cnt; ++pg_no, ++lu_ptr, ++pg_ptr) {
                ...
                if (pg_ptr->buff == NULL) {
                        ...
                        if (debugging_dio_init) {
                                printf(__FILE__
                                       "428 cache_init: alloc failed pgsz=%d\n",
                                       pgsize);
                                fflush(stdout);
                        }
#endif
...
void dio_free() {
        ...
        FREE(&Db_lookup);
        for (pgt_lc = db_pgtab_sz, pg_ptr = dbpg_table; --pgt_lc >= 0;
             ++pg_ptr) {
                MEM_UNLOCK(&pg_ptr->Buff);
                FREE(&pg_ptr->Buff);
        }
        ...
        if (use_ovfl) {
                ...
                FREE(&Ix_lookup);
                for (pgt_lc = ix_pgtab_sz, pg_ptr = ixpg_table; --pgt_lc >= 0;
                     ++pg_ptr) {
                        MEM_UNLOCK(&pg_ptr->Buff);
                        FREE(&pg_ptr->Buff);
                }
 ...
 */
int dio_clrfile(fno) register FILE_NO fno;
 ...
 */
int dio_clear() { return (clear_cache(0, size_ft)); }
 ...
 */
static int clear_cache(fr_file,
                       to_file) FILE_NO fr_file; /* clear from file "fr_file" */
        ...
        */
        for (s_file = e_file = fr_file, file_ptr = &file_table[e_file],
            appl_ptr = &app_locks[e_file], excl_ptr = &excl_locks[e_file];
             s_file < to_file; ++file_ptr, ++appl_ptr, ++excl_ptr) {
#else
        for (s_file = e_file = fr_file, file_ptr = &file_table[e_file];
             s_file < to_file; ++file_ptr) {
#endif
                if ((e_file < to_file) &&
#ifndef SINGLE_USER
                    ...
                    !(file_ptr->ft_flags & STATIC))
                        ...
                        if (s_file < e_file) {
                                ...
                                if (lu_ptr < lu2_ptr) { /* otherwise file has no
                                                           pages in cache */
                                        /* adjust lookup table entries */
                                        while ((lu_ptr > db_lookup) &&
                                               ((--lu_ptr)->file >= 0)) {
                                                --lu2_ptr;
                                                ...
                                                dbpg_table[pg_slot].lu_slot =
                                                    lu2_ptr - db_lookup;
                                                dbpg_table[lu_ptr->pg_slot]
                                                    .lu_slot =
                                                    lu_ptr - db_lookup;
                                        }
...
int dio_flush() {
        ...
        if (debugging_dio_close) {
                printf(__FILE__
                       "685 dio_flush: check cache dbpgtab=%p count=%d\n",
        ...
        for (pgt_lc = db_pgtab_sz, pg_ptr = dbpg_table; --pgt_lc >= 0;
             ++pg_ptr) {
...
#endif
#ifndef NO_TRANS
                if ((dboptions & TRLOGGING) && trans_id && !trcommit &&
                    use_ovfl) {
                        /* flush to overflow/log file -- before tr commit */
                        if (o_write(pg_ptr, lu_ptr) != S_OKAY)
                                return (db_status);
                ...
                if (debugging_dio_close) {
                        printf(__FILE__
                               "723 dio_flush: write modified pg#%d @ %p\n",
                               db_pgtab_sz - pgt_lc, (void *)pg_ptr);
                        fflush(stdout);
                }
#endif
                if (dio_out(pg_ptr, lu_ptr) != S_OKAY)
                        return (db_status);
#else
                if (dio_out(pg_ptr, lu_ptr, 1) != S_OKAY)
                        return (db_status);
#endif
                pg_ptr->holdcnt = 0;
                pg_ptr->modified = FALSE;
 ...
 */
void dio_setdef(file_no) FILE_NO file_no;
 ...
 */
int dio_get(page_no, page_ptr, hold) F_ADDR page_no;
...
int hold;
{
        ...
        if (dio_findpg(working_file, page_no, dbpg_table, &pg_ptr, NULL) ==
            S_OKAY) {
                MEM_LOCK(&pg_ptr->Buff);
 ...
 */
int dio_touch(page_no) F_ADDR page_no;
{
        PAGE_ENTRY FAR *pg_ptr;
 ...
 */
int dio_read(dba, recptr, hold) DB_ADDR dba;
char FAR *FAR *recptr;
int hold;
{
        FILE_NO file;
        ...
        if (dbopen == 1) {
                /* check shared access priviledges */
                if (!app_locks[file] && !excl_locks[file] &&
                    !(file_ptr->ft_flags & STATIC))
                        return (dberr(S_NOTLOCKED));
        }
#endif
        ...
        if (dio_findpg(file, us2 + 1, dbpg_table, &pg_ptr, NULL) == S_OKAY) {
                ...
                offset = file_ptr->ft_slsize *
                             (int)(us1 - 1 - us2 * file_ptr->ft_slots) +
                         PGHDRSIZE;
                MEM_LOCK(&pg_ptr->Buff);
 ...
 */
int dio_write(dba, recptr, release) DB_ADDR dba;
        ...
        if (dio_findpg(file, us2 + 1, dbpg_table, &pg_ptr, NULL) == S_OKAY) {
                ...
                if (recptr != NULL) {
                        offset = file_ptr->ft_slsize *
                                 ...
                                 PGHDRSIZE;
                        MEM_LOCK(&pg_ptr->Buff);
                        bytecpy(&pg_ptr->buff[offset], recptr,
                                file_ptr->ft_slsize);
                        MEM_UNLOCK(&pg_ptr->Buff);
                }
                if (!pg_ptr->modified) {
 ...
 */
int dio_release(dba) DB_ADDR dba;
{
        FILE_NO file;
        F_ADDR us1, us2;
        PAGE_ENTRY FAR *pg_ptr;
...
INT rid;
{
        ...
        if (dbopen > 1 || (app_locks[file] || excl_locks[file])) {
                ...
                if (dio_findpg(file, page, dbpg_table, &pg_ptr, NULL) ==
                    S_OKAY) {
                        MEM_LOCK(&pg_ptr->Buff);
                        ...
                        if (clr_in_tx) {
                                /* clearing within a transaction requires
                                 * touching page */
                                if (!pg_ptr->modified) {
                                        pg_ptr->modified = TRUE;
                                        if (!pg_ptr->holdcnt)
        ...
        if (!clr_in_tx) {
                ...
                if (dio_open(file) == S_OKAY) {
                        ...
                        DB_LSEEK(file_ptr->ft_desc, (off_t)offset, 0);
                        if (DB_READ(file_ptr->ft_desc, (char FAR *)&trid,
                        ...
                        trid =
                            htons(rid); /* make a copy in trid for byte swap */
                        DB_LSEEK(file_ptr->ft_desc, (off_t)offset,
                                 0); /* reseek */
                        if (DB_WRITE(file_ptr->ft_desc, (char FAR *)&trid,
                                     sizeof(INT)) != sizeof(INT))
                                dberr(S_BADWRITE);
                }
        }
 ...
 */
int dio_findpg(file, page, pg_table, xpg_ptr,
               xlu_ptr) FILE_NO file; /* file number = 0..size_ft-1 */
...
#define tempbuff Tempbuff.ptr
        ...
        if ((file == last_dblu.file) && (page == last_dblu.pageno)) {
                ...
                if (xpg_ptr != NULL)
                        *xpg_ptr =
                            &dbpg_table[db_lookup[last_dblu.slot].pg_slot];
                return (db_status = S_OKAY);
        }
        lookup = db_lookup;
        ...
        /* check if desired page was last one */
#ifdef MULTI_TASK
                if ((Currtask.v.ptr == last_dblu.task) &&
                    (file == last_dblu.file) && (page == last_dblu.pageno)) {
#else
                if ((file == last_dblu.file) && (page == last_dblu.pageno)) {
#endif
                        if (xlu_ptr != NULL)
                                *xlu_ptr = &db_lookup[last_dblu.slot];
                        if (xpg_ptr != NULL)
                                *xpg_ptr = &dbpg_table[db_lookup[last_dblu.slot]
                                                           .pg_slot];
                        return (db_status = S_OKAY);
                }
                lookup = db_lookup;
        ...
        lru_ptr = &dbpg_lru_slot;
#endif /* NO_TRANS */
        for (cnt = 2 * pgtab_sz, pg_slot = *lru_ptr,
            pg_ptr = &pg_table[pg_slot];
             --cnt >= 0; ++pg_slot, ++pg_ptr) {
                if (pg_slot >= pgtab_sz) {
                        pg_slot = 0;
                        pg_ptr = pg_table;
                }
...
#else
                        if (pg_ptr->modified) {
                        /* allow updates outside transactions for single-user
                         * mode */
#ifdef SINGLE_USER
                                ...
                                if (!db_cache ||
                                    ((EXCL_OPEN() ||
                                              ...
                                              (replu_ptr->task ==
                                               Currtask.v.ptr)
                                          ? excl_locks[lookup[pg_ptr->lu_slot]
                                                           .file]
                                          : replu_ptr->task->Excl_locks
                                                .ptr[replu_ptr->file]) &&
                                     !trans_id)) {
#else
                                      excl_locks[lookup[pg_ptr->lu_slot]
                                     ...
                                     !trans_id)) {
                                        ...
                                        if (!use_ovfl || !cache_ovfl)
                                                continue; /* skip modified pages
                                                           */
                                        /* Write out modified page */
                                        pg_ptr->modified = FALSE;
                                        --no_modheld; /* must be in db cache */
                                        if (o_write(pg_ptr, replu_ptr) !=
                                            S_OKAY)
                                                return (db_status);
                                }
#ifdef MULTI_TASK
        ...
        if (replu_ptr < lu_ptr) {
                ...
                while (replu_ptr < lu_ptr) {
                        ...
                        pg_table[replu_ptr->pg_slot].lu_slot =
                            replu_ptr - lookup;
                        ++replu_ptr;
                }
...
#else
        pgsize = (db_cache)
                     ? file_table[(lu_ptr->file > -1) ? lu_ptr->file : file]
                           .ft_pgsize
        ...
        if (pgsize != file_table[file].ft_pgsize) {
                Tempbuff.ptr = NULL;
                tempbuff =
                    ALLOC(&Tempbuff, file_table[file].ft_pgsize, "dbpgbuff");
                if (!tempbuff) {
                        return (dberr(S_NOMEMORY));
                }
                if (pg_ptr->buff) {
 ...
 */
ULONG dio_pzsetts(fno) FILE_NO fno;
{
        ULONG ts;
        PGZERO FAR *pgzero_ptr;
 ...
 */
ULONG dio_pzgetts(fno) FILE_NO fno;
{
...
static int dio_pzinit() {
        ...
        if (ALLOC_TABLE(&db_global.Pgzero, size_ft * sizeof(PGZERO),
                ...
                if (debugging_dio_init) {
                        printf(
                            __FILE__
                            "1444 pzinit: alloc_table failed, db_status=%d\n",
                            db_status);
                        fflush(stdout);
                }
#endif
        ...
        /* read in page zeros */
        for (i = old_size_ft, pgzero_ptr = pgzero; i < size_ft;
             ++i, ++pgzero_ptr) {
...
static int dio_pzflush() {
        ...
        if ((dboptions & TRLOGGING) && trans_id && !trcommit && use_ovfl) {
                ...
                for (i = 0, pgzero_ptr = pgzero; i < size_ft;
                     ++i, ++pgzero_ptr) {
                        if (pgzero_ptr->pz_modified)
                                if (o_pzwrite(i) != S_OKAY)
                                        return (db_status);
                }
        } else {
#endif
                /* flush modified page zeroes to database files */
                for (i = 0, pgzero_ptr = pgzero, file_ptr = file_table;
                     i < size_ft; ++i, ++pgzero_ptr, ++file_ptr) {
                        if (pgzero_ptr->pz_modified) {
                                ...
                                if (DB_WRITE(desc, (char FAR *)pgzero_ptr,
                                             PGZEROSZ) != PGZEROSZ)
                                        return (dberr(S_BADWRITE));
                                pgzero_ptr->pz_modified = FALSE;
#ifndef NO_TRANS
                                if (trlog_flag)
                                        d_trlog(i, 0, (char FAR *)pgzero_ptr,
                                                PGZEROSZ);
#endif
                        }
#ifdef CLOSE_FILES
 ...
 */
int dio_pzread(fno) FILE_NO fno; /* file number */
{
        FILE_ENTRY FAR *file_ptr;
        PGZERO FAR *pgzero_ptr;
 ...
 */
int dio_pzalloc(fno, loc) FILE_NO fno; /* file number */
F_ADDR *loc;                           /* pointer to allocated location */
{
        ...
        if (file_table[fno].ft_type == KEY) {
                ...
                if (pgzero_ptr->pz_dchain == NONE || !(dboptions & DCHAINUSE)) {
                        ...
                        if (dio_get(pg, (char FAR *FAR *)&ptr, NOPGHOLD) !=
                            S_OKAY)
                                return (db_status);
                ...
                if (!pgzero_ptr->pz_dchain || !(dboptions & DCHAINUSE)) {
                        ...
                        pg = pgzero_ptr->pz_dchain;
                        dba = ((NUM2EXT(fno, ft_offset) & FILEMASK)
                               << FILESHIFT) |
                              pg;
                        if (dio_read(dba, (char FAR *FAR *)&ptr, NOPGHOLD) !=
                            S_OKAY)
                                return (db_status);
                        bytecpy(&pgzero_ptr->pz_dchain, ptr + sizeof(INT),
                                sizeof(F_ADDR));
                }
        }
        *loc = pg;
 ...
 */
int dio_pzdel(fno, loc) FILE_NO fno; /* file number */
F_ADDR loc;                          /* location to be freed */
{
        ...
        if (file_table[fno].ft_type == KEY) {
                ...
                bytecpy(ptr + sizeof(INT), &pgzero_ptr->pz_dchain,
                        sizeof(F_ADDR));
                pgzero_ptr->pz_dchain = loc;
 ...
 */
F_ADDR dio_pznext(fno) FILE_NO fno;
{
        if (pgzero[fno].pz_next == 0L)
                dio_pzread(fno);
 ...
 * -f/usr/users/master/config/nonwin dio.c */
