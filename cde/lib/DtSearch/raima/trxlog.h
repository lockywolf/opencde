...
#define IX_BASE(file, pcnt)                                                    \
        ((long)(BM_BASE(file) + (BITMAP_SIZE(pcnt) * sizeof(unsigned int))))
#define PZ_BASE(file, pcnt)                                                    \
        ((long)(IX_BASE(file, pcnt) + (IX_SIZE(pcnt) * IX_PAGESIZE)))
 ...
 * -f/usr/users/master/config/nonwin trxlog.h */
