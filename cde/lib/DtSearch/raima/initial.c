...
typedef union INIT_PAGE_U {
        ...
        struct {
                ...
                char init_addr[sizeof(
                    LONG)]; /* NONE node pointer on key file;
                            System record db_addr on data file */
                char init_crts[sizeof(
                    LONG)]; /* if system record is timestamped */
                char init_upts[sizeof(
                    LONG)]; /* if system record is timestamped */
        } pg1;
} INIT_PAGE;

typedef struct {
        union INIT_PAGE_U FAR *ptr;
        LOCK_DESC
} INIT_PAGE_P;
 ...
 */
int d_initialize(TASK_DBN_ONLY) TASK_DECL DBN_DECL {
        FILE_NO fno;
 ...
 */
int d_initfile(fno TASK_PARM DBN_PARM)
...
#define page (Page.ptr)
        ...
        if (file_ptr->ft_status == OPEN)
                ...
                /* We don't have enuf files, open then close this file to free
                 * one */
                dio_open(fno);
                dio_close(fno);
        }
        ...
        if ((dbfile = DB_OPEN(file_ptr->ft_name, O_RDWR | O_CREAT | O_TRUNC)) <
            0)
        ...
        if (file_ptr->ft_type == KEY) {
                ...
                for (rec = 0, rec_ptr = record_table; rec < size_rt;
                     ++rec, ++rec_ptr) {
                        if ((rec_ptr->rt_fdtot == -1) &&
                            (rec_ptr->rt_file == fno)) {

                                /*---Write special DATA FILE PAGE 0 for system
                                 * record file ---*/
                                page->pg0.next = htonl(2);
                                DB_WRITE(dbfile, (char FAR *)page,
                                         (int)file_ptr->ft_pgsize);

                                /*--------- Write DATA FILE PAGE 1 for system
                                 * record ---------*/
                                byteset(page, '\0', file_ptr->ft_pgsize);
                                ...
                                page->pg1.chg_date = extern_timestamp;
                                addr =
                                    (((LONG)NUM2EXT(fno, ft_offset) & FILEMASK)
                                     << FILESHIFT) |
                                    1;
                                HTONL(addr);
                                bytecpy(page->pg1.init_addr, &addr,
                                        sizeof(F_ADDR));

                                /* Slot 1 (system record) header is recid, dba,
                                 * maybe two phony timestamps, no set ptrs or
                                 * member ptrs.
                                 */
                                ...
                                if (rec_ptr->rt_flags & TIMESTAMPED) {
                                        ...
                                        ts = htonl(1);
                                        bytecpy(page->pg1.init_crts, &ts,
                                                sizeof(ULONG));
                                        bytecpy(page->pg1.init_upts, &ts,
                                                sizeof(ULONG));
                                }
                                DB_WRITE(dbfile, (char FAR *)page,
                                         (int)file_ptr->ft_pgsize);
 ...
 * -f/usr/users/master/config/nonwin initial.c */
