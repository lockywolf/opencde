...
void gen_vec(char *fname_huffcode_tab) {
        ...
        while (fgets(tab_filebuf, sizeof(tab_filebuf) - 1, tab_stream) !=
               NULL) {
                i = atoi(
                    _XStrtok(tab_filebuf, DELIMITERS, strtok_buf)); /* char */
                ...
                if (temp[0] == ' ') {
                        ...
                        *(code_length + i) = *(code_length + LITERAL_NUM) + 8;
                        ...
                        huff_code[i] = (char *)malloc(*(code_length + i) + 1);
...
/*						*/
/************************************************/
int hc_encode(struct or_blobrec *targblobrec, UCHAR *charbuf, int charcount,
              int file_pos) {
        ...
        ************/
        ...
        for (i = 0; i < charcount; i++) {
                if (!last_call) {
                        ret_code =
                            process_char(charbuf[i], (char *)blobrec.or_blob);
                }
