...
char *teskey_parser(PARG *parg) {
        ...
        static int wordlen;
        static enum { BETW_WORDS, IN_WORD, TOO_LONG } tpstate;
        ...
        if (parg) {
                ...
                if (charmap == NULL) {
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_lang, 4,
                ...
                if (parg->string) {
                        ...
                        fprintf(aa_stderr,
                                catgets(dtsearch_catd, MS_lang, 5,
                                        "%s Program Error: parg contains "
                                        "neither file nor string.\n"),
                                PROGNAME "327");
                        DtSearchExit(27);
                }
                ...
                if (outbufsz <= maxwordsz) {
                        ...
                        outbuf =
                            austext_malloc(outbufsz + 8, PROGNAME "807", NULL);
                }
                endmaxword = outbuf + maxwordsz;
                if (debugging_teskey)
                        fprintf(aa_stderr,
                                "teskey: start of text block, maxwsz=%d "
                                "outbufsz=%lu\n",
...
READ_ANOTHER_WORD:
                                        ...
                                        *outp = charmap[*outp] & 0x00ff;
                                        ...
                                        if (debugging_teskey)
                                                fprintf(aa_stderr,
                                                        ...
                                                        outbuf);
                                        if (add_msgs) {
                                                char msgbuf[DtSrMAXWIDTH_HWORD +
                                                            100];
                                                sprintf(
                                                    msgbuf,
        ...
        if (outbuf[0] == 0) {
                ...
                if (add_msgs) {
                        char msgbuf[200];
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_lang, 12,
                                        "%s '%.120s' is not a valid word in "
                                        "database '%s'."),
        ...
        if (debugging_teskey)
                fprintf(aa_stderr, "teskey: ofs=%3ld \"%s\"", candidate_offset,
                        outbuf);
        ...
        if (wordlen < minwordsz) {
                ...
                if (add_msgs) {
                        char msgbuf[200];
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_lang, 17,
        ...
        if (dblk->stoplist != NULL) {
                if (search_wordtree(dblk->stoplist, outbuf)) {
                        ...
                        if (add_msgs) {
                                char msgbuf[200];
                                sprintf(msgbuf,
                                        catgets(dtsearch_catd, MS_lang, 19,
                                                "%s The word '%s' is not "
                                                "indexed in database '%s'."),
                                        PROGNAME "558", parg->string,
                                        dblk->label);
                                DtSearchAddMessage(msgbuf);
                                return NULL;
                        }
 ...
 */
int load_wordtree(WORDTREE **treetop, DBLK *dblk, char *fname,
                  int do_teskey_test) {
        int i;
        int errcount;
        int is_duplicate;
        ...
        while (fgets(readbuf, sizeof(readbuf), fileid) != NULL) {
                ...
                if (do_teskey_test)
                        if (!is_concordable(token, dblk->charmap)) {
                                sprintf(sprintbuf,
                                        catgets(dtsearch_catd, MS_misc, 400,
                                                "%s: %s, line %ld: Invalid "
                                                "chars in word '%s'."),
                                        PROGNAME "400", fname, linecount,
                                        token);
                                DtSearchAddMessage(sprintbuf);
                                errcount++;
                                continue;
                ...
                if (errcount)
                        continue;
                i = strlen(token);
                new = austext_malloc(sizeof(WORDTREE) + i + 4, PROGNAME "104",
                                     NULL);
                new->llink = NULL;
                new->rlink = NULL;
                new->len = i;
        ...
        if (errcount) {
                ...
                if (debugging_loadlang)
                        fprintf(aa_stderr,
                                PROGNAME
                                "1193 load word file '%s' successful.\n",
                                fname);
                return 0;
        }
} /* load_wordtree() */
                ...
                if (dblk->name[0] == 0) {
                        ...
                        if (debugging_loadlang)
                                fprintf(aa_stderr, PROGNAME
                                        "1339 "
                                        "No inclist because neither fname nor "
                                        "dbname provided.\n");
                        return TRUE;
                }
                if (dblk->path == NULL)
...
static int load_stop_list(DBLK *dblk, DBLK *dblist) {
        ...
        if (dblk->fname_stp == NULL) {
                ...
                if (errno == ENOENT) {
                        strcpy(dblk->fname_stp, dblk->path);
                        ensure_end_slash(dblk->fname_stp);
                        strcat(dblk->fname_stp,
                               lang_fnames[dblk->dbrec.or_language]);
                        strcat(dblk->fname_stp, EXT_STOPLIST);
                }
        }
        if (debugging_loadlang)
                fprintf(aa_stderr, PROGNAME "1448 Stoplist file name = '%s'.\n",
                        dblk->fname_stp);
...
static int load_paice_suffixes(DBLK *dblk, DBLK *dblist) {
        ...
        if (debugging_loadlang)
                fprintf(aa_stderr,
                        PROGNAME
                        "1715 Load paice suffixes: db='%s' lang=#%d,%s\n",
                        NULLORSTR(dblk->name), (int)dblk->dbrec.or_language,
                        language_name(dblk->dbrec.or_language));
         ...
         */
        rules_table = austext_malloc(256 * sizeof(PRULE *), PROGNAME "199",
                                     &ausapi_msglist);
        ...
        while (fgets(readbuf, sizeof(readbuf), fp) != NULL) {
                 ...
                 */
                if ((suffix = (UCHAR *)_XStrtok(readbuf, SFX_DELIMS,
                                                strtok_buf)) == NULL)
                        continue;
                ...
                if ((token = (UCHAR *)_XStrtok(NULL, SFX_DELIMS, strtok_buf)) ==
                    NULL) {
                BAD_RULE:
                        sprintf(msgbuf,
                                catgets(dtsearch_catd, MS_lang, 51,
                                        "%s %s, Line %d: Invalid Paice Rule "
                                        "for suffix '%s'."),
                                PROGNAME "898", dblk->fname_sfx, lineno,
                                suffix);
                        DtSearchAddMessage(msgbuf);
                        errcount++;
                        continue;
                ...
                if (token[0] == '*') {
                        must_be_intact = TRUE;
                        /* Read next token... */
                        if ((token = (UCHAR *)_XStrtok(NULL, SFX_DELIMS,
                                                       strtok_buf)) == NULL)
                                goto BAD_RULE;
                }
                ...
                if (debugging_loadword) {
                        fprintf(aa_stderr,
                                ...
                                (int)must_be_intact, (int)is_last_rule,
                                (int)remove_count, suffix);
                        if (apndstr)
...
static char *paice_stemmer(char *wordin, DBLK *dblk) {
        ...
        if ((rules_table = (PRULE **)dblk->stem_extra) == NULL) {
                fprintf(aa_stderr,
                        catgets(dtsearch_catd, MS_lang, 31,
        ...
        for (;;) { /*-------- Main Stemming Loop ---------*/
                ...
                if (rule == NULL) {
                        if (debugging_paice)
                                fprintf(
                                    aa_stderr,
                                    "  rules list '%c' is exhausted, stop.\n",
                                    finalc);
                        break;
                }
 ...
 */
char *null_lstrupr(char *s, DBLK *d) { return s; }
...
int load_language(DBLK *dblk, DBLK *dblist) {
        ...
        if (debugging_loadlang)
                fprintf(aa_stderr,
                        "\n" PROGNAME
                        "1920 Loading language #%d, %s, for dblk '%s'.\n",
        ...
        case DtSrLaDEU:
                dblk->charmap =
                    (language == DtSrLaENG) ? ascii_charmap : latin_charmap;
