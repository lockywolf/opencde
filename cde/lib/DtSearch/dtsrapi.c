...
static void signal_abort(int sig) {
        ...
        fprintf(aa_stderr,
                catgets(dtsearch_catd, MS_ausapi, 216,
                        "\n%s %s Caught signal %d.\n"),
...
static int valid_dbname(char *dbname) {
        ...
        if (db == NULL) {
                sprintf(sprintbuf,
                        catgets(dtsearch_catd, MS_ausapi, 1,
...
static void build_dbnames_array(void) {
        ...
        for (db = usrblk.dblist; db != NULL; db = db->link) {
                ausapi_dbnamesc++;
                mallocsz +=
                    ((db->label) ? strlen(db->label) : sizeof(db->name)) + 2L;
        }
         ...
         */
        targ =
            (char *)ausapi_dbnamesv + (sizeof(char *) * (1 + ausapi_dbnamesc));
 ...
 */
int DtSearchInit(char *argv0, char *userid, long switches, char *config_file,
                 FILE *err_file, char ***dbnames, int *dbcount) {
        if (aa_is_initialized) {
                sprintf(sprintbuf,
                        catgets(dtsearch_catd, MS_ausapi, 621,
        ...
        if (switches & DtSrInSIGNAL) {
                ...
                signal(SIGKILL,
                       signal_abort); /* (kill -9, cannot be trapped) */
                ...
                signal(SIGDANGER,
                       signal_abort); /* imminent paging space crash */
#endif
        }
                ...
                if ((userid = (char *)getenv("LOGNAME")) == NULL) {
                        sprintf(sprintbuf,
                                catgets(dtsearch_catd, MS_ausapi, 187,
                                        "%1$s Missing both userid and LOGNAME "
        ...
        if (dbnames) {
                *dbnames = ausapi_dbnamesv;
                if (!dbcount) {
                        sprintf(
                            sprintbuf,
                            catgets(dtsearch_catd, MS_ausapi, 7,
                  ...
                  int *stemcount)   /* put size of stems array */
{
        ...
        default:
                sprintf(sprintbuf,
                        catgets(dtsearch_catd, MS_ausapi, 20,
        ...
        QUERY_ERROR:
                sprintf(sprintbuf,
                        catgets(dtsearch_catd, MS_ausapi, 30,
        ...
        if (qryarg == TEXT && qry != usrblk.query) {
                if (usrblk.query != NULL)
                        free(usrblk.query);
                usrblk.query =
                    austext_malloc(strlen(qry) + 16, PROGNAME "102", NULL);
        ...
        if (usrblk.debug & USRDBG_SRCHCMPL) {
                ...
                *(--ptr) = 0;
                ...
                fprintf(
                    ...
                    NULLORSTR(date1), date1str, NULLORSTR(date2),
                    objdate2fzkstr(usrblk.objdate2), NULLORSTR(usrblk.query));
                fflush(aa_stderr);
        }
...
END_OF_SEARCH:
        if (usrblk.debug & USRDBG_SRCHCMPL) {
                ...
                for (llp = ausapi_msglist; llp != NULL; llp = llp->link)
                        i++;
                fprintf(aa_stderr,
                        PROGNAME "380 Return from Search:  "
                ...
                if (ausapi_msglist == NULL) {
                        sprintf(
                            ...
                            catgets(dtsearch_catd, MS_ausapi, 806,
                                    ...
                                    "incompatible with database '%s' to "
                                    "commence search."),
...
int DtSearchHighlight(char *dbname,    /* database name */
        ...
        if (cleartext == NULL || cleartext[0] == 0) {
                sprintf(
                    sprintbuf,
                    catgets(dtsearch_catd, MS_ausapi, 40,
        ...
        if (cleartext != usrblk.cleartext) {
                ...
                usrblk.cleartext =
                    austext_malloc(usrblk.clearlen + 16, PROGNAME "267", NULL);
                ...
                if (!ausapi_msglist) {
                        sprintf(sprintbuf,
                                catgets(dtsearch_catd, MS_ausapi, 1342,
                                        "%s Search Engine Error %d for "
                                        "highlight request for "
                                        "database '%s', hit word count=%ld, "
                                        "search type='%c', "
                                        "text='%.30s'"),
                                PROGNAME "1342", usrblk.retncode, dbname,
                                *hitwcount, search_type, NULLORSTR(cleartext));
                        DtSearchAddMessage(sprintbuf);
                }
                return DtSrERROR;
        } /* end switch */
