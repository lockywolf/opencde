...
static void print_usage(void) {
        fprintf(
            ...
            "  dbname: 1 - 8 char database name = the old d99/d2x files to be "
            "updated.\n"
            "     Files found in local directory or DBFPATH environment "
            "variable.\n"
            "  newpath: Specifies where the new d99/d2x files will be placed.\n"
            "     If first char is not slash, path is relative to local "
            "directory.\n"
            "OPTIONS:\n"
            "  -pN: Progress dots printed every N records (default %lu).\n"
            "     Complete progress message printed every %d dots.\n"
            "  -oy: Authorizes overlaying preexisting d99/d2x files in "
            "newpath.\n"
            "  -on: Forces exit if preexisting d99/d2x files in newpath.\n"
            "  -v:  Validates d99 and d00 links, uncorrupts d99 file, and "
            "ensures\n"
            "     accurate record count.  Also use -c0 to uncorrupt entire "
            "database.\n"
            "  -v<fname>:  Same as -v but also writes all d00 recs "
            "unreferenced by "
            "d99\n"
            "     to fname in format suitable for albeniz to extract into fzk "
            "format.\n"
            /********
            "  -mN: Changes max # database documents from curr count in database
            to N.\n"
            *********/
            "  -cN: Exits if more than N corrupted/incomplete links (default "
            "%d).\n"
...
static void end_of_job(int exitcode, int show_flags) {
        ...
        if (exitcode >= 100) {
                fprintf(aa_stderr,
                        PROGNAME "66 Aborting after interrupt signal %d.\n",
                        exitcode - 100);
        }
        ...
        if (corruption_count > 0L) {
                if (max_corruption > 0L && corruption_count >= max_corruption)
                        fprintf(aa_stderr,
                                PROGNAME
                                "193 Aborting at %ld corrupted links.\n",
                                corruption_count);
                else
                        fprintf(
                            aa_stderr,
...
static void user_args_processor(int argc, char **argv) {
        ...
        while (--argc > 0) {
                ...
                case 'm':
                        fprintf(
                            aa_stderr, PROGNAME
                            "301 The -m argument is no longer necessary.\n");
                        break;
                        ...
                        if (argptr[2] != '\0') {
                                ...
                                if ((frecids = fopen(argptr + 2, "w")) ==
                                    NULL) {
                                        fprintf(aa_stderr,
                                                PROGNAME
                                                "802 Unable to open '%s' "
                                                "to output unreferenced d00 "
                                                "records: %s\n",
                ...
                default:
                        fprintf(aa_stderr,
                                PROGNAME "159 Unknown argument: '%s'.\n",
                                argptr);
                        oops = TRUE;
                        break;
                } /* end switch */
        ...
        if (argc != 2) {
                if (argc <= 0)
                        fputs(PROGNAME
                              "210 Missing required dbname argument.\n",
                              aa_stderr);
                if (argc <= 1)
                        fputs(PROGNAME
                              "211 Missing required newpath argument.\n",
                              aa_stderr);
                if (argc > 2)
        ...
        if (strlen(arg_dbname) > 8) {
                fprintf(aa_stderr, PROGNAME "229 Invalid database name '%s'.\n",
                        arg_dbname);
                end_of_job(2, SHOW_USAGE);
        }
...
static void open_all_files(FILE **fp, char *fname, char *mode, size_t *size,
        ...
        if (fstat(fileno(*fp), &fstatbuf) == -1) {
                fprintf(aa_stderr,
                        PROGNAME "440 Can't access status of %s: %s\n", fname,
                        strerror(errno));
                *oops = TRUE;
                return;
        }
...
static void copy_old_d2x_to_new(char *fname_old, char *fname_new, FILE *fp_old,
        ...
        for (;;) /* loop ends when eof set on input stream */
        {
                ...
                if (errno) {
                        fprintf(aa_stderr,
                                PROGNAME "517 Read error on %s: %s.\n",
                ...
                if (i != j) {
                        fprintf(aa_stderr,
                                PROGNAME "489 Write error on %s: %s.\n",
...
static void copy_new_d99(long keyfield) {
        ...
        while (db_status == S_OKAY) {
                ...
                ****************/
                 ...
                 * "holes". In the first portion of record holes are filled with
                 * representations of valid database addresses + statistical
                 * weights. In the second portion the holes are "free slots" for
                 * future expansion which are conventionally initialized with a
                 * -1.
                 */
                ...
                while (!done) /* loop on each block in this word */
                {
                        ...
                        errno = 0;
                        fread(word_addrs, sizeof(DB_ADDR), num_reads,
                              fp_d99_old);
                        if (errno) {
                                TERMINATE_LINE();
                                fprintf(aa_stderr,
                                        PROGNAME "657 Read error on %s: %s.\n",
                         ...
                         * The vista file number is known from the #define
                         * constant OR_D00, and the vista dba/slot# is mapped
                         * from rec# by mult/div number of slots per rec,
                         * plus/minus dbrec offset.
                         */
                        if (validation_mode) {
                        ...
                        /* If NOT in validation mode, just write out the new d99
                         * block as an exact copy of the input block.
                         */
                        else {
                                num_writes = fwrite(word_addrs, sizeof(DB_ADDR),
                                                    num_reads, fp_d99_new);
                                if (num_writes != num_reads) {
                                WRITE_ERROR:
                                        fprintf(aa_stderr,
                                                PROGNAME
                                                "665 Write error on %s: %s.\n",
...
int main(int argc, char *argv[]) {
        ...
        strftime(dbfpath, sizeof(dbfpath), /* just use any ol' buffer */
                 ...
                 time_ptr);
        printf(
            catgets(dtsearch_catd, MS_misc, 23, "%s: Version %s%s.  Run %s.\n"),
        ...
        if ((ptr = getenv("DBFPATH")) != NULL) {
                if (*ptr == 0)
                        fprintf(aa_stderr,
                                "%s: Ignoring empty DBFPATH environment "
                                "variable.\n",
                                aa_argv0);
                else {
        ...
        if (strcmp(full_dbname_old, full_dbname_new) == 0) {
                fprintf(aa_stderr,
                        PROGNAME
                        "393 Old and new directories are identical: '%s'\n.",
                        full_dbname_old);
                end_of_job(2, SHOW_USAGE);
        }
        ...
        if (!overlay_yes) {
                ...
                if (oops) {
                        ...
                        fputs("    Is it ok to overlay files in new directory? "
                              "[y/n] ",
                              aa_stderr);
        ...
        **********************/
        ...
        /* this is where real dba was changed to record number (still called
         * dba) */
        ...
        fprintf(aa_stderr,
                ...
                aa_argv0, dbrec.or_reccount, dbrec.or_maxdba, dbrec.or_recslots,
                total_num_addrs);
        ...
        if (validation_mode) {
                ...
                fprintf(aa_stderr,
                        "%s: Reading d00 file.  Each dot appx %ld database "
                        "documents...\n",
                        aa_argv0, x);
                reccount = 0L;
                dot_count = 0L;
                ...
                while (db_status == S_OKAY) {
                        ...
                        dba &=
                            0x00ffffff; /* mask out file number in high order
                                           byte */
                        dba1 = (dba + dba_offset) /
                        ...
                        if (dba1 >= total_num_addrs) {
                                ...
                                fprintf(
                                    ...
                                    PROGNAME
                                    "561 "
                                    "DBA '%d:%ld' (rec #%ld) in d00 exceeds "
                                    "total num addrs %ld;\n"
                                    "  Bit vector overflow because maxdba %ld"
                                    " in dbrec is incorrect.\n",
                                    /******		OR_D00, dba, dba1,
                                       max_totrecs);******/
                                    OR_D00, dba, dba1, total_num_addrs,
                                    dbrec.or_maxdba);
        ...
        bytes_in = 0L;
        dot_count =
            DOTS_PER_MSG; /* force initial msg after first blk of recs */
        TERMINATE_LINE();
        fprintf(aa_stderr,
        ...
        if (validation_mode) {
                for (x = 0L, bvptr = bit_vector; x < max_bitvec; x++, bvptr++) {
                        for (j = 0; j < 8;
                             j += 4) /* j = 0 or 4, amount of bit shift */
                        {
                                ...
                                if (a & 0x10 && !(a & 0x20)) {
                                        ...
                                        fprintf(
                                            aa_stderr,
                                            "*** " PROGNAME "444 "
                                            "d00 record '%s' is not referenced "
                                            "in d99.\n"
                                            "  DBA = %d:%ld (x%02x:%06lx).\n",
                                            readbuf, OR_D00, dba, OR_D00, dba);
                                        /***readbuf, OR_D00, dbaorig, OR_D00,
                                         * dbaorig);****/
