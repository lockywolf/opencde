...
void print_dittolist(DtSrResult *dittolist, char *prefix) {
        ...
        for (dit = dittolist; dit != NULL && maxcount-- > 0; dit = dit->link) {
                ...
                fprintf(aa_stderr,
                        ...
                        dit->proximity, (long)dit->objsize, datebuf,
                        dit->reckey);
                if (dit->abstractp)
...
char *get_hitlist_text(int maxlen) {
        ...
        if (usrblk.dittocount <= 0L) {
                sprintf(
                    sprintbuf,
                    catgets(dtsearch_catd, MS_misc, 96, "%s Hitlist is empty."),
                    PROGNAME "96");
                DtSearchAddMessage(sprintbuf);
                return NULL;
        }
        if (maxlen > 0 && maxlen < DtSrMAX_DB_KEYSIZE + 7) {
                sprintf(sprintbuf, PROGNAME "97 maxlen = %d is too small.",
                        maxlen);
                DtSearchAddMessage(sprintbuf);
                return NULL;
        }
        ...
        mallocsz =
            usrblk.dittocount * (usrblk.abstrbufsz + DtSrMAX_DB_KEYSIZE + 80L);
        if (text != NULL)
                free(text);
        ...
        for (dit = usrblk.dittolist; dit != NULL; dit = dit->link) {
                if (maxlen == 0) {
                        sprintf(targ, "%5d %-*s %s\n%n", dit->proximity,
                                DtSrMAX_DB_KEYSIZE, dit->reckey, dit->abstractp,
                                &sofar);
                        targ += sofar;
                } else /* (maxlen > 0) */
                       /*
                ...
                {
                        eol = targ + maxlen;
                        sprintf(targ, "%5d %-*s %n", dit->proximity,
                                DtSrMAX_DB_KEYSIZE, dit->reckey, &sofar);
                        targ += sofar;
                        src = dit->abstractp;
                        for (;;) {
...
void print_usrblk_record(char *prefix) {
        fprintf(aa_stderr,
                "%s usrblk_record(): dba=%ld:%ld objkey='%s'\n"
                "  notesp=%p clearlen=%d cleartxt='%.30s'\n"
                "  #hitw=%d hitw=%p abstr='%.24s'\n",
                prefix, (long)usrblk.dba >> 24, (long)usrblk.dba & 0xffffffL,
                usrblk.objrec.or_objkey, (void *)usrblk.notes,
                (int)usrblk.clearlen, NULLORSTR(usrblk.cleartext),
                (int)usrblk.hitwcount, (void *)usrblk.hitwords,
