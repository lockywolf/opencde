 ...
 */
#define CMSTAT(s, a, b)                                                        \
        if (d_cmstat(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define CMTYPE(s, a, b, c)                                                     \
        if (d_cmtype(a, b, c) < S_OKAY)                                        \
        vista_abort(s)
#define CONNECT(s, a, b)                                                       \
        if (d_connect(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define COSTAT(s, a, b)                                                        \
        if (d_costat(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define COTYPE(s, a, b, c)                                                     \
        if (d_cotype(a, b, c) < S_OKAY)                                        \
        vista_abort(s)
#define CRGET(s, a, b)                                                         \
        if (d_crget(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define CRREAD(s, a, b, c)                                                     \
        if (d_crread(a, b, c) < S_OKAY)                                        \
        vista_abort(s)
#define CRSET(s, a, b)                                                         \
        if (d_crset(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define CRSTAT(s, a)                                                           \
        if (d_crstat(a) < S_OKAY)                                              \
        vista_abort(s)
#define CRTYPE(s, a, b)                                                        \
        if (d_crtype(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define CRWRITE(s, a, b, c)                                                    \
        if (d_crwrite(a, b, c) < S_OKAY)                                       \
        vista_abort(s)
#define CSMGET(s, a, b, c)                                                     \
        if (d_csmget(a, b, c) < S_OKAY)                                        \
        vista_abort(s)
#define CSMREAD(s, a, b, c, d)                                                 \
        if (d_csmread(a, b, c, d) < S_OKAY)                                    \
        vista_abort(s)
#define CSMSET(s, a, b, c)                                                     \
        if (d_csmset(a, b, c) < S_OKAY)                                        \
        vista_abort(s)
#define CSMWRITE(s, a, b, c, d)                                                \
        if (d_csmwrite(a, b, c, d) < S_OKAY)                                   \
        vista_abort(s)
#define CSOGET(s, a, b, c)                                                     \
        if (d_csoget(a, b, c) < S_OKAY)                                        \
        vista_abort(s)
#define CSOREAD(s, a, b, c, d)                                                 \
        if (d_csoread(a, b, c, d) < S_OKAY)                                    \
        vista_abort(s)
#define CSOSET(s, a, b, c)                                                     \
        if (d_csoset(a, b, c) < S_OKAY)                                        \
        vista_abort(s)
#define CSOWRITE(s, a, b, c, d)                                                \
        if (d_csowrite(a, b, c, d) < S_OKAY)                                   \
        vista_abort(s)
#define CSSTAT(s, a, b)                                                        \
        if (d_csstat(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define CTBPATH(s, a)                                                          \
        if (d_ctbpath(a) < S_OKAY)                                             \
        vista_abort(s)
#define CTSCM(s, a, b, c)                                                      \
        if (d_ctscm(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define CTSCO(s, a, b, c)                                                      \
        if (d_ctsco(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define CTSCR(s, a, b)                                                         \
        if (d_ctscr(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define DBDPATH(s, a)                                                          \
        if (d_dbdpath(a) < S_OKAY)                                             \
        vista_abort(s)
#define DBFPATH(s, a)                                                          \
        if (d_dbfpath(a) < S_OKAY)                                             \
        vista_abort(s)
#define DBLOG(s, a)                                                            \
        if (d_dblog(a) < S_OKAY)                                               \
        vista_abort(s)
#define DBTAF(s, a)                                                            \
        ...
        vista_abort(s)
#define DBUSERID(s, a)                                                         \
        if (d_dbuserid(a) < S_OKAY)                                            \
        vista_abort(s)
#define DECODE_DBA(s, a, b, c)                                                 \
        if (d_decode_dba(a, b, c) < S_OKAY)                                    \
        vista_abort(s)
#define DELETE(s, a)                                                           \
        if (d_delete(a) < S_OKAY)                                              \
        vista_abort(s)
#define DESTROY(s, a)                                                          \
        if (d_desoy(a) < S_OKAY)                                               \
        vista_abort(s)
#define DISCON(s, a, b)                                                        \
        if (d_discon(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define DISDEL(s, a)                                                           \
        if (d_disdel(a) < S_OKAY)                                              \
        vista_abort(s)
#define ENCODE_DBA(s, a, b, c)                                                 \
        if (d_encode_dba(a, b, c) < S_OKAY)                                    \
        vista_abort(s)
#define FILLNEW(s, a, b, c)                                                    \
        if (d_fillnew(a, b, c) < S_OKAY)                                       \
        vista_abort(s)
#define FINDCO(s, a, b)                                                        \
        if (d_findco(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define FINDFM(s, a, b)                                                        \
        if (d_findfm(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define FINDLM(s, a, b)                                                        \
        if (d_findlm(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define FINDNM(s, a, b)                                                        \
        if (d_findnm(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define FINDPM(s, a, b)                                                        \
        if (d_findpm(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define FREEALL(s)                                                             \
        if (d_freeall() < S_OKAY)                                              \
        vista_abort(s)
#define GTSCM(s, a, b, c)                                                      \
        if (d_gtscm(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define GTSCO(s, a, b, c)                                                      \
        if (d_gtsco(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define GTSCR(s, a, b)                                                         \
        if (d_gtscr(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define GTSCS(s, a, b, c)                                                      \
        if (d_gtscs(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define INITFILE(s, a, b)                                                      \
        if (d_initfile(a, b) < S_OKAY)                                         \
        vista_abort(s)
#define INITIALIZE(s, a)                                                       \
        if (d_initialize(a) < S_OKAY)                                          \
        vista_abort(s)
#define ISMEMBER(s, a, b)                                                      \
        if (d_ismember(a, b) < S_OKAY)                                         \
        vista_abort(s)
#define ISOWNER(s, a, b)                                                       \
        if (d_isowner(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define KEYDEL(s, a, b)                                                        \
        if (d_keydel(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define KEYEXIST(s, a, b)                                                      \
        if (d_keyexist(a, b) < S_OKAY)                                         \
        vista_abort(s)
#define KEYFIND(s, a, b, c)                                                    \
        if (d_keyfind(a, b, c) < S_OKAY)                                       \
        vista_abort(s)
#define KEYFREE(s, a, b)                                                       \
        if (d_keyfree(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define KEYFRST(s, a, b)                                                       \
        if (d_keyfrst(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define KEYLAST(s, a, b)                                                       \
        if (d_keylast(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define KEYLOCK(s, a, b, c)                                                    \
        if (d_keylock(a, b, c) < S_OKAY)                                       \
        vista_abort(s)
#define KEYLSTAT(s, a, b, c)                                                   \
        if (d_keylstat(a, b, c) < S_OKAY)                                      \
        vista_abort(s)
#define KEYNEXT(s, a, b)                                                       \
        if (d_keynext(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define KEYPREV(s, a, b)                                                       \
        if (d_keyprev(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define KEYREAD(s, a)                                                          \
        if (d_keyread(a) < S_OKAY)                                             \
        vista_abort(s)
#define KEYSTORE(s, a, b)                                                      \
        if (d_keystore(a, b) < S_OKAY)                                         \
        vista_abort(s)
#define LOCK(s, a, b, c)                                                       \
        if (d_lock(a, b, c) < S_OKAY)                                          \
        vista_abort(s)
#define MAKENEW(s, a, b)                                                       \
        if (d_makenew(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define MAPCHAR(s, a, b, c, d)                                                 \
        if (d_mapchar(a, b, c, d) < S_OKAY)                                    \
        vista_abort(s)
#define MEMBERS(s, a, b, c)                                                    \
        if (d_members(a, b, c) < S_OKAY)                                       \
        vista_abort(s)
#define OFF_OPT(s, a)                                                          \
        if (d_off_opt(a) < S_OKAY)                                             \
        vista_abort(s)
#define ON_OPT(s, a)                                                           \
        if (d_on_opt(a) < S_OKAY)                                              \
        vista_abort(s)
#define OPEN(s, a, b)                                                          \
        if (d_open(a, b) < S_OKAY)                                             \
        vista_abort(s)
#define RDCURR(s, a, b)                                                        \
        if (d_rdcurr(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define RECFREE(s, a, b)                                                       \
        if (d_recfree(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define RECFRST(s, a, b)                                                       \
        if (d_recfrst(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define RECLAST(s, a, b)                                                       \
        if (d_reclast(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define RECLOCK(s, a, b, c)                                                    \
        if (d_reclock(a, b, c) < S_OKAY)                                       \
        vista_abort(s)
#define RECLSTAT(s, a, b, c)                                                   \
        if (d_reclstat(a, b, c) < S_OKAY)                                      \
        vista_abort(s)
#define RECNEXT(s, a)                                                          \
        if (d_recnext(a) < S_OKAY)                                             \
        vista_abort(s)
#define RECOVER(s, a)                                                          \
        if (d_recover(a) < S_OKAY)                                             \
        vista_abort(s)
#define RECPREV(s, a)                                                          \
        if (d_recprev(a) < S_OKAY)                                             \
        vista_abort(s)
#define RECREAD(s, a, b)                                                       \
        if (d_recread(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define RECSET(s, a, b)                                                        \
        if (d_recset(a, b) < S_OKAY)                                           \
        vista_abort(s)
#define RECSTAT(s, a, b, c)                                                    \
        if (d_recstat(a, b, c) < S_OKAY)                                       \
        vista_abort(s)
#define RECWRITE(s, a, b)                                                      \
        if (d_recwrite(a, b) < S_OKAY)                                         \
        vista_abort(s)
#define RENFILE(s, a, b, c)                                                    \
        if (d_renfile(a, b, c) < S_OKAY)                                       \
        vista_abort(s)
#define RLBCLR(s, a)                                                           \
        if (d_rlbclr(a) < S_OKAY)                                              \
        vista_abort(s)
#define RLBSET(s, a)                                                           \
        if (d_rlbset(a) < S_OKAY)                                              \
        vista_abort(s)
#define RLBTST(s, a)                                                           \
        if (d_rlbtst(a) < S_OKAY)                                              \
        vista_abort(s)
#define SETDB(s, a)                                                            \
        if (d_setdb(a) < S_OKAY)                                               \
        vista_abort(s)
#define SETFILES(s, a)                                                         \
        if (d_setfiles(a) < S_OKAY)                                            \
        vista_abort(s)
#define SETFREE(s, a, b)                                                       \
        if (d_setfree(a, b) < S_OKAY)                                          \
        vista_abort(s)
#define SETLOCK(s, a, b, c)                                                    \
        if (d_setlock(a, b, c) < S_OKAY)                                       \
        vista_abort(s)
#define SETLSTAT(s, a, b, c)                                                   \
        if (d_setlstat(a, b, c) < S_OKAY)                                      \
        vista_abort(s)
#define SETMM(s, a, b, c)                                                      \
        if (d_setmm(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define SETMO(s, a, b, c)                                                      \
        if (d_setmo(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define SETMR(s, a, b)                                                         \
        if (d_setmr(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define SETOM(s, a, b, c)                                                      \
        if (d_setom(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define SETOO(s, a, b, c)                                                      \
        if (d_setoo(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define SETOR(s, a, b)                                                         \
        if (d_setor(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define SETPAGES(s, a, b)                                                      \
        if (d_setpages(a, b) < S_OKAY)                                         \
        vista_abort(s)
#define SETRM(s, a, b)                                                         \
        if (d_setrm(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define SETRO(s, a, b)                                                         \
        if (d_setro(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define STSCM(s, a, b, c)                                                      \
        if (d_stscm(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define STSCO(s, a, b, c)                                                      \
        if (d_stsco(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define STSCR(s, a, b)                                                         \
        if (d_stscr(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define STSCS(s, a, b, c)                                                      \
        if (d_stscs(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define TIMEOUT(s, a)                                                          \
        if (d_timeout(a) < S_OKAY)                                             \
        vista_abort(s)
#define TRABORT(s)                                                             \
        if (d_trabort() < S_OKAY)                                              \
        vista_abort(s)
#define TRBEGIN(s, a)                                                          \
        if (d_trbegin(a) < S_OKAY)                                             \
        vista_abort(s)
#define TREND(s)                                                               \
        if (d_trend() < S_OKAY)                                                \
        vista_abort(s)
#define UTSCM(s, a, b, c)                                                      \
        if (d_utscm(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define UTSCO(s, a, b, c)                                                      \
        if (d_utsco(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define UTSCR(s, a, b)                                                         \
        if (d_utscr(a, b) < S_OKAY)                                            \
        vista_abort(s)
#define UTSCS(s, a, b, c)                                                      \
        if (d_utscs(a, b, c) < S_OKAY)                                         \
        vista_abort(s)
#define WRCURR(s, a)                                                           \
        if (d_wrcurr(a) < S_OKAY)                                              \
        vista_abort(s)
/*------------- end DMACROS.H -------------------*/
...
extern unsigned long duprec_hashsize; /* isduprec.c */
extern int latin_charmap[];           /* langmap.c */

/*------------------------ FUNCTION PROTOTYPES ------------------------*/
extern void add_free_space(FREE_SPACE_STR *del_rec, FILE_HEADER *flh);
extern void append_ext(char *buffer, int buflen, char *fname, char *fext);
extern int austext_dopen(char *dbname, char *dbpath, char *d2xpath,
                         int cache_size, DBREC *bufptr);
extern void *austext_malloc(size_t size, char *location, void *ignored);
extern char *bmhcore(UCHAR *text, size_t txtlen, UCHAR *pattern, size_t patlen,
                     size_t *bmhtable);
extern void bmhtable_build(UCHAR *pattern, size_t patlen, size_t *bmhtable);
extern char *bmstrstr(UCHAR *text, size_t txtlen, UCHAR *pattern,
                      size_t patlen);
...
extern FREE_SPACE_STR *find_free_space(DtSrINT32 req_size, FILE_HEADER *flh);
...
extern int hc_encode(struct or_blobrec *targ, UCHAR *src, int srclen,
                     int force_flush);
...
extern int load_wordtree(WORDTREE **treetop, DBLK *dblk, char *fname,
                         int do_teskey_test);
...
extern struct tm *objdate2tm(DtSrObjdate objdate);
extern int objdate_in_range(DtSrObjdate recdate, DtSrObjdate date1,
                            DtSrObjdate date2);
...
extern void replace_ext(char *buffer, int buflen, char *fname, char *fext);
extern LLIST *sort_llist(LLIST *list_header);
extern char *teskey_parser(PARG *parg);
extern DtSrObjdate tm2objdate(struct tm *tmptr);
#if !defined(linux)
#ifndef _ALL_SOURCE
