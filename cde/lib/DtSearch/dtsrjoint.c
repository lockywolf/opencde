...
#ifdef DEBUG_VALDATE
                fprintf(aa_stderr,
                        PROGNAME "269 No numeric digits in string\n");
#endif
        INVALID_DATESTR:
                sprintf(
                    msgbuf,
...
DATESTR_OK:
        ...
        fprintf(aa_stderr,
                ...
                yy, mm, dd, objdate2fzkstr(objdate));
        fflush(aa_stderr);
#endif
        return objdate;
} /* DtSearchValidDateString() */
...
/*						*/
/************************************************/
int DtSearchGetMaxResults(void) { return aa_maxhits; }

void DtSearchSetMaxResults(int newmax) { aa_maxhits = newmax; }
...
static DtSrResult *merge_by_prox(DtSrResult *l1, DtSrResult *l2) {
        ...
        while ((l1 != NULL) && (l2 != NULL)) {
                 ...
                 */
                mynext = (l1->proximity < l2->proximity) ? ditto_pop(&l1)
                                                         : ditto_pop(&l2);
...
static DtSrResult *merge_by_date(DtSrResult *l1, DtSrResult *l2) {
        ...
        while ((l1 != NULL) && (l2 != NULL)) {
                ...
                if (l1->objdate == l2->objdate)
                        ...
                        mynext = (l1->objdate > l2->objdate) ? ditto_pop(&l1)
                                                             : ditto_pop(&l2);
