...
int ttdnd_init(int Xid, const char **validtypes, Ttdnd_send_data_func *sendfunc,
               Ttdnd_input_func **input_func, int *fd_ptr) {
        int mark = tt_mark();
        const char *t, **vt;
 ...
 */
char *ttdnd_negotiate_types(const char **sourcetypes,
 ...
 * going.
 */
void ttdnd_start_transfer(int sourceXid, const char *source_type,
 ...
 */
static Tt_callback_action _ttdnd_continue_transfer(m, p) Tt_message m;
Tt_pattern p;
{
        Ttdnd_receive_data_func *receivefunc;
        int sourceXid;
        ...
        mark = tt_mark();
        receivefunc =
            (Ttdnd_receive_data_func *)tt_message_user(m, USER_FUNC_KEY);
        ...
        case TT_HANDLED:
                ...
                receivefunc_quits = (*receivefunc)(
                    chunknum, sourcetype, (void *)chunkptr, chunksize, is_last);
                ...
                if (receivefunc_quits) {
                        ...
                        _ttdnd_get_contents(sourceXid, sourcetype, receivefunc,
                                            chunknum + 1);
                }
                break;
 ...
 */
static Tt_callback_action _ttdnd_get_contents_callback(m, p) Tt_message m;
Tt_pattern p;
{
        int chunknumber;
        int maxchunk;
        ...
        sendfunc_quits = (*sendfunc)(chunknumber, maxchunk, type, buffer,
                                     &chunksize, &is_last);
        if (sendfunc_quits) {
                tt_message_fail(m);
        } else {
                tt_message_arg_bval_set(m, 1, (unsigned char *)buffer,
                                        chunksize);
...
void _ttdnd_receive() {
        ...
        if (m)
                tt_message_destroy(
                    m); /* should not be needed, but to be safe. */
}
