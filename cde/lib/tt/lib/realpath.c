...
char *_tt_internal_realpath(pathname, finalpath) char *pathname;
char *finalpath;
{
        struct stat sbuf;
        char curpath[MAXPATHLEN], workpath[MAXPATHLEN], linkpath[MAXPATHLEN],
            namebuf[MAXPATHLEN], *where, *ptr, *last;
        int len;
...
loop:
        ...
        while (*where != NULL) {
                ...
                if ((sbuf.st_mode & S_IFLNK) == S_IFLNK) {
                        ...
                        *(linkpath + len) =
                            NULL; /* readlink doesn't null-terminate result */
                        if (*linkpath == '/')
                                *workpath = NULL;
                        if (*where) {
                                strcat(linkpath, "/");
