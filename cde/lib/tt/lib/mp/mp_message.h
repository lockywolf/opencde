...
class _Tt_message : public _Tt_object {
      public:
        ...
        Tt_class message_class() const { return _message_class; }
        Tt_state state() const { return _state; }

        Tt_scope scope() const { return _scope; }
        int opnum() const { return _opnum; }
        const _Tt_string &handler_ptype() const { return _handler_ptype; }
        Tt_disposition reliability() const { return _reliability; }

        Tt_address paradigm() const { return _paradigm; }
        const _Tt_string &file() const { return _file; }
        const _Tt_session_ptr &session() const { return _session; }
        const _Tt_string &op() const { return _op; }
        const _Tt_arg_list_ptr &args() const { return _args; }
        _Tt_msg_context_ptr context(const char *slotname) const;
        _Tt_msg_context_ptr context(int i) const;
        int contextsCount() const;
        const _Tt_string &object() const { return _object; }
        const _Tt_string &otype() const { return _otype; }
        const _Tt_procid_ptr &sender() const { return _sender; }
        const _Tt_procid_ptr &handler() const { return _handler; }
        const _Tt_procid_list_ptr &abstainers() const { return _abstainers; }
        const _Tt_procid_list_ptr &accepters() const { return _accepters; }
        const _Tt_procid_list_ptr &rejecters() const { return _rejecters; }
        const _Tt_string &sender_ptype() const { return _sender_ptype; }
        int queue() const { return reliability() & TT_QUEUE; }
        int start() const { return reliability() & TT_START; }
        int id() const;
        ...
        Tt_status add_voter(const _Tt_procid_ptr &p, Tt_state vote);
        ...
        int is_observer() { return (_flags & (1 << _TT_MSG_OBSERVER)); }
        bool_t xdr(XDR *xdrs);
