...
class _Tt_procid : public _Tt_object {
      public:
        ...
        int is_equal(const _Tt_procid_ptr &p) { return (_id == p->_id); }
        int is_equal(_Tt_string &id);
        int joined_to_file(_Tt_string fpath);
        int processing(const _Tt_message &m);
        _Tt_pattern_list_ptr &patterns() { return _patterns; }
        pid_t pid() const { return _pid; }
        int port() const;
        void print(const _Tt_ostream &os) const;
        _Tt_host_ptr &proc_host() { return _proc_host; }
        int program() const { return _program; }
        void set_start_token(const _Tt_string &token) { _start_token = token; }
        _Tt_string &start_token() { return _start_token; }
        int version() const { return _version; }
        _Tt_string &proc_host_ipaddr() { return _proc_host_ipaddr; }
        _Tt_string_list_ptr &ptypes() { return _declared_ptypes; }
        bool_t xdr(XDR *xdrs);
