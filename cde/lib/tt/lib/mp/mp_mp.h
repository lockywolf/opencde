...
enum _Tt_mp_flags { _TT_MP_IN_SERVER, _TT_MP_INIT_DONE };

class _Tt_mp : public _Tt_object {
      public:
        _Tt_mp();
        virtual ~_Tt_mp();
        int in_server() { return (_flags & (1 << _TT_MP_IN_SERVER)); };
        Tt_status find_file(_Tt_string path, _Tt_file_ptr &f, int create_ifnot);
        ...
        int active_messages;
        Tt_status find_session(_Tt_string id, _Tt_session_ptr &sp,
                               int create_ifnot, int auto_start = 0);
