...
enum _Tt_env { _TT_ENV_X11, _TT_ENV_PROCESS_TREE, _TT_ENV_LAST };
...
class _Tt_session : public _Tt_object {
      public:
        ...
        _Tt_string auth_cookie() { return _auth.auth_cookie(); }
        _Tt_auth_level auth_level() { return _auth.auth_level(); }
        Tt_status call(int rpc_proc, xdrproc_t xdr_arg_fn, char *arg,
                       xdrproc_t xdr_res_fn, char *res, int timeout = -1,
                       int rebind = 0);
        _Tt_env env();
        const _Tt_host_ptr &host() { return _host; }
        const _Tt_string &id() const { return _id; }

        Tt_status ping();
        // void			print(FILE *fs = stdout) const;
        int rpc_program() { return _rpc_program; }
        int rpc_version() { return _rpc_version; }
