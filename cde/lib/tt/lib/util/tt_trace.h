...
class _Tt_trace : public _Tt_allocated {
      public:
        ...
        static int init(int even_if_no_envariable = 0);
        ...
        void entry(_Tt_message &msg, Tt_state old_state);
        void entry(_Tt_message &msg, const _Tt_procid &recipient);
        void entry(_Tt_message &msg, _Tt_dispatch_reason reason);

        // API tracing entry call
        void entry(const char *argskey, _Tt_entry_pt func, va_list ap);

        // callback tracing call
        void entry(Tt_message_callback cb, Tt_message mh, Tt_pattern ph);

        const _Tt_trace &operator<<(_Tt_message &msg) const;
        const _Tt_trace &operator<<(const _Tt_pattern &pat) const;
        const _Tt_trace &operator<<(const _Tt_string &s) const;
        const _Tt_trace &operator<<(Tt_status status) const;
        const _Tt_trace &operator<<(const char *s) const;
        const _Tt_trace &operator<<(int n) const;
        const _Tt_trace &print(_Tt_object_printfn print_it,
        ...
        void exit();
        void exit(void *val);
        void exit(Tt_message val);
        void exit(Tt_pattern val);
        void exit(char *val);
        void exit(int val);
        void exit(Tt_status val);
        void exit(Tt_callback_action val);
        void exit(Tt_mode val);
        void exit(Tt_scope val);
        void exit(Tt_class val);
        void exit(Tt_category val);
        void exit(Tt_address val);
        void exit(Tt_disposition val);
        void exit(Tt_state val);
      ...
      private:
        ...
        void _exit(int val, const char *(*enumname)(int), int drop = 1);
        //
