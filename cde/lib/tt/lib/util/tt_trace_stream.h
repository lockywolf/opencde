...
class _Tt_trace_stream : public _Tt_trace_parser, public _Tt_ostream {
      public:
        _Tt_trace_stream();
        _Tt_trace_stream(const char *script);
        _Tt_trace_stream(int scriptfd);
        ~_Tt_trace_stream();

        Tt_status init();

        void set_is_entered(int on_off) { _is_entered = on_off; }
        int get_is_entered() const { return _is_entered; }
...
// operator << methods.
//
const _Tt_trace_stream &operator<<(_Tt_trace_stream &os, _Tt_message &msg);
