...
#define declare_table_of(TYPE)                                                 \
        class table_of(TYPE) : public _Tt_object_table {                       \
              ...
              public:                                                          \
                ...
                table_of(TYPE)(_Tt_object_table_keyfn kf, int nbkts = 19)      \
                    : _Tt_object_table(nbkts) {                                \
                        set_keyfn(kf);                                         \
                }                                                              \
                virtual ~table_of(TYPE)();                                     \
                void insert(ptr_to(TYPE) & o) { _Tt_object_table::insert(o); } \
        ...
        declare_derived_ptr_to(table_of(TYPE), _Tt_object_table)               \
                                                                               \
            class cursor_in_table_of(TYPE)                                     \
            : public _Tt_object_table_cursor {                                 \
              public:                                                          \
                ...
                cursor_in_table_of(TYPE) & reset() {                           \
                        return (cursor_in_table_of(                            \
                            TYPE) &)_Tt_object_table_cursor::reset();          \
                }                                                              \
                cursor_in_table_of(TYPE) & reset(ptr_to(table_of(TYPE)) & l) { \
                        return (cursor_in_table_of(                            \
                            TYPE) &)_Tt_object_table_cursor::reset(l);         \
                }                                                              \
                int next() { return _Tt_object_table_cursor::next(); }         \
                int prev() { return _Tt_object_table_cursor::prev(); }         \
                int is_valid() const {                                         \
                        return _Tt_object_table_cursor::is_valid();            \
                }                                                              \
                ptr_to(TYPE) &operator*() {                                    \
                        return (ptr_to(                                        \
                            TYPE) &)_Tt_object_table_cursor::operator*();      \
                }                                                              \
                ptr_to(TYPE) &operator->() {                                   \
                        return (ptr_to(                                        \
                            TYPE) &)_Tt_object_table_cursor::operator*();      \
...
#define implement_table_of(TYPE)                                               \
        table_of(TYPE)::~table_of(TYPE)() {}                                   \
                                                                               \
        bool_t table_of(TYPE)::xdr(XDR *xdrs) {                                \
                return _Tt_object_table::xdr(                                  \
                    xdrs, (_Tt_new_xdrfn)xdrfn_of(TYPE),                       \
                    (_Tt_object * (*)()) constructor_of(TYPE));                \
        }                                                                      \
        cursor_in_table_of(TYPE)::~cursor_in_table_of(TYPE)() {}               \
