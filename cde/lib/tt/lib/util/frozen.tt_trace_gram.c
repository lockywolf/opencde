...
#endif
{
        register TT_TRACE_STYPE
            *tt_trace_pvt; /* top of value stack for $vars */

#if defined(__cplusplus) || defined(lint)
        /*
                hacks to please C++ and lint - goto's inside switch should never
           be executed; tt_trace_pvt is set to 0 to avoid "used before set"
           warning.
        */
        ...
        */
...

        {
                ...
                goto tt_trace_stack;      /* moved from 6 lines above to here to
                                             please C++ */
        ...
        tt_trace__stack:
...
#endif /* TT_TRACE_DEBUG */
                if (++tt_trace__ps >=
                    &tt_trace_s[tt_trace_maxdepth]) /* room on stack? */
                {
                        /*
                        ...
                        */
...
#else
                        tt_trace_newmax =
                            2 * tt_trace_maxdepth; /* double table size */
                        if (tt_trace_maxdepth ==
                            TT_TRACE_MAXDEPTH) /* first time growth */
                        {
                                char *newtt_trace_s = (char *)TT_TRACE_NEW(int);
                                char *newtt_trace_v =
                                    (char *)TT_TRACE_NEW(TT_TRACE_STYPE);
                                if (newtt_trace_s != 0 && newtt_trace_v != 0) {
                                        tt_trace_s = TT_TRACE_COPY(
                                            newtt_trace_s, tt_trace_s, int);
                                        tt_trace_v = TT_TRACE_COPY(
                                            newtt_trace_v, tt_trace_v,
                                            TT_TRACE_STYPE);
                                } else
                        ...
                        } else /* not first time */
                        {
                                ...
                                tt_trace_v = TT_TRACE_ENLARGE(tt_trace_v,
                                                              TT_TRACE_STYPE);
...
#endif
                        if (tt_trace_newmax <=
                            tt_trace_maxdepth) /* tables not expanded */
                        {
                ...
                */
        tt_trace__newstate:
                if ((tt_trace__n = tt_trace_pact[tt_trace__state]) <=
                    TT_TRACE_FLAG)
...
#endif /* TT_TRACE_DEBUG */
                if (((tt_trace__n += tt_trace_char) < 0) ||
                    (tt_trace__n >= TT_TRACE_LAST))
                ...
                if ((tt_trace__n = tt_trace_def[tt_trace__state]) == -2) {
                        ...
                        if (tt_trace_debug && tt_trace_tmp) {
                                ...
                                if (tt_trace_char == 0)
                                        ...
                                        for (tt_trace__i = 0;
                                             tt_trace_toks[tt_trace__i].t_val >=
                                             0;
                                             tt_trace__i++) {
                                                if (tt_trace_toks[tt_trace__i]
                                                        .t_val ==
                                                    tt_trace_char) {
                                                        break;
                                                }
                        ...
                        case 2: /* incompletely recovered error */
                                ...
                                */
                                while (tt_trace__ps >= tt_trace_s) {
                                        tt_trace__n =
                                            tt_trace_pact[*tt_trace__ps] +
                                            TT_TRACE_ERRCODE;
                                        if (tt_trace__n >= 0 &&
                                            tt_trace__n < TT_TRACE_LAST &&
                                            tt_trace_chk[tt_trace_act
                                                             [tt_trace__n]] ==
                                                TT_TRACE_ERRCODE) {
                                                /*
                ...
                */
                {
                        ...
                        if (!(tt_trace__len & 01)) {
                                tt_trace__len >>= 1;
                                tt_trace_val = (tt_trace__pv -=
                                                tt_trace__len)[1]; /* $$ = $1 */
                                tt_trace__state =
                                    tt_trace_pgo[tt_trace__n =
                                                     tt_trace_r1[tt_trace__n]] +
                                    *(tt_trace__ps -= tt_trace__len) + 1;
                                if (tt_trace__state >= TT_TRACE_LAST ||
                                    tt_trace_chk[tt_trace__state = tt_trace_act
                                                     [tt_trace__state]] !=
                                        -tt_trace__n) {
                                        tt_trace__state = tt_trace_act
                                            [tt_trace_pgo[tt_trace__n]];
                                }
                                goto tt_trace__stack;
                        }
                        tt_trace__len >>= 1;
                        tt_trace_val =
                            (tt_trace__pv -= tt_trace__len)[1]; /* $$ = $1 */
                        tt_trace__state =
                            tt_trace_pgo[tt_trace__n =
                                             tt_trace_r1[tt_trace__n]] +
                            *(tt_trace__ps -= tt_trace__len) + 1;
                        if (tt_trace__state >= TT_TRACE_LAST ||
                            tt_trace_chk[tt_trace__state =
                                             tt_trace_act[tt_trace__state]] !=
                                -tt_trace__n) {
                                tt_trace__state =
                                    tt_trace_act[tt_trace_pgo[tt_trace__n]];
                        }
                }
...
#line 182 "util/tt_trace_gram.y"
        {
                if (!_tt_process_state(tt_trace_pvt[-0].stateval.state_type,
                        ...
                        _tt_trace_iserr = 2;
                        tt_trace_error(
                            "Can't mix none with other state values");
                }
        } break;
        case 28:
