        ...
        declare_derived_ptr_to(TYPE, PARENT)                                   \
            ...
            : public list_of(PARENT) {                                         \
              ...
              public:                                                          \
                ...
                ~list_of(TYPE)();                                              \
                                                                               \
                void flush() { _Tt_object_list::flush(); }                     \
                ...
                list_of(TYPE) & append_destructive(ptr_to_list_of(TYPE) l) {   \
                        return (list_of(                                       \
                            TYPE) &)_Tt_object_list::append_destructive(l);    \
                }                                                              \
                ...
                ptr_to(TYPE) &operator[](int n) const {                        \
                        return (ptr_to(TYPE) &)_Tt_object_list::operator[](n); \
                                                                               ...
                                                                               \
                int is_empty() const { return _Tt_object_list::is_empty(); }   \
                int count() const { return _Tt_object_list::count(); }         \
                bool_t xdr(XDR *xdrs);                                         \
        };                                                                     \
                                                                               \
        class cursor_in_list_of(TYPE) : public _Tt_object_list_cursor {        \
              public:                                                          \
                ...
                ptr_to(TYPE) &operator*() const {                              \
                        return (ptr_to(                                        \
                            TYPE) &)_Tt_object_list_cursor::operator*();       \
                }                                                              \
                ptr_to(TYPE) &operator->() const {                             \
                        return (ptr_to(                                        \
                            TYPE) &)_Tt_object_list_cursor::operator*();       \
                ...
                cursor_in_list_of(TYPE) & reset() {                            \
                        return (cursor_in_list_of(                             \
                            TYPE) &)_Tt_object_list_cursor::reset();           \
                }                                                              \
                cursor_in_list_of(TYPE) &                                      \
                    reset(const ptr_to_list_of(TYPE) & l) {                    \
                        return (cursor_in_list_of(                             \
                            TYPE) &)_Tt_object_list_cursor::reset(l);          \
                }                                                              \
                int next() { return _Tt_object_list_cursor::next(); }          \
                int prev() { return _Tt_object_list_cursor::prev(); }          \
                cursor_in_list_of(TYPE) & insert(const ptr_to(TYPE) & p) {     \
                        return (cursor_in_list_of(                             \
                            TYPE) &)_Tt_object_list_cursor::insert(p);         \
                }                                                              \
                cursor_in_list_of(TYPE) & remove() {                           \
                        return (cursor_in_list_of(                             \
                            TYPE) &)_Tt_object_list_cursor::remove();          \
                }                                                              \
                int is_valid() const {                                         \
...
#define implement_list_of(TYPE)                                                \
        implement_derived_ptr_to(TYPE, _Tt_object)                             \
            implement_derived_ptr_to(list_of(TYPE), _Tt_object_list)           \
                list_of(TYPE)::list_of(TYPE)() {}                              \
        list_of(TYPE)::~list_of(TYPE)() {}                                     \
                                                                               \
        bool_t list_of(TYPE)::xdr(XDR *xdrs) {                                 \
                return _Tt_object_list::xdr(                                   \
                    xdrs, (_Tt_new_xdrfn)xdrfn_of(TYPE),                       \
                    (_Tt_object * (*)()) constructor_of(TYPE));                \
        }                                                                      \
                                                                               \
        cursor_in_list_of(TYPE)::~cursor_in_list_of(TYPE)() {}
