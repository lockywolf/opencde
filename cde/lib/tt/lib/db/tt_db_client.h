...
class _Tt_db_client : public _Tt_object {
      public:
        ...
        // Sets and gets the current access mode to be used when reading,
        // writing and removing data to and from the database.  If not used, the
        // default access is world read and write.
        ...
        // exist in the database, the file entry is also created.  The cache
        // level returned can be used to initialize the cache levels for the
        // types of info stored for this object.
        ...
        // Writes the specified file property to the database.  All values of
        // the the file property with the same name are overwritten.  If the
        // value of the property is NULL, this deletes all properties with the
        ...
        // and will not be added to the list of values if the combination
        // already exists.
        _Tt_db_results addFileProperty(const _Tt_string &file,
                                       const _Tt_db_property_ptr &property,
                                       bool_t unique, int &cache_level);
        ...
        _Tt_db_results getFileProperty(const _Tt_string &file,
                                       const _Tt_string &name, int &cache_level,
        ...
        _Tt_db_results getFileObjects(const _Tt_string &file, int &cache_level,
        ...
        // Writes the specified object property to the database.  All values of
        // the the object property with the same name are overwritten.  If the
        // value of the property is NULL, this deletes all properties with the
        // same name from the database.  If the cache level in the database is
        // higher then the cache level passed in, the write is not done and the
        // values of all properties in the database are returned through the
        // properties argument. The latest cache level is always returned.
        _Tt_db_results setObjectProperty(const _Tt_string &objid,
                                         const _Tt_db_property_ptr &property,
                                         _Tt_db_property_list_ptr &properties,
                                         int &cache_level);

        // Writes the specified object properties to the database.  All values
        // of the object properties with the same names are overwritten.  If the
        // value of some of the properties are NULL, this deletes all properties
        // with the same name from the database.  If the cache level in the
        // database is higher then the cache level passed in, the write is not
        // done and all of the object properties in the database are returned
        // through the out_properties argument.  The latest cache level is
        // always returned.
        _Tt_db_results
        ...
        // the property name and value combination is guaranteed to be unique
        // and will not be added to the list of values if the combination
        // already exists.  If the cache level in the database is higher then
        // the cache level passed in, the write is not done and the values of
        // all the properties in the database are returned through the
        // properties argument. The latest cache level is always returned.
        ...
        // If the cache level in the database is higher then the cache level
        // passed in, the write is not done and the values of all the properties
        // in the database are returned through the properties argument.
        // The latest cache level is always returned.
        _Tt_db_results deleteObjectProperty(
            const _Tt_string &objid, const _Tt_db_property_ptr &property,
            _Tt_db_property_list_ptr &properties, int &cache_level);
        ...
        _Tt_db_results getObjectProperties(const _Tt_string &objid,
                                           int &cache_level,
        ...
        _Tt_db_results getObjectFile(const _Tt_string &objid, _Tt_string &file);
        ...
        _Tt_db_results isFileInDatabase(const _Tt_string &file,
                                        bool_t &directory_flag);

        _Tt_db_results isObjectInDatabase(const _Tt_string &objid,
        ...
        // to the specified ptypes.  If there are no other ptypes that this
        // message is addressed to, the message is deleteed from the database.
        _Tt_db_results dequeueMessages(const _Tt_string &file,
                                       const _Tt_string_list_ptr &ptypes,
                                       _Tt_message_list_ptr &messages);

        // Gets the remote ends' idea of what the netfile version of the
        // specified file is.
        _Tt_db_results file_netfile(const _Tt_string &file,
                                    _Tt_string &netfile);

        // Gets the remote ends' idea of what the file version of the specified
        // netfile is.
        _Tt_db_results netfile_file(const _Tt_string &netfile,
                                    _Tt_string &file);
        ...
        _Tt_db_results delsession(const _Tt_string &file,
                                  const _Tt_string &session);
        _Tt_db_results gettype(const _Tt_string &objid, _Tt_string &type);
