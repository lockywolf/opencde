 ...
 */
/*
 * tt_db_object.h - Declare the TT DB server object class.  This class defines
 * the object properties interface to the DB server.
 *
...
class _Tt_db_object : public _Tt_object {
      public:
        ...
        // Removes the spec from the database and/or memory and leaves the
        // specified forward pointer in its place.
        ...
        // the object property with the same name are overwritten.  If the value
        // of the property is empty, this deletes all properties with the same
        ...
        // the object properties with the same names are overwritten.  If the
        // value of some of the properties are empty, this deletes all
        // properties with the same name from the database.
        _Tt_db_results
        setProperties(const _Tt_db_property_list_ptr &properties);

        // Adds an object property to the list of values of the object property
        // with the same name in memory.  If the property does not already
        // exist, then the property is created.  If the "unique" argument is
        // TRUE, then the property name and value combination is guaranteed to
        // be unique and will not be added to the list of values if the
        // combination already exists.
        _Tt_db_results addProperty(const _Tt_db_property_ptr &property,
                                   bool_t unique = FALSE);

        // Deletes the object property value with the same name and value from
        // the DB.  If the value is left empty, then all values of the property
        // are deleted.  The deletion is not cached.  The deletion is
        // immediately executed on disk.
        _Tt_db_results deleteProperty(const _Tt_db_property_ptr &property);

        // Gets the values of the object property with the specified name from
        // the memory cache.  If the property does not exist in memory, then the
        // value is obtained from the DB.
        _Tt_db_property_ptr getProperty(const _Tt_string &name);

        // Gets a list of all of the object properties currently written to the
        // DB. If the object has never been written to the DB, then an error is
        // returned.
        ...
        const _Tt_string &getObjectID() const { return dbObjectID; }

        // Returns the key porition of this object's object ID.
        _Tt_string getObjectKey() const;

        // Gets the results of the last database operation made with this
        // object.
        _Tt_db_results getDBResults() const { return dbResults; }
      ...
      private:
        ...
        _Tt_db_results createMemoryObject(const _Tt_string &,
                                          const _Tt_string &);
        ...
        void setCurrentDBAccess() {
                if (!dbObjectDatabase.is_null()) {
                        dbObjectDatabase->setCurrentAccess(
                            dbObjectCurrentAccess);
                }
        }
};
