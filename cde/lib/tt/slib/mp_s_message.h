...
class _Tt_s_message : public _Tt_message {
      public:
        ...
        int deliver(const _Tt_msg_trace &trace, int deliver_to_observers = 1);
        void deliver_to_observers_and_handlers(const _Tt_msg_trace &trace);
        void set_return_sender_flags();
        void set_send_handler_flags();
        Tt_status change_state(const _Tt_procid_ptr &changer, Tt_state s,
                               const _Tt_msg_trace &trace);
        void remove_procid(const _Tt_procid_ptr &p);
        int is_handler_copy() const { return (_observer.is_null()); }
        const _Tt_observer_ptr &observer() const { return _observer; }
      ...
      private:
        ...
        Tt_status hdispatch();
        void handle_no_recipients(const _Tt_msg_trace &trace);
        Tt_status handler_dispatch(const _Tt_msg_trace &trace);
        int match_handler(const _Tt_s_pattern &pat, const _Tt_msg_trace &trace,
                          int &best_match, Tt_category &best_category,
                          unsigned int &best_timestamp) const;
        int match_handler(const _Tt_signature &sig, const _Tt_msg_trace &trace,
                          int &best_match, Tt_category &best_category,
                          unsigned int &best_timestamp) const;
        ...
        Tt_status match_signatures(_Tt_signature_list_ptr &s,
                                   const _Tt_msg_trace &trace);
        int match_super_sig(_Tt_otype_ptr ot, _Tt_signature_ptr &sig,
                            const _Tt_msg_trace &trace);
        Tt_status object_oriented_dispatch(const _Tt_msg_trace &trace);
        Tt_status procedural_dispatch(const _Tt_msg_trace &trace);
