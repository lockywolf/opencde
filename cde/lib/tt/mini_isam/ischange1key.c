...
int _change1key(fcb, pkeydesc2, record, oldrecord, recnum, newkey) Fcb *fcb;
Keydesc2 *pkeydesc2;
char *record;
char *oldrecord;
Recno recnum;
char *newkey;
{
        ...
        if (memcmp(keybuf1, keybuf2, pkeydesc2->k2_len)) {
                ...
                if (ALLOWS_DUPS2(pkeydesc2)) {
                        ...
                        /*
                         * We must now scan all the duplicates till we found one
                         * with matching recno. We are positioned just before
                         * the first duplicate. Remember that the duplicates are
                         * ordered by their serial number.
                         *
                         * If too many duplicate entries exists, the performance
                         * will be poor.
                         */
                        ...
                        if (pkey == NULL)
                                _isfatal_error(
                                    "_del1key() cannot find entry in B tree");
                ...
                if (ALLOWS_DUPS2(pkeydesc2)) {
                         ...
                         * Try to read the last duplicate (with the highest
                         * serial number). If such duplicate exists, the
                         * inserted key entry will be assigned next higher value
                         * for duplicate serial number. If there is no such
                         * duplicate, assign duplicate serial number 1.
                         */
                        strecno(recnum, keybuf2 + KEY_RECNO_OFF);
                        memcpy(keybuf2 + KEY_DUPS_OFF, (char *)ismaxlong,
                               LONGSIZE);
                        ...
                        if ((pkey = _isbtree_current(btree)) != NULL &&
                            memcmp(pkey + RECNOSIZE + DUPIDSIZE,
                                   ...
                                   pkeydesc2->k2_len - RECNOSIZE - DUPIDSIZE) ==
                                0) {

                                /*
                                 * A duplicate exists. Assign +1 value for the
                                 * new serial number.
                                 */
                                stlong(ldlong(pkey + KEY_DUPS_OFF) + 1,
                                       keybuf2 + KEY_DUPS_OFF);

                                /*
                                 * Indicate that there are allowed duplicate key
                                 * values.
                                 */
                                isdupl = 1;
                        } else {
                                ...
                                /*
                                 * This is the first duplicate. Assign serial
                                 * number 1.
                                 */
                         ...
                         * Set TID part to maximum value, causing the search
                         * path to point just past the possible duplicate in the
                         * key file If there is no duplicate, this is the
                         * correct spot to insert the new key entry.
                         */

                        memcpy(keybuf2 + KEY_RECNO_OFF, (char *)ismaxlong,
                               LONGSIZE);
                        _isbtree_search(btree, keybuf2);
