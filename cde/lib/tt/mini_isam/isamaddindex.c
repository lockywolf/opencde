...
/*ARGSUSED*/
int _amaddindex(isfhandle, keydesc, errcode) Bytearray *isfhandle;
...
/*ARGSUSED*/
int _amaddprimary(isfhandle, keydesc, errcode) Bytearray *isfhandle;
struct keydesc *keydesc;
struct errcode *errcode;
{
        Fcb *fcb = NULL;
...
int _create_index(fcb, pkeydesc2) Fcb *fcb;
Keydesc2 *pkeydesc2;
{
        Issort *srt;
         ...
         */
        if (fcb->nrecords == 0L) {
                pkeydesc2->k2_rootnode =
                    _buildbtree(fcb, pkeydesc2, (Issort *)NULL);
                return (ISOK);
        }
...
Static void _readallrecords(fcb, srt, pkeydesc2) Fcb *fcb;
Issort *srt;
Keydesc2 *pkeydesc2;
{
...
Static void _attach_dups_serial(srt, pkeydesc2) Issort *srt;
Keydesc2 *pkeydesc2;
{
        ...
        while (curkey = _issort_read(srt)) {
                if (lastkey &&
                    memcmp(lastkey + RECNOSIZE + DUPIDSIZE,
                           curkey + RECNOSIZE + DUPIDSIZE, netkeylength) == 0)
                        dup_serial++;
                else
                        dup_serial = 1;
...
Static Blkno _buildbtree(fcb, pkeydesc2, srt) Fcb *fcb;
Keydesc2 *pkeydesc2;
Issort *srt;
{
        Bufhdr *_allockpage();
        ...
        for (depth = 1; blocks_in_this_level > 1; depth++) {
                ...
                blocks_in_this_level =
                    (blocks_in_prev_level - 1) / nleafcapac + 1;
                keyspernode[depth] =
                    blocks_in_prev_level / blocks_in_this_level;
        ...
        while ((keyp = _issort_read(srt)) != (char *)NULL) {
                ...
                if (curindex[0] >= keyspernode[0] + (one_more[0] > 0)) {
                        ...
                        /*
                         * Commit all changed buffers here to avoid buffer pool
                         * overflow.
                         */
                        if (nodebufhd[0]) {
                        ...
                        /* Allocate new leaf. */
                        nodebufhd[0] =
                            _allockpage(fcb, leafcapac, 0, pageblkno + 0);
                ...
                if (curindex[0]++ == 0) { /* Store first key in  */
                        ...
                        for (i = 1; i < depth; i++) {
                                if (curindex[i] >=
                                    keyspernode[i] + (one_more[i] > 0)) {

                                        /* Unfix buffer. */
                                        if (nodebufhd[i])
                                                _isdisk_commit1(nodebufhd[i]);

                                        nodebufhd[i] = _allockpage(
                                            fcb, nleafcapac, i, pageblkno + i);
                                ...
                                /* Copy key into page. */
                                memcpy(nodebuf[i] + BT_KEYS_OFF +
                                           keylength * curindex[i],
                                       keyp, keylength);

                                /* Store pointer to level below. */
                                stblkno(pageblkno[i - 1],
                                        nodebuf[i] + ISPAGESIZE -
                                            (curindex[i] + 1) * BLKNOSIZE);

                                stshort((short)curindex[i] + 1,
                                        nodebuf[i] + BT_NKEYS_OFF);
                                if (curindex[i]++ > 0) {
                                        break;
                                }
                        }
...
Static int _duplicate_exist(srt, keylength) Issort *srt;
int keylength;
{
        char *curkey;
        char *lastkey = (char *)0;
