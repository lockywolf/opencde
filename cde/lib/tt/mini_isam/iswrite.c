...
int iswrite(isfd, record) int isfd;
char *record;
{
        int _am_write();
        register Fab *fab;
...
Static int _am_write(fab, record, reclen, curpos, recnum) register Fab *fab;
char *record;
int reclen;
Bytearray *curpos;
Recno *recnum;
{
        return (_amwrite(&fab->isfhandle, record, reclen, curpos, recnum,
                         &fab->errcode));
}
