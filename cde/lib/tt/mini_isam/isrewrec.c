...
int isrewrec(isfd, recnum, record) int isfd;
long recnum;
char *record;
{
        register Fab *fab;
        ...
        if ((ret = _amrewrec(&fab->isfhandle, record, reclen, recnum,
                             &fab->errcode)) == ISOK) {
...
static int _amrewrec(isfhandle, record, reclen, recnum,
...
static int _changekeys(fcb, record, oldrecord, recnum) Fcb *fcb;
char *record;
char *oldrecord;
Recno recnum;
{
        ...
        for (i = 0; i < nkeys; i++) {
                if ((err = _change1key(fcb, fcb->keys + i, record, oldrecord,
                                       recnum, (char *)NULL)) != ISOK)
                        return (err);
        }
