...
isrepair(isfname, verbose) char *isfname;
int verbose;
{
        ...
        /*
         *  Check for maxreclen field value of -1. This could have occured due
         * to ISMAXRECLEN being incorrectly set to 65535 in an earlier version.
         * If this field is -1 and it's a variable length record, reset to the
         * new value of MAXRECLEN. This means that this field will be repaired
         * when the control block is written back to disk.
         */
        ...
        isreclen = minreclen;
        if ((isfd = isbuild(namebuf, maxreclen, nokey,
                            ISINOUT + ISEXCLLOCK + (varflag ? ISVARLEN : 0))) ==
            ISERROR) {
        ...
        while (recnum <= lastrecno && offset < recfile_end - minreclen) {
                ...
                if (varflag) {
                        prec = rp_readrecord_v(datfd, varfd, offset, minreclen,
                                               maxreclen);
        ...
        if (diff == 0)
                ...
                (void)print("%d records found - %d records %s than in header\n",
                            nrecords_found, diff, diff > 0 ? "more" : "less");
...
Static rp_readcntlpg(datfd, cntlpg) int datfd;
char *cntlpg;
{
...
Static char *rp_readrecord_f(datfd, offset, reclen) int datfd;
long offset;
int reclen;
{
...
Static char *rp_readrecord_v(datfd, varfd, offset, minreclen,
                             maxreclen) int datfd,
    varfd;
long offset;
int minreclen, maxreclen;
{
        ...
        if (read(datfd, recordbuffer, minreclen + LONGSIZE) !=
            (minreclen + LONGSIZE))
                return ((char *)NULL);
        ...
        if (tailoff != VL_RECNOTAIL) {
                ...
                if (read(varfd, recordbuffer + LONGSIZE + isreclen, taillen) !=
                    taillen)
                        goto OKEXIT;
...
static int isnoprintf(const char *s, ...) { return (0); }

static int printkey(int n, struct keydesc *pkdesc,
                    int (*print)(const char *, ...)) {
        int i;
        struct keypart *pk;
        ...
        if (n == 1)
                print("P%3d: %s ", n,
                      (pkdesc->k_flags & ISDUPS) ? "DUPS  " : "NODUPS");
        else
                print(" %3d: %s ", n,
                      (pkdesc->k_flags & ISDUPS) ? "DUPS  " : "NODUPS");
...
static void cmd_error(const char *str, int (*print)(const char *, ...)) {
        (void)print("%s: ISAM error %d\n", str, iserrno);
}

static int typeletter(type) int type;
{
        switch (type) {
        case INTTYPE:
                return 'I';
