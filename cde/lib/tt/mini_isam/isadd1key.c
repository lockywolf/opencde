...
int _add1key(fcb, pkeydesc2, record, recnum, newkey) Fcb *fcb;
Keydesc2 *pkeydesc2;
char *record;
Recno recnum;
char *newkey;
{
        ...
        if (ALLOWS_DUPS2(pkeydesc2)) {
                 ...
                 * Try to read the last duplicate (with the highest serial
                 * number). If such duplicate exists, the inserted key entry
                 * will be assigned next higher value for duplicate serial
                 * number. If there is no such duplicate, assign duplicate
                 * serial number 1.
                 */
                ...
                if ((pkey = _isbtree_current(btree)) != NULL &&
                    memcmp(pkey + RECNOSIZE + DUPIDSIZE,
                        ...
                        /*
                         * A duplicate exists. Assign +1 value for the new
                         * serial number.
                         */
