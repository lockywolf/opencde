...
Blkno _isindfreel_alloc(fcb) Fcb *fcb;
{
        Bufhdr *pbhdr;
        char *p;
        int npointers;
...
void _isindfreel_free(fcb, blkno) Fcb *fcb;
Blkno blkno;
{
        Bufhdr *pbhdr;
        char *p;
        ...
        if (fcb->indfreelist != FREELIST_NOPAGE) {
                pbhdr =
                    _isdisk_fix(fcb, fcb->indfd, fcb->indfreelist, ISFIXWRITE);
                p = pbhdr->isb_buffer;
                ...
                if (npointers < FL_MAXNPOINTERS) {
                        stblkno(blkno,
                                p + FL_POINTERS_OFF + npointers * BLKNOSIZE);
                        npointers++;
                        stshort((short)npointers, p + FL_NPOINTERS_OFF);
