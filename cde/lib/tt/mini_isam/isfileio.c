...
void _cp_tofile(fcb, unixfd, data, pos, len) Fcb *fcb;
int unixfd;
char *data;
long pos;
int len;
{
        ...
        while (len > 0) {
                ...
                nbytes =
                    (len < ISPAGESIZE - offset) ? len : ISPAGESIZE - offset;
...
void _cp_fromfile(fcb, unixfd, data, pos, len) Fcb *fcb;
int unixfd;
char *data;
long pos;
int len;
...
Blkno _extend_file(fcb, unixfd, oldsize) Fcb *fcb;
int unixfd;
Blkno oldsize;
{
        Bufhdr *bufhdr;
...
Static int _getoffset(pos) long pos;
...
Static Blkno _getblkno(pos) long pos;
{ return ((int)(pos / ISPAGESIZE)); }
