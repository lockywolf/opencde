...
void _isbtree_insert(Btree *btree, char *key) {
        ...
        for (i = btree->depth - 1; i >= 0; i--) {
                ...
                if (nkeys == capac) {
                         ...
                         * Allocate new block, move half of the entries into the
                         * new block, and insert the key into the appropriate
                         * block. If this block is not root, the loop will
                         * iterate to update upper levels.
                         */

                        /* Get new page. */
                        kp2bhdr =
                            _allockpage(btree->fcb, capac, level, &blkno2);
                        ...
                        if (btree->curpos[i] > halfcapac - 2) {
                                ...
                                insert_key(btree, pkp2,
                                           btree->curpos[i] - halfcapac, keybuf,
                                           blkno);
                        } else {
                                /* Insert entry into left block. */
                                insert_key(btree, pkp, btree->curpos[i], keybuf,
                                           blkno);

                                if (btree->curpos[i] ==
                                    -1) /* Leftmost entry inserted - */
                                        leftkey_up(
                                            btree,
                                            i); /* propagate to upper levels. */
                        }

                        /* Set variables for next loop iteration. */
                        memcpy(keybuf, pkp2 + BT_KEYS_OFF,
                               keylength); /* Leftmost key */
                        blkno = blkno2;
                         ...
                         * the block. If the inserted key becomes the leftmost
                         * entry in the block,  update upper levels.
                         */
...
void leftkey_up(Btree *btree, int level) {
        ...
        while (--level >= 0) {
                ...
                btree->bufhdr[level] =
                    _isdisk_refix(btree->bufhdr[level], ISFIXWRITE);
                pkp = btree->bufhdr[level]->isb_buffer;

                memcpy(pkp + BT_KEYS_OFF + (btree->curpos[level] * keylength),
                       key, keylength);
...
static void insert_key(Btree *btree, char *pkp, int pos, char *key,
                       Blkno blkno) {
        int keylength = btree->keydesc2->k2_len;
...
static void splitblock(Btree *btree, char *fullpage, char *newpage, int pos) {
        int keylength = btree->keydesc2->k2_len;
