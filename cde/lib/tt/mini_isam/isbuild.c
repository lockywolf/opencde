...
int isbuild(isfname, recordlength, primkey, mode) char *isfname;
int recordlength;
...
int mode;
{
        ...
        /* Create a Fab object. */
        fab =
            _fab_new(isfname, openmode, (Bool)((mode & ISLENMODE) == ISVARLEN),
                     minreclen, recordlength);
        if (fab == NULL) {
...
/* ARGSUSED */
static int _ambuild(isfname, openmode, varflag, minlen, maxlen, primkey, owner,
                    group, umask, isfhandle, curpos, errcode) char *isfname;
enum openmode openmode;
Bool varflag;
int minlen, maxlen;
struct keydesc *primkey;
...
int umask;
{
         ...
         */
        if ((fcb = _isfcb_create(isfname, 1, (primkey->k_nparts != 0),
                                 (int)varflag, owner, group, umask, errcode)) ==
            NULL) {
                goto ERROR;
        }
        ...
        if (FCB_NOPRIMARY_KEY(fcb)) {
                ...
                crp = (Crp *)_ismalloc(
                    (unsigned)(sizeof(*crp) + fcb->keys[0].k2_len));
                ...
                /*
                 * Set full key length as the number of bytes to match in key
                 * comparison
                 */
