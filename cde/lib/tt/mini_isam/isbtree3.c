...
void _isbtree_remove(Btree *btree) {
        ...
        _iskeycmp_set(pkeydesc2,
                      pkeydesc2->k2_nparts + 1); /* +1 for recno field */
        ...
        for (i = btree->depth - 1; i >= 0; i--) {
                ...
                nkeys =
                    ldshort(pkp + BT_NKEYS_OFF); /* Number of keys in block */
                ...
                if (nkeys >= halfcapac || i == 0) {
                        ...
                        if (btree->curpos[i - 1] > 0) {
                                ...
                                /*
                                 * Block pkp is not the leftmost descendent of
                                 * its parent. Either merge with left brother,
                                 * or move a few entries from it.
                                 */

                                pkp3 = btree->bufhdr[i - 1]->isb_buffer;

                                blkno2 =
                                    ldblkno(pkp3 + ISPAGESIZE -
                                            (btree->curpos[i - 1]) * BLKNOSIZE);

                                /* Fix the left brother. */
                                kp2bhdr =
                                    _isdisk_fix(btree->fcb, btree->fcb->indfd,
                                                blkno2, ISFIXWRITE);
                                pkp2 = kp2bhdr->isb_buffer;

                                if (ldshort(pkp2 + BT_NKEYS_OFF) + nkeys >
                                    capac) {

                                        /*
                                         * Current page cannot be merged with
                                         * the left brother. Move some entries
                                         * from the left brother into current.
                                         */

                                        move_keys =
                                            (ldshort(pkp2 + BT_NKEYS_OFF) -
                                             halfcapac + 1) /
                                            2;
                                        assert(move_keys > 0);

                                        move_from_left(btree, pkp2, pkp,
                                                       move_keys);

                                        leftkey_up(
                                            btree,
                                            i); /* Propagate new leftmost key */

                                        break; /* From main loop */
                                } else {

                                        /*
                                         * Current page and left brotehr will be
                                         * merged. Move all entries from current
                                         * into left, and free current.
                                         */

                                        /* Move all entries from current into
                                         * left. */
                                        move_from_right(btree, pkp2, pkp,
                                                        nkeys);

                                        /* Free current page. */
                                        _isindfreel_free(
                                            btree->fcb,
                                            btree->bufhdr[i]->isb_blkno);

                                        btree->bufhdr[i] =
                                            kp2bhdr; /* Update search path */

                                        /* The loop must iterate to delete entry
                                         * in parent block. */
                                }
                        } else {

                                /*
                                 * Block pkp is the leftmost descendent of its
                                 * parent. Either merge with right brother, or
                                 * move a few entries from it.
                                 */
                                ...
                                if (ldshort(pkp2 + BT_NKEYS_OFF) + nkeys >
                                    capac) {
                                        ...
                                        /*
                                         * Current page cannot be merged with
                                         * the right brother. Move some entries
                                         * from the right brother into current.
                                         */
                                        ...
                                        move_from_right(btree, pkp, pkp2,
                                                        move_keys);

                                        /* Update search path to go through
                                         * right brother. */
                                        btree->curpos[i - 1]++;
                                        ...
                                        /*
                                         * Current page and right brother will
                                         * be merged. Move all entries from
                                         * right into current, and free right.
                                         */

                                        /* Move all entries from right brother
                                         * into current. */
                                        move_from_right(
                                            btree, pkp, pkp2,
                                            ldshort(pkp2 + BT_NKEYS_OFF));
                                        ...
                                        _isindfreel_free(btree->fcb,
                                                         kp2bhdr->isb_blkno);

                                        /* Update search path to point at right
                                         * brother*/
                                        btree->curpos[i - 1]++;
                                        /* The loop must iterate to delete entry
                                         * in parent block. */
        ...
        if (btree->depth > 1) {
                ...
                if (ldshort(pkp + BT_NKEYS_OFF) < 2) {
                        ...
                        /*
                         * Root now has only 1 entry and it is not the sole
                         * block. Replace root with its only descendant, don't
                         * change root blkno.
                         */
...
static void remove_entry(Btree *btree, char *pkp, int pos) {
...
static void move_from_right(Btree *btree, char *l, char *r, int move_keys) {
        int keylength = btree->keydesc2->k2_len;
...
static void move_from_left(Btree *btree, char *l, char *r, int move_keys) {
        int keylength = btree->keydesc2->k2_len;
        int lnkeys = ldshort(l + BT_NKEYS_OFF);
        int rnkeys = ldshort(r + BT_NKEYS_OFF);
        int level = ldshort(r + BT_LEVEL_OFF);
        ...
        /* Move move_keys from l into r block. */
        memcpy(r + BT_KEYS_OFF,
               l + BT_KEYS_OFF + (lnkeys - move_keys) * keylength,
               move_keys * keylength);
        ...
        if (level > 0) {
                ...
                memcpy(r + ISPAGESIZE - (rnkeys + move_keys) * BLKNOSIZE,
                       r + ISPAGESIZE - rnkeys * BLKNOSIZE, rnkeys * BLKNOSIZE);
