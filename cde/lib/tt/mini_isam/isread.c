...
int isread(isfd, record, mode) int isfd;
char *record;
int mode;
{
        register Fab *fab;
        ...
        if ((ret = _amread(&fab->isfhandle, record, &reclen, readmode,
                           &fab->curpos, &recnum, &fab->errcode)) == ISOK) {
...
static int _amread(isfhandle, record, reclen, readmode, curpos, recnum,
...
struct errcode *errcode;
{
        ...
        if (crp->keyid == PHYS_ORDER) {
                ...
                case RM_EQUAL:
                        ...
                        if ((err = rec_read(fcb, record, recnum2, reclen)) !=
                            ISOK) {
                ...
                case RM_GREAT:
                         ...
                         */
                        while ((err = rec_read(fcb, record, recnum2, reclen)) !=
                                   ISOK &&
                               err == ENOREC)
                                recnum2++;
                ...
                *recnum = recnum2;
                ...
                case RM_GTEQ:
                        ...
                        memcpy((void *)crp->key, (const void *)pkey,
                               pkeydesc2->k2_len);
                ...
                case RM_CURR:
                        ...
                        case CRP_AFTER:
                                /*
                                 * We have check if the record has not been
                                 * deleted since the current record position was
                                 * set up.
                                 */
                ...
                case RM_PREV:
                        ...
                        case CRP_BEFORE:
                                /*
                                 * To get to the previous record, we must
                                 * decrement the TID part for unique keys, or
                                 * duplicate serial number for non-unique keys.
                                 */
                                memcpy((void *)keybuf1, (const void *)crp->key,
                                       pkeydesc2->k2_len);
                                if (ALLOWS_DUPS2(pkeydesc2)) {
                                        stlong(ldlong(keybuf1 + KEY_DUPS_OFF) -
                                                   1,
                                               keybuf1 + KEY_DUPS_OFF);
                                } else {
                                        strecno(
                                            ldrecno(keybuf1 + KEY_RECNO_OFF) -
                                                1,
