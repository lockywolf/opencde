...
Fcb *_isfcb_create(isfname, crdat, crind, crvar, owner, group, u_mask,
                   errcode) char *isfname;
...
void _isfcb_setreclength(fcb, varflag, minreclen, maxreclen) register Fcb *fcb;
Bool varflag;
...
Fcb *_isfcb_open(isfname, errcode) char *isfname;
struct errcode *errcode;
{
        register Fcb *fcb;
        int dat_fd = -1;
         ...
         */
        if ((dat_fd = _open_datfile(isfname, &rdonly)) == -1 ||
            errno == EMFILE) {
                _amseterrcode(errcode, errno);
                return (NULL);
        }
...
int _isfcb_nfds(fcb) register Fcb *fcb;
{
...
void _isfcb_remove(fcb) register Fcb *fcb;
{
        if (fcb->datfd)
...
void _isfcb_close(fcb) register Fcb *fcb;
{
        assert(fcb != NULL);
...
int _isfcb_cntlpg_w(fcb) register Fcb *fcb;
{
...
int _isfcb_cntlpg_w2(fcb) register Fcb *fcb;
{
...
int _isfcb_cntlpg_r(fcb) register Fcb *fcb;
{
        char cntl_page[ISCNTLSIZE];
        int dat_fd = fcb->datfd;
        int i;
        ...
        fcb->keys = (Keydesc2 *)_isrealloc(
            (char *)fcb->keys, (unsigned)(sizeof(Keydesc2) * fcb->nkeys));
...
int _isfcb_cntlpg_r2(fcb) register Fcb *fcb;
{
        char cntl_page[CP_VAREND_OFF + CP_VAREND_LEN];
        int dat_fd = fcb->datfd;
        ...
        if (ldlong(cntl_page + CP_CHANGESTAMP1_OFF) != fcb->changestamp1) {
                 ...
                 * We cannot test changestamp2; we must read the info into the
                 *FCB always.
                 */
...
Static int _create_datfile(isfname) char *isfname;
{
        int fd;
...
Static int _create_indfile(isfname) char *isfname;
{
        int fd;
        char namebuf[MAXPATHLEN];
...
Static int _create_varfile(isfname) char *isfname;
{
        int fd;
        char namebuf[MAXPATHLEN];
...
Static void _remove_datfile(isfname) char *isfname;
{
        char namebuf[MAXPATHLEN];
...
Static void _remove_indfile(isfname) char *isfname;
{
        char namebuf[MAXPATHLEN];
...
Static void _remove_varfile(isfname) char *isfname;
{
        char namebuf[MAXPATHLEN];
...
Static int _open_datfile(isfname, rdonly) char *isfname;
Bool *rdonly;
{
        char namebuf[MAXPATHLEN];
        int ret;
...
Static int _open_indfile(isfname, rdonly) char *isfname;
Bool rdonly;
{
        int fd;
        char namebuf[MAXPATHLEN];
...
Static int _open_varfile(isfname, rdonly) char *isfname;
Bool rdonly;
{
        int fd;
        char namebuf[MAXPATHLEN];
...
int _check_isam_magic(fcb) Fcb *fcb;
{
...
int _open2_indfile(fcb) Fcb *fcb;
{
        char namebuf[MAXPATHLEN];
        struct stat buf;
        int openmode;
