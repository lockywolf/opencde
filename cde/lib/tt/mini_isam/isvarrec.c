...
/*ARGSUSED*/
int _vlrec_write(fcb, record, recnum, reclen) register Fcb *fcb;
char *record;
Recno *recnum;
int reclen;
{
        ...
        if (fcb->freerecno != NULL_RECNO) {
                 ...
                 */
                rec_position =
                    _vl_getpos(fcb, recnum2); /* Offset in .rec file */
                _cp_fromfile(fcb, fcb->datfd, recnobuf, rec_position + LONGSIZE,
                             RECNOSIZE);
                 ...
                 */
                while (_vl_getpos(fcb, recnum2 + 1) >
                       fcb->datsize * ISPAGESIZE) {
                        fcb->datsize =
                            _extend_file(fcb, fcb->datfd, fcb->datsize);
                }
                rec_position =
                    _vl_getpos(fcb, recnum2); /* Offset in .rec file */
        }
        ...
        _cp_tofile(fcb, fcb->datfd, record, rec_position + LONGSIZE,
                   fcb->minreclen);
...
int _vlrec_read(fcb, record, recnum, reclen) register Fcb *fcb;
char *record;
Recno recnum;
int *reclen;
{
         ...
         */
        _cp_fromfile(fcb, fcb->datfd, record, rec_position + LONGSIZE,
                     fcb->minreclen);
        ...
        *reclen += _istail_read(fcb, tailoff, record + fcb->minreclen);
        ...
        if (*reclen > fcb->maxreclen)
                _isfatal_error(
                    "Corrupted file: too long variable length record");
...
long _vl_getpos(fcb, recnum) Fcb *fcb;
Recno recnum;
...
/*ARGSUSED*/
int _vlrec_rewrite(fcb, record, recnum, reclen) register Fcb *fcb;
char *record;
Recno recnum;
int reclen;
{
...
int _vlrec_delete(fcb, recnum) register Fcb *fcb;
Recno recnum;
{
        long rec_position;
        long tailoff;
        ...
        _cp_tofile(fcb, fcb->datfd, recnobuf, rec_position + LONGSIZE,
                   RECNOSIZE);
...
/*ARGSUSED*/
int _vlrec_wrrec(fcb, record, recnum, reclen) register Fcb *fcb;
char *record;
Recno recnum;
int reclen;
{
        ...
        if (recnum > fcb->lastrecno) {
                ...
                /*
                 * If the recnum is bigger than the highest record number in the
                 * .rec file, extend the .rec file.
                 */
                while (_vl_getpos(fcb, recnum + 1) >
                       fcb->datsize * ISPAGESIZE) {
                        fcb->datsize =
                            _extend_file(fcb, fcb->datfd, fcb->datsize);
                ...
                tailoff = VL_RECDELETED;
                stlong(tailoff, tailoffbuf);
                for (recnum2 = fcb->lastrecno + 1; recnum2 <= recnum;
                     recnum2++) {
                        _cp_tofile(fcb, fcb->datfd, tailoffbuf,
                                   _vl_getpos(fcb, recnum2), LONGSIZE);
...
static void remove_from_chain2(fcb, recnum) Fcb *fcb;
Recno recnum;
{
        ...
        if (fcb->freerecno == recnum) {
                ...
                do {
                        ...
                        _cp_fromfile(fcb, fcb->datfd, recnobuf2,
                                     pos2 + LONGSIZE, RECNOSIZE);
                        recnum2 = ldrecno(recnobuf2);
                } while (recnum2 != recnum && recnum2 != NULL_RECNO);

                _cp_tofile(fcb, fcb->datfd, recnobuf1, pos2 + LONGSIZE,
                           RECNOSIZE);
        }
}
...
long _istail_insert(fcb, tailp, taillen) Fcb *fcb;
char *tailp;
int taillen;
{
...
/* ARGSUSED */
Static void _istail_delete(fcb, offset) Fcb *fcb;
long offset;
{
        /*
...
Static _istail_read(fcb, offset, buffer) Fcb *fcb;
long offset;
char *buffer;
{
        char frameheadbuf[2 * SHORTSIZE];
...
int _istail_modify(fcb, offset, tailp, taillen) Fcb *fcb;
long offset;
int taillen;
char *tailp;
{
        ...
        if (offset != VL_RECNOTAIL) {
                 ...
                 * Read frame header.
                 */
                _cp_fromfile(fcb, fcb->varfd, frameheadbuf, offset,
                             2 * SHORTSIZE);
        ...
        if (taillen > framelen) {
                 ...
                 */
                stshort((short)taillen, frameheadbuf + VR_TAILLEN_OFF);
                _cp_tofile(fcb, fcb->varfd, frameheadbuf, offset,
                           2 * SHORTSIZE);
                _cp_tofile(fcb, fcb->varfd, tailp, offset + 2 * SHORTSIZE,
                           taillen);
