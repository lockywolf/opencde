...
Issort *_issort_create(reclen, nrecs, compfunc) int reclen;
int nrecs;
int (*compfunc)();
{
        Issort *p;
        ...
        p->ist_array =
            _ismalloc((unsigned)(reclen * nrecs)); /* Allocate array */
                                                   /* for storing records */
...
void _issort_destroy(srt) Issort *srt;
{
...
void _issort_insert(srt, record) register Issort *srt;
char *record;
{
...
void _issort_sort(srt) Issort *srt;
{
        if (srt->ist_nrecs > 1)
                qsort(srt->ist_array, srt->ist_nrecs, srt->ist_reclength,
                      srt->ist_compf);
...
void _issort_rewind(srt) Issort *srt;
{ srt->ist_currec = 0; }
...
char *_issort_read(srt) register Issort *srt;
{
