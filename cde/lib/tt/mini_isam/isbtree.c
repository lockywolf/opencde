...
Btree *_isbtree_create(fcb, pkeydesc2) Fcb *fcb;
Keydesc2 *pkeydesc2;
{
        register Btree *p;
...
void _isbtree_destroy(btree) register Btree *btree;
{
        register int i;
...
void _isbtree_search(btree, key) register Btree *btree;
char *key; /* Search for this key */
{
        ...
        blkno = rootblkno;
        do {
                btree->bufhdr[index] = _isdisk_fix(
                    btree->fcb, btree->fcb->indfd, blkno, ISFIXREAD);
                p = btree->bufhdr[index]->isb_buffer; /* pointer to buffer */

                /* Load some fields from the key page. */
                nkeys =
                    ldshort(p + BT_NKEYS_OFF); /* Number of keys in the page */
                elevation = ldshort(p + BT_LEVEL_OFF); /* Level of the page */

                /* Binary search in the key page to find equal or next lowere
                 * key. */
                key2 = _isbsearch(key, p + BT_KEYS_OFF, nkeys, keylength,
                                  _iskeycmp);
                ...
                if (elevation > 0)
                        blkno =
                            ldblkno(p + ISPAGESIZE - (curpos + 1) * BLKNOSIZE);
...
char *_isbtree_current(btree) register Btree *btree;
{
        int curpos;
...
char *_isbtree_next(btree) register Btree *btree;
{
        int curpos;
        ...
        /*
         * Move up along the path, find first block where we can move to the
         * right.
         */
        ...
        while (++level < depth) {
                ...
                btree->bufhdr[level] = _isdisk_fix(
                    btree->fcb, btree->fcb->indfd, blkno, ISFIXREAD);
                p = btree->bufhdr[level]->isb_buffer;
