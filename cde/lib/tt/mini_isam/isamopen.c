...
/* ARGSUSED */
int _amopen(isfname, openmode, varflag, minlen, maxlen, isfhandle, curpos,
...
struct errcode *errcode;
{
         ...
         */
        if (fcb->rdonly == TRUE &&
            (openmode == OM_INOUT || openmode == OM_OUTPUT)) {
...
Fcb *_openfcb(isfhandle, errcode) Bytearray *isfhandle;
struct errcode *errcode;
{
        Fcb *fcb;
...
out:
        ...
        if (fcb->varflag == TRUE && fcb->varfd == -1) {
                ...
                if (strlen(fmt) + strlen(fcb->isfname) + 1 >= BUFSIZ)
                        errmsg0 = (char *)malloc(strlen(fmt) +
                                                 strlen(fcb->isfname) + 1);
                else
                        errmsg0 = errbuf;
...
char *_getisfname(isfhandle) Bytearray *isfhandle;
...
Bytearray _makeisfhandle(isfname) char *isfname;
