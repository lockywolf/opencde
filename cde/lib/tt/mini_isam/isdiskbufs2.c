...
Bufhdr *_isdisk_fix(fcb, unixfd, blkno, mode) Fcb *fcb;
...
int mode;
{
        ...
        if ((p = _findblock(fcb, unixfd, blkno)) == NULL) {
                ...
                if (mode != ISFIXNOREAD) {
                        ...
                        _isdln_insert(hashl,
                                      &p->isb_hash); /* Insert into hash list */
        ...
        if (mode == ISFIXREAD) {
                ...
                /* If buffer is already fixed for write, no other actions are
                 * necces. */
                if (p && p->isb_flags & ISB_FIXED) {
                        assert((p->isb_flags & ISB_RFIXED) ==
                               0); /* Buffer cannot be */
                ...
                _isdln_insert(pfixl,
                              &p2->isb_flist); /* Insert into pfixl list */

                p2->isb_fcb = fcb;
                p2->isb_unixfd = unixfd;
                p2->isb_blkno = blkno;
                p2->isb_flags =
                    ISB_READ | ISB_WFIXED; /* Mark buffer as dirty */
...
void _isdisk_unfix(p) register Bufhdr *p;
{
        if (!(p->isb_flags & ISB_FIXED))
        ...
        if (p->isb_flags & ISB_CHANGE)
                _isdln_append(pchangl,
                              &p->isb_aclist); /* Append to pchangl list */
        else {
                _isdln_append(pavail,
                              &p->isb_aclist); /* Append to pavail list */
                availn++;
        }
}

void _isdisk_commit1(p) Bufhdr *p;
{ _commit1buffer(p); }
...
Bufhdr *_isdisk_refix(p, newmode) Bufhdr *p;
int newmode;
{
        ...
        for (p = bufhdrs, i = 0; i < ISMAXBUFFERS; p++, i++)
                if (p->isb_flags != ISB_NODATA)
                        (void)printf("%3d: %3d  %6d   %2x     %3d\n", i,
                                     _isfd_getisfd(p->isb_pisfd), p->isb_blkno,
                                     p->isb_flags,
                                     p->isb_oldcopy ? (p->isb_oldcopy - bufhdrs)
                                                    : -1);
}

aclistdump(lh) struct dlink *lh;
{
        register Bufhdr *p;
        struct dlink *e;
...
Static Bufhdr *_findblock(fcb, unixfd, blkno) Fcb *fcb;
int unixfd;
Blkno blkno;
{
        register Bufhdr *p;
        ...
        for (e = _isdln_first(lh); e != lh; e = _isdln_next(e)) {
                ...
                if (p->isb_blkno == blkno && p->isb_fcb == fcb &&
                    p->isb_unixfd == unixfd) {
...
Static void _commit1buffer(p) register Bufhdr *p;
{
...
Static void _rollback1buffer(p) register Bufhdr *p;
{
        register Bufhdr *p2;
        ...
        if ((p2 = p->isb_oldcopy) != NULL) {
                ...
                _isdln_append(&p->isb_hash,
                              &p2->isb_hash); /* Insert into hash list */
        }
...
Static void _makenodata(p) register Bufhdr *p;
{
...
Static void _flush1buffer(p) register Bufhdr *p;
{
        assert(p->isb_flags & ISB_CHANGE);
