...
/*ARGSUSED*/
int _flrec_write(fcb, record, recnum, reclen) register Fcb *fcb;
char *record;
Recno *recnum;
int reclen;
{
        ...
        if (fcb->freerecno != NULL_RECNO) {
                 ...
                 */
                rec_position =
                    _fl_getpos(fcb, recnum2); /* Offset in .rec file */
                _cp_fromfile(fcb, fcb->datfd, recnobuf, rec_position + 1,
                             RECNOSIZE);
                 ...
                 */
                while (_fl_getpos(fcb, recnum2 + 1) >
...
int _flrec_read(fcb, record, recnum, reclen) register Fcb *fcb;
char *record;
Recno recnum;
int *reclen;
{
...
long _fl_getpos(fcb, recnum) Fcb *fcb;
Recno recnum;
...
/*ARGSUSED*/
int _flrec_rewrite(fcb, record, recnum, reclen) register Fcb *fcb;
char *record;
Recno recnum;
int reclen;
{
...
int _flrec_delete(fcb, recnum) register Fcb *fcb;
Recno recnum;
{
        long rec_position;
        char delflag;
...
/*ARGSUSED*/
int _flrec_wrrec(fcb, record, recnum, reclen) register Fcb *fcb;
char *record;
Recno recnum;
int reclen;
{
        ...
        if (recnum > fcb->lastrecno) {
                ...
                for (recnum2 = fcb->lastrecno + 1; recnum2 <= recnum;
                     recnum2++) {
                        _cp_tofile(fcb, fcb->datfd, &delflag,
                                   _fl_getpos(fcb, recnum2), 1);
...
static void remove_from_chain(fcb, recnum) Fcb *fcb;
Recno recnum;
{
        ...
        if (fcb->freerecno == recnum) {
                ...
                do {
                        ...
                        _cp_fromfile(fcb, fcb->datfd, recnobuf2, pos2 + 1,
                                     RECNOSIZE);
