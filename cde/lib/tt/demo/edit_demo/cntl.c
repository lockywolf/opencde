...
int is_window_showing(widget) Widget widget;
{ return (XtIsManaged(widget)); }

void get_screen_size(widget, width, height) Widget widget;
int *width, *height;
{
...
void force_popup_on_screen(popup, px, py) Widget popup;
int *px, *py;
{
        ...
        left = (Position)x;
        top = (Position)y;
        XtVaSetValues(popup, XmNx, left, XmNy, top, 0);

        *px = x;
        *py = y;
}

void position_popup(base, popup) Widget base, popup;
{
        int bw, bx, by, px, py;
        ...
        XtVaGetValues(base, XmNx, &base_x, XmNy, &base_y, XmNwidth, &base_width,
                      XmNheight, &base_height, 0);
        bx = (int)base_x;
        by = (int)base_y;
        bw = (int)base_width;

        XtVaGetValues(popup, XmNx, &popup_x, XmNy, &popup_y, XmNwidth,
                      &popup_width, XmNheight, &popup_height, 0);
...
void show_popup(widget) Widget widget;
{
...
void dismiss_popup(widget, client_data, cbs) Widget widget;
...
void add_delete_callback(widget) Widget widget;
{
        Atom WM_DELETE_WINDOW;

        WM_DELETE_WINDOW =
            XmInternAtom(XtDisplay(widget), "WM_DELETE_WINDOW", False);
        XmAddWMProtocolCallback(widget, WM_DELETE_WINDOW,
                                (XtCallbackProc)dismiss_popup, NULL);
}

void write_footer(message) char *message;
{
        XmString label;
 ...
 */
void cntl_receive_tt_message(client_data, fid, id) XtPointer client_data;
int *fid;
XtInputId *id;
{
        Tt_message msg;
 ...
 */
Tt_callback_action cntl_update_obj_panel_callback(m, p) Tt_message m;
Tt_pattern p;
{
        int mark;
 ...
 */
Tt_callback_action cntl_msg_callback(m, p) Tt_message m;
Tt_pattern p;
{
        int mark;
        char msg[255];
        ...
        case TT_FAILED:
                ...
                if (tt_pointer_error(errstr) == TT_OK && errstr) {
                        ...
                        sprintf(msg, "%s failed: %s", tt_message_op(m),
                                tt_status_message(tt_message_status(m)));
                }
                write_footer(msg);
                break;
 ...
 */
void cntl_close(file) char *file;
{
        Tt_message msg;
 ...
 * under a new name.
 */
void cntl_save_as(file, dir, newfile) char *file;
char *dir;
char *newfile;
{
        Tt_message msg;
 ...
 */
void cntl_ui_edit(widget, client_data, call_data) Widget widget;
 ...
 */
void cntl_ui_save(widget, client_data, call_data) Widget widget;
XtPointer client_data, call_data;
{
        write_footer("");
 ...
 */
cntl_ui_save_as(widget, client_data, call_data) Widget widget;
XtPointer client_data, call_data;
{
        write_footer("");
 ...
 */
cntl_ui_close(widget, client_data, call_data) Widget widget;
XtPointer client_data, call_data;
{
        write_footer("");
 ...
 */
void cntl_ui_save_as_button_handler(widget, client_data,
 ...
 */
Tt_filter_action cntl_gather_specs(objid, list_count, acc) char *objid;
void *list_count;
void *acc;
{
...
int cntl_update_obj_panel() {
        ...
        if (tt_file_objects_query(file, (Tt_filter_function)cntl_gather_specs,
 ...
 */
void cntl_ui_file_objects(widget, client_data, call_data) Widget widget;
XtPointer client_data, call_data;
{
        char *string;
 ...
 */
void cntl_ui_olist_handler(widget, client_data, cbs) Widget widget;
XtPointer client_data;
XmListCallbackStruct *cbs;
{
        char *string;

        string = XmStringUnparse(cbs->item, NULL, XmCHARSET_TEXT,
 ...
 */
void cntl_ui_hilite_button_handler(widget, client_data,
                                   call_data) Widget widget;
XtPointer client_data, call_data;
{
        Tt_message msg;
...
void cntl_ui_initialize() {
        ...
        cntl_ui_base_window = XtAppCreateShell(
            0, "base_frame", applicationShellWidgetClass, dpy, 0, 0);
        XtVaSetValues(cntl_ui_base_window, XmNwidth, 339, XmNheight, 100,
                      XmNtitle, "Sun_EditDemo_cntl", 0);

        cntl_ui_base_controls = XtVaCreateManagedWidget(
            "controls", xmFormWidgetClass, cntl_ui_base_window, 0);
        ...
        n++;
        cntl_ui_file_menu =
            (Widget)XmCreateMenuBar(cntl_ui_base_controls, "menubar", args, n);

        cntl_ui_file_button =
            (Widget)XmCreatePulldownMenu(cntl_ui_file_menu, "File", NULL, 0);
        label = XmStringCreateSimple("File");
        XtVaCreateManagedWidget("File", xmCascadeButtonWidgetClass,
                                cntl_ui_file_menu, XmNlabelString, label,
                                XmNmnemonic, 'F', XmNsubMenuId,
                                cntl_ui_file_button, 0);
        XmStringFree(label);
        but = XtVaCreateManagedWidget("Edit", xmPushButtonWidgetClass,
                                      cntl_ui_file_button, 0);
        XtAddCallback(but, XmNactivateCallback, (XtCallbackProc)cntl_ui_edit,
                      0);
        but = XtVaCreateManagedWidget("Save", xmPushButtonWidgetClass,
                                      cntl_ui_file_button, 0);
        XtAddCallback(but, XmNactivateCallback, (XtCallbackProc)cntl_ui_save,
                      0);
        but = XtVaCreateManagedWidget("Save as...", xmPushButtonWidgetClass,
                                      cntl_ui_file_button, 0);
        XtAddCallback(but, XmNactivateCallback, (XtCallbackProc)cntl_ui_save_as,
                      0);
        but = XtVaCreateManagedWidget(
            "File Objects...", xmPushButtonWidgetClass, cntl_ui_file_button, 0);
        XtAddCallback(but, XmNactivateCallback,
                      (XtCallbackProc)cntl_ui_file_objects, 0);
        but = XtVaCreateManagedWidget("Close", xmPushButtonWidgetClass,
                                      cntl_ui_file_button, 0);
        XtAddCallback(but, XmNactivateCallback, (XtCallbackProc)cntl_ui_close,
                      0);
        XtManageChild(cntl_ui_file_menu);

        cntl_ui_file_label = XtVaCreateManagedWidget(
            "File:", xmLabelWidgetClass, cntl_ui_base_controls,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, cntl_ui_file_menu,
            XmNtopOffset, 5, XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 5,
            0);
        label = XmStringCreateSimple("File:");
        XtVaSetValues(cntl_ui_file_label, XmNlabelString, label, 0);
        XmStringFree(label);

        cntl_ui_file_field = XtVaCreateManagedWidget(
            "text", xmTextWidgetClass, cntl_ui_base_controls, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, cntl_ui_file_menu, XmNtopOffset, 5,
            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
            cntl_ui_file_label, XmNleftOffset, 5, XmNrightAttachment,
            XmATTACH_FORM, XmNrightOffset, 5, 0);
        label = XmStringCreateSimple("");
        XtVaSetValues(cntl_ui_file_field, XmNlabelString, label, 0);
        XmStringFree(label);

        cntl_ui_message = XtVaCreateManagedWidget(
            "message", xmLabelWidgetClass, cntl_ui_base_controls,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, cntl_ui_file_field,
            XmNtopOffset, 10, XmNleftAttachment, XmATTACH_FORM, XmNleftOffset,
            5, XmNrightAttachment, XmATTACH_FORM, XmNrightOffset, 5,
            XmNbottomAttachment, XmATTACH_FORM, XmNbottomOffset, 5, 0);
        label = XmStringCreateSimple("");
        XtVaSetValues(cntl_ui_message, XmNlabelString, label, 0);
        XmStringFree(label);
...
void cntl_ui_saveas_popup_initialize() {
        ...
        n = 0;
        XtSetArg(args[n], XmNtitle, "Sun_EditMenu_cntl: Save As");
        n++;
        XtSetArg(args[n], XmNmappedWhenManaged, False);
        n++;
        XtSetArg(args[n], XmNdeleteResponse, XmDO_NOTHING);
        n++;
        cntl_ui_saveas_popup =
            XmCreateDialogShell(cntl_ui_base_window, "saveas", args, n);

        cntl_ui_saveas_controls = XtVaCreateManagedWidget(
            "controls", xmFormWidgetClass, cntl_ui_saveas_popup,
            XmNrubberPositioning, False, XmNresizePolicy, XmRESIZE_ANY,
            XmNautoUnmanage, False, XmNwidth, 350, XmNheight, 120, 0);

        label = XmStringCreateSimple("Directory:");
        cntl_ui_save_as_dlabel = XtVaCreateManagedWidget(
            "dlabel", xmLabelWidgetClass, cntl_ui_saveas_controls,
            XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 5, XmNleftAttachment,
            XmATTACH_FORM, XmNleftOffset, 5, XmNlabelString, label, 0);
        XmStringFree(label);

        cntl_ui_save_as_directory = XtVaCreateManagedWidget(
            "text", xmTextWidgetClass, cntl_ui_saveas_controls, XmNcolumns, 35,
            XmNtopAttachment, XmATTACH_FORM, XmNtopOffset, 5, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, cntl_ui_save_as_dlabel,
            XmNleftOffset, 5, XmNrightAttachment, XmATTACH_FORM, XmNrightOffset,
            5, 0);

        label = XmStringCreateSimple("Filename:");
        cntl_ui_new_flabel = XtVaCreateManagedWidget(
            "flabel", xmLabelWidgetClass, cntl_ui_saveas_controls,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
            cntl_ui_save_as_directory, XmNtopOffset, 5, XmNleftAttachment,
            XmATTACH_FORM, XmNleftOffset, 5, XmNlabelString, label, 0);
        XmStringFree(label);

        label = XmStringCreateSimple("");
        cntl_ui_new_filename = XtVaCreateManagedWidget(
            "text", xmTextWidgetClass, cntl_ui_saveas_controls, XmNcolumns, 35,
            XmNlabelString, label, XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, cntl_ui_save_as_directory, XmNtopOffset, 5,
            XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
            cntl_ui_save_as_dlabel, XmNleftOffset, 5, XmNrightAttachment,
            XmATTACH_FORM, XmNrightOffset, 5, 0);
        XmStringFree(label);

        label = XmStringCreateSimple("Save");
        cntl_ui_save_as_button = XtVaCreateManagedWidget(
            "button", xmPushButtonWidgetClass, cntl_ui_saveas_controls,
            XmNleftAttachment, XmATTACH_FORM, XmNleftOffset, 5,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
            cntl_ui_new_filename, XmNtopOffset, 5, XmNbottomAttachment,
            XmATTACH_FORM, XmNbottomOffset, 5, XmNlabelString, label, 0);
        XmStringFree(label);
        XtAddCallback(cntl_ui_save_as_button, XmNactivateCallback,
                      cntl_ui_save_as_button_handler, 0);
...
void cntl_ui_obj_popup_initialize() {
        ...
        n = 0;
        XtSetArg(args[n], XmNtitle, "Sun_EditMenu_cntl: File Objects");
        n++;
        XtSetArg(args[n], XmNmappedWhenManaged, False);
        n++;
        XtSetArg(args[n], XmNdeleteResponse, XmDO_NOTHING);
        n++;
        cntl_ui_obj_popup =
            XmCreateDialogShell(cntl_ui_base_window, "object", args, n);

        cntl_ui_obj_controls = XtVaCreateManagedWidget(
            "controls", xmFormWidgetClass, cntl_ui_obj_popup,
            XmNrubberPositioning, False, XmNresizePolicy, XmRESIZE_ANY,
            XmNautoUnmanage, False, XmNwidth, 350, XmNheight, 120, 0);
        ...
        XtSetArg(args[n], XmNtopOffset, 5);
        n++;
        XtSetArg(args[n], XmNvisibleItemCount, 4);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
        n++;
        XtSetArg(args[n], XmNleftWidget, cntl_ui_olist_label);
        n++;
        XtSetArg(args[n], XmNleftOffset, 5);
        n++;
        XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
        ...
        XtAddCallback(cntl_ui_olist, XmNsingleSelectionCallback,
                      cntl_ui_olist_handler, 0);
        XtManageChild(cntl_ui_olist);

        label = XmStringCreateSimple("Display");
        cntl_ui_hilite_button = XtVaCreateManagedWidget(
            "button", xmPushButtonWidgetClass, cntl_ui_obj_controls,
            XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, cntl_ui_olist,
            XmNtopOffset, 5, XmNbottomAttachment, XmATTACH_FORM,
            XmNbottomOffset, 5, XmNlabelString, label, 0);
        XmStringFree(label);
        XtAddCallback(cntl_ui_hilite_button, XmNactivateCallback,
                      cntl_ui_hilite_button_handler, 0);
        XtManageChild(cntl_ui_obj_controls);
        add_delete_callback(cntl_ui_obj_popup);
}

int cntl_is_same_file(f1, f2) char *f1;
char *f2;
{
        struct stat f1_stat;
        struct stat f2_stat;
        ...
        return ((0 == stat(f1, &f1_stat)) && (0 == stat(f2, &f2_stat)) &&
