...
Tt_status read_file(widget, file) Widget widget;
char *file;
{
...
Tt_status write_file(widget, file) Widget widget;
char *file;
{
        char *text;
        int len;
 ...
 */
void edit_receive_tt_message(client_data, fid, id) XtPointer client_data;
int *fid;
XtInputId *id;
{
        Tt_message msg_in;
 ...
 * Handle the "edit" op.
 */
int edit_edit(msg) Tt_message msg;
{
        int mark = tt_mark();
        ...
        if (access(file, R_OK) != 0) {
                ...
                if (dfile == (char *)0 || 0 != strcmp(dfile, file)) {
                        ...
                        XtVaSetValues(edit_ui_base_window, XmNtitle, buf, 0);
                }
                tt_release(mark);
                return 1;
        }
 ...
 * Handle the "save" op.
 */
int edit_save(msg) Tt_message msg;
{
        int mark = tt_mark();
        char *new_file;
 ...
 * Handle the "close" op.
 */
int edit_close(msg) Tt_message msg;
{
        Atom wmchs;
        Window w;
        ...
        if (text_modified > 0) {
                ...
                /* Send an event to force the event loop to "click on" and quit
                 * the program. */
 ...
 */
int edit_hilite_obj(msg) Tt_message msg;
{
        int mark = tt_mark();
        ...
        if (select_region(edit_ui_textpane, obj_start_text, obj_end_text) ==
            1) {
                tt_release(mark);
                return 1;
        } else {
 ...
 */
void edit_ui_make_object(widget, client_data, call_data) Widget widget;
XtPointer client_data, call_data;
{
        ...
        sprintf(obj_start_text, " /* begin_object(%s) */", objid);
        sprintf(obj_end_text, "	/* end_object(%s) */", objid);
        (void)wrap_selection(edit_ui_textpane, obj_start_text, obj_end_text);
 ...
 * in textsw.
 */
int get_selection(widget, selection, first_ptr, last_ptr) Widget widget;
char **selection;
...
int text_find_bytes(widget, first, last, str, reverse) Widget widget;
XmTextPosition *first, *last;
int reverse;
char *str;
{
 ...
 * successful.
 */
int select_region(widget, begin, end) Widget widget;
char *begin;
char *end;
{
        ...
        XmTextSetSelection(widget, (inspoint = (blast < elast) ? blast : elast),
                           (efirst > bfirst) ? efirst : bfirst, CurrentTime);
 ...
 */
int wrap_selection(widget, begin, end) Widget widget;
char *begin;
char *end;
{
        char *buf;
        ...
        if (!(sel_status =
                  get_selection(widget, &sel, &sel_first, &sel_last))) {
                return sel_status;
        }
 ...
 *  -1, to counter this.
 */
text_modify(widget, client_data, cbs) Widget widget;
...
edit_ui_initialize() {
        ...
        static unsigned short icon_bits[] = {
            0x3FFF, 0xFF00, 0x2000, 0x0180, 0x2000, 0x0140, 0x2000, 0x0120,
            0x2000, 0x0110, 0x2000, 0x0108, 0x2000, 0x0104, 0x2000, 0x01FE,
            0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006,
            0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006,
            0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006,
            0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006,
            0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006, 0x2000, 0x0006,
            0x2000, 0x0006, 0x2000, 0x0006, 0x3FFF, 0xFFFE, 0x1FFF, 0xFFFE};

        edit_ui_base_window = XtAppCreateShell(
        ...
        XtVaSetValues(edit_ui_base_window, XmNwidth, 509, XmNheight, 420,
                      XmNtitle, "Sun_EditDemo_edit", XmNiconName,
                      "Sun_EditDemo_edit", XmNiconPixmap, icon, 0);

        edit_ui_panel = XtVaCreateManagedWidget("panel", xmFormWidgetClass,
                                                edit_ui_base_window, 0);

        label = XmStringCreateSimple("Make object");
        edit_ui_obj_button = XtVaCreateManagedWidget(
            "button", xmPushButtonWidgetClass, edit_ui_panel, XmNtopAttachment,
            XmATTACH_FORM, XmNtopOffset, 5, XmNleftAttachment, XmATTACH_FORM,
            XmNleftOffset, 5, XmNlabelString, label, 0);
        XmStringFree(label);
        XtAddCallback(edit_ui_obj_button, XmNactivateCallback,
                      edit_ui_make_object, 0);
        ...
        n++;
        XtSetArg(args[n], XmNtopWidget, edit_ui_obj_button);
        n++;
        ...
        n++;
        edit_ui_textpane =
            XmCreateScrolledText(edit_ui_panel, "textpane", args, n);

        XtAddCallback(edit_ui_textpane, XmNmodifyVerifyCallback, text_modify,
                      0);
        edit_ui_message = XtVaCreateManagedWidget(
            "message", xmLabelWidgetClass, edit_ui_panel, XmNtopAttachment,
            XmATTACH_WIDGET, XmNtopWidget, edit_ui_textpane, XmNtopOffset, 10,
