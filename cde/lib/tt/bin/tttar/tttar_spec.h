...
declare_list_of(Lstar_spec_prop)
    ...
    class Lstar_spec : public _Tt_object {
      public:
        ...
        bool_t xdr(XDR *xdrs);
        Tt_status read_self();
        char *write_self(_Tt_string where, bool_t preserve__props,
                         Tt_status *err);
        void print(FILE *fs = stdout) const;

        static void do_print(const _Tt_ostream &os, const _Tt_object *obj);
...
class Lstar_spec_prop : public _Tt_object {
      public:
        ...
        bool_t xdr(XDR *xdrs);
        Tt_status write_self(char *spec_id, bool_t preserve__props);
        void print(FILE *fs = stdout) const;
