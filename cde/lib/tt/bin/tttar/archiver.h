...
typedef enum archive_mode { NO_MODE, CREATE, LIST, EXTRACT } Archive_mode;
...
class archiver : public _Tt_object {
      public:
        ...
        int do_tar(_Tt_string tttardir, bool_t only_tttarfile);
        bool_t do_tttar(char *tttarfile_name, bool_t silent);
        ...
        bool_t only_1_look_at_tarfile() { return _only_1_look_at_tarfile; }
        _Tt_string tarfile_arg() { return _tarfile_arg; }

      private:
        FILE *_open_io_stream(char *filename, bool_t complain);
