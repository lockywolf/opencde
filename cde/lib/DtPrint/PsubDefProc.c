 ...
 *
 */
static XtEnum BuildPrinterLists(Widget w, DtPrintDefaultProcData *dpd,
                                int *item_count);
static XmStringTable BuildPrinterSelectionItems(Display *display,
                                                DtPrintDefaultProcData *dpd,
                                                int item_count,
                                                String initial_printer,
                                                XmStringTag tag);
static void CloseSelectPrinterInfoConnection(DtPrintDefaultProcData *dpd);
static int CompareSelectPrinterRecs(const void *, const void *);
static String CompoundTextToString(Display *display,
                                   unsigned char *compound_text);
static XtEnum CreateFileSelectionBox(Widget parent, Widget psub,
                                     DtPrintDefaultProcData *dpd);
static XtEnum CreatePrinterInfoBox(Widget parent, Widget psub,
                                   DtPrintDefaultProcData *dpd);
static XtEnum CreatePrinterSelectionBox(Widget parent, Widget psub,
                                        DtPrintDefaultProcData *dpd);
static void DestroyWidgetCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void DtPrintDefProcDestroyCB(Widget w, XtPointer client_data,
                                    XtPointer call_data);
static void ErrorMessageDestroyCB(Widget w, XtPointer client_data,
                                  XtPointer call_data);
static String FindSelectedPrinter(Display *display,
                                  DtPrintDefaultProcData *dpd);
static void FreeSelectPrinterData(DtPrintDefaultProcData *dpd);
static Widget GetWMShellAncestor(Widget w);
static void HelpDialogDestroyCB(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void InfoBoxLayoutCB(Widget widget, XtPointer client_data,
                            XtPointer call_data);
static void ParseFileNameSpec(const char *file_name, XmString *pattern,
                              char **name_only);
static void PresentErrorDialog(Widget w, String title, String message, ...);
static void PresentHelp(Widget w, const char *help_volume,
                        const char *location_id);
static void PresentVerifyError(Widget w, XtEnum status, String printer_spec);
static void PrinterInfoDestroyCB(Widget w, XtPointer client_data,
                                 XtPointer call_data);
static void PrinterInfoHelpCB(Widget w, XtPointer client_data,
                              XtPointer call_data);
static void SelectFileDestroyCB(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void SelectFileHelpCB(Widget w, XtPointer client_data,
                             XtPointer call_data);
static void SelectPrinterCB(Widget w, XtPointer client_data,
                            XtPointer call_data);
static void SelectPrinterDestroyCB(Widget w, XtPointer client_data,
                                   XtPointer call_data);
static void SelectPrinterHelpCB(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void SelectPrinterInfoCB(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void SelectPrinterItemCB(Widget w, XtPointer client_data,
                                XtPointer call_data);
static void SetListBoxSelection(Widget list_box, int position);
static void UpdateFileNameCB(Widget w, XtPointer client_data,
                             XtPointer call_data);
...
static XtEnum BuildPrinterLists(Widget w, DtPrintDefaultProcData *dpd,
    ...
    XPPrinterList xp_printer_list;
         ...
         */
        for (server_count = 0; (String)NULL != server_list[server_count];
             server_count++)
                ;
        dpd->xp_server_list = (String *)XtCalloc(server_count, sizeof(String));
        dpd->printer_lists = (DtPrintSelectPrinterList *)XtCalloc(
            server_count, sizeof(DtPrintSelectPrinterList));
        dpd->printer_counts = (int *)XtCalloc(server_count, sizeof(int));
	...
	xp_printer_list =
	    XpGetPrinterList(display, (char*)NULL,
                 ...
                 */
                dpd->printer_lists[dpd->xp_server_count] =
                    (DtPrintSelectPrinterList)XtCalloc(
                        dpd->printer_counts[dpd->xp_server_count],
...
static XmStringTable BuildPrinterSelectionItems(Display *display,
        ...
        for (i = 0, current_item = 0; i < dpd->xp_server_count; i++) {
                ...
                for (j = 0; j < dpd->printer_counts[i]; j++, current_item++) {
                        ...
                        full_name = _DtPrintCreateXPrinterSpecifier(
                            name, server_name, DtPRINT_NET_UNSPECIFIED, -1, -1);
                        XtFree(name);
                        /*
                         * check to see if this should be the initially selected
                         * printer
                         */
                        if (dpd->selected_printer == 0 &&
                            (String)NULL != initial_printer)
                                if (strcmp(initial_printer, full_name) == 0)
                                        dpd->selected_printer =
                                            current_item + 1;
                        /*
                         ...
                         */
                        sprintf(buf, "%s\t%s", full_name ? full_name : "",
                                desc ? desc : "");
                        XtFree(desc);
                        XtFree(full_name);
                        items[current_item] =
 ...
 *
 */
static int CompareSelectPrinterRecs(const void *spr1, const void *spr2) {
...
static String CompoundTextToString(Display *display,
        ...
        if ((unsigned char *)NULL != compound_text) {
                ...
                if (Success == XmbTextPropertyToTextList(display, &text_prop,
                                                         &list, &count)) {
                        if (count > 0)
...
static XtEnum CreateFileSelectionBox(Widget parent, Widget psub,
         ...
         * dialog resources
         */
        XtSetArg(args[n], XmNdialogTitle, title_xmstr);
        n++;
         ...
         */
        dpd->file_selection_box = XmCreateFileSelectionDialog(
            parent, "_PsubDefProcFileSelectionBox", args, n);
         ...
         * add callbacks
         */
        XtAddCallback(dpd->file_selection_box, XmNokCallback, UpdateFileNameCB,
                      (XtPointer)psub);
...
static XtEnum CreatePrinterInfoBox(Widget parent, Widget psub,
         ...
         * create the dialog
         */
        dpd->printer_info_box = XmCreateInformationDialog(
            parent, "_PsubDefProcPrinterInfoBox", args, n);
        XmStringFree(title_xmstr);
         ...
         * add callbacks
         */
        XtAddCallback(dpd->printer_info_box, XmNhelpCallback, PrinterInfoHelpCB,
                      (XtPointer)psub);
        XtAddCallback(dpd->printer_info_box, XmNdestroyCallback,
                      PrinterInfoDestroyCB, (XtPointer)psub);
        XtAddCallback(dpd->printer_info_box, XmNmapCallback, InfoBoxLayoutCB,
                      (XtPointer)dpd);
        /*
         ...
         */
        manager = XtVaCreateManagedWidget("PrinterInfoForm", xmFormWidgetClass,
                                          dpd->printer_info_box,
                                          XmNallowOverlap, False, NULL);
        /*
         * printer description
         */
        label = XmStringCreateLocalized(DESCRIPTION_LABEL);
        description_label = XtVaCreateManagedWidget(
            "DescriptionLabel", xmLabelWidgetClass, manager, XmNlabelString,
            label, XmNleftAttachment, XmATTACH_FORM, XmNtopAttachment,
            XmATTACH_FORM, XmNtopOffset, 5, NULL);
        XmStringFree(label);
        description = XtVaCreateManagedWidget(
            "Description", xmLabelWidgetClass, manager, XmNalignment,
            XmALIGNMENT_BEGINNING, XmNleftAttachment, XmATTACH_WIDGET,
            XmNleftWidget, description_label, XmNleftOffset, 10,
            XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET, XmNtopWidget,
        ...
        name_label = XtVaCreateManagedWidget(
            "NameLabel", xmLabelWidgetClass, manager, XmNlabelString, label,
            XmNleftAttachment, XmATTACH_FORM, XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, description, XmNtopOffset, 5, NULL);
        XmStringFree(label);
        name = XtVaCreateManagedWidget(
            "Name", xmLabelWidgetClass, manager, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, name_label, XmNleftOffset, 10,
            XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET, XmNtopWidget,
            name_label, NULL);
        /*
         * printer format
        ...
        format_label = XtVaCreateManagedWidget(
            "FormatLabel", xmLabelWidgetClass, manager, XmNlabelString, label,
            XmNleftAttachment, XmATTACH_FORM, XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, name_label, XmNtopOffset, 5, NULL);
        XmStringFree(label);
        format = XtVaCreateManagedWidget(
            "Format", xmLabelWidgetClass, manager, XmNleftAttachment,
            XmATTACH_WIDGET, XmNleftWidget, format_label, XmNleftOffset, 10,
            XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET, XmNtopWidget,
            format_label, NULL);
        /*
         * printer model
        ...
        model_label = XtVaCreateManagedWidget(
            "ModelLabel", xmLabelWidgetClass, manager, XmNlabelString, label,
            XmNleftAttachment, XmATTACH_FORM, XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, format_label, XmNtopOffset, 5, NULL);
        XmStringFree(label);
        model = XtVaCreateManagedWidget(
            "Model", xmLabelWidgetClass, manager, XmNalignment,
            XmALIGNMENT_BEGINNING, XmNleftAttachment, XmATTACH_WIDGET,
            XmNleftWidget, model_label, XmNleftOffset, 10, XmNtopAttachment,
            XmATTACH_OPPOSITE_WIDGET, XmNtopWidget, model_label, NULL);
        /*
         * return
         */
        return DtPRINT_SUCCESS;
...
static XtEnum CreatePrinterSelectionBox(Widget parent, Widget psub,
        ...
        XtSetArg(args[n], XmNautoUnmanage, True);
        n++;
        XtSetArg(args[n], XmNlistLabelString, list_label);
        n++;
        XtSetArg(args[n], XmNapplyLabelString, info_label);
        n++;
        /*
         * create the dialog
         */
        dpd->printer_selection_box = XmCreateSelectionDialog(
            parent, "_PsubDefProcPrinterSelectionBox", args, n);
...
static void DtPrintDefProcDestroyCB(Widget w, XtPointer client_data,
        ...
        if (dpd->error_message_box) {
                ...
                ErrorMessageDestroyCB(dpd->error_message_box, (XtPointer)w,
                                      (XtPointer)NULL);
        }
        if (dpd->help_dialog) {
                ...
                HelpDialogDestroyCB(dpd->help_dialog, (XtPointer)w,
                                    (XtPointer)NULL);
        }
        if (dpd->file_selection_box) {
                ...
                SelectFileDestroyCB(dpd->file_selection_box, (XtPointer)w,
                                    (XtPointer)NULL);
        }
        if (dpd->printer_selection_box) {
                ...
                SelectPrinterDestroyCB(dpd->printer_selection_box, (XtPointer)w,
                                       (XtPointer)NULL);
        }
        if (dpd->printer_info_box) {
                ...
                PrinterInfoDestroyCB(dpd->printer_info_box, (XtPointer)w,
                                     (XtPointer)NULL);
        }
}
...
static String FindSelectedPrinter(Display *display,
        ...
        if (i < dpd->xp_server_count) {
                ...
                printer_name = CompoundTextToString(
                    display, (unsigned char *)printer_name_ct);
                if ((String)NULL != printer_name) {
                        printer_spec = _DtPrintCreateXPrinterSpecifier(
                            printer_name, display_spec, DtPRINT_NET_UNSPECIFIED,
                            -1, -1);
                        XtFree(printer_name);
                        return printer_spec;
                }
...
static void InfoBoxLayoutCB(Widget widget, XtPointer client_data,
        ...
        w[1] =
            XtNameToWidget(dpd->printer_info_box, "PrinterInfoForm.NameLabel");
        w[2] = XtNameToWidget(dpd->printer_info_box,
                              "PrinterInfoForm.FormatLabel");
        w[3] =
            XtNameToWidget(dpd->printer_info_box, "PrinterInfoForm.ModelLabel");
        /*
        ...
        for (i = 0; i < 4; i++)
                if (i != widest)
                        XtVaSetValues(w[i], XmNleftAttachment, XmATTACH_NONE,
                                      XmNrightAttachment,
                                      XmATTACH_OPPOSITE_WIDGET, XmNrightWidget,
                                      w[widest], NULL);
}
 ...
 *
 */
static void ParseFileNameSpec(const char *file_name, XmString *pattern_xmstr,
                              char **name_only) {
        String ptr;
        String pattern;
        /*
...
static void PresentErrorDialog(Widget w, String title, String message, ...) {
        ...
        if (wmshell_ancestor) {
                n = 0;
                XtSetArg(args[n], XmNchildren, &wmshell_ancestors_children);
                n++;
        ...
        XtSetArg(args[n], XmNdialogTitle, title_xmstr);
        n++;
        XtSetArg(args[n], XmNmessageString, message_xmstr);
        n++;
        XtSetArg(args[n], XmNdialogStyle, XmDIALOG_PRIMARY_APPLICATION_MODAL);
        n++;
        XtSetArg(args[n], XmNdeleteResponse, XmDESTROY);
        n++;
        dpd->error_message_box = XmCreateErrorDialog(
            wmshell_ancestors_children ? wmshell_ancestors_children[0]
                                       : wmshell_ancestor,
        ...
        XtAddCallback(dpd->error_message_box, XmNokCallback, DestroyWidgetCB,
                      (XtPointer)dpd->error_message_box);
                ...
                if (window) {
                        ...
                        if (status == 0 || attr.map_state == IsViewable) {
                                ...
                                for (child = XmGetFocusWidget(w);
                                     child != wmshell_ancestor &&
                                     child != name_w && child != (Widget)NULL;
...
static void PresentHelp(Widget w, const char *help_volume,
        ...
        if ((Widget)NULL == dpd->help_dialog) {
                ...
                n++;
                dpd->help_dialog = DtCreateHelpDialog(
                    wmshell_ancestor, "_PsubDefProcHelpDialog", args, n);
                XmStringFree(title);
         ...
         */
        XtVaSetValues(dpd->help_dialog, DtNhelpVolume, help_volume,
                      DtNlocationId, location_id, DtNhelpType,
                      DtHELP_TYPE_TOPIC, NULL);
        /*
...
static void PresentVerifyError(Widget w, XtEnum status, String printer_spec) {
        ...
        case DtPRINT_PRINTER_MISSING:
                PresentErrorDialog(w, INVALID_PRINTER_TITLE,
                                   PRINTER_MISSING_MESSAGE, printer_spec,
                                   (String)NULL);
                break;

        case DtPRINT_NO_DEFAULT:
                PresentErrorDialog(w, INVALID_PRINTER_TITLE, NO_DEFAULT_MESSAGE,
                                   (String)NULL);
                break;

        case DtPRINT_NO_DEFAULT_DISPLAY:
                PresentErrorDialog(w, INVALID_PRINTER_TITLE,
                                   NO_DEFAULT_DISPLAY_MESSAGE, printer_name,
                                   (String)NULL);
                break;

        case DtPRINT_NO_PRINTER:
                PresentErrorDialog(w, INVALID_PRINTER_TITLE,
                                   INVALID_PRINTER_MESSAGE, printer_name,
                                   display_spec, (String)NULL);
                break;

        case DtPRINT_NOT_XP_DISPLAY:
                PresentErrorDialog(w, INVALID_PRINTER_TITLE,
                                   NOT_XP_DISPLAY_MESSAGE, display_spec,
                                   (String)NULL);
                break;

        case DtPRINT_INVALID_DISPLAY:
                PresentErrorDialog(w, INVALID_PRINTER_TITLE,
                                   INVALID_DISPLAY_MESSAGE, display_spec,
                                   (String)NULL);
                break;
        }
        XtFree(printer_name);
...
static void SelectPrinterInfoCB(Widget w, XtPointer client_data,
        ...
        if ((DtPrintSetupProc)NULL != info_proc) {
                ...
                if ((String)NULL != printer_spec) {
                         ...
                         */
                        status = _DtPrintVerifyXPrinter(
                            psub, printer_spec, &new_printer_spec,
		...
		psd.print_context = dpd->select_printer_info_context;
                                ...
                                /*
                                 * this should only happen if, while a user is
                                 * viewing the printer selection dialog, a
                                 * server rehash is performed that deletes the
...
static void SelectPrinterItemCB(Widget w, XtPointer client_data,
        ...
        if (dpd->selected_printer != cbs->item_position) {
                ...
                info_button =
                    XtNameToWidget(dpd->printer_selection_box, "*Apply");
                if (0 == dpd->selected_printer)
...
static void SetListBoxSelection(Widget list_box, int position) {
         ...
         */
        XtVaGetValues(list_box, XmNitemCount, &item_count, XmNvisibleItemCount,
 ...
 *
 */
void _DtPrintDefProcInitialize(Widget w) {
        DtPrintDefaultProcData *dpd = &PSUB_DefaultProcData(w);
    ...
    dpd->select_printer_info_context = (XPContext)None;
        ...
        XtAddCallback(w, XmNdestroyCallback, DtPrintDefProcDestroyCB,
                      (XtPointer)NULL);
}
 ...
 *
 */
void _DtPrintDefProcManageErrorBox(DtPrintDefaultProcData *dpd) {
        if (dpd->error_message_box != (Widget)NULL)
                XtManageChild(dpd->error_message_box);
}
 ...
 *
 */
XtEnum _DtPrintSetupBoxXPrinterInfoProc(Widget w,
        ...
        if (dpd->printer_info_box == (Widget)NULL) {
                 ...
                 */
                manager =
                    XtNameToWidget(dpd->printer_info_box, "*PrinterInfoForm");
    ...
    attr_value_ct = XpGetOneAttribute(print_data->print_display,
        ...
        if ((char *)NULL != attr_value_ct) {
                attr_value = CompoundTextToString(
                    XtDisplay(w), (unsigned char *)attr_value_ct);
                XFree(attr_value_ct);
                if ((String)NULL != attr_value) {
                        label = XmStringGenerate(
                            (XtPointer)attr_value, (XmStringTag)NULL,
 ...
 *
 */
XtEnum _DtPrintSetupBoxSelectFileProc(Widget w, DtPrintSetupData *print_data) {
        DtPrintDefaultProcData *dpd = &PSUB_DefaultProcData(w);
        String name_only;
        String file_name;
        XmString pattern;
        ...
        if (dpd->file_selection_box == (Widget)NULL) {
                XtEnum status =
                    CreateFileSelectionBox(wmshell_ancestor, w, dpd);
                if (status != DtPRINT_SUCCESS) {
                        _DtTurnOffHourGlass(wmshell_ancestor);
                        return status;
                }
        ...
        if (file_list != (Widget)NULL) {
                ...
                if (text_field != (Widget)NULL) {
                        String dir_spec;
                        /*
                         * recreate the file name from the name_only and the
                         * current dir spec as determined by the file selection
                         * box
                         *
                         * Note: the char* value retrieved directly from the
                         * text field is used, because forming the file name
                         * from a concatenation of XmStrings causes
                         * XmListItemPos to fail even though the file name is
                         * actually in the list.
                         */
                        XtVaGetValues(text_field, XmNvalue, &dir_spec, NULL);
                        file_name =
                            XtMalloc(strlen(dir_spec) + strlen(name_only) + 1);
                ...
                if (position == 0) {
                         ...
                         */
                        XtVaSetValues(dpd->file_selection_box, XmNdirSpec,
                                      file_name_xmstr, NULL);
                }
 ...
 *
 */
XtEnum _DtPrintSetupBoxSelectXPrinterProc(Widget w,
                                          DtPrintSetupData *print_data) {
        DtPrintDefaultProcData *dpd = &PSUB_DefaultProcData(w);
        Widget wmshell_ancestor;
        XmStringTable items;
        ...
        if (DtPRINT_SUCCESS != BuildPrinterLists(w, dpd, &item_count)) {
                PresentErrorDialog(w, PRLIST_ERROR_TITLE, NO_PRINTERS_MESSAGE,
                                   (String)NULL);
                _DtTurnOffHourGlass(wmshell_ancestor);
                return DtPRINT_FAILURE;
        }
        ...
        if (dpd->printer_selection_box == (Widget)NULL) {
                XtEnum status =
                    CreatePrinterSelectionBox(wmshell_ancestor, w, dpd);
                if (status != DtPRINT_SUCCESS) {
                        _DtTurnOffHourGlass(wmshell_ancestor);
                        return status;
                }
         ...
         */
        XtVaGetValues(dpd->printer_list_box, XmNrenderTable, &render_table,
                      NULL);
         ...
         */
        tab_list = XmStringTableProposeTablist(
            items, item_count, dpd->printer_list_box, 20, XmRELATIVE);
        /*
         ...
         */
        n = 0;
        XtSetArg(args[n], XmNtabList, tab_list);
        n++;
         ...
         */
        XtVaSetValues(dpd->printer_list_box, XmNrenderTable, new_render_table,
 ...
 *
 */
XtEnum _DtPrintSetupBoxVerifyXPrinterProc(Widget w, DtPrintSetupData *psd) {
        DtPrintDefaultProcData *dpd = &PSUB_DefaultProcData(w);
        XtEnum status;
    ...
    XPContext new_context;
         ...
         * verify the printer
         */
        status = _DtPrintVerifyXPrinter(w, psd->printer_name, &new_printer_spec,
                                        &new_display
#if 0 && defined(PRINTING_SUPPORTED)
				    ,&new_context
#endif /* PRINTING_SUPPORTED */
	...
	psd->print_context = new_context;
                ...
                PresentVerifyError(w, status,
                                   new_printer_spec ? new_printer_spec
                                                    : psd->printer_name);
                /*
        ...
        if (new_printer_spec) {
                 ...
                 */
                XtVaSetValues(w, DtNprinterName, new_printer_spec, NULL);
