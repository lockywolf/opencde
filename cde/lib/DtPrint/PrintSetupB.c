 ...
 */
#define IsButton(w)                                                            \
        (((XtPointer)XmeTraitGet((XtPointer)XtClass((w)), XmQTactivatable) !=  \
          NULL))

#define IsAutoButton(psub, w)                                                  \
        (w == PSUB_PrinterInfoButton(psub) ||                                  \
         w == PSUB_SelectPrinterButton(psub) ||                                \
         w == PSUB_SelectFileButton(psub) || w == PSUB_PrintButton(psub) ||    \
         w == PSUB_SetupButton(psub) || w == PSUB_CancelButton(psub) ||        \
...
static void ClosePrintConnection(DtPrintSetupBoxWidget psub,
                                 String old_printer_name, Boolean update_gui);
static void ClosePrintConnectionCallback(Widget w, XtPointer client_data,
                                         XtPointer call_data);
static void ColumnGeoFixUp(XmGeoMatrix geoSpec, int action,
                           XmGeoMajorLayout layoutPtr, XmKidGeometry rowPtr);
static void ColumnGeoPreSet(PSUB_GeoExtension ext, int row);
static void ColumnGeoRestoreWidths(PSUB_GeoExtension ext, int row);
static void ColumnGeoSaveWidths(PSUB_GeoExtension ext, int row);
static void ColumnGeoSetPreferredWidths(PSUB_GeoExtension ext, int row);
static Boolean ConvertStringToEnum(Display *dpy, XrmValuePtr args,
                                   Cardinal *num_args, XrmValuePtr from,
                                   XrmValuePtr to, XtPointer *data);
static void CopiesTextValueChangedCallback(Widget w, XtPointer client_data,
                                           XtPointer call_data);
static Widget CreateButton(Widget parent, String label, String name,
                           XmNavigationType nav_type);
static Widget CreateButtonGadget(Widget parent, String label, String name,
                                 XmNavigationType nav_type);
static Widget CreateLabelGadget(Widget parent, String label, String name);
static String CreateModalPrinterSpec(XtEnum name_mode,
                                     const String full_printer_name);
static void DeleteChild(Widget child);
static void DestinationChangedCallback(Widget w, XtPointer client_data,
                                       XtPointer call_data);
static void Destroy(Widget w);
static XtEnum EstablishPrinter(DtPrintSetupBoxWidget psub,
                               DtPrintSetupData *psd);
static void GetPrintAttributes(DtPrintSetupBoxWidget psub);
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args);
static void InsertChild(Widget child);
static Boolean IsSetupRequired(DtPrintSetupBoxWidget psub);
static void MenuBarFixUp(XmGeoMatrix geoSpec, int action,
                         XmGeoMajorLayout layoutPtr, XmKidGeometry rowPtr);
static void PrintSetupBoxCallback(Widget w, XtPointer client_data,
                                  XtPointer call_data);
static void SeparatorFixUp(XmGeoMatrix geoSpec, int action,
                           XmGeoMajorLayout layoutPtr, XmKidGeometry rowPtr);
static void SetNewPrinterName(DtPrintSetupBoxWidget psub,
                              String new_printer_name, String old_printer_name);
static void SetPrintAttributes(DtPrintSetupBoxWidget psub);
static void SetPSDDestination(DtPrintSetupBoxWidget psub,
                              DtPrintSetupData *psd);
static XmImportOperator SetSyntheticResForChild(Widget widget, int offset,
                                                XtArgVal *value);
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args,
                         Cardinal *num_args);
static int SpanNonWhitespace(const char *string);
static int SpanWhitespace(const char *string);
static void SynthGetFileName(DtPrintSetupBoxWidget psub);
static void SynthGetPrinterName(DtPrintSetupBoxWidget psub);
static void SynthSetCopies(DtPrintSetupBoxWidget new_w,
                           DtPrintSetupBoxWidget current);
static void SynthSetFileName(DtPrintSetupBoxWidget new_w,
                             DtPrintSetupBoxWidget current);
static void SynthSetPrintDestination(DtPrintSetupBoxWidget new_w,
                                     DtPrintSetupBoxWidget current);
static void SynthSetPrinterName(DtPrintSetupBoxWidget psub,
                                String new_printer_name,
                                String old_printer_name);
static void UpdatePrinterNameCallback(Widget w, XtPointer client_data,
                                      XtPointer call_data);
static void UpdatePrinterNameTimeoutProc(XtPointer client_data,
                                         XtIntervalId *id);
static void UpdateString(Widget w, XmString string,
                         XmStringDirection direction);
static void ValidatePrintSetupMode(DtPrintSetupBoxWidget new_w,
                                   DtPrintSetupBoxWidget current);
static void ValidateWorkAreaLocation(DtPrintSetupBoxWidget new_w,
                                     DtPrintSetupBoxWidget current);
/*
 * ------------------------------------------------------------------------
...
static XmSyntheticResource syn_resources[] = {
    {DtNcopies, sizeof(int),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.copies),
     _DtPrintSetupBoxGetCopies, SetSyntheticResForChild},
    {DtNdescription, sizeof(XmString),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.description_string),
     _DtPrintSetupBoxGetDescription, SetSyntheticResForChild},
    {DtNfileName, sizeof(String),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.file_name),
     _DtPrintSetupBoxGetFileName, SetSyntheticResForChild},
    {DtNprinterName, sizeof(String),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.printer_name),
     _DtPrintSetupBoxGetPrinterName, SetSyntheticResForChild}};

/*
 * Resource Definitions
 */
static XtResource resources[] = {
    {XmNnoResize, XmNnoResize, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(XmBulletinBoardRec, bulletin_board.no_resize), XmRImmediate,
     (XtPointer)True},
    {DtNcancelCallback, DtCCancelCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.cancel_callback),
     XmRImmediate, (XtPointer)NULL},
    {DtNclosePrintDisplayCallback, DtCClosePrintDisplayCallback, XmRCallback,
     sizeof(XtCallbackList),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.close_display_callback),
     XmRImmediate, (XtPointer)NULL},
    {DtNcopies, DtCCopies, XmRInt, sizeof(int),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.copies), XmRImmediate,
     (XtPointer)1},
    {DtNdescription, DtCDescription, XmRXmString, sizeof(XmString),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.description_string),
     XmRString, (XtPointer)NULL},
    {DtNfileName, DtCFileName, XmRString, sizeof(String),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.file_name), XmRString,
     (XtPointer)NULL},
    {DtNminimizeButtons, DtCMinimizeButtons, XmRBoolean, sizeof(Boolean),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.minimize_buttons),
     XmRImmediate, (XtPointer)False},
    {DtNprintCallback, DtCPrintCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.print_callback),
     XmRImmediate, (XtPointer)NULL},
    {DtNoptionCount, DtCOptionCount, XmRCardinal, sizeof(Cardinal),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.option_count), XmRImmediate,
     (XtPointer)0},
    {DtNoptions, DtCOptions, XmRPointer, sizeof(XtPointer),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.options), XmRImmediate,
     (XtPointer)NULL},
    {
        DtNprintDestination,
        DtCPrintDestination,
        XmREnum,
        sizeof(XtEnum),
        XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.print_destination),
        XmRImmediate,
        (XtPointer)DtPRINT_TO_PRINTER,
    },
    {DtNprinterInfoProc, DtCPrinterInfoProc, DtRPrintSetupProc,
     sizeof(DtPrintSetupProc),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.printer_info_proc),
     XmRImmediate, (XtPointer)DtUNSPECIFIED},
    {DtNprinterName, DtCPrinterName, XmRString, sizeof(String),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.printer_name), XmRString,
     (XtPointer)NULL},
    {
        DtNprintSetupMode,
        DtCPrintSetupMode,
        XmREnum,
        sizeof(XtEnum),
        XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.print_setup_mode),
        XmRImmediate,
        (XtPointer)SETUP_MODE_DEFAULT,
    },
    {DtNselectFileProc, DtCSelectFileProc, DtRPrintSetupProc,
     sizeof(DtPrintSetupProc),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.select_file_proc),
     XmRImmediate, (XtPointer)_DtPrintSetupBoxSelectFileProc},
    {DtNselectPrinterProc, DtCSelectPrinterProc, DtRPrintSetupProc,
     sizeof(DtPrintSetupProc),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.select_printer_proc),
     XmRImmediate, (XtPointer)DtUNSPECIFIED},
    {DtNsetupCallback, DtCSetupCallback, XmRCallback, sizeof(XtCallbackList),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.setup_callback),
     XmRImmediate, (XtPointer)NULL},
    {DtNverifyPrinterProc, DtCVerifyPrinterProc, DtRPrintSetupProc,
     sizeof(DtPrintSetupProc),
     XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.verify_printer_proc),
     XmRImmediate, (XtPointer)DtUNSPECIFIED},
    {
        DtNworkAreaLocation,
        DtCWorkAreaLocation,
        XmREnum,
        sizeof(XtEnum),
        XtOffsetOf(DtPrintSetupBoxRec, print_setup_box.work_area_location),
        XmRImmediate,
        (XtPointer)WORK_AREA_DEFAULT,
    },
};

externaldef(dtprintsetupboxclassrec)
...
externaldef(dtprintsetupboxwidgetclass) WidgetClass dtPrintSetupBoxWidgetClass =
...
static void ClassInitialize(void) {
         ...
         */
        XtSetTypeConverter("String", "Enum", ConvertStringToEnum, NULL, 0,
                           XtCacheAll, NULL);
}
...
static void ClosePrintConnection(DtPrintSetupBoxWidget psub,
    ...
    psd.print_context = PSUB_Context(psub);
        ...
        XtCallCallbackList((Widget)psub, PSUB_CloseDisplayCallback(psub),
                           (XtPointer)&cbs);
        /*
        ...
        if (update_gui) {
                empty_label =
                    XmStringGenerate((XtPointer) "", (XmStringTag)NULL,
                                     XmMULTIBYTE_TEXT, (XmStringTag)NULL);
                UpdateString(PSUB_Description((Widget)psub), empty_label,
                             PSUB_StringDirection((Widget)psub));
        ...
        if (!PSUB_CopyCountSupported(psub)) {
                ...
                if (PSUB_PrintDestination(psub) == DtPRINT_TO_PRINTER &&
                    update_gui) {
                        if (PSUB_CopiesSpinBox(psub))
                                XtSetSensitive(PSUB_CopiesSpinBox(psub), True);
                        if (PSUB_CopiesControl(psub)) {
                                Widget copies_label = XtNameToWidget(
                                    PSUB_CopiesControl(psub), "CopiesLabel");
 ...
 *
 */
static void ColumnGeoFixUp(XmGeoMatrix geo_spec, int action,
                           XmGeoMajorLayout layout_ptr, XmKidGeometry row_ptr) {
        ...
        for (col = 0; col < PSUB_GEO_COL_COUNT; col++) {
                ...
                if (pbox->geo) {
                        ...
                        case PSUB_GEO_ALIGN_RIGHT:
                                delta += (ext->col_width)[col] -
                                         pbox->geo->box.width;
                                pbox->geo->box.x +=
                                    (ext->col_width)[col] - pbox->orig_width;
...
static void CopiesTextValueChangedCallback(Widget w, XtPointer client_data,
        ...
        if (XmCR_VALUE_CHANGED == cbs->reason) {
                ...
                if (value)
                        value_int = strtol(value, (char **)NULL, 10);

                if (1 <= value_int || value_int <= MAX_COPIES) {
                        PSUB_Copies(psub) = value_int;
                        XtVaSetValues(PSUB_CopiesSpinBox(psub), XmNposition,
                                      value_int, NULL);
                } else {
                        XmeWarning((Widget)w, WARN_COPY_COUNT);
                        XtVaGetValues(PSUB_CopiesSpinBox(psub), XmNposition,
                                      &value_int, NULL);
                ...
                if (value)
                        XtVaSetValues(w, XmNcursorPosition, strlen(value),
                                      NULL);
        }
}
...
static Widget CreateButton(Widget parent, String label, String name,
        ...
        label_string = XmStringGenerate((XtPointer)label, (XmStringTag)NULL,
                                        XmMULTIBYTE_TEXT, (XmStringTag)NULL);
        w = XtVaCreateWidget(name, xmPushButtonWidgetClass, parent,
                             XmNlabelString, label_string, XmNnavigationType,
                             nav_type, NULL);

        if (label_string)
                XmStringFree(label_string);

        trait_default = (XmTakesDefaultTrait)XmeTraitGet((XtPointer)XtClass(w),
                                                         XmQTtakesDefault);
        if (trait_default)
...
static Widget CreateButtonGadget(Widget parent, String label, String name,
        ...
        label_string = XmStringGenerate((XtPointer)label, (XmStringTag)NULL,
                                        XmMULTIBYTE_TEXT, (XmStringTag)NULL);
        w = XtVaCreateWidget(name, xmPushButtonGadgetClass, parent,
                             XmNlabelString, label_string, XmNnavigationType,
                             nav_type, NULL);
...
static Widget CreateLabelGadget(Widget parent, String label, String name) {
        ...
        label_string = XmStringGenerate((XtPointer)label, (XmStringTag)NULL,
                                        XmMULTIBYTE_TEXT, (XmStringTag)NULL);
        w = XtVaCreateWidget(name, xmLabelGadgetClass, parent, XmNlabelString,
                             label_string, NULL);
...
static String CreateModalPrinterSpec(XtEnum name_mode,
         ...
         * name components
         */
        _DtPrintParseXPrinterSpecifier(full_printer_name, &printer_name,
                                       &display_spec);
        /*
        ...
        if (name_mode == DtSHORT_NAME) {
                ...
                switch (name_mode) {
                case DtMEDIUM_NAME:
                        modal_printer_spec = _DtPrintCreateXPrinterSpecifier(
                ...
                case DtLONG_NAME:
                        modal_printer_spec = _DtPrintCreateXPrinterSpecifier(
                            printer_name, host_name, spec_net, display_num,
                            screen_num);
                        break;
...
static XtEnum EstablishPrinter(DtPrintSetupBoxWidget psub,
        ...
        case DtPRINT_NOT_VERIFIED:
                ...
                if (PSUB_VerifyPrinterProc(psub) != NULL) {
                        ...
                        status =
                            (*PSUB_VerifyPrinterProc(psub))((Widget)psub, psd);
                        if (status != DtPRINT_SUCCESS) {
                                /*
                                 * the printer isn't valid - set the verify
                                 * printer state back to "not verified" and
                                 * return
                                 */
                                PSUB_VerifyPrinterState(psub) =
		...
		PSUB_Context(psub) = psd->print_context;
                                 ...
                                 */
                                {
                                        ...
                                        while ((Widget)NULL != video_widget) {
                                                if (XtIsApplicationShell(
                                                        video_widget))
                                                        break;
                                                else
                                                        video_widget = XtParent(
                                                            video_widget);
                                        }
                                        if ((Widget)NULL == video_widget)
                                                video_widget = (Widget)psub;
                                        /*
                                         * if not an applicationShell, argc/argv
                                         * will stay NULL in the GetValues call,
                                         * no need to check explicitly
                                         */
                                        XtVaGetValues(video_widget, XmNargc,
                                                      &argc, XmNargv, &argv,
                                                      NULL);

                                        /* copy argv, modified by
                                         * XtDisplayInitialize below */
                                        argv_copy = (String *)XtMalloc(
                                            sizeof(String) * (argc + 1));
                                        for (i = 0; i < argc; i++) {
                                                argv_copy[i] = argv[i];
                                        }
                                        argv_copy[argc] = NULL;
                                        ...
                                        /*
                                         * now add the print display in our app
                                         * context, so that event dispatching
                                         * works
                                         */
                                        XtGetApplicationNameAndClass(
                                            XtDisplay(video_widget),
                                            &video_name, &video_class);
                                        app_context =
                                            XtWidgetToApplicationContext(
                                                video_widget);
                                        XtDisplayInitialize(
                                            app_context, PSUB_Display(psub),
                                            video_name, video_class,
                                            PSUB_Options(psub),
                                            PSUB_OptionCount(psub), &argc,
                                            argv_copy);
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args) {
        ...
        if (PSUB_PrintSetupMode(new_w) == DtPRINT_SETUP_XP) {
                if (PSUB_PrinterInfoProc(new_w) ==
                    (DtPrintSetupProc)DtUNSPECIFIED)
                        PSUB_PrinterInfoProc(new_w) =
                            _DtPrintSetupBoxXPrinterInfoProc;
                if (PSUB_SelectPrinterProc(new_w) ==
                    (DtPrintSetupProc)DtUNSPECIFIED)
                        PSUB_SelectPrinterProc(new_w) =
                            _DtPrintSetupBoxSelectXPrinterProc;
                if (PSUB_VerifyPrinterProc(new_w) ==
                    (DtPrintSetupProc)DtUNSPECIFIED)
                        PSUB_VerifyPrinterProc(new_w) =
    ...
    PSUB_Context(new_w) = (XPContext)NULL;
         ...
         */
        {
                ...
                XtGetApplicationResources(
                    nw, (XtPointer)&PSUB_XpPrinterNameMode(new_w), &res_struct,
                    1, (ArgList)NULL, 0);
        }
        /*
         ...
         */
        SynthSetPrinterName(new_w, XtNewString(PSUB_PrinterName(new_w)),
                            (String)NULL);
        /*
...
static void PrintSetupBoxCallback(Widget w, XtPointer client_data,
        ...
        case DtPRINT_CANCEL_BUTTON:
                ...
                XtCallCallbackList((Widget)psub, PSUB_CancelCallback(psub),
                                   (XtPointer)&cbs);
                break;
        ...
        case DtPRINT_INFO_BUTTON:
                if (PSUB_PrinterInfoProc(psub)) {
                        /*
                         * pick up the printer name from the widget's Printer
                         * Name text field
                         */
                        ...
                        if (EstablishPrinter(psub, &psd) == DtPRINT_SUCCESS) {
                                 ...
                                 */
                                (*PSUB_PrinterInfoProc(psub))((Widget)psub,
                                                              &psd);
                        }
                }
                break;
                ...
                if (EstablishPrinter(psub, &psd) == DtPRINT_SUCCESS) {
                        ...
                        if (PSUB_PrintSetupMode(psub) == DtPRINT_SETUP_XP &&
                            IsSetupRequired(psub)) {
                                /*
                                 * setup is needed; call the setup callback
                                 * instead
                                 */
                                ...
                                /*
                                 * The printer name is OK. Go ahead and unmanage
                                 * the print setup box if the auto-unmanage
                                 * resource is set.
                                 */
        ...
        if (PSUB_PrinterName(new_w) != PSUB_PrinterName(current)) {
                if ((PSUB_PrinterName(new_w) == NULL ||
                     PSUB_PrinterName(current) == NULL)
                        ? True
                        : strcmp(PSUB_PrinterName(new_w),
                                 PSUB_PrinterName(current)) != 0) {
                        /*
                         * the printer names are different; set the new printer
                         * name into the instance structure and the printer name
                         * text field
                         */
                        SynthSetPrinterName(
                            new_w, XtNewString(PSUB_PrinterName(new_w)),
...
static int SpanNonWhitespace(const char *string) {
        const char *ptr;
        for (ptr = string; *ptr != '\0' && !DtIsspace((char *)ptr);
...
static void SynthGetFileName(DtPrintSetupBoxWidget psub) {
        ...
        if (PSUB_FileNameText(psub) != (Widget)NULL)
                XtVaGetValues((Widget)PSUB_FileNameText(psub), XmNvalue,
                              &PSUB_FileName(psub), NULL);
        else
...
static void SynthGetPrinterName(DtPrintSetupBoxWidget psub) {
        ...
        if (PSUB_PrinterNameText(psub) != (Widget)NULL) {
                ...
                XtVaGetValues((Widget)PSUB_PrinterNameText(psub), XmNvalue,
                              &PSUB_ModalPrinterSpec(psub), NULL);
                /*
                ...
                case DtPRINT_SETUP_PLAIN:
                        ...
                        } else if (strcmp(PSUB_ModalPrinterSpec(psub),
                                          PSUB_PrinterName(psub)) != 0) {
                                printer_spec_changed = True;
                        }
                        break;
                ...
                case DtPRINT_SETUP_XP:
                        if (PSUB_PrinterName(psub) == (String)NULL ||
                            ...
                            strcmp(previous_modal_spec,
                                   PSUB_ModalPrinterSpec(psub)) != 0) {
                                printer_spec_changed = True;
                        }
                        break;
                }
        ...
        if (printer_spec_changed) {
                SetNewPrinterName(
                    psub, XtNewString(PSUB_ModalPrinterSpec(psub)), /*new name*/
...
static void SynthSetCopies(DtPrintSetupBoxWidget new_w,
        ...
        if (PSUB_CopiesSpinBox(new_w) != (Widget)NULL)
                XtVaSetValues(PSUB_CopiesSpinBox(new_w), XmNposition,
                              PSUB_Copies(new_w), NULL);
}
...
static void SynthSetFileName(DtPrintSetupBoxWidget new_w,
        ...
        if (PSUB_FileNameText(new_w) != (Widget)NULL) {
                ...
                XmTextFieldSetString(PSUB_FileNameText(new_w),
                                     PSUB_FileName(new_w));
                last_position =
                    XmTextFieldGetLastPosition(PSUB_FileNameText(new_w));
...
static void SynthSetPrintDestination(DtPrintSetupBoxWidget new_w,
        ...
        if (PSUB_PrintDestination(new_w) != DtPRINT_TO_PRINTER &&
            PSUB_PrintDestination(new_w) != DtPRINT_TO_FILE) {
                ...
                if (current == (DtPrintSetupBoxWidget)NULL)
                        ...
                        PSUB_PrintDestination(new_w) =
                            PSUB_PrintDestination(current);
        }
        if ((Widget)NULL != PSUB_DestinationRadioBox(new_w)) {
                ...
                case DtPRINT_TO_PRINTER:
                        button = XtNameToWidget(PSUB_DestinationRadioBox(new_w),
                                                "button_0");
...
static void SynthSetPrinterName(DtPrintSetupBoxWidget psub,
        ...
        case DtPRINT_SETUP_PLAIN:
                ...
                PSUB_ModalPrinterSpec(psub) =
                    XtNewString(PSUB_PrinterName(psub));
                break;
        ...
        case DtPRINT_SETUP_XP:
                 ...
                 */
                PSUB_ModalPrinterSpec(psub) = CreateModalPrinterSpec(
                    PSUB_XpPrinterNameMode(psub), PSUB_PrinterName(psub));
                break;
        }
        /*
        ...
        if (PSUB_PrinterNameText(psub)) {
                XtVaSetValues(PSUB_PrinterNameText(psub), XmNvalue,
                              PSUB_ModalPrinterSpec(psub), NULL);
        ...
        if (PSUB_VerifyPrinterState(psub) == DtPRINT_NOT_VERIFIED) {
                ...
                if (window) {
                        ...
                        Status status = XGetWindowAttributes(
                            XtDisplay((Widget)psub), window, &attr);
...
static void UpdatePrinterNameCallback(Widget w, XtPointer client_data,
        ...
        PSUB_TimeoutId(psub) =
            XtAppAddTimeOut(XtWidgetToApplicationContext(w), (unsigned long)0,
                            UpdatePrinterNameTimeoutProc, (XtPointer)psub);
}
 ...
 *
 */
void _DtPrintSetupBoxCreateBottomSeparator(DtPrintSetupBoxWidget psub) {
        Arg al[10];
        register int ac = 0;

        XtSetArg(al[ac], XmNhighlightThickness, 0);
        ac++;
        PSUB_BottomSeparator(psub) = XmCreateSeparatorGadget(
            (Widget)psub, "BottomWorkAreaSeparator", al, ac);
}
 ...
 *
 */
void _DtPrintSetupBoxCreateButtonSeparator(DtPrintSetupBoxWidget sel) {
        Arg al[10];
        register int ac = 0;
 ...
 *
 */
void _DtPrintSetupBoxCreateCancelButton(DtPrintSetupBoxWidget psub) {
 ...
 *
 */
void _DtPrintSetupBoxCreateCopiesControl(DtPrintSetupBoxWidget psub) {
        Widget copies_label;
        Widget copies_text;
        ...
        PSUB_CopiesControl(psub) =
            XtVaCreateWidget("CopiesControl", xmRowColumnWidgetClass,
                             (Widget)psub, XmNorientation, XmHORIZONTAL, NULL);
        /*
         * create the label
         */
        copies_label = CreateLabelGadget(PSUB_CopiesControl(psub), COPIES_LABEL,
                                         "CopiesLabel");
        XtManageChild(copies_label);
        /*
         * spin box
         */
        PSUB_CopiesSpinBox(psub) = XtVaCreateManagedWidget(
            "Copies", xmSimpleSpinBoxWidgetClass, PSUB_CopiesControl(psub),
            XmNspinBoxChildType, XmNUMERIC, XmNminimumValue, 1, XmNmaximumValue,
            MAX_COPIES, XmNposition, 1, XmNpositionType, XmPOSITION_VALUE,
            XmNcolumns, 5, XmNwrap, False, NULL);

        copies_text = NULL;
        copies_text_editable = FALSE;
        XtVaGetValues(PSUB_CopiesSpinBox(psub), XmNtextField, &copies_text,
                      XmNeditable, &copies_text_editable, NULL);

        if (copies_text && copies_text_editable)
                XtAddCallback(copies_text, XmNvalueChangedCallback,
 ...
 *
 */
void _DtPrintSetupBoxCreateDescription(DtPrintSetupBoxWidget psub) {
        ...
        PSUB_Description(psub) = XtVaCreateWidget(
            "Description", xmLabelWidgetClass, (Widget)psub, XmNalignment,
            XmALIGNMENT_BEGINNING, XmNlabelString, empty_label, NULL);
        XmStringFree(empty_label);
        /*
         * get the maximum character width for the default font of the gadget
         */
        XtVaGetValues(PSUB_Description(psub), XmNrenderTable, &render_table,
                      NULL);
        if (XmeRenderTableGetDefaultFont(render_table, &font)) {
                 ...
                 */
                XtVaSetValues(PSUB_Description(psub), XmNwidth,
                              (Dimension)(DESCRIPTION_COLUMNS * char_width),
 ...
 *
 */
void _DtPrintSetupBoxCreateDescriptionLabel(DtPrintSetupBoxWidget psub) {
        PSUB_DescriptionLabel(psub) = CreateLabelGadget(
            (Widget)psub, DESCRIPTION_LABEL, "DescriptionLabel");
}
 ...
 *
 */
void _DtPrintSetupBoxCreateDestinationRadioBox(DtPrintSetupBoxWidget psub) {
        Widget button;
        XmString label;

        PSUB_DestinationRadioBox(psub) = XtVaCreateWidget(
            "DestRadioBox", xmRowColumnWidgetClass, (Widget)psub,
            XmNradioBehavior, True, XmNorientation, XmHORIZONTAL, XmNpacking,
            XmPACK_TIGHT, NULL);

        label = XmStringGenerate((XtPointer)PRINT_TO_PRINTER_LABEL,
                                 (XmStringTag)NULL, XmMULTIBYTE_TEXT,
                                 (XmStringTag)NULL);
        button = XtVaCreateManagedWidget("button_0", xmToggleButtonWidgetClass,
                                         PSUB_DestinationRadioBox(psub),
                                         XmNlabelString, label, XmNuserData,
                                         (XtPointer)DtPRINT_TO_PRINTER, NULL);
        XmStringFree(label);
        XtAddCallback(button, XmNvalueChangedCallback,
                      DestinationChangedCallback, (XtPointer)psub);

        label =
            XmStringGenerate((XtPointer)PRINT_TO_FILE_LABEL, (XmStringTag)NULL,
                             XmMULTIBYTE_TEXT, (XmStringTag)NULL);
        button = XtVaCreateManagedWidget("button_1", xmToggleButtonWidgetClass,
                                         PSUB_DestinationRadioBox(psub),
                                         XmNlabelString, label, XmNuserData,
                                         (XtPointer)DtPRINT_TO_FILE, NULL);
        XmStringFree(label);
        XtAddCallback(button, XmNvalueChangedCallback,
                      DestinationChangedCallback, (XtPointer)psub);
}
 ...
 *
 */
void _DtPrintSetupBoxCreateFileNameLabel(DtPrintSetupBoxWidget psub) {
 ...
 *
 */
void _DtPrintSetupBoxCreateFileNameText(DtPrintSetupBoxWidget psub) {
        PSUB_FileNameText(psub) =
            XtVaCreateWidget("FileName", xmTextFieldWidgetClass, (Widget)psub,
                             XmNcolumns, FILE_NAME_COLUMNS, NULL);
}
 ...
 *
 */
void _DtPrintSetupBoxCreateHelpButton(DtPrintSetupBoxWidget psub) {
 ...
 *
 */
void _DtPrintSetupBoxCreatePrintButton(DtPrintSetupBoxWidget psub) {
 ...
 *
 */
void _DtPrintSetupBoxCreatePrinterInfoButton(DtPrintSetupBoxWidget psub) {
 ...
 *
 */
void _DtPrintSetupBoxCreatePrinterNameCombo(DtPrintSetupBoxWidget psub) {
        PSUB_PrinterNameCombo(psub) = XtVaCreateWidget(
            "Name", xmComboBoxWidgetClass, (Widget)psub, XmNcomboBoxType,
            XmDROP_DOWN_COMBO_BOX, XmNcolumns, PRINTER_NAME_COLUMNS,
            XmNpositionMode, XmZERO_BASED, NULL);
        ...
        if (PSUB_PrintSetupMode(psub) == DtPRINT_SETUP_XP) {
                 ...
                 */
                XtAddCallback(PSUB_PrinterNameText(psub),
                ...
                if (printer_list != (String *)NULL) {
                        int count;
                        /*
                         * count the number of printer names in the returned
                         * list,
                         */
                        for (count = 0; printer_list[count] != (String)NULL;
                             count++)
                                ;
                        if (count > 0) {
                                 ...
                                 */
                                printer_xmstr_list = (XmString *)XtCalloc(
                                 ...
                                 */
                                combo_list = XtNameToWidget(
                                    PSUB_PrinterNameCombo(psub), "*List");
                                if (combo_list) {
                                         ...
                                         */
                                        XtVaGetValues(
                                            combo_list, XmNvisibleItemCount,
                                            &visible_item_count, NULL);
                                        /*
                                         * reduce the visible item count if
                                         * needed
                                         */
                                        if (count < visible_item_count)
                                                visible_item_count = count;
                                        /*
                                         * set the XmString printer list in the
                                         * combo box list
                                         */
                                        XtVaSetValues(combo_list, XmNitemCount,
                                                      count, XmNitems,
                                                      printer_xmstr_list,
                                                      XmNvisibleItemCount,
                                                      visible_item_count, NULL);
                ...
                if (hide_arrow) {
                        ...
                        XtVaGetValues(PSUB_PrinterNameText(psub),
                                      XmNshadowThickness,
                                      &text_shadow_thickness, NULL);
                        XtVaSetValues(PSUB_PrinterNameCombo(psub), XmNarrowSize,
                                      0, XmNarrowSpacing, 0, XmNmarginWidth, 0,
                                      XmNmarginHeight, 0, XmNshadowThickness, 0,
                                      NULL);
 ...
 *
 */
void _DtPrintSetupBoxCreatePrinterNameLabel(DtPrintSetupBoxWidget psub) {
        PSUB_PrinterNameLabel(psub) =
            CreateLabelGadget((Widget)psub, PRINTER_NAME_LABEL, "NameLabel");
}
 ...
 *
 */
void _DtPrintSetupBoxCreateSelectFileButton(DtPrintSetupBoxWidget psub) {
        PSUB_SelectFileButton(psub) = CreateButtonGadget(
            (Widget)psub, SELECT_FILE_LABEL, "SelectFile", XmTAB_GROUP);
 ...
 *
 */
void _DtPrintSetupBoxCreateSelectPrinterButton(DtPrintSetupBoxWidget psub) {
        PSUB_SelectPrinterButton(psub) = CreateButtonGadget(
            (Widget)psub, SELECT_PRINTER_LABEL, "SelectPrinter", XmTAB_GROUP);

        XtRemoveAllCallbacks(PSUB_SelectPrinterButton(psub),
                             XmNactivateCallback);
        XtAddCallback(PSUB_SelectPrinterButton(psub), XmNactivateCallback,
                      PrintSetupBoxCallback,
                      (XtPointer)DtPRINT_PRINTERS_BUTTON);
}
 ...
 *
 */
void _DtPrintSetupBoxCreateSetupButton(DtPrintSetupBoxWidget psub) {
 ...
 *
 */
void _DtPrintSetupBoxCreateTopSeparator(DtPrintSetupBoxWidget psub) {
        Arg al[10];
        register int ac = 0;

        XtSetArg(al[ac], XmNhighlightThickness, 0);
        ac++;
        PSUB_TopSeparator(psub) = XmCreateSeparatorGadget(
            (Widget)psub, "TopWorkAreaSeparator", al, ac);
}
 ...
 *
 */
XmGeoMatrix _DtPrintSetupBoxGeoMatrixCreate(Widget wid, Widget instigator,
        ...
        geoSpec->margin_w =
            BB_MarginWidth(psub) + psub->manager.shadow_thickness;
        geoSpec->margin_h =
            BB_MarginHeight(psub) + psub->manager.shadow_thickness;
        ...
        for (i = 0; i < psub->composite.num_children; i++) {
                Widget w = psub->composite.children[i];
                if (IsButton(w) && !IsAutoButton(psub, w) &&
                    w != PSUB_TopWorkArea(psub) &&
                    w != PSUB_BottomWorkArea(psub)) {
 ...
 *
 */
Boolean _DtPrintSetupBoxNoGeoRequest(XmGeoMatrix geoSpec) {
 ...
 *
 */
void _DtPrintSetupBoxGetCopies(Widget wid, int resource_offset,
                               XtArgVal *value) {
        DtPrintSetupBoxWidget psub = (DtPrintSetupBoxWidget)wid;
        int copies;

        if (PSUB_CopiesSpinBox(psub) != (Widget)NULL) {
                XtVaGetValues(PSUB_CopiesSpinBox(psub), XmNposition, &copies,
                              NULL);
 ...
 *
 */
void _DtPrintSetupBoxGetDescription(Widget wid, int resource_offset,
                                    XtArgVal *value) {
        DtPrintSetupBoxWidget psub = (DtPrintSetupBoxWidget)wid;
        XmString data;

        if (PSUB_Description(psub) != (Widget)NULL) {
                XtVaGetValues((Widget)PSUB_Description(psub), XmNlabelString,
                              &data, NULL);
                *value = (XtArgVal)data;
        } else {
                *value = (XtArgVal)NULL;
 ...
 *
 */
void _DtPrintSetupBoxGetFileName(Widget w, int resource_offset,
                                 XtArgVal *value) {
        DtPrintSetupBoxWidget psub = (DtPrintSetupBoxWidget)w;
 ...
 *
 */
void _DtPrintSetupBoxGetPrinterName(Widget w, int resource_offset,
                                    XtArgVal *value) {
        DtPrintSetupBoxWidget psub = (DtPrintSetupBoxWidget)w;
 ...
 *
 */
Widget DtCreatePrintSetupBox(Widget p, String name, ArgList args, Cardinal n) {
 ...
 *
 */
Widget DtCreatePrintSetupDialog(Widget parent, String name, ArgList args,
                                Cardinal n) {
        return XmeCreateClassDialog(dtPrintSetupBoxWidgetClass, parent, name,
                                    args, n);
}
 ...
 *
 */
DtPrintSetupData *DtPrintCopySetupData(DtPrintSetupData *target,
            ...
            target != (DtPrintSetupData *)NULL) {
                if ((source->printer_name == (String)NULL ||
                     target->printer_name == (String)NULL)
                        ? True
                        : strcmp(source->printer_name, target->printer_name) !=
                              0) {
                        XtFree(target->printer_name);
                        target->printer_name =
                            XtNewString(source->printer_name);
                }
 ...
 *
 */
XtEnum DtPrintFillSetupData(Widget w, DtPrintSetupData *print_data) {
        DtPrintSetupBoxWidget psub = (DtPrintSetupBoxWidget)w;
	...
	XPContext new_context;
                 ...
                 */
                status = _DtPrintVerifyXPrinter(NULL, print_data->printer_name,
                                                &new_printer_spec, &new_display
#if 0 && defined(PRINTING_SUPPORTED)
					,&new_context
#endif /* PRINTING_SUPPORTED */
                );
	    ...
	    print_data->print_context = new_context;
                ...
                if (print_data->printer_name != (String)NULL) {
                        if ((PSUB_PrinterName(psub) == NULL)
                                ? True
                                : strcmp(print_data->printer_name,
                                         PSUB_PrinterName(psub)) != 0) {
                                /*
                                 * if different, set the passed printer name
                                 * into the passed print setup box widget
                                 */
                                SynthSetPrinterName(
                                    psub, XtNewString(print_data->printer_name),
                ...
                if (status == DtPRINT_SUCCESS) {
                        ...
                        SetPSDDestination(psub, &psd);
                        /*
                         * update the passed print setup data struct based on
                         * the internal version of the print setup data struct
                         */
                        DtPrintCopySetupData(print_data, &psd);
                } else {
                        /*
                         * Unable to establish the printer connection; manage
                         * the print setup box in order to present the error box
                         * created by EstablishPrinter() to the user, allowing
                         * the user to select a new printer or cancel.
                         */
                        XtManageChild((Widget)psub);
                        /*
                         * if the default verifyPrintProc created an error box,
                         * display it. If the app installed its own verify proc,
                         * the app will need to check the return status of
                         * DtPrintFillSetupData, and manage its error dialog
                         * itself.
                         */
                        _DtPrintDefProcManageErrorBox(
                            &PSUB_DefaultProcData(psub));
                }
 ...
 *
 */
XtEnum DtPrintResetConnection(Widget w, DtPrintResetConnectionMode mode) {
        DtPrintSetupBoxWidget psub = (DtPrintSetupBoxWidget)w;
        _DtPrintWidgetToAppContext((Widget)psub);
