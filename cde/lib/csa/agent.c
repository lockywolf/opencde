...
extern void _DtCm_init_agent() {
        ...
        if (rpc_reg(_DtCm_transient, AGENTVERS, update_callback,
                    (const char *(*)())_DtCm_update_callback_1,
                    _DtCm_xdr_Table_Res_4, _DtCm_xdr_Update_Status,
                    "udp") == -1) {
...
Update_Status *_DtCm_update_callback_1(Table_Res_4 *t, _DtCm_Connection *conn) {
 ...
 */
void *cmcb_update_callback_2_svc(cmcb_update_callback_args *args,
                                 struct svc_req *d) {
        _CallbackInfo *cbi;
...
static CSA_return_code _ConvertCallbackData(cmcb_update_callback_args *args,
        ...
        if ((user = (CSA_calendar_user *)calloc(
        ...
        case CSA_CB_CALENDAR_LOGON:
                if ((ldata = (CSA_logon_callback_data *)calloc(
                         1, sizeof(CSA_logon_callback_data))) == NULL) {
                        free(user);
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
        ...
        case CSA_CB_CALENDAR_DELETED:
                if ((rdata = (CSA_calendar_deleted_callback_data *)calloc(
                         1, sizeof(CSA_calendar_deleted_callback_data))) ==
                    NULL) {
                        free(user);
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
        ...
        case CSA_CB_CALENDAR_ATTRIBUTE_UPDATED:
                if ((cdata = (CSA_calendar_attr_update_callback_data *)calloc(
                         1, sizeof(CSA_calendar_attr_update_callback_data))) ==
                    NULL) {
                        free(user);
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
        ...
        case CSA_CB_ENTRY_ADDED:
                if ((adata = (CSA_add_entry_callback_data *)calloc(
                         1, sizeof(CSA_add_entry_callback_data))) == NULL) {
                        free(user);
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
                ...
                if (args->data.data.adata->id &&
                    (adata->added_entry_id.data = (unsigned char *)strdup(
                         args->data.data.adata->id)) == NULL) {
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                } else
        ...
        case CSA_CB_ENTRY_DELETED:
                if ((ddata = (CSA_delete_entry_callback_data *)calloc(
                         1, sizeof(CSA_delete_entry_callback_data))) == NULL) {
                        free(user);
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
                ...
                if (args->data.data.ddata->id &&
                    (ddata->deleted_entry_id.data = (unsigned char *)strdup(
                         args->data.data.ddata->id)) == NULL) {
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                } else
        ...
        case CSA_CB_ENTRY_UPDATED:
                if ((udata = (CSA_update_entry_callback_data *)calloc(
                         1, sizeof(CSA_update_entry_callback_data))) == NULL) {
                        free(user);
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
                ...
                if (args->data.data.udata->newid &&
                    (udata->new_entry_id.data = (unsigned char *)strdup(
                         args->data.data.udata->newid)) == NULL) {
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                } else
                ...
                if (args->data.data.udata->oldid &&
                    (udata->old_entry_id.data = (unsigned char *)strdup(
                         args->data.data.udata->oldid)) == NULL) {
                        _FreeCallbackInfo(ncbi);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                } else
                        ...
                        for (cb = cal->cb_list; cb != NULL; cb = cb->next) {
                                if (cal->do_reasons & cb->reason) {
                                        cb->handler(
                                            ...
                                            (CSA_buffer)NULL, cb->client_data,
                                            (CSA_extension *)NULL);
                                }
                        }
                } else if (cal->all_reasons &
                           (CSA_CB_ENTRY_ADDED | CSA_CB_ENTRY_DELETED |
                            CSA_CB_ENTRY_UPDATED))
                        keep = B_TRUE;
        }
...
static void _FreeCallbackInfo(_CallbackInfo *ptr) {
        ...
        if (ptr) {
                ...
                if (ptr->data)
                        ...
                        case CSA_CB_ENTRY_ADDED:
                                adata =
                                    (CSA_add_entry_callback_data *)ptr->data;
                                if (adata->user)
                                        free(adata->user);
                        ...
                        case CSA_CB_ENTRY_DELETED:
                                ddata =
                                    (CSA_delete_entry_callback_data *)ptr->data;
                                if (ddata->date_and_time)
                                        free(ddata->date_and_time);
                                if (ddata->user)
                                        free(ddata->user);
                        ...
                        case CSA_CB_ENTRY_UPDATED:
                                udata =
                                    (CSA_update_entry_callback_data *)ptr->data;
                                if (udata->user)
                                        free(udata->user);
                                if (udata->date_and_time)
                                        free(udata->date_and_time);
