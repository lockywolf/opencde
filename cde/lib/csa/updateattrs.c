...
_DtCmUpdateAttributes(uint numsrc, cms_attribute *srcattrs, uint *numdst,
                      cms_attribute **dstattrs, _DtCmNameTable **tbl,
                      boolean_t caltbl, int **types, boolean_t makecopy) {
        ...
        if (makecopy && *numdst > 0 &&
            (stat = _DtCm_copy_cms_attributes(*numdst, *dstattrs, &oldnum,
        ...
        for (i = 0; i < numsrc && stat == CSA_SUCCESS; i++) {
                ...
                if (index < 0 || index > (*tbl)->size) {
                        ...
                        if ((stat = _DtCmExtendNameTable(
                                 srcattrs[i].name.name, index > 0 ? index : 0,
                        ...
                        if ((*tbl)->names[index] == NULL) {
                                ...
                                if ((stat = _DtCm_add_name_to_table(
                                         *tbl, index, srcattrs[i].name.name)) !=
                                    CSA_SUCCESS)
                                        break;
...
extern CSA_return_code _DtCmUpdateAttribute(cms_attribute *from,
                                            cms_attribute *to) {
        CSA_return_code stat = CSA_SUCCESS;
        ...
        if (from->value) {
                ...
                case CSA_VALUE_UINT32:
                        stat =
                            _DtCmUpdateSint32AttrVal(from->value, &to->value);
                        break;
                ...
                case CSA_VALUE_TIME_DURATION:
                        stat =
                            _DtCmUpdateStringAttrVal(from->value, &to->value);
                        break;

                case CSA_VALUE_REMINDER:
                        stat =
                            _DtCmUpdateReminderAttrVal(from->value, &to->value);
                        break;
...
extern CSA_return_code
_DtCmUpdateAccessListAttrVal(cms_attribute_value *newval,
...
extern CSA_return_code _DtCmUpdateSint32AttrVal(cms_attribute_value *newval,
                                                cms_attribute_value **attrval) {
        cms_attribute_value *val;
...
extern CSA_return_code _DtCmUpdateStringAttrVal(cms_attribute_value *newval,
                                                cms_attribute_value **attrval) {
        cms_attribute_value *val;
        char *newstring = NULL;
...
extern CSA_return_code
_DtCmUpdateReminderAttrVal(cms_attribute_value *newval,
                           cms_attribute_value **attrval) {
...
extern CSA_return_code
_DtCmUpdateDateTimeListAttrVal(cms_attribute_value *newval,
                               cms_attribute_value **attrval) {
        cms_attribute_value *val;
        ...
        if (newval && newval->item.date_time_list_value) {
                ...
                *attrval = val;
                ...
                _DtCm_free_date_time_list(
                    (*attrval)->item.date_time_list_value);
                free(*attrval);
                *attrval = NULL;
        }
...
extern CSA_return_code
_DtCmUpdateOpaqueDataAttrVal(cms_attribute_value *newval,
                             cms_attribute_value **attrval) {
        CSA_return_code stat;
        cms_attribute_value *val;
