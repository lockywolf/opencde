...
extern CSA_return_code _DtCm_make_libentry(cms_entry *e,
                                           _DtCm_libentry **entry_r) {
        _DtCm_libentry *ptr;
        ...
        if ((ptr = (_DtCm_libentry *)calloc(1, sizeof(_DtCm_libentry))) == NULL)
...
_DtCm_get_entry_attrs_by_name(_DtCm_libentry *entry, CSA_uint32 num_names,
        ...
        for (i = 0, j = 0; i < num_names; i++) {
                if (names[i] != NULL) {
                        ...
                        if (index >= 0 && entry->e->attrs[index].value) {
                                if (attrs_r[j].name = strdup(
                                        entry->e->attrs[index].name.name)) {
...
extern CSA_return_code
_DtCmCmsentriesToLibentries(_DtCmNameTable **tbl, cms_entry *entries,
...
extern CSA_return_code _DtCm_appt4_to_libentries(char *calname, Appt_4 *appt4,
                                                 _DtCm_libentry **libentries) {
        CSA_return_code stat = CSA_SUCCESS;
        _DtCm_libentry *entry, *head, *prev;
        ...
        while (appt4 != NULL) {
                ...
                if ((stat = _DtCm_appt4_to_attrs(
                         calname, appt4, entry->e->num_attrs, entry->e->attrs,
                         B_FALSE)) != CSA_SUCCESS)
                        break;
...
extern CSA_return_code _DtCm_libentries_to_appt4(_DtCm_libentry *entries,
                                                 Appt_4 **appt4) {
        CSA_return_code stat = CSA_SUCCESS;
        Appt_4 *a4, *head, *prev;
        ...
        while (entries != NULL) {
                ...
                if ((stat = _DtCm_cms_entry_to_appt4(entries->e, &a4)) !=
                    CSA_SUCCESS)
                        break;
...
extern CSA_return_code
_DtCm_reminder4_to_csareminder(Reminder_4 *r4, CSA_uint32 *num_rems,
...
extern CSA_return_code
_DtCm_cms2csa_reminder_ref(cms_reminder_ref *cmsrems, CSA_uint32 *num_rems,
        ...
        while (cmsrems != NULL && stat == CSA_SUCCESS) {
                ...
                if ((rem_r[i].attribute_name =
                         strdup(cmsrems->reminder_name)) == NULL) {
                        stat = CSA_E_INSUFFICIENT_MEMORY;
                        break;
                }
                ...
                if ((stat = _DtCm_make_libentry(NULL, &entry)) == CSA_SUCCESS) {
                        ...
                        stat = _DtCm_set_opaque_attrval(
                            &opq,
                            &entry->e
                                 ->attrs[CSA_ENTRY_ATTR_REFERENCE_IDENTIFIER_I]
                                 .value);
...
extern void _DtCm_free_entry_content(uint dummy, _DtCm_libentry *entry) {
        _DtCm_remove_from_entry_list(entry->cal, (caddr_t)entry,
                                     (caddr_t)entry);
        if (entry->e)
                _DtCm_free_cms_entry(entry->e);
 ...
 */
extern void _DtCm_free_libentries_from_list(_DtCm_libentry *head,
                                            _DtCm_libentry *tail) {
 ...
 */
extern void _DtCm_free_libentries(_DtCm_libentry *entry) {
        _DtCm_libentry *nptr;
 ...
 */
extern void _DtCm_free_reminder_references(uint num_rems,
                                           CSA_reminder_reference *rems) {
...
extern void _DtCm_free_entry_handles(uint num_entries,
...
static CSA_return_code _CmsentryToLibentry(_DtCmNameTable **tbl, cms_entry *e,
        ...
        if ((stat = _DtCmUpdateAttributes(
                 e->num_attrs, e->attrs, &entry->e->num_attrs, &entry->e->attrs,
                 tbl, B_FALSE, NULL, B_FALSE)) != CSA_SUCCESS) {
                _DtCm_free_libentries(entry);
        } else {
                entry->e->key = e->key;
                entry->filled = B_TRUE;
