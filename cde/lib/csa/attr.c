...
_DtCm_check_cal_csa_attributes(int fvers, uint num_attrs, CSA_attribute *attrs,
        ...
        stat = _DtCm_check_hashed_cal_attributes(
            fvers, hnum, hattrs, NULL, cname, checkreadonly, firsttime, B_TRUE);
...
extern CSA_return_code _DtCm_copy_cms_attr_val(cms_attribute_value *from,
        ...
        if (from == NULL)
                ...
                if ((val = (cms_attribute_value *)calloc(
                         1, sizeof(cms_attribute_value))) == NULL)
                        return (CSA_E_INSUFFICIENT_MEMORY);
                        ...
                        if (from->item.date_time_list_value &&
                            (val->item.date_time_list_value =
                                 _DtCm_copy_date_time_list(
                                     from->item.date_time_list_value)) ==
                                NULL) {
...
extern CSA_return_code _DtCm_cms2csa_attrval(cms_attribute_value *from,
        ...
        if (from == NULL)
                ...
                if ((val = (CSA_attribute_value *)calloc(
                         1, sizeof(CSA_attribute_value))) == NULL)
                        return (CSA_E_INSUFFICIENT_MEMORY);
...
_DtCm_set_csa_access_attrval(cms_access_entry *aval,
        ...
        if ((val = (CSA_attribute_value *)malloc(
...
extern CSA_return_code _DtCm_set_opaque_attrval(CSA_opaque_data *data,
        ...
        if ((stat = _DtCm_copy_opaque_data(
                 data, &val->item.opaque_data_value)) != CSA_SUCCESS) {
                free(val);
                return (stat);
        } else {
...
extern CSA_return_code _DtCm_cms2csa_access_list(cms_access_entry *cmslist,
        ...
        head = prev = NULL;
        while (cmslist != NULL) {
                if ((to = (CSA_access_rights *)calloc(
                         1, sizeof(CSA_access_rights))) == NULL) {
                        stat = CSA_E_INSUFFICIENT_MEMORY;
                        break;
                }
...
extern CSA_date_time_list _DtCm_copy_date_time_list(CSA_date_time_list dlist) {
        ...
        prev = head = NULL;
        while (dlist != NULL) {
                if ((l = (CSA_date_time_entry *)calloc(
                         1, sizeof(CSA_date_time_entry))) == NULL) {
                        cleanup = B_TRUE;
                        break;
                }
...
extern CSA_return_code _DtCm_copy_reminder(CSA_reminder *from,
        ...
        if (from->reminder_data.size > 0) {
                ...
                if ((newval->reminder_data.data =
                         malloc(newval->reminder_data.size)) == NULL) {
                        _DtCm_free_reminder(newval);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
...
check_predefined_attrs(int fver, uint num_attrs, cms_attribute *attrs,
        ...
        for (i = 0; i < num_attrs; i++) {
                ...
                if (tbl == NULL)
                        ...
                        index =
                            _DtCm_get_index_from_table(tbl, attrs[i].name.name);
...
static CSA_return_code convert_cms_user_to_csa_user(char *from,
        ...
        if ((newval = (CSA_calendar_user *)calloc(
                 1, sizeof(CSA_calendar_user))) == NULL)
                return (CSA_E_INSUFFICIENT_MEMORY);
...
static CSA_return_code hash_entry_attrs(uint num_attrs, CSA_attribute *csaattrs,
        ...
        if ((nattrs = (cms_attribute *)calloc(
                 1, sizeof(cms_attribute) *
                        (num_attrs + _DtCM_DEFINED_ENTRY_ATTR_SIZE + 1))) ==
            NULL)
                return (CSA_E_INSUFFICIENT_MEMORY);
    ...
    boolean_t checkreadonly, boolean_t firsttime, boolean_t csatype) {
        ...
        if (firsttime) {
                if (attrs[CSA_CAL_ATTR_CALENDAR_OWNER_I].name.name) {
                        if (attrs[CSA_CAL_ATTR_CALENDAR_OWNER_I].value ==
                                NULL ||
                        ...
                        if (csatype) {
                                ...
                                stat = _CheckCalendarOwner(
                                    ...
                                    (csaval && csaval->item.calendar_user_value
                                         ? csaval->item.calendar_user_value
                                               ->user_name
                                         : NULL));
                        } else {
                                cmsval =
                ...
                if (attrs[CSA_CAL_ATTR_CALENDAR_NAME_I].name.name) {
                        if (attrs[CSA_CAL_ATTR_CALENDAR_NAME_I].value == NULL ||
                            attrs[CSA_CAL_ATTR_CALENDAR_NAME_I]
        ...
        stat = check_predefined_attrs(
            fvers, num_attrs + 1, attrs, checkreadonly, NULL,
...
static CSA_return_code _DtCm_check_hashed_entry_attributes(int fvers,
        ...
        if ((stat = check_predefined_attrs(
                 fvers, num_attrs + 1, attrs, (utype == 0 ? B_FALSE : B_TRUE),
                 NULL, _DtCM_DEFINED_ENTRY_ATTR_SIZE, _CSA_entry_attr_info)) !=
            CSA_SUCCESS) {
