...
#ifdef SunOS
extern enum clnt_stat _DtCm_clnt_call P((_DtCm_Connection * conn, u_long proc,
                                         xdrproc_t inproc, caddr_t in,
                                         xdrproc_t outproc, caddr_t out,
                                         struct timeval tout));
#endif

extern CSA_return_code _DtCm_add_registration P((_DtCm_Client_Info * ci,
                                                 char *cal,
                                                 unsigned long update_type));

extern void _DtCm_remove_registration P((_DtCm_Client_Info * ci, char *cal));

extern CSA_return_code _DtCm_create_udp_client P((char *host, int version,
                                                  int timeout,
                                                  _DtCm_Client_Info **clnt));

extern CSA_return_code _DtCm_create_tcp_client P((char *host, int version,
                                                  int timeout,
                                                  _DtCm_Client_Info **clnt));
