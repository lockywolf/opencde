...
extern CSA_return_code _DtCm_appt4_to_attrs(char *calname, Appt_4 *a4,
        ...
        while (stat == CSA_SUCCESS && reminders != NULL) {
                if ((index = _DtCm_old_reminder_name_to_index(
                         reminders->attr)) >= 0) {
                        if ((stat = _DtCmAttr4ToReminderValue(
                                 reminders, &attrs[index].value)) !=
                            CSA_SUCCESS) {
                                break;
                        }
                }
        ...
        if (a4->appt_id.key > 0) {
                ...
                if (ptr1) {
                        snprintf(buf, sizeof buf, "%ld:%s%s%s", a4->appt_id.key,
                                 calname, (ptr2 == NULL ? "." : ""),
                                 (ptr2 == NULL ? _DtCmGetLocalDomain(ptr1 + 1)
                                               : ""));
                } else {
                        snprintf(buf, sizeof buf, "%ld:%s@%s", a4->appt_id.key,
                                 calname, _DtCmGetHostAtDomain());
                }
                opq.size = strlen(buf);
            ...
            (a4->appt_id.key > 0 &&
             (stat = _DtCm_set_opaque_attrval(
                ...
                ;
                      ...
                      _csa_tick_to_iso8601(a4->appt_id.tick + a4->duration, buf)
                          ? CSA_E_INVALID_DATE_TIME
                          : CSA_SUCCESS) ||
                 (stat = _DtCm_set_string_attrval(
                ...
                ;
                 ...
                 ((stat = _DtCm_set_uint32_attrval(
                  ...
                  (a4->period.period >= everyNthDay_4 &&
                   ...
                   (stat = _DtCm_set_uint32_attrval(
                        a4->period.nth,
                        &attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_INTERVAL_I]
                             .value))) ||
                  (a4->period.enddate != 0 &&
                   ((stat = _csa_tick_to_iso8601(a4->period.enddate, buf)
                    ...
                    (stat = _DtCm_set_string_attrval(
                         buf,
                         &attrs[CSA_X_DT_ENTRY_ATTR_SEQUENCE_END_DATE_I].value,
                         CSA_VALUE_DATE_TIME))))))
                ;
        else {
                return (CSA_SUCCESS);
...
static CSA_return_code eventtype4_to_attrs(Event_Type_4 tag, uint num_attrs,
        ...
        case toDo_4:
                return (_DtCm_set_uint32_attrval(
                    CSA_TYPE_TODO, &attrs[CSA_ENTRY_ATTR_TYPE_I].value));
        ...
        case holiday_4:
                ...
                return (_DtCm_set_string_attrval(
                    CSA_SUBTYPE_HOLIDAY, &attrs[CSA_ENTRY_ATTR_SUBTYPE_I].value,
                    CSA_VALUE_STRING));
        ...
        default:
                return (_DtCm_set_uint32_attrval(
                    CSA_X_DT_TYPE_OTHER, &attrs[CSA_ENTRY_ATTR_TYPE_I].value));
        }
}
...
static CSA_return_code _RepeatInfoToRule(Appt_4 *a4,
        ...
        if (a4->period.period == single_4 ||
            a4->period.period == otherPeriod_4) {
                *attrval = NULL;
                return (CSA_SUCCESS);
        }
        ...
        case everyNthWeek_4:
                ...
                break;
        case everyNthMonth_4:
                snprintf(buf, sizeof buf, "MD%d #%d ", a4->period.nth,
                         duration);
                break;
        case monThruFri_4:
