...
csa_list_calendars(CSA_service_reference calendar_service,
                   CSA_uint32 *number_names, CSA_calendar_user **calendar_names,
 ...
 */
extern CSA_return_code csa_logon(CSA_service_reference calendar_service,
                                 CSA_calendar_user *user, CSA_string password,
 ...
 */
extern CSA_return_code
csa_add_calendar(CSA_session_handle session, CSA_calendar_user *user,
        ...
        if (number_attributes > 0 &&
            (stat = _DtCm_check_cal_csa_attributes(
                 cal->rpc_version - 1, number_attributes, calendar_attributes,
                 user->calendar_address, B_TRUE, B_TRUE, B_FALSE)) !=
                CSA_SUCCESS) {
                return (stat);
        }
 ...
 * logoff_extensions
 */
extern CSA_return_code csa_logoff(CSA_session_handle session,
 ...
 */
extern CSA_return_code
csa_delete_calendar(CSA_session_handle session,
...
extern CSA_return_code csa_list_calendar_attributes(
    CSA_session_handle session, CSA_uint32 *number_names,
    ...
    CSA_extension *list_calendar_attributes_extensions) {
        ...
        if (cal->rpc_version < _DtCM_FIRST_EXTENSIBLE_SERVER_VERSION) {
                return (_DtCm_list_old_cal_attr_names(
                    cal, number_names, calendar_attributes_names));
        } else
                return (_DtCm_rpc_list_calendar_attributes(
                    cal, number_names, calendar_attributes_names));
}

extern CSA_return_code csa_save(CSA_session_handle session,
                                CSA_string archive_name,
                                CSA_uint32 number_attributes,
                                CSA_attribute *attributes, CSA_enum *operators,
...
extern CSA_return_code
csa_restore(CSA_session_handle session, CSA_string archive_name,
            CSA_uint32 number_attributes, CSA_attribute *attributes,
            CSA_enum *operators, CSA_extension *restore_extensions) {
 ...
 */
extern CSA_return_code
csa_list_entries(CSA_session_handle session, CSA_uint32 number_attributes,
                 CSA_attribute *entry_attributes, CSA_enum *list_operators,
                 CSA_uint32 *number_entries, CSA_entry_handle **entries,
        ...
        if (list_operators && (stat = _DtCm_check_operator(
                                   number_attributes, entry_attributes, NULL,
                                   list_operators)) != CSA_SUCCESS) {

                return (stat);
        }

        /* check data type */
        if ((stat = _DtCm_check_entry_attributes(
                 cal->file_version, number_attributes, entry_attributes, 0,
                 B_FALSE)) != CSA_SUCCESS) {
                /*
        ...
        if ((stat = _DtCm_rpc_lookup_entries(cal, number_attributes,
                                             entry_attributes, list_operators,
                                             &elist)) == CSA_SUCCESS) {

                if (elist) {
                        *number_entries =
                            _DtCm_add_to_entry_list(cal, (caddr_t)elist);
...
extern CSA_return_code
csa_list_entry_attributes(CSA_session_handle session, CSA_entry_handle entryh,
...
extern CSA_return_code csa_read_entry_attributes(
    CSA_session_handle session, CSA_entry_handle entryh,
    CSA_uint32 number_names, CSA_attribute_reference *attribute_names,
    CSA_uint32 *number_attributes, CSA_attribute **entry_attributes,
    CSA_extension *read_entry_attributes_extensions) {
        ...
        if (number_names > 0) {
                return (_DtCm_get_entry_attrs_by_name(
                    entry, number_names, attribute_names, number_attributes,
                    entry_attributes));
        } else {
...
extern CSA_return_code csa_free(CSA_buffer memory) {
        DP(("api.c: csa_free\n"));
...
extern CSA_return_code
csa_look_up(CSA_session_handle session, CSA_calendar_user *users,
            CSA_flags look_up_flags, CSA_uint32 *number_users,
            CSA_calendar_user **user_list, CSA_extension *look_up_extensions) {
...
extern CSA_return_code
csa_query_configuration(CSA_session_handle session, CSA_enum item,
...
extern CSA_return_code csa_read_calendar_attributes(
    CSA_session_handle session, CSA_uint32 number_names,
    CSA_attribute_reference *attribute_names, CSA_uint32 *number_attributes,
    CSA_attribute **calendar_attributes,
    CSA_extension *read_calendar_attributes_extensions) {
        ...
        if (number_names > 0) {
                return (_DtCm_get_cal_attrs_by_name(
                    cal, number_names, attribute_names, number_attributes,
                    calendar_attributes));
        } else {
...
extern CSA_return_code
csa_register_callback(CSA_session_handle session, CSA_flags reason,
                      CSA_callback callback, CSA_buffer client_data,
        ...
        if ((cb_entry = (_DtCmCallbackEntry *)malloc(
                 sizeof(_DtCmCallbackEntry))) == NULL)
                return (CSA_E_INSUFFICIENT_MEMORY);
 ...
 */
extern CSA_return_code
csa_unregister_callback(CSA_session_handle session, CSA_flags reason,
                        CSA_callback handler, CSA_buffer client_data,
...
CSA_return_code csa_call_callbacks(CSA_session_handle session, CSA_flags reason,
...
extern CSA_return_code csa_update_calendar_attributes(
    CSA_session_handle session, CSA_uint32 num_attrs, CSA_attribute *attrs,
    CSA_extension *update_calendar_attributes_extensions) {
        ...
        if ((cal->file_version >= _DtCM_FIRST_EXTENSIBLE_DATA_VERSION &&
             !(cal->access &
               (CSA_OWNER_RIGHTS | CSA_INSERT_CALENDAR_ATTRIBUTES |
        ...
        /* check data type */
        if ((stat = _DtCm_check_cal_csa_attributes(
                 cal->file_version, num_attrs, attrs, cal->name, B_TRUE,
                 B_FALSE, B_TRUE)) != CSA_SUCCESS) {
                return (stat);
        }
...
extern CSA_return_code csa_add_entry(CSA_session_handle session,
        ...
        if ((stat = _DtCm_check_entry_attributes(cal->file_version, num_attrs,
                                                 attrs, CSA_CB_ENTRY_ADDED,
                                                 B_TRUE)) != CSA_SUCCESS) {
                return (stat);
        }
        ...
        if ((stat = _DtCm_rpc_insert_entry(cal, num_attrs, attrs, &entry)) ==
            CSA_SUCCESS) {
                ...
                if (entry_r != NULL) {
                        if ((entry = _DtCm_convert_entry_wheader(entry)) ==
                            NULL)
 ...
 */
extern CSA_return_code
csa_delete_entry(CSA_session_handle session, CSA_entry_handle entryh,
...
extern CSA_return_code
csa_free_time_search(CSA_session_handle session,
                     CSA_date_time_range date_time_range,
                     CSA_time_duration time_duration, CSA_uint32 number_users,
                     CSA_calendar_user *users, CSA_free_time **free_time,
...
extern CSA_return_code
csa_list_entry_sequence(CSA_session_handle session, CSA_entry_handle entryh,
        ...
        if (entry->cal->file_version < _DtCM_FIRST_EXTENSIBLE_DATA_VERSION) {
                ...
                if (rtype->value == NULL || rtype->value->item.sint32_value ==
        ...
        if (start == 0 && end == 0 &&
            (rtimes->value &&
             rtimes->value->item.uint32_value == CSA_X_DT_DT_REPEAT_FOREVER))
                return (CSA_E_INVALID_PARAMETER);
        ...
        if ((stat = _DtCm_rpc_enumerate_sequence(entry->cal, entry, start, end,
                ...
                if (elist) {
                        *number_entries =
                            _DtCm_add_to_entry_list(entry->cal, (caddr_t)elist);
                        stat = _DtCm_libentry_to_entryh(elist, number_entries,
                                                        entries);
                }
        }
 ...
 */
extern CSA_return_code
csa_read_next_reminder(CSA_session_handle session, CSA_uint32 number_names,
                       CSA_attribute_reference *reminder_names,
                       CSA_date_time given_time, CSA_uint32 *number_reminders,
        ...
        /* lookup reminders */
        if ((stat = _DtCm_rpc_lookup_reminder(cal, timeval, number_names,
                                              reminder_names, number_reminders,
                                              reminders)) == CSA_SUCCESS) {
 ...
 */
extern CSA_return_code
csa_update_entry_attributes(CSA_session_handle session, CSA_entry_handle entry,
                            CSA_enum scope, CSA_boolean update_propagation,
                            CSA_uint32 num_attrs, CSA_attribute *attrs,
        ...
        /* check data type, readonly attributes, etc */
        if ((stat = _DtCm_check_entry_attributes(
                 oentry->cal->file_version, num_attrs, attrs,
                 CSA_CB_ENTRY_UPDATED, B_TRUE)) != CSA_SUCCESS) {
                return (stat);
        }
...
/* ADD EVENT */
extern CSA_return_code csa_add_event(
    CSA_service_reference calendar_service, CSA_string calendar_address,
    CSA_string logon_user, CSA_string logon_password, CSA_string attendee,
    CSA_enum attendee_priority, CSA_enum attendee_status,
    CSA_boolean attendee_rsvp_requested, CSA_date_time start_date,
    CSA_date_time end_date, CSA_string organizer, CSA_string sponsor,
    CSA_string summary, CSA_string description, CSA_string recurrence_rule,
    CSA_string exception_rule, CSA_string subtype, CSA_enum classification,
    CSA_string delimiters, CSA_string add_event_extensions) {
...
/* ADD TODO */
CSA_return_code
csa_add_todo(CSA_service_reference calendar_service,
             CSA_string calendar_address, CSA_string logon_user,
             CSA_string logon_password, CSA_enum attendee_priority,
             CSA_enum attendee_status, CSA_boolean attendee_rsvp_requested,
             CSA_date_time start_date, CSA_date_time due_date,
             CSA_uint32 priority, CSA_string summary, CSA_string description,
             CSA_enum classification, CSA_string delimiters,
...
/* ADD MEMO */
CSA_return_code csa_add_memo(CSA_service_reference calendar_service,
                             CSA_string calendar_address, CSA_string logon_user,
                             CSA_string logon_password,
                             CSA_date_time start_date, CSA_string summary,
