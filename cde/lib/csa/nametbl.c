...
extern _DtCmNameTable *_DtCm_make_name_table(int size, char **names) {
        _DtCmNameTable *tbl;
        int i;
        ...
        if ((tbl->names = (char **)malloc(sizeof(char *) * (size + 1))) ==
            NULL) {
 ...
 * table by (index - size)
 */
extern CSA_return_code _DtCm_add_name_to_table(_DtCmNameTable *tbl, int index,
                                               char *newname) {
        int *ptr;
        char **newptr;
        ...
        if (index > tbl->size) {
                if ((newptr = (char **)realloc(
                         tbl->names, sizeof(char *) * (index + 1))) == NULL) {
                        *ptr = -1;
...
extern int _DtCm_get_index_from_table(_DtCmNameTable *tbl, char *name) {
        int *ptr;
 ...
 * table by (index - size)
 */
extern CSA_return_code _DtCmExtendNameTable(char *name, int index, int type,
                                            _DtCmNameTable *base, int basesize,
                                            char **basenames,
                                            _DtCmNameTable **tbl, int **types) {
        _DtCmNameTable *ntbl;
        int *newarray;
        int newindex;
        ...
        if (*tbl == base) {
                if ((ntbl = _DtCm_make_name_table(basesize, basenames)) ==
                    NULL) {
        ...
        if (types) {
                ...
                if ((newarray = (int *)realloc(
                         *types, sizeof(int) * (newindex + 1))) == NULL) {
                        return (CSA_E_INSUFFICIENT_MEMORY);
                } else {
                        *types = newarray;
                        (*types)[newindex] = type;
