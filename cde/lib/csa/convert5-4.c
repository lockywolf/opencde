...
_DtCm_cmsattrs_to_apptdata(uint size, cms_attribute *attrs, Appt_4 *appt) {
        ...
        if ((stat = _DtCm_attrs_to_eventtype4(
                 ...
                 attrs[CSA_ENTRY_ATTR_SUBTYPE_I].value, appt->tag)) !=
            CSA_SUCCESS)
                return (stat);
        ...
        if (aptr->value && aptr->value->item.reminder_value) {
                if ((stat = _DtCm_add_reminder(_DtCM_OLD_ATTR_BEEP_REMINDER,
                                               aptr->value->item.reminder_value,
                                               appt)) != CSA_SUCCESS)
                        return (stat);
        }
        ...
        if (_DtCm_rtype_to_interval4(attrs[CSA_X_DT_ENTRY_ATTR_REPEAT_TYPE_I]
                                         .value->item.sint32_value,
                                     &appt->period.period))
                return (CSA_E_INVALID_ATTRIBUTE_VALUE);
 ...
 */
extern CSA_return_code _DtCm_attrs_to_apptdata(uint size, CSA_attribute *attrs,
                                               Appt_4 *appt) {
        CSA_return_code stat = CSA_SUCCESS;
        ...
        for (i = 0; i < size && stat == CSA_SUCCESS; i++) {
                ...
                if ((stat = _DtCm_get_old_attr_by_name(
                         attrs[i].name, &oldattr)) != CSA_SUCCESS)
                        break;
        ...
        if (stat == CSA_SUCCESS) {
                ...
                if (tindex || stindex) {
                        ...
                        stat = _DtCm_attrs_to_eventtype4(
                            (cms_attribute_value *)(tindex ? attrs[tindex].value
                                                           : &otype),
                            (cms_attribute_value *)(stindex
                                                        ? attrs[stindex].value
                                                        : NULL),
                            appt->tag);
                }
...
extern CSA_return_code _DtCm_cms_entry_to_appt4(cms_entry *entry,
                                                Appt_4 **appt4_r) {
...
_DtCm_attrs_to_appt4(uint num_attrs, CSA_attribute *attrs, Appt_4 **appt4_r) {
        ...
        if ((stat = _DtCm_attrs_to_apptdata(num_attrs, attrs, appt)) !=
            CSA_SUCCESS) {
                _DtCm_free_appt4(appt);
                return (stat);
        } else {
...
extern CSA_return_code _DtCm_scope_to_options4(CSA_enum scope, Options_4 *opt) {
        if (opt == NULL)
                return (CSA_E_INVALID_PARAMETER);
...
extern CSA_return_code _DtCm_csaaccesslist_toaccessentry4(CSA_access_list alist,
                                                          Access_Entry_4 **a4) {
        CSA_return_code stat = CSA_SUCCESS;
...
extern int _DtCm_rtype_to_interval4(int val, Interval_4 *period) {
        switch (val) {
...
extern int _DtCm_status_to_apptstatus4(int val, Appt_Status_4 *astat) {
        switch (val) {
...
extern int _DtCm_classification_to_privacy4(int val, Privacy_Level_4 *privacy) {
        switch (val) {
...
extern CSA_return_code _DtCm_attrs_to_eventtype4(cms_attribute_value *type,
 ...
 */
extern CSA_return_code _DtCm_add_reminder(char *rem, CSA_reminder *val,
                                          Appt_4 *appt) {
        ...
        */
        ...
        if (val->reminder_data.size > 0) {
                if ((newattr->clientdata =
                         malloc(val->reminder_data.size + 1)) == NULL) {
                        _DtCm_free_attr4(newattr);
                        return (CSA_E_INSUFFICIENT_MEMORY);
                }
