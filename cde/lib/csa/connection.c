...
extern CSA_return_code _DtCm_create_udp_client(char *host, int version,
                                               int timeout,
        ...
        cl = clnt_create_vers(host, TABLEPROG, &vers_out, TABLEVERS_2, version,
                              "udp");
        if (cl == NULL) {
 ...
 */
extern CSA_return_code _DtCm_create_tcp_client(char *host, int version,
                                               int timeout,
                                               _DtCm_Client_Info **clnt) {
        CSA_return_code stat;
        _DtCm_Client_Info *ci;
...
#if defined(SunOS) || defined(USL) || defined(__uxp__)
                cl = clnt_create_vers(host, TABLEPROG, &vers_out, TABLEVERS_2,
                                      version, "tcp");
#else
 ...
 */
extern enum clnt_stat _DtCm_clnt_call(_DtCm_Connection *conn, u_long proc,
                                      xdrproc_t inproc, caddr_t in,
                                      xdrproc_t outproc, caddr_t out,
                                      struct timeval tout) {
        _DtCm_Client_Info *ci;
        ...
        while (B_TRUE) {
                ...
                status =
                    clnt_call((ttype == tcp_transport ? ci->tcpcl : ci->udpcl),
        ...
        if (status != RPC_SUCCESS && conn->ci != NULL) {
                _DtCm_print_errmsg(clnt_sperror(
                    (ttype == tcp_transport ? ci->tcpcl : ci->udpcl),
                    ci->host));
        }
        conn->stat = status;
        return status;
...
extern CSA_return_code _DtCm_add_registration(_DtCm_Client_Info *ci, char *cal,
        ...
        if ((listitem = (_DtCm_Target_List *)calloc(
                 1, sizeof(_DtCm_Target_List))) == NULL)
                return (CSA_E_INSUFFICIENT_MEMORY);
...
extern void _DtCm_remove_registration(_DtCm_Client_Info *ci, char *cal) {
        _DtCm_Target_List *listp, *prev;
        _DtCm_Client_Info *c;
        int result;
...
extern CSA_return_code _DtCm_get_server_rpc_version(char *host, int *version) {
        CSA_return_code stat;
        _DtCm_Client_Info *ci;
        ...
        if ((stat = _DtCm_create_tcp_client(
                 host, TABLEVERS, _DtCM_INITIAL_TIMEOUT, &ci)) == CSA_SUCCESS)
                *version = ci->vers_out;
...
static void insert_client_info(_DtCm_Client_Info *ci) {
        ...
        fprintf(stderr, "%s: head = %d, tail = %d, newitem = %d\n",
                "insert_client_info", client_cache_head, client_cache_tail, ci);
...
static void delete_client_info(_DtCm_Client_Info *oldci) {
        ...
        fprintf(stderr, "%s: head = %d, tail = %d, olditem = %d\n",
                "delete_client_info", client_cache_head, client_cache_tail,
                oldci);
#endif
}
 ...
 */
static CSA_return_code get_client_handle(const char *host,
...
#if defined(__hpux)
                if ((cl = clnt_create((char *)host, prognum, vers, nettype)) !=
                    NULL) {
#else
                ...
                if ((cl = clnt_create(host, prognum, vers, nettype)) != NULL) {
                        ...
                        status =
                            clnt_call(cl, 0, (xdrproc_t)xdr_void, (char *)NULL,
                                      (xdrproc_t)xdr_void, (char *)NULL, tv);
                        ...
                        return (
                            _DtCm_clntstat_to_csastat(rpc_createerr.cf_stat));
                }
        }
