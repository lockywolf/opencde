...
csa2cmsattrs(CSA_uint32 num_attrs, CSA_attribute *csaattrs, CSA_enum *ops,
             CSA_uint32 *newnum, cms_attribute **newattrs, CSA_enum **newops);
...
extern CSA_return_code _DtCm_rpc_open_calendar(Calendar *cal) {
        ...
        if (ci->vers_out == TABLEVERS) {
                 ...
                 * the calendar
                 */
                if ((stat = _DtCm_table_lookup_range(
                         cal, 0, 0, B_TRUE, 0, 0, 0, 0, NULL, NULL,
                         &entries)) == CSA_SUCCESS) {
                        if (entries)
...
extern CSA_return_code _DtCm_rpc_create_calendar(Calendar *cal,
        ...
        if (ci->vers_out == TABLEVERS) {
                ...
                if ((stat = _DtCm_table_create(cal)) == CSA_SUCCESS &&
                    num_attrs > 0) {
...
_DtCm_rpc_lookup_reminder(Calendar *cal, time_t tick, CSA_uint32 num_names,
        ...
        if (ci->vers_out == TABLEVERS) {
                ...
                if (res != NULL) {
                        if ((stat = res->stat) == CSA_SUCCESS)
                                stat = _DtCm_cms2csa_reminder_ref(
                ...
                stat = _DtCm_table_lookup_reminder(
                    cal, num_names, reminder_names, num_rems, rems);
        }
        ...
        *elist_r = NULL;
        ...
        if (ci->vers_out == TABLEVERS) {
                ...
                if ((stat = _DtCmHashCriteria(
                         ...
                         &no_start_time_range, &no_end_time_range, &start1,
                         &start2, &end1, &end2, &id, &hnum, &hattrs, &hops)) ==
                ...
                if (id > 0 && no_start_time_range && no_end_time_range) {
                        ...
                        key.id = 0;
                        if ((stat = _DtCm_table_lookup_key_range(
                                 cal, NULL, id, _DtCM_BOT, _DtCM_EOT, &key,
                                 NULL)) == CSA_SUCCESS &&
                            key.id > 0) {

                                /* get the detail */
                                if ((stat = _DtCm_make_libentry(
                                         NULL, &entry)) == CSA_SUCCESS) {
                                        entry->e->key = key;

                                        if ((stat = _DtCm_table_lookup(
                                                 cal, entry)) == CSA_SUCCESS)
                                                *elist_r = entry;
                                        else
                        ...
                        stat = _DtCm_table_lookup_range(
                            cal, start1, start2, no_end_time_range, end1, end2,
...
extern CSA_return_code _DtCm_rpc_lookup_entry_by_id(Calendar *cal,
        ...
        if (ci->vers_out == TABLEVERS) {
                ...
                if (res)
                        xdr_free((xdrproc_t)xdr_cms_get_entry_attr_res,
                                 (char *)res);
        } else {
...
extern CSA_return_code _DtCm_rpc_get_cal_attrs(Calendar *cal, int index,
        ...
        if (ci->vers_out == TABLEVERS) {
                ...
                if ((stat = csa2cmsnames(num_names, names, &num_cmsnames,
                ...
                if (res != NULL) {
                        if ((stat = res->stat) == CSA_SUCCESS) {
                                if ((stat = _DtCmUpdateAttributes(
                                         ...
                                         &cal->cal_tbl, B_TRUE, NULL,
                                         B_TRUE)) == CSA_SUCCESS)
                                        cal->got_attrs = B_TRUE;
                        }

                        xdr_free((xdrproc_t)xdr_cms_get_cal_attr_res,
                                 (char *)res);
                } else {
                        stat = (cal->conn.stat == RPC_SUCCESS)
                                   ? CSA_E_SERVICE_UNAVAILABLE
                ...
                case CSA_CAL_ATTR_ACCESS_LIST_I:
                        if ((stat = _DtCm_table_get_access(
                                 cal, &val.item.access_list_value)) ==
                            CSA_SUCCESS) {
                ...
                case CSA_CAL_ATTR_NUMBER_ENTRIES_I:
                        if ((stat = _DtCm_table_size(
                                 cal, (int *)&val.item.uint32_value)) ==
                            CSA_SUCCESS) {
                                val.type = CSA_VALUE_UINT32;
                                stat = _DtCmUpdateSint32AttrVal(
                                    &val,
                                    &cal->attrs[CSA_CAL_ATTR_NUMBER_ENTRIES_I]
                                         .value);
                        }
                        break;
                }
...
_DtCm_rpc_update_entry(Calendar *cal, _DtCm_libentry *oentry,
        ...
        if (ci->vers_out == TABLEVERS) {
                ...
                stat = _DtCm_table_update(cal, oentry, num_attrs, attrs, scope,
                                          nentry);
        }
...
extern CSA_return_code _DtCm_do_registration(_DtCm_Connection *conn, char *cal,
        ...
        if (stat == CSA_SUCCESS) {
                if ((stat = _DtCm_add_registration(
                         conn->ci, cal, update_type)) != CSA_SUCCESS) {
        ...
        for (i = 0, j = 0; i < num_attrs && stat == CSA_SUCCESS; i++) {
                ...
                case CSA_VALUE_CALENDAR_USER:
                        if (csaattrs[i].value->item.calendar_user_value &&
                            csaattrs[i]
                                .value->item.calendar_user_value->user_name) {
        ...
        if (_DtCmIsUserName(cal->name) == B_TRUE) {
                if (strcmp(user, cal->name) == 0) {
                        ...
                        if (dom = strchr(cal->location, '.'))
                                dom++;
                        if (dom == NULL ||
                            _DtCmIsSamePath(_DtCmGetLocalDomain(NULL), dom) ==
                                B_TRUE)
                                isowner = B_TRUE;
                }
        } else {
