 ...
 *****************************************************************************/
static boolean_t match_this_attribute(Appt_4 *appt, cms_attribute attr,
                                      CSA_enum op);
...
static boolean_t match_old_reminder_attribute(Attribute_4 *rem4,
...
extern CSA_return_code _DtCmHashCriteria(
    _DtCmNameTable *tbl, CSA_uint32 num_attrs, CSA_attribute *csaattrs,
    cms_attribute *cmsattrs, CSA_enum *ops, boolean_t *no_match,
    boolean_t *no_start_time_range, boolean_t *no_end_time_range,
    time_t *start1, time_t *start2, time_t *end1, time_t *end2, long *id,
    CSA_uint32 *hnum, cms_attribute **hattrs, CSA_enum **hops) {
                ...
                if (index == CSA_ENTRY_ATTR_START_DATE_I && val == NULL) {
                        ...
                        if (val && val->item.opaque_data_value &&
                            val->item.opaque_data_value->data) {
                                *id = atoi(
                                    (char *)val->item.opaque_data_value->data);
                        }
                }

                if (keep == B_TRUE) {
                        if (*hattrs == NULL) {
                                if ((*hattrs = (cms_attribute *)calloc(
                                         1, sizeof(cms_attribute) *
                                                    (num_attrs - i) +
                                                sizeof(char *))) == NULL)
                                        return (CSA_E_INSUFFICIENT_MEMORY);
                                if ((hval = (cms_attribute_value *)calloc(
                                         1, sizeof(cms_attribute_value) *
                                                (num_attrs - i))) == NULL) {
                                        free(*hattrs);
                                        return (CSA_E_INSUFFICIENT_MEMORY);
                                } else {
...
extern void _DtCmFreeHashedArrays(CSA_uint32 hnum, cms_attribute *hattrs,
                                  CSA_enum *hops) {
        char *ptr, *ptr1;
 ...
 */
extern Appt_4 *_DtCm_match_appts(Appt_4 *appts, long id,
        ...
        while (appts != NULL) {
                ...
                if ((id > 0 && id != appts->appt_id.key) ||
                    (!no_end_time_range &&
                     (appts->duration == 0 || endtick < end1 ||
                      endtick >= end2)) ||
                    (num_attrs > 0 &&
                     !_DtCm_match_one_appt(appts, num_attrs, attrs, ops))) {
                        /* not match */
                        appts->next = freelist;
                        freelist = appts;
                } else {
...
extern boolean_t _DtCm_match_one_appt(Appt_4 *appt, uint num_attrs,
                                      cms_attribute *attrs, CSA_enum *ops) {
        int i;
...
extern Reminder_4 *_DtCm_match_reminders(Reminder_4 *rems, uint num_names,
                                         char **names) {
...
extern CSA_return_code _DtCm_check_operator(uint size, CSA_attribute *csaattrs,
        ...
        for (i = 0; i < size; i++) {
                ...
                if ((name = csaattrs ? csaattrs[i].name
                                     : cmsattrs[i].name.name) == NULL)
                        continue;
                 ...
                 */
                if (val == NULL &&
                    (ops[i] != CSA_MATCH_ANY && ops[i] != CSA_MATCH_EQUAL_TO &&
                     ops[i] != CSA_MATCH_NOT_EQUAL_TO))
                        return (CSA_E_INVALID_ENUM);
...
extern boolean_t _DtCm_match_time_duration_attribute(cms_attribute_value *val1,
        ...
        if (val1) {
                tval1.type = CSA_VALUE_SINT32;
                if (_csa_iso8601_to_duration(
                        val1->item.time_duration_value,
                        (time_t *)&tval1.item.sint32_value))
                        return (B_FALSE);
        }
...
extern boolean_t _DtCm_match_sint32_attribute(cms_attribute_value *val1,
        ...
        case CSA_MATCH_EQUAL_TO:
                if (val2 == NULL) {
                        ...
                        if (val1 &&
                            val1->item.sint32_value == val2->item.sint32_value)
                                return (B_TRUE);
                        else
                                return (B_FALSE);
                }
...
extern boolean_t _DtCm_match_uint32_attribute(cms_attribute_value *val1,
        ...
        case CSA_MATCH_EQUAL_TO:
                if (val2 == NULL) {
                        ...
                        if (val1 &&
                            val1->item.uint32_value == val2->item.uint32_value)
                                return (B_TRUE);
                        else
                                return (B_FALSE);
                }
        ...
        */
        ...
        case _DtCm_old_attr_beep_reminder:
                ...
                return (match_old_reminder_attribute(
                    get_appt_reminder(appt->attr, _DtCM_OLD_ATTR_BEEP_REMINDER),
                    attr.value, op));
        ...
        case _DtCm_old_attr_mail_reminder:
                ...
                return (match_old_reminder_attribute(
                    get_appt_reminder(appt->attr, _DtCM_OLD_ATTR_MAIL_REMINDER),
                    attr.value, op));
        ...
        case _DtCm_old_attr_end_date:
                ...
                if (attr.value) {
                        ...
                        _csa_iso8601_to_tick(
                            attr.value->item.date_time_value,
                            (time_t *)&matchattr.item.sint32_value);
                }

                return (_DtCm_match_sint32_attribute(
                    &oldattr, (attr.value ? &matchattr : NULL), op));
        defalut:
                return (B_FALSE);
        }
}

static Attribute_4 *get_appt_reminder(Attribute_4 *attr4, char *oldattr) {
...
static boolean_t match_names(char *oldattr, uint num_names, char **names) {
        int i;
        char *attrname;
...
static boolean_t match_old_reminder_attribute(Attribute_4 *rem4,
        ...
        if (rem4 != NULL) {
                ...
                rem.repeat_count = 0;
                rem.reminder_data.size =
                    (rem4->clientdata ? strlen(rem4->clientdata) : 0);
        ...
        return (
            _DtCm_match_reminder_attribute((rem4 ? &oldattr : NULL), val, op));
}
