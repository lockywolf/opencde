...
static int GetShortHostname(char *, unsigned int);

static char *GetSessionDirProperty(Display *display);

static char *GetDisplayName(Display *display);
...
char *_DtCreateDtDirs(Display *display) {
        char *tmpPath;
        ...
        if ((sessionDir = GetSessionDirProperty(display)) != NULL) {
                if (!strcmp(sessionDir, DtSM_SESSION_DIRECTORY)) {
                ...
                } else if (!strcmp(sessionDir,
                                   DtSM_SESSION_DISPLAY_DIRECTORY)) {
                        /*
                         * Create a directory for a display-specific session if
                         * necessary
                         */
                        if ((displayName = GetDisplayName(display)) != NULL) {
                                ...
                                /*
                                 * Something's wrong with the display, use the
                                 * fallback
                                 */
                                useOldSession = True;
                        }
                        ...
                        /*
                         * The property contains an unrecognized value, fallback
                         * to other session selection algorithm.
                         */
                        useOldSession = True;
                }
                XFree(sessionDir);
        ...
        if (useOldSession) {
                ...
                if ((displayName = GetDisplayName(display)) != NULL) {
                        ...
                        strncpy(tmpPath, home, MAXPATHLEN);
                        strncat(tmpPath, "/" DtPERSONAL_CONFIG_DIRECTORY,
                                MAXPATHLEN);
                        strncat(tmpPath, "/", MAXPATHLEN);
                        strncat(tmpPath, displayName, MAXPATHLEN);
        ...
        if (needSessionsDir) {
                /*
                 *  If we don't have an old style directory - we check for a
                 * sessions directory, and create it if it doesn't exist
                 */
                strncpy(tmpPath, home, MAXPATHLEN);
                strncat(tmpPath, "/" DtPERSONAL_CONFIG_DIRECTORY, MAXPATHLEN);
...
static int GetShortHostname(char *buffer, unsigned int bufsize) {
        char *ptr;
        int status;
...
static char *GetSessionDirProperty(Display *display) {
        ...
        propStatus = XGetWindowProperty(
            display, RootWindow(display, 0), (Atom)tmpAtom, 0L, 1000000L, False,
            AnyPropertyType, &actualType, &actualFormat, &nitems, &leftover,
