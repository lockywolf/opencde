 ...
 */
/*
 * File:         lock.c $XConsortium: lock.c /main/4 1995/10/26 15:33:05
 * rswiston $ Language:     C
 *
...
int _DtGetLock(Display *display, char *lock_name) {
        Atom lock_atom;
        ...
        if ((selection_owner = XGetSelectionOwner(display, lock_atom)) == None)
                XSetSelectionOwner(display, lock_atom, root_window,
                                   CurrentTime);
...
void _DtReleaseLock(Display *display, char *lock_name) {
        Atom lock_atom;
...
int _DtTestLock(Display *display, char *lock_name) {
        Atom lock_atom;
