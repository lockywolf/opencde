...
$PFUNBEG$ : ExpandPath() $1LINER$
    ...
    : If path is absolute,
      ...
      inserts the CWD in front of the relative path and returns a mallocedd
          ...
          The caller must free the memory when no longer needed.$ARGS$
    : filespec : the pathname $RETURNS$ : ptr to mallocedd memory
          or NULL
/*================================================$SKIP$==*/
#endif
...
$PFUNBEG$ : DeleteDbMem() $1LINER$ : Zeros out the db mem &frees it $SUMMARY$
    : Zeros out the db mem &frees it The xrmDb should have already been
      destroyed before calling this function.$ARGS$ : io_db : db to
                                                              delete $RETURNS$ :
/*================================================$SKIP$==*/
#endif
             ...
             *old != EOS; *nxt = *old) /* xfer but don't advance */
        {
                if (*old == MATCHALL_CHAR) /* if an escaped char follows */
                {
                        ...
                        if (NULL == subex || isdigit(*(old + 1)) == 0) {
                                ...
                                *nxt++ = *old++; /* keep just the char that was
                                                    escaped and assign here to
                                                    avoid tranlating that
                                                    character, then move on to
                                                    the next one */
                                continue;        /* CONTINUE */
                        ...
                        {      /* get the refNum and advance the ptr */
                                ...
                                old += numLen; /* move old ptr past the ref
                                                  number */

                                /* printf("%d=%s\n", refNum,
                                 * &matchedStr[subex[refNum].rm_so]); ** DBG */
                                ...
                                if (refNum >= 0 && refNum < MAXSPECQUARKS &&
                                    subex[refNum].rm_so != -1) {
                                        ...
                                        newOff = nxt - *pStr;
                                        oldOff = old - *pStr;
                                        repLen = subex[refNum].rm_eo -
                                                 subex[refNum].rm_so;
                                        strLen = strlen(*pStr);

                                        /* expand memory and reset pointers */
                                        *pStr =
                                            realloc(*pStr, strLen + repLen + 1);
                                        if (NULL == *pStr)
                                                return; /* RETURN */
                                        nxt = *pStr + newOff;
                                        old = *pStr + oldOff;

                                        /* move rest back to leave room for the
                                         * replacement value */
                                        oldTmp =
                                            *pStr + strLen; /* pts to old EOS */
                                        newTmp = *pStr + strLen -
                                                 (oldOff - newOff) +
                                                 repLen; /*pts to new EOS*/
                                        while (oldTmp >= old)
                                                *newTmp-- = *oldTmp--;

                                        /* replace the ref with a value but
                                         * don't append EOS */
                                        strncpy(
                                            nxt,
                                            &matchedStr[subex[refNum].rm_so],
                                            repLen);
                                        nxt += repLen; /* move new to end of
                                                          replace string */
                                        old += repLen - (oldOff - newOff);
                                        /* move old to end of expanded old
                                         * string */
...
$PFUNBEG$ : StripMetaAndReplaceEscSubex() $1LINER$
    ...
    the routine assumes that the string pointed to by pStr was allocated
        using malloc() and
             ...
             *old != EOS; *nxt = *old) /* xfer but don't advance */
        {
                if (*old == ESCAPE_CHAR) /* if an escaped char follows */
                {
                        /* if ESCAPE_CHAR is not followed by a digit, e.g. \1
                           or no replacement values exist, ignore it */
                        if (NULL == replaceValues || keepEscChar ||
                            isdigit(*(old + 1)) == 0) {
                                if (!keepEscChar)
                                        old++; /* go past the escape char */
                                else
                                        *nxt++ =
                                            *old++; /* copy esc char over */
                                *nxt++ = *old++; /* keep just the char that was
                                                    escaped and assign here to
                                                    avoid tranlating that
                                                    character, then move on to
                        ...
                        {      /* get the refNum and advance the ptr */
                                ...
                                /*  printf("%x=%s\n", replaceValues[refNum],
                                 * replaceValues[refNum]); ** DBG */
                                ...
                                if (refNum >= 0 && refNum < MAXSPECQUARKS &&
                                    replaceValues[refNum] != NULL) {
                                        ...
                                        *pStr = realloc(*pStr, strlen(*pStr) +
                                        ...
                                        /* replace the ref with a value but
                                         * don't append EOS */
                                        strncpy(nxt, replaceValues[refNum],
                                                repLen);
                                        nxt += repLen; /* move new to end of
                                                          replace string */
                                        old += repLen - (oldOff - newOff);
...
$PFUNBEG$ : strCaseiCmp $1LINER$ : case insensitive string comparison $SUMMARY$
    : Rolled my own because
      strcasecmp() not available on all platforms.$ARGS$ : $RETURNS$ : True
    : strings match False
    : they dont
/*================================================$SKIP$==*/
#endif
    ...
    CheckForMatchall(__DtXlateSrchData *srchData, const char *matchallString,
            ...
            refNum < XtNumber(srchData->curSpecRefs)) {
                ...
                if (matchingString[0] == MATCHALL_CHAR &&
                    matchingString[1] == EOS) {
                        ...
                        /* don't add to score for a matchall with no replacement
                         * value */
                } else {
        ...
        } else /* not a value reference; just determine if a plain match */
        {
                if (matchingString[0] == MATCHALL_CHAR &&
                    matchingString[1] == EOS)
                        score =
                            1; /* a plain matchall matches a plain matchall */
        }
...
/*================================================$SKIP$==*/
    ...
    CheckSearchPlatformMatchesSpec(__DtXlateSrchData *srchData,
        ...
        if (srchData->platformQuark != NULLQUARK) {
                ...
                if (CheckForMatchall(srchData, specStr,
                                     srchData->platformStr) == True)
                        return True; /* RETURN: platform matches */
        }
        if (srchData->platformQuark == specPlatformQuark) {
                srchData->curScore +=
                    2;       /* perfect match better than matchall match */
                return True; /* RETURN: platform matches */
        }
...
{        /*$CODE$*/
        ...
        do {
                ...
                matched:
                        srchData->curScore +=
                            2; /*perfect match better than a matchall match*/
...
/*================================================$SKIP$==*/
    ...
    CheckSearchStdValueMatchesSpec(__DtXlateSrchData *srchData,
        ...
        for (unmatched = 0; *specStdValueQuarks != NULLQUARK;
             specStdValueQuarks++) {
                unmatched++;
        }
...
/*================================================$SKIP$==*/
    ...
    CheckSearchOpValueMatchesSpec(__DtXlateSrchData *srchData,
        ...
        if (srchData->curTransType == __DtXLATE_TYPE_REGEX) {
                ...
                if ((ex = compile(opValBuf, NULL, NULL)) != NULL &&
                    advance(srchData->opValue, ex) != 0) {
                        ...
                        /* need due to bug in advance()--operation doesn't meet
                         * documentation */
                        if (NULL == loc1)
                        ...
                        matchSize =
                            loc2 -
                            loc1; /*loc[12] are Sun globals for advance()*/
                        ...
                        if (matchSize == strlen(srchData->opValue)) {
                                /* if the matchSize is the length of
                                   srchData->opValue, then we have a complete
                                   match.  In this case, use the specificity of
                                   the pattern to pick the best match */
                                /* NOTE: opValLen is a crude measure of
                                   specificity. A better measure would be to
                                   count the number of literals/ranges that
                                   matched exactly.  When doing this, a perfect
                                   match without regex syntax should rank higher
                                   than a perfect match with regex syntax.  This
                                   is one area where the current algorithm
                                   breaks.  For example: opValue=23, pat1=23,
                                   pat2=[0-9]3. Both patterns match and pat1 is
                                   a better match, but not with the current
                                   length-based algorithm. */
                                /* NOTE: this formula does not advance the score
                                   to sizeof(opValBuf) for a perfect match.
                                   Other match formulas use sizeof(opValBuf) as
                                   the max value to indicate a perfect match. */
                                srchData->curScore += matchSize + opValLen;
                        } else {
                                /* if its not a complete match, inc score by
                                 * match size */
                                srchData->curScore += matchSize;
                        }

                        /* put sub expression matching stuff in
                         * srchData->curSubEx */
                        for (; nbra > 0; nbra--) {
                                srchData->curSubEx[nbra].rm_so =
                                    braslist[nbra - 1] - loc1;
                                srchData->curSubEx[nbra].rm_eo =
                                    braelist[nbra - 1] - loc1;
                        }
                }
                if (ex)
                        free(ex);
                ...
                if (regcomp(&re, opValBuf, 0) == 0 &&
                    regexec(&re, srchData->opValue,
                            XtNumber(srchData->curSubEx), srchData->curSubEx,
                            0) == 0) {
                        int matchSize;
                        matches = True; /* if got this far */
                        ...
                        /* sub expression matching stuff already in
                         * srchData->curSubEx */
                }
                regfree(&re);
#endif
                  ...
                  __DtXLATE_TYPE_CONTAINS */
        {
                ...
                /* if matches, inc score to show a perfect match (max poss
                 * value) */
                if (matches)
                        srchData->curScore += sizeof(opValBuf);

                /* don't test for contains if a perfect match or a pure match
                 * spec */
                ...
                matches = (opValBuf[0] != EOS && srchData->opValue[0] != EOS) &&
                          ((opValueInBuf =
                                strstr(opValBuf, srchData->opValue)) != NULL ||
...
/*================================================$SKIP$==*/
    ...
    static Bool
    FindStdToOpMatchCB(XrmDatabase *database, XrmBindingList bindings,
                       XrmQuarkList quarks, XrmRepresentation *type,
                       XrmValue *value, XPointer client_data) { /*$CODE$*/
        ...
        /* check for a match */
        if (CheckSearchPlatformMatchesSpec(srchData, quarks[PLATFORM_QUARK]) ==
            False)
        ...
        if (srchData->curScore >= srchData->bestScore) {
                /* recall that all strings are owned by Xrm==>no need to free
                 * them */
                srchData->bestScore = srchData->curScore;
                srchData->bestTransType = srchData->curTransType;
                memcpy(
                    srchData->bestSpecRefs, srchData->curSpecRefs,
                    sizeof(
                        srchData->bestSpecRefs)); /* no array assignment in C */
...
/*================================================$SKIP$==*/
    ...
    static Bool
    FindOpToStdMatchCB(XrmDatabase *database, XrmBindingList bindings,
                       XrmQuarkList quarks, XrmRepresentation *type,
                       XrmValue *value, XPointer client_data) { /*$CODE$*/
        __DtXlateSrchData *srchData = (__DtXlateSrchData *)client_data;
        ...
        if (srchData->curScore >= srchData->bestScore) {
                ...
                srchData->bestTransType = srchData->curTransType;
                memcpy(
                    srchData->bestSubEx, srchData->curSubEx,
...
Failed:
        if ((*io_db) && (*io_db)->debugMode)
                fprintf(stderr,
                        "_DtXlateOpenAndMergeDb: open failed; file: %s\n",
...
$FUNBEG$ : _DtXlateMergeDbs() $1LINER$
    ...
    : Merges two databases into one and closes the merged db
      ...
      The io_dbToMerge database must be a valid translation database
          .The io_dbToMerge database is merged into the io_mergeIntoDb.The
      io_mergeIntoDb may either be invalid
    or valid.If invalid,
    ...
    : database to hold merged result $RETURNS$ : 0 : no error occurred
            - 1 : if io_dbToMerge
        or io_mergeIntoDb is NULL if *io_dbToMerge is NULL or
        uninitialized
/*================================================$SKIP$==*/
#endif
        ...
        /* check args */
        if (NULL == io_mergeIntoDb || NULL == io_dbToMerge ||
            NULL == *io_dbToMerge ||
...
/*================================================$SKIP$==*/
      ...
      int _DtXlateOpenAllDbs(const char *searchPaths, const char *databaseName,
        ...
        do {
                ...
                /* add a slash to end of path component, if needed */
                *(dbFile + workLen) =
                    EOS; /* add an EOS for _DtMBStrrchr to find */
                _DtMBStrrchr(dbFile, DIR_SLASH, -1, &slash);
                if (slash !=
                    dbFile + workLen - 1) /* is slash last char of path? */
                {
...
{       /*$CODE$*/
        ...
        if (NULL == stdValue) {
                if (db->debugMode)
                        fprintf(stderr,
                                "_DtXlateStdToOpValue: NULL std value\n");
                if (ret_opValue)
                        *ret_opValue = NULL;
                return -1; /* RETURN error */
        }
        ...
        if (srchData.opValue != NULL &&
            srchData.bestTransType != __DtXLATE_TYPE_INHIBIT) {
                ...
                /* do quote and escape removal and ref replacement in the
                 * opValue */
                if (srchData.bestTransType == __DtXLATE_TYPE_REGEX)
                        StripMetaAndReplaceEscSubex(&opValue, False,
                                                    srchData.bestSpecRefs);
                else
...
{       /*$CODE$*/
        ...
        if (NULL == opValue) {
                if (db->debugMode)
                        fprintf(stderr,
                                "_DtXlateOpToStdValue: NULL op value\n");
                if (ret_stdValue)
                        *ret_stdValue = NULL;
                return -1; /* RETURN error */
        }
        ...
        if (srchData.stdValueQuarks[0] != NULLQUARK &&
            srchData.bestTransType != __DtXLATE_TYPE_INHIBIT) {
                ...
                /* make a STDVALUE_SEPARATOR separated string out of the std
                 * data */
                for (stdQ = srchData.stdValueQuarks; *stdQ != NULLQUARK;
                     stdQ++) {
                ...
                *stdValue = EOS;
                ...
                if (srchData.bestTransType == __DtXLATE_TYPE_REGEX)
                        ReplaceMatchallSubex(&stdValue, srchData.bestSubEx,
                                             srchData.opValue);
...
xlateStd2Op(_DtXlateDb db,Boolean runtimeOp, char * operation,char * stdVal)
{
        ...
        if (ret == 0)
                _DtXlateStdToOpValue(db, platform, version, operation, stdVal,
                                     &opVal, NULL);
...
{       /*$CODE$*/
        ...
        if (ret_AppExecEnvPlatform) {
                strncpy(ret_AppExecEnvPlatform, names.sysname,
                        _DtPLATFORM_MAX_LEN - 1);
...
<vischar>    ::= any non-whitespace character;
...
_DtLcxXlateOpToStd() translation.Use
    '>' if the<opvalue> contains value replacement expressions
    using values from the<stdvalue> strings; the specification may only be used by a
...
to specify that a given translation does not exist,
    the '0' translation type should be used or
        the specification should not occur
            .
        ...
        <comment> : a comment begins on an empty line with an
                    ...
                    <matchall>
    : The matchall character is a universal quantifier,
...
\<number>,
    as in "\1".The occurance is replaced with the matched string.
/*=$END$================================================*/
#endif
    ...
    !!These are examples of bi -
        ...
        999.setlocale. =.en_US.hp -
                        roman8 : american HP - UX .900 -
                                 999.setlocale. =.en_US.iso88591
    : american.iso88591 HP - UX .900 - 999.setlocale. =.nl_BE.hp -
                                                       roman8.fold
    : dutch @fold HP - UX .900 - 999.setlocale. =.nl_BE.hp -
                                                         roman8
                                                             .nofold
    : dutch @nofold

      !!These are examples of OpToStd translations utilizing
          regular !!expression patterns to match the search value.HP -
                                                         UX.
                                                     ?.version.< .900
                                                     : "A\\.09\\..*" !!any
                                                                   HPUX 9.x version
                                                                             ...
                                                                             : "[0-9] 3" SunOS
                                                                                   ...
                                                                                   : "5\\.3.*" SunOS
                                                                                         ...
                                                                                         : "5\\..*"
                                                                                                       ...
                                                                                                       !!These are examples
                                                                                                           of StdToOp translations utilizing
                                                                                                               matchall !!specifiers
                                                                                                                   in the std value
                                                                                                                       to match the
                                                                                                                           search value
                                                                                                     ...
                                                                                                     : greek8 HP - UX .900 +
                                                                                                           ...
                                                                                                           : kana8 HP - UX .900 +
                                                                                                                 ...
                                                                                                                 : roman8
                                                                                                                   ...
                                                                                                                   !!The
                                                                                                                       following
                                                                                                                           examples use value replacement as part of their !!specifications
                                                                                                                               .Using this can
                                                                                                                                   lead to
                                                                                                                                       much sparser
                                                                                                                                           tables,
        but !!it depends on op - specific and
                std values sharing the same strings.HP - UX .900 -
                        999.setlocale. <
                        .nl_BE.hp - roman8.
            ? 1
            : [dD][uU][tT][cC][hH] @\\(.*\\) !HP - UX .900 - 999.setlocale. <
                          .nl_BE.hp - roman8.
                  ? 1
                  : dutch @\\(.*\\) HP - UX .1000 +.setlocale.>.
                        ? 1.hp - roman8
                        :    \\1.roman8 !!all non - modif cases HP -
                                         UX .1000 +.setlocale.>
                                         .
