 ...
 */
/*
 * File:         FileUtil.c $XConsortium: FileUtil.c /main/6 1996/11/01 10:06:23
 * drk $ Language:     C
 *
...
int _DtCreateDirs(char *path, int mode) {
        struct stat st_buf;
        int st_status;
        int ret_status;
        char *parent_path;
        ...
        /* If we can't stat it, make sure it is simply because some component
           of the path doesn't exist. */
                ...
                /* Some component of the path doesn't exist.  Recursively make
                   the parent of the current directory, then make the current
                   directory. */
                ...
                /* If no error has been encountered, now make the final
...
int _DtIsOpenableDirContext(char *path, char **ret_path) {
        char *real_path = NULL;
        char *tmp_real_path;
        DIR *dirp;
        int ret_status;
...
int _DtIsOpenableDir(char *path) {
