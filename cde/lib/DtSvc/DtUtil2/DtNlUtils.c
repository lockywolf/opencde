...
char *Dt_strtok(char *s1, char *s2) {
        static char *ptr;
        char *return_ptr;
        int len;
        int offset;
...
char *Dt_strtok_r(char *s1, char *s2, char **ptr) {
        char *return_ptr;
        int len;
        int offset;
...
int Dt_strspn(char *s1, char *s2) {
        ...
        while (*s1) {
                ...
                match = False;
                while (*ptr) {
                        /* Extract the next character from s2; cannot be
                         * multibyte */
                        s2char = *ptr++;
                ...
                /*
                 * If we made it here because all of s2 was searched, and a
                 * match was not found against s1, then we are done.
                 */
                if (!match)
                        return (count);
        }
...
int Dt_strcspn(char *s1, char *s2) {
        wchar_t s1char, s2char;
        int s1len, s2len;
        int i;
        int count;
        ...
        while (*s1) {
                ...
                /*
                 * If we've made it here, then we searched all of s2, and none
                 * of its components matched s1; continue with the next
                 * character in s1.
                 */
                count += s1len;
        }
...
void Dt_lastChar(char *s, char **cptr, int *lenptr) {
        int len = 0;
 ...
 *
 ******************************************************************************/
char *_Dt_PrevChar(const char *start, char *s) {
        char *p;
        int len;
        ...
        for (p = (char *)start;
             p + (len =
                      (mblen(p, MB_CUR_MAX) == -1 ? 1 : mblen(p, MB_CUR_MAX))) <
             s;
             p += len)
                /* NULL STATEMENT */;
...
char *_DtGetNthChar(char *s, int n) {
        int count;
        int len;
 ...
 */
char *_dt_strpbrk(char *cs, char *ct) {
        int len;
        size_t i;
 ...
 */
int _is_previous_single(char *s1, char *s2) {
        int n = 1;
...
char *_dt_strpbrk(char *cs, char *ct) { return (strpbrk(cs, ct)); }

int _is_previous_single(char *s1, char *s2) { return (1); }
#endif /* NLS16 */
