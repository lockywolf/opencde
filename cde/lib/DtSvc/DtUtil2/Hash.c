...
static unsigned int GetTableIndex(register DtHashTable tab,
...
static void ExpandHashTable(register DtHashTable tab);
...
#define RELEASE_KEY(tab, entry, key)                                           \
        {                                                                      \
                if (tab->types[entry->hash.type]->hash.releaseKeyProc)         \
                        (*(tab->types[entry->hash.type]                        \
                               ->hash.releaseKeyProc))(entry, key);            \
        }
...
void _DtRegisterHashEntry(register DtHashTable tab, register DtHashKey key,
                          register DtHashEntry entry) {
        unsigned int idx;
...
void _DtUnregisterHashEntry(register DtHashTable tab,
                            register DtHashEntry entry) {
...
static void ExpandHashTable(register DtHashTable tab) {
        ...
        entries = tab->entries =
            (DtHashEntry *)XtCalloc(tab->mask + 1, sizeof(DtHashEntry));
        for (oldidx = 0; oldidx <= oldmask; oldidx++) {
                if ((entry = oldentries[oldidx]) && entry != &DtHashfake) {
                        newidx =
                            GetTableIndex(tab, key = KEY(tab, entry), True);
                        RELEASE_KEY(tab, entry, key);
...
DtHashEntry _DtEnumerateHashTable(register DtHashTable tab,
                                  register DtHashEnumerateFunc enumFunc,
                                  register XtPointer clientData) {
        register unsigned int i;

        for (i = 0; i <= tab->mask; i++)
                if (tab->entries[i] && tab->entries[i] != &DtHashfake &&
                    ((*enumFunc)(tab->entries[i], clientData)))
                        return tab->entries[i];
        return NULL;
}

DtHashEntry _DtKeyToHashEntry(register DtHashTable tab,
                              register DtHashKey key) {
...
DtHashTable _DtAllocHashTable(DtHashEntryType *hashEntryTypes,
...
void _DtFreeHashTable(DtHashTable hashTable) {
