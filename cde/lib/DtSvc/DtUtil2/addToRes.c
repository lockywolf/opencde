...
static void _DtAppendToBuffer(register Buffer *b, char *str, register int len);
static void _DtAppendEntryToBuffer(register Buffer *buffer, Entry entry);

static Entries *_DtAllocEntries(void);
static void _DtFreeEntries(Entries *);
static void _DtAddEntry(register Entries *e, Entry entry);
static int _DtCompareEntries(Entry *e1, Entry *e2);
static char *_DtFindFirst(register char *string, register char dest);
static void _DtGetEntries(register Entries *entries, Buffer *buff, int dosort);
static void _DtMergeEntries(Buffer *buffer, Entries new, Entries old);
static void _DtAddToResProp(Display *dpy, unsigned int id, Entries db);
static void _getWinProp(Display *dpy, unsigned int id, Window *win, Atom *prop);
...
static void _DtAppendToBuffer(register Buffer *b, char *str, register int len) {
        while (b->used + len > b->room) {
                b->buff =
                    (char *)realloc(b->buff, 2 * b->room * (sizeof(char)));
                b->room *= 2;
        }
...
static void _DtFreeEntries(Entries *e) {
        int n = 0;
...
static void _DtAddEntry(register Entries *e, Entry entry) {
        ...
        for (n = 0; n < e->used; n++) {
                if (strcmp(e->entry[n].tag, entry.tag) ==
                    0) { /* overwrite old entry  - free its memory first*/
                        free(e->entry[n].tag);
                        free(e->entry[n].value);
                        e->entry[n] = entry;
                        return; /* ok to leave, now there's only one of each tag
                                   in db */
                }
        }
        ...
        if (e->used == e->room) {
                e->entry =
                    (Entry *)realloc(e->entry, 2 * e->room * (sizeof(Entry)));
                e->room *= 2;
        }
        entry.usable = True;
...
static void _DtGetEntries(register Entries *entries, Buffer *buff, int dosort) {
        ...
        for (; str < buff->buff + buff->used; str = line + 1) {
                ...
                if (colon > line) {
                        line[0] = '\0';
                        fprintf(stderr,
                                "%s:  colon missing on line %d, ignoring entry "
                                "\"%s\"\n",
                                "dtprefs", lineno, str);
                        continue;
                }
...
static void _getWinProp(Display *dpy, unsigned int id, Window *win,
                        Atom *prop) {
...
static void _DtAddToResProp(Display *dpy, unsigned int id, Entries db) {
         ...
         */
        defStatus = XGetWindowProperty(
            dpy, win, prop, 0L, 100000000L, False, XA_STRING, &actualType,
            &actualFormat, &nitems, &leftover, (unsigned char **)&xdefs);
         ...
         */
        if (oldBuffer->buff)
                XFree(oldBuffer->buff);
        _DtFreeBuffer(oldBuffer);
        _DtFreeBuffer(newBuffer);
        _DtFreeEntries(oldDB);
}

char *_DtGetResString(Display *dpy, unsigned int id) {
        char *xdefs;
        Buffer *oldBuffer, *newBuffer;
        Entries *oldDB;
        int defStatus;
...
void _DtAddToResource(Display *dpy, const char *data) {
        _DtAddResString(dpy, data, _DT_ATR_RESMGR | _DT_ATR_PREFS);
}

void _DtAddResString(Display *dpy, const char *data, unsigned int flags) {
        char *xdefs;
        int i;
        Buffer *buffer;
        Entries *newDB;
