...
int _DtPrintDefaultError(Display *dpy, XErrorEvent *event, char *msg) {
...
int _DtPrintDefaultErrorSafe(Display *dpy, XErrorEvent *event, char *msg,
                             int bytes) {
        char buffer[BUFSIZ];
        char fpBuf[BUFSIZ];
        char *fp;
...

        {
                ...
                XGetErrorDatabaseText(dpy, mtype, "XError", "X Error", mesg,
                                      BUFSIZ);
...

        {
                if (event->request_code < 128) {
                        ...
                        for (ext = dpy->ext_procs;
                             ext &&
                             (ext->codes.major_opcode != event->request_code);
                             ext = ext->next)
                                ;
                        if (ext) {
...

        {
                if (event->request_code >= 128) {
                        ...
                        if (ext) {
                                ...
                                sprintf(fp, "%s.%d", ext->name,
                                        event->minor_code);

                                XGetErrorDatabaseText(dpy, "XRequest", fp, "",
                                                      buffer, BUFSIZ);
                ...
                if (buffer[0]) {
                        if (strlen(buffer) < BUFSIZ - 10)
                                sprintf(buffer, "%s.%d", ext->name,
                                        event->error_code -
                                            ext->codes.first_error);
                } else
                        strcpy(buffer, "Value");
                XGetErrorDatabaseText(dpy, mtype, buffer, "Value 0x%x", mesg,
                                      BUFSIZ);
                if (*mesg) {
                        if (strlen(mesg) < BUFSIZ - 10)
                                fp = fpBuf;
