...
static char *makeDefaultIconPath(const char *varname, const char *basedefault,
        ...
        if (NULL != homedir) {
                ...
                for (; *suffixes; ++suffixes)
                        bytes_needed +=
                            (homedir_bytes + fmt_bytes + strlen(*suffixes));
        }
 ...
 */
static char *makeDefaultIconPmPath() {
        static char *defaultIconPmPath = NULL;

        if (NULL == defaultIconPmPath)
                defaultIconPmPath = makeDefaultIconPath(
                    PM_PATH_ENVIRON, DTPMSYSDEFAULT, iconPmSuffixes);
 ...
 */
static char *makeDefaultIconBmPath() {
        static char *defaultIconBmPath = NULL;

        if (NULL == defaultIconBmPath)
                defaultIconBmPath = makeDefaultIconPath(
                    BM_PATH_ENVIRON, DTBMSYSDEFAULT, iconBmSuffixes);
...
int _DtEnvControl(int mode) {
        ...
        case DT_ENV_SET:
                if (!environSetup) /* first time through */
                {
                         ...
                         *
                         *  Check for NULL environment strings before the
                         * sprintf
                         *
                         */
                        ...
                        /*
                         *  First, ensure that BIN_PATH_STRING isn't already
                         * there.
                         */
                        if (!tempString ||
                            !(strstr(tempString, BIN_PATH_STRING))) {
                                if (!tempString) {
                                        /*
                                         * No existing PATH environment
                                         * variable. Just create the new DT
                                         * environment.
                                         */
                                        bytes_needed =
                                            strlen(BIN_PATH_ENVIRON) +
                                            strlen(BIN_PATH_STRING) + 2;

                                        _postDtEnvironment.binPath =
                                            XtMalloc(bytes_needed);
                                        sprintf(_postDtEnvironment.binPath,
                                                "%s=%s", BIN_PATH_ENVIRON,
                                                BIN_PATH_STRING);
                                } else {
                                        /*
                                        ...
                                        {
                                                /*
                                                 * Shorten the string in
                                                 * tempString to the initial
                                                 * segment, up to the initial
                                                 * slash in "/usr/bin/X11"
                                                 */
                                                if (ptr != tempString) {
                                                        /*
                                                         * then put our dt
                                                         * string just ahead of
                                                         * "/usr/bin/X11" in the
                                                         * new PATH
                                                         */
                                                        *(ptr - 1) = '\0';
                                ...
                                _EnvAdd(BIN_PATH_ENVIRON,
                                        _postDtEnvironment.binPath, BV_BINPATH);
                        }

                        tempString = getenv(PM_PATH_ENVIRON);
                        if (tempString) {
                                bytes_needed = strlen(PM_PATH_ENVIRON) +
                                               strlen(tempString) + 2;
                                ...
                                _EnvAdd(PM_PATH_ENVIRON,
                                        _postDtEnvironment.pmPath, BV_PMPATH);
                        }

                        tempString = getenv(BM_PATH_ENVIRON);
                        if (tempString) {
                                bytes_needed = strlen(BM_PATH_ENVIRON) +
                                               strlen(tempString) + 2;
                                ...
                                _EnvAdd(BM_PATH_ENVIRON,
                                        _postDtEnvironment.bmPath, BV_BMPATH);
                        }

                        /* Do the admin for the NLSPATH env variable */
                        tempString = getenv(NLS_PATH_ENVIRON);
                        if (!tempString) {
                                /* If it doesn't exist, set it to the CDE
                                 * default */
                                bytes_needed = strlen(NLS_PATH_ENVIRON) +
                                               strlen(NLS_PATH_STRING) + 2;

                                _postDtEnvironment.nlsPath =
                                    XtMalloc(bytes_needed);
                                sprintf(_postDtEnvironment.nlsPath, "%s=%s",
                                ...
                                _preDtEnvironment.nlsPath =
                                    XtMalloc(bytes_needed);
                                sprintf(_preDtEnvironment.nlsPath, "%s=%s",
                                        NLS_PATH_ENVIRON, tempString);

                                /* ... then append the CDE default to the
                                 * existing value
                                 */
                                bytes_needed = strlen(NLS_PATH_ENVIRON) +
                                               strlen(tempString) +
                                ...
                                _postDtEnvironment.nlsPath =
                                    XtMalloc(bytes_needed);
                                sprintf(_postDtEnvironment.nlsPath, "%s=%s:%s",
                                        NLS_PATH_ENVIRON, tempString,
                                        NLS_PATH_STRING);
                        }
                        ...
                        if (!tempString) {
                                ...
                                _postDtEnvironment.sysApplPath =
                                    XtMalloc(bytes_needed);
                                sprintf(_postDtEnvironment.sysApplPath, "%s=%s",
                                ...
                                _preDtEnvironment.sysApplPath =
                                    XtMalloc(bytes_needed);
                                sprintf(_preDtEnvironment.sysApplPath, "%s=%s",
                                ...
                                sprintf(_postDtEnvironment.sysApplPath,
                                        "%s=%s:%s", SYSTEM_APPL_PATH_ENVIRON,
                                        tempString, SYSTEM_APPL_PATH_STRING);
                        }

                        _EnvAdd(SYSTEM_APPL_PATH_ENVIRON,
                                _postDtEnvironment.sysApplPath, BV_SYSAPPLPATH);
                ...
                if (environSetup) {
                        if (_preDtEnvironment.nlsPath) {
                                _EnvAdd(NLS_PATH_ENVIRON,
                                        _preDtEnvironment.nlsPath, BV_NLSPATH);
                        } else {
                        ...
                        if (_preDtEnvironment.pmPath) {
                                _EnvAdd(PM_PATH_ENVIRON,
                                        _preDtEnvironment.pmPath, BV_PMPATH);
                        } else {
                        ...
                        if (_preDtEnvironment.bmPath) {
                                _EnvAdd(BM_PATH_ENVIRON,
                                        _preDtEnvironment.bmPath, BV_BMPATH);
                        } else {
        ...
        case DT_ENV_RESTORE_POST_DT:
                if (environSetup) {
                        ...
                        if (_postDtEnvironment.nlsPath) {
                                _EnvAdd(NLS_PATH_ENVIRON,
                                        _postDtEnvironment.nlsPath, BV_NLSPATH);
                        }
...
static void _EnvAdd(char *envVar, char *envVarSetting, unsigned int bv_flag) {
        ...
        if (envBitVector & bv_flag) {
                ...
                /* if we have previously put this environment variable out to
                 * the environment, we can retrieve it and reuse it rather than
                 * letting it disappear into the ether
                 */
...

{
        ...
        if (!sOldEnv) {
                ...
                while (sNext != NULL) {
                        ...
                        if (bytes_needed >= MAX_ENV_STRING)
                                ...
                                postDtEnvironmentString =
                                    postDtEnvironmentStringBuf;

                        if (useMultiColorIcons)
                                sprintf(postDtEnvironmentString, pm_bm_fmt,
                                        sNext, sNext, sNext);
                        else
                                sprintf(postDtEnvironmentString, bm_pm_fmt,
                                        sNext, sNext, sNext);

                        _postDtEnvironment.pmPath = _AddToPath(
                            _postDtEnvironment.pmPath, postDtEnvironmentString);
                ...
                if (!_preDtEnvironment.pmPath) {
                        bytes_needed =
                            strlen(PM_PATH_ENVIRON) + strlen(sOldEnv) + 2;
                        _preDtEnvironment.pmPath = XtMalloc(bytes_needed);
                        sprintf(_preDtEnvironment.pmPath, "%s=%s",
                                PM_PATH_ENVIRON, sOldEnv);
                }
                ...
                sprintf(_postDtEnvironment.pmPath, "%s=%s", PM_PATH_ENVIRON,
                        sOldEnv);
        ...
        if (!sOldEnv) {
                ...
                while (sNext != NULL) {
                        ...
                        sprintf(postDtEnvironmentString, bm_pm_fmt, sNext,
                                sNext, sNext);

                        _postDtEnvironment.bmPath = _AddToPath(
                            _postDtEnvironment.bmPath, postDtEnvironmentString);
                ...
                if (!_preDtEnvironment.bmPath) {
                        bytes_needed =
                            strlen(BM_PATH_ENVIRON) + strlen(sOldEnv) + 2;
                        _preDtEnvironment.pmPath = XtMalloc(bytes_needed);
                        sprintf(_preDtEnvironment.pmPath, "%s=%s",
                                BM_PATH_ENVIRON, sOldEnv);
                }
                ...
                sprintf(_postDtEnvironment.bmPath, "%s=%s", BM_PATH_ENVIRON,
                        sOldEnv);
