...
/*
 * myDtChkpntMsgSend --- Helper function: Send a checkpoint message to the
 * listener
 */
static int myDtChkpntMsgSend(char *message, char *type) {
        static long msgcount = 0; /* Running count of messages */
        static int propnum = 0;   /* Which property are we using ? */
        static Time oldtime =
            INVALID_TIME; /* Recycled from old PropertyNotify events */
        ...
        sprintf(buf_seconds, "%lf",
                time_val.tv_sec + (time_val.tv_usec / 1000000.0));
        dtcp_msg.record.seconds = buf_seconds; /* Info from gettimeofday()*/
        dtcp_msg.record.message = message;     /* Actual message string */

        /*
         * We maintain a list of properties and use them round robin -- hoping
         * to never run out. The listener should then track the message count to
         * see if messages are getting dropped.
         */

        /* Fill the window property with necessary information */
        status = XStringListToTextProperty(dtcp_msg.array,
                                           DT_PERF_CHKPNT_MSG_SIZE, &tp);
        ...
        XSetTextProperty(dtcp_info.display, dtcp_info.window, &tp,
                         dtcp_info.aProperty[propnum]);
        XFree(tp.value);
        ...
        if (oldtime != INVALID_TIME) { /* Valid timestamp to be recycled */
                ...
                if (bool == True) {
                        if (event.type == PropertyNotify)
                                ...
                                timestamp = event.xselection.time;
        ...
        do {
                bool = XCheckIfEvent(dtcp_info.display, &event,
                                     myCheckClientEvent, NULL);
                if (event.type ==
                    PropertyNotify) /* Save timestamp for recycling */
                        oldtime = event.xproperty.time;
                else
                        oldtime = event.xselection.time;
        } while (bool == True);
 ...
 */
int _DtPerfChkpntInit(Display *display, Window parentwin, char *prog_name,
                      Boolean bChkpnt) {
         ...
         */
        dtcp_info.aSelection =
            XInternAtom(dtcp_info.display, DT_PERF_CHKPNT_SEL, False);
        ...
        for (i = 0; i < dtcp_info.maxprops; i++) {
                ...
                dtcp_info.aProperty[i] =
                    XInternAtom(dtcp_info.display, propname, False);
        }
        ...
        dtcp_info.window =
            XCreateSimpleWindow(tmpdisplay, parentwin, 1, 1, 100, 100, 1,
 ...
 */
void _DtPerfChkpntMsgSend(char *message) {
