...
int
#ifdef _NOPROTO
    _DtPerfChkpntListenInit(display,
                            parentwin) Display *display; /* Current display */
...
#endif
{
        ...
        dtsvc_info.window =
            XCreateSimpleWindow(display, parentwin, 1, 1, 100, 100, 1,
        ...
        if (XGetSelectionOwner(dtsvc_info.display, dtsvc_info.aSelection) ==
            None) {
                if (timestamp ==
                    INVALID_TIME) { /* Generate a valid timestamp */
                        XEvent event;
                        Atom aProperty;
                        char propname[80]; /* Temp buffer for property name */

                        sprintf(propname, "%s_%x", DT_PERF_CLIENT_CHKPNT_PROP,
                                0);
                        aProperty =
                            XInternAtom(dtsvc_info.display, propname, False);
                        XSelectInput(dtsvc_info.display, dtsvc_info.window,
                                     PropertyChangeMask);

                        XChangeProperty(dtsvc_info.display, dtsvc_info.window,
                                        aProperty, XA_STRING, 8, PropModeAppend,
                ...
                tmpwin = XGetSelectionOwner(dtsvc_info.display,
                                            dtsvc_info.aSelection);
                if (tmpwin == dtsvc_info.window)
                        bsuccess = True; /* We are now the listener! */
        }

        if (bsuccess == False) {
                fprintf(stderr, "\t** Chkpnt listener: Cannot gain ownership "
                                "of root selection **\n");
                fprintf(stderr,
                        "\t** Chkpnt listener: Selection is owned by window: "
                        "%lx **\n",
                        (long)tmpwin);
...
static Bool myCheckSelectionEvent(Display *display, XEvent *event, char *args) {
        ...
        if (event->xany.window !=
            dtsvc_info.window) { /* Only listener window events*/
                _DtSvcProcessUnlock();
                return (False);
        } else
                switch (event->type) { /* Selection stuff ? */
                case SelectionClear:
...
Bool _DtPerfChkpntMsgReceive(DtChkpntMsg *dtcp_msg, Bool bBlock) {
        ...
        if (bBlock == True)
                XIfEvent(dtsvc_info.display, &event, myCheckSelectionEvent,
                         NULL);
        else
        ...
        if (bool == True) {
                ...
                case SelectionRequest: /* Message received from a client */
                        ...
                        if (event.xselectionrequest.selection ==
                            dtsvc_info.aSelection) {
                                /* Correct selection, now fetch the property */
                                /*
                                 * Note: Need to handle errors if the client is
                                 * dead and the property no longer exists.
                                 */
                                XGetTextProperty(
                                    dtsvc_info.display,
                                    event.xselectionrequest.requestor, &tp,
                                    event.xselectionrequest.property);
                                XDeleteProperty(
                                    dtsvc_info.display,
                                    event.xselectionrequest.requestor,
                                    event.xselectionrequest.property);
                                XTextPropertyToStringList(&tp, &Stringlist,
                                                          &numfields);
                                XFree(tp.value);
                                for (i = 0; i < DT_PERF_CHKPNT_MSG_SIZE; i++)
                                        dtcp_msg->array[i] = (Stringlist)[i];

                                /* End? Destroy the, RetainPermanent client
                                 * window */
                                if (!strcmp(dtcp_msg->record.type,
                                            DT_PERF_CHKPNT_MSG_END))
                                        XDestroyWindow(
                                            dtsvc_info.display,
                                            event.xselectionrequest.requestor);
                        }
                ...
                default:
                        fprintf(stderr, "\t** Chkpnt listener: Warning - loss "
