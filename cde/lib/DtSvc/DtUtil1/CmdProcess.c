...
static char *_GetRemoteTerminalsResource(Display *display, char *appClass,
                                         char *appName);
static void GetRemoteTerminalsResource(Display *display, char *appClass,
                                       char *appName);
static void GetLocalTerminalResource(Display *display, char *appClass,
                                     char *appName);
static void GetWaitTimeResource(Display *display, char *appClass,
                                char *appName);
static void GetDtexecPath(Display *display, char *appClass, char *appName);
static void GetExecutionHostLoggingResource(Display *display, char *appClass,
                                            char *appName);
static char *GetRemoteTerminal(char *host);
...
static char *_GetRemoteTerminalsResource(Display *display, char *appClass,
        ...
        class =
            XtMalloc(strlen(DtREMOTE_TERMINALS_CLASS) + strlen(appClass) + 4);
...
static void GetRemoteTerminalsResource(Display *display, char *appClass,
        ...
        if ((terminals = _GetRemoteTerminalsResource(display, appClass,
                                                     appName)) == NULL) {
                /*
        ...
        for (pch = terminals, j = 0; j < num && pch != NULL && *pch != '\0';
             j++) {
                ...
                *pch = '\0';
                ...
                if ((pch = DtStrchr(pch, ',')) == NULL) {
                        /*
                         * This is the last pair, advance "j" and break the
                         * loop.
                         */
                        j++;
                        break;
...
static void GetLocalTerminalResource(Display *display, char *appClass,
        ...
        if (XrmGetResource(db, name, class, &rep_type, &resource_value))
                cmd_Resources.localTerminal =
                    (char *)XtNewString(resource_value.addr);
        else
...
void _DtCmdCreateTerminalCommand(char **theCommand, int windowType,
                                 char *execString, char *execParms,
         ...
         */
        *theCommand = (char *)XtMalloc(
            strlen(defaultTerminal) + strlen(options) +
            ...
            9 /* 2 quotes, "-e", 7 blanks, null */);

        if (windowType == TERMINAL) {
                (void)sprintf(
                    *theCommand, "%s %s -e %s -open %d -ttprocid '%s' %s %s",
                    defaultTerminal, options, cmd_Resources.dtexecPath,
                    cmd_Resources.waitTime, _DtActNULL_GUARD(procId),
                    _DtActNULL_GUARD(tmpFiles), execString);
        } else {
                (void)sprintf(
                    *theCommand, "%s %s -e %s -open -1 -ttprocid '%s' %s %s",
                    defaultTerminal, options, cmd_Resources.dtexecPath,
...
Boolean _DtCmdCheckForExecutable(char *fileName) {
        char **ppch;
        int access;
...
Boolean _DtCmdValidDir(char *clientHost, char *contextDir, char *contextHost) {
        int retValue = 0;
...
void _DtCmdGetResources(Display *display) {
        char *appClass = NULL;
        char *appName = NULL;
