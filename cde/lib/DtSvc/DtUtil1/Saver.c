 ...
 * Local functions.
 */
static void RegisterSaverCB(Widget w, XtPointer client_data, XEvent *event,
 ...
 *
 *************************************<->***********************************/
void *_DtSaverStart(Display *display, Widget *drawArea, int count,
                    char *saverAction, Widget wAction) {
...
#endif
                xa_saver_register =
                    XInternAtom(display, "_DT_SAVER_REGISTER", False);
        }
         ...
         */
        if (!(state =
                  (struct saver_state *)malloc(sizeof(struct saver_state)))) {
                _DtSvcProcessUnlock();
                return (NULL);
        }
         ...
         */
        sprintf(envdata, "DTSAVERINFO=%u %i %lx", state->serial, count,
                XtWindow(drawArea[0]));
        for (i = 1; i < count; i++) {
         ...
         * Launch saver.
         */
        DtActionInvoke(wAction, saverAction, NULL, NULL, NULL, NULL, NULL, NULL,
                       NULL, NULL);
...
void _DtSaverStop(Display *display, void *pstate) {
...
Boolean DtSaverGetWindows(Display *display, Window **window, int *count) {
        char *envdata, *p, *q;
        unsigned short serial;
        int envcount;
        ...
        *window = (Window *)malloc((envcount) * sizeof(Window *));
         ...
         */
        xid_window =
            XCreateWindow(display, DefaultRootWindow(display), 0, 0, 1, 1, 0,
        ...
        XSendEvent(display, (*window)[0], False, NoEventMask, (XEvent *)&event);
...
static void RegisterSaverCB(Widget w, XtPointer client_data, XEvent *event,
        ...
        if (event->type == ClientMessage) {
                ...
                if (cEvent->message_type == xa_saver_register) {
                        unsigned short serial =
                            (unsigned short)cEvent->data.l[1];
                        ...
                        if (state != NULL) {
                                /*
                                 * _DtSaverStop() not yet called for this saver.
                                 * Store xid in saver's state for
                                 * _DtSaverStop()'s use.
                                 */
                                state->xid = win;
                        } else {
                                /*
                                 * _DtSaverStop() has already been called for
                                 * this saver, but at the time, the saver had
                                 * not yet registered. Kill the saver client.
                                 */
