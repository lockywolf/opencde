 ...
 *
 *************************************<->***********************************/
int _DtWsmGetDtWmHints(Display *display, Window window,
        ...
        *ppDtWmHints = NULL;
        if ((rcode = XGetWindowProperty(
                 display, window, property, 0L,      /* offset */
                 length, False,                      /* delete */
                 property,                           /* req_type */
                 &actualType, &actualFormat, &items, /* items returned */
                 &leftover, (unsigned char **)ppDtWmHints)) == Success) {
                ...
                *ppDtWmHints = NULL;
                if ((rcode = XGetWindowProperty(
                         ...
                         &leftover, (unsigned char **)ppDtWmHints)) ==
                    Success) {

                        if ((actualType != property) || (items < oldlength)) {
                                /* wrong type, force failure */
                                rcode = BadValue;
                                if (actualType != None) {
                                        XFree((char *)*ppDtWmHints);
                        ...
                        if (*ppDtWmHints && (items < length)) {
                                ...
                                pvh =
                                    (DtWmHints *)malloc(length * sizeof(long));
