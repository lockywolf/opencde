...
static void DtexecTerminator(SPC_Channel_Ptr cmdChannel, int pid, int type,
                             int cause, unsigned long ind);
...
int _DtSPCSpawn(char *path, char *cwd, char **args, char **env,
        ...
        if ((retVal = XeSPCSpawn(path, cwd, args, env, chan)) == SPC_ERROR) {
                ...
                case SPC_cannot_Chdir:
                        (void)sprintf(errorMessage, errorChdir, contextDir,
                                      execHost);
                        break;
                ...
                case SPC_Arg_Too_Long:
                        (void)sprintf(errorMessage, errorLength, SPC_BUFSIZ);
                        break;
                default:
                        /*
                         * SPC_Cannot_Exec
                         */
                        (void)sprintf(errorMessage, errorSpawn, execHost, path);
                }
        }
...
SPC_Channel_Ptr _DtSPCOpen(char *hostname, int iomode, char *errorMessage) {
        ...
        if ((chan = XeSPCOpen(hostname, iomode)) == SPC_ERROR) {
                ...
                case SPC_Unknown_Host:
                        (void)sprintf(errorMessage, errorUnknownHost, hostname);
                        break;
                case SPC_Bad_Connect:
                        (void)sprintf(errorMessage, errorBadConnect, hostname,
                                      SPC_SERVICE, _cmdClientHost);
                        break;
                case SPC_Bad_Service:
                        (void)sprintf(errorMessage, errorBadService,
                                      SPC_SERVICE, _cmdClientHost);
                        break;
                case SPC_Register_Handshake:
                        ...
                        (void)sprintf(errorMessage, errorRegisterHandshake,
                                      hostname, username, this_uid,
                                      _cmdClientHost, hostname);
                        break;
                case SPC_Register_Username:
                        ...
                        if ((pwd_ret = _XGetpwuid(this_uid, pwd_buf)) == NULL)
                                ...
                                username = pwd_ret->pw_name;
                        (void)sprintf(errorMessage, errorRegisterUsername,
                                      hostname, username);
                        break;
                case SPC_Register_Netrc:
                        (void)sprintf(errorMessage, errorRegisterNetrc,
                                      hostname);
                        break;
                case SPC_Env_Too_Big:
                        (void)sprintf(errorMessage, errorEnvTooBig, hostname,
                                      SPC_BUFSIZ);
                        break;
                case SPC_Connection_EOF:
#ifdef __hpux
                        (void)sprintf(errorMessage, errorInetSecurity, hostname,
                                      _cmdClientHost, SPC_SERVICE,
                                      _cmdClientHost, SPC_SERVICE,
                                      INETD_SECURITY_FILE, hostname);
                ...
                default:
                         ...
                         * SPC_Register_Open:
                         */
                        (void)sprintf(errorMessage, errorRegisterOpen,
                                      hostname);
                }
        }
...
static void ExecuteQueuedRequest(unsigned long requestNum) {
        ...
        if ((pNode->channel = (_DtSPCOpen(pNode->exec_host, iomode,
                                          errorMessage))) == SPC_ERROR) {
                success = False;
        }

        if (success)
                if ((_DtSPCSpawn(pNode->argv[0], pNode->context, pNode->argv,
                                 NULL, pNode->channel, pNode->exec_host, NULL,
                                 NULL, errorMessage)) == SPC_ERROR) {
                        success = False;
        ...
        if (success && pNode->success_proc != NULL) {
                ...
                if (cmd_Resources.executionHostLogging) {
                        if (DtSPCErrorNumber == SPC_Arg_Too_Long) {
                                int cmdlen, i;
                                char *cmdp; /* pointer to complete command
                                               string */
                                char *tmp_message;

                                /*
                                 * The message should include all of the command
                                 * because the problem may be with some on the
                                 * internally generated parts of the command
                                 * (e.g. the terminal emulator and args). This
                                 * means going through all of argv to determine
                                 * the length of the string.
                                 */
                                for (cmdlen = 0, i = 0; pNode->argv[i]; i++) {
                                        cmdlen += strlen(pNode->argv[i]);
                                        cmdlen += 2; /* make room for a space +
                                                        string terminator */
                                }

                                tmp_message = (char *)XtMalloc(
                                    strlen(errorSpawn) +
                                ...
                                *cmdp = '\0';
                                ...
                                (void)sprintf(tmp_message, errorSpawn,
                                              pNode->exec_host, cmdp);
                ...
                if (pNode->failure_proc != NULL)
                        (*pNode->failure_proc)(errorMessage,
                                               pNode->failure_data);
        }
...

{
        ...
        if (windowType == PERM_TERMINAL || windowType == TERMINAL) {
                ...
                sprintf(theCommand, "%s -open %d -ttprocid '%s' %s %s",
                        ...
                        _DtActNULL_GUARD(procId), _DtActNULL_GUARD(tmpFiles),
                        execString);
        }
        ...
        if (!localExecution) {
                ...
                ioMode =
                    SPCIO_NOIO | SPCIO_SYNC_TERMINATOR | SPCIO_FORCE_CONTEXT;

                if ((cmdChannel = (_DtSPCOpen(execHost, ioMode,
                                              errorMessage))) == SPC_ERROR) {
                ...
                netfile = (char *)tt_host_file_netfile(
                    ...
                    ((contextDir == NULL) ? (char *)getenv("HOME")
                                          : contextDir));

                if (tt_pointer_error(netfile) != TT_OK) {
                        (void)sprintf(
                            errorMessage, cmd_Globals.error_directory_name_map,
                            ((contextDir == NULL) ? (char *)getenv("HOME")
                                                  : contextDir),
                            ((contextHost == NULL) ? execHost : contextHost),
                ...
                if ((_DtSPCSpawn(argv[0], context, argv, NULL, cmdChannel,
                                 execHost, contextHost, contextDir,
                                 errorMessage)) == SPC_ERROR) {
                        if (DtSPCErrorNumber != SPC_cannot_Chdir &&
                            ...
                            DtSPCErrorNumber != SPC_Arg_Too_Long)
                                /*
                                 * The Error message must mention that the
                                 * dtexec process is not executable so must
                                 * overwrite the error message returned by the
                                 * Spawn function with an appropriate message.
                                 */
                                (void)sprintf(errorMessage,
                                              errorRemoteSubprocess, execHost,
                                              cmd_Resources.dtexecPath);
                ...
                if ((_DtSPCSpawn(argv[0], context, argv, NULL, cmdChannel,
                                 ...
                                 errorMessage)) == SPC_ERROR) {
                        DtSPCClose(cmdChannel);
                        (void)sprintf(errorMessage, errorRemoteSubprocess,
                                      execHost, argv[0]);
                        Cmd_FreeAllocatedStringVector(commandArray);
                        XtFree((char *)tmp);
                        _DtSvcProcessUnlock();
                 ...
                 *
                 * Since the terminal emulator is pre-appended onto the
                 * execution string, don't want to check it (should have been
                 * done during startup (in _DtInitializeCommandInvoker)) but
                 * must check the execution string that was passed in as part of
                 * the message.
                 */
                ...
                if (!_DtCmdCheckForExecutable(commandArray2[0])) {
                        (void)sprintf(errorMessage, errorExec,
                                      commandArray2[0]);
                        Cmd_FreeAllocatedStringVector(commandArray);
                        _DtCmdFreeStringVector(commandArray2);
                        return (_CMD_EXECUTE_FAILURE);
                ...
                for (index1 = 0; (index1 < 10) && ((commandPid = fork()) < 0);
                     index1++) {
                        /* Out of resources ? */
                        if (errno != EAGAIN)
                                break;
                ...
                if (commandPid == 0) {
                        ...
                        if (!terminalRequest) {
                                ...
                                /*
                                 * Close stdout and redirect it to /dev/null.
                                 * If this is not done and the request writes to
                                 * stdout, the output will be queued in an
                                 * "unlinked" file in /tmp until the client
                                 * using this code terminates.
                                 */
    ...
    unsigned long requestNum)            /* Specifies the request number. */
{
        ...
        if (cause == COMMAND_CHECK_FAILURE) {
                if (pNode->failure_proc != NULL) {
                        (void)sprintf(errorMessage, errorSpawn,
                                      pNode->exec_host, pNode->exec_string);
                        if (cmd_Resources.executionHostLogging)
                                _DtCmdLogErrorMessage(errorMessage);
                        (*pNode->failure_proc)(errorMessage,
                                               pNode->failure_data);
...
int _DtActionCommandInvoke(long wintype, char *cwdHost, char *cwdDir,
                           ...
                           char *procId, char *tmpFiles, void (*success_proc)(),
                           void *success_data, void (*failure_proc)(),
                           void *failure_data) {
        int status;
        char errorMessage[MAX_BUF_SIZE * 2];

        status = _DtCmdCommandInvokerExecute(
            errorMessage, NULL, wintype, cwdHost, cwdDir, NULL, termOpts,
            execHost, execString, procId, tmpFiles, success_proc, success_data,
...

{
        ...
        _DtActionCommandInvoke(_DtAct_NO_STDIO_BIT, context_host, context_dir,
                               exec_string, exec_parameters, exec_host, NULL,
                               NULL, success_proc, success_data, failure_proc,
                               failure_data);
...
static void _DtCmdInitializeErrorMessages(void) {
        ...
        errorChdir = strdup(((char *)Dt11GETMESSAGE(
            ...
            "host \"%s\" failed.\n\nCheck the spelling and permissions and "
        ...
        cmd_Globals.error_terminal = strdup(((char *)Dt11GETMESSAGE(
            ...
            "emulator cannot be executed:\n\n  %s\n\nCheck that the program "
            "exists, has the correct permissions\nand is executable.  This "
            "problem may have occurred because the\nprogram is not in your "
            "\"PATH\".")));

        errorLength = strdup(((char *)Dt11GETMESSAGE(
            3, 9,
            "The total number of characters in this action exceeds the limit "
            "of \"%d\".\n\nYou may need to break the action into more than one "
            "action.")));

        errorFork = strdup(((char *)Dt11GETMESSAGE(
            3, 11,
            "An attempt to start a new process on host \"%s\" failed.\n\nTo "
            "continue, you may need to stop an unneeded process on this "
            "host.")));

        errorRequest = strdup(((char *)Dt11GETMESSAGE(
            3, 17,
            "This action's WINDOW_TYPE \"%s\" is un-recognized.\n\nThe "
            "WINDOW_TYPE should be one of the following:\n\n  %s, %s, %s,\n   "
            "%s, %s, or %s\n")));

        cmd_Globals.error_subprocess = strdup(((char *)Dt11GETMESSAGE(
            3, 18,
            "This action cannot be started because the DT subprocess "
            "program:\n\n   %s\n\ncannot be executed.  Check that the program "
            "has the correct\npermissions and is executable.")));

        errorRemoteSubprocess = strdup(((char *)Dt11GETMESSAGE(
            3, 20,
            "This action cannot be executed on host \"%s\"\nbecause the "
            "following required program either\ndoes not exist or it is not "
            "executable:\n\n   %s\n")));
        ...
        errorRegisterHandshake = XtNewString(((char *)Dt11GETMESSAGE(
            ...
            "user\n\"%s\" has a user id of \"%d\" on host \"%s\" and this "
            "does\nnot match the username and user id on the "
            "action\ninvocation host \"%s\".")));

        errorRegisterUsername = XtNewString(((char *)Dt11GETMESSAGE(
            3, 28,
            "This action cannot be executed on host \"%s\" because\nuser "
            "\"%s\" does not have an account on this host.")));

        errorRegisterNetrc = XtNewString(((char *)Dt11GETMESSAGE(
            3, 29,
            "This action cannot be executed on host \"%s\" because\na pathname "
            "to the authentication file cannot be created.")));
        ...
        errorInetSecurity = XtNewString(((char *)Dt11GETMESSAGE(
            3, 32,
            "This action cannot be executed on host \"%s\" because\nhost "
            "\"%s\" is not authorized to use the \"%s\" service.\n\nTo fix "
            "this, add host \"%s\" to the \"%s\" service\nentry in file \"%s\" "
            "on host \"%s\".")));
        ...
        errorSpcTerminator = strdup((
            (char *)Dt11GETMESSAGE(3, 15,
                                   "An attempt to register the output log from "
                                   "a remote host failed.\n\nTo continue, you "
                                   "may need to stop an existing process.")));

        successHost = strdup(((char *)Dt11GETMESSAGE(
            3, 21,
            "The action \"%s\" was successfully executed on host \"%s\".")));

        cmd_Globals.error_directory_name_map = strdup(((char *)Dt11GETMESSAGE(
            3, 22,
