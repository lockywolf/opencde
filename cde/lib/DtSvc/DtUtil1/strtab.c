...
typedef struct strtab {
        ...
        unsigned int st_stroffset; /* offset (bytes) from this structure to
                                      contained strings 	*/
        unsigned int st_taboffset; /* offset (bytes) from this structure to
                                      table offset           */
...
typedef struct strtab_entry {
        unsigned int
            key; /* offset into contained strings */
        unsigned short
            first; /* offset in table to first string to hash to this spot
                    */
        unsigned short next; /* offset in table to next item to hash to this
                                spot		*/
} strtab_entry_t;
...
typedef struct strlist {
        ...
        DtHashTbl sl_bosons; /* reverse hash table for Nolan's lookup function
                              */
        unsigned int
            sl_charcount; /* count of current bytes of strings */
} strlist_t;
...
const char *_DtShmBosonToString(DtShmStrtab strtab, int boson) {
        const char *s =
            (const char *)strtab + ((strtab_t *)strtab)->st_stroffset;
        return (s + boson);
}
...
DtShmBoson _DtShmStringToBoson(DtShmStrtab strtab, const char *string) {
        ...
        register strtab_entry_t *ptr =
            (strtab_entry_t *)((unsigned char *)strtab +
...
DtShmProtoStrtab _DtShmProtoInitStrtab(int sizeguess) {
...
int _DtShmProtoDestroyStrtab(DtShmProtoStrtab strlist) {
...
DtShmBoson _DtShmProtoAddStrtab(DtShmProtoStrtab strlist, const char *string,
                                int *isnew) {
        strlist_t *ptr = (strlist_t *)strlist;

        int *bucket =
            (int *)_DtUtilGetHash(ptr->sl_hash, (const unsigned char *)string);
        ...
        if (*bucket == 0) /* new */ {
                ...
                *bucket = ret = ptr->sl_charcount;
                sptr = (unsigned char **)_DtUtilGetHash(
                    ptr->sl_bosons, (const unsigned char *)ret);
...
const char *_DtShmProtoLookUpStrtab(DtShmProtoStrtab prototab,
                                    DtShmBoson boson) {
        strlist_t *ptr = (strlist_t *)prototab;
        unsigned char **sptr;

        sptr = (unsigned char **)_DtUtilFindHash(ptr->sl_bosons,
                                                 (const unsigned char *)boson);

        return (sptr ? ((const char *)*sptr) : NULL);
}

int _DtShmProtoSizeStrtab(DtShmProtoStrtab strlist) {
        int foo[2];
        int size;
        strlist_t *ptr = (strlist_t *)strlist;
        ...
        size = sizeof(strtab_t) + (foo[0]) * sizeof(strtab_entry_t) + foo[0] +
               foo[1] + 3;
...
DtShmStrtab _DtShmProtoCopyStrtab(DtShmProtoStrtab in, void *destination) {
        ...
        size = sizeof(strtab_t) + (foo[0]) * sizeof(strtab_entry_t) +
                   (foo[0] + foo[1] + 3) &
               ~3;
        ...
        building.tabstart =
            (strtab_entry_t *)((unsigned char *)ptr + ptr->st_taboffset);
        building.strtab = ptr;
