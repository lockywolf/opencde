...
static void build_file_list(DtShmProtoIntList, DtDirPaths *, DtDtsMMHeader *,
                            const char *);
...
static int build_new_db(DtShmProtoStrtab, DtShmProtoIntList, int,
                        DtDtsDbDatabase **);
static int build_name_list(DtDtsDbDatabase *, DtShmProtoIntList,
                           DtDtsMMHeader *);
...
int _MMWriteDb(DtDirPaths *dirs, int num_db, DtDtsDbDatabase **db_list,
               const char *CacheFile) {
        ...
        header.db_offset =
            build_new_db(shm_handle, int_handle, num_db, db_list);
        ...
        tbl_data = (void *)_DtShmProtoAddIntLst(
            int_handle, tbl_size / sizeof(int), &header.str_tbl_offset);
...
static void build_file_list(DtShmProtoIntList int_handle, DtDirPaths *dirs,
        ...
        for (i = 0; dirs->paths[i]; i++) {
                ...
                mtime_list =
                    (time_t *)realloc(mtime_list, count * sizeof(time_t));

                mtime_list[count - 1] = buf.st_mtime;
                boson_list[count - 1] =
                    _DtShmProtoAddStrtab(shm_handle, dirs->paths[i], &isnew);
                dirp = opendir(".");
                while ((result = _XReaddir(dirp, dirEntryBuf)) != NULL) {
                        ...
                        if (c && strcmp(c, suffix) == 0) {
                                ...
                                sprintf(pathname, "%s/%s", dirs->paths[i],
                                        result->d_name);
                                stat(result->d_name, &buf);
                                count++;
                                boson_list = (int *)realloc(
                                    boson_list, count * sizeof(int));
                                mtime_list = (time_t *)realloc(
                                    mtime_list, count * sizeof(time_t));
                                mtime_list[count - 1] = buf.st_mtime;
                                boson_list[count - 1] = _DtShmProtoAddStrtab(
                                    shm_handle, pathname, &isnew);
                                if (pathname)
                                        free(pathname);
                                continue;
                        }
        ...
        data = _DtShmProtoAddIntLst(int_handle,
                                    count * sizeof(time_t) / sizeof(int),
...
static int db_table_size(int num_db, DtDtsDbDatabase **db_list) {
        int db;
...
static int build_new_db(DtShmProtoStrtab shm_handle,
                        DtShmProtoIntList int_handle, int num_db,
        ...
        new_db_list = (DtDtsMMDatabase *)_DtShmProtoAddIntLst(
            int_handle, num_db * sizeof(DtDtsMMDatabase) / sizeof(int),
            &db_index);
        for (db = 0; db < num_db; db++) {
                int last_boson = -1;
                int list_count = 0;
                ...
                new_db_ptr->databaseName = _DtShmProtoAddStrtab(
                    shm_handle, db_ptr->databaseName, &isnew);
                ...
                for (rec = 0; rec < db_ptr->recordCount; rec++) {
                        ...
                        if (new_rec_ptr->recordName != last_boson) {
                                /* save name position */
                                _DtShmProtoAddInttab(
                                    nameIndex, new_rec_ptr->recordName, rec);

                                last_boson = new_rec_ptr->recordName;
                        }
                        new_rec_ptr->pathId = _DtShmProtoAddStrtab(
                            shm_handle,
                            tmp = _DtDbPathIdToString(rec_ptr->pathId), &isnew);
                        XtFree(tmp);
                        ...
                        new_fld_ptr_list = (DtDtsMMField *)_DtShmProtoAddIntLst(
                            int_handle,
                            rec_ptr->fieldCount * sizeof(DtDtsMMField) /
                                sizeof(int),
                            &index);
                        ...
                        for (fld = 0; fld < rec_ptr->fieldCount; fld++) {
                                ...
                                new_fld_ptr->fieldName =
                                    QtB(fld_ptr->fieldName);
                                new_fld_ptr->fieldValue =
                                    fld_ptr->fieldValue
                                        ? _DtShmProtoAddStrtab(
                                              shm_handle, fld_ptr->fieldValue,
                                              &isnew)
                                        : 0;
                        }
                }
                ...
                idx = _DtShmProtoAddIntLst(int_handle, size / sizeof(int),
                                           &new_db_ptr->nameIndex);
...
static void showtable(DtDtsDbDatabase *db, struct list *name_index,
                      struct list *other, DtDtsMMHeader *head,
                      int other_break) {
        int i;
        ...
        for (i = 0; name_index[i].boson; i++) {
                printf(
                    "%20s -> %s\n",
                    XrmQuarkToString(
                        db->recordList[name_index[i].rec]->recordName),
                    _DtShmProtoLookUpStrtab(shm_handle, name_index[i].boson));
        }
        printf("%d entries\n", i);
        ...
        for (i = 0; i < other_break; i++) {
                printf("%s\n", XrmQuarkToString(
                                   db->recordList[other[i].rec]->recordName));
        }
        printf("%d entries\n", i);
}

static int build_name_list(DtDtsDbDatabase *db, DtShmProtoIntList int_handle,
                           DtDtsMMHeader *head) {
        struct list *other;
        int i;
        char *c;
        ...
        name_index =
            (struct list *)calloc(db->recordCount * 2, sizeof(struct list));
        ...
        for (i = 0; i < db->recordCount; i++) {
                ...
                if (c && *c == '\0') {
                        ...
                        name_index[next].boson = _DtShmProtoAddStrtab(
                            shm_handle, (const char *)attr, &isnew);
        ...
        */
        ...
        for (i = 0; i <= next; i++) {
                if (i != next &&
                    (last_boson == -1 || name_index[i].boson == last_boson)) {
                        /* this a new list of records or an addition to one */
                        list_of_recs = (int *)realloc(
                            list_of_recs, ++list_count * sizeof(int));
                        ...
                        */
                        if (list_count == 1) {
                                ...
                                _DtShmProtoAddInttab(indexList, last_boson,
                                                     list_of_recs[0]);
                        } else {
                                /* if there are multiple items in the list
                                   create a table for them */
                                int *list = _DtShmProtoAddIntLst(
                                    int_handle, list_count, &index);
        ...
        if (other_break > 0) {
                /* create the space */
                int *list = _DtShmProtoAddIntLst(int_handle, other_break,
                                                 &head->no_name_offset);
...
static int write_db(DtDtsMMHeader *header, void *index, int size,
        ...
        if (fd == -1) {
                _DtSimpleError(DtProgName, DtError, NULL, (char *)tmpfile,
                               NULL);
                return (0);
        }
        ...
        if (rename((const char *)tmpfile, CacheFile) == -1) {
                _DtSimpleError(DtProgName, DtError, NULL, (char *)CacheFile,
                               NULL);
...
_DtActionCompareRecordBoson(DtDtsMMRecord *record1, DtDtsMMRecord *record2) {
