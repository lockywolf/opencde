 ...
 *
 *************************************<->***********************************/
void _DtWsmSetWorkspaceHints(Display *display, Window window,
        ...
        *pP++ = pWsHints->flags;
        ...
        XChangeProperty(display, window, property, property, 32,
                        PropModeReplace, (unsigned char *)pPropValue,
                        (iSizeWsHints / sizeof(long)));
}
 ...
 *
 *************************************<->***********************************/
int _DtWsmGetWorkspaceHints(Display *display, Window window,
        ...
        if ((rcode = XGetWindowProperty(
                 ...
                 &actualType, &actualFormat, &lcount, &leftover,
                ...
                if (actualType != property) {
                        ...
                        pP = pProp;
                        pWsH = (DtWorkspaceHints *)XtMalloc(
                            sizeof(DtWorkspaceHints) * sizeof(long));
                        ...
                        if (pWsH->version > 5) {
                                /*
                                 * Assume old version of hints which was a
                        ...
                        if (pWsH->flags & DT_WORKSPACE_HINTS_WORKSPACES) {
                                pWsH->pWorkspaces = (Atom *)XtMalloc(
                                    pWsH->numWorkspaces * sizeof(Atom));
 ...
 *  ---------
 *************************************<->***********************************/
void _DtWsmFreeWorkspaceHints(DtWorkspaceHints *pWsHints) {
        if (pWsHints) {
