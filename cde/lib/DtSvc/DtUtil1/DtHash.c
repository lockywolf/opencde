...
Synopsis:
           ...
           void _DtUtilDestroyHash(void * tbl, int (*des_func)(), void *
usr_arg)
...
table size should be a prime number (a side effect of the hash alorithm).
  ...
  DtUtilMakeIHash performs the same function as DtUtilMakeHash, except that the
...
returned.  If no such entry exists, the routine returns NULL.
  ...
  DtUtilDelHash deletes the specified table entry and returns the associated
data ptr.  If the entry did not exist ( or the data ptr was NULL), the routine
returns NULL.
...
with the specified des_func function as described for DtUtilOperateHash.  All
...
void *_DtUtilDelHash(DtHashTbl t, const unsigned char *key) {
        ...
        if (tmp->right_entry != NULL) /* not first in chain.... */
                tmp->right_entry->left_entry =
                    (tmp->left_entry ? tmp->left_entry->right_entry : NULL);
        else
                tbl->start =
                    (tmp->left_entry ? tmp->left_entry->right_entry : NULL);
        return (tmp->data);
