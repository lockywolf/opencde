 ...
 *
 *************************************<->***********************************/
static void _DtWsmWsChangeHandler(Widget widget, Atom aWS, DtWsmWsReason reason,
        ...
        if (reason == DtWSM_REASON_CURRENT) {
                 ...
                 */
                (*(pCbCtx->ws_cb))(pCbCtx->widget, aWS, pCbCtx->client_data);
        }
 ...
 *
 *************************************<->***********************************/
DtWsmCBContext DtWsmAddCurrentWorkspaceCallback(Widget widget,
         ...
         */
        pCbCtx =
            (struct _DtWsmCBContext *)XtMalloc(sizeof(struct _DtWsmCBContext));
         ...
         */
        pCbCtx->nested_context = (XtPointer)DtWsmAddWorkspaceModifiedCallback(
            widget, (DtWsmWsModifiedProc)_DtWsmWsChangeHandler,
            (XtPointer)pCbCtx);
 ...
 *
 *************************************<->***********************************/
static Tt_callback_action _WsModifiedCB(Tt_message m, Tt_pattern p) {
        struct _DtWsmCBContext *pCbCtx;
        Atom aWs;
         ...
         */
        reason =
            (DtWsmWsReason)strtoul(tt_message_arg_val(m, 2), (char **)NULL, 0);
 ...
 *
 *************************************<->***********************************/
void DtWsmRemoveWorkspaceCallback(DtWsmCBContext pCbCtx) {
        /*
        ...
        if (pCbCtx && (pCbCtx->widget != NULL)) {
                ...
                if (pCbCtx->nested_context) {
                        /*
                         * This was a convenience callback for just the
                         * workspace change info.
                         */
 ...
 *
 *************************************<->***********************************/
String _DtWsmSelectionNameForScreen(int scr) {
        String sName;
...
Tt_callback_action _DtWsmConsumeReply(Tt_message msg, Tt_pattern pat) {
