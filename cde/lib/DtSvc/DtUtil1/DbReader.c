...
static int ReadNextEntry(Boolean *versionCanBeSet, FILE *fd,
                         DtDtsDbField **fieldPtr,
                         DtDbRecordDesc *recordDescriptions,
                         int numRecordDescriptions, int *returnRrecordIndex,
                         char *file_name, int *NumberFieldsAllocated,
                         char **tmpBuffer, char **tmpLine);
static void ResolveVariableReference(char *varName, int varNameLen, char **buf,
                                     int *buf_size, int start, int len,
                                     int *buf_len, int escape, int brackets);
static char *read_line(char string[], int n, FILE *fd);
static void clean_line(char **string);
void _DtDbFillVariables(char **line);
static Boolean get_variable(char *key, int key_len, char **value, int escape,
                            int brackets, char *var);
static void ClearVariables(void);
static Boolean DefineVariable(Boolean *versionCanBeSet, char *varString,
                              char *fileName, int *variableSetSize);
static void SplitField(DtDtsDbField **fields, int *slotToUse, char *ptr,
                       int *NumberFieldsAllocated);
static int MatchKeyword(char *line, DtDbRecordDesc *recordDescriptions,
...
char *_DtDbPathIdToString(DtDbPathId pathId) {
        char *path;
        char *newPath;
...
void _DtDbRead(DtDirPaths *dirs, char *suffix,
               DtDbRecordDesc *recordDescriptions, int numRecordDescriptions) {
        ...
        for (i = 0; dirs->dirs[i] != NULL; i++) {
                char *host = NULL, *dir = NULL, *pch = NULL, *tmp = NULL,
                     *host_prefix = NULL;
                ...
                if ((pch = DtStrchr(tmp, ':')) != NULL) {
                        ...
                        *pch = '\0';
                        if (strcmp(local_host, host)) {
                                ...
                                /*
                                 * The database host is the same as the invoking
                                 * host.
                                 */
                                host_prefix = NULL;
                        }
                ...
                while (db_files->paths[nextIndex] != NULL) {
                        ...
                        if ((fd = fopen(db_files->paths[nextIndex], "r")) ==
                            NULL) {
                                nextIndex++;
                                continue;
                                ;
                        ...
                        while (ReadNextEntry(
                            &versionCanBeSet, fd, &entryFields,
                            recordDescriptions, numRecordDescriptions,
                            &recordIndex, db_files->dirs[nextIndex],
                            &NumberFieldsAllocated, &tmpBuffer, &tmpLine)) {
                                /*
                                 * Invoke all of the callbacks registered for
                                 * this record type, allowing each one to parse
                                 * the entry, and add it to the appropriate
                                 * in-memory database.
                                 */
                                for (j = 0, rejectionStatus = False;
                                     recordDescriptions[recordIndex]
                                         .converters[j];
                                     j++) {
                                        rejectionStatus |=
                                            (*recordDescriptions[recordIndex]
                                                  .converters[j])(
                                                entryFields, pathIndex,
                                                host_prefix, rejectionStatus);
                                }
...
static Boolean get_variable(char *key, int key_len, char **value, int escape,
        ...
        for (i = 0; i < variableCount; i++)
                if ((strncmp(key, variableSet[i].varName, (size_t)key_len) !=
                     0) ||
...
static char *read_line(char string[], /* MODIFIED */
                       int n, FILE *fd) {
...
#endif
                        /*
                         * We have to take care of the case when mblen() returns
                         * -1
                         */
                        if ((len == 1) || (len == -1)) {
...
#endif
                                        /*
                                         * We have to take care of the case when
                                         * mblen() returns -1
                                         */
                                        if ((len == 1) || (len == -1)) {
                                                ret_string[j++] = *ch;
                                                /* Skip this char., unless its
                                                 * the terminater */
                                                if (*ch != '\0')
                                                        ch++;
                                        } else {
...
#endif /* NLS16 */
                {
                        /* This is a bugfix for AIX. Need to return true in case
                         * of one-byte Katakana code.
                         */
                        if (((isgraph(*head)) || (iscntrl(*head))) &&
                            (!isspace((unsigned char)*head)))
                                tail = head;
                        head++;
                }
        }
...
void _DtDbFillVariables(char **line) {
                ...
                if (*start == '$' && !escape) {
...
#ifdef NLS16
                            (!is_multibyte ||
                             (mblen(replacementEnd, MB_CUR_MAX) == 1)) &&
#endif
                            (*replacementEnd == '{')) {
                                replacementEnd++;
                                bracketSeen = TRUE;
                        ...
                        if (!bracketSeen) {
                                ...
                                ResolveVariableReference(
                                    variableStart, variableEnd - variableStart,
                                    line, &lineSize, currentOffset,
                                    replacementEnd - replacementStart, &lineLen,
                                    escape, brackets);
         ...
         *      This function is responsible for handling syntactic things like
         *string variables and comments.
         *
         ***********************/
...
static int ReadNextEntry(Boolean *versionCanBeSet, FILE *fd,
                         DtDtsDbField **fieldPtr,
                         DtDbRecordDesc *recordDescriptions,
                         int numRecordDescriptions, int *returnRecordIndex,
                         char *file_name, int *NumberFieldsAllocated,
                         char **tmpBuffer, char **tmpLine) {
        int i;
        int len;
        ...
        while (1) {
                ...
                if ((strncmp(start, "set ", 4) == 0) && (strlen(start) >= 4)) {
                        if (!DefineVariable(versionCanBeSet, start + 4,
                                            file_name, &variableSetSize)) {
                                /*
                                 * An attempt was made to redefine the Db
                                 * version; this is only allowed as the first
                                 * keyword in a file.  We will ignore this file.
                                 * An error msg was already reported.
                                 */
                                return (False);
                        }

                        /*
                         * Once we've read something other than a comment, then
                         * the version can no longer be changed.  We also need
                         * to verify that the specified version string is valid.
                         */
                        if (*versionCanBeSet) {
                                *versionCanBeSet = False;
                                ...
                                if ((strlen(version) > 0) &&
                                    (strcmp(version, VERSION_ID))) {
                                        /* Bad version number; give error, and
                                         * ignore this file */
                                        error_buffer = XtMalloc(MAXPATHLEN);
                                        sprintf(error_buffer, invalidVersion,
                                                file_name);
                                        _DtSimpleError(DtProgName, DtError,
                                                       NULL, "%s",
                                                       error_buffer);
                                        XtFree(version);
                                        if (error_buffer)
                ...
                *versionCanBeSet = False;
                ...
                if ((*returnRecordIndex =
                         MatchKeyword(start, recordDescriptions,
                ...
                /*
                 * At this point, the beginning of a definition has been
                 * recognized. Must now get the data for the fields.
                 */
                clean_line(tmpLine);
                if (strchr(*tmpLine, '$'))
                ...
                line_cont =
                    False; /* True if the current line has a continuation
                ...
                while (1) {
                        ...
                        if (read_line(*tmpLine, MAX_LINE_LENGTH, fd) ==
                            NULL) /* EOF */
                        {
                                if (multi_line || (indx >= 1)) {
                                        ...
                                        errorName =
                                            (*fieldPtr)[0].fieldValue
                                                ? (*fieldPtr)[0].fieldValue
                                                : XrmQuarkToString(
                                                      (*fieldPtr)[0].fieldName);
                                        (void)sprintf(error_buffer,
                                                      incompleteDefn, errorName,
                                                      file_name);
                                        _DtSimpleError(DtProgName, DtError,
                                                       NULL, "%s",
                                                       error_buffer);
                        ...
                        if ((strncmp(start, "set ", 4) == 0) &&
                            (strlen(start) >= 4)) {
                                if (!DefineVariable(versionCanBeSet, start + 4,
                                                    ...
                                                    &variableSetSize)) {
                                        /*
                                         * An attempt was made to redefine the
                                         * Db version; this is only allowed as
                                         * the first keyword in a file.  We will
                                         * ignore this file.  An error msg was
                                         * already reported.
                                         */
                                        remove_defn = True;
                                        break;
                        ...
                        if (strncmp(start, "{", 1) == 0) {
                                /* if '{' found in the middle of definition,
                                 * ignore defn */
                                ...
                                startDef = START_SYMBOL; /* not at start, '{'
                                                            symbol found  */
                                continue;
                        }

                        /* we are no longer at definition start, note if start
                         * symbol found */
                        if (startDef == AT_START) {
                                if (error_buffer)
                                        XtMalloc(MAXPATHLEN);
                                errorName = (*fieldPtr)[0].fieldValue
                                                ...
                                                : XrmQuarkToString(
                                                      (*fieldPtr)[0].fieldName);
                                (void)sprintf(error_buffer, missingStartSymbol,
                                              errorName, file_name);
                                _DtSimpleError(DtProgName, DtError, NULL, "%s",
                                               error_buffer);
                                if (error_buffer)
                                        XtFree(error_buffer);
                                remove_defn = True;
                        ...
                        if (strncmp(start, "}", 1) == 0) {
                                ...
                                /* if '}' end found with no starting '{', ignore
                                 * defn */
                                if (strncmp(start, "}", 1) == 0 &&
                                    startDef != START_SYMBOL) {
                                        error_buffer = XtMalloc(MAXPATHLEN);
                                        errorName =
                                            (*fieldPtr)[0].fieldValue
                                                ? (*fieldPtr)[0].fieldValue
                        ...
                        /*
                         * At this point, "line" contains data for a field.  If
                         * "indx" is already equal to "num_fields - 1" (the last
                         * entry is a NULL), then something has gone wrong with
                         * this definition - like a field is missing the
                         * continuation mark.
                         */
                        if ((recordDescriptions[*returnRecordIndex].maxFields !=
                             DTUNLIMITEDFIELDS) &&
                            (indx > recordDescriptions[*returnRecordIndex]
                                        .maxFields)) {
                                error_buffer = XtMalloc(MAXPATHLEN);
                                errorName = (*fieldPtr)[0].fieldValue
                                                ? (*fieldPtr)[0].fieldValue
                                                : XrmQuarkToString(
                                                      (*fieldPtr)[0].fieldName);
                                (void)sprintf(error_buffer, tooManyFields,
                                              errorName, file_name);
                                _DtSimpleError(DtProgName, DtError, NULL, "%s",
                                               error_buffer);
                                if (error_buffer)
                        ...
                        /* Need to see if this line is part of a multi_line
                         * field. */
                        if ((last = DtStrrchr(start, '\\')) != NULL) {
                                /*
                                 * Need to see if 'last' is the last
                                 * non-whitespace char. Strip off leading white
                                 * space.
                                 */
                                ...
                                if (*start == '\0') {
                                        /*
                                         * Only whitespace followed the slash,
                                         * so remove it unless the slash was
                                         * escaped.
                                         */
                                        if (
#ifdef NLS16
                        ...
                        if (multi_line) {
                                /*
                                 * This is a multi-line field - the previous
                                 * line was continued so append the current line
                                 * to the previous line "tmpBuffer".
                                 */
                                int tmp_len =
                                    strlen(*tmpBuffer) + strlen(start) + 2;
                                if (tmp_len >= MAX_LINE_LENGTH)
                                        *tmpBuffer = (char *)XtRealloc(
                                            (char *)*tmpBuffer, tmp_len);
                                (void)strcat(*tmpBuffer, start);

                                if (!line_cont) {
                                        /*
                                         * This line is not continued so the
                                         * field is complete - save the data.
                                         */
                                ...
                                if (line_cont) {
                                        /*
                                         * This line has a continuation mark so
                                         * save it in "tmpBuffer".
                                         */
                                        ...
                                        /*
                                         * This field is a single-line field.
                                         * Save it.
                                         */
                                        clean_line(&start);
...
static void ResolveVariableReference(char *varName, int varNameLen, char **buf,
        ...
        *var = '\0';
        if ((get_variable(varName, varNameLen, &value, escape, brackets,
                ...
                /*
                 * The way in which we do our work will be dependent upon
                 * whether the replacement value is longer, shorter or equal in
                 * size to the variable reference.
                 */
                if (valueLen == len) {
                        ...
                        /*
                         * The replacement value is shorter than the reference
                         * currently in the input line; write in the replacement
                         * value, then shift down the remaining contents of the
                         * buffer, overwriting the leftover portion of the
                         * variable reference.
                         */
                        (void)strncpy(*buf + start, value, (size_t)valueLen);
                        (void)strcpy(*buf + start + valueLen,
                                     *buf + start + len);
                        *buf_len -= (len - valueLen);
                } else {
                        /*
                         * The replacement value is longer than the variable
                         * reference; we may need to grow the input line.  In
                         * any case, before we can copy in the replacement
                         * value, we need to shift the remainder of the input
                         * line up, so that we will no overwrite it.
                         */
                        delta = valueLen - len;
                        if (*buf_len + delta >= *buf_size) {
                                /* Grow the buffer */
                                *buf_size += delta + 100;
                                *buf = (char *)XtRealloc((void *)*buf,
                                                         (size_t)*buf_size + 1);
                        }
...
static Boolean DefineVariable(Boolean *versionCanBeSet, char *varString,
        ...
        if (variableCount >= *variableSetSize) {
                *variableSetSize += 10;
                if (variableSet == NULL) {
                        variableSet = (VariableEntry *)malloc(
                            sizeof(VariableEntry) * (*variableSetSize));
                } else {
                        variableSet = (VariableEntry *)realloc(
                            (void *)variableSet,
                            (size_t)(sizeof(VariableEntry) *
                                     (*variableSetSize)));
                }
        }
...
static void SplitField(DtDtsDbField **fields, int *slotToUse, char *ptr,
        ...
        if (ptr != NULL) {
                ...
                *nextChar = savedChar;
                ...
                if (*nextChar) {
                        (*fields)[*slotToUse].fieldValue =
                            XtMalloc(strlen(nextChar) + 1);
...
static int MatchKeyword(char *line, DtDbRecordDesc *recordDescriptions,
        ...
        for (i = 0; i < numRecordDescriptions; i++) {
                ...
                if (keyword && (strncmp(line, keyword, strlen(keyword)) == 0)) {
                        /*
                         * A preliminary match was found.  Now, verify that the
                         * keywords match completely, by making sure that the
                         * next character in the line is either a white space or
                         * a NULL; i.e. "foo" and "foobar" would pass the
                         * preliminary test above, but would fail the second
                         * half of the test.
                         */
...
static void InitializeLocalizedStrings(void) {
        incompleteDefn = strdup(((char *)Dt11GETMESSAGE(
            4, 8,
            "The definition \"%s\" in the file\n  \"%s\"\n  is incomplete. The "
            "definition may be missing\n  the \"}\" field.\n")));

        multiLineDefn = strdup(((char *)Dt11GETMESSAGE(
            4, 3,
            "A multi-line field in the definition \"%s\"\n  in the file\n  "
            "\"%s\"\n  is incomplete.  A \"\\\" character may be missing\n  in "
            "the multi-line field.\n")));

        tooManyFields = strdup(
            ((char *)Dt11GETMESSAGE(4, 4,
                                    "The definition \"%s\" in the file\n  "
                                    "\"%s\"\n  has too many fields.\n")));

        emptyTypesDirs = strdup(((char *)Dt11GETMESSAGE(
            4, 5,
            "An attempt to read the action and filetypes databases failed.\n  "
            "This may be caused by the resource \"Dt.TypesDirs\" being "
            "incorrectly set.\n")));

        noStartSymbol = strdup(((char *)Dt11GETMESSAGE(
            4, 6,
            "The definition \"%s\" in the file\n  \"%s\"\n  has an end symbol "
            "'}' with no start symbol '{'.\n")));

        invalidStartSymbol = strdup(((char *)Dt11GETMESSAGE(
            4, 7,
            "The definition \"%s\" in the file\n  \"%s\"\n  has an unexpected "
            "start symbol '{'.\n")));

        missingStartSymbol = strdup(((char *)Dt11GETMESSAGE(
            4, 9,
            "The definition \"%s\" in the file\n  \"%s\"\n  is missing a start "
            "symbol '{'.\n")));

        cantSetVersion = strdup(((char *)Dt11GETMESSAGE(
            4, 10,
            "The DtDbVersion variable can only be set\nat the beginning of the "
            "file.  The remainder of the file\n'%s' is being ignored.\n")));

        invalidVersion = strdup(((char *)Dt11GETMESSAGE(
            4, 11,
