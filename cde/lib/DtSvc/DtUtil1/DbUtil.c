...
static char DTDATABASESEARCHPATH_DEFAULT[] =
    "%s/.dt/types/%%L,"
    "%s/.dt/types," CDE_CONFIGURATION_TOP
    "/appconfig/types/%%L," CDE_CONFIGURATION_TOP
    "/appconfig/types," CDE_INSTALLATION_TOP
    "/appconfig/types/%%L," CDE_INSTALLATION_TOP "/appconfig/types";

/****   Substitution records used by XtFindFile() in _DtExpandLang()   ****/
static SubstitutionRec langSubstitutions[] = {{'L', (char *)NULL},
                                              {'l', (char *)NULL},
                                              {'t', (char *)NULL},
                                              {'c', (char *)NULL}};
...
static void __setupLangSubstitutions(void) {
        ...
        if ((tcPtr = DtStrchr(ttPtr ? ttPtr : tlPtr, '.')) != (char *)NULL) {
                endPtr = tcPtr++;
                if (*tcPtr != '\0')
                        codesetPart = XtNewString(
                            tcPtr); /* free'd in __freeLangSubstitutions() */
        }
        ...
        if (ttPtr) {
                if (endPtr) {
                        ...
                        if (ttLen > 0) {
                                ...
                                territoryPart = (char *)XtMalloc((ttLen + 1) *
                                                                 sizeof(char));
                                strncpy(territoryPart, ttPtr, ttLen);
                                territoryPart[ttLen] = '\0';
                        }
                } else
                        territoryPart = XtNewString(
                            ttPtr); /* free'd in __freeLangSubstitutions() */
        ...
        if (endPtr) {
                ...
                if (tlLen > 0) {
                        /* free'd in __freeLangSubstitutions() */
                        languagePart =
                            (char *)XtMalloc((tlLen + 1) * sizeof(char));
                        strncpy(languagePart, tlPtr, tlLen);
                        languagePart[tlLen] = '\0';
                }
        } else
                languagePart = XtNewString(
                    tlPtr); /* free'd in __freeLangSubstitutions() */
...
static char *_DtExpandLang(char *string) {
        ...
        for (tmpPtr = string; (tmpPtr1 = DtStrchr(tmpPtr, ':')) != (char *)NULL;
             tmpPtr = tmpPtr1 + 1) {
             ...
             thisPath = DtStrtok_r((char *)NULL, ",", &tokPtr)) {
                ...
                if (modPath) {
                        ...
                        if (pathLen + modLen + 2 > maxPathLen) {
                                maxPathLen =
                                    ((pathLen + modLen + 2 + MAXPATHLEN) /
                                     MAXPATHLEN) *
                                    MAXPATHLEN;
                                newPath = (char *)XtRealloc(
                                    newPath, maxPathLen * sizeof(char));
                        }
...
DtDirPaths *_DtFindMatchingFiles(DtDirPaths *dirs, char *suffix,
                                 Boolean sort_files) {
        /* LOCAL VARIABLES */

        register DtDirPaths
            *files;         /* An array of pointers to the filenames which
                            have been found. */
        ...
        while (dirs->paths[nextIndex] != NULL) {
                ...
                while ((result = _XReaddir(dirp, dirEntryBuf)) != NULL) {
                        ...
                        if (strlen(result->d_name) >= strlen(suffix)) {
                                /* Find the end of the name and compare it to
                                 * the suffix. */
                                /* Get the number of chars (not bytes) in each
                                 * string */
                                suffixLen = DtCharCount(suffix);
                                nameLen = DtCharCount(result->d_name);
                                file_suffix = _DtGetNthChar(
                                    result->d_name, nameLen - suffixLen);
                                if (file_suffix &&
                                    (strcmp(file_suffix, suffix) == 0) &&
                                    !_DtIsDir((char *)next_path,
                                              (char *)result->d_name)) {

                                        /* The file is a match.  See if there is
                                           room in the array or whether we need
                                           to realloc.  The "-1" is to save room
                                        ...
                                        /* Get some memory and copy the filename
                                         * to the array. */
                                        files->dirs[num_found] = next_file =
                                            (char *)XtMalloc((Cardinal)(
                                                strlen(dirs->dirs[nextIndex]) +
                                                strlen(result->d_name) + 2));
                                        (void)strcpy(next_file,
                                                     dirs->dirs[nextIndex]);
                                        (void)strcat(next_file, "/");
                                        (void)strcat(next_file, result->d_name);

                                        files->paths[num_found] = next_file =
                                            (char *)XtMalloc((Cardinal)(
                                                strlen(next_path) +
                                                strlen(result->d_name) + 2));
...
char *_DtDbGetDataBaseEnv(void) {
        ...
        snprintf(temp_buf, slen - 1, DTDATABASESEARCHPATH_DEFAULT, nwh_dir,
                 nwh_dir);
        return temp_buf;
}
...
DtDirPaths *_DtGetDatabaseDirPaths(void) {
        ...
        for (next_dir = dir_vector; *next_dir != NULL; next_dir++) {
                ...
                if (!_DtIsOpenableDirContext(*next_dir, &internal)) {
                        ...
                        for (i = 0; i < valid_dirs; i++) {
                                if (strcmp(ret_paths->paths[i], internal) ==
                                    0) {
                                        break;
                                }
                        }
                        ...
                        if (i == valid_dirs) {
                                valid_dirs++;
                                ret_paths->dirs = (char **)XtRealloc(
                                    (char *)ret_paths->dirs,
                                    (Cardinal)(sizeof(char *) * valid_dirs));

                                /* Make sure the directory name is
                                   fully-qualified with a host component. */
                                if (DtStrchr(*next_dir, ':') != NULL)
                                        dir = XtNewString(*next_dir);

                                /* If there is no host component, see if there
                                   is an absolute path. */
                                else if (
#ifdef NLS16
                                    (!is_multibyte ||
                                     (mblen(*next_dir, MB_LEN_MAX) == 1)) &&
#endif
                                    (**next_dir == '/')) {
                                        dir = XtMalloc(
                                            (Cardinal)(strlen(nwh_host) + 2 +
                                                       strlen(*next_dir)));
                                        (void)sprintf(dir, "%s:%s", nwh_host,
                                                      *next_dir);
                                } else
                                        dir = XtNewString(*next_dir);

                                ret_paths->dirs[valid_dirs - 1] = dir;
                                ret_paths->paths = (char **)XtRealloc(
                                    (char *)ret_paths->paths,
                                    (Cardinal)(sizeof(char *) * valid_dirs));
        ...
        ret_paths->dirs =
            (char **)XtRealloc((char *)ret_paths->dirs,
                               (Cardinal)(sizeof(char *) * (valid_dirs + 1)));
        ret_paths->dirs[valid_dirs] = NULL;
        ret_paths->paths =
            (char **)XtRealloc((char *)ret_paths->paths,
                               (Cardinal)(sizeof(char *) * (valid_dirs + 1)));
