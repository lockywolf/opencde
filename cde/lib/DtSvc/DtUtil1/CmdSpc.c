...
SbInputId _DtCmdSPCAddInputHandler(int fd, SbInputCallbackProc proc,
                                   void *data) {
        if (cmd_Globals.app_context == NULL)
                return (XtAddInput(fd, (XtPointer)XtInputReadMask, proc, data));
        else
                return (XtAppAddInput(cmd_Globals.app_context, fd,
                                      (XtPointer)XtInputReadMask, proc, data));
}

/******************************************************************************
 *
 * _DtCmdSPCAddExceptionHandler - Register the Command Invoker
 *AddExceptionHandler
 *
...
SbInputId _DtCmdSPCAddExceptionHandler(int fd, SbInputCallbackProc proc,
                                       void *data) {
        if (cmd_Globals.app_context == NULL)
                return (
                    XtAddInput(fd, (XtPointer)XtInputExceptMask, proc, data));
        else
                return (XtAppAddInput(cmd_Globals.app_context, fd,
                                      (XtPointer)XtInputExceptMask, proc,
                                      data));
}
...
void _DtCmdLogErrorMessage(char *message) {
