...
void _DtProcessTtRequest(Widget w, ActionRequest *request, char *relPathHost,
                         char *relPathDir);

Tt_status _DtInitializeToolTalk(Widget w);

extern void _DtCreateErrorDialog(Widget w, char *actionName, XmString msg);
extern Boolean _DtCompileMessagePiece(Widget w, ActionRequest *request,
...
extern void _DtFreeRequest(register ActionRequest *request);
/********    End Public Function Declarations    ********/

/********    Static Function Declarations    ********/

static Boolean ResolveTtRequestMessagePieces(Widget w, ActionRequest *request,
                                             char *relPathHost,
                                             char *relPathDir);

static void InitiateTtRequest(Widget w, ActionRequest *request);

static Tt_callback_action TtRequestCallbackHandler(Tt_message message,
                                                   Tt_pattern pattern);
...
static Tt_message _DtTtContractIgnoreMsgCB(Tt_message msg, void *clientdata,
                                           Tt_message contract);
...
static Tt_callback_action _DtActReceiveExecId(Tt_message msg, Tt_pattern pat) {
        ...
        switch (
            tt_message_arg_ival(msg, 1, &childId)) /* child Id (arg 1 value ) */
        {
...
static Tt_callback_action _DtActReceiveDoneMsg(Tt_message msg, Tt_pattern pat) {
        ...
        switch (tt_message_arg_ival(msg, 2,
                                    &doneCode)) /* done code (arg 2 value ) */
        {
...

{
        ...
        if (errorMsg) {
                buf =
                    XtMalloc(strlen(ToolTalkErrorMsg2) + strlen(errorMsg) + 10);
...

{
        ...
        if (ResolveTtRequestMessagePieces(w, request, relPathHost,
                                          relPathDir)) {
                /*
                ...
                /*
                 * fdt: there really needs to be some policy defining where an
                 * error message will be returned.  Also, is this really the
                 * right message to display here?  Was the failure really caused
                 * by a file mapping problem?  In fact, is this a case we even
                 * can get to???? [Yes]
                 */
                /* fdt: this string must be localized */
                ReportToolTalkError(w, request->clonedAction->label,
                                    TtFileArgMapErr);
        }
}
...

{
        ...
        if (_DtCompileMessagePiece(w, request, relPathHost, relPathDir,
                                   &(tt->tt_op), True, 0, &paramUsed,
                                   &promptDataIndex) &&
            _DtCompileMessagePiece(w, request, relPathHost, relPathDir,
                                   &(tt->tt_file), False, 0, &paramUsed,
                                   &promptDataIndex)) {
                for (i = 0, success = True; (i < tt->value_count) && success;
                     i++) {
                        success = _DtCompileMessagePiece(
                            w, request, relPathHost, relPathDir,
 ...
 */
Tt_message Ttmedia_to_Dt_StatusUpdateCB(Tt_message message, void *clientdata,
                                        Tttk_op op, unsigned char *contents,
                                        int len, char *file) {
        CallbackData *data;
        char *errorMsg;
        Tt_state state;
        ...
        /*
         * Process a TT_FAILED, TT_HANDLED, TT_RETURNED or TT_SEND related to
         ...
         *
         * data = (CallbackData *) tt_message_user( message, 0 );   not good
         * enough
         */
        ...
        if (state == TT_FAILED) {
                if (status != TT_DESKTOP_ECANCELED) {
                        /*
                         * Determine whether ToolTalk or the receiver failed the
                         * message
                         */
                ...
                if (file == NULL) {
                        ...
                        if (upIdx != -1) {
                                if ((IS_BUFFER_OBJ(invRec->info[upIdx].mask) &&
                                     IS_WRITE_OBJ(invRec->info[upIdx].mask))) {
                                         ...
                                         */
                                        upVType =
                                            tt_message_arg_type(message, i);

                                        newArgp[upIdx].argClass =
                                            DtACTION_BUFFER;
                                        newArgp[upIdx].u.buffer.size = len;
                                        newArgp[upIdx].u.buffer.type =
                                            XtNewString(upVType);
                                        newArgp[upIdx].u.buffer.writable = 1;

                                        newArgp[upIdx].u.buffer.bp =
                                            (void *)XtMalloc(len);
                                        memcpy(newArgp[upIdx].u.buffer.bp,
                                               contents, len);
                        ...
                        /*
                         * Calculate index into original argument list from the
                         * user.  Notice that we're shipping data because of
                         * TT_FILE and not TT_ARG0_FILE.  To do this, argMap is
                         * arranged as: argMap[ ARG0, ... , ARGn,
                         * TT_FILE].argIdx
                         *
                         * The array-index for TT_FILE is *one beyond* all the
                         * normal TT_ARGn_VALUE array-index values.   The .argN
                         * for TT_FILE will be -1 fyi.
                         *
        ...
        else if ((message == childRec->u.tt.reqMessage) &&
                 (state == TT_FAILED)) {
                if (status == TT_DESKTOP_ECANCELED)
...

{
        ...
        if (state == TT_FAILED) {
                ...
                for (i = 0; i < totalArgs; i++) {
                        /*
                         * Look through the child's arg list to see which ones
                         * need to be returned.
                         */
                        upIdx = childRec->argMap[i].argIdx;
                        ...
                        if (upIdx != -1) {
                                if ((IS_BUFFER_OBJ(invRec->info[upIdx].mask) &&
                                    ...
                                    IS_FILE_OBJ(invRec->info[upIdx].mask)) {
                                         ...
                                         */
                                        argRepType =
                                            _DtAct_tt_message_arg_reptype(
                                                message, i);

                                        /*
                                         * If we think TT_REP_BUFFER so far, but
                                         * don't have an argClass of
                                         * DtACTION_BUFFER, then we really have
                                        ...
                                        case DtACT_TT_REP_STRING:
                                                /*
                                                 * By convention, if we're here,
                                                 * the buffer is writable.
                                                 * Also, STRINGs can be fetched
                                                 * using bval.
                                                 */
                                                status = tt_message_arg_bval(
                                                    message, i,
                                        ...
                                        case DtACTION_BUFFER:
                                                /*
                                                 * Convert from tt_free()-able
                                                 * to XtFree()-able.
                                                 */
                                                newArgp[upIdx].argClass =
                                                    upArgClass;
                                                newArgp[upIdx].u.buffer.size =
                                                    upttbuflen;
                                                newArgp[upIdx].u.buffer.type =
                                                    XtNewString(upVType);
                                                newArgp[upIdx]
        ...
        return (
            (Tt_callback_action)TT_CALLBACK_PROCESSED); /* message consumed */
}
...

{
            ...
            (tt->tt_argn_rep_type[0] != DtACT_TT_REP_INT)) {
                ...
                if (tt->mode_count == 2) {
                        if ((tt->tt_argn_mode[1] != TT_IN) ||
                            ...
                            (strcmp(tt->tt_argn_vtype[1].compiledMessage,
                                    "title"))) {
                                ttmedia_test = 0;
                        }
                }
        ...
        if (ttmedia_test) {
                 ...
                 * TT_MEDIA_LOAD()
                 *
                 * Special case media-load requests *and* any other requests
                 * that follow the "ttmedia style".
                 *
                ...
                if (bufFilename) {
                        contents = (unsigned char *)_DtActReadTmpFileToBuffer(
                            bufFilename, &length);
                        /* length plucked from above */
                } else {
                        contents = (unsigned char *)tt->tt_argn_value[0]
                                       .compiledMessage;
                ...
                message =
                    ttmedia_load((Tt_message)0, /* Am top level req */
                                 ...
                                 tt->tt_argn_vtype[0].compiledMessage, contents,
                                 length, file, docname, FALSE);
                ...
                if (!tt_is_err(status)) {
                        ...
                        actChildRecP->childState =
                            _DtActCHILD_PENDING_START; /* starting */

                        /*
                         * I'm assuming w is non-NULL, hence the lack of a
                         * callback and callback data.
                         */
                        subcon_patterns = ttdt_subcontract_manage(
                            message, (Ttdt_contract_cb)_DtTtContractIgnoreMsgCB,
                            w, (void *)NULL);
                ...
                if (status != TT_OK) {
                        ...
                        if ((tt_pointer_error(errorMsg) == TT_OK) && errorMsg) {
                                errorMsg = WrapMessageLines(errorMsg);
                                ReportToolTalkError(
                                    w, request->clonedAction->label, errorMsg);
                                tt_free(errorMsg);
                        } else
                                ReportToolTalkError(
                                    w, request->clonedAction->label, NULL);

                        /*
                         * Free information since no callbacks will be
                         * triggered:
                         ...
                         */
                        actChildRecP->u.tt.TTProcId =
                            tt_message_sender(message);
                 ...
                 * FREE FORM TT_MSG
                 *
                 * All other Requests and Notices go out "free form".  Do best
                 * case.
                 */
                ...
                for (i = 0; i < tt->mode_count; i++) {
                        ...
                        length = -1; /* -1 means not a buffer */
                        bufFilename =
                            (char *)NULL; /* possible buffer tmp file */

                        if (actChildRecP->argMap[i].argIdx != -1) {
                                if (IS_BUFFER_OBJ(
                                        actInvRecP
                                            ->info[actChildRecP->argMap[i]
                                                       .argIdx]
                                            .mask)) {
                                        length =
                                            actInvRecP
                        ...
                        if ((i < tt->rep_type_count) &&
                            (tt->tt_argn_rep_type[i] !=
                             DtACT_TT_REP_UNDEFINED)) {
                                ...
                                /*
                                 * No representation type given. If the argument
                                 * was provided and is a DtACTION_BUFFER, then
                                 * use DtACT_TT_REP_BUFFER. In all other cases,
                                 * use DtACT_TT_REP_STRING.
                                 */
                                if (length != -1)
                        ...
                        if (i < tt->value_count) {
                                ...
                                if (length != -1) {
                                        /*
                                         * Have a buffer - see if it's value can
                                         * be fetched from a tmp file.   If so,
                                         * will need to free later!
                                         */
                                        if (bufFilename) {
                                                value =
                                                    _DtActReadTmpFileToBuffer(
                                                        bufFilename, &length);
                                        } else {
                                                value = tt->tt_argn_value[i]
                                                            .compiledMessage;
                                        }
                                } else {
                                        value = tt->tt_argn_value[i]
                                ...
                                /*
                                 * Use the specified vtype setting, if supplied;
                                 * else default it
                                 */
                                if (i < tt->vtype_count)
                                        vtype = tt->tt_argn_vtype[i]
                                                    .compiledMessage;
                                else {
                                        /*
                                         * Time to guess what the vtype should
                                         * be.  This is very funky.
                                         */
                        ...
                        /*
                         * Based on REP_TYPE, add the argument to the message.
                         * In effect, rep_type is an instruction on how to
                         * package the argument in the message.
                         */
                        if (repType == DtACT_TT_REP_INT) {
                                if (value)
                                ...
                                tt_message_iarg_add(message, mode, vtype,
                                                    intValue);
                        } else if (repType == DtACT_TT_REP_STRING) {
                        ...
                        if ((length != -1) && (bufFilename)) {
                                /*
                                 * The value was from a buffer that we pulled
                                 * from a tmp file. Free the transition buffer
                                 * since Tooltalk will have made its own copy by
                                 * now.
                                 */
                                XtFree(value);
                        }
                ...
                if (tt->tt_class == TT_REQUEST) {
                        /*
                         * For a request, we need to attach a callback, which
                         * will be invoked when the reply is received.  When the
                         * reply is received, then we can free up the original
                         * message, and determine whether we succeeded or
                         * failed.
                         */
                        ...
                        data = (CallbackData *)XtMalloc(sizeof(CallbackData));
                        data->actionLabel =
                            XtNewString(request->clonedAction->label);
                        data->associatedWidget = w;
                        data->actionPtr = action;
                        data->requestPtr = _DtCloneRequest(request);
                        data->actInvId = actInvRecP->id;
                        ...
                        tt_message_callback_add(message,
                                                TtRequestCallbackHandler);
                }
                 ...
                 */
                subcon_patterns = ttdt_subcontract_manage(
                    message, (Ttdt_contract_cb)_DtTtContractIgnoreMsgCB, w,
                    (void *)NULL);
                ...
                if (status != TT_OK) {
                         ...
                         * DtActionInvoke() will do a child evaluation and
                         * cleanup.
                         */
                        actChildRecP->childState = _DtActCHILD_FAILED;
                        ...
                        /*
                         * Clean up now, cause there will not be a reply coming
                         * back
                         */
                        if (tt->tt_class == TT_REQUEST) {
                                _DtFreeRequest(data->requestPtr);
                        ...
                        if (tt->tt_class == TT_NOTICE) {
                                 ...
                                 * The child is done.
                                 *
                                 * By marking the child as _DtActCHILD_DONE*,
                                 * see the comment at the end of this routine
                                 * that describes how DtActionInvoke() will do a
                                 * child evaluation and cleanup.
                                 */
                                actChildRecP->childState = _DtActCHILD_DONE;

                                /*
                                 * Need to clean up after a notify, since there
                                 * is no response.
                                 */
                                tttk_message_destroy(message);
                        } else {
...
Tt_status _DtInitializeToolTalk(Widget w) {
        ...
        if (!RegisteredPatterns) {
                ...
                tt_pattern_callback_add(
                    pat, (Tt_message_callback)_DtActReceiveDoneMsg);
                tt_pattern_session_add(pat, session_id);
