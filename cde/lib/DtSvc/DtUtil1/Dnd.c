...
static void dndInitTargets(Display *dpy) {
        _DtSvcProcessLock();
        if (XA_TARGETS != 0) {
                _DtSvcProcessUnlock();
                return;
        ...
        XA_SUN_ATM_METHODS = DtGetAtom(dpy, "_SUN_ALTERNATE_TRANSPORT_METHODS");
        _DtSvcProcessUnlock();
}
...
static DtDndProtocol dndProtocolList[] = {
    DtDND_FILENAME_TRANSFER, DtDND_BUFFER_TRANSFER, DtDND_TEXT_TRANSFER,
    DtDND_NOOP_TRANSFER};
 ...
 */
static DtDndMethods *dndGetTransferMethods(Display *display,
 ...
 */
DtDndTransfer *_DtDndCreateExportTransfer(DtDragInfo *dtDragInfo) {
        ...
        (*transfer->methods->getExportTargets)(dtDragInfo, &(transfer->targets),
                                               &(transfer->numTargets));
 ...
 */
DtDndTransfer *_DtDndCreateImportTransfers(DtDropInfo *dtDropInfo,
        ...
        *numTransfers = 0;
        ...
        transfers =
            (DtDndTransfer *)XtMalloc(*numTransfers * sizeof(DtDndTransfer));
        ...
        for (ii = 0, jj = 0; dndProtocolList[ii] != DtDND_NOOP_TRANSFER; ii++) {
                if (dtDropInfo->protocols & dndProtocolList[ii]) {
                        ...
                        (*transfer->methods->getImportTargets)(
                            dtDropInfo, &(transfer->targets),
                            &(transfer->numTargets));
                }
        }
 ...
 */
void _DtDndDestroyTransfers(DtDndTransfer *transfers, Cardinal numTransfers) {
        Cardinal ii;
 ...
 */
DtDndTransfer *_DtDndTransferFromTargets(DtDndTransfer *transfers,
                                         Cardinal numTransfers, Atom *targets,
                                         Cardinal numTargets) {
        Cardinal ii, jj, kk;
 ...
 */
void _DtDndTransferAdd(Widget dropTransfer, DtDropInfo *dtDropInfo,
                       Atom *transferTargets, Cardinal numTransferTargets) {
        ...
        transferEntries = (XmDropTransferEntryRec *)XtMalloc(
            numTransferTargets * sizeof(XmDropTransferEntryRec));

        numTransfers = transferInfo->numTransferTargets + numTransferTargets;

        transferInfo->transferTargets = (Atom *)XtRealloc(
            (char *)transferInfo->transferTargets, numTransfers * sizeof(Atom));
...
#ifdef DEBUG
void _DtDndPrintTargets(Display *display, Atom *targets, Cardinal numTargets) {
        Cardinal ii;
        char *name;
...
void _DtDndPrintTransfers(Display *display, DtDndTransfer *transfers,
                          Cardinal numTransfers) {
        Cardinal ii;

        for (ii = 0; ii < numTransfers; ii++) {
                printf("%s\tTargets: ", transfers[ii].methods->name);
                _DtDndPrintTargets(display, transfers[ii].targets,
                                   transfers[ii].numTargets);
        }
}
#endif
 ...
 * Copy CallbackList
 */
XtCallbackList _DtDndCopyCallbackList(XtCallbackList callbacks) {
        XtCallbackList cl;
        int ii, count;
 ...
 * Call CallbackList
 */
void _DtDndCallCallbackList(Widget widget, XtCallbackList callbacks,
                            XtPointer calldata) {
        XtCallbackList cl;
...
int _DtDndCountVarArgs(va_list vaList) {
        XtPointer argPtr;
        Cardinal argCount;

        argCount = 0;
        for (argPtr = va_arg(vaList, String); argPtr != NULL;
...
void _DtDndArgListFromVarArgs(va_list vaList, Cardinal maxArgs,
...
String _DtDndGetHostName() {
        ...
        if (nodename == NULL) {
                ...
                if (uname(&un) == -1) {
                        _DtSimpleError(DtProgName, DtError, NULL, "uname",
                                       NULL);
