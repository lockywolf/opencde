...
void _DtFreeRequest(register ActionRequest *request);
...
static void TmpFileOpenError(Widget w, char *actionName, char *fileName);
...
static int _DtAddEntry(register char *string, register char ***arrayPtr,
...
static Boolean ParseFileArgument(Widget w, ActionRequest *request,
                                 ...
                                 Boolean typeFile);
static void AddPrompt(register int argNum, char *prompt,
                      register int *numPrompts, register PromptEntry **prompts);
...
static ActionPtr CloneActionDBEntry(register ActionPtr action);
static void CloneParsedMessage(register parsedMsg *old_pmsg,
                               register parsedMsg *new_pmsg);
static void FreeParsedMessage(register parsedMsg *parsedMessage);
static parsedMsg *CloneParsedMessageArray(register parsedMsg *pmsgArray,
                                          register int count);
static void FreeParsedMessageArray(register parsedMsg *parsedMessageArray,
                                   int count);
static Boolean InsertArgumentString(Widget w, register char **bufPtr,
                                    int *bufSizePtr, ActionRequest *request,
                                    register ObjectData *object,
                                    unsigned long mask, char *relPathHost,
                                    char *relPathDir, Boolean addLeadingSpace,
                                    unsigned long processingMask);
static void InsertUnmappedArgumentString(register char **bufPtr,
                                         int *bufSizePtr,
                                         register ObjectData *object,
...
static void InitiateDtRequest(Widget w, ActionRequest *request);
static Boolean ResolveDtNotifyMessagePieces(Widget w, ActionRequest *request,
                                            char *relPathHost,
                                            char *relPathDir);
static void InitiateDtNotifyMessage(Widget w, ActionRequest *request);
static void PrepareAndExecuteAction(Widget w, register ActionRequest *request);
static void __ExtractCWD(register ActionRequest *request, char **hostPtr,
                         char **dirPtr, Boolean useObjectInfo);
...
static void CreateContinueDialog(Widget w, ActionRequest *request,
                                 int numPrompts, register PromptEntry *prompts);
static void CancelPromptDialog(Widget widget, PromptDialog *dialog,
                               XtPointer call_data);
static void ProcessPromptDialog(Widget widget, register PromptDialog *dialog,
                                XtPointer call_data);
static void ChangePromptTraversal(Widget widget, register PromptDialog *dialog,
                                  XtPointer call_data);
static void CreatePromptDialog(Widget w, ActionRequest *request,
                               register int numPrompts,
                               register PromptEntry *prompts);
static Boolean MoreArgumentsToProcess(register ActionRequest *request);
static Boolean ProcessRequest(Widget w, register ActionRequest *request);
...
DtActionInvocationID DtActionInvoke(Widget w, char *action, DtActionArg *aap,
        ...
        if (!initialized) {
                ...
                if (mkdir(tmpDir, mode)) {
                         ...
                         *
                         * Make one attempt to create the parent directory if
                         * the error was because of a missing path component --
                         * It may be because the "$HOME/.dt" directory hasn't
                         * yet been created.
                         */
                        if (errno == ENOENT) {
        ...
        if (!IS_INV_FINISHED(invp->state) &&
            (request = CreateActionRequest(w, action, aap, aac, termOpts,
                                           execHost, contextHost, cwd, invp)) !=
                NULL) {
                if (ProcessRequest(w, request)) {
                        /* all done invoking ? */
                        RESET_INV_PENDING(invp->state);

                        /* We should only get here if all requests have been
                         * honored */
        ...
        XtAppAddTimeOut(
            XtWidgetToApplicationContext(w), 0 /* call back immediately */,
...
static void _DtActTimerCB(XtPointer clientData, XtIntervalId IntId) {
        _DtActExecutionLeafNodeCleanup((unsigned long)clientData, NULL, 0,
                                       True);
}
...

{
        ...
        title = (char *)XtMalloc(
            (Cardinal)(strlen(PromptDialogTitle) + strlen(actionName) +
...

{
        ...
        /* Construct the error message */
        if ((host == NULL) && (dir == NULL) && (file == NULL) &&
            (type == NULL)) {
                (void)sprintf(msgbuf, NoActionMsg2, actionName);
        } else if ((type != NULL) && (file == NULL) && (dir == NULL)) {
                /*
                 * We are dealing with a buffer object for which an action
                 * couldn't be located.
                 */
                (void)sprintf(msgbuf, NoActionMsg3, actionName, type);
        } else {
                name = (char *)XtMalloc((Cardinal)(
                    (host ? strlen(host) : 0) + (dir ? strlen(dir) : 0) +
...
{
        PromptDialogTitle =
            XtNewString(((char *)Dt11GETMESSAGE(2, 3, "Action:  ")));
        ErrorPostfix =
            XtNewString(((char *)Dt11GETMESSAGE(2, 4, "   [Error]")));
        PromptDialogLabel = XtNewString(((char *)Dt11GETMESSAGE(
            2, 5, "Please enter the following information:")));
        ContinueMessage = XtNewString(((char *)Dt11GETMESSAGE(
            ...
            "'Cancel' to terminate the action.")));
        ...
        HostErrorMsg2 = XtNewString(((char *)Dt11GETMESSAGE(
            ...
            "been made.\n\n(See \"The Common Desktop Environment User's "
        ...
        MapErrorMsg = XtNewString(((char *)Dt11GETMESSAGE(
            ...
            "levels of MAPs, or the mapping is \"circular\".")));
        MultiHostErrorMsg = XtNewString(((char *)Dt11GETMESSAGE(
            ...
            "run out of room to execute new processes?")));
        ...
        ToolTalkErrorMsg2 = XtNewString(((char *)Dt11GETMESSAGE(
            2, 17,
            "The request to service this action has failed for the following "
            "reason:\n\n    %s")));
        TtFileArgMapErr = XtNewString(
            (char *)Dt11GETMESSAGE(2, 18,
                                   "An error occurred while attempting to map "
                                   "one of\nthe file arguments."));
        NoActionMsg3 = XtNewString(((char *)Dt11GETMESSAGE(
            2, 19,
            "Either action \"%s\" was not found\n         or\nthis action does "
            "not apply to buffers of type:\n  \"%s\"\n\n")));
        UnSupportedObjMsg = XtNewString(((char *)Dt11GETMESSAGE(
            2, 21,
            "Unsupported input object class: \"%d\"\nfor action: \"%s\".")));
        TmpFileCreateErrorMsg = XtNewString(((char *)Dt11GETMESSAGE(
            2, 22,
            "Unable to create a temporary file in directory: \"%s\"\nfor the "
            "action named: \"%s\"")));
        ...
        TmpFileWriteErrorMsg = XtNewString(
            ((char *)Dt11GETMESSAGE(2, 24,
                                    "Unable to write a temporary file: "
                                    "\"%s\"\nfor the action named: \"%s\"")));
}
...

{
        ...
        for (i = 0; i < aac; i++) {
                ...
                if ((aap + i)->argClass == DtACTION_FILE) {
                        ...
                        /*
                         * Check if we've already created a tmp file for this
                         * buffer if so fill in the request structure as if this
                         * were a file object.
                         */
                        if (invp->info[i].name) {
                                /*
                                 * Use the tmp file name and type stored in the
                                 * invocation rec. The FILE bit will be set in
                                 * the object mask -- we will also set the
                                 * BUFFER bit to indicate that this is a tmp
                                 * file representing a buffer.
                                 */
                                if (ParseFileArgument(w, request, &objectData,
                                                      NULL, invp->info[i].name,
                                                      invp->info[i].type,
                                                      True)) {
                                        XtFree((char *)objectDataArray);
                                        return (NULL);
                                ...
                                /*
                                 * Set the buffer object bit as well -- and
                                 * check whether this buffer is intended to be
                                 * writable, if not reset the writable bit set
                                 * in ParseFileArgument().
                                 */
                                ...
                                if (aap[i].u.buffer.type)
                                        objectData.type = _DtDtsMMStringToBoson(
                                            aap[i].u.buffer.type);

                                /*
                                 * Save the original buffer pointer and size for
                                 * this pseudo-file.
                                 */
                                if (aap[i].u.buffer.bp) {
                                        objectData.u.file.bp =
                                            aap[i].u.buffer.bp;
                                        objectData.u.file.sizebp =
                                            aap[i].u.buffer.size;
                                }
                                ...
                                /*
                                 * If the buffer type has been passed in to us
                                 * save its quark in the object structure now.
                                 * When/if the type is determined later this
                                 * object record should be filled in with the
                                 * necessary quark.
                                 */
                                if (aap[i].u.buffer.type)
                                        objectData.type = _DtDtsMMStringToBoson(
                                        ...
                                        /*
                                         * We have already determined the buffer
                                         * type when creating the invocation
                                         * record. So get the type string from
                                         * the invocation record.
                                         */
                                        myassert(invp->info[i].type);
                                        if (invp->info[i].type) {
                                                objectData.type =
                                                    _DtDtsMMStringToBoson(
                                                        invp->info[i].type);
                                        }
                                }
                                 ...
                                 */
                                if (aap[i].u.buffer.bp) {
                                        objectData.u.buffer.size =
                                            aap[i].u.buffer.size;
                                        objectData.u.buffer.bp =
                                            aap[i].u.buffer.bp;
                                } else {
...
static _DtActInvRecT *CreateInvocationRecord(char *actionName, Widget w,
        ...
        for (i = 0; i < aac; i++) {
                if (aap[i].argClass == DtACTION_BUFFER) {
                        ...
                        if (aap[i].u.buffer.name ||
                            (aap[i].u.buffer.type == NULL)) {
                                tmp = DtDtsBufferToDataType(
                                    ...
                                    aap[i].u.buffer.name);
                                /*
                                 * Malloc our own copy of the type string so we
                                 * won't have to worry about when to call
                                 * DtDtsFreeDataType() later.
                                 */
                                invp->info[i].type = XtNewString(tmp);
                                DtDtsFreeDataType(tmp);
                        } else {
                                invp->info[i].type =
                                    XtNewString(aap[i].u.buffer.type);
                        }

                        /*
                         * Simply create tmp files for ALL buffers.
                         *
                         * This allows us to work around problems related to
                         * client programs making subsequent changes to or
                         * freeing the memory associated with the buffer before
                         ...
                         */
                        is_executable = DtDtsDataTypeToAttributeValue(
                            invp->info[i].type, _DtActIS_EXECUTABLE, NULL);
                        /*
                        ...
                        if (aap[i].u.buffer.name) {
                                /*
                                 * Attempt to use the name supplied for the
                                 * buffer.
                                 */
                                invp->info[i].name = _DtActGenerateTmpFile(
                                    NULL, aap[i].u.buffer.name, mode, &fd);
                        }
                        if (!invp->info[i].name) {
                                /*
                                 * Generate tmp file based on format supplied
                                 * for the file type.
                                 */
                                format = DtDtsDataTypeToAttributeValue(
                                    invp->info[i].type, _DtActNAME_TEMPLATE,
                                    NULL);
                        ...
                        if (!invp->info[i].name) {
                                 ...
                                 */
                                /*
                                 * Error message makes assertion message
                                 * redundant. myassert(invp->info[i].name);
                                 */
                                TmpFileCreateError(w, actionName,
                                                   _DtGetDtTmpDir());
                         ...
                         */
                        myassert(fd >= 0);
                        for (bytesToWrite = aap[i].u.buffer.size,
                            bytesWritten = 0;
                             bytesToWrite > 0; bytesToWrite -= bytesWritten) {
                                bytesWritten =
                                    write(fd, aap[i].u.buffer.bp, bytesToWrite);
                                if (bytesWritten < 0) {
                                        if (errno == EINTR) {
                                                bytesWritten = 0;
                                                continue;
                        ...
                        while (close(fd)) {
                                ...
                                case EINTR: /* interrupted sys call */
                                        closeAttempts++;
                                        continue; /* try again */
                                case ENOSPC: /* Not enough space on NFS-mounted
                                                dev */
                                ...
                                break; /* only try again for conditions with
                                          continue */
                        }

                        /*
                         * Now that we have created a tmp file for this buffer
                         * object set the FILE_OBJ flag as well as the buffer
                         * flag. Objects with both the BUFFER and FILE flags set
                         * will be recognized as buffers which have been written
                         * to tmp files.
                         */
...

{
        ...
        &request->numHostNames); XtFree(host);
           }
           else
        ********************************************************************/
        if (hostname) {
                ...
                if (request->cwdHost != NULL) {
                        hostId =
                            _DtAddEntry(request->cwdHost, &request->hostNames,
                                        &request->numHostNames);
                } else {
        ...
        if ((dirName = _DtDirname(filename)) == NULL) {
                ...
                tmpName =
                    XtRealloc(tmpName, strlen(tmpName) + strlen(dirName) + 2);
                (void)strcat(tmpName, "/");
...

{
        ...
        if ((tmp = getcwd(buf, MAXPATHLEN)) == NULL) {
                _DtSimpleError(DtProgName, DtError, NULL,
                               "getcwd(): unable to get current directory",
                               NULL);
                tmp = "/";
        }
...
static int _DtAddEntry(register String string, register String **arrayPtr,
                       register int *sizePtr)

{
        ...
        i = *sizePtr;
        (*sizePtr)++;
        (*arrayPtr) = (String *)XtRealloc(
            (String)*arrayPtr, (Cardinal)(sizeof(String) * (*sizePtr)));
        ...
        while ((link_len = readlink(file_name, link_path, MAXPATHLEN)) > 0) {
                ...
                for (i = 0; i < history_count; i++) {
                        if (strcmp(file_name, history[i]) == 0) {
                                /* Drop back to last non-recursive portion of
                                 * the path */
                ...
                if (history_count >= history_size) {
                        history_size += 100;
                        history = (char **)XtRealloc(
                            (char *)history, sizeof(char *) * history_size);
                }
...

{
        ...
        if (request->clonedAction == NULL) {
                ...
                if (actionQuark == -1 || (action = _DtActionFindDBEntry(
                                              request, actionQuark)) == NULL) {
                        /*
                        ...
                        if (IS_TOO_MANY_MAPS(request->mask)) {
                                ...
                                if (IS_FILE_OBJ(request->objects[0].mask)) {
                                        NoActionError(
                                            ...
                                            (char *)_DtDtsMMBosonToString(
                                                request->objects[0].type),
                                            request->hostNames
                                                [request->objects[0]
                                                     .u.file.hostIndex],
                                            request->dirNames
                                            ...
                                            request->objects[0]
                                                .u.file.baseFilename);
                                } else if (IS_BUFFER_OBJ(
                                               request->objects[0].mask)) {
                                        /*
                                         * RWV -- may have to modify this call
                                         * to generate a message more suitable
                                         * for buffer objects.
                                         */
                                        NoActionError(
                                            w, actionQuark, request->actionName,
                                 ...
                                 * else if
                                 * (IS_STRING_OBJ(request->objects[0].mask))
                                 */
                        } else
                                NoActionError(w, actionQuark,
                                              request->actionName, NULL, NULL,
                                              NULL, NULL);

                        /*
                         * If we are in the middle of reprocessing a single
                         * argument action, then continue with the next
                         * parameter.  Otherwise, this error terminates the
                         * request, so return.
                         */
                        if (IS_REPROCESSING(request->mask) &&
                         ...
                         */
                        {
                                _DtActInvRecT *invRecP =
                                    _DtActFindInvRec(request->invocId);
                                if (invRecP)
                ...
                /*
                 * If this is a ToolTalk message, then before proceeding any
                 * further, make sure we can get connected to a ToolTalk
                 * session.  If we can't, then we need to bail out.
                 */
                if (IS_TT_MSG(action->mask) &&
                    (status = _DtInitializeToolTalk(NULL)) != TT_OK) {
                        NoToolTalkConnectionError(
                            w, request->clonedAction->label, status);
                        {
                                _DtActInvRecT *invRecP =
                                    _DtActFindInvRec(request->invocId);
                                if (invRecP)
                 ...
                 *       to abort or continue, do the abort/continue dialog
                 * BEFORE the prompt dialog; there's little sense in collecting
                 *       additional input if the user is going to abort the
                 * action!
                 */
                if ((prompts != NULL) &&
                ...
                /* Were too many parameters supplied? */
                        ...
                        /*
                         * If the action only needs a single parameter, then we
                         * need to fire off multiple instances of the action;
                         * otherwise, prompt the user to continue or abort.  An
                         * action requiring no parameters is also treated like a
                         * single parameter action.
                         */
                        if (IS_ARG_SINGLE_ARG(action->mask) ||
                            IS_ARG_NONE_FOUND(action->mask)) {

                                PrepareAndExecuteAction(w, request);

                                /* See if there are still more parameters to be
                                 * processed */
                                if (MoreArgumentsToProcess(request))
                                        return (ProcessRequest(w, request));
                        } else {
                                /*
                                 * Postpone any further processing until the
                                 * user either tells us to continue, or abort.
                                 */
                                CreateContinueDialog(w, request, numPrompts,
                                                     prompts);
                                XtFree((char *)prompts);
                                return (False);
                        }
                ...
                /*
                 * If this is a single argument action, and we have more
                 * parameters waiting to be processed, then continue processing
                 * them.
                 */
                if ((IS_ARG_SINGLE_ARG(action->mask) ||
                     IS_ARG_NONE_FOUND(action->mask)) &&
        ...
        if (IS_CMD(action->mask)) {
                ...
                if (childrecp && childrecp->numObjects > 0) {
                        childrecp->argMap = (_DtActArgMap *)XtCalloc(
                            childrecp->numObjects, sizeof(_DtActArgMap));

                        for (i = 0; i < childrecp->numObjects && i < invp->ac;
                             i++) {
                                childrecp->argMap[i].argN =
                                    i + 1; /*  ignored for CMD actions */
                                childrecp->argMap[i].argIdx =
                                    i +
                                    request
                                        ->objOffset; /* idx into invp->info[] */
                        }
                }
                ProcessCommandInvokerRequest(w, request, relPathHost,
                                             relPathDir);
                ...
                if (childrecp) {
                         ...
                         *
                         * The requirement is that one and only one action
                         * argument may appear in a TT_ARGn_VALUE field.
                         *
                         * argMap is a sparse array which maps TT_ARGn_VALUEs to
                         * input parameters.  If a TT_ARGn_VALUE does not have
                         * an input parameter as a value then the sentinel value
                         * "-1" is provided as the index. Allocate enough space
                         * for all the TT_ARGn_VALUEs plus one for TT_FILE.
                         *
                        ...
                        childrecp->argMap = (_DtActArgMap *)XtCalloc(
                            action->u.tt_msg.value_count + 1,
                            sizeof(_DtActArgMap));

                        for (i = 0; i < action->u.tt_msg.value_count; i++) {
                                /*
                                 * Set index value to "-1".  This value will
                                 * indicate TT_ARGn_VALUES which are NOT
                                 * associated with input parameters (action
                                 * arguments).  If there is an action argument
                                 * associated with this TT_ARGn_VALUE we will
                                 * set it below.
                                 */
                                childrecp->argMap[i].argIdx = -1;
                                childrecp->argMap[i].argN = i;

                                /* null argn value is valid -- so check
                                 * MsgParts*/
                                if (!action->u.tt_msg.tt_argn_value[i]
                                         .numMsgParts)
                                        continue;
                                ...
                                /*
                                 * TT_ARGn_VALUE fields should have only one arg
                                 * keyword.
                                 */
                                myassert(action->u.tt_msg.tt_argn_value[i]
                                             .numMsgParts == 1);
                                argNum = action->u.tt_msg.tt_argn_value[i]
                                             .parsedMessage[0]
                                             .argNum;
                                ...
                                if ((argNum > 0) && (argNum <= invp->ac)) {
                                        ...
                                        myassert(childrecp->argMap[i].argIdx >=
                                                 0);
                                }
                        }
                        /*
                         * Add an argMap entry for the value of the TT_FILE
                         * field. Tooltalk (e.g. media messages) sometimes uses
                         * this field to pass values such as file names to the
                         * message receipient. If the TT_FILE field has a single
                         * ARG keyword then record that parameter number
                         * otherwise record "-1" as was done for the value
                         * arguments above.
                         */
                        childrecp->argMap[i].argIdx = -1;
                        childrecp->argMap[i].argN =
                            -1; /* Use "-1" as TT_FILE entry idx */
                        if (action->u.tt_msg.tt_file.numMsgParts &&
                            action->u.tt_msg.tt_file.parsedMessage[0].keyword ==
                                ARG) {
                                argNum =
                                    action->u.tt_msg.tt_file.parsedMessage[0]
                                        .argNum;
                                if ((argNum > 0) && (argNum <= invp->ac)) {
                                        /* The ith message part should be the
                                         * last argMap entry */
                                        childrecp->argMap[i].argIdx =
                                            argNum + request->objOffset - 1;
                                        myassert(childrecp->argMap[i].argIdx >=
...

{
        ...
        if (useObjectInfo && (IS_CMD(action->mask)) &&
            (request->numObjects > 0) && (request->objects[0].type >= 0) &&
            IS_FILE_OBJ(request->objects[0].mask)) {
                if (action->u.cmd.contextHost != NULL)
                        *hostPtr = XtNewString(action->u.cmd.contextHost);
                else {
                        *hostPtr = XtNewString(
                            request->hostNames[request->objects[0]
                                                   .u.file.hostIndex]);
                }
                ...
                if (IS_UNKNOWN_IF_DIR(request->objects[0].mask)) {
                        ...
                        /*
                         * The file may not have been checked yet, if it was
                         * never referenced in the execution string; so .. we'll
                         * check here.
                         */
                        theHost = request->hostNames[request->objects[0]
                                                         .u.file.hostIndex];
                        theDir =
                            request
                ...
                if (IS_DIR_OBJ(request->objects[0].mask)) {
                        if (action->u.cmd.contextDir != NULL)
                                ...
                                *dirPtr = XtMalloc((Cardinal)(
                                    strlen(
                                        request
                                            ->dirNames[request->objects[0]
                                                           .u.file.dirIndex]) +
                                    strlen(request->objects[0]
                                               .u.file.baseFilename) +
                                    2));
                                strcpy(*dirPtr,
                                       request->dirNames[request->objects[0]
                                                             .u.file.dirIndex]);
...

{
        ...
        if (request->numObjects > 0) {
                newRequest->objects = (ObjectData *)XtMalloc(
                    sizeof(ObjectData) * request->numObjects);
                ...
                for (i = 0; i < request->numObjects; i++) {
                        ...
                        if (IS_FILE_OBJ(request->objects[i].mask)) {
                                ...
                                /*
                                 * RWV:
                                 * Since we are creating tmp files for all
                                 * buffers we should never have to copy a
                                 * buffer's contents.
                                 *
                                 * We should never reach this code because the
                                 * FILE_OBJ bit is set when we create tmp files
                                 * for buffers.
                                 */
                                myassert(0);
                                /*
                                 * RWV:
                                 * Can we get by without copying buffer object
                                 * contents? if so  -- how do we avoid freeing
                                 * it twice OR not freeing it at all?
                                 */
                                /* make a copy of the buffer */
                                if (request->objects[i].u.buffer.bp) {
                                        myassert(
                                            newRequest->objects[i]
                                                .u.buffer.size ==
                                            request->objects[i].u.buffer.size);
                                        newRequest->objects[i].u.buffer.bp =
                                            XtMalloc(request->objects[i]
                                                         .u.buffer.size);
                                        memcpy(
                                            newRequest->objects[i].u.buffer.bp,
                                ...
                                myassert(
                                    0 /* no other object types supported */);
                }
        }

        newRequest->numPromptInputs = request->numPromptInputs;
        if (request->numPromptInputs > 0) {
                newRequest->promptInputs = (char **)XtMalloc(
                    sizeof(char *) * request->numPromptInputs);
                for (i = 0; i < request->numPromptInputs; i++)
                        newRequest->promptInputs[i] =
                            XtNewString(request->promptInputs[i]);
        }
        ...
        if (request->numHostNames > 0) {
                ...
                for (i = 0; i < request->numHostNames; i++)
                        newRequest->hostNames[i] =
                            XtNewString(request->hostNames[i]);
        }
        ...
        if (request->numDirNames > 0) {
                ...
                for (i = 0; i < request->numDirNames; i++)
                        newRequest->dirNames[i] =
                            XtNewString(request->dirNames[i]);
        }
        ...
        if (request->clonedAction)
                newRequest->clonedAction =
                    CloneActionDBEntry(request->clonedAction);
        else
...
{
        register ActionPtr newAction =
            (ActionPtr)XtMalloc((Cardinal)sizeof(Action));
        register int i;
        ...
        if (action->type_count > 0) {
                newAction->arg_types = (DtShmBoson *)XtMalloc(
                    sizeof(DtShmBoson) * newAction->type_count);
        ...
        if (IS_CMD(action->mask)) {
                ...
                CloneParsedMessage(&(oldCmd->execString),
                                   &(newCmd->execString));
                ...
                if (oldCmd->execHostCount > 0) {
                        newCmd->execHostArray = (char **)XtMalloc(
                            sizeof(char *) * newCmd->execHostCount);
                        for (i = 0; i < newCmd->execHostCount; i++)
                                newCmd->execHostArray[i] =
                                    XtNewString(oldCmd->execHostArray[i]);
                } else {
                ...
                if (oldMsg->mode_count > 0) {
                        ...
                        for (i = 0; i < newMsg->mode_count; i++)
                                newMsg->tt_argn_mode[i] =
                                    oldMsg->tt_argn_mode[i];
                } else {
                        newMsg->tt_argn_mode = NULL;
                }

                newMsg->vtype_count = oldMsg->vtype_count;
                newMsg->tt_argn_vtype = CloneParsedMessageArray(
                    oldMsg->tt_argn_vtype, oldMsg->vtype_count);

                newMsg->value_count = oldMsg->value_count;
                newMsg->tt_argn_value = CloneParsedMessageArray(
                    oldMsg->tt_argn_value, oldMsg->value_count);

                newMsg->rep_type_count = oldMsg->rep_type_count;
                if (oldMsg->rep_type_count > 0) {
                        newMsg->tt_argn_rep_type = (int *)XtMalloc(
                            sizeof(int) * newMsg->rep_type_count);
                        for (i = 0; i < newMsg->rep_type_count; i++)
                                newMsg->tt_argn_rep_type[i] =
                                    oldMsg->tt_argn_rep_type[i];
                } else {
...
void _DtFreeActionStruct(register ActionPtr action)
        ...
        if (old_pmsg->numMsgParts > 0) {
                ...
                for (i = 0; i < old_pmsg->numMsgParts; i++) {
                        ...
                        if (piece->precedingText)
                                newPiece->precedingText =
                                    XtNewString(piece->precedingText);
                        else
...

{
        ...
        for (i = 0; i < piece->numMsgParts; i++) {
                ...
                if (segment->precedingText) {
                        compiledMsg =
                            GrowMsgBuffer(compiledMsg, &compiledMsgSize,
                ...
                case DATABASE_HOST: {
                        /*
                         * Add in the host associated with the DB file from
                         * which this action was loaded.
                         */
                        char *fullPath;
                        char *host;

                        fullPath = _DtDbPathIdToString(
                            request->clonedAction->file_name_id);
                ...
                case DISPLAY_HOST: {
                        /*
                         * Use the displayHostName determined the first time
                         * thru
                         */
                        compiledMsg =
                            GrowMsgBuffer(compiledMsg, &compiledMsgSize,
                ...
                case SESSION_HOST: {
                         ...
                         * display management.  (i.e. the host where the login
                         * client is running.)
                         */
                        compiledMsg =
                            GrowMsgBuffer(compiledMsg, &compiledMsgSize,
                ...
                case NO_KEYWORD: {
                        /*
                         * If this is an entry which simply collected some user
                         * input, then add the user's input to the message
                         * buffer. This corresponds to the keywords:
                         *
                         *         %"prompt"%
                         *         %(String)"prompt"%
                         */
                        if (segment->prompt) {
                                 ...
                                 */
                                if (ParseFileArgument(
                                        w, request, &tmpObjData, NULL,
                                        request->promptInputs[*promptDataIndex],
                                        NULL, False)) {
                                        XtFree(compiledMsg);
                                        return (False);
                                }

                                if (!InsertArgumentString(
                                        w, &compiledMsg, &compiledMsgSize,
                                        request, &tmpObjData, segment->mask,
                                        relPathHost, relPathDir, False, 0)) {
                                        XtFree(compiledMsg);
                                        return (False);
                                }
                ...
                case ARG: {
                        if (segment->argNum == ALL_ARGS) {
                                ...
                                for (j = 0, firstParmUsed = False;
                                     j < request->numObjects; j++) {
                                        ...
                                        if ((*paramUsed)[j] == False) {
                                                ...
                                                /*
                                                 * RWV:
                                                 * Since we use tmp files for
                                                 * buffers and do not support
                                                 * strings; we need not add
                                                 * special code for buffer and
                                                 * string support.
                                                 */
                                                /* fdt: add support for buffers
                                                 * and strings else if
                                                 * (IS_BUFFER_OBJ(request->objects[i].mask)
                                                 * &&
                                                 *          request->objects[i].u.buffer.buffer)
                                                 * else if
                                                 * (IS_STRING_OBJ(request->objects[i].mask)
                                                 * &&
                                ...
                                if (IS_FILE_OBJ(
                                    ...
                                    request->objects[segment->argNum - 1]
                                         ...
                                         * argument */
                                        (*paramUsed)[segment->argNum - 1] =
                                            True;
                                        /*
                                         * All buffer objects have been written
                                         * to tmp files. This code replaces a
                                         * reference to an object with its (tmp)
                                         * file name. Tooltalk processing code
                                         * elsewhere (ActionTt.c) detects
                                         * the conditions under which a buffer
                                         * object reference should be replaced
                                         * by the buffer contents instead of the
                                         * tmp file name.  (i.e. a value field
                                         * with a single argument reference with
                                         * no additional text).  In such cases
                                         * the compiled message string will be
                                         * ignored.
                                         */
                                        if (!InsertArgumentString(
                                                w, &compiledMsg,
                                                &compiledMsgSize, request,
                                                request->objects +
                                                    segment->argNum - 1,
                                                segment->mask, relPathHost,
                                                relPathDir, False,
                                                processingMask)) {
                                                XtFree(compiledMsg);
                                                return (False);
                                ...
                                /*
                                 * RWV:
                                 * Since we use tmp files for buffers and do not
                                 * support strings; we need not add special code
                                 * for buffer and string support.
                                 */
...
static Boolean
InsertArgumentString(Widget w, register char **bufPtr, int *bufSizePtr,
                     ActionRequest *request, register ObjectData *object,
                     unsigned long mask, char *relPathHost, char *relPathDir,
                     Boolean addLeadingSpace, unsigned long processingMask)
        ...
        if (IS_TREAT_AS_FILE(mask)) {
                if (object->type == -1) {
                        ...
                        if (IS_FILE_OBJ(object->mask)) {
                                ...
                                ParseFileArgument(w, request, object, NULL,
                                                  origInfo, NULL, True);
                                XtFree(origInfo);
                        }
                        /*
                ...
                if (IS_FILE_OBJ(object->mask)) {
                        if (processingMask & _DTAct_TT_VTYPE) {
                                /*
                                 * Instead of inserting the object referred to
                                 * by "Arg_n", we need to instead insert the
                                 * MEDIA attribute for the object.  If the MEDIA
                                 * attribute is not defined for the datatype
                                 * associated with this object, then use the
                                 * datatype name itself.  If the thing can't be
                                ...
                                if (object->type != (-1)) {
                                        ...
                                        if ((path = _DtActMapFileName(
                                                 request->hostNames
                                                     [object->u.file.hostIndex],
                                                 request->dirNames
                                                     [object->u.file.dirIndex],
                                        ...
                                        *bufPtr =
                                            GrowMsgBuffer(*bufPtr, bufSizePtr,
                                                          strlen(value) + 1);
                                        if (addLeadingSpace)
                                                strcat(*bufPtr, " ");
                                        strcat(*bufPtr, value);
                        ...
                        if (IS_CMD(request->clonedAction->mask)) {
                                /* Map into a real path, relative to the
                                 * execution host */
                                if ((path = _DtActMapFileName(
                                         request->hostNames[object->u.file
                                                                .hostIndex],
                                         request->dirNames[object->u.file
                                                               .dirIndex],
                                         object->u.file.baseFilename,
                                         request->currentHost)) == NULL) {
                                        AddFailedHostToList(
                                            request, request->currentHost);
                                        return (False);
                                }

                                *bufPtr = GrowMsgBuffer(*bufPtr, bufSizePtr,
                                                        strlen(path) + 1);
                                if (addLeadingSpace)
                                        (void)strcat(*bufPtr, " ");
                                strcat(*bufPtr, path);
                                ...
                                /*
                                 * ToolTalk automatically translates the
                                 * 'filename' field within a message, and
                                 * expects the incoming name to be relative to
                                 * the local host.  So ... we simply need to map
                                 * the name to be relative to the local host.
                                 * However, if this is not the filename, but is
                                 * instead one of the 'args', then we must
                                 * insert it in a 'neutral' form.
                                 */
                                if (processingMask & _DTAct_TT_ARG) {
                                        /* Map into "host:/path" */
                                        /* fdt: May need to instead map into
                                         * 'network indep' form */
                                        InsertUnmappedArgumentString(
                                            bufPtr, bufSizePtr, object,
                                            addLeadingSpace);
                                } else {
                                        if ((path = _DtActMapFileName(
                                                 request->hostNames
...

{
        ...
        if ((currentBufUsed + count + 1) >= *size) {
                ...
                /* If this is the first alloc for the buffer, then terminate the
                 * buffer */
                if (currentBufUsed == 0)
                        buffer[0] = '\0';
        }
...
static void AddPrompt(register int argNum, String prompt,
                      register int *numPrompts, register PromptEntry **prompts)
...
static int MatchParamsToAction(ActionRequest *request, int *numPrompts,
        ...
        *prompts = NULL;
        ...
        if (unused > 0) {
                paramUsed =
                    (Boolean *)XtMalloc((Cardinal)(sizeof(Boolean) * unused));
                for (i = 0; i < unused; i++)
                        paramUsed[i] = False;
        }
        ...
        if (IS_CMD(action->mask)) {
                /*
                 * NOTE: The current implementation of prompt strings requires
                 *that the segments be evaluated in the same order in which the
                ...
                for (i = 0; i < action->u.tt_msg.vtype_count; i++) {
                        ProcessOneSegment(
                            request, &(action->u.tt_msg.tt_argn_vtype[i]),
                ...
                for (i = 0; i < action->u.tt_msg.value_count; i++) {
                         ...
                         */
                        ProcessOneSegment(
                            request, &(action->u.tt_msg.tt_argn_value[i]),
                            prompts, numPrompts, &argsOptionFound,
                            &lastArgReferenced, &unused, paramUsed);
                }
        }
        ...
        if (argsOptionFound)
                 ...
                 * is referenced, but arg1 and arg3 are not, then only count
                 * arg3 as an unused (and thus extra) parameter.
                 */
                for (i = 0;
                     ((i < lastArgReferenced - 1) && (i < request->numObjects));
                     i++) {
                        if (!paramUsed[i])
                                unused--;
                }
        ...
        for (i = 0; i < msg->numMsgParts; i++) {
                ...
                if (piece->keyword == ARG) {
                        if (piece->argNum == ALL_ARGS) {
                                /*
                                 * When a %Args% keyword is found, this implies
                                 * that there will ultimately be no unused
                                 * parameters, because this keyword is replaced
                                 * by all unused parameters.
                                 */
                                *argsOptionFound = True;
                        } else if (piece->argNum > 0) {
                                /* Keep track of the largest arg index
                                 * referenced */
                                if (piece->argNum > *lastArgReferenced)
                                        *lastArgReferenced = piece->argNum;

                                /* See if a parameter was supplied for this
                                 * argNum */
                                if (piece->argNum > request->numObjects) {
                                        /* Parameter is missing; see if a prompt
                                         * was given */
                                        if (piece->prompt)
                                                AddPrompt(piece->argNum,
                                                          piece->prompt,
                                                          numPrompts, prompts);
                                } else {
                                        /* Mark this parameter as having been
                                         * used */
                                        if (!paramUsed[piece->argNum - 1]) {
                                                paramUsed[piece->argNum - 1] =
                                                    True;
                                                (*unused)--;
                                        }
                                }
...

{
         ...
         * to worry about since this cancel effectively aborts further
         * processing.
         *
         * We may have to return  values  to the caller.
         */
...

{
        ...
        for (i = 0; i < dialog->numPrompts; i++) {
                ...
                if (dialog->prompts[i].argIndex > 0) {
                        ...
                        if (dialog->prompts[i].argIndex >
                            dialog->request->numObjects) {
                                dialog->request->objects =
                                    (ObjectData *)XtRealloc(
                                        (char *)dialog->request->objects,
                                        (Cardinal)(
                                            sizeof(ObjectData) *
                                            (dialog->prompts[i].argIndex)));

                                /* Initialize the new array entries */
                                for (j = dialog->request->numObjects;
                                     j < dialog->prompts[i].argIndex; j++) {
                                        dialog->request->objects[j].mask = 0;
                                        SET_FILE_OBJ(
                                            dialog->request->objects[j].mask);
                                        dialog->request->objects[j].type = -1;
                                        dialog->request->objects[j]
                                            .u.file.hostIndex = -1;
                                        dialog->request->objects[j]
                                            .u.file.dirIndex = -1;
                                        dialog->request->objects[j]
                                            .u.file.origFilename = NULL;
                                        dialog->request->objects[j]
                                            .u.file.origHostname = NULL;
                                        dialog->request->objects[j]
                                            .u.file.baseFilename = NULL;
                                        dialog->request->objects[j].u.file.bp =
                                            0;
                                        dialog->request->objects[j]
                                            .u.file.sizebp = 0;
                                        dialog->request->objects[j]
                        ...
                        /*
                         * These values cannot be broken up into host/dir/file
                         * components, nor can they be typed, until we know it
                         * they refer to a file. This can't be determined until
                         * we construct the action message.
                         */
                        dialog->request
                            ->objects[dialog->prompts[i].argIndex - 1]
                            .u.file.origFilename = value;
                } else /* Prompt-only input */
                {
                        /*
                         * Prompt-only input can't fit in our ordered object
                         * array, since they don't have a unique argIndex which
                         * can be used as the index into the object array.
                         */
                        dialog->request->numPromptInputs++;
                        dialog->request->promptInputs = (String *)XtRealloc(
                            (char *)dialog->request->promptInputs,
                            (Cardinal)(dialog->request->numPromptInputs *
                                       sizeof(String)));

                        dialog->request
                            ->promptInputs[dialog->request->numPromptInputs -
                                           1] = value;
                }
        }
        ...
        if (ProcessRequest(dialog->associatedWidget, dialog->request)) {
                ...
                if ((invp = _DtActFindInvRec(dialog->request->invocId))) {
                        ...
                        /*
                         * evaluate whether all child actions have been
                         * completed and if its time to call the user callback.
                         */
                        _DtActExecutionLeafNodeCleanup(invp->id, NULL, 0, True);
                }
...

{
        ...
        _DtSvcProcessLock();
        if (first) {
                XtAppAddActions(XtWidgetToApplicationContext(w), actionTable,
                                1);
        ...
        title = (char *)XtMalloc(
            (Cardinal)(strlen(PromptDialogTitle) +
                       strlen(request->clonedAction->label) + 1));
        (void)sprintf(title, "%1$s%2$s", PromptDialogTitle,
        ...
        for (count = 0; count < numPrompts; count++) {
                ...
                n++;
                promptDes[count].promptWidget =
                    XmCreateTextField(form, "text", args, n);
        ...
        if (numPrompts >= 0)
                XmProcessTraversal(promptDes[0].promptWidget,
                                   XmTRAVERSE_CURRENT);
}
...

{
        ...
        if (dialog->numPrompts == 0) {
                if (ProcessRequest(dialog->associatedWidget, dialog->request)) {
                        ...
                        if ((invp = _DtActFindInvRec(
                                 dialog->request->invocId)) != NULL) {
                                /* all done invoking ? */
                                RESET_INV_PENDING(invp->state);

                                /* We should only get here if all requests have
                                 * been honored */
                                SET_INV_COMPLETE(invp->state);

                                /*
                                 * evaluate whether all child actions have been
                                 * completed and if its time to call the user
                                 * callback.
                                 */
                                _DtActExecutionLeafNodeCleanup(invp->id, NULL,
                                                               0, True);
                        }
...

{
         ...
         */
        if (prompts) {
                dialog->prompts = (PromptEntry *)XtMalloc(
                    (Cardinal)(sizeof(PromptEntry) * numPrompts));
                for (i = 0; i < numPrompts; i++) {
                        dialog->prompts[i].argIndex = prompts[i].argIndex;
                        dialog->prompts[i].prompt =
                            XtNewString(prompts[i].prompt);
                }
        } else
                dialog->prompts = NULL;
        ...
        (void)sprintf(title, fmt, PromptDialogTitle,
                      request->clonedAction->label);
        ...
        XtUnmanageChild(
            XmMessageBoxGetChild(dialog->topLevel, XmDIALOG_HELP_BUTTON));
...

{
        ...
        if (ResolveCommandInvokerMessagePieces(w, request, relPathHost,
                                               relPathDir)) {
                /*
                ...
                if (!(childp = _DtActFindChildRec(request->invocId,
                                                  request->childId)))
                        myassert(0 /* could not find child record */);

                /*
                 * The only way we could have reached here is if the execution
                 * host was not accessible, and we tried to map one of the data
                 * files to be relative to this host.  If there are other hosts
                 * to be tried, then we will retry the request on the next host;
                 * otherwise, we will post an error dialog, and bail out.
                 */
                ...
                if (request->hostIndex >= action->u.cmd.execHostCount) {
                        ...
                        /*
                         * Cleanup should happen later when we return up the
                         * stack.
                         */

                        if (action->u.cmd.execHostCount <= 1) {
                                /* Display error dialog listing just the one
                                 * failed exec host */
                                HostAccessError(w, request->clonedAction->label,
                                                request->badHostList);
                        } else {
                                /* Display error dialog listing all failed exec
                                 * hosts */
                        ...
                        if (invp && childp) {
                                /*
                                 * Delete child record for failed exec on this
                                 * host
                                 */
...

{
        ...
        CallbackData *data =
            (CallbackData *)XtMalloc((Cardinal)sizeof(CallbackData));
        ActionPtr action = request->clonedAction;
        ...
        /*
         * The string generated for procId should never exceed the procId buf
         * size.
         */
        sprintf(procIdBuf, "%s_%d_%lu", _DtActNULL_GUARD(procId),
        ...
        if ((invp = _DtActFindInvRec(request->invocId)) != NULL) {
                if ((childp = _DtActFindChildRec(request->invocId,
                        ...
                        for (i = 0; i < childp->numObjects; i++) {
                                ...
                                if (!(IS_BUFFER_OBJ(
                                        invp->info[childp->argMap[i].argIdx]
                                            .mask)))
                                        continue; /* not a buffer object */

                                if (!(p = invp->info[childp->argMap[i].argIdx]
                                              .name))
                                        continue; /* no tmp file name */

                                /* Add up the string length of the file name */
                                if ((len += strlen(" -tmp ") + strlen(p)) <
                                    sizeof(tmpFileBuf)) {
                                        /*
                                         * Use the automatic tmpFileBuf if
                                         * possible
                                         */
        ...
        if (_DtActionCommandInvoke(
                action->mask & _DtAct_WINTYPE_BITS, host, dir,
                action->u.cmd.execString.compiledMessage,
                action->u.cmd.termOpts.compiledMessage, request->currentHost,
                procId, tmpFiles, CmdInvSuccessfulRequest, (XtPointer)data,
...

{
        ...
        if (action->u.cmd.execHostArray == NULL) {
                /* Explicitly specified execHost overrides action definition
                 * execHost */
                if (request->execHost)
                        ParseHostList(request->execHost, &hostList,
                                      &hostListSize, &hostCount);
                else if (action->u.cmd.execHosts.compiledMessage) {
                        ParseHostList(action->u.cmd.execHosts.compiledMessage,
                                      &hostList, &hostListSize, &hostCount);
                }
        ...
        if (action->u.cmd.execHostCount == 0) {
                /*
                 * Oh boy ... someone is trying to be nasty!  The only way we
                 * could have gotten here was to have the action's 'EXEC_HOST'
                 * field set to nothing but a prompt string, and then the user
                 * left the string empty!  We'll default the local host, and
                 * hope it works.
                 */
                /*
                 * fdt: we really should default to whatever has been configured
                 *      as the default execution host, followed by the
                 * LocalHost, if they are not the same.
                 */
                request->currentHost = _DtGetLocalHostName();
        } else {
                request->currentHost = XtNewString(
                    action->u.cmd.execHostArray[request->hostIndex]);
        }
}
...

{
        ...
        if ((invp = _DtActFindInvRec(data->requestPtr->invocId)) != NULL) {
                 ...
                 * command actions.  The child process may already have
                 * communicated its status via TT messaging OR it may already
                 * have exited. For now we set the state here -- till we find a
                 * better place.
                 */
                if ((childrecp = _DtActFindChildRec(invp->id,
                                                    data->requestPtr->childId)))
        ...
        if (request->hostIndex < action->u.cmd.execHostCount) {
                ...
                if (action->u.cmd.execHostCount <= 1) {
                        ...
                        CommandInvokerError(data->associatedWidget,
                                            action->label, msg + data->offset);
                } else {
                        MultiHostAccessError(data->associatedWidget,
                                             request->clonedAction->label,
                                             request->badHostList);
                }
...
char *_DtActMapFileName(const char *curHost, const char *dir, const char *file,
        ...
        if (newHost) {
                 ...
                 */
                switch (tt_ptr_error(netpath =
                                         tt_host_file_netfile(curHost, buf))) {
                case TT_OK:
                        break;
                case TT_ERR_PATH:
                        netpath = NULL;
                ...
                if (netpath) {
                        switch (tt_ptr_error(
                            path = tt_host_netfile_file(newHost, netpath))) {
                        case TT_OK:
                                break;
                        case TT_ERR_PATH:
                                path = NULL;
                ...
                /*
                 * The current host is not the same as the local host -- find
                 * the cannonical netfile name then reinterpret it on the local
                 * host.
                 */
                switch (tt_ptr_error(netpath =
                                         tt_host_file_netfile(curHost, buf))) {
