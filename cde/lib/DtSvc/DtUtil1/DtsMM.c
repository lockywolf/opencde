...
int *_DtDtsMMGetDbName(DtDtsMMDatabase *db, DtShmBoson boson) {
...
void _DtDtsMMPrintFld(int fld, DtDtsMMField *fld_ptr, FILE *fd_in) {
        const char *tmp;
        const char *tmpv;
        FILE *fd = fd_in;
...
void _DtDtsMMPrintRec(int rec, DtDtsMMRecord *rec_ptr, FILE *fd_in) {
        int fld;
        ...
        fprintf(fd, "\tRec[%d] name = %s(%d)\n\t%d Fields\n", rec, tmp,
                rec_ptr->recordName, rec_ptr->fieldCount);
...
void _DtDtsMMPrintDb(int db, DtDtsMMDatabase *db_ptr, FILE *fd_in) {
        int rec;
...
int _DtDtsMMCompareRecordNames(DtDtsMMRecord *a, DtDtsMMRecord *b) {
        return (a->recordName - b->recordName);
}

int _DtDtsMMCompareFieldNames(DtDtsMMField *a, DtDtsMMField *b) {
        return (a->fieldName - b->fieldName);
}

#include <Dt/Dts.h>

int _DtDtsMMInit(int override) {
...
char **_DtsMMListDb() {
        ...
        for (i = 0; i < head->num_db; i++) {
                list[i] =
                    (char *)_DtDtsMMBosonToString(db_list[i].databaseName);
        }
        list[i] = 0;
        _DtSvcProcessUnlock();
        return (list);
...
DtDtsMMDatabase *_DtDtsMMGet(const char *name) {
        int i;
...
DtDtsMMField *_DtDtsMMGetField(DtDtsMMRecord *rec, const char *name) {
        register int i;
        int fld;
        DtDtsMMField *fld_ptr;
        DtDtsMMField *fld_ptr_list;
...
const char *_DtDtsMMGetFieldByName(DtDtsMMRecord *rec, const char *name) {
        DtDtsMMField *result;
...
DtDtsMMRecord *_DtDtsMMGetRecordByName(DtDtsMMDatabase *db, const char *name) {
        DtDtsMMRecord srch;
...
int _DtDtsMMPathHash(DtDirPaths *dirs) {
        int pathhash = 0;
        DIR *dirp;
...
char *_DtDtsMMCacheName(int override) {
        ...
        if (override && dsp) {
                results = malloc(strlen(_DTDTSMMTEMPDIR) +
                                 strlen(_DTDTSMMTEMPFILE) + strlen(dsp) + 3);
                sprintf(results, "%s/%s%s\0", _DTDTSMMTEMPDIR, _DTDTSMMTEMPFILE,
                        dsp);
                c = strchr(results, ':');
                c = strchr(c, '.');
                if (c) {
                ...
                /* This creates problems for rename() if "tmpfile" and
                 * "cacheFile" */
                ...
                results =
                    (char *)malloc(strlen(_DTDTSMMTEMPDIR) +
                                   strlen(_DTDTSMMTEMPFILE) + L_tmpnam + 3);
                tmpnam(tmpnam_buf);
...
int _DtDtsMMapDB(const char *CacheFile) {
        ...
        if (mmaped_fd != -1) {
                if (fstat(mmaped_fd, &buf) == 0 && buf.st_uid == getuid()) {
                        mmaped_db =
                            (char *)mmap(NULL, buf.st_size, PROT_READ,
                        ...
                        if (mmaped_db != (void *)-1) {
                                ...
                                db_list = (DtDtsMMDatabase *)_DtDtsMMGetPtr(
                                    head->db_offset);
                        } else {
                                _DtSimpleError(DtProgName, DtError, NULL,
                                               (char *)CacheFile, NULL);
...
static int _debug_print_name(char *name, char *label) {
        ...
        if (db)
                _DtSimpleError(db, DtInformation, NULL, "%s - db name = %s\n",
                               label, name);
#endif /* DEBUG */
        return (0);
}
