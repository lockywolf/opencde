...
_DtDtsDbPrintFields(DtDtsDbRecord *rec_ptr, FILE *fd) {
        ...
        for (fld = 0; fld < rec_ptr->fieldCount; fld++) {
                ...
                fprintf(fd, "\t\t[%d]\t%s\t%s\n", fld,
                        ...
                        fld_ptr->fieldValue ? fld_ptr->fieldValue : "(NULL)");
        }
}
...
void _DtDtsDbPrint(FILE *org_fd) {
        int db;
        int rec;
...
int _DtDtsDbCompareRecordNames(DtDtsDbRecord **a, DtDtsDbRecord **b) {
        return ((*a)->recordName - (*b)->recordName);
}

int _DtDtsDbCompareFieldNames(DtDtsDbField **a, DtDtsDbField **b) {
        return ((*a)->fieldName - (*b)->fieldName);
}

#include <Dt/Dts.h>

DtDtsDbDatabase **_DtDtsDbInit(void) {
        DtDtsDbDatabase **db;

        _DtSvcProcessLock();
        num_db = 0;
        db = db_list =
            (DtDtsDbDatabase **)calloc(num_db + 3, sizeof(DtDtsDbDatabase *));
...
DtDtsDbDatabase *_DtDtsDbAddDatabase(char *db_name) {
        int i = 0;
        ...
        new_db_list =
            (DtDtsDbDatabase **)calloc(i + 2, sizeof(DtDtsDbDatabase *));
...
int _DtDtsDbDeleteDb(DtDtsDbDatabase *db) {
        int i;
        int flag = 0;
...
DtDtsDbDatabase *_DtDtsDbGet(char *name) {
        DtDtsDbDatabase *ret_db;
        int i;
...
void _DtDtsDbFieldSort(DtDtsDbRecord *rec, _DtDtsDbFieldCompare compare) {
        if (compare == NULL) {
                compare = _DtDtsDbCompareFieldNames;
        }
        qsort(rec->fieldList, rec->fieldCount, sizeof(DtDtsDbField *),
              (genfunc)compare);
        rec->compare = compare;
}

void _DtDtsDbRecordSort(DtDtsDbDatabase *db, _DtDtsDbRecordCompare compare) {
        if (compare == NULL) {
                compare = _DtDtsDbCompareRecordNames;
        }
        qsort(db->recordList, db->recordCount, sizeof(DtDtsDbRecord *),
              (genfunc)compare);
        db->compare = compare;
}

DtDtsDbField *_DtDtsDbGetField(DtDtsDbRecord *rec, char *name) {
        register int i;
...
char *_DtDtsDbGetFieldByName(DtDtsDbRecord *rec, char *name) {
        DtDtsDbField *result;
...
DtDtsDbRecord *_DtDtsDbGetRecordByName(DtDtsDbDatabase *db, char *name) {
        DtDtsDbRecord srch;
        ...
        if (db->recordCount == 0 || db->recordList == NULL) {
                ...
                result = (DtDtsDbRecord **)bsearch(
                    &s, db->recordList, db->recordCount,
...
DtDtsDbRecord *_DtDtsDbAddRecord(DtDtsDbDatabase *db) {
...
int _DtDtsDbDeleteRecord(DtDtsDbRecord *rec, DtDtsDbDatabase *db) {
        int i;
        ...
        for (i = 0; i < db->recordCount; i++) {
                if (db->recordList[i] == rec) {
                        memmove(&(db->recordList[i]), &(db->recordList[i + 1]),
                                (db->recordCount - i - 1) *
                                    sizeof(DtDtsDbRecord *));
                        db->recordCount--;
                        return (1);
...
int _DtDtsDbDeleteRecords(DtDtsDbDatabase *db) {
        int i;
...
DtDtsDbField *_DtDtsDbAddField(DtDtsDbRecord *rec) {
...
int _DtDtsDbDeleteField(DtDtsDbField *fld, DtDtsDbRecord *rec) {
        int i;
        ...
        for (i = 0; i < rec->fieldCount; i++) {
                if (rec->fieldList[i] == fld) {
                        memmove(&(rec->fieldList[i]), &(rec->fieldList[i + 1]),
                                (rec->fieldCount - i - 1) *
                                    sizeof(DtDtsDbField *));
                        rec->fieldCount--;
                        return (1);
...
int _DtDtsDbDeleteFields(DtDtsDbRecord *rec) {
        int i;
