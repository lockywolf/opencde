...
/* local functions */
static DtDtsMMRecord *name_list(type_info_t *linfo, DtDtsMMDatabase *db,
...
static int gmatch(const char *string, const char *pattern) {
        wchar_t string_ch;
        int k;
        wchar_t pattern_ch;
        wchar_t lower_bound;
 ...
 */
static int csh_match_star(const char *string, const char *pattern) {
        wchar_t pattern_ch;
        wchar_t string_ch;
...
static char *max_buf(size_t size, type_info_t *info) {
        if (!info->mb) {
...
static type_info_t *set_vals(const char *fn, const char *buf, const int bs,
                             const struct stat *fs, const char *ln,
                             const struct stat *ls, const char *on) {
        char *tmp;
        ...
        if (fs) {
                ...
                memcpy((void *)linfo->file_stat, (struct stat *)fs,
                       sizeof(struct stat));
        }
        if (ln) {
                if (*ln == '/') {
        ...
        if (ls) {
                ...
                memcpy((void *)linfo->file_lstat, (struct stat *)ls,
                       sizeof(struct stat));
        }
        if (on) {
...
static char *cleanup(char *ot, type_info_t *info) {
        if (ot) {
                ot = strdup(ot);
        } else if (info->ot) {
                ot = strdup(info->ot);
...
static const char *get_opt_name(type_info_t *info) { return (info->opt_name); }

static const char *get_name(type_info_t *info) { return (info->name); }

static const char *get_file_path(type_info_t *info) {
        if (info->file_path) {
...
static int get_fd(type_info_t *info) {
        if (!info->error && info->file_fd == -1) {
                if (info->file_path == 0) {
                        return (-1);
                }
                if ((info->file_fd =
                         open(info->file_path, O_RDONLY | O_NOCTTY, 0)) == -1) {
                        info->error = errno;
                        return (-1);
                }
        }
...
static const struct stat *get_lstat(type_info_t *info) {
        struct stat buf;
...
static int islink(type_info_t *info) {
...
void get_link_info(type_info_t *info) {
...
static const char *get_link_path(type_info_t *info) {
        get_link_info(info);
        return (info->link_path);
}

static const char *get_link_name(type_info_t *info) {
        get_link_info(info);
        return (info->link_name);
}

static const char *get_buff(type_info_t *info) {
        ...
        if (!info->buffer && info->buffer != (char *)-1) {
                ...
                if (buf && buf->st_size) {
                        ...
                        if ((info->buffer = mmap(NULL, buf->st_size, PROT_READ,
                                                 MAP_PRIVATE, info->file_fd,
                                                 0)) == (char *)-1) {
                                info->mmap_size_to_free = -1;
...
int get_buff_size(type_info_t *info) { return (info->buff_size); }
static DtDtsMMDatabase *get_dc_db() {
...
static DtDtsMMDatabase *get_da_db() {
...
#define DTSATTRVAL(attr_name)                                                  \
        if (_DtDtsMMStringToBoson(attr_name) == fld_ptr->fieldName)

static int type_content(char *attr, type_info_t *info) {
        int match = 0;
        char *c;
        int end = 0, s;
        const char *buff = 0;
        ...
        if (!isdigit(*c) && *c != '-') {
                ...
                case 't': /* string */
                {
                        ...
                        c = _XStrtok(NULL, "\n", strtok_buf);
                        cl = strlen(c);
                        if ((info->buff_size - s) >= cl &&
                            memcmp(&buff[s], c, cl) == 0) {
                                match = 1;
                        } else {
                                match = 0;
                        }
                ...
                case 'y': /* byte */
                {
                        ...
                        while (c = _XStrtok(NULL, "\t \n", strtok_buf)) {
                                if (s + i * sizeof(lv) + sizeof(lv) >
                                    get_buff_size(info)) {
                                        match = 0;
                                        break;
                                }
                                memcpy(&lv, &buff[s + i * sizeof(lv)],
                                       sizeof(lv));
...
static char *_DtDtsGetDataType(const char *file) {
        int fd;
        char *name;
        ...
        if ((fd = open(name, O_RDONLY, 0644)) != -1) {
                if (fstat(fd, &file_stat) == 0) {
                        buff =
                            (u_char *)calloc((size_t)1, file_stat.st_size + 1);
...
static int type_mode(char *attr, type_info_t *info) {
        int match = 0;
        ...
        do {
                ...
                case 'l':
                        ...
                        break;
                case 'r':
                        if (buf->st_mode & S_IROTH || buf->st_mode & S_IRGRP ||
                            buf->st_mode & S_IRUSR) {
                                match = 1;
                        } else {
                                match = 0;
...
static int type_name(const char *name, char *attr) {
        int match = 0;
...
static int type_path(const char *path, char *attr) {
        char *c;
        int match = 0;
...
static char *next_sep(char *str, char *sep) {
        char *c;
        char *prev;
        ...
        while (*c) {
                ...
                prev = c - 1;
                if ((c == str) ||
                    (_is_previous_single(str, c) && (*prev != '\\'))) {
                        return (c);
                }
                _MBADV(c);
        }
...
char *DtDtsDataToDataType(const char *fp, const void *buf, const int bs,
                          const struct stat *fs, const char *ln,
                          const struct stat *ls, const char *on) {
        DtDtsMMDatabase *db;
        ...
        if (!dtdts_path_pattern) {
                ...
                dtdts_data_attributes_name =
                    _DtDtsMMStringToBoson(DtDTS_DATA_ATTRIBUTES_NAME);
                ...
                dtdts_da_description =
                    _DtDtsMMStringToBoson(DtDTS_DA_DESCRIPTION);
                        ...
                        do {
                                ...
                                */
                                ...
                                if (fld_ptr->fieldName == dtdts_path_pattern) {
                                        atr_m =
                                            type_path(get_file_path(info),
                                                      max_buf((size_t)0, info));
                                } else if (fld_ptr->fieldName ==
                                           dtdts_name_pattern) {
                                        ...
                                        if (get_file_path(info)) {
                                           ...
                                           dtdts_link_name) {
                                        atr_m =
                                            type_name(get_link_name(info),
                                                      max_buf((size_t)0, info));

                                } else if (fld_ptr->fieldName ==
                                           dtdts_link_path) {
                                        atr_m =
                                            type_path(get_link_path(info),
                                                      max_buf((size_t)0, info));
                                } else if (fld_ptr->fieldName ==
                                           dtdts_content) {
                                        atr_m = type_content(
                                           ...
                                           dtdts_data_attributes_name) {
                                        info->ot =
                                            strdup(max_buf((size_t)0, info));
                                }

                                if (info->error == ELOOP) {
                                        _DtSvcProcessUnlock();
                                        _DtSvcAppUnlockDefault();
                                        return (cleanup(DtDTS_DT_RECURSIVE_LINK,
                                                        info));
                                }
                                if (info->ot) {
                                        _DtSvcProcessUnlock();
...
char *DtDtsBufferToDataType(const void *buffer, const int size,
                            const char *name) {
        char *dt = DtDtsDataToDataType(0, buffer, size, 0, 0, 0, name);
        return (dt);
}

static char *expand_keyword(const char *attr_in, const char *in_pathname) {
        char *buf;
        char *netPath;
        char *c;
        char *tmp;
                ...
                for (c = attr, i = 0; *c && i < n; c++) {
                        ...
                        if (!strncmp(c, "%file%", 6)) {
                                ...
                                *tmp = '/';
                                ...
                                c += 5;
                        } else { /* no match -- just copy the character */
                                buf[i++] = *c;
                                buf[i] = 0; /* ensure null string termination */
                                continue;
                        }
...
char *expand_shell(const char *attr) {
        char *srch;
        char *nattr;
        char *start, *end;
        int size;
...
static char *expand_value(DtShmBoson attr, char *in_pathname) {
        char *tmp;
        char *exp;
        char *shell_exp;
...
char *DtDtsDataTypeToAttributeValue(const char *obj_type, const char *attr,
                                    const char *filename) {
        DtDtsMMDatabase *db;
        DtDtsMMRecord *entry;
        char *value = 0;
        ...
        if (db && obj_type) {
                ...
                if (entry) {
                        char *name = 0;
                        DtDtsMMField *fld =
                            _DtDtsMMGetField(entry, (char *)attr);
...
DtDtsAttribute **DtDtsDataTypeToAttributeList(const char *obj_type,
                                              const char *filename) {
        DtDtsMMDatabase *db;
        DtDtsMMRecord *entry;
        DtDtsMMField *fld_ptr_list;
        ...
        for (i = 0; i < entry->fieldCount; i++) {
                ...
                list[i]->name =
                    expand_value(fld_ptr->fieldName, (char *)filename);
...
void DtDtsFreeAttributeList(DtDtsAttribute **list) {
...
DtDtsAttribute **DtDtsBufferToAttributeList(const void *buffer, const int size,
                                            const char *name) {
...
char *DtDtsFileToAttributeValue(const char *filepath, const char *attr) {
        char *ot = DtDtsFileToDataType(filepath);
...
char *DtDtsBufferToAttributeValue(const void *buffer, const int size,
...
void DtDtsFreeAttributeValue(char *value) {
        if (value)
                free(value);
}
...
char **DtDtsDataTypeNames(void) {
        ...
        for (i = 0; i < db->recordCount; i++) {
                rec_ptr = &rec_list[i];
                names[j] =
                    strdup((char *)_DtDtsMMBosonToString(rec_ptr->recordName));
                j++;
        }
        names[j] = 0;
        _DtSvcProcessUnlock();
...
char **DtDtsFindAttribute(const char *name, const char *value) {
        DtDtsMMDatabase *ot;
        int i;
        int j = 0;
        char **list;
        ...
        for (i = 0; i < ot->recordCount; i++) {
                ...
                v = _DtDtsMMExpandValue(
                    _DtDtsMMGetFieldByName(rec_ptr, (char *)name));
                if (v && !strcmp(value, v)) {
                        list[j++] =
                            strdup(_DtDtsMMBosonToString(rec_ptr->recordName));
                }
                _DtDtsMMSafeFree(v);
        }
        list[j] = 0;
...
void DtDtsFreeDataTypeNames(char **list) {
        int i = 0;
...
char *DtDtsSetDataType(const char *filename, const char *datatype_in,
                       const int overide) {
        int fsize;
        char *file;
        int fd;
        ...
        if ((fd = open(file, O_EXCL | O_CREAT | O_RDWR, 0644)) != -1) {
                ...
                sprintf((char *)buff, DIR_INFO, DtDTS_DATA_ATTRIBUTES_NAME,
                        datatype_in);

                write_size = write(fd, buff, size);
                if (write_size != size) {
                        _DtSimpleError(DtProgName, DtError, NULL, (char *)buff,
                                       NULL);
                }
                ...
                if (dt == NULL) {
                        ...
                        sprintf((char *)buff, DIR_INFO,
                                DtDTS_DATA_ATTRIBUTES_NAME, datatype_in);
                        write_size = write(fd, buff, size);
                        if (write_size != size) {
                                _DtSimpleError(DtProgName, DtError, NULL,
                                               (char *)buff, NULL);
                        ...
                        off = write(fd, &buff[end],
                                    strlen((char *)&buff[end]) + 1);
                        if (off != strlen((char *)&buff[end]) + 1) {
                                _DtSimpleError(DtProgName, DtError, NULL,
                                               (char *)file, NULL);
                        } else {
                                total =
                                    file_stat.st_size - (end - start) + dtsize;
                                ftruncate(fd, total);
                                datatype = strdup(datatype_in);
                        }
                }
...
int *get_name_list(char *name, int *count) {
        ...
        *count = 0;
        ...
        results =
            (int *)_DtShmFindIntTabEntry(_DtDtsMMGetDCNameIndex(&size), boson);
        if (!results) {
                return ((int *)-1);
        }
        if (*results < 0) {
...
static DtDtsMMRecord *name_list(type_info_t *linfo, DtDtsMMDatabase *db,
        ...
        if (rec_ptr == 0) {
                ...
                if (!name) {
                        ...
                        linfo->name_prev =
                            (int *)_DtDtsMMGetBufferIndex(&linfo->name_count);
                        linfo->name_type = 3;
                } else {
                        if (name && *name) {
                ...
                if (!linfo->name_count && name) {
                        ...
                        linfo->name_prev =
                            get_name_list(name, &linfo->name_count);
                        linfo->name_type = 1;
                }
                if (!linfo->name_count && suffix) {
                        /* find if suffix exist in our list */
                        linfo->name_prev =
                            get_name_list(suffix, &linfo->name_count);
                        linfo->name_type = 2;
                }
                if (!linfo->name_count) {
                        /* neither exist so check ambugious ones */
                        linfo->name_prev =
                            (int *)_DtDtsMMGetNoNameIndex(&linfo->name_count);
                        linfo->name_type = 3;
                }
        } else {
                ...
                if (linfo->name_count > 0) {
                        ...
                        case 1:
                                ...
                                if (suffix) {
                                        /* if found us it */
                                        linfo->name_prev = get_name_list(
                                            suffix, &linfo->name_count);
                                        linfo->name_type = 2;
                                        break;
                                }
                                /* other wise */
...
Boolean DtDtsIsTrue(const char *str) {
        if (str &&
