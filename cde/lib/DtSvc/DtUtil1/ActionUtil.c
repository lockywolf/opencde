...
char *_DtGetActionIconDefault(void) {
        ...
        sprintf(name, "%s*%s", _DtActNULL_GUARD(_DtApplicationName),
        ...
        if (bytesNeeded > _DtAct_MAX_BUF_SIZE)
                ...
                class = classBuf;
        sprintf(class, "%s*%s", _DtActNULL_GUARD(_DtApplicationClass),
        ...
        if (_DtDisplay)
                ...
                db = 0;
        if (db &&
            XrmGetResource(db, nameBuf, classBuf, &rep_type, &resource_value))
...
char *_DtGetExecHostsDefault(void) {
        ...
        bytesNeeded =
            strlen(DtEXEC_HOSTS_NAME) + strlen(_DtApplicationName) + 4;
        ...
        bytesNeeded =
            strlen(DtEXEC_HOSTS_CLASS) + strlen(_DtApplicationClass) + 4;
        if (bytesNeeded > _DtAct_MAX_BUF_SIZE)
                class = XtMalloc(bytesNeeded);
        else
                class = classBuf;
...
char *_DtGetDtTmpDir(void) {
        ...
        if (db &&
            XrmGetResource(db, nameBuf, classBuf, &rep_type, &resource_value))
                ...
                snprintf(dirBuf, MAXPATHLEN, "%s/%s", home,
                         DtACTION_DTTMPDIR_DEFAULT);
...
void _DtRemoveTrailingBlanksInPlace(char **s) {
        ...
        for (p = DtPrevChar(*s, *s + strlen(*s)); DtIsspace(p) && (p > *s);
             p = DtPrevChar(*s, p))
                *p = '\0';
}
...
DtActionInvocationID _DtActAllocID() {
        ...
        do {
                ...
                /*
                 * Need to track down better define than INT_MAX to determine
                 * cap
                 */
...
_DtActInvRecT *_DtActAllocInvRec() {
        ...
        if (first_time) {
                ...
                _DtActInvRecArray = (_DtActInvRecT **)XtMalloc(
        ...
        if (newslot == -1) {
                ...
                _DtActInvRecArray = (_DtActInvRecT **)XtRealloc(
                    (char *)_DtActInvRecArray,
                    sizeof(_DtActInvRecT *) * _actInvRecArraySize);
         ...
         */
        _DtActInvRecArray[newslot] =
            (_DtActInvRecT *)XtCalloc(1, sizeof(_DtActInvRecT));
...
static void _DtActFreeChildRec(_DtActChildRecT *childRecP) {
        ...
        if (IS_CMD(childRecP->mask)) {
                ...
                if (childRecP->u.cmd.reqMessage) {
                        data = (CallbackData *)tt_message_user(
                            childRecP->u.cmd.reqMessage, 0);
        ...
        for (i = 0; i < _actInvRecArraySize; i++) {
                if (_DtActInvRecArray[i]) {
                        if ((_DtActInvRecArray[i]->id) == id) {
                                ...
                                /*
                                 * Check for any tmp files created to house
                                 * buffers Delete them if they still exist.
                                 */
                                        ...
                                        if (IS_BUFFER_OBJ(infop->mask) &&
                                            IS_FILE_OBJ(infop->mask)) {
                                                ...
                                                if (!infop->name)
                                                        continue;
                                                /*
                                                 * These tmp names should have
                                                 * been created by the execution
                                                 * management code and such
                                                 * names should not refer to
                                                 * directories. We will not
                                ...
                                /*
                                 * Cleanup invocation record.  Don't worry about
                                 * squeezing and realloc'ing the
                                 * _DtActInvRecArray.
                                 */
...
_DtActChildRecT *_DtActAllocChildRec(_DtActInvRecT *invRec) {
        ...
        if (invRec) {
                ...
                if (invRec->numChildren == 1) {
                         ...
                         */
                        invRec->childRec = (_DtActChildRecT **)XtMalloc(
                            sizeof(_DtActChildRecT *));
                } else {
                        invRec->childRec = (_DtActChildRecT **)XtRealloc(
                            (char *)invRec->childRec,
                            sizeof(_DtActChildRecT *) * invRec->numChildren);
                }

                /*
                 * Hang a child rec off and initialize
                 */
                invRec->childRec[invRec->numChildren - 1] =
                    (_DtActChildRecT *)XtMalloc(sizeof(_DtActChildRecT));

                tchildRec =
                    invRec->childRec[invRec->numChildren - 1]; /* shorthand */

                memset(tchildRec, 0, sizeof(_DtActChildRecT));
                tchildRec->childId =
                    invRec->numChildren; /* serial # of sorts */
 ...
 */
_DtActChildRecT *_DtActFindChildRec(DtActionInvocationID id,
 ...
 */
static void _DtActCacheArgs(_DtActInvRecT *invRecP, DtActionArg *newArgp,
                            int newArgc, DtActionStatus userStatus) {
        ...
        if (invRecP->cachedUploadCnt)
                invRecP->cachedUploads = (_DtActUpdateCache *)XtRealloc(
                    (char *)invRecP->cachedUploads,
                    (invRecP->cachedUploadCnt + 1) * sizeof(_DtActUpdateCache));
        else
                invRecP->cachedUploads =
                    (_DtActUpdateCache *)XtMalloc(sizeof(_DtActUpdateCache));
        ...
        invRecP->cachedUploads[invRecP->cachedUploadCnt].userStatus =
            userStatus;
 ...
 */
void _DtActExecutionLeafNodeCleanup(DtActionInvocationID id,
                                    DtActionArg *newArgp, int newArgc,
        ...
        if (userStatus == DtACTION_STATUS_UPDATE) {
                ...
                if ((newArgc) && (invRecP->cb)) {
                        if (useCache) {
                                _DtActCacheArgs(invRecP, newArgp, newArgc,
                                                userStatus);
                        } else {
                                (invRecP->cb)(id, invRecP->client_data, newArgp,
                                              newArgc, userStatus);
                        }
                }
        } else {
...
void *_DtActReadTmpFileToBuffer(char *fname, int *sizep) {
        ...
        for (size = 0, space = MAX_BUF_SIZE;
             (bytes = read(fd, buf + size, space)) != 0; size += bytes) {
                if (bytes < 0) {
                ...
                if ((space -= bytes) <= 0) {
                         ...
                         */
                        buf = (char *)XtRealloc((char *)buf,
                                                size + bytes + MAX_BUF_SIZE);
                        space = MAX_BUF_SIZE;
                }
        }
        ...
        for (i = 0; i < childp->numObjects; i++) {
                ...
                if (IS_BUFFER_OBJ(infop->mask)) {
                        if (IS_FILE_OBJ(infop->mask)) {
                                ...
                                newArgp[Idx].u.buffer.type =
                                    XtNewString(infop->type);
                                /*
                                 * RWV:
                                 *     If did the following, the returned buffer
                                 * name may NOT be the same as the name which
                                 * was passed in to us (e.g. we were unable
                                 * to create a tmp file with the desired
                                 * name).  At present there is nothing that
                                 * preseves the original buffer name.
                                 *
                                 *   newArgp[Idx].u.buffer.name =
                                 * _DtBasename(infop->name);
                                 *
                                 *     instead we'll set the returned name field
                                 * to NULL; this is after all a new copy of the
                                 * buffer.
                                 */
                                 ...
                                 *  -- THIS SHOULD NEVER BE THE CASE FOR COMMAND
                                 * ACTIONS--
                                 */
                        ...
                        /*
                         * RWV
                         * What about file objects passed in with the String
                         * qualifier? This returns the original string string
                         * which may or may not have been a file name.  Are we
                         * OK here?
                         */
