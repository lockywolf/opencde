...
DtDtsMMRecord *_DtActionLocateRecord(DtShmBoson actQuark, unsigned long mask,
                                     DtShmBoson arg_type, int arg_count,
...
static int _DtActMMCompareObjClassMask(unsigned long objClassMask,
                                       char *actClass);
...
static int _DtActionCompareRecordQuarks(DtDtsDbRecord **record1,
                                        DtDtsDbRecord **record2);
static void _DtActionGetRecordWeight(DtDtsDbRecord **record,
                                     long *primaryWeightPtr,
                                     long *secondaryWeightPtr);
static void ParseMessage(register ActionPtr action, register char *str,
                         register parsedMsg *msg);
static Boolean ValidKeyword(char *start, char *end, char **prompt,
                            int *keywordId, int *argNum, Boolean *isFile);
...
static int _DtActInputSeqCheck(DtDtsDbRecord *r1, DtDtsDbRecord *r2);
static char *_DtActDbChooseLabel(DtDtsDbRecord *rec);
static char *_DtActMMChooseLabel(DtDtsMMRecord *rec);
static void _DtCheckForDuplicateRecord(DtDtsDbRecord *rec,
...
static char *_DtActionDbDescription(char *s) {
        ...
        for (act_rec = act_db->recordList; n && act_rec && *act_rec;
             act_rec++, n--) {
        ...
        if (last_rec_found) {
                _DtSvcProcessUnlock();
                return XtNewString(_DtActGetDtsDbField(*last_rec_found,
...
Boolean DtActionExists(char *s) {
        ...
        _DtSvcAppLockDefault();
        actionExists =
            (use_in_memory_db) ? _DtActionDbExists(s) : _DtActionMMExists(s);
        _DtSvcAppUnlockDefault();
...
char *DtActionLabel(char *s) {
        ...
        _DtSvcAppLockDefault();
        actionLabel =
            (use_in_memory_db) ? _DtActionDbLabel(s) : _DtActionMMLabel(s);
        _DtSvcAppUnlockDefault();
...
char *DtActionIcon(char *s) {
        ...
        _DtSvcAppLockDefault();
        actionIcon =
            (use_in_memory_db) ? _DtActionDbIcon(s) : _DtActionMMIcon(s);
        _DtSvcAppUnlockDefault();
...
static char *_DtActionDbIcon(char *s) {
        ...
        if (last_rec_found) {
                 ...
                 */
                if ((iconString = _DtActGetDtsDbField(
                         *last_rec_found, _DtACTION_ICON)) != NULL) {
                        _DtSvcProcessUnlock();
...
static char *_DtActionMMIcon(char *s) {
        ...
        if (last_rec_found) {
                 ...
                 * Obtain the associated icon name (if any).
                 */
                if ((iconString = _DtActGetDtsMMField(
                         last_rec_found, _DtACTION_ICON)) != NULL) {
                        _DtSvcProcessUnlock();
                        return iconString;
                }
...
void _DtSortActionDb(void) {
        ...
        for (i = 0; i < actionDB->recordCount; i++) {
                if (actionDB->recordList[i]->pathId == 0) {
                        ...
                        /*
                         * Record deletion has adjusted the recordCount field
                         * for the database.  We must now adjust our counter (i)
                         * because record deletion shifts all subsequent records
                         * down to backfill holes in the record array. This
                         * means we must check the record at "i" again because
                         * it is now a new, unchecked record.
                         */
                        i--;

                        /*
                         * RWV: We should probably add the capacity to announce
                         * removal of such duplicate records.
                         */
                }
        }
        _DtSvcProcessUnlock();
...
static int _DtActionCompareRecordQuarks(DtDtsDbRecord **record1,
        ...
        if (r1PrimaryWeight > r2PrimaryWeight)
                ...
                if (r1SecondaryWeight > r2SecondaryWeight)
                         ...
                         * We cannot simply assume that record1 preceded record2
                         * in the input order since the quick sort itself may
                         * have done some rearranging, we must compare sequence
                         * numbers to determine which action was defined first.
                         *
                         */
...
_DtActMMParseArgTypesList(char *tlist, DtShmBoson **quarkAddr, int *countp) {
        ...
        *countp = tcount;
        myassert(tcount != 0);
        qp = *quarkAddr =
            (DtShmBoson *)XtCalloc(tcount + 1, sizeof(DtShmBoson));
...
static void _DtActMMGetTtMsgInfo(DtDtsMMRecord *actRecp, ActionPtr actp) {
        char argNname[sizeof(_DtACTION_TTN_ARG) /* space for prefix */
                      ...
                      + sizeof(_DtACTION_TTN_REP_TYPE)]; /* space for longest
                                                            suffix */
        int i;
        char *s;
        char *tmp = NULL;
        ...
        for (i = 0; True; i++) {
                sprintf(argNname, "%s%d%s", _DtACTION_TTN_ARG, i,
                        _DtACTION_TTN_MODE);
                if (!(s = _DtActGetDtsMMField(actRecp, argNname)))
                        break; /* no more args */
                actp->u.tt_msg.tt_argn_mode = (int *)XtRealloc(
                    (char *)actp->u.tt_msg.tt_argn_mode, sizeof(int) * (i + 1));
                ...
                actp->u.tt_msg.tt_argn_vtype =
                    (parsedMsg *)XtRealloc((char *)actp->u.tt_msg.tt_argn_vtype,
                ...
                actp->u.tt_msg.tt_argn_rep_type =
                    (int *)XtRealloc((char *)actp->u.tt_msg.tt_argn_rep_type,
                                     sizeof(int) * (i + 1));
                _DtDtsMMSafeFree(s);
                s = (char *)0;
                if (!(s = _DtActGetDtsMMField(actRecp, argNname)))
...
static void _DtActDbGetDtNtfyInfo(DtDtsDbRecord *actRecp, ActionPtr actp) {
        ...
        for (i = 0; True; i++) {
                sprintf(argNval, "%s%d%s", _DtACTION_DTN_ARG, i,
                        _DtACTION_DTN_VALUE);
                if (!(s = _DtActGetDtsDbField(actRecp, argNval)))
                        break; /* no more args */
                actp->u.dt_notify.argn_value =
                    (parsedMsg *)XtRealloc((char *)actp->u.dt_notify.argn_value,
                                           sizeof(parsedMsg) * (i + 1));
...
static void _DtActDbGetDtReqInfo(DtDtsDbRecord *actRecp, ActionPtr actp) {
        ...
        for (i = 0; True; i++) {
                ...
                actp->u.dt_req.argn_value =
                    (parsedMsg *)XtRealloc((char *)actp->u.dt_req.argn_value,
                                           sizeof(parsedMsg) * (i + 1));
...
static int _DtActMMCompareMode(unsigned long objMask, char *actMode) {
        if (!actMode || !strcmp(actMode, _DtACT_ARG_MODE_ANY) ||
...
DtDtsMMRecord *_DtActionLocateRecord(DtShmBoson actQuark,
        ...
        for (n = *start; n < act_db->recordCount; n++) {
                ...
                if (!_DtActMMCompareObjClassMask(
                        obj_mask, tmp = _DtActGetDtsMMField(
                                      act_rec, _DtACTION_ARG_CLASS)))
                        continue;

                _DtDtsMMSafeFree(tmp);
                if (!_DtActMMCompareType(
                        arg_type,
                ...
                if (!_DtActMMCompareCount(arg_count,
                                          tmp = _DtActGetDtsMMField(
                                              act_rec, _DtACTION_ARG_COUNT)))
                        continue;

                _DtDtsMMSafeFree(tmp);
                if (!_DtActMMCompareMode(
                        obj_mask,
...

{
        ...
        *argNum = NO_ARG;
        ...
        if ((len >= stringQualifierLen) &&
            (strncmp(_DtACT_STRING_QUALIFIER, start,
                     (size_t)stringQualifierLen) == 0)) {
                FoundQualifier = True;
                *isFile = False;
                start += stringQualifierLen;
        } else if ((len >= fileQualifierLen) &&
                   (strncmp(_DtACT_FILE_QUALIFIER, start,
                            (size_t)fileQualifierLen) == 0)) {
                FoundQualifier = True;
                *isFile = True;
                start += fileQualifierLen;
        } else
                *isFile =
                    False; /* default to string behavior (except for Arg*) */
            ...
            (strncmp(_DtACT_LOCALHOST_STR, start, (size_t)localHostLen) == 0)) {
                ...
                *argNum = NO_ARG;
                ...
                start += localHostLen;
                   ...
                   (strncmp(_DtACT_DATABASEHOST_STR, start,
                            (size_t)databaseHostLen) == 0)) {
                *isFile = False;
                ...
                *argNum = NO_ARG;
                ...
                start += databaseHostLen;
                   ...
                   (strncmp(_DtACT_DISPLAYHOST_STR, start,
                            (size_t)displayHostLen) == 0)) {
                *isFile = False;
                ...
                *argNum = NO_ARG;
                ...
                start += displayHostLen;
                   ...
                   (strncmp(_DtACT_SESSIONHOST_STR, start,
                            (size_t)sessionHostLen) == 0)) {
                *isFile = False;
                ...
                *argNum = ALL_ARGS;
                ...
                start += argsLen;
        } else if ((len >= arg_Len) && (strncmp(_DtACT_ARG_UNDER_STR, start,
                                                (size_t)arg_Len) == 0)) {
                /* Flag that a keyword was found */
                *keywordId = ARG;
                /*
        ...
        if ((start != end) && (*start == '"') && (lastChLen == 1) &&
            (*lastCh == '"')) {
...
ActionPtr _DtActionFindDBEntry(ActionRequest *reqp, DtShmBoson actQuark) {
        ...
        for (actRecp = _DtActionLocateRecord(actQuark, mask, type, num, act_db);
             actRecp;
             actRecp = _DtActionLocateRecord(_DtDtsMMStringToBoson(mapto), mask,
                                             type, num, act_db),
            numSearches++) {
                /*
                 ...
                 */
                if (!(mapto =
                          _DtActGetDtsMMField(actRecp, _DtACTION_MAP_ACTION)))
                        break;
        ...
        if (actRecp) {
                ...
                actp->description =
                    _DtActGetDtsMMField(actRecp, _DtACTION_DESCRIPTION);
...

{
        ...
        while (1) {
                ...
                if ((keywordStart == NULL) || (keywordEnd == NULL)) {
                        /*
                         * No more keywords left; bundle the remaining string
                         * into a parse block, and return.
                         */
                        if (strlen(string) > 0) {
                                msg->numMsgParts++;
                                msg->parsedMessage = (MsgComponent *)XtRealloc(
                                    (char *)msg->parsedMessage,
                                    (Cardinal)(sizeof(MsgComponent) *
                                               msg->numMsgParts));

                                newBlock =
                                    msg->parsedMessage + (msg->numMsgParts - 1);
                        ...
                        /*
                         * We've encountered what looks to be a keyword; see if
                         * it is a valid one.  If it's not valid, then continue
                         * looking; otherwise, save it, and continue the parsing
                         * processing.
                         */
                        if (ValidKeyword(keywordStart + 1, keywordEnd - 1,
                                         &prompt, &keyword, &argNum, &isFile)) {
                                msg->numMsgParts++;
                                msg->parsedMessage = (MsgComponent *)XtRealloc(
                                    (char *)msg->parsedMessage,
                                    (Cardinal)(sizeof(MsgComponent) *
                                ...
                                if (keywordStart != string) {
                                        /* Extract what we need; convert to NULL
                                         * terminated */
                                        newBlock->precedingText =
                                            (char *)XtMalloc((Cardinal)(
                                                keywordStart - string + 1));
                                        (void)strncpy(
                                            newBlock->precedingText, string,
                                            (size_t)(keywordStart - string));
                                        newBlock->precedingText[keywordStart -
                                                                string] = '\0';
                                } else
