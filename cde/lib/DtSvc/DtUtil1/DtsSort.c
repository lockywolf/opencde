...
static char *get_value(DtDtsDbRecord *ce_entry, char *value) {
        int i = 0;
...
void parts_of_pattern(char *c, int *spec, int *count, int *front) {
        int nested = 0;
        int found = 0;
        int len;
...
int check_pattern(char *value1, char *value2) {
...
check_content(char *val1, char *val2) {
        ...
        while (isspace((u_char)*cnt1))
                cnt1++;

        loc2 = _XStrtok(v2, "\t \n", strtok_buf);
        type2 = _XStrtok(NULL, "\t \n", strtok_buf);
        cnt2 = &type2[strlen(type2) + 1];
        while (isspace((u_char)*cnt2))
                cnt2++;
...
static int sfe(DtDtsDbRecord *item1, DtDtsDbRecord *item2) {
        int test1 = 0;
        int test2 = 0;
        int loc1;
        int loc2;
...
int cde_dc_compare(DtDtsDbRecord **a, DtDtsDbRecord **b) {
...
static int cde_ft_field_value(XrmQuark name_quark) {
...
int cde_dc_field_compare(DtDtsDbField **a, DtDtsDbField **b) {
