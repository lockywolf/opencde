...
#define NUM_FIELD_NAMES                                                        \
        sizeof(_DtActNamesAndBits) / sizeof(_DtActNameAndBit) - 1
...
static char *_DtActFieldNameLookup(long bitmask) {
        register int j;
 ...
 *
 ******************************************************************************/
static Boolean _DtActDupFieldNameCheck(DtDtsDbField **fields, int numFields,
                                       XrmQuark name) {
        register int i;
 ...
 *
 ******************************************************************************/
static char *_DtActValidateFieldValue(long bit, char *value, char *actName,
                                      char *filename) {
        char *val;
        char *start;
        char *p = NULL;
        ...
        case _ActDb_TT_ARGN_RTYP_SET:
                /*
                 * REP_TYPE -- representation type of the corresponding tooltalk
                 * value. Valid values are: UNDEFINED, INTEGER, BUFFER, and
                 * STRING.
                 */
...
static char *_DtActCheckRecord(DtDtsDbRecord *actp, char *actionType,
                               unsigned long mask, char *fileName) {
        int i;
        char *s;
        char argnFieldName
        ...
        if (!strcmp(actionType, _DtACTION_COMMAND)) {
                 ...
                 */
                if (mask & (_ActDb_CMD_BITS | _ActDb_DT_NOTIFY_BITS |
                            _ActDb_TT_BITS | _ActDb_MAP_BITS)) {
                        /*
                 ...
                 */
                if (mask & (_ActDb_DT_REQUEST_BITS | _ActDb_CMD_BITS |
                            _ActDb_TT_BITS | _ActDb_MAP_BITS)) {
                        /*
                 ...
                 */
                buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
                (void)sprintf(buf, unrecogType, _DtACTION_TYPE, actionType,
                              fileName);
                return buf;
        }
...
Boolean _DtActionConverter(DtDtsDbField *fields, DtDbPathId pathId,
        ...
        for (i = 1; fields[i].fieldName; i++) {
                ...
                if ((ThisBit = _DtActFieldBitLookup(
                         XrmQuarkToString(fields[i].fieldName))) != 0) {
                        switch (
                            (intptr_t)(fieldVal = _DtActValidateFieldValue(
                                           ThisBit, fields[i].fieldValue,
                ...
                } else /* Unrecognized Field */
                {
                         ...
                         */
                        buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
                        (void)sprintf(buf, unrecogField, fields[0].fieldValue,
                                      fileName,
                ...
                if (mask & ThisBit) {
                        /*
                         * An exhaustive name check is required for the
                         * "*_ARGn_" fields -- a simple bitmask check is
                         * sufficient for other field names.
                         */
                        if (((ThisBit & _ActDb_ARGN_BITS) &&
                            ...
                            !(ThisBit & _ActDb_ARGN_BITS)) {
                                ...
                                buf = XtMalloc(_DtAct_MAX_BUF_SIZE);
                                (void)sprintf(
                                    buf, dupFields, fields[0].fieldValue,
                                    fileName,
                                    XrmQuarkToString(fields[i].fieldName));
                                _DtSimpleError(DtProgName, DtError, NULL, "%s",
                                               buf);
...
static void InitializeLocalizedStrings(void) {
        ...
        unrecogField = XtNewString(((char *)Dt11GETMESSAGE(
            6, 2,
            "The action definition \"%s\" in the file\n  \"%s\"\n  contains "
            "the following unrecognized field name and value:\n  "
            "\"%s\":\"%s\"\n")));

        unrecogType = XtNewString(((char *)Dt11GETMESSAGE(
            6, 4,
        ...
        noMapTo = XtNewString(((char *)Dt11GETMESSAGE(
            6, 5,
            "The action definition \"%s\" in the file\n  \"%s\"\n  is not "
            "mapped properly.  The \"%s\" field\n  should be: TYPE MAP "
            "action_mapped_to.\n")));

        noExecString = XtNewString(((char *)Dt11GETMESSAGE(
            6, 8,
            "The action definition \"%s\" in the file\n  \"%s\"\n  is a "
            "\"COMMAND\" action type but lacks the\n  required field "
            "\"%s\".\n")));

        unrecogMsgType = XtNewString(((char *)Dt11GETMESSAGE(
            6, 11,
            "The action definition \"%s\" in the file\n  \"%s\"\n  has the "
            "illegal value \"%s\" in the \"%s\" field.\n")));

        dupFields = XtNewString(((char *)Dt11GETMESSAGE(
            6, 12,
            "The action definition \"%s\" in the file\n  \"%s\"\n  contains "
            "duplicate fields named:\n \"%s\".\n")));

        invldField = XtNewString(((char *)Dt11GETMESSAGE(
            6, 13,
            "The action definition \"%s\" in the file\n  \"%s\"\n  contains "
            "invalid fields for \"%s\" type actions.\n")));

        missingField = XtNewString(((char *)Dt11GETMESSAGE(
            6, 14,
            "The action definition \"%s\" in the file\n  \"%s\"\n  is missing "
            "the required field, \"%s\"\n for \"%s\" type actions.\n")));

        missingName = XtNewString(((char *)Dt11GETMESSAGE(
            4, 1,
            "A \"%s\" definition in the file:\n  \"%s\"\n  is missing a name.  "
            "Add the name\n  to the defintion.\n")));

        unrecogMsgType2 = XtNewString(((char *)Dt11GETMESSAGE(
            6, 16,
            "The action definition \"%s\" in the file\n  \"%s\"\n  has the "
