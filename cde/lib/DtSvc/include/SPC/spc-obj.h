 ...
 */
/*
 * File:         spc-obj.h $XConsortium: spc-obj.h /main/3 1995/10/26 15:43:20
 * rswiston $ Language:     C
 *
...
#define channel_class_part                                                     \
        root_class_part SPC_F_open open; /* opening a channel */               \
        ...
        SPC_F_wait_for_termination                                             \
            wait_for_termination;  /* wait for subprocess to exit */           \
        SPC_F_pre_fork pre_fork;   /* setup before a fork */                   \
        SPC_F_post_fork post_fork; /* setup after a fork */                    \
        SPC_F_remove_logfile                                                   \
            remove_logfile;      /* remove log file associated with channel */ \
...
#define call_parent_method(channel, func, args, result)                        \
        {                                                                      \
                pipe_channel_clasp parent_class =                              \
                    (pipe_channel_clasp)(channel)->class_ptr;                  \
