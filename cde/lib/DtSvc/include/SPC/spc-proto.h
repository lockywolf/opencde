 ...
 */
/*
 * File:         spc-proto.h $XConsortium: spc-proto.h /main/3 1995/10/26
 * 15:43:39 rswiston $ Language:     C
 *
...
#define NREQS                                                                  \
        27 /* total number of requests.  If                                    \
...
#define WRITE_INT(pdata, val) sprintf_len(PDRP(pdata), (XeString) "%x", val)
...
#define WRITE_APPLICATION_SPAWN(pdata, path, dir, argv, envp)                  \
        sprint_application_data(PDRP(pdata), (XeString) "%s %d %d ", path,     \
                                dir, argv, envp, REQUEST_HEADER_LENGTH)
#define READ_APPLICATION_SPAWN(pdata, path, dir, argv, envp)                   \
        sscan_application_data(PDRP(pdata), (XeString) "%s %d %d ", &path,     \
                               &dir, &argv, &envp, REQUEST_HEADER_LENGTH)
...
#define WRITE_HEADER(pdata, cid, type, len, seq)                               \
        sprintf_len(pdata->data, (XeString) "%08x%02x%04x%04x", cid, type,     \
                    len, seq)
...
#define REPLY_EXPECTED(a, retval)                                              \
        (((a == REPLY) || (a == APPLICATION_DATA) ||                           \
          (a == APPLICATION_STDOUT) || (a == APPLICATION_STDERR) ||            \
          (a == REGISTER) || (a == QUERY_LOGFILE) || (a == QUERY_DEVICES) ||   \
          (a == ABORT) || (a == SERVER_ERROR))                                 \
             ? NO_REPLY_VAL                                                    \
             : retval)
...
int sprint_application_data(XeString buf, XeString fmt, XeString path,
                            XeString dir, XeString *argv, XeString *envp,
                            int chars_used);
int sscan_application_data(XeString buf, XeString fmt, XeString *path,
                           XeString *dir, XeString **argv, XeString **envp,
                           int offset);
