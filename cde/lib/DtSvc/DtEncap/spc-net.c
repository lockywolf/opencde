...
/*----------------------------------------------------------------------+*/
{
        ...
        while (alias_count--) {
                copy->h_aliases[alias_count] =
                    SPC_copy_string(host_def->h_aliases[alias_count]);
        }
...
/*----------------------------------------------------------------------+*/
{
        ...
        if ((connection = SPC_Make_Connection(canonical_hostname)) ==
            SPC_ERROR) {
                SPC_Close_Connection(connection);
                XeFree(canonical_hostname);
                return (SPC_ERROR);
        ...
        if ((connection->remote = SPC_Lookup_Host(canonical_hostname)) ==
            SPC_ERROR) {
                SPC_Close_Connection(connection);
                XeFree(canonical_hostname);
                return (SPC_ERROR);
        ...
        if (connection->protocol_version >= 2) {
                seqno =
                    SPC_Write_Protocol_Request(connection, NULL, RESET_TERMIOS);
                if (SPC_Waitfor_Reply(connection, NULL, seqno) == SPC_ERROR) {
                        SPC_Close_Connection(connection);
                        XeFree(canonical_hostname);
                        return (SPC_ERROR);
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (connect(connection->sid, (struct sockaddr *)&saddr,
                    sizeof(saddr)) == ERROR) {
                SPC_Error(SPC_Bad_Connect, XeFindShortHost(remote->h_name));
                return (SPC_ERROR);
        }
...
SPC_Connection_Ptr SPC_Standalone_Daemon(SPC_Connection_Ptr conn) {
...
#if defined(__aix)
        if (setsockopt(conn->sid, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) ==
            ERROR) {
#else
        ...
        while (bind(conn->sid, (struct sockaddr *)&saddr, sizeof(saddr)) ==
               ERROR) {
                if (errno == EADDRINUSE) {
        ...
        for (;;) {
                ...
                from =
                    accept(conn->sid, (struct sockaddr *)&client_saddr, &len);
                if (from == ERROR) {
                ...
                addr_ret = _XGethostbyaddr((char *)&client_saddr.sin_addr,
                                           ...
                                           client_saddr.sin_family, addr_buf);
                ...
                if (pid == ERROR) {
                        ...
                        /* We don't return here, but simply go around for a new
                         * try */
                }

                if (!pid)
                        /* We are the child.  Do whatever processing we need to
                           do on the connection & return */
