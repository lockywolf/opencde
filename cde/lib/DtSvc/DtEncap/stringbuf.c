 ...
 */
/*
 * File:         stringbuf.c $XConsortium: stringbuf.c /main/3 1995/10/26
 * 15:41:59 rswiston $ Language:     C
 *
...
#if DEBUG >= 3
        sprintf(debug_string, "expanding string buffer. current size is: %d",
                buffer->size);
...
/*------------------------------------------------------------------------+*/
int XeAppendToStringBuffer(XeStringBuffer buffer, XeString string)
/*------------------------------------------------------------------------+*/
{
...
#if DEBUG >= 5
        sprintf(debug_string, "appending: \"%s\" to: \"%s\"", string,
                buffer->buffer);
        ...
        for (i = buffer->last_char; *string; i++) {
                if (i == size) {
                        ...
                        if (-1 == ExpandStringBuffer(buffer))
                                return (-1);
                        size = buffer->size;
...
/*------------------------------------------------------------------------+*/
void XeClearStringBuffer(XeStringBuffer buffer)
/*------------------------------------------------------------------------+*/
{
...
/*------------------------------------------------------------------------+*/
XeStringBuffer XeMakeStringBuffer(int increment_size)
/*------------------------------------------------------------------------+*/
{
        XeStringBuffer new_buffer;
        increment_size = (increment_size > BUFFER_MIN_SIZE) ? increment_size
                                                            : BUFFER_MIN_SIZE;
