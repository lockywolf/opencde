...
/*------------------------------------------------------------------------+*/
XeString XeCreateContextString(XeString host, XeString directory, XeString file)
/*------------------------------------------------------------------------+*/
{
        ...
        if ((strequal(directory, (XeString) "")) || (directory == NULL)) {
                if ((strequal(file, (XeString) "")) || (file == NULL)) {
                        ...
                        context_string =
                            XeMalloc(strlen(host) + strlen(file) + 2);
                        sprintf(context_string, "%s:%s", host, file);
                }
        } else {
                if ((strequal(file, (XeString) "")) || (file == NULL)) {
                        context_string =
                            XeMalloc(strlen(host) + strlen(directory) + 2);
                        sprintf(context_string, "%s:%s", host, directory);
                } else {
                        context_string =
                            XeMalloc(strlen(host) + strlen(directory) +
                                     strlen(file) + 3);
                        sprintf(context_string, "%s:%s/%s", host, directory,
                                file);
                }
        }
        Xe_release_str(host);
...
/*------------------------------------------------------------------------+*/
XeString XeFindShortHost(XeString host_spec)
/*------------------------------------------------------------------------+*/
{
        XeString host, ptr, ptr2;
        XeChar localhost[MAXHOSTNAMELEN];

        if (!host_spec || !host_spec[0] ||
            strequal(host_spec, (XeString) "-")) {
                _DtSvcProcessLock();
                if (!context_host) {
        ...
        if (!ptr) /* short name already */
                ...
                if (ptr2 &&
                    strequal(ptr, ptr2)) { /* domains same, can eliminate */
                        host = Xe_make_ntype(ptr - host_spec + 1, XeChar);
                        strncpy(host, host_spec,
                                ptr - host_spec);     /* copy only up to "." */
...
/*------------------------------------------------------------------------+*/
XeString XeFindHost(XeString host_spec)
/*------------------------------------------------------------------------+*/
{
        if (!host_spec || !host_spec[0] ||
            strequal(host_spec, (XeString) "-")) {
...
/*------------------------------------------------------------------------+*/
{
        ...
        if (ptr)
                ...
                ptr = strstr(
                    localhost,
                    (XeString) "."); /* points to domain name if one exists */
                if (ptr) {
                        host = Xe_make_ntype(
                            strlen(host_spec) + strlen(ptr) + 1, XeChar);
...
/*------------------------------------------------------------------------+*/
int XeParseFileString(XeString line, XeString *host_addr, XeString *path_addr)
/*------------------------------------------------------------------------+*/
{
...
/*------------------------------------------------------------------------+*/
{
        ...
        _DtSvcProcessLock();
        if (domainname && ptr &&
            strcmp(domainname, ptr)) /* domains are different */
                _DtSimpleError(XeProgName, XeWarning, NULL,
                               (XeString) "><Domain configured in hostname and "
                                          "domain server are different: '%s', "
                                          "'%s'",
                               domainname, ptr);
        if (!domainname && ptr)
        ...
        if (!domainname && (firstPass)) {
                ...
                if (host_ret == NULL) {
                        _DtSimpleError(
                            XeProgName, XeWarning, NULL,
                            (XeString) "><%s not found in hosts database",
                            tmpbuf);
                        _DtSvcProcessUnlock();
                        return -1;
                }
                if ((ptr =
                         strstr(host_ret->h_name,
                                (XeString) "."))) /* if dot in canonical name */
                        domainname = strdup(ptr);
        }
...
/*------------------------------------------------------------------------+*/
int Xegetshorthostname(XeString buffer, unsigned int bufsize)
/*------------------------------------------------------------------------+*/
{
        XeString ptr;
        int status;
...
/*------------------------------------------------------------------------+*/
int Xegethostname(XeString buffer, unsigned int bufsize)
/*------------------------------------------------------------------------+*/
{
...
/*------------------------------------------------------------------------+*/
Boolean XeIsLocalHostP(XeString hostname)
/*------------------------------------------------------------------------+*/
{
        XeChar localhost[MAXHOSTNAMELEN];
...
/*------------------------------------------------------------------------+*/
Boolean XeIsSameHostP(XeString host1, XeString host2)
/*------------------------------------------------------------------------+*/
{
        ...
        if (!host1 || !host1[0] || !strcmp(host1, (XeString) "-") || !host2 ||
            !host2[0] || !strcmp(host2, (XeString) "-"))
...
static void copying_putenv(char *env) {
...
/*------------------------------------------------------------------------+*/
char *Xegetcwd(char *buf, int size)
/*------------------------------------------------------------------------+*/
{
        ...
        if ((current_dir = getenv("PWD"))) { /* use PWD instead of slow call */
                ...
                /* All this is still much faster the getcwd() esp. on UX
                 * discless. */
                s1 = stat(buf, &sb1);
                s2 = stat(".", &sb2);

                /* If device and inode are the same, we have a match */
                pwd_ok = ((s1 == 0 && s2 == 0) && (sb1.st_dev == sb2.st_dev &&
        ...
        if (!pwd_ok) {
                ...
                copying_putenv(
                    env); /* set PWD if necessary for later cache use */
                if (env)
                        XeFree(env);
        }
        return current_dir;
