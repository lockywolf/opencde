...
/*----------------------------------------------------------------------+*/
{
        ...
        if (IS_SPCIO_STDERR(iomode)) {
                if (!tmpwire || IS_SPCIO_SEPARATE(iomode)) {
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (result == 0) {
                ...
                SPC_XtRemoveInput(&channel->wires[connector]->read_toolkit_id,
                                  SPC_Input);
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (parentp) { /* Master process */
                ...
                /* Dup the file descriptors to fd's 3, 4, 5.
                   spc_dup2 is used to make sure these guys are hooked to
                   something
                   (/dev/null if necessary).  We do this step here just in case
                   any of the source file descriptors are 0, 1, or 2. */
