 ...
 */
static char *get_path_from_context(char *context);
static int remove_variable(char *string);
static void resolve_variable_reference(char **string);

Boolean _path_search(XeString path, XeString filename,
                     path_search_predicate p); /* XXX */
...
/*----------------------------------------------------------------------+*/
{
        ...
        SPC_XtAddInput(NULL, &read_terminator->termination_id,
                       read_terminator->sid, SPC_Conditional_Packet_Handler,
                       SPC_Terminator);
...
/*----------------------------------------------------------------------+*/
{
        ...
        for (tmp_argv = channel->argv; tmp_argv && *tmp_argv; tmp_argv++) {
                ...
                if ((tmplen + argtwolen) < _POSIX_ARG_MAX - 1) {
                        ...
                        if (errbuf) {
                                SPC_Free_Envp(argv);
                                sprintf(errbuf, "(%d chars), max. length is %d",
                                        tmplen, _POSIX_ARG_MAX);
                                SPC_Error(SPC_Arg_Too_Long, tmp_argv,
                                          _POSIX_ARG_MAX);
                                free(errbuf);
                        }
                        return (SPC_ERROR);
...
/*----------------------------------------------------------------------+*/
{
        ...
        while ((pid = waitpid(wait_pid, &status, WNOHANG))) {
...
#ifdef DEBUG
                fprintf(stderr,
                        (XeString) "got SIGCHLD, pid: %d, channel: %p\n", pid,
                        channel);
#endif
                ...
                if (IS_SPCIO_SYNC_TERM(channel->IOMode)) {
                        ...
                        /* This code is basically what
                           SPC_Write_Protocol_Request does. It is replicated
                           here because a call to SPC_W_P_R would have to be
                           re-enterant if we called it here, and SPC_W_P_R is
                           not re-enterant at this time. */

                        SPC_Reset_Protocol_Ptr(prot, channel, APPLICATION_DIED,
                                               0);
                        pdata->len = WRITE_APPLICATION_DIED(pdata, status);
                        length =
                            WRITE_HEADER(pdata, channel->cid,
                                         prot->request_type, pdata->len, 0);
                        pdata->data[length] = (XeChar)' ';
                        length = pdata->len + REQUEST_HEADER_LENGTH;
                        if (write(write_terminator->sid, pdata->data, length) ==
                            ERROR)
                                SPC_Error(SPC_Internal_Error);
                        pdata->offset = REQUEST_HEADER_LENGTH;
                        print_protocol_request(
                            (XeString)(
                                XeString) "  <-- INTERNAL APPLICATION_DIED",
                            prot);
                } else {
                        ...
                        if (channel->Terminate_Handler) {
                                ...
                                (*channel->Terminate_Handler)(
                                    channel, channel->pid, type, cause,
                                    channel->Terminate_Data);
                        }
                }
...
static Boolean executable_predicate(XeString path, XeString dir,
                                    XeString file) {
...
/*----------------------------------------------------------------------+*/
{
       ...
       */
        ...
        if (!_path_search(SPC_Getenv("PATH", channel->envp), channel->path,
        ...
        if ((channel->context_dir) && (channel->context_dir[0])) {
                ...
                if (!ok && IS_SPCIO_FORCE_CONTEXT(channel->IOMode)) {
                        if (dir != NULL &&
                            (strcmp(dir, channel->context_dir) != 0))
        ...
        if (SPC_pid_list == NULL)
                ...
                if (!reuse_pid)
                        SPC_pid_list = (pid_t *)realloc(
                            SPC_pid_list, (i + 2) * sizeof(pid_t));
        }
        if (!reuse_pid) {
                SPC_pid_list[i] = pid;
                SPC_pid_list[i + 1] = 0;
        ...
        if (pid) {
                ...
                /* Child process: connect wires, make environment and exec
                 * sub-process */

                sigprocmask(SIG_SETMASK, &oldsigmask, (sigset_t *)NULL);

                /* Make sure the child is the process group leader.  In the case
                   of ptys, we also want to break the current terminal
                   affiliation. We want to be the process group leader so
                   XeSPCKillProcess (which does a kill(-pid, 9)) will kill all
                   processes associated with us.

                   For PTY's, we need to break the terminal affiliation so the
                   next open (which will be a pty) will cause us to become
                   affiliated with the pty.  We do this so when the parent
                   process closes the master side of the pty, the slave side
                   processes get SIGHUP.  If they ignore SIGHUP, they will never
                   die.  So it goes...
                   */
                ...
                if (IS_SPCIO_PTY(channel->IOMode))
                        ...
                        if (setpgid(tmppid, tmppid) == -1)
                                fprintf(
                                    stderr,
                                    (XeString) "setpgid failed, errno: %d\n",
                                    errno);
                }
...
static void resolve_variable_reference(char **string) /* MODIFIED */
{
        char *string_end;
        char *var_start;
        char *pch;
         ...
         */
                ...
                while (*pch) {
                        ...
                        if (len > 0)
                                ...
                                /*
                                 * pch is null or it points to an invalid
                                 * mulit-byte character.
                                 */
                                break;
                }
#else
...
#ifdef NLS16
                        if (found_brace && (mblen(pch, MB_CUR_MAX) == 1) &&
                            (*pch == '}'))
#else
                        if (found_brace && *pch == '}')
#endif                          /* NLS16 */
                ...
                if ((value = getenv(variable)) == NULL) {
                        /*
                         * Leave what looks like an environment variable in
                         * place.
                         */
                        pbeg = pch;
                        continue;
