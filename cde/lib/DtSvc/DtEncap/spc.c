...
/* spc.c */
int SPC_Process_Single_Prot_Request(protocol_request_ptr req,
...
/*----------------------------------------------------------------------+*/
SPC_Channel_Ptr XeSPCOpen(XeString hostname, int iomode)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
int XeSPCClose(SPC_Channel_Ptr channel)
/*----------------------------------------------------------------------+*/
{
        if (channel == SPC_ERROR) {
...
/*----------------------------------------------------------------------+*/
int XeSPCReset(SPC_Channel_Ptr channel)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
int XeSPCRead(SPC_Channel_Ptr channel, int connector, /* STDOUT or STDERR */
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (channel == SPC_ERROR || !buffer ||
            !(connector == STDOUT || connector == STDERR) || (length < 0)) {
                SPC_Error(SPC_Bad_Argument);
                return (SPC_ERROR);
        }
        ...
        if (!IS_DATA(channel) ||
            !IS_SPCIO_DATA(channel->wires[connector]->flags))
                return (0);
...
/*----------------------------------------------------------------------+*/
int XeSPCWrite(SPC_Channel_Ptr channel, XeString buffer, int length)
/*----------------------------------------------------------------------+*/
{
        int n;
...
/*----------------------------------------------------------------------+*/
int XeSPCActive(SPC_Channel_Ptr channel)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
int XeSPCData(SPC_Channel_Ptr channel)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
int XeSPCExecuteProcess(SPC_Channel_Ptr channel)
/*----------------------------------------------------------------------+*/
{
        int retval;
...
/*----------------------------------------------------------------------+*/
int XeSPCSignalProcess(SPC_Channel_Ptr channel, int sig)
/*----------------------------------------------------------------------+*/
{

        if ((channel == SPC_ERROR) || (channel->pid <= 0) || (sig < 0)
...
/*----------------------------------------------------------------------+*/
int XeSPCRegisterTerminator(SPC_Channel_Ptr channel,
...
/*----------------------------------------------------------------------+*/
int XeSPCAttach(SPC_Channel_Ptr channel, int pid)
/*----------------------------------------------------------------------+*/
...
/*----------------------------------------------------------------------+*/
int XeSPCDetach(SPC_Channel_Ptr channel)
/*----------------------------------------------------------------------+*/
{
        if (channel == SPC_ERROR) {
                SPC_Error(SPC_Bad_Argument);
...
/*----------------------------------------------------------------------+*/
int XeSPCSpawn(XeString pathname, XeString context_dir, XeString *argv,
...
/*----------------------------------------------------------------------+*/
SPC_Channel_Ptr XeSPCOpenAndSpawn(XeString hostname, int iomode,
                                  XeString pathname, XeString context_dir,
                                  XeString *argv, XeString *envp)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
void XeSPCKillProcesses(int wait)
/*----------------------------------------------------------------------+*/
{
        ...
        _DtSvcProcessLock();
        for (spc = spc_activation_list; spc != (SPC_Channel_Ptr)NULL;
             spc = spc->next)
...
/*----------------------------------------------------------------------+*/
int XeSPCKillProcess(SPC_Channel_Ptr channel, int wait)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
int XeSPCInterruptProcess(SPC_Channel_Ptr channel)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
XeString XeSPCGetDevice(SPC_Channel_Ptr channel, int connector, int side)
/*----------------------------------------------------------------------+*/
{
        if (!channel) {
                SPC_Error(SPC_Bad_Argument);
...
/*----------------------------------------------------------------------+*/
int XeSPCGetProcessStatus(SPC_Channel_Ptr channel, int *type, int *cause)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
int XeSPCGetPID(SPC_Channel_Ptr channel)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
int XeSPCGetLogfile(SPC_Channel_Ptr channel, XeString *host, XeString *file)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
int XeSPCGetChannelSyncFd(SPC_Channel_Ptr channel)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
{
                ...
                for (wirelist = channel->wire_list; wirelist;
                     wirelist = wirelist->next) {
                        if (wirelist->read_toolkit_id != -1)
                                SPC_XtRemoveInput(&wirelist->read_toolkit_id,
                                                  SPC_Input);
                        if (wirelist->except_toolkit_id != -1)
                                SPC_XtRemoveInput(&wirelist->except_toolkit_id,
                                                  SPC_Exception);
                }
        ...
        while (conn) {
                if (conn->termination_id != -1)
                        SPC_XtRemoveInput(&conn->termination_id,
                                          SPC_Terminator);
                conn = conn->next;
        }
        _DtSvcProcessUnlock();
...
/*----------------------------------------------------------------------+*/
int XeSetpgrp(int read_current_termio)
/*----------------------------------------------------------------------+*/
{
