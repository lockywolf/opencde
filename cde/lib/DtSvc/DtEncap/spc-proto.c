...
static int SPC_Send_B00_Spawn(SPC_Connection_Ptr connection,
                              protocol_request_ptr prot, char *path, char *dir,
                              char **argv, char **envp);
...
/*----------------------------------------------------------------------+*/
void SPC_Reset_Protocol_Ptr(protocol_request_ptr prot, SPC_Channel_Ptr channel,
                            XeChar req, int len)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
protocol_request_ptr SPC_New_Protocol_Ptr(SPC_Channel_Ptr channel, XeChar req,
                                          int len)
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
{
        ...
        for (spc = spc_activation_list; spc; spc = spc->next)
                /* This test is here because:
                   ...
                   c. channels on the remote daemon have cid's, but a null
                   connection.
                   */
                if ((spc->cid == cid) &&
                    (!spc->connection || (spc->connection == connection))) {
                        _DtSvcProcessUnlock();
                        return (spc);
                }
...
/*----------------------------------------------------------------------+*/
{
        /* Search for an existing connection to a server, using fd (file
         descriptor) as a key */
        SPC_Connection_Ptr conn;
...
/*----------------------------------------------------------------------+*/
{
        ...
        while (numread < request_len) {
                ...
                if (numchars == 0) {
                        ...
                        XeFree(connection_hostname);
                        return (
                            SPC_ERROR); /* Bad news, EOF on incoming channel */
                }
                charptr += numchars;
                numread += numchars;
                numtoread -= numchars;
        ...
        while (numwritten < request_len) {
                ...
                if (SPC_Print_Protocol)
                        fprintf(SPC_Print_Protocol,
                                "SPC_Write_Chars -- wrote: %d of %d, expected: "
                                "%d, errno: %d\n",
...
/*----------------------------------------------------------------------+*/
{
        ...
        READ_HEADER(dptr, &channel_id, &prot->request_type, &dptr->len,
                    &prot->seqno);
        prot->channel = SPC_Lookup_Channel(channel_id, connection);

        /* JET - 11/12/2001 - correct an exploitable buffer overrun where the
         * user */
        ...
        if (dptr->len >=
            MAXREQLEN) { /* we have a problem.  Initiate DefCon 1 */
...
protocol_request_ptr SPC_Filter_Connection(SPC_Connection_Ptr connection,
                                           SPC_Channel_Ptr channel, int reqtype,
                                           int deletep)
/*----------------------------------------------------------------------+*/
{
        ...
        if (channel && (tmpqueue = channel->queued_remote_data)) {
                tmpqueue = channel->queued_remote_data;
                {
                        Xe_for_queue(protocol_request_ptr, retval, tmpqueue) {
                                /* found a queued packet.  Is it what we are
                                 * looking for? */
                                if (retval->request_type == reqtype) {
                                        /* yes. */
                                        if (deletep)
                                                Xe_delete_queue_element(
                                                    channel->queued_remote_data,
                                                    retval);
                                        return (retval);
                                }
                        }
        ...
        while (TRUE) {
                ...
                if ((conn_channel == channel) && (protreqtype == reqtype)) {
                        /* We found one that matches.  Check if we need to queue
                         * it up. */
                        if (!deletep)
                                Xe_push_queue(conn_channel->queued_remote_data,
                                              retval);
                        return (retval);
                }
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (channel->connection &&
            (tmpqueue = channel->connection->queued_remote_data)) {
...
/*----------------------------------------------------------------------+*/
int SPC_Read_Remote_Data(SPC_Channel_Ptr channel, int connector,
                         XeString client_buffer, int nbytes)
/*----------------------------------------------------------------------+*/
{
        int ret_len;
...
/*----------------------------------------------------------------------+*/
{
        ...
        fprintf(SPC_Print_Protocol,
                ...
                proto->seqno, dptr->data + dptr->offset);
...
int SPC_Write_Protocol_Request(SPC_Connection_Ptr connection,
                               SPC_Channel_Ptr channel, int request, ...)
/*----------------------------------------------------------------------+*/
{
        va_list ap;
        ...
        prot_request->channel =
            (SPC_Channel_Ptr)(intptr_t)(channel ? channel->cid : 0);
        ...
        case REGISTER: {
                ...
                va_end(ap);
                pdata->len = WRITE_REGISTER(pdata, username, passwd, proto_ver,
                                            hostinfo);
        ...
        case APPLICATION_SPAWN: {
                ...
                va_end(ap);
                pdata->len =
                    WRITE_APPLICATION_SPAWN(pdata, path, dir, argv, envp);
        ...
        case LOGFILE_REPLY: {
                ...
                va_end(ap);
                pdata->len =
                    WRITE_LOGFILE_REPLY(pdata, fname, proto_ver, hostinfo);
        ...
        case CHANNEL_SEND_EOF:
                ...
                if (connection->protocol_version < 3) {
                        SPC_Error(SPC_Protocol_Version_Error, 3,
                                  channel->connection->protocol_version);
                        return (SPC_ERROR);
                }
        ...
        case CHANNEL_TERMIOS:
...

                {
                        ...
                        pdata->len =
                            WRITE_TERMIOS(pdata, connector, side, buffer);
        ...
        case APP_B00_SPAWN:
...

                {
                        ...
                        /* It is left as an exercise to the reader to figure out
                           what would happen if we didn't use these temp.
                           variables and instead used the va_arg macros directly
                           in the WRITE_APPLICATION_SPAWN... */
                        ...
                        retval = SPC_Send_B00_Spawn(connection, prot_request,
                                                    path, dir, argv, envp);
                        return (retval);
                }
        }
...
/*----------------------------------------------------------------------+*/
int SPC_Write_Single_Prot_Request(SPC_Connection_Ptr connection, XeString name,
                                  protocol_request_ptr prot)
/*----------------------------------------------------------------------+*/
{
        ...
        reply_expected = REPLY_EXPECTED(prot->request_type, prot->seqno);
        length = WRITE_HEADER(pdata, prot->channel, prot->request_type,
                              pdata->len, prot->seqno);
...
/*----------------------------------------------------------------------+*/
int SPC_Waitfor_Reply(SPC_Connection_Ptr connection, SPC_Channel_Ptr channel,
                      int seqno)
/*----------------------------------------------------------------------+*/
{
        ...
        if (retval < 0) {
                ...
                if (retval != SPC_Protocol_Abort) {
                        if (retval == SPC_Cannot_Create_Netfilename)
                                SPC_Error(retval, channel->context_dir,
                                          connection->hostname);
...
/*----------------------------------------------------------------------+*/
int SPC_Write_Reply(SPC_Connection_Ptr conn, protocol_request_ptr proto,
                    int retval, int errval)
/*----------------------------------------------------------------------+*/
{
        _DtSvcProcessLock();
        if (retval == ERROR)
...
int SPC_Send_Multi_Packet(SPC_Connection_Ptr connection,
                          protocol_request_ptr prot, char **str_vect,
                          int num_str, int req, XeString name, int errid) {
        int counter, tmp_len;
        ...
        for (counter = 0; counter < num_str; counter++) {
                ...
                if (tmp_len > SPC_BUFSIZ - 1) {
                        SPC_Error(errid, this_str, SPC_BUFSIZ - 1);
                        SPC_Free_Protocol_Ptr(prot);
                        if (this_seqno != current_sequence_number)
                                SPC_Write_Protocol_Request(connection, NULL,
                                                           ABORT);
                        _DtSvcProcessUnlock();
                        return (reply_seqno);
                }
        ...
        if (numbytes) {
                *buf = '\0';
                prot->dataptr->len = numbytes + 1;
                SPC_Write_Single_Prot_Request(
                    connection, (XeString) "  <-- ENVIRON_RESET", prot);
                SPC_Free_Protocol_Ptr(prot);
        }
...
char **SPC_Get_Multi_Packet(SPC_Connection_Ptr connection,
                            protocol_request_ptr prot, char **out, int *outlen,
                            int request, XeString name)

{
        ...
        for (i = 0; i < num_vars; i++) {
                ...
                if (len == 0) {
                        ...
                        prot = SPC_Filter_Connection(connection, NULL, request,
                                                     TRUE);
                        if (prot == SPC_ERROR)
                                return (SPC_ERROR);
                        print_protocol_request(name, prot);
...
/*----------------------------------------------------------------------+*/
int sprint_counted_string(XeString buf, int count, XeString *vect,
                          int orig_limit)
/*----------------------------------------------------------------------+*/
{
        XeString bufptr = buf;
...
#define ERRBUFLEN 100
        ...
        if (len > limit) {
                sprintf(errbuf, "(%d chars), max. length is %d", len,
                        orig_limit);
                SPC_Error(SPC_Arg_Too_Long, bufptr, errbuf);
                return (SPC_ERROR);
        }
        bufptr += len;
        limit -= len;
        ...
        for (i = 0; i < count; (i++, vect++)) {
                ...
                if (len > limit) {
                        sprintf(errbuf, "(%d chars), max. length is %d", len,
                                orig_limit);
                        SPC_Error(SPC_Arg_Too_Long, *vect, errbuf);
                        return (SPC_ERROR);
                }
...
/*----------------------------------------------------------------------+*/
{
        ...
        for (i = 0; i < numstrings; (i++, tmpidx++)) {
                len = strlen(bufptr) +
                      1; /* len is string SIZE (with room for NULL) */
...
/*----------------------------------------------------------------------+*/
int sprint_application_data(XeString buf, XeString UNUSED_PARM(fmt),
                            XeString path, XeString dir, XeString *argv,
                            XeString *envp, int UNUSED_PARM(chars_used))
/*----------------------------------------------------------------------+*/
{
...
/*----------------------------------------------------------------------+*/
int sscan_application_data(XeString buf, XeString UNUSED_PARM(fmt),
                           XeString *path, XeString *dir, XeString **argv,
                           XeString **envp, int UNUSED_PARM(offset))
/*----------------------------------------------------------------------+*/
{
        XeString bufptr;
        XeString *tmp_vect;
...
/*----------------------------------------------------------------------+*/
int sprint_device_data(XeString buf, XeString m0, XeString s0, XeString m1,
                       XeString s1, XeString m2, XeString s2)
/*----------------------------------------------------------------------+*/
{
        XeString args[6];
        int i;
...
/*----------------------------------------------------------------------+*/
int sscan_device_data(XeString buf, XeString *m0, XeString *s0, XeString *m1,
                      XeString *s1, XeString *m2, XeString *s2)
/*----------------------------------------------------------------------+*/
{
        XeString *args;
...
/*----------------------------------------------------------------------+*/
int sprint_logfile_data(XeString buf, XeString logfile, XeString proto_ver,
                        XeString hostinfo)
/*----------------------------------------------------------------------+*/
{
        int i;
...
/*----------------------------------------------------------------------+*/
int sscan_logfile_data(XeString buf, XeString *logfile, XeString *proto_ver,
                       XeString *hostinfo)
/*----------------------------------------------------------------------+*/
{
        XeString *args;
        ...
        *logfile = args[0];
        /* args[1] and args[2] will only be around for protocol revision 2 or
         * later */
...
/*----------------------------------------------------------------------+*/
int sprint_register_data(XeString buf, XeString username, XeString passwd,
                         XeString proto_ver, XeString hostinfo)
/*----------------------------------------------------------------------+*/
{
        int i;
        XeString args[4];
...
/*----------------------------------------------------------------------+*/
int sscan_register_data(XeString buf, XeString *username, XeString *passwd,
                        XeString *proto_ver, XeString *hostinfo)
/*----------------------------------------------------------------------+*/
{
        XeString *args;
        ...
        *passwd = args[1];
        ...
        /* args[2] and args[3] will only be around for protocol revision 2 or
         * later */
...
/*----------------------------------------------------------------------+*/
{
        ...
        READ_DEVICE_REPLY(prot->dataptr, &(channel->wires[STDIN]->master_name),
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (!s) {
                if (uname(&name) >= 0) {
                        s_len = strlen(name.sysname) + strlen(name.nodename) +
                                strlen(name.release) + strlen(name.machine) + 4;
                        s = (XeString)XeMalloc(s_len * sizeof(XeChar));
                        sprintf(s, "%s:%s:%s:%s", name.nodename, name.sysname,
                                name.release, name.machine);
                } else {
                        s_len = 4 * strlen(UNK_TOKEN) + 4;
                        s = (XeString)XeMalloc(s_len * sizeof(XeChar));
                        sprintf(s, "%s:%s:%s:%s", UNK_TOKEN, UNK_TOKEN,
                                UNK_TOKEN, UNK_TOKEN);
                }
        }
...
/* Called by client to register itself to spcd */
{
        ...
        if ((pwd_ret = _XGetpwuid(this_uid, pwd_buf)) == NULL) {
                 ...
                 */
                username = XeString_NULL; /* we'll use the original /tmp
                                             subdirectory */
        } else {
                username = (XeString)(pwd_ret->pw_name);
        }

        SPC_Write_Protocol_Request(connection, NULL, REGISTER, username,
                                   XeString_Empty, SPC_PROTOCOL_VERSION_STR,
                                   hostinfo);
        ...
        /* In repsonse to the register, the daemon will send back a
         * LOGFILE_REPLY */
        ...
        if (tt_ptr_error(path) != TT_OK) {
                ...
                SPC_Error(SPC_Register_Netrc, logfile, connection_hostname);
                XeFree(connection_hostname);
                if (logfile)
                        XeFree(logfile);
                return (SPC_ERROR);
        ...
        if (open_status == ERROR) {
                SPC_Write_Protocol_Request(connection, NULL, ABORT);
                SPC_Error(SPC_Register_Open, path, connection_hostname);
                tt_free(path);
                XeFree(connection_hostname);
                if (logfile)
                        XeFree(logfile);
        ...
        SPC_Write_Protocol_Request(connection, NULL, REGISTER, logfile, NULL,
                                   NULL, NULL);
        prot = SPC_Filter_Connection(connection, NULL, LOGFILE_REPLY, TRUE);
        ...
        if (!strcmp(logfile, FAILED_FILE_NAME)) {
                SPC_Error(SPC_Register_Handshake, username,
                          connection_hostname);
                XeFree(connection_hostname);
                if (logfile)
                        XeFree(logfile);
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (XeTermioStruct == NULL) {
                XeTermioStruct =
                    (struct termios *)XeMalloc(sizeof(struct termios));
...
static int SPC_Send_B00_Spawn(SPC_Connection_Ptr connection,
        ...
        retval = SPC_Send_Multi_Packet(connection, prot, merged_ptr, num_elts,
