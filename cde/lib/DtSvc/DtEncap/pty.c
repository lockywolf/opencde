                ...
                *(mptr) = *(sptr) = MASTER_NAMES[c];
                ...
                for (d = 0; d < 15; d++) { /* 2nd char:  0..9,a..f */
                        ...
                        *(mptr + 1) = hexdigits[d];
                        ...
                        if ((wire->fd[MASTER_SIDE] = open(master, O_RDWR)) <
                            OK) {
                        ...
                        *(sptr + 1) = hexdigits[d];
                        ...
                        /* check that we can eventually open the slave side,
                           using the access system call */
                        if (access(slave, R_OK | W_OK) == ERROR) {
                                /* cannot access the slave pty.  Close master
                                 * and try next one */
                                close(wire->fd[MASTER_SIDE]);
                                continue;
                        }
...
/*----------------------------------------------------------------------+*/
{
...
#endif
        {
                ...
                if (!getptypair(wire)) {
                        ...
                        if (!getptypair(wire)) {
                                /* No available pty's ?? */
                                wire->master_name[0] = wire->slave_name[0] =
                                    (XeChar)'\0';
                                wire->fd[MASTER_SIDE] = wire->fd[SLAVE_SIDE] =
                                    -1;
...
static void init_termio(struct termios *term_state) {
        ...
        /* Basically, we want the pty channel to act like like a pipe
           ...
           echo the data, and reads will return when there is at least one
           character in the buffer to be read. */
...
#ifdef __hpux_pty
{
        ...
        call_parent_method(channel, read, (channel, connector, buffer, nbytes),
                           result);
        ...
        do
                select_value =
                    select(fd + 1, &read_mask, NULL, &except_mask, timeptr);
        ...
        if (select_value != 0) {
                ...
                /* Clear the request (Not really necessary in the case of a
                   close, but do it anyway) */
        ...
        if (numbytes == 0)
                do {
                        ...
                        if ((result == -1) && errno == ESRCH) {
                                SPC_XtRemoveInput(
                                    &channel->wires[connector]->read_toolkit_id,
                                    SPC_Input);
                                SPC_Change_State(channel, connector, 0, -1);
                                return (0);
                        }
                        ...
                        /*
                         **
                         ** This call to select doesn't have the cast to (int*),
                         *because
...
/*----------------------------------------------------------------------+*/
{
        ...
        for (wirelist = channel->wire_list; wirelist;
             wirelist = wirelist->next) {
                if (master_pty(wirelist->fd[MASTER_SIDE],
                               &wirelist->master_termio) == SPC_ERROR)
                        result = SPC_ERROR;
        }
        ...
        /* I am not particularly enamored of this macro.  However, the style of
           the SCANBITS macro kinda forces me to write it this way.  In
           particular, I am a bit worried about the reference to except_mask,
           which is a "nonlocal reference" */

#define clear_trap(fd)                                                         \
        {                                                                      \
                struct request_info req_info;                                  \
                int my_fd = (fd);                                              \
        ...
        if (parentp) { /* Master process */
#ifdef __hpux_pty
                {
                        ...
                        while (result) {
                                ...
                                select_value = select(max_fds, NULL, NULL,
                                                      &temp_mask, NULL);
                ...
                if (IS_SPCIO_STDIN(iomode)) {
                        if ((stdinfd = open(channel->wires[STDIN]->slave_name,
                                            O_RDWR)) < 0) {
...
#ifndef __hpux_pty
                /* The pty trapping stuff handles EOF for us.  Use the "sync"
                 * pipe */
                ...
                /* Duplicate these file descriptors to 3, 4, 5 so we don't have
                   to worry about any of std[in|out|err]fd being 0, 1, or 2. */
                ...
                if (IS_SPCIO_STDIN(iomode))
                        set_pty_state(STDIN,
                                      &(channel->wires[STDIN]->slave_termio));
                if (IS_SPCIO_STDOUT(iomode))
                        set_pty_state(STDOUT,
                                      &(channel->wires[STDOUT]->slave_termio));
                if (IS_SPCIO_STDERR(iomode))
                        set_pty_state(STDERR,
                                      &(channel->wires[STDERR]->slave_termio));
...
/*----------------------------------------------------------------------+*/
{
        ...
        call_parent_method(channel, add_input, (channel, handler, data),
                           result);
        ...
        for (wirelist = channel->wire_list; wirelist;
             wirelist = wirelist->next) {
                ...
                SPC_XtAddInput(channel, &wirelist->read_toolkit_id, fd,
                               channel->class_ptr->input, SPC_Input);
#ifdef __hpux_pty
                SPC_XtAddInput(channel, &wirelist->except_toolkit_id, fd,
                               channel->class_ptr->input, SPC_Exception);
#endif /* __hpux_pty */
        }
...
/*----------------------------------------------------------------------+*/
{
        ...
        if ((setpgrp_wire->fd[SLAVE_SIDE] =
                 open(setpgrp_wire->slave_name, O_RDWR)) < 0) {
                _DtSvcProcessUnlock();
                return (SPC_ERROR);
        }

#if !defined(USL) && !defined(__uxp__)
        if (tcsetattr(setpgrp_wire->fd[SLAVE_SIDE], TCSANOW, XeTermioStruct) ==
            ERROR) {
                SPC_Error(SPC_Bad_tc_Call, (XeString) "tcsetattr");
                _DtSvcProcessUnlock();
                return (SPC_ERROR);
...
int set_termio_pty_channel_object(SPC_Channel_Ptr channel, int connection,
                                  int side, struct termios *termio) {
