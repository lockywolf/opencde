...
static int is_dir_usable(char *dir);
...
static char *get_tmp_dir(void) {
        ...
        dir = malloc(strlen(pwd_ret->pw_dir) + strlen(SPCD_ENV_HOME_DIRECTORY) +
                     8);

        if (!dir)
                return (NULL);

        (void)sprintf(dir, "%s/%s/tmp", pwd_ret->pw_dir,
                      SPCD_ENV_HOME_DIRECTORY);
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (IS_SPCIO_USE_LOGFILE(iomode)) {
                ...
                _DtSvcProcessLock();
                /* Storage from tempnam() freed in
                 * remove_logfile_local_channel_object */
                if (SPCD_Authentication_Dir != NULL)
                        channel->logfile =
                            tempnam(SPCD_Authentication_Dir, "SPC");
                else {
                ...
                if (SPC_logfile_list == NULL)
                        ...
                        for (i = 0; SPC_logfile_list[i] != NULL; i++)
                                ;
                        SPC_logfile_list = (char **)realloc(
                            SPC_logfile_list, (i + 2) * sizeof(char *));
                }
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (parentp) { /* Master process */
                ...
                if (IS_SPCIO_USE_LOGFILE(channel->IOMode))
                        if ((fd = open(channel->logfile, O_WRONLY | O_CREAT,
                                       0666)) == ERROR) {
                                SPC_Error(SPC_Cannot_Open_Log,
                                          channel->logfile);
                                return (SPC_ERROR);
                        }
                spc_dup2(fd, STDOUT);
