 ...
 */
/*
 * File:         spc-util.c $XConsortium: spc-util.c /main/5 1996/06/21 17:33:16
 * ageorge $ Language:     C
 *
...
/*----------------------------------------------------------------------+*/
{
        /* Attempt to return a channel which currently handles process number
         * PID */
        SPC_Channel_Ptr spc;
...
/*----------------------------------------------------------------------+*/
void SPC_Conditional_Packet_Handler(void *UNUSED_PARM(client_data), int *source,
...
/*----------------------------------------------------------------------+*/
{
        ...
        int filename_len;
        int component_seperator =
            (int)':'; /* this is here because strchr takes an in */
            ...
            next_colon = strchr(path_rest, component_seperator));
             ...
             next_colon = strchr(path_rest, component_seperator)) {
                ...
                if (next_colon) { /* found colon */
                        ...
                        if (!*path_rest)
                                /* We've seen a ':' at the end of the string.
                                   Make path_rest be "." next go-round */
                                path_rest = ".";
                } else { /* no colon */
                ...
                if ((path_component_len + filename_len + 1) < MAXPATHLEN) {
                        ...
                        /*
                         ** Check this file.  If the predicate returns true, we
                         *return true.
                         */
