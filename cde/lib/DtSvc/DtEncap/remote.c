 ...
 */
/*
 * File:         remote.c $XConsortium: remote.c /main/5 1996/06/21 17:34:53
 * ageorge $ Language:     C
 *
                ...
                if ((tmpqueue = channel->queued_remote_data)) {
                        Xe_for_queue(protocol_request_ptr, prot, tmpqueue) {
                                Xe_delete_queue_element(
                                    channel->queued_remote_data, prot);
...
/*----------------------------------------------------------------------+*/
{
        ...
        if ((channel->connection->protocol_version > 2) ||
            IS_SPCIO_FORCE_CONTEXT(channel->IOMode))
                seqno = SPC_Write_Protocol_Request(
                    channel->connection, channel, APP_B00_SPAWN, channel->path,
                    channel->context_dir, channel->argv, channel->envp);
        else
                seqno = SPC_Write_Protocol_Request(
                    channel->connection, channel, APPLICATION_SPAWN,
                    channel->path, channel->context_dir, channel->argv,
                    channel->envp);
...
/*----------------------------------------------------------------------+*/
{
        ...
        call_parent_method(channel, read, (channel, connection, buffer, len),
                           result);
...
int set_termio_remote_channel_object(SPC_Channel_Ptr channel, int connector,
                                     int side, struct termios *termios) {
        int retval, seqno;
        ...
        retval = SPC_Write_Protocol_Request(channel->connection, channel,
                                            CHANNEL_TERMIOS, connector, side,
                                            termios);
        seqno = SPC_Waitfor_Reply(channel->connection, channel, retval);
