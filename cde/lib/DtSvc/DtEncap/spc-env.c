...
/*----------------------------------------------------------------------+*/
{
        ...
        while (fgets(fbuffer, BUFSIZ, f)) {
                ...
                if (n == XeChar_NULL || fbuffer[0] == Pad ||
                    fbuffer[0] == Pound)
                        continue;
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (old_envp)
                ...
                envp =
                    (XeString *)XeMalloc(sizeof(XeString) * DEFAULT_ENVP_SIZE);
                envp[0] = XeChar_NULL;
        }

        if (!(envVar = getenv("DISPLAY"))) {
                display = (XeChar *)malloc((strlen(official_hostname) + 11) *
                                           sizeof(XeChar));
                if (display != (XeChar *)NULL)
                        sprintf(display, "DISPLAY=%s:0", official_hostname);
        } else {
                display =
                    (XeChar *)malloc((strlen(envVar) + 9) * sizeof(XeChar));
                if (display != (XeChar *)NULL)
        ...
        if ((envVar = getenv("LANG")) != (XeString)NULL) {
                ...
                if ((langBuf = (XeChar *)malloc((strlen(envVar) + 6) *
                                                sizeof(XeChar))) !=
                    (XeChar *)NULL) {
        ...
        (void)sprintf(sys_env_path, "%s/%s", SPCD_ENV_INSTALL_DIRECTORY,
                      SPCD_ENV_FILE);
         ...
         */
        sys_env_path =
            (XeString)realloc(sys_env_path, strlen(SPCD_ENV_CONFIG_DIRECTORY) +
                                                strlen(SPCD_ENV_FILE) + 3);
        (void)sprintf(sys_env_path, "%s/%s", SPCD_ENV_CONFIG_DIRECTORY,
                      SPCD_ENV_FILE);
        envp = SPC_Add_Env_File(sys_env_path, envp);
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (IS_REMOTE(channel)) {
                ...
                /* I don't particularly like hard coding these values here.
                   I have been bitten before by doing so, and probably will
                   again... */
                if (!strncmp(disp, (XeString) "unix", 4) ||
                    ...
                    !strncmp(disp, (XeString) ":", 1)) {
                        ...
                        dispBuf = (XeChar *)malloc(
                            MAXHOSTNAMELEN + sizeof((XeString) "DISPLAY=:0.0") +
                            1);
                        if (dispBuf) {
                ...
                if (myLang) {
                        ...
                        if (_DtLcxOpenAllDbs(&db) == 0) {
                                if ((_DtXlateGetXlateEnv(db, platform, &execVer,
                                                         &compVer) == 0) &&
                                    (_DtLcxXlateOpToStd(db, platform, compVer,
                                        ...
                                        if ((langBuf = (XeChar *)malloc(
                                                 ...
                                                 sizeof(XeChar))) !=
                                            (XeChar *)NULL) {
                                                sprintf(langBuf, "LANG=%s",
                                                        stdLang);
                                                envp =
                                                    SPC_Putenv(langBuf, envp);
                                                free(langBuf);
                                        }
