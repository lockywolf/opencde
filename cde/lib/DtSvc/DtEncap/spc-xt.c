...
/*-----------------------------------------------------------------------+*/
{
        ...
        if (SPC_Fd_Mapping == NULL) {
                SPC_Fd_Mapping = (SPC_Callback_Struct **)XeMalloc(
                    FD_SETSIZE * sizeof(SPC_Callback_Struct *));
                memset(SPC_Fd_Mapping, 0,
                       FD_SETSIZE * sizeof(SPC_Callback_Struct *));
        }
        structptr = SPC_LOOKUP_FD_MAPPING(source);

        if (!structptr) {
                structptr = (SPC_Callback_Struct *)XeMalloc(
                    sizeof(SPC_Callback_Struct));
        ...
        if ((*id_addr) != -1) {
                ...
                case SPC_Client:
                        ...
                        if (SbRemoveInput_hookfn == NULL)
                                (void)fprintf(stderr,
                                              "Error: SbRemoveInput = NULL\n");
                        else
                                (*SbRemoveInput_hookfn)(
                                    SPC_RemoveInput(*id_addr, condition));
                        break;
                ...
                case SPC_Exception:
                        ...
                        if (SbRemoveException_hookfn == NULL)
                                (void)fprintf(
                                    stderr,
                                    "Error: SbRemoveException = NULL\n");
                        else
                                (*SbRemoveException_hookfn)(
                                    SPC_RemoveInput(*id_addr, condition));
                        break;
                }
