...
/*----------------------------------------------------------------------+*/
{
        ...
        if (SPC_who_am_i == SPC_I_AM_A_DAEMON) {
                ...
                if (sigaction(SIGCHLD, &svect, (struct sigaction *)NULL) ==
                    ERROR) {
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (home = getenv("HOME")) {
                spc_user_environment_file = (XeString)XeMalloc(
                    strlen(home) + strlen(SPCD_ENV_HOME_DIRECTORY) +
                    strlen(SPCD_ENV_FILE) + 3);
                sprintf(spc_user_environment_file, "%s/%s/%s", home,
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (!SPC_Local_Hostname(hostname)) {
                channel = (SPC_Channel_Ptr)object_create(
                    (object_clasp)remote_channel_class);
                ...
                if (IS_SPCIO_NOIOMODE(iomode))
                        channel = (SPC_Channel_Ptr)object_create(
                            (object_clasp)noio_channel_class);

                if (IS_SPCIO_PIPE(iomode))
                        channel = (SPC_Channel_Ptr)object_create(
                            (object_clasp)pipe_channel_class);

                if (IS_SPCIO_PTY(iomode))
                        channel = (SPC_Channel_Ptr)object_create(
                            (object_clasp)pty_channel_class);
        }
...
/*----------------------------------------------------------------------+*/
{
        ...
        if (channel->Terminate_Handler) {
                ...
                (*channel->Terminate_Handler)(channel, channel->pid, type,
                                              cause, channel->Terminate_Data);
        }
...
/*----------------------------------------------------------------------+*/
{
        ...
        do {
                ...
                while (nchars < ntoread && nchars_this_buffer < scalarlen &&
                       !nlcopied) {
                ...
                cbuf->offset = 0;
                do {
                        cbuf->len = mempf3(channel, read, connector, cbufptr,
                                           SPC_BUFSIZ);
...
/*----------------------------------------------------------------------+*/
{
        ...
        do {
                ...
                nchars = (*channel->read_filter)(channel, connector,
                                                 spc_iobuffer, SPC_BUFSIZ);

                /* Check nchars.  If it is EXCEPT_FLAG, we had a special
                   occurance (such as an ioctl on a PTY).  In any case, don't do
                   any more processing */
                ...
                if (channel->Input_Handler)
                        (*channel->Input_Handler)(channel->client_data,
                                                  spc_iobuffer, nchars,
                                                  connector);
...
int read_channel_object(SPC_Channel_Ptr UNUSED_PARM(channel),
                        ...
                        XeString UNUSED_PARM(buffer), int UNUSED_PARM(nbytes))
/*----------------------------------------------------------------------+*/
...
/*----------------------------------------------------------------------+*/
int signal_channel_object(SPC_Channel_Ptr UNUSED_PARM(channel),
                          int UNUSED_PARM(sig))
/*----------------------------------------------------------------------+*/
{
        return (TRUE);
...
/*----------------------------------------------------------------------+*/
int attach_channel_object(SPC_Channel_Ptr UNUSED_PARM(channel),
                          int UNUSED_PARM(pid))
/*----------------------------------------------------------------------+*/
{
        return (TRUE);
...
/*----------------------------------------------------------------------+*/
int add_input_channel_object(SPC_Channel_Ptr UNUSED_PARM(channel),
